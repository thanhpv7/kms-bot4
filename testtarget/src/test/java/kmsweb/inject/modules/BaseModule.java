 ...
 */
package kmsweb.inject.modules;

import kmsweb.cucumber.suites.TestSuite;
...
public class BaseModule extends AbstractModule {
	...
	public Properties properties() {
		...
		properties.putIfAbsent("screenHeight", "900");
		properties.putIfAbsent("screenWidth", "1440");
	...
	public WebDriver webDriver(
		...
		if (properties.getOrDefault("isLocal", "true").equals("true")) {
			...
			// % protected region % [Customise remote driver configuration here] off begin
			...
			// % protected region % [Customise remote driver configuration here] end
		}
		...
		switch (TestSuite.getBrowser().get().toLowerCase()) {
			...
			case "chrome-edge":
				...
				if (Boolean.parseBoolean((String)properties.get("webdriver.chrome.headless"))) {
					...
					chromeOptions.addArguments("--window-size=" + properties.get("screenWidth") + "," 
							+ properties.get("screenHeight"));
				}
