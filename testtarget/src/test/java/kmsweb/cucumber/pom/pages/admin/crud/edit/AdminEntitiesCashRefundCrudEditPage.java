/*
 * @bot-written
 *
 * WARNING AND NOTICE
 * Any access, download, storage, and/or use of this source code is subject to the terms and conditions of the
 * Full Software Licence as accepted by you before being granted access to this source code and other materials,
 * the terms of which can be accessed on the Codebots website at https://codebots.com/full-software-licence. Any
 * commercial use in contravention of the terms of the Full Software Licence may be pursued by Codebots through
 * licence termination and further legal action, and be required to indemnify Codebots for any loss or damage,
 * including interest and costs. You are deemed to have accepted the terms of the Full Software Licence on any
 * access, download, storage, and/or use of this source code.
 *
 * BOT WARNING
 * This file is bot-written.
 * Any changes out side of "protected regions" will be lost next time the bot makes any changes.
 */
package kmsweb.cucumber.pom.pages.admin.crud.edit;

import com.google.inject.Inject;
import kmsweb.cucumber.utils.*;
import kmsweb.entities.AbstractEntity;
import kmsweb.entities.CashRefundEntity;

import cucumber.runtime.java.guice.ScenarioScoped;
import lombok.Getter;
import lombok.extern.slf4j.Slf4j;
import lombok.NonNull;
import org.openqa.selenium.By;
import org.openqa.selenium.Keys;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.support.FindBy;
import org.openqa.selenium.support.How;
import org.openqa.selenium.interactions.Actions;
import java.util.*;

// % protected region % [Add any additional imports here] off begin
// % protected region % [Add any additional imports here] end

/**
 * Cash RefundPage is a Page POM that is associated with the admin/entities/cash-refund url.
 *
 */
@Slf4j
@ScenarioScoped
public class AdminEntitiesCashRefundCrudEditPage extends CrudEdit {

	// % protected region % [Add any additional fields here] off begin
	// % protected region % [Add any additional fields here] end


	@FindBy(how = How.XPATH, using = "//*[@id='refundNumber-field']")
	private WebElement refundNumberField;
	// TODO default handling dataAttribute refundDateField;
	@FindBy(how = How.XPATH, using = "//*[@id='refundDate-field']")
	private WebElement refundDateField;
	@FindBy(how = How.XPATH, using = "//*[@id='description-field']")
	private WebElement descriptionField;
	@FindBy(how = How.XPATH, using = "//*[@id='cashierLocation-field']")
	private WebElement cashierLocationField;
	@FindBy(how = How.XPATH, using = "//*[@id='isvoid-field']")
	private WebElement isvoidField;
	// TODO default handling dataAttribute voidDateTimeField;
	@FindBy(how = How.XPATH, using = "//*[@id='voidDateTime-field']")
	private WebElement voidDateTimeField;
	@FindBy(how = How.XPATH, using = "//*[@id='paymentType-field']")
	private WebElement paymentTypeField;
	// TODO default handling dataAttribute refundAmountField;
	@FindBy(how = How.XPATH, using = "//*[@id='refundAmount-field']")
	private WebElement refundAmountField;
	@FindBy(how = How.XPATH, using = "//*[@id='hospitalBankAccount-field']")
	private WebElement hospitalBankAccountField;
	@FindBy(how = How.XPATH, using = "//*[@id='referenceNumber-field']")
	private WebElement referenceNumberField;
	

	// Outgoing one-to-one

	// Incoming one-to-one

	// Outgoing one-to-many

	// Incoming one-to-many
	@FindBy(how = How.XPATH, using = "//ng-select[@id='patientGeneralInfoId-field']")
	private WebElement patientGeneralInfoField;

	// Outgoing many-to-many

	// Incoming many-to-many

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	@Inject
	public AdminEntitiesCashRefundCrudEditPage(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			@NonNull WebDriver webDriver,
			@NonNull Properties properties
	) {
		super(
			// % protected region % [Add any additional constructor arguments here] off begin
			// % protected region % [Add any additional constructor arguments here] end
			webDriver,
			properties,
			"admin/entities/cash-refund/create"
		);

		// % protected region % [Add any additional constructor logic here] off begin
		// % protected region % [Add any additional constructor logic here] end

		log.trace("Initialised {}", this.getClass().getSimpleName());
	}

	@Override
	protected void fillInEntityInformation(AbstractEntity abstractEntity)
	{
		var entity = (CashRefundEntity) abstractEntity;
		refundNumberField.sendKeys(entity.getRefundNumber());
		// TODO default handling for dataAttribute refundDateField
		refundDateField.sendKeys(entity.getRefundDate().toString());

		descriptionField.sendKeys(entity.getDescription());
		cashierLocationField.sendKeys(entity.getCashierLocation());
		if (entity.getIsvoid()) {
			isvoidField.click();
		}
		// TODO default handling for dataAttribute voidDateTimeField
		voidDateTimeField.sendKeys(entity.getVoidDateTime().toString());

		paymentTypeField.sendKeys(entity.getPaymentType());
		// TODO default handling for dataAttribute refundAmountField
		refundAmountField.sendKeys(entity.getRefundAmount().toString());

		hospitalBankAccountField.sendKeys(entity.getHospitalBankAccount());
		referenceNumberField.sendKeys(entity.getReferenceNumber());

		patientGeneralInfoField.click();
		(new Actions(webDriver)).sendKeys(Keys.ARROW_DOWN, Keys.ENTER).build().perform();
		saveButton.click();
	}
	
	// % protected region % [Add any additional methods here] off begin
	// % protected region % [Add any additional methods here] end
}