...
@Slf4j
@ScenarioScoped
public class AdminEntitiesRegistrationCrudEditPage extends CrudEdit {
	...
	// TODO default handling dataAttribute scheduledDateField;
	@FindBy(how = How.XPATH, using = "//*[@id='scheduledDate-field']")
	private WebElement scheduledDateField;
	// TODO default handling dataAttribute actualInDateField;
	...
	// TODO default handling dataAttribute queueNoField;
	...
	// TODO default handling dataAttribute supportingDocumentField;
	...
	@Inject
	public AdminEntitiesRegistrationCrudEditPage(
		...
		super(
			...
			webDriver,
			properties,
			"admin/entities/registration/create"
		);
	...
	{
		var entity = (RegistrationEntity) abstractEntity;
		registrationNumberField.sendKeys(entity.getRegistrationNumber());
		// TODO default handling for dataAttribute scheduledDateField
		scheduledDateField.sendKeys(entity.getScheduledDate().toString());

		// TODO default handling for dataAttribute actualInDateField
		actualInDateField.sendKeys(entity.getActualInDate().toString());
		...
		// TODO default handling for dataAttribute queueNoField
		queueNoField.sendKeys(entity.getQueueNo().toString());
		...
		bookedTreatmentClassField.sendKeys(entity.getBookedTreatmentClass());
		shiftField.sendKeys(entity.getShift());
		if (entity.getCito()) {
			citoField.click();
		}
		if (entity.getOneDayCare()) {
			oneDayCareField.click();
		}
		if (entity.getReferral()) {
			referralField.click();
		}
		registrationStatusField.sendKeys(entity.getRegistrationStatus());
		referralTypeField.sendKeys(entity.getReferralType());
		purposeField.sendKeys(entity.getPurpose());
		referralNotesField.sendKeys(entity.getReferralNotes());
		// TODO default handling for dataAttribute supportingDocumentField
		supportingDocumentField.sendKeys(entity.getSupportingDocument().toString());

		admissionDiagnosisField.sendKeys(entity.getAdmissionDiagnosis());
		// TODO default handling for dataAttribute printWristbandField
		printWristbandField.sendKeys(entity.getPrintWristband().toString());

		// TODO default handling for dataAttribute printLabelField
		printLabelField.sendKeys(entity.getPrintLabel().toString());

		// TODO default handling for dataAttribute uploadedDateTimeField
		uploadedDateTimeField.sendKeys(entity.getUploadedDateTime().toString());

		visitStatusField.sendKeys(entity.getVisitStatus());
		instructionField.sendKeys(entity.getInstruction());
		if (entity.getIsAnAppointment()) {
			isAnAppointmentField.click();
		}
		referralNumberField.sendKeys(entity.getReferralNumber());
		notesField.sendKeys(entity.getNotes());
		kodepoliField.sendKeys(entity.getKodepoli());
		deathCertificateNoField.sendKeys(entity.getDeathCertificateNo());
		// TODO default handling for dataAttribute timeOfDeathField
		timeOfDeathField.sendKeys(entity.getTimeOfDeath().toString());

		if (entity.getDismissalReferral()) {
			dismissalReferralField.click();
		}
		dismissalReferralDetailField.sendKeys(entity.getDismissalReferralDetail());
		dismissalReferralTypeField.sendKeys(entity.getDismissalReferralType());
		dismissalReferringNotesField.sendKeys(entity.getDismissalReferringNotes());
		if (entity.getPatientIsReferredBack()) {
			patientIsReferredBackField.click();
		}
		mcuPackageField.sendKeys(entity.getMcuPackage());
		parentRegistrationNumberField.sendKeys(entity.getParentRegistrationNumber());
		if (entity.getIsUnknownPatient()) {
			isUnknownPatientField.click();
		}
