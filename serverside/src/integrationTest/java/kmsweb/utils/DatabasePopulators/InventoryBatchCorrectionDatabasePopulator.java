...
@Component
public class InventoryBatchCorrectionDatabasePopulator implements DatabasePopulatorInterface<InventoryBatchCorrectionEntity> {
	...
	public InventoryBatchCorrectionDatabasePopulator() {}
	...
	public void populateRelatedEntitiesInDatabase(
		@NotNull InventoryBatchCorrectionEntity entity,
		...
		if (generateEntities) {
			warehouse = warehouseFactory.getObject();
		} else {
			if (entity.getWarehouse() == null) {
				throw new IllegalArgumentException("Warehouse relation does not exist and generateEntities has been set to false");
			}
			warehouse = entity.getWarehouse();
		}
		
		assert warehouse != null;
		warehouseDatabasePopulator.populateRelatedEntitiesInDatabase(warehouse, true, true, false);
		AnonymousHelper.runAnonymously(() -> {
			...
			if (addIdToParent) {
				entity.setWarehouseId(savedWarehouse.getId());
			}
			if (addEntityToParent) {
				...
				entity.setWarehouse(warehouseRepository.findById(savedWarehouse.getId()).get(), false);
			}
		});
