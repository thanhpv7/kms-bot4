...
@Component
public class InvoiceDatabasePopulator implements DatabasePopulatorInterface<InvoiceEntity> {
	...
	public InvoiceDatabasePopulator() {}
	...
	public void populateRelatedEntitiesInDatabase(
		...
		TariffSchemaEntity tariffSchema;
		if (generateEntities) {
			tariffSchema = tariffSchemaFactory.getObject();
		} else {
			if (entity.getTariffSchema() == null) {
				throw new IllegalArgumentException("Tariff Schema relation does not exist and generateEntities has been set to false");
			}
			tariffSchema = entity.getTariffSchema();
		}
		
		assert tariffSchema != null;
		tariffSchemaDatabasePopulator.populateRelatedEntitiesInDatabase(tariffSchema, true, true, false);
		AnonymousHelper.runAnonymously(() -> {
			...
			TariffSchemaEntity savedTariffSchema = tariffSchemaRepository.save(tariffSchema);
			if (addIdToParent) {
				entity.setTariffSchemaId(savedTariffSchema.getId());
			}
			if (addEntityToParent) {
				...
				entity.setTariffSchema(tariffSchemaRepository.findById(savedTariffSchema.getId()).get(), false);
			}
		});
