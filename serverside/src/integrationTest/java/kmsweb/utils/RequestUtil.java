...
public class RequestUtil {
	...
	public static void checkErrorResponse(@NonNull ResultActions result, @NonNull String expectedError,
		...
		ErrorResponseDto actual = new ObjectMapper().readValue(response.getContentAsString(), ErrorResponseDto.class);

		Assertions.assertEquals(1, actual.getErrors().size());

		ErrorDto error = actual.getErrors().get(0);

		// Check the body of failed response
		Assertions.assertEquals(expectedError, error.getError());
		Assertions.assertEquals(expectedDescription, error.getMessage());
		Assertions.assertEquals(response.getStatus(), statusCode);
	...
	public static void checkSuccessfulLoginResponse(@NonNull ResultActions result, @NonNull String expectedUserId, @NonNull String expectedUsername, @NonNull List<String> expectedRoles) throws  Exception {
		...
		result.andDo(rs -> {
			...
			Assertions.assertEquals(expectedUserId, actual.get("id"));
			Assertions.assertEquals(expectedUsername, actual.get("username"));

			@SuppressWarnings("unchecked")
			var groups = (List<String>) actual.get("groups");;
