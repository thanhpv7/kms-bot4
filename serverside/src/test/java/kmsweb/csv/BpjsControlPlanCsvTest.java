...
import kmsweb.controllers.BpjsControlPlanController;
...
@Tag("CSV")
public class BpjsControlPlanCsvTest {

	@Mock
	private BpjsControlPlanService service;
	...
	private BpjsControlPlanController controller;

	private CsvUtils<BpjsControlPlanEntity, BpjsControlPlanRepository, BpjsControlPlanEntityAudit, BpjsControlPlanService> csvUtils;

	private final BpjsControlPlanFactory factory = new BpjsControlPlanFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsControlPlanController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsControlPlanEntity.class);
	}

	@Test
	void whenExportingBpjsControlPlanEntities_thenHeadersAreValid() throws Exception {
		List<BpjsControlPlanEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsControlPlanEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsControlPlanEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsControlPlanEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPolyclinicCode())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsControlPlanEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsControlPlanEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsControlPlanEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsControlPlanEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsControlPlanEntity.class));
		when(service.validateEntityRelations(any(BpjsControlPlanEntity.class))).thenReturn(new HashSet<>());
