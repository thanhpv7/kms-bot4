...
import kmsweb.controllers.BpjsOutsideFacilityReferralController;
...
@Tag("CSV")
public class BpjsOutsideFacilityReferralCsvTest {

	@Mock
	private BpjsOutsideFacilityReferralService service;
	...
	private BpjsOutsideFacilityReferralController controller;

	private CsvUtils<BpjsOutsideFacilityReferralEntity, BpjsOutsideFacilityReferralRepository, BpjsOutsideFacilityReferralEntityAudit, BpjsOutsideFacilityReferralService> csvUtils;

	private final BpjsOutsideFacilityReferralFactory factory = new BpjsOutsideFacilityReferralFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsOutsideFacilityReferralController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsOutsideFacilityReferralEntity.class);
	}

	@Test
	void whenExportingBpjsOutsideFacilityReferralEntities_thenHeadersAreValid() throws Exception {
		List<BpjsOutsideFacilityReferralEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsOutsideFacilityReferralEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsOutsideFacilityReferralEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsOutsideFacilityReferralEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferredPPKCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferredPPKName())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVisitPlanDate())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferralPolyclinic())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsOutsideFacilityReferralEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsOutsideFacilityReferralEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsOutsideFacilityReferralEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsOutsideFacilityReferralEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsOutsideFacilityReferralEntity.class));
		when(service.validateEntityRelations(any(BpjsOutsideFacilityReferralEntity.class))).thenReturn(new HashSet<>());
