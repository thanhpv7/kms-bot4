...
import kmsweb.controllers.PatientDetailAddressController;
...
@Tag("CSV")
public class PatientDetailAddressCsvTest {

	@Mock
	private PatientDetailAddressService service;
	...
	private PatientDetailAddressController controller;

	private CsvUtils<PatientDetailAddressEntity, PatientDetailAddressRepository, PatientDetailAddressEntityAudit, PatientDetailAddressService> csvUtils;

	private final PatientDetailAddressFactory factory = new PatientDetailAddressFactory();
	...
	public void setup() {
		...
		this.controller = new PatientDetailAddressController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientDetailAddressEntity.class);
	}

	@Test
	void whenExportingPatientDetailAddressEntities_thenHeadersAreValid() throws Exception {
		List<PatientDetailAddressEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientDetailAddressEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientDetailAddressEntities_thenEntitiesAreValid() throws Exception {
		List<PatientDetailAddressEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRt())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRw())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientDetailAddressEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientDetailAddressEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientDetailAddressEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientDetailAddressEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientDetailAddressEntity.class));
		when(service.validateEntityRelations(any(PatientDetailAddressEntity.class))).thenReturn(new HashSet<>());
