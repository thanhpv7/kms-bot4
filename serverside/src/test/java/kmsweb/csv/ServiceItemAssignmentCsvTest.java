...
import kmsweb.controllers.ServiceItemAssignmentController;
...
@Tag("CSV")
public class ServiceItemAssignmentCsvTest {

	@Mock
	private ServiceItemAssignmentService service;
	...
	private ServiceItemAssignmentController controller;

	private CsvUtils<ServiceItemAssignmentEntity, ServiceItemAssignmentRepository, ServiceItemAssignmentEntityAudit, ServiceItemAssignmentService> csvUtils;

	private final ServiceItemAssignmentFactory factory = new ServiceItemAssignmentFactory();
	...
	public void setup() {
		...
		this.controller = new ServiceItemAssignmentController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ServiceItemAssignmentEntity.class);
	}

	@Test
	void whenExportingServiceItemAssignmentEntities_thenHeadersAreValid() throws Exception {
		List<ServiceItemAssignmentEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ServiceItemAssignmentEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingServiceItemAssignmentEntities_thenEntitiesAreValid() throws Exception {
		List<ServiceItemAssignmentEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ServiceItemAssignmentEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ServiceItemAssignmentEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ServiceItemAssignmentEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ServiceItemAssignmentEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ServiceItemAssignmentEntity.class));
		when(service.validateEntityRelations(any(ServiceItemAssignmentEntity.class))).thenReturn(new HashSet<>());
