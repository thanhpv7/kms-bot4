...
import kmsweb.controllers.BpjsTreatmentRoomController;
...
@Tag("CSV")
public class BpjsTreatmentRoomCsvTest {

	@Mock
	private BpjsTreatmentRoomService service;
	...
	private BpjsTreatmentRoomController controller;

	private CsvUtils<BpjsTreatmentRoomEntity, BpjsTreatmentRoomRepository, BpjsTreatmentRoomEntityAudit, BpjsTreatmentRoomService> csvUtils;

	private final BpjsTreatmentRoomFactory factory = new BpjsTreatmentRoomFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsTreatmentRoomController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsTreatmentRoomEntity.class);
	}

	@Test
	void whenExportingBpjsTreatmentRoomEntities_thenHeadersAreValid() throws Exception {
		List<BpjsTreatmentRoomEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTreatmentRoomEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsTreatmentRoomEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsTreatmentRoomEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKode())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTreatmentRoomEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsTreatmentRoomEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsTreatmentRoomEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsTreatmentRoomEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsTreatmentRoomEntity.class));
		when(service.validateEntityRelations(any(BpjsTreatmentRoomEntity.class))).thenReturn(new HashSet<>());
