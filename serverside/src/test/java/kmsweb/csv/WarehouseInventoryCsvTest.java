...
import kmsweb.controllers.WarehouseInventoryController;
...
@Tag("CSV")
public class WarehouseInventoryCsvTest {
	...
	private CsvUtils<WarehouseInventoryEntity, WarehouseInventoryRepository, WarehouseInventoryEntityAudit, WarehouseInventoryService> csvUtils;

	private final WarehouseInventoryFactory factory = new WarehouseInventoryFactory();
	...
	public void setup() {
		...
		this.controller = new WarehouseInventoryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, WarehouseInventoryEntity.class);
	}

	@Test
	void whenExportingWarehouseInventoryEntities_thenHeadersAreValid() throws Exception {
		List<WarehouseInventoryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseInventoryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingWarehouseInventoryEntities_thenEntitiesAreValid() throws Exception {
		List<WarehouseInventoryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReservedQuantity()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseInventoryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		WarehouseInventoryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(WarehouseInventoryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(WarehouseInventoryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(WarehouseInventoryEntity.class));
		when(service.validateEntityRelations(any(WarehouseInventoryEntity.class))).thenReturn(new HashSet<>());
