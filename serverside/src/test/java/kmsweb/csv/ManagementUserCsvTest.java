...
import kmsweb.controllers.ManagementUserController;
...
@Tag("CSV")
public class ManagementUserCsvTest {
	...
	private CsvUtils<ManagementUserEntity, ManagementUserRepository, ManagementUserEntityAudit, ManagementUserService> csvUtils;

	private final ManagementUserFactory factory = new ManagementUserFactory();
	...
	public void setup() {
		...
		this.controller = new ManagementUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ManagementUserEntity.class);
	}

	@Test
	void whenExportingManagementUserEntities_thenHeadersAreValid() throws Exception {
		List<ManagementUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ManagementUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingManagementUserEntities_thenEntitiesAreValid() throws Exception {
		List<ManagementUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ManagementUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ManagementUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ManagementUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ManagementUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ManagementUserEntity.class));
		when(service.validateEntityRelations(any(ManagementUserEntity.class))).thenReturn(new HashSet<>());
