...
import kmsweb.controllers.BatchStockBalanceController;
...
@Tag("CSV")
public class BatchStockBalanceCsvTest {

	@Mock
	private BatchStockBalanceService service;
	...
	private BatchStockBalanceController controller;

	private CsvUtils<BatchStockBalanceEntity, BatchStockBalanceRepository, BatchStockBalanceEntityAudit, BatchStockBalanceService> csvUtils;

	private final BatchStockBalanceFactory factory = new BatchStockBalanceFactory();
	...
	public void setup() {
		...
		this.controller = new BatchStockBalanceController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BatchStockBalanceEntity.class);
	}

	@Test
	void whenExportingBatchStockBalanceEntities_thenHeadersAreValid() throws Exception {
		List<BatchStockBalanceEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BatchStockBalanceEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBatchStockBalanceEntities_thenEntitiesAreValid() throws Exception {
		List<BatchStockBalanceEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BatchStockBalanceEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BatchStockBalanceEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BatchStockBalanceEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BatchStockBalanceEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BatchStockBalanceEntity.class));
		when(service.validateEntityRelations(any(BatchStockBalanceEntity.class))).thenReturn(new HashSet<>());
