...
import kmsweb.controllers.DoctorUserController;
...
@Tag("CSV")
public class DoctorUserCsvTest {

	@Mock
	private DoctorUserService service;
	...
	private DoctorUserController controller;

	private CsvUtils<DoctorUserEntity, DoctorUserRepository, DoctorUserEntityAudit, DoctorUserService> csvUtils;
	...
	public void setup() {
		...
		this.controller = new DoctorUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DoctorUserEntity.class);
	}

	@Test
	void whenExportingDoctorUserEntities_thenHeadersAreValid() throws Exception {
		List<DoctorUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DoctorUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDoctorUserEntities_thenEntitiesAreValid() throws Exception {
		List<DoctorUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DoctorUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DoctorUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DoctorUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DoctorUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DoctorUserEntity.class));
		when(service.validateEntityRelations(any(DoctorUserEntity.class))).thenReturn(new HashSet<>());
