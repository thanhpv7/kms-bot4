...
import kmsweb.controllers.NhisTypeOfServiceController;
...
@Tag("CSV")
public class NhisTypeOfServiceCsvTest {

	@Mock
	private NhisTypeOfServiceService service;
	...
	private NhisTypeOfServiceController controller;

	private CsvUtils<NhisTypeOfServiceEntity, NhisTypeOfServiceRepository, NhisTypeOfServiceEntityAudit, NhisTypeOfServiceService> csvUtils;

	private final NhisTypeOfServiceFactory factory = new NhisTypeOfServiceFactory();
	...
	public void setup() {
		...
		this.controller = new NhisTypeOfServiceController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisTypeOfServiceEntity.class);
	}

	@Test
	void whenExportingNhisTypeOfServiceEntities_thenHeadersAreValid() throws Exception {
		List<NhisTypeOfServiceEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisTypeOfServiceEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisTypeOfServiceEntities_thenEntitiesAreValid() throws Exception {
		List<NhisTypeOfServiceEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSections())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisTypeOfServiceEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisTypeOfServiceEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisTypeOfServiceEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisTypeOfServiceEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisTypeOfServiceEntity.class));
		when(service.validateEntityRelations(any(NhisTypeOfServiceEntity.class))).thenReturn(new HashSet<>());
