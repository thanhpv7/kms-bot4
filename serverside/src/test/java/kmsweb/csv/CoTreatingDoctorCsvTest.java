...
import kmsweb.controllers.CoTreatingDoctorController;
...
@Tag("CSV")
public class CoTreatingDoctorCsvTest {

	@Mock
	private CoTreatingDoctorService service;
	...
	private CoTreatingDoctorController controller;

	private CsvUtils<CoTreatingDoctorEntity, CoTreatingDoctorRepository, CoTreatingDoctorEntityAudit, CoTreatingDoctorService> csvUtils;

	private final CoTreatingDoctorFactory factory = new CoTreatingDoctorFactory();
	...
	public void setup() {
		...
		this.controller = new CoTreatingDoctorController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CoTreatingDoctorEntity.class);
	}

	@Test
	void whenExportingCoTreatingDoctorEntities_thenHeadersAreValid() throws Exception {
		List<CoTreatingDoctorEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CoTreatingDoctorEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCoTreatingDoctorEntities_thenEntitiesAreValid() throws Exception {
		List<CoTreatingDoctorEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDoctorNotes())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CoTreatingDoctorEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CoTreatingDoctorEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CoTreatingDoctorEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CoTreatingDoctorEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CoTreatingDoctorEntity.class));
		when(service.validateEntityRelations(any(CoTreatingDoctorEntity.class))).thenReturn(new HashSet<>());
