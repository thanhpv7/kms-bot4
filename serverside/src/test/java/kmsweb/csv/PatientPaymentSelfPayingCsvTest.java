...
import kmsweb.controllers.PatientPaymentSelfPayingController;
...
@Tag("CSV")
public class PatientPaymentSelfPayingCsvTest {

	@Mock
	private PatientPaymentSelfPayingService service;
	...
	private PatientPaymentSelfPayingController controller;

	private CsvUtils<PatientPaymentSelfPayingEntity, PatientPaymentSelfPayingRepository, PatientPaymentSelfPayingEntityAudit, PatientPaymentSelfPayingService> csvUtils;

	private final PatientPaymentSelfPayingFactory factory = new PatientPaymentSelfPayingFactory();
	...
	public void setup() {
		...
		this.controller = new PatientPaymentSelfPayingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientPaymentSelfPayingEntity.class);
	}

	@Test
	void whenExportingPatientPaymentSelfPayingEntities_thenHeadersAreValid() throws Exception {
		List<PatientPaymentSelfPayingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentSelfPayingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientPaymentSelfPayingEntities_thenEntitiesAreValid() throws Exception {
		List<PatientPaymentSelfPayingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentSelfPayingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientPaymentSelfPayingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientPaymentSelfPayingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientPaymentSelfPayingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientPaymentSelfPayingEntity.class));
		when(service.validateEntityRelations(any(PatientPaymentSelfPayingEntity.class))).thenReturn(new HashSet<>());
