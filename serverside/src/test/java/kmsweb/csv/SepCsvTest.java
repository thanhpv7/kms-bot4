...
import kmsweb.controllers.SepController;
...
public class SepCsvTest {
	...
	private CsvUtils<SepEntity, SepRepository, SepEntityAudit, SepService> csvUtils;

	private final SepFactory factory = new SepFactory();
	...
	public void setup() {
		...
		this.controller = new SepController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SepEntity.class);
	}

	@Test
	void whenExportingSepEntities_thenHeadersAreValid() throws Exception {
		List<SepEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SepEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSepEntities_thenEntitiesAreValid() throws Exception {
		List<SepEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSepData())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SepEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SepEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SepEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SepEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SepEntity.class));
		when(service.validateEntityRelations(any(SepEntity.class))).thenReturn(new HashSet<>());
