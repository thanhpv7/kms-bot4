...
import kmsweb.controllers.PatientGeneralInfoController;
...
@Tag("CSV")
public class PatientGeneralInfoCsvTest {

	@Mock
	private PatientGeneralInfoService service;
	...
	private PatientGeneralInfoController controller;

	private CsvUtils<PatientGeneralInfoEntity, PatientGeneralInfoRepository, PatientGeneralInfoEntityAudit, PatientGeneralInfoService> csvUtils;

	private final PatientGeneralInfoFactory factory = new PatientGeneralInfoFactory();
	...
	public void setup() {
		...
		this.controller = new PatientGeneralInfoController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientGeneralInfoEntity.class);
	}

	@Test
	void whenExportingPatientGeneralInfoEntities_thenHeadersAreValid() throws Exception {
		List<PatientGeneralInfoEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientGeneralInfoEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientGeneralInfoEntities_thenEntitiesAreValid() throws Exception {
		List<PatientGeneralInfoEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getImportantNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientPicture()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHasOutstandingPayment()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientGeneralInfoEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientGeneralInfoEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientGeneralInfoEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientGeneralInfoEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientGeneralInfoEntity.class));
		when(service.validateEntityRelations(any(PatientGeneralInfoEntity.class))).thenReturn(new HashSet<>());
