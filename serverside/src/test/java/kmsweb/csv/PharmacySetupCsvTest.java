...
import kmsweb.controllers.PharmacySetupController;
...
@Tag("CSV")
public class PharmacySetupCsvTest {

	@Mock
	private PharmacySetupService service;
	...
	private PharmacySetupController controller;

	private CsvUtils<PharmacySetupEntity, PharmacySetupRepository, PharmacySetupEntityAudit, PharmacySetupService> csvUtils;

	private final PharmacySetupFactory factory = new PharmacySetupFactory();
	...
	public void setup() {
		...
		this.controller = new PharmacySetupController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PharmacySetupEntity.class);
	}

	@Test
	void whenExportingPharmacySetupEntities_thenHeadersAreValid() throws Exception {
		List<PharmacySetupEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PharmacySetupEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPharmacySetupEntities_thenEntitiesAreValid() throws Exception {
		List<PharmacySetupEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHospitalPharmacy())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHeadOfPharmacy())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPharmacistLicenseNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLabelSetup())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PharmacySetupEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PharmacySetupEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PharmacySetupEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PharmacySetupEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PharmacySetupEntity.class));
		when(service.validateEntityRelations(any(PharmacySetupEntity.class))).thenReturn(new HashSet<>());
