...
import kmsweb.controllers.NhisGDRGController;
...
@Tag("CSV")
public class NhisGDRGCsvTest {

	@Mock
	private NhisGDRGService service;
	...
	private NhisGDRGController controller;

	private CsvUtils<NhisGDRGEntity, NhisGDRGRepository, NhisGDRGEntityAudit, NhisGDRGService> csvUtils;

	private final NhisGDRGFactory factory = new NhisGDRGFactory();
	...
	public void setup() {
		...
		this.controller = new NhisGDRGController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisGDRGEntity.class);
	}

	@Test
	void whenExportingNhisGDRGEntities_thenHeadersAreValid() throws Exception {
		List<NhisGDRGEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisGDRGEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisGDRGEntities_thenEntitiesAreValid() throws Exception {
		List<NhisGDRGEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCoreGDRGCode())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLevelOfHealthcare())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInvestigationCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIcd10s())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisGDRGEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisGDRGEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisGDRGEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisGDRGEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisGDRGEntity.class));
		when(service.validateEntityRelations(any(NhisGDRGEntity.class))).thenReturn(new HashSet<>());
