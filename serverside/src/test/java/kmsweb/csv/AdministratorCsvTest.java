...
import kmsweb.controllers.AdministratorController;
...
@Tag("CSV")
public class AdministratorCsvTest {
	...
	private CsvUtils<AdministratorEntity, AdministratorRepository, AdministratorEntityAudit, AdministratorService> csvUtils;

	private final AdministratorFactory factory = new AdministratorFactory();
	...
	public void setup() {
		...
		this.controller = new AdministratorController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, AdministratorEntity.class);
	}

	@Test
	void whenExportingAdministratorEntities_thenHeadersAreValid() throws Exception {
		List<AdministratorEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(AdministratorEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingAdministratorEntities_thenEntitiesAreValid() throws Exception {
		List<AdministratorEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(AdministratorEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		AdministratorEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(AdministratorEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(AdministratorEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(AdministratorEntity.class));
		when(service.validateEntityRelations(any(AdministratorEntity.class))).thenReturn(new HashSet<>());
