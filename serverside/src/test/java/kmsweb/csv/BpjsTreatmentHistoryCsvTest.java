...
import kmsweb.controllers.BpjsTreatmentHistoryController;
...
@Tag("CSV")
public class BpjsTreatmentHistoryCsvTest {

	@Mock
	private BpjsTreatmentHistoryService service;
	...
	private BpjsTreatmentHistoryController controller;

	private CsvUtils<BpjsTreatmentHistoryEntity, BpjsTreatmentHistoryRepository, BpjsTreatmentHistoryEntityAudit, BpjsTreatmentHistoryService> csvUtils;

	private final BpjsTreatmentHistoryFactory factory = new BpjsTreatmentHistoryFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsTreatmentHistoryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsTreatmentHistoryEntity.class);
	}

	@Test
	void whenExportingBpjsTreatmentHistoryEntities_thenHeadersAreValid() throws Exception {
		List<BpjsTreatmentHistoryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTreatmentHistoryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsTreatmentHistoryEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsTreatmentHistoryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHealthFacility())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTreatmentHistoryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsTreatmentHistoryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsTreatmentHistoryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsTreatmentHistoryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsTreatmentHistoryEntity.class));
		when(service.validateEntityRelations(any(BpjsTreatmentHistoryEntity.class))).thenReturn(new HashSet<>());
