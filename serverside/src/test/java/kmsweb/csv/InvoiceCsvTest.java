...
import kmsweb.controllers.InvoiceController;
...
public class InvoiceCsvTest {
	...
	private CsvUtils<InvoiceEntity, InvoiceRepository, InvoiceEntityAudit, InvoiceService> csvUtils;
	...
	public void setup() {
		...
		this.controller = new InvoiceController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InvoiceEntity.class);
	}

	@Test
	void whenExportingInvoiceEntities_thenHeadersAreValid() throws Exception {
		List<InvoiceEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInvoiceEntities_thenEntitiesAreValid() throws Exception {
		List<InvoiceEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInvoiceType()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDisplayICD10OnPrint()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDisplayICD9CMOnPrint()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDoctorCode())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDischargedDate()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InvoiceEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InvoiceEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InvoiceEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InvoiceEntity.class));
		when(service.validateEntityRelations(any(InvoiceEntity.class))).thenReturn(new HashSet<>());
