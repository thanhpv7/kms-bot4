...
import kmsweb.controllers.NhisClaimInvestigationDetailController;
...
@Tag("CSV")
public class NhisClaimInvestigationDetailCsvTest {

	@Mock
	private NhisClaimInvestigationDetailService service;
	...
	private NhisClaimInvestigationDetailController controller;

	private CsvUtils<NhisClaimInvestigationDetailEntity, NhisClaimInvestigationDetailRepository, NhisClaimInvestigationDetailEntityAudit, NhisClaimInvestigationDetailService> csvUtils;

	private final NhisClaimInvestigationDetailFactory factory = new NhisClaimInvestigationDetailFactory();
	...
	public void setup() {
		...
		this.controller = new NhisClaimInvestigationDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisClaimInvestigationDetailEntity.class);
	}

	@Test
	void whenExportingNhisClaimInvestigationDetailEntities_thenHeadersAreValid() throws Exception {
		List<NhisClaimInvestigationDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisClaimInvestigationDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisClaimInvestigationDetailEntities_thenEntitiesAreValid() throws Exception {
		List<NhisClaimInvestigationDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisClaimInvestigationDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisClaimInvestigationDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisClaimInvestigationDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisClaimInvestigationDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisClaimInvestigationDetailEntity.class));
		when(service.validateEntityRelations(any(NhisClaimInvestigationDetailEntity.class))).thenReturn(new HashSet<>());
