...
import kmsweb.controllers.MedicalCertificateBirthController;
...
@Tag("CSV")
public class MedicalCertificateBirthCsvTest {

	@Mock
	private MedicalCertificateBirthService service;
	...
	private MedicalCertificateBirthController controller;

	private CsvUtils<MedicalCertificateBirthEntity, MedicalCertificateBirthRepository, MedicalCertificateBirthEntityAudit, MedicalCertificateBirthService> csvUtils;

	private final MedicalCertificateBirthFactory factory = new MedicalCertificateBirthFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalCertificateBirthController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalCertificateBirthEntity.class);
	}

	@Test
	void whenExportingMedicalCertificateBirthEntities_thenHeadersAreValid() throws Exception {
		List<MedicalCertificateBirthEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateBirthEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalCertificateBirthEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalCertificateBirthEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSpouseName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNumberOfChildren()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBirthNotes())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateBirthEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalCertificateBirthEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalCertificateBirthEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalCertificateBirthEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalCertificateBirthEntity.class));
		when(service.validateEntityRelations(any(MedicalCertificateBirthEntity.class))).thenReturn(new HashSet<>());
