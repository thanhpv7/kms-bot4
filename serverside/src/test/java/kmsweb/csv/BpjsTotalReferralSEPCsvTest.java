...
import kmsweb.controllers.BpjsTotalReferralSEPController;
...
@Tag("CSV")
public class BpjsTotalReferralSEPCsvTest {

	@Mock
	private BpjsTotalReferralSEPService service;
	...
	private BpjsTotalReferralSEPController controller;

	private CsvUtils<BpjsTotalReferralSEPEntity, BpjsTotalReferralSEPRepository, BpjsTotalReferralSEPEntityAudit, BpjsTotalReferralSEPService> csvUtils;

	private final BpjsTotalReferralSEPFactory factory = new BpjsTotalReferralSEPFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsTotalReferralSEPController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsTotalReferralSEPEntity.class);
	}

	@Test
	void whenExportingBpjsTotalReferralSEPEntities_thenHeadersAreValid() throws Exception {
		List<BpjsTotalReferralSEPEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTotalReferralSEPEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsTotalReferralSEPEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsTotalReferralSEPEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalSEP())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTotalReferralSEPEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsTotalReferralSEPEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsTotalReferralSEPEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsTotalReferralSEPEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsTotalReferralSEPEntity.class));
		when(service.validateEntityRelations(any(BpjsTotalReferralSEPEntity.class))).thenReturn(new HashSet<>());
