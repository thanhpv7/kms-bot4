...
import kmsweb.controllers.PcareTindakanController;
...
@Tag("CSV")
public class PcareTindakanCsvTest {

	@Mock
	private PcareTindakanService service;
	...
	private PcareTindakanController controller;

	private CsvUtils<PcareTindakanEntity, PcareTindakanRepository, PcareTindakanEntityAudit, PcareTindakanService> csvUtils;

	private final PcareTindakanFactory factory = new PcareTindakanFactory();
	...
	public void setup() {
		...
		this.controller = new PcareTindakanController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareTindakanEntity.class);
	}

	@Test
	void whenExportingPcareTindakanEntities_thenHeadersAreValid() throws Exception {
		List<PcareTindakanEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareTindakanEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareTindakanEntities_thenEntitiesAreValid() throws Exception {
		List<PcareTindakanEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodeTindakan())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaTindakan())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMaksimalTarif())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getWithvalue()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTindakanInternal())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareTindakanEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareTindakanEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareTindakanEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareTindakanEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareTindakanEntity.class));
		when(service.validateEntityRelations(any(PcareTindakanEntity.class))).thenReturn(new HashSet<>());
