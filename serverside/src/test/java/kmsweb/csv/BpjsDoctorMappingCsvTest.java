...
import kmsweb.controllers.BpjsDoctorMappingController;
...
@Tag("CSV")
public class BpjsDoctorMappingCsvTest {

	@Mock
	private BpjsDoctorMappingService service;
	...
	private BpjsDoctorMappingController controller;

	private CsvUtils<BpjsDoctorMappingEntity, BpjsDoctorMappingRepository, BpjsDoctorMappingEntityAudit, BpjsDoctorMappingService> csvUtils;

	private final BpjsDoctorMappingFactory factory = new BpjsDoctorMappingFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsDoctorMappingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsDoctorMappingEntity.class);
	}

	@Test
	void whenExportingBpjsDoctorMappingEntities_thenHeadersAreValid() throws Exception {
		List<BpjsDoctorMappingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDoctorMappingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsDoctorMappingEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsDoctorMappingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBpjsMedicalStaff())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefCodeService())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDoctorMappingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsDoctorMappingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsDoctorMappingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsDoctorMappingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsDoctorMappingEntity.class));
		when(service.validateEntityRelations(any(BpjsDoctorMappingEntity.class))).thenReturn(new HashSet<>());
