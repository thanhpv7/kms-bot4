...
import kmsweb.controllers.NhisSpecialtyAttendedController;
...
@Tag("CSV")
public class NhisSpecialtyAttendedCsvTest {

	@Mock
	private NhisSpecialtyAttendedService service;
	...
	private NhisSpecialtyAttendedController controller;

	private CsvUtils<NhisSpecialtyAttendedEntity, NhisSpecialtyAttendedRepository, NhisSpecialtyAttendedEntityAudit, NhisSpecialtyAttendedService> csvUtils;

	private final NhisSpecialtyAttendedFactory factory = new NhisSpecialtyAttendedFactory();
	...
	public void setup() {
		...
		this.controller = new NhisSpecialtyAttendedController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisSpecialtyAttendedEntity.class);
	}

	@Test
	void whenExportingNhisSpecialtyAttendedEntities_thenHeadersAreValid() throws Exception {
		List<NhisSpecialtyAttendedEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisSpecialtyAttendedEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisSpecialtyAttendedEntities_thenEntitiesAreValid() throws Exception {
		List<NhisSpecialtyAttendedEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getServices())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisSpecialtyAttendedEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisSpecialtyAttendedEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisSpecialtyAttendedEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisSpecialtyAttendedEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisSpecialtyAttendedEntity.class));
		when(service.validateEntityRelations(any(NhisSpecialtyAttendedEntity.class))).thenReturn(new HashSet<>());
