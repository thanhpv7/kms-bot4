...
import kmsweb.controllers.PcareUmumController;
...
@Tag("CSV")
public class PcareUmumCsvTest {

	@Mock
	private PcareUmumService service;
	...
	private PcareUmumController controller;

	private CsvUtils<PcareUmumEntity, PcareUmumRepository, PcareUmumEntityAudit, PcareUmumService> csvUtils;

	private final PcareUmumFactory factory = new PcareUmumFactory();
	...
	public void setup() {
		...
		this.controller = new PcareUmumController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareUmumEntity.class);
	}

	@Test
	void whenExportingPcareUmumEntities_thenHeadersAreValid() throws Exception {
		List<PcareUmumEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareUmumEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareUmumEntities_thenEntitiesAreValid() throws Exception {
		List<PcareUmumEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getWebServiceURL())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAuthUsername())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAuthPassword())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodeAplikasi())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUserKey())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRegionalDivision())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBranchOffice())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareUmumEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareUmumEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareUmumEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareUmumEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareUmumEntity.class));
		when(service.validateEntityRelations(any(PcareUmumEntity.class))).thenReturn(new HashSet<>());
