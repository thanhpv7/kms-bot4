...
import kmsweb.controllers.BpjsProcedureController;
...
@Tag("CSV")
public class BpjsProcedureCsvTest {

	@Mock
	private BpjsProcedureService service;
	...
	private BpjsProcedureController controller;

	private CsvUtils<BpjsProcedureEntity, BpjsProcedureRepository, BpjsProcedureEntityAudit, BpjsProcedureService> csvUtils;

	private final BpjsProcedureFactory factory = new BpjsProcedureFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsProcedureController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsProcedureEntity.class);
	}

	@Test
	void whenExportingBpjsProcedureEntities_thenHeadersAreValid() throws Exception {
		List<BpjsProcedureEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsProcedureEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsProcedureEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsProcedureEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProcedureCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProcedureName())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsProcedureEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsProcedureEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsProcedureEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsProcedureEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsProcedureEntity.class));
		when(service.validateEntityRelations(any(BpjsProcedureEntity.class))).thenReturn(new HashSet<>());
