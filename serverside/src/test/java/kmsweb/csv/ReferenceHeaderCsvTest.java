...
import kmsweb.controllers.ReferenceHeaderController;
...
@Tag("CSV")
public class ReferenceHeaderCsvTest {

	@Mock
	private ReferenceHeaderService service;
	...
	private ReferenceHeaderController controller;

	private CsvUtils<ReferenceHeaderEntity, ReferenceHeaderRepository, ReferenceHeaderEntityAudit, ReferenceHeaderService> csvUtils;

	private final ReferenceHeaderFactory factory = new ReferenceHeaderFactory();
	...
	public void setup() {
		...
		this.controller = new ReferenceHeaderController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ReferenceHeaderEntity.class);
	}

	@Test
	void whenExportingReferenceHeaderEntities_thenHeadersAreValid() throws Exception {
		List<ReferenceHeaderEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ReferenceHeaderEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingReferenceHeaderEntities_thenEntitiesAreValid() throws Exception {
		List<ReferenceHeaderEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ReferenceHeaderEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ReferenceHeaderEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ReferenceHeaderEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ReferenceHeaderEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ReferenceHeaderEntity.class));
		when(service.validateEntityRelations(any(ReferenceHeaderEntity.class))).thenReturn(new HashSet<>());
