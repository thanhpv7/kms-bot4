...
import kmsweb.controllers.VaccinationOrderController;
...
@Tag("CSV")
public class VaccinationOrderCsvTest {

	@Mock
	private VaccinationOrderService service;
	...
	private VaccinationOrderController controller;

	private CsvUtils<VaccinationOrderEntity, VaccinationOrderRepository, VaccinationOrderEntityAudit, VaccinationOrderService> csvUtils;

	private final VaccinationOrderFactory factory = new VaccinationOrderFactory();
	...
	public void setup() {
		...
		this.controller = new VaccinationOrderController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, VaccinationOrderEntity.class);
	}

	@Test
	void whenExportingVaccinationOrderEntities_thenHeadersAreValid() throws Exception {
		List<VaccinationOrderEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(VaccinationOrderEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingVaccinationOrderEntities_thenEntitiesAreValid() throws Exception {
		List<VaccinationOrderEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVaccinationLocation())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVaccinationReaction())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPurpose())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVaccinationNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReactionNotes())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(VaccinationOrderEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		VaccinationOrderEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(VaccinationOrderEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(VaccinationOrderEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(VaccinationOrderEntity.class));
		when(service.validateEntityRelations(any(VaccinationOrderEntity.class))).thenReturn(new HashSet<>());
