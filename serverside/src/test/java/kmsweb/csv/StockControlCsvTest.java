...
import kmsweb.controllers.StockControlController;
...
@Tag("CSV")
public class StockControlCsvTest {

	@Mock
	private StockControlService service;
	...
	private CsvUtils<StockControlEntity, StockControlRepository, StockControlEntityAudit, StockControlService> csvUtils;

	private final StockControlFactory factory = new StockControlFactory();
	...
	public void setup() {
		...
		this.controller = new StockControlController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, StockControlEntity.class);
	}

	@Test
	void whenExportingStockControlEntities_thenHeadersAreValid() throws Exception {
		List<StockControlEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(StockControlEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingStockControlEntities_thenEntitiesAreValid() throws Exception {
		List<StockControlEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAvailableQty()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOnReservedQty()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUoi())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMinLevel()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMaxLevel()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInTransitQty()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(StockControlEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		StockControlEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(StockControlEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(StockControlEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(StockControlEntity.class));
		when(service.validateEntityRelations(any(StockControlEntity.class))).thenReturn(new HashSet<>());
