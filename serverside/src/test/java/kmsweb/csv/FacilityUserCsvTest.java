...
import kmsweb.controllers.FacilityUserController;
...
@Tag("CSV")
public class FacilityUserCsvTest {

	@Mock
	private FacilityUserService service;
	...
	private FacilityUserController controller;

	private CsvUtils<FacilityUserEntity, FacilityUserRepository, FacilityUserEntityAudit, FacilityUserService> csvUtils;

	private final FacilityUserFactory factory = new FacilityUserFactory();
	...
	public void setup() {
		...
		this.controller = new FacilityUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, FacilityUserEntity.class);
	}

	@Test
	void whenExportingFacilityUserEntities_thenHeadersAreValid() throws Exception {
		List<FacilityUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(FacilityUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingFacilityUserEntities_thenEntitiesAreValid() throws Exception {
		List<FacilityUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(FacilityUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		FacilityUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(FacilityUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(FacilityUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(FacilityUserEntity.class));
		when(service.validateEntityRelations(any(FacilityUserEntity.class))).thenReturn(new HashSet<>());
