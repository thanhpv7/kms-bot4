...
import kmsweb.controllers.InvoicePaymentOtherController;
...
@Tag("CSV")
public class InvoicePaymentOtherCsvTest {

	@Mock
	private InvoicePaymentOtherService service;
	...
	private InvoicePaymentOtherController controller;

	private CsvUtils<InvoicePaymentOtherEntity, InvoicePaymentOtherRepository, InvoicePaymentOtherEntityAudit, InvoicePaymentOtherService> csvUtils;

	private final InvoicePaymentOtherFactory factory = new InvoicePaymentOtherFactory();
	...
	public void setup() {
		...
		this.controller = new InvoicePaymentOtherController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InvoicePaymentOtherEntity.class);
	}

	@Test
	void whenExportingInvoicePaymentOtherEntities_thenHeadersAreValid() throws Exception {
		List<InvoicePaymentOtherEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InvoicePaymentOtherEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInvoicePaymentOtherEntities_thenEntitiesAreValid() throws Exception {
		List<InvoicePaymentOtherEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InvoicePaymentOtherEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InvoicePaymentOtherEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InvoicePaymentOtherEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InvoicePaymentOtherEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InvoicePaymentOtherEntity.class));
		when(service.validateEntityRelations(any(InvoicePaymentOtherEntity.class))).thenReturn(new HashSet<>());
