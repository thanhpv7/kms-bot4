...
import kmsweb.controllers.PrescriptionHeaderController;
...
@Tag("CSV")
public class PrescriptionHeaderCsvTest {

	@Mock
	private PrescriptionHeaderService service;
	...
	private PrescriptionHeaderController controller;

	private CsvUtils<PrescriptionHeaderEntity, PrescriptionHeaderRepository, PrescriptionHeaderEntityAudit, PrescriptionHeaderService> csvUtils;

	private final PrescriptionHeaderFactory factory = new PrescriptionHeaderFactory();
	...
	public void setup() {
		...
		this.controller = new PrescriptionHeaderController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PrescriptionHeaderEntity.class);
	}

	@Test
	void whenExportingPrescriptionHeaderEntities_thenHeadersAreValid() throws Exception {
		List<PrescriptionHeaderEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PrescriptionHeaderEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPrescriptionHeaderEntities_thenEntitiesAreValid() throws Exception {
		List<PrescriptionHeaderEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPrescriptionNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPrescriptionType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getQueueNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOriginalPrescriptionNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPrescriptionDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsCompound()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsIteration()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getStatus())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsUrgent()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRestricted()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRevisionNo()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCopyNo()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PrescriptionHeaderEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PrescriptionHeaderEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PrescriptionHeaderEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PrescriptionHeaderEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PrescriptionHeaderEntity.class));
		when(service.validateEntityRelations(any(PrescriptionHeaderEntity.class))).thenReturn(new HashSet<>());
