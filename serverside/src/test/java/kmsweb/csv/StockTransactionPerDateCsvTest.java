...
import kmsweb.controllers.StockTransactionPerDateController;
...
@Tag("CSV")
public class StockTransactionPerDateCsvTest {

	@Mock
	private StockTransactionPerDateService service;
	...
	private StockTransactionPerDateController controller;

	private CsvUtils<StockTransactionPerDateEntity, StockTransactionPerDateRepository, StockTransactionPerDateEntityAudit, StockTransactionPerDateService> csvUtils;

	private final StockTransactionPerDateFactory factory = new StockTransactionPerDateFactory();
	...
	public void setup() {
		...
		this.controller = new StockTransactionPerDateController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, StockTransactionPerDateEntity.class);
	}

	@Test
	void whenExportingStockTransactionPerDateEntities_thenHeadersAreValid() throws Exception {
		List<StockTransactionPerDateEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(StockTransactionPerDateEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingStockTransactionPerDateEntities_thenEntitiesAreValid() throws Exception {
		List<StockTransactionPerDateEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(StockTransactionPerDateEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		StockTransactionPerDateEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(StockTransactionPerDateEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(StockTransactionPerDateEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(StockTransactionPerDateEntity.class));
		when(service.validateEntityRelations(any(StockTransactionPerDateEntity.class))).thenReturn(new HashSet<>());
