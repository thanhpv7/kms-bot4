...
import kmsweb.controllers.McuPackageItemController;
...
@Tag("CSV")
public class McuPackageItemCsvTest {

	@Mock
	private McuPackageItemService service;
	...
	private McuPackageItemController controller;

	private CsvUtils<McuPackageItemEntity, McuPackageItemRepository, McuPackageItemEntityAudit, McuPackageItemService> csvUtils;

	private final McuPackageItemFactory factory = new McuPackageItemFactory();
	...
	public void setup() {
		...
		this.controller = new McuPackageItemController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, McuPackageItemEntity.class);
	}

	@Test
	void whenExportingMcuPackageItemEntities_thenHeadersAreValid() throws Exception {
		List<McuPackageItemEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(McuPackageItemEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMcuPackageItemEntities_thenEntitiesAreValid() throws Exception {
		List<McuPackageItemEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(McuPackageItemEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		McuPackageItemEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(McuPackageItemEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(McuPackageItemEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(McuPackageItemEntity.class));
		when(service.validateEntityRelations(any(McuPackageItemEntity.class))).thenReturn(new HashSet<>());
