...
import kmsweb.controllers.PatientVisitPatientPaymentInsuranceController;
...
@Tag("CSV")
public class PatientVisitPatientPaymentInsuranceCsvTest {

	@Mock
	private PatientVisitPatientPaymentInsuranceService service;
	...
	private PatientVisitPatientPaymentInsuranceController controller;

	private CsvUtils<PatientVisitPatientPaymentInsuranceEntity, PatientVisitPatientPaymentInsuranceRepository, PatientVisitPatientPaymentInsuranceEntityAudit, PatientVisitPatientPaymentInsuranceService> csvUtils;

	private final PatientVisitPatientPaymentInsuranceFactory factory = new PatientVisitPatientPaymentInsuranceFactory();
	...
	public void setup() {
		...
		this.controller = new PatientVisitPatientPaymentInsuranceController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientVisitPatientPaymentInsuranceEntity.class);
	}

	@Test
	void whenExportingPatientVisitPatientPaymentInsuranceEntities_thenHeadersAreValid() throws Exception {
		List<PatientVisitPatientPaymentInsuranceEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitPatientPaymentInsuranceEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientVisitPatientPaymentInsuranceEntities_thenEntitiesAreValid() throws Exception {
		List<PatientVisitPatientPaymentInsuranceEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPolicyHolderNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getEligilityVerification())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getGuaranteeLetterNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCoverageBalance()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitPatientPaymentInsuranceEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientVisitPatientPaymentInsuranceEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientVisitPatientPaymentInsuranceEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientVisitPatientPaymentInsuranceEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientVisitPatientPaymentInsuranceEntity.class));
		when(service.validateEntityRelations(any(PatientVisitPatientPaymentInsuranceEntity.class))).thenReturn(new HashSet<>());
