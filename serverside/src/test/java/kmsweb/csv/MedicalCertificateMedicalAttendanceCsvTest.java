...
import kmsweb.controllers.MedicalCertificateMedicalAttendanceController;
...
@Tag("CSV")
public class MedicalCertificateMedicalAttendanceCsvTest {

	@Mock
	private MedicalCertificateMedicalAttendanceService service;
	...
	private MedicalCertificateMedicalAttendanceController controller;

	private CsvUtils<MedicalCertificateMedicalAttendanceEntity, MedicalCertificateMedicalAttendanceRepository, MedicalCertificateMedicalAttendanceEntityAudit, MedicalCertificateMedicalAttendanceService> csvUtils;

	private final MedicalCertificateMedicalAttendanceFactory factory = new MedicalCertificateMedicalAttendanceFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalCertificateMedicalAttendanceController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalCertificateMedicalAttendanceEntity.class);
	}

	@Test
	void whenExportingMedicalCertificateMedicalAttendanceEntities_thenHeadersAreValid() throws Exception {
		List<MedicalCertificateMedicalAttendanceEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateMedicalAttendanceEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalCertificateMedicalAttendanceEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalCertificateMedicalAttendanceEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getStartTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getEndTime()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateMedicalAttendanceEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalCertificateMedicalAttendanceEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalCertificateMedicalAttendanceEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalCertificateMedicalAttendanceEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalCertificateMedicalAttendanceEntity.class));
		when(service.validateEntityRelations(any(MedicalCertificateMedicalAttendanceEntity.class))).thenReturn(new HashSet<>());
