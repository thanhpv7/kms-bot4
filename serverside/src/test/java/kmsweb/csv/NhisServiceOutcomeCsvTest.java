...
import kmsweb.controllers.NhisServiceOutcomeController;
...
@Tag("CSV")
public class NhisServiceOutcomeCsvTest {

	@Mock
	private NhisServiceOutcomeService service;
	...
	private NhisServiceOutcomeController controller;

	private CsvUtils<NhisServiceOutcomeEntity, NhisServiceOutcomeRepository, NhisServiceOutcomeEntityAudit, NhisServiceOutcomeService> csvUtils;

	private final NhisServiceOutcomeFactory factory = new NhisServiceOutcomeFactory();
	...
	public void setup() {
		...
		this.controller = new NhisServiceOutcomeController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisServiceOutcomeEntity.class);
	}

	@Test
	void whenExportingNhisServiceOutcomeEntities_thenHeadersAreValid() throws Exception {
		List<NhisServiceOutcomeEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisServiceOutcomeEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisServiceOutcomeEntities_thenEntitiesAreValid() throws Exception {
		List<NhisServiceOutcomeEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDismissalTypes())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisServiceOutcomeEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisServiceOutcomeEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisServiceOutcomeEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisServiceOutcomeEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisServiceOutcomeEntity.class));
		when(service.validateEntityRelations(any(NhisServiceOutcomeEntity.class))).thenReturn(new HashSet<>());
