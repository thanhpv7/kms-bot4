...
import kmsweb.controllers.BpjsJasaRaharjaController;
...
@Tag("CSV")
public class BpjsJasaRaharjaCsvTest {

	@Mock
	private BpjsJasaRaharjaService service;
	...
	private BpjsJasaRaharjaController controller;

	private CsvUtils<BpjsJasaRaharjaEntity, BpjsJasaRaharjaRepository, BpjsJasaRaharjaEntityAudit, BpjsJasaRaharjaService> csvUtils;

	private final BpjsJasaRaharjaFactory factory = new BpjsJasaRaharjaFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsJasaRaharjaController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsJasaRaharjaEntity.class);
	}

	@Test
	void whenExportingBpjsJasaRaharjaEntities_thenHeadersAreValid() throws Exception {
		List<BpjsJasaRaharjaEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsJasaRaharjaEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsJasaRaharjaEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsJasaRaharjaEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInitialSEPNo())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRegistrationNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNoSEPSuplesi())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTanggalSEPSuplesi()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsJasaRaharjaEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsJasaRaharjaEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsJasaRaharjaEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsJasaRaharjaEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsJasaRaharjaEntity.class));
		when(service.validateEntityRelations(any(BpjsJasaRaharjaEntity.class))).thenReturn(new HashSet<>());
