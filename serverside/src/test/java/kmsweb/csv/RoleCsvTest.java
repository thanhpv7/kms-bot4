...
import kmsweb.controllers.RoleController;
...
public class RoleCsvTest {
	...
	private CsvUtils<RoleEntity, RoleRepository, RoleEntityAudit, RoleService> csvUtils;

	private final RoleFactory factory = new RoleFactory();
	...
	public void setup() {
		...
		this.controller = new RoleController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, RoleEntity.class);
	}

	@Test
	void whenExportingRoleEntities_thenHeadersAreValid() throws Exception {
		List<RoleEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(RoleEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingRoleEntities_thenEntitiesAreValid() throws Exception {
		List<RoleEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(RoleEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		RoleEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(RoleEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(RoleEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(RoleEntity.class));
		when(service.validateEntityRelations(any(RoleEntity.class))).thenReturn(new HashSet<>());
