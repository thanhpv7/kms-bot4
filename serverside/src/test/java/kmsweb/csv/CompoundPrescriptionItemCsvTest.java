...
import kmsweb.controllers.CompoundPrescriptionItemController;
...
@Tag("CSV")
public class CompoundPrescriptionItemCsvTest {

	@Mock
	private CompoundPrescriptionItemService service;
	...
	private CompoundPrescriptionItemController controller;

	private CsvUtils<CompoundPrescriptionItemEntity, CompoundPrescriptionItemRepository, CompoundPrescriptionItemEntityAudit, CompoundPrescriptionItemService> csvUtils;

	private final CompoundPrescriptionItemFactory factory = new CompoundPrescriptionItemFactory();
	...
	public void setup() {
		...
		this.controller = new CompoundPrescriptionItemController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CompoundPrescriptionItemEntity.class);
	}

	@Test
	void whenExportingCompoundPrescriptionItemEntities_thenHeadersAreValid() throws Exception {
		List<CompoundPrescriptionItemEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CompoundPrescriptionItemEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCompoundPrescriptionItemEntities_thenEntitiesAreValid() throws Exception {
		List<CompoundPrescriptionItemEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CompoundPrescriptionItemEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CompoundPrescriptionItemEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CompoundPrescriptionItemEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CompoundPrescriptionItemEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CompoundPrescriptionItemEntity.class));
		when(service.validateEntityRelations(any(CompoundPrescriptionItemEntity.class))).thenReturn(new HashSet<>());
