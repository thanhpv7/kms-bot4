...
import kmsweb.controllers.TariffDefinitionOtherComponentController;
...
@Tag("CSV")
public class TariffDefinitionOtherComponentCsvTest {

	@Mock
	private TariffDefinitionOtherComponentService service;
	...
	private TariffDefinitionOtherComponentController controller;

	private CsvUtils<TariffDefinitionOtherComponentEntity, TariffDefinitionOtherComponentRepository, TariffDefinitionOtherComponentEntityAudit, TariffDefinitionOtherComponentService> csvUtils;

	private final TariffDefinitionOtherComponentFactory factory = new TariffDefinitionOtherComponentFactory();
	...
	public void setup() {
		...
		this.controller = new TariffDefinitionOtherComponentController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TariffDefinitionOtherComponentEntity.class);
	}

	@Test
	void whenExportingTariffDefinitionOtherComponentEntities_thenHeadersAreValid() throws Exception {
		List<TariffDefinitionOtherComponentEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TariffDefinitionOtherComponentEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTariffDefinitionOtherComponentEntities_thenEntitiesAreValid() throws Exception {
		List<TariffDefinitionOtherComponentEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TariffDefinitionOtherComponentEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TariffDefinitionOtherComponentEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TariffDefinitionOtherComponentEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TariffDefinitionOtherComponentEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TariffDefinitionOtherComponentEntity.class));
		when(service.validateEntityRelations(any(TariffDefinitionOtherComponentEntity.class))).thenReturn(new HashSet<>());
