...
import kmsweb.controllers.PcareActionExaminationController;
...
@Tag("CSV")
public class PcareActionExaminationCsvTest {

	@Mock
	private PcareActionExaminationService service;
	...
	private PcareActionExaminationController controller;

	private CsvUtils<PcareActionExaminationEntity, PcareActionExaminationRepository, PcareActionExaminationEntityAudit, PcareActionExaminationService> csvUtils;

	private final PcareActionExaminationFactory factory = new PcareActionExaminationFactory();
	...
	public void setup() {
		...
		this.controller = new PcareActionExaminationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareActionExaminationEntity.class);
	}

	@Test
	void whenExportingPcareActionExaminationEntities_thenHeadersAreValid() throws Exception {
		List<PcareActionExaminationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareActionExaminationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareActionExaminationEntities_thenEntitiesAreValid() throws Exception {
		List<PcareActionExaminationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getActionCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFee()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKdtindakansk())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHasil()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareActionExaminationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareActionExaminationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareActionExaminationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareActionExaminationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareActionExaminationEntity.class));
		when(service.validateEntityRelations(any(PcareActionExaminationEntity.class))).thenReturn(new HashSet<>());
