...
import kmsweb.controllers.PatientPaymentBPJSController;
...
@Tag("CSV")
public class PatientPaymentBPJSCsvTest {

	@Mock
	private PatientPaymentBPJSService service;
	...
	private PatientPaymentBPJSController controller;

	private CsvUtils<PatientPaymentBPJSEntity, PatientPaymentBPJSRepository, PatientPaymentBPJSEntityAudit, PatientPaymentBPJSService> csvUtils;

	private final PatientPaymentBPJSFactory factory = new PatientPaymentBPJSFactory();
	...
	public void setup() {
		...
		this.controller = new PatientPaymentBPJSController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientPaymentBPJSEntity.class);
	}

	@Test
	void whenExportingPatientPaymentBPJSEntities_thenHeadersAreValid() throws Exception {
		List<PatientPaymentBPJSEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentBPJSEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientPaymentBPJSEntities_thenEntitiesAreValid() throws Exception {
		List<PatientPaymentBPJSEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentBPJSEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientPaymentBPJSEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientPaymentBPJSEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientPaymentBPJSEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientPaymentBPJSEntity.class));
		when(service.validateEntityRelations(any(PatientPaymentBPJSEntity.class))).thenReturn(new HashSet<>());
