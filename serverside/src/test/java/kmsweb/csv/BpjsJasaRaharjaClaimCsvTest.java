...
import kmsweb.controllers.BpjsJasaRaharjaClaimController;
...
@Tag("CSV")
public class BpjsJasaRaharjaClaimCsvTest {

	@Mock
	private BpjsJasaRaharjaClaimService service;
	...
	private BpjsJasaRaharjaClaimController controller;

	private CsvUtils<BpjsJasaRaharjaClaimEntity, BpjsJasaRaharjaClaimRepository, BpjsJasaRaharjaClaimEntityAudit, BpjsJasaRaharjaClaimService> csvUtils;

	private final BpjsJasaRaharjaClaimFactory factory = new BpjsJasaRaharjaClaimFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsJasaRaharjaClaimController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsJasaRaharjaClaimEntity.class);
	}

	@Test
	void whenExportingBpjsJasaRaharjaClaimEntities_thenHeadersAreValid() throws Exception {
		List<BpjsJasaRaharjaClaimEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsJasaRaharjaClaimEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsJasaRaharjaClaimEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsJasaRaharjaClaimEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIncidentDate()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsJasaRaharjaClaimEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsJasaRaharjaClaimEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsJasaRaharjaClaimEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsJasaRaharjaClaimEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsJasaRaharjaClaimEntity.class));
		when(service.validateEntityRelations(any(BpjsJasaRaharjaClaimEntity.class))).thenReturn(new HashSet<>());
