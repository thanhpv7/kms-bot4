...
import kmsweb.controllers.PatientContactInfoController;
...
@Tag("CSV")
public class PatientContactInfoCsvTest {
	...
	private PatientContactInfoController controller;

	private CsvUtils<PatientContactInfoEntity, PatientContactInfoRepository, PatientContactInfoEntityAudit, PatientContactInfoService> csvUtils;

	private final PatientContactInfoFactory factory = new PatientContactInfoFactory();
	...
	public void setup() {
		...
		this.controller = new PatientContactInfoController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientContactInfoEntity.class);
	}

	@Test
	void whenExportingPatientContactInfoEntities_thenHeadersAreValid() throws Exception {
		List<PatientContactInfoEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientContactInfoEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientContactInfoEntities_thenEntitiesAreValid() throws Exception {
		List<PatientContactInfoEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientContactInfoEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientContactInfoEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientContactInfoEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientContactInfoEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientContactInfoEntity.class));
		when(service.validateEntityRelations(any(PatientContactInfoEntity.class))).thenReturn(new HashSet<>());
