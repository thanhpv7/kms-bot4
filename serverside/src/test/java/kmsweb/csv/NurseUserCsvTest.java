...
import kmsweb.controllers.NurseUserController;
...
@Tag("CSV")
public class NurseUserCsvTest {

	@Mock
	private NurseUserService service;
	...
	private NurseUserController controller;

	private CsvUtils<NurseUserEntity, NurseUserRepository, NurseUserEntityAudit, NurseUserService> csvUtils;

	private final NurseUserFactory factory = new NurseUserFactory();
	...
	public void setup() {
		...
		this.controller = new NurseUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NurseUserEntity.class);
	}

	@Test
	void whenExportingNurseUserEntities_thenHeadersAreValid() throws Exception {
		List<NurseUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NurseUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNurseUserEntities_thenEntitiesAreValid() throws Exception {
		List<NurseUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NurseUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NurseUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NurseUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NurseUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NurseUserEntity.class));
		when(service.validateEntityRelations(any(NurseUserEntity.class))).thenReturn(new HashSet<>());
