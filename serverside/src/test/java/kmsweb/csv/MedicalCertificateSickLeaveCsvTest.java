...
import kmsweb.controllers.MedicalCertificateSickLeaveController;
...
@Tag("CSV")
public class MedicalCertificateSickLeaveCsvTest {

	@Mock
	private MedicalCertificateSickLeaveService service;
	...
	private MedicalCertificateSickLeaveController controller;

	private CsvUtils<MedicalCertificateSickLeaveEntity, MedicalCertificateSickLeaveRepository, MedicalCertificateSickLeaveEntityAudit, MedicalCertificateSickLeaveService> csvUtils;

	private final MedicalCertificateSickLeaveFactory factory = new MedicalCertificateSickLeaveFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalCertificateSickLeaveController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalCertificateSickLeaveEntity.class);
	}

	@Test
	void whenExportingMedicalCertificateSickLeaveEntities_thenHeadersAreValid() throws Exception {
		List<MedicalCertificateSickLeaveEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateSickLeaveEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalCertificateSickLeaveEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalCertificateSickLeaveEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateSickLeaveEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalCertificateSickLeaveEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalCertificateSickLeaveEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalCertificateSickLeaveEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalCertificateSickLeaveEntity.class));
		when(service.validateEntityRelations(any(MedicalCertificateSickLeaveEntity.class))).thenReturn(new HashSet<>());
