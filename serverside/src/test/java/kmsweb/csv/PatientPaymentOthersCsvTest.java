...
import kmsweb.controllers.PatientPaymentOthersController;
...
@Tag("CSV")
public class PatientPaymentOthersCsvTest {

	@Mock
	private PatientPaymentOthersService service;
	...
	private PatientPaymentOthersController controller;

	private CsvUtils<PatientPaymentOthersEntity, PatientPaymentOthersRepository, PatientPaymentOthersEntityAudit, PatientPaymentOthersService> csvUtils;

	private final PatientPaymentOthersFactory factory = new PatientPaymentOthersFactory();
	...
	public void setup() {
		...
		this.controller = new PatientPaymentOthersController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientPaymentOthersEntity.class);
	}

	@Test
	void whenExportingPatientPaymentOthersEntities_thenHeadersAreValid() throws Exception {
		List<PatientPaymentOthersEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentOthersEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientPaymentOthersEntities_thenEntitiesAreValid() throws Exception {
		List<PatientPaymentOthersEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentOthersEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientPaymentOthersEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientPaymentOthersEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientPaymentOthersEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientPaymentOthersEntity.class));
		when(service.validateEntityRelations(any(PatientPaymentOthersEntity.class))).thenReturn(new HashSet<>());
