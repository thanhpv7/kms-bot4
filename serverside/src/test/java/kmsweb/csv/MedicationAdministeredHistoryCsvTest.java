...
import kmsweb.controllers.MedicationAdministeredHistoryController;
...
@Tag("CSV")
public class MedicationAdministeredHistoryCsvTest {

	@Mock
	private MedicationAdministeredHistoryService service;
	...
	private MedicationAdministeredHistoryController controller;

	private CsvUtils<MedicationAdministeredHistoryEntity, MedicationAdministeredHistoryRepository, MedicationAdministeredHistoryEntityAudit, MedicationAdministeredHistoryService> csvUtils;

	private final MedicationAdministeredHistoryFactory factory = new MedicationAdministeredHistoryFactory();
	...
	public void setup() {
		...
		this.controller = new MedicationAdministeredHistoryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicationAdministeredHistoryEntity.class);
	}

	@Test
	void whenExportingMedicationAdministeredHistoryEntities_thenHeadersAreValid() throws Exception {
		List<MedicationAdministeredHistoryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicationAdministeredHistoryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicationAdministeredHistoryEntities_thenEntitiesAreValid() throws Exception {
		List<MedicationAdministeredHistoryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAdministrationNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAdministeredQuantity()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTakenHomeQuantity()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReturnedQuantity()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicationAdministeredHistoryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicationAdministeredHistoryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicationAdministeredHistoryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicationAdministeredHistoryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicationAdministeredHistoryEntity.class));
		when(service.validateEntityRelations(any(MedicationAdministeredHistoryEntity.class))).thenReturn(new HashSet<>());
