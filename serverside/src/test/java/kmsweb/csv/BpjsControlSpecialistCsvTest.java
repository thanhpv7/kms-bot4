...
import kmsweb.controllers.BpjsControlSpecialistController;
...
@Tag("CSV")
public class BpjsControlSpecialistCsvTest {

	@Mock
	private BpjsControlSpecialistService service;
	...
	private BpjsControlSpecialistController controller;

	private CsvUtils<BpjsControlSpecialistEntity, BpjsControlSpecialistRepository, BpjsControlSpecialistEntityAudit, BpjsControlSpecialistService> csvUtils;

	private final BpjsControlSpecialistFactory factory = new BpjsControlSpecialistFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsControlSpecialistController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsControlSpecialistEntity.class);
	}

	@Test
	void whenExportingBpjsControlSpecialistEntities_thenHeadersAreValid() throws Exception {
		List<BpjsControlSpecialistEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsControlSpecialistEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsControlSpecialistEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsControlSpecialistEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSpecialistCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSpecialistName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCapacity())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalControlPlanReferral())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPercentage())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsControlSpecialistEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsControlSpecialistEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsControlSpecialistEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsControlSpecialistEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsControlSpecialistEntity.class));
		when(service.validateEntityRelations(any(BpjsControlSpecialistEntity.class))).thenReturn(new HashSet<>());
