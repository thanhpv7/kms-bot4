...
import kmsweb.controllers.PriceComponentController;
...
@Tag("CSV")
public class PriceComponentCsvTest {

	@Mock
	private PriceComponentService service;
	...
	private PriceComponentController controller;

	private CsvUtils<PriceComponentEntity, PriceComponentRepository, PriceComponentEntityAudit, PriceComponentService> csvUtils;

	private final PriceComponentFactory factory = new PriceComponentFactory();
	...
	public void setup() {
		...
		this.controller = new PriceComponentController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PriceComponentEntity.class);
	}

	@Test
	void whenExportingPriceComponentEntities_thenHeadersAreValid() throws Exception {
		List<PriceComponentEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PriceComponentEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPriceComponentEntities_thenEntitiesAreValid() throws Exception {
		List<PriceComponentEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getComponentCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getComponentName())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PriceComponentEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PriceComponentEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PriceComponentEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PriceComponentEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PriceComponentEntity.class));
		when(service.validateEntityRelations(any(PriceComponentEntity.class))).thenReturn(new HashSet<>());
