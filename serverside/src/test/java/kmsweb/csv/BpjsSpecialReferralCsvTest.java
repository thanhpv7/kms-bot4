...
import kmsweb.controllers.BpjsSpecialReferralController;
...
@Tag("CSV")
public class BpjsSpecialReferralCsvTest {

	@Mock
	private BpjsSpecialReferralService service;
	...
	private BpjsSpecialReferralController controller;

	private CsvUtils<BpjsSpecialReferralEntity, BpjsSpecialReferralRepository, BpjsSpecialReferralEntityAudit, BpjsSpecialReferralService> csvUtils;

	private final BpjsSpecialReferralFactory factory = new BpjsSpecialReferralFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsSpecialReferralController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsSpecialReferralEntity.class);
	}

	@Test
	void whenExportingBpjsSpecialReferralEntities_thenHeadersAreValid() throws Exception {
		List<BpjsSpecialReferralEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSpecialReferralEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsSpecialReferralEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsSpecialReferralEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferralID())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPpkDiagnose())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferralStartDate())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferralEndDate())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSpecialReferralEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsSpecialReferralEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsSpecialReferralEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsSpecialReferralEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsSpecialReferralEntity.class));
		when(service.validateEntityRelations(any(BpjsSpecialReferralEntity.class))).thenReturn(new HashSet<>());
