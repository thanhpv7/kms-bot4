...
import kmsweb.controllers.SampleCollectionItemsController;
...
@Tag("CSV")
public class SampleCollectionItemsCsvTest {

	@Mock
	private SampleCollectionItemsService service;
	...
	private SampleCollectionItemsController controller;

	private CsvUtils<SampleCollectionItemsEntity, SampleCollectionItemsRepository, SampleCollectionItemsEntityAudit, SampleCollectionItemsService> csvUtils;

	private final SampleCollectionItemsFactory factory = new SampleCollectionItemsFactory();
	...
	public void setup() {
		...
		this.controller = new SampleCollectionItemsController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SampleCollectionItemsEntity.class);
	}

	@Test
	void whenExportingSampleCollectionItemsEntities_thenHeadersAreValid() throws Exception {
		List<SampleCollectionItemsEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SampleCollectionItemsEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSampleCollectionItemsEntities_thenEntitiesAreValid() throws Exception {
		List<SampleCollectionItemsEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleTakingLocation())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVolume())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getContainer())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTransport())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleCollectionDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleProcessingDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleDeliveryDateTime()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SampleCollectionItemsEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SampleCollectionItemsEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SampleCollectionItemsEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SampleCollectionItemsEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SampleCollectionItemsEntity.class));
		when(service.validateEntityRelations(any(SampleCollectionItemsEntity.class))).thenReturn(new HashSet<>());
