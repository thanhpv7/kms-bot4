...
import kmsweb.controllers.Icd10Controller;
...
public class Icd10CsvTest {
	...
	private Icd10Controller controller;

	private CsvUtils<Icd10Entity, Icd10Repository, Icd10EntityAudit, Icd10Service> csvUtils;

	private final Icd10Factory factory = new Icd10Factory();
	...
	public void setup() {
		...
		this.controller = new Icd10Controller(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, Icd10Entity.class);
	}

	@Test
	void whenExportingIcd10Entities_thenHeadersAreValid() throws Exception {
		List<Icd10Entity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(Icd10Entity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingIcd10Entities_thenEntitiesAreValid() throws Exception {
		List<Icd10Entity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassificationDisease()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassificationInfectious()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassificationChronic()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassificationTopDisease()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassificationRestrictedCase()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassificationReportableCase()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassificationAcute()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(Icd10Entity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		Icd10Entity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(Icd10Entity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(Icd10Entity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(Icd10Entity.class));
		when(service.validateEntityRelations(any(Icd10Entity.class))).thenReturn(new HashSet<>());
