...
import kmsweb.controllers.PatientVitalInformationController;
...
@Tag("CSV")
public class PatientVitalInformationCsvTest {

	@Mock
	private PatientVitalInformationService service;
	...
	private PatientVitalInformationController controller;

	private CsvUtils<PatientVitalInformationEntity, PatientVitalInformationRepository, PatientVitalInformationEntityAudit, PatientVitalInformationService> csvUtils;

	private final PatientVitalInformationFactory factory = new PatientVitalInformationFactory();
	...
	public void setup() {
		...
		this.controller = new PatientVitalInformationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientVitalInformationEntity.class);
	}

	@Test
	void whenExportingPatientVitalInformationEntities_thenHeadersAreValid() throws Exception {
		List<PatientVitalInformationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientVitalInformationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientVitalInformationEntities_thenEntitiesAreValid() throws Exception {
		List<PatientVitalInformationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPulseRhythm())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getStrength())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRespiratory())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientVitalInformationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientVitalInformationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientVitalInformationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientVitalInformationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientVitalInformationEntity.class));
		when(service.validateEntityRelations(any(PatientVitalInformationEntity.class))).thenReturn(new HashSet<>());
