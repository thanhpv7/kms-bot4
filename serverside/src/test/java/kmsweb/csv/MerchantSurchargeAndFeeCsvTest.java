...
import kmsweb.controllers.MerchantSurchargeAndFeeController;
...
@Tag("CSV")
public class MerchantSurchargeAndFeeCsvTest {

	@Mock
	private MerchantSurchargeAndFeeService service;
	...
	private MerchantSurchargeAndFeeController controller;

	private CsvUtils<MerchantSurchargeAndFeeEntity, MerchantSurchargeAndFeeRepository, MerchantSurchargeAndFeeEntityAudit, MerchantSurchargeAndFeeService> csvUtils;

	private final MerchantSurchargeAndFeeFactory factory = new MerchantSurchargeAndFeeFactory();
	...
	public void setup() {
		...
		this.controller = new MerchantSurchargeAndFeeController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MerchantSurchargeAndFeeEntity.class);
	}

	@Test
	void whenExportingMerchantSurchargeAndFeeEntities_thenHeadersAreValid() throws Exception {
		List<MerchantSurchargeAndFeeEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MerchantSurchargeAndFeeEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMerchantSurchargeAndFeeEntities_thenEntitiesAreValid() throws Exception {
		List<MerchantSurchargeAndFeeEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSurcharge()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFee())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MerchantSurchargeAndFeeEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MerchantSurchargeAndFeeEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MerchantSurchargeAndFeeEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MerchantSurchargeAndFeeEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MerchantSurchargeAndFeeEntity.class));
		when(service.validateEntityRelations(any(MerchantSurchargeAndFeeEntity.class))).thenReturn(new HashSet<>());
