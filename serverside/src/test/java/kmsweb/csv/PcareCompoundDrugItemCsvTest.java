...
import kmsweb.controllers.PcareCompoundDrugItemController;
...
@Tag("CSV")
public class PcareCompoundDrugItemCsvTest {

	@Mock
	private PcareCompoundDrugItemService service;
	...
	private PcareCompoundDrugItemController controller;

	private CsvUtils<PcareCompoundDrugItemEntity, PcareCompoundDrugItemRepository, PcareCompoundDrugItemEntityAudit, PcareCompoundDrugItemService> csvUtils;

	private final PcareCompoundDrugItemFactory factory = new PcareCompoundDrugItemFactory();
	...
	public void setup() {
		...
		this.controller = new PcareCompoundDrugItemController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareCompoundDrugItemEntity.class);
	}

	@Test
	void whenExportingPcareCompoundDrugItemEntities_thenHeadersAreValid() throws Exception {
		List<PcareCompoundDrugItemEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareCompoundDrugItemEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareCompoundDrugItemEntities_thenEntitiesAreValid() throws Exception {
		List<PcareCompoundDrugItemEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRequest()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareCompoundDrugItemEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareCompoundDrugItemEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareCompoundDrugItemEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareCompoundDrugItemEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareCompoundDrugItemEntity.class));
		when(service.validateEntityRelations(any(PcareCompoundDrugItemEntity.class))).thenReturn(new HashSet<>());
