...
import kmsweb.controllers.DiagnosisExaminationRecordController;
...
@Tag("CSV")
public class DiagnosisExaminationRecordCsvTest {

	@Mock
	private DiagnosisExaminationRecordService service;
	...
	private DiagnosisExaminationRecordController controller;

	private CsvUtils<DiagnosisExaminationRecordEntity, DiagnosisExaminationRecordRepository, DiagnosisExaminationRecordEntityAudit, DiagnosisExaminationRecordService> csvUtils;

	private final DiagnosisExaminationRecordFactory factory = new DiagnosisExaminationRecordFactory();
	...
	public void setup() {
		...
		this.controller = new DiagnosisExaminationRecordController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DiagnosisExaminationRecordEntity.class);
	}

	@Test
	void whenExportingDiagnosisExaminationRecordEntities_thenHeadersAreValid() throws Exception {
		List<DiagnosisExaminationRecordEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosisExaminationRecordEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDiagnosisExaminationRecordEntities_thenEntitiesAreValid() throws Exception {
		List<DiagnosisExaminationRecordEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDiagnosisSequence()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDiagnosisNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getActionNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNewCase()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPrintingNumber()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosisExaminationRecordEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DiagnosisExaminationRecordEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DiagnosisExaminationRecordEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DiagnosisExaminationRecordEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DiagnosisExaminationRecordEntity.class));
		when(service.validateEntityRelations(any(DiagnosisExaminationRecordEntity.class))).thenReturn(new HashSet<>());
