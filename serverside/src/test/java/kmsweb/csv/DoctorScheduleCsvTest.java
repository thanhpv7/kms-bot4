...
import kmsweb.controllers.DoctorScheduleController;
...
@Tag("CSV")
public class DoctorScheduleCsvTest {
	...
	private CsvUtils<DoctorScheduleEntity, DoctorScheduleRepository, DoctorScheduleEntityAudit, DoctorScheduleService> csvUtils;

	private final DoctorScheduleFactory factory = new DoctorScheduleFactory();
	...
	public void setup() {
		...
		this.controller = new DoctorScheduleController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DoctorScheduleEntity.class);
	}

	@Test
	void whenExportingDoctorScheduleEntities_thenHeadersAreValid() throws Exception {
		List<DoctorScheduleEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DoctorScheduleEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDoctorScheduleEntities_thenEntitiesAreValid() throws Exception {
		List<DoctorScheduleEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIntervalInMinutes()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLocation())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSchedulerType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMonday()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTuesday()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getWednesday()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getThursday()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFriday()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSaturday()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSunday()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DoctorScheduleEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DoctorScheduleEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DoctorScheduleEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DoctorScheduleEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DoctorScheduleEntity.class));
		when(service.validateEntityRelations(any(DoctorScheduleEntity.class))).thenReturn(new HashSet<>());
