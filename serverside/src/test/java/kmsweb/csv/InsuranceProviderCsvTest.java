...
import kmsweb.controllers.InsuranceProviderController;
...
@Tag("CSV")
public class InsuranceProviderCsvTest {
	...
	private InsuranceProviderController controller;

	private CsvUtils<InsuranceProviderEntity, InsuranceProviderRepository, InsuranceProviderEntityAudit, InsuranceProviderService> csvUtils;

	private final InsuranceProviderFactory factory = new InsuranceProviderFactory();
	...
	public void setup() {
		...
		this.controller = new InsuranceProviderController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InsuranceProviderEntity.class);
	}

	@Test
	void whenExportingInsuranceProviderEntities_thenHeadersAreValid() throws Exception {
		List<InsuranceProviderEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InsuranceProviderEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInsuranceProviderEntities_thenEntitiesAreValid() throws Exception {
		List<InsuranceProviderEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProviderCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProviderName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProviderType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBenefitProvider())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExpiryDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExpiringDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCreatedDate()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InsuranceProviderEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InsuranceProviderEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InsuranceProviderEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InsuranceProviderEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InsuranceProviderEntity.class));
		when(service.validateEntityRelations(any(InsuranceProviderEntity.class))).thenReturn(new HashSet<>());
