...
import kmsweb.controllers.MedicalCertificateDischargeResumeController;
...
@Tag("CSV")
public class MedicalCertificateDischargeResumeCsvTest {

	@Mock
	private MedicalCertificateDischargeResumeService service;
	...
	private MedicalCertificateDischargeResumeController controller;

	private CsvUtils<MedicalCertificateDischargeResumeEntity, MedicalCertificateDischargeResumeRepository, MedicalCertificateDischargeResumeEntityAudit, MedicalCertificateDischargeResumeService> csvUtils;

	private final MedicalCertificateDischargeResumeFactory factory = new MedicalCertificateDischargeResumeFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalCertificateDischargeResumeController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalCertificateDischargeResumeEntity.class);
	}

	@Test
	void whenExportingMedicalCertificateDischargeResumeEntities_thenHeadersAreValid() throws Exception {
		List<MedicalCertificateDischargeResumeEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateDischargeResumeEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalCertificateDischargeResumeEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalCertificateDischargeResumeEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMainDiagnosis())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAction())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateDischargeResumeEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalCertificateDischargeResumeEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalCertificateDischargeResumeEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalCertificateDischargeResumeEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalCertificateDischargeResumeEntity.class));
		when(service.validateEntityRelations(any(MedicalCertificateDischargeResumeEntity.class))).thenReturn(new HashSet<>());
