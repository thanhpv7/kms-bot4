...
import kmsweb.controllers.LibraryController;
...
public class LibraryCsvTest {
	...
	private CsvUtils<LibraryEntity, LibraryRepository, LibraryEntityAudit, LibraryService> csvUtils;

	private final LibraryFactory factory = new LibraryFactory();
	...
	public void setup() {
		...
		this.controller = new LibraryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, LibraryEntity.class);
	}

	@Test
	void whenExportingLibraryEntities_thenHeadersAreValid() throws Exception {
		List<LibraryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(LibraryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingLibraryEntities_thenEntitiesAreValid() throws Exception {
		List<LibraryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLibraryDocument()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLibraryItemCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLibraryItemDescription())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClassification())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAuthor())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPublisher())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(LibraryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		LibraryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(LibraryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(LibraryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(LibraryEntity.class));
		when(service.validateEntityRelations(any(LibraryEntity.class))).thenReturn(new HashSet<>());
