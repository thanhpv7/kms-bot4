...
import kmsweb.controllers.BpjsPcareKesadaranController;
...
@Tag("CSV")
public class BpjsPcareKesadaranCsvTest {

	@Mock
	private BpjsPcareKesadaranService service;
	...
	private BpjsPcareKesadaranController controller;

	private CsvUtils<BpjsPcareKesadaranEntity, BpjsPcareKesadaranRepository, BpjsPcareKesadaranEntityAudit, BpjsPcareKesadaranService> csvUtils;

	private final BpjsPcareKesadaranFactory factory = new BpjsPcareKesadaranFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPcareKesadaranController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPcareKesadaranEntity.class);
	}

	@Test
	void whenExportingBpjsPcareKesadaranEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPcareKesadaranEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareKesadaranEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPcareKesadaranEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPcareKesadaranEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKdsadar())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNmsadar())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKesadaranInternal())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareKesadaranEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPcareKesadaranEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPcareKesadaranEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPcareKesadaranEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPcareKesadaranEntity.class));
		when(service.validateEntityRelations(any(BpjsPcareKesadaranEntity.class))).thenReturn(new HashSet<>());
