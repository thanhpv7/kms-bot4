...
import kmsweb.controllers.PurchaseRequisitionController;
...
@Tag("CSV")
public class PurchaseRequisitionCsvTest {
	...
	private CsvUtils<PurchaseRequisitionEntity, PurchaseRequisitionRepository, PurchaseRequisitionEntityAudit, PurchaseRequisitionService> csvUtils;

	private final PurchaseRequisitionFactory factory = new PurchaseRequisitionFactory();
	...
	public void setup() {
		...
		this.controller = new PurchaseRequisitionController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PurchaseRequisitionEntity.class);
	}

	@Test
	void whenExportingPurchaseRequisitionEntities_thenHeadersAreValid() throws Exception {
		List<PurchaseRequisitionEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PurchaseRequisitionEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPurchaseRequisitionEntities_thenEntitiesAreValid() throws Exception {
		List<PurchaseRequisitionEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPurchaseRequisitionNumber())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRequiredDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalEstimationOrder()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRedraftingDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSubmissionDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getApprovalDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRejectionDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVoidingDateTime()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PurchaseRequisitionEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PurchaseRequisitionEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PurchaseRequisitionEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PurchaseRequisitionEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PurchaseRequisitionEntity.class));
		when(service.validateEntityRelations(any(PurchaseRequisitionEntity.class))).thenReturn(new HashSet<>());
