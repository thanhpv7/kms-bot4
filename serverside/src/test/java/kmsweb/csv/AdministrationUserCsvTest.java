...
import kmsweb.controllers.AdministrationUserController;
...
@Tag("CSV")
public class AdministrationUserCsvTest {
	...
	private AdministrationUserController controller;
	...
	private final AdministrationUserFactory factory = new AdministrationUserFactory();
	...
	public void setup() {
		...
		this.controller = new AdministrationUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, AdministrationUserEntity.class);
	}

	@Test
	void whenExportingAdministrationUserEntities_thenHeadersAreValid() throws Exception {
		List<AdministrationUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(AdministrationUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingAdministrationUserEntities_thenEntitiesAreValid() throws Exception {
		List<AdministrationUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(AdministrationUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		AdministrationUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(AdministrationUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(AdministrationUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(AdministrationUserEntity.class));
		when(service.validateEntityRelations(any(AdministrationUserEntity.class))).thenReturn(new HashSet<>());
