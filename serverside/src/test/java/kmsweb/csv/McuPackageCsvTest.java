...
import kmsweb.controllers.McuPackageController;
...
@Tag("CSV")
public class McuPackageCsvTest {

	@Mock
	private McuPackageService service;
	...
	private McuPackageController controller;

	private CsvUtils<McuPackageEntity, McuPackageRepository, McuPackageEntityAudit, McuPackageService> csvUtils;

	private final McuPackageFactory factory = new McuPackageFactory();
	...
	public void setup() {
		...
		this.controller = new McuPackageController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, McuPackageEntity.class);
	}

	@Test
	void whenExportingMcuPackageEntities_thenHeadersAreValid() throws Exception {
		List<McuPackageEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(McuPackageEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMcuPackageEntities_thenEntitiesAreValid() throws Exception {
		List<McuPackageEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMcuPackageCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDescription())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPackageType())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(McuPackageEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		McuPackageEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(McuPackageEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(McuPackageEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(McuPackageEntity.class));
		when(service.validateEntityRelations(any(McuPackageEntity.class))).thenReturn(new HashSet<>());
