...
import kmsweb.controllers.PatientDetailController;
...
@Tag("CSV")
public class PatientDetailCsvTest {
	...
	private CsvUtils<PatientDetailEntity, PatientDetailRepository, PatientDetailEntityAudit, PatientDetailService> csvUtils;

	private final PatientDetailFactory factory = new PatientDetailFactory();
	...
	public void setup() {
		...
		this.controller = new PatientDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientDetailEntity.class);
	}

	@Test
	void whenExportingPatientDetailEntities_thenHeadersAreValid() throws Exception {
		List<PatientDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientDetailEntities_thenEntitiesAreValid() throws Exception {
		List<PatientDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getActivePatientID())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientClassification())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientType())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientDetailEntity.class));
		when(service.validateEntityRelations(any(PatientDetailEntity.class))).thenReturn(new HashSet<>());
