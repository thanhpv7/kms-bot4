...
import kmsweb.controllers.DiagnosticStaffExaminationSummaryController;
...
@Tag("CSV")
public class DiagnosticStaffExaminationSummaryCsvTest {

	@Mock
	private DiagnosticStaffExaminationSummaryService service;
	...
	private DiagnosticStaffExaminationSummaryController controller;

	private CsvUtils<DiagnosticStaffExaminationSummaryEntity, DiagnosticStaffExaminationSummaryRepository, DiagnosticStaffExaminationSummaryEntityAudit, DiagnosticStaffExaminationSummaryService> csvUtils;

	private final DiagnosticStaffExaminationSummaryFactory factory = new DiagnosticStaffExaminationSummaryFactory();
	...
	public void setup() {
		...
		this.controller = new DiagnosticStaffExaminationSummaryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DiagnosticStaffExaminationSummaryEntity.class);
	}

	@Test
	void whenExportingDiagnosticStaffExaminationSummaryEntities_thenHeadersAreValid() throws Exception {
		List<DiagnosticStaffExaminationSummaryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticStaffExaminationSummaryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDiagnosticStaffExaminationSummaryEntities_thenEntitiesAreValid() throws Exception {
		List<DiagnosticStaffExaminationSummaryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMedicalTranscriber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferringRoom())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferingUnit())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExaminationLocation())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAdditionalNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getConclusion())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticStaffExaminationSummaryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DiagnosticStaffExaminationSummaryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DiagnosticStaffExaminationSummaryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DiagnosticStaffExaminationSummaryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DiagnosticStaffExaminationSummaryEntity.class));
		when(service.validateEntityRelations(any(DiagnosticStaffExaminationSummaryEntity.class))).thenReturn(new HashSet<>());
