...
import kmsweb.controllers.InvoicePaymentInsuranceController;
...
@Tag("CSV")
public class InvoicePaymentInsuranceCsvTest {

	@Mock
	private InvoicePaymentInsuranceService service;
	...
	private InvoicePaymentInsuranceController controller;

	private CsvUtils<InvoicePaymentInsuranceEntity, InvoicePaymentInsuranceRepository, InvoicePaymentInsuranceEntityAudit, InvoicePaymentInsuranceService> csvUtils;

	private final InvoicePaymentInsuranceFactory factory = new InvoicePaymentInsuranceFactory();
	...
	public void setup() {
		...
		this.controller = new InvoicePaymentInsuranceController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InvoicePaymentInsuranceEntity.class);
	}

	@Test
	void whenExportingInvoicePaymentInsuranceEntities_thenHeadersAreValid() throws Exception {
		List<InvoicePaymentInsuranceEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InvoicePaymentInsuranceEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInvoicePaymentInsuranceEntities_thenEntitiesAreValid() throws Exception {
		List<InvoicePaymentInsuranceEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMainInsurance()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InvoicePaymentInsuranceEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InvoicePaymentInsuranceEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InvoicePaymentInsuranceEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InvoicePaymentInsuranceEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InvoicePaymentInsuranceEntity.class));
		when(service.validateEntityRelations(any(InvoicePaymentInsuranceEntity.class))).thenReturn(new HashSet<>());
