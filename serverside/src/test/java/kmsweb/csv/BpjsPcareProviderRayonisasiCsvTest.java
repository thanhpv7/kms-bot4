...
import kmsweb.controllers.BpjsPcareProviderRayonisasiController;
...
@Tag("CSV")
public class BpjsPcareProviderRayonisasiCsvTest {

	@Mock
	private BpjsPcareProviderRayonisasiService service;
	...
	private BpjsPcareProviderRayonisasiController controller;

	private CsvUtils<BpjsPcareProviderRayonisasiEntity, BpjsPcareProviderRayonisasiRepository, BpjsPcareProviderRayonisasiEntityAudit, BpjsPcareProviderRayonisasiService> csvUtils;

	private final BpjsPcareProviderRayonisasiFactory factory = new BpjsPcareProviderRayonisasiFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPcareProviderRayonisasiController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPcareProviderRayonisasiEntity.class);
	}

	@Test
	void whenExportingBpjsPcareProviderRayonisasiEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPcareProviderRayonisasiEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareProviderRayonisasiEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPcareProviderRayonisasiEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPcareProviderRayonisasiEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareProviderRayonisasiEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPcareProviderRayonisasiEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPcareProviderRayonisasiEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPcareProviderRayonisasiEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPcareProviderRayonisasiEntity.class));
		when(service.validateEntityRelations(any(BpjsPcareProviderRayonisasiEntity.class))).thenReturn(new HashSet<>());
