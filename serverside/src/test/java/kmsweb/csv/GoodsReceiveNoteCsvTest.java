...
import kmsweb.controllers.GoodsReceiveNoteController;
...
@Tag("CSV")
public class GoodsReceiveNoteCsvTest {

	@Mock
	private GoodsReceiveNoteService service;
	...
	private GoodsReceiveNoteController controller;

	private CsvUtils<GoodsReceiveNoteEntity, GoodsReceiveNoteRepository, GoodsReceiveNoteEntityAudit, GoodsReceiveNoteService> csvUtils;

	private final GoodsReceiveNoteFactory factory = new GoodsReceiveNoteFactory();
	...
	public void setup() {
		...
		this.controller = new GoodsReceiveNoteController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, GoodsReceiveNoteEntity.class);
	}

	@Test
	void whenExportingGoodsReceiveNoteEntities_thenHeadersAreValid() throws Exception {
		List<GoodsReceiveNoteEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(GoodsReceiveNoteEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingGoodsReceiveNoteEntities_thenEntitiesAreValid() throws Exception {
		List<GoodsReceiveNoteEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReceiveNumber())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSourceDocumentNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReceiveDate()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSupplierInvoiceNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getShippingMethod())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getShippingNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getShippingDate()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(GoodsReceiveNoteEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		GoodsReceiveNoteEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(GoodsReceiveNoteEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(GoodsReceiveNoteEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(GoodsReceiveNoteEntity.class));
		when(service.validateEntityRelations(any(GoodsReceiveNoteEntity.class))).thenReturn(new HashSet<>());
