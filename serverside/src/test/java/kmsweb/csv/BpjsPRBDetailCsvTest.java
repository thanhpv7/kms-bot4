...
import kmsweb.controllers.BpjsPRBDetailController;
...
@Tag("CSV")
public class BpjsPRBDetailCsvTest {

	@Mock
	private BpjsPRBDetailService service;
	...
	private BpjsPRBDetailController controller;

	private CsvUtils<BpjsPRBDetailEntity, BpjsPRBDetailRepository, BpjsPRBDetailEntityAudit, BpjsPRBDetailService> csvUtils;

	private final BpjsPRBDetailFactory factory = new BpjsPRBDetailFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPRBDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPRBDetailEntity.class);
	}

	@Test
	void whenExportingBpjsPRBDetailEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPRBDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPRBDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPRBDetailEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPRBDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSrbNo())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSigna1())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSigna2())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getQty())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPRBDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPRBDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPRBDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPRBDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPRBDetailEntity.class));
		when(service.validateEntityRelations(any(BpjsPRBDetailEntity.class))).thenReturn(new HashSet<>());
