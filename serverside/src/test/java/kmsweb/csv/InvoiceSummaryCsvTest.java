...
import kmsweb.controllers.InvoiceSummaryController;
...
@Tag("CSV")
public class InvoiceSummaryCsvTest {
	...
	private InvoiceSummaryController controller;

	private CsvUtils<InvoiceSummaryEntity, InvoiceSummaryRepository, InvoiceSummaryEntityAudit, InvoiceSummaryService> csvUtils;

	private final InvoiceSummaryFactory factory = new InvoiceSummaryFactory();
	...
	public void setup() {
		...
		this.controller = new InvoiceSummaryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InvoiceSummaryEntity.class);
	}

	@Test
	void whenExportingInvoiceSummaryEntities_thenHeadersAreValid() throws Exception {
		List<InvoiceSummaryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceSummaryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInvoiceSummaryEntities_thenEntitiesAreValid() throws Exception {
		List<InvoiceSummaryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTaxOnOtherDiscount()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOtherDiscount()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTaxOnOtherFee()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOtherFee()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTax()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalInvoice()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAvailableDownPayment()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalCoverage()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBpjsCoverage()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceSummaryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InvoiceSummaryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InvoiceSummaryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InvoiceSummaryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InvoiceSummaryEntity.class));
		when(service.validateEntityRelations(any(InvoiceSummaryEntity.class))).thenReturn(new HashSet<>());
