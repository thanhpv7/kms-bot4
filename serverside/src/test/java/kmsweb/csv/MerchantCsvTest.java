...
import kmsweb.controllers.MerchantController;
...
public class MerchantCsvTest {
	...
	private CsvUtils<MerchantEntity, MerchantRepository, MerchantEntityAudit, MerchantService> csvUtils;

	private final MerchantFactory factory = new MerchantFactory();
	...
	public void setup() {
		...
		this.controller = new MerchantController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MerchantEntity.class);
	}

	@Test
	void whenExportingMerchantEntities_thenHeadersAreValid() throws Exception {
		List<MerchantEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MerchantEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMerchantEntities_thenEntitiesAreValid() throws Exception {
		List<MerchantEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMerchantName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMerchantNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBankAccount())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MerchantEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MerchantEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MerchantEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MerchantEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MerchantEntity.class));
		when(service.validateEntityRelations(any(MerchantEntity.class))).thenReturn(new HashSet<>());
