...
import kmsweb.controllers.Icd9CMController;
...
@Tag("CSV")
public class Icd9CMCsvTest {

	@Mock
	private Icd9CMService service;
	...
	private Icd9CMController controller;

	private CsvUtils<Icd9CMEntity, Icd9CMRepository, Icd9CMEntityAudit, Icd9CMService> csvUtils;

	private final Icd9CMFactory factory = new Icd9CMFactory();
	...
	public void setup() {
		...
		this.controller = new Icd9CMController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, Icd9CMEntity.class);
	}

	@Test
	void whenExportingIcd9CMEntities_thenHeadersAreValid() throws Exception {
		List<Icd9CMEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(Icd9CMEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingIcd9CMEntities_thenEntitiesAreValid() throws Exception {
		List<Icd9CMEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTopProcedure()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(Icd9CMEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		Icd9CMEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(Icd9CMEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(Icd9CMEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(Icd9CMEntity.class));
		when(service.validateEntityRelations(any(Icd9CMEntity.class))).thenReturn(new HashSet<>());
