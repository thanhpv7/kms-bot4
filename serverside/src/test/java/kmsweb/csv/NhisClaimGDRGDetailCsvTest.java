...
import kmsweb.controllers.NhisClaimGDRGDetailController;
...
@Tag("CSV")
public class NhisClaimGDRGDetailCsvTest {

	@Mock
	private NhisClaimGDRGDetailService service;
	...
	private NhisClaimGDRGDetailController controller;

	private CsvUtils<NhisClaimGDRGDetailEntity, NhisClaimGDRGDetailRepository, NhisClaimGDRGDetailEntityAudit, NhisClaimGDRGDetailService> csvUtils;

	private final NhisClaimGDRGDetailFactory factory = new NhisClaimGDRGDetailFactory();
	...
	public void setup() {
		...
		this.controller = new NhisClaimGDRGDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisClaimGDRGDetailEntity.class);
	}

	@Test
	void whenExportingNhisClaimGDRGDetailEntities_thenHeadersAreValid() throws Exception {
		List<NhisClaimGDRGDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisClaimGDRGDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisClaimGDRGDetailEntities_thenEntitiesAreValid() throws Exception {
		List<NhisClaimGDRGDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisClaimGDRGDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisClaimGDRGDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisClaimGDRGDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisClaimGDRGDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisClaimGDRGDetailEntity.class));
		when(service.validateEntityRelations(any(NhisClaimGDRGDetailEntity.class))).thenReturn(new HashSet<>());
