...
import kmsweb.controllers.CashAllocationController;
...
@Tag("CSV")
public class CashAllocationCsvTest {

	@Mock
	private CashAllocationService service;
	...
	private CashAllocationController controller;

	private CsvUtils<CashAllocationEntity, CashAllocationRepository, CashAllocationEntityAudit, CashAllocationService> csvUtils;

	private final CashAllocationFactory factory = new CashAllocationFactory();
	...
	public void setup() {
		...
		this.controller = new CashAllocationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CashAllocationEntity.class);
	}

	@Test
	void whenExportingCashAllocationEntities_thenHeadersAreValid() throws Exception {
		List<CashAllocationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CashAllocationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCashAllocationEntities_thenEntitiesAreValid() throws Exception {
		List<CashAllocationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCashType()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAllocatedDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSourceDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCashNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSourceDocumentType()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSourceNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSourceAmount()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CashAllocationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CashAllocationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CashAllocationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CashAllocationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CashAllocationEntity.class));
		when(service.validateEntityRelations(any(CashAllocationEntity.class))).thenReturn(new HashSet<>());
