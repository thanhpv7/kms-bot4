...
import kmsweb.controllers.BpjsSEPINACBGIntegrationController;
...
@Tag("CSV")
public class BpjsSEPINACBGIntegrationCsvTest {

	@Mock
	private BpjsSEPINACBGIntegrationService service;
	...
	private BpjsSEPINACBGIntegrationController controller;

	private CsvUtils<BpjsSEPINACBGIntegrationEntity, BpjsSEPINACBGIntegrationRepository, BpjsSEPINACBGIntegrationEntityAudit, BpjsSEPINACBGIntegrationService> csvUtils;

	private final BpjsSEPINACBGIntegrationFactory factory = new BpjsSEPINACBGIntegrationFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsSEPINACBGIntegrationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsSEPINACBGIntegrationEntity.class);
	}

	@Test
	void whenExportingBpjsSEPINACBGIntegrationEntities_thenHeadersAreValid() throws Exception {
		List<BpjsSEPINACBGIntegrationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPINACBGIntegrationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsSEPINACBGIntegrationEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsSEPINACBGIntegrationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVisitDate())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPINACBGIntegrationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsSEPINACBGIntegrationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsSEPINACBGIntegrationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsSEPINACBGIntegrationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsSEPINACBGIntegrationEntity.class));
		when(service.validateEntityRelations(any(BpjsSEPINACBGIntegrationEntity.class))).thenReturn(new HashSet<>());
