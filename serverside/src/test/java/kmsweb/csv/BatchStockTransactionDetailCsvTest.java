...
import kmsweb.controllers.BatchStockTransactionDetailController;
...
@Tag("CSV")
public class BatchStockTransactionDetailCsvTest {

	@Mock
	private BatchStockTransactionDetailService service;
	...
	private BatchStockTransactionDetailController controller;

	private CsvUtils<BatchStockTransactionDetailEntity, BatchStockTransactionDetailRepository, BatchStockTransactionDetailEntityAudit, BatchStockTransactionDetailService> csvUtils;

	private final BatchStockTransactionDetailFactory factory = new BatchStockTransactionDetailFactory();
	...
	public void setup() {
		...
		this.controller = new BatchStockTransactionDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BatchStockTransactionDetailEntity.class);
	}

	@Test
	void whenExportingBatchStockTransactionDetailEntities_thenHeadersAreValid() throws Exception {
		List<BatchStockTransactionDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BatchStockTransactionDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBatchStockTransactionDetailEntities_thenEntitiesAreValid() throws Exception {
		List<BatchStockTransactionDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBatchNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExpirationDate()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBatchTransactionType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBatchQtyUoi()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBatchQtyUomTransaction()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBatchItemNotes())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BatchStockTransactionDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BatchStockTransactionDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BatchStockTransactionDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BatchStockTransactionDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BatchStockTransactionDetailEntity.class));
		when(service.validateEntityRelations(any(BatchStockTransactionDetailEntity.class))).thenReturn(new HashSet<>());
