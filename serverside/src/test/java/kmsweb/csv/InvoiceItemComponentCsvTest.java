...
import kmsweb.controllers.InvoiceItemComponentController;
...
@Tag("CSV")
public class InvoiceItemComponentCsvTest {

	@Mock
	private InvoiceItemComponentService service;
	...
	private InvoiceItemComponentController controller;

	private CsvUtils<InvoiceItemComponentEntity, InvoiceItemComponentRepository, InvoiceItemComponentEntityAudit, InvoiceItemComponentService> csvUtils;

	private final InvoiceItemComponentFactory factory = new InvoiceItemComponentFactory();
	...
	public void setup() {
		...
		this.controller = new InvoiceItemComponentController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InvoiceItemComponentEntity.class);
	}

	@Test
	void whenExportingInvoiceItemComponentEntities_thenHeadersAreValid() throws Exception {
		List<InvoiceItemComponentEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceItemComponentEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInvoiceItemComponentEntities_thenEntitiesAreValid() throws Exception {
		List<InvoiceItemComponentEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceItemComponentEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InvoiceItemComponentEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InvoiceItemComponentEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InvoiceItemComponentEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InvoiceItemComponentEntity.class));
		when(service.validateEntityRelations(any(InvoiceItemComponentEntity.class))).thenReturn(new HashSet<>());
