...
import kmsweb.controllers.PcareReferensiSaranaController;
...
@Tag("CSV")
public class PcareReferensiSaranaCsvTest {

	@Mock
	private PcareReferensiSaranaService service;
	...
	private PcareReferensiSaranaController controller;

	private CsvUtils<PcareReferensiSaranaEntity, PcareReferensiSaranaRepository, PcareReferensiSaranaEntityAudit, PcareReferensiSaranaService> csvUtils;

	private final PcareReferensiSaranaFactory factory = new PcareReferensiSaranaFactory();
	...
	public void setup() {
		...
		this.controller = new PcareReferensiSaranaController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareReferensiSaranaEntity.class);
	}

	@Test
	void whenExportingPcareReferensiSaranaEntities_thenHeadersAreValid() throws Exception {
		List<PcareReferensiSaranaEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareReferensiSaranaEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareReferensiSaranaEntities_thenEntitiesAreValid() throws Exception {
		List<PcareReferensiSaranaEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodeSarana())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaSarana())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareReferensiSaranaEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareReferensiSaranaEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareReferensiSaranaEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareReferensiSaranaEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareReferensiSaranaEntity.class));
		when(service.validateEntityRelations(any(PcareReferensiSaranaEntity.class))).thenReturn(new HashSet<>());
