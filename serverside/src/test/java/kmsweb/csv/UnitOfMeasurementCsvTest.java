...
import kmsweb.controllers.UnitOfMeasurementController;
...
@Tag("CSV")
public class UnitOfMeasurementCsvTest {
	...
	private UnitOfMeasurementController controller;

	private CsvUtils<UnitOfMeasurementEntity, UnitOfMeasurementRepository, UnitOfMeasurementEntityAudit, UnitOfMeasurementService> csvUtils;

	private final UnitOfMeasurementFactory factory = new UnitOfMeasurementFactory();
	...
	public void setup() {
		...
		this.controller = new UnitOfMeasurementController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, UnitOfMeasurementEntity.class);
	}

	@Test
	void whenExportingUnitOfMeasurementEntities_thenHeadersAreValid() throws Exception {
		List<UnitOfMeasurementEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(UnitOfMeasurementEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingUnitOfMeasurementEntities_thenEntitiesAreValid() throws Exception {
		List<UnitOfMeasurementEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSymbol())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDescription())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUnitCategory())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getConsumptionUnit()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(UnitOfMeasurementEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		UnitOfMeasurementEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(UnitOfMeasurementEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(UnitOfMeasurementEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(UnitOfMeasurementEntity.class));
		when(service.validateEntityRelations(any(UnitOfMeasurementEntity.class))).thenReturn(new HashSet<>());
