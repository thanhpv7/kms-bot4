...
import kmsweb.controllers.OdontogramExaminationController;
...
@Tag("CSV")
public class OdontogramExaminationCsvTest {

	@Mock
	private OdontogramExaminationService service;
	...
	private OdontogramExaminationController controller;

	private CsvUtils<OdontogramExaminationEntity, OdontogramExaminationRepository, OdontogramExaminationEntityAudit, OdontogramExaminationService> csvUtils;

	private final OdontogramExaminationFactory factory = new OdontogramExaminationFactory();
	...
	public void setup() {
		...
		this.controller = new OdontogramExaminationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, OdontogramExaminationEntity.class);
	}

	@Test
	void whenExportingOdontogramExaminationEntities_thenHeadersAreValid() throws Exception {
		List<OdontogramExaminationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(OdontogramExaminationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingOdontogramExaminationEntities_thenEntitiesAreValid() throws Exception {
		List<OdontogramExaminationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOdontogramData())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsPermanent()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(OdontogramExaminationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		OdontogramExaminationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(OdontogramExaminationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(OdontogramExaminationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(OdontogramExaminationEntity.class));
		when(service.validateEntityRelations(any(OdontogramExaminationEntity.class))).thenReturn(new HashSet<>());
