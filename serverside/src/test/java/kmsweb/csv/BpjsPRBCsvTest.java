...
import kmsweb.controllers.BpjsPRBController;
...
@Tag("CSV")
public class BpjsPRBCsvTest {

	@Mock
	private BpjsPRBService service;
	...
	private BpjsPRBController controller;

	private CsvUtils<BpjsPRBEntity, BpjsPRBRepository, BpjsPRBEntityAudit, BpjsPRBService> csvUtils;

	private final BpjsPRBFactory factory = new BpjsPRBFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPRBController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPRBEntity.class);
	}

	@Test
	void whenExportingBpjsPRBEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPRBEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPRBEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPRBEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPRBEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPrbProgramCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPrbProgramName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDoctorCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDoctorName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRecommendation())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPRBEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPRBEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPRBEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPRBEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPRBEntity.class));
		when(service.validateEntityRelations(any(BpjsPRBEntity.class))).thenReturn(new HashSet<>());
