...
import kmsweb.controllers.CashierUserController;
...
@Tag("CSV")
public class CashierUserCsvTest {

	@Mock
	private CashierUserService service;
	...
	private CashierUserController controller;

	private CsvUtils<CashierUserEntity, CashierUserRepository, CashierUserEntityAudit, CashierUserService> csvUtils;

	private final CashierUserFactory factory = new CashierUserFactory();
	...
	public void setup() {
		...
		this.controller = new CashierUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CashierUserEntity.class);
	}

	@Test
	void whenExportingCashierUserEntities_thenHeadersAreValid() throws Exception {
		List<CashierUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CashierUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCashierUserEntities_thenEntitiesAreValid() throws Exception {
		List<CashierUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CashierUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CashierUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CashierUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CashierUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CashierUserEntity.class));
		when(service.validateEntityRelations(any(CashierUserEntity.class))).thenReturn(new HashSet<>());
