...
import kmsweb.controllers.MedicalCertificateMentalHealthController;
...
@Tag("CSV")
public class MedicalCertificateMentalHealthCsvTest {

	@Mock
	private MedicalCertificateMentalHealthService service;
	...
	private MedicalCertificateMentalHealthController controller;

	private CsvUtils<MedicalCertificateMentalHealthEntity, MedicalCertificateMentalHealthRepository, MedicalCertificateMentalHealthEntityAudit, MedicalCertificateMentalHealthService> csvUtils;

	private final MedicalCertificateMentalHealthFactory factory = new MedicalCertificateMentalHealthFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalCertificateMentalHealthController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalCertificateMentalHealthEntity.class);
	}

	@Test
	void whenExportingMedicalCertificateMentalHealthEntities_thenHeadersAreValid() throws Exception {
		List<MedicalCertificateMentalHealthEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateMentalHealthEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalCertificateMentalHealthEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalCertificateMentalHealthEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateMentalHealthEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalCertificateMentalHealthEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalCertificateMentalHealthEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalCertificateMentalHealthEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalCertificateMentalHealthEntity.class));
		when(service.validateEntityRelations(any(MedicalCertificateMentalHealthEntity.class))).thenReturn(new HashSet<>());
