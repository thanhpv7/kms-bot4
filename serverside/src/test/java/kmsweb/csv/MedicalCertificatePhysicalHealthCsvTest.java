...
import kmsweb.controllers.MedicalCertificatePhysicalHealthController;
...
@Tag("CSV")
public class MedicalCertificatePhysicalHealthCsvTest {

	@Mock
	private MedicalCertificatePhysicalHealthService service;
	...
	private MedicalCertificatePhysicalHealthController controller;

	private CsvUtils<MedicalCertificatePhysicalHealthEntity, MedicalCertificatePhysicalHealthRepository, MedicalCertificatePhysicalHealthEntityAudit, MedicalCertificatePhysicalHealthService> csvUtils;

	private final MedicalCertificatePhysicalHealthFactory factory = new MedicalCertificatePhysicalHealthFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalCertificatePhysicalHealthController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalCertificatePhysicalHealthEntity.class);
	}

	@Test
	void whenExportingMedicalCertificatePhysicalHealthEntities_thenHeadersAreValid() throws Exception {
		List<MedicalCertificatePhysicalHealthEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificatePhysicalHealthEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalCertificatePhysicalHealthEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalCertificatePhysicalHealthEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExaminationDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExaminationResult()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificatePhysicalHealthEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalCertificatePhysicalHealthEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalCertificatePhysicalHealthEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalCertificatePhysicalHealthEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalCertificatePhysicalHealthEntity.class));
		when(service.validateEntityRelations(any(MedicalCertificatePhysicalHealthEntity.class))).thenReturn(new HashSet<>());
