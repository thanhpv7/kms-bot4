...
import kmsweb.controllers.BpjsSEPSubmissionController;
...
@Tag("CSV")
public class BpjsSEPSubmissionCsvTest {

	@Mock
	private BpjsSEPSubmissionService service;
	...
	private BpjsSEPSubmissionController controller;

	private CsvUtils<BpjsSEPSubmissionEntity, BpjsSEPSubmissionRepository, BpjsSEPSubmissionEntityAudit, BpjsSEPSubmissionService> csvUtils;

	private final BpjsSEPSubmissionFactory factory = new BpjsSEPSubmissionFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsSEPSubmissionController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsSEPSubmissionEntity.class);
	}

	@Test
	void whenExportingBpjsSEPSubmissionEntities_thenHeadersAreValid() throws Exception {
		List<BpjsSEPSubmissionEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPSubmissionEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsSEPSubmissionEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsSEPSubmissionEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSubmissionType())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPSubmissionEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsSEPSubmissionEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsSEPSubmissionEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsSEPSubmissionEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsSEPSubmissionEntity.class));
		when(service.validateEntityRelations(any(BpjsSEPSubmissionEntity.class))).thenReturn(new HashSet<>());
