...
import kmsweb.controllers.BpjsListFingerPrintController;
...
@Tag("CSV")
public class BpjsListFingerPrintCsvTest {

	@Mock
	private BpjsListFingerPrintService service;
	...
	private BpjsListFingerPrintController controller;

	private CsvUtils<BpjsListFingerPrintEntity, BpjsListFingerPrintRepository, BpjsListFingerPrintEntityAudit, BpjsListFingerPrintService> csvUtils;

	private final BpjsListFingerPrintFactory factory = new BpjsListFingerPrintFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsListFingerPrintController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsListFingerPrintEntity.class);
	}

	@Test
	void whenExportingBpjsListFingerPrintEntities_thenHeadersAreValid() throws Exception {
		List<BpjsListFingerPrintEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsListFingerPrintEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsListFingerPrintEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsListFingerPrintEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsListFingerPrintEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsListFingerPrintEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsListFingerPrintEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsListFingerPrintEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsListFingerPrintEntity.class));
		when(service.validateEntityRelations(any(BpjsListFingerPrintEntity.class))).thenReturn(new HashSet<>());
