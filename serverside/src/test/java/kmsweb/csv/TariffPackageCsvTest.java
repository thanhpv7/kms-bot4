...
import kmsweb.controllers.TariffPackageController;
...
@Tag("CSV")
public class TariffPackageCsvTest {

	@Mock
	private TariffPackageService service;
	...
	private TariffPackageController controller;

	private CsvUtils<TariffPackageEntity, TariffPackageRepository, TariffPackageEntityAudit, TariffPackageService> csvUtils;

	private final TariffPackageFactory factory = new TariffPackageFactory();
	...
	public void setup() {
		...
		this.controller = new TariffPackageController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TariffPackageEntity.class);
	}

	@Test
	void whenExportingTariffPackageEntities_thenHeadersAreValid() throws Exception {
		List<TariffPackageEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TariffPackageEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTariffPackageEntities_thenEntitiesAreValid() throws Exception {
		List<TariffPackageEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTariffPackageCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTariffPackageName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTariffPackageClassification())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TariffPackageEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TariffPackageEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TariffPackageEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TariffPackageEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TariffPackageEntity.class));
		when(service.validateEntityRelations(any(TariffPackageEntity.class))).thenReturn(new HashSet<>());
