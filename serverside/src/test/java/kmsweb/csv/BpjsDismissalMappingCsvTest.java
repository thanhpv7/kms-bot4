...
import kmsweb.controllers.BpjsDismissalMappingController;
...
@Tag("CSV")
public class BpjsDismissalMappingCsvTest {

	@Mock
	private BpjsDismissalMappingService service;
	...
	private BpjsDismissalMappingController controller;

	private CsvUtils<BpjsDismissalMappingEntity, BpjsDismissalMappingRepository, BpjsDismissalMappingEntityAudit, BpjsDismissalMappingService> csvUtils;

	private final BpjsDismissalMappingFactory factory = new BpjsDismissalMappingFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsDismissalMappingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsDismissalMappingEntity.class);
	}

	@Test
	void whenExportingBpjsDismissalMappingEntities_thenHeadersAreValid() throws Exception {
		List<BpjsDismissalMappingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDismissalMappingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsDismissalMappingEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsDismissalMappingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInternalDismissal())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDismissalMappingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsDismissalMappingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsDismissalMappingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsDismissalMappingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsDismissalMappingEntity.class));
		when(service.validateEntityRelations(any(BpjsDismissalMappingEntity.class))).thenReturn(new HashSet<>());
