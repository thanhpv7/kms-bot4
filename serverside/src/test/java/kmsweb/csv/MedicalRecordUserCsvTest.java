...
import kmsweb.controllers.MedicalRecordUserController;
...
@Tag("CSV")
public class MedicalRecordUserCsvTest {

	@Mock
	private MedicalRecordUserService service;
	...
	private MedicalRecordUserController controller;

	private CsvUtils<MedicalRecordUserEntity, MedicalRecordUserRepository, MedicalRecordUserEntityAudit, MedicalRecordUserService> csvUtils;

	private final MedicalRecordUserFactory factory = new MedicalRecordUserFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalRecordUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalRecordUserEntity.class);
	}

	@Test
	void whenExportingMedicalRecordUserEntities_thenHeadersAreValid() throws Exception {
		List<MedicalRecordUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalRecordUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalRecordUserEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalRecordUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalRecordUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalRecordUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalRecordUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalRecordUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalRecordUserEntity.class));
		when(service.validateEntityRelations(any(MedicalRecordUserEntity.class))).thenReturn(new HashSet<>());
