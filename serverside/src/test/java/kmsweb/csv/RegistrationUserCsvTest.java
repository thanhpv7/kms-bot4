...
import kmsweb.controllers.RegistrationUserController;
...
@Tag("CSV")
public class RegistrationUserCsvTest {
	...
	private CsvUtils<RegistrationUserEntity, RegistrationUserRepository, RegistrationUserEntityAudit, RegistrationUserService> csvUtils;

	private final RegistrationUserFactory factory = new RegistrationUserFactory();
	...
	public void setup() {
		...
		this.controller = new RegistrationUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, RegistrationUserEntity.class);
	}

	@Test
	void whenExportingRegistrationUserEntities_thenHeadersAreValid() throws Exception {
		List<RegistrationUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(RegistrationUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingRegistrationUserEntities_thenEntitiesAreValid() throws Exception {
		List<RegistrationUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(RegistrationUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		RegistrationUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(RegistrationUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(RegistrationUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(RegistrationUserEntity.class));
		when(service.validateEntityRelations(any(RegistrationUserEntity.class))).thenReturn(new HashSet<>());
