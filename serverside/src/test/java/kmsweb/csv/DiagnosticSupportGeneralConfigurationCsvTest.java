...
import kmsweb.controllers.DiagnosticSupportGeneralConfigurationController;
...
@Tag("CSV")
public class DiagnosticSupportGeneralConfigurationCsvTest {

	@Mock
	private DiagnosticSupportGeneralConfigurationService service;
	...
	private DiagnosticSupportGeneralConfigurationController controller;

	private CsvUtils<DiagnosticSupportGeneralConfigurationEntity, DiagnosticSupportGeneralConfigurationRepository, DiagnosticSupportGeneralConfigurationEntityAudit, DiagnosticSupportGeneralConfigurationService> csvUtils;

	private final DiagnosticSupportGeneralConfigurationFactory factory = new DiagnosticSupportGeneralConfigurationFactory();
	...
	public void setup() {
		...
		this.controller = new DiagnosticSupportGeneralConfigurationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DiagnosticSupportGeneralConfigurationEntity.class);
	}

	@Test
	void whenExportingDiagnosticSupportGeneralConfigurationEntities_thenHeadersAreValid() throws Exception {
		List<DiagnosticSupportGeneralConfigurationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticSupportGeneralConfigurationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDiagnosticSupportGeneralConfigurationEntities_thenEntitiesAreValid() throws Exception {
		List<DiagnosticSupportGeneralConfigurationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsActiveLIS()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsLISRegistration()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLisWebServiceURL())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticSupportGeneralConfigurationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DiagnosticSupportGeneralConfigurationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DiagnosticSupportGeneralConfigurationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DiagnosticSupportGeneralConfigurationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DiagnosticSupportGeneralConfigurationEntity.class));
		when(service.validateEntityRelations(any(DiagnosticSupportGeneralConfigurationEntity.class))).thenReturn(new HashSet<>());
