...
import kmsweb.controllers.CashRefundController;
...
@Tag("CSV")
public class CashRefundCsvTest {

	@Mock
	private CashRefundService service;
	...
	private CashRefundController controller;

	private CsvUtils<CashRefundEntity, CashRefundRepository, CashRefundEntityAudit, CashRefundService> csvUtils;

	private final CashRefundFactory factory = new CashRefundFactory();
	...
	public void setup() {
		...
		this.controller = new CashRefundController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CashRefundEntity.class);
	}

	@Test
	void whenExportingCashRefundEntities_thenHeadersAreValid() throws Exception {
		List<CashRefundEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CashRefundEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCashRefundEntities_thenEntitiesAreValid() throws Exception {
		List<CashRefundEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefundNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefundDate()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefundAmount()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CashRefundEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CashRefundEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CashRefundEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CashRefundEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CashRefundEntity.class));
		when(service.validateEntityRelations(any(CashRefundEntity.class))).thenReturn(new HashSet<>());
