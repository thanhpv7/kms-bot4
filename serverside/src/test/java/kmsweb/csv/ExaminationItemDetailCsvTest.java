...
import kmsweb.controllers.ExaminationItemDetailController;
...
@Tag("CSV")
public class ExaminationItemDetailCsvTest {

	@Mock
	private ExaminationItemDetailService service;
	...
	private ExaminationItemDetailController controller;

	private CsvUtils<ExaminationItemDetailEntity, ExaminationItemDetailRepository, ExaminationItemDetailEntityAudit, ExaminationItemDetailService> csvUtils;

	private final ExaminationItemDetailFactory factory = new ExaminationItemDetailFactory();
	...
	public void setup() {
		...
		this.controller = new ExaminationItemDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ExaminationItemDetailEntity.class);
	}

	@Test
	void whenExportingExaminationItemDetailEntities_thenHeadersAreValid() throws Exception {
		List<ExaminationItemDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ExaminationItemDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingExaminationItemDetailEntities_thenEntitiesAreValid() throws Exception {
		List<ExaminationItemDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAgeMinimum()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAgeMaximum()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getGender())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNormalRangeLow()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNormalRangeHigh()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUnit())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCriticalValueLow()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCriticalValueHigh()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAgeMinUnit())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAgeMaxUnit())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ExaminationItemDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ExaminationItemDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ExaminationItemDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ExaminationItemDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ExaminationItemDetailEntity.class));
		when(service.validateEntityRelations(any(ExaminationItemDetailEntity.class))).thenReturn(new HashSet<>());
