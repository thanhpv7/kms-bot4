...
import kmsweb.controllers.BpjsDismissalConditionController;
...
@Tag("CSV")
public class BpjsDismissalConditionCsvTest {

	@Mock
	private BpjsDismissalConditionService service;
	...
	private BpjsDismissalConditionController controller;

	private CsvUtils<BpjsDismissalConditionEntity, BpjsDismissalConditionRepository, BpjsDismissalConditionEntityAudit, BpjsDismissalConditionService> csvUtils;

	private final BpjsDismissalConditionFactory factory = new BpjsDismissalConditionFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsDismissalConditionController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsDismissalConditionEntity.class);
	}

	@Test
	void whenExportingBpjsDismissalConditionEntities_thenHeadersAreValid() throws Exception {
		List<BpjsDismissalConditionEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDismissalConditionEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsDismissalConditionEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsDismissalConditionEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getConditionName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getConditionCode())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDismissalConditionEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsDismissalConditionEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsDismissalConditionEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsDismissalConditionEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsDismissalConditionEntity.class));
		when(service.validateEntityRelations(any(BpjsDismissalConditionEntity.class))).thenReturn(new HashSet<>());
