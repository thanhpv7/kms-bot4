...
import kmsweb.controllers.StockTransactionPerDateTypeController;
...
@Tag("CSV")
public class StockTransactionPerDateTypeCsvTest {

	@Mock
	private StockTransactionPerDateTypeService service;
	...
	private StockTransactionPerDateTypeController controller;

	private CsvUtils<StockTransactionPerDateTypeEntity, StockTransactionPerDateTypeRepository, StockTransactionPerDateTypeEntityAudit, StockTransactionPerDateTypeService> csvUtils;

	private final StockTransactionPerDateTypeFactory factory = new StockTransactionPerDateTypeFactory();
	...
	public void setup() {
		...
		this.controller = new StockTransactionPerDateTypeController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, StockTransactionPerDateTypeEntity.class);
	}

	@Test
	void whenExportingStockTransactionPerDateTypeEntities_thenHeadersAreValid() throws Exception {
		List<StockTransactionPerDateTypeEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(StockTransactionPerDateTypeEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingStockTransactionPerDateTypeEntities_thenEntitiesAreValid() throws Exception {
		List<StockTransactionPerDateTypeEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(StockTransactionPerDateTypeEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		StockTransactionPerDateTypeEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(StockTransactionPerDateTypeEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(StockTransactionPerDateTypeEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(StockTransactionPerDateTypeEntity.class));
		when(service.validateEntityRelations(any(StockTransactionPerDateTypeEntity.class))).thenReturn(new HashSet<>());
