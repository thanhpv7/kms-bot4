...
import kmsweb.controllers.InventoryAdjusmentBatchController;
...
@Tag("CSV")
public class InventoryAdjusmentBatchCsvTest {

	@Mock
	private InventoryAdjusmentBatchService service;
	...
	private InventoryAdjusmentBatchController controller;

	private CsvUtils<InventoryAdjusmentBatchEntity, InventoryAdjusmentBatchRepository, InventoryAdjusmentBatchEntityAudit, InventoryAdjusmentBatchService> csvUtils;

	private final InventoryAdjusmentBatchFactory factory = new InventoryAdjusmentBatchFactory();
	...
	public void setup() {
		...
		this.controller = new InventoryAdjusmentBatchController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InventoryAdjusmentBatchEntity.class);
	}

	@Test
	void whenExportingInventoryAdjusmentBatchEntities_thenHeadersAreValid() throws Exception {
		List<InventoryAdjusmentBatchEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InventoryAdjusmentBatchEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInventoryAdjusmentBatchEntities_thenEntitiesAreValid() throws Exception {
		List<InventoryAdjusmentBatchEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InventoryAdjusmentBatchEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InventoryAdjusmentBatchEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InventoryAdjusmentBatchEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InventoryAdjusmentBatchEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InventoryAdjusmentBatchEntity.class));
		when(service.validateEntityRelations(any(InventoryAdjusmentBatchEntity.class))).thenReturn(new HashSet<>());
