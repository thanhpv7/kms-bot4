...
import kmsweb.controllers.WarehouseInitialStockController;
...
@Tag("CSV")
public class WarehouseInitialStockCsvTest {

	@Mock
	private WarehouseInitialStockService service;
	...
	private WarehouseInitialStockController controller;

	private CsvUtils<WarehouseInitialStockEntity, WarehouseInitialStockRepository, WarehouseInitialStockEntityAudit, WarehouseInitialStockService> csvUtils;

	private final WarehouseInitialStockFactory factory = new WarehouseInitialStockFactory();
	...
	public void setup() {
		...
		this.controller = new WarehouseInitialStockController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, WarehouseInitialStockEntity.class);
	}

	@Test
	void whenExportingWarehouseInitialStockEntities_thenHeadersAreValid() throws Exception {
		List<WarehouseInitialStockEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseInitialStockEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingWarehouseInitialStockEntities_thenEntitiesAreValid() throws Exception {
		List<WarehouseInitialStockEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMinimumLevel()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMaximumLevel()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseInitialStockEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		WarehouseInitialStockEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(WarehouseInitialStockEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(WarehouseInitialStockEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(WarehouseInitialStockEntity.class));
		when(service.validateEntityRelations(any(WarehouseInitialStockEntity.class))).thenReturn(new HashSet<>());
