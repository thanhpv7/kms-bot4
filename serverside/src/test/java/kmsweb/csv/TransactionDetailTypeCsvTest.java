...
import kmsweb.controllers.TransactionDetailTypeController;
...
@Tag("CSV")
public class TransactionDetailTypeCsvTest {

	@Mock
	private TransactionDetailTypeService service;
	...
	private TransactionDetailTypeController controller;

	private CsvUtils<TransactionDetailTypeEntity, TransactionDetailTypeRepository, TransactionDetailTypeEntityAudit, TransactionDetailTypeService> csvUtils;

	private final TransactionDetailTypeFactory factory = new TransactionDetailTypeFactory();
	...
	public void setup() {
		...
		this.controller = new TransactionDetailTypeController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TransactionDetailTypeEntity.class);
	}

	@Test
	void whenExportingTransactionDetailTypeEntities_thenHeadersAreValid() throws Exception {
		List<TransactionDetailTypeEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TransactionDetailTypeEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTransactionDetailTypeEntities_thenEntitiesAreValid() throws Exception {
		List<TransactionDetailTypeEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTransactionDetail())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TransactionDetailTypeEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TransactionDetailTypeEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TransactionDetailTypeEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TransactionDetailTypeEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TransactionDetailTypeEntity.class));
		when(service.validateEntityRelations(any(TransactionDetailTypeEntity.class))).thenReturn(new HashSet<>());
