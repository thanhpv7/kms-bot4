...
import kmsweb.controllers.InventoryBatchCorrectionItemController;
...
@Tag("CSV")
public class InventoryBatchCorrectionItemCsvTest {

	@Mock
	private InventoryBatchCorrectionItemService service;
	...
	private InventoryBatchCorrectionItemController controller;

	private CsvUtils<InventoryBatchCorrectionItemEntity, InventoryBatchCorrectionItemRepository, InventoryBatchCorrectionItemEntityAudit, InventoryBatchCorrectionItemService> csvUtils;

	private final InventoryBatchCorrectionItemFactory factory = new InventoryBatchCorrectionItemFactory();
	...
	public void setup() {
		...
		this.controller = new InventoryBatchCorrectionItemController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InventoryBatchCorrectionItemEntity.class);
	}

	@Test
	void whenExportingInventoryBatchCorrectionItemEntities_thenHeadersAreValid() throws Exception {
		List<InventoryBatchCorrectionItemEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InventoryBatchCorrectionItemEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInventoryBatchCorrectionItemEntities_thenEntitiesAreValid() throws Exception {
		List<InventoryBatchCorrectionItemEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAvailableQtyBatch()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getActualQtyBatch()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getQtyAdjusted()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InventoryBatchCorrectionItemEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InventoryBatchCorrectionItemEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InventoryBatchCorrectionItemEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InventoryBatchCorrectionItemEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InventoryBatchCorrectionItemEntity.class));
		when(service.validateEntityRelations(any(InventoryBatchCorrectionItemEntity.class))).thenReturn(new HashSet<>());
