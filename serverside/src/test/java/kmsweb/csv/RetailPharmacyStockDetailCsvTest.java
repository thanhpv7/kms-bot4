...
import kmsweb.controllers.RetailPharmacyStockDetailController;
...
@Tag("CSV")
public class RetailPharmacyStockDetailCsvTest {

	@Mock
	private RetailPharmacyStockDetailService service;
	...
	private RetailPharmacyStockDetailController controller;

	private CsvUtils<RetailPharmacyStockDetailEntity, RetailPharmacyStockDetailRepository, RetailPharmacyStockDetailEntityAudit, RetailPharmacyStockDetailService> csvUtils;

	private final RetailPharmacyStockDetailFactory factory = new RetailPharmacyStockDetailFactory();
	...
	public void setup() {
		...
		this.controller = new RetailPharmacyStockDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, RetailPharmacyStockDetailEntity.class);
	}

	@Test
	void whenExportingRetailPharmacyStockDetailEntities_thenHeadersAreValid() throws Exception {
		List<RetailPharmacyStockDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(RetailPharmacyStockDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingRetailPharmacyStockDetailEntities_thenEntitiesAreValid() throws Exception {
		List<RetailPharmacyStockDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDiscount()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTaxIncluded()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(RetailPharmacyStockDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		RetailPharmacyStockDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(RetailPharmacyStockDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(RetailPharmacyStockDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(RetailPharmacyStockDetailEntity.class));
		when(service.validateEntityRelations(any(RetailPharmacyStockDetailEntity.class))).thenReturn(new HashSet<>());
