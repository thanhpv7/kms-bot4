...
import kmsweb.controllers.InvoiceItemController;
...
@Tag("CSV")
public class InvoiceItemCsvTest {
	...
	private CsvUtils<InvoiceItemEntity, InvoiceItemRepository, InvoiceItemEntityAudit, InvoiceItemService> csvUtils;

	private final InvoiceItemFactory factory = new InvoiceItemFactory();
	...
	public void setup() {
		...
		this.controller = new InvoiceItemController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InvoiceItemEntity.class);
	}

	@Test
	void whenExportingInvoiceItemEntities_thenHeadersAreValid() throws Exception {
		List<InvoiceItemEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceItemEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInvoiceItemEntities_thenEntitiesAreValid() throws Exception {
		List<InvoiceItemEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalOtherComponentPrice()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getItemTotal()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InvoiceItemEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InvoiceItemEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InvoiceItemEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InvoiceItemEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InvoiceItemEntity.class));
		when(service.validateEntityRelations(any(InvoiceItemEntity.class))).thenReturn(new HashSet<>());
