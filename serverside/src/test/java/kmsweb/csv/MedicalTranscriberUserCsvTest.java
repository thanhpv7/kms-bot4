...
import kmsweb.controllers.MedicalTranscriberUserController;
...
@Tag("CSV")
public class MedicalTranscriberUserCsvTest {

	@Mock
	private MedicalTranscriberUserService service;
	...
	private MedicalTranscriberUserController controller;

	private CsvUtils<MedicalTranscriberUserEntity, MedicalTranscriberUserRepository, MedicalTranscriberUserEntityAudit, MedicalTranscriberUserService> csvUtils;

	private final MedicalTranscriberUserFactory factory = new MedicalTranscriberUserFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalTranscriberUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalTranscriberUserEntity.class);
	}

	@Test
	void whenExportingMedicalTranscriberUserEntities_thenHeadersAreValid() throws Exception {
		List<MedicalTranscriberUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalTranscriberUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalTranscriberUserEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalTranscriberUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalTranscriberUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalTranscriberUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalTranscriberUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalTranscriberUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalTranscriberUserEntity.class));
		when(service.validateEntityRelations(any(MedicalTranscriberUserEntity.class))).thenReturn(new HashSet<>());
