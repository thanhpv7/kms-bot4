...
import kmsweb.controllers.PatientVisitPaymentSelfPayingController;
...
@Tag("CSV")
public class PatientVisitPaymentSelfPayingCsvTest {

	@Mock
	private PatientVisitPaymentSelfPayingService service;
	...
	private PatientVisitPaymentSelfPayingController controller;

	private CsvUtils<PatientVisitPaymentSelfPayingEntity, PatientVisitPaymentSelfPayingRepository, PatientVisitPaymentSelfPayingEntityAudit, PatientVisitPaymentSelfPayingService> csvUtils;

	private final PatientVisitPaymentSelfPayingFactory factory = new PatientVisitPaymentSelfPayingFactory();
	...
	public void setup() {
		...
		this.controller = new PatientVisitPaymentSelfPayingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientVisitPaymentSelfPayingEntity.class);
	}

	@Test
	void whenExportingPatientVisitPaymentSelfPayingEntities_thenHeadersAreValid() throws Exception {
		List<PatientVisitPaymentSelfPayingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitPaymentSelfPayingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientVisitPaymentSelfPayingEntities_thenEntitiesAreValid() throws Exception {
		List<PatientVisitPaymentSelfPayingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSponsorName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSponsorAddress())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSponsorPhoneNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSponsorship())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitPaymentSelfPayingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientVisitPaymentSelfPayingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientVisitPaymentSelfPayingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientVisitPaymentSelfPayingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientVisitPaymentSelfPayingEntity.class));
		when(service.validateEntityRelations(any(PatientVisitPaymentSelfPayingEntity.class))).thenReturn(new HashSet<>());
