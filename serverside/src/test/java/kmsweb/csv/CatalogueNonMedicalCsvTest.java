...
import kmsweb.controllers.CatalogueNonMedicalController;
...
@Tag("CSV")
public class CatalogueNonMedicalCsvTest {

	@Mock
	private CatalogueNonMedicalService service;
	...
	private CatalogueNonMedicalController controller;

	private CsvUtils<CatalogueNonMedicalEntity, CatalogueNonMedicalRepository, CatalogueNonMedicalEntityAudit, CatalogueNonMedicalService> csvUtils;

	private final CatalogueNonMedicalFactory factory = new CatalogueNonMedicalFactory();
	...
	public void setup() {
		...
		this.controller = new CatalogueNonMedicalController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CatalogueNonMedicalEntity.class);
	}

	@Test
	void whenExportingCatalogueNonMedicalEntities_thenHeadersAreValid() throws Exception {
		List<CatalogueNonMedicalEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CatalogueNonMedicalEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCatalogueNonMedicalEntities_thenEntitiesAreValid() throws Exception {
		List<CatalogueNonMedicalEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getManufacturer())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getManufacturerItemNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsExpensiveItem()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsConsignmentItem()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getModel())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CatalogueNonMedicalEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CatalogueNonMedicalEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CatalogueNonMedicalEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CatalogueNonMedicalEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CatalogueNonMedicalEntity.class));
		when(service.validateEntityRelations(any(CatalogueNonMedicalEntity.class))).thenReturn(new HashSet<>());
