...
import kmsweb.controllers.BpjsClaimSubmissionController;
...
@Tag("CSV")
public class BpjsClaimSubmissionCsvTest {

	@Mock
	private BpjsClaimSubmissionService service;
	...
	private BpjsClaimSubmissionController controller;

	private CsvUtils<BpjsClaimSubmissionEntity, BpjsClaimSubmissionRepository, BpjsClaimSubmissionEntityAudit, BpjsClaimSubmissionService> csvUtils;

	private final BpjsClaimSubmissionFactory factory = new BpjsClaimSubmissionFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsClaimSubmissionController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsClaimSubmissionEntity.class);
	}

	@Test
	void whenExportingBpjsClaimSubmissionEntities_thenHeadersAreValid() throws Exception {
		List<BpjsClaimSubmissionEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsClaimSubmissionEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsClaimSubmissionEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsClaimSubmissionEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInsuranceProvider())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTreatmentType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFollowUpPlan())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getControlDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAdditionalDiagnosis())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAdditionalProcedure())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKelasRawat()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsClaimSubmissionEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsClaimSubmissionEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsClaimSubmissionEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsClaimSubmissionEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsClaimSubmissionEntity.class));
		when(service.validateEntityRelations(any(BpjsClaimSubmissionEntity.class))).thenReturn(new HashSet<>());
