...
import kmsweb.controllers.BpjsListTaskController;
...
@Tag("CSV")
public class BpjsListTaskCsvTest {

	@Mock
	private BpjsListTaskService service;
	...
	private BpjsListTaskController controller;

	private CsvUtils<BpjsListTaskEntity, BpjsListTaskRepository, BpjsListTaskEntityAudit, BpjsListTaskService> csvUtils;

	private final BpjsListTaskFactory factory = new BpjsListTaskFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsListTaskController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsListTaskEntity.class);
	}

	@Test
	void whenExportingBpjsListTaskEntities_thenHeadersAreValid() throws Exception {
		List<BpjsListTaskEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsListTaskEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsListTaskEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsListTaskEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodebooking())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTaskname())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTaskid()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getWaktu())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getWakturs())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsListTaskEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsListTaskEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsListTaskEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsListTaskEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsListTaskEntity.class));
		when(service.validateEntityRelations(any(BpjsListTaskEntity.class))).thenReturn(new HashSet<>());
