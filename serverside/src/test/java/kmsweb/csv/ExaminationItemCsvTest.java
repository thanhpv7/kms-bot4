...
import kmsweb.controllers.ExaminationItemController;
...
@Tag("CSV")
public class ExaminationItemCsvTest {

	@Mock
	private ExaminationItemService service;
	...
	private ExaminationItemController controller;

	private CsvUtils<ExaminationItemEntity, ExaminationItemRepository, ExaminationItemEntityAudit, ExaminationItemService> csvUtils;

	private final ExaminationItemFactory factory = new ExaminationItemFactory();
	...
	public void setup() {
		...
		this.controller = new ExaminationItemController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ExaminationItemEntity.class);
	}

	@Test
	void whenExportingExaminationItemEntities_thenHeadersAreValid() throws Exception {
		List<ExaminationItemEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ExaminationItemEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingExaminationItemEntities_thenEntitiesAreValid() throws Exception {
		List<ExaminationItemEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getItemCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDescription())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSample())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExaminationLevel())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExaminationType())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ExaminationItemEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ExaminationItemEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ExaminationItemEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ExaminationItemEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ExaminationItemEntity.class));
		when(service.validateEntityRelations(any(ExaminationItemEntity.class))).thenReturn(new HashSet<>());
