...
import kmsweb.controllers.BpjsFingerPrintController;
...
@Tag("CSV")
public class BpjsFingerPrintCsvTest {

	@Mock
	private BpjsFingerPrintService service;
	...
	private BpjsFingerPrintController controller;

	private CsvUtils<BpjsFingerPrintEntity, BpjsFingerPrintRepository, BpjsFingerPrintEntityAudit, BpjsFingerPrintService> csvUtils;

	private final BpjsFingerPrintFactory factory = new BpjsFingerPrintFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsFingerPrintController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsFingerPrintEntity.class);
	}

	@Test
	void whenExportingBpjsFingerPrintEntities_thenHeadersAreValid() throws Exception {
		List<BpjsFingerPrintEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsFingerPrintEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsFingerPrintEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsFingerPrintEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsFingerPrintEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsFingerPrintEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsFingerPrintEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsFingerPrintEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsFingerPrintEntity.class));
		when(service.validateEntityRelations(any(BpjsFingerPrintEntity.class))).thenReturn(new HashSet<>());
