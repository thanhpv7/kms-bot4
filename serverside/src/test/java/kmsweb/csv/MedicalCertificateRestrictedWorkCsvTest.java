...
import kmsweb.controllers.MedicalCertificateRestrictedWorkController;
...
@Tag("CSV")
public class MedicalCertificateRestrictedWorkCsvTest {

	@Mock
	private MedicalCertificateRestrictedWorkService service;
	...
	private MedicalCertificateRestrictedWorkController controller;

	private CsvUtils<MedicalCertificateRestrictedWorkEntity, MedicalCertificateRestrictedWorkRepository, MedicalCertificateRestrictedWorkEntityAudit, MedicalCertificateRestrictedWorkService> csvUtils;

	private final MedicalCertificateRestrictedWorkFactory factory = new MedicalCertificateRestrictedWorkFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalCertificateRestrictedWorkController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalCertificateRestrictedWorkEntity.class);
	}

	@Test
	void whenExportingMedicalCertificateRestrictedWorkEntities_thenHeadersAreValid() throws Exception {
		List<MedicalCertificateRestrictedWorkEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateRestrictedWorkEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalCertificateRestrictedWorkEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalCertificateRestrictedWorkEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPeriod()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalCertificateRestrictedWorkEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalCertificateRestrictedWorkEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalCertificateRestrictedWorkEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalCertificateRestrictedWorkEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalCertificateRestrictedWorkEntity.class));
		when(service.validateEntityRelations(any(MedicalCertificateRestrictedWorkEntity.class))).thenReturn(new HashSet<>());
