...
import kmsweb.controllers.BpjsSearchSEPController;
...
@Tag("CSV")
public class BpjsSearchSEPCsvTest {

	@Mock
	private BpjsSearchSEPService service;
	...
	private BpjsSearchSEPController controller;

	private CsvUtils<BpjsSearchSEPEntity, BpjsSearchSEPRepository, BpjsSearchSEPEntityAudit, BpjsSearchSEPService> csvUtils;

	private final BpjsSearchSEPFactory factory = new BpjsSearchSEPFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsSearchSEPController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsSearchSEPEntity.class);
	}

	@Test
	void whenExportingBpjsSearchSEPEntities_thenHeadersAreValid() throws Exception {
		List<BpjsSearchSEPEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSearchSEPEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsSearchSEPEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsSearchSEPEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSearchSEPEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsSearchSEPEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsSearchSEPEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsSearchSEPEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsSearchSEPEntity.class));
		when(service.validateEntityRelations(any(BpjsSearchSEPEntity.class))).thenReturn(new HashSet<>());
