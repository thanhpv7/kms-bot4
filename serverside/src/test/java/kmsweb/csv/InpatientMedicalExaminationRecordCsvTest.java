...
import kmsweb.controllers.InpatientMedicalExaminationRecordController;
...
@Tag("CSV")
public class InpatientMedicalExaminationRecordCsvTest {

	@Mock
	private InpatientMedicalExaminationRecordService service;
	...
	private InpatientMedicalExaminationRecordController controller;

	private CsvUtils<InpatientMedicalExaminationRecordEntity, InpatientMedicalExaminationRecordRepository, InpatientMedicalExaminationRecordEntityAudit, InpatientMedicalExaminationRecordService> csvUtils;

	private final InpatientMedicalExaminationRecordFactory factory = new InpatientMedicalExaminationRecordFactory();
	...
	public void setup() {
		...
		this.controller = new InpatientMedicalExaminationRecordController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InpatientMedicalExaminationRecordEntity.class);
	}

	@Test
	void whenExportingInpatientMedicalExaminationRecordEntities_thenHeadersAreValid() throws Exception {
		List<InpatientMedicalExaminationRecordEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InpatientMedicalExaminationRecordEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInpatientMedicalExaminationRecordEntities_thenEntitiesAreValid() throws Exception {
		List<InpatientMedicalExaminationRecordEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInpatientCase())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getServiceCase())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDependencyLevel())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCurrentDependency())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInfantCondition())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InpatientMedicalExaminationRecordEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InpatientMedicalExaminationRecordEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InpatientMedicalExaminationRecordEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InpatientMedicalExaminationRecordEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InpatientMedicalExaminationRecordEntity.class));
		when(service.validateEntityRelations(any(InpatientMedicalExaminationRecordEntity.class))).thenReturn(new HashSet<>());
