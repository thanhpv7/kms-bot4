...
import kmsweb.controllers.TariffFormulaController;
...
@Tag("CSV")
public class TariffFormulaCsvTest {

	@Mock
	private TariffFormulaService service;
	...
	private TariffFormulaController controller;

	private CsvUtils<TariffFormulaEntity, TariffFormulaRepository, TariffFormulaEntityAudit, TariffFormulaService> csvUtils;

	private final TariffFormulaFactory factory = new TariffFormulaFactory();
	...
	public void setup() {
		...
		this.controller = new TariffFormulaController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TariffFormulaEntity.class);
	}

	@Test
	void whenExportingTariffFormulaEntities_thenHeadersAreValid() throws Exception {
		List<TariffFormulaEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TariffFormulaEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTariffFormulaEntities_thenEntitiesAreValid() throws Exception {
		List<TariffFormulaEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMultiplier()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRounding()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRoundingMethod()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TariffFormulaEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TariffFormulaEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TariffFormulaEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TariffFormulaEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TariffFormulaEntity.class));
		when(service.validateEntityRelations(any(TariffFormulaEntity.class))).thenReturn(new HashSet<>());
