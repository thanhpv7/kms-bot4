...
import kmsweb.controllers.SatuSehatConfigurationController;
...
@Tag("CSV")
public class SatuSehatConfigurationCsvTest {

	@Mock
	private SatuSehatConfigurationService service;
	...
	private SatuSehatConfigurationController controller;

	private CsvUtils<SatuSehatConfigurationEntity, SatuSehatConfigurationRepository, SatuSehatConfigurationEntityAudit, SatuSehatConfigurationService> csvUtils;

	private final SatuSehatConfigurationFactory factory = new SatuSehatConfigurationFactory();
	...
	public void setup() {
		...
		this.controller = new SatuSehatConfigurationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SatuSehatConfigurationEntity.class);
	}

	@Test
	void whenExportingSatuSehatConfigurationEntities_thenHeadersAreValid() throws Exception {
		List<SatuSehatConfigurationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SatuSehatConfigurationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSatuSehatConfigurationEntities_thenEntitiesAreValid() throws Exception {
		List<SatuSehatConfigurationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOrganizationID())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getClientID())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSecretKey())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTokenRequest())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBaseURL())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SatuSehatConfigurationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SatuSehatConfigurationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SatuSehatConfigurationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SatuSehatConfigurationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SatuSehatConfigurationEntity.class));
		when(service.validateEntityRelations(any(SatuSehatConfigurationEntity.class))).thenReturn(new HashSet<>());
