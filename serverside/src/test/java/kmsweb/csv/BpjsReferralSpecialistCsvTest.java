...
import kmsweb.controllers.BpjsReferralSpecialistController;
...
@Tag("CSV")
public class BpjsReferralSpecialistCsvTest {

	@Mock
	private BpjsReferralSpecialistService service;
	...
	private BpjsReferralSpecialistController controller;

	private CsvUtils<BpjsReferralSpecialistEntity, BpjsReferralSpecialistRepository, BpjsReferralSpecialistEntityAudit, BpjsReferralSpecialistService> csvUtils;

	private final BpjsReferralSpecialistFactory factory = new BpjsReferralSpecialistFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsReferralSpecialistController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsReferralSpecialistEntity.class);
	}

	@Test
	void whenExportingBpjsReferralSpecialistEntities_thenHeadersAreValid() throws Exception {
		List<BpjsReferralSpecialistEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsReferralSpecialistEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsReferralSpecialistEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsReferralSpecialistEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalReferral())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsReferralSpecialistEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsReferralSpecialistEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsReferralSpecialistEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsReferralSpecialistEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsReferralSpecialistEntity.class));
		when(service.validateEntityRelations(any(BpjsReferralSpecialistEntity.class))).thenReturn(new HashSet<>());
