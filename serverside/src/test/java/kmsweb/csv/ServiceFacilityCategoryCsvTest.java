...
import kmsweb.controllers.ServiceFacilityCategoryController;
...
@Tag("CSV")
public class ServiceFacilityCategoryCsvTest {

	@Mock
	private ServiceFacilityCategoryService service;
	...
	private ServiceFacilityCategoryController controller;

	private CsvUtils<ServiceFacilityCategoryEntity, ServiceFacilityCategoryRepository, ServiceFacilityCategoryEntityAudit, ServiceFacilityCategoryService> csvUtils;

	private final ServiceFacilityCategoryFactory factory = new ServiceFacilityCategoryFactory();
	...
	public void setup() {
		...
		this.controller = new ServiceFacilityCategoryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ServiceFacilityCategoryEntity.class);
	}

	@Test
	void whenExportingServiceFacilityCategoryEntities_thenHeadersAreValid() throws Exception {
		List<ServiceFacilityCategoryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ServiceFacilityCategoryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingServiceFacilityCategoryEntities_thenEntitiesAreValid() throws Exception {
		List<ServiceFacilityCategoryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ServiceFacilityCategoryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ServiceFacilityCategoryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ServiceFacilityCategoryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ServiceFacilityCategoryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ServiceFacilityCategoryEntity.class));
		when(service.validateEntityRelations(any(ServiceFacilityCategoryEntity.class))).thenReturn(new HashSet<>());
