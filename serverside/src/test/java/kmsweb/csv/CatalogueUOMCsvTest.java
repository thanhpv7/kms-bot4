...
import kmsweb.controllers.CatalogueUOMController;
...
@Tag("CSV")
public class CatalogueUOMCsvTest {

	@Mock
	private CatalogueUOMService service;
	...
	private CatalogueUOMController controller;

	private CsvUtils<CatalogueUOMEntity, CatalogueUOMRepository, CatalogueUOMEntityAudit, CatalogueUOMService> csvUtils;

	private final CatalogueUOMFactory factory = new CatalogueUOMFactory();
	...
	public void setup() {
		...
		this.controller = new CatalogueUOMController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CatalogueUOMEntity.class);
	}

	@Test
	void whenExportingCatalogueUOMEntities_thenHeadersAreValid() throws Exception {
		List<CatalogueUOMEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CatalogueUOMEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCatalogueUOMEntities_thenEntitiesAreValid() throws Exception {
		List<CatalogueUOMEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getConversion())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDefaultTransaction()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDefaultStockCard()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDefaultPurchasing()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CatalogueUOMEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CatalogueUOMEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CatalogueUOMEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CatalogueUOMEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CatalogueUOMEntity.class));
		when(service.validateEntityRelations(any(CatalogueUOMEntity.class))).thenReturn(new HashSet<>());
