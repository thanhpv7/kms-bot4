...
import kmsweb.controllers.FacilityHistoryController;
...
@Tag("CSV")
public class FacilityHistoryCsvTest {
	...
	private FacilityHistoryController controller;

	private CsvUtils<FacilityHistoryEntity, FacilityHistoryRepository, FacilityHistoryEntityAudit, FacilityHistoryService> csvUtils;

	private final FacilityHistoryFactory factory = new FacilityHistoryFactory();
	...
	public void setup() {
		...
		this.controller = new FacilityHistoryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, FacilityHistoryEntity.class);
	}

	@Test
	void whenExportingFacilityHistoryEntities_thenHeadersAreValid() throws Exception {
		List<FacilityHistoryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(FacilityHistoryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingFacilityHistoryEntities_thenEntitiesAreValid() throws Exception {
		List<FacilityHistoryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getChangeDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFacilityNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCurrentQty()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getChangedQty()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFacilityStatus())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFacilityClass())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFacilityType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFacilityService())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBedFacilityId())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(FacilityHistoryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		FacilityHistoryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(FacilityHistoryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(FacilityHistoryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(FacilityHistoryEntity.class));
		when(service.validateEntityRelations(any(FacilityHistoryEntity.class))).thenReturn(new HashSet<>());
