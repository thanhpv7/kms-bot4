...
import kmsweb.controllers.PostOperativeDetailsController;
...
@Tag("CSV")
public class PostOperativeDetailsCsvTest {

	@Mock
	private PostOperativeDetailsService service;
	...
	private PostOperativeDetailsController controller;

	private CsvUtils<PostOperativeDetailsEntity, PostOperativeDetailsRepository, PostOperativeDetailsEntityAudit, PostOperativeDetailsService> csvUtils;

	private final PostOperativeDetailsFactory factory = new PostOperativeDetailsFactory();
	...
	public void setup() {
		...
		this.controller = new PostOperativeDetailsController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PostOperativeDetailsEntity.class);
	}

	@Test
	void whenExportingPostOperativeDetailsEntities_thenHeadersAreValid() throws Exception {
		List<PostOperativeDetailsEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PostOperativeDetailsEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPostOperativeDetailsEntities_thenEntitiesAreValid() throws Exception {
		List<PostOperativeDetailsEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRecoveryStartDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRecoveryEndDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPostoperativeMainDiagnosis())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPostoperativeAction())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNewCase()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPostoperativeDiagnosisSupportOrder())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAnesthesiaPostoperativeMonitoring())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAnesthesiaPostoperativeAction())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PostOperativeDetailsEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PostOperativeDetailsEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PostOperativeDetailsEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PostOperativeDetailsEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PostOperativeDetailsEntity.class));
		when(service.validateEntityRelations(any(PostOperativeDetailsEntity.class))).thenReturn(new HashSet<>());
