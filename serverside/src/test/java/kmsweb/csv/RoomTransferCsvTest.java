...
import kmsweb.controllers.RoomTransferController;
...
@Tag("CSV")
public class RoomTransferCsvTest {
	...
	private RoomTransferController controller;

	private CsvUtils<RoomTransferEntity, RoomTransferRepository, RoomTransferEntityAudit, RoomTransferService> csvUtils;

	private final RoomTransferFactory factory = new RoomTransferFactory();
	...
	public void setup() {
		...
		this.controller = new RoomTransferController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, RoomTransferEntity.class);
	}

	@Test
	void whenExportingRoomTransferEntities_thenHeadersAreValid() throws Exception {
		List<RoomTransferEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(RoomTransferEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingRoomTransferEntities_thenEntitiesAreValid() throws Exception {
		List<RoomTransferEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTransferringStaffName())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(RoomTransferEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		RoomTransferEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(RoomTransferEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(RoomTransferEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(RoomTransferEntity.class));
		when(service.validateEntityRelations(any(RoomTransferEntity.class))).thenReturn(new HashSet<>());
