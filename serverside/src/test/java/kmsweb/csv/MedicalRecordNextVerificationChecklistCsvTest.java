...
import kmsweb.controllers.MedicalRecordNextVerificationChecklistController;
...
@Tag("CSV")
public class MedicalRecordNextVerificationChecklistCsvTest {

	@Mock
	private MedicalRecordNextVerificationChecklistService service;
	...
	private MedicalRecordNextVerificationChecklistController controller;

	private CsvUtils<MedicalRecordNextVerificationChecklistEntity, MedicalRecordNextVerificationChecklistRepository, MedicalRecordNextVerificationChecklistEntityAudit, MedicalRecordNextVerificationChecklistService> csvUtils;

	private final MedicalRecordNextVerificationChecklistFactory factory = new MedicalRecordNextVerificationChecklistFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalRecordNextVerificationChecklistController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalRecordNextVerificationChecklistEntity.class);
	}

	@Test
	void whenExportingMedicalRecordNextVerificationChecklistEntities_thenHeadersAreValid() throws Exception {
		List<MedicalRecordNextVerificationChecklistEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalRecordNextVerificationChecklistEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalRecordNextVerificationChecklistEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalRecordNextVerificationChecklistEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalRecordNextVerificationChecklistEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalRecordNextVerificationChecklistEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalRecordNextVerificationChecklistEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalRecordNextVerificationChecklistEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalRecordNextVerificationChecklistEntity.class));
		when(service.validateEntityRelations(any(MedicalRecordNextVerificationChecklistEntity.class))).thenReturn(new HashSet<>());
