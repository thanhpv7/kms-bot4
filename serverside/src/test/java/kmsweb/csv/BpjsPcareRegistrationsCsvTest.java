...
import kmsweb.controllers.BpjsPcareRegistrationsController;
...
@Tag("CSV")
public class BpjsPcareRegistrationsCsvTest {

	@Mock
	private BpjsPcareRegistrationsService service;
	...
	private BpjsPcareRegistrationsController controller;

	private CsvUtils<BpjsPcareRegistrationsEntity, BpjsPcareRegistrationsRepository, BpjsPcareRegistrationsEntityAudit, BpjsPcareRegistrationsService> csvUtils;

	private final BpjsPcareRegistrationsFactory factory = new BpjsPcareRegistrationsFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPcareRegistrationsController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPcareRegistrationsEntity.class);
	}

	@Test
	void whenExportingBpjsPcareRegistrationsEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPcareRegistrationsEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareRegistrationsEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPcareRegistrationsEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPcareRegistrationsEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKdproviderpeserta())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRujukinternal())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareRegistrationsEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPcareRegistrationsEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPcareRegistrationsEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPcareRegistrationsEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPcareRegistrationsEntity.class));
		when(service.validateEntityRelations(any(BpjsPcareRegistrationsEntity.class))).thenReturn(new HashSet<>());
