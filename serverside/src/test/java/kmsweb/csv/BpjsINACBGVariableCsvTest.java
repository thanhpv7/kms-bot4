...
import kmsweb.controllers.BpjsINACBGVariableController;
...
@Tag("CSV")
public class BpjsINACBGVariableCsvTest {

	@Mock
	private BpjsINACBGVariableService service;
	...
	private BpjsINACBGVariableController controller;

	private CsvUtils<BpjsINACBGVariableEntity, BpjsINACBGVariableRepository, BpjsINACBGVariableEntityAudit, BpjsINACBGVariableService> csvUtils;

	private final BpjsINACBGVariableFactory factory = new BpjsINACBGVariableFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsINACBGVariableController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsINACBGVariableEntity.class);
	}

	@Test
	void whenExportingBpjsINACBGVariableEntities_thenHeadersAreValid() throws Exception {
		List<BpjsINACBGVariableEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsINACBGVariableEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsINACBGVariableEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsINACBGVariableEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVariableCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getEnglish())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIndonesian())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsINACBGVariableEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsINACBGVariableEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsINACBGVariableEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsINACBGVariableEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsINACBGVariableEntity.class));
		when(service.validateEntityRelations(any(BpjsINACBGVariableEntity.class))).thenReturn(new HashSet<>());
