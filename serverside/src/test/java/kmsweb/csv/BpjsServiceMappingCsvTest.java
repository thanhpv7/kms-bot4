...
import kmsweb.controllers.BpjsServiceMappingController;
...
@Tag("CSV")
public class BpjsServiceMappingCsvTest {

	@Mock
	private BpjsServiceMappingService service;
	...
	private BpjsServiceMappingController controller;

	private CsvUtils<BpjsServiceMappingEntity, BpjsServiceMappingRepository, BpjsServiceMappingEntityAudit, BpjsServiceMappingService> csvUtils;

	private final BpjsServiceMappingFactory factory = new BpjsServiceMappingFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsServiceMappingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsServiceMappingEntity.class);
	}

	@Test
	void whenExportingBpjsServiceMappingEntities_thenHeadersAreValid() throws Exception {
		List<BpjsServiceMappingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsServiceMappingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsServiceMappingEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsServiceMappingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefService())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCategorizedAsSurgery()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsServiceMappingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsServiceMappingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsServiceMappingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsServiceMappingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsServiceMappingEntity.class));
		when(service.validateEntityRelations(any(BpjsServiceMappingEntity.class))).thenReturn(new HashSet<>());
