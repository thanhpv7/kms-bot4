...
import kmsweb.controllers.BpjsDiagnoseController;
...
@Tag("CSV")
public class BpjsDiagnoseCsvTest {

	@Mock
	private BpjsDiagnoseService service;
	...
	private BpjsDiagnoseController controller;

	private CsvUtils<BpjsDiagnoseEntity, BpjsDiagnoseRepository, BpjsDiagnoseEntityAudit, BpjsDiagnoseService> csvUtils;

	private final BpjsDiagnoseFactory factory = new BpjsDiagnoseFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsDiagnoseController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsDiagnoseEntity.class);
	}

	@Test
	void whenExportingBpjsDiagnoseEntities_thenHeadersAreValid() throws Exception {
		List<BpjsDiagnoseEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDiagnoseEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsDiagnoseEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsDiagnoseEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDiagnoseEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsDiagnoseEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsDiagnoseEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsDiagnoseEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsDiagnoseEntity.class));
		when(service.validateEntityRelations(any(BpjsDiagnoseEntity.class))).thenReturn(new HashSet<>());
