...
import kmsweb.controllers.NhisMedicineController;
...
@Tag("CSV")
public class NhisMedicineCsvTest {

	@Mock
	private NhisMedicineService service;
	...
	private NhisMedicineController controller;

	private CsvUtils<NhisMedicineEntity, NhisMedicineRepository, NhisMedicineEntityAudit, NhisMedicineService> csvUtils;

	private final NhisMedicineFactory factory = new NhisMedicineFactory();
	...
	public void setup() {
		...
		this.controller = new NhisMedicineController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisMedicineEntity.class);
	}

	@Test
	void whenExportingNhisMedicineEntities_thenHeadersAreValid() throws Exception {
		List<NhisMedicineEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisMedicineEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisMedicineEntities_thenEntitiesAreValid() throws Exception {
		List<NhisMedicineEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMedicineCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMedicineName())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDispensedQty()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTotalCost()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getServiceDate()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisMedicineEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisMedicineEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisMedicineEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisMedicineEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisMedicineEntity.class));
		when(service.validateEntityRelations(any(NhisMedicineEntity.class))).thenReturn(new HashSet<>());
