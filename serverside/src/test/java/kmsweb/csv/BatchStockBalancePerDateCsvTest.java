...
import kmsweb.controllers.BatchStockBalancePerDateController;
...
@Tag("CSV")
public class BatchStockBalancePerDateCsvTest {

	@Mock
	private BatchStockBalancePerDateService service;
	...
	private BatchStockBalancePerDateController controller;

	private CsvUtils<BatchStockBalancePerDateEntity, BatchStockBalancePerDateRepository, BatchStockBalancePerDateEntityAudit, BatchStockBalancePerDateService> csvUtils;

	private final BatchStockBalancePerDateFactory factory = new BatchStockBalancePerDateFactory();
	...
	public void setup() {
		...
		this.controller = new BatchStockBalancePerDateController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BatchStockBalancePerDateEntity.class);
	}

	@Test
	void whenExportingBatchStockBalancePerDateEntities_thenHeadersAreValid() throws Exception {
		List<BatchStockBalancePerDateEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BatchStockBalancePerDateEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBatchStockBalancePerDateEntities_thenEntitiesAreValid() throws Exception {
		List<BatchStockBalancePerDateEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAsOfDate()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BatchStockBalancePerDateEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BatchStockBalancePerDateEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BatchStockBalancePerDateEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BatchStockBalancePerDateEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BatchStockBalancePerDateEntity.class));
		when(service.validateEntityRelations(any(BatchStockBalancePerDateEntity.class))).thenReturn(new HashSet<>());
