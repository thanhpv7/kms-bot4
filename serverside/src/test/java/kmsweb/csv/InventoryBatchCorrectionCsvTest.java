...
import kmsweb.controllers.InventoryBatchCorrectionController;
...
@Tag("CSV")
public class InventoryBatchCorrectionCsvTest {

	@Mock
	private InventoryBatchCorrectionService service;
	...
	private InventoryBatchCorrectionController controller;

	private CsvUtils<InventoryBatchCorrectionEntity, InventoryBatchCorrectionRepository, InventoryBatchCorrectionEntityAudit, InventoryBatchCorrectionService> csvUtils;

	private final InventoryBatchCorrectionFactory factory = new InventoryBatchCorrectionFactory();
	...
	public void setup() {
		...
		this.controller = new InventoryBatchCorrectionController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InventoryBatchCorrectionEntity.class);
	}

	@Test
	void whenExportingInventoryBatchCorrectionEntities_thenHeadersAreValid() throws Exception {
		List<InventoryBatchCorrectionEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InventoryBatchCorrectionEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInventoryBatchCorrectionEntities_thenEntitiesAreValid() throws Exception {
		List<InventoryBatchCorrectionEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBatchCorrectionNumber())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTransactionType()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InventoryBatchCorrectionEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InventoryBatchCorrectionEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InventoryBatchCorrectionEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InventoryBatchCorrectionEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InventoryBatchCorrectionEntity.class));
		when(service.validateEntityRelations(any(InventoryBatchCorrectionEntity.class))).thenReturn(new HashSet<>());
