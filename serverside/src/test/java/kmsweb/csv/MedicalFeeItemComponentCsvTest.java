...
import kmsweb.controllers.MedicalFeeItemComponentController;
...
@Tag("CSV")
public class MedicalFeeItemComponentCsvTest {

	@Mock
	private MedicalFeeItemComponentService service;
	...
	private MedicalFeeItemComponentController controller;

	private CsvUtils<MedicalFeeItemComponentEntity, MedicalFeeItemComponentRepository, MedicalFeeItemComponentEntityAudit, MedicalFeeItemComponentService> csvUtils;

	private final MedicalFeeItemComponentFactory factory = new MedicalFeeItemComponentFactory();
	...
	public void setup() {
		...
		this.controller = new MedicalFeeItemComponentController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicalFeeItemComponentEntity.class);
	}

	@Test
	void whenExportingMedicalFeeItemComponentEntities_thenHeadersAreValid() throws Exception {
		List<MedicalFeeItemComponentEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicalFeeItemComponentEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicalFeeItemComponentEntities_thenEntitiesAreValid() throws Exception {
		List<MedicalFeeItemComponentEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicalFeeItemComponentEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicalFeeItemComponentEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicalFeeItemComponentEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicalFeeItemComponentEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicalFeeItemComponentEntity.class));
		when(service.validateEntityRelations(any(MedicalFeeItemComponentEntity.class))).thenReturn(new HashSet<>());
