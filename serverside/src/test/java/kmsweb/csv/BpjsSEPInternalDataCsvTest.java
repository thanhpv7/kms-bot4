...
import kmsweb.controllers.BpjsSEPInternalDataController;
...
@Tag("CSV")
public class BpjsSEPInternalDataCsvTest {

	@Mock
	private BpjsSEPInternalDataService service;
	...
	private BpjsSEPInternalDataController controller;

	private CsvUtils<BpjsSEPInternalDataEntity, BpjsSEPInternalDataRepository, BpjsSEPInternalDataEntityAudit, BpjsSEPInternalDataService> csvUtils;

	private final BpjsSEPInternalDataFactory factory = new BpjsSEPInternalDataFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsSEPInternalDataController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsSEPInternalDataEntity.class);
	}

	@Test
	void whenExportingBpjsSEPInternalDataEntities_thenHeadersAreValid() throws Exception {
		List<BpjsSEPInternalDataEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPInternalDataEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsSEPInternalDataEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsSEPInternalDataEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferredPolyclinic())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferringPolyclinic())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInternalReferralDate())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSepNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefSEPNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSepHealthFacility())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLetterNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getIsInternal())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferringPolyclinicCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDiagnosticSupport())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferringDiagnose())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPInternalDataEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsSEPInternalDataEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsSEPInternalDataEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsSEPInternalDataEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsSEPInternalDataEntity.class));
		when(service.validateEntityRelations(any(BpjsSEPInternalDataEntity.class))).thenReturn(new HashSet<>());
