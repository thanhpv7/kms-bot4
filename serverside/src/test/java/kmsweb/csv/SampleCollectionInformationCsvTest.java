...
import kmsweb.controllers.SampleCollectionInformationController;
...
@Tag("CSV")
public class SampleCollectionInformationCsvTest {

	@Mock
	private SampleCollectionInformationService service;
	...
	private SampleCollectionInformationController controller;

	private CsvUtils<SampleCollectionInformationEntity, SampleCollectionInformationRepository, SampleCollectionInformationEntityAudit, SampleCollectionInformationService> csvUtils;

	private final SampleCollectionInformationFactory factory = new SampleCollectionInformationFactory();
	...
	public void setup() {
		...
		this.controller = new SampleCollectionInformationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SampleCollectionInformationEntity.class);
	}

	@Test
	void whenExportingSampleCollectionInformationEntities_thenHeadersAreValid() throws Exception {
		List<SampleCollectionInformationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SampleCollectionInformationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSampleCollectionInformationEntities_thenEntitiesAreValid() throws Exception {
		List<SampleCollectionInformationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientInformed()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientConsented()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientFastedEnough()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMedicationTakenIn24Hours()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMedicationTakenIn72Hours()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SampleCollectionInformationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SampleCollectionInformationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SampleCollectionInformationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SampleCollectionInformationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SampleCollectionInformationEntity.class));
		when(service.validateEntityRelations(any(SampleCollectionInformationEntity.class))).thenReturn(new HashSet<>());
