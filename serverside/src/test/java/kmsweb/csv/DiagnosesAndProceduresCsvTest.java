...
import kmsweb.controllers.DiagnosesAndProceduresController;
...
@Tag("CSV")
public class DiagnosesAndProceduresCsvTest {

	@Mock
	private DiagnosesAndProceduresService service;
	...
	private DiagnosesAndProceduresController controller;

	private CsvUtils<DiagnosesAndProceduresEntity, DiagnosesAndProceduresRepository, DiagnosesAndProceduresEntityAudit, DiagnosesAndProceduresService> csvUtils;

	private final DiagnosesAndProceduresFactory factory = new DiagnosesAndProceduresFactory();
	...
	public void setup() {
		...
		this.controller = new DiagnosesAndProceduresController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DiagnosesAndProceduresEntity.class);
	}

	@Test
	void whenExportingDiagnosesAndProceduresEntities_thenHeadersAreValid() throws Exception {
		List<DiagnosesAndProceduresEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosesAndProceduresEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDiagnosesAndProceduresEntities_thenEntitiesAreValid() throws Exception {
		List<DiagnosesAndProceduresEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getItemType())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosesAndProceduresEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DiagnosesAndProceduresEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DiagnosesAndProceduresEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DiagnosesAndProceduresEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DiagnosesAndProceduresEntity.class));
		when(service.validateEntityRelations(any(DiagnosesAndProceduresEntity.class))).thenReturn(new HashSet<>());
