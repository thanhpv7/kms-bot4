...
import kmsweb.controllers.BpjsBedAvailabilityController;
...
@Tag("CSV")
public class BpjsBedAvailabilityCsvTest {

	@Mock
	private BpjsBedAvailabilityService service;
	...
	private BpjsBedAvailabilityController controller;

	private CsvUtils<BpjsBedAvailabilityEntity, BpjsBedAvailabilityRepository, BpjsBedAvailabilityEntityAudit, BpjsBedAvailabilityService> csvUtils;

	private final BpjsBedAvailabilityFactory factory = new BpjsBedAvailabilityFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsBedAvailabilityController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsBedAvailabilityEntity.class);
	}

	@Test
	void whenExportingBpjsBedAvailabilityEntities_thenHeadersAreValid() throws Exception {
		List<BpjsBedAvailabilityEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsBedAvailabilityEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsBedAvailabilityEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsBedAvailabilityEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKoderuang())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaruang())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLastupdate())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKapasitas()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTersedia()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTersediapria()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTersediawanita()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTersediapriawanita()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsBedAvailabilityEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsBedAvailabilityEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsBedAvailabilityEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsBedAvailabilityEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsBedAvailabilityEntity.class));
		when(service.validateEntityRelations(any(BpjsBedAvailabilityEntity.class))).thenReturn(new HashSet<>());
