...
import kmsweb.controllers.TariffPackageItemController;
...
@Tag("CSV")
public class TariffPackageItemCsvTest {

	@Mock
	private TariffPackageItemService service;
	...
	private TariffPackageItemController controller;

	private CsvUtils<TariffPackageItemEntity, TariffPackageItemRepository, TariffPackageItemEntityAudit, TariffPackageItemService> csvUtils;

	private final TariffPackageItemFactory factory = new TariffPackageItemFactory();
	...
	public void setup() {
		...
		this.controller = new TariffPackageItemController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TariffPackageItemEntity.class);
	}

	@Test
	void whenExportingTariffPackageItemEntities_thenHeadersAreValid() throws Exception {
		List<TariffPackageItemEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TariffPackageItemEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTariffPackageItemEntities_thenEntitiesAreValid() throws Exception {
		List<TariffPackageItemEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTariffName())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getModifiable()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TariffPackageItemEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TariffPackageItemEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TariffPackageItemEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TariffPackageItemEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TariffPackageItemEntity.class));
		when(service.validateEntityRelations(any(TariffPackageItemEntity.class))).thenReturn(new HashSet<>());
