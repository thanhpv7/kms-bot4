...
import kmsweb.controllers.RoomFacilityController;
...
@Tag("CSV")
public class RoomFacilityCsvTest {
	...
	private RoomFacilityController controller;

	private CsvUtils<RoomFacilityEntity, RoomFacilityRepository, RoomFacilityEntityAudit, RoomFacilityService> csvUtils;

	private final RoomFacilityFactory factory = new RoomFacilityFactory();
	...
	public void setup() {
		...
		this.controller = new RoomFacilityController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, RoomFacilityEntity.class);
	}

	@Test
	void whenExportingRoomFacilityEntities_thenHeadersAreValid() throws Exception {
		List<RoomFacilityEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(RoomFacilityEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingRoomFacilityEntities_thenEntitiesAreValid() throws Exception {
		List<RoomFacilityEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRoomNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRoomName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDepartment())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getGenderAllocation())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRoomFacilityPicture()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCategory())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(RoomFacilityEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		RoomFacilityEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(RoomFacilityEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(RoomFacilityEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(RoomFacilityEntity.class));
		when(service.validateEntityRelations(any(RoomFacilityEntity.class))).thenReturn(new HashSet<>());
