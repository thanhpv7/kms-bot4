...
import kmsweb.controllers.BpjsDrugGenericPRBController;
...
@Tag("CSV")
public class BpjsDrugGenericPRBCsvTest {

	@Mock
	private BpjsDrugGenericPRBService service;
	...
	private BpjsDrugGenericPRBController controller;

	private CsvUtils<BpjsDrugGenericPRBEntity, BpjsDrugGenericPRBRepository, BpjsDrugGenericPRBEntityAudit, BpjsDrugGenericPRBService> csvUtils;

	private final BpjsDrugGenericPRBFactory factory = new BpjsDrugGenericPRBFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsDrugGenericPRBController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsDrugGenericPRBEntity.class);
	}

	@Test
	void whenExportingBpjsDrugGenericPRBEntities_thenHeadersAreValid() throws Exception {
		List<BpjsDrugGenericPRBEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDrugGenericPRBEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsDrugGenericPRBEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsDrugGenericPRBEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefDrugCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefDrugName())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDrugGenericPRBEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsDrugGenericPRBEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsDrugGenericPRBEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsDrugGenericPRBEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsDrugGenericPRBEntity.class));
		when(service.validateEntityRelations(any(BpjsDrugGenericPRBEntity.class))).thenReturn(new HashSet<>());
