...
import kmsweb.controllers.BpjsClaimDataController;
...
@Tag("CSV")
public class BpjsClaimDataCsvTest {

	@Mock
	private BpjsClaimDataService service;
	...
	private BpjsClaimDataController controller;

	private CsvUtils<BpjsClaimDataEntity, BpjsClaimDataRepository, BpjsClaimDataEntityAudit, BpjsClaimDataService> csvUtils;

	private final BpjsClaimDataFactory factory = new BpjsClaimDataFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsClaimDataController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsClaimDataEntity.class);
	}

	@Test
	void whenExportingBpjsClaimDataEntities_thenHeadersAreValid() throws Exception {
		List<BpjsClaimDataEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsClaimDataEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsClaimDataEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsClaimDataEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInacbgCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInacbgName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSubmittedTariff())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getApproval())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getGrouperTariff())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHospitalTariff())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTopUp())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTreatmentClass())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFpkNo())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSepDate()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsClaimDataEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsClaimDataEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsClaimDataEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsClaimDataEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsClaimDataEntity.class));
		when(service.validateEntityRelations(any(BpjsClaimDataEntity.class))).thenReturn(new HashSet<>());
