...
import kmsweb.controllers.BpjsHospitalizationPlanController;
...
@Tag("CSV")
public class BpjsHospitalizationPlanCsvTest {

	@Mock
	private BpjsHospitalizationPlanService service;
	...
	private BpjsHospitalizationPlanController controller;

	private CsvUtils<BpjsHospitalizationPlanEntity, BpjsHospitalizationPlanRepository, BpjsHospitalizationPlanEntityAudit, BpjsHospitalizationPlanService> csvUtils;

	private final BpjsHospitalizationPlanFactory factory = new BpjsHospitalizationPlanFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsHospitalizationPlanController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsHospitalizationPlanEntity.class);
	}

	@Test
	void whenExportingBpjsHospitalizationPlanEntities_thenHeadersAreValid() throws Exception {
		List<BpjsHospitalizationPlanEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsHospitalizationPlanEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsHospitalizationPlanEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsHospitalizationPlanEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTglrencanakontrol()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNospri())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsHospitalizationPlanEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsHospitalizationPlanEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsHospitalizationPlanEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsHospitalizationPlanEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsHospitalizationPlanEntity.class));
		when(service.validateEntityRelations(any(BpjsHospitalizationPlanEntity.class))).thenReturn(new HashSet<>());
