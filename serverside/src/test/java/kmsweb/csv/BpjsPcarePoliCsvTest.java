...
import kmsweb.controllers.BpjsPcarePoliController;
...
@Tag("CSV")
public class BpjsPcarePoliCsvTest {

	@Mock
	private BpjsPcarePoliService service;
	...
	private BpjsPcarePoliController controller;

	private CsvUtils<BpjsPcarePoliEntity, BpjsPcarePoliRepository, BpjsPcarePoliEntityAudit, BpjsPcarePoliService> csvUtils;

	private final BpjsPcarePoliFactory factory = new BpjsPcarePoliFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPcarePoliController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPcarePoliEntity.class);
	}

	@Test
	void whenExportingBpjsPcarePoliEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPcarePoliEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcarePoliEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPcarePoliEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPcarePoliEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPolisakit()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcarePoliEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPcarePoliEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPcarePoliEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPcarePoliEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPcarePoliEntity.class));
		when(service.validateEntityRelations(any(BpjsPcarePoliEntity.class))).thenReturn(new HashSet<>());
