...
import kmsweb.controllers.BpjsPatientReferralController;
...
@Tag("CSV")
public class BpjsPatientReferralCsvTest {

	@Mock
	private BpjsPatientReferralService service;
	...
	private BpjsPatientReferralController controller;

	private CsvUtils<BpjsPatientReferralEntity, BpjsPatientReferralRepository, BpjsPatientReferralEntityAudit, BpjsPatientReferralService> csvUtils;

	private final BpjsPatientReferralFactory factory = new BpjsPatientReferralFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPatientReferralController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPatientReferralEntity.class);
	}

	@Test
	void whenExportingBpjsPatientReferralEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPatientReferralEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPatientReferralEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPatientReferralEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPatientReferralEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferredType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferralDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferalDatePlan()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPatientReferralEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPatientReferralEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPatientReferralEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPatientReferralEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPatientReferralEntity.class));
		when(service.validateEntityRelations(any(BpjsPatientReferralEntity.class))).thenReturn(new HashSet<>());
