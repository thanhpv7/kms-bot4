...
import kmsweb.controllers.BedReserveController;
...
@Tag("CSV")
public class BedReserveCsvTest {

	@Mock
	private BedReserveService service;
	...
	private BedReserveController controller;

	private CsvUtils<BedReserveEntity, BedReserveRepository, BedReserveEntityAudit, BedReserveService> csvUtils;

	private final BedReserveFactory factory = new BedReserveFactory();
	...
	public void setup() {
		...
		this.controller = new BedReserveController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BedReserveEntity.class);
	}

	@Test
	void whenExportingBedReserveEntities_thenHeadersAreValid() throws Exception {
		List<BedReserveEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BedReserveEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBedReserveEntities_thenEntitiesAreValid() throws Exception {
		List<BedReserveEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BedReserveEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BedReserveEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BedReserveEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BedReserveEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BedReserveEntity.class));
		when(service.validateEntityRelations(any(BedReserveEntity.class))).thenReturn(new HashSet<>());
