...
import kmsweb.controllers.BedFacilityController;
...
@Tag("CSV")
public class BedFacilityCsvTest {

	@Mock
	private BedFacilityService service;
	...
	private BedFacilityController controller;

	private CsvUtils<BedFacilityEntity, BedFacilityRepository, BedFacilityEntityAudit, BedFacilityService> csvUtils;

	private final BedFacilityFactory factory = new BedFacilityFactory();
	...
	public void setup() {
		...
		this.controller = new BedFacilityController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BedFacilityEntity.class);
	}

	@Test
	void whenExportingBedFacilityEntities_thenHeadersAreValid() throws Exception {
		List<BedFacilityEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BedFacilityEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBedFacilityEntities_thenEntitiesAreValid() throws Exception {
		List<BedFacilityEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBedNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBedName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTreatmentClass())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAvailability())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BedFacilityEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BedFacilityEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BedFacilityEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BedFacilityEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BedFacilityEntity.class));
		when(service.validateEntityRelations(any(BedFacilityEntity.class))).thenReturn(new HashSet<>());
