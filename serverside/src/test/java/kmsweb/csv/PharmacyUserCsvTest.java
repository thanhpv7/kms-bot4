...
import kmsweb.controllers.PharmacyUserController;
...
@Tag("CSV")
public class PharmacyUserCsvTest {

	@Mock
	private PharmacyUserService service;
	...
	private PharmacyUserController controller;

	private CsvUtils<PharmacyUserEntity, PharmacyUserRepository, PharmacyUserEntityAudit, PharmacyUserService> csvUtils;

	private final PharmacyUserFactory factory = new PharmacyUserFactory();
	...
	public void setup() {
		...
		this.controller = new PharmacyUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PharmacyUserEntity.class);
	}

	@Test
	void whenExportingPharmacyUserEntities_thenHeadersAreValid() throws Exception {
		List<PharmacyUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PharmacyUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPharmacyUserEntities_thenEntitiesAreValid() throws Exception {
		List<PharmacyUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PharmacyUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PharmacyUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PharmacyUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PharmacyUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PharmacyUserEntity.class));
		when(service.validateEntityRelations(any(PharmacyUserEntity.class))).thenReturn(new HashSet<>());
