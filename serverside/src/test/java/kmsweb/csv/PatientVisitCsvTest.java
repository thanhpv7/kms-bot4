...
import kmsweb.controllers.PatientVisitController;
...
@Tag("CSV")
public class PatientVisitCsvTest {
	...
	private PatientVisitController controller;

	private CsvUtils<PatientVisitEntity, PatientVisitRepository, PatientVisitEntityAudit, PatientVisitService> csvUtils;

	private final PatientVisitFactory factory = new PatientVisitFactory();
	...
	public void setup() {
		...
		this.controller = new PatientVisitController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientVisitEntity.class);
	}

	@Test
	void whenExportingPatientVisitEntities_thenHeadersAreValid() throws Exception {
		List<PatientVisitEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientVisitEntities_thenEntitiesAreValid() throws Exception {
		List<PatientVisitEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVisitNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRegistrationType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVisitDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRegistrationLocation())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientVisitEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientVisitEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientVisitEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientVisitEntity.class));
		when(service.validateEntityRelations(any(PatientVisitEntity.class))).thenReturn(new HashSet<>());
