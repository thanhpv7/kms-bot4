...
import kmsweb.controllers.DiagnosisNandaController;
...
@Tag("CSV")
public class DiagnosisNandaCsvTest {

	@Mock
	private DiagnosisNandaService service;
	...
	private DiagnosisNandaController controller;

	private CsvUtils<DiagnosisNandaEntity, DiagnosisNandaRepository, DiagnosisNandaEntityAudit, DiagnosisNandaService> csvUtils;

	private final DiagnosisNandaFactory factory = new DiagnosisNandaFactory();
	...
	public void setup() {
		...
		this.controller = new DiagnosisNandaController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DiagnosisNandaEntity.class);
	}

	@Test
	void whenExportingDiagnosisNandaEntities_thenHeadersAreValid() throws Exception {
		List<DiagnosisNandaEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosisNandaEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDiagnosisNandaEntities_thenEntitiesAreValid() throws Exception {
		List<DiagnosisNandaEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNursingNotes())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosisNandaEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DiagnosisNandaEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DiagnosisNandaEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DiagnosisNandaEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DiagnosisNandaEntity.class));
		when(service.validateEntityRelations(any(DiagnosisNandaEntity.class))).thenReturn(new HashSet<>());
