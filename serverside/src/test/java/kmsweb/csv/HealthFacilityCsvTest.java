...
import kmsweb.controllers.HealthFacilityController;
...
@Tag("CSV")
public class HealthFacilityCsvTest {
	...
	private HealthFacilityController controller;

	private CsvUtils<HealthFacilityEntity, HealthFacilityRepository, HealthFacilityEntityAudit, HealthFacilityService> csvUtils;

	private final HealthFacilityFactory factory = new HealthFacilityFactory();
	...
	public void setup() {
		...
		this.controller = new HealthFacilityController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, HealthFacilityEntity.class);
	}

	@Test
	void whenExportingHealthFacilityEntities_thenHeadersAreValid() throws Exception {
		List<HealthFacilityEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(HealthFacilityEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingHealthFacilityEntities_thenEntitiesAreValid() throws Exception {
		List<HealthFacilityEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHealthFacilityID())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProvince())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMobilePhoneNumber())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSalutation())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPhoneNumberPOC())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMobilePhoneNumberPOC())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getEmailPOC())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHealthFacilityType())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(HealthFacilityEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		HealthFacilityEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(HealthFacilityEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(HealthFacilityEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(HealthFacilityEntity.class));
		when(service.validateEntityRelations(any(HealthFacilityEntity.class))).thenReturn(new HashSet<>());
