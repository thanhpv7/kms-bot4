...
import kmsweb.controllers.WarehouseController;
...
public class WarehouseCsvTest {
	...
	private CsvUtils<WarehouseEntity, WarehouseRepository, WarehouseEntityAudit, WarehouseService> csvUtils;

	private final WarehouseFactory factory = new WarehouseFactory();
	...
	public void setup() {
		...
		this.controller = new WarehouseController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, WarehouseEntity.class);
	}

	@Test
	void whenExportingWarehouseEntities_thenHeadersAreValid() throws Exception {
		List<WarehouseEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingWarehouseEntities_thenEntitiesAreValid() throws Exception {
		List<WarehouseEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getWarehouseNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getWarehouseName())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		WarehouseEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(WarehouseEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(WarehouseEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(WarehouseEntity.class));
		when(service.validateEntityRelations(any(WarehouseEntity.class))).thenReturn(new HashSet<>());
