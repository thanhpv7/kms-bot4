...
import kmsweb.controllers.ChartOfAccountMappingController;
...
@Tag("CSV")
public class ChartOfAccountMappingCsvTest {

	@Mock
	private ChartOfAccountMappingService service;
	...
	private ChartOfAccountMappingController controller;

	private CsvUtils<ChartOfAccountMappingEntity, ChartOfAccountMappingRepository, ChartOfAccountMappingEntityAudit, ChartOfAccountMappingService> csvUtils;

	private final ChartOfAccountMappingFactory factory = new ChartOfAccountMappingFactory();
	...
	public void setup() {
		...
		this.controller = new ChartOfAccountMappingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ChartOfAccountMappingEntity.class);
	}

	@Test
	void whenExportingChartOfAccountMappingEntities_thenHeadersAreValid() throws Exception {
		List<ChartOfAccountMappingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ChartOfAccountMappingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingChartOfAccountMappingEntities_thenEntitiesAreValid() throws Exception {
		List<ChartOfAccountMappingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAccountTypeDescription())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ChartOfAccountMappingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ChartOfAccountMappingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ChartOfAccountMappingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ChartOfAccountMappingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ChartOfAccountMappingEntity.class));
		when(service.validateEntityRelations(any(ChartOfAccountMappingEntity.class))).thenReturn(new HashSet<>());
