...
import kmsweb.controllers.SystemAdminUserController;
...
@Tag("CSV")
public class SystemAdminUserCsvTest {

	@Mock
	private SystemAdminUserService service;
	...
	private SystemAdminUserController controller;

	private CsvUtils<SystemAdminUserEntity, SystemAdminUserRepository, SystemAdminUserEntityAudit, SystemAdminUserService> csvUtils;

	private final SystemAdminUserFactory factory = new SystemAdminUserFactory();
	...
	public void setup() {
		...
		this.controller = new SystemAdminUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SystemAdminUserEntity.class);
	}

	@Test
	void whenExportingSystemAdminUserEntities_thenHeadersAreValid() throws Exception {
		List<SystemAdminUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SystemAdminUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSystemAdminUserEntities_thenEntitiesAreValid() throws Exception {
		List<SystemAdminUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SystemAdminUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SystemAdminUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SystemAdminUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SystemAdminUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SystemAdminUserEntity.class));
		when(service.validateEntityRelations(any(SystemAdminUserEntity.class))).thenReturn(new HashSet<>());
