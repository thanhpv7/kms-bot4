...
import kmsweb.controllers.HospitalGroupInformationController;
...
@Tag("CSV")
public class HospitalGroupInformationCsvTest {

	@Mock
	private HospitalGroupInformationService service;
	...
	private HospitalGroupInformationController controller;

	private CsvUtils<HospitalGroupInformationEntity, HospitalGroupInformationRepository, HospitalGroupInformationEntityAudit, HospitalGroupInformationService> csvUtils;

	private final HospitalGroupInformationFactory factory = new HospitalGroupInformationFactory();
	...
	public void setup() {
		...
		this.controller = new HospitalGroupInformationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, HospitalGroupInformationEntity.class);
	}

	@Test
	void whenExportingHospitalGroupInformationEntities_thenHeadersAreValid() throws Exception {
		List<HospitalGroupInformationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(HospitalGroupInformationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingHospitalGroupInformationEntities_thenEntitiesAreValid() throws Exception {
		List<HospitalGroupInformationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHospitalGroupCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHospitalGroupName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHospitalGroupCorporationName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHospitalGroupDirector())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOperationalLicenseNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHospitalGroupCorporationStatus())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(HospitalGroupInformationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		HospitalGroupInformationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(HospitalGroupInformationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(HospitalGroupInformationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(HospitalGroupInformationEntity.class));
		when(service.validateEntityRelations(any(HospitalGroupInformationEntity.class))).thenReturn(new HashSet<>());
