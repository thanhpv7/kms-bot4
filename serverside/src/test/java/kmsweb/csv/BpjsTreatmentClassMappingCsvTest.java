...
import kmsweb.controllers.BpjsTreatmentClassMappingController;
...
@Tag("CSV")
public class BpjsTreatmentClassMappingCsvTest {

	@Mock
	private BpjsTreatmentClassMappingService service;
	...
	private BpjsTreatmentClassMappingController controller;

	private CsvUtils<BpjsTreatmentClassMappingEntity, BpjsTreatmentClassMappingRepository, BpjsTreatmentClassMappingEntityAudit, BpjsTreatmentClassMappingService> csvUtils;

	private final BpjsTreatmentClassMappingFactory factory = new BpjsTreatmentClassMappingFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsTreatmentClassMappingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsTreatmentClassMappingEntity.class);
	}

	@Test
	void whenExportingBpjsTreatmentClassMappingEntities_thenHeadersAreValid() throws Exception {
		List<BpjsTreatmentClassMappingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTreatmentClassMappingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsTreatmentClassMappingEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsTreatmentClassMappingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRefName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getInternalTreatmentClass())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsTreatmentClassMappingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsTreatmentClassMappingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsTreatmentClassMappingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsTreatmentClassMappingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsTreatmentClassMappingEntity.class));
		when(service.validateEntityRelations(any(BpjsTreatmentClassMappingEntity.class))).thenReturn(new HashSet<>());
