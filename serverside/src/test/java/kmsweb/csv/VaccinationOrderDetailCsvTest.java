...
import kmsweb.controllers.VaccinationOrderDetailController;
...
@Tag("CSV")
public class VaccinationOrderDetailCsvTest {

	@Mock
	private VaccinationOrderDetailService service;
	...
	private VaccinationOrderDetailController controller;

	private CsvUtils<VaccinationOrderDetailEntity, VaccinationOrderDetailRepository, VaccinationOrderDetailEntityAudit, VaccinationOrderDetailService> csvUtils;

	private final VaccinationOrderDetailFactory factory = new VaccinationOrderDetailFactory();
	...
	public void setup() {
		...
		this.controller = new VaccinationOrderDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, VaccinationOrderDetailEntity.class);
	}

	@Test
	void whenExportingVaccinationOrderDetailEntities_thenHeadersAreValid() throws Exception {
		List<VaccinationOrderDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(VaccinationOrderDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingVaccinationOrderDetailEntities_thenEntitiesAreValid() throws Exception {
		List<VaccinationOrderDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVaccinationDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVaccinationType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getVaccinationDetail())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(VaccinationOrderDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		VaccinationOrderDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(VaccinationOrderDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(VaccinationOrderDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(VaccinationOrderDetailEntity.class));
		when(service.validateEntityRelations(any(VaccinationOrderDetailEntity.class))).thenReturn(new HashSet<>());
