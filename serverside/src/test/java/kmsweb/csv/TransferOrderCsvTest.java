...
import kmsweb.controllers.TransferOrderController;
...
@Tag("CSV")
public class TransferOrderCsvTest {
	...
	private CsvUtils<TransferOrderEntity, TransferOrderRepository, TransferOrderEntityAudit, TransferOrderService> csvUtils;

	private final TransferOrderFactory factory = new TransferOrderFactory();
	...
	public void setup() {
		...
		this.controller = new TransferOrderController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TransferOrderEntity.class);
	}

	@Test
	void whenExportingTransferOrderEntities_thenHeadersAreValid() throws Exception {
		List<TransferOrderEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TransferOrderEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTransferOrderEntities_thenEntitiesAreValid() throws Exception {
		List<TransferOrderEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTransferOrderNumber())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TransferOrderEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TransferOrderEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TransferOrderEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TransferOrderEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TransferOrderEntity.class));
		when(service.validateEntityRelations(any(TransferOrderEntity.class))).thenReturn(new HashSet<>());
