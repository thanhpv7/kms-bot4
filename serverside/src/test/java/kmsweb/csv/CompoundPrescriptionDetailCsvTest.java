...
import kmsweb.controllers.CompoundPrescriptionDetailController;
...
@Tag("CSV")
public class CompoundPrescriptionDetailCsvTest {

	@Mock
	private CompoundPrescriptionDetailService service;
	...
	private CompoundPrescriptionDetailController controller;

	private CsvUtils<CompoundPrescriptionDetailEntity, CompoundPrescriptionDetailRepository, CompoundPrescriptionDetailEntityAudit, CompoundPrescriptionDetailService> csvUtils;

	private final CompoundPrescriptionDetailFactory factory = new CompoundPrescriptionDetailFactory();
	...
	public void setup() {
		...
		this.controller = new CompoundPrescriptionDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, CompoundPrescriptionDetailEntity.class);
	}

	@Test
	void whenExportingCompoundPrescriptionDetailEntities_thenHeadersAreValid() throws Exception {
		List<CompoundPrescriptionDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(CompoundPrescriptionDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingCompoundPrescriptionDetailEntities_thenEntitiesAreValid() throws Exception {
		List<CompoundPrescriptionDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRequestQuantity()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(CompoundPrescriptionDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		CompoundPrescriptionDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(CompoundPrescriptionDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(CompoundPrescriptionDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(CompoundPrescriptionDetailEntity.class));
		when(service.validateEntityRelations(any(CompoundPrescriptionDetailEntity.class))).thenReturn(new HashSet<>());
