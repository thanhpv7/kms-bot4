...
import kmsweb.controllers.BpjsUpdateDismissalDateController;
...
@Tag("CSV")
public class BpjsUpdateDismissalDateCsvTest {

	@Mock
	private BpjsUpdateDismissalDateService service;
	...
	private BpjsUpdateDismissalDateController controller;

	private CsvUtils<BpjsUpdateDismissalDateEntity, BpjsUpdateDismissalDateRepository, BpjsUpdateDismissalDateEntityAudit, BpjsUpdateDismissalDateService> csvUtils;

	private final BpjsUpdateDismissalDateFactory factory = new BpjsUpdateDismissalDateFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsUpdateDismissalDateController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsUpdateDismissalDateEntity.class);
	}

	@Test
	void whenExportingBpjsUpdateDismissalDateEntities_thenHeadersAreValid() throws Exception {
		List<BpjsUpdateDismissalDateEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsUpdateDismissalDateEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsUpdateDismissalDateEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsUpdateDismissalDateEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDeathCertificateLetterNo())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPoliceReportNo())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsUpdateDismissalDateEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsUpdateDismissalDateEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsUpdateDismissalDateEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsUpdateDismissalDateEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsUpdateDismissalDateEntity.class));
		when(service.validateEntityRelations(any(BpjsUpdateDismissalDateEntity.class))).thenReturn(new HashSet<>());
