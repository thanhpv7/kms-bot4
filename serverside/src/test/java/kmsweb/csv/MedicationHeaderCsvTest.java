...
import kmsweb.controllers.MedicationHeaderController;
...
@Tag("CSV")
public class MedicationHeaderCsvTest {

	@Mock
	private MedicationHeaderService service;
	...
	private MedicationHeaderController controller;

	private CsvUtils<MedicationHeaderEntity, MedicationHeaderRepository, MedicationHeaderEntityAudit, MedicationHeaderService> csvUtils;

	private final MedicationHeaderFactory factory = new MedicationHeaderFactory();
	...
	public void setup() {
		...
		this.controller = new MedicationHeaderController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, MedicationHeaderEntity.class);
	}

	@Test
	void whenExportingMedicationHeaderEntities_thenHeadersAreValid() throws Exception {
		List<MedicationHeaderEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(MedicationHeaderEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingMedicationHeaderEntities_thenEntitiesAreValid() throws Exception {
		List<MedicationHeaderEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMedicationNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDispensingType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOriginalMedicationNumber())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(MedicationHeaderEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		MedicationHeaderEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(MedicationHeaderEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(MedicationHeaderEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(MedicationHeaderEntity.class));
		when(service.validateEntityRelations(any(MedicationHeaderEntity.class))).thenReturn(new HashSet<>());
