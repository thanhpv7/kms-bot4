...
import kmsweb.controllers.PatientPaymentInsuranceController;
...
@Tag("CSV")
public class PatientPaymentInsuranceCsvTest {

	@Mock
	private PatientPaymentInsuranceService service;
	...
	private PatientPaymentInsuranceController controller;

	private CsvUtils<PatientPaymentInsuranceEntity, PatientPaymentInsuranceRepository, PatientPaymentInsuranceEntityAudit, PatientPaymentInsuranceService> csvUtils;

	private final PatientPaymentInsuranceFactory factory = new PatientPaymentInsuranceFactory();
	...
	public void setup() {
		...
		this.controller = new PatientPaymentInsuranceController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientPaymentInsuranceEntity.class);
	}

	@Test
	void whenExportingPatientPaymentInsuranceEntities_thenHeadersAreValid() throws Exception {
		List<PatientPaymentInsuranceEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentInsuranceEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientPaymentInsuranceEntities_thenEntitiesAreValid() throws Exception {
		List<PatientPaymentInsuranceEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientPaymentInsuranceEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientPaymentInsuranceEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientPaymentInsuranceEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientPaymentInsuranceEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientPaymentInsuranceEntity.class));
		when(service.validateEntityRelations(any(PatientPaymentInsuranceEntity.class))).thenReturn(new HashSet<>());
