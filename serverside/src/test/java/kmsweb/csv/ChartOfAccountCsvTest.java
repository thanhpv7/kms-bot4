...
import kmsweb.controllers.ChartOfAccountController;
...
@Tag("CSV")
public class ChartOfAccountCsvTest {
	...
	private CsvUtils<ChartOfAccountEntity, ChartOfAccountRepository, ChartOfAccountEntityAudit, ChartOfAccountService> csvUtils;

	private final ChartOfAccountFactory factory = new ChartOfAccountFactory();
	...
	public void setup() {
		...
		this.controller = new ChartOfAccountController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ChartOfAccountEntity.class);
	}

	@Test
	void whenExportingChartOfAccountEntities_thenHeadersAreValid() throws Exception {
		List<ChartOfAccountEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ChartOfAccountEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingChartOfAccountEntities_thenEntitiesAreValid() throws Exception {
		List<ChartOfAccountEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAccountCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAccountName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAccountType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTaxCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDashboard()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExpenseClaim()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getEnablePayment()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ChartOfAccountEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ChartOfAccountEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ChartOfAccountEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ChartOfAccountEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ChartOfAccountEntity.class));
		when(service.validateEntityRelations(any(ChartOfAccountEntity.class))).thenReturn(new HashSet<>());
