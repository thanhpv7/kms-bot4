...
import kmsweb.controllers.BpjsHealthFacilityController;
...
@Tag("CSV")
public class BpjsHealthFacilityCsvTest {

	@Mock
	private BpjsHealthFacilityService service;
	...
	private BpjsHealthFacilityController controller;

	private CsvUtils<BpjsHealthFacilityEntity, BpjsHealthFacilityRepository, BpjsHealthFacilityEntityAudit, BpjsHealthFacilityService> csvUtils;

	private final BpjsHealthFacilityFactory factory = new BpjsHealthFacilityFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsHealthFacilityController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsHealthFacilityEntity.class);
	}

	@Test
	void whenExportingBpjsHealthFacilityEntities_thenHeadersAreValid() throws Exception {
		List<BpjsHealthFacilityEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsHealthFacilityEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsHealthFacilityEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsHealthFacilityEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsHealthFacilityEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsHealthFacilityEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsHealthFacilityEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsHealthFacilityEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsHealthFacilityEntity.class));
		when(service.validateEntityRelations(any(BpjsHealthFacilityEntity.class))).thenReturn(new HashSet<>());
