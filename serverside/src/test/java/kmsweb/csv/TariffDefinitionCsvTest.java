...
import kmsweb.controllers.TariffDefinitionController;
...
@Tag("CSV")
public class TariffDefinitionCsvTest {

	@Mock
	private TariffDefinitionService service;
	...
	private TariffDefinitionController controller;

	private CsvUtils<TariffDefinitionEntity, TariffDefinitionRepository, TariffDefinitionEntityAudit, TariffDefinitionService> csvUtils;

	private final TariffDefinitionFactory factory = new TariffDefinitionFactory();
	...
	public void setup() {
		...
		this.controller = new TariffDefinitionController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TariffDefinitionEntity.class);
	}

	@Test
	void whenExportingTariffDefinitionEntities_thenHeadersAreValid() throws Exception {
		List<TariffDefinitionEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TariffDefinitionEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTariffDefinitionEntities_thenEntitiesAreValid() throws Exception {
		List<TariffDefinitionEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTariffPrice()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TariffDefinitionEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TariffDefinitionEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TariffDefinitionEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TariffDefinitionEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TariffDefinitionEntity.class));
		when(service.validateEntityRelations(any(TariffDefinitionEntity.class))).thenReturn(new HashSet<>());
