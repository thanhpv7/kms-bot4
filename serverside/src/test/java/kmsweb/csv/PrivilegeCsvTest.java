...
import kmsweb.controllers.PrivilegeController;
...
public class PrivilegeCsvTest {
	...
	private CsvUtils<PrivilegeEntity, PrivilegeRepository, PrivilegeEntityAudit, PrivilegeService> csvUtils;

	private final PrivilegeFactory factory = new PrivilegeFactory();
	...
	public void setup() {
		...
		this.controller = new PrivilegeController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PrivilegeEntity.class);
	}

	@Test
	void whenExportingPrivilegeEntities_thenHeadersAreValid() throws Exception {
		List<PrivilegeEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PrivilegeEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPrivilegeEntities_thenEntitiesAreValid() throws Exception {
		List<PrivilegeEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTargetEntity())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAllowCreate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAllowRead()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAllowUpdate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAllowDelete()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PrivilegeEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PrivilegeEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PrivilegeEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PrivilegeEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PrivilegeEntity.class));
		when(service.validateEntityRelations(any(PrivilegeEntity.class))).thenReturn(new HashSet<>());
