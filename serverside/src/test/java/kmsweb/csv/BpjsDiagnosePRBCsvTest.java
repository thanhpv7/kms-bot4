...
import kmsweb.controllers.BpjsDiagnosePRBController;
...
@Tag("CSV")
public class BpjsDiagnosePRBCsvTest {

	@Mock
	private BpjsDiagnosePRBService service;
	...
	private BpjsDiagnosePRBController controller;

	private CsvUtils<BpjsDiagnosePRBEntity, BpjsDiagnosePRBRepository, BpjsDiagnosePRBEntityAudit, BpjsDiagnosePRBService> csvUtils;

	private final BpjsDiagnosePRBFactory factory = new BpjsDiagnosePRBFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsDiagnosePRBController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsDiagnosePRBEntity.class);
	}

	@Test
	void whenExportingBpjsDiagnosePRBEntities_thenHeadersAreValid() throws Exception {
		List<BpjsDiagnosePRBEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDiagnosePRBEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsDiagnosePRBEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsDiagnosePRBEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsDiagnosePRBEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsDiagnosePRBEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsDiagnosePRBEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsDiagnosePRBEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsDiagnosePRBEntity.class));
		when(service.validateEntityRelations(any(BpjsDiagnosePRBEntity.class))).thenReturn(new HashSet<>());
