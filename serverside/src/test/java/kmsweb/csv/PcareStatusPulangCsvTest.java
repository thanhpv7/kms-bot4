...
import kmsweb.controllers.PcareStatusPulangController;
...
@Tag("CSV")
public class PcareStatusPulangCsvTest {

	@Mock
	private PcareStatusPulangService service;
	...
	private PcareStatusPulangController controller;

	private CsvUtils<PcareStatusPulangEntity, PcareStatusPulangRepository, PcareStatusPulangEntityAudit, PcareStatusPulangService> csvUtils;

	private final PcareStatusPulangFactory factory = new PcareStatusPulangFactory();
	...
	public void setup() {
		...
		this.controller = new PcareStatusPulangController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareStatusPulangEntity.class);
	}

	@Test
	void whenExportingPcareStatusPulangEntities_thenHeadersAreValid() throws Exception {
		List<PcareStatusPulangEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareStatusPulangEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareStatusPulangEntities_thenEntitiesAreValid() throws Exception {
		List<PcareStatusPulangEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodeStatusPulang())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaStatusPulang())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodeReferensiInternal())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareStatusPulangEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareStatusPulangEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareStatusPulangEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareStatusPulangEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareStatusPulangEntity.class));
		when(service.validateEntityRelations(any(PcareStatusPulangEntity.class))).thenReturn(new HashSet<>());
