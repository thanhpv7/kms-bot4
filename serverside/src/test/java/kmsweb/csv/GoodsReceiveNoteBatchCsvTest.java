...
import kmsweb.controllers.GoodsReceiveNoteBatchController;
...
@Tag("CSV")
public class GoodsReceiveNoteBatchCsvTest {

	@Mock
	private GoodsReceiveNoteBatchService service;
	...
	private GoodsReceiveNoteBatchController controller;

	private CsvUtils<GoodsReceiveNoteBatchEntity, GoodsReceiveNoteBatchRepository, GoodsReceiveNoteBatchEntityAudit, GoodsReceiveNoteBatchService> csvUtils;

	private final GoodsReceiveNoteBatchFactory factory = new GoodsReceiveNoteBatchFactory();
	...
	public void setup() {
		...
		this.controller = new GoodsReceiveNoteBatchController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, GoodsReceiveNoteBatchEntity.class);
	}

	@Test
	void whenExportingGoodsReceiveNoteBatchEntities_thenHeadersAreValid() throws Exception {
		List<GoodsReceiveNoteBatchEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(GoodsReceiveNoteBatchEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingGoodsReceiveNoteBatchEntities_thenEntitiesAreValid() throws Exception {
		List<GoodsReceiveNoteBatchEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(GoodsReceiveNoteBatchEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		GoodsReceiveNoteBatchEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(GoodsReceiveNoteBatchEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(GoodsReceiveNoteBatchEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(GoodsReceiveNoteBatchEntity.class));
		when(service.validateEntityRelations(any(GoodsReceiveNoteBatchEntity.class))).thenReturn(new HashSet<>());
