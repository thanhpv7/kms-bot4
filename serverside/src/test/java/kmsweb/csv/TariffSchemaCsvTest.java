...
import kmsweb.controllers.TariffSchemaController;
...
@Tag("CSV")
public class TariffSchemaCsvTest {

	@Mock
	private TariffSchemaService service;
	...
	private TariffSchemaController controller;

	private CsvUtils<TariffSchemaEntity, TariffSchemaRepository, TariffSchemaEntityAudit, TariffSchemaService> csvUtils;

	private final TariffSchemaFactory factory = new TariffSchemaFactory();
	...
	public void setup() {
		...
		this.controller = new TariffSchemaController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TariffSchemaEntity.class);
	}

	@Test
	void whenExportingTariffSchemaEntities_thenHeadersAreValid() throws Exception {
		List<TariffSchemaEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TariffSchemaEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTariffSchemaEntities_thenEntitiesAreValid() throws Exception {
		List<TariffSchemaEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSchemaCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSchemeName())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TariffSchemaEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TariffSchemaEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TariffSchemaEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TariffSchemaEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TariffSchemaEntity.class));
		when(service.validateEntityRelations(any(TariffSchemaEntity.class))).thenReturn(new HashSet<>());
