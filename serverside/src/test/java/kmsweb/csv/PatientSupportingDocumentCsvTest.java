...
import kmsweb.controllers.PatientSupportingDocumentController;
...
@Tag("CSV")
public class PatientSupportingDocumentCsvTest {

	@Mock
	private PatientSupportingDocumentService service;
	...
	private PatientSupportingDocumentController controller;

	private CsvUtils<PatientSupportingDocumentEntity, PatientSupportingDocumentRepository, PatientSupportingDocumentEntityAudit, PatientSupportingDocumentService> csvUtils;

	private final PatientSupportingDocumentFactory factory = new PatientSupportingDocumentFactory();
	...
	public void setup() {
		...
		this.controller = new PatientSupportingDocumentController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientSupportingDocumentEntity.class);
	}

	@Test
	void whenExportingPatientSupportingDocumentEntities_thenHeadersAreValid() throws Exception {
		List<PatientSupportingDocumentEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientSupportingDocumentEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientSupportingDocumentEntities_thenEntitiesAreValid() throws Exception {
		List<PatientSupportingDocumentEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFileName())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientSupportingDocumentEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientSupportingDocumentEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientSupportingDocumentEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientSupportingDocumentEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientSupportingDocumentEntity.class));
		when(service.validateEntityRelations(any(PatientSupportingDocumentEntity.class))).thenReturn(new HashSet<>());
