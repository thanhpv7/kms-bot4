...
import kmsweb.controllers.ReferenceDataController;
...
@Tag("CSV")
public class ReferenceDataCsvTest {
	...
	private CsvUtils<ReferenceDataEntity, ReferenceDataRepository, ReferenceDataEntityAudit, ReferenceDataService> csvUtils;

	private final ReferenceDataFactory factory = new ReferenceDataFactory();
	...
	public void setup() {
		...
		this.controller = new ReferenceDataController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ReferenceDataEntity.class);
	}

	@Test
	void whenExportingReferenceDataEntities_thenHeadersAreValid() throws Exception {
		List<ReferenceDataEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ReferenceDataEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingReferenceDataEntities_thenEntitiesAreValid() throws Exception {
		List<ReferenceDataEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferenceType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDescription())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAssociatedValues())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSequence()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFilter())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ReferenceDataEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ReferenceDataEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ReferenceDataEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ReferenceDataEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ReferenceDataEntity.class));
		when(service.validateEntityRelations(any(ReferenceDataEntity.class))).thenReturn(new HashSet<>());
