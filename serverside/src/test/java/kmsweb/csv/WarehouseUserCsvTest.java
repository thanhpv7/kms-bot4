...
import kmsweb.controllers.WarehouseUserController;
...
@Tag("CSV")
public class WarehouseUserCsvTest {
	...
	private WarehouseUserController controller;

	private CsvUtils<WarehouseUserEntity, WarehouseUserRepository, WarehouseUserEntityAudit, WarehouseUserService> csvUtils;

	private final WarehouseUserFactory factory = new WarehouseUserFactory();
	...
	public void setup() {
		...
		this.controller = new WarehouseUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, WarehouseUserEntity.class);
	}

	@Test
	void whenExportingWarehouseUserEntities_thenHeadersAreValid() throws Exception {
		List<WarehouseUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingWarehouseUserEntities_thenEntitiesAreValid() throws Exception {
		List<WarehouseUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(WarehouseUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		WarehouseUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(WarehouseUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(WarehouseUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(WarehouseUserEntity.class));
		when(service.validateEntityRelations(any(WarehouseUserEntity.class))).thenReturn(new HashSet<>());
