...
import kmsweb.controllers.PcareDiagnosaController;
...
@Tag("CSV")
public class PcareDiagnosaCsvTest {

	@Mock
	private PcareDiagnosaService service;
	...
	private PcareDiagnosaController controller;

	private CsvUtils<PcareDiagnosaEntity, PcareDiagnosaRepository, PcareDiagnosaEntityAudit, PcareDiagnosaService> csvUtils;

	private final PcareDiagnosaFactory factory = new PcareDiagnosaFactory();
	...
	public void setup() {
		...
		this.controller = new PcareDiagnosaController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareDiagnosaEntity.class);
	}

	@Test
	void whenExportingPcareDiagnosaEntities_thenHeadersAreValid() throws Exception {
		List<PcareDiagnosaEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareDiagnosaEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareDiagnosaEntities_thenEntitiesAreValid() throws Exception {
		List<PcareDiagnosaEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodeDiagnosa())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaDiagnosa())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNonSpesialis()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareDiagnosaEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareDiagnosaEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareDiagnosaEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareDiagnosaEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareDiagnosaEntity.class));
		when(service.validateEntityRelations(any(PcareDiagnosaEntity.class))).thenReturn(new HashSet<>());
