...
import kmsweb.controllers.BpjsAccidentMasterController;
...
@Tag("CSV")
public class BpjsAccidentMasterCsvTest {

	@Mock
	private BpjsAccidentMasterService service;
	...
	private BpjsAccidentMasterController controller;

	private CsvUtils<BpjsAccidentMasterEntity, BpjsAccidentMasterRepository, BpjsAccidentMasterEntityAudit, BpjsAccidentMasterService> csvUtils;

	private final BpjsAccidentMasterFactory factory = new BpjsAccidentMasterFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsAccidentMasterController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsAccidentMasterEntity.class);
	}

	@Test
	void whenExportingBpjsAccidentMasterEntities_thenHeadersAreValid() throws Exception {
		List<BpjsAccidentMasterEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsAccidentMasterEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsAccidentMasterEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsAccidentMasterEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNosep())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTglkejadian())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPpkpelsep())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKdprop())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKdkab())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKdkec())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKetkejadian())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNosepsuplesi())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsAccidentMasterEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsAccidentMasterEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsAccidentMasterEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsAccidentMasterEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsAccidentMasterEntity.class));
		when(service.validateEntityRelations(any(BpjsAccidentMasterEntity.class))).thenReturn(new HashSet<>());
