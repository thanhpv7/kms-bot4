...
import kmsweb.controllers.NhisInvestigationController;
...
@Tag("CSV")
public class NhisInvestigationCsvTest {

	@Mock
	private NhisInvestigationService service;
	...
	private NhisInvestigationController controller;

	private CsvUtils<NhisInvestigationEntity, NhisInvestigationRepository, NhisInvestigationEntityAudit, NhisInvestigationService> csvUtils;

	private final NhisInvestigationFactory factory = new NhisInvestigationFactory();
	...
	public void setup() {
		...
		this.controller = new NhisInvestigationController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, NhisInvestigationEntity.class);
	}

	@Test
	void whenExportingNhisInvestigationEntities_thenHeadersAreValid() throws Exception {
		List<NhisInvestigationEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(NhisInvestigationEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingNhisInvestigationEntities_thenEntitiesAreValid() throws Exception {
		List<NhisInvestigationEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getGdrgCode())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(NhisInvestigationEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		NhisInvestigationEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(NhisInvestigationEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(NhisInvestigationEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(NhisInvestigationEntity.class));
		when(service.validateEntityRelations(any(NhisInvestigationEntity.class))).thenReturn(new HashSet<>());
