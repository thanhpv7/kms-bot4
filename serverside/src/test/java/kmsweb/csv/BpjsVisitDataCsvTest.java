...
import kmsweb.controllers.BpjsVisitDataController;
...
@Tag("CSV")
public class BpjsVisitDataCsvTest {

	@Mock
	private BpjsVisitDataService service;
	...
	private BpjsVisitDataController controller;

	private CsvUtils<BpjsVisitDataEntity, BpjsVisitDataRepository, BpjsVisitDataEntityAudit, BpjsVisitDataService> csvUtils;

	private final BpjsVisitDataFactory factory = new BpjsVisitDataFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsVisitDataController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsVisitDataEntity.class);
	}

	@Test
	void whenExportingBpjsVisitDataEntities_thenHeadersAreValid() throws Exception {
		List<BpjsVisitDataEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsVisitDataEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsVisitDataEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsVisitDataEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSepDismissalDate())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsVisitDataEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsVisitDataEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsVisitDataEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsVisitDataEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsVisitDataEntity.class));
		when(service.validateEntityRelations(any(BpjsVisitDataEntity.class))).thenReturn(new HashSet<>());
