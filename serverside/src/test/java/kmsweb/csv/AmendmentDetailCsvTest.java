...
import kmsweb.controllers.AmendmentDetailController;
...
@Tag("CSV")
public class AmendmentDetailCsvTest {

	@Mock
	private AmendmentDetailService service;
	...
	private AmendmentDetailController controller;

	private CsvUtils<AmendmentDetailEntity, AmendmentDetailRepository, AmendmentDetailEntityAudit, AmendmentDetailService> csvUtils;

	private final AmendmentDetailFactory factory = new AmendmentDetailFactory();
	...
	public void setup() {
		...
		this.controller = new AmendmentDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, AmendmentDetailEntity.class);
	}

	@Test
	void whenExportingAmendmentDetailEntities_thenHeadersAreValid() throws Exception {
		List<AmendmentDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(AmendmentDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingAmendmentDetailEntities_thenEntitiesAreValid() throws Exception {
		List<AmendmentDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReason())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(AmendmentDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		AmendmentDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(AmendmentDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(AmendmentDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(AmendmentDetailEntity.class));
		when(service.validateEntityRelations(any(AmendmentDetailEntity.class))).thenReturn(new HashSet<>());
