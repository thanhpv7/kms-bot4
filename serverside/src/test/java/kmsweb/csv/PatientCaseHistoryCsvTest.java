...
import kmsweb.controllers.PatientCaseHistoryController;
...
@Tag("CSV")
public class PatientCaseHistoryCsvTest {

	@Mock
	private PatientCaseHistoryService service;
	...
	private PatientCaseHistoryController controller;

	private CsvUtils<PatientCaseHistoryEntity, PatientCaseHistoryRepository, PatientCaseHistoryEntityAudit, PatientCaseHistoryService> csvUtils;

	private final PatientCaseHistoryFactory factory = new PatientCaseHistoryFactory();
	...
	public void setup() {
		...
		this.controller = new PatientCaseHistoryController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientCaseHistoryEntity.class);
	}

	@Test
	void whenExportingPatientCaseHistoryEntities_thenHeadersAreValid() throws Exception {
		List<PatientCaseHistoryEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientCaseHistoryEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientCaseHistoryEntities_thenEntitiesAreValid() throws Exception {
		List<PatientCaseHistoryEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAllergy())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDisabilityNotes())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHistoryOfDisease())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCaseHistoryDateTime()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getPatientHabits())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNutritionHIstory())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getFamilyHistory())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientCaseHistoryEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientCaseHistoryEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientCaseHistoryEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientCaseHistoryEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientCaseHistoryEntity.class));
		when(service.validateEntityRelations(any(PatientCaseHistoryEntity.class))).thenReturn(new HashSet<>());
