...
import kmsweb.controllers.PatientVisitPatientPaymentOthersController;
...
@Tag("CSV")
public class PatientVisitPatientPaymentOthersCsvTest {

	@Mock
	private PatientVisitPatientPaymentOthersService service;
	...
	private PatientVisitPatientPaymentOthersController controller;

	private CsvUtils<PatientVisitPatientPaymentOthersEntity, PatientVisitPatientPaymentOthersRepository, PatientVisitPatientPaymentOthersEntityAudit, PatientVisitPatientPaymentOthersService> csvUtils;

	private final PatientVisitPatientPaymentOthersFactory factory = new PatientVisitPatientPaymentOthersFactory();
	...
	public void setup() {
		...
		this.controller = new PatientVisitPatientPaymentOthersController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PatientVisitPatientPaymentOthersEntity.class);
	}

	@Test
	void whenExportingPatientVisitPatientPaymentOthersEntities_thenHeadersAreValid() throws Exception {
		List<PatientVisitPatientPaymentOthersEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitPatientPaymentOthersEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPatientVisitPatientPaymentOthersEntities_thenEntitiesAreValid() throws Exception {
		List<PatientVisitPatientPaymentOthersEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PatientVisitPatientPaymentOthersEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PatientVisitPatientPaymentOthersEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PatientVisitPatientPaymentOthersEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PatientVisitPatientPaymentOthersEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PatientVisitPatientPaymentOthersEntity.class));
		when(service.validateEntityRelations(any(PatientVisitPatientPaymentOthersEntity.class))).thenReturn(new HashSet<>());
