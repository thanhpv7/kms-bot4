...
import kmsweb.controllers.PcareClubProlanisController;
...
@Tag("CSV")
public class PcareClubProlanisCsvTest {

	@Mock
	private PcareClubProlanisService service;
	...
	private PcareClubProlanisController controller;

	private CsvUtils<PcareClubProlanisEntity, PcareClubProlanisRepository, PcareClubProlanisEntityAudit, PcareClubProlanisService> csvUtils;

	private final PcareClubProlanisFactory factory = new PcareClubProlanisFactory();
	...
	public void setup() {
		...
		this.controller = new PcareClubProlanisController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PcareClubProlanisEntity.class);
	}

	@Test
	void whenExportingPcareClubProlanisEntities_thenHeadersAreValid() throws Exception {
		List<PcareClubProlanisEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PcareClubProlanisEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPcareClubProlanisEntities_thenEntitiesAreValid() throws Exception {
		List<PcareClubProlanisEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKodeProgram())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaProgram())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTanggalMulai())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTanggalAkhir())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAlamatClub())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaClub())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNoHPKetua())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNamaKetua())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PcareClubProlanisEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PcareClubProlanisEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PcareClubProlanisEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PcareClubProlanisEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PcareClubProlanisEntity.class));
		when(service.validateEntityRelations(any(PcareClubProlanisEntity.class))).thenReturn(new HashSet<>());
