...
import kmsweb.controllers.ProcessWorkflowController;
...
@Tag("CSV")
public class ProcessWorkflowCsvTest {
	...
	private ProcessWorkflowController controller;

	private CsvUtils<ProcessWorkflowEntity, ProcessWorkflowRepository, ProcessWorkflowEntityAudit, ProcessWorkflowService> csvUtils;

	private final ProcessWorkflowFactory factory = new ProcessWorkflowFactory();
	...
	public void setup() {
		...
		this.controller = new ProcessWorkflowController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ProcessWorkflowEntity.class);
	}

	@Test
	void whenExportingProcessWorkflowEntities_thenHeadersAreValid() throws Exception {
		List<ProcessWorkflowEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ProcessWorkflowEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingProcessWorkflowEntities_thenEntitiesAreValid() throws Exception {
		List<ProcessWorkflowEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getStepNumber()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getModuleName()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProcessName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getReferenceID())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getProcessDateTime()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ProcessWorkflowEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ProcessWorkflowEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ProcessWorkflowEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ProcessWorkflowEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ProcessWorkflowEntity.class));
		when(service.validateEntityRelations(any(ProcessWorkflowEntity.class))).thenReturn(new HashSet<>());
