...
import kmsweb.controllers.SupplierProductDetailController;
...
@Tag("CSV")
public class SupplierProductDetailCsvTest {

	@Mock
	private SupplierProductDetailService service;
	...
	private SupplierProductDetailController controller;

	private CsvUtils<SupplierProductDetailEntity, SupplierProductDetailRepository, SupplierProductDetailEntityAudit, SupplierProductDetailService> csvUtils;

	private final SupplierProductDetailFactory factory = new SupplierProductDetailFactory();
	...
	public void setup() {
		...
		this.controller = new SupplierProductDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SupplierProductDetailEntity.class);
	}

	@Test
	void whenExportingSupplierProductDetailEntities_thenHeadersAreValid() throws Exception {
		List<SupplierProductDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SupplierProductDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSupplierProductDetailEntities_thenEntitiesAreValid() throws Exception {
		List<SupplierProductDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUnitCode())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUnitName())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SupplierProductDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SupplierProductDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SupplierProductDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SupplierProductDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SupplierProductDetailEntity.class));
		when(service.validateEntityRelations(any(SupplierProductDetailEntity.class))).thenReturn(new HashSet<>());
