...
import kmsweb.controllers.ClaimUserController;
...
@Tag("CSV")
public class ClaimUserCsvTest {

	@Mock
	private ClaimUserService service;
	...
	private ClaimUserController controller;

	private CsvUtils<ClaimUserEntity, ClaimUserRepository, ClaimUserEntityAudit, ClaimUserService> csvUtils;

	private final ClaimUserFactory factory = new ClaimUserFactory();
	...
	public void setup() {
		...
		this.controller = new ClaimUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ClaimUserEntity.class);
	}

	@Test
	void whenExportingClaimUserEntities_thenHeadersAreValid() throws Exception {
		List<ClaimUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ClaimUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingClaimUserEntities_thenEntitiesAreValid() throws Exception {
		List<ClaimUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ClaimUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ClaimUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ClaimUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ClaimUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ClaimUserEntity.class));
		when(service.validateEntityRelations(any(ClaimUserEntity.class))).thenReturn(new HashSet<>());
