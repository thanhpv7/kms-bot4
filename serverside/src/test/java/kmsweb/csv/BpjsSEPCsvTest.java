...
import kmsweb.controllers.BpjsSEPController;
...
@Tag("CSV")
public class BpjsSEPCsvTest {

	@Mock
	private BpjsSEPService service;
	...
	private BpjsSEPController controller;

	private CsvUtils<BpjsSEPEntity, BpjsSEPRepository, BpjsSEPEntityAudit, BpjsSEPService> csvUtils;

	private final BpjsSEPFactory factory = new BpjsSEPFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsSEPController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsSEPEntity.class);
	}

	@Test
	void whenExportingBpjsSEPEntities_thenHeadersAreValid() throws Exception {
		List<BpjsSEPEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsSEPEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsSEPEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsSEPEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsSEPEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsSEPEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsSEPEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsSEPEntity.class));
		when(service.validateEntityRelations(any(BpjsSEPEntity.class))).thenReturn(new HashSet<>());
