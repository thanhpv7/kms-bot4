...
import kmsweb.controllers.SupplierContactDetailController;
...
@Tag("CSV")
public class SupplierContactDetailCsvTest {

	@Mock
	private SupplierContactDetailService service;
	...
	private SupplierContactDetailController controller;

	private CsvUtils<SupplierContactDetailEntity, SupplierContactDetailRepository, SupplierContactDetailEntityAudit, SupplierContactDetailService> csvUtils;

	private final SupplierContactDetailFactory factory = new SupplierContactDetailFactory();
	...
	public void setup() {
		...
		this.controller = new SupplierContactDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SupplierContactDetailEntity.class);
	}

	@Test
	void whenExportingSupplierContactDetailEntities_thenHeadersAreValid() throws Exception {
		List<SupplierContactDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SupplierContactDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSupplierContactDetailEntities_thenEntitiesAreValid() throws Exception {
		List<SupplierContactDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getExt())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SupplierContactDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SupplierContactDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SupplierContactDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SupplierContactDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SupplierContactDetailEntity.class));
		when(service.validateEntityRelations(any(SupplierContactDetailEntity.class))).thenReturn(new HashSet<>());
