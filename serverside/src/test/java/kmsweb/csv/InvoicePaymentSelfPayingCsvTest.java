...
import kmsweb.controllers.InvoicePaymentSelfPayingController;
...
@Tag("CSV")
public class InvoicePaymentSelfPayingCsvTest {

	@Mock
	private InvoicePaymentSelfPayingService service;
	...
	private InvoicePaymentSelfPayingController controller;

	private CsvUtils<InvoicePaymentSelfPayingEntity, InvoicePaymentSelfPayingRepository, InvoicePaymentSelfPayingEntityAudit, InvoicePaymentSelfPayingService> csvUtils;

	private final InvoicePaymentSelfPayingFactory factory = new InvoicePaymentSelfPayingFactory();
	...
	public void setup() {
		...
		this.controller = new InvoicePaymentSelfPayingController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InvoicePaymentSelfPayingEntity.class);
	}

	@Test
	void whenExportingInvoicePaymentSelfPayingEntities_thenHeadersAreValid() throws Exception {
		List<InvoicePaymentSelfPayingEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InvoicePaymentSelfPayingEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInvoicePaymentSelfPayingEntities_thenEntitiesAreValid() throws Exception {
		List<InvoicePaymentSelfPayingEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InvoicePaymentSelfPayingEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InvoicePaymentSelfPayingEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InvoicePaymentSelfPayingEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InvoicePaymentSelfPayingEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InvoicePaymentSelfPayingEntity.class));
		when(service.validateEntityRelations(any(InvoicePaymentSelfPayingEntity.class))).thenReturn(new HashSet<>());
