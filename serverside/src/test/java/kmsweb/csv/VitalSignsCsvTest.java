...
import kmsweb.controllers.VitalSignsController;
...
@Tag("CSV")
public class VitalSignsCsvTest {
	...
	private VitalSignsController controller;

	private CsvUtils<VitalSignsEntity, VitalSignsRepository, VitalSignsEntityAudit, VitalSignsService> csvUtils;

	private final VitalSignsFactory factory = new VitalSignsFactory();
	...
	public void setup() {
		...
		this.controller = new VitalSignsController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, VitalSignsEntity.class);
	}

	@Test
	void whenExportingVitalSignsEntities_thenHeadersAreValid() throws Exception {
		List<VitalSignsEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(VitalSignsEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingVitalSignsEntities_thenEntitiesAreValid() throws Exception {
		List<VitalSignsEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTime())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHr())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRr())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTemprature())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getO2Saturation())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRemarks())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(VitalSignsEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		VitalSignsEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(VitalSignsEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(VitalSignsEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(VitalSignsEntity.class));
		when(service.validateEntityRelations(any(VitalSignsEntity.class))).thenReturn(new HashSet<>());
