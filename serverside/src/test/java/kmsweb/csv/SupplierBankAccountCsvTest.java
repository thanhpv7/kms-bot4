...
import kmsweb.controllers.SupplierBankAccountController;
...
@Tag("CSV")
public class SupplierBankAccountCsvTest {

	@Mock
	private SupplierBankAccountService service;
	...
	private SupplierBankAccountController controller;

	private CsvUtils<SupplierBankAccountEntity, SupplierBankAccountRepository, SupplierBankAccountEntityAudit, SupplierBankAccountService> csvUtils;

	private final SupplierBankAccountFactory factory = new SupplierBankAccountFactory();
	...
	public void setup() {
		...
		this.controller = new SupplierBankAccountController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, SupplierBankAccountEntity.class);
	}

	@Test
	void whenExportingSupplierBankAccountEntities_thenHeadersAreValid() throws Exception {
		List<SupplierBankAccountEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(SupplierBankAccountEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingSupplierBankAccountEntities_thenEntitiesAreValid() throws Exception {
		List<SupplierBankAccountEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBranch())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(SupplierBankAccountEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		SupplierBankAccountEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(SupplierBankAccountEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(SupplierBankAccountEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(SupplierBankAccountEntity.class));
		when(service.validateEntityRelations(any(SupplierBankAccountEntity.class))).thenReturn(new HashSet<>());
