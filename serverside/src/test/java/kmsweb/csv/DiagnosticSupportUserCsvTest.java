...
import kmsweb.controllers.DiagnosticSupportUserController;
...
@Tag("CSV")
public class DiagnosticSupportUserCsvTest {

	@Mock
	private DiagnosticSupportUserService service;
	...
	private DiagnosticSupportUserController controller;

	private CsvUtils<DiagnosticSupportUserEntity, DiagnosticSupportUserRepository, DiagnosticSupportUserEntityAudit, DiagnosticSupportUserService> csvUtils;

	private final DiagnosticSupportUserFactory factory = new DiagnosticSupportUserFactory();
	...
	public void setup() {
		...
		this.controller = new DiagnosticSupportUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DiagnosticSupportUserEntity.class);
	}

	@Test
	void whenExportingDiagnosticSupportUserEntities_thenHeadersAreValid() throws Exception {
		List<DiagnosticSupportUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticSupportUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDiagnosticSupportUserEntities_thenEntitiesAreValid() throws Exception {
		List<DiagnosticSupportUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticSupportUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DiagnosticSupportUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DiagnosticSupportUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DiagnosticSupportUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DiagnosticSupportUserEntity.class));
		when(service.validateEntityRelations(any(DiagnosticSupportUserEntity.class))).thenReturn(new HashSet<>());
