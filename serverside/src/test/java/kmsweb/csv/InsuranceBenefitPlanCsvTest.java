...
import kmsweb.controllers.InsuranceBenefitPlanController;
...
@Tag("CSV")
public class InsuranceBenefitPlanCsvTest {

	@Mock
	private InsuranceBenefitPlanService service;
	...
	private InsuranceBenefitPlanController controller;

	private CsvUtils<InsuranceBenefitPlanEntity, InsuranceBenefitPlanRepository, InsuranceBenefitPlanEntityAudit, InsuranceBenefitPlanService> csvUtils;

	private final InsuranceBenefitPlanFactory factory = new InsuranceBenefitPlanFactory();
	...
	public void setup() {
		...
		this.controller = new InsuranceBenefitPlanController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, InsuranceBenefitPlanEntity.class);
	}

	@Test
	void whenExportingInsuranceBenefitPlanEntities_thenHeadersAreValid() throws Exception {
		List<InsuranceBenefitPlanEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(InsuranceBenefitPlanEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingInsuranceBenefitPlanEntities_thenEntitiesAreValid() throws Exception {
		List<InsuranceBenefitPlanEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBenefitPlanNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getBenefitPlanName())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMaximumCoveragePerTreatment()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getMaximumCoveragePerYear()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCreatedDate()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUpdatedDate()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(InsuranceBenefitPlanEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		InsuranceBenefitPlanEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(InsuranceBenefitPlanEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(InsuranceBenefitPlanEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(InsuranceBenefitPlanEntity.class));
		when(service.validateEntityRelations(any(InsuranceBenefitPlanEntity.class))).thenReturn(new HashSet<>());
