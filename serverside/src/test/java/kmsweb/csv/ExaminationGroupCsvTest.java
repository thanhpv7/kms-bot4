...
import kmsweb.controllers.ExaminationGroupController;
...
@Tag("CSV")
public class ExaminationGroupCsvTest {

	@Mock
	private ExaminationGroupService service;
	...
	private ExaminationGroupController controller;

	private CsvUtils<ExaminationGroupEntity, ExaminationGroupRepository, ExaminationGroupEntityAudit, ExaminationGroupService> csvUtils;

	private final ExaminationGroupFactory factory = new ExaminationGroupFactory();
	...
	public void setup() {
		...
		this.controller = new ExaminationGroupController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, ExaminationGroupEntity.class);
	}

	@Test
	void whenExportingExaminationGroupEntities_thenHeadersAreValid() throws Exception {
		List<ExaminationGroupEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(ExaminationGroupEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingExaminationGroupEntities_thenEntitiesAreValid() throws Exception {
		List<ExaminationGroupEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getDiagnosticGroupCode())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(ExaminationGroupEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		ExaminationGroupEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(ExaminationGroupEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(ExaminationGroupEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(ExaminationGroupEntity.class));
		when(service.validateEntityRelations(any(ExaminationGroupEntity.class))).thenReturn(new HashSet<>());
