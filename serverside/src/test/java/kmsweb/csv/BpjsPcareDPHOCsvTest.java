...
import kmsweb.controllers.BpjsPcareDPHOController;
...
@Tag("CSV")
public class BpjsPcareDPHOCsvTest {

	@Mock
	private BpjsPcareDPHOService service;
	...
	private BpjsPcareDPHOController controller;

	private CsvUtils<BpjsPcareDPHOEntity, BpjsPcareDPHORepository, BpjsPcareDPHOEntityAudit, BpjsPcareDPHOService> csvUtils;

	private final BpjsPcareDPHOFactory factory = new BpjsPcareDPHOFactory();
	...
	public void setup() {
		...
		this.controller = new BpjsPcareDPHOController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, BpjsPcareDPHOEntity.class);
	}

	@Test
	void whenExportingBpjsPcareDPHOEntities_thenHeadersAreValid() throws Exception {
		List<BpjsPcareDPHOEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareDPHOEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingBpjsPcareDPHOEntities_thenEntitiesAreValid() throws Exception {
		List<BpjsPcareDPHOEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getKdobat())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getNmobat())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSedia())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(BpjsPcareDPHOEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		BpjsPcareDPHOEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(BpjsPcareDPHOEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(BpjsPcareDPHOEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(BpjsPcareDPHOEntity.class));
		when(service.validateEntityRelations(any(BpjsPcareDPHOEntity.class))).thenReturn(new HashSet<>());
