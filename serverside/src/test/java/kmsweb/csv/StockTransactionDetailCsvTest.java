...
import kmsweb.controllers.StockTransactionDetailController;
...
@Tag("CSV")
public class StockTransactionDetailCsvTest {

	@Mock
	private StockTransactionDetailService service;
	...
	private StockTransactionDetailController controller;

	private CsvUtils<StockTransactionDetailEntity, StockTransactionDetailRepository, StockTransactionDetailEntityAudit, StockTransactionDetailService> csvUtils;

	private final StockTransactionDetailFactory factory = new StockTransactionDetailFactory();
	...
	public void setup() {
		...
		this.controller = new StockTransactionDetailController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, StockTransactionDetailEntity.class);
	}

	@Test
	void whenExportingStockTransactionDetailEntities_thenHeadersAreValid() throws Exception {
		List<StockTransactionDetailEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(StockTransactionDetailEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingStockTransactionDetailEntities_thenEntitiesAreValid() throws Exception {
		List<StockTransactionDetailEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTransactionNumber())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getTransactionNotes())));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getQtyUomTransaction()).toString()));
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUomTransaction())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSourceOrderNumber())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getQtyTransitUoi()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getQtyTransitUomTransaction()).toString()));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(StockTransactionDetailEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		StockTransactionDetailEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(StockTransactionDetailEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(StockTransactionDetailEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(StockTransactionDetailEntity.class));
		when(service.validateEntityRelations(any(StockTransactionDetailEntity.class))).thenReturn(new HashSet<>());
