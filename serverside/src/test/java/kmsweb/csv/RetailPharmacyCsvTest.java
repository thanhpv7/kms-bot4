...
import kmsweb.controllers.RetailPharmacyController;
...
@Tag("CSV")
public class RetailPharmacyCsvTest {

	@Mock
	private RetailPharmacyService service;
	...
	private RetailPharmacyController controller;

	private CsvUtils<RetailPharmacyEntity, RetailPharmacyRepository, RetailPharmacyEntityAudit, RetailPharmacyService> csvUtils;

	private final RetailPharmacyFactory factory = new RetailPharmacyFactory();
	...
	public void setup() {
		...
		this.controller = new RetailPharmacyController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, RetailPharmacyEntity.class);
	}

	@Test
	void whenExportingRetailPharmacyEntities_thenHeadersAreValid() throws Exception {
		List<RetailPharmacyEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(RetailPharmacyEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingRetailPharmacyEntities_thenEntitiesAreValid() throws Exception {
		List<RetailPharmacyEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getCustomerType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRetailNumber())));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(RetailPharmacyEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		RetailPharmacyEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(RetailPharmacyEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(RetailPharmacyEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(RetailPharmacyEntity.class));
		when(service.validateEntityRelations(any(RetailPharmacyEntity.class))).thenReturn(new HashSet<>());
