...
import kmsweb.controllers.DiagnosticExaminationResultsController;
...
@Tag("CSV")
public class DiagnosticExaminationResultsCsvTest {

	@Mock
	private DiagnosticExaminationResultsService service;
	...
	private DiagnosticExaminationResultsController controller;

	private CsvUtils<DiagnosticExaminationResultsEntity, DiagnosticExaminationResultsRepository, DiagnosticExaminationResultsEntityAudit, DiagnosticExaminationResultsService> csvUtils;

	private final DiagnosticExaminationResultsFactory factory = new DiagnosticExaminationResultsFactory();
	...
	public void setup() {
		...
		this.controller = new DiagnosticExaminationResultsController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, DiagnosticExaminationResultsEntity.class);
	}

	@Test
	void whenExportingDiagnosticExaminationResultsEntities_thenHeadersAreValid() throws Exception {
		List<DiagnosticExaminationResultsEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticExaminationResultsEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingDiagnosticExaminationResultsEntities_thenEntitiesAreValid() throws Exception {
		List<DiagnosticExaminationResultsEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getResult())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getOutsideRange()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getLow()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getHigh()).toString()));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getUnit())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleNo())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getSampleType())));

		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getRepeated()).toString()));
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(DiagnosticExaminationResultsEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		DiagnosticExaminationResultsEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(DiagnosticExaminationResultsEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(DiagnosticExaminationResultsEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(DiagnosticExaminationResultsEntity.class));
		when(service.validateEntityRelations(any(DiagnosticExaminationResultsEntity.class))).thenReturn(new HashSet<>());
