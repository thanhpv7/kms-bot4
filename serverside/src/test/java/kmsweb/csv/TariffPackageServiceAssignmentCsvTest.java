...
import kmsweb.controllers.TariffPackageServiceAssignmentController;
...
@Tag("CSV")
public class TariffPackageServiceAssignmentCsvTest {

	@Mock
	private TariffPackageServiceAssignmentService service;
	...
	private TariffPackageServiceAssignmentController controller;

	private CsvUtils<TariffPackageServiceAssignmentEntity, TariffPackageServiceAssignmentRepository, TariffPackageServiceAssignmentEntityAudit, TariffPackageServiceAssignmentService> csvUtils;

	private final TariffPackageServiceAssignmentFactory factory = new TariffPackageServiceAssignmentFactory();
	...
	public void setup() {
		...
		this.controller = new TariffPackageServiceAssignmentController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, TariffPackageServiceAssignmentEntity.class);
	}

	@Test
	void whenExportingTariffPackageServiceAssignmentEntities_thenHeadersAreValid() throws Exception {
		List<TariffPackageServiceAssignmentEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(TariffPackageServiceAssignmentEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingTariffPackageServiceAssignmentEntities_thenEntitiesAreValid() throws Exception {
		List<TariffPackageServiceAssignmentEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		Assertions.assertTrue(exportedEntityCsv.contains(Objects.requireNonNull(entitiesToExport.get(0).getAssignCode())));
	}
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(TariffPackageServiceAssignmentEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		TariffPackageServiceAssignmentEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(TariffPackageServiceAssignmentEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(TariffPackageServiceAssignmentEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(TariffPackageServiceAssignmentEntity.class));
		when(service.validateEntityRelations(any(TariffPackageServiceAssignmentEntity.class))).thenReturn(new HashSet<>());
