...
import kmsweb.controllers.PurchasingUserController;
...
@Tag("CSV")
public class PurchasingUserCsvTest {

	@Mock
	private PurchasingUserService service;
	...
	private PurchasingUserController controller;

	private CsvUtils<PurchasingUserEntity, PurchasingUserRepository, PurchasingUserEntityAudit, PurchasingUserService> csvUtils;

	private final PurchasingUserFactory factory = new PurchasingUserFactory();
	...
	public void setup() {
		...
		this.controller = new PurchasingUserController(authenticationService, service);
		this.csvUtils = new CsvUtils<>(service, PurchasingUserEntity.class);
	}

	@Test
	void whenExportingPurchasingUserEntities_thenHeadersAreValid() throws Exception {
		List<PurchasingUserEntity> entitiesToExport = new ArrayList<>();
		...
		List<String> expectedHeaders = Arrays.asList(PurchasingUserEntity.getExampleCsvHeader().concat(",CREATED,CREATED_BY,MODIFIED,MODIFIED_BY").split(","));
	...
	@Test
	void whenExportingPurchasingUserEntities_thenEntitiesAreValid() throws Exception {
		List<PurchasingUserEntity> entitiesToExport = new ArrayList<>();
		MockHttpServletResponse response = new MockHttpServletResponse();
	...
	void whenDownloadingImportExample_thenImportIsValid() throws Exception {
		...
		List<String> expectedHeaders = Arrays.asList(PurchasingUserEntity.getExampleCsvHeader().split(","));
	...
	void whenEntityValidationFailsInImport_thenImportIsInvalid() throws Exception {
		PurchasingUserEntity generatedEntity = factory.getObject();
		...
		doThrow(new ConstraintViolationException(constraintViolations)).when(service).validateEntity(any(PurchasingUserEntity.class));
	...
	void whenEntityRelationValidationFailsInImport_thenImportIsInvalid() throws Exception {
		...
		when(service.validateEntityRelations(any(PurchasingUserEntity.class))).thenReturn(relationValidationErrors);
	...
	void whenValidatingImportCreateEntity_thenImportIsValid() throws Exception {
		...
		doNothing().when(service).validateEntity(any(PurchasingUserEntity.class));
		when(service.validateEntityRelations(any(PurchasingUserEntity.class))).thenReturn(new HashSet<>());
