...
public class BpjsServiceMappingFactory implements FactoryBean<BpjsServiceMappingEntity> {
	 ...
	 */
	@Override
	public BpjsServiceMappingEntity getObject() throws Exception {
	 ...
	 * @see #getObject(boolean, boolean)
	 */
	public BpjsServiceMappingEntity getObjectWithReferences(boolean includeIds) throws Exception {
	 ...
	 * @throws Exception
	 */
	public BpjsServiceMappingEntity getObject(boolean includeReferences, boolean includeId) throws Exception {
		...
		String randomStringForRefService = mockNeat
				.strings()
				.size(200)
				.get();
		...
		newEntity.setCategorizedAsSurgery(mockNeat.bools().get());
		...
		if (includeReferences) {
			...
			// % protected region % [Override the reference inclusion for outgoingOneToMany poliKontrol here] off begin
			// Outgoing one to many reference
			var polikontrolFactory = new BpjsHospitalizationPlanFactory();
			newEntity.setPolikontrol(Collections.singletonList(polikontrolFactory.getObject(false, includeId)), false);
			// % protected region % [Override the reference inclusion for outgoingOneToMany poliKontrol here] end

		}

		// % protected region % [Apply any additional logic for bpjsServiceMapping with ref here] off begin
		// % protected region % [Apply any additional logic for bpjsServiceMapping with ref here] end

		return newEntity;
	}

	@Override
	public Class<BpjsServiceMappingEntity> getObjectType() {
