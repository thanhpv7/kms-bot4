...
import kmsweb.entities.InvoiceItemComponentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Invoice Item Component
 */
// % protected region % [Update the API route or description for the Invoice Item ComponentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InvoiceItemComponentController"))
@RestController
@Slf4j
@RequestMapping("/api/invoiceitemcomponent")
// % protected region % [Update the API route or description for the Invoice Item ComponentController here] end
public class InvoiceItemComponentController {

	private final InvoiceItemComponentService invoiceItemComponentService;
	private final CsvUtils<InvoiceItemComponentEntity, InvoiceItemComponentRepository, InvoiceItemComponentEntityAudit,  InvoiceItemComponentService> csvUtils;
	...
	@Autowired
	public InvoiceItemComponentController(
		...
		this.csvUtils = new CsvUtils<>(invoiceItemComponentService, InvoiceItemComponentEntity.class);
	...
	public ResponseEntity<List<InvoiceItemComponentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InvoiceItemComponentEntity> invoiceItemComponents = invoiceItemComponentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(invoiceItemComponents, HttpStatus.OK);
	}

	/**
	 * Return the Invoice Item Component that has the same id as the given id.
	 *
	 * @param id      The id of the InvoiceItemComponentEntity to be returned
	 * @return the Invoice Item Component that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InvoiceItemComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InvoiceItemComponent entity not found"),
	})
	@PreAuthorize("hasPermission('InvoiceItemComponentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InvoiceItemComponentEntity> getById(
			@Parameter(description = "The UUID of the InvoiceItemComponent to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InvoiceItemComponentEntity> invoiceItemComponentEntity = invoiceItemComponentService.findById(id);
	...
	/**
	 * Create/Update the given Invoice Item Component. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceItemComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInvoiceItemComponent' endpoint here] off begin
	@Operation(
			description = "Create a InvoiceItemComponent if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InvoiceItemComponent."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InvoiceItemComponent, duplicate record found")
	})
	@PreAuthorize("hasPermission('InvoiceItemComponentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInvoiceItemComponent' endpoint here] end
	public ResponseEntity<?> saveInvoiceItemComponent(
			@Parameter(description = "The InvoiceItemComponentEntity to create or update.")
			@RequestBody InvoiceItemComponentEntityDto invoiceItemComponentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InvoiceItemComponentEntity invoiceItemComponentEntity = invoiceItemComponentService.create(new InvoiceItemComponentEntity(invoiceItemComponentEntityDto));
			return new ResponseEntity<>(invoiceItemComponentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Invoice Item Component. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceItemComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInvoiceItemComponent' endpoint here] off begin
	@Operation(
			description = "Update a InvoiceItemComponent entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InvoiceItemComponent."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InvoiceItemComponent, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InvoiceItemComponentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInvoiceItemComponent' endpoint here] end
	public ResponseEntity<?> updateInvoiceItemComponent(
			@Parameter(description = "The InvoiceItemComponentEntity to create or update.")
			@RequestBody InvoiceItemComponentEntityDto invoiceItemComponentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InvoiceItemComponentEntity> checkInvoiceItemComponentExists = invoiceItemComponentService.findById(invoiceItemComponentEntityDto.getId());
		InvoiceItemComponentEntity invoiceItemComponentEntity = new InvoiceItemComponentEntity(invoiceItemComponentEntityDto);

		if (checkInvoiceItemComponentExists.isPresent()) {
			try {
				invoiceItemComponentEntity = invoiceItemComponentService.update(invoiceItemComponentEntity);
				return new ResponseEntity<>(invoiceItemComponentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(invoiceItemComponentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Invoice Item Component that has the same id as the given id.
	 *
	 * @param id The id of the InvoiceItemComponentEntity to be deleted
	...
	@Operation(
			description = "Delete a single InvoiceItemComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InvoiceItemComponent to delete.")
		...
		invoiceItemComponentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItemComponent' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Invoice Item Component entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItemComponent' endpoint here] end
	public void exportInvoiceItemComponent(
			@Parameter(description = "The ids of the InvoiceItemComponentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InvoiceItemComponentEntity> entitiesToExport = invoiceItemComponentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InvoiceItemComponent_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InvoiceItemComponentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItemComponentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItemComponentExcludingIds' endpoint here] end
	public void exportInvoiceItemComponentExcludingIds(
			@Parameter(description = "The ids of the InvoiceItemComponentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InvoiceItemComponentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = invoiceItemComponentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = invoiceItemComponentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InvoiceItemComponent entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InvoiceItemComponent_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InvoiceItemComponentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Invoice Item Component entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InvoiceItemComponentEntity', 'create')  || hasPermission('InvoiceItemComponentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InvoiceItemComponentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InvoiceItemComponentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InvoiceItemComponentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InvoiceItemComponentEntity> csvToBean = new CsvToBeanBuilder<InvoiceItemComponentEntity>(reader)
				.withType(InvoiceItemComponentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InvoiceItemComponentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InvoiceItemComponentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Invoice Item Component entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Invoice Item Component entity and you do not have permission");
					}
