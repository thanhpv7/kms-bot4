...
import kmsweb.entities.PatientPaymentBPJSEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Payment BPJS
 */
// % protected region % [Update the API route or description for the Patient Payment BPJSController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientPaymentBPJSController"))
@RestController
@Slf4j
@RequestMapping("/api/patientpaymentbpjs")
// % protected region % [Update the API route or description for the Patient Payment BPJSController here] end
public class PatientPaymentBPJSController {

	private final PatientPaymentBPJSService patientPaymentBPJSService;
	private final CsvUtils<PatientPaymentBPJSEntity, PatientPaymentBPJSRepository, PatientPaymentBPJSEntityAudit,  PatientPaymentBPJSService> csvUtils;
	...
	@Autowired
	public PatientPaymentBPJSController(
		...
		this.csvUtils = new CsvUtils<>(patientPaymentBPJSService, PatientPaymentBPJSEntity.class);
	...
	public ResponseEntity<List<PatientPaymentBPJSEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientPaymentBPJSEntity> patientPaymentBPJSs = patientPaymentBPJSService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientPaymentBPJSs, HttpStatus.OK);
	}

	/**
	 * Return the Patient Payment BPJS that has the same id as the given id.
	 *
	 * @param id      The id of the PatientPaymentBPJSEntity to be returned
	 * @return the Patient Payment BPJS that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientPaymentBPJS as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientPaymentBPJS entity not found"),
	})
	@PreAuthorize("hasPermission('PatientPaymentBPJSEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientPaymentBPJSEntity> getById(
			@Parameter(description = "The UUID of the PatientPaymentBPJS to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientPaymentBPJSEntity> patientPaymentBPJSEntity = patientPaymentBPJSService.findById(id);
	...
	/**
	 * Create/Update the given Patient Payment BPJS. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentBPJSEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentBPJS' endpoint here] off begin
	@Operation(
			description = "Create a PatientPaymentBPJS if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientPaymentBPJS."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientPaymentBPJS, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientPaymentBPJSEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentBPJS' endpoint here] end
	public ResponseEntity<?> savePatientPaymentBPJS(
			@Parameter(description = "The PatientPaymentBPJSEntity to create or update.")
			@RequestBody PatientPaymentBPJSEntityDto patientPaymentBPJSEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientPaymentBPJSEntity patientPaymentBPJSEntity = patientPaymentBPJSService.create(new PatientPaymentBPJSEntity(patientPaymentBPJSEntityDto));
			return new ResponseEntity<>(patientPaymentBPJSEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Payment BPJS. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentBPJSEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentBPJS' endpoint here] off begin
	@Operation(
			description = "Update a PatientPaymentBPJS entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientPaymentBPJS."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientPaymentBPJS, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientPaymentBPJSEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentBPJS' endpoint here] end
	public ResponseEntity<?> updatePatientPaymentBPJS(
			@Parameter(description = "The PatientPaymentBPJSEntity to create or update.")
			@RequestBody PatientPaymentBPJSEntityDto patientPaymentBPJSEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientPaymentBPJSEntity> checkPatientPaymentBPJSExists = patientPaymentBPJSService.findById(patientPaymentBPJSEntityDto.getId());
		PatientPaymentBPJSEntity patientPaymentBPJSEntity = new PatientPaymentBPJSEntity(patientPaymentBPJSEntityDto);

		if (checkPatientPaymentBPJSExists.isPresent()) {
			try {
				patientPaymentBPJSEntity = patientPaymentBPJSService.update(patientPaymentBPJSEntity);
				return new ResponseEntity<>(patientPaymentBPJSEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientPaymentBPJSEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Payment BPJS that has the same id as the given id.
	 *
	 * @param id The id of the PatientPaymentBPJSEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientPaymentBPJS as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientPaymentBPJS to delete.")
		...
		patientPaymentBPJSService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentBPJS' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Payment BPJS entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentBPJS' endpoint here] end
	public void exportPatientPaymentBPJS(
			@Parameter(description = "The ids of the PatientPaymentBPJSEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientPaymentBPJSEntity> entitiesToExport = patientPaymentBPJSService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientPaymentBPJS_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientPaymentBPJSEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentBPJSExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentBPJSExcludingIds' endpoint here] end
	public void exportPatientPaymentBPJSExcludingIds(
			@Parameter(description = "The ids of the PatientPaymentBPJSEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientPaymentBPJSEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientPaymentBPJSService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientPaymentBPJSService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientPaymentBPJS entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientPaymentBPJS_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientPaymentBPJSEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Payment BPJS entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientPaymentBPJSEntity', 'create')  || hasPermission('PatientPaymentBPJSEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientPaymentBPJSEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientPaymentBPJSEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientPaymentBPJSEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientPaymentBPJSEntity> csvToBean = new CsvToBeanBuilder<PatientPaymentBPJSEntity>(reader)
				.withType(PatientPaymentBPJSEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientPaymentBPJSEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientPaymentBPJSEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Payment BPJS entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Payment BPJS entity and you do not have permission");
					}
