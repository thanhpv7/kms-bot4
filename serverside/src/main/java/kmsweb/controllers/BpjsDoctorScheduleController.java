...
import kmsweb.entities.BpjsDoctorScheduleEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Doctor Schedule
 */
// % protected region % [Update the API route or description for the BPJS Doctor ScheduleController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsDoctorScheduleController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsdoctorschedule")
// % protected region % [Update the API route or description for the BPJS Doctor ScheduleController here] end
public class BpjsDoctorScheduleController {

	private final BpjsDoctorScheduleService bpjsDoctorScheduleService;
	private final CsvUtils<BpjsDoctorScheduleEntity, BpjsDoctorScheduleRepository, BpjsDoctorScheduleEntityAudit,  BpjsDoctorScheduleService> csvUtils;
	...
	@Autowired
	public BpjsDoctorScheduleController(
		...
		this.csvUtils = new CsvUtils<>(bpjsDoctorScheduleService, BpjsDoctorScheduleEntity.class);
	...
	public ResponseEntity<List<BpjsDoctorScheduleEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsDoctorScheduleEntity> bpjsDoctorSchedules = bpjsDoctorScheduleService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsDoctorSchedules, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Doctor Schedule that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsDoctorScheduleEntity to be returned
	 * @return the BPJS Doctor Schedule that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsDoctorSchedule as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsDoctorSchedule entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsDoctorScheduleEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsDoctorScheduleEntity> getById(
			@Parameter(description = "The UUID of the BpjsDoctorSchedule to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsDoctorScheduleEntity> bpjsDoctorScheduleEntity = bpjsDoctorScheduleService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Doctor Schedule. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDoctorScheduleEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsDoctorSchedule' endpoint here] off begin
	@Operation(
			description = "Create a BpjsDoctorSchedule if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsDoctorSchedule."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsDoctorSchedule, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsDoctorScheduleEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsDoctorSchedule' endpoint here] end
	public ResponseEntity<?> saveBpjsDoctorSchedule(
			@Parameter(description = "The BpjsDoctorScheduleEntity to create or update.")
			@RequestBody BpjsDoctorScheduleEntityDto bpjsDoctorScheduleEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsDoctorScheduleEntity bpjsDoctorScheduleEntity = bpjsDoctorScheduleService.create(new BpjsDoctorScheduleEntity(bpjsDoctorScheduleEntityDto));
			return new ResponseEntity<>(bpjsDoctorScheduleEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Doctor Schedule. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDoctorScheduleEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsDoctorSchedule' endpoint here] off begin
	@Operation(
			description = "Update a BpjsDoctorSchedule entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsDoctorSchedule."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsDoctorSchedule, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsDoctorScheduleEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsDoctorSchedule' endpoint here] end
	public ResponseEntity<?> updateBpjsDoctorSchedule(
			@Parameter(description = "The BpjsDoctorScheduleEntity to create or update.")
			@RequestBody BpjsDoctorScheduleEntityDto bpjsDoctorScheduleEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsDoctorScheduleEntity> checkBpjsDoctorScheduleExists = bpjsDoctorScheduleService.findById(bpjsDoctorScheduleEntityDto.getId());
		BpjsDoctorScheduleEntity bpjsDoctorScheduleEntity = new BpjsDoctorScheduleEntity(bpjsDoctorScheduleEntityDto);

		if (checkBpjsDoctorScheduleExists.isPresent()) {
			try {
				bpjsDoctorScheduleEntity = bpjsDoctorScheduleService.update(bpjsDoctorScheduleEntity);
				return new ResponseEntity<>(bpjsDoctorScheduleEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsDoctorScheduleEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Doctor Schedule that has the same id as the given id.
	 *
	 * @param id The id of the BpjsDoctorScheduleEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsDoctorSchedule as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsDoctorSchedule to delete.")
		...
		bpjsDoctorScheduleService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorSchedule' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Doctor Schedule entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorSchedule' endpoint here] end
	public void exportBpjsDoctorSchedule(
			@Parameter(description = "The ids of the BpjsDoctorScheduleEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsDoctorScheduleEntity> entitiesToExport = bpjsDoctorScheduleService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsDoctorSchedule_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsDoctorScheduleEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorScheduleExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorScheduleExcludingIds' endpoint here] end
	public void exportBpjsDoctorScheduleExcludingIds(
			@Parameter(description = "The ids of the BpjsDoctorScheduleEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsDoctorScheduleEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsDoctorScheduleService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsDoctorScheduleService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsDoctorSchedule entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsDoctorSchedule_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsDoctorScheduleEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Doctor Schedule entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsDoctorScheduleEntity', 'create')  || hasPermission('BpjsDoctorScheduleEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsDoctorScheduleEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsDoctorScheduleEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsDoctorScheduleEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsDoctorScheduleEntity> csvToBean = new CsvToBeanBuilder<BpjsDoctorScheduleEntity>(reader)
				.withType(BpjsDoctorScheduleEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsDoctorScheduleEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsDoctorScheduleEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Doctor Schedule entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Doctor Schedule entity and you do not have permission");
					}
