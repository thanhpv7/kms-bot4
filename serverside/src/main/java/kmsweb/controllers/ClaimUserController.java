...
import kmsweb.dtos.ClaimUserEntityDto;
import kmsweb.entities.ClaimUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Claim User
 */
// % protected region % [Update the API route or description for the Claim UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ClaimUserController"))
@RestController
@Slf4j
@RequestMapping("/api/claimuser")
// % protected region % [Update the API route or description for the Claim UserController here] end
public class ClaimUserController {

	private final ClaimUserService claimUserService;
	private final CsvUtils<ClaimUserEntity, ClaimUserRepository, ClaimUserEntityAudit,  ClaimUserService> csvUtils;
	...
	public ClaimUserController(
		...
		this.csvUtils = new CsvUtils<>(claimUserService, ClaimUserEntity.class);
	...
	public ResponseEntity<List<ClaimUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ClaimUserEntity> claimUsers = claimUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(claimUsers, HttpStatus.OK);
	}

	/**
	 * Return the Claim User that has the same id as the given id.
	 *
	 * @param id      The id of the ClaimUserEntity to be returned
	 * @return the Claim User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ClaimUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ClaimUser entity not found"),
	})
	@PreAuthorize("hasPermission('ClaimUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ClaimUserEntity> getById(
			@Parameter(description = "The UUID of the ClaimUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ClaimUserEntity> claimUserEntity = claimUserService.findById(id);
	...
	/**
	 * Create/Update the given Claim User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param claimUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveClaimUser' endpoint here] off begin
	@Operation(
			description = "Create a ClaimUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ClaimUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ClaimUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('ClaimUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveClaimUser' endpoint here] end
	public ResponseEntity<?> saveClaimUser(
			@Parameter(description = "The ClaimUserEntity to create or update.")
			@RequestBody ClaimUserEntityDto claimUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ClaimUserEntity claimUserEntity = claimUserService.create(new ClaimUserEntity(claimUserEntityDto));
			return new ResponseEntity<>(claimUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Claim User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param claimUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateClaimUser' endpoint here] off begin
	@Operation(
			description = "Update a ClaimUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ClaimUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ClaimUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ClaimUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateClaimUser' endpoint here] end
	public ResponseEntity<?> updateClaimUser(
			@Parameter(description = "The ClaimUserEntity to create or update.")
			@RequestBody ClaimUserEntityDto claimUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ClaimUserEntity> checkClaimUserExists = claimUserService.findById(claimUserEntityDto.getId());
		ClaimUserEntity claimUserEntity = new ClaimUserEntity(claimUserEntityDto);

		if (checkClaimUserExists.isPresent()) {
			try {
				claimUserEntity = claimUserService.update(claimUserEntity);
				return new ResponseEntity<>(claimUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(claimUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Claim User that has the same id as the given id.
	 *
	 * @param id The id of the ClaimUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single ClaimUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ClaimUser to delete.")
		...
		claimUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportClaimUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Claim User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportClaimUser' endpoint here] end
	public void exportClaimUser(
			@Parameter(description = "The ids of the ClaimUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ClaimUserEntity> entitiesToExport = claimUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ClaimUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ClaimUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportClaimUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportClaimUserExcludingIds' endpoint here] end
	public void exportClaimUserExcludingIds(
			@Parameter(description = "The ids of the ClaimUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ClaimUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = claimUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = claimUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ClaimUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ClaimUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ClaimUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Claim User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ClaimUserEntity', 'create')  || hasPermission('ClaimUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ClaimUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ClaimUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ClaimUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ClaimUserEntity> csvToBean = new CsvToBeanBuilder<ClaimUserEntity>(reader)
				.withType(ClaimUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ClaimUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ClaimUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Claim User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Claim User entity and you do not have permission");
					}
