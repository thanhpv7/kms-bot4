...
import kmsweb.dtos.FacilityUserEntityDto;
...
/**
 * Controller used to handle all REST operations regarding Facility User
 */
// % protected region % [Update the API route or description for the Facility UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "FacilityUserController"))
@RestController
@Slf4j
@RequestMapping("/api/facilityuser")
// % protected region % [Update the API route or description for the Facility UserController here] end
public class FacilityUserController {

	private final FacilityUserService facilityUserService;
	private final CsvUtils<FacilityUserEntity, FacilityUserRepository, FacilityUserEntityAudit,  FacilityUserService> csvUtils;
	...
	public FacilityUserController(
		...
		this.csvUtils = new CsvUtils<>(facilityUserService, FacilityUserEntity.class);
	...
	public ResponseEntity<List<FacilityUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<FacilityUserEntity> facilityUsers = facilityUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(facilityUsers, HttpStatus.OK);
	}

	/**
	 * Return the Facility User that has the same id as the given id.
	 *
	 * @param id      The id of the FacilityUserEntity to be returned
	 * @return the Facility User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single FacilityUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "FacilityUser entity not found"),
	})
	@PreAuthorize("hasPermission('FacilityUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<FacilityUserEntity> getById(
			@Parameter(description = "The UUID of the FacilityUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<FacilityUserEntity> facilityUserEntity = facilityUserService.findById(id);
	...
	/**
	 * Create/Update the given Facility User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param facilityUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveFacilityUser' endpoint here] off begin
	@Operation(
			description = "Create a FacilityUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new FacilityUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new FacilityUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('FacilityUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveFacilityUser' endpoint here] end
	public ResponseEntity<?> saveFacilityUser(
			@Parameter(description = "The FacilityUserEntity to create or update.")
			@RequestBody FacilityUserEntityDto facilityUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			FacilityUserEntity facilityUserEntity = facilityUserService.create(new FacilityUserEntity(facilityUserEntityDto));
			return new ResponseEntity<>(facilityUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Facility User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param facilityUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateFacilityUser' endpoint here] off begin
	@Operation(
			description = "Update a FacilityUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the FacilityUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the FacilityUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('FacilityUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateFacilityUser' endpoint here] end
	public ResponseEntity<?> updateFacilityUser(
			@Parameter(description = "The FacilityUserEntity to create or update.")
			@RequestBody FacilityUserEntityDto facilityUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<FacilityUserEntity> checkFacilityUserExists = facilityUserService.findById(facilityUserEntityDto.getId());
		FacilityUserEntity facilityUserEntity = new FacilityUserEntity(facilityUserEntityDto);

		if (checkFacilityUserExists.isPresent()) {
			try {
				facilityUserEntity = facilityUserService.update(facilityUserEntity);
				return new ResponseEntity<>(facilityUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(facilityUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Facility User that has the same id as the given id.
	 *
	 * @param id The id of the FacilityUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single FacilityUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the FacilityUser to delete.")
		...
		facilityUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportFacilityUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Facility User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportFacilityUser' endpoint here] end
	public void exportFacilityUser(
			@Parameter(description = "The ids of the FacilityUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<FacilityUserEntity> entitiesToExport = facilityUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "FacilityUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (FacilityUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportFacilityUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportFacilityUserExcludingIds' endpoint here] end
	public void exportFacilityUserExcludingIds(
			@Parameter(description = "The ids of the FacilityUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<FacilityUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = facilityUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = facilityUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the FacilityUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "FacilityUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(FacilityUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Facility User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('FacilityUserEntity', 'create')  || hasPermission('FacilityUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with FacilityUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("FacilityUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(FacilityUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<FacilityUserEntity> csvToBean = new CsvToBeanBuilder<FacilityUserEntity>(reader)
				.withType(FacilityUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<FacilityUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<FacilityUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Facility User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Facility User entity and you do not have permission");
					}
