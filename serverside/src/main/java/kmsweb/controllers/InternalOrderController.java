...
/**
 * Controller used to handle all REST operations regarding Internal Order
 */
// % protected region % [Update the API route or description for the Internal OrderController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InternalOrderController"))
@RestController
@Slf4j
@RequestMapping("/api/internalorder")
// % protected region % [Update the API route or description for the Internal OrderController here] end
public class InternalOrderController {

	private final InternalOrderService internalOrderService;
	private final CsvUtils<InternalOrderEntity, InternalOrderRepository, InternalOrderEntityAudit,  InternalOrderService> csvUtils;
	...
	public InternalOrderController(
		...
		this.csvUtils = new CsvUtils<>(internalOrderService, InternalOrderEntity.class);
	...
	public ResponseEntity<List<InternalOrderEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InternalOrderEntity> internalOrders = internalOrderService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(internalOrders, HttpStatus.OK);
	}

	/**
	 * Return the Internal Order that has the same id as the given id.
	 *
	 * @param id      The id of the InternalOrderEntity to be returned
	 * @return the Internal Order that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InternalOrder as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InternalOrder entity not found"),
	})
	@PreAuthorize("hasPermission('InternalOrderEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InternalOrderEntity> getById(
			@Parameter(description = "The UUID of the InternalOrder to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InternalOrderEntity> internalOrderEntity = internalOrderService.findById(id);
	...
	/**
	 * Create/Update the given Internal Order. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param internalOrderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInternalOrder' endpoint here] off begin
	@Operation(
			description = "Create a InternalOrder if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InternalOrder."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InternalOrder, duplicate record found")
	})
	@PreAuthorize("hasPermission('InternalOrderEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInternalOrder' endpoint here] end
	public ResponseEntity<?> saveInternalOrder(
			@Parameter(description = "The InternalOrderEntity to create or update.")
			@RequestBody InternalOrderEntityDto internalOrderEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InternalOrderEntity internalOrderEntity = internalOrderService.create(new InternalOrderEntity(internalOrderEntityDto));
			return new ResponseEntity<>(internalOrderEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Internal Order. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param internalOrderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInternalOrder' endpoint here] off begin
	@Operation(
			description = "Update a InternalOrder entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InternalOrder."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InternalOrder, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InternalOrderEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInternalOrder' endpoint here] end
	public ResponseEntity<?> updateInternalOrder(
			@Parameter(description = "The InternalOrderEntity to create or update.")
			@RequestBody InternalOrderEntityDto internalOrderEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InternalOrderEntity> checkInternalOrderExists = internalOrderService.findById(internalOrderEntityDto.getId());
		InternalOrderEntity internalOrderEntity = new InternalOrderEntity(internalOrderEntityDto);

		if (checkInternalOrderExists.isPresent()) {
			try {
				internalOrderEntity = internalOrderService.update(internalOrderEntity);
				return new ResponseEntity<>(internalOrderEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(internalOrderEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Internal Order that has the same id as the given id.
	 *
	 * @param id The id of the InternalOrderEntity to be deleted
	...
	@Operation(
			description = "Delete a single InternalOrder as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InternalOrder to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInternalOrder' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Internal Order entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInternalOrder' endpoint here] end
	public void exportInternalOrder(
			@Parameter(description = "The ids of the InternalOrderEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InternalOrderEntity> entitiesToExport = internalOrderService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InternalOrder_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InternalOrderEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInternalOrderExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInternalOrderExcludingIds' endpoint here] end
	public void exportInternalOrderExcludingIds(
			@Parameter(description = "The ids of the InternalOrderEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InternalOrderEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = internalOrderService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = internalOrderService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InternalOrder entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InternalOrder_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InternalOrderEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Internal Order entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InternalOrderEntity', 'create')  || hasPermission('InternalOrderEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InternalOrderEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InternalOrderEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InternalOrderEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InternalOrderEntity> csvToBean = new CsvToBeanBuilder<InternalOrderEntity>(reader)
				.withType(InternalOrderEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InternalOrderEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InternalOrderEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Internal Order entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Internal Order entity and you do not have permission");
					}
