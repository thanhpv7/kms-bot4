...
/**
 * Controller used to handle all REST operations regarding Catalogue Non Medical
 */
// % protected region % [Update the API route or description for the Catalogue Non MedicalController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CatalogueNonMedicalController"))
@RestController
@Slf4j
@RequestMapping("/api/cataloguenonmedical")
// % protected region % [Update the API route or description for the Catalogue Non MedicalController here] end
public class CatalogueNonMedicalController {

	private final CatalogueNonMedicalService catalogueNonMedicalService;
	private final CsvUtils<CatalogueNonMedicalEntity, CatalogueNonMedicalRepository, CatalogueNonMedicalEntityAudit,  CatalogueNonMedicalService> csvUtils;
	...
	@Autowired
	public CatalogueNonMedicalController(
		...
		this.csvUtils = new CsvUtils<>(catalogueNonMedicalService, CatalogueNonMedicalEntity.class);
	...
	public ResponseEntity<List<CatalogueNonMedicalEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CatalogueNonMedicalEntity> catalogueNonMedicals = catalogueNonMedicalService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(catalogueNonMedicals, HttpStatus.OK);
	}

	/**
	 * Return the Catalogue Non Medical that has the same id as the given id.
	 *
	 * @param id      The id of the CatalogueNonMedicalEntity to be returned
	 * @return the Catalogue Non Medical that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CatalogueNonMedical as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CatalogueNonMedical entity not found"),
	})
	@PreAuthorize("hasPermission('CatalogueNonMedicalEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CatalogueNonMedicalEntity> getById(
			@Parameter(description = "The UUID of the CatalogueNonMedical to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CatalogueNonMedicalEntity> catalogueNonMedicalEntity = catalogueNonMedicalService.findById(id);
	...
	/**
	 * Create/Update the given Catalogue Non Medical. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param catalogueNonMedicalEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCatalogueNonMedical' endpoint here] off begin
	@Operation(
			description = "Create a CatalogueNonMedical if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CatalogueNonMedical."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CatalogueNonMedical, duplicate record found")
	})
	@PreAuthorize("hasPermission('CatalogueNonMedicalEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCatalogueNonMedical' endpoint here] end
	public ResponseEntity<?> saveCatalogueNonMedical(
			@Parameter(description = "The CatalogueNonMedicalEntity to create or update.")
			@RequestBody CatalogueNonMedicalEntityDto catalogueNonMedicalEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CatalogueNonMedicalEntity catalogueNonMedicalEntity = catalogueNonMedicalService.create(new CatalogueNonMedicalEntity(catalogueNonMedicalEntityDto));
			return new ResponseEntity<>(catalogueNonMedicalEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Catalogue Non Medical. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param catalogueNonMedicalEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCatalogueNonMedical' endpoint here] off begin
	@Operation(
			description = "Update a CatalogueNonMedical entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CatalogueNonMedical."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CatalogueNonMedical, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CatalogueNonMedicalEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCatalogueNonMedical' endpoint here] end
	public ResponseEntity<?> updateCatalogueNonMedical(
			@Parameter(description = "The CatalogueNonMedicalEntity to create or update.")
			@RequestBody CatalogueNonMedicalEntityDto catalogueNonMedicalEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CatalogueNonMedicalEntity> checkCatalogueNonMedicalExists = catalogueNonMedicalService.findById(catalogueNonMedicalEntityDto.getId());
		CatalogueNonMedicalEntity catalogueNonMedicalEntity = new CatalogueNonMedicalEntity(catalogueNonMedicalEntityDto);

		if (checkCatalogueNonMedicalExists.isPresent()) {
			try {
				catalogueNonMedicalEntity = catalogueNonMedicalService.update(catalogueNonMedicalEntity);
				return new ResponseEntity<>(catalogueNonMedicalEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(catalogueNonMedicalEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Catalogue Non Medical that has the same id as the given id.
	 *
	 * @param id The id of the CatalogueNonMedicalEntity to be deleted
	...
	@Operation(
			description = "Delete a single CatalogueNonMedical as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CatalogueNonMedical to delete.")
		...
		catalogueNonMedicalService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCatalogueNonMedical' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Catalogue Non Medical entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCatalogueNonMedical' endpoint here] end
	public void exportCatalogueNonMedical(
			@Parameter(description = "The ids of the CatalogueNonMedicalEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CatalogueNonMedicalEntity> entitiesToExport = catalogueNonMedicalService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CatalogueNonMedical_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CatalogueNonMedicalEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCatalogueNonMedicalExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCatalogueNonMedicalExcludingIds' endpoint here] end
	public void exportCatalogueNonMedicalExcludingIds(
			@Parameter(description = "The ids of the CatalogueNonMedicalEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CatalogueNonMedicalEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = catalogueNonMedicalService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = catalogueNonMedicalService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CatalogueNonMedical entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CatalogueNonMedical_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CatalogueNonMedicalEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Catalogue Non Medical entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CatalogueNonMedicalEntity', 'create')  || hasPermission('CatalogueNonMedicalEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CatalogueNonMedicalEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CatalogueNonMedicalEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CatalogueNonMedicalEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CatalogueNonMedicalEntity> csvToBean = new CsvToBeanBuilder<CatalogueNonMedicalEntity>(reader)
				.withType(CatalogueNonMedicalEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CatalogueNonMedicalEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CatalogueNonMedicalEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Catalogue Non Medical entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Catalogue Non Medical entity and you do not have permission");
					}
