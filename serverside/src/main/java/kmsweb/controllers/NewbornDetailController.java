...
/**
 * Controller used to handle all REST operations regarding Newborn Detail
 */
// % protected region % [Update the API route or description for the Newborn DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NewbornDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/newborndetail")
// % protected region % [Update the API route or description for the Newborn DetailController here] end
public class NewbornDetailController {

	private final NewbornDetailService newbornDetailService;
	private final CsvUtils<NewbornDetailEntity, NewbornDetailRepository, NewbornDetailEntityAudit,  NewbornDetailService> csvUtils;
	...
	@Autowired
	public NewbornDetailController(
		...
		this.csvUtils = new CsvUtils<>(newbornDetailService, NewbornDetailEntity.class);
	...
	public ResponseEntity<List<NewbornDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NewbornDetailEntity> newbornDetails = newbornDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(newbornDetails, HttpStatus.OK);
	}

	/**
	 * Return the Newborn Detail that has the same id as the given id.
	 *
	 * @param id      The id of the NewbornDetailEntity to be returned
	 * @return the Newborn Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NewbornDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NewbornDetail entity not found"),
	})
	@PreAuthorize("hasPermission('NewbornDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NewbornDetailEntity> getById(
			@Parameter(description = "The UUID of the NewbornDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NewbornDetailEntity> newbornDetailEntity = newbornDetailService.findById(id);
	...
	/**
	 * Create/Update the given Newborn Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param newbornDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNewbornDetail' endpoint here] off begin
	@Operation(
			description = "Create a NewbornDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NewbornDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NewbornDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('NewbornDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNewbornDetail' endpoint here] end
	public ResponseEntity<?> saveNewbornDetail(
			@Parameter(description = "The NewbornDetailEntity to create or update.")
			@RequestBody NewbornDetailEntityDto newbornDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NewbornDetailEntity newbornDetailEntity = newbornDetailService.create(new NewbornDetailEntity(newbornDetailEntityDto));
			return new ResponseEntity<>(newbornDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Newborn Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param newbornDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNewbornDetail' endpoint here] off begin
	@Operation(
			description = "Update a NewbornDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NewbornDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NewbornDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NewbornDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNewbornDetail' endpoint here] end
	public ResponseEntity<?> updateNewbornDetail(
			@Parameter(description = "The NewbornDetailEntity to create or update.")
			@RequestBody NewbornDetailEntityDto newbornDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NewbornDetailEntity> checkNewbornDetailExists = newbornDetailService.findById(newbornDetailEntityDto.getId());
		NewbornDetailEntity newbornDetailEntity = new NewbornDetailEntity(newbornDetailEntityDto);

		if (checkNewbornDetailExists.isPresent()) {
			try {
				newbornDetailEntity = newbornDetailService.update(newbornDetailEntity);
				return new ResponseEntity<>(newbornDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(newbornDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Newborn Detail that has the same id as the given id.
	 *
	 * @param id The id of the NewbornDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single NewbornDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NewbornDetail to delete.")
		...
		newbornDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNewbornDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Newborn Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNewbornDetail' endpoint here] end
	public void exportNewbornDetail(
			@Parameter(description = "The ids of the NewbornDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NewbornDetailEntity> entitiesToExport = newbornDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NewbornDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NewbornDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNewbornDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNewbornDetailExcludingIds' endpoint here] end
	public void exportNewbornDetailExcludingIds(
			@Parameter(description = "The ids of the NewbornDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NewbornDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = newbornDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = newbornDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NewbornDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NewbornDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NewbornDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Newborn Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NewbornDetailEntity', 'create')  || hasPermission('NewbornDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NewbornDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NewbornDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NewbornDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NewbornDetailEntity> csvToBean = new CsvToBeanBuilder<NewbornDetailEntity>(reader)
				.withType(NewbornDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NewbornDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NewbornDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Newborn Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Newborn Detail entity and you do not have permission");
					}
