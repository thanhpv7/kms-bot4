...
/**
 * Controller used to handle all REST operations regarding Transaction Detail Type Mapping
 */
// % protected region % [Update the API route or description for the Transaction Detail Type MappingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TransactionDetailTypeMappingController"))
@RestController
@Slf4j
@RequestMapping("/api/transactiondetailtypemapping")
// % protected region % [Update the API route or description for the Transaction Detail Type MappingController here] end
public class TransactionDetailTypeMappingController {

	private final TransactionDetailTypeMappingService transactionDetailTypeMappingService;
	private final CsvUtils<TransactionDetailTypeMappingEntity, TransactionDetailTypeMappingRepository, TransactionDetailTypeMappingEntityAudit,  TransactionDetailTypeMappingService> csvUtils;
	...
	@Autowired
	public TransactionDetailTypeMappingController(
		...
		this.csvUtils = new CsvUtils<>(transactionDetailTypeMappingService, TransactionDetailTypeMappingEntity.class);
	...
	/**
	 * Return all the Transaction Detail Type Mappings.
	 *
	 * @return all the Transaction Detail Type Mappings
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of TransactionDetailTypeMappings",
		security = {
	...
	@PreAuthorize("hasPermission('TransactionDetailTypeMappingEntity', 'read')")
	@GetMapping(produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] end
	public ResponseEntity<List<TransactionDetailTypeMappingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TransactionDetailTypeMappingEntity> transactionDetailTypeMappings = transactionDetailTypeMappingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(transactionDetailTypeMappings, HttpStatus.OK);
	}

	/**
	 * Return the Transaction Detail Type Mapping that has the same id as the given id.
	 *
	 * @param id      The id of the TransactionDetailTypeMappingEntity to be returned
	 * @return the Transaction Detail Type Mapping that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TransactionDetailTypeMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TransactionDetailTypeMapping entity not found"),
	})
	@PreAuthorize("hasPermission('TransactionDetailTypeMappingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TransactionDetailTypeMappingEntity> getById(
			@Parameter(description = "The UUID of the TransactionDetailTypeMapping to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TransactionDetailTypeMappingEntity> transactionDetailTypeMappingEntity = transactionDetailTypeMappingService.findById(id);
	...
	/**
	 * Create/Update the given Transaction Detail Type Mapping. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param transactionDetailTypeMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTransactionDetailTypeMapping' endpoint here] off begin
	@Operation(
			description = "Create a TransactionDetailTypeMapping if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TransactionDetailTypeMapping."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TransactionDetailTypeMapping, duplicate record found")
	})
	@PreAuthorize("hasPermission('TransactionDetailTypeMappingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTransactionDetailTypeMapping' endpoint here] end
	public ResponseEntity<?> saveTransactionDetailTypeMapping(
			@Parameter(description = "The TransactionDetailTypeMappingEntity to create or update.")
			@RequestBody TransactionDetailTypeMappingEntityDto transactionDetailTypeMappingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TransactionDetailTypeMappingEntity transactionDetailTypeMappingEntity = transactionDetailTypeMappingService.create(new TransactionDetailTypeMappingEntity(transactionDetailTypeMappingEntityDto));
			return new ResponseEntity<>(transactionDetailTypeMappingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Transaction Detail Type Mapping. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param transactionDetailTypeMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTransactionDetailTypeMapping' endpoint here] off begin
	@Operation(
			description = "Update a TransactionDetailTypeMapping entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TransactionDetailTypeMapping."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TransactionDetailTypeMapping, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TransactionDetailTypeMappingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTransactionDetailTypeMapping' endpoint here] end
	public ResponseEntity<?> updateTransactionDetailTypeMapping(
			@Parameter(description = "The TransactionDetailTypeMappingEntity to create or update.")
			@RequestBody TransactionDetailTypeMappingEntityDto transactionDetailTypeMappingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TransactionDetailTypeMappingEntity> checkTransactionDetailTypeMappingExists = transactionDetailTypeMappingService.findById(transactionDetailTypeMappingEntityDto.getId());
		TransactionDetailTypeMappingEntity transactionDetailTypeMappingEntity = new TransactionDetailTypeMappingEntity(transactionDetailTypeMappingEntityDto);

		if (checkTransactionDetailTypeMappingExists.isPresent()) {
			try {
				transactionDetailTypeMappingEntity = transactionDetailTypeMappingService.update(transactionDetailTypeMappingEntity);
				return new ResponseEntity<>(transactionDetailTypeMappingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(transactionDetailTypeMappingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Transaction Detail Type Mapping that has the same id as the given id.
	 *
	 * @param id The id of the TransactionDetailTypeMappingEntity to be deleted
	...
	@Operation(
			description = "Delete a single TransactionDetailTypeMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('TransactionDetailTypeMappingEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TransactionDetailTypeMapping to delete.")
		...
		transactionDetailTypeMappingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailTypeMapping' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Transaction Detail Type Mapping entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailTypeMapping' endpoint here] end
	public void exportTransactionDetailTypeMapping(
			@Parameter(description = "The ids of the TransactionDetailTypeMappingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TransactionDetailTypeMappingEntity> entitiesToExport = transactionDetailTypeMappingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TransactionDetailTypeMapping_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TransactionDetailTypeMappingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailTypeMappingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailTypeMappingExcludingIds' endpoint here] end
	public void exportTransactionDetailTypeMappingExcludingIds(
			@Parameter(description = "The ids of the TransactionDetailTypeMappingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TransactionDetailTypeMappingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = transactionDetailTypeMappingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = transactionDetailTypeMappingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TransactionDetailTypeMapping entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TransactionDetailTypeMapping_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TransactionDetailTypeMappingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Transaction Detail Type Mapping entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TransactionDetailTypeMappingEntity', 'create')  || hasPermission('TransactionDetailTypeMappingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TransactionDetailTypeMappingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TransactionDetailTypeMappingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TransactionDetailTypeMappingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TransactionDetailTypeMappingEntity> csvToBean = new CsvToBeanBuilder<TransactionDetailTypeMappingEntity>(reader)
				.withType(TransactionDetailTypeMappingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TransactionDetailTypeMappingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TransactionDetailTypeMappingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Transaction Detail Type Mapping entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Transaction Detail Type Mapping entity and you do not have permission");
					}
