...
/**
 * Controller used to handle all REST operations regarding Service Facility Category
 */
// % protected region % [Update the API route or description for the Service Facility CategoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ServiceFacilityCategoryController"))
@RestController
@Slf4j
@RequestMapping("/api/servicefacilitycategory")
// % protected region % [Update the API route or description for the Service Facility CategoryController here] end
public class ServiceFacilityCategoryController {

	private final ServiceFacilityCategoryService serviceFacilityCategoryService;
	private final CsvUtils<ServiceFacilityCategoryEntity, ServiceFacilityCategoryRepository, ServiceFacilityCategoryEntityAudit,  ServiceFacilityCategoryService> csvUtils;
	...
	@Autowired
	public ServiceFacilityCategoryController(
		...
		this.csvUtils = new CsvUtils<>(serviceFacilityCategoryService, ServiceFacilityCategoryEntity.class);
	...
	public ResponseEntity<List<ServiceFacilityCategoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ServiceFacilityCategoryEntity> serviceFacilityCategorys = serviceFacilityCategoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(serviceFacilityCategorys, HttpStatus.OK);
	}

	/**
	 * Return the Service Facility Category that has the same id as the given id.
	 *
	 * @param id      The id of the ServiceFacilityCategoryEntity to be returned
	 * @return the Service Facility Category that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ServiceFacilityCategory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ServiceFacilityCategory entity not found"),
	})
	@PreAuthorize("hasPermission('ServiceFacilityCategoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ServiceFacilityCategoryEntity> getById(
			@Parameter(description = "The UUID of the ServiceFacilityCategory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ServiceFacilityCategoryEntity> serviceFacilityCategoryEntity = serviceFacilityCategoryService.findById(id);
	...
	/**
	 * Create/Update the given Service Facility Category. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param serviceFacilityCategoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveServiceFacilityCategory' endpoint here] off begin
	@Operation(
			description = "Create a ServiceFacilityCategory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ServiceFacilityCategory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ServiceFacilityCategory, duplicate record found")
	})
	@PreAuthorize("hasPermission('ServiceFacilityCategoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveServiceFacilityCategory' endpoint here] end
	public ResponseEntity<?> saveServiceFacilityCategory(
			@Parameter(description = "The ServiceFacilityCategoryEntity to create or update.")
			@RequestBody ServiceFacilityCategoryEntityDto serviceFacilityCategoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ServiceFacilityCategoryEntity serviceFacilityCategoryEntity = serviceFacilityCategoryService.create(new ServiceFacilityCategoryEntity(serviceFacilityCategoryEntityDto));
			return new ResponseEntity<>(serviceFacilityCategoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Service Facility Category. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param serviceFacilityCategoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateServiceFacilityCategory' endpoint here] off begin
	@Operation(
			description = "Update a ServiceFacilityCategory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ServiceFacilityCategory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ServiceFacilityCategory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ServiceFacilityCategoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateServiceFacilityCategory' endpoint here] end
	public ResponseEntity<?> updateServiceFacilityCategory(
			@Parameter(description = "The ServiceFacilityCategoryEntity to create or update.")
			@RequestBody ServiceFacilityCategoryEntityDto serviceFacilityCategoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ServiceFacilityCategoryEntity> checkServiceFacilityCategoryExists = serviceFacilityCategoryService.findById(serviceFacilityCategoryEntityDto.getId());
		ServiceFacilityCategoryEntity serviceFacilityCategoryEntity = new ServiceFacilityCategoryEntity(serviceFacilityCategoryEntityDto);

		if (checkServiceFacilityCategoryExists.isPresent()) {
			try {
				serviceFacilityCategoryEntity = serviceFacilityCategoryService.update(serviceFacilityCategoryEntity);
				return new ResponseEntity<>(serviceFacilityCategoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(serviceFacilityCategoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Service Facility Category that has the same id as the given id.
	 *
	 * @param id The id of the ServiceFacilityCategoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single ServiceFacilityCategory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('ServiceFacilityCategoryEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ServiceFacilityCategory to delete.")
		...
		serviceFacilityCategoryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportServiceFacilityCategory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Service Facility Category entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportServiceFacilityCategory' endpoint here] end
	public void exportServiceFacilityCategory(
			@Parameter(description = "The ids of the ServiceFacilityCategoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ServiceFacilityCategoryEntity> entitiesToExport = serviceFacilityCategoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ServiceFacilityCategory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ServiceFacilityCategoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportServiceFacilityCategoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportServiceFacilityCategoryExcludingIds' endpoint here] end
	public void exportServiceFacilityCategoryExcludingIds(
			@Parameter(description = "The ids of the ServiceFacilityCategoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ServiceFacilityCategoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = serviceFacilityCategoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = serviceFacilityCategoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ServiceFacilityCategory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ServiceFacilityCategory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ServiceFacilityCategoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Service Facility Category entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ServiceFacilityCategoryEntity', 'create')  || hasPermission('ServiceFacilityCategoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ServiceFacilityCategoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ServiceFacilityCategoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ServiceFacilityCategoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ServiceFacilityCategoryEntity> csvToBean = new CsvToBeanBuilder<ServiceFacilityCategoryEntity>(reader)
				.withType(ServiceFacilityCategoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ServiceFacilityCategoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ServiceFacilityCategoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Service Facility Category entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Service Facility Category entity and you do not have permission");
					}
