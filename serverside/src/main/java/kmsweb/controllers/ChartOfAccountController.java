...
import kmsweb.entities.ChartOfAccountEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Chart of Account
 */
// % protected region % [Update the API route or description for the Chart of AccountController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ChartOfAccountController"))
@RestController
@Slf4j
@RequestMapping("/api/chartofaccount")
// % protected region % [Update the API route or description for the Chart of AccountController here] end
public class ChartOfAccountController {

	private final ChartOfAccountService chartOfAccountService;
	private final CsvUtils<ChartOfAccountEntity, ChartOfAccountRepository, ChartOfAccountEntityAudit,  ChartOfAccountService> csvUtils;
	...
	public ChartOfAccountController(
		...
		this.csvUtils = new CsvUtils<>(chartOfAccountService, ChartOfAccountEntity.class);
	...
	public ResponseEntity<List<ChartOfAccountEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ChartOfAccountEntity> chartOfAccounts = chartOfAccountService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Chart of Account that has the same id as the given id.
	 *
	 * @param id      The id of the ChartOfAccountEntity to be returned
	 * @return the Chart of Account that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ChartOfAccount as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ChartOfAccount entity not found"),
	})
	@PreAuthorize("hasPermission('ChartOfAccountEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ChartOfAccountEntity> getById(
			@Parameter(description = "The UUID of the ChartOfAccount to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ChartOfAccountEntity> chartOfAccountEntity = chartOfAccountService.findById(id);
	...
	/**
	 * Create/Update the given Chart of Account. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param chartOfAccountEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveChartOfAccount' endpoint here] off begin
	@Operation(
			description = "Create a ChartOfAccount if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ChartOfAccount."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ChartOfAccount, duplicate record found")
	})
	@PreAuthorize("hasPermission('ChartOfAccountEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveChartOfAccount' endpoint here] end
	public ResponseEntity<?> saveChartOfAccount(
			@Parameter(description = "The ChartOfAccountEntity to create or update.")
			@RequestBody ChartOfAccountEntityDto chartOfAccountEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ChartOfAccountEntity chartOfAccountEntity = chartOfAccountService.create(new ChartOfAccountEntity(chartOfAccountEntityDto));
			return new ResponseEntity<>(chartOfAccountEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Chart of Account. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param chartOfAccountEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateChartOfAccount' endpoint here] off begin
	@Operation(
			description = "Update a ChartOfAccount entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ChartOfAccount."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ChartOfAccount, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ChartOfAccountEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateChartOfAccount' endpoint here] end
	public ResponseEntity<?> updateChartOfAccount(
		...
		Optional<ChartOfAccountEntity> checkChartOfAccountExists = chartOfAccountService.findById(chartOfAccountEntityDto.getId());
		ChartOfAccountEntity chartOfAccountEntity = new ChartOfAccountEntity(chartOfAccountEntityDto);

		if (checkChartOfAccountExists.isPresent()) {
			try {
				chartOfAccountEntity = chartOfAccountService.update(chartOfAccountEntity);
				return new ResponseEntity<>(chartOfAccountEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(chartOfAccountEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Chart of Account that has the same id as the given id.
	 *
	 * @param id The id of the ChartOfAccountEntity to be deleted
	...
	@Operation(
			description = "Delete a single ChartOfAccount as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ChartOfAccount to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccount' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Chart of Account entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccount' endpoint here] end
	public void exportChartOfAccount(
			@Parameter(description = "The ids of the ChartOfAccountEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ChartOfAccountEntity> entitiesToExport = chartOfAccountService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ChartOfAccount_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ChartOfAccountEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccountExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccountExcludingIds' endpoint here] end
	public void exportChartOfAccountExcludingIds(
			@Parameter(description = "The ids of the ChartOfAccountEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ChartOfAccountEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = chartOfAccountService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = chartOfAccountService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ChartOfAccount entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ChartOfAccount_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ChartOfAccountEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Chart of Account entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ChartOfAccountEntity', 'create')  || hasPermission('ChartOfAccountEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ChartOfAccountEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ChartOfAccountEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ChartOfAccountEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ChartOfAccountEntity> csvToBean = new CsvToBeanBuilder<ChartOfAccountEntity>(reader)
				.withType(ChartOfAccountEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ChartOfAccountEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ChartOfAccountEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Chart of Account entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Chart of Account entity and you do not have permission");
					}
