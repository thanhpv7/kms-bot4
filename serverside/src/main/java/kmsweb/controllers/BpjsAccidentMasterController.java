...
import kmsweb.entities.BpjsAccidentMasterEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Accident Master
 */
// % protected region % [Update the API route or description for the BPJS Accident MasterController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsAccidentMasterController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsaccidentmaster")
// % protected region % [Update the API route or description for the BPJS Accident MasterController here] end
public class BpjsAccidentMasterController {

	private final BpjsAccidentMasterService bpjsAccidentMasterService;
	private final CsvUtils<BpjsAccidentMasterEntity, BpjsAccidentMasterRepository, BpjsAccidentMasterEntityAudit,  BpjsAccidentMasterService> csvUtils;
	...
	@Autowired
	public BpjsAccidentMasterController(
		...
		this.csvUtils = new CsvUtils<>(bpjsAccidentMasterService, BpjsAccidentMasterEntity.class);
	...
	public ResponseEntity<List<BpjsAccidentMasterEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsAccidentMasterEntity> bpjsAccidentMasters = bpjsAccidentMasterService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsAccidentMasters, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Accident Master that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsAccidentMasterEntity to be returned
	 * @return the BPJS Accident Master that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsAccidentMaster as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsAccidentMaster entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsAccidentMasterEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsAccidentMasterEntity> getById(
			@Parameter(description = "The UUID of the BpjsAccidentMaster to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsAccidentMasterEntity> bpjsAccidentMasterEntity = bpjsAccidentMasterService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Accident Master. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsAccidentMasterEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsAccidentMaster' endpoint here] off begin
	@Operation(
			description = "Create a BpjsAccidentMaster if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsAccidentMaster."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsAccidentMaster, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsAccidentMasterEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsAccidentMaster' endpoint here] end
	public ResponseEntity<?> saveBpjsAccidentMaster(
			@Parameter(description = "The BpjsAccidentMasterEntity to create or update.")
			@RequestBody BpjsAccidentMasterEntityDto bpjsAccidentMasterEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsAccidentMasterEntity bpjsAccidentMasterEntity = bpjsAccidentMasterService.create(new BpjsAccidentMasterEntity(bpjsAccidentMasterEntityDto));
			return new ResponseEntity<>(bpjsAccidentMasterEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Accident Master. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsAccidentMasterEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsAccidentMaster' endpoint here] off begin
	@Operation(
			description = "Update a BpjsAccidentMaster entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsAccidentMaster."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsAccidentMaster, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsAccidentMasterEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsAccidentMaster' endpoint here] end
	public ResponseEntity<?> updateBpjsAccidentMaster(
			@Parameter(description = "The BpjsAccidentMasterEntity to create or update.")
			@RequestBody BpjsAccidentMasterEntityDto bpjsAccidentMasterEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsAccidentMasterEntity> checkBpjsAccidentMasterExists = bpjsAccidentMasterService.findById(bpjsAccidentMasterEntityDto.getId());
		BpjsAccidentMasterEntity bpjsAccidentMasterEntity = new BpjsAccidentMasterEntity(bpjsAccidentMasterEntityDto);

		if (checkBpjsAccidentMasterExists.isPresent()) {
			try {
				bpjsAccidentMasterEntity = bpjsAccidentMasterService.update(bpjsAccidentMasterEntity);
				return new ResponseEntity<>(bpjsAccidentMasterEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsAccidentMasterEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Accident Master that has the same id as the given id.
	 *
	 * @param id The id of the BpjsAccidentMasterEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsAccidentMaster as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsAccidentMaster to delete.")
		...
		bpjsAccidentMasterService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsAccidentMaster' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Accident Master entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsAccidentMaster' endpoint here] end
	public void exportBpjsAccidentMaster(
			@Parameter(description = "The ids of the BpjsAccidentMasterEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsAccidentMasterEntity> entitiesToExport = bpjsAccidentMasterService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsAccidentMaster_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsAccidentMasterEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsAccidentMasterExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsAccidentMasterExcludingIds' endpoint here] end
	public void exportBpjsAccidentMasterExcludingIds(
			@Parameter(description = "The ids of the BpjsAccidentMasterEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsAccidentMasterEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsAccidentMasterService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsAccidentMasterService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsAccidentMaster entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsAccidentMaster_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsAccidentMasterEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Accident Master entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsAccidentMasterEntity', 'create')  || hasPermission('BpjsAccidentMasterEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsAccidentMasterEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsAccidentMasterEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsAccidentMasterEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsAccidentMasterEntity> csvToBean = new CsvToBeanBuilder<BpjsAccidentMasterEntity>(reader)
				.withType(BpjsAccidentMasterEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsAccidentMasterEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsAccidentMasterEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Accident Master entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Accident Master entity and you do not have permission");
					}
