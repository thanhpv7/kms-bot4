...
import kmsweb.entities.InternalOrderStockDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Internal Order Stock Detail
 */
// % protected region % [Update the API route or description for the Internal Order Stock DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InternalOrderStockDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/internalorderstockdetail")
// % protected region % [Update the API route or description for the Internal Order Stock DetailController here] end
public class InternalOrderStockDetailController {

	private final InternalOrderStockDetailService internalOrderStockDetailService;
	private final CsvUtils<InternalOrderStockDetailEntity, InternalOrderStockDetailRepository, InternalOrderStockDetailEntityAudit,  InternalOrderStockDetailService> csvUtils;
	...
	@Autowired
	public InternalOrderStockDetailController(
		...
		this.csvUtils = new CsvUtils<>(internalOrderStockDetailService, InternalOrderStockDetailEntity.class);
	...
	/**
	 * Return all the Internal Order Stock Details.
	 *
	 * @return all the Internal Order Stock Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of InternalOrderStockDetails",
		security = {
	...
	public ResponseEntity<List<InternalOrderStockDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InternalOrderStockDetailEntity> internalOrderStockDetails = internalOrderStockDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(internalOrderStockDetails, HttpStatus.OK);
	}

	/**
	 * Return the Internal Order Stock Detail that has the same id as the given id.
	 *
	 * @param id      The id of the InternalOrderStockDetailEntity to be returned
	 * @return the Internal Order Stock Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InternalOrderStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InternalOrderStockDetail entity not found"),
	})
	@PreAuthorize("hasPermission('InternalOrderStockDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InternalOrderStockDetailEntity> getById(
			@Parameter(description = "The UUID of the InternalOrderStockDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InternalOrderStockDetailEntity> internalOrderStockDetailEntity = internalOrderStockDetailService.findById(id);
	...
	/**
	 * Create/Update the given Internal Order Stock Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param internalOrderStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInternalOrderStockDetail' endpoint here] off begin
	@Operation(
			description = "Create a InternalOrderStockDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InternalOrderStockDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InternalOrderStockDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('InternalOrderStockDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInternalOrderStockDetail' endpoint here] end
	public ResponseEntity<?> saveInternalOrderStockDetail(
			@Parameter(description = "The InternalOrderStockDetailEntity to create or update.")
			@RequestBody InternalOrderStockDetailEntityDto internalOrderStockDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InternalOrderStockDetailEntity internalOrderStockDetailEntity = internalOrderStockDetailService.create(new InternalOrderStockDetailEntity(internalOrderStockDetailEntityDto));
			return new ResponseEntity<>(internalOrderStockDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Internal Order Stock Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param internalOrderStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInternalOrderStockDetail' endpoint here] off begin
	@Operation(
			description = "Update a InternalOrderStockDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InternalOrderStockDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InternalOrderStockDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InternalOrderStockDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInternalOrderStockDetail' endpoint here] end
	public ResponseEntity<?> updateInternalOrderStockDetail(
			@Parameter(description = "The InternalOrderStockDetailEntity to create or update.")
			@RequestBody InternalOrderStockDetailEntityDto internalOrderStockDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InternalOrderStockDetailEntity> checkInternalOrderStockDetailExists = internalOrderStockDetailService.findById(internalOrderStockDetailEntityDto.getId());
		InternalOrderStockDetailEntity internalOrderStockDetailEntity = new InternalOrderStockDetailEntity(internalOrderStockDetailEntityDto);

		if (checkInternalOrderStockDetailExists.isPresent()) {
			try {
				internalOrderStockDetailEntity = internalOrderStockDetailService.update(internalOrderStockDetailEntity);
				return new ResponseEntity<>(internalOrderStockDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(internalOrderStockDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Internal Order Stock Detail that has the same id as the given id.
	 *
	 * @param id The id of the InternalOrderStockDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single InternalOrderStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InternalOrderStockDetail to delete.")
		...
		internalOrderStockDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInternalOrderStockDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Internal Order Stock Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInternalOrderStockDetail' endpoint here] end
	public void exportInternalOrderStockDetail(
			@Parameter(description = "The ids of the InternalOrderStockDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InternalOrderStockDetailEntity> entitiesToExport = internalOrderStockDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InternalOrderStockDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InternalOrderStockDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInternalOrderStockDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInternalOrderStockDetailExcludingIds' endpoint here] end
	public void exportInternalOrderStockDetailExcludingIds(
			@Parameter(description = "The ids of the InternalOrderStockDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InternalOrderStockDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = internalOrderStockDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = internalOrderStockDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InternalOrderStockDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InternalOrderStockDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InternalOrderStockDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Internal Order Stock Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InternalOrderStockDetailEntity', 'create')  || hasPermission('InternalOrderStockDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InternalOrderStockDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InternalOrderStockDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InternalOrderStockDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InternalOrderStockDetailEntity> csvToBean = new CsvToBeanBuilder<InternalOrderStockDetailEntity>(reader)
				.withType(InternalOrderStockDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InternalOrderStockDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InternalOrderStockDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Internal Order Stock Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Internal Order Stock Detail entity and you do not have permission");
					}
