...
/**
 * Controller used to handle all REST operations regarding Inventory Batch Correction
 */
// % protected region % [Update the API route or description for the Inventory Batch CorrectionController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InventoryBatchCorrectionController"))
@RestController
@Slf4j
@RequestMapping("/api/inventorybatchcorrection")
// % protected region % [Update the API route or description for the Inventory Batch CorrectionController here] end
public class InventoryBatchCorrectionController {

	private final InventoryBatchCorrectionService inventoryBatchCorrectionService;
	private final CsvUtils<InventoryBatchCorrectionEntity, InventoryBatchCorrectionRepository, InventoryBatchCorrectionEntityAudit,  InventoryBatchCorrectionService> csvUtils;
	...
	@Autowired
	public InventoryBatchCorrectionController(
		...
		this.csvUtils = new CsvUtils<>(inventoryBatchCorrectionService, InventoryBatchCorrectionEntity.class);
	...
	/**
	 * Return all the Inventory Batch Corrections.
	 *
	 * @return all the Inventory Batch Corrections
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of InventoryBatchCorrections",
		security = {
	...
	public ResponseEntity<List<InventoryBatchCorrectionEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InventoryBatchCorrectionEntity> inventoryBatchCorrections = inventoryBatchCorrectionService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(inventoryBatchCorrections, HttpStatus.OK);
	}

	/**
	 * Return the Inventory Batch Correction that has the same id as the given id.
	 *
	 * @param id      The id of the InventoryBatchCorrectionEntity to be returned
	 * @return the Inventory Batch Correction that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InventoryBatchCorrection as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InventoryBatchCorrection entity not found"),
	})
	@PreAuthorize("hasPermission('InventoryBatchCorrectionEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InventoryBatchCorrectionEntity> getById(
			@Parameter(description = "The UUID of the InventoryBatchCorrection to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InventoryBatchCorrectionEntity> inventoryBatchCorrectionEntity = inventoryBatchCorrectionService.findById(id);
	...
	/**
	 * Create/Update the given Inventory Batch Correction. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryBatchCorrectionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInventoryBatchCorrection' endpoint here] off begin
	@Operation(
			description = "Create a InventoryBatchCorrection if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InventoryBatchCorrection."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InventoryBatchCorrection, duplicate record found")
	})
	@PreAuthorize("hasPermission('InventoryBatchCorrectionEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInventoryBatchCorrection' endpoint here] end
	public ResponseEntity<?> saveInventoryBatchCorrection(
			@Parameter(description = "The InventoryBatchCorrectionEntity to create or update.")
			@RequestBody InventoryBatchCorrectionEntityDto inventoryBatchCorrectionEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InventoryBatchCorrectionEntity inventoryBatchCorrectionEntity = inventoryBatchCorrectionService.create(new InventoryBatchCorrectionEntity(inventoryBatchCorrectionEntityDto));
			return new ResponseEntity<>(inventoryBatchCorrectionEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Inventory Batch Correction. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryBatchCorrectionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInventoryBatchCorrection' endpoint here] off begin
	@Operation(
			description = "Update a InventoryBatchCorrection entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InventoryBatchCorrection."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InventoryBatchCorrection, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InventoryBatchCorrectionEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInventoryBatchCorrection' endpoint here] end
	public ResponseEntity<?> updateInventoryBatchCorrection(
			@Parameter(description = "The InventoryBatchCorrectionEntity to create or update.")
			@RequestBody InventoryBatchCorrectionEntityDto inventoryBatchCorrectionEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InventoryBatchCorrectionEntity> checkInventoryBatchCorrectionExists = inventoryBatchCorrectionService.findById(inventoryBatchCorrectionEntityDto.getId());
		InventoryBatchCorrectionEntity inventoryBatchCorrectionEntity = new InventoryBatchCorrectionEntity(inventoryBatchCorrectionEntityDto);

		if (checkInventoryBatchCorrectionExists.isPresent()) {
			try {
				inventoryBatchCorrectionEntity = inventoryBatchCorrectionService.update(inventoryBatchCorrectionEntity);
				return new ResponseEntity<>(inventoryBatchCorrectionEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(inventoryBatchCorrectionEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Inventory Batch Correction that has the same id as the given id.
	 *
	 * @param id The id of the InventoryBatchCorrectionEntity to be deleted
	...
	@Operation(
			description = "Delete a single InventoryBatchCorrection as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InventoryBatchCorrection to delete.")
		...
		inventoryBatchCorrectionService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrection' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Inventory Batch Correction entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrection' endpoint here] end
	public void exportInventoryBatchCorrection(
			@Parameter(description = "The ids of the InventoryBatchCorrectionEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InventoryBatchCorrectionEntity> entitiesToExport = inventoryBatchCorrectionService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InventoryBatchCorrection_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InventoryBatchCorrectionEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrectionExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrectionExcludingIds' endpoint here] end
	public void exportInventoryBatchCorrectionExcludingIds(
			@Parameter(description = "The ids of the InventoryBatchCorrectionEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InventoryBatchCorrectionEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = inventoryBatchCorrectionService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = inventoryBatchCorrectionService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InventoryBatchCorrection entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InventoryBatchCorrection_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InventoryBatchCorrectionEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Inventory Batch Correction entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InventoryBatchCorrectionEntity', 'create')  || hasPermission('InventoryBatchCorrectionEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InventoryBatchCorrectionEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InventoryBatchCorrectionEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InventoryBatchCorrectionEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InventoryBatchCorrectionEntity> csvToBean = new CsvToBeanBuilder<InventoryBatchCorrectionEntity>(reader)
				.withType(InventoryBatchCorrectionEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InventoryBatchCorrectionEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InventoryBatchCorrectionEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Inventory Batch Correction entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Inventory Batch Correction entity and you do not have permission");
					}
