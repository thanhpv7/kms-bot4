...
import kmsweb.entities.InvoicePaymentSelfPayingEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Invoice Payment Self Paying
 */
// % protected region % [Update the API route or description for the Invoice Payment Self PayingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InvoicePaymentSelfPayingController"))
@RestController
@Slf4j
@RequestMapping("/api/invoicepaymentselfpaying")
// % protected region % [Update the API route or description for the Invoice Payment Self PayingController here] end
public class InvoicePaymentSelfPayingController {

	private final InvoicePaymentSelfPayingService invoicePaymentSelfPayingService;
	private final CsvUtils<InvoicePaymentSelfPayingEntity, InvoicePaymentSelfPayingRepository, InvoicePaymentSelfPayingEntityAudit,  InvoicePaymentSelfPayingService> csvUtils;
	...
	@Autowired
	public InvoicePaymentSelfPayingController(
		...
		this.csvUtils = new CsvUtils<>(invoicePaymentSelfPayingService, InvoicePaymentSelfPayingEntity.class);
	...
	/**
	 * Return all the Invoice Payment Self Payings.
	 *
	 * @return all the Invoice Payment Self Payings
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of InvoicePaymentSelfPayings",
		security = {
	...
	public ResponseEntity<List<InvoicePaymentSelfPayingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InvoicePaymentSelfPayingEntity> invoicePaymentSelfPayings = invoicePaymentSelfPayingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(invoicePaymentSelfPayings, HttpStatus.OK);
	}

	/**
	 * Return the Invoice Payment Self Paying that has the same id as the given id.
	 *
	 * @param id      The id of the InvoicePaymentSelfPayingEntity to be returned
	 * @return the Invoice Payment Self Paying that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InvoicePaymentSelfPaying as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InvoicePaymentSelfPaying entity not found"),
	})
	@PreAuthorize("hasPermission('InvoicePaymentSelfPayingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InvoicePaymentSelfPayingEntity> getById(
			@Parameter(description = "The UUID of the InvoicePaymentSelfPaying to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InvoicePaymentSelfPayingEntity> invoicePaymentSelfPayingEntity = invoicePaymentSelfPayingService.findById(id);
	...
	/**
	 * Create/Update the given Invoice Payment Self Paying. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param invoicePaymentSelfPayingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInvoicePaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Create a InvoicePaymentSelfPaying if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InvoicePaymentSelfPaying."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InvoicePaymentSelfPaying, duplicate record found")
	})
	@PreAuthorize("hasPermission('InvoicePaymentSelfPayingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInvoicePaymentSelfPaying' endpoint here] end
	public ResponseEntity<?> saveInvoicePaymentSelfPaying(
			@Parameter(description = "The InvoicePaymentSelfPayingEntity to create or update.")
			@RequestBody InvoicePaymentSelfPayingEntityDto invoicePaymentSelfPayingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InvoicePaymentSelfPayingEntity invoicePaymentSelfPayingEntity = invoicePaymentSelfPayingService.create(new InvoicePaymentSelfPayingEntity(invoicePaymentSelfPayingEntityDto));
			return new ResponseEntity<>(invoicePaymentSelfPayingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Invoice Payment Self Paying. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param invoicePaymentSelfPayingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInvoicePaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Update a InvoicePaymentSelfPaying entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InvoicePaymentSelfPaying."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InvoicePaymentSelfPaying, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InvoicePaymentSelfPayingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInvoicePaymentSelfPaying' endpoint here] end
	public ResponseEntity<?> updateInvoicePaymentSelfPaying(
			@Parameter(description = "The InvoicePaymentSelfPayingEntity to create or update.")
			@RequestBody InvoicePaymentSelfPayingEntityDto invoicePaymentSelfPayingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InvoicePaymentSelfPayingEntity> checkInvoicePaymentSelfPayingExists = invoicePaymentSelfPayingService.findById(invoicePaymentSelfPayingEntityDto.getId());
		InvoicePaymentSelfPayingEntity invoicePaymentSelfPayingEntity = new InvoicePaymentSelfPayingEntity(invoicePaymentSelfPayingEntityDto);

		if (checkInvoicePaymentSelfPayingExists.isPresent()) {
			try {
				invoicePaymentSelfPayingEntity = invoicePaymentSelfPayingService.update(invoicePaymentSelfPayingEntity);
				return new ResponseEntity<>(invoicePaymentSelfPayingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(invoicePaymentSelfPayingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Invoice Payment Self Paying that has the same id as the given id.
	 *
	 * @param id The id of the InvoicePaymentSelfPayingEntity to be deleted
	...
	@Operation(
			description = "Delete a single InvoicePaymentSelfPaying as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InvoicePaymentSelfPaying to delete.")
		...
		invoicePaymentSelfPayingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Invoice Payment Self Paying entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentSelfPaying' endpoint here] end
	public void exportInvoicePaymentSelfPaying(
			@Parameter(description = "The ids of the InvoicePaymentSelfPayingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InvoicePaymentSelfPayingEntity> entitiesToExport = invoicePaymentSelfPayingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InvoicePaymentSelfPaying_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InvoicePaymentSelfPayingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentSelfPayingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentSelfPayingExcludingIds' endpoint here] end
	public void exportInvoicePaymentSelfPayingExcludingIds(
			@Parameter(description = "The ids of the InvoicePaymentSelfPayingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InvoicePaymentSelfPayingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = invoicePaymentSelfPayingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = invoicePaymentSelfPayingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InvoicePaymentSelfPaying entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InvoicePaymentSelfPaying_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InvoicePaymentSelfPayingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Invoice Payment Self Paying entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InvoicePaymentSelfPayingEntity', 'create')  || hasPermission('InvoicePaymentSelfPayingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InvoicePaymentSelfPayingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InvoicePaymentSelfPayingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InvoicePaymentSelfPayingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InvoicePaymentSelfPayingEntity> csvToBean = new CsvToBeanBuilder<InvoicePaymentSelfPayingEntity>(reader)
				.withType(InvoicePaymentSelfPayingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InvoicePaymentSelfPayingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InvoicePaymentSelfPayingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Invoice Payment Self Paying entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Invoice Payment Self Paying entity and you do not have permission");
					}
