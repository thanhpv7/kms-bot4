...
/**
 * Controller used to handle all REST operations regarding Patient Detail
 */
// % protected region % [Update the API route or description for the Patient DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/patientdetail")
// % protected region % [Update the API route or description for the Patient DetailController here] end
public class PatientDetailController {

	private final PatientDetailService patientDetailService;
	private final CsvUtils<PatientDetailEntity, PatientDetailRepository, PatientDetailEntityAudit,  PatientDetailService> csvUtils;
	...
	public PatientDetailController(
		...
		this.csvUtils = new CsvUtils<>(patientDetailService, PatientDetailEntity.class);
	...
	@Operation(
			description = "Returns a single page of PatientDetails",
		security = {
	...
	public ResponseEntity<List<PatientDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientDetailEntity> patientDetails = patientDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Patient Detail that has the same id as the given id.
	 *
	 * @param id      The id of the PatientDetailEntity to be returned
	 * @return the Patient Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientDetail entity not found"),
	})
	@PreAuthorize("hasPermission('PatientDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientDetailEntity> getById(
			@Parameter(description = "The UUID of the PatientDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientDetailEntity> patientDetailEntity = patientDetailService.findById(id);
	...
	/**
	 * Create/Update the given Patient Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientDetail' endpoint here] off begin
	@Operation(
			description = "Create a PatientDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientDetail' endpoint here] end
	public ResponseEntity<?> savePatientDetail(
			@Parameter(description = "The PatientDetailEntity to create or update.")
			@RequestBody PatientDetailEntityDto patientDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientDetailEntity patientDetailEntity = patientDetailService.create(new PatientDetailEntity(patientDetailEntityDto));
			return new ResponseEntity<>(patientDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientDetail' endpoint here] off begin
	@Operation(
			description = "Update a PatientDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientDetail' endpoint here] end
	public ResponseEntity<?> updatePatientDetail(
			@Parameter(description = "The PatientDetailEntity to create or update.")
			@RequestBody PatientDetailEntityDto patientDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientDetailEntity> checkPatientDetailExists = patientDetailService.findById(patientDetailEntityDto.getId());
		PatientDetailEntity patientDetailEntity = new PatientDetailEntity(patientDetailEntityDto);

		if (checkPatientDetailExists.isPresent()) {
			try {
				patientDetailEntity = patientDetailService.update(patientDetailEntity);
				return new ResponseEntity<>(patientDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Detail that has the same id as the given id.
	 *
	 * @param id The id of the PatientDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientDetail to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientDetail' endpoint here] end
	public void exportPatientDetail(
			@Parameter(description = "The ids of the PatientDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientDetailEntity> entitiesToExport = patientDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientDetailExcludingIds' endpoint here] end
	public void exportPatientDetailExcludingIds(
			@Parameter(description = "The ids of the PatientDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientDetailEntity', 'create')  || hasPermission('PatientDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientDetailEntity> csvToBean = new CsvToBeanBuilder<PatientDetailEntity>(reader)
				.withType(PatientDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Detail entity and you do not have permission");
					}
