...
/**
 * Controller used to handle all REST operations regarding Prescription Compound
 */
// % protected region % [Update the API route or description for the Prescription CompoundController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PrescriptionCompoundController"))
@RestController
@Slf4j
@RequestMapping("/api/prescriptioncompound")
// % protected region % [Update the API route or description for the Prescription CompoundController here] end
public class PrescriptionCompoundController {

	private final PrescriptionCompoundService prescriptionCompoundService;
	private final CsvUtils<PrescriptionCompoundEntity, PrescriptionCompoundRepository, PrescriptionCompoundEntityAudit,  PrescriptionCompoundService> csvUtils;
	...
	public PrescriptionCompoundController(
		...
		this.csvUtils = new CsvUtils<>(prescriptionCompoundService, PrescriptionCompoundEntity.class);
	...
	public ResponseEntity<List<PrescriptionCompoundEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PrescriptionCompoundEntity> prescriptionCompounds = prescriptionCompoundService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(prescriptionCompounds, HttpStatus.OK);
	}

	/**
	 * Return the Prescription Compound that has the same id as the given id.
	 *
	 * @param id      The id of the PrescriptionCompoundEntity to be returned
	 * @return the Prescription Compound that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PrescriptionCompound as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PrescriptionCompound entity not found"),
	})
	@PreAuthorize("hasPermission('PrescriptionCompoundEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PrescriptionCompoundEntity> getById(
			@Parameter(description = "The UUID of the PrescriptionCompound to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PrescriptionCompoundEntity> prescriptionCompoundEntity = prescriptionCompoundService.findById(id);
	...
	/**
	 * Create/Update the given Prescription Compound. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param prescriptionCompoundEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePrescriptionCompound' endpoint here] off begin
	@Operation(
			description = "Create a PrescriptionCompound if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PrescriptionCompound."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PrescriptionCompound, duplicate record found")
	})
	@PreAuthorize("hasPermission('PrescriptionCompoundEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePrescriptionCompound' endpoint here] end
	public ResponseEntity<?> savePrescriptionCompound(
			@Parameter(description = "The PrescriptionCompoundEntity to create or update.")
			@RequestBody PrescriptionCompoundEntityDto prescriptionCompoundEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PrescriptionCompoundEntity prescriptionCompoundEntity = prescriptionCompoundService.create(new PrescriptionCompoundEntity(prescriptionCompoundEntityDto));
			return new ResponseEntity<>(prescriptionCompoundEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Prescription Compound. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param prescriptionCompoundEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePrescriptionCompound' endpoint here] off begin
	@Operation(
			description = "Update a PrescriptionCompound entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PrescriptionCompound."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PrescriptionCompound, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PrescriptionCompoundEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePrescriptionCompound' endpoint here] end
	public ResponseEntity<?> updatePrescriptionCompound(
			@Parameter(description = "The PrescriptionCompoundEntity to create or update.")
			@RequestBody PrescriptionCompoundEntityDto prescriptionCompoundEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PrescriptionCompoundEntity> checkPrescriptionCompoundExists = prescriptionCompoundService.findById(prescriptionCompoundEntityDto.getId());
		PrescriptionCompoundEntity prescriptionCompoundEntity = new PrescriptionCompoundEntity(prescriptionCompoundEntityDto);

		if (checkPrescriptionCompoundExists.isPresent()) {
			try {
				prescriptionCompoundEntity = prescriptionCompoundService.update(prescriptionCompoundEntity);
				return new ResponseEntity<>(prescriptionCompoundEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(prescriptionCompoundEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Prescription Compound that has the same id as the given id.
	 *
	 * @param id The id of the PrescriptionCompoundEntity to be deleted
	...
	@Operation(
			description = "Delete a single PrescriptionCompound as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PrescriptionCompound to delete.")
		...
		prescriptionCompoundService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionCompound' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Prescription Compound entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionCompound' endpoint here] end
	public void exportPrescriptionCompound(
			@Parameter(description = "The ids of the PrescriptionCompoundEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PrescriptionCompoundEntity> entitiesToExport = prescriptionCompoundService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PrescriptionCompound_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PrescriptionCompoundEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionCompoundExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionCompoundExcludingIds' endpoint here] end
	public void exportPrescriptionCompoundExcludingIds(
			@Parameter(description = "The ids of the PrescriptionCompoundEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PrescriptionCompoundEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = prescriptionCompoundService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = prescriptionCompoundService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PrescriptionCompound entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PrescriptionCompound_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PrescriptionCompoundEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Prescription Compound entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PrescriptionCompoundEntity', 'create')  || hasPermission('PrescriptionCompoundEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PrescriptionCompoundEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PrescriptionCompoundEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PrescriptionCompoundEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PrescriptionCompoundEntity> csvToBean = new CsvToBeanBuilder<PrescriptionCompoundEntity>(reader)
				.withType(PrescriptionCompoundEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PrescriptionCompoundEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PrescriptionCompoundEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Prescription Compound entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Prescription Compound entity and you do not have permission");
					}
