...
import kmsweb.entities.TariffPackageServiceAssignmentEntityAudit;
import kmsweb.repositories.TariffPackageServiceAssignmentRepository;
import kmsweb.services.TariffPackageServiceAssignmentService;
...
/**
 * Controller used to handle all REST operations regarding Tariff Package Service Assignment
 */
// % protected region % [Update the API route or description for the Tariff Package Service AssignmentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TariffPackageServiceAssignmentController"))
@RestController
@Slf4j
@RequestMapping("/api/tariffpackageserviceassignment")
// % protected region % [Update the API route or description for the Tariff Package Service AssignmentController here] end
public class TariffPackageServiceAssignmentController {

	private final TariffPackageServiceAssignmentService tariffPackageServiceAssignmentService;
	private final CsvUtils<TariffPackageServiceAssignmentEntity, TariffPackageServiceAssignmentRepository, TariffPackageServiceAssignmentEntityAudit,  TariffPackageServiceAssignmentService> csvUtils;
	...
	@Autowired
	public TariffPackageServiceAssignmentController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			TariffPackageServiceAssignmentService tariffPackageServiceAssignmentService
	) {
		this.authService = authService;
		this.tariffPackageServiceAssignmentService = tariffPackageServiceAssignmentService;
		this.csvUtils = new CsvUtils<>(tariffPackageServiceAssignmentService, TariffPackageServiceAssignmentEntity.class);
	...
	/**
	 * Return all the Tariff Package Service Assignments.
	 *
	 * @return all the Tariff Package Service Assignments
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of TariffPackageServiceAssignments",
		security = {
	...
	public ResponseEntity<List<TariffPackageServiceAssignmentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TariffPackageServiceAssignmentEntity> tariffPackageServiceAssignments = tariffPackageServiceAssignmentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(tariffPackageServiceAssignments, HttpStatus.OK);
	}

	/**
	 * Return the Tariff Package Service Assignment that has the same id as the given id.
	 *
	 * @param id      The id of the TariffPackageServiceAssignmentEntity to be returned
	 * @return the Tariff Package Service Assignment that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TariffPackageServiceAssignment as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TariffPackageServiceAssignment entity not found"),
	})
	@PreAuthorize("hasPermission('TariffPackageServiceAssignmentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TariffPackageServiceAssignmentEntity> getById(
			@Parameter(description = "The UUID of the TariffPackageServiceAssignment to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TariffPackageServiceAssignmentEntity> tariffPackageServiceAssignmentEntity = tariffPackageServiceAssignmentService.findById(id);
	...
	/**
	 * Create/Update the given Tariff Package Service Assignment. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param tariffPackageServiceAssignmentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTariffPackageServiceAssignment' endpoint here] off begin
	@Operation(
			description = "Create a TariffPackageServiceAssignment if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TariffPackageServiceAssignment."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TariffPackageServiceAssignment, duplicate record found")
	})
	@PreAuthorize("hasPermission('TariffPackageServiceAssignmentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTariffPackageServiceAssignment' endpoint here] end
	public ResponseEntity<?> saveTariffPackageServiceAssignment(
			@Parameter(description = "The TariffPackageServiceAssignmentEntity to create or update.")
			@RequestBody TariffPackageServiceAssignmentEntityDto tariffPackageServiceAssignmentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TariffPackageServiceAssignmentEntity tariffPackageServiceAssignmentEntity = tariffPackageServiceAssignmentService.create(new TariffPackageServiceAssignmentEntity(tariffPackageServiceAssignmentEntityDto));
			return new ResponseEntity<>(tariffPackageServiceAssignmentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Tariff Package Service Assignment. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param tariffPackageServiceAssignmentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTariffPackageServiceAssignment' endpoint here] off begin
	@Operation(
			description = "Update a TariffPackageServiceAssignment entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TariffPackageServiceAssignment."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TariffPackageServiceAssignment, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TariffPackageServiceAssignmentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTariffPackageServiceAssignment' endpoint here] end
	public ResponseEntity<?> updateTariffPackageServiceAssignment(
			@Parameter(description = "The TariffPackageServiceAssignmentEntity to create or update.")
			@RequestBody TariffPackageServiceAssignmentEntityDto tariffPackageServiceAssignmentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TariffPackageServiceAssignmentEntity> checkTariffPackageServiceAssignmentExists = tariffPackageServiceAssignmentService.findById(tariffPackageServiceAssignmentEntityDto.getId());
		TariffPackageServiceAssignmentEntity tariffPackageServiceAssignmentEntity = new TariffPackageServiceAssignmentEntity(tariffPackageServiceAssignmentEntityDto);

		if (checkTariffPackageServiceAssignmentExists.isPresent()) {
			try {
				tariffPackageServiceAssignmentEntity = tariffPackageServiceAssignmentService.update(tariffPackageServiceAssignmentEntity);
				return new ResponseEntity<>(tariffPackageServiceAssignmentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(tariffPackageServiceAssignmentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Tariff Package Service Assignment that has the same id as the given id.
	 *
	 * @param id The id of the TariffPackageServiceAssignmentEntity to be deleted
	...
	@Operation(
			description = "Delete a single TariffPackageServiceAssignment as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TariffPackageServiceAssignment to delete.")
		...
		tariffPackageServiceAssignmentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageServiceAssignment' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Tariff Package Service Assignment entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageServiceAssignment' endpoint here] end
	public void exportTariffPackageServiceAssignment(
			@Parameter(description = "The ids of the TariffPackageServiceAssignmentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TariffPackageServiceAssignmentEntity> entitiesToExport = tariffPackageServiceAssignmentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TariffPackageServiceAssignment_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TariffPackageServiceAssignmentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageServiceAssignmentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageServiceAssignmentExcludingIds' endpoint here] end
	public void exportTariffPackageServiceAssignmentExcludingIds(
			@Parameter(description = "The ids of the TariffPackageServiceAssignmentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TariffPackageServiceAssignmentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = tariffPackageServiceAssignmentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = tariffPackageServiceAssignmentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TariffPackageServiceAssignment entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TariffPackageServiceAssignment_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TariffPackageServiceAssignmentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Tariff Package Service Assignment entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TariffPackageServiceAssignmentEntity', 'create')  || hasPermission('TariffPackageServiceAssignmentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TariffPackageServiceAssignmentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TariffPackageServiceAssignmentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TariffPackageServiceAssignmentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TariffPackageServiceAssignmentEntity> csvToBean = new CsvToBeanBuilder<TariffPackageServiceAssignmentEntity>(reader)
				.withType(TariffPackageServiceAssignmentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TariffPackageServiceAssignmentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TariffPackageServiceAssignmentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Tariff Package Service Assignment entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Tariff Package Service Assignment entity and you do not have permission");
					}
