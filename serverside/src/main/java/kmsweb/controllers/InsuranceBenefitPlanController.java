...
import kmsweb.entities.InsuranceBenefitPlanEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Insurance Benefit Plan
 */
// % protected region % [Update the API route or description for the Insurance Benefit PlanController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InsuranceBenefitPlanController"))
@RestController
@Slf4j
@RequestMapping("/api/insurancebenefitplan")
// % protected region % [Update the API route or description for the Insurance Benefit PlanController here] end
public class InsuranceBenefitPlanController {

	private final InsuranceBenefitPlanService insuranceBenefitPlanService;
	private final CsvUtils<InsuranceBenefitPlanEntity, InsuranceBenefitPlanRepository, InsuranceBenefitPlanEntityAudit,  InsuranceBenefitPlanService> csvUtils;
	...
	@Autowired
	public InsuranceBenefitPlanController(
		...
		this.csvUtils = new CsvUtils<>(insuranceBenefitPlanService, InsuranceBenefitPlanEntity.class);
	...
	public ResponseEntity<List<InsuranceBenefitPlanEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InsuranceBenefitPlanEntity> insuranceBenefitPlans = insuranceBenefitPlanService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(insuranceBenefitPlans, HttpStatus.OK);
	}

	/**
	 * Return the Insurance Benefit Plan that has the same id as the given id.
	 *
	 * @param id      The id of the InsuranceBenefitPlanEntity to be returned
	 * @return the Insurance Benefit Plan that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InsuranceBenefitPlan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InsuranceBenefitPlan entity not found"),
	})
	@PreAuthorize("hasPermission('InsuranceBenefitPlanEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InsuranceBenefitPlanEntity> getById(
			@Parameter(description = "The UUID of the InsuranceBenefitPlan to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InsuranceBenefitPlanEntity> insuranceBenefitPlanEntity = insuranceBenefitPlanService.findById(id);
	...
	/**
	 * Create/Update the given Insurance Benefit Plan. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param insuranceBenefitPlanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInsuranceBenefitPlan' endpoint here] off begin
	@Operation(
			description = "Create a InsuranceBenefitPlan if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InsuranceBenefitPlan."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InsuranceBenefitPlan, duplicate record found")
	})
	@PreAuthorize("hasPermission('InsuranceBenefitPlanEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInsuranceBenefitPlan' endpoint here] end
	public ResponseEntity<?> saveInsuranceBenefitPlan(
			@Parameter(description = "The InsuranceBenefitPlanEntity to create or update.")
			@RequestBody InsuranceBenefitPlanEntityDto insuranceBenefitPlanEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InsuranceBenefitPlanEntity insuranceBenefitPlanEntity = insuranceBenefitPlanService.create(new InsuranceBenefitPlanEntity(insuranceBenefitPlanEntityDto));
			return new ResponseEntity<>(insuranceBenefitPlanEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Insurance Benefit Plan. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param insuranceBenefitPlanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInsuranceBenefitPlan' endpoint here] off begin
	@Operation(
			description = "Update a InsuranceBenefitPlan entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InsuranceBenefitPlan."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InsuranceBenefitPlan, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InsuranceBenefitPlanEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInsuranceBenefitPlan' endpoint here] end
	public ResponseEntity<?> updateInsuranceBenefitPlan(
			@Parameter(description = "The InsuranceBenefitPlanEntity to create or update.")
			@RequestBody InsuranceBenefitPlanEntityDto insuranceBenefitPlanEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InsuranceBenefitPlanEntity> checkInsuranceBenefitPlanExists = insuranceBenefitPlanService.findById(insuranceBenefitPlanEntityDto.getId());
		InsuranceBenefitPlanEntity insuranceBenefitPlanEntity = new InsuranceBenefitPlanEntity(insuranceBenefitPlanEntityDto);

		if (checkInsuranceBenefitPlanExists.isPresent()) {
			try {
				insuranceBenefitPlanEntity = insuranceBenefitPlanService.update(insuranceBenefitPlanEntity);
				return new ResponseEntity<>(insuranceBenefitPlanEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(insuranceBenefitPlanEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Insurance Benefit Plan that has the same id as the given id.
	 *
	 * @param id The id of the InsuranceBenefitPlanEntity to be deleted
	...
	@Operation(
			description = "Delete a single InsuranceBenefitPlan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InsuranceBenefitPlan to delete.")
		...
		insuranceBenefitPlanService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInsuranceBenefitPlan' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Insurance Benefit Plan entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInsuranceBenefitPlan' endpoint here] end
	public void exportInsuranceBenefitPlan(
			@Parameter(description = "The ids of the InsuranceBenefitPlanEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InsuranceBenefitPlanEntity> entitiesToExport = insuranceBenefitPlanService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InsuranceBenefitPlan_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InsuranceBenefitPlanEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInsuranceBenefitPlanExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInsuranceBenefitPlanExcludingIds' endpoint here] end
	public void exportInsuranceBenefitPlanExcludingIds(
			@Parameter(description = "The ids of the InsuranceBenefitPlanEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InsuranceBenefitPlanEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = insuranceBenefitPlanService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = insuranceBenefitPlanService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InsuranceBenefitPlan entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InsuranceBenefitPlan_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InsuranceBenefitPlanEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Insurance Benefit Plan entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InsuranceBenefitPlanEntity', 'create')  || hasPermission('InsuranceBenefitPlanEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InsuranceBenefitPlanEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InsuranceBenefitPlanEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InsuranceBenefitPlanEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InsuranceBenefitPlanEntity> csvToBean = new CsvToBeanBuilder<InsuranceBenefitPlanEntity>(reader)
				.withType(InsuranceBenefitPlanEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InsuranceBenefitPlanEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InsuranceBenefitPlanEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Insurance Benefit Plan entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Insurance Benefit Plan entity and you do not have permission");
					}
