...
/**
 * Controller used to handle all REST operations regarding Formulir Konseling Tes Hiv
 */
// % protected region % [Update the API route or description for the Formulir Konseling Tes HivController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "FormulirKonselingTesHivController"))
@RestController
@Slf4j
@RequestMapping("/api/formulirkonselingteshiv")
// % protected region % [Update the API route or description for the Formulir Konseling Tes HivController here] end
public class FormulirKonselingTesHivController {

	private final FormulirKonselingTesHivService formulirKonselingTesHivService;
	private final CsvUtils<FormulirKonselingTesHivEntity, FormulirKonselingTesHivRepository, FormulirKonselingTesHivEntityAudit,  FormulirKonselingTesHivService> csvUtils;
	...
	@Autowired
	public FormulirKonselingTesHivController(
		...
		this.csvUtils = new CsvUtils<>(formulirKonselingTesHivService, FormulirKonselingTesHivEntity.class);
	...
	/**
	 * Return all the Formulir Konseling Tes Hivs.
	 *
	 * @return all the Formulir Konseling Tes Hivs
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of FormulirKonselingTesHivs",
		security = {
	...
	public ResponseEntity<List<FormulirKonselingTesHivEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<FormulirKonselingTesHivEntity> formulirKonselingTesHivs = formulirKonselingTesHivService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(formulirKonselingTesHivs, HttpStatus.OK);
	}

	/**
	 * Return the Formulir Konseling Tes Hiv that has the same id as the given id.
	 *
	 * @param id      The id of the FormulirKonselingTesHivEntity to be returned
	 * @return the Formulir Konseling Tes Hiv that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single FormulirKonselingTesHiv as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "FormulirKonselingTesHiv entity not found"),
	})
	@PreAuthorize("hasPermission('FormulirKonselingTesHivEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<FormulirKonselingTesHivEntity> getById(
			@Parameter(description = "The UUID of the FormulirKonselingTesHiv to return.")
			@PathVariable("id") UUID id
	) {
		Optional<FormulirKonselingTesHivEntity> formulirKonselingTesHivEntity = formulirKonselingTesHivService.findById(id);
	...
	/**
	 * Create/Update the given Formulir Konseling Tes Hiv. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param formulirKonselingTesHivEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveFormulirKonselingTesHiv' endpoint here] off begin
	@Operation(
			description = "Create a FormulirKonselingTesHiv if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new FormulirKonselingTesHiv."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new FormulirKonselingTesHiv, duplicate record found")
	})
	@PreAuthorize("hasPermission('FormulirKonselingTesHivEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveFormulirKonselingTesHiv' endpoint here] end
	public ResponseEntity<?> saveFormulirKonselingTesHiv(
			@Parameter(description = "The FormulirKonselingTesHivEntity to create or update.")
			@RequestBody FormulirKonselingTesHivEntityDto formulirKonselingTesHivEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			FormulirKonselingTesHivEntity formulirKonselingTesHivEntity = formulirKonselingTesHivService.create(new FormulirKonselingTesHivEntity(formulirKonselingTesHivEntityDto));
			return new ResponseEntity<>(formulirKonselingTesHivEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Formulir Konseling Tes Hiv. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param formulirKonselingTesHivEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateFormulirKonselingTesHiv' endpoint here] off begin
	@Operation(
			description = "Update a FormulirKonselingTesHiv entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the FormulirKonselingTesHiv."),
		@ApiResponse(responseCode = "400", description = "Failed to update the FormulirKonselingTesHiv, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('FormulirKonselingTesHivEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateFormulirKonselingTesHiv' endpoint here] end
	public ResponseEntity<?> updateFormulirKonselingTesHiv(
			@Parameter(description = "The FormulirKonselingTesHivEntity to create or update.")
			@RequestBody FormulirKonselingTesHivEntityDto formulirKonselingTesHivEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<FormulirKonselingTesHivEntity> checkFormulirKonselingTesHivExists = formulirKonselingTesHivService.findById(formulirKonselingTesHivEntityDto.getId());
		FormulirKonselingTesHivEntity formulirKonselingTesHivEntity = new FormulirKonselingTesHivEntity(formulirKonselingTesHivEntityDto);

		if (checkFormulirKonselingTesHivExists.isPresent()) {
			try {
				formulirKonselingTesHivEntity = formulirKonselingTesHivService.update(formulirKonselingTesHivEntity);
				return new ResponseEntity<>(formulirKonselingTesHivEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(formulirKonselingTesHivEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Formulir Konseling Tes Hiv that has the same id as the given id.
	 *
	 * @param id The id of the FormulirKonselingTesHivEntity to be deleted
	...
	@Operation(
			description = "Delete a single FormulirKonselingTesHiv as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the FormulirKonselingTesHiv to delete.")
		...
		formulirKonselingTesHivService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportFormulirKonselingTesHiv' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Formulir Konseling Tes Hiv entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportFormulirKonselingTesHiv' endpoint here] end
	public void exportFormulirKonselingTesHiv(
			@Parameter(description = "The ids of the FormulirKonselingTesHivEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<FormulirKonselingTesHivEntity> entitiesToExport = formulirKonselingTesHivService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "FormulirKonselingTesHiv_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (FormulirKonselingTesHivEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportFormulirKonselingTesHivExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportFormulirKonselingTesHivExcludingIds' endpoint here] end
	public void exportFormulirKonselingTesHivExcludingIds(
			@Parameter(description = "The ids of the FormulirKonselingTesHivEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<FormulirKonselingTesHivEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = formulirKonselingTesHivService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = formulirKonselingTesHivService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the FormulirKonselingTesHiv entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "FormulirKonselingTesHiv_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(FormulirKonselingTesHivEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Formulir Konseling Tes Hiv entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('FormulirKonselingTesHivEntity', 'create')  || hasPermission('FormulirKonselingTesHivEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with FormulirKonselingTesHivEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("FormulirKonselingTesHivEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(FormulirKonselingTesHivEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<FormulirKonselingTesHivEntity> csvToBean = new CsvToBeanBuilder<FormulirKonselingTesHivEntity>(reader)
				.withType(FormulirKonselingTesHivEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<FormulirKonselingTesHivEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<FormulirKonselingTesHivEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Formulir Konseling Tes Hiv entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Formulir Konseling Tes Hiv entity and you do not have permission");
					}
