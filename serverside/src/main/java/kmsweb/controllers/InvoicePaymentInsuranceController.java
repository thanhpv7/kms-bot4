...
import kmsweb.entities.InvoicePaymentInsuranceEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Invoice Payment Insurance
 */
// % protected region % [Update the API route or description for the Invoice Payment InsuranceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InvoicePaymentInsuranceController"))
@RestController
@Slf4j
@RequestMapping("/api/invoicepaymentinsurance")
// % protected region % [Update the API route or description for the Invoice Payment InsuranceController here] end
public class InvoicePaymentInsuranceController {

	private final InvoicePaymentInsuranceService invoicePaymentInsuranceService;
	private final CsvUtils<InvoicePaymentInsuranceEntity, InvoicePaymentInsuranceRepository, InvoicePaymentInsuranceEntityAudit,  InvoicePaymentInsuranceService> csvUtils;
	...
	@Autowired
	public InvoicePaymentInsuranceController(
		...
		this.csvUtils = new CsvUtils<>(invoicePaymentInsuranceService, InvoicePaymentInsuranceEntity.class);
	...
	public ResponseEntity<List<InvoicePaymentInsuranceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InvoicePaymentInsuranceEntity> invoicePaymentInsurances = invoicePaymentInsuranceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(invoicePaymentInsurances, HttpStatus.OK);
	}

	/**
	 * Return the Invoice Payment Insurance that has the same id as the given id.
	 *
	 * @param id      The id of the InvoicePaymentInsuranceEntity to be returned
	 * @return the Invoice Payment Insurance that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InvoicePaymentInsurance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InvoicePaymentInsurance entity not found"),
	})
	@PreAuthorize("hasPermission('InvoicePaymentInsuranceEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InvoicePaymentInsuranceEntity> getById(
			@Parameter(description = "The UUID of the InvoicePaymentInsurance to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InvoicePaymentInsuranceEntity> invoicePaymentInsuranceEntity = invoicePaymentInsuranceService.findById(id);
	...
	/**
	 * Create/Update the given Invoice Payment Insurance. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param invoicePaymentInsuranceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInvoicePaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Create a InvoicePaymentInsurance if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InvoicePaymentInsurance."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InvoicePaymentInsurance, duplicate record found")
	})
	@PreAuthorize("hasPermission('InvoicePaymentInsuranceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInvoicePaymentInsurance' endpoint here] end
	public ResponseEntity<?> saveInvoicePaymentInsurance(
			@Parameter(description = "The InvoicePaymentInsuranceEntity to create or update.")
			@RequestBody InvoicePaymentInsuranceEntityDto invoicePaymentInsuranceEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InvoicePaymentInsuranceEntity invoicePaymentInsuranceEntity = invoicePaymentInsuranceService.create(new InvoicePaymentInsuranceEntity(invoicePaymentInsuranceEntityDto));
			return new ResponseEntity<>(invoicePaymentInsuranceEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Invoice Payment Insurance. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param invoicePaymentInsuranceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInvoicePaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Update a InvoicePaymentInsurance entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InvoicePaymentInsurance."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InvoicePaymentInsurance, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InvoicePaymentInsuranceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInvoicePaymentInsurance' endpoint here] end
	public ResponseEntity<?> updateInvoicePaymentInsurance(
			@Parameter(description = "The InvoicePaymentInsuranceEntity to create or update.")
			@RequestBody InvoicePaymentInsuranceEntityDto invoicePaymentInsuranceEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InvoicePaymentInsuranceEntity> checkInvoicePaymentInsuranceExists = invoicePaymentInsuranceService.findById(invoicePaymentInsuranceEntityDto.getId());
		InvoicePaymentInsuranceEntity invoicePaymentInsuranceEntity = new InvoicePaymentInsuranceEntity(invoicePaymentInsuranceEntityDto);

		if (checkInvoicePaymentInsuranceExists.isPresent()) {
			try {
				invoicePaymentInsuranceEntity = invoicePaymentInsuranceService.update(invoicePaymentInsuranceEntity);
				return new ResponseEntity<>(invoicePaymentInsuranceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(invoicePaymentInsuranceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Invoice Payment Insurance that has the same id as the given id.
	 *
	 * @param id The id of the InvoicePaymentInsuranceEntity to be deleted
	...
	@Operation(
			description = "Delete a single InvoicePaymentInsurance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InvoicePaymentInsurance to delete.")
		...
		invoicePaymentInsuranceService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Invoice Payment Insurance entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentInsurance' endpoint here] end
	public void exportInvoicePaymentInsurance(
			@Parameter(description = "The ids of the InvoicePaymentInsuranceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InvoicePaymentInsuranceEntity> entitiesToExport = invoicePaymentInsuranceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InvoicePaymentInsurance_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InvoicePaymentInsuranceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentInsuranceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentInsuranceExcludingIds' endpoint here] end
	public void exportInvoicePaymentInsuranceExcludingIds(
			@Parameter(description = "The ids of the InvoicePaymentInsuranceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InvoicePaymentInsuranceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = invoicePaymentInsuranceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = invoicePaymentInsuranceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InvoicePaymentInsurance entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InvoicePaymentInsurance_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InvoicePaymentInsuranceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Invoice Payment Insurance entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InvoicePaymentInsuranceEntity', 'create')  || hasPermission('InvoicePaymentInsuranceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InvoicePaymentInsuranceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InvoicePaymentInsuranceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InvoicePaymentInsuranceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InvoicePaymentInsuranceEntity> csvToBean = new CsvToBeanBuilder<InvoicePaymentInsuranceEntity>(reader)
				.withType(InvoicePaymentInsuranceEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InvoicePaymentInsuranceEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InvoicePaymentInsuranceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Invoice Payment Insurance entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Invoice Payment Insurance entity and you do not have permission");
					}
