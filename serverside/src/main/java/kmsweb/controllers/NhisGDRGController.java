...
import kmsweb.entities.NhisGDRGEntityAudit;
import kmsweb.repositories.NhisGDRGRepository;
import kmsweb.services.NhisGDRGService;
...
/**
 * Controller used to handle all REST operations regarding NHIS GDRG
 */
// % protected region % [Update the API route or description for the NHIS GDRGController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisGDRGController"))
@RestController
@Slf4j
@RequestMapping("/api/nhisgdrg")
// % protected region % [Update the API route or description for the NHIS GDRGController here] end
public class NhisGDRGController {

	private final NhisGDRGService nhisGDRGService;
	private final CsvUtils<NhisGDRGEntity, NhisGDRGRepository, NhisGDRGEntityAudit,  NhisGDRGService> csvUtils;
	...
	@Autowired
	public NhisGDRGController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NhisGDRGService nhisGDRGService
	) {
		this.authService = authService;
		this.nhisGDRGService = nhisGDRGService;
		this.csvUtils = new CsvUtils<>(nhisGDRGService, NhisGDRGEntity.class);
	...
	public ResponseEntity<List<NhisGDRGEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisGDRGEntity> nhisGDRGs = nhisGDRGService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisGDRGs, HttpStatus.OK);
	}

	/**
	 * Return the NHIS GDRG that has the same id as the given id.
	 *
	 * @param id      The id of the NhisGDRGEntity to be returned
	 * @return the NHIS GDRG that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisGDRG as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisGDRG entity not found"),
	})
	@PreAuthorize("hasPermission('NhisGDRGEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisGDRGEntity> getById(
			@Parameter(description = "The UUID of the NhisGDRG to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisGDRGEntity> nhisGDRGEntity = nhisGDRGService.findById(id);
	...
	/**
	 * Create/Update the given NHIS GDRG. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisGDRGEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisGDRG' endpoint here] off begin
	@Operation(
			description = "Create a NhisGDRG if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisGDRG."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisGDRG, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisGDRGEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisGDRG' endpoint here] end
	public ResponseEntity<?> saveNhisGDRG(
			@Parameter(description = "The NhisGDRGEntity to create or update.")
			@RequestBody NhisGDRGEntityDto nhisGDRGEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisGDRGEntity nhisGDRGEntity = nhisGDRGService.create(new NhisGDRGEntity(nhisGDRGEntityDto));
			return new ResponseEntity<>(nhisGDRGEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS GDRG. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisGDRGEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisGDRG' endpoint here] off begin
	@Operation(
			description = "Update a NhisGDRG entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisGDRG."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisGDRG, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisGDRGEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisGDRG' endpoint here] end
	public ResponseEntity<?> updateNhisGDRG(
			@Parameter(description = "The NhisGDRGEntity to create or update.")
			@RequestBody NhisGDRGEntityDto nhisGDRGEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisGDRGEntity> checkNhisGDRGExists = nhisGDRGService.findById(nhisGDRGEntityDto.getId());
		NhisGDRGEntity nhisGDRGEntity = new NhisGDRGEntity(nhisGDRGEntityDto);

		if (checkNhisGDRGExists.isPresent()) {
			try {
				nhisGDRGEntity = nhisGDRGService.update(nhisGDRGEntity);
				return new ResponseEntity<>(nhisGDRGEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisGDRGEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS GDRG that has the same id as the given id.
	 *
	 * @param id The id of the NhisGDRGEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisGDRG as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisGDRG to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisGDRG' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS GDRG entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisGDRG' endpoint here] end
	public void exportNhisGDRG(
			@Parameter(description = "The ids of the NhisGDRGEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisGDRGEntity> entitiesToExport = nhisGDRGService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisGDRG_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisGDRGEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisGDRGExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisGDRGExcludingIds' endpoint here] end
	public void exportNhisGDRGExcludingIds(
			@Parameter(description = "The ids of the NhisGDRGEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisGDRGEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisGDRGService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisGDRGService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisGDRG entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisGDRG_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisGDRGEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS GDRG entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisGDRGEntity', 'create')  || hasPermission('NhisGDRGEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisGDRGEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisGDRGEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisGDRGEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisGDRGEntity> csvToBean = new CsvToBeanBuilder<NhisGDRGEntity>(reader)
				.withType(NhisGDRGEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisGDRGEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisGDRGEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS GDRG entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS GDRG entity and you do not have permission");
					}
