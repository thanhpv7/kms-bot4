...
import kmsweb.entities.StockTransactionPerDateEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Stock Transaction Per Date
 */
// % protected region % [Update the API route or description for the Stock Transaction Per DateController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "StockTransactionPerDateController"))
@RestController
@Slf4j
@RequestMapping("/api/stocktransactionperdate")
// % protected region % [Update the API route or description for the Stock Transaction Per DateController here] end
public class StockTransactionPerDateController {

	private final StockTransactionPerDateService stockTransactionPerDateService;
	private final CsvUtils<StockTransactionPerDateEntity, StockTransactionPerDateRepository, StockTransactionPerDateEntityAudit,  StockTransactionPerDateService> csvUtils;
	...
	@Autowired
	public StockTransactionPerDateController(
		...
		this.csvUtils = new CsvUtils<>(stockTransactionPerDateService, StockTransactionPerDateEntity.class);
	...
	/**
	 * Return all the Stock Transaction Per Dates.
	 *
	 * @return all the Stock Transaction Per Dates
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of StockTransactionPerDates",
		security = {
	...
	public ResponseEntity<List<StockTransactionPerDateEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<StockTransactionPerDateEntity> stockTransactionPerDates = stockTransactionPerDateService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(stockTransactionPerDates, HttpStatus.OK);
	}

	/**
	 * Return the Stock Transaction Per Date that has the same id as the given id.
	 *
	 * @param id      The id of the StockTransactionPerDateEntity to be returned
	 * @return the Stock Transaction Per Date that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single StockTransactionPerDate as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "StockTransactionPerDate entity not found"),
	})
	@PreAuthorize("hasPermission('StockTransactionPerDateEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<StockTransactionPerDateEntity> getById(
			@Parameter(description = "The UUID of the StockTransactionPerDate to return.")
			@PathVariable("id") UUID id
	) {
		Optional<StockTransactionPerDateEntity> stockTransactionPerDateEntity = stockTransactionPerDateService.findById(id);
	...
	/**
	 * Create/Update the given Stock Transaction Per Date. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param stockTransactionPerDateEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveStockTransactionPerDate' endpoint here] off begin
	@Operation(
			description = "Create a StockTransactionPerDate if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new StockTransactionPerDate."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new StockTransactionPerDate, duplicate record found")
	})
	@PreAuthorize("hasPermission('StockTransactionPerDateEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveStockTransactionPerDate' endpoint here] end
	public ResponseEntity<?> saveStockTransactionPerDate(
			@Parameter(description = "The StockTransactionPerDateEntity to create or update.")
			@RequestBody StockTransactionPerDateEntityDto stockTransactionPerDateEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			StockTransactionPerDateEntity stockTransactionPerDateEntity = stockTransactionPerDateService.create(new StockTransactionPerDateEntity(stockTransactionPerDateEntityDto));
			return new ResponseEntity<>(stockTransactionPerDateEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Stock Transaction Per Date. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param stockTransactionPerDateEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateStockTransactionPerDate' endpoint here] off begin
	@Operation(
			description = "Update a StockTransactionPerDate entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the StockTransactionPerDate."),
		@ApiResponse(responseCode = "400", description = "Failed to update the StockTransactionPerDate, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('StockTransactionPerDateEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateStockTransactionPerDate' endpoint here] end
	public ResponseEntity<?> updateStockTransactionPerDate(
			@Parameter(description = "The StockTransactionPerDateEntity to create or update.")
			@RequestBody StockTransactionPerDateEntityDto stockTransactionPerDateEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<StockTransactionPerDateEntity> checkStockTransactionPerDateExists = stockTransactionPerDateService.findById(stockTransactionPerDateEntityDto.getId());
		StockTransactionPerDateEntity stockTransactionPerDateEntity = new StockTransactionPerDateEntity(stockTransactionPerDateEntityDto);

		if (checkStockTransactionPerDateExists.isPresent()) {
			try {
				stockTransactionPerDateEntity = stockTransactionPerDateService.update(stockTransactionPerDateEntity);
				return new ResponseEntity<>(stockTransactionPerDateEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(stockTransactionPerDateEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Stock Transaction Per Date that has the same id as the given id.
	 *
	 * @param id The id of the StockTransactionPerDateEntity to be deleted
	...
	@Operation(
			description = "Delete a single StockTransactionPerDate as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the StockTransactionPerDate to delete.")
		...
		stockTransactionPerDateService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDate' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Stock Transaction Per Date entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDate' endpoint here] end
	public void exportStockTransactionPerDate(
			@Parameter(description = "The ids of the StockTransactionPerDateEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<StockTransactionPerDateEntity> entitiesToExport = stockTransactionPerDateService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "StockTransactionPerDate_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (StockTransactionPerDateEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDateExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDateExcludingIds' endpoint here] end
	public void exportStockTransactionPerDateExcludingIds(
			@Parameter(description = "The ids of the StockTransactionPerDateEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<StockTransactionPerDateEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = stockTransactionPerDateService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = stockTransactionPerDateService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the StockTransactionPerDate entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "StockTransactionPerDate_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(StockTransactionPerDateEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Stock Transaction Per Date entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('StockTransactionPerDateEntity', 'create')  || hasPermission('StockTransactionPerDateEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with StockTransactionPerDateEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("StockTransactionPerDateEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(StockTransactionPerDateEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<StockTransactionPerDateEntity> csvToBean = new CsvToBeanBuilder<StockTransactionPerDateEntity>(reader)
				.withType(StockTransactionPerDateEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<StockTransactionPerDateEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<StockTransactionPerDateEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Stock Transaction Per Date entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Stock Transaction Per Date entity and you do not have permission");
					}
