...
import kmsweb.entities.BpjsVisitDataEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Visit Data
 */
// % protected region % [Update the API route or description for the BPJS Visit DataController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsVisitDataController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsvisitdata")
// % protected region % [Update the API route or description for the BPJS Visit DataController here] end
public class BpjsVisitDataController {

	private final BpjsVisitDataService bpjsVisitDataService;
	private final CsvUtils<BpjsVisitDataEntity, BpjsVisitDataRepository, BpjsVisitDataEntityAudit,  BpjsVisitDataService> csvUtils;
	...
	@Autowired
	public BpjsVisitDataController(
		...
		this.csvUtils = new CsvUtils<>(bpjsVisitDataService, BpjsVisitDataEntity.class);
	...
	public ResponseEntity<List<BpjsVisitDataEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsVisitDataEntity> bpjsVisitDatas = bpjsVisitDataService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsVisitDatas, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Visit Data that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsVisitDataEntity to be returned
	 * @return the BPJS Visit Data that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsVisitData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsVisitData entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsVisitDataEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsVisitDataEntity> getById(
			@Parameter(description = "The UUID of the BpjsVisitData to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsVisitDataEntity> bpjsVisitDataEntity = bpjsVisitDataService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Visit Data. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsVisitDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsVisitData' endpoint here] off begin
	@Operation(
			description = "Create a BpjsVisitData if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsVisitData."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsVisitData, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsVisitDataEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsVisitData' endpoint here] end
	public ResponseEntity<?> saveBpjsVisitData(
			@Parameter(description = "The BpjsVisitDataEntity to create or update.")
			@RequestBody BpjsVisitDataEntityDto bpjsVisitDataEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsVisitDataEntity bpjsVisitDataEntity = bpjsVisitDataService.create(new BpjsVisitDataEntity(bpjsVisitDataEntityDto));
			return new ResponseEntity<>(bpjsVisitDataEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Visit Data. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsVisitDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsVisitData' endpoint here] off begin
	@Operation(
			description = "Update a BpjsVisitData entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsVisitData."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsVisitData, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsVisitDataEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsVisitData' endpoint here] end
	public ResponseEntity<?> updateBpjsVisitData(
			@Parameter(description = "The BpjsVisitDataEntity to create or update.")
			@RequestBody BpjsVisitDataEntityDto bpjsVisitDataEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsVisitDataEntity> checkBpjsVisitDataExists = bpjsVisitDataService.findById(bpjsVisitDataEntityDto.getId());
		BpjsVisitDataEntity bpjsVisitDataEntity = new BpjsVisitDataEntity(bpjsVisitDataEntityDto);

		if (checkBpjsVisitDataExists.isPresent()) {
			try {
				bpjsVisitDataEntity = bpjsVisitDataService.update(bpjsVisitDataEntity);
				return new ResponseEntity<>(bpjsVisitDataEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsVisitDataEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Visit Data that has the same id as the given id.
	 *
	 * @param id The id of the BpjsVisitDataEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsVisitData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsVisitData to delete.")
		...
		bpjsVisitDataService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsVisitData' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Visit Data entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsVisitData' endpoint here] end
	public void exportBpjsVisitData(
			@Parameter(description = "The ids of the BpjsVisitDataEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsVisitDataEntity> entitiesToExport = bpjsVisitDataService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsVisitData_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsVisitDataEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsVisitDataExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsVisitDataExcludingIds' endpoint here] end
	public void exportBpjsVisitDataExcludingIds(
			@Parameter(description = "The ids of the BpjsVisitDataEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsVisitDataEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsVisitDataService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsVisitDataService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsVisitData entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsVisitData_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsVisitDataEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Visit Data entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsVisitDataEntity', 'create')  || hasPermission('BpjsVisitDataEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsVisitDataEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsVisitDataEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsVisitDataEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsVisitDataEntity> csvToBean = new CsvToBeanBuilder<BpjsVisitDataEntity>(reader)
				.withType(BpjsVisitDataEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsVisitDataEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsVisitDataEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Visit Data entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Visit Data entity and you do not have permission");
					}
