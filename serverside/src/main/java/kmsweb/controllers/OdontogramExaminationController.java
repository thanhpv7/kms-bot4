...
import kmsweb.entities.OdontogramExaminationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Odontogram Examination
 */
// % protected region % [Update the API route or description for the Odontogram ExaminationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "OdontogramExaminationController"))
@RestController
@Slf4j
@RequestMapping("/api/odontogramexamination")
// % protected region % [Update the API route or description for the Odontogram ExaminationController here] end
public class OdontogramExaminationController {

	private final OdontogramExaminationService odontogramExaminationService;
	private final CsvUtils<OdontogramExaminationEntity, OdontogramExaminationRepository, OdontogramExaminationEntityAudit,  OdontogramExaminationService> csvUtils;
	...
	@Autowired
	public OdontogramExaminationController(
		...
		this.csvUtils = new CsvUtils<>(odontogramExaminationService, OdontogramExaminationEntity.class);
	...
	public ResponseEntity<List<OdontogramExaminationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<OdontogramExaminationEntity> odontogramExaminations = odontogramExaminationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(odontogramExaminations, HttpStatus.OK);
	}

	/**
	 * Return the Odontogram Examination that has the same id as the given id.
	 *
	 * @param id      The id of the OdontogramExaminationEntity to be returned
	 * @return the Odontogram Examination that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single OdontogramExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "OdontogramExamination entity not found"),
	})
	@PreAuthorize("hasPermission('OdontogramExaminationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<OdontogramExaminationEntity> getById(
			@Parameter(description = "The UUID of the OdontogramExamination to return.")
			@PathVariable("id") UUID id
	) {
		Optional<OdontogramExaminationEntity> odontogramExaminationEntity = odontogramExaminationService.findById(id);
	...
	/**
	 * Create/Update the given Odontogram Examination. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param odontogramExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveOdontogramExamination' endpoint here] off begin
	@Operation(
			description = "Create a OdontogramExamination if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new OdontogramExamination."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new OdontogramExamination, duplicate record found")
	})
	@PreAuthorize("hasPermission('OdontogramExaminationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveOdontogramExamination' endpoint here] end
	public ResponseEntity<?> saveOdontogramExamination(
			@Parameter(description = "The OdontogramExaminationEntity to create or update.")
			@RequestBody OdontogramExaminationEntityDto odontogramExaminationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			OdontogramExaminationEntity odontogramExaminationEntity = odontogramExaminationService.create(new OdontogramExaminationEntity(odontogramExaminationEntityDto));
			return new ResponseEntity<>(odontogramExaminationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Odontogram Examination. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param odontogramExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateOdontogramExamination' endpoint here] off begin
	@Operation(
			description = "Update a OdontogramExamination entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the OdontogramExamination."),
		@ApiResponse(responseCode = "400", description = "Failed to update the OdontogramExamination, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('OdontogramExaminationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateOdontogramExamination' endpoint here] end
	public ResponseEntity<?> updateOdontogramExamination(
			@Parameter(description = "The OdontogramExaminationEntity to create or update.")
			@RequestBody OdontogramExaminationEntityDto odontogramExaminationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<OdontogramExaminationEntity> checkOdontogramExaminationExists = odontogramExaminationService.findById(odontogramExaminationEntityDto.getId());
		OdontogramExaminationEntity odontogramExaminationEntity = new OdontogramExaminationEntity(odontogramExaminationEntityDto);

		if (checkOdontogramExaminationExists.isPresent()) {
			try {
				odontogramExaminationEntity = odontogramExaminationService.update(odontogramExaminationEntity);
				return new ResponseEntity<>(odontogramExaminationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(odontogramExaminationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Odontogram Examination that has the same id as the given id.
	 *
	 * @param id The id of the OdontogramExaminationEntity to be deleted
	...
	@Operation(
			description = "Delete a single OdontogramExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the OdontogramExamination to delete.")
		...
		odontogramExaminationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportOdontogramExamination' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Odontogram Examination entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportOdontogramExamination' endpoint here] end
	public void exportOdontogramExamination(
			@Parameter(description = "The ids of the OdontogramExaminationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<OdontogramExaminationEntity> entitiesToExport = odontogramExaminationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "OdontogramExamination_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (OdontogramExaminationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportOdontogramExaminationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportOdontogramExaminationExcludingIds' endpoint here] end
	public void exportOdontogramExaminationExcludingIds(
			@Parameter(description = "The ids of the OdontogramExaminationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<OdontogramExaminationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = odontogramExaminationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = odontogramExaminationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the OdontogramExamination entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "OdontogramExamination_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(OdontogramExaminationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Odontogram Examination entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('OdontogramExaminationEntity', 'create')  || hasPermission('OdontogramExaminationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with OdontogramExaminationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("OdontogramExaminationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(OdontogramExaminationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<OdontogramExaminationEntity> csvToBean = new CsvToBeanBuilder<OdontogramExaminationEntity>(reader)
				.withType(OdontogramExaminationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<OdontogramExaminationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<OdontogramExaminationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Odontogram Examination entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Odontogram Examination entity and you do not have permission");
					}
