...
import kmsweb.entities.RoleEntityAudit;
 ...
 */
// % protected region % [Update the API route or description for the RoleController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RoleController"))
@RestController
@Slf4j
@RequestMapping("/api/role")
// % protected region % [Update the API route or description for the RoleController here] end
public class RoleController {

	private final RoleService roleService;
	private final CsvUtils<RoleEntity, RoleRepository, RoleEntityAudit,  RoleService> csvUtils;
	...
	public RoleController(
		...
		this.csvUtils = new CsvUtils<>(roleService, RoleEntity.class);
	...
	@Operation(
			description = "Returns a single page of Roles",
		security = {
	...
	@Operation(
			description = "Return a single Role as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Role entity not found"),
	})
	@PreAuthorize("hasPermission('RoleEntity', 'read')")
	...
	public ResponseEntity<RoleEntity> getById(
			@Parameter(description = "The UUID of the Role to return.")
	 ...
	 *
	 * @param roleEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRole' endpoint here] off begin
	@Operation(
			description = "Create a Role if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "409", description = "Failed to create the new Role, duplicate record found")
	})
	@PreAuthorize("hasPermission('RoleEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRole' endpoint here] end
	public ResponseEntity<?> saveRole(
			@Parameter(description = "The RoleEntity to create or update.")
			@RequestBody RoleEntityDto roleEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RoleEntity roleEntity = roleService.create(new RoleEntity(roleEntityDto));
	...
	/**
	 * Update the given Role. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param roleEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRole' endpoint here] off begin
	@Operation(
			description = "Update a Role entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		...
		@ApiResponse(responseCode = "400", description = "Failed to update the Role, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RoleEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRole' endpoint here] end
	public ResponseEntity<?> updateRole(
		...
		Optional<RoleEntity> checkRoleExists = roleService.findById(roleEntityDto.getId());
		RoleEntity roleEntity = new RoleEntity(roleEntityDto);

		if (checkRoleExists.isPresent()) {
			try {
	...
	@Operation(
			description = "Delete a single Role as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Role to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRole' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Role entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRole' endpoint here] end
	public void exportRole(
			@Parameter(description = "The ids of the RoleEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RoleEntity> entitiesToExport = roleService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Role_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RoleEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRoleExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRoleExcludingIds' endpoint here] end
	public void exportRoleExcludingIds(
			@Parameter(description = "The ids of the RoleEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RoleEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = roleService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = roleService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Role entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Role_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RoleEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Role entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RoleEntity', 'create')  || hasPermission('RoleEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RoleEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RoleEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RoleEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RoleEntity> csvToBean = new CsvToBeanBuilder<RoleEntity>(reader)
				.withType(RoleEntity.class)
		...
		try {
			Iterator<RoleEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Role entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Role entity and you do not have permission");
					}
