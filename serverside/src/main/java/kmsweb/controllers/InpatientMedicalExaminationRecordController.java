...
import kmsweb.entities.InpatientMedicalExaminationRecordEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Inpatient Medical Examination Record
 */
// % protected region % [Update the API route or description for the Inpatient Medical Examination RecordController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InpatientMedicalExaminationRecordController"))
@RestController
@Slf4j
@RequestMapping("/api/inpatientmedicalexaminationrecord")
// % protected region % [Update the API route or description for the Inpatient Medical Examination RecordController here] end
public class InpatientMedicalExaminationRecordController {

	private final InpatientMedicalExaminationRecordService inpatientMedicalExaminationRecordService;
	private final CsvUtils<InpatientMedicalExaminationRecordEntity, InpatientMedicalExaminationRecordRepository, InpatientMedicalExaminationRecordEntityAudit,  InpatientMedicalExaminationRecordService> csvUtils;
	...
	@Autowired
	public InpatientMedicalExaminationRecordController(
		...
		this.csvUtils = new CsvUtils<>(inpatientMedicalExaminationRecordService, InpatientMedicalExaminationRecordEntity.class);
	...
	/**
	 * Return all the Inpatient Medical Examination Records.
	 *
	 * @return all the Inpatient Medical Examination Records
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of InpatientMedicalExaminationRecords",
		security = {
	...
	public ResponseEntity<List<InpatientMedicalExaminationRecordEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InpatientMedicalExaminationRecordEntity> inpatientMedicalExaminationRecords = inpatientMedicalExaminationRecordService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(inpatientMedicalExaminationRecords, HttpStatus.OK);
	}

	/**
	 * Return the Inpatient Medical Examination Record that has the same id as the given id.
	 *
	 * @param id      The id of the InpatientMedicalExaminationRecordEntity to be returned
	 * @return the Inpatient Medical Examination Record that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InpatientMedicalExaminationRecord as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InpatientMedicalExaminationRecord entity not found"),
	})
	@PreAuthorize("hasPermission('InpatientMedicalExaminationRecordEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InpatientMedicalExaminationRecordEntity> getById(
			@Parameter(description = "The UUID of the InpatientMedicalExaminationRecord to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InpatientMedicalExaminationRecordEntity> inpatientMedicalExaminationRecordEntity = inpatientMedicalExaminationRecordService.findById(id);
	...
	/**
	 * Create/Update the given Inpatient Medical Examination Record. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param inpatientMedicalExaminationRecordEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInpatientMedicalExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Create a InpatientMedicalExaminationRecord if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InpatientMedicalExaminationRecord."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InpatientMedicalExaminationRecord, duplicate record found")
	})
	@PreAuthorize("hasPermission('InpatientMedicalExaminationRecordEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInpatientMedicalExaminationRecord' endpoint here] end
	public ResponseEntity<?> saveInpatientMedicalExaminationRecord(
			@Parameter(description = "The InpatientMedicalExaminationRecordEntity to create or update.")
			@RequestBody InpatientMedicalExaminationRecordEntityDto inpatientMedicalExaminationRecordEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InpatientMedicalExaminationRecordEntity inpatientMedicalExaminationRecordEntity = inpatientMedicalExaminationRecordService.create(new InpatientMedicalExaminationRecordEntity(inpatientMedicalExaminationRecordEntityDto));
			return new ResponseEntity<>(inpatientMedicalExaminationRecordEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Inpatient Medical Examination Record. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param inpatientMedicalExaminationRecordEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInpatientMedicalExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Update a InpatientMedicalExaminationRecord entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InpatientMedicalExaminationRecord."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InpatientMedicalExaminationRecord, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InpatientMedicalExaminationRecordEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInpatientMedicalExaminationRecord' endpoint here] end
	public ResponseEntity<?> updateInpatientMedicalExaminationRecord(
			@Parameter(description = "The InpatientMedicalExaminationRecordEntity to create or update.")
			@RequestBody InpatientMedicalExaminationRecordEntityDto inpatientMedicalExaminationRecordEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InpatientMedicalExaminationRecordEntity> checkInpatientMedicalExaminationRecordExists = inpatientMedicalExaminationRecordService.findById(inpatientMedicalExaminationRecordEntityDto.getId());
		InpatientMedicalExaminationRecordEntity inpatientMedicalExaminationRecordEntity = new InpatientMedicalExaminationRecordEntity(inpatientMedicalExaminationRecordEntityDto);

		if (checkInpatientMedicalExaminationRecordExists.isPresent()) {
			try {
				inpatientMedicalExaminationRecordEntity = inpatientMedicalExaminationRecordService.update(inpatientMedicalExaminationRecordEntity);
				return new ResponseEntity<>(inpatientMedicalExaminationRecordEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(inpatientMedicalExaminationRecordEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Inpatient Medical Examination Record that has the same id as the given id.
	 *
	 * @param id The id of the InpatientMedicalExaminationRecordEntity to be deleted
	...
	@Operation(
			description = "Delete a single InpatientMedicalExaminationRecord as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InpatientMedicalExaminationRecord to delete.")
		...
		inpatientMedicalExaminationRecordService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInpatientMedicalExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Inpatient Medical Examination Record entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInpatientMedicalExaminationRecord' endpoint here] end
	public void exportInpatientMedicalExaminationRecord(
			@Parameter(description = "The ids of the InpatientMedicalExaminationRecordEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InpatientMedicalExaminationRecordEntity> entitiesToExport = inpatientMedicalExaminationRecordService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InpatientMedicalExaminationRecord_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InpatientMedicalExaminationRecordEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInpatientMedicalExaminationRecordExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInpatientMedicalExaminationRecordExcludingIds' endpoint here] end
	public void exportInpatientMedicalExaminationRecordExcludingIds(
			@Parameter(description = "The ids of the InpatientMedicalExaminationRecordEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InpatientMedicalExaminationRecordEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = inpatientMedicalExaminationRecordService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = inpatientMedicalExaminationRecordService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InpatientMedicalExaminationRecord entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InpatientMedicalExaminationRecord_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InpatientMedicalExaminationRecordEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Inpatient Medical Examination Record entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InpatientMedicalExaminationRecordEntity', 'create')  || hasPermission('InpatientMedicalExaminationRecordEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InpatientMedicalExaminationRecordEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InpatientMedicalExaminationRecordEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InpatientMedicalExaminationRecordEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InpatientMedicalExaminationRecordEntity> csvToBean = new CsvToBeanBuilder<InpatientMedicalExaminationRecordEntity>(reader)
				.withType(InpatientMedicalExaminationRecordEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InpatientMedicalExaminationRecordEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InpatientMedicalExaminationRecordEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Inpatient Medical Examination Record entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Inpatient Medical Examination Record entity and you do not have permission");
					}
