...
/**
 * Controller used to handle all REST operations regarding Body Chart Examination
 */
// % protected region % [Update the API route or description for the Body Chart ExaminationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BodyChartExaminationController"))
@RestController
@Slf4j
@RequestMapping("/api/bodychartexamination")
// % protected region % [Update the API route or description for the Body Chart ExaminationController here] end
public class BodyChartExaminationController {

	private final BodyChartExaminationService bodyChartExaminationService;
	private final CsvUtils<BodyChartExaminationEntity, BodyChartExaminationRepository, BodyChartExaminationEntityAudit,  BodyChartExaminationService> csvUtils;
	...
	@Autowired
	public BodyChartExaminationController(
		...
		this.csvUtils = new CsvUtils<>(bodyChartExaminationService, BodyChartExaminationEntity.class);
	...
	public ResponseEntity<List<BodyChartExaminationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BodyChartExaminationEntity> bodyChartExaminations = bodyChartExaminationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bodyChartExaminations, HttpStatus.OK);
	}

	/**
	 * Return the Body Chart Examination that has the same id as the given id.
	 *
	 * @param id      The id of the BodyChartExaminationEntity to be returned
	 * @return the Body Chart Examination that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BodyChartExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BodyChartExamination entity not found"),
	})
	@PreAuthorize("hasPermission('BodyChartExaminationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BodyChartExaminationEntity> getById(
			@Parameter(description = "The UUID of the BodyChartExamination to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BodyChartExaminationEntity> bodyChartExaminationEntity = bodyChartExaminationService.findById(id);
	...
	/**
	 * Create/Update the given Body Chart Examination. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bodyChartExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBodyChartExamination' endpoint here] off begin
	@Operation(
			description = "Create a BodyChartExamination if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BodyChartExamination."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BodyChartExamination, duplicate record found")
	})
	@PreAuthorize("hasPermission('BodyChartExaminationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBodyChartExamination' endpoint here] end
	public ResponseEntity<?> saveBodyChartExamination(
			@Parameter(description = "The BodyChartExaminationEntity to create or update.")
			@RequestBody BodyChartExaminationEntityDto bodyChartExaminationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BodyChartExaminationEntity bodyChartExaminationEntity = bodyChartExaminationService.create(new BodyChartExaminationEntity(bodyChartExaminationEntityDto));
			return new ResponseEntity<>(bodyChartExaminationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Body Chart Examination. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bodyChartExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBodyChartExamination' endpoint here] off begin
	@Operation(
			description = "Update a BodyChartExamination entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BodyChartExamination."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BodyChartExamination, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BodyChartExaminationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBodyChartExamination' endpoint here] end
	public ResponseEntity<?> updateBodyChartExamination(
			@Parameter(description = "The BodyChartExaminationEntity to create or update.")
			@RequestBody BodyChartExaminationEntityDto bodyChartExaminationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BodyChartExaminationEntity> checkBodyChartExaminationExists = bodyChartExaminationService.findById(bodyChartExaminationEntityDto.getId());
		BodyChartExaminationEntity bodyChartExaminationEntity = new BodyChartExaminationEntity(bodyChartExaminationEntityDto);

		if (checkBodyChartExaminationExists.isPresent()) {
			try {
				bodyChartExaminationEntity = bodyChartExaminationService.update(bodyChartExaminationEntity);
				return new ResponseEntity<>(bodyChartExaminationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bodyChartExaminationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Body Chart Examination that has the same id as the given id.
	 *
	 * @param id The id of the BodyChartExaminationEntity to be deleted
	...
	@Operation(
			description = "Delete a single BodyChartExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BodyChartExamination to delete.")
		...
		bodyChartExaminationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBodyChartExamination' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Body Chart Examination entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBodyChartExamination' endpoint here] end
	public void exportBodyChartExamination(
			@Parameter(description = "The ids of the BodyChartExaminationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BodyChartExaminationEntity> entitiesToExport = bodyChartExaminationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BodyChartExamination_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BodyChartExaminationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBodyChartExaminationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBodyChartExaminationExcludingIds' endpoint here] end
	public void exportBodyChartExaminationExcludingIds(
			@Parameter(description = "The ids of the BodyChartExaminationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BodyChartExaminationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bodyChartExaminationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bodyChartExaminationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BodyChartExamination entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BodyChartExamination_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BodyChartExaminationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Body Chart Examination entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BodyChartExaminationEntity', 'create')  || hasPermission('BodyChartExaminationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BodyChartExaminationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BodyChartExaminationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BodyChartExaminationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BodyChartExaminationEntity> csvToBean = new CsvToBeanBuilder<BodyChartExaminationEntity>(reader)
				.withType(BodyChartExaminationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BodyChartExaminationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BodyChartExaminationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Body Chart Examination entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Body Chart Examination entity and you do not have permission");
					}
