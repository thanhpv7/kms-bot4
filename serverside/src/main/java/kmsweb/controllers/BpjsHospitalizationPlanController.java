...
import kmsweb.entities.BpjsHospitalizationPlanEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Hospitalization Plan
 */
// % protected region % [Update the API route or description for the BPJS Hospitalization PlanController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsHospitalizationPlanController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjshospitalizationplan")
// % protected region % [Update the API route or description for the BPJS Hospitalization PlanController here] end
public class BpjsHospitalizationPlanController {

	private final BpjsHospitalizationPlanService bpjsHospitalizationPlanService;
	private final CsvUtils<BpjsHospitalizationPlanEntity, BpjsHospitalizationPlanRepository, BpjsHospitalizationPlanEntityAudit,  BpjsHospitalizationPlanService> csvUtils;
	...
	@Autowired
	public BpjsHospitalizationPlanController(
		...
		this.csvUtils = new CsvUtils<>(bpjsHospitalizationPlanService, BpjsHospitalizationPlanEntity.class);
	...
	public ResponseEntity<List<BpjsHospitalizationPlanEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsHospitalizationPlanEntity> bpjsHospitalizationPlans = bpjsHospitalizationPlanService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsHospitalizationPlans, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Hospitalization Plan that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsHospitalizationPlanEntity to be returned
	 * @return the BPJS Hospitalization Plan that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsHospitalizationPlan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsHospitalizationPlan entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsHospitalizationPlanEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsHospitalizationPlanEntity> getById(
			@Parameter(description = "The UUID of the BpjsHospitalizationPlan to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsHospitalizationPlanEntity> bpjsHospitalizationPlanEntity = bpjsHospitalizationPlanService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Hospitalization Plan. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsHospitalizationPlanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsHospitalizationPlan' endpoint here] off begin
	@Operation(
			description = "Create a BpjsHospitalizationPlan if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsHospitalizationPlan."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsHospitalizationPlan, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsHospitalizationPlanEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsHospitalizationPlan' endpoint here] end
	public ResponseEntity<?> saveBpjsHospitalizationPlan(
			@Parameter(description = "The BpjsHospitalizationPlanEntity to create or update.")
			@RequestBody BpjsHospitalizationPlanEntityDto bpjsHospitalizationPlanEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsHospitalizationPlanEntity bpjsHospitalizationPlanEntity = bpjsHospitalizationPlanService.create(new BpjsHospitalizationPlanEntity(bpjsHospitalizationPlanEntityDto));
			return new ResponseEntity<>(bpjsHospitalizationPlanEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Hospitalization Plan. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsHospitalizationPlanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsHospitalizationPlan' endpoint here] off begin
	@Operation(
			description = "Update a BpjsHospitalizationPlan entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsHospitalizationPlan."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsHospitalizationPlan, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsHospitalizationPlanEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsHospitalizationPlan' endpoint here] end
	public ResponseEntity<?> updateBpjsHospitalizationPlan(
			@Parameter(description = "The BpjsHospitalizationPlanEntity to create or update.")
			@RequestBody BpjsHospitalizationPlanEntityDto bpjsHospitalizationPlanEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsHospitalizationPlanEntity> checkBpjsHospitalizationPlanExists = bpjsHospitalizationPlanService.findById(bpjsHospitalizationPlanEntityDto.getId());
		BpjsHospitalizationPlanEntity bpjsHospitalizationPlanEntity = new BpjsHospitalizationPlanEntity(bpjsHospitalizationPlanEntityDto);

		if (checkBpjsHospitalizationPlanExists.isPresent()) {
			try {
				bpjsHospitalizationPlanEntity = bpjsHospitalizationPlanService.update(bpjsHospitalizationPlanEntity);
				return new ResponseEntity<>(bpjsHospitalizationPlanEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsHospitalizationPlanEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Hospitalization Plan that has the same id as the given id.
	 *
	 * @param id The id of the BpjsHospitalizationPlanEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsHospitalizationPlan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsHospitalizationPlan to delete.")
		...
		bpjsHospitalizationPlanService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsHospitalizationPlan' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Hospitalization Plan entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsHospitalizationPlan' endpoint here] end
	public void exportBpjsHospitalizationPlan(
			@Parameter(description = "The ids of the BpjsHospitalizationPlanEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsHospitalizationPlanEntity> entitiesToExport = bpjsHospitalizationPlanService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsHospitalizationPlan_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsHospitalizationPlanEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsHospitalizationPlanExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsHospitalizationPlanExcludingIds' endpoint here] end
	public void exportBpjsHospitalizationPlanExcludingIds(
			@Parameter(description = "The ids of the BpjsHospitalizationPlanEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsHospitalizationPlanEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsHospitalizationPlanService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsHospitalizationPlanService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsHospitalizationPlan entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsHospitalizationPlan_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsHospitalizationPlanEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Hospitalization Plan entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsHospitalizationPlanEntity', 'create')  || hasPermission('BpjsHospitalizationPlanEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsHospitalizationPlanEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsHospitalizationPlanEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsHospitalizationPlanEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsHospitalizationPlanEntity> csvToBean = new CsvToBeanBuilder<BpjsHospitalizationPlanEntity>(reader)
				.withType(BpjsHospitalizationPlanEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsHospitalizationPlanEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsHospitalizationPlanEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Hospitalization Plan entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Hospitalization Plan entity and you do not have permission");
					}
