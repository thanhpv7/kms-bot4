...
/**
 * Controller used to handle all REST operations regarding BPJS Special Referral
 */
// % protected region % [Update the API route or description for the BPJS Special ReferralController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsSpecialReferralController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsspecialreferral")
// % protected region % [Update the API route or description for the BPJS Special ReferralController here] end
public class BpjsSpecialReferralController {

	private final BpjsSpecialReferralService bpjsSpecialReferralService;
	private final CsvUtils<BpjsSpecialReferralEntity, BpjsSpecialReferralRepository, BpjsSpecialReferralEntityAudit,  BpjsSpecialReferralService> csvUtils;
	...
	@Autowired
	public BpjsSpecialReferralController(
		...
		this.csvUtils = new CsvUtils<>(bpjsSpecialReferralService, BpjsSpecialReferralEntity.class);
	...
	public ResponseEntity<List<BpjsSpecialReferralEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsSpecialReferralEntity> bpjsSpecialReferrals = bpjsSpecialReferralService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsSpecialReferrals, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Special Referral that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsSpecialReferralEntity to be returned
	 * @return the BPJS Special Referral that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsSpecialReferral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsSpecialReferral entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsSpecialReferralEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsSpecialReferralEntity> getById(
			@Parameter(description = "The UUID of the BpjsSpecialReferral to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsSpecialReferralEntity> bpjsSpecialReferralEntity = bpjsSpecialReferralService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Special Referral. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSpecialReferralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsSpecialReferral' endpoint here] off begin
	@Operation(
			description = "Create a BpjsSpecialReferral if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsSpecialReferral."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsSpecialReferral, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsSpecialReferralEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsSpecialReferral' endpoint here] end
	public ResponseEntity<?> saveBpjsSpecialReferral(
			@Parameter(description = "The BpjsSpecialReferralEntity to create or update.")
			@RequestBody BpjsSpecialReferralEntityDto bpjsSpecialReferralEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsSpecialReferralEntity bpjsSpecialReferralEntity = bpjsSpecialReferralService.create(new BpjsSpecialReferralEntity(bpjsSpecialReferralEntityDto));
			return new ResponseEntity<>(bpjsSpecialReferralEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Special Referral. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSpecialReferralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsSpecialReferral' endpoint here] off begin
	@Operation(
			description = "Update a BpjsSpecialReferral entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsSpecialReferral."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsSpecialReferral, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsSpecialReferralEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsSpecialReferral' endpoint here] end
	public ResponseEntity<?> updateBpjsSpecialReferral(
			@Parameter(description = "The BpjsSpecialReferralEntity to create or update.")
			@RequestBody BpjsSpecialReferralEntityDto bpjsSpecialReferralEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsSpecialReferralEntity> checkBpjsSpecialReferralExists = bpjsSpecialReferralService.findById(bpjsSpecialReferralEntityDto.getId());
		BpjsSpecialReferralEntity bpjsSpecialReferralEntity = new BpjsSpecialReferralEntity(bpjsSpecialReferralEntityDto);

		if (checkBpjsSpecialReferralExists.isPresent()) {
			try {
				bpjsSpecialReferralEntity = bpjsSpecialReferralService.update(bpjsSpecialReferralEntity);
				return new ResponseEntity<>(bpjsSpecialReferralEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsSpecialReferralEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Special Referral that has the same id as the given id.
	 *
	 * @param id The id of the BpjsSpecialReferralEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsSpecialReferral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsSpecialReferral to delete.")
		...
		bpjsSpecialReferralService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSpecialReferral' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Special Referral entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSpecialReferral' endpoint here] end
	public void exportBpjsSpecialReferral(
			@Parameter(description = "The ids of the BpjsSpecialReferralEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsSpecialReferralEntity> entitiesToExport = bpjsSpecialReferralService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsSpecialReferral_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsSpecialReferralEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSpecialReferralExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSpecialReferralExcludingIds' endpoint here] end
	public void exportBpjsSpecialReferralExcludingIds(
			@Parameter(description = "The ids of the BpjsSpecialReferralEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsSpecialReferralEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsSpecialReferralService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsSpecialReferralService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsSpecialReferral entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsSpecialReferral_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsSpecialReferralEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Special Referral entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsSpecialReferralEntity', 'create')  || hasPermission('BpjsSpecialReferralEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsSpecialReferralEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsSpecialReferralEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsSpecialReferralEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsSpecialReferralEntity> csvToBean = new CsvToBeanBuilder<BpjsSpecialReferralEntity>(reader)
				.withType(BpjsSpecialReferralEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsSpecialReferralEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsSpecialReferralEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Special Referral entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Special Referral entity and you do not have permission");
					}
