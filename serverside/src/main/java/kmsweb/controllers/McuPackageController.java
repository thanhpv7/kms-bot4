...
import kmsweb.entities.McuPackageEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding MCU Package
 */
// % protected region % [Update the API route or description for the MCU PackageController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "McuPackageController"))
@RestController
@Slf4j
@RequestMapping("/api/mcupackage")
// % protected region % [Update the API route or description for the MCU PackageController here] end
public class McuPackageController {

	private final McuPackageService mcuPackageService;
	private final CsvUtils<McuPackageEntity, McuPackageRepository, McuPackageEntityAudit,  McuPackageService> csvUtils;
	...
	public McuPackageController(
		...
		this.csvUtils = new CsvUtils<>(mcuPackageService, McuPackageEntity.class);
	...
	public ResponseEntity<List<McuPackageEntity>> getAllWithPage(
		...
		return new ResponseEntity<>(mcuPackages, HttpStatus.OK);
	}

	/**
	 * Return the MCU Package that has the same id as the given id.
	 *
	 * @param id      The id of the McuPackageEntity to be returned
	 * @return the MCU Package that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single McuPackage as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "McuPackage entity not found"),
	})
	@PreAuthorize("hasPermission('McuPackageEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<McuPackageEntity> getById(
			@Parameter(description = "The UUID of the McuPackage to return.")
			@PathVariable("id") UUID id
	) {
		Optional<McuPackageEntity> mcuPackageEntity = mcuPackageService.findById(id);
	...
	/**
	 * Create/Update the given MCU Package. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param mcuPackageEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMcuPackage' endpoint here] off begin
	@Operation(
			description = "Create a McuPackage if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new McuPackage."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new McuPackage, duplicate record found")
	})
	@PreAuthorize("hasPermission('McuPackageEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMcuPackage' endpoint here] end
	public ResponseEntity<?> saveMcuPackage(
			@Parameter(description = "The McuPackageEntity to create or update.")
			@RequestBody McuPackageEntityDto mcuPackageEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			McuPackageEntity mcuPackageEntity = mcuPackageService.create(new McuPackageEntity(mcuPackageEntityDto));
			return new ResponseEntity<>(mcuPackageEntity, HttpStatus.OK);
	...
	/**
	 * Update the given MCU Package. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param mcuPackageEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMcuPackage' endpoint here] off begin
	@Operation(
			description = "Update a McuPackage entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the McuPackage."),
		@ApiResponse(responseCode = "400", description = "Failed to update the McuPackage, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('McuPackageEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMcuPackage' endpoint here] end
	public ResponseEntity<?> updateMcuPackage(
			@Parameter(description = "The McuPackageEntity to create or update.")
			@RequestBody McuPackageEntityDto mcuPackageEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<McuPackageEntity> checkMcuPackageExists = mcuPackageService.findById(mcuPackageEntityDto.getId());
		McuPackageEntity mcuPackageEntity = new McuPackageEntity(mcuPackageEntityDto);

		if (checkMcuPackageExists.isPresent()) {
			try {
				mcuPackageEntity = mcuPackageService.update(mcuPackageEntity);
				return new ResponseEntity<>(mcuPackageEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(mcuPackageEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the MCU Package that has the same id as the given id.
	 *
	 * @param id The id of the McuPackageEntity to be deleted
	...
	@Operation(
			description = "Delete a single McuPackage as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the McuPackage to delete.")
		...
		mcuPackageService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMcuPackage' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all MCU Package entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMcuPackage' endpoint here] end
	public void exportMcuPackage(
			@Parameter(description = "The ids of the McuPackageEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<McuPackageEntity> entitiesToExport = mcuPackageService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "McuPackage_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (McuPackageEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMcuPackageExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMcuPackageExcludingIds' endpoint here] end
	public void exportMcuPackageExcludingIds(
			@Parameter(description = "The ids of the McuPackageEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<McuPackageEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = mcuPackageService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = mcuPackageService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the McuPackage entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "McuPackage_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(McuPackageEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for MCU Package entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('McuPackageEntity', 'create')  || hasPermission('McuPackageEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with McuPackageEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("McuPackageEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(McuPackageEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<McuPackageEntity> csvToBean = new CsvToBeanBuilder<McuPackageEntity>(reader)
				.withType(McuPackageEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<McuPackageEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<McuPackageEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a MCU Package entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a MCU Package entity and you do not have permission");
					}
