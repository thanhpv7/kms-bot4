...
import kmsweb.entities.BedReserveEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Bed Reserve
 */
// % protected region % [Update the API route or description for the Bed ReserveController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BedReserveController"))
@RestController
@Slf4j
@RequestMapping("/api/bedreserve")
// % protected region % [Update the API route or description for the Bed ReserveController here] end
public class BedReserveController {

	private final BedReserveService bedReserveService;
	private final CsvUtils<BedReserveEntity, BedReserveRepository, BedReserveEntityAudit,  BedReserveService> csvUtils;
	...
	public BedReserveController(
		...
		this.csvUtils = new CsvUtils<>(bedReserveService, BedReserveEntity.class);
	...
	public ResponseEntity<List<BedReserveEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BedReserveEntity> bedReserves = bedReserveService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bedReserves, HttpStatus.OK);
	}

	/**
	 * Return the Bed Reserve that has the same id as the given id.
	 *
	 * @param id      The id of the BedReserveEntity to be returned
	 * @return the Bed Reserve that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BedReserve as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BedReserve entity not found"),
	})
	@PreAuthorize("hasPermission('BedReserveEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BedReserveEntity> getById(
			@Parameter(description = "The UUID of the BedReserve to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BedReserveEntity> bedReserveEntity = bedReserveService.findById(id);
	...
	/**
	 * Create/Update the given Bed Reserve. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bedReserveEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBedReserve' endpoint here] off begin
	@Operation(
			description = "Create a BedReserve if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BedReserve."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BedReserve, duplicate record found")
	})
	@PreAuthorize("hasPermission('BedReserveEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBedReserve' endpoint here] end
	public ResponseEntity<?> saveBedReserve(
			@Parameter(description = "The BedReserveEntity to create or update.")
			@RequestBody BedReserveEntityDto bedReserveEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BedReserveEntity bedReserveEntity = bedReserveService.create(new BedReserveEntity(bedReserveEntityDto));
			return new ResponseEntity<>(bedReserveEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Bed Reserve. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bedReserveEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBedReserve' endpoint here] off begin
	@Operation(
			description = "Update a BedReserve entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BedReserve."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BedReserve, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BedReserveEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBedReserve' endpoint here] end
	public ResponseEntity<?> updateBedReserve(
			@Parameter(description = "The BedReserveEntity to create or update.")
			@RequestBody BedReserveEntityDto bedReserveEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BedReserveEntity> checkBedReserveExists = bedReserveService.findById(bedReserveEntityDto.getId());
		BedReserveEntity bedReserveEntity = new BedReserveEntity(bedReserveEntityDto);

		if (checkBedReserveExists.isPresent()) {
			try {
				bedReserveEntity = bedReserveService.update(bedReserveEntity);
				return new ResponseEntity<>(bedReserveEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bedReserveEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Bed Reserve that has the same id as the given id.
	 *
	 * @param id The id of the BedReserveEntity to be deleted
	...
	@Operation(
			description = "Delete a single BedReserve as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BedReserve to delete.")
		...
		bedReserveService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBedReserve' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Bed Reserve entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBedReserve' endpoint here] end
	public void exportBedReserve(
			@Parameter(description = "The ids of the BedReserveEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BedReserveEntity> entitiesToExport = bedReserveService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BedReserve_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BedReserveEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBedReserveExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBedReserveExcludingIds' endpoint here] end
	public void exportBedReserveExcludingIds(
			@Parameter(description = "The ids of the BedReserveEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BedReserveEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bedReserveService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bedReserveService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BedReserve entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BedReserve_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BedReserveEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Bed Reserve entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BedReserveEntity', 'create')  || hasPermission('BedReserveEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BedReserveEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BedReserveEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BedReserveEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BedReserveEntity> csvToBean = new CsvToBeanBuilder<BedReserveEntity>(reader)
				.withType(BedReserveEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BedReserveEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BedReserveEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Bed Reserve entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Bed Reserve entity and you do not have permission");
					}
