...
import kmsweb.entities.BpjsDoctorMappingEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Doctor Mapping
 */
// % protected region % [Update the API route or description for the BPJS Doctor MappingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsDoctorMappingController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsdoctormapping")
// % protected region % [Update the API route or description for the BPJS Doctor MappingController here] end
public class BpjsDoctorMappingController {

	private final BpjsDoctorMappingService bpjsDoctorMappingService;
	private final CsvUtils<BpjsDoctorMappingEntity, BpjsDoctorMappingRepository, BpjsDoctorMappingEntityAudit,  BpjsDoctorMappingService> csvUtils;
	...
	@Autowired
	public BpjsDoctorMappingController(
		...
		this.csvUtils = new CsvUtils<>(bpjsDoctorMappingService, BpjsDoctorMappingEntity.class);
	...
	public ResponseEntity<List<BpjsDoctorMappingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsDoctorMappingEntity> bpjsDoctorMappings = bpjsDoctorMappingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsDoctorMappings, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Doctor Mapping that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsDoctorMappingEntity to be returned
	 * @return the BPJS Doctor Mapping that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsDoctorMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsDoctorMapping entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsDoctorMappingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsDoctorMappingEntity> getById(
			@Parameter(description = "The UUID of the BpjsDoctorMapping to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsDoctorMappingEntity> bpjsDoctorMappingEntity = bpjsDoctorMappingService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Doctor Mapping. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDoctorMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsDoctorMapping' endpoint here] off begin
	@Operation(
			description = "Create a BpjsDoctorMapping if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsDoctorMapping."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsDoctorMapping, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsDoctorMappingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsDoctorMapping' endpoint here] end
	public ResponseEntity<?> saveBpjsDoctorMapping(
			@Parameter(description = "The BpjsDoctorMappingEntity to create or update.")
			@RequestBody BpjsDoctorMappingEntityDto bpjsDoctorMappingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsDoctorMappingEntity bpjsDoctorMappingEntity = bpjsDoctorMappingService.create(new BpjsDoctorMappingEntity(bpjsDoctorMappingEntityDto));
			return new ResponseEntity<>(bpjsDoctorMappingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Doctor Mapping. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDoctorMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsDoctorMapping' endpoint here] off begin
	@Operation(
			description = "Update a BpjsDoctorMapping entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsDoctorMapping."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsDoctorMapping, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsDoctorMappingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsDoctorMapping' endpoint here] end
	public ResponseEntity<?> updateBpjsDoctorMapping(
			@Parameter(description = "The BpjsDoctorMappingEntity to create or update.")
			@RequestBody BpjsDoctorMappingEntityDto bpjsDoctorMappingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsDoctorMappingEntity> checkBpjsDoctorMappingExists = bpjsDoctorMappingService.findById(bpjsDoctorMappingEntityDto.getId());
		BpjsDoctorMappingEntity bpjsDoctorMappingEntity = new BpjsDoctorMappingEntity(bpjsDoctorMappingEntityDto);

		if (checkBpjsDoctorMappingExists.isPresent()) {
			try {
				bpjsDoctorMappingEntity = bpjsDoctorMappingService.update(bpjsDoctorMappingEntity);
				return new ResponseEntity<>(bpjsDoctorMappingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsDoctorMappingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Doctor Mapping that has the same id as the given id.
	 *
	 * @param id The id of the BpjsDoctorMappingEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsDoctorMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsDoctorMapping to delete.")
		...
		bpjsDoctorMappingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorMapping' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Doctor Mapping entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorMapping' endpoint here] end
	public void exportBpjsDoctorMapping(
			@Parameter(description = "The ids of the BpjsDoctorMappingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsDoctorMappingEntity> entitiesToExport = bpjsDoctorMappingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsDoctorMapping_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsDoctorMappingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorMappingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDoctorMappingExcludingIds' endpoint here] end
	public void exportBpjsDoctorMappingExcludingIds(
			@Parameter(description = "The ids of the BpjsDoctorMappingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsDoctorMappingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsDoctorMappingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsDoctorMappingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsDoctorMapping entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsDoctorMapping_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsDoctorMappingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Doctor Mapping entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsDoctorMappingEntity', 'create')  || hasPermission('BpjsDoctorMappingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsDoctorMappingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsDoctorMappingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsDoctorMappingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsDoctorMappingEntity> csvToBean = new CsvToBeanBuilder<BpjsDoctorMappingEntity>(reader)
				.withType(BpjsDoctorMappingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsDoctorMappingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsDoctorMappingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Doctor Mapping entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Doctor Mapping entity and you do not have permission");
					}
