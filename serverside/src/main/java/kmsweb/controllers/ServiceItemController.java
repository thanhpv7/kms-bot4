...
/**
 * Controller used to handle all REST operations regarding Service Item
 */
// % protected region % [Update the API route or description for the Service ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ServiceItemController"))
@RestController
@Slf4j
@RequestMapping("/api/serviceitem")
// % protected region % [Update the API route or description for the Service ItemController here] end
public class ServiceItemController {

	private final ServiceItemService serviceItemService;
	private final CsvUtils<ServiceItemEntity, ServiceItemRepository, ServiceItemEntityAudit,  ServiceItemService> csvUtils;
	...
	public ServiceItemController(
		...
		this.csvUtils = new CsvUtils<>(serviceItemService, ServiceItemEntity.class);
	...
	public ResponseEntity<List<ServiceItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ServiceItemEntity> serviceItems = serviceItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Service Item that has the same id as the given id.
	 *
	 * @param id      The id of the ServiceItemEntity to be returned
	 * @return the Service Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ServiceItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ServiceItem entity not found"),
	})
	@PreAuthorize("hasPermission('ServiceItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ServiceItemEntity> getById(
			@Parameter(description = "The UUID of the ServiceItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ServiceItemEntity> serviceItemEntity = serviceItemService.findById(id);
	...
	/**
	 * Create/Update the given Service Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param serviceItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveServiceItem' endpoint here] off begin
	@Operation(
			description = "Create a ServiceItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ServiceItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ServiceItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('ServiceItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveServiceItem' endpoint here] end
	public ResponseEntity<?> saveServiceItem(
			@Parameter(description = "The ServiceItemEntity to create or update.")
			@RequestBody ServiceItemEntityDto serviceItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ServiceItemEntity serviceItemEntity = serviceItemService.create(new ServiceItemEntity(serviceItemEntityDto));
			return new ResponseEntity<>(serviceItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Service Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param serviceItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateServiceItem' endpoint here] off begin
	@Operation(
			description = "Update a ServiceItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ServiceItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ServiceItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ServiceItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateServiceItem' endpoint here] end
	public ResponseEntity<?> updateServiceItem(
			@Parameter(description = "The ServiceItemEntity to create or update.")
			@RequestBody ServiceItemEntityDto serviceItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ServiceItemEntity> checkServiceItemExists = serviceItemService.findById(serviceItemEntityDto.getId());
		ServiceItemEntity serviceItemEntity = new ServiceItemEntity(serviceItemEntityDto);

		if (checkServiceItemExists.isPresent()) {
			try {
				serviceItemEntity = serviceItemService.update(serviceItemEntity);
				return new ResponseEntity<>(serviceItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(serviceItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Service Item that has the same id as the given id.
	 *
	 * @param id The id of the ServiceItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single ServiceItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ServiceItem to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportServiceItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Service Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportServiceItem' endpoint here] end
	public void exportServiceItem(
			@Parameter(description = "The ids of the ServiceItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ServiceItemEntity> entitiesToExport = serviceItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ServiceItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ServiceItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportServiceItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportServiceItemExcludingIds' endpoint here] end
	public void exportServiceItemExcludingIds(
			@Parameter(description = "The ids of the ServiceItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ServiceItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = serviceItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = serviceItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ServiceItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ServiceItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ServiceItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Service Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ServiceItemEntity', 'create')  || hasPermission('ServiceItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ServiceItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ServiceItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ServiceItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ServiceItemEntity> csvToBean = new CsvToBeanBuilder<ServiceItemEntity>(reader)
				.withType(ServiceItemEntity.class)
		...
		try {
			Iterator<ServiceItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Service Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Service Item entity and you do not have permission");
					}
