...
/**
 * Controller used to handle all REST operations regarding Patient Visit Payment Self Paying
 */
// % protected region % [Update the API route or description for the Patient Visit Payment Self PayingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientVisitPaymentSelfPayingController"))
@RestController
@Slf4j
@RequestMapping("/api/patientvisitpaymentselfpaying")
// % protected region % [Update the API route or description for the Patient Visit Payment Self PayingController here] end
public class PatientVisitPaymentSelfPayingController {

	private final PatientVisitPaymentSelfPayingService patientVisitPaymentSelfPayingService;
	private final CsvUtils<PatientVisitPaymentSelfPayingEntity, PatientVisitPaymentSelfPayingRepository, PatientVisitPaymentSelfPayingEntityAudit,  PatientVisitPaymentSelfPayingService> csvUtils;
	...
	@Autowired
	public PatientVisitPaymentSelfPayingController(
		...
		this.csvUtils = new CsvUtils<>(patientVisitPaymentSelfPayingService, PatientVisitPaymentSelfPayingEntity.class);
	...
	/**
	 * Return all the Patient Visit Payment Self Payings.
	 *
	 * @return all the Patient Visit Payment Self Payings
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of PatientVisitPaymentSelfPayings",
		security = {
	...
	public ResponseEntity<List<PatientVisitPaymentSelfPayingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientVisitPaymentSelfPayingEntity> patientVisitPaymentSelfPayings = patientVisitPaymentSelfPayingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientVisitPaymentSelfPayings, HttpStatus.OK);
	}

	/**
	 * Return the Patient Visit Payment Self Paying that has the same id as the given id.
	 *
	 * @param id      The id of the PatientVisitPaymentSelfPayingEntity to be returned
	 * @return the Patient Visit Payment Self Paying that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientVisitPaymentSelfPaying as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientVisitPaymentSelfPaying entity not found"),
	})
	@PreAuthorize("hasPermission('PatientVisitPaymentSelfPayingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientVisitPaymentSelfPayingEntity> getById(
			@Parameter(description = "The UUID of the PatientVisitPaymentSelfPaying to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientVisitPaymentSelfPayingEntity> patientVisitPaymentSelfPayingEntity = patientVisitPaymentSelfPayingService.findById(id);
	...
	/**
	 * Create/Update the given Patient Visit Payment Self Paying. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientVisitPaymentSelfPayingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientVisitPaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Create a PatientVisitPaymentSelfPaying if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientVisitPaymentSelfPaying."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientVisitPaymentSelfPaying, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientVisitPaymentSelfPayingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientVisitPaymentSelfPaying' endpoint here] end
	public ResponseEntity<?> savePatientVisitPaymentSelfPaying(
			@Parameter(description = "The PatientVisitPaymentSelfPayingEntity to create or update.")
			@RequestBody PatientVisitPaymentSelfPayingEntityDto patientVisitPaymentSelfPayingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientVisitPaymentSelfPayingEntity patientVisitPaymentSelfPayingEntity = patientVisitPaymentSelfPayingService.create(new PatientVisitPaymentSelfPayingEntity(patientVisitPaymentSelfPayingEntityDto));
			return new ResponseEntity<>(patientVisitPaymentSelfPayingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Visit Payment Self Paying. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientVisitPaymentSelfPayingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientVisitPaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Update a PatientVisitPaymentSelfPaying entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientVisitPaymentSelfPaying."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientVisitPaymentSelfPaying, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientVisitPaymentSelfPayingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientVisitPaymentSelfPaying' endpoint here] end
	public ResponseEntity<?> updatePatientVisitPaymentSelfPaying(
			@Parameter(description = "The PatientVisitPaymentSelfPayingEntity to create or update.")
			@RequestBody PatientVisitPaymentSelfPayingEntityDto patientVisitPaymentSelfPayingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientVisitPaymentSelfPayingEntity> checkPatientVisitPaymentSelfPayingExists = patientVisitPaymentSelfPayingService.findById(patientVisitPaymentSelfPayingEntityDto.getId());
		PatientVisitPaymentSelfPayingEntity patientVisitPaymentSelfPayingEntity = new PatientVisitPaymentSelfPayingEntity(patientVisitPaymentSelfPayingEntityDto);

		if (checkPatientVisitPaymentSelfPayingExists.isPresent()) {
			try {
				patientVisitPaymentSelfPayingEntity = patientVisitPaymentSelfPayingService.update(patientVisitPaymentSelfPayingEntity);
				return new ResponseEntity<>(patientVisitPaymentSelfPayingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientVisitPaymentSelfPayingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Visit Payment Self Paying that has the same id as the given id.
	 *
	 * @param id The id of the PatientVisitPaymentSelfPayingEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientVisitPaymentSelfPaying as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientVisitPaymentSelfPaying to delete.")
		...
		patientVisitPaymentSelfPayingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Visit Payment Self Paying entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPaymentSelfPaying' endpoint here] end
	public void exportPatientVisitPaymentSelfPaying(
			@Parameter(description = "The ids of the PatientVisitPaymentSelfPayingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientVisitPaymentSelfPayingEntity> entitiesToExport = patientVisitPaymentSelfPayingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientVisitPaymentSelfPaying_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientVisitPaymentSelfPayingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPaymentSelfPayingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPaymentSelfPayingExcludingIds' endpoint here] end
	public void exportPatientVisitPaymentSelfPayingExcludingIds(
			@Parameter(description = "The ids of the PatientVisitPaymentSelfPayingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientVisitPaymentSelfPayingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientVisitPaymentSelfPayingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientVisitPaymentSelfPayingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientVisitPaymentSelfPaying entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientVisitPaymentSelfPaying_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientVisitPaymentSelfPayingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Visit Payment Self Paying entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientVisitPaymentSelfPayingEntity', 'create')  || hasPermission('PatientVisitPaymentSelfPayingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientVisitPaymentSelfPayingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientVisitPaymentSelfPayingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientVisitPaymentSelfPayingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientVisitPaymentSelfPayingEntity> csvToBean = new CsvToBeanBuilder<PatientVisitPaymentSelfPayingEntity>(reader)
				.withType(PatientVisitPaymentSelfPayingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientVisitPaymentSelfPayingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientVisitPaymentSelfPayingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Visit Payment Self Paying entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Visit Payment Self Paying entity and you do not have permission");
					}
