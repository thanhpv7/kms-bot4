...
import kmsweb.entities.HealthFacilityEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Health Facility
 */
// % protected region % [Update the API route or description for the Health FacilityController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "HealthFacilityController"))
@RestController
@Slf4j
@RequestMapping("/api/healthfacility")
// % protected region % [Update the API route or description for the Health FacilityController here] end
public class HealthFacilityController {

	private final HealthFacilityService healthFacilityService;
	private final CsvUtils<HealthFacilityEntity, HealthFacilityRepository, HealthFacilityEntityAudit,  HealthFacilityService> csvUtils;
	...
	public HealthFacilityController(
		...
		this.csvUtils = new CsvUtils<>(healthFacilityService, HealthFacilityEntity.class);
	...
	public ResponseEntity<List<HealthFacilityEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<HealthFacilityEntity> healthFacilitys = healthFacilityService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(healthFacilitys, HttpStatus.OK);
	}

	/**
	 * Return the Health Facility that has the same id as the given id.
	 *
	 * @param id      The id of the HealthFacilityEntity to be returned
	 * @return the Health Facility that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single HealthFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "HealthFacility entity not found"),
	})
	@PreAuthorize("hasPermission('HealthFacilityEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<HealthFacilityEntity> getById(
			@Parameter(description = "The UUID of the HealthFacility to return.")
			@PathVariable("id") UUID id
	) {
		Optional<HealthFacilityEntity> healthFacilityEntity = healthFacilityService.findById(id);
	...
	/**
	 * Create/Update the given Health Facility. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param healthFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveHealthFacility' endpoint here] off begin
	@Operation(
			description = "Create a HealthFacility if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new HealthFacility."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new HealthFacility, duplicate record found")
	})
	@PreAuthorize("hasPermission('HealthFacilityEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveHealthFacility' endpoint here] end
	public ResponseEntity<?> saveHealthFacility(
			@Parameter(description = "The HealthFacilityEntity to create or update.")
			@RequestBody HealthFacilityEntityDto healthFacilityEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			HealthFacilityEntity healthFacilityEntity = healthFacilityService.create(new HealthFacilityEntity(healthFacilityEntityDto));
			return new ResponseEntity<>(healthFacilityEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Health Facility. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param healthFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateHealthFacility' endpoint here] off begin
	@Operation(
			description = "Update a HealthFacility entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the HealthFacility."),
		@ApiResponse(responseCode = "400", description = "Failed to update the HealthFacility, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('HealthFacilityEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateHealthFacility' endpoint here] end
	public ResponseEntity<?> updateHealthFacility(
			@Parameter(description = "The HealthFacilityEntity to create or update.")
			@RequestBody HealthFacilityEntityDto healthFacilityEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<HealthFacilityEntity> checkHealthFacilityExists = healthFacilityService.findById(healthFacilityEntityDto.getId());
		HealthFacilityEntity healthFacilityEntity = new HealthFacilityEntity(healthFacilityEntityDto);

		if (checkHealthFacilityExists.isPresent()) {
			try {
				healthFacilityEntity = healthFacilityService.update(healthFacilityEntity);
				return new ResponseEntity<>(healthFacilityEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(healthFacilityEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Health Facility that has the same id as the given id.
	 *
	 * @param id The id of the HealthFacilityEntity to be deleted
	...
	@Operation(
			description = "Delete a single HealthFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the HealthFacility to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportHealthFacility' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Health Facility entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportHealthFacility' endpoint here] end
	public void exportHealthFacility(
			@Parameter(description = "The ids of the HealthFacilityEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<HealthFacilityEntity> entitiesToExport = healthFacilityService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "HealthFacility_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (HealthFacilityEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportHealthFacilityExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportHealthFacilityExcludingIds' endpoint here] end
	public void exportHealthFacilityExcludingIds(
			@Parameter(description = "The ids of the HealthFacilityEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<HealthFacilityEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = healthFacilityService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = healthFacilityService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the HealthFacility entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "HealthFacility_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(HealthFacilityEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Health Facility entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('HealthFacilityEntity', 'create')  || hasPermission('HealthFacilityEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with HealthFacilityEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("HealthFacilityEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(HealthFacilityEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<HealthFacilityEntity> csvToBean = new CsvToBeanBuilder<HealthFacilityEntity>(reader)
				.withType(HealthFacilityEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<HealthFacilityEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<HealthFacilityEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Health Facility entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Health Facility entity and you do not have permission");
					}
