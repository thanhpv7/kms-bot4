...
import kmsweb.entities.BpjsPcareRegistrationsEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Pcare Registrations
 */
// % protected region % [Update the API route or description for the BPJS Pcare RegistrationsController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcareRegistrationsController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcareregistrations")
// % protected region % [Update the API route or description for the BPJS Pcare RegistrationsController here] end
public class BpjsPcareRegistrationsController {

	private final BpjsPcareRegistrationsService bpjsPcareRegistrationsService;
	private final CsvUtils<BpjsPcareRegistrationsEntity, BpjsPcareRegistrationsRepository, BpjsPcareRegistrationsEntityAudit,  BpjsPcareRegistrationsService> csvUtils;
	...
	@Autowired
	public BpjsPcareRegistrationsController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcareRegistrationsService, BpjsPcareRegistrationsEntity.class);
	...
	public ResponseEntity<List<BpjsPcareRegistrationsEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcareRegistrationsEntity> bpjsPcareRegistrationss = bpjsPcareRegistrationsService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcareRegistrationss, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Pcare Registrations that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcareRegistrationsEntity to be returned
	 * @return the BPJS Pcare Registrations that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcareRegistrations as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcareRegistrations entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcareRegistrationsEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcareRegistrationsEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcareRegistrations to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcareRegistrationsEntity> bpjsPcareRegistrationsEntity = bpjsPcareRegistrationsService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Pcare Registrations. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareRegistrationsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareRegistrations' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcareRegistrations if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcareRegistrations."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcareRegistrations, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcareRegistrationsEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareRegistrations' endpoint here] end
	public ResponseEntity<?> saveBpjsPcareRegistrations(
			@Parameter(description = "The BpjsPcareRegistrationsEntity to create or update.")
			@RequestBody BpjsPcareRegistrationsEntityDto bpjsPcareRegistrationsEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcareRegistrationsEntity bpjsPcareRegistrationsEntity = bpjsPcareRegistrationsService.create(new BpjsPcareRegistrationsEntity(bpjsPcareRegistrationsEntityDto));
			return new ResponseEntity<>(bpjsPcareRegistrationsEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Pcare Registrations. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareRegistrationsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareRegistrations' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcareRegistrations entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcareRegistrations."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcareRegistrations, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcareRegistrationsEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareRegistrations' endpoint here] end
	public ResponseEntity<?> updateBpjsPcareRegistrations(
			@Parameter(description = "The BpjsPcareRegistrationsEntity to create or update.")
			@RequestBody BpjsPcareRegistrationsEntityDto bpjsPcareRegistrationsEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcareRegistrationsEntity> checkBpjsPcareRegistrationsExists = bpjsPcareRegistrationsService.findById(bpjsPcareRegistrationsEntityDto.getId());
		BpjsPcareRegistrationsEntity bpjsPcareRegistrationsEntity = new BpjsPcareRegistrationsEntity(bpjsPcareRegistrationsEntityDto);

		if (checkBpjsPcareRegistrationsExists.isPresent()) {
			try {
				bpjsPcareRegistrationsEntity = bpjsPcareRegistrationsService.update(bpjsPcareRegistrationsEntity);
				return new ResponseEntity<>(bpjsPcareRegistrationsEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcareRegistrationsEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Pcare Registrations that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcareRegistrationsEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcareRegistrations as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcareRegistrations to delete.")
		...
		bpjsPcareRegistrationsService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareRegistrations' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Pcare Registrations entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareRegistrations' endpoint here] end
	public void exportBpjsPcareRegistrations(
			@Parameter(description = "The ids of the BpjsPcareRegistrationsEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcareRegistrationsEntity> entitiesToExport = bpjsPcareRegistrationsService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcareRegistrations_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcareRegistrationsEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareRegistrationsExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareRegistrationsExcludingIds' endpoint here] end
	public void exportBpjsPcareRegistrationsExcludingIds(
			@Parameter(description = "The ids of the BpjsPcareRegistrationsEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcareRegistrationsEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcareRegistrationsService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcareRegistrationsService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcareRegistrations entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcareRegistrations_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcareRegistrationsEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Pcare Registrations entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcareRegistrationsEntity', 'create')  || hasPermission('BpjsPcareRegistrationsEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcareRegistrationsEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcareRegistrationsEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcareRegistrationsEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcareRegistrationsEntity> csvToBean = new CsvToBeanBuilder<BpjsPcareRegistrationsEntity>(reader)
				.withType(BpjsPcareRegistrationsEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcareRegistrationsEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcareRegistrationsEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Pcare Registrations entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Pcare Registrations entity and you do not have permission");
					}
