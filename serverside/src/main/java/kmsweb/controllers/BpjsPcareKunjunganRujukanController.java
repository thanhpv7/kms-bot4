...
/**
 * Controller used to handle all REST operations regarding Bpjs Pcare Kunjungan Rujukan
 */
// % protected region % [Update the API route or description for the Bpjs Pcare Kunjungan RujukanController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcareKunjunganRujukanController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcarekunjunganrujukan")
// % protected region % [Update the API route or description for the Bpjs Pcare Kunjungan RujukanController here] end
public class BpjsPcareKunjunganRujukanController {

	private final BpjsPcareKunjunganRujukanService bpjsPcareKunjunganRujukanService;
	private final CsvUtils<BpjsPcareKunjunganRujukanEntity, BpjsPcareKunjunganRujukanRepository, BpjsPcareKunjunganRujukanEntityAudit,  BpjsPcareKunjunganRujukanService> csvUtils;
	...
	@Autowired
	public BpjsPcareKunjunganRujukanController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcareKunjunganRujukanService, BpjsPcareKunjunganRujukanEntity.class);
	...
	/**
	 * Return all the Bpjs Pcare Kunjungan Rujukans.
	 *
	 * @return all the Bpjs Pcare Kunjungan Rujukans
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsPcareKunjunganRujukans",
		security = {
	...
	public ResponseEntity<List<BpjsPcareKunjunganRujukanEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcareKunjunganRujukanEntity> bpjsPcareKunjunganRujukans = bpjsPcareKunjunganRujukanService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcareKunjunganRujukans, HttpStatus.OK);
	}

	/**
	 * Return the Bpjs Pcare Kunjungan Rujukan that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcareKunjunganRujukanEntity to be returned
	 * @return the Bpjs Pcare Kunjungan Rujukan that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcareKunjunganRujukan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcareKunjunganRujukan entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcareKunjunganRujukanEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcareKunjunganRujukanEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcareKunjunganRujukan to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcareKunjunganRujukanEntity> bpjsPcareKunjunganRujukanEntity = bpjsPcareKunjunganRujukanService.findById(id);
	...
	/**
	 * Create/Update the given Bpjs Pcare Kunjungan Rujukan. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareKunjunganRujukanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareKunjunganRujukan' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcareKunjunganRujukan if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcareKunjunganRujukan."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcareKunjunganRujukan, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcareKunjunganRujukanEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareKunjunganRujukan' endpoint here] end
	public ResponseEntity<?> saveBpjsPcareKunjunganRujukan(
			@Parameter(description = "The BpjsPcareKunjunganRujukanEntity to create or update.")
			@RequestBody BpjsPcareKunjunganRujukanEntityDto bpjsPcareKunjunganRujukanEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcareKunjunganRujukanEntity bpjsPcareKunjunganRujukanEntity = bpjsPcareKunjunganRujukanService.create(new BpjsPcareKunjunganRujukanEntity(bpjsPcareKunjunganRujukanEntityDto));
			return new ResponseEntity<>(bpjsPcareKunjunganRujukanEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Bpjs Pcare Kunjungan Rujukan. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareKunjunganRujukanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareKunjunganRujukan' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcareKunjunganRujukan entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcareKunjunganRujukan."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcareKunjunganRujukan, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcareKunjunganRujukanEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareKunjunganRujukan' endpoint here] end
	public ResponseEntity<?> updateBpjsPcareKunjunganRujukan(
			@Parameter(description = "The BpjsPcareKunjunganRujukanEntity to create or update.")
			@RequestBody BpjsPcareKunjunganRujukanEntityDto bpjsPcareKunjunganRujukanEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcareKunjunganRujukanEntity> checkBpjsPcareKunjunganRujukanExists = bpjsPcareKunjunganRujukanService.findById(bpjsPcareKunjunganRujukanEntityDto.getId());
		BpjsPcareKunjunganRujukanEntity bpjsPcareKunjunganRujukanEntity = new BpjsPcareKunjunganRujukanEntity(bpjsPcareKunjunganRujukanEntityDto);

		if (checkBpjsPcareKunjunganRujukanExists.isPresent()) {
			try {
				bpjsPcareKunjunganRujukanEntity = bpjsPcareKunjunganRujukanService.update(bpjsPcareKunjunganRujukanEntity);
				return new ResponseEntity<>(bpjsPcareKunjunganRujukanEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcareKunjunganRujukanEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Bpjs Pcare Kunjungan Rujukan that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcareKunjunganRujukanEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcareKunjunganRujukan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcareKunjunganRujukan to delete.")
		...
		bpjsPcareKunjunganRujukanService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKunjunganRujukan' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Bpjs Pcare Kunjungan Rujukan entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKunjunganRujukan' endpoint here] end
	public void exportBpjsPcareKunjunganRujukan(
			@Parameter(description = "The ids of the BpjsPcareKunjunganRujukanEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcareKunjunganRujukanEntity> entitiesToExport = bpjsPcareKunjunganRujukanService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcareKunjunganRujukan_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcareKunjunganRujukanEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKunjunganRujukanExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKunjunganRujukanExcludingIds' endpoint here] end
	public void exportBpjsPcareKunjunganRujukanExcludingIds(
			@Parameter(description = "The ids of the BpjsPcareKunjunganRujukanEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcareKunjunganRujukanEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcareKunjunganRujukanService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcareKunjunganRujukanService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcareKunjunganRujukan entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcareKunjunganRujukan_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcareKunjunganRujukanEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Bpjs Pcare Kunjungan Rujukan entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcareKunjunganRujukanEntity', 'create')  || hasPermission('BpjsPcareKunjunganRujukanEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcareKunjunganRujukanEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcareKunjunganRujukanEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcareKunjunganRujukanEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcareKunjunganRujukanEntity> csvToBean = new CsvToBeanBuilder<BpjsPcareKunjunganRujukanEntity>(reader)
				.withType(BpjsPcareKunjunganRujukanEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcareKunjunganRujukanEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcareKunjunganRujukanEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Bpjs Pcare Kunjungan Rujukan entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Bpjs Pcare Kunjungan Rujukan entity and you do not have permission");
					}
