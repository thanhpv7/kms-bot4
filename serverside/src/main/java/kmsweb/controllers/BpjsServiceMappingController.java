...
import kmsweb.entities.BpjsServiceMappingEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Service Mapping
 */
// % protected region % [Update the API route or description for the BPJS Service MappingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsServiceMappingController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsservicemapping")
// % protected region % [Update the API route or description for the BPJS Service MappingController here] end
public class BpjsServiceMappingController {

	private final BpjsServiceMappingService bpjsServiceMappingService;
	private final CsvUtils<BpjsServiceMappingEntity, BpjsServiceMappingRepository, BpjsServiceMappingEntityAudit,  BpjsServiceMappingService> csvUtils;
	...
	@Autowired
	public BpjsServiceMappingController(
		...
		this.csvUtils = new CsvUtils<>(bpjsServiceMappingService, BpjsServiceMappingEntity.class);
	...
	public ResponseEntity<List<BpjsServiceMappingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsServiceMappingEntity> bpjsServiceMappings = bpjsServiceMappingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsServiceMappings, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Service Mapping that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsServiceMappingEntity to be returned
	 * @return the BPJS Service Mapping that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsServiceMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsServiceMapping entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsServiceMappingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsServiceMappingEntity> getById(
			@Parameter(description = "The UUID of the BpjsServiceMapping to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsServiceMappingEntity> bpjsServiceMappingEntity = bpjsServiceMappingService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Service Mapping. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsServiceMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsServiceMapping' endpoint here] off begin
	@Operation(
			description = "Create a BpjsServiceMapping if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsServiceMapping."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsServiceMapping, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsServiceMappingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsServiceMapping' endpoint here] end
	public ResponseEntity<?> saveBpjsServiceMapping(
			@Parameter(description = "The BpjsServiceMappingEntity to create or update.")
			@RequestBody BpjsServiceMappingEntityDto bpjsServiceMappingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsServiceMappingEntity bpjsServiceMappingEntity = bpjsServiceMappingService.create(new BpjsServiceMappingEntity(bpjsServiceMappingEntityDto));
			return new ResponseEntity<>(bpjsServiceMappingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Service Mapping. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsServiceMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsServiceMapping' endpoint here] off begin
	@Operation(
			description = "Update a BpjsServiceMapping entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsServiceMapping."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsServiceMapping, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsServiceMappingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsServiceMapping' endpoint here] end
	public ResponseEntity<?> updateBpjsServiceMapping(
			@Parameter(description = "The BpjsServiceMappingEntity to create or update.")
			@RequestBody BpjsServiceMappingEntityDto bpjsServiceMappingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsServiceMappingEntity> checkBpjsServiceMappingExists = bpjsServiceMappingService.findById(bpjsServiceMappingEntityDto.getId());
		BpjsServiceMappingEntity bpjsServiceMappingEntity = new BpjsServiceMappingEntity(bpjsServiceMappingEntityDto);

		if (checkBpjsServiceMappingExists.isPresent()) {
			try {
				bpjsServiceMappingEntity = bpjsServiceMappingService.update(bpjsServiceMappingEntity);
				return new ResponseEntity<>(bpjsServiceMappingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsServiceMappingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Service Mapping that has the same id as the given id.
	 *
	 * @param id The id of the BpjsServiceMappingEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsServiceMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsServiceMapping to delete.")
		...
		bpjsServiceMappingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsServiceMapping' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Service Mapping entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsServiceMapping' endpoint here] end
	public void exportBpjsServiceMapping(
			@Parameter(description = "The ids of the BpjsServiceMappingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsServiceMappingEntity> entitiesToExport = bpjsServiceMappingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsServiceMapping_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsServiceMappingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsServiceMappingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsServiceMappingExcludingIds' endpoint here] end
	public void exportBpjsServiceMappingExcludingIds(
			@Parameter(description = "The ids of the BpjsServiceMappingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsServiceMappingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsServiceMappingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsServiceMappingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsServiceMapping entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsServiceMapping_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsServiceMappingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Service Mapping entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsServiceMappingEntity', 'create')  || hasPermission('BpjsServiceMappingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsServiceMappingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsServiceMappingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsServiceMappingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsServiceMappingEntity> csvToBean = new CsvToBeanBuilder<BpjsServiceMappingEntity>(reader)
				.withType(BpjsServiceMappingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsServiceMappingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsServiceMappingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Service Mapping entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Service Mapping entity and you do not have permission");
					}
