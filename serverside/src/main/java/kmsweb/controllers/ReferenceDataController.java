...
/**
 * Controller used to handle all REST operations regarding Reference Data
 */
// % protected region % [Update the API route or description for the Reference DataController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ReferenceDataController"))
@RestController
@Slf4j
@RequestMapping("/api/referencedata")
// % protected region % [Update the API route or description for the Reference DataController here] end
public class ReferenceDataController {

	private final ReferenceDataService referenceDataService;
	private final CsvUtils<ReferenceDataEntity, ReferenceDataRepository, ReferenceDataEntityAudit,  ReferenceDataService> csvUtils;
	...
	public ReferenceDataController(
		...
		this.csvUtils = new CsvUtils<>(referenceDataService, ReferenceDataEntity.class);
	...
	public ResponseEntity<List<ReferenceDataEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ReferenceDataEntity> referenceDatas = referenceDataService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(referenceDatas, HttpStatus.OK);
	}

	/**
	 * Return the Reference Data that has the same id as the given id.
	 *
	 * @param id      The id of the ReferenceDataEntity to be returned
	 * @return the Reference Data that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ReferenceData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ReferenceData entity not found"),
	})
	@PreAuthorize("hasPermission('ReferenceDataEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ReferenceDataEntity> getById(
			@Parameter(description = "The UUID of the ReferenceData to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ReferenceDataEntity> referenceDataEntity = referenceDataService.findById(id);
	...
	/**
	 * Create/Update the given Reference Data. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param referenceDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveReferenceData' endpoint here] off begin
	@Operation(
			description = "Create a ReferenceData if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ReferenceData."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ReferenceData, duplicate record found")
	})
	@PreAuthorize("hasPermission('ReferenceDataEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveReferenceData' endpoint here] end
	public ResponseEntity<?> saveReferenceData(
			@Parameter(description = "The ReferenceDataEntity to create or update.")
			@RequestBody ReferenceDataEntityDto referenceDataEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ReferenceDataEntity referenceDataEntity = referenceDataService.create(new ReferenceDataEntity(referenceDataEntityDto));
			return new ResponseEntity<>(referenceDataEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Reference Data. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param referenceDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateReferenceData' endpoint here] off begin
	@Operation(
			description = "Update a ReferenceData entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ReferenceData."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ReferenceData, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ReferenceDataEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateReferenceData' endpoint here] end
	public ResponseEntity<?> updateReferenceData(
			@Parameter(description = "The ReferenceDataEntity to create or update.")
			@RequestBody ReferenceDataEntityDto referenceDataEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ReferenceDataEntity> checkReferenceDataExists = referenceDataService.findById(referenceDataEntityDto.getId());
		ReferenceDataEntity referenceDataEntity = new ReferenceDataEntity(referenceDataEntityDto);

		if (checkReferenceDataExists.isPresent()) {
			try {
				referenceDataEntity = referenceDataService.update(referenceDataEntity);
				return new ResponseEntity<>(referenceDataEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(referenceDataEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Reference Data that has the same id as the given id.
	 *
	 * @param id The id of the ReferenceDataEntity to be deleted
	...
	@Operation(
			description = "Delete a single ReferenceData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ReferenceData to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportReferenceData' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Reference Data entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportReferenceData' endpoint here] end
	public void exportReferenceData(
			@Parameter(description = "The ids of the ReferenceDataEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ReferenceDataEntity> entitiesToExport = referenceDataService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ReferenceData_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ReferenceDataEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportReferenceDataExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportReferenceDataExcludingIds' endpoint here] end
	public void exportReferenceDataExcludingIds(
			@Parameter(description = "The ids of the ReferenceDataEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ReferenceDataEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = referenceDataService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = referenceDataService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ReferenceData entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ReferenceData_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ReferenceDataEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Reference Data entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ReferenceDataEntity', 'create')  || hasPermission('ReferenceDataEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ReferenceDataEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ReferenceDataEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ReferenceDataEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ReferenceDataEntity> csvToBean = new CsvToBeanBuilder<ReferenceDataEntity>(reader)
				.withType(ReferenceDataEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ReferenceDataEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ReferenceDataEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Reference Data entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Reference Data entity and you do not have permission");
					}
