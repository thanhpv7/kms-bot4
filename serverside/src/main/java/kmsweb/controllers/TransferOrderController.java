...
/**
 * Controller used to handle all REST operations regarding Transfer Order
 */
// % protected region % [Update the API route or description for the Transfer OrderController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TransferOrderController"))
@RestController
@Slf4j
@RequestMapping("/api/transferorder")
// % protected region % [Update the API route or description for the Transfer OrderController here] end
public class TransferOrderController {

	private final TransferOrderService transferOrderService;
	private final CsvUtils<TransferOrderEntity, TransferOrderRepository, TransferOrderEntityAudit,  TransferOrderService> csvUtils;
	...
	public TransferOrderController(
		...
		this.csvUtils = new CsvUtils<>(transferOrderService, TransferOrderEntity.class);
	...
	public ResponseEntity<List<TransferOrderEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TransferOrderEntity> transferOrders = transferOrderService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(transferOrders, HttpStatus.OK);
	}

	/**
	 * Return the Transfer Order that has the same id as the given id.
	 *
	 * @param id      The id of the TransferOrderEntity to be returned
	 * @return the Transfer Order that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TransferOrder as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TransferOrder entity not found"),
	})
	@PreAuthorize("hasPermission('TransferOrderEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TransferOrderEntity> getById(
			@Parameter(description = "The UUID of the TransferOrder to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TransferOrderEntity> transferOrderEntity = transferOrderService.findById(id);
	...
	/**
	 * Create/Update the given Transfer Order. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param transferOrderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTransferOrder' endpoint here] off begin
	@Operation(
			description = "Create a TransferOrder if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TransferOrder."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TransferOrder, duplicate record found")
	})
	@PreAuthorize("hasPermission('TransferOrderEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTransferOrder' endpoint here] end
	public ResponseEntity<?> saveTransferOrder(
			@Parameter(description = "The TransferOrderEntity to create or update.")
			@RequestBody TransferOrderEntityDto transferOrderEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TransferOrderEntity transferOrderEntity = transferOrderService.create(new TransferOrderEntity(transferOrderEntityDto));
			return new ResponseEntity<>(transferOrderEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Transfer Order. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param transferOrderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTransferOrder' endpoint here] off begin
	@Operation(
			description = "Update a TransferOrder entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TransferOrder."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TransferOrder, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TransferOrderEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTransferOrder' endpoint here] end
	public ResponseEntity<?> updateTransferOrder(
			@Parameter(description = "The TransferOrderEntity to create or update.")
			@RequestBody TransferOrderEntityDto transferOrderEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TransferOrderEntity> checkTransferOrderExists = transferOrderService.findById(transferOrderEntityDto.getId());
		TransferOrderEntity transferOrderEntity = new TransferOrderEntity(transferOrderEntityDto);

		if (checkTransferOrderExists.isPresent()) {
			try {
				transferOrderEntity = transferOrderService.update(transferOrderEntity);
				return new ResponseEntity<>(transferOrderEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(transferOrderEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Transfer Order that has the same id as the given id.
	 *
	 * @param id The id of the TransferOrderEntity to be deleted
	...
	@Operation(
			description = "Delete a single TransferOrder as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TransferOrder to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransferOrder' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Transfer Order entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTransferOrder' endpoint here] end
	public void exportTransferOrder(
			@Parameter(description = "The ids of the TransferOrderEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TransferOrderEntity> entitiesToExport = transferOrderService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TransferOrder_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TransferOrderEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransferOrderExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTransferOrderExcludingIds' endpoint here] end
	public void exportTransferOrderExcludingIds(
			@Parameter(description = "The ids of the TransferOrderEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TransferOrderEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = transferOrderService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = transferOrderService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TransferOrder entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TransferOrder_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TransferOrderEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Transfer Order entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TransferOrderEntity', 'create')  || hasPermission('TransferOrderEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TransferOrderEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TransferOrderEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TransferOrderEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TransferOrderEntity> csvToBean = new CsvToBeanBuilder<TransferOrderEntity>(reader)
				.withType(TransferOrderEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TransferOrderEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TransferOrderEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Transfer Order entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Transfer Order entity and you do not have permission");
					}
