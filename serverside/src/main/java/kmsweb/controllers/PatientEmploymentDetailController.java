...
/**
 * Controller used to handle all REST operations regarding Patient Employment Detail 
 */
// % protected region % [Update the API route or description for the Patient Employment Detail Controller here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientEmploymentDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/patientemploymentdetail")
// % protected region % [Update the API route or description for the Patient Employment Detail Controller here] end
public class PatientEmploymentDetailController {

	private final PatientEmploymentDetailService patientEmploymentDetailService;
	private final CsvUtils<PatientEmploymentDetailEntity, PatientEmploymentDetailRepository, PatientEmploymentDetailEntityAudit,  PatientEmploymentDetailService> csvUtils;
	...
	@Autowired
	public PatientEmploymentDetailController(
		...
		this.csvUtils = new CsvUtils<>(patientEmploymentDetailService, PatientEmploymentDetailEntity.class);
	...
	/**
	 * Return all the Patient Employment Detail s.
	 *
	 * @return all the Patient Employment Detail s
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of PatientEmploymentDetails",
		security = {
	...
	public ResponseEntity<List<PatientEmploymentDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientEmploymentDetailEntity> patientEmploymentDetails = patientEmploymentDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientEmploymentDetails, HttpStatus.OK);
	}

	/**
	 * Return the Patient Employment Detail  that has the same id as the given id.
	 *
	 * @param id      The id of the PatientEmploymentDetailEntity to be returned
	 * @return the Patient Employment Detail  that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientEmploymentDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientEmploymentDetail entity not found"),
	})
	@PreAuthorize("hasPermission('PatientEmploymentDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientEmploymentDetailEntity> getById(
			@Parameter(description = "The UUID of the PatientEmploymentDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientEmploymentDetailEntity> patientEmploymentDetailEntity = patientEmploymentDetailService.findById(id);
	...
	/**
	 * Create/Update the given Patient Employment Detail . If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientEmploymentDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientEmploymentDetail' endpoint here] off begin
	@Operation(
			description = "Create a PatientEmploymentDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientEmploymentDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientEmploymentDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientEmploymentDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientEmploymentDetail' endpoint here] end
	public ResponseEntity<?> savePatientEmploymentDetail(
			@Parameter(description = "The PatientEmploymentDetailEntity to create or update.")
			@RequestBody PatientEmploymentDetailEntityDto patientEmploymentDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientEmploymentDetailEntity patientEmploymentDetailEntity = patientEmploymentDetailService.create(new PatientEmploymentDetailEntity(patientEmploymentDetailEntityDto));
			return new ResponseEntity<>(patientEmploymentDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Employment Detail . If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientEmploymentDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientEmploymentDetail' endpoint here] off begin
	@Operation(
			description = "Update a PatientEmploymentDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientEmploymentDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientEmploymentDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientEmploymentDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientEmploymentDetail' endpoint here] end
	public ResponseEntity<?> updatePatientEmploymentDetail(
			@Parameter(description = "The PatientEmploymentDetailEntity to create or update.")
			@RequestBody PatientEmploymentDetailEntityDto patientEmploymentDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientEmploymentDetailEntity> checkPatientEmploymentDetailExists = patientEmploymentDetailService.findById(patientEmploymentDetailEntityDto.getId());
		PatientEmploymentDetailEntity patientEmploymentDetailEntity = new PatientEmploymentDetailEntity(patientEmploymentDetailEntityDto);

		if (checkPatientEmploymentDetailExists.isPresent()) {
			try {
				patientEmploymentDetailEntity = patientEmploymentDetailService.update(patientEmploymentDetailEntity);
				return new ResponseEntity<>(patientEmploymentDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientEmploymentDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Employment Detail  that has the same id as the given id.
	 *
	 * @param id The id of the PatientEmploymentDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientEmploymentDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientEmploymentDetail to delete.")
		...
		patientEmploymentDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientEmploymentDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Employment Detail  entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientEmploymentDetail' endpoint here] end
	public void exportPatientEmploymentDetail(
			@Parameter(description = "The ids of the PatientEmploymentDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientEmploymentDetailEntity> entitiesToExport = patientEmploymentDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientEmploymentDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientEmploymentDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientEmploymentDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientEmploymentDetailExcludingIds' endpoint here] end
	public void exportPatientEmploymentDetailExcludingIds(
			@Parameter(description = "The ids of the PatientEmploymentDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientEmploymentDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientEmploymentDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientEmploymentDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientEmploymentDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientEmploymentDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientEmploymentDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Employment Detail  entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientEmploymentDetailEntity', 'create')  || hasPermission('PatientEmploymentDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientEmploymentDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientEmploymentDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientEmploymentDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientEmploymentDetailEntity> csvToBean = new CsvToBeanBuilder<PatientEmploymentDetailEntity>(reader)
				.withType(PatientEmploymentDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientEmploymentDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientEmploymentDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Employment Detail  entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Employment Detail  entity and you do not have permission");
					}
