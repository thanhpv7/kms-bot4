...
import kmsweb.entities.BpjsOutsideFacilityReferralEntityAudit;
import kmsweb.repositories.BpjsOutsideFacilityReferralRepository;
import kmsweb.services.BpjsOutsideFacilityReferralService;
...
/**
 * Controller used to handle all REST operations regarding BPJS Outside Facility Referral
 */
// % protected region % [Update the API route or description for the BPJS Outside Facility ReferralController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsOutsideFacilityReferralController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsoutsidefacilityreferral")
// % protected region % [Update the API route or description for the BPJS Outside Facility ReferralController here] end
public class BpjsOutsideFacilityReferralController {

	private final BpjsOutsideFacilityReferralService bpjsOutsideFacilityReferralService;
	private final CsvUtils<BpjsOutsideFacilityReferralEntity, BpjsOutsideFacilityReferralRepository, BpjsOutsideFacilityReferralEntityAudit,  BpjsOutsideFacilityReferralService> csvUtils;
	...
	@Autowired
	public BpjsOutsideFacilityReferralController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsOutsideFacilityReferralService bpjsOutsideFacilityReferralService
	) {
		this.authService = authService;
		this.bpjsOutsideFacilityReferralService = bpjsOutsideFacilityReferralService;
		this.csvUtils = new CsvUtils<>(bpjsOutsideFacilityReferralService, BpjsOutsideFacilityReferralEntity.class);
	...
	/**
	 * Return all the BPJS Outside Facility Referrals.
	 *
	 * @return all the BPJS Outside Facility Referrals
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsOutsideFacilityReferrals",
		security = {
	...
	public ResponseEntity<List<BpjsOutsideFacilityReferralEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsOutsideFacilityReferralEntity> bpjsOutsideFacilityReferrals = bpjsOutsideFacilityReferralService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsOutsideFacilityReferrals, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Outside Facility Referral that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsOutsideFacilityReferralEntity to be returned
	 * @return the BPJS Outside Facility Referral that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsOutsideFacilityReferral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsOutsideFacilityReferral entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsOutsideFacilityReferralEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsOutsideFacilityReferralEntity> getById(
			@Parameter(description = "The UUID of the BpjsOutsideFacilityReferral to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsOutsideFacilityReferralEntity> bpjsOutsideFacilityReferralEntity = bpjsOutsideFacilityReferralService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Outside Facility Referral. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsOutsideFacilityReferralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsOutsideFacilityReferral' endpoint here] off begin
	@Operation(
			description = "Create a BpjsOutsideFacilityReferral if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsOutsideFacilityReferral."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsOutsideFacilityReferral, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsOutsideFacilityReferralEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsOutsideFacilityReferral' endpoint here] end
	public ResponseEntity<?> saveBpjsOutsideFacilityReferral(
			@Parameter(description = "The BpjsOutsideFacilityReferralEntity to create or update.")
			@RequestBody BpjsOutsideFacilityReferralEntityDto bpjsOutsideFacilityReferralEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsOutsideFacilityReferralEntity bpjsOutsideFacilityReferralEntity = bpjsOutsideFacilityReferralService.create(new BpjsOutsideFacilityReferralEntity(bpjsOutsideFacilityReferralEntityDto));
			return new ResponseEntity<>(bpjsOutsideFacilityReferralEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Outside Facility Referral. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsOutsideFacilityReferralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsOutsideFacilityReferral' endpoint here] off begin
	@Operation(
			description = "Update a BpjsOutsideFacilityReferral entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsOutsideFacilityReferral."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsOutsideFacilityReferral, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsOutsideFacilityReferralEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsOutsideFacilityReferral' endpoint here] end
	public ResponseEntity<?> updateBpjsOutsideFacilityReferral(
			@Parameter(description = "The BpjsOutsideFacilityReferralEntity to create or update.")
			@RequestBody BpjsOutsideFacilityReferralEntityDto bpjsOutsideFacilityReferralEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsOutsideFacilityReferralEntity> checkBpjsOutsideFacilityReferralExists = bpjsOutsideFacilityReferralService.findById(bpjsOutsideFacilityReferralEntityDto.getId());
		BpjsOutsideFacilityReferralEntity bpjsOutsideFacilityReferralEntity = new BpjsOutsideFacilityReferralEntity(bpjsOutsideFacilityReferralEntityDto);

		if (checkBpjsOutsideFacilityReferralExists.isPresent()) {
			try {
				bpjsOutsideFacilityReferralEntity = bpjsOutsideFacilityReferralService.update(bpjsOutsideFacilityReferralEntity);
				return new ResponseEntity<>(bpjsOutsideFacilityReferralEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsOutsideFacilityReferralEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Outside Facility Referral that has the same id as the given id.
	 *
	 * @param id The id of the BpjsOutsideFacilityReferralEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsOutsideFacilityReferral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsOutsideFacilityReferral to delete.")
		...
		bpjsOutsideFacilityReferralService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsOutsideFacilityReferral' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Outside Facility Referral entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsOutsideFacilityReferral' endpoint here] end
	public void exportBpjsOutsideFacilityReferral(
			@Parameter(description = "The ids of the BpjsOutsideFacilityReferralEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsOutsideFacilityReferralEntity> entitiesToExport = bpjsOutsideFacilityReferralService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsOutsideFacilityReferral_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsOutsideFacilityReferralEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsOutsideFacilityReferralExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsOutsideFacilityReferralExcludingIds' endpoint here] end
	public void exportBpjsOutsideFacilityReferralExcludingIds(
			@Parameter(description = "The ids of the BpjsOutsideFacilityReferralEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsOutsideFacilityReferralEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsOutsideFacilityReferralService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsOutsideFacilityReferralService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsOutsideFacilityReferral entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsOutsideFacilityReferral_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsOutsideFacilityReferralEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Outside Facility Referral entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsOutsideFacilityReferralEntity', 'create')  || hasPermission('BpjsOutsideFacilityReferralEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsOutsideFacilityReferralEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsOutsideFacilityReferralEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsOutsideFacilityReferralEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsOutsideFacilityReferralEntity> csvToBean = new CsvToBeanBuilder<BpjsOutsideFacilityReferralEntity>(reader)
				.withType(BpjsOutsideFacilityReferralEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsOutsideFacilityReferralEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsOutsideFacilityReferralEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Outside Facility Referral entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Outside Facility Referral entity and you do not have permission");
					}
