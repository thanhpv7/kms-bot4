...
/**
 * Controller used to handle all REST operations regarding Hospital Branch Information
 */
// % protected region % [Update the API route or description for the Hospital Branch InformationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "HospitalBranchInformationController"))
@RestController
@Slf4j
@RequestMapping("/api/hospitalbranchinformation")
// % protected region % [Update the API route or description for the Hospital Branch InformationController here] end
public class HospitalBranchInformationController {

	private final HospitalBranchInformationService hospitalBranchInformationService;
	private final CsvUtils<HospitalBranchInformationEntity, HospitalBranchInformationRepository, HospitalBranchInformationEntityAudit,  HospitalBranchInformationService> csvUtils;
	...
	@Autowired
	public HospitalBranchInformationController(
		...
		this.csvUtils = new CsvUtils<>(hospitalBranchInformationService, HospitalBranchInformationEntity.class);
	...
	/**
	 * Return all the Hospital Branch Informations.
	 *
	 * @return all the Hospital Branch Informations
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of HospitalBranchInformations",
		security = {
	...
	public ResponseEntity<List<HospitalBranchInformationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<HospitalBranchInformationEntity> hospitalBranchInformations = hospitalBranchInformationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(hospitalBranchInformations, HttpStatus.OK);
	}

	/**
	 * Return the Hospital Branch Information that has the same id as the given id.
	 *
	 * @param id      The id of the HospitalBranchInformationEntity to be returned
	 * @return the Hospital Branch Information that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single HospitalBranchInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "HospitalBranchInformation entity not found"),
	})
	@PreAuthorize("hasPermission('HospitalBranchInformationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<HospitalBranchInformationEntity> getById(
			@Parameter(description = "The UUID of the HospitalBranchInformation to return.")
			@PathVariable("id") UUID id
	) {
		Optional<HospitalBranchInformationEntity> hospitalBranchInformationEntity = hospitalBranchInformationService.findById(id);
	...
	/**
	 * Create/Update the given Hospital Branch Information. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param hospitalBranchInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveHospitalBranchInformation' endpoint here] off begin
	@Operation(
			description = "Create a HospitalBranchInformation if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new HospitalBranchInformation."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new HospitalBranchInformation, duplicate record found")
	})
	@PreAuthorize("hasPermission('HospitalBranchInformationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveHospitalBranchInformation' endpoint here] end
	public ResponseEntity<?> saveHospitalBranchInformation(
			@Parameter(description = "The HospitalBranchInformationEntity to create or update.")
			@RequestBody HospitalBranchInformationEntityDto hospitalBranchInformationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			HospitalBranchInformationEntity hospitalBranchInformationEntity = hospitalBranchInformationService.create(new HospitalBranchInformationEntity(hospitalBranchInformationEntityDto));
			return new ResponseEntity<>(hospitalBranchInformationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Hospital Branch Information. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param hospitalBranchInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateHospitalBranchInformation' endpoint here] off begin
	@Operation(
			description = "Update a HospitalBranchInformation entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the HospitalBranchInformation."),
		@ApiResponse(responseCode = "400", description = "Failed to update the HospitalBranchInformation, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('HospitalBranchInformationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateHospitalBranchInformation' endpoint here] end
	public ResponseEntity<?> updateHospitalBranchInformation(
			@Parameter(description = "The HospitalBranchInformationEntity to create or update.")
			@RequestBody HospitalBranchInformationEntityDto hospitalBranchInformationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<HospitalBranchInformationEntity> checkHospitalBranchInformationExists = hospitalBranchInformationService.findById(hospitalBranchInformationEntityDto.getId());
		HospitalBranchInformationEntity hospitalBranchInformationEntity = new HospitalBranchInformationEntity(hospitalBranchInformationEntityDto);

		if (checkHospitalBranchInformationExists.isPresent()) {
			try {
				hospitalBranchInformationEntity = hospitalBranchInformationService.update(hospitalBranchInformationEntity);
				return new ResponseEntity<>(hospitalBranchInformationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(hospitalBranchInformationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Hospital Branch Information that has the same id as the given id.
	 *
	 * @param id The id of the HospitalBranchInformationEntity to be deleted
	...
	@Operation(
			description = "Delete a single HospitalBranchInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the HospitalBranchInformation to delete.")
		...
		hospitalBranchInformationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportHospitalBranchInformation' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Hospital Branch Information entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportHospitalBranchInformation' endpoint here] end
	public void exportHospitalBranchInformation(
			@Parameter(description = "The ids of the HospitalBranchInformationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<HospitalBranchInformationEntity> entitiesToExport = hospitalBranchInformationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "HospitalBranchInformation_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (HospitalBranchInformationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportHospitalBranchInformationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportHospitalBranchInformationExcludingIds' endpoint here] end
	public void exportHospitalBranchInformationExcludingIds(
			@Parameter(description = "The ids of the HospitalBranchInformationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<HospitalBranchInformationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = hospitalBranchInformationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = hospitalBranchInformationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the HospitalBranchInformation entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "HospitalBranchInformation_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(HospitalBranchInformationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Hospital Branch Information entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('HospitalBranchInformationEntity', 'create')  || hasPermission('HospitalBranchInformationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with HospitalBranchInformationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("HospitalBranchInformationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(HospitalBranchInformationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<HospitalBranchInformationEntity> csvToBean = new CsvToBeanBuilder<HospitalBranchInformationEntity>(reader)
				.withType(HospitalBranchInformationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<HospitalBranchInformationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<HospitalBranchInformationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Hospital Branch Information entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Hospital Branch Information entity and you do not have permission");
					}
