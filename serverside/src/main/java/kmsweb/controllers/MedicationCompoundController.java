...
import kmsweb.entities.MedicationCompoundEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medication Compound
 */
// % protected region % [Update the API route or description for the Medication CompoundController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicationCompoundController"))
@RestController
@Slf4j
@RequestMapping("/api/medicationcompound")
// % protected region % [Update the API route or description for the Medication CompoundController here] end
public class MedicationCompoundController {

	private final MedicationCompoundService medicationCompoundService;
	private final CsvUtils<MedicationCompoundEntity, MedicationCompoundRepository, MedicationCompoundEntityAudit,  MedicationCompoundService> csvUtils;
	...
	@Autowired
	public MedicationCompoundController(
		...
		this.csvUtils = new CsvUtils<>(medicationCompoundService, MedicationCompoundEntity.class);
	...
	public ResponseEntity<List<MedicationCompoundEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicationCompoundEntity> medicationCompounds = medicationCompoundService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicationCompounds, HttpStatus.OK);
	}

	/**
	 * Return the Medication Compound that has the same id as the given id.
	 *
	 * @param id      The id of the MedicationCompoundEntity to be returned
	 * @return the Medication Compound that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicationCompound as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicationCompound entity not found"),
	})
	@PreAuthorize("hasPermission('MedicationCompoundEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicationCompoundEntity> getById(
			@Parameter(description = "The UUID of the MedicationCompound to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicationCompoundEntity> medicationCompoundEntity = medicationCompoundService.findById(id);
	...
	/**
	 * Create/Update the given Medication Compound. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicationCompoundEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicationCompound' endpoint here] off begin
	@Operation(
			description = "Create a MedicationCompound if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicationCompound."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicationCompound, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicationCompoundEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicationCompound' endpoint here] end
	public ResponseEntity<?> saveMedicationCompound(
			@Parameter(description = "The MedicationCompoundEntity to create or update.")
			@RequestBody MedicationCompoundEntityDto medicationCompoundEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicationCompoundEntity medicationCompoundEntity = medicationCompoundService.create(new MedicationCompoundEntity(medicationCompoundEntityDto));
			return new ResponseEntity<>(medicationCompoundEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medication Compound. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicationCompoundEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicationCompound' endpoint here] off begin
	@Operation(
			description = "Update a MedicationCompound entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicationCompound."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicationCompound, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicationCompoundEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicationCompound' endpoint here] end
	public ResponseEntity<?> updateMedicationCompound(
			@Parameter(description = "The MedicationCompoundEntity to create or update.")
			@RequestBody MedicationCompoundEntityDto medicationCompoundEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicationCompoundEntity> checkMedicationCompoundExists = medicationCompoundService.findById(medicationCompoundEntityDto.getId());
		MedicationCompoundEntity medicationCompoundEntity = new MedicationCompoundEntity(medicationCompoundEntityDto);

		if (checkMedicationCompoundExists.isPresent()) {
			try {
				medicationCompoundEntity = medicationCompoundService.update(medicationCompoundEntity);
				return new ResponseEntity<>(medicationCompoundEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicationCompoundEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medication Compound that has the same id as the given id.
	 *
	 * @param id The id of the MedicationCompoundEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicationCompound as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicationCompound to delete.")
		...
		medicationCompoundService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationCompound' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medication Compound entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationCompound' endpoint here] end
	public void exportMedicationCompound(
			@Parameter(description = "The ids of the MedicationCompoundEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicationCompoundEntity> entitiesToExport = medicationCompoundService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicationCompound_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicationCompoundEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationCompoundExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationCompoundExcludingIds' endpoint here] end
	public void exportMedicationCompoundExcludingIds(
			@Parameter(description = "The ids of the MedicationCompoundEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicationCompoundEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicationCompoundService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicationCompoundService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicationCompound entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicationCompound_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicationCompoundEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medication Compound entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicationCompoundEntity', 'create')  || hasPermission('MedicationCompoundEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicationCompoundEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicationCompoundEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicationCompoundEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicationCompoundEntity> csvToBean = new CsvToBeanBuilder<MedicationCompoundEntity>(reader)
				.withType(MedicationCompoundEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicationCompoundEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicationCompoundEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medication Compound entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medication Compound entity and you do not have permission");
					}
