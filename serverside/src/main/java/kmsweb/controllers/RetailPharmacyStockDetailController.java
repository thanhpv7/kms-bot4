...
/**
 * Controller used to handle all REST operations regarding Retail Pharmacy Stock Detail
 */
// % protected region % [Update the API route or description for the Retail Pharmacy Stock DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RetailPharmacyStockDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/retailpharmacystockdetail")
// % protected region % [Update the API route or description for the Retail Pharmacy Stock DetailController here] end
public class RetailPharmacyStockDetailController {

	private final RetailPharmacyStockDetailService retailPharmacyStockDetailService;
	private final CsvUtils<RetailPharmacyStockDetailEntity, RetailPharmacyStockDetailRepository, RetailPharmacyStockDetailEntityAudit,  RetailPharmacyStockDetailService> csvUtils;
	...
	@Autowired
	public RetailPharmacyStockDetailController(
		...
		this.csvUtils = new CsvUtils<>(retailPharmacyStockDetailService, RetailPharmacyStockDetailEntity.class);
	...
	/**
	 * Return all the Retail Pharmacy Stock Details.
	 *
	 * @return all the Retail Pharmacy Stock Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of RetailPharmacyStockDetails",
		security = {
	...
	public ResponseEntity<List<RetailPharmacyStockDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<RetailPharmacyStockDetailEntity> retailPharmacyStockDetails = retailPharmacyStockDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(retailPharmacyStockDetails, HttpStatus.OK);
	}

	/**
	 * Return the Retail Pharmacy Stock Detail that has the same id as the given id.
	 *
	 * @param id      The id of the RetailPharmacyStockDetailEntity to be returned
	 * @return the Retail Pharmacy Stock Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single RetailPharmacyStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "RetailPharmacyStockDetail entity not found"),
	})
	@PreAuthorize("hasPermission('RetailPharmacyStockDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<RetailPharmacyStockDetailEntity> getById(
			@Parameter(description = "The UUID of the RetailPharmacyStockDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<RetailPharmacyStockDetailEntity> retailPharmacyStockDetailEntity = retailPharmacyStockDetailService.findById(id);
	...
	/**
	 * Create/Update the given Retail Pharmacy Stock Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param retailPharmacyStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRetailPharmacyStockDetail' endpoint here] off begin
	@Operation(
			description = "Create a RetailPharmacyStockDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new RetailPharmacyStockDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new RetailPharmacyStockDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('RetailPharmacyStockDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRetailPharmacyStockDetail' endpoint here] end
	public ResponseEntity<?> saveRetailPharmacyStockDetail(
			@Parameter(description = "The RetailPharmacyStockDetailEntity to create or update.")
			@RequestBody RetailPharmacyStockDetailEntityDto retailPharmacyStockDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RetailPharmacyStockDetailEntity retailPharmacyStockDetailEntity = retailPharmacyStockDetailService.create(new RetailPharmacyStockDetailEntity(retailPharmacyStockDetailEntityDto));
			return new ResponseEntity<>(retailPharmacyStockDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Retail Pharmacy Stock Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param retailPharmacyStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRetailPharmacyStockDetail' endpoint here] off begin
	@Operation(
			description = "Update a RetailPharmacyStockDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the RetailPharmacyStockDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the RetailPharmacyStockDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RetailPharmacyStockDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRetailPharmacyStockDetail' endpoint here] end
	public ResponseEntity<?> updateRetailPharmacyStockDetail(
			@Parameter(description = "The RetailPharmacyStockDetailEntity to create or update.")
			@RequestBody RetailPharmacyStockDetailEntityDto retailPharmacyStockDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<RetailPharmacyStockDetailEntity> checkRetailPharmacyStockDetailExists = retailPharmacyStockDetailService.findById(retailPharmacyStockDetailEntityDto.getId());
		RetailPharmacyStockDetailEntity retailPharmacyStockDetailEntity = new RetailPharmacyStockDetailEntity(retailPharmacyStockDetailEntityDto);

		if (checkRetailPharmacyStockDetailExists.isPresent()) {
			try {
				retailPharmacyStockDetailEntity = retailPharmacyStockDetailService.update(retailPharmacyStockDetailEntity);
				return new ResponseEntity<>(retailPharmacyStockDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(retailPharmacyStockDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Retail Pharmacy Stock Detail that has the same id as the given id.
	 *
	 * @param id The id of the RetailPharmacyStockDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single RetailPharmacyStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the RetailPharmacyStockDetail to delete.")
		...
		retailPharmacyStockDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyStockDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Retail Pharmacy Stock Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyStockDetail' endpoint here] end
	public void exportRetailPharmacyStockDetail(
			@Parameter(description = "The ids of the RetailPharmacyStockDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RetailPharmacyStockDetailEntity> entitiesToExport = retailPharmacyStockDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "RetailPharmacyStockDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RetailPharmacyStockDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyStockDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyStockDetailExcludingIds' endpoint here] end
	public void exportRetailPharmacyStockDetailExcludingIds(
			@Parameter(description = "The ids of the RetailPharmacyStockDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RetailPharmacyStockDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = retailPharmacyStockDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = retailPharmacyStockDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the RetailPharmacyStockDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "RetailPharmacyStockDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RetailPharmacyStockDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Retail Pharmacy Stock Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RetailPharmacyStockDetailEntity', 'create')  || hasPermission('RetailPharmacyStockDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RetailPharmacyStockDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RetailPharmacyStockDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RetailPharmacyStockDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RetailPharmacyStockDetailEntity> csvToBean = new CsvToBeanBuilder<RetailPharmacyStockDetailEntity>(reader)
				.withType(RetailPharmacyStockDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<RetailPharmacyStockDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<RetailPharmacyStockDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Retail Pharmacy Stock Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Retail Pharmacy Stock Detail entity and you do not have permission");
					}
