...
/**
 * Controller used to handle all REST operations regarding Medication Administered History
 */
// % protected region % [Update the API route or description for the Medication Administered HistoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicationAdministeredHistoryController"))
@RestController
@Slf4j
@RequestMapping("/api/medicationadministeredhistory")
// % protected region % [Update the API route or description for the Medication Administered HistoryController here] end
public class MedicationAdministeredHistoryController {

	private final MedicationAdministeredHistoryService medicationAdministeredHistoryService;
	private final CsvUtils<MedicationAdministeredHistoryEntity, MedicationAdministeredHistoryRepository, MedicationAdministeredHistoryEntityAudit,  MedicationAdministeredHistoryService> csvUtils;
	...
	@Autowired
	public MedicationAdministeredHistoryController(
		...
		this.csvUtils = new CsvUtils<>(medicationAdministeredHistoryService, MedicationAdministeredHistoryEntity.class);
	...
	/**
	 * Return all the Medication Administered Historys.
	 *
	 * @return all the Medication Administered Historys
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of MedicationAdministeredHistorys",
		security = {
	...
	public ResponseEntity<List<MedicationAdministeredHistoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicationAdministeredHistoryEntity> medicationAdministeredHistorys = medicationAdministeredHistoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicationAdministeredHistorys, HttpStatus.OK);
	}

	/**
	 * Return the Medication Administered History that has the same id as the given id.
	 *
	 * @param id      The id of the MedicationAdministeredHistoryEntity to be returned
	 * @return the Medication Administered History that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicationAdministeredHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicationAdministeredHistory entity not found"),
	})
	@PreAuthorize("hasPermission('MedicationAdministeredHistoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicationAdministeredHistoryEntity> getById(
			@Parameter(description = "The UUID of the MedicationAdministeredHistory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicationAdministeredHistoryEntity> medicationAdministeredHistoryEntity = medicationAdministeredHistoryService.findById(id);
	...
	/**
	 * Create/Update the given Medication Administered History. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicationAdministeredHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicationAdministeredHistory' endpoint here] off begin
	@Operation(
			description = "Create a MedicationAdministeredHistory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicationAdministeredHistory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicationAdministeredHistory, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicationAdministeredHistoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicationAdministeredHistory' endpoint here] end
	public ResponseEntity<?> saveMedicationAdministeredHistory(
			@Parameter(description = "The MedicationAdministeredHistoryEntity to create or update.")
			@RequestBody MedicationAdministeredHistoryEntityDto medicationAdministeredHistoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicationAdministeredHistoryEntity medicationAdministeredHistoryEntity = medicationAdministeredHistoryService.create(new MedicationAdministeredHistoryEntity(medicationAdministeredHistoryEntityDto));
			return new ResponseEntity<>(medicationAdministeredHistoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medication Administered History. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicationAdministeredHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicationAdministeredHistory' endpoint here] off begin
	@Operation(
			description = "Update a MedicationAdministeredHistory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicationAdministeredHistory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicationAdministeredHistory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicationAdministeredHistoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicationAdministeredHistory' endpoint here] end
	public ResponseEntity<?> updateMedicationAdministeredHistory(
			@Parameter(description = "The MedicationAdministeredHistoryEntity to create or update.")
			@RequestBody MedicationAdministeredHistoryEntityDto medicationAdministeredHistoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicationAdministeredHistoryEntity> checkMedicationAdministeredHistoryExists = medicationAdministeredHistoryService.findById(medicationAdministeredHistoryEntityDto.getId());
		MedicationAdministeredHistoryEntity medicationAdministeredHistoryEntity = new MedicationAdministeredHistoryEntity(medicationAdministeredHistoryEntityDto);

		if (checkMedicationAdministeredHistoryExists.isPresent()) {
			try {
				medicationAdministeredHistoryEntity = medicationAdministeredHistoryService.update(medicationAdministeredHistoryEntity);
				return new ResponseEntity<>(medicationAdministeredHistoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicationAdministeredHistoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medication Administered History that has the same id as the given id.
	 *
	 * @param id The id of the MedicationAdministeredHistoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicationAdministeredHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicationAdministeredHistory to delete.")
		...
		medicationAdministeredHistoryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationAdministeredHistory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medication Administered History entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationAdministeredHistory' endpoint here] end
	public void exportMedicationAdministeredHistory(
			@Parameter(description = "The ids of the MedicationAdministeredHistoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicationAdministeredHistoryEntity> entitiesToExport = medicationAdministeredHistoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicationAdministeredHistory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicationAdministeredHistoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationAdministeredHistoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationAdministeredHistoryExcludingIds' endpoint here] end
	public void exportMedicationAdministeredHistoryExcludingIds(
			@Parameter(description = "The ids of the MedicationAdministeredHistoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicationAdministeredHistoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicationAdministeredHistoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicationAdministeredHistoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicationAdministeredHistory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicationAdministeredHistory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicationAdministeredHistoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medication Administered History entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicationAdministeredHistoryEntity', 'create')  || hasPermission('MedicationAdministeredHistoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicationAdministeredHistoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicationAdministeredHistoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicationAdministeredHistoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicationAdministeredHistoryEntity> csvToBean = new CsvToBeanBuilder<MedicationAdministeredHistoryEntity>(reader)
				.withType(MedicationAdministeredHistoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicationAdministeredHistoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicationAdministeredHistoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medication Administered History entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medication Administered History entity and you do not have permission");
					}
