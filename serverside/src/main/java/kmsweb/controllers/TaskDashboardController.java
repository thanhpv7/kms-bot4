...
import kmsweb.entities.TaskDashboardEntityAudit;
import kmsweb.repositories.TaskDashboardRepository;
import kmsweb.services.TaskDashboardService;
...
/**
 * Controller used to handle all REST operations regarding Task Dashboard
 */
// % protected region % [Update the API route or description for the Task DashboardController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TaskDashboardController"))
@RestController
@Slf4j
@RequestMapping("/api/taskdashboard")
// % protected region % [Update the API route or description for the Task DashboardController here] end
public class TaskDashboardController {

	private final TaskDashboardService taskDashboardService;
	private final CsvUtils<TaskDashboardEntity, TaskDashboardRepository, TaskDashboardEntityAudit,  TaskDashboardService> csvUtils;
	...
	public TaskDashboardController(
		...
		this.csvUtils = new CsvUtils<>(taskDashboardService, TaskDashboardEntity.class);
	...
	public ResponseEntity<List<TaskDashboardEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TaskDashboardEntity> taskDashboards = taskDashboardService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(taskDashboards, HttpStatus.OK);
	}

	/**
	 * Return the Task Dashboard that has the same id as the given id.
	 *
	 * @param id      The id of the TaskDashboardEntity to be returned
	 * @return the Task Dashboard that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TaskDashboard as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TaskDashboard entity not found"),
	})
	@PreAuthorize("hasPermission('TaskDashboardEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TaskDashboardEntity> getById(
			@Parameter(description = "The UUID of the TaskDashboard to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TaskDashboardEntity> taskDashboardEntity = taskDashboardService.findById(id);
	...
	/**
	 * Create/Update the given Task Dashboard. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param taskDashboardEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTaskDashboard' endpoint here] off begin
	@Operation(
			description = "Create a TaskDashboard if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TaskDashboard."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TaskDashboard, duplicate record found")
	})
	@PreAuthorize("hasPermission('TaskDashboardEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTaskDashboard' endpoint here] end
	public ResponseEntity<?> saveTaskDashboard(
			@Parameter(description = "The TaskDashboardEntity to create or update.")
			@RequestBody TaskDashboardEntityDto taskDashboardEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TaskDashboardEntity taskDashboardEntity = taskDashboardService.create(new TaskDashboardEntity(taskDashboardEntityDto));
			return new ResponseEntity<>(taskDashboardEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Task Dashboard. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param taskDashboardEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTaskDashboard' endpoint here] off begin
	@Operation(
			description = "Update a TaskDashboard entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TaskDashboard."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TaskDashboard, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TaskDashboardEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTaskDashboard' endpoint here] end
	public ResponseEntity<?> updateTaskDashboard(
			@Parameter(description = "The TaskDashboardEntity to create or update.")
			@RequestBody TaskDashboardEntityDto taskDashboardEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TaskDashboardEntity> checkTaskDashboardExists = taskDashboardService.findById(taskDashboardEntityDto.getId());
		TaskDashboardEntity taskDashboardEntity = new TaskDashboardEntity(taskDashboardEntityDto);

		if (checkTaskDashboardExists.isPresent()) {
			try {
				taskDashboardEntity = taskDashboardService.update(taskDashboardEntity);
				return new ResponseEntity<>(taskDashboardEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(taskDashboardEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Task Dashboard that has the same id as the given id.
	 *
	 * @param id The id of the TaskDashboardEntity to be deleted
	...
	@Operation(
			description = "Delete a single TaskDashboard as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TaskDashboard to delete.")
		...
		taskDashboardService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTaskDashboard' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Task Dashboard entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTaskDashboard' endpoint here] end
	public void exportTaskDashboard(
			@Parameter(description = "The ids of the TaskDashboardEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TaskDashboardEntity> entitiesToExport = taskDashboardService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TaskDashboard_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TaskDashboardEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTaskDashboardExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTaskDashboardExcludingIds' endpoint here] end
	public void exportTaskDashboardExcludingIds(
			@Parameter(description = "The ids of the TaskDashboardEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TaskDashboardEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = taskDashboardService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = taskDashboardService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TaskDashboard entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TaskDashboard_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TaskDashboardEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Task Dashboard entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TaskDashboardEntity', 'create')  || hasPermission('TaskDashboardEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TaskDashboardEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TaskDashboardEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TaskDashboardEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TaskDashboardEntity> csvToBean = new CsvToBeanBuilder<TaskDashboardEntity>(reader)
				.withType(TaskDashboardEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TaskDashboardEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TaskDashboardEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Task Dashboard entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Task Dashboard entity and you do not have permission");
					}
