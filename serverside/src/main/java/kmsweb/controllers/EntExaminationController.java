...
/**
 * Controller used to handle all REST operations regarding ENT Examination
 */
// % protected region % [Update the API route or description for the ENT ExaminationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "EntExaminationController"))
@RestController
@Slf4j
@RequestMapping("/api/entexamination")
// % protected region % [Update the API route or description for the ENT ExaminationController here] end
public class EntExaminationController {

	private final EntExaminationService entExaminationService;
	private final CsvUtils<EntExaminationEntity, EntExaminationRepository, EntExaminationEntityAudit,  EntExaminationService> csvUtils;
	...
	@Autowired
	public EntExaminationController(
		...
		this.csvUtils = new CsvUtils<>(entExaminationService, EntExaminationEntity.class);
	...
	public ResponseEntity<List<EntExaminationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<EntExaminationEntity> entExaminations = entExaminationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(entExaminations, HttpStatus.OK);
	}

	/**
	 * Return the ENT Examination that has the same id as the given id.
	 *
	 * @param id      The id of the EntExaminationEntity to be returned
	 * @return the ENT Examination that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single EntExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "EntExamination entity not found"),
	})
	@PreAuthorize("hasPermission('EntExaminationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<EntExaminationEntity> getById(
			@Parameter(description = "The UUID of the EntExamination to return.")
			@PathVariable("id") UUID id
	) {
		Optional<EntExaminationEntity> entExaminationEntity = entExaminationService.findById(id);
	...
	/**
	 * Create/Update the given ENT Examination. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param entExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveEntExamination' endpoint here] off begin
	@Operation(
			description = "Create a EntExamination if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new EntExamination."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new EntExamination, duplicate record found")
	})
	@PreAuthorize("hasPermission('EntExaminationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveEntExamination' endpoint here] end
	public ResponseEntity<?> saveEntExamination(
			@Parameter(description = "The EntExaminationEntity to create or update.")
			@RequestBody EntExaminationEntityDto entExaminationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			EntExaminationEntity entExaminationEntity = entExaminationService.create(new EntExaminationEntity(entExaminationEntityDto));
			return new ResponseEntity<>(entExaminationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given ENT Examination. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param entExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateEntExamination' endpoint here] off begin
	@Operation(
			description = "Update a EntExamination entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the EntExamination."),
		@ApiResponse(responseCode = "400", description = "Failed to update the EntExamination, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('EntExaminationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateEntExamination' endpoint here] end
	public ResponseEntity<?> updateEntExamination(
			@Parameter(description = "The EntExaminationEntity to create or update.")
			@RequestBody EntExaminationEntityDto entExaminationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<EntExaminationEntity> checkEntExaminationExists = entExaminationService.findById(entExaminationEntityDto.getId());
		EntExaminationEntity entExaminationEntity = new EntExaminationEntity(entExaminationEntityDto);

		if (checkEntExaminationExists.isPresent()) {
			try {
				entExaminationEntity = entExaminationService.update(entExaminationEntity);
				return new ResponseEntity<>(entExaminationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(entExaminationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the ENT Examination that has the same id as the given id.
	 *
	 * @param id The id of the EntExaminationEntity to be deleted
	...
	@Operation(
			description = "Delete a single EntExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the EntExamination to delete.")
		...
		entExaminationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportEntExamination' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all ENT Examination entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportEntExamination' endpoint here] end
	public void exportEntExamination(
			@Parameter(description = "The ids of the EntExaminationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<EntExaminationEntity> entitiesToExport = entExaminationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "EntExamination_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (EntExaminationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportEntExaminationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportEntExaminationExcludingIds' endpoint here] end
	public void exportEntExaminationExcludingIds(
			@Parameter(description = "The ids of the EntExaminationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<EntExaminationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = entExaminationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = entExaminationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the EntExamination entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "EntExamination_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(EntExaminationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for ENT Examination entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('EntExaminationEntity', 'create')  || hasPermission('EntExaminationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with EntExaminationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("EntExaminationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(EntExaminationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<EntExaminationEntity> csvToBean = new CsvToBeanBuilder<EntExaminationEntity>(reader)
				.withType(EntExaminationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<EntExaminationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<EntExaminationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a ENT Examination entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a ENT Examination entity and you do not have permission");
					}
