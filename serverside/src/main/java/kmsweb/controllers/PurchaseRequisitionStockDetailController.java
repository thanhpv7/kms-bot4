...
import kmsweb.entities.PurchaseRequisitionStockDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Purchase Requisition Stock Detail
 */
// % protected region % [Update the API route or description for the Purchase Requisition Stock DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PurchaseRequisitionStockDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/purchaserequisitionstockdetail")
// % protected region % [Update the API route or description for the Purchase Requisition Stock DetailController here] end
public class PurchaseRequisitionStockDetailController {

	private final PurchaseRequisitionStockDetailService purchaseRequisitionStockDetailService;
	private final CsvUtils<PurchaseRequisitionStockDetailEntity, PurchaseRequisitionStockDetailRepository, PurchaseRequisitionStockDetailEntityAudit,  PurchaseRequisitionStockDetailService> csvUtils;
	...
	@Autowired
	public PurchaseRequisitionStockDetailController(
		...
		this.csvUtils = new CsvUtils<>(purchaseRequisitionStockDetailService, PurchaseRequisitionStockDetailEntity.class);
	...
	/**
	 * Return all the Purchase Requisition Stock Details.
	 *
	 * @return all the Purchase Requisition Stock Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of PurchaseRequisitionStockDetails",
		security = {
	...
	public ResponseEntity<List<PurchaseRequisitionStockDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PurchaseRequisitionStockDetailEntity> purchaseRequisitionStockDetails = purchaseRequisitionStockDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(purchaseRequisitionStockDetails, HttpStatus.OK);
	}

	/**
	 * Return the Purchase Requisition Stock Detail that has the same id as the given id.
	 *
	 * @param id      The id of the PurchaseRequisitionStockDetailEntity to be returned
	 * @return the Purchase Requisition Stock Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PurchaseRequisitionStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PurchaseRequisitionStockDetail entity not found"),
	})
	@PreAuthorize("hasPermission('PurchaseRequisitionStockDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PurchaseRequisitionStockDetailEntity> getById(
			@Parameter(description = "The UUID of the PurchaseRequisitionStockDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PurchaseRequisitionStockDetailEntity> purchaseRequisitionStockDetailEntity = purchaseRequisitionStockDetailService.findById(id);
	...
	/**
	 * Create/Update the given Purchase Requisition Stock Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param purchaseRequisitionStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePurchaseRequisitionStockDetail' endpoint here] off begin
	@Operation(
			description = "Create a PurchaseRequisitionStockDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PurchaseRequisitionStockDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PurchaseRequisitionStockDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('PurchaseRequisitionStockDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePurchaseRequisitionStockDetail' endpoint here] end
	public ResponseEntity<?> savePurchaseRequisitionStockDetail(
			@Parameter(description = "The PurchaseRequisitionStockDetailEntity to create or update.")
			@RequestBody PurchaseRequisitionStockDetailEntityDto purchaseRequisitionStockDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PurchaseRequisitionStockDetailEntity purchaseRequisitionStockDetailEntity = purchaseRequisitionStockDetailService.create(new PurchaseRequisitionStockDetailEntity(purchaseRequisitionStockDetailEntityDto));
			return new ResponseEntity<>(purchaseRequisitionStockDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Purchase Requisition Stock Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param purchaseRequisitionStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePurchaseRequisitionStockDetail' endpoint here] off begin
	@Operation(
			description = "Update a PurchaseRequisitionStockDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PurchaseRequisitionStockDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PurchaseRequisitionStockDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PurchaseRequisitionStockDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePurchaseRequisitionStockDetail' endpoint here] end
	public ResponseEntity<?> updatePurchaseRequisitionStockDetail(
			@Parameter(description = "The PurchaseRequisitionStockDetailEntity to create or update.")
			@RequestBody PurchaseRequisitionStockDetailEntityDto purchaseRequisitionStockDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PurchaseRequisitionStockDetailEntity> checkPurchaseRequisitionStockDetailExists = purchaseRequisitionStockDetailService.findById(purchaseRequisitionStockDetailEntityDto.getId());
		PurchaseRequisitionStockDetailEntity purchaseRequisitionStockDetailEntity = new PurchaseRequisitionStockDetailEntity(purchaseRequisitionStockDetailEntityDto);

		if (checkPurchaseRequisitionStockDetailExists.isPresent()) {
			try {
				purchaseRequisitionStockDetailEntity = purchaseRequisitionStockDetailService.update(purchaseRequisitionStockDetailEntity);
				return new ResponseEntity<>(purchaseRequisitionStockDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(purchaseRequisitionStockDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Purchase Requisition Stock Detail that has the same id as the given id.
	 *
	 * @param id The id of the PurchaseRequisitionStockDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single PurchaseRequisitionStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PurchaseRequisitionStockDetail to delete.")
		...
		purchaseRequisitionStockDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisitionStockDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Purchase Requisition Stock Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisitionStockDetail' endpoint here] end
	public void exportPurchaseRequisitionStockDetail(
			@Parameter(description = "The ids of the PurchaseRequisitionStockDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PurchaseRequisitionStockDetailEntity> entitiesToExport = purchaseRequisitionStockDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PurchaseRequisitionStockDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PurchaseRequisitionStockDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisitionStockDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisitionStockDetailExcludingIds' endpoint here] end
	public void exportPurchaseRequisitionStockDetailExcludingIds(
			@Parameter(description = "The ids of the PurchaseRequisitionStockDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PurchaseRequisitionStockDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = purchaseRequisitionStockDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = purchaseRequisitionStockDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PurchaseRequisitionStockDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PurchaseRequisitionStockDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PurchaseRequisitionStockDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Purchase Requisition Stock Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PurchaseRequisitionStockDetailEntity', 'create')  || hasPermission('PurchaseRequisitionStockDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PurchaseRequisitionStockDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PurchaseRequisitionStockDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PurchaseRequisitionStockDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PurchaseRequisitionStockDetailEntity> csvToBean = new CsvToBeanBuilder<PurchaseRequisitionStockDetailEntity>(reader)
				.withType(PurchaseRequisitionStockDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PurchaseRequisitionStockDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PurchaseRequisitionStockDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Purchase Requisition Stock Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Purchase Requisition Stock Detail entity and you do not have permission");
					}
