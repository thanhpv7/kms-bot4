...
import kmsweb.dtos.MedicalCertificateMentalHealthEntityDto;
import kmsweb.entities.MedicalCertificateMentalHealthEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medical Certificate Mental Health
 */
// % protected region % [Update the API route or description for the Medical Certificate Mental HealthController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicalCertificateMentalHealthController"))
@RestController
@Slf4j
@RequestMapping("/api/medicalcertificatementalhealth")
// % protected region % [Update the API route or description for the Medical Certificate Mental HealthController here] end
public class MedicalCertificateMentalHealthController {

	private final MedicalCertificateMentalHealthService medicalCertificateMentalHealthService;
	private final CsvUtils<MedicalCertificateMentalHealthEntity, MedicalCertificateMentalHealthRepository, MedicalCertificateMentalHealthEntityAudit,  MedicalCertificateMentalHealthService> csvUtils;
	...
	@Autowired
	public MedicalCertificateMentalHealthController(
		...
		this.csvUtils = new CsvUtils<>(medicalCertificateMentalHealthService, MedicalCertificateMentalHealthEntity.class);
	...
	/**
	 * Return all the Medical Certificate Mental Healths.
	 *
	 * @return all the Medical Certificate Mental Healths
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of MedicalCertificateMentalHealths",
		security = {
	...
	public ResponseEntity<List<MedicalCertificateMentalHealthEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicalCertificateMentalHealthEntity> medicalCertificateMentalHealths = medicalCertificateMentalHealthService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicalCertificateMentalHealths, HttpStatus.OK);
	}

	/**
	 * Return the Medical Certificate Mental Health that has the same id as the given id.
	 *
	 * @param id      The id of the MedicalCertificateMentalHealthEntity to be returned
	 * @return the Medical Certificate Mental Health that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicalCertificateMentalHealth as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicalCertificateMentalHealth entity not found"),
	})
	@PreAuthorize("hasPermission('MedicalCertificateMentalHealthEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicalCertificateMentalHealthEntity> getById(
			@Parameter(description = "The UUID of the MedicalCertificateMentalHealth to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicalCertificateMentalHealthEntity> medicalCertificateMentalHealthEntity = medicalCertificateMentalHealthService.findById(id);
	...
	/**
	 * Create/Update the given Medical Certificate Mental Health. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicalCertificateMentalHealthEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicalCertificateMentalHealth' endpoint here] off begin
	@Operation(
			description = "Create a MedicalCertificateMentalHealth if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicalCertificateMentalHealth."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicalCertificateMentalHealth, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicalCertificateMentalHealthEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicalCertificateMentalHealth' endpoint here] end
	public ResponseEntity<?> saveMedicalCertificateMentalHealth(
			@Parameter(description = "The MedicalCertificateMentalHealthEntity to create or update.")
			@RequestBody MedicalCertificateMentalHealthEntityDto medicalCertificateMentalHealthEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicalCertificateMentalHealthEntity medicalCertificateMentalHealthEntity = medicalCertificateMentalHealthService.create(new MedicalCertificateMentalHealthEntity(medicalCertificateMentalHealthEntityDto));
			return new ResponseEntity<>(medicalCertificateMentalHealthEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medical Certificate Mental Health. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicalCertificateMentalHealthEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicalCertificateMentalHealth' endpoint here] off begin
	@Operation(
			description = "Update a MedicalCertificateMentalHealth entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicalCertificateMentalHealth."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicalCertificateMentalHealth, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicalCertificateMentalHealthEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicalCertificateMentalHealth' endpoint here] end
	public ResponseEntity<?> updateMedicalCertificateMentalHealth(
			@Parameter(description = "The MedicalCertificateMentalHealthEntity to create or update.")
			@RequestBody MedicalCertificateMentalHealthEntityDto medicalCertificateMentalHealthEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicalCertificateMentalHealthEntity> checkMedicalCertificateMentalHealthExists = medicalCertificateMentalHealthService.findById(medicalCertificateMentalHealthEntityDto.getId());
		MedicalCertificateMentalHealthEntity medicalCertificateMentalHealthEntity = new MedicalCertificateMentalHealthEntity(medicalCertificateMentalHealthEntityDto);

		if (checkMedicalCertificateMentalHealthExists.isPresent()) {
			try {
				medicalCertificateMentalHealthEntity = medicalCertificateMentalHealthService.update(medicalCertificateMentalHealthEntity);
				return new ResponseEntity<>(medicalCertificateMentalHealthEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicalCertificateMentalHealthEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medical Certificate Mental Health that has the same id as the given id.
	 *
	 * @param id The id of the MedicalCertificateMentalHealthEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicalCertificateMentalHealth as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicalCertificateMentalHealth to delete.")
		...
		medicalCertificateMentalHealthService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateMentalHealth' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medical Certificate Mental Health entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateMentalHealth' endpoint here] end
	public void exportMedicalCertificateMentalHealth(
			@Parameter(description = "The ids of the MedicalCertificateMentalHealthEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicalCertificateMentalHealthEntity> entitiesToExport = medicalCertificateMentalHealthService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicalCertificateMentalHealth_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicalCertificateMentalHealthEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateMentalHealthExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateMentalHealthExcludingIds' endpoint here] end
	public void exportMedicalCertificateMentalHealthExcludingIds(
			@Parameter(description = "The ids of the MedicalCertificateMentalHealthEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicalCertificateMentalHealthEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicalCertificateMentalHealthService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicalCertificateMentalHealthService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicalCertificateMentalHealth entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicalCertificateMentalHealth_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicalCertificateMentalHealthEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medical Certificate Mental Health entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicalCertificateMentalHealthEntity', 'create')  || hasPermission('MedicalCertificateMentalHealthEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicalCertificateMentalHealthEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicalCertificateMentalHealthEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicalCertificateMentalHealthEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicalCertificateMentalHealthEntity> csvToBean = new CsvToBeanBuilder<MedicalCertificateMentalHealthEntity>(reader)
				.withType(MedicalCertificateMentalHealthEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicalCertificateMentalHealthEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicalCertificateMentalHealthEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medical Certificate Mental Health entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medical Certificate Mental Health entity and you do not have permission");
					}
