...
import kmsweb.entities.ExaminationItemDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Examination Item Detail
 */
// % protected region % [Update the API route or description for the Examination Item DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ExaminationItemDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/examinationitemdetail")
// % protected region % [Update the API route or description for the Examination Item DetailController here] end
public class ExaminationItemDetailController {

	private final ExaminationItemDetailService examinationItemDetailService;
	private final CsvUtils<ExaminationItemDetailEntity, ExaminationItemDetailRepository, ExaminationItemDetailEntityAudit,  ExaminationItemDetailService> csvUtils;
	...
	@Autowired
	public ExaminationItemDetailController(
		...
		this.csvUtils = new CsvUtils<>(examinationItemDetailService, ExaminationItemDetailEntity.class);
	...
	public ResponseEntity<List<ExaminationItemDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ExaminationItemDetailEntity> examinationItemDetails = examinationItemDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(examinationItemDetails, HttpStatus.OK);
	}

	/**
	 * Return the Examination Item Detail that has the same id as the given id.
	 *
	 * @param id      The id of the ExaminationItemDetailEntity to be returned
	 * @return the Examination Item Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ExaminationItemDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ExaminationItemDetail entity not found"),
	})
	@PreAuthorize("hasPermission('ExaminationItemDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ExaminationItemDetailEntity> getById(
			@Parameter(description = "The UUID of the ExaminationItemDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ExaminationItemDetailEntity> examinationItemDetailEntity = examinationItemDetailService.findById(id);
	...
	/**
	 * Create/Update the given Examination Item Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param examinationItemDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveExaminationItemDetail' endpoint here] off begin
	@Operation(
			description = "Create a ExaminationItemDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ExaminationItemDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ExaminationItemDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('ExaminationItemDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveExaminationItemDetail' endpoint here] end
	public ResponseEntity<?> saveExaminationItemDetail(
			@Parameter(description = "The ExaminationItemDetailEntity to create or update.")
			@RequestBody ExaminationItemDetailEntityDto examinationItemDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ExaminationItemDetailEntity examinationItemDetailEntity = examinationItemDetailService.create(new ExaminationItemDetailEntity(examinationItemDetailEntityDto));
			return new ResponseEntity<>(examinationItemDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Examination Item Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param examinationItemDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateExaminationItemDetail' endpoint here] off begin
	@Operation(
			description = "Update a ExaminationItemDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ExaminationItemDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ExaminationItemDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ExaminationItemDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateExaminationItemDetail' endpoint here] end
	public ResponseEntity<?> updateExaminationItemDetail(
			@Parameter(description = "The ExaminationItemDetailEntity to create or update.")
			@RequestBody ExaminationItemDetailEntityDto examinationItemDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ExaminationItemDetailEntity> checkExaminationItemDetailExists = examinationItemDetailService.findById(examinationItemDetailEntityDto.getId());
		ExaminationItemDetailEntity examinationItemDetailEntity = new ExaminationItemDetailEntity(examinationItemDetailEntityDto);

		if (checkExaminationItemDetailExists.isPresent()) {
			try {
				examinationItemDetailEntity = examinationItemDetailService.update(examinationItemDetailEntity);
				return new ResponseEntity<>(examinationItemDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(examinationItemDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Examination Item Detail that has the same id as the given id.
	 *
	 * @param id The id of the ExaminationItemDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single ExaminationItemDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ExaminationItemDetail to delete.")
		...
		examinationItemDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportExaminationItemDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Examination Item Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportExaminationItemDetail' endpoint here] end
	public void exportExaminationItemDetail(
			@Parameter(description = "The ids of the ExaminationItemDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ExaminationItemDetailEntity> entitiesToExport = examinationItemDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ExaminationItemDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ExaminationItemDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportExaminationItemDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportExaminationItemDetailExcludingIds' endpoint here] end
	public void exportExaminationItemDetailExcludingIds(
			@Parameter(description = "The ids of the ExaminationItemDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ExaminationItemDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = examinationItemDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = examinationItemDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ExaminationItemDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ExaminationItemDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ExaminationItemDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Examination Item Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ExaminationItemDetailEntity', 'create')  || hasPermission('ExaminationItemDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ExaminationItemDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ExaminationItemDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ExaminationItemDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ExaminationItemDetailEntity> csvToBean = new CsvToBeanBuilder<ExaminationItemDetailEntity>(reader)
				.withType(ExaminationItemDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ExaminationItemDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ExaminationItemDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Examination Item Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Examination Item Detail entity and you do not have permission");
					}
