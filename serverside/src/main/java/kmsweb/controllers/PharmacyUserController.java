...
/**
 * Controller used to handle all REST operations regarding Pharmacy User
 */
// % protected region % [Update the API route or description for the Pharmacy UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PharmacyUserController"))
@RestController
@Slf4j
@RequestMapping("/api/pharmacyuser")
// % protected region % [Update the API route or description for the Pharmacy UserController here] end
public class PharmacyUserController {

	private final PharmacyUserService pharmacyUserService;
	private final CsvUtils<PharmacyUserEntity, PharmacyUserRepository, PharmacyUserEntityAudit,  PharmacyUserService> csvUtils;
	...
	public PharmacyUserController(
		...
		this.csvUtils = new CsvUtils<>(pharmacyUserService, PharmacyUserEntity.class);
	...
	public ResponseEntity<List<PharmacyUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PharmacyUserEntity> pharmacyUsers = pharmacyUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Pharmacy User that has the same id as the given id.
	 *
	 * @param id      The id of the PharmacyUserEntity to be returned
	 * @return the Pharmacy User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PharmacyUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PharmacyUser entity not found"),
	})
	@PreAuthorize("hasPermission('PharmacyUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PharmacyUserEntity> getById(
			@Parameter(description = "The UUID of the PharmacyUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PharmacyUserEntity> pharmacyUserEntity = pharmacyUserService.findById(id);
	...
	/**
	 * Create/Update the given Pharmacy User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pharmacyUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePharmacyUser' endpoint here] off begin
	@Operation(
			description = "Create a PharmacyUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PharmacyUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PharmacyUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('PharmacyUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePharmacyUser' endpoint here] end
	public ResponseEntity<?> savePharmacyUser(
			@Parameter(description = "The PharmacyUserEntity to create or update.")
			@RequestBody PharmacyUserEntityDto pharmacyUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PharmacyUserEntity pharmacyUserEntity = pharmacyUserService.create(new PharmacyUserEntity(pharmacyUserEntityDto));
			return new ResponseEntity<>(pharmacyUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Pharmacy User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pharmacyUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePharmacyUser' endpoint here] off begin
	@Operation(
			description = "Update a PharmacyUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PharmacyUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PharmacyUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PharmacyUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePharmacyUser' endpoint here] end
	public ResponseEntity<?> updatePharmacyUser(
			@Parameter(description = "The PharmacyUserEntity to create or update.")
			@RequestBody PharmacyUserEntityDto pharmacyUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PharmacyUserEntity> checkPharmacyUserExists = pharmacyUserService.findById(pharmacyUserEntityDto.getId());
		PharmacyUserEntity pharmacyUserEntity = new PharmacyUserEntity(pharmacyUserEntityDto);

		if (checkPharmacyUserExists.isPresent()) {
			try {
				pharmacyUserEntity = pharmacyUserService.update(pharmacyUserEntity);
				return new ResponseEntity<>(pharmacyUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pharmacyUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Pharmacy User that has the same id as the given id.
	 *
	 * @param id The id of the PharmacyUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single PharmacyUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PharmacyUser to delete.")
		...
		pharmacyUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPharmacyUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Pharmacy User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPharmacyUser' endpoint here] end
	public void exportPharmacyUser(
			@Parameter(description = "The ids of the PharmacyUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PharmacyUserEntity> entitiesToExport = pharmacyUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PharmacyUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PharmacyUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPharmacyUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPharmacyUserExcludingIds' endpoint here] end
	public void exportPharmacyUserExcludingIds(
			@Parameter(description = "The ids of the PharmacyUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PharmacyUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pharmacyUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pharmacyUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PharmacyUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PharmacyUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PharmacyUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Pharmacy User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PharmacyUserEntity', 'create')  || hasPermission('PharmacyUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PharmacyUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PharmacyUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PharmacyUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PharmacyUserEntity> csvToBean = new CsvToBeanBuilder<PharmacyUserEntity>(reader)
				.withType(PharmacyUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PharmacyUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PharmacyUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Pharmacy User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Pharmacy User entity and you do not have permission");
					}
