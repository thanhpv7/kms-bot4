...
import kmsweb.entities.RetailPharmacyEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Retail Pharmacy
 */
// % protected region % [Update the API route or description for the Retail PharmacyController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RetailPharmacyController"))
@RestController
@Slf4j
@RequestMapping("/api/retailpharmacy")
// % protected region % [Update the API route or description for the Retail PharmacyController here] end
public class RetailPharmacyController {

	private final RetailPharmacyService retailPharmacyService;
	private final CsvUtils<RetailPharmacyEntity, RetailPharmacyRepository, RetailPharmacyEntityAudit,  RetailPharmacyService> csvUtils;
	...
	@Autowired
	public RetailPharmacyController(
		...
		this.csvUtils = new CsvUtils<>(retailPharmacyService, RetailPharmacyEntity.class);
	...
	public ResponseEntity<List<RetailPharmacyEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<RetailPharmacyEntity> retailPharmacys = retailPharmacyService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(retailPharmacys, HttpStatus.OK);
	}

	/**
	 * Return the Retail Pharmacy that has the same id as the given id.
	 *
	 * @param id      The id of the RetailPharmacyEntity to be returned
	 * @return the Retail Pharmacy that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single RetailPharmacy as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "RetailPharmacy entity not found"),
	})
	@PreAuthorize("hasPermission('RetailPharmacyEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<RetailPharmacyEntity> getById(
			@Parameter(description = "The UUID of the RetailPharmacy to return.")
			@PathVariable("id") UUID id
	) {
		Optional<RetailPharmacyEntity> retailPharmacyEntity = retailPharmacyService.findById(id);
	...
	/**
	 * Create/Update the given Retail Pharmacy. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param retailPharmacyEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRetailPharmacy' endpoint here] off begin
	@Operation(
			description = "Create a RetailPharmacy if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new RetailPharmacy."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new RetailPharmacy, duplicate record found")
	})
	@PreAuthorize("hasPermission('RetailPharmacyEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRetailPharmacy' endpoint here] end
	public ResponseEntity<?> saveRetailPharmacy(
			@Parameter(description = "The RetailPharmacyEntity to create or update.")
			@RequestBody RetailPharmacyEntityDto retailPharmacyEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RetailPharmacyEntity retailPharmacyEntity = retailPharmacyService.create(new RetailPharmacyEntity(retailPharmacyEntityDto));
			return new ResponseEntity<>(retailPharmacyEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Retail Pharmacy. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param retailPharmacyEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRetailPharmacy' endpoint here] off begin
	@Operation(
			description = "Update a RetailPharmacy entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the RetailPharmacy."),
		@ApiResponse(responseCode = "400", description = "Failed to update the RetailPharmacy, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RetailPharmacyEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRetailPharmacy' endpoint here] end
	public ResponseEntity<?> updateRetailPharmacy(
			@Parameter(description = "The RetailPharmacyEntity to create or update.")
			@RequestBody RetailPharmacyEntityDto retailPharmacyEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<RetailPharmacyEntity> checkRetailPharmacyExists = retailPharmacyService.findById(retailPharmacyEntityDto.getId());
		RetailPharmacyEntity retailPharmacyEntity = new RetailPharmacyEntity(retailPharmacyEntityDto);

		if (checkRetailPharmacyExists.isPresent()) {
			try {
				retailPharmacyEntity = retailPharmacyService.update(retailPharmacyEntity);
				return new ResponseEntity<>(retailPharmacyEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(retailPharmacyEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Retail Pharmacy that has the same id as the given id.
	 *
	 * @param id The id of the RetailPharmacyEntity to be deleted
	...
	@Operation(
			description = "Delete a single RetailPharmacy as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the RetailPharmacy to delete.")
		...
		retailPharmacyService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacy' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Retail Pharmacy entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacy' endpoint here] end
	public void exportRetailPharmacy(
			@Parameter(description = "The ids of the RetailPharmacyEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RetailPharmacyEntity> entitiesToExport = retailPharmacyService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "RetailPharmacy_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RetailPharmacyEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyExcludingIds' endpoint here] end
	public void exportRetailPharmacyExcludingIds(
			@Parameter(description = "The ids of the RetailPharmacyEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RetailPharmacyEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = retailPharmacyService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = retailPharmacyService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the RetailPharmacy entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "RetailPharmacy_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RetailPharmacyEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Retail Pharmacy entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RetailPharmacyEntity', 'create')  || hasPermission('RetailPharmacyEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RetailPharmacyEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RetailPharmacyEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RetailPharmacyEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RetailPharmacyEntity> csvToBean = new CsvToBeanBuilder<RetailPharmacyEntity>(reader)
				.withType(RetailPharmacyEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<RetailPharmacyEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<RetailPharmacyEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Retail Pharmacy entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Retail Pharmacy entity and you do not have permission");
					}
