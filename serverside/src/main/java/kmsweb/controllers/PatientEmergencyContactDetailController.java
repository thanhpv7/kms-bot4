...
/**
 * Controller used to handle all REST operations regarding Patient Emergency Contact Detail
 */
// % protected region % [Update the API route or description for the Patient Emergency Contact DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientEmergencyContactDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/patientemergencycontactdetail")
// % protected region % [Update the API route or description for the Patient Emergency Contact DetailController here] end
public class PatientEmergencyContactDetailController {

	private final PatientEmergencyContactDetailService patientEmergencyContactDetailService;
	private final CsvUtils<PatientEmergencyContactDetailEntity, PatientEmergencyContactDetailRepository, PatientEmergencyContactDetailEntityAudit,  PatientEmergencyContactDetailService> csvUtils;
	...
	@Autowired
	public PatientEmergencyContactDetailController(
		...
		this.csvUtils = new CsvUtils<>(patientEmergencyContactDetailService, PatientEmergencyContactDetailEntity.class);
	...
	/**
	 * Return all the Patient Emergency Contact Details.
	 *
	 * @return all the Patient Emergency Contact Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of PatientEmergencyContactDetails",
		security = {
	...
	public ResponseEntity<List<PatientEmergencyContactDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientEmergencyContactDetailEntity> patientEmergencyContactDetails = patientEmergencyContactDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientEmergencyContactDetails, HttpStatus.OK);
	}

	/**
	 * Return the Patient Emergency Contact Detail that has the same id as the given id.
	 *
	 * @param id      The id of the PatientEmergencyContactDetailEntity to be returned
	 * @return the Patient Emergency Contact Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientEmergencyContactDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientEmergencyContactDetail entity not found"),
	})
	@PreAuthorize("hasPermission('PatientEmergencyContactDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientEmergencyContactDetailEntity> getById(
			@Parameter(description = "The UUID of the PatientEmergencyContactDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientEmergencyContactDetailEntity> patientEmergencyContactDetailEntity = patientEmergencyContactDetailService.findById(id);
	...
	/**
	 * Create/Update the given Patient Emergency Contact Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientEmergencyContactDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientEmergencyContactDetail' endpoint here] off begin
	@Operation(
			description = "Create a PatientEmergencyContactDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientEmergencyContactDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientEmergencyContactDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientEmergencyContactDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientEmergencyContactDetail' endpoint here] end
	public ResponseEntity<?> savePatientEmergencyContactDetail(
			@Parameter(description = "The PatientEmergencyContactDetailEntity to create or update.")
			@RequestBody PatientEmergencyContactDetailEntityDto patientEmergencyContactDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientEmergencyContactDetailEntity patientEmergencyContactDetailEntity = patientEmergencyContactDetailService.create(new PatientEmergencyContactDetailEntity(patientEmergencyContactDetailEntityDto));
			return new ResponseEntity<>(patientEmergencyContactDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Emergency Contact Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientEmergencyContactDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientEmergencyContactDetail' endpoint here] off begin
	@Operation(
			description = "Update a PatientEmergencyContactDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientEmergencyContactDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientEmergencyContactDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientEmergencyContactDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientEmergencyContactDetail' endpoint here] end
	public ResponseEntity<?> updatePatientEmergencyContactDetail(
			@Parameter(description = "The PatientEmergencyContactDetailEntity to create or update.")
			@RequestBody PatientEmergencyContactDetailEntityDto patientEmergencyContactDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientEmergencyContactDetailEntity> checkPatientEmergencyContactDetailExists = patientEmergencyContactDetailService.findById(patientEmergencyContactDetailEntityDto.getId());
		PatientEmergencyContactDetailEntity patientEmergencyContactDetailEntity = new PatientEmergencyContactDetailEntity(patientEmergencyContactDetailEntityDto);

		if (checkPatientEmergencyContactDetailExists.isPresent()) {
			try {
				patientEmergencyContactDetailEntity = patientEmergencyContactDetailService.update(patientEmergencyContactDetailEntity);
				return new ResponseEntity<>(patientEmergencyContactDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientEmergencyContactDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Emergency Contact Detail that has the same id as the given id.
	 *
	 * @param id The id of the PatientEmergencyContactDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientEmergencyContactDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientEmergencyContactDetail to delete.")
		...
		patientEmergencyContactDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientEmergencyContactDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Emergency Contact Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientEmergencyContactDetail' endpoint here] end
	public void exportPatientEmergencyContactDetail(
			@Parameter(description = "The ids of the PatientEmergencyContactDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientEmergencyContactDetailEntity> entitiesToExport = patientEmergencyContactDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientEmergencyContactDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientEmergencyContactDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientEmergencyContactDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientEmergencyContactDetailExcludingIds' endpoint here] end
	public void exportPatientEmergencyContactDetailExcludingIds(
			@Parameter(description = "The ids of the PatientEmergencyContactDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientEmergencyContactDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientEmergencyContactDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientEmergencyContactDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientEmergencyContactDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientEmergencyContactDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientEmergencyContactDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Emergency Contact Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientEmergencyContactDetailEntity', 'create')  || hasPermission('PatientEmergencyContactDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientEmergencyContactDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientEmergencyContactDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientEmergencyContactDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientEmergencyContactDetailEntity> csvToBean = new CsvToBeanBuilder<PatientEmergencyContactDetailEntity>(reader)
				.withType(PatientEmergencyContactDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientEmergencyContactDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientEmergencyContactDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Emergency Contact Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Emergency Contact Detail entity and you do not have permission");
					}
