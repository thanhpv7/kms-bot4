...
import kmsweb.entities.RegistrationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Registration
 */
// % protected region % [Update the API route or description for the RegistrationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RegistrationController"))
@RestController
@Slf4j
@RequestMapping("/api/registration")
// % protected region % [Update the API route or description for the RegistrationController here] end
public class RegistrationController {

	private final RegistrationService registrationService;
	private final CsvUtils<RegistrationEntity, RegistrationRepository, RegistrationEntityAudit,  RegistrationService> csvUtils;
	...
	public RegistrationController(
		...
		this.csvUtils = new CsvUtils<>(registrationService, RegistrationEntity.class);
	...
	@Operation(
			description = "Returns a single page of Registrations",
		security = {
	...
	public ResponseEntity<List<RegistrationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<RegistrationEntity> registrations = registrationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Registration that has the same id as the given id.
	 *
	 * @param id      The id of the RegistrationEntity to be returned
	 * @return the Registration that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Registration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Registration entity not found"),
	})
	@PreAuthorize("hasPermission('RegistrationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<RegistrationEntity> getById(
			@Parameter(description = "The UUID of the Registration to return.")
			@PathVariable("id") UUID id
	) {
		Optional<RegistrationEntity> registrationEntity = registrationService.findById(id);
	...
	/**
	 * Create/Update the given Registration. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param registrationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRegistration' endpoint here] off begin
	@Operation(
			description = "Create a Registration if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Registration."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Registration, duplicate record found")
	})
	@PreAuthorize("hasPermission('RegistrationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRegistration' endpoint here] end
	public ResponseEntity<?> saveRegistration(
			@Parameter(description = "The RegistrationEntity to create or update.")
			@RequestBody RegistrationEntityDto registrationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RegistrationEntity registrationEntity = registrationService.create(new RegistrationEntity(registrationEntityDto));
	...
	/**
	 * Update the given Registration. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param registrationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRegistration' endpoint here] off begin
	@Operation(
			description = "Update a Registration entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Registration."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Registration, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RegistrationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRegistration' endpoint here] end
	public ResponseEntity<?> updateRegistration(
		...
		Optional<RegistrationEntity> checkRegistrationExists = registrationService.findById(registrationEntityDto.getId());
		RegistrationEntity registrationEntity = new RegistrationEntity(registrationEntityDto);

		if (checkRegistrationExists.isPresent()) {
			try {
				registrationEntity = registrationService.update(registrationEntity);
				return new ResponseEntity<>(registrationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(registrationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Registration that has the same id as the given id.
	 *
	 * @param id The id of the RegistrationEntity to be deleted
	...
	@Operation(
			description = "Delete a single Registration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Registration to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRegistration' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Registration entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRegistration' endpoint here] end
	public void exportRegistration(
			@Parameter(description = "The ids of the RegistrationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RegistrationEntity> entitiesToExport = registrationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Registration_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RegistrationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRegistrationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRegistrationExcludingIds' endpoint here] end
	public void exportRegistrationExcludingIds(
			@Parameter(description = "The ids of the RegistrationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RegistrationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = registrationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = registrationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Registration entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Registration_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RegistrationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Registration entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RegistrationEntity', 'create')  || hasPermission('RegistrationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RegistrationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RegistrationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RegistrationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RegistrationEntity> csvToBean = new CsvToBeanBuilder<RegistrationEntity>(reader)
				.withType(RegistrationEntity.class)
		...
		try {
			Iterator<RegistrationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Registration entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Registration entity and you do not have permission");
					}
