...
import kmsweb.entities.LocationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Location
 */
// % protected region % [Update the API route or description for the LocationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "LocationController"))
@RestController
@Slf4j
@RequestMapping("/api/location")
// % protected region % [Update the API route or description for the LocationController here] end
public class LocationController {

	private final LocationService locationService;
	private final CsvUtils<LocationEntity, LocationRepository, LocationEntityAudit,  LocationService> csvUtils;
	...
	public LocationController(
		...
		this.csvUtils = new CsvUtils<>(locationService, LocationEntity.class);
	...
	@Operation(
			description = "Returns a single page of Locations",
		security = {
	...
	public ResponseEntity<List<LocationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<LocationEntity> locations = locationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Location that has the same id as the given id.
	 *
	 * @param id      The id of the LocationEntity to be returned
	 * @return the Location that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Location as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<LocationEntity> getById(
			@Parameter(description = "The UUID of the Location to return.")
			@PathVariable("id") UUID id
	) {
		Optional<LocationEntity> locationEntity = locationService.findById(id);
	...
	/**
	 * Create/Update the given Location. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param locationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveLocation' endpoint here] off begin
	@Operation(
			description = "Create a Location if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Location."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Location, duplicate record found")
	})
	@PreAuthorize("hasPermission('LocationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveLocation' endpoint here] end
	public ResponseEntity<?> saveLocation(
			@Parameter(description = "The LocationEntity to create or update.")
			@RequestBody LocationEntityDto locationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			LocationEntity locationEntity = locationService.create(new LocationEntity(locationEntityDto));
			return new ResponseEntity<>(locationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Location. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param locationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateLocation' endpoint here] off begin
	@Operation(
			description = "Update a Location entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		...
		@ApiResponse(responseCode = "400", description = "Failed to update the Location, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('LocationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateLocation' endpoint here] end
	public ResponseEntity<?> updateLocation(
		...
		Optional<LocationEntity> checkLocationExists = locationService.findById(locationEntityDto.getId());
		LocationEntity locationEntity = new LocationEntity(locationEntityDto);

		if (checkLocationExists.isPresent()) {
			try {
				locationEntity = locationService.update(locationEntity);
				return new ResponseEntity<>(locationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(locationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Location that has the same id as the given id.
	 *
	 * @param id The id of the LocationEntity to be deleted
	...
	@Operation(
			description = "Delete a single Location as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Location to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportLocation' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Location entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportLocation' endpoint here] end
	public void exportLocation(
			@Parameter(description = "The ids of the LocationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<LocationEntity> entitiesToExport = locationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Location_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (LocationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportLocationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportLocationExcludingIds' endpoint here] end
	public void exportLocationExcludingIds(
			@Parameter(description = "The ids of the LocationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<LocationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = locationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = locationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Location entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Location_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(LocationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Location entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('LocationEntity', 'create')  || hasPermission('LocationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with LocationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("LocationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(LocationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<LocationEntity> csvToBean = new CsvToBeanBuilder<LocationEntity>(reader)
				.withType(LocationEntity.class)
		...
		try {
			Iterator<LocationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Location entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Location entity and you do not have permission");
					}
