...
/**
 * Controller used to handle all REST operations regarding Patient Contact Info
 */
// % protected region % [Update the API route or description for the Patient Contact InfoController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientContactInfoController"))
@RestController
@Slf4j
@RequestMapping("/api/patientcontactinfo")
// % protected region % [Update the API route or description for the Patient Contact InfoController here] end
public class PatientContactInfoController {

	private final PatientContactInfoService patientContactInfoService;
	private final CsvUtils<PatientContactInfoEntity, PatientContactInfoRepository, PatientContactInfoEntityAudit,  PatientContactInfoService> csvUtils;
	...
	@Autowired
	public PatientContactInfoController(
		...
		this.csvUtils = new CsvUtils<>(patientContactInfoService, PatientContactInfoEntity.class);
	...
	public ResponseEntity<List<PatientContactInfoEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientContactInfoEntity> patientContactInfos = patientContactInfoService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientContactInfos, HttpStatus.OK);
	}

	/**
	 * Return the Patient Contact Info that has the same id as the given id.
	 *
	 * @param id      The id of the PatientContactInfoEntity to be returned
	 * @return the Patient Contact Info that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientContactInfo as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientContactInfo entity not found"),
	})
	@PreAuthorize("hasPermission('PatientContactInfoEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientContactInfoEntity> getById(
			@Parameter(description = "The UUID of the PatientContactInfo to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientContactInfoEntity> patientContactInfoEntity = patientContactInfoService.findById(id);
	...
	/**
	 * Create/Update the given Patient Contact Info. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientContactInfoEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientContactInfo' endpoint here] off begin
	@Operation(
			description = "Create a PatientContactInfo if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientContactInfo."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientContactInfo, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientContactInfoEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientContactInfo' endpoint here] end
	public ResponseEntity<?> savePatientContactInfo(
			@Parameter(description = "The PatientContactInfoEntity to create or update.")
			@RequestBody PatientContactInfoEntityDto patientContactInfoEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientContactInfoEntity patientContactInfoEntity = patientContactInfoService.create(new PatientContactInfoEntity(patientContactInfoEntityDto));
			return new ResponseEntity<>(patientContactInfoEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Contact Info. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientContactInfoEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientContactInfo' endpoint here] off begin
	@Operation(
			description = "Update a PatientContactInfo entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientContactInfo."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientContactInfo, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientContactInfoEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientContactInfo' endpoint here] end
	public ResponseEntity<?> updatePatientContactInfo(
			@Parameter(description = "The PatientContactInfoEntity to create or update.")
			@RequestBody PatientContactInfoEntityDto patientContactInfoEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientContactInfoEntity> checkPatientContactInfoExists = patientContactInfoService.findById(patientContactInfoEntityDto.getId());
		PatientContactInfoEntity patientContactInfoEntity = new PatientContactInfoEntity(patientContactInfoEntityDto);

		if (checkPatientContactInfoExists.isPresent()) {
			try {
				patientContactInfoEntity = patientContactInfoService.update(patientContactInfoEntity);
				return new ResponseEntity<>(patientContactInfoEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientContactInfoEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Contact Info that has the same id as the given id.
	 *
	 * @param id The id of the PatientContactInfoEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientContactInfo as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientContactInfo to delete.")
		...
		patientContactInfoService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientContactInfo' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Contact Info entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientContactInfo' endpoint here] end
	public void exportPatientContactInfo(
			@Parameter(description = "The ids of the PatientContactInfoEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientContactInfoEntity> entitiesToExport = patientContactInfoService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientContactInfo_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientContactInfoEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientContactInfoExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientContactInfoExcludingIds' endpoint here] end
	public void exportPatientContactInfoExcludingIds(
			@Parameter(description = "The ids of the PatientContactInfoEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientContactInfoEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientContactInfoService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientContactInfoService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientContactInfo entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientContactInfo_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientContactInfoEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Contact Info entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientContactInfoEntity', 'create')  || hasPermission('PatientContactInfoEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientContactInfoEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientContactInfoEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientContactInfoEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientContactInfoEntity> csvToBean = new CsvToBeanBuilder<PatientContactInfoEntity>(reader)
				.withType(PatientContactInfoEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientContactInfoEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientContactInfoEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Contact Info entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Contact Info entity and you do not have permission");
					}
