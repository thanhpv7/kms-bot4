...
/**
 * Controller used to handle all REST operations regarding Supplier Bank Account
 */
// % protected region % [Update the API route or description for the Supplier Bank AccountController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SupplierBankAccountController"))
@RestController
@Slf4j
@RequestMapping("/api/supplierbankaccount")
// % protected region % [Update the API route or description for the Supplier Bank AccountController here] end
public class SupplierBankAccountController {

	private final SupplierBankAccountService supplierBankAccountService;
	private final CsvUtils<SupplierBankAccountEntity, SupplierBankAccountRepository, SupplierBankAccountEntityAudit,  SupplierBankAccountService> csvUtils;
	...
	@Autowired
	public SupplierBankAccountController(
		...
		this.csvUtils = new CsvUtils<>(supplierBankAccountService, SupplierBankAccountEntity.class);
	...
	public ResponseEntity<List<SupplierBankAccountEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SupplierBankAccountEntity> supplierBankAccounts = supplierBankAccountService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(supplierBankAccounts, HttpStatus.OK);
	}

	/**
	 * Return the Supplier Bank Account that has the same id as the given id.
	 *
	 * @param id      The id of the SupplierBankAccountEntity to be returned
	 * @return the Supplier Bank Account that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single SupplierBankAccount as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "SupplierBankAccount entity not found"),
	})
	@PreAuthorize("hasPermission('SupplierBankAccountEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SupplierBankAccountEntity> getById(
			@Parameter(description = "The UUID of the SupplierBankAccount to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SupplierBankAccountEntity> supplierBankAccountEntity = supplierBankAccountService.findById(id);
	...
	/**
	 * Create/Update the given Supplier Bank Account. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param supplierBankAccountEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSupplierBankAccount' endpoint here] off begin
	@Operation(
			description = "Create a SupplierBankAccount if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new SupplierBankAccount."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new SupplierBankAccount, duplicate record found")
	})
	@PreAuthorize("hasPermission('SupplierBankAccountEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSupplierBankAccount' endpoint here] end
	public ResponseEntity<?> saveSupplierBankAccount(
			@Parameter(description = "The SupplierBankAccountEntity to create or update.")
			@RequestBody SupplierBankAccountEntityDto supplierBankAccountEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SupplierBankAccountEntity supplierBankAccountEntity = supplierBankAccountService.create(new SupplierBankAccountEntity(supplierBankAccountEntityDto));
			return new ResponseEntity<>(supplierBankAccountEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Supplier Bank Account. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param supplierBankAccountEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSupplierBankAccount' endpoint here] off begin
	@Operation(
			description = "Update a SupplierBankAccount entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the SupplierBankAccount."),
		@ApiResponse(responseCode = "400", description = "Failed to update the SupplierBankAccount, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SupplierBankAccountEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSupplierBankAccount' endpoint here] end
	public ResponseEntity<?> updateSupplierBankAccount(
			@Parameter(description = "The SupplierBankAccountEntity to create or update.")
			@RequestBody SupplierBankAccountEntityDto supplierBankAccountEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SupplierBankAccountEntity> checkSupplierBankAccountExists = supplierBankAccountService.findById(supplierBankAccountEntityDto.getId());
		SupplierBankAccountEntity supplierBankAccountEntity = new SupplierBankAccountEntity(supplierBankAccountEntityDto);

		if (checkSupplierBankAccountExists.isPresent()) {
			try {
				supplierBankAccountEntity = supplierBankAccountService.update(supplierBankAccountEntity);
				return new ResponseEntity<>(supplierBankAccountEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(supplierBankAccountEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Supplier Bank Account that has the same id as the given id.
	 *
	 * @param id The id of the SupplierBankAccountEntity to be deleted
	...
	@Operation(
			description = "Delete a single SupplierBankAccount as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the SupplierBankAccount to delete.")
		...
		supplierBankAccountService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplierBankAccount' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Supplier Bank Account entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplierBankAccount' endpoint here] end
	public void exportSupplierBankAccount(
			@Parameter(description = "The ids of the SupplierBankAccountEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SupplierBankAccountEntity> entitiesToExport = supplierBankAccountService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "SupplierBankAccount_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SupplierBankAccountEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplierBankAccountExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplierBankAccountExcludingIds' endpoint here] end
	public void exportSupplierBankAccountExcludingIds(
			@Parameter(description = "The ids of the SupplierBankAccountEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SupplierBankAccountEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = supplierBankAccountService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = supplierBankAccountService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the SupplierBankAccount entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "SupplierBankAccount_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SupplierBankAccountEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Supplier Bank Account entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SupplierBankAccountEntity', 'create')  || hasPermission('SupplierBankAccountEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SupplierBankAccountEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SupplierBankAccountEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SupplierBankAccountEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SupplierBankAccountEntity> csvToBean = new CsvToBeanBuilder<SupplierBankAccountEntity>(reader)
				.withType(SupplierBankAccountEntity.class)
		...
		try {
			Iterator<SupplierBankAccountEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Supplier Bank Account entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Supplier Bank Account entity and you do not have permission");
					}
