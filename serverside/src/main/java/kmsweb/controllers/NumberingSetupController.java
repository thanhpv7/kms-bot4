...
import kmsweb.entities.NumberingSetupEntityAudit;
import kmsweb.repositories.NumberingSetupRepository;
import kmsweb.services.NumberingSetupService;
...
/**
 * Controller used to handle all REST operations regarding Numbering Setup
 */
// % protected region % [Update the API route or description for the Numbering SetupController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NumberingSetupController"))
@RestController
@Slf4j
@RequestMapping("/api/numberingsetup")
// % protected region % [Update the API route or description for the Numbering SetupController here] end
public class NumberingSetupController {

	private final NumberingSetupService numberingSetupService;
	private final CsvUtils<NumberingSetupEntity, NumberingSetupRepository, NumberingSetupEntityAudit,  NumberingSetupService> csvUtils;
	...
	@Autowired
	public NumberingSetupController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NumberingSetupService numberingSetupService
	) {
		this.authService = authService;
		this.numberingSetupService = numberingSetupService;
		this.csvUtils = new CsvUtils<>(numberingSetupService, NumberingSetupEntity.class);
	...
	public ResponseEntity<List<NumberingSetupEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NumberingSetupEntity> numberingSetups = numberingSetupService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(numberingSetups, HttpStatus.OK);
	}

	/**
	 * Return the Numbering Setup that has the same id as the given id.
	 *
	 * @param id      The id of the NumberingSetupEntity to be returned
	 * @return the Numbering Setup that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NumberingSetup as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NumberingSetup entity not found"),
	})
	@PreAuthorize("hasPermission('NumberingSetupEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NumberingSetupEntity> getById(
			@Parameter(description = "The UUID of the NumberingSetup to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NumberingSetupEntity> numberingSetupEntity = numberingSetupService.findById(id);
	...
	/**
	 * Create/Update the given Numbering Setup. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param numberingSetupEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNumberingSetup' endpoint here] off begin
	@Operation(
			description = "Create a NumberingSetup if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NumberingSetup."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NumberingSetup, duplicate record found")
	})
	@PreAuthorize("hasPermission('NumberingSetupEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNumberingSetup' endpoint here] end
	public ResponseEntity<?> saveNumberingSetup(
			@Parameter(description = "The NumberingSetupEntity to create or update.")
			@RequestBody NumberingSetupEntityDto numberingSetupEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NumberingSetupEntity numberingSetupEntity = numberingSetupService.create(new NumberingSetupEntity(numberingSetupEntityDto));
			return new ResponseEntity<>(numberingSetupEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Numbering Setup. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param numberingSetupEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNumberingSetup' endpoint here] off begin
	@Operation(
			description = "Update a NumberingSetup entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NumberingSetup."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NumberingSetup, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NumberingSetupEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNumberingSetup' endpoint here] end
	public ResponseEntity<?> updateNumberingSetup(
			@Parameter(description = "The NumberingSetupEntity to create or update.")
			@RequestBody NumberingSetupEntityDto numberingSetupEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NumberingSetupEntity> checkNumberingSetupExists = numberingSetupService.findById(numberingSetupEntityDto.getId());
		NumberingSetupEntity numberingSetupEntity = new NumberingSetupEntity(numberingSetupEntityDto);

		if (checkNumberingSetupExists.isPresent()) {
			try {
				numberingSetupEntity = numberingSetupService.update(numberingSetupEntity);
				return new ResponseEntity<>(numberingSetupEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(numberingSetupEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Numbering Setup that has the same id as the given id.
	 *
	 * @param id The id of the NumberingSetupEntity to be deleted
	...
	@Operation(
			description = "Delete a single NumberingSetup as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NumberingSetup to delete.")
		...
		numberingSetupService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNumberingSetup' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Numbering Setup entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNumberingSetup' endpoint here] end
	public void exportNumberingSetup(
			@Parameter(description = "The ids of the NumberingSetupEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NumberingSetupEntity> entitiesToExport = numberingSetupService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NumberingSetup_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NumberingSetupEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNumberingSetupExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNumberingSetupExcludingIds' endpoint here] end
	public void exportNumberingSetupExcludingIds(
			@Parameter(description = "The ids of the NumberingSetupEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NumberingSetupEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = numberingSetupService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = numberingSetupService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NumberingSetup entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NumberingSetup_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NumberingSetupEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Numbering Setup entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NumberingSetupEntity', 'create')  || hasPermission('NumberingSetupEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NumberingSetupEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NumberingSetupEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NumberingSetupEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NumberingSetupEntity> csvToBean = new CsvToBeanBuilder<NumberingSetupEntity>(reader)
				.withType(NumberingSetupEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NumberingSetupEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NumberingSetupEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Numbering Setup entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Numbering Setup entity and you do not have permission");
					}
