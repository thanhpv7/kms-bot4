...
import kmsweb.entities.CatalogueUOMEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Catalogue UOM
 */
// % protected region % [Update the API route or description for the Catalogue UOMController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CatalogueUOMController"))
@RestController
@Slf4j
@RequestMapping("/api/catalogueuom")
// % protected region % [Update the API route or description for the Catalogue UOMController here] end
public class CatalogueUOMController {

	private final CatalogueUOMService catalogueUOMService;
	private final CsvUtils<CatalogueUOMEntity, CatalogueUOMRepository, CatalogueUOMEntityAudit,  CatalogueUOMService> csvUtils;
	...
	public CatalogueUOMController(
		...
		this.csvUtils = new CsvUtils<>(catalogueUOMService, CatalogueUOMEntity.class);
	...
	public ResponseEntity<List<CatalogueUOMEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CatalogueUOMEntity> catalogueUOMs = catalogueUOMService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(catalogueUOMs, HttpStatus.OK);
	}

	/**
	 * Return the Catalogue UOM that has the same id as the given id.
	 *
	 * @param id      The id of the CatalogueUOMEntity to be returned
	 * @return the Catalogue UOM that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CatalogueUOM as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CatalogueUOM entity not found"),
	})
	@PreAuthorize("hasPermission('CatalogueUOMEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CatalogueUOMEntity> getById(
			@Parameter(description = "The UUID of the CatalogueUOM to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CatalogueUOMEntity> catalogueUOMEntity = catalogueUOMService.findById(id);
	...
	/**
	 * Create/Update the given Catalogue UOM. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param catalogueUOMEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCatalogueUOM' endpoint here] off begin
	@Operation(
			description = "Create a CatalogueUOM if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CatalogueUOM."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CatalogueUOM, duplicate record found")
	})
	@PreAuthorize("hasPermission('CatalogueUOMEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCatalogueUOM' endpoint here] end
	public ResponseEntity<?> saveCatalogueUOM(
			@Parameter(description = "The CatalogueUOMEntity to create or update.")
			@RequestBody CatalogueUOMEntityDto catalogueUOMEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CatalogueUOMEntity catalogueUOMEntity = catalogueUOMService.create(new CatalogueUOMEntity(catalogueUOMEntityDto));
			return new ResponseEntity<>(catalogueUOMEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Catalogue UOM. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param catalogueUOMEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCatalogueUOM' endpoint here] off begin
	@Operation(
			description = "Update a CatalogueUOM entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CatalogueUOM."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CatalogueUOM, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CatalogueUOMEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCatalogueUOM' endpoint here] end
	public ResponseEntity<?> updateCatalogueUOM(
			@Parameter(description = "The CatalogueUOMEntity to create or update.")
			@RequestBody CatalogueUOMEntityDto catalogueUOMEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CatalogueUOMEntity> checkCatalogueUOMExists = catalogueUOMService.findById(catalogueUOMEntityDto.getId());
		CatalogueUOMEntity catalogueUOMEntity = new CatalogueUOMEntity(catalogueUOMEntityDto);

		if (checkCatalogueUOMExists.isPresent()) {
			try {
				catalogueUOMEntity = catalogueUOMService.update(catalogueUOMEntity);
				return new ResponseEntity<>(catalogueUOMEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(catalogueUOMEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Catalogue UOM that has the same id as the given id.
	 *
	 * @param id The id of the CatalogueUOMEntity to be deleted
	...
	@Operation(
			description = "Delete a single CatalogueUOM as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CatalogueUOM to delete.")
		...
		catalogueUOMService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCatalogueUOM' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Catalogue UOM entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCatalogueUOM' endpoint here] end
	public void exportCatalogueUOM(
			@Parameter(description = "The ids of the CatalogueUOMEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CatalogueUOMEntity> entitiesToExport = catalogueUOMService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CatalogueUOM_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CatalogueUOMEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCatalogueUOMExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCatalogueUOMExcludingIds' endpoint here] end
	public void exportCatalogueUOMExcludingIds(
			@Parameter(description = "The ids of the CatalogueUOMEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CatalogueUOMEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = catalogueUOMService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = catalogueUOMService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CatalogueUOM entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CatalogueUOM_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CatalogueUOMEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Catalogue UOM entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CatalogueUOMEntity', 'create')  || hasPermission('CatalogueUOMEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CatalogueUOMEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CatalogueUOMEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CatalogueUOMEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CatalogueUOMEntity> csvToBean = new CsvToBeanBuilder<CatalogueUOMEntity>(reader)
				.withType(CatalogueUOMEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CatalogueUOMEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CatalogueUOMEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Catalogue UOM entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Catalogue UOM entity and you do not have permission");
					}
