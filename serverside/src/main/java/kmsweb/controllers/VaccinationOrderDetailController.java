...
/**
 * Controller used to handle all REST operations regarding Vaccination Order Detail
 */
// % protected region % [Update the API route or description for the Vaccination Order DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "VaccinationOrderDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/vaccinationorderdetail")
// % protected region % [Update the API route or description for the Vaccination Order DetailController here] end
public class VaccinationOrderDetailController {

	private final VaccinationOrderDetailService vaccinationOrderDetailService;
	private final CsvUtils<VaccinationOrderDetailEntity, VaccinationOrderDetailRepository, VaccinationOrderDetailEntityAudit,  VaccinationOrderDetailService> csvUtils;
	...
	@Autowired
	public VaccinationOrderDetailController(
		...
		this.csvUtils = new CsvUtils<>(vaccinationOrderDetailService, VaccinationOrderDetailEntity.class);
	...
	public ResponseEntity<List<VaccinationOrderDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<VaccinationOrderDetailEntity> vaccinationOrderDetails = vaccinationOrderDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(vaccinationOrderDetails, HttpStatus.OK);
	}

	/**
	 * Return the Vaccination Order Detail that has the same id as the given id.
	 *
	 * @param id      The id of the VaccinationOrderDetailEntity to be returned
	 * @return the Vaccination Order Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single VaccinationOrderDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "VaccinationOrderDetail entity not found"),
	})
	@PreAuthorize("hasPermission('VaccinationOrderDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<VaccinationOrderDetailEntity> getById(
			@Parameter(description = "The UUID of the VaccinationOrderDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<VaccinationOrderDetailEntity> vaccinationOrderDetailEntity = vaccinationOrderDetailService.findById(id);
	...
	/**
	 * Create/Update the given Vaccination Order Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param vaccinationOrderDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveVaccinationOrderDetail' endpoint here] off begin
	@Operation(
			description = "Create a VaccinationOrderDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new VaccinationOrderDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new VaccinationOrderDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('VaccinationOrderDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveVaccinationOrderDetail' endpoint here] end
	public ResponseEntity<?> saveVaccinationOrderDetail(
			@Parameter(description = "The VaccinationOrderDetailEntity to create or update.")
			@RequestBody VaccinationOrderDetailEntityDto vaccinationOrderDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			VaccinationOrderDetailEntity vaccinationOrderDetailEntity = vaccinationOrderDetailService.create(new VaccinationOrderDetailEntity(vaccinationOrderDetailEntityDto));
			return new ResponseEntity<>(vaccinationOrderDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Vaccination Order Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param vaccinationOrderDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateVaccinationOrderDetail' endpoint here] off begin
	@Operation(
			description = "Update a VaccinationOrderDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the VaccinationOrderDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the VaccinationOrderDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('VaccinationOrderDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateVaccinationOrderDetail' endpoint here] end
	public ResponseEntity<?> updateVaccinationOrderDetail(
			@Parameter(description = "The VaccinationOrderDetailEntity to create or update.")
			@RequestBody VaccinationOrderDetailEntityDto vaccinationOrderDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<VaccinationOrderDetailEntity> checkVaccinationOrderDetailExists = vaccinationOrderDetailService.findById(vaccinationOrderDetailEntityDto.getId());
		VaccinationOrderDetailEntity vaccinationOrderDetailEntity = new VaccinationOrderDetailEntity(vaccinationOrderDetailEntityDto);

		if (checkVaccinationOrderDetailExists.isPresent()) {
			try {
				vaccinationOrderDetailEntity = vaccinationOrderDetailService.update(vaccinationOrderDetailEntity);
				return new ResponseEntity<>(vaccinationOrderDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(vaccinationOrderDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Vaccination Order Detail that has the same id as the given id.
	 *
	 * @param id The id of the VaccinationOrderDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single VaccinationOrderDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the VaccinationOrderDetail to delete.")
		...
		vaccinationOrderDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportVaccinationOrderDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Vaccination Order Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportVaccinationOrderDetail' endpoint here] end
	public void exportVaccinationOrderDetail(
			@Parameter(description = "The ids of the VaccinationOrderDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<VaccinationOrderDetailEntity> entitiesToExport = vaccinationOrderDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "VaccinationOrderDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (VaccinationOrderDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportVaccinationOrderDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportVaccinationOrderDetailExcludingIds' endpoint here] end
	public void exportVaccinationOrderDetailExcludingIds(
			@Parameter(description = "The ids of the VaccinationOrderDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<VaccinationOrderDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = vaccinationOrderDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = vaccinationOrderDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the VaccinationOrderDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "VaccinationOrderDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(VaccinationOrderDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Vaccination Order Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('VaccinationOrderDetailEntity', 'create')  || hasPermission('VaccinationOrderDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with VaccinationOrderDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("VaccinationOrderDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(VaccinationOrderDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<VaccinationOrderDetailEntity> csvToBean = new CsvToBeanBuilder<VaccinationOrderDetailEntity>(reader)
				.withType(VaccinationOrderDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<VaccinationOrderDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<VaccinationOrderDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Vaccination Order Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Vaccination Order Detail entity and you do not have permission");
					}
