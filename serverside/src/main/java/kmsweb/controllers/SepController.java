...
import kmsweb.entities.SepEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding SEP
 */
// % protected region % [Update the API route or description for the SEPController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SepController"))
@RestController
@Slf4j
@RequestMapping("/api/sep")
// % protected region % [Update the API route or description for the SEPController here] end
public class SepController {

	private final SepService sepService;
	private final CsvUtils<SepEntity, SepRepository, SepEntityAudit,  SepService> csvUtils;
	...
	public SepController(
		...
		this.csvUtils = new CsvUtils<>(sepService, SepEntity.class);
	...
	@Operation(
			description = "Returns a single page of Seps",
		security = {
	...
	public ResponseEntity<List<SepEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SepEntity> seps = sepService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(seps, HttpStatus.OK);
	}

	/**
	 * Return the SEP that has the same id as the given id.
	 *
	 * @param id      The id of the SepEntity to be returned
	 * @return the SEP that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Sep as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Sep entity not found"),
	})
	@PreAuthorize("hasPermission('SepEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SepEntity> getById(
			@Parameter(description = "The UUID of the Sep to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SepEntity> sepEntity = sepService.findById(id);
	...
	/**
	 * Create/Update the given SEP. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param sepEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSep' endpoint here] off begin
	@Operation(
			description = "Create a Sep if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Sep."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Sep, duplicate record found")
	})
	@PreAuthorize("hasPermission('SepEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSep' endpoint here] end
	public ResponseEntity<?> saveSep(
			@Parameter(description = "The SepEntity to create or update.")
			@RequestBody SepEntityDto sepEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SepEntity sepEntity = sepService.create(new SepEntity(sepEntityDto));
			return new ResponseEntity<>(sepEntity, HttpStatus.OK);
	...
	/**
	 * Update the given SEP. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param sepEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSep' endpoint here] off begin
	@Operation(
			description = "Update a Sep entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Sep."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Sep, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SepEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSep' endpoint here] end
	public ResponseEntity<?> updateSep(
			@Parameter(description = "The SepEntity to create or update.")
			@RequestBody SepEntityDto sepEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SepEntity> checkSepExists = sepService.findById(sepEntityDto.getId());
		SepEntity sepEntity = new SepEntity(sepEntityDto);

		if (checkSepExists.isPresent()) {
			try {
				sepEntity = sepService.update(sepEntity);
				return new ResponseEntity<>(sepEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(sepEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the SEP that has the same id as the given id.
	 *
	 * @param id The id of the SepEntity to be deleted
	...
	@Operation(
			description = "Delete a single Sep as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Sep to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSep' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all SEP entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSep' endpoint here] end
	public void exportSep(
			@Parameter(description = "The ids of the SepEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SepEntity> entitiesToExport = sepService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Sep_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SepEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSepExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSepExcludingIds' endpoint here] end
	public void exportSepExcludingIds(
			@Parameter(description = "The ids of the SepEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SepEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = sepService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = sepService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Sep entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Sep_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SepEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for SEP entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SepEntity', 'create')  || hasPermission('SepEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SepEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SepEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SepEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SepEntity> csvToBean = new CsvToBeanBuilder<SepEntity>(reader)
				.withType(SepEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<SepEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<SepEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a SEP entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a SEP entity and you do not have permission");
					}
