...
import kmsweb.entities.DiagnosticExaminationResultsEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Diagnostic Examination Results
 */
// % protected region % [Update the API route or description for the Diagnostic Examination ResultsController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DiagnosticExaminationResultsController"))
@RestController
@Slf4j
@RequestMapping("/api/diagnosticexaminationresults")
// % protected region % [Update the API route or description for the Diagnostic Examination ResultsController here] end
public class DiagnosticExaminationResultsController {

	private final DiagnosticExaminationResultsService diagnosticExaminationResultsService;
	private final CsvUtils<DiagnosticExaminationResultsEntity, DiagnosticExaminationResultsRepository, DiagnosticExaminationResultsEntityAudit,  DiagnosticExaminationResultsService> csvUtils;
	...
	@Autowired
	public DiagnosticExaminationResultsController(
		...
		this.csvUtils = new CsvUtils<>(diagnosticExaminationResultsService, DiagnosticExaminationResultsEntity.class);
	...
	/**
	 * Return all the Diagnostic Examination Resultss.
	 *
	 * @return all the Diagnostic Examination Resultss
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of DiagnosticExaminationResultss",
		security = {
	...
	public ResponseEntity<List<DiagnosticExaminationResultsEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DiagnosticExaminationResultsEntity> diagnosticExaminationResultss = diagnosticExaminationResultsService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(diagnosticExaminationResultss, HttpStatus.OK);
	}

	/**
	 * Return the Diagnostic Examination Results that has the same id as the given id.
	 *
	 * @param id      The id of the DiagnosticExaminationResultsEntity to be returned
	 * @return the Diagnostic Examination Results that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DiagnosticExaminationResults as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DiagnosticExaminationResults entity not found"),
	})
	@PreAuthorize("hasPermission('DiagnosticExaminationResultsEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DiagnosticExaminationResultsEntity> getById(
			@Parameter(description = "The UUID of the DiagnosticExaminationResults to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DiagnosticExaminationResultsEntity> diagnosticExaminationResultsEntity = diagnosticExaminationResultsService.findById(id);
	...
	/**
	 * Create/Update the given Diagnostic Examination Results. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosticExaminationResultsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDiagnosticExaminationResults' endpoint here] off begin
	@Operation(
			description = "Create a DiagnosticExaminationResults if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DiagnosticExaminationResults."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DiagnosticExaminationResults, duplicate record found")
	})
	@PreAuthorize("hasPermission('DiagnosticExaminationResultsEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDiagnosticExaminationResults' endpoint here] end
	public ResponseEntity<?> saveDiagnosticExaminationResults(
			@Parameter(description = "The DiagnosticExaminationResultsEntity to create or update.")
			@RequestBody DiagnosticExaminationResultsEntityDto diagnosticExaminationResultsEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DiagnosticExaminationResultsEntity diagnosticExaminationResultsEntity = diagnosticExaminationResultsService.create(new DiagnosticExaminationResultsEntity(diagnosticExaminationResultsEntityDto));
			return new ResponseEntity<>(diagnosticExaminationResultsEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Diagnostic Examination Results. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosticExaminationResultsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDiagnosticExaminationResults' endpoint here] off begin
	@Operation(
			description = "Update a DiagnosticExaminationResults entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DiagnosticExaminationResults."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DiagnosticExaminationResults, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DiagnosticExaminationResultsEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDiagnosticExaminationResults' endpoint here] end
	public ResponseEntity<?> updateDiagnosticExaminationResults(
			@Parameter(description = "The DiagnosticExaminationResultsEntity to create or update.")
			@RequestBody DiagnosticExaminationResultsEntityDto diagnosticExaminationResultsEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DiagnosticExaminationResultsEntity> checkDiagnosticExaminationResultsExists = diagnosticExaminationResultsService.findById(diagnosticExaminationResultsEntityDto.getId());
		DiagnosticExaminationResultsEntity diagnosticExaminationResultsEntity = new DiagnosticExaminationResultsEntity(diagnosticExaminationResultsEntityDto);

		if (checkDiagnosticExaminationResultsExists.isPresent()) {
			try {
				diagnosticExaminationResultsEntity = diagnosticExaminationResultsService.update(diagnosticExaminationResultsEntity);
				return new ResponseEntity<>(diagnosticExaminationResultsEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(diagnosticExaminationResultsEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Diagnostic Examination Results that has the same id as the given id.
	 *
	 * @param id The id of the DiagnosticExaminationResultsEntity to be deleted
	...
	@Operation(
			description = "Delete a single DiagnosticExaminationResults as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DiagnosticExaminationResults to delete.")
		...
		diagnosticExaminationResultsService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticExaminationResults' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Diagnostic Examination Results entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticExaminationResults' endpoint here] end
	public void exportDiagnosticExaminationResults(
			@Parameter(description = "The ids of the DiagnosticExaminationResultsEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DiagnosticExaminationResultsEntity> entitiesToExport = diagnosticExaminationResultsService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DiagnosticExaminationResults_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DiagnosticExaminationResultsEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticExaminationResultsExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticExaminationResultsExcludingIds' endpoint here] end
	public void exportDiagnosticExaminationResultsExcludingIds(
			@Parameter(description = "The ids of the DiagnosticExaminationResultsEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DiagnosticExaminationResultsEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = diagnosticExaminationResultsService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = diagnosticExaminationResultsService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DiagnosticExaminationResults entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DiagnosticExaminationResults_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DiagnosticExaminationResultsEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Diagnostic Examination Results entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DiagnosticExaminationResultsEntity', 'create')  || hasPermission('DiagnosticExaminationResultsEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DiagnosticExaminationResultsEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DiagnosticExaminationResultsEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DiagnosticExaminationResultsEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DiagnosticExaminationResultsEntity> csvToBean = new CsvToBeanBuilder<DiagnosticExaminationResultsEntity>(reader)
				.withType(DiagnosticExaminationResultsEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DiagnosticExaminationResultsEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DiagnosticExaminationResultsEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Diagnostic Examination Results entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Diagnostic Examination Results entity and you do not have permission");
					}
