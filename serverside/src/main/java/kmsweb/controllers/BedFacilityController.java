...
import kmsweb.entities.BedFacilityEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Bed Facility
 */
// % protected region % [Update the API route or description for the Bed FacilityController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BedFacilityController"))
@RestController
@Slf4j
@RequestMapping("/api/bedfacility")
// % protected region % [Update the API route or description for the Bed FacilityController here] end
public class BedFacilityController {

	private final BedFacilityService bedFacilityService;
	private final CsvUtils<BedFacilityEntity, BedFacilityRepository, BedFacilityEntityAudit,  BedFacilityService> csvUtils;
	...
	public BedFacilityController(
		...
		this.csvUtils = new CsvUtils<>(bedFacilityService, BedFacilityEntity.class);
	...
	public ResponseEntity<List<BedFacilityEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BedFacilityEntity> bedFacilitys = bedFacilityService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bedFacilitys, HttpStatus.OK);
	}

	/**
	 * Return the Bed Facility that has the same id as the given id.
	 *
	 * @param id      The id of the BedFacilityEntity to be returned
	 * @return the Bed Facility that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BedFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BedFacility entity not found"),
	})
	@PreAuthorize("hasPermission('BedFacilityEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BedFacilityEntity> getById(
			@Parameter(description = "The UUID of the BedFacility to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BedFacilityEntity> bedFacilityEntity = bedFacilityService.findById(id);
	...
	/**
	 * Create/Update the given Bed Facility. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bedFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBedFacility' endpoint here] off begin
	@Operation(
			description = "Create a BedFacility if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BedFacility."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BedFacility, duplicate record found")
	})
	@PreAuthorize("hasPermission('BedFacilityEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBedFacility' endpoint here] end
	public ResponseEntity<?> saveBedFacility(
			@Parameter(description = "The BedFacilityEntity to create or update.")
			@RequestBody BedFacilityEntityDto bedFacilityEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BedFacilityEntity bedFacilityEntity = bedFacilityService.create(new BedFacilityEntity(bedFacilityEntityDto));
			return new ResponseEntity<>(bedFacilityEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Bed Facility. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bedFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBedFacility' endpoint here] off begin
	@Operation(
			description = "Update a BedFacility entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BedFacility."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BedFacility, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BedFacilityEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBedFacility' endpoint here] end
	public ResponseEntity<?> updateBedFacility(
			@Parameter(description = "The BedFacilityEntity to create or update.")
			@RequestBody BedFacilityEntityDto bedFacilityEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BedFacilityEntity> checkBedFacilityExists = bedFacilityService.findById(bedFacilityEntityDto.getId());
		BedFacilityEntity bedFacilityEntity = new BedFacilityEntity(bedFacilityEntityDto);

		if (checkBedFacilityExists.isPresent()) {
			try {
				bedFacilityEntity = bedFacilityService.update(bedFacilityEntity);
				return new ResponseEntity<>(bedFacilityEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bedFacilityEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Bed Facility that has the same id as the given id.
	 *
	 * @param id The id of the BedFacilityEntity to be deleted
	...
	@Operation(
			description = "Delete a single BedFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BedFacility to delete.")
		...
		bedFacilityService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBedFacility' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Bed Facility entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBedFacility' endpoint here] end
	public void exportBedFacility(
			@Parameter(description = "The ids of the BedFacilityEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BedFacilityEntity> entitiesToExport = bedFacilityService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BedFacility_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BedFacilityEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBedFacilityExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBedFacilityExcludingIds' endpoint here] end
	public void exportBedFacilityExcludingIds(
			@Parameter(description = "The ids of the BedFacilityEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BedFacilityEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bedFacilityService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bedFacilityService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BedFacility entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BedFacility_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BedFacilityEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Bed Facility entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BedFacilityEntity', 'create')  || hasPermission('BedFacilityEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BedFacilityEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BedFacilityEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BedFacilityEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BedFacilityEntity> csvToBean = new CsvToBeanBuilder<BedFacilityEntity>(reader)
				.withType(BedFacilityEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BedFacilityEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BedFacilityEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Bed Facility entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Bed Facility entity and you do not have permission");
					}
