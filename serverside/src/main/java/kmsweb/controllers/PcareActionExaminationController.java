...
import kmsweb.entities.PcareActionExaminationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding PCare Action Examination
 */
// % protected region % [Update the API route or description for the PCare Action ExaminationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareActionExaminationController"))
@RestController
@Slf4j
@RequestMapping("/api/pcareactionexamination")
// % protected region % [Update the API route or description for the PCare Action ExaminationController here] end
public class PcareActionExaminationController {

	private final PcareActionExaminationService pcareActionExaminationService;
	private final CsvUtils<PcareActionExaminationEntity, PcareActionExaminationRepository, PcareActionExaminationEntityAudit,  PcareActionExaminationService> csvUtils;
	...
	@Autowired
	public PcareActionExaminationController(
		...
		this.csvUtils = new CsvUtils<>(pcareActionExaminationService, PcareActionExaminationEntity.class);
	...
	public ResponseEntity<List<PcareActionExaminationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareActionExaminationEntity> pcareActionExaminations = pcareActionExaminationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareActionExaminations, HttpStatus.OK);
	}

	/**
	 * Return the PCare Action Examination that has the same id as the given id.
	 *
	 * @param id      The id of the PcareActionExaminationEntity to be returned
	 * @return the PCare Action Examination that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareActionExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareActionExamination entity not found"),
	})
	@PreAuthorize("hasPermission('PcareActionExaminationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareActionExaminationEntity> getById(
			@Parameter(description = "The UUID of the PcareActionExamination to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareActionExaminationEntity> pcareActionExaminationEntity = pcareActionExaminationService.findById(id);
	...
	/**
	 * Create/Update the given PCare Action Examination. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareActionExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareActionExamination' endpoint here] off begin
	@Operation(
			description = "Create a PcareActionExamination if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareActionExamination."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareActionExamination, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareActionExaminationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareActionExamination' endpoint here] end
	public ResponseEntity<?> savePcareActionExamination(
			@Parameter(description = "The PcareActionExaminationEntity to create or update.")
			@RequestBody PcareActionExaminationEntityDto pcareActionExaminationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareActionExaminationEntity pcareActionExaminationEntity = pcareActionExaminationService.create(new PcareActionExaminationEntity(pcareActionExaminationEntityDto));
			return new ResponseEntity<>(pcareActionExaminationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Action Examination. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareActionExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareActionExamination' endpoint here] off begin
	@Operation(
			description = "Update a PcareActionExamination entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareActionExamination."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareActionExamination, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareActionExaminationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareActionExamination' endpoint here] end
	public ResponseEntity<?> updatePcareActionExamination(
			@Parameter(description = "The PcareActionExaminationEntity to create or update.")
			@RequestBody PcareActionExaminationEntityDto pcareActionExaminationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareActionExaminationEntity> checkPcareActionExaminationExists = pcareActionExaminationService.findById(pcareActionExaminationEntityDto.getId());
		PcareActionExaminationEntity pcareActionExaminationEntity = new PcareActionExaminationEntity(pcareActionExaminationEntityDto);

		if (checkPcareActionExaminationExists.isPresent()) {
			try {
				pcareActionExaminationEntity = pcareActionExaminationService.update(pcareActionExaminationEntity);
				return new ResponseEntity<>(pcareActionExaminationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareActionExaminationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Action Examination that has the same id as the given id.
	 *
	 * @param id The id of the PcareActionExaminationEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareActionExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareActionExamination to delete.")
		...
		pcareActionExaminationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareActionExamination' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Action Examination entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareActionExamination' endpoint here] end
	public void exportPcareActionExamination(
			@Parameter(description = "The ids of the PcareActionExaminationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareActionExaminationEntity> entitiesToExport = pcareActionExaminationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareActionExamination_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareActionExaminationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareActionExaminationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareActionExaminationExcludingIds' endpoint here] end
	public void exportPcareActionExaminationExcludingIds(
			@Parameter(description = "The ids of the PcareActionExaminationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareActionExaminationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareActionExaminationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareActionExaminationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareActionExamination entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareActionExamination_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareActionExaminationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Action Examination entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareActionExaminationEntity', 'create')  || hasPermission('PcareActionExaminationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareActionExaminationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareActionExaminationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareActionExaminationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareActionExaminationEntity> csvToBean = new CsvToBeanBuilder<PcareActionExaminationEntity>(reader)
				.withType(PcareActionExaminationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareActionExaminationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareActionExaminationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Action Examination entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Action Examination entity and you do not have permission");
					}
