...
import kmsweb.entities.PatientDetailAddressEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Detail Address
 */
// % protected region % [Update the API route or description for the Patient Detail AddressController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientDetailAddressController"))
@RestController
@Slf4j
@RequestMapping("/api/patientdetailaddress")
// % protected region % [Update the API route or description for the Patient Detail AddressController here] end
public class PatientDetailAddressController {

	private final PatientDetailAddressService patientDetailAddressService;
	private final CsvUtils<PatientDetailAddressEntity, PatientDetailAddressRepository, PatientDetailAddressEntityAudit,  PatientDetailAddressService> csvUtils;
	...
	@Autowired
	public PatientDetailAddressController(
		...
		this.csvUtils = new CsvUtils<>(patientDetailAddressService, PatientDetailAddressEntity.class);
	...
	public ResponseEntity<List<PatientDetailAddressEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientDetailAddressEntity> patientDetailAddresss = patientDetailAddressService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientDetailAddresss, HttpStatus.OK);
	}

	/**
	 * Return the Patient Detail Address that has the same id as the given id.
	 *
	 * @param id      The id of the PatientDetailAddressEntity to be returned
	 * @return the Patient Detail Address that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientDetailAddress as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientDetailAddress entity not found"),
	})
	@PreAuthorize("hasPermission('PatientDetailAddressEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientDetailAddressEntity> getById(
			@Parameter(description = "The UUID of the PatientDetailAddress to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientDetailAddressEntity> patientDetailAddressEntity = patientDetailAddressService.findById(id);
	...
	/**
	 * Create/Update the given Patient Detail Address. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientDetailAddressEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientDetailAddress' endpoint here] off begin
	@Operation(
			description = "Create a PatientDetailAddress if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientDetailAddress."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientDetailAddress, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientDetailAddressEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientDetailAddress' endpoint here] end
	public ResponseEntity<?> savePatientDetailAddress(
			@Parameter(description = "The PatientDetailAddressEntity to create or update.")
			@RequestBody PatientDetailAddressEntityDto patientDetailAddressEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientDetailAddressEntity patientDetailAddressEntity = patientDetailAddressService.create(new PatientDetailAddressEntity(patientDetailAddressEntityDto));
			return new ResponseEntity<>(patientDetailAddressEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Detail Address. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientDetailAddressEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientDetailAddress' endpoint here] off begin
	@Operation(
			description = "Update a PatientDetailAddress entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientDetailAddress."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientDetailAddress, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientDetailAddressEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientDetailAddress' endpoint here] end
	public ResponseEntity<?> updatePatientDetailAddress(
			@Parameter(description = "The PatientDetailAddressEntity to create or update.")
			@RequestBody PatientDetailAddressEntityDto patientDetailAddressEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientDetailAddressEntity> checkPatientDetailAddressExists = patientDetailAddressService.findById(patientDetailAddressEntityDto.getId());
		PatientDetailAddressEntity patientDetailAddressEntity = new PatientDetailAddressEntity(patientDetailAddressEntityDto);

		if (checkPatientDetailAddressExists.isPresent()) {
			try {
				patientDetailAddressEntity = patientDetailAddressService.update(patientDetailAddressEntity);
				return new ResponseEntity<>(patientDetailAddressEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientDetailAddressEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Detail Address that has the same id as the given id.
	 *
	 * @param id The id of the PatientDetailAddressEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientDetailAddress as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientDetailAddress to delete.")
		...
		patientDetailAddressService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientDetailAddress' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Detail Address entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientDetailAddress' endpoint here] end
	public void exportPatientDetailAddress(
			@Parameter(description = "The ids of the PatientDetailAddressEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientDetailAddressEntity> entitiesToExport = patientDetailAddressService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientDetailAddress_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientDetailAddressEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientDetailAddressExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientDetailAddressExcludingIds' endpoint here] end
	public void exportPatientDetailAddressExcludingIds(
			@Parameter(description = "The ids of the PatientDetailAddressEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientDetailAddressEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientDetailAddressService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientDetailAddressService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientDetailAddress entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientDetailAddress_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientDetailAddressEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Detail Address entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientDetailAddressEntity', 'create')  || hasPermission('PatientDetailAddressEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientDetailAddressEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientDetailAddressEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientDetailAddressEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientDetailAddressEntity> csvToBean = new CsvToBeanBuilder<PatientDetailAddressEntity>(reader)
				.withType(PatientDetailAddressEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientDetailAddressEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientDetailAddressEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Detail Address entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Detail Address entity and you do not have permission");
					}
