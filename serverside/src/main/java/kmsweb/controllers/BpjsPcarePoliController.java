...
import kmsweb.entities.BpjsPcarePoliEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Pcare Poli
 */
// % protected region % [Update the API route or description for the BPJS Pcare PoliController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcarePoliController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcarepoli")
// % protected region % [Update the API route or description for the BPJS Pcare PoliController here] end
public class BpjsPcarePoliController {

	private final BpjsPcarePoliService bpjsPcarePoliService;
	private final CsvUtils<BpjsPcarePoliEntity, BpjsPcarePoliRepository, BpjsPcarePoliEntityAudit,  BpjsPcarePoliService> csvUtils;
	...
	@Autowired
	public BpjsPcarePoliController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcarePoliService, BpjsPcarePoliEntity.class);
	...
	public ResponseEntity<List<BpjsPcarePoliEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcarePoliEntity> bpjsPcarePolis = bpjsPcarePoliService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcarePolis, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Pcare Poli that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcarePoliEntity to be returned
	 * @return the BPJS Pcare Poli that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcarePoli as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcarePoli entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcarePoliEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcarePoliEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcarePoli to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcarePoliEntity> bpjsPcarePoliEntity = bpjsPcarePoliService.findById(id);
		...
		return bpjsPcarePoliEntity
				.map(entity -> new ResponseEntity<>(entity, HttpStatus.OK))
				.orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
	}

	/**
	 * Create/Update the given BPJS Pcare Poli. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcarePoliEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcarePoli' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcarePoli if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcarePoli."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcarePoli, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcarePoliEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcarePoli' endpoint here] end
	public ResponseEntity<?> saveBpjsPcarePoli(
			@Parameter(description = "The BpjsPcarePoliEntity to create or update.")
			@RequestBody BpjsPcarePoliEntityDto bpjsPcarePoliEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcarePoliEntity bpjsPcarePoliEntity = bpjsPcarePoliService.create(new BpjsPcarePoliEntity(bpjsPcarePoliEntityDto));
			return new ResponseEntity<>(bpjsPcarePoliEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Pcare Poli. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcarePoliEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcarePoli' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcarePoli entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcarePoli."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcarePoli, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcarePoliEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcarePoli' endpoint here] end
	public ResponseEntity<?> updateBpjsPcarePoli(
			@Parameter(description = "The BpjsPcarePoliEntity to create or update.")
			@RequestBody BpjsPcarePoliEntityDto bpjsPcarePoliEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcarePoliEntity> checkBpjsPcarePoliExists = bpjsPcarePoliService.findById(bpjsPcarePoliEntityDto.getId());
		BpjsPcarePoliEntity bpjsPcarePoliEntity = new BpjsPcarePoliEntity(bpjsPcarePoliEntityDto);

		if (checkBpjsPcarePoliExists.isPresent()) {
			try {
				bpjsPcarePoliEntity = bpjsPcarePoliService.update(bpjsPcarePoliEntity);
				return new ResponseEntity<>(bpjsPcarePoliEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcarePoliEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Pcare Poli that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcarePoliEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcarePoli as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcarePoli to delete.")
		...
		bpjsPcarePoliService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePoli' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Pcare Poli entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePoli' endpoint here] end
	public void exportBpjsPcarePoli(
			@Parameter(description = "The ids of the BpjsPcarePoliEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcarePoliEntity> entitiesToExport = bpjsPcarePoliService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcarePoli_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcarePoliEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePoliExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePoliExcludingIds' endpoint here] end
	public void exportBpjsPcarePoliExcludingIds(
			@Parameter(description = "The ids of the BpjsPcarePoliEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcarePoliEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcarePoliService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcarePoliService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcarePoli entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcarePoli_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcarePoliEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Pcare Poli entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcarePoliEntity', 'create')  || hasPermission('BpjsPcarePoliEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcarePoliEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcarePoliEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcarePoliEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcarePoliEntity> csvToBean = new CsvToBeanBuilder<BpjsPcarePoliEntity>(reader)
				.withType(BpjsPcarePoliEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcarePoliEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcarePoliEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Pcare Poli entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Pcare Poli entity and you do not have permission");
					}
