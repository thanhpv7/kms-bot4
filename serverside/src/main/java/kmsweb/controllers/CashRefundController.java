...
import kmsweb.entities.CashRefundEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Cash Refund
 */
// % protected region % [Update the API route or description for the Cash RefundController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CashRefundController"))
@RestController
@Slf4j
@RequestMapping("/api/cashrefund")
// % protected region % [Update the API route or description for the Cash RefundController here] end
public class CashRefundController {

	private final CashRefundService cashRefundService;
	private final CsvUtils<CashRefundEntity, CashRefundRepository, CashRefundEntityAudit,  CashRefundService> csvUtils;
	...
	public CashRefundController(
		...
		this.csvUtils = new CsvUtils<>(cashRefundService, CashRefundEntity.class);
	...
	public ResponseEntity<List<CashRefundEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CashRefundEntity> cashRefunds = cashRefundService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(cashRefunds, HttpStatus.OK);
	}

	/**
	 * Return the Cash Refund that has the same id as the given id.
	 *
	 * @param id      The id of the CashRefundEntity to be returned
	 * @return the Cash Refund that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CashRefund as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CashRefund entity not found"),
	})
	@PreAuthorize("hasPermission('CashRefundEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CashRefundEntity> getById(
			@Parameter(description = "The UUID of the CashRefund to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CashRefundEntity> cashRefundEntity = cashRefundService.findById(id);
	...
	/**
	 * Create/Update the given Cash Refund. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param cashRefundEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCashRefund' endpoint here] off begin
	@Operation(
			description = "Create a CashRefund if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CashRefund."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CashRefund, duplicate record found")
	})
	@PreAuthorize("hasPermission('CashRefundEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCashRefund' endpoint here] end
	public ResponseEntity<?> saveCashRefund(
			@Parameter(description = "The CashRefundEntity to create or update.")
			@RequestBody CashRefundEntityDto cashRefundEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CashRefundEntity cashRefundEntity = cashRefundService.create(new CashRefundEntity(cashRefundEntityDto));
			return new ResponseEntity<>(cashRefundEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Cash Refund. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param cashRefundEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCashRefund' endpoint here] off begin
	@Operation(
			description = "Update a CashRefund entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CashRefund."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CashRefund, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CashRefundEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCashRefund' endpoint here] end
	public ResponseEntity<?> updateCashRefund(
			@Parameter(description = "The CashRefundEntity to create or update.")
			@RequestBody CashRefundEntityDto cashRefundEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CashRefundEntity> checkCashRefundExists = cashRefundService.findById(cashRefundEntityDto.getId());
		CashRefundEntity cashRefundEntity = new CashRefundEntity(cashRefundEntityDto);

		if (checkCashRefundExists.isPresent()) {
			try {
				cashRefundEntity = cashRefundService.update(cashRefundEntity);
				return new ResponseEntity<>(cashRefundEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(cashRefundEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Cash Refund that has the same id as the given id.
	 *
	 * @param id The id of the CashRefundEntity to be deleted
	...
	@Operation(
			description = "Delete a single CashRefund as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CashRefund to delete.")
		...
		cashRefundService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashRefund' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Cash Refund entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCashRefund' endpoint here] end
	public void exportCashRefund(
			@Parameter(description = "The ids of the CashRefundEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CashRefundEntity> entitiesToExport = cashRefundService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CashRefund_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CashRefundEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashRefundExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCashRefundExcludingIds' endpoint here] end
	public void exportCashRefundExcludingIds(
			@Parameter(description = "The ids of the CashRefundEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CashRefundEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = cashRefundService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = cashRefundService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CashRefund entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CashRefund_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CashRefundEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Cash Refund entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CashRefundEntity', 'create')  || hasPermission('CashRefundEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CashRefundEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CashRefundEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CashRefundEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CashRefundEntity> csvToBean = new CsvToBeanBuilder<CashRefundEntity>(reader)
				.withType(CashRefundEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CashRefundEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CashRefundEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Cash Refund entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Cash Refund entity and you do not have permission");
					}
