...
import kmsweb.entities.PcareCompoundDrugItemEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding PCare Compound Drug Item
 */
// % protected region % [Update the API route or description for the PCare Compound Drug ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareCompoundDrugItemController"))
@RestController
@Slf4j
@RequestMapping("/api/pcarecompounddrugitem")
// % protected region % [Update the API route or description for the PCare Compound Drug ItemController here] end
public class PcareCompoundDrugItemController {

	private final PcareCompoundDrugItemService pcareCompoundDrugItemService;
	private final CsvUtils<PcareCompoundDrugItemEntity, PcareCompoundDrugItemRepository, PcareCompoundDrugItemEntityAudit,  PcareCompoundDrugItemService> csvUtils;
	...
	@Autowired
	public PcareCompoundDrugItemController(
		...
		this.csvUtils = new CsvUtils<>(pcareCompoundDrugItemService, PcareCompoundDrugItemEntity.class);
	...
	public ResponseEntity<List<PcareCompoundDrugItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareCompoundDrugItemEntity> pcareCompoundDrugItems = pcareCompoundDrugItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareCompoundDrugItems, HttpStatus.OK);
	}

	/**
	 * Return the PCare Compound Drug Item that has the same id as the given id.
	 *
	 * @param id      The id of the PcareCompoundDrugItemEntity to be returned
	 * @return the PCare Compound Drug Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareCompoundDrugItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareCompoundDrugItem entity not found"),
	})
	@PreAuthorize("hasPermission('PcareCompoundDrugItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareCompoundDrugItemEntity> getById(
			@Parameter(description = "The UUID of the PcareCompoundDrugItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareCompoundDrugItemEntity> pcareCompoundDrugItemEntity = pcareCompoundDrugItemService.findById(id);
	...
	/**
	 * Create/Update the given PCare Compound Drug Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareCompoundDrugItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareCompoundDrugItem' endpoint here] off begin
	@Operation(
			description = "Create a PcareCompoundDrugItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareCompoundDrugItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareCompoundDrugItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareCompoundDrugItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareCompoundDrugItem' endpoint here] end
	public ResponseEntity<?> savePcareCompoundDrugItem(
			@Parameter(description = "The PcareCompoundDrugItemEntity to create or update.")
			@RequestBody PcareCompoundDrugItemEntityDto pcareCompoundDrugItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareCompoundDrugItemEntity pcareCompoundDrugItemEntity = pcareCompoundDrugItemService.create(new PcareCompoundDrugItemEntity(pcareCompoundDrugItemEntityDto));
			return new ResponseEntity<>(pcareCompoundDrugItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Compound Drug Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareCompoundDrugItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareCompoundDrugItem' endpoint here] off begin
	@Operation(
			description = "Update a PcareCompoundDrugItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareCompoundDrugItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareCompoundDrugItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareCompoundDrugItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareCompoundDrugItem' endpoint here] end
	public ResponseEntity<?> updatePcareCompoundDrugItem(
			@Parameter(description = "The PcareCompoundDrugItemEntity to create or update.")
			@RequestBody PcareCompoundDrugItemEntityDto pcareCompoundDrugItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareCompoundDrugItemEntity> checkPcareCompoundDrugItemExists = pcareCompoundDrugItemService.findById(pcareCompoundDrugItemEntityDto.getId());
		PcareCompoundDrugItemEntity pcareCompoundDrugItemEntity = new PcareCompoundDrugItemEntity(pcareCompoundDrugItemEntityDto);

		if (checkPcareCompoundDrugItemExists.isPresent()) {
			try {
				pcareCompoundDrugItemEntity = pcareCompoundDrugItemService.update(pcareCompoundDrugItemEntity);
				return new ResponseEntity<>(pcareCompoundDrugItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareCompoundDrugItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Compound Drug Item that has the same id as the given id.
	 *
	 * @param id The id of the PcareCompoundDrugItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareCompoundDrugItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareCompoundDrugItem to delete.")
		...
		pcareCompoundDrugItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrugItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Compound Drug Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrugItem' endpoint here] end
	public void exportPcareCompoundDrugItem(
			@Parameter(description = "The ids of the PcareCompoundDrugItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareCompoundDrugItemEntity> entitiesToExport = pcareCompoundDrugItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareCompoundDrugItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareCompoundDrugItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrugItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrugItemExcludingIds' endpoint here] end
	public void exportPcareCompoundDrugItemExcludingIds(
			@Parameter(description = "The ids of the PcareCompoundDrugItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareCompoundDrugItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareCompoundDrugItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareCompoundDrugItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareCompoundDrugItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareCompoundDrugItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareCompoundDrugItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Compound Drug Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareCompoundDrugItemEntity', 'create')  || hasPermission('PcareCompoundDrugItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareCompoundDrugItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareCompoundDrugItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareCompoundDrugItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareCompoundDrugItemEntity> csvToBean = new CsvToBeanBuilder<PcareCompoundDrugItemEntity>(reader)
				.withType(PcareCompoundDrugItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareCompoundDrugItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareCompoundDrugItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Compound Drug Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Compound Drug Item entity and you do not have permission");
					}
