...
/**
 * Controller used to handle all REST operations regarding ICD 9 CM
 */
// % protected region % [Update the API route or description for the ICD 9 CMController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "Icd9CMController"))
@RestController
@Slf4j
@RequestMapping("/api/icd9cm")
// % protected region % [Update the API route or description for the ICD 9 CMController here] end
public class Icd9CMController {

	private final Icd9CMService icd9CMService;
	private final CsvUtils<Icd9CMEntity, Icd9CMRepository, Icd9CMEntityAudit,  Icd9CMService> csvUtils;
	...
	public Icd9CMController(
		...
		this.csvUtils = new CsvUtils<>(icd9CMService, Icd9CMEntity.class);
	...
	public ResponseEntity<List<Icd9CMEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<Icd9CMEntity> icd9CMs = icd9CMService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(icd9CMs, HttpStatus.OK);
	}

	/**
	 * Return the ICD 9 CM that has the same id as the given id.
	 *
	 * @param id      The id of the Icd9CMEntity to be returned
	 * @return the ICD 9 CM that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Icd9CM as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Icd9CM entity not found"),
	})
	@PreAuthorize("hasPermission('Icd9CMEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<Icd9CMEntity> getById(
			@Parameter(description = "The UUID of the Icd9CM to return.")
			@PathVariable("id") UUID id
	) {
		Optional<Icd9CMEntity> icd9CMEntity = icd9CMService.findById(id);
	...
	/**
	 * Create/Update the given ICD 9 CM. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param icd9CMEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveIcd9CM' endpoint here] off begin
	@Operation(
			description = "Create a Icd9CM if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Icd9CM."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Icd9CM, duplicate record found")
	})
	@PreAuthorize("hasPermission('Icd9CMEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveIcd9CM' endpoint here] end
	public ResponseEntity<?> saveIcd9CM(
			@Parameter(description = "The Icd9CMEntity to create or update.")
			@RequestBody Icd9CMEntityDto icd9CMEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			Icd9CMEntity icd9CMEntity = icd9CMService.create(new Icd9CMEntity(icd9CMEntityDto));
			return new ResponseEntity<>(icd9CMEntity, HttpStatus.OK);
	...
	/**
	 * Update the given ICD 9 CM. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param icd9CMEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateIcd9CM' endpoint here] off begin
	@Operation(
			description = "Update a Icd9CM entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Icd9CM."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Icd9CM, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('Icd9CMEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateIcd9CM' endpoint here] end
	public ResponseEntity<?> updateIcd9CM(
			@Parameter(description = "The Icd9CMEntity to create or update.")
			@RequestBody Icd9CMEntityDto icd9CMEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<Icd9CMEntity> checkIcd9CMExists = icd9CMService.findById(icd9CMEntityDto.getId());
		Icd9CMEntity icd9CMEntity = new Icd9CMEntity(icd9CMEntityDto);

		if (checkIcd9CMExists.isPresent()) {
			try {
				icd9CMEntity = icd9CMService.update(icd9CMEntity);
				return new ResponseEntity<>(icd9CMEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(icd9CMEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the ICD 9 CM that has the same id as the given id.
	 *
	 * @param id The id of the Icd9CMEntity to be deleted
	...
	@Operation(
			description = "Delete a single Icd9CM as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Icd9CM to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportIcd9CM' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all ICD 9 CM entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportIcd9CM' endpoint here] end
	public void exportIcd9CM(
			@Parameter(description = "The ids of the Icd9CMEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<Icd9CMEntity> entitiesToExport = icd9CMService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Icd9CM_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (Icd9CMEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportIcd9CMExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportIcd9CMExcludingIds' endpoint here] end
	public void exportIcd9CMExcludingIds(
			@Parameter(description = "The ids of the Icd9CMEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<Icd9CMEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = icd9CMService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = icd9CMService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Icd9CM entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Icd9CM_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(Icd9CMEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for ICD 9 CM entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('Icd9CMEntity', 'create')  || hasPermission('Icd9CMEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with Icd9CMEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("Icd9CMEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(Icd9CMEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<Icd9CMEntity> csvToBean = new CsvToBeanBuilder<Icd9CMEntity>(reader)
				.withType(Icd9CMEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<Icd9CMEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<Icd9CMEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a ICD 9 CM entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a ICD 9 CM entity and you do not have permission");
					}
