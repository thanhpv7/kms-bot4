...
import kmsweb.entities.BpjsControlSpecialistEntityAudit;
import kmsweb.repositories.BpjsControlSpecialistRepository;
import kmsweb.services.BpjsControlSpecialistService;
...
/**
 * Controller used to handle all REST operations regarding BPJS Control Specialist
 */
// % protected region % [Update the API route or description for the BPJS Control SpecialistController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsControlSpecialistController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjscontrolspecialist")
// % protected region % [Update the API route or description for the BPJS Control SpecialistController here] end
public class BpjsControlSpecialistController {

	private final BpjsControlSpecialistService bpjsControlSpecialistService;
	private final CsvUtils<BpjsControlSpecialistEntity, BpjsControlSpecialistRepository, BpjsControlSpecialistEntityAudit,  BpjsControlSpecialistService> csvUtils;
	...
	@Autowired
	public BpjsControlSpecialistController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsControlSpecialistService bpjsControlSpecialistService
	) {
		this.authService = authService;
		this.bpjsControlSpecialistService = bpjsControlSpecialistService;
		this.csvUtils = new CsvUtils<>(bpjsControlSpecialistService, BpjsControlSpecialistEntity.class);
	...
	public ResponseEntity<List<BpjsControlSpecialistEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsControlSpecialistEntity> bpjsControlSpecialists = bpjsControlSpecialistService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsControlSpecialists, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Control Specialist that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsControlSpecialistEntity to be returned
	 * @return the BPJS Control Specialist that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsControlSpecialist as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsControlSpecialist entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsControlSpecialistEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsControlSpecialistEntity> getById(
			@Parameter(description = "The UUID of the BpjsControlSpecialist to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsControlSpecialistEntity> bpjsControlSpecialistEntity = bpjsControlSpecialistService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Control Specialist. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsControlSpecialistEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsControlSpecialist' endpoint here] off begin
	@Operation(
			description = "Create a BpjsControlSpecialist if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsControlSpecialist."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsControlSpecialist, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsControlSpecialistEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsControlSpecialist' endpoint here] end
	public ResponseEntity<?> saveBpjsControlSpecialist(
			@Parameter(description = "The BpjsControlSpecialistEntity to create or update.")
			@RequestBody BpjsControlSpecialistEntityDto bpjsControlSpecialistEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsControlSpecialistEntity bpjsControlSpecialistEntity = bpjsControlSpecialistService.create(new BpjsControlSpecialistEntity(bpjsControlSpecialistEntityDto));
			return new ResponseEntity<>(bpjsControlSpecialistEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Control Specialist. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsControlSpecialistEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsControlSpecialist' endpoint here] off begin
	@Operation(
			description = "Update a BpjsControlSpecialist entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsControlSpecialist."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsControlSpecialist, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsControlSpecialistEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsControlSpecialist' endpoint here] end
	public ResponseEntity<?> updateBpjsControlSpecialist(
			@Parameter(description = "The BpjsControlSpecialistEntity to create or update.")
			@RequestBody BpjsControlSpecialistEntityDto bpjsControlSpecialistEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsControlSpecialistEntity> checkBpjsControlSpecialistExists = bpjsControlSpecialistService.findById(bpjsControlSpecialistEntityDto.getId());
		BpjsControlSpecialistEntity bpjsControlSpecialistEntity = new BpjsControlSpecialistEntity(bpjsControlSpecialistEntityDto);

		if (checkBpjsControlSpecialistExists.isPresent()) {
			try {
				bpjsControlSpecialistEntity = bpjsControlSpecialistService.update(bpjsControlSpecialistEntity);
				return new ResponseEntity<>(bpjsControlSpecialistEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsControlSpecialistEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Control Specialist that has the same id as the given id.
	 *
	 * @param id The id of the BpjsControlSpecialistEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsControlSpecialist as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsControlSpecialist to delete.")
		...
		bpjsControlSpecialistService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlSpecialist' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Control Specialist entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlSpecialist' endpoint here] end
	public void exportBpjsControlSpecialist(
			@Parameter(description = "The ids of the BpjsControlSpecialistEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsControlSpecialistEntity> entitiesToExport = bpjsControlSpecialistService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsControlSpecialist_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsControlSpecialistEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlSpecialistExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlSpecialistExcludingIds' endpoint here] end
	public void exportBpjsControlSpecialistExcludingIds(
			@Parameter(description = "The ids of the BpjsControlSpecialistEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsControlSpecialistEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsControlSpecialistService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsControlSpecialistService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsControlSpecialist entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsControlSpecialist_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsControlSpecialistEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Control Specialist entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsControlSpecialistEntity', 'create')  || hasPermission('BpjsControlSpecialistEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsControlSpecialistEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsControlSpecialistEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsControlSpecialistEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsControlSpecialistEntity> csvToBean = new CsvToBeanBuilder<BpjsControlSpecialistEntity>(reader)
				.withType(BpjsControlSpecialistEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsControlSpecialistEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsControlSpecialistEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Control Specialist entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Control Specialist entity and you do not have permission");
					}
