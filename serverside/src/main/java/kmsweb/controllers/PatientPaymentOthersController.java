...
/**
 * Controller used to handle all REST operations regarding Patient Payment Others
 */
// % protected region % [Update the API route or description for the Patient Payment OthersController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientPaymentOthersController"))
@RestController
@Slf4j
@RequestMapping("/api/patientpaymentothers")
// % protected region % [Update the API route or description for the Patient Payment OthersController here] end
public class PatientPaymentOthersController {

	private final PatientPaymentOthersService patientPaymentOthersService;
	private final CsvUtils<PatientPaymentOthersEntity, PatientPaymentOthersRepository, PatientPaymentOthersEntityAudit,  PatientPaymentOthersService> csvUtils;
	...
	@Autowired
	public PatientPaymentOthersController(
		...
		this.csvUtils = new CsvUtils<>(patientPaymentOthersService, PatientPaymentOthersEntity.class);
	...
	public ResponseEntity<List<PatientPaymentOthersEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientPaymentOthersEntity> patientPaymentOtherss = patientPaymentOthersService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientPaymentOtherss, HttpStatus.OK);
	}

	/**
	 * Return the Patient Payment Others that has the same id as the given id.
	 *
	 * @param id      The id of the PatientPaymentOthersEntity to be returned
	 * @return the Patient Payment Others that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientPaymentOthers as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientPaymentOthers entity not found"),
	})
	@PreAuthorize("hasPermission('PatientPaymentOthersEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientPaymentOthersEntity> getById(
			@Parameter(description = "The UUID of the PatientPaymentOthers to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientPaymentOthersEntity> patientPaymentOthersEntity = patientPaymentOthersService.findById(id);
	...
	/**
	 * Create/Update the given Patient Payment Others. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentOthersEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentOthers' endpoint here] off begin
	@Operation(
			description = "Create a PatientPaymentOthers if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientPaymentOthers."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientPaymentOthers, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientPaymentOthersEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentOthers' endpoint here] end
	public ResponseEntity<?> savePatientPaymentOthers(
			@Parameter(description = "The PatientPaymentOthersEntity to create or update.")
			@RequestBody PatientPaymentOthersEntityDto patientPaymentOthersEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientPaymentOthersEntity patientPaymentOthersEntity = patientPaymentOthersService.create(new PatientPaymentOthersEntity(patientPaymentOthersEntityDto));
			return new ResponseEntity<>(patientPaymentOthersEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Payment Others. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentOthersEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentOthers' endpoint here] off begin
	@Operation(
			description = "Update a PatientPaymentOthers entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientPaymentOthers."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientPaymentOthers, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientPaymentOthersEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentOthers' endpoint here] end
	public ResponseEntity<?> updatePatientPaymentOthers(
			@Parameter(description = "The PatientPaymentOthersEntity to create or update.")
			@RequestBody PatientPaymentOthersEntityDto patientPaymentOthersEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientPaymentOthersEntity> checkPatientPaymentOthersExists = patientPaymentOthersService.findById(patientPaymentOthersEntityDto.getId());
		PatientPaymentOthersEntity patientPaymentOthersEntity = new PatientPaymentOthersEntity(patientPaymentOthersEntityDto);

		if (checkPatientPaymentOthersExists.isPresent()) {
			try {
				patientPaymentOthersEntity = patientPaymentOthersService.update(patientPaymentOthersEntity);
				return new ResponseEntity<>(patientPaymentOthersEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientPaymentOthersEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Payment Others that has the same id as the given id.
	 *
	 * @param id The id of the PatientPaymentOthersEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientPaymentOthers as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientPaymentOthers to delete.")
		...
		patientPaymentOthersService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentOthers' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Payment Others entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentOthers' endpoint here] end
	public void exportPatientPaymentOthers(
			@Parameter(description = "The ids of the PatientPaymentOthersEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientPaymentOthersEntity> entitiesToExport = patientPaymentOthersService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientPaymentOthers_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientPaymentOthersEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentOthersExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentOthersExcludingIds' endpoint here] end
	public void exportPatientPaymentOthersExcludingIds(
			@Parameter(description = "The ids of the PatientPaymentOthersEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientPaymentOthersEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientPaymentOthersService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientPaymentOthersService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientPaymentOthers entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientPaymentOthers_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientPaymentOthersEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Payment Others entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientPaymentOthersEntity', 'create')  || hasPermission('PatientPaymentOthersEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientPaymentOthersEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientPaymentOthersEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientPaymentOthersEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientPaymentOthersEntity> csvToBean = new CsvToBeanBuilder<PatientPaymentOthersEntity>(reader)
				.withType(PatientPaymentOthersEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientPaymentOthersEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientPaymentOthersEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Payment Others entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Payment Others entity and you do not have permission");
					}
