...
import kmsweb.entities.AdministratorEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Administrator
 */
// % protected region % [Update the API route or description for the AdministratorController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "AdministratorController"))
@RestController
@Slf4j
@RequestMapping("/api/administrator")
// % protected region % [Update the API route or description for the AdministratorController here] end
public class AdministratorController {

	private final AdministratorService administratorService;
	private final CsvUtils<AdministratorEntity, AdministratorRepository, AdministratorEntityAudit,  AdministratorService> csvUtils;
	...
	public AdministratorController(
		...
		this.csvUtils = new CsvUtils<>(administratorService, AdministratorEntity.class);
	...
	@Operation(
			description = "Returns a single page of Administrators",
		security = {
	...
	public ResponseEntity<List<AdministratorEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<AdministratorEntity> administrators = administratorService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Administrator that has the same id as the given id.
	 *
	 * @param id      The id of the AdministratorEntity to be returned
	 * @return the Administrator that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Administrator as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Administrator entity not found"),
	})
	@PreAuthorize("hasPermission('AdministratorEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<AdministratorEntity> getById(
			@Parameter(description = "The UUID of the Administrator to return.")
			@PathVariable("id") UUID id
	) {
		Optional<AdministratorEntity> administratorEntity = administratorService.findById(id);
	...
	/**
	 * Create/Update the given Administrator. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param administratorEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveAdministrator' endpoint here] off begin
	@Operation(
			description = "Create a Administrator if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Administrator."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Administrator, duplicate record found")
	})
	@PreAuthorize("hasPermission('AdministratorEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveAdministrator' endpoint here] end
	public ResponseEntity<?> saveAdministrator(
			@Parameter(description = "The AdministratorEntity to create or update.")
			@RequestBody AdministratorEntityDto administratorEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			AdministratorEntity administratorEntity = administratorService.create(new AdministratorEntity(administratorEntityDto));
			return new ResponseEntity<>(administratorEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Administrator. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param administratorEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateAdministrator' endpoint here] off begin
	@Operation(
			description = "Update a Administrator entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Administrator."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Administrator, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('AdministratorEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateAdministrator' endpoint here] end
	public ResponseEntity<?> updateAdministrator(
		...
		Optional<AdministratorEntity> checkAdministratorExists = administratorService.findById(administratorEntityDto.getId());
		AdministratorEntity administratorEntity = new AdministratorEntity(administratorEntityDto);

		if (checkAdministratorExists.isPresent()) {
			try {
				administratorEntity = administratorService.update(administratorEntity);
				return new ResponseEntity<>(administratorEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(administratorEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Administrator that has the same id as the given id.
	 *
	 * @param id The id of the AdministratorEntity to be deleted
	...
	@Operation(
			description = "Delete a single Administrator as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('AdministratorEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Administrator to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportAdministrator' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Administrator entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportAdministrator' endpoint here] end
	public void exportAdministrator(
			@Parameter(description = "The ids of the AdministratorEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<AdministratorEntity> entitiesToExport = administratorService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Administrator_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (AdministratorEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportAdministratorExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportAdministratorExcludingIds' endpoint here] end
	public void exportAdministratorExcludingIds(
			@Parameter(description = "The ids of the AdministratorEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<AdministratorEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = administratorService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = administratorService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Administrator entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Administrator_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(AdministratorEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Administrator entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('AdministratorEntity', 'create')  || hasPermission('AdministratorEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with AdministratorEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("AdministratorEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(AdministratorEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<AdministratorEntity> csvToBean = new CsvToBeanBuilder<AdministratorEntity>(reader)
				.withType(AdministratorEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<AdministratorEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<AdministratorEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Administrator entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Administrator entity and you do not have permission");
					}
