...
import kmsweb.entities.DiagnosticSupportGeneralConfigurationEntityAudit;
import kmsweb.repositories.DiagnosticSupportGeneralConfigurationRepository;
import kmsweb.services.DiagnosticSupportGeneralConfigurationService;
...
/**
 * Controller used to handle all REST operations regarding Diagnostic Support General Configuration
 */
// % protected region % [Update the API route or description for the Diagnostic Support General ConfigurationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DiagnosticSupportGeneralConfigurationController"))
@RestController
@Slf4j
@RequestMapping("/api/diagnosticsupportgeneralconfiguration")
// % protected region % [Update the API route or description for the Diagnostic Support General ConfigurationController here] end
public class DiagnosticSupportGeneralConfigurationController {

	private final DiagnosticSupportGeneralConfigurationService diagnosticSupportGeneralConfigurationService;
	private final CsvUtils<DiagnosticSupportGeneralConfigurationEntity, DiagnosticSupportGeneralConfigurationRepository, DiagnosticSupportGeneralConfigurationEntityAudit,  DiagnosticSupportGeneralConfigurationService> csvUtils;
	...
	@Autowired
	public DiagnosticSupportGeneralConfigurationController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			DiagnosticSupportGeneralConfigurationService diagnosticSupportGeneralConfigurationService
	) {
		this.authService = authService;
		this.diagnosticSupportGeneralConfigurationService = diagnosticSupportGeneralConfigurationService;
		this.csvUtils = new CsvUtils<>(diagnosticSupportGeneralConfigurationService, DiagnosticSupportGeneralConfigurationEntity.class);
	...
	/**
	 * Return all the Diagnostic Support General Configurations.
	 *
	 * @return all the Diagnostic Support General Configurations
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of DiagnosticSupportGeneralConfigurations",
		security = {
	...
	@PreAuthorize("hasPermission('DiagnosticSupportGeneralConfigurationEntity', 'read')")
	@GetMapping(produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] end
	public ResponseEntity<List<DiagnosticSupportGeneralConfigurationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DiagnosticSupportGeneralConfigurationEntity> diagnosticSupportGeneralConfigurations = diagnosticSupportGeneralConfigurationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(diagnosticSupportGeneralConfigurations, HttpStatus.OK);
	}

	/**
	 * Return the Diagnostic Support General Configuration that has the same id as the given id.
	 *
	 * @param id      The id of the DiagnosticSupportGeneralConfigurationEntity to be returned
	 * @return the Diagnostic Support General Configuration that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DiagnosticSupportGeneralConfiguration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DiagnosticSupportGeneralConfiguration entity not found"),
	})
	@PreAuthorize("hasPermission('DiagnosticSupportGeneralConfigurationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DiagnosticSupportGeneralConfigurationEntity> getById(
			@Parameter(description = "The UUID of the DiagnosticSupportGeneralConfiguration to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DiagnosticSupportGeneralConfigurationEntity> diagnosticSupportGeneralConfigurationEntity = diagnosticSupportGeneralConfigurationService.findById(id);
		...
		return diagnosticSupportGeneralConfigurationEntity
				.map(entity -> new ResponseEntity<>(entity, HttpStatus.OK))
				.orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
	}

	/**
	 * Create/Update the given Diagnostic Support General Configuration. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosticSupportGeneralConfigurationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDiagnosticSupportGeneralConfiguration' endpoint here] off begin
	@Operation(
			description = "Create a DiagnosticSupportGeneralConfiguration if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DiagnosticSupportGeneralConfiguration."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DiagnosticSupportGeneralConfiguration, duplicate record found")
	})
	@PreAuthorize("hasPermission('DiagnosticSupportGeneralConfigurationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDiagnosticSupportGeneralConfiguration' endpoint here] end
	public ResponseEntity<?> saveDiagnosticSupportGeneralConfiguration(
			@Parameter(description = "The DiagnosticSupportGeneralConfigurationEntity to create or update.")
			@RequestBody DiagnosticSupportGeneralConfigurationEntityDto diagnosticSupportGeneralConfigurationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DiagnosticSupportGeneralConfigurationEntity diagnosticSupportGeneralConfigurationEntity = diagnosticSupportGeneralConfigurationService.create(new DiagnosticSupportGeneralConfigurationEntity(diagnosticSupportGeneralConfigurationEntityDto));
			return new ResponseEntity<>(diagnosticSupportGeneralConfigurationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Diagnostic Support General Configuration. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosticSupportGeneralConfigurationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDiagnosticSupportGeneralConfiguration' endpoint here] off begin
	@Operation(
			description = "Update a DiagnosticSupportGeneralConfiguration entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DiagnosticSupportGeneralConfiguration."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DiagnosticSupportGeneralConfiguration, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DiagnosticSupportGeneralConfigurationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDiagnosticSupportGeneralConfiguration' endpoint here] end
	public ResponseEntity<?> updateDiagnosticSupportGeneralConfiguration(
			@Parameter(description = "The DiagnosticSupportGeneralConfigurationEntity to create or update.")
			@RequestBody DiagnosticSupportGeneralConfigurationEntityDto diagnosticSupportGeneralConfigurationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DiagnosticSupportGeneralConfigurationEntity> checkDiagnosticSupportGeneralConfigurationExists = diagnosticSupportGeneralConfigurationService.findById(diagnosticSupportGeneralConfigurationEntityDto.getId());
		DiagnosticSupportGeneralConfigurationEntity diagnosticSupportGeneralConfigurationEntity = new DiagnosticSupportGeneralConfigurationEntity(diagnosticSupportGeneralConfigurationEntityDto);

		if (checkDiagnosticSupportGeneralConfigurationExists.isPresent()) {
			try {
				diagnosticSupportGeneralConfigurationEntity = diagnosticSupportGeneralConfigurationService.update(diagnosticSupportGeneralConfigurationEntity);
				return new ResponseEntity<>(diagnosticSupportGeneralConfigurationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(diagnosticSupportGeneralConfigurationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Diagnostic Support General Configuration that has the same id as the given id.
	 *
	 * @param id The id of the DiagnosticSupportGeneralConfigurationEntity to be deleted
	...
	@Operation(
			description = "Delete a single DiagnosticSupportGeneralConfiguration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('DiagnosticSupportGeneralConfigurationEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DiagnosticSupportGeneralConfiguration to delete.")
		...
		diagnosticSupportGeneralConfigurationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportGeneralConfiguration' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Diagnostic Support General Configuration entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportGeneralConfiguration' endpoint here] end
	public void exportDiagnosticSupportGeneralConfiguration(
			@Parameter(description = "The ids of the DiagnosticSupportGeneralConfigurationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DiagnosticSupportGeneralConfigurationEntity> entitiesToExport = diagnosticSupportGeneralConfigurationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DiagnosticSupportGeneralConfiguration_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DiagnosticSupportGeneralConfigurationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportGeneralConfigurationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportGeneralConfigurationExcludingIds' endpoint here] end
	public void exportDiagnosticSupportGeneralConfigurationExcludingIds(
			@Parameter(description = "The ids of the DiagnosticSupportGeneralConfigurationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DiagnosticSupportGeneralConfigurationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = diagnosticSupportGeneralConfigurationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = diagnosticSupportGeneralConfigurationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DiagnosticSupportGeneralConfiguration entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DiagnosticSupportGeneralConfiguration_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DiagnosticSupportGeneralConfigurationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Diagnostic Support General Configuration entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DiagnosticSupportGeneralConfigurationEntity', 'create')  || hasPermission('DiagnosticSupportGeneralConfigurationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DiagnosticSupportGeneralConfigurationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DiagnosticSupportGeneralConfigurationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DiagnosticSupportGeneralConfigurationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DiagnosticSupportGeneralConfigurationEntity> csvToBean = new CsvToBeanBuilder<DiagnosticSupportGeneralConfigurationEntity>(reader)
				.withType(DiagnosticSupportGeneralConfigurationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DiagnosticSupportGeneralConfigurationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DiagnosticSupportGeneralConfigurationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Diagnostic Support General Configuration entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Diagnostic Support General Configuration entity and you do not have permission");
					}
