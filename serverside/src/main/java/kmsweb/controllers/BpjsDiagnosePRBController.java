...
/**
 * Controller used to handle all REST operations regarding BPJS Diagnose PRB
 */
// % protected region % [Update the API route or description for the BPJS Diagnose PRBController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsDiagnosePRBController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsdiagnoseprb")
// % protected region % [Update the API route or description for the BPJS Diagnose PRBController here] end
public class BpjsDiagnosePRBController {

	private final BpjsDiagnosePRBService bpjsDiagnosePRBService;
	private final CsvUtils<BpjsDiagnosePRBEntity, BpjsDiagnosePRBRepository, BpjsDiagnosePRBEntityAudit,  BpjsDiagnosePRBService> csvUtils;
	...
	@Autowired
	public BpjsDiagnosePRBController(
		...
		this.csvUtils = new CsvUtils<>(bpjsDiagnosePRBService, BpjsDiagnosePRBEntity.class);
	...
	public ResponseEntity<List<BpjsDiagnosePRBEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsDiagnosePRBEntity> bpjsDiagnosePRBs = bpjsDiagnosePRBService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsDiagnosePRBs, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Diagnose PRB that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsDiagnosePRBEntity to be returned
	 * @return the BPJS Diagnose PRB that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsDiagnosePRB as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsDiagnosePRB entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsDiagnosePRBEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsDiagnosePRBEntity> getById(
			@Parameter(description = "The UUID of the BpjsDiagnosePRB to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsDiagnosePRBEntity> bpjsDiagnosePRBEntity = bpjsDiagnosePRBService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Diagnose PRB. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDiagnosePRBEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsDiagnosePRB' endpoint here] off begin
	@Operation(
			description = "Create a BpjsDiagnosePRB if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsDiagnosePRB."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsDiagnosePRB, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsDiagnosePRBEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsDiagnosePRB' endpoint here] end
	public ResponseEntity<?> saveBpjsDiagnosePRB(
			@Parameter(description = "The BpjsDiagnosePRBEntity to create or update.")
			@RequestBody BpjsDiagnosePRBEntityDto bpjsDiagnosePRBEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsDiagnosePRBEntity bpjsDiagnosePRBEntity = bpjsDiagnosePRBService.create(new BpjsDiagnosePRBEntity(bpjsDiagnosePRBEntityDto));
			return new ResponseEntity<>(bpjsDiagnosePRBEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Diagnose PRB. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDiagnosePRBEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsDiagnosePRB' endpoint here] off begin
	@Operation(
			description = "Update a BpjsDiagnosePRB entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsDiagnosePRB."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsDiagnosePRB, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsDiagnosePRBEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsDiagnosePRB' endpoint here] end
	public ResponseEntity<?> updateBpjsDiagnosePRB(
			@Parameter(description = "The BpjsDiagnosePRBEntity to create or update.")
			@RequestBody BpjsDiagnosePRBEntityDto bpjsDiagnosePRBEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsDiagnosePRBEntity> checkBpjsDiagnosePRBExists = bpjsDiagnosePRBService.findById(bpjsDiagnosePRBEntityDto.getId());
		BpjsDiagnosePRBEntity bpjsDiagnosePRBEntity = new BpjsDiagnosePRBEntity(bpjsDiagnosePRBEntityDto);

		if (checkBpjsDiagnosePRBExists.isPresent()) {
			try {
				bpjsDiagnosePRBEntity = bpjsDiagnosePRBService.update(bpjsDiagnosePRBEntity);
				return new ResponseEntity<>(bpjsDiagnosePRBEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsDiagnosePRBEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Diagnose PRB that has the same id as the given id.
	 *
	 * @param id The id of the BpjsDiagnosePRBEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsDiagnosePRB as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsDiagnosePRB to delete.")
		...
		bpjsDiagnosePRBService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnosePRB' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Diagnose PRB entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnosePRB' endpoint here] end
	public void exportBpjsDiagnosePRB(
			@Parameter(description = "The ids of the BpjsDiagnosePRBEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsDiagnosePRBEntity> entitiesToExport = bpjsDiagnosePRBService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsDiagnosePRB_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsDiagnosePRBEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnosePRBExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnosePRBExcludingIds' endpoint here] end
	public void exportBpjsDiagnosePRBExcludingIds(
			@Parameter(description = "The ids of the BpjsDiagnosePRBEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsDiagnosePRBEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsDiagnosePRBService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsDiagnosePRBService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsDiagnosePRB entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsDiagnosePRB_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsDiagnosePRBEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Diagnose PRB entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsDiagnosePRBEntity', 'create')  || hasPermission('BpjsDiagnosePRBEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsDiagnosePRBEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsDiagnosePRBEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsDiagnosePRBEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsDiagnosePRBEntity> csvToBean = new CsvToBeanBuilder<BpjsDiagnosePRBEntity>(reader)
				.withType(BpjsDiagnosePRBEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsDiagnosePRBEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsDiagnosePRBEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Diagnose PRB entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Diagnose PRB entity and you do not have permission");
					}
