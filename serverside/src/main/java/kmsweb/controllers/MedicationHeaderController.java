...
import kmsweb.entities.MedicationHeaderEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medication Header
 */
// % protected region % [Update the API route or description for the Medication HeaderController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicationHeaderController"))
@RestController
@Slf4j
@RequestMapping("/api/medicationheader")
// % protected region % [Update the API route or description for the Medication HeaderController here] end
public class MedicationHeaderController {

	private final MedicationHeaderService medicationHeaderService;
	private final CsvUtils<MedicationHeaderEntity, MedicationHeaderRepository, MedicationHeaderEntityAudit,  MedicationHeaderService> csvUtils;
	...
	@Autowired
	public MedicationHeaderController(
		...
		this.csvUtils = new CsvUtils<>(medicationHeaderService, MedicationHeaderEntity.class);
	...
	public ResponseEntity<List<MedicationHeaderEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicationHeaderEntity> medicationHeaders = medicationHeaderService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicationHeaders, HttpStatus.OK);
	}

	/**
	 * Return the Medication Header that has the same id as the given id.
	 *
	 * @param id      The id of the MedicationHeaderEntity to be returned
	 * @return the Medication Header that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicationHeader as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicationHeader entity not found"),
	})
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicationHeaderEntity> getById(
			@Parameter(description = "The UUID of the MedicationHeader to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicationHeaderEntity> medicationHeaderEntity = medicationHeaderService.findById(id);
	...
	/**
	 * Create/Update the given Medication Header. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicationHeaderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicationHeader' endpoint here] off begin
	@Operation(
			description = "Create a MedicationHeader if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicationHeader."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicationHeader, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicationHeader' endpoint here] end
	public ResponseEntity<?> saveMedicationHeader(
			@Parameter(description = "The MedicationHeaderEntity to create or update.")
			@RequestBody MedicationHeaderEntityDto medicationHeaderEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicationHeaderEntity medicationHeaderEntity = medicationHeaderService.create(new MedicationHeaderEntity(medicationHeaderEntityDto));
			return new ResponseEntity<>(medicationHeaderEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medication Header. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicationHeaderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicationHeader' endpoint here] off begin
	@Operation(
			description = "Update a MedicationHeader entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicationHeader."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicationHeader, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicationHeader' endpoint here] end
	public ResponseEntity<?> updateMedicationHeader(
			@Parameter(description = "The MedicationHeaderEntity to create or update.")
			@RequestBody MedicationHeaderEntityDto medicationHeaderEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicationHeaderEntity> checkMedicationHeaderExists = medicationHeaderService.findById(medicationHeaderEntityDto.getId());
		MedicationHeaderEntity medicationHeaderEntity = new MedicationHeaderEntity(medicationHeaderEntityDto);

		if (checkMedicationHeaderExists.isPresent()) {
			try {
				medicationHeaderEntity = medicationHeaderService.update(medicationHeaderEntity);
				return new ResponseEntity<>(medicationHeaderEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicationHeaderEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medication Header that has the same id as the given id.
	 *
	 * @param id The id of the MedicationHeaderEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicationHeader as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicationHeader to delete.")
		...
		medicationHeaderService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationHeader' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medication Header entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationHeader' endpoint here] end
	public void exportMedicationHeader(
			@Parameter(description = "The ids of the MedicationHeaderEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicationHeaderEntity> entitiesToExport = medicationHeaderService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicationHeader_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicationHeaderEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationHeaderExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationHeaderExcludingIds' endpoint here] end
	public void exportMedicationHeaderExcludingIds(
			@Parameter(description = "The ids of the MedicationHeaderEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicationHeaderEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicationHeaderService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicationHeaderService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicationHeader entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicationHeader_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicationHeaderEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medication Header entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'create')  || hasPermission('MedicationHeaderEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicationHeaderEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicationHeaderEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicationHeaderEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicationHeaderEntity> csvToBean = new CsvToBeanBuilder<MedicationHeaderEntity>(reader)
				.withType(MedicationHeaderEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicationHeaderEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicationHeaderEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medication Header entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medication Header entity and you do not have permission");
					}
