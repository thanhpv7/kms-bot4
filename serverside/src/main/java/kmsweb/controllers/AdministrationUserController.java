...
import kmsweb.dtos.AdministrationUserEntityDto;
...
/**
 * Controller used to handle all REST operations regarding Administration User
 */
// % protected region % [Update the API route or description for the Administration UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "AdministrationUserController"))
@RestController
@Slf4j
@RequestMapping("/api/administrationuser")
// % protected region % [Update the API route or description for the Administration UserController here] end
public class AdministrationUserController {

	private final AdministrationUserService administrationUserService;
	private final CsvUtils<AdministrationUserEntity, AdministrationUserRepository, AdministrationUserEntityAudit,  AdministrationUserService> csvUtils;
	...
	public AdministrationUserController(
		...
		this.csvUtils = new CsvUtils<>(administrationUserService, AdministrationUserEntity.class);
	...
	public ResponseEntity<List<AdministrationUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<AdministrationUserEntity> administrationUsers = administrationUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(administrationUsers, HttpStatus.OK);
	}

	/**
	 * Return the Administration User that has the same id as the given id.
	 *
	 * @param id      The id of the AdministrationUserEntity to be returned
	 * @return the Administration User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single AdministrationUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "AdministrationUser entity not found"),
	})
	@PreAuthorize("hasPermission('AdministrationUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<AdministrationUserEntity> getById(
			@Parameter(description = "The UUID of the AdministrationUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<AdministrationUserEntity> administrationUserEntity = administrationUserService.findById(id);
	...
	/**
	 * Create/Update the given Administration User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param administrationUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveAdministrationUser' endpoint here] off begin
	@Operation(
			description = "Create a AdministrationUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new AdministrationUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new AdministrationUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('AdministrationUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveAdministrationUser' endpoint here] end
	public ResponseEntity<?> saveAdministrationUser(
			@Parameter(description = "The AdministrationUserEntity to create or update.")
			@RequestBody AdministrationUserEntityDto administrationUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			AdministrationUserEntity administrationUserEntity = administrationUserService.create(new AdministrationUserEntity(administrationUserEntityDto));
			return new ResponseEntity<>(administrationUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Administration User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param administrationUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateAdministrationUser' endpoint here] off begin
	@Operation(
			description = "Update a AdministrationUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the AdministrationUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the AdministrationUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('AdministrationUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateAdministrationUser' endpoint here] end
	public ResponseEntity<?> updateAdministrationUser(
			@Parameter(description = "The AdministrationUserEntity to create or update.")
			@RequestBody AdministrationUserEntityDto administrationUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<AdministrationUserEntity> checkAdministrationUserExists = administrationUserService.findById(administrationUserEntityDto.getId());
		AdministrationUserEntity administrationUserEntity = new AdministrationUserEntity(administrationUserEntityDto);

		if (checkAdministrationUserExists.isPresent()) {
			try {
				administrationUserEntity = administrationUserService.update(administrationUserEntity);
				return new ResponseEntity<>(administrationUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(administrationUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Administration User that has the same id as the given id.
	 *
	 * @param id The id of the AdministrationUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single AdministrationUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the AdministrationUser to delete.")
		...
		administrationUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportAdministrationUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Administration User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportAdministrationUser' endpoint here] end
	public void exportAdministrationUser(
			@Parameter(description = "The ids of the AdministrationUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<AdministrationUserEntity> entitiesToExport = administrationUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "AdministrationUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (AdministrationUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportAdministrationUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportAdministrationUserExcludingIds' endpoint here] end
	public void exportAdministrationUserExcludingIds(
			@Parameter(description = "The ids of the AdministrationUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<AdministrationUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = administrationUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = administrationUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the AdministrationUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "AdministrationUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(AdministrationUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Administration User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('AdministrationUserEntity', 'create')  || hasPermission('AdministrationUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with AdministrationUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("AdministrationUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(AdministrationUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<AdministrationUserEntity> csvToBean = new CsvToBeanBuilder<AdministrationUserEntity>(reader)
				.withType(AdministrationUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<AdministrationUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<AdministrationUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Administration User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Administration User entity and you do not have permission");
					}
