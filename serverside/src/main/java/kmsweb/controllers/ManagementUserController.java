...
import kmsweb.dtos.ManagementUserEntityDto;
...
/**
 * Controller used to handle all REST operations regarding Management User
 */
// % protected region % [Update the API route or description for the Management UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ManagementUserController"))
@RestController
@Slf4j
@RequestMapping("/api/managementuser")
// % protected region % [Update the API route or description for the Management UserController here] end
public class ManagementUserController {

	private final ManagementUserService managementUserService;
	private final CsvUtils<ManagementUserEntity, ManagementUserRepository, ManagementUserEntityAudit,  ManagementUserService> csvUtils;
	...
	public ManagementUserController(
		...
		this.csvUtils = new CsvUtils<>(managementUserService, ManagementUserEntity.class);
	...
	public ResponseEntity<List<ManagementUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ManagementUserEntity> managementUsers = managementUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(managementUsers, HttpStatus.OK);
	}

	/**
	 * Return the Management User that has the same id as the given id.
	 *
	 * @param id      The id of the ManagementUserEntity to be returned
	 * @return the Management User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ManagementUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ManagementUser entity not found"),
	})
	@PreAuthorize("hasPermission('ManagementUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ManagementUserEntity> getById(
			@Parameter(description = "The UUID of the ManagementUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ManagementUserEntity> managementUserEntity = managementUserService.findById(id);
	...
	/**
	 * Create/Update the given Management User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param managementUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveManagementUser' endpoint here] off begin
	@Operation(
			description = "Create a ManagementUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ManagementUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ManagementUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('ManagementUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveManagementUser' endpoint here] end
	public ResponseEntity<?> saveManagementUser(
			@Parameter(description = "The ManagementUserEntity to create or update.")
			@RequestBody ManagementUserEntityDto managementUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ManagementUserEntity managementUserEntity = managementUserService.create(new ManagementUserEntity(managementUserEntityDto));
			return new ResponseEntity<>(managementUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Management User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param managementUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateManagementUser' endpoint here] off begin
	@Operation(
			description = "Update a ManagementUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ManagementUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ManagementUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ManagementUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateManagementUser' endpoint here] end
	public ResponseEntity<?> updateManagementUser(
			@Parameter(description = "The ManagementUserEntity to create or update.")
			@RequestBody ManagementUserEntityDto managementUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ManagementUserEntity> checkManagementUserExists = managementUserService.findById(managementUserEntityDto.getId());
		ManagementUserEntity managementUserEntity = new ManagementUserEntity(managementUserEntityDto);

		if (checkManagementUserExists.isPresent()) {
			try {
				managementUserEntity = managementUserService.update(managementUserEntity);
				return new ResponseEntity<>(managementUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(managementUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Management User that has the same id as the given id.
	 *
	 * @param id The id of the ManagementUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single ManagementUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('ManagementUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ManagementUser to delete.")
		...
		managementUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportManagementUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Management User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportManagementUser' endpoint here] end
	public void exportManagementUser(
			@Parameter(description = "The ids of the ManagementUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ManagementUserEntity> entitiesToExport = managementUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ManagementUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ManagementUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportManagementUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportManagementUserExcludingIds' endpoint here] end
	public void exportManagementUserExcludingIds(
			@Parameter(description = "The ids of the ManagementUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ManagementUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = managementUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = managementUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ManagementUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ManagementUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ManagementUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Management User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ManagementUserEntity', 'create')  || hasPermission('ManagementUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ManagementUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ManagementUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ManagementUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ManagementUserEntity> csvToBean = new CsvToBeanBuilder<ManagementUserEntity>(reader)
				.withType(ManagementUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ManagementUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ManagementUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Management User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Management User entity and you do not have permission");
					}
