...
import kmsweb.entities.InventoryAdjusmentBatchEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Inventory Adjusment Batch
 */
// % protected region % [Update the API route or description for the Inventory Adjusment BatchController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InventoryAdjusmentBatchController"))
@RestController
@Slf4j
@RequestMapping("/api/inventoryadjusmentbatch")
// % protected region % [Update the API route or description for the Inventory Adjusment BatchController here] end
public class InventoryAdjusmentBatchController {

	private final InventoryAdjusmentBatchService inventoryAdjusmentBatchService;
	private final CsvUtils<InventoryAdjusmentBatchEntity, InventoryAdjusmentBatchRepository, InventoryAdjusmentBatchEntityAudit,  InventoryAdjusmentBatchService> csvUtils;
	...
	@Autowired
	public InventoryAdjusmentBatchController(
		...
		this.csvUtils = new CsvUtils<>(inventoryAdjusmentBatchService, InventoryAdjusmentBatchEntity.class);
	...
	public ResponseEntity<List<InventoryAdjusmentBatchEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InventoryAdjusmentBatchEntity> inventoryAdjusmentBatchs = inventoryAdjusmentBatchService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(inventoryAdjusmentBatchs, HttpStatus.OK);
	}

	/**
	 * Return the Inventory Adjusment Batch that has the same id as the given id.
	 *
	 * @param id      The id of the InventoryAdjusmentBatchEntity to be returned
	 * @return the Inventory Adjusment Batch that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InventoryAdjusmentBatch as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InventoryAdjusmentBatch entity not found"),
	})
	@PreAuthorize("hasPermission('InventoryAdjusmentBatchEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InventoryAdjusmentBatchEntity> getById(
			@Parameter(description = "The UUID of the InventoryAdjusmentBatch to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InventoryAdjusmentBatchEntity> inventoryAdjusmentBatchEntity = inventoryAdjusmentBatchService.findById(id);
	...
	/**
	 * Create/Update the given Inventory Adjusment Batch. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryAdjusmentBatchEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInventoryAdjusmentBatch' endpoint here] off begin
	@Operation(
			description = "Create a InventoryAdjusmentBatch if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InventoryAdjusmentBatch."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InventoryAdjusmentBatch, duplicate record found")
	})
	@PreAuthorize("hasPermission('InventoryAdjusmentBatchEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInventoryAdjusmentBatch' endpoint here] end
	public ResponseEntity<?> saveInventoryAdjusmentBatch(
			@Parameter(description = "The InventoryAdjusmentBatchEntity to create or update.")
			@RequestBody InventoryAdjusmentBatchEntityDto inventoryAdjusmentBatchEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InventoryAdjusmentBatchEntity inventoryAdjusmentBatchEntity = inventoryAdjusmentBatchService.create(new InventoryAdjusmentBatchEntity(inventoryAdjusmentBatchEntityDto));
			return new ResponseEntity<>(inventoryAdjusmentBatchEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Inventory Adjusment Batch. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryAdjusmentBatchEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInventoryAdjusmentBatch' endpoint here] off begin
	@Operation(
			description = "Update a InventoryAdjusmentBatch entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InventoryAdjusmentBatch."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InventoryAdjusmentBatch, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InventoryAdjusmentBatchEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInventoryAdjusmentBatch' endpoint here] end
	public ResponseEntity<?> updateInventoryAdjusmentBatch(
			@Parameter(description = "The InventoryAdjusmentBatchEntity to create or update.")
			@RequestBody InventoryAdjusmentBatchEntityDto inventoryAdjusmentBatchEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InventoryAdjusmentBatchEntity> checkInventoryAdjusmentBatchExists = inventoryAdjusmentBatchService.findById(inventoryAdjusmentBatchEntityDto.getId());
		InventoryAdjusmentBatchEntity inventoryAdjusmentBatchEntity = new InventoryAdjusmentBatchEntity(inventoryAdjusmentBatchEntityDto);

		if (checkInventoryAdjusmentBatchExists.isPresent()) {
			try {
				inventoryAdjusmentBatchEntity = inventoryAdjusmentBatchService.update(inventoryAdjusmentBatchEntity);
				return new ResponseEntity<>(inventoryAdjusmentBatchEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(inventoryAdjusmentBatchEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Inventory Adjusment Batch that has the same id as the given id.
	 *
	 * @param id The id of the InventoryAdjusmentBatchEntity to be deleted
	...
	@Operation(
			description = "Delete a single InventoryAdjusmentBatch as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InventoryAdjusmentBatch to delete.")
		...
		inventoryAdjusmentBatchService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjusmentBatch' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Inventory Adjusment Batch entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjusmentBatch' endpoint here] end
	public void exportInventoryAdjusmentBatch(
			@Parameter(description = "The ids of the InventoryAdjusmentBatchEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InventoryAdjusmentBatchEntity> entitiesToExport = inventoryAdjusmentBatchService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InventoryAdjusmentBatch_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InventoryAdjusmentBatchEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjusmentBatchExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjusmentBatchExcludingIds' endpoint here] end
	public void exportInventoryAdjusmentBatchExcludingIds(
			@Parameter(description = "The ids of the InventoryAdjusmentBatchEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InventoryAdjusmentBatchEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = inventoryAdjusmentBatchService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = inventoryAdjusmentBatchService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InventoryAdjusmentBatch entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InventoryAdjusmentBatch_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InventoryAdjusmentBatchEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Inventory Adjusment Batch entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InventoryAdjusmentBatchEntity', 'create')  || hasPermission('InventoryAdjusmentBatchEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InventoryAdjusmentBatchEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InventoryAdjusmentBatchEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InventoryAdjusmentBatchEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InventoryAdjusmentBatchEntity> csvToBean = new CsvToBeanBuilder<InventoryAdjusmentBatchEntity>(reader)
				.withType(InventoryAdjusmentBatchEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InventoryAdjusmentBatchEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InventoryAdjusmentBatchEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Inventory Adjusment Batch entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Inventory Adjusment Batch entity and you do not have permission");
					}
