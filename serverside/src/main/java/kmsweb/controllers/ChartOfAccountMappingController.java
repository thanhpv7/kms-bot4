...
/**
 * Controller used to handle all REST operations regarding Chart of Account Mapping
 */
// % protected region % [Update the API route or description for the Chart of Account MappingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ChartOfAccountMappingController"))
@RestController
@Slf4j
@RequestMapping("/api/chartofaccountmapping")
// % protected region % [Update the API route or description for the Chart of Account MappingController here] end
public class ChartOfAccountMappingController {

	private final ChartOfAccountMappingService chartOfAccountMappingService;
	private final CsvUtils<ChartOfAccountMappingEntity, ChartOfAccountMappingRepository, ChartOfAccountMappingEntityAudit,  ChartOfAccountMappingService> csvUtils;
	...
	@Autowired
	public ChartOfAccountMappingController(
		...
		this.csvUtils = new CsvUtils<>(chartOfAccountMappingService, ChartOfAccountMappingEntity.class);
	...
	public ResponseEntity<List<ChartOfAccountMappingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ChartOfAccountMappingEntity> chartOfAccountMappings = chartOfAccountMappingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(chartOfAccountMappings, HttpStatus.OK);
	}

	/**
	 * Return the Chart of Account Mapping that has the same id as the given id.
	 *
	 * @param id      The id of the ChartOfAccountMappingEntity to be returned
	 * @return the Chart of Account Mapping that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ChartOfAccountMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ChartOfAccountMapping entity not found"),
	})
	@PreAuthorize("hasPermission('ChartOfAccountMappingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ChartOfAccountMappingEntity> getById(
			@Parameter(description = "The UUID of the ChartOfAccountMapping to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ChartOfAccountMappingEntity> chartOfAccountMappingEntity = chartOfAccountMappingService.findById(id);
	...
	/**
	 * Create/Update the given Chart of Account Mapping. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param chartOfAccountMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveChartOfAccountMapping' endpoint here] off begin
	@Operation(
			description = "Create a ChartOfAccountMapping if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ChartOfAccountMapping."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ChartOfAccountMapping, duplicate record found")
	})
	@PreAuthorize("hasPermission('ChartOfAccountMappingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveChartOfAccountMapping' endpoint here] end
	public ResponseEntity<?> saveChartOfAccountMapping(
			@Parameter(description = "The ChartOfAccountMappingEntity to create or update.")
			@RequestBody ChartOfAccountMappingEntityDto chartOfAccountMappingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ChartOfAccountMappingEntity chartOfAccountMappingEntity = chartOfAccountMappingService.create(new ChartOfAccountMappingEntity(chartOfAccountMappingEntityDto));
			return new ResponseEntity<>(chartOfAccountMappingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Chart of Account Mapping. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param chartOfAccountMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateChartOfAccountMapping' endpoint here] off begin
	@Operation(
			description = "Update a ChartOfAccountMapping entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ChartOfAccountMapping."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ChartOfAccountMapping, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ChartOfAccountMappingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateChartOfAccountMapping' endpoint here] end
	public ResponseEntity<?> updateChartOfAccountMapping(
			@Parameter(description = "The ChartOfAccountMappingEntity to create or update.")
			@RequestBody ChartOfAccountMappingEntityDto chartOfAccountMappingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ChartOfAccountMappingEntity> checkChartOfAccountMappingExists = chartOfAccountMappingService.findById(chartOfAccountMappingEntityDto.getId());
		ChartOfAccountMappingEntity chartOfAccountMappingEntity = new ChartOfAccountMappingEntity(chartOfAccountMappingEntityDto);

		if (checkChartOfAccountMappingExists.isPresent()) {
			try {
				chartOfAccountMappingEntity = chartOfAccountMappingService.update(chartOfAccountMappingEntity);
				return new ResponseEntity<>(chartOfAccountMappingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(chartOfAccountMappingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Chart of Account Mapping that has the same id as the given id.
	 *
	 * @param id The id of the ChartOfAccountMappingEntity to be deleted
	...
	@Operation(
			description = "Delete a single ChartOfAccountMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('ChartOfAccountMappingEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ChartOfAccountMapping to delete.")
		...
		chartOfAccountMappingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccountMapping' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Chart of Account Mapping entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccountMapping' endpoint here] end
	public void exportChartOfAccountMapping(
			@Parameter(description = "The ids of the ChartOfAccountMappingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ChartOfAccountMappingEntity> entitiesToExport = chartOfAccountMappingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ChartOfAccountMapping_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ChartOfAccountMappingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccountMappingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportChartOfAccountMappingExcludingIds' endpoint here] end
	public void exportChartOfAccountMappingExcludingIds(
			@Parameter(description = "The ids of the ChartOfAccountMappingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ChartOfAccountMappingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = chartOfAccountMappingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = chartOfAccountMappingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ChartOfAccountMapping entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ChartOfAccountMapping_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ChartOfAccountMappingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Chart of Account Mapping entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ChartOfAccountMappingEntity', 'create')  || hasPermission('ChartOfAccountMappingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ChartOfAccountMappingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ChartOfAccountMappingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ChartOfAccountMappingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ChartOfAccountMappingEntity> csvToBean = new CsvToBeanBuilder<ChartOfAccountMappingEntity>(reader)
				.withType(ChartOfAccountMappingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ChartOfAccountMappingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ChartOfAccountMappingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Chart of Account Mapping entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Chart of Account Mapping entity and you do not have permission");
					}
