...
/**
 * Controller used to handle all REST operations regarding BPJS List Finger Print
 */
// % protected region % [Update the API route or description for the BPJS List Finger PrintController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsListFingerPrintController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjslistfingerprint")
// % protected region % [Update the API route or description for the BPJS List Finger PrintController here] end
public class BpjsListFingerPrintController {

	private final BpjsListFingerPrintService bpjsListFingerPrintService;
	private final CsvUtils<BpjsListFingerPrintEntity, BpjsListFingerPrintRepository, BpjsListFingerPrintEntityAudit,  BpjsListFingerPrintService> csvUtils;
	...
	@Autowired
	public BpjsListFingerPrintController(
		...
		this.csvUtils = new CsvUtils<>(bpjsListFingerPrintService, BpjsListFingerPrintEntity.class);
	...
	public ResponseEntity<List<BpjsListFingerPrintEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsListFingerPrintEntity> bpjsListFingerPrints = bpjsListFingerPrintService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsListFingerPrints, HttpStatus.OK);
	}

	/**
	 * Return the BPJS List Finger Print that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsListFingerPrintEntity to be returned
	 * @return the BPJS List Finger Print that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsListFingerPrint as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsListFingerPrint entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsListFingerPrintEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsListFingerPrintEntity> getById(
			@Parameter(description = "The UUID of the BpjsListFingerPrint to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsListFingerPrintEntity> bpjsListFingerPrintEntity = bpjsListFingerPrintService.findById(id);
	...
	/**
	 * Create/Update the given BPJS List Finger Print. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsListFingerPrintEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsListFingerPrint' endpoint here] off begin
	@Operation(
			description = "Create a BpjsListFingerPrint if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsListFingerPrint."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsListFingerPrint, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsListFingerPrintEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsListFingerPrint' endpoint here] end
	public ResponseEntity<?> saveBpjsListFingerPrint(
			@Parameter(description = "The BpjsListFingerPrintEntity to create or update.")
			@RequestBody BpjsListFingerPrintEntityDto bpjsListFingerPrintEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsListFingerPrintEntity bpjsListFingerPrintEntity = bpjsListFingerPrintService.create(new BpjsListFingerPrintEntity(bpjsListFingerPrintEntityDto));
			return new ResponseEntity<>(bpjsListFingerPrintEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS List Finger Print. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsListFingerPrintEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsListFingerPrint' endpoint here] off begin
	@Operation(
			description = "Update a BpjsListFingerPrint entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsListFingerPrint."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsListFingerPrint, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsListFingerPrintEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsListFingerPrint' endpoint here] end
	public ResponseEntity<?> updateBpjsListFingerPrint(
			@Parameter(description = "The BpjsListFingerPrintEntity to create or update.")
			@RequestBody BpjsListFingerPrintEntityDto bpjsListFingerPrintEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsListFingerPrintEntity> checkBpjsListFingerPrintExists = bpjsListFingerPrintService.findById(bpjsListFingerPrintEntityDto.getId());
		BpjsListFingerPrintEntity bpjsListFingerPrintEntity = new BpjsListFingerPrintEntity(bpjsListFingerPrintEntityDto);

		if (checkBpjsListFingerPrintExists.isPresent()) {
			try {
				bpjsListFingerPrintEntity = bpjsListFingerPrintService.update(bpjsListFingerPrintEntity);
				return new ResponseEntity<>(bpjsListFingerPrintEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsListFingerPrintEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS List Finger Print that has the same id as the given id.
	 *
	 * @param id The id of the BpjsListFingerPrintEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsListFingerPrint as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('BpjsListFingerPrintEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsListFingerPrint to delete.")
		...
		bpjsListFingerPrintService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsListFingerPrint' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS List Finger Print entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsListFingerPrint' endpoint here] end
	public void exportBpjsListFingerPrint(
			@Parameter(description = "The ids of the BpjsListFingerPrintEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsListFingerPrintEntity> entitiesToExport = bpjsListFingerPrintService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsListFingerPrint_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsListFingerPrintEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsListFingerPrintExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsListFingerPrintExcludingIds' endpoint here] end
	public void exportBpjsListFingerPrintExcludingIds(
			@Parameter(description = "The ids of the BpjsListFingerPrintEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsListFingerPrintEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsListFingerPrintService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsListFingerPrintService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsListFingerPrint entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsListFingerPrint_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsListFingerPrintEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS List Finger Print entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsListFingerPrintEntity', 'create')  || hasPermission('BpjsListFingerPrintEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsListFingerPrintEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsListFingerPrintEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsListFingerPrintEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsListFingerPrintEntity> csvToBean = new CsvToBeanBuilder<BpjsListFingerPrintEntity>(reader)
				.withType(BpjsListFingerPrintEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsListFingerPrintEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsListFingerPrintEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS List Finger Print entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS List Finger Print entity and you do not have permission");
					}
