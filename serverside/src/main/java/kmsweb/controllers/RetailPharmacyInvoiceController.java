...
import kmsweb.entities.RetailPharmacyInvoiceEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Retail Pharmacy Invoice
 */
// % protected region % [Update the API route or description for the Retail Pharmacy InvoiceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RetailPharmacyInvoiceController"))
@RestController
@Slf4j
@RequestMapping("/api/retailpharmacyinvoice")
// % protected region % [Update the API route or description for the Retail Pharmacy InvoiceController here] end
public class RetailPharmacyInvoiceController {

	private final RetailPharmacyInvoiceService retailPharmacyInvoiceService;
	private final CsvUtils<RetailPharmacyInvoiceEntity, RetailPharmacyInvoiceRepository, RetailPharmacyInvoiceEntityAudit,  RetailPharmacyInvoiceService> csvUtils;
	...
	@Autowired
	public RetailPharmacyInvoiceController(
		...
		this.csvUtils = new CsvUtils<>(retailPharmacyInvoiceService, RetailPharmacyInvoiceEntity.class);
	...
	public ResponseEntity<List<RetailPharmacyInvoiceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<RetailPharmacyInvoiceEntity> retailPharmacyInvoices = retailPharmacyInvoiceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(retailPharmacyInvoices, HttpStatus.OK);
	}

	/**
	 * Return the Retail Pharmacy Invoice that has the same id as the given id.
	 *
	 * @param id      The id of the RetailPharmacyInvoiceEntity to be returned
	 * @return the Retail Pharmacy Invoice that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single RetailPharmacyInvoice as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "RetailPharmacyInvoice entity not found"),
	})
	@PreAuthorize("hasPermission('RetailPharmacyInvoiceEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<RetailPharmacyInvoiceEntity> getById(
			@Parameter(description = "The UUID of the RetailPharmacyInvoice to return.")
			@PathVariable("id") UUID id
	) {
		Optional<RetailPharmacyInvoiceEntity> retailPharmacyInvoiceEntity = retailPharmacyInvoiceService.findById(id);
	...
	/**
	 * Create/Update the given Retail Pharmacy Invoice. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param retailPharmacyInvoiceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRetailPharmacyInvoice' endpoint here] off begin
	@Operation(
			description = "Create a RetailPharmacyInvoice if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new RetailPharmacyInvoice."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new RetailPharmacyInvoice, duplicate record found")
	})
	@PreAuthorize("hasPermission('RetailPharmacyInvoiceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRetailPharmacyInvoice' endpoint here] end
	public ResponseEntity<?> saveRetailPharmacyInvoice(
			@Parameter(description = "The RetailPharmacyInvoiceEntity to create or update.")
			@RequestBody RetailPharmacyInvoiceEntityDto retailPharmacyInvoiceEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RetailPharmacyInvoiceEntity retailPharmacyInvoiceEntity = retailPharmacyInvoiceService.create(new RetailPharmacyInvoiceEntity(retailPharmacyInvoiceEntityDto));
			return new ResponseEntity<>(retailPharmacyInvoiceEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Retail Pharmacy Invoice. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param retailPharmacyInvoiceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRetailPharmacyInvoice' endpoint here] off begin
	@Operation(
			description = "Update a RetailPharmacyInvoice entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the RetailPharmacyInvoice."),
		@ApiResponse(responseCode = "400", description = "Failed to update the RetailPharmacyInvoice, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RetailPharmacyInvoiceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRetailPharmacyInvoice' endpoint here] end
	public ResponseEntity<?> updateRetailPharmacyInvoice(
			@Parameter(description = "The RetailPharmacyInvoiceEntity to create or update.")
			@RequestBody RetailPharmacyInvoiceEntityDto retailPharmacyInvoiceEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<RetailPharmacyInvoiceEntity> checkRetailPharmacyInvoiceExists = retailPharmacyInvoiceService.findById(retailPharmacyInvoiceEntityDto.getId());
		RetailPharmacyInvoiceEntity retailPharmacyInvoiceEntity = new RetailPharmacyInvoiceEntity(retailPharmacyInvoiceEntityDto);

		if (checkRetailPharmacyInvoiceExists.isPresent()) {
			try {
				retailPharmacyInvoiceEntity = retailPharmacyInvoiceService.update(retailPharmacyInvoiceEntity);
				return new ResponseEntity<>(retailPharmacyInvoiceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(retailPharmacyInvoiceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Retail Pharmacy Invoice that has the same id as the given id.
	 *
	 * @param id The id of the RetailPharmacyInvoiceEntity to be deleted
	...
	@Operation(
			description = "Delete a single RetailPharmacyInvoice as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the RetailPharmacyInvoice to delete.")
		...
		retailPharmacyInvoiceService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyInvoice' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Retail Pharmacy Invoice entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyInvoice' endpoint here] end
	public void exportRetailPharmacyInvoice(
			@Parameter(description = "The ids of the RetailPharmacyInvoiceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RetailPharmacyInvoiceEntity> entitiesToExport = retailPharmacyInvoiceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "RetailPharmacyInvoice_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RetailPharmacyInvoiceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyInvoiceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRetailPharmacyInvoiceExcludingIds' endpoint here] end
	public void exportRetailPharmacyInvoiceExcludingIds(
			@Parameter(description = "The ids of the RetailPharmacyInvoiceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RetailPharmacyInvoiceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = retailPharmacyInvoiceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = retailPharmacyInvoiceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the RetailPharmacyInvoice entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "RetailPharmacyInvoice_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RetailPharmacyInvoiceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Retail Pharmacy Invoice entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RetailPharmacyInvoiceEntity', 'create')  || hasPermission('RetailPharmacyInvoiceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RetailPharmacyInvoiceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RetailPharmacyInvoiceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RetailPharmacyInvoiceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RetailPharmacyInvoiceEntity> csvToBean = new CsvToBeanBuilder<RetailPharmacyInvoiceEntity>(reader)
				.withType(RetailPharmacyInvoiceEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<RetailPharmacyInvoiceEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<RetailPharmacyInvoiceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Retail Pharmacy Invoice entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Retail Pharmacy Invoice entity and you do not have permission");
					}
