...
import kmsweb.entities.ReferenceHeaderEntity;
import kmsweb.dtos.ReferenceHeaderEntityDto;
import kmsweb.entities.ReferenceHeaderEntityAudit;
import kmsweb.repositories.ReferenceHeaderRepository;
import kmsweb.services.ReferenceHeaderService;
...
/**
 * Controller used to handle all REST operations regarding Reference Header
 */
// % protected region % [Update the API route or description for the Reference HeaderController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ReferenceHeaderController"))
@RestController
@Slf4j
@RequestMapping("/api/referenceheader")
// % protected region % [Update the API route or description for the Reference HeaderController here] end
public class ReferenceHeaderController {

	private final ReferenceHeaderService referenceHeaderService;
	private final CsvUtils<ReferenceHeaderEntity, ReferenceHeaderRepository, ReferenceHeaderEntityAudit,  ReferenceHeaderService> csvUtils;
	...
	@Autowired
	public ReferenceHeaderController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			ReferenceHeaderService referenceHeaderService
	) {
		this.authService = authService;
		this.referenceHeaderService = referenceHeaderService;
		this.csvUtils = new CsvUtils<>(referenceHeaderService, ReferenceHeaderEntity.class);
	...
	public ResponseEntity<List<ReferenceHeaderEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ReferenceHeaderEntity> referenceHeaders = referenceHeaderService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(referenceHeaders, HttpStatus.OK);
	}

	/**
	 * Return the Reference Header that has the same id as the given id.
	 *
	 * @param id      The id of the ReferenceHeaderEntity to be returned
	 * @return the Reference Header that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ReferenceHeader as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ReferenceHeader entity not found"),
	})
	@PreAuthorize("hasPermission('ReferenceHeaderEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ReferenceHeaderEntity> getById(
			@Parameter(description = "The UUID of the ReferenceHeader to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ReferenceHeaderEntity> referenceHeaderEntity = referenceHeaderService.findById(id);
	...
	/**
	 * Create/Update the given Reference Header. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param referenceHeaderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveReferenceHeader' endpoint here] off begin
	@Operation(
			description = "Create a ReferenceHeader if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ReferenceHeader."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ReferenceHeader, duplicate record found")
	})
	@PreAuthorize("hasPermission('ReferenceHeaderEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveReferenceHeader' endpoint here] end
	public ResponseEntity<?> saveReferenceHeader(
			@Parameter(description = "The ReferenceHeaderEntity to create or update.")
			@RequestBody ReferenceHeaderEntityDto referenceHeaderEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ReferenceHeaderEntity referenceHeaderEntity = referenceHeaderService.create(new ReferenceHeaderEntity(referenceHeaderEntityDto));
			return new ResponseEntity<>(referenceHeaderEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Reference Header. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param referenceHeaderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateReferenceHeader' endpoint here] off begin
	@Operation(
			description = "Update a ReferenceHeader entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ReferenceHeader."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ReferenceHeader, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ReferenceHeaderEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateReferenceHeader' endpoint here] end
	public ResponseEntity<?> updateReferenceHeader(
			@Parameter(description = "The ReferenceHeaderEntity to create or update.")
			@RequestBody ReferenceHeaderEntityDto referenceHeaderEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ReferenceHeaderEntity> checkReferenceHeaderExists = referenceHeaderService.findById(referenceHeaderEntityDto.getId());
		ReferenceHeaderEntity referenceHeaderEntity = new ReferenceHeaderEntity(referenceHeaderEntityDto);

		if (checkReferenceHeaderExists.isPresent()) {
			try {
				referenceHeaderEntity = referenceHeaderService.update(referenceHeaderEntity);
				return new ResponseEntity<>(referenceHeaderEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(referenceHeaderEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Reference Header that has the same id as the given id.
	 *
	 * @param id The id of the ReferenceHeaderEntity to be deleted
	...
	@Operation(
			description = "Delete a single ReferenceHeader as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ReferenceHeader to delete.")
		...
		referenceHeaderService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportReferenceHeader' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Reference Header entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportReferenceHeader' endpoint here] end
	public void exportReferenceHeader(
			@Parameter(description = "The ids of the ReferenceHeaderEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ReferenceHeaderEntity> entitiesToExport = referenceHeaderService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ReferenceHeader_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ReferenceHeaderEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportReferenceHeaderExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportReferenceHeaderExcludingIds' endpoint here] end
	public void exportReferenceHeaderExcludingIds(
			@Parameter(description = "The ids of the ReferenceHeaderEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ReferenceHeaderEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = referenceHeaderService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = referenceHeaderService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ReferenceHeader entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ReferenceHeader_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ReferenceHeaderEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Reference Header entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ReferenceHeaderEntity', 'create')  || hasPermission('ReferenceHeaderEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ReferenceHeaderEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ReferenceHeaderEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ReferenceHeaderEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ReferenceHeaderEntity> csvToBean = new CsvToBeanBuilder<ReferenceHeaderEntity>(reader)
				.withType(ReferenceHeaderEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ReferenceHeaderEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ReferenceHeaderEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Reference Header entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Reference Header entity and you do not have permission");
					}
