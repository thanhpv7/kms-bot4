...
import kmsweb.entities.CompoundPrescriptionDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Compound Prescription Detail
 */
// % protected region % [Update the API route or description for the Compound Prescription DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CompoundPrescriptionDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/compoundprescriptiondetail")
// % protected region % [Update the API route or description for the Compound Prescription DetailController here] end
public class CompoundPrescriptionDetailController {

	private final CompoundPrescriptionDetailService compoundPrescriptionDetailService;
	private final CsvUtils<CompoundPrescriptionDetailEntity, CompoundPrescriptionDetailRepository, CompoundPrescriptionDetailEntityAudit,  CompoundPrescriptionDetailService> csvUtils;
	...
	@Autowired
	public CompoundPrescriptionDetailController(
		...
		this.csvUtils = new CsvUtils<>(compoundPrescriptionDetailService, CompoundPrescriptionDetailEntity.class);
	...
	/**
	 * Return all the Compound Prescription Details.
	 *
	 * @return all the Compound Prescription Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of CompoundPrescriptionDetails",
		security = {
	...
	public ResponseEntity<List<CompoundPrescriptionDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CompoundPrescriptionDetailEntity> compoundPrescriptionDetails = compoundPrescriptionDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(compoundPrescriptionDetails, HttpStatus.OK);
	}

	/**
	 * Return the Compound Prescription Detail that has the same id as the given id.
	 *
	 * @param id      The id of the CompoundPrescriptionDetailEntity to be returned
	 * @return the Compound Prescription Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CompoundPrescriptionDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CompoundPrescriptionDetail entity not found"),
	})
	@PreAuthorize("hasPermission('CompoundPrescriptionDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CompoundPrescriptionDetailEntity> getById(
			@Parameter(description = "The UUID of the CompoundPrescriptionDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CompoundPrescriptionDetailEntity> compoundPrescriptionDetailEntity = compoundPrescriptionDetailService.findById(id);
	...
	/**
	 * Create/Update the given Compound Prescription Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param compoundPrescriptionDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCompoundPrescriptionDetail' endpoint here] off begin
	@Operation(
			description = "Create a CompoundPrescriptionDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CompoundPrescriptionDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CompoundPrescriptionDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('CompoundPrescriptionDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCompoundPrescriptionDetail' endpoint here] end
	public ResponseEntity<?> saveCompoundPrescriptionDetail(
			@Parameter(description = "The CompoundPrescriptionDetailEntity to create or update.")
			@RequestBody CompoundPrescriptionDetailEntityDto compoundPrescriptionDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CompoundPrescriptionDetailEntity compoundPrescriptionDetailEntity = compoundPrescriptionDetailService.create(new CompoundPrescriptionDetailEntity(compoundPrescriptionDetailEntityDto));
			return new ResponseEntity<>(compoundPrescriptionDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Compound Prescription Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param compoundPrescriptionDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCompoundPrescriptionDetail' endpoint here] off begin
	@Operation(
			description = "Update a CompoundPrescriptionDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CompoundPrescriptionDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CompoundPrescriptionDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CompoundPrescriptionDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCompoundPrescriptionDetail' endpoint here] end
	public ResponseEntity<?> updateCompoundPrescriptionDetail(
			@Parameter(description = "The CompoundPrescriptionDetailEntity to create or update.")
			@RequestBody CompoundPrescriptionDetailEntityDto compoundPrescriptionDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CompoundPrescriptionDetailEntity> checkCompoundPrescriptionDetailExists = compoundPrescriptionDetailService.findById(compoundPrescriptionDetailEntityDto.getId());
		CompoundPrescriptionDetailEntity compoundPrescriptionDetailEntity = new CompoundPrescriptionDetailEntity(compoundPrescriptionDetailEntityDto);

		if (checkCompoundPrescriptionDetailExists.isPresent()) {
			try {
				compoundPrescriptionDetailEntity = compoundPrescriptionDetailService.update(compoundPrescriptionDetailEntity);
				return new ResponseEntity<>(compoundPrescriptionDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(compoundPrescriptionDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Compound Prescription Detail that has the same id as the given id.
	 *
	 * @param id The id of the CompoundPrescriptionDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single CompoundPrescriptionDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CompoundPrescriptionDetail to delete.")
		...
		compoundPrescriptionDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Compound Prescription Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionDetail' endpoint here] end
	public void exportCompoundPrescriptionDetail(
			@Parameter(description = "The ids of the CompoundPrescriptionDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CompoundPrescriptionDetailEntity> entitiesToExport = compoundPrescriptionDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CompoundPrescriptionDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CompoundPrescriptionDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionDetailExcludingIds' endpoint here] end
	public void exportCompoundPrescriptionDetailExcludingIds(
			@Parameter(description = "The ids of the CompoundPrescriptionDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CompoundPrescriptionDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = compoundPrescriptionDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = compoundPrescriptionDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CompoundPrescriptionDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CompoundPrescriptionDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CompoundPrescriptionDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Compound Prescription Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CompoundPrescriptionDetailEntity', 'create')  || hasPermission('CompoundPrescriptionDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CompoundPrescriptionDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CompoundPrescriptionDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CompoundPrescriptionDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CompoundPrescriptionDetailEntity> csvToBean = new CsvToBeanBuilder<CompoundPrescriptionDetailEntity>(reader)
				.withType(CompoundPrescriptionDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CompoundPrescriptionDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CompoundPrescriptionDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Compound Prescription Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Compound Prescription Detail entity and you do not have permission");
					}
