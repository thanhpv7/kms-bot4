...
import kmsweb.entities.JournalEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Journal
 */
// % protected region % [Update the API route or description for the JournalController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "JournalController"))
@RestController
@Slf4j
@RequestMapping("/api/journal")
// % protected region % [Update the API route or description for the JournalController here] end
public class JournalController {

	private final JournalService journalService;
	private final CsvUtils<JournalEntity, JournalRepository, JournalEntityAudit,  JournalService> csvUtils;
	...
	public JournalController(
		...
		this.csvUtils = new CsvUtils<>(journalService, JournalEntity.class);
	...
	@Operation(
			description = "Returns a single page of Journals",
		security = {
	...
	public ResponseEntity<List<JournalEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<JournalEntity> journals = journalService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Journal that has the same id as the given id.
	 *
	 * @param id      The id of the JournalEntity to be returned
	 * @return the Journal that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Journal as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Journal entity not found"),
	})
	@PreAuthorize("hasPermission('JournalEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<JournalEntity> getById(
			@Parameter(description = "The UUID of the Journal to return.")
			@PathVariable("id") UUID id
	) {
		Optional<JournalEntity> journalEntity = journalService.findById(id);
	...
	/**
	 * Create/Update the given Journal. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param journalEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveJournal' endpoint here] off begin
	@Operation(
			description = "Create a Journal if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Journal."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Journal, duplicate record found")
	})
	@PreAuthorize("hasPermission('JournalEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveJournal' endpoint here] end
	public ResponseEntity<?> saveJournal(
			@Parameter(description = "The JournalEntity to create or update.")
			@RequestBody JournalEntityDto journalEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			JournalEntity journalEntity = journalService.create(new JournalEntity(journalEntityDto));
	...
	/**
	 * Update the given Journal. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param journalEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateJournal' endpoint here] off begin
	@Operation(
			description = "Update a Journal entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Journal."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Journal, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('JournalEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateJournal' endpoint here] end
	public ResponseEntity<?> updateJournal(
		...
		Optional<JournalEntity> checkJournalExists = journalService.findById(journalEntityDto.getId());
		JournalEntity journalEntity = new JournalEntity(journalEntityDto);

		if (checkJournalExists.isPresent()) {
			try {
	...
	/**
	 * Delete the Journal that has the same id as the given id.
	 *
	 * @param id The id of the JournalEntity to be deleted
	...
	@Operation(
			description = "Delete a single Journal as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Journal to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportJournal' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Journal entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportJournal' endpoint here] end
	public void exportJournal(
			@Parameter(description = "The ids of the JournalEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<JournalEntity> entitiesToExport = journalService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Journal_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (JournalEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportJournalExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportJournalExcludingIds' endpoint here] end
	public void exportJournalExcludingIds(
			@Parameter(description = "The ids of the JournalEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<JournalEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = journalService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = journalService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Journal entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Journal_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(JournalEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Journal entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('JournalEntity', 'create')  || hasPermission('JournalEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with JournalEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("JournalEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(JournalEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<JournalEntity> csvToBean = new CsvToBeanBuilder<JournalEntity>(reader)
				.withType(JournalEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<JournalEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<JournalEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Journal entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Journal entity and you do not have permission");
					}
