...
/**
 * Controller used to handle all REST operations regarding Service
 */
// % protected region % [Update the API route or description for the ServiceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ServiceController"))
@RestController
@Slf4j
@RequestMapping("/api/service")
// % protected region % [Update the API route or description for the ServiceController here] end
public class ServiceController {

	private final ServiceService serviceService;
	private final CsvUtils<ServiceEntity, ServiceRepository, ServiceEntityAudit,  ServiceService> csvUtils;
	...
	public ServiceController(
		...
		this.csvUtils = new CsvUtils<>(serviceService, ServiceEntity.class);
	...
	@Operation(
			description = "Returns a single page of Services",
		security = {
	...
	public ResponseEntity<List<ServiceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ServiceEntity> services = serviceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Service that has the same id as the given id.
	 *
	 * @param id      The id of the ServiceEntity to be returned
	 * @return the Service that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Service as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Service entity not found"),
	})
	@PreAuthorize("hasPermission('ServiceEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ServiceEntity> getById(
			@Parameter(description = "The UUID of the Service to return.")
	...
	/**
	 * Create/Update the given Service. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param serviceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveService' endpoint here] off begin
	@Operation(
			description = "Create a Service if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Service."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Service, duplicate record found")
	})
	@PreAuthorize("hasPermission('ServiceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveService' endpoint here] end
	public ResponseEntity<?> saveService(
			@Parameter(description = "The ServiceEntity to create or update.")
		...
		try {
			ServiceEntity serviceEntity = serviceService.create(new ServiceEntity(serviceEntityDto));
	...
	/**
	 * Update the given Service. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param serviceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateService' endpoint here] off begin
	@Operation(
			description = "Update a Service entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		...
		@ApiResponse(responseCode = "400", description = "Failed to update the Service, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ServiceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateService' endpoint here] end
	public ResponseEntity<?> updateService(
		...
		Optional<ServiceEntity> checkServiceExists = serviceService.findById(serviceEntityDto.getId());
		ServiceEntity serviceEntity = new ServiceEntity(serviceEntityDto);

		if (checkServiceExists.isPresent()) {
			try {
				serviceEntity = serviceService.update(serviceEntity);
				return new ResponseEntity<>(serviceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(serviceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Service that has the same id as the given id.
	 *
	 * @param id The id of the ServiceEntity to be deleted
	...
	@Operation(
			description = "Delete a single Service as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Service to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportService' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Service entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportService' endpoint here] end
	public void exportService(
			@Parameter(description = "The ids of the ServiceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ServiceEntity> entitiesToExport = serviceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Service_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ServiceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportServiceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportServiceExcludingIds' endpoint here] end
	public void exportServiceExcludingIds(
			@Parameter(description = "The ids of the ServiceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ServiceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = serviceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = serviceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Service entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Service_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ServiceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Service entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ServiceEntity', 'create')  || hasPermission('ServiceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ServiceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ServiceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ServiceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ServiceEntity> csvToBean = new CsvToBeanBuilder<ServiceEntity>(reader)
				.withType(ServiceEntity.class)
		...
		try {
			Iterator<ServiceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Service entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Service entity and you do not have permission");
					}
