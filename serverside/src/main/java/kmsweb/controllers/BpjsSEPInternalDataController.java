...
/**
 * Controller used to handle all REST operations regarding BPJS SEP Internal Data
 */
// % protected region % [Update the API route or description for the BPJS SEP Internal DataController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsSEPInternalDataController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjssepinternaldata")
// % protected region % [Update the API route or description for the BPJS SEP Internal DataController here] end
public class BpjsSEPInternalDataController {

	private final BpjsSEPInternalDataService bpjsSEPInternalDataService;
	private final CsvUtils<BpjsSEPInternalDataEntity, BpjsSEPInternalDataRepository, BpjsSEPInternalDataEntityAudit,  BpjsSEPInternalDataService> csvUtils;
	...
	@Autowired
	public BpjsSEPInternalDataController(
		...
		this.csvUtils = new CsvUtils<>(bpjsSEPInternalDataService, BpjsSEPInternalDataEntity.class);
	...
	public ResponseEntity<List<BpjsSEPInternalDataEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsSEPInternalDataEntity> bpjsSEPInternalDatas = bpjsSEPInternalDataService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsSEPInternalDatas, HttpStatus.OK);
	}

	/**
	 * Return the BPJS SEP Internal Data that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsSEPInternalDataEntity to be returned
	 * @return the BPJS SEP Internal Data that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsSEPInternalData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsSEPInternalData entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsSEPInternalDataEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsSEPInternalDataEntity> getById(
			@Parameter(description = "The UUID of the BpjsSEPInternalData to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsSEPInternalDataEntity> bpjsSEPInternalDataEntity = bpjsSEPInternalDataService.findById(id);
	...
	/**
	 * Create/Update the given BPJS SEP Internal Data. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPInternalDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEPInternalData' endpoint here] off begin
	@Operation(
			description = "Create a BpjsSEPInternalData if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsSEPInternalData."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsSEPInternalData, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsSEPInternalDataEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEPInternalData' endpoint here] end
	public ResponseEntity<?> saveBpjsSEPInternalData(
			@Parameter(description = "The BpjsSEPInternalDataEntity to create or update.")
			@RequestBody BpjsSEPInternalDataEntityDto bpjsSEPInternalDataEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsSEPInternalDataEntity bpjsSEPInternalDataEntity = bpjsSEPInternalDataService.create(new BpjsSEPInternalDataEntity(bpjsSEPInternalDataEntityDto));
			return new ResponseEntity<>(bpjsSEPInternalDataEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS SEP Internal Data. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPInternalDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEPInternalData' endpoint here] off begin
	@Operation(
			description = "Update a BpjsSEPInternalData entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsSEPInternalData."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsSEPInternalData, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsSEPInternalDataEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEPInternalData' endpoint here] end
	public ResponseEntity<?> updateBpjsSEPInternalData(
			@Parameter(description = "The BpjsSEPInternalDataEntity to create or update.")
			@RequestBody BpjsSEPInternalDataEntityDto bpjsSEPInternalDataEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsSEPInternalDataEntity> checkBpjsSEPInternalDataExists = bpjsSEPInternalDataService.findById(bpjsSEPInternalDataEntityDto.getId());
		BpjsSEPInternalDataEntity bpjsSEPInternalDataEntity = new BpjsSEPInternalDataEntity(bpjsSEPInternalDataEntityDto);

		if (checkBpjsSEPInternalDataExists.isPresent()) {
			try {
				bpjsSEPInternalDataEntity = bpjsSEPInternalDataService.update(bpjsSEPInternalDataEntity);
				return new ResponseEntity<>(bpjsSEPInternalDataEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsSEPInternalDataEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS SEP Internal Data that has the same id as the given id.
	 *
	 * @param id The id of the BpjsSEPInternalDataEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsSEPInternalData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsSEPInternalData to delete.")
		...
		bpjsSEPInternalDataService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPInternalData' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS SEP Internal Data entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPInternalData' endpoint here] end
	public void exportBpjsSEPInternalData(
			@Parameter(description = "The ids of the BpjsSEPInternalDataEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsSEPInternalDataEntity> entitiesToExport = bpjsSEPInternalDataService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsSEPInternalData_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsSEPInternalDataEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPInternalDataExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPInternalDataExcludingIds' endpoint here] end
	public void exportBpjsSEPInternalDataExcludingIds(
			@Parameter(description = "The ids of the BpjsSEPInternalDataEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsSEPInternalDataEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsSEPInternalDataService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsSEPInternalDataService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsSEPInternalData entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsSEPInternalData_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsSEPInternalDataEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS SEP Internal Data entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsSEPInternalDataEntity', 'create')  || hasPermission('BpjsSEPInternalDataEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsSEPInternalDataEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsSEPInternalDataEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsSEPInternalDataEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsSEPInternalDataEntity> csvToBean = new CsvToBeanBuilder<BpjsSEPInternalDataEntity>(reader)
				.withType(BpjsSEPInternalDataEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsSEPInternalDataEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsSEPInternalDataEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS SEP Internal Data entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS SEP Internal Data entity and you do not have permission");
					}
