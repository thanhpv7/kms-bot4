...
/**
 * Controller used to handle all REST operations regarding Stock Control
 */
// % protected region % [Update the API route or description for the Stock ControlController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "StockControlController"))
@RestController
@Slf4j
@RequestMapping("/api/stockcontrol")
// % protected region % [Update the API route or description for the Stock ControlController here] end
public class StockControlController {

	private final StockControlService stockControlService;
	private final CsvUtils<StockControlEntity, StockControlRepository, StockControlEntityAudit,  StockControlService> csvUtils;
	...
	public StockControlController(
		...
		this.csvUtils = new CsvUtils<>(stockControlService, StockControlEntity.class);
	...
	public ResponseEntity<List<StockControlEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<StockControlEntity> stockControls = stockControlService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(stockControls, HttpStatus.OK);
	}

	/**
	 * Return the Stock Control that has the same id as the given id.
	 *
	 * @param id      The id of the StockControlEntity to be returned
	 * @return the Stock Control that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single StockControl as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "StockControl entity not found"),
	})
	@PreAuthorize("hasPermission('StockControlEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<StockControlEntity> getById(
			@Parameter(description = "The UUID of the StockControl to return.")
			@PathVariable("id") UUID id
	) {
		Optional<StockControlEntity> stockControlEntity = stockControlService.findById(id);
	...
	/**
	 * Create/Update the given Stock Control. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param stockControlEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveStockControl' endpoint here] off begin
	@Operation(
			description = "Create a StockControl if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new StockControl."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new StockControl, duplicate record found")
	})
	@PreAuthorize("hasPermission('StockControlEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveStockControl' endpoint here] end
	public ResponseEntity<?> saveStockControl(
			@Parameter(description = "The StockControlEntity to create or update.")
			@RequestBody StockControlEntityDto stockControlEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			StockControlEntity stockControlEntity = stockControlService.create(new StockControlEntity(stockControlEntityDto));
			return new ResponseEntity<>(stockControlEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Stock Control. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param stockControlEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateStockControl' endpoint here] off begin
	@Operation(
			description = "Update a StockControl entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the StockControl."),
		@ApiResponse(responseCode = "400", description = "Failed to update the StockControl, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('StockControlEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateStockControl' endpoint here] end
	public ResponseEntity<?> updateStockControl(
			@Parameter(description = "The StockControlEntity to create or update.")
			@RequestBody StockControlEntityDto stockControlEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<StockControlEntity> checkStockControlExists = stockControlService.findById(stockControlEntityDto.getId());
		StockControlEntity stockControlEntity = new StockControlEntity(stockControlEntityDto);

		if (checkStockControlExists.isPresent()) {
			try {
				stockControlEntity = stockControlService.update(stockControlEntity);
				return new ResponseEntity<>(stockControlEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(stockControlEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Stock Control that has the same id as the given id.
	 *
	 * @param id The id of the StockControlEntity to be deleted
	...
	@Operation(
			description = "Delete a single StockControl as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the StockControl to delete.")
		...
		stockControlService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockControl' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Stock Control entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportStockControl' endpoint here] end
	public void exportStockControl(
			@Parameter(description = "The ids of the StockControlEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<StockControlEntity> entitiesToExport = stockControlService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "StockControl_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (StockControlEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockControlExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportStockControlExcludingIds' endpoint here] end
	public void exportStockControlExcludingIds(
			@Parameter(description = "The ids of the StockControlEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<StockControlEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = stockControlService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = stockControlService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the StockControl entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "StockControl_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(StockControlEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Stock Control entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('StockControlEntity', 'create')  || hasPermission('StockControlEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with StockControlEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("StockControlEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(StockControlEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<StockControlEntity> csvToBean = new CsvToBeanBuilder<StockControlEntity>(reader)
				.withType(StockControlEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<StockControlEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<StockControlEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Stock Control entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Stock Control entity and you do not have permission");
					}
