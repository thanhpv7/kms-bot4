...
import kmsweb.dtos.CashierUserEntityDto;
import kmsweb.entities.CashierUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Cashier User
 */
// % protected region % [Update the API route or description for the Cashier UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CashierUserController"))
@RestController
@Slf4j
@RequestMapping("/api/cashieruser")
// % protected region % [Update the API route or description for the Cashier UserController here] end
public class CashierUserController {

	private final CashierUserService cashierUserService;
	private final CsvUtils<CashierUserEntity, CashierUserRepository, CashierUserEntityAudit,  CashierUserService> csvUtils;
	...
	public CashierUserController(
		...
		this.csvUtils = new CsvUtils<>(cashierUserService, CashierUserEntity.class);
	...
	public ResponseEntity<List<CashierUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CashierUserEntity> cashierUsers = cashierUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(cashierUsers, HttpStatus.OK);
	}

	/**
	 * Return the Cashier User that has the same id as the given id.
	 *
	 * @param id      The id of the CashierUserEntity to be returned
	 * @return the Cashier User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CashierUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CashierUser entity not found"),
	})
	@PreAuthorize("hasPermission('CashierUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CashierUserEntity> getById(
			@Parameter(description = "The UUID of the CashierUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CashierUserEntity> cashierUserEntity = cashierUserService.findById(id);
	...
	/**
	 * Create/Update the given Cashier User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param cashierUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCashierUser' endpoint here] off begin
	@Operation(
			description = "Create a CashierUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CashierUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CashierUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('CashierUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCashierUser' endpoint here] end
	public ResponseEntity<?> saveCashierUser(
			@Parameter(description = "The CashierUserEntity to create or update.")
			@RequestBody CashierUserEntityDto cashierUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CashierUserEntity cashierUserEntity = cashierUserService.create(new CashierUserEntity(cashierUserEntityDto));
			return new ResponseEntity<>(cashierUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Cashier User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param cashierUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCashierUser' endpoint here] off begin
	@Operation(
			description = "Update a CashierUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CashierUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CashierUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CashierUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCashierUser' endpoint here] end
	public ResponseEntity<?> updateCashierUser(
			@Parameter(description = "The CashierUserEntity to create or update.")
			@RequestBody CashierUserEntityDto cashierUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CashierUserEntity> checkCashierUserExists = cashierUserService.findById(cashierUserEntityDto.getId());
		CashierUserEntity cashierUserEntity = new CashierUserEntity(cashierUserEntityDto);

		if (checkCashierUserExists.isPresent()) {
			try {
				cashierUserEntity = cashierUserService.update(cashierUserEntity);
				return new ResponseEntity<>(cashierUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(cashierUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Cashier User that has the same id as the given id.
	 *
	 * @param id The id of the CashierUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single CashierUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CashierUser to delete.")
		...
		cashierUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashierUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Cashier User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCashierUser' endpoint here] end
	public void exportCashierUser(
			@Parameter(description = "The ids of the CashierUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CashierUserEntity> entitiesToExport = cashierUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CashierUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CashierUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashierUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCashierUserExcludingIds' endpoint here] end
	public void exportCashierUserExcludingIds(
			@Parameter(description = "The ids of the CashierUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CashierUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = cashierUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = cashierUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CashierUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CashierUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CashierUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Cashier User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CashierUserEntity', 'create')  || hasPermission('CashierUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CashierUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CashierUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CashierUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CashierUserEntity> csvToBean = new CsvToBeanBuilder<CashierUserEntity>(reader)
				.withType(CashierUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CashierUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CashierUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Cashier User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Cashier User entity and you do not have permission");
					}
