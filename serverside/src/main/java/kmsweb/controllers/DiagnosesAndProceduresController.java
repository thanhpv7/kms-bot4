...
import kmsweb.entities.DiagnosesAndProceduresEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Diagnoses and Procedures
 */
// % protected region % [Update the API route or description for the Diagnoses and ProceduresController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DiagnosesAndProceduresController"))
@RestController
@Slf4j
@RequestMapping("/api/diagnosesandprocedures")
// % protected region % [Update the API route or description for the Diagnoses and ProceduresController here] end
public class DiagnosesAndProceduresController {

	private final DiagnosesAndProceduresService diagnosesAndProceduresService;
	private final CsvUtils<DiagnosesAndProceduresEntity, DiagnosesAndProceduresRepository, DiagnosesAndProceduresEntityAudit,  DiagnosesAndProceduresService> csvUtils;
	...
	@Autowired
	public DiagnosesAndProceduresController(
		...
		this.csvUtils = new CsvUtils<>(diagnosesAndProceduresService, DiagnosesAndProceduresEntity.class);
	...
	public ResponseEntity<List<DiagnosesAndProceduresEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DiagnosesAndProceduresEntity> diagnosesAndProceduress = diagnosesAndProceduresService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(diagnosesAndProceduress, HttpStatus.OK);
	}

	/**
	 * Return the Diagnoses and Procedures that has the same id as the given id.
	 *
	 * @param id      The id of the DiagnosesAndProceduresEntity to be returned
	 * @return the Diagnoses and Procedures that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DiagnosesAndProcedures as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DiagnosesAndProcedures entity not found"),
	})
	@PreAuthorize("hasPermission('DiagnosesAndProceduresEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DiagnosesAndProceduresEntity> getById(
			@Parameter(description = "The UUID of the DiagnosesAndProcedures to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DiagnosesAndProceduresEntity> diagnosesAndProceduresEntity = diagnosesAndProceduresService.findById(id);
	...
	/**
	 * Create/Update the given Diagnoses and Procedures. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosesAndProceduresEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDiagnosesAndProcedures' endpoint here] off begin
	@Operation(
			description = "Create a DiagnosesAndProcedures if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DiagnosesAndProcedures."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DiagnosesAndProcedures, duplicate record found")
	})
	@PreAuthorize("hasPermission('DiagnosesAndProceduresEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDiagnosesAndProcedures' endpoint here] end
	public ResponseEntity<?> saveDiagnosesAndProcedures(
			@Parameter(description = "The DiagnosesAndProceduresEntity to create or update.")
			@RequestBody DiagnosesAndProceduresEntityDto diagnosesAndProceduresEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DiagnosesAndProceduresEntity diagnosesAndProceduresEntity = diagnosesAndProceduresService.create(new DiagnosesAndProceduresEntity(diagnosesAndProceduresEntityDto));
			return new ResponseEntity<>(diagnosesAndProceduresEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Diagnoses and Procedures. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosesAndProceduresEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDiagnosesAndProcedures' endpoint here] off begin
	@Operation(
			description = "Update a DiagnosesAndProcedures entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DiagnosesAndProcedures."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DiagnosesAndProcedures, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DiagnosesAndProceduresEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDiagnosesAndProcedures' endpoint here] end
	public ResponseEntity<?> updateDiagnosesAndProcedures(
			@Parameter(description = "The DiagnosesAndProceduresEntity to create or update.")
			@RequestBody DiagnosesAndProceduresEntityDto diagnosesAndProceduresEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DiagnosesAndProceduresEntity> checkDiagnosesAndProceduresExists = diagnosesAndProceduresService.findById(diagnosesAndProceduresEntityDto.getId());
		DiagnosesAndProceduresEntity diagnosesAndProceduresEntity = new DiagnosesAndProceduresEntity(diagnosesAndProceduresEntityDto);

		if (checkDiagnosesAndProceduresExists.isPresent()) {
			try {
				diagnosesAndProceduresEntity = diagnosesAndProceduresService.update(diagnosesAndProceduresEntity);
				return new ResponseEntity<>(diagnosesAndProceduresEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(diagnosesAndProceduresEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Diagnoses and Procedures that has the same id as the given id.
	 *
	 * @param id The id of the DiagnosesAndProceduresEntity to be deleted
	...
	@Operation(
			description = "Delete a single DiagnosesAndProcedures as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DiagnosesAndProcedures to delete.")
		...
		diagnosesAndProceduresService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosesAndProcedures' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Diagnoses and Procedures entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosesAndProcedures' endpoint here] end
	public void exportDiagnosesAndProcedures(
			@Parameter(description = "The ids of the DiagnosesAndProceduresEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DiagnosesAndProceduresEntity> entitiesToExport = diagnosesAndProceduresService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DiagnosesAndProcedures_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DiagnosesAndProceduresEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosesAndProceduresExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosesAndProceduresExcludingIds' endpoint here] end
	public void exportDiagnosesAndProceduresExcludingIds(
			@Parameter(description = "The ids of the DiagnosesAndProceduresEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DiagnosesAndProceduresEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = diagnosesAndProceduresService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = diagnosesAndProceduresService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DiagnosesAndProcedures entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DiagnosesAndProcedures_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DiagnosesAndProceduresEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Diagnoses and Procedures entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DiagnosesAndProceduresEntity', 'create')  || hasPermission('DiagnosesAndProceduresEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DiagnosesAndProceduresEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DiagnosesAndProceduresEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DiagnosesAndProceduresEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DiagnosesAndProceduresEntity> csvToBean = new CsvToBeanBuilder<DiagnosesAndProceduresEntity>(reader)
				.withType(DiagnosesAndProceduresEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DiagnosesAndProceduresEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DiagnosesAndProceduresEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Diagnoses and Procedures entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Diagnoses and Procedures entity and you do not have permission");
					}
