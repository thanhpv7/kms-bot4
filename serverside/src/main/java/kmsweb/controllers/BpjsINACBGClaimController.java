...
/**
 * Controller used to handle all REST operations regarding BPJS INACBG Claim
 */
// % protected region % [Update the API route or description for the BPJS INACBG ClaimController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsINACBGClaimController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsinacbgclaim")
// % protected region % [Update the API route or description for the BPJS INACBG ClaimController here] end
public class BpjsINACBGClaimController {

	private final BpjsINACBGClaimService bpjsINACBGClaimService;
	private final CsvUtils<BpjsINACBGClaimEntity, BpjsINACBGClaimRepository, BpjsINACBGClaimEntityAudit,  BpjsINACBGClaimService> csvUtils;
	...
	@Autowired
	public BpjsINACBGClaimController(
		...
		this.csvUtils = new CsvUtils<>(bpjsINACBGClaimService, BpjsINACBGClaimEntity.class);
	...
	public ResponseEntity<List<BpjsINACBGClaimEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsINACBGClaimEntity> bpjsINACBGClaims = bpjsINACBGClaimService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsINACBGClaims, HttpStatus.OK);
	}

	/**
	 * Return the BPJS INACBG Claim that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsINACBGClaimEntity to be returned
	 * @return the BPJS INACBG Claim that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsINACBGClaim as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsINACBGClaim entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsINACBGClaimEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsINACBGClaimEntity> getById(
			@Parameter(description = "The UUID of the BpjsINACBGClaim to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsINACBGClaimEntity> bpjsINACBGClaimEntity = bpjsINACBGClaimService.findById(id);
	...
	/**
	 * Create/Update the given BPJS INACBG Claim. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsINACBGClaimEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsINACBGClaim' endpoint here] off begin
	@Operation(
			description = "Create a BpjsINACBGClaim if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsINACBGClaim."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsINACBGClaim, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsINACBGClaimEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsINACBGClaim' endpoint here] end
	public ResponseEntity<?> saveBpjsINACBGClaim(
			@Parameter(description = "The BpjsINACBGClaimEntity to create or update.")
			@RequestBody BpjsINACBGClaimEntityDto bpjsINACBGClaimEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsINACBGClaimEntity bpjsINACBGClaimEntity = bpjsINACBGClaimService.create(new BpjsINACBGClaimEntity(bpjsINACBGClaimEntityDto));
			return new ResponseEntity<>(bpjsINACBGClaimEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS INACBG Claim. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsINACBGClaimEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsINACBGClaim' endpoint here] off begin
	@Operation(
			description = "Update a BpjsINACBGClaim entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsINACBGClaim."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsINACBGClaim, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsINACBGClaimEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsINACBGClaim' endpoint here] end
	public ResponseEntity<?> updateBpjsINACBGClaim(
			@Parameter(description = "The BpjsINACBGClaimEntity to create or update.")
			@RequestBody BpjsINACBGClaimEntityDto bpjsINACBGClaimEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsINACBGClaimEntity> checkBpjsINACBGClaimExists = bpjsINACBGClaimService.findById(bpjsINACBGClaimEntityDto.getId());
		BpjsINACBGClaimEntity bpjsINACBGClaimEntity = new BpjsINACBGClaimEntity(bpjsINACBGClaimEntityDto);

		if (checkBpjsINACBGClaimExists.isPresent()) {
			try {
				bpjsINACBGClaimEntity = bpjsINACBGClaimService.update(bpjsINACBGClaimEntity);
				return new ResponseEntity<>(bpjsINACBGClaimEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsINACBGClaimEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS INACBG Claim that has the same id as the given id.
	 *
	 * @param id The id of the BpjsINACBGClaimEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsINACBGClaim as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsINACBGClaim to delete.")
		...
		bpjsINACBGClaimService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGClaim' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS INACBG Claim entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGClaim' endpoint here] end
	public void exportBpjsINACBGClaim(
			@Parameter(description = "The ids of the BpjsINACBGClaimEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsINACBGClaimEntity> entitiesToExport = bpjsINACBGClaimService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsINACBGClaim_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsINACBGClaimEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGClaimExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGClaimExcludingIds' endpoint here] end
	public void exportBpjsINACBGClaimExcludingIds(
			@Parameter(description = "The ids of the BpjsINACBGClaimEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsINACBGClaimEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsINACBGClaimService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsINACBGClaimService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsINACBGClaim entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsINACBGClaim_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsINACBGClaimEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS INACBG Claim entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsINACBGClaimEntity', 'create')  || hasPermission('BpjsINACBGClaimEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsINACBGClaimEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsINACBGClaimEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsINACBGClaimEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsINACBGClaimEntity> csvToBean = new CsvToBeanBuilder<BpjsINACBGClaimEntity>(reader)
				.withType(BpjsINACBGClaimEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsINACBGClaimEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsINACBGClaimEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS INACBG Claim entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS INACBG Claim entity and you do not have permission");
					}
