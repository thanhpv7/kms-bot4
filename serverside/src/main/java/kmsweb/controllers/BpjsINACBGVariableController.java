...
import kmsweb.entities.BpjsINACBGVariableEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS INACBG Variable
 */
// % protected region % [Update the API route or description for the BPJS INACBG VariableController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsINACBGVariableController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsinacbgvariable")
// % protected region % [Update the API route or description for the BPJS INACBG VariableController here] end
public class BpjsINACBGVariableController {

	private final BpjsINACBGVariableService bpjsINACBGVariableService;
	private final CsvUtils<BpjsINACBGVariableEntity, BpjsINACBGVariableRepository, BpjsINACBGVariableEntityAudit,  BpjsINACBGVariableService> csvUtils;
	...
	@Autowired
	public BpjsINACBGVariableController(
		...
		this.csvUtils = new CsvUtils<>(bpjsINACBGVariableService, BpjsINACBGVariableEntity.class);
	...
	public ResponseEntity<List<BpjsINACBGVariableEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsINACBGVariableEntity> bpjsINACBGVariables = bpjsINACBGVariableService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsINACBGVariables, HttpStatus.OK);
	}

	/**
	 * Return the BPJS INACBG Variable that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsINACBGVariableEntity to be returned
	 * @return the BPJS INACBG Variable that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsINACBGVariable as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsINACBGVariable entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsINACBGVariableEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsINACBGVariableEntity> getById(
			@Parameter(description = "The UUID of the BpjsINACBGVariable to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsINACBGVariableEntity> bpjsINACBGVariableEntity = bpjsINACBGVariableService.findById(id);
	...
	/**
	 * Create/Update the given BPJS INACBG Variable. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsINACBGVariableEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsINACBGVariable' endpoint here] off begin
	@Operation(
			description = "Create a BpjsINACBGVariable if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsINACBGVariable."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsINACBGVariable, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsINACBGVariableEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsINACBGVariable' endpoint here] end
	public ResponseEntity<?> saveBpjsINACBGVariable(
			@Parameter(description = "The BpjsINACBGVariableEntity to create or update.")
			@RequestBody BpjsINACBGVariableEntityDto bpjsINACBGVariableEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsINACBGVariableEntity bpjsINACBGVariableEntity = bpjsINACBGVariableService.create(new BpjsINACBGVariableEntity(bpjsINACBGVariableEntityDto));
			return new ResponseEntity<>(bpjsINACBGVariableEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS INACBG Variable. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsINACBGVariableEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsINACBGVariable' endpoint here] off begin
	@Operation(
			description = "Update a BpjsINACBGVariable entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsINACBGVariable."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsINACBGVariable, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsINACBGVariableEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsINACBGVariable' endpoint here] end
	public ResponseEntity<?> updateBpjsINACBGVariable(
			@Parameter(description = "The BpjsINACBGVariableEntity to create or update.")
			@RequestBody BpjsINACBGVariableEntityDto bpjsINACBGVariableEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsINACBGVariableEntity> checkBpjsINACBGVariableExists = bpjsINACBGVariableService.findById(bpjsINACBGVariableEntityDto.getId());
		BpjsINACBGVariableEntity bpjsINACBGVariableEntity = new BpjsINACBGVariableEntity(bpjsINACBGVariableEntityDto);

		if (checkBpjsINACBGVariableExists.isPresent()) {
			try {
				bpjsINACBGVariableEntity = bpjsINACBGVariableService.update(bpjsINACBGVariableEntity);
				return new ResponseEntity<>(bpjsINACBGVariableEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsINACBGVariableEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS INACBG Variable that has the same id as the given id.
	 *
	 * @param id The id of the BpjsINACBGVariableEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsINACBGVariable as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsINACBGVariable to delete.")
		...
		bpjsINACBGVariableService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGVariable' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS INACBG Variable entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGVariable' endpoint here] end
	public void exportBpjsINACBGVariable(
			@Parameter(description = "The ids of the BpjsINACBGVariableEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsINACBGVariableEntity> entitiesToExport = bpjsINACBGVariableService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsINACBGVariable_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsINACBGVariableEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGVariableExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGVariableExcludingIds' endpoint here] end
	public void exportBpjsINACBGVariableExcludingIds(
			@Parameter(description = "The ids of the BpjsINACBGVariableEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsINACBGVariableEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsINACBGVariableService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsINACBGVariableService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsINACBGVariable entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsINACBGVariable_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsINACBGVariableEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS INACBG Variable entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsINACBGVariableEntity', 'create')  || hasPermission('BpjsINACBGVariableEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsINACBGVariableEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsINACBGVariableEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsINACBGVariableEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsINACBGVariableEntity> csvToBean = new CsvToBeanBuilder<BpjsINACBGVariableEntity>(reader)
				.withType(BpjsINACBGVariableEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsINACBGVariableEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsINACBGVariableEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS INACBG Variable entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS INACBG Variable entity and you do not have permission");
					}
