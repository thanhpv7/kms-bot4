...
import kmsweb.entities.ReturnSupplierStockDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Return Supplier Stock Detail
 */
// % protected region % [Update the API route or description for the Return Supplier Stock DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ReturnSupplierStockDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/returnsupplierstockdetail")
// % protected region % [Update the API route or description for the Return Supplier Stock DetailController here] end
public class ReturnSupplierStockDetailController {

	private final ReturnSupplierStockDetailService returnSupplierStockDetailService;
	private final CsvUtils<ReturnSupplierStockDetailEntity, ReturnSupplierStockDetailRepository, ReturnSupplierStockDetailEntityAudit,  ReturnSupplierStockDetailService> csvUtils;
	...
	@Autowired
	public ReturnSupplierStockDetailController(
		...
		this.csvUtils = new CsvUtils<>(returnSupplierStockDetailService, ReturnSupplierStockDetailEntity.class);
	...
	/**
	 * Return all the Return Supplier Stock Details.
	 *
	 * @return all the Return Supplier Stock Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of ReturnSupplierStockDetails",
		security = {
	...
	public ResponseEntity<List<ReturnSupplierStockDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ReturnSupplierStockDetailEntity> returnSupplierStockDetails = returnSupplierStockDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(returnSupplierStockDetails, HttpStatus.OK);
	}

	/**
	 * Return the Return Supplier Stock Detail that has the same id as the given id.
	 *
	 * @param id      The id of the ReturnSupplierStockDetailEntity to be returned
	 * @return the Return Supplier Stock Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ReturnSupplierStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ReturnSupplierStockDetail entity not found"),
	})
	@PreAuthorize("hasPermission('ReturnSupplierStockDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ReturnSupplierStockDetailEntity> getById(
			@Parameter(description = "The UUID of the ReturnSupplierStockDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ReturnSupplierStockDetailEntity> returnSupplierStockDetailEntity = returnSupplierStockDetailService.findById(id);
	...
	/**
	 * Create/Update the given Return Supplier Stock Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param returnSupplierStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveReturnSupplierStockDetail' endpoint here] off begin
	@Operation(
			description = "Create a ReturnSupplierStockDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ReturnSupplierStockDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ReturnSupplierStockDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('ReturnSupplierStockDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveReturnSupplierStockDetail' endpoint here] end
	public ResponseEntity<?> saveReturnSupplierStockDetail(
			@Parameter(description = "The ReturnSupplierStockDetailEntity to create or update.")
			@RequestBody ReturnSupplierStockDetailEntityDto returnSupplierStockDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ReturnSupplierStockDetailEntity returnSupplierStockDetailEntity = returnSupplierStockDetailService.create(new ReturnSupplierStockDetailEntity(returnSupplierStockDetailEntityDto));
			return new ResponseEntity<>(returnSupplierStockDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Return Supplier Stock Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param returnSupplierStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateReturnSupplierStockDetail' endpoint here] off begin
	@Operation(
			description = "Update a ReturnSupplierStockDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ReturnSupplierStockDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ReturnSupplierStockDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ReturnSupplierStockDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateReturnSupplierStockDetail' endpoint here] end
	public ResponseEntity<?> updateReturnSupplierStockDetail(
			@Parameter(description = "The ReturnSupplierStockDetailEntity to create or update.")
			@RequestBody ReturnSupplierStockDetailEntityDto returnSupplierStockDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ReturnSupplierStockDetailEntity> checkReturnSupplierStockDetailExists = returnSupplierStockDetailService.findById(returnSupplierStockDetailEntityDto.getId());
		ReturnSupplierStockDetailEntity returnSupplierStockDetailEntity = new ReturnSupplierStockDetailEntity(returnSupplierStockDetailEntityDto);

		if (checkReturnSupplierStockDetailExists.isPresent()) {
			try {
				returnSupplierStockDetailEntity = returnSupplierStockDetailService.update(returnSupplierStockDetailEntity);
				return new ResponseEntity<>(returnSupplierStockDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(returnSupplierStockDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Return Supplier Stock Detail that has the same id as the given id.
	 *
	 * @param id The id of the ReturnSupplierStockDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single ReturnSupplierStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ReturnSupplierStockDetail to delete.")
		...
		returnSupplierStockDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplierStockDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Return Supplier Stock Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplierStockDetail' endpoint here] end
	public void exportReturnSupplierStockDetail(
			@Parameter(description = "The ids of the ReturnSupplierStockDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ReturnSupplierStockDetailEntity> entitiesToExport = returnSupplierStockDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ReturnSupplierStockDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ReturnSupplierStockDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplierStockDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplierStockDetailExcludingIds' endpoint here] end
	public void exportReturnSupplierStockDetailExcludingIds(
			@Parameter(description = "The ids of the ReturnSupplierStockDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ReturnSupplierStockDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = returnSupplierStockDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = returnSupplierStockDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ReturnSupplierStockDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ReturnSupplierStockDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ReturnSupplierStockDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Return Supplier Stock Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ReturnSupplierStockDetailEntity', 'create')  || hasPermission('ReturnSupplierStockDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ReturnSupplierStockDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ReturnSupplierStockDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ReturnSupplierStockDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ReturnSupplierStockDetailEntity> csvToBean = new CsvToBeanBuilder<ReturnSupplierStockDetailEntity>(reader)
				.withType(ReturnSupplierStockDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ReturnSupplierStockDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ReturnSupplierStockDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Return Supplier Stock Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Return Supplier Stock Detail entity and you do not have permission");
					}
