...
import kmsweb.entities.MedicalFeeEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medical Fee
 */
// % protected region % [Update the API route or description for the Medical FeeController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicalFeeController"))
@RestController
@Slf4j
@RequestMapping("/api/medicalfee")
// % protected region % [Update the API route or description for the Medical FeeController here] end
public class MedicalFeeController {

	private final MedicalFeeService medicalFeeService;
	private final CsvUtils<MedicalFeeEntity, MedicalFeeRepository, MedicalFeeEntityAudit,  MedicalFeeService> csvUtils;
	...
	public MedicalFeeController(
		...
		this.csvUtils = new CsvUtils<>(medicalFeeService, MedicalFeeEntity.class);
	...
	public ResponseEntity<List<MedicalFeeEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicalFeeEntity> medicalFees = medicalFeeService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicalFees, HttpStatus.OK);
	}

	/**
	 * Return the Medical Fee that has the same id as the given id.
	 *
	 * @param id      The id of the MedicalFeeEntity to be returned
	 * @return the Medical Fee that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicalFee as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicalFee entity not found"),
	})
	@PreAuthorize("hasPermission('MedicalFeeEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicalFeeEntity> getById(
			@Parameter(description = "The UUID of the MedicalFee to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicalFeeEntity> medicalFeeEntity = medicalFeeService.findById(id);
	...
	/**
	 * Create/Update the given Medical Fee. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicalFeeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicalFee' endpoint here] off begin
	@Operation(
			description = "Create a MedicalFee if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicalFee."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicalFee, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicalFeeEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicalFee' endpoint here] end
	public ResponseEntity<?> saveMedicalFee(
			@Parameter(description = "The MedicalFeeEntity to create or update.")
			@RequestBody MedicalFeeEntityDto medicalFeeEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicalFeeEntity medicalFeeEntity = medicalFeeService.create(new MedicalFeeEntity(medicalFeeEntityDto));
			return new ResponseEntity<>(medicalFeeEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medical Fee. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicalFeeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicalFee' endpoint here] off begin
	@Operation(
			description = "Update a MedicalFee entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicalFee."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicalFee, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicalFeeEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicalFee' endpoint here] end
	public ResponseEntity<?> updateMedicalFee(
			@Parameter(description = "The MedicalFeeEntity to create or update.")
			@RequestBody MedicalFeeEntityDto medicalFeeEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicalFeeEntity> checkMedicalFeeExists = medicalFeeService.findById(medicalFeeEntityDto.getId());
		MedicalFeeEntity medicalFeeEntity = new MedicalFeeEntity(medicalFeeEntityDto);

		if (checkMedicalFeeExists.isPresent()) {
			try {
				medicalFeeEntity = medicalFeeService.update(medicalFeeEntity);
				return new ResponseEntity<>(medicalFeeEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicalFeeEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medical Fee that has the same id as the given id.
	 *
	 * @param id The id of the MedicalFeeEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicalFee as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicalFee to delete.")
		...
		medicalFeeService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalFee' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medical Fee entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalFee' endpoint here] end
	public void exportMedicalFee(
			@Parameter(description = "The ids of the MedicalFeeEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicalFeeEntity> entitiesToExport = medicalFeeService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicalFee_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicalFeeEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalFeeExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalFeeExcludingIds' endpoint here] end
	public void exportMedicalFeeExcludingIds(
			@Parameter(description = "The ids of the MedicalFeeEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicalFeeEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicalFeeService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicalFeeService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicalFee entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicalFee_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicalFeeEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medical Fee entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicalFeeEntity', 'create')  || hasPermission('MedicalFeeEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicalFeeEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicalFeeEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicalFeeEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicalFeeEntity> csvToBean = new CsvToBeanBuilder<MedicalFeeEntity>(reader)
				.withType(MedicalFeeEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicalFeeEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicalFeeEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medical Fee entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medical Fee entity and you do not have permission");
					}
