...
/**
 * Controller used to handle all REST operations regarding Fluid Balance Detail
 */
// % protected region % [Update the API route or description for the Fluid Balance DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "FluidBalanceDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/fluidbalancedetail")
// % protected region % [Update the API route or description for the Fluid Balance DetailController here] end
public class FluidBalanceDetailController {

	private final FluidBalanceDetailService fluidBalanceDetailService;
	private final CsvUtils<FluidBalanceDetailEntity, FluidBalanceDetailRepository, FluidBalanceDetailEntityAudit,  FluidBalanceDetailService> csvUtils;
	...
	@Autowired
	public FluidBalanceDetailController(
		...
		this.csvUtils = new CsvUtils<>(fluidBalanceDetailService, FluidBalanceDetailEntity.class);
	...
	public ResponseEntity<List<FluidBalanceDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<FluidBalanceDetailEntity> fluidBalanceDetails = fluidBalanceDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(fluidBalanceDetails, HttpStatus.OK);
	}

	/**
	 * Return the Fluid Balance Detail that has the same id as the given id.
	 *
	 * @param id      The id of the FluidBalanceDetailEntity to be returned
	 * @return the Fluid Balance Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single FluidBalanceDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "FluidBalanceDetail entity not found"),
	})
	@PreAuthorize("hasPermission('FluidBalanceDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<FluidBalanceDetailEntity> getById(
			@Parameter(description = "The UUID of the FluidBalanceDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<FluidBalanceDetailEntity> fluidBalanceDetailEntity = fluidBalanceDetailService.findById(id);
	...
	/**
	 * Create/Update the given Fluid Balance Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param fluidBalanceDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveFluidBalanceDetail' endpoint here] off begin
	@Operation(
			description = "Create a FluidBalanceDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new FluidBalanceDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new FluidBalanceDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('FluidBalanceDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveFluidBalanceDetail' endpoint here] end
	public ResponseEntity<?> saveFluidBalanceDetail(
			@Parameter(description = "The FluidBalanceDetailEntity to create or update.")
			@RequestBody FluidBalanceDetailEntityDto fluidBalanceDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			FluidBalanceDetailEntity fluidBalanceDetailEntity = fluidBalanceDetailService.create(new FluidBalanceDetailEntity(fluidBalanceDetailEntityDto));
			return new ResponseEntity<>(fluidBalanceDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Fluid Balance Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param fluidBalanceDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateFluidBalanceDetail' endpoint here] off begin
	@Operation(
			description = "Update a FluidBalanceDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the FluidBalanceDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the FluidBalanceDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('FluidBalanceDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateFluidBalanceDetail' endpoint here] end
	public ResponseEntity<?> updateFluidBalanceDetail(
			@Parameter(description = "The FluidBalanceDetailEntity to create or update.")
			@RequestBody FluidBalanceDetailEntityDto fluidBalanceDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<FluidBalanceDetailEntity> checkFluidBalanceDetailExists = fluidBalanceDetailService.findById(fluidBalanceDetailEntityDto.getId());
		FluidBalanceDetailEntity fluidBalanceDetailEntity = new FluidBalanceDetailEntity(fluidBalanceDetailEntityDto);

		if (checkFluidBalanceDetailExists.isPresent()) {
			try {
				fluidBalanceDetailEntity = fluidBalanceDetailService.update(fluidBalanceDetailEntity);
				return new ResponseEntity<>(fluidBalanceDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(fluidBalanceDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Fluid Balance Detail that has the same id as the given id.
	 *
	 * @param id The id of the FluidBalanceDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single FluidBalanceDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the FluidBalanceDetail to delete.")
		...
		fluidBalanceDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportFluidBalanceDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Fluid Balance Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportFluidBalanceDetail' endpoint here] end
	public void exportFluidBalanceDetail(
			@Parameter(description = "The ids of the FluidBalanceDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<FluidBalanceDetailEntity> entitiesToExport = fluidBalanceDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "FluidBalanceDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (FluidBalanceDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportFluidBalanceDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportFluidBalanceDetailExcludingIds' endpoint here] end
	public void exportFluidBalanceDetailExcludingIds(
			@Parameter(description = "The ids of the FluidBalanceDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<FluidBalanceDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = fluidBalanceDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = fluidBalanceDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the FluidBalanceDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "FluidBalanceDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(FluidBalanceDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Fluid Balance Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('FluidBalanceDetailEntity', 'create')  || hasPermission('FluidBalanceDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with FluidBalanceDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("FluidBalanceDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(FluidBalanceDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<FluidBalanceDetailEntity> csvToBean = new CsvToBeanBuilder<FluidBalanceDetailEntity>(reader)
				.withType(FluidBalanceDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<FluidBalanceDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<FluidBalanceDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Fluid Balance Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Fluid Balance Detail entity and you do not have permission");
					}
