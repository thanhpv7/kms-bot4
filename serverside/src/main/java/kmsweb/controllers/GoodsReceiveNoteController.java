...
import kmsweb.entities.GoodsReceiveNoteEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Goods Receive Note
 */
// % protected region % [Update the API route or description for the Goods Receive NoteController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "GoodsReceiveNoteController"))
@RestController
@Slf4j
@RequestMapping("/api/goodsreceivenote")
// % protected region % [Update the API route or description for the Goods Receive NoteController here] end
public class GoodsReceiveNoteController {

	private final GoodsReceiveNoteService goodsReceiveNoteService;
	private final CsvUtils<GoodsReceiveNoteEntity, GoodsReceiveNoteRepository, GoodsReceiveNoteEntityAudit,  GoodsReceiveNoteService> csvUtils;
	...
	@Autowired
	public GoodsReceiveNoteController(
		...
		this.csvUtils = new CsvUtils<>(goodsReceiveNoteService, GoodsReceiveNoteEntity.class);
	...
	public ResponseEntity<List<GoodsReceiveNoteEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<GoodsReceiveNoteEntity> goodsReceiveNotes = goodsReceiveNoteService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(goodsReceiveNotes, HttpStatus.OK);
	}

	/**
	 * Return the Goods Receive Note that has the same id as the given id.
	 *
	 * @param id      The id of the GoodsReceiveNoteEntity to be returned
	 * @return the Goods Receive Note that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single GoodsReceiveNote as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "GoodsReceiveNote entity not found"),
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<GoodsReceiveNoteEntity> getById(
			@Parameter(description = "The UUID of the GoodsReceiveNote to return.")
			@PathVariable("id") UUID id
	) {
		Optional<GoodsReceiveNoteEntity> goodsReceiveNoteEntity = goodsReceiveNoteService.findById(id);
	...
	/**
	 * Create/Update the given Goods Receive Note. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param goodsReceiveNoteEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveGoodsReceiveNote' endpoint here] off begin
	@Operation(
			description = "Create a GoodsReceiveNote if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new GoodsReceiveNote."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new GoodsReceiveNote, duplicate record found")
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveGoodsReceiveNote' endpoint here] end
	public ResponseEntity<?> saveGoodsReceiveNote(
			@Parameter(description = "The GoodsReceiveNoteEntity to create or update.")
			@RequestBody GoodsReceiveNoteEntityDto goodsReceiveNoteEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			GoodsReceiveNoteEntity goodsReceiveNoteEntity = goodsReceiveNoteService.create(new GoodsReceiveNoteEntity(goodsReceiveNoteEntityDto));
			return new ResponseEntity<>(goodsReceiveNoteEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Goods Receive Note. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param goodsReceiveNoteEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateGoodsReceiveNote' endpoint here] off begin
	@Operation(
			description = "Update a GoodsReceiveNote entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the GoodsReceiveNote."),
		@ApiResponse(responseCode = "400", description = "Failed to update the GoodsReceiveNote, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateGoodsReceiveNote' endpoint here] end
	public ResponseEntity<?> updateGoodsReceiveNote(
			@Parameter(description = "The GoodsReceiveNoteEntity to create or update.")
			@RequestBody GoodsReceiveNoteEntityDto goodsReceiveNoteEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<GoodsReceiveNoteEntity> checkGoodsReceiveNoteExists = goodsReceiveNoteService.findById(goodsReceiveNoteEntityDto.getId());
		GoodsReceiveNoteEntity goodsReceiveNoteEntity = new GoodsReceiveNoteEntity(goodsReceiveNoteEntityDto);

		if (checkGoodsReceiveNoteExists.isPresent()) {
			try {
				goodsReceiveNoteEntity = goodsReceiveNoteService.update(goodsReceiveNoteEntity);
				return new ResponseEntity<>(goodsReceiveNoteEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(goodsReceiveNoteEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Goods Receive Note that has the same id as the given id.
	 *
	 * @param id The id of the GoodsReceiveNoteEntity to be deleted
	...
	@Operation(
			description = "Delete a single GoodsReceiveNote as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the GoodsReceiveNote to delete.")
		...
		goodsReceiveNoteService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNote' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Goods Receive Note entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNote' endpoint here] end
	public void exportGoodsReceiveNote(
			@Parameter(description = "The ids of the GoodsReceiveNoteEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<GoodsReceiveNoteEntity> entitiesToExport = goodsReceiveNoteService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "GoodsReceiveNote_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (GoodsReceiveNoteEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteExcludingIds' endpoint here] end
	public void exportGoodsReceiveNoteExcludingIds(
			@Parameter(description = "The ids of the GoodsReceiveNoteEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<GoodsReceiveNoteEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = goodsReceiveNoteService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = goodsReceiveNoteService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the GoodsReceiveNote entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "GoodsReceiveNote_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(GoodsReceiveNoteEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Goods Receive Note entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('GoodsReceiveNoteEntity', 'create')  || hasPermission('GoodsReceiveNoteEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with GoodsReceiveNoteEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("GoodsReceiveNoteEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(GoodsReceiveNoteEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<GoodsReceiveNoteEntity> csvToBean = new CsvToBeanBuilder<GoodsReceiveNoteEntity>(reader)
				.withType(GoodsReceiveNoteEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<GoodsReceiveNoteEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<GoodsReceiveNoteEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Goods Receive Note entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Goods Receive Note entity and you do not have permission");
					}
