...
import kmsweb.entities.BatchStockTransactionDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Batch Stock Transaction Detail
 */
// % protected region % [Update the API route or description for the Batch Stock Transaction DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BatchStockTransactionDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/batchstocktransactiondetail")
// % protected region % [Update the API route or description for the Batch Stock Transaction DetailController here] end
public class BatchStockTransactionDetailController {

	private final BatchStockTransactionDetailService batchStockTransactionDetailService;
	private final CsvUtils<BatchStockTransactionDetailEntity, BatchStockTransactionDetailRepository, BatchStockTransactionDetailEntityAudit,  BatchStockTransactionDetailService> csvUtils;
	...
	@Autowired
	public BatchStockTransactionDetailController(
		...
		this.csvUtils = new CsvUtils<>(batchStockTransactionDetailService, BatchStockTransactionDetailEntity.class);
	...
	/**
	 * Return all the Batch Stock Transaction Details.
	 *
	 * @return all the Batch Stock Transaction Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BatchStockTransactionDetails",
		security = {
	...
	public ResponseEntity<List<BatchStockTransactionDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BatchStockTransactionDetailEntity> batchStockTransactionDetails = batchStockTransactionDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(batchStockTransactionDetails, HttpStatus.OK);
	}

	/**
	 * Return the Batch Stock Transaction Detail that has the same id as the given id.
	 *
	 * @param id      The id of the BatchStockTransactionDetailEntity to be returned
	 * @return the Batch Stock Transaction Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BatchStockTransactionDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BatchStockTransactionDetail entity not found"),
	})
	@PreAuthorize("hasPermission('BatchStockTransactionDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BatchStockTransactionDetailEntity> getById(
			@Parameter(description = "The UUID of the BatchStockTransactionDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BatchStockTransactionDetailEntity> batchStockTransactionDetailEntity = batchStockTransactionDetailService.findById(id);
	...
	/**
	 * Create/Update the given Batch Stock Transaction Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param batchStockTransactionDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBatchStockTransactionDetail' endpoint here] off begin
	@Operation(
			description = "Create a BatchStockTransactionDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BatchStockTransactionDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BatchStockTransactionDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('BatchStockTransactionDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBatchStockTransactionDetail' endpoint here] end
	public ResponseEntity<?> saveBatchStockTransactionDetail(
			@Parameter(description = "The BatchStockTransactionDetailEntity to create or update.")
			@RequestBody BatchStockTransactionDetailEntityDto batchStockTransactionDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BatchStockTransactionDetailEntity batchStockTransactionDetailEntity = batchStockTransactionDetailService.create(new BatchStockTransactionDetailEntity(batchStockTransactionDetailEntityDto));
			return new ResponseEntity<>(batchStockTransactionDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Batch Stock Transaction Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param batchStockTransactionDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBatchStockTransactionDetail' endpoint here] off begin
	@Operation(
			description = "Update a BatchStockTransactionDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BatchStockTransactionDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BatchStockTransactionDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BatchStockTransactionDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBatchStockTransactionDetail' endpoint here] end
	public ResponseEntity<?> updateBatchStockTransactionDetail(
			@Parameter(description = "The BatchStockTransactionDetailEntity to create or update.")
			@RequestBody BatchStockTransactionDetailEntityDto batchStockTransactionDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BatchStockTransactionDetailEntity> checkBatchStockTransactionDetailExists = batchStockTransactionDetailService.findById(batchStockTransactionDetailEntityDto.getId());
		BatchStockTransactionDetailEntity batchStockTransactionDetailEntity = new BatchStockTransactionDetailEntity(batchStockTransactionDetailEntityDto);

		if (checkBatchStockTransactionDetailExists.isPresent()) {
			try {
				batchStockTransactionDetailEntity = batchStockTransactionDetailService.update(batchStockTransactionDetailEntity);
				return new ResponseEntity<>(batchStockTransactionDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(batchStockTransactionDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Batch Stock Transaction Detail that has the same id as the given id.
	 *
	 * @param id The id of the BatchStockTransactionDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single BatchStockTransactionDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BatchStockTransactionDetail to delete.")
		...
		batchStockTransactionDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBatchStockTransactionDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Batch Stock Transaction Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBatchStockTransactionDetail' endpoint here] end
	public void exportBatchStockTransactionDetail(
			@Parameter(description = "The ids of the BatchStockTransactionDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BatchStockTransactionDetailEntity> entitiesToExport = batchStockTransactionDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BatchStockTransactionDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BatchStockTransactionDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBatchStockTransactionDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBatchStockTransactionDetailExcludingIds' endpoint here] end
	public void exportBatchStockTransactionDetailExcludingIds(
			@Parameter(description = "The ids of the BatchStockTransactionDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BatchStockTransactionDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = batchStockTransactionDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = batchStockTransactionDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BatchStockTransactionDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BatchStockTransactionDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BatchStockTransactionDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Batch Stock Transaction Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BatchStockTransactionDetailEntity', 'create')  || hasPermission('BatchStockTransactionDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BatchStockTransactionDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BatchStockTransactionDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BatchStockTransactionDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BatchStockTransactionDetailEntity> csvToBean = new CsvToBeanBuilder<BatchStockTransactionDetailEntity>(reader)
				.withType(BatchStockTransactionDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BatchStockTransactionDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BatchStockTransactionDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Batch Stock Transaction Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Batch Stock Transaction Detail entity and you do not have permission");
					}
