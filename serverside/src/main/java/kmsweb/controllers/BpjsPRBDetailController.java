...
import kmsweb.entities.BpjsPRBDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS PRB Detail
 */
// % protected region % [Update the API route or description for the BPJS PRB DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPRBDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsprbdetail")
// % protected region % [Update the API route or description for the BPJS PRB DetailController here] end
public class BpjsPRBDetailController {

	private final BpjsPRBDetailService bpjsPRBDetailService;
	private final CsvUtils<BpjsPRBDetailEntity, BpjsPRBDetailRepository, BpjsPRBDetailEntityAudit,  BpjsPRBDetailService> csvUtils;
	...
	@Autowired
	public BpjsPRBDetailController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPRBDetailService, BpjsPRBDetailEntity.class);
	...
	public ResponseEntity<List<BpjsPRBDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPRBDetailEntity> bpjsPRBDetails = bpjsPRBDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPRBDetails, HttpStatus.OK);
	}

	/**
	 * Return the BPJS PRB Detail that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPRBDetailEntity to be returned
	 * @return the BPJS PRB Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPRBDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPRBDetail entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPRBDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPRBDetailEntity> getById(
			@Parameter(description = "The UUID of the BpjsPRBDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPRBDetailEntity> bpjsPRBDetailEntity = bpjsPRBDetailService.findById(id);
	...
	/**
	 * Create/Update the given BPJS PRB Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPRBDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPRBDetail' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPRBDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPRBDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPRBDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPRBDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPRBDetail' endpoint here] end
	public ResponseEntity<?> saveBpjsPRBDetail(
			@Parameter(description = "The BpjsPRBDetailEntity to create or update.")
			@RequestBody BpjsPRBDetailEntityDto bpjsPRBDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPRBDetailEntity bpjsPRBDetailEntity = bpjsPRBDetailService.create(new BpjsPRBDetailEntity(bpjsPRBDetailEntityDto));
			return new ResponseEntity<>(bpjsPRBDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS PRB Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPRBDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPRBDetail' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPRBDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPRBDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPRBDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPRBDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPRBDetail' endpoint here] end
	public ResponseEntity<?> updateBpjsPRBDetail(
			@Parameter(description = "The BpjsPRBDetailEntity to create or update.")
			@RequestBody BpjsPRBDetailEntityDto bpjsPRBDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPRBDetailEntity> checkBpjsPRBDetailExists = bpjsPRBDetailService.findById(bpjsPRBDetailEntityDto.getId());
		BpjsPRBDetailEntity bpjsPRBDetailEntity = new BpjsPRBDetailEntity(bpjsPRBDetailEntityDto);

		if (checkBpjsPRBDetailExists.isPresent()) {
			try {
				bpjsPRBDetailEntity = bpjsPRBDetailService.update(bpjsPRBDetailEntity);
				return new ResponseEntity<>(bpjsPRBDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPRBDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS PRB Detail that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPRBDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPRBDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPRBDetail to delete.")
		...
		bpjsPRBDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRBDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS PRB Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRBDetail' endpoint here] end
	public void exportBpjsPRBDetail(
			@Parameter(description = "The ids of the BpjsPRBDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPRBDetailEntity> entitiesToExport = bpjsPRBDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPRBDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPRBDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRBDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRBDetailExcludingIds' endpoint here] end
	public void exportBpjsPRBDetailExcludingIds(
			@Parameter(description = "The ids of the BpjsPRBDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPRBDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPRBDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPRBDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPRBDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPRBDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPRBDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS PRB Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPRBDetailEntity', 'create')  || hasPermission('BpjsPRBDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPRBDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPRBDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPRBDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPRBDetailEntity> csvToBean = new CsvToBeanBuilder<BpjsPRBDetailEntity>(reader)
				.withType(BpjsPRBDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPRBDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPRBDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS PRB Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS PRB Detail entity and you do not have permission");
					}
