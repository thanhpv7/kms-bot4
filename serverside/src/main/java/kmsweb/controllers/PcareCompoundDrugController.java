...
import kmsweb.entities.PcareCompoundDrugEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding PCare Compound Drug
 */
// % protected region % [Update the API route or description for the PCare Compound DrugController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareCompoundDrugController"))
@RestController
@Slf4j
@RequestMapping("/api/pcarecompounddrug")
// % protected region % [Update the API route or description for the PCare Compound DrugController here] end
public class PcareCompoundDrugController {

	private final PcareCompoundDrugService pcareCompoundDrugService;
	private final CsvUtils<PcareCompoundDrugEntity, PcareCompoundDrugRepository, PcareCompoundDrugEntityAudit,  PcareCompoundDrugService> csvUtils;
	...
	@Autowired
	public PcareCompoundDrugController(
		...
		this.csvUtils = new CsvUtils<>(pcareCompoundDrugService, PcareCompoundDrugEntity.class);
	...
	public ResponseEntity<List<PcareCompoundDrugEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareCompoundDrugEntity> pcareCompoundDrugs = pcareCompoundDrugService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareCompoundDrugs, HttpStatus.OK);
	}

	/**
	 * Return the PCare Compound Drug that has the same id as the given id.
	 *
	 * @param id      The id of the PcareCompoundDrugEntity to be returned
	 * @return the PCare Compound Drug that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareCompoundDrug as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareCompoundDrug entity not found"),
	})
	@PreAuthorize("hasPermission('PcareCompoundDrugEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareCompoundDrugEntity> getById(
			@Parameter(description = "The UUID of the PcareCompoundDrug to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareCompoundDrugEntity> pcareCompoundDrugEntity = pcareCompoundDrugService.findById(id);
	...
	/**
	 * Create/Update the given PCare Compound Drug. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareCompoundDrugEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareCompoundDrug' endpoint here] off begin
	@Operation(
			description = "Create a PcareCompoundDrug if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareCompoundDrug."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareCompoundDrug, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareCompoundDrugEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareCompoundDrug' endpoint here] end
	public ResponseEntity<?> savePcareCompoundDrug(
			@Parameter(description = "The PcareCompoundDrugEntity to create or update.")
			@RequestBody PcareCompoundDrugEntityDto pcareCompoundDrugEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareCompoundDrugEntity pcareCompoundDrugEntity = pcareCompoundDrugService.create(new PcareCompoundDrugEntity(pcareCompoundDrugEntityDto));
			return new ResponseEntity<>(pcareCompoundDrugEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Compound Drug. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareCompoundDrugEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareCompoundDrug' endpoint here] off begin
	@Operation(
			description = "Update a PcareCompoundDrug entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareCompoundDrug."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareCompoundDrug, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareCompoundDrugEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareCompoundDrug' endpoint here] end
	public ResponseEntity<?> updatePcareCompoundDrug(
			@Parameter(description = "The PcareCompoundDrugEntity to create or update.")
			@RequestBody PcareCompoundDrugEntityDto pcareCompoundDrugEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareCompoundDrugEntity> checkPcareCompoundDrugExists = pcareCompoundDrugService.findById(pcareCompoundDrugEntityDto.getId());
		PcareCompoundDrugEntity pcareCompoundDrugEntity = new PcareCompoundDrugEntity(pcareCompoundDrugEntityDto);

		if (checkPcareCompoundDrugExists.isPresent()) {
			try {
				pcareCompoundDrugEntity = pcareCompoundDrugService.update(pcareCompoundDrugEntity);
				return new ResponseEntity<>(pcareCompoundDrugEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareCompoundDrugEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Compound Drug that has the same id as the given id.
	 *
	 * @param id The id of the PcareCompoundDrugEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareCompoundDrug as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareCompoundDrug to delete.")
		...
		pcareCompoundDrugService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrug' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Compound Drug entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrug' endpoint here] end
	public void exportPcareCompoundDrug(
			@Parameter(description = "The ids of the PcareCompoundDrugEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareCompoundDrugEntity> entitiesToExport = pcareCompoundDrugService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareCompoundDrug_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareCompoundDrugEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrugExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareCompoundDrugExcludingIds' endpoint here] end
	public void exportPcareCompoundDrugExcludingIds(
			@Parameter(description = "The ids of the PcareCompoundDrugEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareCompoundDrugEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareCompoundDrugService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareCompoundDrugService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareCompoundDrug entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareCompoundDrug_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareCompoundDrugEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Compound Drug entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareCompoundDrugEntity', 'create')  || hasPermission('PcareCompoundDrugEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareCompoundDrugEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareCompoundDrugEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareCompoundDrugEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareCompoundDrugEntity> csvToBean = new CsvToBeanBuilder<PcareCompoundDrugEntity>(reader)
				.withType(PcareCompoundDrugEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareCompoundDrugEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareCompoundDrugEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Compound Drug entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Compound Drug entity and you do not have permission");
					}
