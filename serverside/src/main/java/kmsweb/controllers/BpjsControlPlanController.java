...
import kmsweb.entities.BpjsControlPlanEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Control Plan
 */
// % protected region % [Update the API route or description for the BPJS Control PlanController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsControlPlanController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjscontrolplan")
// % protected region % [Update the API route or description for the BPJS Control PlanController here] end
public class BpjsControlPlanController {

	private final BpjsControlPlanService bpjsControlPlanService;
	private final CsvUtils<BpjsControlPlanEntity, BpjsControlPlanRepository, BpjsControlPlanEntityAudit,  BpjsControlPlanService> csvUtils;
	...
	@Autowired
	public BpjsControlPlanController(
		...
		this.csvUtils = new CsvUtils<>(bpjsControlPlanService, BpjsControlPlanEntity.class);
	...
	public ResponseEntity<List<BpjsControlPlanEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsControlPlanEntity> bpjsControlPlans = bpjsControlPlanService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsControlPlans, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Control Plan that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsControlPlanEntity to be returned
	 * @return the BPJS Control Plan that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsControlPlan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsControlPlan entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsControlPlanEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsControlPlanEntity> getById(
			@Parameter(description = "The UUID of the BpjsControlPlan to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsControlPlanEntity> bpjsControlPlanEntity = bpjsControlPlanService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Control Plan. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsControlPlanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsControlPlan' endpoint here] off begin
	@Operation(
			description = "Create a BpjsControlPlan if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsControlPlan."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsControlPlan, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsControlPlanEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsControlPlan' endpoint here] end
	public ResponseEntity<?> saveBpjsControlPlan(
			@Parameter(description = "The BpjsControlPlanEntity to create or update.")
			@RequestBody BpjsControlPlanEntityDto bpjsControlPlanEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsControlPlanEntity bpjsControlPlanEntity = bpjsControlPlanService.create(new BpjsControlPlanEntity(bpjsControlPlanEntityDto));
			return new ResponseEntity<>(bpjsControlPlanEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Control Plan. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsControlPlanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsControlPlan' endpoint here] off begin
	@Operation(
			description = "Update a BpjsControlPlan entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsControlPlan."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsControlPlan, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsControlPlanEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsControlPlan' endpoint here] end
	public ResponseEntity<?> updateBpjsControlPlan(
			@Parameter(description = "The BpjsControlPlanEntity to create or update.")
			@RequestBody BpjsControlPlanEntityDto bpjsControlPlanEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsControlPlanEntity> checkBpjsControlPlanExists = bpjsControlPlanService.findById(bpjsControlPlanEntityDto.getId());
		BpjsControlPlanEntity bpjsControlPlanEntity = new BpjsControlPlanEntity(bpjsControlPlanEntityDto);

		if (checkBpjsControlPlanExists.isPresent()) {
			try {
				bpjsControlPlanEntity = bpjsControlPlanService.update(bpjsControlPlanEntity);
				return new ResponseEntity<>(bpjsControlPlanEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsControlPlanEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Control Plan that has the same id as the given id.
	 *
	 * @param id The id of the BpjsControlPlanEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsControlPlan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsControlPlan to delete.")
		...
		bpjsControlPlanService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlPlan' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Control Plan entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlPlan' endpoint here] end
	public void exportBpjsControlPlan(
			@Parameter(description = "The ids of the BpjsControlPlanEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsControlPlanEntity> entitiesToExport = bpjsControlPlanService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsControlPlan_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsControlPlanEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlPlanExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsControlPlanExcludingIds' endpoint here] end
	public void exportBpjsControlPlanExcludingIds(
			@Parameter(description = "The ids of the BpjsControlPlanEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsControlPlanEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsControlPlanService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsControlPlanService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsControlPlan entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsControlPlan_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsControlPlanEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Control Plan entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsControlPlanEntity', 'create')  || hasPermission('BpjsControlPlanEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsControlPlanEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsControlPlanEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsControlPlanEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsControlPlanEntity> csvToBean = new CsvToBeanBuilder<BpjsControlPlanEntity>(reader)
				.withType(BpjsControlPlanEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsControlPlanEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsControlPlanEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Control Plan entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Control Plan entity and you do not have permission");
					}
