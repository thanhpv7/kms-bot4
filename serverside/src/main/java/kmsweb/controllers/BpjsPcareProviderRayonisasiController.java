...
import kmsweb.dtos.BpjsPcareProviderRayonisasiEntityDto;
import kmsweb.entities.BpjsPcareProviderRayonisasiEntityAudit;
import kmsweb.repositories.BpjsPcareProviderRayonisasiRepository;
...
/**
 * Controller used to handle all REST operations regarding BPJS Pcare Provider Rayonisasi
 */
// % protected region % [Update the API route or description for the BPJS Pcare Provider RayonisasiController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcareProviderRayonisasiController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcareproviderrayonisasi")
// % protected region % [Update the API route or description for the BPJS Pcare Provider RayonisasiController here] end
public class BpjsPcareProviderRayonisasiController {

	private final BpjsPcareProviderRayonisasiService bpjsPcareProviderRayonisasiService;
	private final CsvUtils<BpjsPcareProviderRayonisasiEntity, BpjsPcareProviderRayonisasiRepository, BpjsPcareProviderRayonisasiEntityAudit,  BpjsPcareProviderRayonisasiService> csvUtils;
	...
	@Autowired
	public BpjsPcareProviderRayonisasiController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcareProviderRayonisasiService, BpjsPcareProviderRayonisasiEntity.class);
	...
	/**
	 * Return all the BPJS Pcare Provider Rayonisasis.
	 *
	 * @return all the BPJS Pcare Provider Rayonisasis
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsPcareProviderRayonisasis",
		security = {
	...
	@PreAuthorize("hasPermission('BpjsPcareProviderRayonisasiEntity', 'read')")
	@GetMapping(produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] end
	public ResponseEntity<List<BpjsPcareProviderRayonisasiEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcareProviderRayonisasiEntity> bpjsPcareProviderRayonisasis = bpjsPcareProviderRayonisasiService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcareProviderRayonisasis, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Pcare Provider Rayonisasi that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcareProviderRayonisasiEntity to be returned
	 * @return the BPJS Pcare Provider Rayonisasi that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcareProviderRayonisasi as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcareProviderRayonisasi entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcareProviderRayonisasiEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcareProviderRayonisasiEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcareProviderRayonisasi to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcareProviderRayonisasiEntity> bpjsPcareProviderRayonisasiEntity = bpjsPcareProviderRayonisasiService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Pcare Provider Rayonisasi. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareProviderRayonisasiEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareProviderRayonisasi' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcareProviderRayonisasi if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcareProviderRayonisasi."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcareProviderRayonisasi, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcareProviderRayonisasiEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareProviderRayonisasi' endpoint here] end
	public ResponseEntity<?> saveBpjsPcareProviderRayonisasi(
			@Parameter(description = "The BpjsPcareProviderRayonisasiEntity to create or update.")
			@RequestBody BpjsPcareProviderRayonisasiEntityDto bpjsPcareProviderRayonisasiEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcareProviderRayonisasiEntity bpjsPcareProviderRayonisasiEntity = bpjsPcareProviderRayonisasiService.create(new BpjsPcareProviderRayonisasiEntity(bpjsPcareProviderRayonisasiEntityDto));
			return new ResponseEntity<>(bpjsPcareProviderRayonisasiEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Pcare Provider Rayonisasi. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareProviderRayonisasiEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareProviderRayonisasi' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcareProviderRayonisasi entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcareProviderRayonisasi."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcareProviderRayonisasi, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcareProviderRayonisasiEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareProviderRayonisasi' endpoint here] end
	public ResponseEntity<?> updateBpjsPcareProviderRayonisasi(
			@Parameter(description = "The BpjsPcareProviderRayonisasiEntity to create or update.")
			@RequestBody BpjsPcareProviderRayonisasiEntityDto bpjsPcareProviderRayonisasiEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcareProviderRayonisasiEntity> checkBpjsPcareProviderRayonisasiExists = bpjsPcareProviderRayonisasiService.findById(bpjsPcareProviderRayonisasiEntityDto.getId());
		BpjsPcareProviderRayonisasiEntity bpjsPcareProviderRayonisasiEntity = new BpjsPcareProviderRayonisasiEntity(bpjsPcareProviderRayonisasiEntityDto);

		if (checkBpjsPcareProviderRayonisasiExists.isPresent()) {
			try {
				bpjsPcareProviderRayonisasiEntity = bpjsPcareProviderRayonisasiService.update(bpjsPcareProviderRayonisasiEntity);
				return new ResponseEntity<>(bpjsPcareProviderRayonisasiEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcareProviderRayonisasiEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Pcare Provider Rayonisasi that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcareProviderRayonisasiEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcareProviderRayonisasi as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcareProviderRayonisasi to delete.")
		...
		bpjsPcareProviderRayonisasiService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareProviderRayonisasi' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Pcare Provider Rayonisasi entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareProviderRayonisasi' endpoint here] end
	public void exportBpjsPcareProviderRayonisasi(
			@Parameter(description = "The ids of the BpjsPcareProviderRayonisasiEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcareProviderRayonisasiEntity> entitiesToExport = bpjsPcareProviderRayonisasiService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcareProviderRayonisasi_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcareProviderRayonisasiEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareProviderRayonisasiExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareProviderRayonisasiExcludingIds' endpoint here] end
	public void exportBpjsPcareProviderRayonisasiExcludingIds(
			@Parameter(description = "The ids of the BpjsPcareProviderRayonisasiEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcareProviderRayonisasiEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcareProviderRayonisasiService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcareProviderRayonisasiService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcareProviderRayonisasi entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcareProviderRayonisasi_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcareProviderRayonisasiEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Pcare Provider Rayonisasi entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcareProviderRayonisasiEntity', 'create')  || hasPermission('BpjsPcareProviderRayonisasiEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcareProviderRayonisasiEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcareProviderRayonisasiEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcareProviderRayonisasiEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcareProviderRayonisasiEntity> csvToBean = new CsvToBeanBuilder<BpjsPcareProviderRayonisasiEntity>(reader)
				.withType(BpjsPcareProviderRayonisasiEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcareProviderRayonisasiEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcareProviderRayonisasiEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Pcare Provider Rayonisasi entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Pcare Provider Rayonisasi entity and you do not have permission");
					}
