...
import kmsweb.entities.RoomFacilityEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Room Facility
 */
// % protected region % [Update the API route or description for the Room FacilityController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RoomFacilityController"))
@RestController
@Slf4j
@RequestMapping("/api/roomfacility")
// % protected region % [Update the API route or description for the Room FacilityController here] end
public class RoomFacilityController {

	private final RoomFacilityService roomFacilityService;
	private final CsvUtils<RoomFacilityEntity, RoomFacilityRepository, RoomFacilityEntityAudit,  RoomFacilityService> csvUtils;
	...
	public RoomFacilityController(
		...
		this.csvUtils = new CsvUtils<>(roomFacilityService, RoomFacilityEntity.class);
	...
	public ResponseEntity<List<RoomFacilityEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<RoomFacilityEntity> roomFacilitys = roomFacilityService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(roomFacilitys, HttpStatus.OK);
	}

	/**
	 * Return the Room Facility that has the same id as the given id.
	 *
	 * @param id      The id of the RoomFacilityEntity to be returned
	 * @return the Room Facility that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single RoomFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "RoomFacility entity not found"),
	})
	@PreAuthorize("hasPermission('RoomFacilityEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<RoomFacilityEntity> getById(
			@Parameter(description = "The UUID of the RoomFacility to return.")
			@PathVariable("id") UUID id
	) {
		Optional<RoomFacilityEntity> roomFacilityEntity = roomFacilityService.findById(id);
	...
	/**
	 * Create/Update the given Room Facility. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param roomFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRoomFacility' endpoint here] off begin
	@Operation(
			description = "Create a RoomFacility if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new RoomFacility."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new RoomFacility, duplicate record found")
	})
	@PreAuthorize("hasPermission('RoomFacilityEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRoomFacility' endpoint here] end
	public ResponseEntity<?> saveRoomFacility(
			@Parameter(description = "The RoomFacilityEntity to create or update.")
			@RequestBody RoomFacilityEntityDto roomFacilityEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RoomFacilityEntity roomFacilityEntity = roomFacilityService.create(new RoomFacilityEntity(roomFacilityEntityDto));
			return new ResponseEntity<>(roomFacilityEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Room Facility. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param roomFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRoomFacility' endpoint here] off begin
	@Operation(
			description = "Update a RoomFacility entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the RoomFacility."),
		@ApiResponse(responseCode = "400", description = "Failed to update the RoomFacility, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RoomFacilityEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRoomFacility' endpoint here] end
	public ResponseEntity<?> updateRoomFacility(
			@Parameter(description = "The RoomFacilityEntity to create or update.")
			@RequestBody RoomFacilityEntityDto roomFacilityEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<RoomFacilityEntity> checkRoomFacilityExists = roomFacilityService.findById(roomFacilityEntityDto.getId());
		RoomFacilityEntity roomFacilityEntity = new RoomFacilityEntity(roomFacilityEntityDto);

		if (checkRoomFacilityExists.isPresent()) {
			try {
				roomFacilityEntity = roomFacilityService.update(roomFacilityEntity);
				return new ResponseEntity<>(roomFacilityEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(roomFacilityEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Room Facility that has the same id as the given id.
	 *
	 * @param id The id of the RoomFacilityEntity to be deleted
	...
	@Operation(
			description = "Delete a single RoomFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the RoomFacility to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRoomFacility' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Room Facility entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRoomFacility' endpoint here] end
	public void exportRoomFacility(
			@Parameter(description = "The ids of the RoomFacilityEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RoomFacilityEntity> entitiesToExport = roomFacilityService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "RoomFacility_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RoomFacilityEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRoomFacilityExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRoomFacilityExcludingIds' endpoint here] end
	public void exportRoomFacilityExcludingIds(
			@Parameter(description = "The ids of the RoomFacilityEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RoomFacilityEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = roomFacilityService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = roomFacilityService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the RoomFacility entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "RoomFacility_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RoomFacilityEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Room Facility entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RoomFacilityEntity', 'create')  || hasPermission('RoomFacilityEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RoomFacilityEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RoomFacilityEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RoomFacilityEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RoomFacilityEntity> csvToBean = new CsvToBeanBuilder<RoomFacilityEntity>(reader)
				.withType(RoomFacilityEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<RoomFacilityEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<RoomFacilityEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Room Facility entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Room Facility entity and you do not have permission");
					}
