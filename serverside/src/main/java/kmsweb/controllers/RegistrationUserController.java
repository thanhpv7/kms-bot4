...
import kmsweb.dtos.RegistrationUserEntityDto;
import kmsweb.entities.RegistrationUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Registration User
 */
// % protected region % [Update the API route or description for the Registration UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RegistrationUserController"))
@RestController
@Slf4j
@RequestMapping("/api/registrationuser")
// % protected region % [Update the API route or description for the Registration UserController here] end
public class RegistrationUserController {

	private final RegistrationUserService registrationUserService;
	private final CsvUtils<RegistrationUserEntity, RegistrationUserRepository, RegistrationUserEntityAudit,  RegistrationUserService> csvUtils;
	...
	public RegistrationUserController(
		...
		this.csvUtils = new CsvUtils<>(registrationUserService, RegistrationUserEntity.class);
	...
	public ResponseEntity<List<RegistrationUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<RegistrationUserEntity> registrationUsers = registrationUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Registration User that has the same id as the given id.
	 *
	 * @param id      The id of the RegistrationUserEntity to be returned
	 * @return the Registration User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single RegistrationUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "RegistrationUser entity not found"),
	})
	@PreAuthorize("hasPermission('RegistrationUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<RegistrationUserEntity> getById(
			@Parameter(description = "The UUID of the RegistrationUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<RegistrationUserEntity> registrationUserEntity = registrationUserService.findById(id);
	...
	/**
	 * Create/Update the given Registration User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param registrationUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRegistrationUser' endpoint here] off begin
	@Operation(
			description = "Create a RegistrationUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new RegistrationUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new RegistrationUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('RegistrationUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRegistrationUser' endpoint here] end
	public ResponseEntity<?> saveRegistrationUser(
			@Parameter(description = "The RegistrationUserEntity to create or update.")
			@RequestBody RegistrationUserEntityDto registrationUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RegistrationUserEntity registrationUserEntity = registrationUserService.create(new RegistrationUserEntity(registrationUserEntityDto));
			return new ResponseEntity<>(registrationUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Registration User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param registrationUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRegistrationUser' endpoint here] off begin
	@Operation(
			description = "Update a RegistrationUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the RegistrationUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the RegistrationUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RegistrationUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRegistrationUser' endpoint here] end
	public ResponseEntity<?> updateRegistrationUser(
			@Parameter(description = "The RegistrationUserEntity to create or update.")
			@RequestBody RegistrationUserEntityDto registrationUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<RegistrationUserEntity> checkRegistrationUserExists = registrationUserService.findById(registrationUserEntityDto.getId());
		RegistrationUserEntity registrationUserEntity = new RegistrationUserEntity(registrationUserEntityDto);

		if (checkRegistrationUserExists.isPresent()) {
			try {
				registrationUserEntity = registrationUserService.update(registrationUserEntity);
				return new ResponseEntity<>(registrationUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(registrationUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Registration User that has the same id as the given id.
	 *
	 * @param id The id of the RegistrationUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single RegistrationUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('RegistrationUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the RegistrationUser to delete.")
		...
		registrationUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRegistrationUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Registration User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRegistrationUser' endpoint here] end
	public void exportRegistrationUser(
			@Parameter(description = "The ids of the RegistrationUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RegistrationUserEntity> entitiesToExport = registrationUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "RegistrationUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RegistrationUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRegistrationUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRegistrationUserExcludingIds' endpoint here] end
	public void exportRegistrationUserExcludingIds(
			@Parameter(description = "The ids of the RegistrationUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RegistrationUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = registrationUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = registrationUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the RegistrationUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "RegistrationUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RegistrationUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Registration User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RegistrationUserEntity', 'create')  || hasPermission('RegistrationUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RegistrationUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RegistrationUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RegistrationUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RegistrationUserEntity> csvToBean = new CsvToBeanBuilder<RegistrationUserEntity>(reader)
				.withType(RegistrationUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<RegistrationUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<RegistrationUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Registration User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Registration User entity and you do not have permission");
					}
