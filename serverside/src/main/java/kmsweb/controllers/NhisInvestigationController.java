...
import kmsweb.entities.NhisInvestigationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding NHIS Investigation
 */
// % protected region % [Update the API route or description for the NHIS InvestigationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisInvestigationController"))
@RestController
@Slf4j
@RequestMapping("/api/nhisinvestigation")
// % protected region % [Update the API route or description for the NHIS InvestigationController here] end
public class NhisInvestigationController {

	private final NhisInvestigationService nhisInvestigationService;
	private final CsvUtils<NhisInvestigationEntity, NhisInvestigationRepository, NhisInvestigationEntityAudit,  NhisInvestigationService> csvUtils;
	...
	@Autowired
	public NhisInvestigationController(
		...
		this.csvUtils = new CsvUtils<>(nhisInvestigationService, NhisInvestigationEntity.class);
	...
	public ResponseEntity<List<NhisInvestigationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisInvestigationEntity> nhisInvestigations = nhisInvestigationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisInvestigations, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Investigation that has the same id as the given id.
	 *
	 * @param id      The id of the NhisInvestigationEntity to be returned
	 * @return the NHIS Investigation that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisInvestigation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisInvestigation entity not found"),
	})
	@PreAuthorize("hasPermission('NhisInvestigationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisInvestigationEntity> getById(
			@Parameter(description = "The UUID of the NhisInvestigation to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisInvestigationEntity> nhisInvestigationEntity = nhisInvestigationService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Investigation. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisInvestigationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisInvestigation' endpoint here] off begin
	@Operation(
			description = "Create a NhisInvestigation if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisInvestigation."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisInvestigation, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisInvestigationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisInvestigation' endpoint here] end
	public ResponseEntity<?> saveNhisInvestigation(
			@Parameter(description = "The NhisInvestigationEntity to create or update.")
			@RequestBody NhisInvestigationEntityDto nhisInvestigationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisInvestigationEntity nhisInvestigationEntity = nhisInvestigationService.create(new NhisInvestigationEntity(nhisInvestigationEntityDto));
			return new ResponseEntity<>(nhisInvestigationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Investigation. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisInvestigationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisInvestigation' endpoint here] off begin
	@Operation(
			description = "Update a NhisInvestigation entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisInvestigation."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisInvestigation, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisInvestigationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisInvestigation' endpoint here] end
	public ResponseEntity<?> updateNhisInvestigation(
			@Parameter(description = "The NhisInvestigationEntity to create or update.")
			@RequestBody NhisInvestigationEntityDto nhisInvestigationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisInvestigationEntity> checkNhisInvestigationExists = nhisInvestigationService.findById(nhisInvestigationEntityDto.getId());
		NhisInvestigationEntity nhisInvestigationEntity = new NhisInvestigationEntity(nhisInvestigationEntityDto);

		if (checkNhisInvestigationExists.isPresent()) {
			try {
				nhisInvestigationEntity = nhisInvestigationService.update(nhisInvestigationEntity);
				return new ResponseEntity<>(nhisInvestigationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisInvestigationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Investigation that has the same id as the given id.
	 *
	 * @param id The id of the NhisInvestigationEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisInvestigation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisInvestigation to delete.")
		...
		nhisInvestigationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisInvestigation' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Investigation entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisInvestigation' endpoint here] end
	public void exportNhisInvestigation(
			@Parameter(description = "The ids of the NhisInvestigationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisInvestigationEntity> entitiesToExport = nhisInvestigationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisInvestigation_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisInvestigationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisInvestigationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisInvestigationExcludingIds' endpoint here] end
	public void exportNhisInvestigationExcludingIds(
			@Parameter(description = "The ids of the NhisInvestigationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisInvestigationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisInvestigationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisInvestigationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisInvestigation entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisInvestigation_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisInvestigationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Investigation entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisInvestigationEntity', 'create')  || hasPermission('NhisInvestigationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisInvestigationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisInvestigationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisInvestigationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisInvestigationEntity> csvToBean = new CsvToBeanBuilder<NhisInvestigationEntity>(reader)
				.withType(NhisInvestigationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisInvestigationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisInvestigationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Investigation entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Investigation entity and you do not have permission");
					}
