...
/**
 * Controller used to handle all REST operations regarding BPJS Treatment History
 */
// % protected region % [Update the API route or description for the BPJS Treatment HistoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsTreatmentHistoryController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjstreatmenthistory")
// % protected region % [Update the API route or description for the BPJS Treatment HistoryController here] end
public class BpjsTreatmentHistoryController {

	private final BpjsTreatmentHistoryService bpjsTreatmentHistoryService;
	private final CsvUtils<BpjsTreatmentHistoryEntity, BpjsTreatmentHistoryRepository, BpjsTreatmentHistoryEntityAudit,  BpjsTreatmentHistoryService> csvUtils;
	...
	@Autowired
	public BpjsTreatmentHistoryController(
		...
		this.csvUtils = new CsvUtils<>(bpjsTreatmentHistoryService, BpjsTreatmentHistoryEntity.class);
	...
	public ResponseEntity<List<BpjsTreatmentHistoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsTreatmentHistoryEntity> bpjsTreatmentHistorys = bpjsTreatmentHistoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsTreatmentHistorys, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Treatment History that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsTreatmentHistoryEntity to be returned
	 * @return the BPJS Treatment History that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsTreatmentHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsTreatmentHistory entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsTreatmentHistoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsTreatmentHistoryEntity> getById(
			@Parameter(description = "The UUID of the BpjsTreatmentHistory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsTreatmentHistoryEntity> bpjsTreatmentHistoryEntity = bpjsTreatmentHistoryService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Treatment History. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTreatmentHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsTreatmentHistory' endpoint here] off begin
	@Operation(
			description = "Create a BpjsTreatmentHistory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsTreatmentHistory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsTreatmentHistory, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsTreatmentHistoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsTreatmentHistory' endpoint here] end
	public ResponseEntity<?> saveBpjsTreatmentHistory(
			@Parameter(description = "The BpjsTreatmentHistoryEntity to create or update.")
			@RequestBody BpjsTreatmentHistoryEntityDto bpjsTreatmentHistoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsTreatmentHistoryEntity bpjsTreatmentHistoryEntity = bpjsTreatmentHistoryService.create(new BpjsTreatmentHistoryEntity(bpjsTreatmentHistoryEntityDto));
			return new ResponseEntity<>(bpjsTreatmentHistoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Treatment History. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTreatmentHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsTreatmentHistory' endpoint here] off begin
	@Operation(
			description = "Update a BpjsTreatmentHistory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsTreatmentHistory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsTreatmentHistory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsTreatmentHistoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsTreatmentHistory' endpoint here] end
	public ResponseEntity<?> updateBpjsTreatmentHistory(
			@Parameter(description = "The BpjsTreatmentHistoryEntity to create or update.")
			@RequestBody BpjsTreatmentHistoryEntityDto bpjsTreatmentHistoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsTreatmentHistoryEntity> checkBpjsTreatmentHistoryExists = bpjsTreatmentHistoryService.findById(bpjsTreatmentHistoryEntityDto.getId());
		BpjsTreatmentHistoryEntity bpjsTreatmentHistoryEntity = new BpjsTreatmentHistoryEntity(bpjsTreatmentHistoryEntityDto);

		if (checkBpjsTreatmentHistoryExists.isPresent()) {
			try {
				bpjsTreatmentHistoryEntity = bpjsTreatmentHistoryService.update(bpjsTreatmentHistoryEntity);
				return new ResponseEntity<>(bpjsTreatmentHistoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsTreatmentHistoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Treatment History that has the same id as the given id.
	 *
	 * @param id The id of the BpjsTreatmentHistoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsTreatmentHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsTreatmentHistory to delete.")
		...
		bpjsTreatmentHistoryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentHistory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Treatment History entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentHistory' endpoint here] end
	public void exportBpjsTreatmentHistory(
			@Parameter(description = "The ids of the BpjsTreatmentHistoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsTreatmentHistoryEntity> entitiesToExport = bpjsTreatmentHistoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsTreatmentHistory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsTreatmentHistoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentHistoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentHistoryExcludingIds' endpoint here] end
	public void exportBpjsTreatmentHistoryExcludingIds(
			@Parameter(description = "The ids of the BpjsTreatmentHistoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsTreatmentHistoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsTreatmentHistoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsTreatmentHistoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsTreatmentHistory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsTreatmentHistory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsTreatmentHistoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Treatment History entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsTreatmentHistoryEntity', 'create')  || hasPermission('BpjsTreatmentHistoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsTreatmentHistoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsTreatmentHistoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsTreatmentHistoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsTreatmentHistoryEntity> csvToBean = new CsvToBeanBuilder<BpjsTreatmentHistoryEntity>(reader)
				.withType(BpjsTreatmentHistoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsTreatmentHistoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsTreatmentHistoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Treatment History entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Treatment History entity and you do not have permission");
					}
