...
import kmsweb.entities.MedicationItemEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medication Item
 */
// % protected region % [Update the API route or description for the Medication ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicationItemController"))
@RestController
@Slf4j
@RequestMapping("/api/medicationitem")
// % protected region % [Update the API route or description for the Medication ItemController here] end
public class MedicationItemController {

	private final MedicationItemService medicationItemService;
	private final CsvUtils<MedicationItemEntity, MedicationItemRepository, MedicationItemEntityAudit,  MedicationItemService> csvUtils;
	...
	@Autowired
	public MedicationItemController(
		...
		this.csvUtils = new CsvUtils<>(medicationItemService, MedicationItemEntity.class);
	...
	public ResponseEntity<List<MedicationItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicationItemEntity> medicationItems = medicationItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicationItems, HttpStatus.OK);
	}

	/**
	 * Return the Medication Item that has the same id as the given id.
	 *
	 * @param id      The id of the MedicationItemEntity to be returned
	 * @return the Medication Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicationItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicationItem entity not found"),
	})
	@PreAuthorize("hasPermission('MedicationItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicationItemEntity> getById(
			@Parameter(description = "The UUID of the MedicationItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicationItemEntity> medicationItemEntity = medicationItemService.findById(id);
	...
	/**
	 * Create/Update the given Medication Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicationItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicationItem' endpoint here] off begin
	@Operation(
			description = "Create a MedicationItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicationItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicationItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicationItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicationItem' endpoint here] end
	public ResponseEntity<?> saveMedicationItem(
			@Parameter(description = "The MedicationItemEntity to create or update.")
			@RequestBody MedicationItemEntityDto medicationItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicationItemEntity medicationItemEntity = medicationItemService.create(new MedicationItemEntity(medicationItemEntityDto));
			return new ResponseEntity<>(medicationItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medication Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicationItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicationItem' endpoint here] off begin
	@Operation(
			description = "Update a MedicationItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicationItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicationItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicationItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicationItem' endpoint here] end
	public ResponseEntity<?> updateMedicationItem(
			@Parameter(description = "The MedicationItemEntity to create or update.")
			@RequestBody MedicationItemEntityDto medicationItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicationItemEntity> checkMedicationItemExists = medicationItemService.findById(medicationItemEntityDto.getId());
		MedicationItemEntity medicationItemEntity = new MedicationItemEntity(medicationItemEntityDto);

		if (checkMedicationItemExists.isPresent()) {
			try {
				medicationItemEntity = medicationItemService.update(medicationItemEntity);
				return new ResponseEntity<>(medicationItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicationItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medication Item that has the same id as the given id.
	 *
	 * @param id The id of the MedicationItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicationItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicationItem to delete.")
		...
		medicationItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medication Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationItem' endpoint here] end
	public void exportMedicationItem(
			@Parameter(description = "The ids of the MedicationItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicationItemEntity> entitiesToExport = medicationItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicationItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicationItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicationItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicationItemExcludingIds' endpoint here] end
	public void exportMedicationItemExcludingIds(
			@Parameter(description = "The ids of the MedicationItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicationItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicationItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicationItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicationItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicationItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicationItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medication Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicationItemEntity', 'create')  || hasPermission('MedicationItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicationItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicationItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicationItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicationItemEntity> csvToBean = new CsvToBeanBuilder<MedicationItemEntity>(reader)
				.withType(MedicationItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicationItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicationItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medication Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medication Item entity and you do not have permission");
					}
