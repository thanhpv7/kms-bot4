...
/**
 * Controller used to handle all REST operations regarding BPJS Finger Print
 */
// % protected region % [Update the API route or description for the BPJS Finger PrintController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsFingerPrintController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsfingerprint")
// % protected region % [Update the API route or description for the BPJS Finger PrintController here] end
public class BpjsFingerPrintController {

	private final BpjsFingerPrintService bpjsFingerPrintService;
	private final CsvUtils<BpjsFingerPrintEntity, BpjsFingerPrintRepository, BpjsFingerPrintEntityAudit,  BpjsFingerPrintService> csvUtils;
	...
	@Autowired
	public BpjsFingerPrintController(
		...
		this.csvUtils = new CsvUtils<>(bpjsFingerPrintService, BpjsFingerPrintEntity.class);
	...
	public ResponseEntity<List<BpjsFingerPrintEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsFingerPrintEntity> bpjsFingerPrints = bpjsFingerPrintService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsFingerPrints, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Finger Print that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsFingerPrintEntity to be returned
	 * @return the BPJS Finger Print that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsFingerPrint as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsFingerPrint entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsFingerPrintEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsFingerPrintEntity> getById(
			@Parameter(description = "The UUID of the BpjsFingerPrint to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsFingerPrintEntity> bpjsFingerPrintEntity = bpjsFingerPrintService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Finger Print. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsFingerPrintEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsFingerPrint' endpoint here] off begin
	@Operation(
			description = "Create a BpjsFingerPrint if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsFingerPrint."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsFingerPrint, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsFingerPrintEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsFingerPrint' endpoint here] end
	public ResponseEntity<?> saveBpjsFingerPrint(
			@Parameter(description = "The BpjsFingerPrintEntity to create or update.")
			@RequestBody BpjsFingerPrintEntityDto bpjsFingerPrintEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsFingerPrintEntity bpjsFingerPrintEntity = bpjsFingerPrintService.create(new BpjsFingerPrintEntity(bpjsFingerPrintEntityDto));
			return new ResponseEntity<>(bpjsFingerPrintEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Finger Print. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsFingerPrintEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsFingerPrint' endpoint here] off begin
	@Operation(
			description = "Update a BpjsFingerPrint entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsFingerPrint."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsFingerPrint, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsFingerPrintEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsFingerPrint' endpoint here] end
	public ResponseEntity<?> updateBpjsFingerPrint(
			@Parameter(description = "The BpjsFingerPrintEntity to create or update.")
			@RequestBody BpjsFingerPrintEntityDto bpjsFingerPrintEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsFingerPrintEntity> checkBpjsFingerPrintExists = bpjsFingerPrintService.findById(bpjsFingerPrintEntityDto.getId());
		BpjsFingerPrintEntity bpjsFingerPrintEntity = new BpjsFingerPrintEntity(bpjsFingerPrintEntityDto);

		if (checkBpjsFingerPrintExists.isPresent()) {
			try {
				bpjsFingerPrintEntity = bpjsFingerPrintService.update(bpjsFingerPrintEntity);
				return new ResponseEntity<>(bpjsFingerPrintEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsFingerPrintEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Finger Print that has the same id as the given id.
	 *
	 * @param id The id of the BpjsFingerPrintEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsFingerPrint as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsFingerPrint to delete.")
		...
		bpjsFingerPrintService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsFingerPrint' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Finger Print entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsFingerPrint' endpoint here] end
	public void exportBpjsFingerPrint(
			@Parameter(description = "The ids of the BpjsFingerPrintEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsFingerPrintEntity> entitiesToExport = bpjsFingerPrintService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsFingerPrint_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsFingerPrintEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsFingerPrintExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsFingerPrintExcludingIds' endpoint here] end
	public void exportBpjsFingerPrintExcludingIds(
			@Parameter(description = "The ids of the BpjsFingerPrintEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsFingerPrintEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsFingerPrintService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsFingerPrintService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsFingerPrint entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsFingerPrint_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsFingerPrintEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Finger Print entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsFingerPrintEntity', 'create')  || hasPermission('BpjsFingerPrintEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsFingerPrintEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsFingerPrintEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsFingerPrintEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsFingerPrintEntity> csvToBean = new CsvToBeanBuilder<BpjsFingerPrintEntity>(reader)
				.withType(BpjsFingerPrintEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsFingerPrintEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsFingerPrintEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Finger Print entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Finger Print entity and you do not have permission");
					}
