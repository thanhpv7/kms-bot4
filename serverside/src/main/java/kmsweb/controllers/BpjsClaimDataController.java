...
import kmsweb.entities.BpjsClaimDataEntityAudit;
import kmsweb.repositories.BpjsClaimDataRepository;
import kmsweb.services.BpjsClaimDataService;
...
/**
 * Controller used to handle all REST operations regarding BPJS Claim Data
 */
// % protected region % [Update the API route or description for the BPJS Claim DataController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsClaimDataController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsclaimdata")
// % protected region % [Update the API route or description for the BPJS Claim DataController here] end
public class BpjsClaimDataController {

	private final BpjsClaimDataService bpjsClaimDataService;
	private final CsvUtils<BpjsClaimDataEntity, BpjsClaimDataRepository, BpjsClaimDataEntityAudit,  BpjsClaimDataService> csvUtils;
	...
	@Autowired
	public BpjsClaimDataController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsClaimDataService bpjsClaimDataService
	) {
		this.authService = authService;
		this.bpjsClaimDataService = bpjsClaimDataService;
		this.csvUtils = new CsvUtils<>(bpjsClaimDataService, BpjsClaimDataEntity.class);
	...
	public ResponseEntity<List<BpjsClaimDataEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsClaimDataEntity> bpjsClaimDatas = bpjsClaimDataService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsClaimDatas, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Claim Data that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsClaimDataEntity to be returned
	 * @return the BPJS Claim Data that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsClaimData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsClaimData entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsClaimDataEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsClaimDataEntity> getById(
			@Parameter(description = "The UUID of the BpjsClaimData to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsClaimDataEntity> bpjsClaimDataEntity = bpjsClaimDataService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Claim Data. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsClaimDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsClaimData' endpoint here] off begin
	@Operation(
			description = "Create a BpjsClaimData if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsClaimData."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsClaimData, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsClaimDataEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsClaimData' endpoint here] end
	public ResponseEntity<?> saveBpjsClaimData(
			@Parameter(description = "The BpjsClaimDataEntity to create or update.")
			@RequestBody BpjsClaimDataEntityDto bpjsClaimDataEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsClaimDataEntity bpjsClaimDataEntity = bpjsClaimDataService.create(new BpjsClaimDataEntity(bpjsClaimDataEntityDto));
			return new ResponseEntity<>(bpjsClaimDataEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Claim Data. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsClaimDataEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsClaimData' endpoint here] off begin
	@Operation(
			description = "Update a BpjsClaimData entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsClaimData."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsClaimData, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsClaimDataEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsClaimData' endpoint here] end
	public ResponseEntity<?> updateBpjsClaimData(
			@Parameter(description = "The BpjsClaimDataEntity to create or update.")
			@RequestBody BpjsClaimDataEntityDto bpjsClaimDataEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsClaimDataEntity> checkBpjsClaimDataExists = bpjsClaimDataService.findById(bpjsClaimDataEntityDto.getId());
		BpjsClaimDataEntity bpjsClaimDataEntity = new BpjsClaimDataEntity(bpjsClaimDataEntityDto);

		if (checkBpjsClaimDataExists.isPresent()) {
			try {
				bpjsClaimDataEntity = bpjsClaimDataService.update(bpjsClaimDataEntity);
				return new ResponseEntity<>(bpjsClaimDataEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsClaimDataEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Claim Data that has the same id as the given id.
	 *
	 * @param id The id of the BpjsClaimDataEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsClaimData as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsClaimData to delete.")
		...
		bpjsClaimDataService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimData' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Claim Data entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimData' endpoint here] end
	public void exportBpjsClaimData(
			@Parameter(description = "The ids of the BpjsClaimDataEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsClaimDataEntity> entitiesToExport = bpjsClaimDataService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsClaimData_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsClaimDataEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimDataExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimDataExcludingIds' endpoint here] end
	public void exportBpjsClaimDataExcludingIds(
			@Parameter(description = "The ids of the BpjsClaimDataEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsClaimDataEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsClaimDataService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsClaimDataService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsClaimData entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsClaimData_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsClaimDataEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Claim Data entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsClaimDataEntity', 'create')  || hasPermission('BpjsClaimDataEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsClaimDataEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsClaimDataEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsClaimDataEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsClaimDataEntity> csvToBean = new CsvToBeanBuilder<BpjsClaimDataEntity>(reader)
				.withType(BpjsClaimDataEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsClaimDataEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsClaimDataEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Claim Data entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Claim Data entity and you do not have permission");
					}
