...
/**
 * Controller used to handle all REST operations regarding BPJS SEP INACBG Integration
 */
// % protected region % [Update the API route or description for the BPJS SEP INACBG IntegrationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsSEPINACBGIntegrationController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjssepinacbgintegration")
// % protected region % [Update the API route or description for the BPJS SEP INACBG IntegrationController here] end
public class BpjsSEPINACBGIntegrationController {

	private final BpjsSEPINACBGIntegrationService bpjsSEPINACBGIntegrationService;
	private final CsvUtils<BpjsSEPINACBGIntegrationEntity, BpjsSEPINACBGIntegrationRepository, BpjsSEPINACBGIntegrationEntityAudit,  BpjsSEPINACBGIntegrationService> csvUtils;
	...
	@Autowired
	public BpjsSEPINACBGIntegrationController(
		...
		this.csvUtils = new CsvUtils<>(bpjsSEPINACBGIntegrationService, BpjsSEPINACBGIntegrationEntity.class);
	...
	/**
	 * Return all the BPJS SEP INACBG Integrations.
	 *
	 * @return all the BPJS SEP INACBG Integrations
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsSEPINACBGIntegrations",
		security = {
	...
	public ResponseEntity<List<BpjsSEPINACBGIntegrationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsSEPINACBGIntegrationEntity> bpjsSEPINACBGIntegrations = bpjsSEPINACBGIntegrationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsSEPINACBGIntegrations, HttpStatus.OK);
	}

	/**
	 * Return the BPJS SEP INACBG Integration that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsSEPINACBGIntegrationEntity to be returned
	 * @return the BPJS SEP INACBG Integration that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsSEPINACBGIntegration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsSEPINACBGIntegration entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsSEPINACBGIntegrationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsSEPINACBGIntegrationEntity> getById(
			@Parameter(description = "The UUID of the BpjsSEPINACBGIntegration to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsSEPINACBGIntegrationEntity> bpjsSEPINACBGIntegrationEntity = bpjsSEPINACBGIntegrationService.findById(id);
	...
	/**
	 * Create/Update the given BPJS SEP INACBG Integration. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPINACBGIntegrationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEPINACBGIntegration' endpoint here] off begin
	@Operation(
			description = "Create a BpjsSEPINACBGIntegration if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsSEPINACBGIntegration."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsSEPINACBGIntegration, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsSEPINACBGIntegrationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEPINACBGIntegration' endpoint here] end
	public ResponseEntity<?> saveBpjsSEPINACBGIntegration(
			@Parameter(description = "The BpjsSEPINACBGIntegrationEntity to create or update.")
			@RequestBody BpjsSEPINACBGIntegrationEntityDto bpjsSEPINACBGIntegrationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsSEPINACBGIntegrationEntity bpjsSEPINACBGIntegrationEntity = bpjsSEPINACBGIntegrationService.create(new BpjsSEPINACBGIntegrationEntity(bpjsSEPINACBGIntegrationEntityDto));
			return new ResponseEntity<>(bpjsSEPINACBGIntegrationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS SEP INACBG Integration. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPINACBGIntegrationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEPINACBGIntegration' endpoint here] off begin
	@Operation(
			description = "Update a BpjsSEPINACBGIntegration entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsSEPINACBGIntegration."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsSEPINACBGIntegration, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsSEPINACBGIntegrationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEPINACBGIntegration' endpoint here] end
	public ResponseEntity<?> updateBpjsSEPINACBGIntegration(
			@Parameter(description = "The BpjsSEPINACBGIntegrationEntity to create or update.")
			@RequestBody BpjsSEPINACBGIntegrationEntityDto bpjsSEPINACBGIntegrationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsSEPINACBGIntegrationEntity> checkBpjsSEPINACBGIntegrationExists = bpjsSEPINACBGIntegrationService.findById(bpjsSEPINACBGIntegrationEntityDto.getId());
		BpjsSEPINACBGIntegrationEntity bpjsSEPINACBGIntegrationEntity = new BpjsSEPINACBGIntegrationEntity(bpjsSEPINACBGIntegrationEntityDto);

		if (checkBpjsSEPINACBGIntegrationExists.isPresent()) {
			try {
				bpjsSEPINACBGIntegrationEntity = bpjsSEPINACBGIntegrationService.update(bpjsSEPINACBGIntegrationEntity);
				return new ResponseEntity<>(bpjsSEPINACBGIntegrationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsSEPINACBGIntegrationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS SEP INACBG Integration that has the same id as the given id.
	 *
	 * @param id The id of the BpjsSEPINACBGIntegrationEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsSEPINACBGIntegration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsSEPINACBGIntegration to delete.")
		...
		bpjsSEPINACBGIntegrationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPINACBGIntegration' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS SEP INACBG Integration entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPINACBGIntegration' endpoint here] end
	public void exportBpjsSEPINACBGIntegration(
			@Parameter(description = "The ids of the BpjsSEPINACBGIntegrationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsSEPINACBGIntegrationEntity> entitiesToExport = bpjsSEPINACBGIntegrationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsSEPINACBGIntegration_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsSEPINACBGIntegrationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPINACBGIntegrationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPINACBGIntegrationExcludingIds' endpoint here] end
	public void exportBpjsSEPINACBGIntegrationExcludingIds(
			@Parameter(description = "The ids of the BpjsSEPINACBGIntegrationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsSEPINACBGIntegrationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsSEPINACBGIntegrationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsSEPINACBGIntegrationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsSEPINACBGIntegration entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsSEPINACBGIntegration_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsSEPINACBGIntegrationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS SEP INACBG Integration entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsSEPINACBGIntegrationEntity', 'create')  || hasPermission('BpjsSEPINACBGIntegrationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsSEPINACBGIntegrationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsSEPINACBGIntegrationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsSEPINACBGIntegrationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsSEPINACBGIntegrationEntity> csvToBean = new CsvToBeanBuilder<BpjsSEPINACBGIntegrationEntity>(reader)
				.withType(BpjsSEPINACBGIntegrationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsSEPINACBGIntegrationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsSEPINACBGIntegrationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS SEP INACBG Integration entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS SEP INACBG Integration entity and you do not have permission");
					}
