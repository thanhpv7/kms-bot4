...
import kmsweb.entities.DoctorScheduleEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Doctor Schedule
 */
// % protected region % [Update the API route or description for the Doctor ScheduleController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DoctorScheduleController"))
@RestController
@Slf4j
@RequestMapping("/api/doctorschedule")
// % protected region % [Update the API route or description for the Doctor ScheduleController here] end
public class DoctorScheduleController {

	private final DoctorScheduleService doctorScheduleService;
	private final CsvUtils<DoctorScheduleEntity, DoctorScheduleRepository, DoctorScheduleEntityAudit,  DoctorScheduleService> csvUtils;
	...
	public DoctorScheduleController(
		...
		this.csvUtils = new CsvUtils<>(doctorScheduleService, DoctorScheduleEntity.class);
	...
	public ResponseEntity<List<DoctorScheduleEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DoctorScheduleEntity> doctorSchedules = doctorScheduleService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Doctor Schedule that has the same id as the given id.
	 *
	 * @param id      The id of the DoctorScheduleEntity to be returned
	 * @return the Doctor Schedule that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DoctorSchedule as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DoctorSchedule entity not found"),
	})
	@PreAuthorize("hasPermission('DoctorScheduleEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DoctorScheduleEntity> getById(
			@Parameter(description = "The UUID of the DoctorSchedule to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DoctorScheduleEntity> doctorScheduleEntity = doctorScheduleService.findById(id);
	...
	/**
	 * Create/Update the given Doctor Schedule. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param doctorScheduleEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDoctorSchedule' endpoint here] off begin
	@Operation(
			description = "Create a DoctorSchedule if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DoctorSchedule."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DoctorSchedule, duplicate record found")
	})
	@PreAuthorize("hasPermission('DoctorScheduleEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDoctorSchedule' endpoint here] end
	public ResponseEntity<?> saveDoctorSchedule(
			@Parameter(description = "The DoctorScheduleEntity to create or update.")
			@RequestBody DoctorScheduleEntityDto doctorScheduleEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DoctorScheduleEntity doctorScheduleEntity = doctorScheduleService.create(new DoctorScheduleEntity(doctorScheduleEntityDto));
			return new ResponseEntity<>(doctorScheduleEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Doctor Schedule. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param doctorScheduleEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDoctorSchedule' endpoint here] off begin
	@Operation(
			description = "Update a DoctorSchedule entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DoctorSchedule."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DoctorSchedule, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DoctorScheduleEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDoctorSchedule' endpoint here] end
	public ResponseEntity<?> updateDoctorSchedule(
		...
		Optional<DoctorScheduleEntity> checkDoctorScheduleExists = doctorScheduleService.findById(doctorScheduleEntityDto.getId());
		DoctorScheduleEntity doctorScheduleEntity = new DoctorScheduleEntity(doctorScheduleEntityDto);

		if (checkDoctorScheduleExists.isPresent()) {
			try {
				doctorScheduleEntity = doctorScheduleService.update(doctorScheduleEntity);
				return new ResponseEntity<>(doctorScheduleEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(doctorScheduleEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Doctor Schedule that has the same id as the given id.
	 *
	 * @param id The id of the DoctorScheduleEntity to be deleted
	...
	@Operation(
			description = "Delete a single DoctorSchedule as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DoctorSchedule to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDoctorSchedule' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Doctor Schedule entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDoctorSchedule' endpoint here] end
	public void exportDoctorSchedule(
			@Parameter(description = "The ids of the DoctorScheduleEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DoctorScheduleEntity> entitiesToExport = doctorScheduleService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DoctorSchedule_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DoctorScheduleEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDoctorScheduleExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDoctorScheduleExcludingIds' endpoint here] end
	public void exportDoctorScheduleExcludingIds(
			@Parameter(description = "The ids of the DoctorScheduleEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DoctorScheduleEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = doctorScheduleService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = doctorScheduleService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DoctorSchedule entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DoctorSchedule_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DoctorScheduleEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Doctor Schedule entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DoctorScheduleEntity', 'create')  || hasPermission('DoctorScheduleEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DoctorScheduleEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DoctorScheduleEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DoctorScheduleEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DoctorScheduleEntity> csvToBean = new CsvToBeanBuilder<DoctorScheduleEntity>(reader)
				.withType(DoctorScheduleEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DoctorScheduleEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DoctorScheduleEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Doctor Schedule entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Doctor Schedule entity and you do not have permission");
					}
