...
import kmsweb.dtos.DoctorUserEntityDto;
import kmsweb.entities.DoctorUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Doctor User
 */
// % protected region % [Update the API route or description for the Doctor UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DoctorUserController"))
@RestController
@Slf4j
@RequestMapping("/api/doctoruser")
// % protected region % [Update the API route or description for the Doctor UserController here] end
public class DoctorUserController {

	private final DoctorUserService doctorUserService;
	private final CsvUtils<DoctorUserEntity, DoctorUserRepository, DoctorUserEntityAudit,  DoctorUserService> csvUtils;
	...
	public DoctorUserController(
		...
		this.csvUtils = new CsvUtils<>(doctorUserService, DoctorUserEntity.class);
	...
	public ResponseEntity<List<DoctorUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DoctorUserEntity> doctorUsers = doctorUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(doctorUsers, HttpStatus.OK);
	}

	/**
	 * Return the Doctor User that has the same id as the given id.
	 *
	 * @param id      The id of the DoctorUserEntity to be returned
	 * @return the Doctor User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DoctorUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DoctorUser entity not found"),
	})
	@PreAuthorize("hasPermission('DoctorUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DoctorUserEntity> getById(
			@Parameter(description = "The UUID of the DoctorUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DoctorUserEntity> doctorUserEntity = doctorUserService.findById(id);
	...
	/**
	 * Create/Update the given Doctor User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param doctorUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDoctorUser' endpoint here] off begin
	@Operation(
			description = "Create a DoctorUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DoctorUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DoctorUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('DoctorUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDoctorUser' endpoint here] end
	public ResponseEntity<?> saveDoctorUser(
			@Parameter(description = "The DoctorUserEntity to create or update.")
			@RequestBody DoctorUserEntityDto doctorUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DoctorUserEntity doctorUserEntity = doctorUserService.create(new DoctorUserEntity(doctorUserEntityDto));
			return new ResponseEntity<>(doctorUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Doctor User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param doctorUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDoctorUser' endpoint here] off begin
	@Operation(
			description = "Update a DoctorUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DoctorUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DoctorUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DoctorUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDoctorUser' endpoint here] end
	public ResponseEntity<?> updateDoctorUser(
			@Parameter(description = "The DoctorUserEntity to create or update.")
			@RequestBody DoctorUserEntityDto doctorUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DoctorUserEntity> checkDoctorUserExists = doctorUserService.findById(doctorUserEntityDto.getId());
		DoctorUserEntity doctorUserEntity = new DoctorUserEntity(doctorUserEntityDto);

		if (checkDoctorUserExists.isPresent()) {
			try {
				doctorUserEntity = doctorUserService.update(doctorUserEntity);
				return new ResponseEntity<>(doctorUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(doctorUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Doctor User that has the same id as the given id.
	 *
	 * @param id The id of the DoctorUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single DoctorUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('DoctorUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DoctorUser to delete.")
		...
		doctorUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDoctorUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Doctor User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDoctorUser' endpoint here] end
	public void exportDoctorUser(
			@Parameter(description = "The ids of the DoctorUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DoctorUserEntity> entitiesToExport = doctorUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DoctorUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DoctorUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDoctorUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDoctorUserExcludingIds' endpoint here] end
	public void exportDoctorUserExcludingIds(
			@Parameter(description = "The ids of the DoctorUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DoctorUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = doctorUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = doctorUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DoctorUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DoctorUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DoctorUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Doctor User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DoctorUserEntity', 'create')  || hasPermission('DoctorUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DoctorUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DoctorUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DoctorUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DoctorUserEntity> csvToBean = new CsvToBeanBuilder<DoctorUserEntity>(reader)
				.withType(DoctorUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DoctorUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DoctorUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Doctor User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Doctor User entity and you do not have permission");
					}
