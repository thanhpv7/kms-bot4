...
import kmsweb.entities.WarehouseInitialStockEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Warehouse Initial Stock 
 */
// % protected region % [Update the API route or description for the Warehouse Initial Stock Controller here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "WarehouseInitialStockController"))
@RestController
@Slf4j
@RequestMapping("/api/warehouseinitialstock")
// % protected region % [Update the API route or description for the Warehouse Initial Stock Controller here] end
public class WarehouseInitialStockController {

	private final WarehouseInitialStockService warehouseInitialStockService;
	private final CsvUtils<WarehouseInitialStockEntity, WarehouseInitialStockRepository, WarehouseInitialStockEntityAudit,  WarehouseInitialStockService> csvUtils;
	...
	@Autowired
	public WarehouseInitialStockController(
		...
		this.csvUtils = new CsvUtils<>(warehouseInitialStockService, WarehouseInitialStockEntity.class);
	...
	public ResponseEntity<List<WarehouseInitialStockEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<WarehouseInitialStockEntity> warehouseInitialStocks = warehouseInitialStockService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(warehouseInitialStocks, HttpStatus.OK);
	}

	/**
	 * Return the Warehouse Initial Stock  that has the same id as the given id.
	 *
	 * @param id      The id of the WarehouseInitialStockEntity to be returned
	 * @return the Warehouse Initial Stock  that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single WarehouseInitialStock as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "WarehouseInitialStock entity not found"),
	})
	@PreAuthorize("hasPermission('WarehouseInitialStockEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<WarehouseInitialStockEntity> getById(
			@Parameter(description = "The UUID of the WarehouseInitialStock to return.")
			@PathVariable("id") UUID id
	) {
		Optional<WarehouseInitialStockEntity> warehouseInitialStockEntity = warehouseInitialStockService.findById(id);
	...
	/**
	 * Create/Update the given Warehouse Initial Stock . If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseInitialStockEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveWarehouseInitialStock' endpoint here] off begin
	@Operation(
			description = "Create a WarehouseInitialStock if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new WarehouseInitialStock."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new WarehouseInitialStock, duplicate record found")
	})
	@PreAuthorize("hasPermission('WarehouseInitialStockEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveWarehouseInitialStock' endpoint here] end
	public ResponseEntity<?> saveWarehouseInitialStock(
			@Parameter(description = "The WarehouseInitialStockEntity to create or update.")
			@RequestBody WarehouseInitialStockEntityDto warehouseInitialStockEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			WarehouseInitialStockEntity warehouseInitialStockEntity = warehouseInitialStockService.create(new WarehouseInitialStockEntity(warehouseInitialStockEntityDto));
			return new ResponseEntity<>(warehouseInitialStockEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Warehouse Initial Stock . If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseInitialStockEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateWarehouseInitialStock' endpoint here] off begin
	@Operation(
			description = "Update a WarehouseInitialStock entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the WarehouseInitialStock."),
		@ApiResponse(responseCode = "400", description = "Failed to update the WarehouseInitialStock, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('WarehouseInitialStockEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateWarehouseInitialStock' endpoint here] end
	public ResponseEntity<?> updateWarehouseInitialStock(
			@Parameter(description = "The WarehouseInitialStockEntity to create or update.")
			@RequestBody WarehouseInitialStockEntityDto warehouseInitialStockEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<WarehouseInitialStockEntity> checkWarehouseInitialStockExists = warehouseInitialStockService.findById(warehouseInitialStockEntityDto.getId());
		WarehouseInitialStockEntity warehouseInitialStockEntity = new WarehouseInitialStockEntity(warehouseInitialStockEntityDto);

		if (checkWarehouseInitialStockExists.isPresent()) {
			try {
				warehouseInitialStockEntity = warehouseInitialStockService.update(warehouseInitialStockEntity);
				return new ResponseEntity<>(warehouseInitialStockEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(warehouseInitialStockEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Warehouse Initial Stock  that has the same id as the given id.
	 *
	 * @param id The id of the WarehouseInitialStockEntity to be deleted
	...
	@Operation(
			description = "Delete a single WarehouseInitialStock as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the WarehouseInitialStock to delete.")
		...
		warehouseInitialStockService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInitialStock' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Warehouse Initial Stock  entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInitialStock' endpoint here] end
	public void exportWarehouseInitialStock(
			@Parameter(description = "The ids of the WarehouseInitialStockEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<WarehouseInitialStockEntity> entitiesToExport = warehouseInitialStockService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "WarehouseInitialStock_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (WarehouseInitialStockEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInitialStockExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInitialStockExcludingIds' endpoint here] end
	public void exportWarehouseInitialStockExcludingIds(
			@Parameter(description = "The ids of the WarehouseInitialStockEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<WarehouseInitialStockEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = warehouseInitialStockService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = warehouseInitialStockService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the WarehouseInitialStock entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "WarehouseInitialStock_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(WarehouseInitialStockEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Warehouse Initial Stock  entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('WarehouseInitialStockEntity', 'create')  || hasPermission('WarehouseInitialStockEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with WarehouseInitialStockEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("WarehouseInitialStockEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(WarehouseInitialStockEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<WarehouseInitialStockEntity> csvToBean = new CsvToBeanBuilder<WarehouseInitialStockEntity>(reader)
				.withType(WarehouseInitialStockEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<WarehouseInitialStockEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<WarehouseInitialStockEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Warehouse Initial Stock  entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Warehouse Initial Stock  entity and you do not have permission");
					}
