...
/**
 * Controller used to handle all REST operations regarding Bpjs Pcare Pendaftaran
 */
// % protected region % [Update the API route or description for the Bpjs Pcare PendaftaranController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcarePendaftaranController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcarependaftaran")
// % protected region % [Update the API route or description for the Bpjs Pcare PendaftaranController here] end
public class BpjsPcarePendaftaranController {

	private final BpjsPcarePendaftaranService bpjsPcarePendaftaranService;
	private final CsvUtils<BpjsPcarePendaftaranEntity, BpjsPcarePendaftaranRepository, BpjsPcarePendaftaranEntityAudit,  BpjsPcarePendaftaranService> csvUtils;
	...
	@Autowired
	public BpjsPcarePendaftaranController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcarePendaftaranService, BpjsPcarePendaftaranEntity.class);
	...
	public ResponseEntity<List<BpjsPcarePendaftaranEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcarePendaftaranEntity> bpjsPcarePendaftarans = bpjsPcarePendaftaranService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcarePendaftarans, HttpStatus.OK);
	}

	/**
	 * Return the Bpjs Pcare Pendaftaran that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcarePendaftaranEntity to be returned
	 * @return the Bpjs Pcare Pendaftaran that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcarePendaftaran as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcarePendaftaran entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcarePendaftaranEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcarePendaftaranEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcarePendaftaran to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcarePendaftaranEntity> bpjsPcarePendaftaranEntity = bpjsPcarePendaftaranService.findById(id);
	...
	/**
	 * Create/Update the given Bpjs Pcare Pendaftaran. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcarePendaftaranEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcarePendaftaran' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcarePendaftaran if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcarePendaftaran."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcarePendaftaran, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcarePendaftaranEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcarePendaftaran' endpoint here] end
	public ResponseEntity<?> saveBpjsPcarePendaftaran(
			@Parameter(description = "The BpjsPcarePendaftaranEntity to create or update.")
			@RequestBody BpjsPcarePendaftaranEntityDto bpjsPcarePendaftaranEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcarePendaftaranEntity bpjsPcarePendaftaranEntity = bpjsPcarePendaftaranService.create(new BpjsPcarePendaftaranEntity(bpjsPcarePendaftaranEntityDto));
			return new ResponseEntity<>(bpjsPcarePendaftaranEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Bpjs Pcare Pendaftaran. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcarePendaftaranEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcarePendaftaran' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcarePendaftaran entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcarePendaftaran."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcarePendaftaran, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcarePendaftaranEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcarePendaftaran' endpoint here] end
	public ResponseEntity<?> updateBpjsPcarePendaftaran(
			@Parameter(description = "The BpjsPcarePendaftaranEntity to create or update.")
			@RequestBody BpjsPcarePendaftaranEntityDto bpjsPcarePendaftaranEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcarePendaftaranEntity> checkBpjsPcarePendaftaranExists = bpjsPcarePendaftaranService.findById(bpjsPcarePendaftaranEntityDto.getId());
		BpjsPcarePendaftaranEntity bpjsPcarePendaftaranEntity = new BpjsPcarePendaftaranEntity(bpjsPcarePendaftaranEntityDto);

		if (checkBpjsPcarePendaftaranExists.isPresent()) {
			try {
				bpjsPcarePendaftaranEntity = bpjsPcarePendaftaranService.update(bpjsPcarePendaftaranEntity);
				return new ResponseEntity<>(bpjsPcarePendaftaranEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcarePendaftaranEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Bpjs Pcare Pendaftaran that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcarePendaftaranEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcarePendaftaran as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcarePendaftaran to delete.")
		...
		bpjsPcarePendaftaranService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePendaftaran' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Bpjs Pcare Pendaftaran entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePendaftaran' endpoint here] end
	public void exportBpjsPcarePendaftaran(
			@Parameter(description = "The ids of the BpjsPcarePendaftaranEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcarePendaftaranEntity> entitiesToExport = bpjsPcarePendaftaranService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcarePendaftaran_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcarePendaftaranEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePendaftaranExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePendaftaranExcludingIds' endpoint here] end
	public void exportBpjsPcarePendaftaranExcludingIds(
			@Parameter(description = "The ids of the BpjsPcarePendaftaranEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcarePendaftaranEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcarePendaftaranService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcarePendaftaranService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcarePendaftaran entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcarePendaftaran_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcarePendaftaranEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Bpjs Pcare Pendaftaran entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcarePendaftaranEntity', 'create')  || hasPermission('BpjsPcarePendaftaranEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcarePendaftaranEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcarePendaftaranEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcarePendaftaranEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcarePendaftaranEntity> csvToBean = new CsvToBeanBuilder<BpjsPcarePendaftaranEntity>(reader)
				.withType(BpjsPcarePendaftaranEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcarePendaftaranEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcarePendaftaranEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Bpjs Pcare Pendaftaran entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Bpjs Pcare Pendaftaran entity and you do not have permission");
					}
