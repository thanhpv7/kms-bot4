...
import kmsweb.dtos.NurseUserEntityDto;
import kmsweb.entities.NurseUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Nurse User
 */
// % protected region % [Update the API route or description for the Nurse UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NurseUserController"))
@RestController
@Slf4j
@RequestMapping("/api/nurseuser")
// % protected region % [Update the API route or description for the Nurse UserController here] end
public class NurseUserController {

	private final NurseUserService nurseUserService;
	private final CsvUtils<NurseUserEntity, NurseUserRepository, NurseUserEntityAudit,  NurseUserService> csvUtils;
	...
	public NurseUserController(
		...
		this.csvUtils = new CsvUtils<>(nurseUserService, NurseUserEntity.class);
	...
	public ResponseEntity<List<NurseUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NurseUserEntity> nurseUsers = nurseUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nurseUsers, HttpStatus.OK);
	}

	/**
	 * Return the Nurse User that has the same id as the given id.
	 *
	 * @param id      The id of the NurseUserEntity to be returned
	 * @return the Nurse User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NurseUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NurseUser entity not found"),
	})
	@PreAuthorize("hasPermission('NurseUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NurseUserEntity> getById(
			@Parameter(description = "The UUID of the NurseUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NurseUserEntity> nurseUserEntity = nurseUserService.findById(id);
	...
	/**
	 * Create/Update the given Nurse User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nurseUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNurseUser' endpoint here] off begin
	@Operation(
			description = "Create a NurseUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NurseUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NurseUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('NurseUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNurseUser' endpoint here] end
	public ResponseEntity<?> saveNurseUser(
			@Parameter(description = "The NurseUserEntity to create or update.")
			@RequestBody NurseUserEntityDto nurseUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NurseUserEntity nurseUserEntity = nurseUserService.create(new NurseUserEntity(nurseUserEntityDto));
			return new ResponseEntity<>(nurseUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Nurse User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nurseUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNurseUser' endpoint here] off begin
	@Operation(
			description = "Update a NurseUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NurseUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NurseUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NurseUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNurseUser' endpoint here] end
	public ResponseEntity<?> updateNurseUser(
			@Parameter(description = "The NurseUserEntity to create or update.")
			@RequestBody NurseUserEntityDto nurseUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NurseUserEntity> checkNurseUserExists = nurseUserService.findById(nurseUserEntityDto.getId());
		NurseUserEntity nurseUserEntity = new NurseUserEntity(nurseUserEntityDto);

		if (checkNurseUserExists.isPresent()) {
			try {
				nurseUserEntity = nurseUserService.update(nurseUserEntity);
				return new ResponseEntity<>(nurseUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nurseUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Nurse User that has the same id as the given id.
	 *
	 * @param id The id of the NurseUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single NurseUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('NurseUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NurseUser to delete.")
		...
		nurseUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNurseUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Nurse User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNurseUser' endpoint here] end
	public void exportNurseUser(
			@Parameter(description = "The ids of the NurseUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NurseUserEntity> entitiesToExport = nurseUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NurseUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NurseUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNurseUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNurseUserExcludingIds' endpoint here] end
	public void exportNurseUserExcludingIds(
			@Parameter(description = "The ids of the NurseUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NurseUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nurseUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nurseUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NurseUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NurseUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NurseUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Nurse User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NurseUserEntity', 'create')  || hasPermission('NurseUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NurseUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NurseUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NurseUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NurseUserEntity> csvToBean = new CsvToBeanBuilder<NurseUserEntity>(reader)
				.withType(NurseUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NurseUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NurseUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Nurse User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Nurse User entity and you do not have permission");
					}
