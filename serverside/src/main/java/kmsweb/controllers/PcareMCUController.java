...
/**
 * Controller used to handle all REST operations regarding PCare MCU
 */
// % protected region % [Update the API route or description for the PCare MCUController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareMCUController"))
@RestController
@Slf4j
@RequestMapping("/api/pcaremcu")
// % protected region % [Update the API route or description for the PCare MCUController here] end
public class PcareMCUController {

	private final PcareMCUService pcareMCUService;
	private final CsvUtils<PcareMCUEntity, PcareMCURepository, PcareMCUEntityAudit,  PcareMCUService> csvUtils;
	...
	public PcareMCUController(
		...
		this.csvUtils = new CsvUtils<>(pcareMCUService, PcareMCUEntity.class);
	...
	public ResponseEntity<List<PcareMCUEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareMCUEntity> pcareMCUs = pcareMCUService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareMCUs, HttpStatus.OK);
	}

	/**
	 * Return the PCare MCU that has the same id as the given id.
	 *
	 * @param id      The id of the PcareMCUEntity to be returned
	 * @return the PCare MCU that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareMCU as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareMCU entity not found"),
	})
	@PreAuthorize("hasPermission('PcareMCUEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareMCUEntity> getById(
			@Parameter(description = "The UUID of the PcareMCU to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareMCUEntity> pcareMCUEntity = pcareMCUService.findById(id);
	...
	/**
	 * Create/Update the given PCare MCU. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareMCUEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareMCU' endpoint here] off begin
	@Operation(
			description = "Create a PcareMCU if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareMCU."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareMCU, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareMCUEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareMCU' endpoint here] end
	public ResponseEntity<?> savePcareMCU(
			@Parameter(description = "The PcareMCUEntity to create or update.")
			@RequestBody PcareMCUEntityDto pcareMCUEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareMCUEntity pcareMCUEntity = pcareMCUService.create(new PcareMCUEntity(pcareMCUEntityDto));
			return new ResponseEntity<>(pcareMCUEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare MCU. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareMCUEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareMCU' endpoint here] off begin
	@Operation(
			description = "Update a PcareMCU entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareMCU."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareMCU, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareMCUEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareMCU' endpoint here] end
	public ResponseEntity<?> updatePcareMCU(
			@Parameter(description = "The PcareMCUEntity to create or update.")
			@RequestBody PcareMCUEntityDto pcareMCUEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareMCUEntity> checkPcareMCUExists = pcareMCUService.findById(pcareMCUEntityDto.getId());
		PcareMCUEntity pcareMCUEntity = new PcareMCUEntity(pcareMCUEntityDto);

		if (checkPcareMCUExists.isPresent()) {
			try {
				pcareMCUEntity = pcareMCUService.update(pcareMCUEntity);
				return new ResponseEntity<>(pcareMCUEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareMCUEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare MCU that has the same id as the given id.
	 *
	 * @param id The id of the PcareMCUEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareMCU as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareMCU to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareMCU' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare MCU entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareMCU' endpoint here] end
	public void exportPcareMCU(
			@Parameter(description = "The ids of the PcareMCUEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareMCUEntity> entitiesToExport = pcareMCUService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareMCU_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareMCUEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareMCUExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareMCUExcludingIds' endpoint here] end
	public void exportPcareMCUExcludingIds(
			@Parameter(description = "The ids of the PcareMCUEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareMCUEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareMCUService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareMCUService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareMCU entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareMCU_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareMCUEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare MCU entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareMCUEntity', 'create')  || hasPermission('PcareMCUEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareMCUEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareMCUEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareMCUEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareMCUEntity> csvToBean = new CsvToBeanBuilder<PcareMCUEntity>(reader)
				.withType(PcareMCUEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareMCUEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareMCUEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare MCU entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare MCU entity and you do not have permission");
					}
