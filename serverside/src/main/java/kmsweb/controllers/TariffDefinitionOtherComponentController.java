...
import kmsweb.entities.TariffDefinitionOtherComponentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Tariff Definition Other Component
 */
// % protected region % [Update the API route or description for the Tariff Definition Other ComponentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TariffDefinitionOtherComponentController"))
@RestController
@Slf4j
@RequestMapping("/api/tariffdefinitionothercomponent")
// % protected region % [Update the API route or description for the Tariff Definition Other ComponentController here] end
public class TariffDefinitionOtherComponentController {

	private final TariffDefinitionOtherComponentService tariffDefinitionOtherComponentService;
	private final CsvUtils<TariffDefinitionOtherComponentEntity, TariffDefinitionOtherComponentRepository, TariffDefinitionOtherComponentEntityAudit,  TariffDefinitionOtherComponentService> csvUtils;
	...
	@Autowired
	public TariffDefinitionOtherComponentController(
		...
		this.csvUtils = new CsvUtils<>(tariffDefinitionOtherComponentService, TariffDefinitionOtherComponentEntity.class);
	...
	/**
	 * Return all the Tariff Definition Other Components.
	 *
	 * @return all the Tariff Definition Other Components
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of TariffDefinitionOtherComponents",
		security = {
	...
	public ResponseEntity<List<TariffDefinitionOtherComponentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TariffDefinitionOtherComponentEntity> tariffDefinitionOtherComponents = tariffDefinitionOtherComponentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(tariffDefinitionOtherComponents, HttpStatus.OK);
	}

	/**
	 * Return the Tariff Definition Other Component that has the same id as the given id.
	 *
	 * @param id      The id of the TariffDefinitionOtherComponentEntity to be returned
	 * @return the Tariff Definition Other Component that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TariffDefinitionOtherComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TariffDefinitionOtherComponent entity not found"),
	})
	@PreAuthorize("hasPermission('TariffDefinitionOtherComponentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TariffDefinitionOtherComponentEntity> getById(
			@Parameter(description = "The UUID of the TariffDefinitionOtherComponent to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TariffDefinitionOtherComponentEntity> tariffDefinitionOtherComponentEntity = tariffDefinitionOtherComponentService.findById(id);
	...
	/**
	 * Create/Update the given Tariff Definition Other Component. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param tariffDefinitionOtherComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTariffDefinitionOtherComponent' endpoint here] off begin
	@Operation(
			description = "Create a TariffDefinitionOtherComponent if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TariffDefinitionOtherComponent."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TariffDefinitionOtherComponent, duplicate record found")
	})
	@PreAuthorize("hasPermission('TariffDefinitionOtherComponentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTariffDefinitionOtherComponent' endpoint here] end
	public ResponseEntity<?> saveTariffDefinitionOtherComponent(
			@Parameter(description = "The TariffDefinitionOtherComponentEntity to create or update.")
			@RequestBody TariffDefinitionOtherComponentEntityDto tariffDefinitionOtherComponentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TariffDefinitionOtherComponentEntity tariffDefinitionOtherComponentEntity = tariffDefinitionOtherComponentService.create(new TariffDefinitionOtherComponentEntity(tariffDefinitionOtherComponentEntityDto));
			return new ResponseEntity<>(tariffDefinitionOtherComponentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Tariff Definition Other Component. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param tariffDefinitionOtherComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTariffDefinitionOtherComponent' endpoint here] off begin
	@Operation(
			description = "Update a TariffDefinitionOtherComponent entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TariffDefinitionOtherComponent."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TariffDefinitionOtherComponent, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TariffDefinitionOtherComponentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTariffDefinitionOtherComponent' endpoint here] end
	public ResponseEntity<?> updateTariffDefinitionOtherComponent(
			@Parameter(description = "The TariffDefinitionOtherComponentEntity to create or update.")
			@RequestBody TariffDefinitionOtherComponentEntityDto tariffDefinitionOtherComponentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TariffDefinitionOtherComponentEntity> checkTariffDefinitionOtherComponentExists = tariffDefinitionOtherComponentService.findById(tariffDefinitionOtherComponentEntityDto.getId());
		TariffDefinitionOtherComponentEntity tariffDefinitionOtherComponentEntity = new TariffDefinitionOtherComponentEntity(tariffDefinitionOtherComponentEntityDto);

		if (checkTariffDefinitionOtherComponentExists.isPresent()) {
			try {
				tariffDefinitionOtherComponentEntity = tariffDefinitionOtherComponentService.update(tariffDefinitionOtherComponentEntity);
				return new ResponseEntity<>(tariffDefinitionOtherComponentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(tariffDefinitionOtherComponentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Tariff Definition Other Component that has the same id as the given id.
	 *
	 * @param id The id of the TariffDefinitionOtherComponentEntity to be deleted
	...
	@Operation(
			description = "Delete a single TariffDefinitionOtherComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TariffDefinitionOtherComponent to delete.")
		...
		tariffDefinitionOtherComponentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinitionOtherComponent' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Tariff Definition Other Component entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinitionOtherComponent' endpoint here] end
	public void exportTariffDefinitionOtherComponent(
			@Parameter(description = "The ids of the TariffDefinitionOtherComponentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TariffDefinitionOtherComponentEntity> entitiesToExport = tariffDefinitionOtherComponentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TariffDefinitionOtherComponent_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TariffDefinitionOtherComponentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinitionOtherComponentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinitionOtherComponentExcludingIds' endpoint here] end
	public void exportTariffDefinitionOtherComponentExcludingIds(
			@Parameter(description = "The ids of the TariffDefinitionOtherComponentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TariffDefinitionOtherComponentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = tariffDefinitionOtherComponentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = tariffDefinitionOtherComponentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TariffDefinitionOtherComponent entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TariffDefinitionOtherComponent_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TariffDefinitionOtherComponentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Tariff Definition Other Component entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TariffDefinitionOtherComponentEntity', 'create')  || hasPermission('TariffDefinitionOtherComponentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TariffDefinitionOtherComponentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TariffDefinitionOtherComponentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TariffDefinitionOtherComponentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TariffDefinitionOtherComponentEntity> csvToBean = new CsvToBeanBuilder<TariffDefinitionOtherComponentEntity>(reader)
				.withType(TariffDefinitionOtherComponentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TariffDefinitionOtherComponentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TariffDefinitionOtherComponentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Tariff Definition Other Component entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Tariff Definition Other Component entity and you do not have permission");
					}
