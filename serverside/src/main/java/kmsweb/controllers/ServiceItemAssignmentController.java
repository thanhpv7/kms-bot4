...
import kmsweb.entities.ServiceItemAssignmentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Service Item Assignment
 */
// % protected region % [Update the API route or description for the Service Item AssignmentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ServiceItemAssignmentController"))
@RestController
@Slf4j
@RequestMapping("/api/serviceitemassignment")
// % protected region % [Update the API route or description for the Service Item AssignmentController here] end
public class ServiceItemAssignmentController {

	private final ServiceItemAssignmentService serviceItemAssignmentService;
	private final CsvUtils<ServiceItemAssignmentEntity, ServiceItemAssignmentRepository, ServiceItemAssignmentEntityAudit,  ServiceItemAssignmentService> csvUtils;
	...
	@Autowired
	public ServiceItemAssignmentController(
		...
		this.csvUtils = new CsvUtils<>(serviceItemAssignmentService, ServiceItemAssignmentEntity.class);
	...
	public ResponseEntity<List<ServiceItemAssignmentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ServiceItemAssignmentEntity> serviceItemAssignments = serviceItemAssignmentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(serviceItemAssignments, HttpStatus.OK);
	}

	/**
	 * Return the Service Item Assignment that has the same id as the given id.
	 *
	 * @param id      The id of the ServiceItemAssignmentEntity to be returned
	 * @return the Service Item Assignment that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ServiceItemAssignment as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ServiceItemAssignment entity not found"),
	})
	@PreAuthorize("hasPermission('ServiceItemAssignmentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ServiceItemAssignmentEntity> getById(
			@Parameter(description = "The UUID of the ServiceItemAssignment to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ServiceItemAssignmentEntity> serviceItemAssignmentEntity = serviceItemAssignmentService.findById(id);
	...
	/**
	 * Create/Update the given Service Item Assignment. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param serviceItemAssignmentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveServiceItemAssignment' endpoint here] off begin
	@Operation(
			description = "Create a ServiceItemAssignment if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ServiceItemAssignment."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ServiceItemAssignment, duplicate record found")
	})
	@PreAuthorize("hasPermission('ServiceItemAssignmentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveServiceItemAssignment' endpoint here] end
	public ResponseEntity<?> saveServiceItemAssignment(
			@Parameter(description = "The ServiceItemAssignmentEntity to create or update.")
			@RequestBody ServiceItemAssignmentEntityDto serviceItemAssignmentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ServiceItemAssignmentEntity serviceItemAssignmentEntity = serviceItemAssignmentService.create(new ServiceItemAssignmentEntity(serviceItemAssignmentEntityDto));
			return new ResponseEntity<>(serviceItemAssignmentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Service Item Assignment. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param serviceItemAssignmentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateServiceItemAssignment' endpoint here] off begin
	@Operation(
			description = "Update a ServiceItemAssignment entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ServiceItemAssignment."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ServiceItemAssignment, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ServiceItemAssignmentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateServiceItemAssignment' endpoint here] end
	public ResponseEntity<?> updateServiceItemAssignment(
			@Parameter(description = "The ServiceItemAssignmentEntity to create or update.")
			@RequestBody ServiceItemAssignmentEntityDto serviceItemAssignmentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ServiceItemAssignmentEntity> checkServiceItemAssignmentExists = serviceItemAssignmentService.findById(serviceItemAssignmentEntityDto.getId());
		ServiceItemAssignmentEntity serviceItemAssignmentEntity = new ServiceItemAssignmentEntity(serviceItemAssignmentEntityDto);

		if (checkServiceItemAssignmentExists.isPresent()) {
			try {
				serviceItemAssignmentEntity = serviceItemAssignmentService.update(serviceItemAssignmentEntity);
				return new ResponseEntity<>(serviceItemAssignmentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(serviceItemAssignmentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Service Item Assignment that has the same id as the given id.
	 *
	 * @param id The id of the ServiceItemAssignmentEntity to be deleted
	...
	@Operation(
			description = "Delete a single ServiceItemAssignment as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ServiceItemAssignment to delete.")
		...
		serviceItemAssignmentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportServiceItemAssignment' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Service Item Assignment entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportServiceItemAssignment' endpoint here] end
	public void exportServiceItemAssignment(
			@Parameter(description = "The ids of the ServiceItemAssignmentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ServiceItemAssignmentEntity> entitiesToExport = serviceItemAssignmentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ServiceItemAssignment_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ServiceItemAssignmentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportServiceItemAssignmentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportServiceItemAssignmentExcludingIds' endpoint here] end
	public void exportServiceItemAssignmentExcludingIds(
			@Parameter(description = "The ids of the ServiceItemAssignmentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ServiceItemAssignmentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = serviceItemAssignmentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = serviceItemAssignmentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ServiceItemAssignment entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ServiceItemAssignment_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ServiceItemAssignmentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Service Item Assignment entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ServiceItemAssignmentEntity', 'create')  || hasPermission('ServiceItemAssignmentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ServiceItemAssignmentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ServiceItemAssignmentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ServiceItemAssignmentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ServiceItemAssignmentEntity> csvToBean = new CsvToBeanBuilder<ServiceItemAssignmentEntity>(reader)
				.withType(ServiceItemAssignmentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ServiceItemAssignmentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ServiceItemAssignmentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Service Item Assignment entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Service Item Assignment entity and you do not have permission");
					}
