...
import kmsweb.entities.PrescriptionHeaderEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Prescription Header
 */
// % protected region % [Update the API route or description for the Prescription HeaderController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PrescriptionHeaderController"))
@RestController
@Slf4j
@RequestMapping("/api/prescriptionheader")
// % protected region % [Update the API route or description for the Prescription HeaderController here] end
public class PrescriptionHeaderController {

	private final PrescriptionHeaderService prescriptionHeaderService;
	private final CsvUtils<PrescriptionHeaderEntity, PrescriptionHeaderRepository, PrescriptionHeaderEntityAudit,  PrescriptionHeaderService> csvUtils;
	...
	public PrescriptionHeaderController(
		...
		this.csvUtils = new CsvUtils<>(prescriptionHeaderService, PrescriptionHeaderEntity.class);
	...
	public ResponseEntity<List<PrescriptionHeaderEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PrescriptionHeaderEntity> prescriptionHeaders = prescriptionHeaderService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(prescriptionHeaders, HttpStatus.OK);
	}

	/**
	 * Return the Prescription Header that has the same id as the given id.
	 *
	 * @param id      The id of the PrescriptionHeaderEntity to be returned
	 * @return the Prescription Header that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PrescriptionHeader as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PrescriptionHeader entity not found"),
	})
	@PreAuthorize("hasPermission('PrescriptionHeaderEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PrescriptionHeaderEntity> getById(
			@Parameter(description = "The UUID of the PrescriptionHeader to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PrescriptionHeaderEntity> prescriptionHeaderEntity = prescriptionHeaderService.findById(id);
	...
	/**
	 * Create/Update the given Prescription Header. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param prescriptionHeaderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePrescriptionHeader' endpoint here] off begin
	@Operation(
			description = "Create a PrescriptionHeader if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PrescriptionHeader."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PrescriptionHeader, duplicate record found")
	})
	@PreAuthorize("hasPermission('PrescriptionHeaderEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePrescriptionHeader' endpoint here] end
	public ResponseEntity<?> savePrescriptionHeader(
			@Parameter(description = "The PrescriptionHeaderEntity to create or update.")
			@RequestBody PrescriptionHeaderEntityDto prescriptionHeaderEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PrescriptionHeaderEntity prescriptionHeaderEntity = prescriptionHeaderService.create(new PrescriptionHeaderEntity(prescriptionHeaderEntityDto));
			return new ResponseEntity<>(prescriptionHeaderEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Prescription Header. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param prescriptionHeaderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePrescriptionHeader' endpoint here] off begin
	@Operation(
			description = "Update a PrescriptionHeader entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PrescriptionHeader."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PrescriptionHeader, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PrescriptionHeaderEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePrescriptionHeader' endpoint here] end
	public ResponseEntity<?> updatePrescriptionHeader(
			@Parameter(description = "The PrescriptionHeaderEntity to create or update.")
			@RequestBody PrescriptionHeaderEntityDto prescriptionHeaderEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PrescriptionHeaderEntity> checkPrescriptionHeaderExists = prescriptionHeaderService.findById(prescriptionHeaderEntityDto.getId());
		PrescriptionHeaderEntity prescriptionHeaderEntity = new PrescriptionHeaderEntity(prescriptionHeaderEntityDto);

		if (checkPrescriptionHeaderExists.isPresent()) {
			try {
				prescriptionHeaderEntity = prescriptionHeaderService.update(prescriptionHeaderEntity);
				return new ResponseEntity<>(prescriptionHeaderEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(prescriptionHeaderEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Prescription Header that has the same id as the given id.
	 *
	 * @param id The id of the PrescriptionHeaderEntity to be deleted
	...
	@Operation(
			description = "Delete a single PrescriptionHeader as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PrescriptionHeader to delete.")
		...
		prescriptionHeaderService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionHeader' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Prescription Header entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionHeader' endpoint here] end
	public void exportPrescriptionHeader(
			@Parameter(description = "The ids of the PrescriptionHeaderEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PrescriptionHeaderEntity> entitiesToExport = prescriptionHeaderService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PrescriptionHeader_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PrescriptionHeaderEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionHeaderExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionHeaderExcludingIds' endpoint here] end
	public void exportPrescriptionHeaderExcludingIds(
			@Parameter(description = "The ids of the PrescriptionHeaderEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PrescriptionHeaderEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = prescriptionHeaderService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = prescriptionHeaderService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PrescriptionHeader entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PrescriptionHeader_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PrescriptionHeaderEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Prescription Header entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PrescriptionHeaderEntity', 'create')  || hasPermission('PrescriptionHeaderEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PrescriptionHeaderEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PrescriptionHeaderEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PrescriptionHeaderEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PrescriptionHeaderEntity> csvToBean = new CsvToBeanBuilder<PrescriptionHeaderEntity>(reader)
				.withType(PrescriptionHeaderEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PrescriptionHeaderEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PrescriptionHeaderEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Prescription Header entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Prescription Header entity and you do not have permission");
					}
