...
/**
 * Controller used to handle all REST operations regarding Stock Catalogue
 */
// % protected region % [Update the API route or description for the Stock CatalogueController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "StockCatalogueController"))
@RestController
@Slf4j
@RequestMapping("/api/stockcatalogue")
// % protected region % [Update the API route or description for the Stock CatalogueController here] end
public class StockCatalogueController {

	private final StockCatalogueService stockCatalogueService;
	private final CsvUtils<StockCatalogueEntity, StockCatalogueRepository, StockCatalogueEntityAudit,  StockCatalogueService> csvUtils;
	...
	@Autowired
	public StockCatalogueController(
		...
		this.csvUtils = new CsvUtils<>(stockCatalogueService, StockCatalogueEntity.class);
	...
	public ResponseEntity<List<StockCatalogueEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<StockCatalogueEntity> stockCatalogues = stockCatalogueService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(stockCatalogues, HttpStatus.OK);
	}

	/**
	 * Return the Stock Catalogue that has the same id as the given id.
	 *
	 * @param id      The id of the StockCatalogueEntity to be returned
	 * @return the Stock Catalogue that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single StockCatalogue as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "StockCatalogue entity not found"),
	})
	@PreAuthorize("hasPermission('StockCatalogueEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<StockCatalogueEntity> getById(
			@Parameter(description = "The UUID of the StockCatalogue to return.")
			@PathVariable("id") UUID id
	) {
		Optional<StockCatalogueEntity> stockCatalogueEntity = stockCatalogueService.findById(id);
	...
	/**
	 * Create/Update the given Stock Catalogue. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param stockCatalogueEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveStockCatalogue' endpoint here] off begin
	@Operation(
			description = "Create a StockCatalogue if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new StockCatalogue."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new StockCatalogue, duplicate record found")
	})
	@PreAuthorize("hasPermission('StockCatalogueEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveStockCatalogue' endpoint here] end
	public ResponseEntity<?> saveStockCatalogue(
			@Parameter(description = "The StockCatalogueEntity to create or update.")
			@RequestBody StockCatalogueEntityDto stockCatalogueEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			StockCatalogueEntity stockCatalogueEntity = stockCatalogueService.create(new StockCatalogueEntity(stockCatalogueEntityDto));
			return new ResponseEntity<>(stockCatalogueEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Stock Catalogue. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param stockCatalogueEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateStockCatalogue' endpoint here] off begin
	@Operation(
			description = "Update a StockCatalogue entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the StockCatalogue."),
		@ApiResponse(responseCode = "400", description = "Failed to update the StockCatalogue, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('StockCatalogueEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateStockCatalogue' endpoint here] end
	public ResponseEntity<?> updateStockCatalogue(
			@Parameter(description = "The StockCatalogueEntity to create or update.")
			@RequestBody StockCatalogueEntityDto stockCatalogueEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<StockCatalogueEntity> checkStockCatalogueExists = stockCatalogueService.findById(stockCatalogueEntityDto.getId());
		StockCatalogueEntity stockCatalogueEntity = new StockCatalogueEntity(stockCatalogueEntityDto);

		if (checkStockCatalogueExists.isPresent()) {
			try {
				stockCatalogueEntity = stockCatalogueService.update(stockCatalogueEntity);
				return new ResponseEntity<>(stockCatalogueEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(stockCatalogueEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Stock Catalogue that has the same id as the given id.
	 *
	 * @param id The id of the StockCatalogueEntity to be deleted
	...
	@Operation(
			description = "Delete a single StockCatalogue as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the StockCatalogue to delete.")
		...
		stockCatalogueService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockCatalogue' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Stock Catalogue entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportStockCatalogue' endpoint here] end
	public void exportStockCatalogue(
			@Parameter(description = "The ids of the StockCatalogueEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<StockCatalogueEntity> entitiesToExport = stockCatalogueService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "StockCatalogue_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (StockCatalogueEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockCatalogueExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportStockCatalogueExcludingIds' endpoint here] end
	public void exportStockCatalogueExcludingIds(
			@Parameter(description = "The ids of the StockCatalogueEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<StockCatalogueEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = stockCatalogueService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = stockCatalogueService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the StockCatalogue entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "StockCatalogue_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(StockCatalogueEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Stock Catalogue entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('StockCatalogueEntity', 'create')  || hasPermission('StockCatalogueEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with StockCatalogueEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("StockCatalogueEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(StockCatalogueEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<StockCatalogueEntity> csvToBean = new CsvToBeanBuilder<StockCatalogueEntity>(reader)
				.withType(StockCatalogueEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<StockCatalogueEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<StockCatalogueEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Stock Catalogue entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Stock Catalogue entity and you do not have permission");
					}
