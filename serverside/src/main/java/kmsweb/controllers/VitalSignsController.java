...
/**
 * Controller used to handle all REST operations regarding Vital Signs
 */
// % protected region % [Update the API route or description for the Vital SignsController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "VitalSignsController"))
@RestController
@Slf4j
@RequestMapping("/api/vitalsigns")
// % protected region % [Update the API route or description for the Vital SignsController here] end
public class VitalSignsController {

	private final VitalSignsService vitalSignsService;
	private final CsvUtils<VitalSignsEntity, VitalSignsRepository, VitalSignsEntityAudit,  VitalSignsService> csvUtils;
	...
	public VitalSignsController(
		...
		this.csvUtils = new CsvUtils<>(vitalSignsService, VitalSignsEntity.class);
	...
	public ResponseEntity<List<VitalSignsEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<VitalSignsEntity> vitalSignss = vitalSignsService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(vitalSignss, HttpStatus.OK);
	}

	/**
	 * Return the Vital Signs that has the same id as the given id.
	 *
	 * @param id      The id of the VitalSignsEntity to be returned
	 * @return the Vital Signs that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single VitalSigns as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "VitalSigns entity not found"),
	})
	@PreAuthorize("hasPermission('VitalSignsEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<VitalSignsEntity> getById(
			@Parameter(description = "The UUID of the VitalSigns to return.")
			@PathVariable("id") UUID id
	) {
		Optional<VitalSignsEntity> vitalSignsEntity = vitalSignsService.findById(id);
	...
	/**
	 * Create/Update the given Vital Signs. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param vitalSignsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveVitalSigns' endpoint here] off begin
	@Operation(
			description = "Create a VitalSigns if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new VitalSigns."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new VitalSigns, duplicate record found")
	})
	@PreAuthorize("hasPermission('VitalSignsEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveVitalSigns' endpoint here] end
	public ResponseEntity<?> saveVitalSigns(
			@Parameter(description = "The VitalSignsEntity to create or update.")
			@RequestBody VitalSignsEntityDto vitalSignsEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			VitalSignsEntity vitalSignsEntity = vitalSignsService.create(new VitalSignsEntity(vitalSignsEntityDto));
			return new ResponseEntity<>(vitalSignsEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Vital Signs. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param vitalSignsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateVitalSigns' endpoint here] off begin
	@Operation(
			description = "Update a VitalSigns entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the VitalSigns."),
		@ApiResponse(responseCode = "400", description = "Failed to update the VitalSigns, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('VitalSignsEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateVitalSigns' endpoint here] end
	public ResponseEntity<?> updateVitalSigns(
		...
		Optional<VitalSignsEntity> checkVitalSignsExists = vitalSignsService.findById(vitalSignsEntityDto.getId());
		VitalSignsEntity vitalSignsEntity = new VitalSignsEntity(vitalSignsEntityDto);

		if (checkVitalSignsExists.isPresent()) {
			try {
				vitalSignsEntity = vitalSignsService.update(vitalSignsEntity);
				return new ResponseEntity<>(vitalSignsEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(vitalSignsEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Vital Signs that has the same id as the given id.
	 *
	 * @param id The id of the VitalSignsEntity to be deleted
	...
	@Operation(
			description = "Delete a single VitalSigns as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the VitalSigns to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportVitalSigns' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Vital Signs entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportVitalSigns' endpoint here] end
	public void exportVitalSigns(
			@Parameter(description = "The ids of the VitalSignsEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<VitalSignsEntity> entitiesToExport = vitalSignsService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "VitalSigns_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (VitalSignsEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportVitalSignsExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportVitalSignsExcludingIds' endpoint here] end
	public void exportVitalSignsExcludingIds(
			@Parameter(description = "The ids of the VitalSignsEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<VitalSignsEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = vitalSignsService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = vitalSignsService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the VitalSigns entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "VitalSigns_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(VitalSignsEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Vital Signs entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('VitalSignsEntity', 'create')  || hasPermission('VitalSignsEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with VitalSignsEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("VitalSignsEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(VitalSignsEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<VitalSignsEntity> csvToBean = new CsvToBeanBuilder<VitalSignsEntity>(reader)
				.withType(VitalSignsEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<VitalSignsEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<VitalSignsEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Vital Signs entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Vital Signs entity and you do not have permission");
					}
