...
import kmsweb.entities.PatientVisitPatientPaymentInsuranceEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Visit Patient Payment Insurance
 */
// % protected region % [Update the API route or description for the Patient Visit Patient Payment InsuranceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientVisitPatientPaymentInsuranceController"))
@RestController
@Slf4j
@RequestMapping("/api/patientvisitpatientpaymentinsurance")
// % protected region % [Update the API route or description for the Patient Visit Patient Payment InsuranceController here] end
public class PatientVisitPatientPaymentInsuranceController {

	private final PatientVisitPatientPaymentInsuranceService patientVisitPatientPaymentInsuranceService;
	private final CsvUtils<PatientVisitPatientPaymentInsuranceEntity, PatientVisitPatientPaymentInsuranceRepository, PatientVisitPatientPaymentInsuranceEntityAudit,  PatientVisitPatientPaymentInsuranceService> csvUtils;
	...
	@Autowired
	public PatientVisitPatientPaymentInsuranceController(
		...
		this.csvUtils = new CsvUtils<>(patientVisitPatientPaymentInsuranceService, PatientVisitPatientPaymentInsuranceEntity.class);
	...
	/**
	 * Return all the Patient Visit Patient Payment Insurances.
	 *
	 * @return all the Patient Visit Patient Payment Insurances
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of PatientVisitPatientPaymentInsurances",
		security = {
	...
	public ResponseEntity<List<PatientVisitPatientPaymentInsuranceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientVisitPatientPaymentInsuranceEntity> patientVisitPatientPaymentInsurances = patientVisitPatientPaymentInsuranceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientVisitPatientPaymentInsurances, HttpStatus.OK);
	}

	/**
	 * Return the Patient Visit Patient Payment Insurance that has the same id as the given id.
	 *
	 * @param id      The id of the PatientVisitPatientPaymentInsuranceEntity to be returned
	 * @return the Patient Visit Patient Payment Insurance that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientVisitPatientPaymentInsurance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientVisitPatientPaymentInsurance entity not found"),
	})
	@PreAuthorize("hasPermission('PatientVisitPatientPaymentInsuranceEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientVisitPatientPaymentInsuranceEntity> getById(
			@Parameter(description = "The UUID of the PatientVisitPatientPaymentInsurance to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientVisitPatientPaymentInsuranceEntity> patientVisitPatientPaymentInsuranceEntity = patientVisitPatientPaymentInsuranceService.findById(id);
	...
	/**
	 * Create/Update the given Patient Visit Patient Payment Insurance. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientVisitPatientPaymentInsuranceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientVisitPatientPaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Create a PatientVisitPatientPaymentInsurance if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientVisitPatientPaymentInsurance."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientVisitPatientPaymentInsurance, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientVisitPatientPaymentInsuranceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientVisitPatientPaymentInsurance' endpoint here] end
	public ResponseEntity<?> savePatientVisitPatientPaymentInsurance(
			@Parameter(description = "The PatientVisitPatientPaymentInsuranceEntity to create or update.")
			@RequestBody PatientVisitPatientPaymentInsuranceEntityDto patientVisitPatientPaymentInsuranceEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientVisitPatientPaymentInsuranceEntity patientVisitPatientPaymentInsuranceEntity = patientVisitPatientPaymentInsuranceService.create(new PatientVisitPatientPaymentInsuranceEntity(patientVisitPatientPaymentInsuranceEntityDto));
			return new ResponseEntity<>(patientVisitPatientPaymentInsuranceEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Visit Patient Payment Insurance. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientVisitPatientPaymentInsuranceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientVisitPatientPaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Update a PatientVisitPatientPaymentInsurance entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientVisitPatientPaymentInsurance."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientVisitPatientPaymentInsurance, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientVisitPatientPaymentInsuranceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientVisitPatientPaymentInsurance' endpoint here] end
	public ResponseEntity<?> updatePatientVisitPatientPaymentInsurance(
			@Parameter(description = "The PatientVisitPatientPaymentInsuranceEntity to create or update.")
			@RequestBody PatientVisitPatientPaymentInsuranceEntityDto patientVisitPatientPaymentInsuranceEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientVisitPatientPaymentInsuranceEntity> checkPatientVisitPatientPaymentInsuranceExists = patientVisitPatientPaymentInsuranceService.findById(patientVisitPatientPaymentInsuranceEntityDto.getId());
		PatientVisitPatientPaymentInsuranceEntity patientVisitPatientPaymentInsuranceEntity = new PatientVisitPatientPaymentInsuranceEntity(patientVisitPatientPaymentInsuranceEntityDto);

		if (checkPatientVisitPatientPaymentInsuranceExists.isPresent()) {
			try {
				patientVisitPatientPaymentInsuranceEntity = patientVisitPatientPaymentInsuranceService.update(patientVisitPatientPaymentInsuranceEntity);
				return new ResponseEntity<>(patientVisitPatientPaymentInsuranceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientVisitPatientPaymentInsuranceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Visit Patient Payment Insurance that has the same id as the given id.
	 *
	 * @param id The id of the PatientVisitPatientPaymentInsuranceEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientVisitPatientPaymentInsurance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientVisitPatientPaymentInsurance to delete.")
		...
		patientVisitPatientPaymentInsuranceService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPatientPaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Visit Patient Payment Insurance entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPatientPaymentInsurance' endpoint here] end
	public void exportPatientVisitPatientPaymentInsurance(
			@Parameter(description = "The ids of the PatientVisitPatientPaymentInsuranceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientVisitPatientPaymentInsuranceEntity> entitiesToExport = patientVisitPatientPaymentInsuranceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientVisitPatientPaymentInsurance_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientVisitPatientPaymentInsuranceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPatientPaymentInsuranceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitPatientPaymentInsuranceExcludingIds' endpoint here] end
	public void exportPatientVisitPatientPaymentInsuranceExcludingIds(
			@Parameter(description = "The ids of the PatientVisitPatientPaymentInsuranceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientVisitPatientPaymentInsuranceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientVisitPatientPaymentInsuranceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientVisitPatientPaymentInsuranceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientVisitPatientPaymentInsurance entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientVisitPatientPaymentInsurance_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientVisitPatientPaymentInsuranceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Visit Patient Payment Insurance entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientVisitPatientPaymentInsuranceEntity', 'create')  || hasPermission('PatientVisitPatientPaymentInsuranceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientVisitPatientPaymentInsuranceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientVisitPatientPaymentInsuranceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientVisitPatientPaymentInsuranceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientVisitPatientPaymentInsuranceEntity> csvToBean = new CsvToBeanBuilder<PatientVisitPatientPaymentInsuranceEntity>(reader)
				.withType(PatientVisitPatientPaymentInsuranceEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientVisitPatientPaymentInsuranceEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientVisitPatientPaymentInsuranceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Visit Patient Payment Insurance entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Visit Patient Payment Insurance entity and you do not have permission");
					}
