...
import kmsweb.entities.PatientVisitEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Visit
 */
// % protected region % [Update the API route or description for the Patient VisitController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientVisitController"))
@RestController
@Slf4j
@RequestMapping("/api/patientvisit")
// % protected region % [Update the API route or description for the Patient VisitController here] end
public class PatientVisitController {

	private final PatientVisitService patientVisitService;
	private final CsvUtils<PatientVisitEntity, PatientVisitRepository, PatientVisitEntityAudit,  PatientVisitService> csvUtils;
	...
	public PatientVisitController(
		...
		this.csvUtils = new CsvUtils<>(patientVisitService, PatientVisitEntity.class);
	...
	public ResponseEntity<List<PatientVisitEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientVisitEntity> patientVisits = patientVisitService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Patient Visit that has the same id as the given id.
	 *
	 * @param id      The id of the PatientVisitEntity to be returned
	 * @return the Patient Visit that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientVisit as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientVisit entity not found"),
	})
	@PreAuthorize("hasPermission('PatientVisitEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientVisitEntity> getById(
			@Parameter(description = "The UUID of the PatientVisit to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientVisitEntity> patientVisitEntity = patientVisitService.findById(id);
	...
	/**
	 * Create/Update the given Patient Visit. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientVisitEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientVisit' endpoint here] off begin
	@Operation(
			description = "Create a PatientVisit if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientVisit."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientVisit, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientVisitEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientVisit' endpoint here] end
	public ResponseEntity<?> savePatientVisit(
			@Parameter(description = "The PatientVisitEntity to create or update.")
			@RequestBody PatientVisitEntityDto patientVisitEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientVisitEntity patientVisitEntity = patientVisitService.create(new PatientVisitEntity(patientVisitEntityDto));
			return new ResponseEntity<>(patientVisitEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Visit. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientVisitEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientVisit' endpoint here] off begin
	@Operation(
			description = "Update a PatientVisit entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientVisit."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientVisit, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientVisitEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientVisit' endpoint here] end
	public ResponseEntity<?> updatePatientVisit(
			@Parameter(description = "The PatientVisitEntity to create or update.")
			@RequestBody PatientVisitEntityDto patientVisitEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientVisitEntity> checkPatientVisitExists = patientVisitService.findById(patientVisitEntityDto.getId());
		PatientVisitEntity patientVisitEntity = new PatientVisitEntity(patientVisitEntityDto);

		if (checkPatientVisitExists.isPresent()) {
			try {
				patientVisitEntity = patientVisitService.update(patientVisitEntity);
				return new ResponseEntity<>(patientVisitEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientVisitEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Visit that has the same id as the given id.
	 *
	 * @param id The id of the PatientVisitEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientVisit as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientVisit to delete.")
		...
		patientVisitService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVisit' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Visit entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVisit' endpoint here] end
	public void exportPatientVisit(
			@Parameter(description = "The ids of the PatientVisitEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientVisitEntity> entitiesToExport = patientVisitService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientVisit_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientVisitEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVisitExcludingIds' endpoint here] end
	public void exportPatientVisitExcludingIds(
			@Parameter(description = "The ids of the PatientVisitEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientVisitEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientVisitService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientVisitService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientVisit entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientVisit_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientVisitEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Visit entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientVisitEntity', 'create')  || hasPermission('PatientVisitEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientVisitEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientVisitEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientVisitEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientVisitEntity> csvToBean = new CsvToBeanBuilder<PatientVisitEntity>(reader)
				.withType(PatientVisitEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientVisitEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientVisitEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Visit entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Visit entity and you do not have permission");
					}
