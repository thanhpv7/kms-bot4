...
/**
 * Controller used to handle all REST operations regarding Hemodialysis Monitoring
 */
// % protected region % [Update the API route or description for the Hemodialysis MonitoringController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "HemodialysisMonitoringController"))
@RestController
@Slf4j
@RequestMapping("/api/hemodialysismonitoring")
// % protected region % [Update the API route or description for the Hemodialysis MonitoringController here] end
public class HemodialysisMonitoringController {

	private final HemodialysisMonitoringService hemodialysisMonitoringService;
	private final CsvUtils<HemodialysisMonitoringEntity, HemodialysisMonitoringRepository, HemodialysisMonitoringEntityAudit,  HemodialysisMonitoringService> csvUtils;
	...
	@Autowired
	public HemodialysisMonitoringController(
		...
		this.csvUtils = new CsvUtils<>(hemodialysisMonitoringService, HemodialysisMonitoringEntity.class);
	...
	public ResponseEntity<List<HemodialysisMonitoringEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<HemodialysisMonitoringEntity> hemodialysisMonitorings = hemodialysisMonitoringService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(hemodialysisMonitorings, HttpStatus.OK);
	}

	/**
	 * Return the Hemodialysis Monitoring that has the same id as the given id.
	 *
	 * @param id      The id of the HemodialysisMonitoringEntity to be returned
	 * @return the Hemodialysis Monitoring that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single HemodialysisMonitoring as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "HemodialysisMonitoring entity not found"),
	})
	@PreAuthorize("hasPermission('HemodialysisMonitoringEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<HemodialysisMonitoringEntity> getById(
			@Parameter(description = "The UUID of the HemodialysisMonitoring to return.")
			@PathVariable("id") UUID id
	) {
		Optional<HemodialysisMonitoringEntity> hemodialysisMonitoringEntity = hemodialysisMonitoringService.findById(id);
	...
	/**
	 * Create/Update the given Hemodialysis Monitoring. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param hemodialysisMonitoringEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveHemodialysisMonitoring' endpoint here] off begin
	@Operation(
			description = "Create a HemodialysisMonitoring if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new HemodialysisMonitoring."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new HemodialysisMonitoring, duplicate record found")
	})
	@PreAuthorize("hasPermission('HemodialysisMonitoringEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveHemodialysisMonitoring' endpoint here] end
	public ResponseEntity<?> saveHemodialysisMonitoring(
			@Parameter(description = "The HemodialysisMonitoringEntity to create or update.")
			@RequestBody HemodialysisMonitoringEntityDto hemodialysisMonitoringEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			HemodialysisMonitoringEntity hemodialysisMonitoringEntity = hemodialysisMonitoringService.create(new HemodialysisMonitoringEntity(hemodialysisMonitoringEntityDto));
			return new ResponseEntity<>(hemodialysisMonitoringEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Hemodialysis Monitoring. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param hemodialysisMonitoringEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateHemodialysisMonitoring' endpoint here] off begin
	@Operation(
			description = "Update a HemodialysisMonitoring entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the HemodialysisMonitoring."),
		@ApiResponse(responseCode = "400", description = "Failed to update the HemodialysisMonitoring, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('HemodialysisMonitoringEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateHemodialysisMonitoring' endpoint here] end
	public ResponseEntity<?> updateHemodialysisMonitoring(
			@Parameter(description = "The HemodialysisMonitoringEntity to create or update.")
			@RequestBody HemodialysisMonitoringEntityDto hemodialysisMonitoringEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<HemodialysisMonitoringEntity> checkHemodialysisMonitoringExists = hemodialysisMonitoringService.findById(hemodialysisMonitoringEntityDto.getId());
		HemodialysisMonitoringEntity hemodialysisMonitoringEntity = new HemodialysisMonitoringEntity(hemodialysisMonitoringEntityDto);

		if (checkHemodialysisMonitoringExists.isPresent()) {
			try {
				hemodialysisMonitoringEntity = hemodialysisMonitoringService.update(hemodialysisMonitoringEntity);
				return new ResponseEntity<>(hemodialysisMonitoringEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(hemodialysisMonitoringEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Hemodialysis Monitoring that has the same id as the given id.
	 *
	 * @param id The id of the HemodialysisMonitoringEntity to be deleted
	...
	@Operation(
			description = "Delete a single HemodialysisMonitoring as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the HemodialysisMonitoring to delete.")
		...
		hemodialysisMonitoringService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisMonitoring' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Hemodialysis Monitoring entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisMonitoring' endpoint here] end
	public void exportHemodialysisMonitoring(
			@Parameter(description = "The ids of the HemodialysisMonitoringEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<HemodialysisMonitoringEntity> entitiesToExport = hemodialysisMonitoringService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "HemodialysisMonitoring_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (HemodialysisMonitoringEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisMonitoringExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisMonitoringExcludingIds' endpoint here] end
	public void exportHemodialysisMonitoringExcludingIds(
			@Parameter(description = "The ids of the HemodialysisMonitoringEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<HemodialysisMonitoringEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = hemodialysisMonitoringService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = hemodialysisMonitoringService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the HemodialysisMonitoring entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "HemodialysisMonitoring_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(HemodialysisMonitoringEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Hemodialysis Monitoring entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('HemodialysisMonitoringEntity', 'create')  || hasPermission('HemodialysisMonitoringEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with HemodialysisMonitoringEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("HemodialysisMonitoringEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(HemodialysisMonitoringEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<HemodialysisMonitoringEntity> csvToBean = new CsvToBeanBuilder<HemodialysisMonitoringEntity>(reader)
				.withType(HemodialysisMonitoringEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<HemodialysisMonitoringEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<HemodialysisMonitoringEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Hemodialysis Monitoring entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Hemodialysis Monitoring entity and you do not have permission");
					}
