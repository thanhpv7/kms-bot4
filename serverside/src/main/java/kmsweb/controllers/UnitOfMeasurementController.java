...
/**
 * Controller used to handle all REST operations regarding Unit Of Measurement
 */
// % protected region % [Update the API route or description for the Unit Of MeasurementController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "UnitOfMeasurementController"))
@RestController
@Slf4j
@RequestMapping("/api/unitofmeasurement")
// % protected region % [Update the API route or description for the Unit Of MeasurementController here] end
public class UnitOfMeasurementController {

	private final UnitOfMeasurementService unitOfMeasurementService;
	private final CsvUtils<UnitOfMeasurementEntity, UnitOfMeasurementRepository, UnitOfMeasurementEntityAudit,  UnitOfMeasurementService> csvUtils;
	...
	public UnitOfMeasurementController(
		...
		this.csvUtils = new CsvUtils<>(unitOfMeasurementService, UnitOfMeasurementEntity.class);
	...
	public ResponseEntity<List<UnitOfMeasurementEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<UnitOfMeasurementEntity> unitOfMeasurements = unitOfMeasurementService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(unitOfMeasurements, HttpStatus.OK);
	}

	/**
	 * Return the Unit Of Measurement that has the same id as the given id.
	 *
	 * @param id      The id of the UnitOfMeasurementEntity to be returned
	 * @return the Unit Of Measurement that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single UnitOfMeasurement as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "UnitOfMeasurement entity not found"),
	})
	@PreAuthorize("hasPermission('UnitOfMeasurementEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<UnitOfMeasurementEntity> getById(
			@Parameter(description = "The UUID of the UnitOfMeasurement to return.")
			@PathVariable("id") UUID id
	) {
		Optional<UnitOfMeasurementEntity> unitOfMeasurementEntity = unitOfMeasurementService.findById(id);
	...
	/**
	 * Create/Update the given Unit Of Measurement. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param unitOfMeasurementEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveUnitOfMeasurement' endpoint here] off begin
	@Operation(
			description = "Create a UnitOfMeasurement if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new UnitOfMeasurement."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new UnitOfMeasurement, duplicate record found")
	})
	@PreAuthorize("hasPermission('UnitOfMeasurementEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveUnitOfMeasurement' endpoint here] end
	public ResponseEntity<?> saveUnitOfMeasurement(
			@Parameter(description = "The UnitOfMeasurementEntity to create or update.")
			@RequestBody UnitOfMeasurementEntityDto unitOfMeasurementEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			UnitOfMeasurementEntity unitOfMeasurementEntity = unitOfMeasurementService.create(new UnitOfMeasurementEntity(unitOfMeasurementEntityDto));
			return new ResponseEntity<>(unitOfMeasurementEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Unit Of Measurement. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param unitOfMeasurementEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateUnitOfMeasurement' endpoint here] off begin
	@Operation(
			description = "Update a UnitOfMeasurement entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the UnitOfMeasurement."),
		@ApiResponse(responseCode = "400", description = "Failed to update the UnitOfMeasurement, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('UnitOfMeasurementEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateUnitOfMeasurement' endpoint here] end
	public ResponseEntity<?> updateUnitOfMeasurement(
			@Parameter(description = "The UnitOfMeasurementEntity to create or update.")
			@RequestBody UnitOfMeasurementEntityDto unitOfMeasurementEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<UnitOfMeasurementEntity> checkUnitOfMeasurementExists = unitOfMeasurementService.findById(unitOfMeasurementEntityDto.getId());
		UnitOfMeasurementEntity unitOfMeasurementEntity = new UnitOfMeasurementEntity(unitOfMeasurementEntityDto);

		if (checkUnitOfMeasurementExists.isPresent()) {
			try {
				unitOfMeasurementEntity = unitOfMeasurementService.update(unitOfMeasurementEntity);
				return new ResponseEntity<>(unitOfMeasurementEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(unitOfMeasurementEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Unit Of Measurement that has the same id as the given id.
	 *
	 * @param id The id of the UnitOfMeasurementEntity to be deleted
	...
	@Operation(
			description = "Delete a single UnitOfMeasurement as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the UnitOfMeasurement to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportUnitOfMeasurement' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Unit Of Measurement entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportUnitOfMeasurement' endpoint here] end
	public void exportUnitOfMeasurement(
			@Parameter(description = "The ids of the UnitOfMeasurementEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<UnitOfMeasurementEntity> entitiesToExport = unitOfMeasurementService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "UnitOfMeasurement_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (UnitOfMeasurementEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportUnitOfMeasurementExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportUnitOfMeasurementExcludingIds' endpoint here] end
	public void exportUnitOfMeasurementExcludingIds(
			@Parameter(description = "The ids of the UnitOfMeasurementEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<UnitOfMeasurementEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = unitOfMeasurementService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = unitOfMeasurementService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the UnitOfMeasurement entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "UnitOfMeasurement_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(UnitOfMeasurementEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Unit Of Measurement entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('UnitOfMeasurementEntity', 'create')  || hasPermission('UnitOfMeasurementEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with UnitOfMeasurementEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("UnitOfMeasurementEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(UnitOfMeasurementEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<UnitOfMeasurementEntity> csvToBean = new CsvToBeanBuilder<UnitOfMeasurementEntity>(reader)
				.withType(UnitOfMeasurementEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<UnitOfMeasurementEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<UnitOfMeasurementEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Unit Of Measurement entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Unit Of Measurement entity and you do not have permission");
					}
