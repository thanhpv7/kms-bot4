...
import kmsweb.entities.BpjsJasaRaharjaEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Jasa Raharja
 */
// % protected region % [Update the API route or description for the BPJS Jasa RaharjaController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsJasaRaharjaController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsjasaraharja")
// % protected region % [Update the API route or description for the BPJS Jasa RaharjaController here] end
public class BpjsJasaRaharjaController {

	private final BpjsJasaRaharjaService bpjsJasaRaharjaService;
	private final CsvUtils<BpjsJasaRaharjaEntity, BpjsJasaRaharjaRepository, BpjsJasaRaharjaEntityAudit,  BpjsJasaRaharjaService> csvUtils;
	...
	@Autowired
	public BpjsJasaRaharjaController(
		...
		this.csvUtils = new CsvUtils<>(bpjsJasaRaharjaService, BpjsJasaRaharjaEntity.class);
	...
	public ResponseEntity<List<BpjsJasaRaharjaEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsJasaRaharjaEntity> bpjsJasaRaharjas = bpjsJasaRaharjaService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsJasaRaharjas, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Jasa Raharja that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsJasaRaharjaEntity to be returned
	 * @return the BPJS Jasa Raharja that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsJasaRaharja as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsJasaRaharja entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsJasaRaharjaEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsJasaRaharjaEntity> getById(
			@Parameter(description = "The UUID of the BpjsJasaRaharja to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsJasaRaharjaEntity> bpjsJasaRaharjaEntity = bpjsJasaRaharjaService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Jasa Raharja. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsJasaRaharjaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsJasaRaharja' endpoint here] off begin
	@Operation(
			description = "Create a BpjsJasaRaharja if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsJasaRaharja."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsJasaRaharja, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsJasaRaharjaEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsJasaRaharja' endpoint here] end
	public ResponseEntity<?> saveBpjsJasaRaharja(
			@Parameter(description = "The BpjsJasaRaharjaEntity to create or update.")
			@RequestBody BpjsJasaRaharjaEntityDto bpjsJasaRaharjaEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsJasaRaharjaEntity bpjsJasaRaharjaEntity = bpjsJasaRaharjaService.create(new BpjsJasaRaharjaEntity(bpjsJasaRaharjaEntityDto));
			return new ResponseEntity<>(bpjsJasaRaharjaEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Jasa Raharja. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsJasaRaharjaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsJasaRaharja' endpoint here] off begin
	@Operation(
			description = "Update a BpjsJasaRaharja entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsJasaRaharja."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsJasaRaharja, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsJasaRaharjaEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsJasaRaharja' endpoint here] end
	public ResponseEntity<?> updateBpjsJasaRaharja(
			@Parameter(description = "The BpjsJasaRaharjaEntity to create or update.")
			@RequestBody BpjsJasaRaharjaEntityDto bpjsJasaRaharjaEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsJasaRaharjaEntity> checkBpjsJasaRaharjaExists = bpjsJasaRaharjaService.findById(bpjsJasaRaharjaEntityDto.getId());
		BpjsJasaRaharjaEntity bpjsJasaRaharjaEntity = new BpjsJasaRaharjaEntity(bpjsJasaRaharjaEntityDto);

		if (checkBpjsJasaRaharjaExists.isPresent()) {
			try {
				bpjsJasaRaharjaEntity = bpjsJasaRaharjaService.update(bpjsJasaRaharjaEntity);
				return new ResponseEntity<>(bpjsJasaRaharjaEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsJasaRaharjaEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Jasa Raharja that has the same id as the given id.
	 *
	 * @param id The id of the BpjsJasaRaharjaEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsJasaRaharja as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsJasaRaharja to delete.")
		...
		bpjsJasaRaharjaService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsJasaRaharja' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Jasa Raharja entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsJasaRaharja' endpoint here] end
	public void exportBpjsJasaRaharja(
			@Parameter(description = "The ids of the BpjsJasaRaharjaEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsJasaRaharjaEntity> entitiesToExport = bpjsJasaRaharjaService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsJasaRaharja_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsJasaRaharjaEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsJasaRaharjaExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsJasaRaharjaExcludingIds' endpoint here] end
	public void exportBpjsJasaRaharjaExcludingIds(
			@Parameter(description = "The ids of the BpjsJasaRaharjaEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsJasaRaharjaEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsJasaRaharjaService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsJasaRaharjaService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsJasaRaharja entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsJasaRaharja_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsJasaRaharjaEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Jasa Raharja entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsJasaRaharjaEntity', 'create')  || hasPermission('BpjsJasaRaharjaEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsJasaRaharjaEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsJasaRaharjaEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsJasaRaharjaEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsJasaRaharjaEntity> csvToBean = new CsvToBeanBuilder<BpjsJasaRaharjaEntity>(reader)
				.withType(BpjsJasaRaharjaEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsJasaRaharjaEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsJasaRaharjaEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Jasa Raharja entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Jasa Raharja entity and you do not have permission");
					}
