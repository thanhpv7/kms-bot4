...
/**
 * Controller used to handle all REST operations regarding BPJS Pcare Peserta Kegiatan Kelompok
 */
// % protected region % [Update the API route or description for the BPJS Pcare Peserta Kegiatan KelompokController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcarePesertaKegiatanKelompokController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcarepesertakegiatankelompok")
// % protected region % [Update the API route or description for the BPJS Pcare Peserta Kegiatan KelompokController here] end
public class BpjsPcarePesertaKegiatanKelompokController {

	private final BpjsPcarePesertaKegiatanKelompokService bpjsPcarePesertaKegiatanKelompokService;
	private final CsvUtils<BpjsPcarePesertaKegiatanKelompokEntity, BpjsPcarePesertaKegiatanKelompokRepository, BpjsPcarePesertaKegiatanKelompokEntityAudit,  BpjsPcarePesertaKegiatanKelompokService> csvUtils;
	...
	@Autowired
	public BpjsPcarePesertaKegiatanKelompokController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcarePesertaKegiatanKelompokService, BpjsPcarePesertaKegiatanKelompokEntity.class);
	...
	/**
	 * Return all the BPJS Pcare Peserta Kegiatan Kelompoks.
	 *
	 * @return all the BPJS Pcare Peserta Kegiatan Kelompoks
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsPcarePesertaKegiatanKelompoks",
		security = {
	...
	public ResponseEntity<List<BpjsPcarePesertaKegiatanKelompokEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcarePesertaKegiatanKelompokEntity> bpjsPcarePesertaKegiatanKelompoks = bpjsPcarePesertaKegiatanKelompokService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcarePesertaKegiatanKelompoks, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Pcare Peserta Kegiatan Kelompok that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcarePesertaKegiatanKelompokEntity to be returned
	 * @return the BPJS Pcare Peserta Kegiatan Kelompok that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcarePesertaKegiatanKelompok as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcarePesertaKegiatanKelompok entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcarePesertaKegiatanKelompokEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcarePesertaKegiatanKelompokEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcarePesertaKegiatanKelompok to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcarePesertaKegiatanKelompokEntity> bpjsPcarePesertaKegiatanKelompokEntity = bpjsPcarePesertaKegiatanKelompokService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Pcare Peserta Kegiatan Kelompok. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcarePesertaKegiatanKelompokEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcarePesertaKegiatanKelompok' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcarePesertaKegiatanKelompok if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcarePesertaKegiatanKelompok."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcarePesertaKegiatanKelompok, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcarePesertaKegiatanKelompokEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcarePesertaKegiatanKelompok' endpoint here] end
	public ResponseEntity<?> saveBpjsPcarePesertaKegiatanKelompok(
			@Parameter(description = "The BpjsPcarePesertaKegiatanKelompokEntity to create or update.")
			@RequestBody BpjsPcarePesertaKegiatanKelompokEntityDto bpjsPcarePesertaKegiatanKelompokEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcarePesertaKegiatanKelompokEntity bpjsPcarePesertaKegiatanKelompokEntity = bpjsPcarePesertaKegiatanKelompokService.create(new BpjsPcarePesertaKegiatanKelompokEntity(bpjsPcarePesertaKegiatanKelompokEntityDto));
			return new ResponseEntity<>(bpjsPcarePesertaKegiatanKelompokEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Pcare Peserta Kegiatan Kelompok. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcarePesertaKegiatanKelompokEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcarePesertaKegiatanKelompok' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcarePesertaKegiatanKelompok entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcarePesertaKegiatanKelompok."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcarePesertaKegiatanKelompok, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcarePesertaKegiatanKelompokEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcarePesertaKegiatanKelompok' endpoint here] end
	public ResponseEntity<?> updateBpjsPcarePesertaKegiatanKelompok(
			@Parameter(description = "The BpjsPcarePesertaKegiatanKelompokEntity to create or update.")
			@RequestBody BpjsPcarePesertaKegiatanKelompokEntityDto bpjsPcarePesertaKegiatanKelompokEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcarePesertaKegiatanKelompokEntity> checkBpjsPcarePesertaKegiatanKelompokExists = bpjsPcarePesertaKegiatanKelompokService.findById(bpjsPcarePesertaKegiatanKelompokEntityDto.getId());
		BpjsPcarePesertaKegiatanKelompokEntity bpjsPcarePesertaKegiatanKelompokEntity = new BpjsPcarePesertaKegiatanKelompokEntity(bpjsPcarePesertaKegiatanKelompokEntityDto);

		if (checkBpjsPcarePesertaKegiatanKelompokExists.isPresent()) {
			try {
				bpjsPcarePesertaKegiatanKelompokEntity = bpjsPcarePesertaKegiatanKelompokService.update(bpjsPcarePesertaKegiatanKelompokEntity);
				return new ResponseEntity<>(bpjsPcarePesertaKegiatanKelompokEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcarePesertaKegiatanKelompokEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Pcare Peserta Kegiatan Kelompok that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcarePesertaKegiatanKelompokEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcarePesertaKegiatanKelompok as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcarePesertaKegiatanKelompok to delete.")
		...
		bpjsPcarePesertaKegiatanKelompokService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePesertaKegiatanKelompok' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Pcare Peserta Kegiatan Kelompok entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePesertaKegiatanKelompok' endpoint here] end
	public void exportBpjsPcarePesertaKegiatanKelompok(
			@Parameter(description = "The ids of the BpjsPcarePesertaKegiatanKelompokEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcarePesertaKegiatanKelompokEntity> entitiesToExport = bpjsPcarePesertaKegiatanKelompokService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcarePesertaKegiatanKelompok_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcarePesertaKegiatanKelompokEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePesertaKegiatanKelompokExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcarePesertaKegiatanKelompokExcludingIds' endpoint here] end
	public void exportBpjsPcarePesertaKegiatanKelompokExcludingIds(
			@Parameter(description = "The ids of the BpjsPcarePesertaKegiatanKelompokEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcarePesertaKegiatanKelompokEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcarePesertaKegiatanKelompokService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcarePesertaKegiatanKelompokService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcarePesertaKegiatanKelompok entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcarePesertaKegiatanKelompok_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcarePesertaKegiatanKelompokEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Pcare Peserta Kegiatan Kelompok entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcarePesertaKegiatanKelompokEntity', 'create')  || hasPermission('BpjsPcarePesertaKegiatanKelompokEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcarePesertaKegiatanKelompokEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcarePesertaKegiatanKelompokEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcarePesertaKegiatanKelompokEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcarePesertaKegiatanKelompokEntity> csvToBean = new CsvToBeanBuilder<BpjsPcarePesertaKegiatanKelompokEntity>(reader)
				.withType(BpjsPcarePesertaKegiatanKelompokEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcarePesertaKegiatanKelompokEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcarePesertaKegiatanKelompokEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Pcare Peserta Kegiatan Kelompok entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Pcare Peserta Kegiatan Kelompok entity and you do not have permission");
					}
