...
import kmsweb.entities.NhisClaimInvestigationDetailEntityAudit;
import kmsweb.repositories.NhisClaimInvestigationDetailRepository;
import kmsweb.services.NhisClaimInvestigationDetailService;
...
/**
 * Controller used to handle all REST operations regarding NHIS Claim Investigation Detail
 */
// % protected region % [Update the API route or description for the NHIS Claim Investigation DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisClaimInvestigationDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/nhisclaiminvestigationdetail")
// % protected region % [Update the API route or description for the NHIS Claim Investigation DetailController here] end
public class NhisClaimInvestigationDetailController {

	private final NhisClaimInvestigationDetailService nhisClaimInvestigationDetailService;
	private final CsvUtils<NhisClaimInvestigationDetailEntity, NhisClaimInvestigationDetailRepository, NhisClaimInvestigationDetailEntityAudit,  NhisClaimInvestigationDetailService> csvUtils;
	...
	@Autowired
	public NhisClaimInvestigationDetailController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NhisClaimInvestigationDetailService nhisClaimInvestigationDetailService
	) {
		this.authService = authService;
		this.nhisClaimInvestigationDetailService = nhisClaimInvestigationDetailService;
		this.csvUtils = new CsvUtils<>(nhisClaimInvestigationDetailService, NhisClaimInvestigationDetailEntity.class);
	...
	/**
	 * Return all the NHIS Claim Investigation Details.
	 *
	 * @return all the NHIS Claim Investigation Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of NhisClaimInvestigationDetails",
		security = {
	...
	@PreAuthorize("hasPermission('NhisClaimInvestigationDetailEntity', 'read')")
	@GetMapping(produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] end
	public ResponseEntity<List<NhisClaimInvestigationDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisClaimInvestigationDetailEntity> nhisClaimInvestigationDetails = nhisClaimInvestigationDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisClaimInvestigationDetails, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Claim Investigation Detail that has the same id as the given id.
	 *
	 * @param id      The id of the NhisClaimInvestigationDetailEntity to be returned
	 * @return the NHIS Claim Investigation Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisClaimInvestigationDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisClaimInvestigationDetail entity not found"),
	})
	@PreAuthorize("hasPermission('NhisClaimInvestigationDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisClaimInvestigationDetailEntity> getById(
			@Parameter(description = "The UUID of the NhisClaimInvestigationDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisClaimInvestigationDetailEntity> nhisClaimInvestigationDetailEntity = nhisClaimInvestigationDetailService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Claim Investigation Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisClaimInvestigationDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisClaimInvestigationDetail' endpoint here] off begin
	@Operation(
			description = "Create a NhisClaimInvestigationDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisClaimInvestigationDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisClaimInvestigationDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisClaimInvestigationDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisClaimInvestigationDetail' endpoint here] end
	public ResponseEntity<?> saveNhisClaimInvestigationDetail(
			@Parameter(description = "The NhisClaimInvestigationDetailEntity to create or update.")
			@RequestBody NhisClaimInvestigationDetailEntityDto nhisClaimInvestigationDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisClaimInvestigationDetailEntity nhisClaimInvestigationDetailEntity = nhisClaimInvestigationDetailService.create(new NhisClaimInvestigationDetailEntity(nhisClaimInvestigationDetailEntityDto));
			return new ResponseEntity<>(nhisClaimInvestigationDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Claim Investigation Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisClaimInvestigationDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisClaimInvestigationDetail' endpoint here] off begin
	@Operation(
			description = "Update a NhisClaimInvestigationDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisClaimInvestigationDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisClaimInvestigationDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisClaimInvestigationDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisClaimInvestigationDetail' endpoint here] end
	public ResponseEntity<?> updateNhisClaimInvestigationDetail(
			@Parameter(description = "The NhisClaimInvestigationDetailEntity to create or update.")
			@RequestBody NhisClaimInvestigationDetailEntityDto nhisClaimInvestigationDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisClaimInvestigationDetailEntity> checkNhisClaimInvestigationDetailExists = nhisClaimInvestigationDetailService.findById(nhisClaimInvestigationDetailEntityDto.getId());
		NhisClaimInvestigationDetailEntity nhisClaimInvestigationDetailEntity = new NhisClaimInvestigationDetailEntity(nhisClaimInvestigationDetailEntityDto);

		if (checkNhisClaimInvestigationDetailExists.isPresent()) {
			try {
				nhisClaimInvestigationDetailEntity = nhisClaimInvestigationDetailService.update(nhisClaimInvestigationDetailEntity);
				return new ResponseEntity<>(nhisClaimInvestigationDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisClaimInvestigationDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Claim Investigation Detail that has the same id as the given id.
	 *
	 * @param id The id of the NhisClaimInvestigationDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisClaimInvestigationDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('NhisClaimInvestigationDetailEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisClaimInvestigationDetail to delete.")
		...
		nhisClaimInvestigationDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimInvestigationDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Claim Investigation Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimInvestigationDetail' endpoint here] end
	public void exportNhisClaimInvestigationDetail(
			@Parameter(description = "The ids of the NhisClaimInvestigationDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisClaimInvestigationDetailEntity> entitiesToExport = nhisClaimInvestigationDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisClaimInvestigationDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisClaimInvestigationDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimInvestigationDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimInvestigationDetailExcludingIds' endpoint here] end
	public void exportNhisClaimInvestigationDetailExcludingIds(
			@Parameter(description = "The ids of the NhisClaimInvestigationDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisClaimInvestigationDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisClaimInvestigationDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisClaimInvestigationDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisClaimInvestigationDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisClaimInvestigationDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisClaimInvestigationDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Claim Investigation Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisClaimInvestigationDetailEntity', 'create')  || hasPermission('NhisClaimInvestigationDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisClaimInvestigationDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisClaimInvestigationDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisClaimInvestigationDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisClaimInvestigationDetailEntity> csvToBean = new CsvToBeanBuilder<NhisClaimInvestigationDetailEntity>(reader)
				.withType(NhisClaimInvestigationDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisClaimInvestigationDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisClaimInvestigationDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Claim Investigation Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Claim Investigation Detail entity and you do not have permission");
					}
