...
import kmsweb.entities.SupplierContactDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Supplier Contact Detail
 */
// % protected region % [Update the API route or description for the Supplier Contact DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SupplierContactDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/suppliercontactdetail")
// % protected region % [Update the API route or description for the Supplier Contact DetailController here] end
public class SupplierContactDetailController {

	private final SupplierContactDetailService supplierContactDetailService;
	private final CsvUtils<SupplierContactDetailEntity, SupplierContactDetailRepository, SupplierContactDetailEntityAudit,  SupplierContactDetailService> csvUtils;
	...
	@Autowired
	public SupplierContactDetailController(
		...
		this.csvUtils = new CsvUtils<>(supplierContactDetailService, SupplierContactDetailEntity.class);
	...
	public ResponseEntity<List<SupplierContactDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SupplierContactDetailEntity> supplierContactDetails = supplierContactDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(supplierContactDetails, HttpStatus.OK);
	}

	/**
	 * Return the Supplier Contact Detail that has the same id as the given id.
	 *
	 * @param id      The id of the SupplierContactDetailEntity to be returned
	 * @return the Supplier Contact Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single SupplierContactDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "SupplierContactDetail entity not found"),
	})
	@PreAuthorize("hasPermission('SupplierContactDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SupplierContactDetailEntity> getById(
			@Parameter(description = "The UUID of the SupplierContactDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SupplierContactDetailEntity> supplierContactDetailEntity = supplierContactDetailService.findById(id);
	...
	/**
	 * Create/Update the given Supplier Contact Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param supplierContactDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSupplierContactDetail' endpoint here] off begin
	@Operation(
			description = "Create a SupplierContactDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new SupplierContactDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new SupplierContactDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('SupplierContactDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSupplierContactDetail' endpoint here] end
	public ResponseEntity<?> saveSupplierContactDetail(
			@Parameter(description = "The SupplierContactDetailEntity to create or update.")
			@RequestBody SupplierContactDetailEntityDto supplierContactDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SupplierContactDetailEntity supplierContactDetailEntity = supplierContactDetailService.create(new SupplierContactDetailEntity(supplierContactDetailEntityDto));
			return new ResponseEntity<>(supplierContactDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Supplier Contact Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param supplierContactDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSupplierContactDetail' endpoint here] off begin
	@Operation(
			description = "Update a SupplierContactDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the SupplierContactDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the SupplierContactDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SupplierContactDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSupplierContactDetail' endpoint here] end
	public ResponseEntity<?> updateSupplierContactDetail(
			@Parameter(description = "The SupplierContactDetailEntity to create or update.")
			@RequestBody SupplierContactDetailEntityDto supplierContactDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SupplierContactDetailEntity> checkSupplierContactDetailExists = supplierContactDetailService.findById(supplierContactDetailEntityDto.getId());
		SupplierContactDetailEntity supplierContactDetailEntity = new SupplierContactDetailEntity(supplierContactDetailEntityDto);

		if (checkSupplierContactDetailExists.isPresent()) {
			try {
				supplierContactDetailEntity = supplierContactDetailService.update(supplierContactDetailEntity);
				return new ResponseEntity<>(supplierContactDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(supplierContactDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Supplier Contact Detail that has the same id as the given id.
	 *
	 * @param id The id of the SupplierContactDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single SupplierContactDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the SupplierContactDetail to delete.")
		...
		supplierContactDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplierContactDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Supplier Contact Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplierContactDetail' endpoint here] end
	public void exportSupplierContactDetail(
			@Parameter(description = "The ids of the SupplierContactDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SupplierContactDetailEntity> entitiesToExport = supplierContactDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "SupplierContactDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SupplierContactDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplierContactDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplierContactDetailExcludingIds' endpoint here] end
	public void exportSupplierContactDetailExcludingIds(
			@Parameter(description = "The ids of the SupplierContactDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SupplierContactDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = supplierContactDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = supplierContactDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the SupplierContactDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "SupplierContactDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SupplierContactDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Supplier Contact Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SupplierContactDetailEntity', 'create')  || hasPermission('SupplierContactDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SupplierContactDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SupplierContactDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SupplierContactDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SupplierContactDetailEntity> csvToBean = new CsvToBeanBuilder<SupplierContactDetailEntity>(reader)
				.withType(SupplierContactDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<SupplierContactDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<SupplierContactDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Supplier Contact Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Supplier Contact Detail entity and you do not have permission");
					}
