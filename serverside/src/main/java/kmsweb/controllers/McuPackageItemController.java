...
import kmsweb.entities.McuPackageItemEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding MCU Package Item
 */
// % protected region % [Update the API route or description for the MCU Package ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "McuPackageItemController"))
@RestController
@Slf4j
@RequestMapping("/api/mcupackageitem")
// % protected region % [Update the API route or description for the MCU Package ItemController here] end
public class McuPackageItemController {

	private final McuPackageItemService mcuPackageItemService;
	private final CsvUtils<McuPackageItemEntity, McuPackageItemRepository, McuPackageItemEntityAudit,  McuPackageItemService> csvUtils;
	...
	@Autowired
	public McuPackageItemController(
		...
		this.csvUtils = new CsvUtils<>(mcuPackageItemService, McuPackageItemEntity.class);
	...
	public ResponseEntity<List<McuPackageItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<McuPackageItemEntity> mcuPackageItems = mcuPackageItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(mcuPackageItems, HttpStatus.OK);
	}

	/**
	 * Return the MCU Package Item that has the same id as the given id.
	 *
	 * @param id      The id of the McuPackageItemEntity to be returned
	 * @return the MCU Package Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single McuPackageItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "McuPackageItem entity not found"),
	})
	@PreAuthorize("hasPermission('McuPackageItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<McuPackageItemEntity> getById(
			@Parameter(description = "The UUID of the McuPackageItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<McuPackageItemEntity> mcuPackageItemEntity = mcuPackageItemService.findById(id);
	...
	/**
	 * Create/Update the given MCU Package Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param mcuPackageItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMcuPackageItem' endpoint here] off begin
	@Operation(
			description = "Create a McuPackageItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new McuPackageItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new McuPackageItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('McuPackageItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMcuPackageItem' endpoint here] end
	public ResponseEntity<?> saveMcuPackageItem(
			@Parameter(description = "The McuPackageItemEntity to create or update.")
			@RequestBody McuPackageItemEntityDto mcuPackageItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			McuPackageItemEntity mcuPackageItemEntity = mcuPackageItemService.create(new McuPackageItemEntity(mcuPackageItemEntityDto));
			return new ResponseEntity<>(mcuPackageItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given MCU Package Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param mcuPackageItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMcuPackageItem' endpoint here] off begin
	@Operation(
			description = "Update a McuPackageItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the McuPackageItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the McuPackageItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('McuPackageItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMcuPackageItem' endpoint here] end
	public ResponseEntity<?> updateMcuPackageItem(
			@Parameter(description = "The McuPackageItemEntity to create or update.")
			@RequestBody McuPackageItemEntityDto mcuPackageItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<McuPackageItemEntity> checkMcuPackageItemExists = mcuPackageItemService.findById(mcuPackageItemEntityDto.getId());
		McuPackageItemEntity mcuPackageItemEntity = new McuPackageItemEntity(mcuPackageItemEntityDto);

		if (checkMcuPackageItemExists.isPresent()) {
			try {
				mcuPackageItemEntity = mcuPackageItemService.update(mcuPackageItemEntity);
				return new ResponseEntity<>(mcuPackageItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(mcuPackageItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the MCU Package Item that has the same id as the given id.
	 *
	 * @param id The id of the McuPackageItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single McuPackageItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the McuPackageItem to delete.")
		...
		mcuPackageItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMcuPackageItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all MCU Package Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMcuPackageItem' endpoint here] end
	public void exportMcuPackageItem(
			@Parameter(description = "The ids of the McuPackageItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<McuPackageItemEntity> entitiesToExport = mcuPackageItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "McuPackageItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (McuPackageItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMcuPackageItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMcuPackageItemExcludingIds' endpoint here] end
	public void exportMcuPackageItemExcludingIds(
			@Parameter(description = "The ids of the McuPackageItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<McuPackageItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = mcuPackageItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = mcuPackageItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the McuPackageItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "McuPackageItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(McuPackageItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for MCU Package Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('McuPackageItemEntity', 'create')  || hasPermission('McuPackageItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with McuPackageItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("McuPackageItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(McuPackageItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<McuPackageItemEntity> csvToBean = new CsvToBeanBuilder<McuPackageItemEntity>(reader)
				.withType(McuPackageItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<McuPackageItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<McuPackageItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a MCU Package Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a MCU Package Item entity and you do not have permission");
					}
