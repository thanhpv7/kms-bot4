...
/**
 * Controller used to handle all REST operations regarding BPJS Pcare Kegiatan Kelompok
 */
// % protected region % [Update the API route or description for the BPJS Pcare Kegiatan KelompokController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcareKegiatanKelompokController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcarekegiatankelompok")
// % protected region % [Update the API route or description for the BPJS Pcare Kegiatan KelompokController here] end
public class BpjsPcareKegiatanKelompokController {

	private final BpjsPcareKegiatanKelompokService bpjsPcareKegiatanKelompokService;
	private final CsvUtils<BpjsPcareKegiatanKelompokEntity, BpjsPcareKegiatanKelompokRepository, BpjsPcareKegiatanKelompokEntityAudit,  BpjsPcareKegiatanKelompokService> csvUtils;
	...
	@Autowired
	public BpjsPcareKegiatanKelompokController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcareKegiatanKelompokService, BpjsPcareKegiatanKelompokEntity.class);
	...
	/**
	 * Return all the BPJS Pcare Kegiatan Kelompoks.
	 *
	 * @return all the BPJS Pcare Kegiatan Kelompoks
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsPcareKegiatanKelompoks",
		security = {
	...
	public ResponseEntity<List<BpjsPcareKegiatanKelompokEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcareKegiatanKelompokEntity> bpjsPcareKegiatanKelompoks = bpjsPcareKegiatanKelompokService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcareKegiatanKelompoks, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Pcare Kegiatan Kelompok that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcareKegiatanKelompokEntity to be returned
	 * @return the BPJS Pcare Kegiatan Kelompok that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcareKegiatanKelompok as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcareKegiatanKelompok entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcareKegiatanKelompokEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcareKegiatanKelompokEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcareKegiatanKelompok to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcareKegiatanKelompokEntity> bpjsPcareKegiatanKelompokEntity = bpjsPcareKegiatanKelompokService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Pcare Kegiatan Kelompok. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareKegiatanKelompokEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareKegiatanKelompok' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcareKegiatanKelompok if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcareKegiatanKelompok."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcareKegiatanKelompok, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcareKegiatanKelompokEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareKegiatanKelompok' endpoint here] end
	public ResponseEntity<?> saveBpjsPcareKegiatanKelompok(
			@Parameter(description = "The BpjsPcareKegiatanKelompokEntity to create or update.")
			@RequestBody BpjsPcareKegiatanKelompokEntityDto bpjsPcareKegiatanKelompokEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcareKegiatanKelompokEntity bpjsPcareKegiatanKelompokEntity = bpjsPcareKegiatanKelompokService.create(new BpjsPcareKegiatanKelompokEntity(bpjsPcareKegiatanKelompokEntityDto));
			return new ResponseEntity<>(bpjsPcareKegiatanKelompokEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Pcare Kegiatan Kelompok. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareKegiatanKelompokEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareKegiatanKelompok' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcareKegiatanKelompok entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcareKegiatanKelompok."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcareKegiatanKelompok, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcareKegiatanKelompokEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareKegiatanKelompok' endpoint here] end
	public ResponseEntity<?> updateBpjsPcareKegiatanKelompok(
			@Parameter(description = "The BpjsPcareKegiatanKelompokEntity to create or update.")
			@RequestBody BpjsPcareKegiatanKelompokEntityDto bpjsPcareKegiatanKelompokEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcareKegiatanKelompokEntity> checkBpjsPcareKegiatanKelompokExists = bpjsPcareKegiatanKelompokService.findById(bpjsPcareKegiatanKelompokEntityDto.getId());
		BpjsPcareKegiatanKelompokEntity bpjsPcareKegiatanKelompokEntity = new BpjsPcareKegiatanKelompokEntity(bpjsPcareKegiatanKelompokEntityDto);

		if (checkBpjsPcareKegiatanKelompokExists.isPresent()) {
			try {
				bpjsPcareKegiatanKelompokEntity = bpjsPcareKegiatanKelompokService.update(bpjsPcareKegiatanKelompokEntity);
				return new ResponseEntity<>(bpjsPcareKegiatanKelompokEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcareKegiatanKelompokEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Pcare Kegiatan Kelompok that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcareKegiatanKelompokEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcareKegiatanKelompok as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcareKegiatanKelompok to delete.")
		...
		bpjsPcareKegiatanKelompokService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKegiatanKelompok' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Pcare Kegiatan Kelompok entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKegiatanKelompok' endpoint here] end
	public void exportBpjsPcareKegiatanKelompok(
			@Parameter(description = "The ids of the BpjsPcareKegiatanKelompokEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcareKegiatanKelompokEntity> entitiesToExport = bpjsPcareKegiatanKelompokService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcareKegiatanKelompok_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcareKegiatanKelompokEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKegiatanKelompokExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKegiatanKelompokExcludingIds' endpoint here] end
	public void exportBpjsPcareKegiatanKelompokExcludingIds(
			@Parameter(description = "The ids of the BpjsPcareKegiatanKelompokEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcareKegiatanKelompokEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcareKegiatanKelompokService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcareKegiatanKelompokService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcareKegiatanKelompok entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcareKegiatanKelompok_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcareKegiatanKelompokEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Pcare Kegiatan Kelompok entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcareKegiatanKelompokEntity', 'create')  || hasPermission('BpjsPcareKegiatanKelompokEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcareKegiatanKelompokEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcareKegiatanKelompokEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcareKegiatanKelompokEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcareKegiatanKelompokEntity> csvToBean = new CsvToBeanBuilder<BpjsPcareKegiatanKelompokEntity>(reader)
				.withType(BpjsPcareKegiatanKelompokEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcareKegiatanKelompokEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcareKegiatanKelompokEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Pcare Kegiatan Kelompok entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Pcare Kegiatan Kelompok entity and you do not have permission");
					}
