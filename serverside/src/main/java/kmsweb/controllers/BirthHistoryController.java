...
import kmsweb.entities.BirthHistoryEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Birth History
 */
// % protected region % [Update the API route or description for the Birth HistoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BirthHistoryController"))
@RestController
@Slf4j
@RequestMapping("/api/birthhistory")
// % protected region % [Update the API route or description for the Birth HistoryController here] end
public class BirthHistoryController {

	private final BirthHistoryService birthHistoryService;
	private final CsvUtils<BirthHistoryEntity, BirthHistoryRepository, BirthHistoryEntityAudit,  BirthHistoryService> csvUtils;
	...
	public BirthHistoryController(
		...
		this.csvUtils = new CsvUtils<>(birthHistoryService, BirthHistoryEntity.class);
	...
	public ResponseEntity<List<BirthHistoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BirthHistoryEntity> birthHistorys = birthHistoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(birthHistorys, HttpStatus.OK);
	}

	/**
	 * Return the Birth History that has the same id as the given id.
	 *
	 * @param id      The id of the BirthHistoryEntity to be returned
	 * @return the Birth History that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BirthHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BirthHistory entity not found"),
	})
	@PreAuthorize("hasPermission('BirthHistoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BirthHistoryEntity> getById(
			@Parameter(description = "The UUID of the BirthHistory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BirthHistoryEntity> birthHistoryEntity = birthHistoryService.findById(id);
	...
	/**
	 * Create/Update the given Birth History. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param birthHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBirthHistory' endpoint here] off begin
	@Operation(
			description = "Create a BirthHistory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BirthHistory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BirthHistory, duplicate record found")
	})
	@PreAuthorize("hasPermission('BirthHistoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBirthHistory' endpoint here] end
	public ResponseEntity<?> saveBirthHistory(
			@Parameter(description = "The BirthHistoryEntity to create or update.")
			@RequestBody BirthHistoryEntityDto birthHistoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BirthHistoryEntity birthHistoryEntity = birthHistoryService.create(new BirthHistoryEntity(birthHistoryEntityDto));
			return new ResponseEntity<>(birthHistoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Birth History. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param birthHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBirthHistory' endpoint here] off begin
	@Operation(
			description = "Update a BirthHistory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BirthHistory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BirthHistory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BirthHistoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBirthHistory' endpoint here] end
	public ResponseEntity<?> updateBirthHistory(
			@Parameter(description = "The BirthHistoryEntity to create or update.")
			@RequestBody BirthHistoryEntityDto birthHistoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BirthHistoryEntity> checkBirthHistoryExists = birthHistoryService.findById(birthHistoryEntityDto.getId());
		BirthHistoryEntity birthHistoryEntity = new BirthHistoryEntity(birthHistoryEntityDto);

		if (checkBirthHistoryExists.isPresent()) {
			try {
				birthHistoryEntity = birthHistoryService.update(birthHistoryEntity);
				return new ResponseEntity<>(birthHistoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(birthHistoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Birth History that has the same id as the given id.
	 *
	 * @param id The id of the BirthHistoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single BirthHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BirthHistory to delete.")
		...
		birthHistoryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBirthHistory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Birth History entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBirthHistory' endpoint here] end
	public void exportBirthHistory(
			@Parameter(description = "The ids of the BirthHistoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BirthHistoryEntity> entitiesToExport = birthHistoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BirthHistory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BirthHistoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBirthHistoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBirthHistoryExcludingIds' endpoint here] end
	public void exportBirthHistoryExcludingIds(
			@Parameter(description = "The ids of the BirthHistoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BirthHistoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = birthHistoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = birthHistoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BirthHistory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BirthHistory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BirthHistoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Birth History entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BirthHistoryEntity', 'create')  || hasPermission('BirthHistoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BirthHistoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BirthHistoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BirthHistoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BirthHistoryEntity> csvToBean = new CsvToBeanBuilder<BirthHistoryEntity>(reader)
				.withType(BirthHistoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BirthHistoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BirthHistoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Birth History entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Birth History entity and you do not have permission");
					}
