...
import kmsweb.entities.CashAllocationEntityAudit;
import kmsweb.repositories.CashAllocationRepository;
import kmsweb.services.CashAllocationService;
...
/**
 * Controller used to handle all REST operations regarding Cash Allocation
 */
// % protected region % [Update the API route or description for the Cash AllocationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CashAllocationController"))
@RestController
@Slf4j
@RequestMapping("/api/cashallocation")
// % protected region % [Update the API route or description for the Cash AllocationController here] end
public class CashAllocationController {

	private final CashAllocationService cashAllocationService;
	private final CsvUtils<CashAllocationEntity, CashAllocationRepository, CashAllocationEntityAudit,  CashAllocationService> csvUtils;
	...
	@Autowired
	public CashAllocationController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			CashAllocationService cashAllocationService
	) {
		this.authService = authService;
		this.cashAllocationService = cashAllocationService;
		this.csvUtils = new CsvUtils<>(cashAllocationService, CashAllocationEntity.class);
	...
	public ResponseEntity<List<CashAllocationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CashAllocationEntity> cashAllocations = cashAllocationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(cashAllocations, HttpStatus.OK);
	}

	/**
	 * Return the Cash Allocation that has the same id as the given id.
	 *
	 * @param id      The id of the CashAllocationEntity to be returned
	 * @return the Cash Allocation that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CashAllocation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CashAllocation entity not found"),
	})
	@PreAuthorize("hasPermission('CashAllocationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CashAllocationEntity> getById(
			@Parameter(description = "The UUID of the CashAllocation to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CashAllocationEntity> cashAllocationEntity = cashAllocationService.findById(id);
	...
	/**
	 * Create/Update the given Cash Allocation. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param cashAllocationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCashAllocation' endpoint here] off begin
	@Operation(
			description = "Create a CashAllocation if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CashAllocation."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CashAllocation, duplicate record found")
	})
	@PreAuthorize("hasPermission('CashAllocationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCashAllocation' endpoint here] end
	public ResponseEntity<?> saveCashAllocation(
			@Parameter(description = "The CashAllocationEntity to create or update.")
			@RequestBody CashAllocationEntityDto cashAllocationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CashAllocationEntity cashAllocationEntity = cashAllocationService.create(new CashAllocationEntity(cashAllocationEntityDto));
			return new ResponseEntity<>(cashAllocationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Cash Allocation. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param cashAllocationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCashAllocation' endpoint here] off begin
	@Operation(
			description = "Update a CashAllocation entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CashAllocation."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CashAllocation, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CashAllocationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCashAllocation' endpoint here] end
	public ResponseEntity<?> updateCashAllocation(
			@Parameter(description = "The CashAllocationEntity to create or update.")
			@RequestBody CashAllocationEntityDto cashAllocationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CashAllocationEntity> checkCashAllocationExists = cashAllocationService.findById(cashAllocationEntityDto.getId());
		CashAllocationEntity cashAllocationEntity = new CashAllocationEntity(cashAllocationEntityDto);

		if (checkCashAllocationExists.isPresent()) {
			try {
				cashAllocationEntity = cashAllocationService.update(cashAllocationEntity);
				return new ResponseEntity<>(cashAllocationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(cashAllocationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Cash Allocation that has the same id as the given id.
	 *
	 * @param id The id of the CashAllocationEntity to be deleted
	...
	@Operation(
			description = "Delete a single CashAllocation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CashAllocation to delete.")
		...
		cashAllocationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashAllocation' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Cash Allocation entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCashAllocation' endpoint here] end
	public void exportCashAllocation(
			@Parameter(description = "The ids of the CashAllocationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CashAllocationEntity> entitiesToExport = cashAllocationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CashAllocation_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CashAllocationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashAllocationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCashAllocationExcludingIds' endpoint here] end
	public void exportCashAllocationExcludingIds(
			@Parameter(description = "The ids of the CashAllocationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CashAllocationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = cashAllocationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = cashAllocationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CashAllocation entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CashAllocation_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CashAllocationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Cash Allocation entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CashAllocationEntity', 'create')  || hasPermission('CashAllocationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CashAllocationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CashAllocationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CashAllocationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CashAllocationEntity> csvToBean = new CsvToBeanBuilder<CashAllocationEntity>(reader)
				.withType(CashAllocationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CashAllocationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CashAllocationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Cash Allocation entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Cash Allocation entity and you do not have permission");
					}
