...
/**
 * Controller used to handle all REST operations regarding Inventory Adjustment Item
 */
// % protected region % [Update the API route or description for the Inventory Adjustment ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InventoryAdjustmentItemController"))
@RestController
@Slf4j
@RequestMapping("/api/inventoryadjustmentitem")
// % protected region % [Update the API route or description for the Inventory Adjustment ItemController here] end
public class InventoryAdjustmentItemController {
	...
	private final CsvUtils<InventoryAdjustmentItemEntity, InventoryAdjustmentItemRepository, InventoryAdjustmentItemEntityAudit,  InventoryAdjustmentItemService> csvUtils;
	...
	@Autowired
	public InventoryAdjustmentItemController(
		...
		this.csvUtils = new CsvUtils<>(inventoryAdjustmentItemService, InventoryAdjustmentItemEntity.class);
	...
	public ResponseEntity<List<InventoryAdjustmentItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InventoryAdjustmentItemEntity> inventoryAdjustmentItems = inventoryAdjustmentItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(inventoryAdjustmentItems, HttpStatus.OK);
	}

	/**
	 * Return the Inventory Adjustment Item that has the same id as the given id.
	 *
	 * @param id      The id of the InventoryAdjustmentItemEntity to be returned
	 * @return the Inventory Adjustment Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InventoryAdjustmentItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InventoryAdjustmentItem entity not found"),
	})
	@PreAuthorize("hasPermission('InventoryAdjustmentItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InventoryAdjustmentItemEntity> getById(
			@Parameter(description = "The UUID of the InventoryAdjustmentItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InventoryAdjustmentItemEntity> inventoryAdjustmentItemEntity = inventoryAdjustmentItemService.findById(id);
	...
	/**
	 * Create/Update the given Inventory Adjustment Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryAdjustmentItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInventoryAdjustmentItem' endpoint here] off begin
	@Operation(
			description = "Create a InventoryAdjustmentItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InventoryAdjustmentItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InventoryAdjustmentItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('InventoryAdjustmentItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInventoryAdjustmentItem' endpoint here] end
	public ResponseEntity<?> saveInventoryAdjustmentItem(
			@Parameter(description = "The InventoryAdjustmentItemEntity to create or update.")
			@RequestBody InventoryAdjustmentItemEntityDto inventoryAdjustmentItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InventoryAdjustmentItemEntity inventoryAdjustmentItemEntity = inventoryAdjustmentItemService.create(new InventoryAdjustmentItemEntity(inventoryAdjustmentItemEntityDto));
			return new ResponseEntity<>(inventoryAdjustmentItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Inventory Adjustment Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryAdjustmentItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInventoryAdjustmentItem' endpoint here] off begin
	@Operation(
			description = "Update a InventoryAdjustmentItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InventoryAdjustmentItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InventoryAdjustmentItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InventoryAdjustmentItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInventoryAdjustmentItem' endpoint here] end
	public ResponseEntity<?> updateInventoryAdjustmentItem(
			@Parameter(description = "The InventoryAdjustmentItemEntity to create or update.")
			@RequestBody InventoryAdjustmentItemEntityDto inventoryAdjustmentItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InventoryAdjustmentItemEntity> checkInventoryAdjustmentItemExists = inventoryAdjustmentItemService.findById(inventoryAdjustmentItemEntityDto.getId());
		InventoryAdjustmentItemEntity inventoryAdjustmentItemEntity = new InventoryAdjustmentItemEntity(inventoryAdjustmentItemEntityDto);

		if (checkInventoryAdjustmentItemExists.isPresent()) {
			try {
				inventoryAdjustmentItemEntity = inventoryAdjustmentItemService.update(inventoryAdjustmentItemEntity);
				return new ResponseEntity<>(inventoryAdjustmentItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(inventoryAdjustmentItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Inventory Adjustment Item that has the same id as the given id.
	 *
	 * @param id The id of the InventoryAdjustmentItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single InventoryAdjustmentItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InventoryAdjustmentItem to delete.")
		...
		inventoryAdjustmentItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustmentItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Inventory Adjustment Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustmentItem' endpoint here] end
	public void exportInventoryAdjustmentItem(
			@Parameter(description = "The ids of the InventoryAdjustmentItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InventoryAdjustmentItemEntity> entitiesToExport = inventoryAdjustmentItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InventoryAdjustmentItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InventoryAdjustmentItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustmentItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustmentItemExcludingIds' endpoint here] end
	public void exportInventoryAdjustmentItemExcludingIds(
			@Parameter(description = "The ids of the InventoryAdjustmentItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InventoryAdjustmentItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = inventoryAdjustmentItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = inventoryAdjustmentItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InventoryAdjustmentItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InventoryAdjustmentItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InventoryAdjustmentItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Inventory Adjustment Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InventoryAdjustmentItemEntity', 'create')  || hasPermission('InventoryAdjustmentItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InventoryAdjustmentItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InventoryAdjustmentItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InventoryAdjustmentItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InventoryAdjustmentItemEntity> csvToBean = new CsvToBeanBuilder<InventoryAdjustmentItemEntity>(reader)
				.withType(InventoryAdjustmentItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InventoryAdjustmentItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InventoryAdjustmentItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Inventory Adjustment Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Inventory Adjustment Item entity and you do not have permission");
					}
