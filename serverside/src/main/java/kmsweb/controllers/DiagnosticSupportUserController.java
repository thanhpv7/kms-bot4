...
import kmsweb.dtos.DiagnosticSupportUserEntityDto;
import kmsweb.entities.DiagnosticSupportUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Diagnostic Support User
 */
// % protected region % [Update the API route or description for the Diagnostic Support UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DiagnosticSupportUserController"))
@RestController
@Slf4j
@RequestMapping("/api/diagnosticsupportuser")
// % protected region % [Update the API route or description for the Diagnostic Support UserController here] end
public class DiagnosticSupportUserController {

	private final DiagnosticSupportUserService diagnosticSupportUserService;
	private final CsvUtils<DiagnosticSupportUserEntity, DiagnosticSupportUserRepository, DiagnosticSupportUserEntityAudit,  DiagnosticSupportUserService> csvUtils;
	...
	@Autowired
	public DiagnosticSupportUserController(
		...
		this.csvUtils = new CsvUtils<>(diagnosticSupportUserService, DiagnosticSupportUserEntity.class);
	...
	public ResponseEntity<List<DiagnosticSupportUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DiagnosticSupportUserEntity> diagnosticSupportUsers = diagnosticSupportUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(diagnosticSupportUsers, HttpStatus.OK);
	}

	/**
	 * Return the Diagnostic Support User that has the same id as the given id.
	 *
	 * @param id      The id of the DiagnosticSupportUserEntity to be returned
	 * @return the Diagnostic Support User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DiagnosticSupportUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DiagnosticSupportUser entity not found"),
	})
	@PreAuthorize("hasPermission('DiagnosticSupportUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DiagnosticSupportUserEntity> getById(
			@Parameter(description = "The UUID of the DiagnosticSupportUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DiagnosticSupportUserEntity> diagnosticSupportUserEntity = diagnosticSupportUserService.findById(id);
	...
	/**
	 * Create/Update the given Diagnostic Support User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosticSupportUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDiagnosticSupportUser' endpoint here] off begin
	@Operation(
			description = "Create a DiagnosticSupportUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DiagnosticSupportUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DiagnosticSupportUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('DiagnosticSupportUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDiagnosticSupportUser' endpoint here] end
	public ResponseEntity<?> saveDiagnosticSupportUser(
			@Parameter(description = "The DiagnosticSupportUserEntity to create or update.")
			@RequestBody DiagnosticSupportUserEntityDto diagnosticSupportUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DiagnosticSupportUserEntity diagnosticSupportUserEntity = diagnosticSupportUserService.create(new DiagnosticSupportUserEntity(diagnosticSupportUserEntityDto));
			return new ResponseEntity<>(diagnosticSupportUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Diagnostic Support User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosticSupportUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDiagnosticSupportUser' endpoint here] off begin
	@Operation(
			description = "Update a DiagnosticSupportUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DiagnosticSupportUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DiagnosticSupportUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DiagnosticSupportUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDiagnosticSupportUser' endpoint here] end
	public ResponseEntity<?> updateDiagnosticSupportUser(
			@Parameter(description = "The DiagnosticSupportUserEntity to create or update.")
			@RequestBody DiagnosticSupportUserEntityDto diagnosticSupportUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DiagnosticSupportUserEntity> checkDiagnosticSupportUserExists = diagnosticSupportUserService.findById(diagnosticSupportUserEntityDto.getId());
		DiagnosticSupportUserEntity diagnosticSupportUserEntity = new DiagnosticSupportUserEntity(diagnosticSupportUserEntityDto);

		if (checkDiagnosticSupportUserExists.isPresent()) {
			try {
				diagnosticSupportUserEntity = diagnosticSupportUserService.update(diagnosticSupportUserEntity);
				return new ResponseEntity<>(diagnosticSupportUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(diagnosticSupportUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Diagnostic Support User that has the same id as the given id.
	 *
	 * @param id The id of the DiagnosticSupportUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single DiagnosticSupportUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DiagnosticSupportUser to delete.")
		...
		diagnosticSupportUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Diagnostic Support User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportUser' endpoint here] end
	public void exportDiagnosticSupportUser(
			@Parameter(description = "The ids of the DiagnosticSupportUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DiagnosticSupportUserEntity> entitiesToExport = diagnosticSupportUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DiagnosticSupportUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DiagnosticSupportUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosticSupportUserExcludingIds' endpoint here] end
	public void exportDiagnosticSupportUserExcludingIds(
			@Parameter(description = "The ids of the DiagnosticSupportUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DiagnosticSupportUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = diagnosticSupportUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = diagnosticSupportUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DiagnosticSupportUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DiagnosticSupportUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DiagnosticSupportUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Diagnostic Support User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DiagnosticSupportUserEntity', 'create')  || hasPermission('DiagnosticSupportUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DiagnosticSupportUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DiagnosticSupportUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DiagnosticSupportUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DiagnosticSupportUserEntity> csvToBean = new CsvToBeanBuilder<DiagnosticSupportUserEntity>(reader)
				.withType(DiagnosticSupportUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DiagnosticSupportUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DiagnosticSupportUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Diagnostic Support User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Diagnostic Support User entity and you do not have permission");
					}
