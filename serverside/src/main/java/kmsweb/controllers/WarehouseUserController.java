...
import kmsweb.dtos.WarehouseUserEntityDto;
import kmsweb.entities.WarehouseUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Warehouse User
 */
// % protected region % [Update the API route or description for the Warehouse UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "WarehouseUserController"))
@RestController
@Slf4j
@RequestMapping("/api/warehouseuser")
// % protected region % [Update the API route or description for the Warehouse UserController here] end
public class WarehouseUserController {

	private final WarehouseUserService warehouseUserService;
	private final CsvUtils<WarehouseUserEntity, WarehouseUserRepository, WarehouseUserEntityAudit,  WarehouseUserService> csvUtils;
	...
	public WarehouseUserController(
		...
		this.csvUtils = new CsvUtils<>(warehouseUserService, WarehouseUserEntity.class);
	...
	public ResponseEntity<List<WarehouseUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<WarehouseUserEntity> warehouseUsers = warehouseUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(warehouseUsers, HttpStatus.OK);
	}

	/**
	 * Return the Warehouse User that has the same id as the given id.
	 *
	 * @param id      The id of the WarehouseUserEntity to be returned
	 * @return the Warehouse User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single WarehouseUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "WarehouseUser entity not found"),
	})
	@PreAuthorize("hasPermission('WarehouseUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<WarehouseUserEntity> getById(
			@Parameter(description = "The UUID of the WarehouseUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<WarehouseUserEntity> warehouseUserEntity = warehouseUserService.findById(id);
	...
	/**
	 * Create/Update the given Warehouse User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveWarehouseUser' endpoint here] off begin
	@Operation(
			description = "Create a WarehouseUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new WarehouseUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new WarehouseUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('WarehouseUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveWarehouseUser' endpoint here] end
	public ResponseEntity<?> saveWarehouseUser(
			@Parameter(description = "The WarehouseUserEntity to create or update.")
			@RequestBody WarehouseUserEntityDto warehouseUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			WarehouseUserEntity warehouseUserEntity = warehouseUserService.create(new WarehouseUserEntity(warehouseUserEntityDto));
			return new ResponseEntity<>(warehouseUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Warehouse User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateWarehouseUser' endpoint here] off begin
	@Operation(
			description = "Update a WarehouseUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the WarehouseUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the WarehouseUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('WarehouseUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateWarehouseUser' endpoint here] end
	public ResponseEntity<?> updateWarehouseUser(
			@Parameter(description = "The WarehouseUserEntity to create or update.")
			@RequestBody WarehouseUserEntityDto warehouseUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<WarehouseUserEntity> checkWarehouseUserExists = warehouseUserService.findById(warehouseUserEntityDto.getId());
		WarehouseUserEntity warehouseUserEntity = new WarehouseUserEntity(warehouseUserEntityDto);

		if (checkWarehouseUserExists.isPresent()) {
			try {
				warehouseUserEntity = warehouseUserService.update(warehouseUserEntity);
				return new ResponseEntity<>(warehouseUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(warehouseUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Warehouse User that has the same id as the given id.
	 *
	 * @param id The id of the WarehouseUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single WarehouseUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('WarehouseUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the WarehouseUser to delete.")
		...
		warehouseUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouseUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Warehouse User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouseUser' endpoint here] end
	public void exportWarehouseUser(
			@Parameter(description = "The ids of the WarehouseUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<WarehouseUserEntity> entitiesToExport = warehouseUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "WarehouseUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (WarehouseUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouseUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouseUserExcludingIds' endpoint here] end
	public void exportWarehouseUserExcludingIds(
			@Parameter(description = "The ids of the WarehouseUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<WarehouseUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = warehouseUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = warehouseUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the WarehouseUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "WarehouseUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(WarehouseUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Warehouse User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('WarehouseUserEntity', 'create')  || hasPermission('WarehouseUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with WarehouseUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("WarehouseUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(WarehouseUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<WarehouseUserEntity> csvToBean = new CsvToBeanBuilder<WarehouseUserEntity>(reader)
				.withType(WarehouseUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<WarehouseUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<WarehouseUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Warehouse User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Warehouse User entity and you do not have permission");
					}
