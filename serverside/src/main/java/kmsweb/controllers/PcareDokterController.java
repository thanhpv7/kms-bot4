...
/**
 * Controller used to handle all REST operations regarding PCare Dokter
 */
// % protected region % [Update the API route or description for the PCare DokterController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareDokterController"))
@RestController
@Slf4j
@RequestMapping("/api/pcaredokter")
// % protected region % [Update the API route or description for the PCare DokterController here] end
public class PcareDokterController {

	private final PcareDokterService pcareDokterService;
	private final CsvUtils<PcareDokterEntity, PcareDokterRepository, PcareDokterEntityAudit,  PcareDokterService> csvUtils;
	...
	public PcareDokterController(
		...
		this.csvUtils = new CsvUtils<>(pcareDokterService, PcareDokterEntity.class);
	...
	public ResponseEntity<List<PcareDokterEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareDokterEntity> pcareDokters = pcareDokterService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareDokters, HttpStatus.OK);
	}

	/**
	 * Return the PCare Dokter that has the same id as the given id.
	 *
	 * @param id      The id of the PcareDokterEntity to be returned
	 * @return the PCare Dokter that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareDokter as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareDokter entity not found"),
	})
	@PreAuthorize("hasPermission('PcareDokterEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareDokterEntity> getById(
			@Parameter(description = "The UUID of the PcareDokter to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareDokterEntity> pcareDokterEntity = pcareDokterService.findById(id);
	...
	/**
	 * Create/Update the given PCare Dokter. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareDokterEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareDokter' endpoint here] off begin
	@Operation(
			description = "Create a PcareDokter if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareDokter."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareDokter, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareDokterEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareDokter' endpoint here] end
	public ResponseEntity<?> savePcareDokter(
			@Parameter(description = "The PcareDokterEntity to create or update.")
			@RequestBody PcareDokterEntityDto pcareDokterEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareDokterEntity pcareDokterEntity = pcareDokterService.create(new PcareDokterEntity(pcareDokterEntityDto));
			return new ResponseEntity<>(pcareDokterEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Dokter. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareDokterEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareDokter' endpoint here] off begin
	@Operation(
			description = "Update a PcareDokter entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareDokter."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareDokter, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareDokterEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareDokter' endpoint here] end
	public ResponseEntity<?> updatePcareDokter(
			@Parameter(description = "The PcareDokterEntity to create or update.")
			@RequestBody PcareDokterEntityDto pcareDokterEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareDokterEntity> checkPcareDokterExists = pcareDokterService.findById(pcareDokterEntityDto.getId());
		PcareDokterEntity pcareDokterEntity = new PcareDokterEntity(pcareDokterEntityDto);

		if (checkPcareDokterExists.isPresent()) {
			try {
				pcareDokterEntity = pcareDokterService.update(pcareDokterEntity);
				return new ResponseEntity<>(pcareDokterEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareDokterEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Dokter that has the same id as the given id.
	 *
	 * @param id The id of the PcareDokterEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareDokter as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareDokter to delete.")
		...
		pcareDokterService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareDokter' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Dokter entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareDokter' endpoint here] end
	public void exportPcareDokter(
			@Parameter(description = "The ids of the PcareDokterEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareDokterEntity> entitiesToExport = pcareDokterService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareDokter_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareDokterEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareDokterExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareDokterExcludingIds' endpoint here] end
	public void exportPcareDokterExcludingIds(
			@Parameter(description = "The ids of the PcareDokterEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareDokterEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareDokterService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareDokterService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareDokter entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareDokter_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareDokterEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Dokter entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareDokterEntity', 'create')  || hasPermission('PcareDokterEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareDokterEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareDokterEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareDokterEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareDokterEntity> csvToBean = new CsvToBeanBuilder<PcareDokterEntity>(reader)
				.withType(PcareDokterEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareDokterEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareDokterEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Dokter entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Dokter entity and you do not have permission");
					}
