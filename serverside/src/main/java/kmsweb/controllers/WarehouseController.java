...
import kmsweb.entities.WarehouseEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Warehouse
 */
// % protected region % [Update the API route or description for the WarehouseController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "WarehouseController"))
@RestController
@Slf4j
@RequestMapping("/api/warehouse")
// % protected region % [Update the API route or description for the WarehouseController here] end
public class WarehouseController {

	private final WarehouseService warehouseService;
	private final CsvUtils<WarehouseEntity, WarehouseRepository, WarehouseEntityAudit,  WarehouseService> csvUtils;
	...
	public WarehouseController(
		...
		this.csvUtils = new CsvUtils<>(warehouseService, WarehouseEntity.class);
	...
	@Operation(
			description = "Returns a single page of Warehouses",
		security = {
	...
	public ResponseEntity<List<WarehouseEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<WarehouseEntity> warehouses = warehouseService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Warehouse that has the same id as the given id.
	 *
	 * @param id      The id of the WarehouseEntity to be returned
	 * @return the Warehouse that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Warehouse as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Warehouse entity not found"),
	})
	@PreAuthorize("hasPermission('WarehouseEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<WarehouseEntity> getById(
			@Parameter(description = "The UUID of the Warehouse to return.")
			@PathVariable("id") UUID id
	) {
		Optional<WarehouseEntity> warehouseEntity = warehouseService.findById(id);
	...
	/**
	 * Create/Update the given Warehouse. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveWarehouse' endpoint here] off begin
	@Operation(
			description = "Create a Warehouse if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Warehouse."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Warehouse, duplicate record found")
	})
	@PreAuthorize("hasPermission('WarehouseEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveWarehouse' endpoint here] end
	public ResponseEntity<?> saveWarehouse(
			@Parameter(description = "The WarehouseEntity to create or update.")
			@RequestBody WarehouseEntityDto warehouseEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			WarehouseEntity warehouseEntity = warehouseService.create(new WarehouseEntity(warehouseEntityDto));
	...
	/**
	 * Update the given Warehouse. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateWarehouse' endpoint here] off begin
	@Operation(
			description = "Update a Warehouse entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		...
		@ApiResponse(responseCode = "400", description = "Failed to update the Warehouse, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('WarehouseEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateWarehouse' endpoint here] end
	public ResponseEntity<?> updateWarehouse(
		...
		Optional<WarehouseEntity> checkWarehouseExists = warehouseService.findById(warehouseEntityDto.getId());
		WarehouseEntity warehouseEntity = new WarehouseEntity(warehouseEntityDto);

		if (checkWarehouseExists.isPresent()) {
			try {
		...
		return new ResponseEntity<>(warehouseEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Warehouse that has the same id as the given id.
	 *
	 * @param id The id of the WarehouseEntity to be deleted
	...
	@Operation(
			description = "Delete a single Warehouse as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Warehouse to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouse' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Warehouse entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouse' endpoint here] end
	public void exportWarehouse(
			@Parameter(description = "The ids of the WarehouseEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<WarehouseEntity> entitiesToExport = warehouseService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Warehouse_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (WarehouseEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouseExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouseExcludingIds' endpoint here] end
	public void exportWarehouseExcludingIds(
			@Parameter(description = "The ids of the WarehouseEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<WarehouseEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = warehouseService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = warehouseService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Warehouse entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Warehouse_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(WarehouseEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Warehouse entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('WarehouseEntity', 'create')  || hasPermission('WarehouseEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with WarehouseEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("WarehouseEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(WarehouseEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<WarehouseEntity> csvToBean = new CsvToBeanBuilder<WarehouseEntity>(reader)
				.withType(WarehouseEntity.class)
		...
		try {
			Iterator<WarehouseEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Warehouse entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Warehouse entity and you do not have permission");
					}
