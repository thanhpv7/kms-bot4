...
import kmsweb.dtos.BatchStockBalanceEntityDto;
...
/**
 * Controller used to handle all REST operations regarding Batch Stock Balance
 */
// % protected region % [Update the API route or description for the Batch Stock BalanceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BatchStockBalanceController"))
@RestController
@Slf4j
@RequestMapping("/api/batchstockbalance")
// % protected region % [Update the API route or description for the Batch Stock BalanceController here] end
public class BatchStockBalanceController {

	private final BatchStockBalanceService batchStockBalanceService;
	private final CsvUtils<BatchStockBalanceEntity, BatchStockBalanceRepository, BatchStockBalanceEntityAudit,  BatchStockBalanceService> csvUtils;
	...
	@Autowired
	public BatchStockBalanceController(
		...
		this.csvUtils = new CsvUtils<>(batchStockBalanceService, BatchStockBalanceEntity.class);
	...
	public ResponseEntity<List<BatchStockBalanceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BatchStockBalanceEntity> batchStockBalances = batchStockBalanceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(batchStockBalances, HttpStatus.OK);
	}

	/**
	 * Return the Batch Stock Balance that has the same id as the given id.
	 *
	 * @param id      The id of the BatchStockBalanceEntity to be returned
	 * @return the Batch Stock Balance that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BatchStockBalance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BatchStockBalance entity not found"),
	})
	@PreAuthorize("hasPermission('BatchStockBalanceEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BatchStockBalanceEntity> getById(
			@Parameter(description = "The UUID of the BatchStockBalance to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BatchStockBalanceEntity> batchStockBalanceEntity = batchStockBalanceService.findById(id);
	...
	/**
	 * Create/Update the given Batch Stock Balance. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param batchStockBalanceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBatchStockBalance' endpoint here] off begin
	@Operation(
			description = "Create a BatchStockBalance if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BatchStockBalance."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BatchStockBalance, duplicate record found")
	})
	@PreAuthorize("hasPermission('BatchStockBalanceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBatchStockBalance' endpoint here] end
	public ResponseEntity<?> saveBatchStockBalance(
			@Parameter(description = "The BatchStockBalanceEntity to create or update.")
			@RequestBody BatchStockBalanceEntityDto batchStockBalanceEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BatchStockBalanceEntity batchStockBalanceEntity = batchStockBalanceService.create(new BatchStockBalanceEntity(batchStockBalanceEntityDto));
			return new ResponseEntity<>(batchStockBalanceEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Batch Stock Balance. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param batchStockBalanceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBatchStockBalance' endpoint here] off begin
	@Operation(
			description = "Update a BatchStockBalance entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BatchStockBalance."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BatchStockBalance, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BatchStockBalanceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBatchStockBalance' endpoint here] end
	public ResponseEntity<?> updateBatchStockBalance(
			@Parameter(description = "The BatchStockBalanceEntity to create or update.")
			@RequestBody BatchStockBalanceEntityDto batchStockBalanceEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BatchStockBalanceEntity> checkBatchStockBalanceExists = batchStockBalanceService.findById(batchStockBalanceEntityDto.getId());
		BatchStockBalanceEntity batchStockBalanceEntity = new BatchStockBalanceEntity(batchStockBalanceEntityDto);

		if (checkBatchStockBalanceExists.isPresent()) {
			try {
				batchStockBalanceEntity = batchStockBalanceService.update(batchStockBalanceEntity);
				return new ResponseEntity<>(batchStockBalanceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(batchStockBalanceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Batch Stock Balance that has the same id as the given id.
	 *
	 * @param id The id of the BatchStockBalanceEntity to be deleted
	...
	@Operation(
			description = "Delete a single BatchStockBalance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BatchStockBalance to delete.")
		...
		batchStockBalanceService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalance' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Batch Stock Balance entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalance' endpoint here] end
	public void exportBatchStockBalance(
			@Parameter(description = "The ids of the BatchStockBalanceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BatchStockBalanceEntity> entitiesToExport = batchStockBalanceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BatchStockBalance_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BatchStockBalanceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalanceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalanceExcludingIds' endpoint here] end
	public void exportBatchStockBalanceExcludingIds(
			@Parameter(description = "The ids of the BatchStockBalanceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BatchStockBalanceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = batchStockBalanceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = batchStockBalanceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BatchStockBalance entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BatchStockBalance_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BatchStockBalanceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Batch Stock Balance entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BatchStockBalanceEntity', 'create')  || hasPermission('BatchStockBalanceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BatchStockBalanceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BatchStockBalanceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BatchStockBalanceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BatchStockBalanceEntity> csvToBean = new CsvToBeanBuilder<BatchStockBalanceEntity>(reader)
				.withType(BatchStockBalanceEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BatchStockBalanceEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BatchStockBalanceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Batch Stock Balance entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Batch Stock Balance entity and you do not have permission");
					}
