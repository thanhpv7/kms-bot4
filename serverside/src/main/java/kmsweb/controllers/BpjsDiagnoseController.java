...
import kmsweb.entities.BpjsDiagnoseEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Diagnose
 */
// % protected region % [Update the API route or description for the BPJS DiagnoseController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsDiagnoseController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsdiagnose")
// % protected region % [Update the API route or description for the BPJS DiagnoseController here] end
public class BpjsDiagnoseController {

	private final BpjsDiagnoseService bpjsDiagnoseService;
	private final CsvUtils<BpjsDiagnoseEntity, BpjsDiagnoseRepository, BpjsDiagnoseEntityAudit,  BpjsDiagnoseService> csvUtils;
	...
	public BpjsDiagnoseController(
		...
		this.csvUtils = new CsvUtils<>(bpjsDiagnoseService, BpjsDiagnoseEntity.class);
	...
	public ResponseEntity<List<BpjsDiagnoseEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsDiagnoseEntity> bpjsDiagnoses = bpjsDiagnoseService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsDiagnoses, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Diagnose that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsDiagnoseEntity to be returned
	 * @return the BPJS Diagnose that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsDiagnose as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsDiagnose entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsDiagnoseEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsDiagnoseEntity> getById(
			@Parameter(description = "The UUID of the BpjsDiagnose to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsDiagnoseEntity> bpjsDiagnoseEntity = bpjsDiagnoseService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Diagnose. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDiagnoseEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsDiagnose' endpoint here] off begin
	@Operation(
			description = "Create a BpjsDiagnose if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsDiagnose."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsDiagnose, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsDiagnoseEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsDiagnose' endpoint here] end
	public ResponseEntity<?> saveBpjsDiagnose(
			@Parameter(description = "The BpjsDiagnoseEntity to create or update.")
			@RequestBody BpjsDiagnoseEntityDto bpjsDiagnoseEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsDiagnoseEntity bpjsDiagnoseEntity = bpjsDiagnoseService.create(new BpjsDiagnoseEntity(bpjsDiagnoseEntityDto));
			return new ResponseEntity<>(bpjsDiagnoseEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Diagnose. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDiagnoseEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsDiagnose' endpoint here] off begin
	@Operation(
			description = "Update a BpjsDiagnose entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsDiagnose."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsDiagnose, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsDiagnoseEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsDiagnose' endpoint here] end
	public ResponseEntity<?> updateBpjsDiagnose(
			@Parameter(description = "The BpjsDiagnoseEntity to create or update.")
			@RequestBody BpjsDiagnoseEntityDto bpjsDiagnoseEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsDiagnoseEntity> checkBpjsDiagnoseExists = bpjsDiagnoseService.findById(bpjsDiagnoseEntityDto.getId());
		BpjsDiagnoseEntity bpjsDiagnoseEntity = new BpjsDiagnoseEntity(bpjsDiagnoseEntityDto);

		if (checkBpjsDiagnoseExists.isPresent()) {
			try {
				bpjsDiagnoseEntity = bpjsDiagnoseService.update(bpjsDiagnoseEntity);
				return new ResponseEntity<>(bpjsDiagnoseEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsDiagnoseEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Diagnose that has the same id as the given id.
	 *
	 * @param id The id of the BpjsDiagnoseEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsDiagnose as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsDiagnose to delete.")
		...
		bpjsDiagnoseService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnose' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Diagnose entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnose' endpoint here] end
	public void exportBpjsDiagnose(
			@Parameter(description = "The ids of the BpjsDiagnoseEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsDiagnoseEntity> entitiesToExport = bpjsDiagnoseService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsDiagnose_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsDiagnoseEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnoseExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDiagnoseExcludingIds' endpoint here] end
	public void exportBpjsDiagnoseExcludingIds(
			@Parameter(description = "The ids of the BpjsDiagnoseEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsDiagnoseEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsDiagnoseService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsDiagnoseService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsDiagnose entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsDiagnose_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsDiagnoseEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Diagnose entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsDiagnoseEntity', 'create')  || hasPermission('BpjsDiagnoseEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsDiagnoseEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsDiagnoseEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsDiagnoseEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsDiagnoseEntity> csvToBean = new CsvToBeanBuilder<BpjsDiagnoseEntity>(reader)
				.withType(BpjsDiagnoseEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsDiagnoseEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsDiagnoseEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Diagnose entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Diagnose entity and you do not have permission");
					}
