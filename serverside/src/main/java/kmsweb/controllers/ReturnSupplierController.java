...
import kmsweb.entities.ReturnSupplierEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Return Supplier
 */
// % protected region % [Update the API route or description for the Return SupplierController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ReturnSupplierController"))
@RestController
@Slf4j
@RequestMapping("/api/returnsupplier")
// % protected region % [Update the API route or description for the Return SupplierController here] end
public class ReturnSupplierController {

	private final ReturnSupplierService returnSupplierService;
	private final CsvUtils<ReturnSupplierEntity, ReturnSupplierRepository, ReturnSupplierEntityAudit,  ReturnSupplierService> csvUtils;
	...
	@Autowired
	public ReturnSupplierController(
		...
		this.csvUtils = new CsvUtils<>(returnSupplierService, ReturnSupplierEntity.class);
	...
	public ResponseEntity<List<ReturnSupplierEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ReturnSupplierEntity> returnSuppliers = returnSupplierService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(returnSuppliers, HttpStatus.OK);
	}

	/**
	 * Return the Return Supplier that has the same id as the given id.
	 *
	 * @param id      The id of the ReturnSupplierEntity to be returned
	 * @return the Return Supplier that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ReturnSupplier as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ReturnSupplier entity not found"),
	})
	@PreAuthorize("hasPermission('ReturnSupplierEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ReturnSupplierEntity> getById(
			@Parameter(description = "The UUID of the ReturnSupplier to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ReturnSupplierEntity> returnSupplierEntity = returnSupplierService.findById(id);
	...
	/**
	 * Create/Update the given Return Supplier. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param returnSupplierEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveReturnSupplier' endpoint here] off begin
	@Operation(
			description = "Create a ReturnSupplier if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ReturnSupplier."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ReturnSupplier, duplicate record found")
	})
	@PreAuthorize("hasPermission('ReturnSupplierEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveReturnSupplier' endpoint here] end
	public ResponseEntity<?> saveReturnSupplier(
			@Parameter(description = "The ReturnSupplierEntity to create or update.")
			@RequestBody ReturnSupplierEntityDto returnSupplierEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ReturnSupplierEntity returnSupplierEntity = returnSupplierService.create(new ReturnSupplierEntity(returnSupplierEntityDto));
			return new ResponseEntity<>(returnSupplierEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Return Supplier. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param returnSupplierEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateReturnSupplier' endpoint here] off begin
	@Operation(
			description = "Update a ReturnSupplier entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ReturnSupplier."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ReturnSupplier, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ReturnSupplierEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateReturnSupplier' endpoint here] end
	public ResponseEntity<?> updateReturnSupplier(
			@Parameter(description = "The ReturnSupplierEntity to create or update.")
			@RequestBody ReturnSupplierEntityDto returnSupplierEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ReturnSupplierEntity> checkReturnSupplierExists = returnSupplierService.findById(returnSupplierEntityDto.getId());
		ReturnSupplierEntity returnSupplierEntity = new ReturnSupplierEntity(returnSupplierEntityDto);

		if (checkReturnSupplierExists.isPresent()) {
			try {
				returnSupplierEntity = returnSupplierService.update(returnSupplierEntity);
				return new ResponseEntity<>(returnSupplierEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(returnSupplierEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Return Supplier that has the same id as the given id.
	 *
	 * @param id The id of the ReturnSupplierEntity to be deleted
	...
	@Operation(
			description = "Delete a single ReturnSupplier as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ReturnSupplier to delete.")
		...
		returnSupplierService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplier' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Return Supplier entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplier' endpoint here] end
	public void exportReturnSupplier(
			@Parameter(description = "The ids of the ReturnSupplierEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ReturnSupplierEntity> entitiesToExport = returnSupplierService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ReturnSupplier_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ReturnSupplierEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplierExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportReturnSupplierExcludingIds' endpoint here] end
	public void exportReturnSupplierExcludingIds(
			@Parameter(description = "The ids of the ReturnSupplierEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ReturnSupplierEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = returnSupplierService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = returnSupplierService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ReturnSupplier entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ReturnSupplier_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ReturnSupplierEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Return Supplier entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ReturnSupplierEntity', 'create')  || hasPermission('ReturnSupplierEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ReturnSupplierEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ReturnSupplierEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ReturnSupplierEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ReturnSupplierEntity> csvToBean = new CsvToBeanBuilder<ReturnSupplierEntity>(reader)
				.withType(ReturnSupplierEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ReturnSupplierEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ReturnSupplierEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Return Supplier entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Return Supplier entity and you do not have permission");
					}
