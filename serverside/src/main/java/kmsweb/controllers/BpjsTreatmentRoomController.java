...
import kmsweb.entities.BpjsTreatmentRoomEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Treatment Room
 */
// % protected region % [Update the API route or description for the BPJS Treatment RoomController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsTreatmentRoomController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjstreatmentroom")
// % protected region % [Update the API route or description for the BPJS Treatment RoomController here] end
public class BpjsTreatmentRoomController {

	private final BpjsTreatmentRoomService bpjsTreatmentRoomService;
	private final CsvUtils<BpjsTreatmentRoomEntity, BpjsTreatmentRoomRepository, BpjsTreatmentRoomEntityAudit,  BpjsTreatmentRoomService> csvUtils;
	...
	@Autowired
	public BpjsTreatmentRoomController(
		...
		this.csvUtils = new CsvUtils<>(bpjsTreatmentRoomService, BpjsTreatmentRoomEntity.class);
	...
	public ResponseEntity<List<BpjsTreatmentRoomEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsTreatmentRoomEntity> bpjsTreatmentRooms = bpjsTreatmentRoomService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsTreatmentRooms, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Treatment Room that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsTreatmentRoomEntity to be returned
	 * @return the BPJS Treatment Room that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsTreatmentRoom as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsTreatmentRoom entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsTreatmentRoomEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsTreatmentRoomEntity> getById(
			@Parameter(description = "The UUID of the BpjsTreatmentRoom to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsTreatmentRoomEntity> bpjsTreatmentRoomEntity = bpjsTreatmentRoomService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Treatment Room. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTreatmentRoomEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsTreatmentRoom' endpoint here] off begin
	@Operation(
			description = "Create a BpjsTreatmentRoom if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsTreatmentRoom."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsTreatmentRoom, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsTreatmentRoomEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsTreatmentRoom' endpoint here] end
	public ResponseEntity<?> saveBpjsTreatmentRoom(
			@Parameter(description = "The BpjsTreatmentRoomEntity to create or update.")
			@RequestBody BpjsTreatmentRoomEntityDto bpjsTreatmentRoomEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsTreatmentRoomEntity bpjsTreatmentRoomEntity = bpjsTreatmentRoomService.create(new BpjsTreatmentRoomEntity(bpjsTreatmentRoomEntityDto));
			return new ResponseEntity<>(bpjsTreatmentRoomEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Treatment Room. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTreatmentRoomEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsTreatmentRoom' endpoint here] off begin
	@Operation(
			description = "Update a BpjsTreatmentRoom entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsTreatmentRoom."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsTreatmentRoom, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsTreatmentRoomEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsTreatmentRoom' endpoint here] end
	public ResponseEntity<?> updateBpjsTreatmentRoom(
			@Parameter(description = "The BpjsTreatmentRoomEntity to create or update.")
			@RequestBody BpjsTreatmentRoomEntityDto bpjsTreatmentRoomEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsTreatmentRoomEntity> checkBpjsTreatmentRoomExists = bpjsTreatmentRoomService.findById(bpjsTreatmentRoomEntityDto.getId());
		BpjsTreatmentRoomEntity bpjsTreatmentRoomEntity = new BpjsTreatmentRoomEntity(bpjsTreatmentRoomEntityDto);

		if (checkBpjsTreatmentRoomExists.isPresent()) {
			try {
				bpjsTreatmentRoomEntity = bpjsTreatmentRoomService.update(bpjsTreatmentRoomEntity);
				return new ResponseEntity<>(bpjsTreatmentRoomEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsTreatmentRoomEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Treatment Room that has the same id as the given id.
	 *
	 * @param id The id of the BpjsTreatmentRoomEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsTreatmentRoom as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsTreatmentRoom to delete.")
		...
		bpjsTreatmentRoomService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentRoom' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Treatment Room entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentRoom' endpoint here] end
	public void exportBpjsTreatmentRoom(
			@Parameter(description = "The ids of the BpjsTreatmentRoomEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsTreatmentRoomEntity> entitiesToExport = bpjsTreatmentRoomService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsTreatmentRoom_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsTreatmentRoomEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentRoomExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentRoomExcludingIds' endpoint here] end
	public void exportBpjsTreatmentRoomExcludingIds(
			@Parameter(description = "The ids of the BpjsTreatmentRoomEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsTreatmentRoomEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsTreatmentRoomService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsTreatmentRoomService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsTreatmentRoom entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsTreatmentRoom_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsTreatmentRoomEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Treatment Room entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsTreatmentRoomEntity', 'create')  || hasPermission('BpjsTreatmentRoomEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsTreatmentRoomEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsTreatmentRoomEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsTreatmentRoomEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsTreatmentRoomEntity> csvToBean = new CsvToBeanBuilder<BpjsTreatmentRoomEntity>(reader)
				.withType(BpjsTreatmentRoomEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsTreatmentRoomEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsTreatmentRoomEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Treatment Room entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Treatment Room entity and you do not have permission");
					}
