...
import kmsweb.dtos.AmendmentDetailEntityDto;
...
/**
 * Controller used to handle all REST operations regarding Amendment Detail
 */
// % protected region % [Update the API route or description for the Amendment DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "AmendmentDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/amendmentdetail")
// % protected region % [Update the API route or description for the Amendment DetailController here] end
public class AmendmentDetailController {

	private final AmendmentDetailService amendmentDetailService;
	private final CsvUtils<AmendmentDetailEntity, AmendmentDetailRepository, AmendmentDetailEntityAudit,  AmendmentDetailService> csvUtils;
	...
	@Autowired
	public AmendmentDetailController(
		...
		this.csvUtils = new CsvUtils<>(amendmentDetailService, AmendmentDetailEntity.class);
	...
	public ResponseEntity<List<AmendmentDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<AmendmentDetailEntity> amendmentDetails = amendmentDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(amendmentDetails, HttpStatus.OK);
	}

	/**
	 * Return the Amendment Detail that has the same id as the given id.
	 *
	 * @param id      The id of the AmendmentDetailEntity to be returned
	 * @return the Amendment Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single AmendmentDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "AmendmentDetail entity not found"),
	})
	@PreAuthorize("hasPermission('AmendmentDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<AmendmentDetailEntity> getById(
			@Parameter(description = "The UUID of the AmendmentDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<AmendmentDetailEntity> amendmentDetailEntity = amendmentDetailService.findById(id);
	...
	/**
	 * Create/Update the given Amendment Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param amendmentDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveAmendmentDetail' endpoint here] off begin
	@Operation(
			description = "Create a AmendmentDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new AmendmentDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new AmendmentDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('AmendmentDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveAmendmentDetail' endpoint here] end
	public ResponseEntity<?> saveAmendmentDetail(
			@Parameter(description = "The AmendmentDetailEntity to create or update.")
			@RequestBody AmendmentDetailEntityDto amendmentDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			AmendmentDetailEntity amendmentDetailEntity = amendmentDetailService.create(new AmendmentDetailEntity(amendmentDetailEntityDto));
			return new ResponseEntity<>(amendmentDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Amendment Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param amendmentDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateAmendmentDetail' endpoint here] off begin
	@Operation(
			description = "Update a AmendmentDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the AmendmentDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the AmendmentDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('AmendmentDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateAmendmentDetail' endpoint here] end
	public ResponseEntity<?> updateAmendmentDetail(
			@Parameter(description = "The AmendmentDetailEntity to create or update.")
			@RequestBody AmendmentDetailEntityDto amendmentDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<AmendmentDetailEntity> checkAmendmentDetailExists = amendmentDetailService.findById(amendmentDetailEntityDto.getId());
		AmendmentDetailEntity amendmentDetailEntity = new AmendmentDetailEntity(amendmentDetailEntityDto);

		if (checkAmendmentDetailExists.isPresent()) {
			try {
				amendmentDetailEntity = amendmentDetailService.update(amendmentDetailEntity);
				return new ResponseEntity<>(amendmentDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(amendmentDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Amendment Detail that has the same id as the given id.
	 *
	 * @param id The id of the AmendmentDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single AmendmentDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('AmendmentDetailEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the AmendmentDetail to delete.")
		...
		amendmentDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportAmendmentDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Amendment Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportAmendmentDetail' endpoint here] end
	public void exportAmendmentDetail(
			@Parameter(description = "The ids of the AmendmentDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<AmendmentDetailEntity> entitiesToExport = amendmentDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "AmendmentDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (AmendmentDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportAmendmentDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportAmendmentDetailExcludingIds' endpoint here] end
	public void exportAmendmentDetailExcludingIds(
			@Parameter(description = "The ids of the AmendmentDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<AmendmentDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = amendmentDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = amendmentDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the AmendmentDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "AmendmentDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(AmendmentDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Amendment Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('AmendmentDetailEntity', 'create')  || hasPermission('AmendmentDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with AmendmentDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("AmendmentDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(AmendmentDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<AmendmentDetailEntity> csvToBean = new CsvToBeanBuilder<AmendmentDetailEntity>(reader)
				.withType(AmendmentDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<AmendmentDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<AmendmentDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Amendment Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Amendment Detail entity and you do not have permission");
					}
