...
import kmsweb.entities.BpjsPcareDPHOEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Pcare DPHO
 */
// % protected region % [Update the API route or description for the BPJS Pcare DPHOController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcareDPHOController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcaredpho")
// % protected region % [Update the API route or description for the BPJS Pcare DPHOController here] end
public class BpjsPcareDPHOController {

	private final BpjsPcareDPHOService bpjsPcareDPHOService;
	private final CsvUtils<BpjsPcareDPHOEntity, BpjsPcareDPHORepository, BpjsPcareDPHOEntityAudit,  BpjsPcareDPHOService> csvUtils;
	...
	@Autowired
	public BpjsPcareDPHOController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPcareDPHOService, BpjsPcareDPHOEntity.class);
	...
	public ResponseEntity<List<BpjsPcareDPHOEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcareDPHOEntity> bpjsPcareDPHOs = bpjsPcareDPHOService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcareDPHOs, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Pcare DPHO that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcareDPHOEntity to be returned
	 * @return the BPJS Pcare DPHO that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcareDPHO as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcareDPHO entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcareDPHOEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcareDPHOEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcareDPHO to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcareDPHOEntity> bpjsPcareDPHOEntity = bpjsPcareDPHOService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Pcare DPHO. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareDPHOEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareDPHO' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcareDPHO if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcareDPHO."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcareDPHO, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcareDPHOEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareDPHO' endpoint here] end
	public ResponseEntity<?> saveBpjsPcareDPHO(
			@Parameter(description = "The BpjsPcareDPHOEntity to create or update.")
			@RequestBody BpjsPcareDPHOEntityDto bpjsPcareDPHOEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcareDPHOEntity bpjsPcareDPHOEntity = bpjsPcareDPHOService.create(new BpjsPcareDPHOEntity(bpjsPcareDPHOEntityDto));
			return new ResponseEntity<>(bpjsPcareDPHOEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Pcare DPHO. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareDPHOEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareDPHO' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcareDPHO entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcareDPHO."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcareDPHO, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcareDPHOEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareDPHO' endpoint here] end
	public ResponseEntity<?> updateBpjsPcareDPHO(
			@Parameter(description = "The BpjsPcareDPHOEntity to create or update.")
			@RequestBody BpjsPcareDPHOEntityDto bpjsPcareDPHOEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcareDPHOEntity> checkBpjsPcareDPHOExists = bpjsPcareDPHOService.findById(bpjsPcareDPHOEntityDto.getId());
		BpjsPcareDPHOEntity bpjsPcareDPHOEntity = new BpjsPcareDPHOEntity(bpjsPcareDPHOEntityDto);

		if (checkBpjsPcareDPHOExists.isPresent()) {
			try {
				bpjsPcareDPHOEntity = bpjsPcareDPHOService.update(bpjsPcareDPHOEntity);
				return new ResponseEntity<>(bpjsPcareDPHOEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcareDPHOEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Pcare DPHO that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcareDPHOEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcareDPHO as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcareDPHO to delete.")
		...
		bpjsPcareDPHOService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareDPHO' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Pcare DPHO entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareDPHO' endpoint here] end
	public void exportBpjsPcareDPHO(
			@Parameter(description = "The ids of the BpjsPcareDPHOEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcareDPHOEntity> entitiesToExport = bpjsPcareDPHOService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcareDPHO_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcareDPHOEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareDPHOExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareDPHOExcludingIds' endpoint here] end
	public void exportBpjsPcareDPHOExcludingIds(
			@Parameter(description = "The ids of the BpjsPcareDPHOEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcareDPHOEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcareDPHOService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcareDPHOService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcareDPHO entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcareDPHO_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcareDPHOEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Pcare DPHO entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcareDPHOEntity', 'create')  || hasPermission('BpjsPcareDPHOEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcareDPHOEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcareDPHOEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcareDPHOEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcareDPHOEntity> csvToBean = new CsvToBeanBuilder<BpjsPcareDPHOEntity>(reader)
				.withType(BpjsPcareDPHOEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcareDPHOEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcareDPHOEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Pcare DPHO entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Pcare DPHO entity and you do not have permission");
					}
