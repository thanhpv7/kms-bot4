...
/**
 * Controller used to handle all REST operations regarding Hemodialysis Examination
 */
// % protected region % [Update the API route or description for the Hemodialysis ExaminationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "HemodialysisExaminationController"))
@RestController
@Slf4j
@RequestMapping("/api/hemodialysisexamination")
// % protected region % [Update the API route or description for the Hemodialysis ExaminationController here] end
public class HemodialysisExaminationController {

	private final HemodialysisExaminationService hemodialysisExaminationService;
	private final CsvUtils<HemodialysisExaminationEntity, HemodialysisExaminationRepository, HemodialysisExaminationEntityAudit,  HemodialysisExaminationService> csvUtils;
	...
	@Autowired
	public HemodialysisExaminationController(
		...
		this.csvUtils = new CsvUtils<>(hemodialysisExaminationService, HemodialysisExaminationEntity.class);
	...
	public ResponseEntity<List<HemodialysisExaminationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<HemodialysisExaminationEntity> hemodialysisExaminations = hemodialysisExaminationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(hemodialysisExaminations, HttpStatus.OK);
	}

	/**
	 * Return the Hemodialysis Examination that has the same id as the given id.
	 *
	 * @param id      The id of the HemodialysisExaminationEntity to be returned
	 * @return the Hemodialysis Examination that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single HemodialysisExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "HemodialysisExamination entity not found"),
	})
	@PreAuthorize("hasPermission('HemodialysisExaminationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<HemodialysisExaminationEntity> getById(
			@Parameter(description = "The UUID of the HemodialysisExamination to return.")
			@PathVariable("id") UUID id
	) {
		Optional<HemodialysisExaminationEntity> hemodialysisExaminationEntity = hemodialysisExaminationService.findById(id);
	...
	/**
	 * Create/Update the given Hemodialysis Examination. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param hemodialysisExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveHemodialysisExamination' endpoint here] off begin
	@Operation(
			description = "Create a HemodialysisExamination if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new HemodialysisExamination."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new HemodialysisExamination, duplicate record found")
	})
	@PreAuthorize("hasPermission('HemodialysisExaminationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveHemodialysisExamination' endpoint here] end
	public ResponseEntity<?> saveHemodialysisExamination(
			@Parameter(description = "The HemodialysisExaminationEntity to create or update.")
			@RequestBody HemodialysisExaminationEntityDto hemodialysisExaminationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			HemodialysisExaminationEntity hemodialysisExaminationEntity = hemodialysisExaminationService.create(new HemodialysisExaminationEntity(hemodialysisExaminationEntityDto));
			return new ResponseEntity<>(hemodialysisExaminationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Hemodialysis Examination. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param hemodialysisExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateHemodialysisExamination' endpoint here] off begin
	@Operation(
			description = "Update a HemodialysisExamination entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the HemodialysisExamination."),
		@ApiResponse(responseCode = "400", description = "Failed to update the HemodialysisExamination, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('HemodialysisExaminationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateHemodialysisExamination' endpoint here] end
	public ResponseEntity<?> updateHemodialysisExamination(
			@Parameter(description = "The HemodialysisExaminationEntity to create or update.")
			@RequestBody HemodialysisExaminationEntityDto hemodialysisExaminationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<HemodialysisExaminationEntity> checkHemodialysisExaminationExists = hemodialysisExaminationService.findById(hemodialysisExaminationEntityDto.getId());
		HemodialysisExaminationEntity hemodialysisExaminationEntity = new HemodialysisExaminationEntity(hemodialysisExaminationEntityDto);

		if (checkHemodialysisExaminationExists.isPresent()) {
			try {
				hemodialysisExaminationEntity = hemodialysisExaminationService.update(hemodialysisExaminationEntity);
				return new ResponseEntity<>(hemodialysisExaminationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(hemodialysisExaminationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Hemodialysis Examination that has the same id as the given id.
	 *
	 * @param id The id of the HemodialysisExaminationEntity to be deleted
	...
	@Operation(
			description = "Delete a single HemodialysisExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the HemodialysisExamination to delete.")
		...
		hemodialysisExaminationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisExamination' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Hemodialysis Examination entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisExamination' endpoint here] end
	public void exportHemodialysisExamination(
			@Parameter(description = "The ids of the HemodialysisExaminationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<HemodialysisExaminationEntity> entitiesToExport = hemodialysisExaminationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "HemodialysisExamination_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (HemodialysisExaminationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisExaminationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportHemodialysisExaminationExcludingIds' endpoint here] end
	public void exportHemodialysisExaminationExcludingIds(
			@Parameter(description = "The ids of the HemodialysisExaminationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<HemodialysisExaminationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = hemodialysisExaminationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = hemodialysisExaminationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the HemodialysisExamination entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "HemodialysisExamination_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(HemodialysisExaminationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Hemodialysis Examination entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('HemodialysisExaminationEntity', 'create')  || hasPermission('HemodialysisExaminationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with HemodialysisExaminationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("HemodialysisExaminationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(HemodialysisExaminationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<HemodialysisExaminationEntity> csvToBean = new CsvToBeanBuilder<HemodialysisExaminationEntity>(reader)
				.withType(HemodialysisExaminationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<HemodialysisExaminationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<HemodialysisExaminationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Hemodialysis Examination entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Hemodialysis Examination entity and you do not have permission");
					}
