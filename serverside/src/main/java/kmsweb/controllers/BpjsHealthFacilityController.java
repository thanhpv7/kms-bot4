...
import kmsweb.entities.BpjsHealthFacilityEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Health Facility
 */
// % protected region % [Update the API route or description for the BPJS Health FacilityController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsHealthFacilityController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjshealthfacility")
// % protected region % [Update the API route or description for the BPJS Health FacilityController here] end
public class BpjsHealthFacilityController {

	private final BpjsHealthFacilityService bpjsHealthFacilityService;
	private final CsvUtils<BpjsHealthFacilityEntity, BpjsHealthFacilityRepository, BpjsHealthFacilityEntityAudit,  BpjsHealthFacilityService> csvUtils;
	...
	@Autowired
	public BpjsHealthFacilityController(
		...
		this.csvUtils = new CsvUtils<>(bpjsHealthFacilityService, BpjsHealthFacilityEntity.class);
	...
	public ResponseEntity<List<BpjsHealthFacilityEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsHealthFacilityEntity> bpjsHealthFacilitys = bpjsHealthFacilityService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsHealthFacilitys, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Health Facility that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsHealthFacilityEntity to be returned
	 * @return the BPJS Health Facility that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsHealthFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsHealthFacility entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsHealthFacilityEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsHealthFacilityEntity> getById(
			@Parameter(description = "The UUID of the BpjsHealthFacility to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsHealthFacilityEntity> bpjsHealthFacilityEntity = bpjsHealthFacilityService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Health Facility. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsHealthFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsHealthFacility' endpoint here] off begin
	@Operation(
			description = "Create a BpjsHealthFacility if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsHealthFacility."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsHealthFacility, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsHealthFacilityEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsHealthFacility' endpoint here] end
	public ResponseEntity<?> saveBpjsHealthFacility(
			@Parameter(description = "The BpjsHealthFacilityEntity to create or update.")
			@RequestBody BpjsHealthFacilityEntityDto bpjsHealthFacilityEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsHealthFacilityEntity bpjsHealthFacilityEntity = bpjsHealthFacilityService.create(new BpjsHealthFacilityEntity(bpjsHealthFacilityEntityDto));
			return new ResponseEntity<>(bpjsHealthFacilityEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Health Facility. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsHealthFacilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsHealthFacility' endpoint here] off begin
	@Operation(
			description = "Update a BpjsHealthFacility entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsHealthFacility."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsHealthFacility, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsHealthFacilityEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsHealthFacility' endpoint here] end
	public ResponseEntity<?> updateBpjsHealthFacility(
			@Parameter(description = "The BpjsHealthFacilityEntity to create or update.")
			@RequestBody BpjsHealthFacilityEntityDto bpjsHealthFacilityEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsHealthFacilityEntity> checkBpjsHealthFacilityExists = bpjsHealthFacilityService.findById(bpjsHealthFacilityEntityDto.getId());
		BpjsHealthFacilityEntity bpjsHealthFacilityEntity = new BpjsHealthFacilityEntity(bpjsHealthFacilityEntityDto);

		if (checkBpjsHealthFacilityExists.isPresent()) {
			try {
				bpjsHealthFacilityEntity = bpjsHealthFacilityService.update(bpjsHealthFacilityEntity);
				return new ResponseEntity<>(bpjsHealthFacilityEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsHealthFacilityEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Health Facility that has the same id as the given id.
	 *
	 * @param id The id of the BpjsHealthFacilityEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsHealthFacility as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsHealthFacility to delete.")
		...
		bpjsHealthFacilityService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsHealthFacility' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Health Facility entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsHealthFacility' endpoint here] end
	public void exportBpjsHealthFacility(
			@Parameter(description = "The ids of the BpjsHealthFacilityEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsHealthFacilityEntity> entitiesToExport = bpjsHealthFacilityService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsHealthFacility_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsHealthFacilityEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsHealthFacilityExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsHealthFacilityExcludingIds' endpoint here] end
	public void exportBpjsHealthFacilityExcludingIds(
			@Parameter(description = "The ids of the BpjsHealthFacilityEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsHealthFacilityEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsHealthFacilityService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsHealthFacilityService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsHealthFacility entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsHealthFacility_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsHealthFacilityEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Health Facility entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsHealthFacilityEntity', 'create')  || hasPermission('BpjsHealthFacilityEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsHealthFacilityEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsHealthFacilityEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsHealthFacilityEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsHealthFacilityEntity> csvToBean = new CsvToBeanBuilder<BpjsHealthFacilityEntity>(reader)
				.withType(BpjsHealthFacilityEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsHealthFacilityEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsHealthFacilityEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Health Facility entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Health Facility entity and you do not have permission");
					}
