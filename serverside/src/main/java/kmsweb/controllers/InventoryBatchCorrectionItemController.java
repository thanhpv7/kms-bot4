...
/**
 * Controller used to handle all REST operations regarding Inventory Batch Correction Item
 */
// % protected region % [Update the API route or description for the Inventory Batch Correction ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InventoryBatchCorrectionItemController"))
@RestController
@Slf4j
@RequestMapping("/api/inventorybatchcorrectionitem")
// % protected region % [Update the API route or description for the Inventory Batch Correction ItemController here] end
public class InventoryBatchCorrectionItemController {

	private final InventoryBatchCorrectionItemService inventoryBatchCorrectionItemService;
	private final CsvUtils<InventoryBatchCorrectionItemEntity, InventoryBatchCorrectionItemRepository, InventoryBatchCorrectionItemEntityAudit,  InventoryBatchCorrectionItemService> csvUtils;
	...
	@Autowired
	public InventoryBatchCorrectionItemController(
		...
		this.csvUtils = new CsvUtils<>(inventoryBatchCorrectionItemService, InventoryBatchCorrectionItemEntity.class);
	...
	/**
	 * Return all the Inventory Batch Correction Items.
	 *
	 * @return all the Inventory Batch Correction Items
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of InventoryBatchCorrectionItems",
		security = {
	...
	public ResponseEntity<List<InventoryBatchCorrectionItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InventoryBatchCorrectionItemEntity> inventoryBatchCorrectionItems = inventoryBatchCorrectionItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(inventoryBatchCorrectionItems, HttpStatus.OK);
	}

	/**
	 * Return the Inventory Batch Correction Item that has the same id as the given id.
	 *
	 * @param id      The id of the InventoryBatchCorrectionItemEntity to be returned
	 * @return the Inventory Batch Correction Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InventoryBatchCorrectionItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InventoryBatchCorrectionItem entity not found"),
	})
	@PreAuthorize("hasPermission('InventoryBatchCorrectionItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InventoryBatchCorrectionItemEntity> getById(
			@Parameter(description = "The UUID of the InventoryBatchCorrectionItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InventoryBatchCorrectionItemEntity> inventoryBatchCorrectionItemEntity = inventoryBatchCorrectionItemService.findById(id);
	...
	/**
	 * Create/Update the given Inventory Batch Correction Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryBatchCorrectionItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInventoryBatchCorrectionItem' endpoint here] off begin
	@Operation(
			description = "Create a InventoryBatchCorrectionItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InventoryBatchCorrectionItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InventoryBatchCorrectionItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('InventoryBatchCorrectionItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInventoryBatchCorrectionItem' endpoint here] end
	public ResponseEntity<?> saveInventoryBatchCorrectionItem(
			@Parameter(description = "The InventoryBatchCorrectionItemEntity to create or update.")
			@RequestBody InventoryBatchCorrectionItemEntityDto inventoryBatchCorrectionItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InventoryBatchCorrectionItemEntity inventoryBatchCorrectionItemEntity = inventoryBatchCorrectionItemService.create(new InventoryBatchCorrectionItemEntity(inventoryBatchCorrectionItemEntityDto));
			return new ResponseEntity<>(inventoryBatchCorrectionItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Inventory Batch Correction Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryBatchCorrectionItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInventoryBatchCorrectionItem' endpoint here] off begin
	@Operation(
			description = "Update a InventoryBatchCorrectionItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InventoryBatchCorrectionItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InventoryBatchCorrectionItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InventoryBatchCorrectionItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInventoryBatchCorrectionItem' endpoint here] end
	public ResponseEntity<?> updateInventoryBatchCorrectionItem(
			@Parameter(description = "The InventoryBatchCorrectionItemEntity to create or update.")
			@RequestBody InventoryBatchCorrectionItemEntityDto inventoryBatchCorrectionItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InventoryBatchCorrectionItemEntity> checkInventoryBatchCorrectionItemExists = inventoryBatchCorrectionItemService.findById(inventoryBatchCorrectionItemEntityDto.getId());
		InventoryBatchCorrectionItemEntity inventoryBatchCorrectionItemEntity = new InventoryBatchCorrectionItemEntity(inventoryBatchCorrectionItemEntityDto);

		if (checkInventoryBatchCorrectionItemExists.isPresent()) {
			try {
				inventoryBatchCorrectionItemEntity = inventoryBatchCorrectionItemService.update(inventoryBatchCorrectionItemEntity);
				return new ResponseEntity<>(inventoryBatchCorrectionItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(inventoryBatchCorrectionItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Inventory Batch Correction Item that has the same id as the given id.
	 *
	 * @param id The id of the InventoryBatchCorrectionItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single InventoryBatchCorrectionItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InventoryBatchCorrectionItem to delete.")
		...
		inventoryBatchCorrectionItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrectionItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Inventory Batch Correction Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrectionItem' endpoint here] end
	public void exportInventoryBatchCorrectionItem(
			@Parameter(description = "The ids of the InventoryBatchCorrectionItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InventoryBatchCorrectionItemEntity> entitiesToExport = inventoryBatchCorrectionItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InventoryBatchCorrectionItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InventoryBatchCorrectionItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrectionItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryBatchCorrectionItemExcludingIds' endpoint here] end
	public void exportInventoryBatchCorrectionItemExcludingIds(
			@Parameter(description = "The ids of the InventoryBatchCorrectionItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InventoryBatchCorrectionItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = inventoryBatchCorrectionItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = inventoryBatchCorrectionItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InventoryBatchCorrectionItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InventoryBatchCorrectionItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InventoryBatchCorrectionItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Inventory Batch Correction Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InventoryBatchCorrectionItemEntity', 'create')  || hasPermission('InventoryBatchCorrectionItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InventoryBatchCorrectionItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InventoryBatchCorrectionItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InventoryBatchCorrectionItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InventoryBatchCorrectionItemEntity> csvToBean = new CsvToBeanBuilder<InventoryBatchCorrectionItemEntity>(reader)
				.withType(InventoryBatchCorrectionItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InventoryBatchCorrectionItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InventoryBatchCorrectionItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Inventory Batch Correction Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Inventory Batch Correction Item entity and you do not have permission");
					}
