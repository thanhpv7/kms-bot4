...
import kmsweb.entities.TariffSchemaEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Tariff Schema
 */
// % protected region % [Update the API route or description for the Tariff SchemaController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TariffSchemaController"))
@RestController
@Slf4j
@RequestMapping("/api/tariffschema")
// % protected region % [Update the API route or description for the Tariff SchemaController here] end
public class TariffSchemaController {

	private final TariffSchemaService tariffSchemaService;
	private final CsvUtils<TariffSchemaEntity, TariffSchemaRepository, TariffSchemaEntityAudit,  TariffSchemaService> csvUtils;
	...
	public TariffSchemaController(
		...
		this.csvUtils = new CsvUtils<>(tariffSchemaService, TariffSchemaEntity.class);
	...
	public ResponseEntity<List<TariffSchemaEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TariffSchemaEntity> tariffSchemas = tariffSchemaService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(tariffSchemas, HttpStatus.OK);
	}

	/**
	 * Return the Tariff Schema that has the same id as the given id.
	 *
	 * @param id      The id of the TariffSchemaEntity to be returned
	 * @return the Tariff Schema that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TariffSchema as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TariffSchema entity not found"),
	})
	@PreAuthorize("hasPermission('TariffSchemaEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TariffSchemaEntity> getById(
			@Parameter(description = "The UUID of the TariffSchema to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TariffSchemaEntity> tariffSchemaEntity = tariffSchemaService.findById(id);
	...
	/**
	 * Create/Update the given Tariff Schema. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param tariffSchemaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTariffSchema' endpoint here] off begin
	@Operation(
			description = "Create a TariffSchema if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TariffSchema."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TariffSchema, duplicate record found")
	})
	@PreAuthorize("hasPermission('TariffSchemaEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTariffSchema' endpoint here] end
	public ResponseEntity<?> saveTariffSchema(
			@Parameter(description = "The TariffSchemaEntity to create or update.")
			@RequestBody TariffSchemaEntityDto tariffSchemaEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TariffSchemaEntity tariffSchemaEntity = tariffSchemaService.create(new TariffSchemaEntity(tariffSchemaEntityDto));
			return new ResponseEntity<>(tariffSchemaEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Tariff Schema. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param tariffSchemaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTariffSchema' endpoint here] off begin
	@Operation(
			description = "Update a TariffSchema entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TariffSchema."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TariffSchema, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TariffSchemaEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTariffSchema' endpoint here] end
	public ResponseEntity<?> updateTariffSchema(
			@Parameter(description = "The TariffSchemaEntity to create or update.")
			@RequestBody TariffSchemaEntityDto tariffSchemaEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TariffSchemaEntity> checkTariffSchemaExists = tariffSchemaService.findById(tariffSchemaEntityDto.getId());
		TariffSchemaEntity tariffSchemaEntity = new TariffSchemaEntity(tariffSchemaEntityDto);

		if (checkTariffSchemaExists.isPresent()) {
			try {
				tariffSchemaEntity = tariffSchemaService.update(tariffSchemaEntity);
				return new ResponseEntity<>(tariffSchemaEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(tariffSchemaEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Tariff Schema that has the same id as the given id.
	 *
	 * @param id The id of the TariffSchemaEntity to be deleted
	...
	@Operation(
			description = "Delete a single TariffSchema as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TariffSchema to delete.")
		...
		tariffSchemaService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffSchema' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Tariff Schema entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffSchema' endpoint here] end
	public void exportTariffSchema(
			@Parameter(description = "The ids of the TariffSchemaEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TariffSchemaEntity> entitiesToExport = tariffSchemaService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TariffSchema_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TariffSchemaEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffSchemaExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffSchemaExcludingIds' endpoint here] end
	public void exportTariffSchemaExcludingIds(
			@Parameter(description = "The ids of the TariffSchemaEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TariffSchemaEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = tariffSchemaService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = tariffSchemaService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TariffSchema entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TariffSchema_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TariffSchemaEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Tariff Schema entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TariffSchemaEntity', 'create')  || hasPermission('TariffSchemaEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TariffSchemaEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TariffSchemaEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TariffSchemaEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TariffSchemaEntity> csvToBean = new CsvToBeanBuilder<TariffSchemaEntity>(reader)
				.withType(TariffSchemaEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TariffSchemaEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TariffSchemaEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Tariff Schema entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Tariff Schema entity and you do not have permission");
					}
