...
import kmsweb.entities.DeliveryProgressEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Delivery Progress
 */
// % protected region % [Update the API route or description for the Delivery ProgressController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DeliveryProgressController"))
@RestController
@Slf4j
@RequestMapping("/api/deliveryprogress")
// % protected region % [Update the API route or description for the Delivery ProgressController here] end
public class DeliveryProgressController {

	private final DeliveryProgressService deliveryProgressService;
	private final CsvUtils<DeliveryProgressEntity, DeliveryProgressRepository, DeliveryProgressEntityAudit,  DeliveryProgressService> csvUtils;
	...
	@Autowired
	public DeliveryProgressController(
		...
		this.csvUtils = new CsvUtils<>(deliveryProgressService, DeliveryProgressEntity.class);
	...
	public ResponseEntity<List<DeliveryProgressEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DeliveryProgressEntity> deliveryProgresss = deliveryProgressService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(deliveryProgresss, HttpStatus.OK);
	}

	/**
	 * Return the Delivery Progress that has the same id as the given id.
	 *
	 * @param id      The id of the DeliveryProgressEntity to be returned
	 * @return the Delivery Progress that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DeliveryProgress as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DeliveryProgress entity not found"),
	})
	@PreAuthorize("hasPermission('DeliveryProgressEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DeliveryProgressEntity> getById(
			@Parameter(description = "The UUID of the DeliveryProgress to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DeliveryProgressEntity> deliveryProgressEntity = deliveryProgressService.findById(id);
	...
	/**
	 * Create/Update the given Delivery Progress. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param deliveryProgressEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDeliveryProgress' endpoint here] off begin
	@Operation(
			description = "Create a DeliveryProgress if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DeliveryProgress."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DeliveryProgress, duplicate record found")
	})
	@PreAuthorize("hasPermission('DeliveryProgressEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDeliveryProgress' endpoint here] end
	public ResponseEntity<?> saveDeliveryProgress(
			@Parameter(description = "The DeliveryProgressEntity to create or update.")
			@RequestBody DeliveryProgressEntityDto deliveryProgressEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DeliveryProgressEntity deliveryProgressEntity = deliveryProgressService.create(new DeliveryProgressEntity(deliveryProgressEntityDto));
			return new ResponseEntity<>(deliveryProgressEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Delivery Progress. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param deliveryProgressEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDeliveryProgress' endpoint here] off begin
	@Operation(
			description = "Update a DeliveryProgress entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DeliveryProgress."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DeliveryProgress, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DeliveryProgressEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDeliveryProgress' endpoint here] end
	public ResponseEntity<?> updateDeliveryProgress(
		...
		Optional<DeliveryProgressEntity> checkDeliveryProgressExists = deliveryProgressService.findById(deliveryProgressEntityDto.getId());
		DeliveryProgressEntity deliveryProgressEntity = new DeliveryProgressEntity(deliveryProgressEntityDto);

		if (checkDeliveryProgressExists.isPresent()) {
			try {
				deliveryProgressEntity = deliveryProgressService.update(deliveryProgressEntity);
				return new ResponseEntity<>(deliveryProgressEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(deliveryProgressEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Delivery Progress that has the same id as the given id.
	 *
	 * @param id The id of the DeliveryProgressEntity to be deleted
	...
	@Operation(
			description = "Delete a single DeliveryProgress as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DeliveryProgress to delete.")
		...
		deliveryProgressService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDeliveryProgress' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Delivery Progress entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDeliveryProgress' endpoint here] end
	public void exportDeliveryProgress(
			@Parameter(description = "The ids of the DeliveryProgressEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DeliveryProgressEntity> entitiesToExport = deliveryProgressService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DeliveryProgress_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DeliveryProgressEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDeliveryProgressExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDeliveryProgressExcludingIds' endpoint here] end
	public void exportDeliveryProgressExcludingIds(
			@Parameter(description = "The ids of the DeliveryProgressEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DeliveryProgressEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = deliveryProgressService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = deliveryProgressService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DeliveryProgress entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DeliveryProgress_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DeliveryProgressEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Delivery Progress entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DeliveryProgressEntity', 'create')  || hasPermission('DeliveryProgressEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DeliveryProgressEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DeliveryProgressEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DeliveryProgressEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DeliveryProgressEntity> csvToBean = new CsvToBeanBuilder<DeliveryProgressEntity>(reader)
				.withType(DeliveryProgressEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DeliveryProgressEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DeliveryProgressEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Delivery Progress entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Delivery Progress entity and you do not have permission");
					}
