...
import kmsweb.entities.MedicalFeeItemComponentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medical Fee Item Component
 */
// % protected region % [Update the API route or description for the Medical Fee Item ComponentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicalFeeItemComponentController"))
@RestController
@Slf4j
@RequestMapping("/api/medicalfeeitemcomponent")
// % protected region % [Update the API route or description for the Medical Fee Item ComponentController here] end
public class MedicalFeeItemComponentController {

	private final MedicalFeeItemComponentService medicalFeeItemComponentService;
	private final CsvUtils<MedicalFeeItemComponentEntity, MedicalFeeItemComponentRepository, MedicalFeeItemComponentEntityAudit,  MedicalFeeItemComponentService> csvUtils;
	...
	@Autowired
	public MedicalFeeItemComponentController(
		...
		this.csvUtils = new CsvUtils<>(medicalFeeItemComponentService, MedicalFeeItemComponentEntity.class);
	...
	/**
	 * Return all the Medical Fee Item Components.
	 *
	 * @return all the Medical Fee Item Components
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of MedicalFeeItemComponents",
		security = {
	...
	public ResponseEntity<List<MedicalFeeItemComponentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicalFeeItemComponentEntity> medicalFeeItemComponents = medicalFeeItemComponentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicalFeeItemComponents, HttpStatus.OK);
	}

	/**
	 * Return the Medical Fee Item Component that has the same id as the given id.
	 *
	 * @param id      The id of the MedicalFeeItemComponentEntity to be returned
	 * @return the Medical Fee Item Component that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicalFeeItemComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicalFeeItemComponent entity not found"),
	})
	@PreAuthorize("hasPermission('MedicalFeeItemComponentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicalFeeItemComponentEntity> getById(
			@Parameter(description = "The UUID of the MedicalFeeItemComponent to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicalFeeItemComponentEntity> medicalFeeItemComponentEntity = medicalFeeItemComponentService.findById(id);
	...
	/**
	 * Create/Update the given Medical Fee Item Component. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicalFeeItemComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicalFeeItemComponent' endpoint here] off begin
	@Operation(
			description = "Create a MedicalFeeItemComponent if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicalFeeItemComponent."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicalFeeItemComponent, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicalFeeItemComponentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicalFeeItemComponent' endpoint here] end
	public ResponseEntity<?> saveMedicalFeeItemComponent(
			@Parameter(description = "The MedicalFeeItemComponentEntity to create or update.")
			@RequestBody MedicalFeeItemComponentEntityDto medicalFeeItemComponentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicalFeeItemComponentEntity medicalFeeItemComponentEntity = medicalFeeItemComponentService.create(new MedicalFeeItemComponentEntity(medicalFeeItemComponentEntityDto));
			return new ResponseEntity<>(medicalFeeItemComponentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medical Fee Item Component. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicalFeeItemComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicalFeeItemComponent' endpoint here] off begin
	@Operation(
			description = "Update a MedicalFeeItemComponent entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicalFeeItemComponent."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicalFeeItemComponent, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicalFeeItemComponentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicalFeeItemComponent' endpoint here] end
	public ResponseEntity<?> updateMedicalFeeItemComponent(
			@Parameter(description = "The MedicalFeeItemComponentEntity to create or update.")
			@RequestBody MedicalFeeItemComponentEntityDto medicalFeeItemComponentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicalFeeItemComponentEntity> checkMedicalFeeItemComponentExists = medicalFeeItemComponentService.findById(medicalFeeItemComponentEntityDto.getId());
		MedicalFeeItemComponentEntity medicalFeeItemComponentEntity = new MedicalFeeItemComponentEntity(medicalFeeItemComponentEntityDto);

		if (checkMedicalFeeItemComponentExists.isPresent()) {
			try {
				medicalFeeItemComponentEntity = medicalFeeItemComponentService.update(medicalFeeItemComponentEntity);
				return new ResponseEntity<>(medicalFeeItemComponentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicalFeeItemComponentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medical Fee Item Component that has the same id as the given id.
	 *
	 * @param id The id of the MedicalFeeItemComponentEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicalFeeItemComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicalFeeItemComponent to delete.")
		...
		medicalFeeItemComponentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalFeeItemComponent' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medical Fee Item Component entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalFeeItemComponent' endpoint here] end
	public void exportMedicalFeeItemComponent(
			@Parameter(description = "The ids of the MedicalFeeItemComponentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicalFeeItemComponentEntity> entitiesToExport = medicalFeeItemComponentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicalFeeItemComponent_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicalFeeItemComponentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalFeeItemComponentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalFeeItemComponentExcludingIds' endpoint here] end
	public void exportMedicalFeeItemComponentExcludingIds(
			@Parameter(description = "The ids of the MedicalFeeItemComponentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicalFeeItemComponentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicalFeeItemComponentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicalFeeItemComponentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicalFeeItemComponent entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicalFeeItemComponent_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicalFeeItemComponentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medical Fee Item Component entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicalFeeItemComponentEntity', 'create')  || hasPermission('MedicalFeeItemComponentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicalFeeItemComponentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicalFeeItemComponentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicalFeeItemComponentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicalFeeItemComponentEntity> csvToBean = new CsvToBeanBuilder<MedicalFeeItemComponentEntity>(reader)
				.withType(MedicalFeeItemComponentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicalFeeItemComponentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicalFeeItemComponentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medical Fee Item Component entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medical Fee Item Component entity and you do not have permission");
					}
