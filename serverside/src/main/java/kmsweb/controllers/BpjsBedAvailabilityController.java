...
/**
 * Controller used to handle all REST operations regarding BPJS Bed Availability
 */
// % protected region % [Update the API route or description for the BPJS Bed AvailabilityController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsBedAvailabilityController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsbedavailability")
// % protected region % [Update the API route or description for the BPJS Bed AvailabilityController here] end
public class BpjsBedAvailabilityController {

	private final BpjsBedAvailabilityService bpjsBedAvailabilityService;
	private final CsvUtils<BpjsBedAvailabilityEntity, BpjsBedAvailabilityRepository, BpjsBedAvailabilityEntityAudit,  BpjsBedAvailabilityService> csvUtils;
	...
	@Autowired
	public BpjsBedAvailabilityController(
		...
		this.csvUtils = new CsvUtils<>(bpjsBedAvailabilityService, BpjsBedAvailabilityEntity.class);
	...
	public ResponseEntity<List<BpjsBedAvailabilityEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsBedAvailabilityEntity> bpjsBedAvailabilitys = bpjsBedAvailabilityService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsBedAvailabilitys, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Bed Availability that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsBedAvailabilityEntity to be returned
	 * @return the BPJS Bed Availability that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsBedAvailability as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsBedAvailability entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsBedAvailabilityEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsBedAvailabilityEntity> getById(
			@Parameter(description = "The UUID of the BpjsBedAvailability to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsBedAvailabilityEntity> bpjsBedAvailabilityEntity = bpjsBedAvailabilityService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Bed Availability. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsBedAvailabilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsBedAvailability' endpoint here] off begin
	@Operation(
			description = "Create a BpjsBedAvailability if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsBedAvailability."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsBedAvailability, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsBedAvailabilityEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsBedAvailability' endpoint here] end
	public ResponseEntity<?> saveBpjsBedAvailability(
			@Parameter(description = "The BpjsBedAvailabilityEntity to create or update.")
			@RequestBody BpjsBedAvailabilityEntityDto bpjsBedAvailabilityEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsBedAvailabilityEntity bpjsBedAvailabilityEntity = bpjsBedAvailabilityService.create(new BpjsBedAvailabilityEntity(bpjsBedAvailabilityEntityDto));
			return new ResponseEntity<>(bpjsBedAvailabilityEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Bed Availability. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsBedAvailabilityEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsBedAvailability' endpoint here] off begin
	@Operation(
			description = "Update a BpjsBedAvailability entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsBedAvailability."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsBedAvailability, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsBedAvailabilityEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsBedAvailability' endpoint here] end
	public ResponseEntity<?> updateBpjsBedAvailability(
			@Parameter(description = "The BpjsBedAvailabilityEntity to create or update.")
			@RequestBody BpjsBedAvailabilityEntityDto bpjsBedAvailabilityEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsBedAvailabilityEntity> checkBpjsBedAvailabilityExists = bpjsBedAvailabilityService.findById(bpjsBedAvailabilityEntityDto.getId());
		BpjsBedAvailabilityEntity bpjsBedAvailabilityEntity = new BpjsBedAvailabilityEntity(bpjsBedAvailabilityEntityDto);

		if (checkBpjsBedAvailabilityExists.isPresent()) {
			try {
				bpjsBedAvailabilityEntity = bpjsBedAvailabilityService.update(bpjsBedAvailabilityEntity);
				return new ResponseEntity<>(bpjsBedAvailabilityEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsBedAvailabilityEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Bed Availability that has the same id as the given id.
	 *
	 * @param id The id of the BpjsBedAvailabilityEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsBedAvailability as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsBedAvailability to delete.")
		...
		bpjsBedAvailabilityService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsBedAvailability' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Bed Availability entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsBedAvailability' endpoint here] end
	public void exportBpjsBedAvailability(
			@Parameter(description = "The ids of the BpjsBedAvailabilityEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsBedAvailabilityEntity> entitiesToExport = bpjsBedAvailabilityService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsBedAvailability_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsBedAvailabilityEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsBedAvailabilityExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsBedAvailabilityExcludingIds' endpoint here] end
	public void exportBpjsBedAvailabilityExcludingIds(
			@Parameter(description = "The ids of the BpjsBedAvailabilityEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsBedAvailabilityEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsBedAvailabilityService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsBedAvailabilityService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsBedAvailability entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsBedAvailability_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsBedAvailabilityEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Bed Availability entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsBedAvailabilityEntity', 'create')  || hasPermission('BpjsBedAvailabilityEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsBedAvailabilityEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsBedAvailabilityEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsBedAvailabilityEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsBedAvailabilityEntity> csvToBean = new CsvToBeanBuilder<BpjsBedAvailabilityEntity>(reader)
				.withType(BpjsBedAvailabilityEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsBedAvailabilityEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsBedAvailabilityEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Bed Availability entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Bed Availability entity and you do not have permission");
					}
