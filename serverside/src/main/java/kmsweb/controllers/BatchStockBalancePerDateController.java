...
/**
 * Controller used to handle all REST operations regarding Batch Stock Balance Per Date
 */
// % protected region % [Update the API route or description for the Batch Stock Balance Per DateController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BatchStockBalancePerDateController"))
@RestController
@Slf4j
@RequestMapping("/api/batchstockbalanceperdate")
// % protected region % [Update the API route or description for the Batch Stock Balance Per DateController here] end
public class BatchStockBalancePerDateController {

	private final BatchStockBalancePerDateService batchStockBalancePerDateService;
	private final CsvUtils<BatchStockBalancePerDateEntity, BatchStockBalancePerDateRepository, BatchStockBalancePerDateEntityAudit,  BatchStockBalancePerDateService> csvUtils;
	...
	@Autowired
	public BatchStockBalancePerDateController(
		...
		this.csvUtils = new CsvUtils<>(batchStockBalancePerDateService, BatchStockBalancePerDateEntity.class);
	...
	/**
	 * Return all the Batch Stock Balance Per Dates.
	 *
	 * @return all the Batch Stock Balance Per Dates
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BatchStockBalancePerDates",
		security = {
	...
	public ResponseEntity<List<BatchStockBalancePerDateEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BatchStockBalancePerDateEntity> batchStockBalancePerDates = batchStockBalancePerDateService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(batchStockBalancePerDates, HttpStatus.OK);
	}

	/**
	 * Return the Batch Stock Balance Per Date that has the same id as the given id.
	 *
	 * @param id      The id of the BatchStockBalancePerDateEntity to be returned
	 * @return the Batch Stock Balance Per Date that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BatchStockBalancePerDate as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BatchStockBalancePerDate entity not found"),
	})
	@PreAuthorize("hasPermission('BatchStockBalancePerDateEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BatchStockBalancePerDateEntity> getById(
			@Parameter(description = "The UUID of the BatchStockBalancePerDate to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BatchStockBalancePerDateEntity> batchStockBalancePerDateEntity = batchStockBalancePerDateService.findById(id);
	...
	/**
	 * Create/Update the given Batch Stock Balance Per Date. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param batchStockBalancePerDateEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBatchStockBalancePerDate' endpoint here] off begin
	@Operation(
			description = "Create a BatchStockBalancePerDate if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BatchStockBalancePerDate."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BatchStockBalancePerDate, duplicate record found")
	})
	@PreAuthorize("hasPermission('BatchStockBalancePerDateEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBatchStockBalancePerDate' endpoint here] end
	public ResponseEntity<?> saveBatchStockBalancePerDate(
			@Parameter(description = "The BatchStockBalancePerDateEntity to create or update.")
			@RequestBody BatchStockBalancePerDateEntityDto batchStockBalancePerDateEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BatchStockBalancePerDateEntity batchStockBalancePerDateEntity = batchStockBalancePerDateService.create(new BatchStockBalancePerDateEntity(batchStockBalancePerDateEntityDto));
			return new ResponseEntity<>(batchStockBalancePerDateEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Batch Stock Balance Per Date. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param batchStockBalancePerDateEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBatchStockBalancePerDate' endpoint here] off begin
	@Operation(
			description = "Update a BatchStockBalancePerDate entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BatchStockBalancePerDate."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BatchStockBalancePerDate, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BatchStockBalancePerDateEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBatchStockBalancePerDate' endpoint here] end
	public ResponseEntity<?> updateBatchStockBalancePerDate(
			@Parameter(description = "The BatchStockBalancePerDateEntity to create or update.")
			@RequestBody BatchStockBalancePerDateEntityDto batchStockBalancePerDateEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BatchStockBalancePerDateEntity> checkBatchStockBalancePerDateExists = batchStockBalancePerDateService.findById(batchStockBalancePerDateEntityDto.getId());
		BatchStockBalancePerDateEntity batchStockBalancePerDateEntity = new BatchStockBalancePerDateEntity(batchStockBalancePerDateEntityDto);

		if (checkBatchStockBalancePerDateExists.isPresent()) {
			try {
				batchStockBalancePerDateEntity = batchStockBalancePerDateService.update(batchStockBalancePerDateEntity);
				return new ResponseEntity<>(batchStockBalancePerDateEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(batchStockBalancePerDateEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Batch Stock Balance Per Date that has the same id as the given id.
	 *
	 * @param id The id of the BatchStockBalancePerDateEntity to be deleted
	...
	@Operation(
			description = "Delete a single BatchStockBalancePerDate as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BatchStockBalancePerDate to delete.")
		...
		batchStockBalancePerDateService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalancePerDate' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Batch Stock Balance Per Date entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalancePerDate' endpoint here] end
	public void exportBatchStockBalancePerDate(
			@Parameter(description = "The ids of the BatchStockBalancePerDateEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BatchStockBalancePerDateEntity> entitiesToExport = batchStockBalancePerDateService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BatchStockBalancePerDate_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BatchStockBalancePerDateEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalancePerDateExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBatchStockBalancePerDateExcludingIds' endpoint here] end
	public void exportBatchStockBalancePerDateExcludingIds(
			@Parameter(description = "The ids of the BatchStockBalancePerDateEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BatchStockBalancePerDateEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = batchStockBalancePerDateService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = batchStockBalancePerDateService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BatchStockBalancePerDate entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BatchStockBalancePerDate_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BatchStockBalancePerDateEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Batch Stock Balance Per Date entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BatchStockBalancePerDateEntity', 'create')  || hasPermission('BatchStockBalancePerDateEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BatchStockBalancePerDateEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BatchStockBalancePerDateEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BatchStockBalancePerDateEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BatchStockBalancePerDateEntity> csvToBean = new CsvToBeanBuilder<BatchStockBalancePerDateEntity>(reader)
				.withType(BatchStockBalancePerDateEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BatchStockBalancePerDateEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BatchStockBalancePerDateEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Batch Stock Balance Per Date entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Batch Stock Balance Per Date entity and you do not have permission");
					}
