...
import kmsweb.entities.MedicalTranscriberUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medical Transcriber User
 */
// % protected region % [Update the API route or description for the Medical Transcriber UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicalTranscriberUserController"))
@RestController
@Slf4j
@RequestMapping("/api/medicaltranscriberuser")
// % protected region % [Update the API route or description for the Medical Transcriber UserController here] end
public class MedicalTranscriberUserController {

	private final MedicalTranscriberUserService medicalTranscriberUserService;
	private final CsvUtils<MedicalTranscriberUserEntity, MedicalTranscriberUserRepository, MedicalTranscriberUserEntityAudit,  MedicalTranscriberUserService> csvUtils;
	...
	@Autowired
	public MedicalTranscriberUserController(
		...
		this.csvUtils = new CsvUtils<>(medicalTranscriberUserService, MedicalTranscriberUserEntity.class);
	...
	public ResponseEntity<List<MedicalTranscriberUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicalTranscriberUserEntity> medicalTranscriberUsers = medicalTranscriberUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicalTranscriberUsers, HttpStatus.OK);
	}

	/**
	 * Return the Medical Transcriber User that has the same id as the given id.
	 *
	 * @param id      The id of the MedicalTranscriberUserEntity to be returned
	 * @return the Medical Transcriber User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicalTranscriberUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicalTranscriberUser entity not found"),
	})
	@PreAuthorize("hasPermission('MedicalTranscriberUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicalTranscriberUserEntity> getById(
			@Parameter(description = "The UUID of the MedicalTranscriberUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicalTranscriberUserEntity> medicalTranscriberUserEntity = medicalTranscriberUserService.findById(id);
	...
	/**
	 * Create/Update the given Medical Transcriber User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicalTranscriberUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicalTranscriberUser' endpoint here] off begin
	@Operation(
			description = "Create a MedicalTranscriberUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicalTranscriberUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicalTranscriberUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicalTranscriberUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicalTranscriberUser' endpoint here] end
	public ResponseEntity<?> saveMedicalTranscriberUser(
			@Parameter(description = "The MedicalTranscriberUserEntity to create or update.")
			@RequestBody MedicalTranscriberUserEntityDto medicalTranscriberUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicalTranscriberUserEntity medicalTranscriberUserEntity = medicalTranscriberUserService.create(new MedicalTranscriberUserEntity(medicalTranscriberUserEntityDto));
			return new ResponseEntity<>(medicalTranscriberUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medical Transcriber User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicalTranscriberUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicalTranscriberUser' endpoint here] off begin
	@Operation(
			description = "Update a MedicalTranscriberUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicalTranscriberUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicalTranscriberUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicalTranscriberUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicalTranscriberUser' endpoint here] end
	public ResponseEntity<?> updateMedicalTranscriberUser(
			@Parameter(description = "The MedicalTranscriberUserEntity to create or update.")
			@RequestBody MedicalTranscriberUserEntityDto medicalTranscriberUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicalTranscriberUserEntity> checkMedicalTranscriberUserExists = medicalTranscriberUserService.findById(medicalTranscriberUserEntityDto.getId());
		MedicalTranscriberUserEntity medicalTranscriberUserEntity = new MedicalTranscriberUserEntity(medicalTranscriberUserEntityDto);

		if (checkMedicalTranscriberUserExists.isPresent()) {
			try {
				medicalTranscriberUserEntity = medicalTranscriberUserService.update(medicalTranscriberUserEntity);
				return new ResponseEntity<>(medicalTranscriberUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicalTranscriberUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medical Transcriber User that has the same id as the given id.
	 *
	 * @param id The id of the MedicalTranscriberUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicalTranscriberUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('MedicalTranscriberUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicalTranscriberUser to delete.")
		...
		medicalTranscriberUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalTranscriberUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medical Transcriber User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalTranscriberUser' endpoint here] end
	public void exportMedicalTranscriberUser(
			@Parameter(description = "The ids of the MedicalTranscriberUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicalTranscriberUserEntity> entitiesToExport = medicalTranscriberUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicalTranscriberUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicalTranscriberUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalTranscriberUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalTranscriberUserExcludingIds' endpoint here] end
	public void exportMedicalTranscriberUserExcludingIds(
			@Parameter(description = "The ids of the MedicalTranscriberUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicalTranscriberUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicalTranscriberUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicalTranscriberUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicalTranscriberUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicalTranscriberUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicalTranscriberUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medical Transcriber User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicalTranscriberUserEntity', 'create')  || hasPermission('MedicalTranscriberUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicalTranscriberUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicalTranscriberUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicalTranscriberUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicalTranscriberUserEntity> csvToBean = new CsvToBeanBuilder<MedicalTranscriberUserEntity>(reader)
				.withType(MedicalTranscriberUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicalTranscriberUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicalTranscriberUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medical Transcriber User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medical Transcriber User entity and you do not have permission");
					}
