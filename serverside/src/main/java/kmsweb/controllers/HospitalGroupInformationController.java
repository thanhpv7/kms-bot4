...
import kmsweb.entities.HospitalGroupInformationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Hospital Group Information
 */
// % protected region % [Update the API route or description for the Hospital Group InformationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "HospitalGroupInformationController"))
@RestController
@Slf4j
@RequestMapping("/api/hospitalgroupinformation")
// % protected region % [Update the API route or description for the Hospital Group InformationController here] end
public class HospitalGroupInformationController {

	private final HospitalGroupInformationService hospitalGroupInformationService;
	private final CsvUtils<HospitalGroupInformationEntity, HospitalGroupInformationRepository, HospitalGroupInformationEntityAudit,  HospitalGroupInformationService> csvUtils;
	...
	@Autowired
	public HospitalGroupInformationController(
		...
		this.csvUtils = new CsvUtils<>(hospitalGroupInformationService, HospitalGroupInformationEntity.class);
	...
	/**
	 * Return all the Hospital Group Informations.
	 *
	 * @return all the Hospital Group Informations
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of HospitalGroupInformations",
		security = {
	...
	public ResponseEntity<List<HospitalGroupInformationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<HospitalGroupInformationEntity> hospitalGroupInformations = hospitalGroupInformationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(hospitalGroupInformations, HttpStatus.OK);
	}

	/**
	 * Return the Hospital Group Information that has the same id as the given id.
	 *
	 * @param id      The id of the HospitalGroupInformationEntity to be returned
	 * @return the Hospital Group Information that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single HospitalGroupInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "HospitalGroupInformation entity not found"),
	})
	@PreAuthorize("hasPermission('HospitalGroupInformationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<HospitalGroupInformationEntity> getById(
			@Parameter(description = "The UUID of the HospitalGroupInformation to return.")
			@PathVariable("id") UUID id
	) {
		Optional<HospitalGroupInformationEntity> hospitalGroupInformationEntity = hospitalGroupInformationService.findById(id);
	...
	/**
	 * Create/Update the given Hospital Group Information. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param hospitalGroupInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveHospitalGroupInformation' endpoint here] off begin
	@Operation(
			description = "Create a HospitalGroupInformation if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new HospitalGroupInformation."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new HospitalGroupInformation, duplicate record found")
	})
	@PreAuthorize("hasPermission('HospitalGroupInformationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveHospitalGroupInformation' endpoint here] end
	public ResponseEntity<?> saveHospitalGroupInformation(
			@Parameter(description = "The HospitalGroupInformationEntity to create or update.")
			@RequestBody HospitalGroupInformationEntityDto hospitalGroupInformationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			HospitalGroupInformationEntity hospitalGroupInformationEntity = hospitalGroupInformationService.create(new HospitalGroupInformationEntity(hospitalGroupInformationEntityDto));
			return new ResponseEntity<>(hospitalGroupInformationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Hospital Group Information. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param hospitalGroupInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateHospitalGroupInformation' endpoint here] off begin
	@Operation(
			description = "Update a HospitalGroupInformation entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the HospitalGroupInformation."),
		@ApiResponse(responseCode = "400", description = "Failed to update the HospitalGroupInformation, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('HospitalGroupInformationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateHospitalGroupInformation' endpoint here] end
	public ResponseEntity<?> updateHospitalGroupInformation(
			@Parameter(description = "The HospitalGroupInformationEntity to create or update.")
			@RequestBody HospitalGroupInformationEntityDto hospitalGroupInformationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<HospitalGroupInformationEntity> checkHospitalGroupInformationExists = hospitalGroupInformationService.findById(hospitalGroupInformationEntityDto.getId());
		HospitalGroupInformationEntity hospitalGroupInformationEntity = new HospitalGroupInformationEntity(hospitalGroupInformationEntityDto);

		if (checkHospitalGroupInformationExists.isPresent()) {
			try {
				hospitalGroupInformationEntity = hospitalGroupInformationService.update(hospitalGroupInformationEntity);
				return new ResponseEntity<>(hospitalGroupInformationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(hospitalGroupInformationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Hospital Group Information that has the same id as the given id.
	 *
	 * @param id The id of the HospitalGroupInformationEntity to be deleted
	...
	@Operation(
			description = "Delete a single HospitalGroupInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the HospitalGroupInformation to delete.")
		...
		hospitalGroupInformationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportHospitalGroupInformation' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Hospital Group Information entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportHospitalGroupInformation' endpoint here] end
	public void exportHospitalGroupInformation(
			@Parameter(description = "The ids of the HospitalGroupInformationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<HospitalGroupInformationEntity> entitiesToExport = hospitalGroupInformationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "HospitalGroupInformation_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (HospitalGroupInformationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportHospitalGroupInformationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportHospitalGroupInformationExcludingIds' endpoint here] end
	public void exportHospitalGroupInformationExcludingIds(
			@Parameter(description = "The ids of the HospitalGroupInformationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<HospitalGroupInformationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = hospitalGroupInformationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = hospitalGroupInformationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the HospitalGroupInformation entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "HospitalGroupInformation_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(HospitalGroupInformationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Hospital Group Information entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('HospitalGroupInformationEntity', 'create')  || hasPermission('HospitalGroupInformationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with HospitalGroupInformationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("HospitalGroupInformationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(HospitalGroupInformationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<HospitalGroupInformationEntity> csvToBean = new CsvToBeanBuilder<HospitalGroupInformationEntity>(reader)
				.withType(HospitalGroupInformationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<HospitalGroupInformationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<HospitalGroupInformationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Hospital Group Information entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Hospital Group Information entity and you do not have permission");
					}
