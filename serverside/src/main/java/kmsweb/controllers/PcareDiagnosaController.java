...
import kmsweb.entities.PcareDiagnosaEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding PCare Diagnosa
 */
// % protected region % [Update the API route or description for the PCare DiagnosaController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareDiagnosaController"))
@RestController
@Slf4j
@RequestMapping("/api/pcarediagnosa")
// % protected region % [Update the API route or description for the PCare DiagnosaController here] end
public class PcareDiagnosaController {

	private final PcareDiagnosaService pcareDiagnosaService;
	private final CsvUtils<PcareDiagnosaEntity, PcareDiagnosaRepository, PcareDiagnosaEntityAudit,  PcareDiagnosaService> csvUtils;
	...
	@Autowired
	public PcareDiagnosaController(
		...
		this.csvUtils = new CsvUtils<>(pcareDiagnosaService, PcareDiagnosaEntity.class);
	...
	public ResponseEntity<List<PcareDiagnosaEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareDiagnosaEntity> pcareDiagnosas = pcareDiagnosaService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareDiagnosas, HttpStatus.OK);
	}

	/**
	 * Return the PCare Diagnosa that has the same id as the given id.
	 *
	 * @param id      The id of the PcareDiagnosaEntity to be returned
	 * @return the PCare Diagnosa that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareDiagnosa as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareDiagnosa entity not found"),
	})
	@PreAuthorize("hasPermission('PcareDiagnosaEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareDiagnosaEntity> getById(
			@Parameter(description = "The UUID of the PcareDiagnosa to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareDiagnosaEntity> pcareDiagnosaEntity = pcareDiagnosaService.findById(id);
	...
	/**
	 * Create/Update the given PCare Diagnosa. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareDiagnosaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareDiagnosa' endpoint here] off begin
	@Operation(
			description = "Create a PcareDiagnosa if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareDiagnosa."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareDiagnosa, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareDiagnosaEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareDiagnosa' endpoint here] end
	public ResponseEntity<?> savePcareDiagnosa(
			@Parameter(description = "The PcareDiagnosaEntity to create or update.")
			@RequestBody PcareDiagnosaEntityDto pcareDiagnosaEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareDiagnosaEntity pcareDiagnosaEntity = pcareDiagnosaService.create(new PcareDiagnosaEntity(pcareDiagnosaEntityDto));
			return new ResponseEntity<>(pcareDiagnosaEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Diagnosa. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareDiagnosaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareDiagnosa' endpoint here] off begin
	@Operation(
			description = "Update a PcareDiagnosa entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareDiagnosa."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareDiagnosa, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareDiagnosaEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareDiagnosa' endpoint here] end
	public ResponseEntity<?> updatePcareDiagnosa(
			@Parameter(description = "The PcareDiagnosaEntity to create or update.")
			@RequestBody PcareDiagnosaEntityDto pcareDiagnosaEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareDiagnosaEntity> checkPcareDiagnosaExists = pcareDiagnosaService.findById(pcareDiagnosaEntityDto.getId());
		PcareDiagnosaEntity pcareDiagnosaEntity = new PcareDiagnosaEntity(pcareDiagnosaEntityDto);

		if (checkPcareDiagnosaExists.isPresent()) {
			try {
				pcareDiagnosaEntity = pcareDiagnosaService.update(pcareDiagnosaEntity);
				return new ResponseEntity<>(pcareDiagnosaEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareDiagnosaEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Diagnosa that has the same id as the given id.
	 *
	 * @param id The id of the PcareDiagnosaEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareDiagnosa as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareDiagnosa to delete.")
		...
		pcareDiagnosaService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareDiagnosa' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Diagnosa entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareDiagnosa' endpoint here] end
	public void exportPcareDiagnosa(
			@Parameter(description = "The ids of the PcareDiagnosaEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareDiagnosaEntity> entitiesToExport = pcareDiagnosaService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareDiagnosa_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareDiagnosaEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareDiagnosaExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareDiagnosaExcludingIds' endpoint here] end
	public void exportPcareDiagnosaExcludingIds(
			@Parameter(description = "The ids of the PcareDiagnosaEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareDiagnosaEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareDiagnosaService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareDiagnosaService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareDiagnosa entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareDiagnosa_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareDiagnosaEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Diagnosa entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareDiagnosaEntity', 'create')  || hasPermission('PcareDiagnosaEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareDiagnosaEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareDiagnosaEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareDiagnosaEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareDiagnosaEntity> csvToBean = new CsvToBeanBuilder<PcareDiagnosaEntity>(reader)
				.withType(PcareDiagnosaEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareDiagnosaEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareDiagnosaEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Diagnosa entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Diagnosa entity and you do not have permission");
					}
