...
import kmsweb.dtos.MedicalRecordUserEntityDto;
import kmsweb.entities.MedicalRecordUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medical Record user
 */
// % protected region % [Update the API route or description for the Medical Record userController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicalRecordUserController"))
@RestController
@Slf4j
@RequestMapping("/api/medicalrecorduser")
// % protected region % [Update the API route or description for the Medical Record userController here] end
public class MedicalRecordUserController {

	private final MedicalRecordUserService medicalRecordUserService;
	private final CsvUtils<MedicalRecordUserEntity, MedicalRecordUserRepository, MedicalRecordUserEntityAudit,  MedicalRecordUserService> csvUtils;
	...
	@Autowired
	public MedicalRecordUserController(
		...
		this.csvUtils = new CsvUtils<>(medicalRecordUserService, MedicalRecordUserEntity.class);
	...
	public ResponseEntity<List<MedicalRecordUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicalRecordUserEntity> medicalRecordUsers = medicalRecordUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicalRecordUsers, HttpStatus.OK);
	}

	/**
	 * Return the Medical Record user that has the same id as the given id.
	 *
	 * @param id      The id of the MedicalRecordUserEntity to be returned
	 * @return the Medical Record user that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicalRecordUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicalRecordUser entity not found"),
	})
	@PreAuthorize("hasPermission('MedicalRecordUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicalRecordUserEntity> getById(
			@Parameter(description = "The UUID of the MedicalRecordUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicalRecordUserEntity> medicalRecordUserEntity = medicalRecordUserService.findById(id);
	...
	/**
	 * Create/Update the given Medical Record user. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicalRecordUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicalRecordUser' endpoint here] off begin
	@Operation(
			description = "Create a MedicalRecordUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicalRecordUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicalRecordUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicalRecordUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicalRecordUser' endpoint here] end
	public ResponseEntity<?> saveMedicalRecordUser(
			@Parameter(description = "The MedicalRecordUserEntity to create or update.")
			@RequestBody MedicalRecordUserEntityDto medicalRecordUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicalRecordUserEntity medicalRecordUserEntity = medicalRecordUserService.create(new MedicalRecordUserEntity(medicalRecordUserEntityDto));
			return new ResponseEntity<>(medicalRecordUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medical Record user. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicalRecordUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicalRecordUser' endpoint here] off begin
	@Operation(
			description = "Update a MedicalRecordUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicalRecordUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicalRecordUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicalRecordUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicalRecordUser' endpoint here] end
	public ResponseEntity<?> updateMedicalRecordUser(
			@Parameter(description = "The MedicalRecordUserEntity to create or update.")
			@RequestBody MedicalRecordUserEntityDto medicalRecordUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicalRecordUserEntity> checkMedicalRecordUserExists = medicalRecordUserService.findById(medicalRecordUserEntityDto.getId());
		MedicalRecordUserEntity medicalRecordUserEntity = new MedicalRecordUserEntity(medicalRecordUserEntityDto);

		if (checkMedicalRecordUserExists.isPresent()) {
			try {
				medicalRecordUserEntity = medicalRecordUserService.update(medicalRecordUserEntity);
				return new ResponseEntity<>(medicalRecordUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicalRecordUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medical Record user that has the same id as the given id.
	 *
	 * @param id The id of the MedicalRecordUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicalRecordUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicalRecordUser to delete.")
		...
		medicalRecordUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medical Record user entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordUser' endpoint here] end
	public void exportMedicalRecordUser(
			@Parameter(description = "The ids of the MedicalRecordUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicalRecordUserEntity> entitiesToExport = medicalRecordUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicalRecordUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicalRecordUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordUserExcludingIds' endpoint here] end
	public void exportMedicalRecordUserExcludingIds(
			@Parameter(description = "The ids of the MedicalRecordUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicalRecordUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicalRecordUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicalRecordUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicalRecordUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicalRecordUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicalRecordUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medical Record user entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicalRecordUserEntity', 'create')  || hasPermission('MedicalRecordUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicalRecordUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicalRecordUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicalRecordUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicalRecordUserEntity> csvToBean = new CsvToBeanBuilder<MedicalRecordUserEntity>(reader)
				.withType(MedicalRecordUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicalRecordUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicalRecordUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medical Record user entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medical Record user entity and you do not have permission");
					}
