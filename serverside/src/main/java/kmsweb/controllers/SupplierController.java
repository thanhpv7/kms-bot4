...
import kmsweb.entities.SupplierEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Supplier
 */
// % protected region % [Update the API route or description for the SupplierController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SupplierController"))
@RestController
@Slf4j
@RequestMapping("/api/supplier")
// % protected region % [Update the API route or description for the SupplierController here] end
public class SupplierController {

	private final SupplierService supplierService;
	private final CsvUtils<SupplierEntity, SupplierRepository, SupplierEntityAudit,  SupplierService> csvUtils;
	...
	public SupplierController(
		...
		this.csvUtils = new CsvUtils<>(supplierService, SupplierEntity.class);
	...
	@Operation(
			description = "Returns a single page of Suppliers",
		security = {
	...
	public ResponseEntity<List<SupplierEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SupplierEntity> suppliers = supplierService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Supplier that has the same id as the given id.
	 *
	 * @param id      The id of the SupplierEntity to be returned
	 * @return the Supplier that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Supplier as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Supplier entity not found"),
	})
	@PreAuthorize("hasPermission('SupplierEntity', 'read')")
	...
	public ResponseEntity<SupplierEntity> getById(
			@Parameter(description = "The UUID of the Supplier to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SupplierEntity> supplierEntity = supplierService.findById(id);
	...
	/**
	 * Create/Update the given Supplier. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param supplierEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSupplier' endpoint here] off begin
	@Operation(
			description = "Create a Supplier if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Supplier."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Supplier, duplicate record found")
	})
	@PreAuthorize("hasPermission('SupplierEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSupplier' endpoint here] end
	public ResponseEntity<?> saveSupplier(
			@Parameter(description = "The SupplierEntity to create or update.")
			@RequestBody SupplierEntityDto supplierEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SupplierEntity supplierEntity = supplierService.create(new SupplierEntity(supplierEntityDto));
	...
	/**
	 * Update the given Supplier. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param supplierEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSupplier' endpoint here] off begin
	@Operation(
			description = "Update a Supplier entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		...
		@ApiResponse(responseCode = "400", description = "Failed to update the Supplier, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SupplierEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSupplier' endpoint here] end
	public ResponseEntity<?> updateSupplier(
		...
		Optional<SupplierEntity> checkSupplierExists = supplierService.findById(supplierEntityDto.getId());
		SupplierEntity supplierEntity = new SupplierEntity(supplierEntityDto);

		if (checkSupplierExists.isPresent()) {
			try {
				supplierEntity = supplierService.update(supplierEntity);
				return new ResponseEntity<>(supplierEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(supplierEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Supplier that has the same id as the given id.
	 *
	 * @param id The id of the SupplierEntity to be deleted
	...
	@Operation(
			description = "Delete a single Supplier as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Supplier to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplier' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Supplier entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplier' endpoint here] end
	public void exportSupplier(
			@Parameter(description = "The ids of the SupplierEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SupplierEntity> entitiesToExport = supplierService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Supplier_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SupplierEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplierExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplierExcludingIds' endpoint here] end
	public void exportSupplierExcludingIds(
			@Parameter(description = "The ids of the SupplierEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SupplierEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = supplierService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = supplierService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Supplier entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Supplier_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SupplierEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Supplier entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SupplierEntity', 'create')  || hasPermission('SupplierEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SupplierEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SupplierEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SupplierEntity.getExampleCsvHeader().split(","));
		...
		try {
			Iterator<SupplierEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Supplier entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Supplier entity and you do not have permission");
					}
