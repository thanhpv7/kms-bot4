...
import kmsweb.dtos.SystemAdminUserEntityDto;
import kmsweb.entities.SystemAdminUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding System Admin User
 */
// % protected region % [Update the API route or description for the System Admin UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SystemAdminUserController"))
@RestController
@Slf4j
@RequestMapping("/api/systemadminuser")
// % protected region % [Update the API route or description for the System Admin UserController here] end
public class SystemAdminUserController {

	private final SystemAdminUserService systemAdminUserService;
	private final CsvUtils<SystemAdminUserEntity, SystemAdminUserRepository, SystemAdminUserEntityAudit,  SystemAdminUserService> csvUtils;
	...
	public SystemAdminUserController(
		...
		this.csvUtils = new CsvUtils<>(systemAdminUserService, SystemAdminUserEntity.class);
	...
	public ResponseEntity<List<SystemAdminUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SystemAdminUserEntity> systemAdminUsers = systemAdminUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(systemAdminUsers, HttpStatus.OK);
	}

	/**
	 * Return the System Admin User that has the same id as the given id.
	 *
	 * @param id      The id of the SystemAdminUserEntity to be returned
	 * @return the System Admin User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single SystemAdminUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "SystemAdminUser entity not found"),
	})
	@PreAuthorize("hasPermission('SystemAdminUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SystemAdminUserEntity> getById(
			@Parameter(description = "The UUID of the SystemAdminUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SystemAdminUserEntity> systemAdminUserEntity = systemAdminUserService.findById(id);
	...
	/**
	 * Create/Update the given System Admin User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param systemAdminUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSystemAdminUser' endpoint here] off begin
	@Operation(
			description = "Create a SystemAdminUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new SystemAdminUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new SystemAdminUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('SystemAdminUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSystemAdminUser' endpoint here] end
	public ResponseEntity<?> saveSystemAdminUser(
			@Parameter(description = "The SystemAdminUserEntity to create or update.")
			@RequestBody SystemAdminUserEntityDto systemAdminUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SystemAdminUserEntity systemAdminUserEntity = systemAdminUserService.create(new SystemAdminUserEntity(systemAdminUserEntityDto));
			return new ResponseEntity<>(systemAdminUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given System Admin User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param systemAdminUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSystemAdminUser' endpoint here] off begin
	@Operation(
			description = "Update a SystemAdminUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the SystemAdminUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the SystemAdminUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SystemAdminUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSystemAdminUser' endpoint here] end
	public ResponseEntity<?> updateSystemAdminUser(
			@Parameter(description = "The SystemAdminUserEntity to create or update.")
			@RequestBody SystemAdminUserEntityDto systemAdminUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SystemAdminUserEntity> checkSystemAdminUserExists = systemAdminUserService.findById(systemAdminUserEntityDto.getId());
		SystemAdminUserEntity systemAdminUserEntity = new SystemAdminUserEntity(systemAdminUserEntityDto);

		if (checkSystemAdminUserExists.isPresent()) {
			try {
				systemAdminUserEntity = systemAdminUserService.update(systemAdminUserEntity);
				return new ResponseEntity<>(systemAdminUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(systemAdminUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the System Admin User that has the same id as the given id.
	 *
	 * @param id The id of the SystemAdminUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single SystemAdminUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('SystemAdminUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the SystemAdminUser to delete.")
		...
		systemAdminUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSystemAdminUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all System Admin User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSystemAdminUser' endpoint here] end
	public void exportSystemAdminUser(
			@Parameter(description = "The ids of the SystemAdminUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SystemAdminUserEntity> entitiesToExport = systemAdminUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "SystemAdminUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SystemAdminUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSystemAdminUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSystemAdminUserExcludingIds' endpoint here] end
	public void exportSystemAdminUserExcludingIds(
			@Parameter(description = "The ids of the SystemAdminUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SystemAdminUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = systemAdminUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = systemAdminUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the SystemAdminUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "SystemAdminUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SystemAdminUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for System Admin User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SystemAdminUserEntity', 'create')  || hasPermission('SystemAdminUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SystemAdminUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SystemAdminUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SystemAdminUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SystemAdminUserEntity> csvToBean = new CsvToBeanBuilder<SystemAdminUserEntity>(reader)
				.withType(SystemAdminUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<SystemAdminUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<SystemAdminUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a System Admin User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a System Admin User entity and you do not have permission");
					}
