...
/**
 * Controller used to handle all REST operations regarding ICD 10
 */
// % protected region % [Update the API route or description for the ICD 10Controller here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "Icd10Controller"))
@RestController
@Slf4j
@RequestMapping("/api/icd10")
// % protected region % [Update the API route or description for the ICD 10Controller here] end
public class Icd10Controller {

	private final Icd10Service icd10Service;
	private final CsvUtils<Icd10Entity, Icd10Repository, Icd10EntityAudit,  Icd10Service> csvUtils;
	...
	public Icd10Controller(
		...
		this.csvUtils = new CsvUtils<>(icd10Service, Icd10Entity.class);
	...
	public ResponseEntity<List<Icd10Entity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<Icd10Entity> icd10s = icd10Service.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the ICD 10 that has the same id as the given id.
	 *
	 * @param id      The id of the Icd10Entity to be returned
	 * @return the ICD 10 that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Icd10 as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Icd10 entity not found"),
	})
	@PreAuthorize("hasPermission('Icd10Entity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<Icd10Entity> getById(
			@Parameter(description = "The UUID of the Icd10 to return.")
			@PathVariable("id") UUID id
	) {
		Optional<Icd10Entity> icd10Entity = icd10Service.findById(id);
	...
	/**
	 * Create/Update the given ICD 10. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param icd10EntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveIcd10' endpoint here] off begin
	@Operation(
			description = "Create a Icd10 if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Icd10."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Icd10, duplicate record found")
	})
	@PreAuthorize("hasPermission('Icd10Entity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveIcd10' endpoint here] end
	public ResponseEntity<?> saveIcd10(
			@Parameter(description = "The Icd10Entity to create or update.")
			@RequestBody Icd10EntityDto icd10EntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			Icd10Entity icd10Entity = icd10Service.create(new Icd10Entity(icd10EntityDto));
			return new ResponseEntity<>(icd10Entity, HttpStatus.OK);
	...
	/**
	 * Update the given ICD 10. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param icd10EntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateIcd10' endpoint here] off begin
	@Operation(
			description = "Update a Icd10 entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Icd10."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Icd10, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('Icd10Entity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateIcd10' endpoint here] end
	public ResponseEntity<?> updateIcd10(
			@Parameter(description = "The Icd10Entity to create or update.")
			@RequestBody Icd10EntityDto icd10EntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<Icd10Entity> checkIcd10Exists = icd10Service.findById(icd10EntityDto.getId());
		Icd10Entity icd10Entity = new Icd10Entity(icd10EntityDto);

		if (checkIcd10Exists.isPresent()) {
			try {
				icd10Entity = icd10Service.update(icd10Entity);
				return new ResponseEntity<>(icd10Entity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(icd10Entity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the ICD 10 that has the same id as the given id.
	 *
	 * @param id The id of the Icd10Entity to be deleted
	...
	@Operation(
			description = "Delete a single Icd10 as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Icd10 to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportIcd10' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all ICD 10 entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportIcd10' endpoint here] end
	public void exportIcd10(
			@Parameter(description = "The ids of the Icd10Entities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<Icd10Entity> entitiesToExport = icd10Service.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Icd10_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (Icd10Entity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportIcd10ExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportIcd10ExcludingIds' endpoint here] end
	public void exportIcd10ExcludingIds(
			@Parameter(description = "The ids of the Icd10Entities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<Icd10Entity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = icd10Service.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = icd10Service.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Icd10 entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Icd10_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(Icd10Entity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for ICD 10 entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('Icd10Entity', 'create')  || hasPermission('Icd10Entity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with Icd10Entity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("Icd10Entity"))
		...
		List<String> allowedHeaders = Arrays.asList(Icd10Entity.getExampleCsvHeader().split(","));
		...
		CsvToBean<Icd10Entity> csvToBean = new CsvToBeanBuilder<Icd10Entity>(reader)
				.withType(Icd10Entity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<Icd10Entity> entities = new ArrayList<>();
		...
		try {
			Iterator<Icd10Entity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a ICD 10 entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a ICD 10 entity and you do not have permission");
					}
