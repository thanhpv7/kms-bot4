...
/**
 * Controller used to handle all REST operations regarding Obstetric And Gynecology History
 */
// % protected region % [Update the API route or description for the Obstetric And Gynecology HistoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ObstetricAndGynecologyHistoryController"))
@RestController
@Slf4j
@RequestMapping("/api/obstetricandgynecologyhistory")
// % protected region % [Update the API route or description for the Obstetric And Gynecology HistoryController here] end
public class ObstetricAndGynecologyHistoryController {

	private final ObstetricAndGynecologyHistoryService obstetricAndGynecologyHistoryService;
	private final CsvUtils<ObstetricAndGynecologyHistoryEntity, ObstetricAndGynecologyHistoryRepository, ObstetricAndGynecologyHistoryEntityAudit,  ObstetricAndGynecologyHistoryService> csvUtils;
	...
	@Autowired
	public ObstetricAndGynecologyHistoryController(
		...
		this.csvUtils = new CsvUtils<>(obstetricAndGynecologyHistoryService, ObstetricAndGynecologyHistoryEntity.class);
	...
	/**
	 * Return all the Obstetric And Gynecology Historys.
	 *
	 * @return all the Obstetric And Gynecology Historys
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of ObstetricAndGynecologyHistorys",
		security = {
	...
	public ResponseEntity<List<ObstetricAndGynecologyHistoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ObstetricAndGynecologyHistoryEntity> obstetricAndGynecologyHistorys = obstetricAndGynecologyHistoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(obstetricAndGynecologyHistorys, HttpStatus.OK);
	}

	/**
	 * Return the Obstetric And Gynecology History that has the same id as the given id.
	 *
	 * @param id      The id of the ObstetricAndGynecologyHistoryEntity to be returned
	 * @return the Obstetric And Gynecology History that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ObstetricAndGynecologyHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ObstetricAndGynecologyHistory entity not found"),
	})
	@PreAuthorize("hasPermission('ObstetricAndGynecologyHistoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ObstetricAndGynecologyHistoryEntity> getById(
			@Parameter(description = "The UUID of the ObstetricAndGynecologyHistory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ObstetricAndGynecologyHistoryEntity> obstetricAndGynecologyHistoryEntity = obstetricAndGynecologyHistoryService.findById(id);
	...
	/**
	 * Create/Update the given Obstetric And Gynecology History. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param obstetricAndGynecologyHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveObstetricAndGynecologyHistory' endpoint here] off begin
	@Operation(
			description = "Create a ObstetricAndGynecologyHistory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ObstetricAndGynecologyHistory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ObstetricAndGynecologyHistory, duplicate record found")
	})
	@PreAuthorize("hasPermission('ObstetricAndGynecologyHistoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveObstetricAndGynecologyHistory' endpoint here] end
	public ResponseEntity<?> saveObstetricAndGynecologyHistory(
			@Parameter(description = "The ObstetricAndGynecologyHistoryEntity to create or update.")
			@RequestBody ObstetricAndGynecologyHistoryEntityDto obstetricAndGynecologyHistoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ObstetricAndGynecologyHistoryEntity obstetricAndGynecologyHistoryEntity = obstetricAndGynecologyHistoryService.create(new ObstetricAndGynecologyHistoryEntity(obstetricAndGynecologyHistoryEntityDto));
			return new ResponseEntity<>(obstetricAndGynecologyHistoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Obstetric And Gynecology History. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param obstetricAndGynecologyHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateObstetricAndGynecologyHistory' endpoint here] off begin
	@Operation(
			description = "Update a ObstetricAndGynecologyHistory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ObstetricAndGynecologyHistory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ObstetricAndGynecologyHistory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ObstetricAndGynecologyHistoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateObstetricAndGynecologyHistory' endpoint here] end
	public ResponseEntity<?> updateObstetricAndGynecologyHistory(
			@Parameter(description = "The ObstetricAndGynecologyHistoryEntity to create or update.")
			@RequestBody ObstetricAndGynecologyHistoryEntityDto obstetricAndGynecologyHistoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ObstetricAndGynecologyHistoryEntity> checkObstetricAndGynecologyHistoryExists = obstetricAndGynecologyHistoryService.findById(obstetricAndGynecologyHistoryEntityDto.getId());
		ObstetricAndGynecologyHistoryEntity obstetricAndGynecologyHistoryEntity = new ObstetricAndGynecologyHistoryEntity(obstetricAndGynecologyHistoryEntityDto);

		if (checkObstetricAndGynecologyHistoryExists.isPresent()) {
			try {
				obstetricAndGynecologyHistoryEntity = obstetricAndGynecologyHistoryService.update(obstetricAndGynecologyHistoryEntity);
				return new ResponseEntity<>(obstetricAndGynecologyHistoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(obstetricAndGynecologyHistoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Obstetric And Gynecology History that has the same id as the given id.
	 *
	 * @param id The id of the ObstetricAndGynecologyHistoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single ObstetricAndGynecologyHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ObstetricAndGynecologyHistory to delete.")
		...
		obstetricAndGynecologyHistoryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportObstetricAndGynecologyHistory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Obstetric And Gynecology History entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportObstetricAndGynecologyHistory' endpoint here] end
	public void exportObstetricAndGynecologyHistory(
			@Parameter(description = "The ids of the ObstetricAndGynecologyHistoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ObstetricAndGynecologyHistoryEntity> entitiesToExport = obstetricAndGynecologyHistoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ObstetricAndGynecologyHistory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ObstetricAndGynecologyHistoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportObstetricAndGynecologyHistoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportObstetricAndGynecologyHistoryExcludingIds' endpoint here] end
	public void exportObstetricAndGynecologyHistoryExcludingIds(
			@Parameter(description = "The ids of the ObstetricAndGynecologyHistoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ObstetricAndGynecologyHistoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = obstetricAndGynecologyHistoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = obstetricAndGynecologyHistoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ObstetricAndGynecologyHistory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ObstetricAndGynecologyHistory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ObstetricAndGynecologyHistoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Obstetric And Gynecology History entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ObstetricAndGynecologyHistoryEntity', 'create')  || hasPermission('ObstetricAndGynecologyHistoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ObstetricAndGynecologyHistoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ObstetricAndGynecologyHistoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ObstetricAndGynecologyHistoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ObstetricAndGynecologyHistoryEntity> csvToBean = new CsvToBeanBuilder<ObstetricAndGynecologyHistoryEntity>(reader)
				.withType(ObstetricAndGynecologyHistoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ObstetricAndGynecologyHistoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ObstetricAndGynecologyHistoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Obstetric And Gynecology History entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Obstetric And Gynecology History entity and you do not have permission");
					}
