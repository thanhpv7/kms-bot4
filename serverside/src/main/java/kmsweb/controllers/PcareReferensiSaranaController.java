...
/**
 * Controller used to handle all REST operations regarding PCare Referensi Sarana
 */
// % protected region % [Update the API route or description for the PCare Referensi SaranaController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareReferensiSaranaController"))
@RestController
@Slf4j
@RequestMapping("/api/pcarereferensisarana")
// % protected region % [Update the API route or description for the PCare Referensi SaranaController here] end
public class PcareReferensiSaranaController {

	private final PcareReferensiSaranaService pcareReferensiSaranaService;
	private final CsvUtils<PcareReferensiSaranaEntity, PcareReferensiSaranaRepository, PcareReferensiSaranaEntityAudit,  PcareReferensiSaranaService> csvUtils;
	...
	@Autowired
	public PcareReferensiSaranaController(
		...
		this.csvUtils = new CsvUtils<>(pcareReferensiSaranaService, PcareReferensiSaranaEntity.class);
	...
	public ResponseEntity<List<PcareReferensiSaranaEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareReferensiSaranaEntity> pcareReferensiSaranas = pcareReferensiSaranaService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareReferensiSaranas, HttpStatus.OK);
	}

	/**
	 * Return the PCare Referensi Sarana that has the same id as the given id.
	 *
	 * @param id      The id of the PcareReferensiSaranaEntity to be returned
	 * @return the PCare Referensi Sarana that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareReferensiSarana as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareReferensiSarana entity not found"),
	})
	@PreAuthorize("hasPermission('PcareReferensiSaranaEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareReferensiSaranaEntity> getById(
			@Parameter(description = "The UUID of the PcareReferensiSarana to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareReferensiSaranaEntity> pcareReferensiSaranaEntity = pcareReferensiSaranaService.findById(id);
	...
	/**
	 * Create/Update the given PCare Referensi Sarana. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareReferensiSaranaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareReferensiSarana' endpoint here] off begin
	@Operation(
			description = "Create a PcareReferensiSarana if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareReferensiSarana."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareReferensiSarana, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareReferensiSaranaEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareReferensiSarana' endpoint here] end
	public ResponseEntity<?> savePcareReferensiSarana(
			@Parameter(description = "The PcareReferensiSaranaEntity to create or update.")
			@RequestBody PcareReferensiSaranaEntityDto pcareReferensiSaranaEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareReferensiSaranaEntity pcareReferensiSaranaEntity = pcareReferensiSaranaService.create(new PcareReferensiSaranaEntity(pcareReferensiSaranaEntityDto));
			return new ResponseEntity<>(pcareReferensiSaranaEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Referensi Sarana. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareReferensiSaranaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareReferensiSarana' endpoint here] off begin
	@Operation(
			description = "Update a PcareReferensiSarana entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareReferensiSarana."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareReferensiSarana, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareReferensiSaranaEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareReferensiSarana' endpoint here] end
	public ResponseEntity<?> updatePcareReferensiSarana(
			@Parameter(description = "The PcareReferensiSaranaEntity to create or update.")
			@RequestBody PcareReferensiSaranaEntityDto pcareReferensiSaranaEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareReferensiSaranaEntity> checkPcareReferensiSaranaExists = pcareReferensiSaranaService.findById(pcareReferensiSaranaEntityDto.getId());
		PcareReferensiSaranaEntity pcareReferensiSaranaEntity = new PcareReferensiSaranaEntity(pcareReferensiSaranaEntityDto);

		if (checkPcareReferensiSaranaExists.isPresent()) {
			try {
				pcareReferensiSaranaEntity = pcareReferensiSaranaService.update(pcareReferensiSaranaEntity);
				return new ResponseEntity<>(pcareReferensiSaranaEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareReferensiSaranaEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Referensi Sarana that has the same id as the given id.
	 *
	 * @param id The id of the PcareReferensiSaranaEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareReferensiSarana as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareReferensiSarana to delete.")
		...
		pcareReferensiSaranaService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareReferensiSarana' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Referensi Sarana entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareReferensiSarana' endpoint here] end
	public void exportPcareReferensiSarana(
			@Parameter(description = "The ids of the PcareReferensiSaranaEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareReferensiSaranaEntity> entitiesToExport = pcareReferensiSaranaService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareReferensiSarana_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareReferensiSaranaEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareReferensiSaranaExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareReferensiSaranaExcludingIds' endpoint here] end
	public void exportPcareReferensiSaranaExcludingIds(
			@Parameter(description = "The ids of the PcareReferensiSaranaEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareReferensiSaranaEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareReferensiSaranaService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareReferensiSaranaService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareReferensiSarana entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareReferensiSarana_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareReferensiSaranaEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Referensi Sarana entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareReferensiSaranaEntity', 'create')  || hasPermission('PcareReferensiSaranaEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareReferensiSaranaEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareReferensiSaranaEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareReferensiSaranaEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareReferensiSaranaEntity> csvToBean = new CsvToBeanBuilder<PcareReferensiSaranaEntity>(reader)
				.withType(PcareReferensiSaranaEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareReferensiSaranaEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareReferensiSaranaEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Referensi Sarana entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Referensi Sarana entity and you do not have permission");
					}
