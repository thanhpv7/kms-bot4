...
import kmsweb.entities.PcareExaminationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding PCare Examination
 */
// % protected region % [Update the API route or description for the PCare ExaminationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareExaminationController"))
@RestController
@Slf4j
@RequestMapping("/api/pcareexamination")
// % protected region % [Update the API route or description for the PCare ExaminationController here] end
public class PcareExaminationController {

	private final PcareExaminationService pcareExaminationService;
	private final CsvUtils<PcareExaminationEntity, PcareExaminationRepository, PcareExaminationEntityAudit,  PcareExaminationService> csvUtils;
	...
	@Autowired
	public PcareExaminationController(
		...
		this.csvUtils = new CsvUtils<>(pcareExaminationService, PcareExaminationEntity.class);
	...
	public ResponseEntity<List<PcareExaminationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareExaminationEntity> pcareExaminations = pcareExaminationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareExaminations, HttpStatus.OK);
	}

	/**
	 * Return the PCare Examination that has the same id as the given id.
	 *
	 * @param id      The id of the PcareExaminationEntity to be returned
	 * @return the PCare Examination that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareExamination entity not found"),
	})
	@PreAuthorize("hasPermission('PcareExaminationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareExaminationEntity> getById(
			@Parameter(description = "The UUID of the PcareExamination to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareExaminationEntity> pcareExaminationEntity = pcareExaminationService.findById(id);
	...
	/**
	 * Create/Update the given PCare Examination. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareExamination' endpoint here] off begin
	@Operation(
			description = "Create a PcareExamination if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareExamination."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareExamination, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareExaminationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareExamination' endpoint here] end
	public ResponseEntity<?> savePcareExamination(
			@Parameter(description = "The PcareExaminationEntity to create or update.")
			@RequestBody PcareExaminationEntityDto pcareExaminationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareExaminationEntity pcareExaminationEntity = pcareExaminationService.create(new PcareExaminationEntity(pcareExaminationEntityDto));
			return new ResponseEntity<>(pcareExaminationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Examination. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareExamination' endpoint here] off begin
	@Operation(
			description = "Update a PcareExamination entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareExamination."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareExamination, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareExaminationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareExamination' endpoint here] end
	public ResponseEntity<?> updatePcareExamination(
			@Parameter(description = "The PcareExaminationEntity to create or update.")
			@RequestBody PcareExaminationEntityDto pcareExaminationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareExaminationEntity> checkPcareExaminationExists = pcareExaminationService.findById(pcareExaminationEntityDto.getId());
		PcareExaminationEntity pcareExaminationEntity = new PcareExaminationEntity(pcareExaminationEntityDto);

		if (checkPcareExaminationExists.isPresent()) {
			try {
				pcareExaminationEntity = pcareExaminationService.update(pcareExaminationEntity);
				return new ResponseEntity<>(pcareExaminationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareExaminationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Examination that has the same id as the given id.
	 *
	 * @param id The id of the PcareExaminationEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareExamination to delete.")
		...
		pcareExaminationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareExamination' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Examination entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareExamination' endpoint here] end
	public void exportPcareExamination(
			@Parameter(description = "The ids of the PcareExaminationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareExaminationEntity> entitiesToExport = pcareExaminationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareExamination_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareExaminationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareExaminationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareExaminationExcludingIds' endpoint here] end
	public void exportPcareExaminationExcludingIds(
			@Parameter(description = "The ids of the PcareExaminationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareExaminationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareExaminationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareExaminationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareExamination entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareExamination_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareExaminationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Examination entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareExaminationEntity', 'create')  || hasPermission('PcareExaminationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareExaminationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareExaminationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareExaminationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareExaminationEntity> csvToBean = new CsvToBeanBuilder<PcareExaminationEntity>(reader)
				.withType(PcareExaminationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareExaminationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareExaminationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Examination entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Examination entity and you do not have permission");
					}
