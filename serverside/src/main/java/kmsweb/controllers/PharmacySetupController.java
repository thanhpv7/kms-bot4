...
import kmsweb.entities.PharmacySetupEntityAudit;
import kmsweb.repositories.PharmacySetupRepository;
import kmsweb.services.PharmacySetupService;
...
/**
 * Controller used to handle all REST operations regarding Pharmacy Setup
 */
// % protected region % [Update the API route or description for the Pharmacy SetupController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PharmacySetupController"))
@RestController
@Slf4j
@RequestMapping("/api/pharmacysetup")
// % protected region % [Update the API route or description for the Pharmacy SetupController here] end
public class PharmacySetupController {

	private final PharmacySetupService pharmacySetupService;
	private final CsvUtils<PharmacySetupEntity, PharmacySetupRepository, PharmacySetupEntityAudit,  PharmacySetupService> csvUtils;
	...
	@Autowired
	public PharmacySetupController(
		...
		this.pharmacySetupService = pharmacySetupService;
		this.csvUtils = new CsvUtils<>(pharmacySetupService, PharmacySetupEntity.class);
	...
	public ResponseEntity<List<PharmacySetupEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PharmacySetupEntity> pharmacySetups = pharmacySetupService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pharmacySetups, HttpStatus.OK);
	}

	/**
	 * Return the Pharmacy Setup that has the same id as the given id.
	 *
	 * @param id      The id of the PharmacySetupEntity to be returned
	 * @return the Pharmacy Setup that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PharmacySetup as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PharmacySetup entity not found"),
	})
	@PreAuthorize("hasPermission('PharmacySetupEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PharmacySetupEntity> getById(
			@Parameter(description = "The UUID of the PharmacySetup to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PharmacySetupEntity> pharmacySetupEntity = pharmacySetupService.findById(id);
	...
	/**
	 * Create/Update the given Pharmacy Setup. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pharmacySetupEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePharmacySetup' endpoint here] off begin
	@Operation(
			description = "Create a PharmacySetup if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PharmacySetup."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PharmacySetup, duplicate record found")
	})
	@PreAuthorize("hasPermission('PharmacySetupEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePharmacySetup' endpoint here] end
	public ResponseEntity<?> savePharmacySetup(
			@Parameter(description = "The PharmacySetupEntity to create or update.")
			@RequestBody PharmacySetupEntityDto pharmacySetupEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PharmacySetupEntity pharmacySetupEntity = pharmacySetupService.create(new PharmacySetupEntity(pharmacySetupEntityDto));
			return new ResponseEntity<>(pharmacySetupEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Pharmacy Setup. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pharmacySetupEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePharmacySetup' endpoint here] off begin
	@Operation(
			description = "Update a PharmacySetup entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PharmacySetup."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PharmacySetup, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PharmacySetupEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePharmacySetup' endpoint here] end
	public ResponseEntity<?> updatePharmacySetup(
			@Parameter(description = "The PharmacySetupEntity to create or update.")
			@RequestBody PharmacySetupEntityDto pharmacySetupEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PharmacySetupEntity> checkPharmacySetupExists = pharmacySetupService.findById(pharmacySetupEntityDto.getId());
		PharmacySetupEntity pharmacySetupEntity = new PharmacySetupEntity(pharmacySetupEntityDto);

		if (checkPharmacySetupExists.isPresent()) {
			try {
				pharmacySetupEntity = pharmacySetupService.update(pharmacySetupEntity);
				return new ResponseEntity<>(pharmacySetupEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pharmacySetupEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Pharmacy Setup that has the same id as the given id.
	 *
	 * @param id The id of the PharmacySetupEntity to be deleted
	...
	@Operation(
			description = "Delete a single PharmacySetup as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PharmacySetup to delete.")
		...
		pharmacySetupService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPharmacySetup' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Pharmacy Setup entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPharmacySetup' endpoint here] end
	public void exportPharmacySetup(
			@Parameter(description = "The ids of the PharmacySetupEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PharmacySetupEntity> entitiesToExport = pharmacySetupService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PharmacySetup_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PharmacySetupEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPharmacySetupExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPharmacySetupExcludingIds' endpoint here] end
	public void exportPharmacySetupExcludingIds(
			@Parameter(description = "The ids of the PharmacySetupEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PharmacySetupEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pharmacySetupService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pharmacySetupService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PharmacySetup entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PharmacySetup_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PharmacySetupEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Pharmacy Setup entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PharmacySetupEntity', 'create')  || hasPermission('PharmacySetupEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PharmacySetupEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PharmacySetupEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PharmacySetupEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PharmacySetupEntity> csvToBean = new CsvToBeanBuilder<PharmacySetupEntity>(reader)
				.withType(PharmacySetupEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PharmacySetupEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PharmacySetupEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Pharmacy Setup entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Pharmacy Setup entity and you do not have permission");
					}
