...
import kmsweb.entities.StockTransactionDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Stock Transaction Detail
 */
// % protected region % [Update the API route or description for the Stock Transaction DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "StockTransactionDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/stocktransactiondetail")
// % protected region % [Update the API route or description for the Stock Transaction DetailController here] end
public class StockTransactionDetailController {

	private final StockTransactionDetailService stockTransactionDetailService;
	private final CsvUtils<StockTransactionDetailEntity, StockTransactionDetailRepository, StockTransactionDetailEntityAudit,  StockTransactionDetailService> csvUtils;
	...
	@Autowired
	public StockTransactionDetailController(
		...
		this.csvUtils = new CsvUtils<>(stockTransactionDetailService, StockTransactionDetailEntity.class);
	...
	public ResponseEntity<List<StockTransactionDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<StockTransactionDetailEntity> stockTransactionDetails = stockTransactionDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(stockTransactionDetails, HttpStatus.OK);
	}

	/**
	 * Return the Stock Transaction Detail that has the same id as the given id.
	 *
	 * @param id      The id of the StockTransactionDetailEntity to be returned
	 * @return the Stock Transaction Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single StockTransactionDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "StockTransactionDetail entity not found"),
	})
	@PreAuthorize("hasPermission('StockTransactionDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<StockTransactionDetailEntity> getById(
			@Parameter(description = "The UUID of the StockTransactionDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<StockTransactionDetailEntity> stockTransactionDetailEntity = stockTransactionDetailService.findById(id);
	...
	/**
	 * Create/Update the given Stock Transaction Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param stockTransactionDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveStockTransactionDetail' endpoint here] off begin
	@Operation(
			description = "Create a StockTransactionDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new StockTransactionDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new StockTransactionDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('StockTransactionDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveStockTransactionDetail' endpoint here] end
	public ResponseEntity<?> saveStockTransactionDetail(
			@Parameter(description = "The StockTransactionDetailEntity to create or update.")
			@RequestBody StockTransactionDetailEntityDto stockTransactionDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			StockTransactionDetailEntity stockTransactionDetailEntity = stockTransactionDetailService.create(new StockTransactionDetailEntity(stockTransactionDetailEntityDto));
			return new ResponseEntity<>(stockTransactionDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Stock Transaction Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param stockTransactionDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateStockTransactionDetail' endpoint here] off begin
	@Operation(
			description = "Update a StockTransactionDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the StockTransactionDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the StockTransactionDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('StockTransactionDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateStockTransactionDetail' endpoint here] end
	public ResponseEntity<?> updateStockTransactionDetail(
			@Parameter(description = "The StockTransactionDetailEntity to create or update.")
			@RequestBody StockTransactionDetailEntityDto stockTransactionDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<StockTransactionDetailEntity> checkStockTransactionDetailExists = stockTransactionDetailService.findById(stockTransactionDetailEntityDto.getId());
		StockTransactionDetailEntity stockTransactionDetailEntity = new StockTransactionDetailEntity(stockTransactionDetailEntityDto);

		if (checkStockTransactionDetailExists.isPresent()) {
			try {
				stockTransactionDetailEntity = stockTransactionDetailService.update(stockTransactionDetailEntity);
				return new ResponseEntity<>(stockTransactionDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(stockTransactionDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Stock Transaction Detail that has the same id as the given id.
	 *
	 * @param id The id of the StockTransactionDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single StockTransactionDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the StockTransactionDetail to delete.")
		...
		stockTransactionDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Stock Transaction Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionDetail' endpoint here] end
	public void exportStockTransactionDetail(
			@Parameter(description = "The ids of the StockTransactionDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<StockTransactionDetailEntity> entitiesToExport = stockTransactionDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "StockTransactionDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (StockTransactionDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionDetailExcludingIds' endpoint here] end
	public void exportStockTransactionDetailExcludingIds(
			@Parameter(description = "The ids of the StockTransactionDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<StockTransactionDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = stockTransactionDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = stockTransactionDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the StockTransactionDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "StockTransactionDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(StockTransactionDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Stock Transaction Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('StockTransactionDetailEntity', 'create')  || hasPermission('StockTransactionDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with StockTransactionDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("StockTransactionDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(StockTransactionDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<StockTransactionDetailEntity> csvToBean = new CsvToBeanBuilder<StockTransactionDetailEntity>(reader)
				.withType(StockTransactionDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<StockTransactionDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<StockTransactionDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Stock Transaction Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Stock Transaction Detail entity and you do not have permission");
					}
