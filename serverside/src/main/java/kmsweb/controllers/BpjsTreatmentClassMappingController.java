...
import kmsweb.entities.BpjsTreatmentClassMappingEntityAudit;
import kmsweb.repositories.BpjsTreatmentClassMappingRepository;
import kmsweb.services.BpjsTreatmentClassMappingService;
...
/**
 * Controller used to handle all REST operations regarding BPJS Treatment Class Mapping
 */
// % protected region % [Update the API route or description for the BPJS Treatment Class MappingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsTreatmentClassMappingController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjstreatmentclassmapping")
// % protected region % [Update the API route or description for the BPJS Treatment Class MappingController here] end
public class BpjsTreatmentClassMappingController {

	private final BpjsTreatmentClassMappingService bpjsTreatmentClassMappingService;
	private final CsvUtils<BpjsTreatmentClassMappingEntity, BpjsTreatmentClassMappingRepository, BpjsTreatmentClassMappingEntityAudit,  BpjsTreatmentClassMappingService> csvUtils;
	...
	@Autowired
	public BpjsTreatmentClassMappingController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsTreatmentClassMappingService bpjsTreatmentClassMappingService
	) {
		this.authService = authService;
		this.bpjsTreatmentClassMappingService = bpjsTreatmentClassMappingService;
		this.csvUtils = new CsvUtils<>(bpjsTreatmentClassMappingService, BpjsTreatmentClassMappingEntity.class);
	...
	/**
	 * Return all the BPJS Treatment Class Mappings.
	 *
	 * @return all the BPJS Treatment Class Mappings
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsTreatmentClassMappings",
		security = {
	...
	public ResponseEntity<List<BpjsTreatmentClassMappingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsTreatmentClassMappingEntity> bpjsTreatmentClassMappings = bpjsTreatmentClassMappingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsTreatmentClassMappings, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Treatment Class Mapping that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsTreatmentClassMappingEntity to be returned
	 * @return the BPJS Treatment Class Mapping that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsTreatmentClassMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsTreatmentClassMapping entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsTreatmentClassMappingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsTreatmentClassMappingEntity> getById(
			@Parameter(description = "The UUID of the BpjsTreatmentClassMapping to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsTreatmentClassMappingEntity> bpjsTreatmentClassMappingEntity = bpjsTreatmentClassMappingService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Treatment Class Mapping. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTreatmentClassMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsTreatmentClassMapping' endpoint here] off begin
	@Operation(
			description = "Create a BpjsTreatmentClassMapping if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsTreatmentClassMapping."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsTreatmentClassMapping, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsTreatmentClassMappingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsTreatmentClassMapping' endpoint here] end
	public ResponseEntity<?> saveBpjsTreatmentClassMapping(
			@Parameter(description = "The BpjsTreatmentClassMappingEntity to create or update.")
			@RequestBody BpjsTreatmentClassMappingEntityDto bpjsTreatmentClassMappingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsTreatmentClassMappingEntity bpjsTreatmentClassMappingEntity = bpjsTreatmentClassMappingService.create(new BpjsTreatmentClassMappingEntity(bpjsTreatmentClassMappingEntityDto));
			return new ResponseEntity<>(bpjsTreatmentClassMappingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Treatment Class Mapping. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTreatmentClassMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsTreatmentClassMapping' endpoint here] off begin
	@Operation(
			description = "Update a BpjsTreatmentClassMapping entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsTreatmentClassMapping."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsTreatmentClassMapping, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsTreatmentClassMappingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsTreatmentClassMapping' endpoint here] end
	public ResponseEntity<?> updateBpjsTreatmentClassMapping(
			@Parameter(description = "The BpjsTreatmentClassMappingEntity to create or update.")
			@RequestBody BpjsTreatmentClassMappingEntityDto bpjsTreatmentClassMappingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsTreatmentClassMappingEntity> checkBpjsTreatmentClassMappingExists = bpjsTreatmentClassMappingService.findById(bpjsTreatmentClassMappingEntityDto.getId());
		BpjsTreatmentClassMappingEntity bpjsTreatmentClassMappingEntity = new BpjsTreatmentClassMappingEntity(bpjsTreatmentClassMappingEntityDto);

		if (checkBpjsTreatmentClassMappingExists.isPresent()) {
			try {
				bpjsTreatmentClassMappingEntity = bpjsTreatmentClassMappingService.update(bpjsTreatmentClassMappingEntity);
				return new ResponseEntity<>(bpjsTreatmentClassMappingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsTreatmentClassMappingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Treatment Class Mapping that has the same id as the given id.
	 *
	 * @param id The id of the BpjsTreatmentClassMappingEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsTreatmentClassMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsTreatmentClassMapping to delete.")
		...
		bpjsTreatmentClassMappingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentClassMapping' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Treatment Class Mapping entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentClassMapping' endpoint here] end
	public void exportBpjsTreatmentClassMapping(
			@Parameter(description = "The ids of the BpjsTreatmentClassMappingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsTreatmentClassMappingEntity> entitiesToExport = bpjsTreatmentClassMappingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsTreatmentClassMapping_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsTreatmentClassMappingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentClassMappingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTreatmentClassMappingExcludingIds' endpoint here] end
	public void exportBpjsTreatmentClassMappingExcludingIds(
			@Parameter(description = "The ids of the BpjsTreatmentClassMappingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsTreatmentClassMappingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsTreatmentClassMappingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsTreatmentClassMappingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsTreatmentClassMapping entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsTreatmentClassMapping_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsTreatmentClassMappingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Treatment Class Mapping entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsTreatmentClassMappingEntity', 'create')  || hasPermission('BpjsTreatmentClassMappingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsTreatmentClassMappingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsTreatmentClassMappingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsTreatmentClassMappingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsTreatmentClassMappingEntity> csvToBean = new CsvToBeanBuilder<BpjsTreatmentClassMappingEntity>(reader)
				.withType(BpjsTreatmentClassMappingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsTreatmentClassMappingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsTreatmentClassMappingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Treatment Class Mapping entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Treatment Class Mapping entity and you do not have permission");
					}
