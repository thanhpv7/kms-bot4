...
import kmsweb.entities.PcareTindakanEntityAudit;
import kmsweb.repositories.PcareTindakanRepository;
import kmsweb.services.PcareTindakanService;
...
/**
 * Controller used to handle all REST operations regarding PCare Tindakan
 */
// % protected region % [Update the API route or description for the PCare TindakanController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareTindakanController"))
@RestController
@Slf4j
@RequestMapping("/api/pcaretindakan")
// % protected region % [Update the API route or description for the PCare TindakanController here] end
public class PcareTindakanController {

	private final PcareTindakanService pcareTindakanService;
	private final CsvUtils<PcareTindakanEntity, PcareTindakanRepository, PcareTindakanEntityAudit,  PcareTindakanService> csvUtils;
	...
	@Autowired
	public PcareTindakanController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			PcareTindakanService pcareTindakanService
	) {
		this.authService = authService;
		this.pcareTindakanService = pcareTindakanService;
		this.csvUtils = new CsvUtils<>(pcareTindakanService, PcareTindakanEntity.class);
	...
	public ResponseEntity<List<PcareTindakanEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareTindakanEntity> pcareTindakans = pcareTindakanService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareTindakans, HttpStatus.OK);
	}

	/**
	 * Return the PCare Tindakan that has the same id as the given id.
	 *
	 * @param id      The id of the PcareTindakanEntity to be returned
	 * @return the PCare Tindakan that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareTindakan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareTindakan entity not found"),
	})
	@PreAuthorize("hasPermission('PcareTindakanEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareTindakanEntity> getById(
			@Parameter(description = "The UUID of the PcareTindakan to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareTindakanEntity> pcareTindakanEntity = pcareTindakanService.findById(id);
	...
	/**
	 * Create/Update the given PCare Tindakan. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareTindakanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareTindakan' endpoint here] off begin
	@Operation(
			description = "Create a PcareTindakan if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareTindakan."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareTindakan, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareTindakanEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareTindakan' endpoint here] end
	public ResponseEntity<?> savePcareTindakan(
			@Parameter(description = "The PcareTindakanEntity to create or update.")
			@RequestBody PcareTindakanEntityDto pcareTindakanEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareTindakanEntity pcareTindakanEntity = pcareTindakanService.create(new PcareTindakanEntity(pcareTindakanEntityDto));
			return new ResponseEntity<>(pcareTindakanEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Tindakan. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareTindakanEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareTindakan' endpoint here] off begin
	@Operation(
			description = "Update a PcareTindakan entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareTindakan."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareTindakan, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareTindakanEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareTindakan' endpoint here] end
	public ResponseEntity<?> updatePcareTindakan(
			@Parameter(description = "The PcareTindakanEntity to create or update.")
			@RequestBody PcareTindakanEntityDto pcareTindakanEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareTindakanEntity> checkPcareTindakanExists = pcareTindakanService.findById(pcareTindakanEntityDto.getId());
		PcareTindakanEntity pcareTindakanEntity = new PcareTindakanEntity(pcareTindakanEntityDto);

		if (checkPcareTindakanExists.isPresent()) {
			try {
				pcareTindakanEntity = pcareTindakanService.update(pcareTindakanEntity);
				return new ResponseEntity<>(pcareTindakanEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareTindakanEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Tindakan that has the same id as the given id.
	 *
	 * @param id The id of the PcareTindakanEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareTindakan as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareTindakan to delete.")
		...
		pcareTindakanService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareTindakan' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Tindakan entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareTindakan' endpoint here] end
	public void exportPcareTindakan(
			@Parameter(description = "The ids of the PcareTindakanEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareTindakanEntity> entitiesToExport = pcareTindakanService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareTindakan_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareTindakanEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareTindakanExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareTindakanExcludingIds' endpoint here] end
	public void exportPcareTindakanExcludingIds(
			@Parameter(description = "The ids of the PcareTindakanEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareTindakanEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareTindakanService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareTindakanService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareTindakan entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareTindakan_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareTindakanEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Tindakan entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareTindakanEntity', 'create')  || hasPermission('PcareTindakanEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareTindakanEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareTindakanEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareTindakanEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareTindakanEntity> csvToBean = new CsvToBeanBuilder<PcareTindakanEntity>(reader)
				.withType(PcareTindakanEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareTindakanEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareTindakanEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Tindakan entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Tindakan entity and you do not have permission");
					}
