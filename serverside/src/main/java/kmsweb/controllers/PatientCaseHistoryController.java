...
import kmsweb.entities.PatientCaseHistoryEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Case History
 */
// % protected region % [Update the API route or description for the Patient Case HistoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientCaseHistoryController"))
@RestController
@Slf4j
@RequestMapping("/api/patientcasehistory")
// % protected region % [Update the API route or description for the Patient Case HistoryController here] end
public class PatientCaseHistoryController {

	private final PatientCaseHistoryService patientCaseHistoryService;
	private final CsvUtils<PatientCaseHistoryEntity, PatientCaseHistoryRepository, PatientCaseHistoryEntityAudit,  PatientCaseHistoryService> csvUtils;
	...
	@Autowired
	public PatientCaseHistoryController(
		...
		this.csvUtils = new CsvUtils<>(patientCaseHistoryService, PatientCaseHistoryEntity.class);
	...
	public ResponseEntity<List<PatientCaseHistoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientCaseHistoryEntity> patientCaseHistorys = patientCaseHistoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientCaseHistorys, HttpStatus.OK);
	}

	/**
	 * Return the Patient Case History that has the same id as the given id.
	 *
	 * @param id      The id of the PatientCaseHistoryEntity to be returned
	 * @return the Patient Case History that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientCaseHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientCaseHistory entity not found"),
	})
	@PreAuthorize("hasPermission('PatientCaseHistoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientCaseHistoryEntity> getById(
			@Parameter(description = "The UUID of the PatientCaseHistory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientCaseHistoryEntity> patientCaseHistoryEntity = patientCaseHistoryService.findById(id);
	...
	/**
	 * Create/Update the given Patient Case History. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientCaseHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientCaseHistory' endpoint here] off begin
	@Operation(
			description = "Create a PatientCaseHistory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientCaseHistory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientCaseHistory, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientCaseHistoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientCaseHistory' endpoint here] end
	public ResponseEntity<?> savePatientCaseHistory(
			@Parameter(description = "The PatientCaseHistoryEntity to create or update.")
			@RequestBody PatientCaseHistoryEntityDto patientCaseHistoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientCaseHistoryEntity patientCaseHistoryEntity = patientCaseHistoryService.create(new PatientCaseHistoryEntity(patientCaseHistoryEntityDto));
			return new ResponseEntity<>(patientCaseHistoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Case History. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientCaseHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientCaseHistory' endpoint here] off begin
	@Operation(
			description = "Update a PatientCaseHistory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientCaseHistory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientCaseHistory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientCaseHistoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientCaseHistory' endpoint here] end
	public ResponseEntity<?> updatePatientCaseHistory(
			@Parameter(description = "The PatientCaseHistoryEntity to create or update.")
			@RequestBody PatientCaseHistoryEntityDto patientCaseHistoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientCaseHistoryEntity> checkPatientCaseHistoryExists = patientCaseHistoryService.findById(patientCaseHistoryEntityDto.getId());
		PatientCaseHistoryEntity patientCaseHistoryEntity = new PatientCaseHistoryEntity(patientCaseHistoryEntityDto);

		if (checkPatientCaseHistoryExists.isPresent()) {
			try {
				patientCaseHistoryEntity = patientCaseHistoryService.update(patientCaseHistoryEntity);
				return new ResponseEntity<>(patientCaseHistoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientCaseHistoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Case History that has the same id as the given id.
	 *
	 * @param id The id of the PatientCaseHistoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientCaseHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientCaseHistory to delete.")
		...
		patientCaseHistoryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientCaseHistory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Case History entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientCaseHistory' endpoint here] end
	public void exportPatientCaseHistory(
			@Parameter(description = "The ids of the PatientCaseHistoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientCaseHistoryEntity> entitiesToExport = patientCaseHistoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientCaseHistory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientCaseHistoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientCaseHistoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientCaseHistoryExcludingIds' endpoint here] end
	public void exportPatientCaseHistoryExcludingIds(
			@Parameter(description = "The ids of the PatientCaseHistoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientCaseHistoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientCaseHistoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientCaseHistoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientCaseHistory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientCaseHistory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientCaseHistoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Case History entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientCaseHistoryEntity', 'create')  || hasPermission('PatientCaseHistoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientCaseHistoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientCaseHistoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientCaseHistoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientCaseHistoryEntity> csvToBean = new CsvToBeanBuilder<PatientCaseHistoryEntity>(reader)
				.withType(PatientCaseHistoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientCaseHistoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientCaseHistoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Case History entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Case History entity and you do not have permission");
					}
