...
/**
 * Controller used to handle all REST operations regarding BPJS Referral Specialist
 */
// % protected region % [Update the API route or description for the BPJS Referral SpecialistController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsReferralSpecialistController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsreferralspecialist")
// % protected region % [Update the API route or description for the BPJS Referral SpecialistController here] end
public class BpjsReferralSpecialistController {

	private final BpjsReferralSpecialistService bpjsReferralSpecialistService;
	private final CsvUtils<BpjsReferralSpecialistEntity, BpjsReferralSpecialistRepository, BpjsReferralSpecialistEntityAudit,  BpjsReferralSpecialistService> csvUtils;
	...
	@Autowired
	public BpjsReferralSpecialistController(
		...
		this.csvUtils = new CsvUtils<>(bpjsReferralSpecialistService, BpjsReferralSpecialistEntity.class);
	...
	public ResponseEntity<List<BpjsReferralSpecialistEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsReferralSpecialistEntity> bpjsReferralSpecialists = bpjsReferralSpecialistService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsReferralSpecialists, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Referral Specialist that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsReferralSpecialistEntity to be returned
	 * @return the BPJS Referral Specialist that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsReferralSpecialist as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsReferralSpecialist entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsReferralSpecialistEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsReferralSpecialistEntity> getById(
			@Parameter(description = "The UUID of the BpjsReferralSpecialist to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsReferralSpecialistEntity> bpjsReferralSpecialistEntity = bpjsReferralSpecialistService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Referral Specialist. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsReferralSpecialistEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsReferralSpecialist' endpoint here] off begin
	@Operation(
			description = "Create a BpjsReferralSpecialist if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsReferralSpecialist."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsReferralSpecialist, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsReferralSpecialistEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsReferralSpecialist' endpoint here] end
	public ResponseEntity<?> saveBpjsReferralSpecialist(
			@Parameter(description = "The BpjsReferralSpecialistEntity to create or update.")
			@RequestBody BpjsReferralSpecialistEntityDto bpjsReferralSpecialistEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsReferralSpecialistEntity bpjsReferralSpecialistEntity = bpjsReferralSpecialistService.create(new BpjsReferralSpecialistEntity(bpjsReferralSpecialistEntityDto));
			return new ResponseEntity<>(bpjsReferralSpecialistEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Referral Specialist. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsReferralSpecialistEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsReferralSpecialist' endpoint here] off begin
	@Operation(
			description = "Update a BpjsReferralSpecialist entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsReferralSpecialist."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsReferralSpecialist, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsReferralSpecialistEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsReferralSpecialist' endpoint here] end
	public ResponseEntity<?> updateBpjsReferralSpecialist(
			@Parameter(description = "The BpjsReferralSpecialistEntity to create or update.")
			@RequestBody BpjsReferralSpecialistEntityDto bpjsReferralSpecialistEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsReferralSpecialistEntity> checkBpjsReferralSpecialistExists = bpjsReferralSpecialistService.findById(bpjsReferralSpecialistEntityDto.getId());
		BpjsReferralSpecialistEntity bpjsReferralSpecialistEntity = new BpjsReferralSpecialistEntity(bpjsReferralSpecialistEntityDto);

		if (checkBpjsReferralSpecialistExists.isPresent()) {
			try {
				bpjsReferralSpecialistEntity = bpjsReferralSpecialistService.update(bpjsReferralSpecialistEntity);
				return new ResponseEntity<>(bpjsReferralSpecialistEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsReferralSpecialistEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Referral Specialist that has the same id as the given id.
	 *
	 * @param id The id of the BpjsReferralSpecialistEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsReferralSpecialist as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsReferralSpecialist to delete.")
		...
		bpjsReferralSpecialistService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsReferralSpecialist' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Referral Specialist entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsReferralSpecialist' endpoint here] end
	public void exportBpjsReferralSpecialist(
			@Parameter(description = "The ids of the BpjsReferralSpecialistEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsReferralSpecialistEntity> entitiesToExport = bpjsReferralSpecialistService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsReferralSpecialist_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsReferralSpecialistEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsReferralSpecialistExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsReferralSpecialistExcludingIds' endpoint here] end
	public void exportBpjsReferralSpecialistExcludingIds(
			@Parameter(description = "The ids of the BpjsReferralSpecialistEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsReferralSpecialistEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsReferralSpecialistService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsReferralSpecialistService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsReferralSpecialist entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsReferralSpecialist_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsReferralSpecialistEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Referral Specialist entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsReferralSpecialistEntity', 'create')  || hasPermission('BpjsReferralSpecialistEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsReferralSpecialistEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsReferralSpecialistEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsReferralSpecialistEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsReferralSpecialistEntity> csvToBean = new CsvToBeanBuilder<BpjsReferralSpecialistEntity>(reader)
				.withType(BpjsReferralSpecialistEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsReferralSpecialistEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsReferralSpecialistEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Referral Specialist entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Referral Specialist entity and you do not have permission");
					}
