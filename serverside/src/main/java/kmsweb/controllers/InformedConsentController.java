...
import kmsweb.entities.InformedConsentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Informed Consent
 */
// % protected region % [Update the API route or description for the Informed ConsentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InformedConsentController"))
@RestController
@Slf4j
@RequestMapping("/api/informedconsent")
// % protected region % [Update the API route or description for the Informed ConsentController here] end
public class InformedConsentController {

	private final InformedConsentService informedConsentService;
	private final CsvUtils<InformedConsentEntity, InformedConsentRepository, InformedConsentEntityAudit,  InformedConsentService> csvUtils;
	...
	public InformedConsentController(
		...
		this.csvUtils = new CsvUtils<>(informedConsentService, InformedConsentEntity.class);
	...
	public ResponseEntity<List<InformedConsentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InformedConsentEntity> informedConsents = informedConsentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(informedConsents, HttpStatus.OK);
	}

	/**
	 * Return the Informed Consent that has the same id as the given id.
	 *
	 * @param id      The id of the InformedConsentEntity to be returned
	 * @return the Informed Consent that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InformedConsent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InformedConsent entity not found"),
	})
	@PreAuthorize("hasPermission('InformedConsentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InformedConsentEntity> getById(
			@Parameter(description = "The UUID of the InformedConsent to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InformedConsentEntity> informedConsentEntity = informedConsentService.findById(id);
	...
	/**
	 * Create/Update the given Informed Consent. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param informedConsentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInformedConsent' endpoint here] off begin
	@Operation(
			description = "Create a InformedConsent if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InformedConsent."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InformedConsent, duplicate record found")
	})
	@PreAuthorize("hasPermission('InformedConsentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInformedConsent' endpoint here] end
	public ResponseEntity<?> saveInformedConsent(
			@Parameter(description = "The InformedConsentEntity to create or update.")
			@RequestBody InformedConsentEntityDto informedConsentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InformedConsentEntity informedConsentEntity = informedConsentService.create(new InformedConsentEntity(informedConsentEntityDto));
			return new ResponseEntity<>(informedConsentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Informed Consent. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param informedConsentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInformedConsent' endpoint here] off begin
	@Operation(
			description = "Update a InformedConsent entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InformedConsent."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InformedConsent, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InformedConsentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInformedConsent' endpoint here] end
	public ResponseEntity<?> updateInformedConsent(
			@Parameter(description = "The InformedConsentEntity to create or update.")
			@RequestBody InformedConsentEntityDto informedConsentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InformedConsentEntity> checkInformedConsentExists = informedConsentService.findById(informedConsentEntityDto.getId());
		InformedConsentEntity informedConsentEntity = new InformedConsentEntity(informedConsentEntityDto);

		if (checkInformedConsentExists.isPresent()) {
			try {
				informedConsentEntity = informedConsentService.update(informedConsentEntity);
				return new ResponseEntity<>(informedConsentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(informedConsentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Informed Consent that has the same id as the given id.
	 *
	 * @param id The id of the InformedConsentEntity to be deleted
	...
	@Operation(
			description = "Delete a single InformedConsent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InformedConsent to delete.")
		...
		informedConsentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInformedConsent' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Informed Consent entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInformedConsent' endpoint here] end
	public void exportInformedConsent(
			@Parameter(description = "The ids of the InformedConsentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InformedConsentEntity> entitiesToExport = informedConsentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InformedConsent_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InformedConsentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInformedConsentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInformedConsentExcludingIds' endpoint here] end
	public void exportInformedConsentExcludingIds(
			@Parameter(description = "The ids of the InformedConsentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InformedConsentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = informedConsentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = informedConsentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InformedConsent entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InformedConsent_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InformedConsentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Informed Consent entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InformedConsentEntity', 'create')  || hasPermission('InformedConsentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InformedConsentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InformedConsentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InformedConsentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InformedConsentEntity> csvToBean = new CsvToBeanBuilder<InformedConsentEntity>(reader)
				.withType(InformedConsentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InformedConsentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InformedConsentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Informed Consent entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Informed Consent entity and you do not have permission");
					}
