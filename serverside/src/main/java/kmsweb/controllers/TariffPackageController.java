...
import kmsweb.entities.TariffPackageEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Tariff Package
 */
// % protected region % [Update the API route or description for the Tariff PackageController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TariffPackageController"))
@RestController
@Slf4j
@RequestMapping("/api/tariffpackage")
// % protected region % [Update the API route or description for the Tariff PackageController here] end
public class TariffPackageController {

	private final TariffPackageService tariffPackageService;
	private final CsvUtils<TariffPackageEntity, TariffPackageRepository, TariffPackageEntityAudit,  TariffPackageService> csvUtils;
	...
	@Autowired
	public TariffPackageController(
		...
		this.csvUtils = new CsvUtils<>(tariffPackageService, TariffPackageEntity.class);
	...
	public ResponseEntity<List<TariffPackageEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TariffPackageEntity> tariffPackages = tariffPackageService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(tariffPackages, HttpStatus.OK);
	}

	/**
	 * Return the Tariff Package that has the same id as the given id.
	 *
	 * @param id      The id of the TariffPackageEntity to be returned
	 * @return the Tariff Package that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TariffPackage as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TariffPackage entity not found"),
	})
	@PreAuthorize("hasPermission('TariffPackageEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TariffPackageEntity> getById(
			@Parameter(description = "The UUID of the TariffPackage to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TariffPackageEntity> tariffPackageEntity = tariffPackageService.findById(id);
	...
	/**
	 * Create/Update the given Tariff Package. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param tariffPackageEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTariffPackage' endpoint here] off begin
	@Operation(
			description = "Create a TariffPackage if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TariffPackage."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TariffPackage, duplicate record found")
	})
	@PreAuthorize("hasPermission('TariffPackageEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTariffPackage' endpoint here] end
	public ResponseEntity<?> saveTariffPackage(
			@Parameter(description = "The TariffPackageEntity to create or update.")
			@RequestBody TariffPackageEntityDto tariffPackageEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TariffPackageEntity tariffPackageEntity = tariffPackageService.create(new TariffPackageEntity(tariffPackageEntityDto));
			return new ResponseEntity<>(tariffPackageEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Tariff Package. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param tariffPackageEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTariffPackage' endpoint here] off begin
	@Operation(
			description = "Update a TariffPackage entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TariffPackage."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TariffPackage, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TariffPackageEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTariffPackage' endpoint here] end
	public ResponseEntity<?> updateTariffPackage(
			@Parameter(description = "The TariffPackageEntity to create or update.")
			@RequestBody TariffPackageEntityDto tariffPackageEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TariffPackageEntity> checkTariffPackageExists = tariffPackageService.findById(tariffPackageEntityDto.getId());
		TariffPackageEntity tariffPackageEntity = new TariffPackageEntity(tariffPackageEntityDto);

		if (checkTariffPackageExists.isPresent()) {
			try {
				tariffPackageEntity = tariffPackageService.update(tariffPackageEntity);
				return new ResponseEntity<>(tariffPackageEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(tariffPackageEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Tariff Package that has the same id as the given id.
	 *
	 * @param id The id of the TariffPackageEntity to be deleted
	...
	@Operation(
			description = "Delete a single TariffPackage as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TariffPackage to delete.")
		...
		tariffPackageService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffPackage' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Tariff Package entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffPackage' endpoint here] end
	public void exportTariffPackage(
			@Parameter(description = "The ids of the TariffPackageEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TariffPackageEntity> entitiesToExport = tariffPackageService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TariffPackage_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TariffPackageEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageExcludingIds' endpoint here] end
	public void exportTariffPackageExcludingIds(
			@Parameter(description = "The ids of the TariffPackageEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TariffPackageEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = tariffPackageService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = tariffPackageService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TariffPackage entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TariffPackage_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TariffPackageEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Tariff Package entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TariffPackageEntity', 'create')  || hasPermission('TariffPackageEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TariffPackageEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TariffPackageEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TariffPackageEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TariffPackageEntity> csvToBean = new CsvToBeanBuilder<TariffPackageEntity>(reader)
				.withType(TariffPackageEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TariffPackageEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TariffPackageEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Tariff Package entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Tariff Package entity and you do not have permission");
					}
