...
import kmsweb.entities.SatuSehatConfigurationEntityAudit;
import kmsweb.repositories.SatuSehatConfigurationRepository;
import kmsweb.services.SatuSehatConfigurationService;
...
/**
 * Controller used to handle all REST operations regarding Satu Sehat Configuration
 */
// % protected region % [Update the API route or description for the Satu Sehat ConfigurationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SatuSehatConfigurationController"))
@RestController
@Slf4j
@RequestMapping("/api/satusehatconfiguration")
// % protected region % [Update the API route or description for the Satu Sehat ConfigurationController here] end
public class SatuSehatConfigurationController {

	private final SatuSehatConfigurationService satuSehatConfigurationService;
	private final CsvUtils<SatuSehatConfigurationEntity, SatuSehatConfigurationRepository, SatuSehatConfigurationEntityAudit,  SatuSehatConfigurationService> csvUtils;
	...
	@Autowired
	public SatuSehatConfigurationController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			SatuSehatConfigurationService satuSehatConfigurationService
	) {
		this.authService = authService;
		this.satuSehatConfigurationService = satuSehatConfigurationService;
		this.csvUtils = new CsvUtils<>(satuSehatConfigurationService, SatuSehatConfigurationEntity.class);
	...
	public ResponseEntity<List<SatuSehatConfigurationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SatuSehatConfigurationEntity> satuSehatConfigurations = satuSehatConfigurationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(satuSehatConfigurations, HttpStatus.OK);
	}

	/**
	 * Return the Satu Sehat Configuration that has the same id as the given id.
	 *
	 * @param id      The id of the SatuSehatConfigurationEntity to be returned
	 * @return the Satu Sehat Configuration that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single SatuSehatConfiguration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "SatuSehatConfiguration entity not found"),
	})
	@PreAuthorize("hasPermission('SatuSehatConfigurationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SatuSehatConfigurationEntity> getById(
			@Parameter(description = "The UUID of the SatuSehatConfiguration to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SatuSehatConfigurationEntity> satuSehatConfigurationEntity = satuSehatConfigurationService.findById(id);
	...
	/**
	 * Create/Update the given Satu Sehat Configuration. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param satuSehatConfigurationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSatuSehatConfiguration' endpoint here] off begin
	@Operation(
			description = "Create a SatuSehatConfiguration if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new SatuSehatConfiguration."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new SatuSehatConfiguration, duplicate record found")
	})
	@PreAuthorize("hasPermission('SatuSehatConfigurationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSatuSehatConfiguration' endpoint here] end
	public ResponseEntity<?> saveSatuSehatConfiguration(
			@Parameter(description = "The SatuSehatConfigurationEntity to create or update.")
			@RequestBody SatuSehatConfigurationEntityDto satuSehatConfigurationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SatuSehatConfigurationEntity satuSehatConfigurationEntity = satuSehatConfigurationService.create(new SatuSehatConfigurationEntity(satuSehatConfigurationEntityDto));
			return new ResponseEntity<>(satuSehatConfigurationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Satu Sehat Configuration. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param satuSehatConfigurationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSatuSehatConfiguration' endpoint here] off begin
	@Operation(
			description = "Update a SatuSehatConfiguration entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the SatuSehatConfiguration."),
		@ApiResponse(responseCode = "400", description = "Failed to update the SatuSehatConfiguration, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SatuSehatConfigurationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSatuSehatConfiguration' endpoint here] end
	public ResponseEntity<?> updateSatuSehatConfiguration(
			@Parameter(description = "The SatuSehatConfigurationEntity to create or update.")
			@RequestBody SatuSehatConfigurationEntityDto satuSehatConfigurationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SatuSehatConfigurationEntity> checkSatuSehatConfigurationExists = satuSehatConfigurationService.findById(satuSehatConfigurationEntityDto.getId());
		SatuSehatConfigurationEntity satuSehatConfigurationEntity = new SatuSehatConfigurationEntity(satuSehatConfigurationEntityDto);

		if (checkSatuSehatConfigurationExists.isPresent()) {
			try {
				satuSehatConfigurationEntity = satuSehatConfigurationService.update(satuSehatConfigurationEntity);
				return new ResponseEntity<>(satuSehatConfigurationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(satuSehatConfigurationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Satu Sehat Configuration that has the same id as the given id.
	 *
	 * @param id The id of the SatuSehatConfigurationEntity to be deleted
	...
	@Operation(
			description = "Delete a single SatuSehatConfiguration as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the SatuSehatConfiguration to delete.")
		...
		satuSehatConfigurationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSatuSehatConfiguration' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Satu Sehat Configuration entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSatuSehatConfiguration' endpoint here] end
	public void exportSatuSehatConfiguration(
			@Parameter(description = "The ids of the SatuSehatConfigurationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SatuSehatConfigurationEntity> entitiesToExport = satuSehatConfigurationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "SatuSehatConfiguration_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SatuSehatConfigurationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSatuSehatConfigurationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSatuSehatConfigurationExcludingIds' endpoint here] end
	public void exportSatuSehatConfigurationExcludingIds(
			@Parameter(description = "The ids of the SatuSehatConfigurationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SatuSehatConfigurationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = satuSehatConfigurationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = satuSehatConfigurationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the SatuSehatConfiguration entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "SatuSehatConfiguration_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SatuSehatConfigurationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Satu Sehat Configuration entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SatuSehatConfigurationEntity', 'create')  || hasPermission('SatuSehatConfigurationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SatuSehatConfigurationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SatuSehatConfigurationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SatuSehatConfigurationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SatuSehatConfigurationEntity> csvToBean = new CsvToBeanBuilder<SatuSehatConfigurationEntity>(reader)
				.withType(SatuSehatConfigurationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<SatuSehatConfigurationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<SatuSehatConfigurationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Satu Sehat Configuration entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Satu Sehat Configuration entity and you do not have permission");
					}
