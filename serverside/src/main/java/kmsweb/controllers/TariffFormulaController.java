...
import kmsweb.entities.TariffFormulaEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Tariff Formula
 */
// % protected region % [Update the API route or description for the Tariff FormulaController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TariffFormulaController"))
@RestController
@Slf4j
@RequestMapping("/api/tariffformula")
// % protected region % [Update the API route or description for the Tariff FormulaController here] end
public class TariffFormulaController {

	private final TariffFormulaService tariffFormulaService;
	private final CsvUtils<TariffFormulaEntity, TariffFormulaRepository, TariffFormulaEntityAudit,  TariffFormulaService> csvUtils;
	...
	@Autowired
	public TariffFormulaController(
		...
		this.csvUtils = new CsvUtils<>(tariffFormulaService, TariffFormulaEntity.class);
	...
	public ResponseEntity<List<TariffFormulaEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TariffFormulaEntity> tariffFormulas = tariffFormulaService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(tariffFormulas, HttpStatus.OK);
	}

	/**
	 * Return the Tariff Formula that has the same id as the given id.
	 *
	 * @param id      The id of the TariffFormulaEntity to be returned
	 * @return the Tariff Formula that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TariffFormula as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TariffFormula entity not found"),
	})
	@PreAuthorize("hasPermission('TariffFormulaEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TariffFormulaEntity> getById(
			@Parameter(description = "The UUID of the TariffFormula to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TariffFormulaEntity> tariffFormulaEntity = tariffFormulaService.findById(id);
	...
	/**
	 * Create/Update the given Tariff Formula. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param tariffFormulaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTariffFormula' endpoint here] off begin
	@Operation(
			description = "Create a TariffFormula if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TariffFormula."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TariffFormula, duplicate record found")
	})
	@PreAuthorize("hasPermission('TariffFormulaEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTariffFormula' endpoint here] end
	public ResponseEntity<?> saveTariffFormula(
			@Parameter(description = "The TariffFormulaEntity to create or update.")
			@RequestBody TariffFormulaEntityDto tariffFormulaEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TariffFormulaEntity tariffFormulaEntity = tariffFormulaService.create(new TariffFormulaEntity(tariffFormulaEntityDto));
			return new ResponseEntity<>(tariffFormulaEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Tariff Formula. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param tariffFormulaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTariffFormula' endpoint here] off begin
	@Operation(
			description = "Update a TariffFormula entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TariffFormula."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TariffFormula, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TariffFormulaEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTariffFormula' endpoint here] end
	public ResponseEntity<?> updateTariffFormula(
			@Parameter(description = "The TariffFormulaEntity to create or update.")
			@RequestBody TariffFormulaEntityDto tariffFormulaEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TariffFormulaEntity> checkTariffFormulaExists = tariffFormulaService.findById(tariffFormulaEntityDto.getId());
		TariffFormulaEntity tariffFormulaEntity = new TariffFormulaEntity(tariffFormulaEntityDto);

		if (checkTariffFormulaExists.isPresent()) {
			try {
				tariffFormulaEntity = tariffFormulaService.update(tariffFormulaEntity);
				return new ResponseEntity<>(tariffFormulaEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(tariffFormulaEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Tariff Formula that has the same id as the given id.
	 *
	 * @param id The id of the TariffFormulaEntity to be deleted
	...
	@Operation(
			description = "Delete a single TariffFormula as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TariffFormula to delete.")
		...
		tariffFormulaService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffFormula' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Tariff Formula entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffFormula' endpoint here] end
	public void exportTariffFormula(
			@Parameter(description = "The ids of the TariffFormulaEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TariffFormulaEntity> entitiesToExport = tariffFormulaService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TariffFormula_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TariffFormulaEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffFormulaExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffFormulaExcludingIds' endpoint here] end
	public void exportTariffFormulaExcludingIds(
			@Parameter(description = "The ids of the TariffFormulaEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TariffFormulaEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = tariffFormulaService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = tariffFormulaService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TariffFormula entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TariffFormula_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TariffFormulaEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Tariff Formula entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TariffFormulaEntity', 'create')  || hasPermission('TariffFormulaEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TariffFormulaEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TariffFormulaEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TariffFormulaEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TariffFormulaEntity> csvToBean = new CsvToBeanBuilder<TariffFormulaEntity>(reader)
				.withType(TariffFormulaEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TariffFormulaEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TariffFormulaEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Tariff Formula entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Tariff Formula entity and you do not have permission");
					}
