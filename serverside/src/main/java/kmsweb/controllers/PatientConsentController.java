...
import kmsweb.entities.PatientConsentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Consent
 */
// % protected region % [Update the API route or description for the Patient ConsentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientConsentController"))
@RestController
@Slf4j
@RequestMapping("/api/patientconsent")
// % protected region % [Update the API route or description for the Patient ConsentController here] end
public class PatientConsentController {

	private final PatientConsentService patientConsentService;
	private final CsvUtils<PatientConsentEntity, PatientConsentRepository, PatientConsentEntityAudit,  PatientConsentService> csvUtils;
	...
	public PatientConsentController(
		...
		this.csvUtils = new CsvUtils<>(patientConsentService, PatientConsentEntity.class);
	...
	public ResponseEntity<List<PatientConsentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientConsentEntity> patientConsents = patientConsentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientConsents, HttpStatus.OK);
	}

	/**
	 * Return the Patient Consent that has the same id as the given id.
	 *
	 * @param id      The id of the PatientConsentEntity to be returned
	 * @return the Patient Consent that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientConsent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientConsent entity not found"),
	})
	@PreAuthorize("hasPermission('PatientConsentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientConsentEntity> getById(
			@Parameter(description = "The UUID of the PatientConsent to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientConsentEntity> patientConsentEntity = patientConsentService.findById(id);
	...
	/**
	 * Create/Update the given Patient Consent. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientConsentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientConsent' endpoint here] off begin
	@Operation(
			description = "Create a PatientConsent if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientConsent."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientConsent, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientConsentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientConsent' endpoint here] end
	public ResponseEntity<?> savePatientConsent(
			@Parameter(description = "The PatientConsentEntity to create or update.")
			@RequestBody PatientConsentEntityDto patientConsentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientConsentEntity patientConsentEntity = patientConsentService.create(new PatientConsentEntity(patientConsentEntityDto));
			return new ResponseEntity<>(patientConsentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Consent. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientConsentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientConsent' endpoint here] off begin
	@Operation(
			description = "Update a PatientConsent entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientConsent."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientConsent, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientConsentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientConsent' endpoint here] end
	public ResponseEntity<?> updatePatientConsent(
			@Parameter(description = "The PatientConsentEntity to create or update.")
			@RequestBody PatientConsentEntityDto patientConsentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientConsentEntity> checkPatientConsentExists = patientConsentService.findById(patientConsentEntityDto.getId());
		PatientConsentEntity patientConsentEntity = new PatientConsentEntity(patientConsentEntityDto);

		if (checkPatientConsentExists.isPresent()) {
			try {
				patientConsentEntity = patientConsentService.update(patientConsentEntity);
				return new ResponseEntity<>(patientConsentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientConsentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Consent that has the same id as the given id.
	 *
	 * @param id The id of the PatientConsentEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientConsent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientConsent to delete.")
		...
		patientConsentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientConsent' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Consent entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientConsent' endpoint here] end
	public void exportPatientConsent(
			@Parameter(description = "The ids of the PatientConsentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientConsentEntity> entitiesToExport = patientConsentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientConsent_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientConsentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientConsentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientConsentExcludingIds' endpoint here] end
	public void exportPatientConsentExcludingIds(
			@Parameter(description = "The ids of the PatientConsentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientConsentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientConsentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientConsentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientConsent entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientConsent_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientConsentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Consent entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientConsentEntity', 'create')  || hasPermission('PatientConsentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientConsentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientConsentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientConsentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientConsentEntity> csvToBean = new CsvToBeanBuilder<PatientConsentEntity>(reader)
				.withType(PatientConsentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientConsentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientConsentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Consent entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Consent entity and you do not have permission");
					}
