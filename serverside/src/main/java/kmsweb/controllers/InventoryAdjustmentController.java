...
/**
 * Controller used to handle all REST operations regarding Inventory Adjustment
 */
// % protected region % [Update the API route or description for the Inventory AdjustmentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InventoryAdjustmentController"))
@RestController
@Slf4j
@RequestMapping("/api/inventoryadjustment")
// % protected region % [Update the API route or description for the Inventory AdjustmentController here] end
public class InventoryAdjustmentController {

	private final InventoryAdjustmentService inventoryAdjustmentService;
	private final CsvUtils<InventoryAdjustmentEntity, InventoryAdjustmentRepository, InventoryAdjustmentEntityAudit,  InventoryAdjustmentService> csvUtils;
	...
	public InventoryAdjustmentController(
		...
		this.csvUtils = new CsvUtils<>(inventoryAdjustmentService, InventoryAdjustmentEntity.class);
	...
	public ResponseEntity<List<InventoryAdjustmentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InventoryAdjustmentEntity> inventoryAdjustments = inventoryAdjustmentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Inventory Adjustment that has the same id as the given id.
	 *
	 * @param id      The id of the InventoryAdjustmentEntity to be returned
	 * @return the Inventory Adjustment that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InventoryAdjustment as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InventoryAdjustment entity not found"),
	})
	@PreAuthorize("hasPermission('InventoryAdjustmentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InventoryAdjustmentEntity> getById(
			@Parameter(description = "The UUID of the InventoryAdjustment to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InventoryAdjustmentEntity> inventoryAdjustmentEntity = inventoryAdjustmentService.findById(id);
	...
	/**
	 * Create/Update the given Inventory Adjustment. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryAdjustmentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInventoryAdjustment' endpoint here] off begin
	@Operation(
			description = "Create a InventoryAdjustment if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InventoryAdjustment."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InventoryAdjustment, duplicate record found")
	})
	@PreAuthorize("hasPermission('InventoryAdjustmentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInventoryAdjustment' endpoint here] end
	public ResponseEntity<?> saveInventoryAdjustment(
			@Parameter(description = "The InventoryAdjustmentEntity to create or update.")
			@RequestBody InventoryAdjustmentEntityDto inventoryAdjustmentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InventoryAdjustmentEntity inventoryAdjustmentEntity = inventoryAdjustmentService.create(new InventoryAdjustmentEntity(inventoryAdjustmentEntityDto));
			return new ResponseEntity<>(inventoryAdjustmentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Inventory Adjustment. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param inventoryAdjustmentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInventoryAdjustment' endpoint here] off begin
	@Operation(
			description = "Update a InventoryAdjustment entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InventoryAdjustment."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InventoryAdjustment, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InventoryAdjustmentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInventoryAdjustment' endpoint here] end
	public ResponseEntity<?> updateInventoryAdjustment(
			@Parameter(description = "The InventoryAdjustmentEntity to create or update.")
			@RequestBody InventoryAdjustmentEntityDto inventoryAdjustmentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InventoryAdjustmentEntity> checkInventoryAdjustmentExists = inventoryAdjustmentService.findById(inventoryAdjustmentEntityDto.getId());
		InventoryAdjustmentEntity inventoryAdjustmentEntity = new InventoryAdjustmentEntity(inventoryAdjustmentEntityDto);

		if (checkInventoryAdjustmentExists.isPresent()) {
			try {
				inventoryAdjustmentEntity = inventoryAdjustmentService.update(inventoryAdjustmentEntity);
				return new ResponseEntity<>(inventoryAdjustmentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(inventoryAdjustmentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Inventory Adjustment that has the same id as the given id.
	 *
	 * @param id The id of the InventoryAdjustmentEntity to be deleted
	...
	@Operation(
			description = "Delete a single InventoryAdjustment as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InventoryAdjustment to delete.")
		...
		inventoryAdjustmentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustment' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Inventory Adjustment entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustment' endpoint here] end
	public void exportInventoryAdjustment(
			@Parameter(description = "The ids of the InventoryAdjustmentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InventoryAdjustmentEntity> entitiesToExport = inventoryAdjustmentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InventoryAdjustment_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InventoryAdjustmentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustmentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInventoryAdjustmentExcludingIds' endpoint here] end
	public void exportInventoryAdjustmentExcludingIds(
			@Parameter(description = "The ids of the InventoryAdjustmentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InventoryAdjustmentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = inventoryAdjustmentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = inventoryAdjustmentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InventoryAdjustment entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InventoryAdjustment_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InventoryAdjustmentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Inventory Adjustment entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InventoryAdjustmentEntity', 'create')  || hasPermission('InventoryAdjustmentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InventoryAdjustmentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InventoryAdjustmentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InventoryAdjustmentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InventoryAdjustmentEntity> csvToBean = new CsvToBeanBuilder<InventoryAdjustmentEntity>(reader)
				.withType(InventoryAdjustmentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InventoryAdjustmentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InventoryAdjustmentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Inventory Adjustment entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Inventory Adjustment entity and you do not have permission");
					}
