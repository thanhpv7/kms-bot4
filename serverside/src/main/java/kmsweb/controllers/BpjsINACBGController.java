...
/**
 * Controller used to handle all REST operations regarding BPJS INACBG
 */
// % protected region % [Update the API route or description for the BPJS INACBGController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsINACBGController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsinacbg")
// % protected region % [Update the API route or description for the BPJS INACBGController here] end
public class BpjsINACBGController {

	private final BpjsINACBGService bpjsINACBGService;
	private final CsvUtils<BpjsINACBGEntity, BpjsINACBGRepository, BpjsINACBGEntityAudit,  BpjsINACBGService> csvUtils;
	...
	@Autowired
	public BpjsINACBGController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsINACBGService bpjsINACBGService
	) {
		this.authService = authService;
		this.bpjsINACBGService = bpjsINACBGService;
		this.csvUtils = new CsvUtils<>(bpjsINACBGService, BpjsINACBGEntity.class);
	...
	public ResponseEntity<List<BpjsINACBGEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsINACBGEntity> bpjsINACBGs = bpjsINACBGService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsINACBGs, HttpStatus.OK);
	}

	/**
	 * Return the BPJS INACBG that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsINACBGEntity to be returned
	 * @return the BPJS INACBG that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsINACBG as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsINACBG entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsINACBGEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsINACBGEntity> getById(
			@Parameter(description = "The UUID of the BpjsINACBG to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsINACBGEntity> bpjsINACBGEntity = bpjsINACBGService.findById(id);
	...
	/**
	 * Create/Update the given BPJS INACBG. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsINACBGEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsINACBG' endpoint here] off begin
	@Operation(
			description = "Create a BpjsINACBG if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsINACBG."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsINACBG, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsINACBGEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsINACBG' endpoint here] end
	public ResponseEntity<?> saveBpjsINACBG(
			@Parameter(description = "The BpjsINACBGEntity to create or update.")
			@RequestBody BpjsINACBGEntityDto bpjsINACBGEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsINACBGEntity bpjsINACBGEntity = bpjsINACBGService.create(new BpjsINACBGEntity(bpjsINACBGEntityDto));
			return new ResponseEntity<>(bpjsINACBGEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS INACBG. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsINACBGEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsINACBG' endpoint here] off begin
	@Operation(
			description = "Update a BpjsINACBG entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsINACBG."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsINACBG, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsINACBGEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsINACBG' endpoint here] end
	public ResponseEntity<?> updateBpjsINACBG(
			@Parameter(description = "The BpjsINACBGEntity to create or update.")
			@RequestBody BpjsINACBGEntityDto bpjsINACBGEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsINACBGEntity> checkBpjsINACBGExists = bpjsINACBGService.findById(bpjsINACBGEntityDto.getId());
		BpjsINACBGEntity bpjsINACBGEntity = new BpjsINACBGEntity(bpjsINACBGEntityDto);

		if (checkBpjsINACBGExists.isPresent()) {
			try {
				bpjsINACBGEntity = bpjsINACBGService.update(bpjsINACBGEntity);
				return new ResponseEntity<>(bpjsINACBGEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsINACBGEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS INACBG that has the same id as the given id.
	 *
	 * @param id The id of the BpjsINACBGEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsINACBG as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsINACBG to delete.")
		...
		bpjsINACBGService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBG' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS INACBG entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBG' endpoint here] end
	public void exportBpjsINACBG(
			@Parameter(description = "The ids of the BpjsINACBGEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsINACBGEntity> entitiesToExport = bpjsINACBGService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsINACBG_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsINACBGEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsINACBGExcludingIds' endpoint here] end
	public void exportBpjsINACBGExcludingIds(
			@Parameter(description = "The ids of the BpjsINACBGEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsINACBGEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsINACBGService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsINACBGService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsINACBG entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsINACBG_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsINACBGEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS INACBG entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsINACBGEntity', 'create')  || hasPermission('BpjsINACBGEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsINACBGEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsINACBGEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsINACBGEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsINACBGEntity> csvToBean = new CsvToBeanBuilder<BpjsINACBGEntity>(reader)
				.withType(BpjsINACBGEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsINACBGEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsINACBGEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS INACBG entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS INACBG entity and you do not have permission");
					}
