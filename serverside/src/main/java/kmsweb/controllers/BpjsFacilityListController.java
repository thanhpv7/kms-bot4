...
/**
 * Controller used to handle all REST operations regarding BPJS Facility List
 */
// % protected region % [Update the API route or description for the BPJS Facility ListController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsFacilityListController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsfacilitylist")
// % protected region % [Update the API route or description for the BPJS Facility ListController here] end
public class BpjsFacilityListController {

	private final BpjsFacilityListService bpjsFacilityListService;
	private final CsvUtils<BpjsFacilityListEntity, BpjsFacilityListRepository, BpjsFacilityListEntityAudit,  BpjsFacilityListService> csvUtils;
	...
	@Autowired
	public BpjsFacilityListController(
		...
		this.csvUtils = new CsvUtils<>(bpjsFacilityListService, BpjsFacilityListEntity.class);
	...
	public ResponseEntity<List<BpjsFacilityListEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsFacilityListEntity> bpjsFacilityLists = bpjsFacilityListService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsFacilityLists, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Facility List that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsFacilityListEntity to be returned
	 * @return the BPJS Facility List that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsFacilityList as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsFacilityList entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsFacilityListEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsFacilityListEntity> getById(
			@Parameter(description = "The UUID of the BpjsFacilityList to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsFacilityListEntity> bpjsFacilityListEntity = bpjsFacilityListService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Facility List. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsFacilityListEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsFacilityList' endpoint here] off begin
	@Operation(
			description = "Create a BpjsFacilityList if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsFacilityList."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsFacilityList, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsFacilityListEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsFacilityList' endpoint here] end
	public ResponseEntity<?> saveBpjsFacilityList(
			@Parameter(description = "The BpjsFacilityListEntity to create or update.")
			@RequestBody BpjsFacilityListEntityDto bpjsFacilityListEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsFacilityListEntity bpjsFacilityListEntity = bpjsFacilityListService.create(new BpjsFacilityListEntity(bpjsFacilityListEntityDto));
			return new ResponseEntity<>(bpjsFacilityListEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Facility List. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsFacilityListEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsFacilityList' endpoint here] off begin
	@Operation(
			description = "Update a BpjsFacilityList entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsFacilityList."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsFacilityList, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsFacilityListEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsFacilityList' endpoint here] end
	public ResponseEntity<?> updateBpjsFacilityList(
			@Parameter(description = "The BpjsFacilityListEntity to create or update.")
			@RequestBody BpjsFacilityListEntityDto bpjsFacilityListEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsFacilityListEntity> checkBpjsFacilityListExists = bpjsFacilityListService.findById(bpjsFacilityListEntityDto.getId());
		BpjsFacilityListEntity bpjsFacilityListEntity = new BpjsFacilityListEntity(bpjsFacilityListEntityDto);

		if (checkBpjsFacilityListExists.isPresent()) {
			try {
				bpjsFacilityListEntity = bpjsFacilityListService.update(bpjsFacilityListEntity);
				return new ResponseEntity<>(bpjsFacilityListEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsFacilityListEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Facility List that has the same id as the given id.
	 *
	 * @param id The id of the BpjsFacilityListEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsFacilityList as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsFacilityList to delete.")
		...
		bpjsFacilityListService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsFacilityList' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Facility List entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsFacilityList' endpoint here] end
	public void exportBpjsFacilityList(
			@Parameter(description = "The ids of the BpjsFacilityListEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsFacilityListEntity> entitiesToExport = bpjsFacilityListService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsFacilityList_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsFacilityListEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsFacilityListExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsFacilityListExcludingIds' endpoint here] end
	public void exportBpjsFacilityListExcludingIds(
			@Parameter(description = "The ids of the BpjsFacilityListEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsFacilityListEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsFacilityListService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsFacilityListService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsFacilityList entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsFacilityList_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsFacilityListEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Facility List entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsFacilityListEntity', 'create')  || hasPermission('BpjsFacilityListEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsFacilityListEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsFacilityListEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsFacilityListEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsFacilityListEntity> csvToBean = new CsvToBeanBuilder<BpjsFacilityListEntity>(reader)
				.withType(BpjsFacilityListEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsFacilityListEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsFacilityListEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Facility List entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Facility List entity and you do not have permission");
					}
