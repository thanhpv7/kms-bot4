...
import kmsweb.entities.InvoicePaymentOtherEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Invoice Payment Other
 */
// % protected region % [Update the API route or description for the Invoice Payment OtherController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InvoicePaymentOtherController"))
@RestController
@Slf4j
@RequestMapping("/api/invoicepaymentother")
// % protected region % [Update the API route or description for the Invoice Payment OtherController here] end
public class InvoicePaymentOtherController {

	private final InvoicePaymentOtherService invoicePaymentOtherService;
	private final CsvUtils<InvoicePaymentOtherEntity, InvoicePaymentOtherRepository, InvoicePaymentOtherEntityAudit,  InvoicePaymentOtherService> csvUtils;
	...
	@Autowired
	public InvoicePaymentOtherController(
		...
		this.csvUtils = new CsvUtils<>(invoicePaymentOtherService, InvoicePaymentOtherEntity.class);
	...
	public ResponseEntity<List<InvoicePaymentOtherEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InvoicePaymentOtherEntity> invoicePaymentOthers = invoicePaymentOtherService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(invoicePaymentOthers, HttpStatus.OK);
	}

	/**
	 * Return the Invoice Payment Other that has the same id as the given id.
	 *
	 * @param id      The id of the InvoicePaymentOtherEntity to be returned
	 * @return the Invoice Payment Other that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InvoicePaymentOther as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InvoicePaymentOther entity not found"),
	})
	@PreAuthorize("hasPermission('InvoicePaymentOtherEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InvoicePaymentOtherEntity> getById(
			@Parameter(description = "The UUID of the InvoicePaymentOther to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InvoicePaymentOtherEntity> invoicePaymentOtherEntity = invoicePaymentOtherService.findById(id);
	...
	/**
	 * Create/Update the given Invoice Payment Other. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param invoicePaymentOtherEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInvoicePaymentOther' endpoint here] off begin
	@Operation(
			description = "Create a InvoicePaymentOther if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InvoicePaymentOther."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InvoicePaymentOther, duplicate record found")
	})
	@PreAuthorize("hasPermission('InvoicePaymentOtherEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInvoicePaymentOther' endpoint here] end
	public ResponseEntity<?> saveInvoicePaymentOther(
			@Parameter(description = "The InvoicePaymentOtherEntity to create or update.")
			@RequestBody InvoicePaymentOtherEntityDto invoicePaymentOtherEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InvoicePaymentOtherEntity invoicePaymentOtherEntity = invoicePaymentOtherService.create(new InvoicePaymentOtherEntity(invoicePaymentOtherEntityDto));
			return new ResponseEntity<>(invoicePaymentOtherEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Invoice Payment Other. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param invoicePaymentOtherEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInvoicePaymentOther' endpoint here] off begin
	@Operation(
			description = "Update a InvoicePaymentOther entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InvoicePaymentOther."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InvoicePaymentOther, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InvoicePaymentOtherEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInvoicePaymentOther' endpoint here] end
	public ResponseEntity<?> updateInvoicePaymentOther(
			@Parameter(description = "The InvoicePaymentOtherEntity to create or update.")
			@RequestBody InvoicePaymentOtherEntityDto invoicePaymentOtherEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InvoicePaymentOtherEntity> checkInvoicePaymentOtherExists = invoicePaymentOtherService.findById(invoicePaymentOtherEntityDto.getId());
		InvoicePaymentOtherEntity invoicePaymentOtherEntity = new InvoicePaymentOtherEntity(invoicePaymentOtherEntityDto);

		if (checkInvoicePaymentOtherExists.isPresent()) {
			try {
				invoicePaymentOtherEntity = invoicePaymentOtherService.update(invoicePaymentOtherEntity);
				return new ResponseEntity<>(invoicePaymentOtherEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(invoicePaymentOtherEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Invoice Payment Other that has the same id as the given id.
	 *
	 * @param id The id of the InvoicePaymentOtherEntity to be deleted
	...
	@Operation(
			description = "Delete a single InvoicePaymentOther as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InvoicePaymentOther to delete.")
		...
		invoicePaymentOtherService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentOther' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Invoice Payment Other entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentOther' endpoint here] end
	public void exportInvoicePaymentOther(
			@Parameter(description = "The ids of the InvoicePaymentOtherEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InvoicePaymentOtherEntity> entitiesToExport = invoicePaymentOtherService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InvoicePaymentOther_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InvoicePaymentOtherEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentOtherExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoicePaymentOtherExcludingIds' endpoint here] end
	public void exportInvoicePaymentOtherExcludingIds(
			@Parameter(description = "The ids of the InvoicePaymentOtherEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InvoicePaymentOtherEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = invoicePaymentOtherService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = invoicePaymentOtherService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InvoicePaymentOther entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InvoicePaymentOther_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InvoicePaymentOtherEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Invoice Payment Other entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InvoicePaymentOtherEntity', 'create')  || hasPermission('InvoicePaymentOtherEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InvoicePaymentOtherEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InvoicePaymentOtherEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InvoicePaymentOtherEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InvoicePaymentOtherEntity> csvToBean = new CsvToBeanBuilder<InvoicePaymentOtherEntity>(reader)
				.withType(InvoicePaymentOtherEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InvoicePaymentOtherEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InvoicePaymentOtherEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Invoice Payment Other entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Invoice Payment Other entity and you do not have permission");
					}
