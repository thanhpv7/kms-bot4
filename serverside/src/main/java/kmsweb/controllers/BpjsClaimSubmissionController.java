...
import kmsweb.entities.BpjsClaimSubmissionEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Claim Submission
 */
// % protected region % [Update the API route or description for the BPJS Claim SubmissionController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsClaimSubmissionController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsclaimsubmission")
// % protected region % [Update the API route or description for the BPJS Claim SubmissionController here] end
public class BpjsClaimSubmissionController {

	private final BpjsClaimSubmissionService bpjsClaimSubmissionService;
	private final CsvUtils<BpjsClaimSubmissionEntity, BpjsClaimSubmissionRepository, BpjsClaimSubmissionEntityAudit,  BpjsClaimSubmissionService> csvUtils;
	...
	@Autowired
	public BpjsClaimSubmissionController(
		...
		this.csvUtils = new CsvUtils<>(bpjsClaimSubmissionService, BpjsClaimSubmissionEntity.class);
	...
	public ResponseEntity<List<BpjsClaimSubmissionEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsClaimSubmissionEntity> bpjsClaimSubmissions = bpjsClaimSubmissionService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsClaimSubmissions, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Claim Submission that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsClaimSubmissionEntity to be returned
	 * @return the BPJS Claim Submission that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsClaimSubmission as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsClaimSubmission entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsClaimSubmissionEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsClaimSubmissionEntity> getById(
			@Parameter(description = "The UUID of the BpjsClaimSubmission to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsClaimSubmissionEntity> bpjsClaimSubmissionEntity = bpjsClaimSubmissionService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Claim Submission. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsClaimSubmissionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsClaimSubmission' endpoint here] off begin
	@Operation(
			description = "Create a BpjsClaimSubmission if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsClaimSubmission."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsClaimSubmission, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsClaimSubmissionEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsClaimSubmission' endpoint here] end
	public ResponseEntity<?> saveBpjsClaimSubmission(
			@Parameter(description = "The BpjsClaimSubmissionEntity to create or update.")
			@RequestBody BpjsClaimSubmissionEntityDto bpjsClaimSubmissionEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsClaimSubmissionEntity bpjsClaimSubmissionEntity = bpjsClaimSubmissionService.create(new BpjsClaimSubmissionEntity(bpjsClaimSubmissionEntityDto));
			return new ResponseEntity<>(bpjsClaimSubmissionEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Claim Submission. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsClaimSubmissionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsClaimSubmission' endpoint here] off begin
	@Operation(
			description = "Update a BpjsClaimSubmission entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsClaimSubmission."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsClaimSubmission, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsClaimSubmissionEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsClaimSubmission' endpoint here] end
	public ResponseEntity<?> updateBpjsClaimSubmission(
			@Parameter(description = "The BpjsClaimSubmissionEntity to create or update.")
			@RequestBody BpjsClaimSubmissionEntityDto bpjsClaimSubmissionEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsClaimSubmissionEntity> checkBpjsClaimSubmissionExists = bpjsClaimSubmissionService.findById(bpjsClaimSubmissionEntityDto.getId());
		BpjsClaimSubmissionEntity bpjsClaimSubmissionEntity = new BpjsClaimSubmissionEntity(bpjsClaimSubmissionEntityDto);

		if (checkBpjsClaimSubmissionExists.isPresent()) {
			try {
				bpjsClaimSubmissionEntity = bpjsClaimSubmissionService.update(bpjsClaimSubmissionEntity);
				return new ResponseEntity<>(bpjsClaimSubmissionEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsClaimSubmissionEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Claim Submission that has the same id as the given id.
	 *
	 * @param id The id of the BpjsClaimSubmissionEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsClaimSubmission as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsClaimSubmission to delete.")
		...
		bpjsClaimSubmissionService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimSubmission' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Claim Submission entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimSubmission' endpoint here] end
	public void exportBpjsClaimSubmission(
			@Parameter(description = "The ids of the BpjsClaimSubmissionEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsClaimSubmissionEntity> entitiesToExport = bpjsClaimSubmissionService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsClaimSubmission_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsClaimSubmissionEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimSubmissionExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsClaimSubmissionExcludingIds' endpoint here] end
	public void exportBpjsClaimSubmissionExcludingIds(
			@Parameter(description = "The ids of the BpjsClaimSubmissionEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsClaimSubmissionEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsClaimSubmissionService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsClaimSubmissionService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsClaimSubmission entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsClaimSubmission_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsClaimSubmissionEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Claim Submission entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsClaimSubmissionEntity', 'create')  || hasPermission('BpjsClaimSubmissionEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsClaimSubmissionEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsClaimSubmissionEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsClaimSubmissionEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsClaimSubmissionEntity> csvToBean = new CsvToBeanBuilder<BpjsClaimSubmissionEntity>(reader)
				.withType(BpjsClaimSubmissionEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsClaimSubmissionEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsClaimSubmissionEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Claim Submission entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Claim Submission entity and you do not have permission");
					}
