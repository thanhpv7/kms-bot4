...
/**
 * Controller used to handle all REST operations regarding Delivery Medical Examination Record
 */
// % protected region % [Update the API route or description for the Delivery Medical Examination RecordController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DeliveryMedicalExaminationRecordController"))
@RestController
@Slf4j
@RequestMapping("/api/deliverymedicalexaminationrecord")
// % protected region % [Update the API route or description for the Delivery Medical Examination RecordController here] end
public class DeliveryMedicalExaminationRecordController {

	private final DeliveryMedicalExaminationRecordService deliveryMedicalExaminationRecordService;
	private final CsvUtils<DeliveryMedicalExaminationRecordEntity, DeliveryMedicalExaminationRecordRepository, DeliveryMedicalExaminationRecordEntityAudit,  DeliveryMedicalExaminationRecordService> csvUtils;
	...
	@Autowired
	public DeliveryMedicalExaminationRecordController(
		...
		this.csvUtils = new CsvUtils<>(deliveryMedicalExaminationRecordService, DeliveryMedicalExaminationRecordEntity.class);
	...
	/**
	 * Return all the Delivery Medical Examination Records.
	 *
	 * @return all the Delivery Medical Examination Records
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of DeliveryMedicalExaminationRecords",
		security = {
	...
	public ResponseEntity<List<DeliveryMedicalExaminationRecordEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DeliveryMedicalExaminationRecordEntity> deliveryMedicalExaminationRecords = deliveryMedicalExaminationRecordService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(deliveryMedicalExaminationRecords, HttpStatus.OK);
	}

	/**
	 * Return the Delivery Medical Examination Record that has the same id as the given id.
	 *
	 * @param id      The id of the DeliveryMedicalExaminationRecordEntity to be returned
	 * @return the Delivery Medical Examination Record that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DeliveryMedicalExaminationRecord as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DeliveryMedicalExaminationRecord entity not found"),
	})
	@PreAuthorize("hasPermission('DeliveryMedicalExaminationRecordEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DeliveryMedicalExaminationRecordEntity> getById(
			@Parameter(description = "The UUID of the DeliveryMedicalExaminationRecord to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DeliveryMedicalExaminationRecordEntity> deliveryMedicalExaminationRecordEntity = deliveryMedicalExaminationRecordService.findById(id);
	...
	/**
	 * Create/Update the given Delivery Medical Examination Record. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param deliveryMedicalExaminationRecordEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDeliveryMedicalExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Create a DeliveryMedicalExaminationRecord if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DeliveryMedicalExaminationRecord."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DeliveryMedicalExaminationRecord, duplicate record found")
	})
	@PreAuthorize("hasPermission('DeliveryMedicalExaminationRecordEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDeliveryMedicalExaminationRecord' endpoint here] end
	public ResponseEntity<?> saveDeliveryMedicalExaminationRecord(
			@Parameter(description = "The DeliveryMedicalExaminationRecordEntity to create or update.")
			@RequestBody DeliveryMedicalExaminationRecordEntityDto deliveryMedicalExaminationRecordEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DeliveryMedicalExaminationRecordEntity deliveryMedicalExaminationRecordEntity = deliveryMedicalExaminationRecordService.create(new DeliveryMedicalExaminationRecordEntity(deliveryMedicalExaminationRecordEntityDto));
			return new ResponseEntity<>(deliveryMedicalExaminationRecordEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Delivery Medical Examination Record. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param deliveryMedicalExaminationRecordEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDeliveryMedicalExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Update a DeliveryMedicalExaminationRecord entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DeliveryMedicalExaminationRecord."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DeliveryMedicalExaminationRecord, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DeliveryMedicalExaminationRecordEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDeliveryMedicalExaminationRecord' endpoint here] end
	public ResponseEntity<?> updateDeliveryMedicalExaminationRecord(
			@Parameter(description = "The DeliveryMedicalExaminationRecordEntity to create or update.")
			@RequestBody DeliveryMedicalExaminationRecordEntityDto deliveryMedicalExaminationRecordEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DeliveryMedicalExaminationRecordEntity> checkDeliveryMedicalExaminationRecordExists = deliveryMedicalExaminationRecordService.findById(deliveryMedicalExaminationRecordEntityDto.getId());
		DeliveryMedicalExaminationRecordEntity deliveryMedicalExaminationRecordEntity = new DeliveryMedicalExaminationRecordEntity(deliveryMedicalExaminationRecordEntityDto);

		if (checkDeliveryMedicalExaminationRecordExists.isPresent()) {
			try {
				deliveryMedicalExaminationRecordEntity = deliveryMedicalExaminationRecordService.update(deliveryMedicalExaminationRecordEntity);
				return new ResponseEntity<>(deliveryMedicalExaminationRecordEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(deliveryMedicalExaminationRecordEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Delivery Medical Examination Record that has the same id as the given id.
	 *
	 * @param id The id of the DeliveryMedicalExaminationRecordEntity to be deleted
	...
	@Operation(
			description = "Delete a single DeliveryMedicalExaminationRecord as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DeliveryMedicalExaminationRecord to delete.")
		...
		deliveryMedicalExaminationRecordService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDeliveryMedicalExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Delivery Medical Examination Record entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDeliveryMedicalExaminationRecord' endpoint here] end
	public void exportDeliveryMedicalExaminationRecord(
			@Parameter(description = "The ids of the DeliveryMedicalExaminationRecordEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DeliveryMedicalExaminationRecordEntity> entitiesToExport = deliveryMedicalExaminationRecordService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DeliveryMedicalExaminationRecord_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DeliveryMedicalExaminationRecordEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDeliveryMedicalExaminationRecordExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDeliveryMedicalExaminationRecordExcludingIds' endpoint here] end
	public void exportDeliveryMedicalExaminationRecordExcludingIds(
			@Parameter(description = "The ids of the DeliveryMedicalExaminationRecordEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DeliveryMedicalExaminationRecordEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = deliveryMedicalExaminationRecordService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = deliveryMedicalExaminationRecordService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DeliveryMedicalExaminationRecord entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DeliveryMedicalExaminationRecord_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DeliveryMedicalExaminationRecordEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Delivery Medical Examination Record entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DeliveryMedicalExaminationRecordEntity', 'create')  || hasPermission('DeliveryMedicalExaminationRecordEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DeliveryMedicalExaminationRecordEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DeliveryMedicalExaminationRecordEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DeliveryMedicalExaminationRecordEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DeliveryMedicalExaminationRecordEntity> csvToBean = new CsvToBeanBuilder<DeliveryMedicalExaminationRecordEntity>(reader)
				.withType(DeliveryMedicalExaminationRecordEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DeliveryMedicalExaminationRecordEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DeliveryMedicalExaminationRecordEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Delivery Medical Examination Record entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Delivery Medical Examination Record entity and you do not have permission");
					}
