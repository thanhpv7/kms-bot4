...
import kmsweb.entities.CashReceiptEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Cash Receipt
 */
// % protected region % [Update the API route or description for the Cash ReceiptController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CashReceiptController"))
@RestController
@Slf4j
@RequestMapping("/api/cashreceipt")
// % protected region % [Update the API route or description for the Cash ReceiptController here] end
public class CashReceiptController {

	private final CashReceiptService cashReceiptService;
	private final CsvUtils<CashReceiptEntity, CashReceiptRepository, CashReceiptEntityAudit,  CashReceiptService> csvUtils;
	...
	public CashReceiptController(
		...
		this.csvUtils = new CsvUtils<>(cashReceiptService, CashReceiptEntity.class);
	...
	public ResponseEntity<List<CashReceiptEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CashReceiptEntity> cashReceipts = cashReceiptService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(cashReceipts, HttpStatus.OK);
	}

	/**
	 * Return the Cash Receipt that has the same id as the given id.
	 *
	 * @param id      The id of the CashReceiptEntity to be returned
	 * @return the Cash Receipt that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CashReceipt as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CashReceipt entity not found"),
	})
	@PreAuthorize("hasPermission('CashReceiptEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CashReceiptEntity> getById(
			@Parameter(description = "The UUID of the CashReceipt to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CashReceiptEntity> cashReceiptEntity = cashReceiptService.findById(id);
	...
	/**
	 * Create/Update the given Cash Receipt. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param cashReceiptEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCashReceipt' endpoint here] off begin
	@Operation(
			description = "Create a CashReceipt if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CashReceipt."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CashReceipt, duplicate record found")
	})
	@PreAuthorize("hasPermission('CashReceiptEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCashReceipt' endpoint here] end
	public ResponseEntity<?> saveCashReceipt(
			@Parameter(description = "The CashReceiptEntity to create or update.")
			@RequestBody CashReceiptEntityDto cashReceiptEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CashReceiptEntity cashReceiptEntity = cashReceiptService.create(new CashReceiptEntity(cashReceiptEntityDto));
			return new ResponseEntity<>(cashReceiptEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Cash Receipt. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param cashReceiptEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCashReceipt' endpoint here] off begin
	@Operation(
			description = "Update a CashReceipt entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CashReceipt."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CashReceipt, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CashReceiptEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCashReceipt' endpoint here] end
	public ResponseEntity<?> updateCashReceipt(
			@Parameter(description = "The CashReceiptEntity to create or update.")
			@RequestBody CashReceiptEntityDto cashReceiptEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CashReceiptEntity> checkCashReceiptExists = cashReceiptService.findById(cashReceiptEntityDto.getId());
		CashReceiptEntity cashReceiptEntity = new CashReceiptEntity(cashReceiptEntityDto);

		if (checkCashReceiptExists.isPresent()) {
			try {
				cashReceiptEntity = cashReceiptService.update(cashReceiptEntity);
				return new ResponseEntity<>(cashReceiptEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(cashReceiptEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Cash Receipt that has the same id as the given id.
	 *
	 * @param id The id of the CashReceiptEntity to be deleted
	...
	@Operation(
			description = "Delete a single CashReceipt as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CashReceipt to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashReceipt' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Cash Receipt entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCashReceipt' endpoint here] end
	public void exportCashReceipt(
			@Parameter(description = "The ids of the CashReceiptEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CashReceiptEntity> entitiesToExport = cashReceiptService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CashReceipt_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CashReceiptEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCashReceiptExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCashReceiptExcludingIds' endpoint here] end
	public void exportCashReceiptExcludingIds(
			@Parameter(description = "The ids of the CashReceiptEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CashReceiptEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = cashReceiptService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = cashReceiptService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CashReceipt entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CashReceipt_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CashReceiptEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Cash Receipt entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CashReceiptEntity', 'create')  || hasPermission('CashReceiptEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CashReceiptEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CashReceiptEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CashReceiptEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CashReceiptEntity> csvToBean = new CsvToBeanBuilder<CashReceiptEntity>(reader)
				.withType(CashReceiptEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CashReceiptEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CashReceiptEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Cash Receipt entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Cash Receipt entity and you do not have permission");
					}
