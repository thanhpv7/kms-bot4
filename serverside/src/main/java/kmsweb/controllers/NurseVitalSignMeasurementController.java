...
/**
 * Controller used to handle all REST operations regarding Nurse Vital Sign Measurement
 */
// % protected region % [Update the API route or description for the Nurse Vital Sign MeasurementController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NurseVitalSignMeasurementController"))
@RestController
@Slf4j
@RequestMapping("/api/nursevitalsignmeasurement")
// % protected region % [Update the API route or description for the Nurse Vital Sign MeasurementController here] end
public class NurseVitalSignMeasurementController {

	private final NurseVitalSignMeasurementService nurseVitalSignMeasurementService;
	private final CsvUtils<NurseVitalSignMeasurementEntity, NurseVitalSignMeasurementRepository, NurseVitalSignMeasurementEntityAudit,  NurseVitalSignMeasurementService> csvUtils;
	...
	@Autowired
	public NurseVitalSignMeasurementController(
		...
		this.csvUtils = new CsvUtils<>(nurseVitalSignMeasurementService, NurseVitalSignMeasurementEntity.class);
	...
	/**
	 * Return all the Nurse Vital Sign Measurements.
	 *
	 * @return all the Nurse Vital Sign Measurements
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of NurseVitalSignMeasurements",
		security = {
	...
	public ResponseEntity<List<NurseVitalSignMeasurementEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NurseVitalSignMeasurementEntity> nurseVitalSignMeasurements = nurseVitalSignMeasurementService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nurseVitalSignMeasurements, HttpStatus.OK);
	}

	/**
	 * Return the Nurse Vital Sign Measurement that has the same id as the given id.
	 *
	 * @param id      The id of the NurseVitalSignMeasurementEntity to be returned
	 * @return the Nurse Vital Sign Measurement that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NurseVitalSignMeasurement as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NurseVitalSignMeasurement entity not found"),
	})
	@PreAuthorize("hasPermission('NurseVitalSignMeasurementEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NurseVitalSignMeasurementEntity> getById(
			@Parameter(description = "The UUID of the NurseVitalSignMeasurement to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NurseVitalSignMeasurementEntity> nurseVitalSignMeasurementEntity = nurseVitalSignMeasurementService.findById(id);
	...
	/**
	 * Create/Update the given Nurse Vital Sign Measurement. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nurseVitalSignMeasurementEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNurseVitalSignMeasurement' endpoint here] off begin
	@Operation(
			description = "Create a NurseVitalSignMeasurement if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NurseVitalSignMeasurement."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NurseVitalSignMeasurement, duplicate record found")
	})
	@PreAuthorize("hasPermission('NurseVitalSignMeasurementEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNurseVitalSignMeasurement' endpoint here] end
	public ResponseEntity<?> saveNurseVitalSignMeasurement(
			@Parameter(description = "The NurseVitalSignMeasurementEntity to create or update.")
			@RequestBody NurseVitalSignMeasurementEntityDto nurseVitalSignMeasurementEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NurseVitalSignMeasurementEntity nurseVitalSignMeasurementEntity = nurseVitalSignMeasurementService.create(new NurseVitalSignMeasurementEntity(nurseVitalSignMeasurementEntityDto));
			return new ResponseEntity<>(nurseVitalSignMeasurementEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Nurse Vital Sign Measurement. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nurseVitalSignMeasurementEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNurseVitalSignMeasurement' endpoint here] off begin
	@Operation(
			description = "Update a NurseVitalSignMeasurement entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NurseVitalSignMeasurement."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NurseVitalSignMeasurement, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NurseVitalSignMeasurementEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNurseVitalSignMeasurement' endpoint here] end
	public ResponseEntity<?> updateNurseVitalSignMeasurement(
			@Parameter(description = "The NurseVitalSignMeasurementEntity to create or update.")
			@RequestBody NurseVitalSignMeasurementEntityDto nurseVitalSignMeasurementEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NurseVitalSignMeasurementEntity> checkNurseVitalSignMeasurementExists = nurseVitalSignMeasurementService.findById(nurseVitalSignMeasurementEntityDto.getId());
		NurseVitalSignMeasurementEntity nurseVitalSignMeasurementEntity = new NurseVitalSignMeasurementEntity(nurseVitalSignMeasurementEntityDto);

		if (checkNurseVitalSignMeasurementExists.isPresent()) {
			try {
				nurseVitalSignMeasurementEntity = nurseVitalSignMeasurementService.update(nurseVitalSignMeasurementEntity);
				return new ResponseEntity<>(nurseVitalSignMeasurementEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nurseVitalSignMeasurementEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Nurse Vital Sign Measurement that has the same id as the given id.
	 *
	 * @param id The id of the NurseVitalSignMeasurementEntity to be deleted
	...
	@Operation(
			description = "Delete a single NurseVitalSignMeasurement as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NurseVitalSignMeasurement to delete.")
		...
		nurseVitalSignMeasurementService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNurseVitalSignMeasurement' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Nurse Vital Sign Measurement entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNurseVitalSignMeasurement' endpoint here] end
	public void exportNurseVitalSignMeasurement(
			@Parameter(description = "The ids of the NurseVitalSignMeasurementEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NurseVitalSignMeasurementEntity> entitiesToExport = nurseVitalSignMeasurementService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NurseVitalSignMeasurement_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NurseVitalSignMeasurementEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNurseVitalSignMeasurementExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNurseVitalSignMeasurementExcludingIds' endpoint here] end
	public void exportNurseVitalSignMeasurementExcludingIds(
			@Parameter(description = "The ids of the NurseVitalSignMeasurementEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NurseVitalSignMeasurementEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nurseVitalSignMeasurementService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nurseVitalSignMeasurementService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NurseVitalSignMeasurement entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NurseVitalSignMeasurement_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NurseVitalSignMeasurementEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Nurse Vital Sign Measurement entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NurseVitalSignMeasurementEntity', 'create')  || hasPermission('NurseVitalSignMeasurementEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NurseVitalSignMeasurementEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NurseVitalSignMeasurementEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NurseVitalSignMeasurementEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NurseVitalSignMeasurementEntity> csvToBean = new CsvToBeanBuilder<NurseVitalSignMeasurementEntity>(reader)
				.withType(NurseVitalSignMeasurementEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NurseVitalSignMeasurementEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NurseVitalSignMeasurementEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Nurse Vital Sign Measurement entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Nurse Vital Sign Measurement entity and you do not have permission");
					}
