...
import kmsweb.entities.BpjsPcareKesadaranEntityAudit;
import kmsweb.repositories.BpjsPcareKesadaranRepository;
import kmsweb.services.BpjsPcareKesadaranService;
...
/**
 * Controller used to handle all REST operations regarding BPJS Pcare Kesadaran
 */
// % protected region % [Update the API route or description for the BPJS Pcare KesadaranController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPcareKesadaranController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspcarekesadaran")
// % protected region % [Update the API route or description for the BPJS Pcare KesadaranController here] end
public class BpjsPcareKesadaranController {

	private final BpjsPcareKesadaranService bpjsPcareKesadaranService;
	private final CsvUtils<BpjsPcareKesadaranEntity, BpjsPcareKesadaranRepository, BpjsPcareKesadaranEntityAudit,  BpjsPcareKesadaranService> csvUtils;
	...
	@Autowired
	public BpjsPcareKesadaranController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsPcareKesadaranService bpjsPcareKesadaranService
	) {
		this.authService = authService;
		this.bpjsPcareKesadaranService = bpjsPcareKesadaranService;
		this.csvUtils = new CsvUtils<>(bpjsPcareKesadaranService, BpjsPcareKesadaranEntity.class);
	...
	public ResponseEntity<List<BpjsPcareKesadaranEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPcareKesadaranEntity> bpjsPcareKesadarans = bpjsPcareKesadaranService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPcareKesadarans, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Pcare Kesadaran that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPcareKesadaranEntity to be returned
	 * @return the BPJS Pcare Kesadaran that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPcareKesadaran as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPcareKesadaran entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPcareKesadaranEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPcareKesadaranEntity> getById(
			@Parameter(description = "The UUID of the BpjsPcareKesadaran to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPcareKesadaranEntity> bpjsPcareKesadaranEntity = bpjsPcareKesadaranService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Pcare Kesadaran. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareKesadaranEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareKesadaran' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPcareKesadaran if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPcareKesadaran."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPcareKesadaran, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPcareKesadaranEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPcareKesadaran' endpoint here] end
	public ResponseEntity<?> saveBpjsPcareKesadaran(
			@Parameter(description = "The BpjsPcareKesadaranEntity to create or update.")
			@RequestBody BpjsPcareKesadaranEntityDto bpjsPcareKesadaranEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPcareKesadaranEntity bpjsPcareKesadaranEntity = bpjsPcareKesadaranService.create(new BpjsPcareKesadaranEntity(bpjsPcareKesadaranEntityDto));
			return new ResponseEntity<>(bpjsPcareKesadaranEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Pcare Kesadaran. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPcareKesadaranEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareKesadaran' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPcareKesadaran entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPcareKesadaran."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPcareKesadaran, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPcareKesadaranEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPcareKesadaran' endpoint here] end
	public ResponseEntity<?> updateBpjsPcareKesadaran(
			@Parameter(description = "The BpjsPcareKesadaranEntity to create or update.")
			@RequestBody BpjsPcareKesadaranEntityDto bpjsPcareKesadaranEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPcareKesadaranEntity> checkBpjsPcareKesadaranExists = bpjsPcareKesadaranService.findById(bpjsPcareKesadaranEntityDto.getId());
		BpjsPcareKesadaranEntity bpjsPcareKesadaranEntity = new BpjsPcareKesadaranEntity(bpjsPcareKesadaranEntityDto);

		if (checkBpjsPcareKesadaranExists.isPresent()) {
			try {
				bpjsPcareKesadaranEntity = bpjsPcareKesadaranService.update(bpjsPcareKesadaranEntity);
				return new ResponseEntity<>(bpjsPcareKesadaranEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPcareKesadaranEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Pcare Kesadaran that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPcareKesadaranEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPcareKesadaran as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPcareKesadaran to delete.")
		...
		bpjsPcareKesadaranService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKesadaran' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Pcare Kesadaran entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKesadaran' endpoint here] end
	public void exportBpjsPcareKesadaran(
			@Parameter(description = "The ids of the BpjsPcareKesadaranEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPcareKesadaranEntity> entitiesToExport = bpjsPcareKesadaranService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPcareKesadaran_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPcareKesadaranEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKesadaranExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPcareKesadaranExcludingIds' endpoint here] end
	public void exportBpjsPcareKesadaranExcludingIds(
			@Parameter(description = "The ids of the BpjsPcareKesadaranEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPcareKesadaranEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPcareKesadaranService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPcareKesadaranService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPcareKesadaran entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPcareKesadaran_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPcareKesadaranEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Pcare Kesadaran entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPcareKesadaranEntity', 'create')  || hasPermission('BpjsPcareKesadaranEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPcareKesadaranEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPcareKesadaranEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPcareKesadaranEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPcareKesadaranEntity> csvToBean = new CsvToBeanBuilder<BpjsPcareKesadaranEntity>(reader)
				.withType(BpjsPcareKesadaranEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPcareKesadaranEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPcareKesadaranEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Pcare Kesadaran entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Pcare Kesadaran entity and you do not have permission");
					}
