...
import kmsweb.entities.PatientPersonalInfoEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Personal Info
 */
// % protected region % [Update the API route or description for the Patient Personal InfoController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientPersonalInfoController"))
@RestController
@Slf4j
@RequestMapping("/api/patientpersonalinfo")
// % protected region % [Update the API route or description for the Patient Personal InfoController here] end
public class PatientPersonalInfoController {

	private final PatientPersonalInfoService patientPersonalInfoService;
	private final CsvUtils<PatientPersonalInfoEntity, PatientPersonalInfoRepository, PatientPersonalInfoEntityAudit,  PatientPersonalInfoService> csvUtils;
	...
	@Autowired
	public PatientPersonalInfoController(
		...
		this.csvUtils = new CsvUtils<>(patientPersonalInfoService, PatientPersonalInfoEntity.class);
	...
	public ResponseEntity<List<PatientPersonalInfoEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientPersonalInfoEntity> patientPersonalInfos = patientPersonalInfoService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientPersonalInfos, HttpStatus.OK);
	}

	/**
	 * Return the Patient Personal Info that has the same id as the given id.
	 *
	 * @param id      The id of the PatientPersonalInfoEntity to be returned
	 * @return the Patient Personal Info that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientPersonalInfo as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientPersonalInfo entity not found"),
	})
	@PreAuthorize("hasPermission('PatientPersonalInfoEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientPersonalInfoEntity> getById(
			@Parameter(description = "The UUID of the PatientPersonalInfo to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientPersonalInfoEntity> patientPersonalInfoEntity = patientPersonalInfoService.findById(id);
	...
	/**
	 * Create/Update the given Patient Personal Info. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientPersonalInfoEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientPersonalInfo' endpoint here] off begin
	@Operation(
			description = "Create a PatientPersonalInfo if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientPersonalInfo."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientPersonalInfo, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientPersonalInfoEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientPersonalInfo' endpoint here] end
	public ResponseEntity<?> savePatientPersonalInfo(
			@Parameter(description = "The PatientPersonalInfoEntity to create or update.")
			@RequestBody PatientPersonalInfoEntityDto patientPersonalInfoEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientPersonalInfoEntity patientPersonalInfoEntity = patientPersonalInfoService.create(new PatientPersonalInfoEntity(patientPersonalInfoEntityDto));
			return new ResponseEntity<>(patientPersonalInfoEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Personal Info. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientPersonalInfoEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientPersonalInfo' endpoint here] off begin
	@Operation(
			description = "Update a PatientPersonalInfo entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientPersonalInfo."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientPersonalInfo, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientPersonalInfoEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientPersonalInfo' endpoint here] end
	public ResponseEntity<?> updatePatientPersonalInfo(
			@Parameter(description = "The PatientPersonalInfoEntity to create or update.")
			@RequestBody PatientPersonalInfoEntityDto patientPersonalInfoEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientPersonalInfoEntity> checkPatientPersonalInfoExists = patientPersonalInfoService.findById(patientPersonalInfoEntityDto.getId());
		PatientPersonalInfoEntity patientPersonalInfoEntity = new PatientPersonalInfoEntity(patientPersonalInfoEntityDto);

		if (checkPatientPersonalInfoExists.isPresent()) {
			try {
				patientPersonalInfoEntity = patientPersonalInfoService.update(patientPersonalInfoEntity);
				return new ResponseEntity<>(patientPersonalInfoEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientPersonalInfoEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Personal Info that has the same id as the given id.
	 *
	 * @param id The id of the PatientPersonalInfoEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientPersonalInfo as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientPersonalInfo to delete.")
		...
		patientPersonalInfoService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPersonalInfo' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Personal Info entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPersonalInfo' endpoint here] end
	public void exportPatientPersonalInfo(
			@Parameter(description = "The ids of the PatientPersonalInfoEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientPersonalInfoEntity> entitiesToExport = patientPersonalInfoService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientPersonalInfo_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientPersonalInfoEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPersonalInfoExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPersonalInfoExcludingIds' endpoint here] end
	public void exportPatientPersonalInfoExcludingIds(
			@Parameter(description = "The ids of the PatientPersonalInfoEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientPersonalInfoEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientPersonalInfoService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientPersonalInfoService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientPersonalInfo entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientPersonalInfo_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientPersonalInfoEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Personal Info entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientPersonalInfoEntity', 'create')  || hasPermission('PatientPersonalInfoEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientPersonalInfoEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientPersonalInfoEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientPersonalInfoEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientPersonalInfoEntity> csvToBean = new CsvToBeanBuilder<PatientPersonalInfoEntity>(reader)
				.withType(PatientPersonalInfoEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientPersonalInfoEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientPersonalInfoEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Personal Info entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Personal Info entity and you do not have permission");
					}
