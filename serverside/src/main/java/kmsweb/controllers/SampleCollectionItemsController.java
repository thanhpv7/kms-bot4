...
import kmsweb.entities.SampleCollectionItemsEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Sample Collection Items
 */
// % protected region % [Update the API route or description for the Sample Collection ItemsController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SampleCollectionItemsController"))
@RestController
@Slf4j
@RequestMapping("/api/samplecollectionitems")
// % protected region % [Update the API route or description for the Sample Collection ItemsController here] end
public class SampleCollectionItemsController {

	private final SampleCollectionItemsService sampleCollectionItemsService;
	private final CsvUtils<SampleCollectionItemsEntity, SampleCollectionItemsRepository, SampleCollectionItemsEntityAudit,  SampleCollectionItemsService> csvUtils;
	...
	@Autowired
	public SampleCollectionItemsController(
		...
		this.csvUtils = new CsvUtils<>(sampleCollectionItemsService, SampleCollectionItemsEntity.class);
	...
	public ResponseEntity<List<SampleCollectionItemsEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SampleCollectionItemsEntity> sampleCollectionItemss = sampleCollectionItemsService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(sampleCollectionItemss, HttpStatus.OK);
	}

	/**
	 * Return the Sample Collection Items that has the same id as the given id.
	 *
	 * @param id      The id of the SampleCollectionItemsEntity to be returned
	 * @return the Sample Collection Items that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single SampleCollectionItems as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "SampleCollectionItems entity not found"),
	})
	@PreAuthorize("hasPermission('SampleCollectionItemsEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SampleCollectionItemsEntity> getById(
			@Parameter(description = "The UUID of the SampleCollectionItems to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SampleCollectionItemsEntity> sampleCollectionItemsEntity = sampleCollectionItemsService.findById(id);
	...
	/**
	 * Create/Update the given Sample Collection Items. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param sampleCollectionItemsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSampleCollectionItems' endpoint here] off begin
	@Operation(
			description = "Create a SampleCollectionItems if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new SampleCollectionItems."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new SampleCollectionItems, duplicate record found")
	})
	@PreAuthorize("hasPermission('SampleCollectionItemsEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSampleCollectionItems' endpoint here] end
	public ResponseEntity<?> saveSampleCollectionItems(
			@Parameter(description = "The SampleCollectionItemsEntity to create or update.")
			@RequestBody SampleCollectionItemsEntityDto sampleCollectionItemsEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SampleCollectionItemsEntity sampleCollectionItemsEntity = sampleCollectionItemsService.create(new SampleCollectionItemsEntity(sampleCollectionItemsEntityDto));
			return new ResponseEntity<>(sampleCollectionItemsEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Sample Collection Items. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param sampleCollectionItemsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSampleCollectionItems' endpoint here] off begin
	@Operation(
			description = "Update a SampleCollectionItems entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the SampleCollectionItems."),
		@ApiResponse(responseCode = "400", description = "Failed to update the SampleCollectionItems, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SampleCollectionItemsEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSampleCollectionItems' endpoint here] end
	public ResponseEntity<?> updateSampleCollectionItems(
			@Parameter(description = "The SampleCollectionItemsEntity to create or update.")
			@RequestBody SampleCollectionItemsEntityDto sampleCollectionItemsEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SampleCollectionItemsEntity> checkSampleCollectionItemsExists = sampleCollectionItemsService.findById(sampleCollectionItemsEntityDto.getId());
		SampleCollectionItemsEntity sampleCollectionItemsEntity = new SampleCollectionItemsEntity(sampleCollectionItemsEntityDto);

		if (checkSampleCollectionItemsExists.isPresent()) {
			try {
				sampleCollectionItemsEntity = sampleCollectionItemsService.update(sampleCollectionItemsEntity);
				return new ResponseEntity<>(sampleCollectionItemsEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(sampleCollectionItemsEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Sample Collection Items that has the same id as the given id.
	 *
	 * @param id The id of the SampleCollectionItemsEntity to be deleted
	...
	@Operation(
			description = "Delete a single SampleCollectionItems as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the SampleCollectionItems to delete.")
		...
		sampleCollectionItemsService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionItems' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Sample Collection Items entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionItems' endpoint here] end
	public void exportSampleCollectionItems(
			@Parameter(description = "The ids of the SampleCollectionItemsEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SampleCollectionItemsEntity> entitiesToExport = sampleCollectionItemsService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "SampleCollectionItems_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SampleCollectionItemsEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionItemsExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionItemsExcludingIds' endpoint here] end
	public void exportSampleCollectionItemsExcludingIds(
			@Parameter(description = "The ids of the SampleCollectionItemsEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SampleCollectionItemsEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = sampleCollectionItemsService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = sampleCollectionItemsService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the SampleCollectionItems entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "SampleCollectionItems_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SampleCollectionItemsEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Sample Collection Items entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SampleCollectionItemsEntity', 'create')  || hasPermission('SampleCollectionItemsEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SampleCollectionItemsEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SampleCollectionItemsEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SampleCollectionItemsEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SampleCollectionItemsEntity> csvToBean = new CsvToBeanBuilder<SampleCollectionItemsEntity>(reader)
				.withType(SampleCollectionItemsEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<SampleCollectionItemsEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<SampleCollectionItemsEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Sample Collection Items entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Sample Collection Items entity and you do not have permission");
					}
