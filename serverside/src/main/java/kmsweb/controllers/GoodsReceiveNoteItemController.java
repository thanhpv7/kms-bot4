...
import kmsweb.entities.GoodsReceiveNoteItemEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Goods Receive Note Item
 */
// % protected region % [Update the API route or description for the Goods Receive Note ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "GoodsReceiveNoteItemController"))
@RestController
@Slf4j
@RequestMapping("/api/goodsreceivenoteitem")
// % protected region % [Update the API route or description for the Goods Receive Note ItemController here] end
public class GoodsReceiveNoteItemController {

	private final GoodsReceiveNoteItemService goodsReceiveNoteItemService;
	private final CsvUtils<GoodsReceiveNoteItemEntity, GoodsReceiveNoteItemRepository, GoodsReceiveNoteItemEntityAudit,  GoodsReceiveNoteItemService> csvUtils;
	...
	@Autowired
	public GoodsReceiveNoteItemController(
		...
		this.csvUtils = new CsvUtils<>(goodsReceiveNoteItemService, GoodsReceiveNoteItemEntity.class);
	...
	public ResponseEntity<List<GoodsReceiveNoteItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<GoodsReceiveNoteItemEntity> goodsReceiveNoteItems = goodsReceiveNoteItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(goodsReceiveNoteItems, HttpStatus.OK);
	}

	/**
	 * Return the Goods Receive Note Item that has the same id as the given id.
	 *
	 * @param id      The id of the GoodsReceiveNoteItemEntity to be returned
	 * @return the Goods Receive Note Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single GoodsReceiveNoteItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "GoodsReceiveNoteItem entity not found"),
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<GoodsReceiveNoteItemEntity> getById(
			@Parameter(description = "The UUID of the GoodsReceiveNoteItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<GoodsReceiveNoteItemEntity> goodsReceiveNoteItemEntity = goodsReceiveNoteItemService.findById(id);
	...
	/**
	 * Create/Update the given Goods Receive Note Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param goodsReceiveNoteItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveGoodsReceiveNoteItem' endpoint here] off begin
	@Operation(
			description = "Create a GoodsReceiveNoteItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new GoodsReceiveNoteItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new GoodsReceiveNoteItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveGoodsReceiveNoteItem' endpoint here] end
	public ResponseEntity<?> saveGoodsReceiveNoteItem(
			@Parameter(description = "The GoodsReceiveNoteItemEntity to create or update.")
			@RequestBody GoodsReceiveNoteItemEntityDto goodsReceiveNoteItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			GoodsReceiveNoteItemEntity goodsReceiveNoteItemEntity = goodsReceiveNoteItemService.create(new GoodsReceiveNoteItemEntity(goodsReceiveNoteItemEntityDto));
			return new ResponseEntity<>(goodsReceiveNoteItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Goods Receive Note Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param goodsReceiveNoteItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateGoodsReceiveNoteItem' endpoint here] off begin
	@Operation(
			description = "Update a GoodsReceiveNoteItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the GoodsReceiveNoteItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the GoodsReceiveNoteItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateGoodsReceiveNoteItem' endpoint here] end
	public ResponseEntity<?> updateGoodsReceiveNoteItem(
			@Parameter(description = "The GoodsReceiveNoteItemEntity to create or update.")
			@RequestBody GoodsReceiveNoteItemEntityDto goodsReceiveNoteItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<GoodsReceiveNoteItemEntity> checkGoodsReceiveNoteItemExists = goodsReceiveNoteItemService.findById(goodsReceiveNoteItemEntityDto.getId());
		GoodsReceiveNoteItemEntity goodsReceiveNoteItemEntity = new GoodsReceiveNoteItemEntity(goodsReceiveNoteItemEntityDto);

		if (checkGoodsReceiveNoteItemExists.isPresent()) {
			try {
				goodsReceiveNoteItemEntity = goodsReceiveNoteItemService.update(goodsReceiveNoteItemEntity);
				return new ResponseEntity<>(goodsReceiveNoteItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(goodsReceiveNoteItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Goods Receive Note Item that has the same id as the given id.
	 *
	 * @param id The id of the GoodsReceiveNoteItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single GoodsReceiveNoteItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the GoodsReceiveNoteItem to delete.")
		...
		goodsReceiveNoteItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Goods Receive Note Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteItem' endpoint here] end
	public void exportGoodsReceiveNoteItem(
			@Parameter(description = "The ids of the GoodsReceiveNoteItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<GoodsReceiveNoteItemEntity> entitiesToExport = goodsReceiveNoteItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "GoodsReceiveNoteItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (GoodsReceiveNoteItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteItemExcludingIds' endpoint here] end
	public void exportGoodsReceiveNoteItemExcludingIds(
			@Parameter(description = "The ids of the GoodsReceiveNoteItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<GoodsReceiveNoteItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = goodsReceiveNoteItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = goodsReceiveNoteItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the GoodsReceiveNoteItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "GoodsReceiveNoteItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(GoodsReceiveNoteItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Goods Receive Note Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('GoodsReceiveNoteItemEntity', 'create')  || hasPermission('GoodsReceiveNoteItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with GoodsReceiveNoteItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("GoodsReceiveNoteItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(GoodsReceiveNoteItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<GoodsReceiveNoteItemEntity> csvToBean = new CsvToBeanBuilder<GoodsReceiveNoteItemEntity>(reader)
				.withType(GoodsReceiveNoteItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<GoodsReceiveNoteItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<GoodsReceiveNoteItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Goods Receive Note Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Goods Receive Note Item entity and you do not have permission");
					}
