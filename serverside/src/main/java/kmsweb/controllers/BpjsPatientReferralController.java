...
import kmsweb.entities.BpjsPatientReferralEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Patient Referral
 */
// % protected region % [Update the API route or description for the BPJS Patient ReferralController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPatientReferralController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjspatientreferral")
// % protected region % [Update the API route or description for the BPJS Patient ReferralController here] end
public class BpjsPatientReferralController {

	private final BpjsPatientReferralService bpjsPatientReferralService;
	private final CsvUtils<BpjsPatientReferralEntity, BpjsPatientReferralRepository, BpjsPatientReferralEntityAudit,  BpjsPatientReferralService> csvUtils;
	...
	@Autowired
	public BpjsPatientReferralController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPatientReferralService, BpjsPatientReferralEntity.class);
	...
	public ResponseEntity<List<BpjsPatientReferralEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPatientReferralEntity> bpjsPatientReferrals = bpjsPatientReferralService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPatientReferrals, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Patient Referral that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPatientReferralEntity to be returned
	 * @return the BPJS Patient Referral that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPatientReferral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPatientReferral entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPatientReferralEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPatientReferralEntity> getById(
			@Parameter(description = "The UUID of the BpjsPatientReferral to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPatientReferralEntity> bpjsPatientReferralEntity = bpjsPatientReferralService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Patient Referral. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPatientReferralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPatientReferral' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPatientReferral if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPatientReferral."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPatientReferral, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPatientReferralEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPatientReferral' endpoint here] end
	public ResponseEntity<?> saveBpjsPatientReferral(
			@Parameter(description = "The BpjsPatientReferralEntity to create or update.")
			@RequestBody BpjsPatientReferralEntityDto bpjsPatientReferralEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPatientReferralEntity bpjsPatientReferralEntity = bpjsPatientReferralService.create(new BpjsPatientReferralEntity(bpjsPatientReferralEntityDto));
			return new ResponseEntity<>(bpjsPatientReferralEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Patient Referral. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPatientReferralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPatientReferral' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPatientReferral entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPatientReferral."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPatientReferral, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPatientReferralEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPatientReferral' endpoint here] end
	public ResponseEntity<?> updateBpjsPatientReferral(
			@Parameter(description = "The BpjsPatientReferralEntity to create or update.")
			@RequestBody BpjsPatientReferralEntityDto bpjsPatientReferralEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPatientReferralEntity> checkBpjsPatientReferralExists = bpjsPatientReferralService.findById(bpjsPatientReferralEntityDto.getId());
		BpjsPatientReferralEntity bpjsPatientReferralEntity = new BpjsPatientReferralEntity(bpjsPatientReferralEntityDto);

		if (checkBpjsPatientReferralExists.isPresent()) {
			try {
				bpjsPatientReferralEntity = bpjsPatientReferralService.update(bpjsPatientReferralEntity);
				return new ResponseEntity<>(bpjsPatientReferralEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPatientReferralEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Patient Referral that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPatientReferralEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPatientReferral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPatientReferral to delete.")
		...
		bpjsPatientReferralService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPatientReferral' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Patient Referral entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPatientReferral' endpoint here] end
	public void exportBpjsPatientReferral(
			@Parameter(description = "The ids of the BpjsPatientReferralEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPatientReferralEntity> entitiesToExport = bpjsPatientReferralService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPatientReferral_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPatientReferralEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPatientReferralExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPatientReferralExcludingIds' endpoint here] end
	public void exportBpjsPatientReferralExcludingIds(
			@Parameter(description = "The ids of the BpjsPatientReferralEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPatientReferralEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPatientReferralService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPatientReferralService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPatientReferral entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPatientReferral_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPatientReferralEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Patient Referral entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPatientReferralEntity', 'create')  || hasPermission('BpjsPatientReferralEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPatientReferralEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPatientReferralEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPatientReferralEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPatientReferralEntity> csvToBean = new CsvToBeanBuilder<BpjsPatientReferralEntity>(reader)
				.withType(BpjsPatientReferralEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPatientReferralEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPatientReferralEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Patient Referral entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Patient Referral entity and you do not have permission");
					}
