...
import kmsweb.entities.PriceComponentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Price Component
 */
// % protected region % [Update the API route or description for the Price ComponentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PriceComponentController"))
@RestController
@Slf4j
@RequestMapping("/api/pricecomponent")
// % protected region % [Update the API route or description for the Price ComponentController here] end
public class PriceComponentController {

	private final PriceComponentService priceComponentService;
	private final CsvUtils<PriceComponentEntity, PriceComponentRepository, PriceComponentEntityAudit,  PriceComponentService> csvUtils;
	...
	@Autowired
	public PriceComponentController(
		...
		this.csvUtils = new CsvUtils<>(priceComponentService, PriceComponentEntity.class);
	...
	public ResponseEntity<List<PriceComponentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PriceComponentEntity> priceComponents = priceComponentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(priceComponents, HttpStatus.OK);
	}

	/**
	 * Return the Price Component that has the same id as the given id.
	 *
	 * @param id      The id of the PriceComponentEntity to be returned
	 * @return the Price Component that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PriceComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PriceComponent entity not found"),
	})
	@PreAuthorize("hasPermission('PriceComponentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PriceComponentEntity> getById(
			@Parameter(description = "The UUID of the PriceComponent to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PriceComponentEntity> priceComponentEntity = priceComponentService.findById(id);
	...
	/**
	 * Create/Update the given Price Component. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param priceComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePriceComponent' endpoint here] off begin
	@Operation(
			description = "Create a PriceComponent if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PriceComponent."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PriceComponent, duplicate record found")
	})
	@PreAuthorize("hasPermission('PriceComponentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePriceComponent' endpoint here] end
	public ResponseEntity<?> savePriceComponent(
			@Parameter(description = "The PriceComponentEntity to create or update.")
			@RequestBody PriceComponentEntityDto priceComponentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PriceComponentEntity priceComponentEntity = priceComponentService.create(new PriceComponentEntity(priceComponentEntityDto));
			return new ResponseEntity<>(priceComponentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Price Component. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param priceComponentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePriceComponent' endpoint here] off begin
	@Operation(
			description = "Update a PriceComponent entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PriceComponent."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PriceComponent, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PriceComponentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePriceComponent' endpoint here] end
	public ResponseEntity<?> updatePriceComponent(
			@Parameter(description = "The PriceComponentEntity to create or update.")
			@RequestBody PriceComponentEntityDto priceComponentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PriceComponentEntity> checkPriceComponentExists = priceComponentService.findById(priceComponentEntityDto.getId());
		PriceComponentEntity priceComponentEntity = new PriceComponentEntity(priceComponentEntityDto);

		if (checkPriceComponentExists.isPresent()) {
			try {
				priceComponentEntity = priceComponentService.update(priceComponentEntity);
				return new ResponseEntity<>(priceComponentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(priceComponentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Price Component that has the same id as the given id.
	 *
	 * @param id The id of the PriceComponentEntity to be deleted
	...
	@Operation(
			description = "Delete a single PriceComponent as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PriceComponent to delete.")
		...
		priceComponentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPriceComponent' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Price Component entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPriceComponent' endpoint here] end
	public void exportPriceComponent(
			@Parameter(description = "The ids of the PriceComponentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PriceComponentEntity> entitiesToExport = priceComponentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PriceComponent_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PriceComponentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPriceComponentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPriceComponentExcludingIds' endpoint here] end
	public void exportPriceComponentExcludingIds(
			@Parameter(description = "The ids of the PriceComponentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PriceComponentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = priceComponentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = priceComponentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PriceComponent entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PriceComponent_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PriceComponentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Price Component entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PriceComponentEntity', 'create')  || hasPermission('PriceComponentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PriceComponentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PriceComponentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PriceComponentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PriceComponentEntity> csvToBean = new CsvToBeanBuilder<PriceComponentEntity>(reader)
				.withType(PriceComponentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PriceComponentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PriceComponentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Price Component entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Price Component entity and you do not have permission");
					}
