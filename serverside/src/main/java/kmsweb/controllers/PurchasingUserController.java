...
import kmsweb.dtos.PurchasingUserEntityDto;
import kmsweb.entities.PurchasingUserEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Purchasing User
 */
// % protected region % [Update the API route or description for the Purchasing UserController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PurchasingUserController"))
@RestController
@Slf4j
@RequestMapping("/api/purchasinguser")
// % protected region % [Update the API route or description for the Purchasing UserController here] end
public class PurchasingUserController {

	private final PurchasingUserService purchasingUserService;
	private final CsvUtils<PurchasingUserEntity, PurchasingUserRepository, PurchasingUserEntityAudit,  PurchasingUserService> csvUtils;
	...
	@Autowired
	public PurchasingUserController(
		...
		this.csvUtils = new CsvUtils<>(purchasingUserService, PurchasingUserEntity.class);
	...
	public ResponseEntity<List<PurchasingUserEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PurchasingUserEntity> purchasingUsers = purchasingUserService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(purchasingUsers, HttpStatus.OK);
	}

	/**
	 * Return the Purchasing User that has the same id as the given id.
	 *
	 * @param id      The id of the PurchasingUserEntity to be returned
	 * @return the Purchasing User that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PurchasingUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PurchasingUser entity not found"),
	})
	@PreAuthorize("hasPermission('PurchasingUserEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PurchasingUserEntity> getById(
			@Parameter(description = "The UUID of the PurchasingUser to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PurchasingUserEntity> purchasingUserEntity = purchasingUserService.findById(id);
	...
	/**
	 * Create/Update the given Purchasing User. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param purchasingUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePurchasingUser' endpoint here] off begin
	@Operation(
			description = "Create a PurchasingUser if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PurchasingUser."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PurchasingUser, duplicate record found")
	})
	@PreAuthorize("hasPermission('PurchasingUserEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePurchasingUser' endpoint here] end
	public ResponseEntity<?> savePurchasingUser(
			@Parameter(description = "The PurchasingUserEntity to create or update.")
			@RequestBody PurchasingUserEntityDto purchasingUserEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PurchasingUserEntity purchasingUserEntity = purchasingUserService.create(new PurchasingUserEntity(purchasingUserEntityDto));
			return new ResponseEntity<>(purchasingUserEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Purchasing User. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param purchasingUserEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePurchasingUser' endpoint here] off begin
	@Operation(
			description = "Update a PurchasingUser entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PurchasingUser."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PurchasingUser, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PurchasingUserEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePurchasingUser' endpoint here] end
	public ResponseEntity<?> updatePurchasingUser(
			@Parameter(description = "The PurchasingUserEntity to create or update.")
			@RequestBody PurchasingUserEntityDto purchasingUserEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PurchasingUserEntity> checkPurchasingUserExists = purchasingUserService.findById(purchasingUserEntityDto.getId());
		PurchasingUserEntity purchasingUserEntity = new PurchasingUserEntity(purchasingUserEntityDto);

		if (checkPurchasingUserExists.isPresent()) {
			try {
				purchasingUserEntity = purchasingUserService.update(purchasingUserEntity);
				return new ResponseEntity<>(purchasingUserEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(purchasingUserEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Purchasing User that has the same id as the given id.
	 *
	 * @param id The id of the PurchasingUserEntity to be deleted
	...
	@Operation(
			description = "Delete a single PurchasingUser as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('PurchasingUserEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PurchasingUser to delete.")
		...
		purchasingUserService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchasingUser' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Purchasing User entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchasingUser' endpoint here] end
	public void exportPurchasingUser(
			@Parameter(description = "The ids of the PurchasingUserEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PurchasingUserEntity> entitiesToExport = purchasingUserService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PurchasingUser_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PurchasingUserEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchasingUserExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchasingUserExcludingIds' endpoint here] end
	public void exportPurchasingUserExcludingIds(
			@Parameter(description = "The ids of the PurchasingUserEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PurchasingUserEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = purchasingUserService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = purchasingUserService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PurchasingUser entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PurchasingUser_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PurchasingUserEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Purchasing User entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PurchasingUserEntity', 'create')  || hasPermission('PurchasingUserEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PurchasingUserEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PurchasingUserEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PurchasingUserEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PurchasingUserEntity> csvToBean = new CsvToBeanBuilder<PurchasingUserEntity>(reader)
				.withType(PurchasingUserEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PurchasingUserEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PurchasingUserEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Purchasing User entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Purchasing User entity and you do not have permission");
					}
