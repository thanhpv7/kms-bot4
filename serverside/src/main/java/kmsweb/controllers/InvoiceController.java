...
import kmsweb.entities.InvoiceEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Invoice
 */
// % protected region % [Update the API route or description for the InvoiceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InvoiceController"))
@RestController
@Slf4j
@RequestMapping("/api/invoice")
// % protected region % [Update the API route or description for the InvoiceController here] end
public class InvoiceController {

	private final InvoiceService invoiceService;
	private final CsvUtils<InvoiceEntity, InvoiceRepository, InvoiceEntityAudit,  InvoiceService> csvUtils;
	...
	public InvoiceController(
		...
		this.csvUtils = new CsvUtils<>(invoiceService, InvoiceEntity.class);
	...
	@Operation(
			description = "Returns a single page of Invoices",
		security = {
	...
	public ResponseEntity<List<InvoiceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InvoiceEntity> invoices = invoiceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Invoice that has the same id as the given id.
	 *
	 * @param id      The id of the InvoiceEntity to be returned
	 * @return the Invoice that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Invoice as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Invoice entity not found"),
	})
	@PreAuthorize("hasPermission('InvoiceEntity', 'read')")
	...
	public ResponseEntity<InvoiceEntity> getById(
			@Parameter(description = "The UUID of the Invoice to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InvoiceEntity> invoiceEntity = invoiceService.findById(id);
	...
	/**
	 * Create/Update the given Invoice. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInvoice' endpoint here] off begin
	@Operation(
			description = "Create a Invoice if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Invoice."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Invoice, duplicate record found")
	})
	@PreAuthorize("hasPermission('InvoiceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInvoice' endpoint here] end
	public ResponseEntity<?> saveInvoice(
			@Parameter(description = "The InvoiceEntity to create or update.")
			@RequestBody InvoiceEntityDto invoiceEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InvoiceEntity invoiceEntity = invoiceService.create(new InvoiceEntity(invoiceEntityDto));
	...
	/**
	 * Update the given Invoice. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInvoice' endpoint here] off begin
	@Operation(
			description = "Update a Invoice entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		...
		@ApiResponse(responseCode = "400", description = "Failed to update the Invoice, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InvoiceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInvoice' endpoint here] end
	public ResponseEntity<?> updateInvoice(
		...
		Optional<InvoiceEntity> checkInvoiceExists = invoiceService.findById(invoiceEntityDto.getId());
		InvoiceEntity invoiceEntity = new InvoiceEntity(invoiceEntityDto);

		if (checkInvoiceExists.isPresent()) {
			try {
				invoiceEntity = invoiceService.update(invoiceEntity);
				return new ResponseEntity<>(invoiceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(invoiceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Invoice that has the same id as the given id.
	 *
	 * @param id The id of the InvoiceEntity to be deleted
	...
	@Operation(
			description = "Delete a single Invoice as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Invoice to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoice' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Invoice entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoice' endpoint here] end
	public void exportInvoice(
			@Parameter(description = "The ids of the InvoiceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InvoiceEntity> entitiesToExport = invoiceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Invoice_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InvoiceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoiceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoiceExcludingIds' endpoint here] end
	public void exportInvoiceExcludingIds(
			@Parameter(description = "The ids of the InvoiceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InvoiceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = invoiceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = invoiceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Invoice entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Invoice_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InvoiceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Invoice entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InvoiceEntity', 'create')  || hasPermission('InvoiceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InvoiceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InvoiceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InvoiceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InvoiceEntity> csvToBean = new CsvToBeanBuilder<InvoiceEntity>(reader)
				.withType(InvoiceEntity.class)
		...
		try {
			Iterator<InvoiceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Invoice entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Invoice entity and you do not have permission");
					}
