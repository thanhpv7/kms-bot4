...
import kmsweb.entities.InsuranceProviderEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Insurance Provider
 */
// % protected region % [Update the API route or description for the Insurance ProviderController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InsuranceProviderController"))
@RestController
@Slf4j
@RequestMapping("/api/insuranceprovider")
// % protected region % [Update the API route or description for the Insurance ProviderController here] end
public class InsuranceProviderController {

	private final InsuranceProviderService insuranceProviderService;
	private final CsvUtils<InsuranceProviderEntity, InsuranceProviderRepository, InsuranceProviderEntityAudit,  InsuranceProviderService> csvUtils;
	...
	public InsuranceProviderController(
		...
		this.csvUtils = new CsvUtils<>(insuranceProviderService, InsuranceProviderEntity.class);
	...
	public ResponseEntity<List<InsuranceProviderEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InsuranceProviderEntity> insuranceProviders = insuranceProviderService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(insuranceProviders, HttpStatus.OK);
	}

	/**
	 * Return the Insurance Provider that has the same id as the given id.
	 *
	 * @param id      The id of the InsuranceProviderEntity to be returned
	 * @return the Insurance Provider that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InsuranceProvider as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InsuranceProvider entity not found"),
	})
	@PreAuthorize("hasPermission('InsuranceProviderEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InsuranceProviderEntity> getById(
			@Parameter(description = "The UUID of the InsuranceProvider to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InsuranceProviderEntity> insuranceProviderEntity = insuranceProviderService.findById(id);
	...
	/**
	 * Create/Update the given Insurance Provider. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param insuranceProviderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInsuranceProvider' endpoint here] off begin
	@Operation(
			description = "Create a InsuranceProvider if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InsuranceProvider."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InsuranceProvider, duplicate record found")
	})
	@PreAuthorize("hasPermission('InsuranceProviderEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInsuranceProvider' endpoint here] end
	public ResponseEntity<?> saveInsuranceProvider(
			@Parameter(description = "The InsuranceProviderEntity to create or update.")
			@RequestBody InsuranceProviderEntityDto insuranceProviderEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InsuranceProviderEntity insuranceProviderEntity = insuranceProviderService.create(new InsuranceProviderEntity(insuranceProviderEntityDto));
			return new ResponseEntity<>(insuranceProviderEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Insurance Provider. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param insuranceProviderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInsuranceProvider' endpoint here] off begin
	@Operation(
			description = "Update a InsuranceProvider entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InsuranceProvider."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InsuranceProvider, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InsuranceProviderEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInsuranceProvider' endpoint here] end
	public ResponseEntity<?> updateInsuranceProvider(
		...
		Optional<InsuranceProviderEntity> checkInsuranceProviderExists = insuranceProviderService.findById(insuranceProviderEntityDto.getId());
		InsuranceProviderEntity insuranceProviderEntity = new InsuranceProviderEntity(insuranceProviderEntityDto);

		if (checkInsuranceProviderExists.isPresent()) {
			try {
				insuranceProviderEntity = insuranceProviderService.update(insuranceProviderEntity);
				return new ResponseEntity<>(insuranceProviderEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(insuranceProviderEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Insurance Provider that has the same id as the given id.
	 *
	 * @param id The id of the InsuranceProviderEntity to be deleted
	...
	@Operation(
			description = "Delete a single InsuranceProvider as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InsuranceProvider to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInsuranceProvider' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Insurance Provider entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInsuranceProvider' endpoint here] end
	public void exportInsuranceProvider(
			@Parameter(description = "The ids of the InsuranceProviderEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InsuranceProviderEntity> entitiesToExport = insuranceProviderService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InsuranceProvider_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InsuranceProviderEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInsuranceProviderExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInsuranceProviderExcludingIds' endpoint here] end
	public void exportInsuranceProviderExcludingIds(
			@Parameter(description = "The ids of the InsuranceProviderEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InsuranceProviderEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = insuranceProviderService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = insuranceProviderService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InsuranceProvider entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InsuranceProvider_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InsuranceProviderEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Insurance Provider entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InsuranceProviderEntity', 'create')  || hasPermission('InsuranceProviderEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InsuranceProviderEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InsuranceProviderEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InsuranceProviderEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InsuranceProviderEntity> csvToBean = new CsvToBeanBuilder<InsuranceProviderEntity>(reader)
				.withType(InsuranceProviderEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InsuranceProviderEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InsuranceProviderEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Insurance Provider entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Insurance Provider entity and you do not have permission");
					}
