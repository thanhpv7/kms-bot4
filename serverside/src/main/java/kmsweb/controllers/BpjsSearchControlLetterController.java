...
import kmsweb.entities.BpjsSearchControlLetterEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Search Control Letter
 */
// % protected region % [Update the API route or description for the BPJS Search Control LetterController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsSearchControlLetterController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjssearchcontrolletter")
// % protected region % [Update the API route or description for the BPJS Search Control LetterController here] end
public class BpjsSearchControlLetterController {

	private final BpjsSearchControlLetterService bpjsSearchControlLetterService;
	private final CsvUtils<BpjsSearchControlLetterEntity, BpjsSearchControlLetterRepository, BpjsSearchControlLetterEntityAudit,  BpjsSearchControlLetterService> csvUtils;
	...
	@Autowired
	public BpjsSearchControlLetterController(
		...
		this.csvUtils = new CsvUtils<>(bpjsSearchControlLetterService, BpjsSearchControlLetterEntity.class);
	...
	/**
	 * Return all the BPJS Search Control Letters.
	 *
	 * @return all the BPJS Search Control Letters
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsSearchControlLetters",
		security = {
	...
	public ResponseEntity<List<BpjsSearchControlLetterEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsSearchControlLetterEntity> bpjsSearchControlLetters = bpjsSearchControlLetterService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsSearchControlLetters, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Search Control Letter that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsSearchControlLetterEntity to be returned
	 * @return the BPJS Search Control Letter that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsSearchControlLetter as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsSearchControlLetter entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsSearchControlLetterEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsSearchControlLetterEntity> getById(
			@Parameter(description = "The UUID of the BpjsSearchControlLetter to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsSearchControlLetterEntity> bpjsSearchControlLetterEntity = bpjsSearchControlLetterService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Search Control Letter. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSearchControlLetterEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsSearchControlLetter' endpoint here] off begin
	@Operation(
			description = "Create a BpjsSearchControlLetter if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsSearchControlLetter."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsSearchControlLetter, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsSearchControlLetterEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsSearchControlLetter' endpoint here] end
	public ResponseEntity<?> saveBpjsSearchControlLetter(
			@Parameter(description = "The BpjsSearchControlLetterEntity to create or update.")
			@RequestBody BpjsSearchControlLetterEntityDto bpjsSearchControlLetterEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsSearchControlLetterEntity bpjsSearchControlLetterEntity = bpjsSearchControlLetterService.create(new BpjsSearchControlLetterEntity(bpjsSearchControlLetterEntityDto));
			return new ResponseEntity<>(bpjsSearchControlLetterEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Search Control Letter. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSearchControlLetterEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsSearchControlLetter' endpoint here] off begin
	@Operation(
			description = "Update a BpjsSearchControlLetter entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsSearchControlLetter."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsSearchControlLetter, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsSearchControlLetterEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsSearchControlLetter' endpoint here] end
	public ResponseEntity<?> updateBpjsSearchControlLetter(
			@Parameter(description = "The BpjsSearchControlLetterEntity to create or update.")
			@RequestBody BpjsSearchControlLetterEntityDto bpjsSearchControlLetterEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsSearchControlLetterEntity> checkBpjsSearchControlLetterExists = bpjsSearchControlLetterService.findById(bpjsSearchControlLetterEntityDto.getId());
		BpjsSearchControlLetterEntity bpjsSearchControlLetterEntity = new BpjsSearchControlLetterEntity(bpjsSearchControlLetterEntityDto);

		if (checkBpjsSearchControlLetterExists.isPresent()) {
			try {
				bpjsSearchControlLetterEntity = bpjsSearchControlLetterService.update(bpjsSearchControlLetterEntity);
				return new ResponseEntity<>(bpjsSearchControlLetterEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsSearchControlLetterEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Search Control Letter that has the same id as the given id.
	 *
	 * @param id The id of the BpjsSearchControlLetterEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsSearchControlLetter as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsSearchControlLetter to delete.")
		...
		bpjsSearchControlLetterService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSearchControlLetter' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Search Control Letter entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSearchControlLetter' endpoint here] end
	public void exportBpjsSearchControlLetter(
			@Parameter(description = "The ids of the BpjsSearchControlLetterEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsSearchControlLetterEntity> entitiesToExport = bpjsSearchControlLetterService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsSearchControlLetter_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsSearchControlLetterEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSearchControlLetterExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSearchControlLetterExcludingIds' endpoint here] end
	public void exportBpjsSearchControlLetterExcludingIds(
			@Parameter(description = "The ids of the BpjsSearchControlLetterEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsSearchControlLetterEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsSearchControlLetterService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsSearchControlLetterService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsSearchControlLetter entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsSearchControlLetter_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsSearchControlLetterEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Search Control Letter entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsSearchControlLetterEntity', 'create')  || hasPermission('BpjsSearchControlLetterEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsSearchControlLetterEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsSearchControlLetterEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsSearchControlLetterEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsSearchControlLetterEntity> csvToBean = new CsvToBeanBuilder<BpjsSearchControlLetterEntity>(reader)
				.withType(BpjsSearchControlLetterEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsSearchControlLetterEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsSearchControlLetterEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Search Control Letter entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Search Control Letter entity and you do not have permission");
					}
