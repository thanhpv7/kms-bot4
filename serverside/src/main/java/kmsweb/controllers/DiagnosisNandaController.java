...
/**
 * Controller used to handle all REST operations regarding Diagnosis Nanda
 */
// % protected region % [Update the API route or description for the Diagnosis NandaController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DiagnosisNandaController"))
@RestController
@Slf4j
@RequestMapping("/api/diagnosisnanda")
// % protected region % [Update the API route or description for the Diagnosis NandaController here] end
public class DiagnosisNandaController {

	private final DiagnosisNandaService diagnosisNandaService;
	private final CsvUtils<DiagnosisNandaEntity, DiagnosisNandaRepository, DiagnosisNandaEntityAudit,  DiagnosisNandaService> csvUtils;
	...
	@Autowired
	public DiagnosisNandaController(
		...
		this.csvUtils = new CsvUtils<>(diagnosisNandaService, DiagnosisNandaEntity.class);
	...
	public ResponseEntity<List<DiagnosisNandaEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DiagnosisNandaEntity> diagnosisNandas = diagnosisNandaService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(diagnosisNandas, HttpStatus.OK);
	}

	/**
	 * Return the Diagnosis Nanda that has the same id as the given id.
	 *
	 * @param id      The id of the DiagnosisNandaEntity to be returned
	 * @return the Diagnosis Nanda that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DiagnosisNanda as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DiagnosisNanda entity not found"),
	})
	@PreAuthorize("hasPermission('DiagnosisNandaEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DiagnosisNandaEntity> getById(
			@Parameter(description = "The UUID of the DiagnosisNanda to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DiagnosisNandaEntity> diagnosisNandaEntity = diagnosisNandaService.findById(id);
	...
	/**
	 * Create/Update the given Diagnosis Nanda. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosisNandaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDiagnosisNanda' endpoint here] off begin
	@Operation(
			description = "Create a DiagnosisNanda if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DiagnosisNanda."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DiagnosisNanda, duplicate record found")
	})
	@PreAuthorize("hasPermission('DiagnosisNandaEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDiagnosisNanda' endpoint here] end
	public ResponseEntity<?> saveDiagnosisNanda(
			@Parameter(description = "The DiagnosisNandaEntity to create or update.")
			@RequestBody DiagnosisNandaEntityDto diagnosisNandaEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DiagnosisNandaEntity diagnosisNandaEntity = diagnosisNandaService.create(new DiagnosisNandaEntity(diagnosisNandaEntityDto));
			return new ResponseEntity<>(diagnosisNandaEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Diagnosis Nanda. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosisNandaEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDiagnosisNanda' endpoint here] off begin
	@Operation(
			description = "Update a DiagnosisNanda entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DiagnosisNanda."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DiagnosisNanda, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DiagnosisNandaEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDiagnosisNanda' endpoint here] end
	public ResponseEntity<?> updateDiagnosisNanda(
			@Parameter(description = "The DiagnosisNandaEntity to create or update.")
			@RequestBody DiagnosisNandaEntityDto diagnosisNandaEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DiagnosisNandaEntity> checkDiagnosisNandaExists = diagnosisNandaService.findById(diagnosisNandaEntityDto.getId());
		DiagnosisNandaEntity diagnosisNandaEntity = new DiagnosisNandaEntity(diagnosisNandaEntityDto);

		if (checkDiagnosisNandaExists.isPresent()) {
			try {
				diagnosisNandaEntity = diagnosisNandaService.update(diagnosisNandaEntity);
				return new ResponseEntity<>(diagnosisNandaEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(diagnosisNandaEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Diagnosis Nanda that has the same id as the given id.
	 *
	 * @param id The id of the DiagnosisNandaEntity to be deleted
	...
	@Operation(
			description = "Delete a single DiagnosisNanda as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DiagnosisNanda to delete.")
		...
		diagnosisNandaService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisNanda' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Diagnosis Nanda entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisNanda' endpoint here] end
	public void exportDiagnosisNanda(
			@Parameter(description = "The ids of the DiagnosisNandaEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DiagnosisNandaEntity> entitiesToExport = diagnosisNandaService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DiagnosisNanda_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DiagnosisNandaEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisNandaExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisNandaExcludingIds' endpoint here] end
	public void exportDiagnosisNandaExcludingIds(
			@Parameter(description = "The ids of the DiagnosisNandaEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DiagnosisNandaEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = diagnosisNandaService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = diagnosisNandaService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DiagnosisNanda entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DiagnosisNanda_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DiagnosisNandaEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Diagnosis Nanda entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DiagnosisNandaEntity', 'create')  || hasPermission('DiagnosisNandaEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DiagnosisNandaEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DiagnosisNandaEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DiagnosisNandaEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DiagnosisNandaEntity> csvToBean = new CsvToBeanBuilder<DiagnosisNandaEntity>(reader)
				.withType(DiagnosisNandaEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DiagnosisNandaEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DiagnosisNandaEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Diagnosis Nanda entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Diagnosis Nanda entity and you do not have permission");
					}
