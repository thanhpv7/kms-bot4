...
import kmsweb.entities.MedicalCertificateBirthEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Medical Certificate Birth
 */
// % protected region % [Update the API route or description for the Medical Certificate BirthController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicalCertificateBirthController"))
@RestController
@Slf4j
@RequestMapping("/api/medicalcertificatebirth")
// % protected region % [Update the API route or description for the Medical Certificate BirthController here] end
public class MedicalCertificateBirthController {

	private final MedicalCertificateBirthService medicalCertificateBirthService;
	private final CsvUtils<MedicalCertificateBirthEntity, MedicalCertificateBirthRepository, MedicalCertificateBirthEntityAudit,  MedicalCertificateBirthService> csvUtils;
	...
	@Autowired
	public MedicalCertificateBirthController(
		...
		this.csvUtils = new CsvUtils<>(medicalCertificateBirthService, MedicalCertificateBirthEntity.class);
	...
	public ResponseEntity<List<MedicalCertificateBirthEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicalCertificateBirthEntity> medicalCertificateBirths = medicalCertificateBirthService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicalCertificateBirths, HttpStatus.OK);
	}

	/**
	 * Return the Medical Certificate Birth that has the same id as the given id.
	 *
	 * @param id      The id of the MedicalCertificateBirthEntity to be returned
	 * @return the Medical Certificate Birth that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicalCertificateBirth as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicalCertificateBirth entity not found"),
	})
	@PreAuthorize("hasPermission('MedicalCertificateBirthEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicalCertificateBirthEntity> getById(
			@Parameter(description = "The UUID of the MedicalCertificateBirth to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicalCertificateBirthEntity> medicalCertificateBirthEntity = medicalCertificateBirthService.findById(id);
	...
	/**
	 * Create/Update the given Medical Certificate Birth. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicalCertificateBirthEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicalCertificateBirth' endpoint here] off begin
	@Operation(
			description = "Create a MedicalCertificateBirth if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicalCertificateBirth."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicalCertificateBirth, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicalCertificateBirthEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicalCertificateBirth' endpoint here] end
	public ResponseEntity<?> saveMedicalCertificateBirth(
			@Parameter(description = "The MedicalCertificateBirthEntity to create or update.")
			@RequestBody MedicalCertificateBirthEntityDto medicalCertificateBirthEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicalCertificateBirthEntity medicalCertificateBirthEntity = medicalCertificateBirthService.create(new MedicalCertificateBirthEntity(medicalCertificateBirthEntityDto));
			return new ResponseEntity<>(medicalCertificateBirthEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medical Certificate Birth. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicalCertificateBirthEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicalCertificateBirth' endpoint here] off begin
	@Operation(
			description = "Update a MedicalCertificateBirth entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicalCertificateBirth."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicalCertificateBirth, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicalCertificateBirthEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicalCertificateBirth' endpoint here] end
	public ResponseEntity<?> updateMedicalCertificateBirth(
			@Parameter(description = "The MedicalCertificateBirthEntity to create or update.")
			@RequestBody MedicalCertificateBirthEntityDto medicalCertificateBirthEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicalCertificateBirthEntity> checkMedicalCertificateBirthExists = medicalCertificateBirthService.findById(medicalCertificateBirthEntityDto.getId());
		MedicalCertificateBirthEntity medicalCertificateBirthEntity = new MedicalCertificateBirthEntity(medicalCertificateBirthEntityDto);

		if (checkMedicalCertificateBirthExists.isPresent()) {
			try {
				medicalCertificateBirthEntity = medicalCertificateBirthService.update(medicalCertificateBirthEntity);
				return new ResponseEntity<>(medicalCertificateBirthEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicalCertificateBirthEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medical Certificate Birth that has the same id as the given id.
	 *
	 * @param id The id of the MedicalCertificateBirthEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicalCertificateBirth as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicalCertificateBirth to delete.")
		...
		medicalCertificateBirthService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateBirth' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medical Certificate Birth entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateBirth' endpoint here] end
	public void exportMedicalCertificateBirth(
			@Parameter(description = "The ids of the MedicalCertificateBirthEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicalCertificateBirthEntity> entitiesToExport = medicalCertificateBirthService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicalCertificateBirth_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicalCertificateBirthEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateBirthExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalCertificateBirthExcludingIds' endpoint here] end
	public void exportMedicalCertificateBirthExcludingIds(
			@Parameter(description = "The ids of the MedicalCertificateBirthEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicalCertificateBirthEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicalCertificateBirthService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicalCertificateBirthService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicalCertificateBirth entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicalCertificateBirth_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicalCertificateBirthEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medical Certificate Birth entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicalCertificateBirthEntity', 'create')  || hasPermission('MedicalCertificateBirthEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicalCertificateBirthEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicalCertificateBirthEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicalCertificateBirthEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicalCertificateBirthEntity> csvToBean = new CsvToBeanBuilder<MedicalCertificateBirthEntity>(reader)
				.withType(MedicalCertificateBirthEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicalCertificateBirthEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicalCertificateBirthEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medical Certificate Birth entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medical Certificate Birth entity and you do not have permission");
					}
