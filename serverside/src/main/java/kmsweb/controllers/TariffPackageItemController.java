...
import kmsweb.entities.TariffPackageItemEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Tariff Package Item
 */
// % protected region % [Update the API route or description for the Tariff Package ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TariffPackageItemController"))
@RestController
@Slf4j
@RequestMapping("/api/tariffpackageitem")
// % protected region % [Update the API route or description for the Tariff Package ItemController here] end
public class TariffPackageItemController {

	private final TariffPackageItemService tariffPackageItemService;
	private final CsvUtils<TariffPackageItemEntity, TariffPackageItemRepository, TariffPackageItemEntityAudit,  TariffPackageItemService> csvUtils;
	...
	@Autowired
	public TariffPackageItemController(
		...
		this.csvUtils = new CsvUtils<>(tariffPackageItemService, TariffPackageItemEntity.class);
	...
	public ResponseEntity<List<TariffPackageItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TariffPackageItemEntity> tariffPackageItems = tariffPackageItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(tariffPackageItems, HttpStatus.OK);
	}

	/**
	 * Return the Tariff Package Item that has the same id as the given id.
	 *
	 * @param id      The id of the TariffPackageItemEntity to be returned
	 * @return the Tariff Package Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TariffPackageItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TariffPackageItem entity not found"),
	})
	@PreAuthorize("hasPermission('TariffPackageItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TariffPackageItemEntity> getById(
			@Parameter(description = "The UUID of the TariffPackageItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TariffPackageItemEntity> tariffPackageItemEntity = tariffPackageItemService.findById(id);
	...
	/**
	 * Create/Update the given Tariff Package Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param tariffPackageItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTariffPackageItem' endpoint here] off begin
	@Operation(
			description = "Create a TariffPackageItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TariffPackageItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TariffPackageItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('TariffPackageItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTariffPackageItem' endpoint here] end
	public ResponseEntity<?> saveTariffPackageItem(
			@Parameter(description = "The TariffPackageItemEntity to create or update.")
			@RequestBody TariffPackageItemEntityDto tariffPackageItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TariffPackageItemEntity tariffPackageItemEntity = tariffPackageItemService.create(new TariffPackageItemEntity(tariffPackageItemEntityDto));
			return new ResponseEntity<>(tariffPackageItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Tariff Package Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param tariffPackageItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTariffPackageItem' endpoint here] off begin
	@Operation(
			description = "Update a TariffPackageItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TariffPackageItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TariffPackageItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TariffPackageItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTariffPackageItem' endpoint here] end
	public ResponseEntity<?> updateTariffPackageItem(
			@Parameter(description = "The TariffPackageItemEntity to create or update.")
			@RequestBody TariffPackageItemEntityDto tariffPackageItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TariffPackageItemEntity> checkTariffPackageItemExists = tariffPackageItemService.findById(tariffPackageItemEntityDto.getId());
		TariffPackageItemEntity tariffPackageItemEntity = new TariffPackageItemEntity(tariffPackageItemEntityDto);

		if (checkTariffPackageItemExists.isPresent()) {
			try {
				tariffPackageItemEntity = tariffPackageItemService.update(tariffPackageItemEntity);
				return new ResponseEntity<>(tariffPackageItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(tariffPackageItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Tariff Package Item that has the same id as the given id.
	 *
	 * @param id The id of the TariffPackageItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single TariffPackageItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TariffPackageItem to delete.")
		...
		tariffPackageItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Tariff Package Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageItem' endpoint here] end
	public void exportTariffPackageItem(
			@Parameter(description = "The ids of the TariffPackageItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TariffPackageItemEntity> entitiesToExport = tariffPackageItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TariffPackageItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TariffPackageItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffPackageItemExcludingIds' endpoint here] end
	public void exportTariffPackageItemExcludingIds(
			@Parameter(description = "The ids of the TariffPackageItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TariffPackageItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = tariffPackageItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = tariffPackageItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TariffPackageItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TariffPackageItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TariffPackageItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Tariff Package Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TariffPackageItemEntity', 'create')  || hasPermission('TariffPackageItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TariffPackageItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TariffPackageItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TariffPackageItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TariffPackageItemEntity> csvToBean = new CsvToBeanBuilder<TariffPackageItemEntity>(reader)
				.withType(TariffPackageItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TariffPackageItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TariffPackageItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Tariff Package Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Tariff Package Item entity and you do not have permission");
					}
