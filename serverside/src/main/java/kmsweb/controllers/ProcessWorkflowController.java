...
import kmsweb.entities.ProcessWorkflowEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Process Workflow
 */
// % protected region % [Update the API route or description for the Process WorkflowController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ProcessWorkflowController"))
@RestController
@Slf4j
@RequestMapping("/api/processworkflow")
// % protected region % [Update the API route or description for the Process WorkflowController here] end
public class ProcessWorkflowController {

	private final ProcessWorkflowService processWorkflowService;
	private final CsvUtils<ProcessWorkflowEntity, ProcessWorkflowRepository, ProcessWorkflowEntityAudit,  ProcessWorkflowService> csvUtils;
	...
	public ProcessWorkflowController(
		...
		this.csvUtils = new CsvUtils<>(processWorkflowService, ProcessWorkflowEntity.class);
	...
	public ResponseEntity<List<ProcessWorkflowEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ProcessWorkflowEntity> processWorkflows = processWorkflowService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(processWorkflows, HttpStatus.OK);
	}

	/**
	 * Return the Process Workflow that has the same id as the given id.
	 *
	 * @param id      The id of the ProcessWorkflowEntity to be returned
	 * @return the Process Workflow that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ProcessWorkflow as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ProcessWorkflow entity not found"),
	})
	@PreAuthorize("hasPermission('ProcessWorkflowEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ProcessWorkflowEntity> getById(
			@Parameter(description = "The UUID of the ProcessWorkflow to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ProcessWorkflowEntity> processWorkflowEntity = processWorkflowService.findById(id);
	...
	/**
	 * Create/Update the given Process Workflow. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param processWorkflowEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveProcessWorkflow' endpoint here] off begin
	@Operation(
			description = "Create a ProcessWorkflow if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ProcessWorkflow."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ProcessWorkflow, duplicate record found")
	})
	@PreAuthorize("hasPermission('ProcessWorkflowEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveProcessWorkflow' endpoint here] end
	public ResponseEntity<?> saveProcessWorkflow(
			@Parameter(description = "The ProcessWorkflowEntity to create or update.")
			@RequestBody ProcessWorkflowEntityDto processWorkflowEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ProcessWorkflowEntity processWorkflowEntity = processWorkflowService.create(new ProcessWorkflowEntity(processWorkflowEntityDto));
			return new ResponseEntity<>(processWorkflowEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Process Workflow. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param processWorkflowEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateProcessWorkflow' endpoint here] off begin
	@Operation(
			description = "Update a ProcessWorkflow entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ProcessWorkflow."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ProcessWorkflow, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ProcessWorkflowEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateProcessWorkflow' endpoint here] end
	public ResponseEntity<?> updateProcessWorkflow(
			@Parameter(description = "The ProcessWorkflowEntity to create or update.")
			@RequestBody ProcessWorkflowEntityDto processWorkflowEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ProcessWorkflowEntity> checkProcessWorkflowExists = processWorkflowService.findById(processWorkflowEntityDto.getId());
		ProcessWorkflowEntity processWorkflowEntity = new ProcessWorkflowEntity(processWorkflowEntityDto);

		if (checkProcessWorkflowExists.isPresent()) {
			try {
				processWorkflowEntity = processWorkflowService.update(processWorkflowEntity);
				return new ResponseEntity<>(processWorkflowEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(processWorkflowEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Process Workflow that has the same id as the given id.
	 *
	 * @param id The id of the ProcessWorkflowEntity to be deleted
	...
	@Operation(
			description = "Delete a single ProcessWorkflow as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ProcessWorkflow to delete.")
		...
		processWorkflowService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportProcessWorkflow' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Process Workflow entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportProcessWorkflow' endpoint here] end
	public void exportProcessWorkflow(
			@Parameter(description = "The ids of the ProcessWorkflowEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ProcessWorkflowEntity> entitiesToExport = processWorkflowService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ProcessWorkflow_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ProcessWorkflowEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportProcessWorkflowExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportProcessWorkflowExcludingIds' endpoint here] end
	public void exportProcessWorkflowExcludingIds(
			@Parameter(description = "The ids of the ProcessWorkflowEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ProcessWorkflowEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = processWorkflowService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = processWorkflowService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ProcessWorkflow entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ProcessWorkflow_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ProcessWorkflowEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Process Workflow entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ProcessWorkflowEntity', 'create')  || hasPermission('ProcessWorkflowEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ProcessWorkflowEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ProcessWorkflowEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ProcessWorkflowEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ProcessWorkflowEntity> csvToBean = new CsvToBeanBuilder<ProcessWorkflowEntity>(reader)
				.withType(ProcessWorkflowEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ProcessWorkflowEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ProcessWorkflowEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Process Workflow entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Process Workflow entity and you do not have permission");
					}
