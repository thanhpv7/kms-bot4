...
import kmsweb.entities.SampleCollectionInformationEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Sample Collection Information
 */
// % protected region % [Update the API route or description for the Sample Collection InformationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SampleCollectionInformationController"))
@RestController
@Slf4j
@RequestMapping("/api/samplecollectioninformation")
// % protected region % [Update the API route or description for the Sample Collection InformationController here] end
public class SampleCollectionInformationController {

	private final SampleCollectionInformationService sampleCollectionInformationService;
	private final CsvUtils<SampleCollectionInformationEntity, SampleCollectionInformationRepository, SampleCollectionInformationEntityAudit,  SampleCollectionInformationService> csvUtils;
	...
	@Autowired
	public SampleCollectionInformationController(
		...
		this.csvUtils = new CsvUtils<>(sampleCollectionInformationService, SampleCollectionInformationEntity.class);
	...
	/**
	 * Return all the Sample Collection Informations.
	 *
	 * @return all the Sample Collection Informations
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of SampleCollectionInformations",
		security = {
	...
	public ResponseEntity<List<SampleCollectionInformationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SampleCollectionInformationEntity> sampleCollectionInformations = sampleCollectionInformationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(sampleCollectionInformations, HttpStatus.OK);
	}

	/**
	 * Return the Sample Collection Information that has the same id as the given id.
	 *
	 * @param id      The id of the SampleCollectionInformationEntity to be returned
	 * @return the Sample Collection Information that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single SampleCollectionInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "SampleCollectionInformation entity not found"),
	})
	@PreAuthorize("hasPermission('SampleCollectionInformationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SampleCollectionInformationEntity> getById(
			@Parameter(description = "The UUID of the SampleCollectionInformation to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SampleCollectionInformationEntity> sampleCollectionInformationEntity = sampleCollectionInformationService.findById(id);
		...
		return sampleCollectionInformationEntity
				.map(entity -> new ResponseEntity<>(entity, HttpStatus.OK))
				.orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
	}

	/**
	 * Create/Update the given Sample Collection Information. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param sampleCollectionInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSampleCollectionInformation' endpoint here] off begin
	@Operation(
			description = "Create a SampleCollectionInformation if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new SampleCollectionInformation."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new SampleCollectionInformation, duplicate record found")
	})
	@PreAuthorize("hasPermission('SampleCollectionInformationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSampleCollectionInformation' endpoint here] end
	public ResponseEntity<?> saveSampleCollectionInformation(
			@Parameter(description = "The SampleCollectionInformationEntity to create or update.")
			@RequestBody SampleCollectionInformationEntityDto sampleCollectionInformationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SampleCollectionInformationEntity sampleCollectionInformationEntity = sampleCollectionInformationService.create(new SampleCollectionInformationEntity(sampleCollectionInformationEntityDto));
			return new ResponseEntity<>(sampleCollectionInformationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Sample Collection Information. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param sampleCollectionInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSampleCollectionInformation' endpoint here] off begin
	@Operation(
			description = "Update a SampleCollectionInformation entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the SampleCollectionInformation."),
		@ApiResponse(responseCode = "400", description = "Failed to update the SampleCollectionInformation, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SampleCollectionInformationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSampleCollectionInformation' endpoint here] end
	public ResponseEntity<?> updateSampleCollectionInformation(
			@Parameter(description = "The SampleCollectionInformationEntity to create or update.")
			@RequestBody SampleCollectionInformationEntityDto sampleCollectionInformationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SampleCollectionInformationEntity> checkSampleCollectionInformationExists = sampleCollectionInformationService.findById(sampleCollectionInformationEntityDto.getId());
		SampleCollectionInformationEntity sampleCollectionInformationEntity = new SampleCollectionInformationEntity(sampleCollectionInformationEntityDto);

		if (checkSampleCollectionInformationExists.isPresent()) {
			try {
				sampleCollectionInformationEntity = sampleCollectionInformationService.update(sampleCollectionInformationEntity);
				return new ResponseEntity<>(sampleCollectionInformationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(sampleCollectionInformationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Sample Collection Information that has the same id as the given id.
	 *
	 * @param id The id of the SampleCollectionInformationEntity to be deleted
	...
	@Operation(
			description = "Delete a single SampleCollectionInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the SampleCollectionInformation to delete.")
		...
		sampleCollectionInformationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionInformation' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Sample Collection Information entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionInformation' endpoint here] end
	public void exportSampleCollectionInformation(
			@Parameter(description = "The ids of the SampleCollectionInformationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SampleCollectionInformationEntity> entitiesToExport = sampleCollectionInformationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "SampleCollectionInformation_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SampleCollectionInformationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionInformationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSampleCollectionInformationExcludingIds' endpoint here] end
	public void exportSampleCollectionInformationExcludingIds(
			@Parameter(description = "The ids of the SampleCollectionInformationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SampleCollectionInformationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = sampleCollectionInformationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = sampleCollectionInformationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the SampleCollectionInformation entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "SampleCollectionInformation_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SampleCollectionInformationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Sample Collection Information entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SampleCollectionInformationEntity', 'create')  || hasPermission('SampleCollectionInformationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SampleCollectionInformationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SampleCollectionInformationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SampleCollectionInformationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SampleCollectionInformationEntity> csvToBean = new CsvToBeanBuilder<SampleCollectionInformationEntity>(reader)
				.withType(SampleCollectionInformationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<SampleCollectionInformationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<SampleCollectionInformationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Sample Collection Information entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Sample Collection Information entity and you do not have permission");
					}
