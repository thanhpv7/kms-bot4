...
import kmsweb.entities.BpjsDrugGenericPRBEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Drug Generic PRB
 */
// % protected region % [Update the API route or description for the BPJS Drug Generic PRBController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsDrugGenericPRBController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsdruggenericprb")
// % protected region % [Update the API route or description for the BPJS Drug Generic PRBController here] end
public class BpjsDrugGenericPRBController {

	private final BpjsDrugGenericPRBService bpjsDrugGenericPRBService;
	private final CsvUtils<BpjsDrugGenericPRBEntity, BpjsDrugGenericPRBRepository, BpjsDrugGenericPRBEntityAudit,  BpjsDrugGenericPRBService> csvUtils;
	...
	@Autowired
	public BpjsDrugGenericPRBController(
		...
		this.csvUtils = new CsvUtils<>(bpjsDrugGenericPRBService, BpjsDrugGenericPRBEntity.class);
	...
	public ResponseEntity<List<BpjsDrugGenericPRBEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsDrugGenericPRBEntity> bpjsDrugGenericPRBs = bpjsDrugGenericPRBService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsDrugGenericPRBs, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Drug Generic PRB that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsDrugGenericPRBEntity to be returned
	 * @return the BPJS Drug Generic PRB that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsDrugGenericPRB as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsDrugGenericPRB entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsDrugGenericPRBEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsDrugGenericPRBEntity> getById(
			@Parameter(description = "The UUID of the BpjsDrugGenericPRB to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsDrugGenericPRBEntity> bpjsDrugGenericPRBEntity = bpjsDrugGenericPRBService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Drug Generic PRB. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDrugGenericPRBEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsDrugGenericPRB' endpoint here] off begin
	@Operation(
			description = "Create a BpjsDrugGenericPRB if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsDrugGenericPRB."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsDrugGenericPRB, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsDrugGenericPRBEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsDrugGenericPRB' endpoint here] end
	public ResponseEntity<?> saveBpjsDrugGenericPRB(
			@Parameter(description = "The BpjsDrugGenericPRBEntity to create or update.")
			@RequestBody BpjsDrugGenericPRBEntityDto bpjsDrugGenericPRBEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsDrugGenericPRBEntity bpjsDrugGenericPRBEntity = bpjsDrugGenericPRBService.create(new BpjsDrugGenericPRBEntity(bpjsDrugGenericPRBEntityDto));
			return new ResponseEntity<>(bpjsDrugGenericPRBEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Drug Generic PRB. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDrugGenericPRBEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsDrugGenericPRB' endpoint here] off begin
	@Operation(
			description = "Update a BpjsDrugGenericPRB entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsDrugGenericPRB."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsDrugGenericPRB, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsDrugGenericPRBEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsDrugGenericPRB' endpoint here] end
	public ResponseEntity<?> updateBpjsDrugGenericPRB(
			@Parameter(description = "The BpjsDrugGenericPRBEntity to create or update.")
			@RequestBody BpjsDrugGenericPRBEntityDto bpjsDrugGenericPRBEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsDrugGenericPRBEntity> checkBpjsDrugGenericPRBExists = bpjsDrugGenericPRBService.findById(bpjsDrugGenericPRBEntityDto.getId());
		BpjsDrugGenericPRBEntity bpjsDrugGenericPRBEntity = new BpjsDrugGenericPRBEntity(bpjsDrugGenericPRBEntityDto);

		if (checkBpjsDrugGenericPRBExists.isPresent()) {
			try {
				bpjsDrugGenericPRBEntity = bpjsDrugGenericPRBService.update(bpjsDrugGenericPRBEntity);
				return new ResponseEntity<>(bpjsDrugGenericPRBEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsDrugGenericPRBEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Drug Generic PRB that has the same id as the given id.
	 *
	 * @param id The id of the BpjsDrugGenericPRBEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsDrugGenericPRB as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsDrugGenericPRB to delete.")
		...
		bpjsDrugGenericPRBService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDrugGenericPRB' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Drug Generic PRB entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDrugGenericPRB' endpoint here] end
	public void exportBpjsDrugGenericPRB(
			@Parameter(description = "The ids of the BpjsDrugGenericPRBEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsDrugGenericPRBEntity> entitiesToExport = bpjsDrugGenericPRBService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsDrugGenericPRB_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsDrugGenericPRBEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDrugGenericPRBExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDrugGenericPRBExcludingIds' endpoint here] end
	public void exportBpjsDrugGenericPRBExcludingIds(
			@Parameter(description = "The ids of the BpjsDrugGenericPRBEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsDrugGenericPRBEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsDrugGenericPRBService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsDrugGenericPRBService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsDrugGenericPRB entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsDrugGenericPRB_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsDrugGenericPRBEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Drug Generic PRB entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsDrugGenericPRBEntity', 'create')  || hasPermission('BpjsDrugGenericPRBEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsDrugGenericPRBEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsDrugGenericPRBEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsDrugGenericPRBEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsDrugGenericPRBEntity> csvToBean = new CsvToBeanBuilder<BpjsDrugGenericPRBEntity>(reader)
				.withType(BpjsDrugGenericPRBEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsDrugGenericPRBEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsDrugGenericPRBEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Drug Generic PRB entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Drug Generic PRB entity and you do not have permission");
					}
