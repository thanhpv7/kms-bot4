...
import kmsweb.entities.CatalogueMedicalEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Catalogue Medical 
 */
// % protected region % [Update the API route or description for the Catalogue Medical Controller here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CatalogueMedicalController"))
@RestController
@Slf4j
@RequestMapping("/api/cataloguemedical")
// % protected region % [Update the API route or description for the Catalogue Medical Controller here] end
public class CatalogueMedicalController {

	private final CatalogueMedicalService catalogueMedicalService;
	private final CsvUtils<CatalogueMedicalEntity, CatalogueMedicalRepository, CatalogueMedicalEntityAudit,  CatalogueMedicalService> csvUtils;
	...
	@Autowired
	public CatalogueMedicalController(
		...
		this.csvUtils = new CsvUtils<>(catalogueMedicalService, CatalogueMedicalEntity.class);
	...
	public ResponseEntity<List<CatalogueMedicalEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CatalogueMedicalEntity> catalogueMedicals = catalogueMedicalService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(catalogueMedicals, HttpStatus.OK);
	}

	/**
	 * Return the Catalogue Medical  that has the same id as the given id.
	 *
	 * @param id      The id of the CatalogueMedicalEntity to be returned
	 * @return the Catalogue Medical  that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CatalogueMedical as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CatalogueMedical entity not found"),
	})
	@PreAuthorize("hasPermission('CatalogueMedicalEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CatalogueMedicalEntity> getById(
			@Parameter(description = "The UUID of the CatalogueMedical to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CatalogueMedicalEntity> catalogueMedicalEntity = catalogueMedicalService.findById(id);
	...
	/**
	 * Create/Update the given Catalogue Medical . If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param catalogueMedicalEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCatalogueMedical' endpoint here] off begin
	@Operation(
			description = "Create a CatalogueMedical if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CatalogueMedical."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CatalogueMedical, duplicate record found")
	})
	@PreAuthorize("hasPermission('CatalogueMedicalEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCatalogueMedical' endpoint here] end
	public ResponseEntity<?> saveCatalogueMedical(
			@Parameter(description = "The CatalogueMedicalEntity to create or update.")
			@RequestBody CatalogueMedicalEntityDto catalogueMedicalEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CatalogueMedicalEntity catalogueMedicalEntity = catalogueMedicalService.create(new CatalogueMedicalEntity(catalogueMedicalEntityDto));
			return new ResponseEntity<>(catalogueMedicalEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Catalogue Medical . If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param catalogueMedicalEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCatalogueMedical' endpoint here] off begin
	@Operation(
			description = "Update a CatalogueMedical entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CatalogueMedical."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CatalogueMedical, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CatalogueMedicalEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCatalogueMedical' endpoint here] end
	public ResponseEntity<?> updateCatalogueMedical(
			@Parameter(description = "The CatalogueMedicalEntity to create or update.")
			@RequestBody CatalogueMedicalEntityDto catalogueMedicalEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CatalogueMedicalEntity> checkCatalogueMedicalExists = catalogueMedicalService.findById(catalogueMedicalEntityDto.getId());
		CatalogueMedicalEntity catalogueMedicalEntity = new CatalogueMedicalEntity(catalogueMedicalEntityDto);

		if (checkCatalogueMedicalExists.isPresent()) {
			try {
				catalogueMedicalEntity = catalogueMedicalService.update(catalogueMedicalEntity);
				return new ResponseEntity<>(catalogueMedicalEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(catalogueMedicalEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Catalogue Medical  that has the same id as the given id.
	 *
	 * @param id The id of the CatalogueMedicalEntity to be deleted
	...
	@Operation(
			description = "Delete a single CatalogueMedical as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CatalogueMedical to delete.")
		...
		catalogueMedicalService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCatalogueMedical' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Catalogue Medical  entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCatalogueMedical' endpoint here] end
	public void exportCatalogueMedical(
			@Parameter(description = "The ids of the CatalogueMedicalEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CatalogueMedicalEntity> entitiesToExport = catalogueMedicalService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CatalogueMedical_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CatalogueMedicalEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCatalogueMedicalExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCatalogueMedicalExcludingIds' endpoint here] end
	public void exportCatalogueMedicalExcludingIds(
			@Parameter(description = "The ids of the CatalogueMedicalEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CatalogueMedicalEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = catalogueMedicalService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = catalogueMedicalService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CatalogueMedical entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CatalogueMedical_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CatalogueMedicalEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Catalogue Medical  entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CatalogueMedicalEntity', 'create')  || hasPermission('CatalogueMedicalEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CatalogueMedicalEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CatalogueMedicalEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CatalogueMedicalEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CatalogueMedicalEntity> csvToBean = new CsvToBeanBuilder<CatalogueMedicalEntity>(reader)
				.withType(CatalogueMedicalEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CatalogueMedicalEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CatalogueMedicalEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Catalogue Medical  entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Catalogue Medical  entity and you do not have permission");
					}
