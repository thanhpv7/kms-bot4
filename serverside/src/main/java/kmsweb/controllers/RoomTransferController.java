...
import kmsweb.entities.RoomTransferEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Room Transfer
 */
// % protected region % [Update the API route or description for the Room TransferController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "RoomTransferController"))
@RestController
@Slf4j
@RequestMapping("/api/roomtransfer")
// % protected region % [Update the API route or description for the Room TransferController here] end
public class RoomTransferController {

	private final RoomTransferService roomTransferService;
	private final CsvUtils<RoomTransferEntity, RoomTransferRepository, RoomTransferEntityAudit,  RoomTransferService> csvUtils;
	...
	public RoomTransferController(
		...
		this.csvUtils = new CsvUtils<>(roomTransferService, RoomTransferEntity.class);
	...
	public ResponseEntity<List<RoomTransferEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<RoomTransferEntity> roomTransfers = roomTransferService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(roomTransfers, HttpStatus.OK);
	}

	/**
	 * Return the Room Transfer that has the same id as the given id.
	 *
	 * @param id      The id of the RoomTransferEntity to be returned
	 * @return the Room Transfer that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single RoomTransfer as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "RoomTransfer entity not found"),
	})
	@PreAuthorize("hasPermission('RoomTransferEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<RoomTransferEntity> getById(
			@Parameter(description = "The UUID of the RoomTransfer to return.")
			@PathVariable("id") UUID id
	) {
		Optional<RoomTransferEntity> roomTransferEntity = roomTransferService.findById(id);
	...
	/**
	 * Create/Update the given Room Transfer. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param roomTransferEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveRoomTransfer' endpoint here] off begin
	@Operation(
			description = "Create a RoomTransfer if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new RoomTransfer."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new RoomTransfer, duplicate record found")
	})
	@PreAuthorize("hasPermission('RoomTransferEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveRoomTransfer' endpoint here] end
	public ResponseEntity<?> saveRoomTransfer(
			@Parameter(description = "The RoomTransferEntity to create or update.")
			@RequestBody RoomTransferEntityDto roomTransferEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			RoomTransferEntity roomTransferEntity = roomTransferService.create(new RoomTransferEntity(roomTransferEntityDto));
			return new ResponseEntity<>(roomTransferEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Room Transfer. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param roomTransferEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateRoomTransfer' endpoint here] off begin
	@Operation(
			description = "Update a RoomTransfer entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the RoomTransfer."),
		@ApiResponse(responseCode = "400", description = "Failed to update the RoomTransfer, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('RoomTransferEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateRoomTransfer' endpoint here] end
	public ResponseEntity<?> updateRoomTransfer(
			@Parameter(description = "The RoomTransferEntity to create or update.")
			@RequestBody RoomTransferEntityDto roomTransferEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<RoomTransferEntity> checkRoomTransferExists = roomTransferService.findById(roomTransferEntityDto.getId());
		RoomTransferEntity roomTransferEntity = new RoomTransferEntity(roomTransferEntityDto);

		if (checkRoomTransferExists.isPresent()) {
			try {
				roomTransferEntity = roomTransferService.update(roomTransferEntity);
				return new ResponseEntity<>(roomTransferEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(roomTransferEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Room Transfer that has the same id as the given id.
	 *
	 * @param id The id of the RoomTransferEntity to be deleted
	...
	@Operation(
			description = "Delete a single RoomTransfer as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the RoomTransfer to delete.")
		...
		roomTransferService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportRoomTransfer' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Room Transfer entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportRoomTransfer' endpoint here] end
	public void exportRoomTransfer(
			@Parameter(description = "The ids of the RoomTransferEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<RoomTransferEntity> entitiesToExport = roomTransferService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "RoomTransfer_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (RoomTransferEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportRoomTransferExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportRoomTransferExcludingIds' endpoint here] end
	public void exportRoomTransferExcludingIds(
			@Parameter(description = "The ids of the RoomTransferEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<RoomTransferEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = roomTransferService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = roomTransferService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the RoomTransfer entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "RoomTransfer_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(RoomTransferEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Room Transfer entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('RoomTransferEntity', 'create')  || hasPermission('RoomTransferEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with RoomTransferEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("RoomTransferEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(RoomTransferEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<RoomTransferEntity> csvToBean = new CsvToBeanBuilder<RoomTransferEntity>(reader)
				.withType(RoomTransferEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<RoomTransferEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<RoomTransferEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Room Transfer entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Room Transfer entity and you do not have permission");
					}
