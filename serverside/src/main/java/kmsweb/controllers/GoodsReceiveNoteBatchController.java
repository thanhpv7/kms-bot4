...
import kmsweb.entities.GoodsReceiveNoteBatchEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Goods Receive Note Batch
 */
// % protected region % [Update the API route or description for the Goods Receive Note BatchController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "GoodsReceiveNoteBatchController"))
@RestController
@Slf4j
@RequestMapping("/api/goodsreceivenotebatch")
// % protected region % [Update the API route or description for the Goods Receive Note BatchController here] end
public class GoodsReceiveNoteBatchController {

	private final GoodsReceiveNoteBatchService goodsReceiveNoteBatchService;
	private final CsvUtils<GoodsReceiveNoteBatchEntity, GoodsReceiveNoteBatchRepository, GoodsReceiveNoteBatchEntityAudit,  GoodsReceiveNoteBatchService> csvUtils;
	...
	@Autowired
	public GoodsReceiveNoteBatchController(
		...
		this.csvUtils = new CsvUtils<>(goodsReceiveNoteBatchService, GoodsReceiveNoteBatchEntity.class);
	...
	public ResponseEntity<List<GoodsReceiveNoteBatchEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<GoodsReceiveNoteBatchEntity> goodsReceiveNoteBatchs = goodsReceiveNoteBatchService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(goodsReceiveNoteBatchs, HttpStatus.OK);
	}

	/**
	 * Return the Goods Receive Note Batch that has the same id as the given id.
	 *
	 * @param id      The id of the GoodsReceiveNoteBatchEntity to be returned
	 * @return the Goods Receive Note Batch that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single GoodsReceiveNoteBatch as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "GoodsReceiveNoteBatch entity not found"),
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteBatchEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<GoodsReceiveNoteBatchEntity> getById(
			@Parameter(description = "The UUID of the GoodsReceiveNoteBatch to return.")
			@PathVariable("id") UUID id
	) {
		Optional<GoodsReceiveNoteBatchEntity> goodsReceiveNoteBatchEntity = goodsReceiveNoteBatchService.findById(id);
	...
	/**
	 * Create/Update the given Goods Receive Note Batch. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param goodsReceiveNoteBatchEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveGoodsReceiveNoteBatch' endpoint here] off begin
	@Operation(
			description = "Create a GoodsReceiveNoteBatch if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new GoodsReceiveNoteBatch."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new GoodsReceiveNoteBatch, duplicate record found")
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteBatchEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveGoodsReceiveNoteBatch' endpoint here] end
	public ResponseEntity<?> saveGoodsReceiveNoteBatch(
			@Parameter(description = "The GoodsReceiveNoteBatchEntity to create or update.")
			@RequestBody GoodsReceiveNoteBatchEntityDto goodsReceiveNoteBatchEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			GoodsReceiveNoteBatchEntity goodsReceiveNoteBatchEntity = goodsReceiveNoteBatchService.create(new GoodsReceiveNoteBatchEntity(goodsReceiveNoteBatchEntityDto));
			return new ResponseEntity<>(goodsReceiveNoteBatchEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Goods Receive Note Batch. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param goodsReceiveNoteBatchEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateGoodsReceiveNoteBatch' endpoint here] off begin
	@Operation(
			description = "Update a GoodsReceiveNoteBatch entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the GoodsReceiveNoteBatch."),
		@ApiResponse(responseCode = "400", description = "Failed to update the GoodsReceiveNoteBatch, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('GoodsReceiveNoteBatchEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateGoodsReceiveNoteBatch' endpoint here] end
	public ResponseEntity<?> updateGoodsReceiveNoteBatch(
			@Parameter(description = "The GoodsReceiveNoteBatchEntity to create or update.")
			@RequestBody GoodsReceiveNoteBatchEntityDto goodsReceiveNoteBatchEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<GoodsReceiveNoteBatchEntity> checkGoodsReceiveNoteBatchExists = goodsReceiveNoteBatchService.findById(goodsReceiveNoteBatchEntityDto.getId());
		GoodsReceiveNoteBatchEntity goodsReceiveNoteBatchEntity = new GoodsReceiveNoteBatchEntity(goodsReceiveNoteBatchEntityDto);

		if (checkGoodsReceiveNoteBatchExists.isPresent()) {
			try {
				goodsReceiveNoteBatchEntity = goodsReceiveNoteBatchService.update(goodsReceiveNoteBatchEntity);
				return new ResponseEntity<>(goodsReceiveNoteBatchEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(goodsReceiveNoteBatchEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Goods Receive Note Batch that has the same id as the given id.
	 *
	 * @param id The id of the GoodsReceiveNoteBatchEntity to be deleted
	...
	@Operation(
			description = "Delete a single GoodsReceiveNoteBatch as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the GoodsReceiveNoteBatch to delete.")
		...
		goodsReceiveNoteBatchService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteBatch' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Goods Receive Note Batch entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteBatch' endpoint here] end
	public void exportGoodsReceiveNoteBatch(
			@Parameter(description = "The ids of the GoodsReceiveNoteBatchEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<GoodsReceiveNoteBatchEntity> entitiesToExport = goodsReceiveNoteBatchService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "GoodsReceiveNoteBatch_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (GoodsReceiveNoteBatchEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteBatchExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportGoodsReceiveNoteBatchExcludingIds' endpoint here] end
	public void exportGoodsReceiveNoteBatchExcludingIds(
			@Parameter(description = "The ids of the GoodsReceiveNoteBatchEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<GoodsReceiveNoteBatchEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = goodsReceiveNoteBatchService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = goodsReceiveNoteBatchService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the GoodsReceiveNoteBatch entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "GoodsReceiveNoteBatch_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(GoodsReceiveNoteBatchEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Goods Receive Note Batch entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('GoodsReceiveNoteBatchEntity', 'create')  || hasPermission('GoodsReceiveNoteBatchEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with GoodsReceiveNoteBatchEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("GoodsReceiveNoteBatchEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(GoodsReceiveNoteBatchEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<GoodsReceiveNoteBatchEntity> csvToBean = new CsvToBeanBuilder<GoodsReceiveNoteBatchEntity>(reader)
				.withType(GoodsReceiveNoteBatchEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<GoodsReceiveNoteBatchEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<GoodsReceiveNoteBatchEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Goods Receive Note Batch entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Goods Receive Note Batch entity and you do not have permission");
					}
