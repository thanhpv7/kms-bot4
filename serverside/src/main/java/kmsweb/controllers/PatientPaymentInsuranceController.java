...
import kmsweb.entities.PatientPaymentInsuranceEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Payment Insurance
 */
// % protected region % [Update the API route or description for the Patient Payment InsuranceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientPaymentInsuranceController"))
@RestController
@Slf4j
@RequestMapping("/api/patientpaymentinsurance")
// % protected region % [Update the API route or description for the Patient Payment InsuranceController here] end
public class PatientPaymentInsuranceController {

	private final PatientPaymentInsuranceService patientPaymentInsuranceService;
	private final CsvUtils<PatientPaymentInsuranceEntity, PatientPaymentInsuranceRepository, PatientPaymentInsuranceEntityAudit,  PatientPaymentInsuranceService> csvUtils;
	...
	@Autowired
	public PatientPaymentInsuranceController(
		...
		this.csvUtils = new CsvUtils<>(patientPaymentInsuranceService, PatientPaymentInsuranceEntity.class);
	...
	public ResponseEntity<List<PatientPaymentInsuranceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientPaymentInsuranceEntity> patientPaymentInsurances = patientPaymentInsuranceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientPaymentInsurances, HttpStatus.OK);
	}

	/**
	 * Return the Patient Payment Insurance that has the same id as the given id.
	 *
	 * @param id      The id of the PatientPaymentInsuranceEntity to be returned
	 * @return the Patient Payment Insurance that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientPaymentInsurance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientPaymentInsurance entity not found"),
	})
	@PreAuthorize("hasPermission('PatientPaymentInsuranceEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientPaymentInsuranceEntity> getById(
			@Parameter(description = "The UUID of the PatientPaymentInsurance to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientPaymentInsuranceEntity> patientPaymentInsuranceEntity = patientPaymentInsuranceService.findById(id);
	...
	/**
	 * Create/Update the given Patient Payment Insurance. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentInsuranceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Create a PatientPaymentInsurance if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientPaymentInsurance."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientPaymentInsurance, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientPaymentInsuranceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentInsurance' endpoint here] end
	public ResponseEntity<?> savePatientPaymentInsurance(
			@Parameter(description = "The PatientPaymentInsuranceEntity to create or update.")
			@RequestBody PatientPaymentInsuranceEntityDto patientPaymentInsuranceEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientPaymentInsuranceEntity patientPaymentInsuranceEntity = patientPaymentInsuranceService.create(new PatientPaymentInsuranceEntity(patientPaymentInsuranceEntityDto));
			return new ResponseEntity<>(patientPaymentInsuranceEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Payment Insurance. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentInsuranceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Update a PatientPaymentInsurance entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientPaymentInsurance."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientPaymentInsurance, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientPaymentInsuranceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentInsurance' endpoint here] end
	public ResponseEntity<?> updatePatientPaymentInsurance(
			@Parameter(description = "The PatientPaymentInsuranceEntity to create or update.")
			@RequestBody PatientPaymentInsuranceEntityDto patientPaymentInsuranceEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientPaymentInsuranceEntity> checkPatientPaymentInsuranceExists = patientPaymentInsuranceService.findById(patientPaymentInsuranceEntityDto.getId());
		PatientPaymentInsuranceEntity patientPaymentInsuranceEntity = new PatientPaymentInsuranceEntity(patientPaymentInsuranceEntityDto);

		if (checkPatientPaymentInsuranceExists.isPresent()) {
			try {
				patientPaymentInsuranceEntity = patientPaymentInsuranceService.update(patientPaymentInsuranceEntity);
				return new ResponseEntity<>(patientPaymentInsuranceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientPaymentInsuranceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Payment Insurance that has the same id as the given id.
	 *
	 * @param id The id of the PatientPaymentInsuranceEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientPaymentInsurance as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientPaymentInsurance to delete.")
		...
		patientPaymentInsuranceService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentInsurance' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Payment Insurance entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentInsurance' endpoint here] end
	public void exportPatientPaymentInsurance(
			@Parameter(description = "The ids of the PatientPaymentInsuranceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientPaymentInsuranceEntity> entitiesToExport = patientPaymentInsuranceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientPaymentInsurance_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientPaymentInsuranceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentInsuranceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentInsuranceExcludingIds' endpoint here] end
	public void exportPatientPaymentInsuranceExcludingIds(
			@Parameter(description = "The ids of the PatientPaymentInsuranceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientPaymentInsuranceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientPaymentInsuranceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientPaymentInsuranceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientPaymentInsurance entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientPaymentInsurance_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientPaymentInsuranceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Payment Insurance entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientPaymentInsuranceEntity', 'create')  || hasPermission('PatientPaymentInsuranceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientPaymentInsuranceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientPaymentInsuranceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientPaymentInsuranceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientPaymentInsuranceEntity> csvToBean = new CsvToBeanBuilder<PatientPaymentInsuranceEntity>(reader)
				.withType(PatientPaymentInsuranceEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientPaymentInsuranceEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientPaymentInsuranceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Payment Insurance entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Payment Insurance entity and you do not have permission");
					}
