...
import kmsweb.entities.SupplierProductDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Supplier Product Detail
 */
// % protected region % [Update the API route or description for the Supplier Product DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "SupplierProductDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/supplierproductdetail")
// % protected region % [Update the API route or description for the Supplier Product DetailController here] end
public class SupplierProductDetailController {

	private final SupplierProductDetailService supplierProductDetailService;
	private final CsvUtils<SupplierProductDetailEntity, SupplierProductDetailRepository, SupplierProductDetailEntityAudit,  SupplierProductDetailService> csvUtils;
	...
	@Autowired
	public SupplierProductDetailController(
		...
		this.csvUtils = new CsvUtils<>(supplierProductDetailService, SupplierProductDetailEntity.class);
	...
	public ResponseEntity<List<SupplierProductDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<SupplierProductDetailEntity> supplierProductDetails = supplierProductDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(supplierProductDetails, HttpStatus.OK);
	}

	/**
	 * Return the Supplier Product Detail that has the same id as the given id.
	 *
	 * @param id      The id of the SupplierProductDetailEntity to be returned
	 * @return the Supplier Product Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single SupplierProductDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "SupplierProductDetail entity not found"),
	})
	@PreAuthorize("hasPermission('SupplierProductDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<SupplierProductDetailEntity> getById(
			@Parameter(description = "The UUID of the SupplierProductDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<SupplierProductDetailEntity> supplierProductDetailEntity = supplierProductDetailService.findById(id);
	...
	/**
	 * Create/Update the given Supplier Product Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param supplierProductDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveSupplierProductDetail' endpoint here] off begin
	@Operation(
			description = "Create a SupplierProductDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new SupplierProductDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new SupplierProductDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('SupplierProductDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveSupplierProductDetail' endpoint here] end
	public ResponseEntity<?> saveSupplierProductDetail(
			@Parameter(description = "The SupplierProductDetailEntity to create or update.")
			@RequestBody SupplierProductDetailEntityDto supplierProductDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			SupplierProductDetailEntity supplierProductDetailEntity = supplierProductDetailService.create(new SupplierProductDetailEntity(supplierProductDetailEntityDto));
			return new ResponseEntity<>(supplierProductDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Supplier Product Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param supplierProductDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateSupplierProductDetail' endpoint here] off begin
	@Operation(
			description = "Update a SupplierProductDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the SupplierProductDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the SupplierProductDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('SupplierProductDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateSupplierProductDetail' endpoint here] end
	public ResponseEntity<?> updateSupplierProductDetail(
			@Parameter(description = "The SupplierProductDetailEntity to create or update.")
			@RequestBody SupplierProductDetailEntityDto supplierProductDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<SupplierProductDetailEntity> checkSupplierProductDetailExists = supplierProductDetailService.findById(supplierProductDetailEntityDto.getId());
		SupplierProductDetailEntity supplierProductDetailEntity = new SupplierProductDetailEntity(supplierProductDetailEntityDto);

		if (checkSupplierProductDetailExists.isPresent()) {
			try {
				supplierProductDetailEntity = supplierProductDetailService.update(supplierProductDetailEntity);
				return new ResponseEntity<>(supplierProductDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(supplierProductDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Supplier Product Detail that has the same id as the given id.
	 *
	 * @param id The id of the SupplierProductDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single SupplierProductDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the SupplierProductDetail to delete.")
		...
		supplierProductDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplierProductDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Supplier Product Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplierProductDetail' endpoint here] end
	public void exportSupplierProductDetail(
			@Parameter(description = "The ids of the SupplierProductDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<SupplierProductDetailEntity> entitiesToExport = supplierProductDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "SupplierProductDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (SupplierProductDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportSupplierProductDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportSupplierProductDetailExcludingIds' endpoint here] end
	public void exportSupplierProductDetailExcludingIds(
			@Parameter(description = "The ids of the SupplierProductDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<SupplierProductDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = supplierProductDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = supplierProductDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the SupplierProductDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "SupplierProductDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(SupplierProductDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Supplier Product Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('SupplierProductDetailEntity', 'create')  || hasPermission('SupplierProductDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with SupplierProductDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("SupplierProductDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(SupplierProductDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<SupplierProductDetailEntity> csvToBean = new CsvToBeanBuilder<SupplierProductDetailEntity>(reader)
				.withType(SupplierProductDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<SupplierProductDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<SupplierProductDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Supplier Product Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Supplier Product Detail entity and you do not have permission");
					}
