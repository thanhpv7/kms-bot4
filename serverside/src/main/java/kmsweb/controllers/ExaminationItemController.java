...
/**
 * Controller used to handle all REST operations regarding Examination Item
 */
// % protected region % [Update the API route or description for the Examination ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ExaminationItemController"))
@RestController
@Slf4j
@RequestMapping("/api/examinationitem")
// % protected region % [Update the API route or description for the Examination ItemController here] end
public class ExaminationItemController {

	private final ExaminationItemService examinationItemService;
	private final CsvUtils<ExaminationItemEntity, ExaminationItemRepository, ExaminationItemEntityAudit,  ExaminationItemService> csvUtils;
	...
	@Autowired
	public ExaminationItemController(
		...
		this.csvUtils = new CsvUtils<>(examinationItemService, ExaminationItemEntity.class);
	...
	public ResponseEntity<List<ExaminationItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ExaminationItemEntity> examinationItems = examinationItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(examinationItems, HttpStatus.OK);
	}

	/**
	 * Return the Examination Item that has the same id as the given id.
	 *
	 * @param id      The id of the ExaminationItemEntity to be returned
	 * @return the Examination Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ExaminationItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ExaminationItem entity not found"),
	})
	@PreAuthorize("hasPermission('ExaminationItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ExaminationItemEntity> getById(
			@Parameter(description = "The UUID of the ExaminationItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ExaminationItemEntity> examinationItemEntity = examinationItemService.findById(id);
	...
	/**
	 * Create/Update the given Examination Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param examinationItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveExaminationItem' endpoint here] off begin
	@Operation(
			description = "Create a ExaminationItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ExaminationItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ExaminationItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('ExaminationItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveExaminationItem' endpoint here] end
	public ResponseEntity<?> saveExaminationItem(
			@Parameter(description = "The ExaminationItemEntity to create or update.")
			@RequestBody ExaminationItemEntityDto examinationItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ExaminationItemEntity examinationItemEntity = examinationItemService.create(new ExaminationItemEntity(examinationItemEntityDto));
			return new ResponseEntity<>(examinationItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Examination Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param examinationItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateExaminationItem' endpoint here] off begin
	@Operation(
			description = "Update a ExaminationItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ExaminationItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ExaminationItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ExaminationItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateExaminationItem' endpoint here] end
	public ResponseEntity<?> updateExaminationItem(
			@Parameter(description = "The ExaminationItemEntity to create or update.")
			@RequestBody ExaminationItemEntityDto examinationItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ExaminationItemEntity> checkExaminationItemExists = examinationItemService.findById(examinationItemEntityDto.getId());
		ExaminationItemEntity examinationItemEntity = new ExaminationItemEntity(examinationItemEntityDto);

		if (checkExaminationItemExists.isPresent()) {
			try {
				examinationItemEntity = examinationItemService.update(examinationItemEntity);
				return new ResponseEntity<>(examinationItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(examinationItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Examination Item that has the same id as the given id.
	 *
	 * @param id The id of the ExaminationItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single ExaminationItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ExaminationItem to delete.")
		...
		examinationItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportExaminationItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Examination Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportExaminationItem' endpoint here] end
	public void exportExaminationItem(
			@Parameter(description = "The ids of the ExaminationItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ExaminationItemEntity> entitiesToExport = examinationItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ExaminationItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ExaminationItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportExaminationItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportExaminationItemExcludingIds' endpoint here] end
	public void exportExaminationItemExcludingIds(
			@Parameter(description = "The ids of the ExaminationItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ExaminationItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = examinationItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = examinationItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ExaminationItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ExaminationItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ExaminationItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Examination Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ExaminationItemEntity', 'create')  || hasPermission('ExaminationItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ExaminationItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ExaminationItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ExaminationItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ExaminationItemEntity> csvToBean = new CsvToBeanBuilder<ExaminationItemEntity>(reader)
				.withType(ExaminationItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ExaminationItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ExaminationItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Examination Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Examination Item entity and you do not have permission");
					}
