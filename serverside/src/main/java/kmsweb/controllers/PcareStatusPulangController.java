...
import kmsweb.entities.PcareStatusPulangEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding PCare Status Pulang
 */
// % protected region % [Update the API route or description for the PCare Status PulangController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareStatusPulangController"))
@RestController
@Slf4j
@RequestMapping("/api/pcarestatuspulang")
// % protected region % [Update the API route or description for the PCare Status PulangController here] end
public class PcareStatusPulangController {

	private final PcareStatusPulangService pcareStatusPulangService;
	private final CsvUtils<PcareStatusPulangEntity, PcareStatusPulangRepository, PcareStatusPulangEntityAudit,  PcareStatusPulangService> csvUtils;
	...
	@Autowired
	public PcareStatusPulangController(
		...
		this.csvUtils = new CsvUtils<>(pcareStatusPulangService, PcareStatusPulangEntity.class);
	...
	public ResponseEntity<List<PcareStatusPulangEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareStatusPulangEntity> pcareStatusPulangs = pcareStatusPulangService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareStatusPulangs, HttpStatus.OK);
	}

	/**
	 * Return the PCare Status Pulang that has the same id as the given id.
	 *
	 * @param id      The id of the PcareStatusPulangEntity to be returned
	 * @return the PCare Status Pulang that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareStatusPulang as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareStatusPulang entity not found"),
	})
	@PreAuthorize("hasPermission('PcareStatusPulangEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareStatusPulangEntity> getById(
			@Parameter(description = "The UUID of the PcareStatusPulang to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareStatusPulangEntity> pcareStatusPulangEntity = pcareStatusPulangService.findById(id);
	...
	/**
	 * Create/Update the given PCare Status Pulang. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareStatusPulangEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareStatusPulang' endpoint here] off begin
	@Operation(
			description = "Create a PcareStatusPulang if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareStatusPulang."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareStatusPulang, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareStatusPulangEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareStatusPulang' endpoint here] end
	public ResponseEntity<?> savePcareStatusPulang(
			@Parameter(description = "The PcareStatusPulangEntity to create or update.")
			@RequestBody PcareStatusPulangEntityDto pcareStatusPulangEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareStatusPulangEntity pcareStatusPulangEntity = pcareStatusPulangService.create(new PcareStatusPulangEntity(pcareStatusPulangEntityDto));
			return new ResponseEntity<>(pcareStatusPulangEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Status Pulang. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareStatusPulangEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareStatusPulang' endpoint here] off begin
	@Operation(
			description = "Update a PcareStatusPulang entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareStatusPulang."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareStatusPulang, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareStatusPulangEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareStatusPulang' endpoint here] end
	public ResponseEntity<?> updatePcareStatusPulang(
			@Parameter(description = "The PcareStatusPulangEntity to create or update.")
			@RequestBody PcareStatusPulangEntityDto pcareStatusPulangEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareStatusPulangEntity> checkPcareStatusPulangExists = pcareStatusPulangService.findById(pcareStatusPulangEntityDto.getId());
		PcareStatusPulangEntity pcareStatusPulangEntity = new PcareStatusPulangEntity(pcareStatusPulangEntityDto);

		if (checkPcareStatusPulangExists.isPresent()) {
			try {
				pcareStatusPulangEntity = pcareStatusPulangService.update(pcareStatusPulangEntity);
				return new ResponseEntity<>(pcareStatusPulangEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareStatusPulangEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Status Pulang that has the same id as the given id.
	 *
	 * @param id The id of the PcareStatusPulangEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareStatusPulang as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareStatusPulang to delete.")
		...
		pcareStatusPulangService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareStatusPulang' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Status Pulang entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareStatusPulang' endpoint here] end
	public void exportPcareStatusPulang(
			@Parameter(description = "The ids of the PcareStatusPulangEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareStatusPulangEntity> entitiesToExport = pcareStatusPulangService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareStatusPulang_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareStatusPulangEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareStatusPulangExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareStatusPulangExcludingIds' endpoint here] end
	public void exportPcareStatusPulangExcludingIds(
			@Parameter(description = "The ids of the PcareStatusPulangEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareStatusPulangEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareStatusPulangService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareStatusPulangService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareStatusPulang entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareStatusPulang_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareStatusPulangEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Status Pulang entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareStatusPulangEntity', 'create')  || hasPermission('PcareStatusPulangEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareStatusPulangEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareStatusPulangEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareStatusPulangEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareStatusPulangEntity> csvToBean = new CsvToBeanBuilder<PcareStatusPulangEntity>(reader)
				.withType(PcareStatusPulangEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareStatusPulangEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareStatusPulangEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Status Pulang entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Status Pulang entity and you do not have permission");
					}
