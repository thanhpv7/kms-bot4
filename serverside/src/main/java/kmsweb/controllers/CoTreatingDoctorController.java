...
/**
 * Controller used to handle all REST operations regarding Co Treating Doctor
 */
// % protected region % [Update the API route or description for the Co Treating DoctorController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CoTreatingDoctorController"))
@RestController
@Slf4j
@RequestMapping("/api/cotreatingdoctor")
// % protected region % [Update the API route or description for the Co Treating DoctorController here] end
public class CoTreatingDoctorController {

	private final CoTreatingDoctorService coTreatingDoctorService;
	private final CsvUtils<CoTreatingDoctorEntity, CoTreatingDoctorRepository, CoTreatingDoctorEntityAudit,  CoTreatingDoctorService> csvUtils;
	...
	@Autowired
	public CoTreatingDoctorController(
		...
		this.csvUtils = new CsvUtils<>(coTreatingDoctorService, CoTreatingDoctorEntity.class);
	...
	public ResponseEntity<List<CoTreatingDoctorEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CoTreatingDoctorEntity> coTreatingDoctors = coTreatingDoctorService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(coTreatingDoctors, HttpStatus.OK);
	}

	/**
	 * Return the Co Treating Doctor that has the same id as the given id.
	 *
	 * @param id      The id of the CoTreatingDoctorEntity to be returned
	 * @return the Co Treating Doctor that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CoTreatingDoctor as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CoTreatingDoctor entity not found"),
	})
	@PreAuthorize("hasPermission('CoTreatingDoctorEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CoTreatingDoctorEntity> getById(
			@Parameter(description = "The UUID of the CoTreatingDoctor to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CoTreatingDoctorEntity> coTreatingDoctorEntity = coTreatingDoctorService.findById(id);
	...
	/**
	 * Create/Update the given Co Treating Doctor. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param coTreatingDoctorEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCoTreatingDoctor' endpoint here] off begin
	@Operation(
			description = "Create a CoTreatingDoctor if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CoTreatingDoctor."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CoTreatingDoctor, duplicate record found")
	})
	@PreAuthorize("hasPermission('CoTreatingDoctorEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCoTreatingDoctor' endpoint here] end
	public ResponseEntity<?> saveCoTreatingDoctor(
			@Parameter(description = "The CoTreatingDoctorEntity to create or update.")
			@RequestBody CoTreatingDoctorEntityDto coTreatingDoctorEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CoTreatingDoctorEntity coTreatingDoctorEntity = coTreatingDoctorService.create(new CoTreatingDoctorEntity(coTreatingDoctorEntityDto));
			return new ResponseEntity<>(coTreatingDoctorEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Co Treating Doctor. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param coTreatingDoctorEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCoTreatingDoctor' endpoint here] off begin
	@Operation(
			description = "Update a CoTreatingDoctor entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CoTreatingDoctor."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CoTreatingDoctor, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CoTreatingDoctorEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCoTreatingDoctor' endpoint here] end
	public ResponseEntity<?> updateCoTreatingDoctor(
			@Parameter(description = "The CoTreatingDoctorEntity to create or update.")
			@RequestBody CoTreatingDoctorEntityDto coTreatingDoctorEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CoTreatingDoctorEntity> checkCoTreatingDoctorExists = coTreatingDoctorService.findById(coTreatingDoctorEntityDto.getId());
		CoTreatingDoctorEntity coTreatingDoctorEntity = new CoTreatingDoctorEntity(coTreatingDoctorEntityDto);

		if (checkCoTreatingDoctorExists.isPresent()) {
			try {
				coTreatingDoctorEntity = coTreatingDoctorService.update(coTreatingDoctorEntity);
				return new ResponseEntity<>(coTreatingDoctorEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(coTreatingDoctorEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Co Treating Doctor that has the same id as the given id.
	 *
	 * @param id The id of the CoTreatingDoctorEntity to be deleted
	...
	@Operation(
			description = "Delete a single CoTreatingDoctor as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CoTreatingDoctor to delete.")
		...
		coTreatingDoctorService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCoTreatingDoctor' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Co Treating Doctor entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCoTreatingDoctor' endpoint here] end
	public void exportCoTreatingDoctor(
			@Parameter(description = "The ids of the CoTreatingDoctorEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CoTreatingDoctorEntity> entitiesToExport = coTreatingDoctorService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CoTreatingDoctor_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CoTreatingDoctorEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCoTreatingDoctorExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCoTreatingDoctorExcludingIds' endpoint here] end
	public void exportCoTreatingDoctorExcludingIds(
			@Parameter(description = "The ids of the CoTreatingDoctorEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CoTreatingDoctorEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = coTreatingDoctorService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = coTreatingDoctorService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CoTreatingDoctor entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CoTreatingDoctor_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CoTreatingDoctorEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Co Treating Doctor entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CoTreatingDoctorEntity', 'create')  || hasPermission('CoTreatingDoctorEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CoTreatingDoctorEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CoTreatingDoctorEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CoTreatingDoctorEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CoTreatingDoctorEntity> csvToBean = new CsvToBeanBuilder<CoTreatingDoctorEntity>(reader)
				.withType(CoTreatingDoctorEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CoTreatingDoctorEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CoTreatingDoctorEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Co Treating Doctor entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Co Treating Doctor entity and you do not have permission");
					}
