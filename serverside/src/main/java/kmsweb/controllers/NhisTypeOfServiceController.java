...
import kmsweb.entities.NhisTypeOfServiceEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding NHIS Type Of Service
 */
// % protected region % [Update the API route or description for the NHIS Type Of ServiceController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisTypeOfServiceController"))
@RestController
@Slf4j
@RequestMapping("/api/nhistypeofservice")
// % protected region % [Update the API route or description for the NHIS Type Of ServiceController here] end
public class NhisTypeOfServiceController {

	private final NhisTypeOfServiceService nhisTypeOfServiceService;
	private final CsvUtils<NhisTypeOfServiceEntity, NhisTypeOfServiceRepository, NhisTypeOfServiceEntityAudit,  NhisTypeOfServiceService> csvUtils;
	...
	@Autowired
	public NhisTypeOfServiceController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NhisTypeOfServiceService nhisTypeOfServiceService
	) {
		this.authService = authService;
		this.nhisTypeOfServiceService = nhisTypeOfServiceService;
		this.csvUtils = new CsvUtils<>(nhisTypeOfServiceService, NhisTypeOfServiceEntity.class);
	...
	public ResponseEntity<List<NhisTypeOfServiceEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisTypeOfServiceEntity> nhisTypeOfServices = nhisTypeOfServiceService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisTypeOfServices, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Type Of Service that has the same id as the given id.
	 *
	 * @param id      The id of the NhisTypeOfServiceEntity to be returned
	 * @return the NHIS Type Of Service that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisTypeOfService as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisTypeOfService entity not found"),
	})
	@PreAuthorize("hasPermission('NhisTypeOfServiceEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisTypeOfServiceEntity> getById(
			@Parameter(description = "The UUID of the NhisTypeOfService to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisTypeOfServiceEntity> nhisTypeOfServiceEntity = nhisTypeOfServiceService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Type Of Service. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisTypeOfServiceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisTypeOfService' endpoint here] off begin
	@Operation(
			description = "Create a NhisTypeOfService if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisTypeOfService."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisTypeOfService, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisTypeOfServiceEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisTypeOfService' endpoint here] end
	public ResponseEntity<?> saveNhisTypeOfService(
			@Parameter(description = "The NhisTypeOfServiceEntity to create or update.")
			@RequestBody NhisTypeOfServiceEntityDto nhisTypeOfServiceEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisTypeOfServiceEntity nhisTypeOfServiceEntity = nhisTypeOfServiceService.create(new NhisTypeOfServiceEntity(nhisTypeOfServiceEntityDto));
			return new ResponseEntity<>(nhisTypeOfServiceEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Type Of Service. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisTypeOfServiceEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisTypeOfService' endpoint here] off begin
	@Operation(
			description = "Update a NhisTypeOfService entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisTypeOfService."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisTypeOfService, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisTypeOfServiceEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisTypeOfService' endpoint here] end
	public ResponseEntity<?> updateNhisTypeOfService(
			@Parameter(description = "The NhisTypeOfServiceEntity to create or update.")
			@RequestBody NhisTypeOfServiceEntityDto nhisTypeOfServiceEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisTypeOfServiceEntity> checkNhisTypeOfServiceExists = nhisTypeOfServiceService.findById(nhisTypeOfServiceEntityDto.getId());
		NhisTypeOfServiceEntity nhisTypeOfServiceEntity = new NhisTypeOfServiceEntity(nhisTypeOfServiceEntityDto);

		if (checkNhisTypeOfServiceExists.isPresent()) {
			try {
				nhisTypeOfServiceEntity = nhisTypeOfServiceService.update(nhisTypeOfServiceEntity);
				return new ResponseEntity<>(nhisTypeOfServiceEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisTypeOfServiceEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Type Of Service that has the same id as the given id.
	 *
	 * @param id The id of the NhisTypeOfServiceEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisTypeOfService as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisTypeOfService to delete.")
		...
		nhisTypeOfServiceService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisTypeOfService' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Type Of Service entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisTypeOfService' endpoint here] end
	public void exportNhisTypeOfService(
			@Parameter(description = "The ids of the NhisTypeOfServiceEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisTypeOfServiceEntity> entitiesToExport = nhisTypeOfServiceService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisTypeOfService_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisTypeOfServiceEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisTypeOfServiceExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisTypeOfServiceExcludingIds' endpoint here] end
	public void exportNhisTypeOfServiceExcludingIds(
			@Parameter(description = "The ids of the NhisTypeOfServiceEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisTypeOfServiceEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisTypeOfServiceService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisTypeOfServiceService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisTypeOfService entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisTypeOfService_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisTypeOfServiceEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Type Of Service entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisTypeOfServiceEntity', 'create')  || hasPermission('NhisTypeOfServiceEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisTypeOfServiceEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisTypeOfServiceEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisTypeOfServiceEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisTypeOfServiceEntity> csvToBean = new CsvToBeanBuilder<NhisTypeOfServiceEntity>(reader)
				.withType(NhisTypeOfServiceEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisTypeOfServiceEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisTypeOfServiceEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Type Of Service entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Type Of Service entity and you do not have permission");
					}
