...
import kmsweb.entities.StaffEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Staff
 */
// % protected region % [Update the API route or description for the StaffController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "StaffController"))
@RestController
@Slf4j
@RequestMapping("/api/staff")
// % protected region % [Update the API route or description for the StaffController here] end
public class StaffController {

	private final StaffService staffService;
	private final CsvUtils<StaffEntity, StaffRepository, StaffEntityAudit,  StaffService> csvUtils;
	...
	public StaffController(
		...
		this.csvUtils = new CsvUtils<>(staffService, StaffEntity.class);
	...
	@Operation(
			description = "Returns a single page of Staffs",
		security = {
	...
	public ResponseEntity<List<StaffEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<StaffEntity> staffs = staffService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Staff that has the same id as the given id.
	 *
	 * @param id      The id of the StaffEntity to be returned
	 * @return the Staff that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Staff as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Staff entity not found"),
	})
	@PreAuthorize("hasPermission('StaffEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<StaffEntity> getById(
			@Parameter(description = "The UUID of the Staff to return.")
			@PathVariable("id") UUID id
	) {
		Optional<StaffEntity> staffEntity = staffService.findById(id);
	...
	/**
	 * Create/Update the given Staff. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param staffEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveStaff' endpoint here] off begin
	@Operation(
			description = "Create a Staff if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Staff."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Staff, duplicate record found")
	})
	@PreAuthorize("hasPermission('StaffEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveStaff' endpoint here] end
	public ResponseEntity<?> saveStaff(
			@Parameter(description = "The StaffEntity to create or update.")
			@RequestBody StaffEntityDto staffEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			StaffEntity staffEntity = staffService.create(new StaffEntity(staffEntityDto));
			return new ResponseEntity<>(staffEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Staff. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param staffEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateStaff' endpoint here] off begin
	@Operation(
			description = "Update a Staff entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Staff."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Staff, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('StaffEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateStaff' endpoint here] end
	public ResponseEntity<?> updateStaff(
		...
		Optional<StaffEntity> checkStaffExists = staffService.findById(staffEntityDto.getId());
		StaffEntity staffEntity = new StaffEntity(staffEntityDto);

		if (checkStaffExists.isPresent()) {
			try {
				staffEntity = staffService.update(staffEntity);
				return new ResponseEntity<>(staffEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(staffEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Staff that has the same id as the given id.
	 *
	 * @param id The id of the StaffEntity to be deleted
	...
	@Operation(
			description = "Delete a single Staff as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Staff to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportStaff' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Staff entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportStaff' endpoint here] end
	public void exportStaff(
			@Parameter(description = "The ids of the StaffEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<StaffEntity> entitiesToExport = staffService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Staff_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (StaffEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportStaffExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportStaffExcludingIds' endpoint here] end
	public void exportStaffExcludingIds(
			@Parameter(description = "The ids of the StaffEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<StaffEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = staffService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = staffService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Staff entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Staff_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(StaffEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Staff entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('StaffEntity', 'create')  || hasPermission('StaffEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with StaffEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("StaffEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(StaffEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<StaffEntity> csvToBean = new CsvToBeanBuilder<StaffEntity>(reader)
				.withType(StaffEntity.class)
		...
		try {
			Iterator<StaffEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Staff entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Staff entity and you do not have permission");
					}
