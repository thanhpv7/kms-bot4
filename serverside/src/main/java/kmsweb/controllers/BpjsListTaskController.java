...
import kmsweb.entities.BpjsListTaskEntityAudit;
import kmsweb.repositories.BpjsListTaskRepository;
import kmsweb.services.BpjsListTaskService;
...
/**
 * Controller used to handle all REST operations regarding BPJS List Task
 */
// % protected region % [Update the API route or description for the BPJS List TaskController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsListTaskController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjslisttask")
// % protected region % [Update the API route or description for the BPJS List TaskController here] end
public class BpjsListTaskController {

	private final BpjsListTaskService bpjsListTaskService;
	private final CsvUtils<BpjsListTaskEntity, BpjsListTaskRepository, BpjsListTaskEntityAudit,  BpjsListTaskService> csvUtils;
	...
	@Autowired
	public BpjsListTaskController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsListTaskService bpjsListTaskService
	) {
		this.authService = authService;
		this.bpjsListTaskService = bpjsListTaskService;
		this.csvUtils = new CsvUtils<>(bpjsListTaskService, BpjsListTaskEntity.class);
	...
	public ResponseEntity<List<BpjsListTaskEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsListTaskEntity> bpjsListTasks = bpjsListTaskService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsListTasks, HttpStatus.OK);
	}

	/**
	 * Return the BPJS List Task that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsListTaskEntity to be returned
	 * @return the BPJS List Task that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsListTask as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsListTask entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsListTaskEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsListTaskEntity> getById(
			@Parameter(description = "The UUID of the BpjsListTask to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsListTaskEntity> bpjsListTaskEntity = bpjsListTaskService.findById(id);
	...
	/**
	 * Create/Update the given BPJS List Task. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsListTaskEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsListTask' endpoint here] off begin
	@Operation(
			description = "Create a BpjsListTask if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsListTask."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsListTask, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsListTaskEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsListTask' endpoint here] end
	public ResponseEntity<?> saveBpjsListTask(
			@Parameter(description = "The BpjsListTaskEntity to create or update.")
			@RequestBody BpjsListTaskEntityDto bpjsListTaskEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsListTaskEntity bpjsListTaskEntity = bpjsListTaskService.create(new BpjsListTaskEntity(bpjsListTaskEntityDto));
			return new ResponseEntity<>(bpjsListTaskEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS List Task. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsListTaskEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsListTask' endpoint here] off begin
	@Operation(
			description = "Update a BpjsListTask entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsListTask."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsListTask, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsListTaskEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsListTask' endpoint here] end
	public ResponseEntity<?> updateBpjsListTask(
			@Parameter(description = "The BpjsListTaskEntity to create or update.")
			@RequestBody BpjsListTaskEntityDto bpjsListTaskEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsListTaskEntity> checkBpjsListTaskExists = bpjsListTaskService.findById(bpjsListTaskEntityDto.getId());
		BpjsListTaskEntity bpjsListTaskEntity = new BpjsListTaskEntity(bpjsListTaskEntityDto);

		if (checkBpjsListTaskExists.isPresent()) {
			try {
				bpjsListTaskEntity = bpjsListTaskService.update(bpjsListTaskEntity);
				return new ResponseEntity<>(bpjsListTaskEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsListTaskEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS List Task that has the same id as the given id.
	 *
	 * @param id The id of the BpjsListTaskEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsListTask as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsListTask to delete.")
		...
		bpjsListTaskService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsListTask' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS List Task entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsListTask' endpoint here] end
	public void exportBpjsListTask(
			@Parameter(description = "The ids of the BpjsListTaskEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsListTaskEntity> entitiesToExport = bpjsListTaskService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsListTask_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsListTaskEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsListTaskExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsListTaskExcludingIds' endpoint here] end
	public void exportBpjsListTaskExcludingIds(
			@Parameter(description = "The ids of the BpjsListTaskEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsListTaskEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsListTaskService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsListTaskService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsListTask entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsListTask_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsListTaskEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS List Task entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsListTaskEntity', 'create')  || hasPermission('BpjsListTaskEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsListTaskEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsListTaskEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsListTaskEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsListTaskEntity> csvToBean = new CsvToBeanBuilder<BpjsListTaskEntity>(reader)
				.withType(BpjsListTaskEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsListTaskEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsListTaskEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS List Task entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS List Task entity and you do not have permission");
					}
