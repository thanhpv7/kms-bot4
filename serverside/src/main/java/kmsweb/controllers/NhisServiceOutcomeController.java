...
import kmsweb.entities.NhisServiceOutcomeEntityAudit;
import kmsweb.repositories.NhisServiceOutcomeRepository;
import kmsweb.services.NhisServiceOutcomeService;
...
/**
 * Controller used to handle all REST operations regarding NHIS Service Outcome
 */
// % protected region % [Update the API route or description for the NHIS Service OutcomeController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisServiceOutcomeController"))
@RestController
@Slf4j
@RequestMapping("/api/nhisserviceoutcome")
// % protected region % [Update the API route or description for the NHIS Service OutcomeController here] end
public class NhisServiceOutcomeController {

	private final NhisServiceOutcomeService nhisServiceOutcomeService;
	private final CsvUtils<NhisServiceOutcomeEntity, NhisServiceOutcomeRepository, NhisServiceOutcomeEntityAudit,  NhisServiceOutcomeService> csvUtils;
	...
	@Autowired
	public NhisServiceOutcomeController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NhisServiceOutcomeService nhisServiceOutcomeService
	) {
		this.authService = authService;
		this.nhisServiceOutcomeService = nhisServiceOutcomeService;
		this.csvUtils = new CsvUtils<>(nhisServiceOutcomeService, NhisServiceOutcomeEntity.class);
	...
	@PreAuthorize("hasPermission('NhisServiceOutcomeEntity', 'read')")
	@GetMapping(produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] end
	public ResponseEntity<List<NhisServiceOutcomeEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisServiceOutcomeEntity> nhisServiceOutcomes = nhisServiceOutcomeService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisServiceOutcomes, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Service Outcome that has the same id as the given id.
	 *
	 * @param id      The id of the NhisServiceOutcomeEntity to be returned
	 * @return the NHIS Service Outcome that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisServiceOutcome as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisServiceOutcome entity not found"),
	})
	@PreAuthorize("hasPermission('NhisServiceOutcomeEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisServiceOutcomeEntity> getById(
			@Parameter(description = "The UUID of the NhisServiceOutcome to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisServiceOutcomeEntity> nhisServiceOutcomeEntity = nhisServiceOutcomeService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Service Outcome. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisServiceOutcomeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisServiceOutcome' endpoint here] off begin
	@Operation(
			description = "Create a NhisServiceOutcome if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisServiceOutcome."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisServiceOutcome, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisServiceOutcomeEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisServiceOutcome' endpoint here] end
	public ResponseEntity<?> saveNhisServiceOutcome(
			@Parameter(description = "The NhisServiceOutcomeEntity to create or update.")
			@RequestBody NhisServiceOutcomeEntityDto nhisServiceOutcomeEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisServiceOutcomeEntity nhisServiceOutcomeEntity = nhisServiceOutcomeService.create(new NhisServiceOutcomeEntity(nhisServiceOutcomeEntityDto));
			return new ResponseEntity<>(nhisServiceOutcomeEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Service Outcome. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisServiceOutcomeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisServiceOutcome' endpoint here] off begin
	@Operation(
			description = "Update a NhisServiceOutcome entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisServiceOutcome."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisServiceOutcome, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisServiceOutcomeEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisServiceOutcome' endpoint here] end
	public ResponseEntity<?> updateNhisServiceOutcome(
			@Parameter(description = "The NhisServiceOutcomeEntity to create or update.")
			@RequestBody NhisServiceOutcomeEntityDto nhisServiceOutcomeEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisServiceOutcomeEntity> checkNhisServiceOutcomeExists = nhisServiceOutcomeService.findById(nhisServiceOutcomeEntityDto.getId());
		NhisServiceOutcomeEntity nhisServiceOutcomeEntity = new NhisServiceOutcomeEntity(nhisServiceOutcomeEntityDto);

		if (checkNhisServiceOutcomeExists.isPresent()) {
			try {
				nhisServiceOutcomeEntity = nhisServiceOutcomeService.update(nhisServiceOutcomeEntity);
				return new ResponseEntity<>(nhisServiceOutcomeEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisServiceOutcomeEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Service Outcome that has the same id as the given id.
	 *
	 * @param id The id of the NhisServiceOutcomeEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisServiceOutcome as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('NhisServiceOutcomeEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisServiceOutcome to delete.")
		...
		nhisServiceOutcomeService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisServiceOutcome' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Service Outcome entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisServiceOutcome' endpoint here] end
	public void exportNhisServiceOutcome(
			@Parameter(description = "The ids of the NhisServiceOutcomeEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisServiceOutcomeEntity> entitiesToExport = nhisServiceOutcomeService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisServiceOutcome_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisServiceOutcomeEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisServiceOutcomeExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisServiceOutcomeExcludingIds' endpoint here] end
	public void exportNhisServiceOutcomeExcludingIds(
			@Parameter(description = "The ids of the NhisServiceOutcomeEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisServiceOutcomeEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisServiceOutcomeService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisServiceOutcomeService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisServiceOutcome entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisServiceOutcome_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisServiceOutcomeEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Service Outcome entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisServiceOutcomeEntity', 'create')  || hasPermission('NhisServiceOutcomeEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisServiceOutcomeEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisServiceOutcomeEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisServiceOutcomeEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisServiceOutcomeEntity> csvToBean = new CsvToBeanBuilder<NhisServiceOutcomeEntity>(reader)
				.withType(NhisServiceOutcomeEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisServiceOutcomeEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisServiceOutcomeEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Service Outcome entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Service Outcome entity and you do not have permission");
					}
