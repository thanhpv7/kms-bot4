...
/**
 * Controller used to handle all REST operations regarding Patient Vital Information
 */
// % protected region % [Update the API route or description for the Patient Vital InformationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientVitalInformationController"))
@RestController
@Slf4j
@RequestMapping("/api/patientvitalinformation")
// % protected region % [Update the API route or description for the Patient Vital InformationController here] end
public class PatientVitalInformationController {

	private final PatientVitalInformationService patientVitalInformationService;
	private final CsvUtils<PatientVitalInformationEntity, PatientVitalInformationRepository, PatientVitalInformationEntityAudit,  PatientVitalInformationService> csvUtils;
	...
	@Autowired
	public PatientVitalInformationController(
		...
		this.csvUtils = new CsvUtils<>(patientVitalInformationService, PatientVitalInformationEntity.class);
	...
	public ResponseEntity<List<PatientVitalInformationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientVitalInformationEntity> patientVitalInformations = patientVitalInformationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientVitalInformations, HttpStatus.OK);
	}

	/**
	 * Return the Patient Vital Information that has the same id as the given id.
	 *
	 * @param id      The id of the PatientVitalInformationEntity to be returned
	 * @return the Patient Vital Information that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientVitalInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientVitalInformation entity not found"),
	})
	@PreAuthorize("hasPermission('PatientVitalInformationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientVitalInformationEntity> getById(
			@Parameter(description = "The UUID of the PatientVitalInformation to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientVitalInformationEntity> patientVitalInformationEntity = patientVitalInformationService.findById(id);
	...
	/**
	 * Create/Update the given Patient Vital Information. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientVitalInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientVitalInformation' endpoint here] off begin
	@Operation(
			description = "Create a PatientVitalInformation if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientVitalInformation."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientVitalInformation, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientVitalInformationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientVitalInformation' endpoint here] end
	public ResponseEntity<?> savePatientVitalInformation(
			@Parameter(description = "The PatientVitalInformationEntity to create or update.")
			@RequestBody PatientVitalInformationEntityDto patientVitalInformationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientVitalInformationEntity patientVitalInformationEntity = patientVitalInformationService.create(new PatientVitalInformationEntity(patientVitalInformationEntityDto));
			return new ResponseEntity<>(patientVitalInformationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Vital Information. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientVitalInformationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientVitalInformation' endpoint here] off begin
	@Operation(
			description = "Update a PatientVitalInformation entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientVitalInformation."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientVitalInformation, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientVitalInformationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientVitalInformation' endpoint here] end
	public ResponseEntity<?> updatePatientVitalInformation(
			@Parameter(description = "The PatientVitalInformationEntity to create or update.")
			@RequestBody PatientVitalInformationEntityDto patientVitalInformationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientVitalInformationEntity> checkPatientVitalInformationExists = patientVitalInformationService.findById(patientVitalInformationEntityDto.getId());
		PatientVitalInformationEntity patientVitalInformationEntity = new PatientVitalInformationEntity(patientVitalInformationEntityDto);

		if (checkPatientVitalInformationExists.isPresent()) {
			try {
				patientVitalInformationEntity = patientVitalInformationService.update(patientVitalInformationEntity);
				return new ResponseEntity<>(patientVitalInformationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientVitalInformationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Vital Information that has the same id as the given id.
	 *
	 * @param id The id of the PatientVitalInformationEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientVitalInformation as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientVitalInformation to delete.")
		...
		patientVitalInformationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVitalInformation' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Vital Information entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVitalInformation' endpoint here] end
	public void exportPatientVitalInformation(
			@Parameter(description = "The ids of the PatientVitalInformationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientVitalInformationEntity> entitiesToExport = patientVitalInformationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientVitalInformation_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientVitalInformationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientVitalInformationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientVitalInformationExcludingIds' endpoint here] end
	public void exportPatientVitalInformationExcludingIds(
			@Parameter(description = "The ids of the PatientVitalInformationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientVitalInformationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientVitalInformationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientVitalInformationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientVitalInformation entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientVitalInformation_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientVitalInformationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Vital Information entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientVitalInformationEntity', 'create')  || hasPermission('PatientVitalInformationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientVitalInformationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientVitalInformationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientVitalInformationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientVitalInformationEntity> csvToBean = new CsvToBeanBuilder<PatientVitalInformationEntity>(reader)
				.withType(PatientVitalInformationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientVitalInformationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientVitalInformationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Vital Information entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Vital Information entity and you do not have permission");
					}
