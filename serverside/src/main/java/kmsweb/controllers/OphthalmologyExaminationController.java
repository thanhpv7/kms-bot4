...
/**
 * Controller used to handle all REST operations regarding Ophthalmology Examination
 */
// % protected region % [Update the API route or description for the Ophthalmology ExaminationController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "OphthalmologyExaminationController"))
@RestController
@Slf4j
@RequestMapping("/api/ophthalmologyexamination")
// % protected region % [Update the API route or description for the Ophthalmology ExaminationController here] end
public class OphthalmologyExaminationController {

	private final OphthalmologyExaminationService ophthalmologyExaminationService;
	private final CsvUtils<OphthalmologyExaminationEntity, OphthalmologyExaminationRepository, OphthalmologyExaminationEntityAudit,  OphthalmologyExaminationService> csvUtils;
	...
	@Autowired
	public OphthalmologyExaminationController(
		...
		this.csvUtils = new CsvUtils<>(ophthalmologyExaminationService, OphthalmologyExaminationEntity.class);
	...
	/**
	 * Return all the Ophthalmology Examinations.
	 *
	 * @return all the Ophthalmology Examinations
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of OphthalmologyExaminations",
		security = {
	...
	public ResponseEntity<List<OphthalmologyExaminationEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<OphthalmologyExaminationEntity> ophthalmologyExaminations = ophthalmologyExaminationService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(ophthalmologyExaminations, HttpStatus.OK);
	}

	/**
	 * Return the Ophthalmology Examination that has the same id as the given id.
	 *
	 * @param id      The id of the OphthalmologyExaminationEntity to be returned
	 * @return the Ophthalmology Examination that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single OphthalmologyExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "OphthalmologyExamination entity not found"),
	})
	@PreAuthorize("hasPermission('OphthalmologyExaminationEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<OphthalmologyExaminationEntity> getById(
			@Parameter(description = "The UUID of the OphthalmologyExamination to return.")
			@PathVariable("id") UUID id
	) {
		Optional<OphthalmologyExaminationEntity> ophthalmologyExaminationEntity = ophthalmologyExaminationService.findById(id);
	...
	/**
	 * Create/Update the given Ophthalmology Examination. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param ophthalmologyExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveOphthalmologyExamination' endpoint here] off begin
	@Operation(
			description = "Create a OphthalmologyExamination if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new OphthalmologyExamination."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new OphthalmologyExamination, duplicate record found")
	})
	@PreAuthorize("hasPermission('OphthalmologyExaminationEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveOphthalmologyExamination' endpoint here] end
	public ResponseEntity<?> saveOphthalmologyExamination(
			@Parameter(description = "The OphthalmologyExaminationEntity to create or update.")
			@RequestBody OphthalmologyExaminationEntityDto ophthalmologyExaminationEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			OphthalmologyExaminationEntity ophthalmologyExaminationEntity = ophthalmologyExaminationService.create(new OphthalmologyExaminationEntity(ophthalmologyExaminationEntityDto));
			return new ResponseEntity<>(ophthalmologyExaminationEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Ophthalmology Examination. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param ophthalmologyExaminationEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateOphthalmologyExamination' endpoint here] off begin
	@Operation(
			description = "Update a OphthalmologyExamination entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the OphthalmologyExamination."),
		@ApiResponse(responseCode = "400", description = "Failed to update the OphthalmologyExamination, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('OphthalmologyExaminationEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateOphthalmologyExamination' endpoint here] end
	public ResponseEntity<?> updateOphthalmologyExamination(
			@Parameter(description = "The OphthalmologyExaminationEntity to create or update.")
			@RequestBody OphthalmologyExaminationEntityDto ophthalmologyExaminationEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<OphthalmologyExaminationEntity> checkOphthalmologyExaminationExists = ophthalmologyExaminationService.findById(ophthalmologyExaminationEntityDto.getId());
		OphthalmologyExaminationEntity ophthalmologyExaminationEntity = new OphthalmologyExaminationEntity(ophthalmologyExaminationEntityDto);

		if (checkOphthalmologyExaminationExists.isPresent()) {
			try {
				ophthalmologyExaminationEntity = ophthalmologyExaminationService.update(ophthalmologyExaminationEntity);
				return new ResponseEntity<>(ophthalmologyExaminationEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(ophthalmologyExaminationEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Ophthalmology Examination that has the same id as the given id.
	 *
	 * @param id The id of the OphthalmologyExaminationEntity to be deleted
	...
	@Operation(
			description = "Delete a single OphthalmologyExamination as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the OphthalmologyExamination to delete.")
		...
		ophthalmologyExaminationService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportOphthalmologyExamination' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Ophthalmology Examination entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportOphthalmologyExamination' endpoint here] end
	public void exportOphthalmologyExamination(
			@Parameter(description = "The ids of the OphthalmologyExaminationEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<OphthalmologyExaminationEntity> entitiesToExport = ophthalmologyExaminationService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "OphthalmologyExamination_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (OphthalmologyExaminationEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportOphthalmologyExaminationExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportOphthalmologyExaminationExcludingIds' endpoint here] end
	public void exportOphthalmologyExaminationExcludingIds(
			@Parameter(description = "The ids of the OphthalmologyExaminationEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<OphthalmologyExaminationEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = ophthalmologyExaminationService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = ophthalmologyExaminationService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the OphthalmologyExamination entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "OphthalmologyExamination_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(OphthalmologyExaminationEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Ophthalmology Examination entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('OphthalmologyExaminationEntity', 'create')  || hasPermission('OphthalmologyExaminationEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with OphthalmologyExaminationEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("OphthalmologyExaminationEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(OphthalmologyExaminationEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<OphthalmologyExaminationEntity> csvToBean = new CsvToBeanBuilder<OphthalmologyExaminationEntity>(reader)
				.withType(OphthalmologyExaminationEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<OphthalmologyExaminationEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<OphthalmologyExaminationEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Ophthalmology Examination entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Ophthalmology Examination entity and you do not have permission");
					}
