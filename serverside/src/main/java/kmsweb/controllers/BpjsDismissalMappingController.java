...
import kmsweb.entities.BpjsDismissalMappingEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Dismissal Mapping
 */
// % protected region % [Update the API route or description for the BPJS Dismissal MappingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsDismissalMappingController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsdismissalmapping")
// % protected region % [Update the API route or description for the BPJS Dismissal MappingController here] end
public class BpjsDismissalMappingController {

	private final BpjsDismissalMappingService bpjsDismissalMappingService;
	private final CsvUtils<BpjsDismissalMappingEntity, BpjsDismissalMappingRepository, BpjsDismissalMappingEntityAudit,  BpjsDismissalMappingService> csvUtils;
	...
	@Autowired
	public BpjsDismissalMappingController(
		...
		this.csvUtils = new CsvUtils<>(bpjsDismissalMappingService, BpjsDismissalMappingEntity.class);
	...
	public ResponseEntity<List<BpjsDismissalMappingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsDismissalMappingEntity> bpjsDismissalMappings = bpjsDismissalMappingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsDismissalMappings, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Dismissal Mapping that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsDismissalMappingEntity to be returned
	 * @return the BPJS Dismissal Mapping that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsDismissalMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsDismissalMapping entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsDismissalMappingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsDismissalMappingEntity> getById(
			@Parameter(description = "The UUID of the BpjsDismissalMapping to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsDismissalMappingEntity> bpjsDismissalMappingEntity = bpjsDismissalMappingService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Dismissal Mapping. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDismissalMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsDismissalMapping' endpoint here] off begin
	@Operation(
			description = "Create a BpjsDismissalMapping if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsDismissalMapping."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsDismissalMapping, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsDismissalMappingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsDismissalMapping' endpoint here] end
	public ResponseEntity<?> saveBpjsDismissalMapping(
			@Parameter(description = "The BpjsDismissalMappingEntity to create or update.")
			@RequestBody BpjsDismissalMappingEntityDto bpjsDismissalMappingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsDismissalMappingEntity bpjsDismissalMappingEntity = bpjsDismissalMappingService.create(new BpjsDismissalMappingEntity(bpjsDismissalMappingEntityDto));
			return new ResponseEntity<>(bpjsDismissalMappingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Dismissal Mapping. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDismissalMappingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsDismissalMapping' endpoint here] off begin
	@Operation(
			description = "Update a BpjsDismissalMapping entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsDismissalMapping."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsDismissalMapping, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsDismissalMappingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsDismissalMapping' endpoint here] end
	public ResponseEntity<?> updateBpjsDismissalMapping(
			@Parameter(description = "The BpjsDismissalMappingEntity to create or update.")
			@RequestBody BpjsDismissalMappingEntityDto bpjsDismissalMappingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsDismissalMappingEntity> checkBpjsDismissalMappingExists = bpjsDismissalMappingService.findById(bpjsDismissalMappingEntityDto.getId());
		BpjsDismissalMappingEntity bpjsDismissalMappingEntity = new BpjsDismissalMappingEntity(bpjsDismissalMappingEntityDto);

		if (checkBpjsDismissalMappingExists.isPresent()) {
			try {
				bpjsDismissalMappingEntity = bpjsDismissalMappingService.update(bpjsDismissalMappingEntity);
				return new ResponseEntity<>(bpjsDismissalMappingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsDismissalMappingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Dismissal Mapping that has the same id as the given id.
	 *
	 * @param id The id of the BpjsDismissalMappingEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsDismissalMapping as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsDismissalMapping to delete.")
		...
		bpjsDismissalMappingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalMapping' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Dismissal Mapping entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalMapping' endpoint here] end
	public void exportBpjsDismissalMapping(
			@Parameter(description = "The ids of the BpjsDismissalMappingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsDismissalMappingEntity> entitiesToExport = bpjsDismissalMappingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsDismissalMapping_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsDismissalMappingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalMappingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalMappingExcludingIds' endpoint here] end
	public void exportBpjsDismissalMappingExcludingIds(
			@Parameter(description = "The ids of the BpjsDismissalMappingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsDismissalMappingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsDismissalMappingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsDismissalMappingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsDismissalMapping entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsDismissalMapping_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsDismissalMappingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Dismissal Mapping entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsDismissalMappingEntity', 'create')  || hasPermission('BpjsDismissalMappingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsDismissalMappingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsDismissalMappingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsDismissalMappingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsDismissalMappingEntity> csvToBean = new CsvToBeanBuilder<BpjsDismissalMappingEntity>(reader)
				.withType(BpjsDismissalMappingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsDismissalMappingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsDismissalMappingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Dismissal Mapping entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Dismissal Mapping entity and you do not have permission");
					}
