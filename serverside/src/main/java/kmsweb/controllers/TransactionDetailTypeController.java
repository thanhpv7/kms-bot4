...
/**
 * Controller used to handle all REST operations regarding Transaction Detail Type
 */
// % protected region % [Update the API route or description for the Transaction Detail TypeController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TransactionDetailTypeController"))
@RestController
@Slf4j
@RequestMapping("/api/transactiondetailtype")
// % protected region % [Update the API route or description for the Transaction Detail TypeController here] end
public class TransactionDetailTypeController {

	private final TransactionDetailTypeService transactionDetailTypeService;
	private final CsvUtils<TransactionDetailTypeEntity, TransactionDetailTypeRepository, TransactionDetailTypeEntityAudit,  TransactionDetailTypeService> csvUtils;
	...
	@Autowired
	public TransactionDetailTypeController(
		...
		this.csvUtils = new CsvUtils<>(transactionDetailTypeService, TransactionDetailTypeEntity.class);
	...
	public ResponseEntity<List<TransactionDetailTypeEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TransactionDetailTypeEntity> transactionDetailTypes = transactionDetailTypeService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(transactionDetailTypes, HttpStatus.OK);
	}

	/**
	 * Return the Transaction Detail Type that has the same id as the given id.
	 *
	 * @param id      The id of the TransactionDetailTypeEntity to be returned
	 * @return the Transaction Detail Type that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TransactionDetailType as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TransactionDetailType entity not found"),
	})
	@PreAuthorize("hasPermission('TransactionDetailTypeEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TransactionDetailTypeEntity> getById(
			@Parameter(description = "The UUID of the TransactionDetailType to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TransactionDetailTypeEntity> transactionDetailTypeEntity = transactionDetailTypeService.findById(id);
	...
	/**
	 * Create/Update the given Transaction Detail Type. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param transactionDetailTypeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTransactionDetailType' endpoint here] off begin
	@Operation(
			description = "Create a TransactionDetailType if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TransactionDetailType."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TransactionDetailType, duplicate record found")
	})
	@PreAuthorize("hasPermission('TransactionDetailTypeEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTransactionDetailType' endpoint here] end
	public ResponseEntity<?> saveTransactionDetailType(
			@Parameter(description = "The TransactionDetailTypeEntity to create or update.")
			@RequestBody TransactionDetailTypeEntityDto transactionDetailTypeEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TransactionDetailTypeEntity transactionDetailTypeEntity = transactionDetailTypeService.create(new TransactionDetailTypeEntity(transactionDetailTypeEntityDto));
			return new ResponseEntity<>(transactionDetailTypeEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Transaction Detail Type. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param transactionDetailTypeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTransactionDetailType' endpoint here] off begin
	@Operation(
			description = "Update a TransactionDetailType entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TransactionDetailType."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TransactionDetailType, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TransactionDetailTypeEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTransactionDetailType' endpoint here] end
	public ResponseEntity<?> updateTransactionDetailType(
			@Parameter(description = "The TransactionDetailTypeEntity to create or update.")
			@RequestBody TransactionDetailTypeEntityDto transactionDetailTypeEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TransactionDetailTypeEntity> checkTransactionDetailTypeExists = transactionDetailTypeService.findById(transactionDetailTypeEntityDto.getId());
		TransactionDetailTypeEntity transactionDetailTypeEntity = new TransactionDetailTypeEntity(transactionDetailTypeEntityDto);

		if (checkTransactionDetailTypeExists.isPresent()) {
			try {
				transactionDetailTypeEntity = transactionDetailTypeService.update(transactionDetailTypeEntity);
				return new ResponseEntity<>(transactionDetailTypeEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(transactionDetailTypeEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Transaction Detail Type that has the same id as the given id.
	 *
	 * @param id The id of the TransactionDetailTypeEntity to be deleted
	...
	@Operation(
			description = "Delete a single TransactionDetailType as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TransactionDetailType to delete.")
		...
		transactionDetailTypeService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailType' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Transaction Detail Type entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailType' endpoint here] end
	public void exportTransactionDetailType(
			@Parameter(description = "The ids of the TransactionDetailTypeEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TransactionDetailTypeEntity> entitiesToExport = transactionDetailTypeService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TransactionDetailType_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TransactionDetailTypeEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailTypeExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTransactionDetailTypeExcludingIds' endpoint here] end
	public void exportTransactionDetailTypeExcludingIds(
			@Parameter(description = "The ids of the TransactionDetailTypeEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TransactionDetailTypeEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = transactionDetailTypeService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = transactionDetailTypeService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TransactionDetailType entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TransactionDetailType_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TransactionDetailTypeEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Transaction Detail Type entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TransactionDetailTypeEntity', 'create')  || hasPermission('TransactionDetailTypeEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TransactionDetailTypeEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TransactionDetailTypeEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TransactionDetailTypeEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TransactionDetailTypeEntity> csvToBean = new CsvToBeanBuilder<TransactionDetailTypeEntity>(reader)
				.withType(TransactionDetailTypeEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TransactionDetailTypeEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TransactionDetailTypeEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Transaction Detail Type entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Transaction Detail Type entity and you do not have permission");
					}
