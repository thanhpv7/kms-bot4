...
import kmsweb.entities.WarehouseInventoryEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Warehouse Inventory
 */
// % protected region % [Update the API route or description for the Warehouse InventoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "WarehouseInventoryController"))
@RestController
@Slf4j
@RequestMapping("/api/warehouseinventory")
// % protected region % [Update the API route or description for the Warehouse InventoryController here] end
public class WarehouseInventoryController {

	private final WarehouseInventoryService warehouseInventoryService;
	private final CsvUtils<WarehouseInventoryEntity, WarehouseInventoryRepository, WarehouseInventoryEntityAudit,  WarehouseInventoryService> csvUtils;
	...
	public WarehouseInventoryController(
		...
		this.csvUtils = new CsvUtils<>(warehouseInventoryService, WarehouseInventoryEntity.class);
	...
	public ResponseEntity<List<WarehouseInventoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<WarehouseInventoryEntity> warehouseInventorys = warehouseInventoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(warehouseInventorys, HttpStatus.OK);
	}

	/**
	 * Return the Warehouse Inventory that has the same id as the given id.
	 *
	 * @param id      The id of the WarehouseInventoryEntity to be returned
	 * @return the Warehouse Inventory that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single WarehouseInventory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "WarehouseInventory entity not found"),
	})
	@PreAuthorize("hasPermission('WarehouseInventoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<WarehouseInventoryEntity> getById(
			@Parameter(description = "The UUID of the WarehouseInventory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<WarehouseInventoryEntity> warehouseInventoryEntity = warehouseInventoryService.findById(id);
	...
	/**
	 * Create/Update the given Warehouse Inventory. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseInventoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveWarehouseInventory' endpoint here] off begin
	@Operation(
			description = "Create a WarehouseInventory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new WarehouseInventory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new WarehouseInventory, duplicate record found")
	})
	@PreAuthorize("hasPermission('WarehouseInventoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveWarehouseInventory' endpoint here] end
	public ResponseEntity<?> saveWarehouseInventory(
			@Parameter(description = "The WarehouseInventoryEntity to create or update.")
			@RequestBody WarehouseInventoryEntityDto warehouseInventoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			WarehouseInventoryEntity warehouseInventoryEntity = warehouseInventoryService.create(new WarehouseInventoryEntity(warehouseInventoryEntityDto));
			return new ResponseEntity<>(warehouseInventoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Warehouse Inventory. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param warehouseInventoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateWarehouseInventory' endpoint here] off begin
	@Operation(
			description = "Update a WarehouseInventory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the WarehouseInventory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the WarehouseInventory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('WarehouseInventoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateWarehouseInventory' endpoint here] end
	public ResponseEntity<?> updateWarehouseInventory(
			@Parameter(description = "The WarehouseInventoryEntity to create or update.")
			@RequestBody WarehouseInventoryEntityDto warehouseInventoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<WarehouseInventoryEntity> checkWarehouseInventoryExists = warehouseInventoryService.findById(warehouseInventoryEntityDto.getId());
		WarehouseInventoryEntity warehouseInventoryEntity = new WarehouseInventoryEntity(warehouseInventoryEntityDto);

		if (checkWarehouseInventoryExists.isPresent()) {
			try {
				warehouseInventoryEntity = warehouseInventoryService.update(warehouseInventoryEntity);
				return new ResponseEntity<>(warehouseInventoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(warehouseInventoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Warehouse Inventory that has the same id as the given id.
	 *
	 * @param id The id of the WarehouseInventoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single WarehouseInventory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the WarehouseInventory to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInventory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Warehouse Inventory entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInventory' endpoint here] end
	public void exportWarehouseInventory(
			@Parameter(description = "The ids of the WarehouseInventoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<WarehouseInventoryEntity> entitiesToExport = warehouseInventoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "WarehouseInventory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (WarehouseInventoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInventoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportWarehouseInventoryExcludingIds' endpoint here] end
	public void exportWarehouseInventoryExcludingIds(
			@Parameter(description = "The ids of the WarehouseInventoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<WarehouseInventoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = warehouseInventoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = warehouseInventoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the WarehouseInventory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "WarehouseInventory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(WarehouseInventoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Warehouse Inventory entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('WarehouseInventoryEntity', 'create')  || hasPermission('WarehouseInventoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with WarehouseInventoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("WarehouseInventoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(WarehouseInventoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<WarehouseInventoryEntity> csvToBean = new CsvToBeanBuilder<WarehouseInventoryEntity>(reader)
				.withType(WarehouseInventoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<WarehouseInventoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<WarehouseInventoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Warehouse Inventory entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Warehouse Inventory entity and you do not have permission");
					}
