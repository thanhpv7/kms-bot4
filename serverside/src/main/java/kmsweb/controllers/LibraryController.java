...
import kmsweb.entities.LibraryEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Library
 */
// % protected region % [Update the API route or description for the LibraryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "LibraryController"))
@RestController
@Slf4j
@RequestMapping("/api/library")
// % protected region % [Update the API route or description for the LibraryController here] end
public class LibraryController {

	private final LibraryService libraryService;
	private final CsvUtils<LibraryEntity, LibraryRepository, LibraryEntityAudit,  LibraryService> csvUtils;
	...
	public LibraryController(
		...
		this.csvUtils = new CsvUtils<>(libraryService, LibraryEntity.class);
	...
	public ResponseEntity<List<LibraryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<LibraryEntity> librarys = libraryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Library that has the same id as the given id.
	 *
	 * @param id      The id of the LibraryEntity to be returned
	 * @return the Library that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single Library as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Library entity not found"),
	})
	@PreAuthorize("hasPermission('LibraryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<LibraryEntity> getById(
			@Parameter(description = "The UUID of the Library to return.")
			@PathVariable("id") UUID id
	) {
		Optional<LibraryEntity> libraryEntity = libraryService.findById(id);
	...
	/**
	 * Create/Update the given Library. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param libraryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveLibrary' endpoint here] off begin
	@Operation(
			description = "Create a Library if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Library."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Library, duplicate record found")
	})
	@PreAuthorize("hasPermission('LibraryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveLibrary' endpoint here] end
	public ResponseEntity<?> saveLibrary(
			@Parameter(description = "The LibraryEntity to create or update.")
			@RequestBody LibraryEntityDto libraryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			LibraryEntity libraryEntity = libraryService.create(new LibraryEntity(libraryEntityDto));
	...
	/**
	 * Update the given Library. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param libraryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateLibrary' endpoint here] off begin
	@Operation(
			description = "Update a Library entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Library."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Library, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('LibraryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateLibrary' endpoint here] end
	public ResponseEntity<?> updateLibrary(
		...
		Optional<LibraryEntity> checkLibraryExists = libraryService.findById(libraryEntityDto.getId());
		LibraryEntity libraryEntity = new LibraryEntity(libraryEntityDto);

		if (checkLibraryExists.isPresent()) {
			try {
				libraryEntity = libraryService.update(libraryEntity);
				return new ResponseEntity<>(libraryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
	...
	/**
	 * Delete the Library that has the same id as the given id.
	 *
	 * @param id The id of the LibraryEntity to be deleted
	...
	@Operation(
			description = "Delete a single Library as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Library to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportLibrary' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Library entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportLibrary' endpoint here] end
	public void exportLibrary(
			@Parameter(description = "The ids of the LibraryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<LibraryEntity> entitiesToExport = libraryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Library_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (LibraryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportLibraryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportLibraryExcludingIds' endpoint here] end
	public void exportLibraryExcludingIds(
			@Parameter(description = "The ids of the LibraryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<LibraryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = libraryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = libraryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Library entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Library_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(LibraryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Library entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('LibraryEntity', 'create')  || hasPermission('LibraryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with LibraryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("LibraryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(LibraryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<LibraryEntity> csvToBean = new CsvToBeanBuilder<LibraryEntity>(reader)
				.withType(LibraryEntity.class)
		...
		try {
			Iterator<LibraryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Library entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Library entity and you do not have permission");
					}
