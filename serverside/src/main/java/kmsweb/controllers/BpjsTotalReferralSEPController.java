...
/**
 * Controller used to handle all REST operations regarding BPJS Total Referral SEP
 */
// % protected region % [Update the API route or description for the BPJS Total Referral SEPController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsTotalReferralSEPController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjstotalreferralsep")
// % protected region % [Update the API route or description for the BPJS Total Referral SEPController here] end
public class BpjsTotalReferralSEPController {

	private final BpjsTotalReferralSEPService bpjsTotalReferralSEPService;
	private final CsvUtils<BpjsTotalReferralSEPEntity, BpjsTotalReferralSEPRepository, BpjsTotalReferralSEPEntityAudit,  BpjsTotalReferralSEPService> csvUtils;
	...
	@Autowired
	public BpjsTotalReferralSEPController(
		...
		this.csvUtils = new CsvUtils<>(bpjsTotalReferralSEPService, BpjsTotalReferralSEPEntity.class);
	...
	public ResponseEntity<List<BpjsTotalReferralSEPEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsTotalReferralSEPEntity> bpjsTotalReferralSEPs = bpjsTotalReferralSEPService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsTotalReferralSEPs, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Total Referral SEP that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsTotalReferralSEPEntity to be returned
	 * @return the BPJS Total Referral SEP that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsTotalReferralSEP as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsTotalReferralSEP entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsTotalReferralSEPEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsTotalReferralSEPEntity> getById(
			@Parameter(description = "The UUID of the BpjsTotalReferralSEP to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsTotalReferralSEPEntity> bpjsTotalReferralSEPEntity = bpjsTotalReferralSEPService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Total Referral SEP. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTotalReferralSEPEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsTotalReferralSEP' endpoint here] off begin
	@Operation(
			description = "Create a BpjsTotalReferralSEP if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsTotalReferralSEP."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsTotalReferralSEP, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsTotalReferralSEPEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsTotalReferralSEP' endpoint here] end
	public ResponseEntity<?> saveBpjsTotalReferralSEP(
			@Parameter(description = "The BpjsTotalReferralSEPEntity to create or update.")
			@RequestBody BpjsTotalReferralSEPEntityDto bpjsTotalReferralSEPEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsTotalReferralSEPEntity bpjsTotalReferralSEPEntity = bpjsTotalReferralSEPService.create(new BpjsTotalReferralSEPEntity(bpjsTotalReferralSEPEntityDto));
			return new ResponseEntity<>(bpjsTotalReferralSEPEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Total Referral SEP. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsTotalReferralSEPEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsTotalReferralSEP' endpoint here] off begin
	@Operation(
			description = "Update a BpjsTotalReferralSEP entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsTotalReferralSEP."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsTotalReferralSEP, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsTotalReferralSEPEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsTotalReferralSEP' endpoint here] end
	public ResponseEntity<?> updateBpjsTotalReferralSEP(
			@Parameter(description = "The BpjsTotalReferralSEPEntity to create or update.")
			@RequestBody BpjsTotalReferralSEPEntityDto bpjsTotalReferralSEPEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsTotalReferralSEPEntity> checkBpjsTotalReferralSEPExists = bpjsTotalReferralSEPService.findById(bpjsTotalReferralSEPEntityDto.getId());
		BpjsTotalReferralSEPEntity bpjsTotalReferralSEPEntity = new BpjsTotalReferralSEPEntity(bpjsTotalReferralSEPEntityDto);

		if (checkBpjsTotalReferralSEPExists.isPresent()) {
			try {
				bpjsTotalReferralSEPEntity = bpjsTotalReferralSEPService.update(bpjsTotalReferralSEPEntity);
				return new ResponseEntity<>(bpjsTotalReferralSEPEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsTotalReferralSEPEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Total Referral SEP that has the same id as the given id.
	 *
	 * @param id The id of the BpjsTotalReferralSEPEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsTotalReferralSEP as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsTotalReferralSEP to delete.")
		...
		bpjsTotalReferralSEPService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTotalReferralSEP' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Total Referral SEP entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTotalReferralSEP' endpoint here] end
	public void exportBpjsTotalReferralSEP(
			@Parameter(description = "The ids of the BpjsTotalReferralSEPEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsTotalReferralSEPEntity> entitiesToExport = bpjsTotalReferralSEPService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsTotalReferralSEP_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsTotalReferralSEPEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsTotalReferralSEPExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsTotalReferralSEPExcludingIds' endpoint here] end
	public void exportBpjsTotalReferralSEPExcludingIds(
			@Parameter(description = "The ids of the BpjsTotalReferralSEPEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsTotalReferralSEPEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsTotalReferralSEPService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsTotalReferralSEPService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsTotalReferralSEP entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsTotalReferralSEP_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsTotalReferralSEPEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Total Referral SEP entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsTotalReferralSEPEntity', 'create')  || hasPermission('BpjsTotalReferralSEPEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsTotalReferralSEPEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsTotalReferralSEPEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsTotalReferralSEPEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsTotalReferralSEPEntity> csvToBean = new CsvToBeanBuilder<BpjsTotalReferralSEPEntity>(reader)
				.withType(BpjsTotalReferralSEPEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsTotalReferralSEPEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsTotalReferralSEPEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Total Referral SEP entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Total Referral SEP entity and you do not have permission");
					}
