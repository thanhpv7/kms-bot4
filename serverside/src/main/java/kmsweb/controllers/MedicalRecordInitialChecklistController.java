...
/**
 * Controller used to handle all REST operations regarding Medical Record Initial Checklist
 */
// % protected region % [Update the API route or description for the Medical Record Initial ChecklistController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MedicalRecordInitialChecklistController"))
@RestController
@Slf4j
@RequestMapping("/api/medicalrecordinitialchecklist")
// % protected region % [Update the API route or description for the Medical Record Initial ChecklistController here] end
public class MedicalRecordInitialChecklistController {

	private final MedicalRecordInitialChecklistService medicalRecordInitialChecklistService;
	private final CsvUtils<MedicalRecordInitialChecklistEntity, MedicalRecordInitialChecklistRepository, MedicalRecordInitialChecklistEntityAudit,  MedicalRecordInitialChecklistService> csvUtils;
	...
	@Autowired
	public MedicalRecordInitialChecklistController(
		...
		this.csvUtils = new CsvUtils<>(medicalRecordInitialChecklistService, MedicalRecordInitialChecklistEntity.class);
	...
	/**
	 * Return all the Medical Record Initial Checklists.
	 *
	 * @return all the Medical Record Initial Checklists
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of MedicalRecordInitialChecklists",
		security = {
	...
	public ResponseEntity<List<MedicalRecordInitialChecklistEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MedicalRecordInitialChecklistEntity> medicalRecordInitialChecklists = medicalRecordInitialChecklistService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(medicalRecordInitialChecklists, HttpStatus.OK);
	}

	/**
	 * Return the Medical Record Initial Checklist that has the same id as the given id.
	 *
	 * @param id      The id of the MedicalRecordInitialChecklistEntity to be returned
	 * @return the Medical Record Initial Checklist that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MedicalRecordInitialChecklist as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MedicalRecordInitialChecklist entity not found"),
	})
	@PreAuthorize("hasPermission('MedicalRecordInitialChecklistEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MedicalRecordInitialChecklistEntity> getById(
			@Parameter(description = "The UUID of the MedicalRecordInitialChecklist to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MedicalRecordInitialChecklistEntity> medicalRecordInitialChecklistEntity = medicalRecordInitialChecklistService.findById(id);
	...
	/**
	 * Create/Update the given Medical Record Initial Checklist. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param medicalRecordInitialChecklistEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMedicalRecordInitialChecklist' endpoint here] off begin
	@Operation(
			description = "Create a MedicalRecordInitialChecklist if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MedicalRecordInitialChecklist."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MedicalRecordInitialChecklist, duplicate record found")
	})
	@PreAuthorize("hasPermission('MedicalRecordInitialChecklistEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMedicalRecordInitialChecklist' endpoint here] end
	public ResponseEntity<?> saveMedicalRecordInitialChecklist(
			@Parameter(description = "The MedicalRecordInitialChecklistEntity to create or update.")
			@RequestBody MedicalRecordInitialChecklistEntityDto medicalRecordInitialChecklistEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MedicalRecordInitialChecklistEntity medicalRecordInitialChecklistEntity = medicalRecordInitialChecklistService.create(new MedicalRecordInitialChecklistEntity(medicalRecordInitialChecklistEntityDto));
			return new ResponseEntity<>(medicalRecordInitialChecklistEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Medical Record Initial Checklist. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param medicalRecordInitialChecklistEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMedicalRecordInitialChecklist' endpoint here] off begin
	@Operation(
			description = "Update a MedicalRecordInitialChecklist entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MedicalRecordInitialChecklist."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MedicalRecordInitialChecklist, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MedicalRecordInitialChecklistEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMedicalRecordInitialChecklist' endpoint here] end
	public ResponseEntity<?> updateMedicalRecordInitialChecklist(
			@Parameter(description = "The MedicalRecordInitialChecklistEntity to create or update.")
			@RequestBody MedicalRecordInitialChecklistEntityDto medicalRecordInitialChecklistEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MedicalRecordInitialChecklistEntity> checkMedicalRecordInitialChecklistExists = medicalRecordInitialChecklistService.findById(medicalRecordInitialChecklistEntityDto.getId());
		MedicalRecordInitialChecklistEntity medicalRecordInitialChecklistEntity = new MedicalRecordInitialChecklistEntity(medicalRecordInitialChecklistEntityDto);

		if (checkMedicalRecordInitialChecklistExists.isPresent()) {
			try {
				medicalRecordInitialChecklistEntity = medicalRecordInitialChecklistService.update(medicalRecordInitialChecklistEntity);
				return new ResponseEntity<>(medicalRecordInitialChecklistEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(medicalRecordInitialChecklistEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Medical Record Initial Checklist that has the same id as the given id.
	 *
	 * @param id The id of the MedicalRecordInitialChecklistEntity to be deleted
	...
	@Operation(
			description = "Delete a single MedicalRecordInitialChecklist as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MedicalRecordInitialChecklist to delete.")
		...
		medicalRecordInitialChecklistService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordInitialChecklist' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Medical Record Initial Checklist entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordInitialChecklist' endpoint here] end
	public void exportMedicalRecordInitialChecklist(
			@Parameter(description = "The ids of the MedicalRecordInitialChecklistEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MedicalRecordInitialChecklistEntity> entitiesToExport = medicalRecordInitialChecklistService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MedicalRecordInitialChecklist_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MedicalRecordInitialChecklistEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordInitialChecklistExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMedicalRecordInitialChecklistExcludingIds' endpoint here] end
	public void exportMedicalRecordInitialChecklistExcludingIds(
			@Parameter(description = "The ids of the MedicalRecordInitialChecklistEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MedicalRecordInitialChecklistEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = medicalRecordInitialChecklistService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = medicalRecordInitialChecklistService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MedicalRecordInitialChecklist entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MedicalRecordInitialChecklist_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MedicalRecordInitialChecklistEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Medical Record Initial Checklist entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MedicalRecordInitialChecklistEntity', 'create')  || hasPermission('MedicalRecordInitialChecklistEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MedicalRecordInitialChecklistEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MedicalRecordInitialChecklistEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MedicalRecordInitialChecklistEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MedicalRecordInitialChecklistEntity> csvToBean = new CsvToBeanBuilder<MedicalRecordInitialChecklistEntity>(reader)
				.withType(MedicalRecordInitialChecklistEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MedicalRecordInitialChecklistEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MedicalRecordInitialChecklistEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Medical Record Initial Checklist entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Medical Record Initial Checklist entity and you do not have permission");
					}
