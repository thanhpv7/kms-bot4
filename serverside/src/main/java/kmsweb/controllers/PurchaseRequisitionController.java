...
/**
 * Controller used to handle all REST operations regarding Purchase Requisition
 */
// % protected region % [Update the API route or description for the Purchase RequisitionController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PurchaseRequisitionController"))
@RestController
@Slf4j
@RequestMapping("/api/purchaserequisition")
// % protected region % [Update the API route or description for the Purchase RequisitionController here] end
public class PurchaseRequisitionController {

	private final PurchaseRequisitionService purchaseRequisitionService;
	private final CsvUtils<PurchaseRequisitionEntity, PurchaseRequisitionRepository, PurchaseRequisitionEntityAudit,  PurchaseRequisitionService> csvUtils;
	...
	public PurchaseRequisitionController(
		...
		this.csvUtils = new CsvUtils<>(purchaseRequisitionService, PurchaseRequisitionEntity.class);
	...
	public ResponseEntity<List<PurchaseRequisitionEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PurchaseRequisitionEntity> purchaseRequisitions = purchaseRequisitionService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Purchase Requisition that has the same id as the given id.
	 *
	 * @param id      The id of the PurchaseRequisitionEntity to be returned
	 * @return the Purchase Requisition that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PurchaseRequisition as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PurchaseRequisition entity not found"),
	})
	@PreAuthorize("hasPermission('PurchaseRequisitionEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PurchaseRequisitionEntity> getById(
			@Parameter(description = "The UUID of the PurchaseRequisition to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PurchaseRequisitionEntity> purchaseRequisitionEntity = purchaseRequisitionService.findById(id);
	...
	/**
	 * Create/Update the given Purchase Requisition. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param purchaseRequisitionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePurchaseRequisition' endpoint here] off begin
	@Operation(
			description = "Create a PurchaseRequisition if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PurchaseRequisition."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PurchaseRequisition, duplicate record found")
	})
	@PreAuthorize("hasPermission('PurchaseRequisitionEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePurchaseRequisition' endpoint here] end
	public ResponseEntity<?> savePurchaseRequisition(
			@Parameter(description = "The PurchaseRequisitionEntity to create or update.")
			@RequestBody PurchaseRequisitionEntityDto purchaseRequisitionEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PurchaseRequisitionEntity purchaseRequisitionEntity = purchaseRequisitionService.create(new PurchaseRequisitionEntity(purchaseRequisitionEntityDto));
			return new ResponseEntity<>(purchaseRequisitionEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Purchase Requisition. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param purchaseRequisitionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePurchaseRequisition' endpoint here] off begin
	@Operation(
			description = "Update a PurchaseRequisition entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PurchaseRequisition."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PurchaseRequisition, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PurchaseRequisitionEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePurchaseRequisition' endpoint here] end
	public ResponseEntity<?> updatePurchaseRequisition(
			@Parameter(description = "The PurchaseRequisitionEntity to create or update.")
			@RequestBody PurchaseRequisitionEntityDto purchaseRequisitionEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PurchaseRequisitionEntity> checkPurchaseRequisitionExists = purchaseRequisitionService.findById(purchaseRequisitionEntityDto.getId());
		PurchaseRequisitionEntity purchaseRequisitionEntity = new PurchaseRequisitionEntity(purchaseRequisitionEntityDto);

		if (checkPurchaseRequisitionExists.isPresent()) {
			try {
				purchaseRequisitionEntity = purchaseRequisitionService.update(purchaseRequisitionEntity);
				return new ResponseEntity<>(purchaseRequisitionEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(purchaseRequisitionEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Purchase Requisition that has the same id as the given id.
	 *
	 * @param id The id of the PurchaseRequisitionEntity to be deleted
	...
	@Operation(
			description = "Delete a single PurchaseRequisition as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PurchaseRequisition to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisition' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Purchase Requisition entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisition' endpoint here] end
	public void exportPurchaseRequisition(
			@Parameter(description = "The ids of the PurchaseRequisitionEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PurchaseRequisitionEntity> entitiesToExport = purchaseRequisitionService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PurchaseRequisition_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PurchaseRequisitionEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisitionExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchaseRequisitionExcludingIds' endpoint here] end
	public void exportPurchaseRequisitionExcludingIds(
			@Parameter(description = "The ids of the PurchaseRequisitionEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PurchaseRequisitionEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = purchaseRequisitionService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = purchaseRequisitionService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PurchaseRequisition entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PurchaseRequisition_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PurchaseRequisitionEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Purchase Requisition entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PurchaseRequisitionEntity', 'create')  || hasPermission('PurchaseRequisitionEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PurchaseRequisitionEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PurchaseRequisitionEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PurchaseRequisitionEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PurchaseRequisitionEntity> csvToBean = new CsvToBeanBuilder<PurchaseRequisitionEntity>(reader)
				.withType(PurchaseRequisitionEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PurchaseRequisitionEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PurchaseRequisitionEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Purchase Requisition entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Purchase Requisition entity and you do not have permission");
					}
