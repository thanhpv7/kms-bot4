...
import kmsweb.entities.InvoiceSummaryEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Invoice Summary
 */
// % protected region % [Update the API route or description for the Invoice SummaryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InvoiceSummaryController"))
@RestController
@Slf4j
@RequestMapping("/api/invoicesummary")
// % protected region % [Update the API route or description for the Invoice SummaryController here] end
public class InvoiceSummaryController {

	private final InvoiceSummaryService invoiceSummaryService;
	private final CsvUtils<InvoiceSummaryEntity, InvoiceSummaryRepository, InvoiceSummaryEntityAudit,  InvoiceSummaryService> csvUtils;
	...
	public InvoiceSummaryController(
		...
		this.csvUtils = new CsvUtils<>(invoiceSummaryService, InvoiceSummaryEntity.class);
	...
	public ResponseEntity<List<InvoiceSummaryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InvoiceSummaryEntity> invoiceSummarys = invoiceSummaryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(invoiceSummarys, HttpStatus.OK);
	}

	/**
	 * Return the Invoice Summary that has the same id as the given id.
	 *
	 * @param id      The id of the InvoiceSummaryEntity to be returned
	 * @return the Invoice Summary that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InvoiceSummary as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InvoiceSummary entity not found"),
	})
	@PreAuthorize("hasPermission('InvoiceSummaryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InvoiceSummaryEntity> getById(
			@Parameter(description = "The UUID of the InvoiceSummary to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InvoiceSummaryEntity> invoiceSummaryEntity = invoiceSummaryService.findById(id);
	...
	/**
	 * Create/Update the given Invoice Summary. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceSummaryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInvoiceSummary' endpoint here] off begin
	@Operation(
			description = "Create a InvoiceSummary if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InvoiceSummary."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InvoiceSummary, duplicate record found")
	})
	@PreAuthorize("hasPermission('InvoiceSummaryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInvoiceSummary' endpoint here] end
	public ResponseEntity<?> saveInvoiceSummary(
			@Parameter(description = "The InvoiceSummaryEntity to create or update.")
			@RequestBody InvoiceSummaryEntityDto invoiceSummaryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InvoiceSummaryEntity invoiceSummaryEntity = invoiceSummaryService.create(new InvoiceSummaryEntity(invoiceSummaryEntityDto));
			return new ResponseEntity<>(invoiceSummaryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Invoice Summary. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceSummaryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInvoiceSummary' endpoint here] off begin
	@Operation(
			description = "Update a InvoiceSummary entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InvoiceSummary."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InvoiceSummary, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InvoiceSummaryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInvoiceSummary' endpoint here] end
	public ResponseEntity<?> updateInvoiceSummary(
			@Parameter(description = "The InvoiceSummaryEntity to create or update.")
			@RequestBody InvoiceSummaryEntityDto invoiceSummaryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<InvoiceSummaryEntity> checkInvoiceSummaryExists = invoiceSummaryService.findById(invoiceSummaryEntityDto.getId());
		InvoiceSummaryEntity invoiceSummaryEntity = new InvoiceSummaryEntity(invoiceSummaryEntityDto);

		if (checkInvoiceSummaryExists.isPresent()) {
			try {
				invoiceSummaryEntity = invoiceSummaryService.update(invoiceSummaryEntity);
				return new ResponseEntity<>(invoiceSummaryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(invoiceSummaryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Invoice Summary that has the same id as the given id.
	 *
	 * @param id The id of the InvoiceSummaryEntity to be deleted
	...
	@Operation(
			description = "Delete a single InvoiceSummary as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InvoiceSummary to delete.")
		...
		invoiceSummaryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoiceSummary' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Invoice Summary entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoiceSummary' endpoint here] end
	public void exportInvoiceSummary(
			@Parameter(description = "The ids of the InvoiceSummaryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InvoiceSummaryEntity> entitiesToExport = invoiceSummaryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InvoiceSummary_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InvoiceSummaryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoiceSummaryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoiceSummaryExcludingIds' endpoint here] end
	public void exportInvoiceSummaryExcludingIds(
			@Parameter(description = "The ids of the InvoiceSummaryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InvoiceSummaryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = invoiceSummaryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = invoiceSummaryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InvoiceSummary entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InvoiceSummary_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InvoiceSummaryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Invoice Summary entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InvoiceSummaryEntity', 'create')  || hasPermission('InvoiceSummaryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InvoiceSummaryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InvoiceSummaryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InvoiceSummaryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InvoiceSummaryEntity> csvToBean = new CsvToBeanBuilder<InvoiceSummaryEntity>(reader)
				.withType(InvoiceSummaryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InvoiceSummaryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InvoiceSummaryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Invoice Summary entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Invoice Summary entity and you do not have permission");
					}
