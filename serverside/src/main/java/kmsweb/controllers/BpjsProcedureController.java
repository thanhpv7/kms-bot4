...
import kmsweb.entities.BpjsProcedureEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS Procedure
 */
// % protected region % [Update the API route or description for the BPJS ProcedureController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsProcedureController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsprocedure")
// % protected region % [Update the API route or description for the BPJS ProcedureController here] end
public class BpjsProcedureController {

	private final BpjsProcedureService bpjsProcedureService;
	private final CsvUtils<BpjsProcedureEntity, BpjsProcedureRepository, BpjsProcedureEntityAudit,  BpjsProcedureService> csvUtils;
	...
	@Autowired
	public BpjsProcedureController(
		...
		this.csvUtils = new CsvUtils<>(bpjsProcedureService, BpjsProcedureEntity.class);
	...
	public ResponseEntity<List<BpjsProcedureEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsProcedureEntity> bpjsProcedures = bpjsProcedureService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsProcedures, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Procedure that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsProcedureEntity to be returned
	 * @return the BPJS Procedure that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsProcedure as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsProcedure entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsProcedureEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsProcedureEntity> getById(
			@Parameter(description = "The UUID of the BpjsProcedure to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsProcedureEntity> bpjsProcedureEntity = bpjsProcedureService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Procedure. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsProcedureEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsProcedure' endpoint here] off begin
	@Operation(
			description = "Create a BpjsProcedure if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsProcedure."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsProcedure, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsProcedureEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsProcedure' endpoint here] end
	public ResponseEntity<?> saveBpjsProcedure(
			@Parameter(description = "The BpjsProcedureEntity to create or update.")
			@RequestBody BpjsProcedureEntityDto bpjsProcedureEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsProcedureEntity bpjsProcedureEntity = bpjsProcedureService.create(new BpjsProcedureEntity(bpjsProcedureEntityDto));
			return new ResponseEntity<>(bpjsProcedureEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Procedure. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsProcedureEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsProcedure' endpoint here] off begin
	@Operation(
			description = "Update a BpjsProcedure entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsProcedure."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsProcedure, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsProcedureEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsProcedure' endpoint here] end
	public ResponseEntity<?> updateBpjsProcedure(
			@Parameter(description = "The BpjsProcedureEntity to create or update.")
			@RequestBody BpjsProcedureEntityDto bpjsProcedureEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsProcedureEntity> checkBpjsProcedureExists = bpjsProcedureService.findById(bpjsProcedureEntityDto.getId());
		BpjsProcedureEntity bpjsProcedureEntity = new BpjsProcedureEntity(bpjsProcedureEntityDto);

		if (checkBpjsProcedureExists.isPresent()) {
			try {
				bpjsProcedureEntity = bpjsProcedureService.update(bpjsProcedureEntity);
				return new ResponseEntity<>(bpjsProcedureEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsProcedureEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Procedure that has the same id as the given id.
	 *
	 * @param id The id of the BpjsProcedureEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsProcedure as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsProcedure to delete.")
		...
		bpjsProcedureService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsProcedure' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Procedure entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsProcedure' endpoint here] end
	public void exportBpjsProcedure(
			@Parameter(description = "The ids of the BpjsProcedureEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsProcedureEntity> entitiesToExport = bpjsProcedureService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsProcedure_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsProcedureEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsProcedureExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsProcedureExcludingIds' endpoint here] end
	public void exportBpjsProcedureExcludingIds(
			@Parameter(description = "The ids of the BpjsProcedureEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsProcedureEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsProcedureService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsProcedureService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsProcedure entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsProcedure_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsProcedureEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Procedure entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsProcedureEntity', 'create')  || hasPermission('BpjsProcedureEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsProcedureEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsProcedureEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsProcedureEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsProcedureEntity> csvToBean = new CsvToBeanBuilder<BpjsProcedureEntity>(reader)
				.withType(BpjsProcedureEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsProcedureEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsProcedureEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Procedure entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Procedure entity and you do not have permission");
					}
