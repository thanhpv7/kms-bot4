...
import kmsweb.entities.BpjsSEPEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS SEP
 */
// % protected region % [Update the API route or description for the BPJS SEPController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsSEPController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjssep")
// % protected region % [Update the API route or description for the BPJS SEPController here] end
public class BpjsSEPController {

	private final BpjsSEPService bpjsSEPService;
	private final CsvUtils<BpjsSEPEntity, BpjsSEPRepository, BpjsSEPEntityAudit,  BpjsSEPService> csvUtils;
	...
	public BpjsSEPController(
		...
		this.csvUtils = new CsvUtils<>(bpjsSEPService, BpjsSEPEntity.class);
	...
	public ResponseEntity<List<BpjsSEPEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsSEPEntity> bpjsSEPs = bpjsSEPService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsSEPs, HttpStatus.OK);
	}

	/**
	 * Return the BPJS SEP that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsSEPEntity to be returned
	 * @return the BPJS SEP that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsSEP as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsSEP entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsSEPEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsSEPEntity> getById(
			@Parameter(description = "The UUID of the BpjsSEP to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsSEPEntity> bpjsSEPEntity = bpjsSEPService.findById(id);
	...
	/**
	 * Create/Update the given BPJS SEP. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEP' endpoint here] off begin
	@Operation(
			description = "Create a BpjsSEP if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsSEP."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsSEP, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsSEPEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEP' endpoint here] end
	public ResponseEntity<?> saveBpjsSEP(
			@Parameter(description = "The BpjsSEPEntity to create or update.")
			@RequestBody BpjsSEPEntityDto bpjsSEPEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsSEPEntity bpjsSEPEntity = bpjsSEPService.create(new BpjsSEPEntity(bpjsSEPEntityDto));
			return new ResponseEntity<>(bpjsSEPEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS SEP. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEP' endpoint here] off begin
	@Operation(
			description = "Update a BpjsSEP entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsSEP."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsSEP, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsSEPEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEP' endpoint here] end
	public ResponseEntity<?> updateBpjsSEP(
			@Parameter(description = "The BpjsSEPEntity to create or update.")
			@RequestBody BpjsSEPEntityDto bpjsSEPEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsSEPEntity> checkBpjsSEPExists = bpjsSEPService.findById(bpjsSEPEntityDto.getId());
		BpjsSEPEntity bpjsSEPEntity = new BpjsSEPEntity(bpjsSEPEntityDto);

		if (checkBpjsSEPExists.isPresent()) {
			try {
				bpjsSEPEntity = bpjsSEPService.update(bpjsSEPEntity);
				return new ResponseEntity<>(bpjsSEPEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsSEPEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS SEP that has the same id as the given id.
	 *
	 * @param id The id of the BpjsSEPEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsSEP as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsSEP to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEP' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS SEP entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEP' endpoint here] end
	public void exportBpjsSEP(
			@Parameter(description = "The ids of the BpjsSEPEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsSEPEntity> entitiesToExport = bpjsSEPService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsSEP_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsSEPEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPExcludingIds' endpoint here] end
	public void exportBpjsSEPExcludingIds(
			@Parameter(description = "The ids of the BpjsSEPEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsSEPEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsSEPService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsSEPService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsSEP entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsSEP_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsSEPEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS SEP entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsSEPEntity', 'create')  || hasPermission('BpjsSEPEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsSEPEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsSEPEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsSEPEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsSEPEntity> csvToBean = new CsvToBeanBuilder<BpjsSEPEntity>(reader)
				.withType(BpjsSEPEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsSEPEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsSEPEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS SEP entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS SEP entity and you do not have permission");
					}
