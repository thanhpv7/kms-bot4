...
import kmsweb.entities.PrivilegeEntityAudit;
 ...
 */
// % protected region % [Update the API route or description for the PrivilegeController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PrivilegeController"))
@RestController
@Slf4j
@RequestMapping("/api/privilege")
// % protected region % [Update the API route or description for the PrivilegeController here] end
public class PrivilegeController {

	private final PrivilegeService privilegeService;
	private final CsvUtils<PrivilegeEntity, PrivilegeRepository, PrivilegeEntityAudit,  PrivilegeService> csvUtils;
	...
	public PrivilegeController(
		...
		this.csvUtils = new CsvUtils<>(privilegeService, PrivilegeEntity.class);
	...
	@Operation(
			description = "Returns a single page of Privileges",
		security = {
	...
	@Operation(
			description = "Return a single Privilege as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "Privilege entity not found"),
	})
	@PreAuthorize("hasPermission('PrivilegeEntity', 'read')")
	...
	public ResponseEntity<PrivilegeEntity> getById(
			@Parameter(description = "The UUID of the Privilege to return.")
	 ...
	 *
	 * @param privilegeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePrivilege' endpoint here] off begin
	@Operation(
			description = "Create a Privilege if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new Privilege."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new Privilege, duplicate record found")
	})
	@PreAuthorize("hasPermission('PrivilegeEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePrivilege' endpoint here] end
	public ResponseEntity<?> savePrivilege(
			@Parameter(description = "The PrivilegeEntity to create or update.")
			@RequestBody PrivilegeEntityDto privilegeEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PrivilegeEntity privilegeEntity = privilegeService.create(new PrivilegeEntity(privilegeEntityDto));
	...
	/**
	 * Update the given Privilege. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param privilegeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePrivilege' endpoint here] off begin
	@Operation(
			description = "Update a Privilege entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the Privilege."),
		@ApiResponse(responseCode = "400", description = "Failed to update the Privilege, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PrivilegeEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePrivilege' endpoint here] end
	public ResponseEntity<?> updatePrivilege(
		...
		Optional<PrivilegeEntity> checkPrivilegeExists = privilegeService.findById(privilegeEntityDto.getId());
		PrivilegeEntity privilegeEntity = new PrivilegeEntity(privilegeEntityDto);

		if (checkPrivilegeExists.isPresent()) {
			try {
		...
		return new ResponseEntity<>(privilegeEntity, HttpStatus.BAD_REQUEST);
	}
	...
	@Operation(
			description = "Delete a single Privilege as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the Privilege to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrivilege' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Privilege entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPrivilege' endpoint here] end
	public void exportPrivilege(
			@Parameter(description = "The ids of the PrivilegeEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PrivilegeEntity> entitiesToExport = privilegeService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "Privilege_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PrivilegeEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrivilegeExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPrivilegeExcludingIds' endpoint here] end
	public void exportPrivilegeExcludingIds(
			@Parameter(description = "The ids of the PrivilegeEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PrivilegeEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = privilegeService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = privilegeService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the Privilege entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "Privilege_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PrivilegeEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Privilege entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PrivilegeEntity', 'create')  || hasPermission('PrivilegeEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PrivilegeEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PrivilegeEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PrivilegeEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PrivilegeEntity> csvToBean = new CsvToBeanBuilder<PrivilegeEntity>(reader)
				.withType(PrivilegeEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PrivilegeEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PrivilegeEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Privilege entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Privilege entity and you do not have permission");
					}
