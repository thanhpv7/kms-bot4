...
import kmsweb.entities.BpjsSEPSubmissionEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding BPJS SEP Submission
 */
// % protected region % [Update the API route or description for the BPJS SEP SubmissionController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsSEPSubmissionController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjssepsubmission")
// % protected region % [Update the API route or description for the BPJS SEP SubmissionController here] end
public class BpjsSEPSubmissionController {

	private final BpjsSEPSubmissionService bpjsSEPSubmissionService;
	private final CsvUtils<BpjsSEPSubmissionEntity, BpjsSEPSubmissionRepository, BpjsSEPSubmissionEntityAudit,  BpjsSEPSubmissionService> csvUtils;
	...
	@Autowired
	public BpjsSEPSubmissionController(
		...
		this.csvUtils = new CsvUtils<>(bpjsSEPSubmissionService, BpjsSEPSubmissionEntity.class);
	...
	public ResponseEntity<List<BpjsSEPSubmissionEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsSEPSubmissionEntity> bpjsSEPSubmissions = bpjsSEPSubmissionService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsSEPSubmissions, HttpStatus.OK);
	}

	/**
	 * Return the BPJS SEP Submission that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsSEPSubmissionEntity to be returned
	 * @return the BPJS SEP Submission that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsSEPSubmission as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsSEPSubmission entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsSEPSubmissionEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsSEPSubmissionEntity> getById(
			@Parameter(description = "The UUID of the BpjsSEPSubmission to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsSEPSubmissionEntity> bpjsSEPSubmissionEntity = bpjsSEPSubmissionService.findById(id);
	...
	/**
	 * Create/Update the given BPJS SEP Submission. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPSubmissionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEPSubmission' endpoint here] off begin
	@Operation(
			description = "Create a BpjsSEPSubmission if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsSEPSubmission."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsSEPSubmission, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsSEPSubmissionEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsSEPSubmission' endpoint here] end
	public ResponseEntity<?> saveBpjsSEPSubmission(
			@Parameter(description = "The BpjsSEPSubmissionEntity to create or update.")
			@RequestBody BpjsSEPSubmissionEntityDto bpjsSEPSubmissionEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsSEPSubmissionEntity bpjsSEPSubmissionEntity = bpjsSEPSubmissionService.create(new BpjsSEPSubmissionEntity(bpjsSEPSubmissionEntityDto));
			return new ResponseEntity<>(bpjsSEPSubmissionEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS SEP Submission. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsSEPSubmissionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEPSubmission' endpoint here] off begin
	@Operation(
			description = "Update a BpjsSEPSubmission entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsSEPSubmission."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsSEPSubmission, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsSEPSubmissionEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsSEPSubmission' endpoint here] end
	public ResponseEntity<?> updateBpjsSEPSubmission(
			@Parameter(description = "The BpjsSEPSubmissionEntity to create or update.")
			@RequestBody BpjsSEPSubmissionEntityDto bpjsSEPSubmissionEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsSEPSubmissionEntity> checkBpjsSEPSubmissionExists = bpjsSEPSubmissionService.findById(bpjsSEPSubmissionEntityDto.getId());
		BpjsSEPSubmissionEntity bpjsSEPSubmissionEntity = new BpjsSEPSubmissionEntity(bpjsSEPSubmissionEntityDto);

		if (checkBpjsSEPSubmissionExists.isPresent()) {
			try {
				bpjsSEPSubmissionEntity = bpjsSEPSubmissionService.update(bpjsSEPSubmissionEntity);
				return new ResponseEntity<>(bpjsSEPSubmissionEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsSEPSubmissionEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS SEP Submission that has the same id as the given id.
	 *
	 * @param id The id of the BpjsSEPSubmissionEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsSEPSubmission as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsSEPSubmission to delete.")
		...
		bpjsSEPSubmissionService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPSubmission' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS SEP Submission entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPSubmission' endpoint here] end
	public void exportBpjsSEPSubmission(
			@Parameter(description = "The ids of the BpjsSEPSubmissionEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsSEPSubmissionEntity> entitiesToExport = bpjsSEPSubmissionService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsSEPSubmission_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsSEPSubmissionEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPSubmissionExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsSEPSubmissionExcludingIds' endpoint here] end
	public void exportBpjsSEPSubmissionExcludingIds(
			@Parameter(description = "The ids of the BpjsSEPSubmissionEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsSEPSubmissionEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsSEPSubmissionService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsSEPSubmissionService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsSEPSubmission entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsSEPSubmission_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsSEPSubmissionEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS SEP Submission entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsSEPSubmissionEntity', 'create')  || hasPermission('BpjsSEPSubmissionEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsSEPSubmissionEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsSEPSubmissionEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsSEPSubmissionEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsSEPSubmissionEntity> csvToBean = new CsvToBeanBuilder<BpjsSEPSubmissionEntity>(reader)
				.withType(BpjsSEPSubmissionEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsSEPSubmissionEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsSEPSubmissionEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS SEP Submission entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS SEP Submission entity and you do not have permission");
					}
