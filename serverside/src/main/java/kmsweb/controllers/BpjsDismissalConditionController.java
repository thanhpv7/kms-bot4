...
/**
 * Controller used to handle all REST operations regarding BPJS Dismissal Condition
 */
// % protected region % [Update the API route or description for the BPJS Dismissal ConditionController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsDismissalConditionController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsdismissalcondition")
// % protected region % [Update the API route or description for the BPJS Dismissal ConditionController here] end
public class BpjsDismissalConditionController {

	private final BpjsDismissalConditionService bpjsDismissalConditionService;
	private final CsvUtils<BpjsDismissalConditionEntity, BpjsDismissalConditionRepository, BpjsDismissalConditionEntityAudit,  BpjsDismissalConditionService> csvUtils;
	...
	@Autowired
	public BpjsDismissalConditionController(
		...
		this.csvUtils = new CsvUtils<>(bpjsDismissalConditionService, BpjsDismissalConditionEntity.class);
	...
	public ResponseEntity<List<BpjsDismissalConditionEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsDismissalConditionEntity> bpjsDismissalConditions = bpjsDismissalConditionService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsDismissalConditions, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Dismissal Condition that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsDismissalConditionEntity to be returned
	 * @return the BPJS Dismissal Condition that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsDismissalCondition as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsDismissalCondition entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsDismissalConditionEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsDismissalConditionEntity> getById(
			@Parameter(description = "The UUID of the BpjsDismissalCondition to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsDismissalConditionEntity> bpjsDismissalConditionEntity = bpjsDismissalConditionService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Dismissal Condition. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDismissalConditionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsDismissalCondition' endpoint here] off begin
	@Operation(
			description = "Create a BpjsDismissalCondition if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsDismissalCondition."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsDismissalCondition, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsDismissalConditionEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsDismissalCondition' endpoint here] end
	public ResponseEntity<?> saveBpjsDismissalCondition(
			@Parameter(description = "The BpjsDismissalConditionEntity to create or update.")
			@RequestBody BpjsDismissalConditionEntityDto bpjsDismissalConditionEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsDismissalConditionEntity bpjsDismissalConditionEntity = bpjsDismissalConditionService.create(new BpjsDismissalConditionEntity(bpjsDismissalConditionEntityDto));
			return new ResponseEntity<>(bpjsDismissalConditionEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Dismissal Condition. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsDismissalConditionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsDismissalCondition' endpoint here] off begin
	@Operation(
			description = "Update a BpjsDismissalCondition entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsDismissalCondition."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsDismissalCondition, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsDismissalConditionEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsDismissalCondition' endpoint here] end
	public ResponseEntity<?> updateBpjsDismissalCondition(
			@Parameter(description = "The BpjsDismissalConditionEntity to create or update.")
			@RequestBody BpjsDismissalConditionEntityDto bpjsDismissalConditionEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsDismissalConditionEntity> checkBpjsDismissalConditionExists = bpjsDismissalConditionService.findById(bpjsDismissalConditionEntityDto.getId());
		BpjsDismissalConditionEntity bpjsDismissalConditionEntity = new BpjsDismissalConditionEntity(bpjsDismissalConditionEntityDto);

		if (checkBpjsDismissalConditionExists.isPresent()) {
			try {
				bpjsDismissalConditionEntity = bpjsDismissalConditionService.update(bpjsDismissalConditionEntity);
				return new ResponseEntity<>(bpjsDismissalConditionEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsDismissalConditionEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Dismissal Condition that has the same id as the given id.
	 *
	 * @param id The id of the BpjsDismissalConditionEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsDismissalCondition as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsDismissalCondition to delete.")
		...
		bpjsDismissalConditionService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalCondition' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Dismissal Condition entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalCondition' endpoint here] end
	public void exportBpjsDismissalCondition(
			@Parameter(description = "The ids of the BpjsDismissalConditionEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsDismissalConditionEntity> entitiesToExport = bpjsDismissalConditionService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsDismissalCondition_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsDismissalConditionEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalConditionExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsDismissalConditionExcludingIds' endpoint here] end
	public void exportBpjsDismissalConditionExcludingIds(
			@Parameter(description = "The ids of the BpjsDismissalConditionEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsDismissalConditionEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsDismissalConditionService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsDismissalConditionService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsDismissalCondition entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsDismissalCondition_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsDismissalConditionEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Dismissal Condition entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsDismissalConditionEntity', 'create')  || hasPermission('BpjsDismissalConditionEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsDismissalConditionEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsDismissalConditionEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsDismissalConditionEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsDismissalConditionEntity> csvToBean = new CsvToBeanBuilder<BpjsDismissalConditionEntity>(reader)
				.withType(BpjsDismissalConditionEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsDismissalConditionEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsDismissalConditionEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Dismissal Condition entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Dismissal Condition entity and you do not have permission");
					}
