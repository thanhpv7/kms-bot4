...
import kmsweb.entities.PatientPaymentSelfPayingEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Payment  Self Paying
 */
// % protected region % [Update the API route or description for the Patient Payment  Self PayingController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientPaymentSelfPayingController"))
@RestController
@Slf4j
@RequestMapping("/api/patientpaymentselfpaying")
// % protected region % [Update the API route or description for the Patient Payment  Self PayingController here] end
public class PatientPaymentSelfPayingController {

	private final PatientPaymentSelfPayingService patientPaymentSelfPayingService;
	private final CsvUtils<PatientPaymentSelfPayingEntity, PatientPaymentSelfPayingRepository, PatientPaymentSelfPayingEntityAudit,  PatientPaymentSelfPayingService> csvUtils;
	...
	@Autowired
	public PatientPaymentSelfPayingController(
		...
		this.csvUtils = new CsvUtils<>(patientPaymentSelfPayingService, PatientPaymentSelfPayingEntity.class);
	...
	/**
	 * Return all the Patient Payment  Self Payings.
	 *
	 * @return all the Patient Payment  Self Payings
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of PatientPaymentSelfPayings",
		security = {
	...
	public ResponseEntity<List<PatientPaymentSelfPayingEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientPaymentSelfPayingEntity> patientPaymentSelfPayings = patientPaymentSelfPayingService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientPaymentSelfPayings, HttpStatus.OK);
	}

	/**
	 * Return the Patient Payment  Self Paying that has the same id as the given id.
	 *
	 * @param id      The id of the PatientPaymentSelfPayingEntity to be returned
	 * @return the Patient Payment  Self Paying that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientPaymentSelfPaying as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientPaymentSelfPaying entity not found"),
	})
	@PreAuthorize("hasPermission('PatientPaymentSelfPayingEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientPaymentSelfPayingEntity> getById(
			@Parameter(description = "The UUID of the PatientPaymentSelfPaying to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientPaymentSelfPayingEntity> patientPaymentSelfPayingEntity = patientPaymentSelfPayingService.findById(id);
	...
	/**
	 * Create/Update the given Patient Payment  Self Paying. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentSelfPayingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Create a PatientPaymentSelfPaying if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientPaymentSelfPaying."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientPaymentSelfPaying, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientPaymentSelfPayingEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientPaymentSelfPaying' endpoint here] end
	public ResponseEntity<?> savePatientPaymentSelfPaying(
			@Parameter(description = "The PatientPaymentSelfPayingEntity to create or update.")
			@RequestBody PatientPaymentSelfPayingEntityDto patientPaymentSelfPayingEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientPaymentSelfPayingEntity patientPaymentSelfPayingEntity = patientPaymentSelfPayingService.create(new PatientPaymentSelfPayingEntity(patientPaymentSelfPayingEntityDto));
			return new ResponseEntity<>(patientPaymentSelfPayingEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Payment  Self Paying. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientPaymentSelfPayingEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Update a PatientPaymentSelfPaying entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientPaymentSelfPaying."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientPaymentSelfPaying, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientPaymentSelfPayingEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientPaymentSelfPaying' endpoint here] end
	public ResponseEntity<?> updatePatientPaymentSelfPaying(
			@Parameter(description = "The PatientPaymentSelfPayingEntity to create or update.")
			@RequestBody PatientPaymentSelfPayingEntityDto patientPaymentSelfPayingEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientPaymentSelfPayingEntity> checkPatientPaymentSelfPayingExists = patientPaymentSelfPayingService.findById(patientPaymentSelfPayingEntityDto.getId());
		PatientPaymentSelfPayingEntity patientPaymentSelfPayingEntity = new PatientPaymentSelfPayingEntity(patientPaymentSelfPayingEntityDto);

		if (checkPatientPaymentSelfPayingExists.isPresent()) {
			try {
				patientPaymentSelfPayingEntity = patientPaymentSelfPayingService.update(patientPaymentSelfPayingEntity);
				return new ResponseEntity<>(patientPaymentSelfPayingEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientPaymentSelfPayingEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Payment  Self Paying that has the same id as the given id.
	 *
	 * @param id The id of the PatientPaymentSelfPayingEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientPaymentSelfPaying as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientPaymentSelfPaying to delete.")
		...
		patientPaymentSelfPayingService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentSelfPaying' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Payment  Self Paying entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentSelfPaying' endpoint here] end
	public void exportPatientPaymentSelfPaying(
			@Parameter(description = "The ids of the PatientPaymentSelfPayingEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientPaymentSelfPayingEntity> entitiesToExport = patientPaymentSelfPayingService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientPaymentSelfPaying_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientPaymentSelfPayingEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentSelfPayingExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientPaymentSelfPayingExcludingIds' endpoint here] end
	public void exportPatientPaymentSelfPayingExcludingIds(
			@Parameter(description = "The ids of the PatientPaymentSelfPayingEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientPaymentSelfPayingEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientPaymentSelfPayingService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientPaymentSelfPayingService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientPaymentSelfPaying entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientPaymentSelfPaying_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientPaymentSelfPayingEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Payment  Self Paying entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientPaymentSelfPayingEntity', 'create')  || hasPermission('PatientPaymentSelfPayingEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientPaymentSelfPayingEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientPaymentSelfPayingEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientPaymentSelfPayingEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientPaymentSelfPayingEntity> csvToBean = new CsvToBeanBuilder<PatientPaymentSelfPayingEntity>(reader)
				.withType(PatientPaymentSelfPayingEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientPaymentSelfPayingEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientPaymentSelfPayingEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Payment  Self Paying entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Payment  Self Paying entity and you do not have permission");
					}
