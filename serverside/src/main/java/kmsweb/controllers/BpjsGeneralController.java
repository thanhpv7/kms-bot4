...
/**
 * Controller used to handle all REST operations regarding BPJS General
 */
// % protected region % [Update the API route or description for the BPJS GeneralController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsGeneralController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsgeneral")
// % protected region % [Update the API route or description for the BPJS GeneralController here] end
public class BpjsGeneralController {

	private final BpjsGeneralService bpjsGeneralService;
	private final CsvUtils<BpjsGeneralEntity, BpjsGeneralRepository, BpjsGeneralEntityAudit,  BpjsGeneralService> csvUtils;
	...
	@Autowired
	public BpjsGeneralController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsGeneralService bpjsGeneralService
	) {
		this.authService = authService;
		this.bpjsGeneralService = bpjsGeneralService;
		this.csvUtils = new CsvUtils<>(bpjsGeneralService, BpjsGeneralEntity.class);
	...
	public ResponseEntity<List<BpjsGeneralEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsGeneralEntity> bpjsGenerals = bpjsGeneralService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsGenerals, HttpStatus.OK);
	}

	/**
	 * Return the BPJS General that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsGeneralEntity to be returned
	 * @return the BPJS General that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsGeneral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsGeneral entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsGeneralEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsGeneralEntity> getById(
			@Parameter(description = "The UUID of the BpjsGeneral to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsGeneralEntity> bpjsGeneralEntity = bpjsGeneralService.findById(id);
	...
	/**
	 * Create/Update the given BPJS General. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsGeneralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsGeneral' endpoint here] off begin
	@Operation(
			description = "Create a BpjsGeneral if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsGeneral."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsGeneral, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsGeneralEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsGeneral' endpoint here] end
	public ResponseEntity<?> saveBpjsGeneral(
			@Parameter(description = "The BpjsGeneralEntity to create or update.")
			@RequestBody BpjsGeneralEntityDto bpjsGeneralEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsGeneralEntity bpjsGeneralEntity = bpjsGeneralService.create(new BpjsGeneralEntity(bpjsGeneralEntityDto));
			return new ResponseEntity<>(bpjsGeneralEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS General. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsGeneralEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsGeneral' endpoint here] off begin
	@Operation(
			description = "Update a BpjsGeneral entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsGeneral."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsGeneral, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsGeneralEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsGeneral' endpoint here] end
	public ResponseEntity<?> updateBpjsGeneral(
			@Parameter(description = "The BpjsGeneralEntity to create or update.")
			@RequestBody BpjsGeneralEntityDto bpjsGeneralEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsGeneralEntity> checkBpjsGeneralExists = bpjsGeneralService.findById(bpjsGeneralEntityDto.getId());
		BpjsGeneralEntity bpjsGeneralEntity = new BpjsGeneralEntity(bpjsGeneralEntityDto);

		if (checkBpjsGeneralExists.isPresent()) {
			try {
				bpjsGeneralEntity = bpjsGeneralService.update(bpjsGeneralEntity);
				return new ResponseEntity<>(bpjsGeneralEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsGeneralEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS General that has the same id as the given id.
	 *
	 * @param id The id of the BpjsGeneralEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsGeneral as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsGeneral to delete.")
		...
		bpjsGeneralService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsGeneral' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS General entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsGeneral' endpoint here] end
	public void exportBpjsGeneral(
			@Parameter(description = "The ids of the BpjsGeneralEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsGeneralEntity> entitiesToExport = bpjsGeneralService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsGeneral_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsGeneralEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsGeneralExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsGeneralExcludingIds' endpoint here] end
	public void exportBpjsGeneralExcludingIds(
			@Parameter(description = "The ids of the BpjsGeneralEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsGeneralEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsGeneralService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsGeneralService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsGeneral entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsGeneral_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsGeneralEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS General entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsGeneralEntity', 'create')  || hasPermission('BpjsGeneralEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsGeneralEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsGeneralEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsGeneralEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsGeneralEntity> csvToBean = new CsvToBeanBuilder<BpjsGeneralEntity>(reader)
				.withType(BpjsGeneralEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsGeneralEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsGeneralEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS General entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS General entity and you do not have permission");
					}
