...
/**
 * Controller used to handle all REST operations regarding PCare Club Prolanis
 */
// % protected region % [Update the API route or description for the PCare Club ProlanisController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareClubProlanisController"))
@RestController
@Slf4j
@RequestMapping("/api/pcareclubprolanis")
// % protected region % [Update the API route or description for the PCare Club ProlanisController here] end
public class PcareClubProlanisController {

	private final PcareClubProlanisService pcareClubProlanisService;
	private final CsvUtils<PcareClubProlanisEntity, PcareClubProlanisRepository, PcareClubProlanisEntityAudit,  PcareClubProlanisService> csvUtils;
	...
	@Autowired
	public PcareClubProlanisController(
		...
		this.csvUtils = new CsvUtils<>(pcareClubProlanisService, PcareClubProlanisEntity.class);
	...
	public ResponseEntity<List<PcareClubProlanisEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareClubProlanisEntity> pcareClubProlaniss = pcareClubProlanisService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareClubProlaniss, HttpStatus.OK);
	}

	/**
	 * Return the PCare Club Prolanis that has the same id as the given id.
	 *
	 * @param id      The id of the PcareClubProlanisEntity to be returned
	 * @return the PCare Club Prolanis that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareClubProlanis as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareClubProlanis entity not found"),
	})
	@PreAuthorize("hasPermission('PcareClubProlanisEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareClubProlanisEntity> getById(
			@Parameter(description = "The UUID of the PcareClubProlanis to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareClubProlanisEntity> pcareClubProlanisEntity = pcareClubProlanisService.findById(id);
	...
	/**
	 * Create/Update the given PCare Club Prolanis. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareClubProlanisEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareClubProlanis' endpoint here] off begin
	@Operation(
			description = "Create a PcareClubProlanis if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareClubProlanis."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareClubProlanis, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareClubProlanisEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareClubProlanis' endpoint here] end
	public ResponseEntity<?> savePcareClubProlanis(
			@Parameter(description = "The PcareClubProlanisEntity to create or update.")
			@RequestBody PcareClubProlanisEntityDto pcareClubProlanisEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareClubProlanisEntity pcareClubProlanisEntity = pcareClubProlanisService.create(new PcareClubProlanisEntity(pcareClubProlanisEntityDto));
			return new ResponseEntity<>(pcareClubProlanisEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Club Prolanis. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareClubProlanisEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareClubProlanis' endpoint here] off begin
	@Operation(
			description = "Update a PcareClubProlanis entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareClubProlanis."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareClubProlanis, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareClubProlanisEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareClubProlanis' endpoint here] end
	public ResponseEntity<?> updatePcareClubProlanis(
			@Parameter(description = "The PcareClubProlanisEntity to create or update.")
			@RequestBody PcareClubProlanisEntityDto pcareClubProlanisEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareClubProlanisEntity> checkPcareClubProlanisExists = pcareClubProlanisService.findById(pcareClubProlanisEntityDto.getId());
		PcareClubProlanisEntity pcareClubProlanisEntity = new PcareClubProlanisEntity(pcareClubProlanisEntityDto);

		if (checkPcareClubProlanisExists.isPresent()) {
			try {
				pcareClubProlanisEntity = pcareClubProlanisService.update(pcareClubProlanisEntity);
				return new ResponseEntity<>(pcareClubProlanisEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareClubProlanisEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Club Prolanis that has the same id as the given id.
	 *
	 * @param id The id of the PcareClubProlanisEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareClubProlanis as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareClubProlanis to delete.")
		...
		pcareClubProlanisService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareClubProlanis' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Club Prolanis entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareClubProlanis' endpoint here] end
	public void exportPcareClubProlanis(
			@Parameter(description = "The ids of the PcareClubProlanisEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareClubProlanisEntity> entitiesToExport = pcareClubProlanisService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareClubProlanis_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareClubProlanisEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareClubProlanisExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareClubProlanisExcludingIds' endpoint here] end
	public void exportPcareClubProlanisExcludingIds(
			@Parameter(description = "The ids of the PcareClubProlanisEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareClubProlanisEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareClubProlanisService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareClubProlanisService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareClubProlanis entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareClubProlanis_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareClubProlanisEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Club Prolanis entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareClubProlanisEntity', 'create')  || hasPermission('PcareClubProlanisEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareClubProlanisEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareClubProlanisEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareClubProlanisEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareClubProlanisEntity> csvToBean = new CsvToBeanBuilder<PcareClubProlanisEntity>(reader)
				.withType(PcareClubProlanisEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareClubProlanisEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareClubProlanisEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Club Prolanis entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Club Prolanis entity and you do not have permission");
					}
