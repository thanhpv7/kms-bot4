...
import kmsweb.entities.NhisSpecialtyAttendedEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding NHIS Specialty Attended
 */
// % protected region % [Update the API route or description for the NHIS Specialty AttendedController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisSpecialtyAttendedController"))
@RestController
@Slf4j
@RequestMapping("/api/nhisspecialtyattended")
// % protected region % [Update the API route or description for the NHIS Specialty AttendedController here] end
public class NhisSpecialtyAttendedController {

	private final NhisSpecialtyAttendedService nhisSpecialtyAttendedService;
	private final CsvUtils<NhisSpecialtyAttendedEntity, NhisSpecialtyAttendedRepository, NhisSpecialtyAttendedEntityAudit,  NhisSpecialtyAttendedService> csvUtils;
	...
	@Autowired
	public NhisSpecialtyAttendedController(
		...
		this.csvUtils = new CsvUtils<>(nhisSpecialtyAttendedService, NhisSpecialtyAttendedEntity.class);
	...
	public ResponseEntity<List<NhisSpecialtyAttendedEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisSpecialtyAttendedEntity> nhisSpecialtyAttendeds = nhisSpecialtyAttendedService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisSpecialtyAttendeds, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Specialty Attended that has the same id as the given id.
	 *
	 * @param id      The id of the NhisSpecialtyAttendedEntity to be returned
	 * @return the NHIS Specialty Attended that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisSpecialtyAttended as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisSpecialtyAttended entity not found"),
	})
	@PreAuthorize("hasPermission('NhisSpecialtyAttendedEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisSpecialtyAttendedEntity> getById(
			@Parameter(description = "The UUID of the NhisSpecialtyAttended to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisSpecialtyAttendedEntity> nhisSpecialtyAttendedEntity = nhisSpecialtyAttendedService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Specialty Attended. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisSpecialtyAttendedEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisSpecialtyAttended' endpoint here] off begin
	@Operation(
			description = "Create a NhisSpecialtyAttended if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisSpecialtyAttended."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisSpecialtyAttended, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisSpecialtyAttendedEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisSpecialtyAttended' endpoint here] end
	public ResponseEntity<?> saveNhisSpecialtyAttended(
			@Parameter(description = "The NhisSpecialtyAttendedEntity to create or update.")
			@RequestBody NhisSpecialtyAttendedEntityDto nhisSpecialtyAttendedEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisSpecialtyAttendedEntity nhisSpecialtyAttendedEntity = nhisSpecialtyAttendedService.create(new NhisSpecialtyAttendedEntity(nhisSpecialtyAttendedEntityDto));
			return new ResponseEntity<>(nhisSpecialtyAttendedEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Specialty Attended. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisSpecialtyAttendedEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisSpecialtyAttended' endpoint here] off begin
	@Operation(
			description = "Update a NhisSpecialtyAttended entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisSpecialtyAttended."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisSpecialtyAttended, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisSpecialtyAttendedEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisSpecialtyAttended' endpoint here] end
	public ResponseEntity<?> updateNhisSpecialtyAttended(
			@Parameter(description = "The NhisSpecialtyAttendedEntity to create or update.")
			@RequestBody NhisSpecialtyAttendedEntityDto nhisSpecialtyAttendedEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisSpecialtyAttendedEntity> checkNhisSpecialtyAttendedExists = nhisSpecialtyAttendedService.findById(nhisSpecialtyAttendedEntityDto.getId());
		NhisSpecialtyAttendedEntity nhisSpecialtyAttendedEntity = new NhisSpecialtyAttendedEntity(nhisSpecialtyAttendedEntityDto);

		if (checkNhisSpecialtyAttendedExists.isPresent()) {
			try {
				nhisSpecialtyAttendedEntity = nhisSpecialtyAttendedService.update(nhisSpecialtyAttendedEntity);
				return new ResponseEntity<>(nhisSpecialtyAttendedEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisSpecialtyAttendedEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Specialty Attended that has the same id as the given id.
	 *
	 * @param id The id of the NhisSpecialtyAttendedEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisSpecialtyAttended as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisSpecialtyAttended to delete.")
		...
		nhisSpecialtyAttendedService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisSpecialtyAttended' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Specialty Attended entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisSpecialtyAttended' endpoint here] end
	public void exportNhisSpecialtyAttended(
			@Parameter(description = "The ids of the NhisSpecialtyAttendedEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisSpecialtyAttendedEntity> entitiesToExport = nhisSpecialtyAttendedService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisSpecialtyAttended_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisSpecialtyAttendedEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisSpecialtyAttendedExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisSpecialtyAttendedExcludingIds' endpoint here] end
	public void exportNhisSpecialtyAttendedExcludingIds(
			@Parameter(description = "The ids of the NhisSpecialtyAttendedEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisSpecialtyAttendedEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisSpecialtyAttendedService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisSpecialtyAttendedService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisSpecialtyAttended entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisSpecialtyAttended_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisSpecialtyAttendedEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Specialty Attended entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisSpecialtyAttendedEntity', 'create')  || hasPermission('NhisSpecialtyAttendedEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisSpecialtyAttendedEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisSpecialtyAttendedEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisSpecialtyAttendedEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisSpecialtyAttendedEntity> csvToBean = new CsvToBeanBuilder<NhisSpecialtyAttendedEntity>(reader)
				.withType(NhisSpecialtyAttendedEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisSpecialtyAttendedEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisSpecialtyAttendedEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Specialty Attended entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Specialty Attended entity and you do not have permission");
					}
