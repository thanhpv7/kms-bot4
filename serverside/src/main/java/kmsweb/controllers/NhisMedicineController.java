...
import kmsweb.entities.NhisMedicineEntityAudit;
import kmsweb.repositories.NhisMedicineRepository;
import kmsweb.services.NhisMedicineService;
...
/**
 * Controller used to handle all REST operations regarding NHIS Medicine
 */
// % protected region % [Update the API route or description for the NHIS MedicineController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisMedicineController"))
@RestController
@Slf4j
@RequestMapping("/api/nhismedicine")
// % protected region % [Update the API route or description for the NHIS MedicineController here] end
public class NhisMedicineController {

	private final NhisMedicineService nhisMedicineService;
	private final CsvUtils<NhisMedicineEntity, NhisMedicineRepository, NhisMedicineEntityAudit,  NhisMedicineService> csvUtils;
	...
	@Autowired
	public NhisMedicineController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NhisMedicineService nhisMedicineService
	) {
		this.authService = authService;
		this.nhisMedicineService = nhisMedicineService;
		this.csvUtils = new CsvUtils<>(nhisMedicineService, NhisMedicineEntity.class);
	...
	public ResponseEntity<List<NhisMedicineEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisMedicineEntity> nhisMedicines = nhisMedicineService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisMedicines, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Medicine that has the same id as the given id.
	 *
	 * @param id      The id of the NhisMedicineEntity to be returned
	 * @return the NHIS Medicine that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisMedicine as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisMedicine entity not found"),
	})
	@PreAuthorize("hasPermission('NhisMedicineEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisMedicineEntity> getById(
			@Parameter(description = "The UUID of the NhisMedicine to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisMedicineEntity> nhisMedicineEntity = nhisMedicineService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Medicine. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisMedicineEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisMedicine' endpoint here] off begin
	@Operation(
			description = "Create a NhisMedicine if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisMedicine."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisMedicine, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisMedicineEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisMedicine' endpoint here] end
	public ResponseEntity<?> saveNhisMedicine(
			@Parameter(description = "The NhisMedicineEntity to create or update.")
			@RequestBody NhisMedicineEntityDto nhisMedicineEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisMedicineEntity nhisMedicineEntity = nhisMedicineService.create(new NhisMedicineEntity(nhisMedicineEntityDto));
			return new ResponseEntity<>(nhisMedicineEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Medicine. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisMedicineEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisMedicine' endpoint here] off begin
	@Operation(
			description = "Update a NhisMedicine entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisMedicine."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisMedicine, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisMedicineEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisMedicine' endpoint here] end
	public ResponseEntity<?> updateNhisMedicine(
			@Parameter(description = "The NhisMedicineEntity to create or update.")
			@RequestBody NhisMedicineEntityDto nhisMedicineEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisMedicineEntity> checkNhisMedicineExists = nhisMedicineService.findById(nhisMedicineEntityDto.getId());
		NhisMedicineEntity nhisMedicineEntity = new NhisMedicineEntity(nhisMedicineEntityDto);

		if (checkNhisMedicineExists.isPresent()) {
			try {
				nhisMedicineEntity = nhisMedicineService.update(nhisMedicineEntity);
				return new ResponseEntity<>(nhisMedicineEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisMedicineEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Medicine that has the same id as the given id.
	 *
	 * @param id The id of the NhisMedicineEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisMedicine as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisMedicine to delete.")
		...
		nhisMedicineService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisMedicine' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Medicine entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisMedicine' endpoint here] end
	public void exportNhisMedicine(
			@Parameter(description = "The ids of the NhisMedicineEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisMedicineEntity> entitiesToExport = nhisMedicineService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisMedicine_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisMedicineEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisMedicineExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisMedicineExcludingIds' endpoint here] end
	public void exportNhisMedicineExcludingIds(
			@Parameter(description = "The ids of the NhisMedicineEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisMedicineEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisMedicineService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisMedicineService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisMedicine entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisMedicine_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisMedicineEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Medicine entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisMedicineEntity', 'create')  || hasPermission('NhisMedicineEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisMedicineEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisMedicineEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisMedicineEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisMedicineEntity> csvToBean = new CsvToBeanBuilder<NhisMedicineEntity>(reader)
				.withType(NhisMedicineEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisMedicineEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisMedicineEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Medicine entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Medicine entity and you do not have permission");
					}
