...
/**
 * Controller used to handle all REST operations regarding BPJS PRB
 */
// % protected region % [Update the API route or description for the BPJS PRBController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsPRBController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsprb")
// % protected region % [Update the API route or description for the BPJS PRBController here] end
public class BpjsPRBController {

	private final BpjsPRBService bpjsPRBService;
	private final CsvUtils<BpjsPRBEntity, BpjsPRBRepository, BpjsPRBEntityAudit,  BpjsPRBService> csvUtils;
	...
	public BpjsPRBController(
		...
		this.csvUtils = new CsvUtils<>(bpjsPRBService, BpjsPRBEntity.class);
	...
	public ResponseEntity<List<BpjsPRBEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsPRBEntity> bpjsPRBs = bpjsPRBService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsPRBs, HttpStatus.OK);
	}

	/**
	 * Return the BPJS PRB that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsPRBEntity to be returned
	 * @return the BPJS PRB that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsPRB as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsPRB entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsPRBEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsPRBEntity> getById(
			@Parameter(description = "The UUID of the BpjsPRB to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsPRBEntity> bpjsPRBEntity = bpjsPRBService.findById(id);
	...
	/**
	 * Create/Update the given BPJS PRB. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPRBEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsPRB' endpoint here] off begin
	@Operation(
			description = "Create a BpjsPRB if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsPRB."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsPRB, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsPRBEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsPRB' endpoint here] end
	public ResponseEntity<?> saveBpjsPRB(
			@Parameter(description = "The BpjsPRBEntity to create or update.")
			@RequestBody BpjsPRBEntityDto bpjsPRBEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsPRBEntity bpjsPRBEntity = bpjsPRBService.create(new BpjsPRBEntity(bpjsPRBEntityDto));
			return new ResponseEntity<>(bpjsPRBEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS PRB. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsPRBEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsPRB' endpoint here] off begin
	@Operation(
			description = "Update a BpjsPRB entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsPRB."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsPRB, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsPRBEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsPRB' endpoint here] end
	public ResponseEntity<?> updateBpjsPRB(
			@Parameter(description = "The BpjsPRBEntity to create or update.")
			@RequestBody BpjsPRBEntityDto bpjsPRBEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsPRBEntity> checkBpjsPRBExists = bpjsPRBService.findById(bpjsPRBEntityDto.getId());
		BpjsPRBEntity bpjsPRBEntity = new BpjsPRBEntity(bpjsPRBEntityDto);

		if (checkBpjsPRBExists.isPresent()) {
			try {
				bpjsPRBEntity = bpjsPRBService.update(bpjsPRBEntity);
				return new ResponseEntity<>(bpjsPRBEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsPRBEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS PRB that has the same id as the given id.
	 *
	 * @param id The id of the BpjsPRBEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsPRB as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsPRB to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRB' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS PRB entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRB' endpoint here] end
	public void exportBpjsPRB(
			@Parameter(description = "The ids of the BpjsPRBEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsPRBEntity> entitiesToExport = bpjsPRBService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsPRB_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsPRBEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRBExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsPRBExcludingIds' endpoint here] end
	public void exportBpjsPRBExcludingIds(
			@Parameter(description = "The ids of the BpjsPRBEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsPRBEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsPRBService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsPRBService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsPRB entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsPRB_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsPRBEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS PRB entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsPRBEntity', 'create')  || hasPermission('BpjsPRBEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsPRBEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsPRBEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsPRBEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsPRBEntity> csvToBean = new CsvToBeanBuilder<BpjsPRBEntity>(reader)
				.withType(BpjsPRBEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsPRBEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsPRBEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS PRB entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS PRB entity and you do not have permission");
					}
