...
/**
 * Controller used to handle all REST operations regarding Invoice Item
 */
// % protected region % [Update the API route or description for the Invoice ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "InvoiceItemController"))
@RestController
@Slf4j
@RequestMapping("/api/invoiceitem")
// % protected region % [Update the API route or description for the Invoice ItemController here] end
public class InvoiceItemController {

	private final InvoiceItemService invoiceItemService;
	private final CsvUtils<InvoiceItemEntity, InvoiceItemRepository, InvoiceItemEntityAudit,  InvoiceItemService> csvUtils;
	...
	public InvoiceItemController(
		...
		this.csvUtils = new CsvUtils<>(invoiceItemService, InvoiceItemEntity.class);
	...
	public ResponseEntity<List<InvoiceItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<InvoiceItemEntity> invoiceItems = invoiceItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Invoice Item that has the same id as the given id.
	 *
	 * @param id      The id of the InvoiceItemEntity to be returned
	 * @return the Invoice Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single InvoiceItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "InvoiceItem entity not found"),
	})
	@PreAuthorize("hasPermission('InvoiceItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<InvoiceItemEntity> getById(
			@Parameter(description = "The UUID of the InvoiceItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<InvoiceItemEntity> invoiceItemEntity = invoiceItemService.findById(id);
	...
	/**
	 * Create/Update the given Invoice Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveInvoiceItem' endpoint here] off begin
	@Operation(
			description = "Create a InvoiceItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new InvoiceItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new InvoiceItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('InvoiceItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveInvoiceItem' endpoint here] end
	public ResponseEntity<?> saveInvoiceItem(
			@Parameter(description = "The InvoiceItemEntity to create or update.")
			@RequestBody InvoiceItemEntityDto invoiceItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			InvoiceItemEntity invoiceItemEntity = invoiceItemService.create(new InvoiceItemEntity(invoiceItemEntityDto));
	...
	/**
	 * Update the given Invoice Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param invoiceItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateInvoiceItem' endpoint here] off begin
	@Operation(
			description = "Update a InvoiceItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the InvoiceItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the InvoiceItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('InvoiceItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateInvoiceItem' endpoint here] end
	public ResponseEntity<?> updateInvoiceItem(
		...
		Optional<InvoiceItemEntity> checkInvoiceItemExists = invoiceItemService.findById(invoiceItemEntityDto.getId());
		InvoiceItemEntity invoiceItemEntity = new InvoiceItemEntity(invoiceItemEntityDto);

		if (checkInvoiceItemExists.isPresent()) {
			try {
				invoiceItemEntity = invoiceItemService.update(invoiceItemEntity);
				return new ResponseEntity<>(invoiceItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(invoiceItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Invoice Item that has the same id as the given id.
	 *
	 * @param id The id of the InvoiceItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single InvoiceItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the InvoiceItem to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Invoice Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItem' endpoint here] end
	public void exportInvoiceItem(
			@Parameter(description = "The ids of the InvoiceItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<InvoiceItemEntity> entitiesToExport = invoiceItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "InvoiceItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (InvoiceItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportInvoiceItemExcludingIds' endpoint here] end
	public void exportInvoiceItemExcludingIds(
			@Parameter(description = "The ids of the InvoiceItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<InvoiceItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = invoiceItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = invoiceItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the InvoiceItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "InvoiceItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(InvoiceItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Invoice Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('InvoiceItemEntity', 'create')  || hasPermission('InvoiceItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with InvoiceItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("InvoiceItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(InvoiceItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<InvoiceItemEntity> csvToBean = new CsvToBeanBuilder<InvoiceItemEntity>(reader)
				.withType(InvoiceItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<InvoiceItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<InvoiceItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Invoice Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Invoice Item entity and you do not have permission");
					}
