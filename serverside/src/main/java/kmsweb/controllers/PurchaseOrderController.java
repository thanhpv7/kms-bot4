...
import kmsweb.entities.PurchaseOrderEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Purchase Order
 */
// % protected region % [Update the API route or description for the Purchase OrderController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PurchaseOrderController"))
@RestController
@Slf4j
@RequestMapping("/api/purchaseorder")
// % protected region % [Update the API route or description for the Purchase OrderController here] end
public class PurchaseOrderController {

	private final PurchaseOrderService purchaseOrderService;
	private final CsvUtils<PurchaseOrderEntity, PurchaseOrderRepository, PurchaseOrderEntityAudit,  PurchaseOrderService> csvUtils;
	...
	public PurchaseOrderController(
		...
		this.csvUtils = new CsvUtils<>(purchaseOrderService, PurchaseOrderEntity.class);
	...
	@Operation(
			description = "Returns a single page of PurchaseOrders",
		security = {
	...
	public ResponseEntity<List<PurchaseOrderEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PurchaseOrderEntity> purchaseOrders = purchaseOrderService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Purchase Order that has the same id as the given id.
	 *
	 * @param id      The id of the PurchaseOrderEntity to be returned
	 * @return the Purchase Order that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PurchaseOrder as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PurchaseOrder entity not found"),
	})
	@PreAuthorize("hasPermission('PurchaseOrderEntity', 'read')")
	...
	public ResponseEntity<PurchaseOrderEntity> getById(
			@Parameter(description = "The UUID of the PurchaseOrder to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PurchaseOrderEntity> purchaseOrderEntity = purchaseOrderService.findById(id);
	...
	/**
	 * Create/Update the given Purchase Order. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param purchaseOrderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePurchaseOrder' endpoint here] off begin
	@Operation(
			description = "Create a PurchaseOrder if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PurchaseOrder."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PurchaseOrder, duplicate record found")
	})
	@PreAuthorize("hasPermission('PurchaseOrderEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePurchaseOrder' endpoint here] end
	public ResponseEntity<?> savePurchaseOrder(
			@Parameter(description = "The PurchaseOrderEntity to create or update.")
			@RequestBody PurchaseOrderEntityDto purchaseOrderEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PurchaseOrderEntity purchaseOrderEntity = purchaseOrderService.create(new PurchaseOrderEntity(purchaseOrderEntityDto));
			return new ResponseEntity<>(purchaseOrderEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Purchase Order. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param purchaseOrderEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePurchaseOrder' endpoint here] off begin
	@Operation(
			description = "Update a PurchaseOrder entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PurchaseOrder."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PurchaseOrder, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PurchaseOrderEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePurchaseOrder' endpoint here] end
	public ResponseEntity<?> updatePurchaseOrder(
		...
		Optional<PurchaseOrderEntity> checkPurchaseOrderExists = purchaseOrderService.findById(purchaseOrderEntityDto.getId());
		PurchaseOrderEntity purchaseOrderEntity = new PurchaseOrderEntity(purchaseOrderEntityDto);

		if (checkPurchaseOrderExists.isPresent()) {
			try {
				purchaseOrderEntity = purchaseOrderService.update(purchaseOrderEntity);
				return new ResponseEntity<>(purchaseOrderEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(purchaseOrderEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Purchase Order that has the same id as the given id.
	 *
	 * @param id The id of the PurchaseOrderEntity to be deleted
	...
	@Operation(
			description = "Delete a single PurchaseOrder as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PurchaseOrder to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchaseOrder' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Purchase Order entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchaseOrder' endpoint here] end
	public void exportPurchaseOrder(
			@Parameter(description = "The ids of the PurchaseOrderEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PurchaseOrderEntity> entitiesToExport = purchaseOrderService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PurchaseOrder_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PurchaseOrderEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPurchaseOrderExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPurchaseOrderExcludingIds' endpoint here] end
	public void exportPurchaseOrderExcludingIds(
			@Parameter(description = "The ids of the PurchaseOrderEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PurchaseOrderEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = purchaseOrderService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = purchaseOrderService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PurchaseOrder entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PurchaseOrder_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PurchaseOrderEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Purchase Order entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PurchaseOrderEntity', 'create')  || hasPermission('PurchaseOrderEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PurchaseOrderEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PurchaseOrderEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PurchaseOrderEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PurchaseOrderEntity> csvToBean = new CsvToBeanBuilder<PurchaseOrderEntity>(reader)
				.withType(PurchaseOrderEntity.class)
		...
		try {
			Iterator<PurchaseOrderEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Purchase Order entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Purchase Order entity and you do not have permission");
					}
