...
import kmsweb.entities.PrescriptionItemEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Prescription Item
 */
// % protected region % [Update the API route or description for the Prescription ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PrescriptionItemController"))
@RestController
@Slf4j
@RequestMapping("/api/prescriptionitem")
// % protected region % [Update the API route or description for the Prescription ItemController here] end
public class PrescriptionItemController {

	private final PrescriptionItemService prescriptionItemService;
	private final CsvUtils<PrescriptionItemEntity, PrescriptionItemRepository, PrescriptionItemEntityAudit,  PrescriptionItemService> csvUtils;
	...
	public PrescriptionItemController(
		...
		this.csvUtils = new CsvUtils<>(prescriptionItemService, PrescriptionItemEntity.class);
	...
	public ResponseEntity<List<PrescriptionItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PrescriptionItemEntity> prescriptionItems = prescriptionItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
	...
	/**
	 * Return the Prescription Item that has the same id as the given id.
	 *
	 * @param id      The id of the PrescriptionItemEntity to be returned
	 * @return the Prescription Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PrescriptionItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PrescriptionItem entity not found"),
	})
	@PreAuthorize("hasPermission('PrescriptionItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PrescriptionItemEntity> getById(
			@Parameter(description = "The UUID of the PrescriptionItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PrescriptionItemEntity> prescriptionItemEntity = prescriptionItemService.findById(id);
	...
	/**
	 * Create/Update the given Prescription Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param prescriptionItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePrescriptionItem' endpoint here] off begin
	@Operation(
			description = "Create a PrescriptionItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PrescriptionItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PrescriptionItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('PrescriptionItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePrescriptionItem' endpoint here] end
	public ResponseEntity<?> savePrescriptionItem(
			@Parameter(description = "The PrescriptionItemEntity to create or update.")
			@RequestBody PrescriptionItemEntityDto prescriptionItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PrescriptionItemEntity prescriptionItemEntity = prescriptionItemService.create(new PrescriptionItemEntity(prescriptionItemEntityDto));
			return new ResponseEntity<>(prescriptionItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Prescription Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param prescriptionItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePrescriptionItem' endpoint here] off begin
	@Operation(
			description = "Update a PrescriptionItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PrescriptionItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PrescriptionItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PrescriptionItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePrescriptionItem' endpoint here] end
	public ResponseEntity<?> updatePrescriptionItem(
		...
		Optional<PrescriptionItemEntity> checkPrescriptionItemExists = prescriptionItemService.findById(prescriptionItemEntityDto.getId());
		PrescriptionItemEntity prescriptionItemEntity = new PrescriptionItemEntity(prescriptionItemEntityDto);

		if (checkPrescriptionItemExists.isPresent()) {
			try {
				prescriptionItemEntity = prescriptionItemService.update(prescriptionItemEntity);
				return new ResponseEntity<>(prescriptionItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(prescriptionItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Prescription Item that has the same id as the given id.
	 *
	 * @param id The id of the PrescriptionItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single PrescriptionItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PrescriptionItem to delete.")
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Prescription Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionItem' endpoint here] end
	public void exportPrescriptionItem(
			@Parameter(description = "The ids of the PrescriptionItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PrescriptionItemEntity> entitiesToExport = prescriptionItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PrescriptionItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PrescriptionItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPrescriptionItemExcludingIds' endpoint here] end
	public void exportPrescriptionItemExcludingIds(
			@Parameter(description = "The ids of the PrescriptionItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PrescriptionItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = prescriptionItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = prescriptionItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PrescriptionItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PrescriptionItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PrescriptionItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Prescription Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PrescriptionItemEntity', 'create')  || hasPermission('PrescriptionItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PrescriptionItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PrescriptionItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PrescriptionItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PrescriptionItemEntity> csvToBean = new CsvToBeanBuilder<PrescriptionItemEntity>(reader)
				.withType(PrescriptionItemEntity.class)
		...
		try {
			Iterator<PrescriptionItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Prescription Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Prescription Item entity and you do not have permission");
					}
