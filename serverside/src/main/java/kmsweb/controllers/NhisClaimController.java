...
import kmsweb.entities.NhisClaimEntityAudit;
import kmsweb.repositories.NhisClaimRepository;
import kmsweb.services.NhisClaimService;
...
/**
 * Controller used to handle all REST operations regarding NHIS Claim
 */
// % protected region % [Update the API route or description for the NHIS ClaimController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisClaimController"))
@RestController
@Slf4j
@RequestMapping("/api/nhisclaim")
// % protected region % [Update the API route or description for the NHIS ClaimController here] end
public class NhisClaimController {

	private final NhisClaimService nhisClaimService;
	private final CsvUtils<NhisClaimEntity, NhisClaimRepository, NhisClaimEntityAudit,  NhisClaimService> csvUtils;
	...
	@Autowired
	public NhisClaimController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NhisClaimService nhisClaimService
	) {
		this.authService = authService;
		this.nhisClaimService = nhisClaimService;
		this.csvUtils = new CsvUtils<>(nhisClaimService, NhisClaimEntity.class);
	...
	public ResponseEntity<List<NhisClaimEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisClaimEntity> nhisClaims = nhisClaimService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisClaims, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Claim that has the same id as the given id.
	 *
	 * @param id      The id of the NhisClaimEntity to be returned
	 * @return the NHIS Claim that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisClaim as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisClaim entity not found"),
	})
	@PreAuthorize("hasPermission('NhisClaimEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisClaimEntity> getById(
			@Parameter(description = "The UUID of the NhisClaim to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisClaimEntity> nhisClaimEntity = nhisClaimService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Claim. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisClaimEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisClaim' endpoint here] off begin
	@Operation(
			description = "Create a NhisClaim if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisClaim."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisClaim, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisClaimEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisClaim' endpoint here] end
	public ResponseEntity<?> saveNhisClaim(
			@Parameter(description = "The NhisClaimEntity to create or update.")
			@RequestBody NhisClaimEntityDto nhisClaimEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisClaimEntity nhisClaimEntity = nhisClaimService.create(new NhisClaimEntity(nhisClaimEntityDto));
			return new ResponseEntity<>(nhisClaimEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Claim. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisClaimEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisClaim' endpoint here] off begin
	@Operation(
			description = "Update a NhisClaim entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisClaim."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisClaim, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisClaimEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisClaim' endpoint here] end
	public ResponseEntity<?> updateNhisClaim(
			@Parameter(description = "The NhisClaimEntity to create or update.")
			@RequestBody NhisClaimEntityDto nhisClaimEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisClaimEntity> checkNhisClaimExists = nhisClaimService.findById(nhisClaimEntityDto.getId());
		NhisClaimEntity nhisClaimEntity = new NhisClaimEntity(nhisClaimEntityDto);

		if (checkNhisClaimExists.isPresent()) {
			try {
				nhisClaimEntity = nhisClaimService.update(nhisClaimEntity);
				return new ResponseEntity<>(nhisClaimEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisClaimEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Claim that has the same id as the given id.
	 *
	 * @param id The id of the NhisClaimEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisClaim as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisClaim to delete.")
		...
		nhisClaimService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisClaim' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Claim entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisClaim' endpoint here] end
	public void exportNhisClaim(
			@Parameter(description = "The ids of the NhisClaimEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisClaimEntity> entitiesToExport = nhisClaimService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisClaim_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisClaimEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimExcludingIds' endpoint here] end
	public void exportNhisClaimExcludingIds(
			@Parameter(description = "The ids of the NhisClaimEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisClaimEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisClaimService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisClaimService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisClaim entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisClaim_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisClaimEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Claim entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisClaimEntity', 'create')  || hasPermission('NhisClaimEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisClaimEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisClaimEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisClaimEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisClaimEntity> csvToBean = new CsvToBeanBuilder<NhisClaimEntity>(reader)
				.withType(NhisClaimEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisClaimEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisClaimEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Claim entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Claim entity and you do not have permission");
					}
