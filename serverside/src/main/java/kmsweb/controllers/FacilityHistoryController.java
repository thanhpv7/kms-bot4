...
import kmsweb.entities.FacilityHistoryEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Facility History
 */
// % protected region % [Update the API route or description for the Facility HistoryController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "FacilityHistoryController"))
@RestController
@Slf4j
@RequestMapping("/api/facilityhistory")
// % protected region % [Update the API route or description for the Facility HistoryController here] end
public class FacilityHistoryController {

	private final FacilityHistoryService facilityHistoryService;
	private final CsvUtils<FacilityHistoryEntity, FacilityHistoryRepository, FacilityHistoryEntityAudit,  FacilityHistoryService> csvUtils;
	...
	@Autowired
	public FacilityHistoryController(
		...
		this.csvUtils = new CsvUtils<>(facilityHistoryService, FacilityHistoryEntity.class);
	...
	public ResponseEntity<List<FacilityHistoryEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<FacilityHistoryEntity> facilityHistorys = facilityHistoryService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(facilityHistorys, HttpStatus.OK);
	}

	/**
	 * Return the Facility History that has the same id as the given id.
	 *
	 * @param id      The id of the FacilityHistoryEntity to be returned
	 * @return the Facility History that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single FacilityHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "FacilityHistory entity not found"),
	})
	@PreAuthorize("hasPermission('FacilityHistoryEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<FacilityHistoryEntity> getById(
			@Parameter(description = "The UUID of the FacilityHistory to return.")
			@PathVariable("id") UUID id
	) {
		Optional<FacilityHistoryEntity> facilityHistoryEntity = facilityHistoryService.findById(id);
	...
	/**
	 * Create/Update the given Facility History. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param facilityHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveFacilityHistory' endpoint here] off begin
	@Operation(
			description = "Create a FacilityHistory if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new FacilityHistory."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new FacilityHistory, duplicate record found")
	})
	@PreAuthorize("hasPermission('FacilityHistoryEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveFacilityHistory' endpoint here] end
	public ResponseEntity<?> saveFacilityHistory(
			@Parameter(description = "The FacilityHistoryEntity to create or update.")
			@RequestBody FacilityHistoryEntityDto facilityHistoryEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			FacilityHistoryEntity facilityHistoryEntity = facilityHistoryService.create(new FacilityHistoryEntity(facilityHistoryEntityDto));
			return new ResponseEntity<>(facilityHistoryEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Facility History. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param facilityHistoryEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateFacilityHistory' endpoint here] off begin
	@Operation(
			description = "Update a FacilityHistory entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the FacilityHistory."),
		@ApiResponse(responseCode = "400", description = "Failed to update the FacilityHistory, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('FacilityHistoryEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateFacilityHistory' endpoint here] end
	public ResponseEntity<?> updateFacilityHistory(
			@Parameter(description = "The FacilityHistoryEntity to create or update.")
			@RequestBody FacilityHistoryEntityDto facilityHistoryEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<FacilityHistoryEntity> checkFacilityHistoryExists = facilityHistoryService.findById(facilityHistoryEntityDto.getId());
		FacilityHistoryEntity facilityHistoryEntity = new FacilityHistoryEntity(facilityHistoryEntityDto);

		if (checkFacilityHistoryExists.isPresent()) {
			try {
				facilityHistoryEntity = facilityHistoryService.update(facilityHistoryEntity);
				return new ResponseEntity<>(facilityHistoryEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(facilityHistoryEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Facility History that has the same id as the given id.
	 *
	 * @param id The id of the FacilityHistoryEntity to be deleted
	...
	@Operation(
			description = "Delete a single FacilityHistory as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the FacilityHistory to delete.")
		...
		facilityHistoryService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportFacilityHistory' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Facility History entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportFacilityHistory' endpoint here] end
	public void exportFacilityHistory(
			@Parameter(description = "The ids of the FacilityHistoryEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<FacilityHistoryEntity> entitiesToExport = facilityHistoryService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "FacilityHistory_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (FacilityHistoryEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportFacilityHistoryExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportFacilityHistoryExcludingIds' endpoint here] end
	public void exportFacilityHistoryExcludingIds(
			@Parameter(description = "The ids of the FacilityHistoryEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<FacilityHistoryEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = facilityHistoryService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = facilityHistoryService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the FacilityHistory entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "FacilityHistory_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(FacilityHistoryEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Facility History entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('FacilityHistoryEntity', 'create')  || hasPermission('FacilityHistoryEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with FacilityHistoryEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("FacilityHistoryEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(FacilityHistoryEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<FacilityHistoryEntity> csvToBean = new CsvToBeanBuilder<FacilityHistoryEntity>(reader)
				.withType(FacilityHistoryEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<FacilityHistoryEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<FacilityHistoryEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Facility History entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Facility History entity and you do not have permission");
					}
