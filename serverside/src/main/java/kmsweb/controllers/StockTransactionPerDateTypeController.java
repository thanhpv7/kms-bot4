...
/**
 * Controller used to handle all REST operations regarding Stock Transaction Per Date Type
 */
// % protected region % [Update the API route or description for the Stock Transaction Per Date TypeController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "StockTransactionPerDateTypeController"))
@RestController
@Slf4j
@RequestMapping("/api/stocktransactionperdatetype")
// % protected region % [Update the API route or description for the Stock Transaction Per Date TypeController here] end
public class StockTransactionPerDateTypeController {

	private final StockTransactionPerDateTypeService stockTransactionPerDateTypeService;
	private final CsvUtils<StockTransactionPerDateTypeEntity, StockTransactionPerDateTypeRepository, StockTransactionPerDateTypeEntityAudit,  StockTransactionPerDateTypeService> csvUtils;
	...
	@Autowired
	public StockTransactionPerDateTypeController(
		...
		this.csvUtils = new CsvUtils<>(stockTransactionPerDateTypeService, StockTransactionPerDateTypeEntity.class);
	...
	/**
	 * Return all the Stock Transaction Per Date Types.
	 *
	 * @return all the Stock Transaction Per Date Types
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of StockTransactionPerDateTypes",
		security = {
	...
	public ResponseEntity<List<StockTransactionPerDateTypeEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<StockTransactionPerDateTypeEntity> stockTransactionPerDateTypes = stockTransactionPerDateTypeService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(stockTransactionPerDateTypes, HttpStatus.OK);
	}

	/**
	 * Return the Stock Transaction Per Date Type that has the same id as the given id.
	 *
	 * @param id      The id of the StockTransactionPerDateTypeEntity to be returned
	 * @return the Stock Transaction Per Date Type that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single StockTransactionPerDateType as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "StockTransactionPerDateType entity not found"),
	})
	@PreAuthorize("hasPermission('StockTransactionPerDateTypeEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<StockTransactionPerDateTypeEntity> getById(
			@Parameter(description = "The UUID of the StockTransactionPerDateType to return.")
			@PathVariable("id") UUID id
	) {
		Optional<StockTransactionPerDateTypeEntity> stockTransactionPerDateTypeEntity = stockTransactionPerDateTypeService.findById(id);
	...
	/**
	 * Create/Update the given Stock Transaction Per Date Type. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param stockTransactionPerDateTypeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveStockTransactionPerDateType' endpoint here] off begin
	@Operation(
			description = "Create a StockTransactionPerDateType if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new StockTransactionPerDateType."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new StockTransactionPerDateType, duplicate record found")
	})
	@PreAuthorize("hasPermission('StockTransactionPerDateTypeEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveStockTransactionPerDateType' endpoint here] end
	public ResponseEntity<?> saveStockTransactionPerDateType(
			@Parameter(description = "The StockTransactionPerDateTypeEntity to create or update.")
			@RequestBody StockTransactionPerDateTypeEntityDto stockTransactionPerDateTypeEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			StockTransactionPerDateTypeEntity stockTransactionPerDateTypeEntity = stockTransactionPerDateTypeService.create(new StockTransactionPerDateTypeEntity(stockTransactionPerDateTypeEntityDto));
			return new ResponseEntity<>(stockTransactionPerDateTypeEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Stock Transaction Per Date Type. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param stockTransactionPerDateTypeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateStockTransactionPerDateType' endpoint here] off begin
	@Operation(
			description = "Update a StockTransactionPerDateType entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the StockTransactionPerDateType."),
		@ApiResponse(responseCode = "400", description = "Failed to update the StockTransactionPerDateType, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('StockTransactionPerDateTypeEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateStockTransactionPerDateType' endpoint here] end
	public ResponseEntity<?> updateStockTransactionPerDateType(
			@Parameter(description = "The StockTransactionPerDateTypeEntity to create or update.")
			@RequestBody StockTransactionPerDateTypeEntityDto stockTransactionPerDateTypeEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<StockTransactionPerDateTypeEntity> checkStockTransactionPerDateTypeExists = stockTransactionPerDateTypeService.findById(stockTransactionPerDateTypeEntityDto.getId());
		StockTransactionPerDateTypeEntity stockTransactionPerDateTypeEntity = new StockTransactionPerDateTypeEntity(stockTransactionPerDateTypeEntityDto);

		if (checkStockTransactionPerDateTypeExists.isPresent()) {
			try {
				stockTransactionPerDateTypeEntity = stockTransactionPerDateTypeService.update(stockTransactionPerDateTypeEntity);
				return new ResponseEntity<>(stockTransactionPerDateTypeEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(stockTransactionPerDateTypeEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Stock Transaction Per Date Type that has the same id as the given id.
	 *
	 * @param id The id of the StockTransactionPerDateTypeEntity to be deleted
	...
	@Operation(
			description = "Delete a single StockTransactionPerDateType as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the StockTransactionPerDateType to delete.")
		...
		stockTransactionPerDateTypeService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDateType' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Stock Transaction Per Date Type entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDateType' endpoint here] end
	public void exportStockTransactionPerDateType(
			@Parameter(description = "The ids of the StockTransactionPerDateTypeEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<StockTransactionPerDateTypeEntity> entitiesToExport = stockTransactionPerDateTypeService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "StockTransactionPerDateType_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (StockTransactionPerDateTypeEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDateTypeExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportStockTransactionPerDateTypeExcludingIds' endpoint here] end
	public void exportStockTransactionPerDateTypeExcludingIds(
			@Parameter(description = "The ids of the StockTransactionPerDateTypeEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<StockTransactionPerDateTypeEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = stockTransactionPerDateTypeService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = stockTransactionPerDateTypeService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the StockTransactionPerDateType entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "StockTransactionPerDateType_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(StockTransactionPerDateTypeEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Stock Transaction Per Date Type entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('StockTransactionPerDateTypeEntity', 'create')  || hasPermission('StockTransactionPerDateTypeEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with StockTransactionPerDateTypeEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("StockTransactionPerDateTypeEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(StockTransactionPerDateTypeEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<StockTransactionPerDateTypeEntity> csvToBean = new CsvToBeanBuilder<StockTransactionPerDateTypeEntity>(reader)
				.withType(StockTransactionPerDateTypeEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<StockTransactionPerDateTypeEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<StockTransactionPerDateTypeEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Stock Transaction Per Date Type entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Stock Transaction Per Date Type entity and you do not have permission");
					}
