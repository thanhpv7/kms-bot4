...
import kmsweb.entities.PatientSupportingDocumentEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Patient Supporting Document
 */
// % protected region % [Update the API route or description for the Patient Supporting DocumentController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PatientSupportingDocumentController"))
@RestController
@Slf4j
@RequestMapping("/api/patientsupportingdocument")
// % protected region % [Update the API route or description for the Patient Supporting DocumentController here] end
public class PatientSupportingDocumentController {

	private final PatientSupportingDocumentService patientSupportingDocumentService;
	private final CsvUtils<PatientSupportingDocumentEntity, PatientSupportingDocumentRepository, PatientSupportingDocumentEntityAudit,  PatientSupportingDocumentService> csvUtils;
	...
	@Autowired
	public PatientSupportingDocumentController(
		...
		this.csvUtils = new CsvUtils<>(patientSupportingDocumentService, PatientSupportingDocumentEntity.class);
	...
	/**
	 * Return all the Patient Supporting Documents.
	 *
	 * @return all the Patient Supporting Documents
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of PatientSupportingDocuments",
		security = {
	...
	public ResponseEntity<List<PatientSupportingDocumentEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PatientSupportingDocumentEntity> patientSupportingDocuments = patientSupportingDocumentService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(patientSupportingDocuments, HttpStatus.OK);
	}

	/**
	 * Return the Patient Supporting Document that has the same id as the given id.
	 *
	 * @param id      The id of the PatientSupportingDocumentEntity to be returned
	 * @return the Patient Supporting Document that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PatientSupportingDocument as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PatientSupportingDocument entity not found"),
	})
	@PreAuthorize("hasPermission('PatientSupportingDocumentEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PatientSupportingDocumentEntity> getById(
			@Parameter(description = "The UUID of the PatientSupportingDocument to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PatientSupportingDocumentEntity> patientSupportingDocumentEntity = patientSupportingDocumentService.findById(id);
	...
	/**
	 * Create/Update the given Patient Supporting Document. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param patientSupportingDocumentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePatientSupportingDocument' endpoint here] off begin
	@Operation(
			description = "Create a PatientSupportingDocument if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PatientSupportingDocument."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PatientSupportingDocument, duplicate record found")
	})
	@PreAuthorize("hasPermission('PatientSupportingDocumentEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePatientSupportingDocument' endpoint here] end
	public ResponseEntity<?> savePatientSupportingDocument(
			@Parameter(description = "The PatientSupportingDocumentEntity to create or update.")
			@RequestBody PatientSupportingDocumentEntityDto patientSupportingDocumentEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PatientSupportingDocumentEntity patientSupportingDocumentEntity = patientSupportingDocumentService.create(new PatientSupportingDocumentEntity(patientSupportingDocumentEntityDto));
			return new ResponseEntity<>(patientSupportingDocumentEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Patient Supporting Document. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param patientSupportingDocumentEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePatientSupportingDocument' endpoint here] off begin
	@Operation(
			description = "Update a PatientSupportingDocument entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PatientSupportingDocument."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PatientSupportingDocument, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PatientSupportingDocumentEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePatientSupportingDocument' endpoint here] end
	public ResponseEntity<?> updatePatientSupportingDocument(
			@Parameter(description = "The PatientSupportingDocumentEntity to create or update.")
			@RequestBody PatientSupportingDocumentEntityDto patientSupportingDocumentEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PatientSupportingDocumentEntity> checkPatientSupportingDocumentExists = patientSupportingDocumentService.findById(patientSupportingDocumentEntityDto.getId());
		PatientSupportingDocumentEntity patientSupportingDocumentEntity = new PatientSupportingDocumentEntity(patientSupportingDocumentEntityDto);

		if (checkPatientSupportingDocumentExists.isPresent()) {
			try {
				patientSupportingDocumentEntity = patientSupportingDocumentService.update(patientSupportingDocumentEntity);
				return new ResponseEntity<>(patientSupportingDocumentEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(patientSupportingDocumentEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Patient Supporting Document that has the same id as the given id.
	 *
	 * @param id The id of the PatientSupportingDocumentEntity to be deleted
	...
	@Operation(
			description = "Delete a single PatientSupportingDocument as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PatientSupportingDocument to delete.")
		...
		patientSupportingDocumentService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientSupportingDocument' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Patient Supporting Document entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientSupportingDocument' endpoint here] end
	public void exportPatientSupportingDocument(
			@Parameter(description = "The ids of the PatientSupportingDocumentEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PatientSupportingDocumentEntity> entitiesToExport = patientSupportingDocumentService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PatientSupportingDocument_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PatientSupportingDocumentEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPatientSupportingDocumentExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPatientSupportingDocumentExcludingIds' endpoint here] end
	public void exportPatientSupportingDocumentExcludingIds(
			@Parameter(description = "The ids of the PatientSupportingDocumentEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PatientSupportingDocumentEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = patientSupportingDocumentService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = patientSupportingDocumentService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PatientSupportingDocument entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PatientSupportingDocument_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PatientSupportingDocumentEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Patient Supporting Document entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PatientSupportingDocumentEntity', 'create')  || hasPermission('PatientSupportingDocumentEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PatientSupportingDocumentEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PatientSupportingDocumentEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PatientSupportingDocumentEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PatientSupportingDocumentEntity> csvToBean = new CsvToBeanBuilder<PatientSupportingDocumentEntity>(reader)
				.withType(PatientSupportingDocumentEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PatientSupportingDocumentEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PatientSupportingDocumentEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Patient Supporting Document entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Patient Supporting Document entity and you do not have permission");
					}
