...
/**
 * Controller used to handle all REST operations regarding PCare Umum
 */
// % protected region % [Update the API route or description for the PCare UmumController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareUmumController"))
@RestController
@Slf4j
@RequestMapping("/api/pcareumum")
// % protected region % [Update the API route or description for the PCare UmumController here] end
public class PcareUmumController {

	private final PcareUmumService pcareUmumService;
	private final CsvUtils<PcareUmumEntity, PcareUmumRepository, PcareUmumEntityAudit,  PcareUmumService> csvUtils;
	...
	public PcareUmumController(
		...
		this.csvUtils = new CsvUtils<>(pcareUmumService, PcareUmumEntity.class);
	...
	public ResponseEntity<List<PcareUmumEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareUmumEntity> pcareUmums = pcareUmumService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareUmums, HttpStatus.OK);
	}

	/**
	 * Return the PCare Umum that has the same id as the given id.
	 *
	 * @param id      The id of the PcareUmumEntity to be returned
	 * @return the PCare Umum that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareUmum as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareUmum entity not found"),
	})
	@PreAuthorize("hasPermission('PcareUmumEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareUmumEntity> getById(
			@Parameter(description = "The UUID of the PcareUmum to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareUmumEntity> pcareUmumEntity = pcareUmumService.findById(id);
	...
	/**
	 * Create/Update the given PCare Umum. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareUmumEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareUmum' endpoint here] off begin
	@Operation(
			description = "Create a PcareUmum if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareUmum."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareUmum, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareUmumEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareUmum' endpoint here] end
	public ResponseEntity<?> savePcareUmum(
			@Parameter(description = "The PcareUmumEntity to create or update.")
			@RequestBody PcareUmumEntityDto pcareUmumEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareUmumEntity pcareUmumEntity = pcareUmumService.create(new PcareUmumEntity(pcareUmumEntityDto));
			return new ResponseEntity<>(pcareUmumEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare Umum. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareUmumEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareUmum' endpoint here] off begin
	@Operation(
			description = "Update a PcareUmum entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareUmum."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareUmum, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareUmumEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareUmum' endpoint here] end
	public ResponseEntity<?> updatePcareUmum(
			@Parameter(description = "The PcareUmumEntity to create or update.")
			@RequestBody PcareUmumEntityDto pcareUmumEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareUmumEntity> checkPcareUmumExists = pcareUmumService.findById(pcareUmumEntityDto.getId());
		PcareUmumEntity pcareUmumEntity = new PcareUmumEntity(pcareUmumEntityDto);

		if (checkPcareUmumExists.isPresent()) {
			try {
				pcareUmumEntity = pcareUmumService.update(pcareUmumEntity);
				return new ResponseEntity<>(pcareUmumEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareUmumEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare Umum that has the same id as the given id.
	 *
	 * @param id The id of the PcareUmumEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareUmum as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareUmum to delete.")
		...
		pcareUmumService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareUmum' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare Umum entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareUmum' endpoint here] end
	public void exportPcareUmum(
			@Parameter(description = "The ids of the PcareUmumEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareUmumEntity> entitiesToExport = pcareUmumService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareUmum_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareUmumEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareUmumExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareUmumExcludingIds' endpoint here] end
	public void exportPcareUmumExcludingIds(
			@Parameter(description = "The ids of the PcareUmumEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareUmumEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareUmumService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareUmumService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareUmum entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareUmum_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareUmumEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare Umum entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareUmumEntity', 'create')  || hasPermission('PcareUmumEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareUmumEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareUmumEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareUmumEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareUmumEntity> csvToBean = new CsvToBeanBuilder<PcareUmumEntity>(reader)
				.withType(PcareUmumEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareUmumEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareUmumEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare Umum entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare Umum entity and you do not have permission");
					}
