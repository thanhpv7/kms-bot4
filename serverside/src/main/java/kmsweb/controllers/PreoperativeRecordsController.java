...
import kmsweb.entities.PreoperativeRecordsEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Preoperative Records
 */
// % protected region % [Update the API route or description for the Preoperative RecordsController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PreoperativeRecordsController"))
@RestController
@Slf4j
@RequestMapping("/api/preoperativerecords")
// % protected region % [Update the API route or description for the Preoperative RecordsController here] end
public class PreoperativeRecordsController {

	private final PreoperativeRecordsService preoperativeRecordsService;
	private final CsvUtils<PreoperativeRecordsEntity, PreoperativeRecordsRepository, PreoperativeRecordsEntityAudit,  PreoperativeRecordsService> csvUtils;
	...
	@Autowired
	public PreoperativeRecordsController(
		...
		this.csvUtils = new CsvUtils<>(preoperativeRecordsService, PreoperativeRecordsEntity.class);
	...
	public ResponseEntity<List<PreoperativeRecordsEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PreoperativeRecordsEntity> preoperativeRecordss = preoperativeRecordsService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(preoperativeRecordss, HttpStatus.OK);
	}

	/**
	 * Return the Preoperative Records that has the same id as the given id.
	 *
	 * @param id      The id of the PreoperativeRecordsEntity to be returned
	 * @return the Preoperative Records that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PreoperativeRecords as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PreoperativeRecords entity not found"),
	})
	@PreAuthorize("hasPermission('PreoperativeRecordsEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PreoperativeRecordsEntity> getById(
			@Parameter(description = "The UUID of the PreoperativeRecords to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PreoperativeRecordsEntity> preoperativeRecordsEntity = preoperativeRecordsService.findById(id);
	...
	/**
	 * Create/Update the given Preoperative Records. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param preoperativeRecordsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePreoperativeRecords' endpoint here] off begin
	@Operation(
			description = "Create a PreoperativeRecords if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PreoperativeRecords."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PreoperativeRecords, duplicate record found")
	})
	@PreAuthorize("hasPermission('PreoperativeRecordsEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePreoperativeRecords' endpoint here] end
	public ResponseEntity<?> savePreoperativeRecords(
			@Parameter(description = "The PreoperativeRecordsEntity to create or update.")
			@RequestBody PreoperativeRecordsEntityDto preoperativeRecordsEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PreoperativeRecordsEntity preoperativeRecordsEntity = preoperativeRecordsService.create(new PreoperativeRecordsEntity(preoperativeRecordsEntityDto));
			return new ResponseEntity<>(preoperativeRecordsEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Preoperative Records. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param preoperativeRecordsEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePreoperativeRecords' endpoint here] off begin
	@Operation(
			description = "Update a PreoperativeRecords entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PreoperativeRecords."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PreoperativeRecords, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PreoperativeRecordsEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePreoperativeRecords' endpoint here] end
	public ResponseEntity<?> updatePreoperativeRecords(
			@Parameter(description = "The PreoperativeRecordsEntity to create or update.")
			@RequestBody PreoperativeRecordsEntityDto preoperativeRecordsEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PreoperativeRecordsEntity> checkPreoperativeRecordsExists = preoperativeRecordsService.findById(preoperativeRecordsEntityDto.getId());
		PreoperativeRecordsEntity preoperativeRecordsEntity = new PreoperativeRecordsEntity(preoperativeRecordsEntityDto);

		if (checkPreoperativeRecordsExists.isPresent()) {
			try {
				preoperativeRecordsEntity = preoperativeRecordsService.update(preoperativeRecordsEntity);
				return new ResponseEntity<>(preoperativeRecordsEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(preoperativeRecordsEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Preoperative Records that has the same id as the given id.
	 *
	 * @param id The id of the PreoperativeRecordsEntity to be deleted
	...
	@Operation(
			description = "Delete a single PreoperativeRecords as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PreoperativeRecords to delete.")
		...
		preoperativeRecordsService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPreoperativeRecords' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Preoperative Records entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPreoperativeRecords' endpoint here] end
	public void exportPreoperativeRecords(
			@Parameter(description = "The ids of the PreoperativeRecordsEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PreoperativeRecordsEntity> entitiesToExport = preoperativeRecordsService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PreoperativeRecords_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PreoperativeRecordsEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPreoperativeRecordsExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPreoperativeRecordsExcludingIds' endpoint here] end
	public void exportPreoperativeRecordsExcludingIds(
			@Parameter(description = "The ids of the PreoperativeRecordsEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PreoperativeRecordsEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = preoperativeRecordsService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = preoperativeRecordsService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PreoperativeRecords entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PreoperativeRecords_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PreoperativeRecordsEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Preoperative Records entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PreoperativeRecordsEntity', 'create')  || hasPermission('PreoperativeRecordsEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PreoperativeRecordsEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PreoperativeRecordsEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PreoperativeRecordsEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PreoperativeRecordsEntity> csvToBean = new CsvToBeanBuilder<PreoperativeRecordsEntity>(reader)
				.withType(PreoperativeRecordsEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PreoperativeRecordsEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PreoperativeRecordsEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Preoperative Records entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Preoperative Records entity and you do not have permission");
					}
