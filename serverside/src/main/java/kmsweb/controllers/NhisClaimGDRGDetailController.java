...
import kmsweb.dtos.NhisClaimGDRGDetailEntityDto;
import kmsweb.entities.NhisClaimGDRGDetailEntityAudit;
import kmsweb.repositories.NhisClaimGDRGDetailRepository;
import kmsweb.services.NhisClaimGDRGDetailService;
...
/**
 * Controller used to handle all REST operations regarding NHIS Claim GDRG Detail
 */
// % protected region % [Update the API route or description for the NHIS Claim GDRG DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "NhisClaimGDRGDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/nhisclaimgdrgdetail")
// % protected region % [Update the API route or description for the NHIS Claim GDRG DetailController here] end
public class NhisClaimGDRGDetailController {

	private final NhisClaimGDRGDetailService nhisClaimGDRGDetailService;
	private final CsvUtils<NhisClaimGDRGDetailEntity, NhisClaimGDRGDetailRepository, NhisClaimGDRGDetailEntityAudit,  NhisClaimGDRGDetailService> csvUtils;
	...
	@Autowired
	public NhisClaimGDRGDetailController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			NhisClaimGDRGDetailService nhisClaimGDRGDetailService
	) {
		this.authService = authService;
		this.nhisClaimGDRGDetailService = nhisClaimGDRGDetailService;
		this.csvUtils = new CsvUtils<>(nhisClaimGDRGDetailService, NhisClaimGDRGDetailEntity.class);
	...
	@PreAuthorize("hasPermission('NhisClaimGDRGDetailEntity', 'read')")
	@GetMapping(produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] end
	public ResponseEntity<List<NhisClaimGDRGDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<NhisClaimGDRGDetailEntity> nhisClaimGDRGDetails = nhisClaimGDRGDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(nhisClaimGDRGDetails, HttpStatus.OK);
	}

	/**
	 * Return the NHIS Claim GDRG Detail that has the same id as the given id.
	 *
	 * @param id      The id of the NhisClaimGDRGDetailEntity to be returned
	 * @return the NHIS Claim GDRG Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single NhisClaimGDRGDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "NhisClaimGDRGDetail entity not found"),
	})
	@PreAuthorize("hasPermission('NhisClaimGDRGDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<NhisClaimGDRGDetailEntity> getById(
			@Parameter(description = "The UUID of the NhisClaimGDRGDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<NhisClaimGDRGDetailEntity> nhisClaimGDRGDetailEntity = nhisClaimGDRGDetailService.findById(id);
	...
	/**
	 * Create/Update the given NHIS Claim GDRG Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param nhisClaimGDRGDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveNhisClaimGDRGDetail' endpoint here] off begin
	@Operation(
			description = "Create a NhisClaimGDRGDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new NhisClaimGDRGDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new NhisClaimGDRGDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('NhisClaimGDRGDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveNhisClaimGDRGDetail' endpoint here] end
	public ResponseEntity<?> saveNhisClaimGDRGDetail(
			@Parameter(description = "The NhisClaimGDRGDetailEntity to create or update.")
			@RequestBody NhisClaimGDRGDetailEntityDto nhisClaimGDRGDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			NhisClaimGDRGDetailEntity nhisClaimGDRGDetailEntity = nhisClaimGDRGDetailService.create(new NhisClaimGDRGDetailEntity(nhisClaimGDRGDetailEntityDto));
			return new ResponseEntity<>(nhisClaimGDRGDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given NHIS Claim GDRG Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param nhisClaimGDRGDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateNhisClaimGDRGDetail' endpoint here] off begin
	@Operation(
			description = "Update a NhisClaimGDRGDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the NhisClaimGDRGDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the NhisClaimGDRGDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('NhisClaimGDRGDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateNhisClaimGDRGDetail' endpoint here] end
	public ResponseEntity<?> updateNhisClaimGDRGDetail(
			@Parameter(description = "The NhisClaimGDRGDetailEntity to create or update.")
			@RequestBody NhisClaimGDRGDetailEntityDto nhisClaimGDRGDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<NhisClaimGDRGDetailEntity> checkNhisClaimGDRGDetailExists = nhisClaimGDRGDetailService.findById(nhisClaimGDRGDetailEntityDto.getId());
		NhisClaimGDRGDetailEntity nhisClaimGDRGDetailEntity = new NhisClaimGDRGDetailEntity(nhisClaimGDRGDetailEntityDto);

		if (checkNhisClaimGDRGDetailExists.isPresent()) {
			try {
				nhisClaimGDRGDetailEntity = nhisClaimGDRGDetailService.update(nhisClaimGDRGDetailEntity);
				return new ResponseEntity<>(nhisClaimGDRGDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(nhisClaimGDRGDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the NHIS Claim GDRG Detail that has the same id as the given id.
	 *
	 * @param id The id of the NhisClaimGDRGDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single NhisClaimGDRGDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@PreAuthorize("hasPermission('NhisClaimGDRGDetailEntity', 'delete')")
	@DeleteMapping(value = "/{id}")
	// % protected region % [Update the route mapping or description for the 'deleteById' endpoint here] end
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the NhisClaimGDRGDetail to delete.")
		...
		nhisClaimGDRGDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimGDRGDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all NHIS Claim GDRG Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimGDRGDetail' endpoint here] end
	public void exportNhisClaimGDRGDetail(
			@Parameter(description = "The ids of the NhisClaimGDRGDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<NhisClaimGDRGDetailEntity> entitiesToExport = nhisClaimGDRGDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "NhisClaimGDRGDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (NhisClaimGDRGDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimGDRGDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportNhisClaimGDRGDetailExcludingIds' endpoint here] end
	public void exportNhisClaimGDRGDetailExcludingIds(
			@Parameter(description = "The ids of the NhisClaimGDRGDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<NhisClaimGDRGDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = nhisClaimGDRGDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = nhisClaimGDRGDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the NhisClaimGDRGDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "NhisClaimGDRGDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(NhisClaimGDRGDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for NHIS Claim GDRG Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('NhisClaimGDRGDetailEntity', 'create')  || hasPermission('NhisClaimGDRGDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with NhisClaimGDRGDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("NhisClaimGDRGDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(NhisClaimGDRGDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<NhisClaimGDRGDetailEntity> csvToBean = new CsvToBeanBuilder<NhisClaimGDRGDetailEntity>(reader)
				.withType(NhisClaimGDRGDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<NhisClaimGDRGDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<NhisClaimGDRGDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a NHIS Claim GDRG Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a NHIS Claim GDRG Detail entity and you do not have permission");
					}
