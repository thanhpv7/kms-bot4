...
import kmsweb.entities.PcareNonCompoundDrugEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding PCare NonCompound Drug
 */
// % protected region % [Update the API route or description for the PCare NonCompound DrugController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "PcareNonCompoundDrugController"))
@RestController
@Slf4j
@RequestMapping("/api/pcarenoncompounddrug")
// % protected region % [Update the API route or description for the PCare NonCompound DrugController here] end
public class PcareNonCompoundDrugController {

	private final PcareNonCompoundDrugService pcareNonCompoundDrugService;
	private final CsvUtils<PcareNonCompoundDrugEntity, PcareNonCompoundDrugRepository, PcareNonCompoundDrugEntityAudit,  PcareNonCompoundDrugService> csvUtils;
	...
	@Autowired
	public PcareNonCompoundDrugController(
		...
		this.csvUtils = new CsvUtils<>(pcareNonCompoundDrugService, PcareNonCompoundDrugEntity.class);
	...
	public ResponseEntity<List<PcareNonCompoundDrugEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<PcareNonCompoundDrugEntity> pcareNonCompoundDrugs = pcareNonCompoundDrugService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(pcareNonCompoundDrugs, HttpStatus.OK);
	}

	/**
	 * Return the PCare NonCompound Drug that has the same id as the given id.
	 *
	 * @param id      The id of the PcareNonCompoundDrugEntity to be returned
	 * @return the PCare NonCompound Drug that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single PcareNonCompoundDrug as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "PcareNonCompoundDrug entity not found"),
	})
	@PreAuthorize("hasPermission('PcareNonCompoundDrugEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<PcareNonCompoundDrugEntity> getById(
			@Parameter(description = "The UUID of the PcareNonCompoundDrug to return.")
			@PathVariable("id") UUID id
	) {
		Optional<PcareNonCompoundDrugEntity> pcareNonCompoundDrugEntity = pcareNonCompoundDrugService.findById(id);
	...
	/**
	 * Create/Update the given PCare NonCompound Drug. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param pcareNonCompoundDrugEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'savePcareNonCompoundDrug' endpoint here] off begin
	@Operation(
			description = "Create a PcareNonCompoundDrug if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new PcareNonCompoundDrug."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new PcareNonCompoundDrug, duplicate record found")
	})
	@PreAuthorize("hasPermission('PcareNonCompoundDrugEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'savePcareNonCompoundDrug' endpoint here] end
	public ResponseEntity<?> savePcareNonCompoundDrug(
			@Parameter(description = "The PcareNonCompoundDrugEntity to create or update.")
			@RequestBody PcareNonCompoundDrugEntityDto pcareNonCompoundDrugEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			PcareNonCompoundDrugEntity pcareNonCompoundDrugEntity = pcareNonCompoundDrugService.create(new PcareNonCompoundDrugEntity(pcareNonCompoundDrugEntityDto));
			return new ResponseEntity<>(pcareNonCompoundDrugEntity, HttpStatus.OK);
	...
	/**
	 * Update the given PCare NonCompound Drug. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param pcareNonCompoundDrugEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updatePcareNonCompoundDrug' endpoint here] off begin
	@Operation(
			description = "Update a PcareNonCompoundDrug entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the PcareNonCompoundDrug."),
		@ApiResponse(responseCode = "400", description = "Failed to update the PcareNonCompoundDrug, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('PcareNonCompoundDrugEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updatePcareNonCompoundDrug' endpoint here] end
	public ResponseEntity<?> updatePcareNonCompoundDrug(
			@Parameter(description = "The PcareNonCompoundDrugEntity to create or update.")
			@RequestBody PcareNonCompoundDrugEntityDto pcareNonCompoundDrugEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<PcareNonCompoundDrugEntity> checkPcareNonCompoundDrugExists = pcareNonCompoundDrugService.findById(pcareNonCompoundDrugEntityDto.getId());
		PcareNonCompoundDrugEntity pcareNonCompoundDrugEntity = new PcareNonCompoundDrugEntity(pcareNonCompoundDrugEntityDto);

		if (checkPcareNonCompoundDrugExists.isPresent()) {
			try {
				pcareNonCompoundDrugEntity = pcareNonCompoundDrugService.update(pcareNonCompoundDrugEntity);
				return new ResponseEntity<>(pcareNonCompoundDrugEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(pcareNonCompoundDrugEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the PCare NonCompound Drug that has the same id as the given id.
	 *
	 * @param id The id of the PcareNonCompoundDrugEntity to be deleted
	...
	@Operation(
			description = "Delete a single PcareNonCompoundDrug as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the PcareNonCompoundDrug to delete.")
		...
		pcareNonCompoundDrugService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareNonCompoundDrug' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all PCare NonCompound Drug entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareNonCompoundDrug' endpoint here] end
	public void exportPcareNonCompoundDrug(
			@Parameter(description = "The ids of the PcareNonCompoundDrugEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<PcareNonCompoundDrugEntity> entitiesToExport = pcareNonCompoundDrugService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "PcareNonCompoundDrug_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (PcareNonCompoundDrugEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportPcareNonCompoundDrugExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportPcareNonCompoundDrugExcludingIds' endpoint here] end
	public void exportPcareNonCompoundDrugExcludingIds(
			@Parameter(description = "The ids of the PcareNonCompoundDrugEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<PcareNonCompoundDrugEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = pcareNonCompoundDrugService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = pcareNonCompoundDrugService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the PcareNonCompoundDrug entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "PcareNonCompoundDrug_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(PcareNonCompoundDrugEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for PCare NonCompound Drug entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('PcareNonCompoundDrugEntity', 'create')  || hasPermission('PcareNonCompoundDrugEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with PcareNonCompoundDrugEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("PcareNonCompoundDrugEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(PcareNonCompoundDrugEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<PcareNonCompoundDrugEntity> csvToBean = new CsvToBeanBuilder<PcareNonCompoundDrugEntity>(reader)
				.withType(PcareNonCompoundDrugEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<PcareNonCompoundDrugEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<PcareNonCompoundDrugEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a PCare NonCompound Drug entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a PCare NonCompound Drug entity and you do not have permission");
					}
