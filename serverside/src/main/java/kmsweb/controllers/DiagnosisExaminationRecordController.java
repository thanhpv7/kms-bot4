...
import kmsweb.entities.DiagnosisExaminationRecordEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Diagnosis Examination Record
 */
// % protected region % [Update the API route or description for the Diagnosis Examination RecordController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DiagnosisExaminationRecordController"))
@RestController
@Slf4j
@RequestMapping("/api/diagnosisexaminationrecord")
// % protected region % [Update the API route or description for the Diagnosis Examination RecordController here] end
public class DiagnosisExaminationRecordController {

	private final DiagnosisExaminationRecordService diagnosisExaminationRecordService;
	private final CsvUtils<DiagnosisExaminationRecordEntity, DiagnosisExaminationRecordRepository, DiagnosisExaminationRecordEntityAudit,  DiagnosisExaminationRecordService> csvUtils;
	...
	@Autowired
	public DiagnosisExaminationRecordController(
		...
		this.csvUtils = new CsvUtils<>(diagnosisExaminationRecordService, DiagnosisExaminationRecordEntity.class);
	...
	/**
	 * Return all the Diagnosis Examination Records.
	 *
	 * @return all the Diagnosis Examination Records
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of DiagnosisExaminationRecords",
		security = {
	...
	public ResponseEntity<List<DiagnosisExaminationRecordEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DiagnosisExaminationRecordEntity> diagnosisExaminationRecords = diagnosisExaminationRecordService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(diagnosisExaminationRecords, HttpStatus.OK);
	}

	/**
	 * Return the Diagnosis Examination Record that has the same id as the given id.
	 *
	 * @param id      The id of the DiagnosisExaminationRecordEntity to be returned
	 * @return the Diagnosis Examination Record that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DiagnosisExaminationRecord as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DiagnosisExaminationRecord entity not found"),
	})
	@PreAuthorize("hasPermission('DiagnosisExaminationRecordEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DiagnosisExaminationRecordEntity> getById(
			@Parameter(description = "The UUID of the DiagnosisExaminationRecord to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DiagnosisExaminationRecordEntity> diagnosisExaminationRecordEntity = diagnosisExaminationRecordService.findById(id);
	...
	/**
	 * Create/Update the given Diagnosis Examination Record. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosisExaminationRecordEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDiagnosisExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Create a DiagnosisExaminationRecord if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DiagnosisExaminationRecord."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DiagnosisExaminationRecord, duplicate record found")
	})
	@PreAuthorize("hasPermission('DiagnosisExaminationRecordEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDiagnosisExaminationRecord' endpoint here] end
	public ResponseEntity<?> saveDiagnosisExaminationRecord(
			@Parameter(description = "The DiagnosisExaminationRecordEntity to create or update.")
			@RequestBody DiagnosisExaminationRecordEntityDto diagnosisExaminationRecordEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DiagnosisExaminationRecordEntity diagnosisExaminationRecordEntity = diagnosisExaminationRecordService.create(new DiagnosisExaminationRecordEntity(diagnosisExaminationRecordEntityDto));
			return new ResponseEntity<>(diagnosisExaminationRecordEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Diagnosis Examination Record. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param diagnosisExaminationRecordEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDiagnosisExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Update a DiagnosisExaminationRecord entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DiagnosisExaminationRecord."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DiagnosisExaminationRecord, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DiagnosisExaminationRecordEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDiagnosisExaminationRecord' endpoint here] end
	public ResponseEntity<?> updateDiagnosisExaminationRecord(
			@Parameter(description = "The DiagnosisExaminationRecordEntity to create or update.")
			@RequestBody DiagnosisExaminationRecordEntityDto diagnosisExaminationRecordEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DiagnosisExaminationRecordEntity> checkDiagnosisExaminationRecordExists = diagnosisExaminationRecordService.findById(diagnosisExaminationRecordEntityDto.getId());
		DiagnosisExaminationRecordEntity diagnosisExaminationRecordEntity = new DiagnosisExaminationRecordEntity(diagnosisExaminationRecordEntityDto);

		if (checkDiagnosisExaminationRecordExists.isPresent()) {
			try {
				diagnosisExaminationRecordEntity = diagnosisExaminationRecordService.update(diagnosisExaminationRecordEntity);
				return new ResponseEntity<>(diagnosisExaminationRecordEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(diagnosisExaminationRecordEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Diagnosis Examination Record that has the same id as the given id.
	 *
	 * @param id The id of the DiagnosisExaminationRecordEntity to be deleted
	...
	@Operation(
			description = "Delete a single DiagnosisExaminationRecord as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DiagnosisExaminationRecord to delete.")
		...
		diagnosisExaminationRecordService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisExaminationRecord' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Diagnosis Examination Record entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisExaminationRecord' endpoint here] end
	public void exportDiagnosisExaminationRecord(
			@Parameter(description = "The ids of the DiagnosisExaminationRecordEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DiagnosisExaminationRecordEntity> entitiesToExport = diagnosisExaminationRecordService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DiagnosisExaminationRecord_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DiagnosisExaminationRecordEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisExaminationRecordExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDiagnosisExaminationRecordExcludingIds' endpoint here] end
	public void exportDiagnosisExaminationRecordExcludingIds(
			@Parameter(description = "The ids of the DiagnosisExaminationRecordEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DiagnosisExaminationRecordEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = diagnosisExaminationRecordService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = diagnosisExaminationRecordService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DiagnosisExaminationRecord entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DiagnosisExaminationRecord_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DiagnosisExaminationRecordEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Diagnosis Examination Record entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DiagnosisExaminationRecordEntity', 'create')  || hasPermission('DiagnosisExaminationRecordEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DiagnosisExaminationRecordEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DiagnosisExaminationRecordEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DiagnosisExaminationRecordEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DiagnosisExaminationRecordEntity> csvToBean = new CsvToBeanBuilder<DiagnosisExaminationRecordEntity>(reader)
				.withType(DiagnosisExaminationRecordEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DiagnosisExaminationRecordEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DiagnosisExaminationRecordEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Diagnosis Examination Record entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Diagnosis Examination Record entity and you do not have permission");
					}
