...
/**
 * Controller used to handle all REST operations regarding Daily Care CPPT
 */
// % protected region % [Update the API route or description for the Daily Care CPPTController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "DailyCareCPPTController"))
@RestController
@Slf4j
@RequestMapping("/api/dailycarecppt")
// % protected region % [Update the API route or description for the Daily Care CPPTController here] end
public class DailyCareCPPTController {

	private final DailyCareCPPTService dailyCareCPPTService;
	private final CsvUtils<DailyCareCPPTEntity, DailyCareCPPTRepository, DailyCareCPPTEntityAudit,  DailyCareCPPTService> csvUtils;
	...
	@Autowired
	public DailyCareCPPTController(
		...
		this.csvUtils = new CsvUtils<>(dailyCareCPPTService, DailyCareCPPTEntity.class);
	...
	public ResponseEntity<List<DailyCareCPPTEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<DailyCareCPPTEntity> dailyCareCPPTs = dailyCareCPPTService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(dailyCareCPPTs, HttpStatus.OK);
	}

	/**
	 * Return the Daily Care CPPT that has the same id as the given id.
	 *
	 * @param id      The id of the DailyCareCPPTEntity to be returned
	 * @return the Daily Care CPPT that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single DailyCareCPPT as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "DailyCareCPPT entity not found"),
	})
	@PreAuthorize("hasPermission('DailyCareCPPTEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<DailyCareCPPTEntity> getById(
			@Parameter(description = "The UUID of the DailyCareCPPT to return.")
			@PathVariable("id") UUID id
	) {
		Optional<DailyCareCPPTEntity> dailyCareCPPTEntity = dailyCareCPPTService.findById(id);
	...
	/**
	 * Create/Update the given Daily Care CPPT. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param dailyCareCPPTEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveDailyCareCPPT' endpoint here] off begin
	@Operation(
			description = "Create a DailyCareCPPT if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new DailyCareCPPT."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new DailyCareCPPT, duplicate record found")
	})
	@PreAuthorize("hasPermission('DailyCareCPPTEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveDailyCareCPPT' endpoint here] end
	public ResponseEntity<?> saveDailyCareCPPT(
			@Parameter(description = "The DailyCareCPPTEntity to create or update.")
			@RequestBody DailyCareCPPTEntityDto dailyCareCPPTEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			DailyCareCPPTEntity dailyCareCPPTEntity = dailyCareCPPTService.create(new DailyCareCPPTEntity(dailyCareCPPTEntityDto));
			return new ResponseEntity<>(dailyCareCPPTEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Daily Care CPPT. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param dailyCareCPPTEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateDailyCareCPPT' endpoint here] off begin
	@Operation(
			description = "Update a DailyCareCPPT entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the DailyCareCPPT."),
		@ApiResponse(responseCode = "400", description = "Failed to update the DailyCareCPPT, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('DailyCareCPPTEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateDailyCareCPPT' endpoint here] end
	public ResponseEntity<?> updateDailyCareCPPT(
			@Parameter(description = "The DailyCareCPPTEntity to create or update.")
			@RequestBody DailyCareCPPTEntityDto dailyCareCPPTEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<DailyCareCPPTEntity> checkDailyCareCPPTExists = dailyCareCPPTService.findById(dailyCareCPPTEntityDto.getId());
		DailyCareCPPTEntity dailyCareCPPTEntity = new DailyCareCPPTEntity(dailyCareCPPTEntityDto);

		if (checkDailyCareCPPTExists.isPresent()) {
			try {
				dailyCareCPPTEntity = dailyCareCPPTService.update(dailyCareCPPTEntity);
				return new ResponseEntity<>(dailyCareCPPTEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(dailyCareCPPTEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Daily Care CPPT that has the same id as the given id.
	 *
	 * @param id The id of the DailyCareCPPTEntity to be deleted
	...
	@Operation(
			description = "Delete a single DailyCareCPPT as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the DailyCareCPPT to delete.")
		...
		dailyCareCPPTService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportDailyCareCPPT' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Daily Care CPPT entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportDailyCareCPPT' endpoint here] end
	public void exportDailyCareCPPT(
			@Parameter(description = "The ids of the DailyCareCPPTEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<DailyCareCPPTEntity> entitiesToExport = dailyCareCPPTService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "DailyCareCPPT_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (DailyCareCPPTEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportDailyCareCPPTExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportDailyCareCPPTExcludingIds' endpoint here] end
	public void exportDailyCareCPPTExcludingIds(
			@Parameter(description = "The ids of the DailyCareCPPTEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<DailyCareCPPTEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = dailyCareCPPTService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = dailyCareCPPTService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the DailyCareCPPT entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "DailyCareCPPT_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(DailyCareCPPTEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Daily Care CPPT entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('DailyCareCPPTEntity', 'create')  || hasPermission('DailyCareCPPTEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with DailyCareCPPTEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("DailyCareCPPTEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(DailyCareCPPTEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<DailyCareCPPTEntity> csvToBean = new CsvToBeanBuilder<DailyCareCPPTEntity>(reader)
				.withType(DailyCareCPPTEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<DailyCareCPPTEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<DailyCareCPPTEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Daily Care CPPT entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Daily Care CPPT entity and you do not have permission");
					}
