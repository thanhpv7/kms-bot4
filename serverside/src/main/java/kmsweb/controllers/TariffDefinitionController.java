...
import kmsweb.entities.TariffDefinitionEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Tariff Definition
 */
// % protected region % [Update the API route or description for the Tariff DefinitionController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TariffDefinitionController"))
@RestController
@Slf4j
@RequestMapping("/api/tariffdefinition")
// % protected region % [Update the API route or description for the Tariff DefinitionController here] end
public class TariffDefinitionController {

	private final TariffDefinitionService tariffDefinitionService;
	private final CsvUtils<TariffDefinitionEntity, TariffDefinitionRepository, TariffDefinitionEntityAudit,  TariffDefinitionService> csvUtils;
	...
	@Autowired
	public TariffDefinitionController(
		...
		this.csvUtils = new CsvUtils<>(tariffDefinitionService, TariffDefinitionEntity.class);
	...
	public ResponseEntity<List<TariffDefinitionEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TariffDefinitionEntity> tariffDefinitions = tariffDefinitionService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(tariffDefinitions, HttpStatus.OK);
	}

	/**
	 * Return the Tariff Definition that has the same id as the given id.
	 *
	 * @param id      The id of the TariffDefinitionEntity to be returned
	 * @return the Tariff Definition that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TariffDefinition as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TariffDefinition entity not found"),
	})
	@PreAuthorize("hasPermission('TariffDefinitionEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TariffDefinitionEntity> getById(
			@Parameter(description = "The UUID of the TariffDefinition to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TariffDefinitionEntity> tariffDefinitionEntity = tariffDefinitionService.findById(id);
	...
	/**
	 * Create/Update the given Tariff Definition. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param tariffDefinitionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTariffDefinition' endpoint here] off begin
	@Operation(
			description = "Create a TariffDefinition if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TariffDefinition."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TariffDefinition, duplicate record found")
	})
	@PreAuthorize("hasPermission('TariffDefinitionEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTariffDefinition' endpoint here] end
	public ResponseEntity<?> saveTariffDefinition(
			@Parameter(description = "The TariffDefinitionEntity to create or update.")
			@RequestBody TariffDefinitionEntityDto tariffDefinitionEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TariffDefinitionEntity tariffDefinitionEntity = tariffDefinitionService.create(new TariffDefinitionEntity(tariffDefinitionEntityDto));
			return new ResponseEntity<>(tariffDefinitionEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Tariff Definition. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param tariffDefinitionEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTariffDefinition' endpoint here] off begin
	@Operation(
			description = "Update a TariffDefinition entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TariffDefinition."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TariffDefinition, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TariffDefinitionEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTariffDefinition' endpoint here] end
	public ResponseEntity<?> updateTariffDefinition(
			@Parameter(description = "The TariffDefinitionEntity to create or update.")
			@RequestBody TariffDefinitionEntityDto tariffDefinitionEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TariffDefinitionEntity> checkTariffDefinitionExists = tariffDefinitionService.findById(tariffDefinitionEntityDto.getId());
		TariffDefinitionEntity tariffDefinitionEntity = new TariffDefinitionEntity(tariffDefinitionEntityDto);

		if (checkTariffDefinitionExists.isPresent()) {
			try {
				tariffDefinitionEntity = tariffDefinitionService.update(tariffDefinitionEntity);
				return new ResponseEntity<>(tariffDefinitionEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(tariffDefinitionEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Tariff Definition that has the same id as the given id.
	 *
	 * @param id The id of the TariffDefinitionEntity to be deleted
	...
	@Operation(
			description = "Delete a single TariffDefinition as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TariffDefinition to delete.")
		...
		tariffDefinitionService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinition' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Tariff Definition entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinition' endpoint here] end
	public void exportTariffDefinition(
			@Parameter(description = "The ids of the TariffDefinitionEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TariffDefinitionEntity> entitiesToExport = tariffDefinitionService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TariffDefinition_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TariffDefinitionEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinitionExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTariffDefinitionExcludingIds' endpoint here] end
	public void exportTariffDefinitionExcludingIds(
			@Parameter(description = "The ids of the TariffDefinitionEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TariffDefinitionEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = tariffDefinitionService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = tariffDefinitionService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TariffDefinition entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TariffDefinition_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TariffDefinitionEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Tariff Definition entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TariffDefinitionEntity', 'create')  || hasPermission('TariffDefinitionEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TariffDefinitionEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TariffDefinitionEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TariffDefinitionEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TariffDefinitionEntity> csvToBean = new CsvToBeanBuilder<TariffDefinitionEntity>(reader)
				.withType(TariffDefinitionEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TariffDefinitionEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TariffDefinitionEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Tariff Definition entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Tariff Definition entity and you do not have permission");
					}
