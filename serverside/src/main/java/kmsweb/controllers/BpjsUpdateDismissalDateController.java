...
import kmsweb.dtos.BpjsUpdateDismissalDateEntityDto;
import kmsweb.entities.BpjsUpdateDismissalDateEntityAudit;
import kmsweb.repositories.BpjsUpdateDismissalDateRepository;
import kmsweb.services.BpjsUpdateDismissalDateService;
...
/**
 * Controller used to handle all REST operations regarding BPJS Update Dismissal Date
 */
// % protected region % [Update the API route or description for the BPJS Update Dismissal DateController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "BpjsUpdateDismissalDateController"))
@RestController
@Slf4j
@RequestMapping("/api/bpjsupdatedismissaldate")
// % protected region % [Update the API route or description for the BPJS Update Dismissal DateController here] end
public class BpjsUpdateDismissalDateController {

	private final BpjsUpdateDismissalDateService bpjsUpdateDismissalDateService;
	private final CsvUtils<BpjsUpdateDismissalDateEntity, BpjsUpdateDismissalDateRepository, BpjsUpdateDismissalDateEntityAudit,  BpjsUpdateDismissalDateService> csvUtils;
	...
	@Autowired
	public BpjsUpdateDismissalDateController(
			// % protected region % [Add any additional constructor parameters here] off begin
			// % protected region % [Add any additional constructor parameters here] end
			AuthenticationService authService,
			BpjsUpdateDismissalDateService bpjsUpdateDismissalDateService
	) {
		this.authService = authService;
		this.bpjsUpdateDismissalDateService = bpjsUpdateDismissalDateService;
		this.csvUtils = new CsvUtils<>(bpjsUpdateDismissalDateService, BpjsUpdateDismissalDateEntity.class);
	...
	/**
	 * Return all the BPJS Update Dismissal Dates.
	 *
	 * @return all the BPJS Update Dismissal Dates
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of BpjsUpdateDismissalDates",
		security = {
	...
	public ResponseEntity<List<BpjsUpdateDismissalDateEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<BpjsUpdateDismissalDateEntity> bpjsUpdateDismissalDates = bpjsUpdateDismissalDateService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(bpjsUpdateDismissalDates, HttpStatus.OK);
	}

	/**
	 * Return the BPJS Update Dismissal Date that has the same id as the given id.
	 *
	 * @param id      The id of the BpjsUpdateDismissalDateEntity to be returned
	 * @return the BPJS Update Dismissal Date that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single BpjsUpdateDismissalDate as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "BpjsUpdateDismissalDate entity not found"),
	})
	@PreAuthorize("hasPermission('BpjsUpdateDismissalDateEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<BpjsUpdateDismissalDateEntity> getById(
			@Parameter(description = "The UUID of the BpjsUpdateDismissalDate to return.")
			@PathVariable("id") UUID id
	) {
		Optional<BpjsUpdateDismissalDateEntity> bpjsUpdateDismissalDateEntity = bpjsUpdateDismissalDateService.findById(id);
	...
	/**
	 * Create/Update the given BPJS Update Dismissal Date. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsUpdateDismissalDateEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveBpjsUpdateDismissalDate' endpoint here] off begin
	@Operation(
			description = "Create a BpjsUpdateDismissalDate if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new BpjsUpdateDismissalDate."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new BpjsUpdateDismissalDate, duplicate record found")
	})
	@PreAuthorize("hasPermission('BpjsUpdateDismissalDateEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveBpjsUpdateDismissalDate' endpoint here] end
	public ResponseEntity<?> saveBpjsUpdateDismissalDate(
			@Parameter(description = "The BpjsUpdateDismissalDateEntity to create or update.")
			@RequestBody BpjsUpdateDismissalDateEntityDto bpjsUpdateDismissalDateEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			BpjsUpdateDismissalDateEntity bpjsUpdateDismissalDateEntity = bpjsUpdateDismissalDateService.create(new BpjsUpdateDismissalDateEntity(bpjsUpdateDismissalDateEntityDto));
			return new ResponseEntity<>(bpjsUpdateDismissalDateEntity, HttpStatus.OK);
	...
	/**
	 * Update the given BPJS Update Dismissal Date. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param bpjsUpdateDismissalDateEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateBpjsUpdateDismissalDate' endpoint here] off begin
	@Operation(
			description = "Update a BpjsUpdateDismissalDate entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the BpjsUpdateDismissalDate."),
		@ApiResponse(responseCode = "400", description = "Failed to update the BpjsUpdateDismissalDate, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('BpjsUpdateDismissalDateEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateBpjsUpdateDismissalDate' endpoint here] end
	public ResponseEntity<?> updateBpjsUpdateDismissalDate(
			@Parameter(description = "The BpjsUpdateDismissalDateEntity to create or update.")
			@RequestBody BpjsUpdateDismissalDateEntityDto bpjsUpdateDismissalDateEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<BpjsUpdateDismissalDateEntity> checkBpjsUpdateDismissalDateExists = bpjsUpdateDismissalDateService.findById(bpjsUpdateDismissalDateEntityDto.getId());
		BpjsUpdateDismissalDateEntity bpjsUpdateDismissalDateEntity = new BpjsUpdateDismissalDateEntity(bpjsUpdateDismissalDateEntityDto);

		if (checkBpjsUpdateDismissalDateExists.isPresent()) {
			try {
				bpjsUpdateDismissalDateEntity = bpjsUpdateDismissalDateService.update(bpjsUpdateDismissalDateEntity);
				return new ResponseEntity<>(bpjsUpdateDismissalDateEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(bpjsUpdateDismissalDateEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the BPJS Update Dismissal Date that has the same id as the given id.
	 *
	 * @param id The id of the BpjsUpdateDismissalDateEntity to be deleted
	...
	@Operation(
			description = "Delete a single BpjsUpdateDismissalDate as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the BpjsUpdateDismissalDate to delete.")
		...
		bpjsUpdateDismissalDateService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsUpdateDismissalDate' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all BPJS Update Dismissal Date entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsUpdateDismissalDate' endpoint here] end
	public void exportBpjsUpdateDismissalDate(
			@Parameter(description = "The ids of the BpjsUpdateDismissalDateEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<BpjsUpdateDismissalDateEntity> entitiesToExport = bpjsUpdateDismissalDateService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "BpjsUpdateDismissalDate_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (BpjsUpdateDismissalDateEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportBpjsUpdateDismissalDateExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportBpjsUpdateDismissalDateExcludingIds' endpoint here] end
	public void exportBpjsUpdateDismissalDateExcludingIds(
			@Parameter(description = "The ids of the BpjsUpdateDismissalDateEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<BpjsUpdateDismissalDateEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = bpjsUpdateDismissalDateService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = bpjsUpdateDismissalDateService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the BpjsUpdateDismissalDate entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "BpjsUpdateDismissalDate_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(BpjsUpdateDismissalDateEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for BPJS Update Dismissal Date entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('BpjsUpdateDismissalDateEntity', 'create')  || hasPermission('BpjsUpdateDismissalDateEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with BpjsUpdateDismissalDateEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("BpjsUpdateDismissalDateEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(BpjsUpdateDismissalDateEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<BpjsUpdateDismissalDateEntity> csvToBean = new CsvToBeanBuilder<BpjsUpdateDismissalDateEntity>(reader)
				.withType(BpjsUpdateDismissalDateEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<BpjsUpdateDismissalDateEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<BpjsUpdateDismissalDateEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a BPJS Update Dismissal Date entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a BPJS Update Dismissal Date entity and you do not have permission");
					}
