...
import kmsweb.entities.CompoundPrescriptionItemEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Compound Prescription Item
 */
// % protected region % [Update the API route or description for the Compound Prescription ItemController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "CompoundPrescriptionItemController"))
@RestController
@Slf4j
@RequestMapping("/api/compoundprescriptionitem")
// % protected region % [Update the API route or description for the Compound Prescription ItemController here] end
public class CompoundPrescriptionItemController {

	private final CompoundPrescriptionItemService compoundPrescriptionItemService;
	private final CsvUtils<CompoundPrescriptionItemEntity, CompoundPrescriptionItemRepository, CompoundPrescriptionItemEntityAudit,  CompoundPrescriptionItemService> csvUtils;
	...
	@Autowired
	public CompoundPrescriptionItemController(
		...
		this.csvUtils = new CsvUtils<>(compoundPrescriptionItemService, CompoundPrescriptionItemEntity.class);
	...
	/**
	 * Return all the Compound Prescription Items.
	 *
	 * @return all the Compound Prescription Items
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of CompoundPrescriptionItems",
		security = {
	...
	public ResponseEntity<List<CompoundPrescriptionItemEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<CompoundPrescriptionItemEntity> compoundPrescriptionItems = compoundPrescriptionItemService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(compoundPrescriptionItems, HttpStatus.OK);
	}

	/**
	 * Return the Compound Prescription Item that has the same id as the given id.
	 *
	 * @param id      The id of the CompoundPrescriptionItemEntity to be returned
	 * @return the Compound Prescription Item that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single CompoundPrescriptionItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "CompoundPrescriptionItem entity not found"),
	})
	@PreAuthorize("hasPermission('CompoundPrescriptionItemEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<CompoundPrescriptionItemEntity> getById(
			@Parameter(description = "The UUID of the CompoundPrescriptionItem to return.")
			@PathVariable("id") UUID id
	) {
		Optional<CompoundPrescriptionItemEntity> compoundPrescriptionItemEntity = compoundPrescriptionItemService.findById(id);
	...
	/**
	 * Create/Update the given Compound Prescription Item. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param compoundPrescriptionItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveCompoundPrescriptionItem' endpoint here] off begin
	@Operation(
			description = "Create a CompoundPrescriptionItem if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new CompoundPrescriptionItem."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new CompoundPrescriptionItem, duplicate record found")
	})
	@PreAuthorize("hasPermission('CompoundPrescriptionItemEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveCompoundPrescriptionItem' endpoint here] end
	public ResponseEntity<?> saveCompoundPrescriptionItem(
			@Parameter(description = "The CompoundPrescriptionItemEntity to create or update.")
			@RequestBody CompoundPrescriptionItemEntityDto compoundPrescriptionItemEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			CompoundPrescriptionItemEntity compoundPrescriptionItemEntity = compoundPrescriptionItemService.create(new CompoundPrescriptionItemEntity(compoundPrescriptionItemEntityDto));
			return new ResponseEntity<>(compoundPrescriptionItemEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Compound Prescription Item. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param compoundPrescriptionItemEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateCompoundPrescriptionItem' endpoint here] off begin
	@Operation(
			description = "Update a CompoundPrescriptionItem entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the CompoundPrescriptionItem."),
		@ApiResponse(responseCode = "400", description = "Failed to update the CompoundPrescriptionItem, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('CompoundPrescriptionItemEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateCompoundPrescriptionItem' endpoint here] end
	public ResponseEntity<?> updateCompoundPrescriptionItem(
			@Parameter(description = "The CompoundPrescriptionItemEntity to create or update.")
			@RequestBody CompoundPrescriptionItemEntityDto compoundPrescriptionItemEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<CompoundPrescriptionItemEntity> checkCompoundPrescriptionItemExists = compoundPrescriptionItemService.findById(compoundPrescriptionItemEntityDto.getId());
		CompoundPrescriptionItemEntity compoundPrescriptionItemEntity = new CompoundPrescriptionItemEntity(compoundPrescriptionItemEntityDto);

		if (checkCompoundPrescriptionItemExists.isPresent()) {
			try {
				compoundPrescriptionItemEntity = compoundPrescriptionItemService.update(compoundPrescriptionItemEntity);
				return new ResponseEntity<>(compoundPrescriptionItemEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(compoundPrescriptionItemEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Compound Prescription Item that has the same id as the given id.
	 *
	 * @param id The id of the CompoundPrescriptionItemEntity to be deleted
	...
	@Operation(
			description = "Delete a single CompoundPrescriptionItem as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the CompoundPrescriptionItem to delete.")
		...
		compoundPrescriptionItemService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionItem' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Compound Prescription Item entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionItem' endpoint here] end
	public void exportCompoundPrescriptionItem(
			@Parameter(description = "The ids of the CompoundPrescriptionItemEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<CompoundPrescriptionItemEntity> entitiesToExport = compoundPrescriptionItemService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "CompoundPrescriptionItem_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (CompoundPrescriptionItemEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionItemExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportCompoundPrescriptionItemExcludingIds' endpoint here] end
	public void exportCompoundPrescriptionItemExcludingIds(
			@Parameter(description = "The ids of the CompoundPrescriptionItemEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<CompoundPrescriptionItemEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = compoundPrescriptionItemService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = compoundPrescriptionItemService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the CompoundPrescriptionItem entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "CompoundPrescriptionItem_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(CompoundPrescriptionItemEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Compound Prescription Item entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('CompoundPrescriptionItemEntity', 'create')  || hasPermission('CompoundPrescriptionItemEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with CompoundPrescriptionItemEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("CompoundPrescriptionItemEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(CompoundPrescriptionItemEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<CompoundPrescriptionItemEntity> csvToBean = new CsvToBeanBuilder<CompoundPrescriptionItemEntity>(reader)
				.withType(CompoundPrescriptionItemEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<CompoundPrescriptionItemEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<CompoundPrescriptionItemEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Compound Prescription Item entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Compound Prescription Item entity and you do not have permission");
					}
