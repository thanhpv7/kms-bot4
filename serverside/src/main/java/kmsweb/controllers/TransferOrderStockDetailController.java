...
import kmsweb.entities.TransferOrderStockDetailEntityAudit;
...
/**
 * Controller used to handle all REST operations regarding Transfer Order Stock Detail
 */
// % protected region % [Update the API route or description for the Transfer Order Stock DetailController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "TransferOrderStockDetailController"))
@RestController
@Slf4j
@RequestMapping("/api/transferorderstockdetail")
// % protected region % [Update the API route or description for the Transfer Order Stock DetailController here] end
public class TransferOrderStockDetailController {

	private final TransferOrderStockDetailService transferOrderStockDetailService;
	private final CsvUtils<TransferOrderStockDetailEntity, TransferOrderStockDetailRepository, TransferOrderStockDetailEntityAudit,  TransferOrderStockDetailService> csvUtils;
	...
	@Autowired
	public TransferOrderStockDetailController(
		...
		this.csvUtils = new CsvUtils<>(transferOrderStockDetailService, TransferOrderStockDetailEntity.class);
	...
	/**
	 * Return all the Transfer Order Stock Details.
	 *
	 * @return all the Transfer Order Stock Details
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of TransferOrderStockDetails",
		security = {
	...
	public ResponseEntity<List<TransferOrderStockDetailEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<TransferOrderStockDetailEntity> transferOrderStockDetails = transferOrderStockDetailService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(transferOrderStockDetails, HttpStatus.OK);
	}

	/**
	 * Return the Transfer Order Stock Detail that has the same id as the given id.
	 *
	 * @param id      The id of the TransferOrderStockDetailEntity to be returned
	 * @return the Transfer Order Stock Detail that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single TransferOrderStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "TransferOrderStockDetail entity not found"),
	})
	@PreAuthorize("hasPermission('TransferOrderStockDetailEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<TransferOrderStockDetailEntity> getById(
			@Parameter(description = "The UUID of the TransferOrderStockDetail to return.")
			@PathVariable("id") UUID id
	) {
		Optional<TransferOrderStockDetailEntity> transferOrderStockDetailEntity = transferOrderStockDetailService.findById(id);
	...
	/**
	 * Create/Update the given Transfer Order Stock Detail. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param transferOrderStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveTransferOrderStockDetail' endpoint here] off begin
	@Operation(
			description = "Create a TransferOrderStockDetail if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new TransferOrderStockDetail."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new TransferOrderStockDetail, duplicate record found")
	})
	@PreAuthorize("hasPermission('TransferOrderStockDetailEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveTransferOrderStockDetail' endpoint here] end
	public ResponseEntity<?> saveTransferOrderStockDetail(
			@Parameter(description = "The TransferOrderStockDetailEntity to create or update.")
			@RequestBody TransferOrderStockDetailEntityDto transferOrderStockDetailEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			TransferOrderStockDetailEntity transferOrderStockDetailEntity = transferOrderStockDetailService.create(new TransferOrderStockDetailEntity(transferOrderStockDetailEntityDto));
			return new ResponseEntity<>(transferOrderStockDetailEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Transfer Order Stock Detail. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param transferOrderStockDetailEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateTransferOrderStockDetail' endpoint here] off begin
	@Operation(
			description = "Update a TransferOrderStockDetail entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the TransferOrderStockDetail."),
		@ApiResponse(responseCode = "400", description = "Failed to update the TransferOrderStockDetail, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('TransferOrderStockDetailEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateTransferOrderStockDetail' endpoint here] end
	public ResponseEntity<?> updateTransferOrderStockDetail(
			@Parameter(description = "The TransferOrderStockDetailEntity to create or update.")
			@RequestBody TransferOrderStockDetailEntityDto transferOrderStockDetailEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<TransferOrderStockDetailEntity> checkTransferOrderStockDetailExists = transferOrderStockDetailService.findById(transferOrderStockDetailEntityDto.getId());
		TransferOrderStockDetailEntity transferOrderStockDetailEntity = new TransferOrderStockDetailEntity(transferOrderStockDetailEntityDto);

		if (checkTransferOrderStockDetailExists.isPresent()) {
			try {
				transferOrderStockDetailEntity = transferOrderStockDetailService.update(transferOrderStockDetailEntity);
				return new ResponseEntity<>(transferOrderStockDetailEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(transferOrderStockDetailEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Transfer Order Stock Detail that has the same id as the given id.
	 *
	 * @param id The id of the TransferOrderStockDetailEntity to be deleted
	...
	@Operation(
			description = "Delete a single TransferOrderStockDetail as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the TransferOrderStockDetail to delete.")
		...
		transferOrderStockDetailService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransferOrderStockDetail' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Transfer Order Stock Detail entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportTransferOrderStockDetail' endpoint here] end
	public void exportTransferOrderStockDetail(
			@Parameter(description = "The ids of the TransferOrderStockDetailEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<TransferOrderStockDetailEntity> entitiesToExport = transferOrderStockDetailService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "TransferOrderStockDetail_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (TransferOrderStockDetailEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportTransferOrderStockDetailExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportTransferOrderStockDetailExcludingIds' endpoint here] end
	public void exportTransferOrderStockDetailExcludingIds(
			@Parameter(description = "The ids of the TransferOrderStockDetailEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<TransferOrderStockDetailEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = transferOrderStockDetailService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = transferOrderStockDetailService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the TransferOrderStockDetail entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "TransferOrderStockDetail_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(TransferOrderStockDetailEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Transfer Order Stock Detail entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('TransferOrderStockDetailEntity', 'create')  || hasPermission('TransferOrderStockDetailEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with TransferOrderStockDetailEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("TransferOrderStockDetailEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(TransferOrderStockDetailEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<TransferOrderStockDetailEntity> csvToBean = new CsvToBeanBuilder<TransferOrderStockDetailEntity>(reader)
				.withType(TransferOrderStockDetailEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<TransferOrderStockDetailEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<TransferOrderStockDetailEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Transfer Order Stock Detail entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Transfer Order Stock Detail entity and you do not have permission");
					}
