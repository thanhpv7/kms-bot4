...
/**
 * Controller used to handle all REST operations regarding Merchant Surcharge and Fee
 */
// % protected region % [Update the API route or description for the Merchant Surcharge and FeeController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "MerchantSurchargeAndFeeController"))
@RestController
@Slf4j
@RequestMapping("/api/merchantsurchargeandfee")
// % protected region % [Update the API route or description for the Merchant Surcharge and FeeController here] end
public class MerchantSurchargeAndFeeController {

	private final MerchantSurchargeAndFeeService merchantSurchargeAndFeeService;
	private final CsvUtils<MerchantSurchargeAndFeeEntity, MerchantSurchargeAndFeeRepository, MerchantSurchargeAndFeeEntityAudit,  MerchantSurchargeAndFeeService> csvUtils;
	...
	@Autowired
	public MerchantSurchargeAndFeeController(
		...
		this.csvUtils = new CsvUtils<>(merchantSurchargeAndFeeService, MerchantSurchargeAndFeeEntity.class);
	...
	/**
	 * Return all the Merchant Surcharge and Fees.
	 *
	 * @return all the Merchant Surcharge and Fees
	 */
	// % protected region % [Update the route mapping or description for the 'getAllWithPage' endpoint here] off begin
	@Operation(
			description = "Returns a single page of MerchantSurchargeAndFees",
		security = {
	...
	public ResponseEntity<List<MerchantSurchargeAndFeeEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<MerchantSurchargeAndFeeEntity> merchantSurchargeAndFees = merchantSurchargeAndFeeService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(merchantSurchargeAndFees, HttpStatus.OK);
	}

	/**
	 * Return the Merchant Surcharge and Fee that has the same id as the given id.
	 *
	 * @param id      The id of the MerchantSurchargeAndFeeEntity to be returned
	 * @return the Merchant Surcharge and Fee that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single MerchantSurchargeAndFee as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "MerchantSurchargeAndFee entity not found"),
	})
	@PreAuthorize("hasPermission('MerchantSurchargeAndFeeEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<MerchantSurchargeAndFeeEntity> getById(
			@Parameter(description = "The UUID of the MerchantSurchargeAndFee to return.")
			@PathVariable("id") UUID id
	) {
		Optional<MerchantSurchargeAndFeeEntity> merchantSurchargeAndFeeEntity = merchantSurchargeAndFeeService.findById(id);
	...
	/**
	 * Create/Update the given Merchant Surcharge and Fee. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param merchantSurchargeAndFeeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveMerchantSurchargeAndFee' endpoint here] off begin
	@Operation(
			description = "Create a MerchantSurchargeAndFee if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new MerchantSurchargeAndFee."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new MerchantSurchargeAndFee, duplicate record found")
	})
	@PreAuthorize("hasPermission('MerchantSurchargeAndFeeEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveMerchantSurchargeAndFee' endpoint here] end
	public ResponseEntity<?> saveMerchantSurchargeAndFee(
			@Parameter(description = "The MerchantSurchargeAndFeeEntity to create or update.")
			@RequestBody MerchantSurchargeAndFeeEntityDto merchantSurchargeAndFeeEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			MerchantSurchargeAndFeeEntity merchantSurchargeAndFeeEntity = merchantSurchargeAndFeeService.create(new MerchantSurchargeAndFeeEntity(merchantSurchargeAndFeeEntityDto));
			return new ResponseEntity<>(merchantSurchargeAndFeeEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Merchant Surcharge and Fee. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param merchantSurchargeAndFeeEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateMerchantSurchargeAndFee' endpoint here] off begin
	@Operation(
			description = "Update a MerchantSurchargeAndFee entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the MerchantSurchargeAndFee."),
		@ApiResponse(responseCode = "400", description = "Failed to update the MerchantSurchargeAndFee, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('MerchantSurchargeAndFeeEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateMerchantSurchargeAndFee' endpoint here] end
	public ResponseEntity<?> updateMerchantSurchargeAndFee(
			@Parameter(description = "The MerchantSurchargeAndFeeEntity to create or update.")
			@RequestBody MerchantSurchargeAndFeeEntityDto merchantSurchargeAndFeeEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<MerchantSurchargeAndFeeEntity> checkMerchantSurchargeAndFeeExists = merchantSurchargeAndFeeService.findById(merchantSurchargeAndFeeEntityDto.getId());
		MerchantSurchargeAndFeeEntity merchantSurchargeAndFeeEntity = new MerchantSurchargeAndFeeEntity(merchantSurchargeAndFeeEntityDto);

		if (checkMerchantSurchargeAndFeeExists.isPresent()) {
			try {
				merchantSurchargeAndFeeEntity = merchantSurchargeAndFeeService.update(merchantSurchargeAndFeeEntity);
				return new ResponseEntity<>(merchantSurchargeAndFeeEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(merchantSurchargeAndFeeEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Merchant Surcharge and Fee that has the same id as the given id.
	 *
	 * @param id The id of the MerchantSurchargeAndFeeEntity to be deleted
	...
	@Operation(
			description = "Delete a single MerchantSurchargeAndFee as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the MerchantSurchargeAndFee to delete.")
		...
		merchantSurchargeAndFeeService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportMerchantSurchargeAndFee' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Merchant Surcharge and Fee entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportMerchantSurchargeAndFee' endpoint here] end
	public void exportMerchantSurchargeAndFee(
			@Parameter(description = "The ids of the MerchantSurchargeAndFeeEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<MerchantSurchargeAndFeeEntity> entitiesToExport = merchantSurchargeAndFeeService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "MerchantSurchargeAndFee_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (MerchantSurchargeAndFeeEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportMerchantSurchargeAndFeeExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportMerchantSurchargeAndFeeExcludingIds' endpoint here] end
	public void exportMerchantSurchargeAndFeeExcludingIds(
			@Parameter(description = "The ids of the MerchantSurchargeAndFeeEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<MerchantSurchargeAndFeeEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = merchantSurchargeAndFeeService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = merchantSurchargeAndFeeService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the MerchantSurchargeAndFee entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "MerchantSurchargeAndFee_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(MerchantSurchargeAndFeeEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Merchant Surcharge and Fee entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('MerchantSurchargeAndFeeEntity', 'create')  || hasPermission('MerchantSurchargeAndFeeEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with MerchantSurchargeAndFeeEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("MerchantSurchargeAndFeeEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(MerchantSurchargeAndFeeEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<MerchantSurchargeAndFeeEntity> csvToBean = new CsvToBeanBuilder<MerchantSurchargeAndFeeEntity>(reader)
				.withType(MerchantSurchargeAndFeeEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<MerchantSurchargeAndFeeEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<MerchantSurchargeAndFeeEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Merchant Surcharge and Fee entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Merchant Surcharge and Fee entity and you do not have permission");
					}
