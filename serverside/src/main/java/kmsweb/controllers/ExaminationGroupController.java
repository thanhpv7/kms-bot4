...
/**
 * Controller used to handle all REST operations regarding Examination Group
 */
// % protected region % [Update the API route or description for the Examination GroupController here] off begin
// The text associated with this tag can be found and edited in SwaggerConfig
@Tags(value = @Tag(name = "ExaminationGroupController"))
@RestController
@Slf4j
@RequestMapping("/api/examinationgroup")
// % protected region % [Update the API route or description for the Examination GroupController here] end
public class ExaminationGroupController {

	private final ExaminationGroupService examinationGroupService;
	private final CsvUtils<ExaminationGroupEntity, ExaminationGroupRepository, ExaminationGroupEntityAudit,  ExaminationGroupService> csvUtils;
	...
	@Autowired
	public ExaminationGroupController(
		...
		this.csvUtils = new CsvUtils<>(examinationGroupService, ExaminationGroupEntity.class);
	...
	public ResponseEntity<List<ExaminationGroupEntity>> getAllWithPage(
			@Parameter(description = "The page to return.")
			@RequestParam(value = "page", defaultValue = "1", required = false) int page,
			@Parameter(description = "The size of the page to return.")
			@RequestParam(value = "pageSize", defaultValue = "10", required = false) int pageSize
	) {
		List<ExaminationGroupEntity> examinationGroups = examinationGroupService.findAllWithPage((page > 0) ? page - 1 : page, pageSize);
		...
		return new ResponseEntity<>(examinationGroups, HttpStatus.OK);
	}

	/**
	 * Return the Examination Group that has the same id as the given id.
	 *
	 * @param id      The id of the ExaminationGroupEntity to be returned
	 * @return the Examination Group that has the same id as the given id
	 */
	...
	@Operation(
			description = "Return a single ExaminationGroup as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "404", description = "ExaminationGroup entity not found"),
	})
	@PreAuthorize("hasPermission('ExaminationGroupEntity', 'read')")
	@GetMapping(value = "/{id}", produces = "application/json")
	// % protected region % [Update the route mapping or description for the 'getById' endpoint here] end
	public ResponseEntity<ExaminationGroupEntity> getById(
			@Parameter(description = "The UUID of the ExaminationGroup to return.")
			@PathVariable("id") UUID id
	) {
		Optional<ExaminationGroupEntity> examinationGroupEntity = examinationGroupService.findById(id);
	...
	/**
	 * Create/Update the given Examination Group. If the entity has an id it will be updated, if not it will be created.
	 * The appropriate status codes will be set.
	 *
	 * @param examinationGroupEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'saveExaminationGroup' endpoint here] off begin
	@Operation(
			description = "Create a ExaminationGroup if id not exists. Return 409 otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully created the new ExaminationGroup."),
			@ApiResponse(responseCode = "409", description = "Failed to create the new ExaminationGroup, duplicate record found")
	})
	@PreAuthorize("hasPermission('ExaminationGroupEntity', 'create')")
	@PostMapping
	// % protected region % [Update the route mapping or description for the 'saveExaminationGroup' endpoint here] end
	public ResponseEntity<?> saveExaminationGroup(
			@Parameter(description = "The ExaminationGroupEntity to create or update.")
			@RequestBody ExaminationGroupEntityDto examinationGroupEntityDto
	) {
		// % protected region % [Add any logic before save here] off begin
		// % protected region % [Add any logic before save here] end

		try {
			ExaminationGroupEntity examinationGroupEntity = examinationGroupService.create(new ExaminationGroupEntity(examinationGroupEntityDto));
			return new ResponseEntity<>(examinationGroupEntity, HttpStatus.OK);
	...
	/**
	 * Update the given Examination Group. If the entity has an id which is associated with an existing entity it will be updated, if not then the request will fail
	 * The appropriate status codes will be set.
	 *
	 * @param examinationGroupEntityDto The Dto of the entity to save or update
	 */
	// % protected region % [Update the route mapping or description for the 'updateExaminationGroup' endpoint here] off begin
	@Operation(
			description = "Update a ExaminationGroup entity if id exists and is associated with an existing entity.  Return BAD_REQUEST otherwise",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	@ApiResponses(value = {
		@ApiResponse(responseCode = "200", description = "Successfully updated the ExaminationGroup."),
		@ApiResponse(responseCode = "400", description = "Failed to update the ExaminationGroup, entity id either does not exist" +
				"or is not associated with an existing entity")
	})
	@PreAuthorize("hasPermission('ExaminationGroupEntity', 'update')")
	@PutMapping
	// % protected region % [Update the route mapping or description for the 'updateExaminationGroup' endpoint here] end
	public ResponseEntity<?> updateExaminationGroup(
			@Parameter(description = "The ExaminationGroupEntity to create or update.")
			@RequestBody ExaminationGroupEntityDto examinationGroupEntityDto
	) {
		// % protected region % [Add any logic before update here] off begin
		// % protected region % [Add any logic before update here] end
		Optional<ExaminationGroupEntity> checkExaminationGroupExists = examinationGroupService.findById(examinationGroupEntityDto.getId());
		ExaminationGroupEntity examinationGroupEntity = new ExaminationGroupEntity(examinationGroupEntityDto);

		if (checkExaminationGroupExists.isPresent()) {
			try {
				examinationGroupEntity = examinationGroupService.update(examinationGroupEntity);
				return new ResponseEntity<>(examinationGroupEntity, HttpStatus.OK);
			} catch (ConstraintViolationException err) {
				return ControllerUtils.failedValidationApiResponse(err);
			}
		...
		return new ResponseEntity<>(examinationGroupEntity, HttpStatus.BAD_REQUEST);
	}

	/**
	 * Delete the Examination Group that has the same id as the given id.
	 *
	 * @param id The id of the ExaminationGroupEntity to be deleted
	...
	@Operation(
			description = "Delete a single ExaminationGroup as defined by the id provided.",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	public ResponseEntity<Void> deleteById(
			@Parameter(description = "The UUID of the ExaminationGroup to delete.")
		...
		examinationGroupService.deleteById(id);
	 ...
	 */
	// % protected region % [Update the route mapping or description for the 'exportExaminationGroup' endpoint here] off begin
	@Operation(
			description = "Download a csv file including all Examination Group entities included in the ids provided",
			security = {
					@SecurityRequirement(name = "Authentication-Token"),
					@SecurityRequirement(name = "CSRF-Token"),
					@SecurityRequirement(name = "CSRF-Cookie")
	...
	// % protected region % [Update the route mapping or description for the 'exportExaminationGroup' endpoint here] end
	public void exportExaminationGroup(
			@Parameter(description = "The ids of the ExaminationGroupEntities to export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		if (ids == null || ids.size() == 0) {
			response.setStatus(HttpServletResponse.SC_BAD_REQUEST);
		} else {
			List<ExaminationGroupEntity> entitiesToExport = examinationGroupService.findAllByIds(ids);
			// % protected region % [Modify the default file name for the Export endpoint here] off begin
			String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
			String fileName = "ExaminationGroup_Export_" + datetime + ".csv";
			// % protected region % [Modify the default file name for the Export endpoint here] end

		for (ExaminationGroupEntity entity: entitiesToExport) {
	 ...
	 * @param response The response object
	 */
	// % protected region % [Update the route mapping or description for the 'exportExaminationGroupExcludingIds' endpoint here] off begin
	@Operation(
	...
	// % protected region % [Update the route mapping or description for the 'exportExaminationGroupExcludingIds' endpoint here] end
	public void exportExaminationGroupExcludingIds(
			@Parameter(description = "The ids of the ExaminationGroupEntities to exclude from the export")
			@RequestParam @NotNull List<UUID> ids, HttpServletResponse response
	) {
		List<ExaminationGroupEntity> entitiesToExport;
		...
		if (ids == null || ids.size() == 0) {
			entitiesToExport = examinationGroupService.findAllExcludingIds(new ArrayList<>());
		} else {
			entitiesToExport = examinationGroupService.findAllExcludingIds(ids);
		}
	...
	/**
	 * Download a CSV file containing all of the headers of the ExaminationGroup entity.
	...
	public void csvImportExample(
		...
		String datetime = new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss").format(new Date());
		String fileName = "ExaminationGroup_Import_Example_" + datetime + ".csv";
		// % protected region % [Modify the default file name for the Example Import endpoint here] end
		try {
			InputStream is = new ByteArrayInputStream(ExaminationGroupEntity.getExampleCsvHeader().getBytes());
	...
	/**
	 * Upload a CSV file containing attributes for Examination Group entities, and import them into the database.  This method
	...
	@PreAuthorize("hasPermission('ExaminationGroupEntity', 'create')  || hasPermission('ExaminationGroupEntity', 'update')")
	...
	public ResponseEntity<String> csvUpload(HttpServletRequest request,
			@Parameter(description = "The CSV file with ExaminationGroupEntity entities to import")
		...
		CustomGrantedAuthority entityAuthority = authorities.stream().filter(x -> x.getTargetEntity().matches("ExaminationGroupEntity"))
		...
		List<String> allowedHeaders = Arrays.asList(ExaminationGroupEntity.getExampleCsvHeader().split(","));
		...
		CsvToBean<ExaminationGroupEntity> csvToBean = new CsvToBeanBuilder<ExaminationGroupEntity>(reader)
				.withType(ExaminationGroupEntity.class)
				.withIgnoreLeadingWhiteSpace(true)
				.withThrowExceptions(true)
				.build();

		List<ExaminationGroupEntity> entities = new ArrayList<>();
		...
		try {
			Iterator<ExaminationGroupEntity> iterator = csvToBean.iterator();
				...
				try {
					...
					if (next.getId() != null && !allowUpdate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to update a Examination Group entity and you do not have permission");
					}

					if (next.getId() == null && !allowCreate) {
						return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Line " + currentLine
								+ ": You attempted to create a Examination Group entity and you do not have permission");
					}
