...
import kmsweb.entities.listeners.RegistrationUserEntityListener;
import kmsweb.serializers.RegistrationUserSerializer;
...
@EntityListeners({RegistrationUserEntityListener.class, AuditingEntityListener.class})
@JsonSerialize(using = RegistrationUserSerializer.class)
@Table(
	uniqueConstraints = {
	}
)
public class RegistrationUserEntity extends UserEntity {

	/**
	 * Takes a RegistrationUserEntityDto and converts it into a RegistrationUserEntity.
	 * Primarily used when endpoints are invoked, as they take DTOs as input, which need to be converted to the entity type
	 *
	 * @param registrationUserEntityDto
	 */
	public RegistrationUserEntity(RegistrationUserEntityDto registrationUserEntityDto) {
		// Need to account for potential that any field is empty.  This will be run before any server validation and
		// endpoints will most likely not be subject to clientside validation

		if (registrationUserEntityDto.getId() != null) {
			this.setId(registrationUserEntityDto.getId());
		}

		if (registrationUserEntityDto.getStaff() != null) {
			this.setStaff(registrationUserEntityDto.getStaff());
		}
	...
	@OneToOne(mappedBy = "registrationUser", cascade = {CascadeType.MERGE}, fetch = FetchType.LAZY)
	@CsvIgnore
	 ...
	 *
	 * @param entity the RegistrationUserEntity to be set or updated
	...
	public void setStaff(@NotNull StaffEntity entity, boolean reverseAdd) {
		...
		if (this.staff != null) {
			this.staff.unsetRegistrationUser();
		}

		this.staff = entity;

		if (reverseAdd) {
			this.staff.setRegistrationUser(this, false);
		}
	...
	public void unsetStaff(boolean reverse) {
		if (reverse && this.staff != null) {
			this.staff.unsetRegistrationUser(false);
		}
		this.staff = null;
	}
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	@Override
	public boolean equals(Object registrationUser) {
		if (this == registrationUser) {
			return true;
		}
		if (registrationUser == null || this.getClass() != registrationUser.getClass()) {
			return false;
		}
		if (!super.equals(registrationUser)) {
			return false;
		}

		// Compare references by ids only
		this.addRelationEntitiesToIdSet();
		RegistrationUserEntity that = (RegistrationUserEntity) registrationUser;
		return
