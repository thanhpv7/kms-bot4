...
import kmsweb.entities.listeners.SystemAdminUserEntityListener;
import kmsweb.serializers.SystemAdminUserSerializer;
...
@EntityListeners({SystemAdminUserEntityListener.class, AuditingEntityListener.class})
@JsonSerialize(using = SystemAdminUserSerializer.class)
@Table(
	uniqueConstraints = {
	}
)
public class SystemAdminUserEntity extends UserEntity {

	/**
	 * Takes a SystemAdminUserEntityDto and converts it into a SystemAdminUserEntity.
	 * Primarily used when endpoints are invoked, as they take DTOs as input, which need to be converted to the entity type
	 *
	 * @param systemAdminUserEntityDto
	 */
	public SystemAdminUserEntity(SystemAdminUserEntityDto systemAdminUserEntityDto) {
		// Need to account for potential that any field is empty.  This will be run before any server validation and
		// endpoints will most likely not be subject to clientside validation

		if (systemAdminUserEntityDto.getId() != null) {
			this.setId(systemAdminUserEntityDto.getId());
		}

		if (systemAdminUserEntityDto.getStaff() != null) {
			this.setStaff(systemAdminUserEntityDto.getStaff());
		}
	...
	@OneToOne(mappedBy = "systemAdminUser", cascade = {CascadeType.MERGE}, fetch = FetchType.LAZY)
	@CsvIgnore
	 ...
	 *
	 * @param entity the SystemAdminUserEntity to be set or updated
	...
	public void setStaff(@NotNull StaffEntity entity, boolean reverseAdd) {
		...
		if (this.staff != null) {
			this.staff.unsetSystemAdminUser();
		}

		this.staff = entity;

		if (reverseAdd) {
			this.staff.setSystemAdminUser(this, false);
		}
	...
	public void unsetStaff(boolean reverse) {
		if (reverse && this.staff != null) {
			this.staff.unsetSystemAdminUser(false);
		}
		this.staff = null;
	}
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	@Override
	public boolean equals(Object systemAdminUser) {
		if (this == systemAdminUser) {
			return true;
		}
		if (systemAdminUser == null || this.getClass() != systemAdminUser.getClass()) {
			return false;
		}
		if (!super.equals(systemAdminUser)) {
			return false;
		}

		// Compare references by ids only
		this.addRelationEntitiesToIdSet();
		SystemAdminUserEntity that = (SystemAdminUserEntity) systemAdminUser;
		return
