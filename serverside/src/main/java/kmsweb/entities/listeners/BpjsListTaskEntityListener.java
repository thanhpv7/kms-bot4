...
import kmsweb.configs.security.auditing.services.BpjsListTaskEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsListTaskEntity.
 */
@Slf4j
@Component
public class BpjsListTaskEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsListTaskEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsListTaskEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsListTaskEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsListTaskEntityListener.auditService = auditService;
		BpjsListTaskEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsListTaskEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsListTaskEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsListTaskEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsListTaskEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsListTaskEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsListTaskEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsListTaskEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsListTaskEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsListTaskEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS List Task is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsListTaskEntity entity) {
	}
