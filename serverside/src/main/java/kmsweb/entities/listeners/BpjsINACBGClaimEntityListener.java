...
import kmsweb.configs.security.auditing.services.BpjsINACBGClaimEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsINACBGClaimEntity.
 */
@Slf4j
@Component
public class BpjsINACBGClaimEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsINACBGClaimEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsINACBGClaimEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsINACBGClaimEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsINACBGClaimEntityListener.auditService = auditService;
		BpjsINACBGClaimEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsINACBGClaimEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsINACBGClaimEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsINACBGClaimEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsINACBGClaimEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsINACBGClaimEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsINACBGClaimEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsINACBGClaimEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsINACBGClaimEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsINACBGClaimEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS INACBG Claim is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsINACBGClaimEntity entity) {
	}
