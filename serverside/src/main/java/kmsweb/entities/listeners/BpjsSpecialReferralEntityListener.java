...
import kmsweb.configs.security.auditing.services.BpjsSpecialReferralEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsSpecialReferralEntity.
 */
@Slf4j
@Component
public class BpjsSpecialReferralEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsSpecialReferralEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsSpecialReferralEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsSpecialReferralEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsSpecialReferralEntityListener.auditService = auditService;
		BpjsSpecialReferralEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsSpecialReferralEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsSpecialReferralEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsSpecialReferralEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsSpecialReferralEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsSpecialReferralEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsSpecialReferralEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsSpecialReferralEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsSpecialReferralEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsSpecialReferralEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Special Referral is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsSpecialReferralEntity entity) {
		entity.unsetAdditionalDiagnose();
		entity.unsetAdditionalProcedure();
		entity.unsetPrimaryDiagnose();
		entity.unsetPrimaryProcedure();
	}
