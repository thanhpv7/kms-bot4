...
import kmsweb.configs.security.auditing.services.CashReceiptEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing CashReceiptEntity.
 */
@Slf4j
@Component
public class CashReceiptEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static CashReceiptEntityReadAuditService auditService;
	...
	public void init(
		...
		CashReceiptEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		CashReceiptEntityListener.permissionEvaluator = permissionEvaluator;
		CashReceiptEntityListener.auditService = auditService;
		CashReceiptEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(CashReceiptEntity entity) {
	...
	@PostPersist
	public void afterPersist(CashReceiptEntity entity) {
	...
	@PreRemove
	public void beforeRemove(CashReceiptEntity entity) {
	...
	@PostRemove
	public void afterRemove(CashReceiptEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(CashReceiptEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(CashReceiptEntity entity) {
	...
	@PostLoad
	public void afterLoad(CashReceiptEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the CashReceiptEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the CashReceiptEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Cash Receipt is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(CashReceiptEntity entity) {
