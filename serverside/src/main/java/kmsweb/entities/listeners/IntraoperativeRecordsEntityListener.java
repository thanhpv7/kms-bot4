...
import kmsweb.configs.security.auditing.services.IntraoperativeRecordsEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing IntraoperativeRecordsEntity.
 */
@Slf4j
@Component
public class IntraoperativeRecordsEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static IntraoperativeRecordsEntityReadAuditService auditService;
	...
	public void init(
		...
		IntraoperativeRecordsEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		IntraoperativeRecordsEntityListener.permissionEvaluator = permissionEvaluator;
		IntraoperativeRecordsEntityListener.auditService = auditService;
		IntraoperativeRecordsEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(IntraoperativeRecordsEntity entity) {
	...
	@PostPersist
	public void afterPersist(IntraoperativeRecordsEntity entity) {
	...
	@PreRemove
	public void beforeRemove(IntraoperativeRecordsEntity entity) {
	...
	@PostRemove
	public void afterRemove(IntraoperativeRecordsEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(IntraoperativeRecordsEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(IntraoperativeRecordsEntity entity) {
	...
	@PostLoad
	public void afterLoad(IntraoperativeRecordsEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the IntraoperativeRecordsEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the IntraoperativeRecordsEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Intraoperative Records is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(IntraoperativeRecordsEntity entity) {
		entity.unsetOperatingTheaterMedicalExaminationRecord();
		entity.unsetOperatingRoom();
