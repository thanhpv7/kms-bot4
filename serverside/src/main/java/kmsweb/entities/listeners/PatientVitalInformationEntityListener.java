...
import kmsweb.configs.security.auditing.services.PatientVitalInformationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientVitalInformationEntity.
 */
@Slf4j
@Component
public class PatientVitalInformationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientVitalInformationEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientVitalInformationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientVitalInformationEntityListener.permissionEvaluator = permissionEvaluator;
		PatientVitalInformationEntityListener.auditService = auditService;
		PatientVitalInformationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientVitalInformationEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientVitalInformationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientVitalInformationEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientVitalInformationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientVitalInformationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientVitalInformationEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientVitalInformationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientVitalInformationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientVitalInformationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Vital Information is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientVitalInformationEntity entity) {
