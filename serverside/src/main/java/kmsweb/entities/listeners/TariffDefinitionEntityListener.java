...
import kmsweb.configs.security.auditing.services.TariffDefinitionEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TariffDefinitionEntity.
 */
@Slf4j
@Component
public class TariffDefinitionEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TariffDefinitionEntityReadAuditService auditService;
	...
	public void init(
		...
		TariffDefinitionEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TariffDefinitionEntityListener.permissionEvaluator = permissionEvaluator;
		TariffDefinitionEntityListener.auditService = auditService;
		TariffDefinitionEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TariffDefinitionEntity entity) {
	...
	@PostPersist
	public void afterPersist(TariffDefinitionEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TariffDefinitionEntity entity) {
	...
	@PostRemove
	public void afterRemove(TariffDefinitionEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TariffDefinitionEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TariffDefinitionEntity entity) {
	...
	@PostLoad
	public void afterLoad(TariffDefinitionEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TariffDefinitionEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TariffDefinitionEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Tariff Definition is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TariffDefinitionEntity entity) {
		entity.unsetPriceSchema();
