...
import kmsweb.configs.security.auditing.services.BedFacilityEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BedFacilityEntity.
 */
@Slf4j
@Component
public class BedFacilityEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BedFacilityEntityReadAuditService auditService;
	...
	public void init(
		...
		BedFacilityEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BedFacilityEntityListener.permissionEvaluator = permissionEvaluator;
		BedFacilityEntityListener.auditService = auditService;
		BedFacilityEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BedFacilityEntity entity) {
	...
	@PostPersist
	public void afterPersist(BedFacilityEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BedFacilityEntity entity) {
	...
	@PostRemove
	public void afterRemove(BedFacilityEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BedFacilityEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BedFacilityEntity entity) {
	...
	@PostLoad
	public void afterLoad(BedFacilityEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BedFacilityEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BedFacilityEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Bed Facility is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BedFacilityEntity entity) {
		entity.unsetIntraoperativeRecords();
		entity.unsetPostOperativeDetails();
		entity.unsetPreoperativeRecords();
		entity.unsetRoomFacility();
		entity.unsetBedReserves();
		entity.unsetInpatientMedicalExaminationRecords();
