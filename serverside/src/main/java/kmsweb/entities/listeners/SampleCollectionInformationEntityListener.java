...
import kmsweb.configs.security.auditing.services.SampleCollectionInformationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SampleCollectionInformationEntity.
 */
@Slf4j
@Component
public class SampleCollectionInformationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SampleCollectionInformationEntityReadAuditService auditService;
	...
	public void init(
		...
		SampleCollectionInformationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SampleCollectionInformationEntityListener.permissionEvaluator = permissionEvaluator;
		SampleCollectionInformationEntityListener.auditService = auditService;
		SampleCollectionInformationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SampleCollectionInformationEntity entity) {
	...
	@PostPersist
	public void afterPersist(SampleCollectionInformationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SampleCollectionInformationEntity entity) {
	...
	@PostRemove
	public void afterRemove(SampleCollectionInformationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SampleCollectionInformationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SampleCollectionInformationEntity entity) {
	...
	@PostLoad
	public void afterLoad(SampleCollectionInformationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SampleCollectionInformationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SampleCollectionInformationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Sample Collection Information is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SampleCollectionInformationEntity entity) {
		entity.unsetDiagnosisExaminationRecord();
		entity.unsetRegistration();
		entity.unsetSampleCollectionItems();
	}
