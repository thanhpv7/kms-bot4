...
import kmsweb.configs.security.auditing.services.PurchaseOrderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PurchaseOrderEntity.
 */
@Slf4j
@Component
public class PurchaseOrderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PurchaseOrderEntityReadAuditService auditService;
	...
	public void init(
		...
		PurchaseOrderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PurchaseOrderEntityListener.permissionEvaluator = permissionEvaluator;
		PurchaseOrderEntityListener.auditService = auditService;
		PurchaseOrderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PurchaseOrderEntity entity) {
	...
	@PostPersist
	public void afterPersist(PurchaseOrderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PurchaseOrderEntity entity) {
	...
	@PostRemove
	public void afterRemove(PurchaseOrderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PurchaseOrderEntity entity) {
	...
	@PostLoad
	public void afterLoad(PurchaseOrderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PurchaseOrderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PurchaseOrderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Purchase Order is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PurchaseOrderEntity entity) {
		entity.unsetGoodsReceiveNote();
