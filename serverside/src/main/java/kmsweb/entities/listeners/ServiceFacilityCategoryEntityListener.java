...
import kmsweb.configs.security.auditing.services.ServiceFacilityCategoryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ServiceFacilityCategoryEntity.
 */
@Slf4j
@Component
public class ServiceFacilityCategoryEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ServiceFacilityCategoryEntityReadAuditService auditService;
	...
	public void init(
		...
		ServiceFacilityCategoryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ServiceFacilityCategoryEntityListener.permissionEvaluator = permissionEvaluator;
		ServiceFacilityCategoryEntityListener.auditService = auditService;
		ServiceFacilityCategoryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ServiceFacilityCategoryEntity entity) {
	...
	@PostPersist
	public void afterPersist(ServiceFacilityCategoryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ServiceFacilityCategoryEntity entity) {
	...
	@PostRemove
	public void afterRemove(ServiceFacilityCategoryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ServiceFacilityCategoryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ServiceFacilityCategoryEntity entity) {
	...
	@PostLoad
	public void afterLoad(ServiceFacilityCategoryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ServiceFacilityCategoryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ServiceFacilityCategoryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Service Facility Category is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ServiceFacilityCategoryEntity entity) {
		entity.unsetService();
	}
