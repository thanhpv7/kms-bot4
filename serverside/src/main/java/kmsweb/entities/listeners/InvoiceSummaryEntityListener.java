...
import kmsweb.configs.security.auditing.services.InvoiceSummaryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InvoiceSummaryEntity.
 */
@Slf4j
@Component
public class InvoiceSummaryEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InvoiceSummaryEntityReadAuditService auditService;
	...
	public void init(
		...
		InvoiceSummaryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InvoiceSummaryEntityListener.permissionEvaluator = permissionEvaluator;
		InvoiceSummaryEntityListener.auditService = auditService;
		InvoiceSummaryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InvoiceSummaryEntity entity) {
	...
	@PostPersist
	public void afterPersist(InvoiceSummaryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InvoiceSummaryEntity entity) {
	...
	@PostRemove
	public void afterRemove(InvoiceSummaryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InvoiceSummaryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InvoiceSummaryEntity entity) {
	...
	@PostLoad
	public void afterLoad(InvoiceSummaryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InvoiceSummaryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InvoiceSummaryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Invoice Summary is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InvoiceSummaryEntity entity) {
		entity.unsetInvoice();
