...
import kmsweb.configs.security.auditing.services.HemodialysisMonitoringEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing HemodialysisMonitoringEntity.
 */
@Slf4j
@Component
public class HemodialysisMonitoringEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static HemodialysisMonitoringEntityReadAuditService auditService;
	...
	public void init(
		...
		HemodialysisMonitoringEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		HemodialysisMonitoringEntityListener.permissionEvaluator = permissionEvaluator;
		HemodialysisMonitoringEntityListener.auditService = auditService;
		HemodialysisMonitoringEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(HemodialysisMonitoringEntity entity) {
	...
	@PostPersist
	public void afterPersist(HemodialysisMonitoringEntity entity) {
	...
	@PreRemove
	public void beforeRemove(HemodialysisMonitoringEntity entity) {
	...
	@PostRemove
	public void afterRemove(HemodialysisMonitoringEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(HemodialysisMonitoringEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(HemodialysisMonitoringEntity entity) {
	...
	@PostLoad
	public void afterLoad(HemodialysisMonitoringEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the HemodialysisMonitoringEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the HemodialysisMonitoringEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Hemodialysis Monitoring is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(HemodialysisMonitoringEntity entity) {
