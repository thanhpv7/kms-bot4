...
import kmsweb.configs.security.auditing.services.ReturnSupplierStockDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ReturnSupplierStockDetailEntity.
 */
@Slf4j
@Component
public class ReturnSupplierStockDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ReturnSupplierStockDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		ReturnSupplierStockDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ReturnSupplierStockDetailEntityListener.permissionEvaluator = permissionEvaluator;
		ReturnSupplierStockDetailEntityListener.auditService = auditService;
		ReturnSupplierStockDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ReturnSupplierStockDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(ReturnSupplierStockDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ReturnSupplierStockDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(ReturnSupplierStockDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ReturnSupplierStockDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ReturnSupplierStockDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(ReturnSupplierStockDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ReturnSupplierStockDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ReturnSupplierStockDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Return Supplier Stock Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ReturnSupplierStockDetailEntity entity) {
		entity.unsetReturnSupplier();
