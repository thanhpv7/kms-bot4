...
import kmsweb.configs.security.auditing.services.InventoryAdjusmentBatchEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InventoryAdjusmentBatchEntity.
 */
@Slf4j
@Component
public class InventoryAdjusmentBatchEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InventoryAdjusmentBatchEntityReadAuditService auditService;
	...
	public void init(
		...
		InventoryAdjusmentBatchEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InventoryAdjusmentBatchEntityListener.permissionEvaluator = permissionEvaluator;
		InventoryAdjusmentBatchEntityListener.auditService = auditService;
		InventoryAdjusmentBatchEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InventoryAdjusmentBatchEntity entity) {
	...
	@PostPersist
	public void afterPersist(InventoryAdjusmentBatchEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InventoryAdjusmentBatchEntity entity) {
	...
	@PostRemove
	public void afterRemove(InventoryAdjusmentBatchEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InventoryAdjusmentBatchEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InventoryAdjusmentBatchEntity entity) {
	...
	@PostLoad
	public void afterLoad(InventoryAdjusmentBatchEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InventoryAdjusmentBatchEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InventoryAdjusmentBatchEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Inventory Adjusment Batch is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InventoryAdjusmentBatchEntity entity) {
		entity.unsetInventoryAdjustmentItem();
