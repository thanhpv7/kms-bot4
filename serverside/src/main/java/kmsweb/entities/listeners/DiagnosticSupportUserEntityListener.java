...
import kmsweb.configs.security.auditing.services.DiagnosticSupportUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DiagnosticSupportUserEntity.
 */
@Slf4j
@Component
public class DiagnosticSupportUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DiagnosticSupportUserEntityReadAuditService auditService;
	...
	public void init(
		...
		DiagnosticSupportUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DiagnosticSupportUserEntityListener.permissionEvaluator = permissionEvaluator;
		DiagnosticSupportUserEntityListener.auditService = auditService;
		DiagnosticSupportUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DiagnosticSupportUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(DiagnosticSupportUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DiagnosticSupportUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(DiagnosticSupportUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DiagnosticSupportUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DiagnosticSupportUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(DiagnosticSupportUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DiagnosticSupportUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DiagnosticSupportUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Diagnostic Support User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DiagnosticSupportUserEntity entity) {
		entity.unsetStaff();
	}
