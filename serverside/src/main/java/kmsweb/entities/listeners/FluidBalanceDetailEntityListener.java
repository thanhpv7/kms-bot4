...
import kmsweb.configs.security.auditing.services.FluidBalanceDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing FluidBalanceDetailEntity.
 */
@Slf4j
@Component
public class FluidBalanceDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static FluidBalanceDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		FluidBalanceDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		FluidBalanceDetailEntityListener.permissionEvaluator = permissionEvaluator;
		FluidBalanceDetailEntityListener.auditService = auditService;
		FluidBalanceDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(FluidBalanceDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(FluidBalanceDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(FluidBalanceDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(FluidBalanceDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(FluidBalanceDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(FluidBalanceDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(FluidBalanceDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the FluidBalanceDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the FluidBalanceDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Fluid Balance Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(FluidBalanceDetailEntity entity) {
