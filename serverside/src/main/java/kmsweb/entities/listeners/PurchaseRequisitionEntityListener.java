...
import kmsweb.configs.security.auditing.services.PurchaseRequisitionEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PurchaseRequisitionEntity.
 */
@Slf4j
@Component
public class PurchaseRequisitionEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PurchaseRequisitionEntityReadAuditService auditService;
	...
	public void init(
		...
		PurchaseRequisitionEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PurchaseRequisitionEntityListener.permissionEvaluator = permissionEvaluator;
		PurchaseRequisitionEntityListener.auditService = auditService;
		PurchaseRequisitionEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PurchaseRequisitionEntity entity) {
	...
	@PostPersist
	public void afterPersist(PurchaseRequisitionEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PurchaseRequisitionEntity entity) {
	...
	@PostRemove
	public void afterRemove(PurchaseRequisitionEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PurchaseRequisitionEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PurchaseRequisitionEntity entity) {
	...
	@PostLoad
	public void afterLoad(PurchaseRequisitionEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PurchaseRequisitionEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PurchaseRequisitionEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Purchase Requisition is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PurchaseRequisitionEntity entity) {
		entity.unsetPurchaseOrder();
		entity.unsetSupplier();
