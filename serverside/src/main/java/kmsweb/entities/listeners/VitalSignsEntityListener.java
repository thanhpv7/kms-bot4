...
import kmsweb.configs.security.auditing.services.VitalSignsEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing VitalSignsEntity.
 */
@Slf4j
@Component
public class VitalSignsEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static VitalSignsEntityReadAuditService auditService;
	...
	public void init(
		...
		VitalSignsEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		VitalSignsEntityListener.permissionEvaluator = permissionEvaluator;
		VitalSignsEntityListener.auditService = auditService;
		VitalSignsEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(VitalSignsEntity entity) {
	...
	@PostPersist
	public void afterPersist(VitalSignsEntity entity) {
	...
	@PreRemove
	public void beforeRemove(VitalSignsEntity entity) {
	...
	@PostRemove
	public void afterRemove(VitalSignsEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(VitalSignsEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(VitalSignsEntity entity) {
	...
	@PostLoad
	public void afterLoad(VitalSignsEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the VitalSignsEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the VitalSignsEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Vital Signs is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(VitalSignsEntity entity) {
		entity.unsetNewbornDetail();
	}
