...
import kmsweb.configs.security.auditing.services.PatientVisitPaymentSelfPayingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientVisitPaymentSelfPayingEntity.
 */
@Slf4j
@Component
public class PatientVisitPaymentSelfPayingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientVisitPaymentSelfPayingEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientVisitPaymentSelfPayingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientVisitPaymentSelfPayingEntityListener.permissionEvaluator = permissionEvaluator;
		PatientVisitPaymentSelfPayingEntityListener.auditService = auditService;
		PatientVisitPaymentSelfPayingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientVisitPaymentSelfPayingEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientVisitPaymentSelfPayingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientVisitPaymentSelfPayingEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientVisitPaymentSelfPayingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientVisitPaymentSelfPayingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientVisitPaymentSelfPayingEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientVisitPaymentSelfPayingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientVisitPaymentSelfPayingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientVisitPaymentSelfPayingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Visit Payment Self Paying is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientVisitPaymentSelfPayingEntity entity) {
