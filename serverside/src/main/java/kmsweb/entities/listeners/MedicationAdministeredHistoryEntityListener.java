...
import kmsweb.configs.security.auditing.services.MedicationAdministeredHistoryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicationAdministeredHistoryEntity.
 */
@Slf4j
@Component
public class MedicationAdministeredHistoryEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicationAdministeredHistoryEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicationAdministeredHistoryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicationAdministeredHistoryEntityListener.permissionEvaluator = permissionEvaluator;
		MedicationAdministeredHistoryEntityListener.auditService = auditService;
		MedicationAdministeredHistoryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicationAdministeredHistoryEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicationAdministeredHistoryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicationAdministeredHistoryEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicationAdministeredHistoryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicationAdministeredHistoryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicationAdministeredHistoryEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicationAdministeredHistoryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicationAdministeredHistoryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicationAdministeredHistoryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medication Administered History is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicationAdministeredHistoryEntity entity) {
		entity.unsetMedicationItem();
		entity.unsetStaff();
		entity.unsetMedicationCompound();
	}
