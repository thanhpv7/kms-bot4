...
import kmsweb.configs.security.auditing.services.PcareUmumEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareUmumEntity.
 */
@Slf4j
@Component
public class PcareUmumEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareUmumEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareUmumEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareUmumEntityListener.permissionEvaluator = permissionEvaluator;
		PcareUmumEntityListener.auditService = auditService;
		PcareUmumEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareUmumEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareUmumEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareUmumEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareUmumEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareUmumEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareUmumEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareUmumEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareUmumEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareUmumEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Umum is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareUmumEntity entity) {
	}
