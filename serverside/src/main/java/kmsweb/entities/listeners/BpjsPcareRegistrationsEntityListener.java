...
import kmsweb.configs.security.auditing.services.BpjsPcareRegistrationsEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPcareRegistrationsEntity.
 */
@Slf4j
@Component
public class BpjsPcareRegistrationsEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPcareRegistrationsEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPcareRegistrationsEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPcareRegistrationsEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPcareRegistrationsEntityListener.auditService = auditService;
		BpjsPcareRegistrationsEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPcareRegistrationsEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPcareRegistrationsEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPcareRegistrationsEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPcareRegistrationsEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPcareRegistrationsEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPcareRegistrationsEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPcareRegistrationsEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPcareRegistrationsEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPcareRegistrationsEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Pcare Registrations is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPcareRegistrationsEntity entity) {
