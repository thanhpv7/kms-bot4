...
import kmsweb.configs.security.auditing.services.BpjsListFingerPrintEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsListFingerPrintEntity.
 */
@Slf4j
@Component
public class BpjsListFingerPrintEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsListFingerPrintEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsListFingerPrintEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsListFingerPrintEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsListFingerPrintEntityListener.auditService = auditService;
		BpjsListFingerPrintEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsListFingerPrintEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsListFingerPrintEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsListFingerPrintEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsListFingerPrintEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsListFingerPrintEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsListFingerPrintEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsListFingerPrintEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsListFingerPrintEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsListFingerPrintEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS List Finger Print is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsListFingerPrintEntity entity) {
	}
