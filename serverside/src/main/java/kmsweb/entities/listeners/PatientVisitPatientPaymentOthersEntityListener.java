...
import kmsweb.configs.security.auditing.services.PatientVisitPatientPaymentOthersEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientVisitPatientPaymentOthersEntity.
 */
@Slf4j
@Component
public class PatientVisitPatientPaymentOthersEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientVisitPatientPaymentOthersEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientVisitPatientPaymentOthersEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientVisitPatientPaymentOthersEntityListener.permissionEvaluator = permissionEvaluator;
		PatientVisitPatientPaymentOthersEntityListener.auditService = auditService;
		PatientVisitPatientPaymentOthersEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientVisitPatientPaymentOthersEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientVisitPatientPaymentOthersEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientVisitPatientPaymentOthersEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientVisitPatientPaymentOthersEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientVisitPatientPaymentOthersEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientVisitPatientPaymentOthersEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientVisitPatientPaymentOthersEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientVisitPatientPaymentOthersEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientVisitPatientPaymentOthersEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Visit Patient Payment Others is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientVisitPatientPaymentOthersEntity entity) {
