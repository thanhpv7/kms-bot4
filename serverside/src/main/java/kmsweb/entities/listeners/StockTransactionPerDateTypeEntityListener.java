...
import kmsweb.configs.security.auditing.services.StockTransactionPerDateTypeEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing StockTransactionPerDateTypeEntity.
 */
@Slf4j
@Component
public class StockTransactionPerDateTypeEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static StockTransactionPerDateTypeEntityReadAuditService auditService;
	...
	public void init(
		...
		StockTransactionPerDateTypeEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		StockTransactionPerDateTypeEntityListener.permissionEvaluator = permissionEvaluator;
		StockTransactionPerDateTypeEntityListener.auditService = auditService;
		StockTransactionPerDateTypeEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(StockTransactionPerDateTypeEntity entity) {
	...
	@PostPersist
	public void afterPersist(StockTransactionPerDateTypeEntity entity) {
	...
	@PreRemove
	public void beforeRemove(StockTransactionPerDateTypeEntity entity) {
	...
	@PostRemove
	public void afterRemove(StockTransactionPerDateTypeEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(StockTransactionPerDateTypeEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(StockTransactionPerDateTypeEntity entity) {
	...
	@PostLoad
	public void afterLoad(StockTransactionPerDateTypeEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the StockTransactionPerDateTypeEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the StockTransactionPerDateTypeEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Stock Transaction Per Date Type is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(StockTransactionPerDateTypeEntity entity) {
