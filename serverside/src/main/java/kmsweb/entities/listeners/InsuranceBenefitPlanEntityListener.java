...
import kmsweb.configs.security.auditing.services.InsuranceBenefitPlanEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InsuranceBenefitPlanEntity.
 */
@Slf4j
@Component
public class InsuranceBenefitPlanEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InsuranceBenefitPlanEntityReadAuditService auditService;
	...
	public void init(
		...
		InsuranceBenefitPlanEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InsuranceBenefitPlanEntityListener.permissionEvaluator = permissionEvaluator;
		InsuranceBenefitPlanEntityListener.auditService = auditService;
		InsuranceBenefitPlanEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InsuranceBenefitPlanEntity entity) {
	...
	@PostPersist
	public void afterPersist(InsuranceBenefitPlanEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InsuranceBenefitPlanEntity entity) {
	...
	@PostRemove
	public void afterRemove(InsuranceBenefitPlanEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InsuranceBenefitPlanEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InsuranceBenefitPlanEntity entity) {
	...
	@PostLoad
	public void afterLoad(InsuranceBenefitPlanEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InsuranceBenefitPlanEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InsuranceBenefitPlanEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Insurance Benefit Plan is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InsuranceBenefitPlanEntity entity) {
		entity.unsetInsuranceProvider();
		entity.unsetInvoicePaymentInsurances();
		entity.unsetPatientPaymentInsurances();
		entity.unsetPatientVisitPatientPaymentInsurances();
	}
