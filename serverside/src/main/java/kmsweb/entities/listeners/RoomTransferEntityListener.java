...
import kmsweb.configs.security.auditing.services.RoomTransferEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing RoomTransferEntity.
 */
@Slf4j
@Component
public class RoomTransferEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static RoomTransferEntityReadAuditService auditService;
	...
	public void init(
		...
		RoomTransferEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		RoomTransferEntityListener.permissionEvaluator = permissionEvaluator;
		RoomTransferEntityListener.auditService = auditService;
		RoomTransferEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(RoomTransferEntity entity) {
	...
	@PostPersist
	public void afterPersist(RoomTransferEntity entity) {
	...
	@PreRemove
	public void beforeRemove(RoomTransferEntity entity) {
	...
	@PostRemove
	public void afterRemove(RoomTransferEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(RoomTransferEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(RoomTransferEntity entity) {
	...
	@PostLoad
	public void afterLoad(RoomTransferEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the RoomTransferEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the RoomTransferEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Room Transfer is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(RoomTransferEntity entity) {
		entity.unsetBedFacility();
		entity.unsetMedicalExaminationRecord();
		entity.unsetTransferringStaff();
	}
