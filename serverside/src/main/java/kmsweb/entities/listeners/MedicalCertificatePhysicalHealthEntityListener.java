...
import kmsweb.configs.security.auditing.services.MedicalCertificatePhysicalHealthEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalCertificatePhysicalHealthEntity.
 */
@Slf4j
@Component
public class MedicalCertificatePhysicalHealthEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalCertificatePhysicalHealthEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalCertificatePhysicalHealthEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalCertificatePhysicalHealthEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalCertificatePhysicalHealthEntityListener.auditService = auditService;
		MedicalCertificatePhysicalHealthEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalCertificatePhysicalHealthEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalCertificatePhysicalHealthEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalCertificatePhysicalHealthEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalCertificatePhysicalHealthEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalCertificatePhysicalHealthEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalCertificatePhysicalHealthEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalCertificatePhysicalHealthEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalCertificatePhysicalHealthEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalCertificatePhysicalHealthEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Certificate Physical Health is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalCertificatePhysicalHealthEntity entity) {
