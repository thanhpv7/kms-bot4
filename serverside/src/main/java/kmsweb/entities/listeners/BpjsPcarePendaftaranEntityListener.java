...
import kmsweb.configs.security.auditing.services.BpjsPcarePendaftaranEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPcarePendaftaranEntity.
 */
@Slf4j
@Component
public class BpjsPcarePendaftaranEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPcarePendaftaranEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPcarePendaftaranEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPcarePendaftaranEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPcarePendaftaranEntityListener.auditService = auditService;
		BpjsPcarePendaftaranEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPcarePendaftaranEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPcarePendaftaranEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPcarePendaftaranEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPcarePendaftaranEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPcarePendaftaranEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPcarePendaftaranEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPcarePendaftaranEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPcarePendaftaranEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPcarePendaftaranEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Bpjs Pcare Pendaftaran is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPcarePendaftaranEntity entity) {
