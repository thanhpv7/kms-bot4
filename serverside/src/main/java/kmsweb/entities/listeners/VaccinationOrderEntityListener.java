...
import kmsweb.configs.security.auditing.services.VaccinationOrderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing VaccinationOrderEntity.
 */
@Slf4j
@Component
public class VaccinationOrderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static VaccinationOrderEntityReadAuditService auditService;
	...
	public void init(
		...
		VaccinationOrderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		VaccinationOrderEntityListener.permissionEvaluator = permissionEvaluator;
		VaccinationOrderEntityListener.auditService = auditService;
		VaccinationOrderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(VaccinationOrderEntity entity) {
	...
	@PostPersist
	public void afterPersist(VaccinationOrderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(VaccinationOrderEntity entity) {
	...
	@PostRemove
	public void afterRemove(VaccinationOrderEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(VaccinationOrderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(VaccinationOrderEntity entity) {
	...
	@PostLoad
	public void afterLoad(VaccinationOrderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the VaccinationOrderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the VaccinationOrderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Vaccination Order is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(VaccinationOrderEntity entity) {
