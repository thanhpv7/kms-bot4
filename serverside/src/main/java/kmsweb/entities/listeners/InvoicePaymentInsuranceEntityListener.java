...
import kmsweb.configs.security.auditing.services.InvoicePaymentInsuranceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InvoicePaymentInsuranceEntity.
 */
@Slf4j
@Component
public class InvoicePaymentInsuranceEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InvoicePaymentInsuranceEntityReadAuditService auditService;
	...
	public void init(
		...
		InvoicePaymentInsuranceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InvoicePaymentInsuranceEntityListener.permissionEvaluator = permissionEvaluator;
		InvoicePaymentInsuranceEntityListener.auditService = auditService;
		InvoicePaymentInsuranceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InvoicePaymentInsuranceEntity entity) {
	...
	@PostPersist
	public void afterPersist(InvoicePaymentInsuranceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InvoicePaymentInsuranceEntity entity) {
	...
	@PostRemove
	public void afterRemove(InvoicePaymentInsuranceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InvoicePaymentInsuranceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InvoicePaymentInsuranceEntity entity) {
	...
	@PostLoad
	public void afterLoad(InvoicePaymentInsuranceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InvoicePaymentInsuranceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InvoicePaymentInsuranceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Invoice Payment Insurance is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InvoicePaymentInsuranceEntity entity) {
