...
import kmsweb.configs.security.auditing.services.PcareNonCompoundDrugEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareNonCompoundDrugEntity.
 */
@Slf4j
@Component
public class PcareNonCompoundDrugEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareNonCompoundDrugEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareNonCompoundDrugEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareNonCompoundDrugEntityListener.permissionEvaluator = permissionEvaluator;
		PcareNonCompoundDrugEntityListener.auditService = auditService;
		PcareNonCompoundDrugEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareNonCompoundDrugEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareNonCompoundDrugEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareNonCompoundDrugEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareNonCompoundDrugEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareNonCompoundDrugEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareNonCompoundDrugEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareNonCompoundDrugEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareNonCompoundDrugEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareNonCompoundDrugEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare NonCompound Drug is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareNonCompoundDrugEntity entity) {
