...
import kmsweb.configs.security.auditing.services.BpjsClaimDataEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsClaimDataEntity.
 */
@Slf4j
@Component
public class BpjsClaimDataEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsClaimDataEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsClaimDataEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsClaimDataEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsClaimDataEntityListener.auditService = auditService;
		BpjsClaimDataEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsClaimDataEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsClaimDataEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsClaimDataEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsClaimDataEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsClaimDataEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsClaimDataEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsClaimDataEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsClaimDataEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsClaimDataEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Claim Data is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsClaimDataEntity entity) {
	}
