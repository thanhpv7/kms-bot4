...
import kmsweb.configs.security.auditing.services.BpjsHealthFacilityEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsHealthFacilityEntity.
 */
@Slf4j
@Component
public class BpjsHealthFacilityEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsHealthFacilityEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsHealthFacilityEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsHealthFacilityEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsHealthFacilityEntityListener.auditService = auditService;
		BpjsHealthFacilityEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsHealthFacilityEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsHealthFacilityEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsHealthFacilityEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsHealthFacilityEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsHealthFacilityEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsHealthFacilityEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsHealthFacilityEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsHealthFacilityEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsHealthFacilityEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Health Facility is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsHealthFacilityEntity entity) {
		entity.unsetBpjsClaimSubmissions();
		entity.unsetBpjsFacilityLists();
		entity.unsetBpjsPatientReferrals();
		entity.unsetBpjsReferralSpecialists();
	}
