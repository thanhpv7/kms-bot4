...
import kmsweb.configs.security.auditing.services.BpjsDismissalMappingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsDismissalMappingEntity.
 */
@Slf4j
@Component
public class BpjsDismissalMappingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsDismissalMappingEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsDismissalMappingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsDismissalMappingEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsDismissalMappingEntityListener.auditService = auditService;
		BpjsDismissalMappingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsDismissalMappingEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsDismissalMappingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsDismissalMappingEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsDismissalMappingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsDismissalMappingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsDismissalMappingEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsDismissalMappingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsDismissalMappingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsDismissalMappingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Dismissal Mapping is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsDismissalMappingEntity entity) {
