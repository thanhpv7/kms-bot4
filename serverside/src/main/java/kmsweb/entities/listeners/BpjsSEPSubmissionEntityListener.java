...
import kmsweb.configs.security.auditing.services.BpjsSEPSubmissionEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsSEPSubmissionEntity.
 */
@Slf4j
@Component
public class BpjsSEPSubmissionEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsSEPSubmissionEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsSEPSubmissionEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsSEPSubmissionEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsSEPSubmissionEntityListener.auditService = auditService;
		BpjsSEPSubmissionEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsSEPSubmissionEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsSEPSubmissionEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsSEPSubmissionEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsSEPSubmissionEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsSEPSubmissionEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsSEPSubmissionEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsSEPSubmissionEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsSEPSubmissionEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsSEPSubmissionEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS SEP Submission is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsSEPSubmissionEntity entity) {
