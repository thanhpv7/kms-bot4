...
import kmsweb.configs.security.auditing.services.FacilityUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing FacilityUserEntity.
 */
@Slf4j
@Component
public class FacilityUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static FacilityUserEntityReadAuditService auditService;
	...
	public void init(
		...
		FacilityUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		FacilityUserEntityListener.permissionEvaluator = permissionEvaluator;
		FacilityUserEntityListener.auditService = auditService;
		FacilityUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(FacilityUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(FacilityUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(FacilityUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(FacilityUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(FacilityUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(FacilityUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(FacilityUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the FacilityUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the FacilityUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Facility User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(FacilityUserEntity entity) {
		entity.unsetStaff();
	}
