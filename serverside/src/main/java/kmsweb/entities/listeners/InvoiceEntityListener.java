...
import kmsweb.configs.security.auditing.services.InvoiceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InvoiceEntity.
 */
@Slf4j
@Component
public class InvoiceEntityListener {
	...
	private static InvoiceEntityReadAuditService auditService;
	...
	public void init(
		...
		InvoiceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InvoiceEntityListener.permissionEvaluator = permissionEvaluator;
		InvoiceEntityListener.auditService = auditService;
		InvoiceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InvoiceEntity entity) {
	...
	@PostPersist
	public void afterPersist(InvoiceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InvoiceEntity entity) {
	...
	@PostRemove
	public void afterRemove(InvoiceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InvoiceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InvoiceEntity entity) {
	...
	@PostLoad
	public void afterLoad(InvoiceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InvoiceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InvoiceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Invoice is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InvoiceEntity entity) {
		entity.unsetMedicationHeader();
		entity.unsetInvoicePaymentOther();
		entity.unsetInvoicePaymentSelfPaying();
