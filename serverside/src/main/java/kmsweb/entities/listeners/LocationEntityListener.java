...
import kmsweb.configs.security.auditing.services.LocationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing LocationEntity.
 */
@Slf4j
@Component
public class LocationEntityListener {
	...
	private static LocationEntityReadAuditService auditService;
	...
	public void init(
		...
		LocationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		LocationEntityListener.permissionEvaluator = permissionEvaluator;
		LocationEntityListener.auditService = auditService;
		LocationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(LocationEntity entity) {
	...
	@PostPersist
	public void afterPersist(LocationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(LocationEntity entity) {
	...
	@PostRemove
	public void afterRemove(LocationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(LocationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(LocationEntity entity) {
	...
	@PostLoad
	public void afterLoad(LocationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the LocationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the LocationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Location is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(LocationEntity entity) {
