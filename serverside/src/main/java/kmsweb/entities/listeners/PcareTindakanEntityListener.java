...
import kmsweb.configs.security.auditing.services.PcareTindakanEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareTindakanEntity.
 */
@Slf4j
@Component
public class PcareTindakanEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareTindakanEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareTindakanEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareTindakanEntityListener.permissionEvaluator = permissionEvaluator;
		PcareTindakanEntityListener.auditService = auditService;
		PcareTindakanEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareTindakanEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareTindakanEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareTindakanEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareTindakanEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareTindakanEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareTindakanEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareTindakanEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareTindakanEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareTindakanEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Tindakan is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareTindakanEntity entity) {
	}
