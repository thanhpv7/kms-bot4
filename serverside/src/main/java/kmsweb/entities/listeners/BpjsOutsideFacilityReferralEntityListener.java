...
import kmsweb.configs.security.auditing.services.BpjsOutsideFacilityReferralEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsOutsideFacilityReferralEntity.
 */
@Slf4j
@Component
public class BpjsOutsideFacilityReferralEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsOutsideFacilityReferralEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsOutsideFacilityReferralEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsOutsideFacilityReferralEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsOutsideFacilityReferralEntityListener.auditService = auditService;
		BpjsOutsideFacilityReferralEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsOutsideFacilityReferralEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsOutsideFacilityReferralEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsOutsideFacilityReferralEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsOutsideFacilityReferralEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsOutsideFacilityReferralEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsOutsideFacilityReferralEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsOutsideFacilityReferralEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsOutsideFacilityReferralEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsOutsideFacilityReferralEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Outside Facility Referral is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsOutsideFacilityReferralEntity entity) {
	}
