...
import kmsweb.configs.security.auditing.services.ServiceItemAssignmentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ServiceItemAssignmentEntity.
 */
@Slf4j
@Component
public class ServiceItemAssignmentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ServiceItemAssignmentEntityReadAuditService auditService;
	...
	public void init(
		...
		ServiceItemAssignmentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ServiceItemAssignmentEntityListener.permissionEvaluator = permissionEvaluator;
		ServiceItemAssignmentEntityListener.auditService = auditService;
		ServiceItemAssignmentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ServiceItemAssignmentEntity entity) {
	...
	@PostPersist
	public void afterPersist(ServiceItemAssignmentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ServiceItemAssignmentEntity entity) {
	...
	@PostRemove
	public void afterRemove(ServiceItemAssignmentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ServiceItemAssignmentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ServiceItemAssignmentEntity entity) {
	...
	@PostLoad
	public void afterLoad(ServiceItemAssignmentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ServiceItemAssignmentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ServiceItemAssignmentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Service Item Assignment is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ServiceItemAssignmentEntity entity) {
		entity.unsetServiceItem();
