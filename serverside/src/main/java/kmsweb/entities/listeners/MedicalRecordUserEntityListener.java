...
import kmsweb.configs.security.auditing.services.MedicalRecordUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalRecordUserEntity.
 */
@Slf4j
@Component
public class MedicalRecordUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalRecordUserEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalRecordUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalRecordUserEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalRecordUserEntityListener.auditService = auditService;
		MedicalRecordUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalRecordUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalRecordUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalRecordUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalRecordUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalRecordUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalRecordUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalRecordUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalRecordUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalRecordUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Record user is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalRecordUserEntity entity) {
		entity.unsetStaff();
	}
