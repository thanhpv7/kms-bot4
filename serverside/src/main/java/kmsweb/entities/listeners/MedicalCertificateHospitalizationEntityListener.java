...
import kmsweb.configs.security.auditing.services.MedicalCertificateHospitalizationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalCertificateHospitalizationEntity.
 */
@Slf4j
@Component
public class MedicalCertificateHospitalizationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalCertificateHospitalizationEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalCertificateHospitalizationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalCertificateHospitalizationEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalCertificateHospitalizationEntityListener.auditService = auditService;
		MedicalCertificateHospitalizationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalCertificateHospitalizationEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalCertificateHospitalizationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalCertificateHospitalizationEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalCertificateHospitalizationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalCertificateHospitalizationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalCertificateHospitalizationEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalCertificateHospitalizationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalCertificateHospitalizationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalCertificateHospitalizationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Certificate Hospitalization is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalCertificateHospitalizationEntity entity) {
