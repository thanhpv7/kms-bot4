...
import kmsweb.configs.security.auditing.services.InventoryBatchCorrectionEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InventoryBatchCorrectionEntity.
 */
@Slf4j
@Component
public class InventoryBatchCorrectionEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InventoryBatchCorrectionEntityReadAuditService auditService;
	...
	public void init(
		...
		InventoryBatchCorrectionEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InventoryBatchCorrectionEntityListener.permissionEvaluator = permissionEvaluator;
		InventoryBatchCorrectionEntityListener.auditService = auditService;
		InventoryBatchCorrectionEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InventoryBatchCorrectionEntity entity) {
	...
	@PostPersist
	public void afterPersist(InventoryBatchCorrectionEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InventoryBatchCorrectionEntity entity) {
	...
	@PostRemove
	public void afterRemove(InventoryBatchCorrectionEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InventoryBatchCorrectionEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InventoryBatchCorrectionEntity entity) {
	...
	@PostLoad
	public void afterLoad(InventoryBatchCorrectionEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InventoryBatchCorrectionEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InventoryBatchCorrectionEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Inventory Batch Correction is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InventoryBatchCorrectionEntity entity) {
