...
import kmsweb.configs.security.auditing.services.ReferenceHeaderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ReferenceHeaderEntity.
 */
@Slf4j
@Component
public class ReferenceHeaderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ReferenceHeaderEntityReadAuditService auditService;
	...
	public void init(
		...
		ReferenceHeaderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ReferenceHeaderEntityListener.permissionEvaluator = permissionEvaluator;
		ReferenceHeaderEntityListener.auditService = auditService;
		ReferenceHeaderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ReferenceHeaderEntity entity) {
	...
	@PostPersist
	public void afterPersist(ReferenceHeaderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ReferenceHeaderEntity entity) {
	...
	@PostRemove
	public void afterRemove(ReferenceHeaderEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ReferenceHeaderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ReferenceHeaderEntity entity) {
	...
	@PostLoad
	public void afterLoad(ReferenceHeaderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ReferenceHeaderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ReferenceHeaderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Reference Header is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ReferenceHeaderEntity entity) {
	}
