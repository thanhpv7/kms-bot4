...
import kmsweb.configs.security.auditing.services.NewbornDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NewbornDetailEntity.
 */
@Slf4j
@Component
public class NewbornDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NewbornDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		NewbornDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NewbornDetailEntityListener.permissionEvaluator = permissionEvaluator;
		NewbornDetailEntityListener.auditService = auditService;
		NewbornDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NewbornDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(NewbornDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NewbornDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(NewbornDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NewbornDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NewbornDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(NewbornDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NewbornDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NewbornDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Newborn Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NewbornDetailEntity entity) {
		entity.unsetDeliveryMedicalExaminationRecord();
		entity.unsetStaff();
		entity.unsetVitalSigns();
	}
