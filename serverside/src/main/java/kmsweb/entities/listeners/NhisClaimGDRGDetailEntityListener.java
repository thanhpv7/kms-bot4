...
import kmsweb.configs.security.auditing.services.NhisClaimGDRGDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisClaimGDRGDetailEntity.
 */
@Slf4j
@Component
public class NhisClaimGDRGDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisClaimGDRGDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisClaimGDRGDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisClaimGDRGDetailEntityListener.permissionEvaluator = permissionEvaluator;
		NhisClaimGDRGDetailEntityListener.auditService = auditService;
		NhisClaimGDRGDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisClaimGDRGDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisClaimGDRGDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisClaimGDRGDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisClaimGDRGDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisClaimGDRGDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisClaimGDRGDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisClaimGDRGDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisClaimGDRGDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisClaimGDRGDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Claim GDRG Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisClaimGDRGDetailEntity entity) {
	}
