...
import kmsweb.configs.security.auditing.services.McuPackageItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing McuPackageItemEntity.
 */
@Slf4j
@Component
public class McuPackageItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static McuPackageItemEntityReadAuditService auditService;
	...
	public void init(
		...
		McuPackageItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		McuPackageItemEntityListener.permissionEvaluator = permissionEvaluator;
		McuPackageItemEntityListener.auditService = auditService;
		McuPackageItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(McuPackageItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(McuPackageItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(McuPackageItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(McuPackageItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(McuPackageItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(McuPackageItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(McuPackageItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the McuPackageItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the McuPackageItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to MCU Package Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(McuPackageItemEntity entity) {
		entity.unsetMcuPackage();
		entity.unsetService();
		entity.unsetStaff();
