...
import kmsweb.configs.security.auditing.services.PrescriptionCompoundEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PrescriptionCompoundEntity.
 */
@Slf4j
@Component
public class PrescriptionCompoundEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PrescriptionCompoundEntityReadAuditService auditService;
	...
	public void init(
		...
		PrescriptionCompoundEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PrescriptionCompoundEntityListener.permissionEvaluator = permissionEvaluator;
		PrescriptionCompoundEntityListener.auditService = auditService;
		PrescriptionCompoundEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PrescriptionCompoundEntity entity) {
	...
	@PostPersist
	public void afterPersist(PrescriptionCompoundEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PrescriptionCompoundEntity entity) {
	...
	@PostRemove
	public void afterRemove(PrescriptionCompoundEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PrescriptionCompoundEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PrescriptionCompoundEntity entity) {
	...
	@PostLoad
	public void afterLoad(PrescriptionCompoundEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PrescriptionCompoundEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PrescriptionCompoundEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Prescription Compound is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PrescriptionCompoundEntity entity) {
		entity.unsetUnit();
		entity.unsetPrescriptionHeader();
		entity.unsetCompoundItems();
	}
