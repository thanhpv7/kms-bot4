...
import kmsweb.configs.security.auditing.services.PharmacySetupEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PharmacySetupEntity.
 */
@Slf4j
@Component
public class PharmacySetupEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PharmacySetupEntityReadAuditService auditService;
	...
	public void init(
		...
		PharmacySetupEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PharmacySetupEntityListener.permissionEvaluator = permissionEvaluator;
		PharmacySetupEntityListener.auditService = auditService;
		PharmacySetupEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PharmacySetupEntity entity) {
	...
	@PostPersist
	public void afterPersist(PharmacySetupEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PharmacySetupEntity entity) {
	...
	@PostRemove
	public void afterRemove(PharmacySetupEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PharmacySetupEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PharmacySetupEntity entity) {
	...
	@PostLoad
	public void afterLoad(PharmacySetupEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PharmacySetupEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PharmacySetupEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Pharmacy Setup is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PharmacySetupEntity entity) {
	}
