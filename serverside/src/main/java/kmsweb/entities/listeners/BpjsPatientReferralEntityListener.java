...
import kmsweb.configs.security.auditing.services.BpjsPatientReferralEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPatientReferralEntity.
 */
@Slf4j
@Component
public class BpjsPatientReferralEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPatientReferralEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPatientReferralEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPatientReferralEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPatientReferralEntityListener.auditService = auditService;
		BpjsPatientReferralEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPatientReferralEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPatientReferralEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPatientReferralEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPatientReferralEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPatientReferralEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPatientReferralEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPatientReferralEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPatientReferralEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPatientReferralEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Patient Referral is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPatientReferralEntity entity) {
