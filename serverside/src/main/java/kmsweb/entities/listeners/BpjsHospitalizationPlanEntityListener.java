...
import kmsweb.configs.security.auditing.services.BpjsHospitalizationPlanEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsHospitalizationPlanEntity.
 */
@Slf4j
@Component
public class BpjsHospitalizationPlanEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsHospitalizationPlanEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsHospitalizationPlanEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsHospitalizationPlanEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsHospitalizationPlanEntityListener.auditService = auditService;
		BpjsHospitalizationPlanEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsHospitalizationPlanEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsHospitalizationPlanEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsHospitalizationPlanEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsHospitalizationPlanEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsHospitalizationPlanEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsHospitalizationPlanEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsHospitalizationPlanEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsHospitalizationPlanEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsHospitalizationPlanEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Hospitalization Plan is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsHospitalizationPlanEntity entity) {
