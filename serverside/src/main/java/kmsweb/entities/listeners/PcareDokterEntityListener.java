...
import kmsweb.configs.security.auditing.services.PcareDokterEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareDokterEntity.
 */
@Slf4j
@Component
public class PcareDokterEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareDokterEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareDokterEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareDokterEntityListener.permissionEvaluator = permissionEvaluator;
		PcareDokterEntityListener.auditService = auditService;
		PcareDokterEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareDokterEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareDokterEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareDokterEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareDokterEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareDokterEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareDokterEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareDokterEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareDokterEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareDokterEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Dokter is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareDokterEntity entity) {
		entity.unsetStaff();
	}
