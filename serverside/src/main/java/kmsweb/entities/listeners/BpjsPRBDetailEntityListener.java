...
import kmsweb.configs.security.auditing.services.BpjsPRBDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPRBDetailEntity.
 */
@Slf4j
@Component
public class BpjsPRBDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPRBDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPRBDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPRBDetailEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPRBDetailEntityListener.auditService = auditService;
		BpjsPRBDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPRBDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPRBDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPRBDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPRBDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPRBDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPRBDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPRBDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPRBDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPRBDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS PRB Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPRBDetailEntity entity) {
