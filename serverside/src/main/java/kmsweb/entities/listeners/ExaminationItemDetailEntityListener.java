...
import kmsweb.configs.security.auditing.services.ExaminationItemDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ExaminationItemDetailEntity.
 */
@Slf4j
@Component
public class ExaminationItemDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ExaminationItemDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		ExaminationItemDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ExaminationItemDetailEntityListener.permissionEvaluator = permissionEvaluator;
		ExaminationItemDetailEntityListener.auditService = auditService;
		ExaminationItemDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ExaminationItemDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(ExaminationItemDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ExaminationItemDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(ExaminationItemDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ExaminationItemDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ExaminationItemDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(ExaminationItemDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ExaminationItemDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ExaminationItemDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Examination Item Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ExaminationItemDetailEntity entity) {
