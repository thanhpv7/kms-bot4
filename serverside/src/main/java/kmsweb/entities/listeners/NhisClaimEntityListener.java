...
import kmsweb.configs.security.auditing.services.NhisClaimEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisClaimEntity.
 */
@Slf4j
@Component
public class NhisClaimEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisClaimEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisClaimEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisClaimEntityListener.permissionEvaluator = permissionEvaluator;
		NhisClaimEntityListener.auditService = auditService;
		NhisClaimEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisClaimEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisClaimEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisClaimEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisClaimEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisClaimEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisClaimEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisClaimEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisClaimEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisClaimEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Claim is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisClaimEntity entity) {
	}
