...
import kmsweb.configs.security.auditing.services.CashierUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing CashierUserEntity.
 */
@Slf4j
@Component
public class CashierUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static CashierUserEntityReadAuditService auditService;
	...
	public void init(
		...
		CashierUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		CashierUserEntityListener.permissionEvaluator = permissionEvaluator;
		CashierUserEntityListener.auditService = auditService;
		CashierUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(CashierUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(CashierUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(CashierUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(CashierUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(CashierUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(CashierUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(CashierUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the CashierUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the CashierUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Cashier User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(CashierUserEntity entity) {
		entity.unsetStaff();
	}
