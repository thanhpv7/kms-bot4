...
import kmsweb.configs.security.auditing.services.BpjsPcarePesertaKegiatanKelompokEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPcarePesertaKegiatanKelompokEntity.
 */
@Slf4j
@Component
public class BpjsPcarePesertaKegiatanKelompokEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPcarePesertaKegiatanKelompokEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPcarePesertaKegiatanKelompokEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPcarePesertaKegiatanKelompokEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPcarePesertaKegiatanKelompokEntityListener.auditService = auditService;
		BpjsPcarePesertaKegiatanKelompokEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPcarePesertaKegiatanKelompokEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPcarePesertaKegiatanKelompokEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPcarePesertaKegiatanKelompokEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPcarePesertaKegiatanKelompokEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPcarePesertaKegiatanKelompokEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPcarePesertaKegiatanKelompokEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPcarePesertaKegiatanKelompokEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPcarePesertaKegiatanKelompokEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPcarePesertaKegiatanKelompokEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Pcare Peserta Kegiatan Kelompok is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPcarePesertaKegiatanKelompokEntity entity) {
	}
