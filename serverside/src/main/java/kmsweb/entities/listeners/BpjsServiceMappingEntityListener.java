...
import kmsweb.configs.security.auditing.services.BpjsServiceMappingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsServiceMappingEntity.
 */
@Slf4j
@Component
public class BpjsServiceMappingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsServiceMappingEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsServiceMappingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsServiceMappingEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsServiceMappingEntityListener.auditService = auditService;
		BpjsServiceMappingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsServiceMappingEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsServiceMappingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsServiceMappingEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsServiceMappingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsServiceMappingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsServiceMappingEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsServiceMappingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsServiceMappingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsServiceMappingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Service Mapping is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsServiceMappingEntity entity) {
		entity.unsetService();
		entity.unsetBpjsClaimSubmissions();
		entity.unsetBpjsControlPlans();
		entity.unsetBpjsPatientReferrals();
		entity.unsetPolikontrol();
	}
