...
import kmsweb.configs.security.auditing.services.GoodsReceiveNoteBatchEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing GoodsReceiveNoteBatchEntity.
 */
@Slf4j
@Component
public class GoodsReceiveNoteBatchEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static GoodsReceiveNoteBatchEntityReadAuditService auditService;
	...
	public void init(
		...
		GoodsReceiveNoteBatchEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		GoodsReceiveNoteBatchEntityListener.permissionEvaluator = permissionEvaluator;
		GoodsReceiveNoteBatchEntityListener.auditService = auditService;
		GoodsReceiveNoteBatchEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(GoodsReceiveNoteBatchEntity entity) {
	...
	@PostPersist
	public void afterPersist(GoodsReceiveNoteBatchEntity entity) {
	...
	@PreRemove
	public void beforeRemove(GoodsReceiveNoteBatchEntity entity) {
	...
	@PostRemove
	public void afterRemove(GoodsReceiveNoteBatchEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(GoodsReceiveNoteBatchEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(GoodsReceiveNoteBatchEntity entity) {
	...
	@PostLoad
	public void afterLoad(GoodsReceiveNoteBatchEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the GoodsReceiveNoteBatchEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the GoodsReceiveNoteBatchEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Goods Receive Note Batch is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(GoodsReceiveNoteBatchEntity entity) {
		entity.unsetGoodsReceiveNoteItem();
