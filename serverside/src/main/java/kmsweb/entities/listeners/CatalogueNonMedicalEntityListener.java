...
import kmsweb.configs.security.auditing.services.CatalogueNonMedicalEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing CatalogueNonMedicalEntity.
 */
@Slf4j
@Component
public class CatalogueNonMedicalEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static CatalogueNonMedicalEntityReadAuditService auditService;
	...
	public void init(
		...
		CatalogueNonMedicalEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		CatalogueNonMedicalEntityListener.permissionEvaluator = permissionEvaluator;
		CatalogueNonMedicalEntityListener.auditService = auditService;
		CatalogueNonMedicalEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(CatalogueNonMedicalEntity entity) {
	...
	@PostPersist
	public void afterPersist(CatalogueNonMedicalEntity entity) {
	...
	@PreRemove
	public void beforeRemove(CatalogueNonMedicalEntity entity) {
	...
	@PostRemove
	public void afterRemove(CatalogueNonMedicalEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(CatalogueNonMedicalEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(CatalogueNonMedicalEntity entity) {
	...
	@PostLoad
	public void afterLoad(CatalogueNonMedicalEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the CatalogueNonMedicalEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the CatalogueNonMedicalEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Catalogue Non Medical is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(CatalogueNonMedicalEntity entity) {
