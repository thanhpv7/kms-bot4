...
import kmsweb.configs.security.auditing.services.PatientCaseHistoryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientCaseHistoryEntity.
 */
@Slf4j
@Component
public class PatientCaseHistoryEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientCaseHistoryEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientCaseHistoryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientCaseHistoryEntityListener.permissionEvaluator = permissionEvaluator;
		PatientCaseHistoryEntityListener.auditService = auditService;
		PatientCaseHistoryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientCaseHistoryEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientCaseHistoryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientCaseHistoryEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientCaseHistoryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientCaseHistoryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientCaseHistoryEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientCaseHistoryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientCaseHistoryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientCaseHistoryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Case History is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientCaseHistoryEntity entity) {
