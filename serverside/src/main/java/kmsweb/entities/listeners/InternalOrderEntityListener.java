...
import kmsweb.configs.security.auditing.services.InternalOrderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InternalOrderEntity.
 */
@Slf4j
@Component
public class InternalOrderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InternalOrderEntityReadAuditService auditService;
	...
	public void init(
		...
		InternalOrderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InternalOrderEntityListener.permissionEvaluator = permissionEvaluator;
		InternalOrderEntityListener.auditService = auditService;
		InternalOrderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InternalOrderEntity entity) {
	...
	@PostPersist
	public void afterPersist(InternalOrderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InternalOrderEntity entity) {
	...
	@PostRemove
	public void afterRemove(InternalOrderEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InternalOrderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InternalOrderEntity entity) {
	...
	@PostLoad
	public void afterLoad(InternalOrderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InternalOrderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InternalOrderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Internal Order is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InternalOrderEntity entity) {
