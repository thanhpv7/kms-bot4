...
import kmsweb.configs.security.auditing.services.ExaminationItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ExaminationItemEntity.
 */
@Slf4j
@Component
public class ExaminationItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ExaminationItemEntityReadAuditService auditService;
	...
	public void init(
		...
		ExaminationItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ExaminationItemEntityListener.permissionEvaluator = permissionEvaluator;
		ExaminationItemEntityListener.auditService = auditService;
		ExaminationItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ExaminationItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(ExaminationItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ExaminationItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(ExaminationItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ExaminationItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ExaminationItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(ExaminationItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ExaminationItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ExaminationItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Examination Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ExaminationItemEntity entity) {
		entity.unsetExaminationGroup();
		entity.unsetServiceItem();
		entity.unsetDiagnosticExaminationResults();
		entity.unsetExaminationItemDetails();
		entity.unsetExaminationItemOfMCUItem();
