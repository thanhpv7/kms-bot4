...
import kmsweb.configs.security.auditing.services.InventoryAdjustmentItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InventoryAdjustmentItemEntity.
 */
@Slf4j
@Component
public class InventoryAdjustmentItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InventoryAdjustmentItemEntityReadAuditService auditService;
	...
	public void init(
		...
		InventoryAdjustmentItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InventoryAdjustmentItemEntityListener.permissionEvaluator = permissionEvaluator;
		InventoryAdjustmentItemEntityListener.auditService = auditService;
		InventoryAdjustmentItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InventoryAdjustmentItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(InventoryAdjustmentItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InventoryAdjustmentItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(InventoryAdjustmentItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InventoryAdjustmentItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InventoryAdjustmentItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(InventoryAdjustmentItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InventoryAdjustmentItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InventoryAdjustmentItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Inventory Adjustment Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InventoryAdjustmentItemEntity entity) {
		entity.unsetInventoryAdjustment();
