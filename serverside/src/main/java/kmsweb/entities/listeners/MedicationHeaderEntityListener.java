...
import kmsweb.configs.security.auditing.services.MedicationHeaderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicationHeaderEntity.
 */
@Slf4j
@Component
public class MedicationHeaderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicationHeaderEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicationHeaderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicationHeaderEntityListener.permissionEvaluator = permissionEvaluator;
		MedicationHeaderEntityListener.auditService = auditService;
		MedicationHeaderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicationHeaderEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicationHeaderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicationHeaderEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicationHeaderEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicationHeaderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicationHeaderEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicationHeaderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicationHeaderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicationHeaderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medication Header is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicationHeaderEntity entity) {
		entity.unsetInvoice();
