...
import kmsweb.configs.security.auditing.services.TariffPackageServiceAssignmentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TariffPackageServiceAssignmentEntity.
 */
@Slf4j
@Component
public class TariffPackageServiceAssignmentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TariffPackageServiceAssignmentEntityReadAuditService auditService;
	...
	public void init(
		...
		TariffPackageServiceAssignmentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TariffPackageServiceAssignmentEntityListener.permissionEvaluator = permissionEvaluator;
		TariffPackageServiceAssignmentEntityListener.auditService = auditService;
		TariffPackageServiceAssignmentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TariffPackageServiceAssignmentEntity entity) {
	...
	@PostPersist
	public void afterPersist(TariffPackageServiceAssignmentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TariffPackageServiceAssignmentEntity entity) {
	...
	@PostRemove
	public void afterRemove(TariffPackageServiceAssignmentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TariffPackageServiceAssignmentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TariffPackageServiceAssignmentEntity entity) {
	...
	@PostLoad
	public void afterLoad(TariffPackageServiceAssignmentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TariffPackageServiceAssignmentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TariffPackageServiceAssignmentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Tariff Package Service Assignment is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TariffPackageServiceAssignmentEntity entity) {
		entity.unsetTariffPackage();
	}
