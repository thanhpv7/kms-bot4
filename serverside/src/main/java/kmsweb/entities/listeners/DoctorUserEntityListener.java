...
import kmsweb.configs.security.auditing.services.DoctorUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DoctorUserEntity.
 */
@Slf4j
@Component
public class DoctorUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DoctorUserEntityReadAuditService auditService;
	...
	public void init(
		...
		DoctorUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DoctorUserEntityListener.permissionEvaluator = permissionEvaluator;
		DoctorUserEntityListener.auditService = auditService;
		DoctorUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DoctorUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(DoctorUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DoctorUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(DoctorUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DoctorUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DoctorUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(DoctorUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DoctorUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DoctorUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Doctor User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DoctorUserEntity entity) {
		entity.unsetStaff();
	}
