...
import kmsweb.configs.security.auditing.services.DeliveryProgressEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DeliveryProgressEntity.
 */
@Slf4j
@Component
public class DeliveryProgressEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DeliveryProgressEntityReadAuditService auditService;
	...
	public void init(
		...
		DeliveryProgressEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DeliveryProgressEntityListener.permissionEvaluator = permissionEvaluator;
		DeliveryProgressEntityListener.auditService = auditService;
		DeliveryProgressEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DeliveryProgressEntity entity) {
	...
	@PostPersist
	public void afterPersist(DeliveryProgressEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DeliveryProgressEntity entity) {
	...
	@PostRemove
	public void afterRemove(DeliveryProgressEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DeliveryProgressEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DeliveryProgressEntity entity) {
	...
	@PostLoad
	public void afterLoad(DeliveryProgressEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DeliveryProgressEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DeliveryProgressEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Delivery Progress is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DeliveryProgressEntity entity) {
