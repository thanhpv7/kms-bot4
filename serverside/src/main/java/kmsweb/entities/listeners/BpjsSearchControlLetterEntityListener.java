...
import kmsweb.configs.security.auditing.services.BpjsSearchControlLetterEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsSearchControlLetterEntity.
 */
@Slf4j
@Component
public class BpjsSearchControlLetterEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsSearchControlLetterEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsSearchControlLetterEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsSearchControlLetterEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsSearchControlLetterEntityListener.auditService = auditService;
		BpjsSearchControlLetterEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsSearchControlLetterEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsSearchControlLetterEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsSearchControlLetterEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsSearchControlLetterEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsSearchControlLetterEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsSearchControlLetterEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsSearchControlLetterEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsSearchControlLetterEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsSearchControlLetterEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Search Control Letter is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsSearchControlLetterEntity entity) {
	}
