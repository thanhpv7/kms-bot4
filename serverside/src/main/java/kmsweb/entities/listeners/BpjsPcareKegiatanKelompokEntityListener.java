...
import kmsweb.configs.security.auditing.services.BpjsPcareKegiatanKelompokEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPcareKegiatanKelompokEntity.
 */
@Slf4j
@Component
public class BpjsPcareKegiatanKelompokEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPcareKegiatanKelompokEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPcareKegiatanKelompokEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPcareKegiatanKelompokEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPcareKegiatanKelompokEntityListener.auditService = auditService;
		BpjsPcareKegiatanKelompokEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPcareKegiatanKelompokEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPcareKegiatanKelompokEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPcareKegiatanKelompokEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPcareKegiatanKelompokEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPcareKegiatanKelompokEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPcareKegiatanKelompokEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPcareKegiatanKelompokEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPcareKegiatanKelompokEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPcareKegiatanKelompokEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Pcare Kegiatan Kelompok is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPcareKegiatanKelompokEntity entity) {
	}
