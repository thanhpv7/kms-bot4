...
import kmsweb.configs.security.auditing.services.BatchStockBalanceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BatchStockBalanceEntity.
 */
@Slf4j
@Component
public class BatchStockBalanceEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BatchStockBalanceEntityReadAuditService auditService;
	...
	public void init(
		...
		BatchStockBalanceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BatchStockBalanceEntityListener.permissionEvaluator = permissionEvaluator;
		BatchStockBalanceEntityListener.auditService = auditService;
		BatchStockBalanceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BatchStockBalanceEntity entity) {
	...
	@PostPersist
	public void afterPersist(BatchStockBalanceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BatchStockBalanceEntity entity) {
	...
	@PostRemove
	public void afterRemove(BatchStockBalanceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BatchStockBalanceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BatchStockBalanceEntity entity) {
	...
	@PostLoad
	public void afterLoad(BatchStockBalanceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BatchStockBalanceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BatchStockBalanceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Batch Stock Balance is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BatchStockBalanceEntity entity) {
