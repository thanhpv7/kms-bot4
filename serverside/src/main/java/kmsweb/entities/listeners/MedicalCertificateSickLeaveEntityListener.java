...
import kmsweb.configs.security.auditing.services.MedicalCertificateSickLeaveEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalCertificateSickLeaveEntity.
 */
@Slf4j
@Component
public class MedicalCertificateSickLeaveEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalCertificateSickLeaveEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalCertificateSickLeaveEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalCertificateSickLeaveEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalCertificateSickLeaveEntityListener.auditService = auditService;
		MedicalCertificateSickLeaveEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalCertificateSickLeaveEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalCertificateSickLeaveEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalCertificateSickLeaveEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalCertificateSickLeaveEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalCertificateSickLeaveEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalCertificateSickLeaveEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalCertificateSickLeaveEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalCertificateSickLeaveEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalCertificateSickLeaveEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Certificate Sick Leave is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalCertificateSickLeaveEntity entity) {
