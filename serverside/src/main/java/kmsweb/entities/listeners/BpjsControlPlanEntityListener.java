...
import kmsweb.configs.security.auditing.services.BpjsControlPlanEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsControlPlanEntity.
 */
@Slf4j
@Component
public class BpjsControlPlanEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsControlPlanEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsControlPlanEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsControlPlanEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsControlPlanEntityListener.auditService = auditService;
		BpjsControlPlanEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsControlPlanEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsControlPlanEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsControlPlanEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsControlPlanEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsControlPlanEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsControlPlanEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsControlPlanEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsControlPlanEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsControlPlanEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Control Plan is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsControlPlanEntity entity) {
