...
import kmsweb.configs.security.auditing.services.BpjsPcareProviderRayonisasiEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPcareProviderRayonisasiEntity.
 */
@Slf4j
@Component
public class BpjsPcareProviderRayonisasiEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPcareProviderRayonisasiEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPcareProviderRayonisasiEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPcareProviderRayonisasiEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPcareProviderRayonisasiEntityListener.auditService = auditService;
		BpjsPcareProviderRayonisasiEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPcareProviderRayonisasiEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPcareProviderRayonisasiEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPcareProviderRayonisasiEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPcareProviderRayonisasiEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPcareProviderRayonisasiEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPcareProviderRayonisasiEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPcareProviderRayonisasiEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPcareProviderRayonisasiEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPcareProviderRayonisasiEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Pcare Provider Rayonisasi is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPcareProviderRayonisasiEntity entity) {
	}
