...
import kmsweb.configs.security.auditing.services.PatientContactInfoEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientContactInfoEntity.
 */
@Slf4j
@Component
public class PatientContactInfoEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientContactInfoEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientContactInfoEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientContactInfoEntityListener.permissionEvaluator = permissionEvaluator;
		PatientContactInfoEntityListener.auditService = auditService;
		PatientContactInfoEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientContactInfoEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientContactInfoEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientContactInfoEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientContactInfoEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientContactInfoEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientContactInfoEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientContactInfoEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientContactInfoEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientContactInfoEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Contact Info is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientContactInfoEntity entity) {
