...
import kmsweb.configs.security.auditing.services.MerchantSurchargeAndFeeEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MerchantSurchargeAndFeeEntity.
 */
@Slf4j
@Component
public class MerchantSurchargeAndFeeEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MerchantSurchargeAndFeeEntityReadAuditService auditService;
	...
	public void init(
		...
		MerchantSurchargeAndFeeEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MerchantSurchargeAndFeeEntityListener.permissionEvaluator = permissionEvaluator;
		MerchantSurchargeAndFeeEntityListener.auditService = auditService;
		MerchantSurchargeAndFeeEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MerchantSurchargeAndFeeEntity entity) {
	...
	@PostPersist
	public void afterPersist(MerchantSurchargeAndFeeEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MerchantSurchargeAndFeeEntity entity) {
	...
	@PostRemove
	public void afterRemove(MerchantSurchargeAndFeeEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MerchantSurchargeAndFeeEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MerchantSurchargeAndFeeEntity entity) {
	...
	@PostLoad
	public void afterLoad(MerchantSurchargeAndFeeEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MerchantSurchargeAndFeeEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MerchantSurchargeAndFeeEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Merchant Surcharge and Fee is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MerchantSurchargeAndFeeEntity entity) {
		entity.unsetMerchant();
	}
