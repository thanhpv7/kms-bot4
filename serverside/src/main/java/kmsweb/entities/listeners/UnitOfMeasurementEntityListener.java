...
import kmsweb.configs.security.auditing.services.UnitOfMeasurementEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing UnitOfMeasurementEntity.
 */
@Slf4j
@Component
public class UnitOfMeasurementEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static UnitOfMeasurementEntityReadAuditService auditService;
	...
	public void init(
		...
		UnitOfMeasurementEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		UnitOfMeasurementEntityListener.permissionEvaluator = permissionEvaluator;
		UnitOfMeasurementEntityListener.auditService = auditService;
		UnitOfMeasurementEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(UnitOfMeasurementEntity entity) {
	...
	@PostPersist
	public void afterPersist(UnitOfMeasurementEntity entity) {
	...
	@PreRemove
	public void beforeRemove(UnitOfMeasurementEntity entity) {
	...
	@PostRemove
	public void afterRemove(UnitOfMeasurementEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(UnitOfMeasurementEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(UnitOfMeasurementEntity entity) {
	...
	@PostLoad
	public void afterLoad(UnitOfMeasurementEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the UnitOfMeasurementEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the UnitOfMeasurementEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Unit Of Measurement is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(UnitOfMeasurementEntity entity) {
		entity.unsetCatalogueUOM();
		entity.unsetCompoundPrescriptionDetails();
		entity.unsetCompoundPrescriptionItems();
		entity.unsetCompoundsUnit();
		entity.unsetMedicationCompounds();
		entity.unsetMedicationItems();
		entity.unsetPrescriptionItems();
	}
