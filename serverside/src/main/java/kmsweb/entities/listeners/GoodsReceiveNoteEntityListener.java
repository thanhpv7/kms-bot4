...
import kmsweb.configs.security.auditing.services.GoodsReceiveNoteEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing GoodsReceiveNoteEntity.
 */
@Slf4j
@Component
public class GoodsReceiveNoteEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static GoodsReceiveNoteEntityReadAuditService auditService;
	...
	public void init(
		...
		GoodsReceiveNoteEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		GoodsReceiveNoteEntityListener.permissionEvaluator = permissionEvaluator;
		GoodsReceiveNoteEntityListener.auditService = auditService;
		GoodsReceiveNoteEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(GoodsReceiveNoteEntity entity) {
	...
	@PostPersist
	public void afterPersist(GoodsReceiveNoteEntity entity) {
	...
	@PreRemove
	public void beforeRemove(GoodsReceiveNoteEntity entity) {
	...
	@PostRemove
	public void afterRemove(GoodsReceiveNoteEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(GoodsReceiveNoteEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(GoodsReceiveNoteEntity entity) {
	...
	@PostLoad
	public void afterLoad(GoodsReceiveNoteEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the GoodsReceiveNoteEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the GoodsReceiveNoteEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Goods Receive Note is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(GoodsReceiveNoteEntity entity) {
