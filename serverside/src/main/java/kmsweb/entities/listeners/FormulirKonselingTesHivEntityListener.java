...
import kmsweb.configs.security.auditing.services.FormulirKonselingTesHivEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing FormulirKonselingTesHivEntity.
 */
@Slf4j
@Component
public class FormulirKonselingTesHivEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static FormulirKonselingTesHivEntityReadAuditService auditService;
	...
	public void init(
		...
		FormulirKonselingTesHivEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		FormulirKonselingTesHivEntityListener.permissionEvaluator = permissionEvaluator;
		FormulirKonselingTesHivEntityListener.auditService = auditService;
		FormulirKonselingTesHivEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(FormulirKonselingTesHivEntity entity) {
	...
	@PostPersist
	public void afterPersist(FormulirKonselingTesHivEntity entity) {
	...
	@PreRemove
	public void beforeRemove(FormulirKonselingTesHivEntity entity) {
	...
	@PostRemove
	public void afterRemove(FormulirKonselingTesHivEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(FormulirKonselingTesHivEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(FormulirKonselingTesHivEntity entity) {
	...
	@PostLoad
	public void afterLoad(FormulirKonselingTesHivEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the FormulirKonselingTesHivEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the FormulirKonselingTesHivEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Formulir Konseling Tes Hiv is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(FormulirKonselingTesHivEntity entity) {
