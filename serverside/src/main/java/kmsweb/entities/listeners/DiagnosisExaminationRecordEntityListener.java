...
import kmsweb.configs.security.auditing.services.DiagnosisExaminationRecordEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DiagnosisExaminationRecordEntity.
 */
@Slf4j
@Component
public class DiagnosisExaminationRecordEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DiagnosisExaminationRecordEntityReadAuditService auditService;
	...
	public void init(
		...
		DiagnosisExaminationRecordEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DiagnosisExaminationRecordEntityListener.permissionEvaluator = permissionEvaluator;
		DiagnosisExaminationRecordEntityListener.auditService = auditService;
		DiagnosisExaminationRecordEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DiagnosisExaminationRecordEntity entity) {
	...
	@PostPersist
	public void afterPersist(DiagnosisExaminationRecordEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DiagnosisExaminationRecordEntity entity) {
	...
	@PostRemove
	public void afterRemove(DiagnosisExaminationRecordEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DiagnosisExaminationRecordEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DiagnosisExaminationRecordEntity entity) {
	...
	@PostLoad
	public void afterLoad(DiagnosisExaminationRecordEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DiagnosisExaminationRecordEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DiagnosisExaminationRecordEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Diagnosis Examination Record is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DiagnosisExaminationRecordEntity entity) {
		entity.unsetDiagnosticStaffExaminationSummary();
		entity.unsetSampleCollectionInformation();
