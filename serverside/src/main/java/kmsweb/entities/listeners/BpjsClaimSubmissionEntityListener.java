...
import kmsweb.configs.security.auditing.services.BpjsClaimSubmissionEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsClaimSubmissionEntity.
 */
@Slf4j
@Component
public class BpjsClaimSubmissionEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsClaimSubmissionEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsClaimSubmissionEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsClaimSubmissionEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsClaimSubmissionEntityListener.auditService = auditService;
		BpjsClaimSubmissionEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsClaimSubmissionEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsClaimSubmissionEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsClaimSubmissionEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsClaimSubmissionEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsClaimSubmissionEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsClaimSubmissionEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsClaimSubmissionEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsClaimSubmissionEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsClaimSubmissionEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Claim Submission is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsClaimSubmissionEntity entity) {
		entity.unsetBpjsDiagnose();
		entity.unsetBpjsDoctorMapping();
		entity.unsetBpjsHealthFacility();
		entity.unsetBpjsProcedure();
		entity.unsetBpjsServiceMapping();
		entity.unsetPatientGeneralInfo();
		entity.unsetPatientPaymentBPJS();
		entity.unsetBpjsDismissalConditions();
		entity.unsetBpjsDismissalMappings();
		entity.unsetBpjsTreatmentRooms();
	}
