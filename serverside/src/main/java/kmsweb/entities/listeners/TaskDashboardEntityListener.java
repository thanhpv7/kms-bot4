...
import kmsweb.configs.security.auditing.services.TaskDashboardEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TaskDashboardEntity.
 */
@Slf4j
@Component
public class TaskDashboardEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TaskDashboardEntityReadAuditService auditService;
	...
	public void init(
		...
		TaskDashboardEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TaskDashboardEntityListener.permissionEvaluator = permissionEvaluator;
		TaskDashboardEntityListener.auditService = auditService;
		TaskDashboardEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TaskDashboardEntity entity) {
	...
	@PostPersist
	public void afterPersist(TaskDashboardEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TaskDashboardEntity entity) {
	...
	@PostRemove
	public void afterRemove(TaskDashboardEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TaskDashboardEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TaskDashboardEntity entity) {
	...
	@PostLoad
	public void afterLoad(TaskDashboardEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TaskDashboardEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TaskDashboardEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Task Dashboard is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TaskDashboardEntity entity) {
	}
