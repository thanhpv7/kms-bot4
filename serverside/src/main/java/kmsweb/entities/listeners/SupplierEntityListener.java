...
import kmsweb.configs.security.auditing.services.SupplierEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SupplierEntity.
 */
@Slf4j
@Component
public class SupplierEntityListener {
	...
	private static SupplierEntityReadAuditService auditService;
	...
	public void init(
		...
		SupplierEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SupplierEntityListener.permissionEvaluator = permissionEvaluator;
		SupplierEntityListener.auditService = auditService;
		SupplierEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SupplierEntity entity) {
	...
	@PostPersist
	public void afterPersist(SupplierEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SupplierEntity entity) {
	...
	@PostRemove
	public void afterRemove(SupplierEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SupplierEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SupplierEntity entity) {
	...
	@PostLoad
	public void afterLoad(SupplierEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SupplierEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SupplierEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Supplier is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SupplierEntity entity) {
		entity.unsetGoodsReceiveNotes();
		entity.unsetPurchaseOrders();
		entity.unsetPurchaseRequisitions();
		entity.unsetReturnSuppliers();
		entity.unsetSuplierContactDetails();
		entity.unsetSupplierBankAccounts();
