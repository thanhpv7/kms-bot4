...
import kmsweb.configs.security.auditing.services.TransactionDetailTypeEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TransactionDetailTypeEntity.
 */
@Slf4j
@Component
public class TransactionDetailTypeEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TransactionDetailTypeEntityReadAuditService auditService;
	...
	public void init(
		...
		TransactionDetailTypeEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TransactionDetailTypeEntityListener.permissionEvaluator = permissionEvaluator;
		TransactionDetailTypeEntityListener.auditService = auditService;
		TransactionDetailTypeEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TransactionDetailTypeEntity entity) {
	...
	@PostPersist
	public void afterPersist(TransactionDetailTypeEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TransactionDetailTypeEntity entity) {
	...
	@PostRemove
	public void afterRemove(TransactionDetailTypeEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TransactionDetailTypeEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TransactionDetailTypeEntity entity) {
	...
	@PostLoad
	public void afterLoad(TransactionDetailTypeEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TransactionDetailTypeEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TransactionDetailTypeEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Transaction Detail Type is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TransactionDetailTypeEntity entity) {
		entity.unsetInventoryAdjustments();
		entity.unsetStockTransactionDetails();
		entity.unsetStockTransactionPerDateTypes();
		entity.unsetTransactionDetailTypeMappings();
	}
