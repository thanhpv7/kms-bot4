...
import kmsweb.configs.security.auditing.services.PostOperativeDetailsEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PostOperativeDetailsEntity.
 */
@Slf4j
@Component
public class PostOperativeDetailsEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PostOperativeDetailsEntityReadAuditService auditService;
	...
	public void init(
		...
		PostOperativeDetailsEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PostOperativeDetailsEntityListener.permissionEvaluator = permissionEvaluator;
		PostOperativeDetailsEntityListener.auditService = auditService;
		PostOperativeDetailsEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PostOperativeDetailsEntity entity) {
	...
	@PostPersist
	public void afterPersist(PostOperativeDetailsEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PostOperativeDetailsEntity entity) {
	...
	@PostRemove
	public void afterRemove(PostOperativeDetailsEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PostOperativeDetailsEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PostOperativeDetailsEntity entity) {
	...
	@PostLoad
	public void afterLoad(PostOperativeDetailsEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PostOperativeDetailsEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PostOperativeDetailsEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Post Operative Details is not permitted.");
		}
	}
