...
import kmsweb.configs.security.auditing.services.PatientDetailAddressEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientDetailAddressEntity.
 */
@Slf4j
@Component
public class PatientDetailAddressEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientDetailAddressEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientDetailAddressEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientDetailAddressEntityListener.permissionEvaluator = permissionEvaluator;
		PatientDetailAddressEntityListener.auditService = auditService;
		PatientDetailAddressEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientDetailAddressEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientDetailAddressEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientDetailAddressEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientDetailAddressEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientDetailAddressEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientDetailAddressEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientDetailAddressEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientDetailAddressEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientDetailAddressEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Detail Address is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientDetailAddressEntity entity) {
