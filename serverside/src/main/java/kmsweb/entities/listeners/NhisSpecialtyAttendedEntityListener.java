...
import kmsweb.configs.security.auditing.services.NhisSpecialtyAttendedEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisSpecialtyAttendedEntity.
 */
@Slf4j
@Component
public class NhisSpecialtyAttendedEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisSpecialtyAttendedEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisSpecialtyAttendedEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisSpecialtyAttendedEntityListener.permissionEvaluator = permissionEvaluator;
		NhisSpecialtyAttendedEntityListener.auditService = auditService;
		NhisSpecialtyAttendedEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisSpecialtyAttendedEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisSpecialtyAttendedEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisSpecialtyAttendedEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisSpecialtyAttendedEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisSpecialtyAttendedEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisSpecialtyAttendedEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisSpecialtyAttendedEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisSpecialtyAttendedEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisSpecialtyAttendedEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Specialty Attended is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisSpecialtyAttendedEntity entity) {
	}
