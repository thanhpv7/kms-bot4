...
import kmsweb.configs.security.auditing.services.NhisClaimInvestigationDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisClaimInvestigationDetailEntity.
 */
@Slf4j
@Component
public class NhisClaimInvestigationDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisClaimInvestigationDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisClaimInvestigationDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisClaimInvestigationDetailEntityListener.permissionEvaluator = permissionEvaluator;
		NhisClaimInvestigationDetailEntityListener.auditService = auditService;
		NhisClaimInvestigationDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisClaimInvestigationDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisClaimInvestigationDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisClaimInvestigationDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisClaimInvestigationDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisClaimInvestigationDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisClaimInvestigationDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisClaimInvestigationDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisClaimInvestigationDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisClaimInvestigationDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Claim Investigation Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisClaimInvestigationDetailEntity entity) {
	}
