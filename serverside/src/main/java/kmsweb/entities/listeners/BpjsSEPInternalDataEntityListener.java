...
import kmsweb.configs.security.auditing.services.BpjsSEPInternalDataEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsSEPInternalDataEntity.
 */
@Slf4j
@Component
public class BpjsSEPInternalDataEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsSEPInternalDataEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsSEPInternalDataEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsSEPInternalDataEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsSEPInternalDataEntityListener.auditService = auditService;
		BpjsSEPInternalDataEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsSEPInternalDataEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsSEPInternalDataEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsSEPInternalDataEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsSEPInternalDataEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsSEPInternalDataEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsSEPInternalDataEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsSEPInternalDataEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsSEPInternalDataEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsSEPInternalDataEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS SEP Internal Data is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsSEPInternalDataEntity entity) {
	}
