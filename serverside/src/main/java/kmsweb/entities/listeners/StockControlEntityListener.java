...
import kmsweb.configs.security.auditing.services.StockControlEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing StockControlEntity.
 */
@Slf4j
@Component
public class StockControlEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static StockControlEntityReadAuditService auditService;
	...
	public void init(
		...
		StockControlEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		StockControlEntityListener.permissionEvaluator = permissionEvaluator;
		StockControlEntityListener.auditService = auditService;
		StockControlEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(StockControlEntity entity) {
	...
	@PostPersist
	public void afterPersist(StockControlEntity entity) {
	...
	@PreRemove
	public void beforeRemove(StockControlEntity entity) {
	...
	@PostRemove
	public void afterRemove(StockControlEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(StockControlEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(StockControlEntity entity) {
	...
	@PostLoad
	public void afterLoad(StockControlEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the StockControlEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the StockControlEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Stock Control is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(StockControlEntity entity) {
