...
import kmsweb.configs.security.auditing.services.RetailPharmacyStockDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing RetailPharmacyStockDetailEntity.
 */
@Slf4j
@Component
public class RetailPharmacyStockDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static RetailPharmacyStockDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		RetailPharmacyStockDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		RetailPharmacyStockDetailEntityListener.permissionEvaluator = permissionEvaluator;
		RetailPharmacyStockDetailEntityListener.auditService = auditService;
		RetailPharmacyStockDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(RetailPharmacyStockDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(RetailPharmacyStockDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(RetailPharmacyStockDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(RetailPharmacyStockDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(RetailPharmacyStockDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(RetailPharmacyStockDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(RetailPharmacyStockDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the RetailPharmacyStockDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the RetailPharmacyStockDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Retail Pharmacy Stock Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(RetailPharmacyStockDetailEntity entity) {
		entity.unsetRetailPharmacy();
