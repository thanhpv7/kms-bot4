...
import kmsweb.configs.security.auditing.services.NhisInvestigationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisInvestigationEntity.
 */
@Slf4j
@Component
public class NhisInvestigationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisInvestigationEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisInvestigationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisInvestigationEntityListener.permissionEvaluator = permissionEvaluator;
		NhisInvestigationEntityListener.auditService = auditService;
		NhisInvestigationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisInvestigationEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisInvestigationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisInvestigationEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisInvestigationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisInvestigationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisInvestigationEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisInvestigationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisInvestigationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisInvestigationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Investigation is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisInvestigationEntity entity) {
	}
