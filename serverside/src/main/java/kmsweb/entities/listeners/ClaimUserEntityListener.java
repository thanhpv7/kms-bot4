...
import kmsweb.configs.security.auditing.services.ClaimUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ClaimUserEntity.
 */
@Slf4j
@Component
public class ClaimUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ClaimUserEntityReadAuditService auditService;
	...
	public void init(
		...
		ClaimUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ClaimUserEntityListener.permissionEvaluator = permissionEvaluator;
		ClaimUserEntityListener.auditService = auditService;
		ClaimUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ClaimUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(ClaimUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ClaimUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(ClaimUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ClaimUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ClaimUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(ClaimUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ClaimUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ClaimUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Claim User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ClaimUserEntity entity) {
		entity.unsetStaff();
	}
