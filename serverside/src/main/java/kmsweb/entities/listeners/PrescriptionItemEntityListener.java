...
import kmsweb.configs.security.auditing.services.PrescriptionItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PrescriptionItemEntity.
 */
@Slf4j
@Component
public class PrescriptionItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PrescriptionItemEntityReadAuditService auditService;
	...
	public void init(
		...
		PrescriptionItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PrescriptionItemEntityListener.permissionEvaluator = permissionEvaluator;
		PrescriptionItemEntityListener.auditService = auditService;
		PrescriptionItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PrescriptionItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(PrescriptionItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PrescriptionItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(PrescriptionItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PrescriptionItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PrescriptionItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(PrescriptionItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PrescriptionItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PrescriptionItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Prescription Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PrescriptionItemEntity entity) {
		entity.unsetCompound();
		entity.unsetStockCatalogueId();
