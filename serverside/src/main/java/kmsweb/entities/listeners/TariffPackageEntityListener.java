...
import kmsweb.configs.security.auditing.services.TariffPackageEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TariffPackageEntity.
 */
@Slf4j
@Component
public class TariffPackageEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TariffPackageEntityReadAuditService auditService;
	...
	public void init(
		...
		TariffPackageEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TariffPackageEntityListener.permissionEvaluator = permissionEvaluator;
		TariffPackageEntityListener.auditService = auditService;
		TariffPackageEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TariffPackageEntity entity) {
	...
	@PostPersist
	public void afterPersist(TariffPackageEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TariffPackageEntity entity) {
	...
	@PostRemove
	public void afterRemove(TariffPackageEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TariffPackageEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TariffPackageEntity entity) {
	...
	@PostLoad
	public void afterLoad(TariffPackageEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TariffPackageEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TariffPackageEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Tariff Package is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TariffPackageEntity entity) {
		entity.unsetServiceItemAssignments();
		entity.unsetTariffPackageItems();
		entity.unsetTariffPackageServiceAssignments();
	}
