...
import kmsweb.configs.security.auditing.services.SepEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SepEntity.
 */
@Slf4j
@Component
public class SepEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SepEntityReadAuditService auditService;
	...
	public void init(
		...
		SepEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SepEntityListener.permissionEvaluator = permissionEvaluator;
		SepEntityListener.auditService = auditService;
		SepEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SepEntity entity) {
	...
	@PostPersist
	public void afterPersist(SepEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SepEntity entity) {
	...
	@PostRemove
	public void afterRemove(SepEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SepEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SepEntity entity) {
	...
	@PostLoad
	public void afterLoad(SepEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SepEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SepEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to SEP is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SepEntity entity) {
