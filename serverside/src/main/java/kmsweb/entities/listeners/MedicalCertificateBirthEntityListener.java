...
import kmsweb.configs.security.auditing.services.MedicalCertificateBirthEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalCertificateBirthEntity.
 */
@Slf4j
@Component
public class MedicalCertificateBirthEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalCertificateBirthEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalCertificateBirthEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalCertificateBirthEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalCertificateBirthEntityListener.auditService = auditService;
		MedicalCertificateBirthEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalCertificateBirthEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalCertificateBirthEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalCertificateBirthEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalCertificateBirthEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalCertificateBirthEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalCertificateBirthEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalCertificateBirthEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalCertificateBirthEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalCertificateBirthEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Certificate Birth is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalCertificateBirthEntity entity) {
