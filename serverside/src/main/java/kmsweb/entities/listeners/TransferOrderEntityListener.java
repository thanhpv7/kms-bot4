...
import kmsweb.configs.security.auditing.services.TransferOrderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TransferOrderEntity.
 */
@Slf4j
@Component
public class TransferOrderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TransferOrderEntityReadAuditService auditService;
	...
	public void init(
		...
		TransferOrderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TransferOrderEntityListener.permissionEvaluator = permissionEvaluator;
		TransferOrderEntityListener.auditService = auditService;
		TransferOrderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TransferOrderEntity entity) {
	...
	@PostPersist
	public void afterPersist(TransferOrderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TransferOrderEntity entity) {
	...
	@PostRemove
	public void afterRemove(TransferOrderEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TransferOrderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TransferOrderEntity entity) {
	...
	@PostLoad
	public void afterLoad(TransferOrderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TransferOrderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TransferOrderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Transfer Order is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TransferOrderEntity entity) {
		entity.unsetDestinationWarehouse();
		entity.unsetSourceWarehouse();
