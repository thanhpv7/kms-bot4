...
import kmsweb.configs.security.auditing.services.NurseUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NurseUserEntity.
 */
@Slf4j
@Component
public class NurseUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NurseUserEntityReadAuditService auditService;
	...
	public void init(
		...
		NurseUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NurseUserEntityListener.permissionEvaluator = permissionEvaluator;
		NurseUserEntityListener.auditService = auditService;
		NurseUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NurseUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(NurseUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NurseUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(NurseUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NurseUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NurseUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(NurseUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NurseUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NurseUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Nurse User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NurseUserEntity entity) {
		entity.unsetStaff();
	}
