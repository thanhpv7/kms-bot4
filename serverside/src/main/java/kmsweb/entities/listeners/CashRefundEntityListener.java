...
import kmsweb.configs.security.auditing.services.CashRefundEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing CashRefundEntity.
 */
@Slf4j
@Component
public class CashRefundEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static CashRefundEntityReadAuditService auditService;
	...
	public void init(
		...
		CashRefundEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		CashRefundEntityListener.permissionEvaluator = permissionEvaluator;
		CashRefundEntityListener.auditService = auditService;
		CashRefundEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(CashRefundEntity entity) {
	...
	@PostPersist
	public void afterPersist(CashRefundEntity entity) {
	...
	@PreRemove
	public void beforeRemove(CashRefundEntity entity) {
	...
	@PostRemove
	public void afterRemove(CashRefundEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(CashRefundEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(CashRefundEntity entity) {
	...
	@PostLoad
	public void afterLoad(CashRefundEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the CashRefundEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the CashRefundEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Cash Refund is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(CashRefundEntity entity) {
