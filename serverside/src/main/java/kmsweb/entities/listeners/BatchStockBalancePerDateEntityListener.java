...
import kmsweb.configs.security.auditing.services.BatchStockBalancePerDateEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BatchStockBalancePerDateEntity.
 */
@Slf4j
@Component
public class BatchStockBalancePerDateEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BatchStockBalancePerDateEntityReadAuditService auditService;
	...
	public void init(
		...
		BatchStockBalancePerDateEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BatchStockBalancePerDateEntityListener.permissionEvaluator = permissionEvaluator;
		BatchStockBalancePerDateEntityListener.auditService = auditService;
		BatchStockBalancePerDateEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BatchStockBalancePerDateEntity entity) {
	...
	@PostPersist
	public void afterPersist(BatchStockBalancePerDateEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BatchStockBalancePerDateEntity entity) {
	...
	@PostRemove
	public void afterRemove(BatchStockBalancePerDateEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BatchStockBalancePerDateEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BatchStockBalancePerDateEntity entity) {
	...
	@PostLoad
	public void afterLoad(BatchStockBalancePerDateEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BatchStockBalancePerDateEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BatchStockBalancePerDateEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Batch Stock Balance Per Date is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BatchStockBalancePerDateEntity entity) {
