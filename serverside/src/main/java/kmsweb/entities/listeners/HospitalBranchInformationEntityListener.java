...
import kmsweb.configs.security.auditing.services.HospitalBranchInformationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing HospitalBranchInformationEntity.
 */
@Slf4j
@Component
public class HospitalBranchInformationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static HospitalBranchInformationEntityReadAuditService auditService;
	...
	public void init(
		...
		HospitalBranchInformationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		HospitalBranchInformationEntityListener.permissionEvaluator = permissionEvaluator;
		HospitalBranchInformationEntityListener.auditService = auditService;
		HospitalBranchInformationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(HospitalBranchInformationEntity entity) {
	...
	@PostPersist
	public void afterPersist(HospitalBranchInformationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(HospitalBranchInformationEntity entity) {
	...
	@PostRemove
	public void afterRemove(HospitalBranchInformationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(HospitalBranchInformationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(HospitalBranchInformationEntity entity) {
	...
	@PostLoad
	public void afterLoad(HospitalBranchInformationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the HospitalBranchInformationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the HospitalBranchInformationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Hospital Branch Information is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(HospitalBranchInformationEntity entity) {
		entity.unsetHospitalGroup();
		entity.unsetStaff();
	}
