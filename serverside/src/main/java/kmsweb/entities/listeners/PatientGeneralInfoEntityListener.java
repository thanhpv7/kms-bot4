...
import kmsweb.configs.security.auditing.services.PatientGeneralInfoEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientGeneralInfoEntity.
 */
@Slf4j
@Component
public class PatientGeneralInfoEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientGeneralInfoEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientGeneralInfoEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientGeneralInfoEntityListener.permissionEvaluator = permissionEvaluator;
		PatientGeneralInfoEntityListener.auditService = auditService;
		PatientGeneralInfoEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientGeneralInfoEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientGeneralInfoEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientGeneralInfoEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientGeneralInfoEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientGeneralInfoEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientGeneralInfoEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientGeneralInfoEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientGeneralInfoEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientGeneralInfoEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient General Info is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientGeneralInfoEntity entity) {
		entity.unsetPatientContactInformation();
		entity.unsetPatientDetailAddress();
		entity.unsetPatientDetail();
		entity.unsetPatientEmergencyContactDetail();
		entity.unsetPatientEmploymentDetail();
		entity.unsetPatientPaymentBPJS();
		entity.unsetPatientPaymentTypeInsurance();
		entity.unsetPatientPaymentTypeOthers();
		entity.unsetPatientPersonalInformation();
		entity.unsetPatientSelfPaying();
		entity.unsetBpjsClaimSubmissions();
		entity.unsetCashRefunds();
		entity.unsetInformedConsents();
		entity.unsetInvoiceReceipts();
		entity.unsetPatientCaseHistories();
		entity.unsetPatientConsents();
		entity.unsetPatientFinalizeProfiles();
