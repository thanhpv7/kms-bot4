...
import kmsweb.configs.security.auditing.services.InsuranceProviderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InsuranceProviderEntity.
 */
@Slf4j
@Component
public class InsuranceProviderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InsuranceProviderEntityReadAuditService auditService;
	...
	public void init(
		...
		InsuranceProviderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InsuranceProviderEntityListener.permissionEvaluator = permissionEvaluator;
		InsuranceProviderEntityListener.auditService = auditService;
		InsuranceProviderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InsuranceProviderEntity entity) {
	...
	@PostPersist
	public void afterPersist(InsuranceProviderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InsuranceProviderEntity entity) {
	...
	@PostRemove
	public void afterRemove(InsuranceProviderEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InsuranceProviderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InsuranceProviderEntity entity) {
	...
	@PostLoad
	public void afterLoad(InsuranceProviderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InsuranceProviderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InsuranceProviderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Insurance Provider is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InsuranceProviderEntity entity) {
		entity.unsetInsuranceBenefitPlans();
	}
