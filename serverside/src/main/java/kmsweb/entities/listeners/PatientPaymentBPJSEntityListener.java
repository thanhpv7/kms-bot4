...
import kmsweb.configs.security.auditing.services.PatientPaymentBPJSEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientPaymentBPJSEntity.
 */
@Slf4j
@Component
public class PatientPaymentBPJSEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientPaymentBPJSEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientPaymentBPJSEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientPaymentBPJSEntityListener.permissionEvaluator = permissionEvaluator;
		PatientPaymentBPJSEntityListener.auditService = auditService;
		PatientPaymentBPJSEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientPaymentBPJSEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientPaymentBPJSEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientPaymentBPJSEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientPaymentBPJSEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientPaymentBPJSEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientPaymentBPJSEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientPaymentBPJSEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientPaymentBPJSEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientPaymentBPJSEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Payment BPJS is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientPaymentBPJSEntity entity) {
		entity.unsetPatientGeneralInfo();
		entity.unsetBpjsAccidentMasters();
		entity.unsetBpjsClaimSubmissions();
		entity.unsetBpjsHospitalizationPlans();
		entity.unsetBpjsJasaRaharjas();
		entity.unsetBpjsPatientReferrals();
		entity.unsetBpjsSEPSubmissions();
		entity.unsetBpjsSEPs();
	}
