...
import kmsweb.configs.security.auditing.services.InventoryAdjustmentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InventoryAdjustmentEntity.
 */
@Slf4j
@Component
public class InventoryAdjustmentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InventoryAdjustmentEntityReadAuditService auditService;
	...
	public void init(
		...
		InventoryAdjustmentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InventoryAdjustmentEntityListener.permissionEvaluator = permissionEvaluator;
		InventoryAdjustmentEntityListener.auditService = auditService;
		InventoryAdjustmentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InventoryAdjustmentEntity entity) {
	...
	@PostPersist
	public void afterPersist(InventoryAdjustmentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InventoryAdjustmentEntity entity) {
	...
	@PostRemove
	public void afterRemove(InventoryAdjustmentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InventoryAdjustmentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InventoryAdjustmentEntity entity) {
	...
	@PostLoad
	public void afterLoad(InventoryAdjustmentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InventoryAdjustmentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InventoryAdjustmentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Inventory Adjustment is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InventoryAdjustmentEntity entity) {
