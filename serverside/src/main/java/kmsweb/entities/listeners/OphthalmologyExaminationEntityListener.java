...
import kmsweb.configs.security.auditing.services.OphthalmologyExaminationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing OphthalmologyExaminationEntity.
 */
@Slf4j
@Component
public class OphthalmologyExaminationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static OphthalmologyExaminationEntityReadAuditService auditService;
	...
	public void init(
		...
		OphthalmologyExaminationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		OphthalmologyExaminationEntityListener.permissionEvaluator = permissionEvaluator;
		OphthalmologyExaminationEntityListener.auditService = auditService;
		OphthalmologyExaminationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(OphthalmologyExaminationEntity entity) {
	...
	@PostPersist
	public void afterPersist(OphthalmologyExaminationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(OphthalmologyExaminationEntity entity) {
	...
	@PostRemove
	public void afterRemove(OphthalmologyExaminationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(OphthalmologyExaminationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(OphthalmologyExaminationEntity entity) {
	...
	@PostLoad
	public void afterLoad(OphthalmologyExaminationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the OphthalmologyExaminationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the OphthalmologyExaminationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Ophthalmology Examination is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(OphthalmologyExaminationEntity entity) {
