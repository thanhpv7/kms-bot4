...
import kmsweb.configs.security.auditing.services.CashAllocationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing CashAllocationEntity.
 */
@Slf4j
@Component
public class CashAllocationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static CashAllocationEntityReadAuditService auditService;
	...
	public void init(
		...
		CashAllocationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		CashAllocationEntityListener.permissionEvaluator = permissionEvaluator;
		CashAllocationEntityListener.auditService = auditService;
		CashAllocationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(CashAllocationEntity entity) {
	...
	@PostPersist
	public void afterPersist(CashAllocationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(CashAllocationEntity entity) {
	...
	@PostRemove
	public void afterRemove(CashAllocationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(CashAllocationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(CashAllocationEntity entity) {
	...
	@PostLoad
	public void afterLoad(CashAllocationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the CashAllocationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the CashAllocationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Cash Allocation is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(CashAllocationEntity entity) {
	}
