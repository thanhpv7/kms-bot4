...
import kmsweb.configs.security.auditing.services.OdontogramExaminationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing OdontogramExaminationEntity.
 */
@Slf4j
@Component
public class OdontogramExaminationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static OdontogramExaminationEntityReadAuditService auditService;
	...
	public void init(
		...
		OdontogramExaminationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		OdontogramExaminationEntityListener.permissionEvaluator = permissionEvaluator;
		OdontogramExaminationEntityListener.auditService = auditService;
		OdontogramExaminationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(OdontogramExaminationEntity entity) {
	...
	@PostPersist
	public void afterPersist(OdontogramExaminationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(OdontogramExaminationEntity entity) {
	...
	@PostRemove
	public void afterRemove(OdontogramExaminationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(OdontogramExaminationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(OdontogramExaminationEntity entity) {
	...
	@PostLoad
	public void afterLoad(OdontogramExaminationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the OdontogramExaminationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the OdontogramExaminationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Odontogram Examination is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(OdontogramExaminationEntity entity) {
