...
import kmsweb.configs.security.auditing.services.BpjsPcareDPHOEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPcareDPHOEntity.
 */
@Slf4j
@Component
public class BpjsPcareDPHOEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPcareDPHOEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPcareDPHOEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPcareDPHOEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPcareDPHOEntityListener.auditService = auditService;
		BpjsPcareDPHOEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPcareDPHOEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPcareDPHOEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPcareDPHOEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPcareDPHOEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPcareDPHOEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPcareDPHOEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPcareDPHOEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPcareDPHOEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPcareDPHOEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Pcare DPHO is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPcareDPHOEntity entity) {
