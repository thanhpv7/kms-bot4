...
import kmsweb.configs.security.auditing.services.BpjsGeneralEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsGeneralEntity.
 */
@Slf4j
@Component
public class BpjsGeneralEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsGeneralEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsGeneralEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsGeneralEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsGeneralEntityListener.auditService = auditService;
		BpjsGeneralEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsGeneralEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsGeneralEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsGeneralEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsGeneralEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsGeneralEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsGeneralEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsGeneralEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsGeneralEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsGeneralEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS General is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsGeneralEntity entity) {
	}
