...
import kmsweb.configs.security.auditing.services.BpjsSEPINACBGIntegrationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsSEPINACBGIntegrationEntity.
 */
@Slf4j
@Component
public class BpjsSEPINACBGIntegrationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsSEPINACBGIntegrationEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsSEPINACBGIntegrationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsSEPINACBGIntegrationEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsSEPINACBGIntegrationEntityListener.auditService = auditService;
		BpjsSEPINACBGIntegrationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsSEPINACBGIntegrationEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsSEPINACBGIntegrationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsSEPINACBGIntegrationEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsSEPINACBGIntegrationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsSEPINACBGIntegrationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsSEPINACBGIntegrationEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsSEPINACBGIntegrationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsSEPINACBGIntegrationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsSEPINACBGIntegrationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS SEP INACBG Integration is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsSEPINACBGIntegrationEntity entity) {
	}
