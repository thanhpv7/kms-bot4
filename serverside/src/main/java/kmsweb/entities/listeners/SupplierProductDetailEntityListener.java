...
import kmsweb.configs.security.auditing.services.SupplierProductDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SupplierProductDetailEntity.
 */
@Slf4j
@Component
public class SupplierProductDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SupplierProductDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		SupplierProductDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SupplierProductDetailEntityListener.permissionEvaluator = permissionEvaluator;
		SupplierProductDetailEntityListener.auditService = auditService;
		SupplierProductDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SupplierProductDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(SupplierProductDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SupplierProductDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(SupplierProductDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SupplierProductDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SupplierProductDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(SupplierProductDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SupplierProductDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SupplierProductDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Supplier Product Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SupplierProductDetailEntity entity) {
