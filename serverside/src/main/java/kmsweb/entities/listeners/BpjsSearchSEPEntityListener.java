...
import kmsweb.configs.security.auditing.services.BpjsSearchSEPEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsSearchSEPEntity.
 */
@Slf4j
@Component
public class BpjsSearchSEPEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsSearchSEPEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsSearchSEPEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsSearchSEPEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsSearchSEPEntityListener.auditService = auditService;
		BpjsSearchSEPEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsSearchSEPEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsSearchSEPEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsSearchSEPEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsSearchSEPEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsSearchSEPEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsSearchSEPEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsSearchSEPEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsSearchSEPEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsSearchSEPEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Search SEP is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsSearchSEPEntity entity) {
	}
