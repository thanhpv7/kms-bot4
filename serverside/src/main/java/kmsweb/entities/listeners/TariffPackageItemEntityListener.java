...
import kmsweb.configs.security.auditing.services.TariffPackageItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TariffPackageItemEntity.
 */
@Slf4j
@Component
public class TariffPackageItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TariffPackageItemEntityReadAuditService auditService;
	...
	public void init(
		...
		TariffPackageItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TariffPackageItemEntityListener.permissionEvaluator = permissionEvaluator;
		TariffPackageItemEntityListener.auditService = auditService;
		TariffPackageItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TariffPackageItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(TariffPackageItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TariffPackageItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(TariffPackageItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TariffPackageItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TariffPackageItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(TariffPackageItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TariffPackageItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TariffPackageItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Tariff Package Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TariffPackageItemEntity entity) {
