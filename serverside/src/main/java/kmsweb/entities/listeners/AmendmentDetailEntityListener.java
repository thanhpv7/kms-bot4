...
import kmsweb.configs.security.auditing.services.AmendmentDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing AmendmentDetailEntity.
 */
@Slf4j
@Component
public class AmendmentDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static AmendmentDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		AmendmentDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		AmendmentDetailEntityListener.permissionEvaluator = permissionEvaluator;
		AmendmentDetailEntityListener.auditService = auditService;
		AmendmentDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(AmendmentDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(AmendmentDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(AmendmentDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(AmendmentDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(AmendmentDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(AmendmentDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(AmendmentDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the AmendmentDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the AmendmentDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Amendment Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(AmendmentDetailEntity entity) {
