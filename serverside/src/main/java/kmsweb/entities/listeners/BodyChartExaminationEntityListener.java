...
import kmsweb.configs.security.auditing.services.BodyChartExaminationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BodyChartExaminationEntity.
 */
@Slf4j
@Component
public class BodyChartExaminationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BodyChartExaminationEntityReadAuditService auditService;
	...
	public void init(
		...
		BodyChartExaminationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BodyChartExaminationEntityListener.permissionEvaluator = permissionEvaluator;
		BodyChartExaminationEntityListener.auditService = auditService;
		BodyChartExaminationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BodyChartExaminationEntity entity) {
	...
	@PostPersist
	public void afterPersist(BodyChartExaminationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BodyChartExaminationEntity entity) {
	...
	@PostRemove
	public void afterRemove(BodyChartExaminationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BodyChartExaminationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BodyChartExaminationEntity entity) {
	...
	@PostLoad
	public void afterLoad(BodyChartExaminationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BodyChartExaminationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BodyChartExaminationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Body Chart Examination is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BodyChartExaminationEntity entity) {
