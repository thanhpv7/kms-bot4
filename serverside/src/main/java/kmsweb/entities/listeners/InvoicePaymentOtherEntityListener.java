...
import kmsweb.configs.security.auditing.services.InvoicePaymentOtherEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InvoicePaymentOtherEntity.
 */
@Slf4j
@Component
public class InvoicePaymentOtherEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InvoicePaymentOtherEntityReadAuditService auditService;
	...
	public void init(
		...
		InvoicePaymentOtherEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InvoicePaymentOtherEntityListener.permissionEvaluator = permissionEvaluator;
		InvoicePaymentOtherEntityListener.auditService = auditService;
		InvoicePaymentOtherEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InvoicePaymentOtherEntity entity) {
	...
	@PostPersist
	public void afterPersist(InvoicePaymentOtherEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InvoicePaymentOtherEntity entity) {
	...
	@PostRemove
	public void afterRemove(InvoicePaymentOtherEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InvoicePaymentOtherEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InvoicePaymentOtherEntity entity) {
	...
	@PostLoad
	public void afterLoad(InvoicePaymentOtherEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InvoicePaymentOtherEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InvoicePaymentOtherEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Invoice Payment Other is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InvoicePaymentOtherEntity entity) {
		entity.unsetInvoice();
	}
