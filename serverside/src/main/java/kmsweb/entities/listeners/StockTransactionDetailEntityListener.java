...
import kmsweb.configs.security.auditing.services.StockTransactionDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing StockTransactionDetailEntity.
 */
@Slf4j
@Component
public class StockTransactionDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static StockTransactionDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		StockTransactionDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		StockTransactionDetailEntityListener.permissionEvaluator = permissionEvaluator;
		StockTransactionDetailEntityListener.auditService = auditService;
		StockTransactionDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(StockTransactionDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(StockTransactionDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(StockTransactionDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(StockTransactionDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(StockTransactionDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(StockTransactionDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(StockTransactionDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the StockTransactionDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the StockTransactionDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Stock Transaction Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(StockTransactionDetailEntity entity) {
