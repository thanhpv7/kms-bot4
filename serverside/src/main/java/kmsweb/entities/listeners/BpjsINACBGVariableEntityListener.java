...
import kmsweb.configs.security.auditing.services.BpjsINACBGVariableEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsINACBGVariableEntity.
 */
@Slf4j
@Component
public class BpjsINACBGVariableEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsINACBGVariableEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsINACBGVariableEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsINACBGVariableEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsINACBGVariableEntityListener.auditService = auditService;
		BpjsINACBGVariableEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsINACBGVariableEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsINACBGVariableEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsINACBGVariableEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsINACBGVariableEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsINACBGVariableEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsINACBGVariableEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsINACBGVariableEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsINACBGVariableEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsINACBGVariableEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS INACBG Variable is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsINACBGVariableEntity entity) {
	}
