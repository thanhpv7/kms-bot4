...
import kmsweb.configs.security.auditing.services.TariffDefinitionOtherComponentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TariffDefinitionOtherComponentEntity.
 */
@Slf4j
@Component
public class TariffDefinitionOtherComponentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TariffDefinitionOtherComponentEntityReadAuditService auditService;
	...
	public void init(
		...
		TariffDefinitionOtherComponentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TariffDefinitionOtherComponentEntityListener.permissionEvaluator = permissionEvaluator;
		TariffDefinitionOtherComponentEntityListener.auditService = auditService;
		TariffDefinitionOtherComponentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TariffDefinitionOtherComponentEntity entity) {
	...
	@PostPersist
	public void afterPersist(TariffDefinitionOtherComponentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TariffDefinitionOtherComponentEntity entity) {
	...
	@PostRemove
	public void afterRemove(TariffDefinitionOtherComponentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TariffDefinitionOtherComponentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TariffDefinitionOtherComponentEntity entity) {
	...
	@PostLoad
	public void afterLoad(TariffDefinitionOtherComponentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TariffDefinitionOtherComponentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TariffDefinitionOtherComponentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Tariff Definition Other Component is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TariffDefinitionOtherComponentEntity entity) {
		entity.unsetPriceComponent();
		entity.unsetPriceDefinition();
	}
