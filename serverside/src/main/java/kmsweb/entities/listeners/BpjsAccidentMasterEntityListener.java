...
import kmsweb.configs.security.auditing.services.BpjsAccidentMasterEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsAccidentMasterEntity.
 */
@Slf4j
@Component
public class BpjsAccidentMasterEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsAccidentMasterEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsAccidentMasterEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsAccidentMasterEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsAccidentMasterEntityListener.auditService = auditService;
		BpjsAccidentMasterEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsAccidentMasterEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsAccidentMasterEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsAccidentMasterEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsAccidentMasterEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsAccidentMasterEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsAccidentMasterEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsAccidentMasterEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsAccidentMasterEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsAccidentMasterEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Accident Master is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsAccidentMasterEntity entity) {
