...
import kmsweb.configs.security.auditing.services.MedicalFeeItemComponentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalFeeItemComponentEntity.
 */
@Slf4j
@Component
public class MedicalFeeItemComponentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalFeeItemComponentEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalFeeItemComponentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalFeeItemComponentEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalFeeItemComponentEntityListener.auditService = auditService;
		MedicalFeeItemComponentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalFeeItemComponentEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalFeeItemComponentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalFeeItemComponentEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalFeeItemComponentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalFeeItemComponentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalFeeItemComponentEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalFeeItemComponentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalFeeItemComponentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalFeeItemComponentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Fee Item Component is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalFeeItemComponentEntity entity) {
		entity.unsetMedicalFee();
