...
import kmsweb.configs.security.auditing.services.InvoiceItemComponentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InvoiceItemComponentEntity.
 */
@Slf4j
@Component
public class InvoiceItemComponentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InvoiceItemComponentEntityReadAuditService auditService;
	...
	public void init(
		...
		InvoiceItemComponentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InvoiceItemComponentEntityListener.permissionEvaluator = permissionEvaluator;
		InvoiceItemComponentEntityListener.auditService = auditService;
		InvoiceItemComponentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InvoiceItemComponentEntity entity) {
	...
	@PostPersist
	public void afterPersist(InvoiceItemComponentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InvoiceItemComponentEntity entity) {
	...
	@PostRemove
	public void afterRemove(InvoiceItemComponentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InvoiceItemComponentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InvoiceItemComponentEntity entity) {
	...
	@PostLoad
	public void afterLoad(InvoiceItemComponentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InvoiceItemComponentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InvoiceItemComponentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Invoice Item Component is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InvoiceItemComponentEntity entity) {
		entity.unsetInvoiceItem();
