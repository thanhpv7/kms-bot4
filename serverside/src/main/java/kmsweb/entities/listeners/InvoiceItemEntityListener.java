...
import kmsweb.configs.security.auditing.services.InvoiceItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InvoiceItemEntity.
 */
@Slf4j
@Component
public class InvoiceItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InvoiceItemEntityReadAuditService auditService;
	...
	public void init(
		...
		InvoiceItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InvoiceItemEntityListener.permissionEvaluator = permissionEvaluator;
		InvoiceItemEntityListener.auditService = auditService;
		InvoiceItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InvoiceItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(InvoiceItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InvoiceItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(InvoiceItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InvoiceItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InvoiceItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(InvoiceItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InvoiceItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InvoiceItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Invoice Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InvoiceItemEntity entity) {
		entity.unsetInvoice();
		entity.unsetMedicalFee();
		entity.unsetMedicationHeader();
		entity.unsetPrescription();
		entity.unsetRetailPharmacyInvoice();
		entity.unsetServiceNumber();
		entity.unsetTariffSchema();
		entity.unsetInvoiceItemComponents();
	}
