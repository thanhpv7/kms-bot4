...
import kmsweb.configs.security.auditing.services.SupplierBankAccountEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SupplierBankAccountEntity.
 */
@Slf4j
@Component
public class SupplierBankAccountEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SupplierBankAccountEntityReadAuditService auditService;
	...
	public void init(
		...
		SupplierBankAccountEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SupplierBankAccountEntityListener.permissionEvaluator = permissionEvaluator;
		SupplierBankAccountEntityListener.auditService = auditService;
		SupplierBankAccountEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SupplierBankAccountEntity entity) {
	...
	@PostPersist
	public void afterPersist(SupplierBankAccountEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SupplierBankAccountEntity entity) {
	...
	@PostRemove
	public void afterRemove(SupplierBankAccountEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SupplierBankAccountEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SupplierBankAccountEntity entity) {
	...
	@PostLoad
	public void afterLoad(SupplierBankAccountEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SupplierBankAccountEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SupplierBankAccountEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Supplier Bank Account is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SupplierBankAccountEntity entity) {
