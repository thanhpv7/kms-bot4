...
import kmsweb.configs.security.auditing.services.LibraryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing LibraryEntity.
 */
@Slf4j
@Component
public class LibraryEntityListener {
	...
	private static LibraryEntityReadAuditService auditService;
	...
	public void init(
		...
		LibraryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		LibraryEntityListener.permissionEvaluator = permissionEvaluator;
		LibraryEntityListener.auditService = auditService;
		LibraryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(LibraryEntity entity) {
	...
	@PostPersist
	public void afterPersist(LibraryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(LibraryEntity entity) {
	...
	@PostRemove
	public void afterRemove(LibraryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(LibraryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(LibraryEntity entity) {
	...
	@PostLoad
	public void afterLoad(LibraryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the LibraryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the LibraryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Library is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(LibraryEntity entity) {
	}
