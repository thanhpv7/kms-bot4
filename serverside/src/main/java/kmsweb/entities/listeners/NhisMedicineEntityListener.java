...
import kmsweb.configs.security.auditing.services.NhisMedicineEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisMedicineEntity.
 */
@Slf4j
@Component
public class NhisMedicineEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisMedicineEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisMedicineEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisMedicineEntityListener.permissionEvaluator = permissionEvaluator;
		NhisMedicineEntityListener.auditService = auditService;
		NhisMedicineEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisMedicineEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisMedicineEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisMedicineEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisMedicineEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisMedicineEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisMedicineEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisMedicineEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisMedicineEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisMedicineEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Medicine is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisMedicineEntity entity) {
	}
