...
import kmsweb.configs.security.auditing.services.PatientVisitPatientPaymentInsuranceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientVisitPatientPaymentInsuranceEntity.
 */
@Slf4j
@Component
public class PatientVisitPatientPaymentInsuranceEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientVisitPatientPaymentInsuranceEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientVisitPatientPaymentInsuranceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientVisitPatientPaymentInsuranceEntityListener.permissionEvaluator = permissionEvaluator;
		PatientVisitPatientPaymentInsuranceEntityListener.auditService = auditService;
		PatientVisitPatientPaymentInsuranceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientVisitPatientPaymentInsuranceEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientVisitPatientPaymentInsuranceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientVisitPatientPaymentInsuranceEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientVisitPatientPaymentInsuranceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientVisitPatientPaymentInsuranceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientVisitPatientPaymentInsuranceEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientVisitPatientPaymentInsuranceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientVisitPatientPaymentInsuranceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientVisitPatientPaymentInsuranceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Visit Patient Payment Insurance is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientVisitPatientPaymentInsuranceEntity entity) {
		entity.unsetPatientVisit();
		entity.unsetInsuranceBenefitPlan();
	}
