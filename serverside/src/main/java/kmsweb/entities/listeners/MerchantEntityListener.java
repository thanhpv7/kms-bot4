...
import kmsweb.configs.security.auditing.services.MerchantEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MerchantEntity.
 */
@Slf4j
@Component
public class MerchantEntityListener {
	...
	private static MerchantEntityReadAuditService auditService;
	...
	public void init(
		...
		MerchantEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MerchantEntityListener.permissionEvaluator = permissionEvaluator;
		MerchantEntityListener.auditService = auditService;
		MerchantEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MerchantEntity entity) {
	...
	@PostPersist
	public void afterPersist(MerchantEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MerchantEntity entity) {
	...
	@PostRemove
	public void afterRemove(MerchantEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MerchantEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MerchantEntity entity) {
	...
	@PostLoad
	public void afterLoad(MerchantEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MerchantEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MerchantEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Merchant is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MerchantEntity entity) {
		entity.unsetCashReceipts();
		entity.unsetMerchantSurchargeAndFees();
	}
