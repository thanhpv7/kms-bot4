...
import kmsweb.configs.security.auditing.services.BpjsSEPEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsSEPEntity.
 */
@Slf4j
@Component
public class BpjsSEPEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsSEPEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsSEPEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsSEPEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsSEPEntityListener.auditService = auditService;
		BpjsSEPEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsSEPEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsSEPEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsSEPEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsSEPEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsSEPEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsSEPEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsSEPEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsSEPEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsSEPEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS SEP is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsSEPEntity entity) {
