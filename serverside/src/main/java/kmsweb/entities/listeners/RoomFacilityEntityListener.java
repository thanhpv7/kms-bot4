...
import kmsweb.configs.security.auditing.services.RoomFacilityEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing RoomFacilityEntity.
 */
@Slf4j
@Component
public class RoomFacilityEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static RoomFacilityEntityReadAuditService auditService;
	...
	public void init(
		...
		RoomFacilityEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		RoomFacilityEntityListener.permissionEvaluator = permissionEvaluator;
		RoomFacilityEntityListener.auditService = auditService;
		RoomFacilityEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(RoomFacilityEntity entity) {
	...
	@PostPersist
	public void afterPersist(RoomFacilityEntity entity) {
	...
	@PreRemove
	public void beforeRemove(RoomFacilityEntity entity) {
	...
	@PostRemove
	public void afterRemove(RoomFacilityEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(RoomFacilityEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(RoomFacilityEntity entity) {
	...
	@PostLoad
	public void afterLoad(RoomFacilityEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the RoomFacilityEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the RoomFacilityEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Room Facility is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(RoomFacilityEntity entity) {
