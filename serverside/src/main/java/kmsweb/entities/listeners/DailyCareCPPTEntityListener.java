...
import kmsweb.configs.security.auditing.services.DailyCareCPPTEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DailyCareCPPTEntity.
 */
@Slf4j
@Component
public class DailyCareCPPTEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DailyCareCPPTEntityReadAuditService auditService;
	...
	public void init(
		...
		DailyCareCPPTEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DailyCareCPPTEntityListener.permissionEvaluator = permissionEvaluator;
		DailyCareCPPTEntityListener.auditService = auditService;
		DailyCareCPPTEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DailyCareCPPTEntity entity) {
	...
	@PostPersist
	public void afterPersist(DailyCareCPPTEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DailyCareCPPTEntity entity) {
	...
	@PostRemove
	public void afterRemove(DailyCareCPPTEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DailyCareCPPTEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DailyCareCPPTEntity entity) {
	...
	@PostLoad
	public void afterLoad(DailyCareCPPTEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DailyCareCPPTEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DailyCareCPPTEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Daily Care CPPT is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DailyCareCPPTEntity entity) {
		entity.unsetIcd10();
		entity.unsetIcd9CM();
		entity.unsetCreatedByStaff();
		entity.unsetDoctorInCharge();
