...
import kmsweb.configs.security.auditing.services.JournalEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing JournalEntity.
 */
@Slf4j
@Component
public class JournalEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static JournalEntityReadAuditService auditService;
	...
	public void init(
		...
		JournalEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		JournalEntityListener.permissionEvaluator = permissionEvaluator;
		JournalEntityListener.auditService = auditService;
		JournalEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(JournalEntity entity) {
	...
	@PostPersist
	public void afterPersist(JournalEntity entity) {
	...
	@PreRemove
	public void beforeRemove(JournalEntity entity) {
	...
	@PostRemove
	public void afterRemove(JournalEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(JournalEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(JournalEntity entity) {
	...
	@PostLoad
	public void afterLoad(JournalEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the JournalEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the JournalEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Journal is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(JournalEntity entity) {
	}
