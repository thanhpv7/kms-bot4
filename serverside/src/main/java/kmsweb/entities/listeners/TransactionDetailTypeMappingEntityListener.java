...
import kmsweb.configs.security.auditing.services.TransactionDetailTypeMappingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TransactionDetailTypeMappingEntity.
 */
@Slf4j
@Component
public class TransactionDetailTypeMappingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TransactionDetailTypeMappingEntityReadAuditService auditService;
	...
	public void init(
		...
		TransactionDetailTypeMappingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TransactionDetailTypeMappingEntityListener.permissionEvaluator = permissionEvaluator;
		TransactionDetailTypeMappingEntityListener.auditService = auditService;
		TransactionDetailTypeMappingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TransactionDetailTypeMappingEntity entity) {
	...
	@PostPersist
	public void afterPersist(TransactionDetailTypeMappingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TransactionDetailTypeMappingEntity entity) {
	...
	@PostRemove
	public void afterRemove(TransactionDetailTypeMappingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TransactionDetailTypeMappingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TransactionDetailTypeMappingEntity entity) {
	...
	@PostLoad
	public void afterLoad(TransactionDetailTypeMappingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TransactionDetailTypeMappingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TransactionDetailTypeMappingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Transaction Detail Type Mapping is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TransactionDetailTypeMappingEntity entity) {
		entity.unsetTransactionDetailType();
	}
