...
import kmsweb.configs.security.auditing.services.SatuSehatConfigurationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SatuSehatConfigurationEntity.
 */
@Slf4j
@Component
public class SatuSehatConfigurationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SatuSehatConfigurationEntityReadAuditService auditService;
	...
	public void init(
		...
		SatuSehatConfigurationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SatuSehatConfigurationEntityListener.permissionEvaluator = permissionEvaluator;
		SatuSehatConfigurationEntityListener.auditService = auditService;
		SatuSehatConfigurationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SatuSehatConfigurationEntity entity) {
	...
	@PostPersist
	public void afterPersist(SatuSehatConfigurationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SatuSehatConfigurationEntity entity) {
	...
	@PostRemove
	public void afterRemove(SatuSehatConfigurationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SatuSehatConfigurationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SatuSehatConfigurationEntity entity) {
	...
	@PostLoad
	public void afterLoad(SatuSehatConfigurationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SatuSehatConfigurationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SatuSehatConfigurationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Satu Sehat Configuration is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SatuSehatConfigurationEntity entity) {
	}
