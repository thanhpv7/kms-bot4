...
import kmsweb.configs.security.auditing.services.McuPackageEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing McuPackageEntity.
 */
@Slf4j
@Component
public class McuPackageEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static McuPackageEntityReadAuditService auditService;
	...
	public void init(
		...
		McuPackageEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		McuPackageEntityListener.permissionEvaluator = permissionEvaluator;
		McuPackageEntityListener.auditService = auditService;
		McuPackageEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(McuPackageEntity entity) {
	...
	@PostPersist
	public void afterPersist(McuPackageEntity entity) {
	...
	@PreRemove
	public void beforeRemove(McuPackageEntity entity) {
	...
	@PostRemove
	public void afterRemove(McuPackageEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(McuPackageEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(McuPackageEntity entity) {
	...
	@PostLoad
	public void afterLoad(McuPackageEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the McuPackageEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the McuPackageEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to MCU Package is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(McuPackageEntity entity) {
