...
import kmsweb.configs.security.auditing.services.PurchasingUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PurchasingUserEntity.
 */
@Slf4j
@Component
public class PurchasingUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PurchasingUserEntityReadAuditService auditService;
	...
	public void init(
		...
		PurchasingUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PurchasingUserEntityListener.permissionEvaluator = permissionEvaluator;
		PurchasingUserEntityListener.auditService = auditService;
		PurchasingUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PurchasingUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(PurchasingUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PurchasingUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(PurchasingUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PurchasingUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PurchasingUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(PurchasingUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PurchasingUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PurchasingUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Purchasing User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PurchasingUserEntity entity) {
		entity.unsetStaff();
	}
