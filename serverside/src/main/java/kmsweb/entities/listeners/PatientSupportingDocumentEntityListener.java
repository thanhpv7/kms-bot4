...
import kmsweb.configs.security.auditing.services.PatientSupportingDocumentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientSupportingDocumentEntity.
 */
@Slf4j
@Component
public class PatientSupportingDocumentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientSupportingDocumentEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientSupportingDocumentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientSupportingDocumentEntityListener.permissionEvaluator = permissionEvaluator;
		PatientSupportingDocumentEntityListener.auditService = auditService;
		PatientSupportingDocumentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientSupportingDocumentEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientSupportingDocumentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientSupportingDocumentEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientSupportingDocumentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientSupportingDocumentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientSupportingDocumentEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientSupportingDocumentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientSupportingDocumentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientSupportingDocumentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Supporting Document is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientSupportingDocumentEntity entity) {
