...
import kmsweb.configs.security.auditing.services.BpjsReferralSpecialistEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsReferralSpecialistEntity.
 */
@Slf4j
@Component
public class BpjsReferralSpecialistEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsReferralSpecialistEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsReferralSpecialistEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsReferralSpecialistEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsReferralSpecialistEntityListener.auditService = auditService;
		BpjsReferralSpecialistEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsReferralSpecialistEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsReferralSpecialistEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsReferralSpecialistEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsReferralSpecialistEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsReferralSpecialistEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsReferralSpecialistEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsReferralSpecialistEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsReferralSpecialistEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsReferralSpecialistEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Referral Specialist is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsReferralSpecialistEntity entity) {
