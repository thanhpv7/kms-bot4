...
import kmsweb.configs.security.auditing.services.MedicalRecordInitialChecklistEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalRecordInitialChecklistEntity.
 */
@Slf4j
@Component
public class MedicalRecordInitialChecklistEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalRecordInitialChecklistEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalRecordInitialChecklistEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalRecordInitialChecklistEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalRecordInitialChecklistEntityListener.auditService = auditService;
		MedicalRecordInitialChecklistEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalRecordInitialChecklistEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalRecordInitialChecklistEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalRecordInitialChecklistEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalRecordInitialChecklistEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalRecordInitialChecklistEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalRecordInitialChecklistEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalRecordInitialChecklistEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalRecordInitialChecklistEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalRecordInitialChecklistEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Record Initial Checklist is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalRecordInitialChecklistEntity entity) {
