...
import kmsweb.configs.security.auditing.services.ReferenceDataEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ReferenceDataEntity.
 */
@Slf4j
@Component
public class ReferenceDataEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ReferenceDataEntityReadAuditService auditService;
	...
	public void init(
		...
		ReferenceDataEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ReferenceDataEntityListener.permissionEvaluator = permissionEvaluator;
		ReferenceDataEntityListener.auditService = auditService;
		ReferenceDataEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ReferenceDataEntity entity) {
	...
	@PostPersist
	public void afterPersist(ReferenceDataEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ReferenceDataEntity entity) {
	...
	@PostRemove
	public void afterRemove(ReferenceDataEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ReferenceDataEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ReferenceDataEntity entity) {
	...
	@PostLoad
	public void afterLoad(ReferenceDataEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ReferenceDataEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ReferenceDataEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Reference Data is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ReferenceDataEntity entity) {
	}
