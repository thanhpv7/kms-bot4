...
import kmsweb.configs.security.auditing.services.WarehouseEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing WarehouseEntity.
 */
@Slf4j
@Component
public class WarehouseEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static WarehouseEntityReadAuditService auditService;
	...
	public void init(
		...
		WarehouseEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		WarehouseEntityListener.permissionEvaluator = permissionEvaluator;
		WarehouseEntityListener.auditService = auditService;
		WarehouseEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(WarehouseEntity entity) {
	...
	@PostPersist
	public void afterPersist(WarehouseEntity entity) {
	...
	@PreRemove
	public void beforeRemove(WarehouseEntity entity) {
	...
	@PostRemove
	public void afterRemove(WarehouseEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(WarehouseEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(WarehouseEntity entity) {
	...
	@PostLoad
	public void afterLoad(WarehouseEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the WarehouseEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the WarehouseEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Warehouse is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(WarehouseEntity entity) {
		entity.unsetBatchStockBalancesWarehouse();
		entity.unsetBatchStockBalances();
		entity.unsetBatchStockTransactionDetails();
		entity.unsetDestinationWarehouses();
		entity.unsetGoodsReceiveNoteBatchs();
		entity.unsetGoodsReceiveNotes();
		entity.unsetInternalOrders();
		entity.unsetInventoryAdjusmentBatchs();
		entity.unsetInventoryAdjustments();
		entity.unsetInventoryBatchCorrections();
		entity.unsetLocations();
		entity.unsetMedicalFees();
		entity.unsetMedicationHeaders();
		entity.unsetPrescriptionHeaders();
		entity.unsetPurchaseOrders();
		entity.unsetPurchaseRequisitions();
		entity.unsetRetailPharmacies();
		entity.unsetReturnSuppliers();
		entity.unsetSourceWarehouses();
		entity.unsetStockCatalogues();
