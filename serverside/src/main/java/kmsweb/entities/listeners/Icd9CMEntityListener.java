...
import kmsweb.configs.security.auditing.services.Icd9CMEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing Icd9CMEntity.
 */
@Slf4j
@Component
public class Icd9CMEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static Icd9CMEntityReadAuditService auditService;
	...
	public void init(
		...
		Icd9CMEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		Icd9CMEntityListener.permissionEvaluator = permissionEvaluator;
		Icd9CMEntityListener.auditService = auditService;
		Icd9CMEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(Icd9CMEntity entity) {
	...
	@PostPersist
	public void afterPersist(Icd9CMEntity entity) {
	...
	@PreRemove
	public void beforeRemove(Icd9CMEntity entity) {
	...
	@PostRemove
	public void afterRemove(Icd9CMEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(Icd9CMEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(Icd9CMEntity entity) {
	...
	@PostLoad
	public void afterLoad(Icd9CMEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the Icd9CMEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the Icd9CMEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to ICD 9 CM is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(Icd9CMEntity entity) {
