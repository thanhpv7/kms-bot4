...
import kmsweb.configs.security.auditing.services.CompoundPrescriptionItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing CompoundPrescriptionItemEntity.
 */
@Slf4j
@Component
public class CompoundPrescriptionItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static CompoundPrescriptionItemEntityReadAuditService auditService;
	...
	public void init(
		...
		CompoundPrescriptionItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		CompoundPrescriptionItemEntityListener.permissionEvaluator = permissionEvaluator;
		CompoundPrescriptionItemEntityListener.auditService = auditService;
		CompoundPrescriptionItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(CompoundPrescriptionItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(CompoundPrescriptionItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(CompoundPrescriptionItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(CompoundPrescriptionItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(CompoundPrescriptionItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(CompoundPrescriptionItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(CompoundPrescriptionItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the CompoundPrescriptionItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the CompoundPrescriptionItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Compound Prescription Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(CompoundPrescriptionItemEntity entity) {
		entity.unsetCompoundPrescriptionDetail();
