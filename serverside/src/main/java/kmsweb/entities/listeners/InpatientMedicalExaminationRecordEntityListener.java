...
import kmsweb.configs.security.auditing.services.InpatientMedicalExaminationRecordEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InpatientMedicalExaminationRecordEntity.
 */
@Slf4j
@Component
public class InpatientMedicalExaminationRecordEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InpatientMedicalExaminationRecordEntityReadAuditService auditService;
	...
	public void init(
		...
		InpatientMedicalExaminationRecordEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InpatientMedicalExaminationRecordEntityListener.permissionEvaluator = permissionEvaluator;
		InpatientMedicalExaminationRecordEntityListener.auditService = auditService;
		InpatientMedicalExaminationRecordEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InpatientMedicalExaminationRecordEntity entity) {
	...
	@PostPersist
	public void afterPersist(InpatientMedicalExaminationRecordEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InpatientMedicalExaminationRecordEntity entity) {
	...
	@PostRemove
	public void afterRemove(InpatientMedicalExaminationRecordEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InpatientMedicalExaminationRecordEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InpatientMedicalExaminationRecordEntity entity) {
	...
	@PostLoad
	public void afterLoad(InpatientMedicalExaminationRecordEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InpatientMedicalExaminationRecordEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InpatientMedicalExaminationRecordEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Inpatient Medical Examination Record is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InpatientMedicalExaminationRecordEntity entity) {
