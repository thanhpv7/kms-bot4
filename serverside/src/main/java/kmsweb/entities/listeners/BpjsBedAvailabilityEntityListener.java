...
import kmsweb.configs.security.auditing.services.BpjsBedAvailabilityEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsBedAvailabilityEntity.
 */
@Slf4j
@Component
public class BpjsBedAvailabilityEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsBedAvailabilityEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsBedAvailabilityEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsBedAvailabilityEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsBedAvailabilityEntityListener.auditService = auditService;
		BpjsBedAvailabilityEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsBedAvailabilityEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsBedAvailabilityEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsBedAvailabilityEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsBedAvailabilityEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsBedAvailabilityEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsBedAvailabilityEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsBedAvailabilityEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsBedAvailabilityEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsBedAvailabilityEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Bed Availability is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsBedAvailabilityEntity entity) {
	}
