...
import kmsweb.configs.security.auditing.services.WarehouseUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing WarehouseUserEntity.
 */
@Slf4j
@Component
public class WarehouseUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static WarehouseUserEntityReadAuditService auditService;
	...
	public void init(
		...
		WarehouseUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		WarehouseUserEntityListener.permissionEvaluator = permissionEvaluator;
		WarehouseUserEntityListener.auditService = auditService;
		WarehouseUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(WarehouseUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(WarehouseUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(WarehouseUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(WarehouseUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(WarehouseUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(WarehouseUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(WarehouseUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the WarehouseUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the WarehouseUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Warehouse User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(WarehouseUserEntity entity) {
		entity.unsetStaff();
	}
