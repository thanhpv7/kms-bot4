...
import kmsweb.configs.security.auditing.services.TransferOrderStockDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TransferOrderStockDetailEntity.
 */
@Slf4j
@Component
public class TransferOrderStockDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TransferOrderStockDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		TransferOrderStockDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TransferOrderStockDetailEntityListener.permissionEvaluator = permissionEvaluator;
		TransferOrderStockDetailEntityListener.auditService = auditService;
		TransferOrderStockDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TransferOrderStockDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(TransferOrderStockDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TransferOrderStockDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(TransferOrderStockDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TransferOrderStockDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TransferOrderStockDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(TransferOrderStockDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TransferOrderStockDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TransferOrderStockDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Transfer Order Stock Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TransferOrderStockDetailEntity entity) {
		entity.unsetStaff();
		entity.unsetStockCatalogue();
		entity.unsetTransferOrder();
	}
