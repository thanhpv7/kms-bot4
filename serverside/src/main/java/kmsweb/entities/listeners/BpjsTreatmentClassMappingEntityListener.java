...
import kmsweb.configs.security.auditing.services.BpjsTreatmentClassMappingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsTreatmentClassMappingEntity.
 */
@Slf4j
@Component
public class BpjsTreatmentClassMappingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsTreatmentClassMappingEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsTreatmentClassMappingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsTreatmentClassMappingEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsTreatmentClassMappingEntityListener.auditService = auditService;
		BpjsTreatmentClassMappingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsTreatmentClassMappingEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsTreatmentClassMappingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsTreatmentClassMappingEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsTreatmentClassMappingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsTreatmentClassMappingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsTreatmentClassMappingEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsTreatmentClassMappingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsTreatmentClassMappingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsTreatmentClassMappingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Treatment Class Mapping is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsTreatmentClassMappingEntity entity) {
	}
