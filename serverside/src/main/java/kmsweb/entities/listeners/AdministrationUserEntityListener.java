...
import kmsweb.configs.security.auditing.services.AdministrationUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing AdministrationUserEntity.
 */
@Slf4j
@Component
public class AdministrationUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static AdministrationUserEntityReadAuditService auditService;
	...
	public void init(
		...
		AdministrationUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		AdministrationUserEntityListener.permissionEvaluator = permissionEvaluator;
		AdministrationUserEntityListener.auditService = auditService;
		AdministrationUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(AdministrationUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(AdministrationUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(AdministrationUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(AdministrationUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(AdministrationUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(AdministrationUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(AdministrationUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the AdministrationUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the AdministrationUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Administration User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(AdministrationUserEntity entity) {
		entity.unsetStaff();
	}
