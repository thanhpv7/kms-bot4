...
import kmsweb.configs.security.auditing.services.TariffSchemaEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TariffSchemaEntity.
 */
@Slf4j
@Component
public class TariffSchemaEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TariffSchemaEntityReadAuditService auditService;
	...
	public void init(
		...
		TariffSchemaEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TariffSchemaEntityListener.permissionEvaluator = permissionEvaluator;
		TariffSchemaEntityListener.auditService = auditService;
		TariffSchemaEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TariffSchemaEntity entity) {
	...
	@PostPersist
	public void afterPersist(TariffSchemaEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TariffSchemaEntity entity) {
	...
	@PostRemove
	public void afterRemove(TariffSchemaEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TariffSchemaEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TariffSchemaEntity entity) {
	...
	@PostLoad
	public void afterLoad(TariffSchemaEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TariffSchemaEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TariffSchemaEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Tariff Schema is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TariffSchemaEntity entity) {
		entity.unsetBaseScheme();
		entity.unsetInvoiceItems();
		entity.unsetInvoices();
		entity.unsetTariffDefinitions();
		entity.unsetTariffScheme();
	}
