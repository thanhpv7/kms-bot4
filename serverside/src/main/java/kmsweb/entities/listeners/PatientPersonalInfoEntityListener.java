...
import kmsweb.configs.security.auditing.services.PatientPersonalInfoEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientPersonalInfoEntity.
 */
@Slf4j
@Component
public class PatientPersonalInfoEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientPersonalInfoEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientPersonalInfoEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientPersonalInfoEntityListener.permissionEvaluator = permissionEvaluator;
		PatientPersonalInfoEntityListener.auditService = auditService;
		PatientPersonalInfoEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientPersonalInfoEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientPersonalInfoEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientPersonalInfoEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientPersonalInfoEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientPersonalInfoEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientPersonalInfoEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientPersonalInfoEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientPersonalInfoEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientPersonalInfoEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Personal Info is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientPersonalInfoEntity entity) {
