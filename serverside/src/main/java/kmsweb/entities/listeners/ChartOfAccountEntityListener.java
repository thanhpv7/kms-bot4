...
import kmsweb.configs.security.auditing.services.ChartOfAccountEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ChartOfAccountEntity.
 */
@Slf4j
@Component
public class ChartOfAccountEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ChartOfAccountEntityReadAuditService auditService;
	...
	public void init(
		...
		ChartOfAccountEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ChartOfAccountEntityListener.permissionEvaluator = permissionEvaluator;
		ChartOfAccountEntityListener.auditService = auditService;
		ChartOfAccountEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ChartOfAccountEntity entity) {
	...
	@PostPersist
	public void afterPersist(ChartOfAccountEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ChartOfAccountEntity entity) {
	...
	@PostRemove
	public void afterRemove(ChartOfAccountEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ChartOfAccountEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ChartOfAccountEntity entity) {
	...
	@PostLoad
	public void afterLoad(ChartOfAccountEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ChartOfAccountEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ChartOfAccountEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Chart of Account is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ChartOfAccountEntity entity) {
		entity.unsetChartOfAccountMappings();
	}
