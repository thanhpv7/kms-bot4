...
import kmsweb.configs.security.auditing.services.SampleCollectionItemsEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SampleCollectionItemsEntity.
 */
@Slf4j
@Component
public class SampleCollectionItemsEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SampleCollectionItemsEntityReadAuditService auditService;
	...
	public void init(
		...
		SampleCollectionItemsEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SampleCollectionItemsEntityListener.permissionEvaluator = permissionEvaluator;
		SampleCollectionItemsEntityListener.auditService = auditService;
		SampleCollectionItemsEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SampleCollectionItemsEntity entity) {
	...
	@PostPersist
	public void afterPersist(SampleCollectionItemsEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SampleCollectionItemsEntity entity) {
	...
	@PostRemove
	public void afterRemove(SampleCollectionItemsEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SampleCollectionItemsEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SampleCollectionItemsEntity entity) {
	...
	@PostLoad
	public void afterLoad(SampleCollectionItemsEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SampleCollectionItemsEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SampleCollectionItemsEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Sample Collection Items is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SampleCollectionItemsEntity entity) {
		entity.unsetSampleCollectionInformation();
		entity.unsetSampleCollectionStaff();
		entity.unsetSampleDeliveryStaff();
		entity.unsetSampleProcessingStaff();
	}
