...
import kmsweb.configs.security.auditing.services.InternalOrderStockDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InternalOrderStockDetailEntity.
 */
@Slf4j
@Component
public class InternalOrderStockDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InternalOrderStockDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		InternalOrderStockDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InternalOrderStockDetailEntityListener.permissionEvaluator = permissionEvaluator;
		InternalOrderStockDetailEntityListener.auditService = auditService;
		InternalOrderStockDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InternalOrderStockDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(InternalOrderStockDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InternalOrderStockDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(InternalOrderStockDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InternalOrderStockDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InternalOrderStockDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(InternalOrderStockDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InternalOrderStockDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InternalOrderStockDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Internal Order Stock Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InternalOrderStockDetailEntity entity) {
		entity.unsetInternalOrder();
