...
import kmsweb.configs.security.auditing.services.PcareActionExaminationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareActionExaminationEntity.
 */
@Slf4j
@Component
public class PcareActionExaminationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareActionExaminationEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareActionExaminationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareActionExaminationEntityListener.permissionEvaluator = permissionEvaluator;
		PcareActionExaminationEntityListener.auditService = auditService;
		PcareActionExaminationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareActionExaminationEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareActionExaminationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareActionExaminationEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareActionExaminationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareActionExaminationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareActionExaminationEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareActionExaminationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareActionExaminationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareActionExaminationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Action Examination is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareActionExaminationEntity entity) {
