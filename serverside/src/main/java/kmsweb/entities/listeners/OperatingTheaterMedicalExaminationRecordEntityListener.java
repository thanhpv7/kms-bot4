...
import kmsweb.configs.security.auditing.services.OperatingTheaterMedicalExaminationRecordEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing OperatingTheaterMedicalExaminationRecordEntity.
 */
@Slf4j
@Component
public class OperatingTheaterMedicalExaminationRecordEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static OperatingTheaterMedicalExaminationRecordEntityReadAuditService auditService;
	...
	public void init(
		...
		OperatingTheaterMedicalExaminationRecordEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		OperatingTheaterMedicalExaminationRecordEntityListener.permissionEvaluator = permissionEvaluator;
		OperatingTheaterMedicalExaminationRecordEntityListener.auditService = auditService;
		OperatingTheaterMedicalExaminationRecordEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(OperatingTheaterMedicalExaminationRecordEntity entity) {
	...
	@PostPersist
	public void afterPersist(OperatingTheaterMedicalExaminationRecordEntity entity) {
	...
	@PreRemove
	public void beforeRemove(OperatingTheaterMedicalExaminationRecordEntity entity) {
	...
	@PostRemove
	public void afterRemove(OperatingTheaterMedicalExaminationRecordEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(OperatingTheaterMedicalExaminationRecordEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(OperatingTheaterMedicalExaminationRecordEntity entity) {
	...
	@PostLoad
	public void afterLoad(OperatingTheaterMedicalExaminationRecordEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the OperatingTheaterMedicalExaminationRecordEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the OperatingTheaterMedicalExaminationRecordEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Operating Theater Medical Examination Record is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(OperatingTheaterMedicalExaminationRecordEntity entity) {
		entity.unsetMedicalExaminationRecord();
		entity.unsetIntraoperativeRecords();
		entity.unsetPostOperativeDetails();
		entity.unsetPreoperativeRecords();
		entity.unsetNursingAnesthesiaDetail();
		entity.unsetNursingDetailSurgicalNursings();
		entity.unsetNursingDetailsAnesthesiaNursings();
		entity.unsetNursingSurgicalDetail();
	}
