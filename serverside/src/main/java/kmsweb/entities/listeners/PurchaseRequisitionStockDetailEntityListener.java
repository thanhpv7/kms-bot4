...
import kmsweb.configs.security.auditing.services.PurchaseRequisitionStockDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PurchaseRequisitionStockDetailEntity.
 */
@Slf4j
@Component
public class PurchaseRequisitionStockDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PurchaseRequisitionStockDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		PurchaseRequisitionStockDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PurchaseRequisitionStockDetailEntityListener.permissionEvaluator = permissionEvaluator;
		PurchaseRequisitionStockDetailEntityListener.auditService = auditService;
		PurchaseRequisitionStockDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PurchaseRequisitionStockDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(PurchaseRequisitionStockDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PurchaseRequisitionStockDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(PurchaseRequisitionStockDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PurchaseRequisitionStockDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PurchaseRequisitionStockDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(PurchaseRequisitionStockDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PurchaseRequisitionStockDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PurchaseRequisitionStockDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Purchase Requisition Stock Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PurchaseRequisitionStockDetailEntity entity) {
		entity.unsetPurchaseRequisition();
