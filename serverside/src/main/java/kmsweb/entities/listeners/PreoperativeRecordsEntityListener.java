...
import kmsweb.configs.security.auditing.services.PreoperativeRecordsEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PreoperativeRecordsEntity.
 */
@Slf4j
@Component
public class PreoperativeRecordsEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PreoperativeRecordsEntityReadAuditService auditService;
	...
	public void init(
		...
		PreoperativeRecordsEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PreoperativeRecordsEntityListener.permissionEvaluator = permissionEvaluator;
		PreoperativeRecordsEntityListener.auditService = auditService;
		PreoperativeRecordsEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PreoperativeRecordsEntity entity) {
	...
	@PostPersist
	public void afterPersist(PreoperativeRecordsEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PreoperativeRecordsEntity entity) {
	...
	@PostRemove
	public void afterRemove(PreoperativeRecordsEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PreoperativeRecordsEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PreoperativeRecordsEntity entity) {
	...
	@PostLoad
	public void afterLoad(PreoperativeRecordsEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PreoperativeRecordsEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PreoperativeRecordsEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Preoperative Records is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PreoperativeRecordsEntity entity) {
		...
		entity.unsetPreoperativeRooms();
		entity.unsetSurgeon();
		entity.unsetSurgicalMedicalTranscriber();
		entity.unsetSurgicalNurse();
		entity.unsetAdditionalAnesthesiologists();
		entity.unsetAdditionalSurgeryNurses();
		entity.unsetBedFacilities();
		entity.unsetCoTreatingAnesthesiologists();
		entity.unsetCoTreatingSurgeons();
	}
