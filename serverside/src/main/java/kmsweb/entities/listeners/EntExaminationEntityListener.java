...
import kmsweb.configs.security.auditing.services.EntExaminationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing EntExaminationEntity.
 */
@Slf4j
@Component
public class EntExaminationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static EntExaminationEntityReadAuditService auditService;
	...
	public void init(
		...
		EntExaminationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		EntExaminationEntityListener.permissionEvaluator = permissionEvaluator;
		EntExaminationEntityListener.auditService = auditService;
		EntExaminationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(EntExaminationEntity entity) {
	...
	@PostPersist
	public void afterPersist(EntExaminationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(EntExaminationEntity entity) {
	...
	@PostRemove
	public void afterRemove(EntExaminationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(EntExaminationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(EntExaminationEntity entity) {
	...
	@PostLoad
	public void afterLoad(EntExaminationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the EntExaminationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the EntExaminationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to ENT Examination is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(EntExaminationEntity entity) {
