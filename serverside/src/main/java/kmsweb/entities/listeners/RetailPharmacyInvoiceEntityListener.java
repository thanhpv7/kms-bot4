...
import kmsweb.configs.security.auditing.services.RetailPharmacyInvoiceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing RetailPharmacyInvoiceEntity.
 */
@Slf4j
@Component
public class RetailPharmacyInvoiceEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static RetailPharmacyInvoiceEntityReadAuditService auditService;
	...
	public void init(
		...
		RetailPharmacyInvoiceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		RetailPharmacyInvoiceEntityListener.permissionEvaluator = permissionEvaluator;
		RetailPharmacyInvoiceEntityListener.auditService = auditService;
		RetailPharmacyInvoiceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(RetailPharmacyInvoiceEntity entity) {
	...
	@PostPersist
	public void afterPersist(RetailPharmacyInvoiceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(RetailPharmacyInvoiceEntity entity) {
	...
	@PostRemove
	public void afterRemove(RetailPharmacyInvoiceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(RetailPharmacyInvoiceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(RetailPharmacyInvoiceEntity entity) {
	...
	@PostLoad
	public void afterLoad(RetailPharmacyInvoiceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the RetailPharmacyInvoiceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the RetailPharmacyInvoiceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Retail Pharmacy Invoice is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(RetailPharmacyInvoiceEntity entity) {
		entity.unsetInvoiceSummary();
		entity.unsetRetailPharmacy();
		entity.unsetCashReceipts();
