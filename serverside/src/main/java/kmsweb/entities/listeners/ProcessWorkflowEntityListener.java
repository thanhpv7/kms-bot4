...
import kmsweb.configs.security.auditing.services.ProcessWorkflowEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ProcessWorkflowEntity.
 */
@Slf4j
@Component
public class ProcessWorkflowEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ProcessWorkflowEntityReadAuditService auditService;
	...
	public void init(
		...
		ProcessWorkflowEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ProcessWorkflowEntityListener.permissionEvaluator = permissionEvaluator;
		ProcessWorkflowEntityListener.auditService = auditService;
		ProcessWorkflowEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ProcessWorkflowEntity entity) {
	...
	@PostPersist
	public void afterPersist(ProcessWorkflowEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ProcessWorkflowEntity entity) {
	...
	@PostRemove
	public void afterRemove(ProcessWorkflowEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ProcessWorkflowEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ProcessWorkflowEntity entity) {
	...
	@PostLoad
	public void afterLoad(ProcessWorkflowEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ProcessWorkflowEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ProcessWorkflowEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Process Workflow is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ProcessWorkflowEntity entity) {
		entity.unsetStaff();
	}
