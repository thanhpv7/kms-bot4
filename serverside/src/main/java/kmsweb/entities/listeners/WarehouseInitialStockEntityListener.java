...
import kmsweb.configs.security.auditing.services.WarehouseInitialStockEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing WarehouseInitialStockEntity.
 */
@Slf4j
@Component
public class WarehouseInitialStockEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static WarehouseInitialStockEntityReadAuditService auditService;
	...
	public void init(
		...
		WarehouseInitialStockEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		WarehouseInitialStockEntityListener.permissionEvaluator = permissionEvaluator;
		WarehouseInitialStockEntityListener.auditService = auditService;
		WarehouseInitialStockEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(WarehouseInitialStockEntity entity) {
	...
	@PostPersist
	public void afterPersist(WarehouseInitialStockEntity entity) {
	...
	@PreRemove
	public void beforeRemove(WarehouseInitialStockEntity entity) {
	...
	@PostRemove
	public void afterRemove(WarehouseInitialStockEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(WarehouseInitialStockEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(WarehouseInitialStockEntity entity) {
	...
	@PostLoad
	public void afterLoad(WarehouseInitialStockEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the WarehouseInitialStockEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the WarehouseInitialStockEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Warehouse Initial Stock  is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(WarehouseInitialStockEntity entity) {
