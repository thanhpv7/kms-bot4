...
import kmsweb.configs.security.auditing.services.PatientVisitEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientVisitEntity.
 */
@Slf4j
@Component
public class PatientVisitEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientVisitEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientVisitEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientVisitEntityListener.permissionEvaluator = permissionEvaluator;
		PatientVisitEntityListener.auditService = auditService;
		PatientVisitEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientVisitEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientVisitEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientVisitEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientVisitEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientVisitEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientVisitEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientVisitEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientVisitEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientVisitEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Visit is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientVisitEntity entity) {
		entity.unsetBpjsPcareRegistrations();
		entity.unsetPatientVisitPatientPaymentInsurance();
		entity.unsetPatientVisitPatientPaymentOthers();
		entity.unsetPatientVisitPaymentSelfPaying();
		entity.unsetPatient();
		entity.unsetBpjsVisitDatas();
		entity.unsetBpjsPcarePendaftarans();
