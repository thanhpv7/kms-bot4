...
import kmsweb.configs.security.auditing.services.MedicalTranscriberUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalTranscriberUserEntity.
 */
@Slf4j
@Component
public class MedicalTranscriberUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalTranscriberUserEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalTranscriberUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalTranscriberUserEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalTranscriberUserEntityListener.auditService = auditService;
		MedicalTranscriberUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalTranscriberUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalTranscriberUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalTranscriberUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalTranscriberUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalTranscriberUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalTranscriberUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalTranscriberUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalTranscriberUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalTranscriberUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Transcriber User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalTranscriberUserEntity entity) {
		entity.unsetStaff();
	}
