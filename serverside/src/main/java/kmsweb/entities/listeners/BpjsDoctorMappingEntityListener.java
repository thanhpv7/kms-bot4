...
import kmsweb.configs.security.auditing.services.BpjsDoctorMappingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsDoctorMappingEntity.
 */
@Slf4j
@Component
public class BpjsDoctorMappingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsDoctorMappingEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsDoctorMappingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsDoctorMappingEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsDoctorMappingEntityListener.auditService = auditService;
		BpjsDoctorMappingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsDoctorMappingEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsDoctorMappingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsDoctorMappingEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsDoctorMappingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsDoctorMappingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsDoctorMappingEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsDoctorMappingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsDoctorMappingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsDoctorMappingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Doctor Mapping is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsDoctorMappingEntity entity) {
		entity.unsetStaff();
		entity.unsetBpjsClaimSubmissions();
		entity.unsetBpjsControlPlans();
		entity.unsetBpjsPRBs();
		entity.unsetKodedokter();
	}
