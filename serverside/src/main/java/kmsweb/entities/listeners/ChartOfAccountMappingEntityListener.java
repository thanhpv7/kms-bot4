...
import kmsweb.configs.security.auditing.services.ChartOfAccountMappingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ChartOfAccountMappingEntity.
 */
@Slf4j
@Component
public class ChartOfAccountMappingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ChartOfAccountMappingEntityReadAuditService auditService;
	...
	public void init(
		...
		ChartOfAccountMappingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ChartOfAccountMappingEntityListener.permissionEvaluator = permissionEvaluator;
		ChartOfAccountMappingEntityListener.auditService = auditService;
		ChartOfAccountMappingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ChartOfAccountMappingEntity entity) {
	...
	@PostPersist
	public void afterPersist(ChartOfAccountMappingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ChartOfAccountMappingEntity entity) {
	...
	@PostRemove
	public void afterRemove(ChartOfAccountMappingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ChartOfAccountMappingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ChartOfAccountMappingEntity entity) {
	...
	@PostLoad
	public void afterLoad(ChartOfAccountMappingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ChartOfAccountMappingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ChartOfAccountMappingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Chart of Account Mapping is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ChartOfAccountMappingEntity entity) {
		entity.unsetChartOfAccount();
	}
