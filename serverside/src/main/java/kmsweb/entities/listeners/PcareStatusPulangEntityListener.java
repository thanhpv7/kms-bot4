...
import kmsweb.configs.security.auditing.services.PcareStatusPulangEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareStatusPulangEntity.
 */
@Slf4j
@Component
public class PcareStatusPulangEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareStatusPulangEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareStatusPulangEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareStatusPulangEntityListener.permissionEvaluator = permissionEvaluator;
		PcareStatusPulangEntityListener.auditService = auditService;
		PcareStatusPulangEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareStatusPulangEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareStatusPulangEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareStatusPulangEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareStatusPulangEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareStatusPulangEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareStatusPulangEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareStatusPulangEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareStatusPulangEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareStatusPulangEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Status Pulang is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareStatusPulangEntity entity) {
	}
