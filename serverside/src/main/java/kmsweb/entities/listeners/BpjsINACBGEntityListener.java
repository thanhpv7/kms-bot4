...
import kmsweb.configs.security.auditing.services.BpjsINACBGEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsINACBGEntity.
 */
@Slf4j
@Component
public class BpjsINACBGEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsINACBGEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsINACBGEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsINACBGEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsINACBGEntityListener.auditService = auditService;
		BpjsINACBGEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsINACBGEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsINACBGEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsINACBGEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsINACBGEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsINACBGEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsINACBGEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsINACBGEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsINACBGEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsINACBGEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS INACBG is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsINACBGEntity entity) {
	}
