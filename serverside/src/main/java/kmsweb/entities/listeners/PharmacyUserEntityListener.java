...
import kmsweb.configs.security.auditing.services.PharmacyUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PharmacyUserEntity.
 */
@Slf4j
@Component
public class PharmacyUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PharmacyUserEntityReadAuditService auditService;
	...
	public void init(
		...
		PharmacyUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PharmacyUserEntityListener.permissionEvaluator = permissionEvaluator;
		PharmacyUserEntityListener.auditService = auditService;
		PharmacyUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PharmacyUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(PharmacyUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PharmacyUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(PharmacyUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PharmacyUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PharmacyUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(PharmacyUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PharmacyUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PharmacyUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Pharmacy User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PharmacyUserEntity entity) {
		entity.unsetStaff();
	}
