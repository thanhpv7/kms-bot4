...
import kmsweb.configs.security.auditing.services.VaccinationOrderDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing VaccinationOrderDetailEntity.
 */
@Slf4j
@Component
public class VaccinationOrderDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static VaccinationOrderDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		VaccinationOrderDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		VaccinationOrderDetailEntityListener.permissionEvaluator = permissionEvaluator;
		VaccinationOrderDetailEntityListener.auditService = auditService;
		VaccinationOrderDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(VaccinationOrderDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(VaccinationOrderDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(VaccinationOrderDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(VaccinationOrderDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(VaccinationOrderDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(VaccinationOrderDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(VaccinationOrderDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the VaccinationOrderDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the VaccinationOrderDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Vaccination Order Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(VaccinationOrderDetailEntity entity) {
		entity.unsetStockCatalogue();
		entity.unsetVaccinationOrder();
