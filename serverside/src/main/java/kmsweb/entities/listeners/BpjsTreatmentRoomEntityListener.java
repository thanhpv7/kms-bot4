...
import kmsweb.configs.security.auditing.services.BpjsTreatmentRoomEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsTreatmentRoomEntity.
 */
@Slf4j
@Component
public class BpjsTreatmentRoomEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsTreatmentRoomEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsTreatmentRoomEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsTreatmentRoomEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsTreatmentRoomEntityListener.auditService = auditService;
		BpjsTreatmentRoomEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsTreatmentRoomEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsTreatmentRoomEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsTreatmentRoomEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsTreatmentRoomEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsTreatmentRoomEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsTreatmentRoomEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsTreatmentRoomEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsTreatmentRoomEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsTreatmentRoomEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Treatment Room is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsTreatmentRoomEntity entity) {
