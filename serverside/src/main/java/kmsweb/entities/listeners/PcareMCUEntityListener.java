...
import kmsweb.configs.security.auditing.services.PcareMCUEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareMCUEntity.
 */
@Slf4j
@Component
public class PcareMCUEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareMCUEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareMCUEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareMCUEntityListener.permissionEvaluator = permissionEvaluator;
		PcareMCUEntityListener.auditService = auditService;
		PcareMCUEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareMCUEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareMCUEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareMCUEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareMCUEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareMCUEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareMCUEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareMCUEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareMCUEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareMCUEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare MCU is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareMCUEntity entity) {
	}
