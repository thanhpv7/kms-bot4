...
import kmsweb.configs.security.auditing.services.BpjsTreatmentHistoryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsTreatmentHistoryEntity.
 */
@Slf4j
@Component
public class BpjsTreatmentHistoryEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsTreatmentHistoryEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsTreatmentHistoryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsTreatmentHistoryEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsTreatmentHistoryEntityListener.auditService = auditService;
		BpjsTreatmentHistoryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsTreatmentHistoryEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsTreatmentHistoryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsTreatmentHistoryEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsTreatmentHistoryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsTreatmentHistoryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsTreatmentHistoryEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsTreatmentHistoryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsTreatmentHistoryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsTreatmentHistoryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Treatment History is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsTreatmentHistoryEntity entity) {
	}
