...
import kmsweb.configs.security.auditing.services.PcareExaminationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareExaminationEntity.
 */
@Slf4j
@Component
public class PcareExaminationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareExaminationEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareExaminationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareExaminationEntityListener.permissionEvaluator = permissionEvaluator;
		PcareExaminationEntityListener.auditService = auditService;
		PcareExaminationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareExaminationEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareExaminationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareExaminationEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareExaminationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareExaminationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareExaminationEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareExaminationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareExaminationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareExaminationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Examination is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareExaminationEntity entity) {
		entity.unsetRegistration();
		entity.unsetPcareActionExaminations();
		entity.unsetPcareCompoundDrugs();
		entity.unsetPcareNonCompoundDrugs();
	}
