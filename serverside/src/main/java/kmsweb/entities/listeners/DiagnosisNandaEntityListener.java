...
import kmsweb.configs.security.auditing.services.DiagnosisNandaEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DiagnosisNandaEntity.
 */
@Slf4j
@Component
public class DiagnosisNandaEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DiagnosisNandaEntityReadAuditService auditService;
	...
	public void init(
		...
		DiagnosisNandaEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DiagnosisNandaEntityListener.permissionEvaluator = permissionEvaluator;
		DiagnosisNandaEntityListener.auditService = auditService;
		DiagnosisNandaEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DiagnosisNandaEntity entity) {
	...
	@PostPersist
	public void afterPersist(DiagnosisNandaEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DiagnosisNandaEntity entity) {
	...
	@PostRemove
	public void afterRemove(DiagnosisNandaEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DiagnosisNandaEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DiagnosisNandaEntity entity) {
	...
	@PostLoad
	public void afterLoad(DiagnosisNandaEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DiagnosisNandaEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DiagnosisNandaEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Diagnosis Nanda is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DiagnosisNandaEntity entity) {
