...
import kmsweb.configs.security.auditing.services.TariffFormulaEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing TariffFormulaEntity.
 */
@Slf4j
@Component
public class TariffFormulaEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static TariffFormulaEntityReadAuditService auditService;
	...
	public void init(
		...
		TariffFormulaEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		TariffFormulaEntityListener.permissionEvaluator = permissionEvaluator;
		TariffFormulaEntityListener.auditService = auditService;
		TariffFormulaEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(TariffFormulaEntity entity) {
	...
	@PostPersist
	public void afterPersist(TariffFormulaEntity entity) {
	...
	@PreRemove
	public void beforeRemove(TariffFormulaEntity entity) {
	...
	@PostRemove
	public void afterRemove(TariffFormulaEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(TariffFormulaEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(TariffFormulaEntity entity) {
	...
	@PostLoad
	public void afterLoad(TariffFormulaEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the TariffFormulaEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the TariffFormulaEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Tariff Formula is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(TariffFormulaEntity entity) {
		entity.unsetBaseSchema();
