...
import kmsweb.configs.security.auditing.services.BpjsDoctorScheduleEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsDoctorScheduleEntity.
 */
@Slf4j
@Component
public class BpjsDoctorScheduleEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsDoctorScheduleEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsDoctorScheduleEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsDoctorScheduleEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsDoctorScheduleEntityListener.auditService = auditService;
		BpjsDoctorScheduleEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsDoctorScheduleEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsDoctorScheduleEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsDoctorScheduleEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsDoctorScheduleEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsDoctorScheduleEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsDoctorScheduleEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsDoctorScheduleEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsDoctorScheduleEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsDoctorScheduleEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Doctor Schedule is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsDoctorScheduleEntity entity) {
	}
