...
import kmsweb.configs.security.auditing.services.InvoicePaymentSelfPayingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InvoicePaymentSelfPayingEntity.
 */
@Slf4j
@Component
public class InvoicePaymentSelfPayingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InvoicePaymentSelfPayingEntityReadAuditService auditService;
	...
	public void init(
		...
		InvoicePaymentSelfPayingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InvoicePaymentSelfPayingEntityListener.permissionEvaluator = permissionEvaluator;
		InvoicePaymentSelfPayingEntityListener.auditService = auditService;
		InvoicePaymentSelfPayingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InvoicePaymentSelfPayingEntity entity) {
	...
	@PostPersist
	public void afterPersist(InvoicePaymentSelfPayingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InvoicePaymentSelfPayingEntity entity) {
	...
	@PostRemove
	public void afterRemove(InvoicePaymentSelfPayingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InvoicePaymentSelfPayingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InvoicePaymentSelfPayingEntity entity) {
	...
	@PostLoad
	public void afterLoad(InvoicePaymentSelfPayingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InvoicePaymentSelfPayingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InvoicePaymentSelfPayingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Invoice Payment Self Paying is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InvoicePaymentSelfPayingEntity entity) {
		entity.unsetInvoice();
	}
