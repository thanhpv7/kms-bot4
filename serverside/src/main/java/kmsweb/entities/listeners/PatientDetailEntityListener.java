...
import kmsweb.configs.security.auditing.services.PatientDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientDetailEntity.
 */
@Slf4j
@Component
public class PatientDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientDetailEntityListener.permissionEvaluator = permissionEvaluator;
		PatientDetailEntityListener.auditService = auditService;
		PatientDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientDetailEntity entity) {
