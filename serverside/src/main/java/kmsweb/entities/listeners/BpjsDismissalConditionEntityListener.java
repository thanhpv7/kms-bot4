...
import kmsweb.configs.security.auditing.services.BpjsDismissalConditionEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsDismissalConditionEntity.
 */
@Slf4j
@Component
public class BpjsDismissalConditionEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsDismissalConditionEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsDismissalConditionEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsDismissalConditionEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsDismissalConditionEntityListener.auditService = auditService;
		BpjsDismissalConditionEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsDismissalConditionEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsDismissalConditionEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsDismissalConditionEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsDismissalConditionEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsDismissalConditionEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsDismissalConditionEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsDismissalConditionEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsDismissalConditionEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsDismissalConditionEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Dismissal Condition is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsDismissalConditionEntity entity) {
		entity.unsetBpjsClaimSubmission();
	}
