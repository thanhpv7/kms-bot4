...
import kmsweb.configs.security.auditing.services.InformedConsentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InformedConsentEntity.
 */
@Slf4j
@Component
public class InformedConsentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InformedConsentEntityReadAuditService auditService;
	...
	public void init(
		...
		InformedConsentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InformedConsentEntityListener.permissionEvaluator = permissionEvaluator;
		InformedConsentEntityListener.auditService = auditService;
		InformedConsentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InformedConsentEntity entity) {
	...
	@PostPersist
	public void afterPersist(InformedConsentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InformedConsentEntity entity) {
	...
	@PostRemove
	public void afterRemove(InformedConsentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InformedConsentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InformedConsentEntity entity) {
	...
	@PostLoad
	public void afterLoad(InformedConsentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InformedConsentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InformedConsentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Informed Consent is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InformedConsentEntity entity) {
		entity.unsetAccompanyingStaff();
		entity.unsetPatientGeneralInfo();
		entity.unsetRegistration();
		entity.unsetInformingStaff();
	}
