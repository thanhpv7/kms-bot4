...
import kmsweb.configs.security.auditing.services.CompoundPrescriptionDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing CompoundPrescriptionDetailEntity.
 */
@Slf4j
@Component
public class CompoundPrescriptionDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static CompoundPrescriptionDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		CompoundPrescriptionDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		CompoundPrescriptionDetailEntityListener.permissionEvaluator = permissionEvaluator;
		CompoundPrescriptionDetailEntityListener.auditService = auditService;
		CompoundPrescriptionDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(CompoundPrescriptionDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(CompoundPrescriptionDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(CompoundPrescriptionDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(CompoundPrescriptionDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(CompoundPrescriptionDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(CompoundPrescriptionDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(CompoundPrescriptionDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the CompoundPrescriptionDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the CompoundPrescriptionDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Compound Prescription Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(CompoundPrescriptionDetailEntity entity) {
		entity.unsetConsumptionUnit();
