...
import kmsweb.configs.security.auditing.services.BpjsUpdateDismissalDateEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsUpdateDismissalDateEntity.
 */
@Slf4j
@Component
public class BpjsUpdateDismissalDateEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsUpdateDismissalDateEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsUpdateDismissalDateEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsUpdateDismissalDateEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsUpdateDismissalDateEntityListener.auditService = auditService;
		BpjsUpdateDismissalDateEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsUpdateDismissalDateEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsUpdateDismissalDateEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsUpdateDismissalDateEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsUpdateDismissalDateEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsUpdateDismissalDateEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsUpdateDismissalDateEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsUpdateDismissalDateEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsUpdateDismissalDateEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsUpdateDismissalDateEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Update Dismissal Date is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsUpdateDismissalDateEntity entity) {
	}
