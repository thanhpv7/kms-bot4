...
import kmsweb.configs.security.auditing.services.NhisServiceOutcomeEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisServiceOutcomeEntity.
 */
@Slf4j
@Component
public class NhisServiceOutcomeEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisServiceOutcomeEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisServiceOutcomeEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisServiceOutcomeEntityListener.permissionEvaluator = permissionEvaluator;
		NhisServiceOutcomeEntityListener.auditService = auditService;
		NhisServiceOutcomeEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisServiceOutcomeEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisServiceOutcomeEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisServiceOutcomeEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisServiceOutcomeEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisServiceOutcomeEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisServiceOutcomeEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisServiceOutcomeEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisServiceOutcomeEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisServiceOutcomeEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Service Outcome is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisServiceOutcomeEntity entity) {
	}
