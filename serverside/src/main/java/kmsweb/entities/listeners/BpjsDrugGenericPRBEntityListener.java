...
import kmsweb.configs.security.auditing.services.BpjsDrugGenericPRBEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsDrugGenericPRBEntity.
 */
@Slf4j
@Component
public class BpjsDrugGenericPRBEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsDrugGenericPRBEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsDrugGenericPRBEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsDrugGenericPRBEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsDrugGenericPRBEntityListener.auditService = auditService;
		BpjsDrugGenericPRBEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsDrugGenericPRBEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsDrugGenericPRBEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsDrugGenericPRBEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsDrugGenericPRBEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsDrugGenericPRBEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsDrugGenericPRBEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsDrugGenericPRBEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsDrugGenericPRBEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsDrugGenericPRBEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Drug Generic PRB is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsDrugGenericPRBEntity entity) {
		entity.unsetStockCatalogue();
		entity.unsetBpjsPRBDetails();
	}
