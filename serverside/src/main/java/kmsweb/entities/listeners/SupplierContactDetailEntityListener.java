...
import kmsweb.configs.security.auditing.services.SupplierContactDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing SupplierContactDetailEntity.
 */
@Slf4j
@Component
public class SupplierContactDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static SupplierContactDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		SupplierContactDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		SupplierContactDetailEntityListener.permissionEvaluator = permissionEvaluator;
		SupplierContactDetailEntityListener.auditService = auditService;
		SupplierContactDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(SupplierContactDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(SupplierContactDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(SupplierContactDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(SupplierContactDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(SupplierContactDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(SupplierContactDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(SupplierContactDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the SupplierContactDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the SupplierContactDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Supplier Contact Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(SupplierContactDetailEntity entity) {
