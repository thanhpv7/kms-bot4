...
import kmsweb.configs.security.auditing.services.MedicationCompoundEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicationCompoundEntity.
 */
@Slf4j
@Component
public class MedicationCompoundEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicationCompoundEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicationCompoundEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicationCompoundEntityListener.permissionEvaluator = permissionEvaluator;
		MedicationCompoundEntityListener.auditService = auditService;
		MedicationCompoundEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicationCompoundEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicationCompoundEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicationCompoundEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicationCompoundEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicationCompoundEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicationCompoundEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicationCompoundEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicationCompoundEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicationCompoundEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medication Compound is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicationCompoundEntity entity) {
		entity.unsetMedicationHeader();
		entity.unsetUnitOfMeasurement();
		entity.unsetMedicationAdministeredHistories();
