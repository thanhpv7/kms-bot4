...
import kmsweb.configs.security.auditing.services.ExaminationGroupEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ExaminationGroupEntity.
 */
@Slf4j
@Component
public class ExaminationGroupEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ExaminationGroupEntityReadAuditService auditService;
	...
	public void init(
		...
		ExaminationGroupEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ExaminationGroupEntityListener.permissionEvaluator = permissionEvaluator;
		ExaminationGroupEntityListener.auditService = auditService;
		ExaminationGroupEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ExaminationGroupEntity entity) {
	...
	@PostPersist
	public void afterPersist(ExaminationGroupEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ExaminationGroupEntity entity) {
	...
	@PostRemove
	public void afterRemove(ExaminationGroupEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ExaminationGroupEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ExaminationGroupEntity entity) {
	...
	@PostLoad
	public void afterLoad(ExaminationGroupEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ExaminationGroupEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ExaminationGroupEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Examination Group is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ExaminationGroupEntity entity) {
		entity.unsetService();
