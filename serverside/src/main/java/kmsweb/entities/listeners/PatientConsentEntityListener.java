...
import kmsweb.configs.security.auditing.services.PatientConsentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientConsentEntity.
 */
@Slf4j
@Component
public class PatientConsentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientConsentEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientConsentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientConsentEntityListener.permissionEvaluator = permissionEvaluator;
		PatientConsentEntityListener.auditService = auditService;
		PatientConsentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientConsentEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientConsentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientConsentEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientConsentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientConsentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientConsentEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientConsentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientConsentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientConsentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Consent is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientConsentEntity entity) {
