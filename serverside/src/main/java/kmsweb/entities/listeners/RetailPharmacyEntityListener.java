...
import kmsweb.configs.security.auditing.services.RetailPharmacyEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing RetailPharmacyEntity.
 */
@Slf4j
@Component
public class RetailPharmacyEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static RetailPharmacyEntityReadAuditService auditService;
	...
	public void init(
		...
		RetailPharmacyEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		RetailPharmacyEntityListener.permissionEvaluator = permissionEvaluator;
		RetailPharmacyEntityListener.auditService = auditService;
		RetailPharmacyEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(RetailPharmacyEntity entity) {
	...
	@PostPersist
	public void afterPersist(RetailPharmacyEntity entity) {
	...
	@PreRemove
	public void beforeRemove(RetailPharmacyEntity entity) {
	...
	@PostRemove
	public void afterRemove(RetailPharmacyEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(RetailPharmacyEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(RetailPharmacyEntity entity) {
	...
	@PostLoad
	public void afterLoad(RetailPharmacyEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the RetailPharmacyEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the RetailPharmacyEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Retail Pharmacy is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(RetailPharmacyEntity entity) {
		entity.unsetPatientGeneralInfo();
		entity.unsetWarehouse();
		entity.unsetRetailPharmacyInvoices();
