...
import kmsweb.configs.security.auditing.services.PrivilegeEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PrivilegeEntity.
 */
@Slf4j
@Component
public class PrivilegeEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PrivilegeEntityReadAuditService auditService;
	...
	public void init(
		...
		PrivilegeEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PrivilegeEntityListener.permissionEvaluator = permissionEvaluator;
		PrivilegeEntityListener.auditService = auditService;
		PrivilegeEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PrivilegeEntity entity) {
	...
	@PostPersist
	public void afterPersist(PrivilegeEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PrivilegeEntity entity) {
	...
	@PostRemove
	public void afterRemove(PrivilegeEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PrivilegeEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PrivilegeEntity entity) {
	...
	@PostLoad
	public void afterLoad(PrivilegeEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PrivilegeEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PrivilegeEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Privilege is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PrivilegeEntity entity) {
		entity.unsetRoles();
	}
