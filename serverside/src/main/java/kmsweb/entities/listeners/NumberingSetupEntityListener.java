...
import kmsweb.configs.security.auditing.services.NumberingSetupEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NumberingSetupEntity.
 */
@Slf4j
@Component
public class NumberingSetupEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NumberingSetupEntityReadAuditService auditService;
	...
	public void init(
		...
		NumberingSetupEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NumberingSetupEntityListener.permissionEvaluator = permissionEvaluator;
		NumberingSetupEntityListener.auditService = auditService;
		NumberingSetupEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NumberingSetupEntity entity) {
	...
	@PostPersist
	public void afterPersist(NumberingSetupEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NumberingSetupEntity entity) {
	...
	@PostRemove
	public void afterRemove(NumberingSetupEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NumberingSetupEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NumberingSetupEntity entity) {
	...
	@PostLoad
	public void afterLoad(NumberingSetupEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NumberingSetupEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NumberingSetupEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Numbering Setup is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NumberingSetupEntity entity) {
	}
