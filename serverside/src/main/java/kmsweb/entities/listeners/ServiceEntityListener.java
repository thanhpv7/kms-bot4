...
import kmsweb.configs.security.auditing.services.ServiceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ServiceEntity.
 */
@Slf4j
@Component
public class ServiceEntityListener {
	...
	private static ServiceEntityReadAuditService auditService;
	...
	public void init(
		...
		ServiceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ServiceEntityListener.permissionEvaluator = permissionEvaluator;
		ServiceEntityListener.auditService = auditService;
		ServiceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ServiceEntity entity) {
	...
	@PostPersist
	public void afterPersist(ServiceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ServiceEntity entity) {
	...
	@PostRemove
	public void afterRemove(ServiceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ServiceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ServiceEntity entity) {
	...
	@PostLoad
	public void afterLoad(ServiceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ServiceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ServiceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Service is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ServiceEntity entity) {
		entity.unsetDefaultMedicalStaff();
		entity.unsetBpjsPcarePoli();
		entity.unsetBpjsServiceMapping();
		entity.unsetDoctorSchedules();
		entity.unsetExaminationGroups();
		entity.unsetMcuPackageItems();
		entity.unsetReferringUnit();
		entity.unsetRegistrations();
		entity.unsetServiceFacilityCategories();
		entity.unsetAssignedMedicalStaff();
		entity.unsetServiceItems();
	}
