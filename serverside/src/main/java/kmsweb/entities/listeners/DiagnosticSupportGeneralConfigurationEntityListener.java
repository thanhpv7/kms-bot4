...
import kmsweb.configs.security.auditing.services.DiagnosticSupportGeneralConfigurationEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DiagnosticSupportGeneralConfigurationEntity.
 */
@Slf4j
@Component
public class DiagnosticSupportGeneralConfigurationEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DiagnosticSupportGeneralConfigurationEntityReadAuditService auditService;
	...
	public void init(
		...
		DiagnosticSupportGeneralConfigurationEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DiagnosticSupportGeneralConfigurationEntityListener.permissionEvaluator = permissionEvaluator;
		DiagnosticSupportGeneralConfigurationEntityListener.auditService = auditService;
		DiagnosticSupportGeneralConfigurationEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DiagnosticSupportGeneralConfigurationEntity entity) {
	...
	@PostPersist
	public void afterPersist(DiagnosticSupportGeneralConfigurationEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DiagnosticSupportGeneralConfigurationEntity entity) {
	...
	@PostRemove
	public void afterRemove(DiagnosticSupportGeneralConfigurationEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DiagnosticSupportGeneralConfigurationEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DiagnosticSupportGeneralConfigurationEntity entity) {
	...
	@PostLoad
	public void afterLoad(DiagnosticSupportGeneralConfigurationEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DiagnosticSupportGeneralConfigurationEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DiagnosticSupportGeneralConfigurationEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Diagnostic Support General Configuration is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DiagnosticSupportGeneralConfigurationEntity entity) {
	}
