...
import kmsweb.configs.security.auditing.services.MedicalCertificateMedicalAttendanceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing MedicalCertificateMedicalAttendanceEntity.
 */
@Slf4j
@Component
public class MedicalCertificateMedicalAttendanceEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static MedicalCertificateMedicalAttendanceEntityReadAuditService auditService;
	...
	public void init(
		...
		MedicalCertificateMedicalAttendanceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		MedicalCertificateMedicalAttendanceEntityListener.permissionEvaluator = permissionEvaluator;
		MedicalCertificateMedicalAttendanceEntityListener.auditService = auditService;
		MedicalCertificateMedicalAttendanceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(MedicalCertificateMedicalAttendanceEntity entity) {
	...
	@PostPersist
	public void afterPersist(MedicalCertificateMedicalAttendanceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(MedicalCertificateMedicalAttendanceEntity entity) {
	...
	@PostRemove
	public void afterRemove(MedicalCertificateMedicalAttendanceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(MedicalCertificateMedicalAttendanceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(MedicalCertificateMedicalAttendanceEntity entity) {
	...
	@PostLoad
	public void afterLoad(MedicalCertificateMedicalAttendanceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the MedicalCertificateMedicalAttendanceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the MedicalCertificateMedicalAttendanceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Medical Certificate Medical Attendance is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(MedicalCertificateMedicalAttendanceEntity entity) {
