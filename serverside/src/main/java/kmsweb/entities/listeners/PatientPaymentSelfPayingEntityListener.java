...
import kmsweb.configs.security.auditing.services.PatientPaymentSelfPayingEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientPaymentSelfPayingEntity.
 */
@Slf4j
@Component
public class PatientPaymentSelfPayingEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientPaymentSelfPayingEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientPaymentSelfPayingEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientPaymentSelfPayingEntityListener.permissionEvaluator = permissionEvaluator;
		PatientPaymentSelfPayingEntityListener.auditService = auditService;
		PatientPaymentSelfPayingEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientPaymentSelfPayingEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientPaymentSelfPayingEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientPaymentSelfPayingEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientPaymentSelfPayingEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientPaymentSelfPayingEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientPaymentSelfPayingEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientPaymentSelfPayingEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientPaymentSelfPayingEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientPaymentSelfPayingEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Payment  Self Paying is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientPaymentSelfPayingEntity entity) {
