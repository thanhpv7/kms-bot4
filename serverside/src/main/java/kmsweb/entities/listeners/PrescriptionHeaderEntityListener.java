...
import kmsweb.configs.security.auditing.services.PrescriptionHeaderEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PrescriptionHeaderEntity.
 */
@Slf4j
@Component
public class PrescriptionHeaderEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PrescriptionHeaderEntityReadAuditService auditService;
	...
	public void init(
		...
		PrescriptionHeaderEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PrescriptionHeaderEntityListener.permissionEvaluator = permissionEvaluator;
		PrescriptionHeaderEntityListener.auditService = auditService;
		PrescriptionHeaderEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PrescriptionHeaderEntity entity) {
	...
	@PostPersist
	public void afterPersist(PrescriptionHeaderEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PrescriptionHeaderEntity entity) {
	...
	@PostRemove
	public void afterRemove(PrescriptionHeaderEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PrescriptionHeaderEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PrescriptionHeaderEntity entity) {
	...
	@PostLoad
	public void afterLoad(PrescriptionHeaderEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PrescriptionHeaderEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PrescriptionHeaderEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Prescription Header is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PrescriptionHeaderEntity entity) {
		entity.unsetInvoice();
		entity.unsetRegistration();
		entity.unsetRequestedBy();
		entity.unsetWarehouse();
		entity.unsetInvoiceItems();
		entity.unsetPrescriptionCompounds();
