...
import kmsweb.configs.security.auditing.services.BatchStockTransactionDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BatchStockTransactionDetailEntity.
 */
@Slf4j
@Component
public class BatchStockTransactionDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BatchStockTransactionDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		BatchStockTransactionDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BatchStockTransactionDetailEntityListener.permissionEvaluator = permissionEvaluator;
		BatchStockTransactionDetailEntityListener.auditService = auditService;
		BatchStockTransactionDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BatchStockTransactionDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(BatchStockTransactionDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BatchStockTransactionDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(BatchStockTransactionDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BatchStockTransactionDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BatchStockTransactionDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(BatchStockTransactionDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BatchStockTransactionDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BatchStockTransactionDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Batch Stock Transaction Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BatchStockTransactionDetailEntity entity) {
