...
import kmsweb.configs.security.auditing.services.PurchaseOrderStockDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PurchaseOrderStockDetailEntity.
 */
@Slf4j
@Component
public class PurchaseOrderStockDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PurchaseOrderStockDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		PurchaseOrderStockDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PurchaseOrderStockDetailEntityListener.permissionEvaluator = permissionEvaluator;
		PurchaseOrderStockDetailEntityListener.auditService = auditService;
		PurchaseOrderStockDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PurchaseOrderStockDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(PurchaseOrderStockDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PurchaseOrderStockDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(PurchaseOrderStockDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PurchaseOrderStockDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PurchaseOrderStockDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(PurchaseOrderStockDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PurchaseOrderStockDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PurchaseOrderStockDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Purchase Order Stock Detail is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PurchaseOrderStockDetailEntity entity) {
		entity.unsetPurchaseOrder();
		entity.unsetPurchaseRequisitionStockDetail();
