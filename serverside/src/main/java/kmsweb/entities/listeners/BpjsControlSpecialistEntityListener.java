...
import kmsweb.configs.security.auditing.services.BpjsControlSpecialistEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsControlSpecialistEntity.
 */
@Slf4j
@Component
public class BpjsControlSpecialistEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsControlSpecialistEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsControlSpecialistEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsControlSpecialistEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsControlSpecialistEntityListener.auditService = auditService;
		BpjsControlSpecialistEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsControlSpecialistEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsControlSpecialistEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsControlSpecialistEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsControlSpecialistEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsControlSpecialistEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsControlSpecialistEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsControlSpecialistEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsControlSpecialistEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsControlSpecialistEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Control Specialist is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsControlSpecialistEntity entity) {
	}
