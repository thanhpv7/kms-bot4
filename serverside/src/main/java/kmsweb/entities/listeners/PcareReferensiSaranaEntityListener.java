...
import kmsweb.configs.security.auditing.services.PcareReferensiSaranaEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareReferensiSaranaEntity.
 */
@Slf4j
@Component
public class PcareReferensiSaranaEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareReferensiSaranaEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareReferensiSaranaEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareReferensiSaranaEntityListener.permissionEvaluator = permissionEvaluator;
		PcareReferensiSaranaEntityListener.auditService = auditService;
		PcareReferensiSaranaEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareReferensiSaranaEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareReferensiSaranaEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareReferensiSaranaEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareReferensiSaranaEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareReferensiSaranaEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareReferensiSaranaEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareReferensiSaranaEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareReferensiSaranaEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareReferensiSaranaEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Referensi Sarana is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareReferensiSaranaEntity entity) {
	}
