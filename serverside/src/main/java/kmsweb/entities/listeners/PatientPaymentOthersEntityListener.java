...
import kmsweb.configs.security.auditing.services.PatientPaymentOthersEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientPaymentOthersEntity.
 */
@Slf4j
@Component
public class PatientPaymentOthersEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientPaymentOthersEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientPaymentOthersEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientPaymentOthersEntityListener.permissionEvaluator = permissionEvaluator;
		PatientPaymentOthersEntityListener.auditService = auditService;
		PatientPaymentOthersEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientPaymentOthersEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientPaymentOthersEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientPaymentOthersEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientPaymentOthersEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientPaymentOthersEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientPaymentOthersEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientPaymentOthersEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientPaymentOthersEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientPaymentOthersEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Payment Others is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientPaymentOthersEntity entity) {
