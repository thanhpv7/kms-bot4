...
import kmsweb.configs.security.auditing.services.NhisTypeOfServiceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing NhisTypeOfServiceEntity.
 */
@Slf4j
@Component
public class NhisTypeOfServiceEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static NhisTypeOfServiceEntityReadAuditService auditService;
	...
	public void init(
		...
		NhisTypeOfServiceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		NhisTypeOfServiceEntityListener.permissionEvaluator = permissionEvaluator;
		NhisTypeOfServiceEntityListener.auditService = auditService;
		NhisTypeOfServiceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(NhisTypeOfServiceEntity entity) {
	...
	@PostPersist
	public void afterPersist(NhisTypeOfServiceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(NhisTypeOfServiceEntity entity) {
	...
	@PostRemove
	public void afterRemove(NhisTypeOfServiceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(NhisTypeOfServiceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(NhisTypeOfServiceEntity entity) {
	...
	@PostLoad
	public void afterLoad(NhisTypeOfServiceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the NhisTypeOfServiceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the NhisTypeOfServiceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to NHIS Type Of Service is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(NhisTypeOfServiceEntity entity) {
	}
