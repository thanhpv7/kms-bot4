...
import kmsweb.configs.security.auditing.services.BpjsProcedureEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsProcedureEntity.
 */
@Slf4j
@Component
public class BpjsProcedureEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsProcedureEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsProcedureEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsProcedureEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsProcedureEntityListener.auditService = auditService;
		BpjsProcedureEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsProcedureEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsProcedureEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsProcedureEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsProcedureEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsProcedureEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsProcedureEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsProcedureEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsProcedureEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsProcedureEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Procedure is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsProcedureEntity entity) {
