...
import kmsweb.configs.security.auditing.services.PcareCompoundDrugEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareCompoundDrugEntity.
 */
@Slf4j
@Component
public class PcareCompoundDrugEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareCompoundDrugEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareCompoundDrugEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareCompoundDrugEntityListener.permissionEvaluator = permissionEvaluator;
		PcareCompoundDrugEntityListener.auditService = auditService;
		PcareCompoundDrugEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareCompoundDrugEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareCompoundDrugEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareCompoundDrugEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareCompoundDrugEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareCompoundDrugEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareCompoundDrugEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareCompoundDrugEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareCompoundDrugEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareCompoundDrugEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Compound Drug is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareCompoundDrugEntity entity) {
		entity.unsetPcareExamination();
		entity.unsetPcareCompoundDrugItems();
	}
