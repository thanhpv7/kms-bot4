...
import kmsweb.configs.security.auditing.services.PcareDiagnosaEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareDiagnosaEntity.
 */
@Slf4j
@Component
public class PcareDiagnosaEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareDiagnosaEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareDiagnosaEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareDiagnosaEntityListener.permissionEvaluator = permissionEvaluator;
		PcareDiagnosaEntityListener.auditService = auditService;
		PcareDiagnosaEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareDiagnosaEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareDiagnosaEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareDiagnosaEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareDiagnosaEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareDiagnosaEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareDiagnosaEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareDiagnosaEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareDiagnosaEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareDiagnosaEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Diagnosa is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareDiagnosaEntity entity) {
	}
