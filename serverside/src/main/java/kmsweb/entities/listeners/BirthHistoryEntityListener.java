...
import kmsweb.configs.security.auditing.services.BirthHistoryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BirthHistoryEntity.
 */
@Slf4j
@Component
public class BirthHistoryEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BirthHistoryEntityReadAuditService auditService;
	...
	public void init(
		...
		BirthHistoryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BirthHistoryEntityListener.permissionEvaluator = permissionEvaluator;
		BirthHistoryEntityListener.auditService = auditService;
		BirthHistoryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BirthHistoryEntity entity) {
	...
	@PostPersist
	public void afterPersist(BirthHistoryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BirthHistoryEntity entity) {
	...
	@PostRemove
	public void afterRemove(BirthHistoryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BirthHistoryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BirthHistoryEntity entity) {
	...
	@PostLoad
	public void afterLoad(BirthHistoryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BirthHistoryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BirthHistoryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Birth History is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BirthHistoryEntity entity) {
