...
import kmsweb.configs.security.auditing.services.PatientPaymentInsuranceEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientPaymentInsuranceEntity.
 */
@Slf4j
@Component
public class PatientPaymentInsuranceEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientPaymentInsuranceEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientPaymentInsuranceEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientPaymentInsuranceEntityListener.permissionEvaluator = permissionEvaluator;
		PatientPaymentInsuranceEntityListener.auditService = auditService;
		PatientPaymentInsuranceEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientPaymentInsuranceEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientPaymentInsuranceEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientPaymentInsuranceEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientPaymentInsuranceEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientPaymentInsuranceEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientPaymentInsuranceEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientPaymentInsuranceEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientPaymentInsuranceEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientPaymentInsuranceEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Payment Insurance is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientPaymentInsuranceEntity entity) {
