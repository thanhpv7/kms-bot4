...
import kmsweb.configs.security.auditing.services.PcareClubProlanisEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PcareClubProlanisEntity.
 */
@Slf4j
@Component
public class PcareClubProlanisEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PcareClubProlanisEntityReadAuditService auditService;
	...
	public void init(
		...
		PcareClubProlanisEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PcareClubProlanisEntityListener.permissionEvaluator = permissionEvaluator;
		PcareClubProlanisEntityListener.auditService = auditService;
		PcareClubProlanisEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PcareClubProlanisEntity entity) {
	...
	@PostPersist
	public void afterPersist(PcareClubProlanisEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PcareClubProlanisEntity entity) {
	...
	@PostRemove
	public void afterRemove(PcareClubProlanisEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PcareClubProlanisEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PcareClubProlanisEntity entity) {
	...
	@PostLoad
	public void afterLoad(PcareClubProlanisEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PcareClubProlanisEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PcareClubProlanisEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to PCare Club Prolanis is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PcareClubProlanisEntity entity) {
	}
