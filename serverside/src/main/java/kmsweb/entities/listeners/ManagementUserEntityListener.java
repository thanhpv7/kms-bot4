...
import kmsweb.configs.security.auditing.services.ManagementUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing ManagementUserEntity.
 */
@Slf4j
@Component
public class ManagementUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static ManagementUserEntityReadAuditService auditService;
	...
	public void init(
		...
		ManagementUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		ManagementUserEntityListener.permissionEvaluator = permissionEvaluator;
		ManagementUserEntityListener.auditService = auditService;
		ManagementUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(ManagementUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(ManagementUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(ManagementUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(ManagementUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(ManagementUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(ManagementUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(ManagementUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the ManagementUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the ManagementUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Management User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(ManagementUserEntity entity) {
		entity.unsetStaff();
	}
