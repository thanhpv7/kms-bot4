...
import kmsweb.configs.security.auditing.services.FacilityHistoryEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing FacilityHistoryEntity.
 */
@Slf4j
@Component
public class FacilityHistoryEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static FacilityHistoryEntityReadAuditService auditService;
	...
	public void init(
		...
		FacilityHistoryEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		FacilityHistoryEntityListener.permissionEvaluator = permissionEvaluator;
		FacilityHistoryEntityListener.auditService = auditService;
		FacilityHistoryEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(FacilityHistoryEntity entity) {
	...
	@PostPersist
	public void afterPersist(FacilityHistoryEntity entity) {
	...
	@PreRemove
	public void beforeRemove(FacilityHistoryEntity entity) {
	...
	@PostRemove
	public void afterRemove(FacilityHistoryEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(FacilityHistoryEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(FacilityHistoryEntity entity) {
	...
	@PostLoad
	public void afterLoad(FacilityHistoryEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the FacilityHistoryEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the FacilityHistoryEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Facility History is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(FacilityHistoryEntity entity) {
	}
