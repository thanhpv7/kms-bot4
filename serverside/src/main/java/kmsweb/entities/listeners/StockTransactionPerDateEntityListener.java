...
import kmsweb.configs.security.auditing.services.StockTransactionPerDateEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing StockTransactionPerDateEntity.
 */
@Slf4j
@Component
public class StockTransactionPerDateEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static StockTransactionPerDateEntityReadAuditService auditService;
	...
	public void init(
		...
		StockTransactionPerDateEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		StockTransactionPerDateEntityListener.permissionEvaluator = permissionEvaluator;
		StockTransactionPerDateEntityListener.auditService = auditService;
		StockTransactionPerDateEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(StockTransactionPerDateEntity entity) {
	...
	@PostPersist
	public void afterPersist(StockTransactionPerDateEntity entity) {
	...
	@PreRemove
	public void beforeRemove(StockTransactionPerDateEntity entity) {
	...
	@PostRemove
	public void afterRemove(StockTransactionPerDateEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(StockTransactionPerDateEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(StockTransactionPerDateEntity entity) {
	...
	@PostLoad
	public void afterLoad(StockTransactionPerDateEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the StockTransactionPerDateEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the StockTransactionPerDateEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Stock Transaction Per Date is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(StockTransactionPerDateEntity entity) {
