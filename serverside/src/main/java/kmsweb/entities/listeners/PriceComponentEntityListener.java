...
import kmsweb.configs.security.auditing.services.PriceComponentEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PriceComponentEntity.
 */
@Slf4j
@Component
public class PriceComponentEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PriceComponentEntityReadAuditService auditService;
	...
	public void init(
		...
		PriceComponentEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PriceComponentEntityListener.permissionEvaluator = permissionEvaluator;
		PriceComponentEntityListener.auditService = auditService;
		PriceComponentEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PriceComponentEntity entity) {
	...
	@PostPersist
	public void afterPersist(PriceComponentEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PriceComponentEntity entity) {
	...
	@PostRemove
	public void afterRemove(PriceComponentEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PriceComponentEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PriceComponentEntity entity) {
	...
	@PostLoad
	public void afterLoad(PriceComponentEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PriceComponentEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PriceComponentEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Price Component is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PriceComponentEntity entity) {
		entity.unsetInvoiceItemComponents();
		entity.unsetMedicalFeeItemComponents();
		entity.unsetTariffDefinitionOtherComponents();
	}
