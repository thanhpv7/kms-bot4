...
import kmsweb.configs.security.auditing.services.RegistrationUserEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing RegistrationUserEntity.
 */
@Slf4j
@Component
public class RegistrationUserEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static RegistrationUserEntityReadAuditService auditService;
	...
	public void init(
		...
		RegistrationUserEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		RegistrationUserEntityListener.permissionEvaluator = permissionEvaluator;
		RegistrationUserEntityListener.auditService = auditService;
		RegistrationUserEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(RegistrationUserEntity entity) {
	...
	@PostPersist
	public void afterPersist(RegistrationUserEntity entity) {
	...
	@PreRemove
	public void beforeRemove(RegistrationUserEntity entity) {
	...
	@PostRemove
	public void afterRemove(RegistrationUserEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(RegistrationUserEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(RegistrationUserEntity entity) {
	...
	@PostLoad
	public void afterLoad(RegistrationUserEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the RegistrationUserEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the RegistrationUserEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Registration User is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(RegistrationUserEntity entity) {
		entity.unsetStaff();
	}
