...
import kmsweb.configs.security.auditing.services.InventoryBatchCorrectionItemEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing InventoryBatchCorrectionItemEntity.
 */
@Slf4j
@Component
public class InventoryBatchCorrectionItemEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static InventoryBatchCorrectionItemEntityReadAuditService auditService;
	...
	public void init(
		...
		InventoryBatchCorrectionItemEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		InventoryBatchCorrectionItemEntityListener.permissionEvaluator = permissionEvaluator;
		InventoryBatchCorrectionItemEntityListener.auditService = auditService;
		InventoryBatchCorrectionItemEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(InventoryBatchCorrectionItemEntity entity) {
	...
	@PostPersist
	public void afterPersist(InventoryBatchCorrectionItemEntity entity) {
	...
	@PreRemove
	public void beforeRemove(InventoryBatchCorrectionItemEntity entity) {
	...
	@PostRemove
	public void afterRemove(InventoryBatchCorrectionItemEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(InventoryBatchCorrectionItemEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(InventoryBatchCorrectionItemEntity entity) {
	...
	@PostLoad
	public void afterLoad(InventoryBatchCorrectionItemEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the InventoryBatchCorrectionItemEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the InventoryBatchCorrectionItemEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Inventory Batch Correction Item is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(InventoryBatchCorrectionItemEntity entity) {
		entity.unsetInventoryBatchCorrection();
