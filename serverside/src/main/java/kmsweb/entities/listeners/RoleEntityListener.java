...
import kmsweb.configs.security.auditing.services.RoleEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing RoleEntity.
 */
@Slf4j
@Component
public class RoleEntityListener {
	...
	private static RoleEntityReadAuditService auditService;
	...
	public void init(
		...
		RoleEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		RoleEntityListener.permissionEvaluator = permissionEvaluator;
		RoleEntityListener.auditService = auditService;
		RoleEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(RoleEntity entity) {
	...
	@PostPersist
	public void afterPersist(RoleEntity entity) {
	...
	@PreRemove
	public void beforeRemove(RoleEntity entity) {
	...
	@PostRemove
	public void afterRemove(RoleEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(RoleEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(RoleEntity entity) {
	...
	@PostLoad
	public void afterLoad(RoleEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the RoleEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the RoleEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Role is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(RoleEntity entity) {
		entity.unsetUsers();
		entity.unsetPrivileges();
	}
