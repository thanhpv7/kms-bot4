...
import kmsweb.configs.security.auditing.services.DoctorScheduleEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing DoctorScheduleEntity.
 */
@Slf4j
@Component
public class DoctorScheduleEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static DoctorScheduleEntityReadAuditService auditService;
	...
	public void init(
		...
		DoctorScheduleEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		DoctorScheduleEntityListener.permissionEvaluator = permissionEvaluator;
		DoctorScheduleEntityListener.auditService = auditService;
		DoctorScheduleEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(DoctorScheduleEntity entity) {
	...
	@PostPersist
	public void afterPersist(DoctorScheduleEntity entity) {
	...
	@PreRemove
	public void beforeRemove(DoctorScheduleEntity entity) {
	...
	@PostRemove
	public void afterRemove(DoctorScheduleEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(DoctorScheduleEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(DoctorScheduleEntity entity) {
	...
	@PostLoad
	public void afterLoad(DoctorScheduleEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the DoctorScheduleEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the DoctorScheduleEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Doctor Schedule is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(DoctorScheduleEntity entity) {
		entity.unsetService();
		entity.unsetStaff();
	}
