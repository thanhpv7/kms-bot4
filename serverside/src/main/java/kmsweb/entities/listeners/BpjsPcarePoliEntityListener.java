...
import kmsweb.configs.security.auditing.services.BpjsPcarePoliEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsPcarePoliEntity.
 */
@Slf4j
@Component
public class BpjsPcarePoliEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsPcarePoliEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsPcarePoliEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsPcarePoliEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsPcarePoliEntityListener.auditService = auditService;
		BpjsPcarePoliEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsPcarePoliEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsPcarePoliEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsPcarePoliEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsPcarePoliEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsPcarePoliEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsPcarePoliEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsPcarePoliEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsPcarePoliEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsPcarePoliEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Pcare Poli is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsPcarePoliEntity entity) {
		entity.unsetService();
	}
