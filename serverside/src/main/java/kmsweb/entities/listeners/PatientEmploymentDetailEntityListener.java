...
import kmsweb.configs.security.auditing.services.PatientEmploymentDetailEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing PatientEmploymentDetailEntity.
 */
@Slf4j
@Component
public class PatientEmploymentDetailEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static PatientEmploymentDetailEntityReadAuditService auditService;
	...
	public void init(
		...
		PatientEmploymentDetailEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		PatientEmploymentDetailEntityListener.permissionEvaluator = permissionEvaluator;
		PatientEmploymentDetailEntityListener.auditService = auditService;
		PatientEmploymentDetailEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(PatientEmploymentDetailEntity entity) {
	...
	@PostPersist
	public void afterPersist(PatientEmploymentDetailEntity entity) {
	...
	@PreRemove
	public void beforeRemove(PatientEmploymentDetailEntity entity) {
	...
	@PostRemove
	public void afterRemove(PatientEmploymentDetailEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(PatientEmploymentDetailEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(PatientEmploymentDetailEntity entity) {
	...
	@PostLoad
	public void afterLoad(PatientEmploymentDetailEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the PatientEmploymentDetailEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the PatientEmploymentDetailEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to Patient Employment Detail  is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(PatientEmploymentDetailEntity entity) {
