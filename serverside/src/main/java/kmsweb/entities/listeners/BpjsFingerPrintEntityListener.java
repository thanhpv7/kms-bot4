...
import kmsweb.configs.security.auditing.services.BpjsFingerPrintEntityReadAuditService;
...
/**
 * Listener class used to tap into life cycle hooks when processing BpjsFingerPrintEntity.
 */
@Slf4j
@Component
public class BpjsFingerPrintEntityListener {

	private static PermissionEvaluator permissionEvaluator;
	private static BpjsFingerPrintEntityReadAuditService auditService;
	...
	public void init(
		...
		BpjsFingerPrintEntityReadAuditService auditService,
		SecurityProperties securityProperties
	) {
		BpjsFingerPrintEntityListener.permissionEvaluator = permissionEvaluator;
		BpjsFingerPrintEntityListener.auditService = auditService;
		BpjsFingerPrintEntityListener.securityProperties = securityProperties;
	}

	@PrePersist
	public void beforePersist(BpjsFingerPrintEntity entity) {
	...
	@PostPersist
	public void afterPersist(BpjsFingerPrintEntity entity) {
	...
	@PreRemove
	public void beforeRemove(BpjsFingerPrintEntity entity) {
	...
	@PostRemove
	public void afterRemove(BpjsFingerPrintEntity entity) {
	...
	@PreUpdate
	public void beforeUpdate(BpjsFingerPrintEntity entity) {
	...
	@PostUpdate
	public void afterUpdate(BpjsFingerPrintEntity entity) {
	...
	@PostLoad
	public void afterLoad(BpjsFingerPrintEntity entity) throws JsonProcessingException {
		checkPermission(entity.getClass().getSimpleName(), "read");

		// % protected region % [Override the global configuration for read audits for the BpjsFingerPrintEntity here] off begin
		if (securityProperties.isReadAuditEnabled()) {
			// Create a new read record against this entity.
			auditService.createWith(entity);
		}
		// % protected region % [Override the global configuration for read audits for the BpjsFingerPrintEntity here] end
	...
	private void checkPermission(@NonNull String entityName, @NonNull String permission) {
		if (!permissionEvaluator.hasPermission(SecurityContextHolder.getContext().getAuthentication(), entityName, permission)) {
			...
			throw new AccessDeniedException("Access to BPJS Finger Print is not permitted.");
		}
	}
	 ...
	 */
	protected void removeAllReferences(BpjsFingerPrintEntity entity) {
	}
