...
import kmsweb.lib.services.email.*;
 ...
 * to {@link kmsweb.services.UserService}. Whereas {@link kmsweb.services.UserService} is simply used as a base
...
public class UserService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		// Load the username from the userEntity to ensure we are referencing the same key each time
		// A username can retrieved from the username field or from the email field.
		loggedInUsers.put(userEntity.getUsername(), userEntity);
	...
	public void forgottenPassword(@NonNull String email) throws Exception {
		...
		emailToSend.setSenderEmailAddress(applicationProperties.getSystemAdminEmail());
	...
	public UserEntity resetPassword(@NonNull String token, @NonNull String username, @NonNull String encodedPassword) {
		...
		if (!userEntity.getResetPasswordToken().remove(passwordResetToken)) {
			log.error("Failed to remove the password reset token");
		}
	...
	private String generateResetPasswordUrl(@NonNull UserEntity userEntity) {
		...
		TokenEntity tokenEntity = tokenService.generateTokenWithExpirationTime(securityProperties.getUserPasswordResetExpirationTimeSeconds());
		...
		String resetPasswordUrl = String.format("%s/reset-password?token=%s&username=%s",
				applicationProperties.getClientSideHost(), tokenEntity.getToken(), userEntity.getUsername());
