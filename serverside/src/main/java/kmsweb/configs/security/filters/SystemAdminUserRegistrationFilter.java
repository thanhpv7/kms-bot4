 ...
 */
public class SystemAdminUserRegistrationFilter extends AbstractRegistrationFilter<SystemAdminUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public SystemAdminUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull SystemAdminUserRepository userRepository,
	...
	public void doFilter(
		...
		final SystemAdminUserEntity systemAdminUserEntity = objectMapper.convertValue(paramMap, SystemAdminUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(systemAdminUserEntity::addRoles));
		...
		systemAdminUserEntity.setPassword(passwordEncoder.encode(systemAdminUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(systemAdminUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> systemAdminUser = roleRepository.findByName("SYSTEM_ADMIN_USER");
		if (systemAdminUser.isEmpty()) {
			throw new RuntimeException("Role systemAdminUser not present on the server.");
		}
