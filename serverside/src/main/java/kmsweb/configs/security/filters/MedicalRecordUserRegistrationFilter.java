 ...
 */
public class MedicalRecordUserRegistrationFilter extends AbstractRegistrationFilter<MedicalRecordUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public MedicalRecordUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull MedicalRecordUserRepository userRepository,
	...
	public void doFilter(
		...
		final MedicalRecordUserEntity medicalRecordUserEntity = objectMapper.convertValue(paramMap, MedicalRecordUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(medicalRecordUserEntity::addRoles));
		...
		medicalRecordUserEntity.setPassword(passwordEncoder.encode(medicalRecordUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(medicalRecordUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> medicalRecordUser = roleRepository.findByName("MEDICAL_RECORD_USER");
		if (medicalRecordUser.isEmpty()) {
			throw new RuntimeException("Role medicalRecordUser not present on the server.");
		}
