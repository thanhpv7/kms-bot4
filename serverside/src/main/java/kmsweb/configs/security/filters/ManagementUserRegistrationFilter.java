 ...
 */
public class ManagementUserRegistrationFilter extends AbstractRegistrationFilter<ManagementUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public ManagementUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull ManagementUserRepository userRepository,
	...
	public void doFilter(
		...
		final ManagementUserEntity managementUserEntity = objectMapper.convertValue(paramMap, ManagementUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(managementUserEntity::addRoles));
		...
		managementUserEntity.setPassword(passwordEncoder.encode(managementUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(managementUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> managementUser = roleRepository.findByName("MANAGEMENT_USER");
		if (managementUser.isEmpty()) {
			throw new RuntimeException("Role managementUser not present on the server.");
		}
		roles.add(managementUser.get());
