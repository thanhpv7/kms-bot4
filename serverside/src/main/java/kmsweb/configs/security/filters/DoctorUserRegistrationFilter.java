 ...
 */
public class DoctorUserRegistrationFilter extends AbstractRegistrationFilter<DoctorUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public DoctorUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull DoctorUserRepository userRepository,
	...
	public void doFilter(
		...
		final DoctorUserEntity doctorUserEntity = objectMapper.convertValue(paramMap, DoctorUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(doctorUserEntity::addRoles));
		...
		doctorUserEntity.setPassword(passwordEncoder.encode(doctorUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(doctorUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> doctorUser = roleRepository.findByName("DOCTOR_USER");
		if (doctorUser.isEmpty()) {
			throw new RuntimeException("Role doctorUser not present on the server.");
		}
