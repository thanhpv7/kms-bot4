 ...
 */
public class MedicalTranscriberUserRegistrationFilter extends AbstractRegistrationFilter<MedicalTranscriberUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public MedicalTranscriberUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull MedicalTranscriberUserRepository userRepository,
	...
	public void doFilter(
		...
		final MedicalTranscriberUserEntity medicalTranscriberUserEntity = objectMapper.convertValue(paramMap, MedicalTranscriberUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(medicalTranscriberUserEntity::addRoles));
		...
		medicalTranscriberUserEntity.setPassword(passwordEncoder.encode(medicalTranscriberUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(medicalTranscriberUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> medicalTranscriberUser = roleRepository.findByName("MEDICAL_TRANSCRIBER_USER");
		if (medicalTranscriberUser.isEmpty()) {
			throw new RuntimeException("Role medicalTranscriberUser not present on the server.");
		}
