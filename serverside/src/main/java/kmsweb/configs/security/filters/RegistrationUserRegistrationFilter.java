 ...
 */
public class RegistrationUserRegistrationFilter extends AbstractRegistrationFilter<RegistrationUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public RegistrationUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull RegistrationUserRepository userRepository,
	...
	public void doFilter(
		...
		final RegistrationUserEntity registrationUserEntity = objectMapper.convertValue(paramMap, RegistrationUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(registrationUserEntity::addRoles));
		...
		registrationUserEntity.setPassword(passwordEncoder.encode(registrationUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(registrationUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> registrationUser = roleRepository.findByName("REGISTRATION_USER");
		if (registrationUser.isEmpty()) {
			throw new RuntimeException("Role registrationUser not present on the server.");
		}
