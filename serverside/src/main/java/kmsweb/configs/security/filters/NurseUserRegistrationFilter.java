 ...
 */
public class NurseUserRegistrationFilter extends AbstractRegistrationFilter<NurseUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public NurseUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull NurseUserRepository userRepository,
	...
	public void doFilter(
		...
		final NurseUserEntity nurseUserEntity = objectMapper.convertValue(paramMap, NurseUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(nurseUserEntity::addRoles));
		...
		nurseUserEntity.setPassword(passwordEncoder.encode(nurseUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(nurseUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> nurseUser = roleRepository.findByName("NURSE_USER");
		if (nurseUser.isEmpty()) {
			throw new RuntimeException("Role nurseUser not present on the server.");
		}
