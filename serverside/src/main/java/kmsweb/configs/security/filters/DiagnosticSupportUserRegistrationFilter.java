 ...
 */
public class DiagnosticSupportUserRegistrationFilter extends AbstractRegistrationFilter<DiagnosticSupportUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public DiagnosticSupportUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull DiagnosticSupportUserRepository userRepository,
	...
	public void doFilter(
		...
		final DiagnosticSupportUserEntity diagnosticSupportUserEntity = objectMapper.convertValue(paramMap, DiagnosticSupportUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(diagnosticSupportUserEntity::addRoles));
		...
		diagnosticSupportUserEntity.setPassword(passwordEncoder.encode(diagnosticSupportUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(diagnosticSupportUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> diagnosticSupportUser = roleRepository.findByName("DIAGNOSTIC_SUPPORT_USER");
		if (diagnosticSupportUser.isEmpty()) {
			throw new RuntimeException("Role diagnosticSupportUser not present on the server.");
		}
		roles.add(diagnosticSupportUser.get());
