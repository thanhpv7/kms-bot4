 ...
 */
public class PharmacyUserRegistrationFilter extends AbstractRegistrationFilter<PharmacyUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public PharmacyUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull PharmacyUserRepository userRepository,
	...
	public void doFilter(
		...
		final PharmacyUserEntity pharmacyUserEntity = objectMapper.convertValue(paramMap, PharmacyUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(pharmacyUserEntity::addRoles));
		...
		pharmacyUserEntity.setPassword(passwordEncoder.encode(pharmacyUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(pharmacyUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> pharmacyUser = roleRepository.findByName("PHARMACY_USER");
		if (pharmacyUser.isEmpty()) {
			throw new RuntimeException("Role pharmacyUser not present on the server.");
		}
