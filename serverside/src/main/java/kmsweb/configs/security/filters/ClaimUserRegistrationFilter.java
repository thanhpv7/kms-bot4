 ...
 */
public class ClaimUserRegistrationFilter extends AbstractRegistrationFilter<ClaimUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public ClaimUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull ClaimUserRepository userRepository,
	...
	public void doFilter(
		...
		final ClaimUserEntity claimUserEntity = objectMapper.convertValue(paramMap, ClaimUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(claimUserEntity::addRoles));
		...
		claimUserEntity.setPassword(passwordEncoder.encode(claimUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(claimUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> claimUser = roleRepository.findByName("CLAIM_USER");
		if (claimUser.isEmpty()) {
			throw new RuntimeException("Role claimUser not present on the server.");
		}
