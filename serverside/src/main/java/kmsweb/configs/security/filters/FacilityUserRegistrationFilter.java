 ...
 */
public class FacilityUserRegistrationFilter extends AbstractRegistrationFilter<FacilityUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public FacilityUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull FacilityUserRepository userRepository,
	...
	public void doFilter(
		...
		final FacilityUserEntity facilityUserEntity = objectMapper.convertValue(paramMap, FacilityUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(facilityUserEntity::addRoles));
		...
		facilityUserEntity.setPassword(passwordEncoder.encode(facilityUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(facilityUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> facilityUser = roleRepository.findByName("FACILITY_USER");
		if (facilityUser.isEmpty()) {
			throw new RuntimeException("Role facilityUser not present on the server.");
		}
