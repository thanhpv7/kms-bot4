 ...
 */
public class CashierUserRegistrationFilter extends AbstractRegistrationFilter<CashierUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public CashierUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull CashierUserRepository userRepository,
	...
	public void doFilter(
		...
		final CashierUserEntity cashierUserEntity = objectMapper.convertValue(paramMap, CashierUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(cashierUserEntity::addRoles));
		...
		cashierUserEntity.setPassword(passwordEncoder.encode(cashierUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(cashierUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> cashierUser = roleRepository.findByName("CASHIER_USER");
		if (cashierUser.isEmpty()) {
			throw new RuntimeException("Role cashierUser not present on the server.");
		}
