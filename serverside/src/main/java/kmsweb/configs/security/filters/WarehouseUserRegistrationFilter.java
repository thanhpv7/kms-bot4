 ...
 */
public class WarehouseUserRegistrationFilter extends AbstractRegistrationFilter<WarehouseUserRepository> {

	// % protected region % [Add any additional class fields here] off begin
	// % protected region % [Add any additional class fields here] end

	public WarehouseUserRegistrationFilter(
			@NonNull String registrationEndpoint,
			@NonNull AuthenticationService authService,
			@NonNull WarehouseUserRepository userRepository,
	...
	public void doFilter(
		...
		final WarehouseUserEntity warehouseUserEntity = objectMapper.convertValue(paramMap, WarehouseUserEntity.class);
		...
		AnonymousHelper.runAnonymously(() -> defaultRoles.forEach(warehouseUserEntity::addRoles));
		...
		warehouseUserEntity.setPassword(passwordEncoder.encode(warehouseUserEntity.getPassword()));
		...
		AnonymousHelper.runAnonymously(() -> userRepository.save(warehouseUserEntity));
	...
	protected List<RoleEntity> getDefaultRoles() {
		...
		Optional<RoleEntity> warehouseUser = roleRepository.findByName("WAREHOUSE_USER");
		if (warehouseUser.isEmpty()) {
			throw new RuntimeException("Role warehouseUser not present on the server.");
		}
