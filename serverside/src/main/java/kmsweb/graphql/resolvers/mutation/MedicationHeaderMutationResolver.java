...
@Slf4j
@Component
public class MedicationHeaderMutationResolver implements GraphQLMutationResolver {
	...
	@Autowired
	public MedicationHeaderMutationResolver(
	...
	public MedicationHeaderEntity createMedicationHeader(@NonNull MedicationHeaderEntity rawEntity, Boolean persistRelations) {
		...
		MedicationHeaderEntity newEntity = medicationHeaderService.create(rawEntity, persistRelations);
	...
	/**
	 * Similar to {@link MedicationHeaderMutationResolver#createMedicationHeader(MedicationHeaderEntity, Boolean)}
	 * but with multiple entities at once.
	 */
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'create')")
	public List<MedicationHeaderEntity> createAllMedicationHeader(@NonNull List<MedicationHeaderEntity> rawEntities) {
		List<MedicationHeaderEntity> newEntities = Lists.newArrayList(medicationHeaderService.createAll(rawEntities));
	...
	public MedicationHeaderEntity updateMedicationHeader(@NonNull MedicationHeaderEntity rawEntity, Boolean persistRelations) {
		if (persistRelations == null) {
			persistRelations = false;
		}
		...
		MedicationHeaderEntity entityFromDb = medicationHeaderService.findById(rawEntity.getId()).orElseThrow();
		rawEntity.setCreatedBy(entityFromDb.getCreatedBy());
		MedicationHeaderEntity newEntity = medicationHeaderService.update(rawEntity, persistRelations);
	...
	/**
	 * Similar to {@link MedicationHeaderMutationResolver#updateMedicationHeader(MedicationHeaderEntity, Boolean)}
	 * but with multiple entities at once.
	 */
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'update')")
	public List<MedicationHeaderEntity> updateAllMedicationHeader(@NonNull List<MedicationHeaderEntity> rawEntities) {
		List<MedicationHeaderEntity> newEntities = Lists.newArrayList(medicationHeaderService.updateAll(rawEntities));
	...
	public String deleteMedicationHeaderById(@NonNull String id) {
		medicationHeaderService.deleteById(UUID.fromString(id));
	...
	/**
	 * Similar to {@link MedicationHeaderMutationResolver#deleteMedicationHeaderById(String)}
	 * but with multiple entities at once.
	 */
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'delete')")
	public List<String> deleteAllMedicationHeaderByIds(@NonNull List<String> ids) {
		medicationHeaderService.deleteAllByIds(ids.stream().map(UUID::fromString).collect(Collectors.toList()));
	...
	@PreAuthorize("hasPermission('MedicationHeaderEntity', 'delete')")
	public List<String> deleteMedicationHeaderExcludingIds(@NonNull List<String> ids) {
		medicationHeaderService.deleteAllExcludingIds(ids.stream().map(UUID::fromString).collect(Collectors.toList()));
