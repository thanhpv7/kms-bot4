...
import { 
	getMcuPackageItemCollectionModels,
	getCountMcuPackageItemModels,
	getMcuPackageItemModelWithId
} from 'src/app/models/mcuPackageItem/mcu_package_item.model.selector';
import { McuPackageItemCrudModule } from '../mcu-package-item-crud.module';
import { McuPackageItemCrudEditComponent } from './mcu-package-item-crud-edit.component';
import {
	McuPackageItemModelState,
	initialState as initialMcuPackageItemModelState
} from 'src/app/models/mcuPackageItem/mcu_package_item.model.state';
...
import { CrudTileMode } from '../mcu-package-item-crud.component';
...
describe('MCU Package Item CRUD Edit component tests', () => {
	let fixture: ComponentFixture<McuPackageItemCrudEditComponent>
	let component: McuPackageItemCrudEditComponent;

	let mcuPackageItemModelState: McuPackageItemModelState;

	let store: MockStore<{ model: McuPackageItemModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let mcuPackageItemCountBehaviorSubject: BehaviorSubject<number>;
	...
	let mcuPackageItemWithId: McuPackageItemModel = defaultMcuPackageItemEntities[0];
	let mcuPackageItemModelWithIdBehaviorSubject: BehaviorSubject<McuPackageItemModel>;

	mcuPackageItemWithId.mcuPackage = defaultMcuPackageEntities[0];
	mcuPackageItemWithId.mcuPackageId = mcuPackageItemWithId.mcuPackage.id;
	let mcuPackageWithIdBehaviorSubject: BehaviorSubject<McuPackageModel>;

	mcuPackageItemWithId.service = defaultServiceEntities[0];
	mcuPackageItemWithId.serviceId = mcuPackageItemWithId.service.id;
	let serviceWithIdBehaviorSubject: BehaviorSubject<ServiceModel>;

	mcuPackageItemWithId.staff = defaultStaffEntities[0];
	mcuPackageItemWithId.staffId = mcuPackageItemWithId.staff.id;
	let staffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;

	mcuPackageItemWithId.examinationItems = defaultExaminationItemEntities;
	mcuPackageItemWithId.examinationItemsIds = mcuPackageItemWithId.examinationItems.map(examinationItems => examinationItems.id);


	const routerState: RouterState = {
		url: 'mcu-package-item-crud',
		urls: ['mcu-package-item-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: McuPackageItemModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		mcuPackageItemModelState = initialMcuPackageItemModelState;
		store.setState({model: mcuPackageItemModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		mcuPackageItemCountBehaviorSubject = new BehaviorSubject(defaultMcuPackageItemCount);
		...
		mcuPackageItemModelWithIdBehaviorSubject = new BehaviorSubject(mcuPackageItemWithId);
		mcuPackageWithIdBehaviorSubject = new BehaviorSubject(mcuPackageItemWithId.mcuPackage);
		serviceWithIdBehaviorSubject = new BehaviorSubject(mcuPackageItemWithId.service);
		staffWithIdBehaviorSubject = new BehaviorSubject(mcuPackageItemWithId.staff);

		spyOn(store, 'select')
			.withArgs(getCountMcuPackageItemModels).and.returnValue(mcuPackageItemCountBehaviorSubject)
			.withArgs(getMcuPackageItemModelWithId, mcuPackageItemWithId.id).and.returnValue(mcuPackageItemModelWithIdBehaviorSubject)
			...
			.withArgs(getMcuPackageModelWithId, mcuPackageItemWithId.mcuPackageId).and.returnValue(mcuPackageWithIdBehaviorSubject)
			.withArgs(getServiceModelWithId, mcuPackageItemWithId.serviceId).and.returnValue(serviceWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, mcuPackageItemWithId.staffId).and.returnValue(staffWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(McuPackageItemCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new McuPackageItemModel();
	 ...
	 */
	it ('Renders the MCU Package Item CRUD Edit Component', () => {
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the MCU Package relation.
	*
	* The button should only exist if mcuPackageMultiCrudActive is true
	*/
	it ('Renders MCU Package Add Entity button in create mode', fakeAsync(() => {
		const mcuPackageButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.mcuPackageDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!mcuPackageButton).toEqual(component.mcuPackageMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new MCU Package', fakeAsync(() => {
		const mcuPackageButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.mcuPackageDisplayName + ' to Model'));
		...
		if (component.mcuPackageMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialMcuPackageRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.mcuPackageDisplayName + ' from Model')).length;

			mcuPackageButton.nativeElement.click();
			fixture.detectChanges();

			const actualMcuPackageRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.mcuPackageDisplayName + ' from Model')).length;

			expect(actualMcuPackageRemoveButtonCount).toEqual(initialMcuPackageRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove MCU Package button', () => {
		const mcuPackageButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.mcuPackageDisplayName + ' to Model'));
		...
		if (component.mcuPackageMultiCrudActive) {
			...
			const initialMcuPackageRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.mcuPackageDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialMcuPackageRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualMcuPackageRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.mcuPackageDisplayName + ' from Model'));

			expect(actualMcuPackageRemoveButtons.length).toEqual(initialMcuPackageRemoveButtons.length - 1);
		}
	});

	it ('should disable the add MCU Package button when one has already been added', () => {
		...
		if (component.mcuPackageMultiCrudActive) {
			...
			const updatedmcuPackageButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.mcuPackageDisplayName + ' to Model'));

			expect(updatedmcuPackageButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related MCU Package entities from model when loading in edit mode', () => {
		component.targetModel = mcuPackageItemWithId;
		component.targetModelId = mcuPackageItemWithId.id;
		...
		if (component.mcuPackageMultiCrudActive) {
			...
			const mcuPackageRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.mcuPackageDisplayName + ' from Model')).length;

			expect(mcuPackageRemoveButtonCount).toEqual(1);
		}
	});
	...
	*/
	...
	it ('renders related Examination Items entities from model when loading in edit mode', () => {
		...
		if (component.examinationItemsMultiCrudActive) {
			...
			expect(examinationItemsRemoveButtonCount).toEqual(mcuPackageItemWithId.examinationItems.length);
		}
	});
