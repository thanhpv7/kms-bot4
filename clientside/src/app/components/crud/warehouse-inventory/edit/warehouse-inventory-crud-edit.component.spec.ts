...
import { 
	getWarehouseInventoryCollectionModels,
	getCountWarehouseInventoryModels,
	getWarehouseInventoryModelWithId
} from 'src/app/models/warehouseInventory/warehouse_inventory.model.selector';
import { WarehouseInventoryCrudModule } from '../warehouse-inventory-crud.module';
import { WarehouseInventoryCrudEditComponent } from './warehouse-inventory-crud-edit.component';
import {
	WarehouseInventoryModelState,
	initialState as initialWarehouseInventoryModelState
} from 'src/app/models/warehouseInventory/warehouse_inventory.model.state';
...
import { CrudTileMode } from '../warehouse-inventory-crud.component';
...
describe('Warehouse Inventory CRUD Edit component tests', () => {
	let fixture: ComponentFixture<WarehouseInventoryCrudEditComponent>
	let component: WarehouseInventoryCrudEditComponent;

	let warehouseInventoryModelState: WarehouseInventoryModelState;

	let store: MockStore<{ model: WarehouseInventoryModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let warehouseInventoryCountBehaviorSubject: BehaviorSubject<number>;
	...
	let warehouseInventoryWithId: WarehouseInventoryModel = defaultWarehouseInventoryEntities[0];
	let warehouseInventoryModelWithIdBehaviorSubject: BehaviorSubject<WarehouseInventoryModel>;

	warehouseInventoryWithId.location = defaultLocationEntities[0];
	warehouseInventoryWithId.locationId = warehouseInventoryWithId.location.id;
	let locationWithIdBehaviorSubject: BehaviorSubject<LocationModel>;

	warehouseInventoryWithId.stockCatalogue = defaultStockCatalogueEntities[0];
	warehouseInventoryWithId.stockCatalogueId = warehouseInventoryWithId.stockCatalogue.id;
	let stockCatalogueWithIdBehaviorSubject: BehaviorSubject<StockCatalogueModel>;

	warehouseInventoryWithId.warehouse = defaultWarehouseEntities[0];
	warehouseInventoryWithId.warehouseId = warehouseInventoryWithId.warehouse.id;
	let warehouseWithIdBehaviorSubject: BehaviorSubject<WarehouseModel>;


	const routerState: RouterState = {
		url: 'warehouse-inventory-crud',
		urls: ['warehouse-inventory-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: WarehouseInventoryModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		warehouseInventoryModelState = initialWarehouseInventoryModelState;
		store.setState({model: warehouseInventoryModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		warehouseInventoryCountBehaviorSubject = new BehaviorSubject(defaultWarehouseInventoryCount);
		...
		warehouseInventoryModelWithIdBehaviorSubject = new BehaviorSubject(warehouseInventoryWithId);
		locationWithIdBehaviorSubject = new BehaviorSubject(warehouseInventoryWithId.location);
		stockCatalogueWithIdBehaviorSubject = new BehaviorSubject(warehouseInventoryWithId.stockCatalogue);
		warehouseWithIdBehaviorSubject = new BehaviorSubject(warehouseInventoryWithId.warehouse);

		spyOn(store, 'select')
			.withArgs(getCountWarehouseInventoryModels).and.returnValue(warehouseInventoryCountBehaviorSubject)
			.withArgs(getWarehouseInventoryModelWithId, warehouseInventoryWithId.id).and.returnValue(warehouseInventoryModelWithIdBehaviorSubject)
			...
			.withArgs(getLocationModelWithId, warehouseInventoryWithId.locationId).and.returnValue(locationWithIdBehaviorSubject)
			.withArgs(getStockCatalogueModelWithId, warehouseInventoryWithId.stockCatalogueId).and.returnValue(stockCatalogueWithIdBehaviorSubject)
			.withArgs(getWarehouseModelWithId, warehouseInventoryWithId.warehouseId).and.returnValue(warehouseWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(WarehouseInventoryCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new WarehouseInventoryModel();
	 ...
	 */
	it ('Renders the Warehouse Inventory CRUD Edit Component', () => {
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Location relation.
	*
	* The button should only exist if locationMultiCrudActive is true
	*/
	it ('Renders Location Add Entity button in create mode', fakeAsync(() => {
		const locationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.locationDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!locationButton).toEqual(component.locationMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Location', fakeAsync(() => {
		const locationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.locationDisplayName + ' to Model'));
		...
		if (component.locationMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialLocationRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.locationDisplayName + ' from Model')).length;

			locationButton.nativeElement.click();
			fixture.detectChanges();

			const actualLocationRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.locationDisplayName + ' from Model')).length;

			expect(actualLocationRemoveButtonCount).toEqual(initialLocationRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Location button', () => {
		const locationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.locationDisplayName + ' to Model'));
		...
		if (component.locationMultiCrudActive) {
			...
			const initialLocationRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.locationDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialLocationRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualLocationRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.locationDisplayName + ' from Model'));

			expect(actualLocationRemoveButtons.length).toEqual(initialLocationRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Location button when one has already been added', () => {
		...
		if (component.locationMultiCrudActive) {
			...
			const updatedlocationButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.locationDisplayName + ' to Model'));

			expect(updatedlocationButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Location entities from model when loading in edit mode', () => {
		component.targetModel = warehouseInventoryWithId;
		component.targetModelId = warehouseInventoryWithId.id;
		...
		if (component.locationMultiCrudActive) {
			...
			const locationRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.locationDisplayName + ' from Model')).length;

			expect(locationRemoveButtonCount).toEqual(1);
		}
	});
