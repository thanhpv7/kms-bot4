...
import { 
	getPatientDetailCollectionModels,
	getCountPatientDetailModels,
	getPatientDetailModelWithId
} from 'src/app/models/patientDetail/patient_detail.model.selector';
import { PatientDetailCrudModule } from '../patient-detail-crud.module';
import { PatientDetailCrudEditComponent } from './patient-detail-crud-edit.component';
import {
	PatientDetailModelState,
	initialState as initialPatientDetailModelState
} from 'src/app/models/patientDetail/patient_detail.model.state';
...
import { CrudTileMode } from '../patient-detail-crud.component';
...
describe('Patient Detail CRUD Edit component tests', () => {
	let fixture: ComponentFixture<PatientDetailCrudEditComponent>
	let component: PatientDetailCrudEditComponent;

	let patientDetailModelState: PatientDetailModelState;

	let store: MockStore<{ model: PatientDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let patientDetailCountBehaviorSubject: BehaviorSubject<number>;
	...
	let patientDetailWithId: PatientDetailModel = defaultPatientDetailEntities[0];
	let patientDetailModelWithIdBehaviorSubject: BehaviorSubject<PatientDetailModel>;

	patientDetailWithId.patientGeneralInformation = defaultPatientGeneralInfoEntities[0];
	patientDetailWithId.patientGeneralInformationId = patientDetailWithId.patientGeneralInformation.id;
	let patientGeneralInformationWithIdBehaviorSubject: BehaviorSubject<PatientGeneralInfoModel>;


	const routerState: RouterState = {
		url: 'patient-detail-crud',
		urls: ['patient-detail-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientDetailModelState = initialPatientDetailModelState;
		store.setState({model: patientDetailModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		patientDetailCountBehaviorSubject = new BehaviorSubject(defaultPatientDetailCount);
		...
		patientDetailModelWithIdBehaviorSubject = new BehaviorSubject(patientDetailWithId);
		patientGeneralInformationWithIdBehaviorSubject = new BehaviorSubject(patientDetailWithId.patientGeneralInformation);

		spyOn(store, 'select')
			.withArgs(getCountPatientDetailModels).and.returnValue(patientDetailCountBehaviorSubject)
			.withArgs(getPatientDetailModelWithId, patientDetailWithId.id).and.returnValue(patientDetailModelWithIdBehaviorSubject)
			...
			.withArgs(getPatientGeneralInfoModelWithId, patientDetailWithId.patientGeneralInformationId).and.returnValue(patientGeneralInformationWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(PatientDetailCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new PatientDetailModel();
	 ...
	 */
	it ('Renders the Patient Detail CRUD Edit Component', () => {
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Patient General Information relation.
	*
	* The button should only exist if patientGeneralInformationMultiCrudActive is true
	*/
	it ('Renders Patient General Information Add Entity button in create mode', fakeAsync(() => {
		const patientGeneralInformationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.patientGeneralInformationDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!patientGeneralInformationButton).toEqual(component.patientGeneralInformationMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Patient General Information', fakeAsync(() => {
		const patientGeneralInformationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.patientGeneralInformationDisplayName + ' to Model'));
		...
		if (component.patientGeneralInformationMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialPatientGeneralInformationRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.patientGeneralInformationDisplayName + ' from Model')).length;

			patientGeneralInformationButton.nativeElement.click();
			fixture.detectChanges();

			const actualPatientGeneralInformationRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.patientGeneralInformationDisplayName + ' from Model')).length;

			expect(actualPatientGeneralInformationRemoveButtonCount).toEqual(initialPatientGeneralInformationRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Patient General Information button', () => {
		const patientGeneralInformationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.patientGeneralInformationDisplayName + ' to Model'));
		...
		if (component.patientGeneralInformationMultiCrudActive) {
			...
			const initialPatientGeneralInformationRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.patientGeneralInformationDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialPatientGeneralInformationRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualPatientGeneralInformationRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.patientGeneralInformationDisplayName + ' from Model'));

			expect(actualPatientGeneralInformationRemoveButtons.length).toEqual(initialPatientGeneralInformationRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Patient General Information button when one has already been added', () => {
		...
		if (component.patientGeneralInformationMultiCrudActive) {
			...
			const updatedpatientGeneralInformationButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.patientGeneralInformationDisplayName + ' to Model'));

			expect(updatedpatientGeneralInformationButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Patient General Information entities from model when loading in edit mode', () => {
		component.targetModel = patientDetailWithId;
		component.targetModelId = patientDetailWithId.id;
		...
		if (component.patientGeneralInformationMultiCrudActive) {
			...
			const patientGeneralInformationRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.patientGeneralInformationDisplayName + ' from Model')).length;

			expect(patientGeneralInformationRemoveButtonCount).toEqual(1);
		}
	});
