...
import {BpjsVisitDataCrudListComponent} from './bpjs-visit-data-crud-list.component';
 ...
 */
describe('BPJS Visit Data Crud List Tile Component', () => {

	let fixture;
	let bpjsVisitDataCrudListComponent: BpjsVisitDataCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsVisitDataModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '14e67552-522c-4e3b-b159-11f7f9873dcd';
	const searchContent = 'text';

	const searchableFieldCount = BpjsVisitDataModel.searchFields.length;
	...
	const bpjsVisitDataDataFactory = new BpjsVisitDataDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsVisitDataDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsVisitDataCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsVisitDataCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(BpjsVisitDataCrudListComponent);
			bpjsVisitDataCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsVisitDataCrudListComponent.bpjsVisitDatasCount).toBe(defaultData.length);
			...
			expect(bpjsVisitDataCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsVisitDataCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsVisitDataDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsVisitDataCrudListComponent.bpjsVisitDatasCount).toBe(filteredDataSize);
			expect(bpjsVisitDataCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsVisitDataCrudListComponent.pageIndex).toBe(0);
			expect(bpjsVisitDataCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsVisitDataCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsVisitDataCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsVisitDataCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
