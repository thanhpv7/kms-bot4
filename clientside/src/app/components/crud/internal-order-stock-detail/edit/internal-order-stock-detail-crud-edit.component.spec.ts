...
import { InternalOrderStockDetailCrudEditComponent } from './internal-order-stock-detail-crud-edit.component';
import {
...
describe('Internal Order Stock Detail CRUD Edit component tests', () => {
	let fixture: ComponentFixture<InternalOrderStockDetailCrudEditComponent>
	let component: InternalOrderStockDetailCrudEditComponent;
	...
	let internalOrderStockDetailCountBehaviorSubject: BehaviorSubject<number>;
	...
	let internalOrderStockDetailWithId: InternalOrderStockDetailModel = defaultInternalOrderStockDetailEntities[0];
	let internalOrderStockDetailModelWithIdBehaviorSubject: BehaviorSubject<InternalOrderStockDetailModel>;

	internalOrderStockDetailWithId.internalOrder = defaultInternalOrderEntities[0];
	internalOrderStockDetailWithId.internalOrderId = internalOrderStockDetailWithId.internalOrder.id;
	let internalOrderWithIdBehaviorSubject: BehaviorSubject<InternalOrderModel>;

	internalOrderStockDetailWithId.stockCatalogue = defaultStockCatalogueEntities[0];
	internalOrderStockDetailWithId.stockCatalogueId = internalOrderStockDetailWithId.stockCatalogue.id;
	...
	function spySelectorsInStore() {
		...
		internalOrderStockDetailCountBehaviorSubject = new BehaviorSubject(defaultInternalOrderStockDetailCount);
		...
		internalOrderStockDetailModelWithIdBehaviorSubject = new BehaviorSubject(internalOrderStockDetailWithId);
		internalOrderWithIdBehaviorSubject = new BehaviorSubject(internalOrderStockDetailWithId.internalOrder);
		stockCatalogueWithIdBehaviorSubject = new BehaviorSubject(internalOrderStockDetailWithId.stockCatalogue);

		spyOn(store, 'select')
			.withArgs(getCountInternalOrderStockDetailModels).and.returnValue(internalOrderStockDetailCountBehaviorSubject)
			.withArgs(getInternalOrderStockDetailModelWithId, internalOrderStockDetailWithId.id).and.returnValue(internalOrderStockDetailModelWithIdBehaviorSubject)
			...
			.withArgs(getInternalOrderModelWithId, internalOrderStockDetailWithId.internalOrderId).and.returnValue(internalOrderWithIdBehaviorSubject)
			.withArgs(getStockCatalogueModelWithId, internalOrderStockDetailWithId.stockCatalogueId).and.returnValue(stockCatalogueWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(InternalOrderStockDetailCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new InternalOrderStockDetailModel();
	 ...
	 */
	it ('Renders the Internal Order Stock Detail CRUD Edit Component', () => {
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Internal Order relation.
	*
	* The button should only exist if internalOrderMultiCrudActive is true
	*/
	it ('Renders Internal Order Add Entity button in create mode', fakeAsync(() => {
		const internalOrderButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.internalOrderDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!internalOrderButton).toEqual(component.internalOrderMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Internal Order', fakeAsync(() => {
		const internalOrderButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.internalOrderDisplayName + ' to Model'));
		...
		if (component.internalOrderMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialInternalOrderRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.internalOrderDisplayName + ' from Model')).length;

			internalOrderButton.nativeElement.click();
			fixture.detectChanges();

			const actualInternalOrderRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.internalOrderDisplayName + ' from Model')).length;

			expect(actualInternalOrderRemoveButtonCount).toEqual(initialInternalOrderRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Internal Order button', () => {
		const internalOrderButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.internalOrderDisplayName + ' to Model'));
		...
		if (component.internalOrderMultiCrudActive) {
			...
			const initialInternalOrderRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.internalOrderDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialInternalOrderRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualInternalOrderRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.internalOrderDisplayName + ' from Model'));

			expect(actualInternalOrderRemoveButtons.length).toEqual(initialInternalOrderRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Internal Order button when one has already been added', () => {
		...
		if (component.internalOrderMultiCrudActive) {
			...
			const updatedinternalOrderButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.internalOrderDisplayName + ' to Model'));

			expect(updatedinternalOrderButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Internal Order entities from model when loading in edit mode', () => {
		component.targetModel = internalOrderStockDetailWithId;
		component.targetModelId = internalOrderStockDetailWithId.id;
		...
		if (component.internalOrderMultiCrudActive) {
			...
			const internalOrderRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.internalOrderDisplayName + ' from Model')).length;

			expect(internalOrderRemoveButtonCount).toEqual(1);
		}
	});
