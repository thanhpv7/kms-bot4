...
import { 
	getHospitalBranchInformationCollectionModels,
...
import { HospitalBranchInformationCrudModule } from '../hospital-branch-information-crud.module';
import { HospitalBranchInformationCrudEditComponent } from './hospital-branch-information-crud-edit.component';
import {
	HospitalBranchInformationModelState,
	initialState as initialHospitalBranchInformationModelState
} from 'src/app/models/hospitalBranchInformation/hospital_branch_information.model.state';
...
describe('Hospital Branch Information CRUD Edit component tests', () => {
	let fixture: ComponentFixture<HospitalBranchInformationCrudEditComponent>
	let component: HospitalBranchInformationCrudEditComponent;

	let hospitalBranchInformationModelState: HospitalBranchInformationModelState;

	let store: MockStore<{ model: HospitalBranchInformationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let hospitalBranchInformationCountBehaviorSubject: BehaviorSubject<number>;
	...
	let hospitalBranchInformationWithId: HospitalBranchInformationModel = defaultHospitalBranchInformationEntities[0];
	let hospitalBranchInformationModelWithIdBehaviorSubject: BehaviorSubject<HospitalBranchInformationModel>;

	hospitalBranchInformationWithId.hospitalGroup = defaultHospitalGroupInformationEntities[0];
	hospitalBranchInformationWithId.hospitalGroupId = hospitalBranchInformationWithId.hospitalGroup.id;
	let hospitalGroupWithIdBehaviorSubject: BehaviorSubject<HospitalGroupInformationModel>;

	hospitalBranchInformationWithId.staff = defaultStaffEntities[0];
	hospitalBranchInformationWithId.staffId = hospitalBranchInformationWithId.staff.id;
	let staffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;


	const routerState: RouterState = {
		url: 'hospital-branch-information-crud',
		urls: ['hospital-branch-information-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: HospitalBranchInformationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		hospitalBranchInformationModelState = initialHospitalBranchInformationModelState;
		store.setState({model: hospitalBranchInformationModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		hospitalBranchInformationCountBehaviorSubject = new BehaviorSubject(defaultHospitalBranchInformationCount);
		...
		hospitalBranchInformationModelWithIdBehaviorSubject = new BehaviorSubject(hospitalBranchInformationWithId);
		hospitalGroupWithIdBehaviorSubject = new BehaviorSubject(hospitalBranchInformationWithId.hospitalGroup);
		staffWithIdBehaviorSubject = new BehaviorSubject(hospitalBranchInformationWithId.staff);

		spyOn(store, 'select')
			.withArgs(getCountHospitalBranchInformationModels).and.returnValue(hospitalBranchInformationCountBehaviorSubject)
			.withArgs(getHospitalBranchInformationModelWithId, hospitalBranchInformationWithId.id).and.returnValue(hospitalBranchInformationModelWithIdBehaviorSubject)
			...
			.withArgs(getHospitalGroupInformationModelWithId, hospitalBranchInformationWithId.hospitalGroupId).and.returnValue(hospitalGroupWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, hospitalBranchInformationWithId.staffId).and.returnValue(staffWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				HospitalBranchInformationCrudModule,
			...
			fixture = TestBed.createComponent(HospitalBranchInformationCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new HospitalBranchInformationModel();
	 ...
	 */
	it ('Renders the Hospital Branch Information CRUD Edit Component', () => {
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Hospital Group relation.
	*
	* The button should only exist if hospitalGroupMultiCrudActive is true
	*/
	it ('Renders Hospital Group Add Entity button in create mode', fakeAsync(() => {
		const hospitalGroupButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.hospitalGroupDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!hospitalGroupButton).toEqual(component.hospitalGroupMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Hospital Group', fakeAsync(() => {
		const hospitalGroupButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.hospitalGroupDisplayName + ' to Model'));
		...
		if (component.hospitalGroupMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialHospitalGroupRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.hospitalGroupDisplayName + ' from Model')).length;

			hospitalGroupButton.nativeElement.click();
			fixture.detectChanges();

			const actualHospitalGroupRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.hospitalGroupDisplayName + ' from Model')).length;

			expect(actualHospitalGroupRemoveButtonCount).toEqual(initialHospitalGroupRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Hospital Group button', () => {
		const hospitalGroupButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.hospitalGroupDisplayName + ' to Model'));
		...
		if (component.hospitalGroupMultiCrudActive) {
			...
			const initialHospitalGroupRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.hospitalGroupDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialHospitalGroupRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualHospitalGroupRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.hospitalGroupDisplayName + ' from Model'));

			expect(actualHospitalGroupRemoveButtons.length).toEqual(initialHospitalGroupRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Hospital Group button when one has already been added', () => {
		...
		if (component.hospitalGroupMultiCrudActive) {
			...
			const updatedhospitalGroupButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.hospitalGroupDisplayName + ' to Model'));

			expect(updatedhospitalGroupButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Hospital Group entities from model when loading in edit mode', () => {
		component.targetModel = hospitalBranchInformationWithId;
		component.targetModelId = hospitalBranchInformationWithId.id;
		...
		if (component.hospitalGroupMultiCrudActive) {
			...
			const hospitalGroupRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.hospitalGroupDisplayName + ' from Model')).length;

			expect(hospitalGroupRemoveButtonCount).toEqual(1);
		}
	});
