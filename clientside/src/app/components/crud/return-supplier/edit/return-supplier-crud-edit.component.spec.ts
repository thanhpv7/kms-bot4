...
import { ReturnSupplierCrudModule } from '../return-supplier-crud.module';
import { ReturnSupplierCrudEditComponent } from './return-supplier-crud-edit.component';
import {
	ReturnSupplierModelState,
	initialState as initialReturnSupplierModelState
} from 'src/app/models/returnSupplier/return_supplier.model.state';
...
describe('Return Supplier CRUD Edit component tests', () => {
	let fixture: ComponentFixture<ReturnSupplierCrudEditComponent>
	let component: ReturnSupplierCrudEditComponent;

	let returnSupplierModelState: ReturnSupplierModelState;

	let store: MockStore<{ model: ReturnSupplierModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let returnSupplierCountBehaviorSubject: BehaviorSubject<number>;
	...
	let returnSupplierWithId: ReturnSupplierModel = defaultReturnSupplierEntities[0];
	let returnSupplierModelWithIdBehaviorSubject: BehaviorSubject<ReturnSupplierModel>;

	returnSupplierWithId.purchaseOrder = defaultPurchaseOrderEntities[0];
	returnSupplierWithId.purchaseOrderId = returnSupplierWithId.purchaseOrder.id;
	let purchaseOrderWithIdBehaviorSubject: BehaviorSubject<PurchaseOrderModel>;

	returnSupplierWithId.supplier = defaultSupplierEntities[0];
	returnSupplierWithId.supplierId = returnSupplierWithId.supplier.id;
	let supplierWithIdBehaviorSubject: BehaviorSubject<SupplierModel>;

	returnSupplierWithId.warehouse = defaultWarehouseEntities[0];
	returnSupplierWithId.warehouseId = returnSupplierWithId.warehouse.id;
	let warehouseWithIdBehaviorSubject: BehaviorSubject<WarehouseModel>;

	returnSupplierWithId.returnSupplierStockDetails = defaultReturnSupplierStockDetailEntities;
	returnSupplierWithId.returnSupplierStockDetailsIds = returnSupplierWithId.returnSupplierStockDetails.map(returnSupplierStockDetails => returnSupplierStockDetails.id);


	const routerState: RouterState = {
		url: 'return-supplier-crud',
		urls: ['return-supplier-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: ReturnSupplierModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		returnSupplierModelState = initialReturnSupplierModelState;
		store.setState({model: returnSupplierModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		returnSupplierCountBehaviorSubject = new BehaviorSubject(defaultReturnSupplierCount);
		...
		returnSupplierModelWithIdBehaviorSubject = new BehaviorSubject(returnSupplierWithId);
		purchaseOrderWithIdBehaviorSubject = new BehaviorSubject(returnSupplierWithId.purchaseOrder);
		supplierWithIdBehaviorSubject = new BehaviorSubject(returnSupplierWithId.supplier);
		warehouseWithIdBehaviorSubject = new BehaviorSubject(returnSupplierWithId.warehouse);

		spyOn(store, 'select')
			.withArgs(getCountReturnSupplierModels).and.returnValue(returnSupplierCountBehaviorSubject)
			.withArgs(getReturnSupplierModelWithId, returnSupplierWithId.id).and.returnValue(returnSupplierModelWithIdBehaviorSubject)
			...
			.withArgs(getPurchaseOrderModelWithId, returnSupplierWithId.purchaseOrderId).and.returnValue(purchaseOrderWithIdBehaviorSubject)
			.withArgs(getSupplierModelWithId, returnSupplierWithId.supplierId).and.returnValue(supplierWithIdBehaviorSubject)
			.withArgs(getWarehouseModelWithId, returnSupplierWithId.warehouseId).and.returnValue(warehouseWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(ReturnSupplierCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new ReturnSupplierModel();
	 ...
	 */
	it ('Renders the Return Supplier CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Purchase Order entities from model when loading in edit mode', () => {
		component.targetModel = returnSupplierWithId;
		component.targetModelId = returnSupplierWithId.id;
	...
	*/
	...
	it ('renders related Return Supplier Stock Details entities from model when loading in edit mode', () => {
		...
		if (component.returnSupplierStockDetailsMultiCrudActive) {
			...
			expect(returnSupplierStockDetailsRemoveButtonCount).toEqual(returnSupplierWithId.returnSupplierStockDetails.length);
		}
	});
