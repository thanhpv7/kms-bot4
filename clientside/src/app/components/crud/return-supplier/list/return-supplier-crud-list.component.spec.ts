...
import {ReturnSupplierCrudListComponent} from './return-supplier-crud-list.component';
 ...
 */
describe('Return Supplier Crud List Tile Component', () => {

	let fixture;
	let returnSupplierCrudListComponent: ReturnSupplierCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ReturnSupplierModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '55cd29fe-e34d-4c60-8095-e9f92011b71a';
	const searchContent = 'text';

	const searchableFieldCount = ReturnSupplierModel.searchFields.length;
	...
	const returnSupplierDataFactory = new ReturnSupplierDataFactory();

	const defaultDataSize = 30;
	const defaultData = returnSupplierDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getReturnSupplierCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getReturnSupplierCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(ReturnSupplierCrudListComponent);
			returnSupplierCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(returnSupplierCrudListComponent.returnSuppliersCount).toBe(defaultData.length);
			...
			expect(returnSupplierCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			returnSupplierCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = returnSupplierDataFactory.createAll(filteredDataSize);
			...
			expect(returnSupplierCrudListComponent.returnSuppliersCount).toBe(filteredDataSize);
			expect(returnSupplierCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(returnSupplierCrudListComponent.pageIndex).toBe(0);
			expect(returnSupplierCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			returnSupplierCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		returnSupplierCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		returnSupplierCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
