...
import { AmendmentDetailCrudModule } from '../amendment-detail-crud.module';
import { AmendmentDetailCrudEditComponent } from './amendment-detail-crud-edit.component';
import {
	AmendmentDetailModelState,
	initialState as initialAmendmentDetailModelState
} from 'src/app/models/amendmentDetail/amendment_detail.model.state';
...
describe('Amendment Detail CRUD Edit component tests', () => {
	let fixture: ComponentFixture<AmendmentDetailCrudEditComponent>
	let component: AmendmentDetailCrudEditComponent;

	let amendmentDetailModelState: AmendmentDetailModelState;

	let store: MockStore<{ model: AmendmentDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let amendmentDetailCountBehaviorSubject: BehaviorSubject<number>;
	...
	let amendmentDetailWithId: AmendmentDetailModel = defaultAmendmentDetailEntities[0];
	let amendmentDetailModelWithIdBehaviorSubject: BehaviorSubject<AmendmentDetailModel>;

	amendmentDetailWithId.medicalExaminationRecord = defaultMedicalExaminationRecordEntities[0];
	amendmentDetailWithId.medicalExaminationRecordId = amendmentDetailWithId.medicalExaminationRecord.id;
	let medicalExaminationRecordWithIdBehaviorSubject: BehaviorSubject<MedicalExaminationRecordModel>;

	amendmentDetailWithId.staff = defaultStaffEntities[0];
	amendmentDetailWithId.staffId = amendmentDetailWithId.staff.id;
	let staffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;


	const routerState: RouterState = {
		url: 'amendment-detail-crud',
		urls: ['amendment-detail-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: AmendmentDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		amendmentDetailModelState = initialAmendmentDetailModelState;
		store.setState({model: amendmentDetailModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		amendmentDetailCountBehaviorSubject = new BehaviorSubject(defaultAmendmentDetailCount);
		...
		amendmentDetailModelWithIdBehaviorSubject = new BehaviorSubject(amendmentDetailWithId);
		medicalExaminationRecordWithIdBehaviorSubject = new BehaviorSubject(amendmentDetailWithId.medicalExaminationRecord);
		staffWithIdBehaviorSubject = new BehaviorSubject(amendmentDetailWithId.staff);

		spyOn(store, 'select')
			.withArgs(getCountAmendmentDetailModels).and.returnValue(amendmentDetailCountBehaviorSubject)
			.withArgs(getAmendmentDetailModelWithId, amendmentDetailWithId.id).and.returnValue(amendmentDetailModelWithIdBehaviorSubject)
			...
			.withArgs(getMedicalExaminationRecordModelWithId, amendmentDetailWithId.medicalExaminationRecordId).and.returnValue(medicalExaminationRecordWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, amendmentDetailWithId.staffId).and.returnValue(staffWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AmendmentDetailCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new AmendmentDetailModel();
	 ...
	 */
	it ('Renders the Amendment Detail CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Medical Examination Record entities from model when loading in edit mode', () => {
		component.targetModel = amendmentDetailWithId;
		component.targetModelId = amendmentDetailWithId.id;
