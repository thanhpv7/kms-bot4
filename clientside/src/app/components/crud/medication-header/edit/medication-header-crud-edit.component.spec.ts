...
import { 
	getMedicationHeaderCollectionModels,
...
import { MedicationHeaderCrudModule } from '../medication-header-crud.module';
import { MedicationHeaderCrudEditComponent } from './medication-header-crud-edit.component';
import {
	MedicationHeaderModelState,
	initialState as initialMedicationHeaderModelState
} from 'src/app/models/medicationHeader/medication_header.model.state';
...
describe('Medication Header CRUD Edit component tests', () => {
	let fixture: ComponentFixture<MedicationHeaderCrudEditComponent>
	let component: MedicationHeaderCrudEditComponent;

	let medicationHeaderModelState: MedicationHeaderModelState;

	let store: MockStore<{ model: MedicationHeaderModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let medicationHeaderCountBehaviorSubject: BehaviorSubject<number>;
	...
	let medicationHeaderWithId: MedicationHeaderModel = defaultMedicationHeaderEntities[0];
	let medicationHeaderModelWithIdBehaviorSubject: BehaviorSubject<MedicationHeaderModel>;

	medicationHeaderWithId.invoice = defaultInvoiceEntities[0];
	medicationHeaderWithId.invoiceId = medicationHeaderWithId.invoice.id;
	let invoiceWithIdBehaviorSubject: BehaviorSubject<InvoiceModel>;

	medicationHeaderWithId.registration = defaultRegistrationEntities[0];
	medicationHeaderWithId.registrationId = medicationHeaderWithId.registration.id;
	let registrationWithIdBehaviorSubject: BehaviorSubject<RegistrationModel>;

	medicationHeaderWithId.requestedBy = defaultStaffEntities[0];
	medicationHeaderWithId.requestedById = medicationHeaderWithId.requestedBy.id;
	let requestedByWithIdBehaviorSubject: BehaviorSubject<StaffModel>;

	medicationHeaderWithId.warehouse = defaultWarehouseEntities[0];
	medicationHeaderWithId.warehouseId = medicationHeaderWithId.warehouse.id;
	let warehouseWithIdBehaviorSubject: BehaviorSubject<WarehouseModel>;

	medicationHeaderWithId.invoiceItems = defaultInvoiceItemEntities;
	medicationHeaderWithId.invoiceItemsIds = medicationHeaderWithId.invoiceItems.map(invoiceItems => invoiceItems.id);

	medicationHeaderWithId.medicationCompounds = defaultMedicationCompoundEntities;
	medicationHeaderWithId.medicationCompoundsIds = medicationHeaderWithId.medicationCompounds.map(medicationCompounds => medicationCompounds.id);

	medicationHeaderWithId.medicationItems = defaultMedicationItemEntities;
	medicationHeaderWithId.medicationItemsIds = medicationHeaderWithId.medicationItems.map(medicationItems => medicationItems.id);


	const routerState: RouterState = {
		url: 'medication-header-crud',
		urls: ['medication-header-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicationHeaderModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicationHeaderModelState = initialMedicationHeaderModelState;
		store.setState({model: medicationHeaderModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		medicationHeaderCountBehaviorSubject = new BehaviorSubject(defaultMedicationHeaderCount);
		...
		medicationHeaderModelWithIdBehaviorSubject = new BehaviorSubject(medicationHeaderWithId);
		invoiceWithIdBehaviorSubject = new BehaviorSubject(medicationHeaderWithId.invoice);
		registrationWithIdBehaviorSubject = new BehaviorSubject(medicationHeaderWithId.registration);
		requestedByWithIdBehaviorSubject = new BehaviorSubject(medicationHeaderWithId.requestedBy);
		warehouseWithIdBehaviorSubject = new BehaviorSubject(medicationHeaderWithId.warehouse);

		spyOn(store, 'select')
			.withArgs(getCountMedicationHeaderModels).and.returnValue(medicationHeaderCountBehaviorSubject)
			.withArgs(getMedicationHeaderModelWithId, medicationHeaderWithId.id).and.returnValue(medicationHeaderModelWithIdBehaviorSubject)
			...
			.withArgs(getInvoiceModelWithId, medicationHeaderWithId.invoiceId).and.returnValue(invoiceWithIdBehaviorSubject)
			.withArgs(getRegistrationModelWithId, medicationHeaderWithId.registrationId).and.returnValue(registrationWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, medicationHeaderWithId.requestedById).and.returnValue(requestedByWithIdBehaviorSubject)
			.withArgs(getWarehouseModelWithId, medicationHeaderWithId.warehouseId).and.returnValue(warehouseWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(MedicationHeaderCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new MedicationHeaderModel();
	 ...
	 */
	it ('Renders the Medication Header CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Invoice entities from model when loading in edit mode', () => {
		component.targetModel = medicationHeaderWithId;
		component.targetModelId = medicationHeaderWithId.id;
	...
	*/
	...
	it ('renders related Invoice Items entities from model when loading in edit mode', () => {
		...
		if (component.invoiceItemsMultiCrudActive) {
			...
			expect(invoiceItemsRemoveButtonCount).toEqual(medicationHeaderWithId.invoiceItems.length);
		}
	});

	/**
	* Checks that the status of the add model button matches the multi entity status for the Medication Compounds relation.
	*
	* The button should only exist if medicationCompoundsMultiCrudActive is true
	*/
	it ('Renders Medication Compounds Add Entity button in create mode', fakeAsync(() => {
		const medicationCompoundsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.medicationCompoundsDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!medicationCompoundsButton).toEqual(component.medicationCompoundsMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Medication Compounds', fakeAsync(() => {
		const medicationCompoundsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.medicationCompoundsDisplayName + ' to Model'));
		...
		if (component.medicationCompoundsMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialMedicationCompoundsRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.medicationCompoundsDisplayName + ' from Model')).length;

			medicationCompoundsButton.nativeElement.click();
			fixture.detectChanges();

			const actualMedicationCompoundsRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.medicationCompoundsDisplayName + ' from Model')).length;

			expect(actualMedicationCompoundsRemoveButtonCount).toEqual(initialMedicationCompoundsRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Medication Compounds button', () => {
		const medicationCompoundsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.medicationCompoundsDisplayName + ' to Model'));
		...
		if (component.medicationCompoundsMultiCrudActive) {
			...
			// create an array of elements here as opposed to just getting the length (like in previous tests)
			// as we need to interact with the button in this test
			const initialMedicationCompoundsRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.medicationCompoundsDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialMedicationCompoundsRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualMedicationCompoundsRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.medicationCompoundsDisplayName + ' from Model'));

			expect(actualMedicationCompoundsRemoveButtons.length).toEqual(initialMedicationCompoundsRemoveButtons.length - 1);
		}
	});

	it ('should not disable the add Medication Compounds button when one has already been added', () => {
		...
		if (component.medicationCompoundsMultiCrudActive) {
			...
			const updatedmedicationCompoundsButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.medicationCompoundsDisplayName + ' to Model'));

			expect(updatedmedicationCompoundsButton.nativeElement.disabled).toBe(false);
		}
	});

	it ('renders related Medication Compounds entities from model when loading in edit mode', () => {
		component.targetModel = medicationHeaderWithId;
		component.targetModelId = medicationHeaderWithId.id;
		component.tileMode = CrudTileMode.Edit;
		component.ngOnInit();
		...
		if (component.medicationCompoundsMultiCrudActive) {
			// As there is always one remove button per additional entity, running a test against the number of remove buttons is a simple
			// and reliable method to determine how many entities of the given type are present in the crud page
			const medicationCompoundsRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.medicationCompoundsDisplayName + ' from Model')).length;

			expect(medicationCompoundsRemoveButtonCount).toEqual(medicationHeaderWithId.medicationCompounds.length);
		}
	});
	...
	*/
	...
	it ('renders related Medication Items entities from model when loading in edit mode', () => {
		...
		if (component.medicationItemsMultiCrudActive) {
			...
			expect(medicationItemsRemoveButtonCount).toEqual(medicationHeaderWithId.medicationItems.length);
		}
	});
