...
import { 
	getNewbornDetailCollectionModels,
	getCountNewbornDetailModels,
	getNewbornDetailModelWithId
} from 'src/app/models/newbornDetail/newborn_detail.model.selector';
import { NewbornDetailCrudModule } from '../newborn-detail-crud.module';
import { NewbornDetailCrudEditComponent } from './newborn-detail-crud-edit.component';
import {
	NewbornDetailModelState,
	initialState as initialNewbornDetailModelState
} from 'src/app/models/newbornDetail/newborn_detail.model.state';
...
describe('Newborn Detail CRUD Edit component tests', () => {
	let fixture: ComponentFixture<NewbornDetailCrudEditComponent>
	let component: NewbornDetailCrudEditComponent;

	let newbornDetailModelState: NewbornDetailModelState;

	let store: MockStore<{ model: NewbornDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let newbornDetailCountBehaviorSubject: BehaviorSubject<number>;
	...
	let newbornDetailWithId: NewbornDetailModel = defaultNewbornDetailEntities[0];
	let newbornDetailModelWithIdBehaviorSubject: BehaviorSubject<NewbornDetailModel>;

	newbornDetailWithId.deliveryMedicalExaminationRecord = defaultDeliveryMedicalExaminationRecordEntities[0];
	newbornDetailWithId.deliveryMedicalExaminationRecordId = newbornDetailWithId.deliveryMedicalExaminationRecord.id;
	let deliveryMedicalExaminationRecordWithIdBehaviorSubject: BehaviorSubject<DeliveryMedicalExaminationRecordModel>;

	newbornDetailWithId.staff = defaultStaffEntities[0];
	newbornDetailWithId.staffId = newbornDetailWithId.staff.id;
	let staffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;

	newbornDetailWithId.vitalSigns = defaultVitalSignsEntities;
	newbornDetailWithId.vitalSignsIds = newbornDetailWithId.vitalSigns.map(vitalSigns => vitalSigns.id);


	const routerState: RouterState = {
		url: 'newborn-detail-crud',
		urls: ['newborn-detail-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NewbornDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		newbornDetailModelState = initialNewbornDetailModelState;
		store.setState({model: newbornDetailModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		newbornDetailCountBehaviorSubject = new BehaviorSubject(defaultNewbornDetailCount);
		...
		newbornDetailModelWithIdBehaviorSubject = new BehaviorSubject(newbornDetailWithId);
		deliveryMedicalExaminationRecordWithIdBehaviorSubject = new BehaviorSubject(newbornDetailWithId.deliveryMedicalExaminationRecord);
		staffWithIdBehaviorSubject = new BehaviorSubject(newbornDetailWithId.staff);

		spyOn(store, 'select')
			.withArgs(getCountNewbornDetailModels).and.returnValue(newbornDetailCountBehaviorSubject)
			.withArgs(getNewbornDetailModelWithId, newbornDetailWithId.id).and.returnValue(newbornDetailModelWithIdBehaviorSubject)
			...
			.withArgs(getDeliveryMedicalExaminationRecordModelWithId, newbornDetailWithId.deliveryMedicalExaminationRecordId).and.returnValue(deliveryMedicalExaminationRecordWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, newbornDetailWithId.staffId).and.returnValue(staffWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(NewbornDetailCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new NewbornDetailModel();
	 ...
	 */
	it ('Renders the Newborn Detail CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Delivery Medical Examination Record entities from model when loading in edit mode', () => {
		component.targetModel = newbornDetailWithId;
		component.targetModelId = newbornDetailWithId.id;
	...
	*/
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Vital Signs relation.
	*
	* The button should only exist if vitalSignsMultiCrudActive is true
	*/
	it ('Renders Vital Signs Add Entity button in create mode', fakeAsync(() => {
		const vitalSignsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.vitalSignsDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!vitalSignsButton).toEqual(component.vitalSignsMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Vital Signs', fakeAsync(() => {
		const vitalSignsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.vitalSignsDisplayName + ' to Model'));
		...
		if (component.vitalSignsMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialVitalSignsRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.vitalSignsDisplayName + ' from Model')).length;

			vitalSignsButton.nativeElement.click();
			fixture.detectChanges();

			const actualVitalSignsRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.vitalSignsDisplayName + ' from Model')).length;

			expect(actualVitalSignsRemoveButtonCount).toEqual(initialVitalSignsRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Vital Signs button', () => {
		const vitalSignsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.vitalSignsDisplayName + ' to Model'));
		...
		if (component.vitalSignsMultiCrudActive) {
			...
			// create an array of elements here as opposed to just getting the length (like in previous tests)
			// as we need to interact with the button in this test
			const initialVitalSignsRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.vitalSignsDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialVitalSignsRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualVitalSignsRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.vitalSignsDisplayName + ' from Model'));

			expect(actualVitalSignsRemoveButtons.length).toEqual(initialVitalSignsRemoveButtons.length - 1);
		}
	});

	it ('should not disable the add Vital Signs button when one has already been added', () => {
		...
		if (component.vitalSignsMultiCrudActive) {
			...
			const updatedvitalSignsButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.vitalSignsDisplayName + ' to Model'));

			expect(updatedvitalSignsButton.nativeElement.disabled).toBe(false);
		}
	});

	it ('renders related Vital Signs entities from model when loading in edit mode', () => {
		component.targetModel = newbornDetailWithId;
		component.targetModelId = newbornDetailWithId.id;
		component.tileMode = CrudTileMode.Edit;
		component.ngOnInit();
		...
		if (component.vitalSignsMultiCrudActive) {
			// As there is always one remove button per additional entity, running a test against the number of remove buttons is a simple
			// and reliable method to determine how many entities of the given type are present in the crud page
			const vitalSignsRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.vitalSignsDisplayName + ' from Model')).length;

			expect(vitalSignsRemoveButtonCount).toEqual(newbornDetailWithId.vitalSigns.length);
		}
	});
