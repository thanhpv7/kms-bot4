...
import { BpjsPRBDetailCrudModule } from '../bpjs-prb-detail-crud.module';
import { BpjsPRBDetailCrudEditComponent } from './bpjs-prb-detail-crud-edit.component';
import {
	BpjsPRBDetailModelState,
	initialState as initialBpjsPRBDetailModelState
} from 'src/app/models/bpjsPRBDetail/bpjs_prb_detail.model.state';
...
describe('BPJS PRB Detail CRUD Edit component tests', () => {
	let fixture: ComponentFixture<BpjsPRBDetailCrudEditComponent>
	let component: BpjsPRBDetailCrudEditComponent;

	let bpjsPRBDetailModelState: BpjsPRBDetailModelState;

	let store: MockStore<{ model: BpjsPRBDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let bpjsPRBDetailCountBehaviorSubject: BehaviorSubject<number>;
	...
	let bpjsPRBDetailWithId: BpjsPRBDetailModel = defaultBpjsPRBDetailEntities[0];
	let bpjsPRBDetailModelWithIdBehaviorSubject: BehaviorSubject<BpjsPRBDetailModel>;

	bpjsPRBDetailWithId.registration = defaultRegistrationEntities[0];
	bpjsPRBDetailWithId.registrationId = bpjsPRBDetailWithId.registration.id;
	let registrationWithIdBehaviorSubject: BehaviorSubject<RegistrationModel>;

	bpjsPRBDetailWithId.bpjsDrugGenericPRB = defaultBpjsDrugGenericPRBEntities[0];
	bpjsPRBDetailWithId.bpjsDrugGenericPRBId = bpjsPRBDetailWithId.bpjsDrugGenericPRB.id;
	let bpjsDrugGenericPRBWithIdBehaviorSubject: BehaviorSubject<BpjsDrugGenericPRBModel>;


	const routerState: RouterState = {
		url: 'bpjs-prb-detail-crud',
		urls: ['bpjs-prb-detail-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsPRBDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsPRBDetailModelState = initialBpjsPRBDetailModelState;
		store.setState({model: bpjsPRBDetailModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		bpjsPRBDetailCountBehaviorSubject = new BehaviorSubject(defaultBpjsPRBDetailCount);
		...
		bpjsPRBDetailModelWithIdBehaviorSubject = new BehaviorSubject(bpjsPRBDetailWithId);
		registrationWithIdBehaviorSubject = new BehaviorSubject(bpjsPRBDetailWithId.registration);
		bpjsDrugGenericPRBWithIdBehaviorSubject = new BehaviorSubject(bpjsPRBDetailWithId.bpjsDrugGenericPRB);

		spyOn(store, 'select')
			.withArgs(getCountBpjsPRBDetailModels).and.returnValue(bpjsPRBDetailCountBehaviorSubject)
			.withArgs(getBpjsPRBDetailModelWithId, bpjsPRBDetailWithId.id).and.returnValue(bpjsPRBDetailModelWithIdBehaviorSubject)
			...
			.withArgs(getRegistrationModelWithId, bpjsPRBDetailWithId.registrationId).and.returnValue(registrationWithIdBehaviorSubject)
			.withArgs(getBpjsDrugGenericPRBModelWithId, bpjsPRBDetailWithId.bpjsDrugGenericPRBId).and.returnValue(bpjsDrugGenericPRBWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(BpjsPRBDetailCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new BpjsPRBDetailModel();
	 ...
	 */
	it ('Renders the BPJS PRB Detail CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Registration entities from model when loading in edit mode', () => {
		component.targetModel = bpjsPRBDetailWithId;
		component.targetModelId = bpjsPRBDetailWithId.id;
