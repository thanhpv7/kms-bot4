...
import { 
	getSampleCollectionItemsCollectionModels,
	getCountSampleCollectionItemsModels,
	getSampleCollectionItemsModelWithId
} from 'src/app/models/sampleCollectionItems/sample_collection_items.model.selector';
import { SampleCollectionItemsCrudModule } from '../sample-collection-items-crud.module';
import { SampleCollectionItemsCrudEditComponent } from './sample-collection-items-crud-edit.component';
import {
	SampleCollectionItemsModelState,
	initialState as initialSampleCollectionItemsModelState
} from 'src/app/models/sampleCollectionItems/sample_collection_items.model.state';
...
import { CrudTileMode } from '../sample-collection-items-crud.component';
...
describe('Sample Collection Items CRUD Edit component tests', () => {
	let fixture: ComponentFixture<SampleCollectionItemsCrudEditComponent>
	let component: SampleCollectionItemsCrudEditComponent;

	let sampleCollectionItemsModelState: SampleCollectionItemsModelState;

	let store: MockStore<{ model: SampleCollectionItemsModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let sampleCollectionItemsCountBehaviorSubject: BehaviorSubject<number>;
	...
	let sampleCollectionItemsWithId: SampleCollectionItemsModel = defaultSampleCollectionItemsEntities[0];
	let sampleCollectionItemsModelWithIdBehaviorSubject: BehaviorSubject<SampleCollectionItemsModel>;

	sampleCollectionItemsWithId.sampleCollectionInformation = defaultSampleCollectionInformationEntities[0];
	sampleCollectionItemsWithId.sampleCollectionInformationId = sampleCollectionItemsWithId.sampleCollectionInformation.id;
	let sampleCollectionInformationWithIdBehaviorSubject: BehaviorSubject<SampleCollectionInformationModel>;

	sampleCollectionItemsWithId.sampleCollectionStaff = defaultStaffEntities[0];
	sampleCollectionItemsWithId.sampleCollectionStaffId = sampleCollectionItemsWithId.sampleCollectionStaff.id;
	let sampleCollectionStaffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;

	sampleCollectionItemsWithId.sampleDeliveryStaff = defaultStaffEntities[0];
	sampleCollectionItemsWithId.sampleDeliveryStaffId = sampleCollectionItemsWithId.sampleDeliveryStaff.id;
	let sampleDeliveryStaffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;

	sampleCollectionItemsWithId.sampleProcessingStaff = defaultStaffEntities[0];
	sampleCollectionItemsWithId.sampleProcessingStaffId = sampleCollectionItemsWithId.sampleProcessingStaff.id;
	let sampleProcessingStaffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;


	const routerState: RouterState = {
		url: 'sample-collection-items-crud',
		urls: ['sample-collection-items-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: SampleCollectionItemsModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		sampleCollectionItemsModelState = initialSampleCollectionItemsModelState;
		store.setState({model: sampleCollectionItemsModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		sampleCollectionItemsCountBehaviorSubject = new BehaviorSubject(defaultSampleCollectionItemsCount);
		...
		sampleCollectionItemsModelWithIdBehaviorSubject = new BehaviorSubject(sampleCollectionItemsWithId);
		sampleCollectionInformationWithIdBehaviorSubject = new BehaviorSubject(sampleCollectionItemsWithId.sampleCollectionInformation);
		sampleCollectionStaffWithIdBehaviorSubject = new BehaviorSubject(sampleCollectionItemsWithId.sampleCollectionStaff);
		sampleDeliveryStaffWithIdBehaviorSubject = new BehaviorSubject(sampleCollectionItemsWithId.sampleDeliveryStaff);
		sampleProcessingStaffWithIdBehaviorSubject = new BehaviorSubject(sampleCollectionItemsWithId.sampleProcessingStaff);

		spyOn(store, 'select')
			.withArgs(getCountSampleCollectionItemsModels).and.returnValue(sampleCollectionItemsCountBehaviorSubject)
			.withArgs(getSampleCollectionItemsModelWithId, sampleCollectionItemsWithId.id).and.returnValue(sampleCollectionItemsModelWithIdBehaviorSubject)
			...
			.withArgs(getSampleCollectionInformationModelWithId, sampleCollectionItemsWithId.sampleCollectionInformationId).and.returnValue(sampleCollectionInformationWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, sampleCollectionItemsWithId.sampleCollectionStaffId).and.returnValue(sampleCollectionStaffWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, sampleCollectionItemsWithId.sampleDeliveryStaffId).and.returnValue(sampleDeliveryStaffWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, sampleCollectionItemsWithId.sampleProcessingStaffId).and.returnValue(sampleProcessingStaffWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				SampleCollectionItemsCrudModule,
			...
			fixture = TestBed.createComponent(SampleCollectionItemsCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new SampleCollectionItemsModel();
	 ...
	 */
	it ('Renders the Sample Collection Items CRUD Edit Component', () => {
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Sample Collection Information relation.
	*
	* The button should only exist if sampleCollectionInformationMultiCrudActive is true
	*/
	it ('Renders Sample Collection Information Add Entity button in create mode', fakeAsync(() => {
		const sampleCollectionInformationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionInformationDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!sampleCollectionInformationButton).toEqual(component.sampleCollectionInformationMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Sample Collection Information', fakeAsync(() => {
		const sampleCollectionInformationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionInformationDisplayName + ' to Model'));
		...
		if (component.sampleCollectionInformationMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialSampleCollectionInformationRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionInformationDisplayName + ' from Model')).length;

			sampleCollectionInformationButton.nativeElement.click();
			fixture.detectChanges();

			const actualSampleCollectionInformationRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionInformationDisplayName + ' from Model')).length;

			expect(actualSampleCollectionInformationRemoveButtonCount).toEqual(initialSampleCollectionInformationRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Sample Collection Information button', () => {
		const sampleCollectionInformationButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionInformationDisplayName + ' to Model'));
		...
		if (component.sampleCollectionInformationMultiCrudActive) {
			...
			const initialSampleCollectionInformationRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionInformationDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialSampleCollectionInformationRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualSampleCollectionInformationRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionInformationDisplayName + ' from Model'));

			expect(actualSampleCollectionInformationRemoveButtons.length).toEqual(initialSampleCollectionInformationRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Sample Collection Information button when one has already been added', () => {
		...
		if (component.sampleCollectionInformationMultiCrudActive) {
			...
			const updatedsampleCollectionInformationButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionInformationDisplayName + ' to Model'));

			expect(updatedsampleCollectionInformationButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Sample Collection Information entities from model when loading in edit mode', () => {
		component.targetModel = sampleCollectionItemsWithId;
		component.targetModelId = sampleCollectionItemsWithId.id;
		...
		if (component.sampleCollectionInformationMultiCrudActive) {
			...
			const sampleCollectionInformationRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionInformationDisplayName + ' from Model')).length;

			expect(sampleCollectionInformationRemoveButtonCount).toEqual(1);
		}
	});

	/**
	* Checks that the status of the add model button matches the multi entity status for the Sample Collection Staff relation.
	*
	* The button should only exist if sampleCollectionStaffMultiCrudActive is true
	*/
	it ('Renders Sample Collection Staff Add Entity button in create mode', fakeAsync(() => {
		const sampleCollectionStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionStaffDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!sampleCollectionStaffButton).toEqual(component.sampleCollectionStaffMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Sample Collection Staff', fakeAsync(() => {
		const sampleCollectionStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionStaffDisplayName + ' to Model'));
		...
		if (component.sampleCollectionStaffMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialSampleCollectionStaffRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionStaffDisplayName + ' from Model')).length;

			sampleCollectionStaffButton.nativeElement.click();
			fixture.detectChanges();

			const actualSampleCollectionStaffRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionStaffDisplayName + ' from Model')).length;

			expect(actualSampleCollectionStaffRemoveButtonCount).toEqual(initialSampleCollectionStaffRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Sample Collection Staff button', () => {
		const sampleCollectionStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionStaffDisplayName + ' to Model'));
		...
		if (component.sampleCollectionStaffMultiCrudActive) {
			...
			// create an array of elements here as opposed to just getting the length (like in previous tests)
			// as we need to interact with the button in this test
			const initialSampleCollectionStaffRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionStaffDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialSampleCollectionStaffRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualSampleCollectionStaffRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionStaffDisplayName + ' from Model'));

			expect(actualSampleCollectionStaffRemoveButtons.length).toEqual(initialSampleCollectionStaffRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Sample Collection Staff button when one has already been added', () => {
		...
		if (component.sampleCollectionStaffMultiCrudActive) {
			...
			const updatedsampleCollectionStaffButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleCollectionStaffDisplayName + ' to Model'));

			expect(updatedsampleCollectionStaffButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Sample Collection Staff entities from model when loading in edit mode', () => {
		component.targetModel = sampleCollectionItemsWithId;
		component.targetModelId = sampleCollectionItemsWithId.id;
		component.tileMode = CrudTileMode.Edit;
		component.ngOnInit();
		...
		if (component.sampleCollectionStaffMultiCrudActive) {
			// As there is always one remove button per additional entity, running a test against the number of remove buttons is a simple
			// and reliable method to determine how many entities of the given type are present in the crud page
			const sampleCollectionStaffRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleCollectionStaffDisplayName + ' from Model')).length;

			expect(sampleCollectionStaffRemoveButtonCount).toEqual(1);
		}
	});

	/**
	* Checks that the status of the add model button matches the multi entity status for the Sample Delivery Staff relation.
	*
	* The button should only exist if sampleDeliveryStaffMultiCrudActive is true
	*/
	it ('Renders Sample Delivery Staff Add Entity button in create mode', fakeAsync(() => {
		const sampleDeliveryStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleDeliveryStaffDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!sampleDeliveryStaffButton).toEqual(component.sampleDeliveryStaffMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Sample Delivery Staff', fakeAsync(() => {
		const sampleDeliveryStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleDeliveryStaffDisplayName + ' to Model'));
		...
		if (component.sampleDeliveryStaffMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialSampleDeliveryStaffRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleDeliveryStaffDisplayName + ' from Model')).length;

			sampleDeliveryStaffButton.nativeElement.click();
			fixture.detectChanges();

			const actualSampleDeliveryStaffRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleDeliveryStaffDisplayName + ' from Model')).length;

			expect(actualSampleDeliveryStaffRemoveButtonCount).toEqual(initialSampleDeliveryStaffRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Sample Delivery Staff button', () => {
		const sampleDeliveryStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleDeliveryStaffDisplayName + ' to Model'));
		...
		if (component.sampleDeliveryStaffMultiCrudActive) {
			...
			// create an array of elements here as opposed to just getting the length (like in previous tests)
			// as we need to interact with the button in this test
			const initialSampleDeliveryStaffRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleDeliveryStaffDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialSampleDeliveryStaffRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualSampleDeliveryStaffRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleDeliveryStaffDisplayName + ' from Model'));

			expect(actualSampleDeliveryStaffRemoveButtons.length).toEqual(initialSampleDeliveryStaffRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Sample Delivery Staff button when one has already been added', () => {
		...
		if (component.sampleDeliveryStaffMultiCrudActive) {
			...
			const updatedsampleDeliveryStaffButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleDeliveryStaffDisplayName + ' to Model'));

			expect(updatedsampleDeliveryStaffButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Sample Delivery Staff entities from model when loading in edit mode', () => {
		component.targetModel = sampleCollectionItemsWithId;
		component.targetModelId = sampleCollectionItemsWithId.id;
		component.tileMode = CrudTileMode.Edit;
		component.ngOnInit();
		...
		if (component.sampleDeliveryStaffMultiCrudActive) {
			// As there is always one remove button per additional entity, running a test against the number of remove buttons is a simple
			// and reliable method to determine how many entities of the given type are present in the crud page
			const sampleDeliveryStaffRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleDeliveryStaffDisplayName + ' from Model')).length;

			expect(sampleDeliveryStaffRemoveButtonCount).toEqual(1);
		}
	});

	/**
	* Checks that the status of the add model button matches the multi entity status for the Sample Processing Staff relation.
	*
	* The button should only exist if sampleProcessingStaffMultiCrudActive is true
	*/
	it ('Renders Sample Processing Staff Add Entity button in create mode', fakeAsync(() => {
		const sampleProcessingStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleProcessingStaffDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!sampleProcessingStaffButton).toEqual(component.sampleProcessingStaffMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Sample Processing Staff', fakeAsync(() => {
		const sampleProcessingStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleProcessingStaffDisplayName + ' to Model'));
		...
		if (component.sampleProcessingStaffMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialSampleProcessingStaffRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleProcessingStaffDisplayName + ' from Model')).length;

			sampleProcessingStaffButton.nativeElement.click();
			fixture.detectChanges();

			const actualSampleProcessingStaffRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleProcessingStaffDisplayName + ' from Model')).length;

			expect(actualSampleProcessingStaffRemoveButtonCount).toEqual(initialSampleProcessingStaffRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Sample Processing Staff button', () => {
		const sampleProcessingStaffButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleProcessingStaffDisplayName + ' to Model'));
		...
		if (component.sampleProcessingStaffMultiCrudActive) {
			...
			// create an array of elements here as opposed to just getting the length (like in previous tests)
			// as we need to interact with the button in this test
			const initialSampleProcessingStaffRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleProcessingStaffDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialSampleProcessingStaffRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualSampleProcessingStaffRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleProcessingStaffDisplayName + ' from Model'));

			expect(actualSampleProcessingStaffRemoveButtons.length).toEqual(initialSampleProcessingStaffRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Sample Processing Staff button when one has already been added', () => {
		...
		if (component.sampleProcessingStaffMultiCrudActive) {
			...
			const updatedsampleProcessingStaffButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.sampleProcessingStaffDisplayName + ' to Model'));

			expect(updatedsampleProcessingStaffButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Sample Processing Staff entities from model when loading in edit mode', () => {
		component.targetModel = sampleCollectionItemsWithId;
		component.targetModelId = sampleCollectionItemsWithId.id;
		component.tileMode = CrudTileMode.Edit;
		component.ngOnInit();
		...
		if (component.sampleProcessingStaffMultiCrudActive) {
			// As there is always one remove button per additional entity, running a test against the number of remove buttons is a simple
			// and reliable method to determine how many entities of the given type are present in the crud page
			const sampleProcessingStaffRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.sampleProcessingStaffDisplayName + ' from Model')).length;

			expect(sampleProcessingStaffRemoveButtonCount).toEqual(1);
		}
	});
