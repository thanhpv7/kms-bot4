...
import {InventoryAdjustmentCrudListComponent} from './inventory-adjustment-crud-list.component';
 ...
 */
describe('Inventory Adjustment Crud List Tile Component', () => {

	let fixture;
	let inventoryAdjustmentCrudListComponent: InventoryAdjustmentCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<InventoryAdjustmentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'bf4a7b85-0478-484a-b12a-3bb8ad593d7a';
	const searchContent = 'text';

	const searchableFieldCount = InventoryAdjustmentModel.searchFields.length;
	...
	const inventoryAdjustmentDataFactory = new InventoryAdjustmentDataFactory();

	const defaultDataSize = 30;
	const defaultData = inventoryAdjustmentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getInventoryAdjustmentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInventoryAdjustmentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(InventoryAdjustmentCrudListComponent);
			inventoryAdjustmentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(inventoryAdjustmentCrudListComponent.inventoryAdjustmentsCount).toBe(defaultData.length);
			...
			expect(inventoryAdjustmentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			inventoryAdjustmentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = inventoryAdjustmentDataFactory.createAll(filteredDataSize);
			...
			expect(inventoryAdjustmentCrudListComponent.inventoryAdjustmentsCount).toBe(filteredDataSize);
			expect(inventoryAdjustmentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(inventoryAdjustmentCrudListComponent.pageIndex).toBe(0);
			expect(inventoryAdjustmentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			inventoryAdjustmentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		inventoryAdjustmentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		inventoryAdjustmentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
