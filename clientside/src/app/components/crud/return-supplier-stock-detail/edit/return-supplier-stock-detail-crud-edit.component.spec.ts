...
import { 
	getReturnSupplierStockDetailCollectionModels,
...
import { ReturnSupplierStockDetailCrudModule } from '../return-supplier-stock-detail-crud.module';
import { ReturnSupplierStockDetailCrudEditComponent } from './return-supplier-stock-detail-crud-edit.component';
import {
	ReturnSupplierStockDetailModelState,
	initialState as initialReturnSupplierStockDetailModelState
} from 'src/app/models/returnSupplierStockDetail/return_supplier_stock_detail.model.state';
...
import { CrudTileMode } from '../return-supplier-stock-detail-crud.component';
...
describe('Return Supplier Stock Detail CRUD Edit component tests', () => {
	let fixture: ComponentFixture<ReturnSupplierStockDetailCrudEditComponent>
	let component: ReturnSupplierStockDetailCrudEditComponent;

	let returnSupplierStockDetailModelState: ReturnSupplierStockDetailModelState;

	let store: MockStore<{ model: ReturnSupplierStockDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let returnSupplierStockDetailCountBehaviorSubject: BehaviorSubject<number>;
	...
	let returnSupplierStockDetailWithId: ReturnSupplierStockDetailModel = defaultReturnSupplierStockDetailEntities[0];
	let returnSupplierStockDetailModelWithIdBehaviorSubject: BehaviorSubject<ReturnSupplierStockDetailModel>;

	returnSupplierStockDetailWithId.returnSupplier = defaultReturnSupplierEntities[0];
	returnSupplierStockDetailWithId.returnSupplierId = returnSupplierStockDetailWithId.returnSupplier.id;
	let returnSupplierWithIdBehaviorSubject: BehaviorSubject<ReturnSupplierModel>;

	returnSupplierStockDetailWithId.stockCatalogue = defaultStockCatalogueEntities[0];
	returnSupplierStockDetailWithId.stockCatalogueId = returnSupplierStockDetailWithId.stockCatalogue.id;
	let stockCatalogueWithIdBehaviorSubject: BehaviorSubject<StockCatalogueModel>;


	const routerState: RouterState = {
		url: 'return-supplier-stock-detail-crud',
		urls: ['return-supplier-stock-detail-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: ReturnSupplierStockDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		returnSupplierStockDetailModelState = initialReturnSupplierStockDetailModelState;
		store.setState({model: returnSupplierStockDetailModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		returnSupplierStockDetailCountBehaviorSubject = new BehaviorSubject(defaultReturnSupplierStockDetailCount);
		...
		returnSupplierStockDetailModelWithIdBehaviorSubject = new BehaviorSubject(returnSupplierStockDetailWithId);
		returnSupplierWithIdBehaviorSubject = new BehaviorSubject(returnSupplierStockDetailWithId.returnSupplier);
		stockCatalogueWithIdBehaviorSubject = new BehaviorSubject(returnSupplierStockDetailWithId.stockCatalogue);

		spyOn(store, 'select')
			.withArgs(getCountReturnSupplierStockDetailModels).and.returnValue(returnSupplierStockDetailCountBehaviorSubject)
			.withArgs(getReturnSupplierStockDetailModelWithId, returnSupplierStockDetailWithId.id).and.returnValue(returnSupplierStockDetailModelWithIdBehaviorSubject)
			...
			.withArgs(getReturnSupplierModelWithId, returnSupplierStockDetailWithId.returnSupplierId).and.returnValue(returnSupplierWithIdBehaviorSubject)
			.withArgs(getStockCatalogueModelWithId, returnSupplierStockDetailWithId.stockCatalogueId).and.returnValue(stockCatalogueWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				ReturnSupplierStockDetailCrudModule,
			...
			fixture = TestBed.createComponent(ReturnSupplierStockDetailCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new ReturnSupplierStockDetailModel();
	 ...
	 */
	it ('Renders the Return Supplier Stock Detail CRUD Edit Component', () => {
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Return Supplier relation.
	*
	* The button should only exist if returnSupplierMultiCrudActive is true
	*/
	it ('Renders Return Supplier Add Entity button in create mode', fakeAsync(() => {
		const returnSupplierButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.returnSupplierDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!returnSupplierButton).toEqual(component.returnSupplierMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Return Supplier', fakeAsync(() => {
		const returnSupplierButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.returnSupplierDisplayName + ' to Model'));
		...
		if (component.returnSupplierMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialReturnSupplierRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.returnSupplierDisplayName + ' from Model')).length;

			returnSupplierButton.nativeElement.click();
			fixture.detectChanges();

			const actualReturnSupplierRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.returnSupplierDisplayName + ' from Model')).length;

			expect(actualReturnSupplierRemoveButtonCount).toEqual(initialReturnSupplierRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Return Supplier button', () => {
		const returnSupplierButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.returnSupplierDisplayName + ' to Model'));
		...
		if (component.returnSupplierMultiCrudActive) {
			...
			const initialReturnSupplierRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.returnSupplierDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialReturnSupplierRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualReturnSupplierRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.returnSupplierDisplayName + ' from Model'));

			expect(actualReturnSupplierRemoveButtons.length).toEqual(initialReturnSupplierRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Return Supplier button when one has already been added', () => {
		...
		if (component.returnSupplierMultiCrudActive) {
			...
			const updatedreturnSupplierButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.returnSupplierDisplayName + ' to Model'));

			expect(updatedreturnSupplierButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Return Supplier entities from model when loading in edit mode', () => {
		component.targetModel = returnSupplierStockDetailWithId;
		component.targetModelId = returnSupplierStockDetailWithId.id;
		...
		if (component.returnSupplierMultiCrudActive) {
			...
			const returnSupplierRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.returnSupplierDisplayName + ' from Model')).length;

			expect(returnSupplierRemoveButtonCount).toEqual(1);
		}
	});
