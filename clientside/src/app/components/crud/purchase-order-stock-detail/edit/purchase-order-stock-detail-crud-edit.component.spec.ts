...
import { 
	getPurchaseOrderStockDetailCollectionModels,
	getCountPurchaseOrderStockDetailModels,
	getPurchaseOrderStockDetailModelWithId
} from 'src/app/models/purchaseOrderStockDetail/purchase_order_stock_detail.model.selector';
import { PurchaseOrderStockDetailCrudModule } from '../purchase-order-stock-detail-crud.module';
import { PurchaseOrderStockDetailCrudEditComponent } from './purchase-order-stock-detail-crud-edit.component';
import {
	PurchaseOrderStockDetailModelState,
	initialState as initialPurchaseOrderStockDetailModelState
} from 'src/app/models/purchaseOrderStockDetail/purchase_order_stock_detail.model.state';
...
import { CrudTileMode } from '../purchase-order-stock-detail-crud.component';
...
describe('Purchase Order Stock Detail CRUD Edit component tests', () => {
	let fixture: ComponentFixture<PurchaseOrderStockDetailCrudEditComponent>
	let component: PurchaseOrderStockDetailCrudEditComponent;

	let purchaseOrderStockDetailModelState: PurchaseOrderStockDetailModelState;

	let store: MockStore<{ model: PurchaseOrderStockDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let purchaseOrderStockDetailCountBehaviorSubject: BehaviorSubject<number>;
	...
	let purchaseOrderStockDetailWithId: PurchaseOrderStockDetailModel = defaultPurchaseOrderStockDetailEntities[0];
	let purchaseOrderStockDetailModelWithIdBehaviorSubject: BehaviorSubject<PurchaseOrderStockDetailModel>;

	purchaseOrderStockDetailWithId.purchaseOrder = defaultPurchaseOrderEntities[0];
	purchaseOrderStockDetailWithId.purchaseOrderId = purchaseOrderStockDetailWithId.purchaseOrder.id;
	let purchaseOrderWithIdBehaviorSubject: BehaviorSubject<PurchaseOrderModel>;

	purchaseOrderStockDetailWithId.purchaseRequisitionStockDetail = defaultPurchaseRequisitionStockDetailEntities[0];
	purchaseOrderStockDetailWithId.purchaseRequisitionStockDetailId = purchaseOrderStockDetailWithId.purchaseRequisitionStockDetail.id;
	let purchaseRequisitionStockDetailWithIdBehaviorSubject: BehaviorSubject<PurchaseRequisitionStockDetailModel>;

	purchaseOrderStockDetailWithId.stockCatalogue = defaultStockCatalogueEntities[0];
	purchaseOrderStockDetailWithId.stockCatalogueId = purchaseOrderStockDetailWithId.stockCatalogue.id;
	let stockCatalogueWithIdBehaviorSubject: BehaviorSubject<StockCatalogueModel>;

	purchaseOrderStockDetailWithId.goodsReceiveNoteItems = defaultGoodsReceiveNoteItemEntities;
	purchaseOrderStockDetailWithId.goodsReceiveNoteItemsIds = purchaseOrderStockDetailWithId.goodsReceiveNoteItems.map(goodsReceiveNoteItems => goodsReceiveNoteItems.id);


	const routerState: RouterState = {
		url: 'purchase-order-stock-detail-crud',
		urls: ['purchase-order-stock-detail-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PurchaseOrderStockDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		purchaseOrderStockDetailModelState = initialPurchaseOrderStockDetailModelState;
		store.setState({model: purchaseOrderStockDetailModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		purchaseOrderStockDetailCountBehaviorSubject = new BehaviorSubject(defaultPurchaseOrderStockDetailCount);
		...
		purchaseOrderStockDetailModelWithIdBehaviorSubject = new BehaviorSubject(purchaseOrderStockDetailWithId);
		purchaseOrderWithIdBehaviorSubject = new BehaviorSubject(purchaseOrderStockDetailWithId.purchaseOrder);
		purchaseRequisitionStockDetailWithIdBehaviorSubject = new BehaviorSubject(purchaseOrderStockDetailWithId.purchaseRequisitionStockDetail);
		stockCatalogueWithIdBehaviorSubject = new BehaviorSubject(purchaseOrderStockDetailWithId.stockCatalogue);

		spyOn(store, 'select')
			.withArgs(getCountPurchaseOrderStockDetailModels).and.returnValue(purchaseOrderStockDetailCountBehaviorSubject)
			.withArgs(getPurchaseOrderStockDetailModelWithId, purchaseOrderStockDetailWithId.id).and.returnValue(purchaseOrderStockDetailModelWithIdBehaviorSubject)
			...
			.withArgs(getPurchaseOrderModelWithId, purchaseOrderStockDetailWithId.purchaseOrderId).and.returnValue(purchaseOrderWithIdBehaviorSubject)
			.withArgs(getPurchaseRequisitionStockDetailModelWithId, purchaseOrderStockDetailWithId.purchaseRequisitionStockDetailId).and.returnValue(purchaseRequisitionStockDetailWithIdBehaviorSubject)
			.withArgs(getStockCatalogueModelWithId, purchaseOrderStockDetailWithId.stockCatalogueId).and.returnValue(stockCatalogueWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				PurchaseOrderStockDetailCrudModule,
			...
			fixture = TestBed.createComponent(PurchaseOrderStockDetailCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new PurchaseOrderStockDetailModel();
	 ...
	 */
	it ('Renders the Purchase Order Stock Detail CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Purchase Order entities from model when loading in edit mode', () => {
		component.targetModel = purchaseOrderStockDetailWithId;
		component.targetModelId = purchaseOrderStockDetailWithId.id;
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the Purchase Requisition Stock Detail relation.
	*
	* The button should only exist if purchaseRequisitionStockDetailMultiCrudActive is true
	*/
	it ('Renders Purchase Requisition Stock Detail Add Entity button in create mode', fakeAsync(() => {
		const purchaseRequisitionStockDetailButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.purchaseRequisitionStockDetailDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!purchaseRequisitionStockDetailButton).toEqual(component.purchaseRequisitionStockDetailMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new Purchase Requisition Stock Detail', fakeAsync(() => {
		const purchaseRequisitionStockDetailButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.purchaseRequisitionStockDetailDisplayName + ' to Model'));
		...
		if (component.purchaseRequisitionStockDetailMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialPurchaseRequisitionStockDetailRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.purchaseRequisitionStockDetailDisplayName + ' from Model')).length;

			purchaseRequisitionStockDetailButton.nativeElement.click();
			fixture.detectChanges();

			const actualPurchaseRequisitionStockDetailRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.purchaseRequisitionStockDetailDisplayName + ' from Model')).length;

			expect(actualPurchaseRequisitionStockDetailRemoveButtonCount).toEqual(initialPurchaseRequisitionStockDetailRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove Purchase Requisition Stock Detail button', () => {
		const purchaseRequisitionStockDetailButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.purchaseRequisitionStockDetailDisplayName + ' to Model'));
		...
		if (component.purchaseRequisitionStockDetailMultiCrudActive) {
			...
			// create an array of elements here as opposed to just getting the length (like in previous tests)
			// as we need to interact with the button in this test
			const initialPurchaseRequisitionStockDetailRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.purchaseRequisitionStockDetailDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialPurchaseRequisitionStockDetailRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualPurchaseRequisitionStockDetailRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.purchaseRequisitionStockDetailDisplayName + ' from Model'));

			expect(actualPurchaseRequisitionStockDetailRemoveButtons.length).toEqual(initialPurchaseRequisitionStockDetailRemoveButtons.length - 1);
		}
	});

	it ('should disable the add Purchase Requisition Stock Detail button when one has already been added', () => {
		...
		if (component.purchaseRequisitionStockDetailMultiCrudActive) {
			...
			const updatedpurchaseRequisitionStockDetailButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.purchaseRequisitionStockDetailDisplayName + ' to Model'));

			expect(updatedpurchaseRequisitionStockDetailButton.nativeElement.disabled).toBe(true);
		}
	});

	it ('renders related Purchase Requisition Stock Detail entities from model when loading in edit mode', () => {
		component.targetModel = purchaseOrderStockDetailWithId;
		component.targetModelId = purchaseOrderStockDetailWithId.id;
		component.tileMode = CrudTileMode.Edit;
		component.ngOnInit();
		...
		if (component.purchaseRequisitionStockDetailMultiCrudActive) {
			// As there is always one remove button per additional entity, running a test against the number of remove buttons is a simple
			// and reliable method to determine how many entities of the given type are present in the crud page
			const purchaseRequisitionStockDetailRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.purchaseRequisitionStockDetailDisplayName + ' from Model')).length;

			expect(purchaseRequisitionStockDetailRemoveButtonCount).toEqual(1);
		}
	});
	...
	*/
	...
	it ('renders related Goods Receive Note Items entities from model when loading in edit mode', () => {
		...
		if (component.goodsReceiveNoteItemsMultiCrudActive) {
			...
			expect(goodsReceiveNoteItemsRemoveButtonCount).toEqual(purchaseOrderStockDetailWithId.goodsReceiveNoteItems.length);
		}
	});
