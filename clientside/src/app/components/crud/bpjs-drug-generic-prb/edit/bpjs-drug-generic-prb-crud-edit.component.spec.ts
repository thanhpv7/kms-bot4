...
import { BpjsDrugGenericPRBCrudEditComponent } from './bpjs-drug-generic-prb-crud-edit.component';
import {
...
describe('BPJS Drug Generic PRB CRUD Edit component tests', () => {
	let fixture: ComponentFixture<BpjsDrugGenericPRBCrudEditComponent>
	let component: BpjsDrugGenericPRBCrudEditComponent;
	...
	let bpjsDrugGenericPRBCountBehaviorSubject: BehaviorSubject<number>;
	...
	let bpjsDrugGenericPRBWithId: BpjsDrugGenericPRBModel = defaultBpjsDrugGenericPRBEntities[0];
	let bpjsDrugGenericPRBModelWithIdBehaviorSubject: BehaviorSubject<BpjsDrugGenericPRBModel>;

	bpjsDrugGenericPRBWithId.stockCatalogue = defaultStockCatalogueEntities[0];
	bpjsDrugGenericPRBWithId.stockCatalogueId = bpjsDrugGenericPRBWithId.stockCatalogue.id;
	let stockCatalogueWithIdBehaviorSubject: BehaviorSubject<StockCatalogueModel>;

	bpjsDrugGenericPRBWithId.bpjsPRBDetails = defaultBpjsPRBDetailEntities;
	bpjsDrugGenericPRBWithId.bpjsPRBDetailsIds = bpjsDrugGenericPRBWithId.bpjsPRBDetails.map(bpjsPRBDetails => bpjsPRBDetails.id);
	...
	function spySelectorsInStore() {
		...
		bpjsDrugGenericPRBCountBehaviorSubject = new BehaviorSubject(defaultBpjsDrugGenericPRBCount);
		...
		bpjsDrugGenericPRBModelWithIdBehaviorSubject = new BehaviorSubject(bpjsDrugGenericPRBWithId);
		stockCatalogueWithIdBehaviorSubject = new BehaviorSubject(bpjsDrugGenericPRBWithId.stockCatalogue);

		spyOn(store, 'select')
			.withArgs(getCountBpjsDrugGenericPRBModels).and.returnValue(bpjsDrugGenericPRBCountBehaviorSubject)
			.withArgs(getBpjsDrugGenericPRBModelWithId, bpjsDrugGenericPRBWithId.id).and.returnValue(bpjsDrugGenericPRBModelWithIdBehaviorSubject)
			...
			.withArgs(getStockCatalogueModelWithId, bpjsDrugGenericPRBWithId.stockCatalogueId).and.returnValue(stockCatalogueWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(BpjsDrugGenericPRBCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new BpjsDrugGenericPRBModel();
	 ...
	 */
	it ('Renders the BPJS Drug Generic PRB CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Stock Catalogue entities from model when loading in edit mode', () => {
		component.targetModel = bpjsDrugGenericPRBWithId;
		component.targetModelId = bpjsDrugGenericPRBWithId.id;
	...
	/**
	* Checks that the status of the add model button matches the multi entity status for the BPJS PRB Details relation.
	*
	* The button should only exist if bpjsPRBDetailsMultiCrudActive is true
	*/
	it ('Renders BPJS PRB Details Add Entity button in create mode', fakeAsync(() => {
		const bpjsPRBDetailsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.bpjsPRBDetailsDisplayName + ' to Model'));

		// For this test we don't need to interact with the button, as we are looking to prove that it exists and is present on the page
		// prepending the element with `!!` will turn the object into a boolean which is true if the element exists and false otherwise
		// If multi crud is off for this entity, then we expect the button to not exist, so we can check for equality with the MultiCrudActive attribute
		expect(!!bpjsPRBDetailsButton).toEqual(component.bpjsPRBDetailsMultiCrudActive);
	}));


	it ('Adds a new entity to the page when clicking button to add new BPJS PRB Details', fakeAsync(() => {
		const bpjsPRBDetailsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.bpjsPRBDetailsDisplayName + ' to Model'));
		...
		if (component.bpjsPRBDetailsMultiCrudActive) {
			// After clicking the add entity button, we will expect that the number of these buttons increases
			// We store this value now so that we can access it following interaction with the component
			const initialBpjsPRBDetailsRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.bpjsPRBDetailsDisplayName + ' from Model')).length;

			bpjsPRBDetailsButton.nativeElement.click();
			fixture.detectChanges();

			const actualBpjsPRBDetailsRemoveButtonCount = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.bpjsPRBDetailsDisplayName + ' from Model')).length;

			expect(actualBpjsPRBDetailsRemoveButtonCount).toEqual(initialBpjsPRBDetailsRemoveButtonCount + 1);
		}
	}));

	it ('Removes an additional entity from the page when clicking remove BPJS PRB Details button', () => {
		const bpjsPRBDetailsButton: DebugElement = fixture.debugElement
				.queryAll(By.css('button.btn__add-entity'))
				.find(element => element.nativeElement.textContent.includes('Add New ' + component.bpjsPRBDetailsDisplayName + ' to Model'));
		...
		if (component.bpjsPRBDetailsMultiCrudActive) {
			...
			// create an array of elements here as opposed to just getting the length (like in previous tests)
			// as we need to interact with the button in this test
			const initialBpjsPRBDetailsRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.bpjsPRBDetailsDisplayName + ' from Model'));

			// As we have already tested that clicking the add button adds the element, we know there will be at lease one button here to click
			initialBpjsPRBDetailsRemoveButtons[0].nativeElement.click();
			fixture.detectChanges();

			const actualBpjsPRBDetailsRemoveButtons = fixture.debugElement
					.queryAll(By.css('button.btn__remove-entity'))
					.filter(element => element.nativeElement.textContent.includes('Remove ' + component.bpjsPRBDetailsDisplayName + ' from Model'));

			expect(actualBpjsPRBDetailsRemoveButtons.length).toEqual(initialBpjsPRBDetailsRemoveButtons.length - 1);
		}
	});

	it ('should not disable the add BPJS PRB Details button when one has already been added', () => {
		...
		if (component.bpjsPRBDetailsMultiCrudActive) {
			...
			const updatedbpjsPRBDetailsButton: DebugElement = fixture.debugElement
					.queryAll(By.css('button.btn__add-entity'))
					.find(element => element.nativeElement.textContent.includes('Add New ' + component.bpjsPRBDetailsDisplayName + ' to Model'));

			expect(updatedbpjsPRBDetailsButton.nativeElement.disabled).toBe(false);
		}
	});

	it ('renders related BPJS PRB Details entities from model when loading in edit mode', () => {
		component.targetModel = bpjsDrugGenericPRBWithId;
		component.targetModelId = bpjsDrugGenericPRBWithId.id;
		component.tileMode = CrudTileMode.Edit;
		component.ngOnInit();
		...
		if (component.bpjsPRBDetailsMultiCrudActive) {
			// As there is always one remove button per additional entity, running a test against the number of remove buttons is a simple
			// and reliable method to determine how many entities of the given type are present in the crud page
			const bpjsPRBDetailsRemoveButtonCount = fixture.debugElement
				.queryAll(By.css('button.btn__remove-entity'))
				.filter(element => element.nativeElement.textContent.includes('Remove ' + component.bpjsPRBDetailsDisplayName + ' from Model')).length;

			expect(bpjsPRBDetailsRemoveButtonCount).toEqual(bpjsDrugGenericPRBWithId.bpjsPRBDetails.length);
		}
	});
