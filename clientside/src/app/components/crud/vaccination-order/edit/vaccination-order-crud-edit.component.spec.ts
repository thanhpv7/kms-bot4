...
import { VaccinationOrderCrudModule } from '../vaccination-order-crud.module';
import { VaccinationOrderCrudEditComponent } from './vaccination-order-crud-edit.component';
import {
	VaccinationOrderModelState,
	initialState as initialVaccinationOrderModelState
} from 'src/app/models/vaccinationOrder/vaccination_order.model.state';
...
describe('Vaccination Order CRUD Edit component tests', () => {
	let fixture: ComponentFixture<VaccinationOrderCrudEditComponent>
	let component: VaccinationOrderCrudEditComponent;

	let vaccinationOrderModelState: VaccinationOrderModelState;

	let store: MockStore<{ model: VaccinationOrderModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;
	let vaccinationOrderCountBehaviorSubject: BehaviorSubject<number>;
	...
	let vaccinationOrderWithId: VaccinationOrderModel = defaultVaccinationOrderEntities[0];
	let vaccinationOrderModelWithIdBehaviorSubject: BehaviorSubject<VaccinationOrderModel>;

	vaccinationOrderWithId.registration = defaultRegistrationEntities[0];
	vaccinationOrderWithId.registrationId = vaccinationOrderWithId.registration.id;
	let registrationWithIdBehaviorSubject: BehaviorSubject<RegistrationModel>;

	vaccinationOrderWithId.staff = defaultStaffEntities[0];
	vaccinationOrderWithId.staffId = vaccinationOrderWithId.staff.id;
	let staffWithIdBehaviorSubject: BehaviorSubject<StaffModel>;

	vaccinationOrderWithId.vaccinationOrderDetails = defaultVaccinationOrderDetailEntities;
	vaccinationOrderWithId.vaccinationOrderDetailsIds = vaccinationOrderWithId.vaccinationOrderDetails.map(vaccinationOrderDetails => vaccinationOrderDetails.id);


	const routerState: RouterState = {
		url: 'vaccination-order-crud',
		urls: ['vaccination-order-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	function spySelectorsInStore() {
		...
		store = TestBed.inject(Store) as MockStore<{ model: VaccinationOrderModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		vaccinationOrderModelState = initialVaccinationOrderModelState;
		store.setState({model: vaccinationOrderModelState});
		routerStateBehaviorSubject = new BehaviorSubject(routerState);
		vaccinationOrderCountBehaviorSubject = new BehaviorSubject(defaultVaccinationOrderCount);
		...
		vaccinationOrderModelWithIdBehaviorSubject = new BehaviorSubject(vaccinationOrderWithId);
		registrationWithIdBehaviorSubject = new BehaviorSubject(vaccinationOrderWithId.registration);
		staffWithIdBehaviorSubject = new BehaviorSubject(vaccinationOrderWithId.staff);

		spyOn(store, 'select')
			.withArgs(getCountVaccinationOrderModels).and.returnValue(vaccinationOrderCountBehaviorSubject)
			.withArgs(getVaccinationOrderModelWithId, vaccinationOrderWithId.id).and.returnValue(vaccinationOrderModelWithIdBehaviorSubject)
			...
			.withArgs(getRegistrationModelWithId, vaccinationOrderWithId.registrationId).and.returnValue(registrationWithIdBehaviorSubject)
			.withArgs(getStaffModelWithId, vaccinationOrderWithId.staffId).and.returnValue(staffWithIdBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(VaccinationOrderCrudEditComponent);
			component = fixture.debugElement.componentInstance;
			component.ngOnInit();
			component.targetModel = new VaccinationOrderModel();
	 ...
	 */
	it ('Renders the Vaccination Order CRUD Edit Component', () => {
	...
	*/
	...
	it ('renders related Registration entities from model when loading in edit mode', () => {
		component.targetModel = vaccinationOrderWithId;
		component.targetModelId = vaccinationOrderWithId.id;
	...
	*/
	...
	it ('renders related Vaccination Order Details entities from model when loading in edit mode', () => {
		...
		if (component.vaccinationOrderDetailsMultiCrudActive) {
			...
			expect(vaccinationOrderDetailsRemoveButtonCount).toEqual(vaccinationOrderWithId.vaccinationOrderDetails.length);
		}
	});
