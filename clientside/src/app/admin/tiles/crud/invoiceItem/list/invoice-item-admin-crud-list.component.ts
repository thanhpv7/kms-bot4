...
import {InvoiceItemModelState} from 'src/app/models/invoiceItem/invoice_item.model.state';
...
import {
	getInvoiceItemCollectionModels,
	getInvoiceItemCollectionCount
...
@Component({
	selector: 'cb-invoice-item-admin-crud-list',
	templateUrl: './invoice-item-admin-crud-list.component.html',
	styleUrls: [
		'./invoice-item-admin-crud-list.component.scss',
...
export class AdminInvoiceItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InvoiceItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InvoiceItemModel.searchFields,
	...
	/**
	 * List of Invoice Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	invoiceItems: Observable<InvoiceItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	invoiceItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.invoiceItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.invoiceItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceItemAction(
				modelAction.InvoiceItemModelActionTypes.INITIALISE_INVOICE_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceItemAction(
				modelAction.InvoiceItemModelActionTypes.FETCH_INVOICE_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.invoiceItems = this.store.select(getInvoiceItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInvoiceItemCollectionCount, this.collectionId).subscribe(
				(count) => this.invoiceItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InvoiceItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InvoiceItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InvoiceItemModelActionTypes.EXPORT_ALL_INVOICE_ITEM;
		} else {
			actionType = modelAction.InvoiceItemModelActionTypes.EXPORT_INVOICE_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InvoiceItemAction(
				modelAction.InvoiceItemModelActionTypes.IMPORT_INVOICE_ITEMS,
				{
					file: file,
				},
				[new modelAction.InvoiceItemAction(
			...
			window.open(environment.API_URL + '/api/invoiceitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InvoiceItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Invoice Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Invoice Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InvoiceItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InvoiceItemModelActionTypes.DELETE_INVOICE_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.InvoiceItemModelActionTypes.DELETE_ALL_INVOICE_ITEM;
						} else {
							actionType = modelAction.InvoiceItemModelActionTypes.DELETE_INVOICE_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InvoiceItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVOICE_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVOICE_ITEM and DELETE_INVOICE_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InvoiceItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.invoiceItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
