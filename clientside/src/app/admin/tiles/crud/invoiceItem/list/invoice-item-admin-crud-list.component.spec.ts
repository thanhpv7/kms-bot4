...
import {AdminInvoiceItemCrudListComponent} from './invoice-item-admin-crud-list.component';
...
import {AdminInvoiceItemCrudModule} from '../invoice-item-admin-crud.module';
import {
	InvoiceItemModelState,
	initialState as initialInvoiceItemModelState
} from 'src/app/models/invoiceItem/invoice_item.model.state';
 ...
 */
describe('Invoice Item Crud List Tile Component', () => {

	let fixture;
	let invoiceItemCrudListComponent: AdminInvoiceItemCrudListComponent;

	let store: MockStore<{ model: InvoiceItemModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let invoiceItemModelState: InvoiceItemModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InvoiceItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ff5ecec2-f883-437a-8a66-0615d2d4c901';
	const searchContent = 'text';

	const searchableFieldCount = InvoiceItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'invoice-item-admin-crud',
		urls: ['invoice-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const invoiceItemDataFactory = new InvoiceItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = invoiceItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InvoiceItemModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		invoiceItemModelState = initialInvoiceItemModelState;
		store.setState({model: invoiceItemModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInvoiceItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInvoiceItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminInvoiceItemCrudListComponent);
			invoiceItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(invoiceItemCrudListComponent.invoiceItemsCount).toBe(defaultData.length);
			...
			expect(invoiceItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			invoiceItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = invoiceItemDataFactory.createAll(filteredDataSize);
			...
			expect(invoiceItemCrudListComponent.invoiceItemsCount).toBe(filteredDataSize);
			expect(invoiceItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(invoiceItemCrudListComponent.pageIndex).toBe(0);
			expect(invoiceItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			invoiceItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		invoiceItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		invoiceItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
