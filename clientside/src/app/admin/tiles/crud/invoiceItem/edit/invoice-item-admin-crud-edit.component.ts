...
import * as modelAction from 'src/app/models/invoiceItem/invoice_item.model.action';
import {InvoiceItemModelState, InvoiceItemModelAudit} from 'src/app/models/invoiceItem/invoice_item.model.state';
import {InvoiceItemModel} from 'src/app/models/invoiceItem/invoice_item.model';
import {
	getInvoiceItemModelAuditsByEntityId,
	getInvoiceItemModelWithId,
	getCountInvoiceItemModels,
...
import * as retailPharmacyInvoiceModelAction from 'src/app/models/retailPharmacyInvoice/retail_pharmacy_invoice.model.action';
import {
	getRetailPharmacyInvoiceCollectionModels, getRetailPharmacyInvoiceModelWithId,
...
import {CrudTileMode} from '../invoice-item-admin-crud.component';
...
@Component({
	selector: 'cb-invoice-item-admin-crud-edit',
	templateUrl: './invoice-item-admin-crud-edit.component.html',
	styleUrls: [
		'./invoice-item-admin-crud-edit.component.scss',
...
export class AdminInvoiceItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'ff5ecec2-f883-437a-8a66-0615d2d4c901';
	 ...
	 */
	invoiceItemAudits$: Observable<InvoiceItemModelAudit[]>;
	...
	invoiceMultiCrudActive: boolean = false;
	...
	invoiceOppositeRelation = 'invoiceItems';
	invoiceRelations: ModelRelation[] = this.invoiceMultiCrudActive ? InvoiceModel.getRelations()
		.map(relation => {
			if (relation.id === this.invoiceOppositeRelation) {
	...
	medicalFeeMultiCrudActive: boolean = false;
	medicalFeeDisplayName = 'Medical Fee';
	...
	medicalFeeOppositeRelation = 'invoiceItems';
	medicalFeeRelations: ModelRelation[] = this.medicalFeeMultiCrudActive ? MedicalFeeModel.getRelations()
		.map(relation => {
			if (relation.id === this.medicalFeeOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Medication Header and Invoice Items here] off begin
	medicationHeaderMultiCrudActive: boolean = false;
	medicationHeaderDisplayName = 'Medication Header';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Medication Header and Invoice Items here] end
	medicationHeaderOppositeRelation = 'invoiceItems';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Prescription and Invoice Items here] off begin
	prescriptionMultiCrudActive: boolean = false;
	prescriptionDisplayName = 'Prescription';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Prescription and Invoice Items here] end
	prescriptionOppositeRelation = 'invoiceItems';
	...
	retailPharmacyInvoiceMultiCrudActive: boolean = false;
	retailPharmacyInvoiceDisplayName = 'Retail Pharmacy Invoice';
	...
	retailPharmacyInvoiceOppositeRelation = 'invoiceItems';
	retailPharmacyInvoiceRelations: ModelRelation[] = this.retailPharmacyInvoiceMultiCrudActive ? RetailPharmacyInvoiceModel.getRelations()
		.map(relation => {
			if (relation.id === this.retailPharmacyInvoiceOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	serviceNumberMultiCrudActive: boolean = false;
	serviceNumberDisplayName = 'Service Number';
	...
	serviceNumberOppositeRelation = 'invoiceItems';
	serviceNumberRelations: ModelRelation[] = this.serviceNumberMultiCrudActive ? RegistrationModel.getRelations()
		.map(relation => {
			if (relation.id === this.serviceNumberOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Schema and Invoice Items here] off begin
	tariffSchemaMultiCrudActive: boolean = false;
	tariffSchemaDisplayName = 'Tariff Schema';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Schema and Invoice Items here] end
	tariffSchemaOppositeRelation = 'invoiceItems';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Invoice Item and Invoice Item Components here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Invoice Item and Invoice Item Components here] end
	invoiceItemComponentsOppositeRelation = 'invoiceItem';
	 ...
	 */
	modelProperties: ModelProperty[] = InvoiceItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InvoiceItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InvoiceItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'invoice' && this.invoiceMultiCrudActive) {
					exp.fields = InvoiceModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'medicalFee' && this.medicalFeeMultiCrudActive) {
				...
				if (relation.id === 'retailPharmacyInvoice' && this.retailPharmacyInvoiceMultiCrudActive) {
					exp.fields = RetailPharmacyInvoiceModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'serviceNumber' && this.serviceNumberMultiCrudActive) {
	...
	constructor(
		private readonly store: Store<{ model: InvoiceItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInvoiceItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InvoiceItemModel();
		...
		if (this.targetModelId) {
			this.invoiceItemAudits$ = this.store.select(getInvoiceItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InvoiceItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InvoiceItemAction(
			modelAction.InvoiceItemModelActionTypes.FETCH_INVOICE_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInvoiceItemModelWithId, this.targetModelId)
			...
			model = new InvoiceItemModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InvoiceItemAction(
			modelAction.InvoiceItemModelActionTypes.FETCH_INVOICE_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InvoiceItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.invoiceMultiCrudActive || this.medicalFeeMultiCrudActive || this.medicationHeaderMultiCrudActive || this.prescriptionMultiCrudActive || this.retailPharmacyInvoiceMultiCrudActive || this.serviceNumberMultiCrudActive || this.tariffSchemaMultiCrudActive || this.invoiceItemComponentsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InvoiceItemAction(
						modelAction.InvoiceItemModelActionTypes.COUNT_INVOICE_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InvoiceItemAction(
				modelAction.InvoiceItemModelActionTypes.CREATE_INVOICE_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InvoiceItemAction(
				modelAction.InvoiceItemModelActionTypes.UPDATE_INVOICE_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InvoiceItemModel = new InvoiceItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInvoiceItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InvoiceItemModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.invoiceDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InvoiceModel.getProps(), this.invoiceRelations, this.isViewOnly),
					new InvoiceModel(),
					InvoiceModel,
					this.invoiceDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.invoiceRelations,
					true
				);
			...
			if (event.entityName === this.medicalFeeDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MedicalFeeModel.getProps(), this.medicalFeeRelations, this.isViewOnly),
					new MedicalFeeModel(),
					MedicalFeeModel,
					this.medicalFeeDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.medicalFeeRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.retailPharmacyInvoiceDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RetailPharmacyInvoiceModel.getProps(), this.retailPharmacyInvoiceRelations, this.isViewOnly),
					new RetailPharmacyInvoiceModel(),
					RetailPharmacyInvoiceModel,
					this.retailPharmacyInvoiceDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.retailPharmacyInvoiceRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.serviceNumberDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RegistrationModel.getProps(), this.serviceNumberRelations, this.isViewOnly),
					new RegistrationModel(),
					RegistrationModel,
					this.serviceNumberDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.serviceNumberRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InvoiceItemModel.getProps();
		this.modelRelations = InvoiceItemModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'invoice';
		}).hideElement = this.invoiceMultiCrudActive || this.modelRelations.find(rel => rel.id === 'invoice').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'medicalFee';
		}).hideElement = this.medicalFeeMultiCrudActive || this.modelRelations.find(rel => rel.id === 'medicalFee').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'retailPharmacyInvoice';
		}).hideElement = this.retailPharmacyInvoiceMultiCrudActive || this.modelRelations.find(rel => rel.id === 'retailPharmacyInvoice').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'serviceNumber';
		}).hideElement = this.serviceNumberMultiCrudActive || this.modelRelations.find(rel => rel.id === 'serviceNumber').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.invoiceMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.invoiceDisplayName, many: false});
		}

		if (this.medicalFeeMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.medicalFeeDisplayName, many: false});
		}
		...
		if (this.retailPharmacyInvoiceMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.retailPharmacyInvoiceDisplayName, many: false});
		}

		if (this.serviceNumberMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.serviceNumberDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.invoice && this.targetModel.invoice.id && this.invoiceMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InvoiceModel,
				this.invoiceOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.invoice.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getInvoiceModelWithId, this.targetModel.invoiceId).subscribe(invoice => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InvoiceModel.getProps(), this.invoiceRelations, this.isViewOnly),
					invoice,
					InvoiceModel,
		...
		if (this.targetModel.medicalFee && this.targetModel.medicalFee.id && this.medicalFeeMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				MedicalFeeModel,
				this.medicalFeeOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.medicalFee.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getMedicalFeeModelWithId, this.targetModel.medicalFeeId).subscribe(medicalFee => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MedicalFeeModel.getProps(), this.medicalFeeRelations, this.isViewOnly),
					medicalFee,
					MedicalFeeModel,
		...
		if (this.targetModel.retailPharmacyInvoice && this.targetModel.retailPharmacyInvoice.id && this.retailPharmacyInvoiceMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				RetailPharmacyInvoiceModel,
				this.retailPharmacyInvoiceOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.retailPharmacyInvoice.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getRetailPharmacyInvoiceModelWithId, this.targetModel.retailPharmacyInvoiceId).subscribe(retailPharmacyInvoice => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RetailPharmacyInvoiceModel.getProps(), this.retailPharmacyInvoiceRelations, this.isViewOnly),
		...
		if (this.targetModel.serviceNumber && this.targetModel.serviceNumber.id && this.serviceNumberMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				RegistrationModel,
				this.serviceNumberOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.serviceNumber.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getRegistrationModelWithId, this.targetModel.serviceNumberId).subscribe(serviceNumber => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RegistrationModel.getProps(), this.serviceNumberRelations, this.isViewOnly),
					serviceNumber,
					RegistrationModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: InvoiceItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InvoiceItemModel {
		let updatedModel: InvoiceItemModel = new InvoiceItemModel(model);

		if (this.invoiceMultiCrudActive) {
			updatedModel.invoice = event.additionalEntities.invoice ? event.additionalEntities.invoice : undefined;

			if (!updatedModel.invoice && event.payload) {
				event.payload.invoiceId = null;
			}
		} else if (event.payload?.invoiceId !== undefined) {
			event.payload.invoice = null;
			updatedModel.invoice = null;
		}

		if (this.medicalFeeMultiCrudActive) {
			updatedModel.medicalFee = event.additionalEntities.medicalFee ? event.additionalEntities.medicalFee : undefined;

			if (!updatedModel.medicalFee && event.payload) {
				event.payload.medicalFeeId = null;
			}
		} else if (event.payload?.medicalFeeId !== undefined) {
			event.payload.medicalFee = null;
			updatedModel.medicalFee = null;
		}
		...
		if (this.retailPharmacyInvoiceMultiCrudActive) {
			updatedModel.retailPharmacyInvoice = event.additionalEntities.retailPharmacyInvoice ? event.additionalEntities.retailPharmacyInvoice : undefined;

			if (!updatedModel.retailPharmacyInvoice && event.payload) {
				event.payload.retailPharmacyInvoiceId = null;
			}
		} else if (event.payload?.retailPharmacyInvoiceId !== undefined) {
			event.payload.retailPharmacyInvoice = null;
			updatedModel.retailPharmacyInvoice = null;
		}

		if (this.serviceNumberMultiCrudActive) {
			updatedModel.serviceNumber = event.additionalEntities.serviceNumber ? event.additionalEntities.serviceNumber : undefined;

			if (!updatedModel.serviceNumber && event.payload) {
				event.payload.serviceNumberId = null;
			}
		} else if (event.payload?.serviceNumberId !== undefined) {
			event.payload.serviceNumber = null;
			updatedModel.serviceNumber = null;
		}
