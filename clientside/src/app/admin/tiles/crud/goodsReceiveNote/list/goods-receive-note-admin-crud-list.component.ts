...
import * as modelAction from 'src/app/models/goodsReceiveNote/goods_receive_note.model.action';
import {GoodsReceiveNoteModelState} from 'src/app/models/goodsReceiveNote/goods_receive_note.model.state';
import {AuthenticationService} from 'src/app/lib/services/authentication/authentication.service';
import {GrnSourceTypeEnum, grnSourceTypeEnumArray} from 'src/app/enums/grn-source-type.enum';
...
import {
	...
	getGoodsReceiveNoteCollectionCount
...
import {CrudTileMode} from '../goods-receive-note-admin-crud.component';
...
@Component({
	selector: 'cb-goods-receive-note-admin-crud-list',
	templateUrl: './goods-receive-note-admin-crud-list.component.html',
	styleUrls: [
		'./goods-receive-note-admin-crud-list.component.scss',
...
export class AdminGoodsReceiveNoteCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<GoodsReceiveNoteModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...GoodsReceiveNoteModel.searchFields,
	...
	/**
	 * List of Goods Receive Notes that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	goodsReceiveNotes: Observable<GoodsReceiveNoteModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	goodsReceiveNotesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = GoodsReceiveNoteModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = GoodsReceiveNoteModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '916821fa-1a38-4ba1-a049-38a2915e5dcb';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.goodsReceiveNotesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.goodsReceiveNotesCount === 0
		},
		{
	...
	filterQuestions: FilterQuestion[] = [
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Source Document Type to Goods Receive Note here] off begin
		{
			filterType: FilterQuestionType.dropdown,
			config: {
				options: grnSourceTypeEnumArray,
				labelField: 'value',
				valueField: 'key',
				id: 'GoodsReceiveNoteSourceDocumentTypeFilter',
				label: 'Source Document Type',
			},
			name: 'sourceDocumentType',
		},
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Source Document Type to Goods Receive Note here] end
	...
	constructor(
		private readonly store: Store<{ model: GoodsReceiveNoteModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
				modelAction.GoodsReceiveNoteModelActionTypes.INITIALISE_GOODS_RECEIVE_NOTE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
				modelAction.GoodsReceiveNoteModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.GoodsReceiveNoteAction(
						modelAction.GoodsReceiveNoteModelActionTypes.COUNT_GOODS_RECEIVE_NOTES,
						null
					)
				]
			))
		...
		this.goodsReceiveNotes = this.store.select(getGoodsReceiveNoteCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getGoodsReceiveNoteCollectionCount, this.collectionId).subscribe(
				(count) => this.goodsReceiveNotesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('goods-receive-note');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: GoodsReceiveNoteModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.GoodsReceiveNoteModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.GoodsReceiveNoteModelActionTypes.EXPORT_ALL_GOODS_RECEIVE_NOTE;
		} else {
			actionType = modelAction.GoodsReceiveNoteModelActionTypes.EXPORT_GOODS_RECEIVE_NOTE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
				modelAction.GoodsReceiveNoteModelActionTypes.IMPORT_GOODS_RECEIVE_NOTES,
				{
					file: file,
				},
				[new modelAction.GoodsReceiveNoteAction(
			...
			window.open(environment.API_URL + '/api/goodsreceivenote/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: GoodsReceiveNoteModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Goods Receive Note entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Goods Receive Note entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.GoodsReceiveNoteModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.GoodsReceiveNoteModelActionTypes.DELETE_GOODS_RECEIVE_NOTE;
						} else if (usingSelected) {
							actionType = modelAction.GoodsReceiveNoteModelActionTypes.DELETE_ALL_GOODS_RECEIVE_NOTE;
						} else {
							actionType = modelAction.GoodsReceiveNoteModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_GOODS_RECEIVE_NOTE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_GOODS_RECEIVE_NOTE and DELETE_GOODS_RECEIVE_NOTE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<GoodsReceiveNoteModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = GoodsReceiveNoteModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.goodsReceiveNotesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
