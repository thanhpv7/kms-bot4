...
import {AdminGoodsReceiveNoteCrudListComponent} from './goods-receive-note-admin-crud-list.component';
...
import {AdminGoodsReceiveNoteCrudModule} from '../goods-receive-note-admin-crud.module';
import {
 ...
 */
describe('Goods Receive Note Crud List Tile Component', () => {

	let fixture;
	let goodsReceiveNoteCrudListComponent: AdminGoodsReceiveNoteCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<GoodsReceiveNoteModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '916821fa-1a38-4ba1-a049-38a2915e5dcb';
	const searchContent = 'text';

	const searchableFieldCount = GoodsReceiveNoteModel.searchFields.length;

	const routerState: RouterState = {
		url: 'goods-receive-note-admin-crud',
		urls: ['goods-receive-note-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const goodsReceiveNoteDataFactory = new GoodsReceiveNoteDataFactory();

	const defaultDataSize = 30;
	const defaultData = goodsReceiveNoteDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getGoodsReceiveNoteCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getGoodsReceiveNoteCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminGoodsReceiveNoteCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminGoodsReceiveNoteCrudListComponent);
			goodsReceiveNoteCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(goodsReceiveNoteCrudListComponent.goodsReceiveNotesCount).toBe(defaultData.length);
			...
			expect(goodsReceiveNoteCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			goodsReceiveNoteCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = goodsReceiveNoteDataFactory.createAll(filteredDataSize);
			...
			expect(goodsReceiveNoteCrudListComponent.goodsReceiveNotesCount).toBe(filteredDataSize);
			expect(goodsReceiveNoteCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(goodsReceiveNoteCrudListComponent.pageIndex).toBe(0);
			expect(goodsReceiveNoteCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			goodsReceiveNoteCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		goodsReceiveNoteCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		goodsReceiveNoteCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
