...
import {GoodsReceiveNoteModelState, GoodsReceiveNoteModelAudit} from 'src/app/models/goodsReceiveNote/goods_receive_note.model.state';
...
import {
	getGoodsReceiveNoteModelAuditsByEntityId,
...
@Component({
	selector: 'cb-goods-receive-note-admin-crud-edit',
	templateUrl: './goods-receive-note-admin-crud-edit.component.html',
	styleUrls: [
		'./goods-receive-note-admin-crud-edit.component.scss',
...
export class AdminGoodsReceiveNoteCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	goodsReceiveNoteAudits$: Observable<GoodsReceiveNoteModelAudit[]>;
	...
	supplierOppositeRelation = 'goodsReceiveNotes';
	...
	warehouseOppositeRelation = 'goodsReceiveNotes';
	...
	goodsReceiveNoteItemsOppositeRelation = 'goodsReceiveNote';
	...
	purchaseOrdersOppositeRelation = 'goodsReceiveNote';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = GoodsReceiveNoteModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: GoodsReceiveNoteModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountGoodsReceiveNoteModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new GoodsReceiveNoteModel();
		...
		if (this.targetModelId) {
			this.goodsReceiveNoteAudits$ = this.store.select(getGoodsReceiveNoteModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<GoodsReceiveNoteModel> = {
		...
		this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
			modelAction.GoodsReceiveNoteModelActionTypes.FETCH_GOODS_RECEIVE_NOTE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getGoodsReceiveNoteModelWithId, this.targetModelId)
			...
			model = new GoodsReceiveNoteModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
			modelAction.GoodsReceiveNoteModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<GoodsReceiveNoteModel> = {
				...
				queryParams: {
					persistRelations: this.supplierMultiCrudActive || this.warehouseMultiCrudActive || this.goodsReceiveNoteItemsMultiCrudActive || this.purchaseOrdersMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
				modelAction.GoodsReceiveNoteModelActionTypes.CREATE_GOODS_RECEIVE_NOTE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.GoodsReceiveNoteAction(
				modelAction.GoodsReceiveNoteModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: GoodsReceiveNoteModel = new GoodsReceiveNoteModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getGoodsReceiveNoteModelWithId, this.targetModelId).subscribe(model => {
					let differences = new GoodsReceiveNoteModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = GoodsReceiveNoteModel.getProps();
		this.modelRelations = GoodsReceiveNoteModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: GoodsReceiveNoteModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): GoodsReceiveNoteModel {
		let updatedModel: GoodsReceiveNoteModel = new GoodsReceiveNoteModel(model);
