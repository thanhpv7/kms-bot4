...
import {PostOperativeDetailsModelState} from 'src/app/models/postOperativeDetails/post_operative_details.model.state';
...
@Component({
	selector: 'cb-post-operative-details-admin-crud-list',
	templateUrl: './post-operative-details-admin-crud-list.component.html',
	styleUrls: [
		'./post-operative-details-admin-crud-list.component.scss',
...
export class AdminPostOperativeDetailsCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PostOperativeDetailsModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PostOperativeDetailsModel.searchFields,
	...
	/**
	 * List of Post Operative Detailss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	postOperativeDetailss: Observable<PostOperativeDetailsModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	postOperativeDetailssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.postOperativeDetailssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.postOperativeDetailssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PostOperativeDetailsAction(
				modelAction.PostOperativeDetailsModelActionTypes.INITIALISE_POST_OPERATIVE_DETAILS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PostOperativeDetailsAction(
				modelAction.PostOperativeDetailsModelActionTypes.FETCH_POST_OPERATIVE_DETAILS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.postOperativeDetailss = this.store.select(getPostOperativeDetailsCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPostOperativeDetailsCollectionCount, this.collectionId).subscribe(
				(count) => this.postOperativeDetailssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PostOperativeDetailsModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PostOperativeDetailsModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PostOperativeDetailsModelActionTypes.EXPORT_ALL_POST_OPERATIVE_DETAILS;
		} else {
			actionType = modelAction.PostOperativeDetailsModelActionTypes.EXPORT_POST_OPERATIVE_DETAILS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PostOperativeDetailsAction(
				modelAction.PostOperativeDetailsModelActionTypes.IMPORT_POST_OPERATIVE_DETAILSS,
				{
					file: file,
				},
				[new modelAction.PostOperativeDetailsAction(
			...
			window.open(environment.API_URL + '/api/postoperativedetails/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PostOperativeDetailsModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Post Operative Details entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Post Operative Details entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PostOperativeDetailsModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PostOperativeDetailsModelActionTypes.DELETE_POST_OPERATIVE_DETAILS;
						} else if (usingSelected) {
							actionType = modelAction.PostOperativeDetailsModelActionTypes.DELETE_ALL_POST_OPERATIVE_DETAILS;
						} else {
							actionType = modelAction.PostOperativeDetailsModelActionTypes.DELETE_POST_OPERATIVE_DETAILS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PostOperativeDetailsAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_POST_OPERATIVE_DETAILS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_POST_OPERATIVE_DETAILS and DELETE_POST_OPERATIVE_DETAILS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PostOperativeDetailsModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.postOperativeDetailssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
