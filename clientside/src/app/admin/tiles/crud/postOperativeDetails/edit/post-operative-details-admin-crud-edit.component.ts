...
import * as modelAction from 'src/app/models/postOperativeDetails/post_operative_details.model.action';
import {PostOperativeDetailsModelState, PostOperativeDetailsModelAudit} from 'src/app/models/postOperativeDetails/post_operative_details.model.state';
import {PostOperativeDetailsModel} from 'src/app/models/postOperativeDetails/post_operative_details.model';
import {
	getPostOperativeDetailsModelAuditsByEntityId,
...
import {CrudTileMode} from '../post-operative-details-admin-crud.component';
...
@Component({
	selector: 'cb-post-operative-details-admin-crud-edit',
	templateUrl: './post-operative-details-admin-crud-edit.component.html',
	styleUrls: [
		'./post-operative-details-admin-crud-edit.component.scss',
...
export class AdminPostOperativeDetailsCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'e85677ec-90f8-4bcb-b76f-a4a85702e765';
	 ...
	 */
	postOperativeDetailsAudits$: Observable<PostOperativeDetailsModelAudit[]>;
	...
	operatingTheaterMedicalExaminationRecordOppositeRelation = 'postOperativeDetails';
	...
	postOperativeICD10Relations: ModelRelation[] = this.postOperativeICD10MultiCrudActive ? DiagnosesAndProceduresModel.getRelations()
		.map(relation => {
	...
	postOperativeICD9CMRelations: ModelRelation[] = this.postOperativeICD9CMMultiCrudActive ? DiagnosesAndProceduresModel.getRelations()
		.map(relation => {
	...
	recoveryRoomsRelations: ModelRelation[] = this.recoveryRoomsMultiCrudActive ? RoomFacilityModel.getRelations()
		.map(relation => {
	...
	bedFacilitiesOppositeRelation = 'postOperativeDetails';
	 ...
	 */
	modelProperties: ModelProperty[] = PostOperativeDetailsModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PostOperativeDetailsModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PostOperativeDetailsModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PostOperativeDetailsModel;
	...
	constructor(
		private readonly store: Store<{ model: PostOperativeDetailsModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPostOperativeDetailsModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PostOperativeDetailsModel();
		...
		if (this.targetModelId) {
			this.postOperativeDetailsAudits$ = this.store.select(getPostOperativeDetailsModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PostOperativeDetailsModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PostOperativeDetailsAction(
			modelAction.PostOperativeDetailsModelActionTypes.FETCH_POST_OPERATIVE_DETAILS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPostOperativeDetailsModelWithId, this.targetModelId)
			...
			model = new PostOperativeDetailsModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('post-operative-details');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PostOperativeDetailsAction(
			modelAction.PostOperativeDetailsModelActionTypes.FETCH_POST_OPERATIVE_DETAILS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PostOperativeDetailsModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.operatingTheaterMedicalExaminationRecordMultiCrudActive || this.postOperativeICD10MultiCrudActive || this.postOperativeICD9CMMultiCrudActive || this.recoveryRoomsMultiCrudActive || this.bedFacilitiesMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PostOperativeDetailsAction(
						modelAction.PostOperativeDetailsModelActionTypes.COUNT_POST_OPERATIVE_DETAILSS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PostOperativeDetailsAction(
				modelAction.PostOperativeDetailsModelActionTypes.CREATE_POST_OPERATIVE_DETAILS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PostOperativeDetailsAction(
				modelAction.PostOperativeDetailsModelActionTypes.UPDATE_POST_OPERATIVE_DETAILS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PostOperativeDetailsModel = new PostOperativeDetailsModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPostOperativeDetailsModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PostOperativeDetailsModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.postOperativeICD10DisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.postOperativeICD10Relations, this.isViewOnly),
			...
			if (event.entityName === this.postOperativeICD9CMDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.postOperativeICD9CMRelations, this.isViewOnly),
					new DiagnosesAndProceduresModel(),
					DiagnosesAndProceduresModel,
			...
			if (event.entityName === this.recoveryRoomsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RoomFacilityModel.getProps(), this.recoveryRoomsRelations, this.isViewOnly),
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PostOperativeDetailsModel.getProps();
		this.modelRelations = PostOperativeDetailsModel.getRelations();
	...
	initialiseMultiEntityCollections() {
		...
		if (this.postOperativeICD10MultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.postOperativeICD10DisplayName, many: false});
		}

		if (this.postOperativeICD9CMMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.postOperativeICD9CMDisplayName, many: false});
		}

		if (this.recoveryRoomsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.recoveryRoomsDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.postOperativeICD10 && this.targetModel.postOperativeICD10.id && this.postOperativeICD10MultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				DiagnosesAndProceduresModel,
				this.postOperativeICD10OppositeRelation,
				this.collectionId,
				1,
				this.targetModel.postOperativeICD10.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getDiagnosesAndProceduresModelWithId, this.targetModel.postOperativeICD10Id).subscribe(postOperativeICD10 => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.postOperativeICD10Relations, this.isViewOnly),
					postOperativeICD10,
					DiagnosesAndProceduresModel,
		...
		if (this.targetModel.postOperativeICD9CM && this.targetModel.postOperativeICD9CM.id && this.postOperativeICD9CMMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				DiagnosesAndProceduresModel,
				this.postOperativeICD9CMOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.postOperativeICD9CM.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getDiagnosesAndProceduresModelWithId, this.targetModel.postOperativeICD9CMId).subscribe(postOperativeICD9CM => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.postOperativeICD9CMRelations, this.isViewOnly),
					postOperativeICD9CM,
					DiagnosesAndProceduresModel,
		...
		if (this.targetModel.recoveryRooms && this.targetModel.recoveryRooms.id && this.recoveryRoomsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				RoomFacilityModel,
				this.recoveryRoomsOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.recoveryRooms.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getRoomFacilityModelWithId, this.targetModel.recoveryRoomsId).subscribe(recoveryRooms => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RoomFacilityModel.getProps(), this.recoveryRoomsRelations, this.isViewOnly),
					recoveryRooms,
					RoomFacilityModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: PostOperativeDetailsModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PostOperativeDetailsModel {
		let updatedModel: PostOperativeDetailsModel = new PostOperativeDetailsModel(model);
		...
		if (this.postOperativeICD10MultiCrudActive) {
			updatedModel.postOperativeICD10 = event.additionalEntities.postOperativeICD10 ? event.additionalEntities.postOperativeICD10 : undefined;

			if (!updatedModel.postOperativeICD10 && event.payload) {
				event.payload.postOperativeICD10Id = null;
			}
		} else if (event.payload?.postOperativeICD10Id !== undefined) {
			event.payload.postOperativeICD10 = null;
			updatedModel.postOperativeICD10 = null;
		}

		if (this.postOperativeICD9CMMultiCrudActive) {
			updatedModel.postOperativeICD9CM = event.additionalEntities.postOperativeICD9CM ? event.additionalEntities.postOperativeICD9CM : undefined;

			if (!updatedModel.postOperativeICD9CM && event.payload) {
				event.payload.postOperativeICD9CMId = null;
			}
		} else if (event.payload?.postOperativeICD9CMId !== undefined) {
			event.payload.postOperativeICD9CM = null;
			updatedModel.postOperativeICD9CM = null;
		}

		if (this.recoveryRoomsMultiCrudActive) {
			updatedModel.recoveryRooms = event.additionalEntities.recoveryRooms ? event.additionalEntities.recoveryRooms : undefined;

			if (!updatedModel.recoveryRooms && event.payload) {
				event.payload.recoveryRoomsId = null;
			}
		} else if (event.payload?.recoveryRoomsId !== undefined) {
			event.payload.recoveryRooms = null;
			updatedModel.recoveryRooms = null;
		}
