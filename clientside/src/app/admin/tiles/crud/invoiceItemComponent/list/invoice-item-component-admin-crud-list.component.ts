...
import {InvoiceItemComponentModelState} from 'src/app/models/invoiceItemComponent/invoice_item_component.model.state';
...
import {
	getInvoiceItemComponentCollectionModels,
	getInvoiceItemComponentCollectionCount
...
@Component({
	selector: 'cb-invoice-item-component-admin-crud-list',
	templateUrl: './invoice-item-component-admin-crud-list.component.html',
	styleUrls: [
		'./invoice-item-component-admin-crud-list.component.scss',
...
export class AdminInvoiceItemComponentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InvoiceItemComponentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InvoiceItemComponentModel.searchFields,
	...
	/**
	 * List of Invoice Item Components that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	invoiceItemComponents: Observable<InvoiceItemComponentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	invoiceItemComponentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.invoiceItemComponentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.invoiceItemComponentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceItemComponentAction(
				modelAction.InvoiceItemComponentModelActionTypes.INITIALISE_INVOICE_ITEM_COMPONENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceItemComponentAction(
				modelAction.InvoiceItemComponentModelActionTypes.FETCH_INVOICE_ITEM_COMPONENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.invoiceItemComponents = this.store.select(getInvoiceItemComponentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInvoiceItemComponentCollectionCount, this.collectionId).subscribe(
				(count) => this.invoiceItemComponentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InvoiceItemComponentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InvoiceItemComponentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InvoiceItemComponentModelActionTypes.EXPORT_ALL_INVOICE_ITEM_COMPONENT;
		} else {
			actionType = modelAction.InvoiceItemComponentModelActionTypes.EXPORT_INVOICE_ITEM_COMPONENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InvoiceItemComponentAction(
				modelAction.InvoiceItemComponentModelActionTypes.IMPORT_INVOICE_ITEM_COMPONENTS,
				{
					file: file,
				},
				[new modelAction.InvoiceItemComponentAction(
			...
			window.open(environment.API_URL + '/api/invoiceitemcomponent/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InvoiceItemComponentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Invoice Item Component entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Invoice Item Component entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InvoiceItemComponentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InvoiceItemComponentModelActionTypes.DELETE_INVOICE_ITEM_COMPONENT;
						} else if (usingSelected) {
							actionType = modelAction.InvoiceItemComponentModelActionTypes.DELETE_ALL_INVOICE_ITEM_COMPONENT;
						} else {
							actionType = modelAction.InvoiceItemComponentModelActionTypes.DELETE_INVOICE_ITEM_COMPONENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InvoiceItemComponentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVOICE_ITEM_COMPONENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVOICE_ITEM_COMPONENT and DELETE_INVOICE_ITEM_COMPONENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InvoiceItemComponentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.invoiceItemComponentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
