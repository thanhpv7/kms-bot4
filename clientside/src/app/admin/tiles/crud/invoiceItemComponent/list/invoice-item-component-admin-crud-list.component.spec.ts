...
import {AdminInvoiceItemComponentCrudListComponent} from './invoice-item-component-admin-crud-list.component';
...
import {AdminInvoiceItemComponentCrudModule} from '../invoice-item-component-admin-crud.module';
import {
	InvoiceItemComponentModelState,
	initialState as initialInvoiceItemComponentModelState
} from 'src/app/models/invoiceItemComponent/invoice_item_component.model.state';
 ...
 */
describe('Invoice Item Component Crud List Tile Component', () => {

	let fixture;
	let invoiceItemComponentCrudListComponent: AdminInvoiceItemComponentCrudListComponent;

	let store: MockStore<{ model: InvoiceItemComponentModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let invoiceItemComponentModelState: InvoiceItemComponentModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InvoiceItemComponentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b8f78c51-9e55-418c-93ff-169987c8e67b';
	const searchContent = 'text';

	const searchableFieldCount = InvoiceItemComponentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'invoice-item-component-admin-crud',
		urls: ['invoice-item-component-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const invoiceItemComponentDataFactory = new InvoiceItemComponentDataFactory();

	const defaultDataSize = 30;
	const defaultData = invoiceItemComponentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InvoiceItemComponentModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		invoiceItemComponentModelState = initialInvoiceItemComponentModelState;
		store.setState({model: invoiceItemComponentModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInvoiceItemComponentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInvoiceItemComponentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInvoiceItemComponentCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInvoiceItemComponentCrudListComponent);
			invoiceItemComponentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(invoiceItemComponentCrudListComponent.invoiceItemComponentsCount).toBe(defaultData.length);
			...
			expect(invoiceItemComponentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			invoiceItemComponentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = invoiceItemComponentDataFactory.createAll(filteredDataSize);
			...
			expect(invoiceItemComponentCrudListComponent.invoiceItemComponentsCount).toBe(filteredDataSize);
			expect(invoiceItemComponentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(invoiceItemComponentCrudListComponent.pageIndex).toBe(0);
			expect(invoiceItemComponentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			invoiceItemComponentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		invoiceItemComponentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		invoiceItemComponentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
