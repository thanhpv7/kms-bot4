...
import * as modelAction from 'src/app/models/invoiceItemComponent/invoice_item_component.model.action';
import {InvoiceItemComponentModelState, InvoiceItemComponentModelAudit} from 'src/app/models/invoiceItemComponent/invoice_item_component.model.state';
import {InvoiceItemComponentModel} from 'src/app/models/invoiceItemComponent/invoice_item_component.model';
import {
	getInvoiceItemComponentModelAuditsByEntityId,
	getInvoiceItemComponentModelWithId,
	getCountInvoiceItemComponentModels,
...
import {CrudTileMode} from '../invoice-item-component-admin-crud.component';
...
@Component({
	selector: 'cb-invoice-item-component-admin-crud-edit',
	templateUrl: './invoice-item-component-admin-crud-edit.component.html',
	styleUrls: [
		'./invoice-item-component-admin-crud-edit.component.scss',
...
export class AdminInvoiceItemComponentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b8f78c51-9e55-418c-93ff-169987c8e67b';
	 ...
	 */
	invoiceItemComponentAudits$: Observable<InvoiceItemComponentModelAudit[]>;
	...
	invoiceItemMultiCrudActive: boolean = false;
	invoiceItemDisplayName = 'Invoice Item';
	...
	invoiceItemOppositeRelation = 'invoiceItemComponents';
	invoiceItemRelations: ModelRelation[] = this.invoiceItemMultiCrudActive ? InvoiceItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.invoiceItemOppositeRelation) {
	...
	priceComponentOppositeRelation = 'invoiceItemComponents';
	 ...
	 */
	modelProperties: ModelProperty[] = InvoiceItemComponentModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InvoiceItemComponentModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InvoiceItemComponentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'invoiceItem' && this.invoiceItemMultiCrudActive) {
	 ...
	 */
	targetModel: InvoiceItemComponentModel;
	...
	constructor(
		private readonly store: Store<{ model: InvoiceItemComponentModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInvoiceItemComponentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InvoiceItemComponentModel();
		...
		if (this.targetModelId) {
			this.invoiceItemComponentAudits$ = this.store.select(getInvoiceItemComponentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InvoiceItemComponentModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InvoiceItemComponentAction(
			modelAction.InvoiceItemComponentModelActionTypes.FETCH_INVOICE_ITEM_COMPONENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInvoiceItemComponentModelWithId, this.targetModelId)
			...
			model = new InvoiceItemComponentModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('invoice-item-component');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InvoiceItemComponentAction(
			modelAction.InvoiceItemComponentModelActionTypes.FETCH_INVOICE_ITEM_COMPONENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InvoiceItemComponentModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.invoiceItemMultiCrudActive || this.priceComponentMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InvoiceItemComponentAction(
						modelAction.InvoiceItemComponentModelActionTypes.COUNT_INVOICE_ITEM_COMPONENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InvoiceItemComponentAction(
				modelAction.InvoiceItemComponentModelActionTypes.CREATE_INVOICE_ITEM_COMPONENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InvoiceItemComponentAction(
				modelAction.InvoiceItemComponentModelActionTypes.UPDATE_INVOICE_ITEM_COMPONENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InvoiceItemComponentModel = new InvoiceItemComponentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInvoiceItemComponentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InvoiceItemComponentModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.invoiceItemDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InvoiceItemModel.getProps(), this.invoiceItemRelations, this.isViewOnly),
					new InvoiceItemModel(),
					InvoiceItemModel,
					this.invoiceItemDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.invoiceItemRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InvoiceItemComponentModel.getProps();
		this.modelRelations = InvoiceItemComponentModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'invoiceItem';
		}).hideElement = this.invoiceItemMultiCrudActive || this.modelRelations.find(rel => rel.id === 'invoiceItem').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.invoiceItemMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.invoiceItemDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.invoiceItem && this.targetModel.invoiceItem.id && this.invoiceItemMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InvoiceItemModel,
				this.invoiceItemOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.invoiceItem.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getInvoiceItemModelWithId, this.targetModel.invoiceItemId).subscribe(invoiceItem => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InvoiceItemModel.getProps(), this.invoiceItemRelations, this.isViewOnly),
					invoiceItem,
					InvoiceItemModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: InvoiceItemComponentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InvoiceItemComponentModel {
		let updatedModel: InvoiceItemComponentModel = new InvoiceItemComponentModel(model);

		if (this.invoiceItemMultiCrudActive) {
			updatedModel.invoiceItem = event.additionalEntities.invoiceItem ? event.additionalEntities.invoiceItem : undefined;

			if (!updatedModel.invoiceItem && event.payload) {
				event.payload.invoiceItemId = null;
			}
		} else if (event.payload?.invoiceItemId !== undefined) {
			event.payload.invoiceItem = null;
			updatedModel.invoiceItem = null;
		}
