...
import {NurseVitalSignMeasurementModelState, NurseVitalSignMeasurementModelAudit} from 'src/app/models/nurseVitalSignMeasurement/nurse_vital_sign_measurement.model.state';
import {NurseVitalSignMeasurementModel} from 'src/app/models/nurseVitalSignMeasurement/nurse_vital_sign_measurement.model';
import {
	getNurseVitalSignMeasurementModelAuditsByEntityId,
	getNurseVitalSignMeasurementModelWithId,
	getCountNurseVitalSignMeasurementModels,
...
@Component({
	selector: 'cb-nurse-vital-sign-measurement-admin-crud-edit',
	templateUrl: './nurse-vital-sign-measurement-admin-crud-edit.component.html',
	styleUrls: [
		'./nurse-vital-sign-measurement-admin-crud-edit.component.scss',
...
export class AdminNurseVitalSignMeasurementCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nurseVitalSignMeasurementAudits$: Observable<NurseVitalSignMeasurementModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'nurseVitalSignMeasurement';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NurseVitalSignMeasurementModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NurseVitalSignMeasurementModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNurseVitalSignMeasurementModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NurseVitalSignMeasurementModel();
		...
		if (this.targetModelId) {
			this.nurseVitalSignMeasurementAudits$ = this.store.select(getNurseVitalSignMeasurementModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NurseVitalSignMeasurementModel> = {
		...
		this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
			modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_NURSE_VITAL_SIGN_MEASUREMENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNurseVitalSignMeasurementModelWithId, this.targetModelId)
			...
			model = new NurseVitalSignMeasurementModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
			modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_NURSE_VITAL_SIGN_MEASUREMENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
				modelAction.NurseVitalSignMeasurementModelActionTypes.CREATE_NURSE_VITAL_SIGN_MEASUREMENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
				modelAction.NurseVitalSignMeasurementModelActionTypes.UPDATE_NURSE_VITAL_SIGN_MEASUREMENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NurseVitalSignMeasurementModel = new NurseVitalSignMeasurementModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNurseVitalSignMeasurementModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NurseVitalSignMeasurementModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NurseVitalSignMeasurementModel.getProps();
		this.modelRelations = NurseVitalSignMeasurementModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NurseVitalSignMeasurementModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NurseVitalSignMeasurementModel {
		let updatedModel: NurseVitalSignMeasurementModel = new NurseVitalSignMeasurementModel(model);
