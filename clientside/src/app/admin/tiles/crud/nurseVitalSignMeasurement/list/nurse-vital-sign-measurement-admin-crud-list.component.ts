...
import * as modelAction from 'src/app/models/nurseVitalSignMeasurement/nurse_vital_sign_measurement.model.action';
import {NurseVitalSignMeasurementModelState} from 'src/app/models/nurseVitalSignMeasurement/nurse_vital_sign_measurement.model.state';
...
import {CrudTileMode} from '../nurse-vital-sign-measurement-admin-crud.component';
...
@Component({
	selector: 'cb-nurse-vital-sign-measurement-admin-crud-list',
	templateUrl: './nurse-vital-sign-measurement-admin-crud-list.component.html',
	styleUrls: [
		'./nurse-vital-sign-measurement-admin-crud-list.component.scss',
...
export class AdminNurseVitalSignMeasurementCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NurseVitalSignMeasurementModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NurseVitalSignMeasurementModel.searchFields,
	...
	/**
	 * List of Nurse Vital Sign Measurements that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	nurseVitalSignMeasurements: Observable<NurseVitalSignMeasurementModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	nurseVitalSignMeasurementsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = NurseVitalSignMeasurementModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NurseVitalSignMeasurementModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '3d5a5bcb-bfdd-4e5d-87be-980cc43041b0';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nurseVitalSignMeasurementsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nurseVitalSignMeasurementsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NurseVitalSignMeasurementModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
				modelAction.NurseVitalSignMeasurementModelActionTypes.INITIALISE_NURSE_VITAL_SIGN_MEASUREMENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
				modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_NURSE_VITAL_SIGN_MEASUREMENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NurseVitalSignMeasurementAction(
						modelAction.NurseVitalSignMeasurementModelActionTypes.COUNT_NURSE_VITAL_SIGN_MEASUREMENTS,
						null
					)
				]
			))
		...
		this.nurseVitalSignMeasurements = this.store.select(getNurseVitalSignMeasurementCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNurseVitalSignMeasurementCollectionCount, this.collectionId).subscribe(
				(count) => this.nurseVitalSignMeasurementsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('nurse-vital-sign-measurement');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NurseVitalSignMeasurementModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NurseVitalSignMeasurementModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NurseVitalSignMeasurementModelActionTypes.EXPORT_ALL_NURSE_VITAL_SIGN_MEASUREMENT;
		} else {
			actionType = modelAction.NurseVitalSignMeasurementModelActionTypes.EXPORT_NURSE_VITAL_SIGN_MEASUREMENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
				modelAction.NurseVitalSignMeasurementModelActionTypes.IMPORT_NURSE_VITAL_SIGN_MEASUREMENTS,
				{
					file: file,
				},
				[new modelAction.NurseVitalSignMeasurementAction(
			...
			window.open(environment.API_URL + '/api/nursevitalsignmeasurement/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NurseVitalSignMeasurementModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Nurse Vital Sign Measurement entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Nurse Vital Sign Measurement entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NurseVitalSignMeasurementModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NurseVitalSignMeasurementModelActionTypes.DELETE_NURSE_VITAL_SIGN_MEASUREMENT;
						} else if (usingSelected) {
							actionType = modelAction.NurseVitalSignMeasurementModelActionTypes.DELETE_ALL_NURSE_VITAL_SIGN_MEASUREMENT;
						} else {
							actionType = modelAction.NurseVitalSignMeasurementModelActionTypes.DELETE_NURSE_VITAL_SIGN_MEASUREMENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NurseVitalSignMeasurementAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NURSE_VITAL_SIGN_MEASUREMENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NURSE_VITAL_SIGN_MEASUREMENT and DELETE_NURSE_VITAL_SIGN_MEASUREMENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NurseVitalSignMeasurementModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NurseVitalSignMeasurementModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nurseVitalSignMeasurementsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
