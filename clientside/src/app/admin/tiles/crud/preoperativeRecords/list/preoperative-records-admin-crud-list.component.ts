...
import {PreoperativeRecordsModelState} from 'src/app/models/preoperativeRecords/preoperative_records.model.state';
...
import {
	...
	getPreoperativeRecordsCollectionCount
...
import {CrudTileMode} from '../preoperative-records-admin-crud.component';
...
@Component({
	selector: 'cb-preoperative-records-admin-crud-list',
	templateUrl: './preoperative-records-admin-crud-list.component.html',
	styleUrls: [
		'./preoperative-records-admin-crud-list.component.scss',
...
export class AdminPreoperativeRecordsCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PreoperativeRecordsModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PreoperativeRecordsModel.searchFields,
	...
	/**
	 * List of Preoperative Recordss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	preoperativeRecordss: Observable<PreoperativeRecordsModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	preoperativeRecordssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.preoperativeRecordssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.preoperativeRecordssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PreoperativeRecordsAction(
				modelAction.PreoperativeRecordsModelActionTypes.INITIALISE_PREOPERATIVE_RECORDS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PreoperativeRecordsAction(
				modelAction.PreoperativeRecordsModelActionTypes.FETCH_PREOPERATIVE_RECORDS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.preoperativeRecordss = this.store.select(getPreoperativeRecordsCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPreoperativeRecordsCollectionCount, this.collectionId).subscribe(
				(count) => this.preoperativeRecordssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PreoperativeRecordsModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PreoperativeRecordsModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PreoperativeRecordsModelActionTypes.EXPORT_ALL_PREOPERATIVE_RECORDS;
		} else {
			actionType = modelAction.PreoperativeRecordsModelActionTypes.EXPORT_PREOPERATIVE_RECORDS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PreoperativeRecordsAction(
				modelAction.PreoperativeRecordsModelActionTypes.IMPORT_PREOPERATIVE_RECORDSS,
				{
					file: file,
				},
				[new modelAction.PreoperativeRecordsAction(
			...
			window.open(environment.API_URL + '/api/preoperativerecords/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PreoperativeRecordsModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Preoperative Records entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Preoperative Records entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PreoperativeRecordsModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PreoperativeRecordsModelActionTypes.DELETE_PREOPERATIVE_RECORDS;
						} else if (usingSelected) {
							actionType = modelAction.PreoperativeRecordsModelActionTypes.DELETE_ALL_PREOPERATIVE_RECORDS;
						} else {
							actionType = modelAction.PreoperativeRecordsModelActionTypes.DELETE_PREOPERATIVE_RECORDS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PreoperativeRecordsAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PREOPERATIVE_RECORDS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PREOPERATIVE_RECORDS and DELETE_PREOPERATIVE_RECORDS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PreoperativeRecordsModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.preoperativeRecordssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
