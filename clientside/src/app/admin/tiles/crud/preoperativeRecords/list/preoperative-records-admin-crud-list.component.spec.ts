...
import {AdminPreoperativeRecordsCrudListComponent} from './preoperative-records-admin-crud-list.component';
...
import {AdminPreoperativeRecordsCrudModule} from '../preoperative-records-admin-crud.module';
import {
 ...
 */
describe('Preoperative Records Crud List Tile Component', () => {

	let fixture;
	let preoperativeRecordsCrudListComponent: AdminPreoperativeRecordsCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PreoperativeRecordsModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b791353d-6836-47b9-9ef5-9d4417e722c8';
	const searchContent = 'text';

	const searchableFieldCount = PreoperativeRecordsModel.searchFields.length;

	const routerState: RouterState = {
		url: 'preoperative-records-admin-crud',
		urls: ['preoperative-records-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const preoperativeRecordsDataFactory = new PreoperativeRecordsDataFactory();

	const defaultDataSize = 30;
	const defaultData = preoperativeRecordsDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPreoperativeRecordsCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPreoperativeRecordsCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPreoperativeRecordsCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPreoperativeRecordsCrudListComponent);
			preoperativeRecordsCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(preoperativeRecordsCrudListComponent.preoperativeRecordssCount).toBe(defaultData.length);
			...
			expect(preoperativeRecordsCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			preoperativeRecordsCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = preoperativeRecordsDataFactory.createAll(filteredDataSize);
			...
			expect(preoperativeRecordsCrudListComponent.preoperativeRecordssCount).toBe(filteredDataSize);
			expect(preoperativeRecordsCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(preoperativeRecordsCrudListComponent.pageIndex).toBe(0);
			expect(preoperativeRecordsCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			preoperativeRecordsCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		preoperativeRecordsCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		preoperativeRecordsCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
