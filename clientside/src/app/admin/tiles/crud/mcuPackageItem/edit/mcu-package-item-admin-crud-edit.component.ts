...
import * as modelAction from 'src/app/models/mcuPackageItem/mcu_package_item.model.action';
import {McuPackageItemModelState, McuPackageItemModelAudit} from 'src/app/models/mcuPackageItem/mcu_package_item.model.state';
import {McuPackageItemModel} from 'src/app/models/mcuPackageItem/mcu_package_item.model';
import {
	getMcuPackageItemModelAuditsByEntityId,
...
import * as mcuPackageModelAction from 'src/app/models/mcuPackage/mcu_package.model.action';
import {
	getMcuPackageCollectionModels, getMcuPackageModelWithId,
...
import {CrudTileMode} from '../mcu-package-item-admin-crud.component';
...
@Component({
	selector: 'cb-mcu-package-item-admin-crud-edit',
	templateUrl: './mcu-package-item-admin-crud-edit.component.html',
	styleUrls: [
		'./mcu-package-item-admin-crud-edit.component.scss',
...
export class AdminMcuPackageItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'de35bc07-e220-4761-875e-ac89068d51f1';
	 ...
	 */
	mcuPackageItemAudits$: Observable<McuPackageItemModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between MCU Package and MCU Package Items here] off begin
	mcuPackageMultiCrudActive: boolean = false;
	mcuPackageDisplayName = 'MCU Package';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between MCU Package and MCU Package Items here] end
	mcuPackageOppositeRelation = 'mcuPackageItems';
	mcuPackageRelations: ModelRelation[] = this.mcuPackageMultiCrudActive ? McuPackageModel.getRelations()
		.map(relation => {
			if (relation.id === this.mcuPackageOppositeRelation) {
	...
	serviceOppositeRelation = 'mcuPackageItems';
	...
	staffOppositeRelation = 'mcuPackageItems';
	...
	examinationItemsOppositeRelation = 'examinationItemOfMCUItem';
	 ...
	 */
	modelProperties: ModelProperty[] = McuPackageItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = McuPackageItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = McuPackageItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'mcuPackage' && this.mcuPackageMultiCrudActive) {
					exp.fields = McuPackageModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: McuPackageItemModel;
	...
	constructor(
		private readonly store: Store<{ model: McuPackageItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMcuPackageItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new McuPackageItemModel();
		...
		if (this.targetModelId) {
			this.mcuPackageItemAudits$ = this.store.select(getMcuPackageItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<McuPackageItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.McuPackageItemAction(
			modelAction.McuPackageItemModelActionTypes.FETCH_MCU_PACKAGE_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMcuPackageItemModelWithId, this.targetModelId)
			...
			model = new McuPackageItemModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('mcu-package-item');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.McuPackageItemAction(
			modelAction.McuPackageItemModelActionTypes.FETCH_MCU_PACKAGE_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<McuPackageItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.mcuPackageMultiCrudActive || this.serviceMultiCrudActive || this.staffMultiCrudActive || this.examinationItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.McuPackageItemAction(
						modelAction.McuPackageItemModelActionTypes.COUNT_MCU_PACKAGE_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.McuPackageItemAction(
				modelAction.McuPackageItemModelActionTypes.CREATE_MCU_PACKAGE_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.McuPackageItemAction(
				modelAction.McuPackageItemModelActionTypes.UPDATE_MCU_PACKAGE_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: McuPackageItemModel = new McuPackageItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMcuPackageItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new McuPackageItemModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.mcuPackageDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(McuPackageModel.getProps(), this.mcuPackageRelations, this.isViewOnly),
					new McuPackageModel(),
					McuPackageModel,
					this.mcuPackageDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.mcuPackageRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = McuPackageItemModel.getProps();
		this.modelRelations = McuPackageItemModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'mcuPackage';
		}).hideElement = this.mcuPackageMultiCrudActive || this.modelRelations.find(rel => rel.id === 'mcuPackage').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.mcuPackageMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.mcuPackageDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.mcuPackage && this.targetModel.mcuPackage.id && this.mcuPackageMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				McuPackageModel,
				this.mcuPackageOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.mcuPackage.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getMcuPackageModelWithId, this.targetModel.mcuPackageId).subscribe(mcuPackage => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(McuPackageModel.getProps(), this.mcuPackageRelations, this.isViewOnly),
					mcuPackage,
					McuPackageModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: McuPackageItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): McuPackageItemModel {
		let updatedModel: McuPackageItemModel = new McuPackageItemModel(model);

		if (this.mcuPackageMultiCrudActive) {
			updatedModel.mcuPackage = event.additionalEntities.mcuPackage ? event.additionalEntities.mcuPackage : undefined;

			if (!updatedModel.mcuPackage && event.payload) {
				event.payload.mcuPackageId = null;
			}
		} else if (event.payload?.mcuPackageId !== undefined) {
			event.payload.mcuPackage = null;
			updatedModel.mcuPackage = null;
		}
