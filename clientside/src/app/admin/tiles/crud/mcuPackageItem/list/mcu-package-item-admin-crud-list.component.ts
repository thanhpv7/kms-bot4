...
import {McuPackageItemModelState} from 'src/app/models/mcuPackageItem/mcu_package_item.model.state';
...
import {
	...
	getMcuPackageItemCollectionCount
...
@Component({
	selector: 'cb-mcu-package-item-admin-crud-list',
	templateUrl: './mcu-package-item-admin-crud-list.component.html',
	styleUrls: [
		'./mcu-package-item-admin-crud-list.component.scss',
...
export class AdminMcuPackageItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<McuPackageItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...McuPackageItemModel.searchFields,
	...
	/**
	 * List of MCU Package Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	mcuPackageItems: Observable<McuPackageItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	mcuPackageItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.mcuPackageItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.mcuPackageItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.McuPackageItemAction(
				modelAction.McuPackageItemModelActionTypes.INITIALISE_MCU_PACKAGE_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.McuPackageItemAction(
				modelAction.McuPackageItemModelActionTypes.FETCH_MCU_PACKAGE_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.mcuPackageItems = this.store.select(getMcuPackageItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMcuPackageItemCollectionCount, this.collectionId).subscribe(
				(count) => this.mcuPackageItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: McuPackageItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.McuPackageItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.McuPackageItemModelActionTypes.EXPORT_ALL_MCU_PACKAGE_ITEM;
		} else {
			actionType = modelAction.McuPackageItemModelActionTypes.EXPORT_MCU_PACKAGE_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.McuPackageItemAction(
				modelAction.McuPackageItemModelActionTypes.IMPORT_MCU_PACKAGE_ITEMS,
				{
					file: file,
				},
				[new modelAction.McuPackageItemAction(
			...
			window.open(environment.API_URL + '/api/mcupackageitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: McuPackageItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected MCU Package Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected MCU Package Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.McuPackageItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.McuPackageItemModelActionTypes.DELETE_MCU_PACKAGE_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.McuPackageItemModelActionTypes.DELETE_ALL_MCU_PACKAGE_ITEM;
						} else {
							actionType = modelAction.McuPackageItemModelActionTypes.DELETE_MCU_PACKAGE_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.McuPackageItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MCU_PACKAGE_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MCU_PACKAGE_ITEM and DELETE_MCU_PACKAGE_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = McuPackageItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.mcuPackageItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
