...
import {AdminMcuPackageItemCrudListComponent} from './mcu-package-item-admin-crud-list.component';
...
import {AdminMcuPackageItemCrudModule} from '../mcu-package-item-admin-crud.module';
import {
 ...
 */
describe('MCU Package Item Crud List Tile Component', () => {

	let fixture;
	let mcuPackageItemCrudListComponent: AdminMcuPackageItemCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<McuPackageItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'de35bc07-e220-4761-875e-ac89068d51f1';
	const searchContent = 'text';

	const searchableFieldCount = McuPackageItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'mcu-package-item-admin-crud',
		urls: ['mcu-package-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const mcuPackageItemDataFactory = new McuPackageItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = mcuPackageItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getMcuPackageItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMcuPackageItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminMcuPackageItemCrudListComponent);
			mcuPackageItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(mcuPackageItemCrudListComponent.mcuPackageItemsCount).toBe(defaultData.length);
			...
			expect(mcuPackageItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			mcuPackageItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = mcuPackageItemDataFactory.createAll(filteredDataSize);
			...
			expect(mcuPackageItemCrudListComponent.mcuPackageItemsCount).toBe(filteredDataSize);
			expect(mcuPackageItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(mcuPackageItemCrudListComponent.pageIndex).toBe(0);
			expect(mcuPackageItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			mcuPackageItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		mcuPackageItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		mcuPackageItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
