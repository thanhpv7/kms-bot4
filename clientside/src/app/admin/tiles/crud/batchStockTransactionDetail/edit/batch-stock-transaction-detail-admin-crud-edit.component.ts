...
import {BatchStockTransactionDetailModelState, BatchStockTransactionDetailModelAudit} from 'src/app/models/batchStockTransactionDetail/batch_stock_transaction_detail.model.state';
...
import {
	getBatchStockTransactionDetailModelAuditsByEntityId,
	getBatchStockTransactionDetailModelWithId,
	getCountBatchStockTransactionDetailModels,
...
@Component({
	selector: 'cb-batch-stock-transaction-detail-admin-crud-edit',
	templateUrl: './batch-stock-transaction-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./batch-stock-transaction-detail-admin-crud-edit.component.scss',
...
export class AdminBatchStockTransactionDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	batchStockTransactionDetailAudits$: Observable<BatchStockTransactionDetailModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'batchStockTransactionDetails';
	...
	warehouseOppositeRelation = 'batchStockTransactionDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BatchStockTransactionDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BatchStockTransactionDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBatchStockTransactionDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BatchStockTransactionDetailModel();
		...
		if (this.targetModelId) {
			this.batchStockTransactionDetailAudits$ = this.store.select(getBatchStockTransactionDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BatchStockTransactionDetailModel> = {
		...
		this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
			modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_BATCH_STOCK_TRANSACTION_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBatchStockTransactionDetailModelWithId, this.targetModelId)
			...
			model = new BatchStockTransactionDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
			modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_BATCH_STOCK_TRANSACTION_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BatchStockTransactionDetailModel> = {
				...
				queryParams: {
					persistRelations: this.stockCatalogueMultiCrudActive || this.warehouseMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
				modelAction.BatchStockTransactionDetailModelActionTypes.CREATE_BATCH_STOCK_TRANSACTION_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
				modelAction.BatchStockTransactionDetailModelActionTypes.UPDATE_BATCH_STOCK_TRANSACTION_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BatchStockTransactionDetailModel = new BatchStockTransactionDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBatchStockTransactionDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BatchStockTransactionDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BatchStockTransactionDetailModel.getProps();
		this.modelRelations = BatchStockTransactionDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BatchStockTransactionDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BatchStockTransactionDetailModel {
		let updatedModel: BatchStockTransactionDetailModel = new BatchStockTransactionDetailModel(model);
