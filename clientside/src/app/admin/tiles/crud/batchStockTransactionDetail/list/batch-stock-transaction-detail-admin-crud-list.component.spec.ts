...
import {AdminBatchStockTransactionDetailCrudListComponent} from './batch-stock-transaction-detail-admin-crud-list.component';
...
import {AdminBatchStockTransactionDetailCrudModule} from '../batch-stock-transaction-detail-admin-crud.module';
import {
	BatchStockTransactionDetailModelState,
	initialState as initialBatchStockTransactionDetailModelState
} from 'src/app/models/batchStockTransactionDetail/batch_stock_transaction_detail.model.state';
 ...
 */
describe('Batch Stock Transaction Detail Crud List Tile Component', () => {

	let fixture;
	let batchStockTransactionDetailCrudListComponent: AdminBatchStockTransactionDetailCrudListComponent;

	let store: MockStore<{ model: BatchStockTransactionDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let batchStockTransactionDetailModelState: BatchStockTransactionDetailModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BatchStockTransactionDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c7f81d33-2b4c-4768-8cc3-ed60cb86389d';
	const searchContent = 'text';

	const searchableFieldCount = BatchStockTransactionDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'batch-stock-transaction-detail-admin-crud',
		urls: ['batch-stock-transaction-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const batchStockTransactionDetailDataFactory = new BatchStockTransactionDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = batchStockTransactionDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BatchStockTransactionDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		batchStockTransactionDetailModelState = initialBatchStockTransactionDetailModelState;
		store.setState({model: batchStockTransactionDetailModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBatchStockTransactionDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBatchStockTransactionDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBatchStockTransactionDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBatchStockTransactionDetailCrudListComponent);
			batchStockTransactionDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(batchStockTransactionDetailCrudListComponent.batchStockTransactionDetailsCount).toBe(defaultData.length);
			...
			expect(batchStockTransactionDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			batchStockTransactionDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = batchStockTransactionDetailDataFactory.createAll(filteredDataSize);
			...
			expect(batchStockTransactionDetailCrudListComponent.batchStockTransactionDetailsCount).toBe(filteredDataSize);
			expect(batchStockTransactionDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(batchStockTransactionDetailCrudListComponent.pageIndex).toBe(0);
			expect(batchStockTransactionDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			batchStockTransactionDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		batchStockTransactionDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		batchStockTransactionDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
