...
import * as modelAction from 'src/app/models/batchStockTransactionDetail/batch_stock_transaction_detail.model.action';
import {BatchStockTransactionDetailModelState} from 'src/app/models/batchStockTransactionDetail/batch_stock_transaction_detail.model.state';
...
import {
	getBatchStockTransactionDetailCollectionModels,
	getBatchStockTransactionDetailCollectionCount
...
import {CrudTileMode} from '../batch-stock-transaction-detail-admin-crud.component';
...
@Component({
	selector: 'cb-batch-stock-transaction-detail-admin-crud-list',
	templateUrl: './batch-stock-transaction-detail-admin-crud-list.component.html',
	styleUrls: [
		'./batch-stock-transaction-detail-admin-crud-list.component.scss',
...
export class AdminBatchStockTransactionDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BatchStockTransactionDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BatchStockTransactionDetailModel.searchFields,
	...
	/**
	 * List of Batch Stock Transaction Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	batchStockTransactionDetails: Observable<BatchStockTransactionDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	batchStockTransactionDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BatchStockTransactionDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BatchStockTransactionDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'c7f81d33-2b4c-4768-8cc3-ed60cb86389d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.batchStockTransactionDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.batchStockTransactionDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BatchStockTransactionDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
				modelAction.BatchStockTransactionDetailModelActionTypes.INITIALISE_BATCH_STOCK_TRANSACTION_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
				modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_BATCH_STOCK_TRANSACTION_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BatchStockTransactionDetailAction(
						modelAction.BatchStockTransactionDetailModelActionTypes.COUNT_BATCH_STOCK_TRANSACTION_DETAILS,
						null
					)
				]
			))
		...
		this.batchStockTransactionDetails = this.store.select(getBatchStockTransactionDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBatchStockTransactionDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.batchStockTransactionDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('batch-stock-transaction-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BatchStockTransactionDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BatchStockTransactionDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BatchStockTransactionDetailModelActionTypes.EXPORT_ALL_BATCH_STOCK_TRANSACTION_DETAIL;
		} else {
			actionType = modelAction.BatchStockTransactionDetailModelActionTypes.EXPORT_BATCH_STOCK_TRANSACTION_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
				modelAction.BatchStockTransactionDetailModelActionTypes.IMPORT_BATCH_STOCK_TRANSACTION_DETAILS,
				{
					file: file,
				},
				[new modelAction.BatchStockTransactionDetailAction(
			...
			window.open(environment.API_URL + '/api/batchstocktransactiondetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BatchStockTransactionDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Batch Stock Transaction Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Batch Stock Transaction Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BatchStockTransactionDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BatchStockTransactionDetailModelActionTypes.DELETE_BATCH_STOCK_TRANSACTION_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.BatchStockTransactionDetailModelActionTypes.DELETE_ALL_BATCH_STOCK_TRANSACTION_DETAIL;
						} else {
							actionType = modelAction.BatchStockTransactionDetailModelActionTypes.DELETE_BATCH_STOCK_TRANSACTION_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BatchStockTransactionDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BATCH_STOCK_TRANSACTION_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BATCH_STOCK_TRANSACTION_DETAIL and DELETE_BATCH_STOCK_TRANSACTION_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BatchStockTransactionDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BatchStockTransactionDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.batchStockTransactionDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
