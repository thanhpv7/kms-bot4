...
import {AdminBpjsSEPCrudListComponent} from './bpjs-sep-admin-crud-list.component';
...
import {AdminBpjsSEPCrudModule} from '../bpjs-sep-admin-crud.module';
import {
	BpjsSEPModelState,
 ...
 */
describe('BPJS SEP Crud List Tile Component', () => {

	let fixture;
	let bpjsSEPCrudListComponent: AdminBpjsSEPCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsSEPModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'bb26f844-01dd-4ddd-823e-9dacdd0d4791';
	const searchContent = 'text';

	const searchableFieldCount = BpjsSEPModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-sep-admin-crud',
		urls: ['bpjs-sep-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsSEPDataFactory = new BpjsSEPDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsSEPDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsSEPCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsSEPCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsSEPCrudListComponent);
			bpjsSEPCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsSEPCrudListComponent.bpjsSEPsCount).toBe(defaultData.length);
			...
			expect(bpjsSEPCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsSEPCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsSEPDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsSEPCrudListComponent.bpjsSEPsCount).toBe(filteredDataSize);
			expect(bpjsSEPCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsSEPCrudListComponent.pageIndex).toBe(0);
			expect(bpjsSEPCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsSEPCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsSEPCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsSEPCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
