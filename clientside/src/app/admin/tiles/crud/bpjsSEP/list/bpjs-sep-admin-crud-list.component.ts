...
import * as modelAction from 'src/app/models/bpjsSEP/bpjs_sep.model.action';
import {BpjsSEPModelState} from 'src/app/models/bpjsSEP/bpjs_sep.model.state';
...
import {CrudTileMode} from '../bpjs-sep-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-sep-admin-crud-list',
	templateUrl: './bpjs-sep-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-sep-admin-crud-list.component.scss',
...
export class AdminBpjsSEPCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsSEPModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsSEPModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsSEPModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'bb26f844-01dd-4ddd-823e-9dacdd0d4791';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsSEPsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsSEPsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsSEPModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPAction(
				modelAction.BpjsSEPModelActionTypes.INITIALISE_BPJS_SEP_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPAction(
				modelAction.BpjsSEPModelActionTypes.FETCH_BPJS_SEP_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsSEPAction(
						modelAction.BpjsSEPModelActionTypes.COUNT_BPJS_SEPS,
						null
					)
				]
			))
		...
		this.bpjsSEPs = this.store.select(getBpjsSEPCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsSEPCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsSEPsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsSEPModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsSEPModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsSEPModelActionTypes.EXPORT_ALL_BPJS_SEP;
		} else {
			actionType = modelAction.BpjsSEPModelActionTypes.EXPORT_BPJS_SEP_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsSEPAction(
				modelAction.BpjsSEPModelActionTypes.IMPORT_BPJS_SEPS,
				{
					file: file,
				},
				[new modelAction.BpjsSEPAction(
			...
			window.open(environment.API_URL + '/api/bpjssep/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsSEPModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsSEPModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsSEPModelActionTypes.DELETE_BPJS_SEP;
						} else if (usingSelected) {
							actionType = modelAction.BpjsSEPModelActionTypes.DELETE_ALL_BPJS_SEP;
						} else {
							actionType = modelAction.BpjsSEPModelActionTypes.DELETE_BPJS_SEP_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsSEPAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SEP when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SEP and DELETE_BPJS_SEP_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsSEPModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsSEPModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsSEPsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
