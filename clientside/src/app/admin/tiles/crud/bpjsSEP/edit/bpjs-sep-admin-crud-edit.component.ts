...
import {BpjsSEPModelState, BpjsSEPModelAudit} from 'src/app/models/bpjsSEP/bpjs_sep.model.state';
import {BpjsSEPModel} from 'src/app/models/bpjsSEP/bpjs_sep.model';
import {
	getBpjsSEPModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-sep-admin-crud-edit',
	templateUrl: './bpjs-sep-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-sep-admin-crud-edit.component.scss',
...
export class AdminBpjsSEPCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsSEPAudits$: Observable<BpjsSEPModelAudit[]>;
	...
	registrationOppositeRelation = 'bpjsSEP';
	...
	patientPaymentBPJSOppositeRelation = 'bpjsSEPs';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsSEPModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsSEPModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsSEPModel();
		...
		if (this.targetModelId) {
			this.bpjsSEPAudits$ = this.store.select(getBpjsSEPModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsSEPModel> = {
		...
		this.store.dispatch(new modelAction.BpjsSEPAction(
			modelAction.BpjsSEPModelActionTypes.FETCH_BPJS_SEP,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsSEPAction(
			modelAction.BpjsSEPModelActionTypes.FETCH_BPJS_SEP_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsSEPModel> = {
				...
				queryParams: {
					persistRelations: this.registrationMultiCrudActive || this.patientPaymentBPJSMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsSEPAction(
				modelAction.BpjsSEPModelActionTypes.CREATE_BPJS_SEP,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsSEPAction(
				modelAction.BpjsSEPModelActionTypes.UPDATE_BPJS_SEP,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsSEPModel = new BpjsSEPModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsSEPModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsSEPModel.getProps();
		this.modelRelations = BpjsSEPModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsSEPModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsSEPModel {
		let updatedModel: BpjsSEPModel = new BpjsSEPModel(model);
