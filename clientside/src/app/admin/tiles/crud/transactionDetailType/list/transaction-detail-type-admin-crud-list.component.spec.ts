...
import {AdminTransactionDetailTypeCrudListComponent} from './transaction-detail-type-admin-crud-list.component';
...
import {AdminTransactionDetailTypeCrudModule} from '../transaction-detail-type-admin-crud.module';
import {
 ...
 */
describe('Transaction Detail Type Crud List Tile Component', () => {

	let fixture;
	let transactionDetailTypeCrudListComponent: AdminTransactionDetailTypeCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<TransactionDetailTypeModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '2b16b403-7972-4d22-bfb3-85c1bd863508';
	const searchContent = 'text';

	const searchableFieldCount = TransactionDetailTypeModel.searchFields.length;

	const routerState: RouterState = {
		url: 'transaction-detail-type-admin-crud',
		urls: ['transaction-detail-type-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const transactionDetailTypeDataFactory = new TransactionDetailTypeDataFactory();

	const defaultDataSize = 30;
	const defaultData = transactionDetailTypeDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getTransactionDetailTypeCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTransactionDetailTypeCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminTransactionDetailTypeCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminTransactionDetailTypeCrudListComponent);
			transactionDetailTypeCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(transactionDetailTypeCrudListComponent.transactionDetailTypesCount).toBe(defaultData.length);
			...
			expect(transactionDetailTypeCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			transactionDetailTypeCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = transactionDetailTypeDataFactory.createAll(filteredDataSize);
			...
			expect(transactionDetailTypeCrudListComponent.transactionDetailTypesCount).toBe(filteredDataSize);
			expect(transactionDetailTypeCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(transactionDetailTypeCrudListComponent.pageIndex).toBe(0);
			expect(transactionDetailTypeCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			transactionDetailTypeCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		transactionDetailTypeCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		transactionDetailTypeCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
