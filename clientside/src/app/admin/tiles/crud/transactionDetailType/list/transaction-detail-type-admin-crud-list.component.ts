...
import {TransactionDetailTypeModelState} from 'src/app/models/transactionDetailType/transaction_detail_type.model.state';
...
import {
	...
	getTransactionDetailTypeCollectionCount
...
import {CrudTileMode} from '../transaction-detail-type-admin-crud.component';
...
@Component({
	selector: 'cb-transaction-detail-type-admin-crud-list',
	templateUrl: './transaction-detail-type-admin-crud-list.component.html',
	styleUrls: [
		'./transaction-detail-type-admin-crud-list.component.scss',
...
export class AdminTransactionDetailTypeCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TransactionDetailTypeModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TransactionDetailTypeModel.searchFields,
	...
	/**
	 * List of Transaction Detail Types that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	transactionDetailTypes: Observable<TransactionDetailTypeModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	transactionDetailTypesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.transactionDetailTypesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.transactionDetailTypesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransactionDetailTypeAction(
				modelAction.TransactionDetailTypeModelActionTypes.INITIALISE_TRANSACTION_DETAIL_TYPE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransactionDetailTypeAction(
				modelAction.TransactionDetailTypeModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.transactionDetailTypes = this.store.select(getTransactionDetailTypeCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTransactionDetailTypeCollectionCount, this.collectionId).subscribe(
				(count) => this.transactionDetailTypesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TransactionDetailTypeModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TransactionDetailTypeModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TransactionDetailTypeModelActionTypes.EXPORT_ALL_TRANSACTION_DETAIL_TYPE;
		} else {
			actionType = modelAction.TransactionDetailTypeModelActionTypes.EXPORT_TRANSACTION_DETAIL_TYPE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TransactionDetailTypeAction(
				modelAction.TransactionDetailTypeModelActionTypes.IMPORT_TRANSACTION_DETAIL_TYPES,
				{
					file: file,
				},
				[new modelAction.TransactionDetailTypeAction(
			...
			window.open(environment.API_URL + '/api/transactiondetailtype/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TransactionDetailTypeModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Transaction Detail Type entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Transaction Detail Type entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TransactionDetailTypeModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TransactionDetailTypeModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE;
						} else if (usingSelected) {
							actionType = modelAction.TransactionDetailTypeModelActionTypes.DELETE_ALL_TRANSACTION_DETAIL_TYPE;
						} else {
							actionType = modelAction.TransactionDetailTypeModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TransactionDetailTypeAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TRANSACTION_DETAIL_TYPE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TRANSACTION_DETAIL_TYPE and DELETE_TRANSACTION_DETAIL_TYPE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TransactionDetailTypeModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.transactionDetailTypesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
