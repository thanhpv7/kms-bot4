...
import {BpjsFacilityListModelState, BpjsFacilityListModelAudit} from 'src/app/models/bpjsFacilityList/bpjs_facility_list.model.state';
import {BpjsFacilityListModel} from 'src/app/models/bpjsFacilityList/bpjs_facility_list.model';
import {
	getBpjsFacilityListModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-facility-list-admin-crud-edit',
	templateUrl: './bpjs-facility-list-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-facility-list-admin-crud-edit.component.scss',
...
export class AdminBpjsFacilityListCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsFacilityListAudits$: Observable<BpjsFacilityListModelAudit[]>;
	...
	bpjsHealthFacilityOppositeRelation = 'bpjsFacilityLists';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsFacilityListModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsFacilityListModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsFacilityListModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsFacilityListModel();
		...
		if (this.targetModelId) {
			this.bpjsFacilityListAudits$ = this.store.select(getBpjsFacilityListModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsFacilityListModel> = {
		...
		this.store.dispatch(new modelAction.BpjsFacilityListAction(
			modelAction.BpjsFacilityListModelActionTypes.FETCH_BPJS_FACILITY_LIST,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsFacilityListModelWithId, this.targetModelId)
			...
			model = new BpjsFacilityListModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsFacilityListAction(
			modelAction.BpjsFacilityListModelActionTypes.FETCH_BPJS_FACILITY_LIST_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsFacilityListAction(
				modelAction.BpjsFacilityListModelActionTypes.CREATE_BPJS_FACILITY_LIST,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsFacilityListAction(
				modelAction.BpjsFacilityListModelActionTypes.UPDATE_BPJS_FACILITY_LIST,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsFacilityListModel = new BpjsFacilityListModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsFacilityListModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsFacilityListModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsFacilityListModel.getProps();
		this.modelRelations = BpjsFacilityListModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsFacilityListModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsFacilityListModel {
		let updatedModel: BpjsFacilityListModel = new BpjsFacilityListModel(model);
