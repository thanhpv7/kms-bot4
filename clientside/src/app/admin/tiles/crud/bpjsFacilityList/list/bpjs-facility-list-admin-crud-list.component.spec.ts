...
import {AdminBpjsFacilityListCrudListComponent} from './bpjs-facility-list-admin-crud-list.component';
...
import {AdminBpjsFacilityListCrudModule} from '../bpjs-facility-list-admin-crud.module';
import {
 ...
 */
describe('BPJS Facility List Crud List Tile Component', () => {

	let fixture;
	let bpjsFacilityListCrudListComponent: AdminBpjsFacilityListCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsFacilityListModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'fc5bf1f4-eea8-48a8-a95e-b846264bb6ca';
	const searchContent = 'text';

	const searchableFieldCount = BpjsFacilityListModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-facility-list-admin-crud',
		urls: ['bpjs-facility-list-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsFacilityListDataFactory = new BpjsFacilityListDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsFacilityListDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsFacilityListCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsFacilityListCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsFacilityListCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsFacilityListCrudListComponent);
			bpjsFacilityListCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsFacilityListCrudListComponent.bpjsFacilityListsCount).toBe(defaultData.length);
			...
			expect(bpjsFacilityListCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsFacilityListCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsFacilityListDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsFacilityListCrudListComponent.bpjsFacilityListsCount).toBe(filteredDataSize);
			expect(bpjsFacilityListCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsFacilityListCrudListComponent.pageIndex).toBe(0);
			expect(bpjsFacilityListCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsFacilityListCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsFacilityListCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsFacilityListCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
