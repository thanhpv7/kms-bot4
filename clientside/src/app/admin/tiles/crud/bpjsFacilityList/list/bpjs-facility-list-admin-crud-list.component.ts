...
import * as modelAction from 'src/app/models/bpjsFacilityList/bpjs_facility_list.model.action';
import {BpjsFacilityListModelState} from 'src/app/models/bpjsFacilityList/bpjs_facility_list.model.state';
...
import {
	...
	getBpjsFacilityListCollectionCount
...
import {CrudTileMode} from '../bpjs-facility-list-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-facility-list-admin-crud-list',
	templateUrl: './bpjs-facility-list-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-facility-list-admin-crud-list.component.scss',
...
export class AdminBpjsFacilityListCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsFacilityListModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsFacilityListModel.searchFields,
	...
	/**
	 * List of BPJS Facility Lists that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsFacilityLists: Observable<BpjsFacilityListModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsFacilityListsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsFacilityListModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsFacilityListModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'fc5bf1f4-eea8-48a8-a95e-b846264bb6ca';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsFacilityListsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsFacilityListsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsFacilityListModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsFacilityListAction(
				modelAction.BpjsFacilityListModelActionTypes.INITIALISE_BPJS_FACILITY_LIST_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsFacilityListAction(
				modelAction.BpjsFacilityListModelActionTypes.FETCH_BPJS_FACILITY_LIST_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsFacilityListAction(
						modelAction.BpjsFacilityListModelActionTypes.COUNT_BPJS_FACILITY_LISTS,
						null
					)
				]
			))
		...
		this.bpjsFacilityLists = this.store.select(getBpjsFacilityListCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsFacilityListCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsFacilityListsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-facility-list');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsFacilityListModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsFacilityListModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsFacilityListModelActionTypes.EXPORT_ALL_BPJS_FACILITY_LIST;
		} else {
			actionType = modelAction.BpjsFacilityListModelActionTypes.EXPORT_BPJS_FACILITY_LIST_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsFacilityListAction(
				modelAction.BpjsFacilityListModelActionTypes.IMPORT_BPJS_FACILITY_LISTS,
				{
					file: file,
				},
				[new modelAction.BpjsFacilityListAction(
			...
			window.open(environment.API_URL + '/api/bpjsfacilitylist/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsFacilityListModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Facility List entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Facility List entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsFacilityListModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsFacilityListModelActionTypes.DELETE_BPJS_FACILITY_LIST;
						} else if (usingSelected) {
							actionType = modelAction.BpjsFacilityListModelActionTypes.DELETE_ALL_BPJS_FACILITY_LIST;
						} else {
							actionType = modelAction.BpjsFacilityListModelActionTypes.DELETE_BPJS_FACILITY_LIST_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsFacilityListAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_FACILITY_LIST when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_FACILITY_LIST and DELETE_BPJS_FACILITY_LIST_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsFacilityListModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsFacilityListModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsFacilityListsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
