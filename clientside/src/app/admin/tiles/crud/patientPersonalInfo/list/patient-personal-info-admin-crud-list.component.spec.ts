...
import {AdminPatientPersonalInfoCrudListComponent} from './patient-personal-info-admin-crud-list.component';
...
import {AdminPatientPersonalInfoCrudModule} from '../patient-personal-info-admin-crud.module';
import {
	PatientPersonalInfoModelState,
	initialState as initialPatientPersonalInfoModelState
} from 'src/app/models/patientPersonalInfo/patient_personal_info.model.state';
 ...
 */
describe('Patient Personal Info Crud List Tile Component', () => {

	let fixture;
	let patientPersonalInfoCrudListComponent: AdminPatientPersonalInfoCrudListComponent;

	let store: MockStore<{ model: PatientPersonalInfoModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientPersonalInfoModelState: PatientPersonalInfoModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientPersonalInfoModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '28f08850-cfba-488c-8ff1-71ce9c4f5788';
	const searchContent = 'text';

	const searchableFieldCount = PatientPersonalInfoModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-personal-info-admin-crud',
		urls: ['patient-personal-info-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientPersonalInfoDataFactory = new PatientPersonalInfoDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientPersonalInfoDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientPersonalInfoModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientPersonalInfoModelState = initialPatientPersonalInfoModelState;
		store.setState({model: patientPersonalInfoModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientPersonalInfoCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientPersonalInfoCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientPersonalInfoCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientPersonalInfoCrudListComponent);
			patientPersonalInfoCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientPersonalInfoCrudListComponent.patientPersonalInfosCount).toBe(defaultData.length);
			...
			expect(patientPersonalInfoCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientPersonalInfoCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientPersonalInfoDataFactory.createAll(filteredDataSize);
			...
			expect(patientPersonalInfoCrudListComponent.patientPersonalInfosCount).toBe(filteredDataSize);
			expect(patientPersonalInfoCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientPersonalInfoCrudListComponent.pageIndex).toBe(0);
			expect(patientPersonalInfoCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientPersonalInfoCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientPersonalInfoCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientPersonalInfoCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
