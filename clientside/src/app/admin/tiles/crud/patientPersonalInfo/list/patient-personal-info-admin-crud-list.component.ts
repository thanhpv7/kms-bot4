...
import {PatientPersonalInfoModelState} from 'src/app/models/patientPersonalInfo/patient_personal_info.model.state';
...
import {
	getPatientPersonalInfoCollectionModels,
	getPatientPersonalInfoCollectionCount
...
@Component({
	selector: 'cb-patient-personal-info-admin-crud-list',
	templateUrl: './patient-personal-info-admin-crud-list.component.html',
	styleUrls: [
		'./patient-personal-info-admin-crud-list.component.scss',
...
export class AdminPatientPersonalInfoCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientPersonalInfoModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientPersonalInfoModel.searchFields,
	...
	/**
	 * List of Patient Personal Infos that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientPersonalInfos: Observable<PatientPersonalInfoModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientPersonalInfosCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientPersonalInfosCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientPersonalInfosCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPersonalInfoAction(
				modelAction.PatientPersonalInfoModelActionTypes.INITIALISE_PATIENT_PERSONAL_INFO_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPersonalInfoAction(
				modelAction.PatientPersonalInfoModelActionTypes.FETCH_PATIENT_PERSONAL_INFO_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientPersonalInfos = this.store.select(getPatientPersonalInfoCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientPersonalInfoCollectionCount, this.collectionId).subscribe(
				(count) => this.patientPersonalInfosCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientPersonalInfoModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientPersonalInfoModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientPersonalInfoModelActionTypes.EXPORT_ALL_PATIENT_PERSONAL_INFO;
		} else {
			actionType = modelAction.PatientPersonalInfoModelActionTypes.EXPORT_PATIENT_PERSONAL_INFO_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientPersonalInfoAction(
				modelAction.PatientPersonalInfoModelActionTypes.IMPORT_PATIENT_PERSONAL_INFOS,
				{
					file: file,
				},
				[new modelAction.PatientPersonalInfoAction(
			...
			window.open(environment.API_URL + '/api/patientpersonalinfo/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientPersonalInfoModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Personal Info entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Personal Info entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientPersonalInfoModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientPersonalInfoModelActionTypes.DELETE_PATIENT_PERSONAL_INFO;
						} else if (usingSelected) {
							actionType = modelAction.PatientPersonalInfoModelActionTypes.DELETE_ALL_PATIENT_PERSONAL_INFO;
						} else {
							actionType = modelAction.PatientPersonalInfoModelActionTypes.DELETE_PATIENT_PERSONAL_INFO_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientPersonalInfoAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_PERSONAL_INFO when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_PERSONAL_INFO and DELETE_PATIENT_PERSONAL_INFO_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientPersonalInfoModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientPersonalInfosCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
