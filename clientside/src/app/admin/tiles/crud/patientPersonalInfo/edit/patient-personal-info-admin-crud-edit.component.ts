...
import * as modelAction from 'src/app/models/patientPersonalInfo/patient_personal_info.model.action';
import {PatientPersonalInfoModelState, PatientPersonalInfoModelAudit} from 'src/app/models/patientPersonalInfo/patient_personal_info.model.state';
import {PatientPersonalInfoModel} from 'src/app/models/patientPersonalInfo/patient_personal_info.model';
import {
	getPatientPersonalInfoModelAuditsByEntityId,
	getPatientPersonalInfoModelWithId,
	getCountPatientPersonalInfoModels,
...
import {CrudTileMode} from '../patient-personal-info-admin-crud.component';
...
@Component({
	selector: 'cb-patient-personal-info-admin-crud-edit',
	templateUrl: './patient-personal-info-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-personal-info-admin-crud-edit.component.scss',
...
export class AdminPatientPersonalInfoCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '28f08850-cfba-488c-8ff1-71ce9c4f5788';
	 ...
	 */
	patientPersonalInfoAudits$: Observable<PatientPersonalInfoModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientPersonalInformation';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientPersonalInfoModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientPersonalInfoModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientPersonalInfoModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientPersonalInfoModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientPersonalInfoModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientPersonalInfoModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientPersonalInfoModel();
		...
		if (this.targetModelId) {
			this.patientPersonalInfoAudits$ = this.store.select(getPatientPersonalInfoModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientPersonalInfoModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientPersonalInfoAction(
			modelAction.PatientPersonalInfoModelActionTypes.FETCH_PATIENT_PERSONAL_INFO,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientPersonalInfoModelWithId, this.targetModelId)
			...
			model = new PatientPersonalInfoModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-personal-info');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientPersonalInfoAction(
			modelAction.PatientPersonalInfoModelActionTypes.FETCH_PATIENT_PERSONAL_INFO_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientPersonalInfoModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientPersonalInfoAction(
						modelAction.PatientPersonalInfoModelActionTypes.COUNT_PATIENT_PERSONAL_INFOS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientPersonalInfoAction(
				modelAction.PatientPersonalInfoModelActionTypes.CREATE_PATIENT_PERSONAL_INFO,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientPersonalInfoAction(
				modelAction.PatientPersonalInfoModelActionTypes.UPDATE_PATIENT_PERSONAL_INFO,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientPersonalInfoModel = new PatientPersonalInfoModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientPersonalInfoModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientPersonalInfoModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientPersonalInfoModel.getProps();
		this.modelRelations = PatientPersonalInfoModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientPersonalInfoModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientPersonalInfoModel {
		let updatedModel: PatientPersonalInfoModel = new PatientPersonalInfoModel(model);
