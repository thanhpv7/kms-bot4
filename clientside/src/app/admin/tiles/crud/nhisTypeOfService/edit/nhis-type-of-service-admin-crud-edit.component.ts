...
import {NhisTypeOfServiceModelState, NhisTypeOfServiceModelAudit} from 'src/app/models/nhisTypeOfService/nhis_type_of_service.model.state';
import {NhisTypeOfServiceModel} from 'src/app/models/nhisTypeOfService/nhis_type_of_service.model';
import {
	getNhisTypeOfServiceModelAuditsByEntityId,
	getNhisTypeOfServiceModelWithId,
	getCountNhisTypeOfServiceModels,
...
@Component({
	selector: 'cb-nhis-type-of-service-admin-crud-edit',
	templateUrl: './nhis-type-of-service-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-type-of-service-admin-crud-edit.component.scss',
...
export class AdminNhisTypeOfServiceCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisTypeOfServiceAudits$: Observable<NhisTypeOfServiceModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisTypeOfServiceModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NhisTypeOfServiceModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisTypeOfServiceModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisTypeOfServiceModel();
		...
		if (this.targetModelId) {
			this.nhisTypeOfServiceAudits$ = this.store.select(getNhisTypeOfServiceModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisTypeOfServiceModel> = {
		...
		this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
			modelAction.NhisTypeOfServiceModelActionTypes.FETCH_NHIS_TYPE_OF_SERVICE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisTypeOfServiceModelWithId, this.targetModelId)
			...
			model = new NhisTypeOfServiceModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
			modelAction.NhisTypeOfServiceModelActionTypes.FETCH_NHIS_TYPE_OF_SERVICE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
				modelAction.NhisTypeOfServiceModelActionTypes.CREATE_NHIS_TYPE_OF_SERVICE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
				modelAction.NhisTypeOfServiceModelActionTypes.UPDATE_NHIS_TYPE_OF_SERVICE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisTypeOfServiceModel = new NhisTypeOfServiceModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisTypeOfServiceModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisTypeOfServiceModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisTypeOfServiceModel.getProps();
		this.modelRelations = NhisTypeOfServiceModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisTypeOfServiceModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisTypeOfServiceModel {
		let updatedModel: NhisTypeOfServiceModel = new NhisTypeOfServiceModel(model);
