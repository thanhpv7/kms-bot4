...
import {AdminNhisTypeOfServiceCrudListComponent} from './nhis-type-of-service-admin-crud-list.component';
import {NhisTypeOfServiceDataFactory} from 'src/app/lib/utils/factories/nhis-type-of-service-data-factory';
import {NhisTypeOfServiceModel} from 'src/app/models/nhisTypeOfService/nhis_type_of_service.model';
import {AdminNhisTypeOfServiceCrudModule} from '../nhis-type-of-service-admin-crud.module';
import {
	NhisTypeOfServiceModelState,
	initialState as initialNhisTypeOfServiceModelState
} from 'src/app/models/nhisTypeOfService/nhis_type_of_service.model.state';
 ...
 */
describe('NHIS Type Of Service Crud List Tile Component', () => {

	let fixture;
	let nhisTypeOfServiceCrudListComponent: AdminNhisTypeOfServiceCrudListComponent;

	let store: MockStore<{ model: NhisTypeOfServiceModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisTypeOfServiceModelState: NhisTypeOfServiceModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisTypeOfServiceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c896bb38-959d-4b16-ab25-e2220e234bc8';
	const searchContent = 'text';

	const searchableFieldCount = NhisTypeOfServiceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-type-of-service-admin-crud',
		urls: ['nhis-type-of-service-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisTypeOfServiceDataFactory = new NhisTypeOfServiceDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisTypeOfServiceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisTypeOfServiceModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisTypeOfServiceModelState = initialNhisTypeOfServiceModelState;
		store.setState({model: nhisTypeOfServiceModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisTypeOfServiceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisTypeOfServiceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminNhisTypeOfServiceCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminNhisTypeOfServiceCrudListComponent);
			nhisTypeOfServiceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisTypeOfServiceCrudListComponent.nhisTypeOfServicesCount).toBe(defaultData.length);
			...
			expect(nhisTypeOfServiceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisTypeOfServiceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisTypeOfServiceDataFactory.createAll(filteredDataSize);
			...
			expect(nhisTypeOfServiceCrudListComponent.nhisTypeOfServicesCount).toBe(filteredDataSize);
			expect(nhisTypeOfServiceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisTypeOfServiceCrudListComponent.pageIndex).toBe(0);
			expect(nhisTypeOfServiceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisTypeOfServiceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisTypeOfServiceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisTypeOfServiceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
