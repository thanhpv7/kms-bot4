...
import * as modelAction from 'src/app/models/nhisTypeOfService/nhis_type_of_service.model.action';
import {NhisTypeOfServiceModelState} from 'src/app/models/nhisTypeOfService/nhis_type_of_service.model.state';
...
import {
	getNhisTypeOfServiceCollectionModels,
	getNhisTypeOfServiceCollectionCount
} from 'src/app/models/nhisTypeOfService/nhis_type_of_service.model.selector';
...
import {CrudTileMode} from '../nhis-type-of-service-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-type-of-service-admin-crud-list',
	templateUrl: './nhis-type-of-service-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-type-of-service-admin-crud-list.component.scss',
...
export class AdminNhisTypeOfServiceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisTypeOfServiceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisTypeOfServiceModel.searchFields,
	...
	/**
	 * List of NHIS Type Of Services that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	nhisTypeOfServices: Observable<NhisTypeOfServiceModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	nhisTypeOfServicesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = NhisTypeOfServiceModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NhisTypeOfServiceModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'c896bb38-959d-4b16-ab25-e2220e234bc8';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisTypeOfServicesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisTypeOfServicesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisTypeOfServiceModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
				modelAction.NhisTypeOfServiceModelActionTypes.INITIALISE_NHIS_TYPE_OF_SERVICE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
				modelAction.NhisTypeOfServiceModelActionTypes.FETCH_NHIS_TYPE_OF_SERVICE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisTypeOfServiceAction(
						modelAction.NhisTypeOfServiceModelActionTypes.COUNT_NHIS_TYPE_OF_SERVICES,
						null
					)
				]
			))
		...
		this.nhisTypeOfServices = this.store.select(getNhisTypeOfServiceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisTypeOfServiceCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisTypeOfServicesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('nhis-type-of-service');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisTypeOfServiceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisTypeOfServiceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisTypeOfServiceModelActionTypes.EXPORT_ALL_NHIS_TYPE_OF_SERVICE;
		} else {
			actionType = modelAction.NhisTypeOfServiceModelActionTypes.EXPORT_NHIS_TYPE_OF_SERVICE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
				modelAction.NhisTypeOfServiceModelActionTypes.IMPORT_NHIS_TYPE_OF_SERVICES,
				{
					file: file,
				},
				[new modelAction.NhisTypeOfServiceAction(
			...
			window.open(environment.API_URL + '/api/nhistypeofservice/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisTypeOfServiceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Type Of Service entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Type Of Service entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisTypeOfServiceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisTypeOfServiceModelActionTypes.DELETE_NHIS_TYPE_OF_SERVICE;
						} else if (usingSelected) {
							actionType = modelAction.NhisTypeOfServiceModelActionTypes.DELETE_ALL_NHIS_TYPE_OF_SERVICE;
						} else {
							actionType = modelAction.NhisTypeOfServiceModelActionTypes.DELETE_NHIS_TYPE_OF_SERVICE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisTypeOfServiceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_TYPE_OF_SERVICE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_TYPE_OF_SERVICE and DELETE_NHIS_TYPE_OF_SERVICE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisTypeOfServiceModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisTypeOfServiceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisTypeOfServicesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
