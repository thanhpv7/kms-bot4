...
@Component({
	selector: 'cb-cash-receipt-admin-crud-list',
	templateUrl: './cash-receipt-admin-crud-list.component.html',
	styleUrls: [
		'./cash-receipt-admin-crud-list.component.scss',
...
export class AdminCashReceiptCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CashReceiptModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CashReceiptModel.searchFields,
	...
	/**
	 * List of Cash Receipts that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	cashReceipts: Observable<CashReceiptModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	cashReceiptsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.cashReceiptsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.cashReceiptsCount === 0
		},
		{
	...
	filterQuestions: FilterQuestion[] = [
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Cash Change Type to Cash Receipt here] off begin
		{
			filterType: FilterQuestionType.dropdown,
			config: {
				options: cashChangeTypeEnumArray,
				labelField: 'value',
				valueField: 'key',
				id: 'CashReceiptCashChangeTypeFilter',
				label: 'Cash Change Type',
			},
			name: 'cashChangeType',
		},
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Cash Change Type to Cash Receipt here] end
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Receipt Type to Cash Receipt here] off begin
		{
			filterType: FilterQuestionType.dropdown,
			config: {
				options: receiptTypeEnumArray,
				labelField: 'value',
				valueField: 'key',
				id: 'CashReceiptReceiptTypeFilter',
				label: 'Receipt Type',
			},
			name: 'receiptType',
		},
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Receipt Type to Cash Receipt here] end
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashReceiptAction(
				modelAction.CashReceiptModelActionTypes.INITIALISE_CASH_RECEIPT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashReceiptAction(
				modelAction.CashReceiptModelActionTypes.FETCH_CASH_RECEIPT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.cashReceipts = this.store.select(getCashReceiptCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCashReceiptCollectionCount, this.collectionId).subscribe(
				(count) => this.cashReceiptsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CashReceiptModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CashReceiptModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CashReceiptModelActionTypes.EXPORT_ALL_CASH_RECEIPT;
		} else {
			actionType = modelAction.CashReceiptModelActionTypes.EXPORT_CASH_RECEIPT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CashReceiptAction(
				modelAction.CashReceiptModelActionTypes.IMPORT_CASH_RECEIPTS,
				{
					file: file,
				},
				[new modelAction.CashReceiptAction(
			...
			window.open(environment.API_URL + '/api/cashreceipt/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CashReceiptModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Cash Receipt entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Cash Receipt entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CashReceiptModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CashReceiptModelActionTypes.DELETE_CASH_RECEIPT;
						} else if (usingSelected) {
							actionType = modelAction.CashReceiptModelActionTypes.DELETE_ALL_CASH_RECEIPT;
						} else {
							actionType = modelAction.CashReceiptModelActionTypes.DELETE_CASH_RECEIPT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CashReceiptAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CASH_RECEIPT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CASH_RECEIPT and DELETE_CASH_RECEIPT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CashReceiptModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.cashReceiptsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
