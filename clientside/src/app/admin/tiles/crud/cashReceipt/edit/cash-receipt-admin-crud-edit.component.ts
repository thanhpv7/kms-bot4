...
import {CashReceiptModelState, CashReceiptModelAudit} from 'src/app/models/cashReceipt/cash_receipt.model.state';
...
import {
	getCashReceiptModelAuditsByEntityId,
...
import * as merchantModelAction from 'src/app/models/merchant/merchant.model.action';
import {
	getMerchantCollectionModels, getMerchantModelWithId,
...
import {CrudTileMode} from '../cash-receipt-admin-crud.component';
...
@Component({
	selector: 'cb-cash-receipt-admin-crud-edit',
	templateUrl: './cash-receipt-admin-crud-edit.component.html',
	styleUrls: [
		'./cash-receipt-admin-crud-edit.component.scss',
...
export class AdminCashReceiptCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'da4f63e8-7359-43cb-8ec1-df4555127c32';
	 ...
	 */
	cashReceiptAudits$: Observable<CashReceiptModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Merchant and Cash Receipts here] off begin
	merchantMultiCrudActive: boolean = false;
	merchantDisplayName = 'Merchant';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Merchant and Cash Receipts here] end
	merchantOppositeRelation = 'cashReceipts';
	merchantRelations: ModelRelation[] = this.merchantMultiCrudActive ? MerchantModel.getRelations()
		.map(relation => {
			if (relation.id === this.merchantOppositeRelation) {
	...
	retailPharmacyInvoiceOppositeRelation = 'cashReceipts';
	...
	cashierStaffRelations: ModelRelation[] = this.cashierStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
	...
	patientOppositeRelation = 'invoiceReceipts';
	 ...
	 */
	modelProperties: ModelProperty[] = CashReceiptModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = CashReceiptModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CashReceiptModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'merchant' && this.merchantMultiCrudActive) {
					exp.fields = MerchantModel.getProps().map(prop => {
						return prop.name;
					});
				}
	...
	constructor(
		private readonly store: Store<{ model: CashReceiptModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCashReceiptModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CashReceiptModel();
		...
		if (this.targetModelId) {
			this.cashReceiptAudits$ = this.store.select(getCashReceiptModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CashReceiptModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.CashReceiptAction(
			modelAction.CashReceiptModelActionTypes.FETCH_CASH_RECEIPT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCashReceiptModelWithId, this.targetModelId)
			...
			model = new CashReceiptModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CashReceiptAction(
			modelAction.CashReceiptModelActionTypes.FETCH_CASH_RECEIPT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<CashReceiptModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.merchantMultiCrudActive || this.retailPharmacyInvoiceMultiCrudActive || this.cashierStaffMultiCrudActive || this.patientMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.CashReceiptAction(
						modelAction.CashReceiptModelActionTypes.COUNT_CASH_RECEIPTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.CashReceiptAction(
				modelAction.CashReceiptModelActionTypes.CREATE_CASH_RECEIPT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CashReceiptAction(
				modelAction.CashReceiptModelActionTypes.UPDATE_CASH_RECEIPT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CashReceiptModel = new CashReceiptModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCashReceiptModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CashReceiptModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.merchantDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MerchantModel.getProps(), this.merchantRelations, this.isViewOnly),
					new MerchantModel(),
					MerchantModel,
			...
			if (event.entityName === this.cashierStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.cashierStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CashReceiptModel.getProps();
		this.modelRelations = CashReceiptModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'merchant';
		}).hideElement = this.merchantMultiCrudActive || this.modelRelations.find(rel => rel.id === 'merchant').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.merchantMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.merchantDisplayName, many: false});
		}
		...
		if (this.cashierStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.cashierStaffDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.merchant && this.targetModel.merchant.id && this.merchantMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				MerchantModel,
				this.merchantOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.merchant.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getMerchantModelWithId, this.targetModel.merchantId).subscribe(merchant => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MerchantModel.getProps(), this.merchantRelations, this.isViewOnly),
					merchant,
					MerchantModel,
		...
		if (this.targetModel.cashierStaff && this.targetModel.cashierStaff.id && this.cashierStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.cashierStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.cashierStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.cashierStaffId).subscribe(cashierStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.cashierStaffRelations, this.isViewOnly),
					cashierStaff,
					StaffModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: CashReceiptModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CashReceiptModel {
		let updatedModel: CashReceiptModel = new CashReceiptModel(model);

		if (this.merchantMultiCrudActive) {
			updatedModel.merchant = event.additionalEntities.merchant ? event.additionalEntities.merchant : undefined;

			if (!updatedModel.merchant && event.payload) {
				event.payload.merchantId = null;
			}
		} else if (event.payload?.merchantId !== undefined) {
			event.payload.merchant = null;
			updatedModel.merchant = null;
		}
		...
		if (this.cashierStaffMultiCrudActive) {
			updatedModel.cashierStaff = event.additionalEntities.cashierStaff ? event.additionalEntities.cashierStaff : undefined;

			if (!updatedModel.cashierStaff && event.payload) {
				event.payload.cashierStaffId = null;
			}
		} else if (event.payload?.cashierStaffId !== undefined) {
			event.payload.cashierStaff = null;
			updatedModel.cashierStaff = null;
		}
