...
import * as modelAction from 'src/app/models/invoicePaymentOther/invoice_payment_other.model.action';
import {InvoicePaymentOtherModelState, InvoicePaymentOtherModelAudit} from 'src/app/models/invoicePaymentOther/invoice_payment_other.model.state';
import {InvoicePaymentOtherModel} from 'src/app/models/invoicePaymentOther/invoice_payment_other.model';
import {
	getInvoicePaymentOtherModelAuditsByEntityId,
	getInvoicePaymentOtherModelWithId,
	getCountInvoicePaymentOtherModels,
...
import {CrudTileMode} from '../invoice-payment-other-admin-crud.component';
...
@Component({
	selector: 'cb-invoice-payment-other-admin-crud-edit',
	templateUrl: './invoice-payment-other-admin-crud-edit.component.html',
	styleUrls: [
		'./invoice-payment-other-admin-crud-edit.component.scss',
...
export class AdminInvoicePaymentOtherCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '66fb689f-92fe-4836-9244-9b0d6af4998e';
	 ...
	 */
	invoicePaymentOtherAudits$: Observable<InvoicePaymentOtherModelAudit[]>;
	...
	invoiceOppositeRelation = 'invoicePaymentOther';
	 ...
	 */
	modelProperties: ModelProperty[] = InvoicePaymentOtherModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InvoicePaymentOtherModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InvoicePaymentOtherModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: InvoicePaymentOtherModel;
	...
	constructor(
		private readonly store: Store<{ model: InvoicePaymentOtherModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInvoicePaymentOtherModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InvoicePaymentOtherModel();
		...
		if (this.targetModelId) {
			this.invoicePaymentOtherAudits$ = this.store.select(getInvoicePaymentOtherModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InvoicePaymentOtherModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
			modelAction.InvoicePaymentOtherModelActionTypes.FETCH_INVOICE_PAYMENT_OTHER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInvoicePaymentOtherModelWithId, this.targetModelId)
			...
			model = new InvoicePaymentOtherModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('invoice-payment-other');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
			modelAction.InvoicePaymentOtherModelActionTypes.FETCH_INVOICE_PAYMENT_OTHER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InvoicePaymentOtherModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InvoicePaymentOtherAction(
						modelAction.InvoicePaymentOtherModelActionTypes.COUNT_INVOICE_PAYMENT_OTHERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
				modelAction.InvoicePaymentOtherModelActionTypes.CREATE_INVOICE_PAYMENT_OTHER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
				modelAction.InvoicePaymentOtherModelActionTypes.UPDATE_INVOICE_PAYMENT_OTHER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InvoicePaymentOtherModel = new InvoicePaymentOtherModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInvoicePaymentOtherModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InvoicePaymentOtherModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InvoicePaymentOtherModel.getProps();
		this.modelRelations = InvoicePaymentOtherModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: InvoicePaymentOtherModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InvoicePaymentOtherModel {
		let updatedModel: InvoicePaymentOtherModel = new InvoicePaymentOtherModel(model);
