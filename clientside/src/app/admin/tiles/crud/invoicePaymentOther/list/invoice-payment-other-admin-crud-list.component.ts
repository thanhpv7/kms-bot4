...
import {InvoicePaymentOtherModelState} from 'src/app/models/invoicePaymentOther/invoice_payment_other.model.state';
...
import {
	getInvoicePaymentOtherCollectionModels,
	getInvoicePaymentOtherCollectionCount
...
@Component({
	selector: 'cb-invoice-payment-other-admin-crud-list',
	templateUrl: './invoice-payment-other-admin-crud-list.component.html',
	styleUrls: [
		'./invoice-payment-other-admin-crud-list.component.scss',
...
export class AdminInvoicePaymentOtherCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InvoicePaymentOtherModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InvoicePaymentOtherModel.searchFields,
	...
	/**
	 * List of Invoice Payment Others that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	invoicePaymentOthers: Observable<InvoicePaymentOtherModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	invoicePaymentOthersCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.invoicePaymentOthersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.invoicePaymentOthersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
				modelAction.InvoicePaymentOtherModelActionTypes.INITIALISE_INVOICE_PAYMENT_OTHER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
				modelAction.InvoicePaymentOtherModelActionTypes.FETCH_INVOICE_PAYMENT_OTHER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.invoicePaymentOthers = this.store.select(getInvoicePaymentOtherCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInvoicePaymentOtherCollectionCount, this.collectionId).subscribe(
				(count) => this.invoicePaymentOthersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InvoicePaymentOtherModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InvoicePaymentOtherModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InvoicePaymentOtherModelActionTypes.EXPORT_ALL_INVOICE_PAYMENT_OTHER;
		} else {
			actionType = modelAction.InvoicePaymentOtherModelActionTypes.EXPORT_INVOICE_PAYMENT_OTHER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
				modelAction.InvoicePaymentOtherModelActionTypes.IMPORT_INVOICE_PAYMENT_OTHERS,
				{
					file: file,
				},
				[new modelAction.InvoicePaymentOtherAction(
			...
			window.open(environment.API_URL + '/api/invoicepaymentother/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InvoicePaymentOtherModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Invoice Payment Other entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Invoice Payment Other entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InvoicePaymentOtherModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InvoicePaymentOtherModelActionTypes.DELETE_INVOICE_PAYMENT_OTHER;
						} else if (usingSelected) {
							actionType = modelAction.InvoicePaymentOtherModelActionTypes.DELETE_ALL_INVOICE_PAYMENT_OTHER;
						} else {
							actionType = modelAction.InvoicePaymentOtherModelActionTypes.DELETE_INVOICE_PAYMENT_OTHER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InvoicePaymentOtherAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVOICE_PAYMENT_OTHER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVOICE_PAYMENT_OTHER and DELETE_INVOICE_PAYMENT_OTHER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InvoicePaymentOtherModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.invoicePaymentOthersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
