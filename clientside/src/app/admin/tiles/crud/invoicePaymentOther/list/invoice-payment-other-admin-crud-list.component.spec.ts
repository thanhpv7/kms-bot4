...
import {AdminInvoicePaymentOtherCrudListComponent} from './invoice-payment-other-admin-crud-list.component';
...
import {AdminInvoicePaymentOtherCrudModule} from '../invoice-payment-other-admin-crud.module';
import {
	InvoicePaymentOtherModelState,
	initialState as initialInvoicePaymentOtherModelState
} from 'src/app/models/invoicePaymentOther/invoice_payment_other.model.state';
 ...
 */
describe('Invoice Payment Other Crud List Tile Component', () => {

	let fixture;
	let invoicePaymentOtherCrudListComponent: AdminInvoicePaymentOtherCrudListComponent;

	let store: MockStore<{ model: InvoicePaymentOtherModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let invoicePaymentOtherModelState: InvoicePaymentOtherModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InvoicePaymentOtherModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '66fb689f-92fe-4836-9244-9b0d6af4998e';
	const searchContent = 'text';

	const searchableFieldCount = InvoicePaymentOtherModel.searchFields.length;

	const routerState: RouterState = {
		url: 'invoice-payment-other-admin-crud',
		urls: ['invoice-payment-other-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const invoicePaymentOtherDataFactory = new InvoicePaymentOtherDataFactory();

	const defaultDataSize = 30;
	const defaultData = invoicePaymentOtherDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InvoicePaymentOtherModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		invoicePaymentOtherModelState = initialInvoicePaymentOtherModelState;
		store.setState({model: invoicePaymentOtherModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInvoicePaymentOtherCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInvoicePaymentOtherCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInvoicePaymentOtherCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInvoicePaymentOtherCrudListComponent);
			invoicePaymentOtherCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(invoicePaymentOtherCrudListComponent.invoicePaymentOthersCount).toBe(defaultData.length);
			...
			expect(invoicePaymentOtherCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			invoicePaymentOtherCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = invoicePaymentOtherDataFactory.createAll(filteredDataSize);
			...
			expect(invoicePaymentOtherCrudListComponent.invoicePaymentOthersCount).toBe(filteredDataSize);
			expect(invoicePaymentOtherCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(invoicePaymentOtherCrudListComponent.pageIndex).toBe(0);
			expect(invoicePaymentOtherCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			invoicePaymentOtherCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		invoicePaymentOtherCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		invoicePaymentOtherCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
