...
import * as modelAction from 'src/app/models/patientPaymentSelfPaying/patient_payment_self_paying.model.action';
import {PatientPaymentSelfPayingModelState, PatientPaymentSelfPayingModelAudit} from 'src/app/models/patientPaymentSelfPaying/patient_payment_self_paying.model.state';
import {PatientPaymentSelfPayingModel} from 'src/app/models/patientPaymentSelfPaying/patient_payment_self_paying.model';
import {
	getPatientPaymentSelfPayingModelAuditsByEntityId,
	getPatientPaymentSelfPayingModelWithId,
	getCountPatientPaymentSelfPayingModels,
...
import {CrudTileMode} from '../patient-payment-self-paying-admin-crud.component';
...
@Component({
	selector: 'cb-patient-payment-self-paying-admin-crud-edit',
	templateUrl: './patient-payment-self-paying-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-payment-self-paying-admin-crud-edit.component.scss',
...
export class AdminPatientPaymentSelfPayingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'ef41237b-5973-4601-af21-afb4bea29c5b';
	 ...
	 */
	patientPaymentSelfPayingAudits$: Observable<PatientPaymentSelfPayingModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientSelfPaying';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientPaymentSelfPayingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientPaymentSelfPayingModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientPaymentSelfPayingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientPaymentSelfPayingModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientPaymentSelfPayingModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientPaymentSelfPayingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientPaymentSelfPayingModel();
		...
		if (this.targetModelId) {
			this.patientPaymentSelfPayingAudits$ = this.store.select(getPatientPaymentSelfPayingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientPaymentSelfPayingModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
			modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_PATIENT_PAYMENT_SELF_PAYING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientPaymentSelfPayingModelWithId, this.targetModelId)
			...
			model = new PatientPaymentSelfPayingModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-payment-self-paying');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
			modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_PATIENT_PAYMENT_SELF_PAYING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientPaymentSelfPayingModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientPaymentSelfPayingAction(
						modelAction.PatientPaymentSelfPayingModelActionTypes.COUNT_PATIENT_PAYMENT_SELF_PAYINGS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
				modelAction.PatientPaymentSelfPayingModelActionTypes.CREATE_PATIENT_PAYMENT_SELF_PAYING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
				modelAction.PatientPaymentSelfPayingModelActionTypes.UPDATE_PATIENT_PAYMENT_SELF_PAYING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientPaymentSelfPayingModel = new PatientPaymentSelfPayingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientPaymentSelfPayingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientPaymentSelfPayingModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientPaymentSelfPayingModel.getProps();
		this.modelRelations = PatientPaymentSelfPayingModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientPaymentSelfPayingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientPaymentSelfPayingModel {
		let updatedModel: PatientPaymentSelfPayingModel = new PatientPaymentSelfPayingModel(model);
