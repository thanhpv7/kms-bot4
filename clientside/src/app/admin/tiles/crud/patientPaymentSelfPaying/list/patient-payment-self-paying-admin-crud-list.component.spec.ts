...
import {AdminPatientPaymentSelfPayingCrudListComponent} from './patient-payment-self-paying-admin-crud-list.component';
...
import {AdminPatientPaymentSelfPayingCrudModule} from '../patient-payment-self-paying-admin-crud.module';
import {
	PatientPaymentSelfPayingModelState,
	initialState as initialPatientPaymentSelfPayingModelState
} from 'src/app/models/patientPaymentSelfPaying/patient_payment_self_paying.model.state';
 ...
 */
describe('Patient Payment  Self Paying Crud List Tile Component', () => {

	let fixture;
	let patientPaymentSelfPayingCrudListComponent: AdminPatientPaymentSelfPayingCrudListComponent;

	let store: MockStore<{ model: PatientPaymentSelfPayingModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientPaymentSelfPayingModelState: PatientPaymentSelfPayingModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientPaymentSelfPayingModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ef41237b-5973-4601-af21-afb4bea29c5b';
	const searchContent = 'text';

	const searchableFieldCount = PatientPaymentSelfPayingModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-payment-self-paying-admin-crud',
		urls: ['patient-payment-self-paying-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientPaymentSelfPayingDataFactory = new PatientPaymentSelfPayingDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientPaymentSelfPayingDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientPaymentSelfPayingModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientPaymentSelfPayingModelState = initialPatientPaymentSelfPayingModelState;
		store.setState({model: patientPaymentSelfPayingModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientPaymentSelfPayingCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientPaymentSelfPayingCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientPaymentSelfPayingCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientPaymentSelfPayingCrudListComponent);
			patientPaymentSelfPayingCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientPaymentSelfPayingCrudListComponent.patientPaymentSelfPayingsCount).toBe(defaultData.length);
			...
			expect(patientPaymentSelfPayingCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientPaymentSelfPayingCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientPaymentSelfPayingDataFactory.createAll(filteredDataSize);
			...
			expect(patientPaymentSelfPayingCrudListComponent.patientPaymentSelfPayingsCount).toBe(filteredDataSize);
			expect(patientPaymentSelfPayingCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientPaymentSelfPayingCrudListComponent.pageIndex).toBe(0);
			expect(patientPaymentSelfPayingCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientPaymentSelfPayingCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientPaymentSelfPayingCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientPaymentSelfPayingCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
