...
import {PatientPaymentSelfPayingModelState} from 'src/app/models/patientPaymentSelfPaying/patient_payment_self_paying.model.state';
...
import {
	getPatientPaymentSelfPayingCollectionModels,
	getPatientPaymentSelfPayingCollectionCount
...
@Component({
	selector: 'cb-patient-payment-self-paying-admin-crud-list',
	templateUrl: './patient-payment-self-paying-admin-crud-list.component.html',
	styleUrls: [
		'./patient-payment-self-paying-admin-crud-list.component.scss',
...
export class AdminPatientPaymentSelfPayingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientPaymentSelfPayingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientPaymentSelfPayingModel.searchFields,
	...
	/**
	 * List of Patient Payment  Self Payings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientPaymentSelfPayings: Observable<PatientPaymentSelfPayingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientPaymentSelfPayingsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientPaymentSelfPayingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientPaymentSelfPayingsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
				modelAction.PatientPaymentSelfPayingModelActionTypes.INITIALISE_PATIENT_PAYMENT_SELF_PAYING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
				modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_PATIENT_PAYMENT_SELF_PAYING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientPaymentSelfPayings = this.store.select(getPatientPaymentSelfPayingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientPaymentSelfPayingCollectionCount, this.collectionId).subscribe(
				(count) => this.patientPaymentSelfPayingsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientPaymentSelfPayingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientPaymentSelfPayingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientPaymentSelfPayingModelActionTypes.EXPORT_ALL_PATIENT_PAYMENT_SELF_PAYING;
		} else {
			actionType = modelAction.PatientPaymentSelfPayingModelActionTypes.EXPORT_PATIENT_PAYMENT_SELF_PAYING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
				modelAction.PatientPaymentSelfPayingModelActionTypes.IMPORT_PATIENT_PAYMENT_SELF_PAYINGS,
				{
					file: file,
				},
				[new modelAction.PatientPaymentSelfPayingAction(
			...
			window.open(environment.API_URL + '/api/patientpaymentselfpaying/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientPaymentSelfPayingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment  Self Paying entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment  Self Paying entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientPaymentSelfPayingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientPaymentSelfPayingModelActionTypes.DELETE_PATIENT_PAYMENT_SELF_PAYING;
						} else if (usingSelected) {
							actionType = modelAction.PatientPaymentSelfPayingModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_SELF_PAYING;
						} else {
							actionType = modelAction.PatientPaymentSelfPayingModelActionTypes.DELETE_PATIENT_PAYMENT_SELF_PAYING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientPaymentSelfPayingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_PAYMENT_SELF_PAYING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_PAYMENT_SELF_PAYING and DELETE_PATIENT_PAYMENT_SELF_PAYING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientPaymentSelfPayingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientPaymentSelfPayingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
