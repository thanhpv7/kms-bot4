...
import {AdminCatalogueMedicalCrudListComponent} from './catalogue-medical-admin-crud-list.component';
...
import {AdminCatalogueMedicalCrudModule} from '../catalogue-medical-admin-crud.module';
import {
	CatalogueMedicalModelState,
	initialState as initialCatalogueMedicalModelState
} from 'src/app/models/catalogueMedical/catalogue_medical.model.state';
 ...
 */
describe('Catalogue Medical  Crud List Tile Component', () => {

	let fixture;
	let catalogueMedicalCrudListComponent: AdminCatalogueMedicalCrudListComponent;

	let store: MockStore<{ model: CatalogueMedicalModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let catalogueMedicalModelState: CatalogueMedicalModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<CatalogueMedicalModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e3f56971-1773-4723-a6c3-d434d75917a5';
	const searchContent = 'text';

	const searchableFieldCount = CatalogueMedicalModel.searchFields.length;

	const routerState: RouterState = {
		url: 'catalogue-medical-admin-crud',
		urls: ['catalogue-medical-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const catalogueMedicalDataFactory = new CatalogueMedicalDataFactory();

	const defaultDataSize = 30;
	const defaultData = catalogueMedicalDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: CatalogueMedicalModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		catalogueMedicalModelState = initialCatalogueMedicalModelState;
		store.setState({model: catalogueMedicalModelState});
		...
		spyOn(store, 'select')
			.withArgs(getCatalogueMedicalCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCatalogueMedicalCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminCatalogueMedicalCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminCatalogueMedicalCrudListComponent);
			catalogueMedicalCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(catalogueMedicalCrudListComponent.catalogueMedicalsCount).toBe(defaultData.length);
			...
			expect(catalogueMedicalCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			catalogueMedicalCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = catalogueMedicalDataFactory.createAll(filteredDataSize);
			...
			expect(catalogueMedicalCrudListComponent.catalogueMedicalsCount).toBe(filteredDataSize);
			expect(catalogueMedicalCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(catalogueMedicalCrudListComponent.pageIndex).toBe(0);
			expect(catalogueMedicalCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			catalogueMedicalCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		catalogueMedicalCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		catalogueMedicalCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
