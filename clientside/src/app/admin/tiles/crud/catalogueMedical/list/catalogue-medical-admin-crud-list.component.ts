...
import * as modelAction from 'src/app/models/catalogueMedical/catalogue_medical.model.action';
import {CatalogueMedicalModelState} from 'src/app/models/catalogueMedical/catalogue_medical.model.state';
...
import {
	getCatalogueMedicalCollectionModels,
	getCatalogueMedicalCollectionCount
...
import {CrudTileMode} from '../catalogue-medical-admin-crud.component';
...
@Component({
	selector: 'cb-catalogue-medical-admin-crud-list',
	templateUrl: './catalogue-medical-admin-crud-list.component.html',
	styleUrls: [
		'./catalogue-medical-admin-crud-list.component.scss',
...
export class AdminCatalogueMedicalCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CatalogueMedicalModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CatalogueMedicalModel.searchFields,
	...
	/**
	 * List of Catalogue Medical s that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	catalogueMedicals: Observable<CatalogueMedicalModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	catalogueMedicalsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = CatalogueMedicalModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = CatalogueMedicalModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'e3f56971-1773-4723-a6c3-d434d75917a5';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.catalogueMedicalsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.catalogueMedicalsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: CatalogueMedicalModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CatalogueMedicalAction(
				modelAction.CatalogueMedicalModelActionTypes.INITIALISE_CATALOGUE_MEDICAL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CatalogueMedicalAction(
				modelAction.CatalogueMedicalModelActionTypes.FETCH_CATALOGUE_MEDICAL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.CatalogueMedicalAction(
						modelAction.CatalogueMedicalModelActionTypes.COUNT_CATALOGUE_MEDICALS,
						null
					)
				]
			))
		...
		this.catalogueMedicals = this.store.select(getCatalogueMedicalCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCatalogueMedicalCollectionCount, this.collectionId).subscribe(
				(count) => this.catalogueMedicalsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('catalogue-medical');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CatalogueMedicalModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CatalogueMedicalModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CatalogueMedicalModelActionTypes.EXPORT_ALL_CATALOGUE_MEDICAL;
		} else {
			actionType = modelAction.CatalogueMedicalModelActionTypes.EXPORT_CATALOGUE_MEDICAL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CatalogueMedicalAction(
				modelAction.CatalogueMedicalModelActionTypes.IMPORT_CATALOGUE_MEDICALS,
				{
					file: file,
				},
				[new modelAction.CatalogueMedicalAction(
			...
			window.open(environment.API_URL + '/api/cataloguemedical/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CatalogueMedicalModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Catalogue Medical  entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Catalogue Medical  entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CatalogueMedicalModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CatalogueMedicalModelActionTypes.DELETE_CATALOGUE_MEDICAL;
						} else if (usingSelected) {
							actionType = modelAction.CatalogueMedicalModelActionTypes.DELETE_ALL_CATALOGUE_MEDICAL;
						} else {
							actionType = modelAction.CatalogueMedicalModelActionTypes.DELETE_CATALOGUE_MEDICAL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CatalogueMedicalAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CATALOGUE_MEDICAL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CATALOGUE_MEDICAL and DELETE_CATALOGUE_MEDICAL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<CatalogueMedicalModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CatalogueMedicalModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.catalogueMedicalsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
