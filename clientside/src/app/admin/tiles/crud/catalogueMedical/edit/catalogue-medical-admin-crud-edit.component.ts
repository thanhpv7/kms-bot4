...
import {CatalogueMedicalModelState, CatalogueMedicalModelAudit} from 'src/app/models/catalogueMedical/catalogue_medical.model.state';
import {CatalogueMedicalModel} from 'src/app/models/catalogueMedical/catalogue_medical.model';
import {
	getCatalogueMedicalModelAuditsByEntityId,
	getCatalogueMedicalModelWithId,
	getCountCatalogueMedicalModels,
...
@Component({
	selector: 'cb-catalogue-medical-admin-crud-edit',
	templateUrl: './catalogue-medical-admin-crud-edit.component.html',
	styleUrls: [
		'./catalogue-medical-admin-crud-edit.component.scss',
...
export class AdminCatalogueMedicalCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	catalogueMedicalAudits$: Observable<CatalogueMedicalModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'stockCatalogueMedical';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CatalogueMedicalModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: CatalogueMedicalModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCatalogueMedicalModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CatalogueMedicalModel();
		...
		if (this.targetModelId) {
			this.catalogueMedicalAudits$ = this.store.select(getCatalogueMedicalModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CatalogueMedicalModel> = {
		...
		this.store.dispatch(new modelAction.CatalogueMedicalAction(
			modelAction.CatalogueMedicalModelActionTypes.FETCH_CATALOGUE_MEDICAL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCatalogueMedicalModelWithId, this.targetModelId)
			...
			model = new CatalogueMedicalModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CatalogueMedicalAction(
			modelAction.CatalogueMedicalModelActionTypes.FETCH_CATALOGUE_MEDICAL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.CatalogueMedicalAction(
				modelAction.CatalogueMedicalModelActionTypes.CREATE_CATALOGUE_MEDICAL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CatalogueMedicalAction(
				modelAction.CatalogueMedicalModelActionTypes.UPDATE_CATALOGUE_MEDICAL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CatalogueMedicalModel = new CatalogueMedicalModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCatalogueMedicalModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CatalogueMedicalModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CatalogueMedicalModel.getProps();
		this.modelRelations = CatalogueMedicalModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: CatalogueMedicalModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CatalogueMedicalModel {
		let updatedModel: CatalogueMedicalModel = new CatalogueMedicalModel(model);
