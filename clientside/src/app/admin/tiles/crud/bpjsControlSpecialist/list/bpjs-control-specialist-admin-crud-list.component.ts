...
import * as modelAction from 'src/app/models/bpjsControlSpecialist/bpjs_control_specialist.model.action';
import {BpjsControlSpecialistModelState} from 'src/app/models/bpjsControlSpecialist/bpjs_control_specialist.model.state';
...
import {CrudTileMode} from '../bpjs-control-specialist-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-control-specialist-admin-crud-list',
	templateUrl: './bpjs-control-specialist-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-control-specialist-admin-crud-list.component.scss',
...
export class AdminBpjsControlSpecialistCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsControlSpecialistModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsControlSpecialistModel.searchFields,
	...
	/**
	 * List of BPJS Control Specialists that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsControlSpecialists: Observable<BpjsControlSpecialistModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsControlSpecialistsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsControlSpecialistModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsControlSpecialistModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '9d5512d2-64d8-420c-86b5-2cca7b8fc0f3';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsControlSpecialistsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsControlSpecialistsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsControlSpecialistModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
				modelAction.BpjsControlSpecialistModelActionTypes.INITIALISE_BPJS_CONTROL_SPECIALIST_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
				modelAction.BpjsControlSpecialistModelActionTypes.FETCH_BPJS_CONTROL_SPECIALIST_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsControlSpecialistAction(
						modelAction.BpjsControlSpecialistModelActionTypes.COUNT_BPJS_CONTROL_SPECIALISTS,
						null
					)
				]
			))
		...
		this.bpjsControlSpecialists = this.store.select(getBpjsControlSpecialistCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsControlSpecialistCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsControlSpecialistsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-control-specialist');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsControlSpecialistModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsControlSpecialistModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsControlSpecialistModelActionTypes.EXPORT_ALL_BPJS_CONTROL_SPECIALIST;
		} else {
			actionType = modelAction.BpjsControlSpecialistModelActionTypes.EXPORT_BPJS_CONTROL_SPECIALIST_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
				modelAction.BpjsControlSpecialistModelActionTypes.IMPORT_BPJS_CONTROL_SPECIALISTS,
				{
					file: file,
				},
				[new modelAction.BpjsControlSpecialistAction(
			...
			window.open(environment.API_URL + '/api/bpjscontrolspecialist/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsControlSpecialistModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Control Specialist entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Control Specialist entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsControlSpecialistModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsControlSpecialistModelActionTypes.DELETE_BPJS_CONTROL_SPECIALIST;
						} else if (usingSelected) {
							actionType = modelAction.BpjsControlSpecialistModelActionTypes.DELETE_ALL_BPJS_CONTROL_SPECIALIST;
						} else {
							actionType = modelAction.BpjsControlSpecialistModelActionTypes.DELETE_BPJS_CONTROL_SPECIALIST_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_CONTROL_SPECIALIST when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_CONTROL_SPECIALIST and DELETE_BPJS_CONTROL_SPECIALIST_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsControlSpecialistModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsControlSpecialistModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsControlSpecialistsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
