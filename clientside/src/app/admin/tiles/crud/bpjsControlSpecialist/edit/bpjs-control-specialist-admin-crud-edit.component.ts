...
import {BpjsControlSpecialistModelState, BpjsControlSpecialistModelAudit} from 'src/app/models/bpjsControlSpecialist/bpjs_control_specialist.model.state';
import {BpjsControlSpecialistModel} from 'src/app/models/bpjsControlSpecialist/bpjs_control_specialist.model';
import {
	getBpjsControlSpecialistModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-control-specialist-admin-crud-edit',
	templateUrl: './bpjs-control-specialist-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-control-specialist-admin-crud-edit.component.scss',
...
export class AdminBpjsControlSpecialistCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsControlSpecialistAudits$: Observable<BpjsControlSpecialistModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsControlSpecialistModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsControlSpecialistModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsControlSpecialistModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsControlSpecialistModel();
		...
		if (this.targetModelId) {
			this.bpjsControlSpecialistAudits$ = this.store.select(getBpjsControlSpecialistModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsControlSpecialistModel> = {
		...
		this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
			modelAction.BpjsControlSpecialistModelActionTypes.FETCH_BPJS_CONTROL_SPECIALIST,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsControlSpecialistModelWithId, this.targetModelId)
			...
			model = new BpjsControlSpecialistModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
			modelAction.BpjsControlSpecialistModelActionTypes.FETCH_BPJS_CONTROL_SPECIALIST_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
				modelAction.BpjsControlSpecialistModelActionTypes.CREATE_BPJS_CONTROL_SPECIALIST,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsControlSpecialistAction(
				modelAction.BpjsControlSpecialistModelActionTypes.UPDATE_BPJS_CONTROL_SPECIALIST,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsControlSpecialistModel = new BpjsControlSpecialistModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsControlSpecialistModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsControlSpecialistModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsControlSpecialistModel.getProps();
		this.modelRelations = BpjsControlSpecialistModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsControlSpecialistModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsControlSpecialistModel {
		let updatedModel: BpjsControlSpecialistModel = new BpjsControlSpecialistModel(model);
