...
import {TariffSchemaModelState} from 'src/app/models/tariffSchema/tariff_schema.model.state';
...
import {
	...
	getTariffSchemaCollectionCount
...
@Component({
	selector: 'cb-tariff-schema-admin-crud-list',
	templateUrl: './tariff-schema-admin-crud-list.component.html',
	styleUrls: [
		'./tariff-schema-admin-crud-list.component.scss',
...
export class AdminTariffSchemaCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TariffSchemaModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TariffSchemaModel.searchFields,
	...
	/**
	 * List of Tariff Schemas that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	tariffSchemas: Observable<TariffSchemaModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	tariffSchemasCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.tariffSchemasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.tariffSchemasCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffSchemaAction(
				modelAction.TariffSchemaModelActionTypes.INITIALISE_TARIFF_SCHEMA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffSchemaAction(
				modelAction.TariffSchemaModelActionTypes.FETCH_TARIFF_SCHEMA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.tariffSchemas = this.store.select(getTariffSchemaCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTariffSchemaCollectionCount, this.collectionId).subscribe(
				(count) => this.tariffSchemasCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TariffSchemaModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TariffSchemaModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TariffSchemaModelActionTypes.EXPORT_ALL_TARIFF_SCHEMA;
		} else {
			actionType = modelAction.TariffSchemaModelActionTypes.EXPORT_TARIFF_SCHEMA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TariffSchemaAction(
				modelAction.TariffSchemaModelActionTypes.IMPORT_TARIFF_SCHEMAS,
				{
					file: file,
				},
				[new modelAction.TariffSchemaAction(
			...
			window.open(environment.API_URL + '/api/tariffschema/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TariffSchemaModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Tariff Schema entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Tariff Schema entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TariffSchemaModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TariffSchemaModelActionTypes.DELETE_TARIFF_SCHEMA;
						} else if (usingSelected) {
							actionType = modelAction.TariffSchemaModelActionTypes.DELETE_ALL_TARIFF_SCHEMA;
						} else {
							actionType = modelAction.TariffSchemaModelActionTypes.DELETE_TARIFF_SCHEMA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TariffSchemaAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TARIFF_SCHEMA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TARIFF_SCHEMA and DELETE_TARIFF_SCHEMA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TariffSchemaModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.tariffSchemasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
