...
import {AdminTariffSchemaCrudListComponent} from './tariff-schema-admin-crud-list.component';
...
import {AdminTariffSchemaCrudModule} from '../tariff-schema-admin-crud.module';
import {
 ...
 */
describe('Tariff Schema Crud List Tile Component', () => {

	let fixture;
	let tariffSchemaCrudListComponent: AdminTariffSchemaCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<TariffSchemaModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '678df279-0fff-4097-8c3e-6ac491d69fc1';
	const searchContent = 'text';

	const searchableFieldCount = TariffSchemaModel.searchFields.length;

	const routerState: RouterState = {
		url: 'tariff-schema-admin-crud',
		urls: ['tariff-schema-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const tariffSchemaDataFactory = new TariffSchemaDataFactory();

	const defaultDataSize = 30;
	const defaultData = tariffSchemaDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getTariffSchemaCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTariffSchemaCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminTariffSchemaCrudListComponent);
			tariffSchemaCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(tariffSchemaCrudListComponent.tariffSchemasCount).toBe(defaultData.length);
			...
			expect(tariffSchemaCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			tariffSchemaCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = tariffSchemaDataFactory.createAll(filteredDataSize);
			...
			expect(tariffSchemaCrudListComponent.tariffSchemasCount).toBe(filteredDataSize);
			expect(tariffSchemaCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(tariffSchemaCrudListComponent.pageIndex).toBe(0);
			expect(tariffSchemaCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			tariffSchemaCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		tariffSchemaCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		tariffSchemaCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
