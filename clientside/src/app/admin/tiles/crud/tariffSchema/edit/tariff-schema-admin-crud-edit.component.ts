...
import * as modelAction from 'src/app/models/tariffSchema/tariff_schema.model.action';
import {TariffSchemaModelState, TariffSchemaModelAudit} from 'src/app/models/tariffSchema/tariff_schema.model.state';
import {TariffSchemaModel} from 'src/app/models/tariffSchema/tariff_schema.model';
import {
	getTariffSchemaModelAuditsByEntityId,
...
import * as tariffDefinitionModelAction from 'src/app/models/tariffDefinition/tariff_definition.model.action';
import {
	getTariffDefinitionCollectionModels, getTariffDefinitionModelWithId,
...
import * as tariffFormulaModelAction from 'src/app/models/tariffFormula/tariff_formula.model.action';
import {
	getTariffFormulaCollectionModels, getTariffFormulaModelWithId,
...
import {CrudTileMode} from '../tariff-schema-admin-crud.component';
...
@Component({
	selector: 'cb-tariff-schema-admin-crud-edit',
	templateUrl: './tariff-schema-admin-crud-edit.component.html',
	styleUrls: [
		'./tariff-schema-admin-crud-edit.component.scss',
...
export class AdminTariffSchemaCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '678df279-0fff-4097-8c3e-6ac491d69fc1';
	 ...
	 */
	tariffSchemaAudits$: Observable<TariffSchemaModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Base Schema and Base Scheme here] off begin
	baseSchemeMultiCrudActive: boolean = false;
	baseSchemeDisplayName = 'Base Scheme';
	baseSchemeCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Base Schema and Base Scheme here] end
	baseSchemeOppositeRelation = 'baseSchema';
	baseSchemeRelations: ModelRelation[] = this.baseSchemeMultiCrudActive ? TariffFormulaModel.getRelations()
		.map(relation => {
			if (relation.id === this.baseSchemeOppositeRelation) {
	...
	invoiceItemsOppositeRelation = 'tariffSchema';
	...
	invoicesOppositeRelation = 'tariffSchema';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Price Schema and Tariff Definitions here] off begin
	tariffDefinitionsMultiCrudActive: boolean = false;
	tariffDefinitionsDisplayName = 'Tariff Definitions';
	tariffDefinitionsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Price Schema and Tariff Definitions here] end
	tariffDefinitionsOppositeRelation = 'priceSchema';
	tariffDefinitionsRelations: ModelRelation[] = this.tariffDefinitionsMultiCrudActive ? TariffDefinitionModel.getRelations()
		.map(relation => {
			if (relation.id === this.tariffDefinitionsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Schema and Tariff Scheme here] off begin
	tariffSchemeMultiCrudActive: boolean = false;
	tariffSchemeDisplayName = 'Tariff Scheme';
	tariffSchemeCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Schema and Tariff Scheme here] end
	tariffSchemeOppositeRelation = 'tariffSchema';
	tariffSchemeRelations: ModelRelation[] = this.tariffSchemeMultiCrudActive ? TariffFormulaModel.getRelations()
		.map(relation => {
			if (relation.id === this.tariffSchemeOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = TariffSchemaModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TariffSchemaModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TariffSchemaModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'baseScheme' && this.baseSchemeMultiCrudActive) {
					exp.fields = TariffFormulaModel.getProps().map(prop => {
						return prop.name;
					});
				}
				...
				if (relation.id === 'tariffDefinitions' && this.tariffDefinitionsMultiCrudActive) {
					exp.fields = TariffDefinitionModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'tariffScheme' && this.tariffSchemeMultiCrudActive) {
					exp.fields = TariffFormulaModel.getProps().map(prop => {
						return prop.name;
					});
				}
	...
	constructor(
		private readonly store: Store<{ model: TariffSchemaModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTariffSchemaModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TariffSchemaModel();
		...
		if (this.targetModelId) {
			this.tariffSchemaAudits$ = this.store.select(getTariffSchemaModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TariffSchemaModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TariffSchemaAction(
			modelAction.TariffSchemaModelActionTypes.FETCH_TARIFF_SCHEMA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTariffSchemaModelWithId, this.targetModelId)
			...
			model = new TariffSchemaModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TariffSchemaAction(
			modelAction.TariffSchemaModelActionTypes.FETCH_TARIFF_SCHEMA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TariffSchemaModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.baseSchemeMultiCrudActive || this.invoiceItemsMultiCrudActive || this.invoicesMultiCrudActive || this.tariffDefinitionsMultiCrudActive || this.tariffSchemeMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TariffSchemaAction(
						modelAction.TariffSchemaModelActionTypes.COUNT_TARIFF_SCHEMAS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TariffSchemaAction(
				modelAction.TariffSchemaModelActionTypes.CREATE_TARIFF_SCHEMA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TariffSchemaAction(
				modelAction.TariffSchemaModelActionTypes.UPDATE_TARIFF_SCHEMA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TariffSchemaModel = new TariffSchemaModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTariffSchemaModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TariffSchemaModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.baseSchemeDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffFormulaModel.getProps(), this.baseSchemeRelations, this.isViewOnly),
					new TariffFormulaModel(),
					TariffFormulaModel,
					this.baseSchemeDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.baseSchemeRelations,
					true
				);
			...
			if (event.entityName === this.tariffDefinitionsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffDefinitionModel.getProps(), this.tariffDefinitionsRelations, this.isViewOnly),
					new TariffDefinitionModel(),
					TariffDefinitionModel,
					this.tariffDefinitionsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.tariffDefinitionsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.tariffSchemeDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffFormulaModel.getProps(), this.tariffSchemeRelations, this.isViewOnly),
					new TariffFormulaModel(),
					TariffFormulaModel,
					this.tariffSchemeDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.tariffSchemeRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TariffSchemaModel.getProps();
		this.modelRelations = TariffSchemaModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'baseScheme';
		}).hideElement = this.baseSchemeMultiCrudActive || this.modelRelations.find(rel => rel.id === 'baseScheme').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'tariffDefinitions';
		}).hideElement = this.tariffDefinitionsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'tariffDefinitions').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'tariffScheme';
		}).hideElement = this.tariffSchemeMultiCrudActive || this.modelRelations.find(rel => rel.id === 'tariffScheme').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.baseSchemeMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.baseSchemeDisplayName, many: true});
			this.store.dispatch(new tariffFormulaModelAction.TariffFormulaAction(
				tariffFormulaModelAction.TariffFormulaModelActionTypes.INITIALISE_TARIFF_FORMULA_COLLECTION_STATE,
				{
					collectionId: this.baseSchemeCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffFormulaCollectionModels, this.baseSchemeCollectionId).subscribe(models => {
				models.forEach(baseScheme => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(TariffFormulaModel.getProps(), this.baseSchemeRelations, this.isViewOnly),
						baseScheme,
						TariffFormulaModel,
		...
		if (this.tariffDefinitionsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.tariffDefinitionsDisplayName, many: true});
			this.store.dispatch(new tariffDefinitionModelAction.TariffDefinitionAction(
				tariffDefinitionModelAction.TariffDefinitionModelActionTypes.INITIALISE_TARIFF_DEFINITION_COLLECTION_STATE,
				{
					collectionId: this.tariffDefinitionsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffDefinitionCollectionModels, this.tariffDefinitionsCollectionId).subscribe(models => {
				models.forEach(tariffDefinitions => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(TariffDefinitionModel.getProps(), this.tariffDefinitionsRelations, this.isViewOnly),
						tariffDefinitions,
						TariffDefinitionModel,
		...
		if (this.tariffSchemeMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.tariffSchemeDisplayName, many: true});
			this.store.dispatch(new tariffFormulaModelAction.TariffFormulaAction(
				tariffFormulaModelAction.TariffFormulaModelActionTypes.INITIALISE_TARIFF_FORMULA_COLLECTION_STATE,
				{
					collectionId: this.tariffSchemeCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffFormulaCollectionModels, this.tariffSchemeCollectionId).subscribe(models => {
				models.forEach(tariffScheme => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(TariffFormulaModel.getProps(), this.tariffSchemeRelations, this.isViewOnly),
						tariffScheme,
						TariffFormulaModel,
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.baseSchemeIds && this.targetModel.baseSchemeIds.length > 0 && this.baseSchemeMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffFormulaModel,
				this.baseSchemeOppositeRelation,
				this.baseSchemeCollectionId,
				this.targetModel.baseSchemeIds.length,
				this.targetModel.id,
				true
			));
		}
		...
		if (this.targetModel.tariffDefinitionsIds && this.targetModel.tariffDefinitionsIds.length > 0 && this.tariffDefinitionsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffDefinitionModel,
				this.tariffDefinitionsOppositeRelation,
				this.tariffDefinitionsCollectionId,
				this.targetModel.tariffDefinitionsIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.tariffSchemeIds && this.targetModel.tariffSchemeIds.length > 0 && this.tariffSchemeMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffFormulaModel,
				this.tariffSchemeOppositeRelation,
				this.tariffSchemeCollectionId,
				this.targetModel.tariffSchemeIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: TariffSchemaModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TariffSchemaModel {
		let updatedModel: TariffSchemaModel = new TariffSchemaModel(model);

		updatedModel.baseScheme = [];
		if (this.baseSchemeMultiCrudActive) {
			if (event.additionalEntities.baseScheme) {
				event.additionalEntities.baseScheme.forEach(entity => {
					updatedModel.baseScheme.push(entity);
				});
			}

			if (!updatedModel.baseScheme.length) {
				updatedModel.baseSchemeIds = [];
			}
		}
		...
		updatedModel.tariffDefinitions = [];
		if (this.tariffDefinitionsMultiCrudActive) {
			if (event.additionalEntities.tariffDefinitions) {
				event.additionalEntities.tariffDefinitions.forEach(entity => {
					updatedModel.tariffDefinitions.push(entity);
				});
			}

			if (!updatedModel.tariffDefinitions.length) {
				updatedModel.tariffDefinitionsIds = [];
			}
		}
		updatedModel.tariffScheme = [];
		if (this.tariffSchemeMultiCrudActive) {
			if (event.additionalEntities.tariffScheme) {
				event.additionalEntities.tariffScheme.forEach(entity => {
					updatedModel.tariffScheme.push(entity);
				});
			}

			if (!updatedModel.tariffScheme.length) {
				updatedModel.tariffSchemeIds = [];
			}
		}
		return updatedModel;
	}
