...
import {BpjsVisitDataModelState, BpjsVisitDataModelAudit} from 'src/app/models/bpjsVisitData/bpjs_visit_data.model.state';
import {BpjsVisitDataModel} from 'src/app/models/bpjsVisitData/bpjs_visit_data.model';
import {
	getBpjsVisitDataModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-visit-data-admin-crud-edit',
	templateUrl: './bpjs-visit-data-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-visit-data-admin-crud-edit.component.scss',
...
export class AdminBpjsVisitDataCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsVisitDataAudits$: Observable<BpjsVisitDataModelAudit[]>;
	...
	patientVisitOppositeRelation = 'bpjsVisitDatas';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsVisitDataModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsVisitDataModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsVisitDataModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsVisitDataModel();
		...
		if (this.targetModelId) {
			this.bpjsVisitDataAudits$ = this.store.select(getBpjsVisitDataModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsVisitDataModel> = {
		...
		this.store.dispatch(new modelAction.BpjsVisitDataAction(
			modelAction.BpjsVisitDataModelActionTypes.FETCH_BPJS_VISIT_DATA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsVisitDataModelWithId, this.targetModelId)
			...
			model = new BpjsVisitDataModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsVisitDataAction(
			modelAction.BpjsVisitDataModelActionTypes.FETCH_BPJS_VISIT_DATA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsVisitDataAction(
				modelAction.BpjsVisitDataModelActionTypes.CREATE_BPJS_VISIT_DATA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsVisitDataAction(
				modelAction.BpjsVisitDataModelActionTypes.UPDATE_BPJS_VISIT_DATA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsVisitDataModel = new BpjsVisitDataModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsVisitDataModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsVisitDataModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsVisitDataModel.getProps();
		this.modelRelations = BpjsVisitDataModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsVisitDataModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsVisitDataModel {
		let updatedModel: BpjsVisitDataModel = new BpjsVisitDataModel(model);
