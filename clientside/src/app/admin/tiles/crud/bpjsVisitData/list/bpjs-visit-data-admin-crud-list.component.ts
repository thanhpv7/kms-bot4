...
import * as modelAction from 'src/app/models/bpjsVisitData/bpjs_visit_data.model.action';
import {BpjsVisitDataModelState} from 'src/app/models/bpjsVisitData/bpjs_visit_data.model.state';
...
import {
	...
	getBpjsVisitDataCollectionCount
...
import {CrudTileMode} from '../bpjs-visit-data-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-visit-data-admin-crud-list',
	templateUrl: './bpjs-visit-data-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-visit-data-admin-crud-list.component.scss',
...
export class AdminBpjsVisitDataCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsVisitDataModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsVisitDataModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsVisitDataModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '14e67552-522c-4e3b-b159-11f7f9873dcd';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsVisitDatasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsVisitDatasCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsVisitDataModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsVisitDataAction(
				modelAction.BpjsVisitDataModelActionTypes.INITIALISE_BPJS_VISIT_DATA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsVisitDataAction(
				modelAction.BpjsVisitDataModelActionTypes.FETCH_BPJS_VISIT_DATA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsVisitDataAction(
						modelAction.BpjsVisitDataModelActionTypes.COUNT_BPJS_VISIT_DATAS,
						null
					)
				]
			))
		...
		this.bpjsVisitDatas = this.store.select(getBpjsVisitDataCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsVisitDataCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsVisitDatasCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsVisitDataModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsVisitDataModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsVisitDataModelActionTypes.EXPORT_ALL_BPJS_VISIT_DATA;
		} else {
			actionType = modelAction.BpjsVisitDataModelActionTypes.EXPORT_BPJS_VISIT_DATA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsVisitDataAction(
				modelAction.BpjsVisitDataModelActionTypes.IMPORT_BPJS_VISIT_DATAS,
				{
					file: file,
				},
				[new modelAction.BpjsVisitDataAction(
			...
			window.open(environment.API_URL + '/api/bpjsvisitdata/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsVisitDataModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Visit Data entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Visit Data entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsVisitDataModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsVisitDataModelActionTypes.DELETE_BPJS_VISIT_DATA;
						} else if (usingSelected) {
							actionType = modelAction.BpjsVisitDataModelActionTypes.DELETE_ALL_BPJS_VISIT_DATA;
						} else {
							actionType = modelAction.BpjsVisitDataModelActionTypes.DELETE_BPJS_VISIT_DATA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsVisitDataAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_VISIT_DATA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_VISIT_DATA and DELETE_BPJS_VISIT_DATA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsVisitDataModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsVisitDataModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsVisitDatasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
