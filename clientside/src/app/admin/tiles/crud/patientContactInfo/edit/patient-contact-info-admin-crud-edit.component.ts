...
import * as modelAction from 'src/app/models/patientContactInfo/patient_contact_info.model.action';
import {PatientContactInfoModelState, PatientContactInfoModelAudit} from 'src/app/models/patientContactInfo/patient_contact_info.model.state';
import {PatientContactInfoModel} from 'src/app/models/patientContactInfo/patient_contact_info.model';
import {
	getPatientContactInfoModelAuditsByEntityId,
	getPatientContactInfoModelWithId,
	getCountPatientContactInfoModels,
...
import {CrudTileMode} from '../patient-contact-info-admin-crud.component';
...
@Component({
	selector: 'cb-patient-contact-info-admin-crud-edit',
	templateUrl: './patient-contact-info-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-contact-info-admin-crud-edit.component.scss',
...
export class AdminPatientContactInfoCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '5901f131-2586-4ee5-a9a8-aa15cbd4b381';
	 ...
	 */
	patientContactInfoAudits$: Observable<PatientContactInfoModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientContactInformation';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientContactInfoModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientContactInfoModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientContactInfoModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientContactInfoModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientContactInfoModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientContactInfoModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientContactInfoModel();
		...
		if (this.targetModelId) {
			this.patientContactInfoAudits$ = this.store.select(getPatientContactInfoModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientContactInfoModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientContactInfoAction(
			modelAction.PatientContactInfoModelActionTypes.FETCH_PATIENT_CONTACT_INFO,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientContactInfoModelWithId, this.targetModelId)
			...
			model = new PatientContactInfoModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-contact-info');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientContactInfoAction(
			modelAction.PatientContactInfoModelActionTypes.FETCH_PATIENT_CONTACT_INFO_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientContactInfoModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientContactInfoAction(
						modelAction.PatientContactInfoModelActionTypes.COUNT_PATIENT_CONTACT_INFOS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientContactInfoAction(
				modelAction.PatientContactInfoModelActionTypes.CREATE_PATIENT_CONTACT_INFO,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientContactInfoAction(
				modelAction.PatientContactInfoModelActionTypes.UPDATE_PATIENT_CONTACT_INFO,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientContactInfoModel = new PatientContactInfoModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientContactInfoModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientContactInfoModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientContactInfoModel.getProps();
		this.modelRelations = PatientContactInfoModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientContactInfoModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientContactInfoModel {
		let updatedModel: PatientContactInfoModel = new PatientContactInfoModel(model);
