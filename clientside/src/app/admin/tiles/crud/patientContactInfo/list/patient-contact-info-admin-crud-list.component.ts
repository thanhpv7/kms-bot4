...
import {PatientContactInfoModelState} from 'src/app/models/patientContactInfo/patient_contact_info.model.state';
...
import {
	getPatientContactInfoCollectionModels,
	getPatientContactInfoCollectionCount
...
@Component({
	selector: 'cb-patient-contact-info-admin-crud-list',
	templateUrl: './patient-contact-info-admin-crud-list.component.html',
	styleUrls: [
		'./patient-contact-info-admin-crud-list.component.scss',
...
export class AdminPatientContactInfoCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientContactInfoModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientContactInfoModel.searchFields,
	...
	/**
	 * List of Patient Contact Infos that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientContactInfos: Observable<PatientContactInfoModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientContactInfosCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientContactInfosCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientContactInfosCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientContactInfoAction(
				modelAction.PatientContactInfoModelActionTypes.INITIALISE_PATIENT_CONTACT_INFO_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientContactInfoAction(
				modelAction.PatientContactInfoModelActionTypes.FETCH_PATIENT_CONTACT_INFO_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientContactInfos = this.store.select(getPatientContactInfoCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientContactInfoCollectionCount, this.collectionId).subscribe(
				(count) => this.patientContactInfosCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientContactInfoModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientContactInfoModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientContactInfoModelActionTypes.EXPORT_ALL_PATIENT_CONTACT_INFO;
		} else {
			actionType = modelAction.PatientContactInfoModelActionTypes.EXPORT_PATIENT_CONTACT_INFO_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientContactInfoAction(
				modelAction.PatientContactInfoModelActionTypes.IMPORT_PATIENT_CONTACT_INFOS,
				{
					file: file,
				},
				[new modelAction.PatientContactInfoAction(
			...
			window.open(environment.API_URL + '/api/patientcontactinfo/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientContactInfoModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Contact Info entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Contact Info entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientContactInfoModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientContactInfoModelActionTypes.DELETE_PATIENT_CONTACT_INFO;
						} else if (usingSelected) {
							actionType = modelAction.PatientContactInfoModelActionTypes.DELETE_ALL_PATIENT_CONTACT_INFO;
						} else {
							actionType = modelAction.PatientContactInfoModelActionTypes.DELETE_PATIENT_CONTACT_INFO_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientContactInfoAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_CONTACT_INFO when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_CONTACT_INFO and DELETE_PATIENT_CONTACT_INFO_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientContactInfoModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientContactInfosCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
