...
import {AdminPatientContactInfoCrudListComponent} from './patient-contact-info-admin-crud-list.component';
...
import {AdminPatientContactInfoCrudModule} from '../patient-contact-info-admin-crud.module';
import {
	PatientContactInfoModelState,
	initialState as initialPatientContactInfoModelState
} from 'src/app/models/patientContactInfo/patient_contact_info.model.state';
 ...
 */
describe('Patient Contact Info Crud List Tile Component', () => {

	let fixture;
	let patientContactInfoCrudListComponent: AdminPatientContactInfoCrudListComponent;

	let store: MockStore<{ model: PatientContactInfoModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientContactInfoModelState: PatientContactInfoModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientContactInfoModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5901f131-2586-4ee5-a9a8-aa15cbd4b381';
	const searchContent = 'text';

	const searchableFieldCount = PatientContactInfoModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-contact-info-admin-crud',
		urls: ['patient-contact-info-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientContactInfoDataFactory = new PatientContactInfoDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientContactInfoDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientContactInfoModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientContactInfoModelState = initialPatientContactInfoModelState;
		store.setState({model: patientContactInfoModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientContactInfoCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientContactInfoCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientContactInfoCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientContactInfoCrudListComponent);
			patientContactInfoCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientContactInfoCrudListComponent.patientContactInfosCount).toBe(defaultData.length);
			...
			expect(patientContactInfoCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientContactInfoCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientContactInfoDataFactory.createAll(filteredDataSize);
			...
			expect(patientContactInfoCrudListComponent.patientContactInfosCount).toBe(filteredDataSize);
			expect(patientContactInfoCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientContactInfoCrudListComponent.pageIndex).toBe(0);
			expect(patientContactInfoCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientContactInfoCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientContactInfoCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientContactInfoCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
