...
import {MedicalCertificateHospitalizationModelState} from 'src/app/models/medicalCertificateHospitalization/medical_certificate_hospitalization.model.state';
...
import {
	getMedicalCertificateHospitalizationCollectionModels,
	getMedicalCertificateHospitalizationCollectionCount
...
@Component({
	selector: 'cb-medical-certificate-hospitalization-admin-crud-list',
	templateUrl: './medical-certificate-hospitalization-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-hospitalization-admin-crud-list.component.scss',
...
export class AdminMedicalCertificateHospitalizationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificateHospitalizationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificateHospitalizationModel.searchFields,
	...
	/**
	 * List of Medical Certificate Hospitalizations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificateHospitalizations: Observable<MedicalCertificateHospitalizationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificateHospitalizationsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificateHospitalizationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificateHospitalizationsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
				modelAction.MedicalCertificateHospitalizationModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_HOSPITALIZATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
				modelAction.MedicalCertificateHospitalizationModelActionTypes.FETCH_MEDICAL_CERTIFICATE_HOSPITALIZATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificateHospitalizations = this.store.select(getMedicalCertificateHospitalizationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificateHospitalizationCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificateHospitalizationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificateHospitalizationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificateHospitalizationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificateHospitalizationModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_HOSPITALIZATION;
		} else {
			actionType = modelAction.MedicalCertificateHospitalizationModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_HOSPITALIZATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
				modelAction.MedicalCertificateHospitalizationModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_HOSPITALIZATIONS,
				{
					file: file,
				},
				[new modelAction.MedicalCertificateHospitalizationAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificatehospitalization/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificateHospitalizationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Hospitalization entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Hospitalization entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificateHospitalizationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificateHospitalizationModelActionTypes.DELETE_MEDICAL_CERTIFICATE_HOSPITALIZATION;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificateHospitalizationModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_HOSPITALIZATION;
						} else {
							actionType = modelAction.MedicalCertificateHospitalizationModelActionTypes.DELETE_MEDICAL_CERTIFICATE_HOSPITALIZATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_HOSPITALIZATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_HOSPITALIZATION and DELETE_MEDICAL_CERTIFICATE_HOSPITALIZATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificateHospitalizationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificateHospitalizationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
