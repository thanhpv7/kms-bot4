...
import {AdminMedicalCertificateHospitalizationCrudListComponent} from './medical-certificate-hospitalization-admin-crud-list.component';
...
import {AdminMedicalCertificateHospitalizationCrudModule} from '../medical-certificate-hospitalization-admin-crud.module';
import {
	MedicalCertificateHospitalizationModelState,
	initialState as initialMedicalCertificateHospitalizationModelState
} from 'src/app/models/medicalCertificateHospitalization/medical_certificate_hospitalization.model.state';
 ...
 */
describe('Medical Certificate Hospitalization Crud List Tile Component', () => {

	let fixture;
	let medicalCertificateHospitalizationCrudListComponent: AdminMedicalCertificateHospitalizationCrudListComponent;

	let store: MockStore<{ model: MedicalCertificateHospitalizationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalCertificateHospitalizationModelState: MedicalCertificateHospitalizationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalCertificateHospitalizationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8636bce0-3d04-4229-8b8e-f1642bdf4a71';
	const searchContent = 'text';

	const searchableFieldCount = MedicalCertificateHospitalizationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-certificate-hospitalization-admin-crud',
		urls: ['medical-certificate-hospitalization-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalCertificateHospitalizationDataFactory = new MedicalCertificateHospitalizationDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalCertificateHospitalizationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalCertificateHospitalizationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalCertificateHospitalizationModelState = initialMedicalCertificateHospitalizationModelState;
		store.setState({model: medicalCertificateHospitalizationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalCertificateHospitalizationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalCertificateHospitalizationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalCertificateHospitalizationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalCertificateHospitalizationCrudListComponent);
			medicalCertificateHospitalizationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalCertificateHospitalizationCrudListComponent.medicalCertificateHospitalizationsCount).toBe(defaultData.length);
			...
			expect(medicalCertificateHospitalizationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalCertificateHospitalizationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalCertificateHospitalizationDataFactory.createAll(filteredDataSize);
			...
			expect(medicalCertificateHospitalizationCrudListComponent.medicalCertificateHospitalizationsCount).toBe(filteredDataSize);
			expect(medicalCertificateHospitalizationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalCertificateHospitalizationCrudListComponent.pageIndex).toBe(0);
			expect(medicalCertificateHospitalizationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalCertificateHospitalizationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalCertificateHospitalizationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalCertificateHospitalizationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
