...
import * as modelAction from 'src/app/models/medicalCertificateHospitalization/medical_certificate_hospitalization.model.action';
import {MedicalCertificateHospitalizationModelState, MedicalCertificateHospitalizationModelAudit} from 'src/app/models/medicalCertificateHospitalization/medical_certificate_hospitalization.model.state';
...
import {
	getMedicalCertificateHospitalizationModelAuditsByEntityId,
	getMedicalCertificateHospitalizationModelWithId,
	getCountMedicalCertificateHospitalizationModels,
...
import {CrudTileMode} from '../medical-certificate-hospitalization-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-hospitalization-admin-crud-edit',
	templateUrl: './medical-certificate-hospitalization-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-hospitalization-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificateHospitalizationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '8636bce0-3d04-4229-8b8e-f1642bdf4a71';
	 ...
	 */
	medicalCertificateHospitalizationAudits$: Observable<MedicalCertificateHospitalizationModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificateHospitalization';
	...
	medicalStaffOppositeRelation = 'medicalCertificateHospitalizations';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificateHospitalizationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificateHospitalizationModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificateHospitalizationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalCertificateHospitalizationModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificateHospitalizationModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificateHospitalizationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificateHospitalizationModel();
		...
		if (this.targetModelId) {
			this.medicalCertificateHospitalizationAudits$ = this.store.select(getMedicalCertificateHospitalizationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificateHospitalizationModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
			modelAction.MedicalCertificateHospitalizationModelActionTypes.FETCH_MEDICAL_CERTIFICATE_HOSPITALIZATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateHospitalizationModelWithId, this.targetModelId)
			...
			model = new MedicalCertificateHospitalizationModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-hospitalization');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
			modelAction.MedicalCertificateHospitalizationModelActionTypes.FETCH_MEDICAL_CERTIFICATE_HOSPITALIZATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificateHospitalizationModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificateHospitalizationAction(
						modelAction.MedicalCertificateHospitalizationModelActionTypes.COUNT_MEDICAL_CERTIFICATE_HOSPITALIZATIONS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
				modelAction.MedicalCertificateHospitalizationModelActionTypes.CREATE_MEDICAL_CERTIFICATE_HOSPITALIZATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificateHospitalizationAction(
				modelAction.MedicalCertificateHospitalizationModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_HOSPITALIZATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificateHospitalizationModel = new MedicalCertificateHospitalizationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateHospitalizationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificateHospitalizationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificateHospitalizationModel.getProps();
		this.modelRelations = MedicalCertificateHospitalizationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificateHospitalizationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificateHospitalizationModel {
		let updatedModel: MedicalCertificateHospitalizationModel = new MedicalCertificateHospitalizationModel(model);
