...
import {TransferOrderStockDetailModelState} from 'src/app/models/transferOrderStockDetail/transfer_order_stock_detail.model.state';
...
import {
	...
	getTransferOrderStockDetailCollectionCount
...
@Component({
	selector: 'cb-transfer-order-stock-detail-admin-crud-list',
	templateUrl: './transfer-order-stock-detail-admin-crud-list.component.html',
	styleUrls: [
		'./transfer-order-stock-detail-admin-crud-list.component.scss',
...
export class AdminTransferOrderStockDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TransferOrderStockDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TransferOrderStockDetailModel.searchFields,
	...
	/**
	 * List of Transfer Order Stock Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	transferOrderStockDetails: Observable<TransferOrderStockDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	transferOrderStockDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.transferOrderStockDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.transferOrderStockDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
				modelAction.TransferOrderStockDetailModelActionTypes.INITIALISE_TRANSFER_ORDER_STOCK_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
				modelAction.TransferOrderStockDetailModelActionTypes.FETCH_TRANSFER_ORDER_STOCK_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.transferOrderStockDetails = this.store.select(getTransferOrderStockDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTransferOrderStockDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.transferOrderStockDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TransferOrderStockDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TransferOrderStockDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TransferOrderStockDetailModelActionTypes.EXPORT_ALL_TRANSFER_ORDER_STOCK_DETAIL;
		} else {
			actionType = modelAction.TransferOrderStockDetailModelActionTypes.EXPORT_TRANSFER_ORDER_STOCK_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
				modelAction.TransferOrderStockDetailModelActionTypes.IMPORT_TRANSFER_ORDER_STOCK_DETAILS,
				{
					file: file,
				},
				[new modelAction.TransferOrderStockDetailAction(
			...
			window.open(environment.API_URL + '/api/transferorderstockdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TransferOrderStockDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Transfer Order Stock Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Transfer Order Stock Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TransferOrderStockDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TransferOrderStockDetailModelActionTypes.DELETE_TRANSFER_ORDER_STOCK_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.TransferOrderStockDetailModelActionTypes.DELETE_ALL_TRANSFER_ORDER_STOCK_DETAIL;
						} else {
							actionType = modelAction.TransferOrderStockDetailModelActionTypes.DELETE_TRANSFER_ORDER_STOCK_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TRANSFER_ORDER_STOCK_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TRANSFER_ORDER_STOCK_DETAIL and DELETE_TRANSFER_ORDER_STOCK_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TransferOrderStockDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.transferOrderStockDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
