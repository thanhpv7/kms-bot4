...
import {AdminTransferOrderStockDetailCrudListComponent} from './transfer-order-stock-detail-admin-crud-list.component';
...
import {AdminTransferOrderStockDetailCrudModule} from '../transfer-order-stock-detail-admin-crud.module';
import {
 ...
 */
describe('Transfer Order Stock Detail Crud List Tile Component', () => {

	let fixture;
	let transferOrderStockDetailCrudListComponent: AdminTransferOrderStockDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<TransferOrderStockDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd56daa0e-4ea3-401e-9d00-51450902419f';
	const searchContent = 'text';

	const searchableFieldCount = TransferOrderStockDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'transfer-order-stock-detail-admin-crud',
		urls: ['transfer-order-stock-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const transferOrderStockDetailDataFactory = new TransferOrderStockDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = transferOrderStockDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getTransferOrderStockDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTransferOrderStockDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminTransferOrderStockDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminTransferOrderStockDetailCrudListComponent);
			transferOrderStockDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(transferOrderStockDetailCrudListComponent.transferOrderStockDetailsCount).toBe(defaultData.length);
			...
			expect(transferOrderStockDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			transferOrderStockDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = transferOrderStockDetailDataFactory.createAll(filteredDataSize);
			...
			expect(transferOrderStockDetailCrudListComponent.transferOrderStockDetailsCount).toBe(filteredDataSize);
			expect(transferOrderStockDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(transferOrderStockDetailCrudListComponent.pageIndex).toBe(0);
			expect(transferOrderStockDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			transferOrderStockDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		transferOrderStockDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		transferOrderStockDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
