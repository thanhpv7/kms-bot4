...
import * as modelAction from 'src/app/models/transferOrderStockDetail/transfer_order_stock_detail.model.action';
import {TransferOrderStockDetailModelState, TransferOrderStockDetailModelAudit} from 'src/app/models/transferOrderStockDetail/transfer_order_stock_detail.model.state';
import {TransferOrderStockDetailModel} from 'src/app/models/transferOrderStockDetail/transfer_order_stock_detail.model';
import {
	getTransferOrderStockDetailModelAuditsByEntityId,
...
import {CrudTileMode} from '../transfer-order-stock-detail-admin-crud.component';
...
@Component({
	selector: 'cb-transfer-order-stock-detail-admin-crud-edit',
	templateUrl: './transfer-order-stock-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./transfer-order-stock-detail-admin-crud-edit.component.scss',
...
export class AdminTransferOrderStockDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'd56daa0e-4ea3-401e-9d00-51450902419f';
	 ...
	 */
	transferOrderStockDetailAudits$: Observable<TransferOrderStockDetailModelAudit[]>;
	...
	staffOppositeRelation = 'transferOrderStockDetails';
	...
	stockCatalogueOppositeRelation = 'transferOrderStockDetails';
	...
	transferOrderMultiCrudActive: boolean = false;
	transferOrderDisplayName = 'Transfer Order';
	...
	transferOrderOppositeRelation = 'transferOrderStockDetails';
	transferOrderRelations: ModelRelation[] = this.transferOrderMultiCrudActive ? TransferOrderModel.getRelations()
		.map(relation => {
			if (relation.id === this.transferOrderOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = TransferOrderStockDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TransferOrderStockDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TransferOrderStockDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'transferOrder' && this.transferOrderMultiCrudActive) {
	 ...
	 */
	targetModel: TransferOrderStockDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: TransferOrderStockDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTransferOrderStockDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TransferOrderStockDetailModel();
		...
		if (this.targetModelId) {
			this.transferOrderStockDetailAudits$ = this.store.select(getTransferOrderStockDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TransferOrderStockDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
			modelAction.TransferOrderStockDetailModelActionTypes.FETCH_TRANSFER_ORDER_STOCK_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTransferOrderStockDetailModelWithId, this.targetModelId)
			...
			model = new TransferOrderStockDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('transfer-order-stock-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
			modelAction.TransferOrderStockDetailModelActionTypes.FETCH_TRANSFER_ORDER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TransferOrderStockDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.staffMultiCrudActive || this.stockCatalogueMultiCrudActive || this.transferOrderMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TransferOrderStockDetailAction(
						modelAction.TransferOrderStockDetailModelActionTypes.COUNT_TRANSFER_ORDER_STOCK_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
				modelAction.TransferOrderStockDetailModelActionTypes.CREATE_TRANSFER_ORDER_STOCK_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TransferOrderStockDetailAction(
				modelAction.TransferOrderStockDetailModelActionTypes.UPDATE_TRANSFER_ORDER_STOCK_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TransferOrderStockDetailModel = new TransferOrderStockDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTransferOrderStockDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TransferOrderStockDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.transferOrderDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TransferOrderModel.getProps(), this.transferOrderRelations, this.isViewOnly),
					new TransferOrderModel(),
					TransferOrderModel,
					this.transferOrderDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.transferOrderRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TransferOrderStockDetailModel.getProps();
		this.modelRelations = TransferOrderStockDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'transferOrder';
		}).hideElement = this.transferOrderMultiCrudActive || this.modelRelations.find(rel => rel.id === 'transferOrder').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.transferOrderMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.transferOrderDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.transferOrder && this.targetModel.transferOrder.id && this.transferOrderMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TransferOrderModel,
				this.transferOrderOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.transferOrder.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getTransferOrderModelWithId, this.targetModel.transferOrderId).subscribe(transferOrder => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TransferOrderModel.getProps(), this.transferOrderRelations, this.isViewOnly),
					transferOrder,
					TransferOrderModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: TransferOrderStockDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TransferOrderStockDetailModel {
		let updatedModel: TransferOrderStockDetailModel = new TransferOrderStockDetailModel(model);
		...
		if (this.transferOrderMultiCrudActive) {
			updatedModel.transferOrder = event.additionalEntities.transferOrder ? event.additionalEntities.transferOrder : undefined;

			if (!updatedModel.transferOrder && event.payload) {
				event.payload.transferOrderId = null;
			}
		} else if (event.payload?.transferOrderId !== undefined) {
			event.payload.transferOrder = null;
			updatedModel.transferOrder = null;
		}
