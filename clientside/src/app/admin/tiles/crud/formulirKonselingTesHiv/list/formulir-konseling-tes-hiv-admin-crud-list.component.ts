...
import * as modelAction from 'src/app/models/formulirKonselingTesHiv/formulir_konseling_tes_hiv.model.action';
import {FormulirKonselingTesHivModelState} from 'src/app/models/formulirKonselingTesHiv/formulir_konseling_tes_hiv.model.state';
...
import {
	getFormulirKonselingTesHivCollectionModels,
	getFormulirKonselingTesHivCollectionCount
...
import {CrudTileMode} from '../formulir-konseling-tes-hiv-admin-crud.component';
...
@Component({
	selector: 'cb-formulir-konseling-tes-hiv-admin-crud-list',
	templateUrl: './formulir-konseling-tes-hiv-admin-crud-list.component.html',
	styleUrls: [
		'./formulir-konseling-tes-hiv-admin-crud-list.component.scss',
...
export class AdminFormulirKonselingTesHivCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<FormulirKonselingTesHivModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...FormulirKonselingTesHivModel.searchFields,
	...
	/**
	 * List of Formulir Konseling Tes Hivs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	formulirKonselingTesHivs: Observable<FormulirKonselingTesHivModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	formulirKonselingTesHivsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = FormulirKonselingTesHivModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = FormulirKonselingTesHivModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'e2faa9b2-d176-4f2d-82a3-08c08ac96832';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.formulirKonselingTesHivsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.formulirKonselingTesHivsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: FormulirKonselingTesHivModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FormulirKonselingTesHivAction(
				modelAction.FormulirKonselingTesHivModelActionTypes.INITIALISE_FORMULIR_KONSELING_TES_HIV_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FormulirKonselingTesHivAction(
				modelAction.FormulirKonselingTesHivModelActionTypes.FETCH_FORMULIR_KONSELING_TES_HIV_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.FormulirKonselingTesHivAction(
						modelAction.FormulirKonselingTesHivModelActionTypes.COUNT_FORMULIR_KONSELING_TES_HIVS,
						null
					)
				]
			))
		...
		this.formulirKonselingTesHivs = this.store.select(getFormulirKonselingTesHivCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getFormulirKonselingTesHivCollectionCount, this.collectionId).subscribe(
				(count) => this.formulirKonselingTesHivsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('formulir-konseling-tes-hiv');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: FormulirKonselingTesHivModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.FormulirKonselingTesHivModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.FormulirKonselingTesHivModelActionTypes.EXPORT_ALL_FORMULIR_KONSELING_TES_HIV;
		} else {
			actionType = modelAction.FormulirKonselingTesHivModelActionTypes.EXPORT_FORMULIR_KONSELING_TES_HIV_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.FormulirKonselingTesHivAction(
				modelAction.FormulirKonselingTesHivModelActionTypes.IMPORT_FORMULIR_KONSELING_TES_HIVS,
				{
					file: file,
				},
				[new modelAction.FormulirKonselingTesHivAction(
			...
			window.open(environment.API_URL + '/api/formulirkonselingteshiv/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: FormulirKonselingTesHivModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Formulir Konseling Tes Hiv entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Formulir Konseling Tes Hiv entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.FormulirKonselingTesHivModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.FormulirKonselingTesHivModelActionTypes.DELETE_FORMULIR_KONSELING_TES_HIV;
						} else if (usingSelected) {
							actionType = modelAction.FormulirKonselingTesHivModelActionTypes.DELETE_ALL_FORMULIR_KONSELING_TES_HIV;
						} else {
							actionType = modelAction.FormulirKonselingTesHivModelActionTypes.DELETE_FORMULIR_KONSELING_TES_HIV_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.FormulirKonselingTesHivAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_FORMULIR_KONSELING_TES_HIV when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_FORMULIR_KONSELING_TES_HIV and DELETE_FORMULIR_KONSELING_TES_HIV_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<FormulirKonselingTesHivModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = FormulirKonselingTesHivModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.formulirKonselingTesHivsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
