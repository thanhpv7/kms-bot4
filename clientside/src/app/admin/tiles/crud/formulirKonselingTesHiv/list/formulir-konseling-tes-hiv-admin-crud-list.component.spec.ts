...
import {AdminFormulirKonselingTesHivCrudListComponent} from './formulir-konseling-tes-hiv-admin-crud-list.component';
...
import {AdminFormulirKonselingTesHivCrudModule} from '../formulir-konseling-tes-hiv-admin-crud.module';
import {
	FormulirKonselingTesHivModelState,
	initialState as initialFormulirKonselingTesHivModelState
} from 'src/app/models/formulirKonselingTesHiv/formulir_konseling_tes_hiv.model.state';
 ...
 */
describe('Formulir Konseling Tes Hiv Crud List Tile Component', () => {

	let fixture;
	let formulirKonselingTesHivCrudListComponent: AdminFormulirKonselingTesHivCrudListComponent;

	let store: MockStore<{ model: FormulirKonselingTesHivModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let formulirKonselingTesHivModelState: FormulirKonselingTesHivModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<FormulirKonselingTesHivModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e2faa9b2-d176-4f2d-82a3-08c08ac96832';
	const searchContent = 'text';

	const searchableFieldCount = FormulirKonselingTesHivModel.searchFields.length;

	const routerState: RouterState = {
		url: 'formulir-konseling-tes-hiv-admin-crud',
		urls: ['formulir-konseling-tes-hiv-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const formulirKonselingTesHivDataFactory = new FormulirKonselingTesHivDataFactory();

	const defaultDataSize = 30;
	const defaultData = formulirKonselingTesHivDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: FormulirKonselingTesHivModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		formulirKonselingTesHivModelState = initialFormulirKonselingTesHivModelState;
		store.setState({model: formulirKonselingTesHivModelState});
		...
		spyOn(store, 'select')
			.withArgs(getFormulirKonselingTesHivCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getFormulirKonselingTesHivCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminFormulirKonselingTesHivCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminFormulirKonselingTesHivCrudListComponent);
			formulirKonselingTesHivCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(formulirKonselingTesHivCrudListComponent.formulirKonselingTesHivsCount).toBe(defaultData.length);
			...
			expect(formulirKonselingTesHivCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			formulirKonselingTesHivCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = formulirKonselingTesHivDataFactory.createAll(filteredDataSize);
			...
			expect(formulirKonselingTesHivCrudListComponent.formulirKonselingTesHivsCount).toBe(filteredDataSize);
			expect(formulirKonselingTesHivCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(formulirKonselingTesHivCrudListComponent.pageIndex).toBe(0);
			expect(formulirKonselingTesHivCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			formulirKonselingTesHivCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		formulirKonselingTesHivCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		formulirKonselingTesHivCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
