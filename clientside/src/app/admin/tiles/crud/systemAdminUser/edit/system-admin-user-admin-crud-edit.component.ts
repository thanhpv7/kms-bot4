...
import {SystemAdminUserModelState, SystemAdminUserModelAudit} from 'src/app/models/systemAdminUser/system_admin_user.model.state';
import {SystemAdminUserModel} from 'src/app/models/systemAdminUser/system_admin_user.model';
import {
	getSystemAdminUserModelAuditsByEntityId,
	getSystemAdminUserModelWithId,
	getCountSystemAdminUserModels,
...
@Component({
	selector: 'cb-system-admin-user-admin-crud-edit',
	templateUrl: './system-admin-user-admin-crud-edit.component.html',
	styleUrls: [
		'./system-admin-user-admin-crud-edit.component.scss',
...
export class AdminSystemAdminUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	systemAdminUserAudits$: Observable<SystemAdminUserModelAudit[]>;
	...
	staffOppositeRelation = 'systemAdminUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SystemAdminUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: SystemAdminUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSystemAdminUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new SystemAdminUserModel();
		...
		if (this.targetModelId) {
			this.systemAdminUserAudits$ = this.store.select(getSystemAdminUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SystemAdminUserModel> = {
		...
		this.store.dispatch(new modelAction.SystemAdminUserAction(
			modelAction.SystemAdminUserModelActionTypes.FETCH_SYSTEM_ADMIN_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSystemAdminUserModelWithId, this.targetModelId)
			...
			model = new SystemAdminUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SystemAdminUserAction(
			modelAction.SystemAdminUserModelActionTypes.FETCH_SYSTEM_ADMIN_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.SystemAdminUserAction(
				modelAction.SystemAdminUserModelActionTypes.CREATE_SYSTEM_ADMIN_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SystemAdminUserAction(
				modelAction.SystemAdminUserModelActionTypes.UPDATE_SYSTEM_ADMIN_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SystemAdminUserModel = new SystemAdminUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSystemAdminUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SystemAdminUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SystemAdminUserModel.getProps();
		this.modelRelations = SystemAdminUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: SystemAdminUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SystemAdminUserModel {
		let updatedModel: SystemAdminUserModel = new SystemAdminUserModel(model);
