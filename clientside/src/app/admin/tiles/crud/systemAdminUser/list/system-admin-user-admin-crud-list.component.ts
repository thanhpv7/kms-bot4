...
import * as modelAction from 'src/app/models/systemAdminUser/system_admin_user.model.action';
import {SystemAdminUserModelState} from 'src/app/models/systemAdminUser/system_admin_user.model.state';
...
import {
	getSystemAdminUserCollectionModels,
	getSystemAdminUserCollectionCount
...
import {CrudTileMode} from '../system-admin-user-admin-crud.component';
...
@Component({
	selector: 'cb-system-admin-user-admin-crud-list',
	templateUrl: './system-admin-user-admin-crud-list.component.html',
	styleUrls: [
		'./system-admin-user-admin-crud-list.component.scss',
...
export class AdminSystemAdminUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SystemAdminUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SystemAdminUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = SystemAdminUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ff652b5e-88e0-4130-be5a-5798bf28528f';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.systemAdminUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.systemAdminUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: SystemAdminUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SystemAdminUserAction(
				modelAction.SystemAdminUserModelActionTypes.INITIALISE_SYSTEM_ADMIN_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SystemAdminUserAction(
				modelAction.SystemAdminUserModelActionTypes.FETCH_SYSTEM_ADMIN_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.SystemAdminUserAction(
						modelAction.SystemAdminUserModelActionTypes.COUNT_SYSTEM_ADMIN_USERS,
						null
					)
				]
			))
		...
		this.systemAdminUsers = this.store.select(getSystemAdminUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSystemAdminUserCollectionCount, this.collectionId).subscribe(
				(count) => this.systemAdminUsersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('admin');
			commands.push('users');
			commands.push('system-admin-user');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SystemAdminUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SystemAdminUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SystemAdminUserModelActionTypes.EXPORT_ALL_SYSTEM_ADMIN_USER;
		} else {
			actionType = modelAction.SystemAdminUserModelActionTypes.EXPORT_SYSTEM_ADMIN_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SystemAdminUserAction(
				modelAction.SystemAdminUserModelActionTypes.IMPORT_SYSTEM_ADMIN_USERS,
				{
					file: file,
				},
				[new modelAction.SystemAdminUserAction(
			...
			window.open(environment.API_URL + '/api/systemadminuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SystemAdminUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected System Admin User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected System Admin User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SystemAdminUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SystemAdminUserModelActionTypes.DELETE_SYSTEM_ADMIN_USER;
						} else if (usingSelected) {
							actionType = modelAction.SystemAdminUserModelActionTypes.DELETE_ALL_SYSTEM_ADMIN_USER;
						} else {
							actionType = modelAction.SystemAdminUserModelActionTypes.DELETE_SYSTEM_ADMIN_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SystemAdminUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SYSTEM_ADMIN_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SYSTEM_ADMIN_USER and DELETE_SYSTEM_ADMIN_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<SystemAdminUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SystemAdminUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.systemAdminUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
