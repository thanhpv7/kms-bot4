...
import {AdminSystemAdminUserCrudListComponent} from './system-admin-user-admin-crud-list.component';
...
import {AdminSystemAdminUserCrudModule} from '../system-admin-user-admin-crud.module';
import {
	SystemAdminUserModelState,
	initialState as initialSystemAdminUserModelState
} from 'src/app/models/systemAdminUser/system_admin_user.model.state';
 ...
 */
describe('System Admin User Crud List Tile Component', () => {

	let fixture;
	let systemAdminUserCrudListComponent: AdminSystemAdminUserCrudListComponent;

	let store: MockStore<{ model: SystemAdminUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let systemAdminUserModelState: SystemAdminUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<SystemAdminUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ff652b5e-88e0-4130-be5a-5798bf28528f';
	const searchContent = 'text';

	const searchableFieldCount = SystemAdminUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'system-admin-user-admin-crud',
		urls: ['system-admin-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const systemAdminUserDataFactory = new SystemAdminUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = systemAdminUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: SystemAdminUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		systemAdminUserModelState = initialSystemAdminUserModelState;
		store.setState({model: systemAdminUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getSystemAdminUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getSystemAdminUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminSystemAdminUserCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminSystemAdminUserCrudListComponent);
			systemAdminUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(systemAdminUserCrudListComponent.systemAdminUsersCount).toBe(defaultData.length);
			...
			expect(systemAdminUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			systemAdminUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = systemAdminUserDataFactory.createAll(filteredDataSize);
			...
			expect(systemAdminUserCrudListComponent.systemAdminUsersCount).toBe(filteredDataSize);
			expect(systemAdminUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(systemAdminUserCrudListComponent.pageIndex).toBe(0);
			expect(systemAdminUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			systemAdminUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		systemAdminUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		systemAdminUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
