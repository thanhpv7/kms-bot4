...
import * as modelAction from 'src/app/models/diagnosticExaminationResults/diagnostic_examination_results.model.action';
import {DiagnosticExaminationResultsModelState, DiagnosticExaminationResultsModelAudit} from 'src/app/models/diagnosticExaminationResults/diagnostic_examination_results.model.state';
...
import {
	getDiagnosticExaminationResultsModelAuditsByEntityId,
	getDiagnosticExaminationResultsModelWithId,
	getCountDiagnosticExaminationResultsModels,
...
import {CrudTileMode} from '../diagnostic-examination-results-admin-crud.component';
...
@Component({
	selector: 'cb-diagnostic-examination-results-admin-crud-edit',
	templateUrl: './diagnostic-examination-results-admin-crud-edit.component.html',
	styleUrls: [
		'./diagnostic-examination-results-admin-crud-edit.component.scss',
...
export class AdminDiagnosticExaminationResultsCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'ac64e185-6f5d-40e8-a19d-b6a49a31683c';
	 ...
	 */
	diagnosticExaminationResultsAudits$: Observable<DiagnosticExaminationResultsModelAudit[]>;
	...
	examinationItemMultiCrudActive: boolean = false;
	examinationItemDisplayName = 'Examination Item';
	...
	examinationItemOppositeRelation = 'diagnosticExaminationResults';
	examinationItemRelations: ModelRelation[] = this.examinationItemMultiCrudActive ? ExaminationItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.examinationItemOppositeRelation) {
	...
	registrationOppositeRelation = 'diagnosticExaminationResults';
	 ...
	 */
	modelProperties: ModelProperty[] = DiagnosticExaminationResultsModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = DiagnosticExaminationResultsModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DiagnosticExaminationResultsModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'examinationItem' && this.examinationItemMultiCrudActive) {
	 ...
	 */
	targetModel: DiagnosticExaminationResultsModel;
	...
	constructor(
		private readonly store: Store<{ model: DiagnosticExaminationResultsModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDiagnosticExaminationResultsModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DiagnosticExaminationResultsModel();
		...
		if (this.targetModelId) {
			this.diagnosticExaminationResultsAudits$ = this.store.select(getDiagnosticExaminationResultsModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DiagnosticExaminationResultsModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
			modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_DIAGNOSTIC_EXAMINATION_RESULTS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDiagnosticExaminationResultsModelWithId, this.targetModelId)
			...
			model = new DiagnosticExaminationResultsModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('diagnostic-examination-results');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
			modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_DIAGNOSTIC_EXAMINATION_RESULTS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<DiagnosticExaminationResultsModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.examinationItemMultiCrudActive || this.registrationMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.DiagnosticExaminationResultsAction(
						modelAction.DiagnosticExaminationResultsModelActionTypes.COUNT_DIAGNOSTIC_EXAMINATION_RESULTSS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
				modelAction.DiagnosticExaminationResultsModelActionTypes.CREATE_DIAGNOSTIC_EXAMINATION_RESULTS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
				modelAction.DiagnosticExaminationResultsModelActionTypes.UPDATE_DIAGNOSTIC_EXAMINATION_RESULTS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DiagnosticExaminationResultsModel = new DiagnosticExaminationResultsModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDiagnosticExaminationResultsModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DiagnosticExaminationResultsModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.examinationItemDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ExaminationItemModel.getProps(), this.examinationItemRelations, this.isViewOnly),
					new ExaminationItemModel(),
					ExaminationItemModel,
					this.examinationItemDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.examinationItemRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DiagnosticExaminationResultsModel.getProps();
		this.modelRelations = DiagnosticExaminationResultsModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'examinationItem';
		}).hideElement = this.examinationItemMultiCrudActive || this.modelRelations.find(rel => rel.id === 'examinationItem').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.examinationItemMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.examinationItemDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.examinationItem && this.targetModel.examinationItem.id && this.examinationItemMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				ExaminationItemModel,
				this.examinationItemOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.examinationItem.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getExaminationItemModelWithId, this.targetModel.examinationItemId).subscribe(examinationItem => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ExaminationItemModel.getProps(), this.examinationItemRelations, this.isViewOnly),
					examinationItem,
					ExaminationItemModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: DiagnosticExaminationResultsModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DiagnosticExaminationResultsModel {
		let updatedModel: DiagnosticExaminationResultsModel = new DiagnosticExaminationResultsModel(model);

		if (this.examinationItemMultiCrudActive) {
			updatedModel.examinationItem = event.additionalEntities.examinationItem ? event.additionalEntities.examinationItem : undefined;

			if (!updatedModel.examinationItem && event.payload) {
				event.payload.examinationItemId = null;
			}
		} else if (event.payload?.examinationItemId !== undefined) {
			event.payload.examinationItem = null;
			updatedModel.examinationItem = null;
		}
