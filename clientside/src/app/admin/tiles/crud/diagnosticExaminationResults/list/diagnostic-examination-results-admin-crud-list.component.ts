...
import {DiagnosticExaminationResultsModelState} from 'src/app/models/diagnosticExaminationResults/diagnostic_examination_results.model.state';
...
import {
	getDiagnosticExaminationResultsCollectionModels,
	getDiagnosticExaminationResultsCollectionCount
...
@Component({
	selector: 'cb-diagnostic-examination-results-admin-crud-list',
	templateUrl: './diagnostic-examination-results-admin-crud-list.component.html',
	styleUrls: [
		'./diagnostic-examination-results-admin-crud-list.component.scss',
...
export class AdminDiagnosticExaminationResultsCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DiagnosticExaminationResultsModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DiagnosticExaminationResultsModel.searchFields,
	...
	/**
	 * List of Diagnostic Examination Resultss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	diagnosticExaminationResultss: Observable<DiagnosticExaminationResultsModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	diagnosticExaminationResultssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.diagnosticExaminationResultssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.diagnosticExaminationResultssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
				modelAction.DiagnosticExaminationResultsModelActionTypes.INITIALISE_DIAGNOSTIC_EXAMINATION_RESULTS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
				modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_DIAGNOSTIC_EXAMINATION_RESULTS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.diagnosticExaminationResultss = this.store.select(getDiagnosticExaminationResultsCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDiagnosticExaminationResultsCollectionCount, this.collectionId).subscribe(
				(count) => this.diagnosticExaminationResultssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DiagnosticExaminationResultsModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DiagnosticExaminationResultsModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DiagnosticExaminationResultsModelActionTypes.EXPORT_ALL_DIAGNOSTIC_EXAMINATION_RESULTS;
		} else {
			actionType = modelAction.DiagnosticExaminationResultsModelActionTypes.EXPORT_DIAGNOSTIC_EXAMINATION_RESULTS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
				modelAction.DiagnosticExaminationResultsModelActionTypes.IMPORT_DIAGNOSTIC_EXAMINATION_RESULTSS,
				{
					file: file,
				},
				[new modelAction.DiagnosticExaminationResultsAction(
			...
			window.open(environment.API_URL + '/api/diagnosticexaminationresults/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DiagnosticExaminationResultsModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Examination Results entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Examination Results entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DiagnosticExaminationResultsModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DiagnosticExaminationResultsModelActionTypes.DELETE_DIAGNOSTIC_EXAMINATION_RESULTS;
						} else if (usingSelected) {
							actionType = modelAction.DiagnosticExaminationResultsModelActionTypes.DELETE_ALL_DIAGNOSTIC_EXAMINATION_RESULTS;
						} else {
							actionType = modelAction.DiagnosticExaminationResultsModelActionTypes.DELETE_DIAGNOSTIC_EXAMINATION_RESULTS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DiagnosticExaminationResultsAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DIAGNOSTIC_EXAMINATION_RESULTS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DIAGNOSTIC_EXAMINATION_RESULTS and DELETE_DIAGNOSTIC_EXAMINATION_RESULTS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DiagnosticExaminationResultsModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.diagnosticExaminationResultssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
