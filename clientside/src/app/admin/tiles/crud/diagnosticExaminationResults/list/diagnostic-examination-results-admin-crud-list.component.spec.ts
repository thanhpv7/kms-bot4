...
import {AdminDiagnosticExaminationResultsCrudListComponent} from './diagnostic-examination-results-admin-crud-list.component';
...
import {AdminDiagnosticExaminationResultsCrudModule} from '../diagnostic-examination-results-admin-crud.module';
import {
	DiagnosticExaminationResultsModelState,
	initialState as initialDiagnosticExaminationResultsModelState
} from 'src/app/models/diagnosticExaminationResults/diagnostic_examination_results.model.state';
 ...
 */
describe('Diagnostic Examination Results Crud List Tile Component', () => {

	let fixture;
	let diagnosticExaminationResultsCrudListComponent: AdminDiagnosticExaminationResultsCrudListComponent;

	let store: MockStore<{ model: DiagnosticExaminationResultsModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let diagnosticExaminationResultsModelState: DiagnosticExaminationResultsModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<DiagnosticExaminationResultsModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ac64e185-6f5d-40e8-a19d-b6a49a31683c';
	const searchContent = 'text';

	const searchableFieldCount = DiagnosticExaminationResultsModel.searchFields.length;

	const routerState: RouterState = {
		url: 'diagnostic-examination-results-admin-crud',
		urls: ['diagnostic-examination-results-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const diagnosticExaminationResultsDataFactory = new DiagnosticExaminationResultsDataFactory();

	const defaultDataSize = 30;
	const defaultData = diagnosticExaminationResultsDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: DiagnosticExaminationResultsModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		diagnosticExaminationResultsModelState = initialDiagnosticExaminationResultsModelState;
		store.setState({model: diagnosticExaminationResultsModelState});
		...
		spyOn(store, 'select')
			.withArgs(getDiagnosticExaminationResultsCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDiagnosticExaminationResultsCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminDiagnosticExaminationResultsCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminDiagnosticExaminationResultsCrudListComponent);
			diagnosticExaminationResultsCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(diagnosticExaminationResultsCrudListComponent.diagnosticExaminationResultssCount).toBe(defaultData.length);
			...
			expect(diagnosticExaminationResultsCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			diagnosticExaminationResultsCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = diagnosticExaminationResultsDataFactory.createAll(filteredDataSize);
			...
			expect(diagnosticExaminationResultsCrudListComponent.diagnosticExaminationResultssCount).toBe(filteredDataSize);
			expect(diagnosticExaminationResultsCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(diagnosticExaminationResultsCrudListComponent.pageIndex).toBe(0);
			expect(diagnosticExaminationResultsCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			diagnosticExaminationResultsCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		diagnosticExaminationResultsCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		diagnosticExaminationResultsCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
