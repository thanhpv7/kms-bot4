...
import * as modelAction from 'src/app/models/batchStockBalancePerDate/batch_stock_balance_per_date.model.action';
import {BatchStockBalancePerDateModelState} from 'src/app/models/batchStockBalancePerDate/batch_stock_balance_per_date.model.state';
...
import {CrudTileMode} from '../batch-stock-balance-per-date-admin-crud.component';
...
@Component({
	selector: 'cb-batch-stock-balance-per-date-admin-crud-list',
	templateUrl: './batch-stock-balance-per-date-admin-crud-list.component.html',
	styleUrls: [
		'./batch-stock-balance-per-date-admin-crud-list.component.scss',
...
export class AdminBatchStockBalancePerDateCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BatchStockBalancePerDateModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BatchStockBalancePerDateModel.searchFields,
	...
	/**
	 * List of Batch Stock Balance Per Dates that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	batchStockBalancePerDates: Observable<BatchStockBalancePerDateModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	batchStockBalancePerDatesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BatchStockBalancePerDateModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BatchStockBalancePerDateModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '4cb2b2e4-7664-4208-b9cd-8e3d9dcca4bb';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.batchStockBalancePerDatesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.batchStockBalancePerDatesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BatchStockBalancePerDateModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
				modelAction.BatchStockBalancePerDateModelActionTypes.INITIALISE_BATCH_STOCK_BALANCE_PER_DATE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
				modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_BATCH_STOCK_BALANCE_PER_DATE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BatchStockBalancePerDateAction(
						modelAction.BatchStockBalancePerDateModelActionTypes.COUNT_BATCH_STOCK_BALANCE_PER_DATES,
						null
					)
				]
			))
		...
		this.batchStockBalancePerDates = this.store.select(getBatchStockBalancePerDateCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBatchStockBalancePerDateCollectionCount, this.collectionId).subscribe(
				(count) => this.batchStockBalancePerDatesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('batch-stock-balance-per-date');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BatchStockBalancePerDateModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BatchStockBalancePerDateModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BatchStockBalancePerDateModelActionTypes.EXPORT_ALL_BATCH_STOCK_BALANCE_PER_DATE;
		} else {
			actionType = modelAction.BatchStockBalancePerDateModelActionTypes.EXPORT_BATCH_STOCK_BALANCE_PER_DATE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
				modelAction.BatchStockBalancePerDateModelActionTypes.IMPORT_BATCH_STOCK_BALANCE_PER_DATES,
				{
					file: file,
				},
				[new modelAction.BatchStockBalancePerDateAction(
			...
			window.open(environment.API_URL + '/api/batchstockbalanceperdate/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BatchStockBalancePerDateModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Batch Stock Balance Per Date entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Batch Stock Balance Per Date entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BatchStockBalancePerDateModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BatchStockBalancePerDateModelActionTypes.DELETE_BATCH_STOCK_BALANCE_PER_DATE;
						} else if (usingSelected) {
							actionType = modelAction.BatchStockBalancePerDateModelActionTypes.DELETE_ALL_BATCH_STOCK_BALANCE_PER_DATE;
						} else {
							actionType = modelAction.BatchStockBalancePerDateModelActionTypes.DELETE_BATCH_STOCK_BALANCE_PER_DATE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BATCH_STOCK_BALANCE_PER_DATE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BATCH_STOCK_BALANCE_PER_DATE and DELETE_BATCH_STOCK_BALANCE_PER_DATE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BatchStockBalancePerDateModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BatchStockBalancePerDateModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.batchStockBalancePerDatesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
