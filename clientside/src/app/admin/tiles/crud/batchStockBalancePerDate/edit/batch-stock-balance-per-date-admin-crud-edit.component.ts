...
import {BatchStockBalancePerDateModelState, BatchStockBalancePerDateModelAudit} from 'src/app/models/batchStockBalancePerDate/batch_stock_balance_per_date.model.state';
import {BatchStockBalancePerDateModel} from 'src/app/models/batchStockBalancePerDate/batch_stock_balance_per_date.model';
import {
	getBatchStockBalancePerDateModelAuditsByEntityId,
	getBatchStockBalancePerDateModelWithId,
	getCountBatchStockBalancePerDateModels,
...
@Component({
	selector: 'cb-batch-stock-balance-per-date-admin-crud-edit',
	templateUrl: './batch-stock-balance-per-date-admin-crud-edit.component.html',
	styleUrls: [
		'./batch-stock-balance-per-date-admin-crud-edit.component.scss',
...
export class AdminBatchStockBalancePerDateCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	batchStockBalancePerDateAudits$: Observable<BatchStockBalancePerDateModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'batchStockBalances';
	...
	warehouseOppositeRelation = 'batchStockBalances';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BatchStockBalancePerDateModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BatchStockBalancePerDateModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBatchStockBalancePerDateModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BatchStockBalancePerDateModel();
		...
		if (this.targetModelId) {
			this.batchStockBalancePerDateAudits$ = this.store.select(getBatchStockBalancePerDateModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BatchStockBalancePerDateModel> = {
		...
		this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
			modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_BATCH_STOCK_BALANCE_PER_DATE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBatchStockBalancePerDateModelWithId, this.targetModelId)
			...
			model = new BatchStockBalancePerDateModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
			modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_BATCH_STOCK_BALANCE_PER_DATE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
				modelAction.BatchStockBalancePerDateModelActionTypes.CREATE_BATCH_STOCK_BALANCE_PER_DATE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BatchStockBalancePerDateAction(
				modelAction.BatchStockBalancePerDateModelActionTypes.UPDATE_BATCH_STOCK_BALANCE_PER_DATE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BatchStockBalancePerDateModel = new BatchStockBalancePerDateModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBatchStockBalancePerDateModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BatchStockBalancePerDateModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BatchStockBalancePerDateModel.getProps();
		this.modelRelations = BatchStockBalancePerDateModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BatchStockBalancePerDateModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BatchStockBalancePerDateModel {
		let updatedModel: BatchStockBalancePerDateModel = new BatchStockBalancePerDateModel(model);
