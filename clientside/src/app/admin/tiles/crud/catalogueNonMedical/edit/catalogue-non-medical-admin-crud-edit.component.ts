...
import {CatalogueNonMedicalModelState, CatalogueNonMedicalModelAudit} from 'src/app/models/catalogueNonMedical/catalogue_non_medical.model.state';
import {CatalogueNonMedicalModel} from 'src/app/models/catalogueNonMedical/catalogue_non_medical.model';
import {
	getCatalogueNonMedicalModelAuditsByEntityId,
	getCatalogueNonMedicalModelWithId,
	getCountCatalogueNonMedicalModels,
...
@Component({
	selector: 'cb-catalogue-non-medical-admin-crud-edit',
	templateUrl: './catalogue-non-medical-admin-crud-edit.component.html',
	styleUrls: [
		'./catalogue-non-medical-admin-crud-edit.component.scss',
...
export class AdminCatalogueNonMedicalCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	catalogueNonMedicalAudits$: Observable<CatalogueNonMedicalModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'stockCatalogueNonMedical';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CatalogueNonMedicalModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: CatalogueNonMedicalModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCatalogueNonMedicalModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CatalogueNonMedicalModel();
		...
		if (this.targetModelId) {
			this.catalogueNonMedicalAudits$ = this.store.select(getCatalogueNonMedicalModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CatalogueNonMedicalModel> = {
		...
		this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
			modelAction.CatalogueNonMedicalModelActionTypes.FETCH_CATALOGUE_NON_MEDICAL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCatalogueNonMedicalModelWithId, this.targetModelId)
			...
			model = new CatalogueNonMedicalModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
			modelAction.CatalogueNonMedicalModelActionTypes.FETCH_CATALOGUE_NON_MEDICAL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<CatalogueNonMedicalModel> = {
				...
				queryParams: {
					persistRelations: this.stockCatalogueMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
				modelAction.CatalogueNonMedicalModelActionTypes.CREATE_CATALOGUE_NON_MEDICAL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
				modelAction.CatalogueNonMedicalModelActionTypes.UPDATE_CATALOGUE_NON_MEDICAL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CatalogueNonMedicalModel = new CatalogueNonMedicalModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCatalogueNonMedicalModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CatalogueNonMedicalModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CatalogueNonMedicalModel.getProps();
		this.modelRelations = CatalogueNonMedicalModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: CatalogueNonMedicalModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CatalogueNonMedicalModel {
		let updatedModel: CatalogueNonMedicalModel = new CatalogueNonMedicalModel(model);
