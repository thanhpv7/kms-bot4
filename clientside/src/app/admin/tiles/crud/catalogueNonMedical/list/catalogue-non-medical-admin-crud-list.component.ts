...
import * as modelAction from 'src/app/models/catalogueNonMedical/catalogue_non_medical.model.action';
import {CatalogueNonMedicalModelState} from 'src/app/models/catalogueNonMedical/catalogue_non_medical.model.state';
...
import {
	getCatalogueNonMedicalCollectionModels,
	getCatalogueNonMedicalCollectionCount
...
import {CrudTileMode} from '../catalogue-non-medical-admin-crud.component';
...
@Component({
	selector: 'cb-catalogue-non-medical-admin-crud-list',
	templateUrl: './catalogue-non-medical-admin-crud-list.component.html',
	styleUrls: [
		'./catalogue-non-medical-admin-crud-list.component.scss',
...
export class AdminCatalogueNonMedicalCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CatalogueNonMedicalModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CatalogueNonMedicalModel.searchFields,
	...
	/**
	 * List of Catalogue Non Medicals that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	catalogueNonMedicals: Observable<CatalogueNonMedicalModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	catalogueNonMedicalsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = CatalogueNonMedicalModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = CatalogueNonMedicalModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '3e0bab90-0363-43ac-b237-fd008194b3b6';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.catalogueNonMedicalsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.catalogueNonMedicalsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: CatalogueNonMedicalModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
				modelAction.CatalogueNonMedicalModelActionTypes.INITIALISE_CATALOGUE_NON_MEDICAL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
				modelAction.CatalogueNonMedicalModelActionTypes.FETCH_CATALOGUE_NON_MEDICAL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.CatalogueNonMedicalAction(
						modelAction.CatalogueNonMedicalModelActionTypes.COUNT_CATALOGUE_NON_MEDICALS,
						null
					)
				]
			))
		...
		this.catalogueNonMedicals = this.store.select(getCatalogueNonMedicalCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCatalogueNonMedicalCollectionCount, this.collectionId).subscribe(
				(count) => this.catalogueNonMedicalsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('catalogue-non-medical');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CatalogueNonMedicalModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CatalogueNonMedicalModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CatalogueNonMedicalModelActionTypes.EXPORT_ALL_CATALOGUE_NON_MEDICAL;
		} else {
			actionType = modelAction.CatalogueNonMedicalModelActionTypes.EXPORT_CATALOGUE_NON_MEDICAL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
				modelAction.CatalogueNonMedicalModelActionTypes.IMPORT_CATALOGUE_NON_MEDICALS,
				{
					file: file,
				},
				[new modelAction.CatalogueNonMedicalAction(
			...
			window.open(environment.API_URL + '/api/cataloguenonmedical/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CatalogueNonMedicalModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Catalogue Non Medical entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Catalogue Non Medical entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CatalogueNonMedicalModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CatalogueNonMedicalModelActionTypes.DELETE_CATALOGUE_NON_MEDICAL;
						} else if (usingSelected) {
							actionType = modelAction.CatalogueNonMedicalModelActionTypes.DELETE_ALL_CATALOGUE_NON_MEDICAL;
						} else {
							actionType = modelAction.CatalogueNonMedicalModelActionTypes.DELETE_CATALOGUE_NON_MEDICAL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CatalogueNonMedicalAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CATALOGUE_NON_MEDICAL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CATALOGUE_NON_MEDICAL and DELETE_CATALOGUE_NON_MEDICAL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<CatalogueNonMedicalModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CatalogueNonMedicalModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.catalogueNonMedicalsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
