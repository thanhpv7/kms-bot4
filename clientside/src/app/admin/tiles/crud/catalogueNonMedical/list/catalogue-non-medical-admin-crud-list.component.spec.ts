...
import {AdminCatalogueNonMedicalCrudListComponent} from './catalogue-non-medical-admin-crud-list.component';
...
import {AdminCatalogueNonMedicalCrudModule} from '../catalogue-non-medical-admin-crud.module';
import {
	CatalogueNonMedicalModelState,
	initialState as initialCatalogueNonMedicalModelState
} from 'src/app/models/catalogueNonMedical/catalogue_non_medical.model.state';
 ...
 */
describe('Catalogue Non Medical Crud List Tile Component', () => {

	let fixture;
	let catalogueNonMedicalCrudListComponent: AdminCatalogueNonMedicalCrudListComponent;

	let store: MockStore<{ model: CatalogueNonMedicalModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let catalogueNonMedicalModelState: CatalogueNonMedicalModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<CatalogueNonMedicalModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '3e0bab90-0363-43ac-b237-fd008194b3b6';
	const searchContent = 'text';

	const searchableFieldCount = CatalogueNonMedicalModel.searchFields.length;

	const routerState: RouterState = {
		url: 'catalogue-non-medical-admin-crud',
		urls: ['catalogue-non-medical-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const catalogueNonMedicalDataFactory = new CatalogueNonMedicalDataFactory();

	const defaultDataSize = 30;
	const defaultData = catalogueNonMedicalDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: CatalogueNonMedicalModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		catalogueNonMedicalModelState = initialCatalogueNonMedicalModelState;
		store.setState({model: catalogueNonMedicalModelState});
		...
		spyOn(store, 'select')
			.withArgs(getCatalogueNonMedicalCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCatalogueNonMedicalCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminCatalogueNonMedicalCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminCatalogueNonMedicalCrudListComponent);
			catalogueNonMedicalCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(catalogueNonMedicalCrudListComponent.catalogueNonMedicalsCount).toBe(defaultData.length);
			...
			expect(catalogueNonMedicalCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			catalogueNonMedicalCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = catalogueNonMedicalDataFactory.createAll(filteredDataSize);
			...
			expect(catalogueNonMedicalCrudListComponent.catalogueNonMedicalsCount).toBe(filteredDataSize);
			expect(catalogueNonMedicalCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(catalogueNonMedicalCrudListComponent.pageIndex).toBe(0);
			expect(catalogueNonMedicalCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			catalogueNonMedicalCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		catalogueNonMedicalCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		catalogueNonMedicalCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
