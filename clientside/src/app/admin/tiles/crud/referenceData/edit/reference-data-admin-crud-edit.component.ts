...
import {ReferenceDataModelState, ReferenceDataModelAudit} from 'src/app/models/referenceData/reference_data.model.state';
import {ReferenceDataModel} from 'src/app/models/referenceData/reference_data.model';
import {
	getReferenceDataModelAuditsByEntityId,
...
@Component({
	selector: 'cb-reference-data-admin-crud-edit',
	templateUrl: './reference-data-admin-crud-edit.component.html',
	styleUrls: [
		'./reference-data-admin-crud-edit.component.scss',
...
export class AdminReferenceDataCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	referenceDataAudits$: Observable<ReferenceDataModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ReferenceDataModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ReferenceDataModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountReferenceDataModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ReferenceDataModel();
		...
		if (this.targetModelId) {
			this.referenceDataAudits$ = this.store.select(getReferenceDataModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ReferenceDataModel> = {
		...
		this.store.dispatch(new modelAction.ReferenceDataAction(
			modelAction.ReferenceDataModelActionTypes.FETCH_REFERENCE_DATA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getReferenceDataModelWithId, this.targetModelId)
			...
			model = new ReferenceDataModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ReferenceDataAction(
			modelAction.ReferenceDataModelActionTypes.FETCH_REFERENCE_DATA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.ReferenceDataAction(
				modelAction.ReferenceDataModelActionTypes.CREATE_REFERENCE_DATA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ReferenceDataAction(
				modelAction.ReferenceDataModelActionTypes.UPDATE_REFERENCE_DATA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ReferenceDataModel = new ReferenceDataModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getReferenceDataModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ReferenceDataModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ReferenceDataModel.getProps();
		this.modelRelations = ReferenceDataModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ReferenceDataModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ReferenceDataModel {
		let updatedModel: ReferenceDataModel = new ReferenceDataModel(model);
