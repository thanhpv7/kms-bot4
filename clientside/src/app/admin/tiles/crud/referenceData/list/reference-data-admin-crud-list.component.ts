...
import * as modelAction from 'src/app/models/referenceData/reference_data.model.action';
import {ReferenceDataModelState} from 'src/app/models/referenceData/reference_data.model.state';
...
import {
	...
	getReferenceDataCollectionCount
...
import {CrudTileMode} from '../reference-data-admin-crud.component';
...
@Component({
	selector: 'cb-reference-data-admin-crud-list',
	templateUrl: './reference-data-admin-crud-list.component.html',
	styleUrls: [
		'./reference-data-admin-crud-list.component.scss',
...
export class AdminReferenceDataCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ReferenceDataModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ReferenceDataModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = ReferenceDataModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '5fea000f-abbc-4491-8c77-61dca0723557';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.referenceDatasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.referenceDatasCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ReferenceDataModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReferenceDataAction(
				modelAction.ReferenceDataModelActionTypes.INITIALISE_REFERENCE_DATA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReferenceDataAction(
				modelAction.ReferenceDataModelActionTypes.FETCH_REFERENCE_DATA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ReferenceDataAction(
						modelAction.ReferenceDataModelActionTypes.COUNT_REFERENCE_DATAS,
						null
					)
				]
			))
		...
		this.referenceDatas = this.store.select(getReferenceDataCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getReferenceDataCollectionCount, this.collectionId).subscribe(
				(count) => this.referenceDatasCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ReferenceDataModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ReferenceDataModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ReferenceDataModelActionTypes.EXPORT_ALL_REFERENCE_DATA;
		} else {
			actionType = modelAction.ReferenceDataModelActionTypes.EXPORT_REFERENCE_DATA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ReferenceDataAction(
				modelAction.ReferenceDataModelActionTypes.IMPORT_REFERENCE_DATAS,
				{
					file: file,
				},
				[new modelAction.ReferenceDataAction(
			...
			window.open(environment.API_URL + '/api/referencedata/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ReferenceDataModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Reference Data entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Reference Data entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ReferenceDataModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ReferenceDataModelActionTypes.DELETE_REFERENCE_DATA;
						} else if (usingSelected) {
							actionType = modelAction.ReferenceDataModelActionTypes.DELETE_ALL_REFERENCE_DATA;
						} else {
							actionType = modelAction.ReferenceDataModelActionTypes.DELETE_REFERENCE_DATA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ReferenceDataAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_REFERENCE_DATA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_REFERENCE_DATA and DELETE_REFERENCE_DATA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ReferenceDataModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ReferenceDataModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.referenceDatasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
