...
import {AdminReferenceDataCrudListComponent} from './reference-data-admin-crud-list.component';
...
import {AdminReferenceDataCrudModule} from '../reference-data-admin-crud.module';
import {
 ...
 */
describe('Reference Data Crud List Tile Component', () => {

	let fixture;
	let referenceDataCrudListComponent: AdminReferenceDataCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ReferenceDataModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5fea000f-abbc-4491-8c77-61dca0723557';
	const searchContent = 'text';

	const searchableFieldCount = ReferenceDataModel.searchFields.length;

	const routerState: RouterState = {
		url: 'reference-data-admin-crud',
		urls: ['reference-data-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const referenceDataDataFactory = new ReferenceDataDataFactory();

	const defaultDataSize = 30;
	const defaultData = referenceDataDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getReferenceDataCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getReferenceDataCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminReferenceDataCrudListComponent);
			referenceDataCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(referenceDataCrudListComponent.referenceDatasCount).toBe(defaultData.length);
			...
			expect(referenceDataCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			referenceDataCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = referenceDataDataFactory.createAll(filteredDataSize);
			...
			expect(referenceDataCrudListComponent.referenceDatasCount).toBe(filteredDataSize);
			expect(referenceDataCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(referenceDataCrudListComponent.pageIndex).toBe(0);
			expect(referenceDataCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			referenceDataCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		referenceDataCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		referenceDataCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
