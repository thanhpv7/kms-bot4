...
import * as modelAction from 'src/app/models/vaccinationOrder/vaccination_order.model.action';
import {VaccinationOrderModelState} from 'src/app/models/vaccinationOrder/vaccination_order.model.state';
...
import {
	getVaccinationOrderCollectionModels,
	getVaccinationOrderCollectionCount
...
import {CrudTileMode} from '../vaccination-order-admin-crud.component';
...
@Component({
	selector: 'cb-vaccination-order-admin-crud-list',
	templateUrl: './vaccination-order-admin-crud-list.component.html',
	styleUrls: [
		'./vaccination-order-admin-crud-list.component.scss',
...
export class AdminVaccinationOrderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<VaccinationOrderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...VaccinationOrderModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = VaccinationOrderModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '37f7b154-ae93-4b79-a808-6eb2ec69b49c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.vaccinationOrdersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.vaccinationOrdersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: VaccinationOrderModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.VaccinationOrderAction(
				modelAction.VaccinationOrderModelActionTypes.INITIALISE_VACCINATION_ORDER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.VaccinationOrderAction(
				modelAction.VaccinationOrderModelActionTypes.FETCH_VACCINATION_ORDER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.VaccinationOrderAction(
						modelAction.VaccinationOrderModelActionTypes.COUNT_VACCINATION_ORDERS,
						null
					)
				]
			))
		...
		this.vaccinationOrders = this.store.select(getVaccinationOrderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getVaccinationOrderCollectionCount, this.collectionId).subscribe(
				(count) => this.vaccinationOrdersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('vaccination-order');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: VaccinationOrderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.VaccinationOrderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.VaccinationOrderModelActionTypes.EXPORT_ALL_VACCINATION_ORDER;
		} else {
			actionType = modelAction.VaccinationOrderModelActionTypes.EXPORT_VACCINATION_ORDER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.VaccinationOrderAction(
				modelAction.VaccinationOrderModelActionTypes.IMPORT_VACCINATION_ORDERS,
				{
					file: file,
				},
				[new modelAction.VaccinationOrderAction(
			...
			window.open(environment.API_URL + '/api/vaccinationorder/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: VaccinationOrderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Vaccination Order entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Vaccination Order entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.VaccinationOrderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.VaccinationOrderModelActionTypes.DELETE_VACCINATION_ORDER;
						} else if (usingSelected) {
							actionType = modelAction.VaccinationOrderModelActionTypes.DELETE_ALL_VACCINATION_ORDER;
						} else {
							actionType = modelAction.VaccinationOrderModelActionTypes.DELETE_VACCINATION_ORDER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.VaccinationOrderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_VACCINATION_ORDER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_VACCINATION_ORDER and DELETE_VACCINATION_ORDER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<VaccinationOrderModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = VaccinationOrderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.vaccinationOrdersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
