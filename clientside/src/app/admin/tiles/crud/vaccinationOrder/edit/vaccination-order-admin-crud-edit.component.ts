...
import {VaccinationOrderModelState, VaccinationOrderModelAudit} from 'src/app/models/vaccinationOrder/vaccination_order.model.state';
import {VaccinationOrderModel} from 'src/app/models/vaccinationOrder/vaccination_order.model';
import {
	getVaccinationOrderModelAuditsByEntityId,
	getVaccinationOrderModelWithId,
	getCountVaccinationOrderModels,
...
@Component({
	selector: 'cb-vaccination-order-admin-crud-edit',
	templateUrl: './vaccination-order-admin-crud-edit.component.html',
	styleUrls: [
		'./vaccination-order-admin-crud-edit.component.scss',
...
export class AdminVaccinationOrderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	vaccinationOrderAudits$: Observable<VaccinationOrderModelAudit[]>;
	...
	registrationOppositeRelation = 'vaccinationOrders';
	...
	staffOppositeRelation = 'vaccinationOrders';
	...
	vaccinationOrderDetailsOppositeRelation = 'vaccinationOrder';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = VaccinationOrderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: VaccinationOrderModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountVaccinationOrderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new VaccinationOrderModel();
		...
		if (this.targetModelId) {
			this.vaccinationOrderAudits$ = this.store.select(getVaccinationOrderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<VaccinationOrderModel> = {
		...
		this.store.dispatch(new modelAction.VaccinationOrderAction(
			modelAction.VaccinationOrderModelActionTypes.FETCH_VACCINATION_ORDER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getVaccinationOrderModelWithId, this.targetModelId)
			...
			model = new VaccinationOrderModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.VaccinationOrderAction(
			modelAction.VaccinationOrderModelActionTypes.FETCH_VACCINATION_ORDER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<VaccinationOrderModel> = {
				...
				queryParams: {
					persistRelations: this.registrationMultiCrudActive || this.staffMultiCrudActive || this.vaccinationOrderDetailsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.VaccinationOrderAction(
				modelAction.VaccinationOrderModelActionTypes.CREATE_VACCINATION_ORDER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.VaccinationOrderAction(
				modelAction.VaccinationOrderModelActionTypes.UPDATE_VACCINATION_ORDER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: VaccinationOrderModel = new VaccinationOrderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getVaccinationOrderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new VaccinationOrderModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = VaccinationOrderModel.getProps();
		this.modelRelations = VaccinationOrderModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: VaccinationOrderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): VaccinationOrderModel {
		let updatedModel: VaccinationOrderModel = new VaccinationOrderModel(model);
