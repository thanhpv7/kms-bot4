...
import {AdminPcareStatusPulangCrudListComponent} from './pcare-status-pulang-admin-crud-list.component';
...
import {AdminPcareStatusPulangCrudModule} from '../pcare-status-pulang-admin-crud.module';
import {
 ...
 */
describe('PCare Status Pulang Crud List Tile Component', () => {

	let fixture;
	let pcareStatusPulangCrudListComponent: AdminPcareStatusPulangCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareStatusPulangModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'fed1ce2b-9ac8-4b9e-94e4-50eeb9b7625d';
	const searchContent = 'text';

	const searchableFieldCount = PcareStatusPulangModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-status-pulang-admin-crud',
		urls: ['pcare-status-pulang-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareStatusPulangDataFactory = new PcareStatusPulangDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareStatusPulangDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPcareStatusPulangCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareStatusPulangCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPcareStatusPulangCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPcareStatusPulangCrudListComponent);
			pcareStatusPulangCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareStatusPulangCrudListComponent.pcareStatusPulangsCount).toBe(defaultData.length);
			...
			expect(pcareStatusPulangCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareStatusPulangCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareStatusPulangDataFactory.createAll(filteredDataSize);
			...
			expect(pcareStatusPulangCrudListComponent.pcareStatusPulangsCount).toBe(filteredDataSize);
			expect(pcareStatusPulangCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareStatusPulangCrudListComponent.pageIndex).toBe(0);
			expect(pcareStatusPulangCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareStatusPulangCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareStatusPulangCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareStatusPulangCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
