...
import * as modelAction from 'src/app/models/pcareStatusPulang/pcare_status_pulang.model.action';
import {PcareStatusPulangModelState} from 'src/app/models/pcareStatusPulang/pcare_status_pulang.model.state';
...
import {
	...
	getPcareStatusPulangCollectionCount
...
import {CrudTileMode} from '../pcare-status-pulang-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-status-pulang-admin-crud-list',
	templateUrl: './pcare-status-pulang-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-status-pulang-admin-crud-list.component.scss',
...
export class AdminPcareStatusPulangCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareStatusPulangModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareStatusPulangModel.searchFields,
	...
	/**
	 * List of PCare Status Pulangs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareStatusPulangs: Observable<PcareStatusPulangModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareStatusPulangsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PcareStatusPulangModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareStatusPulangModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'fed1ce2b-9ac8-4b9e-94e4-50eeb9b7625d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareStatusPulangsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareStatusPulangsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareStatusPulangModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareStatusPulangAction(
				modelAction.PcareStatusPulangModelActionTypes.INITIALISE_PCARE_STATUS_PULANG_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareStatusPulangAction(
				modelAction.PcareStatusPulangModelActionTypes.FETCH_PCARE_STATUS_PULANG_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareStatusPulangAction(
						modelAction.PcareStatusPulangModelActionTypes.COUNT_PCARE_STATUS_PULANGS,
						null
					)
				]
			))
		...
		this.pcareStatusPulangs = this.store.select(getPcareStatusPulangCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareStatusPulangCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareStatusPulangsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-status-pulang');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareStatusPulangModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareStatusPulangModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareStatusPulangModelActionTypes.EXPORT_ALL_PCARE_STATUS_PULANG;
		} else {
			actionType = modelAction.PcareStatusPulangModelActionTypes.EXPORT_PCARE_STATUS_PULANG_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareStatusPulangAction(
				modelAction.PcareStatusPulangModelActionTypes.IMPORT_PCARE_STATUS_PULANGS,
				{
					file: file,
				},
				[new modelAction.PcareStatusPulangAction(
			...
			window.open(environment.API_URL + '/api/pcarestatuspulang/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareStatusPulangModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Status Pulang entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Status Pulang entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareStatusPulangModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareStatusPulangModelActionTypes.DELETE_PCARE_STATUS_PULANG;
						} else if (usingSelected) {
							actionType = modelAction.PcareStatusPulangModelActionTypes.DELETE_ALL_PCARE_STATUS_PULANG;
						} else {
							actionType = modelAction.PcareStatusPulangModelActionTypes.DELETE_PCARE_STATUS_PULANG_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareStatusPulangAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_STATUS_PULANG when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_STATUS_PULANG and DELETE_PCARE_STATUS_PULANG_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareStatusPulangModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareStatusPulangModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareStatusPulangsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
