...
import {PcareStatusPulangModelState, PcareStatusPulangModelAudit} from 'src/app/models/pcareStatusPulang/pcare_status_pulang.model.state';
import {PcareStatusPulangModel} from 'src/app/models/pcareStatusPulang/pcare_status_pulang.model';
import {
	getPcareStatusPulangModelAuditsByEntityId,
...
@Component({
	selector: 'cb-pcare-status-pulang-admin-crud-edit',
	templateUrl: './pcare-status-pulang-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-status-pulang-admin-crud-edit.component.scss',
...
export class AdminPcareStatusPulangCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareStatusPulangAudits$: Observable<PcareStatusPulangModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareStatusPulangModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PcareStatusPulangModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareStatusPulangModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareStatusPulangModel();
		...
		if (this.targetModelId) {
			this.pcareStatusPulangAudits$ = this.store.select(getPcareStatusPulangModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareStatusPulangModel> = {
		...
		this.store.dispatch(new modelAction.PcareStatusPulangAction(
			modelAction.PcareStatusPulangModelActionTypes.FETCH_PCARE_STATUS_PULANG,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareStatusPulangModelWithId, this.targetModelId)
			...
			model = new PcareStatusPulangModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareStatusPulangAction(
			modelAction.PcareStatusPulangModelActionTypes.FETCH_PCARE_STATUS_PULANG_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareStatusPulangAction(
				modelAction.PcareStatusPulangModelActionTypes.CREATE_PCARE_STATUS_PULANG,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareStatusPulangAction(
				modelAction.PcareStatusPulangModelActionTypes.UPDATE_PCARE_STATUS_PULANG,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareStatusPulangModel = new PcareStatusPulangModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareStatusPulangModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareStatusPulangModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareStatusPulangModel.getProps();
		this.modelRelations = PcareStatusPulangModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareStatusPulangModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareStatusPulangModel {
		let updatedModel: PcareStatusPulangModel = new PcareStatusPulangModel(model);
