...
import {BpjsPcareRiwayatKunjunganModelState, BpjsPcareRiwayatKunjunganModelAudit} from 'src/app/models/bpjsPcareRiwayatKunjungan/bpjs_pcare_riwayat_kunjungan.model.state';
import {BpjsPcareRiwayatKunjunganModel} from 'src/app/models/bpjsPcareRiwayatKunjungan/bpjs_pcare_riwayat_kunjungan.model';
import {
	getBpjsPcareRiwayatKunjunganModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-pcare-riwayat-kunjungan-admin-crud-edit',
	templateUrl: './bpjs-pcare-riwayat-kunjungan-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-riwayat-kunjungan-admin-crud-edit.component.scss',
...
export class AdminBpjsPcareRiwayatKunjunganCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcareRiwayatKunjunganAudits$: Observable<BpjsPcareRiwayatKunjunganModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcareRiwayatKunjunganModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcareRiwayatKunjunganModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcareRiwayatKunjunganModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcareRiwayatKunjunganModel();
		...
		if (this.targetModelId) {
			this.bpjsPcareRiwayatKunjunganAudits$ = this.store.select(getBpjsPcareRiwayatKunjunganModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcareRiwayatKunjunganModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
			modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_BPJS_PCARE_RIWAYAT_KUNJUNGAN,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareRiwayatKunjunganModelWithId, this.targetModelId)
			...
			model = new BpjsPcareRiwayatKunjunganModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
			modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_BPJS_PCARE_RIWAYAT_KUNJUNGAN_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
				modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.CREATE_BPJS_PCARE_RIWAYAT_KUNJUNGAN,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
				modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.UPDATE_BPJS_PCARE_RIWAYAT_KUNJUNGAN,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcareRiwayatKunjunganModel = new BpjsPcareRiwayatKunjunganModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareRiwayatKunjunganModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcareRiwayatKunjunganModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcareRiwayatKunjunganModel.getProps();
		this.modelRelations = BpjsPcareRiwayatKunjunganModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcareRiwayatKunjunganModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcareRiwayatKunjunganModel {
		let updatedModel: BpjsPcareRiwayatKunjunganModel = new BpjsPcareRiwayatKunjunganModel(model);
