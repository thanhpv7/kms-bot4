...
import {AdminBpjsPcareRiwayatKunjunganCrudListComponent} from './bpjs-pcare-riwayat-kunjungan-admin-crud-list.component';
...
import {AdminBpjsPcareRiwayatKunjunganCrudModule} from '../bpjs-pcare-riwayat-kunjungan-admin-crud.module';
import {
 ...
 */
describe('Bpjs Pcare Riwayat Kunjungan Crud List Tile Component', () => {

	let fixture;
	let bpjsPcareRiwayatKunjunganCrudListComponent: AdminBpjsPcareRiwayatKunjunganCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPcareRiwayatKunjunganModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cf237e65-f614-4906-ae7e-11c4ee7b0980';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPcareRiwayatKunjunganModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-pcare-riwayat-kunjungan-admin-crud',
		urls: ['bpjs-pcare-riwayat-kunjungan-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPcareRiwayatKunjunganDataFactory = new BpjsPcareRiwayatKunjunganDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPcareRiwayatKunjunganDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPcareRiwayatKunjunganCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPcareRiwayatKunjunganCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsPcareRiwayatKunjunganCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsPcareRiwayatKunjunganCrudListComponent);
			bpjsPcareRiwayatKunjunganCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPcareRiwayatKunjunganCrudListComponent.bpjsPcareRiwayatKunjungansCount).toBe(defaultData.length);
			...
			expect(bpjsPcareRiwayatKunjunganCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPcareRiwayatKunjunganCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPcareRiwayatKunjunganDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPcareRiwayatKunjunganCrudListComponent.bpjsPcareRiwayatKunjungansCount).toBe(filteredDataSize);
			expect(bpjsPcareRiwayatKunjunganCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPcareRiwayatKunjunganCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPcareRiwayatKunjunganCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPcareRiwayatKunjunganCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPcareRiwayatKunjunganCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPcareRiwayatKunjunganCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
