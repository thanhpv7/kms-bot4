...
import * as modelAction from 'src/app/models/bpjsPcareRiwayatKunjungan/bpjs_pcare_riwayat_kunjungan.model.action';
import {BpjsPcareRiwayatKunjunganModelState} from 'src/app/models/bpjsPcareRiwayatKunjungan/bpjs_pcare_riwayat_kunjungan.model.state';
...
import {
	...
	getBpjsPcareRiwayatKunjunganCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-riwayat-kunjungan-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-riwayat-kunjungan-admin-crud-list',
	templateUrl: './bpjs-pcare-riwayat-kunjungan-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-riwayat-kunjungan-admin-crud-list.component.scss',
...
export class AdminBpjsPcareRiwayatKunjunganCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcareRiwayatKunjunganModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcareRiwayatKunjunganModel.searchFields,
	...
	/**
	 * List of Bpjs Pcare Riwayat Kunjungans that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcareRiwayatKunjungans: Observable<BpjsPcareRiwayatKunjunganModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcareRiwayatKunjungansCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcareRiwayatKunjunganModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcareRiwayatKunjunganModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'cf237e65-f614-4906-ae7e-11c4ee7b0980';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcareRiwayatKunjungansCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcareRiwayatKunjungansCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcareRiwayatKunjunganModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
				modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.INITIALISE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
				modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_BPJS_PCARE_RIWAYAT_KUNJUNGAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcareRiwayatKunjunganAction(
						modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.COUNT_BPJS_PCARE_RIWAYAT_KUNJUNGANS,
						null
					)
				]
			))
		...
		this.bpjsPcareRiwayatKunjungans = this.store.select(getBpjsPcareRiwayatKunjunganCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcareRiwayatKunjunganCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcareRiwayatKunjungansCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-riwayat-kunjungan');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcareRiwayatKunjunganModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcareRiwayatKunjunganModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.EXPORT_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN;
		} else {
			actionType = modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.EXPORT_BPJS_PCARE_RIWAYAT_KUNJUNGAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
				modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.IMPORT_BPJS_PCARE_RIWAYAT_KUNJUNGANS,
				{
					file: file,
				},
				[new modelAction.BpjsPcareRiwayatKunjunganAction(
			...
			window.open(environment.API_URL + '/api/bpjspcareriwayatkunjungan/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcareRiwayatKunjunganModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Bpjs Pcare Riwayat Kunjungan entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Bpjs Pcare Riwayat Kunjungan entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcareRiwayatKunjunganModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.DELETE_BPJS_PCARE_RIWAYAT_KUNJUNGAN;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.DELETE_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN;
						} else {
							actionType = modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.DELETE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcareRiwayatKunjunganAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_RIWAYAT_KUNJUNGAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN and DELETE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcareRiwayatKunjunganModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcareRiwayatKunjunganModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcareRiwayatKunjungansCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
