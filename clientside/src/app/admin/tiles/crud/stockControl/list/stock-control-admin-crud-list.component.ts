...
import {StockControlModelState} from 'src/app/models/stockControl/stock_control.model.state';
...
import {
	getStockControlCollectionModels,
	getStockControlCollectionCount
...
@Component({
	selector: 'cb-stock-control-admin-crud-list',
	templateUrl: './stock-control-admin-crud-list.component.html',
	styleUrls: [
		'./stock-control-admin-crud-list.component.scss',
...
export class AdminStockControlCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<StockControlModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...StockControlModel.searchFields,
	...
	/**
	 * List of Stock Controls that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	stockControls: Observable<StockControlModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	stockControlsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.stockControlsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.stockControlsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockControlAction(
				modelAction.StockControlModelActionTypes.INITIALISE_STOCK_CONTROL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockControlAction(
				modelAction.StockControlModelActionTypes.FETCH_STOCK_CONTROL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.stockControls = this.store.select(getStockControlCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getStockControlCollectionCount, this.collectionId).subscribe(
				(count) => this.stockControlsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: StockControlModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.StockControlModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.StockControlModelActionTypes.EXPORT_ALL_STOCK_CONTROL;
		} else {
			actionType = modelAction.StockControlModelActionTypes.EXPORT_STOCK_CONTROL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.StockControlAction(
				modelAction.StockControlModelActionTypes.IMPORT_STOCK_CONTROLS,
				{
					file: file,
				},
				[new modelAction.StockControlAction(
			...
			window.open(environment.API_URL + '/api/stockcontrol/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: StockControlModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Stock Control entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Stock Control entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.StockControlModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.StockControlModelActionTypes.DELETE_STOCK_CONTROL;
						} else if (usingSelected) {
							actionType = modelAction.StockControlModelActionTypes.DELETE_ALL_STOCK_CONTROL;
						} else {
							actionType = modelAction.StockControlModelActionTypes.DELETE_STOCK_CONTROL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.StockControlAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_STOCK_CONTROL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_STOCK_CONTROL and DELETE_STOCK_CONTROL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = StockControlModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.stockControlsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
