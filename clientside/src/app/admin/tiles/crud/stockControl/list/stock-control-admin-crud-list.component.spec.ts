...
import {AdminStockControlCrudListComponent} from './stock-control-admin-crud-list.component';
...
import {AdminStockControlCrudModule} from '../stock-control-admin-crud.module';
import {
	StockControlModelState,
	initialState as initialStockControlModelState
} from 'src/app/models/stockControl/stock_control.model.state';
 ...
 */
describe('Stock Control Crud List Tile Component', () => {

	let fixture;
	let stockControlCrudListComponent: AdminStockControlCrudListComponent;

	let store: MockStore<{ model: StockControlModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let stockControlModelState: StockControlModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<StockControlModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '3728c725-1c2d-4d29-a344-753545a92fb1';
	const searchContent = 'text';

	const searchableFieldCount = StockControlModel.searchFields.length;

	const routerState: RouterState = {
		url: 'stock-control-admin-crud',
		urls: ['stock-control-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const stockControlDataFactory = new StockControlDataFactory();

	const defaultDataSize = 30;
	const defaultData = stockControlDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: StockControlModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		stockControlModelState = initialStockControlModelState;
		store.setState({model: stockControlModelState});
		...
		spyOn(store, 'select')
			.withArgs(getStockControlCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getStockControlCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminStockControlCrudListComponent);
			stockControlCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(stockControlCrudListComponent.stockControlsCount).toBe(defaultData.length);
			...
			expect(stockControlCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			stockControlCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = stockControlDataFactory.createAll(filteredDataSize);
			...
			expect(stockControlCrudListComponent.stockControlsCount).toBe(filteredDataSize);
			expect(stockControlCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(stockControlCrudListComponent.pageIndex).toBe(0);
			expect(stockControlCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			stockControlCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		stockControlCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		stockControlCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
