...
import * as modelAction from 'src/app/models/stockControl/stock_control.model.action';
import {StockControlModelState, StockControlModelAudit} from 'src/app/models/stockControl/stock_control.model.state';
import {StockControlModel} from 'src/app/models/stockControl/stock_control.model';
import {
	getStockControlModelAuditsByEntityId,
	getStockControlModelWithId,
	getCountStockControlModels,
...
import {CrudTileMode} from '../stock-control-admin-crud.component';
...
@Component({
	selector: 'cb-stock-control-admin-crud-edit',
	templateUrl: './stock-control-admin-crud-edit.component.html',
	styleUrls: [
		'./stock-control-admin-crud-edit.component.scss',
...
export class AdminStockControlCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '3728c725-1c2d-4d29-a344-753545a92fb1';
	 ...
	 */
	stockControlAudits$: Observable<StockControlModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Location and Stock Controls here] off begin
	locationMultiCrudActive: boolean = false;
	locationDisplayName = 'Location';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Location and Stock Controls here] end
	locationOppositeRelation = 'stockControls';
	...
	stockCatalogueOppositeRelation = 'stockControls';
	...
	warehouseOppositeRelation = 'stockControls';
	 ...
	 */
	modelProperties: ModelProperty[] = StockControlModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = StockControlModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = StockControlModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	constructor(
		private readonly store: Store<{ model: StockControlModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountStockControlModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new StockControlModel();
		...
		if (this.targetModelId) {
			this.stockControlAudits$ = this.store.select(getStockControlModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<StockControlModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.StockControlAction(
			modelAction.StockControlModelActionTypes.FETCH_STOCK_CONTROL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getStockControlModelWithId, this.targetModelId)
			...
			model = new StockControlModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.StockControlAction(
			modelAction.StockControlModelActionTypes.FETCH_STOCK_CONTROL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<StockControlModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.StockControlAction(
						modelAction.StockControlModelActionTypes.COUNT_STOCK_CONTROLS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.StockControlAction(
				modelAction.StockControlModelActionTypes.CREATE_STOCK_CONTROL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.StockControlAction(
				modelAction.StockControlModelActionTypes.UPDATE_STOCK_CONTROL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: StockControlModel = new StockControlModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getStockControlModelWithId, this.targetModelId).subscribe(model => {
					let differences = new StockControlModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = StockControlModel.getProps();
		this.modelRelations = StockControlModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: StockControlModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): StockControlModel {
		let updatedModel: StockControlModel = new StockControlModel(model);
