...
import * as modelAction from 'src/app/models/location/location.model.action';
import {LocationModelState, LocationModelAudit} from 'src/app/models/location/location.model.state';
import {LocationModel} from 'src/app/models/location/location.model';
import {
	getLocationModelAuditsByEntityId,
	getLocationModelWithId,
	getCountLocationModels,
...
import {CrudTileMode} from '../location-admin-crud.component';
...
@Component({
	selector: 'cb-location-admin-crud-edit',
	templateUrl: './location-admin-crud-edit.component.html',
	styleUrls: [
		'./location-admin-crud-edit.component.scss',
...
export class AdminLocationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '1ccb75f0-010d-4ed2-ad80-2fb1eab4c6ff';
	 ...
	 */
	locationAudits$: Observable<LocationModelAudit[]>;
	...
	warehouseOppositeRelation = 'locations';
	...
	stockControlsOppositeRelation = 'location';
	...
	warehouseInitialStockSOppositeRelation = 'location';
	...
	warehouseInventoriesOppositeRelation = 'location';
	 ...
	 */
	modelProperties: ModelProperty[] = LocationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = LocationModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = LocationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	constructor(
		private readonly store: Store<{ model: LocationModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountLocationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new LocationModel();
		...
		if (this.targetModelId) {
			this.locationAudits$ = this.store.select(getLocationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<LocationModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.LocationAction(
			modelAction.LocationModelActionTypes.FETCH_LOCATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getLocationModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.LocationAction(
			modelAction.LocationModelActionTypes.FETCH_LOCATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<LocationModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.warehouseMultiCrudActive || this.stockControlsMultiCrudActive || this.warehouseInitialStockSMultiCrudActive || this.warehouseInventoriesMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.LocationAction(
						modelAction.LocationModelActionTypes.COUNT_LOCATIONS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.LocationAction(
				modelAction.LocationModelActionTypes.CREATE_LOCATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.LocationAction(
				modelAction.LocationModelActionTypes.UPDATE_LOCATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: LocationModel = new LocationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getLocationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new LocationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = LocationModel.getProps();
		this.modelRelations = LocationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: LocationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): LocationModel {
		let updatedModel: LocationModel = new LocationModel(model);
