...
import {LocationModelState} from 'src/app/models/location/location.model.state';
...
import {
	getLocationCollectionModels,
	getLocationCollectionCount
...
@Component({
	selector: 'cb-location-admin-crud-list',
	templateUrl: './location-admin-crud-list.component.html',
	styleUrls: [
		'./location-admin-crud-list.component.scss',
...
export class AdminLocationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<LocationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...LocationModel.searchFields,
	...
	/**
	 * List of Locations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.locationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.locationsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.LocationAction(
				modelAction.LocationModelActionTypes.INITIALISE_LOCATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.LocationAction(
				modelAction.LocationModelActionTypes.FETCH_LOCATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.locations = this.store.select(getLocationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getLocationCollectionCount, this.collectionId).subscribe(
				(count) => this.locationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: LocationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.LocationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.LocationModelActionTypes.EXPORT_ALL_LOCATION;
		} else {
			actionType = modelAction.LocationModelActionTypes.EXPORT_LOCATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.LocationAction(
				modelAction.LocationModelActionTypes.IMPORT_LOCATIONS,
				{
					file: file,
				},
				[new modelAction.LocationAction(
			...
			window.open(environment.API_URL + '/api/location/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: LocationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Location entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Location entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.LocationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.LocationModelActionTypes.DELETE_LOCATION;
						} else if (usingSelected) {
							actionType = modelAction.LocationModelActionTypes.DELETE_ALL_LOCATION;
						} else {
							actionType = modelAction.LocationModelActionTypes.DELETE_LOCATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.LocationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_LOCATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_LOCATION and DELETE_LOCATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = LocationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.locationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
