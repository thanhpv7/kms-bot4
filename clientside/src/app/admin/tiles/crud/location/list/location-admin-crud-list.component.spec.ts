...
import {AdminLocationCrudListComponent} from './location-admin-crud-list.component';
...
import {AdminLocationCrudModule} from '../location-admin-crud.module';
import {
	LocationModelState,
	initialState as initialLocationModelState
} from 'src/app/models/location/location.model.state';
 ...
 */
describe('Location Crud List Tile Component', () => {

	let fixture;
	let locationCrudListComponent: AdminLocationCrudListComponent;

	let store: MockStore<{ model: LocationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let locationModelState: LocationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<LocationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '1ccb75f0-010d-4ed2-ad80-2fb1eab4c6ff';
	const searchContent = 'text';

	const searchableFieldCount = LocationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'location-admin-crud',
		urls: ['location-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const locationDataFactory = new LocationDataFactory();

	const defaultDataSize = 30;
	const defaultData = locationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: LocationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		locationModelState = initialLocationModelState;
		store.setState({model: locationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getLocationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getLocationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminLocationCrudListComponent);
			locationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(locationCrudListComponent.locationsCount).toBe(defaultData.length);
			...
			expect(locationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			locationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = locationDataFactory.createAll(filteredDataSize);
			...
			expect(locationCrudListComponent.locationsCount).toBe(filteredDataSize);
			expect(locationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(locationCrudListComponent.pageIndex).toBe(0);
			expect(locationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			locationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		locationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		locationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
