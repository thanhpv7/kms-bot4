...
import {BpjsPcarePesertaKegiatanKelompokModelState, BpjsPcarePesertaKegiatanKelompokModelAudit} from 'src/app/models/bpjsPcarePesertaKegiatanKelompok/bpjs_pcare_peserta_kegiatan_kelompok.model.state';
...
import {
	getBpjsPcarePesertaKegiatanKelompokModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-pcare-peserta-kegiatan-kelompok-admin-crud-edit',
	templateUrl: './bpjs-pcare-peserta-kegiatan-kelompok-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-peserta-kegiatan-kelompok-admin-crud-edit.component.scss',
...
export class AdminBpjsPcarePesertaKegiatanKelompokCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcarePesertaKegiatanKelompokAudits$: Observable<BpjsPcarePesertaKegiatanKelompokModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcarePesertaKegiatanKelompokModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcarePesertaKegiatanKelompokModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcarePesertaKegiatanKelompokModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcarePesertaKegiatanKelompokModel();
		...
		if (this.targetModelId) {
			this.bpjsPcarePesertaKegiatanKelompokAudits$ = this.store.select(getBpjsPcarePesertaKegiatanKelompokModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcarePesertaKegiatanKelompokModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
			modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcarePesertaKegiatanKelompokModelWithId, this.targetModelId)
			...
			model = new BpjsPcarePesertaKegiatanKelompokModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
			modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
				modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.CREATE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
				modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.UPDATE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcarePesertaKegiatanKelompokModel = new BpjsPcarePesertaKegiatanKelompokModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcarePesertaKegiatanKelompokModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcarePesertaKegiatanKelompokModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcarePesertaKegiatanKelompokModel.getProps();
		this.modelRelations = BpjsPcarePesertaKegiatanKelompokModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcarePesertaKegiatanKelompokModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcarePesertaKegiatanKelompokModel {
		let updatedModel: BpjsPcarePesertaKegiatanKelompokModel = new BpjsPcarePesertaKegiatanKelompokModel(model);
