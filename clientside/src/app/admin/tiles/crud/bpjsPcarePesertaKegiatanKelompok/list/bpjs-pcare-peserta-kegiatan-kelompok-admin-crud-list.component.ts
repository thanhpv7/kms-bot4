...
import * as modelAction from 'src/app/models/bpjsPcarePesertaKegiatanKelompok/bpjs_pcare_peserta_kegiatan_kelompok.model.action';
import {BpjsPcarePesertaKegiatanKelompokModelState} from 'src/app/models/bpjsPcarePesertaKegiatanKelompok/bpjs_pcare_peserta_kegiatan_kelompok.model.state';
...
import {
	...
	getBpjsPcarePesertaKegiatanKelompokCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-peserta-kegiatan-kelompok-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-peserta-kegiatan-kelompok-admin-crud-list',
	templateUrl: './bpjs-pcare-peserta-kegiatan-kelompok-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-peserta-kegiatan-kelompok-admin-crud-list.component.scss',
...
export class AdminBpjsPcarePesertaKegiatanKelompokCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcarePesertaKegiatanKelompokModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcarePesertaKegiatanKelompokModel.searchFields,
	...
	/**
	 * List of BPJS Pcare Peserta Kegiatan Kelompoks that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcarePesertaKegiatanKelompoks: Observable<BpjsPcarePesertaKegiatanKelompokModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcarePesertaKegiatanKelompoksCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcarePesertaKegiatanKelompokModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcarePesertaKegiatanKelompokModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'dfe5878b-e724-4661-bd3c-def63a2c62f9';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcarePesertaKegiatanKelompoksCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcarePesertaKegiatanKelompoksCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcarePesertaKegiatanKelompokModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
				modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.INITIALISE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
				modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
						modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.COUNT_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOKS,
						null
					)
				]
			))
		...
		this.bpjsPcarePesertaKegiatanKelompoks = this.store.select(getBpjsPcarePesertaKegiatanKelompokCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcarePesertaKegiatanKelompokCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcarePesertaKegiatanKelompoksCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-peserta-kegiatan-kelompok');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcarePesertaKegiatanKelompokModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.EXPORT_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK;
		} else {
			actionType = modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.EXPORT_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
				modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.IMPORT_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOKS,
				{
					file: file,
				},
				[new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
			...
			window.open(environment.API_URL + '/api/bpjspcarepesertakegiatankelompok/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcarePesertaKegiatanKelompokModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Peserta Kegiatan Kelompok entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Peserta Kegiatan Kelompok entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.DELETE_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK;
						} else {
							actionType = modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcarePesertaKegiatanKelompokAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK and DELETE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcarePesertaKegiatanKelompokModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcarePesertaKegiatanKelompokModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcarePesertaKegiatanKelompoksCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
