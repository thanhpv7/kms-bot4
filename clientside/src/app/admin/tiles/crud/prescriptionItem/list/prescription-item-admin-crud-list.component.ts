...
import {PrescriptionItemModelState} from 'src/app/models/prescriptionItem/prescription_item.model.state';
...
import {CrudTileMode} from '../prescription-item-admin-crud.component';
...
@Component({
	selector: 'cb-prescription-item-admin-crud-list',
	templateUrl: './prescription-item-admin-crud-list.component.html',
	styleUrls: [
		'./prescription-item-admin-crud-list.component.scss',
...
export class AdminPrescriptionItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PrescriptionItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PrescriptionItemModel.searchFields,
	...
	/**
	 * List of Prescription Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	prescriptionItems: Observable<PrescriptionItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	prescriptionItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.prescriptionItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.prescriptionItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PrescriptionItemAction(
				modelAction.PrescriptionItemModelActionTypes.INITIALISE_PRESCRIPTION_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PrescriptionItemAction(
				modelAction.PrescriptionItemModelActionTypes.FETCH_PRESCRIPTION_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.prescriptionItems = this.store.select(getPrescriptionItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPrescriptionItemCollectionCount, this.collectionId).subscribe(
				(count) => this.prescriptionItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PrescriptionItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PrescriptionItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PrescriptionItemModelActionTypes.EXPORT_ALL_PRESCRIPTION_ITEM;
		} else {
			actionType = modelAction.PrescriptionItemModelActionTypes.EXPORT_PRESCRIPTION_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PrescriptionItemAction(
				modelAction.PrescriptionItemModelActionTypes.IMPORT_PRESCRIPTION_ITEMS,
				{
					file: file,
				},
				[new modelAction.PrescriptionItemAction(
			...
			window.open(environment.API_URL + '/api/prescriptionitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PrescriptionItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Prescription Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Prescription Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PrescriptionItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PrescriptionItemModelActionTypes.DELETE_PRESCRIPTION_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.PrescriptionItemModelActionTypes.DELETE_ALL_PRESCRIPTION_ITEM;
						} else {
							actionType = modelAction.PrescriptionItemModelActionTypes.DELETE_PRESCRIPTION_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PrescriptionItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PRESCRIPTION_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PRESCRIPTION_ITEM and DELETE_PRESCRIPTION_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PrescriptionItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.prescriptionItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
