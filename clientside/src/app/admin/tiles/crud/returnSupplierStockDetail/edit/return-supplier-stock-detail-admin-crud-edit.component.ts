...
import * as modelAction from 'src/app/models/returnSupplierStockDetail/return_supplier_stock_detail.model.action';
import {ReturnSupplierStockDetailModelState, ReturnSupplierStockDetailModelAudit} from 'src/app/models/returnSupplierStockDetail/return_supplier_stock_detail.model.state';
import {ReturnSupplierStockDetailModel} from 'src/app/models/returnSupplierStockDetail/return_supplier_stock_detail.model';
import {
	getReturnSupplierStockDetailModelAuditsByEntityId,
	getReturnSupplierStockDetailModelWithId,
	getCountReturnSupplierStockDetailModels,
...
import {CrudTileMode} from '../return-supplier-stock-detail-admin-crud.component';
...
@Component({
	selector: 'cb-return-supplier-stock-detail-admin-crud-edit',
	templateUrl: './return-supplier-stock-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./return-supplier-stock-detail-admin-crud-edit.component.scss',
...
export class AdminReturnSupplierStockDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '13b23a73-b3a8-47a8-8f61-10d5d497517e';
	 ...
	 */
	returnSupplierStockDetailAudits$: Observable<ReturnSupplierStockDetailModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Return Supplier and Return Supplier Stock Details here] off begin
	returnSupplierMultiCrudActive: boolean = false;
	returnSupplierDisplayName = 'Return Supplier';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Return Supplier and Return Supplier Stock Details here] end
	returnSupplierOppositeRelation = 'returnSupplierStockDetails';
	returnSupplierRelations: ModelRelation[] = this.returnSupplierMultiCrudActive ? ReturnSupplierModel.getRelations()
		.map(relation => {
			if (relation.id === this.returnSupplierOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'returnSupplierStockDetails';
	 ...
	 */
	modelProperties: ModelProperty[] = ReturnSupplierStockDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ReturnSupplierStockDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ReturnSupplierStockDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'returnSupplier' && this.returnSupplierMultiCrudActive) {
	 ...
	 */
	targetModel: ReturnSupplierStockDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: ReturnSupplierStockDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountReturnSupplierStockDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ReturnSupplierStockDetailModel();
		...
		if (this.targetModelId) {
			this.returnSupplierStockDetailAudits$ = this.store.select(getReturnSupplierStockDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ReturnSupplierStockDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
			modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_RETURN_SUPPLIER_STOCK_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getReturnSupplierStockDetailModelWithId, this.targetModelId)
			...
			model = new ReturnSupplierStockDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('return-supplier-stock-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
			modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_RETURN_SUPPLIER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ReturnSupplierStockDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.returnSupplierMultiCrudActive || this.stockCatalogueMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.ReturnSupplierStockDetailAction(
						modelAction.ReturnSupplierStockDetailModelActionTypes.COUNT_RETURN_SUPPLIER_STOCK_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
				modelAction.ReturnSupplierStockDetailModelActionTypes.CREATE_RETURN_SUPPLIER_STOCK_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
				modelAction.ReturnSupplierStockDetailModelActionTypes.UPDATE_RETURN_SUPPLIER_STOCK_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ReturnSupplierStockDetailModel = new ReturnSupplierStockDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getReturnSupplierStockDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ReturnSupplierStockDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.returnSupplierDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ReturnSupplierModel.getProps(), this.returnSupplierRelations, this.isViewOnly),
					new ReturnSupplierModel(),
					ReturnSupplierModel,
					this.returnSupplierDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.returnSupplierRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ReturnSupplierStockDetailModel.getProps();
		this.modelRelations = ReturnSupplierStockDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'returnSupplier';
		}).hideElement = this.returnSupplierMultiCrudActive || this.modelRelations.find(rel => rel.id === 'returnSupplier').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.returnSupplierMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.returnSupplierDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.returnSupplier && this.targetModel.returnSupplier.id && this.returnSupplierMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				ReturnSupplierModel,
				this.returnSupplierOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.returnSupplier.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getReturnSupplierModelWithId, this.targetModel.returnSupplierId).subscribe(returnSupplier => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ReturnSupplierModel.getProps(), this.returnSupplierRelations, this.isViewOnly),
					returnSupplier,
					ReturnSupplierModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: ReturnSupplierStockDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ReturnSupplierStockDetailModel {
		let updatedModel: ReturnSupplierStockDetailModel = new ReturnSupplierStockDetailModel(model);

		if (this.returnSupplierMultiCrudActive) {
			updatedModel.returnSupplier = event.additionalEntities.returnSupplier ? event.additionalEntities.returnSupplier : undefined;

			if (!updatedModel.returnSupplier && event.payload) {
				event.payload.returnSupplierId = null;
			}
		} else if (event.payload?.returnSupplierId !== undefined) {
			event.payload.returnSupplier = null;
			updatedModel.returnSupplier = null;
		}
