...
import {AdminReturnSupplierStockDetailCrudListComponent} from './return-supplier-stock-detail-admin-crud-list.component';
...
import {AdminReturnSupplierStockDetailCrudModule} from '../return-supplier-stock-detail-admin-crud.module';
import {
 ...
 */
describe('Return Supplier Stock Detail Crud List Tile Component', () => {

	let fixture;
	let returnSupplierStockDetailCrudListComponent: AdminReturnSupplierStockDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ReturnSupplierStockDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '13b23a73-b3a8-47a8-8f61-10d5d497517e';
	const searchContent = 'text';

	const searchableFieldCount = ReturnSupplierStockDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'return-supplier-stock-detail-admin-crud',
		urls: ['return-supplier-stock-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const returnSupplierStockDetailDataFactory = new ReturnSupplierStockDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = returnSupplierStockDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getReturnSupplierStockDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getReturnSupplierStockDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminReturnSupplierStockDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminReturnSupplierStockDetailCrudListComponent);
			returnSupplierStockDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(returnSupplierStockDetailCrudListComponent.returnSupplierStockDetailsCount).toBe(defaultData.length);
			...
			expect(returnSupplierStockDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			returnSupplierStockDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = returnSupplierStockDetailDataFactory.createAll(filteredDataSize);
			...
			expect(returnSupplierStockDetailCrudListComponent.returnSupplierStockDetailsCount).toBe(filteredDataSize);
			expect(returnSupplierStockDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(returnSupplierStockDetailCrudListComponent.pageIndex).toBe(0);
			expect(returnSupplierStockDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			returnSupplierStockDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		returnSupplierStockDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		returnSupplierStockDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
