...
import {ReturnSupplierStockDetailModelState} from 'src/app/models/returnSupplierStockDetail/return_supplier_stock_detail.model.state';
...
import {
	...
	getReturnSupplierStockDetailCollectionCount
...
@Component({
	selector: 'cb-return-supplier-stock-detail-admin-crud-list',
	templateUrl: './return-supplier-stock-detail-admin-crud-list.component.html',
	styleUrls: [
		'./return-supplier-stock-detail-admin-crud-list.component.scss',
...
export class AdminReturnSupplierStockDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ReturnSupplierStockDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ReturnSupplierStockDetailModel.searchFields,
	...
	/**
	 * List of Return Supplier Stock Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	returnSupplierStockDetails: Observable<ReturnSupplierStockDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	returnSupplierStockDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.returnSupplierStockDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.returnSupplierStockDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
				modelAction.ReturnSupplierStockDetailModelActionTypes.INITIALISE_RETURN_SUPPLIER_STOCK_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
				modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_RETURN_SUPPLIER_STOCK_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.returnSupplierStockDetails = this.store.select(getReturnSupplierStockDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getReturnSupplierStockDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.returnSupplierStockDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ReturnSupplierStockDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ReturnSupplierStockDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ReturnSupplierStockDetailModelActionTypes.EXPORT_ALL_RETURN_SUPPLIER_STOCK_DETAIL;
		} else {
			actionType = modelAction.ReturnSupplierStockDetailModelActionTypes.EXPORT_RETURN_SUPPLIER_STOCK_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
				modelAction.ReturnSupplierStockDetailModelActionTypes.IMPORT_RETURN_SUPPLIER_STOCK_DETAILS,
				{
					file: file,
				},
				[new modelAction.ReturnSupplierStockDetailAction(
			...
			window.open(environment.API_URL + '/api/returnsupplierstockdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ReturnSupplierStockDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Return Supplier Stock Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Return Supplier Stock Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ReturnSupplierStockDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ReturnSupplierStockDetailModelActionTypes.DELETE_RETURN_SUPPLIER_STOCK_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.ReturnSupplierStockDetailModelActionTypes.DELETE_ALL_RETURN_SUPPLIER_STOCK_DETAIL;
						} else {
							actionType = modelAction.ReturnSupplierStockDetailModelActionTypes.DELETE_RETURN_SUPPLIER_STOCK_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ReturnSupplierStockDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_RETURN_SUPPLIER_STOCK_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_RETURN_SUPPLIER_STOCK_DETAIL and DELETE_RETURN_SUPPLIER_STOCK_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ReturnSupplierStockDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.returnSupplierStockDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
