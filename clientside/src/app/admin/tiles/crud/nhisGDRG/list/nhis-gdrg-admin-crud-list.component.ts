...
import * as modelAction from 'src/app/models/nhisGDRG/nhis_gdrg.model.action';
import {NhisGDRGModelState} from 'src/app/models/nhisGDRG/nhis_gdrg.model.state';
...
import {
	getNhisGDRGCollectionModels,
	getNhisGDRGCollectionCount
} from 'src/app/models/nhisGDRG/nhis_gdrg.model.selector';
...
import {CrudTileMode} from '../nhis-gdrg-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-gdrg-admin-crud-list',
	templateUrl: './nhis-gdrg-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-gdrg-admin-crud-list.component.scss',
...
export class AdminNhisGDRGCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisGDRGModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisGDRGModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = NhisGDRGModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '04d0a682-653d-4db8-b9a7-7b51770528cb';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisGDRGsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisGDRGsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisGDRGModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisGDRGAction(
				modelAction.NhisGDRGModelActionTypes.INITIALISE_NHIS_GDRG_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisGDRGAction(
				modelAction.NhisGDRGModelActionTypes.FETCH_NHIS_GDRG_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisGDRGAction(
						modelAction.NhisGDRGModelActionTypes.COUNT_NHIS_GDRGS,
						null
					)
				]
			))
		...
		this.nhisGDRGs = this.store.select(getNhisGDRGCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisGDRGCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisGDRGsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisGDRGModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisGDRGModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisGDRGModelActionTypes.EXPORT_ALL_NHIS_GDRG;
		} else {
			actionType = modelAction.NhisGDRGModelActionTypes.EXPORT_NHIS_GDRG_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisGDRGAction(
				modelAction.NhisGDRGModelActionTypes.IMPORT_NHIS_GDRGS,
				{
					file: file,
				},
				[new modelAction.NhisGDRGAction(
			...
			window.open(environment.API_URL + '/api/nhisgdrg/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisGDRGModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS GDRG entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS GDRG entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisGDRGModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisGDRGModelActionTypes.DELETE_NHIS_GDRG;
						} else if (usingSelected) {
							actionType = modelAction.NhisGDRGModelActionTypes.DELETE_ALL_NHIS_GDRG;
						} else {
							actionType = modelAction.NhisGDRGModelActionTypes.DELETE_NHIS_GDRG_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisGDRGAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_GDRG when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_GDRG and DELETE_NHIS_GDRG_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisGDRGModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisGDRGModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisGDRGsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
