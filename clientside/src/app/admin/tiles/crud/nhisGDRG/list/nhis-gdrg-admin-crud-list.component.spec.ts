...
import {AdminNhisGDRGCrudListComponent} from './nhis-gdrg-admin-crud-list.component';
import {NhisGDRGDataFactory} from 'src/app/lib/utils/factories/nhis-gdrg-data-factory';
import {NhisGDRGModel} from 'src/app/models/nhisGDRG/nhis_gdrg.model';
import {AdminNhisGDRGCrudModule} from '../nhis-gdrg-admin-crud.module';
import {
	NhisGDRGModelState,
	initialState as initialNhisGDRGModelState
} from 'src/app/models/nhisGDRG/nhis_gdrg.model.state';
 ...
 */
describe('NHIS GDRG Crud List Tile Component', () => {

	let fixture;
	let nhisGDRGCrudListComponent: AdminNhisGDRGCrudListComponent;

	let store: MockStore<{ model: NhisGDRGModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisGDRGModelState: NhisGDRGModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisGDRGModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '04d0a682-653d-4db8-b9a7-7b51770528cb';
	const searchContent = 'text';

	const searchableFieldCount = NhisGDRGModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-gdrg-admin-crud',
		urls: ['nhis-gdrg-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisGDRGDataFactory = new NhisGDRGDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisGDRGDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisGDRGModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisGDRGModelState = initialNhisGDRGModelState;
		store.setState({model: nhisGDRGModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisGDRGCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisGDRGCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminNhisGDRGCrudListComponent);
			nhisGDRGCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisGDRGCrudListComponent.nhisGDRGsCount).toBe(defaultData.length);
			...
			expect(nhisGDRGCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisGDRGCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisGDRGDataFactory.createAll(filteredDataSize);
			...
			expect(nhisGDRGCrudListComponent.nhisGDRGsCount).toBe(filteredDataSize);
			expect(nhisGDRGCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisGDRGCrudListComponent.pageIndex).toBe(0);
			expect(nhisGDRGCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisGDRGCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisGDRGCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisGDRGCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
