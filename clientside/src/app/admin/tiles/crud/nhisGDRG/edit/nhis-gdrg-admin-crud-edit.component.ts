...
import {NhisGDRGModelState, NhisGDRGModelAudit} from 'src/app/models/nhisGDRG/nhis_gdrg.model.state';
import {NhisGDRGModel} from 'src/app/models/nhisGDRG/nhis_gdrg.model';
import {
	getNhisGDRGModelAuditsByEntityId,
	getNhisGDRGModelWithId,
	getCountNhisGDRGModels,
...
@Component({
	selector: 'cb-nhis-gdrg-admin-crud-edit',
	templateUrl: './nhis-gdrg-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-gdrg-admin-crud-edit.component.scss',
...
export class AdminNhisGDRGCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisGDRGAudits$: Observable<NhisGDRGModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisGDRGModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisGDRGModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisGDRGModel();
		...
		if (this.targetModelId) {
			this.nhisGDRGAudits$ = this.store.select(getNhisGDRGModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisGDRGModel> = {
		...
		this.store.dispatch(new modelAction.NhisGDRGAction(
			modelAction.NhisGDRGModelActionTypes.FETCH_NHIS_GDRG,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisGDRGModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisGDRGAction(
			modelAction.NhisGDRGModelActionTypes.FETCH_NHIS_GDRG_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisGDRGAction(
				modelAction.NhisGDRGModelActionTypes.CREATE_NHIS_GDRG,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisGDRGAction(
				modelAction.NhisGDRGModelActionTypes.UPDATE_NHIS_GDRG,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisGDRGModel = new NhisGDRGModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisGDRGModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisGDRGModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisGDRGModel.getProps();
		this.modelRelations = NhisGDRGModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisGDRGModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisGDRGModel {
		let updatedModel: NhisGDRGModel = new NhisGDRGModel(model);
