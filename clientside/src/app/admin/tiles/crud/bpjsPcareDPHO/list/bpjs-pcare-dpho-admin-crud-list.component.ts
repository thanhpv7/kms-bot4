...
import * as modelAction from 'src/app/models/bpjsPcareDPHO/bpjs_pcare_dpho.model.action';
import {BpjsPcareDPHOModelState} from 'src/app/models/bpjsPcareDPHO/bpjs_pcare_dpho.model.state';
...
import {
	getBpjsPcareDPHOCollectionModels,
	getBpjsPcareDPHOCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-dpho-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-dpho-admin-crud-list',
	templateUrl: './bpjs-pcare-dpho-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-dpho-admin-crud-list.component.scss',
...
export class AdminBpjsPcareDPHOCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcareDPHOModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcareDPHOModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcareDPHOModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '5812ad7e-2e8a-4fab-aa91-6540e03583c4';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcareDPHOsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcareDPHOsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcareDPHOModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
				modelAction.BpjsPcareDPHOModelActionTypes.INITIALISE_BPJS_PCARE_DPHO_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
				modelAction.BpjsPcareDPHOModelActionTypes.FETCH_BPJS_PCARE_DPHO_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcareDPHOAction(
						modelAction.BpjsPcareDPHOModelActionTypes.COUNT_BPJS_PCARE_DPHOS,
						null
					)
				]
			))
		...
		this.bpjsPcareDPHOs = this.store.select(getBpjsPcareDPHOCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcareDPHOCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcareDPHOsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcareDPHOModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcareDPHOModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcareDPHOModelActionTypes.EXPORT_ALL_BPJS_PCARE_DPHO;
		} else {
			actionType = modelAction.BpjsPcareDPHOModelActionTypes.EXPORT_BPJS_PCARE_DPHO_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
				modelAction.BpjsPcareDPHOModelActionTypes.IMPORT_BPJS_PCARE_DPHOS,
				{
					file: file,
				},
				[new modelAction.BpjsPcareDPHOAction(
			...
			window.open(environment.API_URL + '/api/bpjspcaredpho/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcareDPHOModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare DPHO entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare DPHO entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcareDPHOModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcareDPHOModelActionTypes.DELETE_BPJS_PCARE_DPHO;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcareDPHOModelActionTypes.DELETE_ALL_BPJS_PCARE_DPHO;
						} else {
							actionType = modelAction.BpjsPcareDPHOModelActionTypes.DELETE_BPJS_PCARE_DPHO_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_DPHO when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_DPHO and DELETE_BPJS_PCARE_DPHO_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcareDPHOModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcareDPHOModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcareDPHOsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
