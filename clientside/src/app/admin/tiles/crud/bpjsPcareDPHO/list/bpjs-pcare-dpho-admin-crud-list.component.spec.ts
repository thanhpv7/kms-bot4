...
import {AdminBpjsPcareDPHOCrudListComponent} from './bpjs-pcare-dpho-admin-crud-list.component';
...
import {AdminBpjsPcareDPHOCrudModule} from '../bpjs-pcare-dpho-admin-crud.module';
import {
	BpjsPcareDPHOModelState,
	initialState as initialBpjsPcareDPHOModelState
} from 'src/app/models/bpjsPcareDPHO/bpjs_pcare_dpho.model.state';
 ...
 */
describe('BPJS Pcare DPHO Crud List Tile Component', () => {

	let fixture;
	let bpjsPcareDPHOCrudListComponent: AdminBpjsPcareDPHOCrudListComponent;

	let store: MockStore<{ model: BpjsPcareDPHOModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsPcareDPHOModelState: BpjsPcareDPHOModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPcareDPHOModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5812ad7e-2e8a-4fab-aa91-6540e03583c4';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPcareDPHOModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-pcare-dpho-admin-crud',
		urls: ['bpjs-pcare-dpho-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPcareDPHODataFactory = new BpjsPcareDPHODataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPcareDPHODataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsPcareDPHOModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsPcareDPHOModelState = initialBpjsPcareDPHOModelState;
		store.setState({model: bpjsPcareDPHOModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPcareDPHOCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPcareDPHOCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsPcareDPHOCrudListComponent);
			bpjsPcareDPHOCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPcareDPHOCrudListComponent.bpjsPcareDPHOsCount).toBe(defaultData.length);
			...
			expect(bpjsPcareDPHOCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPcareDPHOCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPcareDPHODataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPcareDPHOCrudListComponent.bpjsPcareDPHOsCount).toBe(filteredDataSize);
			expect(bpjsPcareDPHOCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPcareDPHOCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPcareDPHOCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPcareDPHOCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPcareDPHOCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPcareDPHOCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
