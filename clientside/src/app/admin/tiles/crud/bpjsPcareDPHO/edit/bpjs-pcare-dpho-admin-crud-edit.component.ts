...
import {BpjsPcareDPHOModelState, BpjsPcareDPHOModelAudit} from 'src/app/models/bpjsPcareDPHO/bpjs_pcare_dpho.model.state';
import {BpjsPcareDPHOModel} from 'src/app/models/bpjsPcareDPHO/bpjs_pcare_dpho.model';
import {
	getBpjsPcareDPHOModelAuditsByEntityId,
	getBpjsPcareDPHOModelWithId,
	getCountBpjsPcareDPHOModels,
...
@Component({
	selector: 'cb-bpjs-pcare-dpho-admin-crud-edit',
	templateUrl: './bpjs-pcare-dpho-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-dpho-admin-crud-edit.component.scss',
...
export class AdminBpjsPcareDPHOCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcareDPHOAudits$: Observable<BpjsPcareDPHOModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'bpjsPcareDPHO';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcareDPHOModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcareDPHOModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcareDPHOModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcareDPHOModel();
		...
		if (this.targetModelId) {
			this.bpjsPcareDPHOAudits$ = this.store.select(getBpjsPcareDPHOModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcareDPHOModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
			modelAction.BpjsPcareDPHOModelActionTypes.FETCH_BPJS_PCARE_DPHO,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareDPHOModelWithId, this.targetModelId)
			...
			model = new BpjsPcareDPHOModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
			modelAction.BpjsPcareDPHOModelActionTypes.FETCH_BPJS_PCARE_DPHO_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
				modelAction.BpjsPcareDPHOModelActionTypes.CREATE_BPJS_PCARE_DPHO,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcareDPHOAction(
				modelAction.BpjsPcareDPHOModelActionTypes.UPDATE_BPJS_PCARE_DPHO,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcareDPHOModel = new BpjsPcareDPHOModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareDPHOModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcareDPHOModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcareDPHOModel.getProps();
		this.modelRelations = BpjsPcareDPHOModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcareDPHOModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcareDPHOModel {
		let updatedModel: BpjsPcareDPHOModel = new BpjsPcareDPHOModel(model);
