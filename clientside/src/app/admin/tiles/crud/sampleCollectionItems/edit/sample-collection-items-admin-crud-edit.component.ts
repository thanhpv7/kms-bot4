...
import * as modelAction from 'src/app/models/sampleCollectionItems/sample_collection_items.model.action';
import {SampleCollectionItemsModelState, SampleCollectionItemsModelAudit} from 'src/app/models/sampleCollectionItems/sample_collection_items.model.state';
import {SampleCollectionItemsModel} from 'src/app/models/sampleCollectionItems/sample_collection_items.model';
import {
	getSampleCollectionItemsModelAuditsByEntityId,
...
import {CrudTileMode} from '../sample-collection-items-admin-crud.component';
...
@Component({
	selector: 'cb-sample-collection-items-admin-crud-edit',
	templateUrl: './sample-collection-items-admin-crud-edit.component.html',
	styleUrls: [
		'./sample-collection-items-admin-crud-edit.component.scss',
...
export class AdminSampleCollectionItemsCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b63f945e-d93a-48cd-a75e-8f19db2b787a';
	 ...
	 */
	sampleCollectionItemsAudits$: Observable<SampleCollectionItemsModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Sample Collection Information and Sample Collection Items here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Sample Collection Information and Sample Collection Items here] end
	sampleCollectionInformationOppositeRelation = 'sampleCollectionItems';
	...
	sampleCollectionStaffMultiCrudActive: boolean = false;
	sampleCollectionStaffDisplayName = 'Sample Collection Staff';
	...
	sampleCollectionStaffOppositeRelation = 'sampleCollectionStaffs';
	sampleCollectionStaffRelations: ModelRelation[] = this.sampleCollectionStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.sampleCollectionStaffOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	sampleDeliveryStaffMultiCrudActive: boolean = false;
	sampleDeliveryStaffDisplayName = 'Sample Delivery Staff';
	...
	sampleDeliveryStaffOppositeRelation = 'sampleDeliveryStaffs';
	sampleDeliveryStaffRelations: ModelRelation[] = this.sampleDeliveryStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.sampleDeliveryStaffOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	sampleProcessingStaffMultiCrudActive: boolean = false;
	sampleProcessingStaffDisplayName = 'Sample Processing Staff';
	...
	sampleProcessingStaffOppositeRelation = 'sampleProcessingStaffs';
	sampleProcessingStaffRelations: ModelRelation[] = this.sampleProcessingStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.sampleProcessingStaffOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = SampleCollectionItemsModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = SampleCollectionItemsModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SampleCollectionItemsModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'sampleCollectionStaff' && this.sampleCollectionStaffMultiCrudActive) {
					exp.fields = StaffModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'sampleDeliveryStaff' && this.sampleDeliveryStaffMultiCrudActive) {
					exp.fields = StaffModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'sampleProcessingStaff' && this.sampleProcessingStaffMultiCrudActive) {
					exp.fields = StaffModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: SampleCollectionItemsModel;
	...
	constructor(
		private readonly store: Store<{ model: SampleCollectionItemsModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSampleCollectionItemsModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new SampleCollectionItemsModel();
		...
		if (this.targetModelId) {
			this.sampleCollectionItemsAudits$ = this.store.select(getSampleCollectionItemsModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SampleCollectionItemsModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.SampleCollectionItemsAction(
			modelAction.SampleCollectionItemsModelActionTypes.FETCH_SAMPLE_COLLECTION_ITEMS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSampleCollectionItemsModelWithId, this.targetModelId)
			...
			model = new SampleCollectionItemsModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('sample-collection-items');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SampleCollectionItemsAction(
			modelAction.SampleCollectionItemsModelActionTypes.FETCH_SAMPLE_COLLECTION_ITEMS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<SampleCollectionItemsModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.sampleCollectionInformationMultiCrudActive || this.sampleCollectionStaffMultiCrudActive || this.sampleDeliveryStaffMultiCrudActive || this.sampleProcessingStaffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.SampleCollectionItemsAction(
						modelAction.SampleCollectionItemsModelActionTypes.COUNT_SAMPLE_COLLECTION_ITEMSS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.SampleCollectionItemsAction(
				modelAction.SampleCollectionItemsModelActionTypes.CREATE_SAMPLE_COLLECTION_ITEMS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SampleCollectionItemsAction(
				modelAction.SampleCollectionItemsModelActionTypes.UPDATE_SAMPLE_COLLECTION_ITEMS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SampleCollectionItemsModel = new SampleCollectionItemsModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSampleCollectionItemsModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SampleCollectionItemsModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.sampleCollectionStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.sampleCollectionStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.sampleCollectionStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.sampleCollectionStaffRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.sampleDeliveryStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.sampleDeliveryStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.sampleDeliveryStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.sampleDeliveryStaffRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.sampleProcessingStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.sampleProcessingStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.sampleProcessingStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.sampleProcessingStaffRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SampleCollectionItemsModel.getProps();
		this.modelRelations = SampleCollectionItemsModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'sampleCollectionStaff';
		}).hideElement = this.sampleCollectionStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'sampleCollectionStaff').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'sampleDeliveryStaff';
		}).hideElement = this.sampleDeliveryStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'sampleDeliveryStaff').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'sampleProcessingStaff';
		}).hideElement = this.sampleProcessingStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'sampleProcessingStaff').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.sampleCollectionStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.sampleCollectionStaffDisplayName, many: false});
		}

		if (this.sampleDeliveryStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.sampleDeliveryStaffDisplayName, many: false});
		}

		if (this.sampleProcessingStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.sampleProcessingStaffDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.sampleCollectionStaff && this.targetModel.sampleCollectionStaff.id && this.sampleCollectionStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.sampleCollectionStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.sampleCollectionStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.sampleCollectionStaffId).subscribe(sampleCollectionStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.sampleCollectionStaffRelations, this.isViewOnly),
		...
		if (this.targetModel.sampleDeliveryStaff && this.targetModel.sampleDeliveryStaff.id && this.sampleDeliveryStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.sampleDeliveryStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.sampleDeliveryStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.sampleDeliveryStaffId).subscribe(sampleDeliveryStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.sampleDeliveryStaffRelations, this.isViewOnly),
					sampleDeliveryStaff,
					StaffModel,
		...
		if (this.targetModel.sampleProcessingStaff && this.targetModel.sampleProcessingStaff.id && this.sampleProcessingStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.sampleProcessingStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.sampleProcessingStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.sampleProcessingStaffId).subscribe(sampleProcessingStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.sampleProcessingStaffRelations, this.isViewOnly),
	 ...
	 */
	addAdditionalEntitiesToModel(model: SampleCollectionItemsModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SampleCollectionItemsModel {
		let updatedModel: SampleCollectionItemsModel = new SampleCollectionItemsModel(model);
		...
		if (this.sampleCollectionStaffMultiCrudActive) {
			updatedModel.sampleCollectionStaff = event.additionalEntities.sampleCollectionStaff ? event.additionalEntities.sampleCollectionStaff : undefined;

			if (!updatedModel.sampleCollectionStaff && event.payload) {
				event.payload.sampleCollectionStaffId = null;
			}
		} else if (event.payload?.sampleCollectionStaffId !== undefined) {
			event.payload.sampleCollectionStaff = null;
			updatedModel.sampleCollectionStaff = null;
		}

		if (this.sampleDeliveryStaffMultiCrudActive) {
			updatedModel.sampleDeliveryStaff = event.additionalEntities.sampleDeliveryStaff ? event.additionalEntities.sampleDeliveryStaff : undefined;

			if (!updatedModel.sampleDeliveryStaff && event.payload) {
				event.payload.sampleDeliveryStaffId = null;
			}
		} else if (event.payload?.sampleDeliveryStaffId !== undefined) {
			event.payload.sampleDeliveryStaff = null;
			updatedModel.sampleDeliveryStaff = null;
		}

		if (this.sampleProcessingStaffMultiCrudActive) {
			updatedModel.sampleProcessingStaff = event.additionalEntities.sampleProcessingStaff ? event.additionalEntities.sampleProcessingStaff : undefined;

			if (!updatedModel.sampleProcessingStaff && event.payload) {
				event.payload.sampleProcessingStaffId = null;
			}
		} else if (event.payload?.sampleProcessingStaffId !== undefined) {
			event.payload.sampleProcessingStaff = null;
			updatedModel.sampleProcessingStaff = null;
		}
