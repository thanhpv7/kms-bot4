...
import {SampleCollectionItemsModelState} from 'src/app/models/sampleCollectionItems/sample_collection_items.model.state';
...
import {
	...
	getSampleCollectionItemsCollectionCount
...
@Component({
	selector: 'cb-sample-collection-items-admin-crud-list',
	templateUrl: './sample-collection-items-admin-crud-list.component.html',
	styleUrls: [
		'./sample-collection-items-admin-crud-list.component.scss',
...
export class AdminSampleCollectionItemsCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SampleCollectionItemsModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SampleCollectionItemsModel.searchFields,
	...
	/**
	 * List of Sample Collection Itemss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	sampleCollectionItemss: Observable<SampleCollectionItemsModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	sampleCollectionItemssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.sampleCollectionItemssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.sampleCollectionItemssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SampleCollectionItemsAction(
				modelAction.SampleCollectionItemsModelActionTypes.INITIALISE_SAMPLE_COLLECTION_ITEMS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SampleCollectionItemsAction(
				modelAction.SampleCollectionItemsModelActionTypes.FETCH_SAMPLE_COLLECTION_ITEMS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.sampleCollectionItemss = this.store.select(getSampleCollectionItemsCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSampleCollectionItemsCollectionCount, this.collectionId).subscribe(
				(count) => this.sampleCollectionItemssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SampleCollectionItemsModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SampleCollectionItemsModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SampleCollectionItemsModelActionTypes.EXPORT_ALL_SAMPLE_COLLECTION_ITEMS;
		} else {
			actionType = modelAction.SampleCollectionItemsModelActionTypes.EXPORT_SAMPLE_COLLECTION_ITEMS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SampleCollectionItemsAction(
				modelAction.SampleCollectionItemsModelActionTypes.IMPORT_SAMPLE_COLLECTION_ITEMSS,
				{
					file: file,
				},
				[new modelAction.SampleCollectionItemsAction(
			...
			window.open(environment.API_URL + '/api/samplecollectionitems/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SampleCollectionItemsModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Sample Collection Items entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Sample Collection Items entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SampleCollectionItemsModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SampleCollectionItemsModelActionTypes.DELETE_SAMPLE_COLLECTION_ITEMS;
						} else if (usingSelected) {
							actionType = modelAction.SampleCollectionItemsModelActionTypes.DELETE_ALL_SAMPLE_COLLECTION_ITEMS;
						} else {
							actionType = modelAction.SampleCollectionItemsModelActionTypes.DELETE_SAMPLE_COLLECTION_ITEMS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SampleCollectionItemsAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SAMPLE_COLLECTION_ITEMS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SAMPLE_COLLECTION_ITEMS and DELETE_SAMPLE_COLLECTION_ITEMS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SampleCollectionItemsModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.sampleCollectionItemssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
