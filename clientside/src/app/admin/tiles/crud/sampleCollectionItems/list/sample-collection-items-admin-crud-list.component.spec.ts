...
import {AdminSampleCollectionItemsCrudListComponent} from './sample-collection-items-admin-crud-list.component';
...
import {AdminSampleCollectionItemsCrudModule} from '../sample-collection-items-admin-crud.module';
import {
 ...
 */
describe('Sample Collection Items Crud List Tile Component', () => {

	let fixture;
	let sampleCollectionItemsCrudListComponent: AdminSampleCollectionItemsCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<SampleCollectionItemsModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b63f945e-d93a-48cd-a75e-8f19db2b787a';
	const searchContent = 'text';

	const searchableFieldCount = SampleCollectionItemsModel.searchFields.length;

	const routerState: RouterState = {
		url: 'sample-collection-items-admin-crud',
		urls: ['sample-collection-items-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const sampleCollectionItemsDataFactory = new SampleCollectionItemsDataFactory();

	const defaultDataSize = 30;
	const defaultData = sampleCollectionItemsDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getSampleCollectionItemsCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getSampleCollectionItemsCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminSampleCollectionItemsCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminSampleCollectionItemsCrudListComponent);
			sampleCollectionItemsCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(sampleCollectionItemsCrudListComponent.sampleCollectionItemssCount).toBe(defaultData.length);
			...
			expect(sampleCollectionItemsCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			sampleCollectionItemsCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = sampleCollectionItemsDataFactory.createAll(filteredDataSize);
			...
			expect(sampleCollectionItemsCrudListComponent.sampleCollectionItemssCount).toBe(filteredDataSize);
			expect(sampleCollectionItemsCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(sampleCollectionItemsCrudListComponent.pageIndex).toBe(0);
			expect(sampleCollectionItemsCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			sampleCollectionItemsCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		sampleCollectionItemsCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		sampleCollectionItemsCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
