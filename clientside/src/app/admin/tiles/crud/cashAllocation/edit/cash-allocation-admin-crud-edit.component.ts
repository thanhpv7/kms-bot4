...
import {CashAllocationModelState, CashAllocationModelAudit} from 'src/app/models/cashAllocation/cash_allocation.model.state';
import {CashAllocationModel} from 'src/app/models/cashAllocation/cash_allocation.model';
import {
	getCashAllocationModelAuditsByEntityId,
	getCashAllocationModelWithId,
	getCountCashAllocationModels,
...
@Component({
	selector: 'cb-cash-allocation-admin-crud-edit',
	templateUrl: './cash-allocation-admin-crud-edit.component.html',
	styleUrls: [
		'./cash-allocation-admin-crud-edit.component.scss',
...
export class AdminCashAllocationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	cashAllocationAudits$: Observable<CashAllocationModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CashAllocationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: CashAllocationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCashAllocationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CashAllocationModel();
		...
		if (this.targetModelId) {
			this.cashAllocationAudits$ = this.store.select(getCashAllocationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CashAllocationModel> = {
		...
		this.store.dispatch(new modelAction.CashAllocationAction(
			modelAction.CashAllocationModelActionTypes.FETCH_CASH_ALLOCATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCashAllocationModelWithId, this.targetModelId)
			...
			model = new CashAllocationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CashAllocationAction(
			modelAction.CashAllocationModelActionTypes.FETCH_CASH_ALLOCATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.CashAllocationAction(
				modelAction.CashAllocationModelActionTypes.CREATE_CASH_ALLOCATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CashAllocationAction(
				modelAction.CashAllocationModelActionTypes.UPDATE_CASH_ALLOCATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CashAllocationModel = new CashAllocationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCashAllocationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CashAllocationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CashAllocationModel.getProps();
		this.modelRelations = CashAllocationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: CashAllocationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CashAllocationModel {
		let updatedModel: CashAllocationModel = new CashAllocationModel(model);
