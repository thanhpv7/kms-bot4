...
import * as modelAction from 'src/app/models/cashAllocation/cash_allocation.model.action';
import {CashAllocationModelState} from 'src/app/models/cashAllocation/cash_allocation.model.state';
...
import {
	getCashAllocationCollectionModels,
	getCashAllocationCollectionCount
} from 'src/app/models/cashAllocation/cash_allocation.model.selector';
...
import {CrudTileMode} from '../cash-allocation-admin-crud.component';
...
@Component({
	selector: 'cb-cash-allocation-admin-crud-list',
	templateUrl: './cash-allocation-admin-crud-list.component.html',
	styleUrls: [
		'./cash-allocation-admin-crud-list.component.scss',
...
export class AdminCashAllocationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CashAllocationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CashAllocationModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = CashAllocationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '6d271c13-c20b-45b3-aadd-a79b6bdf8d33';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.cashAllocationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.cashAllocationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: CashAllocationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashAllocationAction(
				modelAction.CashAllocationModelActionTypes.INITIALISE_CASH_ALLOCATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashAllocationAction(
				modelAction.CashAllocationModelActionTypes.FETCH_CASH_ALLOCATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.CashAllocationAction(
						modelAction.CashAllocationModelActionTypes.COUNT_CASH_ALLOCATIONS,
						null
					)
				]
			))
		...
		this.cashAllocations = this.store.select(getCashAllocationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCashAllocationCollectionCount, this.collectionId).subscribe(
				(count) => this.cashAllocationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CashAllocationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CashAllocationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CashAllocationModelActionTypes.EXPORT_ALL_CASH_ALLOCATION;
		} else {
			actionType = modelAction.CashAllocationModelActionTypes.EXPORT_CASH_ALLOCATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CashAllocationAction(
				modelAction.CashAllocationModelActionTypes.IMPORT_CASH_ALLOCATIONS,
				{
					file: file,
				},
				[new modelAction.CashAllocationAction(
			...
			window.open(environment.API_URL + '/api/cashallocation/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CashAllocationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Cash Allocation entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Cash Allocation entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CashAllocationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CashAllocationModelActionTypes.DELETE_CASH_ALLOCATION;
						} else if (usingSelected) {
							actionType = modelAction.CashAllocationModelActionTypes.DELETE_ALL_CASH_ALLOCATION;
						} else {
							actionType = modelAction.CashAllocationModelActionTypes.DELETE_CASH_ALLOCATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CashAllocationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CASH_ALLOCATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CASH_ALLOCATION and DELETE_CASH_ALLOCATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<CashAllocationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CashAllocationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.cashAllocationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
