...
import {AdminCashAllocationCrudListComponent} from './cash-allocation-admin-crud-list.component';
import {CashAllocationDataFactory} from 'src/app/lib/utils/factories/cash-allocation-data-factory';
import {CashAllocationModel} from 'src/app/models/cashAllocation/cash_allocation.model';
import {AdminCashAllocationCrudModule} from '../cash-allocation-admin-crud.module';
import {
	CashAllocationModelState,
	initialState as initialCashAllocationModelState
} from 'src/app/models/cashAllocation/cash_allocation.model.state';
 ...
 */
describe('Cash Allocation Crud List Tile Component', () => {

	let fixture;
	let cashAllocationCrudListComponent: AdminCashAllocationCrudListComponent;

	let store: MockStore<{ model: CashAllocationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let cashAllocationModelState: CashAllocationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<CashAllocationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6d271c13-c20b-45b3-aadd-a79b6bdf8d33';
	const searchContent = 'text';

	const searchableFieldCount = CashAllocationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'cash-allocation-admin-crud',
		urls: ['cash-allocation-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const cashAllocationDataFactory = new CashAllocationDataFactory();

	const defaultDataSize = 30;
	const defaultData = cashAllocationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: CashAllocationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		cashAllocationModelState = initialCashAllocationModelState;
		store.setState({model: cashAllocationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getCashAllocationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCashAllocationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminCashAllocationCrudListComponent);
			cashAllocationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(cashAllocationCrudListComponent.cashAllocationsCount).toBe(defaultData.length);
			...
			expect(cashAllocationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			cashAllocationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = cashAllocationDataFactory.createAll(filteredDataSize);
			...
			expect(cashAllocationCrudListComponent.cashAllocationsCount).toBe(filteredDataSize);
			expect(cashAllocationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(cashAllocationCrudListComponent.pageIndex).toBe(0);
			expect(cashAllocationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			cashAllocationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		cashAllocationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		cashAllocationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
