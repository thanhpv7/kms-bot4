...
import {AdminStaffCrudListComponent} from './staff-admin-crud-list.component';
import {StaffDataFactory} from 'src/app/lib/utils/factories/staff-data-factory';
import {StaffModel} from 'src/app/models/staff/staff.model';
import {AdminStaffCrudModule} from '../staff-admin-crud.module';
import {
	StaffModelState,
 ...
 */
describe('Staff Crud List Tile Component', () => {

	let fixture;
	let staffCrudListComponent: AdminStaffCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<StaffModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8f257cf4-7e6a-4b9e-b8f8-403fd34b6479';
	const searchContent = 'text';

	const searchableFieldCount = StaffModel.searchFields.length;

	const routerState: RouterState = {
		url: 'staff-admin-crud',
		urls: ['staff-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const staffDataFactory = new StaffDataFactory();

	const defaultDataSize = 30;
	const defaultData = staffDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getStaffCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getStaffCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminStaffCrudListComponent);
			staffCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(staffCrudListComponent.staffsCount).toBe(defaultData.length);
			...
			expect(staffCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			staffCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = staffDataFactory.createAll(filteredDataSize);
			...
			expect(staffCrudListComponent.staffsCount).toBe(filteredDataSize);
			expect(staffCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(staffCrudListComponent.pageIndex).toBe(0);
			expect(staffCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			staffCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		staffCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		staffCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
