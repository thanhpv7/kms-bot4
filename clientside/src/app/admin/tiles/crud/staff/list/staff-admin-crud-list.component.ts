...
import {StaffModelState} from 'src/app/models/staff/staff.model.state';
...
@Component({
	selector: 'cb-staff-admin-crud-list',
	templateUrl: './staff-admin-crud-list.component.html',
	styleUrls: [
		'./staff-admin-crud-list.component.scss',
...
export class AdminStaffCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<StaffModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...StaffModel.searchFields,
	...
	/**
	 * List of Staffs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.staffsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.staffsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StaffAction(
				modelAction.StaffModelActionTypes.INITIALISE_STAFF_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StaffAction(
				modelAction.StaffModelActionTypes.FETCH_STAFF_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.staffs = this.store.select(getStaffCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getStaffCollectionCount, this.collectionId).subscribe(
				(count) => this.staffsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: StaffModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.StaffModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.StaffModelActionTypes.EXPORT_ALL_STAFF;
		} else {
			actionType = modelAction.StaffModelActionTypes.EXPORT_STAFF_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.StaffAction(
				modelAction.StaffModelActionTypes.IMPORT_STAFFS,
				{
					file: file,
				},
				[new modelAction.StaffAction(
			...
			window.open(environment.API_URL + '/api/staff/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: StaffModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Staff entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Staff entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.StaffModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.StaffModelActionTypes.DELETE_STAFF;
						} else if (usingSelected) {
							actionType = modelAction.StaffModelActionTypes.DELETE_ALL_STAFF;
						} else {
							actionType = modelAction.StaffModelActionTypes.DELETE_STAFF_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.StaffAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_STAFF when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_STAFF and DELETE_STAFF_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = StaffModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.staffsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
