...
import {PatientPaymentOthersModelState} from 'src/app/models/patientPaymentOthers/patient_payment_others.model.state';
...
import {
	getPatientPaymentOthersCollectionModels,
	getPatientPaymentOthersCollectionCount
...
@Component({
	selector: 'cb-patient-payment-others-admin-crud-list',
	templateUrl: './patient-payment-others-admin-crud-list.component.html',
	styleUrls: [
		'./patient-payment-others-admin-crud-list.component.scss',
...
export class AdminPatientPaymentOthersCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientPaymentOthersModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientPaymentOthersModel.searchFields,
	...
	/**
	 * List of Patient Payment Otherss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientPaymentOtherss: Observable<PatientPaymentOthersModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientPaymentOtherssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientPaymentOtherssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientPaymentOtherssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentOthersAction(
				modelAction.PatientPaymentOthersModelActionTypes.INITIALISE_PATIENT_PAYMENT_OTHERS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentOthersAction(
				modelAction.PatientPaymentOthersModelActionTypes.FETCH_PATIENT_PAYMENT_OTHERS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientPaymentOtherss = this.store.select(getPatientPaymentOthersCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientPaymentOthersCollectionCount, this.collectionId).subscribe(
				(count) => this.patientPaymentOtherssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientPaymentOthersModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientPaymentOthersModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientPaymentOthersModelActionTypes.EXPORT_ALL_PATIENT_PAYMENT_OTHERS;
		} else {
			actionType = modelAction.PatientPaymentOthersModelActionTypes.EXPORT_PATIENT_PAYMENT_OTHERS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientPaymentOthersAction(
				modelAction.PatientPaymentOthersModelActionTypes.IMPORT_PATIENT_PAYMENT_OTHERSS,
				{
					file: file,
				},
				[new modelAction.PatientPaymentOthersAction(
			...
			window.open(environment.API_URL + '/api/patientpaymentothers/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientPaymentOthersModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment Others entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment Others entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientPaymentOthersModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientPaymentOthersModelActionTypes.DELETE_PATIENT_PAYMENT_OTHERS;
						} else if (usingSelected) {
							actionType = modelAction.PatientPaymentOthersModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_OTHERS;
						} else {
							actionType = modelAction.PatientPaymentOthersModelActionTypes.DELETE_PATIENT_PAYMENT_OTHERS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientPaymentOthersAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_PAYMENT_OTHERS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_PAYMENT_OTHERS and DELETE_PATIENT_PAYMENT_OTHERS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientPaymentOthersModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientPaymentOtherssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
