...
import {AdminPatientPaymentOthersCrudListComponent} from './patient-payment-others-admin-crud-list.component';
...
import {AdminPatientPaymentOthersCrudModule} from '../patient-payment-others-admin-crud.module';
import {
	PatientPaymentOthersModelState,
	initialState as initialPatientPaymentOthersModelState
} from 'src/app/models/patientPaymentOthers/patient_payment_others.model.state';
 ...
 */
describe('Patient Payment Others Crud List Tile Component', () => {

	let fixture;
	let patientPaymentOthersCrudListComponent: AdminPatientPaymentOthersCrudListComponent;

	let store: MockStore<{ model: PatientPaymentOthersModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientPaymentOthersModelState: PatientPaymentOthersModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientPaymentOthersModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'eb0ac11a-1bcd-469d-9d9a-83308fa60425';
	const searchContent = 'text';

	const searchableFieldCount = PatientPaymentOthersModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-payment-others-admin-crud',
		urls: ['patient-payment-others-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientPaymentOthersDataFactory = new PatientPaymentOthersDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientPaymentOthersDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientPaymentOthersModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientPaymentOthersModelState = initialPatientPaymentOthersModelState;
		store.setState({model: patientPaymentOthersModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientPaymentOthersCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientPaymentOthersCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientPaymentOthersCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientPaymentOthersCrudListComponent);
			patientPaymentOthersCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientPaymentOthersCrudListComponent.patientPaymentOtherssCount).toBe(defaultData.length);
			...
			expect(patientPaymentOthersCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientPaymentOthersCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientPaymentOthersDataFactory.createAll(filteredDataSize);
			...
			expect(patientPaymentOthersCrudListComponent.patientPaymentOtherssCount).toBe(filteredDataSize);
			expect(patientPaymentOthersCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientPaymentOthersCrudListComponent.pageIndex).toBe(0);
			expect(patientPaymentOthersCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientPaymentOthersCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientPaymentOthersCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientPaymentOthersCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
