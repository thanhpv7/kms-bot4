...
import * as modelAction from 'src/app/models/patientPaymentOthers/patient_payment_others.model.action';
import {PatientPaymentOthersModelState, PatientPaymentOthersModelAudit} from 'src/app/models/patientPaymentOthers/patient_payment_others.model.state';
import {PatientPaymentOthersModel} from 'src/app/models/patientPaymentOthers/patient_payment_others.model';
import {
	getPatientPaymentOthersModelAuditsByEntityId,
	getPatientPaymentOthersModelWithId,
	getCountPatientPaymentOthersModels,
...
import {CrudTileMode} from '../patient-payment-others-admin-crud.component';
...
@Component({
	selector: 'cb-patient-payment-others-admin-crud-edit',
	templateUrl: './patient-payment-others-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-payment-others-admin-crud-edit.component.scss',
...
export class AdminPatientPaymentOthersCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'eb0ac11a-1bcd-469d-9d9a-83308fa60425';
	 ...
	 */
	patientPaymentOthersAudits$: Observable<PatientPaymentOthersModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientPaymentTypeOthers';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientPaymentOthersModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientPaymentOthersModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientPaymentOthersModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientPaymentOthersModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientPaymentOthersModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientPaymentOthersModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientPaymentOthersModel();
		...
		if (this.targetModelId) {
			this.patientPaymentOthersAudits$ = this.store.select(getPatientPaymentOthersModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientPaymentOthersModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientPaymentOthersAction(
			modelAction.PatientPaymentOthersModelActionTypes.FETCH_PATIENT_PAYMENT_OTHERS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientPaymentOthersModelWithId, this.targetModelId)
			...
			model = new PatientPaymentOthersModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-payment-others');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientPaymentOthersAction(
			modelAction.PatientPaymentOthersModelActionTypes.FETCH_PATIENT_PAYMENT_OTHERS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientPaymentOthersModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientPaymentOthersAction(
						modelAction.PatientPaymentOthersModelActionTypes.COUNT_PATIENT_PAYMENT_OTHERSS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientPaymentOthersAction(
				modelAction.PatientPaymentOthersModelActionTypes.CREATE_PATIENT_PAYMENT_OTHERS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientPaymentOthersAction(
				modelAction.PatientPaymentOthersModelActionTypes.UPDATE_PATIENT_PAYMENT_OTHERS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientPaymentOthersModel = new PatientPaymentOthersModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientPaymentOthersModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientPaymentOthersModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientPaymentOthersModel.getProps();
		this.modelRelations = PatientPaymentOthersModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientPaymentOthersModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientPaymentOthersModel {
		let updatedModel: PatientPaymentOthersModel = new PatientPaymentOthersModel(model);
