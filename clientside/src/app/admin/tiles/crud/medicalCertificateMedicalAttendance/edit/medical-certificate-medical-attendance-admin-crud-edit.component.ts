...
import * as modelAction from 'src/app/models/medicalCertificateMedicalAttendance/medical_certificate_medical_attendance.model.action';
import {MedicalCertificateMedicalAttendanceModelState, MedicalCertificateMedicalAttendanceModelAudit} from 'src/app/models/medicalCertificateMedicalAttendance/medical_certificate_medical_attendance.model.state';
...
import {
	getMedicalCertificateMedicalAttendanceModelAuditsByEntityId,
	getMedicalCertificateMedicalAttendanceModelWithId,
	getCountMedicalCertificateMedicalAttendanceModels,
...
import {CrudTileMode} from '../medical-certificate-medical-attendance-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-medical-attendance-admin-crud-edit',
	templateUrl: './medical-certificate-medical-attendance-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-medical-attendance-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificateMedicalAttendanceCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '0964a9f9-9c13-4e40-a03e-cf6232687a3e';
	 ...
	 */
	medicalCertificateMedicalAttendanceAudits$: Observable<MedicalCertificateMedicalAttendanceModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificateMedicalAttendance';
	...
	medicalStaffOppositeRelation = 'medicalStaffOfMedicalAttendancesCertificate';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificateMedicalAttendanceModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificateMedicalAttendanceModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificateMedicalAttendanceModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalCertificateMedicalAttendanceModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificateMedicalAttendanceModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificateMedicalAttendanceModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificateMedicalAttendanceModel();
		...
		if (this.targetModelId) {
			this.medicalCertificateMedicalAttendanceAudits$ = this.store.select(getMedicalCertificateMedicalAttendanceModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificateMedicalAttendanceModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
			modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.FETCH_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateMedicalAttendanceModelWithId, this.targetModelId)
			...
			model = new MedicalCertificateMedicalAttendanceModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-medical-attendance');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
			modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.FETCH_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificateMedicalAttendanceModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.registrationMultiCrudActive || this.medicalStaffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificateMedicalAttendanceAction(
						modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.COUNT_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
				modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.CREATE_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
				modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificateMedicalAttendanceModel = new MedicalCertificateMedicalAttendanceModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateMedicalAttendanceModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificateMedicalAttendanceModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificateMedicalAttendanceModel.getProps();
		this.modelRelations = MedicalCertificateMedicalAttendanceModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificateMedicalAttendanceModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificateMedicalAttendanceModel {
		let updatedModel: MedicalCertificateMedicalAttendanceModel = new MedicalCertificateMedicalAttendanceModel(model);
