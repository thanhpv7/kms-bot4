...
import {MedicalCertificateMedicalAttendanceModelState} from 'src/app/models/medicalCertificateMedicalAttendance/medical_certificate_medical_attendance.model.state';
...
import {
	getMedicalCertificateMedicalAttendanceCollectionModels,
	getMedicalCertificateMedicalAttendanceCollectionCount
...
@Component({
	selector: 'cb-medical-certificate-medical-attendance-admin-crud-list',
	templateUrl: './medical-certificate-medical-attendance-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-medical-attendance-admin-crud-list.component.scss',
...
export class AdminMedicalCertificateMedicalAttendanceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificateMedicalAttendanceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificateMedicalAttendanceModel.searchFields,
	...
	/**
	 * List of Medical Certificate Medical Attendances that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificateMedicalAttendances: Observable<MedicalCertificateMedicalAttendanceModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificateMedicalAttendancesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificateMedicalAttendancesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificateMedicalAttendancesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
				modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
				modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.FETCH_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificateMedicalAttendances = this.store.select(getMedicalCertificateMedicalAttendanceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificateMedicalAttendanceCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificateMedicalAttendancesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificateMedicalAttendanceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificateMedicalAttendanceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE;
		} else {
			actionType = modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
				modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCES,
				{
					file: file,
				},
				[new modelAction.MedicalCertificateMedicalAttendanceAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificatemedicalattendance/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificateMedicalAttendanceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Medical Attendance entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Medical Attendance entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificateMedicalAttendanceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.DELETE_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE;
						} else {
							actionType = modelAction.MedicalCertificateMedicalAttendanceModelActionTypes.DELETE_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificateMedicalAttendanceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE and DELETE_MEDICAL_CERTIFICATE_MEDICAL_ATTENDANCE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificateMedicalAttendanceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificateMedicalAttendancesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
