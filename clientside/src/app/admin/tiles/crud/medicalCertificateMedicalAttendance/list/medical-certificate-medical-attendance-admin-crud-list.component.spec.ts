...
import {AdminMedicalCertificateMedicalAttendanceCrudListComponent} from './medical-certificate-medical-attendance-admin-crud-list.component';
...
import {AdminMedicalCertificateMedicalAttendanceCrudModule} from '../medical-certificate-medical-attendance-admin-crud.module';
import {
	MedicalCertificateMedicalAttendanceModelState,
	initialState as initialMedicalCertificateMedicalAttendanceModelState
} from 'src/app/models/medicalCertificateMedicalAttendance/medical_certificate_medical_attendance.model.state';
 ...
 */
describe('Medical Certificate Medical Attendance Crud List Tile Component', () => {

	let fixture;
	let medicalCertificateMedicalAttendanceCrudListComponent: AdminMedicalCertificateMedicalAttendanceCrudListComponent;

	let store: MockStore<{ model: MedicalCertificateMedicalAttendanceModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalCertificateMedicalAttendanceModelState: MedicalCertificateMedicalAttendanceModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalCertificateMedicalAttendanceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '0964a9f9-9c13-4e40-a03e-cf6232687a3e';
	const searchContent = 'text';

	const searchableFieldCount = MedicalCertificateMedicalAttendanceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-certificate-medical-attendance-admin-crud',
		urls: ['medical-certificate-medical-attendance-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalCertificateMedicalAttendanceDataFactory = new MedicalCertificateMedicalAttendanceDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalCertificateMedicalAttendanceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalCertificateMedicalAttendanceModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalCertificateMedicalAttendanceModelState = initialMedicalCertificateMedicalAttendanceModelState;
		store.setState({model: medicalCertificateMedicalAttendanceModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalCertificateMedicalAttendanceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalCertificateMedicalAttendanceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalCertificateMedicalAttendanceCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalCertificateMedicalAttendanceCrudListComponent);
			medicalCertificateMedicalAttendanceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalCertificateMedicalAttendanceCrudListComponent.medicalCertificateMedicalAttendancesCount).toBe(defaultData.length);
			...
			expect(medicalCertificateMedicalAttendanceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalCertificateMedicalAttendanceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalCertificateMedicalAttendanceDataFactory.createAll(filteredDataSize);
			...
			expect(medicalCertificateMedicalAttendanceCrudListComponent.medicalCertificateMedicalAttendancesCount).toBe(filteredDataSize);
			expect(medicalCertificateMedicalAttendanceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalCertificateMedicalAttendanceCrudListComponent.pageIndex).toBe(0);
			expect(medicalCertificateMedicalAttendanceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalCertificateMedicalAttendanceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalCertificateMedicalAttendanceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalCertificateMedicalAttendanceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
