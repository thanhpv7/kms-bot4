...
import {EntExaminationModelState, EntExaminationModelAudit} from 'src/app/models/entExamination/ent_examination.model.state';
import {EntExaminationModel} from 'src/app/models/entExamination/ent_examination.model';
import {
	getEntExaminationModelAuditsByEntityId,
	getEntExaminationModelWithId,
	getCountEntExaminationModels,
...
@Component({
	selector: 'cb-ent-examination-admin-crud-edit',
	templateUrl: './ent-examination-admin-crud-edit.component.html',
	styleUrls: [
		'./ent-examination-admin-crud-edit.component.scss',
...
export class AdminEntExaminationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	entExaminationAudits$: Observable<EntExaminationModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'entExamination';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = EntExaminationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: EntExaminationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountEntExaminationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new EntExaminationModel();
		...
		if (this.targetModelId) {
			this.entExaminationAudits$ = this.store.select(getEntExaminationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<EntExaminationModel> = {
		...
		this.store.dispatch(new modelAction.EntExaminationAction(
			modelAction.EntExaminationModelActionTypes.FETCH_ENT_EXAMINATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getEntExaminationModelWithId, this.targetModelId)
			...
			model = new EntExaminationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.EntExaminationAction(
			modelAction.EntExaminationModelActionTypes.FETCH_ENT_EXAMINATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.EntExaminationAction(
				modelAction.EntExaminationModelActionTypes.CREATE_ENT_EXAMINATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.EntExaminationAction(
				modelAction.EntExaminationModelActionTypes.UPDATE_ENT_EXAMINATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: EntExaminationModel = new EntExaminationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getEntExaminationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new EntExaminationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = EntExaminationModel.getProps();
		this.modelRelations = EntExaminationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: EntExaminationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): EntExaminationModel {
		let updatedModel: EntExaminationModel = new EntExaminationModel(model);
