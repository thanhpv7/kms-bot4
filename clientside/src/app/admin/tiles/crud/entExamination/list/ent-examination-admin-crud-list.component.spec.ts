...
import {AdminEntExaminationCrudListComponent} from './ent-examination-admin-crud-list.component';
...
import {AdminEntExaminationCrudModule} from '../ent-examination-admin-crud.module';
import {
	EntExaminationModelState,
	initialState as initialEntExaminationModelState
} from 'src/app/models/entExamination/ent_examination.model.state';
 ...
 */
describe('ENT Examination Crud List Tile Component', () => {

	let fixture;
	let entExaminationCrudListComponent: AdminEntExaminationCrudListComponent;

	let store: MockStore<{ model: EntExaminationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let entExaminationModelState: EntExaminationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<EntExaminationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '0df7f5f9-84cf-4df2-bade-2b986ebd60b9';
	const searchContent = 'text';

	const searchableFieldCount = EntExaminationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'ent-examination-admin-crud',
		urls: ['ent-examination-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const entExaminationDataFactory = new EntExaminationDataFactory();

	const defaultDataSize = 30;
	const defaultData = entExaminationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: EntExaminationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		entExaminationModelState = initialEntExaminationModelState;
		store.setState({model: entExaminationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getEntExaminationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getEntExaminationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminEntExaminationCrudListComponent);
			entExaminationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(entExaminationCrudListComponent.entExaminationsCount).toBe(defaultData.length);
			...
			expect(entExaminationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			entExaminationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = entExaminationDataFactory.createAll(filteredDataSize);
			...
			expect(entExaminationCrudListComponent.entExaminationsCount).toBe(filteredDataSize);
			expect(entExaminationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(entExaminationCrudListComponent.pageIndex).toBe(0);
			expect(entExaminationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			entExaminationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		entExaminationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		entExaminationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
