...
import * as modelAction from 'src/app/models/entExamination/ent_examination.model.action';
import {EntExaminationModelState} from 'src/app/models/entExamination/ent_examination.model.state';
...
import {
	getEntExaminationCollectionModels,
	getEntExaminationCollectionCount
...
import {CrudTileMode} from '../ent-examination-admin-crud.component';
...
@Component({
	selector: 'cb-ent-examination-admin-crud-list',
	templateUrl: './ent-examination-admin-crud-list.component.html',
	styleUrls: [
		'./ent-examination-admin-crud-list.component.scss',
...
export class AdminEntExaminationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<EntExaminationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...EntExaminationModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = EntExaminationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '0df7f5f9-84cf-4df2-bade-2b986ebd60b9';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.entExaminationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.entExaminationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: EntExaminationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.EntExaminationAction(
				modelAction.EntExaminationModelActionTypes.INITIALISE_ENT_EXAMINATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.EntExaminationAction(
				modelAction.EntExaminationModelActionTypes.FETCH_ENT_EXAMINATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.EntExaminationAction(
						modelAction.EntExaminationModelActionTypes.COUNT_ENT_EXAMINATIONS,
						null
					)
				]
			))
		...
		this.entExaminations = this.store.select(getEntExaminationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getEntExaminationCollectionCount, this.collectionId).subscribe(
				(count) => this.entExaminationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: EntExaminationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.EntExaminationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.EntExaminationModelActionTypes.EXPORT_ALL_ENT_EXAMINATION;
		} else {
			actionType = modelAction.EntExaminationModelActionTypes.EXPORT_ENT_EXAMINATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.EntExaminationAction(
				modelAction.EntExaminationModelActionTypes.IMPORT_ENT_EXAMINATIONS,
				{
					file: file,
				},
				[new modelAction.EntExaminationAction(
			...
			window.open(environment.API_URL + '/api/entexamination/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: EntExaminationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected ENT Examination entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected ENT Examination entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.EntExaminationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.EntExaminationModelActionTypes.DELETE_ENT_EXAMINATION;
						} else if (usingSelected) {
							actionType = modelAction.EntExaminationModelActionTypes.DELETE_ALL_ENT_EXAMINATION;
						} else {
							actionType = modelAction.EntExaminationModelActionTypes.DELETE_ENT_EXAMINATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.EntExaminationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ENT_EXAMINATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ENT_EXAMINATION and DELETE_ENT_EXAMINATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<EntExaminationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = EntExaminationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.entExaminationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
