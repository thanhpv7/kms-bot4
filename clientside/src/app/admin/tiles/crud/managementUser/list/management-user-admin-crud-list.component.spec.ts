...
import {AdminManagementUserCrudListComponent} from './management-user-admin-crud-list.component';
...
import {AdminManagementUserCrudModule} from '../management-user-admin-crud.module';
import {
	ManagementUserModelState,
	initialState as initialManagementUserModelState
} from 'src/app/models/managementUser/management_user.model.state';
 ...
 */
describe('Management User Crud List Tile Component', () => {

	let fixture;
	let managementUserCrudListComponent: AdminManagementUserCrudListComponent;

	let store: MockStore<{ model: ManagementUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let managementUserModelState: ManagementUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<ManagementUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd11bda8a-281a-4ab5-90fc-c33229799508';
	const searchContent = 'text';

	const searchableFieldCount = ManagementUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'management-user-admin-crud',
		urls: ['management-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const managementUserDataFactory = new ManagementUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = managementUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: ManagementUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		managementUserModelState = initialManagementUserModelState;
		store.setState({model: managementUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getManagementUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getManagementUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminManagementUserCrudListComponent);
			managementUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(managementUserCrudListComponent.managementUsersCount).toBe(defaultData.length);
			...
			expect(managementUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			managementUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = managementUserDataFactory.createAll(filteredDataSize);
			...
			expect(managementUserCrudListComponent.managementUsersCount).toBe(filteredDataSize);
			expect(managementUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(managementUserCrudListComponent.pageIndex).toBe(0);
			expect(managementUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			managementUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		managementUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		managementUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
