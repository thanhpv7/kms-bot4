...
import * as modelAction from 'src/app/models/managementUser/management_user.model.action';
import {ManagementUserModelState} from 'src/app/models/managementUser/management_user.model.state';
...
import {
	getManagementUserCollectionModels,
	getManagementUserCollectionCount
...
import {CrudTileMode} from '../management-user-admin-crud.component';
...
@Component({
	selector: 'cb-management-user-admin-crud-list',
	templateUrl: './management-user-admin-crud-list.component.html',
	styleUrls: [
		'./management-user-admin-crud-list.component.scss',
...
export class AdminManagementUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ManagementUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ManagementUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = ManagementUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'd11bda8a-281a-4ab5-90fc-c33229799508';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.managementUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.managementUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ManagementUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ManagementUserAction(
				modelAction.ManagementUserModelActionTypes.INITIALISE_MANAGEMENT_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ManagementUserAction(
				modelAction.ManagementUserModelActionTypes.FETCH_MANAGEMENT_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ManagementUserAction(
						modelAction.ManagementUserModelActionTypes.COUNT_MANAGEMENT_USERS,
						null
					)
				]
			))
		...
		this.managementUsers = this.store.select(getManagementUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getManagementUserCollectionCount, this.collectionId).subscribe(
				(count) => this.managementUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ManagementUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ManagementUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ManagementUserModelActionTypes.EXPORT_ALL_MANAGEMENT_USER;
		} else {
			actionType = modelAction.ManagementUserModelActionTypes.EXPORT_MANAGEMENT_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ManagementUserAction(
				modelAction.ManagementUserModelActionTypes.IMPORT_MANAGEMENT_USERS,
				{
					file: file,
				},
				[new modelAction.ManagementUserAction(
			...
			window.open(environment.API_URL + '/api/managementuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ManagementUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Management User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Management User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ManagementUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ManagementUserModelActionTypes.DELETE_MANAGEMENT_USER;
						} else if (usingSelected) {
							actionType = modelAction.ManagementUserModelActionTypes.DELETE_ALL_MANAGEMENT_USER;
						} else {
							actionType = modelAction.ManagementUserModelActionTypes.DELETE_MANAGEMENT_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ManagementUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MANAGEMENT_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MANAGEMENT_USER and DELETE_MANAGEMENT_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ManagementUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ManagementUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.managementUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
