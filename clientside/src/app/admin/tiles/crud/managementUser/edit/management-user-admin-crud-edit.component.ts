...
import {ManagementUserModelState, ManagementUserModelAudit} from 'src/app/models/managementUser/management_user.model.state';
import {ManagementUserModel} from 'src/app/models/managementUser/management_user.model';
import {
	getManagementUserModelAuditsByEntityId,
	getManagementUserModelWithId,
	getCountManagementUserModels,
...
@Component({
	selector: 'cb-management-user-admin-crud-edit',
	templateUrl: './management-user-admin-crud-edit.component.html',
	styleUrls: [
		'./management-user-admin-crud-edit.component.scss',
...
export class AdminManagementUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	managementUserAudits$: Observable<ManagementUserModelAudit[]>;
	...
	staffOppositeRelation = 'managementUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ManagementUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ManagementUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountManagementUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ManagementUserModel();
		...
		if (this.targetModelId) {
			this.managementUserAudits$ = this.store.select(getManagementUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ManagementUserModel> = {
		...
		this.store.dispatch(new modelAction.ManagementUserAction(
			modelAction.ManagementUserModelActionTypes.FETCH_MANAGEMENT_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getManagementUserModelWithId, this.targetModelId)
			...
			model = new ManagementUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ManagementUserAction(
			modelAction.ManagementUserModelActionTypes.FETCH_MANAGEMENT_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.ManagementUserAction(
				modelAction.ManagementUserModelActionTypes.CREATE_MANAGEMENT_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ManagementUserAction(
				modelAction.ManagementUserModelActionTypes.UPDATE_MANAGEMENT_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ManagementUserModel = new ManagementUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getManagementUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ManagementUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ManagementUserModel.getProps();
		this.modelRelations = ManagementUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ManagementUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ManagementUserModel {
		let updatedModel: ManagementUserModel = new ManagementUserModel(model);
