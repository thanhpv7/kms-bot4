...
import {AdminRegistrationUserCrudListComponent} from './registration-user-admin-crud-list.component';
...
import {AdminRegistrationUserCrudModule} from '../registration-user-admin-crud.module';
import {
	RegistrationUserModelState,
	initialState as initialRegistrationUserModelState
} from 'src/app/models/registrationUser/registration_user.model.state';
 ...
 */
describe('Registration User Crud List Tile Component', () => {

	let fixture;
	let registrationUserCrudListComponent: AdminRegistrationUserCrudListComponent;

	let store: MockStore<{ model: RegistrationUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let registrationUserModelState: RegistrationUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<RegistrationUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'fc651984-a60a-4edb-afd0-c123f6e1de84';
	const searchContent = 'text';

	const searchableFieldCount = RegistrationUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'registration-user-admin-crud',
		urls: ['registration-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const registrationUserDataFactory = new RegistrationUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = registrationUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: RegistrationUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		registrationUserModelState = initialRegistrationUserModelState;
		store.setState({model: registrationUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getRegistrationUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getRegistrationUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminRegistrationUserCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminRegistrationUserCrudListComponent);
			registrationUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(registrationUserCrudListComponent.registrationUsersCount).toBe(defaultData.length);
			...
			expect(registrationUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			registrationUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = registrationUserDataFactory.createAll(filteredDataSize);
			...
			expect(registrationUserCrudListComponent.registrationUsersCount).toBe(filteredDataSize);
			expect(registrationUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(registrationUserCrudListComponent.pageIndex).toBe(0);
			expect(registrationUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			registrationUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		registrationUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		registrationUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
