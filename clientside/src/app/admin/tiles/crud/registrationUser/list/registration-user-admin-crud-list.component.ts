...
import * as modelAction from 'src/app/models/registrationUser/registration_user.model.action';
import {RegistrationUserModelState} from 'src/app/models/registrationUser/registration_user.model.state';
...
import {
	getRegistrationUserCollectionModels,
	getRegistrationUserCollectionCount
...
import {CrudTileMode} from '../registration-user-admin-crud.component';
...
@Component({
	selector: 'cb-registration-user-admin-crud-list',
	templateUrl: './registration-user-admin-crud-list.component.html',
	styleUrls: [
		'./registration-user-admin-crud-list.component.scss',
...
export class AdminRegistrationUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<RegistrationUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...RegistrationUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = RegistrationUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'fc651984-a60a-4edb-afd0-c123f6e1de84';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.registrationUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.registrationUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: RegistrationUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RegistrationUserAction(
				modelAction.RegistrationUserModelActionTypes.INITIALISE_REGISTRATION_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RegistrationUserAction(
				modelAction.RegistrationUserModelActionTypes.FETCH_REGISTRATION_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.RegistrationUserAction(
						modelAction.RegistrationUserModelActionTypes.COUNT_REGISTRATION_USERS,
						null
					)
				]
			))
		...
		this.registrationUsers = this.store.select(getRegistrationUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getRegistrationUserCollectionCount, this.collectionId).subscribe(
				(count) => this.registrationUsersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('admin');
			commands.push('users');
			commands.push('registration-user');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: RegistrationUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.RegistrationUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.RegistrationUserModelActionTypes.EXPORT_ALL_REGISTRATION_USER;
		} else {
			actionType = modelAction.RegistrationUserModelActionTypes.EXPORT_REGISTRATION_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.RegistrationUserAction(
				modelAction.RegistrationUserModelActionTypes.IMPORT_REGISTRATION_USERS,
				{
					file: file,
				},
				[new modelAction.RegistrationUserAction(
			...
			window.open(environment.API_URL + '/api/registrationuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: RegistrationUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Registration User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Registration User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.RegistrationUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.RegistrationUserModelActionTypes.DELETE_REGISTRATION_USER;
						} else if (usingSelected) {
							actionType = modelAction.RegistrationUserModelActionTypes.DELETE_ALL_REGISTRATION_USER;
						} else {
							actionType = modelAction.RegistrationUserModelActionTypes.DELETE_REGISTRATION_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.RegistrationUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_REGISTRATION_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_REGISTRATION_USER and DELETE_REGISTRATION_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<RegistrationUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = RegistrationUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.registrationUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
