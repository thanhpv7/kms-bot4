...
import {RegistrationUserModelState, RegistrationUserModelAudit} from 'src/app/models/registrationUser/registration_user.model.state';
import {RegistrationUserModel} from 'src/app/models/registrationUser/registration_user.model';
import {
	getRegistrationUserModelAuditsByEntityId,
	getRegistrationUserModelWithId,
	getCountRegistrationUserModels,
...
@Component({
	selector: 'cb-registration-user-admin-crud-edit',
	templateUrl: './registration-user-admin-crud-edit.component.html',
	styleUrls: [
		'./registration-user-admin-crud-edit.component.scss',
...
export class AdminRegistrationUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	registrationUserAudits$: Observable<RegistrationUserModelAudit[]>;
	...
	staffOppositeRelation = 'registrationUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = RegistrationUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: RegistrationUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountRegistrationUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new RegistrationUserModel();
		...
		if (this.targetModelId) {
			this.registrationUserAudits$ = this.store.select(getRegistrationUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<RegistrationUserModel> = {
		...
		this.store.dispatch(new modelAction.RegistrationUserAction(
			modelAction.RegistrationUserModelActionTypes.FETCH_REGISTRATION_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getRegistrationUserModelWithId, this.targetModelId)
			...
			model = new RegistrationUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.RegistrationUserAction(
			modelAction.RegistrationUserModelActionTypes.FETCH_REGISTRATION_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.RegistrationUserAction(
				modelAction.RegistrationUserModelActionTypes.CREATE_REGISTRATION_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.RegistrationUserAction(
				modelAction.RegistrationUserModelActionTypes.UPDATE_REGISTRATION_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: RegistrationUserModel = new RegistrationUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getRegistrationUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new RegistrationUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = RegistrationUserModel.getProps();
		this.modelRelations = RegistrationUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: RegistrationUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): RegistrationUserModel {
		let updatedModel: RegistrationUserModel = new RegistrationUserModel(model);
