...
import * as modelAction from 'src/app/models/bpjsReferralSpecialist/bpjs_referral_specialist.model.action';
import {BpjsReferralSpecialistModelState} from 'src/app/models/bpjsReferralSpecialist/bpjs_referral_specialist.model.state';
...
import {CrudTileMode} from '../bpjs-referral-specialist-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-referral-specialist-admin-crud-list',
	templateUrl: './bpjs-referral-specialist-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-referral-specialist-admin-crud-list.component.scss',
...
export class AdminBpjsReferralSpecialistCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsReferralSpecialistModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsReferralSpecialistModel.searchFields,
	...
	/**
	 * List of BPJS Referral Specialists that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsReferralSpecialists: Observable<BpjsReferralSpecialistModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsReferralSpecialistsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsReferralSpecialistModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsReferralSpecialistModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'f052f86d-bb48-4fef-a5d3-b5076103c0b3';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsReferralSpecialistsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsReferralSpecialistsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsReferralSpecialistModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
				modelAction.BpjsReferralSpecialistModelActionTypes.INITIALISE_BPJS_REFERRAL_SPECIALIST_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
				modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_BPJS_REFERRAL_SPECIALIST_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsReferralSpecialistAction(
						modelAction.BpjsReferralSpecialistModelActionTypes.COUNT_BPJS_REFERRAL_SPECIALISTS,
						null
					)
				]
			))
		...
		this.bpjsReferralSpecialists = this.store.select(getBpjsReferralSpecialistCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsReferralSpecialistCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsReferralSpecialistsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-referral-specialist');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsReferralSpecialistModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsReferralSpecialistModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsReferralSpecialistModelActionTypes.EXPORT_ALL_BPJS_REFERRAL_SPECIALIST;
		} else {
			actionType = modelAction.BpjsReferralSpecialistModelActionTypes.EXPORT_BPJS_REFERRAL_SPECIALIST_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
				modelAction.BpjsReferralSpecialistModelActionTypes.IMPORT_BPJS_REFERRAL_SPECIALISTS,
				{
					file: file,
				},
				[new modelAction.BpjsReferralSpecialistAction(
			...
			window.open(environment.API_URL + '/api/bpjsreferralspecialist/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsReferralSpecialistModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Referral Specialist entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Referral Specialist entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsReferralSpecialistModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsReferralSpecialistModelActionTypes.DELETE_BPJS_REFERRAL_SPECIALIST;
						} else if (usingSelected) {
							actionType = modelAction.BpjsReferralSpecialistModelActionTypes.DELETE_ALL_BPJS_REFERRAL_SPECIALIST;
						} else {
							actionType = modelAction.BpjsReferralSpecialistModelActionTypes.DELETE_BPJS_REFERRAL_SPECIALIST_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_REFERRAL_SPECIALIST when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_REFERRAL_SPECIALIST and DELETE_BPJS_REFERRAL_SPECIALIST_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsReferralSpecialistModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsReferralSpecialistModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsReferralSpecialistsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
