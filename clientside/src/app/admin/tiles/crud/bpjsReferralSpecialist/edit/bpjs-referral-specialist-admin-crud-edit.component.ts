...
import {BpjsReferralSpecialistModelState, BpjsReferralSpecialistModelAudit} from 'src/app/models/bpjsReferralSpecialist/bpjs_referral_specialist.model.state';
import {BpjsReferralSpecialistModel} from 'src/app/models/bpjsReferralSpecialist/bpjs_referral_specialist.model';
import {
	getBpjsReferralSpecialistModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-referral-specialist-admin-crud-edit',
	templateUrl: './bpjs-referral-specialist-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-referral-specialist-admin-crud-edit.component.scss',
...
export class AdminBpjsReferralSpecialistCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsReferralSpecialistAudits$: Observable<BpjsReferralSpecialistModelAudit[]>;
	...
	bpjsHealthFacilityOppositeRelation = 'bpjsReferralSpecialists';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsReferralSpecialistModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsReferralSpecialistModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsReferralSpecialistModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsReferralSpecialistModel();
		...
		if (this.targetModelId) {
			this.bpjsReferralSpecialistAudits$ = this.store.select(getBpjsReferralSpecialistModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsReferralSpecialistModel> = {
		...
		this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
			modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_BPJS_REFERRAL_SPECIALIST,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsReferralSpecialistModelWithId, this.targetModelId)
			...
			model = new BpjsReferralSpecialistModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
			modelAction.BpjsReferralSpecialistModelActionTypes.FETCH_BPJS_REFERRAL_SPECIALIST_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsReferralSpecialistModel> = {
				...
				queryParams: {
					persistRelations: this.bpjsHealthFacilityMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
				modelAction.BpjsReferralSpecialistModelActionTypes.CREATE_BPJS_REFERRAL_SPECIALIST,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsReferralSpecialistAction(
				modelAction.BpjsReferralSpecialistModelActionTypes.UPDATE_BPJS_REFERRAL_SPECIALIST,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsReferralSpecialistModel = new BpjsReferralSpecialistModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsReferralSpecialistModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsReferralSpecialistModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsReferralSpecialistModel.getProps();
		this.modelRelations = BpjsReferralSpecialistModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsReferralSpecialistModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsReferralSpecialistModel {
		let updatedModel: BpjsReferralSpecialistModel = new BpjsReferralSpecialistModel(model);
