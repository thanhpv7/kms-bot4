...
import {MedicalTranscriberUserModelState, MedicalTranscriberUserModelAudit} from 'src/app/models/medicalTranscriberUser/medical_transcriber_user.model.state';
import {MedicalTranscriberUserModel} from 'src/app/models/medicalTranscriberUser/medical_transcriber_user.model';
import {
	getMedicalTranscriberUserModelAuditsByEntityId,
	getMedicalTranscriberUserModelWithId,
	getCountMedicalTranscriberUserModels,
...
@Component({
	selector: 'cb-medical-transcriber-user-admin-crud-edit',
	templateUrl: './medical-transcriber-user-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-transcriber-user-admin-crud-edit.component.scss',
...
export class AdminMedicalTranscriberUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	medicalTranscriberUserAudits$: Observable<MedicalTranscriberUserModelAudit[]>;
	...
	staffOppositeRelation = 'medicalTranscriberUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalTranscriberUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalTranscriberUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalTranscriberUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalTranscriberUserModel();
		...
		if (this.targetModelId) {
			this.medicalTranscriberUserAudits$ = this.store.select(getMedicalTranscriberUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalTranscriberUserModel> = {
		...
		this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
			modelAction.MedicalTranscriberUserModelActionTypes.FETCH_MEDICAL_TRANSCRIBER_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalTranscriberUserModelWithId, this.targetModelId)
			...
			model = new MedicalTranscriberUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
			modelAction.MedicalTranscriberUserModelActionTypes.FETCH_MEDICAL_TRANSCRIBER_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalTranscriberUserModel> = {
				...
				queryParams: {
					persistRelations: this.staffMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
				modelAction.MedicalTranscriberUserModelActionTypes.CREATE_MEDICAL_TRANSCRIBER_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
				modelAction.MedicalTranscriberUserModelActionTypes.UPDATE_MEDICAL_TRANSCRIBER_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalTranscriberUserModel = new MedicalTranscriberUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalTranscriberUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalTranscriberUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalTranscriberUserModel.getProps();
		this.modelRelations = MedicalTranscriberUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalTranscriberUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalTranscriberUserModel {
		let updatedModel: MedicalTranscriberUserModel = new MedicalTranscriberUserModel(model);
