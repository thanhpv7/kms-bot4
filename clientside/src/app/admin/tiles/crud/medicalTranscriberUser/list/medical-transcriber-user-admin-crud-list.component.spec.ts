...
import {AdminMedicalTranscriberUserCrudListComponent} from './medical-transcriber-user-admin-crud-list.component';
...
import {AdminMedicalTranscriberUserCrudModule} from '../medical-transcriber-user-admin-crud.module';
import {
	MedicalTranscriberUserModelState,
	initialState as initialMedicalTranscriberUserModelState
} from 'src/app/models/medicalTranscriberUser/medical_transcriber_user.model.state';
 ...
 */
describe('Medical Transcriber User Crud List Tile Component', () => {

	let fixture;
	let medicalTranscriberUserCrudListComponent: AdminMedicalTranscriberUserCrudListComponent;

	let store: MockStore<{ model: MedicalTranscriberUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalTranscriberUserModelState: MedicalTranscriberUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalTranscriberUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5b88a524-86ac-41b7-a0e6-d35679a32989';
	const searchContent = 'text';

	const searchableFieldCount = MedicalTranscriberUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-transcriber-user-admin-crud',
		urls: ['medical-transcriber-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalTranscriberUserDataFactory = new MedicalTranscriberUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalTranscriberUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalTranscriberUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalTranscriberUserModelState = initialMedicalTranscriberUserModelState;
		store.setState({model: medicalTranscriberUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalTranscriberUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalTranscriberUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalTranscriberUserCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalTranscriberUserCrudListComponent);
			medicalTranscriberUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalTranscriberUserCrudListComponent.medicalTranscriberUsersCount).toBe(defaultData.length);
			...
			expect(medicalTranscriberUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalTranscriberUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalTranscriberUserDataFactory.createAll(filteredDataSize);
			...
			expect(medicalTranscriberUserCrudListComponent.medicalTranscriberUsersCount).toBe(filteredDataSize);
			expect(medicalTranscriberUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalTranscriberUserCrudListComponent.pageIndex).toBe(0);
			expect(medicalTranscriberUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalTranscriberUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalTranscriberUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalTranscriberUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
