...
import * as modelAction from 'src/app/models/medicalTranscriberUser/medical_transcriber_user.model.action';
import {MedicalTranscriberUserModelState} from 'src/app/models/medicalTranscriberUser/medical_transcriber_user.model.state';
...
import {
	getMedicalTranscriberUserCollectionModels,
	getMedicalTranscriberUserCollectionCount
...
import {CrudTileMode} from '../medical-transcriber-user-admin-crud.component';
...
@Component({
	selector: 'cb-medical-transcriber-user-admin-crud-list',
	templateUrl: './medical-transcriber-user-admin-crud-list.component.html',
	styleUrls: [
		'./medical-transcriber-user-admin-crud-list.component.scss',
...
export class AdminMedicalTranscriberUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalTranscriberUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalTranscriberUserModel.searchFields,
	...
	/**
	 * List of Medical Transcriber Users that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalTranscriberUsers: Observable<MedicalTranscriberUserModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalTranscriberUsersCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = MedicalTranscriberUserModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalTranscriberUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '5b88a524-86ac-41b7-a0e6-d35679a32989';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalTranscriberUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalTranscriberUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: MedicalTranscriberUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
				modelAction.MedicalTranscriberUserModelActionTypes.INITIALISE_MEDICAL_TRANSCRIBER_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
				modelAction.MedicalTranscriberUserModelActionTypes.FETCH_MEDICAL_TRANSCRIBER_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.MedicalTranscriberUserAction(
						modelAction.MedicalTranscriberUserModelActionTypes.COUNT_MEDICAL_TRANSCRIBER_USERS,
						null
					)
				]
			))
		...
		this.medicalTranscriberUsers = this.store.select(getMedicalTranscriberUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalTranscriberUserCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalTranscriberUsersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('admin');
			commands.push('users');
			commands.push('medical-transcriber-user');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalTranscriberUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalTranscriberUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalTranscriberUserModelActionTypes.EXPORT_ALL_MEDICAL_TRANSCRIBER_USER;
		} else {
			actionType = modelAction.MedicalTranscriberUserModelActionTypes.EXPORT_MEDICAL_TRANSCRIBER_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
				modelAction.MedicalTranscriberUserModelActionTypes.IMPORT_MEDICAL_TRANSCRIBER_USERS,
				{
					file: file,
				},
				[new modelAction.MedicalTranscriberUserAction(
			...
			window.open(environment.API_URL + '/api/medicaltranscriberuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalTranscriberUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Transcriber User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Transcriber User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalTranscriberUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalTranscriberUserModelActionTypes.DELETE_MEDICAL_TRANSCRIBER_USER;
						} else if (usingSelected) {
							actionType = modelAction.MedicalTranscriberUserModelActionTypes.DELETE_ALL_MEDICAL_TRANSCRIBER_USER;
						} else {
							actionType = modelAction.MedicalTranscriberUserModelActionTypes.DELETE_MEDICAL_TRANSCRIBER_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalTranscriberUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_TRANSCRIBER_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_TRANSCRIBER_USER and DELETE_MEDICAL_TRANSCRIBER_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<MedicalTranscriberUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalTranscriberUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalTranscriberUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
