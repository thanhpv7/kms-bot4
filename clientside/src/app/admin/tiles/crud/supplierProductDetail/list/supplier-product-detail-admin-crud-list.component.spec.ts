...
import {AdminSupplierProductDetailCrudListComponent} from './supplier-product-detail-admin-crud-list.component';
...
import {AdminSupplierProductDetailCrudModule} from '../supplier-product-detail-admin-crud.module';
import {
 ...
 */
describe('Supplier Product Detail Crud List Tile Component', () => {

	let fixture;
	let supplierProductDetailCrudListComponent: AdminSupplierProductDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<SupplierProductDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '38c57ad1-3dea-449b-b1c6-145950b552de';
	const searchContent = 'text';

	const searchableFieldCount = SupplierProductDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'supplier-product-detail-admin-crud',
		urls: ['supplier-product-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const supplierProductDetailDataFactory = new SupplierProductDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = supplierProductDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getSupplierProductDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getSupplierProductDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminSupplierProductDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminSupplierProductDetailCrudListComponent);
			supplierProductDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(supplierProductDetailCrudListComponent.supplierProductDetailsCount).toBe(defaultData.length);
			...
			expect(supplierProductDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			supplierProductDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = supplierProductDetailDataFactory.createAll(filteredDataSize);
			...
			expect(supplierProductDetailCrudListComponent.supplierProductDetailsCount).toBe(filteredDataSize);
			expect(supplierProductDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(supplierProductDetailCrudListComponent.pageIndex).toBe(0);
			expect(supplierProductDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			supplierProductDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		supplierProductDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		supplierProductDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
