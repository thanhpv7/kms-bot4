...
import * as modelAction from 'src/app/models/supplierProductDetail/supplier_product_detail.model.action';
import {SupplierProductDetailModelState} from 'src/app/models/supplierProductDetail/supplier_product_detail.model.state';
...
import {
	...
	getSupplierProductDetailCollectionCount
...
import {CrudTileMode} from '../supplier-product-detail-admin-crud.component';
...
@Component({
	selector: 'cb-supplier-product-detail-admin-crud-list',
	templateUrl: './supplier-product-detail-admin-crud-list.component.html',
	styleUrls: [
		'./supplier-product-detail-admin-crud-list.component.scss',
...
export class AdminSupplierProductDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SupplierProductDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SupplierProductDetailModel.searchFields,
	...
	/**
	 * List of Supplier Product Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	supplierProductDetails: Observable<SupplierProductDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	supplierProductDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = SupplierProductDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = SupplierProductDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '38c57ad1-3dea-449b-b1c6-145950b552de';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.supplierProductDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.supplierProductDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: SupplierProductDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierProductDetailAction(
				modelAction.SupplierProductDetailModelActionTypes.INITIALISE_SUPPLIER_PRODUCT_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierProductDetailAction(
				modelAction.SupplierProductDetailModelActionTypes.FETCH_SUPPLIER_PRODUCT_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.SupplierProductDetailAction(
						modelAction.SupplierProductDetailModelActionTypes.COUNT_SUPPLIER_PRODUCT_DETAILS,
						null
					)
				]
			))
		...
		this.supplierProductDetails = this.store.select(getSupplierProductDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSupplierProductDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.supplierProductDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('supplier-product-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SupplierProductDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SupplierProductDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SupplierProductDetailModelActionTypes.EXPORT_ALL_SUPPLIER_PRODUCT_DETAIL;
		} else {
			actionType = modelAction.SupplierProductDetailModelActionTypes.EXPORT_SUPPLIER_PRODUCT_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SupplierProductDetailAction(
				modelAction.SupplierProductDetailModelActionTypes.IMPORT_SUPPLIER_PRODUCT_DETAILS,
				{
					file: file,
				},
				[new modelAction.SupplierProductDetailAction(
			...
			window.open(environment.API_URL + '/api/supplierproductdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SupplierProductDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Supplier Product Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Supplier Product Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SupplierProductDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SupplierProductDetailModelActionTypes.DELETE_SUPPLIER_PRODUCT_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.SupplierProductDetailModelActionTypes.DELETE_ALL_SUPPLIER_PRODUCT_DETAIL;
						} else {
							actionType = modelAction.SupplierProductDetailModelActionTypes.DELETE_SUPPLIER_PRODUCT_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SupplierProductDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SUPPLIER_PRODUCT_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SUPPLIER_PRODUCT_DETAIL and DELETE_SUPPLIER_PRODUCT_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<SupplierProductDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SupplierProductDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.supplierProductDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
