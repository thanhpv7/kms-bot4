...
import {SupplierProductDetailModelState, SupplierProductDetailModelAudit} from 'src/app/models/supplierProductDetail/supplier_product_detail.model.state';
import {SupplierProductDetailModel} from 'src/app/models/supplierProductDetail/supplier_product_detail.model';
import {
	getSupplierProductDetailModelAuditsByEntityId,
...
@Component({
	selector: 'cb-supplier-product-detail-admin-crud-edit',
	templateUrl: './supplier-product-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./supplier-product-detail-admin-crud-edit.component.scss',
...
export class AdminSupplierProductDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	supplierProductDetailAudits$: Observable<SupplierProductDetailModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'supplierProductDetails';
	...
	supplierOppositeRelation = 'supplierProductDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SupplierProductDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: SupplierProductDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSupplierProductDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new SupplierProductDetailModel();
		...
		if (this.targetModelId) {
			this.supplierProductDetailAudits$ = this.store.select(getSupplierProductDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SupplierProductDetailModel> = {
		...
		this.store.dispatch(new modelAction.SupplierProductDetailAction(
			modelAction.SupplierProductDetailModelActionTypes.FETCH_SUPPLIER_PRODUCT_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSupplierProductDetailModelWithId, this.targetModelId)
			...
			model = new SupplierProductDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SupplierProductDetailAction(
			modelAction.SupplierProductDetailModelActionTypes.FETCH_SUPPLIER_PRODUCT_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<SupplierProductDetailModel> = {
				...
				queryParams: {
					persistRelations: this.stockCatalogueMultiCrudActive || this.supplierMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.SupplierProductDetailAction(
				modelAction.SupplierProductDetailModelActionTypes.CREATE_SUPPLIER_PRODUCT_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SupplierProductDetailAction(
				modelAction.SupplierProductDetailModelActionTypes.UPDATE_SUPPLIER_PRODUCT_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SupplierProductDetailModel = new SupplierProductDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSupplierProductDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SupplierProductDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SupplierProductDetailModel.getProps();
		this.modelRelations = SupplierProductDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: SupplierProductDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SupplierProductDetailModel {
		let updatedModel: SupplierProductDetailModel = new SupplierProductDetailModel(model);
