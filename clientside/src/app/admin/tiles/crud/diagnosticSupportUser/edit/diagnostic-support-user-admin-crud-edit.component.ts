...
import {DiagnosticSupportUserModelState, DiagnosticSupportUserModelAudit} from 'src/app/models/diagnosticSupportUser/diagnostic_support_user.model.state';
import {DiagnosticSupportUserModel} from 'src/app/models/diagnosticSupportUser/diagnostic_support_user.model';
import {
	getDiagnosticSupportUserModelAuditsByEntityId,
	getDiagnosticSupportUserModelWithId,
	getCountDiagnosticSupportUserModels,
...
@Component({
	selector: 'cb-diagnostic-support-user-admin-crud-edit',
	templateUrl: './diagnostic-support-user-admin-crud-edit.component.html',
	styleUrls: [
		'./diagnostic-support-user-admin-crud-edit.component.scss',
...
export class AdminDiagnosticSupportUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	diagnosticSupportUserAudits$: Observable<DiagnosticSupportUserModelAudit[]>;
	...
	staffOppositeRelation = 'diagnosticSupportUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DiagnosticSupportUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: DiagnosticSupportUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDiagnosticSupportUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DiagnosticSupportUserModel();
		...
		if (this.targetModelId) {
			this.diagnosticSupportUserAudits$ = this.store.select(getDiagnosticSupportUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DiagnosticSupportUserModel> = {
		...
		this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
			modelAction.DiagnosticSupportUserModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDiagnosticSupportUserModelWithId, this.targetModelId)
			...
			model = new DiagnosticSupportUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
			modelAction.DiagnosticSupportUserModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
				modelAction.DiagnosticSupportUserModelActionTypes.CREATE_DIAGNOSTIC_SUPPORT_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
				modelAction.DiagnosticSupportUserModelActionTypes.UPDATE_DIAGNOSTIC_SUPPORT_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DiagnosticSupportUserModel = new DiagnosticSupportUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDiagnosticSupportUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DiagnosticSupportUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DiagnosticSupportUserModel.getProps();
		this.modelRelations = DiagnosticSupportUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: DiagnosticSupportUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DiagnosticSupportUserModel {
		let updatedModel: DiagnosticSupportUserModel = new DiagnosticSupportUserModel(model);
