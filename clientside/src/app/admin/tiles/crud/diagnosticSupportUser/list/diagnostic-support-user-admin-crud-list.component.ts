...
import * as modelAction from 'src/app/models/diagnosticSupportUser/diagnostic_support_user.model.action';
import {DiagnosticSupportUserModelState} from 'src/app/models/diagnosticSupportUser/diagnostic_support_user.model.state';
...
import {CrudTileMode} from '../diagnostic-support-user-admin-crud.component';
...
@Component({
	selector: 'cb-diagnostic-support-user-admin-crud-list',
	templateUrl: './diagnostic-support-user-admin-crud-list.component.html',
	styleUrls: [
		'./diagnostic-support-user-admin-crud-list.component.scss',
...
export class AdminDiagnosticSupportUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DiagnosticSupportUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DiagnosticSupportUserModel.searchFields,
	...
	/**
	 * List of Diagnostic Support Users that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	diagnosticSupportUsers: Observable<DiagnosticSupportUserModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	diagnosticSupportUsersCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = DiagnosticSupportUserModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = DiagnosticSupportUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '9e2eafec-893e-4cbe-b695-544830629da2';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.diagnosticSupportUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.diagnosticSupportUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: DiagnosticSupportUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
				modelAction.DiagnosticSupportUserModelActionTypes.INITIALISE_DIAGNOSTIC_SUPPORT_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
				modelAction.DiagnosticSupportUserModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.DiagnosticSupportUserAction(
						modelAction.DiagnosticSupportUserModelActionTypes.COUNT_DIAGNOSTIC_SUPPORT_USERS,
						null
					)
				]
			))
		...
		this.diagnosticSupportUsers = this.store.select(getDiagnosticSupportUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDiagnosticSupportUserCollectionCount, this.collectionId).subscribe(
				(count) => this.diagnosticSupportUsersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('admin');
			commands.push('users');
			commands.push('diagnostic-support-user');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DiagnosticSupportUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DiagnosticSupportUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DiagnosticSupportUserModelActionTypes.EXPORT_ALL_DIAGNOSTIC_SUPPORT_USER;
		} else {
			actionType = modelAction.DiagnosticSupportUserModelActionTypes.EXPORT_DIAGNOSTIC_SUPPORT_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
				modelAction.DiagnosticSupportUserModelActionTypes.IMPORT_DIAGNOSTIC_SUPPORT_USERS,
				{
					file: file,
				},
				[new modelAction.DiagnosticSupportUserAction(
			...
			window.open(environment.API_URL + '/api/diagnosticsupportuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DiagnosticSupportUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Support User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Support User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DiagnosticSupportUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DiagnosticSupportUserModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_USER;
						} else if (usingSelected) {
							actionType = modelAction.DiagnosticSupportUserModelActionTypes.DELETE_ALL_DIAGNOSTIC_SUPPORT_USER;
						} else {
							actionType = modelAction.DiagnosticSupportUserModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DiagnosticSupportUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DIAGNOSTIC_SUPPORT_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DIAGNOSTIC_SUPPORT_USER and DELETE_DIAGNOSTIC_SUPPORT_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<DiagnosticSupportUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DiagnosticSupportUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.diagnosticSupportUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
