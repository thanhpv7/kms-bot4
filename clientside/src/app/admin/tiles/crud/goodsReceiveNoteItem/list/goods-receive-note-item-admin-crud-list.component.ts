...
import {GoodsReceiveNoteItemModelState} from 'src/app/models/goodsReceiveNoteItem/goods_receive_note_item.model.state';
...
import {
	...
	getGoodsReceiveNoteItemCollectionCount
...
import {CrudTileMode} from '../goods-receive-note-item-admin-crud.component';
...
@Component({
	selector: 'cb-goods-receive-note-item-admin-crud-list',
	templateUrl: './goods-receive-note-item-admin-crud-list.component.html',
	styleUrls: [
		'./goods-receive-note-item-admin-crud-list.component.scss',
...
export class AdminGoodsReceiveNoteItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<GoodsReceiveNoteItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...GoodsReceiveNoteItemModel.searchFields,
	...
	/**
	 * List of Goods Receive Note Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	goodsReceiveNoteItems: Observable<GoodsReceiveNoteItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	goodsReceiveNoteItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.goodsReceiveNoteItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.goodsReceiveNoteItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.GoodsReceiveNoteItemAction(
				modelAction.GoodsReceiveNoteItemModelActionTypes.INITIALISE_GOODS_RECEIVE_NOTE_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.GoodsReceiveNoteItemAction(
				modelAction.GoodsReceiveNoteItemModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.goodsReceiveNoteItems = this.store.select(getGoodsReceiveNoteItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getGoodsReceiveNoteItemCollectionCount, this.collectionId).subscribe(
				(count) => this.goodsReceiveNoteItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: GoodsReceiveNoteItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.GoodsReceiveNoteItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.GoodsReceiveNoteItemModelActionTypes.EXPORT_ALL_GOODS_RECEIVE_NOTE_ITEM;
		} else {
			actionType = modelAction.GoodsReceiveNoteItemModelActionTypes.EXPORT_GOODS_RECEIVE_NOTE_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.GoodsReceiveNoteItemAction(
				modelAction.GoodsReceiveNoteItemModelActionTypes.IMPORT_GOODS_RECEIVE_NOTE_ITEMS,
				{
					file: file,
				},
				[new modelAction.GoodsReceiveNoteItemAction(
			...
			window.open(environment.API_URL + '/api/goodsreceivenoteitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: GoodsReceiveNoteItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Goods Receive Note Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Goods Receive Note Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.GoodsReceiveNoteItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.GoodsReceiveNoteItemModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.GoodsReceiveNoteItemModelActionTypes.DELETE_ALL_GOODS_RECEIVE_NOTE_ITEM;
						} else {
							actionType = modelAction.GoodsReceiveNoteItemModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.GoodsReceiveNoteItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_GOODS_RECEIVE_NOTE_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_GOODS_RECEIVE_NOTE_ITEM and DELETE_GOODS_RECEIVE_NOTE_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = GoodsReceiveNoteItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.goodsReceiveNoteItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
