...
import {AdminGoodsReceiveNoteItemCrudListComponent} from './goods-receive-note-item-admin-crud-list.component';
...
import {AdminGoodsReceiveNoteItemCrudModule} from '../goods-receive-note-item-admin-crud.module';
import {
 ...
 */
describe('Goods Receive Note Item Crud List Tile Component', () => {

	let fixture;
	let goodsReceiveNoteItemCrudListComponent: AdminGoodsReceiveNoteItemCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<GoodsReceiveNoteItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f310695b-7a1b-4751-9cc1-057555780cfd';
	const searchContent = 'text';

	const searchableFieldCount = GoodsReceiveNoteItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'goods-receive-note-item-admin-crud',
		urls: ['goods-receive-note-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const goodsReceiveNoteItemDataFactory = new GoodsReceiveNoteItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = goodsReceiveNoteItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getGoodsReceiveNoteItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getGoodsReceiveNoteItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminGoodsReceiveNoteItemCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminGoodsReceiveNoteItemCrudListComponent);
			goodsReceiveNoteItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(goodsReceiveNoteItemCrudListComponent.goodsReceiveNoteItemsCount).toBe(defaultData.length);
			...
			expect(goodsReceiveNoteItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			goodsReceiveNoteItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = goodsReceiveNoteItemDataFactory.createAll(filteredDataSize);
			...
			expect(goodsReceiveNoteItemCrudListComponent.goodsReceiveNoteItemsCount).toBe(filteredDataSize);
			expect(goodsReceiveNoteItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(goodsReceiveNoteItemCrudListComponent.pageIndex).toBe(0);
			expect(goodsReceiveNoteItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			goodsReceiveNoteItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		goodsReceiveNoteItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		goodsReceiveNoteItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
