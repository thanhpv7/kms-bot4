...
import {PcareMCUModelState, PcareMCUModelAudit} from 'src/app/models/pcareMCU/pcare_mcu.model.state';
import {PcareMCUModel} from 'src/app/models/pcareMCU/pcare_mcu.model';
import {
	getPcareMCUModelAuditsByEntityId,
...
@Component({
	selector: 'cb-pcare-mcu-admin-crud-edit',
	templateUrl: './pcare-mcu-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-mcu-admin-crud-edit.component.scss',
...
export class AdminPcareMCUCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareMCUAudits$: Observable<PcareMCUModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareMCUModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareMCUModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareMCUModel();
		...
		if (this.targetModelId) {
			this.pcareMCUAudits$ = this.store.select(getPcareMCUModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareMCUModel> = {
		...
		this.store.dispatch(new modelAction.PcareMCUAction(
			modelAction.PcareMCUModelActionTypes.FETCH_PCARE_MCU,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareMCUModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareMCUAction(
			modelAction.PcareMCUModelActionTypes.FETCH_PCARE_MCU_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareMCUAction(
				modelAction.PcareMCUModelActionTypes.CREATE_PCARE_MCU,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareMCUAction(
				modelAction.PcareMCUModelActionTypes.UPDATE_PCARE_MCU,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareMCUModel = new PcareMCUModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareMCUModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareMCUModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareMCUModel.getProps();
		this.modelRelations = PcareMCUModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareMCUModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareMCUModel {
		let updatedModel: PcareMCUModel = new PcareMCUModel(model);
