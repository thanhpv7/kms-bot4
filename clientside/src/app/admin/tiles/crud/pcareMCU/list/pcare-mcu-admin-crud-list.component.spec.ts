...
import {AdminPcareMCUCrudListComponent} from './pcare-mcu-admin-crud-list.component';
...
import {AdminPcareMCUCrudModule} from '../pcare-mcu-admin-crud.module';
import {
	PcareMCUModelState,
 ...
 */
describe('PCare MCU Crud List Tile Component', () => {

	let fixture;
	let pcareMCUCrudListComponent: AdminPcareMCUCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareMCUModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cd96a598-c9da-4a4b-906f-8684cf78986e';
	const searchContent = 'text';

	const searchableFieldCount = PcareMCUModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-mcu-admin-crud',
		urls: ['pcare-mcu-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareMCUDataFactory = new PcareMCUDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareMCUDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPcareMCUCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareMCUCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPcareMCUCrudListComponent);
			pcareMCUCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareMCUCrudListComponent.pcareMCUsCount).toBe(defaultData.length);
			...
			expect(pcareMCUCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareMCUCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareMCUDataFactory.createAll(filteredDataSize);
			...
			expect(pcareMCUCrudListComponent.pcareMCUsCount).toBe(filteredDataSize);
			expect(pcareMCUCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareMCUCrudListComponent.pageIndex).toBe(0);
			expect(pcareMCUCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareMCUCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareMCUCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareMCUCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
