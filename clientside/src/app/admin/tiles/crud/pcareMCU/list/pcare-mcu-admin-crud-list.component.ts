...
import * as modelAction from 'src/app/models/pcareMCU/pcare_mcu.model.action';
import {PcareMCUModelState} from 'src/app/models/pcareMCU/pcare_mcu.model.state';
...
import {CrudTileMode} from '../pcare-mcu-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-mcu-admin-crud-list',
	templateUrl: './pcare-mcu-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-mcu-admin-crud-list.component.scss',
...
export class AdminPcareMCUCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareMCUModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareMCUModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PcareMCUModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'cd96a598-c9da-4a4b-906f-8684cf78986e';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareMCUsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareMCUsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareMCUModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareMCUAction(
				modelAction.PcareMCUModelActionTypes.INITIALISE_PCARE_MCU_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareMCUAction(
				modelAction.PcareMCUModelActionTypes.FETCH_PCARE_MCU_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareMCUAction(
						modelAction.PcareMCUModelActionTypes.COUNT_PCARE_MCUS,
						null
					)
				]
			))
		...
		this.pcareMCUs = this.store.select(getPcareMCUCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareMCUCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareMCUsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareMCUModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareMCUModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareMCUModelActionTypes.EXPORT_ALL_PCARE_MCU;
		} else {
			actionType = modelAction.PcareMCUModelActionTypes.EXPORT_PCARE_MCU_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareMCUAction(
				modelAction.PcareMCUModelActionTypes.IMPORT_PCARE_MCUS,
				{
					file: file,
				},
				[new modelAction.PcareMCUAction(
			...
			window.open(environment.API_URL + '/api/pcaremcu/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareMCUModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare MCU entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare MCU entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareMCUModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareMCUModelActionTypes.DELETE_PCARE_MCU;
						} else if (usingSelected) {
							actionType = modelAction.PcareMCUModelActionTypes.DELETE_ALL_PCARE_MCU;
						} else {
							actionType = modelAction.PcareMCUModelActionTypes.DELETE_PCARE_MCU_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareMCUAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_MCU when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_MCU and DELETE_PCARE_MCU_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareMCUModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareMCUModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareMCUsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
