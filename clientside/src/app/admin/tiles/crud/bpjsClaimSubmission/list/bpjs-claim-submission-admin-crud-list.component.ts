...
import {BpjsClaimSubmissionModelState} from 'src/app/models/bpjsClaimSubmission/bpjs_claim_submission.model.state';
...
import {
	...
	getBpjsClaimSubmissionCollectionCount
...
@Component({
	selector: 'cb-bpjs-claim-submission-admin-crud-list',
	templateUrl: './bpjs-claim-submission-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-claim-submission-admin-crud-list.component.scss',
...
export class AdminBpjsClaimSubmissionCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsClaimSubmissionModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsClaimSubmissionModel.searchFields,
	...
	/**
	 * List of BPJS Claim Submissions that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsClaimSubmissions: Observable<BpjsClaimSubmissionModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsClaimSubmissionsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsClaimSubmissionsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsClaimSubmissionsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsClaimSubmissionAction(
				modelAction.BpjsClaimSubmissionModelActionTypes.INITIALISE_BPJS_CLAIM_SUBMISSION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsClaimSubmissionAction(
				modelAction.BpjsClaimSubmissionModelActionTypes.FETCH_BPJS_CLAIM_SUBMISSION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsClaimSubmissions = this.store.select(getBpjsClaimSubmissionCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsClaimSubmissionCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsClaimSubmissionsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsClaimSubmissionModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsClaimSubmissionModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsClaimSubmissionModelActionTypes.EXPORT_ALL_BPJS_CLAIM_SUBMISSION;
		} else {
			actionType = modelAction.BpjsClaimSubmissionModelActionTypes.EXPORT_BPJS_CLAIM_SUBMISSION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsClaimSubmissionAction(
				modelAction.BpjsClaimSubmissionModelActionTypes.IMPORT_BPJS_CLAIM_SUBMISSIONS,
				{
					file: file,
				},
				[new modelAction.BpjsClaimSubmissionAction(
			...
			window.open(environment.API_URL + '/api/bpjsclaimsubmission/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsClaimSubmissionModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Claim Submission entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Claim Submission entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsClaimSubmissionModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsClaimSubmissionModelActionTypes.DELETE_BPJS_CLAIM_SUBMISSION;
						} else if (usingSelected) {
							actionType = modelAction.BpjsClaimSubmissionModelActionTypes.DELETE_ALL_BPJS_CLAIM_SUBMISSION;
						} else {
							actionType = modelAction.BpjsClaimSubmissionModelActionTypes.DELETE_BPJS_CLAIM_SUBMISSION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsClaimSubmissionAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_CLAIM_SUBMISSION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_CLAIM_SUBMISSION and DELETE_BPJS_CLAIM_SUBMISSION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsClaimSubmissionModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsClaimSubmissionsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
