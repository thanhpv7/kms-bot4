...
import {AdminBpjsClaimSubmissionCrudListComponent} from './bpjs-claim-submission-admin-crud-list.component';
...
import {AdminBpjsClaimSubmissionCrudModule} from '../bpjs-claim-submission-admin-crud.module';
import {
 ...
 */
describe('BPJS Claim Submission Crud List Tile Component', () => {

	let fixture;
	let bpjsClaimSubmissionCrudListComponent: AdminBpjsClaimSubmissionCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsClaimSubmissionModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '1ba77a86-2440-464c-9dfc-e734edb11968';
	const searchContent = 'text';

	const searchableFieldCount = BpjsClaimSubmissionModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-claim-submission-admin-crud',
		urls: ['bpjs-claim-submission-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsClaimSubmissionDataFactory = new BpjsClaimSubmissionDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsClaimSubmissionDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsClaimSubmissionCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsClaimSubmissionCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsClaimSubmissionCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsClaimSubmissionCrudListComponent);
			bpjsClaimSubmissionCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsClaimSubmissionCrudListComponent.bpjsClaimSubmissionsCount).toBe(defaultData.length);
			...
			expect(bpjsClaimSubmissionCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsClaimSubmissionCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsClaimSubmissionDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsClaimSubmissionCrudListComponent.bpjsClaimSubmissionsCount).toBe(filteredDataSize);
			expect(bpjsClaimSubmissionCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsClaimSubmissionCrudListComponent.pageIndex).toBe(0);
			expect(bpjsClaimSubmissionCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsClaimSubmissionCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsClaimSubmissionCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsClaimSubmissionCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
