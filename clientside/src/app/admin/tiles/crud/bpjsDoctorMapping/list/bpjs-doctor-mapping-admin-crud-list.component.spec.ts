...
import {AdminBpjsDoctorMappingCrudListComponent} from './bpjs-doctor-mapping-admin-crud-list.component';
...
import {AdminBpjsDoctorMappingCrudModule} from '../bpjs-doctor-mapping-admin-crud.module';
import {
 ...
 */
describe('BPJS Doctor Mapping Crud List Tile Component', () => {

	let fixture;
	let bpjsDoctorMappingCrudListComponent: AdminBpjsDoctorMappingCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsDoctorMappingModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8e13cf6d-2476-4cc1-b594-7e4142aec8fd';
	const searchContent = 'text';

	const searchableFieldCount = BpjsDoctorMappingModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-doctor-mapping-admin-crud',
		urls: ['bpjs-doctor-mapping-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsDoctorMappingDataFactory = new BpjsDoctorMappingDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsDoctorMappingDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsDoctorMappingCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsDoctorMappingCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsDoctorMappingCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsDoctorMappingCrudListComponent);
			bpjsDoctorMappingCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsDoctorMappingCrudListComponent.bpjsDoctorMappingsCount).toBe(defaultData.length);
			...
			expect(bpjsDoctorMappingCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsDoctorMappingCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsDoctorMappingDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsDoctorMappingCrudListComponent.bpjsDoctorMappingsCount).toBe(filteredDataSize);
			expect(bpjsDoctorMappingCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsDoctorMappingCrudListComponent.pageIndex).toBe(0);
			expect(bpjsDoctorMappingCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsDoctorMappingCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsDoctorMappingCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsDoctorMappingCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
