...
import {BpjsDoctorMappingModelState} from 'src/app/models/bpjsDoctorMapping/bpjs_doctor_mapping.model.state';
...
import {
	...
	getBpjsDoctorMappingCollectionCount
...
@Component({
	selector: 'cb-bpjs-doctor-mapping-admin-crud-list',
	templateUrl: './bpjs-doctor-mapping-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-doctor-mapping-admin-crud-list.component.scss',
...
export class AdminBpjsDoctorMappingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsDoctorMappingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsDoctorMappingModel.searchFields,
	...
	/**
	 * List of BPJS Doctor Mappings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsDoctorMappings: Observable<BpjsDoctorMappingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsDoctorMappingsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsDoctorMappingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsDoctorMappingsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
				modelAction.BpjsDoctorMappingModelActionTypes.INITIALISE_BPJS_DOCTOR_MAPPING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
				modelAction.BpjsDoctorMappingModelActionTypes.FETCH_BPJS_DOCTOR_MAPPING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsDoctorMappings = this.store.select(getBpjsDoctorMappingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsDoctorMappingCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsDoctorMappingsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsDoctorMappingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsDoctorMappingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsDoctorMappingModelActionTypes.EXPORT_ALL_BPJS_DOCTOR_MAPPING;
		} else {
			actionType = modelAction.BpjsDoctorMappingModelActionTypes.EXPORT_BPJS_DOCTOR_MAPPING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
				modelAction.BpjsDoctorMappingModelActionTypes.IMPORT_BPJS_DOCTOR_MAPPINGS,
				{
					file: file,
				},
				[new modelAction.BpjsDoctorMappingAction(
			...
			window.open(environment.API_URL + '/api/bpjsdoctormapping/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsDoctorMappingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Doctor Mapping entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Doctor Mapping entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsDoctorMappingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsDoctorMappingModelActionTypes.DELETE_BPJS_DOCTOR_MAPPING;
						} else if (usingSelected) {
							actionType = modelAction.BpjsDoctorMappingModelActionTypes.DELETE_ALL_BPJS_DOCTOR_MAPPING;
						} else {
							actionType = modelAction.BpjsDoctorMappingModelActionTypes.DELETE_BPJS_DOCTOR_MAPPING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_DOCTOR_MAPPING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_DOCTOR_MAPPING and DELETE_BPJS_DOCTOR_MAPPING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsDoctorMappingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsDoctorMappingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
