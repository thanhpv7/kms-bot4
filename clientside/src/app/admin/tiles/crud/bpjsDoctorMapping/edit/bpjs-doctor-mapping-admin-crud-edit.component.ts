...
import * as modelAction from 'src/app/models/bpjsDoctorMapping/bpjs_doctor_mapping.model.action';
import {BpjsDoctorMappingModelState, BpjsDoctorMappingModelAudit} from 'src/app/models/bpjsDoctorMapping/bpjs_doctor_mapping.model.state';
import {BpjsDoctorMappingModel} from 'src/app/models/bpjsDoctorMapping/bpjs_doctor_mapping.model';
import {
	getBpjsDoctorMappingModelAuditsByEntityId,
...
import * as bpjsPRBModelAction from 'src/app/models/bpjsPRB/bpjs_prb.model.action';
import {
	getBpjsPRBCollectionModels, getBpjsPRBModelWithId,
...
import {CrudTileMode} from '../bpjs-doctor-mapping-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-doctor-mapping-admin-crud-edit',
	templateUrl: './bpjs-doctor-mapping-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-doctor-mapping-admin-crud-edit.component.scss',
...
export class AdminBpjsDoctorMappingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '8e13cf6d-2476-4cc1-b594-7e4142aec8fd';
	 ...
	 */
	bpjsDoctorMappingAudits$: Observable<BpjsDoctorMappingModelAudit[]>;
	...
	staffOppositeRelation = 'bpjsDoctorMapping';
	...
	bpjsClaimSubmissionsOppositeRelation = 'bpjsDoctorMapping';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Doctor Mapping and BPJS Control Plans here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Doctor Mapping and BPJS Control Plans here] end
	bpjsControlPlansOppositeRelation = 'bpjsDoctorMapping';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Doctor Mapping and BPJS PRBs here] off begin
	bpjsPRBsMultiCrudActive: boolean = false;
	bpjsPRBsDisplayName = 'BPJS PRBs';
	bpjsPRBsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Doctor Mapping and BPJS PRBs here] end
	bpjsPRBsOppositeRelation = 'bpjsDoctorMapping';
	bpjsPRBsRelations: ModelRelation[] = this.bpjsPRBsMultiCrudActive ? BpjsPRBModel.getRelations()
		.map(relation => {
			if (relation.id === this.bpjsPRBsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Doctor Mapping and kodeDokter here] off begin
	kodedokterMultiCrudActive: boolean = false;
	kodedokterDisplayName = 'kodeDokter';
	kodedokterCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Doctor Mapping and kodeDokter here] end
	kodedokterOppositeRelation = 'bpjsDoctorMapping';
	kodedokterRelations: ModelRelation[] = this.kodedokterMultiCrudActive ? BpjsHospitalizationPlanModel.getRelations()
		.map(relation => {
			if (relation.id === this.kodedokterOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsDoctorMappingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsDoctorMappingModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsDoctorMappingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'bpjsPRBs' && this.bpjsPRBsMultiCrudActive) {
					exp.fields = BpjsPRBModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'kodedokter' && this.kodedokterMultiCrudActive) {
	 ...
	 */
	targetModel: BpjsDoctorMappingModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsDoctorMappingModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsDoctorMappingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsDoctorMappingModel();
		...
		if (this.targetModelId) {
			this.bpjsDoctorMappingAudits$ = this.store.select(getBpjsDoctorMappingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsDoctorMappingModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
			modelAction.BpjsDoctorMappingModelActionTypes.FETCH_BPJS_DOCTOR_MAPPING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsDoctorMappingModelWithId, this.targetModelId)
			...
			model = new BpjsDoctorMappingModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-doctor-mapping');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
			modelAction.BpjsDoctorMappingModelActionTypes.FETCH_BPJS_DOCTOR_MAPPING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsDoctorMappingModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.staffMultiCrudActive || this.bpjsClaimSubmissionsMultiCrudActive || this.bpjsControlPlansMultiCrudActive || this.bpjsPRBsMultiCrudActive || this.kodedokterMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsDoctorMappingAction(
						modelAction.BpjsDoctorMappingModelActionTypes.COUNT_BPJS_DOCTOR_MAPPINGS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
				modelAction.BpjsDoctorMappingModelActionTypes.CREATE_BPJS_DOCTOR_MAPPING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsDoctorMappingAction(
				modelAction.BpjsDoctorMappingModelActionTypes.UPDATE_BPJS_DOCTOR_MAPPING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsDoctorMappingModel = new BpjsDoctorMappingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsDoctorMappingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsDoctorMappingModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.bpjsPRBsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsPRBModel.getProps(), this.bpjsPRBsRelations, this.isViewOnly),
					new BpjsPRBModel(),
					BpjsPRBModel,
					this.bpjsPRBsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.bpjsPRBsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.kodedokterDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsHospitalizationPlanModel.getProps(), this.kodedokterRelations, this.isViewOnly),
					new BpjsHospitalizationPlanModel(),
					BpjsHospitalizationPlanModel,
					this.kodedokterDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.kodedokterRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsDoctorMappingModel.getProps();
		this.modelRelations = BpjsDoctorMappingModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'bpjsPRBs';
		}).hideElement = this.bpjsPRBsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'bpjsPRBs').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'kodedokter';
		}).hideElement = this.kodedokterMultiCrudActive || this.modelRelations.find(rel => rel.id === 'kodedokter').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.bpjsPRBsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.bpjsPRBsDisplayName, many: true});
			this.store.dispatch(new bpjsPRBModelAction.BpjsPRBAction(
				bpjsPRBModelAction.BpjsPRBModelActionTypes.INITIALISE_BPJS_PRB_COLLECTION_STATE,
				{
					collectionId: this.bpjsPRBsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsPRBCollectionModels, this.bpjsPRBsCollectionId).subscribe(models => {
				models.forEach(bpjsPRBs => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(BpjsPRBModel.getProps(), this.bpjsPRBsRelations, this.isViewOnly),
						bpjsPRBs,
						BpjsPRBModel,
		...
		if (this.kodedokterMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.kodedokterDisplayName, many: true});
			this.store.dispatch(new bpjsHospitalizationPlanModelAction.BpjsHospitalizationPlanAction(
				...
				{
					collectionId: this.kodedokterCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsHospitalizationPlanCollectionModels, this.kodedokterCollectionId).subscribe(models => {
				models.forEach(kodedokter => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(BpjsHospitalizationPlanModel.getProps(), this.kodedokterRelations, this.isViewOnly),
						kodedokter,
						BpjsHospitalizationPlanModel,
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.bpjsPRBsIds && this.targetModel.bpjsPRBsIds.length > 0 && this.bpjsPRBsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsPRBModel,
				this.bpjsPRBsOppositeRelation,
				this.bpjsPRBsCollectionId,
				this.targetModel.bpjsPRBsIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.kodedokterIds && this.targetModel.kodedokterIds.length > 0 && this.kodedokterMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsHospitalizationPlanModel,
				this.kodedokterOppositeRelation,
				this.kodedokterCollectionId,
				this.targetModel.kodedokterIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsDoctorMappingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsDoctorMappingModel {
		let updatedModel: BpjsDoctorMappingModel = new BpjsDoctorMappingModel(model);
		...
		updatedModel.bpjsPRBs = [];
		if (this.bpjsPRBsMultiCrudActive) {
			if (event.additionalEntities.bpjsPRBs) {
				event.additionalEntities.bpjsPRBs.forEach(entity => {
					updatedModel.bpjsPRBs.push(entity);
				});
			}

			if (!updatedModel.bpjsPRBs.length) {
				updatedModel.bpjsPRBsIds = [];
			}
		}
		updatedModel.kodedokter = [];
		if (this.kodedokterMultiCrudActive) {
			if (event.additionalEntities.kodedokter) {
				event.additionalEntities.kodedokter.forEach(entity => {
					updatedModel.kodedokter.push(entity);
				});
			}

			if (!updatedModel.kodedokter.length) {
				updatedModel.kodedokterIds = [];
			}
		}
		return updatedModel;
	}
