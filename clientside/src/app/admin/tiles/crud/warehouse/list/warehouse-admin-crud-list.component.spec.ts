...
import {AdminWarehouseCrudListComponent} from './warehouse-admin-crud-list.component';
...
import {AdminWarehouseCrudModule} from '../warehouse-admin-crud.module';
import {
	WarehouseModelState,
 ...
 */
describe('Warehouse Crud List Tile Component', () => {

	let fixture;
	let warehouseCrudListComponent: AdminWarehouseCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<WarehouseModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ea2f2887-6641-46f1-a3d6-542d78c3c9c7';
	const searchContent = 'text';

	const searchableFieldCount = WarehouseModel.searchFields.length;

	const routerState: RouterState = {
		url: 'warehouse-admin-crud',
		urls: ['warehouse-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const warehouseDataFactory = new WarehouseDataFactory();

	const defaultDataSize = 30;
	const defaultData = warehouseDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getWarehouseCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getWarehouseCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminWarehouseCrudListComponent);
			warehouseCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(warehouseCrudListComponent.warehousesCount).toBe(defaultData.length);
			...
			expect(warehouseCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			warehouseCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = warehouseDataFactory.createAll(filteredDataSize);
			...
			expect(warehouseCrudListComponent.warehousesCount).toBe(filteredDataSize);
			expect(warehouseCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(warehouseCrudListComponent.pageIndex).toBe(0);
			expect(warehouseCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			warehouseCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		warehouseCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		warehouseCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
