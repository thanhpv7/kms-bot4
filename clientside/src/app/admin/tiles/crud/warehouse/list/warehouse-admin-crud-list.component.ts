...
import {WarehouseModelState} from 'src/app/models/warehouse/warehouse.model.state';
...
@Component({
	selector: 'cb-warehouse-admin-crud-list',
	templateUrl: './warehouse-admin-crud-list.component.html',
	styleUrls: [
		'./warehouse-admin-crud-list.component.scss',
...
export class AdminWarehouseCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<WarehouseModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...WarehouseModel.searchFields,
	...
	/**
	 * List of Warehouses that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.warehousesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.warehousesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseAction(
				modelAction.WarehouseModelActionTypes.INITIALISE_WAREHOUSE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseAction(
				modelAction.WarehouseModelActionTypes.FETCH_WAREHOUSE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.warehouses = this.store.select(getWarehouseCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getWarehouseCollectionCount, this.collectionId).subscribe(
				(count) => this.warehousesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: WarehouseModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.WarehouseModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.WarehouseModelActionTypes.EXPORT_ALL_WAREHOUSE;
		} else {
			actionType = modelAction.WarehouseModelActionTypes.EXPORT_WAREHOUSE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.WarehouseAction(
				modelAction.WarehouseModelActionTypes.IMPORT_WAREHOUSES,
				{
					file: file,
				},
				[new modelAction.WarehouseAction(
			...
			window.open(environment.API_URL + '/api/warehouse/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: WarehouseModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Warehouse entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Warehouse entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.WarehouseModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.WarehouseModelActionTypes.DELETE_WAREHOUSE;
						} else if (usingSelected) {
							actionType = modelAction.WarehouseModelActionTypes.DELETE_ALL_WAREHOUSE;
						} else {
							actionType = modelAction.WarehouseModelActionTypes.DELETE_WAREHOUSE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.WarehouseAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_WAREHOUSE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_WAREHOUSE and DELETE_WAREHOUSE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = WarehouseModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.warehousesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
