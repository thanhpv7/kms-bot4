...
import {AdminMedicalCertificateMentalHealthCrudListComponent} from './medical-certificate-mental-health-admin-crud-list.component';
...
import {AdminMedicalCertificateMentalHealthCrudModule} from '../medical-certificate-mental-health-admin-crud.module';
import {
	MedicalCertificateMentalHealthModelState,
	initialState as initialMedicalCertificateMentalHealthModelState
} from 'src/app/models/medicalCertificateMentalHealth/medical_certificate_mental_health.model.state';
 ...
 */
describe('Medical Certificate Mental Health Crud List Tile Component', () => {

	let fixture;
	let medicalCertificateMentalHealthCrudListComponent: AdminMedicalCertificateMentalHealthCrudListComponent;

	let store: MockStore<{ model: MedicalCertificateMentalHealthModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalCertificateMentalHealthModelState: MedicalCertificateMentalHealthModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalCertificateMentalHealthModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b1ab042c-76ed-4065-ae87-19d7fe2ea65a';
	const searchContent = 'text';

	const searchableFieldCount = MedicalCertificateMentalHealthModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-certificate-mental-health-admin-crud',
		urls: ['medical-certificate-mental-health-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalCertificateMentalHealthDataFactory = new MedicalCertificateMentalHealthDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalCertificateMentalHealthDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalCertificateMentalHealthModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalCertificateMentalHealthModelState = initialMedicalCertificateMentalHealthModelState;
		store.setState({model: medicalCertificateMentalHealthModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalCertificateMentalHealthCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalCertificateMentalHealthCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalCertificateMentalHealthCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalCertificateMentalHealthCrudListComponent);
			medicalCertificateMentalHealthCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalCertificateMentalHealthCrudListComponent.medicalCertificateMentalHealthsCount).toBe(defaultData.length);
			...
			expect(medicalCertificateMentalHealthCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalCertificateMentalHealthCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalCertificateMentalHealthDataFactory.createAll(filteredDataSize);
			...
			expect(medicalCertificateMentalHealthCrudListComponent.medicalCertificateMentalHealthsCount).toBe(filteredDataSize);
			expect(medicalCertificateMentalHealthCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalCertificateMentalHealthCrudListComponent.pageIndex).toBe(0);
			expect(medicalCertificateMentalHealthCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalCertificateMentalHealthCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalCertificateMentalHealthCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalCertificateMentalHealthCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
