...
import {MedicalCertificateMentalHealthModelState} from 'src/app/models/medicalCertificateMentalHealth/medical_certificate_mental_health.model.state';
...
import {
	getMedicalCertificateMentalHealthCollectionModels,
	getMedicalCertificateMentalHealthCollectionCount
...
@Component({
	selector: 'cb-medical-certificate-mental-health-admin-crud-list',
	templateUrl: './medical-certificate-mental-health-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-mental-health-admin-crud-list.component.scss',
...
export class AdminMedicalCertificateMentalHealthCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificateMentalHealthModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificateMentalHealthModel.searchFields,
	...
	/**
	 * List of Medical Certificate Mental Healths that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificateMentalHealths: Observable<MedicalCertificateMentalHealthModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificateMentalHealthsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificateMentalHealthsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificateMentalHealthsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
				modelAction.MedicalCertificateMentalHealthModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_MENTAL_HEALTH_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
				modelAction.MedicalCertificateMentalHealthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_MENTAL_HEALTH_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificateMentalHealths = this.store.select(getMedicalCertificateMentalHealthCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificateMentalHealthCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificateMentalHealthsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificateMentalHealthModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificateMentalHealthModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificateMentalHealthModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_MENTAL_HEALTH;
		} else {
			actionType = modelAction.MedicalCertificateMentalHealthModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_MENTAL_HEALTH_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
				modelAction.MedicalCertificateMentalHealthModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_MENTAL_HEALTHS,
				{
					file: file,
				},
				[new modelAction.MedicalCertificateMentalHealthAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificatementalhealth/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificateMentalHealthModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Mental Health entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Mental Health entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificateMentalHealthModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificateMentalHealthModelActionTypes.DELETE_MEDICAL_CERTIFICATE_MENTAL_HEALTH;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificateMentalHealthModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_MENTAL_HEALTH;
						} else {
							actionType = modelAction.MedicalCertificateMentalHealthModelActionTypes.DELETE_MEDICAL_CERTIFICATE_MENTAL_HEALTH_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_MENTAL_HEALTH when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_MENTAL_HEALTH and DELETE_MEDICAL_CERTIFICATE_MENTAL_HEALTH_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificateMentalHealthModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificateMentalHealthsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
