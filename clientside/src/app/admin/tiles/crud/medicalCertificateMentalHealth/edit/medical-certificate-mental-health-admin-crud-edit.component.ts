...
import * as modelAction from 'src/app/models/medicalCertificateMentalHealth/medical_certificate_mental_health.model.action';
import {MedicalCertificateMentalHealthModelState, MedicalCertificateMentalHealthModelAudit} from 'src/app/models/medicalCertificateMentalHealth/medical_certificate_mental_health.model.state';
...
import {
	getMedicalCertificateMentalHealthModelAuditsByEntityId,
	getMedicalCertificateMentalHealthModelWithId,
	getCountMedicalCertificateMentalHealthModels,
...
import {CrudTileMode} from '../medical-certificate-mental-health-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-mental-health-admin-crud-edit',
	templateUrl: './medical-certificate-mental-health-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-mental-health-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificateMentalHealthCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b1ab042c-76ed-4065-ae87-19d7fe2ea65a';
	 ...
	 */
	medicalCertificateMentalHealthAudits$: Observable<MedicalCertificateMentalHealthModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificateMentalHealth';
	...
	medicalStaffOppositeRelation = 'medicalStaffOfMentalHealths';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificateMentalHealthModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificateMentalHealthModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificateMentalHealthModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalCertificateMentalHealthModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificateMentalHealthModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificateMentalHealthModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificateMentalHealthModel();
		...
		if (this.targetModelId) {
			this.medicalCertificateMentalHealthAudits$ = this.store.select(getMedicalCertificateMentalHealthModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificateMentalHealthModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
			modelAction.MedicalCertificateMentalHealthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_MENTAL_HEALTH,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateMentalHealthModelWithId, this.targetModelId)
			...
			model = new MedicalCertificateMentalHealthModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-mental-health');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
			modelAction.MedicalCertificateMentalHealthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_MENTAL_HEALTH_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificateMentalHealthModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificateMentalHealthAction(
						modelAction.MedicalCertificateMentalHealthModelActionTypes.COUNT_MEDICAL_CERTIFICATE_MENTAL_HEALTHS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
				modelAction.MedicalCertificateMentalHealthModelActionTypes.CREATE_MEDICAL_CERTIFICATE_MENTAL_HEALTH,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificateMentalHealthAction(
				modelAction.MedicalCertificateMentalHealthModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_MENTAL_HEALTH,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificateMentalHealthModel = new MedicalCertificateMentalHealthModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateMentalHealthModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificateMentalHealthModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificateMentalHealthModel.getProps();
		this.modelRelations = MedicalCertificateMentalHealthModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificateMentalHealthModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificateMentalHealthModel {
		let updatedModel: MedicalCertificateMentalHealthModel = new MedicalCertificateMentalHealthModel(model);
