...
import {StockTransactionDetailModelState, StockTransactionDetailModelAudit} from 'src/app/models/stockTransactionDetail/stock_transaction_detail.model.state';
import {StockTransactionDetailModel} from 'src/app/models/stockTransactionDetail/stock_transaction_detail.model';
import {
	getStockTransactionDetailModelAuditsByEntityId,
...
@Component({
	selector: 'cb-stock-transaction-detail-admin-crud-edit',
	templateUrl: './stock-transaction-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./stock-transaction-detail-admin-crud-edit.component.scss',
...
export class AdminStockTransactionDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	stockTransactionDetailAudits$: Observable<StockTransactionDetailModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'stockTransactionDetails';
	...
	transactionDetailTypeOppositeRelation = 'stockTransactionDetails';
	...
	warehouseOppositeRelation = 'stockTransactionDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = StockTransactionDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: StockTransactionDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountStockTransactionDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new StockTransactionDetailModel();
		...
		if (this.targetModelId) {
			this.stockTransactionDetailAudits$ = this.store.select(getStockTransactionDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<StockTransactionDetailModel> = {
		...
		this.store.dispatch(new modelAction.StockTransactionDetailAction(
			modelAction.StockTransactionDetailModelActionTypes.FETCH_STOCK_TRANSACTION_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getStockTransactionDetailModelWithId, this.targetModelId)
			...
			model = new StockTransactionDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.StockTransactionDetailAction(
			modelAction.StockTransactionDetailModelActionTypes.FETCH_STOCK_TRANSACTION_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.StockTransactionDetailAction(
				modelAction.StockTransactionDetailModelActionTypes.CREATE_STOCK_TRANSACTION_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.StockTransactionDetailAction(
				modelAction.StockTransactionDetailModelActionTypes.UPDATE_STOCK_TRANSACTION_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: StockTransactionDetailModel = new StockTransactionDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getStockTransactionDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new StockTransactionDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = StockTransactionDetailModel.getProps();
		this.modelRelations = StockTransactionDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: StockTransactionDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): StockTransactionDetailModel {
		let updatedModel: StockTransactionDetailModel = new StockTransactionDetailModel(model);
