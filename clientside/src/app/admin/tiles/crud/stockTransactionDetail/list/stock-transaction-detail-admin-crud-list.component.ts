...
import * as modelAction from 'src/app/models/stockTransactionDetail/stock_transaction_detail.model.action';
import {StockTransactionDetailModelState} from 'src/app/models/stockTransactionDetail/stock_transaction_detail.model.state';
...
import {
	getStockTransactionDetailCollectionModels,
	getStockTransactionDetailCollectionCount
...
import {CrudTileMode} from '../stock-transaction-detail-admin-crud.component';
...
@Component({
	selector: 'cb-stock-transaction-detail-admin-crud-list',
	templateUrl: './stock-transaction-detail-admin-crud-list.component.html',
	styleUrls: [
		'./stock-transaction-detail-admin-crud-list.component.scss',
...
export class AdminStockTransactionDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<StockTransactionDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...StockTransactionDetailModel.searchFields,
	...
	/**
	 * List of Stock Transaction Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	stockTransactionDetails: Observable<StockTransactionDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	stockTransactionDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = StockTransactionDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = StockTransactionDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '8d091ad2-2b68-444d-b833-7572b2ee2279';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.stockTransactionDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.stockTransactionDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: StockTransactionDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockTransactionDetailAction(
				modelAction.StockTransactionDetailModelActionTypes.INITIALISE_STOCK_TRANSACTION_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockTransactionDetailAction(
				modelAction.StockTransactionDetailModelActionTypes.FETCH_STOCK_TRANSACTION_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.StockTransactionDetailAction(
						modelAction.StockTransactionDetailModelActionTypes.COUNT_STOCK_TRANSACTION_DETAILS,
						null
					)
				]
			))
		...
		this.stockTransactionDetails = this.store.select(getStockTransactionDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getStockTransactionDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.stockTransactionDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('stock-transaction-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: StockTransactionDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.StockTransactionDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.StockTransactionDetailModelActionTypes.EXPORT_ALL_STOCK_TRANSACTION_DETAIL;
		} else {
			actionType = modelAction.StockTransactionDetailModelActionTypes.EXPORT_STOCK_TRANSACTION_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.StockTransactionDetailAction(
				modelAction.StockTransactionDetailModelActionTypes.IMPORT_STOCK_TRANSACTION_DETAILS,
				{
					file: file,
				},
				[new modelAction.StockTransactionDetailAction(
			...
			window.open(environment.API_URL + '/api/stocktransactiondetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: StockTransactionDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Stock Transaction Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Stock Transaction Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.StockTransactionDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.StockTransactionDetailModelActionTypes.DELETE_STOCK_TRANSACTION_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.StockTransactionDetailModelActionTypes.DELETE_ALL_STOCK_TRANSACTION_DETAIL;
						} else {
							actionType = modelAction.StockTransactionDetailModelActionTypes.DELETE_STOCK_TRANSACTION_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.StockTransactionDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_STOCK_TRANSACTION_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_STOCK_TRANSACTION_DETAIL and DELETE_STOCK_TRANSACTION_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<StockTransactionDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = StockTransactionDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.stockTransactionDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
