...
import {AdminStockTransactionDetailCrudListComponent} from './stock-transaction-detail-admin-crud-list.component';
...
import {AdminStockTransactionDetailCrudModule} from '../stock-transaction-detail-admin-crud.module';
import {
 ...
 */
describe('Stock Transaction Detail Crud List Tile Component', () => {

	let fixture;
	let stockTransactionDetailCrudListComponent: AdminStockTransactionDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<StockTransactionDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8d091ad2-2b68-444d-b833-7572b2ee2279';
	const searchContent = 'text';

	const searchableFieldCount = StockTransactionDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'stock-transaction-detail-admin-crud',
		urls: ['stock-transaction-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const stockTransactionDetailDataFactory = new StockTransactionDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = stockTransactionDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getStockTransactionDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getStockTransactionDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminStockTransactionDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminStockTransactionDetailCrudListComponent);
			stockTransactionDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(stockTransactionDetailCrudListComponent.stockTransactionDetailsCount).toBe(defaultData.length);
			...
			expect(stockTransactionDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			stockTransactionDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = stockTransactionDetailDataFactory.createAll(filteredDataSize);
			...
			expect(stockTransactionDetailCrudListComponent.stockTransactionDetailsCount).toBe(filteredDataSize);
			expect(stockTransactionDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(stockTransactionDetailCrudListComponent.pageIndex).toBe(0);
			expect(stockTransactionDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			stockTransactionDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		stockTransactionDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		stockTransactionDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
