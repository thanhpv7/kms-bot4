...
import * as modelAction from 'src/app/models/informedConsent/informed_consent.model.action';
import {InformedConsentModelState, InformedConsentModelAudit} from 'src/app/models/informedConsent/informed_consent.model.state';
import {InformedConsentModel} from 'src/app/models/informedConsent/informed_consent.model';
import {
	getInformedConsentModelAuditsByEntityId,
	getInformedConsentModelWithId,
	getCountInformedConsentModels,
...
import {CrudTileMode} from '../informed-consent-admin-crud.component';
...
@Component({
	selector: 'cb-informed-consent-admin-crud-edit',
	templateUrl: './informed-consent-admin-crud-edit.component.html',
	styleUrls: [
		'./informed-consent-admin-crud-edit.component.scss',
...
export class AdminInformedConsentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '8af5c7ce-6e8f-494a-b163-eee632c11cf4';
	 ...
	 */
	informedConsentAudits$: Observable<InformedConsentModelAudit[]>;
	...
	accompanyingStaffMultiCrudActive: boolean = false;
	accompanyingStaffDisplayName = 'Accompanying Staff';
	...
	accompanyingStaffOppositeRelation = 'accompanyingConsents';
	accompanyingStaffRelations: ModelRelation[] = this.accompanyingStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.accompanyingStaffOppositeRelation) {
	...
	patientGeneralInfoOppositeRelation = 'informedConsents';
	...
	registrationOppositeRelation = 'informedConsents';
	...
	informingStaffMultiCrudActive: boolean = false;
	informingStaffDisplayName = 'Informing Staff';
	...
	informingStaffOppositeRelation = 'informingConsents';
	informingStaffRelations: ModelRelation[] = this.informingStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.informingStaffOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = InformedConsentModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InformedConsentModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InformedConsentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'accompanyingStaff' && this.accompanyingStaffMultiCrudActive) {
				...
				if (relation.id === 'informingStaff' && this.informingStaffMultiCrudActive) {
					exp.fields = StaffModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: InformedConsentModel;
	...
	constructor(
		private readonly store: Store<{ model: InformedConsentModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInformedConsentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InformedConsentModel();
		...
		if (this.targetModelId) {
			this.informedConsentAudits$ = this.store.select(getInformedConsentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InformedConsentModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InformedConsentAction(
			modelAction.InformedConsentModelActionTypes.FETCH_INFORMED_CONSENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInformedConsentModelWithId, this.targetModelId)
			...
			model = new InformedConsentModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('informed-consent');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InformedConsentAction(
			modelAction.InformedConsentModelActionTypes.FETCH_INFORMED_CONSENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InformedConsentModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.accompanyingStaffMultiCrudActive || this.patientGeneralInfoMultiCrudActive || this.registrationMultiCrudActive || this.informingStaffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InformedConsentAction(
						modelAction.InformedConsentModelActionTypes.COUNT_INFORMED_CONSENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InformedConsentAction(
				modelAction.InformedConsentModelActionTypes.CREATE_INFORMED_CONSENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InformedConsentAction(
				modelAction.InformedConsentModelActionTypes.UPDATE_INFORMED_CONSENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InformedConsentModel = new InformedConsentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInformedConsentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InformedConsentModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.accompanyingStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.accompanyingStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.accompanyingStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.accompanyingStaffRelations,
					true
				);
			...
			if (event.entityName === this.informingStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.informingStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.informingStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.informingStaffRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InformedConsentModel.getProps();
		this.modelRelations = InformedConsentModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'accompanyingStaff';
		}).hideElement = this.accompanyingStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'accompanyingStaff').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'informingStaff';
		}).hideElement = this.informingStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'informingStaff').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.accompanyingStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.accompanyingStaffDisplayName, many: false});
		}
		...
		if (this.informingStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.informingStaffDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.accompanyingStaff && this.targetModel.accompanyingStaff.id && this.accompanyingStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.accompanyingStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.accompanyingStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.accompanyingStaffId).subscribe(accompanyingStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.accompanyingStaffRelations, this.isViewOnly),
					accompanyingStaff,
					StaffModel,
		...
		if (this.targetModel.informingStaff && this.targetModel.informingStaff.id && this.informingStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.informingStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.informingStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.informingStaffId).subscribe(informingStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.informingStaffRelations, this.isViewOnly),
					informingStaff,
					StaffModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: InformedConsentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InformedConsentModel {
		let updatedModel: InformedConsentModel = new InformedConsentModel(model);

		if (this.accompanyingStaffMultiCrudActive) {
			updatedModel.accompanyingStaff = event.additionalEntities.accompanyingStaff ? event.additionalEntities.accompanyingStaff : undefined;

			if (!updatedModel.accompanyingStaff && event.payload) {
				event.payload.accompanyingStaffId = null;
			}
		} else if (event.payload?.accompanyingStaffId !== undefined) {
			event.payload.accompanyingStaff = null;
			updatedModel.accompanyingStaff = null;
		}
		...
		if (this.informingStaffMultiCrudActive) {
			updatedModel.informingStaff = event.additionalEntities.informingStaff ? event.additionalEntities.informingStaff : undefined;

			if (!updatedModel.informingStaff && event.payload) {
				event.payload.informingStaffId = null;
			}
		} else if (event.payload?.informingStaffId !== undefined) {
			event.payload.informingStaff = null;
			updatedModel.informingStaff = null;
		}
