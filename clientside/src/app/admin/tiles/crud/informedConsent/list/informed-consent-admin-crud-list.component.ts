...
import {InformedConsentModelState} from 'src/app/models/informedConsent/informed_consent.model.state';
...
import {
	getInformedConsentCollectionModels,
	getInformedConsentCollectionCount
...
@Component({
	selector: 'cb-informed-consent-admin-crud-list',
	templateUrl: './informed-consent-admin-crud-list.component.html',
	styleUrls: [
		'./informed-consent-admin-crud-list.component.scss',
...
export class AdminInformedConsentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InformedConsentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InformedConsentModel.searchFields,
	...
	/**
	 * List of Informed Consents that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	informedConsents: Observable<InformedConsentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	informedConsentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.informedConsentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.informedConsentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InformedConsentAction(
				modelAction.InformedConsentModelActionTypes.INITIALISE_INFORMED_CONSENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InformedConsentAction(
				modelAction.InformedConsentModelActionTypes.FETCH_INFORMED_CONSENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.informedConsents = this.store.select(getInformedConsentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInformedConsentCollectionCount, this.collectionId).subscribe(
				(count) => this.informedConsentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InformedConsentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InformedConsentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InformedConsentModelActionTypes.EXPORT_ALL_INFORMED_CONSENT;
		} else {
			actionType = modelAction.InformedConsentModelActionTypes.EXPORT_INFORMED_CONSENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InformedConsentAction(
				modelAction.InformedConsentModelActionTypes.IMPORT_INFORMED_CONSENTS,
				{
					file: file,
				},
				[new modelAction.InformedConsentAction(
			...
			window.open(environment.API_URL + '/api/informedconsent/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InformedConsentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Informed Consent entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Informed Consent entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InformedConsentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InformedConsentModelActionTypes.DELETE_INFORMED_CONSENT;
						} else if (usingSelected) {
							actionType = modelAction.InformedConsentModelActionTypes.DELETE_ALL_INFORMED_CONSENT;
						} else {
							actionType = modelAction.InformedConsentModelActionTypes.DELETE_INFORMED_CONSENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InformedConsentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INFORMED_CONSENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INFORMED_CONSENT and DELETE_INFORMED_CONSENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InformedConsentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.informedConsentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
