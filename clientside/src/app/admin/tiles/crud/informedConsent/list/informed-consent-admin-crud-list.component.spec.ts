...
import {AdminInformedConsentCrudListComponent} from './informed-consent-admin-crud-list.component';
...
import {AdminInformedConsentCrudModule} from '../informed-consent-admin-crud.module';
import {
	InformedConsentModelState,
	initialState as initialInformedConsentModelState
} from 'src/app/models/informedConsent/informed_consent.model.state';
 ...
 */
describe('Informed Consent Crud List Tile Component', () => {

	let fixture;
	let informedConsentCrudListComponent: AdminInformedConsentCrudListComponent;

	let store: MockStore<{ model: InformedConsentModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let informedConsentModelState: InformedConsentModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InformedConsentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8af5c7ce-6e8f-494a-b163-eee632c11cf4';
	const searchContent = 'text';

	const searchableFieldCount = InformedConsentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'informed-consent-admin-crud',
		urls: ['informed-consent-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const informedConsentDataFactory = new InformedConsentDataFactory();

	const defaultDataSize = 30;
	const defaultData = informedConsentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InformedConsentModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		informedConsentModelState = initialInformedConsentModelState;
		store.setState({model: informedConsentModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInformedConsentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInformedConsentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInformedConsentCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInformedConsentCrudListComponent);
			informedConsentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(informedConsentCrudListComponent.informedConsentsCount).toBe(defaultData.length);
			...
			expect(informedConsentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			informedConsentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = informedConsentDataFactory.createAll(filteredDataSize);
			...
			expect(informedConsentCrudListComponent.informedConsentsCount).toBe(filteredDataSize);
			expect(informedConsentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(informedConsentCrudListComponent.pageIndex).toBe(0);
			expect(informedConsentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			informedConsentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		informedConsentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		informedConsentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
