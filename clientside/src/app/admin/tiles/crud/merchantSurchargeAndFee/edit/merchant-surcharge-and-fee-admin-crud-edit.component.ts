...
import * as modelAction from 'src/app/models/merchantSurchargeAndFee/merchant_surcharge_and_fee.model.action';
import {MerchantSurchargeAndFeeModelState, MerchantSurchargeAndFeeModelAudit} from 'src/app/models/merchantSurchargeAndFee/merchant_surcharge_and_fee.model.state';
import {MerchantSurchargeAndFeeModel} from 'src/app/models/merchantSurchargeAndFee/merchant_surcharge_and_fee.model';
import {
	getMerchantSurchargeAndFeeModelAuditsByEntityId,
...
import {CrudTileMode} from '../merchant-surcharge-and-fee-admin-crud.component';
...
@Component({
	selector: 'cb-merchant-surcharge-and-fee-admin-crud-edit',
	templateUrl: './merchant-surcharge-and-fee-admin-crud-edit.component.html',
	styleUrls: [
		'./merchant-surcharge-and-fee-admin-crud-edit.component.scss',
...
export class AdminMerchantSurchargeAndFeeCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '12004cc2-862c-411e-bb45-3e3257f35763';
	 ...
	 */
	merchantSurchargeAndFeeAudits$: Observable<MerchantSurchargeAndFeeModelAudit[]>;
	...
	merchantOppositeRelation = 'merchantSurchargeAndFees';
	 ...
	 */
	modelProperties: ModelProperty[] = MerchantSurchargeAndFeeModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MerchantSurchargeAndFeeModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MerchantSurchargeAndFeeModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MerchantSurchargeAndFeeModel;
	...
	constructor(
		private readonly store: Store<{ model: MerchantSurchargeAndFeeModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMerchantSurchargeAndFeeModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MerchantSurchargeAndFeeModel();
		...
		if (this.targetModelId) {
			this.merchantSurchargeAndFeeAudits$ = this.store.select(getMerchantSurchargeAndFeeModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MerchantSurchargeAndFeeModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
			modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_MERCHANT_SURCHARGE_AND_FEE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMerchantSurchargeAndFeeModelWithId, this.targetModelId)
			...
			model = new MerchantSurchargeAndFeeModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('merchant-surcharge-and-fee');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
			modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_MERCHANT_SURCHARGE_AND_FEE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MerchantSurchargeAndFeeModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.merchantMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MerchantSurchargeAndFeeAction(
						modelAction.MerchantSurchargeAndFeeModelActionTypes.COUNT_MERCHANT_SURCHARGE_AND_FEES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
				modelAction.MerchantSurchargeAndFeeModelActionTypes.CREATE_MERCHANT_SURCHARGE_AND_FEE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
				modelAction.MerchantSurchargeAndFeeModelActionTypes.UPDATE_MERCHANT_SURCHARGE_AND_FEE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MerchantSurchargeAndFeeModel = new MerchantSurchargeAndFeeModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMerchantSurchargeAndFeeModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MerchantSurchargeAndFeeModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MerchantSurchargeAndFeeModel.getProps();
		this.modelRelations = MerchantSurchargeAndFeeModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MerchantSurchargeAndFeeModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MerchantSurchargeAndFeeModel {
		let updatedModel: MerchantSurchargeAndFeeModel = new MerchantSurchargeAndFeeModel(model);
