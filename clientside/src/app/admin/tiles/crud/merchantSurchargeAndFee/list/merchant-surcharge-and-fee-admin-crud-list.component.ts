...
import {MerchantSurchargeAndFeeModelState} from 'src/app/models/merchantSurchargeAndFee/merchant_surcharge_and_fee.model.state';
...
@Component({
	selector: 'cb-merchant-surcharge-and-fee-admin-crud-list',
	templateUrl: './merchant-surcharge-and-fee-admin-crud-list.component.html',
	styleUrls: [
		'./merchant-surcharge-and-fee-admin-crud-list.component.scss',
...
export class AdminMerchantSurchargeAndFeeCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MerchantSurchargeAndFeeModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MerchantSurchargeAndFeeModel.searchFields,
	...
	/**
	 * List of Merchant Surcharge and Fees that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	merchantSurchargeAndFees: Observable<MerchantSurchargeAndFeeModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	merchantSurchargeAndFeesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.merchantSurchargeAndFeesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.merchantSurchargeAndFeesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
				modelAction.MerchantSurchargeAndFeeModelActionTypes.INITIALISE_MERCHANT_SURCHARGE_AND_FEE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
				modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_MERCHANT_SURCHARGE_AND_FEE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.merchantSurchargeAndFees = this.store.select(getMerchantSurchargeAndFeeCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMerchantSurchargeAndFeeCollectionCount, this.collectionId).subscribe(
				(count) => this.merchantSurchargeAndFeesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MerchantSurchargeAndFeeModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MerchantSurchargeAndFeeModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MerchantSurchargeAndFeeModelActionTypes.EXPORT_ALL_MERCHANT_SURCHARGE_AND_FEE;
		} else {
			actionType = modelAction.MerchantSurchargeAndFeeModelActionTypes.EXPORT_MERCHANT_SURCHARGE_AND_FEE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
				modelAction.MerchantSurchargeAndFeeModelActionTypes.IMPORT_MERCHANT_SURCHARGE_AND_FEES,
				{
					file: file,
				},
				[new modelAction.MerchantSurchargeAndFeeAction(
			...
			window.open(environment.API_URL + '/api/merchantsurchargeandfee/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MerchantSurchargeAndFeeModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Merchant Surcharge and Fee entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Merchant Surcharge and Fee entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MerchantSurchargeAndFeeModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MerchantSurchargeAndFeeModelActionTypes.DELETE_MERCHANT_SURCHARGE_AND_FEE;
						} else if (usingSelected) {
							actionType = modelAction.MerchantSurchargeAndFeeModelActionTypes.DELETE_ALL_MERCHANT_SURCHARGE_AND_FEE;
						} else {
							actionType = modelAction.MerchantSurchargeAndFeeModelActionTypes.DELETE_MERCHANT_SURCHARGE_AND_FEE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MerchantSurchargeAndFeeAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MERCHANT_SURCHARGE_AND_FEE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MERCHANT_SURCHARGE_AND_FEE and DELETE_MERCHANT_SURCHARGE_AND_FEE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MerchantSurchargeAndFeeModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.merchantSurchargeAndFeesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
