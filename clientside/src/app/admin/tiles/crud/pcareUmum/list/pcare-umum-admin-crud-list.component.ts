...
import * as modelAction from 'src/app/models/pcareUmum/pcare_umum.model.action';
import {PcareUmumModelState} from 'src/app/models/pcareUmum/pcare_umum.model.state';
...
import {
	getPcareUmumCollectionModels,
	getPcareUmumCollectionCount
} from 'src/app/models/pcareUmum/pcare_umum.model.selector';
...
import {CrudTileMode} from '../pcare-umum-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-umum-admin-crud-list',
	templateUrl: './pcare-umum-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-umum-admin-crud-list.component.scss',
...
export class AdminPcareUmumCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareUmumModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareUmumModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PcareUmumModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '93f7b8b0-d9c1-4245-9612-85b502b4abeb';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareUmumsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareUmumsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareUmumModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareUmumAction(
				modelAction.PcareUmumModelActionTypes.INITIALISE_PCARE_UMUM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareUmumAction(
				modelAction.PcareUmumModelActionTypes.FETCH_PCARE_UMUM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareUmumAction(
						modelAction.PcareUmumModelActionTypes.COUNT_PCARE_UMUMS,
						null
					)
				]
			))
		...
		this.pcareUmums = this.store.select(getPcareUmumCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareUmumCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareUmumsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareUmumModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareUmumModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareUmumModelActionTypes.EXPORT_ALL_PCARE_UMUM;
		} else {
			actionType = modelAction.PcareUmumModelActionTypes.EXPORT_PCARE_UMUM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareUmumAction(
				modelAction.PcareUmumModelActionTypes.IMPORT_PCARE_UMUMS,
				{
					file: file,
				},
				[new modelAction.PcareUmumAction(
			...
			window.open(environment.API_URL + '/api/pcareumum/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareUmumModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Umum entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Umum entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareUmumModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareUmumModelActionTypes.DELETE_PCARE_UMUM;
						} else if (usingSelected) {
							actionType = modelAction.PcareUmumModelActionTypes.DELETE_ALL_PCARE_UMUM;
						} else {
							actionType = modelAction.PcareUmumModelActionTypes.DELETE_PCARE_UMUM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareUmumAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_UMUM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_UMUM and DELETE_PCARE_UMUM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareUmumModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareUmumModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareUmumsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
