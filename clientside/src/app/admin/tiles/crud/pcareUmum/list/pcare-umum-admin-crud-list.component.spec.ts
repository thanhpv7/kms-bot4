...
import {AdminPcareUmumCrudListComponent} from './pcare-umum-admin-crud-list.component';
import {PcareUmumDataFactory} from 'src/app/lib/utils/factories/pcare-umum-data-factory';
import {PcareUmumModel} from 'src/app/models/pcareUmum/pcare_umum.model';
import {AdminPcareUmumCrudModule} from '../pcare-umum-admin-crud.module';
import {
	PcareUmumModelState,
	initialState as initialPcareUmumModelState
} from 'src/app/models/pcareUmum/pcare_umum.model.state';
 ...
 */
describe('PCare Umum Crud List Tile Component', () => {

	let fixture;
	let pcareUmumCrudListComponent: AdminPcareUmumCrudListComponent;

	let store: MockStore<{ model: PcareUmumModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareUmumModelState: PcareUmumModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareUmumModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '93f7b8b0-d9c1-4245-9612-85b502b4abeb';
	const searchContent = 'text';

	const searchableFieldCount = PcareUmumModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-umum-admin-crud',
		urls: ['pcare-umum-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareUmumDataFactory = new PcareUmumDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareUmumDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareUmumModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareUmumModelState = initialPcareUmumModelState;
		store.setState({model: pcareUmumModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareUmumCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareUmumCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPcareUmumCrudListComponent);
			pcareUmumCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareUmumCrudListComponent.pcareUmumsCount).toBe(defaultData.length);
			...
			expect(pcareUmumCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareUmumCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareUmumDataFactory.createAll(filteredDataSize);
			...
			expect(pcareUmumCrudListComponent.pcareUmumsCount).toBe(filteredDataSize);
			expect(pcareUmumCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareUmumCrudListComponent.pageIndex).toBe(0);
			expect(pcareUmumCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareUmumCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareUmumCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareUmumCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
