...
import {PcareUmumModelState, PcareUmumModelAudit} from 'src/app/models/pcareUmum/pcare_umum.model.state';
import {PcareUmumModel} from 'src/app/models/pcareUmum/pcare_umum.model';
import {
	getPcareUmumModelAuditsByEntityId,
	getPcareUmumModelWithId,
	getCountPcareUmumModels,
...
@Component({
	selector: 'cb-pcare-umum-admin-crud-edit',
	templateUrl: './pcare-umum-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-umum-admin-crud-edit.component.scss',
...
export class AdminPcareUmumCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareUmumAudits$: Observable<PcareUmumModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareUmumModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareUmumModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareUmumModel();
		...
		if (this.targetModelId) {
			this.pcareUmumAudits$ = this.store.select(getPcareUmumModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareUmumModel> = {
		...
		this.store.dispatch(new modelAction.PcareUmumAction(
			modelAction.PcareUmumModelActionTypes.FETCH_PCARE_UMUM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareUmumModelWithId, this.targetModelId)
			...
			model = new PcareUmumModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareUmumAction(
			modelAction.PcareUmumModelActionTypes.FETCH_PCARE_UMUM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareUmumAction(
				modelAction.PcareUmumModelActionTypes.CREATE_PCARE_UMUM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareUmumAction(
				modelAction.PcareUmumModelActionTypes.UPDATE_PCARE_UMUM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareUmumModel = new PcareUmumModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareUmumModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareUmumModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareUmumModel.getProps();
		this.modelRelations = PcareUmumModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareUmumModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareUmumModel {
		let updatedModel: PcareUmumModel = new PcareUmumModel(model);
