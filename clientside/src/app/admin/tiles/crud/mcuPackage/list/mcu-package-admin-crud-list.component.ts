...
import * as modelAction from 'src/app/models/mcuPackage/mcu_package.model.action';
import {McuPackageModelState} from 'src/app/models/mcuPackage/mcu_package.model.state';
...
import {CrudTileMode} from '../mcu-package-admin-crud.component';
...
@Component({
	selector: 'cb-mcu-package-admin-crud-list',
	templateUrl: './mcu-package-admin-crud-list.component.html',
	styleUrls: [
		'./mcu-package-admin-crud-list.component.scss',
...
export class AdminMcuPackageCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<McuPackageModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...McuPackageModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = McuPackageModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ffd84094-a16f-45b8-89eb-77c069bc5053';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.mcuPackagesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.mcuPackagesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: McuPackageModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.McuPackageAction(
				modelAction.McuPackageModelActionTypes.INITIALISE_MCU_PACKAGE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.McuPackageAction(
				modelAction.McuPackageModelActionTypes.FETCH_MCU_PACKAGE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.McuPackageAction(
						modelAction.McuPackageModelActionTypes.COUNT_MCU_PACKAGES,
						null
					)
				]
			))
		...
		this.mcuPackages = this.store.select(getMcuPackageCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMcuPackageCollectionCount, this.collectionId).subscribe(
				(count) => this.mcuPackagesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: McuPackageModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.McuPackageModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.McuPackageModelActionTypes.EXPORT_ALL_MCU_PACKAGE;
		} else {
			actionType = modelAction.McuPackageModelActionTypes.EXPORT_MCU_PACKAGE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.McuPackageAction(
				modelAction.McuPackageModelActionTypes.IMPORT_MCU_PACKAGES,
				{
					file: file,
				},
				[new modelAction.McuPackageAction(
			...
			window.open(environment.API_URL + '/api/mcupackage/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: McuPackageModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected MCU Package entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected MCU Package entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.McuPackageModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.McuPackageModelActionTypes.DELETE_MCU_PACKAGE;
						} else if (usingSelected) {
							actionType = modelAction.McuPackageModelActionTypes.DELETE_ALL_MCU_PACKAGE;
						} else {
							actionType = modelAction.McuPackageModelActionTypes.DELETE_MCU_PACKAGE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.McuPackageAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MCU_PACKAGE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MCU_PACKAGE and DELETE_MCU_PACKAGE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<McuPackageModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = McuPackageModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.mcuPackagesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
