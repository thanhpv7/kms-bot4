...
import {AdminMcuPackageCrudListComponent} from './mcu-package-admin-crud-list.component';
...
import {AdminMcuPackageCrudModule} from '../mcu-package-admin-crud.module';
import {
 ...
 */
describe('MCU Package Crud List Tile Component', () => {

	let fixture;
	let mcuPackageCrudListComponent: AdminMcuPackageCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<McuPackageModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ffd84094-a16f-45b8-89eb-77c069bc5053';
	const searchContent = 'text';

	const searchableFieldCount = McuPackageModel.searchFields.length;

	const routerState: RouterState = {
		url: 'mcu-package-admin-crud',
		urls: ['mcu-package-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const mcuPackageDataFactory = new McuPackageDataFactory();

	const defaultDataSize = 30;
	const defaultData = mcuPackageDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getMcuPackageCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMcuPackageCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminMcuPackageCrudListComponent);
			mcuPackageCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(mcuPackageCrudListComponent.mcuPackagesCount).toBe(defaultData.length);
			...
			expect(mcuPackageCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			mcuPackageCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = mcuPackageDataFactory.createAll(filteredDataSize);
			...
			expect(mcuPackageCrudListComponent.mcuPackagesCount).toBe(filteredDataSize);
			expect(mcuPackageCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(mcuPackageCrudListComponent.pageIndex).toBe(0);
			expect(mcuPackageCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			mcuPackageCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		mcuPackageCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		mcuPackageCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
