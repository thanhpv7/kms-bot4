...
import {McuPackageModelState, McuPackageModelAudit} from 'src/app/models/mcuPackage/mcu_package.model.state';
import {McuPackageModel} from 'src/app/models/mcuPackage/mcu_package.model';
import {
	getMcuPackageModelAuditsByEntityId,
...
@Component({
	selector: 'cb-mcu-package-admin-crud-edit',
	templateUrl: './mcu-package-admin-crud-edit.component.html',
	styleUrls: [
		'./mcu-package-admin-crud-edit.component.scss',
...
export class AdminMcuPackageCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	mcuPackageAudits$: Observable<McuPackageModelAudit[]>;
	...
	mcuPackageItemsOppositeRelation = 'mcuPackage';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = McuPackageModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMcuPackageModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new McuPackageModel();
		...
		if (this.targetModelId) {
			this.mcuPackageAudits$ = this.store.select(getMcuPackageModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<McuPackageModel> = {
		...
		this.store.dispatch(new modelAction.McuPackageAction(
			modelAction.McuPackageModelActionTypes.FETCH_MCU_PACKAGE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMcuPackageModelWithId, this.targetModelId)
			...
			model = new McuPackageModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.McuPackageAction(
			modelAction.McuPackageModelActionTypes.FETCH_MCU_PACKAGE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<McuPackageModel> = {
				...
				queryParams: {
					persistRelations: this.mcuPackageItemsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.McuPackageAction(
				modelAction.McuPackageModelActionTypes.CREATE_MCU_PACKAGE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.McuPackageAction(
				modelAction.McuPackageModelActionTypes.UPDATE_MCU_PACKAGE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: McuPackageModel = new McuPackageModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMcuPackageModelWithId, this.targetModelId).subscribe(model => {
					let differences = new McuPackageModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = McuPackageModel.getProps();
		this.modelRelations = McuPackageModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: McuPackageModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): McuPackageModel {
		let updatedModel: McuPackageModel = new McuPackageModel(model);
