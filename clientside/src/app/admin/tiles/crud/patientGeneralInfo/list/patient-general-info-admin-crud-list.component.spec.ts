...
import {AdminPatientGeneralInfoCrudListComponent} from './patient-general-info-admin-crud-list.component';
...
import {AdminPatientGeneralInfoCrudModule} from '../patient-general-info-admin-crud.module';
import {
 ...
 */
describe('Patient General Info Crud List Tile Component', () => {

	let fixture;
	let patientGeneralInfoCrudListComponent: AdminPatientGeneralInfoCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientGeneralInfoModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a1e2ccec-5d5c-4184-847d-2706d3a5d22d';
	const searchContent = 'text';

	const searchableFieldCount = PatientGeneralInfoModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-general-info-admin-crud',
		urls: ['patient-general-info-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientGeneralInfoDataFactory = new PatientGeneralInfoDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientGeneralInfoDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPatientGeneralInfoCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientGeneralInfoCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientGeneralInfoCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientGeneralInfoCrudListComponent);
			patientGeneralInfoCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientGeneralInfoCrudListComponent.patientGeneralInfosCount).toBe(defaultData.length);
			...
			expect(patientGeneralInfoCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientGeneralInfoCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientGeneralInfoDataFactory.createAll(filteredDataSize);
			...
			expect(patientGeneralInfoCrudListComponent.patientGeneralInfosCount).toBe(filteredDataSize);
			expect(patientGeneralInfoCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientGeneralInfoCrudListComponent.pageIndex).toBe(0);
			expect(patientGeneralInfoCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientGeneralInfoCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientGeneralInfoCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientGeneralInfoCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
