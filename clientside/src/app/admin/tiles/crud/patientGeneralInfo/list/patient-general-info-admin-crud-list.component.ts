...
import {PatientGeneralInfoModelState} from 'src/app/models/patientGeneralInfo/patient_general_info.model.state';
...
import {
	...
	getPatientGeneralInfoCollectionCount
...
import {CrudTileMode} from '../patient-general-info-admin-crud.component';
...
@Component({
	selector: 'cb-patient-general-info-admin-crud-list',
	templateUrl: './patient-general-info-admin-crud-list.component.html',
	styleUrls: [
		'./patient-general-info-admin-crud-list.component.scss',
...
export class AdminPatientGeneralInfoCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientGeneralInfoModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientGeneralInfoModel.searchFields,
	...
	/**
	 * List of Patient General Infos that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientGeneralInfos: Observable<PatientGeneralInfoModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientGeneralInfosCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientGeneralInfosCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientGeneralInfosCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientGeneralInfoAction(
				modelAction.PatientGeneralInfoModelActionTypes.INITIALISE_PATIENT_GENERAL_INFO_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientGeneralInfoAction(
				modelAction.PatientGeneralInfoModelActionTypes.FETCH_PATIENT_GENERAL_INFO_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientGeneralInfos = this.store.select(getPatientGeneralInfoCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientGeneralInfoCollectionCount, this.collectionId).subscribe(
				(count) => this.patientGeneralInfosCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientGeneralInfoModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientGeneralInfoModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientGeneralInfoModelActionTypes.EXPORT_ALL_PATIENT_GENERAL_INFO;
		} else {
			actionType = modelAction.PatientGeneralInfoModelActionTypes.EXPORT_PATIENT_GENERAL_INFO_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientGeneralInfoAction(
				modelAction.PatientGeneralInfoModelActionTypes.IMPORT_PATIENT_GENERAL_INFOS,
				{
					file: file,
				},
				[new modelAction.PatientGeneralInfoAction(
			...
			window.open(environment.API_URL + '/api/patientgeneralinfo/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientGeneralInfoModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient General Info entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient General Info entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientGeneralInfoModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientGeneralInfoModelActionTypes.DELETE_PATIENT_GENERAL_INFO;
						} else if (usingSelected) {
							actionType = modelAction.PatientGeneralInfoModelActionTypes.DELETE_ALL_PATIENT_GENERAL_INFO;
						} else {
							actionType = modelAction.PatientGeneralInfoModelActionTypes.DELETE_PATIENT_GENERAL_INFO_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientGeneralInfoAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_GENERAL_INFO when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_GENERAL_INFO and DELETE_PATIENT_GENERAL_INFO_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientGeneralInfoModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientGeneralInfosCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
