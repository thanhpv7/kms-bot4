...
import {AdminBpjsSearchSEPCrudListComponent} from './bpjs-search-sep-admin-crud-list.component';
import {BpjsSearchSEPDataFactory} from 'src/app/lib/utils/factories/bpjs-search-sep-data-factory';
import {BpjsSearchSEPModel} from 'src/app/models/bpjsSearchSEP/bpjs_search_sep.model';
import {AdminBpjsSearchSEPCrudModule} from '../bpjs-search-sep-admin-crud.module';
import {
	BpjsSearchSEPModelState,
	initialState as initialBpjsSearchSEPModelState
} from 'src/app/models/bpjsSearchSEP/bpjs_search_sep.model.state';
 ...
 */
describe('BPJS Search SEP Crud List Tile Component', () => {

	let fixture;
	let bpjsSearchSEPCrudListComponent: AdminBpjsSearchSEPCrudListComponent;

	let store: MockStore<{ model: BpjsSearchSEPModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsSearchSEPModelState: BpjsSearchSEPModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsSearchSEPModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '2f7a9645-0fe8-4e93-bd76-2bde602c6d19';
	const searchContent = 'text';

	const searchableFieldCount = BpjsSearchSEPModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-search-sep-admin-crud',
		urls: ['bpjs-search-sep-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsSearchSEPDataFactory = new BpjsSearchSEPDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsSearchSEPDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsSearchSEPModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsSearchSEPModelState = initialBpjsSearchSEPModelState;
		store.setState({model: bpjsSearchSEPModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsSearchSEPCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsSearchSEPCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsSearchSEPCrudListComponent);
			bpjsSearchSEPCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsSearchSEPCrudListComponent.bpjsSearchSEPsCount).toBe(defaultData.length);
			...
			expect(bpjsSearchSEPCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsSearchSEPCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsSearchSEPDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsSearchSEPCrudListComponent.bpjsSearchSEPsCount).toBe(filteredDataSize);
			expect(bpjsSearchSEPCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsSearchSEPCrudListComponent.pageIndex).toBe(0);
			expect(bpjsSearchSEPCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsSearchSEPCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsSearchSEPCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsSearchSEPCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
