...
import * as modelAction from 'src/app/models/bpjsSearchSEP/bpjs_search_sep.model.action';
import {BpjsSearchSEPModelState} from 'src/app/models/bpjsSearchSEP/bpjs_search_sep.model.state';
...
import {
	getBpjsSearchSEPCollectionModels,
	getBpjsSearchSEPCollectionCount
} from 'src/app/models/bpjsSearchSEP/bpjs_search_sep.model.selector';
...
import {CrudTileMode} from '../bpjs-search-sep-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-search-sep-admin-crud-list',
	templateUrl: './bpjs-search-sep-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-search-sep-admin-crud-list.component.scss',
...
export class AdminBpjsSearchSEPCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsSearchSEPModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsSearchSEPModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsSearchSEPModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '2f7a9645-0fe8-4e93-bd76-2bde602c6d19';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsSearchSEPsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsSearchSEPsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsSearchSEPModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSearchSEPAction(
				modelAction.BpjsSearchSEPModelActionTypes.INITIALISE_BPJS_SEARCH_SEP_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSearchSEPAction(
				modelAction.BpjsSearchSEPModelActionTypes.FETCH_BPJS_SEARCH_SEP_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsSearchSEPAction(
						modelAction.BpjsSearchSEPModelActionTypes.COUNT_BPJS_SEARCH_SEPS,
						null
					)
				]
			))
		...
		this.bpjsSearchSEPs = this.store.select(getBpjsSearchSEPCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsSearchSEPCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsSearchSEPsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsSearchSEPModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsSearchSEPModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsSearchSEPModelActionTypes.EXPORT_ALL_BPJS_SEARCH_SEP;
		} else {
			actionType = modelAction.BpjsSearchSEPModelActionTypes.EXPORT_BPJS_SEARCH_SEP_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsSearchSEPAction(
				modelAction.BpjsSearchSEPModelActionTypes.IMPORT_BPJS_SEARCH_SEPS,
				{
					file: file,
				},
				[new modelAction.BpjsSearchSEPAction(
			...
			window.open(environment.API_URL + '/api/bpjssearchsep/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsSearchSEPModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Search SEP entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Search SEP entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsSearchSEPModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsSearchSEPModelActionTypes.DELETE_BPJS_SEARCH_SEP;
						} else if (usingSelected) {
							actionType = modelAction.BpjsSearchSEPModelActionTypes.DELETE_ALL_BPJS_SEARCH_SEP;
						} else {
							actionType = modelAction.BpjsSearchSEPModelActionTypes.DELETE_BPJS_SEARCH_SEP_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsSearchSEPAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SEARCH_SEP when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SEARCH_SEP and DELETE_BPJS_SEARCH_SEP_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsSearchSEPModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsSearchSEPModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsSearchSEPsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
