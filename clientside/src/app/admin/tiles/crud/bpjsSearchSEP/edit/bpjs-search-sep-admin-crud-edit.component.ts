...
import {BpjsSearchSEPModelState, BpjsSearchSEPModelAudit} from 'src/app/models/bpjsSearchSEP/bpjs_search_sep.model.state';
import {BpjsSearchSEPModel} from 'src/app/models/bpjsSearchSEP/bpjs_search_sep.model';
import {
	getBpjsSearchSEPModelAuditsByEntityId,
	getBpjsSearchSEPModelWithId,
	getCountBpjsSearchSEPModels,
...
@Component({
	selector: 'cb-bpjs-search-sep-admin-crud-edit',
	templateUrl: './bpjs-search-sep-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-search-sep-admin-crud-edit.component.scss',
...
export class AdminBpjsSearchSEPCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsSearchSEPAudits$: Observable<BpjsSearchSEPModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsSearchSEPModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsSearchSEPModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsSearchSEPModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsSearchSEPModel();
		...
		if (this.targetModelId) {
			this.bpjsSearchSEPAudits$ = this.store.select(getBpjsSearchSEPModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsSearchSEPModel> = {
		...
		this.store.dispatch(new modelAction.BpjsSearchSEPAction(
			modelAction.BpjsSearchSEPModelActionTypes.FETCH_BPJS_SEARCH_SEP,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsSearchSEPModelWithId, this.targetModelId)
			...
			model = new BpjsSearchSEPModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsSearchSEPAction(
			modelAction.BpjsSearchSEPModelActionTypes.FETCH_BPJS_SEARCH_SEP_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsSearchSEPAction(
				modelAction.BpjsSearchSEPModelActionTypes.CREATE_BPJS_SEARCH_SEP,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsSearchSEPAction(
				modelAction.BpjsSearchSEPModelActionTypes.UPDATE_BPJS_SEARCH_SEP,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsSearchSEPModel = new BpjsSearchSEPModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsSearchSEPModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsSearchSEPModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsSearchSEPModel.getProps();
		this.modelRelations = BpjsSearchSEPModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsSearchSEPModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsSearchSEPModel {
		let updatedModel: BpjsSearchSEPModel = new BpjsSearchSEPModel(model);
