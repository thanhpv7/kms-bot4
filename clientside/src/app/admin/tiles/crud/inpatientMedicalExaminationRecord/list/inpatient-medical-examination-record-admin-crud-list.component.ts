...
import * as modelAction from 'src/app/models/inpatientMedicalExaminationRecord/inpatient_medical_examination_record.model.action';
import {InpatientMedicalExaminationRecordModelState} from 'src/app/models/inpatientMedicalExaminationRecord/inpatient_medical_examination_record.model.state';
...
import {
	getInpatientMedicalExaminationRecordCollectionModels,
	getInpatientMedicalExaminationRecordCollectionCount
...
import {CrudTileMode} from '../inpatient-medical-examination-record-admin-crud.component';
...
@Component({
	selector: 'cb-inpatient-medical-examination-record-admin-crud-list',
	templateUrl: './inpatient-medical-examination-record-admin-crud-list.component.html',
	styleUrls: [
		'./inpatient-medical-examination-record-admin-crud-list.component.scss',
...
export class AdminInpatientMedicalExaminationRecordCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InpatientMedicalExaminationRecordModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InpatientMedicalExaminationRecordModel.searchFields,
	...
	/**
	 * List of Inpatient Medical Examination Records that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	inpatientMedicalExaminationRecords: Observable<InpatientMedicalExaminationRecordModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	inpatientMedicalExaminationRecordsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = InpatientMedicalExaminationRecordModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InpatientMedicalExaminationRecordModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '6d84693f-41fd-45df-a524-b92df9b99c82';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.inpatientMedicalExaminationRecordsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.inpatientMedicalExaminationRecordsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: InpatientMedicalExaminationRecordModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
				modelAction.InpatientMedicalExaminationRecordModelActionTypes.INITIALISE_INPATIENT_MEDICAL_EXAMINATION_RECORD_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
				modelAction.InpatientMedicalExaminationRecordModelActionTypes.FETCH_INPATIENT_MEDICAL_EXAMINATION_RECORD_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.InpatientMedicalExaminationRecordAction(
						modelAction.InpatientMedicalExaminationRecordModelActionTypes.COUNT_INPATIENT_MEDICAL_EXAMINATION_RECORDS,
						null
					)
				]
			))
		...
		this.inpatientMedicalExaminationRecords = this.store.select(getInpatientMedicalExaminationRecordCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInpatientMedicalExaminationRecordCollectionCount, this.collectionId).subscribe(
				(count) => this.inpatientMedicalExaminationRecordsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('inpatient-medical-examination-record');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InpatientMedicalExaminationRecordModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InpatientMedicalExaminationRecordModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InpatientMedicalExaminationRecordModelActionTypes.EXPORT_ALL_INPATIENT_MEDICAL_EXAMINATION_RECORD;
		} else {
			actionType = modelAction.InpatientMedicalExaminationRecordModelActionTypes.EXPORT_INPATIENT_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
				modelAction.InpatientMedicalExaminationRecordModelActionTypes.IMPORT_INPATIENT_MEDICAL_EXAMINATION_RECORDS,
				{
					file: file,
				},
				[new modelAction.InpatientMedicalExaminationRecordAction(
			...
			window.open(environment.API_URL + '/api/inpatientmedicalexaminationrecord/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InpatientMedicalExaminationRecordModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Inpatient Medical Examination Record entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Inpatient Medical Examination Record entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InpatientMedicalExaminationRecordModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InpatientMedicalExaminationRecordModelActionTypes.DELETE_INPATIENT_MEDICAL_EXAMINATION_RECORD;
						} else if (usingSelected) {
							actionType = modelAction.InpatientMedicalExaminationRecordModelActionTypes.DELETE_ALL_INPATIENT_MEDICAL_EXAMINATION_RECORD;
						} else {
							actionType = modelAction.InpatientMedicalExaminationRecordModelActionTypes.DELETE_INPATIENT_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INPATIENT_MEDICAL_EXAMINATION_RECORD when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INPATIENT_MEDICAL_EXAMINATION_RECORD and DELETE_INPATIENT_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<InpatientMedicalExaminationRecordModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InpatientMedicalExaminationRecordModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.inpatientMedicalExaminationRecordsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
