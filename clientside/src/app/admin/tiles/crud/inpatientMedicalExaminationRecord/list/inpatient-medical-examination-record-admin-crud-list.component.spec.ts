...
import {AdminInpatientMedicalExaminationRecordCrudListComponent} from './inpatient-medical-examination-record-admin-crud-list.component';
...
import {AdminInpatientMedicalExaminationRecordCrudModule} from '../inpatient-medical-examination-record-admin-crud.module';
import {
	InpatientMedicalExaminationRecordModelState,
	initialState as initialInpatientMedicalExaminationRecordModelState
} from 'src/app/models/inpatientMedicalExaminationRecord/inpatient_medical_examination_record.model.state';
 ...
 */
describe('Inpatient Medical Examination Record Crud List Tile Component', () => {

	let fixture;
	let inpatientMedicalExaminationRecordCrudListComponent: AdminInpatientMedicalExaminationRecordCrudListComponent;

	let store: MockStore<{ model: InpatientMedicalExaminationRecordModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let inpatientMedicalExaminationRecordModelState: InpatientMedicalExaminationRecordModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InpatientMedicalExaminationRecordModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6d84693f-41fd-45df-a524-b92df9b99c82';
	const searchContent = 'text';

	const searchableFieldCount = InpatientMedicalExaminationRecordModel.searchFields.length;

	const routerState: RouterState = {
		url: 'inpatient-medical-examination-record-admin-crud',
		urls: ['inpatient-medical-examination-record-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const inpatientMedicalExaminationRecordDataFactory = new InpatientMedicalExaminationRecordDataFactory();

	const defaultDataSize = 30;
	const defaultData = inpatientMedicalExaminationRecordDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InpatientMedicalExaminationRecordModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		inpatientMedicalExaminationRecordModelState = initialInpatientMedicalExaminationRecordModelState;
		store.setState({model: inpatientMedicalExaminationRecordModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInpatientMedicalExaminationRecordCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInpatientMedicalExaminationRecordCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInpatientMedicalExaminationRecordCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInpatientMedicalExaminationRecordCrudListComponent);
			inpatientMedicalExaminationRecordCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(inpatientMedicalExaminationRecordCrudListComponent.inpatientMedicalExaminationRecordsCount).toBe(defaultData.length);
			...
			expect(inpatientMedicalExaminationRecordCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			inpatientMedicalExaminationRecordCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = inpatientMedicalExaminationRecordDataFactory.createAll(filteredDataSize);
			...
			expect(inpatientMedicalExaminationRecordCrudListComponent.inpatientMedicalExaminationRecordsCount).toBe(filteredDataSize);
			expect(inpatientMedicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(inpatientMedicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
			expect(inpatientMedicalExaminationRecordCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			inpatientMedicalExaminationRecordCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		inpatientMedicalExaminationRecordCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		inpatientMedicalExaminationRecordCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
