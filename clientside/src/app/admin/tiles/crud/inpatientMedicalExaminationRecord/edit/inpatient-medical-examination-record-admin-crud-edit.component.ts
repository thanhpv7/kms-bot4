...
import {InpatientMedicalExaminationRecordModelState, InpatientMedicalExaminationRecordModelAudit} from 'src/app/models/inpatientMedicalExaminationRecord/inpatient_medical_examination_record.model.state';
...
import {
	getInpatientMedicalExaminationRecordModelAuditsByEntityId,
	getInpatientMedicalExaminationRecordModelWithId,
	getCountInpatientMedicalExaminationRecordModels,
...
@Component({
	selector: 'cb-inpatient-medical-examination-record-admin-crud-edit',
	templateUrl: './inpatient-medical-examination-record-admin-crud-edit.component.html',
	styleUrls: [
		'./inpatient-medical-examination-record-admin-crud-edit.component.scss',
...
export class AdminInpatientMedicalExaminationRecordCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	inpatientMedicalExaminationRecordAudits$: Observable<InpatientMedicalExaminationRecordModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'inpatientMedicalExaminationRecord';
	...
	bedFacilityOppositeRelation = 'inpatientMedicalExaminationRecords';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InpatientMedicalExaminationRecordModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: InpatientMedicalExaminationRecordModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInpatientMedicalExaminationRecordModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InpatientMedicalExaminationRecordModel();
		...
		if (this.targetModelId) {
			this.inpatientMedicalExaminationRecordAudits$ = this.store.select(getInpatientMedicalExaminationRecordModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InpatientMedicalExaminationRecordModel> = {
		...
		this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
			modelAction.InpatientMedicalExaminationRecordModelActionTypes.FETCH_INPATIENT_MEDICAL_EXAMINATION_RECORD,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInpatientMedicalExaminationRecordModelWithId, this.targetModelId)
			...
			model = new InpatientMedicalExaminationRecordModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
			modelAction.InpatientMedicalExaminationRecordModelActionTypes.FETCH_INPATIENT_MEDICAL_EXAMINATION_RECORD_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InpatientMedicalExaminationRecordModel> = {
				...
				queryParams: {
					persistRelations: this.medicalExaminationRecordMultiCrudActive || this.bedFacilityMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
				modelAction.InpatientMedicalExaminationRecordModelActionTypes.CREATE_INPATIENT_MEDICAL_EXAMINATION_RECORD,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InpatientMedicalExaminationRecordAction(
				modelAction.InpatientMedicalExaminationRecordModelActionTypes.UPDATE_INPATIENT_MEDICAL_EXAMINATION_RECORD,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InpatientMedicalExaminationRecordModel = new InpatientMedicalExaminationRecordModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInpatientMedicalExaminationRecordModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InpatientMedicalExaminationRecordModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InpatientMedicalExaminationRecordModel.getProps();
		this.modelRelations = InpatientMedicalExaminationRecordModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: InpatientMedicalExaminationRecordModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InpatientMedicalExaminationRecordModel {
		let updatedModel: InpatientMedicalExaminationRecordModel = new InpatientMedicalExaminationRecordModel(model);
