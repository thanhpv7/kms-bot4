...
import {BpjsTreatmentClassMappingModelState, BpjsTreatmentClassMappingModelAudit} from 'src/app/models/bpjsTreatmentClassMapping/bpjs_treatment_class_mapping.model.state';
import {BpjsTreatmentClassMappingModel} from 'src/app/models/bpjsTreatmentClassMapping/bpjs_treatment_class_mapping.model';
import {
	getBpjsTreatmentClassMappingModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-treatment-class-mapping-admin-crud-edit',
	templateUrl: './bpjs-treatment-class-mapping-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-treatment-class-mapping-admin-crud-edit.component.scss',
...
export class AdminBpjsTreatmentClassMappingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsTreatmentClassMappingAudits$: Observable<BpjsTreatmentClassMappingModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsTreatmentClassMappingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsTreatmentClassMappingModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsTreatmentClassMappingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsTreatmentClassMappingModel();
		...
		if (this.targetModelId) {
			this.bpjsTreatmentClassMappingAudits$ = this.store.select(getBpjsTreatmentClassMappingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsTreatmentClassMappingModel> = {
		...
		this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
			modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_BPJS_TREATMENT_CLASS_MAPPING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsTreatmentClassMappingModelWithId, this.targetModelId)
			...
			model = new BpjsTreatmentClassMappingModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
			modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_BPJS_TREATMENT_CLASS_MAPPING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
				modelAction.BpjsTreatmentClassMappingModelActionTypes.CREATE_BPJS_TREATMENT_CLASS_MAPPING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
				modelAction.BpjsTreatmentClassMappingModelActionTypes.UPDATE_BPJS_TREATMENT_CLASS_MAPPING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsTreatmentClassMappingModel = new BpjsTreatmentClassMappingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsTreatmentClassMappingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsTreatmentClassMappingModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsTreatmentClassMappingModel.getProps();
		this.modelRelations = BpjsTreatmentClassMappingModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsTreatmentClassMappingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsTreatmentClassMappingModel {
		let updatedModel: BpjsTreatmentClassMappingModel = new BpjsTreatmentClassMappingModel(model);
