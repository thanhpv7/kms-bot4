...
import * as modelAction from 'src/app/models/bpjsTreatmentClassMapping/bpjs_treatment_class_mapping.model.action';
import {BpjsTreatmentClassMappingModelState} from 'src/app/models/bpjsTreatmentClassMapping/bpjs_treatment_class_mapping.model.state';
...
import {
	...
	getBpjsTreatmentClassMappingCollectionCount
...
import {CrudTileMode} from '../bpjs-treatment-class-mapping-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-treatment-class-mapping-admin-crud-list',
	templateUrl: './bpjs-treatment-class-mapping-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-treatment-class-mapping-admin-crud-list.component.scss',
...
export class AdminBpjsTreatmentClassMappingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsTreatmentClassMappingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsTreatmentClassMappingModel.searchFields,
	...
	/**
	 * List of BPJS Treatment Class Mappings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsTreatmentClassMappings: Observable<BpjsTreatmentClassMappingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsTreatmentClassMappingsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsTreatmentClassMappingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsTreatmentClassMappingModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '101ac4fd-92d9-49bc-a073-d8e30a9ae92c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsTreatmentClassMappingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsTreatmentClassMappingsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsTreatmentClassMappingModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
				modelAction.BpjsTreatmentClassMappingModelActionTypes.INITIALISE_BPJS_TREATMENT_CLASS_MAPPING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
				modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_BPJS_TREATMENT_CLASS_MAPPING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsTreatmentClassMappingAction(
						modelAction.BpjsTreatmentClassMappingModelActionTypes.COUNT_BPJS_TREATMENT_CLASS_MAPPINGS,
						null
					)
				]
			))
		...
		this.bpjsTreatmentClassMappings = this.store.select(getBpjsTreatmentClassMappingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsTreatmentClassMappingCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsTreatmentClassMappingsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-treatment-class-mapping');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsTreatmentClassMappingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsTreatmentClassMappingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsTreatmentClassMappingModelActionTypes.EXPORT_ALL_BPJS_TREATMENT_CLASS_MAPPING;
		} else {
			actionType = modelAction.BpjsTreatmentClassMappingModelActionTypes.EXPORT_BPJS_TREATMENT_CLASS_MAPPING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
				modelAction.BpjsTreatmentClassMappingModelActionTypes.IMPORT_BPJS_TREATMENT_CLASS_MAPPINGS,
				{
					file: file,
				},
				[new modelAction.BpjsTreatmentClassMappingAction(
			...
			window.open(environment.API_URL + '/api/bpjstreatmentclassmapping/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsTreatmentClassMappingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Treatment Class Mapping entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Treatment Class Mapping entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsTreatmentClassMappingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsTreatmentClassMappingModelActionTypes.DELETE_BPJS_TREATMENT_CLASS_MAPPING;
						} else if (usingSelected) {
							actionType = modelAction.BpjsTreatmentClassMappingModelActionTypes.DELETE_ALL_BPJS_TREATMENT_CLASS_MAPPING;
						} else {
							actionType = modelAction.BpjsTreatmentClassMappingModelActionTypes.DELETE_BPJS_TREATMENT_CLASS_MAPPING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsTreatmentClassMappingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_TREATMENT_CLASS_MAPPING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_TREATMENT_CLASS_MAPPING and DELETE_BPJS_TREATMENT_CLASS_MAPPING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsTreatmentClassMappingModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsTreatmentClassMappingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsTreatmentClassMappingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
