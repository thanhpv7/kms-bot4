...
import {AdminPcareExaminationCrudListComponent} from './pcare-examination-admin-crud-list.component';
...
import {AdminPcareExaminationCrudModule} from '../pcare-examination-admin-crud.module';
import {
	PcareExaminationModelState,
	initialState as initialPcareExaminationModelState
} from 'src/app/models/pcareExamination/pcare_examination.model.state';
 ...
 */
describe('PCare Examination Crud List Tile Component', () => {

	let fixture;
	let pcareExaminationCrudListComponent: AdminPcareExaminationCrudListComponent;

	let store: MockStore<{ model: PcareExaminationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareExaminationModelState: PcareExaminationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareExaminationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f6fd0d9e-c356-45ad-8740-1dc1668f44b3';
	const searchContent = 'text';

	const searchableFieldCount = PcareExaminationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-examination-admin-crud',
		urls: ['pcare-examination-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareExaminationDataFactory = new PcareExaminationDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareExaminationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareExaminationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareExaminationModelState = initialPcareExaminationModelState;
		store.setState({model: pcareExaminationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareExaminationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareExaminationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPcareExaminationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPcareExaminationCrudListComponent);
			pcareExaminationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareExaminationCrudListComponent.pcareExaminationsCount).toBe(defaultData.length);
			...
			expect(pcareExaminationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareExaminationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareExaminationDataFactory.createAll(filteredDataSize);
			...
			expect(pcareExaminationCrudListComponent.pcareExaminationsCount).toBe(filteredDataSize);
			expect(pcareExaminationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareExaminationCrudListComponent.pageIndex).toBe(0);
			expect(pcareExaminationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareExaminationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareExaminationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareExaminationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
