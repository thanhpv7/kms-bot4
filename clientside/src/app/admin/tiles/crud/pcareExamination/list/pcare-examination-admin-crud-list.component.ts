...
import {PcareExaminationModelState} from 'src/app/models/pcareExamination/pcare_examination.model.state';
...
import {
	getPcareExaminationCollectionModels,
	getPcareExaminationCollectionCount
...
@Component({
	selector: 'cb-pcare-examination-admin-crud-list',
	templateUrl: './pcare-examination-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-examination-admin-crud-list.component.scss',
...
export class AdminPcareExaminationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareExaminationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareExaminationModel.searchFields,
	...
	/**
	 * List of PCare Examinations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareExaminations: Observable<PcareExaminationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareExaminationsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareExaminationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareExaminationsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareExaminationAction(
				modelAction.PcareExaminationModelActionTypes.INITIALISE_PCARE_EXAMINATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareExaminationAction(
				modelAction.PcareExaminationModelActionTypes.FETCH_PCARE_EXAMINATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.pcareExaminations = this.store.select(getPcareExaminationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareExaminationCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareExaminationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareExaminationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareExaminationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareExaminationModelActionTypes.EXPORT_ALL_PCARE_EXAMINATION;
		} else {
			actionType = modelAction.PcareExaminationModelActionTypes.EXPORT_PCARE_EXAMINATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareExaminationAction(
				modelAction.PcareExaminationModelActionTypes.IMPORT_PCARE_EXAMINATIONS,
				{
					file: file,
				},
				[new modelAction.PcareExaminationAction(
			...
			window.open(environment.API_URL + '/api/pcareexamination/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareExaminationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Examination entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Examination entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareExaminationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareExaminationModelActionTypes.DELETE_PCARE_EXAMINATION;
						} else if (usingSelected) {
							actionType = modelAction.PcareExaminationModelActionTypes.DELETE_ALL_PCARE_EXAMINATION;
						} else {
							actionType = modelAction.PcareExaminationModelActionTypes.DELETE_PCARE_EXAMINATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareExaminationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_EXAMINATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_EXAMINATION and DELETE_PCARE_EXAMINATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareExaminationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareExaminationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
