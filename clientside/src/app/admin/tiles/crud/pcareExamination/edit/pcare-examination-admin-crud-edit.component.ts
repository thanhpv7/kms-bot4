...
import * as modelAction from 'src/app/models/pcareExamination/pcare_examination.model.action';
import {PcareExaminationModelState, PcareExaminationModelAudit} from 'src/app/models/pcareExamination/pcare_examination.model.state';
import {PcareExaminationModel} from 'src/app/models/pcareExamination/pcare_examination.model';
import {
	getPcareExaminationModelAuditsByEntityId,
	getPcareExaminationModelWithId,
	getCountPcareExaminationModels,
...
import * as pcareActionExaminationModelAction from 'src/app/models/pcareActionExamination/pcare_action_examination.model.action';
import {
	getPcareActionExaminationCollectionModels, getPcareActionExaminationModelWithId,
} from 'src/app/models/pcareActionExamination/pcare_action_examination.model.selector';
import {PcareCompoundDrugModel} from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model';
import * as pcareCompoundDrugModelAction from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model.action';
import {
	getPcareCompoundDrugCollectionModels, getPcareCompoundDrugModelWithId,
} from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model.selector';
import {PcareNonCompoundDrugModel} from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model';
import * as pcareNonCompoundDrugModelAction from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model.action';
import {
	getPcareNonCompoundDrugCollectionModels, getPcareNonCompoundDrugModelWithId,
} from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model.selector';
...
import {CrudTileMode} from '../pcare-examination-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-examination-admin-crud-edit',
	templateUrl: './pcare-examination-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-examination-admin-crud-edit.component.scss',
...
export class AdminPcareExaminationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'f6fd0d9e-c356-45ad-8740-1dc1668f44b3';
	 ...
	 */
	pcareExaminationAudits$: Observable<PcareExaminationModelAudit[]>;
	...
	registrationOppositeRelation = 'pcareExamination';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Examination and PCare Action Examinations here] off begin
	pcareActionExaminationsMultiCrudActive: boolean = false;
	pcareActionExaminationsDisplayName = 'PCare Action Examinations';
	pcareActionExaminationsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Examination and PCare Action Examinations here] end
	pcareActionExaminationsOppositeRelation = 'pcareExamination';
	pcareActionExaminationsRelations: ModelRelation[] = this.pcareActionExaminationsMultiCrudActive ? PcareActionExaminationModel.getRelations()
		.map(relation => {
			if (relation.id === this.pcareActionExaminationsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Examination and PCare Compound Drugs here] off begin
	pcareCompoundDrugsMultiCrudActive: boolean = false;
	pcareCompoundDrugsDisplayName = 'PCare Compound Drugs';
	pcareCompoundDrugsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Examination and PCare Compound Drugs here] end
	pcareCompoundDrugsOppositeRelation = 'pcareExamination';
	pcareCompoundDrugsRelations: ModelRelation[] = this.pcareCompoundDrugsMultiCrudActive ? PcareCompoundDrugModel.getRelations()
		.map(relation => {
			if (relation.id === this.pcareCompoundDrugsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Examination and PCare NonCompound Drugs here] off begin
	pcareNonCompoundDrugsMultiCrudActive: boolean = false;
	pcareNonCompoundDrugsDisplayName = 'PCare NonCompound Drugs';
	pcareNonCompoundDrugsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Examination and PCare NonCompound Drugs here] end
	pcareNonCompoundDrugsOppositeRelation = 'pcareExamination';
	pcareNonCompoundDrugsRelations: ModelRelation[] = this.pcareNonCompoundDrugsMultiCrudActive ? PcareNonCompoundDrugModel.getRelations()
		.map(relation => {
			if (relation.id === this.pcareNonCompoundDrugsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = PcareExaminationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareExaminationModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareExaminationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'pcareActionExaminations' && this.pcareActionExaminationsMultiCrudActive) {
					exp.fields = PcareActionExaminationModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'pcareCompoundDrugs' && this.pcareCompoundDrugsMultiCrudActive) {
					exp.fields = PcareCompoundDrugModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'pcareNonCompoundDrugs' && this.pcareNonCompoundDrugsMultiCrudActive) {
					exp.fields = PcareNonCompoundDrugModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: PcareExaminationModel;
	...
	constructor(
		private readonly store: Store<{ model: PcareExaminationModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareExaminationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareExaminationModel();
		...
		if (this.targetModelId) {
			this.pcareExaminationAudits$ = this.store.select(getPcareExaminationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareExaminationModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PcareExaminationAction(
			modelAction.PcareExaminationModelActionTypes.FETCH_PCARE_EXAMINATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareExaminationModelWithId, this.targetModelId)
			...
			model = new PcareExaminationModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-examination');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareExaminationAction(
			modelAction.PcareExaminationModelActionTypes.FETCH_PCARE_EXAMINATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PcareExaminationModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.registrationMultiCrudActive || this.pcareActionExaminationsMultiCrudActive || this.pcareCompoundDrugsMultiCrudActive || this.pcareNonCompoundDrugsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PcareExaminationAction(
						modelAction.PcareExaminationModelActionTypes.COUNT_PCARE_EXAMINATIONS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PcareExaminationAction(
				modelAction.PcareExaminationModelActionTypes.CREATE_PCARE_EXAMINATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareExaminationAction(
				modelAction.PcareExaminationModelActionTypes.UPDATE_PCARE_EXAMINATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareExaminationModel = new PcareExaminationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareExaminationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareExaminationModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.pcareActionExaminationsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PcareActionExaminationModel.getProps(), this.pcareActionExaminationsRelations, this.isViewOnly),
					new PcareActionExaminationModel(),
					PcareActionExaminationModel,
					this.pcareActionExaminationsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.pcareActionExaminationsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.pcareCompoundDrugsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PcareCompoundDrugModel.getProps(), this.pcareCompoundDrugsRelations, this.isViewOnly),
					new PcareCompoundDrugModel(),
					PcareCompoundDrugModel,
					this.pcareCompoundDrugsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.pcareCompoundDrugsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.pcareNonCompoundDrugsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PcareNonCompoundDrugModel.getProps(), this.pcareNonCompoundDrugsRelations, this.isViewOnly),
					new PcareNonCompoundDrugModel(),
					PcareNonCompoundDrugModel,
					this.pcareNonCompoundDrugsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.pcareNonCompoundDrugsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareExaminationModel.getProps();
		this.modelRelations = PcareExaminationModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'pcareActionExaminations';
		}).hideElement = this.pcareActionExaminationsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'pcareActionExaminations').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'pcareCompoundDrugs';
		}).hideElement = this.pcareCompoundDrugsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'pcareCompoundDrugs').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'pcareNonCompoundDrugs';
		}).hideElement = this.pcareNonCompoundDrugsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'pcareNonCompoundDrugs').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.pcareActionExaminationsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.pcareActionExaminationsDisplayName, many: true});
			this.store.dispatch(new pcareActionExaminationModelAction.PcareActionExaminationAction(
				pcareActionExaminationModelAction.PcareActionExaminationModelActionTypes.INITIALISE_PCARE_ACTION_EXAMINATION_COLLECTION_STATE,
				{
					collectionId: this.pcareActionExaminationsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getPcareActionExaminationCollectionModels, this.pcareActionExaminationsCollectionId).subscribe(models => {
				models.forEach(pcareActionExaminations => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(PcareActionExaminationModel.getProps(), this.pcareActionExaminationsRelations, this.isViewOnly),
		...
		if (this.pcareCompoundDrugsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.pcareCompoundDrugsDisplayName, many: true});
			this.store.dispatch(new pcareCompoundDrugModelAction.PcareCompoundDrugAction(
				pcareCompoundDrugModelAction.PcareCompoundDrugModelActionTypes.INITIALISE_PCARE_COMPOUND_DRUG_COLLECTION_STATE,
				{
					collectionId: this.pcareCompoundDrugsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getPcareCompoundDrugCollectionModels, this.pcareCompoundDrugsCollectionId).subscribe(models => {
				models.forEach(pcareCompoundDrugs => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(PcareCompoundDrugModel.getProps(), this.pcareCompoundDrugsRelations, this.isViewOnly),
						pcareCompoundDrugs,
						PcareCompoundDrugModel,
		...
		if (this.pcareNonCompoundDrugsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.pcareNonCompoundDrugsDisplayName, many: true});
			this.store.dispatch(new pcareNonCompoundDrugModelAction.PcareNonCompoundDrugAction(
				pcareNonCompoundDrugModelAction.PcareNonCompoundDrugModelActionTypes.INITIALISE_PCARE_NONCOMPOUND_DRUG_COLLECTION_STATE,
				{
					collectionId: this.pcareNonCompoundDrugsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getPcareNonCompoundDrugCollectionModels, this.pcareNonCompoundDrugsCollectionId).subscribe(models => {
				models.forEach(pcareNonCompoundDrugs => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(PcareNonCompoundDrugModel.getProps(), this.pcareNonCompoundDrugsRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.pcareActionExaminationsIds && this.targetModel.pcareActionExaminationsIds.length > 0 && this.pcareActionExaminationsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PcareActionExaminationModel,
				this.pcareActionExaminationsOppositeRelation,
				this.pcareActionExaminationsCollectionId,
				this.targetModel.pcareActionExaminationsIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.pcareCompoundDrugsIds && this.targetModel.pcareCompoundDrugsIds.length > 0 && this.pcareCompoundDrugsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PcareCompoundDrugModel,
				this.pcareCompoundDrugsOppositeRelation,
				this.pcareCompoundDrugsCollectionId,
				this.targetModel.pcareCompoundDrugsIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.pcareNonCompoundDrugsIds && this.targetModel.pcareNonCompoundDrugsIds.length > 0 && this.pcareNonCompoundDrugsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PcareNonCompoundDrugModel,
				this.pcareNonCompoundDrugsOppositeRelation,
				this.pcareNonCompoundDrugsCollectionId,
				this.targetModel.pcareNonCompoundDrugsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareExaminationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareExaminationModel {
		let updatedModel: PcareExaminationModel = new PcareExaminationModel(model);
		...
		updatedModel.pcareActionExaminations = [];
		if (this.pcareActionExaminationsMultiCrudActive) {
			if (event.additionalEntities.pcareActionExaminations) {
				event.additionalEntities.pcareActionExaminations.forEach(entity => {
					updatedModel.pcareActionExaminations.push(entity);
				});
			}

			if (!updatedModel.pcareActionExaminations.length) {
				updatedModel.pcareActionExaminationsIds = [];
			}
		}
		updatedModel.pcareCompoundDrugs = [];
		if (this.pcareCompoundDrugsMultiCrudActive) {
			if (event.additionalEntities.pcareCompoundDrugs) {
				event.additionalEntities.pcareCompoundDrugs.forEach(entity => {
					updatedModel.pcareCompoundDrugs.push(entity);
				});
			}

			if (!updatedModel.pcareCompoundDrugs.length) {
				updatedModel.pcareCompoundDrugsIds = [];
			}
		}
		updatedModel.pcareNonCompoundDrugs = [];
		if (this.pcareNonCompoundDrugsMultiCrudActive) {
			if (event.additionalEntities.pcareNonCompoundDrugs) {
				event.additionalEntities.pcareNonCompoundDrugs.forEach(entity => {
					updatedModel.pcareNonCompoundDrugs.push(entity);
				});
			}

			if (!updatedModel.pcareNonCompoundDrugs.length) {
				updatedModel.pcareNonCompoundDrugsIds = [];
			}
		}
		return updatedModel;
	}
