...
import {AdminTariffDefinitionCrudListComponent} from './tariff-definition-admin-crud-list.component';
...
import {AdminTariffDefinitionCrudModule} from '../tariff-definition-admin-crud.module';
import {
 ...
 */
describe('Tariff Definition Crud List Tile Component', () => {

	let fixture;
	let tariffDefinitionCrudListComponent: AdminTariffDefinitionCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<TariffDefinitionModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'bb493f41-5538-4e05-a741-5e9dec312112';
	const searchContent = 'text';

	const searchableFieldCount = TariffDefinitionModel.searchFields.length;

	const routerState: RouterState = {
		url: 'tariff-definition-admin-crud',
		urls: ['tariff-definition-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const tariffDefinitionDataFactory = new TariffDefinitionDataFactory();

	const defaultDataSize = 30;
	const defaultData = tariffDefinitionDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getTariffDefinitionCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTariffDefinitionCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminTariffDefinitionCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminTariffDefinitionCrudListComponent);
			tariffDefinitionCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(tariffDefinitionCrudListComponent.tariffDefinitionsCount).toBe(defaultData.length);
			...
			expect(tariffDefinitionCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			tariffDefinitionCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = tariffDefinitionDataFactory.createAll(filteredDataSize);
			...
			expect(tariffDefinitionCrudListComponent.tariffDefinitionsCount).toBe(filteredDataSize);
			expect(tariffDefinitionCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(tariffDefinitionCrudListComponent.pageIndex).toBe(0);
			expect(tariffDefinitionCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			tariffDefinitionCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		tariffDefinitionCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		tariffDefinitionCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
