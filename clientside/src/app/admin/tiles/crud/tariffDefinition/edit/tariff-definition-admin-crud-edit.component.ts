...
import {TariffDefinitionModelState, TariffDefinitionModelAudit} from 'src/app/models/tariffDefinition/tariff_definition.model.state';
...
import {
	getTariffDefinitionModelAuditsByEntityId,
...
@Component({
	selector: 'cb-tariff-definition-admin-crud-edit',
	templateUrl: './tariff-definition-admin-crud-edit.component.html',
	styleUrls: [
		'./tariff-definition-admin-crud-edit.component.scss',
...
export class AdminTariffDefinitionCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	tariffDefinitionAudits$: Observable<TariffDefinitionModelAudit[]>;
	...
	priceSchemaMultiCrudActive: boolean = false;
	priceSchemaDisplayName = 'Price Schema';
	...
	priceSchemaOppositeRelation = 'tariffDefinitions';
	priceSchemaRelations: ModelRelation[] = this.priceSchemaMultiCrudActive ? TariffSchemaModel.getRelations()
		.map(relation => {
			if (relation.id === this.priceSchemaOppositeRelation) {
	...
	tariffDefinitionOtherComponentsOppositeRelation = 'priceDefinition';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TariffDefinitionModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'priceSchema' && this.priceSchemaMultiCrudActive) {
	 ...
	 */
	targetModel: TariffDefinitionModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTariffDefinitionModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TariffDefinitionModel();
		...
		if (this.targetModelId) {
			this.tariffDefinitionAudits$ = this.store.select(getTariffDefinitionModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TariffDefinitionModel> = {
		...
		this.store.dispatch(new modelAction.TariffDefinitionAction(
			modelAction.TariffDefinitionModelActionTypes.FETCH_TARIFF_DEFINITION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTariffDefinitionModelWithId, this.targetModelId)
			...
			model = new TariffDefinitionModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TariffDefinitionAction(
			modelAction.TariffDefinitionModelActionTypes.FETCH_TARIFF_DEFINITION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TariffDefinitionModel> = {
				...
				queryParams: {
					persistRelations: this.priceSchemaMultiCrudActive || this.tariffDefinitionOtherComponentsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.TariffDefinitionAction(
				modelAction.TariffDefinitionModelActionTypes.CREATE_TARIFF_DEFINITION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TariffDefinitionAction(
				modelAction.TariffDefinitionModelActionTypes.UPDATE_TARIFF_DEFINITION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TariffDefinitionModel = new TariffDefinitionModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTariffDefinitionModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TariffDefinitionModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.priceSchemaDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffSchemaModel.getProps(), this.priceSchemaRelations, this.isViewOnly),
					new TariffSchemaModel(),
					TariffSchemaModel,
					this.priceSchemaDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.priceSchemaRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TariffDefinitionModel.getProps();
		this.modelRelations = TariffDefinitionModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'priceSchema';
		}).hideElement = this.priceSchemaMultiCrudActive || this.modelRelations.find(rel => rel.id === 'priceSchema').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.priceSchemaMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.priceSchemaDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.priceSchema && this.targetModel.priceSchema.id && this.priceSchemaMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffSchemaModel,
				this.priceSchemaOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.priceSchema.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffSchemaModelWithId, this.targetModel.priceSchemaId).subscribe(priceSchema => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffSchemaModel.getProps(), this.priceSchemaRelations, this.isViewOnly),
					priceSchema,
					TariffSchemaModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: TariffDefinitionModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TariffDefinitionModel {
		let updatedModel: TariffDefinitionModel = new TariffDefinitionModel(model);

		if (this.priceSchemaMultiCrudActive) {
			updatedModel.priceSchema = event.additionalEntities.priceSchema ? event.additionalEntities.priceSchema : undefined;

			if (!updatedModel.priceSchema && event.payload) {
				event.payload.priceSchemaId = null;
			}
		} else if (event.payload?.priceSchemaId !== undefined) {
			event.payload.priceSchema = null;
			updatedModel.priceSchema = null;
		}
