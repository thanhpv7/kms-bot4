...
import {RegistrationModelState} from 'src/app/models/registration/registration.model.state';
...
import {
	...
	getRegistrationCollectionCount
...
@Component({
	selector: 'cb-registration-admin-crud-list',
	templateUrl: './registration-admin-crud-list.component.html',
	styleUrls: [
		'./registration-admin-crud-list.component.scss',
...
export class AdminRegistrationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<RegistrationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...RegistrationModel.searchFields,
	...
	/**
	 * List of Registrations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	registrations: Observable<RegistrationModel[]>;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.registrationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.registrationsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RegistrationAction(
				modelAction.RegistrationModelActionTypes.INITIALISE_REGISTRATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RegistrationAction(
				modelAction.RegistrationModelActionTypes.FETCH_REGISTRATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.registrations = this.store.select(getRegistrationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getRegistrationCollectionCount, this.collectionId).subscribe(
				(count) => this.registrationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: RegistrationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.RegistrationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.RegistrationModelActionTypes.EXPORT_ALL_REGISTRATION;
		} else {
			actionType = modelAction.RegistrationModelActionTypes.EXPORT_REGISTRATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.RegistrationAction(
				modelAction.RegistrationModelActionTypes.IMPORT_REGISTRATIONS,
				{
					file: file,
				},
				[new modelAction.RegistrationAction(
			...
			window.open(environment.API_URL + '/api/registration/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: RegistrationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Registration entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Registration entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.RegistrationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.RegistrationModelActionTypes.DELETE_REGISTRATION;
						} else if (usingSelected) {
							actionType = modelAction.RegistrationModelActionTypes.DELETE_ALL_REGISTRATION;
						} else {
							actionType = modelAction.RegistrationModelActionTypes.DELETE_REGISTRATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.RegistrationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_REGISTRATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_REGISTRATION and DELETE_REGISTRATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = RegistrationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.registrationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
