...
import {AdminRegistrationCrudListComponent} from './registration-admin-crud-list.component';
...
import {AdminRegistrationCrudModule} from '../registration-admin-crud.module';
import {
 ...
 */
describe('Registration Crud List Tile Component', () => {

	let fixture;
	let registrationCrudListComponent: AdminRegistrationCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<RegistrationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '91096a56-b194-4f29-82ea-718263d4a45f';
	const searchContent = 'text';

	const searchableFieldCount = RegistrationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'registration-admin-crud',
		urls: ['registration-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const registrationDataFactory = new RegistrationDataFactory();

	const defaultDataSize = 30;
	const defaultData = registrationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getRegistrationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getRegistrationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminRegistrationCrudListComponent);
			registrationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(registrationCrudListComponent.registrationsCount).toBe(defaultData.length);
			...
			expect(registrationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			registrationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = registrationDataFactory.createAll(filteredDataSize);
			...
			expect(registrationCrudListComponent.registrationsCount).toBe(filteredDataSize);
			expect(registrationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(registrationCrudListComponent.pageIndex).toBe(0);
			expect(registrationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			registrationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		registrationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		registrationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
