...
import {OphthalmologyExaminationModelState, OphthalmologyExaminationModelAudit} from 'src/app/models/ophthalmologyExamination/ophthalmology_examination.model.state';
import {OphthalmologyExaminationModel} from 'src/app/models/ophthalmologyExamination/ophthalmology_examination.model';
import {
	getOphthalmologyExaminationModelAuditsByEntityId,
	getOphthalmologyExaminationModelWithId,
	getCountOphthalmologyExaminationModels,
...
@Component({
	selector: 'cb-ophthalmology-examination-admin-crud-edit',
	templateUrl: './ophthalmology-examination-admin-crud-edit.component.html',
	styleUrls: [
		'./ophthalmology-examination-admin-crud-edit.component.scss',
...
export class AdminOphthalmologyExaminationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	ophthalmologyExaminationAudits$: Observable<OphthalmologyExaminationModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'ophthalmologyExamination';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = OphthalmologyExaminationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: OphthalmologyExaminationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountOphthalmologyExaminationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new OphthalmologyExaminationModel();
		...
		if (this.targetModelId) {
			this.ophthalmologyExaminationAudits$ = this.store.select(getOphthalmologyExaminationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<OphthalmologyExaminationModel> = {
		...
		this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
			modelAction.OphthalmologyExaminationModelActionTypes.FETCH_OPHTHALMOLOGY_EXAMINATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getOphthalmologyExaminationModelWithId, this.targetModelId)
			...
			model = new OphthalmologyExaminationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
			modelAction.OphthalmologyExaminationModelActionTypes.FETCH_OPHTHALMOLOGY_EXAMINATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
				modelAction.OphthalmologyExaminationModelActionTypes.CREATE_OPHTHALMOLOGY_EXAMINATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
				modelAction.OphthalmologyExaminationModelActionTypes.UPDATE_OPHTHALMOLOGY_EXAMINATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: OphthalmologyExaminationModel = new OphthalmologyExaminationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getOphthalmologyExaminationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new OphthalmologyExaminationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = OphthalmologyExaminationModel.getProps();
		this.modelRelations = OphthalmologyExaminationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: OphthalmologyExaminationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): OphthalmologyExaminationModel {
		let updatedModel: OphthalmologyExaminationModel = new OphthalmologyExaminationModel(model);
