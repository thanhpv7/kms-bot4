...
import {AdminOphthalmologyExaminationCrudListComponent} from './ophthalmology-examination-admin-crud-list.component';
...
import {AdminOphthalmologyExaminationCrudModule} from '../ophthalmology-examination-admin-crud.module';
import {
	OphthalmologyExaminationModelState,
	initialState as initialOphthalmologyExaminationModelState
} from 'src/app/models/ophthalmologyExamination/ophthalmology_examination.model.state';
 ...
 */
describe('Ophthalmology Examination Crud List Tile Component', () => {

	let fixture;
	let ophthalmologyExaminationCrudListComponent: AdminOphthalmologyExaminationCrudListComponent;

	let store: MockStore<{ model: OphthalmologyExaminationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let ophthalmologyExaminationModelState: OphthalmologyExaminationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<OphthalmologyExaminationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd4732505-9b2a-417b-841e-ff5d779ce39c';
	const searchContent = 'text';

	const searchableFieldCount = OphthalmologyExaminationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'ophthalmology-examination-admin-crud',
		urls: ['ophthalmology-examination-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const ophthalmologyExaminationDataFactory = new OphthalmologyExaminationDataFactory();

	const defaultDataSize = 30;
	const defaultData = ophthalmologyExaminationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: OphthalmologyExaminationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		ophthalmologyExaminationModelState = initialOphthalmologyExaminationModelState;
		store.setState({model: ophthalmologyExaminationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getOphthalmologyExaminationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getOphthalmologyExaminationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminOphthalmologyExaminationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminOphthalmologyExaminationCrudListComponent);
			ophthalmologyExaminationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(ophthalmologyExaminationCrudListComponent.ophthalmologyExaminationsCount).toBe(defaultData.length);
			...
			expect(ophthalmologyExaminationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			ophthalmologyExaminationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = ophthalmologyExaminationDataFactory.createAll(filteredDataSize);
			...
			expect(ophthalmologyExaminationCrudListComponent.ophthalmologyExaminationsCount).toBe(filteredDataSize);
			expect(ophthalmologyExaminationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(ophthalmologyExaminationCrudListComponent.pageIndex).toBe(0);
			expect(ophthalmologyExaminationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			ophthalmologyExaminationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		ophthalmologyExaminationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		ophthalmologyExaminationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
