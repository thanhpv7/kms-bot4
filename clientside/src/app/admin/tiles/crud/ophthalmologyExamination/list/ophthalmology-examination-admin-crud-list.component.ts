...
import * as modelAction from 'src/app/models/ophthalmologyExamination/ophthalmology_examination.model.action';
import {OphthalmologyExaminationModelState} from 'src/app/models/ophthalmologyExamination/ophthalmology_examination.model.state';
...
import {
	getOphthalmologyExaminationCollectionModels,
	getOphthalmologyExaminationCollectionCount
...
import {CrudTileMode} from '../ophthalmology-examination-admin-crud.component';
...
@Component({
	selector: 'cb-ophthalmology-examination-admin-crud-list',
	templateUrl: './ophthalmology-examination-admin-crud-list.component.html',
	styleUrls: [
		'./ophthalmology-examination-admin-crud-list.component.scss',
...
export class AdminOphthalmologyExaminationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<OphthalmologyExaminationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...OphthalmologyExaminationModel.searchFields,
	...
	/**
	 * List of Ophthalmology Examinations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	ophthalmologyExaminations: Observable<OphthalmologyExaminationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	ophthalmologyExaminationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = OphthalmologyExaminationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = OphthalmologyExaminationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'd4732505-9b2a-417b-841e-ff5d779ce39c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.ophthalmologyExaminationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.ophthalmologyExaminationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: OphthalmologyExaminationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
				modelAction.OphthalmologyExaminationModelActionTypes.INITIALISE_OPHTHALMOLOGY_EXAMINATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
				modelAction.OphthalmologyExaminationModelActionTypes.FETCH_OPHTHALMOLOGY_EXAMINATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.OphthalmologyExaminationAction(
						modelAction.OphthalmologyExaminationModelActionTypes.COUNT_OPHTHALMOLOGY_EXAMINATIONS,
						null
					)
				]
			))
		...
		this.ophthalmologyExaminations = this.store.select(getOphthalmologyExaminationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getOphthalmologyExaminationCollectionCount, this.collectionId).subscribe(
				(count) => this.ophthalmologyExaminationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('ophthalmology-examination');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: OphthalmologyExaminationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.OphthalmologyExaminationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.OphthalmologyExaminationModelActionTypes.EXPORT_ALL_OPHTHALMOLOGY_EXAMINATION;
		} else {
			actionType = modelAction.OphthalmologyExaminationModelActionTypes.EXPORT_OPHTHALMOLOGY_EXAMINATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
				modelAction.OphthalmologyExaminationModelActionTypes.IMPORT_OPHTHALMOLOGY_EXAMINATIONS,
				{
					file: file,
				},
				[new modelAction.OphthalmologyExaminationAction(
			...
			window.open(environment.API_URL + '/api/ophthalmologyexamination/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: OphthalmologyExaminationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Ophthalmology Examination entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Ophthalmology Examination entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.OphthalmologyExaminationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.OphthalmologyExaminationModelActionTypes.DELETE_OPHTHALMOLOGY_EXAMINATION;
						} else if (usingSelected) {
							actionType = modelAction.OphthalmologyExaminationModelActionTypes.DELETE_ALL_OPHTHALMOLOGY_EXAMINATION;
						} else {
							actionType = modelAction.OphthalmologyExaminationModelActionTypes.DELETE_OPHTHALMOLOGY_EXAMINATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.OphthalmologyExaminationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_OPHTHALMOLOGY_EXAMINATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_OPHTHALMOLOGY_EXAMINATION and DELETE_OPHTHALMOLOGY_EXAMINATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<OphthalmologyExaminationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = OphthalmologyExaminationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.ophthalmologyExaminationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
