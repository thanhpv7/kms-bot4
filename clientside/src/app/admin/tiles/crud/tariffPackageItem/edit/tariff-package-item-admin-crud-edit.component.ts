...
import * as modelAction from 'src/app/models/tariffPackageItem/tariff_package_item.model.action';
import {TariffPackageItemModelState, TariffPackageItemModelAudit} from 'src/app/models/tariffPackageItem/tariff_package_item.model.state';
import {TariffPackageItemModel} from 'src/app/models/tariffPackageItem/tariff_package_item.model';
import {
	getTariffPackageItemModelAuditsByEntityId,
	getTariffPackageItemModelWithId,
	getCountTariffPackageItemModels,
...
import {CrudTileMode} from '../tariff-package-item-admin-crud.component';
...
@Component({
	selector: 'cb-tariff-package-item-admin-crud-edit',
	templateUrl: './tariff-package-item-admin-crud-edit.component.html',
	styleUrls: [
		'./tariff-package-item-admin-crud-edit.component.scss',
...
export class AdminTariffPackageItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'e59a40f8-22d5-4624-8e3a-4780000a6bba';
	 ...
	 */
	tariffPackageItemAudits$: Observable<TariffPackageItemModelAudit[]>;
	...
	tariffPackageOppositeRelation = 'tariffPackageItems';
	 ...
	 */
	modelProperties: ModelProperty[] = TariffPackageItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TariffPackageItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TariffPackageItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: TariffPackageItemModel;
	...
	constructor(
		private readonly store: Store<{ model: TariffPackageItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTariffPackageItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TariffPackageItemModel();
		...
		if (this.targetModelId) {
			this.tariffPackageItemAudits$ = this.store.select(getTariffPackageItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TariffPackageItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TariffPackageItemAction(
			modelAction.TariffPackageItemModelActionTypes.FETCH_TARIFF_PACKAGE_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTariffPackageItemModelWithId, this.targetModelId)
			...
			model = new TariffPackageItemModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('tariff-package-item');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TariffPackageItemAction(
			modelAction.TariffPackageItemModelActionTypes.FETCH_TARIFF_PACKAGE_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TariffPackageItemModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TariffPackageItemAction(
						modelAction.TariffPackageItemModelActionTypes.COUNT_TARIFF_PACKAGE_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TariffPackageItemAction(
				modelAction.TariffPackageItemModelActionTypes.CREATE_TARIFF_PACKAGE_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TariffPackageItemAction(
				modelAction.TariffPackageItemModelActionTypes.UPDATE_TARIFF_PACKAGE_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TariffPackageItemModel = new TariffPackageItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTariffPackageItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TariffPackageItemModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TariffPackageItemModel.getProps();
		this.modelRelations = TariffPackageItemModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: TariffPackageItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TariffPackageItemModel {
		let updatedModel: TariffPackageItemModel = new TariffPackageItemModel(model);
