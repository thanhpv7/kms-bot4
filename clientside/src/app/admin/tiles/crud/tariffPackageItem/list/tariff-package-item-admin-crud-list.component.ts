...
import {TariffPackageItemModelState} from 'src/app/models/tariffPackageItem/tariff_package_item.model.state';
...
import {
	getTariffPackageItemCollectionModels,
	getTariffPackageItemCollectionCount
...
@Component({
	selector: 'cb-tariff-package-item-admin-crud-list',
	templateUrl: './tariff-package-item-admin-crud-list.component.html',
	styleUrls: [
		'./tariff-package-item-admin-crud-list.component.scss',
...
export class AdminTariffPackageItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TariffPackageItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TariffPackageItemModel.searchFields,
	...
	/**
	 * List of Tariff Package Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	tariffPackageItems: Observable<TariffPackageItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	tariffPackageItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.tariffPackageItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.tariffPackageItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffPackageItemAction(
				modelAction.TariffPackageItemModelActionTypes.INITIALISE_TARIFF_PACKAGE_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffPackageItemAction(
				modelAction.TariffPackageItemModelActionTypes.FETCH_TARIFF_PACKAGE_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.tariffPackageItems = this.store.select(getTariffPackageItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTariffPackageItemCollectionCount, this.collectionId).subscribe(
				(count) => this.tariffPackageItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TariffPackageItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TariffPackageItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TariffPackageItemModelActionTypes.EXPORT_ALL_TARIFF_PACKAGE_ITEM;
		} else {
			actionType = modelAction.TariffPackageItemModelActionTypes.EXPORT_TARIFF_PACKAGE_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TariffPackageItemAction(
				modelAction.TariffPackageItemModelActionTypes.IMPORT_TARIFF_PACKAGE_ITEMS,
				{
					file: file,
				},
				[new modelAction.TariffPackageItemAction(
			...
			window.open(environment.API_URL + '/api/tariffpackageitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TariffPackageItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Tariff Package Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Tariff Package Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TariffPackageItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TariffPackageItemModelActionTypes.DELETE_TARIFF_PACKAGE_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.TariffPackageItemModelActionTypes.DELETE_ALL_TARIFF_PACKAGE_ITEM;
						} else {
							actionType = modelAction.TariffPackageItemModelActionTypes.DELETE_TARIFF_PACKAGE_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TariffPackageItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TARIFF_PACKAGE_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TARIFF_PACKAGE_ITEM and DELETE_TARIFF_PACKAGE_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TariffPackageItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.tariffPackageItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
