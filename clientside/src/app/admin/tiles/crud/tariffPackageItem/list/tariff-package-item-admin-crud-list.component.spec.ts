...
import {AdminTariffPackageItemCrudListComponent} from './tariff-package-item-admin-crud-list.component';
...
import {AdminTariffPackageItemCrudModule} from '../tariff-package-item-admin-crud.module';
import {
	TariffPackageItemModelState,
	initialState as initialTariffPackageItemModelState
} from 'src/app/models/tariffPackageItem/tariff_package_item.model.state';
 ...
 */
describe('Tariff Package Item Crud List Tile Component', () => {

	let fixture;
	let tariffPackageItemCrudListComponent: AdminTariffPackageItemCrudListComponent;

	let store: MockStore<{ model: TariffPackageItemModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let tariffPackageItemModelState: TariffPackageItemModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<TariffPackageItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e59a40f8-22d5-4624-8e3a-4780000a6bba';
	const searchContent = 'text';

	const searchableFieldCount = TariffPackageItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'tariff-package-item-admin-crud',
		urls: ['tariff-package-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const tariffPackageItemDataFactory = new TariffPackageItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = tariffPackageItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: TariffPackageItemModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		tariffPackageItemModelState = initialTariffPackageItemModelState;
		store.setState({model: tariffPackageItemModelState});
		...
		spyOn(store, 'select')
			.withArgs(getTariffPackageItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTariffPackageItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminTariffPackageItemCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminTariffPackageItemCrudListComponent);
			tariffPackageItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(tariffPackageItemCrudListComponent.tariffPackageItemsCount).toBe(defaultData.length);
			...
			expect(tariffPackageItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			tariffPackageItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = tariffPackageItemDataFactory.createAll(filteredDataSize);
			...
			expect(tariffPackageItemCrudListComponent.tariffPackageItemsCount).toBe(filteredDataSize);
			expect(tariffPackageItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(tariffPackageItemCrudListComponent.pageIndex).toBe(0);
			expect(tariffPackageItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			tariffPackageItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		tariffPackageItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		tariffPackageItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
