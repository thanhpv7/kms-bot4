...
import {AdminSupplierContactDetailCrudListComponent} from './supplier-contact-detail-admin-crud-list.component';
...
import {AdminSupplierContactDetailCrudModule} from '../supplier-contact-detail-admin-crud.module';
import {
	SupplierContactDetailModelState,
	initialState as initialSupplierContactDetailModelState
} from 'src/app/models/supplierContactDetail/supplier_contact_detail.model.state';
 ...
 */
describe('Supplier Contact Detail Crud List Tile Component', () => {

	let fixture;
	let supplierContactDetailCrudListComponent: AdminSupplierContactDetailCrudListComponent;

	let store: MockStore<{ model: SupplierContactDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let supplierContactDetailModelState: SupplierContactDetailModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<SupplierContactDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f25ee88c-3f14-4c83-96cd-b40681841ed7';
	const searchContent = 'text';

	const searchableFieldCount = SupplierContactDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'supplier-contact-detail-admin-crud',
		urls: ['supplier-contact-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const supplierContactDetailDataFactory = new SupplierContactDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = supplierContactDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: SupplierContactDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		supplierContactDetailModelState = initialSupplierContactDetailModelState;
		store.setState({model: supplierContactDetailModelState});
		...
		spyOn(store, 'select')
			.withArgs(getSupplierContactDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getSupplierContactDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminSupplierContactDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminSupplierContactDetailCrudListComponent);
			supplierContactDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(supplierContactDetailCrudListComponent.supplierContactDetailsCount).toBe(defaultData.length);
			...
			expect(supplierContactDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			supplierContactDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = supplierContactDetailDataFactory.createAll(filteredDataSize);
			...
			expect(supplierContactDetailCrudListComponent.supplierContactDetailsCount).toBe(filteredDataSize);
			expect(supplierContactDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(supplierContactDetailCrudListComponent.pageIndex).toBe(0);
			expect(supplierContactDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			supplierContactDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		supplierContactDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		supplierContactDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
