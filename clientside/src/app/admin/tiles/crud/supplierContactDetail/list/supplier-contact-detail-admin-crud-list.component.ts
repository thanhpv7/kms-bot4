...
import * as modelAction from 'src/app/models/supplierContactDetail/supplier_contact_detail.model.action';
import {SupplierContactDetailModelState} from 'src/app/models/supplierContactDetail/supplier_contact_detail.model.state';
...
import {
	getSupplierContactDetailCollectionModels,
	getSupplierContactDetailCollectionCount
...
import {CrudTileMode} from '../supplier-contact-detail-admin-crud.component';
...
@Component({
	selector: 'cb-supplier-contact-detail-admin-crud-list',
	templateUrl: './supplier-contact-detail-admin-crud-list.component.html',
	styleUrls: [
		'./supplier-contact-detail-admin-crud-list.component.scss',
...
export class AdminSupplierContactDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SupplierContactDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SupplierContactDetailModel.searchFields,
	...
	/**
	 * List of Supplier Contact Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	supplierContactDetails: Observable<SupplierContactDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	supplierContactDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = SupplierContactDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = SupplierContactDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'f25ee88c-3f14-4c83-96cd-b40681841ed7';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.supplierContactDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.supplierContactDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: SupplierContactDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierContactDetailAction(
				modelAction.SupplierContactDetailModelActionTypes.INITIALISE_SUPPLIER_CONTACT_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierContactDetailAction(
				modelAction.SupplierContactDetailModelActionTypes.FETCH_SUPPLIER_CONTACT_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.SupplierContactDetailAction(
						modelAction.SupplierContactDetailModelActionTypes.COUNT_SUPPLIER_CONTACT_DETAILS,
						null
					)
				]
			))
		...
		this.supplierContactDetails = this.store.select(getSupplierContactDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSupplierContactDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.supplierContactDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('supplier-contact-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SupplierContactDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SupplierContactDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SupplierContactDetailModelActionTypes.EXPORT_ALL_SUPPLIER_CONTACT_DETAIL;
		} else {
			actionType = modelAction.SupplierContactDetailModelActionTypes.EXPORT_SUPPLIER_CONTACT_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SupplierContactDetailAction(
				modelAction.SupplierContactDetailModelActionTypes.IMPORT_SUPPLIER_CONTACT_DETAILS,
				{
					file: file,
				},
				[new modelAction.SupplierContactDetailAction(
			...
			window.open(environment.API_URL + '/api/suppliercontactdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SupplierContactDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Supplier Contact Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Supplier Contact Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SupplierContactDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SupplierContactDetailModelActionTypes.DELETE_SUPPLIER_CONTACT_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.SupplierContactDetailModelActionTypes.DELETE_ALL_SUPPLIER_CONTACT_DETAIL;
						} else {
							actionType = modelAction.SupplierContactDetailModelActionTypes.DELETE_SUPPLIER_CONTACT_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SupplierContactDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SUPPLIER_CONTACT_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SUPPLIER_CONTACT_DETAIL and DELETE_SUPPLIER_CONTACT_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<SupplierContactDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SupplierContactDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.supplierContactDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
