...
import {SupplierContactDetailModelState, SupplierContactDetailModelAudit} from 'src/app/models/supplierContactDetail/supplier_contact_detail.model.state';
import {SupplierContactDetailModel} from 'src/app/models/supplierContactDetail/supplier_contact_detail.model';
import {
	getSupplierContactDetailModelAuditsByEntityId,
	getSupplierContactDetailModelWithId,
	getCountSupplierContactDetailModels,
...
@Component({
	selector: 'cb-supplier-contact-detail-admin-crud-edit',
	templateUrl: './supplier-contact-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./supplier-contact-detail-admin-crud-edit.component.scss',
...
export class AdminSupplierContactDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	supplierContactDetailAudits$: Observable<SupplierContactDetailModelAudit[]>;
	...
	supplierOppositeRelation = 'suplierContactDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SupplierContactDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: SupplierContactDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSupplierContactDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new SupplierContactDetailModel();
		...
		if (this.targetModelId) {
			this.supplierContactDetailAudits$ = this.store.select(getSupplierContactDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SupplierContactDetailModel> = {
		...
		this.store.dispatch(new modelAction.SupplierContactDetailAction(
			modelAction.SupplierContactDetailModelActionTypes.FETCH_SUPPLIER_CONTACT_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSupplierContactDetailModelWithId, this.targetModelId)
			...
			model = new SupplierContactDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SupplierContactDetailAction(
			modelAction.SupplierContactDetailModelActionTypes.FETCH_SUPPLIER_CONTACT_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<SupplierContactDetailModel> = {
				...
				queryParams: {
					persistRelations: this.supplierMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.SupplierContactDetailAction(
				modelAction.SupplierContactDetailModelActionTypes.CREATE_SUPPLIER_CONTACT_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SupplierContactDetailAction(
				modelAction.SupplierContactDetailModelActionTypes.UPDATE_SUPPLIER_CONTACT_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SupplierContactDetailModel = new SupplierContactDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSupplierContactDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SupplierContactDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SupplierContactDetailModel.getProps();
		this.modelRelations = SupplierContactDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: SupplierContactDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SupplierContactDetailModel {
		let updatedModel: SupplierContactDetailModel = new SupplierContactDetailModel(model);
