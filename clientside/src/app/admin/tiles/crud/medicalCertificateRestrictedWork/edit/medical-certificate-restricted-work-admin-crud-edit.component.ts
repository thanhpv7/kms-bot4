...
import * as modelAction from 'src/app/models/medicalCertificateRestrictedWork/medical_certificate_restricted_work.model.action';
import {MedicalCertificateRestrictedWorkModelState, MedicalCertificateRestrictedWorkModelAudit} from 'src/app/models/medicalCertificateRestrictedWork/medical_certificate_restricted_work.model.state';
...
import {
	getMedicalCertificateRestrictedWorkModelAuditsByEntityId,
	getMedicalCertificateRestrictedWorkModelWithId,
	getCountMedicalCertificateRestrictedWorkModels,
...
import {CrudTileMode} from '../medical-certificate-restricted-work-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-restricted-work-admin-crud-edit',
	templateUrl: './medical-certificate-restricted-work-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-restricted-work-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificateRestrictedWorkCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '55a280ab-bfcb-4610-902e-085128737fa6';
	 ...
	 */
	medicalCertificateRestrictedWorkAudits$: Observable<MedicalCertificateRestrictedWorkModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificateRestrictedWork';
	...
	medicalStaffOppositeRelation = 'medicalStaffOfRestrictedWorksCertificate';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificateRestrictedWorkModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificateRestrictedWorkModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificateRestrictedWorkModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalCertificateRestrictedWorkModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificateRestrictedWorkModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificateRestrictedWorkModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificateRestrictedWorkModel();
		...
		if (this.targetModelId) {
			this.medicalCertificateRestrictedWorkAudits$ = this.store.select(getMedicalCertificateRestrictedWorkModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificateRestrictedWorkModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
			modelAction.MedicalCertificateRestrictedWorkModelActionTypes.FETCH_MEDICAL_CERTIFICATE_RESTRICTED_WORK,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateRestrictedWorkModelWithId, this.targetModelId)
			...
			model = new MedicalCertificateRestrictedWorkModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-restricted-work');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
			modelAction.MedicalCertificateRestrictedWorkModelActionTypes.FETCH_MEDICAL_CERTIFICATE_RESTRICTED_WORK_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificateRestrictedWorkModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificateRestrictedWorkAction(
						modelAction.MedicalCertificateRestrictedWorkModelActionTypes.COUNT_MEDICAL_CERTIFICATE_RESTRICTED_WORKS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
				modelAction.MedicalCertificateRestrictedWorkModelActionTypes.CREATE_MEDICAL_CERTIFICATE_RESTRICTED_WORK,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
				modelAction.MedicalCertificateRestrictedWorkModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_RESTRICTED_WORK,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificateRestrictedWorkModel = new MedicalCertificateRestrictedWorkModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateRestrictedWorkModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificateRestrictedWorkModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificateRestrictedWorkModel.getProps();
		this.modelRelations = MedicalCertificateRestrictedWorkModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificateRestrictedWorkModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificateRestrictedWorkModel {
		let updatedModel: MedicalCertificateRestrictedWorkModel = new MedicalCertificateRestrictedWorkModel(model);
