...
import {MedicalCertificateRestrictedWorkModelState} from 'src/app/models/medicalCertificateRestrictedWork/medical_certificate_restricted_work.model.state';
...
import {
	getMedicalCertificateRestrictedWorkCollectionModels,
	getMedicalCertificateRestrictedWorkCollectionCount
...
@Component({
	selector: 'cb-medical-certificate-restricted-work-admin-crud-list',
	templateUrl: './medical-certificate-restricted-work-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-restricted-work-admin-crud-list.component.scss',
...
export class AdminMedicalCertificateRestrictedWorkCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificateRestrictedWorkModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificateRestrictedWorkModel.searchFields,
	...
	/**
	 * List of Medical Certificate Restricted Works that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificateRestrictedWorks: Observable<MedicalCertificateRestrictedWorkModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificateRestrictedWorksCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificateRestrictedWorksCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificateRestrictedWorksCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
				modelAction.MedicalCertificateRestrictedWorkModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_RESTRICTED_WORK_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
				modelAction.MedicalCertificateRestrictedWorkModelActionTypes.FETCH_MEDICAL_CERTIFICATE_RESTRICTED_WORK_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificateRestrictedWorks = this.store.select(getMedicalCertificateRestrictedWorkCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificateRestrictedWorkCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificateRestrictedWorksCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificateRestrictedWorkModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificateRestrictedWorkModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificateRestrictedWorkModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_RESTRICTED_WORK;
		} else {
			actionType = modelAction.MedicalCertificateRestrictedWorkModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_RESTRICTED_WORK_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
				modelAction.MedicalCertificateRestrictedWorkModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_RESTRICTED_WORKS,
				{
					file: file,
				},
				[new modelAction.MedicalCertificateRestrictedWorkAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificaterestrictedwork/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificateRestrictedWorkModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Restricted Work entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Restricted Work entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificateRestrictedWorkModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificateRestrictedWorkModelActionTypes.DELETE_MEDICAL_CERTIFICATE_RESTRICTED_WORK;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificateRestrictedWorkModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_RESTRICTED_WORK;
						} else {
							actionType = modelAction.MedicalCertificateRestrictedWorkModelActionTypes.DELETE_MEDICAL_CERTIFICATE_RESTRICTED_WORK_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificateRestrictedWorkAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_RESTRICTED_WORK when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_RESTRICTED_WORK and DELETE_MEDICAL_CERTIFICATE_RESTRICTED_WORK_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificateRestrictedWorkModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificateRestrictedWorksCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
