...
import {AdminMedicalCertificateRestrictedWorkCrudListComponent} from './medical-certificate-restricted-work-admin-crud-list.component';
...
import {AdminMedicalCertificateRestrictedWorkCrudModule} from '../medical-certificate-restricted-work-admin-crud.module';
import {
	MedicalCertificateRestrictedWorkModelState,
	initialState as initialMedicalCertificateRestrictedWorkModelState
} from 'src/app/models/medicalCertificateRestrictedWork/medical_certificate_restricted_work.model.state';
 ...
 */
describe('Medical Certificate Restricted Work Crud List Tile Component', () => {

	let fixture;
	let medicalCertificateRestrictedWorkCrudListComponent: AdminMedicalCertificateRestrictedWorkCrudListComponent;

	let store: MockStore<{ model: MedicalCertificateRestrictedWorkModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalCertificateRestrictedWorkModelState: MedicalCertificateRestrictedWorkModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalCertificateRestrictedWorkModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '55a280ab-bfcb-4610-902e-085128737fa6';
	const searchContent = 'text';

	const searchableFieldCount = MedicalCertificateRestrictedWorkModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-certificate-restricted-work-admin-crud',
		urls: ['medical-certificate-restricted-work-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalCertificateRestrictedWorkDataFactory = new MedicalCertificateRestrictedWorkDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalCertificateRestrictedWorkDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalCertificateRestrictedWorkModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalCertificateRestrictedWorkModelState = initialMedicalCertificateRestrictedWorkModelState;
		store.setState({model: medicalCertificateRestrictedWorkModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalCertificateRestrictedWorkCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalCertificateRestrictedWorkCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalCertificateRestrictedWorkCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalCertificateRestrictedWorkCrudListComponent);
			medicalCertificateRestrictedWorkCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalCertificateRestrictedWorkCrudListComponent.medicalCertificateRestrictedWorksCount).toBe(defaultData.length);
			...
			expect(medicalCertificateRestrictedWorkCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalCertificateRestrictedWorkCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalCertificateRestrictedWorkDataFactory.createAll(filteredDataSize);
			...
			expect(medicalCertificateRestrictedWorkCrudListComponent.medicalCertificateRestrictedWorksCount).toBe(filteredDataSize);
			expect(medicalCertificateRestrictedWorkCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalCertificateRestrictedWorkCrudListComponent.pageIndex).toBe(0);
			expect(medicalCertificateRestrictedWorkCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalCertificateRestrictedWorkCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalCertificateRestrictedWorkCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalCertificateRestrictedWorkCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
