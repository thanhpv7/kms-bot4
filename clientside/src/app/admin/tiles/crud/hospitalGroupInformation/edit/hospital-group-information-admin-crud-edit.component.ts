...
import {HospitalGroupInformationModelState, HospitalGroupInformationModelAudit} from 'src/app/models/hospitalGroupInformation/hospital_group_information.model.state';
import {HospitalGroupInformationModel} from 'src/app/models/hospitalGroupInformation/hospital_group_information.model';
import {
	getHospitalGroupInformationModelAuditsByEntityId,
...
@Component({
	selector: 'cb-hospital-group-information-admin-crud-edit',
	templateUrl: './hospital-group-information-admin-crud-edit.component.html',
	styleUrls: [
		'./hospital-group-information-admin-crud-edit.component.scss',
...
export class AdminHospitalGroupInformationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	hospitalGroupInformationAudits$: Observable<HospitalGroupInformationModelAudit[]>;
	...
	hospitalBranchInformationsOppositeRelation = 'hospitalGroup';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = HospitalGroupInformationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: HospitalGroupInformationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountHospitalGroupInformationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new HospitalGroupInformationModel();
		...
		if (this.targetModelId) {
			this.hospitalGroupInformationAudits$ = this.store.select(getHospitalGroupInformationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<HospitalGroupInformationModel> = {
		...
		this.store.dispatch(new modelAction.HospitalGroupInformationAction(
			modelAction.HospitalGroupInformationModelActionTypes.FETCH_HOSPITAL_GROUP_INFORMATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getHospitalGroupInformationModelWithId, this.targetModelId)
			...
			model = new HospitalGroupInformationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.HospitalGroupInformationAction(
			modelAction.HospitalGroupInformationModelActionTypes.FETCH_HOSPITAL_GROUP_INFORMATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<HospitalGroupInformationModel> = {
				...
				queryParams: {
					persistRelations: this.hospitalBranchInformationsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.HospitalGroupInformationAction(
				modelAction.HospitalGroupInformationModelActionTypes.CREATE_HOSPITAL_GROUP_INFORMATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.HospitalGroupInformationAction(
				modelAction.HospitalGroupInformationModelActionTypes.UPDATE_HOSPITAL_GROUP_INFORMATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: HospitalGroupInformationModel = new HospitalGroupInformationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getHospitalGroupInformationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new HospitalGroupInformationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = HospitalGroupInformationModel.getProps();
		this.modelRelations = HospitalGroupInformationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: HospitalGroupInformationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): HospitalGroupInformationModel {
		let updatedModel: HospitalGroupInformationModel = new HospitalGroupInformationModel(model);
