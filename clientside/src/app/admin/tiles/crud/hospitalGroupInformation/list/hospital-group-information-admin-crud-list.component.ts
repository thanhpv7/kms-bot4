...
import * as modelAction from 'src/app/models/hospitalGroupInformation/hospital_group_information.model.action';
import {HospitalGroupInformationModelState} from 'src/app/models/hospitalGroupInformation/hospital_group_information.model.state';
...
import {
	...
	getHospitalGroupInformationCollectionCount
...
import {CrudTileMode} from '../hospital-group-information-admin-crud.component';
...
@Component({
	selector: 'cb-hospital-group-information-admin-crud-list',
	templateUrl: './hospital-group-information-admin-crud-list.component.html',
	styleUrls: [
		'./hospital-group-information-admin-crud-list.component.scss',
...
export class AdminHospitalGroupInformationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<HospitalGroupInformationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...HospitalGroupInformationModel.searchFields,
	...
	/**
	 * List of Hospital Group Informations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	hospitalGroupInformations: Observable<HospitalGroupInformationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	hospitalGroupInformationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = HospitalGroupInformationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = HospitalGroupInformationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '61dc0409-647b-4d67-ac78-6e047c86ee02';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.hospitalGroupInformationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.hospitalGroupInformationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: HospitalGroupInformationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HospitalGroupInformationAction(
				modelAction.HospitalGroupInformationModelActionTypes.INITIALISE_HOSPITAL_GROUP_INFORMATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HospitalGroupInformationAction(
				modelAction.HospitalGroupInformationModelActionTypes.FETCH_HOSPITAL_GROUP_INFORMATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.HospitalGroupInformationAction(
						modelAction.HospitalGroupInformationModelActionTypes.COUNT_HOSPITAL_GROUP_INFORMATIONS,
						null
					)
				]
			))
		...
		this.hospitalGroupInformations = this.store.select(getHospitalGroupInformationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getHospitalGroupInformationCollectionCount, this.collectionId).subscribe(
				(count) => this.hospitalGroupInformationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('hospital-group-information');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: HospitalGroupInformationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.HospitalGroupInformationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.HospitalGroupInformationModelActionTypes.EXPORT_ALL_HOSPITAL_GROUP_INFORMATION;
		} else {
			actionType = modelAction.HospitalGroupInformationModelActionTypes.EXPORT_HOSPITAL_GROUP_INFORMATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.HospitalGroupInformationAction(
				modelAction.HospitalGroupInformationModelActionTypes.IMPORT_HOSPITAL_GROUP_INFORMATIONS,
				{
					file: file,
				},
				[new modelAction.HospitalGroupInformationAction(
			...
			window.open(environment.API_URL + '/api/hospitalgroupinformation/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: HospitalGroupInformationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Hospital Group Information entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Hospital Group Information entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.HospitalGroupInformationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.HospitalGroupInformationModelActionTypes.DELETE_HOSPITAL_GROUP_INFORMATION;
						} else if (usingSelected) {
							actionType = modelAction.HospitalGroupInformationModelActionTypes.DELETE_ALL_HOSPITAL_GROUP_INFORMATION;
						} else {
							actionType = modelAction.HospitalGroupInformationModelActionTypes.DELETE_HOSPITAL_GROUP_INFORMATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.HospitalGroupInformationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_HOSPITAL_GROUP_INFORMATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_HOSPITAL_GROUP_INFORMATION and DELETE_HOSPITAL_GROUP_INFORMATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<HospitalGroupInformationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = HospitalGroupInformationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.hospitalGroupInformationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
