...
import {AdminHospitalGroupInformationCrudListComponent} from './hospital-group-information-admin-crud-list.component';
...
import {AdminHospitalGroupInformationCrudModule} from '../hospital-group-information-admin-crud.module';
import {
 ...
 */
describe('Hospital Group Information Crud List Tile Component', () => {

	let fixture;
	let hospitalGroupInformationCrudListComponent: AdminHospitalGroupInformationCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<HospitalGroupInformationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '61dc0409-647b-4d67-ac78-6e047c86ee02';
	const searchContent = 'text';

	const searchableFieldCount = HospitalGroupInformationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'hospital-group-information-admin-crud',
		urls: ['hospital-group-information-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const hospitalGroupInformationDataFactory = new HospitalGroupInformationDataFactory();

	const defaultDataSize = 30;
	const defaultData = hospitalGroupInformationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getHospitalGroupInformationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getHospitalGroupInformationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminHospitalGroupInformationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminHospitalGroupInformationCrudListComponent);
			hospitalGroupInformationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(hospitalGroupInformationCrudListComponent.hospitalGroupInformationsCount).toBe(defaultData.length);
			...
			expect(hospitalGroupInformationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			hospitalGroupInformationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = hospitalGroupInformationDataFactory.createAll(filteredDataSize);
			...
			expect(hospitalGroupInformationCrudListComponent.hospitalGroupInformationsCount).toBe(filteredDataSize);
			expect(hospitalGroupInformationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(hospitalGroupInformationCrudListComponent.pageIndex).toBe(0);
			expect(hospitalGroupInformationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			hospitalGroupInformationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		hospitalGroupInformationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		hospitalGroupInformationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
