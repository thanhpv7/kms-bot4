...
import {AdminDiagnosisExaminationRecordCrudListComponent} from './diagnosis-examination-record-admin-crud-list.component';
...
import {AdminDiagnosisExaminationRecordCrudModule} from '../diagnosis-examination-record-admin-crud.module';
import {
	DiagnosisExaminationRecordModelState,
	initialState as initialDiagnosisExaminationRecordModelState
} from 'src/app/models/diagnosisExaminationRecord/diagnosis_examination_record.model.state';
 ...
 */
describe('Diagnosis Examination Record Crud List Tile Component', () => {

	let fixture;
	let diagnosisExaminationRecordCrudListComponent: AdminDiagnosisExaminationRecordCrudListComponent;

	let store: MockStore<{ model: DiagnosisExaminationRecordModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let diagnosisExaminationRecordModelState: DiagnosisExaminationRecordModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<DiagnosisExaminationRecordModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '2084c96e-037c-466a-aa91-77607d2d9393';
	const searchContent = 'text';

	const searchableFieldCount = DiagnosisExaminationRecordModel.searchFields.length;

	const routerState: RouterState = {
		url: 'diagnosis-examination-record-admin-crud',
		urls: ['diagnosis-examination-record-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const diagnosisExaminationRecordDataFactory = new DiagnosisExaminationRecordDataFactory();

	const defaultDataSize = 30;
	const defaultData = diagnosisExaminationRecordDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: DiagnosisExaminationRecordModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		diagnosisExaminationRecordModelState = initialDiagnosisExaminationRecordModelState;
		store.setState({model: diagnosisExaminationRecordModelState});
		...
		spyOn(store, 'select')
			.withArgs(getDiagnosisExaminationRecordCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDiagnosisExaminationRecordCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminDiagnosisExaminationRecordCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminDiagnosisExaminationRecordCrudListComponent);
			diagnosisExaminationRecordCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(diagnosisExaminationRecordCrudListComponent.diagnosisExaminationRecordsCount).toBe(defaultData.length);
			...
			expect(diagnosisExaminationRecordCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			diagnosisExaminationRecordCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = diagnosisExaminationRecordDataFactory.createAll(filteredDataSize);
			...
			expect(diagnosisExaminationRecordCrudListComponent.diagnosisExaminationRecordsCount).toBe(filteredDataSize);
			expect(diagnosisExaminationRecordCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(diagnosisExaminationRecordCrudListComponent.pageIndex).toBe(0);
			expect(diagnosisExaminationRecordCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			diagnosisExaminationRecordCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		diagnosisExaminationRecordCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		diagnosisExaminationRecordCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
