...
import {DiagnosisExaminationRecordModelState} from 'src/app/models/diagnosisExaminationRecord/diagnosis_examination_record.model.state';
...
import {
	getDiagnosisExaminationRecordCollectionModels,
	getDiagnosisExaminationRecordCollectionCount
...
@Component({
	selector: 'cb-diagnosis-examination-record-admin-crud-list',
	templateUrl: './diagnosis-examination-record-admin-crud-list.component.html',
	styleUrls: [
		'./diagnosis-examination-record-admin-crud-list.component.scss',
...
export class AdminDiagnosisExaminationRecordCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DiagnosisExaminationRecordModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DiagnosisExaminationRecordModel.searchFields,
	...
	/**
	 * List of Diagnosis Examination Records that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	diagnosisExaminationRecords: Observable<DiagnosisExaminationRecordModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	diagnosisExaminationRecordsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.diagnosisExaminationRecordsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.diagnosisExaminationRecordsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosisExaminationRecordAction(
				modelAction.DiagnosisExaminationRecordModelActionTypes.INITIALISE_DIAGNOSIS_EXAMINATION_RECORD_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosisExaminationRecordAction(
				modelAction.DiagnosisExaminationRecordModelActionTypes.FETCH_DIAGNOSIS_EXAMINATION_RECORD_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.diagnosisExaminationRecords = this.store.select(getDiagnosisExaminationRecordCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDiagnosisExaminationRecordCollectionCount, this.collectionId).subscribe(
				(count) => this.diagnosisExaminationRecordsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DiagnosisExaminationRecordModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DiagnosisExaminationRecordModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DiagnosisExaminationRecordModelActionTypes.EXPORT_ALL_DIAGNOSIS_EXAMINATION_RECORD;
		} else {
			actionType = modelAction.DiagnosisExaminationRecordModelActionTypes.EXPORT_DIAGNOSIS_EXAMINATION_RECORD_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DiagnosisExaminationRecordAction(
				modelAction.DiagnosisExaminationRecordModelActionTypes.IMPORT_DIAGNOSIS_EXAMINATION_RECORDS,
				{
					file: file,
				},
				[new modelAction.DiagnosisExaminationRecordAction(
			...
			window.open(environment.API_URL + '/api/diagnosisexaminationrecord/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DiagnosisExaminationRecordModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Diagnosis Examination Record entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Diagnosis Examination Record entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DiagnosisExaminationRecordModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DiagnosisExaminationRecordModelActionTypes.DELETE_DIAGNOSIS_EXAMINATION_RECORD;
						} else if (usingSelected) {
							actionType = modelAction.DiagnosisExaminationRecordModelActionTypes.DELETE_ALL_DIAGNOSIS_EXAMINATION_RECORD;
						} else {
							actionType = modelAction.DiagnosisExaminationRecordModelActionTypes.DELETE_DIAGNOSIS_EXAMINATION_RECORD_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DiagnosisExaminationRecordAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DIAGNOSIS_EXAMINATION_RECORD when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DIAGNOSIS_EXAMINATION_RECORD and DELETE_DIAGNOSIS_EXAMINATION_RECORD_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DiagnosisExaminationRecordModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.diagnosisExaminationRecordsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
