...
import * as modelAction from 'src/app/models/patientVisitPatientPaymentOthers/patient_visit_patient_payment_others.model.action';
import {PatientVisitPatientPaymentOthersModelState} from 'src/app/models/patientVisitPatientPaymentOthers/patient_visit_patient_payment_others.model.state';
...
import {
	getPatientVisitPatientPaymentOthersCollectionModels,
	getPatientVisitPatientPaymentOthersCollectionCount
...
import {CrudTileMode} from '../patient-visit-patient-payment-others-admin-crud.component';
...
@Component({
	selector: 'cb-patient-visit-patient-payment-others-admin-crud-list',
	templateUrl: './patient-visit-patient-payment-others-admin-crud-list.component.html',
	styleUrls: [
		'./patient-visit-patient-payment-others-admin-crud-list.component.scss',
...
export class AdminPatientVisitPatientPaymentOthersCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientVisitPatientPaymentOthersModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientVisitPatientPaymentOthersModel.searchFields,
	...
	/**
	 * List of Patient Visit Patient Payment Otherss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientVisitPatientPaymentOtherss: Observable<PatientVisitPatientPaymentOthersModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientVisitPatientPaymentOtherssCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PatientVisitPatientPaymentOthersModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientVisitPatientPaymentOthersModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ac7183aa-1605-44c0-9c95-9002167d699d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientVisitPatientPaymentOtherssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientVisitPatientPaymentOtherssCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PatientVisitPatientPaymentOthersModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
				modelAction.PatientVisitPatientPaymentOthersModelActionTypes.INITIALISE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
				modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PatientVisitPatientPaymentOthersAction(
						modelAction.PatientVisitPatientPaymentOthersModelActionTypes.COUNT_PATIENT_VISIT_PATIENT_PAYMENT_OTHERSS,
						null
					)
				]
			))
		...
		this.patientVisitPatientPaymentOtherss = this.store.select(getPatientVisitPatientPaymentOthersCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientVisitPatientPaymentOthersCollectionCount, this.collectionId).subscribe(
				(count) => this.patientVisitPatientPaymentOtherssCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-visit-patient-payment-others');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientVisitPatientPaymentOthersModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientVisitPatientPaymentOthersModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientVisitPatientPaymentOthersModelActionTypes.EXPORT_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS;
		} else {
			actionType = modelAction.PatientVisitPatientPaymentOthersModelActionTypes.EXPORT_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
				modelAction.PatientVisitPatientPaymentOthersModelActionTypes.IMPORT_PATIENT_VISIT_PATIENT_PAYMENT_OTHERSS,
				{
					file: file,
				},
				[new modelAction.PatientVisitPatientPaymentOthersAction(
			...
			window.open(environment.API_URL + '/api/patientvisitpatientpaymentothers/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientVisitPatientPaymentOthersModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit Patient Payment Others entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit Patient Payment Others entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientVisitPatientPaymentOthersModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientVisitPatientPaymentOthersModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS;
						} else if (usingSelected) {
							actionType = modelAction.PatientVisitPatientPaymentOthersModelActionTypes.DELETE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS;
						} else {
							actionType = modelAction.PatientVisitPatientPaymentOthersModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS and DELETE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PatientVisitPatientPaymentOthersModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientVisitPatientPaymentOthersModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientVisitPatientPaymentOtherssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
