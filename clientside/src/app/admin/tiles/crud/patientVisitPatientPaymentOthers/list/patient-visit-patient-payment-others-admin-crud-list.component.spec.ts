...
import {AdminPatientVisitPatientPaymentOthersCrudListComponent} from './patient-visit-patient-payment-others-admin-crud-list.component';
...
import {AdminPatientVisitPatientPaymentOthersCrudModule} from '../patient-visit-patient-payment-others-admin-crud.module';
import {
	PatientVisitPatientPaymentOthersModelState,
	initialState as initialPatientVisitPatientPaymentOthersModelState
} from 'src/app/models/patientVisitPatientPaymentOthers/patient_visit_patient_payment_others.model.state';
 ...
 */
describe('Patient Visit Patient Payment Others Crud List Tile Component', () => {

	let fixture;
	let patientVisitPatientPaymentOthersCrudListComponent: AdminPatientVisitPatientPaymentOthersCrudListComponent;

	let store: MockStore<{ model: PatientVisitPatientPaymentOthersModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientVisitPatientPaymentOthersModelState: PatientVisitPatientPaymentOthersModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientVisitPatientPaymentOthersModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ac7183aa-1605-44c0-9c95-9002167d699d';
	const searchContent = 'text';

	const searchableFieldCount = PatientVisitPatientPaymentOthersModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-visit-patient-payment-others-admin-crud',
		urls: ['patient-visit-patient-payment-others-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientVisitPatientPaymentOthersDataFactory = new PatientVisitPatientPaymentOthersDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientVisitPatientPaymentOthersDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientVisitPatientPaymentOthersModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientVisitPatientPaymentOthersModelState = initialPatientVisitPatientPaymentOthersModelState;
		store.setState({model: patientVisitPatientPaymentOthersModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientVisitPatientPaymentOthersCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientVisitPatientPaymentOthersCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientVisitPatientPaymentOthersCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientVisitPatientPaymentOthersCrudListComponent);
			patientVisitPatientPaymentOthersCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientVisitPatientPaymentOthersCrudListComponent.patientVisitPatientPaymentOtherssCount).toBe(defaultData.length);
			...
			expect(patientVisitPatientPaymentOthersCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientVisitPatientPaymentOthersCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientVisitPatientPaymentOthersDataFactory.createAll(filteredDataSize);
			...
			expect(patientVisitPatientPaymentOthersCrudListComponent.patientVisitPatientPaymentOtherssCount).toBe(filteredDataSize);
			expect(patientVisitPatientPaymentOthersCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientVisitPatientPaymentOthersCrudListComponent.pageIndex).toBe(0);
			expect(patientVisitPatientPaymentOthersCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientVisitPatientPaymentOthersCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientVisitPatientPaymentOthersCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientVisitPatientPaymentOthersCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
