...
import {PatientVisitPatientPaymentOthersModelState, PatientVisitPatientPaymentOthersModelAudit} from 'src/app/models/patientVisitPatientPaymentOthers/patient_visit_patient_payment_others.model.state';
...
import {
	getPatientVisitPatientPaymentOthersModelAuditsByEntityId,
	getPatientVisitPatientPaymentOthersModelWithId,
	getCountPatientVisitPatientPaymentOthersModels,
...
@Component({
	selector: 'cb-patient-visit-patient-payment-others-admin-crud-edit',
	templateUrl: './patient-visit-patient-payment-others-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-visit-patient-payment-others-admin-crud-edit.component.scss',
...
export class AdminPatientVisitPatientPaymentOthersCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	patientVisitPatientPaymentOthersAudits$: Observable<PatientVisitPatientPaymentOthersModelAudit[]>;
	...
	patientVisitOppositeRelation = 'patientVisitPatientPaymentOthers';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientVisitPatientPaymentOthersModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientVisitPatientPaymentOthersModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientVisitPatientPaymentOthersModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientVisitPatientPaymentOthersModel();
		...
		if (this.targetModelId) {
			this.patientVisitPatientPaymentOthersAudits$ = this.store.select(getPatientVisitPatientPaymentOthersModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientVisitPatientPaymentOthersModel> = {
		...
		this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
			modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientVisitPatientPaymentOthersModelWithId, this.targetModelId)
			...
			model = new PatientVisitPatientPaymentOthersModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
			modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientVisitPatientPaymentOthersModel> = {
				...
				queryParams: {
					persistRelations: this.patientVisitMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
				modelAction.PatientVisitPatientPaymentOthersModelActionTypes.CREATE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentOthersAction(
				modelAction.PatientVisitPatientPaymentOthersModelActionTypes.UPDATE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientVisitPatientPaymentOthersModel = new PatientVisitPatientPaymentOthersModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientVisitPatientPaymentOthersModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientVisitPatientPaymentOthersModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientVisitPatientPaymentOthersModel.getProps();
		this.modelRelations = PatientVisitPatientPaymentOthersModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientVisitPatientPaymentOthersModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientVisitPatientPaymentOthersModel {
		let updatedModel: PatientVisitPatientPaymentOthersModel = new PatientVisitPatientPaymentOthersModel(model);
