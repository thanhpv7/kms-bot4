...
import {ServiceModelState} from 'src/app/models/service/service.model.state';
...
import {
	getServiceCollectionModels,
	getServiceCollectionCount
...
import {CrudTileMode} from '../service-admin-crud.component';
...
@Component({
	selector: 'cb-service-admin-crud-list',
	templateUrl: './service-admin-crud-list.component.html',
	styleUrls: [
		'./service-admin-crud-list.component.scss',
...
export class AdminServiceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ServiceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ServiceModel.searchFields,
	...
	/**
	 * List of Services that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.servicesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.servicesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceAction(
				modelAction.ServiceModelActionTypes.INITIALISE_SERVICE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceAction(
				modelAction.ServiceModelActionTypes.FETCH_SERVICE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.services = this.store.select(getServiceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getServiceCollectionCount, this.collectionId).subscribe(
				(count) => this.servicesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ServiceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ServiceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ServiceModelActionTypes.EXPORT_ALL_SERVICE;
		} else {
			actionType = modelAction.ServiceModelActionTypes.EXPORT_SERVICE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ServiceAction(
				modelAction.ServiceModelActionTypes.IMPORT_SERVICES,
				{
					file: file,
				},
				[new modelAction.ServiceAction(
			...
			window.open(environment.API_URL + '/api/service/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ServiceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Service entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Service entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ServiceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ServiceModelActionTypes.DELETE_SERVICE;
						} else if (usingSelected) {
							actionType = modelAction.ServiceModelActionTypes.DELETE_ALL_SERVICE;
						} else {
							actionType = modelAction.ServiceModelActionTypes.DELETE_SERVICE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ServiceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SERVICE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SERVICE and DELETE_SERVICE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ServiceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.servicesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
