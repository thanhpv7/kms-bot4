...
import {AdminServiceCrudListComponent} from './service-admin-crud-list.component';
...
import {AdminServiceCrudModule} from '../service-admin-crud.module';
import {
	ServiceModelState,
	initialState as initialServiceModelState
} from 'src/app/models/service/service.model.state';
 ...
 */
describe('Service Crud List Tile Component', () => {

	let fixture;
	let serviceCrudListComponent: AdminServiceCrudListComponent;

	let store: MockStore<{ model: ServiceModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let serviceModelState: ServiceModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<ServiceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '4c979c0a-c1e6-4d0c-9e61-577662fee27d';
	const searchContent = 'text';

	const searchableFieldCount = ServiceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'service-admin-crud',
		urls: ['service-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const serviceDataFactory = new ServiceDataFactory();

	const defaultDataSize = 30;
	const defaultData = serviceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: ServiceModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		serviceModelState = initialServiceModelState;
		store.setState({model: serviceModelState});
		...
		spyOn(store, 'select')
			.withArgs(getServiceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getServiceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminServiceCrudListComponent);
			serviceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(serviceCrudListComponent.servicesCount).toBe(defaultData.length);
			...
			expect(serviceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			serviceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = serviceDataFactory.createAll(filteredDataSize);
			...
			expect(serviceCrudListComponent.servicesCount).toBe(filteredDataSize);
			expect(serviceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(serviceCrudListComponent.pageIndex).toBe(0);
			expect(serviceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			serviceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		serviceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		serviceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
