...
import {BpjsSEPINACBGIntegrationModelState, BpjsSEPINACBGIntegrationModelAudit} from 'src/app/models/bpjsSEPINACBGIntegration/bpjs_sep_inacbg_integration.model.state';
import {BpjsSEPINACBGIntegrationModel} from 'src/app/models/bpjsSEPINACBGIntegration/bpjs_sep_inacbg_integration.model';
import {
	getBpjsSEPINACBGIntegrationModelAuditsByEntityId,
	getBpjsSEPINACBGIntegrationModelWithId,
	getCountBpjsSEPINACBGIntegrationModels,
...
@Component({
	selector: 'cb-bpjs-sep-inacbg-integration-admin-crud-edit',
	templateUrl: './bpjs-sep-inacbg-integration-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-sep-inacbg-integration-admin-crud-edit.component.scss',
...
export class AdminBpjsSEPINACBGIntegrationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsSEPINACBGIntegrationAudits$: Observable<BpjsSEPINACBGIntegrationModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsSEPINACBGIntegrationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsSEPINACBGIntegrationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsSEPINACBGIntegrationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsSEPINACBGIntegrationModel();
		...
		if (this.targetModelId) {
			this.bpjsSEPINACBGIntegrationAudits$ = this.store.select(getBpjsSEPINACBGIntegrationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsSEPINACBGIntegrationModel> = {
		...
		this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
			modelAction.BpjsSEPINACBGIntegrationModelActionTypes.FETCH_BPJS_SEP_INACBG_INTEGRATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPINACBGIntegrationModelWithId, this.targetModelId)
			...
			model = new BpjsSEPINACBGIntegrationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
			modelAction.BpjsSEPINACBGIntegrationModelActionTypes.FETCH_BPJS_SEP_INACBG_INTEGRATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
				modelAction.BpjsSEPINACBGIntegrationModelActionTypes.CREATE_BPJS_SEP_INACBG_INTEGRATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
				modelAction.BpjsSEPINACBGIntegrationModelActionTypes.UPDATE_BPJS_SEP_INACBG_INTEGRATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsSEPINACBGIntegrationModel = new BpjsSEPINACBGIntegrationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPINACBGIntegrationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsSEPINACBGIntegrationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsSEPINACBGIntegrationModel.getProps();
		this.modelRelations = BpjsSEPINACBGIntegrationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsSEPINACBGIntegrationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsSEPINACBGIntegrationModel {
		let updatedModel: BpjsSEPINACBGIntegrationModel = new BpjsSEPINACBGIntegrationModel(model);
