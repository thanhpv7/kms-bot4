...
import * as modelAction from 'src/app/models/bpjsSEPINACBGIntegration/bpjs_sep_inacbg_integration.model.action';
import {BpjsSEPINACBGIntegrationModelState} from 'src/app/models/bpjsSEPINACBGIntegration/bpjs_sep_inacbg_integration.model.state';
...
import {
	getBpjsSEPINACBGIntegrationCollectionModels,
	getBpjsSEPINACBGIntegrationCollectionCount
} from 'src/app/models/bpjsSEPINACBGIntegration/bpjs_sep_inacbg_integration.model.selector';
...
import {CrudTileMode} from '../bpjs-sep-inacbg-integration-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-sep-inacbg-integration-admin-crud-list',
	templateUrl: './bpjs-sep-inacbg-integration-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-sep-inacbg-integration-admin-crud-list.component.scss',
...
export class AdminBpjsSEPINACBGIntegrationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsSEPINACBGIntegrationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsSEPINACBGIntegrationModel.searchFields,
	...
	/**
	 * List of BPJS SEP INACBG Integrations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsSEPINACBGIntegrations: Observable<BpjsSEPINACBGIntegrationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsSEPINACBGIntegrationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsSEPINACBGIntegrationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsSEPINACBGIntegrationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '92e27799-944a-4b7d-902a-e8a91fd116fa';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsSEPINACBGIntegrationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsSEPINACBGIntegrationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsSEPINACBGIntegrationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
				modelAction.BpjsSEPINACBGIntegrationModelActionTypes.INITIALISE_BPJS_SEP_INACBG_INTEGRATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
				modelAction.BpjsSEPINACBGIntegrationModelActionTypes.FETCH_BPJS_SEP_INACBG_INTEGRATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsSEPINACBGIntegrationAction(
						modelAction.BpjsSEPINACBGIntegrationModelActionTypes.COUNT_BPJS_SEP_INACBG_INTEGRATIONS,
						null
					)
				]
			))
		...
		this.bpjsSEPINACBGIntegrations = this.store.select(getBpjsSEPINACBGIntegrationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsSEPINACBGIntegrationCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsSEPINACBGIntegrationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-sep-inacbg-integration');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsSEPINACBGIntegrationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsSEPINACBGIntegrationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsSEPINACBGIntegrationModelActionTypes.EXPORT_ALL_BPJS_SEP_INACBG_INTEGRATION;
		} else {
			actionType = modelAction.BpjsSEPINACBGIntegrationModelActionTypes.EXPORT_BPJS_SEP_INACBG_INTEGRATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
				modelAction.BpjsSEPINACBGIntegrationModelActionTypes.IMPORT_BPJS_SEP_INACBG_INTEGRATIONS,
				{
					file: file,
				},
				[new modelAction.BpjsSEPINACBGIntegrationAction(
			...
			window.open(environment.API_URL + '/api/bpjssepinacbgintegration/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsSEPINACBGIntegrationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP INACBG Integration entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP INACBG Integration entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsSEPINACBGIntegrationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsSEPINACBGIntegrationModelActionTypes.DELETE_BPJS_SEP_INACBG_INTEGRATION;
						} else if (usingSelected) {
							actionType = modelAction.BpjsSEPINACBGIntegrationModelActionTypes.DELETE_ALL_BPJS_SEP_INACBG_INTEGRATION;
						} else {
							actionType = modelAction.BpjsSEPINACBGIntegrationModelActionTypes.DELETE_BPJS_SEP_INACBG_INTEGRATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsSEPINACBGIntegrationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SEP_INACBG_INTEGRATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SEP_INACBG_INTEGRATION and DELETE_BPJS_SEP_INACBG_INTEGRATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsSEPINACBGIntegrationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsSEPINACBGIntegrationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsSEPINACBGIntegrationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
