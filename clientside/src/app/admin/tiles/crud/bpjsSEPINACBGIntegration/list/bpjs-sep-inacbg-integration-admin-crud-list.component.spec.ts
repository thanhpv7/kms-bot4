...
import {AdminBpjsSEPINACBGIntegrationCrudListComponent} from './bpjs-sep-inacbg-integration-admin-crud-list.component';
import {BpjsSEPINACBGIntegrationDataFactory} from 'src/app/lib/utils/factories/bpjs-sep-inacbg-integration-data-factory';
import {BpjsSEPINACBGIntegrationModel} from 'src/app/models/bpjsSEPINACBGIntegration/bpjs_sep_inacbg_integration.model';
import {AdminBpjsSEPINACBGIntegrationCrudModule} from '../bpjs-sep-inacbg-integration-admin-crud.module';
import {
	BpjsSEPINACBGIntegrationModelState,
	initialState as initialBpjsSEPINACBGIntegrationModelState
} from 'src/app/models/bpjsSEPINACBGIntegration/bpjs_sep_inacbg_integration.model.state';
 ...
 */
describe('BPJS SEP INACBG Integration Crud List Tile Component', () => {

	let fixture;
	let bpjsSEPINACBGIntegrationCrudListComponent: AdminBpjsSEPINACBGIntegrationCrudListComponent;

	let store: MockStore<{ model: BpjsSEPINACBGIntegrationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsSEPINACBGIntegrationModelState: BpjsSEPINACBGIntegrationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsSEPINACBGIntegrationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '92e27799-944a-4b7d-902a-e8a91fd116fa';
	const searchContent = 'text';

	const searchableFieldCount = BpjsSEPINACBGIntegrationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-sep-inacbg-integration-admin-crud',
		urls: ['bpjs-sep-inacbg-integration-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsSEPINACBGIntegrationDataFactory = new BpjsSEPINACBGIntegrationDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsSEPINACBGIntegrationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsSEPINACBGIntegrationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsSEPINACBGIntegrationModelState = initialBpjsSEPINACBGIntegrationModelState;
		store.setState({model: bpjsSEPINACBGIntegrationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsSEPINACBGIntegrationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsSEPINACBGIntegrationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsSEPINACBGIntegrationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsSEPINACBGIntegrationCrudListComponent);
			bpjsSEPINACBGIntegrationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsSEPINACBGIntegrationCrudListComponent.bpjsSEPINACBGIntegrationsCount).toBe(defaultData.length);
			...
			expect(bpjsSEPINACBGIntegrationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsSEPINACBGIntegrationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsSEPINACBGIntegrationDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsSEPINACBGIntegrationCrudListComponent.bpjsSEPINACBGIntegrationsCount).toBe(filteredDataSize);
			expect(bpjsSEPINACBGIntegrationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsSEPINACBGIntegrationCrudListComponent.pageIndex).toBe(0);
			expect(bpjsSEPINACBGIntegrationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsSEPINACBGIntegrationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsSEPINACBGIntegrationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsSEPINACBGIntegrationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
