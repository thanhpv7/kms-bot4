...
import * as modelAction from 'src/app/models/patientEmergencyContactDetail/patient_emergency_contact_detail.model.action';
import {PatientEmergencyContactDetailModelState, PatientEmergencyContactDetailModelAudit} from 'src/app/models/patientEmergencyContactDetail/patient_emergency_contact_detail.model.state';
...
import {
	getPatientEmergencyContactDetailModelAuditsByEntityId,
	getPatientEmergencyContactDetailModelWithId,
	getCountPatientEmergencyContactDetailModels,
...
import {CrudTileMode} from '../patient-emergency-contact-detail-admin-crud.component';
...
@Component({
	selector: 'cb-patient-emergency-contact-detail-admin-crud-edit',
	templateUrl: './patient-emergency-contact-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-emergency-contact-detail-admin-crud-edit.component.scss',
...
export class AdminPatientEmergencyContactDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '48c4d527-613f-41d4-8084-11c3ba04e11b';
	 ...
	 */
	patientEmergencyContactDetailAudits$: Observable<PatientEmergencyContactDetailModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientEmergencyContactDetail';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientEmergencyContactDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientEmergencyContactDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientEmergencyContactDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientEmergencyContactDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientEmergencyContactDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientEmergencyContactDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientEmergencyContactDetailModel();
		...
		if (this.targetModelId) {
			this.patientEmergencyContactDetailAudits$ = this.store.select(getPatientEmergencyContactDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientEmergencyContactDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
			modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_PATIENT_EMERGENCY_CONTACT_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientEmergencyContactDetailModelWithId, this.targetModelId)
			...
			model = new PatientEmergencyContactDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-emergency-contact-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
			modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_PATIENT_EMERGENCY_CONTACT_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientEmergencyContactDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.patientGeneralInformationMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientEmergencyContactDetailAction(
						modelAction.PatientEmergencyContactDetailModelActionTypes.COUNT_PATIENT_EMERGENCY_CONTACT_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
				modelAction.PatientEmergencyContactDetailModelActionTypes.CREATE_PATIENT_EMERGENCY_CONTACT_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
				modelAction.PatientEmergencyContactDetailModelActionTypes.UPDATE_PATIENT_EMERGENCY_CONTACT_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientEmergencyContactDetailModel = new PatientEmergencyContactDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientEmergencyContactDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientEmergencyContactDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientEmergencyContactDetailModel.getProps();
		this.modelRelations = PatientEmergencyContactDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientEmergencyContactDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientEmergencyContactDetailModel {
		let updatedModel: PatientEmergencyContactDetailModel = new PatientEmergencyContactDetailModel(model);
