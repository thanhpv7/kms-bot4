...
import {AdminPatientEmergencyContactDetailCrudListComponent} from './patient-emergency-contact-detail-admin-crud-list.component';
...
import {AdminPatientEmergencyContactDetailCrudModule} from '../patient-emergency-contact-detail-admin-crud.module';
import {
	PatientEmergencyContactDetailModelState,
	initialState as initialPatientEmergencyContactDetailModelState
} from 'src/app/models/patientEmergencyContactDetail/patient_emergency_contact_detail.model.state';
 ...
 */
describe('Patient Emergency Contact Detail Crud List Tile Component', () => {

	let fixture;
	let patientEmergencyContactDetailCrudListComponent: AdminPatientEmergencyContactDetailCrudListComponent;

	let store: MockStore<{ model: PatientEmergencyContactDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientEmergencyContactDetailModelState: PatientEmergencyContactDetailModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientEmergencyContactDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '48c4d527-613f-41d4-8084-11c3ba04e11b';
	const searchContent = 'text';

	const searchableFieldCount = PatientEmergencyContactDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-emergency-contact-detail-admin-crud',
		urls: ['patient-emergency-contact-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientEmergencyContactDetailDataFactory = new PatientEmergencyContactDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientEmergencyContactDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientEmergencyContactDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientEmergencyContactDetailModelState = initialPatientEmergencyContactDetailModelState;
		store.setState({model: patientEmergencyContactDetailModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientEmergencyContactDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientEmergencyContactDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientEmergencyContactDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientEmergencyContactDetailCrudListComponent);
			patientEmergencyContactDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientEmergencyContactDetailCrudListComponent.patientEmergencyContactDetailsCount).toBe(defaultData.length);
			...
			expect(patientEmergencyContactDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientEmergencyContactDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientEmergencyContactDetailDataFactory.createAll(filteredDataSize);
			...
			expect(patientEmergencyContactDetailCrudListComponent.patientEmergencyContactDetailsCount).toBe(filteredDataSize);
			expect(patientEmergencyContactDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientEmergencyContactDetailCrudListComponent.pageIndex).toBe(0);
			expect(patientEmergencyContactDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientEmergencyContactDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientEmergencyContactDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientEmergencyContactDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
