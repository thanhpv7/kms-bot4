...
import {PatientEmergencyContactDetailModelState} from 'src/app/models/patientEmergencyContactDetail/patient_emergency_contact_detail.model.state';
...
import {
	getPatientEmergencyContactDetailCollectionModels,
	getPatientEmergencyContactDetailCollectionCount
...
@Component({
	selector: 'cb-patient-emergency-contact-detail-admin-crud-list',
	templateUrl: './patient-emergency-contact-detail-admin-crud-list.component.html',
	styleUrls: [
		'./patient-emergency-contact-detail-admin-crud-list.component.scss',
...
export class AdminPatientEmergencyContactDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientEmergencyContactDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientEmergencyContactDetailModel.searchFields,
	...
	/**
	 * List of Patient Emergency Contact Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientEmergencyContactDetails: Observable<PatientEmergencyContactDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientEmergencyContactDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientEmergencyContactDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientEmergencyContactDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
				modelAction.PatientEmergencyContactDetailModelActionTypes.INITIALISE_PATIENT_EMERGENCY_CONTACT_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
				modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_PATIENT_EMERGENCY_CONTACT_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientEmergencyContactDetails = this.store.select(getPatientEmergencyContactDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientEmergencyContactDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.patientEmergencyContactDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientEmergencyContactDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientEmergencyContactDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientEmergencyContactDetailModelActionTypes.EXPORT_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL;
		} else {
			actionType = modelAction.PatientEmergencyContactDetailModelActionTypes.EXPORT_PATIENT_EMERGENCY_CONTACT_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
				modelAction.PatientEmergencyContactDetailModelActionTypes.IMPORT_PATIENT_EMERGENCY_CONTACT_DETAILS,
				{
					file: file,
				},
				[new modelAction.PatientEmergencyContactDetailAction(
			...
			window.open(environment.API_URL + '/api/patientemergencycontactdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientEmergencyContactDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Emergency Contact Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Emergency Contact Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientEmergencyContactDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientEmergencyContactDetailModelActionTypes.DELETE_PATIENT_EMERGENCY_CONTACT_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.PatientEmergencyContactDetailModelActionTypes.DELETE_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL;
						} else {
							actionType = modelAction.PatientEmergencyContactDetailModelActionTypes.DELETE_PATIENT_EMERGENCY_CONTACT_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientEmergencyContactDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_EMERGENCY_CONTACT_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL and DELETE_PATIENT_EMERGENCY_CONTACT_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientEmergencyContactDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientEmergencyContactDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
