...
import {AdminAmendmentDetailCrudListComponent} from './amendment-detail-admin-crud-list.component';
...
import {AdminAmendmentDetailCrudModule} from '../amendment-detail-admin-crud.module';
import {
 ...
 */
describe('Amendment Detail Crud List Tile Component', () => {

	let fixture;
	let amendmentDetailCrudListComponent: AdminAmendmentDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<AmendmentDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'fc49b652-5964-4169-8cc7-278424e2fc5e';
	const searchContent = 'text';

	const searchableFieldCount = AmendmentDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'amendment-detail-admin-crud',
		urls: ['amendment-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const amendmentDetailDataFactory = new AmendmentDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = amendmentDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getAmendmentDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getAmendmentDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminAmendmentDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminAmendmentDetailCrudListComponent);
			amendmentDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(amendmentDetailCrudListComponent.amendmentDetailsCount).toBe(defaultData.length);
			...
			expect(amendmentDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			amendmentDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = amendmentDetailDataFactory.createAll(filteredDataSize);
			...
			expect(amendmentDetailCrudListComponent.amendmentDetailsCount).toBe(filteredDataSize);
			expect(amendmentDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(amendmentDetailCrudListComponent.pageIndex).toBe(0);
			expect(amendmentDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			amendmentDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		amendmentDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		amendmentDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
