...
import * as modelAction from 'src/app/models/amendmentDetail/amendment_detail.model.action';
import {AmendmentDetailModelState} from 'src/app/models/amendmentDetail/amendment_detail.model.state';
...
import {
	getAmendmentDetailCollectionModels,
	getAmendmentDetailCollectionCount
...
import {CrudTileMode} from '../amendment-detail-admin-crud.component';
...
@Component({
	selector: 'cb-amendment-detail-admin-crud-list',
	templateUrl: './amendment-detail-admin-crud-list.component.html',
	styleUrls: [
		'./amendment-detail-admin-crud-list.component.scss',
...
export class AdminAmendmentDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<AmendmentDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...AmendmentDetailModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = AmendmentDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'fc49b652-5964-4169-8cc7-278424e2fc5e';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.amendmentDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.amendmentDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: AmendmentDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.AmendmentDetailAction(
				modelAction.AmendmentDetailModelActionTypes.INITIALISE_AMENDMENT_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.AmendmentDetailAction(
				modelAction.AmendmentDetailModelActionTypes.FETCH_AMENDMENT_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.AmendmentDetailAction(
						modelAction.AmendmentDetailModelActionTypes.COUNT_AMENDMENT_DETAILS,
						null
					)
				]
			))
		...
		this.amendmentDetails = this.store.select(getAmendmentDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getAmendmentDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.amendmentDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('amendment-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: AmendmentDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.AmendmentDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.AmendmentDetailModelActionTypes.EXPORT_ALL_AMENDMENT_DETAIL;
		} else {
			actionType = modelAction.AmendmentDetailModelActionTypes.EXPORT_AMENDMENT_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.AmendmentDetailAction(
				modelAction.AmendmentDetailModelActionTypes.IMPORT_AMENDMENT_DETAILS,
				{
					file: file,
				},
				[new modelAction.AmendmentDetailAction(
			...
			window.open(environment.API_URL + '/api/amendmentdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: AmendmentDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Amendment Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Amendment Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.AmendmentDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.AmendmentDetailModelActionTypes.DELETE_AMENDMENT_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.AmendmentDetailModelActionTypes.DELETE_ALL_AMENDMENT_DETAIL;
						} else {
							actionType = modelAction.AmendmentDetailModelActionTypes.DELETE_AMENDMENT_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.AmendmentDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_AMENDMENT_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_AMENDMENT_DETAIL and DELETE_AMENDMENT_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<AmendmentDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = AmendmentDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.amendmentDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
