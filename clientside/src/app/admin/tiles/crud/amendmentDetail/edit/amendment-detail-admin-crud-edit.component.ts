...
import {AmendmentDetailModelState, AmendmentDetailModelAudit} from 'src/app/models/amendmentDetail/amendment_detail.model.state';
import {AmendmentDetailModel} from 'src/app/models/amendmentDetail/amendment_detail.model';
import {
	getAmendmentDetailModelAuditsByEntityId,
	getAmendmentDetailModelWithId,
	getCountAmendmentDetailModels,
...
@Component({
	selector: 'cb-amendment-detail-admin-crud-edit',
	templateUrl: './amendment-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./amendment-detail-admin-crud-edit.component.scss',
...
export class AdminAmendmentDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	amendmentDetailAudits$: Observable<AmendmentDetailModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'amendmentDetails';
	...
	staffOppositeRelation = 'amendmentDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = AmendmentDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: AmendmentDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountAmendmentDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new AmendmentDetailModel();
		...
		if (this.targetModelId) {
			this.amendmentDetailAudits$ = this.store.select(getAmendmentDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<AmendmentDetailModel> = {
		...
		this.store.dispatch(new modelAction.AmendmentDetailAction(
			modelAction.AmendmentDetailModelActionTypes.FETCH_AMENDMENT_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getAmendmentDetailModelWithId, this.targetModelId)
			...
			model = new AmendmentDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.AmendmentDetailAction(
			modelAction.AmendmentDetailModelActionTypes.FETCH_AMENDMENT_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<AmendmentDetailModel> = {
				...
				queryParams: {
					persistRelations: this.medicalExaminationRecordMultiCrudActive || this.staffMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.AmendmentDetailAction(
				modelAction.AmendmentDetailModelActionTypes.CREATE_AMENDMENT_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.AmendmentDetailAction(
				modelAction.AmendmentDetailModelActionTypes.UPDATE_AMENDMENT_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: AmendmentDetailModel = new AmendmentDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getAmendmentDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new AmendmentDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = AmendmentDetailModel.getProps();
		this.modelRelations = AmendmentDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: AmendmentDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): AmendmentDetailModel {
		let updatedModel: AmendmentDetailModel = new AmendmentDetailModel(model);
