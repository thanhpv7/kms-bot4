...
import {InsuranceProviderModelState} from 'src/app/models/insuranceProvider/insurance_provider.model.state';
...
import {
	...
	getInsuranceProviderCollectionCount
...
@Component({
	selector: 'cb-insurance-provider-admin-crud-list',
	templateUrl: './insurance-provider-admin-crud-list.component.html',
	styleUrls: [
		'./insurance-provider-admin-crud-list.component.scss',
...
export class AdminInsuranceProviderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InsuranceProviderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InsuranceProviderModel.searchFields,
	...
	/**
	 * List of Insurance Providers that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	insuranceProviders: Observable<InsuranceProviderModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	insuranceProvidersCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.insuranceProvidersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.insuranceProvidersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InsuranceProviderAction(
				modelAction.InsuranceProviderModelActionTypes.INITIALISE_INSURANCE_PROVIDER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InsuranceProviderAction(
				modelAction.InsuranceProviderModelActionTypes.FETCH_INSURANCE_PROVIDER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.insuranceProviders = this.store.select(getInsuranceProviderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInsuranceProviderCollectionCount, this.collectionId).subscribe(
				(count) => this.insuranceProvidersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InsuranceProviderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InsuranceProviderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InsuranceProviderModelActionTypes.EXPORT_ALL_INSURANCE_PROVIDER;
		} else {
			actionType = modelAction.InsuranceProviderModelActionTypes.EXPORT_INSURANCE_PROVIDER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InsuranceProviderAction(
				modelAction.InsuranceProviderModelActionTypes.IMPORT_INSURANCE_PROVIDERS,
				{
					file: file,
				},
				[new modelAction.InsuranceProviderAction(
			...
			window.open(environment.API_URL + '/api/insuranceprovider/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InsuranceProviderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Insurance Provider entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Insurance Provider entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InsuranceProviderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InsuranceProviderModelActionTypes.DELETE_INSURANCE_PROVIDER;
						} else if (usingSelected) {
							actionType = modelAction.InsuranceProviderModelActionTypes.DELETE_ALL_INSURANCE_PROVIDER;
						} else {
							actionType = modelAction.InsuranceProviderModelActionTypes.DELETE_INSURANCE_PROVIDER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InsuranceProviderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INSURANCE_PROVIDER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INSURANCE_PROVIDER and DELETE_INSURANCE_PROVIDER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InsuranceProviderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.insuranceProvidersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
