...
import {AdminInsuranceProviderCrudListComponent} from './insurance-provider-admin-crud-list.component';
...
import {AdminInsuranceProviderCrudModule} from '../insurance-provider-admin-crud.module';
import {
 ...
 */
describe('Insurance Provider Crud List Tile Component', () => {

	let fixture;
	let insuranceProviderCrudListComponent: AdminInsuranceProviderCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<InsuranceProviderModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '57b903bb-b764-4ec1-b353-52eb94a96f65';
	const searchContent = 'text';

	const searchableFieldCount = InsuranceProviderModel.searchFields.length;

	const routerState: RouterState = {
		url: 'insurance-provider-admin-crud',
		urls: ['insurance-provider-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const insuranceProviderDataFactory = new InsuranceProviderDataFactory();

	const defaultDataSize = 30;
	const defaultData = insuranceProviderDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getInsuranceProviderCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInsuranceProviderCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInsuranceProviderCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInsuranceProviderCrudListComponent);
			insuranceProviderCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(insuranceProviderCrudListComponent.insuranceProvidersCount).toBe(defaultData.length);
			...
			expect(insuranceProviderCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			insuranceProviderCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = insuranceProviderDataFactory.createAll(filteredDataSize);
			...
			expect(insuranceProviderCrudListComponent.insuranceProvidersCount).toBe(filteredDataSize);
			expect(insuranceProviderCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(insuranceProviderCrudListComponent.pageIndex).toBe(0);
			expect(insuranceProviderCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			insuranceProviderCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		insuranceProviderCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		insuranceProviderCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
