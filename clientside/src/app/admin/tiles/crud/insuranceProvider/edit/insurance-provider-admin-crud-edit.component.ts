...
import * as modelAction from 'src/app/models/insuranceProvider/insurance_provider.model.action';
import {InsuranceProviderModelState, InsuranceProviderModelAudit} from 'src/app/models/insuranceProvider/insurance_provider.model.state';
import {InsuranceProviderModel} from 'src/app/models/insuranceProvider/insurance_provider.model';
import {
	getInsuranceProviderModelAuditsByEntityId,
...
import {CrudTileMode} from '../insurance-provider-admin-crud.component';
...
@Component({
	selector: 'cb-insurance-provider-admin-crud-edit',
	templateUrl: './insurance-provider-admin-crud-edit.component.html',
	styleUrls: [
		'./insurance-provider-admin-crud-edit.component.scss',
...
export class AdminInsuranceProviderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '57b903bb-b764-4ec1-b353-52eb94a96f65';
	 ...
	 */
	insuranceProviderAudits$: Observable<InsuranceProviderModelAudit[]>;
	...
	insuranceBenefitPlansMultiCrudActive: boolean = false;
	insuranceBenefitPlansDisplayName = 'Insurance Benefit Plans';
	insuranceBenefitPlansCollectionId: string = uuidv4();
	...
	insuranceBenefitPlansOppositeRelation = 'insuranceProvider';
	insuranceBenefitPlansRelations: ModelRelation[] = this.insuranceBenefitPlansMultiCrudActive ? InsuranceBenefitPlanModel.getRelations()
		.map(relation => {
			if (relation.id === this.insuranceBenefitPlansOppositeRelation) {
	 ...
	 */
	modelProperties: ModelProperty[] = InsuranceProviderModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InsuranceProviderModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InsuranceProviderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'insuranceBenefitPlans' && this.insuranceBenefitPlansMultiCrudActive) {
	 ...
	 */
	targetModel: InsuranceProviderModel;
	...
	constructor(
		private readonly store: Store<{ model: InsuranceProviderModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInsuranceProviderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InsuranceProviderModel();
		...
		if (this.targetModelId) {
			this.insuranceProviderAudits$ = this.store.select(getInsuranceProviderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InsuranceProviderModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InsuranceProviderAction(
			modelAction.InsuranceProviderModelActionTypes.FETCH_INSURANCE_PROVIDER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInsuranceProviderModelWithId, this.targetModelId)
			...
			model = new InsuranceProviderModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('insurance-provider');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InsuranceProviderAction(
			modelAction.InsuranceProviderModelActionTypes.FETCH_INSURANCE_PROVIDER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InsuranceProviderModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.insuranceBenefitPlansMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InsuranceProviderAction(
						modelAction.InsuranceProviderModelActionTypes.COUNT_INSURANCE_PROVIDERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InsuranceProviderAction(
				modelAction.InsuranceProviderModelActionTypes.CREATE_INSURANCE_PROVIDER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InsuranceProviderAction(
				modelAction.InsuranceProviderModelActionTypes.UPDATE_INSURANCE_PROVIDER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InsuranceProviderModel = new InsuranceProviderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInsuranceProviderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InsuranceProviderModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.insuranceBenefitPlansDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InsuranceBenefitPlanModel.getProps(), this.insuranceBenefitPlansRelations, this.isViewOnly),
					new InsuranceBenefitPlanModel(),
					InsuranceBenefitPlanModel,
					this.insuranceBenefitPlansDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.insuranceBenefitPlansRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InsuranceProviderModel.getProps();
		this.modelRelations = InsuranceProviderModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'insuranceBenefitPlans';
		}).hideElement = this.insuranceBenefitPlansMultiCrudActive || this.modelRelations.find(rel => rel.id === 'insuranceBenefitPlans').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.insuranceBenefitPlansMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.insuranceBenefitPlansDisplayName, many: true});
			this.store.dispatch(new insuranceBenefitPlanModelAction.InsuranceBenefitPlanAction(
				insuranceBenefitPlanModelAction.InsuranceBenefitPlanModelActionTypes.INITIALISE_INSURANCE_BENEFIT_PLAN_COLLECTION_STATE,
				{
					collectionId: this.insuranceBenefitPlansCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getInsuranceBenefitPlanCollectionModels, this.insuranceBenefitPlansCollectionId).subscribe(models => {
				models.forEach(insuranceBenefitPlans => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(InsuranceBenefitPlanModel.getProps(), this.insuranceBenefitPlansRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.insuranceBenefitPlansIds && this.targetModel.insuranceBenefitPlansIds.length > 0 && this.insuranceBenefitPlansMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InsuranceBenefitPlanModel,
				this.insuranceBenefitPlansOppositeRelation,
				this.insuranceBenefitPlansCollectionId,
				this.targetModel.insuranceBenefitPlansIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: InsuranceProviderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InsuranceProviderModel {
		let updatedModel: InsuranceProviderModel = new InsuranceProviderModel(model);

		updatedModel.insuranceBenefitPlans = [];
		if (this.insuranceBenefitPlansMultiCrudActive) {
			if (event.additionalEntities.insuranceBenefitPlans) {
				event.additionalEntities.insuranceBenefitPlans.forEach(entity => {
					updatedModel.insuranceBenefitPlans.push(entity);
				});
			}

			if (!updatedModel.insuranceBenefitPlans.length) {
				updatedModel.insuranceBenefitPlansIds = [];
			}
		}
		return updatedModel;
	}
