...
import {AdminProcessWorkflowCrudListComponent} from './process-workflow-admin-crud-list.component';
...
import {AdminProcessWorkflowCrudModule} from '../process-workflow-admin-crud.module';
import {
	ProcessWorkflowModelState,
	initialState as initialProcessWorkflowModelState
} from 'src/app/models/processWorkflow/process_workflow.model.state';
 ...
 */
describe('Process Workflow Crud List Tile Component', () => {

	let fixture;
	let processWorkflowCrudListComponent: AdminProcessWorkflowCrudListComponent;

	let store: MockStore<{ model: ProcessWorkflowModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let processWorkflowModelState: ProcessWorkflowModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<ProcessWorkflowModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '775afa98-21e2-4f10-8ae5-3a473b07bee3';
	const searchContent = 'text';

	const searchableFieldCount = ProcessWorkflowModel.searchFields.length;

	const routerState: RouterState = {
		url: 'process-workflow-admin-crud',
		urls: ['process-workflow-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const processWorkflowDataFactory = new ProcessWorkflowDataFactory();

	const defaultDataSize = 30;
	const defaultData = processWorkflowDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: ProcessWorkflowModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		processWorkflowModelState = initialProcessWorkflowModelState;
		store.setState({model: processWorkflowModelState});
		...
		spyOn(store, 'select')
			.withArgs(getProcessWorkflowCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getProcessWorkflowCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminProcessWorkflowCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminProcessWorkflowCrudListComponent);
			processWorkflowCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(processWorkflowCrudListComponent.processWorkflowsCount).toBe(defaultData.length);
			...
			expect(processWorkflowCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			processWorkflowCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = processWorkflowDataFactory.createAll(filteredDataSize);
			...
			expect(processWorkflowCrudListComponent.processWorkflowsCount).toBe(filteredDataSize);
			expect(processWorkflowCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(processWorkflowCrudListComponent.pageIndex).toBe(0);
			expect(processWorkflowCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			processWorkflowCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		processWorkflowCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		processWorkflowCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
