...
import * as modelAction from 'src/app/models/processWorkflow/process_workflow.model.action';
import {ProcessWorkflowModelState} from 'src/app/models/processWorkflow/process_workflow.model.state';
...
import {
	getProcessWorkflowCollectionModels,
	getProcessWorkflowCollectionCount
...
import {CrudTileMode} from '../process-workflow-admin-crud.component';
...
@Component({
	selector: 'cb-process-workflow-admin-crud-list',
	templateUrl: './process-workflow-admin-crud-list.component.html',
	styleUrls: [
		'./process-workflow-admin-crud-list.component.scss',
...
export class AdminProcessWorkflowCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ProcessWorkflowModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ProcessWorkflowModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = ProcessWorkflowModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '775afa98-21e2-4f10-8ae5-3a473b07bee3';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.processWorkflowsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.processWorkflowsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ProcessWorkflowModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ProcessWorkflowAction(
				modelAction.ProcessWorkflowModelActionTypes.INITIALISE_PROCESS_WORKFLOW_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ProcessWorkflowAction(
				modelAction.ProcessWorkflowModelActionTypes.FETCH_PROCESS_WORKFLOW_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ProcessWorkflowAction(
						modelAction.ProcessWorkflowModelActionTypes.COUNT_PROCESS_WORKFLOWS,
						null
					)
				]
			))
		...
		this.processWorkflows = this.store.select(getProcessWorkflowCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getProcessWorkflowCollectionCount, this.collectionId).subscribe(
				(count) => this.processWorkflowsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('process-workflow');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ProcessWorkflowModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ProcessWorkflowModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ProcessWorkflowModelActionTypes.EXPORT_ALL_PROCESS_WORKFLOW;
		} else {
			actionType = modelAction.ProcessWorkflowModelActionTypes.EXPORT_PROCESS_WORKFLOW_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ProcessWorkflowAction(
				modelAction.ProcessWorkflowModelActionTypes.IMPORT_PROCESS_WORKFLOWS,
				{
					file: file,
				},
				[new modelAction.ProcessWorkflowAction(
			...
			window.open(environment.API_URL + '/api/processworkflow/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ProcessWorkflowModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Process Workflow entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Process Workflow entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ProcessWorkflowModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ProcessWorkflowModelActionTypes.DELETE_PROCESS_WORKFLOW;
						} else if (usingSelected) {
							actionType = modelAction.ProcessWorkflowModelActionTypes.DELETE_ALL_PROCESS_WORKFLOW;
						} else {
							actionType = modelAction.ProcessWorkflowModelActionTypes.DELETE_PROCESS_WORKFLOW_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ProcessWorkflowAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PROCESS_WORKFLOW when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PROCESS_WORKFLOW and DELETE_PROCESS_WORKFLOW_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ProcessWorkflowModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ProcessWorkflowModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.processWorkflowsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
