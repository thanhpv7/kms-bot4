...
import {ProcessWorkflowModelState, ProcessWorkflowModelAudit} from 'src/app/models/processWorkflow/process_workflow.model.state';
import {ProcessWorkflowModel} from 'src/app/models/processWorkflow/process_workflow.model';
import {
	getProcessWorkflowModelAuditsByEntityId,
	getProcessWorkflowModelWithId,
	getCountProcessWorkflowModels,
...
@Component({
	selector: 'cb-process-workflow-admin-crud-edit',
	templateUrl: './process-workflow-admin-crud-edit.component.html',
	styleUrls: [
		'./process-workflow-admin-crud-edit.component.scss',
...
export class AdminProcessWorkflowCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	processWorkflowAudits$: Observable<ProcessWorkflowModelAudit[]>;
	...
	staffOppositeRelation = 'processWorkflowLogs';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ProcessWorkflowModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ProcessWorkflowModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountProcessWorkflowModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ProcessWorkflowModel();
		...
		if (this.targetModelId) {
			this.processWorkflowAudits$ = this.store.select(getProcessWorkflowModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ProcessWorkflowModel> = {
		...
		this.store.dispatch(new modelAction.ProcessWorkflowAction(
			modelAction.ProcessWorkflowModelActionTypes.FETCH_PROCESS_WORKFLOW,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getProcessWorkflowModelWithId, this.targetModelId)
			...
			model = new ProcessWorkflowModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ProcessWorkflowAction(
			modelAction.ProcessWorkflowModelActionTypes.FETCH_PROCESS_WORKFLOW_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.ProcessWorkflowAction(
				modelAction.ProcessWorkflowModelActionTypes.CREATE_PROCESS_WORKFLOW,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ProcessWorkflowAction(
				modelAction.ProcessWorkflowModelActionTypes.UPDATE_PROCESS_WORKFLOW,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ProcessWorkflowModel = new ProcessWorkflowModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getProcessWorkflowModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ProcessWorkflowModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ProcessWorkflowModel.getProps();
		this.modelRelations = ProcessWorkflowModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ProcessWorkflowModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ProcessWorkflowModel {
		let updatedModel: ProcessWorkflowModel = new ProcessWorkflowModel(model);
