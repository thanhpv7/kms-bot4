...
import {AdminMedicalFeeCrudListComponent} from './medical-fee-admin-crud-list.component';
...
import {AdminMedicalFeeCrudModule} from '../medical-fee-admin-crud.module';
import {
 ...
 */
describe('Medical Fee Crud List Tile Component', () => {

	let fixture;
	let medicalFeeCrudListComponent: AdminMedicalFeeCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalFeeModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '79dcd371-2355-4026-8bfd-33d28ef90393';
	const searchContent = 'text';

	const searchableFieldCount = MedicalFeeModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-fee-admin-crud',
		urls: ['medical-fee-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalFeeDataFactory = new MedicalFeeDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalFeeDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getMedicalFeeCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalFeeCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminMedicalFeeCrudListComponent);
			medicalFeeCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalFeeCrudListComponent.medicalFeesCount).toBe(defaultData.length);
			...
			expect(medicalFeeCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalFeeCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalFeeDataFactory.createAll(filteredDataSize);
			...
			expect(medicalFeeCrudListComponent.medicalFeesCount).toBe(filteredDataSize);
			expect(medicalFeeCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalFeeCrudListComponent.pageIndex).toBe(0);
			expect(medicalFeeCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalFeeCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalFeeCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalFeeCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
