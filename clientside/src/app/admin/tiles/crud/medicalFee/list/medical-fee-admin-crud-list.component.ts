...
import {MedicalFeeModelState} from 'src/app/models/medicalFee/medical_fee.model.state';
...
import {CrudTileMode} from '../medical-fee-admin-crud.component';
...
@Component({
	selector: 'cb-medical-fee-admin-crud-list',
	templateUrl: './medical-fee-admin-crud-list.component.html',
	styleUrls: [
		'./medical-fee-admin-crud-list.component.scss',
...
export class AdminMedicalFeeCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalFeeModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalFeeModel.searchFields,
	...
	/**
	 * List of Medical Fees that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalFees: Observable<MedicalFeeModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalFeesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalFeesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalFeesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalFeeAction(
				modelAction.MedicalFeeModelActionTypes.INITIALISE_MEDICAL_FEE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalFeeAction(
				modelAction.MedicalFeeModelActionTypes.FETCH_MEDICAL_FEE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalFees = this.store.select(getMedicalFeeCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalFeeCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalFeesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalFeeModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalFeeModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalFeeModelActionTypes.EXPORT_ALL_MEDICAL_FEE;
		} else {
			actionType = modelAction.MedicalFeeModelActionTypes.EXPORT_MEDICAL_FEE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalFeeAction(
				modelAction.MedicalFeeModelActionTypes.IMPORT_MEDICAL_FEES,
				{
					file: file,
				},
				[new modelAction.MedicalFeeAction(
			...
			window.open(environment.API_URL + '/api/medicalfee/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalFeeModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Fee entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Fee entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalFeeModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalFeeModelActionTypes.DELETE_MEDICAL_FEE;
						} else if (usingSelected) {
							actionType = modelAction.MedicalFeeModelActionTypes.DELETE_ALL_MEDICAL_FEE;
						} else {
							actionType = modelAction.MedicalFeeModelActionTypes.DELETE_MEDICAL_FEE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalFeeAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_FEE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_FEE and DELETE_MEDICAL_FEE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalFeeModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalFeesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
