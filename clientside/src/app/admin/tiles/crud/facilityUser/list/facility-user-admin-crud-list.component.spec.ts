...
import {AdminFacilityUserCrudListComponent} from './facility-user-admin-crud-list.component';
...
import {AdminFacilityUserCrudModule} from '../facility-user-admin-crud.module';
import {
	FacilityUserModelState,
	initialState as initialFacilityUserModelState
} from 'src/app/models/facilityUser/facility_user.model.state';
 ...
 */
describe('Facility User Crud List Tile Component', () => {

	let fixture;
	let facilityUserCrudListComponent: AdminFacilityUserCrudListComponent;

	let store: MockStore<{ model: FacilityUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let facilityUserModelState: FacilityUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<FacilityUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '80cebab1-0b18-41c9-8354-9b544d4e8ad4';
	const searchContent = 'text';

	const searchableFieldCount = FacilityUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'facility-user-admin-crud',
		urls: ['facility-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const facilityUserDataFactory = new FacilityUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = facilityUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: FacilityUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		facilityUserModelState = initialFacilityUserModelState;
		store.setState({model: facilityUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getFacilityUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getFacilityUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminFacilityUserCrudListComponent);
			facilityUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(facilityUserCrudListComponent.facilityUsersCount).toBe(defaultData.length);
			...
			expect(facilityUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			facilityUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = facilityUserDataFactory.createAll(filteredDataSize);
			...
			expect(facilityUserCrudListComponent.facilityUsersCount).toBe(filteredDataSize);
			expect(facilityUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(facilityUserCrudListComponent.pageIndex).toBe(0);
			expect(facilityUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			facilityUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		facilityUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		facilityUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
