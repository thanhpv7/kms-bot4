...
import * as modelAction from 'src/app/models/facilityUser/facility_user.model.action';
import {FacilityUserModelState} from 'src/app/models/facilityUser/facility_user.model.state';
...
import {
	getFacilityUserCollectionModels,
	getFacilityUserCollectionCount
...
import {CrudTileMode} from '../facility-user-admin-crud.component';
...
@Component({
	selector: 'cb-facility-user-admin-crud-list',
	templateUrl: './facility-user-admin-crud-list.component.html',
	styleUrls: [
		'./facility-user-admin-crud-list.component.scss',
...
export class AdminFacilityUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<FacilityUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...FacilityUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = FacilityUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '80cebab1-0b18-41c9-8354-9b544d4e8ad4';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.facilityUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.facilityUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: FacilityUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FacilityUserAction(
				modelAction.FacilityUserModelActionTypes.INITIALISE_FACILITY_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FacilityUserAction(
				modelAction.FacilityUserModelActionTypes.FETCH_FACILITY_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.FacilityUserAction(
						modelAction.FacilityUserModelActionTypes.COUNT_FACILITY_USERS,
						null
					)
				]
			))
		...
		this.facilityUsers = this.store.select(getFacilityUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getFacilityUserCollectionCount, this.collectionId).subscribe(
				(count) => this.facilityUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: FacilityUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.FacilityUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.FacilityUserModelActionTypes.EXPORT_ALL_FACILITY_USER;
		} else {
			actionType = modelAction.FacilityUserModelActionTypes.EXPORT_FACILITY_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.FacilityUserAction(
				modelAction.FacilityUserModelActionTypes.IMPORT_FACILITY_USERS,
				{
					file: file,
				},
				[new modelAction.FacilityUserAction(
			...
			window.open(environment.API_URL + '/api/facilityuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: FacilityUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Facility User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Facility User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.FacilityUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.FacilityUserModelActionTypes.DELETE_FACILITY_USER;
						} else if (usingSelected) {
							actionType = modelAction.FacilityUserModelActionTypes.DELETE_ALL_FACILITY_USER;
						} else {
							actionType = modelAction.FacilityUserModelActionTypes.DELETE_FACILITY_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.FacilityUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_FACILITY_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_FACILITY_USER and DELETE_FACILITY_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<FacilityUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = FacilityUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.facilityUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
