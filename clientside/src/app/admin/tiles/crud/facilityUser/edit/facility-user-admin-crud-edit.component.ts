...
import {FacilityUserModelState, FacilityUserModelAudit} from 'src/app/models/facilityUser/facility_user.model.state';
import {FacilityUserModel} from 'src/app/models/facilityUser/facility_user.model';
import {
	getFacilityUserModelAuditsByEntityId,
	getFacilityUserModelWithId,
	getCountFacilityUserModels,
...
@Component({
	selector: 'cb-facility-user-admin-crud-edit',
	templateUrl: './facility-user-admin-crud-edit.component.html',
	styleUrls: [
		'./facility-user-admin-crud-edit.component.scss',
...
export class AdminFacilityUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	facilityUserAudits$: Observable<FacilityUserModelAudit[]>;
	...
	staffOppositeRelation = 'facilityUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = FacilityUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountFacilityUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new FacilityUserModel();
		...
		if (this.targetModelId) {
			this.facilityUserAudits$ = this.store.select(getFacilityUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<FacilityUserModel> = {
		...
		this.store.dispatch(new modelAction.FacilityUserAction(
			modelAction.FacilityUserModelActionTypes.FETCH_FACILITY_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getFacilityUserModelWithId, this.targetModelId)
			...
			model = new FacilityUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.FacilityUserAction(
			modelAction.FacilityUserModelActionTypes.FETCH_FACILITY_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.FacilityUserAction(
				modelAction.FacilityUserModelActionTypes.CREATE_FACILITY_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.FacilityUserAction(
				modelAction.FacilityUserModelActionTypes.UPDATE_FACILITY_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: FacilityUserModel = new FacilityUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getFacilityUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new FacilityUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = FacilityUserModel.getProps();
		this.modelRelations = FacilityUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: FacilityUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): FacilityUserModel {
		let updatedModel: FacilityUserModel = new FacilityUserModel(model);
