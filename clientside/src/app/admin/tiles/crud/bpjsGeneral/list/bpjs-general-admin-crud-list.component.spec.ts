...
import {AdminBpjsGeneralCrudListComponent} from './bpjs-general-admin-crud-list.component';
...
import {AdminBpjsGeneralCrudModule} from '../bpjs-general-admin-crud.module';
import {
 ...
 */
describe('BPJS General Crud List Tile Component', () => {

	let fixture;
	let bpjsGeneralCrudListComponent: AdminBpjsGeneralCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsGeneralModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '0645a56d-ebde-4a98-b207-dba1013eb52e';
	const searchContent = 'text';

	const searchableFieldCount = BpjsGeneralModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-general-admin-crud',
		urls: ['bpjs-general-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsGeneralDataFactory = new BpjsGeneralDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsGeneralDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsGeneralCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsGeneralCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsGeneralCrudListComponent);
			bpjsGeneralCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsGeneralCrudListComponent.bpjsGeneralsCount).toBe(defaultData.length);
			...
			expect(bpjsGeneralCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsGeneralCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsGeneralDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsGeneralCrudListComponent.bpjsGeneralsCount).toBe(filteredDataSize);
			expect(bpjsGeneralCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsGeneralCrudListComponent.pageIndex).toBe(0);
			expect(bpjsGeneralCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsGeneralCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsGeneralCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsGeneralCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
