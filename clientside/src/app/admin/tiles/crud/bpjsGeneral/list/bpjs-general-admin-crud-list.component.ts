...
import * as modelAction from 'src/app/models/bpjsGeneral/bpjs_general.model.action';
import {BpjsGeneralModelState} from 'src/app/models/bpjsGeneral/bpjs_general.model.state';
...
import {CrudTileMode} from '../bpjs-general-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-general-admin-crud-list',
	templateUrl: './bpjs-general-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-general-admin-crud-list.component.scss',
...
export class AdminBpjsGeneralCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsGeneralModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsGeneralModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsGeneralModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '0645a56d-ebde-4a98-b207-dba1013eb52e';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsGeneralsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsGeneralsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsGeneralModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsGeneralAction(
				modelAction.BpjsGeneralModelActionTypes.INITIALISE_BPJS_GENERAL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsGeneralAction(
				modelAction.BpjsGeneralModelActionTypes.FETCH_BPJS_GENERAL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsGeneralAction(
						modelAction.BpjsGeneralModelActionTypes.COUNT_BPJS_GENERALS,
						null
					)
				]
			))
		...
		this.bpjsGenerals = this.store.select(getBpjsGeneralCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsGeneralCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsGeneralsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsGeneralModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsGeneralModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsGeneralModelActionTypes.EXPORT_ALL_BPJS_GENERAL;
		} else {
			actionType = modelAction.BpjsGeneralModelActionTypes.EXPORT_BPJS_GENERAL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsGeneralAction(
				modelAction.BpjsGeneralModelActionTypes.IMPORT_BPJS_GENERALS,
				{
					file: file,
				},
				[new modelAction.BpjsGeneralAction(
			...
			window.open(environment.API_URL + '/api/bpjsgeneral/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsGeneralModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS General entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS General entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsGeneralModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsGeneralModelActionTypes.DELETE_BPJS_GENERAL;
						} else if (usingSelected) {
							actionType = modelAction.BpjsGeneralModelActionTypes.DELETE_ALL_BPJS_GENERAL;
						} else {
							actionType = modelAction.BpjsGeneralModelActionTypes.DELETE_BPJS_GENERAL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsGeneralAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_GENERAL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_GENERAL and DELETE_BPJS_GENERAL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsGeneralModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsGeneralModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsGeneralsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
