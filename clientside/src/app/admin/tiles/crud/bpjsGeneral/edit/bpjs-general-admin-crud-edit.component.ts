...
import {BpjsGeneralModelState, BpjsGeneralModelAudit} from 'src/app/models/bpjsGeneral/bpjs_general.model.state';
import {BpjsGeneralModel} from 'src/app/models/bpjsGeneral/bpjs_general.model';
import {
	getBpjsGeneralModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-general-admin-crud-edit',
	templateUrl: './bpjs-general-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-general-admin-crud-edit.component.scss',
...
export class AdminBpjsGeneralCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsGeneralAudits$: Observable<BpjsGeneralModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsGeneralModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsGeneralModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsGeneralModel();
		...
		if (this.targetModelId) {
			this.bpjsGeneralAudits$ = this.store.select(getBpjsGeneralModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsGeneralModel> = {
		...
		this.store.dispatch(new modelAction.BpjsGeneralAction(
			modelAction.BpjsGeneralModelActionTypes.FETCH_BPJS_GENERAL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsGeneralModelWithId, this.targetModelId)
			...
			model = new BpjsGeneralModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsGeneralAction(
			modelAction.BpjsGeneralModelActionTypes.FETCH_BPJS_GENERAL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsGeneralAction(
				modelAction.BpjsGeneralModelActionTypes.CREATE_BPJS_GENERAL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsGeneralAction(
				modelAction.BpjsGeneralModelActionTypes.UPDATE_BPJS_GENERAL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsGeneralModel = new BpjsGeneralModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsGeneralModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsGeneralModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsGeneralModel.getProps();
		this.modelRelations = BpjsGeneralModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsGeneralModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsGeneralModel {
		let updatedModel: BpjsGeneralModel = new BpjsGeneralModel(model);
