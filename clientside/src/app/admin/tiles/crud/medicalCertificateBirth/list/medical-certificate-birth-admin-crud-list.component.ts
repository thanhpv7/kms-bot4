...
import {MedicalCertificateBirthModelState} from 'src/app/models/medicalCertificateBirth/medical_certificate_birth.model.state';
...
import {
	getMedicalCertificateBirthCollectionModels,
	getMedicalCertificateBirthCollectionCount
...
@Component({
	selector: 'cb-medical-certificate-birth-admin-crud-list',
	templateUrl: './medical-certificate-birth-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-birth-admin-crud-list.component.scss',
...
export class AdminMedicalCertificateBirthCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificateBirthModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificateBirthModel.searchFields,
	...
	/**
	 * List of Medical Certificate Births that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificateBirths: Observable<MedicalCertificateBirthModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificateBirthsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificateBirthsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificateBirthsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
				modelAction.MedicalCertificateBirthModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_BIRTH_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
				modelAction.MedicalCertificateBirthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_BIRTH_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificateBirths = this.store.select(getMedicalCertificateBirthCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificateBirthCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificateBirthsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificateBirthModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificateBirthModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificateBirthModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_BIRTH;
		} else {
			actionType = modelAction.MedicalCertificateBirthModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_BIRTH_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
				modelAction.MedicalCertificateBirthModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_BIRTHS,
				{
					file: file,
				},
				[new modelAction.MedicalCertificateBirthAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificatebirth/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificateBirthModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Birth entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Birth entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificateBirthModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificateBirthModelActionTypes.DELETE_MEDICAL_CERTIFICATE_BIRTH;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificateBirthModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_BIRTH;
						} else {
							actionType = modelAction.MedicalCertificateBirthModelActionTypes.DELETE_MEDICAL_CERTIFICATE_BIRTH_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_BIRTH when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_BIRTH and DELETE_MEDICAL_CERTIFICATE_BIRTH_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificateBirthModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificateBirthsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
