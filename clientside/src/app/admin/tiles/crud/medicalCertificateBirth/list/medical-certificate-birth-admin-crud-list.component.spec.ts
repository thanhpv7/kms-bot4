...
import {AdminMedicalCertificateBirthCrudListComponent} from './medical-certificate-birth-admin-crud-list.component';
...
import {AdminMedicalCertificateBirthCrudModule} from '../medical-certificate-birth-admin-crud.module';
import {
	MedicalCertificateBirthModelState,
	initialState as initialMedicalCertificateBirthModelState
} from 'src/app/models/medicalCertificateBirth/medical_certificate_birth.model.state';
 ...
 */
describe('Medical Certificate Birth Crud List Tile Component', () => {

	let fixture;
	let medicalCertificateBirthCrudListComponent: AdminMedicalCertificateBirthCrudListComponent;

	let store: MockStore<{ model: MedicalCertificateBirthModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalCertificateBirthModelState: MedicalCertificateBirthModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalCertificateBirthModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8b68bd3f-45d7-4551-80e5-c6e6cad45b98';
	const searchContent = 'text';

	const searchableFieldCount = MedicalCertificateBirthModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-certificate-birth-admin-crud',
		urls: ['medical-certificate-birth-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalCertificateBirthDataFactory = new MedicalCertificateBirthDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalCertificateBirthDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalCertificateBirthModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalCertificateBirthModelState = initialMedicalCertificateBirthModelState;
		store.setState({model: medicalCertificateBirthModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalCertificateBirthCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalCertificateBirthCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalCertificateBirthCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalCertificateBirthCrudListComponent);
			medicalCertificateBirthCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalCertificateBirthCrudListComponent.medicalCertificateBirthsCount).toBe(defaultData.length);
			...
			expect(medicalCertificateBirthCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalCertificateBirthCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalCertificateBirthDataFactory.createAll(filteredDataSize);
			...
			expect(medicalCertificateBirthCrudListComponent.medicalCertificateBirthsCount).toBe(filteredDataSize);
			expect(medicalCertificateBirthCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalCertificateBirthCrudListComponent.pageIndex).toBe(0);
			expect(medicalCertificateBirthCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalCertificateBirthCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalCertificateBirthCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalCertificateBirthCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
