...
import * as modelAction from 'src/app/models/medicalCertificateBirth/medical_certificate_birth.model.action';
import {MedicalCertificateBirthModelState, MedicalCertificateBirthModelAudit} from 'src/app/models/medicalCertificateBirth/medical_certificate_birth.model.state';
import {MedicalCertificateBirthModel} from 'src/app/models/medicalCertificateBirth/medical_certificate_birth.model';
import {
	getMedicalCertificateBirthModelAuditsByEntityId,
	getMedicalCertificateBirthModelWithId,
	getCountMedicalCertificateBirthModels,
...
import {CrudTileMode} from '../medical-certificate-birth-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-birth-admin-crud-edit',
	templateUrl: './medical-certificate-birth-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-birth-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificateBirthCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '8b68bd3f-45d7-4551-80e5-c6e6cad45b98';
	 ...
	 */
	medicalCertificateBirthAudits$: Observable<MedicalCertificateBirthModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificateBirth';
	...
	medicalStaffOppositeRelation = 'medicalCertificateBirths';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificateBirthModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificateBirthModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificateBirthModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalCertificateBirthModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificateBirthModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificateBirthModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificateBirthModel();
		...
		if (this.targetModelId) {
			this.medicalCertificateBirthAudits$ = this.store.select(getMedicalCertificateBirthModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificateBirthModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
			modelAction.MedicalCertificateBirthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_BIRTH,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateBirthModelWithId, this.targetModelId)
			...
			model = new MedicalCertificateBirthModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-birth');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
			modelAction.MedicalCertificateBirthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_BIRTH_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificateBirthModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificateBirthAction(
						modelAction.MedicalCertificateBirthModelActionTypes.COUNT_MEDICAL_CERTIFICATE_BIRTHS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
				modelAction.MedicalCertificateBirthModelActionTypes.CREATE_MEDICAL_CERTIFICATE_BIRTH,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificateBirthAction(
				modelAction.MedicalCertificateBirthModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_BIRTH,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificateBirthModel = new MedicalCertificateBirthModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateBirthModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificateBirthModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificateBirthModel.getProps();
		this.modelRelations = MedicalCertificateBirthModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificateBirthModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificateBirthModel {
		let updatedModel: MedicalCertificateBirthModel = new MedicalCertificateBirthModel(model);
