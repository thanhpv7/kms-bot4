...
import {CashRefundModelState, CashRefundModelAudit} from 'src/app/models/cashRefund/cash_refund.model.state';
import {CashRefundModel} from 'src/app/models/cashRefund/cash_refund.model';
import {
	getCashRefundModelAuditsByEntityId,
...
@Component({
	selector: 'cb-cash-refund-admin-crud-edit',
	templateUrl: './cash-refund-admin-crud-edit.component.html',
	styleUrls: [
		'./cash-refund-admin-crud-edit.component.scss',
...
export class AdminCashRefundCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	cashRefundAudits$: Observable<CashRefundModelAudit[]>;
	...
	patientGeneralInfoOppositeRelation = 'cashRefunds';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CashRefundModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCashRefundModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CashRefundModel();
		...
		if (this.targetModelId) {
			this.cashRefundAudits$ = this.store.select(getCashRefundModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CashRefundModel> = {
		...
		this.store.dispatch(new modelAction.CashRefundAction(
			modelAction.CashRefundModelActionTypes.FETCH_CASH_REFUND,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCashRefundModelWithId, this.targetModelId)
			...
			model = new CashRefundModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CashRefundAction(
			modelAction.CashRefundModelActionTypes.FETCH_CASH_REFUND_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.CashRefundAction(
				modelAction.CashRefundModelActionTypes.CREATE_CASH_REFUND,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CashRefundAction(
				modelAction.CashRefundModelActionTypes.UPDATE_CASH_REFUND,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CashRefundModel = new CashRefundModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCashRefundModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CashRefundModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CashRefundModel.getProps();
		this.modelRelations = CashRefundModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: CashRefundModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CashRefundModel {
		let updatedModel: CashRefundModel = new CashRefundModel(model);
