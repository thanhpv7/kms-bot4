...
import * as modelAction from 'src/app/models/cashRefund/cash_refund.model.action';
import {CashRefundModelState} from 'src/app/models/cashRefund/cash_refund.model.state';
...
import {CrudTileMode} from '../cash-refund-admin-crud.component';
...
@Component({
	selector: 'cb-cash-refund-admin-crud-list',
	templateUrl: './cash-refund-admin-crud-list.component.html',
	styleUrls: [
		'./cash-refund-admin-crud-list.component.scss',
...
export class AdminCashRefundCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CashRefundModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CashRefundModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = CashRefundModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '07af1994-5923-4582-92f3-8b943ccd70ab';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.cashRefundsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.cashRefundsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: CashRefundModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashRefundAction(
				modelAction.CashRefundModelActionTypes.INITIALISE_CASH_REFUND_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashRefundAction(
				modelAction.CashRefundModelActionTypes.FETCH_CASH_REFUND_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.CashRefundAction(
						modelAction.CashRefundModelActionTypes.COUNT_CASH_REFUNDS,
						null
					)
				]
			))
		...
		this.cashRefunds = this.store.select(getCashRefundCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCashRefundCollectionCount, this.collectionId).subscribe(
				(count) => this.cashRefundsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CashRefundModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CashRefundModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CashRefundModelActionTypes.EXPORT_ALL_CASH_REFUND;
		} else {
			actionType = modelAction.CashRefundModelActionTypes.EXPORT_CASH_REFUND_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CashRefundAction(
				modelAction.CashRefundModelActionTypes.IMPORT_CASH_REFUNDS,
				{
					file: file,
				},
				[new modelAction.CashRefundAction(
			...
			window.open(environment.API_URL + '/api/cashrefund/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CashRefundModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Cash Refund entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Cash Refund entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CashRefundModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CashRefundModelActionTypes.DELETE_CASH_REFUND;
						} else if (usingSelected) {
							actionType = modelAction.CashRefundModelActionTypes.DELETE_ALL_CASH_REFUND;
						} else {
							actionType = modelAction.CashRefundModelActionTypes.DELETE_CASH_REFUND_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CashRefundAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CASH_REFUND when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CASH_REFUND and DELETE_CASH_REFUND_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<CashRefundModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CashRefundModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.cashRefundsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
