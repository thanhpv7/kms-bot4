...
import {AdminCashRefundCrudListComponent} from './cash-refund-admin-crud-list.component';
...
import {AdminCashRefundCrudModule} from '../cash-refund-admin-crud.module';
import {
 ...
 */
describe('Cash Refund Crud List Tile Component', () => {

	let fixture;
	let cashRefundCrudListComponent: AdminCashRefundCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<CashRefundModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '07af1994-5923-4582-92f3-8b943ccd70ab';
	const searchContent = 'text';

	const searchableFieldCount = CashRefundModel.searchFields.length;

	const routerState: RouterState = {
		url: 'cash-refund-admin-crud',
		urls: ['cash-refund-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const cashRefundDataFactory = new CashRefundDataFactory();

	const defaultDataSize = 30;
	const defaultData = cashRefundDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getCashRefundCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCashRefundCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminCashRefundCrudListComponent);
			cashRefundCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(cashRefundCrudListComponent.cashRefundsCount).toBe(defaultData.length);
			...
			expect(cashRefundCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			cashRefundCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = cashRefundDataFactory.createAll(filteredDataSize);
			...
			expect(cashRefundCrudListComponent.cashRefundsCount).toBe(filteredDataSize);
			expect(cashRefundCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(cashRefundCrudListComponent.pageIndex).toBe(0);
			expect(cashRefundCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			cashRefundCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		cashRefundCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		cashRefundCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
