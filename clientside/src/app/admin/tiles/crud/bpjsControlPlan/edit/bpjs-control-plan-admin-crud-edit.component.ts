...
import {BpjsControlPlanModelState, BpjsControlPlanModelAudit} from 'src/app/models/bpjsControlPlan/bpjs_control_plan.model.state';
import {BpjsControlPlanModel} from 'src/app/models/bpjsControlPlan/bpjs_control_plan.model';
import {
	getBpjsControlPlanModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-control-plan-admin-crud-edit',
	templateUrl: './bpjs-control-plan-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-control-plan-admin-crud-edit.component.scss',
...
export class AdminBpjsControlPlanCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsControlPlanAudits$: Observable<BpjsControlPlanModelAudit[]>;
	...
	bpjsDoctorMappingOppositeRelation = 'bpjsControlPlans';
	...
	bpjsServiceMappingOppositeRelation = 'bpjsControlPlans';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsControlPlanModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsControlPlanModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsControlPlanModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsControlPlanModel();
		...
		if (this.targetModelId) {
			this.bpjsControlPlanAudits$ = this.store.select(getBpjsControlPlanModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsControlPlanModel> = {
		...
		this.store.dispatch(new modelAction.BpjsControlPlanAction(
			modelAction.BpjsControlPlanModelActionTypes.FETCH_BPJS_CONTROL_PLAN,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsControlPlanModelWithId, this.targetModelId)
			...
			model = new BpjsControlPlanModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsControlPlanAction(
			modelAction.BpjsControlPlanModelActionTypes.FETCH_BPJS_CONTROL_PLAN_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsControlPlanModel> = {
				...
				queryParams: {
					persistRelations: this.bpjsDoctorMappingMultiCrudActive || this.bpjsServiceMappingMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsControlPlanAction(
				modelAction.BpjsControlPlanModelActionTypes.CREATE_BPJS_CONTROL_PLAN,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsControlPlanAction(
				modelAction.BpjsControlPlanModelActionTypes.UPDATE_BPJS_CONTROL_PLAN,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsControlPlanModel = new BpjsControlPlanModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsControlPlanModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsControlPlanModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsControlPlanModel.getProps();
		this.modelRelations = BpjsControlPlanModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsControlPlanModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsControlPlanModel {
		let updatedModel: BpjsControlPlanModel = new BpjsControlPlanModel(model);
