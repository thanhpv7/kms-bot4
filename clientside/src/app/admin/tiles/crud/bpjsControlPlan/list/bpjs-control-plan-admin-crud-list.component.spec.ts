...
import {AdminBpjsControlPlanCrudListComponent} from './bpjs-control-plan-admin-crud-list.component';
...
import {AdminBpjsControlPlanCrudModule} from '../bpjs-control-plan-admin-crud.module';
import {
 ...
 */
describe('BPJS Control Plan Crud List Tile Component', () => {

	let fixture;
	let bpjsControlPlanCrudListComponent: AdminBpjsControlPlanCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsControlPlanModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6b78705c-3e4f-4755-bed0-c98b05f02ff2';
	const searchContent = 'text';

	const searchableFieldCount = BpjsControlPlanModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-control-plan-admin-crud',
		urls: ['bpjs-control-plan-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsControlPlanDataFactory = new BpjsControlPlanDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsControlPlanDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsControlPlanCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsControlPlanCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsControlPlanCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsControlPlanCrudListComponent);
			bpjsControlPlanCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsControlPlanCrudListComponent.bpjsControlPlansCount).toBe(defaultData.length);
			...
			expect(bpjsControlPlanCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsControlPlanCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsControlPlanDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsControlPlanCrudListComponent.bpjsControlPlansCount).toBe(filteredDataSize);
			expect(bpjsControlPlanCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsControlPlanCrudListComponent.pageIndex).toBe(0);
			expect(bpjsControlPlanCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsControlPlanCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsControlPlanCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsControlPlanCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
