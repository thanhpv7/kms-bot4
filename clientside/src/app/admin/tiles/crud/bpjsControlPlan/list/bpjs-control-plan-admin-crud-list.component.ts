...
import * as modelAction from 'src/app/models/bpjsControlPlan/bpjs_control_plan.model.action';
import {BpjsControlPlanModelState} from 'src/app/models/bpjsControlPlan/bpjs_control_plan.model.state';
...
import {
	...
	getBpjsControlPlanCollectionCount
...
import {CrudTileMode} from '../bpjs-control-plan-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-control-plan-admin-crud-list',
	templateUrl: './bpjs-control-plan-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-control-plan-admin-crud-list.component.scss',
...
export class AdminBpjsControlPlanCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsControlPlanModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsControlPlanModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsControlPlanModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '6b78705c-3e4f-4755-bed0-c98b05f02ff2';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsControlPlansCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsControlPlansCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsControlPlanModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsControlPlanAction(
				modelAction.BpjsControlPlanModelActionTypes.INITIALISE_BPJS_CONTROL_PLAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsControlPlanAction(
				modelAction.BpjsControlPlanModelActionTypes.FETCH_BPJS_CONTROL_PLAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsControlPlanAction(
						modelAction.BpjsControlPlanModelActionTypes.COUNT_BPJS_CONTROL_PLANS,
						null
					)
				]
			))
		...
		this.bpjsControlPlans = this.store.select(getBpjsControlPlanCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsControlPlanCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsControlPlansCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-control-plan');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsControlPlanModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsControlPlanModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsControlPlanModelActionTypes.EXPORT_ALL_BPJS_CONTROL_PLAN;
		} else {
			actionType = modelAction.BpjsControlPlanModelActionTypes.EXPORT_BPJS_CONTROL_PLAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsControlPlanAction(
				modelAction.BpjsControlPlanModelActionTypes.IMPORT_BPJS_CONTROL_PLANS,
				{
					file: file,
				},
				[new modelAction.BpjsControlPlanAction(
			...
			window.open(environment.API_URL + '/api/bpjscontrolplan/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsControlPlanModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Control Plan entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Control Plan entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsControlPlanModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsControlPlanModelActionTypes.DELETE_BPJS_CONTROL_PLAN;
						} else if (usingSelected) {
							actionType = modelAction.BpjsControlPlanModelActionTypes.DELETE_ALL_BPJS_CONTROL_PLAN;
						} else {
							actionType = modelAction.BpjsControlPlanModelActionTypes.DELETE_BPJS_CONTROL_PLAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsControlPlanAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_CONTROL_PLAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_CONTROL_PLAN and DELETE_BPJS_CONTROL_PLAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsControlPlanModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsControlPlanModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsControlPlansCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
