...
import {BpjsDismissalConditionModelState} from 'src/app/models/bpjsDismissalCondition/bpjs_dismissal_condition.model.state';
...
@Component({
	selector: 'cb-bpjs-dismissal-condition-admin-crud-list',
	templateUrl: './bpjs-dismissal-condition-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-dismissal-condition-admin-crud-list.component.scss',
...
export class AdminBpjsDismissalConditionCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsDismissalConditionModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsDismissalConditionModel.searchFields,
	...
	/**
	 * List of BPJS Dismissal Conditions that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsDismissalConditions: Observable<BpjsDismissalConditionModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsDismissalConditionsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsDismissalConditionsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsDismissalConditionsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
				modelAction.BpjsDismissalConditionModelActionTypes.INITIALISE_BPJS_DISMISSAL_CONDITION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
				modelAction.BpjsDismissalConditionModelActionTypes.FETCH_BPJS_DISMISSAL_CONDITION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsDismissalConditions = this.store.select(getBpjsDismissalConditionCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsDismissalConditionCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsDismissalConditionsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsDismissalConditionModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsDismissalConditionModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsDismissalConditionModelActionTypes.EXPORT_ALL_BPJS_DISMISSAL_CONDITION;
		} else {
			actionType = modelAction.BpjsDismissalConditionModelActionTypes.EXPORT_BPJS_DISMISSAL_CONDITION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
				modelAction.BpjsDismissalConditionModelActionTypes.IMPORT_BPJS_DISMISSAL_CONDITIONS,
				{
					file: file,
				},
				[new modelAction.BpjsDismissalConditionAction(
			...
			window.open(environment.API_URL + '/api/bpjsdismissalcondition/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsDismissalConditionModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Dismissal Condition entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Dismissal Condition entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsDismissalConditionModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsDismissalConditionModelActionTypes.DELETE_BPJS_DISMISSAL_CONDITION;
						} else if (usingSelected) {
							actionType = modelAction.BpjsDismissalConditionModelActionTypes.DELETE_ALL_BPJS_DISMISSAL_CONDITION;
						} else {
							actionType = modelAction.BpjsDismissalConditionModelActionTypes.DELETE_BPJS_DISMISSAL_CONDITION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_DISMISSAL_CONDITION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_DISMISSAL_CONDITION and DELETE_BPJS_DISMISSAL_CONDITION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsDismissalConditionModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsDismissalConditionsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
