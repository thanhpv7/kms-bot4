...
import * as modelAction from 'src/app/models/bpjsDismissalCondition/bpjs_dismissal_condition.model.action';
import {BpjsDismissalConditionModelState, BpjsDismissalConditionModelAudit} from 'src/app/models/bpjsDismissalCondition/bpjs_dismissal_condition.model.state';
import {BpjsDismissalConditionModel} from 'src/app/models/bpjsDismissalCondition/bpjs_dismissal_condition.model';
import {
	getBpjsDismissalConditionModelAuditsByEntityId,
	getBpjsDismissalConditionModelWithId,
	getCountBpjsDismissalConditionModels,
...
import {CrudTileMode} from '../bpjs-dismissal-condition-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-dismissal-condition-admin-crud-edit',
	templateUrl: './bpjs-dismissal-condition-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-dismissal-condition-admin-crud-edit.component.scss',
...
export class AdminBpjsDismissalConditionCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '10a228cc-5fa3-4390-a68b-9a23e1c73ab1';
	 ...
	 */
	bpjsDismissalConditionAudits$: Observable<BpjsDismissalConditionModelAudit[]>;
	...
	bpjsClaimSubmissionMultiCrudActive: boolean = false;
	bpjsClaimSubmissionDisplayName = 'BPJS Claim Submission';
	...
	bpjsClaimSubmissionOppositeRelation = 'bpjsDismissalConditions';
	bpjsClaimSubmissionRelations: ModelRelation[] = this.bpjsClaimSubmissionMultiCrudActive ? BpjsClaimSubmissionModel.getRelations()
		.map(relation => {
			if (relation.id === this.bpjsClaimSubmissionOppositeRelation) {
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsDismissalConditionModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsDismissalConditionModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsDismissalConditionModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'bpjsClaimSubmission' && this.bpjsClaimSubmissionMultiCrudActive) {
	 ...
	 */
	targetModel: BpjsDismissalConditionModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsDismissalConditionModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsDismissalConditionModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsDismissalConditionModel();
		...
		if (this.targetModelId) {
			this.bpjsDismissalConditionAudits$ = this.store.select(getBpjsDismissalConditionModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsDismissalConditionModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
			modelAction.BpjsDismissalConditionModelActionTypes.FETCH_BPJS_DISMISSAL_CONDITION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsDismissalConditionModelWithId, this.targetModelId)
			...
			model = new BpjsDismissalConditionModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-dismissal-condition');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
			modelAction.BpjsDismissalConditionModelActionTypes.FETCH_BPJS_DISMISSAL_CONDITION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsDismissalConditionModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.bpjsClaimSubmissionMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsDismissalConditionAction(
						modelAction.BpjsDismissalConditionModelActionTypes.COUNT_BPJS_DISMISSAL_CONDITIONS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
				modelAction.BpjsDismissalConditionModelActionTypes.CREATE_BPJS_DISMISSAL_CONDITION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsDismissalConditionAction(
				modelAction.BpjsDismissalConditionModelActionTypes.UPDATE_BPJS_DISMISSAL_CONDITION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsDismissalConditionModel = new BpjsDismissalConditionModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsDismissalConditionModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsDismissalConditionModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.bpjsClaimSubmissionDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsClaimSubmissionModel.getProps(), this.bpjsClaimSubmissionRelations, this.isViewOnly),
					new BpjsClaimSubmissionModel(),
					BpjsClaimSubmissionModel,
					this.bpjsClaimSubmissionDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.bpjsClaimSubmissionRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsDismissalConditionModel.getProps();
		this.modelRelations = BpjsDismissalConditionModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'bpjsClaimSubmission';
		}).hideElement = this.bpjsClaimSubmissionMultiCrudActive || this.modelRelations.find(rel => rel.id === 'bpjsClaimSubmission').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.bpjsClaimSubmissionMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.bpjsClaimSubmissionDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.bpjsClaimSubmission && this.targetModel.bpjsClaimSubmission.id && this.bpjsClaimSubmissionMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsClaimSubmissionModel,
				this.bpjsClaimSubmissionOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.bpjsClaimSubmission.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsClaimSubmissionModelWithId, this.targetModel.bpjsClaimSubmissionId).subscribe(bpjsClaimSubmission => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsClaimSubmissionModel.getProps(), this.bpjsClaimSubmissionRelations, this.isViewOnly),
					bpjsClaimSubmission,
					BpjsClaimSubmissionModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsDismissalConditionModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsDismissalConditionModel {
		let updatedModel: BpjsDismissalConditionModel = new BpjsDismissalConditionModel(model);

		if (this.bpjsClaimSubmissionMultiCrudActive) {
			updatedModel.bpjsClaimSubmission = event.additionalEntities.bpjsClaimSubmission ? event.additionalEntities.bpjsClaimSubmission : undefined;

			if (!updatedModel.bpjsClaimSubmission && event.payload) {
				event.payload.bpjsClaimSubmissionId = null;
			}
		} else if (event.payload?.bpjsClaimSubmissionId !== undefined) {
			event.payload.bpjsClaimSubmission = null;
			updatedModel.bpjsClaimSubmission = null;
		}
