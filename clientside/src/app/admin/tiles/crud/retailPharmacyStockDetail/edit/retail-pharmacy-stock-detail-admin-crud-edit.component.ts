...
import {RetailPharmacyStockDetailModelState, RetailPharmacyStockDetailModelAudit} from 'src/app/models/retailPharmacyStockDetail/retail_pharmacy_stock_detail.model.state';
import {RetailPharmacyStockDetailModel} from 'src/app/models/retailPharmacyStockDetail/retail_pharmacy_stock_detail.model';
import {
	getRetailPharmacyStockDetailModelAuditsByEntityId,
	getRetailPharmacyStockDetailModelWithId,
	getCountRetailPharmacyStockDetailModels,
...
@Component({
	selector: 'cb-retail-pharmacy-stock-detail-admin-crud-edit',
	templateUrl: './retail-pharmacy-stock-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./retail-pharmacy-stock-detail-admin-crud-edit.component.scss',
...
export class AdminRetailPharmacyStockDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	retailPharmacyStockDetailAudits$: Observable<RetailPharmacyStockDetailModelAudit[]>;
	...
	retailPharmacyOppositeRelation = 'retailPharmacyStockDetails';
	...
	stockCatalogueOppositeRelation = 'retailPharmacyStockDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = RetailPharmacyStockDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: RetailPharmacyStockDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountRetailPharmacyStockDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new RetailPharmacyStockDetailModel();
		...
		if (this.targetModelId) {
			this.retailPharmacyStockDetailAudits$ = this.store.select(getRetailPharmacyStockDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<RetailPharmacyStockDetailModel> = {
		...
		this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
			modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_RETAIL_PHARMACY_STOCK_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getRetailPharmacyStockDetailModelWithId, this.targetModelId)
			...
			model = new RetailPharmacyStockDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
			modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_RETAIL_PHARMACY_STOCK_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<RetailPharmacyStockDetailModel> = {
				...
				queryParams: {
					persistRelations: this.retailPharmacyMultiCrudActive || this.stockCatalogueMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
				modelAction.RetailPharmacyStockDetailModelActionTypes.CREATE_RETAIL_PHARMACY_STOCK_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
				modelAction.RetailPharmacyStockDetailModelActionTypes.UPDATE_RETAIL_PHARMACY_STOCK_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: RetailPharmacyStockDetailModel = new RetailPharmacyStockDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getRetailPharmacyStockDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new RetailPharmacyStockDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = RetailPharmacyStockDetailModel.getProps();
		this.modelRelations = RetailPharmacyStockDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: RetailPharmacyStockDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): RetailPharmacyStockDetailModel {
		let updatedModel: RetailPharmacyStockDetailModel = new RetailPharmacyStockDetailModel(model);
