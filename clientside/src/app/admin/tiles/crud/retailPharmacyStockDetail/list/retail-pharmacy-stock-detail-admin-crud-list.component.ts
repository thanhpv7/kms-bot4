...
import * as modelAction from 'src/app/models/retailPharmacyStockDetail/retail_pharmacy_stock_detail.model.action';
import {RetailPharmacyStockDetailModelState} from 'src/app/models/retailPharmacyStockDetail/retail_pharmacy_stock_detail.model.state';
...
import {
	getRetailPharmacyStockDetailCollectionModels,
	getRetailPharmacyStockDetailCollectionCount
...
import {CrudTileMode} from '../retail-pharmacy-stock-detail-admin-crud.component';
...
@Component({
	selector: 'cb-retail-pharmacy-stock-detail-admin-crud-list',
	templateUrl: './retail-pharmacy-stock-detail-admin-crud-list.component.html',
	styleUrls: [
		'./retail-pharmacy-stock-detail-admin-crud-list.component.scss',
...
export class AdminRetailPharmacyStockDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<RetailPharmacyStockDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...RetailPharmacyStockDetailModel.searchFields,
	...
	/**
	 * List of Retail Pharmacy Stock Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	retailPharmacyStockDetails: Observable<RetailPharmacyStockDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	retailPharmacyStockDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = RetailPharmacyStockDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = RetailPharmacyStockDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '252a4124-add9-42c2-a43f-0dbd5d3cc755';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.retailPharmacyStockDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.retailPharmacyStockDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: RetailPharmacyStockDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
				modelAction.RetailPharmacyStockDetailModelActionTypes.INITIALISE_RETAIL_PHARMACY_STOCK_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
				modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_RETAIL_PHARMACY_STOCK_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.RetailPharmacyStockDetailAction(
						modelAction.RetailPharmacyStockDetailModelActionTypes.COUNT_RETAIL_PHARMACY_STOCK_DETAILS,
						null
					)
				]
			))
		...
		this.retailPharmacyStockDetails = this.store.select(getRetailPharmacyStockDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getRetailPharmacyStockDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.retailPharmacyStockDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('retail-pharmacy-stock-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: RetailPharmacyStockDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.RetailPharmacyStockDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.RetailPharmacyStockDetailModelActionTypes.EXPORT_ALL_RETAIL_PHARMACY_STOCK_DETAIL;
		} else {
			actionType = modelAction.RetailPharmacyStockDetailModelActionTypes.EXPORT_RETAIL_PHARMACY_STOCK_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
				modelAction.RetailPharmacyStockDetailModelActionTypes.IMPORT_RETAIL_PHARMACY_STOCK_DETAILS,
				{
					file: file,
				},
				[new modelAction.RetailPharmacyStockDetailAction(
			...
			window.open(environment.API_URL + '/api/retailpharmacystockdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: RetailPharmacyStockDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Retail Pharmacy Stock Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Retail Pharmacy Stock Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.RetailPharmacyStockDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.RetailPharmacyStockDetailModelActionTypes.DELETE_RETAIL_PHARMACY_STOCK_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.RetailPharmacyStockDetailModelActionTypes.DELETE_ALL_RETAIL_PHARMACY_STOCK_DETAIL;
						} else {
							actionType = modelAction.RetailPharmacyStockDetailModelActionTypes.DELETE_RETAIL_PHARMACY_STOCK_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.RetailPharmacyStockDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_RETAIL_PHARMACY_STOCK_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_RETAIL_PHARMACY_STOCK_DETAIL and DELETE_RETAIL_PHARMACY_STOCK_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<RetailPharmacyStockDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = RetailPharmacyStockDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.retailPharmacyStockDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
