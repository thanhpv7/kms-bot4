...
import {AdminRetailPharmacyStockDetailCrudListComponent} from './retail-pharmacy-stock-detail-admin-crud-list.component';
...
import {AdminRetailPharmacyStockDetailCrudModule} from '../retail-pharmacy-stock-detail-admin-crud.module';
import {
	RetailPharmacyStockDetailModelState,
	initialState as initialRetailPharmacyStockDetailModelState
} from 'src/app/models/retailPharmacyStockDetail/retail_pharmacy_stock_detail.model.state';
 ...
 */
describe('Retail Pharmacy Stock Detail Crud List Tile Component', () => {

	let fixture;
	let retailPharmacyStockDetailCrudListComponent: AdminRetailPharmacyStockDetailCrudListComponent;

	let store: MockStore<{ model: RetailPharmacyStockDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let retailPharmacyStockDetailModelState: RetailPharmacyStockDetailModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<RetailPharmacyStockDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '252a4124-add9-42c2-a43f-0dbd5d3cc755';
	const searchContent = 'text';

	const searchableFieldCount = RetailPharmacyStockDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'retail-pharmacy-stock-detail-admin-crud',
		urls: ['retail-pharmacy-stock-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const retailPharmacyStockDetailDataFactory = new RetailPharmacyStockDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = retailPharmacyStockDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: RetailPharmacyStockDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		retailPharmacyStockDetailModelState = initialRetailPharmacyStockDetailModelState;
		store.setState({model: retailPharmacyStockDetailModelState});
		...
		spyOn(store, 'select')
			.withArgs(getRetailPharmacyStockDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getRetailPharmacyStockDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminRetailPharmacyStockDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminRetailPharmacyStockDetailCrudListComponent);
			retailPharmacyStockDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(retailPharmacyStockDetailCrudListComponent.retailPharmacyStockDetailsCount).toBe(defaultData.length);
			...
			expect(retailPharmacyStockDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			retailPharmacyStockDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = retailPharmacyStockDetailDataFactory.createAll(filteredDataSize);
			...
			expect(retailPharmacyStockDetailCrudListComponent.retailPharmacyStockDetailsCount).toBe(filteredDataSize);
			expect(retailPharmacyStockDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(retailPharmacyStockDetailCrudListComponent.pageIndex).toBe(0);
			expect(retailPharmacyStockDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			retailPharmacyStockDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		retailPharmacyStockDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		retailPharmacyStockDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
