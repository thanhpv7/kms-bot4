...
import * as modelAction from 'src/app/models/pharmacyUser/pharmacy_user.model.action';
import {PharmacyUserModelState} from 'src/app/models/pharmacyUser/pharmacy_user.model.state';
...
import {
	getPharmacyUserCollectionModels,
	getPharmacyUserCollectionCount
...
import {CrudTileMode} from '../pharmacy-user-admin-crud.component';
...
@Component({
	selector: 'cb-pharmacy-user-admin-crud-list',
	templateUrl: './pharmacy-user-admin-crud-list.component.html',
	styleUrls: [
		'./pharmacy-user-admin-crud-list.component.scss',
...
export class AdminPharmacyUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PharmacyUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PharmacyUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PharmacyUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'a7a551c7-43ca-4b2d-bf0c-d558f451eb91';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pharmacyUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pharmacyUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PharmacyUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PharmacyUserAction(
				modelAction.PharmacyUserModelActionTypes.INITIALISE_PHARMACY_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PharmacyUserAction(
				modelAction.PharmacyUserModelActionTypes.FETCH_PHARMACY_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PharmacyUserAction(
						modelAction.PharmacyUserModelActionTypes.COUNT_PHARMACY_USERS,
						null
					)
				]
			))
		...
		this.pharmacyUsers = this.store.select(getPharmacyUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPharmacyUserCollectionCount, this.collectionId).subscribe(
				(count) => this.pharmacyUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PharmacyUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PharmacyUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PharmacyUserModelActionTypes.EXPORT_ALL_PHARMACY_USER;
		} else {
			actionType = modelAction.PharmacyUserModelActionTypes.EXPORT_PHARMACY_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PharmacyUserAction(
				modelAction.PharmacyUserModelActionTypes.IMPORT_PHARMACY_USERS,
				{
					file: file,
				},
				[new modelAction.PharmacyUserAction(
			...
			window.open(environment.API_URL + '/api/pharmacyuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PharmacyUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Pharmacy User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Pharmacy User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PharmacyUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PharmacyUserModelActionTypes.DELETE_PHARMACY_USER;
						} else if (usingSelected) {
							actionType = modelAction.PharmacyUserModelActionTypes.DELETE_ALL_PHARMACY_USER;
						} else {
							actionType = modelAction.PharmacyUserModelActionTypes.DELETE_PHARMACY_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PharmacyUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PHARMACY_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PHARMACY_USER and DELETE_PHARMACY_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PharmacyUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PharmacyUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pharmacyUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
