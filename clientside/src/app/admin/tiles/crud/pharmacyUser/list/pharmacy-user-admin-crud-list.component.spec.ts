...
import {AdminPharmacyUserCrudListComponent} from './pharmacy-user-admin-crud-list.component';
...
import {AdminPharmacyUserCrudModule} from '../pharmacy-user-admin-crud.module';
import {
	PharmacyUserModelState,
	initialState as initialPharmacyUserModelState
} from 'src/app/models/pharmacyUser/pharmacy_user.model.state';
 ...
 */
describe('Pharmacy User Crud List Tile Component', () => {

	let fixture;
	let pharmacyUserCrudListComponent: AdminPharmacyUserCrudListComponent;

	let store: MockStore<{ model: PharmacyUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pharmacyUserModelState: PharmacyUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PharmacyUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a7a551c7-43ca-4b2d-bf0c-d558f451eb91';
	const searchContent = 'text';

	const searchableFieldCount = PharmacyUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pharmacy-user-admin-crud',
		urls: ['pharmacy-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pharmacyUserDataFactory = new PharmacyUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = pharmacyUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PharmacyUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pharmacyUserModelState = initialPharmacyUserModelState;
		store.setState({model: pharmacyUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPharmacyUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPharmacyUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPharmacyUserCrudListComponent);
			pharmacyUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pharmacyUserCrudListComponent.pharmacyUsersCount).toBe(defaultData.length);
			...
			expect(pharmacyUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pharmacyUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pharmacyUserDataFactory.createAll(filteredDataSize);
			...
			expect(pharmacyUserCrudListComponent.pharmacyUsersCount).toBe(filteredDataSize);
			expect(pharmacyUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pharmacyUserCrudListComponent.pageIndex).toBe(0);
			expect(pharmacyUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pharmacyUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pharmacyUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pharmacyUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
