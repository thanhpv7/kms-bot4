...
import {PharmacyUserModelState, PharmacyUserModelAudit} from 'src/app/models/pharmacyUser/pharmacy_user.model.state';
import {PharmacyUserModel} from 'src/app/models/pharmacyUser/pharmacy_user.model';
import {
	getPharmacyUserModelAuditsByEntityId,
	getPharmacyUserModelWithId,
	getCountPharmacyUserModels,
...
@Component({
	selector: 'cb-pharmacy-user-admin-crud-edit',
	templateUrl: './pharmacy-user-admin-crud-edit.component.html',
	styleUrls: [
		'./pharmacy-user-admin-crud-edit.component.scss',
...
export class AdminPharmacyUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pharmacyUserAudits$: Observable<PharmacyUserModelAudit[]>;
	...
	staffOppositeRelation = 'pharmacyUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PharmacyUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPharmacyUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PharmacyUserModel();
		...
		if (this.targetModelId) {
			this.pharmacyUserAudits$ = this.store.select(getPharmacyUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PharmacyUserModel> = {
		...
		this.store.dispatch(new modelAction.PharmacyUserAction(
			modelAction.PharmacyUserModelActionTypes.FETCH_PHARMACY_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPharmacyUserModelWithId, this.targetModelId)
			...
			model = new PharmacyUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PharmacyUserAction(
			modelAction.PharmacyUserModelActionTypes.FETCH_PHARMACY_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PharmacyUserAction(
				modelAction.PharmacyUserModelActionTypes.CREATE_PHARMACY_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PharmacyUserAction(
				modelAction.PharmacyUserModelActionTypes.UPDATE_PHARMACY_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PharmacyUserModel = new PharmacyUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPharmacyUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PharmacyUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PharmacyUserModel.getProps();
		this.modelRelations = PharmacyUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PharmacyUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PharmacyUserModel {
		let updatedModel: PharmacyUserModel = new PharmacyUserModel(model);
