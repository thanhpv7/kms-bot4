...
import {PrescriptionHeaderModelState} from 'src/app/models/prescriptionHeader/prescription_header.model.state';
...
import {
	...
	getPrescriptionHeaderCollectionCount
...
@Component({
	selector: 'cb-prescription-header-admin-crud-list',
	templateUrl: './prescription-header-admin-crud-list.component.html',
	styleUrls: [
		'./prescription-header-admin-crud-list.component.scss',
...
export class AdminPrescriptionHeaderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PrescriptionHeaderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PrescriptionHeaderModel.searchFields,
	...
	/**
	 * List of Prescription Headers that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	prescriptionHeaders: Observable<PrescriptionHeaderModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	prescriptionHeadersCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.prescriptionHeadersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.prescriptionHeadersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PrescriptionHeaderAction(
				modelAction.PrescriptionHeaderModelActionTypes.INITIALISE_PRESCRIPTION_HEADER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PrescriptionHeaderAction(
				modelAction.PrescriptionHeaderModelActionTypes.FETCH_PRESCRIPTION_HEADER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.prescriptionHeaders = this.store.select(getPrescriptionHeaderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPrescriptionHeaderCollectionCount, this.collectionId).subscribe(
				(count) => this.prescriptionHeadersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PrescriptionHeaderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PrescriptionHeaderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PrescriptionHeaderModelActionTypes.EXPORT_ALL_PRESCRIPTION_HEADER;
		} else {
			actionType = modelAction.PrescriptionHeaderModelActionTypes.EXPORT_PRESCRIPTION_HEADER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PrescriptionHeaderAction(
				modelAction.PrescriptionHeaderModelActionTypes.IMPORT_PRESCRIPTION_HEADERS,
				{
					file: file,
				},
				[new modelAction.PrescriptionHeaderAction(
			...
			window.open(environment.API_URL + '/api/prescriptionheader/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PrescriptionHeaderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Prescription Header entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Prescription Header entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PrescriptionHeaderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PrescriptionHeaderModelActionTypes.DELETE_PRESCRIPTION_HEADER;
						} else if (usingSelected) {
							actionType = modelAction.PrescriptionHeaderModelActionTypes.DELETE_ALL_PRESCRIPTION_HEADER;
						} else {
							actionType = modelAction.PrescriptionHeaderModelActionTypes.DELETE_PRESCRIPTION_HEADER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PrescriptionHeaderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PRESCRIPTION_HEADER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PRESCRIPTION_HEADER and DELETE_PRESCRIPTION_HEADER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PrescriptionHeaderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.prescriptionHeadersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
