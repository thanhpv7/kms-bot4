...
import {AdminPrescriptionHeaderCrudListComponent} from './prescription-header-admin-crud-list.component';
...
import {AdminPrescriptionHeaderCrudModule} from '../prescription-header-admin-crud.module';
import {
 ...
 */
describe('Prescription Header Crud List Tile Component', () => {

	let fixture;
	let prescriptionHeaderCrudListComponent: AdminPrescriptionHeaderCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PrescriptionHeaderModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a7097e84-671c-4b47-923e-0eef27f77687';
	const searchContent = 'text';

	const searchableFieldCount = PrescriptionHeaderModel.searchFields.length;

	const routerState: RouterState = {
		url: 'prescription-header-admin-crud',
		urls: ['prescription-header-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const prescriptionHeaderDataFactory = new PrescriptionHeaderDataFactory();

	const defaultDataSize = 30;
	const defaultData = prescriptionHeaderDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPrescriptionHeaderCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPrescriptionHeaderCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPrescriptionHeaderCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPrescriptionHeaderCrudListComponent);
			prescriptionHeaderCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(prescriptionHeaderCrudListComponent.prescriptionHeadersCount).toBe(defaultData.length);
			...
			expect(prescriptionHeaderCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			prescriptionHeaderCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = prescriptionHeaderDataFactory.createAll(filteredDataSize);
			...
			expect(prescriptionHeaderCrudListComponent.prescriptionHeadersCount).toBe(filteredDataSize);
			expect(prescriptionHeaderCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(prescriptionHeaderCrudListComponent.pageIndex).toBe(0);
			expect(prescriptionHeaderCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			prescriptionHeaderCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		prescriptionHeaderCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		prescriptionHeaderCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
