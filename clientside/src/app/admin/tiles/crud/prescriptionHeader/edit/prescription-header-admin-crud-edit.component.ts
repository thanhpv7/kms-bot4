...
import * as modelAction from 'src/app/models/prescriptionHeader/prescription_header.model.action';
import {PrescriptionHeaderModelState, PrescriptionHeaderModelAudit} from 'src/app/models/prescriptionHeader/prescription_header.model.state';
import {PrescriptionHeaderModel} from 'src/app/models/prescriptionHeader/prescription_header.model';
import {
	getPrescriptionHeaderModelAuditsByEntityId,
...
import {CrudTileMode} from '../prescription-header-admin-crud.component';
...
@Component({
	selector: 'cb-prescription-header-admin-crud-edit',
	templateUrl: './prescription-header-admin-crud-edit.component.html',
	styleUrls: [
		'./prescription-header-admin-crud-edit.component.scss',
...
export class AdminPrescriptionHeaderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'a7097e84-671c-4b47-923e-0eef27f77687';
	 ...
	 */
	prescriptionHeaderAudits$: Observable<PrescriptionHeaderModelAudit[]>;
	...
	invoiceOppositeRelation = 'prescriptionHeader';
	...
	registrationOppositeRelation = 'prescriptionHeaders';
	...
	requestedByOppositeRelation = 'prescriptionHeaders';
	...
	warehouseOppositeRelation = 'prescriptionHeaders';
	...
	invoiceItemsOppositeRelation = 'prescription';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Prescription Header and Prescription Compounds here] off begin
	prescriptionCompoundsMultiCrudActive: boolean = false;
	prescriptionCompoundsDisplayName = 'Prescription Compounds';
	prescriptionCompoundsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Prescription Header and Prescription Compounds here] end
	prescriptionCompoundsOppositeRelation = 'prescriptionHeader';
	prescriptionCompoundsRelations: ModelRelation[] = this.prescriptionCompoundsMultiCrudActive ? PrescriptionCompoundModel.getRelations()
		.map(relation => {
			if (relation.id === this.prescriptionCompoundsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	prescriptionItemsOppositeRelation = 'prescription';
	 ...
	 */
	modelProperties: ModelProperty[] = PrescriptionHeaderModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PrescriptionHeaderModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PrescriptionHeaderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'prescriptionCompounds' && this.prescriptionCompoundsMultiCrudActive) {
	 ...
	 */
	targetModel: PrescriptionHeaderModel;
	...
	constructor(
		private readonly store: Store<{ model: PrescriptionHeaderModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPrescriptionHeaderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PrescriptionHeaderModel();
		...
		if (this.targetModelId) {
			this.prescriptionHeaderAudits$ = this.store.select(getPrescriptionHeaderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PrescriptionHeaderModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PrescriptionHeaderAction(
			modelAction.PrescriptionHeaderModelActionTypes.FETCH_PRESCRIPTION_HEADER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPrescriptionHeaderModelWithId, this.targetModelId)
			...
			model = new PrescriptionHeaderModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('prescription-header');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PrescriptionHeaderAction(
			modelAction.PrescriptionHeaderModelActionTypes.FETCH_PRESCRIPTION_HEADER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PrescriptionHeaderModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.invoiceMultiCrudActive || this.registrationMultiCrudActive || this.requestedByMultiCrudActive || this.warehouseMultiCrudActive || this.invoiceItemsMultiCrudActive || this.prescriptionCompoundsMultiCrudActive || this.prescriptionItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PrescriptionHeaderAction(
						modelAction.PrescriptionHeaderModelActionTypes.COUNT_PRESCRIPTION_HEADERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PrescriptionHeaderAction(
				modelAction.PrescriptionHeaderModelActionTypes.CREATE_PRESCRIPTION_HEADER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PrescriptionHeaderAction(
				modelAction.PrescriptionHeaderModelActionTypes.UPDATE_PRESCRIPTION_HEADER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PrescriptionHeaderModel = new PrescriptionHeaderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPrescriptionHeaderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PrescriptionHeaderModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.prescriptionCompoundsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PrescriptionCompoundModel.getProps(), this.prescriptionCompoundsRelations, this.isViewOnly),
					new PrescriptionCompoundModel(),
					PrescriptionCompoundModel,
					this.prescriptionCompoundsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.prescriptionCompoundsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PrescriptionHeaderModel.getProps();
		this.modelRelations = PrescriptionHeaderModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'prescriptionCompounds';
		}).hideElement = this.prescriptionCompoundsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'prescriptionCompounds').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.prescriptionCompoundsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.prescriptionCompoundsDisplayName, many: true});
			this.store.dispatch(new prescriptionCompoundModelAction.PrescriptionCompoundAction(
				...
				{
					collectionId: this.prescriptionCompoundsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getPrescriptionCompoundCollectionModels, this.prescriptionCompoundsCollectionId).subscribe(models => {
				models.forEach(prescriptionCompounds => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(PrescriptionCompoundModel.getProps(), this.prescriptionCompoundsRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.prescriptionCompoundsIds && this.targetModel.prescriptionCompoundsIds.length > 0 && this.prescriptionCompoundsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PrescriptionCompoundModel,
				this.prescriptionCompoundsOppositeRelation,
				this.prescriptionCompoundsCollectionId,
				this.targetModel.prescriptionCompoundsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: PrescriptionHeaderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PrescriptionHeaderModel {
		let updatedModel: PrescriptionHeaderModel = new PrescriptionHeaderModel(model);
		...
		updatedModel.prescriptionCompounds = [];
		if (this.prescriptionCompoundsMultiCrudActive) {
			if (event.additionalEntities.prescriptionCompounds) {
				event.additionalEntities.prescriptionCompounds.forEach(entity => {
					updatedModel.prescriptionCompounds.push(entity);
				});
			}

			if (!updatedModel.prescriptionCompounds.length) {
				updatedModel.prescriptionCompoundsIds = [];
			}
		}
