...
import * as modelAction from 'src/app/models/bpjsDiagnosePRB/bpjs_diagnose_prb.model.action';
import {BpjsDiagnosePRBModelState, BpjsDiagnosePRBModelAudit} from 'src/app/models/bpjsDiagnosePRB/bpjs_diagnose_prb.model.state';
import {BpjsDiagnosePRBModel} from 'src/app/models/bpjsDiagnosePRB/bpjs_diagnose_prb.model';
import {
	getBpjsDiagnosePRBModelAuditsByEntityId,
...
import {CrudTileMode} from '../bpjs-diagnose-prb-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-diagnose-prb-admin-crud-edit',
	templateUrl: './bpjs-diagnose-prb-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-diagnose-prb-admin-crud-edit.component.scss',
...
export class AdminBpjsDiagnosePRBCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b96b2109-7681-4d13-92d9-eecd3fc6fa69';
	 ...
	 */
	bpjsDiagnosePRBAudits$: Observable<BpjsDiagnosePRBModelAudit[]>;
	...
	diagnosesAndProceduresOppositeRelation = 'bpjsDiagnosePRB';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Diagnose PRB and BPJS PRBs here] off begin
	bpjsPRBsMultiCrudActive: boolean = false;
	bpjsPRBsDisplayName = 'BPJS PRBs';
	bpjsPRBsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Diagnose PRB and BPJS PRBs here] end
	bpjsPRBsOppositeRelation = 'bpjsDiagnosePRB';
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsDiagnosePRBModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsDiagnosePRBModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsDiagnosePRBModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsDiagnosePRBModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsDiagnosePRBModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsDiagnosePRBModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsDiagnosePRBModel();
		...
		if (this.targetModelId) {
			this.bpjsDiagnosePRBAudits$ = this.store.select(getBpjsDiagnosePRBModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsDiagnosePRBModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
			modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_BPJS_DIAGNOSE_PRB,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsDiagnosePRBModelWithId, this.targetModelId)
			...
			model = new BpjsDiagnosePRBModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-diagnose-prb');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
			modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_BPJS_DIAGNOSE_PRB_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsDiagnosePRBModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.diagnosesAndProceduresMultiCrudActive || this.bpjsPRBsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsDiagnosePRBAction(
						modelAction.BpjsDiagnosePRBModelActionTypes.COUNT_BPJS_DIAGNOSE_PRBS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
				modelAction.BpjsDiagnosePRBModelActionTypes.CREATE_BPJS_DIAGNOSE_PRB,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
				modelAction.BpjsDiagnosePRBModelActionTypes.UPDATE_BPJS_DIAGNOSE_PRB,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsDiagnosePRBModel = new BpjsDiagnosePRBModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsDiagnosePRBModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsDiagnosePRBModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsDiagnosePRBModel.getProps();
		this.modelRelations = BpjsDiagnosePRBModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsDiagnosePRBModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsDiagnosePRBModel {
		let updatedModel: BpjsDiagnosePRBModel = new BpjsDiagnosePRBModel(model);
