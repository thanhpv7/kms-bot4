...
import {AdminBpjsDiagnosePRBCrudListComponent} from './bpjs-diagnose-prb-admin-crud-list.component';
...
import {AdminBpjsDiagnosePRBCrudModule} from '../bpjs-diagnose-prb-admin-crud.module';
import {
 ...
 */
describe('BPJS Diagnose PRB Crud List Tile Component', () => {

	let fixture;
	let bpjsDiagnosePRBCrudListComponent: AdminBpjsDiagnosePRBCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsDiagnosePRBModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b96b2109-7681-4d13-92d9-eecd3fc6fa69';
	const searchContent = 'text';

	const searchableFieldCount = BpjsDiagnosePRBModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-diagnose-prb-admin-crud',
		urls: ['bpjs-diagnose-prb-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsDiagnosePRBDataFactory = new BpjsDiagnosePRBDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsDiagnosePRBDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsDiagnosePRBCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsDiagnosePRBCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsDiagnosePRBCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsDiagnosePRBCrudListComponent);
			bpjsDiagnosePRBCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsDiagnosePRBCrudListComponent.bpjsDiagnosePRBsCount).toBe(defaultData.length);
			...
			expect(bpjsDiagnosePRBCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsDiagnosePRBCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsDiagnosePRBDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsDiagnosePRBCrudListComponent.bpjsDiagnosePRBsCount).toBe(filteredDataSize);
			expect(bpjsDiagnosePRBCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsDiagnosePRBCrudListComponent.pageIndex).toBe(0);
			expect(bpjsDiagnosePRBCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsDiagnosePRBCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsDiagnosePRBCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsDiagnosePRBCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
