...
import {BpjsDiagnosePRBModelState} from 'src/app/models/bpjsDiagnosePRB/bpjs_diagnose_prb.model.state';
...
import {
	...
	getBpjsDiagnosePRBCollectionCount
...
@Component({
	selector: 'cb-bpjs-diagnose-prb-admin-crud-list',
	templateUrl: './bpjs-diagnose-prb-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-diagnose-prb-admin-crud-list.component.scss',
...
export class AdminBpjsDiagnosePRBCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsDiagnosePRBModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsDiagnosePRBModel.searchFields,
	...
	/**
	 * List of BPJS Diagnose PRBs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsDiagnosePRBs: Observable<BpjsDiagnosePRBModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsDiagnosePRBsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsDiagnosePRBsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsDiagnosePRBsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
				modelAction.BpjsDiagnosePRBModelActionTypes.INITIALISE_BPJS_DIAGNOSE_PRB_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
				modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_BPJS_DIAGNOSE_PRB_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsDiagnosePRBs = this.store.select(getBpjsDiagnosePRBCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsDiagnosePRBCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsDiagnosePRBsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsDiagnosePRBModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsDiagnosePRBModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsDiagnosePRBModelActionTypes.EXPORT_ALL_BPJS_DIAGNOSE_PRB;
		} else {
			actionType = modelAction.BpjsDiagnosePRBModelActionTypes.EXPORT_BPJS_DIAGNOSE_PRB_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
				modelAction.BpjsDiagnosePRBModelActionTypes.IMPORT_BPJS_DIAGNOSE_PRBS,
				{
					file: file,
				},
				[new modelAction.BpjsDiagnosePRBAction(
			...
			window.open(environment.API_URL + '/api/bpjsdiagnoseprb/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsDiagnosePRBModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Diagnose PRB entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Diagnose PRB entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsDiagnosePRBModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsDiagnosePRBModelActionTypes.DELETE_BPJS_DIAGNOSE_PRB;
						} else if (usingSelected) {
							actionType = modelAction.BpjsDiagnosePRBModelActionTypes.DELETE_ALL_BPJS_DIAGNOSE_PRB;
						} else {
							actionType = modelAction.BpjsDiagnosePRBModelActionTypes.DELETE_BPJS_DIAGNOSE_PRB_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsDiagnosePRBAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_DIAGNOSE_PRB when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_DIAGNOSE_PRB and DELETE_BPJS_DIAGNOSE_PRB_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsDiagnosePRBModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsDiagnosePRBsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
