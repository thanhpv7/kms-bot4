...
import {AdminBpjsPcareRegistrationsCrudListComponent} from './bpjs-pcare-registrations-admin-crud-list.component';
...
import {AdminBpjsPcareRegistrationsCrudModule} from '../bpjs-pcare-registrations-admin-crud.module';
import {
	BpjsPcareRegistrationsModelState,
	initialState as initialBpjsPcareRegistrationsModelState
} from 'src/app/models/bpjsPcareRegistrations/bpjs_pcare_registrations.model.state';
 ...
 */
describe('BPJS Pcare Registrations Crud List Tile Component', () => {

	let fixture;
	let bpjsPcareRegistrationsCrudListComponent: AdminBpjsPcareRegistrationsCrudListComponent;

	let store: MockStore<{ model: BpjsPcareRegistrationsModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsPcareRegistrationsModelState: BpjsPcareRegistrationsModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPcareRegistrationsModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '0ef9164c-9b10-4435-9400-5557a057fbb0';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPcareRegistrationsModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-pcare-registrations-admin-crud',
		urls: ['bpjs-pcare-registrations-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPcareRegistrationsDataFactory = new BpjsPcareRegistrationsDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPcareRegistrationsDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsPcareRegistrationsModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsPcareRegistrationsModelState = initialBpjsPcareRegistrationsModelState;
		store.setState({model: bpjsPcareRegistrationsModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPcareRegistrationsCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPcareRegistrationsCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsPcareRegistrationsCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsPcareRegistrationsCrudListComponent);
			bpjsPcareRegistrationsCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPcareRegistrationsCrudListComponent.bpjsPcareRegistrationssCount).toBe(defaultData.length);
			...
			expect(bpjsPcareRegistrationsCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPcareRegistrationsCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPcareRegistrationsDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPcareRegistrationsCrudListComponent.bpjsPcareRegistrationssCount).toBe(filteredDataSize);
			expect(bpjsPcareRegistrationsCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPcareRegistrationsCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPcareRegistrationsCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPcareRegistrationsCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPcareRegistrationsCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPcareRegistrationsCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
