...
import * as modelAction from 'src/app/models/bpjsPcareRegistrations/bpjs_pcare_registrations.model.action';
import {BpjsPcareRegistrationsModelState} from 'src/app/models/bpjsPcareRegistrations/bpjs_pcare_registrations.model.state';
...
import {
	getBpjsPcareRegistrationsCollectionModels,
	getBpjsPcareRegistrationsCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-registrations-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-registrations-admin-crud-list',
	templateUrl: './bpjs-pcare-registrations-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-registrations-admin-crud-list.component.scss',
...
export class AdminBpjsPcareRegistrationsCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcareRegistrationsModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcareRegistrationsModel.searchFields,
	...
	/**
	 * List of BPJS Pcare Registrationss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcareRegistrationss: Observable<BpjsPcareRegistrationsModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcareRegistrationssCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcareRegistrationsModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcareRegistrationsModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '0ef9164c-9b10-4435-9400-5557a057fbb0';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcareRegistrationssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcareRegistrationssCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcareRegistrationsModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
				modelAction.BpjsPcareRegistrationsModelActionTypes.INITIALISE_BPJS_PCARE_REGISTRATIONS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
				modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_BPJS_PCARE_REGISTRATIONS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcareRegistrationsAction(
						modelAction.BpjsPcareRegistrationsModelActionTypes.COUNT_BPJS_PCARE_REGISTRATIONSS,
						null
					)
				]
			))
		...
		this.bpjsPcareRegistrationss = this.store.select(getBpjsPcareRegistrationsCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcareRegistrationsCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcareRegistrationssCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-registrations');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcareRegistrationsModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcareRegistrationsModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcareRegistrationsModelActionTypes.EXPORT_ALL_BPJS_PCARE_REGISTRATIONS;
		} else {
			actionType = modelAction.BpjsPcareRegistrationsModelActionTypes.EXPORT_BPJS_PCARE_REGISTRATIONS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
				modelAction.BpjsPcareRegistrationsModelActionTypes.IMPORT_BPJS_PCARE_REGISTRATIONSS,
				{
					file: file,
				},
				[new modelAction.BpjsPcareRegistrationsAction(
			...
			window.open(environment.API_URL + '/api/bpjspcareregistrations/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcareRegistrationsModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Registrations entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Registrations entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcareRegistrationsModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcareRegistrationsModelActionTypes.DELETE_BPJS_PCARE_REGISTRATIONS;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcareRegistrationsModelActionTypes.DELETE_ALL_BPJS_PCARE_REGISTRATIONS;
						} else {
							actionType = modelAction.BpjsPcareRegistrationsModelActionTypes.DELETE_BPJS_PCARE_REGISTRATIONS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_REGISTRATIONS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_REGISTRATIONS and DELETE_BPJS_PCARE_REGISTRATIONS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcareRegistrationsModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcareRegistrationsModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcareRegistrationssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
