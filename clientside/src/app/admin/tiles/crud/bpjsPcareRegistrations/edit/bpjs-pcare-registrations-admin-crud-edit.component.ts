...
import {BpjsPcareRegistrationsModelState, BpjsPcareRegistrationsModelAudit} from 'src/app/models/bpjsPcareRegistrations/bpjs_pcare_registrations.model.state';
import {BpjsPcareRegistrationsModel} from 'src/app/models/bpjsPcareRegistrations/bpjs_pcare_registrations.model';
import {
	getBpjsPcareRegistrationsModelAuditsByEntityId,
	getBpjsPcareRegistrationsModelWithId,
	getCountBpjsPcareRegistrationsModels,
...
@Component({
	selector: 'cb-bpjs-pcare-registrations-admin-crud-edit',
	templateUrl: './bpjs-pcare-registrations-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-registrations-admin-crud-edit.component.scss',
...
export class AdminBpjsPcareRegistrationsCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcareRegistrationsAudits$: Observable<BpjsPcareRegistrationsModelAudit[]>;
	...
	patientVisitOppositeRelation = 'bpjsPcareRegistrations';
	...
	registrationOppositeRelation = 'bpjsPcareRegistrations';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcareRegistrationsModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcareRegistrationsModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcareRegistrationsModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcareRegistrationsModel();
		...
		if (this.targetModelId) {
			this.bpjsPcareRegistrationsAudits$ = this.store.select(getBpjsPcareRegistrationsModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcareRegistrationsModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
			modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_BPJS_PCARE_REGISTRATIONS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareRegistrationsModelWithId, this.targetModelId)
			...
			model = new BpjsPcareRegistrationsModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
			modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_BPJS_PCARE_REGISTRATIONS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsPcareRegistrationsModel> = {
				...
				queryParams: {
					persistRelations: this.patientVisitMultiCrudActive || this.registrationMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
				modelAction.BpjsPcareRegistrationsModelActionTypes.CREATE_BPJS_PCARE_REGISTRATIONS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcareRegistrationsAction(
				modelAction.BpjsPcareRegistrationsModelActionTypes.UPDATE_BPJS_PCARE_REGISTRATIONS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcareRegistrationsModel = new BpjsPcareRegistrationsModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareRegistrationsModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcareRegistrationsModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcareRegistrationsModel.getProps();
		this.modelRelations = BpjsPcareRegistrationsModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcareRegistrationsModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcareRegistrationsModel {
		let updatedModel: BpjsPcareRegistrationsModel = new BpjsPcareRegistrationsModel(model);
