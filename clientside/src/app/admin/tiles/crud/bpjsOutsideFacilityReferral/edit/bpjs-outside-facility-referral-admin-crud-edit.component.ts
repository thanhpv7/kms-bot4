...
import {BpjsOutsideFacilityReferralModelState, BpjsOutsideFacilityReferralModelAudit} from 'src/app/models/bpjsOutsideFacilityReferral/bpjs_outside_facility_referral.model.state';
...
import {
	getBpjsOutsideFacilityReferralModelAuditsByEntityId,
	getBpjsOutsideFacilityReferralModelWithId,
	getCountBpjsOutsideFacilityReferralModels,
...
@Component({
	selector: 'cb-bpjs-outside-facility-referral-admin-crud-edit',
	templateUrl: './bpjs-outside-facility-referral-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-outside-facility-referral-admin-crud-edit.component.scss',
...
export class AdminBpjsOutsideFacilityReferralCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsOutsideFacilityReferralAudits$: Observable<BpjsOutsideFacilityReferralModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsOutsideFacilityReferralModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsOutsideFacilityReferralModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsOutsideFacilityReferralModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsOutsideFacilityReferralModel();
		...
		if (this.targetModelId) {
			this.bpjsOutsideFacilityReferralAudits$ = this.store.select(getBpjsOutsideFacilityReferralModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsOutsideFacilityReferralModel> = {
		...
		this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
			modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_BPJS_OUTSIDE_FACILITY_REFERRAL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsOutsideFacilityReferralModelWithId, this.targetModelId)
			...
			model = new BpjsOutsideFacilityReferralModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
			modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_BPJS_OUTSIDE_FACILITY_REFERRAL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
				modelAction.BpjsOutsideFacilityReferralModelActionTypes.CREATE_BPJS_OUTSIDE_FACILITY_REFERRAL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
				modelAction.BpjsOutsideFacilityReferralModelActionTypes.UPDATE_BPJS_OUTSIDE_FACILITY_REFERRAL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsOutsideFacilityReferralModel = new BpjsOutsideFacilityReferralModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsOutsideFacilityReferralModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsOutsideFacilityReferralModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsOutsideFacilityReferralModel.getProps();
		this.modelRelations = BpjsOutsideFacilityReferralModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsOutsideFacilityReferralModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsOutsideFacilityReferralModel {
		let updatedModel: BpjsOutsideFacilityReferralModel = new BpjsOutsideFacilityReferralModel(model);
