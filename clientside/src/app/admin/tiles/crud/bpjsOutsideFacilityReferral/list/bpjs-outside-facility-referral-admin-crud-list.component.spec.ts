...
import {AdminBpjsOutsideFacilityReferralCrudListComponent} from './bpjs-outside-facility-referral-admin-crud-list.component';
import {BpjsOutsideFacilityReferralDataFactory} from 'src/app/lib/utils/factories/bpjs-outside-facility-referral-data-factory';
...
import {AdminBpjsOutsideFacilityReferralCrudModule} from '../bpjs-outside-facility-referral-admin-crud.module';
import {
	BpjsOutsideFacilityReferralModelState,
	initialState as initialBpjsOutsideFacilityReferralModelState
} from 'src/app/models/bpjsOutsideFacilityReferral/bpjs_outside_facility_referral.model.state';
 ...
 */
describe('BPJS Outside Facility Referral Crud List Tile Component', () => {

	let fixture;
	let bpjsOutsideFacilityReferralCrudListComponent: AdminBpjsOutsideFacilityReferralCrudListComponent;

	let store: MockStore<{ model: BpjsOutsideFacilityReferralModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsOutsideFacilityReferralModelState: BpjsOutsideFacilityReferralModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsOutsideFacilityReferralModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '40d08c41-67db-4c37-a864-adc7ebfe990d';
	const searchContent = 'text';

	const searchableFieldCount = BpjsOutsideFacilityReferralModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-outside-facility-referral-admin-crud',
		urls: ['bpjs-outside-facility-referral-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsOutsideFacilityReferralDataFactory = new BpjsOutsideFacilityReferralDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsOutsideFacilityReferralDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsOutsideFacilityReferralModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsOutsideFacilityReferralModelState = initialBpjsOutsideFacilityReferralModelState;
		store.setState({model: bpjsOutsideFacilityReferralModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsOutsideFacilityReferralCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsOutsideFacilityReferralCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsOutsideFacilityReferralCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsOutsideFacilityReferralCrudListComponent);
			bpjsOutsideFacilityReferralCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsOutsideFacilityReferralCrudListComponent.bpjsOutsideFacilityReferralsCount).toBe(defaultData.length);
			...
			expect(bpjsOutsideFacilityReferralCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsOutsideFacilityReferralCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsOutsideFacilityReferralDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsOutsideFacilityReferralCrudListComponent.bpjsOutsideFacilityReferralsCount).toBe(filteredDataSize);
			expect(bpjsOutsideFacilityReferralCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsOutsideFacilityReferralCrudListComponent.pageIndex).toBe(0);
			expect(bpjsOutsideFacilityReferralCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsOutsideFacilityReferralCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsOutsideFacilityReferralCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsOutsideFacilityReferralCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
