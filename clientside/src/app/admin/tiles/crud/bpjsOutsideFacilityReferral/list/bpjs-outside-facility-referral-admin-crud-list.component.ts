...
import * as modelAction from 'src/app/models/bpjsOutsideFacilityReferral/bpjs_outside_facility_referral.model.action';
import {BpjsOutsideFacilityReferralModelState} from 'src/app/models/bpjsOutsideFacilityReferral/bpjs_outside_facility_referral.model.state';
...
import {
	getBpjsOutsideFacilityReferralCollectionModels,
	getBpjsOutsideFacilityReferralCollectionCount
} from 'src/app/models/bpjsOutsideFacilityReferral/bpjs_outside_facility_referral.model.selector';
...
import {CrudTileMode} from '../bpjs-outside-facility-referral-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-outside-facility-referral-admin-crud-list',
	templateUrl: './bpjs-outside-facility-referral-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-outside-facility-referral-admin-crud-list.component.scss',
...
export class AdminBpjsOutsideFacilityReferralCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsOutsideFacilityReferralModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsOutsideFacilityReferralModel.searchFields,
	...
	/**
	 * List of BPJS Outside Facility Referrals that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsOutsideFacilityReferrals: Observable<BpjsOutsideFacilityReferralModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsOutsideFacilityReferralsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsOutsideFacilityReferralModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsOutsideFacilityReferralModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '40d08c41-67db-4c37-a864-adc7ebfe990d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsOutsideFacilityReferralsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsOutsideFacilityReferralsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsOutsideFacilityReferralModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
				modelAction.BpjsOutsideFacilityReferralModelActionTypes.INITIALISE_BPJS_OUTSIDE_FACILITY_REFERRAL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
				modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_BPJS_OUTSIDE_FACILITY_REFERRAL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsOutsideFacilityReferralAction(
						modelAction.BpjsOutsideFacilityReferralModelActionTypes.COUNT_BPJS_OUTSIDE_FACILITY_REFERRALS,
						null
					)
				]
			))
		...
		this.bpjsOutsideFacilityReferrals = this.store.select(getBpjsOutsideFacilityReferralCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsOutsideFacilityReferralCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsOutsideFacilityReferralsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-outside-facility-referral');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsOutsideFacilityReferralModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsOutsideFacilityReferralModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsOutsideFacilityReferralModelActionTypes.EXPORT_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL;
		} else {
			actionType = modelAction.BpjsOutsideFacilityReferralModelActionTypes.EXPORT_BPJS_OUTSIDE_FACILITY_REFERRAL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
				modelAction.BpjsOutsideFacilityReferralModelActionTypes.IMPORT_BPJS_OUTSIDE_FACILITY_REFERRALS,
				{
					file: file,
				},
				[new modelAction.BpjsOutsideFacilityReferralAction(
			...
			window.open(environment.API_URL + '/api/bpjsoutsidefacilityreferral/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsOutsideFacilityReferralModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Outside Facility Referral entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Outside Facility Referral entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsOutsideFacilityReferralModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsOutsideFacilityReferralModelActionTypes.DELETE_BPJS_OUTSIDE_FACILITY_REFERRAL;
						} else if (usingSelected) {
							actionType = modelAction.BpjsOutsideFacilityReferralModelActionTypes.DELETE_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL;
						} else {
							actionType = modelAction.BpjsOutsideFacilityReferralModelActionTypes.DELETE_BPJS_OUTSIDE_FACILITY_REFERRAL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsOutsideFacilityReferralAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_OUTSIDE_FACILITY_REFERRAL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL and DELETE_BPJS_OUTSIDE_FACILITY_REFERRAL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsOutsideFacilityReferralModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsOutsideFacilityReferralModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsOutsideFacilityReferralsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
