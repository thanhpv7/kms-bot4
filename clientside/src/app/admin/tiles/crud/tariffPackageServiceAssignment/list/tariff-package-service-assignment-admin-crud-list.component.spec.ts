...
import {AdminTariffPackageServiceAssignmentCrudListComponent} from './tariff-package-service-assignment-admin-crud-list.component';
...
import {AdminTariffPackageServiceAssignmentCrudModule} from '../tariff-package-service-assignment-admin-crud.module';
import {
	TariffPackageServiceAssignmentModelState,
	initialState as initialTariffPackageServiceAssignmentModelState
} from 'src/app/models/tariffPackageServiceAssignment/tariff_package_service_assignment.model.state';
 ...
 */
describe('Tariff Package Service Assignment Crud List Tile Component', () => {

	let fixture;
	let tariffPackageServiceAssignmentCrudListComponent: AdminTariffPackageServiceAssignmentCrudListComponent;

	let store: MockStore<{ model: TariffPackageServiceAssignmentModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let tariffPackageServiceAssignmentModelState: TariffPackageServiceAssignmentModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<TariffPackageServiceAssignmentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '87a991a2-5cdd-4d1d-8be2-45850522ea27';
	const searchContent = 'text';

	const searchableFieldCount = TariffPackageServiceAssignmentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'tariff-package-service-assignment-admin-crud',
		urls: ['tariff-package-service-assignment-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const tariffPackageServiceAssignmentDataFactory = new TariffPackageServiceAssignmentDataFactory();

	const defaultDataSize = 30;
	const defaultData = tariffPackageServiceAssignmentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: TariffPackageServiceAssignmentModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		tariffPackageServiceAssignmentModelState = initialTariffPackageServiceAssignmentModelState;
		store.setState({model: tariffPackageServiceAssignmentModelState});
		...
		spyOn(store, 'select')
			.withArgs(getTariffPackageServiceAssignmentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTariffPackageServiceAssignmentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminTariffPackageServiceAssignmentCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminTariffPackageServiceAssignmentCrudListComponent);
			tariffPackageServiceAssignmentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(tariffPackageServiceAssignmentCrudListComponent.tariffPackageServiceAssignmentsCount).toBe(defaultData.length);
			...
			expect(tariffPackageServiceAssignmentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			tariffPackageServiceAssignmentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = tariffPackageServiceAssignmentDataFactory.createAll(filteredDataSize);
			...
			expect(tariffPackageServiceAssignmentCrudListComponent.tariffPackageServiceAssignmentsCount).toBe(filteredDataSize);
			expect(tariffPackageServiceAssignmentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(tariffPackageServiceAssignmentCrudListComponent.pageIndex).toBe(0);
			expect(tariffPackageServiceAssignmentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			tariffPackageServiceAssignmentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		tariffPackageServiceAssignmentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		tariffPackageServiceAssignmentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
