...
import {TariffPackageServiceAssignmentModelState} from 'src/app/models/tariffPackageServiceAssignment/tariff_package_service_assignment.model.state';
...
import {
	getTariffPackageServiceAssignmentCollectionModels,
	getTariffPackageServiceAssignmentCollectionCount
...
@Component({
	selector: 'cb-tariff-package-service-assignment-admin-crud-list',
	templateUrl: './tariff-package-service-assignment-admin-crud-list.component.html',
	styleUrls: [
		'./tariff-package-service-assignment-admin-crud-list.component.scss',
...
export class AdminTariffPackageServiceAssignmentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TariffPackageServiceAssignmentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TariffPackageServiceAssignmentModel.searchFields,
	...
	/**
	 * List of Tariff Package Service Assignments that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	tariffPackageServiceAssignments: Observable<TariffPackageServiceAssignmentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	tariffPackageServiceAssignmentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.tariffPackageServiceAssignmentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.tariffPackageServiceAssignmentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
				modelAction.TariffPackageServiceAssignmentModelActionTypes.INITIALISE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
				modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.tariffPackageServiceAssignments = this.store.select(getTariffPackageServiceAssignmentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTariffPackageServiceAssignmentCollectionCount, this.collectionId).subscribe(
				(count) => this.tariffPackageServiceAssignmentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TariffPackageServiceAssignmentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TariffPackageServiceAssignmentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TariffPackageServiceAssignmentModelActionTypes.EXPORT_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT;
		} else {
			actionType = modelAction.TariffPackageServiceAssignmentModelActionTypes.EXPORT_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
				modelAction.TariffPackageServiceAssignmentModelActionTypes.IMPORT_TARIFF_PACKAGE_SERVICE_ASSIGNMENTS,
				{
					file: file,
				},
				[new modelAction.TariffPackageServiceAssignmentAction(
			...
			window.open(environment.API_URL + '/api/tariffpackageserviceassignment/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TariffPackageServiceAssignmentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Tariff Package Service Assignment entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Tariff Package Service Assignment entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TariffPackageServiceAssignmentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TariffPackageServiceAssignmentModelActionTypes.DELETE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT;
						} else if (usingSelected) {
							actionType = modelAction.TariffPackageServiceAssignmentModelActionTypes.DELETE_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT;
						} else {
							actionType = modelAction.TariffPackageServiceAssignmentModelActionTypes.DELETE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT and DELETE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TariffPackageServiceAssignmentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.tariffPackageServiceAssignmentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
