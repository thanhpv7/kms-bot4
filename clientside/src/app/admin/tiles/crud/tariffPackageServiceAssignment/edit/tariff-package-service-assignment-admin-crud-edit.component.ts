...
import * as modelAction from 'src/app/models/tariffPackageServiceAssignment/tariff_package_service_assignment.model.action';
import {TariffPackageServiceAssignmentModelState, TariffPackageServiceAssignmentModelAudit} from 'src/app/models/tariffPackageServiceAssignment/tariff_package_service_assignment.model.state';
...
import {
	getTariffPackageServiceAssignmentModelAuditsByEntityId,
	getTariffPackageServiceAssignmentModelWithId,
	getCountTariffPackageServiceAssignmentModels,
...
import * as tariffPackageModelAction from 'src/app/models/tariffPackage/tariff_package.model.action';
import {
	getTariffPackageCollectionModels, getTariffPackageModelWithId,
...
import {CrudTileMode} from '../tariff-package-service-assignment-admin-crud.component';
...
@Component({
	selector: 'cb-tariff-package-service-assignment-admin-crud-edit',
	templateUrl: './tariff-package-service-assignment-admin-crud-edit.component.html',
	styleUrls: [
		'./tariff-package-service-assignment-admin-crud-edit.component.scss',
...
export class AdminTariffPackageServiceAssignmentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '87a991a2-5cdd-4d1d-8be2-45850522ea27';
	 ...
	 */
	tariffPackageServiceAssignmentAudits$: Observable<TariffPackageServiceAssignmentModelAudit[]>;
	...
	tariffPackageMultiCrudActive: boolean = false;
	tariffPackageDisplayName = 'Tariff Package';
	...
	tariffPackageOppositeRelation = 'tariffPackageServiceAssignments';
	tariffPackageRelations: ModelRelation[] = this.tariffPackageMultiCrudActive ? TariffPackageModel.getRelations()
		.map(relation => {
			if (relation.id === this.tariffPackageOppositeRelation) {
	 ...
	 */
	modelProperties: ModelProperty[] = TariffPackageServiceAssignmentModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TariffPackageServiceAssignmentModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TariffPackageServiceAssignmentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'tariffPackage' && this.tariffPackageMultiCrudActive) {
					exp.fields = TariffPackageModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: TariffPackageServiceAssignmentModel;
	...
	constructor(
		private readonly store: Store<{ model: TariffPackageServiceAssignmentModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTariffPackageServiceAssignmentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TariffPackageServiceAssignmentModel();
		...
		if (this.targetModelId) {
			this.tariffPackageServiceAssignmentAudits$ = this.store.select(getTariffPackageServiceAssignmentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TariffPackageServiceAssignmentModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
			modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_TARIFF_PACKAGE_SERVICE_ASSIGNMENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTariffPackageServiceAssignmentModelWithId, this.targetModelId)
			...
			model = new TariffPackageServiceAssignmentModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('tariff-package-service-assignment');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
			modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TariffPackageServiceAssignmentModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.tariffPackageMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TariffPackageServiceAssignmentAction(
						modelAction.TariffPackageServiceAssignmentModelActionTypes.COUNT_TARIFF_PACKAGE_SERVICE_ASSIGNMENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
				modelAction.TariffPackageServiceAssignmentModelActionTypes.CREATE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TariffPackageServiceAssignmentAction(
				modelAction.TariffPackageServiceAssignmentModelActionTypes.UPDATE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TariffPackageServiceAssignmentModel = new TariffPackageServiceAssignmentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTariffPackageServiceAssignmentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TariffPackageServiceAssignmentModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.tariffPackageDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffPackageModel.getProps(), this.tariffPackageRelations, this.isViewOnly),
					new TariffPackageModel(),
					TariffPackageModel,
					this.tariffPackageDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.tariffPackageRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TariffPackageServiceAssignmentModel.getProps();
		this.modelRelations = TariffPackageServiceAssignmentModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'tariffPackage';
		}).hideElement = this.tariffPackageMultiCrudActive || this.modelRelations.find(rel => rel.id === 'tariffPackage').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.tariffPackageMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.tariffPackageDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.tariffPackage && this.targetModel.tariffPackage.id && this.tariffPackageMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffPackageModel,
				this.tariffPackageOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.tariffPackage.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffPackageModelWithId, this.targetModel.tariffPackageId).subscribe(tariffPackage => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffPackageModel.getProps(), this.tariffPackageRelations, this.isViewOnly),
					tariffPackage,
					TariffPackageModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: TariffPackageServiceAssignmentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TariffPackageServiceAssignmentModel {
		let updatedModel: TariffPackageServiceAssignmentModel = new TariffPackageServiceAssignmentModel(model);

		if (this.tariffPackageMultiCrudActive) {
			updatedModel.tariffPackage = event.additionalEntities.tariffPackage ? event.additionalEntities.tariffPackage : undefined;

			if (!updatedModel.tariffPackage && event.payload) {
				event.payload.tariffPackageId = null;
			}
		} else if (event.payload?.tariffPackageId !== undefined) {
			event.payload.tariffPackage = null;
			updatedModel.tariffPackage = null;
		}
