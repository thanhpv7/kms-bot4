...
import {AdminUnitOfMeasurementCrudListComponent} from './unit-of-measurement-admin-crud-list.component';
...
import {AdminUnitOfMeasurementCrudModule} from '../unit-of-measurement-admin-crud.module';
import {
	UnitOfMeasurementModelState,
	initialState as initialUnitOfMeasurementModelState
} from 'src/app/models/unitOfMeasurement/unit_of_measurement.model.state';
 ...
 */
describe('Unit Of Measurement Crud List Tile Component', () => {

	let fixture;
	let unitOfMeasurementCrudListComponent: AdminUnitOfMeasurementCrudListComponent;

	let store: MockStore<{ model: UnitOfMeasurementModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let unitOfMeasurementModelState: UnitOfMeasurementModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<UnitOfMeasurementModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8a0fe0d4-609f-4bb2-be4d-0e79c036b035';
	const searchContent = 'text';

	const searchableFieldCount = UnitOfMeasurementModel.searchFields.length;

	const routerState: RouterState = {
		url: 'unit-of-measurement-admin-crud',
		urls: ['unit-of-measurement-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const unitOfMeasurementDataFactory = new UnitOfMeasurementDataFactory();

	const defaultDataSize = 30;
	const defaultData = unitOfMeasurementDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: UnitOfMeasurementModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		unitOfMeasurementModelState = initialUnitOfMeasurementModelState;
		store.setState({model: unitOfMeasurementModelState});
		...
		spyOn(store, 'select')
			.withArgs(getUnitOfMeasurementCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getUnitOfMeasurementCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminUnitOfMeasurementCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminUnitOfMeasurementCrudListComponent);
			unitOfMeasurementCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(unitOfMeasurementCrudListComponent.unitOfMeasurementsCount).toBe(defaultData.length);
			...
			expect(unitOfMeasurementCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			unitOfMeasurementCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = unitOfMeasurementDataFactory.createAll(filteredDataSize);
			...
			expect(unitOfMeasurementCrudListComponent.unitOfMeasurementsCount).toBe(filteredDataSize);
			expect(unitOfMeasurementCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(unitOfMeasurementCrudListComponent.pageIndex).toBe(0);
			expect(unitOfMeasurementCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			unitOfMeasurementCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		unitOfMeasurementCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		unitOfMeasurementCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
