...
import {UnitOfMeasurementModelState} from 'src/app/models/unitOfMeasurement/unit_of_measurement.model.state';
...
import {
	getUnitOfMeasurementCollectionModels,
	getUnitOfMeasurementCollectionCount
...
@Component({
	selector: 'cb-unit-of-measurement-admin-crud-list',
	templateUrl: './unit-of-measurement-admin-crud-list.component.html',
	styleUrls: [
		'./unit-of-measurement-admin-crud-list.component.scss',
...
export class AdminUnitOfMeasurementCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<UnitOfMeasurementModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...UnitOfMeasurementModel.searchFields,
	...
	/**
	 * List of Unit Of Measurements that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	unitOfMeasurements: Observable<UnitOfMeasurementModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	unitOfMeasurementsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.unitOfMeasurementsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.unitOfMeasurementsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.UnitOfMeasurementAction(
				modelAction.UnitOfMeasurementModelActionTypes.INITIALISE_UNIT_OF_MEASUREMENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.UnitOfMeasurementAction(
				modelAction.UnitOfMeasurementModelActionTypes.FETCH_UNIT_OF_MEASUREMENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.unitOfMeasurements = this.store.select(getUnitOfMeasurementCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getUnitOfMeasurementCollectionCount, this.collectionId).subscribe(
				(count) => this.unitOfMeasurementsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: UnitOfMeasurementModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.UnitOfMeasurementModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.UnitOfMeasurementModelActionTypes.EXPORT_ALL_UNIT_OF_MEASUREMENT;
		} else {
			actionType = modelAction.UnitOfMeasurementModelActionTypes.EXPORT_UNIT_OF_MEASUREMENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.UnitOfMeasurementAction(
				modelAction.UnitOfMeasurementModelActionTypes.IMPORT_UNIT_OF_MEASUREMENTS,
				{
					file: file,
				},
				[new modelAction.UnitOfMeasurementAction(
			...
			window.open(environment.API_URL + '/api/unitofmeasurement/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: UnitOfMeasurementModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Unit Of Measurement entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Unit Of Measurement entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.UnitOfMeasurementModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.UnitOfMeasurementModelActionTypes.DELETE_UNIT_OF_MEASUREMENT;
						} else if (usingSelected) {
							actionType = modelAction.UnitOfMeasurementModelActionTypes.DELETE_ALL_UNIT_OF_MEASUREMENT;
						} else {
							actionType = modelAction.UnitOfMeasurementModelActionTypes.DELETE_UNIT_OF_MEASUREMENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.UnitOfMeasurementAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_UNIT_OF_MEASUREMENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_UNIT_OF_MEASUREMENT and DELETE_UNIT_OF_MEASUREMENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = UnitOfMeasurementModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.unitOfMeasurementsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
