...
import * as modelAction from 'src/app/models/unitOfMeasurement/unit_of_measurement.model.action';
import {UnitOfMeasurementModelState, UnitOfMeasurementModelAudit} from 'src/app/models/unitOfMeasurement/unit_of_measurement.model.state';
import {UnitOfMeasurementModel} from 'src/app/models/unitOfMeasurement/unit_of_measurement.model';
import {
	getUnitOfMeasurementModelAuditsByEntityId,
	getUnitOfMeasurementModelWithId,
	getCountUnitOfMeasurementModels,
...
import * as compoundPrescriptionDetailModelAction from 'src/app/models/compoundPrescriptionDetail/compound_prescription_detail.model.action';
import {
	getCompoundPrescriptionDetailCollectionModels, getCompoundPrescriptionDetailModelWithId,
...
import * as medicationCompoundModelAction from 'src/app/models/medicationCompound/medication_compound.model.action';
import {
	getMedicationCompoundCollectionModels, getMedicationCompoundModelWithId,
...
import {CrudTileMode} from '../unit-of-measurement-admin-crud.component';
...
@Component({
	selector: 'cb-unit-of-measurement-admin-crud-edit',
	templateUrl: './unit-of-measurement-admin-crud-edit.component.html',
	styleUrls: [
		'./unit-of-measurement-admin-crud-edit.component.scss',
...
export class AdminUnitOfMeasurementCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '8a0fe0d4-609f-4bb2-be4d-0e79c036b035';
	 ...
	 */
	unitOfMeasurementAudits$: Observable<UnitOfMeasurementModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Catalogue UOM and Unit Of Measurement here] off begin
	catalogueUOMMultiCrudActive: boolean = false;
	catalogueUOMDisplayName = 'Catalogue UOM';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Catalogue UOM and Unit Of Measurement here] end
	catalogueUOMOppositeRelation = 'unitOfMeasurement';
	catalogueUOMRelations: ModelRelation[] = this.catalogueUOMMultiCrudActive ? CatalogueUOMModel.getRelations()
		.map(relation => {
			if (relation.id === this.catalogueUOMOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Consumption Unit and Compound Prescription Details here] off begin
	compoundPrescriptionDetailsMultiCrudActive: boolean = false;
	compoundPrescriptionDetailsDisplayName = 'Compound Prescription Details';
	compoundPrescriptionDetailsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Consumption Unit and Compound Prescription Details here] end
	compoundPrescriptionDetailsOppositeRelation = 'consumptionUnit';
	compoundPrescriptionDetailsRelations: ModelRelation[] = this.compoundPrescriptionDetailsMultiCrudActive ? CompoundPrescriptionDetailModel.getRelations()
		.map(relation => {
			if (relation.id === this.compoundPrescriptionDetailsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Unit and Compound Prescription Items here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Unit and Compound Prescription Items here] end
	compoundPrescriptionItemsOppositeRelation = 'unit';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Unit and Compounds Unit here] off begin
	compoundsUnitMultiCrudActive: boolean = false;
	compoundsUnitDisplayName = 'Compounds Unit';
	compoundsUnitCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Unit and Compounds Unit here] end
	compoundsUnitOppositeRelation = 'unit';
	compoundsUnitRelations: ModelRelation[] = this.compoundsUnitMultiCrudActive ? PrescriptionCompoundModel.getRelations()
		.map(relation => {
			if (relation.id === this.compoundsUnitOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	medicationCompoundsMultiCrudActive: boolean = false;
	medicationCompoundsDisplayName = 'Medication Compounds';
	medicationCompoundsCollectionId: string = uuidv4();
	...
	medicationCompoundsOppositeRelation = 'unitOfMeasurement';
	medicationCompoundsRelations: ModelRelation[] = this.medicationCompoundsMultiCrudActive ? MedicationCompoundModel.getRelations()
		.map(relation => {
			if (relation.id === this.medicationCompoundsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Unit Of Measurement and Medication Items here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Unit Of Measurement and Medication Items here] end
	medicationItemsOppositeRelation = 'unitOfMeasurement';
	...
	prescriptionItemsMultiCrudActive: boolean = false;
	prescriptionItemsDisplayName = 'Prescription Items';
	prescriptionItemsCollectionId: string = uuidv4();
	...
	prescriptionItemsOppositeRelation = 'unit';
	prescriptionItemsRelations: ModelRelation[] = this.prescriptionItemsMultiCrudActive ? PrescriptionItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.prescriptionItemsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = UnitOfMeasurementModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = UnitOfMeasurementModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = UnitOfMeasurementModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'catalogueUOM' && this.catalogueUOMMultiCrudActive) {
					exp.fields = CatalogueUOMModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'compoundPrescriptionDetails' && this.compoundPrescriptionDetailsMultiCrudActive) {
					exp.fields = CompoundPrescriptionDetailModel.getProps().map(prop => {
						return prop.name;
					});
				}
				...
				if (relation.id === 'compoundsUnit' && this.compoundsUnitMultiCrudActive) {
					exp.fields = PrescriptionCompoundModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'medicationCompounds' && this.medicationCompoundsMultiCrudActive) {
					exp.fields = MedicationCompoundModel.getProps().map(prop => {
						return prop.name;
					});
				}
				...
				if (relation.id === 'prescriptionItems' && this.prescriptionItemsMultiCrudActive) {
	 ...
	 */
	targetModel: UnitOfMeasurementModel;
	...
	constructor(
		private readonly store: Store<{ model: UnitOfMeasurementModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountUnitOfMeasurementModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new UnitOfMeasurementModel();
		...
		if (this.targetModelId) {
			this.unitOfMeasurementAudits$ = this.store.select(getUnitOfMeasurementModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<UnitOfMeasurementModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.UnitOfMeasurementAction(
			modelAction.UnitOfMeasurementModelActionTypes.FETCH_UNIT_OF_MEASUREMENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getUnitOfMeasurementModelWithId, this.targetModelId)
			...
			model = new UnitOfMeasurementModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('unit-of-measurement');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.UnitOfMeasurementAction(
			modelAction.UnitOfMeasurementModelActionTypes.FETCH_UNIT_OF_MEASUREMENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<UnitOfMeasurementModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.catalogueUOMMultiCrudActive || this.compoundPrescriptionDetailsMultiCrudActive || this.compoundPrescriptionItemsMultiCrudActive || this.compoundsUnitMultiCrudActive || this.medicationCompoundsMultiCrudActive || this.medicationItemsMultiCrudActive || this.prescriptionItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.UnitOfMeasurementAction(
						modelAction.UnitOfMeasurementModelActionTypes.COUNT_UNIT_OF_MEASUREMENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.UnitOfMeasurementAction(
				modelAction.UnitOfMeasurementModelActionTypes.CREATE_UNIT_OF_MEASUREMENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.UnitOfMeasurementAction(
				modelAction.UnitOfMeasurementModelActionTypes.UPDATE_UNIT_OF_MEASUREMENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: UnitOfMeasurementModel = new UnitOfMeasurementModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getUnitOfMeasurementModelWithId, this.targetModelId).subscribe(model => {
					let differences = new UnitOfMeasurementModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.catalogueUOMDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(CatalogueUOMModel.getProps(), this.catalogueUOMRelations, this.isViewOnly),
					new CatalogueUOMModel(),
					CatalogueUOMModel,
					this.catalogueUOMDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.catalogueUOMRelations,
					true
				);
			...
			if (event.entityName === this.compoundPrescriptionDetailsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(CompoundPrescriptionDetailModel.getProps(), this.compoundPrescriptionDetailsRelations, this.isViewOnly),
					new CompoundPrescriptionDetailModel(),
					CompoundPrescriptionDetailModel,
					this.compoundPrescriptionDetailsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.compoundPrescriptionDetailsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.compoundsUnitDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PrescriptionCompoundModel.getProps(), this.compoundsUnitRelations, this.isViewOnly),
					new PrescriptionCompoundModel(),
					PrescriptionCompoundModel,
					this.compoundsUnitDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.compoundsUnitRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.medicationCompoundsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MedicationCompoundModel.getProps(), this.medicationCompoundsRelations, this.isViewOnly),
					new MedicationCompoundModel(),
					MedicationCompoundModel,
					this.medicationCompoundsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.medicationCompoundsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.prescriptionItemsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PrescriptionItemModel.getProps(), this.prescriptionItemsRelations, this.isViewOnly),
					new PrescriptionItemModel(),
					PrescriptionItemModel,
					this.prescriptionItemsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.prescriptionItemsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = UnitOfMeasurementModel.getProps();
		this.modelRelations = UnitOfMeasurementModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'catalogueUOM';
		}).hideElement = this.catalogueUOMMultiCrudActive || this.modelRelations.find(rel => rel.id === 'catalogueUOM').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'compoundPrescriptionDetails';
		}).hideElement = this.compoundPrescriptionDetailsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'compoundPrescriptionDetails').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'compoundsUnit';
		}).hideElement = this.compoundsUnitMultiCrudActive || this.modelRelations.find(rel => rel.id === 'compoundsUnit').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'medicationCompounds';
		}).hideElement = this.medicationCompoundsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'medicationCompounds').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'prescriptionItems';
		}).hideElement = this.prescriptionItemsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'prescriptionItems').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.catalogueUOMMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.catalogueUOMDisplayName, many: false});
		}

		if (this.compoundPrescriptionDetailsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.compoundPrescriptionDetailsDisplayName, many: true});
			this.store.dispatch(new compoundPrescriptionDetailModelAction.CompoundPrescriptionDetailAction(
				compoundPrescriptionDetailModelAction.CompoundPrescriptionDetailModelActionTypes.INITIALISE_COMPOUND_PRESCRIPTION_DETAIL_COLLECTION_STATE,
				{
					collectionId: this.compoundPrescriptionDetailsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getCompoundPrescriptionDetailCollectionModels, this.compoundPrescriptionDetailsCollectionId).subscribe(models => {
				models.forEach(compoundPrescriptionDetails => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(CompoundPrescriptionDetailModel.getProps(), this.compoundPrescriptionDetailsRelations, this.isViewOnly),
		...
		if (this.compoundsUnitMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.compoundsUnitDisplayName, many: true});
			this.store.dispatch(new prescriptionCompoundModelAction.PrescriptionCompoundAction(
				prescriptionCompoundModelAction.PrescriptionCompoundModelActionTypes.INITIALISE_PRESCRIPTION_COMPOUND_COLLECTION_STATE,
				{
					collectionId: this.compoundsUnitCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getPrescriptionCompoundCollectionModels, this.compoundsUnitCollectionId).subscribe(models => {
				models.forEach(compoundsUnit => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(PrescriptionCompoundModel.getProps(), this.compoundsUnitRelations, this.isViewOnly),
						compoundsUnit,
						PrescriptionCompoundModel,
		...
		if (this.medicationCompoundsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.medicationCompoundsDisplayName, many: true});
			this.store.dispatch(new medicationCompoundModelAction.MedicationCompoundAction(
				medicationCompoundModelAction.MedicationCompoundModelActionTypes.INITIALISE_MEDICATION_COMPOUND_COLLECTION_STATE,
				{
					collectionId: this.medicationCompoundsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getMedicationCompoundCollectionModels, this.medicationCompoundsCollectionId).subscribe(models => {
				models.forEach(medicationCompounds => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(MedicationCompoundModel.getProps(), this.medicationCompoundsRelations, this.isViewOnly),
						medicationCompounds,
						MedicationCompoundModel,
		...
		if (this.prescriptionItemsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.prescriptionItemsDisplayName, many: true});
			this.store.dispatch(new prescriptionItemModelAction.PrescriptionItemAction(
				...
				{
					collectionId: this.prescriptionItemsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getPrescriptionItemCollectionModels, this.prescriptionItemsCollectionId).subscribe(models => {
				models.forEach(prescriptionItems => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(PrescriptionItemModel.getProps(), this.prescriptionItemsRelations, this.isViewOnly),
						prescriptionItems,
						PrescriptionItemModel,
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.catalogueUOM && this.targetModel.catalogueUOM.id && this.catalogueUOMMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				CatalogueUOMModel,
				this.catalogueUOMOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.catalogueUOM.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getCatalogueUOMModelWithId, this.targetModel.catalogueUOMId).subscribe(catalogueUOM => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(CatalogueUOMModel.getProps(), this.catalogueUOMRelations, this.isViewOnly),
					catalogueUOM,
					CatalogueUOMModel,
		...
		if (this.targetModel.compoundPrescriptionDetailsIds && this.targetModel.compoundPrescriptionDetailsIds.length > 0 && this.compoundPrescriptionDetailsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				CompoundPrescriptionDetailModel,
				this.compoundPrescriptionDetailsOppositeRelation,
				this.compoundPrescriptionDetailsCollectionId,
				this.targetModel.compoundPrescriptionDetailsIds.length,
				this.targetModel.id,
				true
			));
		}
		...
		if (this.targetModel.compoundsUnitIds && this.targetModel.compoundsUnitIds.length > 0 && this.compoundsUnitMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PrescriptionCompoundModel,
				this.compoundsUnitOppositeRelation,
				this.compoundsUnitCollectionId,
				this.targetModel.compoundsUnitIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.medicationCompoundsIds && this.targetModel.medicationCompoundsIds.length > 0 && this.medicationCompoundsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				MedicationCompoundModel,
				this.medicationCompoundsOppositeRelation,
				this.medicationCompoundsCollectionId,
				this.targetModel.medicationCompoundsIds.length,
				this.targetModel.id,
				true
			));
		}
		...
		if (this.targetModel.prescriptionItemsIds && this.targetModel.prescriptionItemsIds.length > 0 && this.prescriptionItemsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PrescriptionItemModel,
				this.prescriptionItemsOppositeRelation,
				this.prescriptionItemsCollectionId,
				this.targetModel.prescriptionItemsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: UnitOfMeasurementModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): UnitOfMeasurementModel {
		let updatedModel: UnitOfMeasurementModel = new UnitOfMeasurementModel(model);

		if (this.catalogueUOMMultiCrudActive) {
			updatedModel.catalogueUOM = event.additionalEntities.catalogueUOM ? event.additionalEntities.catalogueUOM : undefined;

			if (!updatedModel.catalogueUOM && event.payload) {
				event.payload.catalogueUOMId = null;
			}
		} else if (event.payload?.catalogueUOMId !== undefined) {
			event.payload.catalogueUOM = null;
			updatedModel.catalogueUOM = null;
		}

		updatedModel.compoundPrescriptionDetails = [];
		if (this.compoundPrescriptionDetailsMultiCrudActive) {
			if (event.additionalEntities.compoundPrescriptionDetails) {
				event.additionalEntities.compoundPrescriptionDetails.forEach(entity => {
					updatedModel.compoundPrescriptionDetails.push(entity);
				});
			}

			if (!updatedModel.compoundPrescriptionDetails.length) {
				updatedModel.compoundPrescriptionDetailsIds = [];
			}
		}
		...
		updatedModel.compoundsUnit = [];
		if (this.compoundsUnitMultiCrudActive) {
			if (event.additionalEntities.compoundsUnit) {
				event.additionalEntities.compoundsUnit.forEach(entity => {
					updatedModel.compoundsUnit.push(entity);
				});
			}

			if (!updatedModel.compoundsUnit.length) {
				updatedModel.compoundsUnitIds = [];
			}
		}
		updatedModel.medicationCompounds = [];
		if (this.medicationCompoundsMultiCrudActive) {
			if (event.additionalEntities.medicationCompounds) {
				event.additionalEntities.medicationCompounds.forEach(entity => {
					updatedModel.medicationCompounds.push(entity);
				});
			}

			if (!updatedModel.medicationCompounds.length) {
				updatedModel.medicationCompoundsIds = [];
			}
		}
		...
		updatedModel.prescriptionItems = [];
		if (this.prescriptionItemsMultiCrudActive) {
			if (event.additionalEntities.prescriptionItems) {
				event.additionalEntities.prescriptionItems.forEach(entity => {
					updatedModel.prescriptionItems.push(entity);
				});
			}

			if (!updatedModel.prescriptionItems.length) {
				updatedModel.prescriptionItemsIds = [];
			}
		}
		return updatedModel;
	}
