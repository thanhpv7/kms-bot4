...
import {AdminBedReserveCrudListComponent} from './bed-reserve-admin-crud-list.component';
...
import {AdminBedReserveCrudModule} from '../bed-reserve-admin-crud.module';
import {
	BedReserveModelState,
	initialState as initialBedReserveModelState
} from 'src/app/models/bedReserve/bed_reserve.model.state';
 ...
 */
describe('Bed Reserve Crud List Tile Component', () => {

	let fixture;
	let bedReserveCrudListComponent: AdminBedReserveCrudListComponent;

	let store: MockStore<{ model: BedReserveModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bedReserveModelState: BedReserveModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BedReserveModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '7af14875-d8f3-424a-9f0e-2e6b39b1fcb8';
	const searchContent = 'text';

	const searchableFieldCount = BedReserveModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bed-reserve-admin-crud',
		urls: ['bed-reserve-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bedReserveDataFactory = new BedReserveDataFactory();

	const defaultDataSize = 30;
	const defaultData = bedReserveDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BedReserveModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bedReserveModelState = initialBedReserveModelState;
		store.setState({model: bedReserveModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBedReserveCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBedReserveCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBedReserveCrudListComponent);
			bedReserveCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bedReserveCrudListComponent.bedReservesCount).toBe(defaultData.length);
			...
			expect(bedReserveCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bedReserveCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bedReserveDataFactory.createAll(filteredDataSize);
			...
			expect(bedReserveCrudListComponent.bedReservesCount).toBe(filteredDataSize);
			expect(bedReserveCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bedReserveCrudListComponent.pageIndex).toBe(0);
			expect(bedReserveCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bedReserveCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bedReserveCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bedReserveCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
