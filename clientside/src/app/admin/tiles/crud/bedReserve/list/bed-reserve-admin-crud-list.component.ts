...
import * as modelAction from 'src/app/models/bedReserve/bed_reserve.model.action';
import {BedReserveModelState} from 'src/app/models/bedReserve/bed_reserve.model.state';
...
import {
	getBedReserveCollectionModels,
	getBedReserveCollectionCount
...
import {CrudTileMode} from '../bed-reserve-admin-crud.component';
...
@Component({
	selector: 'cb-bed-reserve-admin-crud-list',
	templateUrl: './bed-reserve-admin-crud-list.component.html',
	styleUrls: [
		'./bed-reserve-admin-crud-list.component.scss',
...
export class AdminBedReserveCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BedReserveModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BedReserveModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BedReserveModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '7af14875-d8f3-424a-9f0e-2e6b39b1fcb8';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bedReservesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bedReservesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BedReserveModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BedReserveAction(
				modelAction.BedReserveModelActionTypes.INITIALISE_BED_RESERVE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BedReserveAction(
				modelAction.BedReserveModelActionTypes.FETCH_BED_RESERVE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BedReserveAction(
						modelAction.BedReserveModelActionTypes.COUNT_BED_RESERVES,
						null
					)
				]
			))
		...
		this.bedReserves = this.store.select(getBedReserveCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBedReserveCollectionCount, this.collectionId).subscribe(
				(count) => this.bedReservesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BedReserveModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BedReserveModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BedReserveModelActionTypes.EXPORT_ALL_BED_RESERVE;
		} else {
			actionType = modelAction.BedReserveModelActionTypes.EXPORT_BED_RESERVE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BedReserveAction(
				modelAction.BedReserveModelActionTypes.IMPORT_BED_RESERVES,
				{
					file: file,
				},
				[new modelAction.BedReserveAction(
			...
			window.open(environment.API_URL + '/api/bedreserve/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BedReserveModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Bed Reserve entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Bed Reserve entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BedReserveModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BedReserveModelActionTypes.DELETE_BED_RESERVE;
						} else if (usingSelected) {
							actionType = modelAction.BedReserveModelActionTypes.DELETE_ALL_BED_RESERVE;
						} else {
							actionType = modelAction.BedReserveModelActionTypes.DELETE_BED_RESERVE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BedReserveAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BED_RESERVE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BED_RESERVE and DELETE_BED_RESERVE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BedReserveModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BedReserveModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bedReservesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
