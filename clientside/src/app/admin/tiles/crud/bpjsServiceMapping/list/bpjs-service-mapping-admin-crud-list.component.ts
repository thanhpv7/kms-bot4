...
import {BpjsServiceMappingModelState} from 'src/app/models/bpjsServiceMapping/bpjs_service_mapping.model.state';
...
import {
	...
	getBpjsServiceMappingCollectionCount
...
import {CrudTileMode} from '../bpjs-service-mapping-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-service-mapping-admin-crud-list',
	templateUrl: './bpjs-service-mapping-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-service-mapping-admin-crud-list.component.scss',
...
export class AdminBpjsServiceMappingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsServiceMappingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsServiceMappingModel.searchFields,
	...
	/**
	 * List of BPJS Service Mappings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsServiceMappings: Observable<BpjsServiceMappingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsServiceMappingsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsServiceMappingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsServiceMappingsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsServiceMappingAction(
				modelAction.BpjsServiceMappingModelActionTypes.INITIALISE_BPJS_SERVICE_MAPPING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsServiceMappingAction(
				modelAction.BpjsServiceMappingModelActionTypes.FETCH_BPJS_SERVICE_MAPPING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsServiceMappings = this.store.select(getBpjsServiceMappingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsServiceMappingCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsServiceMappingsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsServiceMappingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsServiceMappingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsServiceMappingModelActionTypes.EXPORT_ALL_BPJS_SERVICE_MAPPING;
		} else {
			actionType = modelAction.BpjsServiceMappingModelActionTypes.EXPORT_BPJS_SERVICE_MAPPING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsServiceMappingAction(
				modelAction.BpjsServiceMappingModelActionTypes.IMPORT_BPJS_SERVICE_MAPPINGS,
				{
					file: file,
				},
				[new modelAction.BpjsServiceMappingAction(
			...
			window.open(environment.API_URL + '/api/bpjsservicemapping/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsServiceMappingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Service Mapping entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Service Mapping entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsServiceMappingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsServiceMappingModelActionTypes.DELETE_BPJS_SERVICE_MAPPING;
						} else if (usingSelected) {
							actionType = modelAction.BpjsServiceMappingModelActionTypes.DELETE_ALL_BPJS_SERVICE_MAPPING;
						} else {
							actionType = modelAction.BpjsServiceMappingModelActionTypes.DELETE_BPJS_SERVICE_MAPPING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsServiceMappingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SERVICE_MAPPING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SERVICE_MAPPING and DELETE_BPJS_SERVICE_MAPPING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsServiceMappingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsServiceMappingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
