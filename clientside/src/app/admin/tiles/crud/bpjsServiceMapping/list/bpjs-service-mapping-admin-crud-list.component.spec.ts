...
import {AdminBpjsServiceMappingCrudListComponent} from './bpjs-service-mapping-admin-crud-list.component';
...
import {AdminBpjsServiceMappingCrudModule} from '../bpjs-service-mapping-admin-crud.module';
import {
 ...
 */
describe('BPJS Service Mapping Crud List Tile Component', () => {

	let fixture;
	let bpjsServiceMappingCrudListComponent: AdminBpjsServiceMappingCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsServiceMappingModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd5ff13f7-d3ed-4261-8318-e7195a828b48';
	const searchContent = 'text';

	const searchableFieldCount = BpjsServiceMappingModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-service-mapping-admin-crud',
		urls: ['bpjs-service-mapping-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsServiceMappingDataFactory = new BpjsServiceMappingDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsServiceMappingDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsServiceMappingCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsServiceMappingCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsServiceMappingCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsServiceMappingCrudListComponent);
			bpjsServiceMappingCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsServiceMappingCrudListComponent.bpjsServiceMappingsCount).toBe(defaultData.length);
			...
			expect(bpjsServiceMappingCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsServiceMappingCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsServiceMappingDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsServiceMappingCrudListComponent.bpjsServiceMappingsCount).toBe(filteredDataSize);
			expect(bpjsServiceMappingCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsServiceMappingCrudListComponent.pageIndex).toBe(0);
			expect(bpjsServiceMappingCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsServiceMappingCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsServiceMappingCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsServiceMappingCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
