...
import * as modelAction from 'src/app/models/administrator/administrator.model.action';
import {AdministratorModelState} from 'src/app/models/administrator/administrator.model.state';
...
import {
	getAdministratorCollectionModels,
	getAdministratorCollectionCount
...
import {CrudTileMode} from '../administrator-admin-crud.component';
...
@Component({
	selector: 'cb-administrator-admin-crud-list',
	templateUrl: './administrator-admin-crud-list.component.html',
	styleUrls: [
		'./administrator-admin-crud-list.component.scss',
...
export class AdminAdministratorCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<AdministratorModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...AdministratorModel.searchFields,
	...
	/**
	 * List of Administrators that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	administrators: Observable<AdministratorModel[]>;
	 ...
	 */
	modelProperties: ModelProperty[] = AdministratorModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = AdministratorModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'b8aa11cb-79d9-4217-98da-82e37b6ae2cf';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.administratorsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.administratorsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: AdministratorModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.AdministratorAction(
				modelAction.AdministratorModelActionTypes.INITIALISE_ADMINISTRATOR_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.AdministratorAction(
				modelAction.AdministratorModelActionTypes.FETCH_ADMINISTRATOR_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.AdministratorAction(
						modelAction.AdministratorModelActionTypes.COUNT_ADMINISTRATORS,
						null
					)
				]
			))
		...
		this.administrators = this.store.select(getAdministratorCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getAdministratorCollectionCount, this.collectionId).subscribe(
				(count) => this.administratorsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: AdministratorModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.AdministratorModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.AdministratorModelActionTypes.EXPORT_ALL_ADMINISTRATOR;
		} else {
			actionType = modelAction.AdministratorModelActionTypes.EXPORT_ADMINISTRATOR_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.AdministratorAction(
				modelAction.AdministratorModelActionTypes.IMPORT_ADMINISTRATORS,
				{
					file: file,
				},
				[new modelAction.AdministratorAction(
			...
			window.open(environment.API_URL + '/api/administrator/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: AdministratorModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Administrator entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Administrator entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.AdministratorModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.AdministratorModelActionTypes.DELETE_ADMINISTRATOR;
						} else if (usingSelected) {
							actionType = modelAction.AdministratorModelActionTypes.DELETE_ALL_ADMINISTRATOR;
						} else {
							actionType = modelAction.AdministratorModelActionTypes.DELETE_ADMINISTRATOR_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.AdministratorAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ADMINISTRATOR when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ADMINISTRATOR and DELETE_ADMINISTRATOR_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<AdministratorModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = AdministratorModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.administratorsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
