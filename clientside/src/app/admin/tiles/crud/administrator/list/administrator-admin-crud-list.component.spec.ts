...
import {AdminAdministratorCrudListComponent} from './administrator-admin-crud-list.component';
...
import {AdminAdministratorCrudModule} from '../administrator-admin-crud.module';
import {
	AdministratorModelState,
	initialState as initialAdministratorModelState
} from 'src/app/models/administrator/administrator.model.state';
 ...
 */
describe('Administrator Crud List Tile Component', () => {

	let fixture;
	let administratorCrudListComponent: AdminAdministratorCrudListComponent;

	let store: MockStore<{ model: AdministratorModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let administratorModelState: AdministratorModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<AdministratorModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b8aa11cb-79d9-4217-98da-82e37b6ae2cf';
	const searchContent = 'text';

	const searchableFieldCount = AdministratorModel.searchFields.length;

	const routerState: RouterState = {
		url: 'administrator-admin-crud',
		urls: ['administrator-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const administratorDataFactory = new AdministratorDataFactory();

	const defaultDataSize = 30;
	const defaultData = administratorDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: AdministratorModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		administratorModelState = initialAdministratorModelState;
		store.setState({model: administratorModelState});
		...
		spyOn(store, 'select')
			.withArgs(getAdministratorCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getAdministratorCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminAdministratorCrudListComponent);
			administratorCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(administratorCrudListComponent.administratorsCount).toBe(defaultData.length);
			...
			expect(administratorCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			administratorCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = administratorDataFactory.createAll(filteredDataSize);
			...
			expect(administratorCrudListComponent.administratorsCount).toBe(filteredDataSize);
			expect(administratorCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(administratorCrudListComponent.pageIndex).toBe(0);
			expect(administratorCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			administratorCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		administratorCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		administratorCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
