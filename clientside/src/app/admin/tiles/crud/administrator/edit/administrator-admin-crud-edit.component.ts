...
import {AdministratorModelState, AdministratorModelAudit} from 'src/app/models/administrator/administrator.model.state';
import {AdministratorModel} from 'src/app/models/administrator/administrator.model';
import {
	getAdministratorModelAuditsByEntityId,
	getAdministratorModelWithId,
	getCountAdministratorModels,
...
@Component({
	selector: 'cb-administrator-admin-crud-edit',
	templateUrl: './administrator-admin-crud-edit.component.html',
	styleUrls: [
		'./administrator-admin-crud-edit.component.scss',
...
export class AdminAdministratorCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	administratorAudits$: Observable<AdministratorModelAudit[]>;
	...
	staffOppositeRelation = 'administrator';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = AdministratorModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: AdministratorModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountAdministratorModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new AdministratorModel();
		...
		if (this.targetModelId) {
			this.administratorAudits$ = this.store.select(getAdministratorModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<AdministratorModel> = {
		...
		this.store.dispatch(new modelAction.AdministratorAction(
			modelAction.AdministratorModelActionTypes.FETCH_ADMINISTRATOR,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getAdministratorModelWithId, this.targetModelId)
			...
			model = new AdministratorModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.AdministratorAction(
			modelAction.AdministratorModelActionTypes.FETCH_ADMINISTRATOR_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.AdministratorAction(
				modelAction.AdministratorModelActionTypes.CREATE_ADMINISTRATOR,
				stateConfig,
			...
			this.store.dispatch(new modelAction.AdministratorAction(
				modelAction.AdministratorModelActionTypes.UPDATE_ADMINISTRATOR,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: AdministratorModel = new AdministratorModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getAdministratorModelWithId, this.targetModelId).subscribe(model => {
					let differences = new AdministratorModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = AdministratorModel.getProps();
		this.modelRelations = AdministratorModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: AdministratorModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): AdministratorModel {
		let updatedModel: AdministratorModel = new AdministratorModel(model);
