...
import {AdminPatientSupportingDocumentCrudListComponent} from './patient-supporting-document-admin-crud-list.component';
...
import {AdminPatientSupportingDocumentCrudModule} from '../patient-supporting-document-admin-crud.module';
import {
	PatientSupportingDocumentModelState,
	initialState as initialPatientSupportingDocumentModelState
} from 'src/app/models/patientSupportingDocument/patient_supporting_document.model.state';
 ...
 */
describe('Patient Supporting Document Crud List Tile Component', () => {

	let fixture;
	let patientSupportingDocumentCrudListComponent: AdminPatientSupportingDocumentCrudListComponent;

	let store: MockStore<{ model: PatientSupportingDocumentModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientSupportingDocumentModelState: PatientSupportingDocumentModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientSupportingDocumentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '538e23e8-1e49-434d-8066-6ce69790a365';
	const searchContent = 'text';

	const searchableFieldCount = PatientSupportingDocumentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-supporting-document-admin-crud',
		urls: ['patient-supporting-document-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientSupportingDocumentDataFactory = new PatientSupportingDocumentDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientSupportingDocumentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientSupportingDocumentModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientSupportingDocumentModelState = initialPatientSupportingDocumentModelState;
		store.setState({model: patientSupportingDocumentModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientSupportingDocumentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientSupportingDocumentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientSupportingDocumentCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientSupportingDocumentCrudListComponent);
			patientSupportingDocumentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientSupportingDocumentCrudListComponent.patientSupportingDocumentsCount).toBe(defaultData.length);
			...
			expect(patientSupportingDocumentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientSupportingDocumentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientSupportingDocumentDataFactory.createAll(filteredDataSize);
			...
			expect(patientSupportingDocumentCrudListComponent.patientSupportingDocumentsCount).toBe(filteredDataSize);
			expect(patientSupportingDocumentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientSupportingDocumentCrudListComponent.pageIndex).toBe(0);
			expect(patientSupportingDocumentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientSupportingDocumentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientSupportingDocumentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientSupportingDocumentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
