...
import {PatientSupportingDocumentModelState} from 'src/app/models/patientSupportingDocument/patient_supporting_document.model.state';
...
import {
	getPatientSupportingDocumentCollectionModels,
	getPatientSupportingDocumentCollectionCount
...
@Component({
	selector: 'cb-patient-supporting-document-admin-crud-list',
	templateUrl: './patient-supporting-document-admin-crud-list.component.html',
	styleUrls: [
		'./patient-supporting-document-admin-crud-list.component.scss',
...
export class AdminPatientSupportingDocumentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientSupportingDocumentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientSupportingDocumentModel.searchFields,
	...
	/**
	 * List of Patient Supporting Documents that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientSupportingDocuments: Observable<PatientSupportingDocumentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientSupportingDocumentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientSupportingDocumentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientSupportingDocumentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
				modelAction.PatientSupportingDocumentModelActionTypes.INITIALISE_PATIENT_SUPPORTING_DOCUMENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
				modelAction.PatientSupportingDocumentModelActionTypes.FETCH_PATIENT_SUPPORTING_DOCUMENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientSupportingDocuments = this.store.select(getPatientSupportingDocumentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientSupportingDocumentCollectionCount, this.collectionId).subscribe(
				(count) => this.patientSupportingDocumentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientSupportingDocumentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientSupportingDocumentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientSupportingDocumentModelActionTypes.EXPORT_ALL_PATIENT_SUPPORTING_DOCUMENT;
		} else {
			actionType = modelAction.PatientSupportingDocumentModelActionTypes.EXPORT_PATIENT_SUPPORTING_DOCUMENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
				modelAction.PatientSupportingDocumentModelActionTypes.IMPORT_PATIENT_SUPPORTING_DOCUMENTS,
				{
					file: file,
				},
				[new modelAction.PatientSupportingDocumentAction(
			...
			window.open(environment.API_URL + '/api/patientsupportingdocument/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientSupportingDocumentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Supporting Document entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Supporting Document entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientSupportingDocumentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientSupportingDocumentModelActionTypes.DELETE_PATIENT_SUPPORTING_DOCUMENT;
						} else if (usingSelected) {
							actionType = modelAction.PatientSupportingDocumentModelActionTypes.DELETE_ALL_PATIENT_SUPPORTING_DOCUMENT;
						} else {
							actionType = modelAction.PatientSupportingDocumentModelActionTypes.DELETE_PATIENT_SUPPORTING_DOCUMENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_SUPPORTING_DOCUMENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_SUPPORTING_DOCUMENT and DELETE_PATIENT_SUPPORTING_DOCUMENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientSupportingDocumentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientSupportingDocumentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
