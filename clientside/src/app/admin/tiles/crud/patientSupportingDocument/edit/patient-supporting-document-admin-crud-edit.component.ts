...
import * as modelAction from 'src/app/models/patientSupportingDocument/patient_supporting_document.model.action';
import {PatientSupportingDocumentModelState, PatientSupportingDocumentModelAudit} from 'src/app/models/patientSupportingDocument/patient_supporting_document.model.state';
import {PatientSupportingDocumentModel} from 'src/app/models/patientSupportingDocument/patient_supporting_document.model';
import {
	getPatientSupportingDocumentModelAuditsByEntityId,
	getPatientSupportingDocumentModelWithId,
	getCountPatientSupportingDocumentModels,
...
import {CrudTileMode} from '../patient-supporting-document-admin-crud.component';
...
@Component({
	selector: 'cb-patient-supporting-document-admin-crud-edit',
	templateUrl: './patient-supporting-document-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-supporting-document-admin-crud-edit.component.scss',
...
export class AdminPatientSupportingDocumentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '538e23e8-1e49-434d-8066-6ce69790a365';
	 ...
	 */
	patientSupportingDocumentAudits$: Observable<PatientSupportingDocumentModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientFinalizeProfiles';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientSupportingDocumentModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientSupportingDocumentModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientSupportingDocumentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientSupportingDocumentModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientSupportingDocumentModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientSupportingDocumentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientSupportingDocumentModel();
		...
		if (this.targetModelId) {
			this.patientSupportingDocumentAudits$ = this.store.select(getPatientSupportingDocumentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientSupportingDocumentModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
			modelAction.PatientSupportingDocumentModelActionTypes.FETCH_PATIENT_SUPPORTING_DOCUMENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientSupportingDocumentModelWithId, this.targetModelId)
			...
			model = new PatientSupportingDocumentModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-supporting-document');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
			modelAction.PatientSupportingDocumentModelActionTypes.FETCH_PATIENT_SUPPORTING_DOCUMENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientSupportingDocumentModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientSupportingDocumentAction(
						modelAction.PatientSupportingDocumentModelActionTypes.COUNT_PATIENT_SUPPORTING_DOCUMENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
				modelAction.PatientSupportingDocumentModelActionTypes.CREATE_PATIENT_SUPPORTING_DOCUMENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientSupportingDocumentAction(
				modelAction.PatientSupportingDocumentModelActionTypes.UPDATE_PATIENT_SUPPORTING_DOCUMENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientSupportingDocumentModel = new PatientSupportingDocumentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientSupportingDocumentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientSupportingDocumentModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientSupportingDocumentModel.getProps();
		this.modelRelations = PatientSupportingDocumentModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientSupportingDocumentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientSupportingDocumentModel {
		let updatedModel: PatientSupportingDocumentModel = new PatientSupportingDocumentModel(model);
