...
import {PcareNonCompoundDrugModelState, PcareNonCompoundDrugModelAudit} from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model.state';
import {PcareNonCompoundDrugModel} from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model';
import {
	getPcareNonCompoundDrugModelAuditsByEntityId,
	getPcareNonCompoundDrugModelWithId,
	getCountPcareNonCompoundDrugModels,
...
@Component({
	selector: 'cb-pcare-noncompound-drug-admin-crud-edit',
	templateUrl: './pcare-noncompound-drug-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-noncompound-drug-admin-crud-edit.component.scss',
...
export class AdminPcareNonCompoundDrugCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareNonCompoundDrugAudits$: Observable<PcareNonCompoundDrugModelAudit[]>;
	...
	pcareExaminationOppositeRelation = 'pcareNonCompoundDrugs';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareNonCompoundDrugModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PcareNonCompoundDrugModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareNonCompoundDrugModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareNonCompoundDrugModel();
		...
		if (this.targetModelId) {
			this.pcareNonCompoundDrugAudits$ = this.store.select(getPcareNonCompoundDrugModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareNonCompoundDrugModel> = {
		...
		this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
			modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_PCARE_NONCOMPOUND_DRUG,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareNonCompoundDrugModelWithId, this.targetModelId)
			...
			model = new PcareNonCompoundDrugModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
			modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_PCARE_NONCOMPOUND_DRUG_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
				modelAction.PcareNonCompoundDrugModelActionTypes.CREATE_PCARE_NONCOMPOUND_DRUG,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
				modelAction.PcareNonCompoundDrugModelActionTypes.UPDATE_PCARE_NONCOMPOUND_DRUG,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareNonCompoundDrugModel = new PcareNonCompoundDrugModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareNonCompoundDrugModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareNonCompoundDrugModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareNonCompoundDrugModel.getProps();
		this.modelRelations = PcareNonCompoundDrugModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareNonCompoundDrugModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareNonCompoundDrugModel {
		let updatedModel: PcareNonCompoundDrugModel = new PcareNonCompoundDrugModel(model);
