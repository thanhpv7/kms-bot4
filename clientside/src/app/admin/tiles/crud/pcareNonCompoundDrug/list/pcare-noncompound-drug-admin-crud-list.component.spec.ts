...
import {AdminPcareNonCompoundDrugCrudListComponent} from './pcare-noncompound-drug-admin-crud-list.component';
...
import {AdminPcareNonCompoundDrugCrudModule} from '../pcare-noncompound-drug-admin-crud.module';
import {
	PcareNonCompoundDrugModelState,
	initialState as initialPcareNonCompoundDrugModelState
} from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model.state';
 ...
 */
describe('PCare NonCompound Drug Crud List Tile Component', () => {

	let fixture;
	let pcareNonCompoundDrugCrudListComponent: AdminPcareNonCompoundDrugCrudListComponent;

	let store: MockStore<{ model: PcareNonCompoundDrugModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareNonCompoundDrugModelState: PcareNonCompoundDrugModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareNonCompoundDrugModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cf573633-cb34-46cc-8872-1791fe165b69';
	const searchContent = 'text';

	const searchableFieldCount = PcareNonCompoundDrugModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-noncompound-drug-admin-crud',
		urls: ['pcare-noncompound-drug-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareNonCompoundDrugDataFactory = new PcareNonCompoundDrugDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareNonCompoundDrugDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareNonCompoundDrugModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareNonCompoundDrugModelState = initialPcareNonCompoundDrugModelState;
		store.setState({model: pcareNonCompoundDrugModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareNonCompoundDrugCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareNonCompoundDrugCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPcareNonCompoundDrugCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPcareNonCompoundDrugCrudListComponent);
			pcareNonCompoundDrugCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareNonCompoundDrugCrudListComponent.pcareNonCompoundDrugsCount).toBe(defaultData.length);
			...
			expect(pcareNonCompoundDrugCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareNonCompoundDrugCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareNonCompoundDrugDataFactory.createAll(filteredDataSize);
			...
			expect(pcareNonCompoundDrugCrudListComponent.pcareNonCompoundDrugsCount).toBe(filteredDataSize);
			expect(pcareNonCompoundDrugCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareNonCompoundDrugCrudListComponent.pageIndex).toBe(0);
			expect(pcareNonCompoundDrugCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareNonCompoundDrugCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareNonCompoundDrugCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareNonCompoundDrugCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
