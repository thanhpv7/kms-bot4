...
import * as modelAction from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model.action';
import {PcareNonCompoundDrugModelState} from 'src/app/models/pcareNonCompoundDrug/pcare_noncompound_drug.model.state';
...
import {
	getPcareNonCompoundDrugCollectionModels,
	getPcareNonCompoundDrugCollectionCount
...
import {CrudTileMode} from '../pcare-noncompound-drug-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-noncompound-drug-admin-crud-list',
	templateUrl: './pcare-noncompound-drug-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-noncompound-drug-admin-crud-list.component.scss',
...
export class AdminPcareNonCompoundDrugCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareNonCompoundDrugModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareNonCompoundDrugModel.searchFields,
	...
	/**
	 * List of PCare NonCompound Drugs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareNonCompoundDrugs: Observable<PcareNonCompoundDrugModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareNonCompoundDrugsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PcareNonCompoundDrugModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareNonCompoundDrugModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'cf573633-cb34-46cc-8872-1791fe165b69';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareNonCompoundDrugsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareNonCompoundDrugsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareNonCompoundDrugModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
				modelAction.PcareNonCompoundDrugModelActionTypes.INITIALISE_PCARE_NONCOMPOUND_DRUG_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
				modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_PCARE_NONCOMPOUND_DRUG_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareNonCompoundDrugAction(
						modelAction.PcareNonCompoundDrugModelActionTypes.COUNT_PCARE_NONCOMPOUND_DRUGS,
						null
					)
				]
			))
		...
		this.pcareNonCompoundDrugs = this.store.select(getPcareNonCompoundDrugCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareNonCompoundDrugCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareNonCompoundDrugsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-noncompound-drug');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareNonCompoundDrugModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareNonCompoundDrugModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareNonCompoundDrugModelActionTypes.EXPORT_ALL_PCARE_NONCOMPOUND_DRUG;
		} else {
			actionType = modelAction.PcareNonCompoundDrugModelActionTypes.EXPORT_PCARE_NONCOMPOUND_DRUG_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
				modelAction.PcareNonCompoundDrugModelActionTypes.IMPORT_PCARE_NONCOMPOUND_DRUGS,
				{
					file: file,
				},
				[new modelAction.PcareNonCompoundDrugAction(
			...
			window.open(environment.API_URL + '/api/pcarenoncompounddrug/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareNonCompoundDrugModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare NonCompound Drug entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare NonCompound Drug entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareNonCompoundDrugModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareNonCompoundDrugModelActionTypes.DELETE_PCARE_NONCOMPOUND_DRUG;
						} else if (usingSelected) {
							actionType = modelAction.PcareNonCompoundDrugModelActionTypes.DELETE_ALL_PCARE_NONCOMPOUND_DRUG;
						} else {
							actionType = modelAction.PcareNonCompoundDrugModelActionTypes.DELETE_PCARE_NONCOMPOUND_DRUG_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareNonCompoundDrugAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_NONCOMPOUND_DRUG when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_NONCOMPOUND_DRUG and DELETE_PCARE_NONCOMPOUND_DRUG_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareNonCompoundDrugModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareNonCompoundDrugModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareNonCompoundDrugsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
