...
import {AdminBpjsPcareKegiatanKelompokCrudListComponent} from './bpjs-pcare-kegiatan-kelompok-admin-crud-list.component';
...
import {AdminBpjsPcareKegiatanKelompokCrudModule} from '../bpjs-pcare-kegiatan-kelompok-admin-crud.module';
import {
 ...
 */
describe('BPJS Pcare Kegiatan Kelompok Crud List Tile Component', () => {

	let fixture;
	let bpjsPcareKegiatanKelompokCrudListComponent: AdminBpjsPcareKegiatanKelompokCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPcareKegiatanKelompokModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '99e3b8af-8043-475b-a62c-5d43f79004e2';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPcareKegiatanKelompokModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-pcare-kegiatan-kelompok-admin-crud',
		urls: ['bpjs-pcare-kegiatan-kelompok-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPcareKegiatanKelompokDataFactory = new BpjsPcareKegiatanKelompokDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPcareKegiatanKelompokDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPcareKegiatanKelompokCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPcareKegiatanKelompokCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsPcareKegiatanKelompokCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsPcareKegiatanKelompokCrudListComponent);
			bpjsPcareKegiatanKelompokCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPcareKegiatanKelompokCrudListComponent.bpjsPcareKegiatanKelompoksCount).toBe(defaultData.length);
			...
			expect(bpjsPcareKegiatanKelompokCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPcareKegiatanKelompokCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPcareKegiatanKelompokDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPcareKegiatanKelompokCrudListComponent.bpjsPcareKegiatanKelompoksCount).toBe(filteredDataSize);
			expect(bpjsPcareKegiatanKelompokCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPcareKegiatanKelompokCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPcareKegiatanKelompokCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPcareKegiatanKelompokCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPcareKegiatanKelompokCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPcareKegiatanKelompokCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
