...
import * as modelAction from 'src/app/models/bpjsPcareKegiatanKelompok/bpjs_pcare_kegiatan_kelompok.model.action';
import {BpjsPcareKegiatanKelompokModelState} from 'src/app/models/bpjsPcareKegiatanKelompok/bpjs_pcare_kegiatan_kelompok.model.state';
...
import {
	...
	getBpjsPcareKegiatanKelompokCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-kegiatan-kelompok-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-kegiatan-kelompok-admin-crud-list',
	templateUrl: './bpjs-pcare-kegiatan-kelompok-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-kegiatan-kelompok-admin-crud-list.component.scss',
...
export class AdminBpjsPcareKegiatanKelompokCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcareKegiatanKelompokModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcareKegiatanKelompokModel.searchFields,
	...
	/**
	 * List of BPJS Pcare Kegiatan Kelompoks that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcareKegiatanKelompoks: Observable<BpjsPcareKegiatanKelompokModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcareKegiatanKelompoksCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcareKegiatanKelompokModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcareKegiatanKelompokModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '99e3b8af-8043-475b-a62c-5d43f79004e2';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcareKegiatanKelompoksCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcareKegiatanKelompoksCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcareKegiatanKelompokModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
				modelAction.BpjsPcareKegiatanKelompokModelActionTypes.INITIALISE_BPJS_PCARE_KEGIATAN_KELOMPOK_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
				modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_KEGIATAN_KELOMPOK_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcareKegiatanKelompokAction(
						modelAction.BpjsPcareKegiatanKelompokModelActionTypes.COUNT_BPJS_PCARE_KEGIATAN_KELOMPOKS,
						null
					)
				]
			))
		...
		this.bpjsPcareKegiatanKelompoks = this.store.select(getBpjsPcareKegiatanKelompokCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcareKegiatanKelompokCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcareKegiatanKelompoksCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-kegiatan-kelompok');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcareKegiatanKelompokModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcareKegiatanKelompokModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcareKegiatanKelompokModelActionTypes.EXPORT_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK;
		} else {
			actionType = modelAction.BpjsPcareKegiatanKelompokModelActionTypes.EXPORT_BPJS_PCARE_KEGIATAN_KELOMPOK_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
				modelAction.BpjsPcareKegiatanKelompokModelActionTypes.IMPORT_BPJS_PCARE_KEGIATAN_KELOMPOKS,
				{
					file: file,
				},
				[new modelAction.BpjsPcareKegiatanKelompokAction(
			...
			window.open(environment.API_URL + '/api/bpjspcarekegiatankelompok/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcareKegiatanKelompokModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Kegiatan Kelompok entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Kegiatan Kelompok entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcareKegiatanKelompokModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcareKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_KEGIATAN_KELOMPOK;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcareKegiatanKelompokModelActionTypes.DELETE_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK;
						} else {
							actionType = modelAction.BpjsPcareKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_KEGIATAN_KELOMPOK_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_KEGIATAN_KELOMPOK when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK and DELETE_BPJS_PCARE_KEGIATAN_KELOMPOK_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcareKegiatanKelompokModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcareKegiatanKelompokModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcareKegiatanKelompoksCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
