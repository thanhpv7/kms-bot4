...
import {BpjsPcareKegiatanKelompokModelState, BpjsPcareKegiatanKelompokModelAudit} from 'src/app/models/bpjsPcareKegiatanKelompok/bpjs_pcare_kegiatan_kelompok.model.state';
import {BpjsPcareKegiatanKelompokModel} from 'src/app/models/bpjsPcareKegiatanKelompok/bpjs_pcare_kegiatan_kelompok.model';
import {
	getBpjsPcareKegiatanKelompokModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-pcare-kegiatan-kelompok-admin-crud-edit',
	templateUrl: './bpjs-pcare-kegiatan-kelompok-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-kegiatan-kelompok-admin-crud-edit.component.scss',
...
export class AdminBpjsPcareKegiatanKelompokCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcareKegiatanKelompokAudits$: Observable<BpjsPcareKegiatanKelompokModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcareKegiatanKelompokModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcareKegiatanKelompokModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcareKegiatanKelompokModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcareKegiatanKelompokModel();
		...
		if (this.targetModelId) {
			this.bpjsPcareKegiatanKelompokAudits$ = this.store.select(getBpjsPcareKegiatanKelompokModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcareKegiatanKelompokModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
			modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_KEGIATAN_KELOMPOK,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareKegiatanKelompokModelWithId, this.targetModelId)
			...
			model = new BpjsPcareKegiatanKelompokModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
			modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_KEGIATAN_KELOMPOK_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
				modelAction.BpjsPcareKegiatanKelompokModelActionTypes.CREATE_BPJS_PCARE_KEGIATAN_KELOMPOK,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcareKegiatanKelompokAction(
				modelAction.BpjsPcareKegiatanKelompokModelActionTypes.UPDATE_BPJS_PCARE_KEGIATAN_KELOMPOK,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcareKegiatanKelompokModel = new BpjsPcareKegiatanKelompokModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareKegiatanKelompokModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcareKegiatanKelompokModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcareKegiatanKelompokModel.getProps();
		this.modelRelations = BpjsPcareKegiatanKelompokModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcareKegiatanKelompokModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcareKegiatanKelompokModel {
		let updatedModel: BpjsPcareKegiatanKelompokModel = new BpjsPcareKegiatanKelompokModel(model);
