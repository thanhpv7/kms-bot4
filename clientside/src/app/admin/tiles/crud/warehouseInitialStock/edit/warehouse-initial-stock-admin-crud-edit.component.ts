...
import * as modelAction from 'src/app/models/warehouseInitialStock/warehouse_initial_stock.model.action';
import {WarehouseInitialStockModelState, WarehouseInitialStockModelAudit} from 'src/app/models/warehouseInitialStock/warehouse_initial_stock.model.state';
import {WarehouseInitialStockModel} from 'src/app/models/warehouseInitialStock/warehouse_initial_stock.model';
import {
	getWarehouseInitialStockModelAuditsByEntityId,
	getWarehouseInitialStockModelWithId,
	getCountWarehouseInitialStockModels,
...
import {CrudTileMode} from '../warehouse-initial-stock-admin-crud.component';
...
@Component({
	selector: 'cb-warehouse-initial-stock-admin-crud-edit',
	templateUrl: './warehouse-initial-stock-admin-crud-edit.component.html',
	styleUrls: [
		'./warehouse-initial-stock-admin-crud-edit.component.scss',
...
export class AdminWarehouseInitialStockCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b9feb6a9-f373-4ee7-8312-20d0c9bec91c';
	 ...
	 */
	warehouseInitialStockAudits$: Observable<WarehouseInitialStockModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Location and Warehouse Initial Stock s here] off begin
	locationMultiCrudActive: boolean = false;
	locationDisplayName = 'Location';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Location and Warehouse Initial Stock s here] end
	locationOppositeRelation = 'warehouseInitialStockS';
	locationRelations: ModelRelation[] = this.locationMultiCrudActive ? LocationModel.getRelations()
		.map(relation => {
			if (relation.id === this.locationOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'warehouseInitialStockS';
	...
	warehouseOppositeRelation = 'warehouseInitialStockS';
	 ...
	 */
	modelProperties: ModelProperty[] = WarehouseInitialStockModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = WarehouseInitialStockModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = WarehouseInitialStockModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'location' && this.locationMultiCrudActive) {
	 ...
	 */
	targetModel: WarehouseInitialStockModel;
	...
	constructor(
		private readonly store: Store<{ model: WarehouseInitialStockModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountWarehouseInitialStockModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new WarehouseInitialStockModel();
		...
		if (this.targetModelId) {
			this.warehouseInitialStockAudits$ = this.store.select(getWarehouseInitialStockModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<WarehouseInitialStockModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.WarehouseInitialStockAction(
			modelAction.WarehouseInitialStockModelActionTypes.FETCH_WAREHOUSE_INITIAL_STOCK,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getWarehouseInitialStockModelWithId, this.targetModelId)
			...
			model = new WarehouseInitialStockModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('warehouse-initial-stock');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.WarehouseInitialStockAction(
			modelAction.WarehouseInitialStockModelActionTypes.FETCH_WAREHOUSE_INITIAL_STOCK_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<WarehouseInitialStockModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.locationMultiCrudActive || this.stockCatalogueMultiCrudActive || this.warehouseMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.WarehouseInitialStockAction(
						modelAction.WarehouseInitialStockModelActionTypes.COUNT_WAREHOUSE_INITIAL_STOCKS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.WarehouseInitialStockAction(
				modelAction.WarehouseInitialStockModelActionTypes.CREATE_WAREHOUSE_INITIAL_STOCK,
				stateConfig,
			...
			this.store.dispatch(new modelAction.WarehouseInitialStockAction(
				modelAction.WarehouseInitialStockModelActionTypes.UPDATE_WAREHOUSE_INITIAL_STOCK,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: WarehouseInitialStockModel = new WarehouseInitialStockModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getWarehouseInitialStockModelWithId, this.targetModelId).subscribe(model => {
					let differences = new WarehouseInitialStockModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.locationDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(LocationModel.getProps(), this.locationRelations, this.isViewOnly),
					new LocationModel(),
					LocationModel,
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = WarehouseInitialStockModel.getProps();
		this.modelRelations = WarehouseInitialStockModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'location';
		}).hideElement = this.locationMultiCrudActive || this.modelRelations.find(rel => rel.id === 'location').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.locationMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.locationDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.location && this.targetModel.location.id && this.locationMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				LocationModel,
				this.locationOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.location.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getLocationModelWithId, this.targetModel.locationId).subscribe(location => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(LocationModel.getProps(), this.locationRelations, this.isViewOnly),
					location,
					LocationModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: WarehouseInitialStockModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): WarehouseInitialStockModel {
		let updatedModel: WarehouseInitialStockModel = new WarehouseInitialStockModel(model);

		if (this.locationMultiCrudActive) {
			updatedModel.location = event.additionalEntities.location ? event.additionalEntities.location : undefined;

			if (!updatedModel.location && event.payload) {
				event.payload.locationId = null;
			}
		} else if (event.payload?.locationId !== undefined) {
			event.payload.location = null;
			updatedModel.location = null;
		}
