...
import {AdminWarehouseInitialStockCrudListComponent} from './warehouse-initial-stock-admin-crud-list.component';
...
import {AdminWarehouseInitialStockCrudModule} from '../warehouse-initial-stock-admin-crud.module';
import {
	WarehouseInitialStockModelState,
	initialState as initialWarehouseInitialStockModelState
} from 'src/app/models/warehouseInitialStock/warehouse_initial_stock.model.state';
 ...
 */
describe('Warehouse Initial Stock  Crud List Tile Component', () => {

	let fixture;
	let warehouseInitialStockCrudListComponent: AdminWarehouseInitialStockCrudListComponent;

	let store: MockStore<{ model: WarehouseInitialStockModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let warehouseInitialStockModelState: WarehouseInitialStockModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<WarehouseInitialStockModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b9feb6a9-f373-4ee7-8312-20d0c9bec91c';
	const searchContent = 'text';

	const searchableFieldCount = WarehouseInitialStockModel.searchFields.length;

	const routerState: RouterState = {
		url: 'warehouse-initial-stock-admin-crud',
		urls: ['warehouse-initial-stock-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const warehouseInitialStockDataFactory = new WarehouseInitialStockDataFactory();

	const defaultDataSize = 30;
	const defaultData = warehouseInitialStockDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: WarehouseInitialStockModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		warehouseInitialStockModelState = initialWarehouseInitialStockModelState;
		store.setState({model: warehouseInitialStockModelState});
		...
		spyOn(store, 'select')
			.withArgs(getWarehouseInitialStockCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getWarehouseInitialStockCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminWarehouseInitialStockCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminWarehouseInitialStockCrudListComponent);
			warehouseInitialStockCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(warehouseInitialStockCrudListComponent.warehouseInitialStocksCount).toBe(defaultData.length);
			...
			expect(warehouseInitialStockCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			warehouseInitialStockCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = warehouseInitialStockDataFactory.createAll(filteredDataSize);
			...
			expect(warehouseInitialStockCrudListComponent.warehouseInitialStocksCount).toBe(filteredDataSize);
			expect(warehouseInitialStockCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(warehouseInitialStockCrudListComponent.pageIndex).toBe(0);
			expect(warehouseInitialStockCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			warehouseInitialStockCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		warehouseInitialStockCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		warehouseInitialStockCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
