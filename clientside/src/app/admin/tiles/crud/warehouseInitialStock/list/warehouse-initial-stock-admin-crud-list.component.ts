...
import {WarehouseInitialStockModelState} from 'src/app/models/warehouseInitialStock/warehouse_initial_stock.model.state';
...
import {
	getWarehouseInitialStockCollectionModels,
	getWarehouseInitialStockCollectionCount
...
@Component({
	selector: 'cb-warehouse-initial-stock-admin-crud-list',
	templateUrl: './warehouse-initial-stock-admin-crud-list.component.html',
	styleUrls: [
		'./warehouse-initial-stock-admin-crud-list.component.scss',
...
export class AdminWarehouseInitialStockCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<WarehouseInitialStockModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...WarehouseInitialStockModel.searchFields,
	...
	/**
	 * List of Warehouse Initial Stock s that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	warehouseInitialStocks: Observable<WarehouseInitialStockModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	warehouseInitialStocksCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.warehouseInitialStocksCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.warehouseInitialStocksCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseInitialStockAction(
				modelAction.WarehouseInitialStockModelActionTypes.INITIALISE_WAREHOUSE_INITIAL_STOCK_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseInitialStockAction(
				modelAction.WarehouseInitialStockModelActionTypes.FETCH_WAREHOUSE_INITIAL_STOCK_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.warehouseInitialStocks = this.store.select(getWarehouseInitialStockCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getWarehouseInitialStockCollectionCount, this.collectionId).subscribe(
				(count) => this.warehouseInitialStocksCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: WarehouseInitialStockModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.WarehouseInitialStockModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.WarehouseInitialStockModelActionTypes.EXPORT_ALL_WAREHOUSE_INITIAL_STOCK;
		} else {
			actionType = modelAction.WarehouseInitialStockModelActionTypes.EXPORT_WAREHOUSE_INITIAL_STOCK_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.WarehouseInitialStockAction(
				modelAction.WarehouseInitialStockModelActionTypes.IMPORT_WAREHOUSE_INITIAL_STOCKS,
				{
					file: file,
				},
				[new modelAction.WarehouseInitialStockAction(
			...
			window.open(environment.API_URL + '/api/warehouseinitialstock/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: WarehouseInitialStockModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Warehouse Initial Stock  entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Warehouse Initial Stock  entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.WarehouseInitialStockModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.WarehouseInitialStockModelActionTypes.DELETE_WAREHOUSE_INITIAL_STOCK;
						} else if (usingSelected) {
							actionType = modelAction.WarehouseInitialStockModelActionTypes.DELETE_ALL_WAREHOUSE_INITIAL_STOCK;
						} else {
							actionType = modelAction.WarehouseInitialStockModelActionTypes.DELETE_WAREHOUSE_INITIAL_STOCK_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.WarehouseInitialStockAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_WAREHOUSE_INITIAL_STOCK when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_WAREHOUSE_INITIAL_STOCK and DELETE_WAREHOUSE_INITIAL_STOCK_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = WarehouseInitialStockModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.warehouseInitialStocksCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
