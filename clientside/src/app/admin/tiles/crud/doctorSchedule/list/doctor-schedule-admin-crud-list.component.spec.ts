...
import {AdminDoctorScheduleCrudListComponent} from './doctor-schedule-admin-crud-list.component';
...
import {AdminDoctorScheduleCrudModule} from '../doctor-schedule-admin-crud.module';
import {
 ...
 */
describe('Doctor Schedule Crud List Tile Component', () => {

	let fixture;
	let doctorScheduleCrudListComponent: AdminDoctorScheduleCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<DoctorScheduleModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '3e0921ec-9e66-488b-a98c-7332e0503712';
	const searchContent = 'text';

	const searchableFieldCount = DoctorScheduleModel.searchFields.length;

	const routerState: RouterState = {
		url: 'doctor-schedule-admin-crud',
		urls: ['doctor-schedule-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const doctorScheduleDataFactory = new DoctorScheduleDataFactory();

	const defaultDataSize = 30;
	const defaultData = doctorScheduleDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getDoctorScheduleCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDoctorScheduleCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminDoctorScheduleCrudListComponent);
			doctorScheduleCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(doctorScheduleCrudListComponent.doctorSchedulesCount).toBe(defaultData.length);
			...
			expect(doctorScheduleCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			doctorScheduleCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = doctorScheduleDataFactory.createAll(filteredDataSize);
			...
			expect(doctorScheduleCrudListComponent.doctorSchedulesCount).toBe(filteredDataSize);
			expect(doctorScheduleCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(doctorScheduleCrudListComponent.pageIndex).toBe(0);
			expect(doctorScheduleCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			doctorScheduleCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		doctorScheduleCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		doctorScheduleCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
