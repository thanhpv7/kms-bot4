...
import * as modelAction from 'src/app/models/doctorSchedule/doctor_schedule.model.action';
import {DoctorScheduleModelState} from 'src/app/models/doctorSchedule/doctor_schedule.model.state';
...
import {
	getDoctorScheduleCollectionModels,
	getDoctorScheduleCollectionCount
...
import {CrudTileMode} from '../doctor-schedule-admin-crud.component';
...
@Component({
	selector: 'cb-doctor-schedule-admin-crud-list',
	templateUrl: './doctor-schedule-admin-crud-list.component.html',
	styleUrls: [
		'./doctor-schedule-admin-crud-list.component.scss',
...
export class AdminDoctorScheduleCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DoctorScheduleModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DoctorScheduleModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = DoctorScheduleModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '3e0921ec-9e66-488b-a98c-7332e0503712';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.doctorSchedulesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.doctorSchedulesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: DoctorScheduleModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DoctorScheduleAction(
				modelAction.DoctorScheduleModelActionTypes.INITIALISE_DOCTOR_SCHEDULE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DoctorScheduleAction(
				modelAction.DoctorScheduleModelActionTypes.FETCH_DOCTOR_SCHEDULE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.DoctorScheduleAction(
						modelAction.DoctorScheduleModelActionTypes.COUNT_DOCTOR_SCHEDULES,
						null
					)
				]
			))
		...
		this.doctorSchedules = this.store.select(getDoctorScheduleCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDoctorScheduleCollectionCount, this.collectionId).subscribe(
				(count) => this.doctorSchedulesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DoctorScheduleModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DoctorScheduleModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DoctorScheduleModelActionTypes.EXPORT_ALL_DOCTOR_SCHEDULE;
		} else {
			actionType = modelAction.DoctorScheduleModelActionTypes.EXPORT_DOCTOR_SCHEDULE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DoctorScheduleAction(
				modelAction.DoctorScheduleModelActionTypes.IMPORT_DOCTOR_SCHEDULES,
				{
					file: file,
				},
				[new modelAction.DoctorScheduleAction(
			...
			window.open(environment.API_URL + '/api/doctorschedule/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DoctorScheduleModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Doctor Schedule entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Doctor Schedule entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DoctorScheduleModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DoctorScheduleModelActionTypes.DELETE_DOCTOR_SCHEDULE;
						} else if (usingSelected) {
							actionType = modelAction.DoctorScheduleModelActionTypes.DELETE_ALL_DOCTOR_SCHEDULE;
						} else {
							actionType = modelAction.DoctorScheduleModelActionTypes.DELETE_DOCTOR_SCHEDULE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DoctorScheduleAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DOCTOR_SCHEDULE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DOCTOR_SCHEDULE and DELETE_DOCTOR_SCHEDULE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<DoctorScheduleModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DoctorScheduleModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.doctorSchedulesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
