...
import {DoctorScheduleModelState, DoctorScheduleModelAudit} from 'src/app/models/doctorSchedule/doctor_schedule.model.state';
import {DoctorScheduleModel} from 'src/app/models/doctorSchedule/doctor_schedule.model';
import {
	getDoctorScheduleModelAuditsByEntityId,
	getDoctorScheduleModelWithId,
	getCountDoctorScheduleModels,
...
@Component({
	selector: 'cb-doctor-schedule-admin-crud-edit',
	templateUrl: './doctor-schedule-admin-crud-edit.component.html',
	styleUrls: [
		'./doctor-schedule-admin-crud-edit.component.scss',
...
export class AdminDoctorScheduleCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	doctorScheduleAudits$: Observable<DoctorScheduleModelAudit[]>;
	...
	serviceOppositeRelation = 'doctorSchedules';
	...
	staffOppositeRelation = 'doctorSchedules';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DoctorScheduleModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: DoctorScheduleModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDoctorScheduleModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DoctorScheduleModel();
		...
		if (this.targetModelId) {
			this.doctorScheduleAudits$ = this.store.select(getDoctorScheduleModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DoctorScheduleModel> = {
		...
		this.store.dispatch(new modelAction.DoctorScheduleAction(
			modelAction.DoctorScheduleModelActionTypes.FETCH_DOCTOR_SCHEDULE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDoctorScheduleModelWithId, this.targetModelId)
			...
			model = new DoctorScheduleModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DoctorScheduleAction(
			modelAction.DoctorScheduleModelActionTypes.FETCH_DOCTOR_SCHEDULE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<DoctorScheduleModel> = {
				...
				queryParams: {
					persistRelations: this.serviceMultiCrudActive || this.staffMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.DoctorScheduleAction(
				modelAction.DoctorScheduleModelActionTypes.CREATE_DOCTOR_SCHEDULE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DoctorScheduleAction(
				modelAction.DoctorScheduleModelActionTypes.UPDATE_DOCTOR_SCHEDULE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DoctorScheduleModel = new DoctorScheduleModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDoctorScheduleModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DoctorScheduleModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DoctorScheduleModel.getProps();
		this.modelRelations = DoctorScheduleModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: DoctorScheduleModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DoctorScheduleModel {
		let updatedModel: DoctorScheduleModel = new DoctorScheduleModel(model);
