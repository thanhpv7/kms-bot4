...
import {AdminBpjsSEPInternalDataCrudListComponent} from './bpjs-sep-internal-data-admin-crud-list.component';
...
import {AdminBpjsSEPInternalDataCrudModule} from '../bpjs-sep-internal-data-admin-crud.module';
import {
 ...
 */
describe('BPJS SEP Internal Data Crud List Tile Component', () => {

	let fixture;
	let bpjsSEPInternalDataCrudListComponent: AdminBpjsSEPInternalDataCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsSEPInternalDataModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cb7bf167-e31e-4ed9-96ab-48b7fce0e022';
	const searchContent = 'text';

	const searchableFieldCount = BpjsSEPInternalDataModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-sep-internal-data-admin-crud',
		urls: ['bpjs-sep-internal-data-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsSEPInternalDataDataFactory = new BpjsSEPInternalDataDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsSEPInternalDataDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsSEPInternalDataCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsSEPInternalDataCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsSEPInternalDataCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsSEPInternalDataCrudListComponent);
			bpjsSEPInternalDataCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsSEPInternalDataCrudListComponent.bpjsSEPInternalDatasCount).toBe(defaultData.length);
			...
			expect(bpjsSEPInternalDataCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsSEPInternalDataCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsSEPInternalDataDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsSEPInternalDataCrudListComponent.bpjsSEPInternalDatasCount).toBe(filteredDataSize);
			expect(bpjsSEPInternalDataCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsSEPInternalDataCrudListComponent.pageIndex).toBe(0);
			expect(bpjsSEPInternalDataCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsSEPInternalDataCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsSEPInternalDataCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsSEPInternalDataCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
