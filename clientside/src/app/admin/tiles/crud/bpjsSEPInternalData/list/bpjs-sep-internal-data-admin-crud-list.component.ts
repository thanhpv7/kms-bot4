...
import * as modelAction from 'src/app/models/bpjsSEPInternalData/bpjs_sep_internal_data.model.action';
import {BpjsSEPInternalDataModelState} from 'src/app/models/bpjsSEPInternalData/bpjs_sep_internal_data.model.state';
...
import {
	...
	getBpjsSEPInternalDataCollectionCount
...
import {CrudTileMode} from '../bpjs-sep-internal-data-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-sep-internal-data-admin-crud-list',
	templateUrl: './bpjs-sep-internal-data-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-sep-internal-data-admin-crud-list.component.scss',
...
export class AdminBpjsSEPInternalDataCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsSEPInternalDataModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsSEPInternalDataModel.searchFields,
	...
	/**
	 * List of BPJS SEP Internal Datas that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsSEPInternalDatas: Observable<BpjsSEPInternalDataModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsSEPInternalDatasCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsSEPInternalDataModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsSEPInternalDataModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'cb7bf167-e31e-4ed9-96ab-48b7fce0e022';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsSEPInternalDatasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsSEPInternalDatasCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsSEPInternalDataModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
				modelAction.BpjsSEPInternalDataModelActionTypes.INITIALISE_BPJS_SEP_INTERNAL_DATA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
				modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_BPJS_SEP_INTERNAL_DATA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsSEPInternalDataAction(
						modelAction.BpjsSEPInternalDataModelActionTypes.COUNT_BPJS_SEP_INTERNAL_DATAS,
						null
					)
				]
			))
		...
		this.bpjsSEPInternalDatas = this.store.select(getBpjsSEPInternalDataCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsSEPInternalDataCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsSEPInternalDatasCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-sep-internal-data');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsSEPInternalDataModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsSEPInternalDataModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsSEPInternalDataModelActionTypes.EXPORT_ALL_BPJS_SEP_INTERNAL_DATA;
		} else {
			actionType = modelAction.BpjsSEPInternalDataModelActionTypes.EXPORT_BPJS_SEP_INTERNAL_DATA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
				modelAction.BpjsSEPInternalDataModelActionTypes.IMPORT_BPJS_SEP_INTERNAL_DATAS,
				{
					file: file,
				},
				[new modelAction.BpjsSEPInternalDataAction(
			...
			window.open(environment.API_URL + '/api/bpjssepinternaldata/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsSEPInternalDataModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP Internal Data entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP Internal Data entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsSEPInternalDataModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsSEPInternalDataModelActionTypes.DELETE_BPJS_SEP_INTERNAL_DATA;
						} else if (usingSelected) {
							actionType = modelAction.BpjsSEPInternalDataModelActionTypes.DELETE_ALL_BPJS_SEP_INTERNAL_DATA;
						} else {
							actionType = modelAction.BpjsSEPInternalDataModelActionTypes.DELETE_BPJS_SEP_INTERNAL_DATA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SEP_INTERNAL_DATA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SEP_INTERNAL_DATA and DELETE_BPJS_SEP_INTERNAL_DATA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsSEPInternalDataModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsSEPInternalDataModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsSEPInternalDatasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
