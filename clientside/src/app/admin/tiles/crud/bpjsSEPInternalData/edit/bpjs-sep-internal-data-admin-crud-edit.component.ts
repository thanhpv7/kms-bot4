...
import {BpjsSEPInternalDataModelState, BpjsSEPInternalDataModelAudit} from 'src/app/models/bpjsSEPInternalData/bpjs_sep_internal_data.model.state';
import {BpjsSEPInternalDataModel} from 'src/app/models/bpjsSEPInternalData/bpjs_sep_internal_data.model';
import {
	getBpjsSEPInternalDataModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-sep-internal-data-admin-crud-edit',
	templateUrl: './bpjs-sep-internal-data-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-sep-internal-data-admin-crud-edit.component.scss',
...
export class AdminBpjsSEPInternalDataCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsSEPInternalDataAudits$: Observable<BpjsSEPInternalDataModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsSEPInternalDataModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsSEPInternalDataModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsSEPInternalDataModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsSEPInternalDataModel();
		...
		if (this.targetModelId) {
			this.bpjsSEPInternalDataAudits$ = this.store.select(getBpjsSEPInternalDataModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsSEPInternalDataModel> = {
		...
		this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
			modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_BPJS_SEP_INTERNAL_DATA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPInternalDataModelWithId, this.targetModelId)
			...
			model = new BpjsSEPInternalDataModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
			modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_BPJS_SEP_INTERNAL_DATA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
				modelAction.BpjsSEPInternalDataModelActionTypes.CREATE_BPJS_SEP_INTERNAL_DATA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsSEPInternalDataAction(
				modelAction.BpjsSEPInternalDataModelActionTypes.UPDATE_BPJS_SEP_INTERNAL_DATA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsSEPInternalDataModel = new BpjsSEPInternalDataModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPInternalDataModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsSEPInternalDataModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsSEPInternalDataModel.getProps();
		this.modelRelations = BpjsSEPInternalDataModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsSEPInternalDataModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsSEPInternalDataModel {
		let updatedModel: BpjsSEPInternalDataModel = new BpjsSEPInternalDataModel(model);
