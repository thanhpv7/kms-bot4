...
import {DeliveryProgressModelState, DeliveryProgressModelAudit} from 'src/app/models/deliveryProgress/delivery_progress.model.state';
import {DeliveryProgressModel} from 'src/app/models/deliveryProgress/delivery_progress.model';
import {
	getDeliveryProgressModelAuditsByEntityId,
	getDeliveryProgressModelWithId,
	getCountDeliveryProgressModels,
...
@Component({
	selector: 'cb-delivery-progress-admin-crud-edit',
	templateUrl: './delivery-progress-admin-crud-edit.component.html',
	styleUrls: [
		'./delivery-progress-admin-crud-edit.component.scss',
...
export class AdminDeliveryProgressCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	deliveryProgressAudits$: Observable<DeliveryProgressModelAudit[]>;
	...
	deliveryMedicalExaminationRecordOppositeRelation = 'deliveryProgresses';
	...
	staffOppositeRelation = 'deliveryProgresses';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DeliveryProgressModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: DeliveryProgressModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDeliveryProgressModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DeliveryProgressModel();
		...
		if (this.targetModelId) {
			this.deliveryProgressAudits$ = this.store.select(getDeliveryProgressModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DeliveryProgressModel> = {
		...
		this.store.dispatch(new modelAction.DeliveryProgressAction(
			modelAction.DeliveryProgressModelActionTypes.FETCH_DELIVERY_PROGRESS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDeliveryProgressModelWithId, this.targetModelId)
			...
			model = new DeliveryProgressModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DeliveryProgressAction(
			modelAction.DeliveryProgressModelActionTypes.FETCH_DELIVERY_PROGRESS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<DeliveryProgressModel> = {
				...
				queryParams: {
					persistRelations: this.deliveryMedicalExaminationRecordMultiCrudActive || this.staffMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.DeliveryProgressAction(
				modelAction.DeliveryProgressModelActionTypes.CREATE_DELIVERY_PROGRESS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DeliveryProgressAction(
				modelAction.DeliveryProgressModelActionTypes.UPDATE_DELIVERY_PROGRESS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DeliveryProgressModel = new DeliveryProgressModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDeliveryProgressModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DeliveryProgressModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DeliveryProgressModel.getProps();
		this.modelRelations = DeliveryProgressModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: DeliveryProgressModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DeliveryProgressModel {
		let updatedModel: DeliveryProgressModel = new DeliveryProgressModel(model);
