...
import * as modelAction from 'src/app/models/deliveryProgress/delivery_progress.model.action';
import {DeliveryProgressModelState} from 'src/app/models/deliveryProgress/delivery_progress.model.state';
...
import {
	getDeliveryProgressCollectionModels,
	getDeliveryProgressCollectionCount
...
import {CrudTileMode} from '../delivery-progress-admin-crud.component';
...
@Component({
	selector: 'cb-delivery-progress-admin-crud-list',
	templateUrl: './delivery-progress-admin-crud-list.component.html',
	styleUrls: [
		'./delivery-progress-admin-crud-list.component.scss',
...
export class AdminDeliveryProgressCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DeliveryProgressModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DeliveryProgressModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = DeliveryProgressModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '150513f3-90fa-4c4c-923f-fac17f13ae51';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.deliveryProgresssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.deliveryProgresssCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: DeliveryProgressModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DeliveryProgressAction(
				modelAction.DeliveryProgressModelActionTypes.INITIALISE_DELIVERY_PROGRESS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DeliveryProgressAction(
				modelAction.DeliveryProgressModelActionTypes.FETCH_DELIVERY_PROGRESS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.DeliveryProgressAction(
						modelAction.DeliveryProgressModelActionTypes.COUNT_DELIVERY_PROGRESSS,
						null
					)
				]
			))
		...
		this.deliveryProgresss = this.store.select(getDeliveryProgressCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDeliveryProgressCollectionCount, this.collectionId).subscribe(
				(count) => this.deliveryProgresssCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('delivery-progress');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DeliveryProgressModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DeliveryProgressModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DeliveryProgressModelActionTypes.EXPORT_ALL_DELIVERY_PROGRESS;
		} else {
			actionType = modelAction.DeliveryProgressModelActionTypes.EXPORT_DELIVERY_PROGRESS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DeliveryProgressAction(
				modelAction.DeliveryProgressModelActionTypes.IMPORT_DELIVERY_PROGRESSS,
				{
					file: file,
				},
				[new modelAction.DeliveryProgressAction(
			...
			window.open(environment.API_URL + '/api/deliveryprogress/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DeliveryProgressModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Delivery Progress entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Delivery Progress entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DeliveryProgressModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DeliveryProgressModelActionTypes.DELETE_DELIVERY_PROGRESS;
						} else if (usingSelected) {
							actionType = modelAction.DeliveryProgressModelActionTypes.DELETE_ALL_DELIVERY_PROGRESS;
						} else {
							actionType = modelAction.DeliveryProgressModelActionTypes.DELETE_DELIVERY_PROGRESS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DeliveryProgressAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DELIVERY_PROGRESS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DELIVERY_PROGRESS and DELETE_DELIVERY_PROGRESS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<DeliveryProgressModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DeliveryProgressModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.deliveryProgresssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
