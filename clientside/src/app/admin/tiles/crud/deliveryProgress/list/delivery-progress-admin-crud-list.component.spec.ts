...
import {AdminDeliveryProgressCrudListComponent} from './delivery-progress-admin-crud-list.component';
...
import {AdminDeliveryProgressCrudModule} from '../delivery-progress-admin-crud.module';
import {
	DeliveryProgressModelState,
	initialState as initialDeliveryProgressModelState
} from 'src/app/models/deliveryProgress/delivery_progress.model.state';
 ...
 */
describe('Delivery Progress Crud List Tile Component', () => {

	let fixture;
	let deliveryProgressCrudListComponent: AdminDeliveryProgressCrudListComponent;

	let store: MockStore<{ model: DeliveryProgressModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let deliveryProgressModelState: DeliveryProgressModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<DeliveryProgressModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '150513f3-90fa-4c4c-923f-fac17f13ae51';
	const searchContent = 'text';

	const searchableFieldCount = DeliveryProgressModel.searchFields.length;

	const routerState: RouterState = {
		url: 'delivery-progress-admin-crud',
		urls: ['delivery-progress-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const deliveryProgressDataFactory = new DeliveryProgressDataFactory();

	const defaultDataSize = 30;
	const defaultData = deliveryProgressDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: DeliveryProgressModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		deliveryProgressModelState = initialDeliveryProgressModelState;
		store.setState({model: deliveryProgressModelState});
		...
		spyOn(store, 'select')
			.withArgs(getDeliveryProgressCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDeliveryProgressCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminDeliveryProgressCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminDeliveryProgressCrudListComponent);
			deliveryProgressCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(deliveryProgressCrudListComponent.deliveryProgresssCount).toBe(defaultData.length);
			...
			expect(deliveryProgressCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			deliveryProgressCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = deliveryProgressDataFactory.createAll(filteredDataSize);
			...
			expect(deliveryProgressCrudListComponent.deliveryProgresssCount).toBe(filteredDataSize);
			expect(deliveryProgressCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(deliveryProgressCrudListComponent.pageIndex).toBe(0);
			expect(deliveryProgressCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			deliveryProgressCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		deliveryProgressCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		deliveryProgressCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
