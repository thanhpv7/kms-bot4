...
import {TaskDashboardModelState, TaskDashboardModelAudit} from 'src/app/models/taskDashboard/task_dashboard.model.state';
import {TaskDashboardModel} from 'src/app/models/taskDashboard/task_dashboard.model';
import {
	getTaskDashboardModelAuditsByEntityId,
	getTaskDashboardModelWithId,
	getCountTaskDashboardModels,
...
@Component({
	selector: 'cb-task-dashboard-admin-crud-edit',
	templateUrl: './task-dashboard-admin-crud-edit.component.html',
	styleUrls: [
		'./task-dashboard-admin-crud-edit.component.scss',
...
export class AdminTaskDashboardCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	taskDashboardAudits$: Observable<TaskDashboardModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TaskDashboardModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: TaskDashboardModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTaskDashboardModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TaskDashboardModel();
		...
		if (this.targetModelId) {
			this.taskDashboardAudits$ = this.store.select(getTaskDashboardModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TaskDashboardModel> = {
		...
		this.store.dispatch(new modelAction.TaskDashboardAction(
			modelAction.TaskDashboardModelActionTypes.FETCH_TASK_DASHBOARD,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTaskDashboardModelWithId, this.targetModelId)
			...
			model = new TaskDashboardModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TaskDashboardAction(
			modelAction.TaskDashboardModelActionTypes.FETCH_TASK_DASHBOARD_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.TaskDashboardAction(
				modelAction.TaskDashboardModelActionTypes.CREATE_TASK_DASHBOARD,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TaskDashboardAction(
				modelAction.TaskDashboardModelActionTypes.UPDATE_TASK_DASHBOARD,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TaskDashboardModel = new TaskDashboardModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTaskDashboardModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TaskDashboardModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TaskDashboardModel.getProps();
		this.modelRelations = TaskDashboardModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: TaskDashboardModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TaskDashboardModel {
		let updatedModel: TaskDashboardModel = new TaskDashboardModel(model);
