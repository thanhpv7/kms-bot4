...
import * as modelAction from 'src/app/models/taskDashboard/task_dashboard.model.action';
import {TaskDashboardModelState} from 'src/app/models/taskDashboard/task_dashboard.model.state';
...
import {
	getTaskDashboardCollectionModels,
	getTaskDashboardCollectionCount
} from 'src/app/models/taskDashboard/task_dashboard.model.selector';
...
import {CrudTileMode} from '../task-dashboard-admin-crud.component';
...
@Component({
	selector: 'cb-task-dashboard-admin-crud-list',
	templateUrl: './task-dashboard-admin-crud-list.component.html',
	styleUrls: [
		'./task-dashboard-admin-crud-list.component.scss',
...
export class AdminTaskDashboardCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TaskDashboardModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TaskDashboardModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = TaskDashboardModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ff86193d-0f7e-42da-ae03-9c00ccbacdc7';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.taskDashboardsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.taskDashboardsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: TaskDashboardModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TaskDashboardAction(
				modelAction.TaskDashboardModelActionTypes.INITIALISE_TASK_DASHBOARD_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TaskDashboardAction(
				modelAction.TaskDashboardModelActionTypes.FETCH_TASK_DASHBOARD_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.TaskDashboardAction(
						modelAction.TaskDashboardModelActionTypes.COUNT_TASK_DASHBOARDS,
						null
					)
				]
			))
		...
		this.taskDashboards = this.store.select(getTaskDashboardCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTaskDashboardCollectionCount, this.collectionId).subscribe(
				(count) => this.taskDashboardsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TaskDashboardModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TaskDashboardModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TaskDashboardModelActionTypes.EXPORT_ALL_TASK_DASHBOARD;
		} else {
			actionType = modelAction.TaskDashboardModelActionTypes.EXPORT_TASK_DASHBOARD_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TaskDashboardAction(
				modelAction.TaskDashboardModelActionTypes.IMPORT_TASK_DASHBOARDS,
				{
					file: file,
				},
				[new modelAction.TaskDashboardAction(
			...
			window.open(environment.API_URL + '/api/taskdashboard/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TaskDashboardModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Task Dashboard entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Task Dashboard entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TaskDashboardModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TaskDashboardModelActionTypes.DELETE_TASK_DASHBOARD;
						} else if (usingSelected) {
							actionType = modelAction.TaskDashboardModelActionTypes.DELETE_ALL_TASK_DASHBOARD;
						} else {
							actionType = modelAction.TaskDashboardModelActionTypes.DELETE_TASK_DASHBOARD_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TaskDashboardAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TASK_DASHBOARD when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TASK_DASHBOARD and DELETE_TASK_DASHBOARD_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<TaskDashboardModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TaskDashboardModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.taskDashboardsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
