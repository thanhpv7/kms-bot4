...
import {AdminTaskDashboardCrudListComponent} from './task-dashboard-admin-crud-list.component';
import {TaskDashboardDataFactory} from 'src/app/lib/utils/factories/task-dashboard-data-factory';
import {TaskDashboardModel} from 'src/app/models/taskDashboard/task_dashboard.model';
import {AdminTaskDashboardCrudModule} from '../task-dashboard-admin-crud.module';
import {
	TaskDashboardModelState,
	initialState as initialTaskDashboardModelState
} from 'src/app/models/taskDashboard/task_dashboard.model.state';
 ...
 */
describe('Task Dashboard Crud List Tile Component', () => {

	let fixture;
	let taskDashboardCrudListComponent: AdminTaskDashboardCrudListComponent;

	let store: MockStore<{ model: TaskDashboardModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let taskDashboardModelState: TaskDashboardModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<TaskDashboardModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ff86193d-0f7e-42da-ae03-9c00ccbacdc7';
	const searchContent = 'text';

	const searchableFieldCount = TaskDashboardModel.searchFields.length;

	const routerState: RouterState = {
		url: 'task-dashboard-admin-crud',
		urls: ['task-dashboard-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const taskDashboardDataFactory = new TaskDashboardDataFactory();

	const defaultDataSize = 30;
	const defaultData = taskDashboardDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: TaskDashboardModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		taskDashboardModelState = initialTaskDashboardModelState;
		store.setState({model: taskDashboardModelState});
		...
		spyOn(store, 'select')
			.withArgs(getTaskDashboardCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTaskDashboardCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminTaskDashboardCrudListComponent);
			taskDashboardCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(taskDashboardCrudListComponent.taskDashboardsCount).toBe(defaultData.length);
			...
			expect(taskDashboardCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			taskDashboardCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = taskDashboardDataFactory.createAll(filteredDataSize);
			...
			expect(taskDashboardCrudListComponent.taskDashboardsCount).toBe(filteredDataSize);
			expect(taskDashboardCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(taskDashboardCrudListComponent.pageIndex).toBe(0);
			expect(taskDashboardCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			taskDashboardCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		taskDashboardCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		taskDashboardCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
