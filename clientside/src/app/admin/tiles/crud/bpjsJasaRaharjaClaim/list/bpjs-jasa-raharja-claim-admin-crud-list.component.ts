...
import * as modelAction from 'src/app/models/bpjsJasaRaharjaClaim/bpjs_jasa_raharja_claim.model.action';
import {BpjsJasaRaharjaClaimModelState} from 'src/app/models/bpjsJasaRaharjaClaim/bpjs_jasa_raharja_claim.model.state';
...
import {
	...
	getBpjsJasaRaharjaClaimCollectionCount
...
import {CrudTileMode} from '../bpjs-jasa-raharja-claim-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-jasa-raharja-claim-admin-crud-list',
	templateUrl: './bpjs-jasa-raharja-claim-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-jasa-raharja-claim-admin-crud-list.component.scss',
...
export class AdminBpjsJasaRaharjaClaimCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsJasaRaharjaClaimModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsJasaRaharjaClaimModel.searchFields,
	...
	/**
	 * List of BPJS Jasa Raharja Claims that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsJasaRaharjaClaims: Observable<BpjsJasaRaharjaClaimModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsJasaRaharjaClaimsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsJasaRaharjaClaimModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsJasaRaharjaClaimModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ea83805e-5c10-4cfb-8751-1401e4fe693a';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsJasaRaharjaClaimsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsJasaRaharjaClaimsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsJasaRaharjaClaimModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
				modelAction.BpjsJasaRaharjaClaimModelActionTypes.INITIALISE_BPJS_JASA_RAHARJA_CLAIM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
				modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_BPJS_JASA_RAHARJA_CLAIM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsJasaRaharjaClaimAction(
						modelAction.BpjsJasaRaharjaClaimModelActionTypes.COUNT_BPJS_JASA_RAHARJA_CLAIMS,
						null
					)
				]
			))
		...
		this.bpjsJasaRaharjaClaims = this.store.select(getBpjsJasaRaharjaClaimCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsJasaRaharjaClaimCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsJasaRaharjaClaimsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-jasa-raharja-claim');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsJasaRaharjaClaimModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsJasaRaharjaClaimModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsJasaRaharjaClaimModelActionTypes.EXPORT_ALL_BPJS_JASA_RAHARJA_CLAIM;
		} else {
			actionType = modelAction.BpjsJasaRaharjaClaimModelActionTypes.EXPORT_BPJS_JASA_RAHARJA_CLAIM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
				modelAction.BpjsJasaRaharjaClaimModelActionTypes.IMPORT_BPJS_JASA_RAHARJA_CLAIMS,
				{
					file: file,
				},
				[new modelAction.BpjsJasaRaharjaClaimAction(
			...
			window.open(environment.API_URL + '/api/bpjsjasaraharjaclaim/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsJasaRaharjaClaimModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Jasa Raharja Claim entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Jasa Raharja Claim entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsJasaRaharjaClaimModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsJasaRaharjaClaimModelActionTypes.DELETE_BPJS_JASA_RAHARJA_CLAIM;
						} else if (usingSelected) {
							actionType = modelAction.BpjsJasaRaharjaClaimModelActionTypes.DELETE_ALL_BPJS_JASA_RAHARJA_CLAIM;
						} else {
							actionType = modelAction.BpjsJasaRaharjaClaimModelActionTypes.DELETE_BPJS_JASA_RAHARJA_CLAIM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_JASA_RAHARJA_CLAIM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_JASA_RAHARJA_CLAIM and DELETE_BPJS_JASA_RAHARJA_CLAIM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsJasaRaharjaClaimModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsJasaRaharjaClaimModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsJasaRaharjaClaimsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
