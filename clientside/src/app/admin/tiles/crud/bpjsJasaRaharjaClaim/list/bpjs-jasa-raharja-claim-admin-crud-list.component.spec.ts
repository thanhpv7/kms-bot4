...
import {AdminBpjsJasaRaharjaClaimCrudListComponent} from './bpjs-jasa-raharja-claim-admin-crud-list.component';
...
import {AdminBpjsJasaRaharjaClaimCrudModule} from '../bpjs-jasa-raharja-claim-admin-crud.module';
import {
 ...
 */
describe('BPJS Jasa Raharja Claim Crud List Tile Component', () => {

	let fixture;
	let bpjsJasaRaharjaClaimCrudListComponent: AdminBpjsJasaRaharjaClaimCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsJasaRaharjaClaimModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ea83805e-5c10-4cfb-8751-1401e4fe693a';
	const searchContent = 'text';

	const searchableFieldCount = BpjsJasaRaharjaClaimModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-jasa-raharja-claim-admin-crud',
		urls: ['bpjs-jasa-raharja-claim-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsJasaRaharjaClaimDataFactory = new BpjsJasaRaharjaClaimDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsJasaRaharjaClaimDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsJasaRaharjaClaimCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsJasaRaharjaClaimCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsJasaRaharjaClaimCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsJasaRaharjaClaimCrudListComponent);
			bpjsJasaRaharjaClaimCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsJasaRaharjaClaimCrudListComponent.bpjsJasaRaharjaClaimsCount).toBe(defaultData.length);
			...
			expect(bpjsJasaRaharjaClaimCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsJasaRaharjaClaimCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsJasaRaharjaClaimDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsJasaRaharjaClaimCrudListComponent.bpjsJasaRaharjaClaimsCount).toBe(filteredDataSize);
			expect(bpjsJasaRaharjaClaimCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsJasaRaharjaClaimCrudListComponent.pageIndex).toBe(0);
			expect(bpjsJasaRaharjaClaimCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsJasaRaharjaClaimCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsJasaRaharjaClaimCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsJasaRaharjaClaimCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
