...
import {BpjsJasaRaharjaClaimModelState, BpjsJasaRaharjaClaimModelAudit} from 'src/app/models/bpjsJasaRaharjaClaim/bpjs_jasa_raharja_claim.model.state';
import {BpjsJasaRaharjaClaimModel} from 'src/app/models/bpjsJasaRaharjaClaim/bpjs_jasa_raharja_claim.model';
import {
	getBpjsJasaRaharjaClaimModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-jasa-raharja-claim-admin-crud-edit',
	templateUrl: './bpjs-jasa-raharja-claim-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-jasa-raharja-claim-admin-crud-edit.component.scss',
...
export class AdminBpjsJasaRaharjaClaimCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsJasaRaharjaClaimAudits$: Observable<BpjsJasaRaharjaClaimModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsJasaRaharjaClaimModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsJasaRaharjaClaimModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsJasaRaharjaClaimModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsJasaRaharjaClaimModel();
		...
		if (this.targetModelId) {
			this.bpjsJasaRaharjaClaimAudits$ = this.store.select(getBpjsJasaRaharjaClaimModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsJasaRaharjaClaimModel> = {
		...
		this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
			modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_BPJS_JASA_RAHARJA_CLAIM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsJasaRaharjaClaimModelWithId, this.targetModelId)
			...
			model = new BpjsJasaRaharjaClaimModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
			modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_BPJS_JASA_RAHARJA_CLAIM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
				modelAction.BpjsJasaRaharjaClaimModelActionTypes.CREATE_BPJS_JASA_RAHARJA_CLAIM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsJasaRaharjaClaimAction(
				modelAction.BpjsJasaRaharjaClaimModelActionTypes.UPDATE_BPJS_JASA_RAHARJA_CLAIM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsJasaRaharjaClaimModel = new BpjsJasaRaharjaClaimModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsJasaRaharjaClaimModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsJasaRaharjaClaimModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsJasaRaharjaClaimModel.getProps();
		this.modelRelations = BpjsJasaRaharjaClaimModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsJasaRaharjaClaimModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsJasaRaharjaClaimModel {
		let updatedModel: BpjsJasaRaharjaClaimModel = new BpjsJasaRaharjaClaimModel(model);
