...
import {AdminMedicationHeaderCrudListComponent} from './medication-header-admin-crud-list.component';
...
import {AdminMedicationHeaderCrudModule} from '../medication-header-admin-crud.module';
import {
 ...
 */
describe('Medication Header Crud List Tile Component', () => {

	let fixture;
	let medicationHeaderCrudListComponent: AdminMedicationHeaderCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicationHeaderModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e88e09ea-f7b6-4e33-87f0-ade187e646c2';
	const searchContent = 'text';

	const searchableFieldCount = MedicationHeaderModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medication-header-admin-crud',
		urls: ['medication-header-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicationHeaderDataFactory = new MedicationHeaderDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicationHeaderDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getMedicationHeaderCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicationHeaderCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicationHeaderCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicationHeaderCrudListComponent);
			medicationHeaderCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicationHeaderCrudListComponent.medicationHeadersCount).toBe(defaultData.length);
			...
			expect(medicationHeaderCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicationHeaderCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicationHeaderDataFactory.createAll(filteredDataSize);
			...
			expect(medicationHeaderCrudListComponent.medicationHeadersCount).toBe(filteredDataSize);
			expect(medicationHeaderCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicationHeaderCrudListComponent.pageIndex).toBe(0);
			expect(medicationHeaderCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicationHeaderCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicationHeaderCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicationHeaderCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
