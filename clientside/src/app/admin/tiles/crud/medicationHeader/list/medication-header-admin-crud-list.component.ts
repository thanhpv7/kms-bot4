...
import {MedicationHeaderModelState} from 'src/app/models/medicationHeader/medication_header.model.state';
...
import {
	...
	getMedicationHeaderCollectionCount
...
@Component({
	selector: 'cb-medication-header-admin-crud-list',
	templateUrl: './medication-header-admin-crud-list.component.html',
	styleUrls: [
		'./medication-header-admin-crud-list.component.scss',
...
export class AdminMedicationHeaderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicationHeaderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicationHeaderModel.searchFields,
	...
	/**
	 * List of Medication Headers that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicationHeaders: Observable<MedicationHeaderModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicationHeadersCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicationHeadersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicationHeadersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationHeaderAction(
				modelAction.MedicationHeaderModelActionTypes.INITIALISE_MEDICATION_HEADER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationHeaderAction(
				modelAction.MedicationHeaderModelActionTypes.FETCH_MEDICATION_HEADER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicationHeaders = this.store.select(getMedicationHeaderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicationHeaderCollectionCount, this.collectionId).subscribe(
				(count) => this.medicationHeadersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicationHeaderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicationHeaderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicationHeaderModelActionTypes.EXPORT_ALL_MEDICATION_HEADER;
		} else {
			actionType = modelAction.MedicationHeaderModelActionTypes.EXPORT_MEDICATION_HEADER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicationHeaderAction(
				modelAction.MedicationHeaderModelActionTypes.IMPORT_MEDICATION_HEADERS,
				{
					file: file,
				},
				[new modelAction.MedicationHeaderAction(
			...
			window.open(environment.API_URL + '/api/medicationheader/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicationHeaderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medication Header entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medication Header entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicationHeaderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicationHeaderModelActionTypes.DELETE_MEDICATION_HEADER;
						} else if (usingSelected) {
							actionType = modelAction.MedicationHeaderModelActionTypes.DELETE_ALL_MEDICATION_HEADER;
						} else {
							actionType = modelAction.MedicationHeaderModelActionTypes.DELETE_MEDICATION_HEADER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicationHeaderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICATION_HEADER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICATION_HEADER and DELETE_MEDICATION_HEADER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicationHeaderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicationHeadersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
