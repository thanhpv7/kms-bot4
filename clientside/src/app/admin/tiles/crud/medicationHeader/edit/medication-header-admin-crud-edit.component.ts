...
import * as modelAction from 'src/app/models/medicationHeader/medication_header.model.action';
import {MedicationHeaderModelState, MedicationHeaderModelAudit} from 'src/app/models/medicationHeader/medication_header.model.state';
import {MedicationHeaderModel} from 'src/app/models/medicationHeader/medication_header.model';
import {
	getMedicationHeaderModelAuditsByEntityId,
	getMedicationHeaderModelWithId,
	getCountMedicationHeaderModels,
...
import {CrudTileMode} from '../medication-header-admin-crud.component';
...
@Component({
	selector: 'cb-medication-header-admin-crud-edit',
	templateUrl: './medication-header-admin-crud-edit.component.html',
	styleUrls: [
		'./medication-header-admin-crud-edit.component.scss',
...
export class AdminMedicationHeaderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'e88e09ea-f7b6-4e33-87f0-ade187e646c2';
	 ...
	 */
	medicationHeaderAudits$: Observable<MedicationHeaderModelAudit[]>;
	...
	invoiceOppositeRelation = 'medicationHeader';
	...
	registrationOppositeRelation = 'medicationHeaders';
	...
	requestedByOppositeRelation = 'medicationHeaders';
	...
	warehouseOppositeRelation = 'medicationHeaders';
	...
	invoiceItemsOppositeRelation = 'medicationHeader';
	...
	medicationCompoundsOppositeRelation = 'medicationHeader';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Medication Header and Medication Items here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Medication Header and Medication Items here] end
	medicationItemsOppositeRelation = 'medicationHeader';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicationHeaderModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicationHeaderModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicationHeaderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicationHeaderModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicationHeaderModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicationHeaderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicationHeaderModel();
		...
		if (this.targetModelId) {
			this.medicationHeaderAudits$ = this.store.select(getMedicationHeaderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicationHeaderModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicationHeaderAction(
			modelAction.MedicationHeaderModelActionTypes.FETCH_MEDICATION_HEADER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicationHeaderModelWithId, this.targetModelId)
			...
			model = new MedicationHeaderModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medication-header');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicationHeaderAction(
			modelAction.MedicationHeaderModelActionTypes.FETCH_MEDICATION_HEADER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicationHeaderModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.invoiceMultiCrudActive || this.registrationMultiCrudActive || this.requestedByMultiCrudActive || this.warehouseMultiCrudActive || this.invoiceItemsMultiCrudActive || this.medicationCompoundsMultiCrudActive || this.medicationItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicationHeaderAction(
						modelAction.MedicationHeaderModelActionTypes.COUNT_MEDICATION_HEADERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicationHeaderAction(
				modelAction.MedicationHeaderModelActionTypes.CREATE_MEDICATION_HEADER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicationHeaderAction(
				modelAction.MedicationHeaderModelActionTypes.UPDATE_MEDICATION_HEADER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicationHeaderModel = new MedicationHeaderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicationHeaderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicationHeaderModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicationHeaderModel.getProps();
		this.modelRelations = MedicationHeaderModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicationHeaderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicationHeaderModel {
		let updatedModel: MedicationHeaderModel = new MedicationHeaderModel(model);
