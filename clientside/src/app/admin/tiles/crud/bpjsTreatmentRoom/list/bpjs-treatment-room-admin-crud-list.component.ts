...
import {BpjsTreatmentRoomModelState} from 'src/app/models/bpjsTreatmentRoom/bpjs_treatment_room.model.state';
...
@Component({
	selector: 'cb-bpjs-treatment-room-admin-crud-list',
	templateUrl: './bpjs-treatment-room-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-treatment-room-admin-crud-list.component.scss',
...
export class AdminBpjsTreatmentRoomCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsTreatmentRoomModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsTreatmentRoomModel.searchFields,
	...
	/**
	 * List of BPJS Treatment Rooms that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsTreatmentRooms: Observable<BpjsTreatmentRoomModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsTreatmentRoomsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsTreatmentRoomsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsTreatmentRoomsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
				modelAction.BpjsTreatmentRoomModelActionTypes.INITIALISE_BPJS_TREATMENT_ROOM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
				modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_BPJS_TREATMENT_ROOM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsTreatmentRooms = this.store.select(getBpjsTreatmentRoomCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsTreatmentRoomCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsTreatmentRoomsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsTreatmentRoomModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsTreatmentRoomModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsTreatmentRoomModelActionTypes.EXPORT_ALL_BPJS_TREATMENT_ROOM;
		} else {
			actionType = modelAction.BpjsTreatmentRoomModelActionTypes.EXPORT_BPJS_TREATMENT_ROOM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
				modelAction.BpjsTreatmentRoomModelActionTypes.IMPORT_BPJS_TREATMENT_ROOMS,
				{
					file: file,
				},
				[new modelAction.BpjsTreatmentRoomAction(
			...
			window.open(environment.API_URL + '/api/bpjstreatmentroom/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsTreatmentRoomModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Treatment Room entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Treatment Room entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsTreatmentRoomModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsTreatmentRoomModelActionTypes.DELETE_BPJS_TREATMENT_ROOM;
						} else if (usingSelected) {
							actionType = modelAction.BpjsTreatmentRoomModelActionTypes.DELETE_ALL_BPJS_TREATMENT_ROOM;
						} else {
							actionType = modelAction.BpjsTreatmentRoomModelActionTypes.DELETE_BPJS_TREATMENT_ROOM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_TREATMENT_ROOM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_TREATMENT_ROOM and DELETE_BPJS_TREATMENT_ROOM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsTreatmentRoomModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsTreatmentRoomsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
