...
import * as modelAction from 'src/app/models/bpjsTreatmentRoom/bpjs_treatment_room.model.action';
import {BpjsTreatmentRoomModelState, BpjsTreatmentRoomModelAudit} from 'src/app/models/bpjsTreatmentRoom/bpjs_treatment_room.model.state';
import {BpjsTreatmentRoomModel} from 'src/app/models/bpjsTreatmentRoom/bpjs_treatment_room.model';
import {
	getBpjsTreatmentRoomModelAuditsByEntityId,
...
import {CrudTileMode} from '../bpjs-treatment-room-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-treatment-room-admin-crud-edit',
	templateUrl: './bpjs-treatment-room-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-treatment-room-admin-crud-edit.component.scss',
...
export class AdminBpjsTreatmentRoomCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '10f7f483-4d88-4163-8ca4-d340812c2ea4';
	 ...
	 */
	bpjsTreatmentRoomAudits$: Observable<BpjsTreatmentRoomModelAudit[]>;
	...
	bpjsClaimSubmissionOppositeRelation = 'bpjsTreatmentRooms';
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsTreatmentRoomModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsTreatmentRoomModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsTreatmentRoomModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsTreatmentRoomModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsTreatmentRoomModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsTreatmentRoomModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsTreatmentRoomModel();
		...
		if (this.targetModelId) {
			this.bpjsTreatmentRoomAudits$ = this.store.select(getBpjsTreatmentRoomModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsTreatmentRoomModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
			modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_BPJS_TREATMENT_ROOM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsTreatmentRoomModelWithId, this.targetModelId)
			...
			model = new BpjsTreatmentRoomModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-treatment-room');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
			modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_BPJS_TREATMENT_ROOM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsTreatmentRoomModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsTreatmentRoomAction(
						modelAction.BpjsTreatmentRoomModelActionTypes.COUNT_BPJS_TREATMENT_ROOMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
				modelAction.BpjsTreatmentRoomModelActionTypes.CREATE_BPJS_TREATMENT_ROOM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsTreatmentRoomAction(
				modelAction.BpjsTreatmentRoomModelActionTypes.UPDATE_BPJS_TREATMENT_ROOM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsTreatmentRoomModel = new BpjsTreatmentRoomModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsTreatmentRoomModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsTreatmentRoomModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsTreatmentRoomModel.getProps();
		this.modelRelations = BpjsTreatmentRoomModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsTreatmentRoomModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsTreatmentRoomModel {
		let updatedModel: BpjsTreatmentRoomModel = new BpjsTreatmentRoomModel(model);
