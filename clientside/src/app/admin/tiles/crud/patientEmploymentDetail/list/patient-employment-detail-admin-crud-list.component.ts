...
import * as modelAction from 'src/app/models/patientEmploymentDetail/patient_employment_detail.model.action';
import {PatientEmploymentDetailModelState} from 'src/app/models/patientEmploymentDetail/patient_employment_detail.model.state';
...
import {
	getPatientEmploymentDetailCollectionModels,
	getPatientEmploymentDetailCollectionCount
...
import {CrudTileMode} from '../patient-employment-detail-admin-crud.component';
...
@Component({
	selector: 'cb-patient-employment-detail-admin-crud-list',
	templateUrl: './patient-employment-detail-admin-crud-list.component.html',
	styleUrls: [
		'./patient-employment-detail-admin-crud-list.component.scss',
...
export class AdminPatientEmploymentDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientEmploymentDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientEmploymentDetailModel.searchFields,
	...
	/**
	 * List of Patient Employment Detail s that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientEmploymentDetails: Observable<PatientEmploymentDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientEmploymentDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PatientEmploymentDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientEmploymentDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '41d9d4f5-8b69-477f-b0d4-060e3a742485';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientEmploymentDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientEmploymentDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PatientEmploymentDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
				modelAction.PatientEmploymentDetailModelActionTypes.INITIALISE_PATIENT_EMPLOYMENT_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
				modelAction.PatientEmploymentDetailModelActionTypes.FETCH_PATIENT_EMPLOYMENT_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PatientEmploymentDetailAction(
						modelAction.PatientEmploymentDetailModelActionTypes.COUNT_PATIENT_EMPLOYMENT_DETAILS,
						null
					)
				]
			))
		...
		this.patientEmploymentDetails = this.store.select(getPatientEmploymentDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientEmploymentDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.patientEmploymentDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-employment-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientEmploymentDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientEmploymentDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientEmploymentDetailModelActionTypes.EXPORT_ALL_PATIENT_EMPLOYMENT_DETAIL;
		} else {
			actionType = modelAction.PatientEmploymentDetailModelActionTypes.EXPORT_PATIENT_EMPLOYMENT_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
				modelAction.PatientEmploymentDetailModelActionTypes.IMPORT_PATIENT_EMPLOYMENT_DETAILS,
				{
					file: file,
				},
				[new modelAction.PatientEmploymentDetailAction(
			...
			window.open(environment.API_URL + '/api/patientemploymentdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientEmploymentDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Employment Detail  entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Employment Detail  entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientEmploymentDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientEmploymentDetailModelActionTypes.DELETE_PATIENT_EMPLOYMENT_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.PatientEmploymentDetailModelActionTypes.DELETE_ALL_PATIENT_EMPLOYMENT_DETAIL;
						} else {
							actionType = modelAction.PatientEmploymentDetailModelActionTypes.DELETE_PATIENT_EMPLOYMENT_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_EMPLOYMENT_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_EMPLOYMENT_DETAIL and DELETE_PATIENT_EMPLOYMENT_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PatientEmploymentDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientEmploymentDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientEmploymentDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
