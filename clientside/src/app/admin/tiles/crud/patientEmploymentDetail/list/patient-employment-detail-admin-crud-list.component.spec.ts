...
import {AdminPatientEmploymentDetailCrudListComponent} from './patient-employment-detail-admin-crud-list.component';
...
import {AdminPatientEmploymentDetailCrudModule} from '../patient-employment-detail-admin-crud.module';
import {
	PatientEmploymentDetailModelState,
	initialState as initialPatientEmploymentDetailModelState
} from 'src/app/models/patientEmploymentDetail/patient_employment_detail.model.state';
 ...
 */
describe('Patient Employment Detail  Crud List Tile Component', () => {

	let fixture;
	let patientEmploymentDetailCrudListComponent: AdminPatientEmploymentDetailCrudListComponent;

	let store: MockStore<{ model: PatientEmploymentDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientEmploymentDetailModelState: PatientEmploymentDetailModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientEmploymentDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '41d9d4f5-8b69-477f-b0d4-060e3a742485';
	const searchContent = 'text';

	const searchableFieldCount = PatientEmploymentDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-employment-detail-admin-crud',
		urls: ['patient-employment-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientEmploymentDetailDataFactory = new PatientEmploymentDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientEmploymentDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientEmploymentDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientEmploymentDetailModelState = initialPatientEmploymentDetailModelState;
		store.setState({model: patientEmploymentDetailModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientEmploymentDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientEmploymentDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientEmploymentDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientEmploymentDetailCrudListComponent);
			patientEmploymentDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientEmploymentDetailCrudListComponent.patientEmploymentDetailsCount).toBe(defaultData.length);
			...
			expect(patientEmploymentDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientEmploymentDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientEmploymentDetailDataFactory.createAll(filteredDataSize);
			...
			expect(patientEmploymentDetailCrudListComponent.patientEmploymentDetailsCount).toBe(filteredDataSize);
			expect(patientEmploymentDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientEmploymentDetailCrudListComponent.pageIndex).toBe(0);
			expect(patientEmploymentDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientEmploymentDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientEmploymentDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientEmploymentDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
