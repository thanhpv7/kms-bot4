...
import {PatientEmploymentDetailModelState, PatientEmploymentDetailModelAudit} from 'src/app/models/patientEmploymentDetail/patient_employment_detail.model.state';
import {PatientEmploymentDetailModel} from 'src/app/models/patientEmploymentDetail/patient_employment_detail.model';
import {
	getPatientEmploymentDetailModelAuditsByEntityId,
	getPatientEmploymentDetailModelWithId,
	getCountPatientEmploymentDetailModels,
...
@Component({
	selector: 'cb-patient-employment-detail-admin-crud-edit',
	templateUrl: './patient-employment-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-employment-detail-admin-crud-edit.component.scss',
...
export class AdminPatientEmploymentDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	patientEmploymentDetailAudits$: Observable<PatientEmploymentDetailModelAudit[]>;
	...
	patientGeneralInfoOppositeRelation = 'patientEmploymentDetail';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientEmploymentDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientEmploymentDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientEmploymentDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientEmploymentDetailModel();
		...
		if (this.targetModelId) {
			this.patientEmploymentDetailAudits$ = this.store.select(getPatientEmploymentDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientEmploymentDetailModel> = {
		...
		this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
			modelAction.PatientEmploymentDetailModelActionTypes.FETCH_PATIENT_EMPLOYMENT_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientEmploymentDetailModelWithId, this.targetModelId)
			...
			model = new PatientEmploymentDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
			modelAction.PatientEmploymentDetailModelActionTypes.FETCH_PATIENT_EMPLOYMENT_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientEmploymentDetailModel> = {
				...
				queryParams: {
					persistRelations: this.patientGeneralInfoMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
				modelAction.PatientEmploymentDetailModelActionTypes.CREATE_PATIENT_EMPLOYMENT_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientEmploymentDetailAction(
				modelAction.PatientEmploymentDetailModelActionTypes.UPDATE_PATIENT_EMPLOYMENT_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientEmploymentDetailModel = new PatientEmploymentDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientEmploymentDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientEmploymentDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientEmploymentDetailModel.getProps();
		this.modelRelations = PatientEmploymentDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientEmploymentDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientEmploymentDetailModel {
		let updatedModel: PatientEmploymentDetailModel = new PatientEmploymentDetailModel(model);
