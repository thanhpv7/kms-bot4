...
import * as modelAction from 'src/app/models/bpjsPcareProviderRayonisasi/bpjs_pcare_provider_rayonisasi.model.action';
import {BpjsPcareProviderRayonisasiModelState} from 'src/app/models/bpjsPcareProviderRayonisasi/bpjs_pcare_provider_rayonisasi.model.state';
...
import {
	...
	getBpjsPcareProviderRayonisasiCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-provider-rayonisasi-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-provider-rayonisasi-admin-crud-list',
	templateUrl: './bpjs-pcare-provider-rayonisasi-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-provider-rayonisasi-admin-crud-list.component.scss',
...
export class AdminBpjsPcareProviderRayonisasiCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcareProviderRayonisasiModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcareProviderRayonisasiModel.searchFields,
	...
	/**
	 * List of BPJS Pcare Provider Rayonisasis that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcareProviderRayonisasis: Observable<BpjsPcareProviderRayonisasiModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcareProviderRayonisasisCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcareProviderRayonisasiModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcareProviderRayonisasiModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'fc7ba115-9b40-48f5-b0d9-e587255f35da';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcareProviderRayonisasisCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcareProviderRayonisasisCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcareProviderRayonisasiModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
				modelAction.BpjsPcareProviderRayonisasiModelActionTypes.INITIALISE_BPJS_PCARE_PROVIDER_RAYONISASI_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
				modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_BPJS_PCARE_PROVIDER_RAYONISASI_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcareProviderRayonisasiAction(
						modelAction.BpjsPcareProviderRayonisasiModelActionTypes.COUNT_BPJS_PCARE_PROVIDER_RAYONISASIS,
						null
					)
				]
			))
		...
		this.bpjsPcareProviderRayonisasis = this.store.select(getBpjsPcareProviderRayonisasiCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcareProviderRayonisasiCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcareProviderRayonisasisCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-provider-rayonisasi');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcareProviderRayonisasiModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcareProviderRayonisasiModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcareProviderRayonisasiModelActionTypes.EXPORT_ALL_BPJS_PCARE_PROVIDER_RAYONISASI;
		} else {
			actionType = modelAction.BpjsPcareProviderRayonisasiModelActionTypes.EXPORT_BPJS_PCARE_PROVIDER_RAYONISASI_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
				modelAction.BpjsPcareProviderRayonisasiModelActionTypes.IMPORT_BPJS_PCARE_PROVIDER_RAYONISASIS,
				{
					file: file,
				},
				[new modelAction.BpjsPcareProviderRayonisasiAction(
			...
			window.open(environment.API_URL + '/api/bpjspcareproviderrayonisasi/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcareProviderRayonisasiModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Provider Rayonisasi entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Provider Rayonisasi entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcareProviderRayonisasiModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcareProviderRayonisasiModelActionTypes.DELETE_BPJS_PCARE_PROVIDER_RAYONISASI;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcareProviderRayonisasiModelActionTypes.DELETE_ALL_BPJS_PCARE_PROVIDER_RAYONISASI;
						} else {
							actionType = modelAction.BpjsPcareProviderRayonisasiModelActionTypes.DELETE_BPJS_PCARE_PROVIDER_RAYONISASI_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_PROVIDER_RAYONISASI when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_PROVIDER_RAYONISASI and DELETE_BPJS_PCARE_PROVIDER_RAYONISASI_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcareProviderRayonisasiModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcareProviderRayonisasiModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcareProviderRayonisasisCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
