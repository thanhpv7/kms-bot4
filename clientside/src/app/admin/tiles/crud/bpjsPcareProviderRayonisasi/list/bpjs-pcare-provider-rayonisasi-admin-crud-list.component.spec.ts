...
import {AdminBpjsPcareProviderRayonisasiCrudListComponent} from './bpjs-pcare-provider-rayonisasi-admin-crud-list.component';
...
import {AdminBpjsPcareProviderRayonisasiCrudModule} from '../bpjs-pcare-provider-rayonisasi-admin-crud.module';
import {
 ...
 */
describe('BPJS Pcare Provider Rayonisasi Crud List Tile Component', () => {

	let fixture;
	let bpjsPcareProviderRayonisasiCrudListComponent: AdminBpjsPcareProviderRayonisasiCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPcareProviderRayonisasiModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'fc7ba115-9b40-48f5-b0d9-e587255f35da';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPcareProviderRayonisasiModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-pcare-provider-rayonisasi-admin-crud',
		urls: ['bpjs-pcare-provider-rayonisasi-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPcareProviderRayonisasiDataFactory = new BpjsPcareProviderRayonisasiDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPcareProviderRayonisasiDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPcareProviderRayonisasiCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPcareProviderRayonisasiCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsPcareProviderRayonisasiCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsPcareProviderRayonisasiCrudListComponent);
			bpjsPcareProviderRayonisasiCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPcareProviderRayonisasiCrudListComponent.bpjsPcareProviderRayonisasisCount).toBe(defaultData.length);
			...
			expect(bpjsPcareProviderRayonisasiCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPcareProviderRayonisasiCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPcareProviderRayonisasiDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPcareProviderRayonisasiCrudListComponent.bpjsPcareProviderRayonisasisCount).toBe(filteredDataSize);
			expect(bpjsPcareProviderRayonisasiCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPcareProviderRayonisasiCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPcareProviderRayonisasiCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPcareProviderRayonisasiCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPcareProviderRayonisasiCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPcareProviderRayonisasiCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
