...
import {BpjsPcareProviderRayonisasiModelState, BpjsPcareProviderRayonisasiModelAudit} from 'src/app/models/bpjsPcareProviderRayonisasi/bpjs_pcare_provider_rayonisasi.model.state';
...
import {
	getBpjsPcareProviderRayonisasiModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-pcare-provider-rayonisasi-admin-crud-edit',
	templateUrl: './bpjs-pcare-provider-rayonisasi-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-provider-rayonisasi-admin-crud-edit.component.scss',
...
export class AdminBpjsPcareProviderRayonisasiCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcareProviderRayonisasiAudits$: Observable<BpjsPcareProviderRayonisasiModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcareProviderRayonisasiModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcareProviderRayonisasiModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcareProviderRayonisasiModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcareProviderRayonisasiModel();
		...
		if (this.targetModelId) {
			this.bpjsPcareProviderRayonisasiAudits$ = this.store.select(getBpjsPcareProviderRayonisasiModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcareProviderRayonisasiModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
			modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_BPJS_PCARE_PROVIDER_RAYONISASI,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareProviderRayonisasiModelWithId, this.targetModelId)
			...
			model = new BpjsPcareProviderRayonisasiModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
			modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_BPJS_PCARE_PROVIDER_RAYONISASI_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
				modelAction.BpjsPcareProviderRayonisasiModelActionTypes.CREATE_BPJS_PCARE_PROVIDER_RAYONISASI,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcareProviderRayonisasiAction(
				modelAction.BpjsPcareProviderRayonisasiModelActionTypes.UPDATE_BPJS_PCARE_PROVIDER_RAYONISASI,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcareProviderRayonisasiModel = new BpjsPcareProviderRayonisasiModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareProviderRayonisasiModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcareProviderRayonisasiModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcareProviderRayonisasiModel.getProps();
		this.modelRelations = BpjsPcareProviderRayonisasiModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcareProviderRayonisasiModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcareProviderRayonisasiModel {
		let updatedModel: BpjsPcareProviderRayonisasiModel = new BpjsPcareProviderRayonisasiModel(model);
