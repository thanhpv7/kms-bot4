...
import {PurchaseRequisitionModelState} from 'src/app/models/purchaseRequisition/purchase_requisition.model.state';
...
import {
	...
	getPurchaseRequisitionCollectionCount
...
@Component({
	selector: 'cb-purchase-requisition-admin-crud-list',
	templateUrl: './purchase-requisition-admin-crud-list.component.html',
	styleUrls: [
		'./purchase-requisition-admin-crud-list.component.scss',
...
export class AdminPurchaseRequisitionCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PurchaseRequisitionModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PurchaseRequisitionModel.searchFields,
	...
	/**
	 * List of Purchase Requisitions that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	purchaseRequisitions: Observable<PurchaseRequisitionModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	purchaseRequisitionsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.purchaseRequisitionsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.purchaseRequisitionsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseRequisitionAction(
				modelAction.PurchaseRequisitionModelActionTypes.INITIALISE_PURCHASE_REQUISITION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseRequisitionAction(
				modelAction.PurchaseRequisitionModelActionTypes.FETCH_PURCHASE_REQUISITION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.purchaseRequisitions = this.store.select(getPurchaseRequisitionCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPurchaseRequisitionCollectionCount, this.collectionId).subscribe(
				(count) => this.purchaseRequisitionsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PurchaseRequisitionModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PurchaseRequisitionModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PurchaseRequisitionModelActionTypes.EXPORT_ALL_PURCHASE_REQUISITION;
		} else {
			actionType = modelAction.PurchaseRequisitionModelActionTypes.EXPORT_PURCHASE_REQUISITION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PurchaseRequisitionAction(
				modelAction.PurchaseRequisitionModelActionTypes.IMPORT_PURCHASE_REQUISITIONS,
				{
					file: file,
				},
				[new modelAction.PurchaseRequisitionAction(
			...
			window.open(environment.API_URL + '/api/purchaserequisition/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PurchaseRequisitionModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Purchase Requisition entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Purchase Requisition entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PurchaseRequisitionModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PurchaseRequisitionModelActionTypes.DELETE_PURCHASE_REQUISITION;
						} else if (usingSelected) {
							actionType = modelAction.PurchaseRequisitionModelActionTypes.DELETE_ALL_PURCHASE_REQUISITION;
						} else {
							actionType = modelAction.PurchaseRequisitionModelActionTypes.DELETE_PURCHASE_REQUISITION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PurchaseRequisitionAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PURCHASE_REQUISITION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PURCHASE_REQUISITION and DELETE_PURCHASE_REQUISITION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PurchaseRequisitionModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.purchaseRequisitionsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
