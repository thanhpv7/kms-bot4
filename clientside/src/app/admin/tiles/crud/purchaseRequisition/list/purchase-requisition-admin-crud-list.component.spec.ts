...
import {AdminPurchaseRequisitionCrudListComponent} from './purchase-requisition-admin-crud-list.component';
...
import {AdminPurchaseRequisitionCrudModule} from '../purchase-requisition-admin-crud.module';
import {
 ...
 */
describe('Purchase Requisition Crud List Tile Component', () => {

	let fixture;
	let purchaseRequisitionCrudListComponent: AdminPurchaseRequisitionCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PurchaseRequisitionModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '38de54c2-7393-4130-bc82-de8e8d635d39';
	const searchContent = 'text';

	const searchableFieldCount = PurchaseRequisitionModel.searchFields.length;

	const routerState: RouterState = {
		url: 'purchase-requisition-admin-crud',
		urls: ['purchase-requisition-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const purchaseRequisitionDataFactory = new PurchaseRequisitionDataFactory();

	const defaultDataSize = 30;
	const defaultData = purchaseRequisitionDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPurchaseRequisitionCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPurchaseRequisitionCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPurchaseRequisitionCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPurchaseRequisitionCrudListComponent);
			purchaseRequisitionCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(purchaseRequisitionCrudListComponent.purchaseRequisitionsCount).toBe(defaultData.length);
			...
			expect(purchaseRequisitionCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			purchaseRequisitionCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = purchaseRequisitionDataFactory.createAll(filteredDataSize);
			...
			expect(purchaseRequisitionCrudListComponent.purchaseRequisitionsCount).toBe(filteredDataSize);
			expect(purchaseRequisitionCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(purchaseRequisitionCrudListComponent.pageIndex).toBe(0);
			expect(purchaseRequisitionCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			purchaseRequisitionCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		purchaseRequisitionCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		purchaseRequisitionCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
