...
import {BpjsINACBGVariableModelState, BpjsINACBGVariableModelAudit} from 'src/app/models/bpjsINACBGVariable/bpjs_inacbg_variable.model.state';
import {BpjsINACBGVariableModel} from 'src/app/models/bpjsINACBGVariable/bpjs_inacbg_variable.model';
import {
	getBpjsINACBGVariableModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-inacbg-variable-admin-crud-edit',
	templateUrl: './bpjs-inacbg-variable-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-inacbg-variable-admin-crud-edit.component.scss',
...
export class AdminBpjsINACBGVariableCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsINACBGVariableAudits$: Observable<BpjsINACBGVariableModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsINACBGVariableModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsINACBGVariableModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsINACBGVariableModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsINACBGVariableModel();
		...
		if (this.targetModelId) {
			this.bpjsINACBGVariableAudits$ = this.store.select(getBpjsINACBGVariableModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsINACBGVariableModel> = {
		...
		this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
			modelAction.BpjsINACBGVariableModelActionTypes.FETCH_BPJS_INACBG_VARIABLE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsINACBGVariableModelWithId, this.targetModelId)
			...
			model = new BpjsINACBGVariableModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
			modelAction.BpjsINACBGVariableModelActionTypes.FETCH_BPJS_INACBG_VARIABLE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
				modelAction.BpjsINACBGVariableModelActionTypes.CREATE_BPJS_INACBG_VARIABLE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
				modelAction.BpjsINACBGVariableModelActionTypes.UPDATE_BPJS_INACBG_VARIABLE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsINACBGVariableModel = new BpjsINACBGVariableModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsINACBGVariableModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsINACBGVariableModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsINACBGVariableModel.getProps();
		this.modelRelations = BpjsINACBGVariableModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsINACBGVariableModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsINACBGVariableModel {
		let updatedModel: BpjsINACBGVariableModel = new BpjsINACBGVariableModel(model);
