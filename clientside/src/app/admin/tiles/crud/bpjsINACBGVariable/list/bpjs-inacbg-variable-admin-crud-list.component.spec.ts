...
import {AdminBpjsINACBGVariableCrudListComponent} from './bpjs-inacbg-variable-admin-crud-list.component';
...
import {AdminBpjsINACBGVariableCrudModule} from '../bpjs-inacbg-variable-admin-crud.module';
import {
 ...
 */
describe('BPJS INACBG Variable Crud List Tile Component', () => {

	let fixture;
	let bpjsINACBGVariableCrudListComponent: AdminBpjsINACBGVariableCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsINACBGVariableModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '267963e9-97bf-4001-a85a-2311716655f9';
	const searchContent = 'text';

	const searchableFieldCount = BpjsINACBGVariableModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-inacbg-variable-admin-crud',
		urls: ['bpjs-inacbg-variable-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsINACBGVariableDataFactory = new BpjsINACBGVariableDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsINACBGVariableDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsINACBGVariableCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsINACBGVariableCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsINACBGVariableCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsINACBGVariableCrudListComponent);
			bpjsINACBGVariableCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsINACBGVariableCrudListComponent.bpjsINACBGVariablesCount).toBe(defaultData.length);
			...
			expect(bpjsINACBGVariableCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsINACBGVariableCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsINACBGVariableDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsINACBGVariableCrudListComponent.bpjsINACBGVariablesCount).toBe(filteredDataSize);
			expect(bpjsINACBGVariableCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsINACBGVariableCrudListComponent.pageIndex).toBe(0);
			expect(bpjsINACBGVariableCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsINACBGVariableCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsINACBGVariableCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsINACBGVariableCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
