...
import * as modelAction from 'src/app/models/bpjsINACBGVariable/bpjs_inacbg_variable.model.action';
import {BpjsINACBGVariableModelState} from 'src/app/models/bpjsINACBGVariable/bpjs_inacbg_variable.model.state';
...
import {
	...
	getBpjsINACBGVariableCollectionCount
...
import {CrudTileMode} from '../bpjs-inacbg-variable-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-inacbg-variable-admin-crud-list',
	templateUrl: './bpjs-inacbg-variable-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-inacbg-variable-admin-crud-list.component.scss',
...
export class AdminBpjsINACBGVariableCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsINACBGVariableModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsINACBGVariableModel.searchFields,
	...
	/**
	 * List of BPJS INACBG Variables that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsINACBGVariables: Observable<BpjsINACBGVariableModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsINACBGVariablesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsINACBGVariableModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsINACBGVariableModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '267963e9-97bf-4001-a85a-2311716655f9';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsINACBGVariablesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsINACBGVariablesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsINACBGVariableModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
				modelAction.BpjsINACBGVariableModelActionTypes.INITIALISE_BPJS_INACBG_VARIABLE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
				modelAction.BpjsINACBGVariableModelActionTypes.FETCH_BPJS_INACBG_VARIABLE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsINACBGVariableAction(
						modelAction.BpjsINACBGVariableModelActionTypes.COUNT_BPJS_INACBG_VARIABLES,
						null
					)
				]
			))
		...
		this.bpjsINACBGVariables = this.store.select(getBpjsINACBGVariableCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsINACBGVariableCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsINACBGVariablesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-inacbg-variable');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsINACBGVariableModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsINACBGVariableModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsINACBGVariableModelActionTypes.EXPORT_ALL_BPJS_INACBG_VARIABLE;
		} else {
			actionType = modelAction.BpjsINACBGVariableModelActionTypes.EXPORT_BPJS_INACBG_VARIABLE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
				modelAction.BpjsINACBGVariableModelActionTypes.IMPORT_BPJS_INACBG_VARIABLES,
				{
					file: file,
				},
				[new modelAction.BpjsINACBGVariableAction(
			...
			window.open(environment.API_URL + '/api/bpjsinacbgvariable/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsINACBGVariableModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS INACBG Variable entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS INACBG Variable entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsINACBGVariableModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsINACBGVariableModelActionTypes.DELETE_BPJS_INACBG_VARIABLE;
						} else if (usingSelected) {
							actionType = modelAction.BpjsINACBGVariableModelActionTypes.DELETE_ALL_BPJS_INACBG_VARIABLE;
						} else {
							actionType = modelAction.BpjsINACBGVariableModelActionTypes.DELETE_BPJS_INACBG_VARIABLE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsINACBGVariableAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_INACBG_VARIABLE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_INACBG_VARIABLE and DELETE_BPJS_INACBG_VARIABLE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsINACBGVariableModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsINACBGVariableModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsINACBGVariablesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
