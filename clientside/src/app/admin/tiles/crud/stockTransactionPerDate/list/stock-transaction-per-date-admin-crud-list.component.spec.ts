...
import {AdminStockTransactionPerDateCrudListComponent} from './stock-transaction-per-date-admin-crud-list.component';
...
import {AdminStockTransactionPerDateCrudModule} from '../stock-transaction-per-date-admin-crud.module';
import {
	StockTransactionPerDateModelState,
	initialState as initialStockTransactionPerDateModelState
} from 'src/app/models/stockTransactionPerDate/stock_transaction_per_date.model.state';
 ...
 */
describe('Stock Transaction Per Date Crud List Tile Component', () => {

	let fixture;
	let stockTransactionPerDateCrudListComponent: AdminStockTransactionPerDateCrudListComponent;

	let store: MockStore<{ model: StockTransactionPerDateModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let stockTransactionPerDateModelState: StockTransactionPerDateModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<StockTransactionPerDateModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '04ec761f-343a-4544-9491-974021a52ec6';
	const searchContent = 'text';

	const searchableFieldCount = StockTransactionPerDateModel.searchFields.length;

	const routerState: RouterState = {
		url: 'stock-transaction-per-date-admin-crud',
		urls: ['stock-transaction-per-date-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const stockTransactionPerDateDataFactory = new StockTransactionPerDateDataFactory();

	const defaultDataSize = 30;
	const defaultData = stockTransactionPerDateDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: StockTransactionPerDateModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		stockTransactionPerDateModelState = initialStockTransactionPerDateModelState;
		store.setState({model: stockTransactionPerDateModelState});
		...
		spyOn(store, 'select')
			.withArgs(getStockTransactionPerDateCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getStockTransactionPerDateCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminStockTransactionPerDateCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminStockTransactionPerDateCrudListComponent);
			stockTransactionPerDateCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(stockTransactionPerDateCrudListComponent.stockTransactionPerDatesCount).toBe(defaultData.length);
			...
			expect(stockTransactionPerDateCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			stockTransactionPerDateCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = stockTransactionPerDateDataFactory.createAll(filteredDataSize);
			...
			expect(stockTransactionPerDateCrudListComponent.stockTransactionPerDatesCount).toBe(filteredDataSize);
			expect(stockTransactionPerDateCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(stockTransactionPerDateCrudListComponent.pageIndex).toBe(0);
			expect(stockTransactionPerDateCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			stockTransactionPerDateCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		stockTransactionPerDateCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		stockTransactionPerDateCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
