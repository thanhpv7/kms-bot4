...
import * as modelAction from 'src/app/models/stockTransactionPerDate/stock_transaction_per_date.model.action';
import {StockTransactionPerDateModelState} from 'src/app/models/stockTransactionPerDate/stock_transaction_per_date.model.state';
...
import {
	getStockTransactionPerDateCollectionModels,
	getStockTransactionPerDateCollectionCount
...
import {CrudTileMode} from '../stock-transaction-per-date-admin-crud.component';
...
@Component({
	selector: 'cb-stock-transaction-per-date-admin-crud-list',
	templateUrl: './stock-transaction-per-date-admin-crud-list.component.html',
	styleUrls: [
		'./stock-transaction-per-date-admin-crud-list.component.scss',
...
export class AdminStockTransactionPerDateCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<StockTransactionPerDateModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...StockTransactionPerDateModel.searchFields,
	...
	/**
	 * List of Stock Transaction Per Dates that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	stockTransactionPerDates: Observable<StockTransactionPerDateModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	stockTransactionPerDatesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = StockTransactionPerDateModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = StockTransactionPerDateModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '04ec761f-343a-4544-9491-974021a52ec6';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.stockTransactionPerDatesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.stockTransactionPerDatesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: StockTransactionPerDateModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockTransactionPerDateAction(
				modelAction.StockTransactionPerDateModelActionTypes.INITIALISE_STOCK_TRANSACTION_PER_DATE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockTransactionPerDateAction(
				modelAction.StockTransactionPerDateModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.StockTransactionPerDateAction(
						modelAction.StockTransactionPerDateModelActionTypes.COUNT_STOCK_TRANSACTION_PER_DATES,
						null
					)
				]
			))
		...
		this.stockTransactionPerDates = this.store.select(getStockTransactionPerDateCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getStockTransactionPerDateCollectionCount, this.collectionId).subscribe(
				(count) => this.stockTransactionPerDatesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('stock-transaction-per-date');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: StockTransactionPerDateModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.StockTransactionPerDateModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.StockTransactionPerDateModelActionTypes.EXPORT_ALL_STOCK_TRANSACTION_PER_DATE;
		} else {
			actionType = modelAction.StockTransactionPerDateModelActionTypes.EXPORT_STOCK_TRANSACTION_PER_DATE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.StockTransactionPerDateAction(
				modelAction.StockTransactionPerDateModelActionTypes.IMPORT_STOCK_TRANSACTION_PER_DATES,
				{
					file: file,
				},
				[new modelAction.StockTransactionPerDateAction(
			...
			window.open(environment.API_URL + '/api/stocktransactionperdate/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: StockTransactionPerDateModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Stock Transaction Per Date entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Stock Transaction Per Date entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.StockTransactionPerDateModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.StockTransactionPerDateModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE;
						} else if (usingSelected) {
							actionType = modelAction.StockTransactionPerDateModelActionTypes.DELETE_ALL_STOCK_TRANSACTION_PER_DATE;
						} else {
							actionType = modelAction.StockTransactionPerDateModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.StockTransactionPerDateAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_STOCK_TRANSACTION_PER_DATE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_STOCK_TRANSACTION_PER_DATE and DELETE_STOCK_TRANSACTION_PER_DATE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<StockTransactionPerDateModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = StockTransactionPerDateModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.stockTransactionPerDatesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
