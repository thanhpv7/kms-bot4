...
import {StockTransactionPerDateModelState, StockTransactionPerDateModelAudit} from 'src/app/models/stockTransactionPerDate/stock_transaction_per_date.model.state';
import {StockTransactionPerDateModel} from 'src/app/models/stockTransactionPerDate/stock_transaction_per_date.model';
import {
	getStockTransactionPerDateModelAuditsByEntityId,
	getStockTransactionPerDateModelWithId,
	getCountStockTransactionPerDateModels,
...
@Component({
	selector: 'cb-stock-transaction-per-date-admin-crud-edit',
	templateUrl: './stock-transaction-per-date-admin-crud-edit.component.html',
	styleUrls: [
		'./stock-transaction-per-date-admin-crud-edit.component.scss',
...
export class AdminStockTransactionPerDateCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	stockTransactionPerDateAudits$: Observable<StockTransactionPerDateModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'stockTransactionPerDates';
	...
	warehouseOppositeRelation = 'stockTransactionPerDates';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = StockTransactionPerDateModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: StockTransactionPerDateModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountStockTransactionPerDateModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new StockTransactionPerDateModel();
		...
		if (this.targetModelId) {
			this.stockTransactionPerDateAudits$ = this.store.select(getStockTransactionPerDateModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<StockTransactionPerDateModel> = {
		...
		this.store.dispatch(new modelAction.StockTransactionPerDateAction(
			modelAction.StockTransactionPerDateModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getStockTransactionPerDateModelWithId, this.targetModelId)
			...
			model = new StockTransactionPerDateModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.StockTransactionPerDateAction(
			modelAction.StockTransactionPerDateModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.StockTransactionPerDateAction(
				modelAction.StockTransactionPerDateModelActionTypes.CREATE_STOCK_TRANSACTION_PER_DATE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.StockTransactionPerDateAction(
				modelAction.StockTransactionPerDateModelActionTypes.UPDATE_STOCK_TRANSACTION_PER_DATE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: StockTransactionPerDateModel = new StockTransactionPerDateModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getStockTransactionPerDateModelWithId, this.targetModelId).subscribe(model => {
					let differences = new StockTransactionPerDateModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = StockTransactionPerDateModel.getProps();
		this.modelRelations = StockTransactionPerDateModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: StockTransactionPerDateModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): StockTransactionPerDateModel {
		let updatedModel: StockTransactionPerDateModel = new StockTransactionPerDateModel(model);
