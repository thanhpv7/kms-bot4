...
import * as modelAction from 'src/app/models/patientDetailAddress/patient_detail_address.model.action';
import {PatientDetailAddressModelState, PatientDetailAddressModelAudit} from 'src/app/models/patientDetailAddress/patient_detail_address.model.state';
import {PatientDetailAddressModel} from 'src/app/models/patientDetailAddress/patient_detail_address.model';
import {
	getPatientDetailAddressModelAuditsByEntityId,
	getPatientDetailAddressModelWithId,
	getCountPatientDetailAddressModels,
...
import {CrudTileMode} from '../patient-detail-address-admin-crud.component';
...
@Component({
	selector: 'cb-patient-detail-address-admin-crud-edit',
	templateUrl: './patient-detail-address-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-detail-address-admin-crud-edit.component.scss',
...
export class AdminPatientDetailAddressCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '6289a187-be01-400f-a667-ee2f5fdd56c1';
	 ...
	 */
	patientDetailAddressAudits$: Observable<PatientDetailAddressModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientDetailAddress';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientDetailAddressModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientDetailAddressModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientDetailAddressModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientDetailAddressModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientDetailAddressModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientDetailAddressModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientDetailAddressModel();
		...
		if (this.targetModelId) {
			this.patientDetailAddressAudits$ = this.store.select(getPatientDetailAddressModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientDetailAddressModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientDetailAddressAction(
			modelAction.PatientDetailAddressModelActionTypes.FETCH_PATIENT_DETAIL_ADDRESS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientDetailAddressModelWithId, this.targetModelId)
			...
			model = new PatientDetailAddressModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-detail-address');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientDetailAddressAction(
			modelAction.PatientDetailAddressModelActionTypes.FETCH_PATIENT_DETAIL_ADDRESS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientDetailAddressModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientDetailAddressAction(
						modelAction.PatientDetailAddressModelActionTypes.COUNT_PATIENT_DETAIL_ADDRESSS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientDetailAddressAction(
				modelAction.PatientDetailAddressModelActionTypes.CREATE_PATIENT_DETAIL_ADDRESS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientDetailAddressAction(
				modelAction.PatientDetailAddressModelActionTypes.UPDATE_PATIENT_DETAIL_ADDRESS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientDetailAddressModel = new PatientDetailAddressModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientDetailAddressModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientDetailAddressModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientDetailAddressModel.getProps();
		this.modelRelations = PatientDetailAddressModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientDetailAddressModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientDetailAddressModel {
		let updatedModel: PatientDetailAddressModel = new PatientDetailAddressModel(model);
