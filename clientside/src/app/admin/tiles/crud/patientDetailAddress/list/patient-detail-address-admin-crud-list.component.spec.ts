...
import {AdminPatientDetailAddressCrudListComponent} from './patient-detail-address-admin-crud-list.component';
...
import {AdminPatientDetailAddressCrudModule} from '../patient-detail-address-admin-crud.module';
import {
	PatientDetailAddressModelState,
	initialState as initialPatientDetailAddressModelState
} from 'src/app/models/patientDetailAddress/patient_detail_address.model.state';
 ...
 */
describe('Patient Detail Address Crud List Tile Component', () => {

	let fixture;
	let patientDetailAddressCrudListComponent: AdminPatientDetailAddressCrudListComponent;

	let store: MockStore<{ model: PatientDetailAddressModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientDetailAddressModelState: PatientDetailAddressModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientDetailAddressModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6289a187-be01-400f-a667-ee2f5fdd56c1';
	const searchContent = 'text';

	const searchableFieldCount = PatientDetailAddressModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-detail-address-admin-crud',
		urls: ['patient-detail-address-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientDetailAddressDataFactory = new PatientDetailAddressDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientDetailAddressDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientDetailAddressModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientDetailAddressModelState = initialPatientDetailAddressModelState;
		store.setState({model: patientDetailAddressModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientDetailAddressCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientDetailAddressCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientDetailAddressCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientDetailAddressCrudListComponent);
			patientDetailAddressCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientDetailAddressCrudListComponent.patientDetailAddresssCount).toBe(defaultData.length);
			...
			expect(patientDetailAddressCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientDetailAddressCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientDetailAddressDataFactory.createAll(filteredDataSize);
			...
			expect(patientDetailAddressCrudListComponent.patientDetailAddresssCount).toBe(filteredDataSize);
			expect(patientDetailAddressCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientDetailAddressCrudListComponent.pageIndex).toBe(0);
			expect(patientDetailAddressCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientDetailAddressCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientDetailAddressCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientDetailAddressCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
