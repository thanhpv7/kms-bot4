...
import {PatientDetailAddressModelState} from 'src/app/models/patientDetailAddress/patient_detail_address.model.state';
...
import {
	getPatientDetailAddressCollectionModels,
	getPatientDetailAddressCollectionCount
...
@Component({
	selector: 'cb-patient-detail-address-admin-crud-list',
	templateUrl: './patient-detail-address-admin-crud-list.component.html',
	styleUrls: [
		'./patient-detail-address-admin-crud-list.component.scss',
...
export class AdminPatientDetailAddressCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientDetailAddressModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientDetailAddressModel.searchFields,
	...
	/**
	 * List of Patient Detail Addresss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientDetailAddresss: Observable<PatientDetailAddressModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientDetailAddresssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientDetailAddresssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientDetailAddresssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientDetailAddressAction(
				modelAction.PatientDetailAddressModelActionTypes.INITIALISE_PATIENT_DETAIL_ADDRESS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientDetailAddressAction(
				modelAction.PatientDetailAddressModelActionTypes.FETCH_PATIENT_DETAIL_ADDRESS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientDetailAddresss = this.store.select(getPatientDetailAddressCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientDetailAddressCollectionCount, this.collectionId).subscribe(
				(count) => this.patientDetailAddresssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientDetailAddressModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientDetailAddressModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientDetailAddressModelActionTypes.EXPORT_ALL_PATIENT_DETAIL_ADDRESS;
		} else {
			actionType = modelAction.PatientDetailAddressModelActionTypes.EXPORT_PATIENT_DETAIL_ADDRESS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientDetailAddressAction(
				modelAction.PatientDetailAddressModelActionTypes.IMPORT_PATIENT_DETAIL_ADDRESSS,
				{
					file: file,
				},
				[new modelAction.PatientDetailAddressAction(
			...
			window.open(environment.API_URL + '/api/patientdetailaddress/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientDetailAddressModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Detail Address entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Detail Address entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientDetailAddressModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientDetailAddressModelActionTypes.DELETE_PATIENT_DETAIL_ADDRESS;
						} else if (usingSelected) {
							actionType = modelAction.PatientDetailAddressModelActionTypes.DELETE_ALL_PATIENT_DETAIL_ADDRESS;
						} else {
							actionType = modelAction.PatientDetailAddressModelActionTypes.DELETE_PATIENT_DETAIL_ADDRESS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientDetailAddressAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_DETAIL_ADDRESS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_DETAIL_ADDRESS and DELETE_PATIENT_DETAIL_ADDRESS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientDetailAddressModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientDetailAddresssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
