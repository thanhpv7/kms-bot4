...
import * as modelAction from 'src/app/models/bpjsListTask/bpjs_list_task.model.action';
import {BpjsListTaskModelState} from 'src/app/models/bpjsListTask/bpjs_list_task.model.state';
...
import {CrudTileMode} from '../bpjs-list-task-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-list-task-admin-crud-list',
	templateUrl: './bpjs-list-task-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-list-task-admin-crud-list.component.scss',
...
export class AdminBpjsListTaskCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsListTaskModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsListTaskModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsListTaskModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '63992d4c-11f5-469b-a757-e422644cb8ce';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsListTasksCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsListTasksCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsListTaskModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsListTaskAction(
				modelAction.BpjsListTaskModelActionTypes.INITIALISE_BPJS_LIST_TASK_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsListTaskAction(
				modelAction.BpjsListTaskModelActionTypes.FETCH_BPJS_LIST_TASK_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsListTaskAction(
						modelAction.BpjsListTaskModelActionTypes.COUNT_BPJS_LIST_TASKS,
						null
					)
				]
			))
		...
		this.bpjsListTasks = this.store.select(getBpjsListTaskCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsListTaskCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsListTasksCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsListTaskModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsListTaskModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsListTaskModelActionTypes.EXPORT_ALL_BPJS_LIST_TASK;
		} else {
			actionType = modelAction.BpjsListTaskModelActionTypes.EXPORT_BPJS_LIST_TASK_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsListTaskAction(
				modelAction.BpjsListTaskModelActionTypes.IMPORT_BPJS_LIST_TASKS,
				{
					file: file,
				},
				[new modelAction.BpjsListTaskAction(
			...
			window.open(environment.API_URL + '/api/bpjslisttask/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsListTaskModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS List Task entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS List Task entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsListTaskModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsListTaskModelActionTypes.DELETE_BPJS_LIST_TASK;
						} else if (usingSelected) {
							actionType = modelAction.BpjsListTaskModelActionTypes.DELETE_ALL_BPJS_LIST_TASK;
						} else {
							actionType = modelAction.BpjsListTaskModelActionTypes.DELETE_BPJS_LIST_TASK_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsListTaskAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_LIST_TASK when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_LIST_TASK and DELETE_BPJS_LIST_TASK_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsListTaskModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsListTaskModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsListTasksCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
