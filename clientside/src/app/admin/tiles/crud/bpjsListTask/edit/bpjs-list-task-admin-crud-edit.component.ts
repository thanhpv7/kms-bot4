...
import {BpjsListTaskModelState, BpjsListTaskModelAudit} from 'src/app/models/bpjsListTask/bpjs_list_task.model.state';
import {BpjsListTaskModel} from 'src/app/models/bpjsListTask/bpjs_list_task.model';
import {
	getBpjsListTaskModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-list-task-admin-crud-edit',
	templateUrl: './bpjs-list-task-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-list-task-admin-crud-edit.component.scss',
...
export class AdminBpjsListTaskCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsListTaskAudits$: Observable<BpjsListTaskModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsListTaskModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsListTaskModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsListTaskModel();
		...
		if (this.targetModelId) {
			this.bpjsListTaskAudits$ = this.store.select(getBpjsListTaskModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsListTaskModel> = {
		...
		this.store.dispatch(new modelAction.BpjsListTaskAction(
			modelAction.BpjsListTaskModelActionTypes.FETCH_BPJS_LIST_TASK,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsListTaskModelWithId, this.targetModelId)
			...
			model = new BpjsListTaskModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsListTaskAction(
			modelAction.BpjsListTaskModelActionTypes.FETCH_BPJS_LIST_TASK_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsListTaskAction(
				modelAction.BpjsListTaskModelActionTypes.CREATE_BPJS_LIST_TASK,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsListTaskAction(
				modelAction.BpjsListTaskModelActionTypes.UPDATE_BPJS_LIST_TASK,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsListTaskModel = new BpjsListTaskModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsListTaskModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsListTaskModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsListTaskModel.getProps();
		this.modelRelations = BpjsListTaskModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsListTaskModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsListTaskModel {
		let updatedModel: BpjsListTaskModel = new BpjsListTaskModel(model);
