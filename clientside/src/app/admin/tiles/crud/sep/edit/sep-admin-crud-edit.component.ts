...
import {SepModelState, SepModelAudit} from 'src/app/models/sep/sep.model.state';
import {SepModel} from 'src/app/models/sep/sep.model';
import {
	getSepModelAuditsByEntityId,
	getSepModelWithId,
	getCountSepModels,
...
@Component({
	selector: 'cb-sep-admin-crud-edit',
	templateUrl: './sep-admin-crud-edit.component.html',
	styleUrls: [
		'./sep-admin-crud-edit.component.scss',
...
export class AdminSepCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	sepAudits$: Observable<SepModelAudit[]>;
	...
	registrationOppositeRelation = 'sep';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SepModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSepModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.targetModelId) {
			this.sepAudits$ = this.store.select(getSepModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SepModel> = {
		...
		this.store.dispatch(new modelAction.SepAction(
			modelAction.SepModelActionTypes.FETCH_SEP,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSepModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SepAction(
			modelAction.SepModelActionTypes.FETCH_SEP_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<SepModel> = {
				...
				queryParams: {
					persistRelations: this.registrationMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.SepAction(
				modelAction.SepModelActionTypes.CREATE_SEP,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SepAction(
				modelAction.SepModelActionTypes.UPDATE_SEP,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SepModel = new SepModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSepModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SepModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SepModel.getProps();
		this.modelRelations = SepModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: SepModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SepModel {
		let updatedModel: SepModel = new SepModel(model);
