...
import * as modelAction from 'src/app/models/sep/sep.model.action';
import {SepModelState} from 'src/app/models/sep/sep.model.state';
...
import {CrudTileMode} from '../sep-admin-crud.component';
...
@Component({
	selector: 'cb-sep-admin-crud-list',
	templateUrl: './sep-admin-crud-list.component.html',
	styleUrls: [
		'./sep-admin-crud-list.component.scss',
...
export class AdminSepCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SepModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SepModel.searchFields,
	...
	/**
	 * List of SEPs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	seps: Observable<SepModel[]>;
	 ...
	 */
	modelProperties: ModelProperty[] = SepModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = SepModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '4e7532f0-1635-4041-95cc-70ab6cc581e4';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.sepsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.sepsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: SepModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SepAction(
				modelAction.SepModelActionTypes.INITIALISE_SEP_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SepAction(
				modelAction.SepModelActionTypes.FETCH_SEP_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.SepAction(
						modelAction.SepModelActionTypes.COUNT_SEPS,
						null
					)
				]
			))
		...
		this.seps = this.store.select(getSepCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSepCollectionCount, this.collectionId).subscribe(
				(count) => this.sepsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SepModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SepModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SepModelActionTypes.EXPORT_ALL_SEP;
		} else {
			actionType = modelAction.SepModelActionTypes.EXPORT_SEP_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SepAction(
				modelAction.SepModelActionTypes.IMPORT_SEPS,
				{
					file: file,
				},
				[new modelAction.SepAction(
			...
			window.open(environment.API_URL + '/api/sep/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SepModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected SEP entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected SEP entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SepModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SepModelActionTypes.DELETE_SEP;
						} else if (usingSelected) {
							actionType = modelAction.SepModelActionTypes.DELETE_ALL_SEP;
						} else {
							actionType = modelAction.SepModelActionTypes.DELETE_SEP_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SepAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SEP when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SEP and DELETE_SEP_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<SepModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SepModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.sepsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
