...
import * as modelAction from 'src/app/models/bpjsDrugGenericPRB/bpjs_drug_generic_prb.model.action';
import {BpjsDrugGenericPRBModelState, BpjsDrugGenericPRBModelAudit} from 'src/app/models/bpjsDrugGenericPRB/bpjs_drug_generic_prb.model.state';
import {BpjsDrugGenericPRBModel} from 'src/app/models/bpjsDrugGenericPRB/bpjs_drug_generic_prb.model';
import {
	getBpjsDrugGenericPRBModelAuditsByEntityId,
	getBpjsDrugGenericPRBModelWithId,
	getCountBpjsDrugGenericPRBModels,
...
import {CrudTileMode} from '../bpjs-drug-generic-prb-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-drug-generic-prb-admin-crud-edit',
	templateUrl: './bpjs-drug-generic-prb-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-drug-generic-prb-admin-crud-edit.component.scss',
...
export class AdminBpjsDrugGenericPRBCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'da971083-8678-46df-914c-3ad1747b52f2';
	 ...
	 */
	bpjsDrugGenericPRBAudits$: Observable<BpjsDrugGenericPRBModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'bpjsDrugGenericPRB';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Drug Generic PRB and BPJS PRB Details here] off begin
	bpjsPRBDetailsMultiCrudActive: boolean = false;
	bpjsPRBDetailsDisplayName = 'BPJS PRB Details';
	bpjsPRBDetailsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Drug Generic PRB and BPJS PRB Details here] end
	bpjsPRBDetailsOppositeRelation = 'bpjsDrugGenericPRB';
	bpjsPRBDetailsRelations: ModelRelation[] = this.bpjsPRBDetailsMultiCrudActive ? BpjsPRBDetailModel.getRelations()
		.map(relation => {
			if (relation.id === this.bpjsPRBDetailsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsDrugGenericPRBModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsDrugGenericPRBModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsDrugGenericPRBModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'bpjsPRBDetails' && this.bpjsPRBDetailsMultiCrudActive) {
	 ...
	 */
	targetModel: BpjsDrugGenericPRBModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsDrugGenericPRBModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsDrugGenericPRBModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsDrugGenericPRBModel();
		...
		if (this.targetModelId) {
			this.bpjsDrugGenericPRBAudits$ = this.store.select(getBpjsDrugGenericPRBModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsDrugGenericPRBModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
			modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_BPJS_DRUG_GENERIC_PRB,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsDrugGenericPRBModelWithId, this.targetModelId)
			...
			model = new BpjsDrugGenericPRBModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-drug-generic-prb');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
			modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_BPJS_DRUG_GENERIC_PRB_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsDrugGenericPRBModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.stockCatalogueMultiCrudActive || this.bpjsPRBDetailsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsDrugGenericPRBAction(
						modelAction.BpjsDrugGenericPRBModelActionTypes.COUNT_BPJS_DRUG_GENERIC_PRBS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
				modelAction.BpjsDrugGenericPRBModelActionTypes.CREATE_BPJS_DRUG_GENERIC_PRB,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
				modelAction.BpjsDrugGenericPRBModelActionTypes.UPDATE_BPJS_DRUG_GENERIC_PRB,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsDrugGenericPRBModel = new BpjsDrugGenericPRBModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsDrugGenericPRBModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsDrugGenericPRBModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.bpjsPRBDetailsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsPRBDetailModel.getProps(), this.bpjsPRBDetailsRelations, this.isViewOnly),
					new BpjsPRBDetailModel(),
					BpjsPRBDetailModel,
					this.bpjsPRBDetailsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.bpjsPRBDetailsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsDrugGenericPRBModel.getProps();
		this.modelRelations = BpjsDrugGenericPRBModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'bpjsPRBDetails';
		}).hideElement = this.bpjsPRBDetailsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'bpjsPRBDetails').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.bpjsPRBDetailsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.bpjsPRBDetailsDisplayName, many: true});
			this.store.dispatch(new bpjsPRBDetailModelAction.BpjsPRBDetailAction(
				bpjsPRBDetailModelAction.BpjsPRBDetailModelActionTypes.INITIALISE_BPJS_PRB_DETAIL_COLLECTION_STATE,
				{
					collectionId: this.bpjsPRBDetailsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsPRBDetailCollectionModels, this.bpjsPRBDetailsCollectionId).subscribe(models => {
				models.forEach(bpjsPRBDetails => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(BpjsPRBDetailModel.getProps(), this.bpjsPRBDetailsRelations, this.isViewOnly),
						bpjsPRBDetails,
						BpjsPRBDetailModel,
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.bpjsPRBDetailsIds && this.targetModel.bpjsPRBDetailsIds.length > 0 && this.bpjsPRBDetailsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsPRBDetailModel,
				this.bpjsPRBDetailsOppositeRelation,
				this.bpjsPRBDetailsCollectionId,
				this.targetModel.bpjsPRBDetailsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsDrugGenericPRBModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsDrugGenericPRBModel {
		let updatedModel: BpjsDrugGenericPRBModel = new BpjsDrugGenericPRBModel(model);
		...
		updatedModel.bpjsPRBDetails = [];
		if (this.bpjsPRBDetailsMultiCrudActive) {
			if (event.additionalEntities.bpjsPRBDetails) {
				event.additionalEntities.bpjsPRBDetails.forEach(entity => {
					updatedModel.bpjsPRBDetails.push(entity);
				});
			}

			if (!updatedModel.bpjsPRBDetails.length) {
				updatedModel.bpjsPRBDetailsIds = [];
			}
		}
		return updatedModel;
	}
