...
import {AdminBpjsDrugGenericPRBCrudListComponent} from './bpjs-drug-generic-prb-admin-crud-list.component';
...
import {AdminBpjsDrugGenericPRBCrudModule} from '../bpjs-drug-generic-prb-admin-crud.module';
import {
	BpjsDrugGenericPRBModelState,
	initialState as initialBpjsDrugGenericPRBModelState
} from 'src/app/models/bpjsDrugGenericPRB/bpjs_drug_generic_prb.model.state';
 ...
 */
describe('BPJS Drug Generic PRB Crud List Tile Component', () => {

	let fixture;
	let bpjsDrugGenericPRBCrudListComponent: AdminBpjsDrugGenericPRBCrudListComponent;

	let store: MockStore<{ model: BpjsDrugGenericPRBModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsDrugGenericPRBModelState: BpjsDrugGenericPRBModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsDrugGenericPRBModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'da971083-8678-46df-914c-3ad1747b52f2';
	const searchContent = 'text';

	const searchableFieldCount = BpjsDrugGenericPRBModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-drug-generic-prb-admin-crud',
		urls: ['bpjs-drug-generic-prb-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsDrugGenericPRBDataFactory = new BpjsDrugGenericPRBDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsDrugGenericPRBDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsDrugGenericPRBModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsDrugGenericPRBModelState = initialBpjsDrugGenericPRBModelState;
		store.setState({model: bpjsDrugGenericPRBModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsDrugGenericPRBCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsDrugGenericPRBCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsDrugGenericPRBCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsDrugGenericPRBCrudListComponent);
			bpjsDrugGenericPRBCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsDrugGenericPRBCrudListComponent.bpjsDrugGenericPRBsCount).toBe(defaultData.length);
			...
			expect(bpjsDrugGenericPRBCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsDrugGenericPRBCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsDrugGenericPRBDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsDrugGenericPRBCrudListComponent.bpjsDrugGenericPRBsCount).toBe(filteredDataSize);
			expect(bpjsDrugGenericPRBCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsDrugGenericPRBCrudListComponent.pageIndex).toBe(0);
			expect(bpjsDrugGenericPRBCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsDrugGenericPRBCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsDrugGenericPRBCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsDrugGenericPRBCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
