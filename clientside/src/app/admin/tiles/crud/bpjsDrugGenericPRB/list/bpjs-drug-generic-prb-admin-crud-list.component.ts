...
import {BpjsDrugGenericPRBModelState} from 'src/app/models/bpjsDrugGenericPRB/bpjs_drug_generic_prb.model.state';
...
import {
	getBpjsDrugGenericPRBCollectionModels,
	getBpjsDrugGenericPRBCollectionCount
...
@Component({
	selector: 'cb-bpjs-drug-generic-prb-admin-crud-list',
	templateUrl: './bpjs-drug-generic-prb-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-drug-generic-prb-admin-crud-list.component.scss',
...
export class AdminBpjsDrugGenericPRBCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsDrugGenericPRBModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsDrugGenericPRBModel.searchFields,
	...
	/**
	 * List of BPJS Drug Generic PRBs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsDrugGenericPRBs: Observable<BpjsDrugGenericPRBModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsDrugGenericPRBsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsDrugGenericPRBsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsDrugGenericPRBsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
				modelAction.BpjsDrugGenericPRBModelActionTypes.INITIALISE_BPJS_DRUG_GENERIC_PRB_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
				modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_BPJS_DRUG_GENERIC_PRB_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsDrugGenericPRBs = this.store.select(getBpjsDrugGenericPRBCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsDrugGenericPRBCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsDrugGenericPRBsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsDrugGenericPRBModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsDrugGenericPRBModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsDrugGenericPRBModelActionTypes.EXPORT_ALL_BPJS_DRUG_GENERIC_PRB;
		} else {
			actionType = modelAction.BpjsDrugGenericPRBModelActionTypes.EXPORT_BPJS_DRUG_GENERIC_PRB_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
				modelAction.BpjsDrugGenericPRBModelActionTypes.IMPORT_BPJS_DRUG_GENERIC_PRBS,
				{
					file: file,
				},
				[new modelAction.BpjsDrugGenericPRBAction(
			...
			window.open(environment.API_URL + '/api/bpjsdruggenericprb/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsDrugGenericPRBModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Drug Generic PRB entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Drug Generic PRB entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsDrugGenericPRBModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsDrugGenericPRBModelActionTypes.DELETE_BPJS_DRUG_GENERIC_PRB;
						} else if (usingSelected) {
							actionType = modelAction.BpjsDrugGenericPRBModelActionTypes.DELETE_ALL_BPJS_DRUG_GENERIC_PRB;
						} else {
							actionType = modelAction.BpjsDrugGenericPRBModelActionTypes.DELETE_BPJS_DRUG_GENERIC_PRB_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsDrugGenericPRBAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_DRUG_GENERIC_PRB when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_DRUG_GENERIC_PRB and DELETE_BPJS_DRUG_GENERIC_PRB_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsDrugGenericPRBModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsDrugGenericPRBsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
