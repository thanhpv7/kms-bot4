...
import * as modelAction from 'src/app/models/diagnosisNanda/diagnosis_nanda.model.action';
import {DiagnosisNandaModelState, DiagnosisNandaModelAudit} from 'src/app/models/diagnosisNanda/diagnosis_nanda.model.state';
import {DiagnosisNandaModel} from 'src/app/models/diagnosisNanda/diagnosis_nanda.model';
import {
	getDiagnosisNandaModelAuditsByEntityId,
	getDiagnosisNandaModelWithId,
	getCountDiagnosisNandaModels,
...
import {CrudTileMode} from '../diagnosis-nanda-admin-crud.component';
...
@Component({
	selector: 'cb-diagnosis-nanda-admin-crud-edit',
	templateUrl: './diagnosis-nanda-admin-crud-edit.component.html',
	styleUrls: [
		'./diagnosis-nanda-admin-crud-edit.component.scss',
...
export class AdminDiagnosisNandaCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '2292d80c-da9e-48c7-8031-5f0516107b85';
	 ...
	 */
	diagnosisNandaAudits$: Observable<DiagnosisNandaModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'diagnosisNandas';
	...
	nursingAnesthesiaDetailCollectionId: string = uuidv4();
	...
	nursingAnesthesiaDetailRelations: ModelRelation[] = this.nursingAnesthesiaDetailMultiCrudActive ? OperatingTheaterMedicalExaminationRecordModel.getRelations()
		.map(relation => {
	...
	nursingSurgicalDetailCollectionId: string = uuidv4();
	...
	nursingSurgicalDetailRelations: ModelRelation[] = this.nursingSurgicalDetailMultiCrudActive ? OperatingTheaterMedicalExaminationRecordModel.getRelations()
		.map(relation => {
	...
	nursingMultiCrudActive: boolean = false;
	nursingDisplayName = 'NURSING';
	nursingCollectionId: string = uuidv4();
	...
	nursingOppositeRelation = 'diagnosisNandas';
	nursingRelations: ModelRelation[] = this.nursingMultiCrudActive ? DiagnosesAndProceduresModel.getRelations()
		.map(relation => {
			if (relation.id === this.nursingOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = DiagnosisNandaModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = DiagnosisNandaModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DiagnosisNandaModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'nursing' && this.nursingMultiCrudActive) {
	 ...
	 */
	targetModel: DiagnosisNandaModel;
	...
	constructor(
		private readonly store: Store<{ model: DiagnosisNandaModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDiagnosisNandaModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DiagnosisNandaModel();
		...
		if (this.targetModelId) {
			this.diagnosisNandaAudits$ = this.store.select(getDiagnosisNandaModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DiagnosisNandaModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.DiagnosisNandaAction(
			modelAction.DiagnosisNandaModelActionTypes.FETCH_DIAGNOSIS_NANDA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDiagnosisNandaModelWithId, this.targetModelId)
			...
			model = new DiagnosisNandaModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DiagnosisNandaAction(
			modelAction.DiagnosisNandaModelActionTypes.FETCH_DIAGNOSIS_NANDA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<DiagnosisNandaModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.medicalExaminationRecordMultiCrudActive || this.nursingAnesthesiaDetailMultiCrudActive || this.nursingSurgicalDetailMultiCrudActive || this.nursingMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.DiagnosisNandaAction(
						modelAction.DiagnosisNandaModelActionTypes.COUNT_DIAGNOSIS_NANDAS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.DiagnosisNandaAction(
				modelAction.DiagnosisNandaModelActionTypes.CREATE_DIAGNOSIS_NANDA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DiagnosisNandaAction(
				modelAction.DiagnosisNandaModelActionTypes.UPDATE_DIAGNOSIS_NANDA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DiagnosisNandaModel = new DiagnosisNandaModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDiagnosisNandaModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DiagnosisNandaModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.nursingAnesthesiaDetailDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(OperatingTheaterMedicalExaminationRecordModel.getProps(), this.nursingAnesthesiaDetailRelations, this.isViewOnly),
			...
			if (event.entityName === this.nursingSurgicalDetailDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(OperatingTheaterMedicalExaminationRecordModel.getProps(), this.nursingSurgicalDetailRelations, this.isViewOnly),
					new OperatingTheaterMedicalExaminationRecordModel(),
					OperatingTheaterMedicalExaminationRecordModel,
			...
			if (event.entityName === this.nursingDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.nursingRelations, this.isViewOnly),
					new DiagnosesAndProceduresModel(),
					DiagnosesAndProceduresModel,
					this.nursingDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.nursingRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DiagnosisNandaModel.getProps();
		this.modelRelations = DiagnosisNandaModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'nursing';
		}).hideElement = this.nursingMultiCrudActive || this.modelRelations.find(rel => rel.id === 'nursing').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.nursingAnesthesiaDetailMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.nursingAnesthesiaDetailDisplayName, many: true});
			this.store.dispatch(new operatingTheaterMedicalExaminationRecordModelAction.OperatingTheaterMedicalExaminationRecordAction(
				...
				{
					collectionId: this.nursingAnesthesiaDetailCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getOperatingTheaterMedicalExaminationRecordCollectionModels, this.nursingAnesthesiaDetailCollectionId).subscribe(models => {
				models.forEach(nursingAnesthesiaDetail => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(OperatingTheaterMedicalExaminationRecordModel.getProps(), this.nursingAnesthesiaDetailRelations, this.isViewOnly),
		...
		if (this.nursingSurgicalDetailMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.nursingSurgicalDetailDisplayName, many: true});
			this.store.dispatch(new operatingTheaterMedicalExaminationRecordModelAction.OperatingTheaterMedicalExaminationRecordAction(
				operatingTheaterMedicalExaminationRecordModelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.INITIALISE_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD_COLLECTION_STATE,
				{
					collectionId: this.nursingSurgicalDetailCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getOperatingTheaterMedicalExaminationRecordCollectionModels, this.nursingSurgicalDetailCollectionId).subscribe(models => {
				models.forEach(nursingSurgicalDetail => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(OperatingTheaterMedicalExaminationRecordModel.getProps(), this.nursingSurgicalDetailRelations, this.isViewOnly),
		...
		if (this.nursingMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.nursingDisplayName, many: true});
			this.store.dispatch(new diagnosesAndProceduresModelAction.DiagnosesAndProceduresAction(
				diagnosesAndProceduresModelAction.DiagnosesAndProceduresModelActionTypes.INITIALISE_DIAGNOSES_AND_PROCEDURES_COLLECTION_STATE,
				{
					collectionId: this.nursingCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getDiagnosesAndProceduresCollectionModels, this.nursingCollectionId).subscribe(models => {
				models.forEach(nursing => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.nursingRelations, this.isViewOnly),
						nursing,
						DiagnosesAndProceduresModel,
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.nursingAnesthesiaDetailIds && this.targetModel.nursingAnesthesiaDetailIds.length > 0 && this.nursingAnesthesiaDetailMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				OperatingTheaterMedicalExaminationRecordModel,
				this.nursingAnesthesiaDetailOppositeRelation,
				this.nursingAnesthesiaDetailCollectionId,
				this.targetModel.nursingAnesthesiaDetailIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.nursingSurgicalDetailIds && this.targetModel.nursingSurgicalDetailIds.length > 0 && this.nursingSurgicalDetailMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				OperatingTheaterMedicalExaminationRecordModel,
				this.nursingSurgicalDetailOppositeRelation,
				this.nursingSurgicalDetailCollectionId,
				this.targetModel.nursingSurgicalDetailIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.nursingIds && this.targetModel.nursingIds.length > 0 && this.nursingMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				DiagnosesAndProceduresModel,
				this.nursingOppositeRelation,
				this.nursingCollectionId,
				this.targetModel.nursingIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: DiagnosisNandaModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DiagnosisNandaModel {
		let updatedModel: DiagnosisNandaModel = new DiagnosisNandaModel(model);
		...
		updatedModel.nursingAnesthesiaDetail = [];
		if (this.nursingAnesthesiaDetailMultiCrudActive) {
			if (event.additionalEntities.nursingAnesthesiaDetail) {
				event.additionalEntities.nursingAnesthesiaDetail.forEach(entity => {
					updatedModel.nursingAnesthesiaDetail.push(entity);
				});
			}

			if (!updatedModel.nursingAnesthesiaDetail.length) {
				updatedModel.nursingAnesthesiaDetailIds = [];
			}
		}
		updatedModel.nursingSurgicalDetail = [];
		if (this.nursingSurgicalDetailMultiCrudActive) {
			if (event.additionalEntities.nursingSurgicalDetail) {
				event.additionalEntities.nursingSurgicalDetail.forEach(entity => {
					updatedModel.nursingSurgicalDetail.push(entity);
				});
			}

			if (!updatedModel.nursingSurgicalDetail.length) {
				updatedModel.nursingSurgicalDetailIds = [];
			}
		}
		updatedModel.nursing = [];
		if (this.nursingMultiCrudActive) {
			if (event.additionalEntities.nursing) {
				event.additionalEntities.nursing.forEach(entity => {
					updatedModel.nursing.push(entity);
				});
			}

			if (!updatedModel.nursing.length) {
				updatedModel.nursingIds = [];
			}
		}
