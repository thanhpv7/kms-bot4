...
import {AdminDiagnosisNandaCrudListComponent} from './diagnosis-nanda-admin-crud-list.component';
...
import {AdminDiagnosisNandaCrudModule} from '../diagnosis-nanda-admin-crud.module';
import {
	DiagnosisNandaModelState,
	initialState as initialDiagnosisNandaModelState
} from 'src/app/models/diagnosisNanda/diagnosis_nanda.model.state';
 ...
 */
describe('Diagnosis Nanda Crud List Tile Component', () => {

	let fixture;
	let diagnosisNandaCrudListComponent: AdminDiagnosisNandaCrudListComponent;

	let store: MockStore<{ model: DiagnosisNandaModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let diagnosisNandaModelState: DiagnosisNandaModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<DiagnosisNandaModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '2292d80c-da9e-48c7-8031-5f0516107b85';
	const searchContent = 'text';

	const searchableFieldCount = DiagnosisNandaModel.searchFields.length;

	const routerState: RouterState = {
		url: 'diagnosis-nanda-admin-crud',
		urls: ['diagnosis-nanda-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const diagnosisNandaDataFactory = new DiagnosisNandaDataFactory();

	const defaultDataSize = 30;
	const defaultData = diagnosisNandaDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: DiagnosisNandaModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		diagnosisNandaModelState = initialDiagnosisNandaModelState;
		store.setState({model: diagnosisNandaModelState});
		...
		spyOn(store, 'select')
			.withArgs(getDiagnosisNandaCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDiagnosisNandaCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminDiagnosisNandaCrudListComponent);
			diagnosisNandaCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(diagnosisNandaCrudListComponent.diagnosisNandasCount).toBe(defaultData.length);
			...
			expect(diagnosisNandaCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			diagnosisNandaCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = diagnosisNandaDataFactory.createAll(filteredDataSize);
			...
			expect(diagnosisNandaCrudListComponent.diagnosisNandasCount).toBe(filteredDataSize);
			expect(diagnosisNandaCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(diagnosisNandaCrudListComponent.pageIndex).toBe(0);
			expect(diagnosisNandaCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			diagnosisNandaCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		diagnosisNandaCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		diagnosisNandaCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
