...
import {DiagnosisNandaModelState} from 'src/app/models/diagnosisNanda/diagnosis_nanda.model.state';
...
import {
	getDiagnosisNandaCollectionModels,
	getDiagnosisNandaCollectionCount
...
@Component({
	selector: 'cb-diagnosis-nanda-admin-crud-list',
	templateUrl: './diagnosis-nanda-admin-crud-list.component.html',
	styleUrls: [
		'./diagnosis-nanda-admin-crud-list.component.scss',
...
export class AdminDiagnosisNandaCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DiagnosisNandaModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DiagnosisNandaModel.searchFields,
	...
	/**
	 * List of Diagnosis Nandas that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	diagnosisNandas: Observable<DiagnosisNandaModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	diagnosisNandasCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.diagnosisNandasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.diagnosisNandasCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosisNandaAction(
				modelAction.DiagnosisNandaModelActionTypes.INITIALISE_DIAGNOSIS_NANDA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosisNandaAction(
				modelAction.DiagnosisNandaModelActionTypes.FETCH_DIAGNOSIS_NANDA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.diagnosisNandas = this.store.select(getDiagnosisNandaCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDiagnosisNandaCollectionCount, this.collectionId).subscribe(
				(count) => this.diagnosisNandasCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DiagnosisNandaModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DiagnosisNandaModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DiagnosisNandaModelActionTypes.EXPORT_ALL_DIAGNOSIS_NANDA;
		} else {
			actionType = modelAction.DiagnosisNandaModelActionTypes.EXPORT_DIAGNOSIS_NANDA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DiagnosisNandaAction(
				modelAction.DiagnosisNandaModelActionTypes.IMPORT_DIAGNOSIS_NANDAS,
				{
					file: file,
				},
				[new modelAction.DiagnosisNandaAction(
			...
			window.open(environment.API_URL + '/api/diagnosisnanda/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DiagnosisNandaModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Diagnosis Nanda entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Diagnosis Nanda entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DiagnosisNandaModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DiagnosisNandaModelActionTypes.DELETE_DIAGNOSIS_NANDA;
						} else if (usingSelected) {
							actionType = modelAction.DiagnosisNandaModelActionTypes.DELETE_ALL_DIAGNOSIS_NANDA;
						} else {
							actionType = modelAction.DiagnosisNandaModelActionTypes.DELETE_DIAGNOSIS_NANDA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DiagnosisNandaAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DIAGNOSIS_NANDA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DIAGNOSIS_NANDA and DELETE_DIAGNOSIS_NANDA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DiagnosisNandaModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.diagnosisNandasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
