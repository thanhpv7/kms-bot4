...
import {AdminNhisClaimGDRGDetailCrudListComponent} from './nhis-claim-gdrg-detail-admin-crud-list.component';
import {NhisClaimGDRGDetailDataFactory} from 'src/app/lib/utils/factories/nhis-claim-gdrg-detail-data-factory';
import {NhisClaimGDRGDetailModel} from 'src/app/models/nhisClaimGDRGDetail/nhis_claim_gdrg_detail.model';
import {AdminNhisClaimGDRGDetailCrudModule} from '../nhis-claim-gdrg-detail-admin-crud.module';
import {
	NhisClaimGDRGDetailModelState,
	initialState as initialNhisClaimGDRGDetailModelState
} from 'src/app/models/nhisClaimGDRGDetail/nhis_claim_gdrg_detail.model.state';
 ...
 */
describe('NHIS Claim GDRG Detail Crud List Tile Component', () => {

	let fixture;
	let nhisClaimGDRGDetailCrudListComponent: AdminNhisClaimGDRGDetailCrudListComponent;

	let store: MockStore<{ model: NhisClaimGDRGDetailModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisClaimGDRGDetailModelState: NhisClaimGDRGDetailModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisClaimGDRGDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b8229994-01dc-48e7-81ec-23e0b14539e2';
	const searchContent = 'text';

	const searchableFieldCount = NhisClaimGDRGDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-claim-gdrg-detail-admin-crud',
		urls: ['nhis-claim-gdrg-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisClaimGDRGDetailDataFactory = new NhisClaimGDRGDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisClaimGDRGDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisClaimGDRGDetailModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisClaimGDRGDetailModelState = initialNhisClaimGDRGDetailModelState;
		store.setState({model: nhisClaimGDRGDetailModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisClaimGDRGDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisClaimGDRGDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminNhisClaimGDRGDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminNhisClaimGDRGDetailCrudListComponent);
			nhisClaimGDRGDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisClaimGDRGDetailCrudListComponent.nhisClaimGDRGDetailsCount).toBe(defaultData.length);
			...
			expect(nhisClaimGDRGDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisClaimGDRGDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisClaimGDRGDetailDataFactory.createAll(filteredDataSize);
			...
			expect(nhisClaimGDRGDetailCrudListComponent.nhisClaimGDRGDetailsCount).toBe(filteredDataSize);
			expect(nhisClaimGDRGDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisClaimGDRGDetailCrudListComponent.pageIndex).toBe(0);
			expect(nhisClaimGDRGDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisClaimGDRGDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisClaimGDRGDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisClaimGDRGDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
