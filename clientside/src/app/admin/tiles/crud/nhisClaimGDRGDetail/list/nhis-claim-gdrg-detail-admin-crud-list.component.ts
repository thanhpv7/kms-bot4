...
import * as modelAction from 'src/app/models/nhisClaimGDRGDetail/nhis_claim_gdrg_detail.model.action';
import {NhisClaimGDRGDetailModelState} from 'src/app/models/nhisClaimGDRGDetail/nhis_claim_gdrg_detail.model.state';
...
import {
	getNhisClaimGDRGDetailCollectionModels,
	getNhisClaimGDRGDetailCollectionCount
} from 'src/app/models/nhisClaimGDRGDetail/nhis_claim_gdrg_detail.model.selector';
...
import {CrudTileMode} from '../nhis-claim-gdrg-detail-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-claim-gdrg-detail-admin-crud-list',
	templateUrl: './nhis-claim-gdrg-detail-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-claim-gdrg-detail-admin-crud-list.component.scss',
...
export class AdminNhisClaimGDRGDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisClaimGDRGDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisClaimGDRGDetailModel.searchFields,
	...
	/**
	 * List of NHIS Claim GDRG Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	nhisClaimGDRGDetails: Observable<NhisClaimGDRGDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	nhisClaimGDRGDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = NhisClaimGDRGDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NhisClaimGDRGDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'b8229994-01dc-48e7-81ec-23e0b14539e2';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisClaimGDRGDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisClaimGDRGDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisClaimGDRGDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
				modelAction.NhisClaimGDRGDetailModelActionTypes.INITIALISE_NHIS_CLAIM_GDRG_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
				modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_NHIS_CLAIM_GDRG_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisClaimGDRGDetailAction(
						modelAction.NhisClaimGDRGDetailModelActionTypes.COUNT_NHIS_CLAIM_GDRG_DETAILS,
						null
					)
				]
			))
		...
		this.nhisClaimGDRGDetails = this.store.select(getNhisClaimGDRGDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisClaimGDRGDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisClaimGDRGDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('nhis-claim-gdrg-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisClaimGDRGDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisClaimGDRGDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisClaimGDRGDetailModelActionTypes.EXPORT_ALL_NHIS_CLAIM_GDRG_DETAIL;
		} else {
			actionType = modelAction.NhisClaimGDRGDetailModelActionTypes.EXPORT_NHIS_CLAIM_GDRG_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
				modelAction.NhisClaimGDRGDetailModelActionTypes.IMPORT_NHIS_CLAIM_GDRG_DETAILS,
				{
					file: file,
				},
				[new modelAction.NhisClaimGDRGDetailAction(
			...
			window.open(environment.API_URL + '/api/nhisclaimgdrgdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisClaimGDRGDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Claim GDRG Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Claim GDRG Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisClaimGDRGDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisClaimGDRGDetailModelActionTypes.DELETE_NHIS_CLAIM_GDRG_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.NhisClaimGDRGDetailModelActionTypes.DELETE_ALL_NHIS_CLAIM_GDRG_DETAIL;
						} else {
							actionType = modelAction.NhisClaimGDRGDetailModelActionTypes.DELETE_NHIS_CLAIM_GDRG_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_CLAIM_GDRG_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_CLAIM_GDRG_DETAIL and DELETE_NHIS_CLAIM_GDRG_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisClaimGDRGDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisClaimGDRGDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisClaimGDRGDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
