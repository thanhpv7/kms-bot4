...
import {NhisClaimGDRGDetailModelState, NhisClaimGDRGDetailModelAudit} from 'src/app/models/nhisClaimGDRGDetail/nhis_claim_gdrg_detail.model.state';
import {NhisClaimGDRGDetailModel} from 'src/app/models/nhisClaimGDRGDetail/nhis_claim_gdrg_detail.model';
import {
	getNhisClaimGDRGDetailModelAuditsByEntityId,
	getNhisClaimGDRGDetailModelWithId,
	getCountNhisClaimGDRGDetailModels,
...
@Component({
	selector: 'cb-nhis-claim-gdrg-detail-admin-crud-edit',
	templateUrl: './nhis-claim-gdrg-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-claim-gdrg-detail-admin-crud-edit.component.scss',
...
export class AdminNhisClaimGDRGDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisClaimGDRGDetailAudits$: Observable<NhisClaimGDRGDetailModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisClaimGDRGDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NhisClaimGDRGDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisClaimGDRGDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisClaimGDRGDetailModel();
		...
		if (this.targetModelId) {
			this.nhisClaimGDRGDetailAudits$ = this.store.select(getNhisClaimGDRGDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisClaimGDRGDetailModel> = {
		...
		this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
			modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_NHIS_CLAIM_GDRG_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisClaimGDRGDetailModelWithId, this.targetModelId)
			...
			model = new NhisClaimGDRGDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
			modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_NHIS_CLAIM_GDRG_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
				modelAction.NhisClaimGDRGDetailModelActionTypes.CREATE_NHIS_CLAIM_GDRG_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisClaimGDRGDetailAction(
				modelAction.NhisClaimGDRGDetailModelActionTypes.UPDATE_NHIS_CLAIM_GDRG_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisClaimGDRGDetailModel = new NhisClaimGDRGDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisClaimGDRGDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisClaimGDRGDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisClaimGDRGDetailModel.getProps();
		this.modelRelations = NhisClaimGDRGDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisClaimGDRGDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisClaimGDRGDetailModel {
		let updatedModel: NhisClaimGDRGDetailModel = new NhisClaimGDRGDetailModel(model);
