...
import * as modelAction from 'src/app/models/bpjsINACBG/bpjs_inacbg.model.action';
import {BpjsINACBGModelState} from 'src/app/models/bpjsINACBG/bpjs_inacbg.model.state';
...
import {CrudTileMode} from '../bpjs-inacbg-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-inacbg-admin-crud-list',
	templateUrl: './bpjs-inacbg-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-inacbg-admin-crud-list.component.scss',
...
export class AdminBpjsINACBGCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsINACBGModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsINACBGModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsINACBGModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '0141e31a-878f-4163-8805-4283e3b65dd0';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsINACBGsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsINACBGsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsINACBGModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsINACBGAction(
				modelAction.BpjsINACBGModelActionTypes.INITIALISE_BPJS_INACBG_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsINACBGAction(
				modelAction.BpjsINACBGModelActionTypes.FETCH_BPJS_INACBG_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsINACBGAction(
						modelAction.BpjsINACBGModelActionTypes.COUNT_BPJS_INACBGS,
						null
					)
				]
			))
		...
		this.bpjsINACBGs = this.store.select(getBpjsINACBGCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsINACBGCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsINACBGsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsINACBGModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsINACBGModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsINACBGModelActionTypes.EXPORT_ALL_BPJS_INACBG;
		} else {
			actionType = modelAction.BpjsINACBGModelActionTypes.EXPORT_BPJS_INACBG_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsINACBGAction(
				modelAction.BpjsINACBGModelActionTypes.IMPORT_BPJS_INACBGS,
				{
					file: file,
				},
				[new modelAction.BpjsINACBGAction(
			...
			window.open(environment.API_URL + '/api/bpjsinacbg/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsINACBGModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS INACBG entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS INACBG entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsINACBGModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsINACBGModelActionTypes.DELETE_BPJS_INACBG;
						} else if (usingSelected) {
							actionType = modelAction.BpjsINACBGModelActionTypes.DELETE_ALL_BPJS_INACBG;
						} else {
							actionType = modelAction.BpjsINACBGModelActionTypes.DELETE_BPJS_INACBG_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsINACBGAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_INACBG when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_INACBG and DELETE_BPJS_INACBG_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsINACBGModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsINACBGModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsINACBGsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
