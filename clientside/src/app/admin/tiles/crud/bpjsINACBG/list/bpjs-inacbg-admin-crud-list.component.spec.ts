...
import {AdminBpjsINACBGCrudListComponent} from './bpjs-inacbg-admin-crud-list.component';
...
import {AdminBpjsINACBGCrudModule} from '../bpjs-inacbg-admin-crud.module';
import {
 ...
 */
describe('BPJS INACBG Crud List Tile Component', () => {

	let fixture;
	let bpjsINACBGCrudListComponent: AdminBpjsINACBGCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsINACBGModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '0141e31a-878f-4163-8805-4283e3b65dd0';
	const searchContent = 'text';

	const searchableFieldCount = BpjsINACBGModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-inacbg-admin-crud',
		urls: ['bpjs-inacbg-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsINACBGDataFactory = new BpjsINACBGDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsINACBGDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsINACBGCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsINACBGCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsINACBGCrudListComponent);
			bpjsINACBGCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsINACBGCrudListComponent.bpjsINACBGsCount).toBe(defaultData.length);
			...
			expect(bpjsINACBGCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsINACBGCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsINACBGDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsINACBGCrudListComponent.bpjsINACBGsCount).toBe(filteredDataSize);
			expect(bpjsINACBGCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsINACBGCrudListComponent.pageIndex).toBe(0);
			expect(bpjsINACBGCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsINACBGCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsINACBGCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsINACBGCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
