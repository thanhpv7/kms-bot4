...
import {BpjsINACBGModelState, BpjsINACBGModelAudit} from 'src/app/models/bpjsINACBG/bpjs_inacbg.model.state';
import {BpjsINACBGModel} from 'src/app/models/bpjsINACBG/bpjs_inacbg.model';
import {
	getBpjsINACBGModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-inacbg-admin-crud-edit',
	templateUrl: './bpjs-inacbg-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-inacbg-admin-crud-edit.component.scss',
...
export class AdminBpjsINACBGCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsINACBGAudits$: Observable<BpjsINACBGModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsINACBGModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsINACBGModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsINACBGModel();
		...
		if (this.targetModelId) {
			this.bpjsINACBGAudits$ = this.store.select(getBpjsINACBGModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsINACBGModel> = {
		...
		this.store.dispatch(new modelAction.BpjsINACBGAction(
			modelAction.BpjsINACBGModelActionTypes.FETCH_BPJS_INACBG,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsINACBGModelWithId, this.targetModelId)
			...
			model = new BpjsINACBGModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsINACBGAction(
			modelAction.BpjsINACBGModelActionTypes.FETCH_BPJS_INACBG_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsINACBGAction(
				modelAction.BpjsINACBGModelActionTypes.CREATE_BPJS_INACBG,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsINACBGAction(
				modelAction.BpjsINACBGModelActionTypes.UPDATE_BPJS_INACBG,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsINACBGModel = new BpjsINACBGModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsINACBGModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsINACBGModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsINACBGModel.getProps();
		this.modelRelations = BpjsINACBGModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsINACBGModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsINACBGModel {
		let updatedModel: BpjsINACBGModel = new BpjsINACBGModel(model);
