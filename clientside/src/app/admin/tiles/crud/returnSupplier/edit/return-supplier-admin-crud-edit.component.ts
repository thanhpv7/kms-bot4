...
import * as modelAction from 'src/app/models/returnSupplier/return_supplier.model.action';
import {ReturnSupplierModelState, ReturnSupplierModelAudit} from 'src/app/models/returnSupplier/return_supplier.model.state';
import {ReturnSupplierModel} from 'src/app/models/returnSupplier/return_supplier.model';
import {
	getReturnSupplierModelAuditsByEntityId,
	getReturnSupplierModelWithId,
	getCountReturnSupplierModels,
...
import {CrudTileMode} from '../return-supplier-admin-crud.component';
...
@Component({
	selector: 'cb-return-supplier-admin-crud-edit',
	templateUrl: './return-supplier-admin-crud-edit.component.html',
	styleUrls: [
		'./return-supplier-admin-crud-edit.component.scss',
...
export class AdminReturnSupplierCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '55cd29fe-e34d-4c60-8095-e9f92011b71a';
	 ...
	 */
	returnSupplierAudits$: Observable<ReturnSupplierModelAudit[]>;
	...
	purchaseOrderOppositeRelation = 'returnSuppliers';
	...
	supplierOppositeRelation = 'returnSuppliers';
	...
	warehouseOppositeRelation = 'returnSuppliers';
	...
	returnSupplierStockDetailsOppositeRelation = 'returnSupplier';
	 ...
	 */
	modelProperties: ModelProperty[] = ReturnSupplierModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ReturnSupplierModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ReturnSupplierModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ReturnSupplierModel;
	...
	constructor(
		private readonly store: Store<{ model: ReturnSupplierModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountReturnSupplierModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ReturnSupplierModel();
		...
		if (this.targetModelId) {
			this.returnSupplierAudits$ = this.store.select(getReturnSupplierModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ReturnSupplierModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.ReturnSupplierAction(
			modelAction.ReturnSupplierModelActionTypes.FETCH_RETURN_SUPPLIER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getReturnSupplierModelWithId, this.targetModelId)
			...
			model = new ReturnSupplierModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ReturnSupplierAction(
			modelAction.ReturnSupplierModelActionTypes.FETCH_RETURN_SUPPLIER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ReturnSupplierModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.purchaseOrderMultiCrudActive || this.supplierMultiCrudActive || this.warehouseMultiCrudActive || this.returnSupplierStockDetailsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.ReturnSupplierAction(
						modelAction.ReturnSupplierModelActionTypes.COUNT_RETURN_SUPPLIERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.ReturnSupplierAction(
				modelAction.ReturnSupplierModelActionTypes.CREATE_RETURN_SUPPLIER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ReturnSupplierAction(
				modelAction.ReturnSupplierModelActionTypes.UPDATE_RETURN_SUPPLIER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ReturnSupplierModel = new ReturnSupplierModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getReturnSupplierModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ReturnSupplierModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ReturnSupplierModel.getProps();
		this.modelRelations = ReturnSupplierModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ReturnSupplierModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ReturnSupplierModel {
		let updatedModel: ReturnSupplierModel = new ReturnSupplierModel(model);
