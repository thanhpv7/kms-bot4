...
import {ReturnSupplierModelState} from 'src/app/models/returnSupplier/return_supplier.model.state';
...
import {
	getReturnSupplierCollectionModels,
	getReturnSupplierCollectionCount
...
@Component({
	selector: 'cb-return-supplier-admin-crud-list',
	templateUrl: './return-supplier-admin-crud-list.component.html',
	styleUrls: [
		'./return-supplier-admin-crud-list.component.scss',
...
export class AdminReturnSupplierCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ReturnSupplierModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ReturnSupplierModel.searchFields,
	...
	/**
	 * List of Return Suppliers that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	returnSuppliers: Observable<ReturnSupplierModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	returnSuppliersCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.returnSuppliersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.returnSuppliersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReturnSupplierAction(
				modelAction.ReturnSupplierModelActionTypes.INITIALISE_RETURN_SUPPLIER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReturnSupplierAction(
				modelAction.ReturnSupplierModelActionTypes.FETCH_RETURN_SUPPLIER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.returnSuppliers = this.store.select(getReturnSupplierCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getReturnSupplierCollectionCount, this.collectionId).subscribe(
				(count) => this.returnSuppliersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ReturnSupplierModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ReturnSupplierModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ReturnSupplierModelActionTypes.EXPORT_ALL_RETURN_SUPPLIER;
		} else {
			actionType = modelAction.ReturnSupplierModelActionTypes.EXPORT_RETURN_SUPPLIER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ReturnSupplierAction(
				modelAction.ReturnSupplierModelActionTypes.IMPORT_RETURN_SUPPLIERS,
				{
					file: file,
				},
				[new modelAction.ReturnSupplierAction(
			...
			window.open(environment.API_URL + '/api/returnsupplier/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ReturnSupplierModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Return Supplier entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Return Supplier entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ReturnSupplierModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ReturnSupplierModelActionTypes.DELETE_RETURN_SUPPLIER;
						} else if (usingSelected) {
							actionType = modelAction.ReturnSupplierModelActionTypes.DELETE_ALL_RETURN_SUPPLIER;
						} else {
							actionType = modelAction.ReturnSupplierModelActionTypes.DELETE_RETURN_SUPPLIER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ReturnSupplierAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_RETURN_SUPPLIER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_RETURN_SUPPLIER and DELETE_RETURN_SUPPLIER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ReturnSupplierModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.returnSuppliersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
