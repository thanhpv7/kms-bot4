...
import {AdminBpjsSearchControlLetterCrudListComponent} from './bpjs-search-control-letter-admin-crud-list.component';
import {BpjsSearchControlLetterDataFactory} from 'src/app/lib/utils/factories/bpjs-search-control-letter-data-factory';
import {BpjsSearchControlLetterModel} from 'src/app/models/bpjsSearchControlLetter/bpjs_search_control_letter.model';
import {AdminBpjsSearchControlLetterCrudModule} from '../bpjs-search-control-letter-admin-crud.module';
import {
	BpjsSearchControlLetterModelState,
	initialState as initialBpjsSearchControlLetterModelState
} from 'src/app/models/bpjsSearchControlLetter/bpjs_search_control_letter.model.state';
 ...
 */
describe('BPJS Search Control Letter Crud List Tile Component', () => {

	let fixture;
	let bpjsSearchControlLetterCrudListComponent: AdminBpjsSearchControlLetterCrudListComponent;

	let store: MockStore<{ model: BpjsSearchControlLetterModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsSearchControlLetterModelState: BpjsSearchControlLetterModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsSearchControlLetterModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '07ddeb83-3355-4561-a474-b41774502ba6';
	const searchContent = 'text';

	const searchableFieldCount = BpjsSearchControlLetterModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-search-control-letter-admin-crud',
		urls: ['bpjs-search-control-letter-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsSearchControlLetterDataFactory = new BpjsSearchControlLetterDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsSearchControlLetterDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsSearchControlLetterModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsSearchControlLetterModelState = initialBpjsSearchControlLetterModelState;
		store.setState({model: bpjsSearchControlLetterModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsSearchControlLetterCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsSearchControlLetterCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsSearchControlLetterCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsSearchControlLetterCrudListComponent);
			bpjsSearchControlLetterCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsSearchControlLetterCrudListComponent.bpjsSearchControlLettersCount).toBe(defaultData.length);
			...
			expect(bpjsSearchControlLetterCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsSearchControlLetterCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsSearchControlLetterDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsSearchControlLetterCrudListComponent.bpjsSearchControlLettersCount).toBe(filteredDataSize);
			expect(bpjsSearchControlLetterCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsSearchControlLetterCrudListComponent.pageIndex).toBe(0);
			expect(bpjsSearchControlLetterCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsSearchControlLetterCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsSearchControlLetterCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsSearchControlLetterCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
