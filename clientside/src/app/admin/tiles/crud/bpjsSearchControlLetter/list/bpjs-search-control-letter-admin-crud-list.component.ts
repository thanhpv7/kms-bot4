...
import * as modelAction from 'src/app/models/bpjsSearchControlLetter/bpjs_search_control_letter.model.action';
import {BpjsSearchControlLetterModelState} from 'src/app/models/bpjsSearchControlLetter/bpjs_search_control_letter.model.state';
...
import {
	getBpjsSearchControlLetterCollectionModels,
	getBpjsSearchControlLetterCollectionCount
} from 'src/app/models/bpjsSearchControlLetter/bpjs_search_control_letter.model.selector';
...
import {CrudTileMode} from '../bpjs-search-control-letter-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-search-control-letter-admin-crud-list',
	templateUrl: './bpjs-search-control-letter-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-search-control-letter-admin-crud-list.component.scss',
...
export class AdminBpjsSearchControlLetterCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsSearchControlLetterModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsSearchControlLetterModel.searchFields,
	...
	/**
	 * List of BPJS Search Control Letters that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsSearchControlLetters: Observable<BpjsSearchControlLetterModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsSearchControlLettersCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsSearchControlLetterModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsSearchControlLetterModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '07ddeb83-3355-4561-a474-b41774502ba6';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsSearchControlLettersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsSearchControlLettersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsSearchControlLetterModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
				modelAction.BpjsSearchControlLetterModelActionTypes.INITIALISE_BPJS_SEARCH_CONTROL_LETTER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
				modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_BPJS_SEARCH_CONTROL_LETTER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsSearchControlLetterAction(
						modelAction.BpjsSearchControlLetterModelActionTypes.COUNT_BPJS_SEARCH_CONTROL_LETTERS,
						null
					)
				]
			))
		...
		this.bpjsSearchControlLetters = this.store.select(getBpjsSearchControlLetterCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsSearchControlLetterCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsSearchControlLettersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-search-control-letter');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsSearchControlLetterModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsSearchControlLetterModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsSearchControlLetterModelActionTypes.EXPORT_ALL_BPJS_SEARCH_CONTROL_LETTER;
		} else {
			actionType = modelAction.BpjsSearchControlLetterModelActionTypes.EXPORT_BPJS_SEARCH_CONTROL_LETTER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
				modelAction.BpjsSearchControlLetterModelActionTypes.IMPORT_BPJS_SEARCH_CONTROL_LETTERS,
				{
					file: file,
				},
				[new modelAction.BpjsSearchControlLetterAction(
			...
			window.open(environment.API_URL + '/api/bpjssearchcontrolletter/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsSearchControlLetterModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Search Control Letter entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Search Control Letter entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsSearchControlLetterModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsSearchControlLetterModelActionTypes.DELETE_BPJS_SEARCH_CONTROL_LETTER;
						} else if (usingSelected) {
							actionType = modelAction.BpjsSearchControlLetterModelActionTypes.DELETE_ALL_BPJS_SEARCH_CONTROL_LETTER;
						} else {
							actionType = modelAction.BpjsSearchControlLetterModelActionTypes.DELETE_BPJS_SEARCH_CONTROL_LETTER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SEARCH_CONTROL_LETTER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SEARCH_CONTROL_LETTER and DELETE_BPJS_SEARCH_CONTROL_LETTER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsSearchControlLetterModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsSearchControlLetterModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsSearchControlLettersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
