...
import {BpjsSearchControlLetterModelState, BpjsSearchControlLetterModelAudit} from 'src/app/models/bpjsSearchControlLetter/bpjs_search_control_letter.model.state';
import {BpjsSearchControlLetterModel} from 'src/app/models/bpjsSearchControlLetter/bpjs_search_control_letter.model';
import {
	getBpjsSearchControlLetterModelAuditsByEntityId,
	getBpjsSearchControlLetterModelWithId,
	getCountBpjsSearchControlLetterModels,
...
@Component({
	selector: 'cb-bpjs-search-control-letter-admin-crud-edit',
	templateUrl: './bpjs-search-control-letter-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-search-control-letter-admin-crud-edit.component.scss',
...
export class AdminBpjsSearchControlLetterCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsSearchControlLetterAudits$: Observable<BpjsSearchControlLetterModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsSearchControlLetterModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsSearchControlLetterModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsSearchControlLetterModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsSearchControlLetterModel();
		...
		if (this.targetModelId) {
			this.bpjsSearchControlLetterAudits$ = this.store.select(getBpjsSearchControlLetterModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsSearchControlLetterModel> = {
		...
		this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
			modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_BPJS_SEARCH_CONTROL_LETTER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsSearchControlLetterModelWithId, this.targetModelId)
			...
			model = new BpjsSearchControlLetterModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
			modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_BPJS_SEARCH_CONTROL_LETTER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
				modelAction.BpjsSearchControlLetterModelActionTypes.CREATE_BPJS_SEARCH_CONTROL_LETTER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsSearchControlLetterAction(
				modelAction.BpjsSearchControlLetterModelActionTypes.UPDATE_BPJS_SEARCH_CONTROL_LETTER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsSearchControlLetterModel = new BpjsSearchControlLetterModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsSearchControlLetterModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsSearchControlLetterModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsSearchControlLetterModel.getProps();
		this.modelRelations = BpjsSearchControlLetterModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsSearchControlLetterModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsSearchControlLetterModel {
		let updatedModel: BpjsSearchControlLetterModel = new BpjsSearchControlLetterModel(model);
