...
import {AdminPcareActionExaminationCrudListComponent} from './pcare-action-examination-admin-crud-list.component';
...
import {AdminPcareActionExaminationCrudModule} from '../pcare-action-examination-admin-crud.module';
import {
	PcareActionExaminationModelState,
	initialState as initialPcareActionExaminationModelState
} from 'src/app/models/pcareActionExamination/pcare_action_examination.model.state';
 ...
 */
describe('PCare Action Examination Crud List Tile Component', () => {

	let fixture;
	let pcareActionExaminationCrudListComponent: AdminPcareActionExaminationCrudListComponent;

	let store: MockStore<{ model: PcareActionExaminationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareActionExaminationModelState: PcareActionExaminationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareActionExaminationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b3e249f9-4899-47d0-b79a-bf3d9d759a61';
	const searchContent = 'text';

	const searchableFieldCount = PcareActionExaminationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-action-examination-admin-crud',
		urls: ['pcare-action-examination-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareActionExaminationDataFactory = new PcareActionExaminationDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareActionExaminationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareActionExaminationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareActionExaminationModelState = initialPcareActionExaminationModelState;
		store.setState({model: pcareActionExaminationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareActionExaminationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareActionExaminationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPcareActionExaminationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPcareActionExaminationCrudListComponent);
			pcareActionExaminationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareActionExaminationCrudListComponent.pcareActionExaminationsCount).toBe(defaultData.length);
			...
			expect(pcareActionExaminationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareActionExaminationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareActionExaminationDataFactory.createAll(filteredDataSize);
			...
			expect(pcareActionExaminationCrudListComponent.pcareActionExaminationsCount).toBe(filteredDataSize);
			expect(pcareActionExaminationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareActionExaminationCrudListComponent.pageIndex).toBe(0);
			expect(pcareActionExaminationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareActionExaminationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareActionExaminationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareActionExaminationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
