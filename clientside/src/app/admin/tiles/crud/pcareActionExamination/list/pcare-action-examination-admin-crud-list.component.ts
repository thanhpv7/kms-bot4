...
import * as modelAction from 'src/app/models/pcareActionExamination/pcare_action_examination.model.action';
import {PcareActionExaminationModelState} from 'src/app/models/pcareActionExamination/pcare_action_examination.model.state';
...
import {
	getPcareActionExaminationCollectionModels,
	getPcareActionExaminationCollectionCount
...
import {CrudTileMode} from '../pcare-action-examination-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-action-examination-admin-crud-list',
	templateUrl: './pcare-action-examination-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-action-examination-admin-crud-list.component.scss',
...
export class AdminPcareActionExaminationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareActionExaminationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareActionExaminationModel.searchFields,
	...
	/**
	 * List of PCare Action Examinations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareActionExaminations: Observable<PcareActionExaminationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareActionExaminationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PcareActionExaminationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareActionExaminationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'b3e249f9-4899-47d0-b79a-bf3d9d759a61';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareActionExaminationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareActionExaminationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareActionExaminationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareActionExaminationAction(
				modelAction.PcareActionExaminationModelActionTypes.INITIALISE_PCARE_ACTION_EXAMINATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareActionExaminationAction(
				modelAction.PcareActionExaminationModelActionTypes.FETCH_PCARE_ACTION_EXAMINATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareActionExaminationAction(
						modelAction.PcareActionExaminationModelActionTypes.COUNT_PCARE_ACTION_EXAMINATIONS,
						null
					)
				]
			))
		...
		this.pcareActionExaminations = this.store.select(getPcareActionExaminationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareActionExaminationCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareActionExaminationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-action-examination');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareActionExaminationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareActionExaminationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareActionExaminationModelActionTypes.EXPORT_ALL_PCARE_ACTION_EXAMINATION;
		} else {
			actionType = modelAction.PcareActionExaminationModelActionTypes.EXPORT_PCARE_ACTION_EXAMINATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareActionExaminationAction(
				modelAction.PcareActionExaminationModelActionTypes.IMPORT_PCARE_ACTION_EXAMINATIONS,
				{
					file: file,
				},
				[new modelAction.PcareActionExaminationAction(
			...
			window.open(environment.API_URL + '/api/pcareactionexamination/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareActionExaminationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Action Examination entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Action Examination entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareActionExaminationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareActionExaminationModelActionTypes.DELETE_PCARE_ACTION_EXAMINATION;
						} else if (usingSelected) {
							actionType = modelAction.PcareActionExaminationModelActionTypes.DELETE_ALL_PCARE_ACTION_EXAMINATION;
						} else {
							actionType = modelAction.PcareActionExaminationModelActionTypes.DELETE_PCARE_ACTION_EXAMINATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareActionExaminationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_ACTION_EXAMINATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_ACTION_EXAMINATION and DELETE_PCARE_ACTION_EXAMINATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareActionExaminationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareActionExaminationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareActionExaminationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
