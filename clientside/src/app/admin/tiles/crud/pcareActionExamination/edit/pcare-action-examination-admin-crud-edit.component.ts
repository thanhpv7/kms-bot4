...
import {PcareActionExaminationModelState, PcareActionExaminationModelAudit} from 'src/app/models/pcareActionExamination/pcare_action_examination.model.state';
import {PcareActionExaminationModel} from 'src/app/models/pcareActionExamination/pcare_action_examination.model';
import {
	getPcareActionExaminationModelAuditsByEntityId,
	getPcareActionExaminationModelWithId,
	getCountPcareActionExaminationModels,
...
@Component({
	selector: 'cb-pcare-action-examination-admin-crud-edit',
	templateUrl: './pcare-action-examination-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-action-examination-admin-crud-edit.component.scss',
...
export class AdminPcareActionExaminationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareActionExaminationAudits$: Observable<PcareActionExaminationModelAudit[]>;
	...
	pcareExaminationOppositeRelation = 'pcareActionExaminations';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareActionExaminationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PcareActionExaminationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareActionExaminationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareActionExaminationModel();
		...
		if (this.targetModelId) {
			this.pcareActionExaminationAudits$ = this.store.select(getPcareActionExaminationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareActionExaminationModel> = {
		...
		this.store.dispatch(new modelAction.PcareActionExaminationAction(
			modelAction.PcareActionExaminationModelActionTypes.FETCH_PCARE_ACTION_EXAMINATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareActionExaminationModelWithId, this.targetModelId)
			...
			model = new PcareActionExaminationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareActionExaminationAction(
			modelAction.PcareActionExaminationModelActionTypes.FETCH_PCARE_ACTION_EXAMINATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PcareActionExaminationModel> = {
				...
				queryParams: {
					persistRelations: this.pcareExaminationMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.PcareActionExaminationAction(
				modelAction.PcareActionExaminationModelActionTypes.CREATE_PCARE_ACTION_EXAMINATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareActionExaminationAction(
				modelAction.PcareActionExaminationModelActionTypes.UPDATE_PCARE_ACTION_EXAMINATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareActionExaminationModel = new PcareActionExaminationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareActionExaminationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareActionExaminationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareActionExaminationModel.getProps();
		this.modelRelations = PcareActionExaminationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareActionExaminationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareActionExaminationModel {
		let updatedModel: PcareActionExaminationModel = new PcareActionExaminationModel(model);
