...
import {InventoryAdjusmentBatchModelState} from 'src/app/models/inventoryAdjusmentBatch/inventory_adjusment_batch.model.state';
...
import {
	getInventoryAdjusmentBatchCollectionModels,
	getInventoryAdjusmentBatchCollectionCount
...
@Component({
	selector: 'cb-inventory-adjusment-batch-admin-crud-list',
	templateUrl: './inventory-adjusment-batch-admin-crud-list.component.html',
	styleUrls: [
		'./inventory-adjusment-batch-admin-crud-list.component.scss',
...
export class AdminInventoryAdjusmentBatchCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InventoryAdjusmentBatchModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InventoryAdjusmentBatchModel.searchFields,
	...
	/**
	 * List of Inventory Adjusment Batchs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	inventoryAdjusmentBatchs: Observable<InventoryAdjusmentBatchModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	inventoryAdjusmentBatchsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.inventoryAdjusmentBatchsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.inventoryAdjusmentBatchsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
				modelAction.InventoryAdjusmentBatchModelActionTypes.INITIALISE_INVENTORY_ADJUSMENT_BATCH_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
				modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_INVENTORY_ADJUSMENT_BATCH_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.inventoryAdjusmentBatchs = this.store.select(getInventoryAdjusmentBatchCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInventoryAdjusmentBatchCollectionCount, this.collectionId).subscribe(
				(count) => this.inventoryAdjusmentBatchsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InventoryAdjusmentBatchModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InventoryAdjusmentBatchModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InventoryAdjusmentBatchModelActionTypes.EXPORT_ALL_INVENTORY_ADJUSMENT_BATCH;
		} else {
			actionType = modelAction.InventoryAdjusmentBatchModelActionTypes.EXPORT_INVENTORY_ADJUSMENT_BATCH_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
				modelAction.InventoryAdjusmentBatchModelActionTypes.IMPORT_INVENTORY_ADJUSMENT_BATCHS,
				{
					file: file,
				},
				[new modelAction.InventoryAdjusmentBatchAction(
			...
			window.open(environment.API_URL + '/api/inventoryadjusmentbatch/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InventoryAdjusmentBatchModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Inventory Adjusment Batch entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Inventory Adjusment Batch entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InventoryAdjusmentBatchModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InventoryAdjusmentBatchModelActionTypes.DELETE_INVENTORY_ADJUSMENT_BATCH;
						} else if (usingSelected) {
							actionType = modelAction.InventoryAdjusmentBatchModelActionTypes.DELETE_ALL_INVENTORY_ADJUSMENT_BATCH;
						} else {
							actionType = modelAction.InventoryAdjusmentBatchModelActionTypes.DELETE_INVENTORY_ADJUSMENT_BATCH_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVENTORY_ADJUSMENT_BATCH when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVENTORY_ADJUSMENT_BATCH and DELETE_INVENTORY_ADJUSMENT_BATCH_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InventoryAdjusmentBatchModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.inventoryAdjusmentBatchsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
