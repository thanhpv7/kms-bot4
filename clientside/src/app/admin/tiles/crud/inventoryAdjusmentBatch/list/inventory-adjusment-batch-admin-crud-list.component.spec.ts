...
import {AdminInventoryAdjusmentBatchCrudListComponent} from './inventory-adjusment-batch-admin-crud-list.component';
...
import {AdminInventoryAdjusmentBatchCrudModule} from '../inventory-adjusment-batch-admin-crud.module';
import {
	InventoryAdjusmentBatchModelState,
	initialState as initialInventoryAdjusmentBatchModelState
} from 'src/app/models/inventoryAdjusmentBatch/inventory_adjusment_batch.model.state';
 ...
 */
describe('Inventory Adjusment Batch Crud List Tile Component', () => {

	let fixture;
	let inventoryAdjusmentBatchCrudListComponent: AdminInventoryAdjusmentBatchCrudListComponent;

	let store: MockStore<{ model: InventoryAdjusmentBatchModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let inventoryAdjusmentBatchModelState: InventoryAdjusmentBatchModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InventoryAdjusmentBatchModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6df47b28-6325-453d-93e0-97446754087e';
	const searchContent = 'text';

	const searchableFieldCount = InventoryAdjusmentBatchModel.searchFields.length;

	const routerState: RouterState = {
		url: 'inventory-adjusment-batch-admin-crud',
		urls: ['inventory-adjusment-batch-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const inventoryAdjusmentBatchDataFactory = new InventoryAdjusmentBatchDataFactory();

	const defaultDataSize = 30;
	const defaultData = inventoryAdjusmentBatchDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InventoryAdjusmentBatchModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		inventoryAdjusmentBatchModelState = initialInventoryAdjusmentBatchModelState;
		store.setState({model: inventoryAdjusmentBatchModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInventoryAdjusmentBatchCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInventoryAdjusmentBatchCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInventoryAdjusmentBatchCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInventoryAdjusmentBatchCrudListComponent);
			inventoryAdjusmentBatchCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(inventoryAdjusmentBatchCrudListComponent.inventoryAdjusmentBatchsCount).toBe(defaultData.length);
			...
			expect(inventoryAdjusmentBatchCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			inventoryAdjusmentBatchCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = inventoryAdjusmentBatchDataFactory.createAll(filteredDataSize);
			...
			expect(inventoryAdjusmentBatchCrudListComponent.inventoryAdjusmentBatchsCount).toBe(filteredDataSize);
			expect(inventoryAdjusmentBatchCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(inventoryAdjusmentBatchCrudListComponent.pageIndex).toBe(0);
			expect(inventoryAdjusmentBatchCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			inventoryAdjusmentBatchCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		inventoryAdjusmentBatchCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		inventoryAdjusmentBatchCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
