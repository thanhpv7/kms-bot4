...
import * as modelAction from 'src/app/models/inventoryAdjusmentBatch/inventory_adjusment_batch.model.action';
import {InventoryAdjusmentBatchModelState, InventoryAdjusmentBatchModelAudit} from 'src/app/models/inventoryAdjusmentBatch/inventory_adjusment_batch.model.state';
import {InventoryAdjusmentBatchModel} from 'src/app/models/inventoryAdjusmentBatch/inventory_adjusment_batch.model';
import {
	getInventoryAdjusmentBatchModelAuditsByEntityId,
	getInventoryAdjusmentBatchModelWithId,
	getCountInventoryAdjusmentBatchModels,
...
import {CrudTileMode} from '../inventory-adjusment-batch-admin-crud.component';
...
@Component({
	selector: 'cb-inventory-adjusment-batch-admin-crud-edit',
	templateUrl: './inventory-adjusment-batch-admin-crud-edit.component.html',
	styleUrls: [
		'./inventory-adjusment-batch-admin-crud-edit.component.scss',
...
export class AdminInventoryAdjusmentBatchCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '6df47b28-6325-453d-93e0-97446754087e';
	 ...
	 */
	inventoryAdjusmentBatchAudits$: Observable<InventoryAdjusmentBatchModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Inventory Adjustment Item and Inventory Adjusment Batchs here] off begin
	inventoryAdjustmentItemMultiCrudActive: boolean = false;
	inventoryAdjustmentItemDisplayName = 'Inventory Adjustment Item';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Inventory Adjustment Item and Inventory Adjusment Batchs here] end
	inventoryAdjustmentItemOppositeRelation = 'inventoryAdjusmentBatchs';
	inventoryAdjustmentItemRelations: ModelRelation[] = this.inventoryAdjustmentItemMultiCrudActive ? InventoryAdjustmentItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.inventoryAdjustmentItemOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'inventoryAdjusmentBatchs';
	...
	warehouseOppositeRelation = 'inventoryAdjusmentBatchs';
	 ...
	 */
	modelProperties: ModelProperty[] = InventoryAdjusmentBatchModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InventoryAdjusmentBatchModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InventoryAdjusmentBatchModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'inventoryAdjustmentItem' && this.inventoryAdjustmentItemMultiCrudActive) {
	 ...
	 */
	targetModel: InventoryAdjusmentBatchModel;
	...
	constructor(
		private readonly store: Store<{ model: InventoryAdjusmentBatchModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInventoryAdjusmentBatchModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InventoryAdjusmentBatchModel();
		...
		if (this.targetModelId) {
			this.inventoryAdjusmentBatchAudits$ = this.store.select(getInventoryAdjusmentBatchModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InventoryAdjusmentBatchModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
			modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_INVENTORY_ADJUSMENT_BATCH,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInventoryAdjusmentBatchModelWithId, this.targetModelId)
			...
			model = new InventoryAdjusmentBatchModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('inventory-adjusment-batch');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
			modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_INVENTORY_ADJUSMENT_BATCH_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InventoryAdjusmentBatchModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.inventoryAdjustmentItemMultiCrudActive || this.stockCatalogueMultiCrudActive || this.warehouseMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InventoryAdjusmentBatchAction(
						modelAction.InventoryAdjusmentBatchModelActionTypes.COUNT_INVENTORY_ADJUSMENT_BATCHS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
				modelAction.InventoryAdjusmentBatchModelActionTypes.CREATE_INVENTORY_ADJUSMENT_BATCH,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InventoryAdjusmentBatchAction(
				modelAction.InventoryAdjusmentBatchModelActionTypes.UPDATE_INVENTORY_ADJUSMENT_BATCH,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InventoryAdjusmentBatchModel = new InventoryAdjusmentBatchModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInventoryAdjusmentBatchModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InventoryAdjusmentBatchModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.inventoryAdjustmentItemDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InventoryAdjustmentItemModel.getProps(), this.inventoryAdjustmentItemRelations, this.isViewOnly),
					new InventoryAdjustmentItemModel(),
					InventoryAdjustmentItemModel,
					this.inventoryAdjustmentItemDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.inventoryAdjustmentItemRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InventoryAdjusmentBatchModel.getProps();
		this.modelRelations = InventoryAdjusmentBatchModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'inventoryAdjustmentItem';
		}).hideElement = this.inventoryAdjustmentItemMultiCrudActive || this.modelRelations.find(rel => rel.id === 'inventoryAdjustmentItem').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.inventoryAdjustmentItemMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.inventoryAdjustmentItemDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.inventoryAdjustmentItem && this.targetModel.inventoryAdjustmentItem.id && this.inventoryAdjustmentItemMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InventoryAdjustmentItemModel,
				this.inventoryAdjustmentItemOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.inventoryAdjustmentItem.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getInventoryAdjustmentItemModelWithId, this.targetModel.inventoryAdjustmentItemId).subscribe(inventoryAdjustmentItem => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InventoryAdjustmentItemModel.getProps(), this.inventoryAdjustmentItemRelations, this.isViewOnly),
	 ...
	 */
	addAdditionalEntitiesToModel(model: InventoryAdjusmentBatchModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InventoryAdjusmentBatchModel {
		let updatedModel: InventoryAdjusmentBatchModel = new InventoryAdjusmentBatchModel(model);

		if (this.inventoryAdjustmentItemMultiCrudActive) {
			updatedModel.inventoryAdjustmentItem = event.additionalEntities.inventoryAdjustmentItem ? event.additionalEntities.inventoryAdjustmentItem : undefined;

			if (!updatedModel.inventoryAdjustmentItem && event.payload) {
				event.payload.inventoryAdjustmentItemId = null;
			}
		} else if (event.payload?.inventoryAdjustmentItemId !== undefined) {
			event.payload.inventoryAdjustmentItem = null;
			updatedModel.inventoryAdjustmentItem = null;
		}
