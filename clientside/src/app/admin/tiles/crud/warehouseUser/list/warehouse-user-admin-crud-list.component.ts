...
import * as modelAction from 'src/app/models/warehouseUser/warehouse_user.model.action';
import {WarehouseUserModelState} from 'src/app/models/warehouseUser/warehouse_user.model.state';
...
import {
	getWarehouseUserCollectionModels,
	getWarehouseUserCollectionCount
...
import {CrudTileMode} from '../warehouse-user-admin-crud.component';
...
@Component({
	selector: 'cb-warehouse-user-admin-crud-list',
	templateUrl: './warehouse-user-admin-crud-list.component.html',
	styleUrls: [
		'./warehouse-user-admin-crud-list.component.scss',
...
export class AdminWarehouseUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<WarehouseUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...WarehouseUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = WarehouseUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'c408c9e4-734e-4d7b-a378-a12cafed65c6';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.warehouseUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.warehouseUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: WarehouseUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseUserAction(
				modelAction.WarehouseUserModelActionTypes.INITIALISE_WAREHOUSE_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseUserAction(
				modelAction.WarehouseUserModelActionTypes.FETCH_WAREHOUSE_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.WarehouseUserAction(
						modelAction.WarehouseUserModelActionTypes.COUNT_WAREHOUSE_USERS,
						null
					)
				]
			))
		...
		this.warehouseUsers = this.store.select(getWarehouseUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getWarehouseUserCollectionCount, this.collectionId).subscribe(
				(count) => this.warehouseUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: WarehouseUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.WarehouseUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.WarehouseUserModelActionTypes.EXPORT_ALL_WAREHOUSE_USER;
		} else {
			actionType = modelAction.WarehouseUserModelActionTypes.EXPORT_WAREHOUSE_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.WarehouseUserAction(
				modelAction.WarehouseUserModelActionTypes.IMPORT_WAREHOUSE_USERS,
				{
					file: file,
				},
				[new modelAction.WarehouseUserAction(
			...
			window.open(environment.API_URL + '/api/warehouseuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: WarehouseUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Warehouse User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Warehouse User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.WarehouseUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.WarehouseUserModelActionTypes.DELETE_WAREHOUSE_USER;
						} else if (usingSelected) {
							actionType = modelAction.WarehouseUserModelActionTypes.DELETE_ALL_WAREHOUSE_USER;
						} else {
							actionType = modelAction.WarehouseUserModelActionTypes.DELETE_WAREHOUSE_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.WarehouseUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_WAREHOUSE_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_WAREHOUSE_USER and DELETE_WAREHOUSE_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<WarehouseUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = WarehouseUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.warehouseUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
