...
import {AdminWarehouseUserCrudListComponent} from './warehouse-user-admin-crud-list.component';
...
import {AdminWarehouseUserCrudModule} from '../warehouse-user-admin-crud.module';
import {
	WarehouseUserModelState,
	initialState as initialWarehouseUserModelState
} from 'src/app/models/warehouseUser/warehouse_user.model.state';
 ...
 */
describe('Warehouse User Crud List Tile Component', () => {

	let fixture;
	let warehouseUserCrudListComponent: AdminWarehouseUserCrudListComponent;

	let store: MockStore<{ model: WarehouseUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let warehouseUserModelState: WarehouseUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<WarehouseUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c408c9e4-734e-4d7b-a378-a12cafed65c6';
	const searchContent = 'text';

	const searchableFieldCount = WarehouseUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'warehouse-user-admin-crud',
		urls: ['warehouse-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const warehouseUserDataFactory = new WarehouseUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = warehouseUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: WarehouseUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		warehouseUserModelState = initialWarehouseUserModelState;
		store.setState({model: warehouseUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getWarehouseUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getWarehouseUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminWarehouseUserCrudListComponent);
			warehouseUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(warehouseUserCrudListComponent.warehouseUsersCount).toBe(defaultData.length);
			...
			expect(warehouseUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			warehouseUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = warehouseUserDataFactory.createAll(filteredDataSize);
			...
			expect(warehouseUserCrudListComponent.warehouseUsersCount).toBe(filteredDataSize);
			expect(warehouseUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(warehouseUserCrudListComponent.pageIndex).toBe(0);
			expect(warehouseUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			warehouseUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		warehouseUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		warehouseUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
