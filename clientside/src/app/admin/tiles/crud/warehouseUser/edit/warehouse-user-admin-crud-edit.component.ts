...
import {WarehouseUserModelState, WarehouseUserModelAudit} from 'src/app/models/warehouseUser/warehouse_user.model.state';
import {WarehouseUserModel} from 'src/app/models/warehouseUser/warehouse_user.model';
import {
	getWarehouseUserModelAuditsByEntityId,
	getWarehouseUserModelWithId,
	getCountWarehouseUserModels,
...
@Component({
	selector: 'cb-warehouse-user-admin-crud-edit',
	templateUrl: './warehouse-user-admin-crud-edit.component.html',
	styleUrls: [
		'./warehouse-user-admin-crud-edit.component.scss',
...
export class AdminWarehouseUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	warehouseUserAudits$: Observable<WarehouseUserModelAudit[]>;
	...
	staffOppositeRelation = 'warehouseUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = WarehouseUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: WarehouseUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountWarehouseUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new WarehouseUserModel();
		...
		if (this.targetModelId) {
			this.warehouseUserAudits$ = this.store.select(getWarehouseUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<WarehouseUserModel> = {
		...
		this.store.dispatch(new modelAction.WarehouseUserAction(
			modelAction.WarehouseUserModelActionTypes.FETCH_WAREHOUSE_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getWarehouseUserModelWithId, this.targetModelId)
			...
			model = new WarehouseUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.WarehouseUserAction(
			modelAction.WarehouseUserModelActionTypes.FETCH_WAREHOUSE_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.WarehouseUserAction(
				modelAction.WarehouseUserModelActionTypes.CREATE_WAREHOUSE_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.WarehouseUserAction(
				modelAction.WarehouseUserModelActionTypes.UPDATE_WAREHOUSE_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: WarehouseUserModel = new WarehouseUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getWarehouseUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new WarehouseUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = WarehouseUserModel.getProps();
		this.modelRelations = WarehouseUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: WarehouseUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): WarehouseUserModel {
		let updatedModel: WarehouseUserModel = new WarehouseUserModel(model);
