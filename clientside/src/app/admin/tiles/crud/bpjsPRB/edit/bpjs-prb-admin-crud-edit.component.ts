...
import * as modelAction from 'src/app/models/bpjsPRB/bpjs_prb.model.action';
import {BpjsPRBModelState, BpjsPRBModelAudit} from 'src/app/models/bpjsPRB/bpjs_prb.model.state';
import {BpjsPRBModel} from 'src/app/models/bpjsPRB/bpjs_prb.model';
import {
	getBpjsPRBModelAuditsByEntityId,
...
import {CrudTileMode} from '../bpjs-prb-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-prb-admin-crud-edit',
	templateUrl: './bpjs-prb-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-prb-admin-crud-edit.component.scss',
...
export class AdminBpjsPRBCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '2b5eaf89-45ba-4b6c-8884-40f933997dfb';
	 ...
	 */
	bpjsPRBAudits$: Observable<BpjsPRBModelAudit[]>;
	...
	bpjsDiagnosePRBOppositeRelation = 'bpjsPRBs';
	...
	bpjsDoctorMappingOppositeRelation = 'bpjsPRBs';
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsPRBModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPRBModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPRBModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	constructor(
		private readonly store: Store<{ model: BpjsPRBModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPRBModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPRBModel();
		...
		if (this.targetModelId) {
			this.bpjsPRBAudits$ = this.store.select(getBpjsPRBModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPRBModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsPRBAction(
			modelAction.BpjsPRBModelActionTypes.FETCH_BPJS_PRB,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPRBModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPRBAction(
			modelAction.BpjsPRBModelActionTypes.FETCH_BPJS_PRB_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsPRBModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.bpjsDiagnosePRBMultiCrudActive || this.bpjsDoctorMappingMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsPRBAction(
						modelAction.BpjsPRBModelActionTypes.COUNT_BPJS_PRBS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsPRBAction(
				modelAction.BpjsPRBModelActionTypes.CREATE_BPJS_PRB,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPRBAction(
				modelAction.BpjsPRBModelActionTypes.UPDATE_BPJS_PRB,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPRBModel = new BpjsPRBModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPRBModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPRBModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPRBModel.getProps();
		this.modelRelations = BpjsPRBModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPRBModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPRBModel {
		let updatedModel: BpjsPRBModel = new BpjsPRBModel(model);
