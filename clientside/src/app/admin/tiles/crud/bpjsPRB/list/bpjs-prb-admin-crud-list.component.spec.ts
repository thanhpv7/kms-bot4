...
import {AdminBpjsPRBCrudListComponent} from './bpjs-prb-admin-crud-list.component';
...
import {AdminBpjsPRBCrudModule} from '../bpjs-prb-admin-crud.module';
import {
	BpjsPRBModelState,
 ...
 */
describe('BPJS PRB Crud List Tile Component', () => {

	let fixture;
	let bpjsPRBCrudListComponent: AdminBpjsPRBCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPRBModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '2b5eaf89-45ba-4b6c-8884-40f933997dfb';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPRBModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-prb-admin-crud',
		urls: ['bpjs-prb-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPRBDataFactory = new BpjsPRBDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPRBDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPRBCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPRBCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsPRBCrudListComponent);
			bpjsPRBCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPRBCrudListComponent.bpjsPRBsCount).toBe(defaultData.length);
			...
			expect(bpjsPRBCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPRBCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPRBDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPRBCrudListComponent.bpjsPRBsCount).toBe(filteredDataSize);
			expect(bpjsPRBCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPRBCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPRBCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPRBCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPRBCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPRBCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
