...
import {BpjsPRBModelState} from 'src/app/models/bpjsPRB/bpjs_prb.model.state';
...
@Component({
	selector: 'cb-bpjs-prb-admin-crud-list',
	templateUrl: './bpjs-prb-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-prb-admin-crud-list.component.scss',
...
export class AdminBpjsPRBCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPRBModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPRBModel.searchFields,
	...
	/**
	 * List of BPJS PRBs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPRBs: Observable<BpjsPRBModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPRBsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPRBsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPRBsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPRBAction(
				modelAction.BpjsPRBModelActionTypes.INITIALISE_BPJS_PRB_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPRBAction(
				modelAction.BpjsPRBModelActionTypes.FETCH_BPJS_PRB_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsPRBs = this.store.select(getBpjsPRBCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPRBCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPRBsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPRBModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPRBModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPRBModelActionTypes.EXPORT_ALL_BPJS_PRB;
		} else {
			actionType = modelAction.BpjsPRBModelActionTypes.EXPORT_BPJS_PRB_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPRBAction(
				modelAction.BpjsPRBModelActionTypes.IMPORT_BPJS_PRBS,
				{
					file: file,
				},
				[new modelAction.BpjsPRBAction(
			...
			window.open(environment.API_URL + '/api/bpjsprb/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPRBModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS PRB entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS PRB entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPRBModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPRBModelActionTypes.DELETE_BPJS_PRB;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPRBModelActionTypes.DELETE_ALL_BPJS_PRB;
						} else {
							actionType = modelAction.BpjsPRBModelActionTypes.DELETE_BPJS_PRB_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPRBAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PRB when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PRB and DELETE_BPJS_PRB_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPRBModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPRBsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
