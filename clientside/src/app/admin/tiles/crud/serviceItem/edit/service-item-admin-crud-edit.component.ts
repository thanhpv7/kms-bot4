...
import * as modelAction from 'src/app/models/serviceItem/service_item.model.action';
import {ServiceItemModelState, ServiceItemModelAudit} from 'src/app/models/serviceItem/service_item.model.state';
import {ServiceItemModel} from 'src/app/models/serviceItem/service_item.model';
import {
	getServiceItemModelAuditsByEntityId,
...
import {CrudTileMode} from '../service-item-admin-crud.component';
...
@Component({
	selector: 'cb-service-item-admin-crud-edit',
	templateUrl: './service-item-admin-crud-edit.component.html',
	styleUrls: [
		'./service-item-admin-crud-edit.component.scss',
...
export class AdminServiceItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'a296928e-a6b9-42c9-97e7-4fa4b6977181';
	 ...
	 */
	serviceItemAudits$: Observable<ServiceItemModelAudit[]>;
	...
	examinationItemsOppositeRelation = 'serviceItem';
	...
	serviceItemAssignmentsOppositeRelation = 'serviceItem';
	...
	servicesOppositeRelation = 'serviceItems';
	 ...
	 */
	modelProperties: ModelProperty[] = ServiceItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ServiceItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ServiceItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	constructor(
		private readonly store: Store<{ model: ServiceItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountServiceItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ServiceItemModel();
		...
		if (this.targetModelId) {
			this.serviceItemAudits$ = this.store.select(getServiceItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ServiceItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.ServiceItemAction(
			modelAction.ServiceItemModelActionTypes.FETCH_SERVICE_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getServiceItemModelWithId, this.targetModelId)
			...
			model = new ServiceItemModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ServiceItemAction(
			modelAction.ServiceItemModelActionTypes.FETCH_SERVICE_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ServiceItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.examinationItemsMultiCrudActive || this.serviceItemAssignmentsMultiCrudActive || this.servicesMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.ServiceItemAction(
						modelAction.ServiceItemModelActionTypes.COUNT_SERVICE_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.ServiceItemAction(
				modelAction.ServiceItemModelActionTypes.CREATE_SERVICE_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ServiceItemAction(
				modelAction.ServiceItemModelActionTypes.UPDATE_SERVICE_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ServiceItemModel = new ServiceItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getServiceItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ServiceItemModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ServiceItemModel.getProps();
		this.modelRelations = ServiceItemModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ServiceItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ServiceItemModel {
		let updatedModel: ServiceItemModel = new ServiceItemModel(model);
