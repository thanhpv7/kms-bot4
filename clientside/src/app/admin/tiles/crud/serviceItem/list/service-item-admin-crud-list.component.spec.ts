...
import {AdminServiceItemCrudListComponent} from './service-item-admin-crud-list.component';
...
import {AdminServiceItemCrudModule} from '../service-item-admin-crud.module';
import {
 ...
 */
describe('Service Item Crud List Tile Component', () => {

	let fixture;
	let serviceItemCrudListComponent: AdminServiceItemCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ServiceItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a296928e-a6b9-42c9-97e7-4fa4b6977181';
	const searchContent = 'text';

	const searchableFieldCount = ServiceItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'service-item-admin-crud',
		urls: ['service-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const serviceItemDataFactory = new ServiceItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = serviceItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getServiceItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getServiceItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminServiceItemCrudListComponent);
			serviceItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(serviceItemCrudListComponent.serviceItemsCount).toBe(defaultData.length);
			...
			expect(serviceItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			serviceItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = serviceItemDataFactory.createAll(filteredDataSize);
			...
			expect(serviceItemCrudListComponent.serviceItemsCount).toBe(filteredDataSize);
			expect(serviceItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(serviceItemCrudListComponent.pageIndex).toBe(0);
			expect(serviceItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			serviceItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		serviceItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		serviceItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
