...
import {ServiceItemModelState} from 'src/app/models/serviceItem/service_item.model.state';
...
@Component({
	selector: 'cb-service-item-admin-crud-list',
	templateUrl: './service-item-admin-crud-list.component.html',
	styleUrls: [
		'./service-item-admin-crud-list.component.scss',
...
export class AdminServiceItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ServiceItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ServiceItemModel.searchFields,
	...
	/**
	 * List of Service Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	serviceItems: Observable<ServiceItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	serviceItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.serviceItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.serviceItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceItemAction(
				modelAction.ServiceItemModelActionTypes.INITIALISE_SERVICE_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceItemAction(
				modelAction.ServiceItemModelActionTypes.FETCH_SERVICE_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.serviceItems = this.store.select(getServiceItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getServiceItemCollectionCount, this.collectionId).subscribe(
				(count) => this.serviceItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ServiceItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ServiceItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ServiceItemModelActionTypes.EXPORT_ALL_SERVICE_ITEM;
		} else {
			actionType = modelAction.ServiceItemModelActionTypes.EXPORT_SERVICE_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ServiceItemAction(
				modelAction.ServiceItemModelActionTypes.IMPORT_SERVICE_ITEMS,
				{
					file: file,
				},
				[new modelAction.ServiceItemAction(
			...
			window.open(environment.API_URL + '/api/serviceitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ServiceItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Service Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Service Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ServiceItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ServiceItemModelActionTypes.DELETE_SERVICE_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.ServiceItemModelActionTypes.DELETE_ALL_SERVICE_ITEM;
						} else {
							actionType = modelAction.ServiceItemModelActionTypes.DELETE_SERVICE_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ServiceItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SERVICE_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SERVICE_ITEM and DELETE_SERVICE_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ServiceItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.serviceItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
