...
import {DoctorUserModelState, DoctorUserModelAudit} from 'src/app/models/doctorUser/doctor_user.model.state';
import {DoctorUserModel} from 'src/app/models/doctorUser/doctor_user.model';
import {
	getDoctorUserModelAuditsByEntityId,
	getDoctorUserModelWithId,
	getCountDoctorUserModels,
...
@Component({
	selector: 'cb-doctor-user-admin-crud-edit',
	templateUrl: './doctor-user-admin-crud-edit.component.html',
	styleUrls: [
		'./doctor-user-admin-crud-edit.component.scss',
...
export class AdminDoctorUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	doctorUserAudits$: Observable<DoctorUserModelAudit[]>;
	...
	staffOppositeRelation = 'doctorUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DoctorUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDoctorUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DoctorUserModel();
		...
		if (this.targetModelId) {
			this.doctorUserAudits$ = this.store.select(getDoctorUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DoctorUserModel> = {
		...
		this.store.dispatch(new modelAction.DoctorUserAction(
			modelAction.DoctorUserModelActionTypes.FETCH_DOCTOR_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDoctorUserModelWithId, this.targetModelId)
			...
			model = new DoctorUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DoctorUserAction(
			modelAction.DoctorUserModelActionTypes.FETCH_DOCTOR_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.DoctorUserAction(
				modelAction.DoctorUserModelActionTypes.CREATE_DOCTOR_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DoctorUserAction(
				modelAction.DoctorUserModelActionTypes.UPDATE_DOCTOR_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DoctorUserModel = new DoctorUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDoctorUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DoctorUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DoctorUserModel.getProps();
		this.modelRelations = DoctorUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: DoctorUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DoctorUserModel {
		let updatedModel: DoctorUserModel = new DoctorUserModel(model);
