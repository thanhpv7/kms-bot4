...
import * as modelAction from 'src/app/models/doctorUser/doctor_user.model.action';
import {DoctorUserModelState} from 'src/app/models/doctorUser/doctor_user.model.state';
...
import {
	getDoctorUserCollectionModels,
	getDoctorUserCollectionCount
...
import {CrudTileMode} from '../doctor-user-admin-crud.component';
...
@Component({
	selector: 'cb-doctor-user-admin-crud-list',
	templateUrl: './doctor-user-admin-crud-list.component.html',
	styleUrls: [
		'./doctor-user-admin-crud-list.component.scss',
...
export class AdminDoctorUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DoctorUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DoctorUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = DoctorUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'e5c1306f-affe-4853-b0e5-f5cd45fe024c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.doctorUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.doctorUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: DoctorUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DoctorUserAction(
				modelAction.DoctorUserModelActionTypes.INITIALISE_DOCTOR_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DoctorUserAction(
				modelAction.DoctorUserModelActionTypes.FETCH_DOCTOR_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.DoctorUserAction(
						modelAction.DoctorUserModelActionTypes.COUNT_DOCTOR_USERS,
						null
					)
				]
			))
		...
		this.doctorUsers = this.store.select(getDoctorUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDoctorUserCollectionCount, this.collectionId).subscribe(
				(count) => this.doctorUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DoctorUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DoctorUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DoctorUserModelActionTypes.EXPORT_ALL_DOCTOR_USER;
		} else {
			actionType = modelAction.DoctorUserModelActionTypes.EXPORT_DOCTOR_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DoctorUserAction(
				modelAction.DoctorUserModelActionTypes.IMPORT_DOCTOR_USERS,
				{
					file: file,
				},
				[new modelAction.DoctorUserAction(
			...
			window.open(environment.API_URL + '/api/doctoruser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DoctorUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Doctor User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Doctor User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DoctorUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DoctorUserModelActionTypes.DELETE_DOCTOR_USER;
						} else if (usingSelected) {
							actionType = modelAction.DoctorUserModelActionTypes.DELETE_ALL_DOCTOR_USER;
						} else {
							actionType = modelAction.DoctorUserModelActionTypes.DELETE_DOCTOR_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DoctorUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DOCTOR_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DOCTOR_USER and DELETE_DOCTOR_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<DoctorUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DoctorUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.doctorUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
