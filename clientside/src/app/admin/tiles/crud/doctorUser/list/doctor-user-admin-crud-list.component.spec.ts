...
import {AdminDoctorUserCrudListComponent} from './doctor-user-admin-crud-list.component';
...
import {AdminDoctorUserCrudModule} from '../doctor-user-admin-crud.module';
import {
	DoctorUserModelState,
	initialState as initialDoctorUserModelState
} from 'src/app/models/doctorUser/doctor_user.model.state';
 ...
 */
describe('Doctor User Crud List Tile Component', () => {

	let fixture;
	let doctorUserCrudListComponent: AdminDoctorUserCrudListComponent;

	let store: MockStore<{ model: DoctorUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let doctorUserModelState: DoctorUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<DoctorUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e5c1306f-affe-4853-b0e5-f5cd45fe024c';
	const searchContent = 'text';

	const searchableFieldCount = DoctorUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'doctor-user-admin-crud',
		urls: ['doctor-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const doctorUserDataFactory = new DoctorUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = doctorUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: DoctorUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		doctorUserModelState = initialDoctorUserModelState;
		store.setState({model: doctorUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getDoctorUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDoctorUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminDoctorUserCrudListComponent);
			doctorUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(doctorUserCrudListComponent.doctorUsersCount).toBe(defaultData.length);
			...
			expect(doctorUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			doctorUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = doctorUserDataFactory.createAll(filteredDataSize);
			...
			expect(doctorUserCrudListComponent.doctorUsersCount).toBe(filteredDataSize);
			expect(doctorUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(doctorUserCrudListComponent.pageIndex).toBe(0);
			expect(doctorUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			doctorUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		doctorUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		doctorUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
