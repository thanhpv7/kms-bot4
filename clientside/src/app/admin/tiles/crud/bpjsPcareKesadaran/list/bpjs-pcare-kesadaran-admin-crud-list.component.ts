...
import * as modelAction from 'src/app/models/bpjsPcareKesadaran/bpjs_pcare_kesadaran.model.action';
import {BpjsPcareKesadaranModelState} from 'src/app/models/bpjsPcareKesadaran/bpjs_pcare_kesadaran.model.state';
...
import {CrudTileMode} from '../bpjs-pcare-kesadaran-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-kesadaran-admin-crud-list',
	templateUrl: './bpjs-pcare-kesadaran-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-kesadaran-admin-crud-list.component.scss',
...
export class AdminBpjsPcareKesadaranCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcareKesadaranModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcareKesadaranModel.searchFields,
	...
	/**
	 * List of BPJS Pcare Kesadarans that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcareKesadarans: Observable<BpjsPcareKesadaranModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcareKesadaransCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcareKesadaranModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcareKesadaranModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '6eccab44-e930-4866-a519-bc0ddab75d0b';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcareKesadaransCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcareKesadaransCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcareKesadaranModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
				modelAction.BpjsPcareKesadaranModelActionTypes.INITIALISE_BPJS_PCARE_KESADARAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
				modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_BPJS_PCARE_KESADARAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcareKesadaranAction(
						modelAction.BpjsPcareKesadaranModelActionTypes.COUNT_BPJS_PCARE_KESADARANS,
						null
					)
				]
			))
		...
		this.bpjsPcareKesadarans = this.store.select(getBpjsPcareKesadaranCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcareKesadaranCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcareKesadaransCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-kesadaran');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcareKesadaranModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcareKesadaranModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcareKesadaranModelActionTypes.EXPORT_ALL_BPJS_PCARE_KESADARAN;
		} else {
			actionType = modelAction.BpjsPcareKesadaranModelActionTypes.EXPORT_BPJS_PCARE_KESADARAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
				modelAction.BpjsPcareKesadaranModelActionTypes.IMPORT_BPJS_PCARE_KESADARANS,
				{
					file: file,
				},
				[new modelAction.BpjsPcareKesadaranAction(
			...
			window.open(environment.API_URL + '/api/bpjspcarekesadaran/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcareKesadaranModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Kesadaran entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Kesadaran entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcareKesadaranModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcareKesadaranModelActionTypes.DELETE_BPJS_PCARE_KESADARAN;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcareKesadaranModelActionTypes.DELETE_ALL_BPJS_PCARE_KESADARAN;
						} else {
							actionType = modelAction.BpjsPcareKesadaranModelActionTypes.DELETE_BPJS_PCARE_KESADARAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_KESADARAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_KESADARAN and DELETE_BPJS_PCARE_KESADARAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcareKesadaranModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcareKesadaranModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcareKesadaransCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
