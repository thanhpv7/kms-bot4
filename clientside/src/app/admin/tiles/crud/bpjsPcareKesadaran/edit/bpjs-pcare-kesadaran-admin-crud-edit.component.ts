...
import {BpjsPcareKesadaranModelState, BpjsPcareKesadaranModelAudit} from 'src/app/models/bpjsPcareKesadaran/bpjs_pcare_kesadaran.model.state';
import {BpjsPcareKesadaranModel} from 'src/app/models/bpjsPcareKesadaran/bpjs_pcare_kesadaran.model';
import {
	getBpjsPcareKesadaranModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-pcare-kesadaran-admin-crud-edit',
	templateUrl: './bpjs-pcare-kesadaran-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-kesadaran-admin-crud-edit.component.scss',
...
export class AdminBpjsPcareKesadaranCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcareKesadaranAudits$: Observable<BpjsPcareKesadaranModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcareKesadaranModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcareKesadaranModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcareKesadaranModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcareKesadaranModel();
		...
		if (this.targetModelId) {
			this.bpjsPcareKesadaranAudits$ = this.store.select(getBpjsPcareKesadaranModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcareKesadaranModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
			modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_BPJS_PCARE_KESADARAN,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareKesadaranModelWithId, this.targetModelId)
			...
			model = new BpjsPcareKesadaranModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
			modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_BPJS_PCARE_KESADARAN_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
				modelAction.BpjsPcareKesadaranModelActionTypes.CREATE_BPJS_PCARE_KESADARAN,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcareKesadaranAction(
				modelAction.BpjsPcareKesadaranModelActionTypes.UPDATE_BPJS_PCARE_KESADARAN,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcareKesadaranModel = new BpjsPcareKesadaranModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareKesadaranModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcareKesadaranModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcareKesadaranModel.getProps();
		this.modelRelations = BpjsPcareKesadaranModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcareKesadaranModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcareKesadaranModel {
		let updatedModel: BpjsPcareKesadaranModel = new BpjsPcareKesadaranModel(model);
