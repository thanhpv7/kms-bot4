...
import * as modelAction from 'src/app/models/deliveryMedicalExaminationRecord/delivery_medical_examination_record.model.action';
import {DeliveryMedicalExaminationRecordModelState, DeliveryMedicalExaminationRecordModelAudit} from 'src/app/models/deliveryMedicalExaminationRecord/delivery_medical_examination_record.model.state';
...
import {
	getDeliveryMedicalExaminationRecordModelAuditsByEntityId,
...
import {CrudTileMode} from '../delivery-medical-examination-record-admin-crud.component';
...
@Component({
	selector: 'cb-delivery-medical-examination-record-admin-crud-edit',
	templateUrl: './delivery-medical-examination-record-admin-crud-edit.component.html',
	styleUrls: [
		'./delivery-medical-examination-record-admin-crud-edit.component.scss',
...
export class AdminDeliveryMedicalExaminationRecordCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '1c690958-cd46-4d92-9b72-a7fb26e56cc0';
	 ...
	 */
	deliveryMedicalExaminationRecordAudits$: Observable<DeliveryMedicalExaminationRecordModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'deliveryMedicalExaminationRecord';
	...
	midwifeMultiCrudActive: boolean = false;
	midwifeDisplayName = 'Midwife';
	...
	midwifeOppositeRelation = 'midwives';
	midwifeRelations: ModelRelation[] = this.midwifeMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.midwifeOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	additionalMidwivesMultiCrudActive: boolean = false;
	additionalMidwivesDisplayName = 'Additional Midwives';
	additionalMidwivesCollectionId: string = uuidv4();
	...
	additionalMidwivesOppositeRelation = 'additionalMidwife';
	additionalMidwivesRelations: ModelRelation[] = this.additionalMidwivesMultiCrudActive ? CoTreatingDoctorModel.getRelations()
		.map(relation => {
			if (relation.id === this.additionalMidwivesOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Delivery Medical Examination Record and Delivery Progresses here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Delivery Medical Examination Record and Delivery Progresses here] end
	deliveryProgressesOppositeRelation = 'deliveryMedicalExaminationRecord';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Delivery Medical Examination Record and Newborn Details here] off begin
	newbornDetailsMultiCrudActive: boolean = false;
	newbornDetailsDisplayName = 'Newborn Details';
	newbornDetailsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Delivery Medical Examination Record and Newborn Details here] end
	newbornDetailsOppositeRelation = 'deliveryMedicalExaminationRecord';
	newbornDetailsRelations: ModelRelation[] = this.newbornDetailsMultiCrudActive ? NewbornDetailModel.getRelations()
		.map(relation => {
			if (relation.id === this.newbornDetailsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = DeliveryMedicalExaminationRecordModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = DeliveryMedicalExaminationRecordModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DeliveryMedicalExaminationRecordModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'midwife' && this.midwifeMultiCrudActive) {
					exp.fields = StaffModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'additionalMidwives' && this.additionalMidwivesMultiCrudActive) {
				...
				if (relation.id === 'newbornDetails' && this.newbornDetailsMultiCrudActive) {
	 ...
	 */
	targetModel: DeliveryMedicalExaminationRecordModel;
	...
	constructor(
		private readonly store: Store<{ model: DeliveryMedicalExaminationRecordModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDeliveryMedicalExaminationRecordModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DeliveryMedicalExaminationRecordModel();
		...
		if (this.targetModelId) {
			this.deliveryMedicalExaminationRecordAudits$ = this.store.select(getDeliveryMedicalExaminationRecordModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DeliveryMedicalExaminationRecordModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
			modelAction.DeliveryMedicalExaminationRecordModelActionTypes.FETCH_DELIVERY_MEDICAL_EXAMINATION_RECORD,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDeliveryMedicalExaminationRecordModelWithId, this.targetModelId)
			...
			model = new DeliveryMedicalExaminationRecordModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('delivery-medical-examination-record');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
			modelAction.DeliveryMedicalExaminationRecordModelActionTypes.FETCH_DELIVERY_MEDICAL_EXAMINATION_RECORD_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<DeliveryMedicalExaminationRecordModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.medicalExaminationRecordMultiCrudActive || this.midwifeMultiCrudActive || this.additionalMidwivesMultiCrudActive || this.deliveryProgressesMultiCrudActive || this.newbornDetailsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.DeliveryMedicalExaminationRecordAction(
						modelAction.DeliveryMedicalExaminationRecordModelActionTypes.COUNT_DELIVERY_MEDICAL_EXAMINATION_RECORDS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
				modelAction.DeliveryMedicalExaminationRecordModelActionTypes.CREATE_DELIVERY_MEDICAL_EXAMINATION_RECORD,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
				modelAction.DeliveryMedicalExaminationRecordModelActionTypes.UPDATE_DELIVERY_MEDICAL_EXAMINATION_RECORD,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DeliveryMedicalExaminationRecordModel = new DeliveryMedicalExaminationRecordModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDeliveryMedicalExaminationRecordModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DeliveryMedicalExaminationRecordModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.midwifeDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.midwifeRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.midwifeDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.midwifeRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.additionalMidwivesDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(CoTreatingDoctorModel.getProps(), this.additionalMidwivesRelations, this.isViewOnly),
					new CoTreatingDoctorModel(),
					CoTreatingDoctorModel,
					this.additionalMidwivesDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.additionalMidwivesRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.newbornDetailsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(NewbornDetailModel.getProps(), this.newbornDetailsRelations, this.isViewOnly),
					new NewbornDetailModel(),
					NewbornDetailModel,
					this.newbornDetailsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.newbornDetailsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DeliveryMedicalExaminationRecordModel.getProps();
		this.modelRelations = DeliveryMedicalExaminationRecordModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'midwife';
		}).hideElement = this.midwifeMultiCrudActive || this.modelRelations.find(rel => rel.id === 'midwife').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'additionalMidwives';
		}).hideElement = this.additionalMidwivesMultiCrudActive || this.modelRelations.find(rel => rel.id === 'additionalMidwives').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'newbornDetails';
		}).hideElement = this.newbornDetailsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'newbornDetails').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.midwifeMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.midwifeDisplayName, many: false});
		}

		if (this.additionalMidwivesMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.additionalMidwivesDisplayName, many: true});
			this.store.dispatch(new coTreatingDoctorModelAction.CoTreatingDoctorAction(
				...
				{
					collectionId: this.additionalMidwivesCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getCoTreatingDoctorCollectionModels, this.additionalMidwivesCollectionId).subscribe(models => {
				models.forEach(additionalMidwives => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(CoTreatingDoctorModel.getProps(), this.additionalMidwivesRelations, this.isViewOnly),
						additionalMidwives,
						CoTreatingDoctorModel,
		...
		if (this.newbornDetailsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.newbornDetailsDisplayName, many: true});
			this.store.dispatch(new newbornDetailModelAction.NewbornDetailAction(
				...
				{
					collectionId: this.newbornDetailsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getNewbornDetailCollectionModels, this.newbornDetailsCollectionId).subscribe(models => {
				models.forEach(newbornDetails => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(NewbornDetailModel.getProps(), this.newbornDetailsRelations, this.isViewOnly),
						newbornDetails,
						NewbornDetailModel,
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.midwife && this.targetModel.midwife.id && this.midwifeMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.midwifeOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.midwife.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.midwifeId).subscribe(midwife => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.midwifeRelations, this.isViewOnly),
					midwife,
					StaffModel,
		...
		if (this.targetModel.additionalMidwivesIds && this.targetModel.additionalMidwivesIds.length > 0 && this.additionalMidwivesMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				CoTreatingDoctorModel,
				this.additionalMidwivesOppositeRelation,
				this.additionalMidwivesCollectionId,
				this.targetModel.additionalMidwivesIds.length,
				this.targetModel.id,
				true
			));
		}
		...
		if (this.targetModel.newbornDetailsIds && this.targetModel.newbornDetailsIds.length > 0 && this.newbornDetailsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				NewbornDetailModel,
				this.newbornDetailsOppositeRelation,
				this.newbornDetailsCollectionId,
				this.targetModel.newbornDetailsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: DeliveryMedicalExaminationRecordModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DeliveryMedicalExaminationRecordModel {
		let updatedModel: DeliveryMedicalExaminationRecordModel = new DeliveryMedicalExaminationRecordModel(model);
		...
		if (this.midwifeMultiCrudActive) {
			updatedModel.midwife = event.additionalEntities.midwife ? event.additionalEntities.midwife : undefined;

			if (!updatedModel.midwife && event.payload) {
				event.payload.midwifeId = null;
			}
		} else if (event.payload?.midwifeId !== undefined) {
			event.payload.midwife = null;
			updatedModel.midwife = null;
		}

		updatedModel.additionalMidwives = [];
		if (this.additionalMidwivesMultiCrudActive) {
			if (event.additionalEntities.additionalMidwives) {
				event.additionalEntities.additionalMidwives.forEach(entity => {
					updatedModel.additionalMidwives.push(entity);
				});
			}

			if (!updatedModel.additionalMidwives.length) {
				updatedModel.additionalMidwivesIds = [];
			}
		}
		...
		updatedModel.newbornDetails = [];
		if (this.newbornDetailsMultiCrudActive) {
			if (event.additionalEntities.newbornDetails) {
				event.additionalEntities.newbornDetails.forEach(entity => {
					updatedModel.newbornDetails.push(entity);
				});
			}

			if (!updatedModel.newbornDetails.length) {
				updatedModel.newbornDetailsIds = [];
			}
		}
		return updatedModel;
	}
