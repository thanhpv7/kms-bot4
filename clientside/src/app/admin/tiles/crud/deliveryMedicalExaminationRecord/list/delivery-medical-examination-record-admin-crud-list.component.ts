...
import {DeliveryMedicalExaminationRecordModelState} from 'src/app/models/deliveryMedicalExaminationRecord/delivery_medical_examination_record.model.state';
...
@Component({
	selector: 'cb-delivery-medical-examination-record-admin-crud-list',
	templateUrl: './delivery-medical-examination-record-admin-crud-list.component.html',
	styleUrls: [
		'./delivery-medical-examination-record-admin-crud-list.component.scss',
...
export class AdminDeliveryMedicalExaminationRecordCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DeliveryMedicalExaminationRecordModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DeliveryMedicalExaminationRecordModel.searchFields,
	...
	/**
	 * List of Delivery Medical Examination Records that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	deliveryMedicalExaminationRecords: Observable<DeliveryMedicalExaminationRecordModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	deliveryMedicalExaminationRecordsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.deliveryMedicalExaminationRecordsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.deliveryMedicalExaminationRecordsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
				modelAction.DeliveryMedicalExaminationRecordModelActionTypes.INITIALISE_DELIVERY_MEDICAL_EXAMINATION_RECORD_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
				modelAction.DeliveryMedicalExaminationRecordModelActionTypes.FETCH_DELIVERY_MEDICAL_EXAMINATION_RECORD_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.deliveryMedicalExaminationRecords = this.store.select(getDeliveryMedicalExaminationRecordCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDeliveryMedicalExaminationRecordCollectionCount, this.collectionId).subscribe(
				(count) => this.deliveryMedicalExaminationRecordsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DeliveryMedicalExaminationRecordModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DeliveryMedicalExaminationRecordModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DeliveryMedicalExaminationRecordModelActionTypes.EXPORT_ALL_DELIVERY_MEDICAL_EXAMINATION_RECORD;
		} else {
			actionType = modelAction.DeliveryMedicalExaminationRecordModelActionTypes.EXPORT_DELIVERY_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
				modelAction.DeliveryMedicalExaminationRecordModelActionTypes.IMPORT_DELIVERY_MEDICAL_EXAMINATION_RECORDS,
				{
					file: file,
				},
				[new modelAction.DeliveryMedicalExaminationRecordAction(
			...
			window.open(environment.API_URL + '/api/deliverymedicalexaminationrecord/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DeliveryMedicalExaminationRecordModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Delivery Medical Examination Record entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Delivery Medical Examination Record entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DeliveryMedicalExaminationRecordModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DeliveryMedicalExaminationRecordModelActionTypes.DELETE_DELIVERY_MEDICAL_EXAMINATION_RECORD;
						} else if (usingSelected) {
							actionType = modelAction.DeliveryMedicalExaminationRecordModelActionTypes.DELETE_ALL_DELIVERY_MEDICAL_EXAMINATION_RECORD;
						} else {
							actionType = modelAction.DeliveryMedicalExaminationRecordModelActionTypes.DELETE_DELIVERY_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DeliveryMedicalExaminationRecordAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DELIVERY_MEDICAL_EXAMINATION_RECORD when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DELIVERY_MEDICAL_EXAMINATION_RECORD and DELETE_DELIVERY_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DeliveryMedicalExaminationRecordModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.deliveryMedicalExaminationRecordsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
