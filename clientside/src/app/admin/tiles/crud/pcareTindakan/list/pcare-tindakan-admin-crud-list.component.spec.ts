...
import {AdminPcareTindakanCrudListComponent} from './pcare-tindakan-admin-crud-list.component';
import {PcareTindakanDataFactory} from 'src/app/lib/utils/factories/pcare-tindakan-data-factory';
import {PcareTindakanModel} from 'src/app/models/pcareTindakan/pcare_tindakan.model';
import {AdminPcareTindakanCrudModule} from '../pcare-tindakan-admin-crud.module';
import {
	PcareTindakanModelState,
	initialState as initialPcareTindakanModelState
} from 'src/app/models/pcareTindakan/pcare_tindakan.model.state';
 ...
 */
describe('PCare Tindakan Crud List Tile Component', () => {

	let fixture;
	let pcareTindakanCrudListComponent: AdminPcareTindakanCrudListComponent;

	let store: MockStore<{ model: PcareTindakanModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareTindakanModelState: PcareTindakanModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareTindakanModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '25611c8e-86de-4e75-86b8-03d06826e597';
	const searchContent = 'text';

	const searchableFieldCount = PcareTindakanModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-tindakan-admin-crud',
		urls: ['pcare-tindakan-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareTindakanDataFactory = new PcareTindakanDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareTindakanDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareTindakanModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareTindakanModelState = initialPcareTindakanModelState;
		store.setState({model: pcareTindakanModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareTindakanCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareTindakanCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPcareTindakanCrudListComponent);
			pcareTindakanCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareTindakanCrudListComponent.pcareTindakansCount).toBe(defaultData.length);
			...
			expect(pcareTindakanCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareTindakanCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareTindakanDataFactory.createAll(filteredDataSize);
			...
			expect(pcareTindakanCrudListComponent.pcareTindakansCount).toBe(filteredDataSize);
			expect(pcareTindakanCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareTindakanCrudListComponent.pageIndex).toBe(0);
			expect(pcareTindakanCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareTindakanCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareTindakanCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareTindakanCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
