...
import * as modelAction from 'src/app/models/pcareTindakan/pcare_tindakan.model.action';
import {PcareTindakanModelState} from 'src/app/models/pcareTindakan/pcare_tindakan.model.state';
...
import {
	getPcareTindakanCollectionModels,
	getPcareTindakanCollectionCount
} from 'src/app/models/pcareTindakan/pcare_tindakan.model.selector';
...
import {CrudTileMode} from '../pcare-tindakan-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-tindakan-admin-crud-list',
	templateUrl: './pcare-tindakan-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-tindakan-admin-crud-list.component.scss',
...
export class AdminPcareTindakanCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareTindakanModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareTindakanModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PcareTindakanModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '25611c8e-86de-4e75-86b8-03d06826e597';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareTindakansCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareTindakansCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareTindakanModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareTindakanAction(
				modelAction.PcareTindakanModelActionTypes.INITIALISE_PCARE_TINDAKAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareTindakanAction(
				modelAction.PcareTindakanModelActionTypes.FETCH_PCARE_TINDAKAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareTindakanAction(
						modelAction.PcareTindakanModelActionTypes.COUNT_PCARE_TINDAKANS,
						null
					)
				]
			))
		...
		this.pcareTindakans = this.store.select(getPcareTindakanCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareTindakanCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareTindakansCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareTindakanModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareTindakanModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareTindakanModelActionTypes.EXPORT_ALL_PCARE_TINDAKAN;
		} else {
			actionType = modelAction.PcareTindakanModelActionTypes.EXPORT_PCARE_TINDAKAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareTindakanAction(
				modelAction.PcareTindakanModelActionTypes.IMPORT_PCARE_TINDAKANS,
				{
					file: file,
				},
				[new modelAction.PcareTindakanAction(
			...
			window.open(environment.API_URL + '/api/pcaretindakan/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareTindakanModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Tindakan entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Tindakan entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareTindakanModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareTindakanModelActionTypes.DELETE_PCARE_TINDAKAN;
						} else if (usingSelected) {
							actionType = modelAction.PcareTindakanModelActionTypes.DELETE_ALL_PCARE_TINDAKAN;
						} else {
							actionType = modelAction.PcareTindakanModelActionTypes.DELETE_PCARE_TINDAKAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareTindakanAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_TINDAKAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_TINDAKAN and DELETE_PCARE_TINDAKAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareTindakanModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareTindakanModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareTindakansCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
