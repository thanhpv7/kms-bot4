...
import {PcareTindakanModelState, PcareTindakanModelAudit} from 'src/app/models/pcareTindakan/pcare_tindakan.model.state';
import {PcareTindakanModel} from 'src/app/models/pcareTindakan/pcare_tindakan.model';
import {
	getPcareTindakanModelAuditsByEntityId,
	getPcareTindakanModelWithId,
	getCountPcareTindakanModels,
...
@Component({
	selector: 'cb-pcare-tindakan-admin-crud-edit',
	templateUrl: './pcare-tindakan-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-tindakan-admin-crud-edit.component.scss',
...
export class AdminPcareTindakanCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareTindakanAudits$: Observable<PcareTindakanModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareTindakanModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PcareTindakanModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareTindakanModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareTindakanModel();
		...
		if (this.targetModelId) {
			this.pcareTindakanAudits$ = this.store.select(getPcareTindakanModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareTindakanModel> = {
		...
		this.store.dispatch(new modelAction.PcareTindakanAction(
			modelAction.PcareTindakanModelActionTypes.FETCH_PCARE_TINDAKAN,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareTindakanModelWithId, this.targetModelId)
			...
			model = new PcareTindakanModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareTindakanAction(
			modelAction.PcareTindakanModelActionTypes.FETCH_PCARE_TINDAKAN_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareTindakanAction(
				modelAction.PcareTindakanModelActionTypes.CREATE_PCARE_TINDAKAN,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareTindakanAction(
				modelAction.PcareTindakanModelActionTypes.UPDATE_PCARE_TINDAKAN,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareTindakanModel = new PcareTindakanModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareTindakanModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareTindakanModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareTindakanModel.getProps();
		this.modelRelations = PcareTindakanModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareTindakanModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareTindakanModel {
		let updatedModel: PcareTindakanModel = new PcareTindakanModel(model);
