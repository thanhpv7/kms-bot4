...
import {AdminPatientCaseHistoryCrudListComponent} from './patient-case-history-admin-crud-list.component';
...
import {AdminPatientCaseHistoryCrudModule} from '../patient-case-history-admin-crud.module';
import {
	PatientCaseHistoryModelState,
	initialState as initialPatientCaseHistoryModelState
} from 'src/app/models/patientCaseHistory/patient_case_history.model.state';
 ...
 */
describe('Patient Case History Crud List Tile Component', () => {

	let fixture;
	let patientCaseHistoryCrudListComponent: AdminPatientCaseHistoryCrudListComponent;

	let store: MockStore<{ model: PatientCaseHistoryModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientCaseHistoryModelState: PatientCaseHistoryModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientCaseHistoryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6a0bbae4-00d5-4146-bbc1-fb4703b9b020';
	const searchContent = 'text';

	const searchableFieldCount = PatientCaseHistoryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-case-history-admin-crud',
		urls: ['patient-case-history-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientCaseHistoryDataFactory = new PatientCaseHistoryDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientCaseHistoryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientCaseHistoryModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientCaseHistoryModelState = initialPatientCaseHistoryModelState;
		store.setState({model: patientCaseHistoryModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientCaseHistoryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientCaseHistoryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientCaseHistoryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientCaseHistoryCrudListComponent);
			patientCaseHistoryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientCaseHistoryCrudListComponent.patientCaseHistorysCount).toBe(defaultData.length);
			...
			expect(patientCaseHistoryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientCaseHistoryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientCaseHistoryDataFactory.createAll(filteredDataSize);
			...
			expect(patientCaseHistoryCrudListComponent.patientCaseHistorysCount).toBe(filteredDataSize);
			expect(patientCaseHistoryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientCaseHistoryCrudListComponent.pageIndex).toBe(0);
			expect(patientCaseHistoryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientCaseHistoryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientCaseHistoryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientCaseHistoryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
