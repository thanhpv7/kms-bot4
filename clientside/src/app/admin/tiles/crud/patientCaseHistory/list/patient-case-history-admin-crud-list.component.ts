...
import {PatientCaseHistoryModelState} from 'src/app/models/patientCaseHistory/patient_case_history.model.state';
...
import {
	getPatientCaseHistoryCollectionModels,
	getPatientCaseHistoryCollectionCount
...
@Component({
	selector: 'cb-patient-case-history-admin-crud-list',
	templateUrl: './patient-case-history-admin-crud-list.component.html',
	styleUrls: [
		'./patient-case-history-admin-crud-list.component.scss',
...
export class AdminPatientCaseHistoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientCaseHistoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientCaseHistoryModel.searchFields,
	...
	/**
	 * List of Patient Case Historys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientCaseHistorys: Observable<PatientCaseHistoryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientCaseHistorysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientCaseHistorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientCaseHistorysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientCaseHistoryAction(
				modelAction.PatientCaseHistoryModelActionTypes.INITIALISE_PATIENT_CASE_HISTORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientCaseHistoryAction(
				modelAction.PatientCaseHistoryModelActionTypes.FETCH_PATIENT_CASE_HISTORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientCaseHistorys = this.store.select(getPatientCaseHistoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientCaseHistoryCollectionCount, this.collectionId).subscribe(
				(count) => this.patientCaseHistorysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientCaseHistoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientCaseHistoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientCaseHistoryModelActionTypes.EXPORT_ALL_PATIENT_CASE_HISTORY;
		} else {
			actionType = modelAction.PatientCaseHistoryModelActionTypes.EXPORT_PATIENT_CASE_HISTORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientCaseHistoryAction(
				modelAction.PatientCaseHistoryModelActionTypes.IMPORT_PATIENT_CASE_HISTORYS,
				{
					file: file,
				},
				[new modelAction.PatientCaseHistoryAction(
			...
			window.open(environment.API_URL + '/api/patientcasehistory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientCaseHistoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Case History entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Case History entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientCaseHistoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientCaseHistoryModelActionTypes.DELETE_PATIENT_CASE_HISTORY;
						} else if (usingSelected) {
							actionType = modelAction.PatientCaseHistoryModelActionTypes.DELETE_ALL_PATIENT_CASE_HISTORY;
						} else {
							actionType = modelAction.PatientCaseHistoryModelActionTypes.DELETE_PATIENT_CASE_HISTORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientCaseHistoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_CASE_HISTORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_CASE_HISTORY and DELETE_PATIENT_CASE_HISTORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientCaseHistoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientCaseHistorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
