...
import * as modelAction from 'src/app/models/patientCaseHistory/patient_case_history.model.action';
import {PatientCaseHistoryModelState, PatientCaseHistoryModelAudit} from 'src/app/models/patientCaseHistory/patient_case_history.model.state';
import {PatientCaseHistoryModel} from 'src/app/models/patientCaseHistory/patient_case_history.model';
import {
	getPatientCaseHistoryModelAuditsByEntityId,
	getPatientCaseHistoryModelWithId,
	getCountPatientCaseHistoryModels,
...
import {CrudTileMode} from '../patient-case-history-admin-crud.component';
...
@Component({
	selector: 'cb-patient-case-history-admin-crud-edit',
	templateUrl: './patient-case-history-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-case-history-admin-crud-edit.component.scss',
...
export class AdminPatientCaseHistoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '6a0bbae4-00d5-4146-bbc1-fb4703b9b020';
	 ...
	 */
	patientCaseHistoryAudits$: Observable<PatientCaseHistoryModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientCaseHistories';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientCaseHistoryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientCaseHistoryModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientCaseHistoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientCaseHistoryModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientCaseHistoryModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientCaseHistoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientCaseHistoryModel();
		...
		if (this.targetModelId) {
			this.patientCaseHistoryAudits$ = this.store.select(getPatientCaseHistoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientCaseHistoryModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientCaseHistoryAction(
			modelAction.PatientCaseHistoryModelActionTypes.FETCH_PATIENT_CASE_HISTORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientCaseHistoryModelWithId, this.targetModelId)
			...
			model = new PatientCaseHistoryModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-case-history');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientCaseHistoryAction(
			modelAction.PatientCaseHistoryModelActionTypes.FETCH_PATIENT_CASE_HISTORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientCaseHistoryModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientCaseHistoryAction(
						modelAction.PatientCaseHistoryModelActionTypes.COUNT_PATIENT_CASE_HISTORYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientCaseHistoryAction(
				modelAction.PatientCaseHistoryModelActionTypes.CREATE_PATIENT_CASE_HISTORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientCaseHistoryAction(
				modelAction.PatientCaseHistoryModelActionTypes.UPDATE_PATIENT_CASE_HISTORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientCaseHistoryModel = new PatientCaseHistoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientCaseHistoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientCaseHistoryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientCaseHistoryModel.getProps();
		this.modelRelations = PatientCaseHistoryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientCaseHistoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientCaseHistoryModel {
		let updatedModel: PatientCaseHistoryModel = new PatientCaseHistoryModel(model);
