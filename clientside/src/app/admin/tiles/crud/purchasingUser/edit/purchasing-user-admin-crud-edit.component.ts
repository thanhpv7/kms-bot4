...
import {PurchasingUserModelState, PurchasingUserModelAudit} from 'src/app/models/purchasingUser/purchasing_user.model.state';
import {PurchasingUserModel} from 'src/app/models/purchasingUser/purchasing_user.model';
import {
	getPurchasingUserModelAuditsByEntityId,
	getPurchasingUserModelWithId,
	getCountPurchasingUserModels,
...
@Component({
	selector: 'cb-purchasing-user-admin-crud-edit',
	templateUrl: './purchasing-user-admin-crud-edit.component.html',
	styleUrls: [
		'./purchasing-user-admin-crud-edit.component.scss',
...
export class AdminPurchasingUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	purchasingUserAudits$: Observable<PurchasingUserModelAudit[]>;
	...
	staffOppositeRelation = 'purchasingUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PurchasingUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PurchasingUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPurchasingUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PurchasingUserModel();
		...
		if (this.targetModelId) {
			this.purchasingUserAudits$ = this.store.select(getPurchasingUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PurchasingUserModel> = {
		...
		this.store.dispatch(new modelAction.PurchasingUserAction(
			modelAction.PurchasingUserModelActionTypes.FETCH_PURCHASING_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPurchasingUserModelWithId, this.targetModelId)
			...
			model = new PurchasingUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PurchasingUserAction(
			modelAction.PurchasingUserModelActionTypes.FETCH_PURCHASING_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PurchasingUserAction(
				modelAction.PurchasingUserModelActionTypes.CREATE_PURCHASING_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PurchasingUserAction(
				modelAction.PurchasingUserModelActionTypes.UPDATE_PURCHASING_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PurchasingUserModel = new PurchasingUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPurchasingUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PurchasingUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PurchasingUserModel.getProps();
		this.modelRelations = PurchasingUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PurchasingUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PurchasingUserModel {
		let updatedModel: PurchasingUserModel = new PurchasingUserModel(model);
