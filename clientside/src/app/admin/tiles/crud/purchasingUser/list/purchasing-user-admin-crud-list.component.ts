...
import * as modelAction from 'src/app/models/purchasingUser/purchasing_user.model.action';
import {PurchasingUserModelState} from 'src/app/models/purchasingUser/purchasing_user.model.state';
...
import {
	getPurchasingUserCollectionModels,
	getPurchasingUserCollectionCount
...
import {CrudTileMode} from '../purchasing-user-admin-crud.component';
...
@Component({
	selector: 'cb-purchasing-user-admin-crud-list',
	templateUrl: './purchasing-user-admin-crud-list.component.html',
	styleUrls: [
		'./purchasing-user-admin-crud-list.component.scss',
...
export class AdminPurchasingUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PurchasingUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PurchasingUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PurchasingUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'c4ebd54c-77c8-4ad2-8a08-1dee2191a44f';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.purchasingUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.purchasingUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PurchasingUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchasingUserAction(
				modelAction.PurchasingUserModelActionTypes.INITIALISE_PURCHASING_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchasingUserAction(
				modelAction.PurchasingUserModelActionTypes.FETCH_PURCHASING_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PurchasingUserAction(
						modelAction.PurchasingUserModelActionTypes.COUNT_PURCHASING_USERS,
						null
					)
				]
			))
		...
		this.purchasingUsers = this.store.select(getPurchasingUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPurchasingUserCollectionCount, this.collectionId).subscribe(
				(count) => this.purchasingUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PurchasingUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PurchasingUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PurchasingUserModelActionTypes.EXPORT_ALL_PURCHASING_USER;
		} else {
			actionType = modelAction.PurchasingUserModelActionTypes.EXPORT_PURCHASING_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PurchasingUserAction(
				modelAction.PurchasingUserModelActionTypes.IMPORT_PURCHASING_USERS,
				{
					file: file,
				},
				[new modelAction.PurchasingUserAction(
			...
			window.open(environment.API_URL + '/api/purchasinguser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PurchasingUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Purchasing User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Purchasing User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PurchasingUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PurchasingUserModelActionTypes.DELETE_PURCHASING_USER;
						} else if (usingSelected) {
							actionType = modelAction.PurchasingUserModelActionTypes.DELETE_ALL_PURCHASING_USER;
						} else {
							actionType = modelAction.PurchasingUserModelActionTypes.DELETE_PURCHASING_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PurchasingUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PURCHASING_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PURCHASING_USER and DELETE_PURCHASING_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PurchasingUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PurchasingUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.purchasingUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
