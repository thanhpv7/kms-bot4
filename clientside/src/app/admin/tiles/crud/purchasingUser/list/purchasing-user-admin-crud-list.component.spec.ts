...
import {AdminPurchasingUserCrudListComponent} from './purchasing-user-admin-crud-list.component';
...
import {AdminPurchasingUserCrudModule} from '../purchasing-user-admin-crud.module';
import {
	PurchasingUserModelState,
	initialState as initialPurchasingUserModelState
} from 'src/app/models/purchasingUser/purchasing_user.model.state';
 ...
 */
describe('Purchasing User Crud List Tile Component', () => {

	let fixture;
	let purchasingUserCrudListComponent: AdminPurchasingUserCrudListComponent;

	let store: MockStore<{ model: PurchasingUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let purchasingUserModelState: PurchasingUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PurchasingUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c4ebd54c-77c8-4ad2-8a08-1dee2191a44f';
	const searchContent = 'text';

	const searchableFieldCount = PurchasingUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'purchasing-user-admin-crud',
		urls: ['purchasing-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const purchasingUserDataFactory = new PurchasingUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = purchasingUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PurchasingUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		purchasingUserModelState = initialPurchasingUserModelState;
		store.setState({model: purchasingUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPurchasingUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPurchasingUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPurchasingUserCrudListComponent);
			purchasingUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(purchasingUserCrudListComponent.purchasingUsersCount).toBe(defaultData.length);
			...
			expect(purchasingUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			purchasingUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = purchasingUserDataFactory.createAll(filteredDataSize);
			...
			expect(purchasingUserCrudListComponent.purchasingUsersCount).toBe(filteredDataSize);
			expect(purchasingUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(purchasingUserCrudListComponent.pageIndex).toBe(0);
			expect(purchasingUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			purchasingUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		purchasingUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		purchasingUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
