...
import {ChartOfAccountMappingModelState} from 'src/app/models/chartOfAccountMapping/chart_of_account_mapping.model.state';
...
import {
	...
	getChartOfAccountMappingCollectionCount
...
@Component({
	selector: 'cb-chart-of-account-mapping-admin-crud-list',
	templateUrl: './chart-of-account-mapping-admin-crud-list.component.html',
	styleUrls: [
		'./chart-of-account-mapping-admin-crud-list.component.scss',
...
export class AdminChartOfAccountMappingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ChartOfAccountMappingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ChartOfAccountMappingModel.searchFields,
	...
	/**
	 * List of Chart of Account Mappings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	chartOfAccountMappings: Observable<ChartOfAccountMappingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	chartOfAccountMappingsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.chartOfAccountMappingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.chartOfAccountMappingsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
				modelAction.ChartOfAccountMappingModelActionTypes.INITIALISE_CHART_OF_ACCOUNT_MAPPING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
				modelAction.ChartOfAccountMappingModelActionTypes.FETCH_CHART_OF_ACCOUNT_MAPPING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.chartOfAccountMappings = this.store.select(getChartOfAccountMappingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getChartOfAccountMappingCollectionCount, this.collectionId).subscribe(
				(count) => this.chartOfAccountMappingsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ChartOfAccountMappingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ChartOfAccountMappingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ChartOfAccountMappingModelActionTypes.EXPORT_ALL_CHART_OF_ACCOUNT_MAPPING;
		} else {
			actionType = modelAction.ChartOfAccountMappingModelActionTypes.EXPORT_CHART_OF_ACCOUNT_MAPPING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
				modelAction.ChartOfAccountMappingModelActionTypes.IMPORT_CHART_OF_ACCOUNT_MAPPINGS,
				{
					file: file,
				},
				[new modelAction.ChartOfAccountMappingAction(
			...
			window.open(environment.API_URL + '/api/chartofaccountmapping/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ChartOfAccountMappingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Chart of Account Mapping entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Chart of Account Mapping entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ChartOfAccountMappingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ChartOfAccountMappingModelActionTypes.DELETE_CHART_OF_ACCOUNT_MAPPING;
						} else if (usingSelected) {
							actionType = modelAction.ChartOfAccountMappingModelActionTypes.DELETE_ALL_CHART_OF_ACCOUNT_MAPPING;
						} else {
							actionType = modelAction.ChartOfAccountMappingModelActionTypes.DELETE_CHART_OF_ACCOUNT_MAPPING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CHART_OF_ACCOUNT_MAPPING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CHART_OF_ACCOUNT_MAPPING and DELETE_CHART_OF_ACCOUNT_MAPPING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ChartOfAccountMappingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.chartOfAccountMappingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
