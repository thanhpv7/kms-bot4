...
import * as modelAction from 'src/app/models/chartOfAccountMapping/chart_of_account_mapping.model.action';
import {ChartOfAccountMappingModelState, ChartOfAccountMappingModelAudit} from 'src/app/models/chartOfAccountMapping/chart_of_account_mapping.model.state';
import {ChartOfAccountMappingModel} from 'src/app/models/chartOfAccountMapping/chart_of_account_mapping.model';
import {
	getChartOfAccountMappingModelAuditsByEntityId,
...
import * as chartOfAccountModelAction from 'src/app/models/chartOfAccount/chart_of_account.model.action';
import {
	getChartOfAccountCollectionModels, getChartOfAccountModelWithId,
...
import {CrudTileMode} from '../chart-of-account-mapping-admin-crud.component';
...
@Component({
	selector: 'cb-chart-of-account-mapping-admin-crud-edit',
	templateUrl: './chart-of-account-mapping-admin-crud-edit.component.html',
	styleUrls: [
		'./chart-of-account-mapping-admin-crud-edit.component.scss',
...
export class AdminChartOfAccountMappingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '59a599ed-f12b-4b0d-9966-cbe5d61311f7';
	 ...
	 */
	chartOfAccountMappingAudits$: Observable<ChartOfAccountMappingModelAudit[]>;
	...
	chartOfAccountMultiCrudActive: boolean = false;
	chartOfAccountDisplayName = 'Chart of Account';
	...
	chartOfAccountOppositeRelation = 'chartOfAccountMappings';
	chartOfAccountRelations: ModelRelation[] = this.chartOfAccountMultiCrudActive ? ChartOfAccountModel.getRelations()
		.map(relation => {
			if (relation.id === this.chartOfAccountOppositeRelation) {
	 ...
	 */
	modelProperties: ModelProperty[] = ChartOfAccountMappingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ChartOfAccountMappingModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ChartOfAccountMappingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'chartOfAccount' && this.chartOfAccountMultiCrudActive) {
					exp.fields = ChartOfAccountModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: ChartOfAccountMappingModel;
	...
	constructor(
		private readonly store: Store<{ model: ChartOfAccountMappingModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountChartOfAccountMappingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ChartOfAccountMappingModel();
		...
		if (this.targetModelId) {
			this.chartOfAccountMappingAudits$ = this.store.select(getChartOfAccountMappingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ChartOfAccountMappingModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
			modelAction.ChartOfAccountMappingModelActionTypes.FETCH_CHART_OF_ACCOUNT_MAPPING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getChartOfAccountMappingModelWithId, this.targetModelId)
			...
			model = new ChartOfAccountMappingModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('chart-of-account-mapping');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
			modelAction.ChartOfAccountMappingModelActionTypes.FETCH_CHART_OF_ACCOUNT_MAPPING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ChartOfAccountMappingModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.chartOfAccountMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.ChartOfAccountMappingAction(
						modelAction.ChartOfAccountMappingModelActionTypes.COUNT_CHART_OF_ACCOUNT_MAPPINGS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
				modelAction.ChartOfAccountMappingModelActionTypes.CREATE_CHART_OF_ACCOUNT_MAPPING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ChartOfAccountMappingAction(
				modelAction.ChartOfAccountMappingModelActionTypes.UPDATE_CHART_OF_ACCOUNT_MAPPING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ChartOfAccountMappingModel = new ChartOfAccountMappingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getChartOfAccountMappingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ChartOfAccountMappingModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.chartOfAccountDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ChartOfAccountModel.getProps(), this.chartOfAccountRelations, this.isViewOnly),
					new ChartOfAccountModel(),
					ChartOfAccountModel,
					this.chartOfAccountDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.chartOfAccountRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ChartOfAccountMappingModel.getProps();
		this.modelRelations = ChartOfAccountMappingModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'chartOfAccount';
		}).hideElement = this.chartOfAccountMultiCrudActive || this.modelRelations.find(rel => rel.id === 'chartOfAccount').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.chartOfAccountMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.chartOfAccountDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.chartOfAccount && this.targetModel.chartOfAccount.id && this.chartOfAccountMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				ChartOfAccountModel,
				this.chartOfAccountOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.chartOfAccount.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getChartOfAccountModelWithId, this.targetModel.chartOfAccountId).subscribe(chartOfAccount => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ChartOfAccountModel.getProps(), this.chartOfAccountRelations, this.isViewOnly),
					chartOfAccount,
					ChartOfAccountModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: ChartOfAccountMappingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ChartOfAccountMappingModel {
		let updatedModel: ChartOfAccountMappingModel = new ChartOfAccountMappingModel(model);

		if (this.chartOfAccountMultiCrudActive) {
			updatedModel.chartOfAccount = event.additionalEntities.chartOfAccount ? event.additionalEntities.chartOfAccount : undefined;

			if (!updatedModel.chartOfAccount && event.payload) {
				event.payload.chartOfAccountId = null;
			}
		} else if (event.payload?.chartOfAccountId !== undefined) {
			event.payload.chartOfAccount = null;
			updatedModel.chartOfAccount = null;
		}
