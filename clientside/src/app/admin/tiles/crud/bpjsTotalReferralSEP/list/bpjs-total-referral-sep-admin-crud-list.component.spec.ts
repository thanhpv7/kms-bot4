...
import {AdminBpjsTotalReferralSEPCrudListComponent} from './bpjs-total-referral-sep-admin-crud-list.component';
...
import {AdminBpjsTotalReferralSEPCrudModule} from '../bpjs-total-referral-sep-admin-crud.module';
import {
 ...
 */
describe('BPJS Total Referral SEP Crud List Tile Component', () => {

	let fixture;
	let bpjsTotalReferralSEPCrudListComponent: AdminBpjsTotalReferralSEPCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsTotalReferralSEPModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c7d5c15f-9e32-493e-b7f5-b86ac0ec1cdd';
	const searchContent = 'text';

	const searchableFieldCount = BpjsTotalReferralSEPModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-total-referral-sep-admin-crud',
		urls: ['bpjs-total-referral-sep-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsTotalReferralSEPDataFactory = new BpjsTotalReferralSEPDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsTotalReferralSEPDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsTotalReferralSEPCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsTotalReferralSEPCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsTotalReferralSEPCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsTotalReferralSEPCrudListComponent);
			bpjsTotalReferralSEPCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsTotalReferralSEPCrudListComponent.bpjsTotalReferralSEPsCount).toBe(defaultData.length);
			...
			expect(bpjsTotalReferralSEPCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsTotalReferralSEPCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsTotalReferralSEPDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsTotalReferralSEPCrudListComponent.bpjsTotalReferralSEPsCount).toBe(filteredDataSize);
			expect(bpjsTotalReferralSEPCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsTotalReferralSEPCrudListComponent.pageIndex).toBe(0);
			expect(bpjsTotalReferralSEPCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsTotalReferralSEPCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsTotalReferralSEPCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsTotalReferralSEPCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
