...
import * as modelAction from 'src/app/models/bpjsTotalReferralSEP/bpjs_total_referral_sep.model.action';
import {BpjsTotalReferralSEPModelState} from 'src/app/models/bpjsTotalReferralSEP/bpjs_total_referral_sep.model.state';
...
import {
	...
	getBpjsTotalReferralSEPCollectionCount
...
import {CrudTileMode} from '../bpjs-total-referral-sep-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-total-referral-sep-admin-crud-list',
	templateUrl: './bpjs-total-referral-sep-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-total-referral-sep-admin-crud-list.component.scss',
...
export class AdminBpjsTotalReferralSEPCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsTotalReferralSEPModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsTotalReferralSEPModel.searchFields,
	...
	/**
	 * List of BPJS Total Referral SEPs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsTotalReferralSEPs: Observable<BpjsTotalReferralSEPModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsTotalReferralSEPsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsTotalReferralSEPModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsTotalReferralSEPModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'c7d5c15f-9e32-493e-b7f5-b86ac0ec1cdd';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsTotalReferralSEPsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsTotalReferralSEPsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsTotalReferralSEPModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
				modelAction.BpjsTotalReferralSEPModelActionTypes.INITIALISE_BPJS_TOTAL_REFERRAL_SEP_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
				modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_BPJS_TOTAL_REFERRAL_SEP_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsTotalReferralSEPAction(
						modelAction.BpjsTotalReferralSEPModelActionTypes.COUNT_BPJS_TOTAL_REFERRAL_SEPS,
						null
					)
				]
			))
		...
		this.bpjsTotalReferralSEPs = this.store.select(getBpjsTotalReferralSEPCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsTotalReferralSEPCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsTotalReferralSEPsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-total-referral-sep');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsTotalReferralSEPModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsTotalReferralSEPModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsTotalReferralSEPModelActionTypes.EXPORT_ALL_BPJS_TOTAL_REFERRAL_SEP;
		} else {
			actionType = modelAction.BpjsTotalReferralSEPModelActionTypes.EXPORT_BPJS_TOTAL_REFERRAL_SEP_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
				modelAction.BpjsTotalReferralSEPModelActionTypes.IMPORT_BPJS_TOTAL_REFERRAL_SEPS,
				{
					file: file,
				},
				[new modelAction.BpjsTotalReferralSEPAction(
			...
			window.open(environment.API_URL + '/api/bpjstotalreferralsep/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsTotalReferralSEPModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Total Referral SEP entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Total Referral SEP entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsTotalReferralSEPModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsTotalReferralSEPModelActionTypes.DELETE_BPJS_TOTAL_REFERRAL_SEP;
						} else if (usingSelected) {
							actionType = modelAction.BpjsTotalReferralSEPModelActionTypes.DELETE_ALL_BPJS_TOTAL_REFERRAL_SEP;
						} else {
							actionType = modelAction.BpjsTotalReferralSEPModelActionTypes.DELETE_BPJS_TOTAL_REFERRAL_SEP_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_TOTAL_REFERRAL_SEP when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_TOTAL_REFERRAL_SEP and DELETE_BPJS_TOTAL_REFERRAL_SEP_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsTotalReferralSEPModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsTotalReferralSEPModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsTotalReferralSEPsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
