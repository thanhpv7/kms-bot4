...
import {BpjsTotalReferralSEPModelState, BpjsTotalReferralSEPModelAudit} from 'src/app/models/bpjsTotalReferralSEP/bpjs_total_referral_sep.model.state';
import {BpjsTotalReferralSEPModel} from 'src/app/models/bpjsTotalReferralSEP/bpjs_total_referral_sep.model';
import {
	getBpjsTotalReferralSEPModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-total-referral-sep-admin-crud-edit',
	templateUrl: './bpjs-total-referral-sep-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-total-referral-sep-admin-crud-edit.component.scss',
...
export class AdminBpjsTotalReferralSEPCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsTotalReferralSEPAudits$: Observable<BpjsTotalReferralSEPModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsTotalReferralSEPModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsTotalReferralSEPModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsTotalReferralSEPModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsTotalReferralSEPModel();
		...
		if (this.targetModelId) {
			this.bpjsTotalReferralSEPAudits$ = this.store.select(getBpjsTotalReferralSEPModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsTotalReferralSEPModel> = {
		...
		this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
			modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_BPJS_TOTAL_REFERRAL_SEP,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsTotalReferralSEPModelWithId, this.targetModelId)
			...
			model = new BpjsTotalReferralSEPModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
			modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_BPJS_TOTAL_REFERRAL_SEP_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
				modelAction.BpjsTotalReferralSEPModelActionTypes.CREATE_BPJS_TOTAL_REFERRAL_SEP,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsTotalReferralSEPAction(
				modelAction.BpjsTotalReferralSEPModelActionTypes.UPDATE_BPJS_TOTAL_REFERRAL_SEP,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsTotalReferralSEPModel = new BpjsTotalReferralSEPModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsTotalReferralSEPModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsTotalReferralSEPModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsTotalReferralSEPModel.getProps();
		this.modelRelations = BpjsTotalReferralSEPModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsTotalReferralSEPModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsTotalReferralSEPModel {
		let updatedModel: BpjsTotalReferralSEPModel = new BpjsTotalReferralSEPModel(model);
