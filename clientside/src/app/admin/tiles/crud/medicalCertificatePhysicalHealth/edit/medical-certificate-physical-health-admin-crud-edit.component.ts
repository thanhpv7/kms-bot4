...
import * as modelAction from 'src/app/models/medicalCertificatePhysicalHealth/medical_certificate_physical_health.model.action';
import {MedicalCertificatePhysicalHealthModelState, MedicalCertificatePhysicalHealthModelAudit} from 'src/app/models/medicalCertificatePhysicalHealth/medical_certificate_physical_health.model.state';
...
import {
	getMedicalCertificatePhysicalHealthModelAuditsByEntityId,
	getMedicalCertificatePhysicalHealthModelWithId,
	getCountMedicalCertificatePhysicalHealthModels,
...
import {CrudTileMode} from '../medical-certificate-physical-health-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-physical-health-admin-crud-edit',
	templateUrl: './medical-certificate-physical-health-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-physical-health-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificatePhysicalHealthCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'e47e40e2-69b9-486f-a3ca-d925ae6ba4f7';
	 ...
	 */
	medicalCertificatePhysicalHealthAudits$: Observable<MedicalCertificatePhysicalHealthModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificatePhysicalHealth';
	...
	medicalStaffOppositeRelation = 'medicalStaffOfPhysicalHealths';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificatePhysicalHealthModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificatePhysicalHealthModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificatePhysicalHealthModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalCertificatePhysicalHealthModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificatePhysicalHealthModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificatePhysicalHealthModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificatePhysicalHealthModel();
		...
		if (this.targetModelId) {
			this.medicalCertificatePhysicalHealthAudits$ = this.store.select(getMedicalCertificatePhysicalHealthModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificatePhysicalHealthModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
			modelAction.MedicalCertificatePhysicalHealthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificatePhysicalHealthModelWithId, this.targetModelId)
			...
			model = new MedicalCertificatePhysicalHealthModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-physical-health');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
			modelAction.MedicalCertificatePhysicalHealthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificatePhysicalHealthModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificatePhysicalHealthAction(
						modelAction.MedicalCertificatePhysicalHealthModelActionTypes.COUNT_MEDICAL_CERTIFICATE_PHYSICAL_HEALTHS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
				modelAction.MedicalCertificatePhysicalHealthModelActionTypes.CREATE_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
				modelAction.MedicalCertificatePhysicalHealthModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificatePhysicalHealthModel = new MedicalCertificatePhysicalHealthModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificatePhysicalHealthModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificatePhysicalHealthModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificatePhysicalHealthModel.getProps();
		this.modelRelations = MedicalCertificatePhysicalHealthModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificatePhysicalHealthModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificatePhysicalHealthModel {
		let updatedModel: MedicalCertificatePhysicalHealthModel = new MedicalCertificatePhysicalHealthModel(model);
