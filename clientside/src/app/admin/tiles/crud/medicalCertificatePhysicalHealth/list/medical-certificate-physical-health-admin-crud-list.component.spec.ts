...
import {AdminMedicalCertificatePhysicalHealthCrudListComponent} from './medical-certificate-physical-health-admin-crud-list.component';
...
import {AdminMedicalCertificatePhysicalHealthCrudModule} from '../medical-certificate-physical-health-admin-crud.module';
import {
	MedicalCertificatePhysicalHealthModelState,
	initialState as initialMedicalCertificatePhysicalHealthModelState
} from 'src/app/models/medicalCertificatePhysicalHealth/medical_certificate_physical_health.model.state';
 ...
 */
describe('Medical Certificate Physical Health Crud List Tile Component', () => {

	let fixture;
	let medicalCertificatePhysicalHealthCrudListComponent: AdminMedicalCertificatePhysicalHealthCrudListComponent;

	let store: MockStore<{ model: MedicalCertificatePhysicalHealthModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalCertificatePhysicalHealthModelState: MedicalCertificatePhysicalHealthModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalCertificatePhysicalHealthModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e47e40e2-69b9-486f-a3ca-d925ae6ba4f7';
	const searchContent = 'text';

	const searchableFieldCount = MedicalCertificatePhysicalHealthModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-certificate-physical-health-admin-crud',
		urls: ['medical-certificate-physical-health-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalCertificatePhysicalHealthDataFactory = new MedicalCertificatePhysicalHealthDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalCertificatePhysicalHealthDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalCertificatePhysicalHealthModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalCertificatePhysicalHealthModelState = initialMedicalCertificatePhysicalHealthModelState;
		store.setState({model: medicalCertificatePhysicalHealthModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalCertificatePhysicalHealthCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalCertificatePhysicalHealthCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalCertificatePhysicalHealthCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalCertificatePhysicalHealthCrudListComponent);
			medicalCertificatePhysicalHealthCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalCertificatePhysicalHealthCrudListComponent.medicalCertificatePhysicalHealthsCount).toBe(defaultData.length);
			...
			expect(medicalCertificatePhysicalHealthCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalCertificatePhysicalHealthCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalCertificatePhysicalHealthDataFactory.createAll(filteredDataSize);
			...
			expect(medicalCertificatePhysicalHealthCrudListComponent.medicalCertificatePhysicalHealthsCount).toBe(filteredDataSize);
			expect(medicalCertificatePhysicalHealthCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalCertificatePhysicalHealthCrudListComponent.pageIndex).toBe(0);
			expect(medicalCertificatePhysicalHealthCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalCertificatePhysicalHealthCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalCertificatePhysicalHealthCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalCertificatePhysicalHealthCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
