...
import {MedicalCertificatePhysicalHealthModelState} from 'src/app/models/medicalCertificatePhysicalHealth/medical_certificate_physical_health.model.state';
...
import {
	getMedicalCertificatePhysicalHealthCollectionModels,
	getMedicalCertificatePhysicalHealthCollectionCount
...
@Component({
	selector: 'cb-medical-certificate-physical-health-admin-crud-list',
	templateUrl: './medical-certificate-physical-health-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-physical-health-admin-crud-list.component.scss',
...
export class AdminMedicalCertificatePhysicalHealthCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificatePhysicalHealthModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificatePhysicalHealthModel.searchFields,
	...
	/**
	 * List of Medical Certificate Physical Healths that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificatePhysicalHealths: Observable<MedicalCertificatePhysicalHealthModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificatePhysicalHealthsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificatePhysicalHealthsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificatePhysicalHealthsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
				modelAction.MedicalCertificatePhysicalHealthModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
				modelAction.MedicalCertificatePhysicalHealthModelActionTypes.FETCH_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificatePhysicalHealths = this.store.select(getMedicalCertificatePhysicalHealthCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificatePhysicalHealthCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificatePhysicalHealthsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificatePhysicalHealthModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificatePhysicalHealthModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificatePhysicalHealthModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH;
		} else {
			actionType = modelAction.MedicalCertificatePhysicalHealthModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
				modelAction.MedicalCertificatePhysicalHealthModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_PHYSICAL_HEALTHS,
				{
					file: file,
				},
				[new modelAction.MedicalCertificatePhysicalHealthAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificatephysicalhealth/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificatePhysicalHealthModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Physical Health entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Physical Health entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificatePhysicalHealthModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificatePhysicalHealthModelActionTypes.DELETE_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificatePhysicalHealthModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH;
						} else {
							actionType = modelAction.MedicalCertificatePhysicalHealthModelActionTypes.DELETE_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificatePhysicalHealthAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH and DELETE_MEDICAL_CERTIFICATE_PHYSICAL_HEALTH_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificatePhysicalHealthModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificatePhysicalHealthsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
