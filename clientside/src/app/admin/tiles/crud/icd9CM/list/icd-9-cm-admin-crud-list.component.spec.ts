...
import {AdminIcd9CMCrudListComponent} from './icd-9-cm-admin-crud-list.component';
...
import {AdminIcd9CMCrudModule} from '../icd-9-cm-admin-crud.module';
import {
	Icd9CMModelState,
 ...
 */
describe('ICD 9 CM Crud List Tile Component', () => {

	let fixture;
	let icd9CMCrudListComponent: AdminIcd9CMCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<Icd9CMModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '76e07007-40f0-4bf8-a299-71da61a8c96c';
	const searchContent = 'text';

	const searchableFieldCount = Icd9CMModel.searchFields.length;

	const routerState: RouterState = {
		url: 'icd-9-cm-admin-crud',
		urls: ['icd-9-cm-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const icd9CMDataFactory = new Icd9CMDataFactory();

	const defaultDataSize = 30;
	const defaultData = icd9CMDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getIcd9CMCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getIcd9CMCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminIcd9CMCrudListComponent);
			icd9CMCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(icd9CMCrudListComponent.icd9CMsCount).toBe(defaultData.length);
			...
			expect(icd9CMCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			icd9CMCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = icd9CMDataFactory.createAll(filteredDataSize);
			...
			expect(icd9CMCrudListComponent.icd9CMsCount).toBe(filteredDataSize);
			expect(icd9CMCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(icd9CMCrudListComponent.pageIndex).toBe(0);
			expect(icd9CMCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			icd9CMCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		icd9CMCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		icd9CMCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
