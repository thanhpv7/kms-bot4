...
import * as modelAction from 'src/app/models/icd9CM/icd_9_cm.model.action';
import {Icd9CMModelState} from 'src/app/models/icd9CM/icd_9_cm.model.state';
...
import {CrudTileMode} from '../icd-9-cm-admin-crud.component';
...
@Component({
	selector: 'cb-icd-9-cm-admin-crud-list',
	templateUrl: './icd-9-cm-admin-crud-list.component.html',
	styleUrls: [
		'./icd-9-cm-admin-crud-list.component.scss',
...
export class AdminIcd9CMCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<Icd9CMModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...Icd9CMModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = Icd9CMModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '76e07007-40f0-4bf8-a299-71da61a8c96c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.icd9CMsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.icd9CMsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: Icd9CMModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.Icd9CMAction(
				modelAction.Icd9CMModelActionTypes.INITIALISE_ICD_9_CM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.Icd9CMAction(
				modelAction.Icd9CMModelActionTypes.FETCH_ICD_9_CM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.Icd9CMAction(
						modelAction.Icd9CMModelActionTypes.COUNT_ICD_9_CMS,
						null
					)
				]
			))
		...
		this.icd9CMs = this.store.select(getIcd9CMCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getIcd9CMCollectionCount, this.collectionId).subscribe(
				(count) => this.icd9CMsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: Icd9CMModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.Icd9CMModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.Icd9CMModelActionTypes.EXPORT_ALL_ICD_9_CM;
		} else {
			actionType = modelAction.Icd9CMModelActionTypes.EXPORT_ICD_9_CM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.Icd9CMAction(
				modelAction.Icd9CMModelActionTypes.IMPORT_ICD_9_CMS,
				{
					file: file,
				},
				[new modelAction.Icd9CMAction(
			...
			window.open(environment.API_URL + '/api/icd9cm/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: Icd9CMModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected ICD 9 CM entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected ICD 9 CM entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.Icd9CMModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.Icd9CMModelActionTypes.DELETE_ICD_9_CM;
						} else if (usingSelected) {
							actionType = modelAction.Icd9CMModelActionTypes.DELETE_ALL_ICD_9_CM;
						} else {
							actionType = modelAction.Icd9CMModelActionTypes.DELETE_ICD_9_CM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.Icd9CMAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ICD_9_CM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ICD_9_CM and DELETE_ICD_9_CM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<Icd9CMModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = Icd9CMModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.icd9CMsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
