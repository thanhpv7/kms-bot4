...
import {Icd9CMModelState, Icd9CMModelAudit} from 'src/app/models/icd9CM/icd_9_cm.model.state';
import {Icd9CMModel} from 'src/app/models/icd9CM/icd_9_cm.model';
import {
	getIcd9CMModelAuditsByEntityId,
...
@Component({
	selector: 'cb-icd-9-cm-admin-crud-edit',
	templateUrl: './icd-9-cm-admin-crud-edit.component.html',
	styleUrls: [
		'./icd-9-cm-admin-crud-edit.component.scss',
...
export class AdminIcd9CMCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	icd9CMAudits$: Observable<Icd9CMModelAudit[]>;
	...
	diagnosesAndProceduresOppositeRelation = 'masterICD9CM';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = Icd9CMModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountIcd9CMModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new Icd9CMModel();
		...
		if (this.targetModelId) {
			this.icd9CMAudits$ = this.store.select(getIcd9CMModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<Icd9CMModel> = {
		...
		this.store.dispatch(new modelAction.Icd9CMAction(
			modelAction.Icd9CMModelActionTypes.FETCH_ICD_9_CM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getIcd9CMModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.Icd9CMAction(
			modelAction.Icd9CMModelActionTypes.FETCH_ICD_9_CM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<Icd9CMModel> = {
				...
				queryParams: {
					persistRelations: this.diagnosesAndProceduresMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.Icd9CMAction(
				modelAction.Icd9CMModelActionTypes.CREATE_ICD_9_CM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.Icd9CMAction(
				modelAction.Icd9CMModelActionTypes.UPDATE_ICD_9_CM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: Icd9CMModel = new Icd9CMModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getIcd9CMModelWithId, this.targetModelId).subscribe(model => {
					let differences = new Icd9CMModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = Icd9CMModel.getProps();
		this.modelRelations = Icd9CMModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: Icd9CMModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): Icd9CMModel {
		let updatedModel: Icd9CMModel = new Icd9CMModel(model);
