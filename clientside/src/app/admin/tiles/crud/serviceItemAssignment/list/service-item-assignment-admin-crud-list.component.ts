...
import {ServiceItemAssignmentModelState} from 'src/app/models/serviceItemAssignment/service_item_assignment.model.state';
...
import {
	...
	getServiceItemAssignmentCollectionCount
...
@Component({
	selector: 'cb-service-item-assignment-admin-crud-list',
	templateUrl: './service-item-assignment-admin-crud-list.component.html',
	styleUrls: [
		'./service-item-assignment-admin-crud-list.component.scss',
...
export class AdminServiceItemAssignmentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ServiceItemAssignmentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ServiceItemAssignmentModel.searchFields,
	...
	/**
	 * List of Service Item Assignments that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	serviceItemAssignments: Observable<ServiceItemAssignmentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	serviceItemAssignmentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.serviceItemAssignmentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.serviceItemAssignmentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
				modelAction.ServiceItemAssignmentModelActionTypes.INITIALISE_SERVICE_ITEM_ASSIGNMENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
				modelAction.ServiceItemAssignmentModelActionTypes.FETCH_SERVICE_ITEM_ASSIGNMENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.serviceItemAssignments = this.store.select(getServiceItemAssignmentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getServiceItemAssignmentCollectionCount, this.collectionId).subscribe(
				(count) => this.serviceItemAssignmentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ServiceItemAssignmentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ServiceItemAssignmentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ServiceItemAssignmentModelActionTypes.EXPORT_ALL_SERVICE_ITEM_ASSIGNMENT;
		} else {
			actionType = modelAction.ServiceItemAssignmentModelActionTypes.EXPORT_SERVICE_ITEM_ASSIGNMENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
				modelAction.ServiceItemAssignmentModelActionTypes.IMPORT_SERVICE_ITEM_ASSIGNMENTS,
				{
					file: file,
				},
				[new modelAction.ServiceItemAssignmentAction(
			...
			window.open(environment.API_URL + '/api/serviceitemassignment/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ServiceItemAssignmentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Service Item Assignment entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Service Item Assignment entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ServiceItemAssignmentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ServiceItemAssignmentModelActionTypes.DELETE_SERVICE_ITEM_ASSIGNMENT;
						} else if (usingSelected) {
							actionType = modelAction.ServiceItemAssignmentModelActionTypes.DELETE_ALL_SERVICE_ITEM_ASSIGNMENT;
						} else {
							actionType = modelAction.ServiceItemAssignmentModelActionTypes.DELETE_SERVICE_ITEM_ASSIGNMENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SERVICE_ITEM_ASSIGNMENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SERVICE_ITEM_ASSIGNMENT and DELETE_SERVICE_ITEM_ASSIGNMENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ServiceItemAssignmentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.serviceItemAssignmentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
