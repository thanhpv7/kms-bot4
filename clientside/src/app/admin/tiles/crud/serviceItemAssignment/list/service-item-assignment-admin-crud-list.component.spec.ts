...
import {AdminServiceItemAssignmentCrudListComponent} from './service-item-assignment-admin-crud-list.component';
...
import {AdminServiceItemAssignmentCrudModule} from '../service-item-assignment-admin-crud.module';
import {
 ...
 */
describe('Service Item Assignment Crud List Tile Component', () => {

	let fixture;
	let serviceItemAssignmentCrudListComponent: AdminServiceItemAssignmentCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ServiceItemAssignmentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e69263ce-50a0-492f-bf77-d4c7303fca2d';
	const searchContent = 'text';

	const searchableFieldCount = ServiceItemAssignmentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'service-item-assignment-admin-crud',
		urls: ['service-item-assignment-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const serviceItemAssignmentDataFactory = new ServiceItemAssignmentDataFactory();

	const defaultDataSize = 30;
	const defaultData = serviceItemAssignmentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getServiceItemAssignmentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getServiceItemAssignmentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminServiceItemAssignmentCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminServiceItemAssignmentCrudListComponent);
			serviceItemAssignmentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(serviceItemAssignmentCrudListComponent.serviceItemAssignmentsCount).toBe(defaultData.length);
			...
			expect(serviceItemAssignmentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			serviceItemAssignmentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = serviceItemAssignmentDataFactory.createAll(filteredDataSize);
			...
			expect(serviceItemAssignmentCrudListComponent.serviceItemAssignmentsCount).toBe(filteredDataSize);
			expect(serviceItemAssignmentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(serviceItemAssignmentCrudListComponent.pageIndex).toBe(0);
			expect(serviceItemAssignmentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			serviceItemAssignmentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		serviceItemAssignmentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		serviceItemAssignmentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
