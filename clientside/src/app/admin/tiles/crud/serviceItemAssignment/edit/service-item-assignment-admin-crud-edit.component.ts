...
import * as modelAction from 'src/app/models/serviceItemAssignment/service_item_assignment.model.action';
import {ServiceItemAssignmentModelState, ServiceItemAssignmentModelAudit} from 'src/app/models/serviceItemAssignment/service_item_assignment.model.state';
import {ServiceItemAssignmentModel} from 'src/app/models/serviceItemAssignment/service_item_assignment.model';
import {
	getServiceItemAssignmentModelAuditsByEntityId,
...
import {CrudTileMode} from '../service-item-assignment-admin-crud.component';
...
@Component({
	selector: 'cb-service-item-assignment-admin-crud-edit',
	templateUrl: './service-item-assignment-admin-crud-edit.component.html',
	styleUrls: [
		'./service-item-assignment-admin-crud-edit.component.scss',
...
export class AdminServiceItemAssignmentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'e69263ce-50a0-492f-bf77-d4c7303fca2d';
	 ...
	 */
	serviceItemAssignmentAudits$: Observable<ServiceItemAssignmentModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Service Item and Service Item Assignments here] off begin
	serviceItemMultiCrudActive: boolean = false;
	serviceItemDisplayName = 'Service Item';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Service Item and Service Item Assignments here] end
	serviceItemOppositeRelation = 'serviceItemAssignments';
	...
	tariffPackageOppositeRelation = 'serviceItemAssignments';
	 ...
	 */
	modelProperties: ModelProperty[] = ServiceItemAssignmentModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ServiceItemAssignmentModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ServiceItemAssignmentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ServiceItemAssignmentModel;
	...
	constructor(
		private readonly store: Store<{ model: ServiceItemAssignmentModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountServiceItemAssignmentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ServiceItemAssignmentModel();
		...
		if (this.targetModelId) {
			this.serviceItemAssignmentAudits$ = this.store.select(getServiceItemAssignmentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ServiceItemAssignmentModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
			modelAction.ServiceItemAssignmentModelActionTypes.FETCH_SERVICE_ITEM_ASSIGNMENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getServiceItemAssignmentModelWithId, this.targetModelId)
			...
			model = new ServiceItemAssignmentModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('service-item-assignment');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
			modelAction.ServiceItemAssignmentModelActionTypes.FETCH_SERVICE_ITEM_ASSIGNMENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ServiceItemAssignmentModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.serviceItemMultiCrudActive || this.tariffPackageMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.ServiceItemAssignmentAction(
						modelAction.ServiceItemAssignmentModelActionTypes.COUNT_SERVICE_ITEM_ASSIGNMENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
				modelAction.ServiceItemAssignmentModelActionTypes.CREATE_SERVICE_ITEM_ASSIGNMENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ServiceItemAssignmentAction(
				modelAction.ServiceItemAssignmentModelActionTypes.UPDATE_SERVICE_ITEM_ASSIGNMENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ServiceItemAssignmentModel = new ServiceItemAssignmentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getServiceItemAssignmentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ServiceItemAssignmentModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ServiceItemAssignmentModel.getProps();
		this.modelRelations = ServiceItemAssignmentModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ServiceItemAssignmentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ServiceItemAssignmentModel {
		let updatedModel: ServiceItemAssignmentModel = new ServiceItemAssignmentModel(model);
