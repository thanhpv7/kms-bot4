...
import {DiagnosesAndProceduresModelState} from 'src/app/models/diagnosesAndProcedures/diagnoses_and_procedures.model.state';
...
import {
	...
	getDiagnosesAndProceduresCollectionCount
...
import {CrudTileMode} from '../diagnoses-and-procedures-admin-crud.component';
...
@Component({
	selector: 'cb-diagnoses-and-procedures-admin-crud-list',
	templateUrl: './diagnoses-and-procedures-admin-crud-list.component.html',
	styleUrls: [
		'./diagnoses-and-procedures-admin-crud-list.component.scss',
...
export class AdminDiagnosesAndProceduresCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DiagnosesAndProceduresModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DiagnosesAndProceduresModel.searchFields,
	...
	/**
	 * List of Diagnoses and Proceduress that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	diagnosesAndProceduress: Observable<DiagnosesAndProceduresModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	diagnosesAndProceduressCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.diagnosesAndProceduressCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.diagnosesAndProceduressCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosesAndProceduresAction(
				modelAction.DiagnosesAndProceduresModelActionTypes.INITIALISE_DIAGNOSES_AND_PROCEDURES_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosesAndProceduresAction(
				modelAction.DiagnosesAndProceduresModelActionTypes.FETCH_DIAGNOSES_AND_PROCEDURES_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.diagnosesAndProceduress = this.store.select(getDiagnosesAndProceduresCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDiagnosesAndProceduresCollectionCount, this.collectionId).subscribe(
				(count) => this.diagnosesAndProceduressCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DiagnosesAndProceduresModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DiagnosesAndProceduresModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DiagnosesAndProceduresModelActionTypes.EXPORT_ALL_DIAGNOSES_AND_PROCEDURES;
		} else {
			actionType = modelAction.DiagnosesAndProceduresModelActionTypes.EXPORT_DIAGNOSES_AND_PROCEDURES_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DiagnosesAndProceduresAction(
				modelAction.DiagnosesAndProceduresModelActionTypes.IMPORT_DIAGNOSES_AND_PROCEDURESS,
				{
					file: file,
				},
				[new modelAction.DiagnosesAndProceduresAction(
			...
			window.open(environment.API_URL + '/api/diagnosesandprocedures/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DiagnosesAndProceduresModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Diagnoses and Procedures entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Diagnoses and Procedures entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DiagnosesAndProceduresModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DiagnosesAndProceduresModelActionTypes.DELETE_DIAGNOSES_AND_PROCEDURES;
						} else if (usingSelected) {
							actionType = modelAction.DiagnosesAndProceduresModelActionTypes.DELETE_ALL_DIAGNOSES_AND_PROCEDURES;
						} else {
							actionType = modelAction.DiagnosesAndProceduresModelActionTypes.DELETE_DIAGNOSES_AND_PROCEDURES_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DiagnosesAndProceduresAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DIAGNOSES_AND_PROCEDURES when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DIAGNOSES_AND_PROCEDURES and DELETE_DIAGNOSES_AND_PROCEDURES_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DiagnosesAndProceduresModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.diagnosesAndProceduressCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
