...
import {AdminDiagnosesAndProceduresCrudListComponent} from './diagnoses-and-procedures-admin-crud-list.component';
...
import {AdminDiagnosesAndProceduresCrudModule} from '../diagnoses-and-procedures-admin-crud.module';
import {
 ...
 */
describe('Diagnoses and Procedures Crud List Tile Component', () => {

	let fixture;
	let diagnosesAndProceduresCrudListComponent: AdminDiagnosesAndProceduresCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<DiagnosesAndProceduresModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5c9fcfb8-1453-4986-92a1-8b320c41ec11';
	const searchContent = 'text';

	const searchableFieldCount = DiagnosesAndProceduresModel.searchFields.length;

	const routerState: RouterState = {
		url: 'diagnoses-and-procedures-admin-crud',
		urls: ['diagnoses-and-procedures-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const diagnosesAndProceduresDataFactory = new DiagnosesAndProceduresDataFactory();

	const defaultDataSize = 30;
	const defaultData = diagnosesAndProceduresDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getDiagnosesAndProceduresCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDiagnosesAndProceduresCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminDiagnosesAndProceduresCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminDiagnosesAndProceduresCrudListComponent);
			diagnosesAndProceduresCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(diagnosesAndProceduresCrudListComponent.diagnosesAndProceduressCount).toBe(defaultData.length);
			...
			expect(diagnosesAndProceduresCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			diagnosesAndProceduresCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = diagnosesAndProceduresDataFactory.createAll(filteredDataSize);
			...
			expect(diagnosesAndProceduresCrudListComponent.diagnosesAndProceduressCount).toBe(filteredDataSize);
			expect(diagnosesAndProceduresCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(diagnosesAndProceduresCrudListComponent.pageIndex).toBe(0);
			expect(diagnosesAndProceduresCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			diagnosesAndProceduresCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		diagnosesAndProceduresCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		diagnosesAndProceduresCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
