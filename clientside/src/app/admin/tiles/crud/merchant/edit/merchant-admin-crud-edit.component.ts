...
import * as modelAction from 'src/app/models/merchant/merchant.model.action';
import {MerchantModelState, MerchantModelAudit} from 'src/app/models/merchant/merchant.model.state';
import {MerchantModel} from 'src/app/models/merchant/merchant.model';
import {
	getMerchantModelAuditsByEntityId,
...
import * as merchantSurchargeAndFeeModelAction from 'src/app/models/merchantSurchargeAndFee/merchant_surcharge_and_fee.model.action';
import {
	getMerchantSurchargeAndFeeCollectionModels, getMerchantSurchargeAndFeeModelWithId,
...
import {CrudTileMode} from '../merchant-admin-crud.component';
...
@Component({
	selector: 'cb-merchant-admin-crud-edit',
	templateUrl: './merchant-admin-crud-edit.component.html',
	styleUrls: [
		'./merchant-admin-crud-edit.component.scss',
...
export class AdminMerchantCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '9af0b29a-1490-447e-92c4-14c3723f8996';
	 ...
	 */
	merchantAudits$: Observable<MerchantModelAudit[]>;
	...
	cashReceiptsOppositeRelation = 'merchant';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Merchant and Merchant Surcharge and Fees here] off begin
	merchantSurchargeAndFeesMultiCrudActive: boolean = false;
	merchantSurchargeAndFeesDisplayName = 'Merchant Surcharge and Fees';
	merchantSurchargeAndFeesCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Merchant and Merchant Surcharge and Fees here] end
	merchantSurchargeAndFeesOppositeRelation = 'merchant';
	merchantSurchargeAndFeesRelations: ModelRelation[] = this.merchantSurchargeAndFeesMultiCrudActive ? MerchantSurchargeAndFeeModel.getRelations()
		.map(relation => {
			if (relation.id === this.merchantSurchargeAndFeesOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = MerchantModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MerchantModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MerchantModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'merchantSurchargeAndFees' && this.merchantSurchargeAndFeesMultiCrudActive) {
					exp.fields = MerchantSurchargeAndFeeModel.getProps().map(prop => {
						return prop.name;
					});
				}
	...
	constructor(
		private readonly store: Store<{ model: MerchantModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMerchantModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MerchantModel();
		...
		if (this.targetModelId) {
			this.merchantAudits$ = this.store.select(getMerchantModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MerchantModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MerchantAction(
			modelAction.MerchantModelActionTypes.FETCH_MERCHANT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMerchantModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MerchantAction(
			modelAction.MerchantModelActionTypes.FETCH_MERCHANT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MerchantModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.cashReceiptsMultiCrudActive || this.merchantSurchargeAndFeesMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MerchantAction(
						modelAction.MerchantModelActionTypes.COUNT_MERCHANTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MerchantAction(
				modelAction.MerchantModelActionTypes.CREATE_MERCHANT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MerchantAction(
				modelAction.MerchantModelActionTypes.UPDATE_MERCHANT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MerchantModel = new MerchantModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMerchantModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MerchantModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.merchantSurchargeAndFeesDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MerchantSurchargeAndFeeModel.getProps(), this.merchantSurchargeAndFeesRelations, this.isViewOnly),
					new MerchantSurchargeAndFeeModel(),
					MerchantSurchargeAndFeeModel,
					this.merchantSurchargeAndFeesDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.merchantSurchargeAndFeesRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MerchantModel.getProps();
		this.modelRelations = MerchantModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'merchantSurchargeAndFees';
		}).hideElement = this.merchantSurchargeAndFeesMultiCrudActive || this.modelRelations.find(rel => rel.id === 'merchantSurchargeAndFees').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.merchantSurchargeAndFeesMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.merchantSurchargeAndFeesDisplayName, many: true});
			this.store.dispatch(new merchantSurchargeAndFeeModelAction.MerchantSurchargeAndFeeAction(
				merchantSurchargeAndFeeModelAction.MerchantSurchargeAndFeeModelActionTypes.INITIALISE_MERCHANT_SURCHARGE_AND_FEE_COLLECTION_STATE,
				{
					collectionId: this.merchantSurchargeAndFeesCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getMerchantSurchargeAndFeeCollectionModels, this.merchantSurchargeAndFeesCollectionId).subscribe(models => {
				models.forEach(merchantSurchargeAndFees => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(MerchantSurchargeAndFeeModel.getProps(), this.merchantSurchargeAndFeesRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.merchantSurchargeAndFeesIds && this.targetModel.merchantSurchargeAndFeesIds.length > 0 && this.merchantSurchargeAndFeesMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				MerchantSurchargeAndFeeModel,
				this.merchantSurchargeAndFeesOppositeRelation,
				this.merchantSurchargeAndFeesCollectionId,
				this.targetModel.merchantSurchargeAndFeesIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: MerchantModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MerchantModel {
		let updatedModel: MerchantModel = new MerchantModel(model);
		...
		updatedModel.merchantSurchargeAndFees = [];
		if (this.merchantSurchargeAndFeesMultiCrudActive) {
			if (event.additionalEntities.merchantSurchargeAndFees) {
				event.additionalEntities.merchantSurchargeAndFees.forEach(entity => {
					updatedModel.merchantSurchargeAndFees.push(entity);
				});
			}

			if (!updatedModel.merchantSurchargeAndFees.length) {
				updatedModel.merchantSurchargeAndFeesIds = [];
			}
		}
		return updatedModel;
	}
