...
import {MerchantModelState} from 'src/app/models/merchant/merchant.model.state';
...
@Component({
	selector: 'cb-merchant-admin-crud-list',
	templateUrl: './merchant-admin-crud-list.component.html',
	styleUrls: [
		'./merchant-admin-crud-list.component.scss',
...
export class AdminMerchantCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MerchantModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MerchantModel.searchFields,
	...
	/**
	 * List of Merchants that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	merchants: Observable<MerchantModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	merchantsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.merchantsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.merchantsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MerchantAction(
				modelAction.MerchantModelActionTypes.INITIALISE_MERCHANT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MerchantAction(
				modelAction.MerchantModelActionTypes.FETCH_MERCHANT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.merchants = this.store.select(getMerchantCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMerchantCollectionCount, this.collectionId).subscribe(
				(count) => this.merchantsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MerchantModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MerchantModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MerchantModelActionTypes.EXPORT_ALL_MERCHANT;
		} else {
			actionType = modelAction.MerchantModelActionTypes.EXPORT_MERCHANT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MerchantAction(
				modelAction.MerchantModelActionTypes.IMPORT_MERCHANTS,
				{
					file: file,
				},
				[new modelAction.MerchantAction(
			...
			window.open(environment.API_URL + '/api/merchant/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MerchantModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Merchant entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Merchant entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MerchantModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MerchantModelActionTypes.DELETE_MERCHANT;
						} else if (usingSelected) {
							actionType = modelAction.MerchantModelActionTypes.DELETE_ALL_MERCHANT;
						} else {
							actionType = modelAction.MerchantModelActionTypes.DELETE_MERCHANT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MerchantAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MERCHANT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MERCHANT and DELETE_MERCHANT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MerchantModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.merchantsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
