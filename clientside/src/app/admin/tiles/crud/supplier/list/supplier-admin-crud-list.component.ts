...
import {SupplierModelState} from 'src/app/models/supplier/supplier.model.state';
...
import {CrudTileMode} from '../supplier-admin-crud.component';
...
@Component({
	selector: 'cb-supplier-admin-crud-list',
	templateUrl: './supplier-admin-crud-list.component.html',
	styleUrls: [
		'./supplier-admin-crud-list.component.scss',
...
export class AdminSupplierCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SupplierModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SupplierModel.searchFields,
	...
	/**
	 * List of Suppliers that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.suppliersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.suppliersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierAction(
				modelAction.SupplierModelActionTypes.INITIALISE_SUPPLIER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierAction(
				modelAction.SupplierModelActionTypes.FETCH_SUPPLIER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.suppliers = this.store.select(getSupplierCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSupplierCollectionCount, this.collectionId).subscribe(
				(count) => this.suppliersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SupplierModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SupplierModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SupplierModelActionTypes.EXPORT_ALL_SUPPLIER;
		} else {
			actionType = modelAction.SupplierModelActionTypes.EXPORT_SUPPLIER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SupplierAction(
				modelAction.SupplierModelActionTypes.IMPORT_SUPPLIERS,
				{
					file: file,
				},
				[new modelAction.SupplierAction(
			...
			window.open(environment.API_URL + '/api/supplier/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SupplierModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Supplier entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Supplier entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SupplierModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SupplierModelActionTypes.DELETE_SUPPLIER;
						} else if (usingSelected) {
							actionType = modelAction.SupplierModelActionTypes.DELETE_ALL_SUPPLIER;
						} else {
							actionType = modelAction.SupplierModelActionTypes.DELETE_SUPPLIER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SupplierAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SUPPLIER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SUPPLIER and DELETE_SUPPLIER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SupplierModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.suppliersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
