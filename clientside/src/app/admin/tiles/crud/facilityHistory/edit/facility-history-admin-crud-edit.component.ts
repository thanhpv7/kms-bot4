...
import {FacilityHistoryModelState, FacilityHistoryModelAudit} from 'src/app/models/facilityHistory/facility_history.model.state';
import {FacilityHistoryModel} from 'src/app/models/facilityHistory/facility_history.model';
import {
	getFacilityHistoryModelAuditsByEntityId,
	getFacilityHistoryModelWithId,
	getCountFacilityHistoryModels,
...
@Component({
	selector: 'cb-facility-history-admin-crud-edit',
	templateUrl: './facility-history-admin-crud-edit.component.html',
	styleUrls: [
		'./facility-history-admin-crud-edit.component.scss',
...
export class AdminFacilityHistoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	facilityHistoryAudits$: Observable<FacilityHistoryModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = FacilityHistoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: FacilityHistoryModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountFacilityHistoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new FacilityHistoryModel();
		...
		if (this.targetModelId) {
			this.facilityHistoryAudits$ = this.store.select(getFacilityHistoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<FacilityHistoryModel> = {
		...
		this.store.dispatch(new modelAction.FacilityHistoryAction(
			modelAction.FacilityHistoryModelActionTypes.FETCH_FACILITY_HISTORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getFacilityHistoryModelWithId, this.targetModelId)
			...
			model = new FacilityHistoryModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.FacilityHistoryAction(
			modelAction.FacilityHistoryModelActionTypes.FETCH_FACILITY_HISTORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.FacilityHistoryAction(
				modelAction.FacilityHistoryModelActionTypes.CREATE_FACILITY_HISTORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.FacilityHistoryAction(
				modelAction.FacilityHistoryModelActionTypes.UPDATE_FACILITY_HISTORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: FacilityHistoryModel = new FacilityHistoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getFacilityHistoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new FacilityHistoryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = FacilityHistoryModel.getProps();
		this.modelRelations = FacilityHistoryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: FacilityHistoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): FacilityHistoryModel {
		let updatedModel: FacilityHistoryModel = new FacilityHistoryModel(model);
