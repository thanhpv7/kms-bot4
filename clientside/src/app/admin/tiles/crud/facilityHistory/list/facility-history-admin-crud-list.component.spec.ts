...
import {AdminFacilityHistoryCrudListComponent} from './facility-history-admin-crud-list.component';
import {FacilityHistoryDataFactory} from 'src/app/lib/utils/factories/facility-history-data-factory';
import {FacilityHistoryModel} from 'src/app/models/facilityHistory/facility_history.model';
import {AdminFacilityHistoryCrudModule} from '../facility-history-admin-crud.module';
import {
	FacilityHistoryModelState,
	initialState as initialFacilityHistoryModelState
} from 'src/app/models/facilityHistory/facility_history.model.state';
 ...
 */
describe('Facility History Crud List Tile Component', () => {

	let fixture;
	let facilityHistoryCrudListComponent: AdminFacilityHistoryCrudListComponent;

	let store: MockStore<{ model: FacilityHistoryModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let facilityHistoryModelState: FacilityHistoryModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<FacilityHistoryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e8ea5d0e-6a02-4c77-a5d7-0a8f1427cfb1';
	const searchContent = 'text';

	const searchableFieldCount = FacilityHistoryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'facility-history-admin-crud',
		urls: ['facility-history-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const facilityHistoryDataFactory = new FacilityHistoryDataFactory();

	const defaultDataSize = 30;
	const defaultData = facilityHistoryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: FacilityHistoryModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		facilityHistoryModelState = initialFacilityHistoryModelState;
		store.setState({model: facilityHistoryModelState});
		...
		spyOn(store, 'select')
			.withArgs(getFacilityHistoryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getFacilityHistoryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminFacilityHistoryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminFacilityHistoryCrudListComponent);
			facilityHistoryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(facilityHistoryCrudListComponent.facilityHistorysCount).toBe(defaultData.length);
			...
			expect(facilityHistoryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			facilityHistoryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = facilityHistoryDataFactory.createAll(filteredDataSize);
			...
			expect(facilityHistoryCrudListComponent.facilityHistorysCount).toBe(filteredDataSize);
			expect(facilityHistoryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(facilityHistoryCrudListComponent.pageIndex).toBe(0);
			expect(facilityHistoryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			facilityHistoryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		facilityHistoryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		facilityHistoryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
