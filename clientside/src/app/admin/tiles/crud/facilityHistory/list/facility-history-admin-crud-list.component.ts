...
import * as modelAction from 'src/app/models/facilityHistory/facility_history.model.action';
import {FacilityHistoryModelState} from 'src/app/models/facilityHistory/facility_history.model.state';
...
import {
	getFacilityHistoryCollectionModels,
	getFacilityHistoryCollectionCount
} from 'src/app/models/facilityHistory/facility_history.model.selector';
...
import {CrudTileMode} from '../facility-history-admin-crud.component';
...
@Component({
	selector: 'cb-facility-history-admin-crud-list',
	templateUrl: './facility-history-admin-crud-list.component.html',
	styleUrls: [
		'./facility-history-admin-crud-list.component.scss',
...
export class AdminFacilityHistoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<FacilityHistoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...FacilityHistoryModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = FacilityHistoryModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'e8ea5d0e-6a02-4c77-a5d7-0a8f1427cfb1';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.facilityHistorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.facilityHistorysCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: FacilityHistoryModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FacilityHistoryAction(
				modelAction.FacilityHistoryModelActionTypes.INITIALISE_FACILITY_HISTORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FacilityHistoryAction(
				modelAction.FacilityHistoryModelActionTypes.FETCH_FACILITY_HISTORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.FacilityHistoryAction(
						modelAction.FacilityHistoryModelActionTypes.COUNT_FACILITY_HISTORYS,
						null
					)
				]
			))
		...
		this.facilityHistorys = this.store.select(getFacilityHistoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getFacilityHistoryCollectionCount, this.collectionId).subscribe(
				(count) => this.facilityHistorysCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('facility-history');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: FacilityHistoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.FacilityHistoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.FacilityHistoryModelActionTypes.EXPORT_ALL_FACILITY_HISTORY;
		} else {
			actionType = modelAction.FacilityHistoryModelActionTypes.EXPORT_FACILITY_HISTORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.FacilityHistoryAction(
				modelAction.FacilityHistoryModelActionTypes.IMPORT_FACILITY_HISTORYS,
				{
					file: file,
				},
				[new modelAction.FacilityHistoryAction(
			...
			window.open(environment.API_URL + '/api/facilityhistory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: FacilityHistoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Facility History entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Facility History entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.FacilityHistoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.FacilityHistoryModelActionTypes.DELETE_FACILITY_HISTORY;
						} else if (usingSelected) {
							actionType = modelAction.FacilityHistoryModelActionTypes.DELETE_ALL_FACILITY_HISTORY;
						} else {
							actionType = modelAction.FacilityHistoryModelActionTypes.DELETE_FACILITY_HISTORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.FacilityHistoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_FACILITY_HISTORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_FACILITY_HISTORY and DELETE_FACILITY_HISTORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<FacilityHistoryModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = FacilityHistoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.facilityHistorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
