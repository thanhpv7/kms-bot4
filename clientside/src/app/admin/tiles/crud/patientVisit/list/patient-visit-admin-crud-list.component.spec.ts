...
import {AdminPatientVisitCrudListComponent} from './patient-visit-admin-crud-list.component';
...
import {AdminPatientVisitCrudModule} from '../patient-visit-admin-crud.module';
import {
	PatientVisitModelState,
	initialState as initialPatientVisitModelState
} from 'src/app/models/patientVisit/patient_visit.model.state';
 ...
 */
describe('Patient Visit Crud List Tile Component', () => {

	let fixture;
	let patientVisitCrudListComponent: AdminPatientVisitCrudListComponent;

	let store: MockStore<{ model: PatientVisitModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientVisitModelState: PatientVisitModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientVisitModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5e5c05b3-d4b7-42aa-9a65-2e116d68c670';
	const searchContent = 'text';

	const searchableFieldCount = PatientVisitModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-visit-admin-crud',
		urls: ['patient-visit-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientVisitDataFactory = new PatientVisitDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientVisitDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientVisitModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientVisitModelState = initialPatientVisitModelState;
		store.setState({model: patientVisitModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientVisitCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientVisitCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPatientVisitCrudListComponent);
			patientVisitCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientVisitCrudListComponent.patientVisitsCount).toBe(defaultData.length);
			...
			expect(patientVisitCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientVisitCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientVisitDataFactory.createAll(filteredDataSize);
			...
			expect(patientVisitCrudListComponent.patientVisitsCount).toBe(filteredDataSize);
			expect(patientVisitCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientVisitCrudListComponent.pageIndex).toBe(0);
			expect(patientVisitCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientVisitCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientVisitCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientVisitCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
