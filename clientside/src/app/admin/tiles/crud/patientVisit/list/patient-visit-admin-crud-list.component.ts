...
import {PatientVisitModelState} from 'src/app/models/patientVisit/patient_visit.model.state';
...
import {
	getPatientVisitCollectionModels,
	getPatientVisitCollectionCount
...
@Component({
	selector: 'cb-patient-visit-admin-crud-list',
	templateUrl: './patient-visit-admin-crud-list.component.html',
	styleUrls: [
		'./patient-visit-admin-crud-list.component.scss',
...
export class AdminPatientVisitCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientVisitModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientVisitModel.searchFields,
	...
	/**
	 * List of Patient Visits that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientVisits: Observable<PatientVisitModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientVisitsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientVisitsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientVisitsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitAction(
				modelAction.PatientVisitModelActionTypes.INITIALISE_PATIENT_VISIT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitAction(
				modelAction.PatientVisitModelActionTypes.FETCH_PATIENT_VISIT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientVisits = this.store.select(getPatientVisitCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientVisitCollectionCount, this.collectionId).subscribe(
				(count) => this.patientVisitsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientVisitModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientVisitModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientVisitModelActionTypes.EXPORT_ALL_PATIENT_VISIT;
		} else {
			actionType = modelAction.PatientVisitModelActionTypes.EXPORT_PATIENT_VISIT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientVisitAction(
				modelAction.PatientVisitModelActionTypes.IMPORT_PATIENT_VISITS,
				{
					file: file,
				},
				[new modelAction.PatientVisitAction(
			...
			window.open(environment.API_URL + '/api/patientvisit/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientVisitModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientVisitModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientVisitModelActionTypes.DELETE_PATIENT_VISIT;
						} else if (usingSelected) {
							actionType = modelAction.PatientVisitModelActionTypes.DELETE_ALL_PATIENT_VISIT;
						} else {
							actionType = modelAction.PatientVisitModelActionTypes.DELETE_PATIENT_VISIT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientVisitAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_VISIT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_VISIT and DELETE_PATIENT_VISIT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientVisitModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientVisitsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
