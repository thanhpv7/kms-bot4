...
import {InventoryBatchCorrectionModelState, InventoryBatchCorrectionModelAudit} from 'src/app/models/inventoryBatchCorrection/inventory_batch_correction.model.state';
...
import {
	getInventoryBatchCorrectionModelAuditsByEntityId,
...
@Component({
	selector: 'cb-inventory-batch-correction-admin-crud-edit',
	templateUrl: './inventory-batch-correction-admin-crud-edit.component.html',
	styleUrls: [
		'./inventory-batch-correction-admin-crud-edit.component.scss',
...
export class AdminInventoryBatchCorrectionCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	inventoryBatchCorrectionAudits$: Observable<InventoryBatchCorrectionModelAudit[]>;
	...
	warehouseOppositeRelation = 'inventoryBatchCorrections';
	...
	inventoryBatchCorrectionItemsOppositeRelation = 'inventoryBatchCorrection';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InventoryBatchCorrectionModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: InventoryBatchCorrectionModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInventoryBatchCorrectionModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InventoryBatchCorrectionModel();
		...
		if (this.targetModelId) {
			this.inventoryBatchCorrectionAudits$ = this.store.select(getInventoryBatchCorrectionModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InventoryBatchCorrectionModel> = {
		...
		this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
			modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInventoryBatchCorrectionModelWithId, this.targetModelId)
			...
			model = new InventoryBatchCorrectionModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
			modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InventoryBatchCorrectionModel> = {
				...
				queryParams: {
					persistRelations: this.warehouseMultiCrudActive || this.inventoryBatchCorrectionItemsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
				modelAction.InventoryBatchCorrectionModelActionTypes.CREATE_INVENTORY_BATCH_CORRECTION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
				modelAction.InventoryBatchCorrectionModelActionTypes.UPDATE_INVENTORY_BATCH_CORRECTION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InventoryBatchCorrectionModel = new InventoryBatchCorrectionModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInventoryBatchCorrectionModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InventoryBatchCorrectionModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InventoryBatchCorrectionModel.getProps();
		this.modelRelations = InventoryBatchCorrectionModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: InventoryBatchCorrectionModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InventoryBatchCorrectionModel {
		let updatedModel: InventoryBatchCorrectionModel = new InventoryBatchCorrectionModel(model);
