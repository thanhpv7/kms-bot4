...
import * as modelAction from 'src/app/models/inventoryBatchCorrection/inventory_batch_correction.model.action';
import {InventoryBatchCorrectionModelState} from 'src/app/models/inventoryBatchCorrection/inventory_batch_correction.model.state';
import {AuthenticationService} from 'src/app/lib/services/authentication/authentication.service';
import {BatchCorrectionTypeEnum, batchCorrectionTypeEnumArray} from 'src/app/enums/batch-correction-type.enum';
...
import {
	...
	getInventoryBatchCorrectionCollectionCount
...
import {CrudTileMode} from '../inventory-batch-correction-admin-crud.component';
...
@Component({
	selector: 'cb-inventory-batch-correction-admin-crud-list',
	templateUrl: './inventory-batch-correction-admin-crud-list.component.html',
	styleUrls: [
		'./inventory-batch-correction-admin-crud-list.component.scss',
...
export class AdminInventoryBatchCorrectionCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InventoryBatchCorrectionModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InventoryBatchCorrectionModel.searchFields,
	...
	/**
	 * List of Inventory Batch Corrections that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	inventoryBatchCorrections: Observable<InventoryBatchCorrectionModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	inventoryBatchCorrectionsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = InventoryBatchCorrectionModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InventoryBatchCorrectionModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'd2212773-e4ae-4eab-bd08-eeaf5b373c62';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.inventoryBatchCorrectionsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.inventoryBatchCorrectionsCount === 0
		},
		{
	...
	filterQuestions: FilterQuestion[] = [
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Transaction Type to Inventory Batch Correction here] off begin
		{
			filterType: FilterQuestionType.dropdown,
			config: {
				options: batchCorrectionTypeEnumArray,
				labelField: 'value',
				valueField: 'key',
				id: 'InventoryBatchCorrectionTransactionTypeFilter',
				label: 'Transaction Type',
			},
			name: 'transactionType',
		},
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Transaction Type to Inventory Batch Correction here] end
	...
	constructor(
		private readonly store: Store<{ model: InventoryBatchCorrectionModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
				modelAction.InventoryBatchCorrectionModelActionTypes.INITIALISE_INVENTORY_BATCH_CORRECTION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
				modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.InventoryBatchCorrectionAction(
						modelAction.InventoryBatchCorrectionModelActionTypes.COUNT_INVENTORY_BATCH_CORRECTIONS,
						null
					)
				]
			))
		...
		this.inventoryBatchCorrections = this.store.select(getInventoryBatchCorrectionCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInventoryBatchCorrectionCollectionCount, this.collectionId).subscribe(
				(count) => this.inventoryBatchCorrectionsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('inventory-batch-correction');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InventoryBatchCorrectionModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InventoryBatchCorrectionModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InventoryBatchCorrectionModelActionTypes.EXPORT_ALL_INVENTORY_BATCH_CORRECTION;
		} else {
			actionType = modelAction.InventoryBatchCorrectionModelActionTypes.EXPORT_INVENTORY_BATCH_CORRECTION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
				modelAction.InventoryBatchCorrectionModelActionTypes.IMPORT_INVENTORY_BATCH_CORRECTIONS,
				{
					file: file,
				},
				[new modelAction.InventoryBatchCorrectionAction(
			...
			window.open(environment.API_URL + '/api/inventorybatchcorrection/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InventoryBatchCorrectionModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Inventory Batch Correction entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Inventory Batch Correction entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InventoryBatchCorrectionModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InventoryBatchCorrectionModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION;
						} else if (usingSelected) {
							actionType = modelAction.InventoryBatchCorrectionModelActionTypes.DELETE_ALL_INVENTORY_BATCH_CORRECTION;
						} else {
							actionType = modelAction.InventoryBatchCorrectionModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InventoryBatchCorrectionAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVENTORY_BATCH_CORRECTION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVENTORY_BATCH_CORRECTION and DELETE_INVENTORY_BATCH_CORRECTION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<InventoryBatchCorrectionModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InventoryBatchCorrectionModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.inventoryBatchCorrectionsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
