...
import {AdminInventoryBatchCorrectionCrudListComponent} from './inventory-batch-correction-admin-crud-list.component';
...
import {AdminInventoryBatchCorrectionCrudModule} from '../inventory-batch-correction-admin-crud.module';
import {
 ...
 */
describe('Inventory Batch Correction Crud List Tile Component', () => {

	let fixture;
	let inventoryBatchCorrectionCrudListComponent: AdminInventoryBatchCorrectionCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<InventoryBatchCorrectionModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd2212773-e4ae-4eab-bd08-eeaf5b373c62';
	const searchContent = 'text';

	const searchableFieldCount = InventoryBatchCorrectionModel.searchFields.length;

	const routerState: RouterState = {
		url: 'inventory-batch-correction-admin-crud',
		urls: ['inventory-batch-correction-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const inventoryBatchCorrectionDataFactory = new InventoryBatchCorrectionDataFactory();

	const defaultDataSize = 30;
	const defaultData = inventoryBatchCorrectionDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getInventoryBatchCorrectionCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInventoryBatchCorrectionCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInventoryBatchCorrectionCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInventoryBatchCorrectionCrudListComponent);
			inventoryBatchCorrectionCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(inventoryBatchCorrectionCrudListComponent.inventoryBatchCorrectionsCount).toBe(defaultData.length);
			...
			expect(inventoryBatchCorrectionCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			inventoryBatchCorrectionCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = inventoryBatchCorrectionDataFactory.createAll(filteredDataSize);
			...
			expect(inventoryBatchCorrectionCrudListComponent.inventoryBatchCorrectionsCount).toBe(filteredDataSize);
			expect(inventoryBatchCorrectionCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(inventoryBatchCorrectionCrudListComponent.pageIndex).toBe(0);
			expect(inventoryBatchCorrectionCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			inventoryBatchCorrectionCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		inventoryBatchCorrectionCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		inventoryBatchCorrectionCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
