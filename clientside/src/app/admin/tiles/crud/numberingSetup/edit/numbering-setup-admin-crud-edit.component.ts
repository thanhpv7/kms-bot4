...
import {NumberingSetupModelState, NumberingSetupModelAudit} from 'src/app/models/numberingSetup/numbering_setup.model.state';
...
import {
	getNumberingSetupModelAuditsByEntityId,
...
@Component({
	selector: 'cb-numbering-setup-admin-crud-edit',
	templateUrl: './numbering-setup-admin-crud-edit.component.html',
	styleUrls: [
		'./numbering-setup-admin-crud-edit.component.scss',
...
export class AdminNumberingSetupCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	numberingSetupAudits$: Observable<NumberingSetupModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NumberingSetupModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NumberingSetupModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNumberingSetupModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NumberingSetupModel();
		...
		if (this.targetModelId) {
			this.numberingSetupAudits$ = this.store.select(getNumberingSetupModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NumberingSetupModel> = {
		...
		this.store.dispatch(new modelAction.NumberingSetupAction(
			modelAction.NumberingSetupModelActionTypes.FETCH_NUMBERING_SETUP,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNumberingSetupModelWithId, this.targetModelId)
			...
			model = new NumberingSetupModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NumberingSetupAction(
			modelAction.NumberingSetupModelActionTypes.FETCH_NUMBERING_SETUP_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NumberingSetupAction(
				modelAction.NumberingSetupModelActionTypes.CREATE_NUMBERING_SETUP,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NumberingSetupAction(
				modelAction.NumberingSetupModelActionTypes.UPDATE_NUMBERING_SETUP,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NumberingSetupModel = new NumberingSetupModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNumberingSetupModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NumberingSetupModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NumberingSetupModel.getProps();
		this.modelRelations = NumberingSetupModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NumberingSetupModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NumberingSetupModel {
		let updatedModel: NumberingSetupModel = new NumberingSetupModel(model);
