...
import * as modelAction from 'src/app/models/numberingSetup/numbering_setup.model.action';
import {NumberingSetupModelState} from 'src/app/models/numberingSetup/numbering_setup.model.state';
import {AuthenticationService} from 'src/app/lib/services/authentication/authentication.service';
import {NumberingTypeEnum, numberingTypeEnumArray} from 'src/app/enums/numbering-type.enum';
import {NumberingSetupModel} from 'src/app/models/numberingSetup/numbering_setup.model';
import {
	getNumberingSetupCollectionModels,
	getNumberingSetupCollectionCount
...
import {CrudTileMode} from '../numbering-setup-admin-crud.component';
...
@Component({
	selector: 'cb-numbering-setup-admin-crud-list',
	templateUrl: './numbering-setup-admin-crud-list.component.html',
	styleUrls: [
		'./numbering-setup-admin-crud-list.component.scss',
...
export class AdminNumberingSetupCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NumberingSetupModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NumberingSetupModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = NumberingSetupModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'e6cac66b-421e-45e7-8067-d2fdaa31bfb1';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.numberingSetupsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.numberingSetupsCount === 0
		},
		{
	...
	filterQuestions: FilterQuestion[] = [
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Numbering Type to Numbering Setup here] off begin
		{
			filterType: FilterQuestionType.dropdown,
			config: {
				options: numberingTypeEnumArray,
				labelField: 'value',
				valueField: 'key',
				id: 'NumberingSetupNumberingTypeFilter',
				label: 'Numbering Type',
			},
			name: 'numberingType',
		},
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Numbering Type to Numbering Setup here] end
	...
	constructor(
		private readonly store: Store<{ model: NumberingSetupModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NumberingSetupAction(
				modelAction.NumberingSetupModelActionTypes.INITIALISE_NUMBERING_SETUP_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NumberingSetupAction(
				modelAction.NumberingSetupModelActionTypes.FETCH_NUMBERING_SETUP_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NumberingSetupAction(
						modelAction.NumberingSetupModelActionTypes.COUNT_NUMBERING_SETUPS,
						null
					)
				]
			))
		...
		this.numberingSetups = this.store.select(getNumberingSetupCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNumberingSetupCollectionCount, this.collectionId).subscribe(
				(count) => this.numberingSetupsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NumberingSetupModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NumberingSetupModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NumberingSetupModelActionTypes.EXPORT_ALL_NUMBERING_SETUP;
		} else {
			actionType = modelAction.NumberingSetupModelActionTypes.EXPORT_NUMBERING_SETUP_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NumberingSetupAction(
				modelAction.NumberingSetupModelActionTypes.IMPORT_NUMBERING_SETUPS,
				{
					file: file,
				},
				[new modelAction.NumberingSetupAction(
			...
			window.open(environment.API_URL + '/api/numberingsetup/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NumberingSetupModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Numbering Setup entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Numbering Setup entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NumberingSetupModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NumberingSetupModelActionTypes.DELETE_NUMBERING_SETUP;
						} else if (usingSelected) {
							actionType = modelAction.NumberingSetupModelActionTypes.DELETE_ALL_NUMBERING_SETUP;
						} else {
							actionType = modelAction.NumberingSetupModelActionTypes.DELETE_NUMBERING_SETUP_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NumberingSetupAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NUMBERING_SETUP when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NUMBERING_SETUP and DELETE_NUMBERING_SETUP_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NumberingSetupModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NumberingSetupModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.numberingSetupsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
