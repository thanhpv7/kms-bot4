...
import {AdminNumberingSetupCrudListComponent} from './numbering-setup-admin-crud-list.component';
...
import {AdminNumberingSetupCrudModule} from '../numbering-setup-admin-crud.module';
import {
 ...
 */
describe('Numbering Setup Crud List Tile Component', () => {

	let fixture;
	let numberingSetupCrudListComponent: AdminNumberingSetupCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<NumberingSetupModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e6cac66b-421e-45e7-8067-d2fdaa31bfb1';
	const searchContent = 'text';

	const searchableFieldCount = NumberingSetupModel.searchFields.length;

	const routerState: RouterState = {
		url: 'numbering-setup-admin-crud',
		urls: ['numbering-setup-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const numberingSetupDataFactory = new NumberingSetupDataFactory();

	const defaultDataSize = 30;
	const defaultData = numberingSetupDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getNumberingSetupCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNumberingSetupCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminNumberingSetupCrudListComponent);
			numberingSetupCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(numberingSetupCrudListComponent.numberingSetupsCount).toBe(defaultData.length);
			...
			expect(numberingSetupCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			numberingSetupCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = numberingSetupDataFactory.createAll(filteredDataSize);
			...
			expect(numberingSetupCrudListComponent.numberingSetupsCount).toBe(filteredDataSize);
			expect(numberingSetupCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(numberingSetupCrudListComponent.pageIndex).toBe(0);
			expect(numberingSetupCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			numberingSetupCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		numberingSetupCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		numberingSetupCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
