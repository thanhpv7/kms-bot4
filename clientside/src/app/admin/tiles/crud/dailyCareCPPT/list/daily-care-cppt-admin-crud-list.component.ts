...
import {DailyCareCPPTModelState} from 'src/app/models/dailyCareCPPT/daily_care_cppt.model.state';
...
import {
	...
	getDailyCareCPPTCollectionCount
...
@Component({
	selector: 'cb-daily-care-cppt-admin-crud-list',
	templateUrl: './daily-care-cppt-admin-crud-list.component.html',
	styleUrls: [
		'./daily-care-cppt-admin-crud-list.component.scss',
...
export class AdminDailyCareCPPTCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DailyCareCPPTModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DailyCareCPPTModel.searchFields,
	...
	/**
	 * List of Daily Care CPPTs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	dailyCareCPPTs: Observable<DailyCareCPPTModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	dailyCareCPPTsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.dailyCareCPPTsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.dailyCareCPPTsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DailyCareCPPTAction(
				modelAction.DailyCareCPPTModelActionTypes.INITIALISE_DAILY_CARE_CPPT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DailyCareCPPTAction(
				modelAction.DailyCareCPPTModelActionTypes.FETCH_DAILY_CARE_CPPT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.dailyCareCPPTs = this.store.select(getDailyCareCPPTCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDailyCareCPPTCollectionCount, this.collectionId).subscribe(
				(count) => this.dailyCareCPPTsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DailyCareCPPTModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DailyCareCPPTModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DailyCareCPPTModelActionTypes.EXPORT_ALL_DAILY_CARE_CPPT;
		} else {
			actionType = modelAction.DailyCareCPPTModelActionTypes.EXPORT_DAILY_CARE_CPPT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DailyCareCPPTAction(
				modelAction.DailyCareCPPTModelActionTypes.IMPORT_DAILY_CARE_CPPTS,
				{
					file: file,
				},
				[new modelAction.DailyCareCPPTAction(
			...
			window.open(environment.API_URL + '/api/dailycarecppt/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DailyCareCPPTModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Daily Care CPPT entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Daily Care CPPT entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DailyCareCPPTModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DailyCareCPPTModelActionTypes.DELETE_DAILY_CARE_CPPT;
						} else if (usingSelected) {
							actionType = modelAction.DailyCareCPPTModelActionTypes.DELETE_ALL_DAILY_CARE_CPPT;
						} else {
							actionType = modelAction.DailyCareCPPTModelActionTypes.DELETE_DAILY_CARE_CPPT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DailyCareCPPTAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DAILY_CARE_CPPT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DAILY_CARE_CPPT and DELETE_DAILY_CARE_CPPT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DailyCareCPPTModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.dailyCareCPPTsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
