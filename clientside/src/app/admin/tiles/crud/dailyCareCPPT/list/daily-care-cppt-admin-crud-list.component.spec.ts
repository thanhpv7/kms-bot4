...
import {AdminDailyCareCPPTCrudListComponent} from './daily-care-cppt-admin-crud-list.component';
...
import {AdminDailyCareCPPTCrudModule} from '../daily-care-cppt-admin-crud.module';
import {
 ...
 */
describe('Daily Care CPPT Crud List Tile Component', () => {

	let fixture;
	let dailyCareCPPTCrudListComponent: AdminDailyCareCPPTCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<DailyCareCPPTModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '153ee6eb-47b5-4686-a646-f75ba2d07bbe';
	const searchContent = 'text';

	const searchableFieldCount = DailyCareCPPTModel.searchFields.length;

	const routerState: RouterState = {
		url: 'daily-care-cppt-admin-crud',
		urls: ['daily-care-cppt-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const dailyCareCPPTDataFactory = new DailyCareCPPTDataFactory();

	const defaultDataSize = 30;
	const defaultData = dailyCareCPPTDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getDailyCareCPPTCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDailyCareCPPTCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminDailyCareCPPTCrudListComponent);
			dailyCareCPPTCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(dailyCareCPPTCrudListComponent.dailyCareCPPTsCount).toBe(defaultData.length);
			...
			expect(dailyCareCPPTCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			dailyCareCPPTCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = dailyCareCPPTDataFactory.createAll(filteredDataSize);
			...
			expect(dailyCareCPPTCrudListComponent.dailyCareCPPTsCount).toBe(filteredDataSize);
			expect(dailyCareCPPTCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(dailyCareCPPTCrudListComponent.pageIndex).toBe(0);
			expect(dailyCareCPPTCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			dailyCareCPPTCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		dailyCareCPPTCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		dailyCareCPPTCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
