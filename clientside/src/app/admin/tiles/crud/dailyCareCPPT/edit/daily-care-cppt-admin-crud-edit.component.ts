...
import * as modelAction from 'src/app/models/dailyCareCPPT/daily_care_cppt.model.action';
import {DailyCareCPPTModelState, DailyCareCPPTModelAudit} from 'src/app/models/dailyCareCPPT/daily_care_cppt.model.state';
import {DailyCareCPPTModel} from 'src/app/models/dailyCareCPPT/daily_care_cppt.model';
import {
	getDailyCareCPPTModelAuditsByEntityId,
...
import {CrudTileMode} from '../daily-care-cppt-admin-crud.component';
...
@Component({
	selector: 'cb-daily-care-cppt-admin-crud-edit',
	templateUrl: './daily-care-cppt-admin-crud-edit.component.html',
	styleUrls: [
		'./daily-care-cppt-admin-crud-edit.component.scss',
...
export class AdminDailyCareCPPTCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '153ee6eb-47b5-4686-a646-f75ba2d07bbe';
	 ...
	 */
	dailyCareCPPTAudits$: Observable<DailyCareCPPTModelAudit[]>;
	...
	icd10OppositeRelation = 'cpptICD10';
	...
	icd9CMOppositeRelation = 'cpptICD9CM';
	...
	createdByStaffMultiCrudActive: boolean = false;
	createdByStaffDisplayName = 'Created By Staff';
	...
	createdByStaffOppositeRelation = 'dailyCareCPPTsCreatedByStaff';
	createdByStaffRelations: ModelRelation[] = this.createdByStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.createdByStaffOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	doctorInChargeMultiCrudActive: boolean = false;
	doctorInChargeDisplayName = 'Doctor in Charge';
	...
	doctorInChargeOppositeRelation = 'dailyCareCPPTsDoctorInCharge';
	doctorInChargeRelations: ModelRelation[] = this.doctorInChargeMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.doctorInChargeOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	medicalTranscriberOppositeRelation = 'dailyCareCPPTsMedicalTranscriber';
	...
	medicalExaminationRecordOppositeRelation = 'dailyCareCPPTs';
	 ...
	 */
	modelProperties: ModelProperty[] = DailyCareCPPTModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = DailyCareCPPTModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DailyCareCPPTModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'createdByStaff' && this.createdByStaffMultiCrudActive) {
					exp.fields = StaffModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'doctorInCharge' && this.doctorInChargeMultiCrudActive) {
					exp.fields = StaffModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: DailyCareCPPTModel;
	...
	constructor(
		private readonly store: Store<{ model: DailyCareCPPTModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDailyCareCPPTModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DailyCareCPPTModel();
		...
		if (this.targetModelId) {
			this.dailyCareCPPTAudits$ = this.store.select(getDailyCareCPPTModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DailyCareCPPTModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.DailyCareCPPTAction(
			modelAction.DailyCareCPPTModelActionTypes.FETCH_DAILY_CARE_CPPT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDailyCareCPPTModelWithId, this.targetModelId)
			...
			model = new DailyCareCPPTModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DailyCareCPPTAction(
			modelAction.DailyCareCPPTModelActionTypes.FETCH_DAILY_CARE_CPPT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<DailyCareCPPTModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.icd10MultiCrudActive || this.icd9CMMultiCrudActive || this.createdByStaffMultiCrudActive || this.doctorInChargeMultiCrudActive || this.medicalTranscriberMultiCrudActive || this.medicalExaminationRecordMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.DailyCareCPPTAction(
						modelAction.DailyCareCPPTModelActionTypes.COUNT_DAILY_CARE_CPPTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.DailyCareCPPTAction(
				modelAction.DailyCareCPPTModelActionTypes.CREATE_DAILY_CARE_CPPT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DailyCareCPPTAction(
				modelAction.DailyCareCPPTModelActionTypes.UPDATE_DAILY_CARE_CPPT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DailyCareCPPTModel = new DailyCareCPPTModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDailyCareCPPTModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DailyCareCPPTModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.createdByStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.createdByStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.createdByStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.createdByStaffRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.doctorInChargeDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.doctorInChargeRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.doctorInChargeDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.doctorInChargeRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DailyCareCPPTModel.getProps();
		this.modelRelations = DailyCareCPPTModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'createdByStaff';
		}).hideElement = this.createdByStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'createdByStaff').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'doctorInCharge';
		}).hideElement = this.doctorInChargeMultiCrudActive || this.modelRelations.find(rel => rel.id === 'doctorInCharge').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.createdByStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.createdByStaffDisplayName, many: false});
		}

		if (this.doctorInChargeMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.doctorInChargeDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.createdByStaff && this.targetModel.createdByStaff.id && this.createdByStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.createdByStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.createdByStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.createdByStaffId).subscribe(createdByStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.createdByStaffRelations, this.isViewOnly),
					createdByStaff,
					StaffModel,
		...
		if (this.targetModel.doctorInCharge && this.targetModel.doctorInCharge.id && this.doctorInChargeMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.doctorInChargeOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.doctorInCharge.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.doctorInChargeId).subscribe(doctorInCharge => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.doctorInChargeRelations, this.isViewOnly),
					doctorInCharge,
					StaffModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: DailyCareCPPTModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DailyCareCPPTModel {
		let updatedModel: DailyCareCPPTModel = new DailyCareCPPTModel(model);
		...
		if (this.createdByStaffMultiCrudActive) {
			updatedModel.createdByStaff = event.additionalEntities.createdByStaff ? event.additionalEntities.createdByStaff : undefined;

			if (!updatedModel.createdByStaff && event.payload) {
				event.payload.createdByStaffId = null;
			}
		} else if (event.payload?.createdByStaffId !== undefined) {
			event.payload.createdByStaff = null;
			updatedModel.createdByStaff = null;
		}

		if (this.doctorInChargeMultiCrudActive) {
			updatedModel.doctorInCharge = event.additionalEntities.doctorInCharge ? event.additionalEntities.doctorInCharge : undefined;

			if (!updatedModel.doctorInCharge && event.payload) {
				event.payload.doctorInChargeId = null;
			}
		} else if (event.payload?.doctorInChargeId !== undefined) {
			event.payload.doctorInCharge = null;
			updatedModel.doctorInCharge = null;
		}
