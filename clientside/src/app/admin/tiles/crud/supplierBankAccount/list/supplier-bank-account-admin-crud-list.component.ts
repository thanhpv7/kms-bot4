...
import * as modelAction from 'src/app/models/supplierBankAccount/supplier_bank_account.model.action';
import {SupplierBankAccountModelState} from 'src/app/models/supplierBankAccount/supplier_bank_account.model.state';
...
import {
	getSupplierBankAccountCollectionModels,
	getSupplierBankAccountCollectionCount
...
import {CrudTileMode} from '../supplier-bank-account-admin-crud.component';
...
@Component({
	selector: 'cb-supplier-bank-account-admin-crud-list',
	templateUrl: './supplier-bank-account-admin-crud-list.component.html',
	styleUrls: [
		'./supplier-bank-account-admin-crud-list.component.scss',
...
export class AdminSupplierBankAccountCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SupplierBankAccountModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SupplierBankAccountModel.searchFields,
	...
	/**
	 * List of Supplier Bank Accounts that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	supplierBankAccounts: Observable<SupplierBankAccountModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	supplierBankAccountsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = SupplierBankAccountModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = SupplierBankAccountModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '4c653561-1a43-43b3-9e18-549aa81a642c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.supplierBankAccountsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.supplierBankAccountsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: SupplierBankAccountModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierBankAccountAction(
				modelAction.SupplierBankAccountModelActionTypes.INITIALISE_SUPPLIER_BANK_ACCOUNT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SupplierBankAccountAction(
				modelAction.SupplierBankAccountModelActionTypes.FETCH_SUPPLIER_BANK_ACCOUNT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.SupplierBankAccountAction(
						modelAction.SupplierBankAccountModelActionTypes.COUNT_SUPPLIER_BANK_ACCOUNTS,
						null
					)
				]
			))
		...
		this.supplierBankAccounts = this.store.select(getSupplierBankAccountCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSupplierBankAccountCollectionCount, this.collectionId).subscribe(
				(count) => this.supplierBankAccountsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('supplier-bank-account');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SupplierBankAccountModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SupplierBankAccountModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SupplierBankAccountModelActionTypes.EXPORT_ALL_SUPPLIER_BANK_ACCOUNT;
		} else {
			actionType = modelAction.SupplierBankAccountModelActionTypes.EXPORT_SUPPLIER_BANK_ACCOUNT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SupplierBankAccountAction(
				modelAction.SupplierBankAccountModelActionTypes.IMPORT_SUPPLIER_BANK_ACCOUNTS,
				{
					file: file,
				},
				[new modelAction.SupplierBankAccountAction(
			...
			window.open(environment.API_URL + '/api/supplierbankaccount/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SupplierBankAccountModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Supplier Bank Account entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Supplier Bank Account entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SupplierBankAccountModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SupplierBankAccountModelActionTypes.DELETE_SUPPLIER_BANK_ACCOUNT;
						} else if (usingSelected) {
							actionType = modelAction.SupplierBankAccountModelActionTypes.DELETE_ALL_SUPPLIER_BANK_ACCOUNT;
						} else {
							actionType = modelAction.SupplierBankAccountModelActionTypes.DELETE_SUPPLIER_BANK_ACCOUNT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SupplierBankAccountAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SUPPLIER_BANK_ACCOUNT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SUPPLIER_BANK_ACCOUNT and DELETE_SUPPLIER_BANK_ACCOUNT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<SupplierBankAccountModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SupplierBankAccountModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.supplierBankAccountsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
