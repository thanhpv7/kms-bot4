...
import {AdminSupplierBankAccountCrudListComponent} from './supplier-bank-account-admin-crud-list.component';
...
import {AdminSupplierBankAccountCrudModule} from '../supplier-bank-account-admin-crud.module';
import {
	SupplierBankAccountModelState,
	initialState as initialSupplierBankAccountModelState
} from 'src/app/models/supplierBankAccount/supplier_bank_account.model.state';
 ...
 */
describe('Supplier Bank Account Crud List Tile Component', () => {

	let fixture;
	let supplierBankAccountCrudListComponent: AdminSupplierBankAccountCrudListComponent;

	let store: MockStore<{ model: SupplierBankAccountModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let supplierBankAccountModelState: SupplierBankAccountModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<SupplierBankAccountModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '4c653561-1a43-43b3-9e18-549aa81a642c';
	const searchContent = 'text';

	const searchableFieldCount = SupplierBankAccountModel.searchFields.length;

	const routerState: RouterState = {
		url: 'supplier-bank-account-admin-crud',
		urls: ['supplier-bank-account-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const supplierBankAccountDataFactory = new SupplierBankAccountDataFactory();

	const defaultDataSize = 30;
	const defaultData = supplierBankAccountDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: SupplierBankAccountModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		supplierBankAccountModelState = initialSupplierBankAccountModelState;
		store.setState({model: supplierBankAccountModelState});
		...
		spyOn(store, 'select')
			.withArgs(getSupplierBankAccountCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getSupplierBankAccountCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminSupplierBankAccountCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminSupplierBankAccountCrudListComponent);
			supplierBankAccountCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(supplierBankAccountCrudListComponent.supplierBankAccountsCount).toBe(defaultData.length);
			...
			expect(supplierBankAccountCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			supplierBankAccountCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = supplierBankAccountDataFactory.createAll(filteredDataSize);
			...
			expect(supplierBankAccountCrudListComponent.supplierBankAccountsCount).toBe(filteredDataSize);
			expect(supplierBankAccountCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(supplierBankAccountCrudListComponent.pageIndex).toBe(0);
			expect(supplierBankAccountCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			supplierBankAccountCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		supplierBankAccountCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		supplierBankAccountCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
