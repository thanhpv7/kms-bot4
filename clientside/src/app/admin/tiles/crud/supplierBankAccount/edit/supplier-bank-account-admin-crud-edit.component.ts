...
import {SupplierBankAccountModelState, SupplierBankAccountModelAudit} from 'src/app/models/supplierBankAccount/supplier_bank_account.model.state';
import {SupplierBankAccountModel} from 'src/app/models/supplierBankAccount/supplier_bank_account.model';
import {
	getSupplierBankAccountModelAuditsByEntityId,
	getSupplierBankAccountModelWithId,
	getCountSupplierBankAccountModels,
...
@Component({
	selector: 'cb-supplier-bank-account-admin-crud-edit',
	templateUrl: './supplier-bank-account-admin-crud-edit.component.html',
	styleUrls: [
		'./supplier-bank-account-admin-crud-edit.component.scss',
...
export class AdminSupplierBankAccountCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	supplierBankAccountAudits$: Observable<SupplierBankAccountModelAudit[]>;
	...
	supplierOppositeRelation = 'supplierBankAccounts';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SupplierBankAccountModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: SupplierBankAccountModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSupplierBankAccountModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new SupplierBankAccountModel();
		...
		if (this.targetModelId) {
			this.supplierBankAccountAudits$ = this.store.select(getSupplierBankAccountModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SupplierBankAccountModel> = {
		...
		this.store.dispatch(new modelAction.SupplierBankAccountAction(
			modelAction.SupplierBankAccountModelActionTypes.FETCH_SUPPLIER_BANK_ACCOUNT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSupplierBankAccountModelWithId, this.targetModelId)
			...
			model = new SupplierBankAccountModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SupplierBankAccountAction(
			modelAction.SupplierBankAccountModelActionTypes.FETCH_SUPPLIER_BANK_ACCOUNT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.SupplierBankAccountAction(
				modelAction.SupplierBankAccountModelActionTypes.CREATE_SUPPLIER_BANK_ACCOUNT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SupplierBankAccountAction(
				modelAction.SupplierBankAccountModelActionTypes.UPDATE_SUPPLIER_BANK_ACCOUNT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SupplierBankAccountModel = new SupplierBankAccountModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSupplierBankAccountModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SupplierBankAccountModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SupplierBankAccountModel.getProps();
		this.modelRelations = SupplierBankAccountModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: SupplierBankAccountModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SupplierBankAccountModel {
		let updatedModel: SupplierBankAccountModel = new SupplierBankAccountModel(model);
