...
import {BodyChartExaminationModelState, BodyChartExaminationModelAudit} from 'src/app/models/bodyChartExamination/body_chart_examination.model.state';
import {BodyChartExaminationModel} from 'src/app/models/bodyChartExamination/body_chart_examination.model';
import {
	getBodyChartExaminationModelAuditsByEntityId,
	getBodyChartExaminationModelWithId,
	getCountBodyChartExaminationModels,
...
@Component({
	selector: 'cb-body-chart-examination-admin-crud-edit',
	templateUrl: './body-chart-examination-admin-crud-edit.component.html',
	styleUrls: [
		'./body-chart-examination-admin-crud-edit.component.scss',
...
export class AdminBodyChartExaminationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bodyChartExaminationAudits$: Observable<BodyChartExaminationModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'bodyChartExamination';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BodyChartExaminationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BodyChartExaminationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBodyChartExaminationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BodyChartExaminationModel();
		...
		if (this.targetModelId) {
			this.bodyChartExaminationAudits$ = this.store.select(getBodyChartExaminationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BodyChartExaminationModel> = {
		...
		this.store.dispatch(new modelAction.BodyChartExaminationAction(
			modelAction.BodyChartExaminationModelActionTypes.FETCH_BODY_CHART_EXAMINATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBodyChartExaminationModelWithId, this.targetModelId)
			...
			model = new BodyChartExaminationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BodyChartExaminationAction(
			modelAction.BodyChartExaminationModelActionTypes.FETCH_BODY_CHART_EXAMINATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BodyChartExaminationAction(
				modelAction.BodyChartExaminationModelActionTypes.CREATE_BODY_CHART_EXAMINATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BodyChartExaminationAction(
				modelAction.BodyChartExaminationModelActionTypes.UPDATE_BODY_CHART_EXAMINATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BodyChartExaminationModel = new BodyChartExaminationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBodyChartExaminationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BodyChartExaminationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BodyChartExaminationModel.getProps();
		this.modelRelations = BodyChartExaminationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BodyChartExaminationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BodyChartExaminationModel {
		let updatedModel: BodyChartExaminationModel = new BodyChartExaminationModel(model);
