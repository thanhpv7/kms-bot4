...
import * as modelAction from 'src/app/models/bodyChartExamination/body_chart_examination.model.action';
import {BodyChartExaminationModelState} from 'src/app/models/bodyChartExamination/body_chart_examination.model.state';
...
import {
	getBodyChartExaminationCollectionModels,
	getBodyChartExaminationCollectionCount
...
import {CrudTileMode} from '../body-chart-examination-admin-crud.component';
...
@Component({
	selector: 'cb-body-chart-examination-admin-crud-list',
	templateUrl: './body-chart-examination-admin-crud-list.component.html',
	styleUrls: [
		'./body-chart-examination-admin-crud-list.component.scss',
...
export class AdminBodyChartExaminationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BodyChartExaminationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BodyChartExaminationModel.searchFields,
	...
	/**
	 * List of Body Chart Examinations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bodyChartExaminations: Observable<BodyChartExaminationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bodyChartExaminationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BodyChartExaminationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BodyChartExaminationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '11a4b0cd-1827-48b1-bf40-e4d39fea8a58';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bodyChartExaminationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bodyChartExaminationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BodyChartExaminationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BodyChartExaminationAction(
				modelAction.BodyChartExaminationModelActionTypes.INITIALISE_BODY_CHART_EXAMINATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BodyChartExaminationAction(
				modelAction.BodyChartExaminationModelActionTypes.FETCH_BODY_CHART_EXAMINATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BodyChartExaminationAction(
						modelAction.BodyChartExaminationModelActionTypes.COUNT_BODY_CHART_EXAMINATIONS,
						null
					)
				]
			))
		...
		this.bodyChartExaminations = this.store.select(getBodyChartExaminationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBodyChartExaminationCollectionCount, this.collectionId).subscribe(
				(count) => this.bodyChartExaminationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('body-chart-examination');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BodyChartExaminationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BodyChartExaminationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BodyChartExaminationModelActionTypes.EXPORT_ALL_BODY_CHART_EXAMINATION;
		} else {
			actionType = modelAction.BodyChartExaminationModelActionTypes.EXPORT_BODY_CHART_EXAMINATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BodyChartExaminationAction(
				modelAction.BodyChartExaminationModelActionTypes.IMPORT_BODY_CHART_EXAMINATIONS,
				{
					file: file,
				},
				[new modelAction.BodyChartExaminationAction(
			...
			window.open(environment.API_URL + '/api/bodychartexamination/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BodyChartExaminationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Body Chart Examination entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Body Chart Examination entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BodyChartExaminationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BodyChartExaminationModelActionTypes.DELETE_BODY_CHART_EXAMINATION;
						} else if (usingSelected) {
							actionType = modelAction.BodyChartExaminationModelActionTypes.DELETE_ALL_BODY_CHART_EXAMINATION;
						} else {
							actionType = modelAction.BodyChartExaminationModelActionTypes.DELETE_BODY_CHART_EXAMINATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BodyChartExaminationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BODY_CHART_EXAMINATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BODY_CHART_EXAMINATION and DELETE_BODY_CHART_EXAMINATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BodyChartExaminationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BodyChartExaminationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bodyChartExaminationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
