...
import {AdminBodyChartExaminationCrudListComponent} from './body-chart-examination-admin-crud-list.component';
...
import {AdminBodyChartExaminationCrudModule} from '../body-chart-examination-admin-crud.module';
import {
	BodyChartExaminationModelState,
	initialState as initialBodyChartExaminationModelState
} from 'src/app/models/bodyChartExamination/body_chart_examination.model.state';
 ...
 */
describe('Body Chart Examination Crud List Tile Component', () => {

	let fixture;
	let bodyChartExaminationCrudListComponent: AdminBodyChartExaminationCrudListComponent;

	let store: MockStore<{ model: BodyChartExaminationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bodyChartExaminationModelState: BodyChartExaminationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BodyChartExaminationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '11a4b0cd-1827-48b1-bf40-e4d39fea8a58';
	const searchContent = 'text';

	const searchableFieldCount = BodyChartExaminationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'body-chart-examination-admin-crud',
		urls: ['body-chart-examination-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bodyChartExaminationDataFactory = new BodyChartExaminationDataFactory();

	const defaultDataSize = 30;
	const defaultData = bodyChartExaminationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BodyChartExaminationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bodyChartExaminationModelState = initialBodyChartExaminationModelState;
		store.setState({model: bodyChartExaminationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBodyChartExaminationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBodyChartExaminationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBodyChartExaminationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBodyChartExaminationCrudListComponent);
			bodyChartExaminationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bodyChartExaminationCrudListComponent.bodyChartExaminationsCount).toBe(defaultData.length);
			...
			expect(bodyChartExaminationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bodyChartExaminationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bodyChartExaminationDataFactory.createAll(filteredDataSize);
			...
			expect(bodyChartExaminationCrudListComponent.bodyChartExaminationsCount).toBe(filteredDataSize);
			expect(bodyChartExaminationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bodyChartExaminationCrudListComponent.pageIndex).toBe(0);
			expect(bodyChartExaminationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bodyChartExaminationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bodyChartExaminationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bodyChartExaminationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
