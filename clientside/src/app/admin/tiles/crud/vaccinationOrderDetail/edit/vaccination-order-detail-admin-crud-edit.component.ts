...
import * as modelAction from 'src/app/models/vaccinationOrderDetail/vaccination_order_detail.model.action';
import {VaccinationOrderDetailModelState, VaccinationOrderDetailModelAudit} from 'src/app/models/vaccinationOrderDetail/vaccination_order_detail.model.state';
import {VaccinationOrderDetailModel} from 'src/app/models/vaccinationOrderDetail/vaccination_order_detail.model';
import {
	getVaccinationOrderDetailModelAuditsByEntityId,
...
import {CrudTileMode} from '../vaccination-order-detail-admin-crud.component';
...
@Component({
	selector: 'cb-vaccination-order-detail-admin-crud-edit',
	templateUrl: './vaccination-order-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./vaccination-order-detail-admin-crud-edit.component.scss',
...
export class AdminVaccinationOrderDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '1ef411ce-a85e-42e6-83be-84651d863c23';
	 ...
	 */
	vaccinationOrderDetailAudits$: Observable<VaccinationOrderDetailModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'vaccinationOrderDetails';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Vaccination Order and Vaccination Order Details here] off begin
	vaccinationOrderMultiCrudActive: boolean = false;
	vaccinationOrderDisplayName = 'Vaccination Order';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Vaccination Order and Vaccination Order Details here] end
	vaccinationOrderOppositeRelation = 'vaccinationOrderDetails';
	vaccinationOrderRelations: ModelRelation[] = this.vaccinationOrderMultiCrudActive ? VaccinationOrderModel.getRelations()
		.map(relation => {
			if (relation.id === this.vaccinationOrderOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	warehouseOppositeRelation = 'vaccinationOrderDetails';
	 ...
	 */
	modelProperties: ModelProperty[] = VaccinationOrderDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = VaccinationOrderDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = VaccinationOrderDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'vaccinationOrder' && this.vaccinationOrderMultiCrudActive) {
	 ...
	 */
	targetModel: VaccinationOrderDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: VaccinationOrderDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountVaccinationOrderDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new VaccinationOrderDetailModel();
		...
		if (this.targetModelId) {
			this.vaccinationOrderDetailAudits$ = this.store.select(getVaccinationOrderDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<VaccinationOrderDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
			modelAction.VaccinationOrderDetailModelActionTypes.FETCH_VACCINATION_ORDER_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getVaccinationOrderDetailModelWithId, this.targetModelId)
			...
			model = new VaccinationOrderDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('vaccination-order-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
			modelAction.VaccinationOrderDetailModelActionTypes.FETCH_VACCINATION_ORDER_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<VaccinationOrderDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.stockCatalogueMultiCrudActive || this.vaccinationOrderMultiCrudActive || this.warehouseMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.VaccinationOrderDetailAction(
						modelAction.VaccinationOrderDetailModelActionTypes.COUNT_VACCINATION_ORDER_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
				modelAction.VaccinationOrderDetailModelActionTypes.CREATE_VACCINATION_ORDER_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
				modelAction.VaccinationOrderDetailModelActionTypes.UPDATE_VACCINATION_ORDER_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: VaccinationOrderDetailModel = new VaccinationOrderDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getVaccinationOrderDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new VaccinationOrderDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.vaccinationOrderDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(VaccinationOrderModel.getProps(), this.vaccinationOrderRelations, this.isViewOnly),
					new VaccinationOrderModel(),
					VaccinationOrderModel,
					this.vaccinationOrderDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.vaccinationOrderRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = VaccinationOrderDetailModel.getProps();
		this.modelRelations = VaccinationOrderDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'vaccinationOrder';
		}).hideElement = this.vaccinationOrderMultiCrudActive || this.modelRelations.find(rel => rel.id === 'vaccinationOrder').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.vaccinationOrderMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.vaccinationOrderDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.vaccinationOrder && this.targetModel.vaccinationOrder.id && this.vaccinationOrderMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				VaccinationOrderModel,
				this.vaccinationOrderOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.vaccinationOrder.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getVaccinationOrderModelWithId, this.targetModel.vaccinationOrderId).subscribe(vaccinationOrder => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(VaccinationOrderModel.getProps(), this.vaccinationOrderRelations, this.isViewOnly),
					vaccinationOrder,
					VaccinationOrderModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: VaccinationOrderDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): VaccinationOrderDetailModel {
		let updatedModel: VaccinationOrderDetailModel = new VaccinationOrderDetailModel(model);
		...
		if (this.vaccinationOrderMultiCrudActive) {
			updatedModel.vaccinationOrder = event.additionalEntities.vaccinationOrder ? event.additionalEntities.vaccinationOrder : undefined;

			if (!updatedModel.vaccinationOrder && event.payload) {
				event.payload.vaccinationOrderId = null;
			}
		} else if (event.payload?.vaccinationOrderId !== undefined) {
			event.payload.vaccinationOrder = null;
			updatedModel.vaccinationOrder = null;
		}
