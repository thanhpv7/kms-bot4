...
import {AdminVaccinationOrderDetailCrudListComponent} from './vaccination-order-detail-admin-crud-list.component';
...
import {AdminVaccinationOrderDetailCrudModule} from '../vaccination-order-detail-admin-crud.module';
import {
 ...
 */
describe('Vaccination Order Detail Crud List Tile Component', () => {

	let fixture;
	let vaccinationOrderDetailCrudListComponent: AdminVaccinationOrderDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<VaccinationOrderDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '1ef411ce-a85e-42e6-83be-84651d863c23';
	const searchContent = 'text';

	const searchableFieldCount = VaccinationOrderDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'vaccination-order-detail-admin-crud',
		urls: ['vaccination-order-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const vaccinationOrderDetailDataFactory = new VaccinationOrderDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = vaccinationOrderDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getVaccinationOrderDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getVaccinationOrderDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminVaccinationOrderDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminVaccinationOrderDetailCrudListComponent);
			vaccinationOrderDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(vaccinationOrderDetailCrudListComponent.vaccinationOrderDetailsCount).toBe(defaultData.length);
			...
			expect(vaccinationOrderDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			vaccinationOrderDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = vaccinationOrderDetailDataFactory.createAll(filteredDataSize);
			...
			expect(vaccinationOrderDetailCrudListComponent.vaccinationOrderDetailsCount).toBe(filteredDataSize);
			expect(vaccinationOrderDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(vaccinationOrderDetailCrudListComponent.pageIndex).toBe(0);
			expect(vaccinationOrderDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			vaccinationOrderDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		vaccinationOrderDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		vaccinationOrderDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
