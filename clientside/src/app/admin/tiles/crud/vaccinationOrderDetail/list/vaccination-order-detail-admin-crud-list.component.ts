...
import {VaccinationOrderDetailModelState} from 'src/app/models/vaccinationOrderDetail/vaccination_order_detail.model.state';
...
import {
	...
	getVaccinationOrderDetailCollectionCount
...
@Component({
	selector: 'cb-vaccination-order-detail-admin-crud-list',
	templateUrl: './vaccination-order-detail-admin-crud-list.component.html',
	styleUrls: [
		'./vaccination-order-detail-admin-crud-list.component.scss',
...
export class AdminVaccinationOrderDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<VaccinationOrderDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...VaccinationOrderDetailModel.searchFields,
	...
	/**
	 * List of Vaccination Order Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	vaccinationOrderDetails: Observable<VaccinationOrderDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	vaccinationOrderDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.vaccinationOrderDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.vaccinationOrderDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
				modelAction.VaccinationOrderDetailModelActionTypes.INITIALISE_VACCINATION_ORDER_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
				modelAction.VaccinationOrderDetailModelActionTypes.FETCH_VACCINATION_ORDER_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.vaccinationOrderDetails = this.store.select(getVaccinationOrderDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getVaccinationOrderDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.vaccinationOrderDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: VaccinationOrderDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.VaccinationOrderDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.VaccinationOrderDetailModelActionTypes.EXPORT_ALL_VACCINATION_ORDER_DETAIL;
		} else {
			actionType = modelAction.VaccinationOrderDetailModelActionTypes.EXPORT_VACCINATION_ORDER_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
				modelAction.VaccinationOrderDetailModelActionTypes.IMPORT_VACCINATION_ORDER_DETAILS,
				{
					file: file,
				},
				[new modelAction.VaccinationOrderDetailAction(
			...
			window.open(environment.API_URL + '/api/vaccinationorderdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: VaccinationOrderDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Vaccination Order Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Vaccination Order Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.VaccinationOrderDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.VaccinationOrderDetailModelActionTypes.DELETE_VACCINATION_ORDER_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.VaccinationOrderDetailModelActionTypes.DELETE_ALL_VACCINATION_ORDER_DETAIL;
						} else {
							actionType = modelAction.VaccinationOrderDetailModelActionTypes.DELETE_VACCINATION_ORDER_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.VaccinationOrderDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_VACCINATION_ORDER_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_VACCINATION_ORDER_DETAIL and DELETE_VACCINATION_ORDER_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = VaccinationOrderDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.vaccinationOrderDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
