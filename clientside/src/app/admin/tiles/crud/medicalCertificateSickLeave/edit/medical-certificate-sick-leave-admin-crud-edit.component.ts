...
import * as modelAction from 'src/app/models/medicalCertificateSickLeave/medical_certificate_sick_leave.model.action';
import {MedicalCertificateSickLeaveModelState, MedicalCertificateSickLeaveModelAudit} from 'src/app/models/medicalCertificateSickLeave/medical_certificate_sick_leave.model.state';
...
import {
	getMedicalCertificateSickLeaveModelAuditsByEntityId,
	getMedicalCertificateSickLeaveModelWithId,
	getCountMedicalCertificateSickLeaveModels,
...
import {CrudTileMode} from '../medical-certificate-sick-leave-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-sick-leave-admin-crud-edit',
	templateUrl: './medical-certificate-sick-leave-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-sick-leave-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificateSickLeaveCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '3c159ea2-1cf6-4683-9d95-884a4105a241';
	 ...
	 */
	medicalCertificateSickLeaveAudits$: Observable<MedicalCertificateSickLeaveModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificateSickLeave';
	...
	medicalStaffOppositeRelation = 'medicalStaffOfSickLeavesCertificate';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificateSickLeaveModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificateSickLeaveModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificateSickLeaveModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalCertificateSickLeaveModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificateSickLeaveModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificateSickLeaveModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificateSickLeaveModel();
		...
		if (this.targetModelId) {
			this.medicalCertificateSickLeaveAudits$ = this.store.select(getMedicalCertificateSickLeaveModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificateSickLeaveModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
			modelAction.MedicalCertificateSickLeaveModelActionTypes.FETCH_MEDICAL_CERTIFICATE_SICK_LEAVE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateSickLeaveModelWithId, this.targetModelId)
			...
			model = new MedicalCertificateSickLeaveModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-sick-leave');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
			modelAction.MedicalCertificateSickLeaveModelActionTypes.FETCH_MEDICAL_CERTIFICATE_SICK_LEAVE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificateSickLeaveModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificateSickLeaveAction(
						modelAction.MedicalCertificateSickLeaveModelActionTypes.COUNT_MEDICAL_CERTIFICATE_SICK_LEAVES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
				modelAction.MedicalCertificateSickLeaveModelActionTypes.CREATE_MEDICAL_CERTIFICATE_SICK_LEAVE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
				modelAction.MedicalCertificateSickLeaveModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_SICK_LEAVE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificateSickLeaveModel = new MedicalCertificateSickLeaveModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateSickLeaveModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificateSickLeaveModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificateSickLeaveModel.getProps();
		this.modelRelations = MedicalCertificateSickLeaveModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificateSickLeaveModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificateSickLeaveModel {
		let updatedModel: MedicalCertificateSickLeaveModel = new MedicalCertificateSickLeaveModel(model);
