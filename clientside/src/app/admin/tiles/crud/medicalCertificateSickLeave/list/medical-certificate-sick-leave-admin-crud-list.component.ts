...
import {MedicalCertificateSickLeaveModelState} from 'src/app/models/medicalCertificateSickLeave/medical_certificate_sick_leave.model.state';
...
@Component({
	selector: 'cb-medical-certificate-sick-leave-admin-crud-list',
	templateUrl: './medical-certificate-sick-leave-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-sick-leave-admin-crud-list.component.scss',
...
export class AdminMedicalCertificateSickLeaveCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificateSickLeaveModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificateSickLeaveModel.searchFields,
	...
	/**
	 * List of Medical Certificate Sick Leaves that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificateSickLeaves: Observable<MedicalCertificateSickLeaveModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificateSickLeavesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificateSickLeavesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificateSickLeavesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
				modelAction.MedicalCertificateSickLeaveModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_SICK_LEAVE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
				modelAction.MedicalCertificateSickLeaveModelActionTypes.FETCH_MEDICAL_CERTIFICATE_SICK_LEAVE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificateSickLeaves = this.store.select(getMedicalCertificateSickLeaveCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificateSickLeaveCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificateSickLeavesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificateSickLeaveModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificateSickLeaveModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificateSickLeaveModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_SICK_LEAVE;
		} else {
			actionType = modelAction.MedicalCertificateSickLeaveModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_SICK_LEAVE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
				modelAction.MedicalCertificateSickLeaveModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_SICK_LEAVES,
				{
					file: file,
				},
				[new modelAction.MedicalCertificateSickLeaveAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificatesickleave/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificateSickLeaveModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Sick Leave entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate Sick Leave entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificateSickLeaveModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificateSickLeaveModelActionTypes.DELETE_MEDICAL_CERTIFICATE_SICK_LEAVE;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificateSickLeaveModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_SICK_LEAVE;
						} else {
							actionType = modelAction.MedicalCertificateSickLeaveModelActionTypes.DELETE_MEDICAL_CERTIFICATE_SICK_LEAVE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificateSickLeaveAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_SICK_LEAVE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_SICK_LEAVE and DELETE_MEDICAL_CERTIFICATE_SICK_LEAVE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificateSickLeaveModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificateSickLeavesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
