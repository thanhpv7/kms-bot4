...
import {AdminInvoiceCrudListComponent} from './invoice-admin-crud-list.component';
...
import {AdminInvoiceCrudModule} from '../invoice-admin-crud.module';
import {
	InvoiceModelState,
 ...
 */
describe('Invoice Crud List Tile Component', () => {

	let fixture;
	let invoiceCrudListComponent: AdminInvoiceCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<InvoiceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cedb5f60-22ec-4a5a-9f4b-2610799812b2';
	const searchContent = 'text';

	const searchableFieldCount = InvoiceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'invoice-admin-crud',
		urls: ['invoice-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};
	...
	const defaultData = invoiceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getInvoiceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInvoiceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminInvoiceCrudListComponent);
			invoiceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(invoiceCrudListComponent.invoicesCount).toBe(defaultData.length);
			...
			expect(invoiceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			invoiceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = invoiceDataFactory.createAll(filteredDataSize);
			...
			expect(invoiceCrudListComponent.invoicesCount).toBe(filteredDataSize);
			expect(invoiceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(invoiceCrudListComponent.pageIndex).toBe(0);
			expect(invoiceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			invoiceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		invoiceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		invoiceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
