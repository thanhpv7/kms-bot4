...
import {InvoiceModelState} from 'src/app/models/invoice/invoice.model.state';
...
import {CrudTileMode} from '../invoice-admin-crud.component';
...
@Component({
	selector: 'cb-invoice-admin-crud-list',
	templateUrl: './invoice-admin-crud-list.component.html',
	styleUrls: [
		'./invoice-admin-crud-list.component.scss',
...
export class AdminInvoiceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InvoiceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InvoiceModel.searchFields,
	...
	/**
	 * List of Invoices that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.invoicesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.invoicesCount === 0
		},
		{
	...
	filterQuestions: FilterQuestion[] = [
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Invoice Type to Invoice here] off begin
		{
			filterType: FilterQuestionType.dropdown,
			config: {
				options: invoiceTypeEnumArray,
				labelField: 'value',
				valueField: 'key',
				id: 'InvoiceInvoiceTypeFilter',
				label: 'Invoice Type',
			},
			name: 'invoiceType',
		},
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Invoice Type to Invoice here] end
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceAction(
				modelAction.InvoiceModelActionTypes.INITIALISE_INVOICE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceAction(
				modelAction.InvoiceModelActionTypes.FETCH_INVOICE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.invoices = this.store.select(getInvoiceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInvoiceCollectionCount, this.collectionId).subscribe(
				(count) => this.invoicesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InvoiceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InvoiceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InvoiceModelActionTypes.EXPORT_ALL_INVOICE;
		} else {
			actionType = modelAction.InvoiceModelActionTypes.EXPORT_INVOICE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InvoiceAction(
				modelAction.InvoiceModelActionTypes.IMPORT_INVOICES,
				{
					file: file,
				},
				[new modelAction.InvoiceAction(
			...
			window.open(environment.API_URL + '/api/invoice/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InvoiceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Invoice entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Invoice entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InvoiceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InvoiceModelActionTypes.DELETE_INVOICE;
						} else if (usingSelected) {
							actionType = modelAction.InvoiceModelActionTypes.DELETE_ALL_INVOICE;
						} else {
							actionType = modelAction.InvoiceModelActionTypes.DELETE_INVOICE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InvoiceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVOICE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVOICE and DELETE_INVOICE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InvoiceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.invoicesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
