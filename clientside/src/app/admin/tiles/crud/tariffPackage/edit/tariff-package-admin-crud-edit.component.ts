...
import * as modelAction from 'src/app/models/tariffPackage/tariff_package.model.action';
import {TariffPackageModelState, TariffPackageModelAudit} from 'src/app/models/tariffPackage/tariff_package.model.state';
import {TariffPackageModel} from 'src/app/models/tariffPackage/tariff_package.model';
import {
	getTariffPackageModelAuditsByEntityId,
	getTariffPackageModelWithId,
	getCountTariffPackageModels,
} from 'src/app/models/tariffPackage/tariff_package.model.selector';
import {getRouterState} from 'src/app/models/model.selector';
import {ServiceItemAssignmentModel} from 'src/app/models/serviceItemAssignment/service_item_assignment.model';
import * as serviceItemAssignmentModelAction from 'src/app/models/serviceItemAssignment/service_item_assignment.model.action';
import {
	getServiceItemAssignmentCollectionModels, getServiceItemAssignmentModelWithId,
...
import * as tariffPackageItemModelAction from 'src/app/models/tariffPackageItem/tariff_package_item.model.action';
import {
	getTariffPackageItemCollectionModels, getTariffPackageItemModelWithId,
} from 'src/app/models/tariffPackageItem/tariff_package_item.model.selector';
...
import * as tariffPackageServiceAssignmentModelAction from 'src/app/models/tariffPackageServiceAssignment/tariff_package_service_assignment.model.action';
import {
	getTariffPackageServiceAssignmentCollectionModels, getTariffPackageServiceAssignmentModelWithId,
} from 'src/app/models/tariffPackageServiceAssignment/tariff_package_service_assignment.model.selector';
...
import {CrudTileMode} from '../tariff-package-admin-crud.component';
...
@Component({
	selector: 'cb-tariff-package-admin-crud-edit',
	templateUrl: './tariff-package-admin-crud-edit.component.html',
	styleUrls: [
		'./tariff-package-admin-crud-edit.component.scss',
...
export class AdminTariffPackageCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'd6554fc0-7906-4a3b-b3e1-0840e7bcf616';
	 ...
	 */
	tariffPackageAudits$: Observable<TariffPackageModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Package and Service Item Assignments here] off begin
	serviceItemAssignmentsMultiCrudActive: boolean = false;
	serviceItemAssignmentsDisplayName = 'Service Item Assignments';
	serviceItemAssignmentsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Package and Service Item Assignments here] end
	serviceItemAssignmentsOppositeRelation = 'tariffPackage';
	serviceItemAssignmentsRelations: ModelRelation[] = this.serviceItemAssignmentsMultiCrudActive ? ServiceItemAssignmentModel.getRelations()
		.map(relation => {
			if (relation.id === this.serviceItemAssignmentsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Package and Tariff Package Items here] off begin
	tariffPackageItemsMultiCrudActive: boolean = false;
	tariffPackageItemsDisplayName = 'Tariff Package Items';
	tariffPackageItemsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Package and Tariff Package Items here] end
	tariffPackageItemsOppositeRelation = 'tariffPackage';
	tariffPackageItemsRelations: ModelRelation[] = this.tariffPackageItemsMultiCrudActive ? TariffPackageItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.tariffPackageItemsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Package and Tariff Package Service Assignments here] off begin
	tariffPackageServiceAssignmentsMultiCrudActive: boolean = false;
	tariffPackageServiceAssignmentsDisplayName = 'Tariff Package Service Assignments';
	tariffPackageServiceAssignmentsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Tariff Package and Tariff Package Service Assignments here] end
	tariffPackageServiceAssignmentsOppositeRelation = 'tariffPackage';
	tariffPackageServiceAssignmentsRelations: ModelRelation[] = this.tariffPackageServiceAssignmentsMultiCrudActive ? TariffPackageServiceAssignmentModel.getRelations()
		.map(relation => {
			if (relation.id === this.tariffPackageServiceAssignmentsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = TariffPackageModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TariffPackageModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TariffPackageModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'serviceItemAssignments' && this.serviceItemAssignmentsMultiCrudActive) {
					exp.fields = ServiceItemAssignmentModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'tariffPackageItems' && this.tariffPackageItemsMultiCrudActive) {
					exp.fields = TariffPackageItemModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'tariffPackageServiceAssignments' && this.tariffPackageServiceAssignmentsMultiCrudActive) {
					exp.fields = TariffPackageServiceAssignmentModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: TariffPackageModel;
	...
	constructor(
		private readonly store: Store<{ model: TariffPackageModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTariffPackageModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TariffPackageModel();
		...
		if (this.targetModelId) {
			this.tariffPackageAudits$ = this.store.select(getTariffPackageModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TariffPackageModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TariffPackageAction(
			modelAction.TariffPackageModelActionTypes.FETCH_TARIFF_PACKAGE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTariffPackageModelWithId, this.targetModelId)
			...
			model = new TariffPackageModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TariffPackageAction(
			modelAction.TariffPackageModelActionTypes.FETCH_TARIFF_PACKAGE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TariffPackageModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.serviceItemAssignmentsMultiCrudActive || this.tariffPackageItemsMultiCrudActive || this.tariffPackageServiceAssignmentsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TariffPackageAction(
						modelAction.TariffPackageModelActionTypes.COUNT_TARIFF_PACKAGES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TariffPackageAction(
				modelAction.TariffPackageModelActionTypes.CREATE_TARIFF_PACKAGE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TariffPackageAction(
				modelAction.TariffPackageModelActionTypes.UPDATE_TARIFF_PACKAGE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TariffPackageModel = new TariffPackageModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTariffPackageModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TariffPackageModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.serviceItemAssignmentsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ServiceItemAssignmentModel.getProps(), this.serviceItemAssignmentsRelations, this.isViewOnly),
					new ServiceItemAssignmentModel(),
					ServiceItemAssignmentModel,
					this.serviceItemAssignmentsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.serviceItemAssignmentsRelations,
					true
				);
			...
			if (event.entityName === this.tariffPackageItemsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffPackageItemModel.getProps(), this.tariffPackageItemsRelations, this.isViewOnly),
					new TariffPackageItemModel(),
					TariffPackageItemModel,
					this.tariffPackageItemsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.tariffPackageItemsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.tariffPackageServiceAssignmentsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffPackageServiceAssignmentModel.getProps(), this.tariffPackageServiceAssignmentsRelations, this.isViewOnly),
					new TariffPackageServiceAssignmentModel(),
					TariffPackageServiceAssignmentModel,
					this.tariffPackageServiceAssignmentsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.tariffPackageServiceAssignmentsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TariffPackageModel.getProps();
		this.modelRelations = TariffPackageModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'serviceItemAssignments';
		}).hideElement = this.serviceItemAssignmentsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'serviceItemAssignments').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'tariffPackageItems';
		}).hideElement = this.tariffPackageItemsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'tariffPackageItems').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'tariffPackageServiceAssignments';
		}).hideElement = this.tariffPackageServiceAssignmentsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'tariffPackageServiceAssignments').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.serviceItemAssignmentsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.serviceItemAssignmentsDisplayName, many: true});
			this.store.dispatch(new serviceItemAssignmentModelAction.ServiceItemAssignmentAction(
				serviceItemAssignmentModelAction.ServiceItemAssignmentModelActionTypes.INITIALISE_SERVICE_ITEM_ASSIGNMENT_COLLECTION_STATE,
				{
					collectionId: this.serviceItemAssignmentsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getServiceItemAssignmentCollectionModels, this.serviceItemAssignmentsCollectionId).subscribe(models => {
				models.forEach(serviceItemAssignments => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(ServiceItemAssignmentModel.getProps(), this.serviceItemAssignmentsRelations, this.isViewOnly),
		...
		if (this.tariffPackageItemsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.tariffPackageItemsDisplayName, many: true});
			this.store.dispatch(new tariffPackageItemModelAction.TariffPackageItemAction(
				tariffPackageItemModelAction.TariffPackageItemModelActionTypes.INITIALISE_TARIFF_PACKAGE_ITEM_COLLECTION_STATE,
				{
					collectionId: this.tariffPackageItemsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffPackageItemCollectionModels, this.tariffPackageItemsCollectionId).subscribe(models => {
				models.forEach(tariffPackageItems => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(TariffPackageItemModel.getProps(), this.tariffPackageItemsRelations, this.isViewOnly),
						tariffPackageItems,
						TariffPackageItemModel,
		...
		if (this.tariffPackageServiceAssignmentsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.tariffPackageServiceAssignmentsDisplayName, many: true});
			this.store.dispatch(new tariffPackageServiceAssignmentModelAction.TariffPackageServiceAssignmentAction(
				tariffPackageServiceAssignmentModelAction.TariffPackageServiceAssignmentModelActionTypes.INITIALISE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_COLLECTION_STATE,
				{
					collectionId: this.tariffPackageServiceAssignmentsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffPackageServiceAssignmentCollectionModels, this.tariffPackageServiceAssignmentsCollectionId).subscribe(models => {
				models.forEach(tariffPackageServiceAssignments => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(TariffPackageServiceAssignmentModel.getProps(), this.tariffPackageServiceAssignmentsRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.serviceItemAssignmentsIds && this.targetModel.serviceItemAssignmentsIds.length > 0 && this.serviceItemAssignmentsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				ServiceItemAssignmentModel,
				this.serviceItemAssignmentsOppositeRelation,
				this.serviceItemAssignmentsCollectionId,
				this.targetModel.serviceItemAssignmentsIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.tariffPackageItemsIds && this.targetModel.tariffPackageItemsIds.length > 0 && this.tariffPackageItemsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffPackageItemModel,
				this.tariffPackageItemsOppositeRelation,
				this.tariffPackageItemsCollectionId,
				this.targetModel.tariffPackageItemsIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.tariffPackageServiceAssignmentsIds && this.targetModel.tariffPackageServiceAssignmentsIds.length > 0 && this.tariffPackageServiceAssignmentsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffPackageServiceAssignmentModel,
				this.tariffPackageServiceAssignmentsOppositeRelation,
				this.tariffPackageServiceAssignmentsCollectionId,
				this.targetModel.tariffPackageServiceAssignmentsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: TariffPackageModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TariffPackageModel {
		let updatedModel: TariffPackageModel = new TariffPackageModel(model);

		updatedModel.serviceItemAssignments = [];
		if (this.serviceItemAssignmentsMultiCrudActive) {
			if (event.additionalEntities.serviceItemAssignments) {
				event.additionalEntities.serviceItemAssignments.forEach(entity => {
					updatedModel.serviceItemAssignments.push(entity);
				});
			}

			if (!updatedModel.serviceItemAssignments.length) {
				updatedModel.serviceItemAssignmentsIds = [];
			}
		}
		updatedModel.tariffPackageItems = [];
		if (this.tariffPackageItemsMultiCrudActive) {
			if (event.additionalEntities.tariffPackageItems) {
				event.additionalEntities.tariffPackageItems.forEach(entity => {
					updatedModel.tariffPackageItems.push(entity);
				});
			}

			if (!updatedModel.tariffPackageItems.length) {
				updatedModel.tariffPackageItemsIds = [];
			}
		}
		updatedModel.tariffPackageServiceAssignments = [];
		if (this.tariffPackageServiceAssignmentsMultiCrudActive) {
			if (event.additionalEntities.tariffPackageServiceAssignments) {
				event.additionalEntities.tariffPackageServiceAssignments.forEach(entity => {
					updatedModel.tariffPackageServiceAssignments.push(entity);
				});
			}

			if (!updatedModel.tariffPackageServiceAssignments.length) {
				updatedModel.tariffPackageServiceAssignmentsIds = [];
			}
		}
		return updatedModel;
	}
