...
import {TariffPackageModelState} from 'src/app/models/tariffPackage/tariff_package.model.state';
...
import {
	getTariffPackageCollectionModels,
	getTariffPackageCollectionCount
...
@Component({
	selector: 'cb-tariff-package-admin-crud-list',
	templateUrl: './tariff-package-admin-crud-list.component.html',
	styleUrls: [
		'./tariff-package-admin-crud-list.component.scss',
...
export class AdminTariffPackageCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TariffPackageModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TariffPackageModel.searchFields,
	...
	/**
	 * List of Tariff Packages that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	tariffPackages: Observable<TariffPackageModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	tariffPackagesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.tariffPackagesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.tariffPackagesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffPackageAction(
				modelAction.TariffPackageModelActionTypes.INITIALISE_TARIFF_PACKAGE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffPackageAction(
				modelAction.TariffPackageModelActionTypes.FETCH_TARIFF_PACKAGE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.tariffPackages = this.store.select(getTariffPackageCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTariffPackageCollectionCount, this.collectionId).subscribe(
				(count) => this.tariffPackagesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TariffPackageModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TariffPackageModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TariffPackageModelActionTypes.EXPORT_ALL_TARIFF_PACKAGE;
		} else {
			actionType = modelAction.TariffPackageModelActionTypes.EXPORT_TARIFF_PACKAGE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TariffPackageAction(
				modelAction.TariffPackageModelActionTypes.IMPORT_TARIFF_PACKAGES,
				{
					file: file,
				},
				[new modelAction.TariffPackageAction(
			...
			window.open(environment.API_URL + '/api/tariffpackage/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TariffPackageModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Tariff Package entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Tariff Package entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TariffPackageModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TariffPackageModelActionTypes.DELETE_TARIFF_PACKAGE;
						} else if (usingSelected) {
							actionType = modelAction.TariffPackageModelActionTypes.DELETE_ALL_TARIFF_PACKAGE;
						} else {
							actionType = modelAction.TariffPackageModelActionTypes.DELETE_TARIFF_PACKAGE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TariffPackageAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TARIFF_PACKAGE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TARIFF_PACKAGE and DELETE_TARIFF_PACKAGE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TariffPackageModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.tariffPackagesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
