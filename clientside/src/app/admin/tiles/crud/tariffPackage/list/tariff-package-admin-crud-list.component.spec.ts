...
import {AdminTariffPackageCrudListComponent} from './tariff-package-admin-crud-list.component';
...
import {AdminTariffPackageCrudModule} from '../tariff-package-admin-crud.module';
import {
	TariffPackageModelState,
	initialState as initialTariffPackageModelState
} from 'src/app/models/tariffPackage/tariff_package.model.state';
 ...
 */
describe('Tariff Package Crud List Tile Component', () => {

	let fixture;
	let tariffPackageCrudListComponent: AdminTariffPackageCrudListComponent;

	let store: MockStore<{ model: TariffPackageModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let tariffPackageModelState: TariffPackageModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<TariffPackageModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd6554fc0-7906-4a3b-b3e1-0840e7bcf616';
	const searchContent = 'text';

	const searchableFieldCount = TariffPackageModel.searchFields.length;

	const routerState: RouterState = {
		url: 'tariff-package-admin-crud',
		urls: ['tariff-package-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const tariffPackageDataFactory = new TariffPackageDataFactory();

	const defaultDataSize = 30;
	const defaultData = tariffPackageDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: TariffPackageModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		tariffPackageModelState = initialTariffPackageModelState;
		store.setState({model: tariffPackageModelState});
		...
		spyOn(store, 'select')
			.withArgs(getTariffPackageCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTariffPackageCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminTariffPackageCrudListComponent);
			tariffPackageCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(tariffPackageCrudListComponent.tariffPackagesCount).toBe(defaultData.length);
			...
			expect(tariffPackageCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			tariffPackageCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = tariffPackageDataFactory.createAll(filteredDataSize);
			...
			expect(tariffPackageCrudListComponent.tariffPackagesCount).toBe(filteredDataSize);
			expect(tariffPackageCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(tariffPackageCrudListComponent.pageIndex).toBe(0);
			expect(tariffPackageCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			tariffPackageCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		tariffPackageCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		tariffPackageCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
