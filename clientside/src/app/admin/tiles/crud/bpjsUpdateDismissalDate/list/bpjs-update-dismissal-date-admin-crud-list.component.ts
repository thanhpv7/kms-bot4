...
import * as modelAction from 'src/app/models/bpjsUpdateDismissalDate/bpjs_update_dismissal_date.model.action';
import {BpjsUpdateDismissalDateModelState} from 'src/app/models/bpjsUpdateDismissalDate/bpjs_update_dismissal_date.model.state';
...
import {
	getBpjsUpdateDismissalDateCollectionModels,
	getBpjsUpdateDismissalDateCollectionCount
} from 'src/app/models/bpjsUpdateDismissalDate/bpjs_update_dismissal_date.model.selector';
...
import {CrudTileMode} from '../bpjs-update-dismissal-date-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-update-dismissal-date-admin-crud-list',
	templateUrl: './bpjs-update-dismissal-date-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-update-dismissal-date-admin-crud-list.component.scss',
...
export class AdminBpjsUpdateDismissalDateCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsUpdateDismissalDateModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsUpdateDismissalDateModel.searchFields,
	...
	/**
	 * List of BPJS Update Dismissal Dates that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsUpdateDismissalDates: Observable<BpjsUpdateDismissalDateModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsUpdateDismissalDatesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsUpdateDismissalDateModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsUpdateDismissalDateModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '0f3eb809-75ea-4580-b385-1c641908dd31';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsUpdateDismissalDatesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsUpdateDismissalDatesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsUpdateDismissalDateModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
				modelAction.BpjsUpdateDismissalDateModelActionTypes.INITIALISE_BPJS_UPDATE_DISMISSAL_DATE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
				modelAction.BpjsUpdateDismissalDateModelActionTypes.FETCH_BPJS_UPDATE_DISMISSAL_DATE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsUpdateDismissalDateAction(
						modelAction.BpjsUpdateDismissalDateModelActionTypes.COUNT_BPJS_UPDATE_DISMISSAL_DATES,
						null
					)
				]
			))
		...
		this.bpjsUpdateDismissalDates = this.store.select(getBpjsUpdateDismissalDateCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsUpdateDismissalDateCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsUpdateDismissalDatesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-update-dismissal-date');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsUpdateDismissalDateModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsUpdateDismissalDateModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsUpdateDismissalDateModelActionTypes.EXPORT_ALL_BPJS_UPDATE_DISMISSAL_DATE;
		} else {
			actionType = modelAction.BpjsUpdateDismissalDateModelActionTypes.EXPORT_BPJS_UPDATE_DISMISSAL_DATE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
				modelAction.BpjsUpdateDismissalDateModelActionTypes.IMPORT_BPJS_UPDATE_DISMISSAL_DATES,
				{
					file: file,
				},
				[new modelAction.BpjsUpdateDismissalDateAction(
			...
			window.open(environment.API_URL + '/api/bpjsupdatedismissaldate/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsUpdateDismissalDateModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Update Dismissal Date entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Update Dismissal Date entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsUpdateDismissalDateModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsUpdateDismissalDateModelActionTypes.DELETE_BPJS_UPDATE_DISMISSAL_DATE;
						} else if (usingSelected) {
							actionType = modelAction.BpjsUpdateDismissalDateModelActionTypes.DELETE_ALL_BPJS_UPDATE_DISMISSAL_DATE;
						} else {
							actionType = modelAction.BpjsUpdateDismissalDateModelActionTypes.DELETE_BPJS_UPDATE_DISMISSAL_DATE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_UPDATE_DISMISSAL_DATE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_UPDATE_DISMISSAL_DATE and DELETE_BPJS_UPDATE_DISMISSAL_DATE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsUpdateDismissalDateModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsUpdateDismissalDateModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsUpdateDismissalDatesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
