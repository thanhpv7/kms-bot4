...
import {AdminBpjsUpdateDismissalDateCrudListComponent} from './bpjs-update-dismissal-date-admin-crud-list.component';
import {BpjsUpdateDismissalDateDataFactory} from 'src/app/lib/utils/factories/bpjs-update-dismissal-date-data-factory';
import {BpjsUpdateDismissalDateModel} from 'src/app/models/bpjsUpdateDismissalDate/bpjs_update_dismissal_date.model';
import {AdminBpjsUpdateDismissalDateCrudModule} from '../bpjs-update-dismissal-date-admin-crud.module';
import {
	BpjsUpdateDismissalDateModelState,
	initialState as initialBpjsUpdateDismissalDateModelState
} from 'src/app/models/bpjsUpdateDismissalDate/bpjs_update_dismissal_date.model.state';
 ...
 */
describe('BPJS Update Dismissal Date Crud List Tile Component', () => {

	let fixture;
	let bpjsUpdateDismissalDateCrudListComponent: AdminBpjsUpdateDismissalDateCrudListComponent;

	let store: MockStore<{ model: BpjsUpdateDismissalDateModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsUpdateDismissalDateModelState: BpjsUpdateDismissalDateModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsUpdateDismissalDateModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '0f3eb809-75ea-4580-b385-1c641908dd31';
	const searchContent = 'text';

	const searchableFieldCount = BpjsUpdateDismissalDateModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-update-dismissal-date-admin-crud',
		urls: ['bpjs-update-dismissal-date-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsUpdateDismissalDateDataFactory = new BpjsUpdateDismissalDateDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsUpdateDismissalDateDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsUpdateDismissalDateModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsUpdateDismissalDateModelState = initialBpjsUpdateDismissalDateModelState;
		store.setState({model: bpjsUpdateDismissalDateModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsUpdateDismissalDateCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsUpdateDismissalDateCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsUpdateDismissalDateCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsUpdateDismissalDateCrudListComponent);
			bpjsUpdateDismissalDateCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsUpdateDismissalDateCrudListComponent.bpjsUpdateDismissalDatesCount).toBe(defaultData.length);
			...
			expect(bpjsUpdateDismissalDateCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsUpdateDismissalDateCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsUpdateDismissalDateDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsUpdateDismissalDateCrudListComponent.bpjsUpdateDismissalDatesCount).toBe(filteredDataSize);
			expect(bpjsUpdateDismissalDateCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsUpdateDismissalDateCrudListComponent.pageIndex).toBe(0);
			expect(bpjsUpdateDismissalDateCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsUpdateDismissalDateCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsUpdateDismissalDateCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsUpdateDismissalDateCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
