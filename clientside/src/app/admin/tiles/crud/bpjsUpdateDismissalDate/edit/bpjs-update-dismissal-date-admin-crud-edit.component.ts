...
import {BpjsUpdateDismissalDateModelState, BpjsUpdateDismissalDateModelAudit} from 'src/app/models/bpjsUpdateDismissalDate/bpjs_update_dismissal_date.model.state';
import {BpjsUpdateDismissalDateModel} from 'src/app/models/bpjsUpdateDismissalDate/bpjs_update_dismissal_date.model';
import {
	getBpjsUpdateDismissalDateModelAuditsByEntityId,
	getBpjsUpdateDismissalDateModelWithId,
	getCountBpjsUpdateDismissalDateModels,
...
@Component({
	selector: 'cb-bpjs-update-dismissal-date-admin-crud-edit',
	templateUrl: './bpjs-update-dismissal-date-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-update-dismissal-date-admin-crud-edit.component.scss',
...
export class AdminBpjsUpdateDismissalDateCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsUpdateDismissalDateAudits$: Observable<BpjsUpdateDismissalDateModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsUpdateDismissalDateModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsUpdateDismissalDateModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsUpdateDismissalDateModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsUpdateDismissalDateModel();
		...
		if (this.targetModelId) {
			this.bpjsUpdateDismissalDateAudits$ = this.store.select(getBpjsUpdateDismissalDateModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsUpdateDismissalDateModel> = {
		...
		this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
			modelAction.BpjsUpdateDismissalDateModelActionTypes.FETCH_BPJS_UPDATE_DISMISSAL_DATE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsUpdateDismissalDateModelWithId, this.targetModelId)
			...
			model = new BpjsUpdateDismissalDateModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
			modelAction.BpjsUpdateDismissalDateModelActionTypes.FETCH_BPJS_UPDATE_DISMISSAL_DATE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
				modelAction.BpjsUpdateDismissalDateModelActionTypes.CREATE_BPJS_UPDATE_DISMISSAL_DATE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsUpdateDismissalDateAction(
				modelAction.BpjsUpdateDismissalDateModelActionTypes.UPDATE_BPJS_UPDATE_DISMISSAL_DATE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsUpdateDismissalDateModel = new BpjsUpdateDismissalDateModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsUpdateDismissalDateModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsUpdateDismissalDateModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsUpdateDismissalDateModel.getProps();
		this.modelRelations = BpjsUpdateDismissalDateModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsUpdateDismissalDateModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsUpdateDismissalDateModel {
		let updatedModel: BpjsUpdateDismissalDateModel = new BpjsUpdateDismissalDateModel(model);
