...
import {InventoryAdjustmentItemModelState} from 'src/app/models/inventoryAdjustmentItem/inventory_adjustment_item.model.state';
...
import {
	getInventoryAdjustmentItemCollectionModels,
	getInventoryAdjustmentItemCollectionCount
...
@Component({
	selector: 'cb-inventory-adjustment-item-admin-crud-list',
	templateUrl: './inventory-adjustment-item-admin-crud-list.component.html',
	styleUrls: [
		'./inventory-adjustment-item-admin-crud-list.component.scss',
...
export class AdminInventoryAdjustmentItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InventoryAdjustmentItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InventoryAdjustmentItemModel.searchFields,
	...
	/**
	 * List of Inventory Adjustment Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	inventoryAdjustmentItems: Observable<InventoryAdjustmentItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	inventoryAdjustmentItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.inventoryAdjustmentItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.inventoryAdjustmentItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
				modelAction.InventoryAdjustmentItemModelActionTypes.INITIALISE_INVENTORY_ADJUSTMENT_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
				modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.inventoryAdjustmentItems = this.store.select(getInventoryAdjustmentItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInventoryAdjustmentItemCollectionCount, this.collectionId).subscribe(
				(count) => this.inventoryAdjustmentItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InventoryAdjustmentItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InventoryAdjustmentItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InventoryAdjustmentItemModelActionTypes.EXPORT_ALL_INVENTORY_ADJUSTMENT_ITEM;
		} else {
			actionType = modelAction.InventoryAdjustmentItemModelActionTypes.EXPORT_INVENTORY_ADJUSTMENT_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
				modelAction.InventoryAdjustmentItemModelActionTypes.IMPORT_INVENTORY_ADJUSTMENT_ITEMS,
				{
					file: file,
				},
				[new modelAction.InventoryAdjustmentItemAction(
			...
			window.open(environment.API_URL + '/api/inventoryadjustmentitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InventoryAdjustmentItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Inventory Adjustment Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Inventory Adjustment Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InventoryAdjustmentItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InventoryAdjustmentItemModelActionTypes.DELETE_INVENTORY_ADJUSTMENT_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.InventoryAdjustmentItemModelActionTypes.DELETE_ALL_INVENTORY_ADJUSTMENT_ITEM;
						} else {
							actionType = modelAction.InventoryAdjustmentItemModelActionTypes.DELETE_INVENTORY_ADJUSTMENT_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVENTORY_ADJUSTMENT_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVENTORY_ADJUSTMENT_ITEM and DELETE_INVENTORY_ADJUSTMENT_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InventoryAdjustmentItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.inventoryAdjustmentItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
