...
import {AdminInventoryAdjustmentItemCrudListComponent} from './inventory-adjustment-item-admin-crud-list.component';
...
import {AdminInventoryAdjustmentItemCrudModule} from '../inventory-adjustment-item-admin-crud.module';
import {
	InventoryAdjustmentItemModelState,
	initialState as initialInventoryAdjustmentItemModelState
} from 'src/app/models/inventoryAdjustmentItem/inventory_adjustment_item.model.state';
 ...
 */
describe('Inventory Adjustment Item Crud List Tile Component', () => {

	let fixture;
	let inventoryAdjustmentItemCrudListComponent: AdminInventoryAdjustmentItemCrudListComponent;

	let store: MockStore<{ model: InventoryAdjustmentItemModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let inventoryAdjustmentItemModelState: InventoryAdjustmentItemModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InventoryAdjustmentItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'fabdb9b1-f85d-4098-9a2d-7591af0de7dd';
	const searchContent = 'text';

	const searchableFieldCount = InventoryAdjustmentItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'inventory-adjustment-item-admin-crud',
		urls: ['inventory-adjustment-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const inventoryAdjustmentItemDataFactory = new InventoryAdjustmentItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = inventoryAdjustmentItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InventoryAdjustmentItemModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		inventoryAdjustmentItemModelState = initialInventoryAdjustmentItemModelState;
		store.setState({model: inventoryAdjustmentItemModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInventoryAdjustmentItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInventoryAdjustmentItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInventoryAdjustmentItemCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInventoryAdjustmentItemCrudListComponent);
			inventoryAdjustmentItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(inventoryAdjustmentItemCrudListComponent.inventoryAdjustmentItemsCount).toBe(defaultData.length);
			...
			expect(inventoryAdjustmentItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			inventoryAdjustmentItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = inventoryAdjustmentItemDataFactory.createAll(filteredDataSize);
			...
			expect(inventoryAdjustmentItemCrudListComponent.inventoryAdjustmentItemsCount).toBe(filteredDataSize);
			expect(inventoryAdjustmentItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(inventoryAdjustmentItemCrudListComponent.pageIndex).toBe(0);
			expect(inventoryAdjustmentItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			inventoryAdjustmentItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		inventoryAdjustmentItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		inventoryAdjustmentItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
