...
import * as modelAction from 'src/app/models/inventoryAdjustmentItem/inventory_adjustment_item.model.action';
import {InventoryAdjustmentItemModelState, InventoryAdjustmentItemModelAudit} from 'src/app/models/inventoryAdjustmentItem/inventory_adjustment_item.model.state';
import {InventoryAdjustmentItemModel} from 'src/app/models/inventoryAdjustmentItem/inventory_adjustment_item.model';
import {
	getInventoryAdjustmentItemModelAuditsByEntityId,
	getInventoryAdjustmentItemModelWithId,
	getCountInventoryAdjustmentItemModels,
...
import {CrudTileMode} from '../inventory-adjustment-item-admin-crud.component';
...
@Component({
	selector: 'cb-inventory-adjustment-item-admin-crud-edit',
	templateUrl: './inventory-adjustment-item-admin-crud-edit.component.html',
	styleUrls: [
		'./inventory-adjustment-item-admin-crud-edit.component.scss',
...
export class AdminInventoryAdjustmentItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'fabdb9b1-f85d-4098-9a2d-7591af0de7dd';
	 ...
	 */
	inventoryAdjustmentItemAudits$: Observable<InventoryAdjustmentItemModelAudit[]>;
	...
	inventoryAdjustmentMultiCrudActive: boolean = false;
	inventoryAdjustmentDisplayName = 'Inventory Adjustment';
	...
	inventoryAdjustmentOppositeRelation = 'inventoryAdjustmentItems';
	inventoryAdjustmentRelations: ModelRelation[] = this.inventoryAdjustmentMultiCrudActive ? InventoryAdjustmentModel.getRelations()
		.map(relation => {
			if (relation.id === this.inventoryAdjustmentOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'inventoryAdjustmentItems';
	...
	inventoryAdjusmentBatchsOppositeRelation = 'inventoryAdjustmentItem';
	 ...
	 */
	modelProperties: ModelProperty[] = InventoryAdjustmentItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InventoryAdjustmentItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InventoryAdjustmentItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'inventoryAdjustment' && this.inventoryAdjustmentMultiCrudActive) {
	 ...
	 */
	targetModel: InventoryAdjustmentItemModel;
	...
	constructor(
		private readonly store: Store<{ model: InventoryAdjustmentItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInventoryAdjustmentItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InventoryAdjustmentItemModel();
		...
		if (this.targetModelId) {
			this.inventoryAdjustmentItemAudits$ = this.store.select(getInventoryAdjustmentItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InventoryAdjustmentItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
			modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInventoryAdjustmentItemModelWithId, this.targetModelId)
			...
			model = new InventoryAdjustmentItemModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('inventory-adjustment-item');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
			modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InventoryAdjustmentItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.inventoryAdjustmentMultiCrudActive || this.stockCatalogueMultiCrudActive || this.inventoryAdjusmentBatchsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InventoryAdjustmentItemAction(
						modelAction.InventoryAdjustmentItemModelActionTypes.COUNT_INVENTORY_ADJUSTMENT_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
				modelAction.InventoryAdjustmentItemModelActionTypes.CREATE_INVENTORY_ADJUSTMENT_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InventoryAdjustmentItemAction(
				modelAction.InventoryAdjustmentItemModelActionTypes.UPDATE_INVENTORY_ADJUSTMENT_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InventoryAdjustmentItemModel = new InventoryAdjustmentItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInventoryAdjustmentItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InventoryAdjustmentItemModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.inventoryAdjustmentDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InventoryAdjustmentModel.getProps(), this.inventoryAdjustmentRelations, this.isViewOnly),
					new InventoryAdjustmentModel(),
					InventoryAdjustmentModel,
					this.inventoryAdjustmentDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.inventoryAdjustmentRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InventoryAdjustmentItemModel.getProps();
		this.modelRelations = InventoryAdjustmentItemModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'inventoryAdjustment';
		}).hideElement = this.inventoryAdjustmentMultiCrudActive || this.modelRelations.find(rel => rel.id === 'inventoryAdjustment').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.inventoryAdjustmentMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.inventoryAdjustmentDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.inventoryAdjustment && this.targetModel.inventoryAdjustment.id && this.inventoryAdjustmentMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InventoryAdjustmentModel,
				this.inventoryAdjustmentOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.inventoryAdjustment.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getInventoryAdjustmentModelWithId, this.targetModel.inventoryAdjustmentId).subscribe(inventoryAdjustment => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InventoryAdjustmentModel.getProps(), this.inventoryAdjustmentRelations, this.isViewOnly),
					inventoryAdjustment,
					InventoryAdjustmentModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: InventoryAdjustmentItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InventoryAdjustmentItemModel {
		let updatedModel: InventoryAdjustmentItemModel = new InventoryAdjustmentItemModel(model);

		if (this.inventoryAdjustmentMultiCrudActive) {
			updatedModel.inventoryAdjustment = event.additionalEntities.inventoryAdjustment ? event.additionalEntities.inventoryAdjustment : undefined;

			if (!updatedModel.inventoryAdjustment && event.payload) {
				event.payload.inventoryAdjustmentId = null;
			}
		} else if (event.payload?.inventoryAdjustmentId !== undefined) {
			event.payload.inventoryAdjustment = null;
			updatedModel.inventoryAdjustment = null;
		}
