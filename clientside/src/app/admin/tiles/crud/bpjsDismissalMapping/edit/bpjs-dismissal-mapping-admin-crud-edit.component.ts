...
import * as modelAction from 'src/app/models/bpjsDismissalMapping/bpjs_dismissal_mapping.model.action';
import {BpjsDismissalMappingModelState, BpjsDismissalMappingModelAudit} from 'src/app/models/bpjsDismissalMapping/bpjs_dismissal_mapping.model.state';
import {BpjsDismissalMappingModel} from 'src/app/models/bpjsDismissalMapping/bpjs_dismissal_mapping.model';
import {
	getBpjsDismissalMappingModelAuditsByEntityId,
...
import {CrudTileMode} from '../bpjs-dismissal-mapping-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-dismissal-mapping-admin-crud-edit',
	templateUrl: './bpjs-dismissal-mapping-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-dismissal-mapping-admin-crud-edit.component.scss',
...
export class AdminBpjsDismissalMappingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '45ea47c4-6b49-4906-89ca-6d276cdfe08a';
	 ...
	 */
	bpjsDismissalMappingAudits$: Observable<BpjsDismissalMappingModelAudit[]>;
	...
	bpjsClaimSubmissionOppositeRelation = 'bpjsDismissalMappings';
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsDismissalMappingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsDismissalMappingModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsDismissalMappingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsDismissalMappingModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsDismissalMappingModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsDismissalMappingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsDismissalMappingModel();
		...
		if (this.targetModelId) {
			this.bpjsDismissalMappingAudits$ = this.store.select(getBpjsDismissalMappingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsDismissalMappingModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
			modelAction.BpjsDismissalMappingModelActionTypes.FETCH_BPJS_DISMISSAL_MAPPING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsDismissalMappingModelWithId, this.targetModelId)
			...
			model = new BpjsDismissalMappingModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-dismissal-mapping');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
			modelAction.BpjsDismissalMappingModelActionTypes.FETCH_BPJS_DISMISSAL_MAPPING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsDismissalMappingModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsDismissalMappingAction(
						modelAction.BpjsDismissalMappingModelActionTypes.COUNT_BPJS_DISMISSAL_MAPPINGS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
				modelAction.BpjsDismissalMappingModelActionTypes.CREATE_BPJS_DISMISSAL_MAPPING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
				modelAction.BpjsDismissalMappingModelActionTypes.UPDATE_BPJS_DISMISSAL_MAPPING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsDismissalMappingModel = new BpjsDismissalMappingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsDismissalMappingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsDismissalMappingModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsDismissalMappingModel.getProps();
		this.modelRelations = BpjsDismissalMappingModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsDismissalMappingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsDismissalMappingModel {
		let updatedModel: BpjsDismissalMappingModel = new BpjsDismissalMappingModel(model);
