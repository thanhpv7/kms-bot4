...
import {BpjsDismissalMappingModelState} from 'src/app/models/bpjsDismissalMapping/bpjs_dismissal_mapping.model.state';
...
import {
	...
	getBpjsDismissalMappingCollectionCount
...
@Component({
	selector: 'cb-bpjs-dismissal-mapping-admin-crud-list',
	templateUrl: './bpjs-dismissal-mapping-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-dismissal-mapping-admin-crud-list.component.scss',
...
export class AdminBpjsDismissalMappingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsDismissalMappingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsDismissalMappingModel.searchFields,
	...
	/**
	 * List of BPJS Dismissal Mappings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsDismissalMappings: Observable<BpjsDismissalMappingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsDismissalMappingsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsDismissalMappingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsDismissalMappingsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
				modelAction.BpjsDismissalMappingModelActionTypes.INITIALISE_BPJS_DISMISSAL_MAPPING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
				modelAction.BpjsDismissalMappingModelActionTypes.FETCH_BPJS_DISMISSAL_MAPPING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsDismissalMappings = this.store.select(getBpjsDismissalMappingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsDismissalMappingCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsDismissalMappingsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsDismissalMappingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsDismissalMappingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsDismissalMappingModelActionTypes.EXPORT_ALL_BPJS_DISMISSAL_MAPPING;
		} else {
			actionType = modelAction.BpjsDismissalMappingModelActionTypes.EXPORT_BPJS_DISMISSAL_MAPPING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
				modelAction.BpjsDismissalMappingModelActionTypes.IMPORT_BPJS_DISMISSAL_MAPPINGS,
				{
					file: file,
				},
				[new modelAction.BpjsDismissalMappingAction(
			...
			window.open(environment.API_URL + '/api/bpjsdismissalmapping/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsDismissalMappingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Dismissal Mapping entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Dismissal Mapping entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsDismissalMappingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsDismissalMappingModelActionTypes.DELETE_BPJS_DISMISSAL_MAPPING;
						} else if (usingSelected) {
							actionType = modelAction.BpjsDismissalMappingModelActionTypes.DELETE_ALL_BPJS_DISMISSAL_MAPPING;
						} else {
							actionType = modelAction.BpjsDismissalMappingModelActionTypes.DELETE_BPJS_DISMISSAL_MAPPING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsDismissalMappingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_DISMISSAL_MAPPING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_DISMISSAL_MAPPING and DELETE_BPJS_DISMISSAL_MAPPING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsDismissalMappingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsDismissalMappingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
