...
import {AdminBpjsDismissalMappingCrudListComponent} from './bpjs-dismissal-mapping-admin-crud-list.component';
...
import {AdminBpjsDismissalMappingCrudModule} from '../bpjs-dismissal-mapping-admin-crud.module';
import {
 ...
 */
describe('BPJS Dismissal Mapping Crud List Tile Component', () => {

	let fixture;
	let bpjsDismissalMappingCrudListComponent: AdminBpjsDismissalMappingCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsDismissalMappingModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '45ea47c4-6b49-4906-89ca-6d276cdfe08a';
	const searchContent = 'text';

	const searchableFieldCount = BpjsDismissalMappingModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-dismissal-mapping-admin-crud',
		urls: ['bpjs-dismissal-mapping-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsDismissalMappingDataFactory = new BpjsDismissalMappingDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsDismissalMappingDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsDismissalMappingCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsDismissalMappingCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsDismissalMappingCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsDismissalMappingCrudListComponent);
			bpjsDismissalMappingCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsDismissalMappingCrudListComponent.bpjsDismissalMappingsCount).toBe(defaultData.length);
			...
			expect(bpjsDismissalMappingCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsDismissalMappingCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsDismissalMappingDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsDismissalMappingCrudListComponent.bpjsDismissalMappingsCount).toBe(filteredDataSize);
			expect(bpjsDismissalMappingCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsDismissalMappingCrudListComponent.pageIndex).toBe(0);
			expect(bpjsDismissalMappingCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsDismissalMappingCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsDismissalMappingCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsDismissalMappingCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
