...
import {HemodialysisMonitoringModelState} from 'src/app/models/hemodialysisMonitoring/hemodialysis_monitoring.model.state';
...
import {
	getHemodialysisMonitoringCollectionModels,
	getHemodialysisMonitoringCollectionCount
...
@Component({
	selector: 'cb-hemodialysis-monitoring-admin-crud-list',
	templateUrl: './hemodialysis-monitoring-admin-crud-list.component.html',
	styleUrls: [
		'./hemodialysis-monitoring-admin-crud-list.component.scss',
...
export class AdminHemodialysisMonitoringCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<HemodialysisMonitoringModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...HemodialysisMonitoringModel.searchFields,
	...
	/**
	 * List of Hemodialysis Monitorings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	hemodialysisMonitorings: Observable<HemodialysisMonitoringModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	hemodialysisMonitoringsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.hemodialysisMonitoringsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.hemodialysisMonitoringsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
				modelAction.HemodialysisMonitoringModelActionTypes.INITIALISE_HEMODIALYSIS_MONITORING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
				modelAction.HemodialysisMonitoringModelActionTypes.FETCH_HEMODIALYSIS_MONITORING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.hemodialysisMonitorings = this.store.select(getHemodialysisMonitoringCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getHemodialysisMonitoringCollectionCount, this.collectionId).subscribe(
				(count) => this.hemodialysisMonitoringsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: HemodialysisMonitoringModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.HemodialysisMonitoringModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.HemodialysisMonitoringModelActionTypes.EXPORT_ALL_HEMODIALYSIS_MONITORING;
		} else {
			actionType = modelAction.HemodialysisMonitoringModelActionTypes.EXPORT_HEMODIALYSIS_MONITORING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
				modelAction.HemodialysisMonitoringModelActionTypes.IMPORT_HEMODIALYSIS_MONITORINGS,
				{
					file: file,
				},
				[new modelAction.HemodialysisMonitoringAction(
			...
			window.open(environment.API_URL + '/api/hemodialysismonitoring/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: HemodialysisMonitoringModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Hemodialysis Monitoring entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Hemodialysis Monitoring entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.HemodialysisMonitoringModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.HemodialysisMonitoringModelActionTypes.DELETE_HEMODIALYSIS_MONITORING;
						} else if (usingSelected) {
							actionType = modelAction.HemodialysisMonitoringModelActionTypes.DELETE_ALL_HEMODIALYSIS_MONITORING;
						} else {
							actionType = modelAction.HemodialysisMonitoringModelActionTypes.DELETE_HEMODIALYSIS_MONITORING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_HEMODIALYSIS_MONITORING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_HEMODIALYSIS_MONITORING and DELETE_HEMODIALYSIS_MONITORING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = HemodialysisMonitoringModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.hemodialysisMonitoringsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
