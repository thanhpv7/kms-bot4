...
import {AdminHemodialysisMonitoringCrudListComponent} from './hemodialysis-monitoring-admin-crud-list.component';
...
import {AdminHemodialysisMonitoringCrudModule} from '../hemodialysis-monitoring-admin-crud.module';
import {
	HemodialysisMonitoringModelState,
	initialState as initialHemodialysisMonitoringModelState
} from 'src/app/models/hemodialysisMonitoring/hemodialysis_monitoring.model.state';
 ...
 */
describe('Hemodialysis Monitoring Crud List Tile Component', () => {

	let fixture;
	let hemodialysisMonitoringCrudListComponent: AdminHemodialysisMonitoringCrudListComponent;

	let store: MockStore<{ model: HemodialysisMonitoringModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let hemodialysisMonitoringModelState: HemodialysisMonitoringModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<HemodialysisMonitoringModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a2ad6a8a-7569-4af7-9a0e-fb7236ce68f6';
	const searchContent = 'text';

	const searchableFieldCount = HemodialysisMonitoringModel.searchFields.length;

	const routerState: RouterState = {
		url: 'hemodialysis-monitoring-admin-crud',
		urls: ['hemodialysis-monitoring-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const hemodialysisMonitoringDataFactory = new HemodialysisMonitoringDataFactory();

	const defaultDataSize = 30;
	const defaultData = hemodialysisMonitoringDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: HemodialysisMonitoringModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		hemodialysisMonitoringModelState = initialHemodialysisMonitoringModelState;
		store.setState({model: hemodialysisMonitoringModelState});
		...
		spyOn(store, 'select')
			.withArgs(getHemodialysisMonitoringCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getHemodialysisMonitoringCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminHemodialysisMonitoringCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminHemodialysisMonitoringCrudListComponent);
			hemodialysisMonitoringCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(hemodialysisMonitoringCrudListComponent.hemodialysisMonitoringsCount).toBe(defaultData.length);
			...
			expect(hemodialysisMonitoringCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			hemodialysisMonitoringCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = hemodialysisMonitoringDataFactory.createAll(filteredDataSize);
			...
			expect(hemodialysisMonitoringCrudListComponent.hemodialysisMonitoringsCount).toBe(filteredDataSize);
			expect(hemodialysisMonitoringCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(hemodialysisMonitoringCrudListComponent.pageIndex).toBe(0);
			expect(hemodialysisMonitoringCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			hemodialysisMonitoringCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		hemodialysisMonitoringCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		hemodialysisMonitoringCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
