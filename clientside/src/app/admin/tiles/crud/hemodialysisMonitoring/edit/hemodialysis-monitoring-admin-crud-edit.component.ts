...
import * as modelAction from 'src/app/models/hemodialysisMonitoring/hemodialysis_monitoring.model.action';
import {HemodialysisMonitoringModelState, HemodialysisMonitoringModelAudit} from 'src/app/models/hemodialysisMonitoring/hemodialysis_monitoring.model.state';
import {HemodialysisMonitoringModel} from 'src/app/models/hemodialysisMonitoring/hemodialysis_monitoring.model';
import {
	getHemodialysisMonitoringModelAuditsByEntityId,
	getHemodialysisMonitoringModelWithId,
	getCountHemodialysisMonitoringModels,
...
import {CrudTileMode} from '../hemodialysis-monitoring-admin-crud.component';
...
@Component({
	selector: 'cb-hemodialysis-monitoring-admin-crud-edit',
	templateUrl: './hemodialysis-monitoring-admin-crud-edit.component.html',
	styleUrls: [
		'./hemodialysis-monitoring-admin-crud-edit.component.scss',
...
export class AdminHemodialysisMonitoringCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'a2ad6a8a-7569-4af7-9a0e-fb7236ce68f6';
	 ...
	 */
	hemodialysisMonitoringAudits$: Observable<HemodialysisMonitoringModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Hemodialysis Examination and Hemodialysis Monitorings here] off begin
	hemodialysisExaminationMultiCrudActive: boolean = false;
	hemodialysisExaminationDisplayName = 'Hemodialysis Examination';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Hemodialysis Examination and Hemodialysis Monitorings here] end
	hemodialysisExaminationOppositeRelation = 'hemodialysisMonitorings';
	...
	medicalStaffOppositeRelation = 'hemodialysisMonitorings';
	 ...
	 */
	modelProperties: ModelProperty[] = HemodialysisMonitoringModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = HemodialysisMonitoringModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = HemodialysisMonitoringModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: HemodialysisMonitoringModel;
	...
	constructor(
		private readonly store: Store<{ model: HemodialysisMonitoringModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountHemodialysisMonitoringModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new HemodialysisMonitoringModel();
		...
		if (this.targetModelId) {
			this.hemodialysisMonitoringAudits$ = this.store.select(getHemodialysisMonitoringModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<HemodialysisMonitoringModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
			modelAction.HemodialysisMonitoringModelActionTypes.FETCH_HEMODIALYSIS_MONITORING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getHemodialysisMonitoringModelWithId, this.targetModelId)
			...
			model = new HemodialysisMonitoringModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('hemodialysis-monitoring');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
			modelAction.HemodialysisMonitoringModelActionTypes.FETCH_HEMODIALYSIS_MONITORING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<HemodialysisMonitoringModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.hemodialysisExaminationMultiCrudActive || this.medicalStaffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.HemodialysisMonitoringAction(
						modelAction.HemodialysisMonitoringModelActionTypes.COUNT_HEMODIALYSIS_MONITORINGS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
				modelAction.HemodialysisMonitoringModelActionTypes.CREATE_HEMODIALYSIS_MONITORING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.HemodialysisMonitoringAction(
				modelAction.HemodialysisMonitoringModelActionTypes.UPDATE_HEMODIALYSIS_MONITORING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: HemodialysisMonitoringModel = new HemodialysisMonitoringModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getHemodialysisMonitoringModelWithId, this.targetModelId).subscribe(model => {
					let differences = new HemodialysisMonitoringModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = HemodialysisMonitoringModel.getProps();
		this.modelRelations = HemodialysisMonitoringModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: HemodialysisMonitoringModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): HemodialysisMonitoringModel {
		let updatedModel: HemodialysisMonitoringModel = new HemodialysisMonitoringModel(model);
