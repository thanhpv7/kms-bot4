...
import * as modelAction from 'src/app/models/coTreatingDoctor/co_treating_doctor.model.action';
import {CoTreatingDoctorModelState, CoTreatingDoctorModelAudit} from 'src/app/models/coTreatingDoctor/co_treating_doctor.model.state';
import {CoTreatingDoctorModel} from 'src/app/models/coTreatingDoctor/co_treating_doctor.model';
import {
	getCoTreatingDoctorModelAuditsByEntityId,
	getCoTreatingDoctorModelWithId,
	getCountCoTreatingDoctorModels,
...
import {CrudTileMode} from '../co-treating-doctor-admin-crud.component';
...
@Component({
	selector: 'cb-co-treating-doctor-admin-crud-edit',
	templateUrl: './co-treating-doctor-admin-crud-edit.component.html',
	styleUrls: [
		'./co-treating-doctor-admin-crud-edit.component.scss',
...
export class AdminCoTreatingDoctorCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'bdb655a6-0f29-446b-bb12-e0aa8b5179b1';
	 ...
	 */
	coTreatingDoctorAudits$: Observable<CoTreatingDoctorModelAudit[]>;
	...
	additionalAnesthesiologistMultiCrudActive: boolean = false;
	additionalAnesthesiologistDisplayName = 'Additional Anesthesiologist';
	...
	additionalAnesthesiologistOppositeRelation = 'additionalAnesthesiologists';
	additionalAnesthesiologistRelations: ModelRelation[] = this.additionalAnesthesiologistMultiCrudActive ? PreoperativeRecordsModel.getRelations()
		.map(relation => {
			if (relation.id === this.additionalAnesthesiologistOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Additional Midwife and Additional Midwives here] off begin
	additionalMidwifeMultiCrudActive: boolean = false;
	additionalMidwifeDisplayName = 'Additional Midwife';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Additional Midwife and Additional Midwives here] end
	additionalMidwifeOppositeRelation = 'additionalMidwives';
	additionalMidwifeRelations: ModelRelation[] = this.additionalMidwifeMultiCrudActive ? DeliveryMedicalExaminationRecordModel.getRelations()
		.map(relation => {
			if (relation.id === this.additionalMidwifeOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	additionalSurgeryNurseMultiCrudActive: boolean = false;
	additionalSurgeryNurseDisplayName = 'Additional Surgery Nurse';
	...
	additionalSurgeryNurseOppositeRelation = 'additionalSurgeryNurses';
	additionalSurgeryNurseRelations: ModelRelation[] = this.additionalSurgeryNurseMultiCrudActive ? PreoperativeRecordsModel.getRelations()
		.map(relation => {
			if (relation.id === this.additionalSurgeryNurseOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	coTreatingAnesthesiologistMultiCrudActive: boolean = false;
	coTreatingAnesthesiologistDisplayName = 'Co Treating Anesthesiologist';
	...
	coTreatingAnesthesiologistOppositeRelation = 'coTreatingAnesthesiologists';
	coTreatingAnesthesiologistRelations: ModelRelation[] = this.coTreatingAnesthesiologistMultiCrudActive ? PreoperativeRecordsModel.getRelations()
		.map(relation => {
			if (relation.id === this.coTreatingAnesthesiologistOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	medicalExaminationRecordOppositeRelation = 'coTreatingDoctors';
	...
	staffOppositeRelation = 'coTreatingDoctors';
	...
	coTreatingSurgeonMultiCrudActive: boolean = false;
	coTreatingSurgeonDisplayName = 'Co Treating Surgeon';
	...
	coTreatingSurgeonOppositeRelation = 'coTreatingSurgeons';
	coTreatingSurgeonRelations: ModelRelation[] = this.coTreatingSurgeonMultiCrudActive ? PreoperativeRecordsModel.getRelations()
		.map(relation => {
			if (relation.id === this.coTreatingSurgeonOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = CoTreatingDoctorModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = CoTreatingDoctorModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CoTreatingDoctorModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'additionalAnesthesiologist' && this.additionalAnesthesiologistMultiCrudActive) {
					exp.fields = PreoperativeRecordsModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'additionalMidwife' && this.additionalMidwifeMultiCrudActive) {
					exp.fields = DeliveryMedicalExaminationRecordModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'additionalSurgeryNurse' && this.additionalSurgeryNurseMultiCrudActive) {
					exp.fields = PreoperativeRecordsModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'coTreatingAnesthesiologist' && this.coTreatingAnesthesiologistMultiCrudActive) {
					exp.fields = PreoperativeRecordsModel.getProps().map(prop => {
						return prop.name;
					});
				}
				...
				if (relation.id === 'coTreatingSurgeon' && this.coTreatingSurgeonMultiCrudActive) {
					exp.fields = PreoperativeRecordsModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: CoTreatingDoctorModel;
	...
	constructor(
		private readonly store: Store<{ model: CoTreatingDoctorModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCoTreatingDoctorModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CoTreatingDoctorModel();
		...
		if (this.targetModelId) {
			this.coTreatingDoctorAudits$ = this.store.select(getCoTreatingDoctorModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CoTreatingDoctorModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.CoTreatingDoctorAction(
			modelAction.CoTreatingDoctorModelActionTypes.FETCH_CO_TREATING_DOCTOR,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCoTreatingDoctorModelWithId, this.targetModelId)
			...
			model = new CoTreatingDoctorModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('co-treating-doctor');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CoTreatingDoctorAction(
			modelAction.CoTreatingDoctorModelActionTypes.FETCH_CO_TREATING_DOCTOR_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<CoTreatingDoctorModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.additionalAnesthesiologistMultiCrudActive || this.additionalMidwifeMultiCrudActive || this.additionalSurgeryNurseMultiCrudActive || this.coTreatingAnesthesiologistMultiCrudActive || this.medicalExaminationRecordMultiCrudActive || this.staffMultiCrudActive || this.coTreatingSurgeonMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.CoTreatingDoctorAction(
						modelAction.CoTreatingDoctorModelActionTypes.COUNT_CO_TREATING_DOCTORS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.CoTreatingDoctorAction(
				modelAction.CoTreatingDoctorModelActionTypes.CREATE_CO_TREATING_DOCTOR,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CoTreatingDoctorAction(
				modelAction.CoTreatingDoctorModelActionTypes.UPDATE_CO_TREATING_DOCTOR,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CoTreatingDoctorModel = new CoTreatingDoctorModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCoTreatingDoctorModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CoTreatingDoctorModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.additionalAnesthesiologistDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.additionalAnesthesiologistRelations, this.isViewOnly),
					new PreoperativeRecordsModel(),
					PreoperativeRecordsModel,
					this.additionalAnesthesiologistDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.additionalAnesthesiologistRelations,
					true
				);
			...
			if (event.entityName === this.additionalMidwifeDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DeliveryMedicalExaminationRecordModel.getProps(), this.additionalMidwifeRelations, this.isViewOnly),
					new DeliveryMedicalExaminationRecordModel(),
					DeliveryMedicalExaminationRecordModel,
					this.additionalMidwifeDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.additionalMidwifeRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.additionalSurgeryNurseDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.additionalSurgeryNurseRelations, this.isViewOnly),
					new PreoperativeRecordsModel(),
					PreoperativeRecordsModel,
					this.additionalSurgeryNurseDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.additionalSurgeryNurseRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.coTreatingAnesthesiologistDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.coTreatingAnesthesiologistRelations, this.isViewOnly),
					new PreoperativeRecordsModel(),
					PreoperativeRecordsModel,
					this.coTreatingAnesthesiologistDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.coTreatingAnesthesiologistRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.coTreatingSurgeonDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.coTreatingSurgeonRelations, this.isViewOnly),
					new PreoperativeRecordsModel(),
					PreoperativeRecordsModel,
					this.coTreatingSurgeonDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.coTreatingSurgeonRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CoTreatingDoctorModel.getProps();
		this.modelRelations = CoTreatingDoctorModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'additionalAnesthesiologist';
		}).hideElement = this.additionalAnesthesiologistMultiCrudActive || this.modelRelations.find(rel => rel.id === 'additionalAnesthesiologist').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'additionalMidwife';
		}).hideElement = this.additionalMidwifeMultiCrudActive || this.modelRelations.find(rel => rel.id === 'additionalMidwife').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'additionalSurgeryNurse';
		}).hideElement = this.additionalSurgeryNurseMultiCrudActive || this.modelRelations.find(rel => rel.id === 'additionalSurgeryNurse').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'coTreatingAnesthesiologist';
		}).hideElement = this.coTreatingAnesthesiologistMultiCrudActive || this.modelRelations.find(rel => rel.id === 'coTreatingAnesthesiologist').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'coTreatingSurgeon';
		}).hideElement = this.coTreatingSurgeonMultiCrudActive || this.modelRelations.find(rel => rel.id === 'coTreatingSurgeon').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.additionalAnesthesiologistMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.additionalAnesthesiologistDisplayName, many: false});
		}

		if (this.additionalMidwifeMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.additionalMidwifeDisplayName, many: false});
		}

		if (this.additionalSurgeryNurseMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.additionalSurgeryNurseDisplayName, many: false});
		}

		if (this.coTreatingAnesthesiologistMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.coTreatingAnesthesiologistDisplayName, many: false});
		}
		...
		if (this.coTreatingSurgeonMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.coTreatingSurgeonDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.additionalAnesthesiologist && this.targetModel.additionalAnesthesiologist.id && this.additionalAnesthesiologistMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PreoperativeRecordsModel,
				this.additionalAnesthesiologistOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.additionalAnesthesiologist.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPreoperativeRecordsModelWithId, this.targetModel.additionalAnesthesiologistId).subscribe(additionalAnesthesiologist => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.additionalAnesthesiologistRelations, this.isViewOnly),
		...
		if (this.targetModel.additionalMidwife && this.targetModel.additionalMidwife.id && this.additionalMidwifeMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				DeliveryMedicalExaminationRecordModel,
				this.additionalMidwifeOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.additionalMidwife.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getDeliveryMedicalExaminationRecordModelWithId, this.targetModel.additionalMidwifeId).subscribe(additionalMidwife => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DeliveryMedicalExaminationRecordModel.getProps(), this.additionalMidwifeRelations, this.isViewOnly),
					additionalMidwife,
					DeliveryMedicalExaminationRecordModel,
		...
		if (this.targetModel.additionalSurgeryNurse && this.targetModel.additionalSurgeryNurse.id && this.additionalSurgeryNurseMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PreoperativeRecordsModel,
				this.additionalSurgeryNurseOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.additionalSurgeryNurse.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPreoperativeRecordsModelWithId, this.targetModel.additionalSurgeryNurseId).subscribe(additionalSurgeryNurse => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.additionalSurgeryNurseRelations, this.isViewOnly),
		...
		if (this.targetModel.coTreatingAnesthesiologist && this.targetModel.coTreatingAnesthesiologist.id && this.coTreatingAnesthesiologistMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PreoperativeRecordsModel,
				this.coTreatingAnesthesiologistOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.coTreatingAnesthesiologist.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPreoperativeRecordsModelWithId, this.targetModel.coTreatingAnesthesiologistId).subscribe(coTreatingAnesthesiologist => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.coTreatingAnesthesiologistRelations, this.isViewOnly),
		...
		if (this.targetModel.coTreatingSurgeon && this.targetModel.coTreatingSurgeon.id && this.coTreatingSurgeonMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PreoperativeRecordsModel,
				this.coTreatingSurgeonOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.coTreatingSurgeon.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPreoperativeRecordsModelWithId, this.targetModel.coTreatingSurgeonId).subscribe(coTreatingSurgeon => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PreoperativeRecordsModel.getProps(), this.coTreatingSurgeonRelations, this.isViewOnly),
					coTreatingSurgeon,
					PreoperativeRecordsModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: CoTreatingDoctorModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CoTreatingDoctorModel {
		let updatedModel: CoTreatingDoctorModel = new CoTreatingDoctorModel(model);

		if (this.additionalAnesthesiologistMultiCrudActive) {
			updatedModel.additionalAnesthesiologist = event.additionalEntities.additionalAnesthesiologist ? event.additionalEntities.additionalAnesthesiologist : undefined;

			if (!updatedModel.additionalAnesthesiologist && event.payload) {
				event.payload.additionalAnesthesiologistId = null;
			}
		} else if (event.payload?.additionalAnesthesiologistId !== undefined) {
			event.payload.additionalAnesthesiologist = null;
			updatedModel.additionalAnesthesiologist = null;
		}

		if (this.additionalMidwifeMultiCrudActive) {
			updatedModel.additionalMidwife = event.additionalEntities.additionalMidwife ? event.additionalEntities.additionalMidwife : undefined;

			if (!updatedModel.additionalMidwife && event.payload) {
				event.payload.additionalMidwifeId = null;
			}
		} else if (event.payload?.additionalMidwifeId !== undefined) {
			event.payload.additionalMidwife = null;
			updatedModel.additionalMidwife = null;
		}

		if (this.additionalSurgeryNurseMultiCrudActive) {
			updatedModel.additionalSurgeryNurse = event.additionalEntities.additionalSurgeryNurse ? event.additionalEntities.additionalSurgeryNurse : undefined;

			if (!updatedModel.additionalSurgeryNurse && event.payload) {
				event.payload.additionalSurgeryNurseId = null;
			}
		} else if (event.payload?.additionalSurgeryNurseId !== undefined) {
			event.payload.additionalSurgeryNurse = null;
			updatedModel.additionalSurgeryNurse = null;
		}

		if (this.coTreatingAnesthesiologistMultiCrudActive) {
			updatedModel.coTreatingAnesthesiologist = event.additionalEntities.coTreatingAnesthesiologist ? event.additionalEntities.coTreatingAnesthesiologist : undefined;

			if (!updatedModel.coTreatingAnesthesiologist && event.payload) {
				event.payload.coTreatingAnesthesiologistId = null;
			}
		} else if (event.payload?.coTreatingAnesthesiologistId !== undefined) {
			event.payload.coTreatingAnesthesiologist = null;
			updatedModel.coTreatingAnesthesiologist = null;
		}
		...
		if (this.coTreatingSurgeonMultiCrudActive) {
			updatedModel.coTreatingSurgeon = event.additionalEntities.coTreatingSurgeon ? event.additionalEntities.coTreatingSurgeon : undefined;

			if (!updatedModel.coTreatingSurgeon && event.payload) {
				event.payload.coTreatingSurgeonId = null;
			}
		} else if (event.payload?.coTreatingSurgeonId !== undefined) {
			event.payload.coTreatingSurgeon = null;
			updatedModel.coTreatingSurgeon = null;
		}
