...
import {AdminCoTreatingDoctorCrudListComponent} from './co-treating-doctor-admin-crud-list.component';
...
import {AdminCoTreatingDoctorCrudModule} from '../co-treating-doctor-admin-crud.module';
import {
	CoTreatingDoctorModelState,
	initialState as initialCoTreatingDoctorModelState
} from 'src/app/models/coTreatingDoctor/co_treating_doctor.model.state';
 ...
 */
describe('Co Treating Doctor Crud List Tile Component', () => {

	let fixture;
	let coTreatingDoctorCrudListComponent: AdminCoTreatingDoctorCrudListComponent;

	let store: MockStore<{ model: CoTreatingDoctorModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let coTreatingDoctorModelState: CoTreatingDoctorModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<CoTreatingDoctorModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'bdb655a6-0f29-446b-bb12-e0aa8b5179b1';
	const searchContent = 'text';

	const searchableFieldCount = CoTreatingDoctorModel.searchFields.length;

	const routerState: RouterState = {
		url: 'co-treating-doctor-admin-crud',
		urls: ['co-treating-doctor-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const coTreatingDoctorDataFactory = new CoTreatingDoctorDataFactory();

	const defaultDataSize = 30;
	const defaultData = coTreatingDoctorDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: CoTreatingDoctorModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		coTreatingDoctorModelState = initialCoTreatingDoctorModelState;
		store.setState({model: coTreatingDoctorModelState});
		...
		spyOn(store, 'select')
			.withArgs(getCoTreatingDoctorCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCoTreatingDoctorCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminCoTreatingDoctorCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminCoTreatingDoctorCrudListComponent);
			coTreatingDoctorCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(coTreatingDoctorCrudListComponent.coTreatingDoctorsCount).toBe(defaultData.length);
			...
			expect(coTreatingDoctorCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			coTreatingDoctorCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = coTreatingDoctorDataFactory.createAll(filteredDataSize);
			...
			expect(coTreatingDoctorCrudListComponent.coTreatingDoctorsCount).toBe(filteredDataSize);
			expect(coTreatingDoctorCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(coTreatingDoctorCrudListComponent.pageIndex).toBe(0);
			expect(coTreatingDoctorCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			coTreatingDoctorCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		coTreatingDoctorCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		coTreatingDoctorCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
