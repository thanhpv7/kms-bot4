...
import {CoTreatingDoctorModelState} from 'src/app/models/coTreatingDoctor/co_treating_doctor.model.state';
...
import {
	getCoTreatingDoctorCollectionModels,
	getCoTreatingDoctorCollectionCount
...
@Component({
	selector: 'cb-co-treating-doctor-admin-crud-list',
	templateUrl: './co-treating-doctor-admin-crud-list.component.html',
	styleUrls: [
		'./co-treating-doctor-admin-crud-list.component.scss',
...
export class AdminCoTreatingDoctorCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CoTreatingDoctorModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CoTreatingDoctorModel.searchFields,
	...
	/**
	 * List of Co Treating Doctors that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	coTreatingDoctors: Observable<CoTreatingDoctorModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	coTreatingDoctorsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.coTreatingDoctorsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.coTreatingDoctorsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CoTreatingDoctorAction(
				modelAction.CoTreatingDoctorModelActionTypes.INITIALISE_CO_TREATING_DOCTOR_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CoTreatingDoctorAction(
				modelAction.CoTreatingDoctorModelActionTypes.FETCH_CO_TREATING_DOCTOR_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.coTreatingDoctors = this.store.select(getCoTreatingDoctorCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCoTreatingDoctorCollectionCount, this.collectionId).subscribe(
				(count) => this.coTreatingDoctorsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CoTreatingDoctorModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CoTreatingDoctorModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CoTreatingDoctorModelActionTypes.EXPORT_ALL_CO_TREATING_DOCTOR;
		} else {
			actionType = modelAction.CoTreatingDoctorModelActionTypes.EXPORT_CO_TREATING_DOCTOR_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CoTreatingDoctorAction(
				modelAction.CoTreatingDoctorModelActionTypes.IMPORT_CO_TREATING_DOCTORS,
				{
					file: file,
				},
				[new modelAction.CoTreatingDoctorAction(
			...
			window.open(environment.API_URL + '/api/cotreatingdoctor/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CoTreatingDoctorModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Co Treating Doctor entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Co Treating Doctor entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CoTreatingDoctorModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CoTreatingDoctorModelActionTypes.DELETE_CO_TREATING_DOCTOR;
						} else if (usingSelected) {
							actionType = modelAction.CoTreatingDoctorModelActionTypes.DELETE_ALL_CO_TREATING_DOCTOR;
						} else {
							actionType = modelAction.CoTreatingDoctorModelActionTypes.DELETE_CO_TREATING_DOCTOR_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CoTreatingDoctorAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CO_TREATING_DOCTOR when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CO_TREATING_DOCTOR and DELETE_CO_TREATING_DOCTOR_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CoTreatingDoctorModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.coTreatingDoctorsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
