...
import {AdminIcd10CrudListComponent} from './icd-10-admin-crud-list.component';
...
import {AdminIcd10CrudModule} from '../icd-10-admin-crud.module';
import {
	Icd10ModelState,
	initialState as initialIcd10ModelState
} from 'src/app/models/icd10/icd_10.model.state';
 ...
 */
describe('ICD 10 Crud List Tile Component', () => {

	let fixture;
	let icd10CrudListComponent: AdminIcd10CrudListComponent;

	let store: MockStore<{ model: Icd10ModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let icd10ModelState: Icd10ModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<Icd10Model[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd7b3b527-4efc-48b7-ab2e-a790c19676be';
	const searchContent = 'text';

	const searchableFieldCount = Icd10Model.searchFields.length;

	const routerState: RouterState = {
		url: 'icd-10-admin-crud',
		urls: ['icd-10-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const icd10DataFactory = new Icd10DataFactory();

	const defaultDataSize = 30;
	const defaultData = icd10DataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: Icd10ModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		icd10ModelState = initialIcd10ModelState;
		store.setState({model: icd10ModelState});
		...
		spyOn(store, 'select')
			.withArgs(getIcd10CollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getIcd10CollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminIcd10CrudListComponent);
			icd10CrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(icd10CrudListComponent.icd10sCount).toBe(defaultData.length);
			...
			expect(icd10CrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			icd10CrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = icd10DataFactory.createAll(filteredDataSize);
			...
			expect(icd10CrudListComponent.icd10sCount).toBe(filteredDataSize);
			expect(icd10CrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(icd10CrudListComponent.pageIndex).toBe(0);
			expect(icd10CrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			icd10CrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		icd10CrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		icd10CrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
