...
import * as modelAction from 'src/app/models/icd10/icd_10.model.action';
import {Icd10ModelState} from 'src/app/models/icd10/icd_10.model.state';
...
import {
	getIcd10CollectionModels,
	getIcd10CollectionCount
...
import {CrudTileMode} from '../icd-10-admin-crud.component';
...
@Component({
	selector: 'cb-icd-10-admin-crud-list',
	templateUrl: './icd-10-admin-crud-list.component.html',
	styleUrls: [
		'./icd-10-admin-crud-list.component.scss',
...
export class AdminIcd10CrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<Icd10Model>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...Icd10Model.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = Icd10Model.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'd7b3b527-4efc-48b7-ab2e-a790c19676be';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.icd10sCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.icd10sCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: Icd10ModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.Icd10Action(
				modelAction.Icd10ModelActionTypes.INITIALISE_ICD_10_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.Icd10Action(
				modelAction.Icd10ModelActionTypes.FETCH_ICD_10_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.Icd10Action(
						modelAction.Icd10ModelActionTypes.COUNT_ICD_10S,
						null
					)
				]
			))
		...
		this.icd10s = this.store.select(getIcd10CollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getIcd10CollectionCount, this.collectionId).subscribe(
				(count) => this.icd10sCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: Icd10Model[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.Icd10ModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.Icd10ModelActionTypes.EXPORT_ALL_ICD_10;
		} else {
			actionType = modelAction.Icd10ModelActionTypes.EXPORT_ICD_10_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.Icd10Action(
				modelAction.Icd10ModelActionTypes.IMPORT_ICD_10S,
				{
					file: file,
				},
				[new modelAction.Icd10Action(
			...
			window.open(environment.API_URL + '/api/icd10/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: Icd10Model[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected ICD 10 entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected ICD 10 entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.Icd10ModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.Icd10ModelActionTypes.DELETE_ICD_10;
						} else if (usingSelected) {
							actionType = modelAction.Icd10ModelActionTypes.DELETE_ALL_ICD_10;
						} else {
							actionType = modelAction.Icd10ModelActionTypes.DELETE_ICD_10_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.Icd10Action(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ICD_10 when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ICD_10 and DELETE_ICD_10_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<Icd10Model> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = Icd10Model.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.icd10sCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
