...
import {Icd10ModelState, Icd10ModelAudit} from 'src/app/models/icd10/icd_10.model.state';
import {Icd10Model} from 'src/app/models/icd10/icd_10.model';
import {
	getIcd10ModelAuditsByEntityId,
	getIcd10ModelWithId,
	getCountIcd10Models,
...
@Component({
	selector: 'cb-icd-10-admin-crud-edit',
	templateUrl: './icd-10-admin-crud-edit.component.html',
	styleUrls: [
		'./icd-10-admin-crud-edit.component.scss',
...
export class AdminIcd10CrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	icd10Audits$: Observable<Icd10ModelAudit[]>;
	...
	diagnosesAndProceduresOppositeRelation = 'masterICD10';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = Icd10Model.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountIcd10Models).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new Icd10Model();
		...
		if (this.targetModelId) {
			this.icd10Audits$ = this.store.select(getIcd10ModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<Icd10Model> = {
		...
		this.store.dispatch(new modelAction.Icd10Action(
			modelAction.Icd10ModelActionTypes.FETCH_ICD_10,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getIcd10ModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.Icd10Action(
			modelAction.Icd10ModelActionTypes.FETCH_ICD_10_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.Icd10Action(
				modelAction.Icd10ModelActionTypes.CREATE_ICD_10,
				stateConfig,
			...
			this.store.dispatch(new modelAction.Icd10Action(
				modelAction.Icd10ModelActionTypes.UPDATE_ICD_10,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: Icd10Model = new Icd10Model(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getIcd10ModelWithId, this.targetModelId).subscribe(model => {
					let differences = new Icd10Model(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = Icd10Model.getProps();
		this.modelRelations = Icd10Model.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: Icd10Model, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): Icd10Model {
		let updatedModel: Icd10Model = new Icd10Model(model);
