...
import {AdminStockCatalogueCrudListComponent} from './stock-catalogue-admin-crud-list.component';
...
import {AdminStockCatalogueCrudModule} from '../stock-catalogue-admin-crud.module';
import {
 ...
 */
describe('Stock Catalogue Crud List Tile Component', () => {

	let fixture;
	let stockCatalogueCrudListComponent: AdminStockCatalogueCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<StockCatalogueModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '10b7b151-3aae-483b-a38f-17d0a96741de';
	const searchContent = 'text';

	const searchableFieldCount = StockCatalogueModel.searchFields.length;

	const routerState: RouterState = {
		url: 'stock-catalogue-admin-crud',
		urls: ['stock-catalogue-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const stockCatalogueDataFactory = new StockCatalogueDataFactory();

	const defaultDataSize = 30;
	const defaultData = stockCatalogueDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getStockCatalogueCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getStockCatalogueCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminStockCatalogueCrudListComponent);
			stockCatalogueCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(stockCatalogueCrudListComponent.stockCataloguesCount).toBe(defaultData.length);
			...
			expect(stockCatalogueCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			stockCatalogueCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = stockCatalogueDataFactory.createAll(filteredDataSize);
			...
			expect(stockCatalogueCrudListComponent.stockCataloguesCount).toBe(filteredDataSize);
			expect(stockCatalogueCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(stockCatalogueCrudListComponent.pageIndex).toBe(0);
			expect(stockCatalogueCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			stockCatalogueCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		stockCatalogueCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		stockCatalogueCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
