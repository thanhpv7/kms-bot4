...
import {StockCatalogueModelState} from 'src/app/models/stockCatalogue/stock_catalogue.model.state';
...
import {
	...
	getStockCatalogueCollectionCount
...
@Component({
	selector: 'cb-stock-catalogue-admin-crud-list',
	templateUrl: './stock-catalogue-admin-crud-list.component.html',
	styleUrls: [
		'./stock-catalogue-admin-crud-list.component.scss',
...
export class AdminStockCatalogueCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<StockCatalogueModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...StockCatalogueModel.searchFields,
	...
	/**
	 * List of Stock Catalogues that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	stockCatalogues: Observable<StockCatalogueModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	stockCataloguesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.stockCataloguesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.stockCataloguesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockCatalogueAction(
				modelAction.StockCatalogueModelActionTypes.INITIALISE_STOCK_CATALOGUE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockCatalogueAction(
				modelAction.StockCatalogueModelActionTypes.FETCH_STOCK_CATALOGUE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.stockCatalogues = this.store.select(getStockCatalogueCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getStockCatalogueCollectionCount, this.collectionId).subscribe(
				(count) => this.stockCataloguesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: StockCatalogueModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.StockCatalogueModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.StockCatalogueModelActionTypes.EXPORT_ALL_STOCK_CATALOGUE;
		} else {
			actionType = modelAction.StockCatalogueModelActionTypes.EXPORT_STOCK_CATALOGUE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.StockCatalogueAction(
				modelAction.StockCatalogueModelActionTypes.IMPORT_STOCK_CATALOGUES,
				{
					file: file,
				},
				[new modelAction.StockCatalogueAction(
			...
			window.open(environment.API_URL + '/api/stockcatalogue/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: StockCatalogueModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Stock Catalogue entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Stock Catalogue entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.StockCatalogueModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.StockCatalogueModelActionTypes.DELETE_STOCK_CATALOGUE;
						} else if (usingSelected) {
							actionType = modelAction.StockCatalogueModelActionTypes.DELETE_ALL_STOCK_CATALOGUE;
						} else {
							actionType = modelAction.StockCatalogueModelActionTypes.DELETE_STOCK_CATALOGUE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.StockCatalogueAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_STOCK_CATALOGUE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_STOCK_CATALOGUE and DELETE_STOCK_CATALOGUE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = StockCatalogueModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.stockCataloguesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
