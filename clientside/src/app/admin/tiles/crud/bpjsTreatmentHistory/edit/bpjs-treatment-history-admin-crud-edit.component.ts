...
import {BpjsTreatmentHistoryModelState, BpjsTreatmentHistoryModelAudit} from 'src/app/models/bpjsTreatmentHistory/bpjs_treatment_history.model.state';
import {BpjsTreatmentHistoryModel} from 'src/app/models/bpjsTreatmentHistory/bpjs_treatment_history.model';
import {
	getBpjsTreatmentHistoryModelAuditsByEntityId,
	getBpjsTreatmentHistoryModelWithId,
	getCountBpjsTreatmentHistoryModels,
...
@Component({
	selector: 'cb-bpjs-treatment-history-admin-crud-edit',
	templateUrl: './bpjs-treatment-history-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-treatment-history-admin-crud-edit.component.scss',
...
export class AdminBpjsTreatmentHistoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsTreatmentHistoryAudits$: Observable<BpjsTreatmentHistoryModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsTreatmentHistoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsTreatmentHistoryModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsTreatmentHistoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsTreatmentHistoryModel();
		...
		if (this.targetModelId) {
			this.bpjsTreatmentHistoryAudits$ = this.store.select(getBpjsTreatmentHistoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsTreatmentHistoryModel> = {
		...
		this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
			modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_BPJS_TREATMENT_HISTORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsTreatmentHistoryModelWithId, this.targetModelId)
			...
			model = new BpjsTreatmentHistoryModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
			modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_BPJS_TREATMENT_HISTORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
				modelAction.BpjsTreatmentHistoryModelActionTypes.CREATE_BPJS_TREATMENT_HISTORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
				modelAction.BpjsTreatmentHistoryModelActionTypes.UPDATE_BPJS_TREATMENT_HISTORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsTreatmentHistoryModel = new BpjsTreatmentHistoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsTreatmentHistoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsTreatmentHistoryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsTreatmentHistoryModel.getProps();
		this.modelRelations = BpjsTreatmentHistoryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsTreatmentHistoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsTreatmentHistoryModel {
		let updatedModel: BpjsTreatmentHistoryModel = new BpjsTreatmentHistoryModel(model);
