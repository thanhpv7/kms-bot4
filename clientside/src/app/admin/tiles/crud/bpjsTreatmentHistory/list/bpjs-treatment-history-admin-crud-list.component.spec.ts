...
import {AdminBpjsTreatmentHistoryCrudListComponent} from './bpjs-treatment-history-admin-crud-list.component';
import {BpjsTreatmentHistoryDataFactory} from 'src/app/lib/utils/factories/bpjs-treatment-history-data-factory';
import {BpjsTreatmentHistoryModel} from 'src/app/models/bpjsTreatmentHistory/bpjs_treatment_history.model';
import {AdminBpjsTreatmentHistoryCrudModule} from '../bpjs-treatment-history-admin-crud.module';
import {
	BpjsTreatmentHistoryModelState,
	initialState as initialBpjsTreatmentHistoryModelState
} from 'src/app/models/bpjsTreatmentHistory/bpjs_treatment_history.model.state';
 ...
 */
describe('BPJS Treatment History Crud List Tile Component', () => {

	let fixture;
	let bpjsTreatmentHistoryCrudListComponent: AdminBpjsTreatmentHistoryCrudListComponent;

	let store: MockStore<{ model: BpjsTreatmentHistoryModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsTreatmentHistoryModelState: BpjsTreatmentHistoryModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsTreatmentHistoryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e4c9321c-5748-4f66-b485-ab5ee314642a';
	const searchContent = 'text';

	const searchableFieldCount = BpjsTreatmentHistoryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-treatment-history-admin-crud',
		urls: ['bpjs-treatment-history-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsTreatmentHistoryDataFactory = new BpjsTreatmentHistoryDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsTreatmentHistoryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsTreatmentHistoryModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsTreatmentHistoryModelState = initialBpjsTreatmentHistoryModelState;
		store.setState({model: bpjsTreatmentHistoryModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsTreatmentHistoryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsTreatmentHistoryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsTreatmentHistoryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsTreatmentHistoryCrudListComponent);
			bpjsTreatmentHistoryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsTreatmentHistoryCrudListComponent.bpjsTreatmentHistorysCount).toBe(defaultData.length);
			...
			expect(bpjsTreatmentHistoryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsTreatmentHistoryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsTreatmentHistoryDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsTreatmentHistoryCrudListComponent.bpjsTreatmentHistorysCount).toBe(filteredDataSize);
			expect(bpjsTreatmentHistoryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsTreatmentHistoryCrudListComponent.pageIndex).toBe(0);
			expect(bpjsTreatmentHistoryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsTreatmentHistoryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsTreatmentHistoryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsTreatmentHistoryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
