...
import * as modelAction from 'src/app/models/bpjsTreatmentHistory/bpjs_treatment_history.model.action';
import {BpjsTreatmentHistoryModelState} from 'src/app/models/bpjsTreatmentHistory/bpjs_treatment_history.model.state';
...
import {
	getBpjsTreatmentHistoryCollectionModels,
	getBpjsTreatmentHistoryCollectionCount
} from 'src/app/models/bpjsTreatmentHistory/bpjs_treatment_history.model.selector';
...
import {CrudTileMode} from '../bpjs-treatment-history-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-treatment-history-admin-crud-list',
	templateUrl: './bpjs-treatment-history-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-treatment-history-admin-crud-list.component.scss',
...
export class AdminBpjsTreatmentHistoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsTreatmentHistoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsTreatmentHistoryModel.searchFields,
	...
	/**
	 * List of BPJS Treatment Historys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsTreatmentHistorys: Observable<BpjsTreatmentHistoryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsTreatmentHistorysCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsTreatmentHistoryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsTreatmentHistoryModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'e4c9321c-5748-4f66-b485-ab5ee314642a';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsTreatmentHistorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsTreatmentHistorysCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsTreatmentHistoryModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
				modelAction.BpjsTreatmentHistoryModelActionTypes.INITIALISE_BPJS_TREATMENT_HISTORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
				modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_BPJS_TREATMENT_HISTORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsTreatmentHistoryAction(
						modelAction.BpjsTreatmentHistoryModelActionTypes.COUNT_BPJS_TREATMENT_HISTORYS,
						null
					)
				]
			))
		...
		this.bpjsTreatmentHistorys = this.store.select(getBpjsTreatmentHistoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsTreatmentHistoryCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsTreatmentHistorysCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-treatment-history');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsTreatmentHistoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsTreatmentHistoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsTreatmentHistoryModelActionTypes.EXPORT_ALL_BPJS_TREATMENT_HISTORY;
		} else {
			actionType = modelAction.BpjsTreatmentHistoryModelActionTypes.EXPORT_BPJS_TREATMENT_HISTORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
				modelAction.BpjsTreatmentHistoryModelActionTypes.IMPORT_BPJS_TREATMENT_HISTORYS,
				{
					file: file,
				},
				[new modelAction.BpjsTreatmentHistoryAction(
			...
			window.open(environment.API_URL + '/api/bpjstreatmenthistory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsTreatmentHistoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Treatment History entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Treatment History entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsTreatmentHistoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsTreatmentHistoryModelActionTypes.DELETE_BPJS_TREATMENT_HISTORY;
						} else if (usingSelected) {
							actionType = modelAction.BpjsTreatmentHistoryModelActionTypes.DELETE_ALL_BPJS_TREATMENT_HISTORY;
						} else {
							actionType = modelAction.BpjsTreatmentHistoryModelActionTypes.DELETE_BPJS_TREATMENT_HISTORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsTreatmentHistoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_TREATMENT_HISTORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_TREATMENT_HISTORY and DELETE_BPJS_TREATMENT_HISTORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsTreatmentHistoryModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsTreatmentHistoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsTreatmentHistorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
