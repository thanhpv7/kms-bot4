...
import {PatientPaymentBPJSModelState} from 'src/app/models/patientPaymentBPJS/patient_payment_bpjs.model.state';
...
import {
	getPatientPaymentBPJSCollectionModels,
	getPatientPaymentBPJSCollectionCount
...
@Component({
	selector: 'cb-patient-payment-bpjs-admin-crud-list',
	templateUrl: './patient-payment-bpjs-admin-crud-list.component.html',
	styleUrls: [
		'./patient-payment-bpjs-admin-crud-list.component.scss',
...
export class AdminPatientPaymentBPJSCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientPaymentBPJSModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientPaymentBPJSModel.searchFields,
	...
	/**
	 * List of Patient Payment BPJSs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientPaymentBPJSs: Observable<PatientPaymentBPJSModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientPaymentBPJSsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientPaymentBPJSsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientPaymentBPJSsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentBPJSAction(
				modelAction.PatientPaymentBPJSModelActionTypes.INITIALISE_PATIENT_PAYMENT_BPJS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentBPJSAction(
				modelAction.PatientPaymentBPJSModelActionTypes.FETCH_PATIENT_PAYMENT_BPJS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientPaymentBPJSs = this.store.select(getPatientPaymentBPJSCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientPaymentBPJSCollectionCount, this.collectionId).subscribe(
				(count) => this.patientPaymentBPJSsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientPaymentBPJSModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientPaymentBPJSModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientPaymentBPJSModelActionTypes.EXPORT_ALL_PATIENT_PAYMENT_BPJS;
		} else {
			actionType = modelAction.PatientPaymentBPJSModelActionTypes.EXPORT_PATIENT_PAYMENT_BPJS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientPaymentBPJSAction(
				modelAction.PatientPaymentBPJSModelActionTypes.IMPORT_PATIENT_PAYMENT_BPJSS,
				{
					file: file,
				},
				[new modelAction.PatientPaymentBPJSAction(
			...
			window.open(environment.API_URL + '/api/patientpaymentbpjs/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientPaymentBPJSModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment BPJS entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment BPJS entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientPaymentBPJSModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientPaymentBPJSModelActionTypes.DELETE_PATIENT_PAYMENT_BPJS;
						} else if (usingSelected) {
							actionType = modelAction.PatientPaymentBPJSModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_BPJS;
						} else {
							actionType = modelAction.PatientPaymentBPJSModelActionTypes.DELETE_PATIENT_PAYMENT_BPJS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientPaymentBPJSAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_PAYMENT_BPJS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_PAYMENT_BPJS and DELETE_PATIENT_PAYMENT_BPJS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientPaymentBPJSModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientPaymentBPJSsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
