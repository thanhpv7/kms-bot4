...
import {AdminPatientPaymentBPJSCrudListComponent} from './patient-payment-bpjs-admin-crud-list.component';
...
import {AdminPatientPaymentBPJSCrudModule} from '../patient-payment-bpjs-admin-crud.module';
import {
	PatientPaymentBPJSModelState,
	initialState as initialPatientPaymentBPJSModelState
} from 'src/app/models/patientPaymentBPJS/patient_payment_bpjs.model.state';
 ...
 */
describe('Patient Payment BPJS Crud List Tile Component', () => {

	let fixture;
	let patientPaymentBPJSCrudListComponent: AdminPatientPaymentBPJSCrudListComponent;

	let store: MockStore<{ model: PatientPaymentBPJSModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientPaymentBPJSModelState: PatientPaymentBPJSModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientPaymentBPJSModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '3ac85a2a-8093-4511-a347-5c45dc670335';
	const searchContent = 'text';

	const searchableFieldCount = PatientPaymentBPJSModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-payment-bpjs-admin-crud',
		urls: ['patient-payment-bpjs-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientPaymentBPJSDataFactory = new PatientPaymentBPJSDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientPaymentBPJSDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientPaymentBPJSModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientPaymentBPJSModelState = initialPatientPaymentBPJSModelState;
		store.setState({model: patientPaymentBPJSModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientPaymentBPJSCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientPaymentBPJSCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientPaymentBPJSCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientPaymentBPJSCrudListComponent);
			patientPaymentBPJSCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientPaymentBPJSCrudListComponent.patientPaymentBPJSsCount).toBe(defaultData.length);
			...
			expect(patientPaymentBPJSCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientPaymentBPJSCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientPaymentBPJSDataFactory.createAll(filteredDataSize);
			...
			expect(patientPaymentBPJSCrudListComponent.patientPaymentBPJSsCount).toBe(filteredDataSize);
			expect(patientPaymentBPJSCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientPaymentBPJSCrudListComponent.pageIndex).toBe(0);
			expect(patientPaymentBPJSCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientPaymentBPJSCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientPaymentBPJSCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientPaymentBPJSCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
