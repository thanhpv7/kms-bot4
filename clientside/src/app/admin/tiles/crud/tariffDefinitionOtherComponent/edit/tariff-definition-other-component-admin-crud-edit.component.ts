...
import * as modelAction from 'src/app/models/tariffDefinitionOtherComponent/tariff_definition_other_component.model.action';
import {TariffDefinitionOtherComponentModelState, TariffDefinitionOtherComponentModelAudit} from 'src/app/models/tariffDefinitionOtherComponent/tariff_definition_other_component.model.state';
...
import {
	getTariffDefinitionOtherComponentModelAuditsByEntityId,
	getTariffDefinitionOtherComponentModelWithId,
	getCountTariffDefinitionOtherComponentModels,
...
import * as priceComponentModelAction from 'src/app/models/priceComponent/price_component.model.action';
import {
	getPriceComponentCollectionModels, getPriceComponentModelWithId,
...
import {CrudTileMode} from '../tariff-definition-other-component-admin-crud.component';
...
@Component({
	selector: 'cb-tariff-definition-other-component-admin-crud-edit',
	templateUrl: './tariff-definition-other-component-admin-crud-edit.component.html',
	styleUrls: [
		'./tariff-definition-other-component-admin-crud-edit.component.scss',
...
export class AdminTariffDefinitionOtherComponentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'cc093472-01a5-4199-8e6d-3d99fc7319a7';
	 ...
	 */
	tariffDefinitionOtherComponentAudits$: Observable<TariffDefinitionOtherComponentModelAudit[]>;
	...
	priceComponentMultiCrudActive: boolean = false;
	priceComponentDisplayName = 'Price Component';
	...
	priceComponentOppositeRelation = 'tariffDefinitionOtherComponents';
	priceComponentRelations: ModelRelation[] = this.priceComponentMultiCrudActive ? PriceComponentModel.getRelations()
		.map(relation => {
			if (relation.id === this.priceComponentOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Price Definition and Tariff Definition Other Components here] off begin
	priceDefinitionMultiCrudActive: boolean = false;
	priceDefinitionDisplayName = 'Price Definition';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Price Definition and Tariff Definition Other Components here] end
	priceDefinitionOppositeRelation = 'tariffDefinitionOtherComponents';
	priceDefinitionRelations: ModelRelation[] = this.priceDefinitionMultiCrudActive ? TariffDefinitionModel.getRelations()
		.map(relation => {
			if (relation.id === this.priceDefinitionOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = TariffDefinitionOtherComponentModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TariffDefinitionOtherComponentModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TariffDefinitionOtherComponentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'priceComponent' && this.priceComponentMultiCrudActive) {
					exp.fields = PriceComponentModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'priceDefinition' && this.priceDefinitionMultiCrudActive) {
	 ...
	 */
	targetModel: TariffDefinitionOtherComponentModel;
	...
	constructor(
		private readonly store: Store<{ model: TariffDefinitionOtherComponentModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTariffDefinitionOtherComponentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TariffDefinitionOtherComponentModel();
		...
		if (this.targetModelId) {
			this.tariffDefinitionOtherComponentAudits$ = this.store.select(getTariffDefinitionOtherComponentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TariffDefinitionOtherComponentModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
			modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_TARIFF_DEFINITION_OTHER_COMPONENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTariffDefinitionOtherComponentModelWithId, this.targetModelId)
			...
			model = new TariffDefinitionOtherComponentModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('tariff-definition-other-component');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
			modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_TARIFF_DEFINITION_OTHER_COMPONENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TariffDefinitionOtherComponentModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.priceComponentMultiCrudActive || this.priceDefinitionMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TariffDefinitionOtherComponentAction(
						modelAction.TariffDefinitionOtherComponentModelActionTypes.COUNT_TARIFF_DEFINITION_OTHER_COMPONENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
				modelAction.TariffDefinitionOtherComponentModelActionTypes.CREATE_TARIFF_DEFINITION_OTHER_COMPONENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
				modelAction.TariffDefinitionOtherComponentModelActionTypes.UPDATE_TARIFF_DEFINITION_OTHER_COMPONENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TariffDefinitionOtherComponentModel = new TariffDefinitionOtherComponentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTariffDefinitionOtherComponentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TariffDefinitionOtherComponentModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.priceComponentDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PriceComponentModel.getProps(), this.priceComponentRelations, this.isViewOnly),
					new PriceComponentModel(),
					PriceComponentModel,
					this.priceComponentDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.priceComponentRelations,
					true
				);
			...
			if (event.entityName === this.priceDefinitionDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffDefinitionModel.getProps(), this.priceDefinitionRelations, this.isViewOnly),
					new TariffDefinitionModel(),
					TariffDefinitionModel,
					this.priceDefinitionDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.priceDefinitionRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TariffDefinitionOtherComponentModel.getProps();
		this.modelRelations = TariffDefinitionOtherComponentModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'priceComponent';
		}).hideElement = this.priceComponentMultiCrudActive || this.modelRelations.find(rel => rel.id === 'priceComponent').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'priceDefinition';
		}).hideElement = this.priceDefinitionMultiCrudActive || this.modelRelations.find(rel => rel.id === 'priceDefinition').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.priceComponentMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.priceComponentDisplayName, many: false});
		}

		if (this.priceDefinitionMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.priceDefinitionDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.priceComponent && this.targetModel.priceComponent.id && this.priceComponentMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PriceComponentModel,
				this.priceComponentOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.priceComponent.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPriceComponentModelWithId, this.targetModel.priceComponentId).subscribe(priceComponent => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PriceComponentModel.getProps(), this.priceComponentRelations, this.isViewOnly),
					priceComponent,
					PriceComponentModel,
		...
		if (this.targetModel.priceDefinition && this.targetModel.priceDefinition.id && this.priceDefinitionMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffDefinitionModel,
				this.priceDefinitionOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.priceDefinition.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffDefinitionModelWithId, this.targetModel.priceDefinitionId).subscribe(priceDefinition => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffDefinitionModel.getProps(), this.priceDefinitionRelations, this.isViewOnly),
					priceDefinition,
					TariffDefinitionModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: TariffDefinitionOtherComponentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TariffDefinitionOtherComponentModel {
		let updatedModel: TariffDefinitionOtherComponentModel = new TariffDefinitionOtherComponentModel(model);

		if (this.priceComponentMultiCrudActive) {
			updatedModel.priceComponent = event.additionalEntities.priceComponent ? event.additionalEntities.priceComponent : undefined;

			if (!updatedModel.priceComponent && event.payload) {
				event.payload.priceComponentId = null;
			}
		} else if (event.payload?.priceComponentId !== undefined) {
			event.payload.priceComponent = null;
			updatedModel.priceComponent = null;
		}

		if (this.priceDefinitionMultiCrudActive) {
			updatedModel.priceDefinition = event.additionalEntities.priceDefinition ? event.additionalEntities.priceDefinition : undefined;

			if (!updatedModel.priceDefinition && event.payload) {
				event.payload.priceDefinitionId = null;
			}
		} else if (event.payload?.priceDefinitionId !== undefined) {
			event.payload.priceDefinition = null;
			updatedModel.priceDefinition = null;
		}
