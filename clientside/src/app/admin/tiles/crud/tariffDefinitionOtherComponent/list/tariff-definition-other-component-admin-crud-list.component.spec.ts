...
import {AdminTariffDefinitionOtherComponentCrudListComponent} from './tariff-definition-other-component-admin-crud-list.component';
...
import {AdminTariffDefinitionOtherComponentCrudModule} from '../tariff-definition-other-component-admin-crud.module';
import {
	TariffDefinitionOtherComponentModelState,
	initialState as initialTariffDefinitionOtherComponentModelState
} from 'src/app/models/tariffDefinitionOtherComponent/tariff_definition_other_component.model.state';
 ...
 */
describe('Tariff Definition Other Component Crud List Tile Component', () => {

	let fixture;
	let tariffDefinitionOtherComponentCrudListComponent: AdminTariffDefinitionOtherComponentCrudListComponent;

	let store: MockStore<{ model: TariffDefinitionOtherComponentModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let tariffDefinitionOtherComponentModelState: TariffDefinitionOtherComponentModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<TariffDefinitionOtherComponentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cc093472-01a5-4199-8e6d-3d99fc7319a7';
	const searchContent = 'text';

	const searchableFieldCount = TariffDefinitionOtherComponentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'tariff-definition-other-component-admin-crud',
		urls: ['tariff-definition-other-component-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const tariffDefinitionOtherComponentDataFactory = new TariffDefinitionOtherComponentDataFactory();

	const defaultDataSize = 30;
	const defaultData = tariffDefinitionOtherComponentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: TariffDefinitionOtherComponentModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		tariffDefinitionOtherComponentModelState = initialTariffDefinitionOtherComponentModelState;
		store.setState({model: tariffDefinitionOtherComponentModelState});
		...
		spyOn(store, 'select')
			.withArgs(getTariffDefinitionOtherComponentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTariffDefinitionOtherComponentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminTariffDefinitionOtherComponentCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminTariffDefinitionOtherComponentCrudListComponent);
			tariffDefinitionOtherComponentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(tariffDefinitionOtherComponentCrudListComponent.tariffDefinitionOtherComponentsCount).toBe(defaultData.length);
			...
			expect(tariffDefinitionOtherComponentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			tariffDefinitionOtherComponentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = tariffDefinitionOtherComponentDataFactory.createAll(filteredDataSize);
			...
			expect(tariffDefinitionOtherComponentCrudListComponent.tariffDefinitionOtherComponentsCount).toBe(filteredDataSize);
			expect(tariffDefinitionOtherComponentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(tariffDefinitionOtherComponentCrudListComponent.pageIndex).toBe(0);
			expect(tariffDefinitionOtherComponentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			tariffDefinitionOtherComponentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		tariffDefinitionOtherComponentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		tariffDefinitionOtherComponentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
