...
import {TariffDefinitionOtherComponentModelState} from 'src/app/models/tariffDefinitionOtherComponent/tariff_definition_other_component.model.state';
...
import {
	getTariffDefinitionOtherComponentCollectionModels,
	getTariffDefinitionOtherComponentCollectionCount
...
@Component({
	selector: 'cb-tariff-definition-other-component-admin-crud-list',
	templateUrl: './tariff-definition-other-component-admin-crud-list.component.html',
	styleUrls: [
		'./tariff-definition-other-component-admin-crud-list.component.scss',
...
export class AdminTariffDefinitionOtherComponentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TariffDefinitionOtherComponentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TariffDefinitionOtherComponentModel.searchFields,
	...
	/**
	 * List of Tariff Definition Other Components that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	tariffDefinitionOtherComponents: Observable<TariffDefinitionOtherComponentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	tariffDefinitionOtherComponentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.tariffDefinitionOtherComponentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.tariffDefinitionOtherComponentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
				modelAction.TariffDefinitionOtherComponentModelActionTypes.INITIALISE_TARIFF_DEFINITION_OTHER_COMPONENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
				modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_TARIFF_DEFINITION_OTHER_COMPONENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.tariffDefinitionOtherComponents = this.store.select(getTariffDefinitionOtherComponentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTariffDefinitionOtherComponentCollectionCount, this.collectionId).subscribe(
				(count) => this.tariffDefinitionOtherComponentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TariffDefinitionOtherComponentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TariffDefinitionOtherComponentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TariffDefinitionOtherComponentModelActionTypes.EXPORT_ALL_TARIFF_DEFINITION_OTHER_COMPONENT;
		} else {
			actionType = modelAction.TariffDefinitionOtherComponentModelActionTypes.EXPORT_TARIFF_DEFINITION_OTHER_COMPONENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
				modelAction.TariffDefinitionOtherComponentModelActionTypes.IMPORT_TARIFF_DEFINITION_OTHER_COMPONENTS,
				{
					file: file,
				},
				[new modelAction.TariffDefinitionOtherComponentAction(
			...
			window.open(environment.API_URL + '/api/tariffdefinitionothercomponent/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TariffDefinitionOtherComponentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Tariff Definition Other Component entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Tariff Definition Other Component entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TariffDefinitionOtherComponentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TariffDefinitionOtherComponentModelActionTypes.DELETE_TARIFF_DEFINITION_OTHER_COMPONENT;
						} else if (usingSelected) {
							actionType = modelAction.TariffDefinitionOtherComponentModelActionTypes.DELETE_ALL_TARIFF_DEFINITION_OTHER_COMPONENT;
						} else {
							actionType = modelAction.TariffDefinitionOtherComponentModelActionTypes.DELETE_TARIFF_DEFINITION_OTHER_COMPONENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TariffDefinitionOtherComponentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TARIFF_DEFINITION_OTHER_COMPONENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TARIFF_DEFINITION_OTHER_COMPONENT and DELETE_TARIFF_DEFINITION_OTHER_COMPONENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TariffDefinitionOtherComponentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.tariffDefinitionOtherComponentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
