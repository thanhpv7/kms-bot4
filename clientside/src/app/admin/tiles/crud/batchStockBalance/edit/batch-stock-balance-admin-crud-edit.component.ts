...
import {BatchStockBalanceModelState, BatchStockBalanceModelAudit} from 'src/app/models/batchStockBalance/batch_stock_balance.model.state';
import {BatchStockBalanceModel} from 'src/app/models/batchStockBalance/batch_stock_balance.model';
import {
	getBatchStockBalanceModelAuditsByEntityId,
	getBatchStockBalanceModelWithId,
	getCountBatchStockBalanceModels,
...
@Component({
	selector: 'cb-batch-stock-balance-admin-crud-edit',
	templateUrl: './batch-stock-balance-admin-crud-edit.component.html',
	styleUrls: [
		'./batch-stock-balance-admin-crud-edit.component.scss',
...
export class AdminBatchStockBalanceCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	batchStockBalanceAudits$: Observable<BatchStockBalanceModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'batchStockBalancesCatalogue';
	...
	warehouseOppositeRelation = 'batchStockBalancesWarehouse';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BatchStockBalanceModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BatchStockBalanceModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBatchStockBalanceModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BatchStockBalanceModel();
		...
		if (this.targetModelId) {
			this.batchStockBalanceAudits$ = this.store.select(getBatchStockBalanceModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BatchStockBalanceModel> = {
		...
		this.store.dispatch(new modelAction.BatchStockBalanceAction(
			modelAction.BatchStockBalanceModelActionTypes.FETCH_BATCH_STOCK_BALANCE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBatchStockBalanceModelWithId, this.targetModelId)
			...
			model = new BatchStockBalanceModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BatchStockBalanceAction(
			modelAction.BatchStockBalanceModelActionTypes.FETCH_BATCH_STOCK_BALANCE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BatchStockBalanceAction(
				modelAction.BatchStockBalanceModelActionTypes.CREATE_BATCH_STOCK_BALANCE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BatchStockBalanceAction(
				modelAction.BatchStockBalanceModelActionTypes.UPDATE_BATCH_STOCK_BALANCE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BatchStockBalanceModel = new BatchStockBalanceModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBatchStockBalanceModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BatchStockBalanceModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BatchStockBalanceModel.getProps();
		this.modelRelations = BatchStockBalanceModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BatchStockBalanceModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BatchStockBalanceModel {
		let updatedModel: BatchStockBalanceModel = new BatchStockBalanceModel(model);
