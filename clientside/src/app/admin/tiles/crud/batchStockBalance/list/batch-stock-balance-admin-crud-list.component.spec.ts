...
import {AdminBatchStockBalanceCrudListComponent} from './batch-stock-balance-admin-crud-list.component';
...
import {AdminBatchStockBalanceCrudModule} from '../batch-stock-balance-admin-crud.module';
import {
	BatchStockBalanceModelState,
	initialState as initialBatchStockBalanceModelState
} from 'src/app/models/batchStockBalance/batch_stock_balance.model.state';
 ...
 */
describe('Batch Stock Balance Crud List Tile Component', () => {

	let fixture;
	let batchStockBalanceCrudListComponent: AdminBatchStockBalanceCrudListComponent;

	let store: MockStore<{ model: BatchStockBalanceModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let batchStockBalanceModelState: BatchStockBalanceModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BatchStockBalanceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '76e2c4f5-80bc-43e5-be85-8b4310bce883';
	const searchContent = 'text';

	const searchableFieldCount = BatchStockBalanceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'batch-stock-balance-admin-crud',
		urls: ['batch-stock-balance-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const batchStockBalanceDataFactory = new BatchStockBalanceDataFactory();

	const defaultDataSize = 30;
	const defaultData = batchStockBalanceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BatchStockBalanceModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		batchStockBalanceModelState = initialBatchStockBalanceModelState;
		store.setState({model: batchStockBalanceModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBatchStockBalanceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBatchStockBalanceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBatchStockBalanceCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBatchStockBalanceCrudListComponent);
			batchStockBalanceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(batchStockBalanceCrudListComponent.batchStockBalancesCount).toBe(defaultData.length);
			...
			expect(batchStockBalanceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			batchStockBalanceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = batchStockBalanceDataFactory.createAll(filteredDataSize);
			...
			expect(batchStockBalanceCrudListComponent.batchStockBalancesCount).toBe(filteredDataSize);
			expect(batchStockBalanceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(batchStockBalanceCrudListComponent.pageIndex).toBe(0);
			expect(batchStockBalanceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			batchStockBalanceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		batchStockBalanceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		batchStockBalanceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
