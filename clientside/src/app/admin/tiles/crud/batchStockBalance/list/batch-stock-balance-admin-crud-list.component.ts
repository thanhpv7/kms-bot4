...
import * as modelAction from 'src/app/models/batchStockBalance/batch_stock_balance.model.action';
import {BatchStockBalanceModelState} from 'src/app/models/batchStockBalance/batch_stock_balance.model.state';
...
import {
	getBatchStockBalanceCollectionModels,
	getBatchStockBalanceCollectionCount
...
import {CrudTileMode} from '../batch-stock-balance-admin-crud.component';
...
@Component({
	selector: 'cb-batch-stock-balance-admin-crud-list',
	templateUrl: './batch-stock-balance-admin-crud-list.component.html',
	styleUrls: [
		'./batch-stock-balance-admin-crud-list.component.scss',
...
export class AdminBatchStockBalanceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BatchStockBalanceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BatchStockBalanceModel.searchFields,
	...
	/**
	 * List of Batch Stock Balances that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	batchStockBalances: Observable<BatchStockBalanceModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	batchStockBalancesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BatchStockBalanceModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BatchStockBalanceModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '76e2c4f5-80bc-43e5-be85-8b4310bce883';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.batchStockBalancesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.batchStockBalancesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BatchStockBalanceModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BatchStockBalanceAction(
				modelAction.BatchStockBalanceModelActionTypes.INITIALISE_BATCH_STOCK_BALANCE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BatchStockBalanceAction(
				modelAction.BatchStockBalanceModelActionTypes.FETCH_BATCH_STOCK_BALANCE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BatchStockBalanceAction(
						modelAction.BatchStockBalanceModelActionTypes.COUNT_BATCH_STOCK_BALANCES,
						null
					)
				]
			))
		...
		this.batchStockBalances = this.store.select(getBatchStockBalanceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBatchStockBalanceCollectionCount, this.collectionId).subscribe(
				(count) => this.batchStockBalancesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('batch-stock-balance');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BatchStockBalanceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BatchStockBalanceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BatchStockBalanceModelActionTypes.EXPORT_ALL_BATCH_STOCK_BALANCE;
		} else {
			actionType = modelAction.BatchStockBalanceModelActionTypes.EXPORT_BATCH_STOCK_BALANCE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BatchStockBalanceAction(
				modelAction.BatchStockBalanceModelActionTypes.IMPORT_BATCH_STOCK_BALANCES,
				{
					file: file,
				},
				[new modelAction.BatchStockBalanceAction(
			...
			window.open(environment.API_URL + '/api/batchstockbalance/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BatchStockBalanceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Batch Stock Balance entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Batch Stock Balance entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BatchStockBalanceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BatchStockBalanceModelActionTypes.DELETE_BATCH_STOCK_BALANCE;
						} else if (usingSelected) {
							actionType = modelAction.BatchStockBalanceModelActionTypes.DELETE_ALL_BATCH_STOCK_BALANCE;
						} else {
							actionType = modelAction.BatchStockBalanceModelActionTypes.DELETE_BATCH_STOCK_BALANCE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BatchStockBalanceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BATCH_STOCK_BALANCE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BATCH_STOCK_BALANCE and DELETE_BATCH_STOCK_BALANCE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BatchStockBalanceModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BatchStockBalanceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.batchStockBalancesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
