...
import {StockTransactionPerDateTypeModelState, StockTransactionPerDateTypeModelAudit} from 'src/app/models/stockTransactionPerDateType/stock_transaction_per_date_type.model.state';
...
import {
	getStockTransactionPerDateTypeModelAuditsByEntityId,
	getStockTransactionPerDateTypeModelWithId,
	getCountStockTransactionPerDateTypeModels,
...
@Component({
	selector: 'cb-stock-transaction-per-date-type-admin-crud-edit',
	templateUrl: './stock-transaction-per-date-type-admin-crud-edit.component.html',
	styleUrls: [
		'./stock-transaction-per-date-type-admin-crud-edit.component.scss',
...
export class AdminStockTransactionPerDateTypeCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	stockTransactionPerDateTypeAudits$: Observable<StockTransactionPerDateTypeModelAudit[]>;
	...
	stockCatalogueOppositeRelation = 'stockTransactionPerDateTypes';
	...
	transactionDetailTypeOppositeRelation = 'stockTransactionPerDateTypes';
	...
	warehouseOppositeRelation = 'stockTransactionPerDateTypes';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = StockTransactionPerDateTypeModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: StockTransactionPerDateTypeModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountStockTransactionPerDateTypeModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new StockTransactionPerDateTypeModel();
		...
		if (this.targetModelId) {
			this.stockTransactionPerDateTypeAudits$ = this.store.select(getStockTransactionPerDateTypeModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<StockTransactionPerDateTypeModel> = {
		...
		this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
			modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_TYPE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getStockTransactionPerDateTypeModelWithId, this.targetModelId)
			...
			model = new StockTransactionPerDateTypeModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
			modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_TYPE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<StockTransactionPerDateTypeModel> = {
				...
				queryParams: {
					persistRelations: this.stockCatalogueMultiCrudActive || this.transactionDetailTypeMultiCrudActive || this.warehouseMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
				modelAction.StockTransactionPerDateTypeModelActionTypes.CREATE_STOCK_TRANSACTION_PER_DATE_TYPE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
				modelAction.StockTransactionPerDateTypeModelActionTypes.UPDATE_STOCK_TRANSACTION_PER_DATE_TYPE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: StockTransactionPerDateTypeModel = new StockTransactionPerDateTypeModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getStockTransactionPerDateTypeModelWithId, this.targetModelId).subscribe(model => {
					let differences = new StockTransactionPerDateTypeModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = StockTransactionPerDateTypeModel.getProps();
		this.modelRelations = StockTransactionPerDateTypeModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: StockTransactionPerDateTypeModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): StockTransactionPerDateTypeModel {
		let updatedModel: StockTransactionPerDateTypeModel = new StockTransactionPerDateTypeModel(model);
