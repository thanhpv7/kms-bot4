...
import * as modelAction from 'src/app/models/stockTransactionPerDateType/stock_transaction_per_date_type.model.action';
import {StockTransactionPerDateTypeModelState} from 'src/app/models/stockTransactionPerDateType/stock_transaction_per_date_type.model.state';
...
import {
	getStockTransactionPerDateTypeCollectionModels,
	getStockTransactionPerDateTypeCollectionCount
...
import {CrudTileMode} from '../stock-transaction-per-date-type-admin-crud.component';
...
@Component({
	selector: 'cb-stock-transaction-per-date-type-admin-crud-list',
	templateUrl: './stock-transaction-per-date-type-admin-crud-list.component.html',
	styleUrls: [
		'./stock-transaction-per-date-type-admin-crud-list.component.scss',
...
export class AdminStockTransactionPerDateTypeCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<StockTransactionPerDateTypeModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...StockTransactionPerDateTypeModel.searchFields,
	...
	/**
	 * List of Stock Transaction Per Date Types that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	stockTransactionPerDateTypes: Observable<StockTransactionPerDateTypeModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	stockTransactionPerDateTypesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = StockTransactionPerDateTypeModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = StockTransactionPerDateTypeModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'a99bd520-7cce-47e7-ae62-173910b57eab';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.stockTransactionPerDateTypesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.stockTransactionPerDateTypesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: StockTransactionPerDateTypeModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
				modelAction.StockTransactionPerDateTypeModelActionTypes.INITIALISE_STOCK_TRANSACTION_PER_DATE_TYPE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
				modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_TYPE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.StockTransactionPerDateTypeAction(
						modelAction.StockTransactionPerDateTypeModelActionTypes.COUNT_STOCK_TRANSACTION_PER_DATE_TYPES,
						null
					)
				]
			))
		...
		this.stockTransactionPerDateTypes = this.store.select(getStockTransactionPerDateTypeCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getStockTransactionPerDateTypeCollectionCount, this.collectionId).subscribe(
				(count) => this.stockTransactionPerDateTypesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('stock-transaction-per-date-type');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: StockTransactionPerDateTypeModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.StockTransactionPerDateTypeModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.StockTransactionPerDateTypeModelActionTypes.EXPORT_ALL_STOCK_TRANSACTION_PER_DATE_TYPE;
		} else {
			actionType = modelAction.StockTransactionPerDateTypeModelActionTypes.EXPORT_STOCK_TRANSACTION_PER_DATE_TYPE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
				modelAction.StockTransactionPerDateTypeModelActionTypes.IMPORT_STOCK_TRANSACTION_PER_DATE_TYPES,
				{
					file: file,
				},
				[new modelAction.StockTransactionPerDateTypeAction(
			...
			window.open(environment.API_URL + '/api/stocktransactionperdatetype/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: StockTransactionPerDateTypeModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Stock Transaction Per Date Type entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Stock Transaction Per Date Type entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.StockTransactionPerDateTypeModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.StockTransactionPerDateTypeModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE_TYPE;
						} else if (usingSelected) {
							actionType = modelAction.StockTransactionPerDateTypeModelActionTypes.DELETE_ALL_STOCK_TRANSACTION_PER_DATE_TYPE;
						} else {
							actionType = modelAction.StockTransactionPerDateTypeModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE_TYPE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.StockTransactionPerDateTypeAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_STOCK_TRANSACTION_PER_DATE_TYPE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_STOCK_TRANSACTION_PER_DATE_TYPE and DELETE_STOCK_TRANSACTION_PER_DATE_TYPE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<StockTransactionPerDateTypeModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = StockTransactionPerDateTypeModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.stockTransactionPerDateTypesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
