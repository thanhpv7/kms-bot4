...
import {AdminStockTransactionPerDateTypeCrudListComponent} from './stock-transaction-per-date-type-admin-crud-list.component';
...
import {AdminStockTransactionPerDateTypeCrudModule} from '../stock-transaction-per-date-type-admin-crud.module';
import {
	StockTransactionPerDateTypeModelState,
	initialState as initialStockTransactionPerDateTypeModelState
} from 'src/app/models/stockTransactionPerDateType/stock_transaction_per_date_type.model.state';
 ...
 */
describe('Stock Transaction Per Date Type Crud List Tile Component', () => {

	let fixture;
	let stockTransactionPerDateTypeCrudListComponent: AdminStockTransactionPerDateTypeCrudListComponent;

	let store: MockStore<{ model: StockTransactionPerDateTypeModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let stockTransactionPerDateTypeModelState: StockTransactionPerDateTypeModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<StockTransactionPerDateTypeModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a99bd520-7cce-47e7-ae62-173910b57eab';
	const searchContent = 'text';

	const searchableFieldCount = StockTransactionPerDateTypeModel.searchFields.length;

	const routerState: RouterState = {
		url: 'stock-transaction-per-date-type-admin-crud',
		urls: ['stock-transaction-per-date-type-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const stockTransactionPerDateTypeDataFactory = new StockTransactionPerDateTypeDataFactory();

	const defaultDataSize = 30;
	const defaultData = stockTransactionPerDateTypeDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: StockTransactionPerDateTypeModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		stockTransactionPerDateTypeModelState = initialStockTransactionPerDateTypeModelState;
		store.setState({model: stockTransactionPerDateTypeModelState});
		...
		spyOn(store, 'select')
			.withArgs(getStockTransactionPerDateTypeCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getStockTransactionPerDateTypeCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminStockTransactionPerDateTypeCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminStockTransactionPerDateTypeCrudListComponent);
			stockTransactionPerDateTypeCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(stockTransactionPerDateTypeCrudListComponent.stockTransactionPerDateTypesCount).toBe(defaultData.length);
			...
			expect(stockTransactionPerDateTypeCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			stockTransactionPerDateTypeCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = stockTransactionPerDateTypeDataFactory.createAll(filteredDataSize);
			...
			expect(stockTransactionPerDateTypeCrudListComponent.stockTransactionPerDateTypesCount).toBe(filteredDataSize);
			expect(stockTransactionPerDateTypeCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(stockTransactionPerDateTypeCrudListComponent.pageIndex).toBe(0);
			expect(stockTransactionPerDateTypeCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			stockTransactionPerDateTypeCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		stockTransactionPerDateTypeCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		stockTransactionPerDateTypeCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
