...
import {NhisInvestigationModelState, NhisInvestigationModelAudit} from 'src/app/models/nhisInvestigation/nhis_investigation.model.state';
import {NhisInvestigationModel} from 'src/app/models/nhisInvestigation/nhis_investigation.model';
import {
	getNhisInvestigationModelAuditsByEntityId,
	getNhisInvestigationModelWithId,
	getCountNhisInvestigationModels,
...
@Component({
	selector: 'cb-nhis-investigation-admin-crud-edit',
	templateUrl: './nhis-investigation-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-investigation-admin-crud-edit.component.scss',
...
export class AdminNhisInvestigationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisInvestigationAudits$: Observable<NhisInvestigationModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisInvestigationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NhisInvestigationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisInvestigationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisInvestigationModel();
		...
		if (this.targetModelId) {
			this.nhisInvestigationAudits$ = this.store.select(getNhisInvestigationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisInvestigationModel> = {
		...
		this.store.dispatch(new modelAction.NhisInvestigationAction(
			modelAction.NhisInvestigationModelActionTypes.FETCH_NHIS_INVESTIGATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisInvestigationModelWithId, this.targetModelId)
			...
			model = new NhisInvestigationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisInvestigationAction(
			modelAction.NhisInvestigationModelActionTypes.FETCH_NHIS_INVESTIGATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisInvestigationAction(
				modelAction.NhisInvestigationModelActionTypes.CREATE_NHIS_INVESTIGATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisInvestigationAction(
				modelAction.NhisInvestigationModelActionTypes.UPDATE_NHIS_INVESTIGATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisInvestigationModel = new NhisInvestigationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisInvestigationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisInvestigationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisInvestigationModel.getProps();
		this.modelRelations = NhisInvestigationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisInvestigationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisInvestigationModel {
		let updatedModel: NhisInvestigationModel = new NhisInvestigationModel(model);
