...
import {AdminNhisInvestigationCrudListComponent} from './nhis-investigation-admin-crud-list.component';
import {NhisInvestigationDataFactory} from 'src/app/lib/utils/factories/nhis-investigation-data-factory';
import {NhisInvestigationModel} from 'src/app/models/nhisInvestigation/nhis_investigation.model';
import {AdminNhisInvestigationCrudModule} from '../nhis-investigation-admin-crud.module';
import {
	NhisInvestigationModelState,
	initialState as initialNhisInvestigationModelState
} from 'src/app/models/nhisInvestigation/nhis_investigation.model.state';
 ...
 */
describe('NHIS Investigation Crud List Tile Component', () => {

	let fixture;
	let nhisInvestigationCrudListComponent: AdminNhisInvestigationCrudListComponent;

	let store: MockStore<{ model: NhisInvestigationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisInvestigationModelState: NhisInvestigationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisInvestigationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '1d7e0908-f03e-489a-877a-4f25432f1592';
	const searchContent = 'text';

	const searchableFieldCount = NhisInvestigationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-investigation-admin-crud',
		urls: ['nhis-investigation-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisInvestigationDataFactory = new NhisInvestigationDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisInvestigationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisInvestigationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisInvestigationModelState = initialNhisInvestigationModelState;
		store.setState({model: nhisInvestigationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisInvestigationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisInvestigationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminNhisInvestigationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminNhisInvestigationCrudListComponent);
			nhisInvestigationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisInvestigationCrudListComponent.nhisInvestigationsCount).toBe(defaultData.length);
			...
			expect(nhisInvestigationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisInvestigationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisInvestigationDataFactory.createAll(filteredDataSize);
			...
			expect(nhisInvestigationCrudListComponent.nhisInvestigationsCount).toBe(filteredDataSize);
			expect(nhisInvestigationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisInvestigationCrudListComponent.pageIndex).toBe(0);
			expect(nhisInvestigationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisInvestigationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisInvestigationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisInvestigationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
