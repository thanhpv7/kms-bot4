...
import * as modelAction from 'src/app/models/nhisInvestigation/nhis_investigation.model.action';
import {NhisInvestigationModelState} from 'src/app/models/nhisInvestigation/nhis_investigation.model.state';
...
import {
	getNhisInvestigationCollectionModels,
	getNhisInvestigationCollectionCount
} from 'src/app/models/nhisInvestigation/nhis_investigation.model.selector';
...
import {CrudTileMode} from '../nhis-investigation-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-investigation-admin-crud-list',
	templateUrl: './nhis-investigation-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-investigation-admin-crud-list.component.scss',
...
export class AdminNhisInvestigationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisInvestigationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisInvestigationModel.searchFields,
	...
	/**
	 * List of NHIS Investigations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	nhisInvestigations: Observable<NhisInvestigationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	nhisInvestigationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = NhisInvestigationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NhisInvestigationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '1d7e0908-f03e-489a-877a-4f25432f1592';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisInvestigationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisInvestigationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisInvestigationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisInvestigationAction(
				modelAction.NhisInvestigationModelActionTypes.INITIALISE_NHIS_INVESTIGATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisInvestigationAction(
				modelAction.NhisInvestigationModelActionTypes.FETCH_NHIS_INVESTIGATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisInvestigationAction(
						modelAction.NhisInvestigationModelActionTypes.COUNT_NHIS_INVESTIGATIONS,
						null
					)
				]
			))
		...
		this.nhisInvestigations = this.store.select(getNhisInvestigationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisInvestigationCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisInvestigationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('nhis-investigation');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisInvestigationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisInvestigationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisInvestigationModelActionTypes.EXPORT_ALL_NHIS_INVESTIGATION;
		} else {
			actionType = modelAction.NhisInvestigationModelActionTypes.EXPORT_NHIS_INVESTIGATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisInvestigationAction(
				modelAction.NhisInvestigationModelActionTypes.IMPORT_NHIS_INVESTIGATIONS,
				{
					file: file,
				},
				[new modelAction.NhisInvestigationAction(
			...
			window.open(environment.API_URL + '/api/nhisinvestigation/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisInvestigationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Investigation entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Investigation entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisInvestigationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisInvestigationModelActionTypes.DELETE_NHIS_INVESTIGATION;
						} else if (usingSelected) {
							actionType = modelAction.NhisInvestigationModelActionTypes.DELETE_ALL_NHIS_INVESTIGATION;
						} else {
							actionType = modelAction.NhisInvestigationModelActionTypes.DELETE_NHIS_INVESTIGATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisInvestigationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_INVESTIGATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_INVESTIGATION and DELETE_NHIS_INVESTIGATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisInvestigationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisInvestigationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisInvestigationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
