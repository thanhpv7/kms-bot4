...
import * as modelAction from 'src/app/models/warehouseInventory/warehouse_inventory.model.action';
import {WarehouseInventoryModelState, WarehouseInventoryModelAudit} from 'src/app/models/warehouseInventory/warehouse_inventory.model.state';
import {WarehouseInventoryModel} from 'src/app/models/warehouseInventory/warehouse_inventory.model';
import {
	getWarehouseInventoryModelAuditsByEntityId,
...
import {CrudTileMode} from '../warehouse-inventory-admin-crud.component';
...
@Component({
	selector: 'cb-warehouse-inventory-admin-crud-edit',
	templateUrl: './warehouse-inventory-admin-crud-edit.component.html',
	styleUrls: [
		'./warehouse-inventory-admin-crud-edit.component.scss',
...
export class AdminWarehouseInventoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'dc3877cf-82f1-4fcb-b8fd-cdd020d29ddd';
	 ...
	 */
	warehouseInventoryAudits$: Observable<WarehouseInventoryModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Location and Warehouse Inventories here] off begin
	locationMultiCrudActive: boolean = false;
	locationDisplayName = 'Location';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Location and Warehouse Inventories here] end
	locationOppositeRelation = 'warehouseInventories';
	...
	stockCatalogueOppositeRelation = 'warehouseInventories';
	...
	warehouseOppositeRelation = 'warehouseInventories';
	 ...
	 */
	modelProperties: ModelProperty[] = WarehouseInventoryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = WarehouseInventoryModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = WarehouseInventoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: WarehouseInventoryModel;
	...
	constructor(
		private readonly store: Store<{ model: WarehouseInventoryModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountWarehouseInventoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new WarehouseInventoryModel();
		...
		if (this.targetModelId) {
			this.warehouseInventoryAudits$ = this.store.select(getWarehouseInventoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<WarehouseInventoryModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.WarehouseInventoryAction(
			modelAction.WarehouseInventoryModelActionTypes.FETCH_WAREHOUSE_INVENTORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getWarehouseInventoryModelWithId, this.targetModelId)
			...
			model = new WarehouseInventoryModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('warehouse-inventory');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.WarehouseInventoryAction(
			modelAction.WarehouseInventoryModelActionTypes.FETCH_WAREHOUSE_INVENTORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<WarehouseInventoryModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.WarehouseInventoryAction(
						modelAction.WarehouseInventoryModelActionTypes.COUNT_WAREHOUSE_INVENTORYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.WarehouseInventoryAction(
				modelAction.WarehouseInventoryModelActionTypes.CREATE_WAREHOUSE_INVENTORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.WarehouseInventoryAction(
				modelAction.WarehouseInventoryModelActionTypes.UPDATE_WAREHOUSE_INVENTORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: WarehouseInventoryModel = new WarehouseInventoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getWarehouseInventoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new WarehouseInventoryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = WarehouseInventoryModel.getProps();
		this.modelRelations = WarehouseInventoryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: WarehouseInventoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): WarehouseInventoryModel {
		let updatedModel: WarehouseInventoryModel = new WarehouseInventoryModel(model);
