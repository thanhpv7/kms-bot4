...
import {AdminWarehouseInventoryCrudListComponent} from './warehouse-inventory-admin-crud-list.component';
...
import {AdminWarehouseInventoryCrudModule} from '../warehouse-inventory-admin-crud.module';
import {
 ...
 */
describe('Warehouse Inventory Crud List Tile Component', () => {

	let fixture;
	let warehouseInventoryCrudListComponent: AdminWarehouseInventoryCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<WarehouseInventoryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'dc3877cf-82f1-4fcb-b8fd-cdd020d29ddd';
	const searchContent = 'text';

	const searchableFieldCount = WarehouseInventoryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'warehouse-inventory-admin-crud',
		urls: ['warehouse-inventory-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const warehouseInventoryDataFactory = new WarehouseInventoryDataFactory();

	const defaultDataSize = 30;
	const defaultData = warehouseInventoryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getWarehouseInventoryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getWarehouseInventoryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminWarehouseInventoryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminWarehouseInventoryCrudListComponent);
			warehouseInventoryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(warehouseInventoryCrudListComponent.warehouseInventorysCount).toBe(defaultData.length);
			...
			expect(warehouseInventoryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			warehouseInventoryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = warehouseInventoryDataFactory.createAll(filteredDataSize);
			...
			expect(warehouseInventoryCrudListComponent.warehouseInventorysCount).toBe(filteredDataSize);
			expect(warehouseInventoryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(warehouseInventoryCrudListComponent.pageIndex).toBe(0);
			expect(warehouseInventoryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			warehouseInventoryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		warehouseInventoryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		warehouseInventoryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
