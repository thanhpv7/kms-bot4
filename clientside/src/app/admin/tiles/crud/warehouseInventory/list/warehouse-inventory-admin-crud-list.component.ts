...
import {WarehouseInventoryModelState} from 'src/app/models/warehouseInventory/warehouse_inventory.model.state';
...
import {
	...
	getWarehouseInventoryCollectionCount
...
@Component({
	selector: 'cb-warehouse-inventory-admin-crud-list',
	templateUrl: './warehouse-inventory-admin-crud-list.component.html',
	styleUrls: [
		'./warehouse-inventory-admin-crud-list.component.scss',
...
export class AdminWarehouseInventoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<WarehouseInventoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...WarehouseInventoryModel.searchFields,
	...
	/**
	 * List of Warehouse Inventorys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	warehouseInventorys: Observable<WarehouseInventoryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	warehouseInventorysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.warehouseInventorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.warehouseInventorysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseInventoryAction(
				modelAction.WarehouseInventoryModelActionTypes.INITIALISE_WAREHOUSE_INVENTORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.WarehouseInventoryAction(
				modelAction.WarehouseInventoryModelActionTypes.FETCH_WAREHOUSE_INVENTORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.warehouseInventorys = this.store.select(getWarehouseInventoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getWarehouseInventoryCollectionCount, this.collectionId).subscribe(
				(count) => this.warehouseInventorysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: WarehouseInventoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.WarehouseInventoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.WarehouseInventoryModelActionTypes.EXPORT_ALL_WAREHOUSE_INVENTORY;
		} else {
			actionType = modelAction.WarehouseInventoryModelActionTypes.EXPORT_WAREHOUSE_INVENTORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.WarehouseInventoryAction(
				modelAction.WarehouseInventoryModelActionTypes.IMPORT_WAREHOUSE_INVENTORYS,
				{
					file: file,
				},
				[new modelAction.WarehouseInventoryAction(
			...
			window.open(environment.API_URL + '/api/warehouseinventory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: WarehouseInventoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Warehouse Inventory entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Warehouse Inventory entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.WarehouseInventoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.WarehouseInventoryModelActionTypes.DELETE_WAREHOUSE_INVENTORY;
						} else if (usingSelected) {
							actionType = modelAction.WarehouseInventoryModelActionTypes.DELETE_ALL_WAREHOUSE_INVENTORY;
						} else {
							actionType = modelAction.WarehouseInventoryModelActionTypes.DELETE_WAREHOUSE_INVENTORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.WarehouseInventoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_WAREHOUSE_INVENTORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_WAREHOUSE_INVENTORY and DELETE_WAREHOUSE_INVENTORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = WarehouseInventoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.warehouseInventorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
