...
import {BpjsPcarePendaftaranModelState, BpjsPcarePendaftaranModelAudit} from 'src/app/models/bpjsPcarePendaftaran/bpjs_pcare_pendaftaran.model.state';
import {BpjsPcarePendaftaranModel} from 'src/app/models/bpjsPcarePendaftaran/bpjs_pcare_pendaftaran.model';
import {
	getBpjsPcarePendaftaranModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-pcare-pendaftaran-admin-crud-edit',
	templateUrl: './bpjs-pcare-pendaftaran-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-pendaftaran-admin-crud-edit.component.scss',
...
export class AdminBpjsPcarePendaftaranCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcarePendaftaranAudits$: Observable<BpjsPcarePendaftaranModelAudit[]>;
	...
	patientVisitOppositeRelation = 'bpjsPcarePendaftarans';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcarePendaftaranModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcarePendaftaranModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcarePendaftaranModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcarePendaftaranModel();
		...
		if (this.targetModelId) {
			this.bpjsPcarePendaftaranAudits$ = this.store.select(getBpjsPcarePendaftaranModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcarePendaftaranModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
			modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_BPJS_PCARE_PENDAFTARAN,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcarePendaftaranModelWithId, this.targetModelId)
			...
			model = new BpjsPcarePendaftaranModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
			modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_BPJS_PCARE_PENDAFTARAN_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
				modelAction.BpjsPcarePendaftaranModelActionTypes.CREATE_BPJS_PCARE_PENDAFTARAN,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
				modelAction.BpjsPcarePendaftaranModelActionTypes.UPDATE_BPJS_PCARE_PENDAFTARAN,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcarePendaftaranModel = new BpjsPcarePendaftaranModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcarePendaftaranModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcarePendaftaranModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcarePendaftaranModel.getProps();
		this.modelRelations = BpjsPcarePendaftaranModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcarePendaftaranModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcarePendaftaranModel {
		let updatedModel: BpjsPcarePendaftaranModel = new BpjsPcarePendaftaranModel(model);
