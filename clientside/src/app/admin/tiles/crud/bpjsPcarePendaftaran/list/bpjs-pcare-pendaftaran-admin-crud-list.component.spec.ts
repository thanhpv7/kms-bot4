...
import {AdminBpjsPcarePendaftaranCrudListComponent} from './bpjs-pcare-pendaftaran-admin-crud-list.component';
...
import {AdminBpjsPcarePendaftaranCrudModule} from '../bpjs-pcare-pendaftaran-admin-crud.module';
import {
 ...
 */
describe('Bpjs Pcare Pendaftaran Crud List Tile Component', () => {

	let fixture;
	let bpjsPcarePendaftaranCrudListComponent: AdminBpjsPcarePendaftaranCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPcarePendaftaranModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c491bb2f-f6df-49fc-b13d-902823fec076';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPcarePendaftaranModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-pcare-pendaftaran-admin-crud',
		urls: ['bpjs-pcare-pendaftaran-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPcarePendaftaranDataFactory = new BpjsPcarePendaftaranDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPcarePendaftaranDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPcarePendaftaranCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPcarePendaftaranCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsPcarePendaftaranCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsPcarePendaftaranCrudListComponent);
			bpjsPcarePendaftaranCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPcarePendaftaranCrudListComponent.bpjsPcarePendaftaransCount).toBe(defaultData.length);
			...
			expect(bpjsPcarePendaftaranCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPcarePendaftaranCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPcarePendaftaranDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPcarePendaftaranCrudListComponent.bpjsPcarePendaftaransCount).toBe(filteredDataSize);
			expect(bpjsPcarePendaftaranCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPcarePendaftaranCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPcarePendaftaranCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPcarePendaftaranCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPcarePendaftaranCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPcarePendaftaranCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
