...
import * as modelAction from 'src/app/models/bpjsPcarePendaftaran/bpjs_pcare_pendaftaran.model.action';
import {BpjsPcarePendaftaranModelState} from 'src/app/models/bpjsPcarePendaftaran/bpjs_pcare_pendaftaran.model.state';
...
import {
	...
	getBpjsPcarePendaftaranCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-pendaftaran-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-pendaftaran-admin-crud-list',
	templateUrl: './bpjs-pcare-pendaftaran-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-pendaftaran-admin-crud-list.component.scss',
...
export class AdminBpjsPcarePendaftaranCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcarePendaftaranModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcarePendaftaranModel.searchFields,
	...
	/**
	 * List of Bpjs Pcare Pendaftarans that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcarePendaftarans: Observable<BpjsPcarePendaftaranModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcarePendaftaransCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcarePendaftaranModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcarePendaftaranModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'c491bb2f-f6df-49fc-b13d-902823fec076';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcarePendaftaransCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcarePendaftaransCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcarePendaftaranModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
				modelAction.BpjsPcarePendaftaranModelActionTypes.INITIALISE_BPJS_PCARE_PENDAFTARAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
				modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_BPJS_PCARE_PENDAFTARAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcarePendaftaranAction(
						modelAction.BpjsPcarePendaftaranModelActionTypes.COUNT_BPJS_PCARE_PENDAFTARANS,
						null
					)
				]
			))
		...
		this.bpjsPcarePendaftarans = this.store.select(getBpjsPcarePendaftaranCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcarePendaftaranCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcarePendaftaransCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-pendaftaran');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcarePendaftaranModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcarePendaftaranModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcarePendaftaranModelActionTypes.EXPORT_ALL_BPJS_PCARE_PENDAFTARAN;
		} else {
			actionType = modelAction.BpjsPcarePendaftaranModelActionTypes.EXPORT_BPJS_PCARE_PENDAFTARAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
				modelAction.BpjsPcarePendaftaranModelActionTypes.IMPORT_BPJS_PCARE_PENDAFTARANS,
				{
					file: file,
				},
				[new modelAction.BpjsPcarePendaftaranAction(
			...
			window.open(environment.API_URL + '/api/bpjspcarependaftaran/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcarePendaftaranModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Bpjs Pcare Pendaftaran entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Bpjs Pcare Pendaftaran entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcarePendaftaranModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcarePendaftaranModelActionTypes.DELETE_BPJS_PCARE_PENDAFTARAN;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcarePendaftaranModelActionTypes.DELETE_ALL_BPJS_PCARE_PENDAFTARAN;
						} else {
							actionType = modelAction.BpjsPcarePendaftaranModelActionTypes.DELETE_BPJS_PCARE_PENDAFTARAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcarePendaftaranAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_PENDAFTARAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_PENDAFTARAN and DELETE_BPJS_PCARE_PENDAFTARAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcarePendaftaranModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcarePendaftaranModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcarePendaftaransCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
