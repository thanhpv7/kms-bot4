...
import * as modelAction from 'src/app/models/invoicePaymentInsurance/invoice_payment_insurance.model.action';
import {InvoicePaymentInsuranceModelState, InvoicePaymentInsuranceModelAudit} from 'src/app/models/invoicePaymentInsurance/invoice_payment_insurance.model.state';
import {InvoicePaymentInsuranceModel} from 'src/app/models/invoicePaymentInsurance/invoice_payment_insurance.model';
import {
	getInvoicePaymentInsuranceModelAuditsByEntityId,
	getInvoicePaymentInsuranceModelWithId,
	getCountInvoicePaymentInsuranceModels,
...
import {CrudTileMode} from '../invoice-payment-insurance-admin-crud.component';
...
@Component({
	selector: 'cb-invoice-payment-insurance-admin-crud-edit',
	templateUrl: './invoice-payment-insurance-admin-crud-edit.component.html',
	styleUrls: [
		'./invoice-payment-insurance-admin-crud-edit.component.scss',
...
export class AdminInvoicePaymentInsuranceCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '316fad99-f361-44ab-a09d-19c6109c0a67';
	 ...
	 */
	invoicePaymentInsuranceAudits$: Observable<InvoicePaymentInsuranceModelAudit[]>;
	...
	insuranceBenefitPlanOppositeRelation = 'invoicePaymentInsurances';
	...
	invoiceOppositeRelation = 'invoicePaymentInsurances';
	 ...
	 */
	modelProperties: ModelProperty[] = InvoicePaymentInsuranceModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InvoicePaymentInsuranceModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InvoicePaymentInsuranceModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: InvoicePaymentInsuranceModel;
	...
	constructor(
		private readonly store: Store<{ model: InvoicePaymentInsuranceModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInvoicePaymentInsuranceModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InvoicePaymentInsuranceModel();
		...
		if (this.targetModelId) {
			this.invoicePaymentInsuranceAudits$ = this.store.select(getInvoicePaymentInsuranceModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InvoicePaymentInsuranceModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
			modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_INVOICE_PAYMENT_INSURANCE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInvoicePaymentInsuranceModelWithId, this.targetModelId)
			...
			model = new InvoicePaymentInsuranceModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('invoice-payment-insurance');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
			modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_INVOICE_PAYMENT_INSURANCE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InvoicePaymentInsuranceModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.insuranceBenefitPlanMultiCrudActive || this.invoiceMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InvoicePaymentInsuranceAction(
						modelAction.InvoicePaymentInsuranceModelActionTypes.COUNT_INVOICE_PAYMENT_INSURANCES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
				modelAction.InvoicePaymentInsuranceModelActionTypes.CREATE_INVOICE_PAYMENT_INSURANCE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
				modelAction.InvoicePaymentInsuranceModelActionTypes.UPDATE_INVOICE_PAYMENT_INSURANCE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InvoicePaymentInsuranceModel = new InvoicePaymentInsuranceModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInvoicePaymentInsuranceModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InvoicePaymentInsuranceModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InvoicePaymentInsuranceModel.getProps();
		this.modelRelations = InvoicePaymentInsuranceModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: InvoicePaymentInsuranceModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InvoicePaymentInsuranceModel {
		let updatedModel: InvoicePaymentInsuranceModel = new InvoicePaymentInsuranceModel(model);
