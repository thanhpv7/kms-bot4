...
import {InvoicePaymentInsuranceModelState} from 'src/app/models/invoicePaymentInsurance/invoice_payment_insurance.model.state';
...
import {
	getInvoicePaymentInsuranceCollectionModels,
	getInvoicePaymentInsuranceCollectionCount
...
@Component({
	selector: 'cb-invoice-payment-insurance-admin-crud-list',
	templateUrl: './invoice-payment-insurance-admin-crud-list.component.html',
	styleUrls: [
		'./invoice-payment-insurance-admin-crud-list.component.scss',
...
export class AdminInvoicePaymentInsuranceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InvoicePaymentInsuranceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InvoicePaymentInsuranceModel.searchFields,
	...
	/**
	 * List of Invoice Payment Insurances that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	invoicePaymentInsurances: Observable<InvoicePaymentInsuranceModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	invoicePaymentInsurancesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.invoicePaymentInsurancesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.invoicePaymentInsurancesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
				modelAction.InvoicePaymentInsuranceModelActionTypes.INITIALISE_INVOICE_PAYMENT_INSURANCE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
				modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_INVOICE_PAYMENT_INSURANCE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.invoicePaymentInsurances = this.store.select(getInvoicePaymentInsuranceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInvoicePaymentInsuranceCollectionCount, this.collectionId).subscribe(
				(count) => this.invoicePaymentInsurancesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InvoicePaymentInsuranceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InvoicePaymentInsuranceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InvoicePaymentInsuranceModelActionTypes.EXPORT_ALL_INVOICE_PAYMENT_INSURANCE;
		} else {
			actionType = modelAction.InvoicePaymentInsuranceModelActionTypes.EXPORT_INVOICE_PAYMENT_INSURANCE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
				modelAction.InvoicePaymentInsuranceModelActionTypes.IMPORT_INVOICE_PAYMENT_INSURANCES,
				{
					file: file,
				},
				[new modelAction.InvoicePaymentInsuranceAction(
			...
			window.open(environment.API_URL + '/api/invoicepaymentinsurance/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InvoicePaymentInsuranceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Invoice Payment Insurance entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Invoice Payment Insurance entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InvoicePaymentInsuranceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InvoicePaymentInsuranceModelActionTypes.DELETE_INVOICE_PAYMENT_INSURANCE;
						} else if (usingSelected) {
							actionType = modelAction.InvoicePaymentInsuranceModelActionTypes.DELETE_ALL_INVOICE_PAYMENT_INSURANCE;
						} else {
							actionType = modelAction.InvoicePaymentInsuranceModelActionTypes.DELETE_INVOICE_PAYMENT_INSURANCE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InvoicePaymentInsuranceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVOICE_PAYMENT_INSURANCE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVOICE_PAYMENT_INSURANCE and DELETE_INVOICE_PAYMENT_INSURANCE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InvoicePaymentInsuranceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.invoicePaymentInsurancesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
