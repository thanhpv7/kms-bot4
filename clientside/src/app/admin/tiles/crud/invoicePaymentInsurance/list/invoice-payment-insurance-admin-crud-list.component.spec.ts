...
import {AdminInvoicePaymentInsuranceCrudListComponent} from './invoice-payment-insurance-admin-crud-list.component';
...
import {AdminInvoicePaymentInsuranceCrudModule} from '../invoice-payment-insurance-admin-crud.module';
import {
	InvoicePaymentInsuranceModelState,
	initialState as initialInvoicePaymentInsuranceModelState
} from 'src/app/models/invoicePaymentInsurance/invoice_payment_insurance.model.state';
 ...
 */
describe('Invoice Payment Insurance Crud List Tile Component', () => {

	let fixture;
	let invoicePaymentInsuranceCrudListComponent: AdminInvoicePaymentInsuranceCrudListComponent;

	let store: MockStore<{ model: InvoicePaymentInsuranceModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let invoicePaymentInsuranceModelState: InvoicePaymentInsuranceModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InvoicePaymentInsuranceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '316fad99-f361-44ab-a09d-19c6109c0a67';
	const searchContent = 'text';

	const searchableFieldCount = InvoicePaymentInsuranceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'invoice-payment-insurance-admin-crud',
		urls: ['invoice-payment-insurance-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const invoicePaymentInsuranceDataFactory = new InvoicePaymentInsuranceDataFactory();

	const defaultDataSize = 30;
	const defaultData = invoicePaymentInsuranceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InvoicePaymentInsuranceModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		invoicePaymentInsuranceModelState = initialInvoicePaymentInsuranceModelState;
		store.setState({model: invoicePaymentInsuranceModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInvoicePaymentInsuranceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInvoicePaymentInsuranceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInvoicePaymentInsuranceCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInvoicePaymentInsuranceCrudListComponent);
			invoicePaymentInsuranceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(invoicePaymentInsuranceCrudListComponent.invoicePaymentInsurancesCount).toBe(defaultData.length);
			...
			expect(invoicePaymentInsuranceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			invoicePaymentInsuranceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = invoicePaymentInsuranceDataFactory.createAll(filteredDataSize);
			...
			expect(invoicePaymentInsuranceCrudListComponent.invoicePaymentInsurancesCount).toBe(filteredDataSize);
			expect(invoicePaymentInsuranceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(invoicePaymentInsuranceCrudListComponent.pageIndex).toBe(0);
			expect(invoicePaymentInsuranceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			invoicePaymentInsuranceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		invoicePaymentInsuranceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		invoicePaymentInsuranceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
