...
import {AdminBpjsDoctorScheduleCrudListComponent} from './bpjs-doctor-schedule-admin-crud-list.component';
...
import {AdminBpjsDoctorScheduleCrudModule} from '../bpjs-doctor-schedule-admin-crud.module';
import {
 ...
 */
describe('BPJS Doctor Schedule Crud List Tile Component', () => {

	let fixture;
	let bpjsDoctorScheduleCrudListComponent: AdminBpjsDoctorScheduleCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsDoctorScheduleModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '56be54a7-e503-49d7-9f37-f1dc4c912a5a';
	const searchContent = 'text';

	const searchableFieldCount = BpjsDoctorScheduleModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-doctor-schedule-admin-crud',
		urls: ['bpjs-doctor-schedule-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsDoctorScheduleDataFactory = new BpjsDoctorScheduleDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsDoctorScheduleDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsDoctorScheduleCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsDoctorScheduleCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsDoctorScheduleCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsDoctorScheduleCrudListComponent);
			bpjsDoctorScheduleCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsDoctorScheduleCrudListComponent.bpjsDoctorSchedulesCount).toBe(defaultData.length);
			...
			expect(bpjsDoctorScheduleCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsDoctorScheduleCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsDoctorScheduleDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsDoctorScheduleCrudListComponent.bpjsDoctorSchedulesCount).toBe(filteredDataSize);
			expect(bpjsDoctorScheduleCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsDoctorScheduleCrudListComponent.pageIndex).toBe(0);
			expect(bpjsDoctorScheduleCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsDoctorScheduleCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsDoctorScheduleCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsDoctorScheduleCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
