...
import * as modelAction from 'src/app/models/bpjsDoctorSchedule/bpjs_doctor_schedule.model.action';
import {BpjsDoctorScheduleModelState} from 'src/app/models/bpjsDoctorSchedule/bpjs_doctor_schedule.model.state';
...
import {
	...
	getBpjsDoctorScheduleCollectionCount
...
import {CrudTileMode} from '../bpjs-doctor-schedule-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-doctor-schedule-admin-crud-list',
	templateUrl: './bpjs-doctor-schedule-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-doctor-schedule-admin-crud-list.component.scss',
...
export class AdminBpjsDoctorScheduleCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsDoctorScheduleModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsDoctorScheduleModel.searchFields,
	...
	/**
	 * List of BPJS Doctor Schedules that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsDoctorSchedules: Observable<BpjsDoctorScheduleModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsDoctorSchedulesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsDoctorScheduleModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsDoctorScheduleModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '56be54a7-e503-49d7-9f37-f1dc4c912a5a';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsDoctorSchedulesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsDoctorSchedulesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsDoctorScheduleModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
				modelAction.BpjsDoctorScheduleModelActionTypes.INITIALISE_BPJS_DOCTOR_SCHEDULE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
				modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_BPJS_DOCTOR_SCHEDULE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsDoctorScheduleAction(
						modelAction.BpjsDoctorScheduleModelActionTypes.COUNT_BPJS_DOCTOR_SCHEDULES,
						null
					)
				]
			))
		...
		this.bpjsDoctorSchedules = this.store.select(getBpjsDoctorScheduleCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsDoctorScheduleCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsDoctorSchedulesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-doctor-schedule');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsDoctorScheduleModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsDoctorScheduleModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsDoctorScheduleModelActionTypes.EXPORT_ALL_BPJS_DOCTOR_SCHEDULE;
		} else {
			actionType = modelAction.BpjsDoctorScheduleModelActionTypes.EXPORT_BPJS_DOCTOR_SCHEDULE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
				modelAction.BpjsDoctorScheduleModelActionTypes.IMPORT_BPJS_DOCTOR_SCHEDULES,
				{
					file: file,
				},
				[new modelAction.BpjsDoctorScheduleAction(
			...
			window.open(environment.API_URL + '/api/bpjsdoctorschedule/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsDoctorScheduleModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Doctor Schedule entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Doctor Schedule entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsDoctorScheduleModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsDoctorScheduleModelActionTypes.DELETE_BPJS_DOCTOR_SCHEDULE;
						} else if (usingSelected) {
							actionType = modelAction.BpjsDoctorScheduleModelActionTypes.DELETE_ALL_BPJS_DOCTOR_SCHEDULE;
						} else {
							actionType = modelAction.BpjsDoctorScheduleModelActionTypes.DELETE_BPJS_DOCTOR_SCHEDULE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_DOCTOR_SCHEDULE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_DOCTOR_SCHEDULE and DELETE_BPJS_DOCTOR_SCHEDULE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsDoctorScheduleModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsDoctorScheduleModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsDoctorSchedulesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
