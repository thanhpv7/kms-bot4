...
import {BpjsDoctorScheduleModelState, BpjsDoctorScheduleModelAudit} from 'src/app/models/bpjsDoctorSchedule/bpjs_doctor_schedule.model.state';
import {BpjsDoctorScheduleModel} from 'src/app/models/bpjsDoctorSchedule/bpjs_doctor_schedule.model';
import {
	getBpjsDoctorScheduleModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-doctor-schedule-admin-crud-edit',
	templateUrl: './bpjs-doctor-schedule-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-doctor-schedule-admin-crud-edit.component.scss',
...
export class AdminBpjsDoctorScheduleCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsDoctorScheduleAudits$: Observable<BpjsDoctorScheduleModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsDoctorScheduleModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsDoctorScheduleModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsDoctorScheduleModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsDoctorScheduleModel();
		...
		if (this.targetModelId) {
			this.bpjsDoctorScheduleAudits$ = this.store.select(getBpjsDoctorScheduleModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsDoctorScheduleModel> = {
		...
		this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
			modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_BPJS_DOCTOR_SCHEDULE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsDoctorScheduleModelWithId, this.targetModelId)
			...
			model = new BpjsDoctorScheduleModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
			modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_BPJS_DOCTOR_SCHEDULE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
				modelAction.BpjsDoctorScheduleModelActionTypes.CREATE_BPJS_DOCTOR_SCHEDULE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsDoctorScheduleAction(
				modelAction.BpjsDoctorScheduleModelActionTypes.UPDATE_BPJS_DOCTOR_SCHEDULE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsDoctorScheduleModel = new BpjsDoctorScheduleModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsDoctorScheduleModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsDoctorScheduleModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsDoctorScheduleModel.getProps();
		this.modelRelations = BpjsDoctorScheduleModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsDoctorScheduleModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsDoctorScheduleModel {
		let updatedModel: BpjsDoctorScheduleModel = new BpjsDoctorScheduleModel(model);
