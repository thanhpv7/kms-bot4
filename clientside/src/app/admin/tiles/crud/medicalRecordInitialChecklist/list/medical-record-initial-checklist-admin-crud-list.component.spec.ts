...
import {AdminMedicalRecordInitialChecklistCrudListComponent} from './medical-record-initial-checklist-admin-crud-list.component';
...
import {AdminMedicalRecordInitialChecklistCrudModule} from '../medical-record-initial-checklist-admin-crud.module';
import {
	MedicalRecordInitialChecklistModelState,
	initialState as initialMedicalRecordInitialChecklistModelState
} from 'src/app/models/medicalRecordInitialChecklist/medical_record_initial_checklist.model.state';
 ...
 */
describe('Medical Record Initial Checklist Crud List Tile Component', () => {

	let fixture;
	let medicalRecordInitialChecklistCrudListComponent: AdminMedicalRecordInitialChecklistCrudListComponent;

	let store: MockStore<{ model: MedicalRecordInitialChecklistModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalRecordInitialChecklistModelState: MedicalRecordInitialChecklistModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalRecordInitialChecklistModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'e1746a24-a493-41d4-95b6-b383ed71b41f';
	const searchContent = 'text';

	const searchableFieldCount = MedicalRecordInitialChecklistModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-record-initial-checklist-admin-crud',
		urls: ['medical-record-initial-checklist-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalRecordInitialChecklistDataFactory = new MedicalRecordInitialChecklistDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalRecordInitialChecklistDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalRecordInitialChecklistModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalRecordInitialChecklistModelState = initialMedicalRecordInitialChecklistModelState;
		store.setState({model: medicalRecordInitialChecklistModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalRecordInitialChecklistCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalRecordInitialChecklistCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalRecordInitialChecklistCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalRecordInitialChecklistCrudListComponent);
			medicalRecordInitialChecklistCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalRecordInitialChecklistCrudListComponent.medicalRecordInitialChecklistsCount).toBe(defaultData.length);
			...
			expect(medicalRecordInitialChecklistCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalRecordInitialChecklistCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalRecordInitialChecklistDataFactory.createAll(filteredDataSize);
			...
			expect(medicalRecordInitialChecklistCrudListComponent.medicalRecordInitialChecklistsCount).toBe(filteredDataSize);
			expect(medicalRecordInitialChecklistCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalRecordInitialChecklistCrudListComponent.pageIndex).toBe(0);
			expect(medicalRecordInitialChecklistCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalRecordInitialChecklistCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalRecordInitialChecklistCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalRecordInitialChecklistCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
