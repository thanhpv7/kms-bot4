...
import * as modelAction from 'src/app/models/medicalRecordInitialChecklist/medical_record_initial_checklist.model.action';
import {MedicalRecordInitialChecklistModelState} from 'src/app/models/medicalRecordInitialChecklist/medical_record_initial_checklist.model.state';
...
import {
	getMedicalRecordInitialChecklistCollectionModels,
	getMedicalRecordInitialChecklistCollectionCount
...
import {CrudTileMode} from '../medical-record-initial-checklist-admin-crud.component';
...
@Component({
	selector: 'cb-medical-record-initial-checklist-admin-crud-list',
	templateUrl: './medical-record-initial-checklist-admin-crud-list.component.html',
	styleUrls: [
		'./medical-record-initial-checklist-admin-crud-list.component.scss',
...
export class AdminMedicalRecordInitialChecklistCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalRecordInitialChecklistModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalRecordInitialChecklistModel.searchFields,
	...
	/**
	 * List of Medical Record Initial Checklists that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalRecordInitialChecklists: Observable<MedicalRecordInitialChecklistModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalRecordInitialChecklistsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = MedicalRecordInitialChecklistModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalRecordInitialChecklistModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'e1746a24-a493-41d4-95b6-b383ed71b41f';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalRecordInitialChecklistsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalRecordInitialChecklistsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: MedicalRecordInitialChecklistModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalRecordInitialChecklistAction(
				modelAction.MedicalRecordInitialChecklistModelActionTypes.INITIALISE_MEDICAL_RECORD_INITIAL_CHECKLIST_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalRecordInitialChecklistAction(
				modelAction.MedicalRecordInitialChecklistModelActionTypes.FETCH_MEDICAL_RECORD_INITIAL_CHECKLIST_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.MedicalRecordInitialChecklistAction(
						modelAction.MedicalRecordInitialChecklistModelActionTypes.COUNT_MEDICAL_RECORD_INITIAL_CHECKLISTS,
						null
					)
				]
			))
		...
		this.medicalRecordInitialChecklists = this.store.select(getMedicalRecordInitialChecklistCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalRecordInitialChecklistCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalRecordInitialChecklistsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-record-initial-checklist');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalRecordInitialChecklistModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalRecordInitialChecklistModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalRecordInitialChecklistModelActionTypes.EXPORT_ALL_MEDICAL_RECORD_INITIAL_CHECKLIST;
		} else {
			actionType = modelAction.MedicalRecordInitialChecklistModelActionTypes.EXPORT_MEDICAL_RECORD_INITIAL_CHECKLIST_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalRecordInitialChecklistAction(
				modelAction.MedicalRecordInitialChecklistModelActionTypes.IMPORT_MEDICAL_RECORD_INITIAL_CHECKLISTS,
				{
					file: file,
				},
				[new modelAction.MedicalRecordInitialChecklistAction(
			...
			window.open(environment.API_URL + '/api/medicalrecordinitialchecklist/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalRecordInitialChecklistModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Record Initial Checklist entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Record Initial Checklist entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalRecordInitialChecklistModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalRecordInitialChecklistModelActionTypes.DELETE_MEDICAL_RECORD_INITIAL_CHECKLIST;
						} else if (usingSelected) {
							actionType = modelAction.MedicalRecordInitialChecklistModelActionTypes.DELETE_ALL_MEDICAL_RECORD_INITIAL_CHECKLIST;
						} else {
							actionType = modelAction.MedicalRecordInitialChecklistModelActionTypes.DELETE_MEDICAL_RECORD_INITIAL_CHECKLIST_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalRecordInitialChecklistAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_RECORD_INITIAL_CHECKLIST when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_RECORD_INITIAL_CHECKLIST and DELETE_MEDICAL_RECORD_INITIAL_CHECKLIST_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<MedicalRecordInitialChecklistModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalRecordInitialChecklistModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalRecordInitialChecklistsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
