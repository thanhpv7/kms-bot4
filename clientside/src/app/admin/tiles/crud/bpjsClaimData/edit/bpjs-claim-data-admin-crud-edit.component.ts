...
import {BpjsClaimDataModelState, BpjsClaimDataModelAudit} from 'src/app/models/bpjsClaimData/bpjs_claim_data.model.state';
import {BpjsClaimDataModel} from 'src/app/models/bpjsClaimData/bpjs_claim_data.model';
import {
	getBpjsClaimDataModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-claim-data-admin-crud-edit',
	templateUrl: './bpjs-claim-data-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-claim-data-admin-crud-edit.component.scss',
...
export class AdminBpjsClaimDataCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsClaimDataAudits$: Observable<BpjsClaimDataModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsClaimDataModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsClaimDataModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsClaimDataModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsClaimDataModel();
		...
		if (this.targetModelId) {
			this.bpjsClaimDataAudits$ = this.store.select(getBpjsClaimDataModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsClaimDataModel> = {
		...
		this.store.dispatch(new modelAction.BpjsClaimDataAction(
			modelAction.BpjsClaimDataModelActionTypes.FETCH_BPJS_CLAIM_DATA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsClaimDataModelWithId, this.targetModelId)
			...
			model = new BpjsClaimDataModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsClaimDataAction(
			modelAction.BpjsClaimDataModelActionTypes.FETCH_BPJS_CLAIM_DATA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsClaimDataAction(
				modelAction.BpjsClaimDataModelActionTypes.CREATE_BPJS_CLAIM_DATA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsClaimDataAction(
				modelAction.BpjsClaimDataModelActionTypes.UPDATE_BPJS_CLAIM_DATA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsClaimDataModel = new BpjsClaimDataModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsClaimDataModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsClaimDataModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsClaimDataModel.getProps();
		this.modelRelations = BpjsClaimDataModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsClaimDataModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsClaimDataModel {
		let updatedModel: BpjsClaimDataModel = new BpjsClaimDataModel(model);
