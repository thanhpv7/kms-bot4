...
import * as modelAction from 'src/app/models/bpjsClaimData/bpjs_claim_data.model.action';
import {BpjsClaimDataModelState} from 'src/app/models/bpjsClaimData/bpjs_claim_data.model.state';
...
import {
	...
	getBpjsClaimDataCollectionCount
...
import {CrudTileMode} from '../bpjs-claim-data-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-claim-data-admin-crud-list',
	templateUrl: './bpjs-claim-data-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-claim-data-admin-crud-list.component.scss',
...
export class AdminBpjsClaimDataCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsClaimDataModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsClaimDataModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsClaimDataModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '8706083e-0897-494d-ab43-b17a453f10a9';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsClaimDatasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsClaimDatasCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsClaimDataModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsClaimDataAction(
				modelAction.BpjsClaimDataModelActionTypes.INITIALISE_BPJS_CLAIM_DATA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsClaimDataAction(
				modelAction.BpjsClaimDataModelActionTypes.FETCH_BPJS_CLAIM_DATA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsClaimDataAction(
						modelAction.BpjsClaimDataModelActionTypes.COUNT_BPJS_CLAIM_DATAS,
						null
					)
				]
			))
		...
		this.bpjsClaimDatas = this.store.select(getBpjsClaimDataCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsClaimDataCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsClaimDatasCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsClaimDataModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsClaimDataModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsClaimDataModelActionTypes.EXPORT_ALL_BPJS_CLAIM_DATA;
		} else {
			actionType = modelAction.BpjsClaimDataModelActionTypes.EXPORT_BPJS_CLAIM_DATA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsClaimDataAction(
				modelAction.BpjsClaimDataModelActionTypes.IMPORT_BPJS_CLAIM_DATAS,
				{
					file: file,
				},
				[new modelAction.BpjsClaimDataAction(
			...
			window.open(environment.API_URL + '/api/bpjsclaimdata/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsClaimDataModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Claim Data entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Claim Data entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsClaimDataModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsClaimDataModelActionTypes.DELETE_BPJS_CLAIM_DATA;
						} else if (usingSelected) {
							actionType = modelAction.BpjsClaimDataModelActionTypes.DELETE_ALL_BPJS_CLAIM_DATA;
						} else {
							actionType = modelAction.BpjsClaimDataModelActionTypes.DELETE_BPJS_CLAIM_DATA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsClaimDataAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_CLAIM_DATA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_CLAIM_DATA and DELETE_BPJS_CLAIM_DATA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsClaimDataModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsClaimDataModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsClaimDatasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
