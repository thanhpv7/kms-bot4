...
import {AdminBpjsClaimDataCrudListComponent} from './bpjs-claim-data-admin-crud-list.component';
...
import {AdminBpjsClaimDataCrudModule} from '../bpjs-claim-data-admin-crud.module';
import {
 ...
 */
describe('BPJS Claim Data Crud List Tile Component', () => {

	let fixture;
	let bpjsClaimDataCrudListComponent: AdminBpjsClaimDataCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsClaimDataModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8706083e-0897-494d-ab43-b17a453f10a9';
	const searchContent = 'text';

	const searchableFieldCount = BpjsClaimDataModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-claim-data-admin-crud',
		urls: ['bpjs-claim-data-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsClaimDataDataFactory = new BpjsClaimDataDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsClaimDataDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsClaimDataCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsClaimDataCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsClaimDataCrudListComponent);
			bpjsClaimDataCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsClaimDataCrudListComponent.bpjsClaimDatasCount).toBe(defaultData.length);
			...
			expect(bpjsClaimDataCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsClaimDataCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsClaimDataDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsClaimDataCrudListComponent.bpjsClaimDatasCount).toBe(filteredDataSize);
			expect(bpjsClaimDataCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsClaimDataCrudListComponent.pageIndex).toBe(0);
			expect(bpjsClaimDataCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsClaimDataCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsClaimDataCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsClaimDataCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
