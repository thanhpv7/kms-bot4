...
import * as modelAction from 'src/app/models/pcareDiagnosa/pcare_diagnosa.model.action';
import {PcareDiagnosaModelState} from 'src/app/models/pcareDiagnosa/pcare_diagnosa.model.state';
...
import {
	getPcareDiagnosaCollectionModels,
	getPcareDiagnosaCollectionCount
} from 'src/app/models/pcareDiagnosa/pcare_diagnosa.model.selector';
...
import {CrudTileMode} from '../pcare-diagnosa-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-diagnosa-admin-crud-list',
	templateUrl: './pcare-diagnosa-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-diagnosa-admin-crud-list.component.scss',
...
export class AdminPcareDiagnosaCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareDiagnosaModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareDiagnosaModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PcareDiagnosaModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'a7c64a5e-f300-4f48-a2cb-610fd29450a7';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareDiagnosasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareDiagnosasCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareDiagnosaModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareDiagnosaAction(
				modelAction.PcareDiagnosaModelActionTypes.INITIALISE_PCARE_DIAGNOSA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareDiagnosaAction(
				modelAction.PcareDiagnosaModelActionTypes.FETCH_PCARE_DIAGNOSA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareDiagnosaAction(
						modelAction.PcareDiagnosaModelActionTypes.COUNT_PCARE_DIAGNOSAS,
						null
					)
				]
			))
		...
		this.pcareDiagnosas = this.store.select(getPcareDiagnosaCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareDiagnosaCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareDiagnosasCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareDiagnosaModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareDiagnosaModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareDiagnosaModelActionTypes.EXPORT_ALL_PCARE_DIAGNOSA;
		} else {
			actionType = modelAction.PcareDiagnosaModelActionTypes.EXPORT_PCARE_DIAGNOSA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareDiagnosaAction(
				modelAction.PcareDiagnosaModelActionTypes.IMPORT_PCARE_DIAGNOSAS,
				{
					file: file,
				},
				[new modelAction.PcareDiagnosaAction(
			...
			window.open(environment.API_URL + '/api/pcarediagnosa/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareDiagnosaModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Diagnosa entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Diagnosa entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareDiagnosaModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareDiagnosaModelActionTypes.DELETE_PCARE_DIAGNOSA;
						} else if (usingSelected) {
							actionType = modelAction.PcareDiagnosaModelActionTypes.DELETE_ALL_PCARE_DIAGNOSA;
						} else {
							actionType = modelAction.PcareDiagnosaModelActionTypes.DELETE_PCARE_DIAGNOSA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareDiagnosaAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_DIAGNOSA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_DIAGNOSA and DELETE_PCARE_DIAGNOSA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareDiagnosaModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareDiagnosaModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareDiagnosasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
