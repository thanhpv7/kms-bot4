...
import {AdminPcareDiagnosaCrudListComponent} from './pcare-diagnosa-admin-crud-list.component';
import {PcareDiagnosaDataFactory} from 'src/app/lib/utils/factories/pcare-diagnosa-data-factory';
import {PcareDiagnosaModel} from 'src/app/models/pcareDiagnosa/pcare_diagnosa.model';
import {AdminPcareDiagnosaCrudModule} from '../pcare-diagnosa-admin-crud.module';
import {
	PcareDiagnosaModelState,
	initialState as initialPcareDiagnosaModelState
} from 'src/app/models/pcareDiagnosa/pcare_diagnosa.model.state';
 ...
 */
describe('PCare Diagnosa Crud List Tile Component', () => {

	let fixture;
	let pcareDiagnosaCrudListComponent: AdminPcareDiagnosaCrudListComponent;

	let store: MockStore<{ model: PcareDiagnosaModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareDiagnosaModelState: PcareDiagnosaModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareDiagnosaModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a7c64a5e-f300-4f48-a2cb-610fd29450a7';
	const searchContent = 'text';

	const searchableFieldCount = PcareDiagnosaModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-diagnosa-admin-crud',
		urls: ['pcare-diagnosa-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareDiagnosaDataFactory = new PcareDiagnosaDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareDiagnosaDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareDiagnosaModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareDiagnosaModelState = initialPcareDiagnosaModelState;
		store.setState({model: pcareDiagnosaModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareDiagnosaCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareDiagnosaCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPcareDiagnosaCrudListComponent);
			pcareDiagnosaCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareDiagnosaCrudListComponent.pcareDiagnosasCount).toBe(defaultData.length);
			...
			expect(pcareDiagnosaCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareDiagnosaCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareDiagnosaDataFactory.createAll(filteredDataSize);
			...
			expect(pcareDiagnosaCrudListComponent.pcareDiagnosasCount).toBe(filteredDataSize);
			expect(pcareDiagnosaCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareDiagnosaCrudListComponent.pageIndex).toBe(0);
			expect(pcareDiagnosaCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareDiagnosaCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareDiagnosaCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareDiagnosaCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
