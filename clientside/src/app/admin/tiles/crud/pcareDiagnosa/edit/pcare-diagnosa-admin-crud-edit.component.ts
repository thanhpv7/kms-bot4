...
import {PcareDiagnosaModelState, PcareDiagnosaModelAudit} from 'src/app/models/pcareDiagnosa/pcare_diagnosa.model.state';
import {PcareDiagnosaModel} from 'src/app/models/pcareDiagnosa/pcare_diagnosa.model';
import {
	getPcareDiagnosaModelAuditsByEntityId,
	getPcareDiagnosaModelWithId,
	getCountPcareDiagnosaModels,
...
@Component({
	selector: 'cb-pcare-diagnosa-admin-crud-edit',
	templateUrl: './pcare-diagnosa-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-diagnosa-admin-crud-edit.component.scss',
...
export class AdminPcareDiagnosaCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareDiagnosaAudits$: Observable<PcareDiagnosaModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareDiagnosaModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PcareDiagnosaModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareDiagnosaModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareDiagnosaModel();
		...
		if (this.targetModelId) {
			this.pcareDiagnosaAudits$ = this.store.select(getPcareDiagnosaModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareDiagnosaModel> = {
		...
		this.store.dispatch(new modelAction.PcareDiagnosaAction(
			modelAction.PcareDiagnosaModelActionTypes.FETCH_PCARE_DIAGNOSA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareDiagnosaModelWithId, this.targetModelId)
			...
			model = new PcareDiagnosaModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareDiagnosaAction(
			modelAction.PcareDiagnosaModelActionTypes.FETCH_PCARE_DIAGNOSA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareDiagnosaAction(
				modelAction.PcareDiagnosaModelActionTypes.CREATE_PCARE_DIAGNOSA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareDiagnosaAction(
				modelAction.PcareDiagnosaModelActionTypes.UPDATE_PCARE_DIAGNOSA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareDiagnosaModel = new PcareDiagnosaModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareDiagnosaModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareDiagnosaModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareDiagnosaModel.getProps();
		this.modelRelations = PcareDiagnosaModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareDiagnosaModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareDiagnosaModel {
		let updatedModel: PcareDiagnosaModel = new PcareDiagnosaModel(model);
