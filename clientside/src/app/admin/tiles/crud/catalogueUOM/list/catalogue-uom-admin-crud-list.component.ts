...
import * as modelAction from 'src/app/models/catalogueUOM/catalogue_uom.model.action';
import {CatalogueUOMModelState} from 'src/app/models/catalogueUOM/catalogue_uom.model.state';
...
import {
	getCatalogueUOMCollectionModels,
	getCatalogueUOMCollectionCount
...
import {CrudTileMode} from '../catalogue-uom-admin-crud.component';
...
@Component({
	selector: 'cb-catalogue-uom-admin-crud-list',
	templateUrl: './catalogue-uom-admin-crud-list.component.html',
	styleUrls: [
		'./catalogue-uom-admin-crud-list.component.scss',
...
export class AdminCatalogueUOMCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CatalogueUOMModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CatalogueUOMModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = CatalogueUOMModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '902d70cd-41ef-485e-bdb0-cd529ea61851';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.catalogueUOMsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.catalogueUOMsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: CatalogueUOMModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CatalogueUOMAction(
				modelAction.CatalogueUOMModelActionTypes.INITIALISE_CATALOGUE_UOM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CatalogueUOMAction(
				modelAction.CatalogueUOMModelActionTypes.FETCH_CATALOGUE_UOM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.CatalogueUOMAction(
						modelAction.CatalogueUOMModelActionTypes.COUNT_CATALOGUE_UOMS,
						null
					)
				]
			))
		...
		this.catalogueUOMs = this.store.select(getCatalogueUOMCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCatalogueUOMCollectionCount, this.collectionId).subscribe(
				(count) => this.catalogueUOMsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CatalogueUOMModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CatalogueUOMModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CatalogueUOMModelActionTypes.EXPORT_ALL_CATALOGUE_UOM;
		} else {
			actionType = modelAction.CatalogueUOMModelActionTypes.EXPORT_CATALOGUE_UOM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CatalogueUOMAction(
				modelAction.CatalogueUOMModelActionTypes.IMPORT_CATALOGUE_UOMS,
				{
					file: file,
				},
				[new modelAction.CatalogueUOMAction(
			...
			window.open(environment.API_URL + '/api/catalogueuom/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CatalogueUOMModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Catalogue UOM entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Catalogue UOM entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CatalogueUOMModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CatalogueUOMModelActionTypes.DELETE_CATALOGUE_UOM;
						} else if (usingSelected) {
							actionType = modelAction.CatalogueUOMModelActionTypes.DELETE_ALL_CATALOGUE_UOM;
						} else {
							actionType = modelAction.CatalogueUOMModelActionTypes.DELETE_CATALOGUE_UOM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CatalogueUOMAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CATALOGUE_UOM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CATALOGUE_UOM and DELETE_CATALOGUE_UOM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<CatalogueUOMModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CatalogueUOMModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.catalogueUOMsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
