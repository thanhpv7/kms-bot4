...
import {AdminCatalogueUOMCrudListComponent} from './catalogue-uom-admin-crud-list.component';
...
import {AdminCatalogueUOMCrudModule} from '../catalogue-uom-admin-crud.module';
import {
	CatalogueUOMModelState,
	initialState as initialCatalogueUOMModelState
} from 'src/app/models/catalogueUOM/catalogue_uom.model.state';
 ...
 */
describe('Catalogue UOM Crud List Tile Component', () => {

	let fixture;
	let catalogueUOMCrudListComponent: AdminCatalogueUOMCrudListComponent;

	let store: MockStore<{ model: CatalogueUOMModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let catalogueUOMModelState: CatalogueUOMModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<CatalogueUOMModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '902d70cd-41ef-485e-bdb0-cd529ea61851';
	const searchContent = 'text';

	const searchableFieldCount = CatalogueUOMModel.searchFields.length;

	const routerState: RouterState = {
		url: 'catalogue-uom-admin-crud',
		urls: ['catalogue-uom-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const catalogueUOMDataFactory = new CatalogueUOMDataFactory();

	const defaultDataSize = 30;
	const defaultData = catalogueUOMDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: CatalogueUOMModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		catalogueUOMModelState = initialCatalogueUOMModelState;
		store.setState({model: catalogueUOMModelState});
		...
		spyOn(store, 'select')
			.withArgs(getCatalogueUOMCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCatalogueUOMCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminCatalogueUOMCrudListComponent);
			catalogueUOMCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(catalogueUOMCrudListComponent.catalogueUOMsCount).toBe(defaultData.length);
			...
			expect(catalogueUOMCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			catalogueUOMCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = catalogueUOMDataFactory.createAll(filteredDataSize);
			...
			expect(catalogueUOMCrudListComponent.catalogueUOMsCount).toBe(filteredDataSize);
			expect(catalogueUOMCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(catalogueUOMCrudListComponent.pageIndex).toBe(0);
			expect(catalogueUOMCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			catalogueUOMCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		catalogueUOMCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		catalogueUOMCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
