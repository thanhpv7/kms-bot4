...
import {CatalogueUOMModelState, CatalogueUOMModelAudit} from 'src/app/models/catalogueUOM/catalogue_uom.model.state';
import {CatalogueUOMModel} from 'src/app/models/catalogueUOM/catalogue_uom.model';
import {
	getCatalogueUOMModelAuditsByEntityId,
	getCatalogueUOMModelWithId,
	getCountCatalogueUOMModels,
...
@Component({
	selector: 'cb-catalogue-uom-admin-crud-edit',
	templateUrl: './catalogue-uom-admin-crud-edit.component.html',
	styleUrls: [
		'./catalogue-uom-admin-crud-edit.component.scss',
...
export class AdminCatalogueUOMCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	catalogueUOMAudits$: Observable<CatalogueUOMModelAudit[]>;
	...
	unitOfMeasurementOppositeRelation = 'catalogueUOM';
	...
	stockCatalogueOppositeRelation = 'catalogueUOMs';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CatalogueUOMModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCatalogueUOMModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CatalogueUOMModel();
		...
		if (this.targetModelId) {
			this.catalogueUOMAudits$ = this.store.select(getCatalogueUOMModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CatalogueUOMModel> = {
		...
		this.store.dispatch(new modelAction.CatalogueUOMAction(
			modelAction.CatalogueUOMModelActionTypes.FETCH_CATALOGUE_UOM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCatalogueUOMModelWithId, this.targetModelId)
			...
			model = new CatalogueUOMModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CatalogueUOMAction(
			modelAction.CatalogueUOMModelActionTypes.FETCH_CATALOGUE_UOM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<CatalogueUOMModel> = {
				...
				queryParams: {
					persistRelations: this.unitOfMeasurementMultiCrudActive || this.stockCatalogueMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.CatalogueUOMAction(
				modelAction.CatalogueUOMModelActionTypes.CREATE_CATALOGUE_UOM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CatalogueUOMAction(
				modelAction.CatalogueUOMModelActionTypes.UPDATE_CATALOGUE_UOM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CatalogueUOMModel = new CatalogueUOMModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCatalogueUOMModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CatalogueUOMModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CatalogueUOMModel.getProps();
		this.modelRelations = CatalogueUOMModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: CatalogueUOMModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CatalogueUOMModel {
		let updatedModel: CatalogueUOMModel = new CatalogueUOMModel(model);
