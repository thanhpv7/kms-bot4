...
import {AdminPcareCompoundDrugCrudListComponent} from './pcare-compound-drug-admin-crud-list.component';
...
import {AdminPcareCompoundDrugCrudModule} from '../pcare-compound-drug-admin-crud.module';
import {
	PcareCompoundDrugModelState,
	initialState as initialPcareCompoundDrugModelState
} from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model.state';
 ...
 */
describe('PCare Compound Drug Crud List Tile Component', () => {

	let fixture;
	let pcareCompoundDrugCrudListComponent: AdminPcareCompoundDrugCrudListComponent;

	let store: MockStore<{ model: PcareCompoundDrugModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareCompoundDrugModelState: PcareCompoundDrugModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareCompoundDrugModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c8446af9-e3cb-4d73-a1cf-144f6ca3e7f2';
	const searchContent = 'text';

	const searchableFieldCount = PcareCompoundDrugModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-compound-drug-admin-crud',
		urls: ['pcare-compound-drug-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareCompoundDrugDataFactory = new PcareCompoundDrugDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareCompoundDrugDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareCompoundDrugModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareCompoundDrugModelState = initialPcareCompoundDrugModelState;
		store.setState({model: pcareCompoundDrugModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareCompoundDrugCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareCompoundDrugCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPcareCompoundDrugCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPcareCompoundDrugCrudListComponent);
			pcareCompoundDrugCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareCompoundDrugCrudListComponent.pcareCompoundDrugsCount).toBe(defaultData.length);
			...
			expect(pcareCompoundDrugCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareCompoundDrugCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareCompoundDrugDataFactory.createAll(filteredDataSize);
			...
			expect(pcareCompoundDrugCrudListComponent.pcareCompoundDrugsCount).toBe(filteredDataSize);
			expect(pcareCompoundDrugCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareCompoundDrugCrudListComponent.pageIndex).toBe(0);
			expect(pcareCompoundDrugCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareCompoundDrugCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareCompoundDrugCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareCompoundDrugCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
