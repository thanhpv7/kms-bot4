...
import {PcareCompoundDrugModelState} from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model.state';
...
import {
	getPcareCompoundDrugCollectionModels,
	getPcareCompoundDrugCollectionCount
...
@Component({
	selector: 'cb-pcare-compound-drug-admin-crud-list',
	templateUrl: './pcare-compound-drug-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-compound-drug-admin-crud-list.component.scss',
...
export class AdminPcareCompoundDrugCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareCompoundDrugModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareCompoundDrugModel.searchFields,
	...
	/**
	 * List of PCare Compound Drugs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareCompoundDrugs: Observable<PcareCompoundDrugModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareCompoundDrugsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareCompoundDrugsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareCompoundDrugsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareCompoundDrugAction(
				modelAction.PcareCompoundDrugModelActionTypes.INITIALISE_PCARE_COMPOUND_DRUG_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareCompoundDrugAction(
				modelAction.PcareCompoundDrugModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.pcareCompoundDrugs = this.store.select(getPcareCompoundDrugCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareCompoundDrugCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareCompoundDrugsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareCompoundDrugModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareCompoundDrugModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareCompoundDrugModelActionTypes.EXPORT_ALL_PCARE_COMPOUND_DRUG;
		} else {
			actionType = modelAction.PcareCompoundDrugModelActionTypes.EXPORT_PCARE_COMPOUND_DRUG_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareCompoundDrugAction(
				modelAction.PcareCompoundDrugModelActionTypes.IMPORT_PCARE_COMPOUND_DRUGS,
				{
					file: file,
				},
				[new modelAction.PcareCompoundDrugAction(
			...
			window.open(environment.API_URL + '/api/pcarecompounddrug/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareCompoundDrugModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Compound Drug entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Compound Drug entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareCompoundDrugModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareCompoundDrugModelActionTypes.DELETE_PCARE_COMPOUND_DRUG;
						} else if (usingSelected) {
							actionType = modelAction.PcareCompoundDrugModelActionTypes.DELETE_ALL_PCARE_COMPOUND_DRUG;
						} else {
							actionType = modelAction.PcareCompoundDrugModelActionTypes.DELETE_PCARE_COMPOUND_DRUG_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareCompoundDrugAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_COMPOUND_DRUG when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_COMPOUND_DRUG and DELETE_PCARE_COMPOUND_DRUG_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareCompoundDrugModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareCompoundDrugsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
