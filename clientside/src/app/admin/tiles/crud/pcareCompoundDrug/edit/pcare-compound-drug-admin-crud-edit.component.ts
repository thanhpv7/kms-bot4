...
import * as modelAction from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model.action';
import {PcareCompoundDrugModelState, PcareCompoundDrugModelAudit} from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model.state';
import {PcareCompoundDrugModel} from 'src/app/models/pcareCompoundDrug/pcare_compound_drug.model';
import {
	getPcareCompoundDrugModelAuditsByEntityId,
	getPcareCompoundDrugModelWithId,
	getCountPcareCompoundDrugModels,
...
import * as pcareCompoundDrugItemModelAction from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model.action';
import {
	getPcareCompoundDrugItemCollectionModels, getPcareCompoundDrugItemModelWithId,
} from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model.selector';
...
import {CrudTileMode} from '../pcare-compound-drug-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-compound-drug-admin-crud-edit',
	templateUrl: './pcare-compound-drug-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-compound-drug-admin-crud-edit.component.scss',
...
export class AdminPcareCompoundDrugCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'c8446af9-e3cb-4d73-a1cf-144f6ca3e7f2';
	 ...
	 */
	pcareCompoundDrugAudits$: Observable<PcareCompoundDrugModelAudit[]>;
	...
	pcareExaminationOppositeRelation = 'pcareCompoundDrugs';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Compound Drug and PCare Compound Drug Items here] off begin
	pcareCompoundDrugItemsMultiCrudActive: boolean = false;
	pcareCompoundDrugItemsDisplayName = 'PCare Compound Drug Items';
	pcareCompoundDrugItemsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between PCare Compound Drug and PCare Compound Drug Items here] end
	pcareCompoundDrugItemsOppositeRelation = 'pcareCompoundDrug';
	pcareCompoundDrugItemsRelations: ModelRelation[] = this.pcareCompoundDrugItemsMultiCrudActive ? PcareCompoundDrugItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.pcareCompoundDrugItemsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = PcareCompoundDrugModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareCompoundDrugModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareCompoundDrugModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'pcareCompoundDrugItems' && this.pcareCompoundDrugItemsMultiCrudActive) {
					exp.fields = PcareCompoundDrugItemModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: PcareCompoundDrugModel;
	...
	constructor(
		private readonly store: Store<{ model: PcareCompoundDrugModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareCompoundDrugModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareCompoundDrugModel();
		...
		if (this.targetModelId) {
			this.pcareCompoundDrugAudits$ = this.store.select(getPcareCompoundDrugModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareCompoundDrugModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PcareCompoundDrugAction(
			modelAction.PcareCompoundDrugModelActionTypes.FETCH_PCARE_COMPOUND_DRUG,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareCompoundDrugModelWithId, this.targetModelId)
			...
			model = new PcareCompoundDrugModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-compound-drug');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareCompoundDrugAction(
			modelAction.PcareCompoundDrugModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PcareCompoundDrugModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.pcareExaminationMultiCrudActive || this.pcareCompoundDrugItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PcareCompoundDrugAction(
						modelAction.PcareCompoundDrugModelActionTypes.COUNT_PCARE_COMPOUND_DRUGS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PcareCompoundDrugAction(
				modelAction.PcareCompoundDrugModelActionTypes.CREATE_PCARE_COMPOUND_DRUG,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareCompoundDrugAction(
				modelAction.PcareCompoundDrugModelActionTypes.UPDATE_PCARE_COMPOUND_DRUG,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareCompoundDrugModel = new PcareCompoundDrugModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareCompoundDrugModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareCompoundDrugModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.pcareCompoundDrugItemsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PcareCompoundDrugItemModel.getProps(), this.pcareCompoundDrugItemsRelations, this.isViewOnly),
					new PcareCompoundDrugItemModel(),
					PcareCompoundDrugItemModel,
					this.pcareCompoundDrugItemsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.pcareCompoundDrugItemsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareCompoundDrugModel.getProps();
		this.modelRelations = PcareCompoundDrugModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'pcareCompoundDrugItems';
		}).hideElement = this.pcareCompoundDrugItemsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'pcareCompoundDrugItems').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.pcareCompoundDrugItemsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.pcareCompoundDrugItemsDisplayName, many: true});
			this.store.dispatch(new pcareCompoundDrugItemModelAction.PcareCompoundDrugItemAction(
				pcareCompoundDrugItemModelAction.PcareCompoundDrugItemModelActionTypes.INITIALISE_PCARE_COMPOUND_DRUG_ITEM_COLLECTION_STATE,
				{
					collectionId: this.pcareCompoundDrugItemsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getPcareCompoundDrugItemCollectionModels, this.pcareCompoundDrugItemsCollectionId).subscribe(models => {
				models.forEach(pcareCompoundDrugItems => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(PcareCompoundDrugItemModel.getProps(), this.pcareCompoundDrugItemsRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.pcareCompoundDrugItemsIds && this.targetModel.pcareCompoundDrugItemsIds.length > 0 && this.pcareCompoundDrugItemsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PcareCompoundDrugItemModel,
				this.pcareCompoundDrugItemsOppositeRelation,
				this.pcareCompoundDrugItemsCollectionId,
				this.targetModel.pcareCompoundDrugItemsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareCompoundDrugModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareCompoundDrugModel {
		let updatedModel: PcareCompoundDrugModel = new PcareCompoundDrugModel(model);
		...
		updatedModel.pcareCompoundDrugItems = [];
		if (this.pcareCompoundDrugItemsMultiCrudActive) {
			if (event.additionalEntities.pcareCompoundDrugItems) {
				event.additionalEntities.pcareCompoundDrugItems.forEach(entity => {
					updatedModel.pcareCompoundDrugItems.push(entity);
				});
			}

			if (!updatedModel.pcareCompoundDrugItems.length) {
				updatedModel.pcareCompoundDrugItemsIds = [];
			}
		}
		return updatedModel;
	}
