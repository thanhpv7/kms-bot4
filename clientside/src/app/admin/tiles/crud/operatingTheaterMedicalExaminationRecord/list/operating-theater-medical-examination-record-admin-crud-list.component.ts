...
import {OperatingTheaterMedicalExaminationRecordModelState} from 'src/app/models/operatingTheaterMedicalExaminationRecord/operating_theater_medical_examination_record.model.state';
...
import {
	...
	getOperatingTheaterMedicalExaminationRecordCollectionCount
...
@Component({
	selector: 'cb-operating-theater-medical-examination-record-admin-crud-list',
	templateUrl: './operating-theater-medical-examination-record-admin-crud-list.component.html',
	styleUrls: [
		'./operating-theater-medical-examination-record-admin-crud-list.component.scss',
...
export class AdminOperatingTheaterMedicalExaminationRecordCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<OperatingTheaterMedicalExaminationRecordModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...OperatingTheaterMedicalExaminationRecordModel.searchFields,
	...
	/**
	 * List of Operating Theater Medical Examination Records that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	operatingTheaterMedicalExaminationRecords: Observable<OperatingTheaterMedicalExaminationRecordModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	operatingTheaterMedicalExaminationRecordsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.operatingTheaterMedicalExaminationRecordsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.operatingTheaterMedicalExaminationRecordsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.OperatingTheaterMedicalExaminationRecordAction(
				modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.INITIALISE_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.OperatingTheaterMedicalExaminationRecordAction(
				modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.FETCH_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.operatingTheaterMedicalExaminationRecords = this.store.select(getOperatingTheaterMedicalExaminationRecordCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getOperatingTheaterMedicalExaminationRecordCollectionCount, this.collectionId).subscribe(
				(count) => this.operatingTheaterMedicalExaminationRecordsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: OperatingTheaterMedicalExaminationRecordModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.EXPORT_ALL_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD;
		} else {
			actionType = modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.EXPORT_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.OperatingTheaterMedicalExaminationRecordAction(
				modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.IMPORT_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORDS,
				{
					file: file,
				},
				[new modelAction.OperatingTheaterMedicalExaminationRecordAction(
			...
			window.open(environment.API_URL + '/api/operatingtheatermedicalexaminationrecord/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: OperatingTheaterMedicalExaminationRecordModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Operating Theater Medical Examination Record entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Operating Theater Medical Examination Record entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.DELETE_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD;
						} else if (usingSelected) {
							actionType = modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.DELETE_ALL_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD;
						} else {
							actionType = modelAction.OperatingTheaterMedicalExaminationRecordModelActionTypes.DELETE_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.OperatingTheaterMedicalExaminationRecordAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD and DELETE_OPERATING_THEATER_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = OperatingTheaterMedicalExaminationRecordModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.operatingTheaterMedicalExaminationRecordsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
