...
import {AdminOperatingTheaterMedicalExaminationRecordCrudListComponent} from './operating-theater-medical-examination-record-admin-crud-list.component';
...
import {AdminOperatingTheaterMedicalExaminationRecordCrudModule} from '../operating-theater-medical-examination-record-admin-crud.module';
import {
 ...
 */
describe('Operating Theater Medical Examination Record Crud List Tile Component', () => {

	let fixture;
	let operatingTheaterMedicalExaminationRecordCrudListComponent: AdminOperatingTheaterMedicalExaminationRecordCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<OperatingTheaterMedicalExaminationRecordModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '05bd1a26-386e-4371-9032-57207299a4c1';
	const searchContent = 'text';

	const searchableFieldCount = OperatingTheaterMedicalExaminationRecordModel.searchFields.length;

	const routerState: RouterState = {
		url: 'operating-theater-medical-examination-record-admin-crud',
		urls: ['operating-theater-medical-examination-record-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const operatingTheaterMedicalExaminationRecordDataFactory = new OperatingTheaterMedicalExaminationRecordDataFactory();

	const defaultDataSize = 30;
	const defaultData = operatingTheaterMedicalExaminationRecordDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getOperatingTheaterMedicalExaminationRecordCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getOperatingTheaterMedicalExaminationRecordCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminOperatingTheaterMedicalExaminationRecordCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminOperatingTheaterMedicalExaminationRecordCrudListComponent);
			operatingTheaterMedicalExaminationRecordCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(operatingTheaterMedicalExaminationRecordCrudListComponent.operatingTheaterMedicalExaminationRecordsCount).toBe(defaultData.length);
			...
			expect(operatingTheaterMedicalExaminationRecordCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			operatingTheaterMedicalExaminationRecordCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = operatingTheaterMedicalExaminationRecordDataFactory.createAll(filteredDataSize);
			...
			expect(operatingTheaterMedicalExaminationRecordCrudListComponent.operatingTheaterMedicalExaminationRecordsCount).toBe(filteredDataSize);
			expect(operatingTheaterMedicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(operatingTheaterMedicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
			expect(operatingTheaterMedicalExaminationRecordCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			operatingTheaterMedicalExaminationRecordCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		operatingTheaterMedicalExaminationRecordCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		operatingTheaterMedicalExaminationRecordCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
