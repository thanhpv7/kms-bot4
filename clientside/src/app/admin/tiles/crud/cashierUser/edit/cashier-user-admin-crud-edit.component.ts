...
import {CashierUserModelState, CashierUserModelAudit} from 'src/app/models/cashierUser/cashier_user.model.state';
import {CashierUserModel} from 'src/app/models/cashierUser/cashier_user.model';
import {
	getCashierUserModelAuditsByEntityId,
	getCashierUserModelWithId,
	getCountCashierUserModels,
...
@Component({
	selector: 'cb-cashier-user-admin-crud-edit',
	templateUrl: './cashier-user-admin-crud-edit.component.html',
	styleUrls: [
		'./cashier-user-admin-crud-edit.component.scss',
...
export class AdminCashierUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	cashierUserAudits$: Observable<CashierUserModelAudit[]>;
	...
	staffOppositeRelation = 'cashierUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CashierUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCashierUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CashierUserModel();
		...
		if (this.targetModelId) {
			this.cashierUserAudits$ = this.store.select(getCashierUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CashierUserModel> = {
		...
		this.store.dispatch(new modelAction.CashierUserAction(
			modelAction.CashierUserModelActionTypes.FETCH_CASHIER_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCashierUserModelWithId, this.targetModelId)
			...
			model = new CashierUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CashierUserAction(
			modelAction.CashierUserModelActionTypes.FETCH_CASHIER_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.CashierUserAction(
				modelAction.CashierUserModelActionTypes.CREATE_CASHIER_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CashierUserAction(
				modelAction.CashierUserModelActionTypes.UPDATE_CASHIER_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CashierUserModel = new CashierUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCashierUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CashierUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CashierUserModel.getProps();
		this.modelRelations = CashierUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: CashierUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CashierUserModel {
		let updatedModel: CashierUserModel = new CashierUserModel(model);
