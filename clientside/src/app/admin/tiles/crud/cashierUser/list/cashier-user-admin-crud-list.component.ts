...
import * as modelAction from 'src/app/models/cashierUser/cashier_user.model.action';
import {CashierUserModelState} from 'src/app/models/cashierUser/cashier_user.model.state';
...
import {
	getCashierUserCollectionModels,
	getCashierUserCollectionCount
...
import {CrudTileMode} from '../cashier-user-admin-crud.component';
...
@Component({
	selector: 'cb-cashier-user-admin-crud-list',
	templateUrl: './cashier-user-admin-crud-list.component.html',
	styleUrls: [
		'./cashier-user-admin-crud-list.component.scss',
...
export class AdminCashierUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CashierUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CashierUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = CashierUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'bf5cee10-633b-49d5-b708-781a53f6f0b6';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.cashierUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.cashierUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: CashierUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashierUserAction(
				modelAction.CashierUserModelActionTypes.INITIALISE_CASHIER_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CashierUserAction(
				modelAction.CashierUserModelActionTypes.FETCH_CASHIER_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.CashierUserAction(
						modelAction.CashierUserModelActionTypes.COUNT_CASHIER_USERS,
						null
					)
				]
			))
		...
		this.cashierUsers = this.store.select(getCashierUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCashierUserCollectionCount, this.collectionId).subscribe(
				(count) => this.cashierUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CashierUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CashierUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CashierUserModelActionTypes.EXPORT_ALL_CASHIER_USER;
		} else {
			actionType = modelAction.CashierUserModelActionTypes.EXPORT_CASHIER_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CashierUserAction(
				modelAction.CashierUserModelActionTypes.IMPORT_CASHIER_USERS,
				{
					file: file,
				},
				[new modelAction.CashierUserAction(
			...
			window.open(environment.API_URL + '/api/cashieruser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CashierUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Cashier User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Cashier User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CashierUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CashierUserModelActionTypes.DELETE_CASHIER_USER;
						} else if (usingSelected) {
							actionType = modelAction.CashierUserModelActionTypes.DELETE_ALL_CASHIER_USER;
						} else {
							actionType = modelAction.CashierUserModelActionTypes.DELETE_CASHIER_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CashierUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CASHIER_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CASHIER_USER and DELETE_CASHIER_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<CashierUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CashierUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.cashierUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
