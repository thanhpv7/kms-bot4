...
import {AdminCashierUserCrudListComponent} from './cashier-user-admin-crud-list.component';
...
import {AdminCashierUserCrudModule} from '../cashier-user-admin-crud.module';
import {
	CashierUserModelState,
	initialState as initialCashierUserModelState
} from 'src/app/models/cashierUser/cashier_user.model.state';
 ...
 */
describe('Cashier User Crud List Tile Component', () => {

	let fixture;
	let cashierUserCrudListComponent: AdminCashierUserCrudListComponent;

	let store: MockStore<{ model: CashierUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let cashierUserModelState: CashierUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<CashierUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'bf5cee10-633b-49d5-b708-781a53f6f0b6';
	const searchContent = 'text';

	const searchableFieldCount = CashierUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'cashier-user-admin-crud',
		urls: ['cashier-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const cashierUserDataFactory = new CashierUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = cashierUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: CashierUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		cashierUserModelState = initialCashierUserModelState;
		store.setState({model: cashierUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getCashierUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCashierUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminCashierUserCrudListComponent);
			cashierUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(cashierUserCrudListComponent.cashierUsersCount).toBe(defaultData.length);
			...
			expect(cashierUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			cashierUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = cashierUserDataFactory.createAll(filteredDataSize);
			...
			expect(cashierUserCrudListComponent.cashierUsersCount).toBe(filteredDataSize);
			expect(cashierUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(cashierUserCrudListComponent.pageIndex).toBe(0);
			expect(cashierUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			cashierUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		cashierUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		cashierUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
