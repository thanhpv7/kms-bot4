...
import {AdminNewbornDetailCrudListComponent} from './newborn-detail-admin-crud-list.component';
...
import {AdminNewbornDetailCrudModule} from '../newborn-detail-admin-crud.module';
import {
 ...
 */
describe('Newborn Detail Crud List Tile Component', () => {

	let fixture;
	let newbornDetailCrudListComponent: AdminNewbornDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<NewbornDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '34a78422-e6b5-47db-b0c8-d84376cc4772';
	const searchContent = 'text';

	const searchableFieldCount = NewbornDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'newborn-detail-admin-crud',
		urls: ['newborn-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const newbornDetailDataFactory = new NewbornDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = newbornDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getNewbornDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNewbornDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminNewbornDetailCrudListComponent);
			newbornDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(newbornDetailCrudListComponent.newbornDetailsCount).toBe(defaultData.length);
			...
			expect(newbornDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			newbornDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = newbornDetailDataFactory.createAll(filteredDataSize);
			...
			expect(newbornDetailCrudListComponent.newbornDetailsCount).toBe(filteredDataSize);
			expect(newbornDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(newbornDetailCrudListComponent.pageIndex).toBe(0);
			expect(newbornDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			newbornDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		newbornDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		newbornDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
