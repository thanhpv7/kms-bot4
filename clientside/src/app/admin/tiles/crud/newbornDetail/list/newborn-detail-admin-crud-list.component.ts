...
import {NewbornDetailModelState} from 'src/app/models/newbornDetail/newborn_detail.model.state';
...
import {
	...
	getNewbornDetailCollectionCount
...
@Component({
	selector: 'cb-newborn-detail-admin-crud-list',
	templateUrl: './newborn-detail-admin-crud-list.component.html',
	styleUrls: [
		'./newborn-detail-admin-crud-list.component.scss',
...
export class AdminNewbornDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NewbornDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NewbornDetailModel.searchFields,
	...
	/**
	 * List of Newborn Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	newbornDetails: Observable<NewbornDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	newbornDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.newbornDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.newbornDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NewbornDetailAction(
				modelAction.NewbornDetailModelActionTypes.INITIALISE_NEWBORN_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NewbornDetailAction(
				modelAction.NewbornDetailModelActionTypes.FETCH_NEWBORN_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.newbornDetails = this.store.select(getNewbornDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNewbornDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.newbornDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NewbornDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NewbornDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NewbornDetailModelActionTypes.EXPORT_ALL_NEWBORN_DETAIL;
		} else {
			actionType = modelAction.NewbornDetailModelActionTypes.EXPORT_NEWBORN_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NewbornDetailAction(
				modelAction.NewbornDetailModelActionTypes.IMPORT_NEWBORN_DETAILS,
				{
					file: file,
				},
				[new modelAction.NewbornDetailAction(
			...
			window.open(environment.API_URL + '/api/newborndetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NewbornDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Newborn Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Newborn Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NewbornDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NewbornDetailModelActionTypes.DELETE_NEWBORN_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.NewbornDetailModelActionTypes.DELETE_ALL_NEWBORN_DETAIL;
						} else {
							actionType = modelAction.NewbornDetailModelActionTypes.DELETE_NEWBORN_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NewbornDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NEWBORN_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NEWBORN_DETAIL and DELETE_NEWBORN_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NewbornDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.newbornDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
