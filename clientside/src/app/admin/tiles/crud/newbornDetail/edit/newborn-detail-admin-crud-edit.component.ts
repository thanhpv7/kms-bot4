...
import * as modelAction from 'src/app/models/newbornDetail/newborn_detail.model.action';
import {NewbornDetailModelState, NewbornDetailModelAudit} from 'src/app/models/newbornDetail/newborn_detail.model.state';
import {NewbornDetailModel} from 'src/app/models/newbornDetail/newborn_detail.model';
import {
	getNewbornDetailModelAuditsByEntityId,
	getNewbornDetailModelWithId,
	getCountNewbornDetailModels,
...
import * as vitalSignsModelAction from 'src/app/models/vitalSigns/vital_signs.model.action';
import {
	getVitalSignsCollectionModels, getVitalSignsModelWithId,
...
import {CrudTileMode} from '../newborn-detail-admin-crud.component';
...
@Component({
	selector: 'cb-newborn-detail-admin-crud-edit',
	templateUrl: './newborn-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./newborn-detail-admin-crud-edit.component.scss',
...
export class AdminNewbornDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '34a78422-e6b5-47db-b0c8-d84376cc4772';
	 ...
	 */
	newbornDetailAudits$: Observable<NewbornDetailModelAudit[]>;
	...
	deliveryMedicalExaminationRecordOppositeRelation = 'newbornDetails';
	...
	staffOppositeRelation = 'pediatricianInCharge';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Newborn Detail and Vital Signs here] off begin
	vitalSignsMultiCrudActive: boolean = false;
	vitalSignsDisplayName = 'Vital Signs';
	vitalSignsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Newborn Detail and Vital Signs here] end
	vitalSignsOppositeRelation = 'newbornDetail';
	vitalSignsRelations: ModelRelation[] = this.vitalSignsMultiCrudActive ? VitalSignsModel.getRelations()
		.map(relation => {
			if (relation.id === this.vitalSignsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = NewbornDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NewbornDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NewbornDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'vitalSigns' && this.vitalSignsMultiCrudActive) {
					exp.fields = VitalSignsModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: NewbornDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: NewbornDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNewbornDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NewbornDetailModel();
		...
		if (this.targetModelId) {
			this.newbornDetailAudits$ = this.store.select(getNewbornDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NewbornDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.NewbornDetailAction(
			modelAction.NewbornDetailModelActionTypes.FETCH_NEWBORN_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNewbornDetailModelWithId, this.targetModelId)
			...
			model = new NewbornDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NewbornDetailAction(
			modelAction.NewbornDetailModelActionTypes.FETCH_NEWBORN_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<NewbornDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.deliveryMedicalExaminationRecordMultiCrudActive || this.staffMultiCrudActive || this.vitalSignsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.NewbornDetailAction(
						modelAction.NewbornDetailModelActionTypes.COUNT_NEWBORN_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.NewbornDetailAction(
				modelAction.NewbornDetailModelActionTypes.CREATE_NEWBORN_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NewbornDetailAction(
				modelAction.NewbornDetailModelActionTypes.UPDATE_NEWBORN_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NewbornDetailModel = new NewbornDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNewbornDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NewbornDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.vitalSignsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(VitalSignsModel.getProps(), this.vitalSignsRelations, this.isViewOnly),
					new VitalSignsModel(),
					VitalSignsModel,
					this.vitalSignsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.vitalSignsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NewbornDetailModel.getProps();
		this.modelRelations = NewbornDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'vitalSigns';
		}).hideElement = this.vitalSignsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'vitalSigns').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.vitalSignsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.vitalSignsDisplayName, many: true});
			this.store.dispatch(new vitalSignsModelAction.VitalSignsAction(
				vitalSignsModelAction.VitalSignsModelActionTypes.INITIALISE_VITAL_SIGNS_COLLECTION_STATE,
				{
					collectionId: this.vitalSignsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getVitalSignsCollectionModels, this.vitalSignsCollectionId).subscribe(models => {
				models.forEach(vitalSigns => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(VitalSignsModel.getProps(), this.vitalSignsRelations, this.isViewOnly),
						vitalSigns,
						VitalSignsModel,
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.vitalSignsIds && this.targetModel.vitalSignsIds.length > 0 && this.vitalSignsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				VitalSignsModel,
				this.vitalSignsOppositeRelation,
				this.vitalSignsCollectionId,
				this.targetModel.vitalSignsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: NewbornDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NewbornDetailModel {
		let updatedModel: NewbornDetailModel = new NewbornDetailModel(model);
		...
		updatedModel.vitalSigns = [];
		if (this.vitalSignsMultiCrudActive) {
			if (event.additionalEntities.vitalSigns) {
				event.additionalEntities.vitalSigns.forEach(entity => {
					updatedModel.vitalSigns.push(entity);
				});
			}

			if (!updatedModel.vitalSigns.length) {
				updatedModel.vitalSignsIds = [];
			}
		}
		return updatedModel;
	}
