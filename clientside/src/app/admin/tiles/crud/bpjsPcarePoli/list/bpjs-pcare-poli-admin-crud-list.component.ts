...
import * as modelAction from 'src/app/models/bpjsPcarePoli/bpjs_pcare_poli.model.action';
import {BpjsPcarePoliModelState} from 'src/app/models/bpjsPcarePoli/bpjs_pcare_poli.model.state';
...
import {
	getBpjsPcarePoliCollectionModels,
	getBpjsPcarePoliCollectionCount
...
import {CrudTileMode} from '../bpjs-pcare-poli-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-poli-admin-crud-list',
	templateUrl: './bpjs-pcare-poli-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-poli-admin-crud-list.component.scss',
...
export class AdminBpjsPcarePoliCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcarePoliModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcarePoliModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcarePoliModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '72b82cf1-3bc2-47f8-a5a2-0f671c6789a3';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcarePolisCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcarePolisCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcarePoliModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcarePoliAction(
				modelAction.BpjsPcarePoliModelActionTypes.INITIALISE_BPJS_PCARE_POLI_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcarePoliAction(
				modelAction.BpjsPcarePoliModelActionTypes.FETCH_BPJS_PCARE_POLI_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcarePoliAction(
						modelAction.BpjsPcarePoliModelActionTypes.COUNT_BPJS_PCARE_POLIS,
						null
					)
				]
			))
		...
		this.bpjsPcarePolis = this.store.select(getBpjsPcarePoliCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcarePoliCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcarePolisCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcarePoliModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcarePoliModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcarePoliModelActionTypes.EXPORT_ALL_BPJS_PCARE_POLI;
		} else {
			actionType = modelAction.BpjsPcarePoliModelActionTypes.EXPORT_BPJS_PCARE_POLI_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcarePoliAction(
				modelAction.BpjsPcarePoliModelActionTypes.IMPORT_BPJS_PCARE_POLIS,
				{
					file: file,
				},
				[new modelAction.BpjsPcarePoliAction(
			...
			window.open(environment.API_URL + '/api/bpjspcarepoli/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcarePoliModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Poli entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Pcare Poli entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcarePoliModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcarePoliModelActionTypes.DELETE_BPJS_PCARE_POLI;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcarePoliModelActionTypes.DELETE_ALL_BPJS_PCARE_POLI;
						} else {
							actionType = modelAction.BpjsPcarePoliModelActionTypes.DELETE_BPJS_PCARE_POLI_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcarePoliAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_POLI when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_POLI and DELETE_BPJS_PCARE_POLI_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcarePoliModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcarePoliModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcarePolisCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
