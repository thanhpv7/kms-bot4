...
import {AdminBpjsPcarePoliCrudListComponent} from './bpjs-pcare-poli-admin-crud-list.component';
...
import {AdminBpjsPcarePoliCrudModule} from '../bpjs-pcare-poli-admin-crud.module';
import {
	BpjsPcarePoliModelState,
	initialState as initialBpjsPcarePoliModelState
} from 'src/app/models/bpjsPcarePoli/bpjs_pcare_poli.model.state';
 ...
 */
describe('BPJS Pcare Poli Crud List Tile Component', () => {

	let fixture;
	let bpjsPcarePoliCrudListComponent: AdminBpjsPcarePoliCrudListComponent;

	let store: MockStore<{ model: BpjsPcarePoliModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsPcarePoliModelState: BpjsPcarePoliModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPcarePoliModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '72b82cf1-3bc2-47f8-a5a2-0f671c6789a3';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPcarePoliModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-pcare-poli-admin-crud',
		urls: ['bpjs-pcare-poli-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPcarePoliDataFactory = new BpjsPcarePoliDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPcarePoliDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsPcarePoliModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsPcarePoliModelState = initialBpjsPcarePoliModelState;
		store.setState({model: bpjsPcarePoliModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPcarePoliCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPcarePoliCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsPcarePoliCrudListComponent);
			bpjsPcarePoliCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPcarePoliCrudListComponent.bpjsPcarePolisCount).toBe(defaultData.length);
			...
			expect(bpjsPcarePoliCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPcarePoliCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPcarePoliDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPcarePoliCrudListComponent.bpjsPcarePolisCount).toBe(filteredDataSize);
			expect(bpjsPcarePoliCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPcarePoliCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPcarePoliCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPcarePoliCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPcarePoliCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPcarePoliCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
