...
import {BpjsPcarePoliModelState, BpjsPcarePoliModelAudit} from 'src/app/models/bpjsPcarePoli/bpjs_pcare_poli.model.state';
import {BpjsPcarePoliModel} from 'src/app/models/bpjsPcarePoli/bpjs_pcare_poli.model';
import {
	getBpjsPcarePoliModelAuditsByEntityId,
	getBpjsPcarePoliModelWithId,
	getCountBpjsPcarePoliModels,
...
@Component({
	selector: 'cb-bpjs-pcare-poli-admin-crud-edit',
	templateUrl: './bpjs-pcare-poli-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-poli-admin-crud-edit.component.scss',
...
export class AdminBpjsPcarePoliCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcarePoliAudits$: Observable<BpjsPcarePoliModelAudit[]>;
	...
	serviceOppositeRelation = 'bpjsPcarePoli';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcarePoliModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcarePoliModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcarePoliModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcarePoliModel();
		...
		if (this.targetModelId) {
			this.bpjsPcarePoliAudits$ = this.store.select(getBpjsPcarePoliModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcarePoliModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcarePoliAction(
			modelAction.BpjsPcarePoliModelActionTypes.FETCH_BPJS_PCARE_POLI,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcarePoliModelWithId, this.targetModelId)
			...
			model = new BpjsPcarePoliModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcarePoliAction(
			modelAction.BpjsPcarePoliModelActionTypes.FETCH_BPJS_PCARE_POLI_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcarePoliAction(
				modelAction.BpjsPcarePoliModelActionTypes.CREATE_BPJS_PCARE_POLI,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcarePoliAction(
				modelAction.BpjsPcarePoliModelActionTypes.UPDATE_BPJS_PCARE_POLI,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcarePoliModel = new BpjsPcarePoliModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcarePoliModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcarePoliModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcarePoliModel.getProps();
		this.modelRelations = BpjsPcarePoliModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcarePoliModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcarePoliModel {
		let updatedModel: BpjsPcarePoliModel = new BpjsPcarePoliModel(model);
