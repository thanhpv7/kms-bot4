...
import {InvoicePaymentSelfPayingModelState} from 'src/app/models/invoicePaymentSelfPaying/invoice_payment_self_paying.model.state';
...
import {
	getInvoicePaymentSelfPayingCollectionModels,
	getInvoicePaymentSelfPayingCollectionCount
...
@Component({
	selector: 'cb-invoice-payment-self-paying-admin-crud-list',
	templateUrl: './invoice-payment-self-paying-admin-crud-list.component.html',
	styleUrls: [
		'./invoice-payment-self-paying-admin-crud-list.component.scss',
...
export class AdminInvoicePaymentSelfPayingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InvoicePaymentSelfPayingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InvoicePaymentSelfPayingModel.searchFields,
	...
	/**
	 * List of Invoice Payment Self Payings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	invoicePaymentSelfPayings: Observable<InvoicePaymentSelfPayingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	invoicePaymentSelfPayingsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.invoicePaymentSelfPayingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.invoicePaymentSelfPayingsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
				modelAction.InvoicePaymentSelfPayingModelActionTypes.INITIALISE_INVOICE_PAYMENT_SELF_PAYING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
				modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_INVOICE_PAYMENT_SELF_PAYING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.invoicePaymentSelfPayings = this.store.select(getInvoicePaymentSelfPayingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInvoicePaymentSelfPayingCollectionCount, this.collectionId).subscribe(
				(count) => this.invoicePaymentSelfPayingsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InvoicePaymentSelfPayingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InvoicePaymentSelfPayingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InvoicePaymentSelfPayingModelActionTypes.EXPORT_ALL_INVOICE_PAYMENT_SELF_PAYING;
		} else {
			actionType = modelAction.InvoicePaymentSelfPayingModelActionTypes.EXPORT_INVOICE_PAYMENT_SELF_PAYING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
				modelAction.InvoicePaymentSelfPayingModelActionTypes.IMPORT_INVOICE_PAYMENT_SELF_PAYINGS,
				{
					file: file,
				},
				[new modelAction.InvoicePaymentSelfPayingAction(
			...
			window.open(environment.API_URL + '/api/invoicepaymentselfpaying/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InvoicePaymentSelfPayingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Invoice Payment Self Paying entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Invoice Payment Self Paying entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InvoicePaymentSelfPayingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InvoicePaymentSelfPayingModelActionTypes.DELETE_INVOICE_PAYMENT_SELF_PAYING;
						} else if (usingSelected) {
							actionType = modelAction.InvoicePaymentSelfPayingModelActionTypes.DELETE_ALL_INVOICE_PAYMENT_SELF_PAYING;
						} else {
							actionType = modelAction.InvoicePaymentSelfPayingModelActionTypes.DELETE_INVOICE_PAYMENT_SELF_PAYING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVOICE_PAYMENT_SELF_PAYING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVOICE_PAYMENT_SELF_PAYING and DELETE_INVOICE_PAYMENT_SELF_PAYING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InvoicePaymentSelfPayingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.invoicePaymentSelfPayingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
