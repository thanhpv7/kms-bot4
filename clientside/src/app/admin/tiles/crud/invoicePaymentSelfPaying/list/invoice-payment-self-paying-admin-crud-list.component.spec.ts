...
import {AdminInvoicePaymentSelfPayingCrudListComponent} from './invoice-payment-self-paying-admin-crud-list.component';
...
import {AdminInvoicePaymentSelfPayingCrudModule} from '../invoice-payment-self-paying-admin-crud.module';
import {
	InvoicePaymentSelfPayingModelState,
	initialState as initialInvoicePaymentSelfPayingModelState
} from 'src/app/models/invoicePaymentSelfPaying/invoice_payment_self_paying.model.state';
 ...
 */
describe('Invoice Payment Self Paying Crud List Tile Component', () => {

	let fixture;
	let invoicePaymentSelfPayingCrudListComponent: AdminInvoicePaymentSelfPayingCrudListComponent;

	let store: MockStore<{ model: InvoicePaymentSelfPayingModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let invoicePaymentSelfPayingModelState: InvoicePaymentSelfPayingModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InvoicePaymentSelfPayingModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a475afc5-d1f9-41d0-93f6-ff9290c9430c';
	const searchContent = 'text';

	const searchableFieldCount = InvoicePaymentSelfPayingModel.searchFields.length;

	const routerState: RouterState = {
		url: 'invoice-payment-self-paying-admin-crud',
		urls: ['invoice-payment-self-paying-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const invoicePaymentSelfPayingDataFactory = new InvoicePaymentSelfPayingDataFactory();

	const defaultDataSize = 30;
	const defaultData = invoicePaymentSelfPayingDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InvoicePaymentSelfPayingModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		invoicePaymentSelfPayingModelState = initialInvoicePaymentSelfPayingModelState;
		store.setState({model: invoicePaymentSelfPayingModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInvoicePaymentSelfPayingCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInvoicePaymentSelfPayingCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInvoicePaymentSelfPayingCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInvoicePaymentSelfPayingCrudListComponent);
			invoicePaymentSelfPayingCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(invoicePaymentSelfPayingCrudListComponent.invoicePaymentSelfPayingsCount).toBe(defaultData.length);
			...
			expect(invoicePaymentSelfPayingCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			invoicePaymentSelfPayingCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = invoicePaymentSelfPayingDataFactory.createAll(filteredDataSize);
			...
			expect(invoicePaymentSelfPayingCrudListComponent.invoicePaymentSelfPayingsCount).toBe(filteredDataSize);
			expect(invoicePaymentSelfPayingCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(invoicePaymentSelfPayingCrudListComponent.pageIndex).toBe(0);
			expect(invoicePaymentSelfPayingCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			invoicePaymentSelfPayingCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		invoicePaymentSelfPayingCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		invoicePaymentSelfPayingCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
