...
import * as modelAction from 'src/app/models/invoicePaymentSelfPaying/invoice_payment_self_paying.model.action';
import {InvoicePaymentSelfPayingModelState, InvoicePaymentSelfPayingModelAudit} from 'src/app/models/invoicePaymentSelfPaying/invoice_payment_self_paying.model.state';
import {InvoicePaymentSelfPayingModel} from 'src/app/models/invoicePaymentSelfPaying/invoice_payment_self_paying.model';
import {
	getInvoicePaymentSelfPayingModelAuditsByEntityId,
	getInvoicePaymentSelfPayingModelWithId,
	getCountInvoicePaymentSelfPayingModels,
...
import {CrudTileMode} from '../invoice-payment-self-paying-admin-crud.component';
...
@Component({
	selector: 'cb-invoice-payment-self-paying-admin-crud-edit',
	templateUrl: './invoice-payment-self-paying-admin-crud-edit.component.html',
	styleUrls: [
		'./invoice-payment-self-paying-admin-crud-edit.component.scss',
...
export class AdminInvoicePaymentSelfPayingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'a475afc5-d1f9-41d0-93f6-ff9290c9430c';
	 ...
	 */
	invoicePaymentSelfPayingAudits$: Observable<InvoicePaymentSelfPayingModelAudit[]>;
	...
	invoiceOppositeRelation = 'invoicePaymentSelfPaying';
	 ...
	 */
	modelProperties: ModelProperty[] = InvoicePaymentSelfPayingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InvoicePaymentSelfPayingModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InvoicePaymentSelfPayingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: InvoicePaymentSelfPayingModel;
	...
	constructor(
		private readonly store: Store<{ model: InvoicePaymentSelfPayingModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInvoicePaymentSelfPayingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InvoicePaymentSelfPayingModel();
		...
		if (this.targetModelId) {
			this.invoicePaymentSelfPayingAudits$ = this.store.select(getInvoicePaymentSelfPayingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InvoicePaymentSelfPayingModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
			modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_INVOICE_PAYMENT_SELF_PAYING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInvoicePaymentSelfPayingModelWithId, this.targetModelId)
			...
			model = new InvoicePaymentSelfPayingModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('invoice-payment-self-paying');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
			modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_INVOICE_PAYMENT_SELF_PAYING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InvoicePaymentSelfPayingModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.invoiceMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InvoicePaymentSelfPayingAction(
						modelAction.InvoicePaymentSelfPayingModelActionTypes.COUNT_INVOICE_PAYMENT_SELF_PAYINGS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
				modelAction.InvoicePaymentSelfPayingModelActionTypes.CREATE_INVOICE_PAYMENT_SELF_PAYING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InvoicePaymentSelfPayingAction(
				modelAction.InvoicePaymentSelfPayingModelActionTypes.UPDATE_INVOICE_PAYMENT_SELF_PAYING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InvoicePaymentSelfPayingModel = new InvoicePaymentSelfPayingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInvoicePaymentSelfPayingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InvoicePaymentSelfPayingModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InvoicePaymentSelfPayingModel.getProps();
		this.modelRelations = InvoicePaymentSelfPayingModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: InvoicePaymentSelfPayingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InvoicePaymentSelfPayingModel {
		let updatedModel: InvoicePaymentSelfPayingModel = new InvoicePaymentSelfPayingModel(model);
