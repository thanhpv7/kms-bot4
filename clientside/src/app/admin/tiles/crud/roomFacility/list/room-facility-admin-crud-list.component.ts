...
import {RoomFacilityModelState} from 'src/app/models/roomFacility/room_facility.model.state';
...
import {
	...
	getRoomFacilityCollectionCount
...
import {CrudTileMode} from '../room-facility-admin-crud.component';
...
@Component({
	selector: 'cb-room-facility-admin-crud-list',
	templateUrl: './room-facility-admin-crud-list.component.html',
	styleUrls: [
		'./room-facility-admin-crud-list.component.scss',
...
export class AdminRoomFacilityCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<RoomFacilityModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...RoomFacilityModel.searchFields,
	...
	/**
	 * List of Room Facilitys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	roomFacilitys: Observable<RoomFacilityModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	roomFacilitysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.roomFacilitysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.roomFacilitysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RoomFacilityAction(
				modelAction.RoomFacilityModelActionTypes.INITIALISE_ROOM_FACILITY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RoomFacilityAction(
				modelAction.RoomFacilityModelActionTypes.FETCH_ROOM_FACILITY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.roomFacilitys = this.store.select(getRoomFacilityCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getRoomFacilityCollectionCount, this.collectionId).subscribe(
				(count) => this.roomFacilitysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: RoomFacilityModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.RoomFacilityModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.RoomFacilityModelActionTypes.EXPORT_ALL_ROOM_FACILITY;
		} else {
			actionType = modelAction.RoomFacilityModelActionTypes.EXPORT_ROOM_FACILITY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.RoomFacilityAction(
				modelAction.RoomFacilityModelActionTypes.IMPORT_ROOM_FACILITYS,
				{
					file: file,
				},
				[new modelAction.RoomFacilityAction(
			...
			window.open(environment.API_URL + '/api/roomfacility/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: RoomFacilityModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Room Facility entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Room Facility entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.RoomFacilityModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.RoomFacilityModelActionTypes.DELETE_ROOM_FACILITY;
						} else if (usingSelected) {
							actionType = modelAction.RoomFacilityModelActionTypes.DELETE_ALL_ROOM_FACILITY;
						} else {
							actionType = modelAction.RoomFacilityModelActionTypes.DELETE_ROOM_FACILITY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.RoomFacilityAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ROOM_FACILITY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ROOM_FACILITY and DELETE_ROOM_FACILITY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = RoomFacilityModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.roomFacilitysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
