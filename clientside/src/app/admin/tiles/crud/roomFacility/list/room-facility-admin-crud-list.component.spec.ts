...
import {AdminRoomFacilityCrudListComponent} from './room-facility-admin-crud-list.component';
...
import {AdminRoomFacilityCrudModule} from '../room-facility-admin-crud.module';
import {
 ...
 */
describe('Room Facility Crud List Tile Component', () => {

	let fixture;
	let roomFacilityCrudListComponent: AdminRoomFacilityCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<RoomFacilityModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '9ac76951-7fe5-4f2f-845f-caf68b50fec3';
	const searchContent = 'text';

	const searchableFieldCount = RoomFacilityModel.searchFields.length;

	const routerState: RouterState = {
		url: 'room-facility-admin-crud',
		urls: ['room-facility-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const roomFacilityDataFactory = new RoomFacilityDataFactory();

	const defaultDataSize = 30;
	const defaultData = roomFacilityDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getRoomFacilityCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getRoomFacilityCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminRoomFacilityCrudListComponent);
			roomFacilityCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(roomFacilityCrudListComponent.roomFacilitysCount).toBe(defaultData.length);
			...
			expect(roomFacilityCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			roomFacilityCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = roomFacilityDataFactory.createAll(filteredDataSize);
			...
			expect(roomFacilityCrudListComponent.roomFacilitysCount).toBe(filteredDataSize);
			expect(roomFacilityCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(roomFacilityCrudListComponent.pageIndex).toBe(0);
			expect(roomFacilityCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			roomFacilityCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		roomFacilityCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		roomFacilityCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
