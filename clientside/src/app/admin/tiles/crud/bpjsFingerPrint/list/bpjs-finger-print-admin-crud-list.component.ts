...
import * as modelAction from 'src/app/models/bpjsFingerPrint/bpjs_finger_print.model.action';
import {BpjsFingerPrintModelState} from 'src/app/models/bpjsFingerPrint/bpjs_finger_print.model.state';
...
import {
	...
	getBpjsFingerPrintCollectionCount
...
import {CrudTileMode} from '../bpjs-finger-print-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-finger-print-admin-crud-list',
	templateUrl: './bpjs-finger-print-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-finger-print-admin-crud-list.component.scss',
...
export class AdminBpjsFingerPrintCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsFingerPrintModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsFingerPrintModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsFingerPrintModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '7980d56d-9410-4d5d-a71c-0e489fd5b109';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsFingerPrintsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsFingerPrintsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsFingerPrintModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsFingerPrintAction(
				modelAction.BpjsFingerPrintModelActionTypes.INITIALISE_BPJS_FINGER_PRINT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsFingerPrintAction(
				modelAction.BpjsFingerPrintModelActionTypes.FETCH_BPJS_FINGER_PRINT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsFingerPrintAction(
						modelAction.BpjsFingerPrintModelActionTypes.COUNT_BPJS_FINGER_PRINTS,
						null
					)
				]
			))
		...
		this.bpjsFingerPrints = this.store.select(getBpjsFingerPrintCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsFingerPrintCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsFingerPrintsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-finger-print');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsFingerPrintModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsFingerPrintModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsFingerPrintModelActionTypes.EXPORT_ALL_BPJS_FINGER_PRINT;
		} else {
			actionType = modelAction.BpjsFingerPrintModelActionTypes.EXPORT_BPJS_FINGER_PRINT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsFingerPrintAction(
				modelAction.BpjsFingerPrintModelActionTypes.IMPORT_BPJS_FINGER_PRINTS,
				{
					file: file,
				},
				[new modelAction.BpjsFingerPrintAction(
			...
			window.open(environment.API_URL + '/api/bpjsfingerprint/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsFingerPrintModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Finger Print entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Finger Print entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsFingerPrintModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsFingerPrintModelActionTypes.DELETE_BPJS_FINGER_PRINT;
						} else if (usingSelected) {
							actionType = modelAction.BpjsFingerPrintModelActionTypes.DELETE_ALL_BPJS_FINGER_PRINT;
						} else {
							actionType = modelAction.BpjsFingerPrintModelActionTypes.DELETE_BPJS_FINGER_PRINT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsFingerPrintAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_FINGER_PRINT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_FINGER_PRINT and DELETE_BPJS_FINGER_PRINT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsFingerPrintModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsFingerPrintModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsFingerPrintsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
