...
import {AdminBpjsFingerPrintCrudListComponent} from './bpjs-finger-print-admin-crud-list.component';
...
import {AdminBpjsFingerPrintCrudModule} from '../bpjs-finger-print-admin-crud.module';
import {
 ...
 */
describe('BPJS Finger Print Crud List Tile Component', () => {

	let fixture;
	let bpjsFingerPrintCrudListComponent: AdminBpjsFingerPrintCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsFingerPrintModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '7980d56d-9410-4d5d-a71c-0e489fd5b109';
	const searchContent = 'text';

	const searchableFieldCount = BpjsFingerPrintModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-finger-print-admin-crud',
		urls: ['bpjs-finger-print-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsFingerPrintDataFactory = new BpjsFingerPrintDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsFingerPrintDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsFingerPrintCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsFingerPrintCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsFingerPrintCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsFingerPrintCrudListComponent);
			bpjsFingerPrintCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsFingerPrintCrudListComponent.bpjsFingerPrintsCount).toBe(defaultData.length);
			...
			expect(bpjsFingerPrintCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsFingerPrintCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsFingerPrintDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsFingerPrintCrudListComponent.bpjsFingerPrintsCount).toBe(filteredDataSize);
			expect(bpjsFingerPrintCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsFingerPrintCrudListComponent.pageIndex).toBe(0);
			expect(bpjsFingerPrintCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsFingerPrintCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsFingerPrintCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsFingerPrintCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
