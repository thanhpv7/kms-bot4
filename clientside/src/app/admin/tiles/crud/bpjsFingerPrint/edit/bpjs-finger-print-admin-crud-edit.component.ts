...
import {BpjsFingerPrintModelState, BpjsFingerPrintModelAudit} from 'src/app/models/bpjsFingerPrint/bpjs_finger_print.model.state';
import {BpjsFingerPrintModel} from 'src/app/models/bpjsFingerPrint/bpjs_finger_print.model';
import {
	getBpjsFingerPrintModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-finger-print-admin-crud-edit',
	templateUrl: './bpjs-finger-print-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-finger-print-admin-crud-edit.component.scss',
...
export class AdminBpjsFingerPrintCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsFingerPrintAudits$: Observable<BpjsFingerPrintModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsFingerPrintModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsFingerPrintModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsFingerPrintModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsFingerPrintModel();
		...
		if (this.targetModelId) {
			this.bpjsFingerPrintAudits$ = this.store.select(getBpjsFingerPrintModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsFingerPrintModel> = {
		...
		this.store.dispatch(new modelAction.BpjsFingerPrintAction(
			modelAction.BpjsFingerPrintModelActionTypes.FETCH_BPJS_FINGER_PRINT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsFingerPrintModelWithId, this.targetModelId)
			...
			model = new BpjsFingerPrintModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsFingerPrintAction(
			modelAction.BpjsFingerPrintModelActionTypes.FETCH_BPJS_FINGER_PRINT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsFingerPrintAction(
				modelAction.BpjsFingerPrintModelActionTypes.CREATE_BPJS_FINGER_PRINT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsFingerPrintAction(
				modelAction.BpjsFingerPrintModelActionTypes.UPDATE_BPJS_FINGER_PRINT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsFingerPrintModel = new BpjsFingerPrintModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsFingerPrintModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsFingerPrintModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsFingerPrintModel.getProps();
		this.modelRelations = BpjsFingerPrintModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsFingerPrintModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsFingerPrintModel {
		let updatedModel: BpjsFingerPrintModel = new BpjsFingerPrintModel(model);
