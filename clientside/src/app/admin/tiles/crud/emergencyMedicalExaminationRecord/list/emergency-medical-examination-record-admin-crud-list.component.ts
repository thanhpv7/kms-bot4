...
import * as modelAction from 'src/app/models/emergencyMedicalExaminationRecord/emergency_medical_examination_record.model.action';
import {EmergencyMedicalExaminationRecordModelState} from 'src/app/models/emergencyMedicalExaminationRecord/emergency_medical_examination_record.model.state';
...
import {
	getEmergencyMedicalExaminationRecordCollectionModels,
	getEmergencyMedicalExaminationRecordCollectionCount
...
import {CrudTileMode} from '../emergency-medical-examination-record-admin-crud.component';
...
@Component({
	selector: 'cb-emergency-medical-examination-record-admin-crud-list',
	templateUrl: './emergency-medical-examination-record-admin-crud-list.component.html',
	styleUrls: [
		'./emergency-medical-examination-record-admin-crud-list.component.scss',
...
export class AdminEmergencyMedicalExaminationRecordCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<EmergencyMedicalExaminationRecordModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...EmergencyMedicalExaminationRecordModel.searchFields,
	...
	/**
	 * List of Emergency Medical Examination Records that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	emergencyMedicalExaminationRecords: Observable<EmergencyMedicalExaminationRecordModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	emergencyMedicalExaminationRecordsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = EmergencyMedicalExaminationRecordModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = EmergencyMedicalExaminationRecordModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ea118095-aa48-409f-9fd3-e507d581384d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.emergencyMedicalExaminationRecordsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.emergencyMedicalExaminationRecordsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: EmergencyMedicalExaminationRecordModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.EmergencyMedicalExaminationRecordAction(
				modelAction.EmergencyMedicalExaminationRecordModelActionTypes.INITIALISE_EMERGENCY_MEDICAL_EXAMINATION_RECORD_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.EmergencyMedicalExaminationRecordAction(
				modelAction.EmergencyMedicalExaminationRecordModelActionTypes.FETCH_EMERGENCY_MEDICAL_EXAMINATION_RECORD_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.EmergencyMedicalExaminationRecordAction(
						modelAction.EmergencyMedicalExaminationRecordModelActionTypes.COUNT_EMERGENCY_MEDICAL_EXAMINATION_RECORDS,
						null
					)
				]
			))
		...
		this.emergencyMedicalExaminationRecords = this.store.select(getEmergencyMedicalExaminationRecordCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getEmergencyMedicalExaminationRecordCollectionCount, this.collectionId).subscribe(
				(count) => this.emergencyMedicalExaminationRecordsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('emergency-medical-examination-record');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: EmergencyMedicalExaminationRecordModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.EmergencyMedicalExaminationRecordModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.EmergencyMedicalExaminationRecordModelActionTypes.EXPORT_ALL_EMERGENCY_MEDICAL_EXAMINATION_RECORD;
		} else {
			actionType = modelAction.EmergencyMedicalExaminationRecordModelActionTypes.EXPORT_EMERGENCY_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.EmergencyMedicalExaminationRecordAction(
				modelAction.EmergencyMedicalExaminationRecordModelActionTypes.IMPORT_EMERGENCY_MEDICAL_EXAMINATION_RECORDS,
				{
					file: file,
				},
				[new modelAction.EmergencyMedicalExaminationRecordAction(
			...
			window.open(environment.API_URL + '/api/emergencymedicalexaminationrecord/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: EmergencyMedicalExaminationRecordModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Emergency Medical Examination Record entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Emergency Medical Examination Record entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.EmergencyMedicalExaminationRecordModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.EmergencyMedicalExaminationRecordModelActionTypes.DELETE_EMERGENCY_MEDICAL_EXAMINATION_RECORD;
						} else if (usingSelected) {
							actionType = modelAction.EmergencyMedicalExaminationRecordModelActionTypes.DELETE_ALL_EMERGENCY_MEDICAL_EXAMINATION_RECORD;
						} else {
							actionType = modelAction.EmergencyMedicalExaminationRecordModelActionTypes.DELETE_EMERGENCY_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.EmergencyMedicalExaminationRecordAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_EMERGENCY_MEDICAL_EXAMINATION_RECORD when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_EMERGENCY_MEDICAL_EXAMINATION_RECORD and DELETE_EMERGENCY_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<EmergencyMedicalExaminationRecordModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = EmergencyMedicalExaminationRecordModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.emergencyMedicalExaminationRecordsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
