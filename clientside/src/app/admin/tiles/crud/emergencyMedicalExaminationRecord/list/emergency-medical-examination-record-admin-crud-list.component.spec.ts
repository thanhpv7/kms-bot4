...
import {AdminEmergencyMedicalExaminationRecordCrudListComponent} from './emergency-medical-examination-record-admin-crud-list.component';
...
import {AdminEmergencyMedicalExaminationRecordCrudModule} from '../emergency-medical-examination-record-admin-crud.module';
import {
 ...
 */
describe('Emergency Medical Examination Record Crud List Tile Component', () => {

	let fixture;
	let emergencyMedicalExaminationRecordCrudListComponent: AdminEmergencyMedicalExaminationRecordCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<EmergencyMedicalExaminationRecordModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ea118095-aa48-409f-9fd3-e507d581384d';
	const searchContent = 'text';

	const searchableFieldCount = EmergencyMedicalExaminationRecordModel.searchFields.length;

	const routerState: RouterState = {
		url: 'emergency-medical-examination-record-admin-crud',
		urls: ['emergency-medical-examination-record-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const emergencyMedicalExaminationRecordDataFactory = new EmergencyMedicalExaminationRecordDataFactory();

	const defaultDataSize = 30;
	const defaultData = emergencyMedicalExaminationRecordDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getEmergencyMedicalExaminationRecordCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getEmergencyMedicalExaminationRecordCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminEmergencyMedicalExaminationRecordCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminEmergencyMedicalExaminationRecordCrudListComponent);
			emergencyMedicalExaminationRecordCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(emergencyMedicalExaminationRecordCrudListComponent.emergencyMedicalExaminationRecordsCount).toBe(defaultData.length);
			...
			expect(emergencyMedicalExaminationRecordCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			emergencyMedicalExaminationRecordCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = emergencyMedicalExaminationRecordDataFactory.createAll(filteredDataSize);
			...
			expect(emergencyMedicalExaminationRecordCrudListComponent.emergencyMedicalExaminationRecordsCount).toBe(filteredDataSize);
			expect(emergencyMedicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(emergencyMedicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
			expect(emergencyMedicalExaminationRecordCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			emergencyMedicalExaminationRecordCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		emergencyMedicalExaminationRecordCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		emergencyMedicalExaminationRecordCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
