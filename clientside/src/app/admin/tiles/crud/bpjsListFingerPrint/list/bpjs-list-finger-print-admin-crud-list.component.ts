...
import * as modelAction from 'src/app/models/bpjsListFingerPrint/bpjs_list_finger_print.model.action';
import {BpjsListFingerPrintModelState} from 'src/app/models/bpjsListFingerPrint/bpjs_list_finger_print.model.state';
...
import {
	getBpjsListFingerPrintCollectionModels,
	getBpjsListFingerPrintCollectionCount
} from 'src/app/models/bpjsListFingerPrint/bpjs_list_finger_print.model.selector';
...
import {CrudTileMode} from '../bpjs-list-finger-print-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-list-finger-print-admin-crud-list',
	templateUrl: './bpjs-list-finger-print-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-list-finger-print-admin-crud-list.component.scss',
...
export class AdminBpjsListFingerPrintCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsListFingerPrintModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsListFingerPrintModel.searchFields,
	...
	/**
	 * List of BPJS List Finger Prints that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsListFingerPrints: Observable<BpjsListFingerPrintModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsListFingerPrintsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsListFingerPrintModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsListFingerPrintModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'f4f7c1d4-c320-438f-a018-84df7a5d4e6f';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsListFingerPrintsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsListFingerPrintsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsListFingerPrintModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
				modelAction.BpjsListFingerPrintModelActionTypes.INITIALISE_BPJS_LIST_FINGER_PRINT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
				modelAction.BpjsListFingerPrintModelActionTypes.FETCH_BPJS_LIST_FINGER_PRINT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsListFingerPrintAction(
						modelAction.BpjsListFingerPrintModelActionTypes.COUNT_BPJS_LIST_FINGER_PRINTS,
						null
					)
				]
			))
		...
		this.bpjsListFingerPrints = this.store.select(getBpjsListFingerPrintCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsListFingerPrintCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsListFingerPrintsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-list-finger-print');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsListFingerPrintModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsListFingerPrintModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsListFingerPrintModelActionTypes.EXPORT_ALL_BPJS_LIST_FINGER_PRINT;
		} else {
			actionType = modelAction.BpjsListFingerPrintModelActionTypes.EXPORT_BPJS_LIST_FINGER_PRINT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
				modelAction.BpjsListFingerPrintModelActionTypes.IMPORT_BPJS_LIST_FINGER_PRINTS,
				{
					file: file,
				},
				[new modelAction.BpjsListFingerPrintAction(
			...
			window.open(environment.API_URL + '/api/bpjslistfingerprint/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsListFingerPrintModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS List Finger Print entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS List Finger Print entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsListFingerPrintModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsListFingerPrintModelActionTypes.DELETE_BPJS_LIST_FINGER_PRINT;
						} else if (usingSelected) {
							actionType = modelAction.BpjsListFingerPrintModelActionTypes.DELETE_ALL_BPJS_LIST_FINGER_PRINT;
						} else {
							actionType = modelAction.BpjsListFingerPrintModelActionTypes.DELETE_BPJS_LIST_FINGER_PRINT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_LIST_FINGER_PRINT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_LIST_FINGER_PRINT and DELETE_BPJS_LIST_FINGER_PRINT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsListFingerPrintModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsListFingerPrintModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsListFingerPrintsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
