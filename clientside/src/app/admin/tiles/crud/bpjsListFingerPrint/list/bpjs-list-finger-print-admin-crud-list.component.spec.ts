...
import {AdminBpjsListFingerPrintCrudListComponent} from './bpjs-list-finger-print-admin-crud-list.component';
import {BpjsListFingerPrintDataFactory} from 'src/app/lib/utils/factories/bpjs-list-finger-print-data-factory';
import {BpjsListFingerPrintModel} from 'src/app/models/bpjsListFingerPrint/bpjs_list_finger_print.model';
import {AdminBpjsListFingerPrintCrudModule} from '../bpjs-list-finger-print-admin-crud.module';
import {
	BpjsListFingerPrintModelState,
	initialState as initialBpjsListFingerPrintModelState
} from 'src/app/models/bpjsListFingerPrint/bpjs_list_finger_print.model.state';
 ...
 */
describe('BPJS List Finger Print Crud List Tile Component', () => {

	let fixture;
	let bpjsListFingerPrintCrudListComponent: AdminBpjsListFingerPrintCrudListComponent;

	let store: MockStore<{ model: BpjsListFingerPrintModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsListFingerPrintModelState: BpjsListFingerPrintModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsListFingerPrintModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f4f7c1d4-c320-438f-a018-84df7a5d4e6f';
	const searchContent = 'text';

	const searchableFieldCount = BpjsListFingerPrintModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-list-finger-print-admin-crud',
		urls: ['bpjs-list-finger-print-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsListFingerPrintDataFactory = new BpjsListFingerPrintDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsListFingerPrintDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsListFingerPrintModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsListFingerPrintModelState = initialBpjsListFingerPrintModelState;
		store.setState({model: bpjsListFingerPrintModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsListFingerPrintCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsListFingerPrintCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsListFingerPrintCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsListFingerPrintCrudListComponent);
			bpjsListFingerPrintCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsListFingerPrintCrudListComponent.bpjsListFingerPrintsCount).toBe(defaultData.length);
			...
			expect(bpjsListFingerPrintCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsListFingerPrintCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsListFingerPrintDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsListFingerPrintCrudListComponent.bpjsListFingerPrintsCount).toBe(filteredDataSize);
			expect(bpjsListFingerPrintCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsListFingerPrintCrudListComponent.pageIndex).toBe(0);
			expect(bpjsListFingerPrintCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsListFingerPrintCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsListFingerPrintCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsListFingerPrintCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
