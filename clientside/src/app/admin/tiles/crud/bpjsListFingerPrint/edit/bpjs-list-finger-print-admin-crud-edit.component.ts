...
import {BpjsListFingerPrintModelState, BpjsListFingerPrintModelAudit} from 'src/app/models/bpjsListFingerPrint/bpjs_list_finger_print.model.state';
import {BpjsListFingerPrintModel} from 'src/app/models/bpjsListFingerPrint/bpjs_list_finger_print.model';
import {
	getBpjsListFingerPrintModelAuditsByEntityId,
	getBpjsListFingerPrintModelWithId,
	getCountBpjsListFingerPrintModels,
...
@Component({
	selector: 'cb-bpjs-list-finger-print-admin-crud-edit',
	templateUrl: './bpjs-list-finger-print-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-list-finger-print-admin-crud-edit.component.scss',
...
export class AdminBpjsListFingerPrintCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsListFingerPrintAudits$: Observable<BpjsListFingerPrintModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsListFingerPrintModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsListFingerPrintModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsListFingerPrintModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsListFingerPrintModel();
		...
		if (this.targetModelId) {
			this.bpjsListFingerPrintAudits$ = this.store.select(getBpjsListFingerPrintModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsListFingerPrintModel> = {
		...
		this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
			modelAction.BpjsListFingerPrintModelActionTypes.FETCH_BPJS_LIST_FINGER_PRINT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsListFingerPrintModelWithId, this.targetModelId)
			...
			model = new BpjsListFingerPrintModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
			modelAction.BpjsListFingerPrintModelActionTypes.FETCH_BPJS_LIST_FINGER_PRINT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
				modelAction.BpjsListFingerPrintModelActionTypes.CREATE_BPJS_LIST_FINGER_PRINT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsListFingerPrintAction(
				modelAction.BpjsListFingerPrintModelActionTypes.UPDATE_BPJS_LIST_FINGER_PRINT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsListFingerPrintModel = new BpjsListFingerPrintModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsListFingerPrintModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsListFingerPrintModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsListFingerPrintModel.getProps();
		this.modelRelations = BpjsListFingerPrintModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsListFingerPrintModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsListFingerPrintModel {
		let updatedModel: BpjsListFingerPrintModel = new BpjsListFingerPrintModel(model);
