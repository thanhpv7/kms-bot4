...
import * as modelAction from 'src/app/models/odontogramExamination/odontogram_examination.model.action';
import {OdontogramExaminationModelState} from 'src/app/models/odontogramExamination/odontogram_examination.model.state';
...
import {
	getOdontogramExaminationCollectionModels,
	getOdontogramExaminationCollectionCount
...
import {CrudTileMode} from '../odontogram-examination-admin-crud.component';
...
@Component({
	selector: 'cb-odontogram-examination-admin-crud-list',
	templateUrl: './odontogram-examination-admin-crud-list.component.html',
	styleUrls: [
		'./odontogram-examination-admin-crud-list.component.scss',
...
export class AdminOdontogramExaminationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<OdontogramExaminationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...OdontogramExaminationModel.searchFields,
	...
	/**
	 * List of Odontogram Examinations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	odontogramExaminations: Observable<OdontogramExaminationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	odontogramExaminationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = OdontogramExaminationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = OdontogramExaminationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'b20a4872-7852-4873-912e-77fa6dabd5e6';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.odontogramExaminationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.odontogramExaminationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: OdontogramExaminationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.OdontogramExaminationAction(
				modelAction.OdontogramExaminationModelActionTypes.INITIALISE_ODONTOGRAM_EXAMINATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.OdontogramExaminationAction(
				modelAction.OdontogramExaminationModelActionTypes.FETCH_ODONTOGRAM_EXAMINATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.OdontogramExaminationAction(
						modelAction.OdontogramExaminationModelActionTypes.COUNT_ODONTOGRAM_EXAMINATIONS,
						null
					)
				]
			))
		...
		this.odontogramExaminations = this.store.select(getOdontogramExaminationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getOdontogramExaminationCollectionCount, this.collectionId).subscribe(
				(count) => this.odontogramExaminationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('odontogram-examination');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: OdontogramExaminationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.OdontogramExaminationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.OdontogramExaminationModelActionTypes.EXPORT_ALL_ODONTOGRAM_EXAMINATION;
		} else {
			actionType = modelAction.OdontogramExaminationModelActionTypes.EXPORT_ODONTOGRAM_EXAMINATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.OdontogramExaminationAction(
				modelAction.OdontogramExaminationModelActionTypes.IMPORT_ODONTOGRAM_EXAMINATIONS,
				{
					file: file,
				},
				[new modelAction.OdontogramExaminationAction(
			...
			window.open(environment.API_URL + '/api/odontogramexamination/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: OdontogramExaminationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Odontogram Examination entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Odontogram Examination entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.OdontogramExaminationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.OdontogramExaminationModelActionTypes.DELETE_ODONTOGRAM_EXAMINATION;
						} else if (usingSelected) {
							actionType = modelAction.OdontogramExaminationModelActionTypes.DELETE_ALL_ODONTOGRAM_EXAMINATION;
						} else {
							actionType = modelAction.OdontogramExaminationModelActionTypes.DELETE_ODONTOGRAM_EXAMINATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.OdontogramExaminationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ODONTOGRAM_EXAMINATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ODONTOGRAM_EXAMINATION and DELETE_ODONTOGRAM_EXAMINATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<OdontogramExaminationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = OdontogramExaminationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.odontogramExaminationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
