...
import {AdminOdontogramExaminationCrudListComponent} from './odontogram-examination-admin-crud-list.component';
...
import {AdminOdontogramExaminationCrudModule} from '../odontogram-examination-admin-crud.module';
import {
	OdontogramExaminationModelState,
	initialState as initialOdontogramExaminationModelState
} from 'src/app/models/odontogramExamination/odontogram_examination.model.state';
 ...
 */
describe('Odontogram Examination Crud List Tile Component', () => {

	let fixture;
	let odontogramExaminationCrudListComponent: AdminOdontogramExaminationCrudListComponent;

	let store: MockStore<{ model: OdontogramExaminationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let odontogramExaminationModelState: OdontogramExaminationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<OdontogramExaminationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b20a4872-7852-4873-912e-77fa6dabd5e6';
	const searchContent = 'text';

	const searchableFieldCount = OdontogramExaminationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'odontogram-examination-admin-crud',
		urls: ['odontogram-examination-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const odontogramExaminationDataFactory = new OdontogramExaminationDataFactory();

	const defaultDataSize = 30;
	const defaultData = odontogramExaminationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: OdontogramExaminationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		odontogramExaminationModelState = initialOdontogramExaminationModelState;
		store.setState({model: odontogramExaminationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getOdontogramExaminationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getOdontogramExaminationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminOdontogramExaminationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminOdontogramExaminationCrudListComponent);
			odontogramExaminationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(odontogramExaminationCrudListComponent.odontogramExaminationsCount).toBe(defaultData.length);
			...
			expect(odontogramExaminationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			odontogramExaminationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = odontogramExaminationDataFactory.createAll(filteredDataSize);
			...
			expect(odontogramExaminationCrudListComponent.odontogramExaminationsCount).toBe(filteredDataSize);
			expect(odontogramExaminationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(odontogramExaminationCrudListComponent.pageIndex).toBe(0);
			expect(odontogramExaminationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			odontogramExaminationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		odontogramExaminationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		odontogramExaminationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
