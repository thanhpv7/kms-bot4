...
import {OdontogramExaminationModelState, OdontogramExaminationModelAudit} from 'src/app/models/odontogramExamination/odontogram_examination.model.state';
import {OdontogramExaminationModel} from 'src/app/models/odontogramExamination/odontogram_examination.model';
import {
	getOdontogramExaminationModelAuditsByEntityId,
	getOdontogramExaminationModelWithId,
	getCountOdontogramExaminationModels,
...
@Component({
	selector: 'cb-odontogram-examination-admin-crud-edit',
	templateUrl: './odontogram-examination-admin-crud-edit.component.html',
	styleUrls: [
		'./odontogram-examination-admin-crud-edit.component.scss',
...
export class AdminOdontogramExaminationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	odontogramExaminationAudits$: Observable<OdontogramExaminationModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'odontogramExamination';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = OdontogramExaminationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: OdontogramExaminationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountOdontogramExaminationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new OdontogramExaminationModel();
		...
		if (this.targetModelId) {
			this.odontogramExaminationAudits$ = this.store.select(getOdontogramExaminationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<OdontogramExaminationModel> = {
		...
		this.store.dispatch(new modelAction.OdontogramExaminationAction(
			modelAction.OdontogramExaminationModelActionTypes.FETCH_ODONTOGRAM_EXAMINATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getOdontogramExaminationModelWithId, this.targetModelId)
			...
			model = new OdontogramExaminationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.OdontogramExaminationAction(
			modelAction.OdontogramExaminationModelActionTypes.FETCH_ODONTOGRAM_EXAMINATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.OdontogramExaminationAction(
				modelAction.OdontogramExaminationModelActionTypes.CREATE_ODONTOGRAM_EXAMINATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.OdontogramExaminationAction(
				modelAction.OdontogramExaminationModelActionTypes.UPDATE_ODONTOGRAM_EXAMINATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: OdontogramExaminationModel = new OdontogramExaminationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getOdontogramExaminationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new OdontogramExaminationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = OdontogramExaminationModel.getProps();
		this.modelRelations = OdontogramExaminationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: OdontogramExaminationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): OdontogramExaminationModel {
		let updatedModel: OdontogramExaminationModel = new OdontogramExaminationModel(model);
