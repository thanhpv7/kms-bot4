...
import * as modelAction from 'src/app/models/bpjsAccidentMaster/bpjs_accident_master.model.action';
import {BpjsAccidentMasterModelState} from 'src/app/models/bpjsAccidentMaster/bpjs_accident_master.model.state';
...
import {
	getBpjsAccidentMasterCollectionModels,
	getBpjsAccidentMasterCollectionCount
...
import {CrudTileMode} from '../bpjs-accident-master-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-accident-master-admin-crud-list',
	templateUrl: './bpjs-accident-master-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-accident-master-admin-crud-list.component.scss',
...
export class AdminBpjsAccidentMasterCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsAccidentMasterModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsAccidentMasterModel.searchFields,
	...
	/**
	 * List of BPJS Accident Masters that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsAccidentMasters: Observable<BpjsAccidentMasterModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsAccidentMastersCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsAccidentMasterModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsAccidentMasterModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '12504718-bd12-4ecf-bdb0-a8b637234359';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsAccidentMastersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsAccidentMastersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsAccidentMasterModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
				modelAction.BpjsAccidentMasterModelActionTypes.INITIALISE_BPJS_ACCIDENT_MASTER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
				modelAction.BpjsAccidentMasterModelActionTypes.FETCH_BPJS_ACCIDENT_MASTER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsAccidentMasterAction(
						modelAction.BpjsAccidentMasterModelActionTypes.COUNT_BPJS_ACCIDENT_MASTERS,
						null
					)
				]
			))
		...
		this.bpjsAccidentMasters = this.store.select(getBpjsAccidentMasterCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsAccidentMasterCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsAccidentMastersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-accident-master');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsAccidentMasterModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsAccidentMasterModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsAccidentMasterModelActionTypes.EXPORT_ALL_BPJS_ACCIDENT_MASTER;
		} else {
			actionType = modelAction.BpjsAccidentMasterModelActionTypes.EXPORT_BPJS_ACCIDENT_MASTER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
				modelAction.BpjsAccidentMasterModelActionTypes.IMPORT_BPJS_ACCIDENT_MASTERS,
				{
					file: file,
				},
				[new modelAction.BpjsAccidentMasterAction(
			...
			window.open(environment.API_URL + '/api/bpjsaccidentmaster/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsAccidentMasterModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Accident Master entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Accident Master entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsAccidentMasterModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsAccidentMasterModelActionTypes.DELETE_BPJS_ACCIDENT_MASTER;
						} else if (usingSelected) {
							actionType = modelAction.BpjsAccidentMasterModelActionTypes.DELETE_ALL_BPJS_ACCIDENT_MASTER;
						} else {
							actionType = modelAction.BpjsAccidentMasterModelActionTypes.DELETE_BPJS_ACCIDENT_MASTER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_ACCIDENT_MASTER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_ACCIDENT_MASTER and DELETE_BPJS_ACCIDENT_MASTER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsAccidentMasterModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsAccidentMasterModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsAccidentMastersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
