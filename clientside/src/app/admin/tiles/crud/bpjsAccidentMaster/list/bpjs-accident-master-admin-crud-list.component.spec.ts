...
import {AdminBpjsAccidentMasterCrudListComponent} from './bpjs-accident-master-admin-crud-list.component';
...
import {AdminBpjsAccidentMasterCrudModule} from '../bpjs-accident-master-admin-crud.module';
import {
	BpjsAccidentMasterModelState,
	initialState as initialBpjsAccidentMasterModelState
} from 'src/app/models/bpjsAccidentMaster/bpjs_accident_master.model.state';
 ...
 */
describe('BPJS Accident Master Crud List Tile Component', () => {

	let fixture;
	let bpjsAccidentMasterCrudListComponent: AdminBpjsAccidentMasterCrudListComponent;

	let store: MockStore<{ model: BpjsAccidentMasterModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsAccidentMasterModelState: BpjsAccidentMasterModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsAccidentMasterModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '12504718-bd12-4ecf-bdb0-a8b637234359';
	const searchContent = 'text';

	const searchableFieldCount = BpjsAccidentMasterModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-accident-master-admin-crud',
		urls: ['bpjs-accident-master-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsAccidentMasterDataFactory = new BpjsAccidentMasterDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsAccidentMasterDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsAccidentMasterModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsAccidentMasterModelState = initialBpjsAccidentMasterModelState;
		store.setState({model: bpjsAccidentMasterModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsAccidentMasterCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsAccidentMasterCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsAccidentMasterCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsAccidentMasterCrudListComponent);
			bpjsAccidentMasterCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsAccidentMasterCrudListComponent.bpjsAccidentMastersCount).toBe(defaultData.length);
			...
			expect(bpjsAccidentMasterCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsAccidentMasterCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsAccidentMasterDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsAccidentMasterCrudListComponent.bpjsAccidentMastersCount).toBe(filteredDataSize);
			expect(bpjsAccidentMasterCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsAccidentMasterCrudListComponent.pageIndex).toBe(0);
			expect(bpjsAccidentMasterCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsAccidentMasterCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsAccidentMasterCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsAccidentMasterCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
