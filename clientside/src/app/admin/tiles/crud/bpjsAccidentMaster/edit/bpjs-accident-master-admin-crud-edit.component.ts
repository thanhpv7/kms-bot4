...
import {BpjsAccidentMasterModelState, BpjsAccidentMasterModelAudit} from 'src/app/models/bpjsAccidentMaster/bpjs_accident_master.model.state';
import {BpjsAccidentMasterModel} from 'src/app/models/bpjsAccidentMaster/bpjs_accident_master.model';
import {
	getBpjsAccidentMasterModelAuditsByEntityId,
	getBpjsAccidentMasterModelWithId,
	getCountBpjsAccidentMasterModels,
...
@Component({
	selector: 'cb-bpjs-accident-master-admin-crud-edit',
	templateUrl: './bpjs-accident-master-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-accident-master-admin-crud-edit.component.scss',
...
export class AdminBpjsAccidentMasterCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsAccidentMasterAudits$: Observable<BpjsAccidentMasterModelAudit[]>;
	...
	patientPaymentBPJSOppositeRelation = 'bpjsAccidentMasters';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsAccidentMasterModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsAccidentMasterModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsAccidentMasterModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsAccidentMasterModel();
		...
		if (this.targetModelId) {
			this.bpjsAccidentMasterAudits$ = this.store.select(getBpjsAccidentMasterModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsAccidentMasterModel> = {
		...
		this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
			modelAction.BpjsAccidentMasterModelActionTypes.FETCH_BPJS_ACCIDENT_MASTER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsAccidentMasterModelWithId, this.targetModelId)
			...
			model = new BpjsAccidentMasterModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
			modelAction.BpjsAccidentMasterModelActionTypes.FETCH_BPJS_ACCIDENT_MASTER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsAccidentMasterModel> = {
				...
				queryParams: {
					persistRelations: this.patientPaymentBPJSMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
				modelAction.BpjsAccidentMasterModelActionTypes.CREATE_BPJS_ACCIDENT_MASTER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsAccidentMasterAction(
				modelAction.BpjsAccidentMasterModelActionTypes.UPDATE_BPJS_ACCIDENT_MASTER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsAccidentMasterModel = new BpjsAccidentMasterModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsAccidentMasterModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsAccidentMasterModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsAccidentMasterModel.getProps();
		this.modelRelations = BpjsAccidentMasterModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsAccidentMasterModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsAccidentMasterModel {
		let updatedModel: BpjsAccidentMasterModel = new BpjsAccidentMasterModel(model);
