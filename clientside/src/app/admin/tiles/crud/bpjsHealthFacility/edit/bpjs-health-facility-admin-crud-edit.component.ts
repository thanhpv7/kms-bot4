...
import * as modelAction from 'src/app/models/bpjsHealthFacility/bpjs_health_facility.model.action';
import {BpjsHealthFacilityModelState, BpjsHealthFacilityModelAudit} from 'src/app/models/bpjsHealthFacility/bpjs_health_facility.model.state';
import {BpjsHealthFacilityModel} from 'src/app/models/bpjsHealthFacility/bpjs_health_facility.model';
import {
	getBpjsHealthFacilityModelAuditsByEntityId,
...
import * as bpjsFacilityListModelAction from 'src/app/models/bpjsFacilityList/bpjs_facility_list.model.action';
import {
	getBpjsFacilityListCollectionModels, getBpjsFacilityListModelWithId,
...
import * as bpjsReferralSpecialistModelAction from 'src/app/models/bpjsReferralSpecialist/bpjs_referral_specialist.model.action';
import {
	getBpjsReferralSpecialistCollectionModels, getBpjsReferralSpecialistModelWithId,
...
import {CrudTileMode} from '../bpjs-health-facility-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-health-facility-admin-crud-edit',
	templateUrl: './bpjs-health-facility-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-health-facility-admin-crud-edit.component.scss',
...
export class AdminBpjsHealthFacilityCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '61f91312-50f2-4110-8eec-e3f1276decc6';
	 ...
	 */
	bpjsHealthFacilityAudits$: Observable<BpjsHealthFacilityModelAudit[]>;
	...
	bpjsClaimSubmissionsOppositeRelation = 'bpjsHealthFacility';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Health Facility and BPJS Facility Lists here] off begin
	bpjsFacilityListsMultiCrudActive: boolean = false;
	bpjsFacilityListsDisplayName = 'BPJS Facility Lists';
	bpjsFacilityListsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Health Facility and BPJS Facility Lists here] end
	bpjsFacilityListsOppositeRelation = 'bpjsHealthFacility';
	bpjsFacilityListsRelations: ModelRelation[] = this.bpjsFacilityListsMultiCrudActive ? BpjsFacilityListModel.getRelations()
		.map(relation => {
			if (relation.id === this.bpjsFacilityListsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Health Facility and BPJS Patient Referrals here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Health Facility and BPJS Patient Referrals here] end
	bpjsPatientReferralsOppositeRelation = 'bpjsHealthFacility';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Health Facility and BPJS Referral Specialists here] off begin
	bpjsReferralSpecialistsMultiCrudActive: boolean = false;
	bpjsReferralSpecialistsDisplayName = 'BPJS Referral Specialists';
	bpjsReferralSpecialistsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between BPJS Health Facility and BPJS Referral Specialists here] end
	bpjsReferralSpecialistsOppositeRelation = 'bpjsHealthFacility';
	bpjsReferralSpecialistsRelations: ModelRelation[] = this.bpjsReferralSpecialistsMultiCrudActive ? BpjsReferralSpecialistModel.getRelations()
		.map(relation => {
			if (relation.id === this.bpjsReferralSpecialistsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsHealthFacilityModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsHealthFacilityModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsHealthFacilityModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'bpjsFacilityLists' && this.bpjsFacilityListsMultiCrudActive) {
					exp.fields = BpjsFacilityListModel.getProps().map(prop => {
						return prop.name;
					});
				}
				...
				if (relation.id === 'bpjsReferralSpecialists' && this.bpjsReferralSpecialistsMultiCrudActive) {
					exp.fields = BpjsReferralSpecialistModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: BpjsHealthFacilityModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsHealthFacilityModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsHealthFacilityModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsHealthFacilityModel();
		...
		if (this.targetModelId) {
			this.bpjsHealthFacilityAudits$ = this.store.select(getBpjsHealthFacilityModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsHealthFacilityModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
			modelAction.BpjsHealthFacilityModelActionTypes.FETCH_BPJS_HEALTH_FACILITY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsHealthFacilityModelWithId, this.targetModelId)
			...
			model = new BpjsHealthFacilityModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-health-facility');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
			modelAction.BpjsHealthFacilityModelActionTypes.FETCH_BPJS_HEALTH_FACILITY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsHealthFacilityModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.bpjsClaimSubmissionsMultiCrudActive || this.bpjsFacilityListsMultiCrudActive || this.bpjsPatientReferralsMultiCrudActive || this.bpjsReferralSpecialistsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsHealthFacilityAction(
						modelAction.BpjsHealthFacilityModelActionTypes.COUNT_BPJS_HEALTH_FACILITYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
				modelAction.BpjsHealthFacilityModelActionTypes.CREATE_BPJS_HEALTH_FACILITY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
				modelAction.BpjsHealthFacilityModelActionTypes.UPDATE_BPJS_HEALTH_FACILITY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsHealthFacilityModel = new BpjsHealthFacilityModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsHealthFacilityModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsHealthFacilityModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.bpjsFacilityListsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsFacilityListModel.getProps(), this.bpjsFacilityListsRelations, this.isViewOnly),
					new BpjsFacilityListModel(),
					BpjsFacilityListModel,
					this.bpjsFacilityListsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.bpjsFacilityListsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.bpjsReferralSpecialistsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsReferralSpecialistModel.getProps(), this.bpjsReferralSpecialistsRelations, this.isViewOnly),
					new BpjsReferralSpecialistModel(),
					BpjsReferralSpecialistModel,
					this.bpjsReferralSpecialistsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.bpjsReferralSpecialistsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsHealthFacilityModel.getProps();
		this.modelRelations = BpjsHealthFacilityModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'bpjsFacilityLists';
		}).hideElement = this.bpjsFacilityListsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'bpjsFacilityLists').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'bpjsReferralSpecialists';
		}).hideElement = this.bpjsReferralSpecialistsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'bpjsReferralSpecialists').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.bpjsFacilityListsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.bpjsFacilityListsDisplayName, many: true});
			this.store.dispatch(new bpjsFacilityListModelAction.BpjsFacilityListAction(
				bpjsFacilityListModelAction.BpjsFacilityListModelActionTypes.INITIALISE_BPJS_FACILITY_LIST_COLLECTION_STATE,
				{
					collectionId: this.bpjsFacilityListsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsFacilityListCollectionModels, this.bpjsFacilityListsCollectionId).subscribe(models => {
				models.forEach(bpjsFacilityLists => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(BpjsFacilityListModel.getProps(), this.bpjsFacilityListsRelations, this.isViewOnly),
						bpjsFacilityLists,
						BpjsFacilityListModel,
		...
		if (this.bpjsReferralSpecialistsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.bpjsReferralSpecialistsDisplayName, many: true});
			this.store.dispatch(new bpjsReferralSpecialistModelAction.BpjsReferralSpecialistAction(
				bpjsReferralSpecialistModelAction.BpjsReferralSpecialistModelActionTypes.INITIALISE_BPJS_REFERRAL_SPECIALIST_COLLECTION_STATE,
				{
					collectionId: this.bpjsReferralSpecialistsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsReferralSpecialistCollectionModels, this.bpjsReferralSpecialistsCollectionId).subscribe(models => {
				models.forEach(bpjsReferralSpecialists => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(BpjsReferralSpecialistModel.getProps(), this.bpjsReferralSpecialistsRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.bpjsFacilityListsIds && this.targetModel.bpjsFacilityListsIds.length > 0 && this.bpjsFacilityListsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsFacilityListModel,
				this.bpjsFacilityListsOppositeRelation,
				this.bpjsFacilityListsCollectionId,
				this.targetModel.bpjsFacilityListsIds.length,
				this.targetModel.id,
				true
			));
		}
		...
		if (this.targetModel.bpjsReferralSpecialistsIds && this.targetModel.bpjsReferralSpecialistsIds.length > 0 && this.bpjsReferralSpecialistsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsReferralSpecialistModel,
				this.bpjsReferralSpecialistsOppositeRelation,
				this.bpjsReferralSpecialistsCollectionId,
				this.targetModel.bpjsReferralSpecialistsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsHealthFacilityModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsHealthFacilityModel {
		let updatedModel: BpjsHealthFacilityModel = new BpjsHealthFacilityModel(model);
		...
		updatedModel.bpjsFacilityLists = [];
		if (this.bpjsFacilityListsMultiCrudActive) {
			if (event.additionalEntities.bpjsFacilityLists) {
				event.additionalEntities.bpjsFacilityLists.forEach(entity => {
					updatedModel.bpjsFacilityLists.push(entity);
				});
			}

			if (!updatedModel.bpjsFacilityLists.length) {
				updatedModel.bpjsFacilityListsIds = [];
			}
		}
		...
		updatedModel.bpjsReferralSpecialists = [];
		if (this.bpjsReferralSpecialistsMultiCrudActive) {
			if (event.additionalEntities.bpjsReferralSpecialists) {
				event.additionalEntities.bpjsReferralSpecialists.forEach(entity => {
					updatedModel.bpjsReferralSpecialists.push(entity);
				});
			}

			if (!updatedModel.bpjsReferralSpecialists.length) {
				updatedModel.bpjsReferralSpecialistsIds = [];
			}
		}
		return updatedModel;
	}
