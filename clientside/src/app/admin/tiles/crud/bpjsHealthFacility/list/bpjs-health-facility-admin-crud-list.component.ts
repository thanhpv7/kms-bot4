...
import {BpjsHealthFacilityModelState} from 'src/app/models/bpjsHealthFacility/bpjs_health_facility.model.state';
...
import {
	...
	getBpjsHealthFacilityCollectionCount
...
@Component({
	selector: 'cb-bpjs-health-facility-admin-crud-list',
	templateUrl: './bpjs-health-facility-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-health-facility-admin-crud-list.component.scss',
...
export class AdminBpjsHealthFacilityCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsHealthFacilityModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsHealthFacilityModel.searchFields,
	...
	/**
	 * List of BPJS Health Facilitys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsHealthFacilitys: Observable<BpjsHealthFacilityModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsHealthFacilitysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsHealthFacilitysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsHealthFacilitysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
				modelAction.BpjsHealthFacilityModelActionTypes.INITIALISE_BPJS_HEALTH_FACILITY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
				modelAction.BpjsHealthFacilityModelActionTypes.FETCH_BPJS_HEALTH_FACILITY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsHealthFacilitys = this.store.select(getBpjsHealthFacilityCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsHealthFacilityCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsHealthFacilitysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsHealthFacilityModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsHealthFacilityModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsHealthFacilityModelActionTypes.EXPORT_ALL_BPJS_HEALTH_FACILITY;
		} else {
			actionType = modelAction.BpjsHealthFacilityModelActionTypes.EXPORT_BPJS_HEALTH_FACILITY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
				modelAction.BpjsHealthFacilityModelActionTypes.IMPORT_BPJS_HEALTH_FACILITYS,
				{
					file: file,
				},
				[new modelAction.BpjsHealthFacilityAction(
			...
			window.open(environment.API_URL + '/api/bpjshealthfacility/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsHealthFacilityModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Health Facility entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Health Facility entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsHealthFacilityModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsHealthFacilityModelActionTypes.DELETE_BPJS_HEALTH_FACILITY;
						} else if (usingSelected) {
							actionType = modelAction.BpjsHealthFacilityModelActionTypes.DELETE_ALL_BPJS_HEALTH_FACILITY;
						} else {
							actionType = modelAction.BpjsHealthFacilityModelActionTypes.DELETE_BPJS_HEALTH_FACILITY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsHealthFacilityAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_HEALTH_FACILITY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_HEALTH_FACILITY and DELETE_BPJS_HEALTH_FACILITY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsHealthFacilityModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsHealthFacilitysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
