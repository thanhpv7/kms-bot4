...
import {AdminBpjsHealthFacilityCrudListComponent} from './bpjs-health-facility-admin-crud-list.component';
...
import {AdminBpjsHealthFacilityCrudModule} from '../bpjs-health-facility-admin-crud.module';
import {
 ...
 */
describe('BPJS Health Facility Crud List Tile Component', () => {

	let fixture;
	let bpjsHealthFacilityCrudListComponent: AdminBpjsHealthFacilityCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsHealthFacilityModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '61f91312-50f2-4110-8eec-e3f1276decc6';
	const searchContent = 'text';

	const searchableFieldCount = BpjsHealthFacilityModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-health-facility-admin-crud',
		urls: ['bpjs-health-facility-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsHealthFacilityDataFactory = new BpjsHealthFacilityDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsHealthFacilityDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsHealthFacilityCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsHealthFacilityCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsHealthFacilityCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsHealthFacilityCrudListComponent);
			bpjsHealthFacilityCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsHealthFacilityCrudListComponent.bpjsHealthFacilitysCount).toBe(defaultData.length);
			...
			expect(bpjsHealthFacilityCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsHealthFacilityCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsHealthFacilityDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsHealthFacilityCrudListComponent.bpjsHealthFacilitysCount).toBe(filteredDataSize);
			expect(bpjsHealthFacilityCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsHealthFacilityCrudListComponent.pageIndex).toBe(0);
			expect(bpjsHealthFacilityCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsHealthFacilityCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsHealthFacilityCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsHealthFacilityCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
