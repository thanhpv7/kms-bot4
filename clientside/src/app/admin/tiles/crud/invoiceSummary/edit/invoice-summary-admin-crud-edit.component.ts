...
import * as modelAction from 'src/app/models/invoiceSummary/invoice_summary.model.action';
import {InvoiceSummaryModelState, InvoiceSummaryModelAudit} from 'src/app/models/invoiceSummary/invoice_summary.model.state';
import {InvoiceSummaryModel} from 'src/app/models/invoiceSummary/invoice_summary.model';
import {
	getInvoiceSummaryModelAuditsByEntityId,
	getInvoiceSummaryModelWithId,
	getCountInvoiceSummaryModels,
...
import {CrudTileMode} from '../invoice-summary-admin-crud.component';
...
@Component({
	selector: 'cb-invoice-summary-admin-crud-edit',
	templateUrl: './invoice-summary-admin-crud-edit.component.html',
	styleUrls: [
		'./invoice-summary-admin-crud-edit.component.scss',
...
export class AdminInvoiceSummaryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'cd9893d8-f08d-45a3-9372-ff88d99c2027';
	 ...
	 */
	invoiceSummaryAudits$: Observable<InvoiceSummaryModelAudit[]>;
	...
	invoiceOppositeRelation = 'invoiceSummary';
	...
	retailPharmacyInvoiceOppositeRelation = 'invoiceSummary';
	 ...
	 */
	modelProperties: ModelProperty[] = InvoiceSummaryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InvoiceSummaryModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InvoiceSummaryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: InvoiceSummaryModel;
	...
	constructor(
		private readonly store: Store<{ model: InvoiceSummaryModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInvoiceSummaryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InvoiceSummaryModel();
		...
		if (this.targetModelId) {
			this.invoiceSummaryAudits$ = this.store.select(getInvoiceSummaryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InvoiceSummaryModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InvoiceSummaryAction(
			modelAction.InvoiceSummaryModelActionTypes.FETCH_INVOICE_SUMMARY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInvoiceSummaryModelWithId, this.targetModelId)
			...
			model = new InvoiceSummaryModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InvoiceSummaryAction(
			modelAction.InvoiceSummaryModelActionTypes.FETCH_INVOICE_SUMMARY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InvoiceSummaryModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.invoiceMultiCrudActive || this.retailPharmacyInvoiceMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InvoiceSummaryAction(
						modelAction.InvoiceSummaryModelActionTypes.COUNT_INVOICE_SUMMARYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InvoiceSummaryAction(
				modelAction.InvoiceSummaryModelActionTypes.CREATE_INVOICE_SUMMARY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InvoiceSummaryAction(
				modelAction.InvoiceSummaryModelActionTypes.UPDATE_INVOICE_SUMMARY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InvoiceSummaryModel = new InvoiceSummaryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInvoiceSummaryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InvoiceSummaryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InvoiceSummaryModel.getProps();
		this.modelRelations = InvoiceSummaryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: InvoiceSummaryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InvoiceSummaryModel {
		let updatedModel: InvoiceSummaryModel = new InvoiceSummaryModel(model);
