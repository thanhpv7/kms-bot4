...
import {InvoiceSummaryModelState} from 'src/app/models/invoiceSummary/invoice_summary.model.state';
...
import {
	getInvoiceSummaryCollectionModels,
	getInvoiceSummaryCollectionCount
...
@Component({
	selector: 'cb-invoice-summary-admin-crud-list',
	templateUrl: './invoice-summary-admin-crud-list.component.html',
	styleUrls: [
		'./invoice-summary-admin-crud-list.component.scss',
...
export class AdminInvoiceSummaryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InvoiceSummaryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InvoiceSummaryModel.searchFields,
	...
	/**
	 * List of Invoice Summarys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	invoiceSummarys: Observable<InvoiceSummaryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	invoiceSummarysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.invoiceSummarysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.invoiceSummarysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceSummaryAction(
				modelAction.InvoiceSummaryModelActionTypes.INITIALISE_INVOICE_SUMMARY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InvoiceSummaryAction(
				modelAction.InvoiceSummaryModelActionTypes.FETCH_INVOICE_SUMMARY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.invoiceSummarys = this.store.select(getInvoiceSummaryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInvoiceSummaryCollectionCount, this.collectionId).subscribe(
				(count) => this.invoiceSummarysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InvoiceSummaryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InvoiceSummaryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InvoiceSummaryModelActionTypes.EXPORT_ALL_INVOICE_SUMMARY;
		} else {
			actionType = modelAction.InvoiceSummaryModelActionTypes.EXPORT_INVOICE_SUMMARY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InvoiceSummaryAction(
				modelAction.InvoiceSummaryModelActionTypes.IMPORT_INVOICE_SUMMARYS,
				{
					file: file,
				},
				[new modelAction.InvoiceSummaryAction(
			...
			window.open(environment.API_URL + '/api/invoicesummary/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InvoiceSummaryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Invoice Summary entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Invoice Summary entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InvoiceSummaryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InvoiceSummaryModelActionTypes.DELETE_INVOICE_SUMMARY;
						} else if (usingSelected) {
							actionType = modelAction.InvoiceSummaryModelActionTypes.DELETE_ALL_INVOICE_SUMMARY;
						} else {
							actionType = modelAction.InvoiceSummaryModelActionTypes.DELETE_INVOICE_SUMMARY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InvoiceSummaryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVOICE_SUMMARY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVOICE_SUMMARY and DELETE_INVOICE_SUMMARY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InvoiceSummaryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.invoiceSummarysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
