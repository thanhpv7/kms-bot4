...
import {AdminInvoiceSummaryCrudListComponent} from './invoice-summary-admin-crud-list.component';
...
import {AdminInvoiceSummaryCrudModule} from '../invoice-summary-admin-crud.module';
import {
	InvoiceSummaryModelState,
	initialState as initialInvoiceSummaryModelState
} from 'src/app/models/invoiceSummary/invoice_summary.model.state';
 ...
 */
describe('Invoice Summary Crud List Tile Component', () => {

	let fixture;
	let invoiceSummaryCrudListComponent: AdminInvoiceSummaryCrudListComponent;

	let store: MockStore<{ model: InvoiceSummaryModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let invoiceSummaryModelState: InvoiceSummaryModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InvoiceSummaryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cd9893d8-f08d-45a3-9372-ff88d99c2027';
	const searchContent = 'text';

	const searchableFieldCount = InvoiceSummaryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'invoice-summary-admin-crud',
		urls: ['invoice-summary-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const invoiceSummaryDataFactory = new InvoiceSummaryDataFactory();

	const defaultDataSize = 30;
	const defaultData = invoiceSummaryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InvoiceSummaryModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		invoiceSummaryModelState = initialInvoiceSummaryModelState;
		store.setState({model: invoiceSummaryModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInvoiceSummaryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInvoiceSummaryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminInvoiceSummaryCrudListComponent);
			invoiceSummaryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(invoiceSummaryCrudListComponent.invoiceSummarysCount).toBe(defaultData.length);
			...
			expect(invoiceSummaryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			invoiceSummaryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = invoiceSummaryDataFactory.createAll(filteredDataSize);
			...
			expect(invoiceSummaryCrudListComponent.invoiceSummarysCount).toBe(filteredDataSize);
			expect(invoiceSummaryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(invoiceSummaryCrudListComponent.pageIndex).toBe(0);
			expect(invoiceSummaryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			invoiceSummaryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		invoiceSummaryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		invoiceSummaryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
