...
import {BpjsJasaRaharjaModelState, BpjsJasaRaharjaModelAudit} from 'src/app/models/bpjsJasaRaharja/bpjs_jasa_raharja.model.state';
import {BpjsJasaRaharjaModel} from 'src/app/models/bpjsJasaRaharja/bpjs_jasa_raharja.model';
import {
	getBpjsJasaRaharjaModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-jasa-raharja-admin-crud-edit',
	templateUrl: './bpjs-jasa-raharja-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-jasa-raharja-admin-crud-edit.component.scss',
...
export class AdminBpjsJasaRaharjaCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsJasaRaharjaAudits$: Observable<BpjsJasaRaharjaModelAudit[]>;
	...
	patientPaymentBPJSOppositeRelation = 'bpjsJasaRaharjas';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsJasaRaharjaModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsJasaRaharjaModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsJasaRaharjaModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsJasaRaharjaModel();
		...
		if (this.targetModelId) {
			this.bpjsJasaRaharjaAudits$ = this.store.select(getBpjsJasaRaharjaModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsJasaRaharjaModel> = {
		...
		this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
			modelAction.BpjsJasaRaharjaModelActionTypes.FETCH_BPJS_JASA_RAHARJA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsJasaRaharjaModelWithId, this.targetModelId)
			...
			model = new BpjsJasaRaharjaModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
			modelAction.BpjsJasaRaharjaModelActionTypes.FETCH_BPJS_JASA_RAHARJA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
				modelAction.BpjsJasaRaharjaModelActionTypes.CREATE_BPJS_JASA_RAHARJA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
				modelAction.BpjsJasaRaharjaModelActionTypes.UPDATE_BPJS_JASA_RAHARJA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsJasaRaharjaModel = new BpjsJasaRaharjaModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsJasaRaharjaModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsJasaRaharjaModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsJasaRaharjaModel.getProps();
		this.modelRelations = BpjsJasaRaharjaModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsJasaRaharjaModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsJasaRaharjaModel {
		let updatedModel: BpjsJasaRaharjaModel = new BpjsJasaRaharjaModel(model);
