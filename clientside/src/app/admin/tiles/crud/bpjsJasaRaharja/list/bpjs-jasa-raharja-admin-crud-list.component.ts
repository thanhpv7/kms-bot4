...
import * as modelAction from 'src/app/models/bpjsJasaRaharja/bpjs_jasa_raharja.model.action';
import {BpjsJasaRaharjaModelState} from 'src/app/models/bpjsJasaRaharja/bpjs_jasa_raharja.model.state';
...
import {
	...
	getBpjsJasaRaharjaCollectionCount
...
import {CrudTileMode} from '../bpjs-jasa-raharja-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-jasa-raharja-admin-crud-list',
	templateUrl: './bpjs-jasa-raharja-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-jasa-raharja-admin-crud-list.component.scss',
...
export class AdminBpjsJasaRaharjaCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsJasaRaharjaModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsJasaRaharjaModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsJasaRaharjaModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'dfa49eb5-255c-480c-bf56-92e4752d968c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsJasaRaharjasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsJasaRaharjasCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsJasaRaharjaModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
				modelAction.BpjsJasaRaharjaModelActionTypes.INITIALISE_BPJS_JASA_RAHARJA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
				modelAction.BpjsJasaRaharjaModelActionTypes.FETCH_BPJS_JASA_RAHARJA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsJasaRaharjaAction(
						modelAction.BpjsJasaRaharjaModelActionTypes.COUNT_BPJS_JASA_RAHARJAS,
						null
					)
				]
			))
		...
		this.bpjsJasaRaharjas = this.store.select(getBpjsJasaRaharjaCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsJasaRaharjaCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsJasaRaharjasCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-jasa-raharja');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsJasaRaharjaModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsJasaRaharjaModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsJasaRaharjaModelActionTypes.EXPORT_ALL_BPJS_JASA_RAHARJA;
		} else {
			actionType = modelAction.BpjsJasaRaharjaModelActionTypes.EXPORT_BPJS_JASA_RAHARJA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
				modelAction.BpjsJasaRaharjaModelActionTypes.IMPORT_BPJS_JASA_RAHARJAS,
				{
					file: file,
				},
				[new modelAction.BpjsJasaRaharjaAction(
			...
			window.open(environment.API_URL + '/api/bpjsjasaraharja/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsJasaRaharjaModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Jasa Raharja entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Jasa Raharja entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsJasaRaharjaModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsJasaRaharjaModelActionTypes.DELETE_BPJS_JASA_RAHARJA;
						} else if (usingSelected) {
							actionType = modelAction.BpjsJasaRaharjaModelActionTypes.DELETE_ALL_BPJS_JASA_RAHARJA;
						} else {
							actionType = modelAction.BpjsJasaRaharjaModelActionTypes.DELETE_BPJS_JASA_RAHARJA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsJasaRaharjaAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_JASA_RAHARJA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_JASA_RAHARJA and DELETE_BPJS_JASA_RAHARJA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsJasaRaharjaModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsJasaRaharjaModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsJasaRaharjasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
