...
import * as modelAction from 'src/app/models/bpjsProcedure/bpjs_procedure.model.action';
import {BpjsProcedureModelState, BpjsProcedureModelAudit} from 'src/app/models/bpjsProcedure/bpjs_procedure.model.state';
import {BpjsProcedureModel} from 'src/app/models/bpjsProcedure/bpjs_procedure.model';
import {
	getBpjsProcedureModelAuditsByEntityId,
...
import {CrudTileMode} from '../bpjs-procedure-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-procedure-admin-crud-edit',
	templateUrl: './bpjs-procedure-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-procedure-admin-crud-edit.component.scss',
...
export class AdminBpjsProcedureCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '987180ab-0853-49ab-9ec0-da06e4336de2';
	 ...
	 */
	bpjsProcedureAudits$: Observable<BpjsProcedureModelAudit[]>;
	...
	additionalProcedureCollectionId: string = uuidv4();
	...
	additionalProcedureRelations: ModelRelation[] = this.additionalProcedureMultiCrudActive ? BpjsSpecialReferralModel.getRelations()
		.map(relation => {
	...
	bpjsClaimSubmissionsOppositeRelation = 'bpjsProcedure';
	...
	primaryProcedureCollectionId: string = uuidv4();
	...
	primaryProcedureRelations: ModelRelation[] = this.primaryProcedureMultiCrudActive ? BpjsSpecialReferralModel.getRelations()
		.map(relation => {
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsProcedureModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsProcedureModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsProcedureModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsProcedureModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsProcedureModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsProcedureModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsProcedureModel();
		...
		if (this.targetModelId) {
			this.bpjsProcedureAudits$ = this.store.select(getBpjsProcedureModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsProcedureModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsProcedureAction(
			modelAction.BpjsProcedureModelActionTypes.FETCH_BPJS_PROCEDURE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsProcedureModelWithId, this.targetModelId)
			...
			model = new BpjsProcedureModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsProcedureAction(
			modelAction.BpjsProcedureModelActionTypes.FETCH_BPJS_PROCEDURE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsProcedureModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.additionalProcedureMultiCrudActive || this.bpjsClaimSubmissionsMultiCrudActive || this.primaryProcedureMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsProcedureAction(
						modelAction.BpjsProcedureModelActionTypes.COUNT_BPJS_PROCEDURES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsProcedureAction(
				modelAction.BpjsProcedureModelActionTypes.CREATE_BPJS_PROCEDURE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsProcedureAction(
				modelAction.BpjsProcedureModelActionTypes.UPDATE_BPJS_PROCEDURE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsProcedureModel = new BpjsProcedureModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsProcedureModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsProcedureModel(model).difference(updatedModel);
		...
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.additionalProcedureDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsSpecialReferralModel.getProps(), this.additionalProcedureRelations, this.isViewOnly),
			...
			if (event.entityName === this.primaryProcedureDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsSpecialReferralModel.getProps(), this.primaryProcedureRelations, this.isViewOnly),
					new BpjsSpecialReferralModel(),
					BpjsSpecialReferralModel,
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsProcedureModel.getProps();
		this.modelRelations = BpjsProcedureModel.getRelations();
	...
	initialiseMultiEntityCollections() {
		if (this.additionalProcedureMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.additionalProcedureDisplayName, many: true});
			this.store.dispatch(new bpjsSpecialReferralModelAction.BpjsSpecialReferralAction(
				...
				{
					collectionId: this.additionalProcedureCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsSpecialReferralCollectionModels, this.additionalProcedureCollectionId).subscribe(models => {
				models.forEach(additionalProcedure => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(BpjsSpecialReferralModel.getProps(), this.additionalProcedureRelations, this.isViewOnly),
						additionalProcedure,
						BpjsSpecialReferralModel,
		...
		if (this.primaryProcedureMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.primaryProcedureDisplayName, many: true});
			this.store.dispatch(new bpjsSpecialReferralModelAction.BpjsSpecialReferralAction(
				bpjsSpecialReferralModelAction.BpjsSpecialReferralModelActionTypes.INITIALISE_BPJS_SPECIAL_REFERRAL_COLLECTION_STATE,
				{
					collectionId: this.primaryProcedureCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsSpecialReferralCollectionModels, this.primaryProcedureCollectionId).subscribe(models => {
				models.forEach(primaryProcedure => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(BpjsSpecialReferralModel.getProps(), this.primaryProcedureRelations, this.isViewOnly),
						primaryProcedure,
						BpjsSpecialReferralModel,
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.additionalProcedureIds && this.targetModel.additionalProcedureIds.length > 0 && this.additionalProcedureMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsSpecialReferralModel,
				this.additionalProcedureOppositeRelation,
				this.additionalProcedureCollectionId,
				this.targetModel.additionalProcedureIds.length,
				this.targetModel.id,
				true
			));
		}
		...
		if (this.targetModel.primaryProcedureIds && this.targetModel.primaryProcedureIds.length > 0 && this.primaryProcedureMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsSpecialReferralModel,
				this.primaryProcedureOppositeRelation,
				this.primaryProcedureCollectionId,
				this.targetModel.primaryProcedureIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsProcedureModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsProcedureModel {
		let updatedModel: BpjsProcedureModel = new BpjsProcedureModel(model);

		updatedModel.additionalProcedure = [];
		if (this.additionalProcedureMultiCrudActive) {
			if (event.additionalEntities.additionalProcedure) {
				event.additionalEntities.additionalProcedure.forEach(entity => {
					updatedModel.additionalProcedure.push(entity);
				});
			}

			if (!updatedModel.additionalProcedure.length) {
				updatedModel.additionalProcedureIds = [];
			}
		}
		...
		updatedModel.primaryProcedure = [];
		if (this.primaryProcedureMultiCrudActive) {
			if (event.additionalEntities.primaryProcedure) {
				event.additionalEntities.primaryProcedure.forEach(entity => {
					updatedModel.primaryProcedure.push(entity);
				});
			}

			if (!updatedModel.primaryProcedure.length) {
				updatedModel.primaryProcedureIds = [];
			}
		}
		return updatedModel;
	}
