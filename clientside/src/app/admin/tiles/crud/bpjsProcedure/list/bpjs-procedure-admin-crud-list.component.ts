...
import {BpjsProcedureModelState} from 'src/app/models/bpjsProcedure/bpjs_procedure.model.state';
...
@Component({
	selector: 'cb-bpjs-procedure-admin-crud-list',
	templateUrl: './bpjs-procedure-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-procedure-admin-crud-list.component.scss',
...
export class AdminBpjsProcedureCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsProcedureModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsProcedureModel.searchFields,
	...
	/**
	 * List of BPJS Procedures that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsProcedures: Observable<BpjsProcedureModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsProceduresCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsProceduresCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsProceduresCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsProcedureAction(
				modelAction.BpjsProcedureModelActionTypes.INITIALISE_BPJS_PROCEDURE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsProcedureAction(
				modelAction.BpjsProcedureModelActionTypes.FETCH_BPJS_PROCEDURE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsProcedures = this.store.select(getBpjsProcedureCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsProcedureCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsProceduresCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsProcedureModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsProcedureModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsProcedureModelActionTypes.EXPORT_ALL_BPJS_PROCEDURE;
		} else {
			actionType = modelAction.BpjsProcedureModelActionTypes.EXPORT_BPJS_PROCEDURE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsProcedureAction(
				modelAction.BpjsProcedureModelActionTypes.IMPORT_BPJS_PROCEDURES,
				{
					file: file,
				},
				[new modelAction.BpjsProcedureAction(
			...
			window.open(environment.API_URL + '/api/bpjsprocedure/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsProcedureModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Procedure entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Procedure entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsProcedureModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsProcedureModelActionTypes.DELETE_BPJS_PROCEDURE;
						} else if (usingSelected) {
							actionType = modelAction.BpjsProcedureModelActionTypes.DELETE_ALL_BPJS_PROCEDURE;
						} else {
							actionType = modelAction.BpjsProcedureModelActionTypes.DELETE_BPJS_PROCEDURE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsProcedureAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PROCEDURE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PROCEDURE and DELETE_BPJS_PROCEDURE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsProcedureModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsProceduresCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
