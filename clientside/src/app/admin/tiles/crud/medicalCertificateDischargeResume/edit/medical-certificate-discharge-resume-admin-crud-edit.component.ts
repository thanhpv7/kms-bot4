...
import * as modelAction from 'src/app/models/medicalCertificateDischargeResume/medical_certificate_discharge_resume.model.action';
import {MedicalCertificateDischargeResumeModelState, MedicalCertificateDischargeResumeModelAudit} from 'src/app/models/medicalCertificateDischargeResume/medical_certificate_discharge_resume.model.state';
...
import {
	getMedicalCertificateDischargeResumeModelAuditsByEntityId,
	getMedicalCertificateDischargeResumeModelWithId,
	getCountMedicalCertificateDischargeResumeModels,
...
import {CrudTileMode} from '../medical-certificate-discharge-resume-admin-crud.component';
...
@Component({
	selector: 'cb-medical-certificate-discharge-resume-admin-crud-edit',
	templateUrl: './medical-certificate-discharge-resume-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-certificate-discharge-resume-admin-crud-edit.component.scss',
...
export class AdminMedicalCertificateDischargeResumeCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '6ee0ad9b-ed5b-4fc6-8d62-318f4575a1a7';
	 ...
	 */
	medicalCertificateDischargeResumeAudits$: Observable<MedicalCertificateDischargeResumeModelAudit[]>;
	...
	registrationOppositeRelation = 'medicalCertificateDischargeResume';
	...
	dischargeICD10Relations: ModelRelation[] = this.dischargeICD10MultiCrudActive ? DiagnosesAndProceduresModel.getRelations()
		.map(relation => {
	...
	dischargeICD9CMRelations: ModelRelation[] = this.dischargeICD9CMMultiCrudActive ? DiagnosesAndProceduresModel.getRelations()
		.map(relation => {
	...
	medicalStaffMultiCrudActive: boolean = false;
	medicalStaffDisplayName = 'Medical Staff';
	...
	medicalStaffOppositeRelation = 'medicalCertificateDischargeResumes';
	medicalStaffRelations: ModelRelation[] = this.medicalStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.medicalStaffOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalCertificateDischargeResumeModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalCertificateDischargeResumeModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalCertificateDischargeResumeModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'medicalStaff' && this.medicalStaffMultiCrudActive) {
	 ...
	 */
	targetModel: MedicalCertificateDischargeResumeModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalCertificateDischargeResumeModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalCertificateDischargeResumeModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalCertificateDischargeResumeModel();
		...
		if (this.targetModelId) {
			this.medicalCertificateDischargeResumeAudits$ = this.store.select(getMedicalCertificateDischargeResumeModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalCertificateDischargeResumeModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
			modelAction.MedicalCertificateDischargeResumeModelActionTypes.FETCH_MEDICAL_CERTIFICATE_DISCHARGE_RESUME,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateDischargeResumeModelWithId, this.targetModelId)
			...
			model = new MedicalCertificateDischargeResumeModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-certificate-discharge-resume');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
			modelAction.MedicalCertificateDischargeResumeModelActionTypes.FETCH_MEDICAL_CERTIFICATE_DISCHARGE_RESUME_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalCertificateDischargeResumeModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.registrationMultiCrudActive || this.dischargeICD10MultiCrudActive || this.dischargeICD9CMMultiCrudActive || this.medicalStaffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalCertificateDischargeResumeAction(
						modelAction.MedicalCertificateDischargeResumeModelActionTypes.COUNT_MEDICAL_CERTIFICATE_DISCHARGE_RESUMES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
				modelAction.MedicalCertificateDischargeResumeModelActionTypes.CREATE_MEDICAL_CERTIFICATE_DISCHARGE_RESUME,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
				modelAction.MedicalCertificateDischargeResumeModelActionTypes.UPDATE_MEDICAL_CERTIFICATE_DISCHARGE_RESUME,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalCertificateDischargeResumeModel = new MedicalCertificateDischargeResumeModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalCertificateDischargeResumeModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalCertificateDischargeResumeModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.dischargeICD10DisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.dischargeICD10Relations, this.isViewOnly),
			...
			if (event.entityName === this.dischargeICD9CMDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.dischargeICD9CMRelations, this.isViewOnly),
					new DiagnosesAndProceduresModel(),
					DiagnosesAndProceduresModel,
			...
			if (event.entityName === this.medicalStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.medicalStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.medicalStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.medicalStaffRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalCertificateDischargeResumeModel.getProps();
		this.modelRelations = MedicalCertificateDischargeResumeModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'medicalStaff';
		}).hideElement = this.medicalStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'medicalStaff').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.dischargeICD10MultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.dischargeICD10DisplayName, many: false});
		}

		if (this.dischargeICD9CMMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.dischargeICD9CMDisplayName, many: false});
		}

		if (this.medicalStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.medicalStaffDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.dischargeICD10 && this.targetModel.dischargeICD10.id && this.dischargeICD10MultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				DiagnosesAndProceduresModel,
				this.dischargeICD10OppositeRelation,
				this.collectionId,
				1,
				this.targetModel.dischargeICD10.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getDiagnosesAndProceduresModelWithId, this.targetModel.dischargeICD10Id).subscribe(dischargeICD10 => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.dischargeICD10Relations, this.isViewOnly),
					dischargeICD10,
					DiagnosesAndProceduresModel,
		...
		if (this.targetModel.dischargeICD9CM && this.targetModel.dischargeICD9CM.id && this.dischargeICD9CMMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				DiagnosesAndProceduresModel,
				this.dischargeICD9CMOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.dischargeICD9CM.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getDiagnosesAndProceduresModelWithId, this.targetModel.dischargeICD9CMId).subscribe(dischargeICD9CM => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosesAndProceduresModel.getProps(), this.dischargeICD9CMRelations, this.isViewOnly),
					dischargeICD9CM,
					DiagnosesAndProceduresModel,
		...
		if (this.targetModel.medicalStaff && this.targetModel.medicalStaff.id && this.medicalStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.medicalStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.medicalStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.medicalStaffId).subscribe(medicalStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.medicalStaffRelations, this.isViewOnly),
					medicalStaff,
					StaffModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalCertificateDischargeResumeModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalCertificateDischargeResumeModel {
		let updatedModel: MedicalCertificateDischargeResumeModel = new MedicalCertificateDischargeResumeModel(model);
		...
		if (this.dischargeICD10MultiCrudActive) {
			updatedModel.dischargeICD10 = event.additionalEntities.dischargeICD10 ? event.additionalEntities.dischargeICD10 : undefined;

			if (!updatedModel.dischargeICD10 && event.payload) {
				event.payload.dischargeICD10Id = null;
			}
		} else if (event.payload?.dischargeICD10Id !== undefined) {
			event.payload.dischargeICD10 = null;
			updatedModel.dischargeICD10 = null;
		}

		if (this.dischargeICD9CMMultiCrudActive) {
			updatedModel.dischargeICD9CM = event.additionalEntities.dischargeICD9CM ? event.additionalEntities.dischargeICD9CM : undefined;

			if (!updatedModel.dischargeICD9CM && event.payload) {
				event.payload.dischargeICD9CMId = null;
			}
		} else if (event.payload?.dischargeICD9CMId !== undefined) {
			event.payload.dischargeICD9CM = null;
			updatedModel.dischargeICD9CM = null;
		}

		if (this.medicalStaffMultiCrudActive) {
			updatedModel.medicalStaff = event.additionalEntities.medicalStaff ? event.additionalEntities.medicalStaff : undefined;

			if (!updatedModel.medicalStaff && event.payload) {
				event.payload.medicalStaffId = null;
			}
		} else if (event.payload?.medicalStaffId !== undefined) {
			event.payload.medicalStaff = null;
			updatedModel.medicalStaff = null;
		}
