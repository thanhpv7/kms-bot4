...
import {MedicalCertificateDischargeResumeModelState} from 'src/app/models/medicalCertificateDischargeResume/medical_certificate_discharge_resume.model.state';
...
import {
	getMedicalCertificateDischargeResumeCollectionModels,
	getMedicalCertificateDischargeResumeCollectionCount
...
@Component({
	selector: 'cb-medical-certificate-discharge-resume-admin-crud-list',
	templateUrl: './medical-certificate-discharge-resume-admin-crud-list.component.html',
	styleUrls: [
		'./medical-certificate-discharge-resume-admin-crud-list.component.scss',
...
export class AdminMedicalCertificateDischargeResumeCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalCertificateDischargeResumeModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalCertificateDischargeResumeModel.searchFields,
	...
	/**
	 * List of Medical Certificate  Discharge Resumes that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalCertificateDischargeResumes: Observable<MedicalCertificateDischargeResumeModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalCertificateDischargeResumesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalCertificateDischargeResumesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalCertificateDischargeResumesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
				modelAction.MedicalCertificateDischargeResumeModelActionTypes.INITIALISE_MEDICAL_CERTIFICATE_DISCHARGE_RESUME_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
				modelAction.MedicalCertificateDischargeResumeModelActionTypes.FETCH_MEDICAL_CERTIFICATE_DISCHARGE_RESUME_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalCertificateDischargeResumes = this.store.select(getMedicalCertificateDischargeResumeCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalCertificateDischargeResumeCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalCertificateDischargeResumesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalCertificateDischargeResumeModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalCertificateDischargeResumeModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalCertificateDischargeResumeModelActionTypes.EXPORT_ALL_MEDICAL_CERTIFICATE_DISCHARGE_RESUME;
		} else {
			actionType = modelAction.MedicalCertificateDischargeResumeModelActionTypes.EXPORT_MEDICAL_CERTIFICATE_DISCHARGE_RESUME_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
				modelAction.MedicalCertificateDischargeResumeModelActionTypes.IMPORT_MEDICAL_CERTIFICATE_DISCHARGE_RESUMES,
				{
					file: file,
				},
				[new modelAction.MedicalCertificateDischargeResumeAction(
			...
			window.open(environment.API_URL + '/api/medicalcertificatedischargeresume/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalCertificateDischargeResumeModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate  Discharge Resume entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Certificate  Discharge Resume entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalCertificateDischargeResumeModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalCertificateDischargeResumeModelActionTypes.DELETE_MEDICAL_CERTIFICATE_DISCHARGE_RESUME;
						} else if (usingSelected) {
							actionType = modelAction.MedicalCertificateDischargeResumeModelActionTypes.DELETE_ALL_MEDICAL_CERTIFICATE_DISCHARGE_RESUME;
						} else {
							actionType = modelAction.MedicalCertificateDischargeResumeModelActionTypes.DELETE_MEDICAL_CERTIFICATE_DISCHARGE_RESUME_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalCertificateDischargeResumeAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_CERTIFICATE_DISCHARGE_RESUME when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_CERTIFICATE_DISCHARGE_RESUME and DELETE_MEDICAL_CERTIFICATE_DISCHARGE_RESUME_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalCertificateDischargeResumeModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalCertificateDischargeResumesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
