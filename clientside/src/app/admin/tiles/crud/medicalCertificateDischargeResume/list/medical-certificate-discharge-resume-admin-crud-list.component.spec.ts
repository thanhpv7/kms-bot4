...
import {AdminMedicalCertificateDischargeResumeCrudListComponent} from './medical-certificate-discharge-resume-admin-crud-list.component';
...
import {AdminMedicalCertificateDischargeResumeCrudModule} from '../medical-certificate-discharge-resume-admin-crud.module';
import {
	MedicalCertificateDischargeResumeModelState,
	initialState as initialMedicalCertificateDischargeResumeModelState
} from 'src/app/models/medicalCertificateDischargeResume/medical_certificate_discharge_resume.model.state';
 ...
 */
describe('Medical Certificate  Discharge Resume Crud List Tile Component', () => {

	let fixture;
	let medicalCertificateDischargeResumeCrudListComponent: AdminMedicalCertificateDischargeResumeCrudListComponent;

	let store: MockStore<{ model: MedicalCertificateDischargeResumeModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalCertificateDischargeResumeModelState: MedicalCertificateDischargeResumeModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalCertificateDischargeResumeModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6ee0ad9b-ed5b-4fc6-8d62-318f4575a1a7';
	const searchContent = 'text';

	const searchableFieldCount = MedicalCertificateDischargeResumeModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-certificate-discharge-resume-admin-crud',
		urls: ['medical-certificate-discharge-resume-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalCertificateDischargeResumeDataFactory = new MedicalCertificateDischargeResumeDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalCertificateDischargeResumeDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalCertificateDischargeResumeModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalCertificateDischargeResumeModelState = initialMedicalCertificateDischargeResumeModelState;
		store.setState({model: medicalCertificateDischargeResumeModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalCertificateDischargeResumeCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalCertificateDischargeResumeCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalCertificateDischargeResumeCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalCertificateDischargeResumeCrudListComponent);
			medicalCertificateDischargeResumeCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalCertificateDischargeResumeCrudListComponent.medicalCertificateDischargeResumesCount).toBe(defaultData.length);
			...
			expect(medicalCertificateDischargeResumeCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalCertificateDischargeResumeCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalCertificateDischargeResumeDataFactory.createAll(filteredDataSize);
			...
			expect(medicalCertificateDischargeResumeCrudListComponent.medicalCertificateDischargeResumesCount).toBe(filteredDataSize);
			expect(medicalCertificateDischargeResumeCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalCertificateDischargeResumeCrudListComponent.pageIndex).toBe(0);
			expect(medicalCertificateDischargeResumeCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalCertificateDischargeResumeCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalCertificateDischargeResumeCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalCertificateDischargeResumeCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
