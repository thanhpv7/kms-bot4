...
import {PatientDetailModelState} from 'src/app/models/patientDetail/patient_detail.model.state';
...
import {
	...
	getPatientDetailCollectionCount
...
@Component({
	selector: 'cb-patient-detail-admin-crud-list',
	templateUrl: './patient-detail-admin-crud-list.component.html',
	styleUrls: [
		'./patient-detail-admin-crud-list.component.scss',
...
export class AdminPatientDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientDetailModel.searchFields,
	...
	/**
	 * List of Patient Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientDetails: Observable<PatientDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientDetailAction(
				modelAction.PatientDetailModelActionTypes.INITIALISE_PATIENT_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientDetailAction(
				modelAction.PatientDetailModelActionTypes.FETCH_PATIENT_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientDetails = this.store.select(getPatientDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.patientDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientDetailModelActionTypes.EXPORT_ALL_PATIENT_DETAIL;
		} else {
			actionType = modelAction.PatientDetailModelActionTypes.EXPORT_PATIENT_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientDetailAction(
				modelAction.PatientDetailModelActionTypes.IMPORT_PATIENT_DETAILS,
				{
					file: file,
				},
				[new modelAction.PatientDetailAction(
			...
			window.open(environment.API_URL + '/api/patientdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientDetailModelActionTypes.DELETE_PATIENT_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.PatientDetailModelActionTypes.DELETE_ALL_PATIENT_DETAIL;
						} else {
							actionType = modelAction.PatientDetailModelActionTypes.DELETE_PATIENT_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_DETAIL and DELETE_PATIENT_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
