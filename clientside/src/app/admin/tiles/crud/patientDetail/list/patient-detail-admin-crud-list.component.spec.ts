...
import {AdminPatientDetailCrudListComponent} from './patient-detail-admin-crud-list.component';
...
import {AdminPatientDetailCrudModule} from '../patient-detail-admin-crud.module';
import {
 ...
 */
describe('Patient Detail Crud List Tile Component', () => {

	let fixture;
	let patientDetailCrudListComponent: AdminPatientDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '9cfb2d84-c070-49bb-91fa-438d227cce1d';
	const searchContent = 'text';

	const searchableFieldCount = PatientDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-detail-admin-crud',
		urls: ['patient-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientDetailDataFactory = new PatientDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPatientDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPatientDetailCrudListComponent);
			patientDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientDetailCrudListComponent.patientDetailsCount).toBe(defaultData.length);
			...
			expect(patientDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientDetailDataFactory.createAll(filteredDataSize);
			...
			expect(patientDetailCrudListComponent.patientDetailsCount).toBe(filteredDataSize);
			expect(patientDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientDetailCrudListComponent.pageIndex).toBe(0);
			expect(patientDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
