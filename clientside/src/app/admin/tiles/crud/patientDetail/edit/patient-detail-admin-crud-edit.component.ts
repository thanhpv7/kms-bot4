...
import * as modelAction from 'src/app/models/patientDetail/patient_detail.model.action';
import {PatientDetailModelState, PatientDetailModelAudit} from 'src/app/models/patientDetail/patient_detail.model.state';
import {PatientDetailModel} from 'src/app/models/patientDetail/patient_detail.model';
import {
	getPatientDetailModelAuditsByEntityId,
...
import {CrudTileMode} from '../patient-detail-admin-crud.component';
...
@Component({
	selector: 'cb-patient-detail-admin-crud-edit',
	templateUrl: './patient-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-detail-admin-crud-edit.component.scss',
...
export class AdminPatientDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '9cfb2d84-c070-49bb-91fa-438d227cce1d';
	 ...
	 */
	patientDetailAudits$: Observable<PatientDetailModelAudit[]>;
	...
	patientGeneralInformationOppositeRelation = 'patientDetail';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientDetailModel();
		...
		if (this.targetModelId) {
			this.patientDetailAudits$ = this.store.select(getPatientDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientDetailAction(
			modelAction.PatientDetailModelActionTypes.FETCH_PATIENT_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientDetailModelWithId, this.targetModelId)
			...
			model = new PatientDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientDetailAction(
			modelAction.PatientDetailModelActionTypes.FETCH_PATIENT_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientDetailModel> = {
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientDetailAction(
						modelAction.PatientDetailModelActionTypes.COUNT_PATIENT_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientDetailAction(
				modelAction.PatientDetailModelActionTypes.CREATE_PATIENT_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientDetailAction(
				modelAction.PatientDetailModelActionTypes.UPDATE_PATIENT_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientDetailModel = new PatientDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientDetailModel.getProps();
		this.modelRelations = PatientDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientDetailModel {
		let updatedModel: PatientDetailModel = new PatientDetailModel(model);
