...
import {InventoryAdjustmentModelState} from 'src/app/models/inventoryAdjustment/inventory_adjustment.model.state';
...
import {
	...
	getInventoryAdjustmentCollectionCount
...
@Component({
	selector: 'cb-inventory-adjustment-admin-crud-list',
	templateUrl: './inventory-adjustment-admin-crud-list.component.html',
	styleUrls: [
		'./inventory-adjustment-admin-crud-list.component.scss',
...
export class AdminInventoryAdjustmentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InventoryAdjustmentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InventoryAdjustmentModel.searchFields,
	...
	/**
	 * List of Inventory Adjustments that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	inventoryAdjustments: Observable<InventoryAdjustmentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	inventoryAdjustmentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.inventoryAdjustmentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.inventoryAdjustmentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryAdjustmentAction(
				modelAction.InventoryAdjustmentModelActionTypes.INITIALISE_INVENTORY_ADJUSTMENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryAdjustmentAction(
				modelAction.InventoryAdjustmentModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.inventoryAdjustments = this.store.select(getInventoryAdjustmentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInventoryAdjustmentCollectionCount, this.collectionId).subscribe(
				(count) => this.inventoryAdjustmentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InventoryAdjustmentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InventoryAdjustmentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InventoryAdjustmentModelActionTypes.EXPORT_ALL_INVENTORY_ADJUSTMENT;
		} else {
			actionType = modelAction.InventoryAdjustmentModelActionTypes.EXPORT_INVENTORY_ADJUSTMENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InventoryAdjustmentAction(
				modelAction.InventoryAdjustmentModelActionTypes.IMPORT_INVENTORY_ADJUSTMENTS,
				{
					file: file,
				},
				[new modelAction.InventoryAdjustmentAction(
			...
			window.open(environment.API_URL + '/api/inventoryadjustment/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InventoryAdjustmentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Inventory Adjustment entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Inventory Adjustment entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InventoryAdjustmentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InventoryAdjustmentModelActionTypes.DELETE_INVENTORY_ADJUSTMENT;
						} else if (usingSelected) {
							actionType = modelAction.InventoryAdjustmentModelActionTypes.DELETE_ALL_INVENTORY_ADJUSTMENT;
						} else {
							actionType = modelAction.InventoryAdjustmentModelActionTypes.DELETE_INVENTORY_ADJUSTMENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InventoryAdjustmentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVENTORY_ADJUSTMENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVENTORY_ADJUSTMENT and DELETE_INVENTORY_ADJUSTMENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InventoryAdjustmentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.inventoryAdjustmentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
