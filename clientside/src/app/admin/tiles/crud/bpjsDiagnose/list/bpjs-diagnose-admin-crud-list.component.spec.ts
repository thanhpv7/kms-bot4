...
import {AdminBpjsDiagnoseCrudListComponent} from './bpjs-diagnose-admin-crud-list.component';
...
import {AdminBpjsDiagnoseCrudModule} from '../bpjs-diagnose-admin-crud.module';
import {
 ...
 */
describe('BPJS Diagnose Crud List Tile Component', () => {

	let fixture;
	let bpjsDiagnoseCrudListComponent: AdminBpjsDiagnoseCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsDiagnoseModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '550a356f-2f4a-4faf-84d1-ce1145b01089';
	const searchContent = 'text';

	const searchableFieldCount = BpjsDiagnoseModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-diagnose-admin-crud',
		urls: ['bpjs-diagnose-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsDiagnoseDataFactory = new BpjsDiagnoseDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsDiagnoseDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsDiagnoseCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsDiagnoseCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsDiagnoseCrudListComponent);
			bpjsDiagnoseCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsDiagnoseCrudListComponent.bpjsDiagnosesCount).toBe(defaultData.length);
			...
			expect(bpjsDiagnoseCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsDiagnoseCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsDiagnoseDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsDiagnoseCrudListComponent.bpjsDiagnosesCount).toBe(filteredDataSize);
			expect(bpjsDiagnoseCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsDiagnoseCrudListComponent.pageIndex).toBe(0);
			expect(bpjsDiagnoseCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsDiagnoseCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsDiagnoseCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsDiagnoseCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
