...
import {BpjsDiagnoseModelState} from 'src/app/models/bpjsDiagnose/bpjs_diagnose.model.state';
...
import {
	...
	getBpjsDiagnoseCollectionCount
...
import {CrudTileMode} from '../bpjs-diagnose-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-diagnose-admin-crud-list',
	templateUrl: './bpjs-diagnose-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-diagnose-admin-crud-list.component.scss',
...
export class AdminBpjsDiagnoseCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsDiagnoseModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsDiagnoseModel.searchFields,
	...
	/**
	 * List of BPJS Diagnoses that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsDiagnoses: Observable<BpjsDiagnoseModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsDiagnosesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsDiagnosesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsDiagnosesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDiagnoseAction(
				modelAction.BpjsDiagnoseModelActionTypes.INITIALISE_BPJS_DIAGNOSE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsDiagnoseAction(
				modelAction.BpjsDiagnoseModelActionTypes.FETCH_BPJS_DIAGNOSE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsDiagnoses = this.store.select(getBpjsDiagnoseCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsDiagnoseCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsDiagnosesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsDiagnoseModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsDiagnoseModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsDiagnoseModelActionTypes.EXPORT_ALL_BPJS_DIAGNOSE;
		} else {
			actionType = modelAction.BpjsDiagnoseModelActionTypes.EXPORT_BPJS_DIAGNOSE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsDiagnoseAction(
				modelAction.BpjsDiagnoseModelActionTypes.IMPORT_BPJS_DIAGNOSES,
				{
					file: file,
				},
				[new modelAction.BpjsDiagnoseAction(
			...
			window.open(environment.API_URL + '/api/bpjsdiagnose/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsDiagnoseModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Diagnose entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Diagnose entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsDiagnoseModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsDiagnoseModelActionTypes.DELETE_BPJS_DIAGNOSE;
						} else if (usingSelected) {
							actionType = modelAction.BpjsDiagnoseModelActionTypes.DELETE_ALL_BPJS_DIAGNOSE;
						} else {
							actionType = modelAction.BpjsDiagnoseModelActionTypes.DELETE_BPJS_DIAGNOSE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsDiagnoseAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_DIAGNOSE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_DIAGNOSE and DELETE_BPJS_DIAGNOSE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsDiagnoseModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsDiagnosesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
