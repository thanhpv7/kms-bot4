...
import {BpjsSEPSubmissionModelState, BpjsSEPSubmissionModelAudit} from 'src/app/models/bpjsSEPSubmission/bpjs_sep_submission.model.state';
import {BpjsSEPSubmissionModel} from 'src/app/models/bpjsSEPSubmission/bpjs_sep_submission.model';
import {
	getBpjsSEPSubmissionModelAuditsByEntityId,
	getBpjsSEPSubmissionModelWithId,
	getCountBpjsSEPSubmissionModels,
...
@Component({
	selector: 'cb-bpjs-sep-submission-admin-crud-edit',
	templateUrl: './bpjs-sep-submission-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-sep-submission-admin-crud-edit.component.scss',
...
export class AdminBpjsSEPSubmissionCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsSEPSubmissionAudits$: Observable<BpjsSEPSubmissionModelAudit[]>;
	...
	patientPaymentBPJSOppositeRelation = 'bpjsSEPSubmissions';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsSEPSubmissionModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsSEPSubmissionModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsSEPSubmissionModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsSEPSubmissionModel();
		...
		if (this.targetModelId) {
			this.bpjsSEPSubmissionAudits$ = this.store.select(getBpjsSEPSubmissionModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsSEPSubmissionModel> = {
		...
		this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
			modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_BPJS_SEP_SUBMISSION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPSubmissionModelWithId, this.targetModelId)
			...
			model = new BpjsSEPSubmissionModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
			modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_BPJS_SEP_SUBMISSION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
				modelAction.BpjsSEPSubmissionModelActionTypes.CREATE_BPJS_SEP_SUBMISSION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
				modelAction.BpjsSEPSubmissionModelActionTypes.UPDATE_BPJS_SEP_SUBMISSION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsSEPSubmissionModel = new BpjsSEPSubmissionModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsSEPSubmissionModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsSEPSubmissionModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsSEPSubmissionModel.getProps();
		this.modelRelations = BpjsSEPSubmissionModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsSEPSubmissionModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsSEPSubmissionModel {
		let updatedModel: BpjsSEPSubmissionModel = new BpjsSEPSubmissionModel(model);
