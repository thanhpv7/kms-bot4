...
import {AdminBpjsSEPSubmissionCrudListComponent} from './bpjs-sep-submission-admin-crud-list.component';
...
import {AdminBpjsSEPSubmissionCrudModule} from '../bpjs-sep-submission-admin-crud.module';
import {
	BpjsSEPSubmissionModelState,
	initialState as initialBpjsSEPSubmissionModelState
} from 'src/app/models/bpjsSEPSubmission/bpjs_sep_submission.model.state';
 ...
 */
describe('BPJS SEP Submission Crud List Tile Component', () => {

	let fixture;
	let bpjsSEPSubmissionCrudListComponent: AdminBpjsSEPSubmissionCrudListComponent;

	let store: MockStore<{ model: BpjsSEPSubmissionModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsSEPSubmissionModelState: BpjsSEPSubmissionModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsSEPSubmissionModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd7a73d12-1869-4824-9159-bf27b2223f3c';
	const searchContent = 'text';

	const searchableFieldCount = BpjsSEPSubmissionModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-sep-submission-admin-crud',
		urls: ['bpjs-sep-submission-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsSEPSubmissionDataFactory = new BpjsSEPSubmissionDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsSEPSubmissionDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsSEPSubmissionModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsSEPSubmissionModelState = initialBpjsSEPSubmissionModelState;
		store.setState({model: bpjsSEPSubmissionModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsSEPSubmissionCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsSEPSubmissionCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsSEPSubmissionCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsSEPSubmissionCrudListComponent);
			bpjsSEPSubmissionCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsSEPSubmissionCrudListComponent.bpjsSEPSubmissionsCount).toBe(defaultData.length);
			...
			expect(bpjsSEPSubmissionCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsSEPSubmissionCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsSEPSubmissionDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsSEPSubmissionCrudListComponent.bpjsSEPSubmissionsCount).toBe(filteredDataSize);
			expect(bpjsSEPSubmissionCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsSEPSubmissionCrudListComponent.pageIndex).toBe(0);
			expect(bpjsSEPSubmissionCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsSEPSubmissionCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsSEPSubmissionCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsSEPSubmissionCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
