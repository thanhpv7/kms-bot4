...
import * as modelAction from 'src/app/models/bpjsSEPSubmission/bpjs_sep_submission.model.action';
import {BpjsSEPSubmissionModelState} from 'src/app/models/bpjsSEPSubmission/bpjs_sep_submission.model.state';
...
import {
	getBpjsSEPSubmissionCollectionModels,
	getBpjsSEPSubmissionCollectionCount
...
import {CrudTileMode} from '../bpjs-sep-submission-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-sep-submission-admin-crud-list',
	templateUrl: './bpjs-sep-submission-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-sep-submission-admin-crud-list.component.scss',
...
export class AdminBpjsSEPSubmissionCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsSEPSubmissionModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsSEPSubmissionModel.searchFields,
	...
	/**
	 * List of BPJS SEP Submissions that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsSEPSubmissions: Observable<BpjsSEPSubmissionModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsSEPSubmissionsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsSEPSubmissionModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsSEPSubmissionModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'd7a73d12-1869-4824-9159-bf27b2223f3c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsSEPSubmissionsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsSEPSubmissionsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsSEPSubmissionModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
				modelAction.BpjsSEPSubmissionModelActionTypes.INITIALISE_BPJS_SEP_SUBMISSION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
				modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_BPJS_SEP_SUBMISSION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsSEPSubmissionAction(
						modelAction.BpjsSEPSubmissionModelActionTypes.COUNT_BPJS_SEP_SUBMISSIONS,
						null
					)
				]
			))
		...
		this.bpjsSEPSubmissions = this.store.select(getBpjsSEPSubmissionCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsSEPSubmissionCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsSEPSubmissionsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-sep-submission');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsSEPSubmissionModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsSEPSubmissionModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsSEPSubmissionModelActionTypes.EXPORT_ALL_BPJS_SEP_SUBMISSION;
		} else {
			actionType = modelAction.BpjsSEPSubmissionModelActionTypes.EXPORT_BPJS_SEP_SUBMISSION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
				modelAction.BpjsSEPSubmissionModelActionTypes.IMPORT_BPJS_SEP_SUBMISSIONS,
				{
					file: file,
				},
				[new modelAction.BpjsSEPSubmissionAction(
			...
			window.open(environment.API_URL + '/api/bpjssepsubmission/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsSEPSubmissionModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP Submission entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS SEP Submission entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsSEPSubmissionModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsSEPSubmissionModelActionTypes.DELETE_BPJS_SEP_SUBMISSION;
						} else if (usingSelected) {
							actionType = modelAction.BpjsSEPSubmissionModelActionTypes.DELETE_ALL_BPJS_SEP_SUBMISSION;
						} else {
							actionType = modelAction.BpjsSEPSubmissionModelActionTypes.DELETE_BPJS_SEP_SUBMISSION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsSEPSubmissionAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SEP_SUBMISSION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SEP_SUBMISSION and DELETE_BPJS_SEP_SUBMISSION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsSEPSubmissionModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsSEPSubmissionModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsSEPSubmissionsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
