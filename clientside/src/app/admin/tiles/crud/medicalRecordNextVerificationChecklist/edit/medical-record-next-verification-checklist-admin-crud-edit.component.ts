...
import {MedicalRecordNextVerificationChecklistModelState, MedicalRecordNextVerificationChecklistModelAudit} from 'src/app/models/medicalRecordNextVerificationChecklist/medical_record_next_verification_checklist.model.state';
...
import {
	getMedicalRecordNextVerificationChecklistModelAuditsByEntityId,
	getMedicalRecordNextVerificationChecklistModelWithId,
	getCountMedicalRecordNextVerificationChecklistModels,
...
@Component({
	selector: 'cb-medical-record-next-verification-checklist-admin-crud-edit',
	templateUrl: './medical-record-next-verification-checklist-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-record-next-verification-checklist-admin-crud-edit.component.scss',
...
export class AdminMedicalRecordNextVerificationChecklistCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	medicalRecordNextVerificationChecklistAudits$: Observable<MedicalRecordNextVerificationChecklistModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'medicalRecordNextVerificationChecklist';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalRecordNextVerificationChecklistModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalRecordNextVerificationChecklistModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalRecordNextVerificationChecklistModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalRecordNextVerificationChecklistModel();
		...
		if (this.targetModelId) {
			this.medicalRecordNextVerificationChecklistAudits$ = this.store.select(getMedicalRecordNextVerificationChecklistModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalRecordNextVerificationChecklistModel> = {
		...
		this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
			modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.FETCH_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalRecordNextVerificationChecklistModelWithId, this.targetModelId)
			...
			model = new MedicalRecordNextVerificationChecklistModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
			modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.FETCH_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalRecordNextVerificationChecklistModel> = {
				...
				queryParams: {
					persistRelations: this.medicalExaminationRecordMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
				modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.CREATE_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
				modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.UPDATE_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalRecordNextVerificationChecklistModel = new MedicalRecordNextVerificationChecklistModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalRecordNextVerificationChecklistModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalRecordNextVerificationChecklistModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalRecordNextVerificationChecklistModel.getProps();
		this.modelRelations = MedicalRecordNextVerificationChecklistModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalRecordNextVerificationChecklistModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalRecordNextVerificationChecklistModel {
		let updatedModel: MedicalRecordNextVerificationChecklistModel = new MedicalRecordNextVerificationChecklistModel(model);
