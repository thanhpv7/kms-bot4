...
import {AdminMedicalRecordNextVerificationChecklistCrudListComponent} from './medical-record-next-verification-checklist-admin-crud-list.component';
...
import {AdminMedicalRecordNextVerificationChecklistCrudModule} from '../medical-record-next-verification-checklist-admin-crud.module';
import {
	MedicalRecordNextVerificationChecklistModelState,
	initialState as initialMedicalRecordNextVerificationChecklistModelState
} from 'src/app/models/medicalRecordNextVerificationChecklist/medical_record_next_verification_checklist.model.state';
 ...
 */
describe('Medical Record Next Verification Checklist Crud List Tile Component', () => {

	let fixture;
	let medicalRecordNextVerificationChecklistCrudListComponent: AdminMedicalRecordNextVerificationChecklistCrudListComponent;

	let store: MockStore<{ model: MedicalRecordNextVerificationChecklistModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalRecordNextVerificationChecklistModelState: MedicalRecordNextVerificationChecklistModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalRecordNextVerificationChecklistModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a6089ccd-bee0-47ca-844f-27c50e2ccf3e';
	const searchContent = 'text';

	const searchableFieldCount = MedicalRecordNextVerificationChecklistModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-record-next-verification-checklist-admin-crud',
		urls: ['medical-record-next-verification-checklist-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalRecordNextVerificationChecklistDataFactory = new MedicalRecordNextVerificationChecklistDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalRecordNextVerificationChecklistDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalRecordNextVerificationChecklistModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalRecordNextVerificationChecklistModelState = initialMedicalRecordNextVerificationChecklistModelState;
		store.setState({model: medicalRecordNextVerificationChecklistModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalRecordNextVerificationChecklistCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalRecordNextVerificationChecklistCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalRecordNextVerificationChecklistCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalRecordNextVerificationChecklistCrudListComponent);
			medicalRecordNextVerificationChecklistCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalRecordNextVerificationChecklistCrudListComponent.medicalRecordNextVerificationChecklistsCount).toBe(defaultData.length);
			...
			expect(medicalRecordNextVerificationChecklistCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalRecordNextVerificationChecklistCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalRecordNextVerificationChecklistDataFactory.createAll(filteredDataSize);
			...
			expect(medicalRecordNextVerificationChecklistCrudListComponent.medicalRecordNextVerificationChecklistsCount).toBe(filteredDataSize);
			expect(medicalRecordNextVerificationChecklistCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalRecordNextVerificationChecklistCrudListComponent.pageIndex).toBe(0);
			expect(medicalRecordNextVerificationChecklistCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalRecordNextVerificationChecklistCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalRecordNextVerificationChecklistCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalRecordNextVerificationChecklistCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
