...
import * as modelAction from 'src/app/models/medicalRecordNextVerificationChecklist/medical_record_next_verification_checklist.model.action';
import {MedicalRecordNextVerificationChecklistModelState} from 'src/app/models/medicalRecordNextVerificationChecklist/medical_record_next_verification_checklist.model.state';
...
import {
	getMedicalRecordNextVerificationChecklistCollectionModels,
	getMedicalRecordNextVerificationChecklistCollectionCount
...
import {CrudTileMode} from '../medical-record-next-verification-checklist-admin-crud.component';
...
@Component({
	selector: 'cb-medical-record-next-verification-checklist-admin-crud-list',
	templateUrl: './medical-record-next-verification-checklist-admin-crud-list.component.html',
	styleUrls: [
		'./medical-record-next-verification-checklist-admin-crud-list.component.scss',
...
export class AdminMedicalRecordNextVerificationChecklistCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalRecordNextVerificationChecklistModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalRecordNextVerificationChecklistModel.searchFields,
	...
	/**
	 * List of Medical Record Next Verification Checklists that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalRecordNextVerificationChecklists: Observable<MedicalRecordNextVerificationChecklistModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalRecordNextVerificationChecklistsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = MedicalRecordNextVerificationChecklistModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalRecordNextVerificationChecklistModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'a6089ccd-bee0-47ca-844f-27c50e2ccf3e';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalRecordNextVerificationChecklistsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalRecordNextVerificationChecklistsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: MedicalRecordNextVerificationChecklistModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
				modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.INITIALISE_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
				modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.FETCH_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.MedicalRecordNextVerificationChecklistAction(
						modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.COUNT_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLISTS,
						null
					)
				]
			))
		...
		this.medicalRecordNextVerificationChecklists = this.store.select(getMedicalRecordNextVerificationChecklistCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalRecordNextVerificationChecklistCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalRecordNextVerificationChecklistsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-record-next-verification-checklist');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalRecordNextVerificationChecklistModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalRecordNextVerificationChecklistModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.EXPORT_ALL_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST;
		} else {
			actionType = modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.EXPORT_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
				modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.IMPORT_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLISTS,
				{
					file: file,
				},
				[new modelAction.MedicalRecordNextVerificationChecklistAction(
			...
			window.open(environment.API_URL + '/api/medicalrecordnextverificationchecklist/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalRecordNextVerificationChecklistModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Record Next Verification Checklist entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Record Next Verification Checklist entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalRecordNextVerificationChecklistModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.DELETE_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST;
						} else if (usingSelected) {
							actionType = modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.DELETE_ALL_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST;
						} else {
							actionType = modelAction.MedicalRecordNextVerificationChecklistModelActionTypes.DELETE_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalRecordNextVerificationChecklistAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST and DELETE_MEDICAL_RECORD_NEXT_VERIFICATION_CHECKLIST_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<MedicalRecordNextVerificationChecklistModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalRecordNextVerificationChecklistModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalRecordNextVerificationChecklistsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
