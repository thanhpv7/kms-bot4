...
import {SampleCollectionInformationModelState} from 'src/app/models/sampleCollectionInformation/sample_collection_information.model.state';
...
import {
	...
	getSampleCollectionInformationCollectionCount
...
@Component({
	selector: 'cb-sample-collection-information-admin-crud-list',
	templateUrl: './sample-collection-information-admin-crud-list.component.html',
	styleUrls: [
		'./sample-collection-information-admin-crud-list.component.scss',
...
export class AdminSampleCollectionInformationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SampleCollectionInformationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SampleCollectionInformationModel.searchFields,
	...
	/**
	 * List of Sample Collection Informations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	sampleCollectionInformations: Observable<SampleCollectionInformationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	sampleCollectionInformationsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.sampleCollectionInformationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.sampleCollectionInformationsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SampleCollectionInformationAction(
				modelAction.SampleCollectionInformationModelActionTypes.INITIALISE_SAMPLE_COLLECTION_INFORMATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SampleCollectionInformationAction(
				modelAction.SampleCollectionInformationModelActionTypes.FETCH_SAMPLE_COLLECTION_INFORMATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.sampleCollectionInformations = this.store.select(getSampleCollectionInformationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSampleCollectionInformationCollectionCount, this.collectionId).subscribe(
				(count) => this.sampleCollectionInformationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SampleCollectionInformationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SampleCollectionInformationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SampleCollectionInformationModelActionTypes.EXPORT_ALL_SAMPLE_COLLECTION_INFORMATION;
		} else {
			actionType = modelAction.SampleCollectionInformationModelActionTypes.EXPORT_SAMPLE_COLLECTION_INFORMATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SampleCollectionInformationAction(
				modelAction.SampleCollectionInformationModelActionTypes.IMPORT_SAMPLE_COLLECTION_INFORMATIONS,
				{
					file: file,
				},
				[new modelAction.SampleCollectionInformationAction(
			...
			window.open(environment.API_URL + '/api/samplecollectioninformation/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SampleCollectionInformationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Sample Collection Information entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Sample Collection Information entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SampleCollectionInformationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SampleCollectionInformationModelActionTypes.DELETE_SAMPLE_COLLECTION_INFORMATION;
						} else if (usingSelected) {
							actionType = modelAction.SampleCollectionInformationModelActionTypes.DELETE_ALL_SAMPLE_COLLECTION_INFORMATION;
						} else {
							actionType = modelAction.SampleCollectionInformationModelActionTypes.DELETE_SAMPLE_COLLECTION_INFORMATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SampleCollectionInformationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SAMPLE_COLLECTION_INFORMATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SAMPLE_COLLECTION_INFORMATION and DELETE_SAMPLE_COLLECTION_INFORMATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SampleCollectionInformationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.sampleCollectionInformationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
