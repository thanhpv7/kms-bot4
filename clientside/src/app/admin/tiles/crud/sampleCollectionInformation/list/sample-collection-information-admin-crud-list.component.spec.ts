...
import {AdminSampleCollectionInformationCrudListComponent} from './sample-collection-information-admin-crud-list.component';
...
import {AdminSampleCollectionInformationCrudModule} from '../sample-collection-information-admin-crud.module';
import {
 ...
 */
describe('Sample Collection Information Crud List Tile Component', () => {

	let fixture;
	let sampleCollectionInformationCrudListComponent: AdminSampleCollectionInformationCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<SampleCollectionInformationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b708e0b4-ee47-4fa1-be3f-b68ce0b29fa8';
	const searchContent = 'text';

	const searchableFieldCount = SampleCollectionInformationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'sample-collection-information-admin-crud',
		urls: ['sample-collection-information-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const sampleCollectionInformationDataFactory = new SampleCollectionInformationDataFactory();

	const defaultDataSize = 30;
	const defaultData = sampleCollectionInformationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getSampleCollectionInformationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getSampleCollectionInformationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminSampleCollectionInformationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminSampleCollectionInformationCrudListComponent);
			sampleCollectionInformationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(sampleCollectionInformationCrudListComponent.sampleCollectionInformationsCount).toBe(defaultData.length);
			...
			expect(sampleCollectionInformationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			sampleCollectionInformationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = sampleCollectionInformationDataFactory.createAll(filteredDataSize);
			...
			expect(sampleCollectionInformationCrudListComponent.sampleCollectionInformationsCount).toBe(filteredDataSize);
			expect(sampleCollectionInformationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(sampleCollectionInformationCrudListComponent.pageIndex).toBe(0);
			expect(sampleCollectionInformationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			sampleCollectionInformationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		sampleCollectionInformationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		sampleCollectionInformationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
