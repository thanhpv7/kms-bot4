...
import * as modelAction from 'src/app/models/sampleCollectionInformation/sample_collection_information.model.action';
import {SampleCollectionInformationModelState, SampleCollectionInformationModelAudit} from 'src/app/models/sampleCollectionInformation/sample_collection_information.model.state';
...
import {
	getSampleCollectionInformationModelAuditsByEntityId,
...
import {CrudTileMode} from '../sample-collection-information-admin-crud.component';
...
@Component({
	selector: 'cb-sample-collection-information-admin-crud-edit',
	templateUrl: './sample-collection-information-admin-crud-edit.component.html',
	styleUrls: [
		'./sample-collection-information-admin-crud-edit.component.scss',
...
export class AdminSampleCollectionInformationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b708e0b4-ee47-4fa1-be3f-b68ce0b29fa8';
	 ...
	 */
	sampleCollectionInformationAudits$: Observable<SampleCollectionInformationModelAudit[]>;
	...
	diagnosisExaminationRecordMultiCrudActive: boolean = false;
	diagnosisExaminationRecordDisplayName = 'Diagnosis Examination Record';
	...
	diagnosisExaminationRecordOppositeRelation = 'sampleCollectionInformation';
	diagnosisExaminationRecordRelations: ModelRelation[] = this.diagnosisExaminationRecordMultiCrudActive ? DiagnosisExaminationRecordModel.getRelations()
		.map(relation => {
			if (relation.id === this.diagnosisExaminationRecordOppositeRelation) {
	...
	registrationOppositeRelation = 'sampleCollectionInformations';
	...
	sampleCollectionItemsMultiCrudActive: boolean = false;
	sampleCollectionItemsDisplayName = 'Sample Collection Items';
	sampleCollectionItemsCollectionId: string = uuidv4();
	...
	sampleCollectionItemsOppositeRelation = 'sampleCollectionInformation';
	sampleCollectionItemsRelations: ModelRelation[] = this.sampleCollectionItemsMultiCrudActive ? SampleCollectionItemsModel.getRelations()
		.map(relation => {
			if (relation.id === this.sampleCollectionItemsOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = SampleCollectionInformationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = SampleCollectionInformationModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SampleCollectionInformationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'diagnosisExaminationRecord' && this.diagnosisExaminationRecordMultiCrudActive) {
				...
				if (relation.id === 'sampleCollectionItems' && this.sampleCollectionItemsMultiCrudActive) {
	 ...
	 */
	targetModel: SampleCollectionInformationModel;
	...
	constructor(
		private readonly store: Store<{ model: SampleCollectionInformationModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSampleCollectionInformationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new SampleCollectionInformationModel();
		...
		if (this.targetModelId) {
			this.sampleCollectionInformationAudits$ = this.store.select(getSampleCollectionInformationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SampleCollectionInformationModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.SampleCollectionInformationAction(
			modelAction.SampleCollectionInformationModelActionTypes.FETCH_SAMPLE_COLLECTION_INFORMATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSampleCollectionInformationModelWithId, this.targetModelId)
			...
			model = new SampleCollectionInformationModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('sample-collection-information');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SampleCollectionInformationAction(
			modelAction.SampleCollectionInformationModelActionTypes.FETCH_SAMPLE_COLLECTION_INFORMATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<SampleCollectionInformationModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.diagnosisExaminationRecordMultiCrudActive || this.registrationMultiCrudActive || this.sampleCollectionItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.SampleCollectionInformationAction(
						modelAction.SampleCollectionInformationModelActionTypes.COUNT_SAMPLE_COLLECTION_INFORMATIONS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.SampleCollectionInformationAction(
				modelAction.SampleCollectionInformationModelActionTypes.CREATE_SAMPLE_COLLECTION_INFORMATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SampleCollectionInformationAction(
				modelAction.SampleCollectionInformationModelActionTypes.UPDATE_SAMPLE_COLLECTION_INFORMATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SampleCollectionInformationModel = new SampleCollectionInformationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSampleCollectionInformationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SampleCollectionInformationModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.diagnosisExaminationRecordDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosisExaminationRecordModel.getProps(), this.diagnosisExaminationRecordRelations, this.isViewOnly),
					new DiagnosisExaminationRecordModel(),
					DiagnosisExaminationRecordModel,
					this.diagnosisExaminationRecordDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.diagnosisExaminationRecordRelations,
					true
				);
			...
			if (event.entityName === this.sampleCollectionItemsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(SampleCollectionItemsModel.getProps(), this.sampleCollectionItemsRelations, this.isViewOnly),
					new SampleCollectionItemsModel(),
					SampleCollectionItemsModel,
					this.sampleCollectionItemsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.sampleCollectionItemsRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SampleCollectionInformationModel.getProps();
		this.modelRelations = SampleCollectionInformationModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'diagnosisExaminationRecord';
		}).hideElement = this.diagnosisExaminationRecordMultiCrudActive || this.modelRelations.find(rel => rel.id === 'diagnosisExaminationRecord').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'sampleCollectionItems';
		}).hideElement = this.sampleCollectionItemsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'sampleCollectionItems').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.diagnosisExaminationRecordMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.diagnosisExaminationRecordDisplayName, many: false});
		}
		...
		if (this.sampleCollectionItemsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.sampleCollectionItemsDisplayName, many: true});
			this.store.dispatch(new sampleCollectionItemsModelAction.SampleCollectionItemsAction(
				...
				{
					collectionId: this.sampleCollectionItemsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getSampleCollectionItemsCollectionModels, this.sampleCollectionItemsCollectionId).subscribe(models => {
				models.forEach(sampleCollectionItems => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(SampleCollectionItemsModel.getProps(), this.sampleCollectionItemsRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.diagnosisExaminationRecord && this.targetModel.diagnosisExaminationRecord.id && this.diagnosisExaminationRecordMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				DiagnosisExaminationRecordModel,
				this.diagnosisExaminationRecordOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.diagnosisExaminationRecord.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getDiagnosisExaminationRecordModelWithId, this.targetModel.diagnosisExaminationRecordId).subscribe(diagnosisExaminationRecord => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(DiagnosisExaminationRecordModel.getProps(), this.diagnosisExaminationRecordRelations, this.isViewOnly),
		...
		if (this.targetModel.sampleCollectionItemsIds && this.targetModel.sampleCollectionItemsIds.length > 0 && this.sampleCollectionItemsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				SampleCollectionItemsModel,
				this.sampleCollectionItemsOppositeRelation,
				this.sampleCollectionItemsCollectionId,
				this.targetModel.sampleCollectionItemsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: SampleCollectionInformationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SampleCollectionInformationModel {
		let updatedModel: SampleCollectionInformationModel = new SampleCollectionInformationModel(model);

		if (this.diagnosisExaminationRecordMultiCrudActive) {
			updatedModel.diagnosisExaminationRecord = event.additionalEntities.diagnosisExaminationRecord ? event.additionalEntities.diagnosisExaminationRecord : undefined;

			if (!updatedModel.diagnosisExaminationRecord && event.payload) {
				event.payload.diagnosisExaminationRecordId = null;
			}
		} else if (event.payload?.diagnosisExaminationRecordId !== undefined) {
			event.payload.diagnosisExaminationRecord = null;
			updatedModel.diagnosisExaminationRecord = null;
		}
		...
		updatedModel.sampleCollectionItems = [];
		if (this.sampleCollectionItemsMultiCrudActive) {
			if (event.additionalEntities.sampleCollectionItems) {
				event.additionalEntities.sampleCollectionItems.forEach(entity => {
					updatedModel.sampleCollectionItems.push(entity);
				});
			}

			if (!updatedModel.sampleCollectionItems.length) {
				updatedModel.sampleCollectionItemsIds = [];
			}
		}
		return updatedModel;
	}
