...
import * as modelAction from 'src/app/models/pcareReferensiSarana/pcare_referensi_sarana.model.action';
import {PcareReferensiSaranaModelState} from 'src/app/models/pcareReferensiSarana/pcare_referensi_sarana.model.state';
...
import {CrudTileMode} from '../pcare-referensi-sarana-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-referensi-sarana-admin-crud-list',
	templateUrl: './pcare-referensi-sarana-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-referensi-sarana-admin-crud-list.component.scss',
...
export class AdminPcareReferensiSaranaCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareReferensiSaranaModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareReferensiSaranaModel.searchFields,
	...
	/**
	 * List of PCare Referensi Saranas that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareReferensiSaranas: Observable<PcareReferensiSaranaModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareReferensiSaranasCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PcareReferensiSaranaModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareReferensiSaranaModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ce43d6e9-cdbb-437e-888f-321a6c5daea0';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareReferensiSaranasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareReferensiSaranasCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareReferensiSaranaModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
				modelAction.PcareReferensiSaranaModelActionTypes.INITIALISE_PCARE_REFERENSI_SARANA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
				modelAction.PcareReferensiSaranaModelActionTypes.FETCH_PCARE_REFERENSI_SARANA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareReferensiSaranaAction(
						modelAction.PcareReferensiSaranaModelActionTypes.COUNT_PCARE_REFERENSI_SARANAS,
						null
					)
				]
			))
		...
		this.pcareReferensiSaranas = this.store.select(getPcareReferensiSaranaCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareReferensiSaranaCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareReferensiSaranasCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-referensi-sarana');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareReferensiSaranaModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareReferensiSaranaModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareReferensiSaranaModelActionTypes.EXPORT_ALL_PCARE_REFERENSI_SARANA;
		} else {
			actionType = modelAction.PcareReferensiSaranaModelActionTypes.EXPORT_PCARE_REFERENSI_SARANA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
				modelAction.PcareReferensiSaranaModelActionTypes.IMPORT_PCARE_REFERENSI_SARANAS,
				{
					file: file,
				},
				[new modelAction.PcareReferensiSaranaAction(
			...
			window.open(environment.API_URL + '/api/pcarereferensisarana/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareReferensiSaranaModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Referensi Sarana entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Referensi Sarana entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareReferensiSaranaModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareReferensiSaranaModelActionTypes.DELETE_PCARE_REFERENSI_SARANA;
						} else if (usingSelected) {
							actionType = modelAction.PcareReferensiSaranaModelActionTypes.DELETE_ALL_PCARE_REFERENSI_SARANA;
						} else {
							actionType = modelAction.PcareReferensiSaranaModelActionTypes.DELETE_PCARE_REFERENSI_SARANA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_REFERENSI_SARANA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_REFERENSI_SARANA and DELETE_PCARE_REFERENSI_SARANA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareReferensiSaranaModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareReferensiSaranaModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareReferensiSaranasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
