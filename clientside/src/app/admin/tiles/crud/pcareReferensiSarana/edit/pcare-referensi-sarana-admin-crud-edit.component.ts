...
import {PcareReferensiSaranaModelState, PcareReferensiSaranaModelAudit} from 'src/app/models/pcareReferensiSarana/pcare_referensi_sarana.model.state';
import {PcareReferensiSaranaModel} from 'src/app/models/pcareReferensiSarana/pcare_referensi_sarana.model';
import {
	getPcareReferensiSaranaModelAuditsByEntityId,
...
@Component({
	selector: 'cb-pcare-referensi-sarana-admin-crud-edit',
	templateUrl: './pcare-referensi-sarana-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-referensi-sarana-admin-crud-edit.component.scss',
...
export class AdminPcareReferensiSaranaCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareReferensiSaranaAudits$: Observable<PcareReferensiSaranaModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareReferensiSaranaModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PcareReferensiSaranaModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareReferensiSaranaModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareReferensiSaranaModel();
		...
		if (this.targetModelId) {
			this.pcareReferensiSaranaAudits$ = this.store.select(getPcareReferensiSaranaModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareReferensiSaranaModel> = {
		...
		this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
			modelAction.PcareReferensiSaranaModelActionTypes.FETCH_PCARE_REFERENSI_SARANA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareReferensiSaranaModelWithId, this.targetModelId)
			...
			model = new PcareReferensiSaranaModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
			modelAction.PcareReferensiSaranaModelActionTypes.FETCH_PCARE_REFERENSI_SARANA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
				modelAction.PcareReferensiSaranaModelActionTypes.CREATE_PCARE_REFERENSI_SARANA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareReferensiSaranaAction(
				modelAction.PcareReferensiSaranaModelActionTypes.UPDATE_PCARE_REFERENSI_SARANA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareReferensiSaranaModel = new PcareReferensiSaranaModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareReferensiSaranaModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareReferensiSaranaModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareReferensiSaranaModel.getProps();
		this.modelRelations = PcareReferensiSaranaModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareReferensiSaranaModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareReferensiSaranaModel {
		let updatedModel: PcareReferensiSaranaModel = new PcareReferensiSaranaModel(model);
