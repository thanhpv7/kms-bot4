...
import {ObstetricAndGynecologyHistoryModelState, ObstetricAndGynecologyHistoryModelAudit} from 'src/app/models/obstetricAndGynecologyHistory/obstetric_and_gynecology_history.model.state';
...
import {
	getObstetricAndGynecologyHistoryModelAuditsByEntityId,
...
@Component({
	selector: 'cb-obstetric-and-gynecology-history-admin-crud-edit',
	templateUrl: './obstetric-and-gynecology-history-admin-crud-edit.component.html',
	styleUrls: [
		'./obstetric-and-gynecology-history-admin-crud-edit.component.scss',
...
export class AdminObstetricAndGynecologyHistoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	obstetricAndGynecologyHistoryAudits$: Observable<ObstetricAndGynecologyHistoryModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'obstetricAndGynecologyHistory';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ObstetricAndGynecologyHistoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ObstetricAndGynecologyHistoryModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountObstetricAndGynecologyHistoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ObstetricAndGynecologyHistoryModel();
		...
		if (this.targetModelId) {
			this.obstetricAndGynecologyHistoryAudits$ = this.store.select(getObstetricAndGynecologyHistoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ObstetricAndGynecologyHistoryModel> = {
		...
		this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
			modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_OBSTETRIC_AND_GYNECOLOGY_HISTORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getObstetricAndGynecologyHistoryModelWithId, this.targetModelId)
			...
			model = new ObstetricAndGynecologyHistoryModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
			modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_OBSTETRIC_AND_GYNECOLOGY_HISTORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
				modelAction.ObstetricAndGynecologyHistoryModelActionTypes.CREATE_OBSTETRIC_AND_GYNECOLOGY_HISTORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
				modelAction.ObstetricAndGynecologyHistoryModelActionTypes.UPDATE_OBSTETRIC_AND_GYNECOLOGY_HISTORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ObstetricAndGynecologyHistoryModel = new ObstetricAndGynecologyHistoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getObstetricAndGynecologyHistoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ObstetricAndGynecologyHistoryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ObstetricAndGynecologyHistoryModel.getProps();
		this.modelRelations = ObstetricAndGynecologyHistoryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ObstetricAndGynecologyHistoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ObstetricAndGynecologyHistoryModel {
		let updatedModel: ObstetricAndGynecologyHistoryModel = new ObstetricAndGynecologyHistoryModel(model);
