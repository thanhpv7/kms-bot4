...
import * as modelAction from 'src/app/models/obstetricAndGynecologyHistory/obstetric_and_gynecology_history.model.action';
import {ObstetricAndGynecologyHistoryModelState} from 'src/app/models/obstetricAndGynecologyHistory/obstetric_and_gynecology_history.model.state';
...
import {
	...
	getObstetricAndGynecologyHistoryCollectionCount
...
import {CrudTileMode} from '../obstetric-and-gynecology-history-admin-crud.component';
...
@Component({
	selector: 'cb-obstetric-and-gynecology-history-admin-crud-list',
	templateUrl: './obstetric-and-gynecology-history-admin-crud-list.component.html',
	styleUrls: [
		'./obstetric-and-gynecology-history-admin-crud-list.component.scss',
...
export class AdminObstetricAndGynecologyHistoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ObstetricAndGynecologyHistoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ObstetricAndGynecologyHistoryModel.searchFields,
	...
	/**
	 * List of Obstetric And Gynecology Historys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	obstetricAndGynecologyHistorys: Observable<ObstetricAndGynecologyHistoryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	obstetricAndGynecologyHistorysCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = ObstetricAndGynecologyHistoryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ObstetricAndGynecologyHistoryModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '2ae0f863-c0ab-4bc0-940f-81aece2912a0';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.obstetricAndGynecologyHistorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.obstetricAndGynecologyHistorysCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ObstetricAndGynecologyHistoryModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
				modelAction.ObstetricAndGynecologyHistoryModelActionTypes.INITIALISE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
				modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_OBSTETRIC_AND_GYNECOLOGY_HISTORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ObstetricAndGynecologyHistoryAction(
						modelAction.ObstetricAndGynecologyHistoryModelActionTypes.COUNT_OBSTETRIC_AND_GYNECOLOGY_HISTORYS,
						null
					)
				]
			))
		...
		this.obstetricAndGynecologyHistorys = this.store.select(getObstetricAndGynecologyHistoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getObstetricAndGynecologyHistoryCollectionCount, this.collectionId).subscribe(
				(count) => this.obstetricAndGynecologyHistorysCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('obstetric-and-gynecology-history');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ObstetricAndGynecologyHistoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ObstetricAndGynecologyHistoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ObstetricAndGynecologyHistoryModelActionTypes.EXPORT_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY;
		} else {
			actionType = modelAction.ObstetricAndGynecologyHistoryModelActionTypes.EXPORT_OBSTETRIC_AND_GYNECOLOGY_HISTORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
				modelAction.ObstetricAndGynecologyHistoryModelActionTypes.IMPORT_OBSTETRIC_AND_GYNECOLOGY_HISTORYS,
				{
					file: file,
				},
				[new modelAction.ObstetricAndGynecologyHistoryAction(
			...
			window.open(environment.API_URL + '/api/obstetricandgynecologyhistory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ObstetricAndGynecologyHistoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Obstetric And Gynecology History entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Obstetric And Gynecology History entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ObstetricAndGynecologyHistoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ObstetricAndGynecologyHistoryModelActionTypes.DELETE_OBSTETRIC_AND_GYNECOLOGY_HISTORY;
						} else if (usingSelected) {
							actionType = modelAction.ObstetricAndGynecologyHistoryModelActionTypes.DELETE_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY;
						} else {
							actionType = modelAction.ObstetricAndGynecologyHistoryModelActionTypes.DELETE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ObstetricAndGynecologyHistoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_OBSTETRIC_AND_GYNECOLOGY_HISTORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY and DELETE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ObstetricAndGynecologyHistoryModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ObstetricAndGynecologyHistoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.obstetricAndGynecologyHistorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
