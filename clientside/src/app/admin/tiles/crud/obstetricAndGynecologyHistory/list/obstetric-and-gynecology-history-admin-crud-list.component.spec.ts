...
import {AdminObstetricAndGynecologyHistoryCrudListComponent} from './obstetric-and-gynecology-history-admin-crud-list.component';
...
import {AdminObstetricAndGynecologyHistoryCrudModule} from '../obstetric-and-gynecology-history-admin-crud.module';
import {
 ...
 */
describe('Obstetric And Gynecology History Crud List Tile Component', () => {

	let fixture;
	let obstetricAndGynecologyHistoryCrudListComponent: AdminObstetricAndGynecologyHistoryCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ObstetricAndGynecologyHistoryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '2ae0f863-c0ab-4bc0-940f-81aece2912a0';
	const searchContent = 'text';

	const searchableFieldCount = ObstetricAndGynecologyHistoryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'obstetric-and-gynecology-history-admin-crud',
		urls: ['obstetric-and-gynecology-history-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const obstetricAndGynecologyHistoryDataFactory = new ObstetricAndGynecologyHistoryDataFactory();

	const defaultDataSize = 30;
	const defaultData = obstetricAndGynecologyHistoryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getObstetricAndGynecologyHistoryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getObstetricAndGynecologyHistoryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminObstetricAndGynecologyHistoryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminObstetricAndGynecologyHistoryCrudListComponent);
			obstetricAndGynecologyHistoryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(obstetricAndGynecologyHistoryCrudListComponent.obstetricAndGynecologyHistorysCount).toBe(defaultData.length);
			...
			expect(obstetricAndGynecologyHistoryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			obstetricAndGynecologyHistoryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = obstetricAndGynecologyHistoryDataFactory.createAll(filteredDataSize);
			...
			expect(obstetricAndGynecologyHistoryCrudListComponent.obstetricAndGynecologyHistorysCount).toBe(filteredDataSize);
			expect(obstetricAndGynecologyHistoryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(obstetricAndGynecologyHistoryCrudListComponent.pageIndex).toBe(0);
			expect(obstetricAndGynecologyHistoryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			obstetricAndGynecologyHistoryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		obstetricAndGynecologyHistoryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		obstetricAndGynecologyHistoryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
