...
import * as modelAction from 'src/app/models/nhisClaim/nhis_claim.model.action';
import {NhisClaimModelState} from 'src/app/models/nhisClaim/nhis_claim.model.state';
...
import {
	getNhisClaimCollectionModels,
	getNhisClaimCollectionCount
} from 'src/app/models/nhisClaim/nhis_claim.model.selector';
...
import {CrudTileMode} from '../nhis-claim-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-claim-admin-crud-list',
	templateUrl: './nhis-claim-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-claim-admin-crud-list.component.scss',
...
export class AdminNhisClaimCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisClaimModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisClaimModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = NhisClaimModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'cc3431cd-048f-4205-a453-e0b14dafc06e';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisClaimsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisClaimsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisClaimModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisClaimAction(
				modelAction.NhisClaimModelActionTypes.INITIALISE_NHIS_CLAIM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisClaimAction(
				modelAction.NhisClaimModelActionTypes.FETCH_NHIS_CLAIM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisClaimAction(
						modelAction.NhisClaimModelActionTypes.COUNT_NHIS_CLAIMS,
						null
					)
				]
			))
		...
		this.nhisClaims = this.store.select(getNhisClaimCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisClaimCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisClaimsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisClaimModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisClaimModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisClaimModelActionTypes.EXPORT_ALL_NHIS_CLAIM;
		} else {
			actionType = modelAction.NhisClaimModelActionTypes.EXPORT_NHIS_CLAIM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisClaimAction(
				modelAction.NhisClaimModelActionTypes.IMPORT_NHIS_CLAIMS,
				{
					file: file,
				},
				[new modelAction.NhisClaimAction(
			...
			window.open(environment.API_URL + '/api/nhisclaim/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisClaimModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Claim entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Claim entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisClaimModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisClaimModelActionTypes.DELETE_NHIS_CLAIM;
						} else if (usingSelected) {
							actionType = modelAction.NhisClaimModelActionTypes.DELETE_ALL_NHIS_CLAIM;
						} else {
							actionType = modelAction.NhisClaimModelActionTypes.DELETE_NHIS_CLAIM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisClaimAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_CLAIM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_CLAIM and DELETE_NHIS_CLAIM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisClaimModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisClaimModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisClaimsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
