...
import {AdminNhisClaimCrudListComponent} from './nhis-claim-admin-crud-list.component';
import {NhisClaimDataFactory} from 'src/app/lib/utils/factories/nhis-claim-data-factory';
import {NhisClaimModel} from 'src/app/models/nhisClaim/nhis_claim.model';
import {AdminNhisClaimCrudModule} from '../nhis-claim-admin-crud.module';
import {
	NhisClaimModelState,
	initialState as initialNhisClaimModelState
} from 'src/app/models/nhisClaim/nhis_claim.model.state';
 ...
 */
describe('NHIS Claim Crud List Tile Component', () => {

	let fixture;
	let nhisClaimCrudListComponent: AdminNhisClaimCrudListComponent;

	let store: MockStore<{ model: NhisClaimModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisClaimModelState: NhisClaimModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisClaimModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cc3431cd-048f-4205-a453-e0b14dafc06e';
	const searchContent = 'text';

	const searchableFieldCount = NhisClaimModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-claim-admin-crud',
		urls: ['nhis-claim-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisClaimDataFactory = new NhisClaimDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisClaimDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisClaimModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisClaimModelState = initialNhisClaimModelState;
		store.setState({model: nhisClaimModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisClaimCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisClaimCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminNhisClaimCrudListComponent);
			nhisClaimCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisClaimCrudListComponent.nhisClaimsCount).toBe(defaultData.length);
			...
			expect(nhisClaimCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisClaimCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisClaimDataFactory.createAll(filteredDataSize);
			...
			expect(nhisClaimCrudListComponent.nhisClaimsCount).toBe(filteredDataSize);
			expect(nhisClaimCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisClaimCrudListComponent.pageIndex).toBe(0);
			expect(nhisClaimCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisClaimCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisClaimCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisClaimCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
