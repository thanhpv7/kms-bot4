...
import {NhisClaimModelState, NhisClaimModelAudit} from 'src/app/models/nhisClaim/nhis_claim.model.state';
import {NhisClaimModel} from 'src/app/models/nhisClaim/nhis_claim.model';
import {
	getNhisClaimModelAuditsByEntityId,
	getNhisClaimModelWithId,
	getCountNhisClaimModels,
...
@Component({
	selector: 'cb-nhis-claim-admin-crud-edit',
	templateUrl: './nhis-claim-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-claim-admin-crud-edit.component.scss',
...
export class AdminNhisClaimCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisClaimAudits$: Observable<NhisClaimModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisClaimModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisClaimModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisClaimModel();
		...
		if (this.targetModelId) {
			this.nhisClaimAudits$ = this.store.select(getNhisClaimModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisClaimModel> = {
		...
		this.store.dispatch(new modelAction.NhisClaimAction(
			modelAction.NhisClaimModelActionTypes.FETCH_NHIS_CLAIM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisClaimModelWithId, this.targetModelId)
			...
			model = new NhisClaimModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisClaimAction(
			modelAction.NhisClaimModelActionTypes.FETCH_NHIS_CLAIM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisClaimAction(
				modelAction.NhisClaimModelActionTypes.CREATE_NHIS_CLAIM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisClaimAction(
				modelAction.NhisClaimModelActionTypes.UPDATE_NHIS_CLAIM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisClaimModel = new NhisClaimModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisClaimModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisClaimModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisClaimModel.getProps();
		this.modelRelations = NhisClaimModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisClaimModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisClaimModel {
		let updatedModel: NhisClaimModel = new NhisClaimModel(model);
