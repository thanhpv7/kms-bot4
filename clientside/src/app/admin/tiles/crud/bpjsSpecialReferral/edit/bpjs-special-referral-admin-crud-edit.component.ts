...
import * as modelAction from 'src/app/models/bpjsSpecialReferral/bpjs_special_referral.model.action';
import {BpjsSpecialReferralModelState, BpjsSpecialReferralModelAudit} from 'src/app/models/bpjsSpecialReferral/bpjs_special_referral.model.state';
import {BpjsSpecialReferralModel} from 'src/app/models/bpjsSpecialReferral/bpjs_special_referral.model';
import {
	getBpjsSpecialReferralModelAuditsByEntityId,
...
import {CrudTileMode} from '../bpjs-special-referral-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-special-referral-admin-crud-edit',
	templateUrl: './bpjs-special-referral-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-special-referral-admin-crud-edit.component.scss',
...
export class AdminBpjsSpecialReferralCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '5790f42f-a386-455d-82c9-a2c9ba32b41c';
	 ...
	 */
	bpjsSpecialReferralAudits$: Observable<BpjsSpecialReferralModelAudit[]>;
	...
	additionalDiagnoseRelations: ModelRelation[] = this.additionalDiagnoseMultiCrudActive ? BpjsDiagnoseModel.getRelations()
		.map(relation => {
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Additional Procedure and Additional Procedure here] off begin
	additionalProcedureMultiCrudActive: boolean = false;
	additionalProcedureDisplayName = 'Additional Procedure';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Additional Procedure and Additional Procedure here] end
	additionalProcedureOppositeRelation = 'additionalProcedure';
	additionalProcedureRelations: ModelRelation[] = this.additionalProcedureMultiCrudActive ? BpjsProcedureModel.getRelations()
		.map(relation => {
			if (relation.id === this.additionalProcedureOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	primaryDiagnoseRelations: ModelRelation[] = this.primaryDiagnoseMultiCrudActive ? BpjsDiagnoseModel.getRelations()
		.map(relation => {
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Primary Procedure and Primary Procedure here] off begin
	primaryProcedureMultiCrudActive: boolean = false;
	primaryProcedureDisplayName = 'Primary Procedure';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Primary Procedure and Primary Procedure here] end
	primaryProcedureOppositeRelation = 'primaryProcedure';
	primaryProcedureRelations: ModelRelation[] = this.primaryProcedureMultiCrudActive ? BpjsProcedureModel.getRelations()
		.map(relation => {
			if (relation.id === this.primaryProcedureOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = BpjsSpecialReferralModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsSpecialReferralModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsSpecialReferralModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'additionalProcedure' && this.additionalProcedureMultiCrudActive) {
				...
				if (relation.id === 'primaryProcedure' && this.primaryProcedureMultiCrudActive) {
					exp.fields = BpjsProcedureModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: BpjsSpecialReferralModel;
	...
	constructor(
		private readonly store: Store<{ model: BpjsSpecialReferralModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsSpecialReferralModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsSpecialReferralModel();
		...
		if (this.targetModelId) {
			this.bpjsSpecialReferralAudits$ = this.store.select(getBpjsSpecialReferralModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsSpecialReferralModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
			modelAction.BpjsSpecialReferralModelActionTypes.FETCH_BPJS_SPECIAL_REFERRAL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsSpecialReferralModelWithId, this.targetModelId)
			...
			model = new BpjsSpecialReferralModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-special-referral');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
			modelAction.BpjsSpecialReferralModelActionTypes.FETCH_BPJS_SPECIAL_REFERRAL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsSpecialReferralModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.additionalDiagnoseMultiCrudActive || this.additionalProcedureMultiCrudActive || this.primaryDiagnoseMultiCrudActive || this.primaryProcedureMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.BpjsSpecialReferralAction(
						modelAction.BpjsSpecialReferralModelActionTypes.COUNT_BPJS_SPECIAL_REFERRALS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
				modelAction.BpjsSpecialReferralModelActionTypes.CREATE_BPJS_SPECIAL_REFERRAL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
				modelAction.BpjsSpecialReferralModelActionTypes.UPDATE_BPJS_SPECIAL_REFERRAL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsSpecialReferralModel = new BpjsSpecialReferralModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsSpecialReferralModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsSpecialReferralModel(model).difference(updatedModel);
		...
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.additionalDiagnoseDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsDiagnoseModel.getProps(), this.additionalDiagnoseRelations, this.isViewOnly),
			...
			if (event.entityName === this.additionalProcedureDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsProcedureModel.getProps(), this.additionalProcedureRelations, this.isViewOnly),
					new BpjsProcedureModel(),
					BpjsProcedureModel,
					this.additionalProcedureDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.additionalProcedureRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
			...
			if (event.entityName === this.primaryDiagnoseDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsDiagnoseModel.getProps(), this.primaryDiagnoseRelations, this.isViewOnly),
					new BpjsDiagnoseModel(),
					BpjsDiagnoseModel,
			...
			if (event.entityName === this.primaryProcedureDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsProcedureModel.getProps(), this.primaryProcedureRelations, this.isViewOnly),
					new BpjsProcedureModel(),
					BpjsProcedureModel,
					this.primaryProcedureDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.primaryProcedureRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsSpecialReferralModel.getProps();
		this.modelRelations = BpjsSpecialReferralModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'additionalProcedure';
		}).hideElement = this.additionalProcedureMultiCrudActive || this.modelRelations.find(rel => rel.id === 'additionalProcedure').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'primaryProcedure';
		}).hideElement = this.primaryProcedureMultiCrudActive || this.modelRelations.find(rel => rel.id === 'primaryProcedure').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.additionalDiagnoseMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.additionalDiagnoseDisplayName, many: false});
		}

		if (this.additionalProcedureMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.additionalProcedureDisplayName, many: false});
		}

		if (this.primaryDiagnoseMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.primaryDiagnoseDisplayName, many: false});
		}

		if (this.primaryProcedureMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.primaryProcedureDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.additionalDiagnose && this.targetModel.additionalDiagnose.id && this.additionalDiagnoseMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsDiagnoseModel,
				this.additionalDiagnoseOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.additionalDiagnose.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsDiagnoseModelWithId, this.targetModel.additionalDiagnoseId).subscribe(additionalDiagnose => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsDiagnoseModel.getProps(), this.additionalDiagnoseRelations, this.isViewOnly),
					additionalDiagnose,
					BpjsDiagnoseModel,
		...
		if (this.targetModel.additionalProcedure && this.targetModel.additionalProcedure.id && this.additionalProcedureMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsProcedureModel,
				this.additionalProcedureOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.additionalProcedure.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsProcedureModelWithId, this.targetModel.additionalProcedureId).subscribe(additionalProcedure => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsProcedureModel.getProps(), this.additionalProcedureRelations, this.isViewOnly),
					additionalProcedure,
					BpjsProcedureModel,
		...
		if (this.targetModel.primaryDiagnose && this.targetModel.primaryDiagnose.id && this.primaryDiagnoseMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsDiagnoseModel,
				this.primaryDiagnoseOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.primaryDiagnose.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsDiagnoseModelWithId, this.targetModel.primaryDiagnoseId).subscribe(primaryDiagnose => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsDiagnoseModel.getProps(), this.primaryDiagnoseRelations, this.isViewOnly),
					primaryDiagnose,
					BpjsDiagnoseModel,
		...
		if (this.targetModel.primaryProcedure && this.targetModel.primaryProcedure.id && this.primaryProcedureMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				BpjsProcedureModel,
				this.primaryProcedureOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.primaryProcedure.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getBpjsProcedureModelWithId, this.targetModel.primaryProcedureId).subscribe(primaryProcedure => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(BpjsProcedureModel.getProps(), this.primaryProcedureRelations, this.isViewOnly),
					primaryProcedure,
					BpjsProcedureModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsSpecialReferralModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsSpecialReferralModel {
		let updatedModel: BpjsSpecialReferralModel = new BpjsSpecialReferralModel(model);

		if (this.additionalDiagnoseMultiCrudActive) {
			updatedModel.additionalDiagnose = event.additionalEntities.additionalDiagnose ? event.additionalEntities.additionalDiagnose : undefined;

			if (!updatedModel.additionalDiagnose && event.payload) {
				event.payload.additionalDiagnoseId = null;
			}
		} else if (event.payload?.additionalDiagnoseId !== undefined) {
			event.payload.additionalDiagnose = null;
			updatedModel.additionalDiagnose = null;
		}

		if (this.additionalProcedureMultiCrudActive) {
			updatedModel.additionalProcedure = event.additionalEntities.additionalProcedure ? event.additionalEntities.additionalProcedure : undefined;

			if (!updatedModel.additionalProcedure && event.payload) {
				event.payload.additionalProcedureId = null;
			}
		} else if (event.payload?.additionalProcedureId !== undefined) {
			event.payload.additionalProcedure = null;
			updatedModel.additionalProcedure = null;
		}

		if (this.primaryDiagnoseMultiCrudActive) {
			updatedModel.primaryDiagnose = event.additionalEntities.primaryDiagnose ? event.additionalEntities.primaryDiagnose : undefined;

			if (!updatedModel.primaryDiagnose && event.payload) {
				event.payload.primaryDiagnoseId = null;
			}
		} else if (event.payload?.primaryDiagnoseId !== undefined) {
			event.payload.primaryDiagnose = null;
			updatedModel.primaryDiagnose = null;
		}

		if (this.primaryProcedureMultiCrudActive) {
			updatedModel.primaryProcedure = event.additionalEntities.primaryProcedure ? event.additionalEntities.primaryProcedure : undefined;

			if (!updatedModel.primaryProcedure && event.payload) {
				event.payload.primaryProcedureId = null;
			}
		} else if (event.payload?.primaryProcedureId !== undefined) {
			event.payload.primaryProcedure = null;
			updatedModel.primaryProcedure = null;
		}
