...
import {AdminBpjsSpecialReferralCrudListComponent} from './bpjs-special-referral-admin-crud-list.component';
...
import {AdminBpjsSpecialReferralCrudModule} from '../bpjs-special-referral-admin-crud.module';
import {
 ...
 */
describe('BPJS Special Referral Crud List Tile Component', () => {

	let fixture;
	let bpjsSpecialReferralCrudListComponent: AdminBpjsSpecialReferralCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsSpecialReferralModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5790f42f-a386-455d-82c9-a2c9ba32b41c';
	const searchContent = 'text';

	const searchableFieldCount = BpjsSpecialReferralModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-special-referral-admin-crud',
		urls: ['bpjs-special-referral-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsSpecialReferralDataFactory = new BpjsSpecialReferralDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsSpecialReferralDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsSpecialReferralCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsSpecialReferralCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsSpecialReferralCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsSpecialReferralCrudListComponent);
			bpjsSpecialReferralCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsSpecialReferralCrudListComponent.bpjsSpecialReferralsCount).toBe(defaultData.length);
			...
			expect(bpjsSpecialReferralCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsSpecialReferralCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsSpecialReferralDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsSpecialReferralCrudListComponent.bpjsSpecialReferralsCount).toBe(filteredDataSize);
			expect(bpjsSpecialReferralCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsSpecialReferralCrudListComponent.pageIndex).toBe(0);
			expect(bpjsSpecialReferralCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsSpecialReferralCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsSpecialReferralCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsSpecialReferralCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
