...
import {BpjsSpecialReferralModelState} from 'src/app/models/bpjsSpecialReferral/bpjs_special_referral.model.state';
...
import {
	...
	getBpjsSpecialReferralCollectionCount
...
@Component({
	selector: 'cb-bpjs-special-referral-admin-crud-list',
	templateUrl: './bpjs-special-referral-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-special-referral-admin-crud-list.component.scss',
...
export class AdminBpjsSpecialReferralCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsSpecialReferralModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsSpecialReferralModel.searchFields,
	...
	/**
	 * List of BPJS Special Referrals that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsSpecialReferrals: Observable<BpjsSpecialReferralModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsSpecialReferralsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsSpecialReferralsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsSpecialReferralsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
				modelAction.BpjsSpecialReferralModelActionTypes.INITIALISE_BPJS_SPECIAL_REFERRAL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
				modelAction.BpjsSpecialReferralModelActionTypes.FETCH_BPJS_SPECIAL_REFERRAL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bpjsSpecialReferrals = this.store.select(getBpjsSpecialReferralCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsSpecialReferralCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsSpecialReferralsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsSpecialReferralModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsSpecialReferralModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsSpecialReferralModelActionTypes.EXPORT_ALL_BPJS_SPECIAL_REFERRAL;
		} else {
			actionType = modelAction.BpjsSpecialReferralModelActionTypes.EXPORT_BPJS_SPECIAL_REFERRAL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
				modelAction.BpjsSpecialReferralModelActionTypes.IMPORT_BPJS_SPECIAL_REFERRALS,
				{
					file: file,
				},
				[new modelAction.BpjsSpecialReferralAction(
			...
			window.open(environment.API_URL + '/api/bpjsspecialreferral/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsSpecialReferralModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Special Referral entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Special Referral entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsSpecialReferralModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsSpecialReferralModelActionTypes.DELETE_BPJS_SPECIAL_REFERRAL;
						} else if (usingSelected) {
							actionType = modelAction.BpjsSpecialReferralModelActionTypes.DELETE_ALL_BPJS_SPECIAL_REFERRAL;
						} else {
							actionType = modelAction.BpjsSpecialReferralModelActionTypes.DELETE_BPJS_SPECIAL_REFERRAL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsSpecialReferralAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_SPECIAL_REFERRAL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_SPECIAL_REFERRAL and DELETE_BPJS_SPECIAL_REFERRAL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsSpecialReferralModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsSpecialReferralsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
