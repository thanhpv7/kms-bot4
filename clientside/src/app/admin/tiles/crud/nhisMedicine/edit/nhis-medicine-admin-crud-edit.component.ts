...
import {NhisMedicineModelState, NhisMedicineModelAudit} from 'src/app/models/nhisMedicine/nhis_medicine.model.state';
import {NhisMedicineModel} from 'src/app/models/nhisMedicine/nhis_medicine.model';
import {
	getNhisMedicineModelAuditsByEntityId,
	getNhisMedicineModelWithId,
	getCountNhisMedicineModels,
...
@Component({
	selector: 'cb-nhis-medicine-admin-crud-edit',
	templateUrl: './nhis-medicine-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-medicine-admin-crud-edit.component.scss',
...
export class AdminNhisMedicineCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisMedicineAudits$: Observable<NhisMedicineModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisMedicineModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisMedicineModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisMedicineModel();
		...
		if (this.targetModelId) {
			this.nhisMedicineAudits$ = this.store.select(getNhisMedicineModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisMedicineModel> = {
		...
		this.store.dispatch(new modelAction.NhisMedicineAction(
			modelAction.NhisMedicineModelActionTypes.FETCH_NHIS_MEDICINE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisMedicineModelWithId, this.targetModelId)
			...
			model = new NhisMedicineModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisMedicineAction(
			modelAction.NhisMedicineModelActionTypes.FETCH_NHIS_MEDICINE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisMedicineAction(
				modelAction.NhisMedicineModelActionTypes.CREATE_NHIS_MEDICINE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisMedicineAction(
				modelAction.NhisMedicineModelActionTypes.UPDATE_NHIS_MEDICINE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisMedicineModel = new NhisMedicineModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisMedicineModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisMedicineModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisMedicineModel.getProps();
		this.modelRelations = NhisMedicineModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisMedicineModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisMedicineModel {
		let updatedModel: NhisMedicineModel = new NhisMedicineModel(model);
