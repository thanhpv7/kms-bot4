...
import * as modelAction from 'src/app/models/nhisMedicine/nhis_medicine.model.action';
import {NhisMedicineModelState} from 'src/app/models/nhisMedicine/nhis_medicine.model.state';
...
import {
	getNhisMedicineCollectionModels,
	getNhisMedicineCollectionCount
} from 'src/app/models/nhisMedicine/nhis_medicine.model.selector';
...
import {CrudTileMode} from '../nhis-medicine-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-medicine-admin-crud-list',
	templateUrl: './nhis-medicine-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-medicine-admin-crud-list.component.scss',
...
export class AdminNhisMedicineCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisMedicineModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisMedicineModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = NhisMedicineModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'da258316-db57-4f12-90da-254c69c9b3eb';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisMedicinesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisMedicinesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisMedicineModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisMedicineAction(
				modelAction.NhisMedicineModelActionTypes.INITIALISE_NHIS_MEDICINE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisMedicineAction(
				modelAction.NhisMedicineModelActionTypes.FETCH_NHIS_MEDICINE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisMedicineAction(
						modelAction.NhisMedicineModelActionTypes.COUNT_NHIS_MEDICINES,
						null
					)
				]
			))
		...
		this.nhisMedicines = this.store.select(getNhisMedicineCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisMedicineCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisMedicinesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisMedicineModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisMedicineModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisMedicineModelActionTypes.EXPORT_ALL_NHIS_MEDICINE;
		} else {
			actionType = modelAction.NhisMedicineModelActionTypes.EXPORT_NHIS_MEDICINE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisMedicineAction(
				modelAction.NhisMedicineModelActionTypes.IMPORT_NHIS_MEDICINES,
				{
					file: file,
				},
				[new modelAction.NhisMedicineAction(
			...
			window.open(environment.API_URL + '/api/nhismedicine/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisMedicineModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Medicine entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Medicine entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisMedicineModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisMedicineModelActionTypes.DELETE_NHIS_MEDICINE;
						} else if (usingSelected) {
							actionType = modelAction.NhisMedicineModelActionTypes.DELETE_ALL_NHIS_MEDICINE;
						} else {
							actionType = modelAction.NhisMedicineModelActionTypes.DELETE_NHIS_MEDICINE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisMedicineAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_MEDICINE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_MEDICINE and DELETE_NHIS_MEDICINE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisMedicineModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisMedicineModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisMedicinesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
