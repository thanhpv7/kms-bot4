...
import {AdminNhisMedicineCrudListComponent} from './nhis-medicine-admin-crud-list.component';
import {NhisMedicineDataFactory} from 'src/app/lib/utils/factories/nhis-medicine-data-factory';
import {NhisMedicineModel} from 'src/app/models/nhisMedicine/nhis_medicine.model';
import {AdminNhisMedicineCrudModule} from '../nhis-medicine-admin-crud.module';
import {
	NhisMedicineModelState,
	initialState as initialNhisMedicineModelState
} from 'src/app/models/nhisMedicine/nhis_medicine.model.state';
 ...
 */
describe('NHIS Medicine Crud List Tile Component', () => {

	let fixture;
	let nhisMedicineCrudListComponent: AdminNhisMedicineCrudListComponent;

	let store: MockStore<{ model: NhisMedicineModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisMedicineModelState: NhisMedicineModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisMedicineModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'da258316-db57-4f12-90da-254c69c9b3eb';
	const searchContent = 'text';

	const searchableFieldCount = NhisMedicineModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-medicine-admin-crud',
		urls: ['nhis-medicine-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisMedicineDataFactory = new NhisMedicineDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisMedicineDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisMedicineModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisMedicineModelState = initialNhisMedicineModelState;
		store.setState({model: nhisMedicineModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisMedicineCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisMedicineCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminNhisMedicineCrudListComponent);
			nhisMedicineCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisMedicineCrudListComponent.nhisMedicinesCount).toBe(defaultData.length);
			...
			expect(nhisMedicineCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisMedicineCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisMedicineDataFactory.createAll(filteredDataSize);
			...
			expect(nhisMedicineCrudListComponent.nhisMedicinesCount).toBe(filteredDataSize);
			expect(nhisMedicineCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisMedicineCrudListComponent.pageIndex).toBe(0);
			expect(nhisMedicineCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisMedicineCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisMedicineCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisMedicineCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
