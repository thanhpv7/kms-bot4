...
import {BedFacilityModelState} from 'src/app/models/bedFacility/bed_facility.model.state';
...
import {CrudTileMode} from '../bed-facility-admin-crud.component';
...
@Component({
	selector: 'cb-bed-facility-admin-crud-list',
	templateUrl: './bed-facility-admin-crud-list.component.html',
	styleUrls: [
		'./bed-facility-admin-crud-list.component.scss',
...
export class AdminBedFacilityCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BedFacilityModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BedFacilityModel.searchFields,
	...
	/**
	 * List of Bed Facilitys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bedFacilitys: Observable<BedFacilityModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bedFacilitysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bedFacilitysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bedFacilitysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BedFacilityAction(
				modelAction.BedFacilityModelActionTypes.INITIALISE_BED_FACILITY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BedFacilityAction(
				modelAction.BedFacilityModelActionTypes.FETCH_BED_FACILITY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.bedFacilitys = this.store.select(getBedFacilityCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBedFacilityCollectionCount, this.collectionId).subscribe(
				(count) => this.bedFacilitysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BedFacilityModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BedFacilityModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BedFacilityModelActionTypes.EXPORT_ALL_BED_FACILITY;
		} else {
			actionType = modelAction.BedFacilityModelActionTypes.EXPORT_BED_FACILITY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BedFacilityAction(
				modelAction.BedFacilityModelActionTypes.IMPORT_BED_FACILITYS,
				{
					file: file,
				},
				[new modelAction.BedFacilityAction(
			...
			window.open(environment.API_URL + '/api/bedfacility/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BedFacilityModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Bed Facility entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Bed Facility entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BedFacilityModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BedFacilityModelActionTypes.DELETE_BED_FACILITY;
						} else if (usingSelected) {
							actionType = modelAction.BedFacilityModelActionTypes.DELETE_ALL_BED_FACILITY;
						} else {
							actionType = modelAction.BedFacilityModelActionTypes.DELETE_BED_FACILITY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BedFacilityAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BED_FACILITY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BED_FACILITY and DELETE_BED_FACILITY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BedFacilityModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bedFacilitysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
