...
import {AdminBedFacilityCrudListComponent} from './bed-facility-admin-crud-list.component';
...
import {AdminBedFacilityCrudModule} from '../bed-facility-admin-crud.module';
import {
 ...
 */
describe('Bed Facility Crud List Tile Component', () => {

	let fixture;
	let bedFacilityCrudListComponent: AdminBedFacilityCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BedFacilityModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd59c0366-cff0-4e29-bafd-05491f19ed36';
	const searchContent = 'text';

	const searchableFieldCount = BedFacilityModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bed-facility-admin-crud',
		urls: ['bed-facility-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bedFacilityDataFactory = new BedFacilityDataFactory();

	const defaultDataSize = 30;
	const defaultData = bedFacilityDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBedFacilityCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBedFacilityCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBedFacilityCrudListComponent);
			bedFacilityCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bedFacilityCrudListComponent.bedFacilitysCount).toBe(defaultData.length);
			...
			expect(bedFacilityCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bedFacilityCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bedFacilityDataFactory.createAll(filteredDataSize);
			...
			expect(bedFacilityCrudListComponent.bedFacilitysCount).toBe(filteredDataSize);
			expect(bedFacilityCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bedFacilityCrudListComponent.pageIndex).toBe(0);
			expect(bedFacilityCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bedFacilityCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bedFacilityCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bedFacilityCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
