...
import {AdminIntraoperativeRecordsCrudListComponent} from './intraoperative-records-admin-crud-list.component';
...
import {AdminIntraoperativeRecordsCrudModule} from '../intraoperative-records-admin-crud.module';
import {
 ...
 */
describe('Intraoperative Records Crud List Tile Component', () => {

	let fixture;
	let intraoperativeRecordsCrudListComponent: AdminIntraoperativeRecordsCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<IntraoperativeRecordsModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b1ac206b-d1aa-4cc3-a627-afce95be244e';
	const searchContent = 'text';

	const searchableFieldCount = IntraoperativeRecordsModel.searchFields.length;

	const routerState: RouterState = {
		url: 'intraoperative-records-admin-crud',
		urls: ['intraoperative-records-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const intraoperativeRecordsDataFactory = new IntraoperativeRecordsDataFactory();

	const defaultDataSize = 30;
	const defaultData = intraoperativeRecordsDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getIntraoperativeRecordsCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getIntraoperativeRecordsCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminIntraoperativeRecordsCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminIntraoperativeRecordsCrudListComponent);
			intraoperativeRecordsCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(intraoperativeRecordsCrudListComponent.intraoperativeRecordssCount).toBe(defaultData.length);
			...
			expect(intraoperativeRecordsCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			intraoperativeRecordsCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = intraoperativeRecordsDataFactory.createAll(filteredDataSize);
			...
			expect(intraoperativeRecordsCrudListComponent.intraoperativeRecordssCount).toBe(filteredDataSize);
			expect(intraoperativeRecordsCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(intraoperativeRecordsCrudListComponent.pageIndex).toBe(0);
			expect(intraoperativeRecordsCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			intraoperativeRecordsCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		intraoperativeRecordsCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		intraoperativeRecordsCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
