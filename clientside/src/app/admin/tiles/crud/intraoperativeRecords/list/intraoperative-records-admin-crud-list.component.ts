...
import {IntraoperativeRecordsModelState} from 'src/app/models/intraoperativeRecords/intraoperative_records.model.state';
...
import {
	...
	getIntraoperativeRecordsCollectionCount
...
@Component({
	selector: 'cb-intraoperative-records-admin-crud-list',
	templateUrl: './intraoperative-records-admin-crud-list.component.html',
	styleUrls: [
		'./intraoperative-records-admin-crud-list.component.scss',
...
export class AdminIntraoperativeRecordsCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<IntraoperativeRecordsModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...IntraoperativeRecordsModel.searchFields,
	...
	/**
	 * List of Intraoperative Recordss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	intraoperativeRecordss: Observable<IntraoperativeRecordsModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	intraoperativeRecordssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.intraoperativeRecordssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.intraoperativeRecordssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
				modelAction.IntraoperativeRecordsModelActionTypes.INITIALISE_INTRAOPERATIVE_RECORDS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
				modelAction.IntraoperativeRecordsModelActionTypes.FETCH_INTRAOPERATIVE_RECORDS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.intraoperativeRecordss = this.store.select(getIntraoperativeRecordsCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getIntraoperativeRecordsCollectionCount, this.collectionId).subscribe(
				(count) => this.intraoperativeRecordssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: IntraoperativeRecordsModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.IntraoperativeRecordsModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.IntraoperativeRecordsModelActionTypes.EXPORT_ALL_INTRAOPERATIVE_RECORDS;
		} else {
			actionType = modelAction.IntraoperativeRecordsModelActionTypes.EXPORT_INTRAOPERATIVE_RECORDS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
				modelAction.IntraoperativeRecordsModelActionTypes.IMPORT_INTRAOPERATIVE_RECORDSS,
				{
					file: file,
				},
				[new modelAction.IntraoperativeRecordsAction(
			...
			window.open(environment.API_URL + '/api/intraoperativerecords/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: IntraoperativeRecordsModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Intraoperative Records entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Intraoperative Records entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.IntraoperativeRecordsModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.IntraoperativeRecordsModelActionTypes.DELETE_INTRAOPERATIVE_RECORDS;
						} else if (usingSelected) {
							actionType = modelAction.IntraoperativeRecordsModelActionTypes.DELETE_ALL_INTRAOPERATIVE_RECORDS;
						} else {
							actionType = modelAction.IntraoperativeRecordsModelActionTypes.DELETE_INTRAOPERATIVE_RECORDS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INTRAOPERATIVE_RECORDS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INTRAOPERATIVE_RECORDS and DELETE_INTRAOPERATIVE_RECORDS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = IntraoperativeRecordsModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.intraoperativeRecordssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
