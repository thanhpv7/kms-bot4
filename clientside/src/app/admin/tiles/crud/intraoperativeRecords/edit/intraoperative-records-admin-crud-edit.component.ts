...
import * as modelAction from 'src/app/models/intraoperativeRecords/intraoperative_records.model.action';
import {IntraoperativeRecordsModelState, IntraoperativeRecordsModelAudit} from 'src/app/models/intraoperativeRecords/intraoperative_records.model.state';
import {IntraoperativeRecordsModel} from 'src/app/models/intraoperativeRecords/intraoperative_records.model';
import {
	getIntraoperativeRecordsModelAuditsByEntityId,
...
import {CrudTileMode} from '../intraoperative-records-admin-crud.component';
...
@Component({
	selector: 'cb-intraoperative-records-admin-crud-edit',
	templateUrl: './intraoperative-records-admin-crud-edit.component.html',
	styleUrls: [
		'./intraoperative-records-admin-crud-edit.component.scss',
...
export class AdminIntraoperativeRecordsCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b1ac206b-d1aa-4cc3-a627-afce95be244e';
	 ...
	 */
	intraoperativeRecordsAudits$: Observable<IntraoperativeRecordsModelAudit[]>;
	...
	operatingTheaterMedicalExaminationRecordOppositeRelation = 'intraoperativeRecords';
	...
	operatingRoomRelations: ModelRelation[] = this.operatingRoomMultiCrudActive ? RoomFacilityModel.getRelations()
		.map(relation => {
	...
	bedFacilitiesOppositeRelation = 'intraoperativeRecords';
	 ...
	 */
	modelProperties: ModelProperty[] = IntraoperativeRecordsModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = IntraoperativeRecordsModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = IntraoperativeRecordsModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: IntraoperativeRecordsModel;
	...
	constructor(
		private readonly store: Store<{ model: IntraoperativeRecordsModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountIntraoperativeRecordsModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new IntraoperativeRecordsModel();
		...
		if (this.targetModelId) {
			this.intraoperativeRecordsAudits$ = this.store.select(getIntraoperativeRecordsModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<IntraoperativeRecordsModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
			modelAction.IntraoperativeRecordsModelActionTypes.FETCH_INTRAOPERATIVE_RECORDS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getIntraoperativeRecordsModelWithId, this.targetModelId)
			...
			model = new IntraoperativeRecordsModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('intraoperative-records');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
			modelAction.IntraoperativeRecordsModelActionTypes.FETCH_INTRAOPERATIVE_RECORDS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<IntraoperativeRecordsModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.operatingTheaterMedicalExaminationRecordMultiCrudActive || this.operatingRoomMultiCrudActive || this.bedFacilitiesMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.IntraoperativeRecordsAction(
						modelAction.IntraoperativeRecordsModelActionTypes.COUNT_INTRAOPERATIVE_RECORDSS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
				modelAction.IntraoperativeRecordsModelActionTypes.CREATE_INTRAOPERATIVE_RECORDS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.IntraoperativeRecordsAction(
				modelAction.IntraoperativeRecordsModelActionTypes.UPDATE_INTRAOPERATIVE_RECORDS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: IntraoperativeRecordsModel = new IntraoperativeRecordsModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getIntraoperativeRecordsModelWithId, this.targetModelId).subscribe(model => {
					let differences = new IntraoperativeRecordsModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.operatingRoomDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RoomFacilityModel.getProps(), this.operatingRoomRelations, this.isViewOnly),
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = IntraoperativeRecordsModel.getProps();
		this.modelRelations = IntraoperativeRecordsModel.getRelations();
	...
	initialiseMultiEntityCollections() {
		...
		if (this.operatingRoomMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.operatingRoomDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.operatingRoom && this.targetModel.operatingRoom.id && this.operatingRoomMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				RoomFacilityModel,
				this.operatingRoomOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.operatingRoom.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getRoomFacilityModelWithId, this.targetModel.operatingRoomId).subscribe(operatingRoom => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RoomFacilityModel.getProps(), this.operatingRoomRelations, this.isViewOnly),
					operatingRoom,
					RoomFacilityModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: IntraoperativeRecordsModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): IntraoperativeRecordsModel {
		let updatedModel: IntraoperativeRecordsModel = new IntraoperativeRecordsModel(model);
		...
		if (this.operatingRoomMultiCrudActive) {
			updatedModel.operatingRoom = event.additionalEntities.operatingRoom ? event.additionalEntities.operatingRoom : undefined;

			if (!updatedModel.operatingRoom && event.payload) {
				event.payload.operatingRoomId = null;
			}
		} else if (event.payload?.operatingRoomId !== undefined) {
			event.payload.operatingRoom = null;
			updatedModel.operatingRoom = null;
		}
