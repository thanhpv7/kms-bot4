...
import {AdminTransferOrderCrudListComponent} from './transfer-order-admin-crud-list.component';
...
import {AdminTransferOrderCrudModule} from '../transfer-order-admin-crud.module';
import {
 ...
 */
describe('Transfer Order Crud List Tile Component', () => {

	let fixture;
	let transferOrderCrudListComponent: AdminTransferOrderCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<TransferOrderModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f8d9164e-1d55-4d4d-a1c3-1c74a4c6e8f0';
	const searchContent = 'text';

	const searchableFieldCount = TransferOrderModel.searchFields.length;

	const routerState: RouterState = {
		url: 'transfer-order-admin-crud',
		urls: ['transfer-order-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const transferOrderDataFactory = new TransferOrderDataFactory();

	const defaultDataSize = 30;
	const defaultData = transferOrderDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getTransferOrderCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTransferOrderCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminTransferOrderCrudListComponent);
			transferOrderCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(transferOrderCrudListComponent.transferOrdersCount).toBe(defaultData.length);
			...
			expect(transferOrderCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			transferOrderCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = transferOrderDataFactory.createAll(filteredDataSize);
			...
			expect(transferOrderCrudListComponent.transferOrdersCount).toBe(filteredDataSize);
			expect(transferOrderCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(transferOrderCrudListComponent.pageIndex).toBe(0);
			expect(transferOrderCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			transferOrderCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		transferOrderCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		transferOrderCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
