...
import {TransferOrderModelState} from 'src/app/models/transferOrder/transfer_order.model.state';
...
import {
	...
	getTransferOrderCollectionCount
...
@Component({
	selector: 'cb-transfer-order-admin-crud-list',
	templateUrl: './transfer-order-admin-crud-list.component.html',
	styleUrls: [
		'./transfer-order-admin-crud-list.component.scss',
...
export class AdminTransferOrderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TransferOrderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TransferOrderModel.searchFields,
	...
	/**
	 * List of Transfer Orders that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	transferOrders: Observable<TransferOrderModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	transferOrdersCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.transferOrdersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.transferOrdersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransferOrderAction(
				modelAction.TransferOrderModelActionTypes.INITIALISE_TRANSFER_ORDER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransferOrderAction(
				modelAction.TransferOrderModelActionTypes.FETCH_TRANSFER_ORDER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.transferOrders = this.store.select(getTransferOrderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTransferOrderCollectionCount, this.collectionId).subscribe(
				(count) => this.transferOrdersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TransferOrderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TransferOrderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TransferOrderModelActionTypes.EXPORT_ALL_TRANSFER_ORDER;
		} else {
			actionType = modelAction.TransferOrderModelActionTypes.EXPORT_TRANSFER_ORDER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TransferOrderAction(
				modelAction.TransferOrderModelActionTypes.IMPORT_TRANSFER_ORDERS,
				{
					file: file,
				},
				[new modelAction.TransferOrderAction(
			...
			window.open(environment.API_URL + '/api/transferorder/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TransferOrderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Transfer Order entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Transfer Order entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TransferOrderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TransferOrderModelActionTypes.DELETE_TRANSFER_ORDER;
						} else if (usingSelected) {
							actionType = modelAction.TransferOrderModelActionTypes.DELETE_ALL_TRANSFER_ORDER;
						} else {
							actionType = modelAction.TransferOrderModelActionTypes.DELETE_TRANSFER_ORDER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TransferOrderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TRANSFER_ORDER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TRANSFER_ORDER and DELETE_TRANSFER_ORDER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TransferOrderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.transferOrdersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
