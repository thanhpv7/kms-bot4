...
import * as modelAction from 'src/app/models/transferOrder/transfer_order.model.action';
import {TransferOrderModelState, TransferOrderModelAudit} from 'src/app/models/transferOrder/transfer_order.model.state';
import {TransferOrderModel} from 'src/app/models/transferOrder/transfer_order.model';
import {
	getTransferOrderModelAuditsByEntityId,
...
import {CrudTileMode} from '../transfer-order-admin-crud.component';
...
@Component({
	selector: 'cb-transfer-order-admin-crud-edit',
	templateUrl: './transfer-order-admin-crud-edit.component.html',
	styleUrls: [
		'./transfer-order-admin-crud-edit.component.scss',
...
export class AdminTransferOrderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'f8d9164e-1d55-4d4d-a1c3-1c74a4c6e8f0';
	 ...
	 */
	transferOrderAudits$: Observable<TransferOrderModelAudit[]>;
	...
	destinationWarehouseMultiCrudActive: boolean = false;
	destinationWarehouseDisplayName = 'Destination Warehouse';
	...
	destinationWarehouseOppositeRelation = 'destinationWarehouses';
	destinationWarehouseRelations: ModelRelation[] = this.destinationWarehouseMultiCrudActive ? WarehouseModel.getRelations()
		.map(relation => {
			if (relation.id === this.destinationWarehouseOppositeRelation) {
	...
	sourceWarehouseMultiCrudActive: boolean = false;
	sourceWarehouseDisplayName = 'Source Warehouse';
	...
	sourceWarehouseOppositeRelation = 'sourceWarehouses';
	sourceWarehouseRelations: ModelRelation[] = this.sourceWarehouseMultiCrudActive ? WarehouseModel.getRelations()
		.map(relation => {
			if (relation.id === this.sourceWarehouseOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Transfer Order and Transfer Order Stock Details here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Transfer Order and Transfer Order Stock Details here] end
	transferOrderStockDetailsOppositeRelation = 'transferOrder';
	 ...
	 */
	modelProperties: ModelProperty[] = TransferOrderModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TransferOrderModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TransferOrderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'destinationWarehouse' && this.destinationWarehouseMultiCrudActive) {
					exp.fields = WarehouseModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'sourceWarehouse' && this.sourceWarehouseMultiCrudActive) {
					exp.fields = WarehouseModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: TransferOrderModel;
	...
	constructor(
		private readonly store: Store<{ model: TransferOrderModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTransferOrderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TransferOrderModel();
		...
		if (this.targetModelId) {
			this.transferOrderAudits$ = this.store.select(getTransferOrderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TransferOrderModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TransferOrderAction(
			modelAction.TransferOrderModelActionTypes.FETCH_TRANSFER_ORDER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTransferOrderModelWithId, this.targetModelId)
			...
			model = new TransferOrderModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TransferOrderAction(
			modelAction.TransferOrderModelActionTypes.FETCH_TRANSFER_ORDER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TransferOrderModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.destinationWarehouseMultiCrudActive || this.sourceWarehouseMultiCrudActive || this.transferOrderStockDetailsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TransferOrderAction(
						modelAction.TransferOrderModelActionTypes.COUNT_TRANSFER_ORDERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TransferOrderAction(
				modelAction.TransferOrderModelActionTypes.CREATE_TRANSFER_ORDER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TransferOrderAction(
				modelAction.TransferOrderModelActionTypes.UPDATE_TRANSFER_ORDER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TransferOrderModel = new TransferOrderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTransferOrderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TransferOrderModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.destinationWarehouseDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(WarehouseModel.getProps(), this.destinationWarehouseRelations, this.isViewOnly),
					new WarehouseModel(),
					WarehouseModel,
					this.destinationWarehouseDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.destinationWarehouseRelations,
					true
				);
			...
			if (event.entityName === this.sourceWarehouseDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(WarehouseModel.getProps(), this.sourceWarehouseRelations, this.isViewOnly),
					new WarehouseModel(),
					WarehouseModel,
					this.sourceWarehouseDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.sourceWarehouseRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TransferOrderModel.getProps();
		this.modelRelations = TransferOrderModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'destinationWarehouse';
		}).hideElement = this.destinationWarehouseMultiCrudActive || this.modelRelations.find(rel => rel.id === 'destinationWarehouse').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'sourceWarehouse';
		}).hideElement = this.sourceWarehouseMultiCrudActive || this.modelRelations.find(rel => rel.id === 'sourceWarehouse').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.destinationWarehouseMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.destinationWarehouseDisplayName, many: false});
		}

		if (this.sourceWarehouseMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.sourceWarehouseDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.destinationWarehouse && this.targetModel.destinationWarehouse.id && this.destinationWarehouseMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				WarehouseModel,
				this.destinationWarehouseOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.destinationWarehouse.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getWarehouseModelWithId, this.targetModel.destinationWarehouseId).subscribe(destinationWarehouse => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(WarehouseModel.getProps(), this.destinationWarehouseRelations, this.isViewOnly),
		...
		if (this.targetModel.sourceWarehouse && this.targetModel.sourceWarehouse.id && this.sourceWarehouseMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				WarehouseModel,
				this.sourceWarehouseOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.sourceWarehouse.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getWarehouseModelWithId, this.targetModel.sourceWarehouseId).subscribe(sourceWarehouse => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(WarehouseModel.getProps(), this.sourceWarehouseRelations, this.isViewOnly),
					sourceWarehouse,
					WarehouseModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: TransferOrderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TransferOrderModel {
		let updatedModel: TransferOrderModel = new TransferOrderModel(model);

		if (this.destinationWarehouseMultiCrudActive) {
			updatedModel.destinationWarehouse = event.additionalEntities.destinationWarehouse ? event.additionalEntities.destinationWarehouse : undefined;

			if (!updatedModel.destinationWarehouse && event.payload) {
				event.payload.destinationWarehouseId = null;
			}
		} else if (event.payload?.destinationWarehouseId !== undefined) {
			event.payload.destinationWarehouse = null;
			updatedModel.destinationWarehouse = null;
		}

		if (this.sourceWarehouseMultiCrudActive) {
			updatedModel.sourceWarehouse = event.additionalEntities.sourceWarehouse ? event.additionalEntities.sourceWarehouse : undefined;

			if (!updatedModel.sourceWarehouse && event.payload) {
				event.payload.sourceWarehouseId = null;
			}
		} else if (event.payload?.sourceWarehouseId !== undefined) {
			event.payload.sourceWarehouse = null;
			updatedModel.sourceWarehouse = null;
		}
