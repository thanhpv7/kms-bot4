...
import * as modelAction from 'src/app/models/medicationItem/medication_item.model.action';
import {MedicationItemModelState, MedicationItemModelAudit} from 'src/app/models/medicationItem/medication_item.model.state';
import {MedicationItemModel} from 'src/app/models/medicationItem/medication_item.model';
import {
	getMedicationItemModelAuditsByEntityId,
...
import {CrudTileMode} from '../medication-item-admin-crud.component';
...
@Component({
	selector: 'cb-medication-item-admin-crud-edit',
	templateUrl: './medication-item-admin-crud-edit.component.html',
	styleUrls: [
		'./medication-item-admin-crud-edit.component.scss',
...
export class AdminMedicationItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '1607ada5-2444-4c91-b083-69b79eab2a8c';
	 ...
	 */
	medicationItemAudits$: Observable<MedicationItemModelAudit[]>;
	...
	medicationCompoundOppositeRelation = 'medicationItems';
	...
	medicationHeaderOppositeRelation = 'medicationItems';
	...
	stockCatalogueOppositeRelation = 'medicationItems';
	...
	unitOfMeasurementOppositeRelation = 'medicationItems';
	...
	medicationAdministeredHisotriesOppositeRelation = 'medicationItem';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicationItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicationItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicationItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicationItemModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicationItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicationItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicationItemModel();
		...
		if (this.targetModelId) {
			this.medicationItemAudits$ = this.store.select(getMedicationItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicationItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicationItemAction(
			modelAction.MedicationItemModelActionTypes.FETCH_MEDICATION_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicationItemModelWithId, this.targetModelId)
			...
			model = new MedicationItemModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicationItemAction(
			modelAction.MedicationItemModelActionTypes.FETCH_MEDICATION_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicationItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.medicationCompoundMultiCrudActive || this.medicationHeaderMultiCrudActive || this.stockCatalogueMultiCrudActive || this.unitOfMeasurementMultiCrudActive || this.medicationAdministeredHisotriesMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicationItemAction(
						modelAction.MedicationItemModelActionTypes.COUNT_MEDICATION_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicationItemAction(
				modelAction.MedicationItemModelActionTypes.CREATE_MEDICATION_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicationItemAction(
				modelAction.MedicationItemModelActionTypes.UPDATE_MEDICATION_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicationItemModel = new MedicationItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicationItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicationItemModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicationItemModel.getProps();
		this.modelRelations = MedicationItemModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicationItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicationItemModel {
		let updatedModel: MedicationItemModel = new MedicationItemModel(model);
