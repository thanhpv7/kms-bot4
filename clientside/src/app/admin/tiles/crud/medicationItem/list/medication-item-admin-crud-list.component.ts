...
import {MedicationItemModelState} from 'src/app/models/medicationItem/medication_item.model.state';
...
@Component({
	selector: 'cb-medication-item-admin-crud-list',
	templateUrl: './medication-item-admin-crud-list.component.html',
	styleUrls: [
		'./medication-item-admin-crud-list.component.scss',
...
export class AdminMedicationItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicationItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicationItemModel.searchFields,
	...
	/**
	 * List of Medication Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicationItems: Observable<MedicationItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicationItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicationItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicationItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationItemAction(
				modelAction.MedicationItemModelActionTypes.INITIALISE_MEDICATION_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationItemAction(
				modelAction.MedicationItemModelActionTypes.FETCH_MEDICATION_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicationItems = this.store.select(getMedicationItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicationItemCollectionCount, this.collectionId).subscribe(
				(count) => this.medicationItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicationItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicationItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicationItemModelActionTypes.EXPORT_ALL_MEDICATION_ITEM;
		} else {
			actionType = modelAction.MedicationItemModelActionTypes.EXPORT_MEDICATION_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicationItemAction(
				modelAction.MedicationItemModelActionTypes.IMPORT_MEDICATION_ITEMS,
				{
					file: file,
				},
				[new modelAction.MedicationItemAction(
			...
			window.open(environment.API_URL + '/api/medicationitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicationItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medication Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medication Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicationItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicationItemModelActionTypes.DELETE_MEDICATION_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.MedicationItemModelActionTypes.DELETE_ALL_MEDICATION_ITEM;
						} else {
							actionType = modelAction.MedicationItemModelActionTypes.DELETE_MEDICATION_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicationItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICATION_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICATION_ITEM and DELETE_MEDICATION_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicationItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicationItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
