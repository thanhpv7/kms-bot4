...
import {InternalOrderModelState, InternalOrderModelAudit} from 'src/app/models/internalOrder/internal_order.model.state';
import {InternalOrderModel} from 'src/app/models/internalOrder/internal_order.model';
import {
	getInternalOrderModelAuditsByEntityId,
	getInternalOrderModelWithId,
	getCountInternalOrderModels,
...
@Component({
	selector: 'cb-internal-order-admin-crud-edit',
	templateUrl: './internal-order-admin-crud-edit.component.html',
	styleUrls: [
		'./internal-order-admin-crud-edit.component.scss',
...
export class AdminInternalOrderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	internalOrderAudits$: Observable<InternalOrderModelAudit[]>;
	...
	warehouseOppositeRelation = 'internalOrders';
	...
	internalOrderStockDetailsOppositeRelation = 'internalOrder';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InternalOrderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: InternalOrderModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInternalOrderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InternalOrderModel();
		...
		if (this.targetModelId) {
			this.internalOrderAudits$ = this.store.select(getInternalOrderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InternalOrderModel> = {
		...
		this.store.dispatch(new modelAction.InternalOrderAction(
			modelAction.InternalOrderModelActionTypes.FETCH_INTERNAL_ORDER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInternalOrderModelWithId, this.targetModelId)
			...
			model = new InternalOrderModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InternalOrderAction(
			modelAction.InternalOrderModelActionTypes.FETCH_INTERNAL_ORDER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InternalOrderModel> = {
				...
				queryParams: {
					persistRelations: this.warehouseMultiCrudActive || this.internalOrderStockDetailsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.InternalOrderAction(
				modelAction.InternalOrderModelActionTypes.CREATE_INTERNAL_ORDER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InternalOrderAction(
				modelAction.InternalOrderModelActionTypes.UPDATE_INTERNAL_ORDER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InternalOrderModel = new InternalOrderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInternalOrderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InternalOrderModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InternalOrderModel.getProps();
		this.modelRelations = InternalOrderModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: InternalOrderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InternalOrderModel {
		let updatedModel: InternalOrderModel = new InternalOrderModel(model);
