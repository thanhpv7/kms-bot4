...
import {AdminInternalOrderCrudListComponent} from './internal-order-admin-crud-list.component';
...
import {AdminInternalOrderCrudModule} from '../internal-order-admin-crud.module';
import {
	InternalOrderModelState,
	initialState as initialInternalOrderModelState
} from 'src/app/models/internalOrder/internal_order.model.state';
 ...
 */
describe('Internal Order Crud List Tile Component', () => {

	let fixture;
	let internalOrderCrudListComponent: AdminInternalOrderCrudListComponent;

	let store: MockStore<{ model: InternalOrderModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let internalOrderModelState: InternalOrderModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InternalOrderModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ea917710-84ef-431f-a4e9-77e29a61e700';
	const searchContent = 'text';

	const searchableFieldCount = InternalOrderModel.searchFields.length;

	const routerState: RouterState = {
		url: 'internal-order-admin-crud',
		urls: ['internal-order-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const internalOrderDataFactory = new InternalOrderDataFactory();

	const defaultDataSize = 30;
	const defaultData = internalOrderDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InternalOrderModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		internalOrderModelState = initialInternalOrderModelState;
		store.setState({model: internalOrderModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInternalOrderCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInternalOrderCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminInternalOrderCrudListComponent);
			internalOrderCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(internalOrderCrudListComponent.internalOrdersCount).toBe(defaultData.length);
			...
			expect(internalOrderCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			internalOrderCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = internalOrderDataFactory.createAll(filteredDataSize);
			...
			expect(internalOrderCrudListComponent.internalOrdersCount).toBe(filteredDataSize);
			expect(internalOrderCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(internalOrderCrudListComponent.pageIndex).toBe(0);
			expect(internalOrderCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			internalOrderCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		internalOrderCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		internalOrderCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
