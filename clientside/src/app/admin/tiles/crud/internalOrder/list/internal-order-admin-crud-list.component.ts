...
import * as modelAction from 'src/app/models/internalOrder/internal_order.model.action';
import {InternalOrderModelState} from 'src/app/models/internalOrder/internal_order.model.state';
...
import {
	getInternalOrderCollectionModels,
	getInternalOrderCollectionCount
...
import {CrudTileMode} from '../internal-order-admin-crud.component';
...
@Component({
	selector: 'cb-internal-order-admin-crud-list',
	templateUrl: './internal-order-admin-crud-list.component.html',
	styleUrls: [
		'./internal-order-admin-crud-list.component.scss',
...
export class AdminInternalOrderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InternalOrderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InternalOrderModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = InternalOrderModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ea917710-84ef-431f-a4e9-77e29a61e700';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.internalOrdersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.internalOrdersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: InternalOrderModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InternalOrderAction(
				modelAction.InternalOrderModelActionTypes.INITIALISE_INTERNAL_ORDER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InternalOrderAction(
				modelAction.InternalOrderModelActionTypes.FETCH_INTERNAL_ORDER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.InternalOrderAction(
						modelAction.InternalOrderModelActionTypes.COUNT_INTERNAL_ORDERS,
						null
					)
				]
			))
		...
		this.internalOrders = this.store.select(getInternalOrderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInternalOrderCollectionCount, this.collectionId).subscribe(
				(count) => this.internalOrdersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InternalOrderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InternalOrderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InternalOrderModelActionTypes.EXPORT_ALL_INTERNAL_ORDER;
		} else {
			actionType = modelAction.InternalOrderModelActionTypes.EXPORT_INTERNAL_ORDER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InternalOrderAction(
				modelAction.InternalOrderModelActionTypes.IMPORT_INTERNAL_ORDERS,
				{
					file: file,
				},
				[new modelAction.InternalOrderAction(
			...
			window.open(environment.API_URL + '/api/internalorder/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InternalOrderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Internal Order entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Internal Order entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InternalOrderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InternalOrderModelActionTypes.DELETE_INTERNAL_ORDER;
						} else if (usingSelected) {
							actionType = modelAction.InternalOrderModelActionTypes.DELETE_ALL_INTERNAL_ORDER;
						} else {
							actionType = modelAction.InternalOrderModelActionTypes.DELETE_INTERNAL_ORDER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InternalOrderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INTERNAL_ORDER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INTERNAL_ORDER and DELETE_INTERNAL_ORDER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<InternalOrderModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InternalOrderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.internalOrdersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
