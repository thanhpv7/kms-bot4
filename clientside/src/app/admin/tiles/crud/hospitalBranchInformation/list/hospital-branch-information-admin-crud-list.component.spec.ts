...
import {AdminHospitalBranchInformationCrudListComponent} from './hospital-branch-information-admin-crud-list.component';
...
import {AdminHospitalBranchInformationCrudModule} from '../hospital-branch-information-admin-crud.module';
import {
 ...
 */
describe('Hospital Branch Information Crud List Tile Component', () => {

	let fixture;
	let hospitalBranchInformationCrudListComponent: AdminHospitalBranchInformationCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<HospitalBranchInformationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b665a6fc-57f3-42b2-a066-e40c6823b892';
	const searchContent = 'text';

	const searchableFieldCount = HospitalBranchInformationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'hospital-branch-information-admin-crud',
		urls: ['hospital-branch-information-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const hospitalBranchInformationDataFactory = new HospitalBranchInformationDataFactory();

	const defaultDataSize = 30;
	const defaultData = hospitalBranchInformationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getHospitalBranchInformationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getHospitalBranchInformationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminHospitalBranchInformationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminHospitalBranchInformationCrudListComponent);
			hospitalBranchInformationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(hospitalBranchInformationCrudListComponent.hospitalBranchInformationsCount).toBe(defaultData.length);
			...
			expect(hospitalBranchInformationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			hospitalBranchInformationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = hospitalBranchInformationDataFactory.createAll(filteredDataSize);
			...
			expect(hospitalBranchInformationCrudListComponent.hospitalBranchInformationsCount).toBe(filteredDataSize);
			expect(hospitalBranchInformationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(hospitalBranchInformationCrudListComponent.pageIndex).toBe(0);
			expect(hospitalBranchInformationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			hospitalBranchInformationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		hospitalBranchInformationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		hospitalBranchInformationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
