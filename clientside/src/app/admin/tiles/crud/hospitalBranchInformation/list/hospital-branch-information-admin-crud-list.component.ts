...
import {HospitalBranchInformationModelState} from 'src/app/models/hospitalBranchInformation/hospital_branch_information.model.state';
...
import {
	...
	getHospitalBranchInformationCollectionCount
...
@Component({
	selector: 'cb-hospital-branch-information-admin-crud-list',
	templateUrl: './hospital-branch-information-admin-crud-list.component.html',
	styleUrls: [
		'./hospital-branch-information-admin-crud-list.component.scss',
...
export class AdminHospitalBranchInformationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<HospitalBranchInformationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...HospitalBranchInformationModel.searchFields,
	...
	/**
	 * List of Hospital Branch Informations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	hospitalBranchInformations: Observable<HospitalBranchInformationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	hospitalBranchInformationsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.hospitalBranchInformationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.hospitalBranchInformationsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HospitalBranchInformationAction(
				modelAction.HospitalBranchInformationModelActionTypes.INITIALISE_HOSPITAL_BRANCH_INFORMATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HospitalBranchInformationAction(
				modelAction.HospitalBranchInformationModelActionTypes.FETCH_HOSPITAL_BRANCH_INFORMATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.hospitalBranchInformations = this.store.select(getHospitalBranchInformationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getHospitalBranchInformationCollectionCount, this.collectionId).subscribe(
				(count) => this.hospitalBranchInformationsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: HospitalBranchInformationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.HospitalBranchInformationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.HospitalBranchInformationModelActionTypes.EXPORT_ALL_HOSPITAL_BRANCH_INFORMATION;
		} else {
			actionType = modelAction.HospitalBranchInformationModelActionTypes.EXPORT_HOSPITAL_BRANCH_INFORMATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.HospitalBranchInformationAction(
				modelAction.HospitalBranchInformationModelActionTypes.IMPORT_HOSPITAL_BRANCH_INFORMATIONS,
				{
					file: file,
				},
				[new modelAction.HospitalBranchInformationAction(
			...
			window.open(environment.API_URL + '/api/hospitalbranchinformation/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: HospitalBranchInformationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Hospital Branch Information entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Hospital Branch Information entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.HospitalBranchInformationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.HospitalBranchInformationModelActionTypes.DELETE_HOSPITAL_BRANCH_INFORMATION;
						} else if (usingSelected) {
							actionType = modelAction.HospitalBranchInformationModelActionTypes.DELETE_ALL_HOSPITAL_BRANCH_INFORMATION;
						} else {
							actionType = modelAction.HospitalBranchInformationModelActionTypes.DELETE_HOSPITAL_BRANCH_INFORMATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.HospitalBranchInformationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_HOSPITAL_BRANCH_INFORMATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_HOSPITAL_BRANCH_INFORMATION and DELETE_HOSPITAL_BRANCH_INFORMATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = HospitalBranchInformationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.hospitalBranchInformationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
