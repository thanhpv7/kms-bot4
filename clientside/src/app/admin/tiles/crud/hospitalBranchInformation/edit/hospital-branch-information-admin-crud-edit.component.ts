...
import * as modelAction from 'src/app/models/hospitalBranchInformation/hospital_branch_information.model.action';
import {HospitalBranchInformationModelState, HospitalBranchInformationModelAudit} from 'src/app/models/hospitalBranchInformation/hospital_branch_information.model.state';
import {HospitalBranchInformationModel} from 'src/app/models/hospitalBranchInformation/hospital_branch_information.model';
import {
	getHospitalBranchInformationModelAuditsByEntityId,
	getHospitalBranchInformationModelWithId,
	getCountHospitalBranchInformationModels,
...
import * as hospitalGroupInformationModelAction from 'src/app/models/hospitalGroupInformation/hospital_group_information.model.action';
import {
	getHospitalGroupInformationCollectionModels, getHospitalGroupInformationModelWithId,
...
import {CrudTileMode} from '../hospital-branch-information-admin-crud.component';
...
@Component({
	selector: 'cb-hospital-branch-information-admin-crud-edit',
	templateUrl: './hospital-branch-information-admin-crud-edit.component.html',
	styleUrls: [
		'./hospital-branch-information-admin-crud-edit.component.scss',
...
export class AdminHospitalBranchInformationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b665a6fc-57f3-42b2-a066-e40c6823b892';
	 ...
	 */
	hospitalBranchInformationAudits$: Observable<HospitalBranchInformationModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Hospital Group and Hospital Branch Informations here] off begin
	hospitalGroupMultiCrudActive: boolean = false;
	hospitalGroupDisplayName = 'Hospital Group';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Hospital Group and Hospital Branch Informations here] end
	hospitalGroupOppositeRelation = 'hospitalBranchInformations';
	hospitalGroupRelations: ModelRelation[] = this.hospitalGroupMultiCrudActive ? HospitalGroupInformationModel.getRelations()
		.map(relation => {
			if (relation.id === this.hospitalGroupOppositeRelation) {
	...
	staffOppositeRelation = 'hospitalBranchInformations';
	 ...
	 */
	modelProperties: ModelProperty[] = HospitalBranchInformationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = HospitalBranchInformationModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = HospitalBranchInformationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'hospitalGroup' && this.hospitalGroupMultiCrudActive) {
					exp.fields = HospitalGroupInformationModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: HospitalBranchInformationModel;
	...
	constructor(
		private readonly store: Store<{ model: HospitalBranchInformationModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountHospitalBranchInformationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new HospitalBranchInformationModel();
		...
		if (this.targetModelId) {
			this.hospitalBranchInformationAudits$ = this.store.select(getHospitalBranchInformationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<HospitalBranchInformationModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.HospitalBranchInformationAction(
			modelAction.HospitalBranchInformationModelActionTypes.FETCH_HOSPITAL_BRANCH_INFORMATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getHospitalBranchInformationModelWithId, this.targetModelId)
			...
			model = new HospitalBranchInformationModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('hospital-branch-information');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.HospitalBranchInformationAction(
			modelAction.HospitalBranchInformationModelActionTypes.FETCH_HOSPITAL_BRANCH_INFORMATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<HospitalBranchInformationModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.hospitalGroupMultiCrudActive || this.staffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.HospitalBranchInformationAction(
						modelAction.HospitalBranchInformationModelActionTypes.COUNT_HOSPITAL_BRANCH_INFORMATIONS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.HospitalBranchInformationAction(
				modelAction.HospitalBranchInformationModelActionTypes.CREATE_HOSPITAL_BRANCH_INFORMATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.HospitalBranchInformationAction(
				modelAction.HospitalBranchInformationModelActionTypes.UPDATE_HOSPITAL_BRANCH_INFORMATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: HospitalBranchInformationModel = new HospitalBranchInformationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getHospitalBranchInformationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new HospitalBranchInformationModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.hospitalGroupDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(HospitalGroupInformationModel.getProps(), this.hospitalGroupRelations, this.isViewOnly),
					new HospitalGroupInformationModel(),
					HospitalGroupInformationModel,
					this.hospitalGroupDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.hospitalGroupRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = HospitalBranchInformationModel.getProps();
		this.modelRelations = HospitalBranchInformationModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'hospitalGroup';
		}).hideElement = this.hospitalGroupMultiCrudActive || this.modelRelations.find(rel => rel.id === 'hospitalGroup').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.hospitalGroupMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.hospitalGroupDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.hospitalGroup && this.targetModel.hospitalGroup.id && this.hospitalGroupMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				HospitalGroupInformationModel,
				this.hospitalGroupOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.hospitalGroup.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getHospitalGroupInformationModelWithId, this.targetModel.hospitalGroupId).subscribe(hospitalGroup => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(HospitalGroupInformationModel.getProps(), this.hospitalGroupRelations, this.isViewOnly),
					hospitalGroup,
					HospitalGroupInformationModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: HospitalBranchInformationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): HospitalBranchInformationModel {
		let updatedModel: HospitalBranchInformationModel = new HospitalBranchInformationModel(model);

		if (this.hospitalGroupMultiCrudActive) {
			updatedModel.hospitalGroup = event.additionalEntities.hospitalGroup ? event.additionalEntities.hospitalGroup : undefined;

			if (!updatedModel.hospitalGroup && event.payload) {
				event.payload.hospitalGroupId = null;
			}
		} else if (event.payload?.hospitalGroupId !== undefined) {
			event.payload.hospitalGroup = null;
			updatedModel.hospitalGroup = null;
		}
