...
import {AdminInsuranceBenefitPlanCrudListComponent} from './insurance-benefit-plan-admin-crud-list.component';
...
import {AdminInsuranceBenefitPlanCrudModule} from '../insurance-benefit-plan-admin-crud.module';
import {
 ...
 */
describe('Insurance Benefit Plan Crud List Tile Component', () => {

	let fixture;
	let insuranceBenefitPlanCrudListComponent: AdminInsuranceBenefitPlanCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<InsuranceBenefitPlanModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b9ebc553-442e-4399-9cf5-126b71a2d563';
	const searchContent = 'text';

	const searchableFieldCount = InsuranceBenefitPlanModel.searchFields.length;

	const routerState: RouterState = {
		url: 'insurance-benefit-plan-admin-crud',
		urls: ['insurance-benefit-plan-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const insuranceBenefitPlanDataFactory = new InsuranceBenefitPlanDataFactory();

	const defaultDataSize = 30;
	const defaultData = insuranceBenefitPlanDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getInsuranceBenefitPlanCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInsuranceBenefitPlanCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInsuranceBenefitPlanCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInsuranceBenefitPlanCrudListComponent);
			insuranceBenefitPlanCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(insuranceBenefitPlanCrudListComponent.insuranceBenefitPlansCount).toBe(defaultData.length);
			...
			expect(insuranceBenefitPlanCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			insuranceBenefitPlanCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = insuranceBenefitPlanDataFactory.createAll(filteredDataSize);
			...
			expect(insuranceBenefitPlanCrudListComponent.insuranceBenefitPlansCount).toBe(filteredDataSize);
			expect(insuranceBenefitPlanCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(insuranceBenefitPlanCrudListComponent.pageIndex).toBe(0);
			expect(insuranceBenefitPlanCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			insuranceBenefitPlanCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		insuranceBenefitPlanCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		insuranceBenefitPlanCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
