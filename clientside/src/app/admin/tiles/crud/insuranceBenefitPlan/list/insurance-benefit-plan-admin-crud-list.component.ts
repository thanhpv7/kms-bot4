...
import {InsuranceBenefitPlanModelState} from 'src/app/models/insuranceBenefitPlan/insurance_benefit_plan.model.state';
...
import {
	...
	getInsuranceBenefitPlanCollectionCount
...
@Component({
	selector: 'cb-insurance-benefit-plan-admin-crud-list',
	templateUrl: './insurance-benefit-plan-admin-crud-list.component.html',
	styleUrls: [
		'./insurance-benefit-plan-admin-crud-list.component.scss',
...
export class AdminInsuranceBenefitPlanCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InsuranceBenefitPlanModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InsuranceBenefitPlanModel.searchFields,
	...
	/**
	 * List of Insurance Benefit Plans that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	insuranceBenefitPlans: Observable<InsuranceBenefitPlanModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	insuranceBenefitPlansCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.insuranceBenefitPlansCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.insuranceBenefitPlansCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InsuranceBenefitPlanAction(
				modelAction.InsuranceBenefitPlanModelActionTypes.INITIALISE_INSURANCE_BENEFIT_PLAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InsuranceBenefitPlanAction(
				modelAction.InsuranceBenefitPlanModelActionTypes.FETCH_INSURANCE_BENEFIT_PLAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.insuranceBenefitPlans = this.store.select(getInsuranceBenefitPlanCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInsuranceBenefitPlanCollectionCount, this.collectionId).subscribe(
				(count) => this.insuranceBenefitPlansCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InsuranceBenefitPlanModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InsuranceBenefitPlanModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InsuranceBenefitPlanModelActionTypes.EXPORT_ALL_INSURANCE_BENEFIT_PLAN;
		} else {
			actionType = modelAction.InsuranceBenefitPlanModelActionTypes.EXPORT_INSURANCE_BENEFIT_PLAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InsuranceBenefitPlanAction(
				modelAction.InsuranceBenefitPlanModelActionTypes.IMPORT_INSURANCE_BENEFIT_PLANS,
				{
					file: file,
				},
				[new modelAction.InsuranceBenefitPlanAction(
			...
			window.open(environment.API_URL + '/api/insurancebenefitplan/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InsuranceBenefitPlanModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Insurance Benefit Plan entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Insurance Benefit Plan entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InsuranceBenefitPlanModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InsuranceBenefitPlanModelActionTypes.DELETE_INSURANCE_BENEFIT_PLAN;
						} else if (usingSelected) {
							actionType = modelAction.InsuranceBenefitPlanModelActionTypes.DELETE_ALL_INSURANCE_BENEFIT_PLAN;
						} else {
							actionType = modelAction.InsuranceBenefitPlanModelActionTypes.DELETE_INSURANCE_BENEFIT_PLAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InsuranceBenefitPlanAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INSURANCE_BENEFIT_PLAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INSURANCE_BENEFIT_PLAN and DELETE_INSURANCE_BENEFIT_PLAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InsuranceBenefitPlanModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.insuranceBenefitPlansCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
