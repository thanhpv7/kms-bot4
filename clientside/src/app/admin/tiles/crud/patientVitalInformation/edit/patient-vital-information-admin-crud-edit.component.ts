...
import {PatientVitalInformationModelState, PatientVitalInformationModelAudit} from 'src/app/models/patientVitalInformation/patient_vital_information.model.state';
import {PatientVitalInformationModel} from 'src/app/models/patientVitalInformation/patient_vital_information.model';
import {
	getPatientVitalInformationModelAuditsByEntityId,
	getPatientVitalInformationModelWithId,
	getCountPatientVitalInformationModels,
...
@Component({
	selector: 'cb-patient-vital-information-admin-crud-edit',
	templateUrl: './patient-vital-information-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-vital-information-admin-crud-edit.component.scss',
...
export class AdminPatientVitalInformationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	patientVitalInformationAudits$: Observable<PatientVitalInformationModelAudit[]>;
	...
	registrationOppositeRelation = 'patientVitalInformations';
	...
	staffOppositeRelation = 'patientVitalInformations';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientVitalInformationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientVitalInformationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientVitalInformationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientVitalInformationModel();
		...
		if (this.targetModelId) {
			this.patientVitalInformationAudits$ = this.store.select(getPatientVitalInformationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientVitalInformationModel> = {
		...
		this.store.dispatch(new modelAction.PatientVitalInformationAction(
			modelAction.PatientVitalInformationModelActionTypes.FETCH_PATIENT_VITAL_INFORMATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientVitalInformationModelWithId, this.targetModelId)
			...
			model = new PatientVitalInformationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientVitalInformationAction(
			modelAction.PatientVitalInformationModelActionTypes.FETCH_PATIENT_VITAL_INFORMATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientVitalInformationModel> = {
				...
				queryParams: {
					persistRelations: this.registrationMultiCrudActive || this.staffMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.PatientVitalInformationAction(
				modelAction.PatientVitalInformationModelActionTypes.CREATE_PATIENT_VITAL_INFORMATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientVitalInformationAction(
				modelAction.PatientVitalInformationModelActionTypes.UPDATE_PATIENT_VITAL_INFORMATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientVitalInformationModel = new PatientVitalInformationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientVitalInformationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientVitalInformationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientVitalInformationModel.getProps();
		this.modelRelations = PatientVitalInformationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientVitalInformationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientVitalInformationModel {
		let updatedModel: PatientVitalInformationModel = new PatientVitalInformationModel(model);
