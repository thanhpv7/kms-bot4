...
import {AdminPatientVitalInformationCrudListComponent} from './patient-vital-information-admin-crud-list.component';
...
import {AdminPatientVitalInformationCrudModule} from '../patient-vital-information-admin-crud.module';
import {
 ...
 */
describe('Patient Vital Information Crud List Tile Component', () => {

	let fixture;
	let patientVitalInformationCrudListComponent: AdminPatientVitalInformationCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientVitalInformationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '2efd8bff-c954-4389-9ec1-26afbeb282b1';
	const searchContent = 'text';

	const searchableFieldCount = PatientVitalInformationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-vital-information-admin-crud',
		urls: ['patient-vital-information-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientVitalInformationDataFactory = new PatientVitalInformationDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientVitalInformationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPatientVitalInformationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientVitalInformationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientVitalInformationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientVitalInformationCrudListComponent);
			patientVitalInformationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientVitalInformationCrudListComponent.patientVitalInformationsCount).toBe(defaultData.length);
			...
			expect(patientVitalInformationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientVitalInformationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientVitalInformationDataFactory.createAll(filteredDataSize);
			...
			expect(patientVitalInformationCrudListComponent.patientVitalInformationsCount).toBe(filteredDataSize);
			expect(patientVitalInformationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientVitalInformationCrudListComponent.pageIndex).toBe(0);
			expect(patientVitalInformationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientVitalInformationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientVitalInformationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientVitalInformationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
