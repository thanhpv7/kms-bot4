...
import * as modelAction from 'src/app/models/patientVitalInformation/patient_vital_information.model.action';
import {PatientVitalInformationModelState} from 'src/app/models/patientVitalInformation/patient_vital_information.model.state';
...
import {
	getPatientVitalInformationCollectionModels,
	getPatientVitalInformationCollectionCount
...
import {CrudTileMode} from '../patient-vital-information-admin-crud.component';
...
@Component({
	selector: 'cb-patient-vital-information-admin-crud-list',
	templateUrl: './patient-vital-information-admin-crud-list.component.html',
	styleUrls: [
		'./patient-vital-information-admin-crud-list.component.scss',
...
export class AdminPatientVitalInformationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientVitalInformationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientVitalInformationModel.searchFields,
	...
	/**
	 * List of Patient Vital Informations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientVitalInformations: Observable<PatientVitalInformationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientVitalInformationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PatientVitalInformationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientVitalInformationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '2efd8bff-c954-4389-9ec1-26afbeb282b1';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientVitalInformationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientVitalInformationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PatientVitalInformationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVitalInformationAction(
				modelAction.PatientVitalInformationModelActionTypes.INITIALISE_PATIENT_VITAL_INFORMATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVitalInformationAction(
				modelAction.PatientVitalInformationModelActionTypes.FETCH_PATIENT_VITAL_INFORMATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PatientVitalInformationAction(
						modelAction.PatientVitalInformationModelActionTypes.COUNT_PATIENT_VITAL_INFORMATIONS,
						null
					)
				]
			))
		...
		this.patientVitalInformations = this.store.select(getPatientVitalInformationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientVitalInformationCollectionCount, this.collectionId).subscribe(
				(count) => this.patientVitalInformationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-vital-information');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientVitalInformationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientVitalInformationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientVitalInformationModelActionTypes.EXPORT_ALL_PATIENT_VITAL_INFORMATION;
		} else {
			actionType = modelAction.PatientVitalInformationModelActionTypes.EXPORT_PATIENT_VITAL_INFORMATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientVitalInformationAction(
				modelAction.PatientVitalInformationModelActionTypes.IMPORT_PATIENT_VITAL_INFORMATIONS,
				{
					file: file,
				},
				[new modelAction.PatientVitalInformationAction(
			...
			window.open(environment.API_URL + '/api/patientvitalinformation/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientVitalInformationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Vital Information entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Vital Information entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientVitalInformationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientVitalInformationModelActionTypes.DELETE_PATIENT_VITAL_INFORMATION;
						} else if (usingSelected) {
							actionType = modelAction.PatientVitalInformationModelActionTypes.DELETE_ALL_PATIENT_VITAL_INFORMATION;
						} else {
							actionType = modelAction.PatientVitalInformationModelActionTypes.DELETE_PATIENT_VITAL_INFORMATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientVitalInformationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_VITAL_INFORMATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_VITAL_INFORMATION and DELETE_PATIENT_VITAL_INFORMATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PatientVitalInformationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientVitalInformationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientVitalInformationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
