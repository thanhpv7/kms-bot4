...
import {AdminCompoundPrescriptionItemCrudListComponent} from './compound-prescription-item-admin-crud-list.component';
...
import {AdminCompoundPrescriptionItemCrudModule} from '../compound-prescription-item-admin-crud.module';
import {
	CompoundPrescriptionItemModelState,
	initialState as initialCompoundPrescriptionItemModelState
} from 'src/app/models/compoundPrescriptionItem/compound_prescription_item.model.state';
 ...
 */
describe('Compound Prescription Item Crud List Tile Component', () => {

	let fixture;
	let compoundPrescriptionItemCrudListComponent: AdminCompoundPrescriptionItemCrudListComponent;

	let store: MockStore<{ model: CompoundPrescriptionItemModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let compoundPrescriptionItemModelState: CompoundPrescriptionItemModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<CompoundPrescriptionItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '9d467fed-a156-4966-85be-ba9585b37240';
	const searchContent = 'text';

	const searchableFieldCount = CompoundPrescriptionItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'compound-prescription-item-admin-crud',
		urls: ['compound-prescription-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const compoundPrescriptionItemDataFactory = new CompoundPrescriptionItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = compoundPrescriptionItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: CompoundPrescriptionItemModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		compoundPrescriptionItemModelState = initialCompoundPrescriptionItemModelState;
		store.setState({model: compoundPrescriptionItemModelState});
		...
		spyOn(store, 'select')
			.withArgs(getCompoundPrescriptionItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCompoundPrescriptionItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminCompoundPrescriptionItemCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminCompoundPrescriptionItemCrudListComponent);
			compoundPrescriptionItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(compoundPrescriptionItemCrudListComponent.compoundPrescriptionItemsCount).toBe(defaultData.length);
			...
			expect(compoundPrescriptionItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			compoundPrescriptionItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = compoundPrescriptionItemDataFactory.createAll(filteredDataSize);
			...
			expect(compoundPrescriptionItemCrudListComponent.compoundPrescriptionItemsCount).toBe(filteredDataSize);
			expect(compoundPrescriptionItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(compoundPrescriptionItemCrudListComponent.pageIndex).toBe(0);
			expect(compoundPrescriptionItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			compoundPrescriptionItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		compoundPrescriptionItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		compoundPrescriptionItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
