...
import {CompoundPrescriptionItemModelState} from 'src/app/models/compoundPrescriptionItem/compound_prescription_item.model.state';
...
import {
	getCompoundPrescriptionItemCollectionModels,
	getCompoundPrescriptionItemCollectionCount
...
@Component({
	selector: 'cb-compound-prescription-item-admin-crud-list',
	templateUrl: './compound-prescription-item-admin-crud-list.component.html',
	styleUrls: [
		'./compound-prescription-item-admin-crud-list.component.scss',
...
export class AdminCompoundPrescriptionItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CompoundPrescriptionItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CompoundPrescriptionItemModel.searchFields,
	...
	/**
	 * List of Compound Prescription Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	compoundPrescriptionItems: Observable<CompoundPrescriptionItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	compoundPrescriptionItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.compoundPrescriptionItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.compoundPrescriptionItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
				modelAction.CompoundPrescriptionItemModelActionTypes.INITIALISE_COMPOUND_PRESCRIPTION_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
				modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.compoundPrescriptionItems = this.store.select(getCompoundPrescriptionItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCompoundPrescriptionItemCollectionCount, this.collectionId).subscribe(
				(count) => this.compoundPrescriptionItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CompoundPrescriptionItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CompoundPrescriptionItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CompoundPrescriptionItemModelActionTypes.EXPORT_ALL_COMPOUND_PRESCRIPTION_ITEM;
		} else {
			actionType = modelAction.CompoundPrescriptionItemModelActionTypes.EXPORT_COMPOUND_PRESCRIPTION_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
				modelAction.CompoundPrescriptionItemModelActionTypes.IMPORT_COMPOUND_PRESCRIPTION_ITEMS,
				{
					file: file,
				},
				[new modelAction.CompoundPrescriptionItemAction(
			...
			window.open(environment.API_URL + '/api/compoundprescriptionitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CompoundPrescriptionItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Compound Prescription Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Compound Prescription Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CompoundPrescriptionItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CompoundPrescriptionItemModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.CompoundPrescriptionItemModelActionTypes.DELETE_ALL_COMPOUND_PRESCRIPTION_ITEM;
						} else {
							actionType = modelAction.CompoundPrescriptionItemModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_COMPOUND_PRESCRIPTION_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_COMPOUND_PRESCRIPTION_ITEM and DELETE_COMPOUND_PRESCRIPTION_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CompoundPrescriptionItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.compoundPrescriptionItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
