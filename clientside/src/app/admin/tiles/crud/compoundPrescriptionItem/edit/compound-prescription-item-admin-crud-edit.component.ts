...
import * as modelAction from 'src/app/models/compoundPrescriptionItem/compound_prescription_item.model.action';
import {CompoundPrescriptionItemModelState, CompoundPrescriptionItemModelAudit} from 'src/app/models/compoundPrescriptionItem/compound_prescription_item.model.state';
import {CompoundPrescriptionItemModel} from 'src/app/models/compoundPrescriptionItem/compound_prescription_item.model';
import {
	getCompoundPrescriptionItemModelAuditsByEntityId,
	getCompoundPrescriptionItemModelWithId,
	getCountCompoundPrescriptionItemModels,
...
import {CrudTileMode} from '../compound-prescription-item-admin-crud.component';
...
@Component({
	selector: 'cb-compound-prescription-item-admin-crud-edit',
	templateUrl: './compound-prescription-item-admin-crud-edit.component.html',
	styleUrls: [
		'./compound-prescription-item-admin-crud-edit.component.scss',
...
export class AdminCompoundPrescriptionItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '9d467fed-a156-4966-85be-ba9585b37240';
	 ...
	 */
	compoundPrescriptionItemAudits$: Observable<CompoundPrescriptionItemModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Compound Prescription Detail and Compound Prescription Items here] off begin
	compoundPrescriptionDetailMultiCrudActive: boolean = false;
	compoundPrescriptionDetailDisplayName = 'Compound Prescription Detail';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Compound Prescription Detail and Compound Prescription Items here] end
	compoundPrescriptionDetailOppositeRelation = 'compoundPrescriptionItems';
	compoundPrescriptionDetailRelations: ModelRelation[] = this.compoundPrescriptionDetailMultiCrudActive ? CompoundPrescriptionDetailModel.getRelations()
		.map(relation => {
			if (relation.id === this.compoundPrescriptionDetailOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'compoundPrescriptionItems';
	...
	unitOppositeRelation = 'compoundPrescriptionItems';
	 ...
	 */
	modelProperties: ModelProperty[] = CompoundPrescriptionItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = CompoundPrescriptionItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CompoundPrescriptionItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'compoundPrescriptionDetail' && this.compoundPrescriptionDetailMultiCrudActive) {
	 ...
	 */
	targetModel: CompoundPrescriptionItemModel;
	...
	constructor(
		private readonly store: Store<{ model: CompoundPrescriptionItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCompoundPrescriptionItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CompoundPrescriptionItemModel();
		...
		if (this.targetModelId) {
			this.compoundPrescriptionItemAudits$ = this.store.select(getCompoundPrescriptionItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CompoundPrescriptionItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
			modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCompoundPrescriptionItemModelWithId, this.targetModelId)
			...
			model = new CompoundPrescriptionItemModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('compound-prescription-item');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
			modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<CompoundPrescriptionItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.compoundPrescriptionDetailMultiCrudActive || this.stockCatalogueMultiCrudActive || this.unitMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.CompoundPrescriptionItemAction(
						modelAction.CompoundPrescriptionItemModelActionTypes.COUNT_COMPOUND_PRESCRIPTION_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
				modelAction.CompoundPrescriptionItemModelActionTypes.CREATE_COMPOUND_PRESCRIPTION_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CompoundPrescriptionItemAction(
				modelAction.CompoundPrescriptionItemModelActionTypes.UPDATE_COMPOUND_PRESCRIPTION_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CompoundPrescriptionItemModel = new CompoundPrescriptionItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCompoundPrescriptionItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CompoundPrescriptionItemModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.compoundPrescriptionDetailDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(CompoundPrescriptionDetailModel.getProps(), this.compoundPrescriptionDetailRelations, this.isViewOnly),
					new CompoundPrescriptionDetailModel(),
					CompoundPrescriptionDetailModel,
					this.compoundPrescriptionDetailDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.compoundPrescriptionDetailRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CompoundPrescriptionItemModel.getProps();
		this.modelRelations = CompoundPrescriptionItemModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'compoundPrescriptionDetail';
		}).hideElement = this.compoundPrescriptionDetailMultiCrudActive || this.modelRelations.find(rel => rel.id === 'compoundPrescriptionDetail').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.compoundPrescriptionDetailMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.compoundPrescriptionDetailDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.compoundPrescriptionDetail && this.targetModel.compoundPrescriptionDetail.id && this.compoundPrescriptionDetailMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				CompoundPrescriptionDetailModel,
				this.compoundPrescriptionDetailOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.compoundPrescriptionDetail.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getCompoundPrescriptionDetailModelWithId, this.targetModel.compoundPrescriptionDetailId).subscribe(compoundPrescriptionDetail => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(CompoundPrescriptionDetailModel.getProps(), this.compoundPrescriptionDetailRelations, this.isViewOnly),
	 ...
	 */
	addAdditionalEntitiesToModel(model: CompoundPrescriptionItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CompoundPrescriptionItemModel {
		let updatedModel: CompoundPrescriptionItemModel = new CompoundPrescriptionItemModel(model);

		if (this.compoundPrescriptionDetailMultiCrudActive) {
			updatedModel.compoundPrescriptionDetail = event.additionalEntities.compoundPrescriptionDetail ? event.additionalEntities.compoundPrescriptionDetail : undefined;

			if (!updatedModel.compoundPrescriptionDetail && event.payload) {
				event.payload.compoundPrescriptionDetailId = null;
			}
		} else if (event.payload?.compoundPrescriptionDetailId !== undefined) {
			event.payload.compoundPrescriptionDetail = null;
			updatedModel.compoundPrescriptionDetail = null;
		}
