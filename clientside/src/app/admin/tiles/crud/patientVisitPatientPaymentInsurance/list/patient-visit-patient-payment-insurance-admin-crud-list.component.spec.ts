...
import {AdminPatientVisitPatientPaymentInsuranceCrudListComponent} from './patient-visit-patient-payment-insurance-admin-crud-list.component';
...
import {AdminPatientVisitPatientPaymentInsuranceCrudModule} from '../patient-visit-patient-payment-insurance-admin-crud.module';
import {
	PatientVisitPatientPaymentInsuranceModelState,
	initialState as initialPatientVisitPatientPaymentInsuranceModelState
} from 'src/app/models/patientVisitPatientPaymentInsurance/patient_visit_patient_payment_insurance.model.state';
 ...
 */
describe('Patient Visit Patient Payment Insurance Crud List Tile Component', () => {

	let fixture;
	let patientVisitPatientPaymentInsuranceCrudListComponent: AdminPatientVisitPatientPaymentInsuranceCrudListComponent;

	let store: MockStore<{ model: PatientVisitPatientPaymentInsuranceModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientVisitPatientPaymentInsuranceModelState: PatientVisitPatientPaymentInsuranceModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientVisitPatientPaymentInsuranceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'bdaee1a8-2f94-448f-84a1-be7c05183399';
	const searchContent = 'text';

	const searchableFieldCount = PatientVisitPatientPaymentInsuranceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-visit-patient-payment-insurance-admin-crud',
		urls: ['patient-visit-patient-payment-insurance-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientVisitPatientPaymentInsuranceDataFactory = new PatientVisitPatientPaymentInsuranceDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientVisitPatientPaymentInsuranceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientVisitPatientPaymentInsuranceModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientVisitPatientPaymentInsuranceModelState = initialPatientVisitPatientPaymentInsuranceModelState;
		store.setState({model: patientVisitPatientPaymentInsuranceModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientVisitPatientPaymentInsuranceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientVisitPatientPaymentInsuranceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientVisitPatientPaymentInsuranceCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientVisitPatientPaymentInsuranceCrudListComponent);
			patientVisitPatientPaymentInsuranceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientVisitPatientPaymentInsuranceCrudListComponent.patientVisitPatientPaymentInsurancesCount).toBe(defaultData.length);
			...
			expect(patientVisitPatientPaymentInsuranceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientVisitPatientPaymentInsuranceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientVisitPatientPaymentInsuranceDataFactory.createAll(filteredDataSize);
			...
			expect(patientVisitPatientPaymentInsuranceCrudListComponent.patientVisitPatientPaymentInsurancesCount).toBe(filteredDataSize);
			expect(patientVisitPatientPaymentInsuranceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientVisitPatientPaymentInsuranceCrudListComponent.pageIndex).toBe(0);
			expect(patientVisitPatientPaymentInsuranceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientVisitPatientPaymentInsuranceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientVisitPatientPaymentInsuranceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientVisitPatientPaymentInsuranceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
