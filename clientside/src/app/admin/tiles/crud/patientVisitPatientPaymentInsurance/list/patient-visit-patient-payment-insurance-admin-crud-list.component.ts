...
import {PatientVisitPatientPaymentInsuranceModelState} from 'src/app/models/patientVisitPatientPaymentInsurance/patient_visit_patient_payment_insurance.model.state';
...
import {
	getPatientVisitPatientPaymentInsuranceCollectionModels,
	getPatientVisitPatientPaymentInsuranceCollectionCount
...
@Component({
	selector: 'cb-patient-visit-patient-payment-insurance-admin-crud-list',
	templateUrl: './patient-visit-patient-payment-insurance-admin-crud-list.component.html',
	styleUrls: [
		'./patient-visit-patient-payment-insurance-admin-crud-list.component.scss',
...
export class AdminPatientVisitPatientPaymentInsuranceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientVisitPatientPaymentInsuranceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientVisitPatientPaymentInsuranceModel.searchFields,
	...
	/**
	 * List of Patient Visit Patient Payment Insurances that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientVisitPatientPaymentInsurances: Observable<PatientVisitPatientPaymentInsuranceModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientVisitPatientPaymentInsurancesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientVisitPatientPaymentInsurancesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientVisitPatientPaymentInsurancesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
				modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.INITIALISE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
				modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientVisitPatientPaymentInsurances = this.store.select(getPatientVisitPatientPaymentInsuranceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientVisitPatientPaymentInsuranceCollectionCount, this.collectionId).subscribe(
				(count) => this.patientVisitPatientPaymentInsurancesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientVisitPatientPaymentInsuranceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.EXPORT_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE;
		} else {
			actionType = modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.EXPORT_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
				modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.IMPORT_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCES,
				{
					file: file,
				},
				[new modelAction.PatientVisitPatientPaymentInsuranceAction(
			...
			window.open(environment.API_URL + '/api/patientvisitpatientpaymentinsurance/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientVisitPatientPaymentInsuranceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit Patient Payment Insurance entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit Patient Payment Insurance entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE;
						} else if (usingSelected) {
							actionType = modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.DELETE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE;
						} else {
							actionType = modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE and DELETE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientVisitPatientPaymentInsuranceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientVisitPatientPaymentInsurancesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
