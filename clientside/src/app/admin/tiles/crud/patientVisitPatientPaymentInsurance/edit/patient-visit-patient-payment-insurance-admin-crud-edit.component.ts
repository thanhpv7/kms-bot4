...
import * as modelAction from 'src/app/models/patientVisitPatientPaymentInsurance/patient_visit_patient_payment_insurance.model.action';
import {PatientVisitPatientPaymentInsuranceModelState, PatientVisitPatientPaymentInsuranceModelAudit} from 'src/app/models/patientVisitPatientPaymentInsurance/patient_visit_patient_payment_insurance.model.state';
...
import {
	getPatientVisitPatientPaymentInsuranceModelAuditsByEntityId,
	getPatientVisitPatientPaymentInsuranceModelWithId,
	getCountPatientVisitPatientPaymentInsuranceModels,
...
import {CrudTileMode} from '../patient-visit-patient-payment-insurance-admin-crud.component';
...
@Component({
	selector: 'cb-patient-visit-patient-payment-insurance-admin-crud-edit',
	templateUrl: './patient-visit-patient-payment-insurance-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-visit-patient-payment-insurance-admin-crud-edit.component.scss',
...
export class AdminPatientVisitPatientPaymentInsuranceCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'bdaee1a8-2f94-448f-84a1-be7c05183399';
	 ...
	 */
	patientVisitPatientPaymentInsuranceAudits$: Observable<PatientVisitPatientPaymentInsuranceModelAudit[]>;
	...
	patientVisitOppositeRelation = 'patientVisitPatientPaymentInsurance';
	...
	insuranceBenefitPlanOppositeRelation = 'patientVisitPatientPaymentInsurances';
	 ...
	 */
	modelProperties: ModelProperty[] = PatientVisitPatientPaymentInsuranceModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientVisitPatientPaymentInsuranceModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientVisitPatientPaymentInsuranceModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientVisitPatientPaymentInsuranceModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientVisitPatientPaymentInsuranceModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientVisitPatientPaymentInsuranceModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientVisitPatientPaymentInsuranceModel();
		...
		if (this.targetModelId) {
			this.patientVisitPatientPaymentInsuranceAudits$ = this.store.select(getPatientVisitPatientPaymentInsuranceModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientVisitPatientPaymentInsuranceModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
			modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientVisitPatientPaymentInsuranceModelWithId, this.targetModelId)
			...
			model = new PatientVisitPatientPaymentInsuranceModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-visit-patient-payment-insurance');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
			modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientVisitPatientPaymentInsuranceModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.patientVisitMultiCrudActive || this.insuranceBenefitPlanMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientVisitPatientPaymentInsuranceAction(
						modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.COUNT_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
				modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.CREATE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientVisitPatientPaymentInsuranceAction(
				modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.UPDATE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientVisitPatientPaymentInsuranceModel = new PatientVisitPatientPaymentInsuranceModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientVisitPatientPaymentInsuranceModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientVisitPatientPaymentInsuranceModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientVisitPatientPaymentInsuranceModel.getProps();
		this.modelRelations = PatientVisitPatientPaymentInsuranceModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientVisitPatientPaymentInsuranceModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientVisitPatientPaymentInsuranceModel {
		let updatedModel: PatientVisitPatientPaymentInsuranceModel = new PatientVisitPatientPaymentInsuranceModel(model);
