...
import {AdminPcareCompoundDrugItemCrudListComponent} from './pcare-compound-drug-item-admin-crud-list.component';
import {PcareCompoundDrugItemDataFactory} from 'src/app/lib/utils/factories/pcare-compound-drug-item-data-factory';
import {PcareCompoundDrugItemModel} from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model';
import {AdminPcareCompoundDrugItemCrudModule} from '../pcare-compound-drug-item-admin-crud.module';
import {
	PcareCompoundDrugItemModelState,
	initialState as initialPcareCompoundDrugItemModelState
} from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model.state';
 ...
 */
describe('PCare Compound Drug Item Crud List Tile Component', () => {

	let fixture;
	let pcareCompoundDrugItemCrudListComponent: AdminPcareCompoundDrugItemCrudListComponent;

	let store: MockStore<{ model: PcareCompoundDrugItemModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareCompoundDrugItemModelState: PcareCompoundDrugItemModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareCompoundDrugItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '0f606559-e125-46e9-9985-f83636d63d0a';
	const searchContent = 'text';

	const searchableFieldCount = PcareCompoundDrugItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-compound-drug-item-admin-crud',
		urls: ['pcare-compound-drug-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareCompoundDrugItemDataFactory = new PcareCompoundDrugItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareCompoundDrugItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareCompoundDrugItemModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareCompoundDrugItemModelState = initialPcareCompoundDrugItemModelState;
		store.setState({model: pcareCompoundDrugItemModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareCompoundDrugItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareCompoundDrugItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPcareCompoundDrugItemCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPcareCompoundDrugItemCrudListComponent);
			pcareCompoundDrugItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareCompoundDrugItemCrudListComponent.pcareCompoundDrugItemsCount).toBe(defaultData.length);
			...
			expect(pcareCompoundDrugItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareCompoundDrugItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareCompoundDrugItemDataFactory.createAll(filteredDataSize);
			...
			expect(pcareCompoundDrugItemCrudListComponent.pcareCompoundDrugItemsCount).toBe(filteredDataSize);
			expect(pcareCompoundDrugItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareCompoundDrugItemCrudListComponent.pageIndex).toBe(0);
			expect(pcareCompoundDrugItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareCompoundDrugItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareCompoundDrugItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareCompoundDrugItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
