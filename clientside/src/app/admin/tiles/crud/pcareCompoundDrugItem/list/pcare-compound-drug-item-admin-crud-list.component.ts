...
import {PcareCompoundDrugItemModelState} from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model.state';
...
import {
	getPcareCompoundDrugItemCollectionModels,
	getPcareCompoundDrugItemCollectionCount
...
@Component({
	selector: 'cb-pcare-compound-drug-item-admin-crud-list',
	templateUrl: './pcare-compound-drug-item-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-compound-drug-item-admin-crud-list.component.scss',
...
export class AdminPcareCompoundDrugItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareCompoundDrugItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareCompoundDrugItemModel.searchFields,
	...
	/**
	 * List of PCare Compound Drug Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareCompoundDrugItems: Observable<PcareCompoundDrugItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareCompoundDrugItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareCompoundDrugItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareCompoundDrugItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
				modelAction.PcareCompoundDrugItemModelActionTypes.INITIALISE_PCARE_COMPOUND_DRUG_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
				modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.pcareCompoundDrugItems = this.store.select(getPcareCompoundDrugItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareCompoundDrugItemCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareCompoundDrugItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareCompoundDrugItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareCompoundDrugItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareCompoundDrugItemModelActionTypes.EXPORT_ALL_PCARE_COMPOUND_DRUG_ITEM;
		} else {
			actionType = modelAction.PcareCompoundDrugItemModelActionTypes.EXPORT_PCARE_COMPOUND_DRUG_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
				modelAction.PcareCompoundDrugItemModelActionTypes.IMPORT_PCARE_COMPOUND_DRUG_ITEMS,
				{
					file: file,
				},
				[new modelAction.PcareCompoundDrugItemAction(
			...
			window.open(environment.API_URL + '/api/pcarecompounddrugitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareCompoundDrugItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Compound Drug Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Compound Drug Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareCompoundDrugItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareCompoundDrugItemModelActionTypes.DELETE_PCARE_COMPOUND_DRUG_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.PcareCompoundDrugItemModelActionTypes.DELETE_ALL_PCARE_COMPOUND_DRUG_ITEM;
						} else {
							actionType = modelAction.PcareCompoundDrugItemModelActionTypes.DELETE_PCARE_COMPOUND_DRUG_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_COMPOUND_DRUG_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_COMPOUND_DRUG_ITEM and DELETE_PCARE_COMPOUND_DRUG_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareCompoundDrugItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareCompoundDrugItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
