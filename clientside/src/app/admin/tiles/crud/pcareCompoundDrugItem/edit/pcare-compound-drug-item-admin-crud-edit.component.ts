...
import * as modelAction from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model.action';
import {PcareCompoundDrugItemModelState, PcareCompoundDrugItemModelAudit} from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model.state';
import {PcareCompoundDrugItemModel} from 'src/app/models/pcareCompoundDrugItem/pcare_compound_drug_item.model';
import {
	getPcareCompoundDrugItemModelAuditsByEntityId,
	getPcareCompoundDrugItemModelWithId,
	getCountPcareCompoundDrugItemModels,
...
import {CrudTileMode} from '../pcare-compound-drug-item-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-compound-drug-item-admin-crud-edit',
	templateUrl: './pcare-compound-drug-item-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-compound-drug-item-admin-crud-edit.component.scss',
...
export class AdminPcareCompoundDrugItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '0f606559-e125-46e9-9985-f83636d63d0a';
	 ...
	 */
	pcareCompoundDrugItemAudits$: Observable<PcareCompoundDrugItemModelAudit[]>;
	...
	pcareCompoundDrugMultiCrudActive: boolean = false;
	pcareCompoundDrugDisplayName = 'PCare Compound Drug';
	...
	pcareCompoundDrugOppositeRelation = 'pcareCompoundDrugItems';
	pcareCompoundDrugRelations: ModelRelation[] = this.pcareCompoundDrugMultiCrudActive ? PcareCompoundDrugModel.getRelations()
		.map(relation => {
			if (relation.id === this.pcareCompoundDrugOppositeRelation) {
	 ...
	 */
	modelProperties: ModelProperty[] = PcareCompoundDrugItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareCompoundDrugItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareCompoundDrugItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'pcareCompoundDrug' && this.pcareCompoundDrugMultiCrudActive) {
	 ...
	 */
	targetModel: PcareCompoundDrugItemModel;
	...
	constructor(
		private readonly store: Store<{ model: PcareCompoundDrugItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareCompoundDrugItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareCompoundDrugItemModel();
		...
		if (this.targetModelId) {
			this.pcareCompoundDrugItemAudits$ = this.store.select(getPcareCompoundDrugItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareCompoundDrugItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
			modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareCompoundDrugItemModelWithId, this.targetModelId)
			...
			model = new PcareCompoundDrugItemModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-compound-drug-item');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
			modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PcareCompoundDrugItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.pcareCompoundDrugMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PcareCompoundDrugItemAction(
						modelAction.PcareCompoundDrugItemModelActionTypes.COUNT_PCARE_COMPOUND_DRUG_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
				modelAction.PcareCompoundDrugItemModelActionTypes.CREATE_PCARE_COMPOUND_DRUG_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareCompoundDrugItemAction(
				modelAction.PcareCompoundDrugItemModelActionTypes.UPDATE_PCARE_COMPOUND_DRUG_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareCompoundDrugItemModel = new PcareCompoundDrugItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareCompoundDrugItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareCompoundDrugItemModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.pcareCompoundDrugDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PcareCompoundDrugModel.getProps(), this.pcareCompoundDrugRelations, this.isViewOnly),
					new PcareCompoundDrugModel(),
					PcareCompoundDrugModel,
					this.pcareCompoundDrugDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.pcareCompoundDrugRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareCompoundDrugItemModel.getProps();
		this.modelRelations = PcareCompoundDrugItemModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'pcareCompoundDrug';
		}).hideElement = this.pcareCompoundDrugMultiCrudActive || this.modelRelations.find(rel => rel.id === 'pcareCompoundDrug').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.pcareCompoundDrugMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.pcareCompoundDrugDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.pcareCompoundDrug && this.targetModel.pcareCompoundDrug.id && this.pcareCompoundDrugMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PcareCompoundDrugModel,
				this.pcareCompoundDrugOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.pcareCompoundDrug.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPcareCompoundDrugModelWithId, this.targetModel.pcareCompoundDrugId).subscribe(pcareCompoundDrug => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PcareCompoundDrugModel.getProps(), this.pcareCompoundDrugRelations, this.isViewOnly),
					pcareCompoundDrug,
					PcareCompoundDrugModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareCompoundDrugItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareCompoundDrugItemModel {
		let updatedModel: PcareCompoundDrugItemModel = new PcareCompoundDrugItemModel(model);

		if (this.pcareCompoundDrugMultiCrudActive) {
			updatedModel.pcareCompoundDrug = event.additionalEntities.pcareCompoundDrug ? event.additionalEntities.pcareCompoundDrug : undefined;

			if (!updatedModel.pcareCompoundDrug && event.payload) {
				event.payload.pcareCompoundDrugId = null;
			}
		} else if (event.payload?.pcareCompoundDrugId !== undefined) {
			event.payload.pcareCompoundDrug = null;
			updatedModel.pcareCompoundDrug = null;
		}
