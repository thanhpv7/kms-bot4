...
import {PurchaseOrderStockDetailModelState} from 'src/app/models/purchaseOrderStockDetail/purchase_order_stock_detail.model.state';
...
import {
	...
	getPurchaseOrderStockDetailCollectionCount
...
@Component({
	selector: 'cb-purchase-order-stock-detail-admin-crud-list',
	templateUrl: './purchase-order-stock-detail-admin-crud-list.component.html',
	styleUrls: [
		'./purchase-order-stock-detail-admin-crud-list.component.scss',
...
export class AdminPurchaseOrderStockDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PurchaseOrderStockDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PurchaseOrderStockDetailModel.searchFields,
	...
	/**
	 * List of Purchase Order Stock Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	purchaseOrderStockDetails: Observable<PurchaseOrderStockDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	purchaseOrderStockDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.purchaseOrderStockDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.purchaseOrderStockDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
				modelAction.PurchaseOrderStockDetailModelActionTypes.INITIALISE_PURCHASE_ORDER_STOCK_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
				modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_PURCHASE_ORDER_STOCK_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.purchaseOrderStockDetails = this.store.select(getPurchaseOrderStockDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPurchaseOrderStockDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.purchaseOrderStockDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PurchaseOrderStockDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PurchaseOrderStockDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PurchaseOrderStockDetailModelActionTypes.EXPORT_ALL_PURCHASE_ORDER_STOCK_DETAIL;
		} else {
			actionType = modelAction.PurchaseOrderStockDetailModelActionTypes.EXPORT_PURCHASE_ORDER_STOCK_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
				modelAction.PurchaseOrderStockDetailModelActionTypes.IMPORT_PURCHASE_ORDER_STOCK_DETAILS,
				{
					file: file,
				},
				[new modelAction.PurchaseOrderStockDetailAction(
			...
			window.open(environment.API_URL + '/api/purchaseorderstockdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PurchaseOrderStockDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Purchase Order Stock Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Purchase Order Stock Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PurchaseOrderStockDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PurchaseOrderStockDetailModelActionTypes.DELETE_PURCHASE_ORDER_STOCK_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.PurchaseOrderStockDetailModelActionTypes.DELETE_ALL_PURCHASE_ORDER_STOCK_DETAIL;
						} else {
							actionType = modelAction.PurchaseOrderStockDetailModelActionTypes.DELETE_PURCHASE_ORDER_STOCK_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PURCHASE_ORDER_STOCK_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PURCHASE_ORDER_STOCK_DETAIL and DELETE_PURCHASE_ORDER_STOCK_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PurchaseOrderStockDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.purchaseOrderStockDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
