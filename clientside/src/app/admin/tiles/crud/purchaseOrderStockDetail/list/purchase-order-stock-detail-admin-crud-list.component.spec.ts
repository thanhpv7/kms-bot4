...
import {AdminPurchaseOrderStockDetailCrudListComponent} from './purchase-order-stock-detail-admin-crud-list.component';
...
import {AdminPurchaseOrderStockDetailCrudModule} from '../purchase-order-stock-detail-admin-crud.module';
import {
 ...
 */
describe('Purchase Order Stock Detail Crud List Tile Component', () => {

	let fixture;
	let purchaseOrderStockDetailCrudListComponent: AdminPurchaseOrderStockDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PurchaseOrderStockDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c1ae9ca0-2878-46f1-8615-dd19a2bc89a1';
	const searchContent = 'text';

	const searchableFieldCount = PurchaseOrderStockDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'purchase-order-stock-detail-admin-crud',
		urls: ['purchase-order-stock-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const purchaseOrderStockDetailDataFactory = new PurchaseOrderStockDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = purchaseOrderStockDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPurchaseOrderStockDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPurchaseOrderStockDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPurchaseOrderStockDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPurchaseOrderStockDetailCrudListComponent);
			purchaseOrderStockDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(purchaseOrderStockDetailCrudListComponent.purchaseOrderStockDetailsCount).toBe(defaultData.length);
			...
			expect(purchaseOrderStockDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			purchaseOrderStockDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = purchaseOrderStockDetailDataFactory.createAll(filteredDataSize);
			...
			expect(purchaseOrderStockDetailCrudListComponent.purchaseOrderStockDetailsCount).toBe(filteredDataSize);
			expect(purchaseOrderStockDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(purchaseOrderStockDetailCrudListComponent.pageIndex).toBe(0);
			expect(purchaseOrderStockDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			purchaseOrderStockDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		purchaseOrderStockDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		purchaseOrderStockDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
