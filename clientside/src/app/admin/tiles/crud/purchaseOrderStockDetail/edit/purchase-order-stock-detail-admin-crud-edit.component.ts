...
import * as modelAction from 'src/app/models/purchaseOrderStockDetail/purchase_order_stock_detail.model.action';
import {PurchaseOrderStockDetailModelState, PurchaseOrderStockDetailModelAudit} from 'src/app/models/purchaseOrderStockDetail/purchase_order_stock_detail.model.state';
import {PurchaseOrderStockDetailModel} from 'src/app/models/purchaseOrderStockDetail/purchase_order_stock_detail.model';
import {
	getPurchaseOrderStockDetailModelAuditsByEntityId,
...
import {CrudTileMode} from '../purchase-order-stock-detail-admin-crud.component';
...
@Component({
	selector: 'cb-purchase-order-stock-detail-admin-crud-edit',
	templateUrl: './purchase-order-stock-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./purchase-order-stock-detail-admin-crud-edit.component.scss',
...
export class AdminPurchaseOrderStockDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'c1ae9ca0-2878-46f1-8615-dd19a2bc89a1';
	 ...
	 */
	purchaseOrderStockDetailAudits$: Observable<PurchaseOrderStockDetailModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Purchase Order and Purchase Order Stock Details here] off begin
	purchaseOrderMultiCrudActive: boolean = false;
	purchaseOrderDisplayName = 'Purchase Order';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Purchase Order and Purchase Order Stock Details here] end
	purchaseOrderOppositeRelation = 'purchaseOrderStockDetails';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Purchase Requisition Stock Detail and Purchase Order Stock Details here] off begin
	purchaseRequisitionStockDetailMultiCrudActive: boolean = false;
	purchaseRequisitionStockDetailDisplayName = 'Purchase Requisition Stock Detail';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Purchase Requisition Stock Detail and Purchase Order Stock Details here] end
	purchaseRequisitionStockDetailOppositeRelation = 'purchaseOrderStockDetails';
	purchaseRequisitionStockDetailRelations: ModelRelation[] = this.purchaseRequisitionStockDetailMultiCrudActive ? PurchaseRequisitionStockDetailModel.getRelations()
		.map(relation => {
			if (relation.id === this.purchaseRequisitionStockDetailOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	stockCatalogueOppositeRelation = 'purchaseOrderStockDetails';
	...
	goodsReceiveNoteItemsOppositeRelation = 'purchaseOrderStockDetail';
	 ...
	 */
	modelProperties: ModelProperty[] = PurchaseOrderStockDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PurchaseOrderStockDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PurchaseOrderStockDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'purchaseRequisitionStockDetail' && this.purchaseRequisitionStockDetailMultiCrudActive) {
	 ...
	 */
	targetModel: PurchaseOrderStockDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: PurchaseOrderStockDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPurchaseOrderStockDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PurchaseOrderStockDetailModel();
		...
		if (this.targetModelId) {
			this.purchaseOrderStockDetailAudits$ = this.store.select(getPurchaseOrderStockDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PurchaseOrderStockDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
			modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_PURCHASE_ORDER_STOCK_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPurchaseOrderStockDetailModelWithId, this.targetModelId)
			...
			model = new PurchaseOrderStockDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('purchase-order-stock-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
			modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_PURCHASE_ORDER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PurchaseOrderStockDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.purchaseOrderMultiCrudActive || this.purchaseRequisitionStockDetailMultiCrudActive || this.stockCatalogueMultiCrudActive || this.goodsReceiveNoteItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PurchaseOrderStockDetailAction(
						modelAction.PurchaseOrderStockDetailModelActionTypes.COUNT_PURCHASE_ORDER_STOCK_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
				modelAction.PurchaseOrderStockDetailModelActionTypes.CREATE_PURCHASE_ORDER_STOCK_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PurchaseOrderStockDetailAction(
				modelAction.PurchaseOrderStockDetailModelActionTypes.UPDATE_PURCHASE_ORDER_STOCK_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PurchaseOrderStockDetailModel = new PurchaseOrderStockDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPurchaseOrderStockDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PurchaseOrderStockDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.purchaseRequisitionStockDetailDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PurchaseRequisitionStockDetailModel.getProps(), this.purchaseRequisitionStockDetailRelations, this.isViewOnly),
					new PurchaseRequisitionStockDetailModel(),
					PurchaseRequisitionStockDetailModel,
					this.purchaseRequisitionStockDetailDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.purchaseRequisitionStockDetailRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PurchaseOrderStockDetailModel.getProps();
		this.modelRelations = PurchaseOrderStockDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'purchaseRequisitionStockDetail';
		}).hideElement = this.purchaseRequisitionStockDetailMultiCrudActive || this.modelRelations.find(rel => rel.id === 'purchaseRequisitionStockDetail').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.purchaseRequisitionStockDetailMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.purchaseRequisitionStockDetailDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.purchaseRequisitionStockDetail && this.targetModel.purchaseRequisitionStockDetail.id && this.purchaseRequisitionStockDetailMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PurchaseRequisitionStockDetailModel,
				this.purchaseRequisitionStockDetailOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.purchaseRequisitionStockDetail.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPurchaseRequisitionStockDetailModelWithId, this.targetModel.purchaseRequisitionStockDetailId).subscribe(purchaseRequisitionStockDetail => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PurchaseRequisitionStockDetailModel.getProps(), this.purchaseRequisitionStockDetailRelations, this.isViewOnly),
	 ...
	 */
	addAdditionalEntitiesToModel(model: PurchaseOrderStockDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PurchaseOrderStockDetailModel {
		let updatedModel: PurchaseOrderStockDetailModel = new PurchaseOrderStockDetailModel(model);
		...
		if (this.purchaseRequisitionStockDetailMultiCrudActive) {
			updatedModel.purchaseRequisitionStockDetail = event.additionalEntities.purchaseRequisitionStockDetail ? event.additionalEntities.purchaseRequisitionStockDetail : undefined;

			if (!updatedModel.purchaseRequisitionStockDetail && event.payload) {
				event.payload.purchaseRequisitionStockDetailId = null;
			}
		} else if (event.payload?.purchaseRequisitionStockDetailId !== undefined) {
			event.payload.purchaseRequisitionStockDetail = null;
			updatedModel.purchaseRequisitionStockDetail = null;
		}
