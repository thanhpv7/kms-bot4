...
import {AdminBpjsBedAvailabilityCrudListComponent} from './bpjs-bed-availability-admin-crud-list.component';
...
import {AdminBpjsBedAvailabilityCrudModule} from '../bpjs-bed-availability-admin-crud.module';
import {
 ...
 */
describe('BPJS Bed Availability Crud List Tile Component', () => {

	let fixture;
	let bpjsBedAvailabilityCrudListComponent: AdminBpjsBedAvailabilityCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsBedAvailabilityModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a533f7e7-d4b5-47db-82bf-ded7bd63d662';
	const searchContent = 'text';

	const searchableFieldCount = BpjsBedAvailabilityModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-bed-availability-admin-crud',
		urls: ['bpjs-bed-availability-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsBedAvailabilityDataFactory = new BpjsBedAvailabilityDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsBedAvailabilityDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsBedAvailabilityCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsBedAvailabilityCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsBedAvailabilityCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsBedAvailabilityCrudListComponent);
			bpjsBedAvailabilityCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsBedAvailabilityCrudListComponent.bpjsBedAvailabilitysCount).toBe(defaultData.length);
			...
			expect(bpjsBedAvailabilityCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsBedAvailabilityCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsBedAvailabilityDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsBedAvailabilityCrudListComponent.bpjsBedAvailabilitysCount).toBe(filteredDataSize);
			expect(bpjsBedAvailabilityCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsBedAvailabilityCrudListComponent.pageIndex).toBe(0);
			expect(bpjsBedAvailabilityCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsBedAvailabilityCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsBedAvailabilityCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsBedAvailabilityCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
