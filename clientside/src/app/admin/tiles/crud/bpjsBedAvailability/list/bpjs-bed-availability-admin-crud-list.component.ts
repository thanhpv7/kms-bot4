...
import * as modelAction from 'src/app/models/bpjsBedAvailability/bpjs_bed_availability.model.action';
import {BpjsBedAvailabilityModelState} from 'src/app/models/bpjsBedAvailability/bpjs_bed_availability.model.state';
...
import {
	...
	getBpjsBedAvailabilityCollectionCount
...
import {CrudTileMode} from '../bpjs-bed-availability-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-bed-availability-admin-crud-list',
	templateUrl: './bpjs-bed-availability-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-bed-availability-admin-crud-list.component.scss',
...
export class AdminBpjsBedAvailabilityCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsBedAvailabilityModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsBedAvailabilityModel.searchFields,
	...
	/**
	 * List of BPJS Bed Availabilitys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsBedAvailabilitys: Observable<BpjsBedAvailabilityModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsBedAvailabilitysCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsBedAvailabilityModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsBedAvailabilityModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'a533f7e7-d4b5-47db-82bf-ded7bd63d662';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsBedAvailabilitysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsBedAvailabilitysCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsBedAvailabilityModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
				modelAction.BpjsBedAvailabilityModelActionTypes.INITIALISE_BPJS_BED_AVAILABILITY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
				modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_BPJS_BED_AVAILABILITY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsBedAvailabilityAction(
						modelAction.BpjsBedAvailabilityModelActionTypes.COUNT_BPJS_BED_AVAILABILITYS,
						null
					)
				]
			))
		...
		this.bpjsBedAvailabilitys = this.store.select(getBpjsBedAvailabilityCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsBedAvailabilityCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsBedAvailabilitysCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-bed-availability');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsBedAvailabilityModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsBedAvailabilityModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsBedAvailabilityModelActionTypes.EXPORT_ALL_BPJS_BED_AVAILABILITY;
		} else {
			actionType = modelAction.BpjsBedAvailabilityModelActionTypes.EXPORT_BPJS_BED_AVAILABILITY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
				modelAction.BpjsBedAvailabilityModelActionTypes.IMPORT_BPJS_BED_AVAILABILITYS,
				{
					file: file,
				},
				[new modelAction.BpjsBedAvailabilityAction(
			...
			window.open(environment.API_URL + '/api/bpjsbedavailability/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsBedAvailabilityModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Bed Availability entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Bed Availability entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsBedAvailabilityModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsBedAvailabilityModelActionTypes.DELETE_BPJS_BED_AVAILABILITY;
						} else if (usingSelected) {
							actionType = modelAction.BpjsBedAvailabilityModelActionTypes.DELETE_ALL_BPJS_BED_AVAILABILITY;
						} else {
							actionType = modelAction.BpjsBedAvailabilityModelActionTypes.DELETE_BPJS_BED_AVAILABILITY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_BED_AVAILABILITY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_BED_AVAILABILITY and DELETE_BPJS_BED_AVAILABILITY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsBedAvailabilityModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsBedAvailabilityModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsBedAvailabilitysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
