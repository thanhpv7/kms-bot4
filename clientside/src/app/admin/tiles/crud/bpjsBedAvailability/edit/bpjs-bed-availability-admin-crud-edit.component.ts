...
import {BpjsBedAvailabilityModelState, BpjsBedAvailabilityModelAudit} from 'src/app/models/bpjsBedAvailability/bpjs_bed_availability.model.state';
import {BpjsBedAvailabilityModel} from 'src/app/models/bpjsBedAvailability/bpjs_bed_availability.model';
import {
	getBpjsBedAvailabilityModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-bed-availability-admin-crud-edit',
	templateUrl: './bpjs-bed-availability-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-bed-availability-admin-crud-edit.component.scss',
...
export class AdminBpjsBedAvailabilityCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsBedAvailabilityAudits$: Observable<BpjsBedAvailabilityModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsBedAvailabilityModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsBedAvailabilityModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsBedAvailabilityModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsBedAvailabilityModel();
		...
		if (this.targetModelId) {
			this.bpjsBedAvailabilityAudits$ = this.store.select(getBpjsBedAvailabilityModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsBedAvailabilityModel> = {
		...
		this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
			modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_BPJS_BED_AVAILABILITY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsBedAvailabilityModelWithId, this.targetModelId)
			...
			model = new BpjsBedAvailabilityModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
			modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_BPJS_BED_AVAILABILITY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
				modelAction.BpjsBedAvailabilityModelActionTypes.CREATE_BPJS_BED_AVAILABILITY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsBedAvailabilityAction(
				modelAction.BpjsBedAvailabilityModelActionTypes.UPDATE_BPJS_BED_AVAILABILITY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsBedAvailabilityModel = new BpjsBedAvailabilityModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsBedAvailabilityModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsBedAvailabilityModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsBedAvailabilityModel.getProps();
		this.modelRelations = BpjsBedAvailabilityModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsBedAvailabilityModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsBedAvailabilityModel {
		let updatedModel: BpjsBedAvailabilityModel = new BpjsBedAvailabilityModel(model);
