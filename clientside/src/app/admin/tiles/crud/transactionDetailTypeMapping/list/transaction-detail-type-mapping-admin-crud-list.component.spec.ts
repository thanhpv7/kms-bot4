...
import {AdminTransactionDetailTypeMappingCrudListComponent} from './transaction-detail-type-mapping-admin-crud-list.component';
...
import {AdminTransactionDetailTypeMappingCrudModule} from '../transaction-detail-type-mapping-admin-crud.module';
import {
	TransactionDetailTypeMappingModelState,
	initialState as initialTransactionDetailTypeMappingModelState
} from 'src/app/models/transactionDetailTypeMapping/transaction_detail_type_mapping.model.state';
 ...
 */
describe('Transaction Detail Type Mapping Crud List Tile Component', () => {

	let fixture;
	let transactionDetailTypeMappingCrudListComponent: AdminTransactionDetailTypeMappingCrudListComponent;

	let store: MockStore<{ model: TransactionDetailTypeMappingModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let transactionDetailTypeMappingModelState: TransactionDetailTypeMappingModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<TransactionDetailTypeMappingModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f6122e3e-8865-416a-abdf-36fbdff5d147';
	const searchContent = 'text';

	const searchableFieldCount = TransactionDetailTypeMappingModel.searchFields.length;

	const routerState: RouterState = {
		url: 'transaction-detail-type-mapping-admin-crud',
		urls: ['transaction-detail-type-mapping-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const transactionDetailTypeMappingDataFactory = new TransactionDetailTypeMappingDataFactory();

	const defaultDataSize = 30;
	const defaultData = transactionDetailTypeMappingDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: TransactionDetailTypeMappingModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		transactionDetailTypeMappingModelState = initialTransactionDetailTypeMappingModelState;
		store.setState({model: transactionDetailTypeMappingModelState});
		...
		spyOn(store, 'select')
			.withArgs(getTransactionDetailTypeMappingCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTransactionDetailTypeMappingCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminTransactionDetailTypeMappingCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminTransactionDetailTypeMappingCrudListComponent);
			transactionDetailTypeMappingCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(transactionDetailTypeMappingCrudListComponent.transactionDetailTypeMappingsCount).toBe(defaultData.length);
			...
			expect(transactionDetailTypeMappingCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			transactionDetailTypeMappingCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = transactionDetailTypeMappingDataFactory.createAll(filteredDataSize);
			...
			expect(transactionDetailTypeMappingCrudListComponent.transactionDetailTypeMappingsCount).toBe(filteredDataSize);
			expect(transactionDetailTypeMappingCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(transactionDetailTypeMappingCrudListComponent.pageIndex).toBe(0);
			expect(transactionDetailTypeMappingCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			transactionDetailTypeMappingCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		transactionDetailTypeMappingCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		transactionDetailTypeMappingCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
