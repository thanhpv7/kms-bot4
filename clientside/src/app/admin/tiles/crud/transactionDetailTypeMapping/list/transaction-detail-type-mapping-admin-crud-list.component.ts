...
import * as modelAction from 'src/app/models/transactionDetailTypeMapping/transaction_detail_type_mapping.model.action';
import {TransactionDetailTypeMappingModelState} from 'src/app/models/transactionDetailTypeMapping/transaction_detail_type_mapping.model.state';
...
import {
	getTransactionDetailTypeMappingCollectionModels,
	getTransactionDetailTypeMappingCollectionCount
...
import {CrudTileMode} from '../transaction-detail-type-mapping-admin-crud.component';
...
@Component({
	selector: 'cb-transaction-detail-type-mapping-admin-crud-list',
	templateUrl: './transaction-detail-type-mapping-admin-crud-list.component.html',
	styleUrls: [
		'./transaction-detail-type-mapping-admin-crud-list.component.scss',
...
export class AdminTransactionDetailTypeMappingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TransactionDetailTypeMappingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TransactionDetailTypeMappingModel.searchFields,
	...
	/**
	 * List of Transaction Detail Type Mappings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	transactionDetailTypeMappings: Observable<TransactionDetailTypeMappingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	transactionDetailTypeMappingsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = TransactionDetailTypeMappingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TransactionDetailTypeMappingModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'f6122e3e-8865-416a-abdf-36fbdff5d147';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.transactionDetailTypeMappingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.transactionDetailTypeMappingsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: TransactionDetailTypeMappingModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
				modelAction.TransactionDetailTypeMappingModelActionTypes.INITIALISE_TRANSACTION_DETAIL_TYPE_MAPPING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
				modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_MAPPING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.TransactionDetailTypeMappingAction(
						modelAction.TransactionDetailTypeMappingModelActionTypes.COUNT_TRANSACTION_DETAIL_TYPE_MAPPINGS,
						null
					)
				]
			))
		...
		this.transactionDetailTypeMappings = this.store.select(getTransactionDetailTypeMappingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTransactionDetailTypeMappingCollectionCount, this.collectionId).subscribe(
				(count) => this.transactionDetailTypeMappingsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('transaction-detail-type-mapping');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TransactionDetailTypeMappingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TransactionDetailTypeMappingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TransactionDetailTypeMappingModelActionTypes.EXPORT_ALL_TRANSACTION_DETAIL_TYPE_MAPPING;
		} else {
			actionType = modelAction.TransactionDetailTypeMappingModelActionTypes.EXPORT_TRANSACTION_DETAIL_TYPE_MAPPING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
				modelAction.TransactionDetailTypeMappingModelActionTypes.IMPORT_TRANSACTION_DETAIL_TYPE_MAPPINGS,
				{
					file: file,
				},
				[new modelAction.TransactionDetailTypeMappingAction(
			...
			window.open(environment.API_URL + '/api/transactiondetailtypemapping/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TransactionDetailTypeMappingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Transaction Detail Type Mapping entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Transaction Detail Type Mapping entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TransactionDetailTypeMappingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TransactionDetailTypeMappingModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE_MAPPING;
						} else if (usingSelected) {
							actionType = modelAction.TransactionDetailTypeMappingModelActionTypes.DELETE_ALL_TRANSACTION_DETAIL_TYPE_MAPPING;
						} else {
							actionType = modelAction.TransactionDetailTypeMappingModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE_MAPPING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TRANSACTION_DETAIL_TYPE_MAPPING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TRANSACTION_DETAIL_TYPE_MAPPING and DELETE_TRANSACTION_DETAIL_TYPE_MAPPING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<TransactionDetailTypeMappingModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TransactionDetailTypeMappingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.transactionDetailTypeMappingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
