...
import {TransactionDetailTypeMappingModelState, TransactionDetailTypeMappingModelAudit} from 'src/app/models/transactionDetailTypeMapping/transaction_detail_type_mapping.model.state';
...
import {
	getTransactionDetailTypeMappingModelAuditsByEntityId,
	getTransactionDetailTypeMappingModelWithId,
	getCountTransactionDetailTypeMappingModels,
...
@Component({
	selector: 'cb-transaction-detail-type-mapping-admin-crud-edit',
	templateUrl: './transaction-detail-type-mapping-admin-crud-edit.component.html',
	styleUrls: [
		'./transaction-detail-type-mapping-admin-crud-edit.component.scss',
...
export class AdminTransactionDetailTypeMappingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	transactionDetailTypeMappingAudits$: Observable<TransactionDetailTypeMappingModelAudit[]>;
	...
	transactionDetailTypeOppositeRelation = 'transactionDetailTypeMappings';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TransactionDetailTypeMappingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: TransactionDetailTypeMappingModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTransactionDetailTypeMappingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TransactionDetailTypeMappingModel();
		...
		if (this.targetModelId) {
			this.transactionDetailTypeMappingAudits$ = this.store.select(getTransactionDetailTypeMappingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TransactionDetailTypeMappingModel> = {
		...
		this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
			modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_MAPPING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTransactionDetailTypeMappingModelWithId, this.targetModelId)
			...
			model = new TransactionDetailTypeMappingModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
			modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_MAPPING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TransactionDetailTypeMappingModel> = {
				...
				queryParams: {
					persistRelations: this.transactionDetailTypeMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
				modelAction.TransactionDetailTypeMappingModelActionTypes.CREATE_TRANSACTION_DETAIL_TYPE_MAPPING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TransactionDetailTypeMappingAction(
				modelAction.TransactionDetailTypeMappingModelActionTypes.UPDATE_TRANSACTION_DETAIL_TYPE_MAPPING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TransactionDetailTypeMappingModel = new TransactionDetailTypeMappingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTransactionDetailTypeMappingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TransactionDetailTypeMappingModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TransactionDetailTypeMappingModel.getProps();
		this.modelRelations = TransactionDetailTypeMappingModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: TransactionDetailTypeMappingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TransactionDetailTypeMappingModel {
		let updatedModel: TransactionDetailTypeMappingModel = new TransactionDetailTypeMappingModel(model);
