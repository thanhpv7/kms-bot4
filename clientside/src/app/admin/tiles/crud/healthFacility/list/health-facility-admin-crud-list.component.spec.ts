...
import {AdminHealthFacilityCrudListComponent} from './health-facility-admin-crud-list.component';
...
import {AdminHealthFacilityCrudModule} from '../health-facility-admin-crud.module';
import {
 ...
 */
describe('Health Facility Crud List Tile Component', () => {

	let fixture;
	let healthFacilityCrudListComponent: AdminHealthFacilityCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<HealthFacilityModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '7fee94f3-5370-49b3-adfc-e66e305391ad';
	const searchContent = 'text';

	const searchableFieldCount = HealthFacilityModel.searchFields.length;

	const routerState: RouterState = {
		url: 'health-facility-admin-crud',
		urls: ['health-facility-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const healthFacilityDataFactory = new HealthFacilityDataFactory();

	const defaultDataSize = 30;
	const defaultData = healthFacilityDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getHealthFacilityCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getHealthFacilityCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminHealthFacilityCrudListComponent);
			healthFacilityCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(healthFacilityCrudListComponent.healthFacilitysCount).toBe(defaultData.length);
			...
			expect(healthFacilityCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			healthFacilityCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = healthFacilityDataFactory.createAll(filteredDataSize);
			...
			expect(healthFacilityCrudListComponent.healthFacilitysCount).toBe(filteredDataSize);
			expect(healthFacilityCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(healthFacilityCrudListComponent.pageIndex).toBe(0);
			expect(healthFacilityCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			healthFacilityCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		healthFacilityCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		healthFacilityCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
