...
import {HealthFacilityModelState} from 'src/app/models/healthFacility/health_facility.model.state';
...
import {
	...
	getHealthFacilityCollectionCount
...
@Component({
	selector: 'cb-health-facility-admin-crud-list',
	templateUrl: './health-facility-admin-crud-list.component.html',
	styleUrls: [
		'./health-facility-admin-crud-list.component.scss',
...
export class AdminHealthFacilityCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<HealthFacilityModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...HealthFacilityModel.searchFields,
	...
	/**
	 * List of Health Facilitys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	healthFacilitys: Observable<HealthFacilityModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	healthFacilitysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.healthFacilitysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.healthFacilitysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HealthFacilityAction(
				modelAction.HealthFacilityModelActionTypes.INITIALISE_HEALTH_FACILITY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HealthFacilityAction(
				modelAction.HealthFacilityModelActionTypes.FETCH_HEALTH_FACILITY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.healthFacilitys = this.store.select(getHealthFacilityCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getHealthFacilityCollectionCount, this.collectionId).subscribe(
				(count) => this.healthFacilitysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: HealthFacilityModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.HealthFacilityModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.HealthFacilityModelActionTypes.EXPORT_ALL_HEALTH_FACILITY;
		} else {
			actionType = modelAction.HealthFacilityModelActionTypes.EXPORT_HEALTH_FACILITY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.HealthFacilityAction(
				modelAction.HealthFacilityModelActionTypes.IMPORT_HEALTH_FACILITYS,
				{
					file: file,
				},
				[new modelAction.HealthFacilityAction(
			...
			window.open(environment.API_URL + '/api/healthfacility/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: HealthFacilityModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Health Facility entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Health Facility entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.HealthFacilityModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.HealthFacilityModelActionTypes.DELETE_HEALTH_FACILITY;
						} else if (usingSelected) {
							actionType = modelAction.HealthFacilityModelActionTypes.DELETE_ALL_HEALTH_FACILITY;
						} else {
							actionType = modelAction.HealthFacilityModelActionTypes.DELETE_HEALTH_FACILITY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.HealthFacilityAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_HEALTH_FACILITY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_HEALTH_FACILITY and DELETE_HEALTH_FACILITY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = HealthFacilityModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.healthFacilitysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
