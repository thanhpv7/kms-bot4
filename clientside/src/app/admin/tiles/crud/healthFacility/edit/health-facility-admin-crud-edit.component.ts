...
import * as modelAction from 'src/app/models/healthFacility/health_facility.model.action';
import {HealthFacilityModelState, HealthFacilityModelAudit} from 'src/app/models/healthFacility/health_facility.model.state';
import {HealthFacilityModel} from 'src/app/models/healthFacility/health_facility.model';
import {
	getHealthFacilityModelAuditsByEntityId,
...
import {CrudTileMode} from '../health-facility-admin-crud.component';
...
@Component({
	selector: 'cb-health-facility-admin-crud-edit',
	templateUrl: './health-facility-admin-crud-edit.component.html',
	styleUrls: [
		'./health-facility-admin-crud-edit.component.scss',
...
export class AdminHealthFacilityCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '7fee94f3-5370-49b3-adfc-e66e305391ad';
	 ...
	 */
	healthFacilityAudits$: Observable<HealthFacilityModelAudit[]>;
	...
	dismissalReferringFacilityCollectionId: string = uuidv4();
	...
	dismissalReferringFacilityRelations: ModelRelation[] = this.dismissalReferringFacilityMultiCrudActive ? RegistrationModel.getRelations()
		.map(relation => {
	...
	referringFacilityCollectionId: string = uuidv4();
	...
	referringFacilityRelations: ModelRelation[] = this.referringFacilityMultiCrudActive ? RegistrationModel.getRelations()
		.map(relation => {
	 ...
	 */
	modelProperties: ModelProperty[] = HealthFacilityModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = HealthFacilityModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = HealthFacilityModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: HealthFacilityModel;
	...
	constructor(
		private readonly store: Store<{ model: HealthFacilityModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountHealthFacilityModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new HealthFacilityModel();
		...
		if (this.targetModelId) {
			this.healthFacilityAudits$ = this.store.select(getHealthFacilityModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<HealthFacilityModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.HealthFacilityAction(
			modelAction.HealthFacilityModelActionTypes.FETCH_HEALTH_FACILITY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getHealthFacilityModelWithId, this.targetModelId)
			...
			model = new HealthFacilityModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.HealthFacilityAction(
			modelAction.HealthFacilityModelActionTypes.FETCH_HEALTH_FACILITY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<HealthFacilityModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.dismissalReferringFacilityMultiCrudActive || this.referringFacilityMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.HealthFacilityAction(
						modelAction.HealthFacilityModelActionTypes.COUNT_HEALTH_FACILITYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.HealthFacilityAction(
				modelAction.HealthFacilityModelActionTypes.CREATE_HEALTH_FACILITY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.HealthFacilityAction(
				modelAction.HealthFacilityModelActionTypes.UPDATE_HEALTH_FACILITY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: HealthFacilityModel = new HealthFacilityModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getHealthFacilityModelWithId, this.targetModelId).subscribe(model => {
					let differences = new HealthFacilityModel(model).difference(updatedModel);
		...
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.dismissalReferringFacilityDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RegistrationModel.getProps(), this.dismissalReferringFacilityRelations, this.isViewOnly),
			...
			if (event.entityName === this.referringFacilityDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RegistrationModel.getProps(), this.referringFacilityRelations, this.isViewOnly),
					new RegistrationModel(),
					RegistrationModel,
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = HealthFacilityModel.getProps();
		this.modelRelations = HealthFacilityModel.getRelations();
	...
	initialiseMultiEntityCollections() {
		if (this.dismissalReferringFacilityMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.dismissalReferringFacilityDisplayName, many: true});
			this.store.dispatch(new registrationModelAction.RegistrationAction(
				...
				{
					collectionId: this.dismissalReferringFacilityCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getRegistrationCollectionModels, this.dismissalReferringFacilityCollectionId).subscribe(models => {
				models.forEach(dismissalReferringFacility => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(RegistrationModel.getProps(), this.dismissalReferringFacilityRelations, this.isViewOnly),
		...
		if (this.referringFacilityMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.referringFacilityDisplayName, many: true});
			this.store.dispatch(new registrationModelAction.RegistrationAction(
				registrationModelAction.RegistrationModelActionTypes.INITIALISE_REGISTRATION_COLLECTION_STATE,
				{
					collectionId: this.referringFacilityCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getRegistrationCollectionModels, this.referringFacilityCollectionId).subscribe(models => {
				models.forEach(referringFacility => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(RegistrationModel.getProps(), this.referringFacilityRelations, this.isViewOnly),
						referringFacility,
						RegistrationModel,
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.dismissalReferringFacilityIds && this.targetModel.dismissalReferringFacilityIds.length > 0 && this.dismissalReferringFacilityMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				RegistrationModel,
				this.dismissalReferringFacilityOppositeRelation,
				this.dismissalReferringFacilityCollectionId,
				this.targetModel.dismissalReferringFacilityIds.length,
				this.targetModel.id,
				true
			));
		}

		if (this.targetModel.referringFacilityIds && this.targetModel.referringFacilityIds.length > 0 && this.referringFacilityMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				RegistrationModel,
				this.referringFacilityOppositeRelation,
				this.referringFacilityCollectionId,
				this.targetModel.referringFacilityIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: HealthFacilityModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): HealthFacilityModel {
		let updatedModel: HealthFacilityModel = new HealthFacilityModel(model);

		updatedModel.dismissalReferringFacility = [];
		if (this.dismissalReferringFacilityMultiCrudActive) {
			if (event.additionalEntities.dismissalReferringFacility) {
				event.additionalEntities.dismissalReferringFacility.forEach(entity => {
					updatedModel.dismissalReferringFacility.push(entity);
				});
			}

			if (!updatedModel.dismissalReferringFacility.length) {
				updatedModel.dismissalReferringFacilityIds = [];
			}
		}
		updatedModel.referringFacility = [];
		if (this.referringFacilityMultiCrudActive) {
			if (event.additionalEntities.referringFacility) {
				event.additionalEntities.referringFacility.forEach(entity => {
					updatedModel.referringFacility.push(entity);
				});
			}

			if (!updatedModel.referringFacility.length) {
				updatedModel.referringFacilityIds = [];
			}
		}
		return updatedModel;
	}
