...
import * as modelAction from 'src/app/models/bpjsPatientReferral/bpjs_patient_referral.model.action';
import {BpjsPatientReferralModelState} from 'src/app/models/bpjsPatientReferral/bpjs_patient_referral.model.state';
...
import {
	...
	getBpjsPatientReferralCollectionCount
...
import {CrudTileMode} from '../bpjs-patient-referral-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-patient-referral-admin-crud-list',
	templateUrl: './bpjs-patient-referral-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-patient-referral-admin-crud-list.component.scss',
...
export class AdminBpjsPatientReferralCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPatientReferralModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPatientReferralModel.searchFields,
	...
	/**
	 * List of BPJS Patient Referrals that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPatientReferrals: Observable<BpjsPatientReferralModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPatientReferralsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPatientReferralModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPatientReferralModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '7ee5e689-d418-4603-901a-d2b425793b42';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPatientReferralsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPatientReferralsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPatientReferralModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPatientReferralAction(
				modelAction.BpjsPatientReferralModelActionTypes.INITIALISE_BPJS_PATIENT_REFERRAL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPatientReferralAction(
				modelAction.BpjsPatientReferralModelActionTypes.FETCH_BPJS_PATIENT_REFERRAL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPatientReferralAction(
						modelAction.BpjsPatientReferralModelActionTypes.COUNT_BPJS_PATIENT_REFERRALS,
						null
					)
				]
			))
		...
		this.bpjsPatientReferrals = this.store.select(getBpjsPatientReferralCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPatientReferralCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPatientReferralsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-patient-referral');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPatientReferralModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPatientReferralModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPatientReferralModelActionTypes.EXPORT_ALL_BPJS_PATIENT_REFERRAL;
		} else {
			actionType = modelAction.BpjsPatientReferralModelActionTypes.EXPORT_BPJS_PATIENT_REFERRAL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPatientReferralAction(
				modelAction.BpjsPatientReferralModelActionTypes.IMPORT_BPJS_PATIENT_REFERRALS,
				{
					file: file,
				},
				[new modelAction.BpjsPatientReferralAction(
			...
			window.open(environment.API_URL + '/api/bpjspatientreferral/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPatientReferralModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Patient Referral entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Patient Referral entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPatientReferralModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPatientReferralModelActionTypes.DELETE_BPJS_PATIENT_REFERRAL;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPatientReferralModelActionTypes.DELETE_ALL_BPJS_PATIENT_REFERRAL;
						} else {
							actionType = modelAction.BpjsPatientReferralModelActionTypes.DELETE_BPJS_PATIENT_REFERRAL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPatientReferralAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PATIENT_REFERRAL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PATIENT_REFERRAL and DELETE_BPJS_PATIENT_REFERRAL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPatientReferralModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPatientReferralModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPatientReferralsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
