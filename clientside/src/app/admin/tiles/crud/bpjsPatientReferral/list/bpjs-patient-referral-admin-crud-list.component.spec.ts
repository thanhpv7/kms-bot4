...
import {AdminBpjsPatientReferralCrudListComponent} from './bpjs-patient-referral-admin-crud-list.component';
...
import {AdminBpjsPatientReferralCrudModule} from '../bpjs-patient-referral-admin-crud.module';
import {
 ...
 */
describe('BPJS Patient Referral Crud List Tile Component', () => {

	let fixture;
	let bpjsPatientReferralCrudListComponent: AdminBpjsPatientReferralCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPatientReferralModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '7ee5e689-d418-4603-901a-d2b425793b42';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPatientReferralModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-patient-referral-admin-crud',
		urls: ['bpjs-patient-referral-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPatientReferralDataFactory = new BpjsPatientReferralDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPatientReferralDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPatientReferralCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPatientReferralCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsPatientReferralCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsPatientReferralCrudListComponent);
			bpjsPatientReferralCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPatientReferralCrudListComponent.bpjsPatientReferralsCount).toBe(defaultData.length);
			...
			expect(bpjsPatientReferralCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPatientReferralCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPatientReferralDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPatientReferralCrudListComponent.bpjsPatientReferralsCount).toBe(filteredDataSize);
			expect(bpjsPatientReferralCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPatientReferralCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPatientReferralCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPatientReferralCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPatientReferralCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPatientReferralCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
