...
import {BpjsPatientReferralModelState, BpjsPatientReferralModelAudit} from 'src/app/models/bpjsPatientReferral/bpjs_patient_referral.model.state';
import {BpjsPatientReferralModel} from 'src/app/models/bpjsPatientReferral/bpjs_patient_referral.model';
import {
	getBpjsPatientReferralModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-patient-referral-admin-crud-edit',
	templateUrl: './bpjs-patient-referral-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-patient-referral-admin-crud-edit.component.scss',
...
export class AdminBpjsPatientReferralCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPatientReferralAudits$: Observable<BpjsPatientReferralModelAudit[]>;
	...
	bpjsDiagnoseOppositeRelation = 'bpjsPatientReferrals';
	...
	bpjsHealthFacilityOppositeRelation = 'bpjsPatientReferrals';
	...
	bpjsServiceMappingOppositeRelation = 'bpjsPatientReferrals';
	...
	patientPaymentBPJSOppositeRelation = 'bpjsPatientReferrals';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPatientReferralModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPatientReferralModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPatientReferralModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPatientReferralModel();
		...
		if (this.targetModelId) {
			this.bpjsPatientReferralAudits$ = this.store.select(getBpjsPatientReferralModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPatientReferralModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPatientReferralAction(
			modelAction.BpjsPatientReferralModelActionTypes.FETCH_BPJS_PATIENT_REFERRAL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPatientReferralModelWithId, this.targetModelId)
			...
			model = new BpjsPatientReferralModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPatientReferralAction(
			modelAction.BpjsPatientReferralModelActionTypes.FETCH_BPJS_PATIENT_REFERRAL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsPatientReferralModel> = {
				...
				queryParams: {
					persistRelations: this.bpjsDiagnoseMultiCrudActive || this.bpjsHealthFacilityMultiCrudActive || this.bpjsServiceMappingMultiCrudActive || this.patientPaymentBPJSMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsPatientReferralAction(
				modelAction.BpjsPatientReferralModelActionTypes.CREATE_BPJS_PATIENT_REFERRAL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPatientReferralAction(
				modelAction.BpjsPatientReferralModelActionTypes.UPDATE_BPJS_PATIENT_REFERRAL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPatientReferralModel = new BpjsPatientReferralModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPatientReferralModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPatientReferralModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPatientReferralModel.getProps();
		this.modelRelations = BpjsPatientReferralModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPatientReferralModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPatientReferralModel {
		let updatedModel: BpjsPatientReferralModel = new BpjsPatientReferralModel(model);
