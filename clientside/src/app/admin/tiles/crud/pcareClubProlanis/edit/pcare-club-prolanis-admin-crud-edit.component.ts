...
import {PcareClubProlanisModelState, PcareClubProlanisModelAudit} from 'src/app/models/pcareClubProlanis/pcare_club_prolanis.model.state';
import {PcareClubProlanisModel} from 'src/app/models/pcareClubProlanis/pcare_club_prolanis.model';
import {
	getPcareClubProlanisModelAuditsByEntityId,
	getPcareClubProlanisModelWithId,
	getCountPcareClubProlanisModels,
...
@Component({
	selector: 'cb-pcare-club-prolanis-admin-crud-edit',
	templateUrl: './pcare-club-prolanis-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-club-prolanis-admin-crud-edit.component.scss',
...
export class AdminPcareClubProlanisCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareClubProlanisAudits$: Observable<PcareClubProlanisModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareClubProlanisModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PcareClubProlanisModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareClubProlanisModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareClubProlanisModel();
		...
		if (this.targetModelId) {
			this.pcareClubProlanisAudits$ = this.store.select(getPcareClubProlanisModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareClubProlanisModel> = {
		...
		this.store.dispatch(new modelAction.PcareClubProlanisAction(
			modelAction.PcareClubProlanisModelActionTypes.FETCH_PCARE_CLUB_PROLANIS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareClubProlanisModelWithId, this.targetModelId)
			...
			model = new PcareClubProlanisModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareClubProlanisAction(
			modelAction.PcareClubProlanisModelActionTypes.FETCH_PCARE_CLUB_PROLANIS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareClubProlanisAction(
				modelAction.PcareClubProlanisModelActionTypes.CREATE_PCARE_CLUB_PROLANIS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareClubProlanisAction(
				modelAction.PcareClubProlanisModelActionTypes.UPDATE_PCARE_CLUB_PROLANIS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareClubProlanisModel = new PcareClubProlanisModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareClubProlanisModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareClubProlanisModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareClubProlanisModel.getProps();
		this.modelRelations = PcareClubProlanisModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareClubProlanisModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareClubProlanisModel {
		let updatedModel: PcareClubProlanisModel = new PcareClubProlanisModel(model);
