...
import {AdminPcareClubProlanisCrudListComponent} from './pcare-club-prolanis-admin-crud-list.component';
import {PcareClubProlanisDataFactory} from 'src/app/lib/utils/factories/pcare-club-prolanis-data-factory';
import {PcareClubProlanisModel} from 'src/app/models/pcareClubProlanis/pcare_club_prolanis.model';
import {AdminPcareClubProlanisCrudModule} from '../pcare-club-prolanis-admin-crud.module';
import {
	PcareClubProlanisModelState,
	initialState as initialPcareClubProlanisModelState
} from 'src/app/models/pcareClubProlanis/pcare_club_prolanis.model.state';
 ...
 */
describe('PCare Club Prolanis Crud List Tile Component', () => {

	let fixture;
	let pcareClubProlanisCrudListComponent: AdminPcareClubProlanisCrudListComponent;

	let store: MockStore<{ model: PcareClubProlanisModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareClubProlanisModelState: PcareClubProlanisModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareClubProlanisModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '83b09eff-ae82-4d2e-a4b2-61f77a1bfde1';
	const searchContent = 'text';

	const searchableFieldCount = PcareClubProlanisModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-club-prolanis-admin-crud',
		urls: ['pcare-club-prolanis-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareClubProlanisDataFactory = new PcareClubProlanisDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareClubProlanisDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareClubProlanisModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareClubProlanisModelState = initialPcareClubProlanisModelState;
		store.setState({model: pcareClubProlanisModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareClubProlanisCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareClubProlanisCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPcareClubProlanisCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPcareClubProlanisCrudListComponent);
			pcareClubProlanisCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareClubProlanisCrudListComponent.pcareClubProlanissCount).toBe(defaultData.length);
			...
			expect(pcareClubProlanisCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareClubProlanisCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareClubProlanisDataFactory.createAll(filteredDataSize);
			...
			expect(pcareClubProlanisCrudListComponent.pcareClubProlanissCount).toBe(filteredDataSize);
			expect(pcareClubProlanisCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareClubProlanisCrudListComponent.pageIndex).toBe(0);
			expect(pcareClubProlanisCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareClubProlanisCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareClubProlanisCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareClubProlanisCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
