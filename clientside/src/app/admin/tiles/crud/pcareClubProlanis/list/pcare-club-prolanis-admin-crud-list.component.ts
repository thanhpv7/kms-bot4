...
import * as modelAction from 'src/app/models/pcareClubProlanis/pcare_club_prolanis.model.action';
import {PcareClubProlanisModelState} from 'src/app/models/pcareClubProlanis/pcare_club_prolanis.model.state';
...
import {
	getPcareClubProlanisCollectionModels,
	getPcareClubProlanisCollectionCount
} from 'src/app/models/pcareClubProlanis/pcare_club_prolanis.model.selector';
...
import {CrudTileMode} from '../pcare-club-prolanis-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-club-prolanis-admin-crud-list',
	templateUrl: './pcare-club-prolanis-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-club-prolanis-admin-crud-list.component.scss',
...
export class AdminPcareClubProlanisCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareClubProlanisModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareClubProlanisModel.searchFields,
	...
	/**
	 * List of PCare Club Prolaniss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	pcareClubProlaniss: Observable<PcareClubProlanisModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	pcareClubProlanissCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PcareClubProlanisModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PcareClubProlanisModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '83b09eff-ae82-4d2e-a4b2-61f77a1bfde1';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareClubProlanissCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareClubProlanissCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareClubProlanisModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareClubProlanisAction(
				modelAction.PcareClubProlanisModelActionTypes.INITIALISE_PCARE_CLUB_PROLANIS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareClubProlanisAction(
				modelAction.PcareClubProlanisModelActionTypes.FETCH_PCARE_CLUB_PROLANIS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareClubProlanisAction(
						modelAction.PcareClubProlanisModelActionTypes.COUNT_PCARE_CLUB_PROLANISS,
						null
					)
				]
			))
		...
		this.pcareClubProlaniss = this.store.select(getPcareClubProlanisCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareClubProlanisCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareClubProlanissCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('pcare-club-prolanis');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareClubProlanisModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareClubProlanisModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareClubProlanisModelActionTypes.EXPORT_ALL_PCARE_CLUB_PROLANIS;
		} else {
			actionType = modelAction.PcareClubProlanisModelActionTypes.EXPORT_PCARE_CLUB_PROLANIS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareClubProlanisAction(
				modelAction.PcareClubProlanisModelActionTypes.IMPORT_PCARE_CLUB_PROLANISS,
				{
					file: file,
				},
				[new modelAction.PcareClubProlanisAction(
			...
			window.open(environment.API_URL + '/api/pcareclubprolanis/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareClubProlanisModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Club Prolanis entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Club Prolanis entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareClubProlanisModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareClubProlanisModelActionTypes.DELETE_PCARE_CLUB_PROLANIS;
						} else if (usingSelected) {
							actionType = modelAction.PcareClubProlanisModelActionTypes.DELETE_ALL_PCARE_CLUB_PROLANIS;
						} else {
							actionType = modelAction.PcareClubProlanisModelActionTypes.DELETE_PCARE_CLUB_PROLANIS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareClubProlanisAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_CLUB_PROLANIS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_CLUB_PROLANIS and DELETE_PCARE_CLUB_PROLANIS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareClubProlanisModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareClubProlanisModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareClubProlanissCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
