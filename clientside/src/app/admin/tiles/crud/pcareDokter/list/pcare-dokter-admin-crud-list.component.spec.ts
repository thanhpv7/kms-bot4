...
import {AdminPcareDokterCrudListComponent} from './pcare-dokter-admin-crud-list.component';
...
import {AdminPcareDokterCrudModule} from '../pcare-dokter-admin-crud.module';
import {
	PcareDokterModelState,
	initialState as initialPcareDokterModelState
} from 'src/app/models/pcareDokter/pcare_dokter.model.state';
 ...
 */
describe('PCare Dokter Crud List Tile Component', () => {

	let fixture;
	let pcareDokterCrudListComponent: AdminPcareDokterCrudListComponent;

	let store: MockStore<{ model: PcareDokterModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pcareDokterModelState: PcareDokterModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PcareDokterModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c0a7643e-d07b-4bad-bb33-410487cc915f';
	const searchContent = 'text';

	const searchableFieldCount = PcareDokterModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pcare-dokter-admin-crud',
		urls: ['pcare-dokter-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pcareDokterDataFactory = new PcareDokterDataFactory();

	const defaultDataSize = 30;
	const defaultData = pcareDokterDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PcareDokterModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pcareDokterModelState = initialPcareDokterModelState;
		store.setState({model: pcareDokterModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPcareDokterCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPcareDokterCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPcareDokterCrudListComponent);
			pcareDokterCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pcareDokterCrudListComponent.pcareDoktersCount).toBe(defaultData.length);
			...
			expect(pcareDokterCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pcareDokterCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pcareDokterDataFactory.createAll(filteredDataSize);
			...
			expect(pcareDokterCrudListComponent.pcareDoktersCount).toBe(filteredDataSize);
			expect(pcareDokterCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pcareDokterCrudListComponent.pageIndex).toBe(0);
			expect(pcareDokterCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pcareDokterCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pcareDokterCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pcareDokterCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
