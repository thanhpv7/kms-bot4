...
import * as modelAction from 'src/app/models/pcareDokter/pcare_dokter.model.action';
import {PcareDokterModelState} from 'src/app/models/pcareDokter/pcare_dokter.model.state';
...
import {
	getPcareDokterCollectionModels,
	getPcareDokterCollectionCount
...
import {CrudTileMode} from '../pcare-dokter-admin-crud.component';
...
@Component({
	selector: 'cb-pcare-dokter-admin-crud-list',
	templateUrl: './pcare-dokter-admin-crud-list.component.html',
	styleUrls: [
		'./pcare-dokter-admin-crud-list.component.scss',
...
export class AdminPcareDokterCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PcareDokterModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PcareDokterModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PcareDokterModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'c0a7643e-d07b-4bad-bb33-410487cc915f';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pcareDoktersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pcareDoktersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PcareDokterModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareDokterAction(
				modelAction.PcareDokterModelActionTypes.INITIALISE_PCARE_DOKTER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PcareDokterAction(
				modelAction.PcareDokterModelActionTypes.FETCH_PCARE_DOKTER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PcareDokterAction(
						modelAction.PcareDokterModelActionTypes.COUNT_PCARE_DOKTERS,
						null
					)
				]
			))
		...
		this.pcareDokters = this.store.select(getPcareDokterCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPcareDokterCollectionCount, this.collectionId).subscribe(
				(count) => this.pcareDoktersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PcareDokterModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PcareDokterModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PcareDokterModelActionTypes.EXPORT_ALL_PCARE_DOKTER;
		} else {
			actionType = modelAction.PcareDokterModelActionTypes.EXPORT_PCARE_DOKTER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PcareDokterAction(
				modelAction.PcareDokterModelActionTypes.IMPORT_PCARE_DOKTERS,
				{
					file: file,
				},
				[new modelAction.PcareDokterAction(
			...
			window.open(environment.API_URL + '/api/pcaredokter/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PcareDokterModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected PCare Dokter entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected PCare Dokter entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PcareDokterModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PcareDokterModelActionTypes.DELETE_PCARE_DOKTER;
						} else if (usingSelected) {
							actionType = modelAction.PcareDokterModelActionTypes.DELETE_ALL_PCARE_DOKTER;
						} else {
							actionType = modelAction.PcareDokterModelActionTypes.DELETE_PCARE_DOKTER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PcareDokterAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PCARE_DOKTER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PCARE_DOKTER and DELETE_PCARE_DOKTER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PcareDokterModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PcareDokterModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pcareDoktersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
