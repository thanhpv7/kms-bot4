...
import {PcareDokterModelState, PcareDokterModelAudit} from 'src/app/models/pcareDokter/pcare_dokter.model.state';
import {PcareDokterModel} from 'src/app/models/pcareDokter/pcare_dokter.model';
import {
	getPcareDokterModelAuditsByEntityId,
	getPcareDokterModelWithId,
	getCountPcareDokterModels,
...
@Component({
	selector: 'cb-pcare-dokter-admin-crud-edit',
	templateUrl: './pcare-dokter-admin-crud-edit.component.html',
	styleUrls: [
		'./pcare-dokter-admin-crud-edit.component.scss',
...
export class AdminPcareDokterCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pcareDokterAudits$: Observable<PcareDokterModelAudit[]>;
	...
	staffOppositeRelation = 'pcareDokter';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PcareDokterModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPcareDokterModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PcareDokterModel();
		...
		if (this.targetModelId) {
			this.pcareDokterAudits$ = this.store.select(getPcareDokterModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PcareDokterModel> = {
		...
		this.store.dispatch(new modelAction.PcareDokterAction(
			modelAction.PcareDokterModelActionTypes.FETCH_PCARE_DOKTER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPcareDokterModelWithId, this.targetModelId)
			...
			model = new PcareDokterModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PcareDokterAction(
			modelAction.PcareDokterModelActionTypes.FETCH_PCARE_DOKTER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PcareDokterAction(
				modelAction.PcareDokterModelActionTypes.CREATE_PCARE_DOKTER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PcareDokterAction(
				modelAction.PcareDokterModelActionTypes.UPDATE_PCARE_DOKTER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PcareDokterModel = new PcareDokterModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPcareDokterModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PcareDokterModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PcareDokterModel.getProps();
		this.modelRelations = PcareDokterModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PcareDokterModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PcareDokterModel {
		let updatedModel: PcareDokterModel = new PcareDokterModel(model);
