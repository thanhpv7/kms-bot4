...
import {HemodialysisExaminationModelState, HemodialysisExaminationModelAudit} from 'src/app/models/hemodialysisExamination/hemodialysis_examination.model.state';
import {HemodialysisExaminationModel} from 'src/app/models/hemodialysisExamination/hemodialysis_examination.model';
import {
	getHemodialysisExaminationModelAuditsByEntityId,
	getHemodialysisExaminationModelWithId,
	getCountHemodialysisExaminationModels,
...
@Component({
	selector: 'cb-hemodialysis-examination-admin-crud-edit',
	templateUrl: './hemodialysis-examination-admin-crud-edit.component.html',
	styleUrls: [
		'./hemodialysis-examination-admin-crud-edit.component.scss',
...
export class AdminHemodialysisExaminationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	hemodialysisExaminationAudits$: Observable<HemodialysisExaminationModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'hemodialysisExamination';
	...
	hemodialysisMonitoringsOppositeRelation = 'hemodialysisExamination';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = HemodialysisExaminationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: HemodialysisExaminationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountHemodialysisExaminationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new HemodialysisExaminationModel();
		...
		if (this.targetModelId) {
			this.hemodialysisExaminationAudits$ = this.store.select(getHemodialysisExaminationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<HemodialysisExaminationModel> = {
		...
		this.store.dispatch(new modelAction.HemodialysisExaminationAction(
			modelAction.HemodialysisExaminationModelActionTypes.FETCH_HEMODIALYSIS_EXAMINATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getHemodialysisExaminationModelWithId, this.targetModelId)
			...
			model = new HemodialysisExaminationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.HemodialysisExaminationAction(
			modelAction.HemodialysisExaminationModelActionTypes.FETCH_HEMODIALYSIS_EXAMINATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<HemodialysisExaminationModel> = {
				...
				queryParams: {
					persistRelations: this.medicalExaminationRecordMultiCrudActive || this.hemodialysisMonitoringsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.HemodialysisExaminationAction(
				modelAction.HemodialysisExaminationModelActionTypes.CREATE_HEMODIALYSIS_EXAMINATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.HemodialysisExaminationAction(
				modelAction.HemodialysisExaminationModelActionTypes.UPDATE_HEMODIALYSIS_EXAMINATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: HemodialysisExaminationModel = new HemodialysisExaminationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getHemodialysisExaminationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new HemodialysisExaminationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = HemodialysisExaminationModel.getProps();
		this.modelRelations = HemodialysisExaminationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: HemodialysisExaminationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): HemodialysisExaminationModel {
		let updatedModel: HemodialysisExaminationModel = new HemodialysisExaminationModel(model);
