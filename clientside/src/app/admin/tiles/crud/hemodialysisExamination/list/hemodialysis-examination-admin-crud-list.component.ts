...
import * as modelAction from 'src/app/models/hemodialysisExamination/hemodialysis_examination.model.action';
import {HemodialysisExaminationModelState} from 'src/app/models/hemodialysisExamination/hemodialysis_examination.model.state';
...
import {
	getHemodialysisExaminationCollectionModels,
	getHemodialysisExaminationCollectionCount
...
import {CrudTileMode} from '../hemodialysis-examination-admin-crud.component';
...
@Component({
	selector: 'cb-hemodialysis-examination-admin-crud-list',
	templateUrl: './hemodialysis-examination-admin-crud-list.component.html',
	styleUrls: [
		'./hemodialysis-examination-admin-crud-list.component.scss',
...
export class AdminHemodialysisExaminationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<HemodialysisExaminationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...HemodialysisExaminationModel.searchFields,
	...
	/**
	 * List of Hemodialysis Examinations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	hemodialysisExaminations: Observable<HemodialysisExaminationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	hemodialysisExaminationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = HemodialysisExaminationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = HemodialysisExaminationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'd5dfbaf7-91eb-49ea-9b9b-5fbdbdd4d69d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.hemodialysisExaminationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.hemodialysisExaminationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: HemodialysisExaminationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HemodialysisExaminationAction(
				modelAction.HemodialysisExaminationModelActionTypes.INITIALISE_HEMODIALYSIS_EXAMINATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.HemodialysisExaminationAction(
				modelAction.HemodialysisExaminationModelActionTypes.FETCH_HEMODIALYSIS_EXAMINATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.HemodialysisExaminationAction(
						modelAction.HemodialysisExaminationModelActionTypes.COUNT_HEMODIALYSIS_EXAMINATIONS,
						null
					)
				]
			))
		...
		this.hemodialysisExaminations = this.store.select(getHemodialysisExaminationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getHemodialysisExaminationCollectionCount, this.collectionId).subscribe(
				(count) => this.hemodialysisExaminationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('hemodialysis-examination');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: HemodialysisExaminationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.HemodialysisExaminationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.HemodialysisExaminationModelActionTypes.EXPORT_ALL_HEMODIALYSIS_EXAMINATION;
		} else {
			actionType = modelAction.HemodialysisExaminationModelActionTypes.EXPORT_HEMODIALYSIS_EXAMINATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.HemodialysisExaminationAction(
				modelAction.HemodialysisExaminationModelActionTypes.IMPORT_HEMODIALYSIS_EXAMINATIONS,
				{
					file: file,
				},
				[new modelAction.HemodialysisExaminationAction(
			...
			window.open(environment.API_URL + '/api/hemodialysisexamination/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: HemodialysisExaminationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Hemodialysis Examination entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Hemodialysis Examination entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.HemodialysisExaminationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.HemodialysisExaminationModelActionTypes.DELETE_HEMODIALYSIS_EXAMINATION;
						} else if (usingSelected) {
							actionType = modelAction.HemodialysisExaminationModelActionTypes.DELETE_ALL_HEMODIALYSIS_EXAMINATION;
						} else {
							actionType = modelAction.HemodialysisExaminationModelActionTypes.DELETE_HEMODIALYSIS_EXAMINATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.HemodialysisExaminationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_HEMODIALYSIS_EXAMINATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_HEMODIALYSIS_EXAMINATION and DELETE_HEMODIALYSIS_EXAMINATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<HemodialysisExaminationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = HemodialysisExaminationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.hemodialysisExaminationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
