...
import {AdminHemodialysisExaminationCrudListComponent} from './hemodialysis-examination-admin-crud-list.component';
...
import {AdminHemodialysisExaminationCrudModule} from '../hemodialysis-examination-admin-crud.module';
import {
	HemodialysisExaminationModelState,
	initialState as initialHemodialysisExaminationModelState
} from 'src/app/models/hemodialysisExamination/hemodialysis_examination.model.state';
 ...
 */
describe('Hemodialysis Examination Crud List Tile Component', () => {

	let fixture;
	let hemodialysisExaminationCrudListComponent: AdminHemodialysisExaminationCrudListComponent;

	let store: MockStore<{ model: HemodialysisExaminationModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let hemodialysisExaminationModelState: HemodialysisExaminationModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<HemodialysisExaminationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'd5dfbaf7-91eb-49ea-9b9b-5fbdbdd4d69d';
	const searchContent = 'text';

	const searchableFieldCount = HemodialysisExaminationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'hemodialysis-examination-admin-crud',
		urls: ['hemodialysis-examination-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const hemodialysisExaminationDataFactory = new HemodialysisExaminationDataFactory();

	const defaultDataSize = 30;
	const defaultData = hemodialysisExaminationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: HemodialysisExaminationModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		hemodialysisExaminationModelState = initialHemodialysisExaminationModelState;
		store.setState({model: hemodialysisExaminationModelState});
		...
		spyOn(store, 'select')
			.withArgs(getHemodialysisExaminationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getHemodialysisExaminationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminHemodialysisExaminationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminHemodialysisExaminationCrudListComponent);
			hemodialysisExaminationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(hemodialysisExaminationCrudListComponent.hemodialysisExaminationsCount).toBe(defaultData.length);
			...
			expect(hemodialysisExaminationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			hemodialysisExaminationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = hemodialysisExaminationDataFactory.createAll(filteredDataSize);
			...
			expect(hemodialysisExaminationCrudListComponent.hemodialysisExaminationsCount).toBe(filteredDataSize);
			expect(hemodialysisExaminationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(hemodialysisExaminationCrudListComponent.pageIndex).toBe(0);
			expect(hemodialysisExaminationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			hemodialysisExaminationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		hemodialysisExaminationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		hemodialysisExaminationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
