...
import * as modelAction from 'src/app/models/claimUser/claim_user.model.action';
import {ClaimUserModelState} from 'src/app/models/claimUser/claim_user.model.state';
...
import {
	getClaimUserCollectionModels,
	getClaimUserCollectionCount
...
import {CrudTileMode} from '../claim-user-admin-crud.component';
...
@Component({
	selector: 'cb-claim-user-admin-crud-list',
	templateUrl: './claim-user-admin-crud-list.component.html',
	styleUrls: [
		'./claim-user-admin-crud-list.component.scss',
...
export class AdminClaimUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ClaimUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ClaimUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = ClaimUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '6e37f584-6383-4e5f-bca8-5106d04aaac3';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.claimUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.claimUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ClaimUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ClaimUserAction(
				modelAction.ClaimUserModelActionTypes.INITIALISE_CLAIM_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ClaimUserAction(
				modelAction.ClaimUserModelActionTypes.FETCH_CLAIM_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ClaimUserAction(
						modelAction.ClaimUserModelActionTypes.COUNT_CLAIM_USERS,
						null
					)
				]
			))
		...
		this.claimUsers = this.store.select(getClaimUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getClaimUserCollectionCount, this.collectionId).subscribe(
				(count) => this.claimUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ClaimUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ClaimUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ClaimUserModelActionTypes.EXPORT_ALL_CLAIM_USER;
		} else {
			actionType = modelAction.ClaimUserModelActionTypes.EXPORT_CLAIM_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ClaimUserAction(
				modelAction.ClaimUserModelActionTypes.IMPORT_CLAIM_USERS,
				{
					file: file,
				},
				[new modelAction.ClaimUserAction(
			...
			window.open(environment.API_URL + '/api/claimuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ClaimUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Claim User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Claim User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ClaimUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ClaimUserModelActionTypes.DELETE_CLAIM_USER;
						} else if (usingSelected) {
							actionType = modelAction.ClaimUserModelActionTypes.DELETE_ALL_CLAIM_USER;
						} else {
							actionType = modelAction.ClaimUserModelActionTypes.DELETE_CLAIM_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ClaimUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CLAIM_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CLAIM_USER and DELETE_CLAIM_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ClaimUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ClaimUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.claimUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
