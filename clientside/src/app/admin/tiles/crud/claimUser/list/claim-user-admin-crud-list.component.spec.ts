...
import {AdminClaimUserCrudListComponent} from './claim-user-admin-crud-list.component';
...
import {AdminClaimUserCrudModule} from '../claim-user-admin-crud.module';
import {
	ClaimUserModelState,
	initialState as initialClaimUserModelState
} from 'src/app/models/claimUser/claim_user.model.state';
 ...
 */
describe('Claim User Crud List Tile Component', () => {

	let fixture;
	let claimUserCrudListComponent: AdminClaimUserCrudListComponent;

	let store: MockStore<{ model: ClaimUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let claimUserModelState: ClaimUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<ClaimUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '6e37f584-6383-4e5f-bca8-5106d04aaac3';
	const searchContent = 'text';

	const searchableFieldCount = ClaimUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'claim-user-admin-crud',
		urls: ['claim-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const claimUserDataFactory = new ClaimUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = claimUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: ClaimUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		claimUserModelState = initialClaimUserModelState;
		store.setState({model: claimUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getClaimUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getClaimUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminClaimUserCrudListComponent);
			claimUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(claimUserCrudListComponent.claimUsersCount).toBe(defaultData.length);
			...
			expect(claimUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			claimUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = claimUserDataFactory.createAll(filteredDataSize);
			...
			expect(claimUserCrudListComponent.claimUsersCount).toBe(filteredDataSize);
			expect(claimUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(claimUserCrudListComponent.pageIndex).toBe(0);
			expect(claimUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			claimUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		claimUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		claimUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
