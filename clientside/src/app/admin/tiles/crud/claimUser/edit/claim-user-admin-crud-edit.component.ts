...
import {ClaimUserModelState, ClaimUserModelAudit} from 'src/app/models/claimUser/claim_user.model.state';
import {ClaimUserModel} from 'src/app/models/claimUser/claim_user.model';
import {
	getClaimUserModelAuditsByEntityId,
	getClaimUserModelWithId,
	getCountClaimUserModels,
...
@Component({
	selector: 'cb-claim-user-admin-crud-edit',
	templateUrl: './claim-user-admin-crud-edit.component.html',
	styleUrls: [
		'./claim-user-admin-crud-edit.component.scss',
...
export class AdminClaimUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	claimUserAudits$: Observable<ClaimUserModelAudit[]>;
	...
	staffOppositeRelation = 'claimUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ClaimUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountClaimUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ClaimUserModel();
		...
		if (this.targetModelId) {
			this.claimUserAudits$ = this.store.select(getClaimUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ClaimUserModel> = {
		...
		this.store.dispatch(new modelAction.ClaimUserAction(
			modelAction.ClaimUserModelActionTypes.FETCH_CLAIM_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getClaimUserModelWithId, this.targetModelId)
			...
			model = new ClaimUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ClaimUserAction(
			modelAction.ClaimUserModelActionTypes.FETCH_CLAIM_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.ClaimUserAction(
				modelAction.ClaimUserModelActionTypes.CREATE_CLAIM_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ClaimUserAction(
				modelAction.ClaimUserModelActionTypes.UPDATE_CLAIM_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ClaimUserModel = new ClaimUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getClaimUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ClaimUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ClaimUserModel.getProps();
		this.modelRelations = ClaimUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ClaimUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ClaimUserModel {
		let updatedModel: ClaimUserModel = new ClaimUserModel(model);
