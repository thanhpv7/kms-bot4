...
import {AdminCompoundPrescriptionDetailCrudListComponent} from './compound-prescription-detail-admin-crud-list.component';
...
import {AdminCompoundPrescriptionDetailCrudModule} from '../compound-prescription-detail-admin-crud.module';
import {
 ...
 */
describe('Compound Prescription Detail Crud List Tile Component', () => {

	let fixture;
	let compoundPrescriptionDetailCrudListComponent: AdminCompoundPrescriptionDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<CompoundPrescriptionDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '095e36d2-9871-456c-b7c8-fbeeaaba7422';
	const searchContent = 'text';

	const searchableFieldCount = CompoundPrescriptionDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'compound-prescription-detail-admin-crud',
		urls: ['compound-prescription-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const compoundPrescriptionDetailDataFactory = new CompoundPrescriptionDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = compoundPrescriptionDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getCompoundPrescriptionDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getCompoundPrescriptionDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminCompoundPrescriptionDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminCompoundPrescriptionDetailCrudListComponent);
			compoundPrescriptionDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(compoundPrescriptionDetailCrudListComponent.compoundPrescriptionDetailsCount).toBe(defaultData.length);
			...
			expect(compoundPrescriptionDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			compoundPrescriptionDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = compoundPrescriptionDetailDataFactory.createAll(filteredDataSize);
			...
			expect(compoundPrescriptionDetailCrudListComponent.compoundPrescriptionDetailsCount).toBe(filteredDataSize);
			expect(compoundPrescriptionDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(compoundPrescriptionDetailCrudListComponent.pageIndex).toBe(0);
			expect(compoundPrescriptionDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			compoundPrescriptionDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		compoundPrescriptionDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		compoundPrescriptionDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
