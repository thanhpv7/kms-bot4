...
import {CompoundPrescriptionDetailModelState} from 'src/app/models/compoundPrescriptionDetail/compound_prescription_detail.model.state';
...
import {
	...
	getCompoundPrescriptionDetailCollectionCount
...
@Component({
	selector: 'cb-compound-prescription-detail-admin-crud-list',
	templateUrl: './compound-prescription-detail-admin-crud-list.component.html',
	styleUrls: [
		'./compound-prescription-detail-admin-crud-list.component.scss',
...
export class AdminCompoundPrescriptionDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<CompoundPrescriptionDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...CompoundPrescriptionDetailModel.searchFields,
	...
	/**
	 * List of Compound Prescription Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	compoundPrescriptionDetails: Observable<CompoundPrescriptionDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	compoundPrescriptionDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.compoundPrescriptionDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.compoundPrescriptionDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
				modelAction.CompoundPrescriptionDetailModelActionTypes.INITIALISE_COMPOUND_PRESCRIPTION_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
				modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.compoundPrescriptionDetails = this.store.select(getCompoundPrescriptionDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getCompoundPrescriptionDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.compoundPrescriptionDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: CompoundPrescriptionDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.CompoundPrescriptionDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.CompoundPrescriptionDetailModelActionTypes.EXPORT_ALL_COMPOUND_PRESCRIPTION_DETAIL;
		} else {
			actionType = modelAction.CompoundPrescriptionDetailModelActionTypes.EXPORT_COMPOUND_PRESCRIPTION_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
				modelAction.CompoundPrescriptionDetailModelActionTypes.IMPORT_COMPOUND_PRESCRIPTION_DETAILS,
				{
					file: file,
				},
				[new modelAction.CompoundPrescriptionDetailAction(
			...
			window.open(environment.API_URL + '/api/compoundprescriptiondetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: CompoundPrescriptionDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Compound Prescription Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Compound Prescription Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.CompoundPrescriptionDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.CompoundPrescriptionDetailModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.CompoundPrescriptionDetailModelActionTypes.DELETE_ALL_COMPOUND_PRESCRIPTION_DETAIL;
						} else {
							actionType = modelAction.CompoundPrescriptionDetailModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_COMPOUND_PRESCRIPTION_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_COMPOUND_PRESCRIPTION_DETAIL and DELETE_COMPOUND_PRESCRIPTION_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = CompoundPrescriptionDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.compoundPrescriptionDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
