...
import * as modelAction from 'src/app/models/compoundPrescriptionDetail/compound_prescription_detail.model.action';
import {CompoundPrescriptionDetailModelState, CompoundPrescriptionDetailModelAudit} from 'src/app/models/compoundPrescriptionDetail/compound_prescription_detail.model.state';
import {CompoundPrescriptionDetailModel} from 'src/app/models/compoundPrescriptionDetail/compound_prescription_detail.model';
import {
	getCompoundPrescriptionDetailModelAuditsByEntityId,
...
import {CrudTileMode} from '../compound-prescription-detail-admin-crud.component';
...
@Component({
	selector: 'cb-compound-prescription-detail-admin-crud-edit',
	templateUrl: './compound-prescription-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./compound-prescription-detail-admin-crud-edit.component.scss',
...
export class AdminCompoundPrescriptionDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '095e36d2-9871-456c-b7c8-fbeeaaba7422';
	 ...
	 */
	compoundPrescriptionDetailAudits$: Observable<CompoundPrescriptionDetailModelAudit[]>;
	...
	consumptionUnitMultiCrudActive: boolean = false;
	consumptionUnitDisplayName = 'Consumption Unit';
	...
	consumptionUnitOppositeRelation = 'compoundPrescriptionDetails';
	consumptionUnitRelations: ModelRelation[] = this.consumptionUnitMultiCrudActive ? UnitOfMeasurementModel.getRelations()
		.map(relation => {
			if (relation.id === this.consumptionUnitOppositeRelation) {
	...
	compoundPrescriptionItemsOppositeRelation = 'compoundPrescriptionDetail';
	 ...
	 */
	modelProperties: ModelProperty[] = CompoundPrescriptionDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = CompoundPrescriptionDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = CompoundPrescriptionDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'consumptionUnit' && this.consumptionUnitMultiCrudActive) {
	 ...
	 */
	targetModel: CompoundPrescriptionDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: CompoundPrescriptionDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountCompoundPrescriptionDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new CompoundPrescriptionDetailModel();
		...
		if (this.targetModelId) {
			this.compoundPrescriptionDetailAudits$ = this.store.select(getCompoundPrescriptionDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<CompoundPrescriptionDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
			modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getCompoundPrescriptionDetailModelWithId, this.targetModelId)
			...
			model = new CompoundPrescriptionDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('compound-prescription-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
			modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<CompoundPrescriptionDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.consumptionUnitMultiCrudActive || this.compoundPrescriptionItemsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.CompoundPrescriptionDetailAction(
						modelAction.CompoundPrescriptionDetailModelActionTypes.COUNT_COMPOUND_PRESCRIPTION_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
				modelAction.CompoundPrescriptionDetailModelActionTypes.CREATE_COMPOUND_PRESCRIPTION_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.CompoundPrescriptionDetailAction(
				modelAction.CompoundPrescriptionDetailModelActionTypes.UPDATE_COMPOUND_PRESCRIPTION_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: CompoundPrescriptionDetailModel = new CompoundPrescriptionDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getCompoundPrescriptionDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new CompoundPrescriptionDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.consumptionUnitDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(UnitOfMeasurementModel.getProps(), this.consumptionUnitRelations, this.isViewOnly),
					new UnitOfMeasurementModel(),
					UnitOfMeasurementModel,
					this.consumptionUnitDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.consumptionUnitRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = CompoundPrescriptionDetailModel.getProps();
		this.modelRelations = CompoundPrescriptionDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'consumptionUnit';
		}).hideElement = this.consumptionUnitMultiCrudActive || this.modelRelations.find(rel => rel.id === 'consumptionUnit').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.consumptionUnitMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.consumptionUnitDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.consumptionUnit && this.targetModel.consumptionUnit.id && this.consumptionUnitMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				UnitOfMeasurementModel,
				this.consumptionUnitOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.consumptionUnit.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getUnitOfMeasurementModelWithId, this.targetModel.consumptionUnitId).subscribe(consumptionUnit => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(UnitOfMeasurementModel.getProps(), this.consumptionUnitRelations, this.isViewOnly),
					consumptionUnit,
					UnitOfMeasurementModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: CompoundPrescriptionDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): CompoundPrescriptionDetailModel {
		let updatedModel: CompoundPrescriptionDetailModel = new CompoundPrescriptionDetailModel(model);

		if (this.consumptionUnitMultiCrudActive) {
			updatedModel.consumptionUnit = event.additionalEntities.consumptionUnit ? event.additionalEntities.consumptionUnit : undefined;

			if (!updatedModel.consumptionUnit && event.payload) {
				event.payload.consumptionUnitId = null;
			}
		} else if (event.payload?.consumptionUnitId !== undefined) {
			event.payload.consumptionUnit = null;
			updatedModel.consumptionUnit = null;
		}
