...
import * as modelAction from 'src/app/models/purchaseOrder/purchase_order.model.action';
import {PurchaseOrderModelState, PurchaseOrderModelAudit} from 'src/app/models/purchaseOrder/purchase_order.model.state';
import {PurchaseOrderModel} from 'src/app/models/purchaseOrder/purchase_order.model';
import {
	getPurchaseOrderModelAuditsByEntityId,
...
import {CrudTileMode} from '../purchase-order-admin-crud.component';
...
@Component({
	selector: 'cb-purchase-order-admin-crud-edit',
	templateUrl: './purchase-order-admin-crud-edit.component.html',
	styleUrls: [
		'./purchase-order-admin-crud-edit.component.scss',
...
export class AdminPurchaseOrderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '4af3a107-a0b2-4b15-9332-d68e098ee3f9';
	 ...
	 */
	purchaseOrderAudits$: Observable<PurchaseOrderModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Goods Receive Note and Purchase Orders here] off begin
	goodsReceiveNoteMultiCrudActive: boolean = false;
	goodsReceiveNoteDisplayName = 'Goods Receive Note';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Goods Receive Note and Purchase Orders here] end
	goodsReceiveNoteOppositeRelation = 'purchaseOrders';
	...
	supplierOppositeRelation = 'purchaseOrders';
	...
	warehouseOppositeRelation = 'purchaseOrders';
	...
	purchaseOrderStockDetailsOppositeRelation = 'purchaseOrder';
	...
	purchaseRequisitionsOppositeRelation = 'purchaseOrder';
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Purchase Order and Return Suppliers here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Purchase Order and Return Suppliers here] end
	returnSuppliersOppositeRelation = 'purchaseOrder';
	 ...
	 */
	modelProperties: ModelProperty[] = PurchaseOrderModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PurchaseOrderModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PurchaseOrderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PurchaseOrderModel;
	...
	constructor(
		private readonly store: Store<{ model: PurchaseOrderModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPurchaseOrderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PurchaseOrderModel();
		...
		if (this.targetModelId) {
			this.purchaseOrderAudits$ = this.store.select(getPurchaseOrderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PurchaseOrderModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PurchaseOrderAction(
			modelAction.PurchaseOrderModelActionTypes.FETCH_PURCHASE_ORDER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPurchaseOrderModelWithId, this.targetModelId)
			...
			model = new PurchaseOrderModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PurchaseOrderAction(
			modelAction.PurchaseOrderModelActionTypes.FETCH_PURCHASE_ORDER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PurchaseOrderModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.goodsReceiveNoteMultiCrudActive || this.supplierMultiCrudActive || this.warehouseMultiCrudActive || this.purchaseOrderStockDetailsMultiCrudActive || this.purchaseRequisitionsMultiCrudActive || this.returnSuppliersMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PurchaseOrderAction(
						modelAction.PurchaseOrderModelActionTypes.COUNT_PURCHASE_ORDERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PurchaseOrderAction(
				modelAction.PurchaseOrderModelActionTypes.CREATE_PURCHASE_ORDER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PurchaseOrderAction(
				modelAction.PurchaseOrderModelActionTypes.UPDATE_PURCHASE_ORDER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PurchaseOrderModel = new PurchaseOrderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPurchaseOrderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PurchaseOrderModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PurchaseOrderModel.getProps();
		this.modelRelations = PurchaseOrderModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PurchaseOrderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PurchaseOrderModel {
		let updatedModel: PurchaseOrderModel = new PurchaseOrderModel(model);
