...
import {PurchaseOrderModelState} from 'src/app/models/purchaseOrder/purchase_order.model.state';
...
import {
	...
	getPurchaseOrderCollectionCount
...
@Component({
	selector: 'cb-purchase-order-admin-crud-list',
	templateUrl: './purchase-order-admin-crud-list.component.html',
	styleUrls: [
		'./purchase-order-admin-crud-list.component.scss',
...
export class AdminPurchaseOrderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PurchaseOrderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PurchaseOrderModel.searchFields,
	...
	/**
	 * List of Purchase Orders that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.purchaseOrdersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.purchaseOrdersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseOrderAction(
				modelAction.PurchaseOrderModelActionTypes.INITIALISE_PURCHASE_ORDER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseOrderAction(
				modelAction.PurchaseOrderModelActionTypes.FETCH_PURCHASE_ORDER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.purchaseOrders = this.store.select(getPurchaseOrderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPurchaseOrderCollectionCount, this.collectionId).subscribe(
				(count) => this.purchaseOrdersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PurchaseOrderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PurchaseOrderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PurchaseOrderModelActionTypes.EXPORT_ALL_PURCHASE_ORDER;
		} else {
			actionType = modelAction.PurchaseOrderModelActionTypes.EXPORT_PURCHASE_ORDER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PurchaseOrderAction(
				modelAction.PurchaseOrderModelActionTypes.IMPORT_PURCHASE_ORDERS,
				{
					file: file,
				},
				[new modelAction.PurchaseOrderAction(
			...
			window.open(environment.API_URL + '/api/purchaseorder/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PurchaseOrderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Purchase Order entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Purchase Order entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PurchaseOrderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PurchaseOrderModelActionTypes.DELETE_PURCHASE_ORDER;
						} else if (usingSelected) {
							actionType = modelAction.PurchaseOrderModelActionTypes.DELETE_ALL_PURCHASE_ORDER;
						} else {
							actionType = modelAction.PurchaseOrderModelActionTypes.DELETE_PURCHASE_ORDER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PurchaseOrderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PURCHASE_ORDER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PURCHASE_ORDER and DELETE_PURCHASE_ORDER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PurchaseOrderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.purchaseOrdersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
