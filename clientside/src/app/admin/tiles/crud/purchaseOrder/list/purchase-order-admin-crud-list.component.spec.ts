...
import {AdminPurchaseOrderCrudListComponent} from './purchase-order-admin-crud-list.component';
...
import {AdminPurchaseOrderCrudModule} from '../purchase-order-admin-crud.module';
import {
 ...
 */
describe('Purchase Order Crud List Tile Component', () => {

	let fixture;
	let purchaseOrderCrudListComponent: AdminPurchaseOrderCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PurchaseOrderModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '4af3a107-a0b2-4b15-9332-d68e098ee3f9';
	const searchContent = 'text';

	const searchableFieldCount = PurchaseOrderModel.searchFields.length;

	const routerState: RouterState = {
		url: 'purchase-order-admin-crud',
		urls: ['purchase-order-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const purchaseOrderDataFactory = new PurchaseOrderDataFactory();

	const defaultDataSize = 30;
	const defaultData = purchaseOrderDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPurchaseOrderCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPurchaseOrderCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPurchaseOrderCrudListComponent);
			purchaseOrderCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(purchaseOrderCrudListComponent.purchaseOrdersCount).toBe(defaultData.length);
			...
			expect(purchaseOrderCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			purchaseOrderCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = purchaseOrderDataFactory.createAll(filteredDataSize);
			...
			expect(purchaseOrderCrudListComponent.purchaseOrdersCount).toBe(filteredDataSize);
			expect(purchaseOrderCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(purchaseOrderCrudListComponent.pageIndex).toBe(0);
			expect(purchaseOrderCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			purchaseOrderCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		purchaseOrderCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		purchaseOrderCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
