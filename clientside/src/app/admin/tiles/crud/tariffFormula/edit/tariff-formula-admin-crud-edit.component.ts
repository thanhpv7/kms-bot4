...
import * as modelAction from 'src/app/models/tariffFormula/tariff_formula.model.action';
import {TariffFormulaModelState, TariffFormulaModelAudit} from 'src/app/models/tariffFormula/tariff_formula.model.state';
...
import {
	getTariffFormulaModelAuditsByEntityId,
...
import {CrudTileMode} from '../tariff-formula-admin-crud.component';
...
@Component({
	selector: 'cb-tariff-formula-admin-crud-edit',
	templateUrl: './tariff-formula-admin-crud-edit.component.html',
	styleUrls: [
		'./tariff-formula-admin-crud-edit.component.scss',
...
export class AdminTariffFormulaCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '5219a9d3-da1d-4eac-a42f-9ff34fea0de7';
	 ...
	 */
	tariffFormulaAudits$: Observable<TariffFormulaModelAudit[]>;
	...
	baseSchemaMultiCrudActive: boolean = false;
	baseSchemaDisplayName = 'Base Schema';
	...
	baseSchemaOppositeRelation = 'baseScheme';
	baseSchemaRelations: ModelRelation[] = this.baseSchemaMultiCrudActive ? TariffSchemaModel.getRelations()
		.map(relation => {
			if (relation.id === this.baseSchemaOppositeRelation) {
	...
	tariffSchemaOppositeRelation = 'tariffScheme';
	 ...
	 */
	modelProperties: ModelProperty[] = TariffFormulaModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = TariffFormulaModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = TariffFormulaModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'baseSchema' && this.baseSchemaMultiCrudActive) {
	 ...
	 */
	targetModel: TariffFormulaModel;
	...
	constructor(
		private readonly store: Store<{ model: TariffFormulaModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountTariffFormulaModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new TariffFormulaModel();
		...
		if (this.targetModelId) {
			this.tariffFormulaAudits$ = this.store.select(getTariffFormulaModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<TariffFormulaModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.TariffFormulaAction(
			modelAction.TariffFormulaModelActionTypes.FETCH_TARIFF_FORMULA,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getTariffFormulaModelWithId, this.targetModelId)
			...
			model = new TariffFormulaModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.TariffFormulaAction(
			modelAction.TariffFormulaModelActionTypes.FETCH_TARIFF_FORMULA_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<TariffFormulaModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.baseSchemaMultiCrudActive || this.tariffSchemaMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.TariffFormulaAction(
						modelAction.TariffFormulaModelActionTypes.COUNT_TARIFF_FORMULAS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.TariffFormulaAction(
				modelAction.TariffFormulaModelActionTypes.CREATE_TARIFF_FORMULA,
				stateConfig,
			...
			this.store.dispatch(new modelAction.TariffFormulaAction(
				modelAction.TariffFormulaModelActionTypes.UPDATE_TARIFF_FORMULA,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: TariffFormulaModel = new TariffFormulaModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getTariffFormulaModelWithId, this.targetModelId).subscribe(model => {
					let differences = new TariffFormulaModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.baseSchemaDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffSchemaModel.getProps(), this.baseSchemaRelations, this.isViewOnly),
					new TariffSchemaModel(),
					TariffSchemaModel,
					this.baseSchemaDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.baseSchemaRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = TariffFormulaModel.getProps();
		this.modelRelations = TariffFormulaModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'baseSchema';
		}).hideElement = this.baseSchemaMultiCrudActive || this.modelRelations.find(rel => rel.id === 'baseSchema').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.baseSchemaMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.baseSchemaDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.baseSchema && this.targetModel.baseSchema.id && this.baseSchemaMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				TariffSchemaModel,
				this.baseSchemaOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.baseSchema.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getTariffSchemaModelWithId, this.targetModel.baseSchemaId).subscribe(baseSchema => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(TariffSchemaModel.getProps(), this.baseSchemaRelations, this.isViewOnly),
					baseSchema,
					TariffSchemaModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: TariffFormulaModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): TariffFormulaModel {
		let updatedModel: TariffFormulaModel = new TariffFormulaModel(model);

		if (this.baseSchemaMultiCrudActive) {
			updatedModel.baseSchema = event.additionalEntities.baseSchema ? event.additionalEntities.baseSchema : undefined;

			if (!updatedModel.baseSchema && event.payload) {
				event.payload.baseSchemaId = null;
			}
		} else if (event.payload?.baseSchemaId !== undefined) {
			event.payload.baseSchema = null;
			updatedModel.baseSchema = null;
		}
