...
import {AdminTariffFormulaCrudListComponent} from './tariff-formula-admin-crud-list.component';
...
import {AdminTariffFormulaCrudModule} from '../tariff-formula-admin-crud.module';
import {
 ...
 */
describe('Tariff Formula Crud List Tile Component', () => {

	let fixture;
	let tariffFormulaCrudListComponent: AdminTariffFormulaCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<TariffFormulaModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '5219a9d3-da1d-4eac-a42f-9ff34fea0de7';
	const searchContent = 'text';

	const searchableFieldCount = TariffFormulaModel.searchFields.length;

	const routerState: RouterState = {
		url: 'tariff-formula-admin-crud',
		urls: ['tariff-formula-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const tariffFormulaDataFactory = new TariffFormulaDataFactory();

	const defaultDataSize = 30;
	const defaultData = tariffFormulaDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getTariffFormulaCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getTariffFormulaCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminTariffFormulaCrudListComponent);
			tariffFormulaCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(tariffFormulaCrudListComponent.tariffFormulasCount).toBe(defaultData.length);
			...
			expect(tariffFormulaCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			tariffFormulaCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = tariffFormulaDataFactory.createAll(filteredDataSize);
			...
			expect(tariffFormulaCrudListComponent.tariffFormulasCount).toBe(filteredDataSize);
			expect(tariffFormulaCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(tariffFormulaCrudListComponent.pageIndex).toBe(0);
			expect(tariffFormulaCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			tariffFormulaCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		tariffFormulaCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		tariffFormulaCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
