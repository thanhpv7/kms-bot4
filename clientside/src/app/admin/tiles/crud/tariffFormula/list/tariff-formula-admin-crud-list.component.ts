...
import {TariffFormulaModelState} from 'src/app/models/tariffFormula/tariff_formula.model.state';
...
import {
	...
	getTariffFormulaCollectionCount
...
@Component({
	selector: 'cb-tariff-formula-admin-crud-list',
	templateUrl: './tariff-formula-admin-crud-list.component.html',
	styleUrls: [
		'./tariff-formula-admin-crud-list.component.scss',
...
export class AdminTariffFormulaCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<TariffFormulaModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...TariffFormulaModel.searchFields,
	...
	/**
	 * List of Tariff Formulas that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	tariffFormulas: Observable<TariffFormulaModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	tariffFormulasCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.tariffFormulasCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.tariffFormulasCount === 0
		},
		{
	...
	filterQuestions: FilterQuestion[] = [
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Tariff Type to Tariff Formula here] off begin
		{
			...
			config: {
				...
				labelField: 'value',
				valueField: 'key',
				id: 'TariffFormulaTariffTypeFilter',
				label: 'Tariff Type',
			},
			name: 'tariffType',
		},
		// % protected region % [Modify the existing config for the dropdown filter question for enum ref Tariff Type to Tariff Formula here] end
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffFormulaAction(
				modelAction.TariffFormulaModelActionTypes.INITIALISE_TARIFF_FORMULA_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.TariffFormulaAction(
				modelAction.TariffFormulaModelActionTypes.FETCH_TARIFF_FORMULA_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.tariffFormulas = this.store.select(getTariffFormulaCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getTariffFormulaCollectionCount, this.collectionId).subscribe(
				(count) => this.tariffFormulasCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: TariffFormulaModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.TariffFormulaModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.TariffFormulaModelActionTypes.EXPORT_ALL_TARIFF_FORMULA;
		} else {
			actionType = modelAction.TariffFormulaModelActionTypes.EXPORT_TARIFF_FORMULA_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.TariffFormulaAction(
				modelAction.TariffFormulaModelActionTypes.IMPORT_TARIFF_FORMULAS,
				{
					file: file,
				},
				[new modelAction.TariffFormulaAction(
			...
			window.open(environment.API_URL + '/api/tariffformula/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: TariffFormulaModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Tariff Formula entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Tariff Formula entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.TariffFormulaModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.TariffFormulaModelActionTypes.DELETE_TARIFF_FORMULA;
						} else if (usingSelected) {
							actionType = modelAction.TariffFormulaModelActionTypes.DELETE_ALL_TARIFF_FORMULA;
						} else {
							actionType = modelAction.TariffFormulaModelActionTypes.DELETE_TARIFF_FORMULA_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.TariffFormulaAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_TARIFF_FORMULA when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_TARIFF_FORMULA and DELETE_TARIFF_FORMULA_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = TariffFormulaModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.tariffFormulasCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
