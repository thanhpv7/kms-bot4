...
import {AdminExaminationItemCrudListComponent} from './examination-item-admin-crud-list.component';
...
import {AdminExaminationItemCrudModule} from '../examination-item-admin-crud.module';
import {
 ...
 */
describe('Examination Item Crud List Tile Component', () => {

	let fixture;
	let examinationItemCrudListComponent: AdminExaminationItemCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ExaminationItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a28b0b8f-ef8a-4791-9b38-c38cab6a8316';
	const searchContent = 'text';

	const searchableFieldCount = ExaminationItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'examination-item-admin-crud',
		urls: ['examination-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const examinationItemDataFactory = new ExaminationItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = examinationItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getExaminationItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getExaminationItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminExaminationItemCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminExaminationItemCrudListComponent);
			examinationItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(examinationItemCrudListComponent.examinationItemsCount).toBe(defaultData.length);
			...
			expect(examinationItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			examinationItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = examinationItemDataFactory.createAll(filteredDataSize);
			...
			expect(examinationItemCrudListComponent.examinationItemsCount).toBe(filteredDataSize);
			expect(examinationItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(examinationItemCrudListComponent.pageIndex).toBe(0);
			expect(examinationItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			examinationItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		examinationItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		examinationItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
