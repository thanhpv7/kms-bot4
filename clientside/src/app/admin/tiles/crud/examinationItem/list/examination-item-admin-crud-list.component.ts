...
import {ExaminationItemModelState} from 'src/app/models/examinationItem/examination_item.model.state';
...
import {
	...
	getExaminationItemCollectionCount
...
import {CrudTileMode} from '../examination-item-admin-crud.component';
...
@Component({
	selector: 'cb-examination-item-admin-crud-list',
	templateUrl: './examination-item-admin-crud-list.component.html',
	styleUrls: [
		'./examination-item-admin-crud-list.component.scss',
...
export class AdminExaminationItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ExaminationItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ExaminationItemModel.searchFields,
	...
	/**
	 * List of Examination Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	examinationItems: Observable<ExaminationItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	examinationItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.examinationItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.examinationItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ExaminationItemAction(
				modelAction.ExaminationItemModelActionTypes.INITIALISE_EXAMINATION_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ExaminationItemAction(
				modelAction.ExaminationItemModelActionTypes.FETCH_EXAMINATION_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.examinationItems = this.store.select(getExaminationItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getExaminationItemCollectionCount, this.collectionId).subscribe(
				(count) => this.examinationItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ExaminationItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ExaminationItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ExaminationItemModelActionTypes.EXPORT_ALL_EXAMINATION_ITEM;
		} else {
			actionType = modelAction.ExaminationItemModelActionTypes.EXPORT_EXAMINATION_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ExaminationItemAction(
				modelAction.ExaminationItemModelActionTypes.IMPORT_EXAMINATION_ITEMS,
				{
					file: file,
				},
				[new modelAction.ExaminationItemAction(
			...
			window.open(environment.API_URL + '/api/examinationitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ExaminationItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Examination Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Examination Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ExaminationItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ExaminationItemModelActionTypes.DELETE_EXAMINATION_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.ExaminationItemModelActionTypes.DELETE_ALL_EXAMINATION_ITEM;
						} else {
							actionType = modelAction.ExaminationItemModelActionTypes.DELETE_EXAMINATION_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ExaminationItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_EXAMINATION_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_EXAMINATION_ITEM and DELETE_EXAMINATION_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ExaminationItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.examinationItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
