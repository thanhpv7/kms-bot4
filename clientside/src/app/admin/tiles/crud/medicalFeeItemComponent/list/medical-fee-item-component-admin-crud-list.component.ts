...
import {MedicalFeeItemComponentModelState} from 'src/app/models/medicalFeeItemComponent/medical_fee_item_component.model.state';
...
import {
	getMedicalFeeItemComponentCollectionModels,
	getMedicalFeeItemComponentCollectionCount
...
@Component({
	selector: 'cb-medical-fee-item-component-admin-crud-list',
	templateUrl: './medical-fee-item-component-admin-crud-list.component.html',
	styleUrls: [
		'./medical-fee-item-component-admin-crud-list.component.scss',
...
export class AdminMedicalFeeItemComponentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalFeeItemComponentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalFeeItemComponentModel.searchFields,
	...
	/**
	 * List of Medical Fee Item Components that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalFeeItemComponents: Observable<MedicalFeeItemComponentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalFeeItemComponentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalFeeItemComponentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalFeeItemComponentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
				modelAction.MedicalFeeItemComponentModelActionTypes.INITIALISE_MEDICAL_FEE_ITEM_COMPONENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
				modelAction.MedicalFeeItemComponentModelActionTypes.FETCH_MEDICAL_FEE_ITEM_COMPONENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalFeeItemComponents = this.store.select(getMedicalFeeItemComponentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalFeeItemComponentCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalFeeItemComponentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalFeeItemComponentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalFeeItemComponentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalFeeItemComponentModelActionTypes.EXPORT_ALL_MEDICAL_FEE_ITEM_COMPONENT;
		} else {
			actionType = modelAction.MedicalFeeItemComponentModelActionTypes.EXPORT_MEDICAL_FEE_ITEM_COMPONENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
				modelAction.MedicalFeeItemComponentModelActionTypes.IMPORT_MEDICAL_FEE_ITEM_COMPONENTS,
				{
					file: file,
				},
				[new modelAction.MedicalFeeItemComponentAction(
			...
			window.open(environment.API_URL + '/api/medicalfeeitemcomponent/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalFeeItemComponentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Fee Item Component entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Fee Item Component entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalFeeItemComponentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalFeeItemComponentModelActionTypes.DELETE_MEDICAL_FEE_ITEM_COMPONENT;
						} else if (usingSelected) {
							actionType = modelAction.MedicalFeeItemComponentModelActionTypes.DELETE_ALL_MEDICAL_FEE_ITEM_COMPONENT;
						} else {
							actionType = modelAction.MedicalFeeItemComponentModelActionTypes.DELETE_MEDICAL_FEE_ITEM_COMPONENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_FEE_ITEM_COMPONENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_FEE_ITEM_COMPONENT and DELETE_MEDICAL_FEE_ITEM_COMPONENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalFeeItemComponentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalFeeItemComponentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
