...
import {AdminMedicalFeeItemComponentCrudListComponent} from './medical-fee-item-component-admin-crud-list.component';
...
import {AdminMedicalFeeItemComponentCrudModule} from '../medical-fee-item-component-admin-crud.module';
import {
	MedicalFeeItemComponentModelState,
	initialState as initialMedicalFeeItemComponentModelState
} from 'src/app/models/medicalFeeItemComponent/medical_fee_item_component.model.state';
 ...
 */
describe('Medical Fee Item Component Crud List Tile Component', () => {

	let fixture;
	let medicalFeeItemComponentCrudListComponent: AdminMedicalFeeItemComponentCrudListComponent;

	let store: MockStore<{ model: MedicalFeeItemComponentModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalFeeItemComponentModelState: MedicalFeeItemComponentModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalFeeItemComponentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '3f0f06f3-a6e5-4b64-b023-28608c872382';
	const searchContent = 'text';

	const searchableFieldCount = MedicalFeeItemComponentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-fee-item-component-admin-crud',
		urls: ['medical-fee-item-component-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalFeeItemComponentDataFactory = new MedicalFeeItemComponentDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalFeeItemComponentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalFeeItemComponentModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalFeeItemComponentModelState = initialMedicalFeeItemComponentModelState;
		store.setState({model: medicalFeeItemComponentModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalFeeItemComponentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalFeeItemComponentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalFeeItemComponentCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalFeeItemComponentCrudListComponent);
			medicalFeeItemComponentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalFeeItemComponentCrudListComponent.medicalFeeItemComponentsCount).toBe(defaultData.length);
			...
			expect(medicalFeeItemComponentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalFeeItemComponentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalFeeItemComponentDataFactory.createAll(filteredDataSize);
			...
			expect(medicalFeeItemComponentCrudListComponent.medicalFeeItemComponentsCount).toBe(filteredDataSize);
			expect(medicalFeeItemComponentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalFeeItemComponentCrudListComponent.pageIndex).toBe(0);
			expect(medicalFeeItemComponentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalFeeItemComponentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalFeeItemComponentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalFeeItemComponentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
