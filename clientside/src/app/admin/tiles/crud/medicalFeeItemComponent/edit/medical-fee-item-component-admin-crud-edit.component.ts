...
import * as modelAction from 'src/app/models/medicalFeeItemComponent/medical_fee_item_component.model.action';
import {MedicalFeeItemComponentModelState, MedicalFeeItemComponentModelAudit} from 'src/app/models/medicalFeeItemComponent/medical_fee_item_component.model.state';
import {MedicalFeeItemComponentModel} from 'src/app/models/medicalFeeItemComponent/medical_fee_item_component.model';
import {
	getMedicalFeeItemComponentModelAuditsByEntityId,
	getMedicalFeeItemComponentModelWithId,
	getCountMedicalFeeItemComponentModels,
...
import {CrudTileMode} from '../medical-fee-item-component-admin-crud.component';
...
@Component({
	selector: 'cb-medical-fee-item-component-admin-crud-edit',
	templateUrl: './medical-fee-item-component-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-fee-item-component-admin-crud-edit.component.scss',
...
export class AdminMedicalFeeItemComponentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '3f0f06f3-a6e5-4b64-b023-28608c872382';
	 ...
	 */
	medicalFeeItemComponentAudits$: Observable<MedicalFeeItemComponentModelAudit[]>;
	...
	medicalFeeOppositeRelation = 'medicalFeeItemComponents';
	...
	priceComponentOppositeRelation = 'medicalFeeItemComponents';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalFeeItemComponentModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalFeeItemComponentModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalFeeItemComponentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalFeeItemComponentModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalFeeItemComponentModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalFeeItemComponentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalFeeItemComponentModel();
		...
		if (this.targetModelId) {
			this.medicalFeeItemComponentAudits$ = this.store.select(getMedicalFeeItemComponentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalFeeItemComponentModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
			modelAction.MedicalFeeItemComponentModelActionTypes.FETCH_MEDICAL_FEE_ITEM_COMPONENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalFeeItemComponentModelWithId, this.targetModelId)
			...
			model = new MedicalFeeItemComponentModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-fee-item-component');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
			modelAction.MedicalFeeItemComponentModelActionTypes.FETCH_MEDICAL_FEE_ITEM_COMPONENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalFeeItemComponentModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.medicalFeeMultiCrudActive || this.priceComponentMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalFeeItemComponentAction(
						modelAction.MedicalFeeItemComponentModelActionTypes.COUNT_MEDICAL_FEE_ITEM_COMPONENTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
				modelAction.MedicalFeeItemComponentModelActionTypes.CREATE_MEDICAL_FEE_ITEM_COMPONENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalFeeItemComponentAction(
				modelAction.MedicalFeeItemComponentModelActionTypes.UPDATE_MEDICAL_FEE_ITEM_COMPONENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalFeeItemComponentModel = new MedicalFeeItemComponentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalFeeItemComponentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalFeeItemComponentModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalFeeItemComponentModel.getProps();
		this.modelRelations = MedicalFeeItemComponentModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalFeeItemComponentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalFeeItemComponentModel {
		let updatedModel: MedicalFeeItemComponentModel = new MedicalFeeItemComponentModel(model);
