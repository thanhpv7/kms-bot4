...
import {FluidBalanceDetailModelState, FluidBalanceDetailModelAudit} from 'src/app/models/fluidBalanceDetail/fluid_balance_detail.model.state';
import {FluidBalanceDetailModel} from 'src/app/models/fluidBalanceDetail/fluid_balance_detail.model';
import {
	getFluidBalanceDetailModelAuditsByEntityId,
...
@Component({
	selector: 'cb-fluid-balance-detail-admin-crud-edit',
	templateUrl: './fluid-balance-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./fluid-balance-detail-admin-crud-edit.component.scss',
...
export class AdminFluidBalanceDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	fluidBalanceDetailAudits$: Observable<FluidBalanceDetailModelAudit[]>;
	...
	registrationOppositeRelation = 'fluidBalanceDetails';
	...
	staffOppositeRelation = 'fluidBalanceDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = FluidBalanceDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: FluidBalanceDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountFluidBalanceDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new FluidBalanceDetailModel();
		...
		if (this.targetModelId) {
			this.fluidBalanceDetailAudits$ = this.store.select(getFluidBalanceDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<FluidBalanceDetailModel> = {
		...
		this.store.dispatch(new modelAction.FluidBalanceDetailAction(
			modelAction.FluidBalanceDetailModelActionTypes.FETCH_FLUID_BALANCE_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getFluidBalanceDetailModelWithId, this.targetModelId)
			...
			model = new FluidBalanceDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.FluidBalanceDetailAction(
			modelAction.FluidBalanceDetailModelActionTypes.FETCH_FLUID_BALANCE_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.FluidBalanceDetailAction(
				modelAction.FluidBalanceDetailModelActionTypes.CREATE_FLUID_BALANCE_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.FluidBalanceDetailAction(
				modelAction.FluidBalanceDetailModelActionTypes.UPDATE_FLUID_BALANCE_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: FluidBalanceDetailModel = new FluidBalanceDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getFluidBalanceDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new FluidBalanceDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = FluidBalanceDetailModel.getProps();
		this.modelRelations = FluidBalanceDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: FluidBalanceDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): FluidBalanceDetailModel {
		let updatedModel: FluidBalanceDetailModel = new FluidBalanceDetailModel(model);
