...
import * as modelAction from 'src/app/models/fluidBalanceDetail/fluid_balance_detail.model.action';
import {FluidBalanceDetailModelState} from 'src/app/models/fluidBalanceDetail/fluid_balance_detail.model.state';
...
import {
	...
	getFluidBalanceDetailCollectionCount
...
import {CrudTileMode} from '../fluid-balance-detail-admin-crud.component';
...
@Component({
	selector: 'cb-fluid-balance-detail-admin-crud-list',
	templateUrl: './fluid-balance-detail-admin-crud-list.component.html',
	styleUrls: [
		'./fluid-balance-detail-admin-crud-list.component.scss',
...
export class AdminFluidBalanceDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<FluidBalanceDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...FluidBalanceDetailModel.searchFields,
	...
	/**
	 * List of Fluid Balance Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	fluidBalanceDetails: Observable<FluidBalanceDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	fluidBalanceDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = FluidBalanceDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = FluidBalanceDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '9c6d5fc1-7cb6-4808-8ac9-8c11dea19210';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.fluidBalanceDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.fluidBalanceDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: FluidBalanceDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FluidBalanceDetailAction(
				modelAction.FluidBalanceDetailModelActionTypes.INITIALISE_FLUID_BALANCE_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.FluidBalanceDetailAction(
				modelAction.FluidBalanceDetailModelActionTypes.FETCH_FLUID_BALANCE_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.FluidBalanceDetailAction(
						modelAction.FluidBalanceDetailModelActionTypes.COUNT_FLUID_BALANCE_DETAILS,
						null
					)
				]
			))
		...
		this.fluidBalanceDetails = this.store.select(getFluidBalanceDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getFluidBalanceDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.fluidBalanceDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('fluid-balance-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: FluidBalanceDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.FluidBalanceDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.FluidBalanceDetailModelActionTypes.EXPORT_ALL_FLUID_BALANCE_DETAIL;
		} else {
			actionType = modelAction.FluidBalanceDetailModelActionTypes.EXPORT_FLUID_BALANCE_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.FluidBalanceDetailAction(
				modelAction.FluidBalanceDetailModelActionTypes.IMPORT_FLUID_BALANCE_DETAILS,
				{
					file: file,
				},
				[new modelAction.FluidBalanceDetailAction(
			...
			window.open(environment.API_URL + '/api/fluidbalancedetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: FluidBalanceDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Fluid Balance Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Fluid Balance Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.FluidBalanceDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.FluidBalanceDetailModelActionTypes.DELETE_FLUID_BALANCE_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.FluidBalanceDetailModelActionTypes.DELETE_ALL_FLUID_BALANCE_DETAIL;
						} else {
							actionType = modelAction.FluidBalanceDetailModelActionTypes.DELETE_FLUID_BALANCE_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.FluidBalanceDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_FLUID_BALANCE_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_FLUID_BALANCE_DETAIL and DELETE_FLUID_BALANCE_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<FluidBalanceDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = FluidBalanceDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.fluidBalanceDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
