...
import {AdminFluidBalanceDetailCrudListComponent} from './fluid-balance-detail-admin-crud-list.component';
...
import {AdminFluidBalanceDetailCrudModule} from '../fluid-balance-detail-admin-crud.module';
import {
 ...
 */
describe('Fluid Balance Detail Crud List Tile Component', () => {

	let fixture;
	let fluidBalanceDetailCrudListComponent: AdminFluidBalanceDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<FluidBalanceDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '9c6d5fc1-7cb6-4808-8ac9-8c11dea19210';
	const searchContent = 'text';

	const searchableFieldCount = FluidBalanceDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'fluid-balance-detail-admin-crud',
		urls: ['fluid-balance-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const fluidBalanceDetailDataFactory = new FluidBalanceDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = fluidBalanceDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getFluidBalanceDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getFluidBalanceDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminFluidBalanceDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminFluidBalanceDetailCrudListComponent);
			fluidBalanceDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(fluidBalanceDetailCrudListComponent.fluidBalanceDetailsCount).toBe(defaultData.length);
			...
			expect(fluidBalanceDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			fluidBalanceDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = fluidBalanceDetailDataFactory.createAll(filteredDataSize);
			...
			expect(fluidBalanceDetailCrudListComponent.fluidBalanceDetailsCount).toBe(filteredDataSize);
			expect(fluidBalanceDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(fluidBalanceDetailCrudListComponent.pageIndex).toBe(0);
			expect(fluidBalanceDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			fluidBalanceDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		fluidBalanceDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		fluidBalanceDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
