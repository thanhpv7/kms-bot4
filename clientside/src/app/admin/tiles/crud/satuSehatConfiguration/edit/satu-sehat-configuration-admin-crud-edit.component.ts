...
import {SatuSehatConfigurationModelState, SatuSehatConfigurationModelAudit} from 'src/app/models/satuSehatConfiguration/satu_sehat_configuration.model.state';
import {SatuSehatConfigurationModel} from 'src/app/models/satuSehatConfiguration/satu_sehat_configuration.model';
import {
	getSatuSehatConfigurationModelAuditsByEntityId,
...
@Component({
	selector: 'cb-satu-sehat-configuration-admin-crud-edit',
	templateUrl: './satu-sehat-configuration-admin-crud-edit.component.html',
	styleUrls: [
		'./satu-sehat-configuration-admin-crud-edit.component.scss',
...
export class AdminSatuSehatConfigurationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	satuSehatConfigurationAudits$: Observable<SatuSehatConfigurationModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = SatuSehatConfigurationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: SatuSehatConfigurationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountSatuSehatConfigurationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new SatuSehatConfigurationModel();
		...
		if (this.targetModelId) {
			this.satuSehatConfigurationAudits$ = this.store.select(getSatuSehatConfigurationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<SatuSehatConfigurationModel> = {
		...
		this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
			modelAction.SatuSehatConfigurationModelActionTypes.FETCH_SATU_SEHAT_CONFIGURATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getSatuSehatConfigurationModelWithId, this.targetModelId)
			...
			model = new SatuSehatConfigurationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
			modelAction.SatuSehatConfigurationModelActionTypes.FETCH_SATU_SEHAT_CONFIGURATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
				modelAction.SatuSehatConfigurationModelActionTypes.CREATE_SATU_SEHAT_CONFIGURATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
				modelAction.SatuSehatConfigurationModelActionTypes.UPDATE_SATU_SEHAT_CONFIGURATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: SatuSehatConfigurationModel = new SatuSehatConfigurationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getSatuSehatConfigurationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new SatuSehatConfigurationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = SatuSehatConfigurationModel.getProps();
		this.modelRelations = SatuSehatConfigurationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: SatuSehatConfigurationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): SatuSehatConfigurationModel {
		let updatedModel: SatuSehatConfigurationModel = new SatuSehatConfigurationModel(model);
