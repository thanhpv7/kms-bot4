...
import {AdminSatuSehatConfigurationCrudListComponent} from './satu-sehat-configuration-admin-crud-list.component';
...
import {AdminSatuSehatConfigurationCrudModule} from '../satu-sehat-configuration-admin-crud.module';
import {
 ...
 */
describe('Satu Sehat Configuration Crud List Tile Component', () => {

	let fixture;
	let satuSehatConfigurationCrudListComponent: AdminSatuSehatConfigurationCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<SatuSehatConfigurationModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'dc0d3d9c-f6bd-46ea-b051-2e6b685ce14d';
	const searchContent = 'text';

	const searchableFieldCount = SatuSehatConfigurationModel.searchFields.length;

	const routerState: RouterState = {
		url: 'satu-sehat-configuration-admin-crud',
		urls: ['satu-sehat-configuration-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const satuSehatConfigurationDataFactory = new SatuSehatConfigurationDataFactory();

	const defaultDataSize = 30;
	const defaultData = satuSehatConfigurationDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getSatuSehatConfigurationCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getSatuSehatConfigurationCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminSatuSehatConfigurationCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminSatuSehatConfigurationCrudListComponent);
			satuSehatConfigurationCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(satuSehatConfigurationCrudListComponent.satuSehatConfigurationsCount).toBe(defaultData.length);
			...
			expect(satuSehatConfigurationCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			satuSehatConfigurationCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = satuSehatConfigurationDataFactory.createAll(filteredDataSize);
			...
			expect(satuSehatConfigurationCrudListComponent.satuSehatConfigurationsCount).toBe(filteredDataSize);
			expect(satuSehatConfigurationCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(satuSehatConfigurationCrudListComponent.pageIndex).toBe(0);
			expect(satuSehatConfigurationCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			satuSehatConfigurationCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		satuSehatConfigurationCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		satuSehatConfigurationCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
