...
import * as modelAction from 'src/app/models/satuSehatConfiguration/satu_sehat_configuration.model.action';
import {SatuSehatConfigurationModelState} from 'src/app/models/satuSehatConfiguration/satu_sehat_configuration.model.state';
...
import {
	...
	getSatuSehatConfigurationCollectionCount
...
import {CrudTileMode} from '../satu-sehat-configuration-admin-crud.component';
...
@Component({
	selector: 'cb-satu-sehat-configuration-admin-crud-list',
	templateUrl: './satu-sehat-configuration-admin-crud-list.component.html',
	styleUrls: [
		'./satu-sehat-configuration-admin-crud-list.component.scss',
...
export class AdminSatuSehatConfigurationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<SatuSehatConfigurationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...SatuSehatConfigurationModel.searchFields,
	...
	/**
	 * List of Satu Sehat Configurations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	satuSehatConfigurations: Observable<SatuSehatConfigurationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	satuSehatConfigurationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = SatuSehatConfigurationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = SatuSehatConfigurationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'dc0d3d9c-f6bd-46ea-b051-2e6b685ce14d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.satuSehatConfigurationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.satuSehatConfigurationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: SatuSehatConfigurationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
				modelAction.SatuSehatConfigurationModelActionTypes.INITIALISE_SATU_SEHAT_CONFIGURATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
				modelAction.SatuSehatConfigurationModelActionTypes.FETCH_SATU_SEHAT_CONFIGURATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.SatuSehatConfigurationAction(
						modelAction.SatuSehatConfigurationModelActionTypes.COUNT_SATU_SEHAT_CONFIGURATIONS,
						null
					)
				]
			))
		...
		this.satuSehatConfigurations = this.store.select(getSatuSehatConfigurationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getSatuSehatConfigurationCollectionCount, this.collectionId).subscribe(
				(count) => this.satuSehatConfigurationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('satu-sehat-configuration');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: SatuSehatConfigurationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.SatuSehatConfigurationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.SatuSehatConfigurationModelActionTypes.EXPORT_ALL_SATU_SEHAT_CONFIGURATION;
		} else {
			actionType = modelAction.SatuSehatConfigurationModelActionTypes.EXPORT_SATU_SEHAT_CONFIGURATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
				modelAction.SatuSehatConfigurationModelActionTypes.IMPORT_SATU_SEHAT_CONFIGURATIONS,
				{
					file: file,
				},
				[new modelAction.SatuSehatConfigurationAction(
			...
			window.open(environment.API_URL + '/api/satusehatconfiguration/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: SatuSehatConfigurationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Satu Sehat Configuration entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Satu Sehat Configuration entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.SatuSehatConfigurationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.SatuSehatConfigurationModelActionTypes.DELETE_SATU_SEHAT_CONFIGURATION;
						} else if (usingSelected) {
							actionType = modelAction.SatuSehatConfigurationModelActionTypes.DELETE_ALL_SATU_SEHAT_CONFIGURATION;
						} else {
							actionType = modelAction.SatuSehatConfigurationModelActionTypes.DELETE_SATU_SEHAT_CONFIGURATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.SatuSehatConfigurationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SATU_SEHAT_CONFIGURATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SATU_SEHAT_CONFIGURATION and DELETE_SATU_SEHAT_CONFIGURATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<SatuSehatConfigurationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = SatuSehatConfigurationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.satuSehatConfigurationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
