...
import {AdminRoomTransferCrudListComponent} from './room-transfer-admin-crud-list.component';
...
import {AdminRoomTransferCrudModule} from '../room-transfer-admin-crud.module';
import {
	RoomTransferModelState,
	initialState as initialRoomTransferModelState
} from 'src/app/models/roomTransfer/room_transfer.model.state';
 ...
 */
describe('Room Transfer Crud List Tile Component', () => {

	let fixture;
	let roomTransferCrudListComponent: AdminRoomTransferCrudListComponent;

	let store: MockStore<{ model: RoomTransferModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let roomTransferModelState: RoomTransferModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<RoomTransferModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'a86a0a06-afa2-47f4-a541-b27c820f8430';
	const searchContent = 'text';

	const searchableFieldCount = RoomTransferModel.searchFields.length;

	const routerState: RouterState = {
		url: 'room-transfer-admin-crud',
		urls: ['room-transfer-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const roomTransferDataFactory = new RoomTransferDataFactory();

	const defaultDataSize = 30;
	const defaultData = roomTransferDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: RoomTransferModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		roomTransferModelState = initialRoomTransferModelState;
		store.setState({model: roomTransferModelState});
		...
		spyOn(store, 'select')
			.withArgs(getRoomTransferCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getRoomTransferCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminRoomTransferCrudListComponent);
			roomTransferCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(roomTransferCrudListComponent.roomTransfersCount).toBe(defaultData.length);
			...
			expect(roomTransferCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			roomTransferCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = roomTransferDataFactory.createAll(filteredDataSize);
			...
			expect(roomTransferCrudListComponent.roomTransfersCount).toBe(filteredDataSize);
			expect(roomTransferCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(roomTransferCrudListComponent.pageIndex).toBe(0);
			expect(roomTransferCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			roomTransferCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		roomTransferCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		roomTransferCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
