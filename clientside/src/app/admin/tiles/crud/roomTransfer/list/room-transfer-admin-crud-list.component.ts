...
import {RoomTransferModelState} from 'src/app/models/roomTransfer/room_transfer.model.state';
...
import {
	getRoomTransferCollectionModels,
	getRoomTransferCollectionCount
...
@Component({
	selector: 'cb-room-transfer-admin-crud-list',
	templateUrl: './room-transfer-admin-crud-list.component.html',
	styleUrls: [
		'./room-transfer-admin-crud-list.component.scss',
...
export class AdminRoomTransferCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<RoomTransferModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...RoomTransferModel.searchFields,
	...
	/**
	 * List of Room Transfers that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	roomTransfers: Observable<RoomTransferModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	roomTransfersCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.roomTransfersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.roomTransfersCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RoomTransferAction(
				modelAction.RoomTransferModelActionTypes.INITIALISE_ROOM_TRANSFER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RoomTransferAction(
				modelAction.RoomTransferModelActionTypes.FETCH_ROOM_TRANSFER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.roomTransfers = this.store.select(getRoomTransferCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getRoomTransferCollectionCount, this.collectionId).subscribe(
				(count) => this.roomTransfersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: RoomTransferModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.RoomTransferModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.RoomTransferModelActionTypes.EXPORT_ALL_ROOM_TRANSFER;
		} else {
			actionType = modelAction.RoomTransferModelActionTypes.EXPORT_ROOM_TRANSFER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.RoomTransferAction(
				modelAction.RoomTransferModelActionTypes.IMPORT_ROOM_TRANSFERS,
				{
					file: file,
				},
				[new modelAction.RoomTransferAction(
			...
			window.open(environment.API_URL + '/api/roomtransfer/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: RoomTransferModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Room Transfer entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Room Transfer entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.RoomTransferModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.RoomTransferModelActionTypes.DELETE_ROOM_TRANSFER;
						} else if (usingSelected) {
							actionType = modelAction.RoomTransferModelActionTypes.DELETE_ALL_ROOM_TRANSFER;
						} else {
							actionType = modelAction.RoomTransferModelActionTypes.DELETE_ROOM_TRANSFER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.RoomTransferAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ROOM_TRANSFER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ROOM_TRANSFER and DELETE_ROOM_TRANSFER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = RoomTransferModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.roomTransfersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
