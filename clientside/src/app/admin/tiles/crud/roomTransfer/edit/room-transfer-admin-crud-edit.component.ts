...
import * as modelAction from 'src/app/models/roomTransfer/room_transfer.model.action';
import {RoomTransferModelState, RoomTransferModelAudit} from 'src/app/models/roomTransfer/room_transfer.model.state';
import {RoomTransferModel} from 'src/app/models/roomTransfer/room_transfer.model';
import {
	getRoomTransferModelAuditsByEntityId,
	getRoomTransferModelWithId,
	getCountRoomTransferModels,
...
import {CrudTileMode} from '../room-transfer-admin-crud.component';
...
@Component({
	selector: 'cb-room-transfer-admin-crud-edit',
	templateUrl: './room-transfer-admin-crud-edit.component.html',
	styleUrls: [
		'./room-transfer-admin-crud-edit.component.scss',
...
export class AdminRoomTransferCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'a86a0a06-afa2-47f4-a541-b27c820f8430';
	 ...
	 */
	roomTransferAudits$: Observable<RoomTransferModelAudit[]>;
	...
	bedFacilityOppositeRelation = 'roomTransfers';
	...
	medicalExaminationRecordOppositeRelation = 'roomTransfers';
	...
	transferringStaffMultiCrudActive: boolean = false;
	transferringStaffDisplayName = 'Transferring Staff';
	...
	transferringStaffOppositeRelation = 'roomTransfers';
	transferringStaffRelations: ModelRelation[] = this.transferringStaffMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.transferringStaffOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = RoomTransferModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = RoomTransferModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = RoomTransferModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'transferringStaff' && this.transferringStaffMultiCrudActive) {
	...
	constructor(
		private readonly store: Store<{ model: RoomTransferModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountRoomTransferModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new RoomTransferModel();
		...
		if (this.targetModelId) {
			this.roomTransferAudits$ = this.store.select(getRoomTransferModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<RoomTransferModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.RoomTransferAction(
			modelAction.RoomTransferModelActionTypes.FETCH_ROOM_TRANSFER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getRoomTransferModelWithId, this.targetModelId)
			...
			model = new RoomTransferModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.RoomTransferAction(
			modelAction.RoomTransferModelActionTypes.FETCH_ROOM_TRANSFER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<RoomTransferModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.bedFacilityMultiCrudActive || this.medicalExaminationRecordMultiCrudActive || this.transferringStaffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.RoomTransferAction(
						modelAction.RoomTransferModelActionTypes.COUNT_ROOM_TRANSFERS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.RoomTransferAction(
				modelAction.RoomTransferModelActionTypes.CREATE_ROOM_TRANSFER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.RoomTransferAction(
				modelAction.RoomTransferModelActionTypes.UPDATE_ROOM_TRANSFER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: RoomTransferModel = new RoomTransferModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getRoomTransferModelWithId, this.targetModelId).subscribe(model => {
					let differences = new RoomTransferModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.transferringStaffDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.transferringStaffRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.transferringStaffDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.transferringStaffRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = RoomTransferModel.getProps();
		this.modelRelations = RoomTransferModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'transferringStaff';
		}).hideElement = this.transferringStaffMultiCrudActive || this.modelRelations.find(rel => rel.id === 'transferringStaff').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.transferringStaffMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.transferringStaffDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.transferringStaff && this.targetModel.transferringStaff.id && this.transferringStaffMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.transferringStaffOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.transferringStaff.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.transferringStaffId).subscribe(transferringStaff => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.transferringStaffRelations, this.isViewOnly),
					transferringStaff,
					StaffModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: RoomTransferModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): RoomTransferModel {
		let updatedModel: RoomTransferModel = new RoomTransferModel(model);
		...
		if (this.transferringStaffMultiCrudActive) {
			updatedModel.transferringStaff = event.additionalEntities.transferringStaff ? event.additionalEntities.transferringStaff : undefined;

			if (!updatedModel.transferringStaff && event.payload) {
				event.payload.transferringStaffId = null;
			}
		} else if (event.payload?.transferringStaffId !== undefined) {
			event.payload.transferringStaff = null;
			updatedModel.transferringStaff = null;
		}
