...
import {AdminMedicalExaminationRecordCrudListComponent} from './medical-examination-record-admin-crud-list.component';
...
import {AdminMedicalExaminationRecordCrudModule} from '../medical-examination-record-admin-crud.module';
import {
 ...
 */
describe('Medical Examination Record Crud List Tile Component', () => {

	let fixture;
	let medicalExaminationRecordCrudListComponent: AdminMedicalExaminationRecordCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalExaminationRecordModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cb2ae6bf-53ab-4c7f-a8d2-4dbef5efc932';
	const searchContent = 'text';

	const searchableFieldCount = MedicalExaminationRecordModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-examination-record-admin-crud',
		urls: ['medical-examination-record-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalExaminationRecordDataFactory = new MedicalExaminationRecordDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalExaminationRecordDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getMedicalExaminationRecordCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalExaminationRecordCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalExaminationRecordCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalExaminationRecordCrudListComponent);
			medicalExaminationRecordCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalExaminationRecordCrudListComponent.medicalExaminationRecordsCount).toBe(defaultData.length);
			...
			expect(medicalExaminationRecordCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalExaminationRecordCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalExaminationRecordDataFactory.createAll(filteredDataSize);
			...
			expect(medicalExaminationRecordCrudListComponent.medicalExaminationRecordsCount).toBe(filteredDataSize);
			expect(medicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalExaminationRecordCrudListComponent.pageIndex).toBe(0);
			expect(medicalExaminationRecordCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalExaminationRecordCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalExaminationRecordCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalExaminationRecordCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
