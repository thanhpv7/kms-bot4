...
import {MedicalExaminationRecordModelState} from 'src/app/models/medicalExaminationRecord/medical_examination_record.model.state';
...
import {
	...
	getMedicalExaminationRecordCollectionCount
...
import {CrudTileMode} from '../medical-examination-record-admin-crud.component';
...
@Component({
	selector: 'cb-medical-examination-record-admin-crud-list',
	templateUrl: './medical-examination-record-admin-crud-list.component.html',
	styleUrls: [
		'./medical-examination-record-admin-crud-list.component.scss',
...
export class AdminMedicalExaminationRecordCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalExaminationRecordModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalExaminationRecordModel.searchFields,
	...
	/**
	 * List of Medical Examination Records that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalExaminationRecords: Observable<MedicalExaminationRecordModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalExaminationRecordsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalExaminationRecordsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalExaminationRecordsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalExaminationRecordAction(
				modelAction.MedicalExaminationRecordModelActionTypes.INITIALISE_MEDICAL_EXAMINATION_RECORD_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalExaminationRecordAction(
				modelAction.MedicalExaminationRecordModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalExaminationRecords = this.store.select(getMedicalExaminationRecordCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalExaminationRecordCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalExaminationRecordsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalExaminationRecordModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalExaminationRecordModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalExaminationRecordModelActionTypes.EXPORT_ALL_MEDICAL_EXAMINATION_RECORD;
		} else {
			actionType = modelAction.MedicalExaminationRecordModelActionTypes.EXPORT_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalExaminationRecordAction(
				modelAction.MedicalExaminationRecordModelActionTypes.IMPORT_MEDICAL_EXAMINATION_RECORDS,
				{
					file: file,
				},
				[new modelAction.MedicalExaminationRecordAction(
			...
			window.open(environment.API_URL + '/api/medicalexaminationrecord/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalExaminationRecordModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Examination Record entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Examination Record entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalExaminationRecordModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalExaminationRecordModelActionTypes.DELETE_MEDICAL_EXAMINATION_RECORD;
						} else if (usingSelected) {
							actionType = modelAction.MedicalExaminationRecordModelActionTypes.DELETE_ALL_MEDICAL_EXAMINATION_RECORD;
						} else {
							actionType = modelAction.MedicalExaminationRecordModelActionTypes.DELETE_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalExaminationRecordAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_EXAMINATION_RECORD when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_EXAMINATION_RECORD and DELETE_MEDICAL_EXAMINATION_RECORD_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalExaminationRecordModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalExaminationRecordsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
