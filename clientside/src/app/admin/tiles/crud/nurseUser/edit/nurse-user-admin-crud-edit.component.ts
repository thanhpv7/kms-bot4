...
import {NurseUserModelState, NurseUserModelAudit} from 'src/app/models/nurseUser/nurse_user.model.state';
import {NurseUserModel} from 'src/app/models/nurseUser/nurse_user.model';
import {
	getNurseUserModelAuditsByEntityId,
	getNurseUserModelWithId,
	getCountNurseUserModels,
...
@Component({
	selector: 'cb-nurse-user-admin-crud-edit',
	templateUrl: './nurse-user-admin-crud-edit.component.html',
	styleUrls: [
		'./nurse-user-admin-crud-edit.component.scss',
...
export class AdminNurseUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nurseUserAudits$: Observable<NurseUserModelAudit[]>;
	...
	staffOppositeRelation = 'nurseUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NurseUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNurseUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NurseUserModel();
		...
		if (this.targetModelId) {
			this.nurseUserAudits$ = this.store.select(getNurseUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NurseUserModel> = {
		...
		this.store.dispatch(new modelAction.NurseUserAction(
			modelAction.NurseUserModelActionTypes.FETCH_NURSE_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNurseUserModelWithId, this.targetModelId)
			...
			model = new NurseUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NurseUserAction(
			modelAction.NurseUserModelActionTypes.FETCH_NURSE_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NurseUserAction(
				modelAction.NurseUserModelActionTypes.CREATE_NURSE_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NurseUserAction(
				modelAction.NurseUserModelActionTypes.UPDATE_NURSE_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NurseUserModel = new NurseUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNurseUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NurseUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NurseUserModel.getProps();
		this.modelRelations = NurseUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NurseUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NurseUserModel {
		let updatedModel: NurseUserModel = new NurseUserModel(model);
