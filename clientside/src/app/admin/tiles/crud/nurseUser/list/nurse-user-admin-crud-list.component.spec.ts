...
import {AdminNurseUserCrudListComponent} from './nurse-user-admin-crud-list.component';
...
import {AdminNurseUserCrudModule} from '../nurse-user-admin-crud.module';
import {
	NurseUserModelState,
	initialState as initialNurseUserModelState
} from 'src/app/models/nurseUser/nurse_user.model.state';
 ...
 */
describe('Nurse User Crud List Tile Component', () => {

	let fixture;
	let nurseUserCrudListComponent: AdminNurseUserCrudListComponent;

	let store: MockStore<{ model: NurseUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nurseUserModelState: NurseUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NurseUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'db2e8ef7-6aab-4a55-a5da-fdb4d6e913ec';
	const searchContent = 'text';

	const searchableFieldCount = NurseUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nurse-user-admin-crud',
		urls: ['nurse-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nurseUserDataFactory = new NurseUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = nurseUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NurseUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nurseUserModelState = initialNurseUserModelState;
		store.setState({model: nurseUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNurseUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNurseUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminNurseUserCrudListComponent);
			nurseUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nurseUserCrudListComponent.nurseUsersCount).toBe(defaultData.length);
			...
			expect(nurseUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nurseUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nurseUserDataFactory.createAll(filteredDataSize);
			...
			expect(nurseUserCrudListComponent.nurseUsersCount).toBe(filteredDataSize);
			expect(nurseUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nurseUserCrudListComponent.pageIndex).toBe(0);
			expect(nurseUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nurseUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nurseUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nurseUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
