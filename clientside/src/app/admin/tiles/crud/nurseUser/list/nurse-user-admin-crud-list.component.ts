...
import * as modelAction from 'src/app/models/nurseUser/nurse_user.model.action';
import {NurseUserModelState} from 'src/app/models/nurseUser/nurse_user.model.state';
...
import {
	getNurseUserCollectionModels,
	getNurseUserCollectionCount
...
import {CrudTileMode} from '../nurse-user-admin-crud.component';
...
@Component({
	selector: 'cb-nurse-user-admin-crud-list',
	templateUrl: './nurse-user-admin-crud-list.component.html',
	styleUrls: [
		'./nurse-user-admin-crud-list.component.scss',
...
export class AdminNurseUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NurseUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NurseUserModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = NurseUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'db2e8ef7-6aab-4a55-a5da-fdb4d6e913ec';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nurseUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nurseUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NurseUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NurseUserAction(
				modelAction.NurseUserModelActionTypes.INITIALISE_NURSE_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NurseUserAction(
				modelAction.NurseUserModelActionTypes.FETCH_NURSE_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NurseUserAction(
						modelAction.NurseUserModelActionTypes.COUNT_NURSE_USERS,
						null
					)
				]
			))
		...
		this.nurseUsers = this.store.select(getNurseUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNurseUserCollectionCount, this.collectionId).subscribe(
				(count) => this.nurseUsersCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NurseUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NurseUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NurseUserModelActionTypes.EXPORT_ALL_NURSE_USER;
		} else {
			actionType = modelAction.NurseUserModelActionTypes.EXPORT_NURSE_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NurseUserAction(
				modelAction.NurseUserModelActionTypes.IMPORT_NURSE_USERS,
				{
					file: file,
				},
				[new modelAction.NurseUserAction(
			...
			window.open(environment.API_URL + '/api/nurseuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NurseUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Nurse User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Nurse User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NurseUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NurseUserModelActionTypes.DELETE_NURSE_USER;
						} else if (usingSelected) {
							actionType = modelAction.NurseUserModelActionTypes.DELETE_ALL_NURSE_USER;
						} else {
							actionType = modelAction.NurseUserModelActionTypes.DELETE_NURSE_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NurseUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NURSE_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NURSE_USER and DELETE_NURSE_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NurseUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NurseUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nurseUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
