...
import * as modelAction from 'src/app/models/patientPaymentInsurance/patient_payment_insurance.model.action';
import {PatientPaymentInsuranceModelState, PatientPaymentInsuranceModelAudit} from 'src/app/models/patientPaymentInsurance/patient_payment_insurance.model.state';
import {PatientPaymentInsuranceModel} from 'src/app/models/patientPaymentInsurance/patient_payment_insurance.model';
import {
	getPatientPaymentInsuranceModelAuditsByEntityId,
	getPatientPaymentInsuranceModelWithId,
	getCountPatientPaymentInsuranceModels,
...
import * as insuranceBenefitPlanModelAction from 'src/app/models/insuranceBenefitPlan/insurance_benefit_plan.model.action';
import {
	getInsuranceBenefitPlanCollectionModels, getInsuranceBenefitPlanModelWithId,
...
import {CrudTileMode} from '../patient-payment-insurance-admin-crud.component';
...
@Component({
	selector: 'cb-patient-payment-insurance-admin-crud-edit',
	templateUrl: './patient-payment-insurance-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-payment-insurance-admin-crud-edit.component.scss',
...
export class AdminPatientPaymentInsuranceCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '598ef058-5fe7-42a5-9227-62ed7d972db8';
	 ...
	 */
	patientPaymentInsuranceAudits$: Observable<PatientPaymentInsuranceModelAudit[]>;
	...
	patientGeneralInformationMultiCrudActive: boolean = false;
	patientGeneralInformationDisplayName = 'Patient General Information';
	...
	patientGeneralInformationOppositeRelation = 'patientPaymentTypeInsurance';
	patientGeneralInformationRelations: ModelRelation[] = this.patientGeneralInformationMultiCrudActive ? PatientGeneralInfoModel.getRelations()
		.map(relation => {
			if (relation.id === this.patientGeneralInformationOppositeRelation) {
	...
	insuranceBenefitPlanMultiCrudActive: boolean = false;
	insuranceBenefitPlanDisplayName = 'Insurance Benefit Plan';
	...
	insuranceBenefitPlanOppositeRelation = 'patientPaymentInsurances';
	insuranceBenefitPlanRelations: ModelRelation[] = this.insuranceBenefitPlanMultiCrudActive ? InsuranceBenefitPlanModel.getRelations()
		.map(relation => {
			if (relation.id === this.insuranceBenefitPlanOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = PatientPaymentInsuranceModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientPaymentInsuranceModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientPaymentInsuranceModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'patientGeneralInformation' && this.patientGeneralInformationMultiCrudActive) {
					exp.fields = PatientGeneralInfoModel.getProps().map(prop => {
						return prop.name;
					});
				}

				if (relation.id === 'insuranceBenefitPlan' && this.insuranceBenefitPlanMultiCrudActive) {
					exp.fields = InsuranceBenefitPlanModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: PatientPaymentInsuranceModel;
	...
	constructor(
		private readonly store: Store<{ model: PatientPaymentInsuranceModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientPaymentInsuranceModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientPaymentInsuranceModel();
		...
		if (this.targetModelId) {
			this.patientPaymentInsuranceAudits$ = this.store.select(getPatientPaymentInsuranceModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientPaymentInsuranceModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
			modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_PAYMENT_INSURANCE,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientPaymentInsuranceModelWithId, this.targetModelId)
			...
			model = new PatientPaymentInsuranceModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-payment-insurance');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
			modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_PAYMENT_INSURANCE_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientPaymentInsuranceModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.patientGeneralInformationMultiCrudActive || this.insuranceBenefitPlanMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PatientPaymentInsuranceAction(
						modelAction.PatientPaymentInsuranceModelActionTypes.COUNT_PATIENT_PAYMENT_INSURANCES,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
				modelAction.PatientPaymentInsuranceModelActionTypes.CREATE_PATIENT_PAYMENT_INSURANCE,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
				modelAction.PatientPaymentInsuranceModelActionTypes.UPDATE_PATIENT_PAYMENT_INSURANCE,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientPaymentInsuranceModel = new PatientPaymentInsuranceModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientPaymentInsuranceModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientPaymentInsuranceModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.patientGeneralInformationDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PatientGeneralInfoModel.getProps(), this.patientGeneralInformationRelations, this.isViewOnly),
					new PatientGeneralInfoModel(),
					PatientGeneralInfoModel,
					this.patientGeneralInformationDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.patientGeneralInformationRelations,
					true
				);
			...
			if (event.entityName === this.insuranceBenefitPlanDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InsuranceBenefitPlanModel.getProps(), this.insuranceBenefitPlanRelations, this.isViewOnly),
					new InsuranceBenefitPlanModel(),
					InsuranceBenefitPlanModel,
					this.insuranceBenefitPlanDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.insuranceBenefitPlanRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientPaymentInsuranceModel.getProps();
		this.modelRelations = PatientPaymentInsuranceModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'patientGeneralInformation';
		}).hideElement = this.patientGeneralInformationMultiCrudActive || this.modelRelations.find(rel => rel.id === 'patientGeneralInformation').hideElement;

		this.modelRelations.find(rel => {
			return rel.id === 'insuranceBenefitPlan';
		}).hideElement = this.insuranceBenefitPlanMultiCrudActive || this.modelRelations.find(rel => rel.id === 'insuranceBenefitPlan').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.patientGeneralInformationMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.patientGeneralInformationDisplayName, many: false});
		}

		if (this.insuranceBenefitPlanMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.insuranceBenefitPlanDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.patientGeneralInformation && this.targetModel.patientGeneralInformation.id && this.patientGeneralInformationMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PatientGeneralInfoModel,
				this.patientGeneralInformationOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.patientGeneralInformation.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPatientGeneralInfoModelWithId, this.targetModel.patientGeneralInformationId).subscribe(patientGeneralInformation => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PatientGeneralInfoModel.getProps(), this.patientGeneralInformationRelations, this.isViewOnly),
		...
		if (this.targetModel.insuranceBenefitPlan && this.targetModel.insuranceBenefitPlan.id && this.insuranceBenefitPlanMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InsuranceBenefitPlanModel,
				this.insuranceBenefitPlanOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.insuranceBenefitPlan.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getInsuranceBenefitPlanModelWithId, this.targetModel.insuranceBenefitPlanId).subscribe(insuranceBenefitPlan => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InsuranceBenefitPlanModel.getProps(), this.insuranceBenefitPlanRelations, this.isViewOnly),
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientPaymentInsuranceModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientPaymentInsuranceModel {
		let updatedModel: PatientPaymentInsuranceModel = new PatientPaymentInsuranceModel(model);

		if (this.patientGeneralInformationMultiCrudActive) {
			updatedModel.patientGeneralInformation = event.additionalEntities.patientGeneralInformation ? event.additionalEntities.patientGeneralInformation : undefined;

			if (!updatedModel.patientGeneralInformation && event.payload) {
				event.payload.patientGeneralInformationId = null;
			}
		} else if (event.payload?.patientGeneralInformationId !== undefined) {
			event.payload.patientGeneralInformation = null;
			updatedModel.patientGeneralInformation = null;
		}

		if (this.insuranceBenefitPlanMultiCrudActive) {
			updatedModel.insuranceBenefitPlan = event.additionalEntities.insuranceBenefitPlan ? event.additionalEntities.insuranceBenefitPlan : undefined;

			if (!updatedModel.insuranceBenefitPlan && event.payload) {
				event.payload.insuranceBenefitPlanId = null;
			}
		} else if (event.payload?.insuranceBenefitPlanId !== undefined) {
			event.payload.insuranceBenefitPlan = null;
			updatedModel.insuranceBenefitPlan = null;
		}
