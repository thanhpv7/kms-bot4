...
import {PatientPaymentInsuranceModelState} from 'src/app/models/patientPaymentInsurance/patient_payment_insurance.model.state';
...
import {
	getPatientPaymentInsuranceCollectionModels,
	getPatientPaymentInsuranceCollectionCount
...
@Component({
	selector: 'cb-patient-payment-insurance-admin-crud-list',
	templateUrl: './patient-payment-insurance-admin-crud-list.component.html',
	styleUrls: [
		'./patient-payment-insurance-admin-crud-list.component.scss',
...
export class AdminPatientPaymentInsuranceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientPaymentInsuranceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientPaymentInsuranceModel.searchFields,
	...
	/**
	 * List of Patient Payment Insurances that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientPaymentInsurances: Observable<PatientPaymentInsuranceModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientPaymentInsurancesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientPaymentInsurancesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientPaymentInsurancesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
				modelAction.PatientPaymentInsuranceModelActionTypes.INITIALISE_PATIENT_PAYMENT_INSURANCE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
				modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_PAYMENT_INSURANCE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.patientPaymentInsurances = this.store.select(getPatientPaymentInsuranceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientPaymentInsuranceCollectionCount, this.collectionId).subscribe(
				(count) => this.patientPaymentInsurancesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientPaymentInsuranceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientPaymentInsuranceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientPaymentInsuranceModelActionTypes.EXPORT_ALL_PATIENT_PAYMENT_INSURANCE;
		} else {
			actionType = modelAction.PatientPaymentInsuranceModelActionTypes.EXPORT_PATIENT_PAYMENT_INSURANCE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
				modelAction.PatientPaymentInsuranceModelActionTypes.IMPORT_PATIENT_PAYMENT_INSURANCES,
				{
					file: file,
				},
				[new modelAction.PatientPaymentInsuranceAction(
			...
			window.open(environment.API_URL + '/api/patientpaymentinsurance/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientPaymentInsuranceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment Insurance entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Payment Insurance entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientPaymentInsuranceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_PAYMENT_INSURANCE;
						} else if (usingSelected) {
							actionType = modelAction.PatientPaymentInsuranceModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_INSURANCE;
						} else {
							actionType = modelAction.PatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_PAYMENT_INSURANCE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientPaymentInsuranceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_PAYMENT_INSURANCE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_PAYMENT_INSURANCE and DELETE_PATIENT_PAYMENT_INSURANCE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientPaymentInsuranceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientPaymentInsurancesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
