...
import {AdminPatientPaymentInsuranceCrudListComponent} from './patient-payment-insurance-admin-crud-list.component';
...
import {AdminPatientPaymentInsuranceCrudModule} from '../patient-payment-insurance-admin-crud.module';
import {
	PatientPaymentInsuranceModelState,
	initialState as initialPatientPaymentInsuranceModelState
} from 'src/app/models/patientPaymentInsurance/patient_payment_insurance.model.state';
 ...
 */
describe('Patient Payment Insurance Crud List Tile Component', () => {

	let fixture;
	let patientPaymentInsuranceCrudListComponent: AdminPatientPaymentInsuranceCrudListComponent;

	let store: MockStore<{ model: PatientPaymentInsuranceModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientPaymentInsuranceModelState: PatientPaymentInsuranceModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientPaymentInsuranceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '598ef058-5fe7-42a5-9227-62ed7d972db8';
	const searchContent = 'text';

	const searchableFieldCount = PatientPaymentInsuranceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-payment-insurance-admin-crud',
		urls: ['patient-payment-insurance-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientPaymentInsuranceDataFactory = new PatientPaymentInsuranceDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientPaymentInsuranceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientPaymentInsuranceModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientPaymentInsuranceModelState = initialPatientPaymentInsuranceModelState;
		store.setState({model: patientPaymentInsuranceModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientPaymentInsuranceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientPaymentInsuranceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientPaymentInsuranceCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientPaymentInsuranceCrudListComponent);
			patientPaymentInsuranceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientPaymentInsuranceCrudListComponent.patientPaymentInsurancesCount).toBe(defaultData.length);
			...
			expect(patientPaymentInsuranceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientPaymentInsuranceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientPaymentInsuranceDataFactory.createAll(filteredDataSize);
			...
			expect(patientPaymentInsuranceCrudListComponent.patientPaymentInsurancesCount).toBe(filteredDataSize);
			expect(patientPaymentInsuranceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientPaymentInsuranceCrudListComponent.pageIndex).toBe(0);
			expect(patientPaymentInsuranceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientPaymentInsuranceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientPaymentInsuranceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientPaymentInsuranceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
