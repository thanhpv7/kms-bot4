...
import * as modelAction from 'src/app/models/purchaseRequisitionStockDetail/purchase_requisition_stock_detail.model.action';
import {PurchaseRequisitionStockDetailModelState, PurchaseRequisitionStockDetailModelAudit} from 'src/app/models/purchaseRequisitionStockDetail/purchase_requisition_stock_detail.model.state';
...
import {
	getPurchaseRequisitionStockDetailModelAuditsByEntityId,
...
import {CrudTileMode} from '../purchase-requisition-stock-detail-admin-crud.component';
...
@Component({
	selector: 'cb-purchase-requisition-stock-detail-admin-crud-edit',
	templateUrl: './purchase-requisition-stock-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./purchase-requisition-stock-detail-admin-crud-edit.component.scss',
...
export class AdminPurchaseRequisitionStockDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'aab331e5-f0c2-4d5c-8312-709b5c6ec1d2';
	 ...
	 */
	purchaseRequisitionStockDetailAudits$: Observable<PurchaseRequisitionStockDetailModelAudit[]>;
	...
	purchaseRequisitionMultiCrudActive: boolean = false;
	purchaseRequisitionDisplayName = 'Purchase Requisition';
	...
	purchaseRequisitionOppositeRelation = 'purchaseRequisitionStockDetails';
	purchaseRequisitionRelations: ModelRelation[] = this.purchaseRequisitionMultiCrudActive ? PurchaseRequisitionModel.getRelations()
		.map(relation => {
			if (relation.id === this.purchaseRequisitionOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'purchaseRequisitionStockDetails';
	...
	purchaseOrderStockDetailsOppositeRelation = 'purchaseRequisitionStockDetail';
	 ...
	 */
	modelProperties: ModelProperty[] = PurchaseRequisitionStockDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PurchaseRequisitionStockDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PurchaseRequisitionStockDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'purchaseRequisition' && this.purchaseRequisitionMultiCrudActive) {
	 ...
	 */
	targetModel: PurchaseRequisitionStockDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: PurchaseRequisitionStockDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPurchaseRequisitionStockDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PurchaseRequisitionStockDetailModel();
		...
		if (this.targetModelId) {
			this.purchaseRequisitionStockDetailAudits$ = this.store.select(getPurchaseRequisitionStockDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PurchaseRequisitionStockDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
			modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_PURCHASE_REQUISITION_STOCK_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPurchaseRequisitionStockDetailModelWithId, this.targetModelId)
			...
			model = new PurchaseRequisitionStockDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('purchase-requisition-stock-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
			modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_PURCHASE_REQUISITION_STOCK_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PurchaseRequisitionStockDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.purchaseRequisitionMultiCrudActive || this.stockCatalogueMultiCrudActive || this.purchaseOrderStockDetailsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.PurchaseRequisitionStockDetailAction(
						modelAction.PurchaseRequisitionStockDetailModelActionTypes.COUNT_PURCHASE_REQUISITION_STOCK_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
				modelAction.PurchaseRequisitionStockDetailModelActionTypes.CREATE_PURCHASE_REQUISITION_STOCK_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
				modelAction.PurchaseRequisitionStockDetailModelActionTypes.UPDATE_PURCHASE_REQUISITION_STOCK_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PurchaseRequisitionStockDetailModel = new PurchaseRequisitionStockDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPurchaseRequisitionStockDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PurchaseRequisitionStockDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.purchaseRequisitionDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PurchaseRequisitionModel.getProps(), this.purchaseRequisitionRelations, this.isViewOnly),
					new PurchaseRequisitionModel(),
					PurchaseRequisitionModel,
					this.purchaseRequisitionDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.purchaseRequisitionRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PurchaseRequisitionStockDetailModel.getProps();
		this.modelRelations = PurchaseRequisitionStockDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'purchaseRequisition';
		}).hideElement = this.purchaseRequisitionMultiCrudActive || this.modelRelations.find(rel => rel.id === 'purchaseRequisition').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.purchaseRequisitionMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.purchaseRequisitionDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.purchaseRequisition && this.targetModel.purchaseRequisition.id && this.purchaseRequisitionMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				PurchaseRequisitionModel,
				this.purchaseRequisitionOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.purchaseRequisition.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getPurchaseRequisitionModelWithId, this.targetModel.purchaseRequisitionId).subscribe(purchaseRequisition => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(PurchaseRequisitionModel.getProps(), this.purchaseRequisitionRelations, this.isViewOnly),
					purchaseRequisition,
					PurchaseRequisitionModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: PurchaseRequisitionStockDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PurchaseRequisitionStockDetailModel {
		let updatedModel: PurchaseRequisitionStockDetailModel = new PurchaseRequisitionStockDetailModel(model);

		if (this.purchaseRequisitionMultiCrudActive) {
			updatedModel.purchaseRequisition = event.additionalEntities.purchaseRequisition ? event.additionalEntities.purchaseRequisition : undefined;

			if (!updatedModel.purchaseRequisition && event.payload) {
				event.payload.purchaseRequisitionId = null;
			}
		} else if (event.payload?.purchaseRequisitionId !== undefined) {
			event.payload.purchaseRequisition = null;
			updatedModel.purchaseRequisition = null;
		}
