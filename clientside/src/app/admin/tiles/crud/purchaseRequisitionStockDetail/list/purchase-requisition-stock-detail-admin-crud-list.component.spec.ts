...
import {AdminPurchaseRequisitionStockDetailCrudListComponent} from './purchase-requisition-stock-detail-admin-crud-list.component';
...
import {AdminPurchaseRequisitionStockDetailCrudModule} from '../purchase-requisition-stock-detail-admin-crud.module';
import {
 ...
 */
describe('Purchase Requisition Stock Detail Crud List Tile Component', () => {

	let fixture;
	let purchaseRequisitionStockDetailCrudListComponent: AdminPurchaseRequisitionStockDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PurchaseRequisitionStockDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'aab331e5-f0c2-4d5c-8312-709b5c6ec1d2';
	const searchContent = 'text';

	const searchableFieldCount = PurchaseRequisitionStockDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'purchase-requisition-stock-detail-admin-crud',
		urls: ['purchase-requisition-stock-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const purchaseRequisitionStockDetailDataFactory = new PurchaseRequisitionStockDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = purchaseRequisitionStockDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPurchaseRequisitionStockDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPurchaseRequisitionStockDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPurchaseRequisitionStockDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPurchaseRequisitionStockDetailCrudListComponent);
			purchaseRequisitionStockDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(purchaseRequisitionStockDetailCrudListComponent.purchaseRequisitionStockDetailsCount).toBe(defaultData.length);
			...
			expect(purchaseRequisitionStockDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			purchaseRequisitionStockDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = purchaseRequisitionStockDetailDataFactory.createAll(filteredDataSize);
			...
			expect(purchaseRequisitionStockDetailCrudListComponent.purchaseRequisitionStockDetailsCount).toBe(filteredDataSize);
			expect(purchaseRequisitionStockDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(purchaseRequisitionStockDetailCrudListComponent.pageIndex).toBe(0);
			expect(purchaseRequisitionStockDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			purchaseRequisitionStockDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		purchaseRequisitionStockDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		purchaseRequisitionStockDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
