...
import {PurchaseRequisitionStockDetailModelState} from 'src/app/models/purchaseRequisitionStockDetail/purchase_requisition_stock_detail.model.state';
...
import {
	...
	getPurchaseRequisitionStockDetailCollectionCount
...
@Component({
	selector: 'cb-purchase-requisition-stock-detail-admin-crud-list',
	templateUrl: './purchase-requisition-stock-detail-admin-crud-list.component.html',
	styleUrls: [
		'./purchase-requisition-stock-detail-admin-crud-list.component.scss',
...
export class AdminPurchaseRequisitionStockDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PurchaseRequisitionStockDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PurchaseRequisitionStockDetailModel.searchFields,
	...
	/**
	 * List of Purchase Requisition Stock Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	purchaseRequisitionStockDetails: Observable<PurchaseRequisitionStockDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	purchaseRequisitionStockDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.purchaseRequisitionStockDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.purchaseRequisitionStockDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
				modelAction.PurchaseRequisitionStockDetailModelActionTypes.INITIALISE_PURCHASE_REQUISITION_STOCK_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
				modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_PURCHASE_REQUISITION_STOCK_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.purchaseRequisitionStockDetails = this.store.select(getPurchaseRequisitionStockDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPurchaseRequisitionStockDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.purchaseRequisitionStockDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PurchaseRequisitionStockDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PurchaseRequisitionStockDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PurchaseRequisitionStockDetailModelActionTypes.EXPORT_ALL_PURCHASE_REQUISITION_STOCK_DETAIL;
		} else {
			actionType = modelAction.PurchaseRequisitionStockDetailModelActionTypes.EXPORT_PURCHASE_REQUISITION_STOCK_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
				modelAction.PurchaseRequisitionStockDetailModelActionTypes.IMPORT_PURCHASE_REQUISITION_STOCK_DETAILS,
				{
					file: file,
				},
				[new modelAction.PurchaseRequisitionStockDetailAction(
			...
			window.open(environment.API_URL + '/api/purchaserequisitionstockdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PurchaseRequisitionStockDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Purchase Requisition Stock Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Purchase Requisition Stock Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PurchaseRequisitionStockDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PurchaseRequisitionStockDetailModelActionTypes.DELETE_PURCHASE_REQUISITION_STOCK_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.PurchaseRequisitionStockDetailModelActionTypes.DELETE_ALL_PURCHASE_REQUISITION_STOCK_DETAIL;
						} else {
							actionType = modelAction.PurchaseRequisitionStockDetailModelActionTypes.DELETE_PURCHASE_REQUISITION_STOCK_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PurchaseRequisitionStockDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PURCHASE_REQUISITION_STOCK_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PURCHASE_REQUISITION_STOCK_DETAIL and DELETE_PURCHASE_REQUISITION_STOCK_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PurchaseRequisitionStockDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.purchaseRequisitionStockDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
