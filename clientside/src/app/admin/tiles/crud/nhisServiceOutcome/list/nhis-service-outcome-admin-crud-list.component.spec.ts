...
import {AdminNhisServiceOutcomeCrudListComponent} from './nhis-service-outcome-admin-crud-list.component';
import {NhisServiceOutcomeDataFactory} from 'src/app/lib/utils/factories/nhis-service-outcome-data-factory';
import {NhisServiceOutcomeModel} from 'src/app/models/nhisServiceOutcome/nhis_service_outcome.model';
import {AdminNhisServiceOutcomeCrudModule} from '../nhis-service-outcome-admin-crud.module';
import {
	NhisServiceOutcomeModelState,
	initialState as initialNhisServiceOutcomeModelState
} from 'src/app/models/nhisServiceOutcome/nhis_service_outcome.model.state';
 ...
 */
describe('NHIS Service Outcome Crud List Tile Component', () => {

	let fixture;
	let nhisServiceOutcomeCrudListComponent: AdminNhisServiceOutcomeCrudListComponent;

	let store: MockStore<{ model: NhisServiceOutcomeModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisServiceOutcomeModelState: NhisServiceOutcomeModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisServiceOutcomeModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '4b428b36-0c4e-423b-a941-221e413d81e6';
	const searchContent = 'text';

	const searchableFieldCount = NhisServiceOutcomeModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-service-outcome-admin-crud',
		urls: ['nhis-service-outcome-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisServiceOutcomeDataFactory = new NhisServiceOutcomeDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisServiceOutcomeDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisServiceOutcomeModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisServiceOutcomeModelState = initialNhisServiceOutcomeModelState;
		store.setState({model: nhisServiceOutcomeModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisServiceOutcomeCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisServiceOutcomeCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminNhisServiceOutcomeCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminNhisServiceOutcomeCrudListComponent);
			nhisServiceOutcomeCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisServiceOutcomeCrudListComponent.nhisServiceOutcomesCount).toBe(defaultData.length);
			...
			expect(nhisServiceOutcomeCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisServiceOutcomeCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisServiceOutcomeDataFactory.createAll(filteredDataSize);
			...
			expect(nhisServiceOutcomeCrudListComponent.nhisServiceOutcomesCount).toBe(filteredDataSize);
			expect(nhisServiceOutcomeCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisServiceOutcomeCrudListComponent.pageIndex).toBe(0);
			expect(nhisServiceOutcomeCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisServiceOutcomeCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisServiceOutcomeCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisServiceOutcomeCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
