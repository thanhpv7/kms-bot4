...
import * as modelAction from 'src/app/models/nhisServiceOutcome/nhis_service_outcome.model.action';
import {NhisServiceOutcomeModelState} from 'src/app/models/nhisServiceOutcome/nhis_service_outcome.model.state';
...
import {
	getNhisServiceOutcomeCollectionModels,
	getNhisServiceOutcomeCollectionCount
} from 'src/app/models/nhisServiceOutcome/nhis_service_outcome.model.selector';
...
import {CrudTileMode} from '../nhis-service-outcome-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-service-outcome-admin-crud-list',
	templateUrl: './nhis-service-outcome-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-service-outcome-admin-crud-list.component.scss',
...
export class AdminNhisServiceOutcomeCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisServiceOutcomeModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisServiceOutcomeModel.searchFields,
	...
	/**
	 * List of NHIS Service Outcomes that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	nhisServiceOutcomes: Observable<NhisServiceOutcomeModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	nhisServiceOutcomesCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = NhisServiceOutcomeModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NhisServiceOutcomeModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '4b428b36-0c4e-423b-a941-221e413d81e6';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisServiceOutcomesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisServiceOutcomesCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisServiceOutcomeModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
				modelAction.NhisServiceOutcomeModelActionTypes.INITIALISE_NHIS_SERVICE_OUTCOME_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
				modelAction.NhisServiceOutcomeModelActionTypes.FETCH_NHIS_SERVICE_OUTCOME_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisServiceOutcomeAction(
						modelAction.NhisServiceOutcomeModelActionTypes.COUNT_NHIS_SERVICE_OUTCOMES,
						null
					)
				]
			))
		...
		this.nhisServiceOutcomes = this.store.select(getNhisServiceOutcomeCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisServiceOutcomeCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisServiceOutcomesCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('nhis-service-outcome');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisServiceOutcomeModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisServiceOutcomeModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisServiceOutcomeModelActionTypes.EXPORT_ALL_NHIS_SERVICE_OUTCOME;
		} else {
			actionType = modelAction.NhisServiceOutcomeModelActionTypes.EXPORT_NHIS_SERVICE_OUTCOME_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
				modelAction.NhisServiceOutcomeModelActionTypes.IMPORT_NHIS_SERVICE_OUTCOMES,
				{
					file: file,
				},
				[new modelAction.NhisServiceOutcomeAction(
			...
			window.open(environment.API_URL + '/api/nhisserviceoutcome/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisServiceOutcomeModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Service Outcome entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Service Outcome entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisServiceOutcomeModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisServiceOutcomeModelActionTypes.DELETE_NHIS_SERVICE_OUTCOME;
						} else if (usingSelected) {
							actionType = modelAction.NhisServiceOutcomeModelActionTypes.DELETE_ALL_NHIS_SERVICE_OUTCOME;
						} else {
							actionType = modelAction.NhisServiceOutcomeModelActionTypes.DELETE_NHIS_SERVICE_OUTCOME_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_SERVICE_OUTCOME when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_SERVICE_OUTCOME and DELETE_NHIS_SERVICE_OUTCOME_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisServiceOutcomeModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisServiceOutcomeModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisServiceOutcomesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
