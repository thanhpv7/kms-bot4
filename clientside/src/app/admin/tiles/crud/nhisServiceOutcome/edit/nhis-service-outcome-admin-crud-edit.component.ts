...
import {NhisServiceOutcomeModelState, NhisServiceOutcomeModelAudit} from 'src/app/models/nhisServiceOutcome/nhis_service_outcome.model.state';
import {NhisServiceOutcomeModel} from 'src/app/models/nhisServiceOutcome/nhis_service_outcome.model';
import {
	getNhisServiceOutcomeModelAuditsByEntityId,
	getNhisServiceOutcomeModelWithId,
	getCountNhisServiceOutcomeModels,
...
@Component({
	selector: 'cb-nhis-service-outcome-admin-crud-edit',
	templateUrl: './nhis-service-outcome-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-service-outcome-admin-crud-edit.component.scss',
...
export class AdminNhisServiceOutcomeCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisServiceOutcomeAudits$: Observable<NhisServiceOutcomeModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisServiceOutcomeModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NhisServiceOutcomeModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisServiceOutcomeModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisServiceOutcomeModel();
		...
		if (this.targetModelId) {
			this.nhisServiceOutcomeAudits$ = this.store.select(getNhisServiceOutcomeModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisServiceOutcomeModel> = {
		...
		this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
			modelAction.NhisServiceOutcomeModelActionTypes.FETCH_NHIS_SERVICE_OUTCOME,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisServiceOutcomeModelWithId, this.targetModelId)
			...
			model = new NhisServiceOutcomeModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
			modelAction.NhisServiceOutcomeModelActionTypes.FETCH_NHIS_SERVICE_OUTCOME_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
				modelAction.NhisServiceOutcomeModelActionTypes.CREATE_NHIS_SERVICE_OUTCOME,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisServiceOutcomeAction(
				modelAction.NhisServiceOutcomeModelActionTypes.UPDATE_NHIS_SERVICE_OUTCOME,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisServiceOutcomeModel = new NhisServiceOutcomeModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisServiceOutcomeModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisServiceOutcomeModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisServiceOutcomeModel.getProps();
		this.modelRelations = NhisServiceOutcomeModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisServiceOutcomeModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisServiceOutcomeModel {
		let updatedModel: NhisServiceOutcomeModel = new NhisServiceOutcomeModel(model);
