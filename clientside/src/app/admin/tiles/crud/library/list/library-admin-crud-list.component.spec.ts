...
import {AdminLibraryCrudListComponent} from './library-admin-crud-list.component';
import {LibraryDataFactory} from 'src/app/lib/utils/factories/library-data-factory';
import {LibraryModel} from 'src/app/models/library/library.model';
import {AdminLibraryCrudModule} from '../library-admin-crud.module';
import {
	LibraryModelState,
	initialState as initialLibraryModelState
} from 'src/app/models/library/library.model.state';
 ...
 */
describe('Library Crud List Tile Component', () => {

	let fixture;
	let libraryCrudListComponent: AdminLibraryCrudListComponent;

	let store: MockStore<{ model: LibraryModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let libraryModelState: LibraryModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<LibraryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'cdb4aebd-7893-4e9b-bc5a-a45d4fc158b2';
	const searchContent = 'text';

	const searchableFieldCount = LibraryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'library-admin-crud',
		urls: ['library-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const libraryDataFactory = new LibraryDataFactory();

	const defaultDataSize = 30;
	const defaultData = libraryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: LibraryModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		libraryModelState = initialLibraryModelState;
		store.setState({model: libraryModelState});
		...
		spyOn(store, 'select')
			.withArgs(getLibraryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getLibraryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminLibraryCrudListComponent);
			libraryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(libraryCrudListComponent.librarysCount).toBe(defaultData.length);
			...
			expect(libraryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			libraryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = libraryDataFactory.createAll(filteredDataSize);
			...
			expect(libraryCrudListComponent.librarysCount).toBe(filteredDataSize);
			expect(libraryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(libraryCrudListComponent.pageIndex).toBe(0);
			expect(libraryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			libraryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		libraryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		libraryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
