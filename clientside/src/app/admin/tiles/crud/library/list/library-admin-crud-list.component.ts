...
import * as modelAction from 'src/app/models/library/library.model.action';
import {LibraryModelState} from 'src/app/models/library/library.model.state';
...
import {
	getLibraryCollectionModels,
	getLibraryCollectionCount
} from 'src/app/models/library/library.model.selector';
...
import {CrudTileMode} from '../library-admin-crud.component';
...
@Component({
	selector: 'cb-library-admin-crud-list',
	templateUrl: './library-admin-crud-list.component.html',
	styleUrls: [
		'./library-admin-crud-list.component.scss',
...
export class AdminLibraryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<LibraryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...LibraryModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = LibraryModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'cdb4aebd-7893-4e9b-bc5a-a45d4fc158b2';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.librarysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.librarysCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: LibraryModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.LibraryAction(
				modelAction.LibraryModelActionTypes.INITIALISE_LIBRARY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.LibraryAction(
				modelAction.LibraryModelActionTypes.FETCH_LIBRARY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.LibraryAction(
						modelAction.LibraryModelActionTypes.COUNT_LIBRARYS,
						null
					)
				]
			))
		...
		this.librarys = this.store.select(getLibraryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getLibraryCollectionCount, this.collectionId).subscribe(
				(count) => this.librarysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: LibraryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.LibraryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.LibraryModelActionTypes.EXPORT_ALL_LIBRARY;
		} else {
			actionType = modelAction.LibraryModelActionTypes.EXPORT_LIBRARY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.LibraryAction(
				modelAction.LibraryModelActionTypes.IMPORT_LIBRARYS,
				{
					file: file,
				},
				[new modelAction.LibraryAction(
			...
			window.open(environment.API_URL + '/api/library/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: LibraryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Library entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Library entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.LibraryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.LibraryModelActionTypes.DELETE_LIBRARY;
						} else if (usingSelected) {
							actionType = modelAction.LibraryModelActionTypes.DELETE_ALL_LIBRARY;
						} else {
							actionType = modelAction.LibraryModelActionTypes.DELETE_LIBRARY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.LibraryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_LIBRARY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_LIBRARY and DELETE_LIBRARY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<LibraryModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = LibraryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.librarysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
