...
import {LibraryModelState, LibraryModelAudit} from 'src/app/models/library/library.model.state';
import {LibraryModel} from 'src/app/models/library/library.model';
import {
	getLibraryModelAuditsByEntityId,
	getLibraryModelWithId,
	getCountLibraryModels,
...
@Component({
	selector: 'cb-library-admin-crud-edit',
	templateUrl: './library-admin-crud-edit.component.html',
	styleUrls: [
		'./library-admin-crud-edit.component.scss',
...
export class AdminLibraryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	libraryAudits$: Observable<LibraryModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = LibraryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountLibraryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new LibraryModel();
		...
		if (this.targetModelId) {
			this.libraryAudits$ = this.store.select(getLibraryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<LibraryModel> = {
		...
		this.store.dispatch(new modelAction.LibraryAction(
			modelAction.LibraryModelActionTypes.FETCH_LIBRARY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getLibraryModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.LibraryAction(
			modelAction.LibraryModelActionTypes.FETCH_LIBRARY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.LibraryAction(
				modelAction.LibraryModelActionTypes.CREATE_LIBRARY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.LibraryAction(
				modelAction.LibraryModelActionTypes.UPDATE_LIBRARY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: LibraryModel = new LibraryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getLibraryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new LibraryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = LibraryModel.getProps();
		this.modelRelations = LibraryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: LibraryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): LibraryModel {
		let updatedModel: LibraryModel = new LibraryModel(model);
