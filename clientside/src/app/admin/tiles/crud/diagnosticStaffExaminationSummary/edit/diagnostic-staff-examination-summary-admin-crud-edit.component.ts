...
import * as modelAction from 'src/app/models/diagnosticStaffExaminationSummary/diagnostic_staff_examination_summary.model.action';
import {DiagnosticStaffExaminationSummaryModelState, DiagnosticStaffExaminationSummaryModelAudit} from 'src/app/models/diagnosticStaffExaminationSummary/diagnostic_staff_examination_summary.model.state';
...
import {
	getDiagnosticStaffExaminationSummaryModelAuditsByEntityId,
	getDiagnosticStaffExaminationSummaryModelWithId,
	getCountDiagnosticStaffExaminationSummaryModels,
...
import {CrudTileMode} from '../diagnostic-staff-examination-summary-admin-crud.component';
...
@Component({
	selector: 'cb-diagnostic-staff-examination-summary-admin-crud-edit',
	templateUrl: './diagnostic-staff-examination-summary-admin-crud-edit.component.html',
	styleUrls: [
		'./diagnostic-staff-examination-summary-admin-crud-edit.component.scss',
...
export class AdminDiagnosticStaffExaminationSummaryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'c8d78524-2df9-42b0-98d8-59bf215af2bd';
	 ...
	 */
	diagnosticStaffExaminationSummaryAudits$: Observable<DiagnosticStaffExaminationSummaryModelAudit[]>;
	...
	registrationOppositeRelation = 'diagnosticStaffExaminationSummary';
	...
	diagnosisExaminationRecordOppositeRelation = 'diagnosticStaffExaminationSummary';
	...
	staffOppositeRelation = 'diagnosticStaffExaminationSummaries';
	 ...
	 */
	modelProperties: ModelProperty[] = DiagnosticStaffExaminationSummaryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = DiagnosticStaffExaminationSummaryModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DiagnosticStaffExaminationSummaryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: DiagnosticStaffExaminationSummaryModel;
	...
	constructor(
		private readonly store: Store<{ model: DiagnosticStaffExaminationSummaryModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDiagnosticStaffExaminationSummaryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DiagnosticStaffExaminationSummaryModel();
		...
		if (this.targetModelId) {
			this.diagnosticStaffExaminationSummaryAudits$ = this.store.select(getDiagnosticStaffExaminationSummaryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DiagnosticStaffExaminationSummaryModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
			modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.FETCH_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDiagnosticStaffExaminationSummaryModelWithId, this.targetModelId)
			...
			model = new DiagnosticStaffExaminationSummaryModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('diagnostic-staff-examination-summary');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
			modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.FETCH_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<DiagnosticStaffExaminationSummaryModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.diagnosisExaminationRecordMultiCrudActive || this.registrationMultiCrudActive || this.staffMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.DiagnosticStaffExaminationSummaryAction(
						modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.COUNT_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
				modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.CREATE_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
				modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.UPDATE_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DiagnosticStaffExaminationSummaryModel = new DiagnosticStaffExaminationSummaryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDiagnosticStaffExaminationSummaryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DiagnosticStaffExaminationSummaryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DiagnosticStaffExaminationSummaryModel.getProps();
		this.modelRelations = DiagnosticStaffExaminationSummaryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: DiagnosticStaffExaminationSummaryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DiagnosticStaffExaminationSummaryModel {
		let updatedModel: DiagnosticStaffExaminationSummaryModel = new DiagnosticStaffExaminationSummaryModel(model);
