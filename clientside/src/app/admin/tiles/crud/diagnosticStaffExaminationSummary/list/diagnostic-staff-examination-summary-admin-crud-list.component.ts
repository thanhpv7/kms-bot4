...
import {DiagnosticStaffExaminationSummaryModelState} from 'src/app/models/diagnosticStaffExaminationSummary/diagnostic_staff_examination_summary.model.state';
...
import {
	getDiagnosticStaffExaminationSummaryCollectionModels,
	getDiagnosticStaffExaminationSummaryCollectionCount
...
@Component({
	selector: 'cb-diagnostic-staff-examination-summary-admin-crud-list',
	templateUrl: './diagnostic-staff-examination-summary-admin-crud-list.component.html',
	styleUrls: [
		'./diagnostic-staff-examination-summary-admin-crud-list.component.scss',
...
export class AdminDiagnosticStaffExaminationSummaryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DiagnosticStaffExaminationSummaryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DiagnosticStaffExaminationSummaryModel.searchFields,
	...
	/**
	 * List of Diagnostic Staff Examination Summarys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	diagnosticStaffExaminationSummarys: Observable<DiagnosticStaffExaminationSummaryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	diagnosticStaffExaminationSummarysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.diagnosticStaffExaminationSummarysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.diagnosticStaffExaminationSummarysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
				modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.INITIALISE_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
				modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.FETCH_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.diagnosticStaffExaminationSummarys = this.store.select(getDiagnosticStaffExaminationSummaryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDiagnosticStaffExaminationSummaryCollectionCount, this.collectionId).subscribe(
				(count) => this.diagnosticStaffExaminationSummarysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DiagnosticStaffExaminationSummaryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DiagnosticStaffExaminationSummaryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.EXPORT_ALL_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY;
		} else {
			actionType = modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.EXPORT_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
				modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.IMPORT_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARYS,
				{
					file: file,
				},
				[new modelAction.DiagnosticStaffExaminationSummaryAction(
			...
			window.open(environment.API_URL + '/api/diagnosticstaffexaminationsummary/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DiagnosticStaffExaminationSummaryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Staff Examination Summary entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Staff Examination Summary entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DiagnosticStaffExaminationSummaryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.DELETE_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY;
						} else if (usingSelected) {
							actionType = modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.DELETE_ALL_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY;
						} else {
							actionType = modelAction.DiagnosticStaffExaminationSummaryModelActionTypes.DELETE_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DiagnosticStaffExaminationSummaryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY and DELETE_DIAGNOSTIC_STAFF_EXAMINATION_SUMMARY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DiagnosticStaffExaminationSummaryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.diagnosticStaffExaminationSummarysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
