...
import {AdminDiagnosticStaffExaminationSummaryCrudListComponent} from './diagnostic-staff-examination-summary-admin-crud-list.component';
...
import {AdminDiagnosticStaffExaminationSummaryCrudModule} from '../diagnostic-staff-examination-summary-admin-crud.module';
import {
	DiagnosticStaffExaminationSummaryModelState,
	initialState as initialDiagnosticStaffExaminationSummaryModelState
} from 'src/app/models/diagnosticStaffExaminationSummary/diagnostic_staff_examination_summary.model.state';
 ...
 */
describe('Diagnostic Staff Examination Summary Crud List Tile Component', () => {

	let fixture;
	let diagnosticStaffExaminationSummaryCrudListComponent: AdminDiagnosticStaffExaminationSummaryCrudListComponent;

	let store: MockStore<{ model: DiagnosticStaffExaminationSummaryModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let diagnosticStaffExaminationSummaryModelState: DiagnosticStaffExaminationSummaryModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<DiagnosticStaffExaminationSummaryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'c8d78524-2df9-42b0-98d8-59bf215af2bd';
	const searchContent = 'text';

	const searchableFieldCount = DiagnosticStaffExaminationSummaryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'diagnostic-staff-examination-summary-admin-crud',
		urls: ['diagnostic-staff-examination-summary-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const diagnosticStaffExaminationSummaryDataFactory = new DiagnosticStaffExaminationSummaryDataFactory();

	const defaultDataSize = 30;
	const defaultData = diagnosticStaffExaminationSummaryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: DiagnosticStaffExaminationSummaryModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		diagnosticStaffExaminationSummaryModelState = initialDiagnosticStaffExaminationSummaryModelState;
		store.setState({model: diagnosticStaffExaminationSummaryModelState});
		...
		spyOn(store, 'select')
			.withArgs(getDiagnosticStaffExaminationSummaryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getDiagnosticStaffExaminationSummaryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminDiagnosticStaffExaminationSummaryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminDiagnosticStaffExaminationSummaryCrudListComponent);
			diagnosticStaffExaminationSummaryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(diagnosticStaffExaminationSummaryCrudListComponent.diagnosticStaffExaminationSummarysCount).toBe(defaultData.length);
			...
			expect(diagnosticStaffExaminationSummaryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			diagnosticStaffExaminationSummaryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = diagnosticStaffExaminationSummaryDataFactory.createAll(filteredDataSize);
			...
			expect(diagnosticStaffExaminationSummaryCrudListComponent.diagnosticStaffExaminationSummarysCount).toBe(filteredDataSize);
			expect(diagnosticStaffExaminationSummaryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(diagnosticStaffExaminationSummaryCrudListComponent.pageIndex).toBe(0);
			expect(diagnosticStaffExaminationSummaryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			diagnosticStaffExaminationSummaryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		diagnosticStaffExaminationSummaryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		diagnosticStaffExaminationSummaryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
