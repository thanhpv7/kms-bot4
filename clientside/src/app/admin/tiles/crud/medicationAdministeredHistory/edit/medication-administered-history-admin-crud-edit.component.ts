...
import * as modelAction from 'src/app/models/medicationAdministeredHistory/medication_administered_history.model.action';
import {MedicationAdministeredHistoryModelState, MedicationAdministeredHistoryModelAudit} from 'src/app/models/medicationAdministeredHistory/medication_administered_history.model.state';
...
import {
	getMedicationAdministeredHistoryModelAuditsByEntityId,
...
import {CrudTileMode} from '../medication-administered-history-admin-crud.component';
...
@Component({
	selector: 'cb-medication-administered-history-admin-crud-edit',
	templateUrl: './medication-administered-history-admin-crud-edit.component.html',
	styleUrls: [
		'./medication-administered-history-admin-crud-edit.component.scss',
...
export class AdminMedicationAdministeredHistoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'ed879bc9-2669-4789-9ea6-6561e90561a3';
	 ...
	 */
	medicationAdministeredHistoryAudits$: Observable<MedicationAdministeredHistoryModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Medication Item and Medication Administered Hisotries here] off begin
	medicationItemMultiCrudActive: boolean = false;
	medicationItemDisplayName = 'Medication Item';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Medication Item and Medication Administered Hisotries here] end
	medicationItemOppositeRelation = 'medicationAdministeredHisotries';
	medicationItemRelations: ModelRelation[] = this.medicationItemMultiCrudActive ? MedicationItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.medicationItemOppositeRelation) {
	...
	staffOppositeRelation = 'medicationAdministeredHisotries';
	...
	medicationCompoundMultiCrudActive: boolean = false;
	medicationCompoundDisplayName = 'Medication Compound';
	...
	medicationCompoundOppositeRelation = 'medicationAdministeredHistories';
	medicationCompoundRelations: ModelRelation[] = this.medicationCompoundMultiCrudActive ? MedicationCompoundModel.getRelations()
		.map(relation => {
			if (relation.id === this.medicationCompoundOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	 ...
	 */
	modelProperties: ModelProperty[] = MedicationAdministeredHistoryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicationAdministeredHistoryModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicationAdministeredHistoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'medicationItem' && this.medicationItemMultiCrudActive) {
				...
				if (relation.id === 'medicationCompound' && this.medicationCompoundMultiCrudActive) {
	 ...
	 */
	targetModel: MedicationAdministeredHistoryModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicationAdministeredHistoryModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicationAdministeredHistoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicationAdministeredHistoryModel();
		...
		if (this.targetModelId) {
			this.medicationAdministeredHistoryAudits$ = this.store.select(getMedicationAdministeredHistoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicationAdministeredHistoryModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
			modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_MEDICATION_ADMINISTERED_HISTORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicationAdministeredHistoryModelWithId, this.targetModelId)
			...
			model = new MedicationAdministeredHistoryModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medication-administered-history');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
			modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_MEDICATION_ADMINISTERED_HISTORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicationAdministeredHistoryModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.medicationItemMultiCrudActive || this.staffMultiCrudActive || this.medicationCompoundMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicationAdministeredHistoryAction(
						modelAction.MedicationAdministeredHistoryModelActionTypes.COUNT_MEDICATION_ADMINISTERED_HISTORYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
				modelAction.MedicationAdministeredHistoryModelActionTypes.CREATE_MEDICATION_ADMINISTERED_HISTORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
				modelAction.MedicationAdministeredHistoryModelActionTypes.UPDATE_MEDICATION_ADMINISTERED_HISTORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicationAdministeredHistoryModel = new MedicationAdministeredHistoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicationAdministeredHistoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicationAdministeredHistoryModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.medicationItemDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MedicationItemModel.getProps(), this.medicationItemRelations, this.isViewOnly),
					new MedicationItemModel(),
					MedicationItemModel,
					this.medicationItemDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.medicationItemRelations,
					true
				);
			...
			if (event.entityName === this.medicationCompoundDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MedicationCompoundModel.getProps(), this.medicationCompoundRelations, this.isViewOnly),
					new MedicationCompoundModel(),
					MedicationCompoundModel,
					this.medicationCompoundDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.medicationCompoundRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicationAdministeredHistoryModel.getProps();
		this.modelRelations = MedicationAdministeredHistoryModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'medicationItem';
		}).hideElement = this.medicationItemMultiCrudActive || this.modelRelations.find(rel => rel.id === 'medicationItem').hideElement;
		...
		this.modelRelations.find(rel => {
			return rel.id === 'medicationCompound';
		}).hideElement = this.medicationCompoundMultiCrudActive || this.modelRelations.find(rel => rel.id === 'medicationCompound').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.medicationItemMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.medicationItemDisplayName, many: false});
		}
		...
		if (this.medicationCompoundMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.medicationCompoundDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.medicationItem && this.targetModel.medicationItem.id && this.medicationItemMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				MedicationItemModel,
				this.medicationItemOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.medicationItem.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getMedicationItemModelWithId, this.targetModel.medicationItemId).subscribe(medicationItem => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MedicationItemModel.getProps(), this.medicationItemRelations, this.isViewOnly),
					medicationItem,
					MedicationItemModel,
		...
		if (this.targetModel.medicationCompound && this.targetModel.medicationCompound.id && this.medicationCompoundMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				MedicationCompoundModel,
				this.medicationCompoundOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.medicationCompound.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getMedicationCompoundModelWithId, this.targetModel.medicationCompoundId).subscribe(medicationCompound => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(MedicationCompoundModel.getProps(), this.medicationCompoundRelations, this.isViewOnly),
					medicationCompound,
					MedicationCompoundModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicationAdministeredHistoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicationAdministeredHistoryModel {
		let updatedModel: MedicationAdministeredHistoryModel = new MedicationAdministeredHistoryModel(model);

		if (this.medicationItemMultiCrudActive) {
			updatedModel.medicationItem = event.additionalEntities.medicationItem ? event.additionalEntities.medicationItem : undefined;

			if (!updatedModel.medicationItem && event.payload) {
				event.payload.medicationItemId = null;
			}
		} else if (event.payload?.medicationItemId !== undefined) {
			event.payload.medicationItem = null;
			updatedModel.medicationItem = null;
		}
		...
		if (this.medicationCompoundMultiCrudActive) {
			updatedModel.medicationCompound = event.additionalEntities.medicationCompound ? event.additionalEntities.medicationCompound : undefined;

			if (!updatedModel.medicationCompound && event.payload) {
				event.payload.medicationCompoundId = null;
			}
		} else if (event.payload?.medicationCompoundId !== undefined) {
			event.payload.medicationCompound = null;
			updatedModel.medicationCompound = null;
		}
