...
import {MedicationAdministeredHistoryModelState} from 'src/app/models/medicationAdministeredHistory/medication_administered_history.model.state';
...
import {
	...
	getMedicationAdministeredHistoryCollectionCount
...
@Component({
	selector: 'cb-medication-administered-history-admin-crud-list',
	templateUrl: './medication-administered-history-admin-crud-list.component.html',
	styleUrls: [
		'./medication-administered-history-admin-crud-list.component.scss',
...
export class AdminMedicationAdministeredHistoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicationAdministeredHistoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicationAdministeredHistoryModel.searchFields,
	...
	/**
	 * List of Medication Administered Historys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicationAdministeredHistorys: Observable<MedicationAdministeredHistoryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicationAdministeredHistorysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicationAdministeredHistorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicationAdministeredHistorysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
				modelAction.MedicationAdministeredHistoryModelActionTypes.INITIALISE_MEDICATION_ADMINISTERED_HISTORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
				modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_MEDICATION_ADMINISTERED_HISTORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicationAdministeredHistorys = this.store.select(getMedicationAdministeredHistoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicationAdministeredHistoryCollectionCount, this.collectionId).subscribe(
				(count) => this.medicationAdministeredHistorysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicationAdministeredHistoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicationAdministeredHistoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicationAdministeredHistoryModelActionTypes.EXPORT_ALL_MEDICATION_ADMINISTERED_HISTORY;
		} else {
			actionType = modelAction.MedicationAdministeredHistoryModelActionTypes.EXPORT_MEDICATION_ADMINISTERED_HISTORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
				modelAction.MedicationAdministeredHistoryModelActionTypes.IMPORT_MEDICATION_ADMINISTERED_HISTORYS,
				{
					file: file,
				},
				[new modelAction.MedicationAdministeredHistoryAction(
			...
			window.open(environment.API_URL + '/api/medicationadministeredhistory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicationAdministeredHistoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medication Administered History entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medication Administered History entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicationAdministeredHistoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicationAdministeredHistoryModelActionTypes.DELETE_MEDICATION_ADMINISTERED_HISTORY;
						} else if (usingSelected) {
							actionType = modelAction.MedicationAdministeredHistoryModelActionTypes.DELETE_ALL_MEDICATION_ADMINISTERED_HISTORY;
						} else {
							actionType = modelAction.MedicationAdministeredHistoryModelActionTypes.DELETE_MEDICATION_ADMINISTERED_HISTORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicationAdministeredHistoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICATION_ADMINISTERED_HISTORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICATION_ADMINISTERED_HISTORY and DELETE_MEDICATION_ADMINISTERED_HISTORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicationAdministeredHistoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicationAdministeredHistorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
