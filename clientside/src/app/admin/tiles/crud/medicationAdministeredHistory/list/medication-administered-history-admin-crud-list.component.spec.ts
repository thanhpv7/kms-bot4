...
import {AdminMedicationAdministeredHistoryCrudListComponent} from './medication-administered-history-admin-crud-list.component';
...
import {AdminMedicationAdministeredHistoryCrudModule} from '../medication-administered-history-admin-crud.module';
import {
 ...
 */
describe('Medication Administered History Crud List Tile Component', () => {

	let fixture;
	let medicationAdministeredHistoryCrudListComponent: AdminMedicationAdministeredHistoryCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicationAdministeredHistoryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ed879bc9-2669-4789-9ea6-6561e90561a3';
	const searchContent = 'text';

	const searchableFieldCount = MedicationAdministeredHistoryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medication-administered-history-admin-crud',
		urls: ['medication-administered-history-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicationAdministeredHistoryDataFactory = new MedicationAdministeredHistoryDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicationAdministeredHistoryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getMedicationAdministeredHistoryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicationAdministeredHistoryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicationAdministeredHistoryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicationAdministeredHistoryCrudListComponent);
			medicationAdministeredHistoryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicationAdministeredHistoryCrudListComponent.medicationAdministeredHistorysCount).toBe(defaultData.length);
			...
			expect(medicationAdministeredHistoryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicationAdministeredHistoryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicationAdministeredHistoryDataFactory.createAll(filteredDataSize);
			...
			expect(medicationAdministeredHistoryCrudListComponent.medicationAdministeredHistorysCount).toBe(filteredDataSize);
			expect(medicationAdministeredHistoryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicationAdministeredHistoryCrudListComponent.pageIndex).toBe(0);
			expect(medicationAdministeredHistoryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicationAdministeredHistoryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicationAdministeredHistoryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicationAdministeredHistoryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
