...
import {JournalModelState, JournalModelAudit} from 'src/app/models/journal/journal.model.state';
import {JournalModel} from 'src/app/models/journal/journal.model';
import {
	getJournalModelAuditsByEntityId,
...
@Component({
	selector: 'cb-journal-admin-crud-edit',
	templateUrl: './journal-admin-crud-edit.component.html',
	styleUrls: [
		'./journal-admin-crud-edit.component.scss',
...
export class AdminJournalCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	journalAudits$: Observable<JournalModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = JournalModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountJournalModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new JournalModel();
		...
		if (this.targetModelId) {
			this.journalAudits$ = this.store.select(getJournalModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<JournalModel> = {
		...
		this.store.dispatch(new modelAction.JournalAction(
			modelAction.JournalModelActionTypes.FETCH_JOURNAL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getJournalModelWithId, this.targetModelId)
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.JournalAction(
			modelAction.JournalModelActionTypes.FETCH_JOURNAL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.JournalAction(
				modelAction.JournalModelActionTypes.CREATE_JOURNAL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.JournalAction(
				modelAction.JournalModelActionTypes.UPDATE_JOURNAL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: JournalModel = new JournalModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getJournalModelWithId, this.targetModelId).subscribe(model => {
					let differences = new JournalModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = JournalModel.getProps();
		this.modelRelations = JournalModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: JournalModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): JournalModel {
		let updatedModel: JournalModel = new JournalModel(model);
