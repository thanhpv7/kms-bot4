...
import * as modelAction from 'src/app/models/journal/journal.model.action';
import {JournalModelState} from 'src/app/models/journal/journal.model.state';
...
import {CrudTileMode} from '../journal-admin-crud.component';
...
@Component({
	selector: 'cb-journal-admin-crud-list',
	templateUrl: './journal-admin-crud-list.component.html',
	styleUrls: [
		'./journal-admin-crud-list.component.scss',
...
export class AdminJournalCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<JournalModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...JournalModel.searchFields,
	...
	/**
	 * List of Journals that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	journals: Observable<JournalModel[]>;
	 ...
	 */
	modelProperties: ModelProperty[] = JournalModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = JournalModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'b8470aa2-8b41-4521-84a1-1859cdb4d2b1';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.journalsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.journalsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: JournalModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.JournalAction(
				modelAction.JournalModelActionTypes.INITIALISE_JOURNAL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.JournalAction(
				modelAction.JournalModelActionTypes.FETCH_JOURNAL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.JournalAction(
						modelAction.JournalModelActionTypes.COUNT_JOURNALS,
						null
					)
				]
			))
		...
		this.journals = this.store.select(getJournalCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getJournalCollectionCount, this.collectionId).subscribe(
				(count) => this.journalsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: JournalModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.JournalModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.JournalModelActionTypes.EXPORT_ALL_JOURNAL;
		} else {
			actionType = modelAction.JournalModelActionTypes.EXPORT_JOURNAL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.JournalAction(
				modelAction.JournalModelActionTypes.IMPORT_JOURNALS,
				{
					file: file,
				},
				[new modelAction.JournalAction(
			...
			window.open(environment.API_URL + '/api/journal/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: JournalModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Journal entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Journal entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.JournalModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.JournalModelActionTypes.DELETE_JOURNAL;
						} else if (usingSelected) {
							actionType = modelAction.JournalModelActionTypes.DELETE_ALL_JOURNAL;
						} else {
							actionType = modelAction.JournalModelActionTypes.DELETE_JOURNAL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.JournalAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_JOURNAL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_JOURNAL and DELETE_JOURNAL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<JournalModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = JournalModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.journalsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
