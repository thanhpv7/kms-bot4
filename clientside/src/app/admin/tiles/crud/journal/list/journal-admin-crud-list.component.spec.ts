...
import {AdminJournalCrudListComponent} from './journal-admin-crud-list.component';
...
import {AdminJournalCrudModule} from '../journal-admin-crud.module';
import {
	JournalModelState,
 ...
 */
describe('Journal Crud List Tile Component', () => {

	let fixture;
	let journalCrudListComponent: AdminJournalCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<JournalModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b8470aa2-8b41-4521-84a1-1859cdb4d2b1';
	const searchContent = 'text';

	const searchableFieldCount = JournalModel.searchFields.length;

	const routerState: RouterState = {
		url: 'journal-admin-crud',
		urls: ['journal-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const journalDataFactory = new JournalDataFactory();

	const defaultDataSize = 30;
	const defaultData = journalDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getJournalCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getJournalCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminJournalCrudListComponent);
			journalCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(journalCrudListComponent.journalsCount).toBe(defaultData.length);
			...
			expect(journalCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			journalCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = journalDataFactory.createAll(filteredDataSize);
			...
			expect(journalCrudListComponent.journalsCount).toBe(filteredDataSize);
			expect(journalCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(journalCrudListComponent.pageIndex).toBe(0);
			expect(journalCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			journalCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		journalCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		journalCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
