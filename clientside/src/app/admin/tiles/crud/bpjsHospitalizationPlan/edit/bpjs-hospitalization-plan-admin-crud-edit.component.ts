...
import {BpjsHospitalizationPlanModelState, BpjsHospitalizationPlanModelAudit} from 'src/app/models/bpjsHospitalizationPlan/bpjs_hospitalization_plan.model.state';
import {BpjsHospitalizationPlanModel} from 'src/app/models/bpjsHospitalizationPlan/bpjs_hospitalization_plan.model';
import {
	getBpjsHospitalizationPlanModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-hospitalization-plan-admin-crud-edit',
	templateUrl: './bpjs-hospitalization-plan-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-hospitalization-plan-admin-crud-edit.component.scss',
...
export class AdminBpjsHospitalizationPlanCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsHospitalizationPlanAudits$: Observable<BpjsHospitalizationPlanModelAudit[]>;
	...
	patientPaymentBPJSOppositeRelation = 'bpjsHospitalizationPlans';
	...
	bpjsDoctorMappingOppositeRelation = 'kodedokter';
	...
	bpjsServiceMappingOppositeRelation = 'polikontrol';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsHospitalizationPlanModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsHospitalizationPlanModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsHospitalizationPlanModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsHospitalizationPlanModel();
		...
		if (this.targetModelId) {
			this.bpjsHospitalizationPlanAudits$ = this.store.select(getBpjsHospitalizationPlanModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsHospitalizationPlanModel> = {
		...
		this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
			modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_BPJS_HOSPITALIZATION_PLAN,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsHospitalizationPlanModelWithId, this.targetModelId)
			...
			model = new BpjsHospitalizationPlanModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
			modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_BPJS_HOSPITALIZATION_PLAN_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsHospitalizationPlanModel> = {
				...
				queryParams: {
					persistRelations: this.patientPaymentBPJSMultiCrudActive || this.bpjsDoctorMappingMultiCrudActive || this.bpjsServiceMappingMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
				modelAction.BpjsHospitalizationPlanModelActionTypes.CREATE_BPJS_HOSPITALIZATION_PLAN,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
				modelAction.BpjsHospitalizationPlanModelActionTypes.UPDATE_BPJS_HOSPITALIZATION_PLAN,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsHospitalizationPlanModel = new BpjsHospitalizationPlanModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsHospitalizationPlanModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsHospitalizationPlanModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsHospitalizationPlanModel.getProps();
		this.modelRelations = BpjsHospitalizationPlanModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsHospitalizationPlanModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsHospitalizationPlanModel {
		let updatedModel: BpjsHospitalizationPlanModel = new BpjsHospitalizationPlanModel(model);
