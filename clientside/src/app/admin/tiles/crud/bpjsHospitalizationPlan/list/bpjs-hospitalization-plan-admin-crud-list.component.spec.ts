...
import {AdminBpjsHospitalizationPlanCrudListComponent} from './bpjs-hospitalization-plan-admin-crud-list.component';
...
import {AdminBpjsHospitalizationPlanCrudModule} from '../bpjs-hospitalization-plan-admin-crud.module';
import {
 ...
 */
describe('BPJS Hospitalization Plan Crud List Tile Component', () => {

	let fixture;
	let bpjsHospitalizationPlanCrudListComponent: AdminBpjsHospitalizationPlanCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsHospitalizationPlanModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8e774fd1-65be-48dd-9e1c-80dd02e7987c';
	const searchContent = 'text';

	const searchableFieldCount = BpjsHospitalizationPlanModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-hospitalization-plan-admin-crud',
		urls: ['bpjs-hospitalization-plan-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsHospitalizationPlanDataFactory = new BpjsHospitalizationPlanDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsHospitalizationPlanDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsHospitalizationPlanCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsHospitalizationPlanCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsHospitalizationPlanCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsHospitalizationPlanCrudListComponent);
			bpjsHospitalizationPlanCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsHospitalizationPlanCrudListComponent.bpjsHospitalizationPlansCount).toBe(defaultData.length);
			...
			expect(bpjsHospitalizationPlanCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsHospitalizationPlanCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsHospitalizationPlanDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsHospitalizationPlanCrudListComponent.bpjsHospitalizationPlansCount).toBe(filteredDataSize);
			expect(bpjsHospitalizationPlanCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsHospitalizationPlanCrudListComponent.pageIndex).toBe(0);
			expect(bpjsHospitalizationPlanCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsHospitalizationPlanCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsHospitalizationPlanCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsHospitalizationPlanCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
