...
import * as modelAction from 'src/app/models/bpjsHospitalizationPlan/bpjs_hospitalization_plan.model.action';
import {BpjsHospitalizationPlanModelState} from 'src/app/models/bpjsHospitalizationPlan/bpjs_hospitalization_plan.model.state';
...
import {
	...
	getBpjsHospitalizationPlanCollectionCount
...
import {CrudTileMode} from '../bpjs-hospitalization-plan-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-hospitalization-plan-admin-crud-list',
	templateUrl: './bpjs-hospitalization-plan-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-hospitalization-plan-admin-crud-list.component.scss',
...
export class AdminBpjsHospitalizationPlanCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsHospitalizationPlanModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsHospitalizationPlanModel.searchFields,
	...
	/**
	 * List of BPJS Hospitalization Plans that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsHospitalizationPlans: Observable<BpjsHospitalizationPlanModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsHospitalizationPlansCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsHospitalizationPlanModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsHospitalizationPlanModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '8e774fd1-65be-48dd-9e1c-80dd02e7987c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsHospitalizationPlansCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsHospitalizationPlansCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsHospitalizationPlanModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
				modelAction.BpjsHospitalizationPlanModelActionTypes.INITIALISE_BPJS_HOSPITALIZATION_PLAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
				modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_BPJS_HOSPITALIZATION_PLAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsHospitalizationPlanAction(
						modelAction.BpjsHospitalizationPlanModelActionTypes.COUNT_BPJS_HOSPITALIZATION_PLANS,
						null
					)
				]
			))
		...
		this.bpjsHospitalizationPlans = this.store.select(getBpjsHospitalizationPlanCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsHospitalizationPlanCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsHospitalizationPlansCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-hospitalization-plan');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsHospitalizationPlanModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsHospitalizationPlanModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsHospitalizationPlanModelActionTypes.EXPORT_ALL_BPJS_HOSPITALIZATION_PLAN;
		} else {
			actionType = modelAction.BpjsHospitalizationPlanModelActionTypes.EXPORT_BPJS_HOSPITALIZATION_PLAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
				modelAction.BpjsHospitalizationPlanModelActionTypes.IMPORT_BPJS_HOSPITALIZATION_PLANS,
				{
					file: file,
				},
				[new modelAction.BpjsHospitalizationPlanAction(
			...
			window.open(environment.API_URL + '/api/bpjshospitalizationplan/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsHospitalizationPlanModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS Hospitalization Plan entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS Hospitalization Plan entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsHospitalizationPlanModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsHospitalizationPlanModelActionTypes.DELETE_BPJS_HOSPITALIZATION_PLAN;
						} else if (usingSelected) {
							actionType = modelAction.BpjsHospitalizationPlanModelActionTypes.DELETE_ALL_BPJS_HOSPITALIZATION_PLAN;
						} else {
							actionType = modelAction.BpjsHospitalizationPlanModelActionTypes.DELETE_BPJS_HOSPITALIZATION_PLAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsHospitalizationPlanAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_HOSPITALIZATION_PLAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_HOSPITALIZATION_PLAN and DELETE_BPJS_HOSPITALIZATION_PLAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsHospitalizationPlanModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsHospitalizationPlanModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsHospitalizationPlansCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
