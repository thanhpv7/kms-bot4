...
import * as modelAction from 'src/app/models/chartOfAccount/chart_of_account.model.action';
import {ChartOfAccountModelState, ChartOfAccountModelAudit} from 'src/app/models/chartOfAccount/chart_of_account.model.state';
import {ChartOfAccountModel} from 'src/app/models/chartOfAccount/chart_of_account.model';
import {
	getChartOfAccountModelAuditsByEntityId,
...
import * as chartOfAccountMappingModelAction from 'src/app/models/chartOfAccountMapping/chart_of_account_mapping.model.action';
import {
	getChartOfAccountMappingCollectionModels, getChartOfAccountMappingModelWithId,
...
import {CrudTileMode} from '../chart-of-account-admin-crud.component';
...
@Component({
	selector: 'cb-chart-of-account-admin-crud-edit',
	templateUrl: './chart-of-account-admin-crud-edit.component.html',
	styleUrls: [
		'./chart-of-account-admin-crud-edit.component.scss',
...
export class AdminChartOfAccountCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '8b78b205-942c-4e05-8bb7-4d5cf5d0d4fa';
	 ...
	 */
	chartOfAccountAudits$: Observable<ChartOfAccountModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Chart of Account and Chart of Account Mappings here] off begin
	chartOfAccountMappingsMultiCrudActive: boolean = false;
	chartOfAccountMappingsDisplayName = 'Chart of Account Mappings';
	chartOfAccountMappingsCollectionId: string = uuidv4();
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Chart of Account and Chart of Account Mappings here] end
	chartOfAccountMappingsOppositeRelation = 'chartOfAccount';
	chartOfAccountMappingsRelations: ModelRelation[] = this.chartOfAccountMappingsMultiCrudActive ? ChartOfAccountMappingModel.getRelations()
		.map(relation => {
			if (relation.id === this.chartOfAccountMappingsOppositeRelation) {
	 ...
	 */
	modelProperties: ModelProperty[] = ChartOfAccountModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ChartOfAccountModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ChartOfAccountModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'chartOfAccountMappings' && this.chartOfAccountMappingsMultiCrudActive) {
					exp.fields = ChartOfAccountMappingModel.getProps().map(prop => {
						return prop.name;
					});
				}
	 ...
	 */
	targetModel: ChartOfAccountModel;
	...
	constructor(
		private readonly store: Store<{ model: ChartOfAccountModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountChartOfAccountModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ChartOfAccountModel();
		...
		if (this.targetModelId) {
			this.chartOfAccountAudits$ = this.store.select(getChartOfAccountModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ChartOfAccountModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.ChartOfAccountAction(
			modelAction.ChartOfAccountModelActionTypes.FETCH_CHART_OF_ACCOUNT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getChartOfAccountModelWithId, this.targetModelId)
			...
			model = new ChartOfAccountModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('chart-of-account');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ChartOfAccountAction(
			modelAction.ChartOfAccountModelActionTypes.FETCH_CHART_OF_ACCOUNT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ChartOfAccountModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.chartOfAccountMappingsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.ChartOfAccountAction(
						modelAction.ChartOfAccountModelActionTypes.COUNT_CHART_OF_ACCOUNTS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.ChartOfAccountAction(
				modelAction.ChartOfAccountModelActionTypes.CREATE_CHART_OF_ACCOUNT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ChartOfAccountAction(
				modelAction.ChartOfAccountModelActionTypes.UPDATE_CHART_OF_ACCOUNT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ChartOfAccountModel = new ChartOfAccountModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getChartOfAccountModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ChartOfAccountModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.chartOfAccountMappingsDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(ChartOfAccountMappingModel.getProps(), this.chartOfAccountMappingsRelations, this.isViewOnly),
					new ChartOfAccountMappingModel(),
					ChartOfAccountMappingModel,
					this.chartOfAccountMappingsDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.chartOfAccountMappingsRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ChartOfAccountModel.getProps();
		this.modelRelations = ChartOfAccountModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'chartOfAccountMappings';
		}).hideElement = this.chartOfAccountMappingsMultiCrudActive || this.modelRelations.find(rel => rel.id === 'chartOfAccountMappings').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.chartOfAccountMappingsMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.chartOfAccountMappingsDisplayName, many: true});
			this.store.dispatch(new chartOfAccountMappingModelAction.ChartOfAccountMappingAction(
				chartOfAccountMappingModelAction.ChartOfAccountMappingModelActionTypes.INITIALISE_CHART_OF_ACCOUNT_MAPPING_COLLECTION_STATE,
				{
					collectionId: this.chartOfAccountMappingsCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getChartOfAccountMappingCollectionModels, this.chartOfAccountMappingsCollectionId).subscribe(models => {
				models.forEach(chartOfAccountMappings => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(ChartOfAccountMappingModel.getProps(), this.chartOfAccountMappingsRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.chartOfAccountMappingsIds && this.targetModel.chartOfAccountMappingsIds.length > 0 && this.chartOfAccountMappingsMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				ChartOfAccountMappingModel,
				this.chartOfAccountMappingsOppositeRelation,
				this.chartOfAccountMappingsCollectionId,
				this.targetModel.chartOfAccountMappingsIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: ChartOfAccountModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ChartOfAccountModel {
		let updatedModel: ChartOfAccountModel = new ChartOfAccountModel(model);

		updatedModel.chartOfAccountMappings = [];
		if (this.chartOfAccountMappingsMultiCrudActive) {
			if (event.additionalEntities.chartOfAccountMappings) {
				event.additionalEntities.chartOfAccountMappings.forEach(entity => {
					updatedModel.chartOfAccountMappings.push(entity);
				});
			}

			if (!updatedModel.chartOfAccountMappings.length) {
				updatedModel.chartOfAccountMappingsIds = [];
			}
		}
		return updatedModel;
	}
