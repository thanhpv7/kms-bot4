...
import {AdminChartOfAccountCrudListComponent} from './chart-of-account-admin-crud-list.component';
...
import {AdminChartOfAccountCrudModule} from '../chart-of-account-admin-crud.module';
import {
 ...
 */
describe('Chart of Account Crud List Tile Component', () => {

	let fixture;
	let chartOfAccountCrudListComponent: AdminChartOfAccountCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ChartOfAccountModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '8b78b205-942c-4e05-8bb7-4d5cf5d0d4fa';
	const searchContent = 'text';

	const searchableFieldCount = ChartOfAccountModel.searchFields.length;

	const routerState: RouterState = {
		url: 'chart-of-account-admin-crud',
		urls: ['chart-of-account-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const chartOfAccountDataFactory = new ChartOfAccountDataFactory();

	const defaultDataSize = 30;
	const defaultData = chartOfAccountDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getChartOfAccountCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getChartOfAccountCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminChartOfAccountCrudListComponent);
			chartOfAccountCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(chartOfAccountCrudListComponent.chartOfAccountsCount).toBe(defaultData.length);
			...
			expect(chartOfAccountCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			chartOfAccountCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = chartOfAccountDataFactory.createAll(filteredDataSize);
			...
			expect(chartOfAccountCrudListComponent.chartOfAccountsCount).toBe(filteredDataSize);
			expect(chartOfAccountCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(chartOfAccountCrudListComponent.pageIndex).toBe(0);
			expect(chartOfAccountCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			chartOfAccountCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		chartOfAccountCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		chartOfAccountCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
