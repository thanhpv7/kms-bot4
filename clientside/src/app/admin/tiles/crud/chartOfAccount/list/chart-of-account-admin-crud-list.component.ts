...
import {ChartOfAccountModelState} from 'src/app/models/chartOfAccount/chart_of_account.model.state';
...
import {
	...
	getChartOfAccountCollectionCount
...
@Component({
	selector: 'cb-chart-of-account-admin-crud-list',
	templateUrl: './chart-of-account-admin-crud-list.component.html',
	styleUrls: [
		'./chart-of-account-admin-crud-list.component.scss',
...
export class AdminChartOfAccountCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ChartOfAccountModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ChartOfAccountModel.searchFields,
	...
	/**
	 * List of Chart of Accounts that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	chartOfAccounts: Observable<ChartOfAccountModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	chartOfAccountsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.chartOfAccountsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.chartOfAccountsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ChartOfAccountAction(
				modelAction.ChartOfAccountModelActionTypes.INITIALISE_CHART_OF_ACCOUNT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ChartOfAccountAction(
				modelAction.ChartOfAccountModelActionTypes.FETCH_CHART_OF_ACCOUNT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.chartOfAccounts = this.store.select(getChartOfAccountCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getChartOfAccountCollectionCount, this.collectionId).subscribe(
				(count) => this.chartOfAccountsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ChartOfAccountModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ChartOfAccountModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ChartOfAccountModelActionTypes.EXPORT_ALL_CHART_OF_ACCOUNT;
		} else {
			actionType = modelAction.ChartOfAccountModelActionTypes.EXPORT_CHART_OF_ACCOUNT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ChartOfAccountAction(
				modelAction.ChartOfAccountModelActionTypes.IMPORT_CHART_OF_ACCOUNTS,
				{
					file: file,
				},
				[new modelAction.ChartOfAccountAction(
			...
			window.open(environment.API_URL + '/api/chartofaccount/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ChartOfAccountModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Chart of Account entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Chart of Account entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ChartOfAccountModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ChartOfAccountModelActionTypes.DELETE_CHART_OF_ACCOUNT;
						} else if (usingSelected) {
							actionType = modelAction.ChartOfAccountModelActionTypes.DELETE_ALL_CHART_OF_ACCOUNT;
						} else {
							actionType = modelAction.ChartOfAccountModelActionTypes.DELETE_CHART_OF_ACCOUNT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ChartOfAccountAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_CHART_OF_ACCOUNT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_CHART_OF_ACCOUNT and DELETE_CHART_OF_ACCOUNT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ChartOfAccountModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.chartOfAccountsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
