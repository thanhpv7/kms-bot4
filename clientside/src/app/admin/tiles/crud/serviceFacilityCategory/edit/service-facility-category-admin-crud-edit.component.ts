...
import {ServiceFacilityCategoryModelState, ServiceFacilityCategoryModelAudit} from 'src/app/models/serviceFacilityCategory/service_facility_category.model.state';
import {ServiceFacilityCategoryModel} from 'src/app/models/serviceFacilityCategory/service_facility_category.model';
import {
	getServiceFacilityCategoryModelAuditsByEntityId,
...
@Component({
	selector: 'cb-service-facility-category-admin-crud-edit',
	templateUrl: './service-facility-category-admin-crud-edit.component.html',
	styleUrls: [
		'./service-facility-category-admin-crud-edit.component.scss',
...
export class AdminServiceFacilityCategoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	serviceFacilityCategoryAudits$: Observable<ServiceFacilityCategoryModelAudit[]>;
	...
	serviceOppositeRelation = 'serviceFacilityCategories';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ServiceFacilityCategoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ServiceFacilityCategoryModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountServiceFacilityCategoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ServiceFacilityCategoryModel();
		...
		if (this.targetModelId) {
			this.serviceFacilityCategoryAudits$ = this.store.select(getServiceFacilityCategoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ServiceFacilityCategoryModel> = {
		...
		this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
			modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_SERVICE_FACILITY_CATEGORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getServiceFacilityCategoryModelWithId, this.targetModelId)
			...
			model = new ServiceFacilityCategoryModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
			modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_SERVICE_FACILITY_CATEGORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ServiceFacilityCategoryModel> = {
				...
				queryParams: {
					persistRelations: this.serviceMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
				modelAction.ServiceFacilityCategoryModelActionTypes.CREATE_SERVICE_FACILITY_CATEGORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
				modelAction.ServiceFacilityCategoryModelActionTypes.UPDATE_SERVICE_FACILITY_CATEGORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ServiceFacilityCategoryModel = new ServiceFacilityCategoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getServiceFacilityCategoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ServiceFacilityCategoryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ServiceFacilityCategoryModel.getProps();
		this.modelRelations = ServiceFacilityCategoryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ServiceFacilityCategoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ServiceFacilityCategoryModel {
		let updatedModel: ServiceFacilityCategoryModel = new ServiceFacilityCategoryModel(model);
