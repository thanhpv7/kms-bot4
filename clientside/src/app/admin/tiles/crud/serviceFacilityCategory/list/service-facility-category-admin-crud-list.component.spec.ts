...
import {AdminServiceFacilityCategoryCrudListComponent} from './service-facility-category-admin-crud-list.component';
...
import {AdminServiceFacilityCategoryCrudModule} from '../service-facility-category-admin-crud.module';
import {
 ...
 */
describe('Service Facility Category Crud List Tile Component', () => {

	let fixture;
	let serviceFacilityCategoryCrudListComponent: AdminServiceFacilityCategoryCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ServiceFacilityCategoryModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '73cf858a-b312-4609-ac90-6d3b8eeea7fe';
	const searchContent = 'text';

	const searchableFieldCount = ServiceFacilityCategoryModel.searchFields.length;

	const routerState: RouterState = {
		url: 'service-facility-category-admin-crud',
		urls: ['service-facility-category-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const serviceFacilityCategoryDataFactory = new ServiceFacilityCategoryDataFactory();

	const defaultDataSize = 30;
	const defaultData = serviceFacilityCategoryDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getServiceFacilityCategoryCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getServiceFacilityCategoryCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminServiceFacilityCategoryCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminServiceFacilityCategoryCrudListComponent);
			serviceFacilityCategoryCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(serviceFacilityCategoryCrudListComponent.serviceFacilityCategorysCount).toBe(defaultData.length);
			...
			expect(serviceFacilityCategoryCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			serviceFacilityCategoryCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = serviceFacilityCategoryDataFactory.createAll(filteredDataSize);
			...
			expect(serviceFacilityCategoryCrudListComponent.serviceFacilityCategorysCount).toBe(filteredDataSize);
			expect(serviceFacilityCategoryCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(serviceFacilityCategoryCrudListComponent.pageIndex).toBe(0);
			expect(serviceFacilityCategoryCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			serviceFacilityCategoryCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		serviceFacilityCategoryCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		serviceFacilityCategoryCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
