...
import * as modelAction from 'src/app/models/serviceFacilityCategory/service_facility_category.model.action';
import {ServiceFacilityCategoryModelState} from 'src/app/models/serviceFacilityCategory/service_facility_category.model.state';
...
import {
	...
	getServiceFacilityCategoryCollectionCount
...
import {CrudTileMode} from '../service-facility-category-admin-crud.component';
...
@Component({
	selector: 'cb-service-facility-category-admin-crud-list',
	templateUrl: './service-facility-category-admin-crud-list.component.html',
	styleUrls: [
		'./service-facility-category-admin-crud-list.component.scss',
...
export class AdminServiceFacilityCategoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ServiceFacilityCategoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ServiceFacilityCategoryModel.searchFields,
	...
	/**
	 * List of Service Facility Categorys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	serviceFacilityCategorys: Observable<ServiceFacilityCategoryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	serviceFacilityCategorysCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = ServiceFacilityCategoryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ServiceFacilityCategoryModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '73cf858a-b312-4609-ac90-6d3b8eeea7fe';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.serviceFacilityCategorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.serviceFacilityCategorysCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ServiceFacilityCategoryModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
				modelAction.ServiceFacilityCategoryModelActionTypes.INITIALISE_SERVICE_FACILITY_CATEGORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
				modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_SERVICE_FACILITY_CATEGORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ServiceFacilityCategoryAction(
						modelAction.ServiceFacilityCategoryModelActionTypes.COUNT_SERVICE_FACILITY_CATEGORYS,
						null
					)
				]
			))
		...
		this.serviceFacilityCategorys = this.store.select(getServiceFacilityCategoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getServiceFacilityCategoryCollectionCount, this.collectionId).subscribe(
				(count) => this.serviceFacilityCategorysCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('service-facility-category');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ServiceFacilityCategoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ServiceFacilityCategoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ServiceFacilityCategoryModelActionTypes.EXPORT_ALL_SERVICE_FACILITY_CATEGORY;
		} else {
			actionType = modelAction.ServiceFacilityCategoryModelActionTypes.EXPORT_SERVICE_FACILITY_CATEGORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
				modelAction.ServiceFacilityCategoryModelActionTypes.IMPORT_SERVICE_FACILITY_CATEGORYS,
				{
					file: file,
				},
				[new modelAction.ServiceFacilityCategoryAction(
			...
			window.open(environment.API_URL + '/api/servicefacilitycategory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ServiceFacilityCategoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Service Facility Category entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Service Facility Category entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ServiceFacilityCategoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ServiceFacilityCategoryModelActionTypes.DELETE_SERVICE_FACILITY_CATEGORY;
						} else if (usingSelected) {
							actionType = modelAction.ServiceFacilityCategoryModelActionTypes.DELETE_ALL_SERVICE_FACILITY_CATEGORY;
						} else {
							actionType = modelAction.ServiceFacilityCategoryModelActionTypes.DELETE_SERVICE_FACILITY_CATEGORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ServiceFacilityCategoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_SERVICE_FACILITY_CATEGORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_SERVICE_FACILITY_CATEGORY and DELETE_SERVICE_FACILITY_CATEGORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ServiceFacilityCategoryModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ServiceFacilityCategoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.serviceFacilityCategorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
