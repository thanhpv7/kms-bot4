...
import * as modelAction from 'src/app/models/bpjsPcareKunjunganRujukan/bpjs_pcare_kunjungan_rujukan.model.action';
import {BpjsPcareKunjunganRujukanModelState} from 'src/app/models/bpjsPcareKunjunganRujukan/bpjs_pcare_kunjungan_rujukan.model.state';
...
import {CrudTileMode} from '../bpjs-pcare-kunjungan-rujukan-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-pcare-kunjungan-rujukan-admin-crud-list',
	templateUrl: './bpjs-pcare-kunjungan-rujukan-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-pcare-kunjungan-rujukan-admin-crud-list.component.scss',
...
export class AdminBpjsPcareKunjunganRujukanCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPcareKunjunganRujukanModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPcareKunjunganRujukanModel.searchFields,
	...
	/**
	 * List of Bpjs Pcare Kunjungan Rujukans that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	bpjsPcareKunjunganRujukans: Observable<BpjsPcareKunjunganRujukanModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	bpjsPcareKunjunganRujukansCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = BpjsPcareKunjunganRujukanModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPcareKunjunganRujukanModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'ca991a2e-d7e6-48e4-89e2-73d070c8d241';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPcareKunjunganRujukansCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPcareKunjunganRujukansCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPcareKunjunganRujukanModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
				modelAction.BpjsPcareKunjunganRujukanModelActionTypes.INITIALISE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
				modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_BPJS_PCARE_KUNJUNGAN_RUJUKAN_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPcareKunjunganRujukanAction(
						modelAction.BpjsPcareKunjunganRujukanModelActionTypes.COUNT_BPJS_PCARE_KUNJUNGAN_RUJUKANS,
						null
					)
				]
			))
		...
		this.bpjsPcareKunjunganRujukans = this.store.select(getBpjsPcareKunjunganRujukanCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPcareKunjunganRujukanCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPcareKunjunganRujukansCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-pcare-kunjungan-rujukan');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPcareKunjunganRujukanModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPcareKunjunganRujukanModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPcareKunjunganRujukanModelActionTypes.EXPORT_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN;
		} else {
			actionType = modelAction.BpjsPcareKunjunganRujukanModelActionTypes.EXPORT_BPJS_PCARE_KUNJUNGAN_RUJUKAN_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
				modelAction.BpjsPcareKunjunganRujukanModelActionTypes.IMPORT_BPJS_PCARE_KUNJUNGAN_RUJUKANS,
				{
					file: file,
				},
				[new modelAction.BpjsPcareKunjunganRujukanAction(
			...
			window.open(environment.API_URL + '/api/bpjspcarekunjunganrujukan/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPcareKunjunganRujukanModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Bpjs Pcare Kunjungan Rujukan entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Bpjs Pcare Kunjungan Rujukan entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPcareKunjunganRujukanModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPcareKunjunganRujukanModelActionTypes.DELETE_BPJS_PCARE_KUNJUNGAN_RUJUKAN;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPcareKunjunganRujukanModelActionTypes.DELETE_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN;
						} else {
							actionType = modelAction.BpjsPcareKunjunganRujukanModelActionTypes.DELETE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PCARE_KUNJUNGAN_RUJUKAN when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN and DELETE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPcareKunjunganRujukanModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPcareKunjunganRujukanModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPcareKunjunganRujukansCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
