...
import {BpjsPcareKunjunganRujukanModelState, BpjsPcareKunjunganRujukanModelAudit} from 'src/app/models/bpjsPcareKunjunganRujukan/bpjs_pcare_kunjungan_rujukan.model.state';
import {BpjsPcareKunjunganRujukanModel} from 'src/app/models/bpjsPcareKunjunganRujukan/bpjs_pcare_kunjungan_rujukan.model';
import {
	getBpjsPcareKunjunganRujukanModelAuditsByEntityId,
...
@Component({
	selector: 'cb-bpjs-pcare-kunjungan-rujukan-admin-crud-edit',
	templateUrl: './bpjs-pcare-kunjungan-rujukan-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-pcare-kunjungan-rujukan-admin-crud-edit.component.scss',
...
export class AdminBpjsPcareKunjunganRujukanCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPcareKunjunganRujukanAudits$: Observable<BpjsPcareKunjunganRujukanModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPcareKunjunganRujukanModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPcareKunjunganRujukanModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPcareKunjunganRujukanModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPcareKunjunganRujukanModel();
		...
		if (this.targetModelId) {
			this.bpjsPcareKunjunganRujukanAudits$ = this.store.select(getBpjsPcareKunjunganRujukanModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPcareKunjunganRujukanModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
			modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_BPJS_PCARE_KUNJUNGAN_RUJUKAN,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareKunjunganRujukanModelWithId, this.targetModelId)
			...
			model = new BpjsPcareKunjunganRujukanModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
			modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_BPJS_PCARE_KUNJUNGAN_RUJUKAN_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
				modelAction.BpjsPcareKunjunganRujukanModelActionTypes.CREATE_BPJS_PCARE_KUNJUNGAN_RUJUKAN,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPcareKunjunganRujukanAction(
				modelAction.BpjsPcareKunjunganRujukanModelActionTypes.UPDATE_BPJS_PCARE_KUNJUNGAN_RUJUKAN,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPcareKunjunganRujukanModel = new BpjsPcareKunjunganRujukanModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPcareKunjunganRujukanModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPcareKunjunganRujukanModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPcareKunjunganRujukanModel.getProps();
		this.modelRelations = BpjsPcareKunjunganRujukanModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPcareKunjunganRujukanModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPcareKunjunganRujukanModel {
		let updatedModel: BpjsPcareKunjunganRujukanModel = new BpjsPcareKunjunganRujukanModel(model);
