...
import {BpjsPRBDetailModelState, BpjsPRBDetailModelAudit} from 'src/app/models/bpjsPRBDetail/bpjs_prb_detail.model.state';
import {BpjsPRBDetailModel} from 'src/app/models/bpjsPRBDetail/bpjs_prb_detail.model';
import {
	getBpjsPRBDetailModelAuditsByEntityId,
	getBpjsPRBDetailModelWithId,
	getCountBpjsPRBDetailModels,
...
@Component({
	selector: 'cb-bpjs-prb-detail-admin-crud-edit',
	templateUrl: './bpjs-prb-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-prb-detail-admin-crud-edit.component.scss',
...
export class AdminBpjsPRBDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsPRBDetailAudits$: Observable<BpjsPRBDetailModelAudit[]>;
	...
	registrationOppositeRelation = 'bpjsPRBDetail';
	...
	bpjsDrugGenericPRBOppositeRelation = 'bpjsPRBDetails';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsPRBDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsPRBDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsPRBDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsPRBDetailModel();
		...
		if (this.targetModelId) {
			this.bpjsPRBDetailAudits$ = this.store.select(getBpjsPRBDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsPRBDetailModel> = {
		...
		this.store.dispatch(new modelAction.BpjsPRBDetailAction(
			modelAction.BpjsPRBDetailModelActionTypes.FETCH_BPJS_PRB_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsPRBDetailModelWithId, this.targetModelId)
			...
			model = new BpjsPRBDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsPRBDetailAction(
			modelAction.BpjsPRBDetailModelActionTypes.FETCH_BPJS_PRB_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<BpjsPRBDetailModel> = {
				...
				queryParams: {
					persistRelations: this.registrationMultiCrudActive || this.bpjsDrugGenericPRBMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.BpjsPRBDetailAction(
				modelAction.BpjsPRBDetailModelActionTypes.CREATE_BPJS_PRB_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsPRBDetailAction(
				modelAction.BpjsPRBDetailModelActionTypes.UPDATE_BPJS_PRB_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsPRBDetailModel = new BpjsPRBDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsPRBDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsPRBDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsPRBDetailModel.getProps();
		this.modelRelations = BpjsPRBDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsPRBDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsPRBDetailModel {
		let updatedModel: BpjsPRBDetailModel = new BpjsPRBDetailModel(model);
