...
import * as modelAction from 'src/app/models/bpjsPRBDetail/bpjs_prb_detail.model.action';
import {BpjsPRBDetailModelState} from 'src/app/models/bpjsPRBDetail/bpjs_prb_detail.model.state';
...
import {
	getBpjsPRBDetailCollectionModels,
	getBpjsPRBDetailCollectionCount
...
import {CrudTileMode} from '../bpjs-prb-detail-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-prb-detail-admin-crud-list',
	templateUrl: './bpjs-prb-detail-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-prb-detail-admin-crud-list.component.scss',
...
export class AdminBpjsPRBDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsPRBDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsPRBDetailModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsPRBDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'de2a3337-0a3c-415b-a719-314451c317a2';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsPRBDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsPRBDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsPRBDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPRBDetailAction(
				modelAction.BpjsPRBDetailModelActionTypes.INITIALISE_BPJS_PRB_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsPRBDetailAction(
				modelAction.BpjsPRBDetailModelActionTypes.FETCH_BPJS_PRB_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsPRBDetailAction(
						modelAction.BpjsPRBDetailModelActionTypes.COUNT_BPJS_PRB_DETAILS,
						null
					)
				]
			))
		...
		this.bpjsPRBDetails = this.store.select(getBpjsPRBDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsPRBDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsPRBDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsPRBDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsPRBDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsPRBDetailModelActionTypes.EXPORT_ALL_BPJS_PRB_DETAIL;
		} else {
			actionType = modelAction.BpjsPRBDetailModelActionTypes.EXPORT_BPJS_PRB_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsPRBDetailAction(
				modelAction.BpjsPRBDetailModelActionTypes.IMPORT_BPJS_PRB_DETAILS,
				{
					file: file,
				},
				[new modelAction.BpjsPRBDetailAction(
			...
			window.open(environment.API_URL + '/api/bpjsprbdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsPRBDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS PRB Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS PRB Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsPRBDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsPRBDetailModelActionTypes.DELETE_BPJS_PRB_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.BpjsPRBDetailModelActionTypes.DELETE_ALL_BPJS_PRB_DETAIL;
						} else {
							actionType = modelAction.BpjsPRBDetailModelActionTypes.DELETE_BPJS_PRB_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsPRBDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_PRB_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_PRB_DETAIL and DELETE_BPJS_PRB_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsPRBDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsPRBDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsPRBDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
