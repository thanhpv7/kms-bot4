...
import {AdminBpjsPRBDetailCrudListComponent} from './bpjs-prb-detail-admin-crud-list.component';
...
import {AdminBpjsPRBDetailCrudModule} from '../bpjs-prb-detail-admin-crud.module';
import {
 ...
 */
describe('BPJS PRB Detail Crud List Tile Component', () => {

	let fixture;
	let bpjsPRBDetailCrudListComponent: AdminBpjsPRBDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsPRBDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'de2a3337-0a3c-415b-a719-314451c317a2';
	const searchContent = 'text';

	const searchableFieldCount = BpjsPRBDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-prb-detail-admin-crud',
		urls: ['bpjs-prb-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsPRBDetailDataFactory = new BpjsPRBDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsPRBDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getBpjsPRBDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsPRBDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminBpjsPRBDetailCrudListComponent);
			bpjsPRBDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsPRBDetailCrudListComponent.bpjsPRBDetailsCount).toBe(defaultData.length);
			...
			expect(bpjsPRBDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsPRBDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsPRBDetailDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsPRBDetailCrudListComponent.bpjsPRBDetailsCount).toBe(filteredDataSize);
			expect(bpjsPRBDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsPRBDetailCrudListComponent.pageIndex).toBe(0);
			expect(bpjsPRBDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsPRBDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsPRBDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsPRBDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
