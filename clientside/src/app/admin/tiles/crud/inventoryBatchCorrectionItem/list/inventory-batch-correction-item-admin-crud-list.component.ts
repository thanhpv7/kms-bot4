...
import {InventoryBatchCorrectionItemModelState} from 'src/app/models/inventoryBatchCorrectionItem/inventory_batch_correction_item.model.state';
...
import {
	getInventoryBatchCorrectionItemCollectionModels,
	getInventoryBatchCorrectionItemCollectionCount
...
@Component({
	selector: 'cb-inventory-batch-correction-item-admin-crud-list',
	templateUrl: './inventory-batch-correction-item-admin-crud-list.component.html',
	styleUrls: [
		'./inventory-batch-correction-item-admin-crud-list.component.scss',
...
export class AdminInventoryBatchCorrectionItemCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InventoryBatchCorrectionItemModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InventoryBatchCorrectionItemModel.searchFields,
	...
	/**
	 * List of Inventory Batch Correction Items that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	inventoryBatchCorrectionItems: Observable<InventoryBatchCorrectionItemModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	inventoryBatchCorrectionItemsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.inventoryBatchCorrectionItemsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.inventoryBatchCorrectionItemsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
				modelAction.InventoryBatchCorrectionItemModelActionTypes.INITIALISE_INVENTORY_BATCH_CORRECTION_ITEM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
				modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_ITEM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.inventoryBatchCorrectionItems = this.store.select(getInventoryBatchCorrectionItemCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInventoryBatchCorrectionItemCollectionCount, this.collectionId).subscribe(
				(count) => this.inventoryBatchCorrectionItemsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InventoryBatchCorrectionItemModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InventoryBatchCorrectionItemModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InventoryBatchCorrectionItemModelActionTypes.EXPORT_ALL_INVENTORY_BATCH_CORRECTION_ITEM;
		} else {
			actionType = modelAction.InventoryBatchCorrectionItemModelActionTypes.EXPORT_INVENTORY_BATCH_CORRECTION_ITEM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
				modelAction.InventoryBatchCorrectionItemModelActionTypes.IMPORT_INVENTORY_BATCH_CORRECTION_ITEMS,
				{
					file: file,
				},
				[new modelAction.InventoryBatchCorrectionItemAction(
			...
			window.open(environment.API_URL + '/api/inventorybatchcorrectionitem/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InventoryBatchCorrectionItemModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Inventory Batch Correction Item entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Inventory Batch Correction Item entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InventoryBatchCorrectionItemModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InventoryBatchCorrectionItemModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION_ITEM;
						} else if (usingSelected) {
							actionType = modelAction.InventoryBatchCorrectionItemModelActionTypes.DELETE_ALL_INVENTORY_BATCH_CORRECTION_ITEM;
						} else {
							actionType = modelAction.InventoryBatchCorrectionItemModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION_ITEM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INVENTORY_BATCH_CORRECTION_ITEM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INVENTORY_BATCH_CORRECTION_ITEM and DELETE_INVENTORY_BATCH_CORRECTION_ITEM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InventoryBatchCorrectionItemModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.inventoryBatchCorrectionItemsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
