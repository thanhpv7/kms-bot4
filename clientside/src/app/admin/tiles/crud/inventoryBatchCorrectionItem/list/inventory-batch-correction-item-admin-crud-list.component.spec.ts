...
import {AdminInventoryBatchCorrectionItemCrudListComponent} from './inventory-batch-correction-item-admin-crud-list.component';
...
import {AdminInventoryBatchCorrectionItemCrudModule} from '../inventory-batch-correction-item-admin-crud.module';
import {
	InventoryBatchCorrectionItemModelState,
	initialState as initialInventoryBatchCorrectionItemModelState
} from 'src/app/models/inventoryBatchCorrectionItem/inventory_batch_correction_item.model.state';
 ...
 */
describe('Inventory Batch Correction Item Crud List Tile Component', () => {

	let fixture;
	let inventoryBatchCorrectionItemCrudListComponent: AdminInventoryBatchCorrectionItemCrudListComponent;

	let store: MockStore<{ model: InventoryBatchCorrectionItemModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let inventoryBatchCorrectionItemModelState: InventoryBatchCorrectionItemModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<InventoryBatchCorrectionItemModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'de1db622-d3a3-4660-b8d1-558ce925c7bb';
	const searchContent = 'text';

	const searchableFieldCount = InventoryBatchCorrectionItemModel.searchFields.length;

	const routerState: RouterState = {
		url: 'inventory-batch-correction-item-admin-crud',
		urls: ['inventory-batch-correction-item-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const inventoryBatchCorrectionItemDataFactory = new InventoryBatchCorrectionItemDataFactory();

	const defaultDataSize = 30;
	const defaultData = inventoryBatchCorrectionItemDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: InventoryBatchCorrectionItemModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		inventoryBatchCorrectionItemModelState = initialInventoryBatchCorrectionItemModelState;
		store.setState({model: inventoryBatchCorrectionItemModelState});
		...
		spyOn(store, 'select')
			.withArgs(getInventoryBatchCorrectionItemCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getInventoryBatchCorrectionItemCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminInventoryBatchCorrectionItemCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminInventoryBatchCorrectionItemCrudListComponent);
			inventoryBatchCorrectionItemCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(inventoryBatchCorrectionItemCrudListComponent.inventoryBatchCorrectionItemsCount).toBe(defaultData.length);
			...
			expect(inventoryBatchCorrectionItemCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			inventoryBatchCorrectionItemCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = inventoryBatchCorrectionItemDataFactory.createAll(filteredDataSize);
			...
			expect(inventoryBatchCorrectionItemCrudListComponent.inventoryBatchCorrectionItemsCount).toBe(filteredDataSize);
			expect(inventoryBatchCorrectionItemCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(inventoryBatchCorrectionItemCrudListComponent.pageIndex).toBe(0);
			expect(inventoryBatchCorrectionItemCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			inventoryBatchCorrectionItemCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		inventoryBatchCorrectionItemCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		inventoryBatchCorrectionItemCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
