...
import * as modelAction from 'src/app/models/inventoryBatchCorrectionItem/inventory_batch_correction_item.model.action';
import {InventoryBatchCorrectionItemModelState, InventoryBatchCorrectionItemModelAudit} from 'src/app/models/inventoryBatchCorrectionItem/inventory_batch_correction_item.model.state';
...
import {
	getInventoryBatchCorrectionItemModelAuditsByEntityId,
	getInventoryBatchCorrectionItemModelWithId,
	getCountInventoryBatchCorrectionItemModels,
...
import {CrudTileMode} from '../inventory-batch-correction-item-admin-crud.component';
...
@Component({
	selector: 'cb-inventory-batch-correction-item-admin-crud-edit',
	templateUrl: './inventory-batch-correction-item-admin-crud-edit.component.html',
	styleUrls: [
		'./inventory-batch-correction-item-admin-crud-edit.component.scss',
...
export class AdminInventoryBatchCorrectionItemCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'de1db622-d3a3-4660-b8d1-558ce925c7bb';
	 ...
	 */
	inventoryBatchCorrectionItemAudits$: Observable<InventoryBatchCorrectionItemModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Inventory Batch Correction and Inventory Batch Correction Items here] off begin
	inventoryBatchCorrectionMultiCrudActive: boolean = false;
	inventoryBatchCorrectionDisplayName = 'Inventory Batch Correction';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Inventory Batch Correction and Inventory Batch Correction Items here] end
	inventoryBatchCorrectionOppositeRelation = 'inventoryBatchCorrectionItems';
	inventoryBatchCorrectionRelations: ModelRelation[] = this.inventoryBatchCorrectionMultiCrudActive ? InventoryBatchCorrectionModel.getRelations()
		.map(relation => {
			if (relation.id === this.inventoryBatchCorrectionOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'inventoryBatchCorrectionItems';
	 ...
	 */
	modelProperties: ModelProperty[] = InventoryBatchCorrectionItemModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InventoryBatchCorrectionItemModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InventoryBatchCorrectionItemModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'inventoryBatchCorrection' && this.inventoryBatchCorrectionMultiCrudActive) {
	 ...
	 */
	targetModel: InventoryBatchCorrectionItemModel;
	...
	constructor(
		private readonly store: Store<{ model: InventoryBatchCorrectionItemModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInventoryBatchCorrectionItemModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InventoryBatchCorrectionItemModel();
		...
		if (this.targetModelId) {
			this.inventoryBatchCorrectionItemAudits$ = this.store.select(getInventoryBatchCorrectionItemModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InventoryBatchCorrectionItemModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
			modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_ITEM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInventoryBatchCorrectionItemModelWithId, this.targetModelId)
			...
			model = new InventoryBatchCorrectionItemModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('inventory-batch-correction-item');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
			modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_ITEM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InventoryBatchCorrectionItemModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.inventoryBatchCorrectionMultiCrudActive || this.stockCatalogueMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InventoryBatchCorrectionItemAction(
						modelAction.InventoryBatchCorrectionItemModelActionTypes.COUNT_INVENTORY_BATCH_CORRECTION_ITEMS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
				modelAction.InventoryBatchCorrectionItemModelActionTypes.CREATE_INVENTORY_BATCH_CORRECTION_ITEM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InventoryBatchCorrectionItemAction(
				modelAction.InventoryBatchCorrectionItemModelActionTypes.UPDATE_INVENTORY_BATCH_CORRECTION_ITEM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InventoryBatchCorrectionItemModel = new InventoryBatchCorrectionItemModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInventoryBatchCorrectionItemModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InventoryBatchCorrectionItemModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.inventoryBatchCorrectionDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InventoryBatchCorrectionModel.getProps(), this.inventoryBatchCorrectionRelations, this.isViewOnly),
					new InventoryBatchCorrectionModel(),
					InventoryBatchCorrectionModel,
					this.inventoryBatchCorrectionDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.inventoryBatchCorrectionRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InventoryBatchCorrectionItemModel.getProps();
		this.modelRelations = InventoryBatchCorrectionItemModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'inventoryBatchCorrection';
		}).hideElement = this.inventoryBatchCorrectionMultiCrudActive || this.modelRelations.find(rel => rel.id === 'inventoryBatchCorrection').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.inventoryBatchCorrectionMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.inventoryBatchCorrectionDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.inventoryBatchCorrection && this.targetModel.inventoryBatchCorrection.id && this.inventoryBatchCorrectionMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InventoryBatchCorrectionModel,
				this.inventoryBatchCorrectionOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.inventoryBatchCorrection.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getInventoryBatchCorrectionModelWithId, this.targetModel.inventoryBatchCorrectionId).subscribe(inventoryBatchCorrection => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InventoryBatchCorrectionModel.getProps(), this.inventoryBatchCorrectionRelations, this.isViewOnly),
	 ...
	 */
	addAdditionalEntitiesToModel(model: InventoryBatchCorrectionItemModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InventoryBatchCorrectionItemModel {
		let updatedModel: InventoryBatchCorrectionItemModel = new InventoryBatchCorrectionItemModel(model);

		if (this.inventoryBatchCorrectionMultiCrudActive) {
			updatedModel.inventoryBatchCorrection = event.additionalEntities.inventoryBatchCorrection ? event.additionalEntities.inventoryBatchCorrection : undefined;

			if (!updatedModel.inventoryBatchCorrection && event.payload) {
				event.payload.inventoryBatchCorrectionId = null;
			}
		} else if (event.payload?.inventoryBatchCorrectionId !== undefined) {
			event.payload.inventoryBatchCorrection = null;
			updatedModel.inventoryBatchCorrection = null;
		}
