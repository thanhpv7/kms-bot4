...
import {AdminPatientConsentCrudListComponent} from './patient-consent-admin-crud-list.component';
...
import {AdminPatientConsentCrudModule} from '../patient-consent-admin-crud.module';
import {
	PatientConsentModelState,
	initialState as initialPatientConsentModelState
} from 'src/app/models/patientConsent/patient_consent.model.state';
 ...
 */
describe('Patient Consent Crud List Tile Component', () => {

	let fixture;
	let patientConsentCrudListComponent: AdminPatientConsentCrudListComponent;

	let store: MockStore<{ model: PatientConsentModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientConsentModelState: PatientConsentModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientConsentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f2bf1397-86b6-4249-8d78-35ea013f7330';
	const searchContent = 'text';

	const searchableFieldCount = PatientConsentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-consent-admin-crud',
		urls: ['patient-consent-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientConsentDataFactory = new PatientConsentDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientConsentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientConsentModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientConsentModelState = initialPatientConsentModelState;
		store.setState({model: patientConsentModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientConsentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientConsentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPatientConsentCrudListComponent);
			patientConsentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientConsentCrudListComponent.patientConsentsCount).toBe(defaultData.length);
			...
			expect(patientConsentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientConsentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientConsentDataFactory.createAll(filteredDataSize);
			...
			expect(patientConsentCrudListComponent.patientConsentsCount).toBe(filteredDataSize);
			expect(patientConsentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientConsentCrudListComponent.pageIndex).toBe(0);
			expect(patientConsentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientConsentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientConsentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientConsentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
