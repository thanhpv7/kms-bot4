...
import * as modelAction from 'src/app/models/patientConsent/patient_consent.model.action';
import {PatientConsentModelState} from 'src/app/models/patientConsent/patient_consent.model.state';
...
import {
	getPatientConsentCollectionModels,
	getPatientConsentCollectionCount
...
import {CrudTileMode} from '../patient-consent-admin-crud.component';
...
@Component({
	selector: 'cb-patient-consent-admin-crud-list',
	templateUrl: './patient-consent-admin-crud-list.component.html',
	styleUrls: [
		'./patient-consent-admin-crud-list.component.scss',
...
export class AdminPatientConsentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientConsentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientConsentModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PatientConsentModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'f2bf1397-86b6-4249-8d78-35ea013f7330';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientConsentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientConsentsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PatientConsentModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientConsentAction(
				modelAction.PatientConsentModelActionTypes.INITIALISE_PATIENT_CONSENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientConsentAction(
				modelAction.PatientConsentModelActionTypes.FETCH_PATIENT_CONSENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PatientConsentAction(
						modelAction.PatientConsentModelActionTypes.COUNT_PATIENT_CONSENTS,
						null
					)
				]
			))
		...
		this.patientConsents = this.store.select(getPatientConsentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientConsentCollectionCount, this.collectionId).subscribe(
				(count) => this.patientConsentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientConsentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientConsentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientConsentModelActionTypes.EXPORT_ALL_PATIENT_CONSENT;
		} else {
			actionType = modelAction.PatientConsentModelActionTypes.EXPORT_PATIENT_CONSENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientConsentAction(
				modelAction.PatientConsentModelActionTypes.IMPORT_PATIENT_CONSENTS,
				{
					file: file,
				},
				[new modelAction.PatientConsentAction(
			...
			window.open(environment.API_URL + '/api/patientconsent/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientConsentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Consent entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Consent entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientConsentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientConsentModelActionTypes.DELETE_PATIENT_CONSENT;
						} else if (usingSelected) {
							actionType = modelAction.PatientConsentModelActionTypes.DELETE_ALL_PATIENT_CONSENT;
						} else {
							actionType = modelAction.PatientConsentModelActionTypes.DELETE_PATIENT_CONSENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientConsentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_CONSENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_CONSENT and DELETE_PATIENT_CONSENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PatientConsentModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientConsentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientConsentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
