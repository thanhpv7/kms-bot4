...
import {PatientConsentModelState, PatientConsentModelAudit} from 'src/app/models/patientConsent/patient_consent.model.state';
import {PatientConsentModel} from 'src/app/models/patientConsent/patient_consent.model';
import {
	getPatientConsentModelAuditsByEntityId,
	getPatientConsentModelWithId,
	getCountPatientConsentModels,
...
@Component({
	selector: 'cb-patient-consent-admin-crud-edit',
	templateUrl: './patient-consent-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-consent-admin-crud-edit.component.scss',
...
export class AdminPatientConsentCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	patientConsentAudits$: Observable<PatientConsentModelAudit[]>;
	...
	patientGeneralInfoOppositeRelation = 'patientConsents';
	...
	staffOppositeRelation = 'patientConsents';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientConsentModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientConsentModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientConsentModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientConsentModel();
		...
		if (this.targetModelId) {
			this.patientConsentAudits$ = this.store.select(getPatientConsentModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientConsentModel> = {
		...
		this.store.dispatch(new modelAction.PatientConsentAction(
			modelAction.PatientConsentModelActionTypes.FETCH_PATIENT_CONSENT,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientConsentModelWithId, this.targetModelId)
			...
			model = new PatientConsentModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientConsentAction(
			modelAction.PatientConsentModelActionTypes.FETCH_PATIENT_CONSENT_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<PatientConsentModel> = {
				...
				queryParams: {
					persistRelations: this.patientGeneralInfoMultiCrudActive || this.staffMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.PatientConsentAction(
				modelAction.PatientConsentModelActionTypes.CREATE_PATIENT_CONSENT,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientConsentAction(
				modelAction.PatientConsentModelActionTypes.UPDATE_PATIENT_CONSENT,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientConsentModel = new PatientConsentModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientConsentModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientConsentModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientConsentModel.getProps();
		this.modelRelations = PatientConsentModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientConsentModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientConsentModel {
		let updatedModel: PatientConsentModel = new PatientConsentModel(model);
