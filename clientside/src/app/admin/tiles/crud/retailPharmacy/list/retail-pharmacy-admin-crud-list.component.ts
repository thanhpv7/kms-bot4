...
import {RetailPharmacyModelState} from 'src/app/models/retailPharmacy/retail_pharmacy.model.state';
...
import {
	...
	getRetailPharmacyCollectionCount
...
@Component({
	selector: 'cb-retail-pharmacy-admin-crud-list',
	templateUrl: './retail-pharmacy-admin-crud-list.component.html',
	styleUrls: [
		'./retail-pharmacy-admin-crud-list.component.scss',
...
export class AdminRetailPharmacyCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<RetailPharmacyModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...RetailPharmacyModel.searchFields,
	...
	/**
	 * List of Retail Pharmacys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	retailPharmacys: Observable<RetailPharmacyModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	retailPharmacysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.retailPharmacysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.retailPharmacysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RetailPharmacyAction(
				modelAction.RetailPharmacyModelActionTypes.INITIALISE_RETAIL_PHARMACY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RetailPharmacyAction(
				modelAction.RetailPharmacyModelActionTypes.FETCH_RETAIL_PHARMACY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.retailPharmacys = this.store.select(getRetailPharmacyCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getRetailPharmacyCollectionCount, this.collectionId).subscribe(
				(count) => this.retailPharmacysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: RetailPharmacyModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.RetailPharmacyModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.RetailPharmacyModelActionTypes.EXPORT_ALL_RETAIL_PHARMACY;
		} else {
			actionType = modelAction.RetailPharmacyModelActionTypes.EXPORT_RETAIL_PHARMACY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.RetailPharmacyAction(
				modelAction.RetailPharmacyModelActionTypes.IMPORT_RETAIL_PHARMACYS,
				{
					file: file,
				},
				[new modelAction.RetailPharmacyAction(
			...
			window.open(environment.API_URL + '/api/retailpharmacy/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: RetailPharmacyModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Retail Pharmacy entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Retail Pharmacy entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.RetailPharmacyModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.RetailPharmacyModelActionTypes.DELETE_RETAIL_PHARMACY;
						} else if (usingSelected) {
							actionType = modelAction.RetailPharmacyModelActionTypes.DELETE_ALL_RETAIL_PHARMACY;
						} else {
							actionType = modelAction.RetailPharmacyModelActionTypes.DELETE_RETAIL_PHARMACY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.RetailPharmacyAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_RETAIL_PHARMACY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_RETAIL_PHARMACY and DELETE_RETAIL_PHARMACY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = RetailPharmacyModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.retailPharmacysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
