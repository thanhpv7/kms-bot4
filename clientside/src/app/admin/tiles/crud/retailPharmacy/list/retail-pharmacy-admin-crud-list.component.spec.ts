...
import {AdminRetailPharmacyCrudListComponent} from './retail-pharmacy-admin-crud-list.component';
...
import {AdminRetailPharmacyCrudModule} from '../retail-pharmacy-admin-crud.module';
import {
 ...
 */
describe('Retail Pharmacy Crud List Tile Component', () => {

	let fixture;
	let retailPharmacyCrudListComponent: AdminRetailPharmacyCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<RetailPharmacyModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '46b159da-8629-4c49-b6fc-9cef67b2dbca';
	const searchContent = 'text';

	const searchableFieldCount = RetailPharmacyModel.searchFields.length;

	const routerState: RouterState = {
		url: 'retail-pharmacy-admin-crud',
		urls: ['retail-pharmacy-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const retailPharmacyDataFactory = new RetailPharmacyDataFactory();

	const defaultDataSize = 30;
	const defaultData = retailPharmacyDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getRetailPharmacyCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getRetailPharmacyCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminRetailPharmacyCrudListComponent);
			retailPharmacyCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(retailPharmacyCrudListComponent.retailPharmacysCount).toBe(defaultData.length);
			...
			expect(retailPharmacyCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			retailPharmacyCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = retailPharmacyDataFactory.createAll(filteredDataSize);
			...
			expect(retailPharmacyCrudListComponent.retailPharmacysCount).toBe(filteredDataSize);
			expect(retailPharmacyCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(retailPharmacyCrudListComponent.pageIndex).toBe(0);
			expect(retailPharmacyCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			retailPharmacyCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		retailPharmacyCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		retailPharmacyCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
