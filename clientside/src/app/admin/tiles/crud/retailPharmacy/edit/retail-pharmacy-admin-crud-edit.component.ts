...
import * as modelAction from 'src/app/models/retailPharmacy/retail_pharmacy.model.action';
import {RetailPharmacyModelState, RetailPharmacyModelAudit} from 'src/app/models/retailPharmacy/retail_pharmacy.model.state';
import {RetailPharmacyModel} from 'src/app/models/retailPharmacy/retail_pharmacy.model';
import {
	getRetailPharmacyModelAuditsByEntityId,
...
import {CrudTileMode} from '../retail-pharmacy-admin-crud.component';
...
@Component({
	selector: 'cb-retail-pharmacy-admin-crud-edit',
	templateUrl: './retail-pharmacy-admin-crud-edit.component.html',
	styleUrls: [
		'./retail-pharmacy-admin-crud-edit.component.scss',
...
export class AdminRetailPharmacyCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '46b159da-8629-4c49-b6fc-9cef67b2dbca';
	 ...
	 */
	retailPharmacyAudits$: Observable<RetailPharmacyModelAudit[]>;
	...
	patientGeneralInfoOppositeRelation = 'retailPharmacies';
	...
	warehouseOppositeRelation = 'retailPharmacies';
	...
	retailPharmacyInvoicesMultiCrudActive: boolean = false;
	retailPharmacyInvoicesDisplayName = 'Retail Pharmacy Invoices';
	retailPharmacyInvoicesCollectionId: string = uuidv4();
	...
	retailPharmacyInvoicesOppositeRelation = 'retailPharmacy';
	retailPharmacyInvoicesRelations: ModelRelation[] = this.retailPharmacyInvoicesMultiCrudActive ? RetailPharmacyInvoiceModel.getRelations()
		.map(relation => {
			if (relation.id === this.retailPharmacyInvoicesOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];

	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Retail Pharmacy and Retail Pharmacy Stock Details here] off begin
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Retail Pharmacy and Retail Pharmacy Stock Details here] end
	retailPharmacyStockDetailsOppositeRelation = 'retailPharmacy';
	 ...
	 */
	modelProperties: ModelProperty[] = RetailPharmacyModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = RetailPharmacyModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = RetailPharmacyModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'retailPharmacyInvoices' && this.retailPharmacyInvoicesMultiCrudActive) {
	 ...
	 */
	targetModel: RetailPharmacyModel;
	...
	constructor(
		private readonly store: Store<{ model: RetailPharmacyModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountRetailPharmacyModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new RetailPharmacyModel();
		...
		if (this.targetModelId) {
			this.retailPharmacyAudits$ = this.store.select(getRetailPharmacyModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<RetailPharmacyModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.RetailPharmacyAction(
			modelAction.RetailPharmacyModelActionTypes.FETCH_RETAIL_PHARMACY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getRetailPharmacyModelWithId, this.targetModelId)
			...
			model = new RetailPharmacyModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.RetailPharmacyAction(
			modelAction.RetailPharmacyModelActionTypes.FETCH_RETAIL_PHARMACY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<RetailPharmacyModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.patientGeneralInfoMultiCrudActive || this.warehouseMultiCrudActive || this.retailPharmacyInvoicesMultiCrudActive || this.retailPharmacyStockDetailsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.RetailPharmacyAction(
						modelAction.RetailPharmacyModelActionTypes.COUNT_RETAIL_PHARMACYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.RetailPharmacyAction(
				modelAction.RetailPharmacyModelActionTypes.CREATE_RETAIL_PHARMACY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.RetailPharmacyAction(
				modelAction.RetailPharmacyModelActionTypes.UPDATE_RETAIL_PHARMACY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: RetailPharmacyModel = new RetailPharmacyModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getRetailPharmacyModelWithId, this.targetModelId).subscribe(model => {
					let differences = new RetailPharmacyModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.retailPharmacyInvoicesDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(RetailPharmacyInvoiceModel.getProps(), this.retailPharmacyInvoicesRelations, this.isViewOnly),
					new RetailPharmacyInvoiceModel(),
					RetailPharmacyInvoiceModel,
					this.retailPharmacyInvoicesDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.retailPharmacyInvoicesRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = RetailPharmacyModel.getProps();
		this.modelRelations = RetailPharmacyModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'retailPharmacyInvoices';
		}).hideElement = this.retailPharmacyInvoicesMultiCrudActive || this.modelRelations.find(rel => rel.id === 'retailPharmacyInvoices').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.retailPharmacyInvoicesMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.retailPharmacyInvoicesDisplayName, many: true});
			this.store.dispatch(new retailPharmacyInvoiceModelAction.RetailPharmacyInvoiceAction(
				retailPharmacyInvoiceModelAction.RetailPharmacyInvoiceModelActionTypes.INITIALISE_RETAIL_PHARMACY_INVOICE_COLLECTION_STATE,
				{
					collectionId: this.retailPharmacyInvoicesCollectionId
				},
			));

			this.creationNavigationSubscription$.add(this.store.select(getRetailPharmacyInvoiceCollectionModels, this.retailPharmacyInvoicesCollectionId).subscribe(models => {
				models.forEach(retailPharmacyInvoices => {
					let updatedProps = getNewModelProperties(
						createReactiveFormFromModel(RetailPharmacyInvoiceModel.getProps(), this.retailPharmacyInvoicesRelations, this.isViewOnly),
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.retailPharmacyInvoicesIds && this.targetModel.retailPharmacyInvoicesIds.length > 0 && this.retailPharmacyInvoicesMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				RetailPharmacyInvoiceModel,
				this.retailPharmacyInvoicesOppositeRelation,
				this.retailPharmacyInvoicesCollectionId,
				this.targetModel.retailPharmacyInvoicesIds.length,
				this.targetModel.id,
				true
			));
		}
	 ...
	 */
	addAdditionalEntitiesToModel(model: RetailPharmacyModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): RetailPharmacyModel {
		let updatedModel: RetailPharmacyModel = new RetailPharmacyModel(model);
		...
		updatedModel.retailPharmacyInvoices = [];
		if (this.retailPharmacyInvoicesMultiCrudActive) {
			if (event.additionalEntities.retailPharmacyInvoices) {
				event.additionalEntities.retailPharmacyInvoices.forEach(entity => {
					updatedModel.retailPharmacyInvoices.push(entity);
				});
			}

			if (!updatedModel.retailPharmacyInvoices.length) {
				updatedModel.retailPharmacyInvoicesIds = [];
			}
		}
