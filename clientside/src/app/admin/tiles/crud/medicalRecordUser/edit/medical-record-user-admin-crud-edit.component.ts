...
import {MedicalRecordUserModelState, MedicalRecordUserModelAudit} from 'src/app/models/medicalRecordUser/medical_record_user.model.state';
import {MedicalRecordUserModel} from 'src/app/models/medicalRecordUser/medical_record_user.model';
import {
	getMedicalRecordUserModelAuditsByEntityId,
	getMedicalRecordUserModelWithId,
	getCountMedicalRecordUserModels,
...
@Component({
	selector: 'cb-medical-record-user-admin-crud-edit',
	templateUrl: './medical-record-user-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-record-user-admin-crud-edit.component.scss',
...
export class AdminMedicalRecordUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	medicalRecordUserAudits$: Observable<MedicalRecordUserModelAudit[]>;
	...
	staffOppositeRelation = 'medicalRecordUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalRecordUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: MedicalRecordUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalRecordUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalRecordUserModel();
		...
		if (this.targetModelId) {
			this.medicalRecordUserAudits$ = this.store.select(getMedicalRecordUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalRecordUserModel> = {
		...
		this.store.dispatch(new modelAction.MedicalRecordUserAction(
			modelAction.MedicalRecordUserModelActionTypes.FETCH_MEDICAL_RECORD_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalRecordUserModelWithId, this.targetModelId)
			...
			model = new MedicalRecordUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalRecordUserAction(
			modelAction.MedicalRecordUserModelActionTypes.FETCH_MEDICAL_RECORD_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.MedicalRecordUserAction(
				modelAction.MedicalRecordUserModelActionTypes.CREATE_MEDICAL_RECORD_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalRecordUserAction(
				modelAction.MedicalRecordUserModelActionTypes.UPDATE_MEDICAL_RECORD_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalRecordUserModel = new MedicalRecordUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalRecordUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalRecordUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalRecordUserModel.getProps();
		this.modelRelations = MedicalRecordUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalRecordUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalRecordUserModel {
		let updatedModel: MedicalRecordUserModel = new MedicalRecordUserModel(model);
