...
import * as modelAction from 'src/app/models/medicalRecordUser/medical_record_user.model.action';
import {MedicalRecordUserModelState} from 'src/app/models/medicalRecordUser/medical_record_user.model.state';
...
import {
	getMedicalRecordUserCollectionModels,
	getMedicalRecordUserCollectionCount
...
import {CrudTileMode} from '../medical-record-user-admin-crud.component';
...
@Component({
	selector: 'cb-medical-record-user-admin-crud-list',
	templateUrl: './medical-record-user-admin-crud-list.component.html',
	styleUrls: [
		'./medical-record-user-admin-crud-list.component.scss',
...
export class AdminMedicalRecordUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalRecordUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalRecordUserModel.searchFields,
	...
	/**
	 * List of Medical Record users that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalRecordUsers: Observable<MedicalRecordUserModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalRecordUsersCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = MedicalRecordUserModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalRecordUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'dc9ae897-a981-4129-8aa2-5b7605f8485d';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalRecordUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalRecordUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: MedicalRecordUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalRecordUserAction(
				modelAction.MedicalRecordUserModelActionTypes.INITIALISE_MEDICAL_RECORD_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalRecordUserAction(
				modelAction.MedicalRecordUserModelActionTypes.FETCH_MEDICAL_RECORD_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.MedicalRecordUserAction(
						modelAction.MedicalRecordUserModelActionTypes.COUNT_MEDICAL_RECORD_USERS,
						null
					)
				]
			))
		...
		this.medicalRecordUsers = this.store.select(getMedicalRecordUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalRecordUserCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalRecordUsersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('admin');
			commands.push('users');
			commands.push('medical-record-user');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalRecordUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalRecordUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalRecordUserModelActionTypes.EXPORT_ALL_MEDICAL_RECORD_USER;
		} else {
			actionType = modelAction.MedicalRecordUserModelActionTypes.EXPORT_MEDICAL_RECORD_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalRecordUserAction(
				modelAction.MedicalRecordUserModelActionTypes.IMPORT_MEDICAL_RECORD_USERS,
				{
					file: file,
				},
				[new modelAction.MedicalRecordUserAction(
			...
			window.open(environment.API_URL + '/api/medicalrecorduser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalRecordUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Record user entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Record user entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalRecordUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalRecordUserModelActionTypes.DELETE_MEDICAL_RECORD_USER;
						} else if (usingSelected) {
							actionType = modelAction.MedicalRecordUserModelActionTypes.DELETE_ALL_MEDICAL_RECORD_USER;
						} else {
							actionType = modelAction.MedicalRecordUserModelActionTypes.DELETE_MEDICAL_RECORD_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalRecordUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_RECORD_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_RECORD_USER and DELETE_MEDICAL_RECORD_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<MedicalRecordUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalRecordUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalRecordUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
