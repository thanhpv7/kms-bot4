...
import {AdminMedicalRecordUserCrudListComponent} from './medical-record-user-admin-crud-list.component';
...
import {AdminMedicalRecordUserCrudModule} from '../medical-record-user-admin-crud.module';
import {
	MedicalRecordUserModelState,
	initialState as initialMedicalRecordUserModelState
} from 'src/app/models/medicalRecordUser/medical_record_user.model.state';
 ...
 */
describe('Medical Record user Crud List Tile Component', () => {

	let fixture;
	let medicalRecordUserCrudListComponent: AdminMedicalRecordUserCrudListComponent;

	let store: MockStore<{ model: MedicalRecordUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let medicalRecordUserModelState: MedicalRecordUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicalRecordUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'dc9ae897-a981-4129-8aa2-5b7605f8485d';
	const searchContent = 'text';

	const searchableFieldCount = MedicalRecordUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medical-record-user-admin-crud',
		urls: ['medical-record-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicalRecordUserDataFactory = new MedicalRecordUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicalRecordUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: MedicalRecordUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		medicalRecordUserModelState = initialMedicalRecordUserModelState;
		store.setState({model: medicalRecordUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getMedicalRecordUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicalRecordUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicalRecordUserCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicalRecordUserCrudListComponent);
			medicalRecordUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicalRecordUserCrudListComponent.medicalRecordUsersCount).toBe(defaultData.length);
			...
			expect(medicalRecordUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicalRecordUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicalRecordUserDataFactory.createAll(filteredDataSize);
			...
			expect(medicalRecordUserCrudListComponent.medicalRecordUsersCount).toBe(filteredDataSize);
			expect(medicalRecordUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicalRecordUserCrudListComponent.pageIndex).toBe(0);
			expect(medicalRecordUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicalRecordUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicalRecordUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicalRecordUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
