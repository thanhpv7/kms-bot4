...
import {AdminPharmacySetupCrudListComponent} from './pharmacy-setup-admin-crud-list.component';
import {PharmacySetupDataFactory} from 'src/app/lib/utils/factories/pharmacy-setup-data-factory';
import {PharmacySetupModel} from 'src/app/models/pharmacySetup/pharmacy_setup.model';
import {AdminPharmacySetupCrudModule} from '../pharmacy-setup-admin-crud.module';
import {
	PharmacySetupModelState,
	initialState as initialPharmacySetupModelState
} from 'src/app/models/pharmacySetup/pharmacy_setup.model.state';
 ...
 */
describe('Pharmacy Setup Crud List Tile Component', () => {

	let fixture;
	let pharmacySetupCrudListComponent: AdminPharmacySetupCrudListComponent;

	let store: MockStore<{ model: PharmacySetupModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let pharmacySetupModelState: PharmacySetupModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PharmacySetupModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'edd3a0c9-b843-4d09-8810-6e8001fc662c';
	const searchContent = 'text';

	const searchableFieldCount = PharmacySetupModel.searchFields.length;

	const routerState: RouterState = {
		url: 'pharmacy-setup-admin-crud',
		urls: ['pharmacy-setup-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const pharmacySetupDataFactory = new PharmacySetupDataFactory();

	const defaultDataSize = 30;
	const defaultData = pharmacySetupDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PharmacySetupModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		pharmacySetupModelState = initialPharmacySetupModelState;
		store.setState({model: pharmacySetupModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPharmacySetupCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPharmacySetupCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPharmacySetupCrudListComponent);
			pharmacySetupCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(pharmacySetupCrudListComponent.pharmacySetupsCount).toBe(defaultData.length);
			...
			expect(pharmacySetupCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			pharmacySetupCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = pharmacySetupDataFactory.createAll(filteredDataSize);
			...
			expect(pharmacySetupCrudListComponent.pharmacySetupsCount).toBe(filteredDataSize);
			expect(pharmacySetupCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(pharmacySetupCrudListComponent.pageIndex).toBe(0);
			expect(pharmacySetupCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			pharmacySetupCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		pharmacySetupCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		pharmacySetupCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
