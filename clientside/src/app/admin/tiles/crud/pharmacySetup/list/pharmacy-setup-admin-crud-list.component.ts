...
import * as modelAction from 'src/app/models/pharmacySetup/pharmacy_setup.model.action';
import {PharmacySetupModelState} from 'src/app/models/pharmacySetup/pharmacy_setup.model.state';
...
import {
	getPharmacySetupCollectionModels,
	getPharmacySetupCollectionCount
} from 'src/app/models/pharmacySetup/pharmacy_setup.model.selector';
...
import {CrudTileMode} from '../pharmacy-setup-admin-crud.component';
...
@Component({
	selector: 'cb-pharmacy-setup-admin-crud-list',
	templateUrl: './pharmacy-setup-admin-crud-list.component.html',
	styleUrls: [
		'./pharmacy-setup-admin-crud-list.component.scss',
...
export class AdminPharmacySetupCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PharmacySetupModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PharmacySetupModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = PharmacySetupModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'edd3a0c9-b843-4d09-8810-6e8001fc662c';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.pharmacySetupsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.pharmacySetupsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PharmacySetupModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PharmacySetupAction(
				modelAction.PharmacySetupModelActionTypes.INITIALISE_PHARMACY_SETUP_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PharmacySetupAction(
				modelAction.PharmacySetupModelActionTypes.FETCH_PHARMACY_SETUP_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PharmacySetupAction(
						modelAction.PharmacySetupModelActionTypes.COUNT_PHARMACY_SETUPS,
						null
					)
				]
			))
		...
		this.pharmacySetups = this.store.select(getPharmacySetupCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPharmacySetupCollectionCount, this.collectionId).subscribe(
				(count) => this.pharmacySetupsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PharmacySetupModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PharmacySetupModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PharmacySetupModelActionTypes.EXPORT_ALL_PHARMACY_SETUP;
		} else {
			actionType = modelAction.PharmacySetupModelActionTypes.EXPORT_PHARMACY_SETUP_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PharmacySetupAction(
				modelAction.PharmacySetupModelActionTypes.IMPORT_PHARMACY_SETUPS,
				{
					file: file,
				},
				[new modelAction.PharmacySetupAction(
			...
			window.open(environment.API_URL + '/api/pharmacysetup/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PharmacySetupModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Pharmacy Setup entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Pharmacy Setup entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PharmacySetupModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PharmacySetupModelActionTypes.DELETE_PHARMACY_SETUP;
						} else if (usingSelected) {
							actionType = modelAction.PharmacySetupModelActionTypes.DELETE_ALL_PHARMACY_SETUP;
						} else {
							actionType = modelAction.PharmacySetupModelActionTypes.DELETE_PHARMACY_SETUP_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PharmacySetupAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PHARMACY_SETUP when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PHARMACY_SETUP and DELETE_PHARMACY_SETUP_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PharmacySetupModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PharmacySetupModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.pharmacySetupsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
