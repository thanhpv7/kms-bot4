...
import {PharmacySetupModelState, PharmacySetupModelAudit} from 'src/app/models/pharmacySetup/pharmacy_setup.model.state';
import {PharmacySetupModel} from 'src/app/models/pharmacySetup/pharmacy_setup.model';
import {
	getPharmacySetupModelAuditsByEntityId,
	getPharmacySetupModelWithId,
	getCountPharmacySetupModels,
...
@Component({
	selector: 'cb-pharmacy-setup-admin-crud-edit',
	templateUrl: './pharmacy-setup-admin-crud-edit.component.html',
	styleUrls: [
		'./pharmacy-setup-admin-crud-edit.component.scss',
...
export class AdminPharmacySetupCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	pharmacySetupAudits$: Observable<PharmacySetupModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PharmacySetupModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PharmacySetupModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPharmacySetupModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PharmacySetupModel();
		...
		if (this.targetModelId) {
			this.pharmacySetupAudits$ = this.store.select(getPharmacySetupModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PharmacySetupModel> = {
		...
		this.store.dispatch(new modelAction.PharmacySetupAction(
			modelAction.PharmacySetupModelActionTypes.FETCH_PHARMACY_SETUP,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPharmacySetupModelWithId, this.targetModelId)
			...
			model = new PharmacySetupModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PharmacySetupAction(
			modelAction.PharmacySetupModelActionTypes.FETCH_PHARMACY_SETUP_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PharmacySetupAction(
				modelAction.PharmacySetupModelActionTypes.CREATE_PHARMACY_SETUP,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PharmacySetupAction(
				modelAction.PharmacySetupModelActionTypes.UPDATE_PHARMACY_SETUP,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PharmacySetupModel = new PharmacySetupModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPharmacySetupModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PharmacySetupModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PharmacySetupModel.getProps();
		this.modelRelations = PharmacySetupModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PharmacySetupModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PharmacySetupModel {
		let updatedModel: PharmacySetupModel = new PharmacySetupModel(model);
