...
import {BpjsINACBGClaimModelState, BpjsINACBGClaimModelAudit} from 'src/app/models/bpjsINACBGClaim/bpjs_inacbg_claim.model.state';
import {BpjsINACBGClaimModel} from 'src/app/models/bpjsINACBGClaim/bpjs_inacbg_claim.model';
import {
	getBpjsINACBGClaimModelAuditsByEntityId,
	getBpjsINACBGClaimModelWithId,
	getCountBpjsINACBGClaimModels,
...
@Component({
	selector: 'cb-bpjs-inacbg-claim-admin-crud-edit',
	templateUrl: './bpjs-inacbg-claim-admin-crud-edit.component.html',
	styleUrls: [
		'./bpjs-inacbg-claim-admin-crud-edit.component.scss',
...
export class AdminBpjsINACBGClaimCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	bpjsINACBGClaimAudits$: Observable<BpjsINACBGClaimModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BpjsINACBGClaimModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: BpjsINACBGClaimModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBpjsINACBGClaimModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BpjsINACBGClaimModel();
		...
		if (this.targetModelId) {
			this.bpjsINACBGClaimAudits$ = this.store.select(getBpjsINACBGClaimModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BpjsINACBGClaimModel> = {
		...
		this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
			modelAction.BpjsINACBGClaimModelActionTypes.FETCH_BPJS_INACBG_CLAIM,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBpjsINACBGClaimModelWithId, this.targetModelId)
			...
			model = new BpjsINACBGClaimModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
			modelAction.BpjsINACBGClaimModelActionTypes.FETCH_BPJS_INACBG_CLAIM_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
				modelAction.BpjsINACBGClaimModelActionTypes.CREATE_BPJS_INACBG_CLAIM,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
				modelAction.BpjsINACBGClaimModelActionTypes.UPDATE_BPJS_INACBG_CLAIM,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BpjsINACBGClaimModel = new BpjsINACBGClaimModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBpjsINACBGClaimModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BpjsINACBGClaimModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BpjsINACBGClaimModel.getProps();
		this.modelRelations = BpjsINACBGClaimModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BpjsINACBGClaimModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BpjsINACBGClaimModel {
		let updatedModel: BpjsINACBGClaimModel = new BpjsINACBGClaimModel(model);
