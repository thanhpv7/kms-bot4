...
import * as modelAction from 'src/app/models/bpjsINACBGClaim/bpjs_inacbg_claim.model.action';
import {BpjsINACBGClaimModelState} from 'src/app/models/bpjsINACBGClaim/bpjs_inacbg_claim.model.state';
...
import {
	getBpjsINACBGClaimCollectionModels,
	getBpjsINACBGClaimCollectionCount
} from 'src/app/models/bpjsINACBGClaim/bpjs_inacbg_claim.model.selector';
...
import {CrudTileMode} from '../bpjs-inacbg-claim-admin-crud.component';
...
@Component({
	selector: 'cb-bpjs-inacbg-claim-admin-crud-list',
	templateUrl: './bpjs-inacbg-claim-admin-crud-list.component.html',
	styleUrls: [
		'./bpjs-inacbg-claim-admin-crud-list.component.scss',
...
export class AdminBpjsINACBGClaimCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BpjsINACBGClaimModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BpjsINACBGClaimModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BpjsINACBGClaimModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'b08217b8-4827-4fd3-95c9-181fcbfa7f53';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.bpjsINACBGClaimsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.bpjsINACBGClaimsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BpjsINACBGClaimModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
				modelAction.BpjsINACBGClaimModelActionTypes.INITIALISE_BPJS_INACBG_CLAIM_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
				modelAction.BpjsINACBGClaimModelActionTypes.FETCH_BPJS_INACBG_CLAIM_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BpjsINACBGClaimAction(
						modelAction.BpjsINACBGClaimModelActionTypes.COUNT_BPJS_INACBG_CLAIMS,
						null
					)
				]
			))
		...
		this.bpjsINACBGClaims = this.store.select(getBpjsINACBGClaimCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBpjsINACBGClaimCollectionCount, this.collectionId).subscribe(
				(count) => this.bpjsINACBGClaimsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('bpjs-inacbg-claim');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BpjsINACBGClaimModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BpjsINACBGClaimModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BpjsINACBGClaimModelActionTypes.EXPORT_ALL_BPJS_INACBG_CLAIM;
		} else {
			actionType = modelAction.BpjsINACBGClaimModelActionTypes.EXPORT_BPJS_INACBG_CLAIM_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
				modelAction.BpjsINACBGClaimModelActionTypes.IMPORT_BPJS_INACBG_CLAIMS,
				{
					file: file,
				},
				[new modelAction.BpjsINACBGClaimAction(
			...
			window.open(environment.API_URL + '/api/bpjsinacbgclaim/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BpjsINACBGClaimModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected BPJS INACBG Claim entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected BPJS INACBG Claim entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BpjsINACBGClaimModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BpjsINACBGClaimModelActionTypes.DELETE_BPJS_INACBG_CLAIM;
						} else if (usingSelected) {
							actionType = modelAction.BpjsINACBGClaimModelActionTypes.DELETE_ALL_BPJS_INACBG_CLAIM;
						} else {
							actionType = modelAction.BpjsINACBGClaimModelActionTypes.DELETE_BPJS_INACBG_CLAIM_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BpjsINACBGClaimAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BPJS_INACBG_CLAIM when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BPJS_INACBG_CLAIM and DELETE_BPJS_INACBG_CLAIM_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BpjsINACBGClaimModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BpjsINACBGClaimModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.bpjsINACBGClaimsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
