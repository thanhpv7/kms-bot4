...
import {AdminBpjsINACBGClaimCrudListComponent} from './bpjs-inacbg-claim-admin-crud-list.component';
import {BpjsINACBGClaimDataFactory} from 'src/app/lib/utils/factories/bpjs-inacbg-claim-data-factory';
import {BpjsINACBGClaimModel} from 'src/app/models/bpjsINACBGClaim/bpjs_inacbg_claim.model';
import {AdminBpjsINACBGClaimCrudModule} from '../bpjs-inacbg-claim-admin-crud.module';
import {
	BpjsINACBGClaimModelState,
	initialState as initialBpjsINACBGClaimModelState
} from 'src/app/models/bpjsINACBGClaim/bpjs_inacbg_claim.model.state';
 ...
 */
describe('BPJS INACBG Claim Crud List Tile Component', () => {

	let fixture;
	let bpjsINACBGClaimCrudListComponent: AdminBpjsINACBGClaimCrudListComponent;

	let store: MockStore<{ model: BpjsINACBGClaimModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let bpjsINACBGClaimModelState: BpjsINACBGClaimModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<BpjsINACBGClaimModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b08217b8-4827-4fd3-95c9-181fcbfa7f53';
	const searchContent = 'text';

	const searchableFieldCount = BpjsINACBGClaimModel.searchFields.length;

	const routerState: RouterState = {
		url: 'bpjs-inacbg-claim-admin-crud',
		urls: ['bpjs-inacbg-claim-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const bpjsINACBGClaimDataFactory = new BpjsINACBGClaimDataFactory();

	const defaultDataSize = 30;
	const defaultData = bpjsINACBGClaimDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: BpjsINACBGClaimModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		bpjsINACBGClaimModelState = initialBpjsINACBGClaimModelState;
		store.setState({model: bpjsINACBGClaimModelState});
		...
		spyOn(store, 'select')
			.withArgs(getBpjsINACBGClaimCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getBpjsINACBGClaimCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminBpjsINACBGClaimCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminBpjsINACBGClaimCrudListComponent);
			bpjsINACBGClaimCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(bpjsINACBGClaimCrudListComponent.bpjsINACBGClaimsCount).toBe(defaultData.length);
			...
			expect(bpjsINACBGClaimCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			bpjsINACBGClaimCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = bpjsINACBGClaimDataFactory.createAll(filteredDataSize);
			...
			expect(bpjsINACBGClaimCrudListComponent.bpjsINACBGClaimsCount).toBe(filteredDataSize);
			expect(bpjsINACBGClaimCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(bpjsINACBGClaimCrudListComponent.pageIndex).toBe(0);
			expect(bpjsINACBGClaimCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			bpjsINACBGClaimCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		bpjsINACBGClaimCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		bpjsINACBGClaimCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
