...
import {AdminExaminationGroupCrudListComponent} from './examination-group-admin-crud-list.component';
...
import {AdminExaminationGroupCrudModule} from '../examination-group-admin-crud.module';
import {
 ...
 */
describe('Examination Group Crud List Tile Component', () => {

	let fixture;
	let examinationGroupCrudListComponent: AdminExaminationGroupCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ExaminationGroupModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f00ad327-e958-43a0-8431-2f226f39955a';
	const searchContent = 'text';

	const searchableFieldCount = ExaminationGroupModel.searchFields.length;

	const routerState: RouterState = {
		url: 'examination-group-admin-crud',
		urls: ['examination-group-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const examinationGroupDataFactory = new ExaminationGroupDataFactory();

	const defaultDataSize = 30;
	const defaultData = examinationGroupDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getExaminationGroupCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getExaminationGroupCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminExaminationGroupCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminExaminationGroupCrudListComponent);
			examinationGroupCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(examinationGroupCrudListComponent.examinationGroupsCount).toBe(defaultData.length);
			...
			expect(examinationGroupCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			examinationGroupCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = examinationGroupDataFactory.createAll(filteredDataSize);
			...
			expect(examinationGroupCrudListComponent.examinationGroupsCount).toBe(filteredDataSize);
			expect(examinationGroupCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(examinationGroupCrudListComponent.pageIndex).toBe(0);
			expect(examinationGroupCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			examinationGroupCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		examinationGroupCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		examinationGroupCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
