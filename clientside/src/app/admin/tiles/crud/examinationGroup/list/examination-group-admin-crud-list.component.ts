...
import * as modelAction from 'src/app/models/examinationGroup/examination_group.model.action';
import {ExaminationGroupModelState} from 'src/app/models/examinationGroup/examination_group.model.state';
...
import {
	...
	getExaminationGroupCollectionCount
...
import {CrudTileMode} from '../examination-group-admin-crud.component';
...
@Component({
	selector: 'cb-examination-group-admin-crud-list',
	templateUrl: './examination-group-admin-crud-list.component.html',
	styleUrls: [
		'./examination-group-admin-crud-list.component.scss',
...
export class AdminExaminationGroupCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ExaminationGroupModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ExaminationGroupModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = ExaminationGroupModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'f00ad327-e958-43a0-8431-2f226f39955a';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.examinationGroupsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.examinationGroupsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ExaminationGroupModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ExaminationGroupAction(
				modelAction.ExaminationGroupModelActionTypes.INITIALISE_EXAMINATION_GROUP_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ExaminationGroupAction(
				modelAction.ExaminationGroupModelActionTypes.FETCH_EXAMINATION_GROUP_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ExaminationGroupAction(
						modelAction.ExaminationGroupModelActionTypes.COUNT_EXAMINATION_GROUPS,
						null
					)
				]
			))
		...
		this.examinationGroups = this.store.select(getExaminationGroupCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getExaminationGroupCollectionCount, this.collectionId).subscribe(
				(count) => this.examinationGroupsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('examination-group');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ExaminationGroupModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ExaminationGroupModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ExaminationGroupModelActionTypes.EXPORT_ALL_EXAMINATION_GROUP;
		} else {
			actionType = modelAction.ExaminationGroupModelActionTypes.EXPORT_EXAMINATION_GROUP_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ExaminationGroupAction(
				modelAction.ExaminationGroupModelActionTypes.IMPORT_EXAMINATION_GROUPS,
				{
					file: file,
				},
				[new modelAction.ExaminationGroupAction(
			...
			window.open(environment.API_URL + '/api/examinationgroup/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ExaminationGroupModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Examination Group entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Examination Group entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ExaminationGroupModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ExaminationGroupModelActionTypes.DELETE_EXAMINATION_GROUP;
						} else if (usingSelected) {
							actionType = modelAction.ExaminationGroupModelActionTypes.DELETE_ALL_EXAMINATION_GROUP;
						} else {
							actionType = modelAction.ExaminationGroupModelActionTypes.DELETE_EXAMINATION_GROUP_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ExaminationGroupAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_EXAMINATION_GROUP when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_EXAMINATION_GROUP and DELETE_EXAMINATION_GROUP_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ExaminationGroupModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ExaminationGroupModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.examinationGroupsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
