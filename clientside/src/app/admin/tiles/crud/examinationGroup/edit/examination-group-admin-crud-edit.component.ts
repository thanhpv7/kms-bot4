...
import {ExaminationGroupModelState, ExaminationGroupModelAudit} from 'src/app/models/examinationGroup/examination_group.model.state';
import {ExaminationGroupModel} from 'src/app/models/examinationGroup/examination_group.model';
import {
	getExaminationGroupModelAuditsByEntityId,
...
@Component({
	selector: 'cb-examination-group-admin-crud-edit',
	templateUrl: './examination-group-admin-crud-edit.component.html',
	styleUrls: [
		'./examination-group-admin-crud-edit.component.scss',
...
export class AdminExaminationGroupCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	examinationGroupAudits$: Observable<ExaminationGroupModelAudit[]>;
	...
	serviceOppositeRelation = 'examinationGroups';
	...
	examinationItemsOppositeRelation = 'examinationGroup';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ExaminationGroupModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ExaminationGroupModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountExaminationGroupModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ExaminationGroupModel();
		...
		if (this.targetModelId) {
			this.examinationGroupAudits$ = this.store.select(getExaminationGroupModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ExaminationGroupModel> = {
		...
		this.store.dispatch(new modelAction.ExaminationGroupAction(
			modelAction.ExaminationGroupModelActionTypes.FETCH_EXAMINATION_GROUP,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getExaminationGroupModelWithId, this.targetModelId)
			...
			model = new ExaminationGroupModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ExaminationGroupAction(
			modelAction.ExaminationGroupModelActionTypes.FETCH_EXAMINATION_GROUP_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ExaminationGroupModel> = {
				...
				queryParams: {
					persistRelations: this.serviceMultiCrudActive || this.examinationItemsMultiCrudActive,
				},
			...
			this.store.dispatch(new modelAction.ExaminationGroupAction(
				modelAction.ExaminationGroupModelActionTypes.CREATE_EXAMINATION_GROUP,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ExaminationGroupAction(
				modelAction.ExaminationGroupModelActionTypes.UPDATE_EXAMINATION_GROUP,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ExaminationGroupModel = new ExaminationGroupModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getExaminationGroupModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ExaminationGroupModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ExaminationGroupModel.getProps();
		this.modelRelations = ExaminationGroupModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ExaminationGroupModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ExaminationGroupModel {
		let updatedModel: ExaminationGroupModel = new ExaminationGroupModel(model);
