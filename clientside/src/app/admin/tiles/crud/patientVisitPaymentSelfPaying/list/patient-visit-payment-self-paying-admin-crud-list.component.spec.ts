...
import {AdminPatientVisitPaymentSelfPayingCrudListComponent} from './patient-visit-payment-self-paying-admin-crud-list.component';
...
import {AdminPatientVisitPaymentSelfPayingCrudModule} from '../patient-visit-payment-self-paying-admin-crud.module';
import {
	PatientVisitPaymentSelfPayingModelState,
	initialState as initialPatientVisitPaymentSelfPayingModelState
} from 'src/app/models/patientVisitPaymentSelfPaying/patient_visit_payment_self_paying.model.state';
 ...
 */
describe('Patient Visit Payment Self Paying Crud List Tile Component', () => {

	let fixture;
	let patientVisitPaymentSelfPayingCrudListComponent: AdminPatientVisitPaymentSelfPayingCrudListComponent;

	let store: MockStore<{ model: PatientVisitPaymentSelfPayingModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let patientVisitPaymentSelfPayingModelState: PatientVisitPaymentSelfPayingModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<PatientVisitPaymentSelfPayingModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '16d29488-2298-4a69-af88-cda7f944939b';
	const searchContent = 'text';

	const searchableFieldCount = PatientVisitPaymentSelfPayingModel.searchFields.length;

	const routerState: RouterState = {
		url: 'patient-visit-payment-self-paying-admin-crud',
		urls: ['patient-visit-payment-self-paying-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const patientVisitPaymentSelfPayingDataFactory = new PatientVisitPaymentSelfPayingDataFactory();

	const defaultDataSize = 30;
	const defaultData = patientVisitPaymentSelfPayingDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: PatientVisitPaymentSelfPayingModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		patientVisitPaymentSelfPayingModelState = initialPatientVisitPaymentSelfPayingModelState;
		store.setState({model: patientVisitPaymentSelfPayingModelState});
		...
		spyOn(store, 'select')
			.withArgs(getPatientVisitPaymentSelfPayingCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPatientVisitPaymentSelfPayingCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPatientVisitPaymentSelfPayingCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPatientVisitPaymentSelfPayingCrudListComponent);
			patientVisitPaymentSelfPayingCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(patientVisitPaymentSelfPayingCrudListComponent.patientVisitPaymentSelfPayingsCount).toBe(defaultData.length);
			...
			expect(patientVisitPaymentSelfPayingCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			patientVisitPaymentSelfPayingCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = patientVisitPaymentSelfPayingDataFactory.createAll(filteredDataSize);
			...
			expect(patientVisitPaymentSelfPayingCrudListComponent.patientVisitPaymentSelfPayingsCount).toBe(filteredDataSize);
			expect(patientVisitPaymentSelfPayingCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(patientVisitPaymentSelfPayingCrudListComponent.pageIndex).toBe(0);
			expect(patientVisitPaymentSelfPayingCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			patientVisitPaymentSelfPayingCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		patientVisitPaymentSelfPayingCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		patientVisitPaymentSelfPayingCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
