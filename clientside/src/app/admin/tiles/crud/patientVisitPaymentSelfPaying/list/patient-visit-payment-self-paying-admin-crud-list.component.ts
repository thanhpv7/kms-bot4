...
import * as modelAction from 'src/app/models/patientVisitPaymentSelfPaying/patient_visit_payment_self_paying.model.action';
import {PatientVisitPaymentSelfPayingModelState} from 'src/app/models/patientVisitPaymentSelfPaying/patient_visit_payment_self_paying.model.state';
...
import {
	getPatientVisitPaymentSelfPayingCollectionModels,
	getPatientVisitPaymentSelfPayingCollectionCount
...
import {CrudTileMode} from '../patient-visit-payment-self-paying-admin-crud.component';
...
@Component({
	selector: 'cb-patient-visit-payment-self-paying-admin-crud-list',
	templateUrl: './patient-visit-payment-self-paying-admin-crud-list.component.html',
	styleUrls: [
		'./patient-visit-payment-self-paying-admin-crud-list.component.scss',
...
export class AdminPatientVisitPaymentSelfPayingCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PatientVisitPaymentSelfPayingModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PatientVisitPaymentSelfPayingModel.searchFields,
	...
	/**
	 * List of Patient Visit Payment Self Payings that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	patientVisitPaymentSelfPayings: Observable<PatientVisitPaymentSelfPayingModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	patientVisitPaymentSelfPayingsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = PatientVisitPaymentSelfPayingModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = PatientVisitPaymentSelfPayingModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '16d29488-2298-4a69-af88-cda7f944939b';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.patientVisitPaymentSelfPayingsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.patientVisitPaymentSelfPayingsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: PatientVisitPaymentSelfPayingModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
				modelAction.PatientVisitPaymentSelfPayingModelActionTypes.INITIALISE_PATIENT_VISIT_PAYMENT_SELF_PAYING_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
				modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_PATIENT_VISIT_PAYMENT_SELF_PAYING_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.PatientVisitPaymentSelfPayingAction(
						modelAction.PatientVisitPaymentSelfPayingModelActionTypes.COUNT_PATIENT_VISIT_PAYMENT_SELF_PAYINGS,
						null
					)
				]
			))
		...
		this.patientVisitPaymentSelfPayings = this.store.select(getPatientVisitPaymentSelfPayingCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPatientVisitPaymentSelfPayingCollectionCount, this.collectionId).subscribe(
				(count) => this.patientVisitPaymentSelfPayingsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('patient-visit-payment-self-paying');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PatientVisitPaymentSelfPayingModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PatientVisitPaymentSelfPayingModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PatientVisitPaymentSelfPayingModelActionTypes.EXPORT_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING;
		} else {
			actionType = modelAction.PatientVisitPaymentSelfPayingModelActionTypes.EXPORT_PATIENT_VISIT_PAYMENT_SELF_PAYING_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
				modelAction.PatientVisitPaymentSelfPayingModelActionTypes.IMPORT_PATIENT_VISIT_PAYMENT_SELF_PAYINGS,
				{
					file: file,
				},
				[new modelAction.PatientVisitPaymentSelfPayingAction(
			...
			window.open(environment.API_URL + '/api/patientvisitpaymentselfpaying/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PatientVisitPaymentSelfPayingModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit Payment Self Paying entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Patient Visit Payment Self Paying entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PatientVisitPaymentSelfPayingModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PatientVisitPaymentSelfPayingModelActionTypes.DELETE_PATIENT_VISIT_PAYMENT_SELF_PAYING;
						} else if (usingSelected) {
							actionType = modelAction.PatientVisitPaymentSelfPayingModelActionTypes.DELETE_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING;
						} else {
							actionType = modelAction.PatientVisitPaymentSelfPayingModelActionTypes.DELETE_PATIENT_VISIT_PAYMENT_SELF_PAYING_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PATIENT_VISIT_PAYMENT_SELF_PAYING when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING and DELETE_PATIENT_VISIT_PAYMENT_SELF_PAYING_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<PatientVisitPaymentSelfPayingModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PatientVisitPaymentSelfPayingModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.patientVisitPaymentSelfPayingsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
