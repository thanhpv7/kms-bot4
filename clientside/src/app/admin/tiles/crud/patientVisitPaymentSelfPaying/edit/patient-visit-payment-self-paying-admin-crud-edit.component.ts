...
import {PatientVisitPaymentSelfPayingModelState, PatientVisitPaymentSelfPayingModelAudit} from 'src/app/models/patientVisitPaymentSelfPaying/patient_visit_payment_self_paying.model.state';
...
import {
	getPatientVisitPaymentSelfPayingModelAuditsByEntityId,
	getPatientVisitPaymentSelfPayingModelWithId,
	getCountPatientVisitPaymentSelfPayingModels,
...
@Component({
	selector: 'cb-patient-visit-payment-self-paying-admin-crud-edit',
	templateUrl: './patient-visit-payment-self-paying-admin-crud-edit.component.html',
	styleUrls: [
		'./patient-visit-payment-self-paying-admin-crud-edit.component.scss',
...
export class AdminPatientVisitPaymentSelfPayingCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	patientVisitPaymentSelfPayingAudits$: Observable<PatientVisitPaymentSelfPayingModelAudit[]>;
	...
	patientVisitOppositeRelation = 'patientVisitPaymentSelfPaying';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = PatientVisitPaymentSelfPayingModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: PatientVisitPaymentSelfPayingModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountPatientVisitPaymentSelfPayingModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new PatientVisitPaymentSelfPayingModel();
		...
		if (this.targetModelId) {
			this.patientVisitPaymentSelfPayingAudits$ = this.store.select(getPatientVisitPaymentSelfPayingModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<PatientVisitPaymentSelfPayingModel> = {
		...
		this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
			modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_PATIENT_VISIT_PAYMENT_SELF_PAYING,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getPatientVisitPaymentSelfPayingModelWithId, this.targetModelId)
			...
			model = new PatientVisitPaymentSelfPayingModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
			modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_PATIENT_VISIT_PAYMENT_SELF_PAYING_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
				modelAction.PatientVisitPaymentSelfPayingModelActionTypes.CREATE_PATIENT_VISIT_PAYMENT_SELF_PAYING,
				stateConfig,
			...
			this.store.dispatch(new modelAction.PatientVisitPaymentSelfPayingAction(
				modelAction.PatientVisitPaymentSelfPayingModelActionTypes.UPDATE_PATIENT_VISIT_PAYMENT_SELF_PAYING,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: PatientVisitPaymentSelfPayingModel = new PatientVisitPaymentSelfPayingModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getPatientVisitPaymentSelfPayingModelWithId, this.targetModelId).subscribe(model => {
					let differences = new PatientVisitPaymentSelfPayingModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = PatientVisitPaymentSelfPayingModel.getProps();
		this.modelRelations = PatientVisitPaymentSelfPayingModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: PatientVisitPaymentSelfPayingModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): PatientVisitPaymentSelfPayingModel {
		let updatedModel: PatientVisitPaymentSelfPayingModel = new PatientVisitPaymentSelfPayingModel(model);
