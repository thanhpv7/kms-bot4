...
import * as modelAction from 'src/app/models/internalOrderStockDetail/internal_order_stock_detail.model.action';
import {InternalOrderStockDetailModelState, InternalOrderStockDetailModelAudit} from 'src/app/models/internalOrderStockDetail/internal_order_stock_detail.model.state';
import {InternalOrderStockDetailModel} from 'src/app/models/internalOrderStockDetail/internal_order_stock_detail.model';
import {
	getInternalOrderStockDetailModelAuditsByEntityId,
	getInternalOrderStockDetailModelWithId,
	getCountInternalOrderStockDetailModels,
...
import {CrudTileMode} from '../internal-order-stock-detail-admin-crud.component';
...
@Component({
	selector: 'cb-internal-order-stock-detail-admin-crud-edit',
	templateUrl: './internal-order-stock-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./internal-order-stock-detail-admin-crud-edit.component.scss',
...
export class AdminInternalOrderStockDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'd68eab4d-e2c9-4d5d-88a0-d116e9bf2627';
	 ...
	 */
	internalOrderStockDetailAudits$: Observable<InternalOrderStockDetailModelAudit[]>;
	...
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Internal Order and Internal Order Stock Details here] off begin
	internalOrderMultiCrudActive: boolean = false;
	internalOrderDisplayName = 'Internal Order';
	// % protected region % [Modify the display name and the flag for enabling multi entity crud and on relation between Internal Order and Internal Order Stock Details here] end
	internalOrderOppositeRelation = 'internalOrderStockDetails';
	internalOrderRelations: ModelRelation[] = this.internalOrderMultiCrudActive ? InternalOrderModel.getRelations()
		.map(relation => {
			if (relation.id === this.internalOrderOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'internalOrderStockDetails';
	 ...
	 */
	modelProperties: ModelProperty[] = InternalOrderStockDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = InternalOrderStockDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = InternalOrderStockDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'internalOrder' && this.internalOrderMultiCrudActive) {
	 ...
	 */
	targetModel: InternalOrderStockDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: InternalOrderStockDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountInternalOrderStockDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new InternalOrderStockDetailModel();
		...
		if (this.targetModelId) {
			this.internalOrderStockDetailAudits$ = this.store.select(getInternalOrderStockDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<InternalOrderStockDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
			modelAction.InternalOrderStockDetailModelActionTypes.FETCH_INTERNAL_ORDER_STOCK_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getInternalOrderStockDetailModelWithId, this.targetModelId)
			...
			model = new InternalOrderStockDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('internal-order-stock-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
			modelAction.InternalOrderStockDetailModelActionTypes.FETCH_INTERNAL_ORDER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<InternalOrderStockDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.internalOrderMultiCrudActive || this.stockCatalogueMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.InternalOrderStockDetailAction(
						modelAction.InternalOrderStockDetailModelActionTypes.COUNT_INTERNAL_ORDER_STOCK_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
				modelAction.InternalOrderStockDetailModelActionTypes.CREATE_INTERNAL_ORDER_STOCK_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
				modelAction.InternalOrderStockDetailModelActionTypes.UPDATE_INTERNAL_ORDER_STOCK_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: InternalOrderStockDetailModel = new InternalOrderStockDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getInternalOrderStockDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new InternalOrderStockDetailModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.internalOrderDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InternalOrderModel.getProps(), this.internalOrderRelations, this.isViewOnly),
					new InternalOrderModel(),
					InternalOrderModel,
					this.internalOrderDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.internalOrderRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = InternalOrderStockDetailModel.getProps();
		this.modelRelations = InternalOrderStockDetailModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'internalOrder';
		}).hideElement = this.internalOrderMultiCrudActive || this.modelRelations.find(rel => rel.id === 'internalOrder').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.internalOrderMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.internalOrderDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.internalOrder && this.targetModel.internalOrder.id && this.internalOrderMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				InternalOrderModel,
				this.internalOrderOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.internalOrder.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getInternalOrderModelWithId, this.targetModel.internalOrderId).subscribe(internalOrder => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(InternalOrderModel.getProps(), this.internalOrderRelations, this.isViewOnly),
					internalOrder,
					InternalOrderModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: InternalOrderStockDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): InternalOrderStockDetailModel {
		let updatedModel: InternalOrderStockDetailModel = new InternalOrderStockDetailModel(model);

		if (this.internalOrderMultiCrudActive) {
			updatedModel.internalOrder = event.additionalEntities.internalOrder ? event.additionalEntities.internalOrder : undefined;

			if (!updatedModel.internalOrder && event.payload) {
				event.payload.internalOrderId = null;
			}
		} else if (event.payload?.internalOrderId !== undefined) {
			event.payload.internalOrder = null;
			updatedModel.internalOrder = null;
		}
