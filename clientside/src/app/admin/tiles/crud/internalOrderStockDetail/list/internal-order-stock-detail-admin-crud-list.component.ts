...
import {InternalOrderStockDetailModelState} from 'src/app/models/internalOrderStockDetail/internal_order_stock_detail.model.state';
...
@Component({
	selector: 'cb-internal-order-stock-detail-admin-crud-list',
	templateUrl: './internal-order-stock-detail-admin-crud-list.component.html',
	styleUrls: [
		'./internal-order-stock-detail-admin-crud-list.component.scss',
...
export class AdminInternalOrderStockDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<InternalOrderStockDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...InternalOrderStockDetailModel.searchFields,
	...
	/**
	 * List of Internal Order Stock Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	internalOrderStockDetails: Observable<InternalOrderStockDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	internalOrderStockDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.internalOrderStockDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.internalOrderStockDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
				modelAction.InternalOrderStockDetailModelActionTypes.INITIALISE_INTERNAL_ORDER_STOCK_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
				modelAction.InternalOrderStockDetailModelActionTypes.FETCH_INTERNAL_ORDER_STOCK_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.internalOrderStockDetails = this.store.select(getInternalOrderStockDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getInternalOrderStockDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.internalOrderStockDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: InternalOrderStockDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.InternalOrderStockDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.InternalOrderStockDetailModelActionTypes.EXPORT_ALL_INTERNAL_ORDER_STOCK_DETAIL;
		} else {
			actionType = modelAction.InternalOrderStockDetailModelActionTypes.EXPORT_INTERNAL_ORDER_STOCK_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
				modelAction.InternalOrderStockDetailModelActionTypes.IMPORT_INTERNAL_ORDER_STOCK_DETAILS,
				{
					file: file,
				},
				[new modelAction.InternalOrderStockDetailAction(
			...
			window.open(environment.API_URL + '/api/internalorderstockdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: InternalOrderStockDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Internal Order Stock Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Internal Order Stock Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.InternalOrderStockDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.InternalOrderStockDetailModelActionTypes.DELETE_INTERNAL_ORDER_STOCK_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.InternalOrderStockDetailModelActionTypes.DELETE_ALL_INTERNAL_ORDER_STOCK_DETAIL;
						} else {
							actionType = modelAction.InternalOrderStockDetailModelActionTypes.DELETE_INTERNAL_ORDER_STOCK_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.InternalOrderStockDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_INTERNAL_ORDER_STOCK_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_INTERNAL_ORDER_STOCK_DETAIL and DELETE_INTERNAL_ORDER_STOCK_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = InternalOrderStockDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.internalOrderStockDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
