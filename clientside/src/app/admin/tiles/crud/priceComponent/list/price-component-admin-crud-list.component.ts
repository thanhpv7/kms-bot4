...
import {PriceComponentModelState} from 'src/app/models/priceComponent/price_component.model.state';
...
import {
	...
	getPriceComponentCollectionCount
...
import {CrudTileMode} from '../price-component-admin-crud.component';
...
@Component({
	selector: 'cb-price-component-admin-crud-list',
	templateUrl: './price-component-admin-crud-list.component.html',
	styleUrls: [
		'./price-component-admin-crud-list.component.scss',
...
export class AdminPriceComponentCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PriceComponentModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PriceComponentModel.searchFields,
	...
	/**
	 * List of Price Components that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	priceComponents: Observable<PriceComponentModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	priceComponentsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.priceComponentsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.priceComponentsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PriceComponentAction(
				modelAction.PriceComponentModelActionTypes.INITIALISE_PRICE_COMPONENT_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PriceComponentAction(
				modelAction.PriceComponentModelActionTypes.FETCH_PRICE_COMPONENT_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.priceComponents = this.store.select(getPriceComponentCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPriceComponentCollectionCount, this.collectionId).subscribe(
				(count) => this.priceComponentsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PriceComponentModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PriceComponentModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PriceComponentModelActionTypes.EXPORT_ALL_PRICE_COMPONENT;
		} else {
			actionType = modelAction.PriceComponentModelActionTypes.EXPORT_PRICE_COMPONENT_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PriceComponentAction(
				modelAction.PriceComponentModelActionTypes.IMPORT_PRICE_COMPONENTS,
				{
					file: file,
				},
				[new modelAction.PriceComponentAction(
			...
			window.open(environment.API_URL + '/api/pricecomponent/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PriceComponentModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Price Component entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Price Component entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PriceComponentModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PriceComponentModelActionTypes.DELETE_PRICE_COMPONENT;
						} else if (usingSelected) {
							actionType = modelAction.PriceComponentModelActionTypes.DELETE_ALL_PRICE_COMPONENT;
						} else {
							actionType = modelAction.PriceComponentModelActionTypes.DELETE_PRICE_COMPONENT_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PriceComponentAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PRICE_COMPONENT when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PRICE_COMPONENT and DELETE_PRICE_COMPONENT_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PriceComponentModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.priceComponentsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
