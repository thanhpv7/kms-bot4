...
import {AdminPriceComponentCrudListComponent} from './price-component-admin-crud-list.component';
...
import {AdminPriceComponentCrudModule} from '../price-component-admin-crud.module';
import {
 ...
 */
describe('Price Component Crud List Tile Component', () => {

	let fixture;
	let priceComponentCrudListComponent: AdminPriceComponentCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PriceComponentModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '7c839d34-f065-4537-a7c3-bd8412164930';
	const searchContent = 'text';

	const searchableFieldCount = PriceComponentModel.searchFields.length;

	const routerState: RouterState = {
		url: 'price-component-admin-crud',
		urls: ['price-component-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const priceComponentDataFactory = new PriceComponentDataFactory();

	const defaultDataSize = 30;
	const defaultData = priceComponentDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPriceComponentCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPriceComponentCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminPriceComponentCrudListComponent);
			priceComponentCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(priceComponentCrudListComponent.priceComponentsCount).toBe(defaultData.length);
			...
			expect(priceComponentCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			priceComponentCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = priceComponentDataFactory.createAll(filteredDataSize);
			...
			expect(priceComponentCrudListComponent.priceComponentsCount).toBe(filteredDataSize);
			expect(priceComponentCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(priceComponentCrudListComponent.pageIndex).toBe(0);
			expect(priceComponentCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			priceComponentCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		priceComponentCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		priceComponentCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
