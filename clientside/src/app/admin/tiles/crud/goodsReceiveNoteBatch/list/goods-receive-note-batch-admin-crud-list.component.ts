...
import {GoodsReceiveNoteBatchModelState} from 'src/app/models/goodsReceiveNoteBatch/goods_receive_note_batch.model.state';
...
import {
	getGoodsReceiveNoteBatchCollectionModels,
	getGoodsReceiveNoteBatchCollectionCount
...
@Component({
	selector: 'cb-goods-receive-note-batch-admin-crud-list',
	templateUrl: './goods-receive-note-batch-admin-crud-list.component.html',
	styleUrls: [
		'./goods-receive-note-batch-admin-crud-list.component.scss',
...
export class AdminGoodsReceiveNoteBatchCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<GoodsReceiveNoteBatchModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...GoodsReceiveNoteBatchModel.searchFields,
	...
	/**
	 * List of Goods Receive Note Batchs that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	goodsReceiveNoteBatchs: Observable<GoodsReceiveNoteBatchModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	goodsReceiveNoteBatchsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.goodsReceiveNoteBatchsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.goodsReceiveNoteBatchsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
				modelAction.GoodsReceiveNoteBatchModelActionTypes.INITIALISE_GOODS_RECEIVE_NOTE_BATCH_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
				modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_BATCH_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.goodsReceiveNoteBatchs = this.store.select(getGoodsReceiveNoteBatchCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getGoodsReceiveNoteBatchCollectionCount, this.collectionId).subscribe(
				(count) => this.goodsReceiveNoteBatchsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: GoodsReceiveNoteBatchModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.GoodsReceiveNoteBatchModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.GoodsReceiveNoteBatchModelActionTypes.EXPORT_ALL_GOODS_RECEIVE_NOTE_BATCH;
		} else {
			actionType = modelAction.GoodsReceiveNoteBatchModelActionTypes.EXPORT_GOODS_RECEIVE_NOTE_BATCH_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
				modelAction.GoodsReceiveNoteBatchModelActionTypes.IMPORT_GOODS_RECEIVE_NOTE_BATCHS,
				{
					file: file,
				},
				[new modelAction.GoodsReceiveNoteBatchAction(
			...
			window.open(environment.API_URL + '/api/goodsreceivenotebatch/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: GoodsReceiveNoteBatchModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Goods Receive Note Batch entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Goods Receive Note Batch entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.GoodsReceiveNoteBatchModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.GoodsReceiveNoteBatchModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_BATCH;
						} else if (usingSelected) {
							actionType = modelAction.GoodsReceiveNoteBatchModelActionTypes.DELETE_ALL_GOODS_RECEIVE_NOTE_BATCH;
						} else {
							actionType = modelAction.GoodsReceiveNoteBatchModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_BATCH_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_GOODS_RECEIVE_NOTE_BATCH when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_GOODS_RECEIVE_NOTE_BATCH and DELETE_GOODS_RECEIVE_NOTE_BATCH_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = GoodsReceiveNoteBatchModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.goodsReceiveNoteBatchsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
