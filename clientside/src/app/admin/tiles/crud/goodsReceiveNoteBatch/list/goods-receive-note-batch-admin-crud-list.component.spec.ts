...
import {AdminGoodsReceiveNoteBatchCrudListComponent} from './goods-receive-note-batch-admin-crud-list.component';
...
import {AdminGoodsReceiveNoteBatchCrudModule} from '../goods-receive-note-batch-admin-crud.module';
import {
	GoodsReceiveNoteBatchModelState,
	initialState as initialGoodsReceiveNoteBatchModelState
} from 'src/app/models/goodsReceiveNoteBatch/goods_receive_note_batch.model.state';
 ...
 */
describe('Goods Receive Note Batch Crud List Tile Component', () => {

	let fixture;
	let goodsReceiveNoteBatchCrudListComponent: AdminGoodsReceiveNoteBatchCrudListComponent;

	let store: MockStore<{ model: GoodsReceiveNoteBatchModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let goodsReceiveNoteBatchModelState: GoodsReceiveNoteBatchModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<GoodsReceiveNoteBatchModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'bd1669ce-0848-4ee3-983f-0f0973152851';
	const searchContent = 'text';

	const searchableFieldCount = GoodsReceiveNoteBatchModel.searchFields.length;

	const routerState: RouterState = {
		url: 'goods-receive-note-batch-admin-crud',
		urls: ['goods-receive-note-batch-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const goodsReceiveNoteBatchDataFactory = new GoodsReceiveNoteBatchDataFactory();

	const defaultDataSize = 30;
	const defaultData = goodsReceiveNoteBatchDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: GoodsReceiveNoteBatchModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		goodsReceiveNoteBatchModelState = initialGoodsReceiveNoteBatchModelState;
		store.setState({model: goodsReceiveNoteBatchModelState});
		...
		spyOn(store, 'select')
			.withArgs(getGoodsReceiveNoteBatchCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getGoodsReceiveNoteBatchCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminGoodsReceiveNoteBatchCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminGoodsReceiveNoteBatchCrudListComponent);
			goodsReceiveNoteBatchCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(goodsReceiveNoteBatchCrudListComponent.goodsReceiveNoteBatchsCount).toBe(defaultData.length);
			...
			expect(goodsReceiveNoteBatchCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			goodsReceiveNoteBatchCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = goodsReceiveNoteBatchDataFactory.createAll(filteredDataSize);
			...
			expect(goodsReceiveNoteBatchCrudListComponent.goodsReceiveNoteBatchsCount).toBe(filteredDataSize);
			expect(goodsReceiveNoteBatchCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(goodsReceiveNoteBatchCrudListComponent.pageIndex).toBe(0);
			expect(goodsReceiveNoteBatchCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			goodsReceiveNoteBatchCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		goodsReceiveNoteBatchCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		goodsReceiveNoteBatchCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
