...
import * as modelAction from 'src/app/models/goodsReceiveNoteBatch/goods_receive_note_batch.model.action';
import {GoodsReceiveNoteBatchModelState, GoodsReceiveNoteBatchModelAudit} from 'src/app/models/goodsReceiveNoteBatch/goods_receive_note_batch.model.state';
import {GoodsReceiveNoteBatchModel} from 'src/app/models/goodsReceiveNoteBatch/goods_receive_note_batch.model';
import {
	getGoodsReceiveNoteBatchModelAuditsByEntityId,
	getGoodsReceiveNoteBatchModelWithId,
	getCountGoodsReceiveNoteBatchModels,
...
import {CrudTileMode} from '../goods-receive-note-batch-admin-crud.component';
...
@Component({
	selector: 'cb-goods-receive-note-batch-admin-crud-edit',
	templateUrl: './goods-receive-note-batch-admin-crud-edit.component.html',
	styleUrls: [
		'./goods-receive-note-batch-admin-crud-edit.component.scss',
...
export class AdminGoodsReceiveNoteBatchCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'bd1669ce-0848-4ee3-983f-0f0973152851';
	 ...
	 */
	goodsReceiveNoteBatchAudits$: Observable<GoodsReceiveNoteBatchModelAudit[]>;
	...
	goodsReceiveNoteItemMultiCrudActive: boolean = false;
	goodsReceiveNoteItemDisplayName = 'Goods Receive Note Item';
	...
	goodsReceiveNoteItemOppositeRelation = 'goodsReceiveNoteBatchs';
	goodsReceiveNoteItemRelations: ModelRelation[] = this.goodsReceiveNoteItemMultiCrudActive ? GoodsReceiveNoteItemModel.getRelations()
		.map(relation => {
			if (relation.id === this.goodsReceiveNoteItemOppositeRelation) {
	...
	stockCatalogueOppositeRelation = 'goodsReceiveNoteBatchs';
	...
	warehouseOppositeRelation = 'goodsReceiveNoteBatchs';
	 ...
	 */
	modelProperties: ModelProperty[] = GoodsReceiveNoteBatchModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = GoodsReceiveNoteBatchModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = GoodsReceiveNoteBatchModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'goodsReceiveNoteItem' && this.goodsReceiveNoteItemMultiCrudActive) {
	 ...
	 */
	targetModel: GoodsReceiveNoteBatchModel;
	...
	constructor(
		private readonly store: Store<{ model: GoodsReceiveNoteBatchModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountGoodsReceiveNoteBatchModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new GoodsReceiveNoteBatchModel();
		...
		if (this.targetModelId) {
			this.goodsReceiveNoteBatchAudits$ = this.store.select(getGoodsReceiveNoteBatchModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<GoodsReceiveNoteBatchModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
			modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_BATCH,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getGoodsReceiveNoteBatchModelWithId, this.targetModelId)
			...
			model = new GoodsReceiveNoteBatchModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('goods-receive-note-batch');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
			modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_BATCH_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<GoodsReceiveNoteBatchModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.goodsReceiveNoteItemMultiCrudActive || this.stockCatalogueMultiCrudActive || this.warehouseMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.GoodsReceiveNoteBatchAction(
						modelAction.GoodsReceiveNoteBatchModelActionTypes.COUNT_GOODS_RECEIVE_NOTE_BATCHS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
				modelAction.GoodsReceiveNoteBatchModelActionTypes.CREATE_GOODS_RECEIVE_NOTE_BATCH,
				stateConfig,
			...
			this.store.dispatch(new modelAction.GoodsReceiveNoteBatchAction(
				modelAction.GoodsReceiveNoteBatchModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE_BATCH,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: GoodsReceiveNoteBatchModel = new GoodsReceiveNoteBatchModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getGoodsReceiveNoteBatchModelWithId, this.targetModelId).subscribe(model => {
					let differences = new GoodsReceiveNoteBatchModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.goodsReceiveNoteItemDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(GoodsReceiveNoteItemModel.getProps(), this.goodsReceiveNoteItemRelations, this.isViewOnly),
					new GoodsReceiveNoteItemModel(),
					GoodsReceiveNoteItemModel,
					this.goodsReceiveNoteItemDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.goodsReceiveNoteItemRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = GoodsReceiveNoteBatchModel.getProps();
		this.modelRelations = GoodsReceiveNoteBatchModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'goodsReceiveNoteItem';
		}).hideElement = this.goodsReceiveNoteItemMultiCrudActive || this.modelRelations.find(rel => rel.id === 'goodsReceiveNoteItem').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.goodsReceiveNoteItemMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.goodsReceiveNoteItemDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.goodsReceiveNoteItem && this.targetModel.goodsReceiveNoteItem.id && this.goodsReceiveNoteItemMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				GoodsReceiveNoteItemModel,
				this.goodsReceiveNoteItemOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.goodsReceiveNoteItem.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getGoodsReceiveNoteItemModelWithId, this.targetModel.goodsReceiveNoteItemId).subscribe(goodsReceiveNoteItem => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(GoodsReceiveNoteItemModel.getProps(), this.goodsReceiveNoteItemRelations, this.isViewOnly),
	 ...
	 */
	addAdditionalEntitiesToModel(model: GoodsReceiveNoteBatchModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): GoodsReceiveNoteBatchModel {
		let updatedModel: GoodsReceiveNoteBatchModel = new GoodsReceiveNoteBatchModel(model);

		if (this.goodsReceiveNoteItemMultiCrudActive) {
			updatedModel.goodsReceiveNoteItem = event.additionalEntities.goodsReceiveNoteItem ? event.additionalEntities.goodsReceiveNoteItem : undefined;

			if (!updatedModel.goodsReceiveNoteItem && event.payload) {
				event.payload.goodsReceiveNoteItemId = null;
			}
		} else if (event.payload?.goodsReceiveNoteItemId !== undefined) {
			event.payload.goodsReceiveNoteItem = null;
			updatedModel.goodsReceiveNoteItem = null;
		}
