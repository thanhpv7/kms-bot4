...
import * as modelAction from 'src/app/models/nhisClaimInvestigationDetail/nhis_claim_investigation_detail.model.action';
import {NhisClaimInvestigationDetailModelState} from 'src/app/models/nhisClaimInvestigationDetail/nhis_claim_investigation_detail.model.state';
...
import {CrudTileMode} from '../nhis-claim-investigation-detail-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-claim-investigation-detail-admin-crud-list',
	templateUrl: './nhis-claim-investigation-detail-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-claim-investigation-detail-admin-crud-list.component.scss',
...
export class AdminNhisClaimInvestigationDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisClaimInvestigationDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisClaimInvestigationDetailModel.searchFields,
	...
	/**
	 * List of NHIS Claim Investigation Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	nhisClaimInvestigationDetails: Observable<NhisClaimInvestigationDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	nhisClaimInvestigationDetailsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = NhisClaimInvestigationDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NhisClaimInvestigationDetailModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '000ce183-b7bf-4b96-bfaa-a815bde16570';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisClaimInvestigationDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisClaimInvestigationDetailsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisClaimInvestigationDetailModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
				modelAction.NhisClaimInvestigationDetailModelActionTypes.INITIALISE_NHIS_CLAIM_INVESTIGATION_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
				modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_NHIS_CLAIM_INVESTIGATION_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisClaimInvestigationDetailAction(
						modelAction.NhisClaimInvestigationDetailModelActionTypes.COUNT_NHIS_CLAIM_INVESTIGATION_DETAILS,
						null
					)
				]
			))
		...
		this.nhisClaimInvestigationDetails = this.store.select(getNhisClaimInvestigationDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisClaimInvestigationDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisClaimInvestigationDetailsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('nhis-claim-investigation-detail');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisClaimInvestigationDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisClaimInvestigationDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisClaimInvestigationDetailModelActionTypes.EXPORT_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL;
		} else {
			actionType = modelAction.NhisClaimInvestigationDetailModelActionTypes.EXPORT_NHIS_CLAIM_INVESTIGATION_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
				modelAction.NhisClaimInvestigationDetailModelActionTypes.IMPORT_NHIS_CLAIM_INVESTIGATION_DETAILS,
				{
					file: file,
				},
				[new modelAction.NhisClaimInvestigationDetailAction(
			...
			window.open(environment.API_URL + '/api/nhisclaiminvestigationdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisClaimInvestigationDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Claim Investigation Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Claim Investigation Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisClaimInvestigationDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisClaimInvestigationDetailModelActionTypes.DELETE_NHIS_CLAIM_INVESTIGATION_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.NhisClaimInvestigationDetailModelActionTypes.DELETE_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL;
						} else {
							actionType = modelAction.NhisClaimInvestigationDetailModelActionTypes.DELETE_NHIS_CLAIM_INVESTIGATION_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_CLAIM_INVESTIGATION_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_CLAIM_INVESTIGATION_DETAIL and DELETE_NHIS_CLAIM_INVESTIGATION_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisClaimInvestigationDetailModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisClaimInvestigationDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisClaimInvestigationDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
