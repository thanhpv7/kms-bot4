...
import {NhisClaimInvestigationDetailModelState, NhisClaimInvestigationDetailModelAudit} from 'src/app/models/nhisClaimInvestigationDetail/nhis_claim_investigation_detail.model.state';
...
import {
	getNhisClaimInvestigationDetailModelAuditsByEntityId,
	getNhisClaimInvestigationDetailModelWithId,
	getCountNhisClaimInvestigationDetailModels,
...
@Component({
	selector: 'cb-nhis-claim-investigation-detail-admin-crud-edit',
	templateUrl: './nhis-claim-investigation-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-claim-investigation-detail-admin-crud-edit.component.scss',
...
export class AdminNhisClaimInvestigationDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisClaimInvestigationDetailAudits$: Observable<NhisClaimInvestigationDetailModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisClaimInvestigationDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NhisClaimInvestigationDetailModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisClaimInvestigationDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisClaimInvestigationDetailModel();
		...
		if (this.targetModelId) {
			this.nhisClaimInvestigationDetailAudits$ = this.store.select(getNhisClaimInvestigationDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisClaimInvestigationDetailModel> = {
		...
		this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
			modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_NHIS_CLAIM_INVESTIGATION_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisClaimInvestigationDetailModelWithId, this.targetModelId)
			...
			model = new NhisClaimInvestigationDetailModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
			modelAction.NhisClaimInvestigationDetailModelActionTypes.FETCH_NHIS_CLAIM_INVESTIGATION_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
				modelAction.NhisClaimInvestigationDetailModelActionTypes.CREATE_NHIS_CLAIM_INVESTIGATION_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisClaimInvestigationDetailAction(
				modelAction.NhisClaimInvestigationDetailModelActionTypes.UPDATE_NHIS_CLAIM_INVESTIGATION_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisClaimInvestigationDetailModel = new NhisClaimInvestigationDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisClaimInvestigationDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisClaimInvestigationDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisClaimInvestigationDetailModel.getProps();
		this.modelRelations = NhisClaimInvestigationDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisClaimInvestigationDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisClaimInvestigationDetailModel {
		let updatedModel: NhisClaimInvestigationDetailModel = new NhisClaimInvestigationDetailModel(model);
