...
import {AdminRetailPharmacyInvoiceCrudListComponent} from './retail-pharmacy-invoice-admin-crud-list.component';
...
import {AdminRetailPharmacyInvoiceCrudModule} from '../retail-pharmacy-invoice-admin-crud.module';
import {
 ...
 */
describe('Retail Pharmacy Invoice Crud List Tile Component', () => {

	let fixture;
	let retailPharmacyInvoiceCrudListComponent: AdminRetailPharmacyInvoiceCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<RetailPharmacyInvoiceModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '54ceb842-ac0f-4d3f-aade-072ee2d2c332';
	const searchContent = 'text';

	const searchableFieldCount = RetailPharmacyInvoiceModel.searchFields.length;

	const routerState: RouterState = {
		url: 'retail-pharmacy-invoice-admin-crud',
		urls: ['retail-pharmacy-invoice-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const retailPharmacyInvoiceDataFactory = new RetailPharmacyInvoiceDataFactory();

	const defaultDataSize = 30;
	const defaultData = retailPharmacyInvoiceDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getRetailPharmacyInvoiceCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getRetailPharmacyInvoiceCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminRetailPharmacyInvoiceCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminRetailPharmacyInvoiceCrudListComponent);
			retailPharmacyInvoiceCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(retailPharmacyInvoiceCrudListComponent.retailPharmacyInvoicesCount).toBe(defaultData.length);
			...
			expect(retailPharmacyInvoiceCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			retailPharmacyInvoiceCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = retailPharmacyInvoiceDataFactory.createAll(filteredDataSize);
			...
			expect(retailPharmacyInvoiceCrudListComponent.retailPharmacyInvoicesCount).toBe(filteredDataSize);
			expect(retailPharmacyInvoiceCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(retailPharmacyInvoiceCrudListComponent.pageIndex).toBe(0);
			expect(retailPharmacyInvoiceCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			retailPharmacyInvoiceCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		retailPharmacyInvoiceCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		retailPharmacyInvoiceCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
