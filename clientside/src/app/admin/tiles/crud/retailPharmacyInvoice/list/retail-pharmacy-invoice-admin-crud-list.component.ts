...
import {RetailPharmacyInvoiceModelState} from 'src/app/models/retailPharmacyInvoice/retail_pharmacy_invoice.model.state';
...
import {
	...
	getRetailPharmacyInvoiceCollectionCount
...
@Component({
	selector: 'cb-retail-pharmacy-invoice-admin-crud-list',
	templateUrl: './retail-pharmacy-invoice-admin-crud-list.component.html',
	styleUrls: [
		'./retail-pharmacy-invoice-admin-crud-list.component.scss',
...
export class AdminRetailPharmacyInvoiceCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<RetailPharmacyInvoiceModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...RetailPharmacyInvoiceModel.searchFields,
	...
	/**
	 * List of Retail Pharmacy Invoices that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	retailPharmacyInvoices: Observable<RetailPharmacyInvoiceModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	retailPharmacyInvoicesCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.retailPharmacyInvoicesCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.retailPharmacyInvoicesCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RetailPharmacyInvoiceAction(
				modelAction.RetailPharmacyInvoiceModelActionTypes.INITIALISE_RETAIL_PHARMACY_INVOICE_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.RetailPharmacyInvoiceAction(
				modelAction.RetailPharmacyInvoiceModelActionTypes.FETCH_RETAIL_PHARMACY_INVOICE_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.retailPharmacyInvoices = this.store.select(getRetailPharmacyInvoiceCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getRetailPharmacyInvoiceCollectionCount, this.collectionId).subscribe(
				(count) => this.retailPharmacyInvoicesCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: RetailPharmacyInvoiceModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.RetailPharmacyInvoiceModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.RetailPharmacyInvoiceModelActionTypes.EXPORT_ALL_RETAIL_PHARMACY_INVOICE;
		} else {
			actionType = modelAction.RetailPharmacyInvoiceModelActionTypes.EXPORT_RETAIL_PHARMACY_INVOICE_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.RetailPharmacyInvoiceAction(
				modelAction.RetailPharmacyInvoiceModelActionTypes.IMPORT_RETAIL_PHARMACY_INVOICES,
				{
					file: file,
				},
				[new modelAction.RetailPharmacyInvoiceAction(
			...
			window.open(environment.API_URL + '/api/retailpharmacyinvoice/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: RetailPharmacyInvoiceModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Retail Pharmacy Invoice entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Retail Pharmacy Invoice entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.RetailPharmacyInvoiceModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.RetailPharmacyInvoiceModelActionTypes.DELETE_RETAIL_PHARMACY_INVOICE;
						} else if (usingSelected) {
							actionType = modelAction.RetailPharmacyInvoiceModelActionTypes.DELETE_ALL_RETAIL_PHARMACY_INVOICE;
						} else {
							actionType = modelAction.RetailPharmacyInvoiceModelActionTypes.DELETE_RETAIL_PHARMACY_INVOICE_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.RetailPharmacyInvoiceAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_RETAIL_PHARMACY_INVOICE when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_RETAIL_PHARMACY_INVOICE and DELETE_RETAIL_PHARMACY_INVOICE_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = RetailPharmacyInvoiceModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.retailPharmacyInvoicesCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
