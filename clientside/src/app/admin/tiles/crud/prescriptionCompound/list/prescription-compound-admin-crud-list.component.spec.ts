...
import {AdminPrescriptionCompoundCrudListComponent} from './prescription-compound-admin-crud-list.component';
...
import {AdminPrescriptionCompoundCrudModule} from '../prescription-compound-admin-crud.module';
import {
 ...
 */
describe('Prescription Compound Crud List Tile Component', () => {

	let fixture;
	let prescriptionCompoundCrudListComponent: AdminPrescriptionCompoundCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<PrescriptionCompoundModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'f7707884-9a43-4286-8bbc-b639340feda5';
	const searchContent = 'text';

	const searchableFieldCount = PrescriptionCompoundModel.searchFields.length;

	const routerState: RouterState = {
		url: 'prescription-compound-admin-crud',
		urls: ['prescription-compound-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const prescriptionCompoundDataFactory = new PrescriptionCompoundDataFactory();

	const defaultDataSize = 30;
	const defaultData = prescriptionCompoundDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getPrescriptionCompoundCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getPrescriptionCompoundCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminPrescriptionCompoundCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminPrescriptionCompoundCrudListComponent);
			prescriptionCompoundCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(prescriptionCompoundCrudListComponent.prescriptionCompoundsCount).toBe(defaultData.length);
			...
			expect(prescriptionCompoundCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			prescriptionCompoundCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = prescriptionCompoundDataFactory.createAll(filteredDataSize);
			...
			expect(prescriptionCompoundCrudListComponent.prescriptionCompoundsCount).toBe(filteredDataSize);
			expect(prescriptionCompoundCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(prescriptionCompoundCrudListComponent.pageIndex).toBe(0);
			expect(prescriptionCompoundCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			prescriptionCompoundCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		prescriptionCompoundCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		prescriptionCompoundCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
