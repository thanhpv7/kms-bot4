...
import {PrescriptionCompoundModelState} from 'src/app/models/prescriptionCompound/prescription_compound.model.state';
...
import {
	...
	getPrescriptionCompoundCollectionCount
...
@Component({
	selector: 'cb-prescription-compound-admin-crud-list',
	templateUrl: './prescription-compound-admin-crud-list.component.html',
	styleUrls: [
		'./prescription-compound-admin-crud-list.component.scss',
...
export class AdminPrescriptionCompoundCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<PrescriptionCompoundModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...PrescriptionCompoundModel.searchFields,
	...
	/**
	 * List of Prescription Compounds that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	prescriptionCompounds: Observable<PrescriptionCompoundModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	prescriptionCompoundsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.prescriptionCompoundsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.prescriptionCompoundsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PrescriptionCompoundAction(
				modelAction.PrescriptionCompoundModelActionTypes.INITIALISE_PRESCRIPTION_COMPOUND_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.PrescriptionCompoundAction(
				modelAction.PrescriptionCompoundModelActionTypes.FETCH_PRESCRIPTION_COMPOUND_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.prescriptionCompounds = this.store.select(getPrescriptionCompoundCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getPrescriptionCompoundCollectionCount, this.collectionId).subscribe(
				(count) => this.prescriptionCompoundsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: PrescriptionCompoundModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.PrescriptionCompoundModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.PrescriptionCompoundModelActionTypes.EXPORT_ALL_PRESCRIPTION_COMPOUND;
		} else {
			actionType = modelAction.PrescriptionCompoundModelActionTypes.EXPORT_PRESCRIPTION_COMPOUND_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.PrescriptionCompoundAction(
				modelAction.PrescriptionCompoundModelActionTypes.IMPORT_PRESCRIPTION_COMPOUNDS,
				{
					file: file,
				},
				[new modelAction.PrescriptionCompoundAction(
			...
			window.open(environment.API_URL + '/api/prescriptioncompound/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: PrescriptionCompoundModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Prescription Compound entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Prescription Compound entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.PrescriptionCompoundModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.PrescriptionCompoundModelActionTypes.DELETE_PRESCRIPTION_COMPOUND;
						} else if (usingSelected) {
							actionType = modelAction.PrescriptionCompoundModelActionTypes.DELETE_ALL_PRESCRIPTION_COMPOUND;
						} else {
							actionType = modelAction.PrescriptionCompoundModelActionTypes.DELETE_PRESCRIPTION_COMPOUND_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.PrescriptionCompoundAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_PRESCRIPTION_COMPOUND when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_PRESCRIPTION_COMPOUND and DELETE_PRESCRIPTION_COMPOUND_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = PrescriptionCompoundModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.prescriptionCompoundsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
