...
import {MedicalExaminationRecordDischargeSummaryModelState} from 'src/app/models/medicalExaminationRecordDischargeSummary/medical_examination_record_discharge_summary.model.state';
...
@Component({
	selector: 'cb-medical-examination-record-discharge-summary-admin-crud-list',
	templateUrl: './medical-examination-record-discharge-summary-admin-crud-list.component.html',
	styleUrls: [
		'./medical-examination-record-discharge-summary-admin-crud-list.component.scss',
...
export class AdminMedicalExaminationRecordDischargeSummaryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicalExaminationRecordDischargeSummaryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicalExaminationRecordDischargeSummaryModel.searchFields,
	...
	/**
	 * List of Medical Examination Record Discharge Summarys that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicalExaminationRecordDischargeSummarys: Observable<MedicalExaminationRecordDischargeSummaryModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicalExaminationRecordDischargeSummarysCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicalExaminationRecordDischargeSummarysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicalExaminationRecordDischargeSummarysCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
				modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.INITIALISE_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
				modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicalExaminationRecordDischargeSummarys = this.store.select(getMedicalExaminationRecordDischargeSummaryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicalExaminationRecordDischargeSummaryCollectionCount, this.collectionId).subscribe(
				(count) => this.medicalExaminationRecordDischargeSummarysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicalExaminationRecordDischargeSummaryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.EXPORT_ALL_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY;
		} else {
			actionType = modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.EXPORT_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
				modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.IMPORT_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARYS,
				{
					file: file,
				},
				[new modelAction.MedicalExaminationRecordDischargeSummaryAction(
			...
			window.open(environment.API_URL + '/api/medicalexaminationrecorddischargesummary/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicalExaminationRecordDischargeSummaryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medical Examination Record Discharge Summary entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medical Examination Record Discharge Summary entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.DELETE_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY;
						} else if (usingSelected) {
							actionType = modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.DELETE_ALL_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY;
						} else {
							actionType = modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.DELETE_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY and DELETE_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicalExaminationRecordDischargeSummaryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicalExaminationRecordDischargeSummarysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
