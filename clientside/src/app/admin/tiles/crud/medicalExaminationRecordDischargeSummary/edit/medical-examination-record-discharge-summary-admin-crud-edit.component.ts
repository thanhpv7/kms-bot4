...
import * as modelAction from 'src/app/models/medicalExaminationRecordDischargeSummary/medical_examination_record_discharge_summary.model.action';
import {MedicalExaminationRecordDischargeSummaryModelState, MedicalExaminationRecordDischargeSummaryModelAudit} from 'src/app/models/medicalExaminationRecordDischargeSummary/medical_examination_record_discharge_summary.model.state';
...
import {
	getMedicalExaminationRecordDischargeSummaryModelAuditsByEntityId,
	getMedicalExaminationRecordDischargeSummaryModelWithId,
	getCountMedicalExaminationRecordDischargeSummaryModels,
...
import {CrudTileMode} from '../medical-examination-record-discharge-summary-admin-crud.component';
...
@Component({
	selector: 'cb-medical-examination-record-discharge-summary-admin-crud-edit',
	templateUrl: './medical-examination-record-discharge-summary-admin-crud-edit.component.html',
	styleUrls: [
		'./medical-examination-record-discharge-summary-admin-crud-edit.component.scss',
...
export class AdminMedicalExaminationRecordDischargeSummaryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = '9f5d89c2-e746-4004-b02e-18eed5a210c8';
	 ...
	 */
	medicalExaminationRecordDischargeSummaryAudits$: Observable<MedicalExaminationRecordDischargeSummaryModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'medicalExaminationRecordDischargeSummary';
	...
	dischargingDoctorMultiCrudActive: boolean = false;
	dischargingDoctorDisplayName = 'Discharging Doctor';
	...
	dischargingDoctorOppositeRelation = 'medicalExaminationRecordDischargeSummarys';
	dischargingDoctorRelations: ModelRelation[] = this.dischargingDoctorMultiCrudActive ? StaffModel.getRelations()
		.map(relation => {
			if (relation.id === this.dischargingDoctorOppositeRelation) {
				relation.hideElement = true;
			}
			return this.prepareSingleReferenceCollection(relation);
		}) : [];
	...
	diagnosisExaminationRecordsOppositeRelation = 'medicalExaminationRecordDischargeSummary';
	 ...
	 */
	modelProperties: ModelProperty[] = MedicalExaminationRecordDischargeSummaryModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = MedicalExaminationRecordDischargeSummaryModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = MedicalExaminationRecordDischargeSummaryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'dischargingDoctor' && this.dischargingDoctorMultiCrudActive) {
	 ...
	 */
	targetModel: MedicalExaminationRecordDischargeSummaryModel;
	...
	constructor(
		private readonly store: Store<{ model: MedicalExaminationRecordDischargeSummaryModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountMedicalExaminationRecordDischargeSummaryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new MedicalExaminationRecordDischargeSummaryModel();
		...
		if (this.targetModelId) {
			this.medicalExaminationRecordDischargeSummaryAudits$ = this.store.select(getMedicalExaminationRecordDischargeSummaryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<MedicalExaminationRecordDischargeSummaryModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
			modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getMedicalExaminationRecordDischargeSummaryModelWithId, this.targetModelId)
			...
			model = new MedicalExaminationRecordDischargeSummaryModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('medical-examination-record-discharge-summary');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
			modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<MedicalExaminationRecordDischargeSummaryModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.medicalExaminationRecordMultiCrudActive || this.dischargingDoctorMultiCrudActive || this.diagnosisExaminationRecordsMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.MedicalExaminationRecordDischargeSummaryAction(
						modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.COUNT_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARYS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
				modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.CREATE_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.MedicalExaminationRecordDischargeSummaryAction(
				modelAction.MedicalExaminationRecordDischargeSummaryModelActionTypes.UPDATE_MEDICAL_EXAMINATION_RECORD_DISCHARGE_SUMMARY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: MedicalExaminationRecordDischargeSummaryModel = new MedicalExaminationRecordDischargeSummaryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getMedicalExaminationRecordDischargeSummaryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new MedicalExaminationRecordDischargeSummaryModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.dischargingDoctorDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.dischargingDoctorRelations, this.isViewOnly),
					new StaffModel(),
					StaffModel,
					this.dischargingDoctorDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.dischargingDoctorRelations,
					true
				);
				this.modelProperties = updatedProps.modelProperties;
				this.modelRelations = updatedProps.modelRelations;
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = MedicalExaminationRecordDischargeSummaryModel.getProps();
		this.modelRelations = MedicalExaminationRecordDischargeSummaryModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'dischargingDoctor';
		}).hideElement = this.dischargingDoctorMultiCrudActive || this.modelRelations.find(rel => rel.id === 'dischargingDoctor').hideElement;
	...
	initialiseMultiEntityCollections() {
		...
		if (this.dischargingDoctorMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.dischargingDoctorDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		...
		if (this.targetModel.dischargingDoctor && this.targetModel.dischargingDoctor.id && this.dischargingDoctorMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				StaffModel,
				this.dischargingDoctorOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.dischargingDoctor.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getStaffModelWithId, this.targetModel.dischargingDoctorId).subscribe(dischargingDoctor => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(StaffModel.getProps(), this.dischargingDoctorRelations, this.isViewOnly),
					dischargingDoctor,
					StaffModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: MedicalExaminationRecordDischargeSummaryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): MedicalExaminationRecordDischargeSummaryModel {
		let updatedModel: MedicalExaminationRecordDischargeSummaryModel = new MedicalExaminationRecordDischargeSummaryModel(model);
		...
		if (this.dischargingDoctorMultiCrudActive) {
			updatedModel.dischargingDoctor = event.additionalEntities.dischargingDoctor ? event.additionalEntities.dischargingDoctor : undefined;

			if (!updatedModel.dischargingDoctor && event.payload) {
				event.payload.dischargingDoctorId = null;
			}
		} else if (event.payload?.dischargingDoctorId !== undefined) {
			event.payload.dischargingDoctor = null;
			updatedModel.dischargingDoctor = null;
		}
