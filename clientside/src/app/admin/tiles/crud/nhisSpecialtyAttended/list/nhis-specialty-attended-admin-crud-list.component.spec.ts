...
import {AdminNhisSpecialtyAttendedCrudListComponent} from './nhis-specialty-attended-admin-crud-list.component';
import {NhisSpecialtyAttendedDataFactory} from 'src/app/lib/utils/factories/nhis-specialty-attended-data-factory';
import {NhisSpecialtyAttendedModel} from 'src/app/models/nhisSpecialtyAttended/nhis_specialty_attended.model';
import {AdminNhisSpecialtyAttendedCrudModule} from '../nhis-specialty-attended-admin-crud.module';
import {
	NhisSpecialtyAttendedModelState,
	initialState as initialNhisSpecialtyAttendedModelState
} from 'src/app/models/nhisSpecialtyAttended/nhis_specialty_attended.model.state';
 ...
 */
describe('NHIS Specialty Attended Crud List Tile Component', () => {

	let fixture;
	let nhisSpecialtyAttendedCrudListComponent: AdminNhisSpecialtyAttendedCrudListComponent;

	let store: MockStore<{ model: NhisSpecialtyAttendedModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let nhisSpecialtyAttendedModelState: NhisSpecialtyAttendedModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<NhisSpecialtyAttendedModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '9f7f4714-8d33-4b4f-9312-ab834ab18369';
	const searchContent = 'text';

	const searchableFieldCount = NhisSpecialtyAttendedModel.searchFields.length;

	const routerState: RouterState = {
		url: 'nhis-specialty-attended-admin-crud',
		urls: ['nhis-specialty-attended-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const nhisSpecialtyAttendedDataFactory = new NhisSpecialtyAttendedDataFactory();

	const defaultDataSize = 30;
	const defaultData = nhisSpecialtyAttendedDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: NhisSpecialtyAttendedModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		nhisSpecialtyAttendedModelState = initialNhisSpecialtyAttendedModelState;
		store.setState({model: nhisSpecialtyAttendedModelState});
		...
		spyOn(store, 'select')
			.withArgs(getNhisSpecialtyAttendedCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getNhisSpecialtyAttendedCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminNhisSpecialtyAttendedCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminNhisSpecialtyAttendedCrudListComponent);
			nhisSpecialtyAttendedCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(nhisSpecialtyAttendedCrudListComponent.nhisSpecialtyAttendedsCount).toBe(defaultData.length);
			...
			expect(nhisSpecialtyAttendedCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			nhisSpecialtyAttendedCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = nhisSpecialtyAttendedDataFactory.createAll(filteredDataSize);
			...
			expect(nhisSpecialtyAttendedCrudListComponent.nhisSpecialtyAttendedsCount).toBe(filteredDataSize);
			expect(nhisSpecialtyAttendedCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(nhisSpecialtyAttendedCrudListComponent.pageIndex).toBe(0);
			expect(nhisSpecialtyAttendedCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			nhisSpecialtyAttendedCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		nhisSpecialtyAttendedCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		nhisSpecialtyAttendedCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
