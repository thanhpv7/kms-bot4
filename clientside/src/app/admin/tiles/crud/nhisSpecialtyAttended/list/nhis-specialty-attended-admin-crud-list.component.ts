...
import * as modelAction from 'src/app/models/nhisSpecialtyAttended/nhis_specialty_attended.model.action';
import {NhisSpecialtyAttendedModelState} from 'src/app/models/nhisSpecialtyAttended/nhis_specialty_attended.model.state';
...
import {
	getNhisSpecialtyAttendedCollectionModels,
	getNhisSpecialtyAttendedCollectionCount
} from 'src/app/models/nhisSpecialtyAttended/nhis_specialty_attended.model.selector';
...
import {CrudTileMode} from '../nhis-specialty-attended-admin-crud.component';
...
@Component({
	selector: 'cb-nhis-specialty-attended-admin-crud-list',
	templateUrl: './nhis-specialty-attended-admin-crud-list.component.html',
	styleUrls: [
		'./nhis-specialty-attended-admin-crud-list.component.scss',
...
export class AdminNhisSpecialtyAttendedCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<NhisSpecialtyAttendedModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...NhisSpecialtyAttendedModel.searchFields,
	...
	/**
	 * List of NHIS Specialty Attendeds that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	nhisSpecialtyAttendeds: Observable<NhisSpecialtyAttendedModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	nhisSpecialtyAttendedsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = NhisSpecialtyAttendedModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = NhisSpecialtyAttendedModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '9f7f4714-8d33-4b4f-9312-ab834ab18369';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.nhisSpecialtyAttendedsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.nhisSpecialtyAttendedsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: NhisSpecialtyAttendedModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
				modelAction.NhisSpecialtyAttendedModelActionTypes.INITIALISE_NHIS_SPECIALTY_ATTENDED_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
				modelAction.NhisSpecialtyAttendedModelActionTypes.FETCH_NHIS_SPECIALTY_ATTENDED_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.NhisSpecialtyAttendedAction(
						modelAction.NhisSpecialtyAttendedModelActionTypes.COUNT_NHIS_SPECIALTY_ATTENDEDS,
						null
					)
				]
			))
		...
		this.nhisSpecialtyAttendeds = this.store.select(getNhisSpecialtyAttendedCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getNhisSpecialtyAttendedCollectionCount, this.collectionId).subscribe(
				(count) => this.nhisSpecialtyAttendedsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('nhis-specialty-attended');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: NhisSpecialtyAttendedModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.NhisSpecialtyAttendedModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.NhisSpecialtyAttendedModelActionTypes.EXPORT_ALL_NHIS_SPECIALTY_ATTENDED;
		} else {
			actionType = modelAction.NhisSpecialtyAttendedModelActionTypes.EXPORT_NHIS_SPECIALTY_ATTENDED_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
				modelAction.NhisSpecialtyAttendedModelActionTypes.IMPORT_NHIS_SPECIALTY_ATTENDEDS,
				{
					file: file,
				},
				[new modelAction.NhisSpecialtyAttendedAction(
			...
			window.open(environment.API_URL + '/api/nhisspecialtyattended/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: NhisSpecialtyAttendedModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected NHIS Specialty Attended entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected NHIS Specialty Attended entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.NhisSpecialtyAttendedModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.NhisSpecialtyAttendedModelActionTypes.DELETE_NHIS_SPECIALTY_ATTENDED;
						} else if (usingSelected) {
							actionType = modelAction.NhisSpecialtyAttendedModelActionTypes.DELETE_ALL_NHIS_SPECIALTY_ATTENDED;
						} else {
							actionType = modelAction.NhisSpecialtyAttendedModelActionTypes.DELETE_NHIS_SPECIALTY_ATTENDED_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_NHIS_SPECIALTY_ATTENDED when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_NHIS_SPECIALTY_ATTENDED and DELETE_NHIS_SPECIALTY_ATTENDED_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<NhisSpecialtyAttendedModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = NhisSpecialtyAttendedModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.nhisSpecialtyAttendedsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
