...
import {NhisSpecialtyAttendedModelState, NhisSpecialtyAttendedModelAudit} from 'src/app/models/nhisSpecialtyAttended/nhis_specialty_attended.model.state';
import {NhisSpecialtyAttendedModel} from 'src/app/models/nhisSpecialtyAttended/nhis_specialty_attended.model';
import {
	getNhisSpecialtyAttendedModelAuditsByEntityId,
	getNhisSpecialtyAttendedModelWithId,
	getCountNhisSpecialtyAttendedModels,
...
@Component({
	selector: 'cb-nhis-specialty-attended-admin-crud-edit',
	templateUrl: './nhis-specialty-attended-admin-crud-edit.component.html',
	styleUrls: [
		'./nhis-specialty-attended-admin-crud-edit.component.scss',
...
export class AdminNhisSpecialtyAttendedCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	nhisSpecialtyAttendedAudits$: Observable<NhisSpecialtyAttendedModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = NhisSpecialtyAttendedModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: NhisSpecialtyAttendedModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountNhisSpecialtyAttendedModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new NhisSpecialtyAttendedModel();
		...
		if (this.targetModelId) {
			this.nhisSpecialtyAttendedAudits$ = this.store.select(getNhisSpecialtyAttendedModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<NhisSpecialtyAttendedModel> = {
		...
		this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
			modelAction.NhisSpecialtyAttendedModelActionTypes.FETCH_NHIS_SPECIALTY_ATTENDED,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getNhisSpecialtyAttendedModelWithId, this.targetModelId)
			...
			model = new NhisSpecialtyAttendedModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
			modelAction.NhisSpecialtyAttendedModelActionTypes.FETCH_NHIS_SPECIALTY_ATTENDED_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
				modelAction.NhisSpecialtyAttendedModelActionTypes.CREATE_NHIS_SPECIALTY_ATTENDED,
				stateConfig,
			...
			this.store.dispatch(new modelAction.NhisSpecialtyAttendedAction(
				modelAction.NhisSpecialtyAttendedModelActionTypes.UPDATE_NHIS_SPECIALTY_ATTENDED,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: NhisSpecialtyAttendedModel = new NhisSpecialtyAttendedModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getNhisSpecialtyAttendedModelWithId, this.targetModelId).subscribe(model => {
					let differences = new NhisSpecialtyAttendedModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = NhisSpecialtyAttendedModel.getProps();
		this.modelRelations = NhisSpecialtyAttendedModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: NhisSpecialtyAttendedModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): NhisSpecialtyAttendedModel {
		let updatedModel: NhisSpecialtyAttendedModel = new NhisSpecialtyAttendedModel(model);
