...
import * as modelAction from 'src/app/models/examinationItemDetail/examination_item_detail.model.action';
import {ExaminationItemDetailModelState, ExaminationItemDetailModelAudit} from 'src/app/models/examinationItemDetail/examination_item_detail.model.state';
import {ExaminationItemDetailModel} from 'src/app/models/examinationItemDetail/examination_item_detail.model';
import {
	getExaminationItemDetailModelAuditsByEntityId,
...
import {CrudTileMode} from '../examination-item-detail-admin-crud.component';
...
@Component({
	selector: 'cb-examination-item-detail-admin-crud-edit',
	templateUrl: './examination-item-detail-admin-crud-edit.component.html',
	styleUrls: [
		'./examination-item-detail-admin-crud-edit.component.scss',
...
export class AdminExaminationItemDetailCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b87428c6-7fc0-4660-8646-c3d46642d8df';
	 ...
	 */
	examinationItemDetailAudits$: Observable<ExaminationItemDetailModelAudit[]>;
	...
	examinationItemOppositeRelation = 'examinationItemDetails';
	 ...
	 */
	modelProperties: ModelProperty[] = ExaminationItemDetailModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = ExaminationItemDetailModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ExaminationItemDetailModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ExaminationItemDetailModel;
	...
	constructor(
		private readonly store: Store<{ model: ExaminationItemDetailModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountExaminationItemDetailModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ExaminationItemDetailModel();
		...
		if (this.targetModelId) {
			this.examinationItemDetailAudits$ = this.store.select(getExaminationItemDetailModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ExaminationItemDetailModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.ExaminationItemDetailAction(
			modelAction.ExaminationItemDetailModelActionTypes.FETCH_EXAMINATION_ITEM_DETAIL,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getExaminationItemDetailModelWithId, this.targetModelId)
			...
			model = new ExaminationItemDetailModel(model);
			model.syncRelations();
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('examination-item-detail');
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ExaminationItemDetailAction(
			modelAction.ExaminationItemDetailModelActionTypes.FETCH_EXAMINATION_ITEM_DETAIL_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<ExaminationItemDetailModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.examinationItemMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.ExaminationItemDetailAction(
						modelAction.ExaminationItemDetailModelActionTypes.COUNT_EXAMINATION_ITEM_DETAILS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.ExaminationItemDetailAction(
				modelAction.ExaminationItemDetailModelActionTypes.CREATE_EXAMINATION_ITEM_DETAIL,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ExaminationItemDetailAction(
				modelAction.ExaminationItemDetailModelActionTypes.UPDATE_EXAMINATION_ITEM_DETAIL,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ExaminationItemDetailModel = new ExaminationItemDetailModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getExaminationItemDetailModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ExaminationItemDetailModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ExaminationItemDetailModel.getProps();
		this.modelRelations = ExaminationItemDetailModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ExaminationItemDetailModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ExaminationItemDetailModel {
		let updatedModel: ExaminationItemDetailModel = new ExaminationItemDetailModel(model);
