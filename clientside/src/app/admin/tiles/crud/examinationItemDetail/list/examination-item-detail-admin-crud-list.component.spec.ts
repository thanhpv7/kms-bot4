...
import {AdminExaminationItemDetailCrudListComponent} from './examination-item-detail-admin-crud-list.component';
...
import {AdminExaminationItemDetailCrudModule} from '../examination-item-detail-admin-crud.module';
import {
 ...
 */
describe('Examination Item Detail Crud List Tile Component', () => {

	let fixture;
	let examinationItemDetailCrudListComponent: AdminExaminationItemDetailCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ExaminationItemDetailModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b87428c6-7fc0-4660-8646-c3d46642d8df';
	const searchContent = 'text';

	const searchableFieldCount = ExaminationItemDetailModel.searchFields.length;

	const routerState: RouterState = {
		url: 'examination-item-detail-admin-crud',
		urls: ['examination-item-detail-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const examinationItemDetailDataFactory = new ExaminationItemDetailDataFactory();

	const defaultDataSize = 30;
	const defaultData = examinationItemDetailDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getExaminationItemDetailCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getExaminationItemDetailCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminExaminationItemDetailCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminExaminationItemDetailCrudListComponent);
			examinationItemDetailCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(examinationItemDetailCrudListComponent.examinationItemDetailsCount).toBe(defaultData.length);
			...
			expect(examinationItemDetailCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			examinationItemDetailCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = examinationItemDetailDataFactory.createAll(filteredDataSize);
			...
			expect(examinationItemDetailCrudListComponent.examinationItemDetailsCount).toBe(filteredDataSize);
			expect(examinationItemDetailCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(examinationItemDetailCrudListComponent.pageIndex).toBe(0);
			expect(examinationItemDetailCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			examinationItemDetailCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		examinationItemDetailCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		examinationItemDetailCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
