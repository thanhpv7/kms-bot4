...
import {ExaminationItemDetailModelState} from 'src/app/models/examinationItemDetail/examination_item_detail.model.state';
...
import {
	...
	getExaminationItemDetailCollectionCount
...
@Component({
	selector: 'cb-examination-item-detail-admin-crud-list',
	templateUrl: './examination-item-detail-admin-crud-list.component.html',
	styleUrls: [
		'./examination-item-detail-admin-crud-list.component.scss',
...
export class AdminExaminationItemDetailCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ExaminationItemDetailModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ExaminationItemDetailModel.searchFields,
	...
	/**
	 * List of Examination Item Details that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	examinationItemDetails: Observable<ExaminationItemDetailModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	examinationItemDetailsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.examinationItemDetailsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.examinationItemDetailsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ExaminationItemDetailAction(
				modelAction.ExaminationItemDetailModelActionTypes.INITIALISE_EXAMINATION_ITEM_DETAIL_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ExaminationItemDetailAction(
				modelAction.ExaminationItemDetailModelActionTypes.FETCH_EXAMINATION_ITEM_DETAIL_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.examinationItemDetails = this.store.select(getExaminationItemDetailCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getExaminationItemDetailCollectionCount, this.collectionId).subscribe(
				(count) => this.examinationItemDetailsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ExaminationItemDetailModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ExaminationItemDetailModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ExaminationItemDetailModelActionTypes.EXPORT_ALL_EXAMINATION_ITEM_DETAIL;
		} else {
			actionType = modelAction.ExaminationItemDetailModelActionTypes.EXPORT_EXAMINATION_ITEM_DETAIL_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ExaminationItemDetailAction(
				modelAction.ExaminationItemDetailModelActionTypes.IMPORT_EXAMINATION_ITEM_DETAILS,
				{
					file: file,
				},
				[new modelAction.ExaminationItemDetailAction(
			...
			window.open(environment.API_URL + '/api/examinationitemdetail/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ExaminationItemDetailModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Examination Item Detail entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Examination Item Detail entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ExaminationItemDetailModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ExaminationItemDetailModelActionTypes.DELETE_EXAMINATION_ITEM_DETAIL;
						} else if (usingSelected) {
							actionType = modelAction.ExaminationItemDetailModelActionTypes.DELETE_ALL_EXAMINATION_ITEM_DETAIL;
						} else {
							actionType = modelAction.ExaminationItemDetailModelActionTypes.DELETE_EXAMINATION_ITEM_DETAIL_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ExaminationItemDetailAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_EXAMINATION_ITEM_DETAIL when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_EXAMINATION_ITEM_DETAIL and DELETE_EXAMINATION_ITEM_DETAIL_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ExaminationItemDetailModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.examinationItemDetailsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
