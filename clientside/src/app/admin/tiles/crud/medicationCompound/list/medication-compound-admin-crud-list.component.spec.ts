...
import {AdminMedicationCompoundCrudListComponent} from './medication-compound-admin-crud-list.component';
...
import {AdminMedicationCompoundCrudModule} from '../medication-compound-admin-crud.module';
import {
 ...
 */
describe('Medication Compound Crud List Tile Component', () => {

	let fixture;
	let medicationCompoundCrudListComponent: AdminMedicationCompoundCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<MedicationCompoundModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'ebf0732e-7161-4b0a-b28e-a7c24f43ad1a';
	const searchContent = 'text';

	const searchableFieldCount = MedicationCompoundModel.searchFields.length;

	const routerState: RouterState = {
		url: 'medication-compound-admin-crud',
		urls: ['medication-compound-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const medicationCompoundDataFactory = new MedicationCompoundDataFactory();

	const defaultDataSize = 30;
	const defaultData = medicationCompoundDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getMedicationCompoundCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getMedicationCompoundCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminMedicationCompoundCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminMedicationCompoundCrudListComponent);
			medicationCompoundCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(medicationCompoundCrudListComponent.medicationCompoundsCount).toBe(defaultData.length);
			...
			expect(medicationCompoundCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			medicationCompoundCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = medicationCompoundDataFactory.createAll(filteredDataSize);
			...
			expect(medicationCompoundCrudListComponent.medicationCompoundsCount).toBe(filteredDataSize);
			expect(medicationCompoundCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(medicationCompoundCrudListComponent.pageIndex).toBe(0);
			expect(medicationCompoundCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			medicationCompoundCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		medicationCompoundCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		medicationCompoundCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
