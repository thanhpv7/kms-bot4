...
import {MedicationCompoundModelState} from 'src/app/models/medicationCompound/medication_compound.model.state';
...
import {
	...
	getMedicationCompoundCollectionCount
...
import {CrudTileMode} from '../medication-compound-admin-crud.component';
...
@Component({
	selector: 'cb-medication-compound-admin-crud-list',
	templateUrl: './medication-compound-admin-crud-list.component.html',
	styleUrls: [
		'./medication-compound-admin-crud-list.component.scss',
...
export class AdminMedicationCompoundCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<MedicationCompoundModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...MedicationCompoundModel.searchFields,
	...
	/**
	 * List of Medication Compounds that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	medicationCompounds: Observable<MedicationCompoundModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	medicationCompoundsCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.medicationCompoundsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.medicationCompoundsCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationCompoundAction(
				modelAction.MedicationCompoundModelActionTypes.INITIALISE_MEDICATION_COMPOUND_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.MedicationCompoundAction(
				modelAction.MedicationCompoundModelActionTypes.FETCH_MEDICATION_COMPOUND_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.medicationCompounds = this.store.select(getMedicationCompoundCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getMedicationCompoundCollectionCount, this.collectionId).subscribe(
				(count) => this.medicationCompoundsCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: MedicationCompoundModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.MedicationCompoundModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.MedicationCompoundModelActionTypes.EXPORT_ALL_MEDICATION_COMPOUND;
		} else {
			actionType = modelAction.MedicationCompoundModelActionTypes.EXPORT_MEDICATION_COMPOUND_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.MedicationCompoundAction(
				modelAction.MedicationCompoundModelActionTypes.IMPORT_MEDICATION_COMPOUNDS,
				{
					file: file,
				},
				[new modelAction.MedicationCompoundAction(
			...
			window.open(environment.API_URL + '/api/medicationcompound/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: MedicationCompoundModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Medication Compound entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Medication Compound entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.MedicationCompoundModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.MedicationCompoundModelActionTypes.DELETE_MEDICATION_COMPOUND;
						} else if (usingSelected) {
							actionType = modelAction.MedicationCompoundModelActionTypes.DELETE_ALL_MEDICATION_COMPOUND;
						} else {
							actionType = modelAction.MedicationCompoundModelActionTypes.DELETE_MEDICATION_COMPOUND_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.MedicationCompoundAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_MEDICATION_COMPOUND when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_MEDICATION_COMPOUND and DELETE_MEDICATION_COMPOUND_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = MedicationCompoundModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.medicationCompoundsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
