...
import * as modelAction from 'src/app/models/birthHistory/birth_history.model.action';
import {BirthHistoryModelState} from 'src/app/models/birthHistory/birth_history.model.state';
...
import {CrudTileMode} from '../birth-history-admin-crud.component';
...
@Component({
	selector: 'cb-birth-history-admin-crud-list',
	templateUrl: './birth-history-admin-crud-list.component.html',
	styleUrls: [
		'./birth-history-admin-crud-list.component.scss',
...
export class AdminBirthHistoryCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<BirthHistoryModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...BirthHistoryModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = BirthHistoryModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '533ead96-9dd8-4b7b-ab41-4c37b5fe1060';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.birthHistorysCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.birthHistorysCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: BirthHistoryModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BirthHistoryAction(
				modelAction.BirthHistoryModelActionTypes.INITIALISE_BIRTH_HISTORY_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.BirthHistoryAction(
				modelAction.BirthHistoryModelActionTypes.FETCH_BIRTH_HISTORY_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.BirthHistoryAction(
						modelAction.BirthHistoryModelActionTypes.COUNT_BIRTH_HISTORYS,
						null
					)
				]
			))
		...
		this.birthHistorys = this.store.select(getBirthHistoryCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getBirthHistoryCollectionCount, this.collectionId).subscribe(
				(count) => this.birthHistorysCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: BirthHistoryModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.BirthHistoryModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.BirthHistoryModelActionTypes.EXPORT_ALL_BIRTH_HISTORY;
		} else {
			actionType = modelAction.BirthHistoryModelActionTypes.EXPORT_BIRTH_HISTORY_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.BirthHistoryAction(
				modelAction.BirthHistoryModelActionTypes.IMPORT_BIRTH_HISTORYS,
				{
					file: file,
				},
				[new modelAction.BirthHistoryAction(
			...
			window.open(environment.API_URL + '/api/birthhistory/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: BirthHistoryModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Birth History entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Birth History entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.BirthHistoryModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.BirthHistoryModelActionTypes.DELETE_BIRTH_HISTORY;
						} else if (usingSelected) {
							actionType = modelAction.BirthHistoryModelActionTypes.DELETE_ALL_BIRTH_HISTORY;
						} else {
							actionType = modelAction.BirthHistoryModelActionTypes.DELETE_BIRTH_HISTORY_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.BirthHistoryAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_BIRTH_HISTORY when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_BIRTH_HISTORY and DELETE_BIRTH_HISTORY_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<BirthHistoryModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = BirthHistoryModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.birthHistorysCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
