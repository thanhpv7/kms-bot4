...
import {BirthHistoryModelState, BirthHistoryModelAudit} from 'src/app/models/birthHistory/birth_history.model.state';
import {BirthHistoryModel} from 'src/app/models/birthHistory/birth_history.model';
import {
	getBirthHistoryModelAuditsByEntityId,
	getBirthHistoryModelWithId,
	getCountBirthHistoryModels,
...
@Component({
	selector: 'cb-birth-history-admin-crud-edit',
	templateUrl: './birth-history-admin-crud-edit.component.html',
	styleUrls: [
		'./birth-history-admin-crud-edit.component.scss',
...
export class AdminBirthHistoryCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	birthHistoryAudits$: Observable<BirthHistoryModelAudit[]>;
	...
	medicalExaminationRecordOppositeRelation = 'birthHistory';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = BirthHistoryModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountBirthHistoryModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new BirthHistoryModel();
		...
		if (this.targetModelId) {
			this.birthHistoryAudits$ = this.store.select(getBirthHistoryModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<BirthHistoryModel> = {
		...
		this.store.dispatch(new modelAction.BirthHistoryAction(
			modelAction.BirthHistoryModelActionTypes.FETCH_BIRTH_HISTORY,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getBirthHistoryModelWithId, this.targetModelId)
			...
			model = new BirthHistoryModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.BirthHistoryAction(
			modelAction.BirthHistoryModelActionTypes.FETCH_BIRTH_HISTORY_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.BirthHistoryAction(
				modelAction.BirthHistoryModelActionTypes.CREATE_BIRTH_HISTORY,
				stateConfig,
			...
			this.store.dispatch(new modelAction.BirthHistoryAction(
				modelAction.BirthHistoryModelActionTypes.UPDATE_BIRTH_HISTORY,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: BirthHistoryModel = new BirthHistoryModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getBirthHistoryModelWithId, this.targetModelId).subscribe(model => {
					let differences = new BirthHistoryModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = BirthHistoryModel.getProps();
		this.modelRelations = BirthHistoryModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: BirthHistoryModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): BirthHistoryModel {
		let updatedModel: BirthHistoryModel = new BirthHistoryModel(model);
