...
import {ReferenceHeaderModelState, ReferenceHeaderModelAudit} from 'src/app/models/referenceHeader/reference_header.model.state';
import {ReferenceHeaderModel} from 'src/app/models/referenceHeader/reference_header.model';
import {
	getReferenceHeaderModelAuditsByEntityId,
...
@Component({
	selector: 'cb-reference-header-admin-crud-edit',
	templateUrl: './reference-header-admin-crud-edit.component.html',
	styleUrls: [
		'./reference-header-admin-crud-edit.component.scss',
...
export class AdminReferenceHeaderCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	referenceHeaderAudits$: Observable<ReferenceHeaderModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = ReferenceHeaderModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: ReferenceHeaderModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountReferenceHeaderModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new ReferenceHeaderModel();
		...
		if (this.targetModelId) {
			this.referenceHeaderAudits$ = this.store.select(getReferenceHeaderModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<ReferenceHeaderModel> = {
		...
		this.store.dispatch(new modelAction.ReferenceHeaderAction(
			modelAction.ReferenceHeaderModelActionTypes.FETCH_REFERENCE_HEADER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getReferenceHeaderModelWithId, this.targetModelId)
			...
			model = new ReferenceHeaderModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.ReferenceHeaderAction(
			modelAction.ReferenceHeaderModelActionTypes.FETCH_REFERENCE_HEADER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.ReferenceHeaderAction(
				modelAction.ReferenceHeaderModelActionTypes.CREATE_REFERENCE_HEADER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.ReferenceHeaderAction(
				modelAction.ReferenceHeaderModelActionTypes.UPDATE_REFERENCE_HEADER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: ReferenceHeaderModel = new ReferenceHeaderModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getReferenceHeaderModelWithId, this.targetModelId).subscribe(model => {
					let differences = new ReferenceHeaderModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = ReferenceHeaderModel.getProps();
		this.modelRelations = ReferenceHeaderModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: ReferenceHeaderModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): ReferenceHeaderModel {
		let updatedModel: ReferenceHeaderModel = new ReferenceHeaderModel(model);
