...
import * as modelAction from 'src/app/models/referenceHeader/reference_header.model.action';
import {ReferenceHeaderModelState} from 'src/app/models/referenceHeader/reference_header.model.state';
...
import {
	...
	getReferenceHeaderCollectionCount
...
import {CrudTileMode} from '../reference-header-admin-crud.component';
...
@Component({
	selector: 'cb-reference-header-admin-crud-list',
	templateUrl: './reference-header-admin-crud-list.component.html',
	styleUrls: [
		'./reference-header-admin-crud-list.component.scss',
...
export class AdminReferenceHeaderCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<ReferenceHeaderModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...ReferenceHeaderModel.searchFields,
	 ...
	 */
	modelRelations: ModelRelation[] = ReferenceHeaderModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '93d6d3b2-dc17-42ab-a6b6-7493f9fd2d54';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.referenceHeadersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.referenceHeadersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: ReferenceHeaderModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReferenceHeaderAction(
				modelAction.ReferenceHeaderModelActionTypes.INITIALISE_REFERENCE_HEADER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.ReferenceHeaderAction(
				modelAction.ReferenceHeaderModelActionTypes.FETCH_REFERENCE_HEADER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.ReferenceHeaderAction(
						modelAction.ReferenceHeaderModelActionTypes.COUNT_REFERENCE_HEADERS,
						null
					)
				]
			))
		...
		this.referenceHeaders = this.store.select(getReferenceHeaderCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getReferenceHeaderCollectionCount, this.collectionId).subscribe(
				(count) => this.referenceHeadersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('reference-header');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: ReferenceHeaderModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.ReferenceHeaderModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.ReferenceHeaderModelActionTypes.EXPORT_ALL_REFERENCE_HEADER;
		} else {
			actionType = modelAction.ReferenceHeaderModelActionTypes.EXPORT_REFERENCE_HEADER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.ReferenceHeaderAction(
				modelAction.ReferenceHeaderModelActionTypes.IMPORT_REFERENCE_HEADERS,
				{
					file: file,
				},
				[new modelAction.ReferenceHeaderAction(
			...
			window.open(environment.API_URL + '/api/referenceheader/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: ReferenceHeaderModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Reference Header entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Reference Header entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.ReferenceHeaderModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.ReferenceHeaderModelActionTypes.DELETE_REFERENCE_HEADER;
						} else if (usingSelected) {
							actionType = modelAction.ReferenceHeaderModelActionTypes.DELETE_ALL_REFERENCE_HEADER;
						} else {
							actionType = modelAction.ReferenceHeaderModelActionTypes.DELETE_REFERENCE_HEADER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.ReferenceHeaderAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_REFERENCE_HEADER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_REFERENCE_HEADER and DELETE_REFERENCE_HEADER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<ReferenceHeaderModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = ReferenceHeaderModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.referenceHeadersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
