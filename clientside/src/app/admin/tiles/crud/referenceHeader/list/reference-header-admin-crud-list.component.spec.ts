...
import {AdminReferenceHeaderCrudListComponent} from './reference-header-admin-crud-list.component';
...
import {AdminReferenceHeaderCrudModule} from '../reference-header-admin-crud.module';
import {
 ...
 */
describe('Reference Header Crud List Tile Component', () => {

	let fixture;
	let referenceHeaderCrudListComponent: AdminReferenceHeaderCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<ReferenceHeaderModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '93d6d3b2-dc17-42ab-a6b6-7493f9fd2d54';
	const searchContent = 'text';

	const searchableFieldCount = ReferenceHeaderModel.searchFields.length;

	const routerState: RouterState = {
		url: 'reference-header-admin-crud',
		urls: ['reference-header-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const referenceHeaderDataFactory = new ReferenceHeaderDataFactory();

	const defaultDataSize = 30;
	const defaultData = referenceHeaderDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getReferenceHeaderCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getReferenceHeaderCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminReferenceHeaderCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminReferenceHeaderCrudListComponent);
			referenceHeaderCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(referenceHeaderCrudListComponent.referenceHeadersCount).toBe(defaultData.length);
			...
			expect(referenceHeaderCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			referenceHeaderCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = referenceHeaderDataFactory.createAll(filteredDataSize);
			...
			expect(referenceHeaderCrudListComponent.referenceHeadersCount).toBe(filteredDataSize);
			expect(referenceHeaderCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(referenceHeaderCrudListComponent.pageIndex).toBe(0);
			expect(referenceHeaderCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			referenceHeaderCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		referenceHeaderCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		referenceHeaderCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
