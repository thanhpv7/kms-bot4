...
import * as modelAction from 'src/app/models/vitalSigns/vital_signs.model.action';
import {VitalSignsModelState, VitalSignsModelAudit} from 'src/app/models/vitalSigns/vital_signs.model.state';
import {VitalSignsModel} from 'src/app/models/vitalSigns/vital_signs.model';
import {
	getVitalSignsModelAuditsByEntityId,
...
import {CrudTileMode} from '../vital-signs-admin-crud.component';
...
@Component({
	selector: 'cb-vital-signs-admin-crud-edit',
	templateUrl: './vital-signs-admin-crud-edit.component.html',
	styleUrls: [
		'./vital-signs-admin-crud-edit.component.scss',
...
export class AdminVitalSignsCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	@Input()
	collectionId: string = 'b08efa2f-98f7-4ffb-93c0-85b99b47c138';
	 ...
	 */
	vitalSignsAudits$: Observable<VitalSignsModelAudit[]>;
	...
	newbornDetailMultiCrudActive: boolean = false;
	newbornDetailDisplayName = 'Newborn Detail';
	...
	newbornDetailOppositeRelation = 'vitalSigns';
	newbornDetailRelations: ModelRelation[] = this.newbornDetailMultiCrudActive ? NewbornDetailModel.getRelations()
		.map(relation => {
			if (relation.id === this.newbornDetailOppositeRelation) {
	 ...
	 */
	modelProperties: ModelProperty[] = VitalSignsModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = VitalSignsModel.getRelations();
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = VitalSignsModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
				...
				if (relation.id === 'newbornDetail' && this.newbornDetailMultiCrudActive) {
	...
	constructor(
		private readonly store: Store<{ model: VitalSignsModelState }>,
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountVitalSignsModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new VitalSignsModel();
		...
		if (this.targetModelId) {
			this.vitalSignsAudits$ = this.store.select(getVitalSignsModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<VitalSignsModel> = {
		...
		// Fetch models
		this.store.dispatch(new modelAction.VitalSignsAction(
			modelAction.VitalSignsModelActionTypes.FETCH_VITAL_SIGNS,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getVitalSignsModelWithId, this.targetModelId)
			...
			model = new VitalSignsModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.VitalSignsAction(
			modelAction.VitalSignsModelActionTypes.FETCH_VITAL_SIGNS_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			let stateConfig: PassableStateConfig<VitalSignsModel> = {
				targetModel: this.targetModel,
				collectionId: this.collectionId,
				queryParams: {
					persistRelations: this.newbornDetailMultiCrudActive,
				},
			...
			if (this.singlePageComponent) {
				afterwardActions = [
					...
					new modelAction.VitalSignsAction(
						modelAction.VitalSignsModelActionTypes.COUNT_VITAL_SIGNSS,
						null
					)
				];
			} else {
			...
			this.store.dispatch(new modelAction.VitalSignsAction(
				modelAction.VitalSignsModelActionTypes.CREATE_VITAL_SIGNS,
				stateConfig,
			...
			this.store.dispatch(new modelAction.VitalSignsAction(
				modelAction.VitalSignsModelActionTypes.UPDATE_VITAL_SIGNS,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: VitalSignsModel = new VitalSignsModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getVitalSignsModelWithId, this.targetModelId).subscribe(model => {
					let differences = new VitalSignsModel(model).difference(updatedModel);
	...
	onMultiEntityAction(event: { groupId: string, entityName: string, payload?: { [s: string]: any } }) {
		if (event.groupId) { // This path deletes the group with the specified id from the list
			...
			if (event.entityName === this.newbornDetailDisplayName) {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(NewbornDetailModel.getProps(), this.newbornDetailRelations, this.isViewOnly),
					new NewbornDetailModel(),
					NewbornDetailModel,
					this.newbornDetailDisplayName,
					this.modelProperties,
					this.modelRelations,
					this.newbornDetailRelations,
					true
				);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = VitalSignsModel.getProps();
		this.modelRelations = VitalSignsModel.getRelations();
		...
		this.modelRelations.find(rel => {
			return rel.id === 'newbornDetail';
		}).hideElement = this.newbornDetailMultiCrudActive || this.modelRelations.find(rel => rel.id === 'newbornDetail').hideElement;
	...
	initialiseMultiEntityCollections() {
		if (this.newbornDetailMultiCrudActive) {
			this.defaultMultiEntityButtons.push({name: this.newbornDetailDisplayName, many: false});
		}
	...
	multiCrudFetchAdditionalEntities() {
		if (this.targetModel.newbornDetail && this.targetModel.newbornDetail.id && this.newbornDetailMultiCrudActive) {
			this.store.dispatch(multiCrudExtraEntityAction(
				NewbornDetailModel,
				this.newbornDetailOppositeRelation,
				this.collectionId,
				1,
				this.targetModel.newbornDetail.id,
				false
			));

			this.creationNavigationSubscription$.add(this.store.select(getNewbornDetailModelWithId, this.targetModel.newbornDetailId).subscribe(newbornDetail => {
				let updatedProps = getNewModelProperties(
					createReactiveFormFromModel(NewbornDetailModel.getProps(), this.newbornDetailRelations, this.isViewOnly),
					newbornDetail,
					NewbornDetailModel,
	 ...
	 */
	addAdditionalEntitiesToModel(model: VitalSignsModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): VitalSignsModel {
		let updatedModel: VitalSignsModel = new VitalSignsModel(model);

		if (this.newbornDetailMultiCrudActive) {
			updatedModel.newbornDetail = event.additionalEntities.newbornDetail ? event.additionalEntities.newbornDetail : undefined;

			if (!updatedModel.newbornDetail && event.payload) {
				event.payload.newbornDetailId = null;
			}
		} else if (event.payload?.newbornDetailId !== undefined) {
			event.payload.newbornDetail = null;
			updatedModel.newbornDetail = null;
		}
