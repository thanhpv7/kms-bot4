...
import {VitalSignsModelState} from 'src/app/models/vitalSigns/vital_signs.model.state';
...
@Component({
	selector: 'cb-vital-signs-admin-crud-list',
	templateUrl: './vital-signs-admin-crud-list.component.html',
	styleUrls: [
		'./vital-signs-admin-crud-list.component.scss',
...
export class AdminVitalSignsCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<VitalSignsModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...VitalSignsModel.searchFields,
	...
	/**
	 * List of Vital Signss that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	vitalSignss: Observable<VitalSignsModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	vitalSignssCount: number = 0;
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.vitalSignssCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.vitalSignssCount === 0
		},
		{
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.VitalSignsAction(
				modelAction.VitalSignsModelActionTypes.INITIALISE_VITAL_SIGNS_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.VitalSignsAction(
				modelAction.VitalSignsModelActionTypes.FETCH_VITAL_SIGNS_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
		...
		this.vitalSignss = this.store.select(getVitalSignsCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getVitalSignsCollectionCount, this.collectionId).subscribe(
				(count) => this.vitalSignssCount = count
			)
		);
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: VitalSignsModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.VitalSignsModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.VitalSignsModelActionTypes.EXPORT_ALL_VITAL_SIGNS;
		} else {
			actionType = modelAction.VitalSignsModelActionTypes.EXPORT_VITAL_SIGNS_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.VitalSignsAction(
				modelAction.VitalSignsModelActionTypes.IMPORT_VITAL_SIGNSS,
				{
					file: file,
				},
				[new modelAction.VitalSignsAction(
			...
			window.open(environment.API_URL + '/api/vitalsigns/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: VitalSignsModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Vital Signs entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Vital Signs entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.VitalSignsModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.VitalSignsModelActionTypes.DELETE_VITAL_SIGNS;
						} else if (usingSelected) {
							actionType = modelAction.VitalSignsModelActionTypes.DELETE_ALL_VITAL_SIGNS;
						} else {
							actionType = modelAction.VitalSignsModelActionTypes.DELETE_VITAL_SIGNS_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.VitalSignsAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_VITAL_SIGNS when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_VITAL_SIGNS and DELETE_VITAL_SIGNS_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = VitalSignsModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.vitalSignssCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
