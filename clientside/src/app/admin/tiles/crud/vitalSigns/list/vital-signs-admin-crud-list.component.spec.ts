...
import {AdminVitalSignsCrudListComponent} from './vital-signs-admin-crud-list.component';
...
import {AdminVitalSignsCrudModule} from '../vital-signs-admin-crud.module';
import {
 ...
 */
describe('Vital Signs Crud List Tile Component', () => {

	let fixture;
	let vitalSignsCrudListComponent: AdminVitalSignsCrudListComponent;
	...
	let collectionModelsBehaviorSubject: BehaviorSubject<VitalSignsModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = 'b08efa2f-98f7-4ffb-93c0-85b99b47c138';
	const searchContent = 'text';

	const searchableFieldCount = VitalSignsModel.searchFields.length;

	const routerState: RouterState = {
		url: 'vital-signs-admin-crud',
		urls: ['vital-signs-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const vitalSignsDataFactory = new VitalSignsDataFactory();

	const defaultDataSize = 30;
	const defaultData = vitalSignsDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		spyOn(store, 'select')
			.withArgs(getVitalSignsCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getVitalSignsCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			...
			fixture = TestBed.createComponent(AdminVitalSignsCrudListComponent);
			vitalSignsCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(vitalSignsCrudListComponent.vitalSignssCount).toBe(defaultData.length);
			...
			expect(vitalSignsCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			vitalSignsCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = vitalSignsDataFactory.createAll(filteredDataSize);
			...
			expect(vitalSignsCrudListComponent.vitalSignssCount).toBe(filteredDataSize);
			expect(vitalSignsCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(vitalSignsCrudListComponent.pageIndex).toBe(0);
			expect(vitalSignsCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			vitalSignsCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		vitalSignsCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		vitalSignsCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
