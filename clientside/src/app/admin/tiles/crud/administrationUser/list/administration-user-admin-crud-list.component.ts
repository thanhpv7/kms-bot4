...
import * as modelAction from 'src/app/models/administrationUser/administration_user.model.action';
import {AdministrationUserModelState} from 'src/app/models/administrationUser/administration_user.model.state';
...
import {
	getAdministrationUserCollectionModels,
	getAdministrationUserCollectionCount
...
import {CrudTileMode} from '../administration-user-admin-crud.component';
...
@Component({
	selector: 'cb-administration-user-admin-crud-list',
	templateUrl: './administration-user-admin-crud-list.component.html',
	styleUrls: [
		'./administration-user-admin-crud-list.component.scss',
...
export class AdminAdministrationUserCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<AdministrationUserModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...AdministrationUserModel.searchFields,
	...
	/**
	 * List of Administration Users that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	administrationUsers: Observable<AdministrationUserModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	administrationUsersCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = AdministrationUserModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = AdministrationUserModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = '010e653a-cf8a-4987-a742-8d062f3ad9ce';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.administrationUsersCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.administrationUsersCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: AdministrationUserModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.AdministrationUserAction(
				modelAction.AdministrationUserModelActionTypes.INITIALISE_ADMINISTRATION_USER_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.AdministrationUserAction(
				modelAction.AdministrationUserModelActionTypes.FETCH_ADMINISTRATION_USER_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.AdministrationUserAction(
						modelAction.AdministrationUserModelActionTypes.COUNT_ADMINISTRATION_USERS,
						null
					)
				]
			))
		...
		this.administrationUsers = this.store.select(getAdministrationUserCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getAdministrationUserCollectionCount, this.collectionId).subscribe(
				(count) => this.administrationUsersCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('admin');
			commands.push('users');
			commands.push('administration-user');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: AdministrationUserModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.AdministrationUserModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.AdministrationUserModelActionTypes.EXPORT_ALL_ADMINISTRATION_USER;
		} else {
			actionType = modelAction.AdministrationUserModelActionTypes.EXPORT_ADMINISTRATION_USER_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.AdministrationUserAction(
				modelAction.AdministrationUserModelActionTypes.IMPORT_ADMINISTRATION_USERS,
				{
					file: file,
				},
				[new modelAction.AdministrationUserAction(
			...
			window.open(environment.API_URL + '/api/administrationuser/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: AdministrationUserModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Administration User entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Administration User entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.AdministrationUserModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.AdministrationUserModelActionTypes.DELETE_ADMINISTRATION_USER;
						} else if (usingSelected) {
							actionType = modelAction.AdministrationUserModelActionTypes.DELETE_ALL_ADMINISTRATION_USER;
						} else {
							actionType = modelAction.AdministrationUserModelActionTypes.DELETE_ADMINISTRATION_USER_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.AdministrationUserAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_ADMINISTRATION_USER when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_ADMINISTRATION_USER and DELETE_ADMINISTRATION_USER_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<AdministrationUserModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = AdministrationUserModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.administrationUsersCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
