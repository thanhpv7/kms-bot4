...
import {AdminAdministrationUserCrudListComponent} from './administration-user-admin-crud-list.component';
...
import {AdminAdministrationUserCrudModule} from '../administration-user-admin-crud.module';
import {
	AdministrationUserModelState,
	initialState as initialAdministrationUserModelState
} from 'src/app/models/administrationUser/administration_user.model.state';
 ...
 */
describe('Administration User Crud List Tile Component', () => {

	let fixture;
	let administrationUserCrudListComponent: AdminAdministrationUserCrudListComponent;

	let store: MockStore<{ model: AdministrationUserModelState }>;
	let routerStore: MockStore<{router: RouterState}>;

	let administrationUserModelState: AdministrationUserModelState;

	let collectionCountBehaviorSubject: BehaviorSubject<number>;
	let collectionModelsBehaviorSubject: BehaviorSubject<AdministrationUserModel[]>;
	let routerStateBehaviorSubject: BehaviorSubject<RouterState>;

	const collectionId = '010e653a-cf8a-4987-a742-8d062f3ad9ce';
	const searchContent = 'text';

	const searchableFieldCount = AdministrationUserModel.searchFields.length;

	const routerState: RouterState = {
		url: 'administration-user-admin-crud',
		urls: ['administration-user-admin-crud'],
		params: {},
		queryParams: {},
		data: {},
	};

	const administrationUserDataFactory = new AdministrationUserDataFactory();

	const defaultDataSize = 30;
	const defaultData = administrationUserDataFactory.createAll(defaultDataSize);
	...
	function spySelectorsInStore()  {
		...
		store = TestBed.inject(Store) as MockStore<{ model: AdministrationUserModelState }>;
		routerStore = TestBed.inject(Store) as MockStore<{ router: RouterState }>;

		administrationUserModelState = initialAdministrationUserModelState;
		store.setState({model: administrationUserModelState});
		...
		spyOn(store, 'select')
			.withArgs(getAdministrationUserCollectionCount, collectionId).and.returnValue(collectionCountBehaviorSubject)
			.withArgs(getAdministrationUserCollectionModels, collectionId).and.returnValue(collectionModelsBehaviorSubject)
	...
	beforeEach(waitForAsync (() => {
		...
		TestBed.configureTestingModule({
			imports: [
				...
				AdminAdministrationUserCrudModule,
			],
			providers: [
				provideMockStore(),
				{
			...
			fixture = TestBed.createComponent(AdminAdministrationUserCrudListComponent);
			administrationUserCrudListComponent = fixture.debugElement.componentInstance;
	...
	if (searchableFieldCount > 0) {
		it ('Reset the page index and update page count when searching', () => {
			...
			expect(administrationUserCrudListComponent.administrationUsersCount).toBe(defaultData.length);
			...
			expect(administrationUserCrudListComponent.pageIndex).toBe(1);
			fixture.detectChanges();

			// Trigger the search function
			administrationUserCrudListComponent.onCollectionSearch(searchContent);
			fixture.detectChanges();

			// Update selector to mock search results\
			const filteredDataSize = 5;
			const filteredData = administrationUserDataFactory.createAll(filteredDataSize);
			...
			expect(administrationUserCrudListComponent.administrationUsersCount).toBe(filteredDataSize);
			expect(administrationUserCrudListComponent.pageIndex).toBe(0);
		...
		it ('returns to the initial page after clearing the search', () => {
			...
			expect(administrationUserCrudListComponent.pageIndex).toBe(0);
			expect(administrationUserCrudListComponent.currentPage).toBe(1); // current page should be the page index prior to search

			// Set search text to empty string.  Should trigger the list to return to the page prior to searching
			administrationUserCrudListComponent.onCollectionSearch('');
			fixture.detectChanges();
	...
	it ('Disables Create Button if user does not have create permissions', () => {
		...
		administrationUserCrudListComponent.collectionActions = collectionActions(canCreate);
		fixture.detectChanges();
	...
	it ('Disables Edit buttons is user does not have edit permissions', () => {
		...
		administrationUserCrudListComponent.itemActions = itemActions(canEdit, canDelete);
		fixture.detectChanges();
