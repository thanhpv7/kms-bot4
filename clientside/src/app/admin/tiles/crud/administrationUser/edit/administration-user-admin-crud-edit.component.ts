...
import {AdministrationUserModelState, AdministrationUserModelAudit} from 'src/app/models/administrationUser/administration_user.model.state';
import {AdministrationUserModel} from 'src/app/models/administrationUser/administration_user.model';
import {
	getAdministrationUserModelAuditsByEntityId,
	getAdministrationUserModelWithId,
	getCountAdministrationUserModels,
...
@Component({
	selector: 'cb-administration-user-admin-crud-edit',
	templateUrl: './administration-user-admin-crud-edit.component.html',
	styleUrls: [
		'./administration-user-admin-crud-edit.component.scss',
...
export class AdminAdministrationUserCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	administrationUserAudits$: Observable<AdministrationUserModelAudit[]>;
	...
	staffOppositeRelation = 'administrationUser';
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = AdministrationUserModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: AdministrationUserModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountAdministrationUserModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new AdministrationUserModel();
		...
		if (this.targetModelId) {
			this.administrationUserAudits$ = this.store.select(getAdministrationUserModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<AdministrationUserModel> = {
		...
		this.store.dispatch(new modelAction.AdministrationUserAction(
			modelAction.AdministrationUserModelActionTypes.FETCH_ADMINISTRATION_USER,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getAdministrationUserModelWithId, this.targetModelId)
			...
			model = new AdministrationUserModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.AdministrationUserAction(
			modelAction.AdministrationUserModelActionTypes.FETCH_ADMINISTRATION_USER_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.AdministrationUserAction(
				modelAction.AdministrationUserModelActionTypes.CREATE_ADMINISTRATION_USER,
				stateConfig,
			...
			this.store.dispatch(new modelAction.AdministrationUserAction(
				modelAction.AdministrationUserModelActionTypes.UPDATE_ADMINISTRATION_USER,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: AdministrationUserModel = new AdministrationUserModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getAdministrationUserModelWithId, this.targetModelId).subscribe(model => {
					let differences = new AdministrationUserModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = AdministrationUserModel.getProps();
		this.modelRelations = AdministrationUserModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: AdministrationUserModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): AdministrationUserModel {
		let updatedModel: AdministrationUserModel = new AdministrationUserModel(model);
