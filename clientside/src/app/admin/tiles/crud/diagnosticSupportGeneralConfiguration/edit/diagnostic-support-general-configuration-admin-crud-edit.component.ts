...
import {DiagnosticSupportGeneralConfigurationModelState, DiagnosticSupportGeneralConfigurationModelAudit} from 'src/app/models/diagnosticSupportGeneralConfiguration/diagnostic_support_general_configuration.model.state';
...
import {
	getDiagnosticSupportGeneralConfigurationModelAuditsByEntityId,
	getDiagnosticSupportGeneralConfigurationModelWithId,
	getCountDiagnosticSupportGeneralConfigurationModels,
...
@Component({
	selector: 'cb-diagnostic-support-general-configuration-admin-crud-edit',
	templateUrl: './diagnostic-support-general-configuration-admin-crud-edit.component.html',
	styleUrls: [
		'./diagnostic-support-general-configuration-admin-crud-edit.component.scss',
...
export class AdminDiagnosticSupportGeneralConfigurationCrudEditComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	 ...
	 */
	diagnosticSupportGeneralConfigurationAudits$: Observable<DiagnosticSupportGeneralConfigurationModelAudit[]>;
	...
	private get defaultExpands(): Expand[] {
		let expands: Expand[] = DiagnosticSupportGeneralConfigurationModel.getRelations().map(
			relation => {
				let exp = {
					name: relation.id,
	 ...
	 */
	targetModel: DiagnosticSupportGeneralConfigurationModel;
	...
	ngOnInit() {
		 ...
		 */
		this.creationNavigationSubscription$.add(this.store.select(getCountDiagnosticSupportGeneralConfigurationModels).subscribe(count => {
	...
	private initializeTargetModel() {
		...
		if (this.tileMode === CrudTileMode.Edit || this.tileMode === CrudTileMode.View) {
			...
			this.targetModel = new DiagnosticSupportGeneralConfigurationModel();
		...
		if (this.targetModelId) {
			this.diagnosticSupportGeneralConfigurationAudits$ = this.store.select(getDiagnosticSupportGeneralConfigurationModelAuditsByEntityId, this.targetModelId);
		}
	 ...
	 */
	private fetchEntity() {
		const stateConfig: PassableStateConfig<DiagnosticSupportGeneralConfigurationModel> = {
		...
		this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
			modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION,
			stateConfig
		));

		this.creationNavigationSubscription$.add(this.store.select(getDiagnosticSupportGeneralConfigurationModelWithId, this.targetModelId)
			...
			model = new DiagnosticSupportGeneralConfigurationModel(model);
			model.syncRelations();
	...
	onViewHistory() {
		...
		this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
			modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_AUDITS_BY_ENTITY_ID,
			{
				targetModelId: this.targetModelId
			}
		));
	...
	onCreateOrSaveClicked(event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }) {
		...
		if (event.isCreate) {
			...
			this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
				modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.CREATE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION,
				stateConfig,
			...
			this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
				modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.UPDATE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION,
				stateConfig,
			...
			if (this.singlePageComponent) {
				let updatedModel: DiagnosticSupportGeneralConfigurationModel = new DiagnosticSupportGeneralConfigurationModel(this.targetModel).applyUpdates(stateConfig.updates);
				...
				this.creationNavigationSubscription$.add(this.store.select(getDiagnosticSupportGeneralConfigurationModelWithId, this.targetModelId).subscribe(model => {
					let differences = new DiagnosticSupportGeneralConfigurationModel(model).difference(updatedModel);
	 ...
	 */
	resetPropsAndRels() {
		this.modelProperties = DiagnosticSupportGeneralConfigurationModel.getProps();
		this.modelRelations = DiagnosticSupportGeneralConfigurationModel.getRelations();
	 ...
	 */
	addAdditionalEntitiesToModel(model: DiagnosticSupportGeneralConfigurationModel, event: { isCreate: boolean, additionalEntities: any, payload?: { [s: string]: any } }): DiagnosticSupportGeneralConfigurationModel {
		let updatedModel: DiagnosticSupportGeneralConfigurationModel = new DiagnosticSupportGeneralConfigurationModel(model);
