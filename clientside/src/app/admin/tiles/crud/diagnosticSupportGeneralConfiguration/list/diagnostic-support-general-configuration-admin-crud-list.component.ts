...
import * as modelAction from 'src/app/models/diagnosticSupportGeneralConfiguration/diagnostic_support_general_configuration.model.action';
import {DiagnosticSupportGeneralConfigurationModelState} from 'src/app/models/diagnosticSupportGeneralConfiguration/diagnostic_support_general_configuration.model.state';
...
import {
	getDiagnosticSupportGeneralConfigurationCollectionModels,
	getDiagnosticSupportGeneralConfigurationCollectionCount
} from 'src/app/models/diagnosticSupportGeneralConfiguration/diagnostic_support_general_configuration.model.selector';
...
import {FilterQuestion} from 'src/app/lib/components/collection/collection-filter.component';
import {CrudTileMode} from '../diagnostic-support-general-configuration-admin-crud.component';
...
@Component({
	selector: 'cb-diagnostic-support-general-configuration-admin-crud-list',
	templateUrl: './diagnostic-support-general-configuration-admin-crud-list.component.html',
	styleUrls: [
		'./diagnostic-support-general-configuration-admin-crud-list.component.scss',
...
export class AdminDiagnosticSupportGeneralConfigurationCrudListComponent
		...
		// % protected region % [Add any additional interfaces to implement here] end
{
	...
	childCollection: CollectionComponent<DiagnosticSupportGeneralConfigurationModel>;

	/**
	 * The name of the fields which are used in the search box
	 */
	searchFields: string[] = [
		...DiagnosticSupportGeneralConfigurationModel.searchFields,
	...
	/**
	 * List of Diagnostic Support General Configurations that will be managed by this CRUD tile. Note that this list is not complete, i.e. it
	 * does not represent all models in the database. Instead it represents a slice or page of the complete list.
	 */
	diagnosticSupportGeneralConfigurations: Observable<DiagnosticSupportGeneralConfigurationModel[]>;

	/**
	 * The total number of entities in the database.
	 */
	diagnosticSupportGeneralConfigurationsCount: number = 0;

	// % protected region % [Change your model properties here] off begin
	/**
	 * List of all property names for this entity.
	 */
	modelProperties: ModelProperty[] = DiagnosticSupportGeneralConfigurationModel.getProps();
	 ...
	 */
	modelRelations: ModelRelation[] = DiagnosticSupportGeneralConfigurationModel.getRelations();
	 ...
	 */
	@Input()
	collectionId: string = 'd03fe7e5-b2b1-41cb-9f4e-eef7bd028be1';
	...
	get isLastPage(): boolean {
		return Math.max(Math.floor((this.diagnosticSupportGeneralConfigurationsCount - 1) / this.pageSize), 0) === this.pageIndex;
	}
	...
	collectionActions: Action[] = [
		...
		},
		{
			...
			buttonColour: ButtonAccentColour.PRIMARY,
			isAdditional: false,
			disableOption: () => this.diagnosticSupportGeneralConfigurationsCount === 0
		},
		{
	...
	constructor(
		private readonly store: Store<{ model: DiagnosticSupportGeneralConfigurationModelState }>,
	...
	ngOnInit() {
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
				modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.INITIALISE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_COLLECTION_STATE,
				{
		...
		this.entitySubscription$.add(
			this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
				modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_WITH_QUERY,
				{
					queryParams: this.queryParams,
					collectionId: this.collectionId
				},
				[
					new modelAction.DiagnosticSupportGeneralConfigurationAction(
						modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.COUNT_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATIONS,
						null
					)
				]
			))
		...
		this.diagnosticSupportGeneralConfigurations = this.store.select(getDiagnosticSupportGeneralConfigurationCollectionModels, this.collectionId);

		this.entitySubscription$.add(
			this.store.select(getDiagnosticSupportGeneralConfigurationCollectionCount, this.collectionId).subscribe(
				(count) => this.diagnosticSupportGeneralConfigurationsCount = count
			)
		);
	...
	private triggerTileModeChange(tileMode: CrudTileMode, id?: string, other?: object) {
		if (this.singlePageComponent) {
			...
			commands.push('diagnostic-support-general-configuration');
	 ...
	 */
	onExportClicked(usingSelected: boolean, models: DiagnosticSupportGeneralConfigurationModel[]) {
		// % protected region % [Customise onExportClicked logic here] off begin
		let actionType: modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes;
		if (usingSelected) {
			actionType = modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.EXPORT_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION;
		} else {
			actionType = modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.EXPORT_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_EXCLUDING_IDS;
		}
	...
	onImportClicked(file?: any) {
		...
		if (file) {
			this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
				modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.IMPORT_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATIONS,
				{
					file: file,
				},
				[new modelAction.DiagnosticSupportGeneralConfigurationAction(
			...
			window.open(environment.API_URL + '/api/diagnosticsupportgeneralconfiguration/example-import');
		}
	 ...
	 */
	onDeleteClicked(usingSelected: boolean, models: DiagnosticSupportGeneralConfigurationModel[], modelsToDelete: number) {
		...
		if (modelsToDelete > 1) {
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Support General Configuration entities?\n' +
			...
			modalText = 'Are you sure you want to delete the selected Diagnostic Support General Configuration entity?\n' +
				'This action cannot be reversed';
			...
			actionButtons: [
				...
				// % protected region % [Add any additional buttons before the default modal buttons here] end
				{
					...
					onAction: () => {
						...
						let actionType: modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes;
						if (modelsToDelete === 1) {
							actionType = modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION;
						} else if (usingSelected) {
							actionType = modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.DELETE_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION;
						} else {
							actionType = modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_EXCLUDING_IDS;
						}
						...
						this.store.dispatch(new modelAction.DiagnosticSupportGeneralConfigurationAction(
							...
							{
								targetModelId: modelsToDelete === 1 ? models[0].id : null, // used in DELETE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION when only one entity is present
								targetModelIds: modelsToDelete > 1 ? models.map(model => model.id) : null, // used in DELETE_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION and DELETE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_EXCLUDING_IDS
								queryParams: this.queryParams,
								collectionId: this.collectionId
							},
							[
	...
	onCollectionSort($event: OrderBy[]) {
		...
		let stateConfig: PassableStateConfig<DiagnosticSupportGeneralConfigurationModel> = {
			queryParams: this.queryParams,
			collectionId: this.collectionId
		};
	...
	onCollectionFilter($event: { isClean?: boolean, filterFormGroup: FormGroup }) {
		...
		this.filterConditions = DiagnosticSupportGeneralConfigurationModel.convertFilterToCondition($event.filterFormGroup);
	...
	onLastPageRequested() {
		...
		this.pageIndex = Math.floor((this.diagnosticSupportGeneralConfigurationsCount - 1) / this.pageSize);
		if (!this.searchingOrFiltering) {
			this.currentPage = this.pageIndex;
		}
