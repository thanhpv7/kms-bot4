...
export class PriceComponentDataFactory extends AbstractDataFactory<PriceComponentModel> {
	...
	create(): PriceComponentModel {
		let newModel = new PriceComponentModel();
	...
	createAll(size: number = 10): PriceComponentModel[] {
		let newModels: PriceComponentModel[] = [];
	...
	/**
	 * Given a blank model of type Price Component, populate the model with its properties.
	 ...
	 */
	populate(model: PriceComponentModel) {
		...
		model.componentCode = faker.random.words();
		model.componentName = faker.random.words();
