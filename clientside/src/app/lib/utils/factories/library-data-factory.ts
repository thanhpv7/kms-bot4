...
export class LibraryDataFactory extends AbstractDataFactory<LibraryModel> {
	...
	createAll(size: number = 10): LibraryModel[] {
		let newModels: LibraryModel[] = [];
	...
	/**
	 * Given a blank model of type Library, populate the model with its properties.
	...
	populate(model: LibraryModel) {
		...
		model.libraryItemCode = faker.random.words();
		model.libraryItemDescription = faker.random.words();
		model.classification = faker.random.words();
		model.author = faker.random.words();
		model.publisher = faker.random.words();
