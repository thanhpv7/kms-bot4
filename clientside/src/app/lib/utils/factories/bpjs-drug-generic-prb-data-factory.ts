...
export class BpjsDrugGenericPRBDataFactory extends AbstractDataFactory<BpjsDrugGenericPRBModel> {
	...
	create(): BpjsDrugGenericPRBModel {
		let newModel = new BpjsDrugGenericPRBModel();
	...
	createAll(size: number = 10): BpjsDrugGenericPRBModel[] {
		let newModels: BpjsDrugGenericPRBModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Drug Generic PRB, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsDrugGenericPRBModel) {
		...
		model.refDrugCode = faker.random.words();
		model.refDrugName = faker.random.words();
