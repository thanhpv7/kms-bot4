...
export class BpjsTreatmentHistoryDataFactory extends AbstractDataFactory<BpjsTreatmentHistoryModel> {
	...
	create(): BpjsTreatmentHistoryModel {
		let newModel = new BpjsTreatmentHistoryModel();
	...
	createAll(size: number = 10): BpjsTreatmentHistoryModel[] {
		let newModels: BpjsTreatmentHistoryModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Treatment History, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsTreatmentHistoryModel) {
		...
		model.healthFacility = faker.random.words();
