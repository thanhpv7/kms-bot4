...
export class PcareUmumDataFactory extends AbstractDataFactory<PcareUmumModel> {
	...
	create(): PcareUmumModel {
		let newModel = new PcareUmumModel();
	...
	createAll(size: number = 10): PcareUmumModel[] {
		let newModels: PcareUmumModel[] = [];
	...
	/**
	 * Given a blank model of type PCare Umum, populate the model with its properties.
	...
	populate(model: PcareUmumModel) {
		...
		model.webServiceURL = faker.random.words();
		model.authUsername = faker.random.words();
		model.authPassword = faker.random.words();
		model.kodeAplikasi = faker.random.words();
		model.userKey = faker.random.words();
		model.regionalDivision = faker.random.words();
		model.branchOffice = faker.random.words();
