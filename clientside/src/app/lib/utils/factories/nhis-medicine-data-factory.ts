...
export class NhisMedicineDataFactory extends AbstractDataFactory<NhisMedicineModel> {
	...
	create(): NhisMedicineModel {
		let newModel = new NhisMedicineModel();
	...
	createAll(size: number = 10): NhisMedicineModel[] {
		let newModels: NhisMedicineModel[] = [];
	...
	/**
	 * Given a blank model of type NHIS Medicine, populate the model with its properties.
	 ...
	 */
	populate(model: NhisMedicineModel) {
		...
		model.medicineCode = faker.random.words();
		model.medicineName = faker.random.words();
		...
		model.dispensedQty = faker.random.number();
		model.totalCost = faker.random.number();
		model.serviceDate = new Date();
