...
export class BpjsServiceMappingDataFactory extends AbstractDataFactory<BpjsServiceMappingModel> {
	...
	create(): BpjsServiceMappingModel {
		let newModel = new BpjsServiceMappingModel();
	...
	createAll(size: number = 10): BpjsServiceMappingModel[] {
		let newModels: BpjsServiceMappingModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Service Mapping, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsServiceMappingModel) {
		...
		model.refCode = faker.random.words();
		model.refService = faker.random.words();
		model.kodeSubspesialis = faker.random.words();
		model.namaSubspesialis = faker.random.words();
		model.categorizedAsSurgery = faker.random.boolean();
