...
export class AmendmentDetailDataFactory extends AbstractDataFactory<AmendmentDetailModel> {
	...
	create(): AmendmentDetailModel {
		let newModel = new AmendmentDetailModel();
	...
	createAll(size: number = 10): AmendmentDetailModel[] {
		let newModels: AmendmentDetailModel[] = [];
	...
	/**
	 * Given a blank model of type Amendment Detail, populate the model with its properties.
	 ...
	 */
	populate(model: AmendmentDetailModel) {
		...
		model.date = new Date();
		model.reason = faker.random.words();
