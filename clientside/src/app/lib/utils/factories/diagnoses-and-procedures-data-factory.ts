...
export class DiagnosesAndProceduresDataFactory extends AbstractDataFactory<DiagnosesAndProceduresModel> {
	...
	create(): DiagnosesAndProceduresModel {
		let newModel = new DiagnosesAndProceduresModel();
	...
	createAll(size: number = 10): DiagnosesAndProceduresModel[] {
		let newModels: DiagnosesAndProceduresModel[] = [];
	...
	/**
	 * Given a blank model of type Diagnoses and Procedures, populate the model with its properties.
	 ...
	 */
	populate(model: DiagnosesAndProceduresModel) {
		...
		model.itemType = faker.random.words();
