...
import {RoomFacilityModel} from '../../../models/roomFacility/room_facility.model';
import * as faker from 'faker';

export class RoomFacilityDataFactory extends AbstractDataFactory<RoomFacilityModel> {
	...
	create(): RoomFacilityModel {
		let newModel = new RoomFacilityModel();
	...
	createAll(size: number = 10): RoomFacilityModel[] {
		let newModels: RoomFacilityModel[] = [];
	...
	/**
	 * Given a blank model of type Room Facility, populate the model with its properties.
	 ...
	 */
	populate(model: RoomFacilityModel) {
		...
		model.roomNumber = faker.random.words();
		model.roomName = faker.random.words();
		model.department = faker.random.words();
		model.genderAllocation = faker.random.words();
		model.location = faker.random.words();
		model.notes = faker.random.words();
		model.category = faker.random.words();
