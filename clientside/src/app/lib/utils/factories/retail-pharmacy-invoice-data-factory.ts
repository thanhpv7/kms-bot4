...
export class RetailPharmacyInvoiceDataFactory extends AbstractDataFactory<RetailPharmacyInvoiceModel> {
	...
	create(): RetailPharmacyInvoiceModel {
		let newModel = new RetailPharmacyInvoiceModel();
	...
	createAll(size: number = 10): RetailPharmacyInvoiceModel[] {
		let newModels: RetailPharmacyInvoiceModel[] = [];
	...
	/**
	 * Given a blank model of type Retail Pharmacy Invoice, populate the model with its properties.
	 ...
	 */
	populate(model: RetailPharmacyInvoiceModel) {
		...
		model.paymentStatus = faker.random.words();
		model.invoiceAmount = faker.random.number();
