...
import {TransferOrderModel} from '../../../models/transferOrder/transfer_order.model';
import * as faker from 'faker';

export class TransferOrderDataFactory extends AbstractDataFactory<TransferOrderModel> {
	...
	create(): TransferOrderModel {
		let newModel = new TransferOrderModel();
	...
	createAll(size: number = 10): TransferOrderModel[] {
		let newModels: TransferOrderModel[] = [];
	...
	/**
	 * Given a blank model of type Transfer Order, populate the model with its properties.
	 ...
	 */
	populate(model: TransferOrderModel) {
		...
		model.transferOrderNumber = faker.random.words();
