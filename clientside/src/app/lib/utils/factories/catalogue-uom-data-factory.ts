...
export class CatalogueUOMDataFactory extends AbstractDataFactory<CatalogueUOMModel> {
	...
	create(): CatalogueUOMModel {
		let newModel = new CatalogueUOMModel();
	...
	createAll(size: number = 10): CatalogueUOMModel[] {
		let newModels: CatalogueUOMModel[] = [];
	...
	/**
	 * Given a blank model of type Catalogue UOM, populate the model with its properties.
	 ...
	 */
	populate(model: CatalogueUOMModel) {
		...
		model.conversion = faker.random.words();
		model.defaultTransaction = faker.random.boolean();
		model.defaultStockCard = faker.random.boolean();
		model.defaultPurchasing = faker.random.boolean();
