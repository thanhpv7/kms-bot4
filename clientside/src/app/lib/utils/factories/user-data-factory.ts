...
export abstract class UserDataFactory<E extends UserModel> extends AbstractDataFactory<E> {
	...
	/**
	 * Given a blank model of type User, populate the model with its properties.
	...
	populate(model: E) {
		...
		model.emailConfirmedConfirmed = faker.random.boolean();
		model.email = faker.random.words();
		model.isArchived = faker.random.boolean();
