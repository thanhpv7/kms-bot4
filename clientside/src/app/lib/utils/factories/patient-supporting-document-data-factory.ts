...
export class PatientSupportingDocumentDataFactory extends AbstractDataFactory<PatientSupportingDocumentModel> {
	...
	create(): PatientSupportingDocumentModel {
		let newModel = new PatientSupportingDocumentModel();
	...
	createAll(size: number = 10): PatientSupportingDocumentModel[] {
		let newModels: PatientSupportingDocumentModel[] = [];
	...
	/**
	 * Given a blank model of type Patient Supporting Document, populate the model with its properties.
	 ...
	 */
	populate(model: PatientSupportingDocumentModel) {
		...
		model.fileName = faker.random.words();
