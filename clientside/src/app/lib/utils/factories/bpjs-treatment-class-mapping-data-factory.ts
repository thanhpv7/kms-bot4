...
export class BpjsTreatmentClassMappingDataFactory extends AbstractDataFactory<BpjsTreatmentClassMappingModel> {
	...
	create(): BpjsTreatmentClassMappingModel {
		let newModel = new BpjsTreatmentClassMappingModel();
	...
	createAll(size: number = 10): BpjsTreatmentClassMappingModel[] {
		let newModels: BpjsTreatmentClassMappingModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Treatment Class Mapping, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsTreatmentClassMappingModel) {
		...
		model.refName = faker.random.words();
		model.internalTreatmentClass = faker.random.words();
