...
export class PrescriptionItemDataFactory extends AbstractDataFactory<PrescriptionItemModel> {
	...
	create(): PrescriptionItemModel {
		let newModel = new PrescriptionItemModel();
	...
	createAll(size: number = 10): PrescriptionItemModel[] {
		let newModels: PrescriptionItemModel[] = [];
	...
	/**
	 * Given a blank model of type Prescription Item, populate the model with its properties.
	 ...
	 */
	populate(model: PrescriptionItemModel) {
		...
		model.externalStockName = faker.random.words();
