...
export class MerchantDataFactory extends AbstractDataFactory<MerchantModel> {
	...
	create(): MerchantModel {
		let newModel = new MerchantModel();
	...
	createAll(size: number = 10): MerchantModel[] {
		let newModels: MerchantModel[] = [];
	...
	/**
	 * Given a blank model of type Merchant, populate the model with its properties.
	...
	populate(model: MerchantModel) {
		...
		model.merchantName = faker.random.words();
		model.merchantNumber = faker.random.words();
		model.bankAccount = faker.random.words();
