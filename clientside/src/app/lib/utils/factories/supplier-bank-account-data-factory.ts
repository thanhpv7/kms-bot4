...
export class SupplierBankAccountDataFactory extends AbstractDataFactory<SupplierBankAccountModel> {
	...
	create(): SupplierBankAccountModel {
		let newModel = new SupplierBankAccountModel();
	...
	createAll(size: number = 10): SupplierBankAccountModel[] {
		let newModels: SupplierBankAccountModel[] = [];
	...
	/**
	 * Given a blank model of type Supplier Bank Account, populate the model with its properties.
	 ...
	 */
	populate(model: SupplierBankAccountModel) {
		...
		model.branch = faker.random.words();
