...
export class ServiceItemDataFactory extends AbstractDataFactory<ServiceItemModel> {
	...
	create(): ServiceItemModel {
		let newModel = new ServiceItemModel();
	...
	createAll(size: number = 10): ServiceItemModel[] {
		let newModels: ServiceItemModel[] = [];
	...
	/**
	 * Given a blank model of type Service Item, populate the model with its properties.
	 ...
	 */
	populate(model: ServiceItemModel) {
		...
		model.serviceItemCode = faker.random.words();
		model.serviceItemName = faker.random.words();
		model.classification = faker.random.words();
		model.applyToAllService = faker.random.boolean();
		...
		model.coaCOGS = faker.random.words();
		model.coaSell = faker.random.words();
		model.coaInventory = faker.random.words();
