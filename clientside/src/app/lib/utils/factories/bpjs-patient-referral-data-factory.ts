...
export class BpjsPatientReferralDataFactory extends AbstractDataFactory<BpjsPatientReferralModel> {
	...
	create(): BpjsPatientReferralModel {
		let newModel = new BpjsPatientReferralModel();
	...
	createAll(size: number = 10): BpjsPatientReferralModel[] {
		let newModels: BpjsPatientReferralModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Patient Referral, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsPatientReferralModel) {
		...
		model.referredType = faker.random.words();
		model.referralDate = new Date();
		model.referalDatePlan = new Date();
