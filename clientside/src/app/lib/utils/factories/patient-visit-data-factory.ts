...
export class PatientVisitDataFactory extends AbstractDataFactory<PatientVisitModel> {
	...
	create(): PatientVisitModel {
		let newModel = new PatientVisitModel();
	...
	createAll(size: number = 10): PatientVisitModel[] {
		let newModels: PatientVisitModel[] = [];
	...
	/**
	 * Given a blank model of type Patient Visit, populate the model with its properties.
	 ...
	 */
	populate(model: PatientVisitModel) {
		...
		model.visitNumber = faker.random.words();
		model.registrationType = faker.random.words();
		model.visitDate = new Date();
		model.registrationLocation = faker.random.words();
