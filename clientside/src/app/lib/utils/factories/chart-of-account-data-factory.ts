...
export class ChartOfAccountDataFactory extends AbstractDataFactory<ChartOfAccountModel> {
	...
	create(): ChartOfAccountModel {
		let newModel = new ChartOfAccountModel();
	...
	createAll(size: number = 10): ChartOfAccountModel[] {
		let newModels: ChartOfAccountModel[] = [];
	...
	/**
	 * Given a blank model of type Chart of Account, populate the model with its properties.
	 ...
	 */
	populate(model: ChartOfAccountModel) {
		...
		model.accountCode = faker.random.words();
		model.accountName = faker.random.words();
		model.accountType = faker.random.words();
		model.taxCode = faker.random.words();
		model.dashboard = faker.random.boolean();
		model.expenseClaim = faker.random.boolean();
		model.enablePayment = faker.random.boolean();
