...
import {PharmacySetupModel} from '../../../models/pharmacySetup/pharmacy_setup.model';
import * as faker from 'faker';

export class PharmacySetupDataFactory extends AbstractDataFactory<PharmacySetupModel> {
	...
	create(): PharmacySetupModel {
		let newModel = new PharmacySetupModel();
	...
	createAll(size: number = 10): PharmacySetupModel[] {
		let newModels: PharmacySetupModel[] = [];
	...
	/**
	 * Given a blank model of type Pharmacy Setup, populate the model with its properties.
	 ...
	 */
	populate(model: PharmacySetupModel) {
		...
		model.hospitalPharmacy = faker.random.words();
		model.headOfPharmacy = faker.random.words();
		model.pharmacistLicenseNo = faker.random.words();
		model.labelSetup = faker.random.words();
