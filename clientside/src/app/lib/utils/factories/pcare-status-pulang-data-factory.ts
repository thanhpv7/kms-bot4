...
export class PcareStatusPulangDataFactory extends AbstractDataFactory<PcareStatusPulangModel> {
	...
	create(): PcareStatusPulangModel {
		let newModel = new PcareStatusPulangModel();
	...
	createAll(size: number = 10): PcareStatusPulangModel[] {
		let newModels: PcareStatusPulangModel[] = [];
	...
	/**
	 * Given a blank model of type PCare Status Pulang, populate the model with its properties.
	 ...
	 */
	populate(model: PcareStatusPulangModel) {
		...
		model.kodeStatusPulang = faker.random.words();
		model.namaStatusPulang = faker.random.words();
		model.kodeReferensiInternal = faker.random.words();
