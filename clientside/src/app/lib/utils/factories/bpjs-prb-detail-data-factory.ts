...
export class BpjsPRBDetailDataFactory extends AbstractDataFactory<BpjsPRBDetailModel> {
	...
	create(): BpjsPRBDetailModel {
		let newModel = new BpjsPRBDetailModel();
	...
	createAll(size: number = 10): BpjsPRBDetailModel[] {
		let newModels: BpjsPRBDetailModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS PRB Detail, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsPRBDetailModel) {
		...
		model.srbNo = faker.random.words();
		model.drugCode = faker.random.words();
		model.drugName = faker.random.words();
		model.signa1 = faker.random.words();
		model.signa2 = faker.random.words();
		model.qty = faker.random.words();
