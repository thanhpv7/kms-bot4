...
export class PcareActionExaminationDataFactory extends AbstractDataFactory<PcareActionExaminationModel> {
	...
	create(): PcareActionExaminationModel {
		let newModel = new PcareActionExaminationModel();
	...
	createAll(size: number = 10): PcareActionExaminationModel[] {
		let newModels: PcareActionExaminationModel[] = [];
	...
	/**
	 * Given a blank model of type PCare Action Examination, populate the model with its properties.
	 ...
	 */
	populate(model: PcareActionExaminationModel) {
		...
		model.actionCode = faker.random.words();
		model.fee = faker.random.number();
		model.notes = faker.random.words();
		model.kdtindakansk = faker.random.words();
		model.hasil = faker.random.number();
