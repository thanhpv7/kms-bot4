...
export class BpjsHospitalizationPlanDataFactory extends AbstractDataFactory<BpjsHospitalizationPlanModel> {
	...
	create(): BpjsHospitalizationPlanModel {
		let newModel = new BpjsHospitalizationPlanModel();
	...
	createAll(size: number = 10): BpjsHospitalizationPlanModel[] {
		let newModels: BpjsHospitalizationPlanModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Hospitalization Plan, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsHospitalizationPlanModel) {
		...
		model.tglrencanakontrol = new Date();
		model.nospri = faker.random.words();
