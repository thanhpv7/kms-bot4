...
export class CashRefundDataFactory extends AbstractDataFactory<CashRefundModel> {
	...
	create(): CashRefundModel {
		let newModel = new CashRefundModel();
	...
	createAll(size: number = 10): CashRefundModel[] {
		let newModels: CashRefundModel[] = [];
	...
	/**
	 * Given a blank model of type Cash Refund, populate the model with its properties.
	 ...
	 */
	populate(model: CashRefundModel) {
		...
		model.refundNumber = faker.random.words();
		model.refundDate = new Date();
		...
		model.refundAmount = faker.random.number();
