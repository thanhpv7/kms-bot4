...
export class InternalOrderDataFactory extends AbstractDataFactory<InternalOrderModel> {
	...
	create(): InternalOrderModel {
		let newModel = new InternalOrderModel();
	...
	createAll(size: number = 10): InternalOrderModel[] {
		let newModels: InternalOrderModel[] = [];
	...
	/**
	 * Given a blank model of type Internal Order, populate the model with its properties.
	 ...
	 */
	populate(model: InternalOrderModel) {
		...
		model.internalOrderNumber = faker.random.words();
		...
		model.requestUnit = faker.random.words();
		model.requestPerson = faker.random.words();
		...
		model.issuedDateTime = new Date();
		model.receivedDateTime = new Date();
