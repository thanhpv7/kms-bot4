...
export class StockControlDataFactory extends AbstractDataFactory<StockControlModel> {
	...
	create(): StockControlModel {
		let newModel = new StockControlModel();
	...
	createAll(size: number = 10): StockControlModel[] {
		let newModels: StockControlModel[] = [];
	...
	/**
	 * Given a blank model of type Stock Control, populate the model with its properties.
	 ...
	 */
	populate(model: StockControlModel) {
		...
		model.availableQty = faker.random.number();
		model.onReservedQty = faker.random.number();
		model.uoi = faker.random.words();
		model.minLevel = faker.random.number();
		model.maxLevel = faker.random.number();
		model.inTransitQty = faker.random.number();
