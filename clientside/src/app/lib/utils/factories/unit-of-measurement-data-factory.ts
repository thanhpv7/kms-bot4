...
export class UnitOfMeasurementDataFactory extends AbstractDataFactory<UnitOfMeasurementModel> {
	...
	create(): UnitOfMeasurementModel {
		let newModel = new UnitOfMeasurementModel();
	...
	createAll(size: number = 10): UnitOfMeasurementModel[] {
		let newModels: UnitOfMeasurementModel[] = [];
	...
	/**
	 * Given a blank model of type Unit Of Measurement, populate the model with its properties.
	 ...
	 */
	populate(model: UnitOfMeasurementModel) {
		...
		model.symbol = faker.random.words();
		model.description = faker.random.words();
		model.unitCategory = faker.random.words();
		model.consumptionUnit = faker.random.boolean();
