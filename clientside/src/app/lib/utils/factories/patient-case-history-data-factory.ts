...
import {PatientCaseHistoryModel} from '../../../models/patientCaseHistory/patient_case_history.model';
import * as faker from 'faker';

export class PatientCaseHistoryDataFactory extends AbstractDataFactory<PatientCaseHistoryModel> {
	...
	create(): PatientCaseHistoryModel {
		let newModel = new PatientCaseHistoryModel();
	...
	createAll(size: number = 10): PatientCaseHistoryModel[] {
		let newModels: PatientCaseHistoryModel[] = [];
	...
	/**
	 * Given a blank model of type Patient Case History, populate the model with its properties.
	 ...
	 */
	populate(model: PatientCaseHistoryModel) {
		...
		model.allergy = faker.random.words();
		model.disabilityNotes = faker.random.words();
		model.historyOfDisease = faker.random.words();
		model.caseHistoryDateTime = new Date();
		model.patientHabits = faker.random.words();
		model.nutritionHIstory = faker.random.words();
		model.familyHistory = faker.random.words();
