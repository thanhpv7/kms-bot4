...
export class PostOperativeDetailsDataFactory extends AbstractDataFactory<PostOperativeDetailsModel> {
	...
	create(): PostOperativeDetailsModel {
		let newModel = new PostOperativeDetailsModel();
	...
	createAll(size: number = 10): PostOperativeDetailsModel[] {
		let newModels: PostOperativeDetailsModel[] = [];
	...
	/**
	 * Given a blank model of type Post Operative Details, populate the model with its properties.
	 ...
	 */
	populate(model: PostOperativeDetailsModel) {
		...
		model.recoveryStartDateTime = new Date();
		model.recoveryEndDateTime = new Date();
		model.postoperativeMainDiagnosis = faker.random.words();
		model.postoperativeAction = faker.random.words();
		model.newCase = faker.random.boolean();
		model.postoperativeDiagnosisSupportOrder = faker.random.words();
		model.anesthesiaPostoperativeMonitoring = faker.random.words();
		model.anesthesiaPostoperativeAction = faker.random.words();
