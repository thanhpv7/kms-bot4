...
export class MedicationHeaderDataFactory extends AbstractDataFactory<MedicationHeaderModel> {
	...
	create(): MedicationHeaderModel {
		let newModel = new MedicationHeaderModel();
	...
	createAll(size: number = 10): MedicationHeaderModel[] {
		let newModels: MedicationHeaderModel[] = [];
	...
	/**
	 * Given a blank model of type Medication Header, populate the model with its properties.
	 ...
	 */
	populate(model: MedicationHeaderModel) {
		...
		model.medicationNumber = faker.random.words();
		model.dispensingType = faker.random.words();
		model.originalMedicationNumber = faker.random.words();
