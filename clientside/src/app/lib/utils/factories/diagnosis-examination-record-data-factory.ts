...
import {DiagnosisExaminationRecordModel} from '../../../models/diagnosisExaminationRecord/diagnosis_examination_record.model';
import * as faker from 'faker';

export class DiagnosisExaminationRecordDataFactory extends AbstractDataFactory<DiagnosisExaminationRecordModel> {
	...
	create(): DiagnosisExaminationRecordModel {
		let newModel = new DiagnosisExaminationRecordModel();
	...
	createAll(size: number = 10): DiagnosisExaminationRecordModel[] {
		let newModels: DiagnosisExaminationRecordModel[] = [];
	...
	/**
	 * Given a blank model of type Diagnosis Examination Record, populate the model with its properties.
	 ...
	 */
	populate(model: DiagnosisExaminationRecordModel) {
		...
		model.diagnosisSequence = faker.random.number();
		model.diagnosisNotes = faker.random.words();
		model.actionNotes = faker.random.words();
		model.newCase = faker.random.boolean();
		model.printingNumber = faker.random.number();
