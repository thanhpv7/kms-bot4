...
export class BpjsPRBDataFactory extends AbstractDataFactory<BpjsPRBModel> {
	...
	createAll(size: number = 10): BpjsPRBModel[] {
		let newModels: BpjsPRBModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS PRB, populate the model with its properties.
	...
	populate(model: BpjsPRBModel) {
		...
		model.prbProgramCode = faker.random.words();
		model.prbProgramName = faker.random.words();
		model.doctorCode = faker.random.words();
		model.doctorName = faker.random.words();
		model.notes = faker.random.words();
		model.recommendation = faker.random.words();
