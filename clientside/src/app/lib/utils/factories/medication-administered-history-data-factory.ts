...
export class MedicationAdministeredHistoryDataFactory extends AbstractDataFactory<MedicationAdministeredHistoryModel> {
	...
	create(): MedicationAdministeredHistoryModel {
		let newModel = new MedicationAdministeredHistoryModel();
	...
	createAll(size: number = 10): MedicationAdministeredHistoryModel[] {
		let newModels: MedicationAdministeredHistoryModel[] = [];
	...
	/**
	 * Given a blank model of type Medication Administered History, populate the model with its properties.
	 ...
	 */
	populate(model: MedicationAdministeredHistoryModel) {
		...
		model.administrationNotes = faker.random.words();
		model.administeredQuantity = faker.random.number();
		model.takenHomeQuantity = faker.random.number();
		model.returnedQuantity = faker.random.number();
