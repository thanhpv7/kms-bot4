...
export class FacilityHistoryDataFactory extends AbstractDataFactory<FacilityHistoryModel> {
	...
	create(): FacilityHistoryModel {
		let newModel = new FacilityHistoryModel();
	...
	createAll(size: number = 10): FacilityHistoryModel[] {
		let newModels: FacilityHistoryModel[] = [];
	...
	/**
	 * Given a blank model of type Facility History, populate the model with its properties.
	 ...
	 */
	populate(model: FacilityHistoryModel) {
		...
		model.changeDate = new Date();
		model.facilityNo = faker.random.words();
		model.currentQty = faker.random.number();
		model.changedQty = faker.random.number();
		model.facilityStatus = faker.random.words();
		model.facilityClass = faker.random.words();
		model.facilityType = faker.random.words();
		model.facilityService = faker.random.words();
		model.bedFacilityId = faker.random.words();
