...
export class NhisGDRGDataFactory extends AbstractDataFactory<NhisGDRGModel> {
	...
	create(): NhisGDRGModel {
		let newModel = new NhisGDRGModel();
	...
	createAll(size: number = 10): NhisGDRGModel[] {
		let newModels: NhisGDRGModel[] = [];
	...
	/**
	 * Given a blank model of type NHIS GDRG, populate the model with its properties.
	...
	populate(model: NhisGDRGModel) {
		...
		model.coreGDRGCode = faker.random.words();
		model.description = faker.random.words();
		model.category = faker.random.words();
		model.levelOfHealthcare = faker.random.words();
		model.investigationCode = faker.random.words();
		model.icd10s = faker.random.words();
