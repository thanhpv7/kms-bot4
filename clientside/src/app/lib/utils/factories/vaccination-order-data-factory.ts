...
export class VaccinationOrderDataFactory extends AbstractDataFactory<VaccinationOrderModel> {
	...
	create(): VaccinationOrderModel {
		let newModel = new VaccinationOrderModel();
	...
	createAll(size: number = 10): VaccinationOrderModel[] {
		let newModels: VaccinationOrderModel[] = [];
	...
	/**
	 * Given a blank model of type Vaccination Order, populate the model with its properties.
	 ...
	 */
	populate(model: VaccinationOrderModel) {
		...
		model.vaccinationLocation = faker.random.words();
		model.vaccinationReaction = faker.random.words();
		model.purpose = faker.random.words();
		model.vaccinationNotes = faker.random.words();
		model.reactionNotes = faker.random.words();
