...
export class BpjsPcareRegistrationsDataFactory extends AbstractDataFactory<BpjsPcareRegistrationsModel> {
	...
	create(): BpjsPcareRegistrationsModel {
		let newModel = new BpjsPcareRegistrationsModel();
	...
	createAll(size: number = 10): BpjsPcareRegistrationsModel[] {
		let newModels: BpjsPcareRegistrationsModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Pcare Registrations, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsPcareRegistrationsModel) {
		...
		model.kdproviderpeserta = faker.random.words();
		...
		model.rujukinternal = faker.random.words();
