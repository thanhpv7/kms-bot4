...
export class CatalogueNonMedicalDataFactory extends AbstractDataFactory<CatalogueNonMedicalModel> {
	...
	create(): CatalogueNonMedicalModel {
		let newModel = new CatalogueNonMedicalModel();
	...
	createAll(size: number = 10): CatalogueNonMedicalModel[] {
		let newModels: CatalogueNonMedicalModel[] = [];
	...
	/**
	 * Given a blank model of type Catalogue Non Medical, populate the model with its properties.
	 ...
	 */
	populate(model: CatalogueNonMedicalModel) {
		...
		model.manufacturer = faker.random.words();
		model.manufacturerItemNumber = faker.random.words();
		model.isExpensiveItem = faker.random.boolean();
		model.isConsignmentItem = faker.random.boolean();
		model.model = faker.random.words();
