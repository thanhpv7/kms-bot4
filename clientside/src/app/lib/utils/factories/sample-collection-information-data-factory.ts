...
export class SampleCollectionInformationDataFactory extends AbstractDataFactory<SampleCollectionInformationModel> {
	...
	create(): SampleCollectionInformationModel {
		let newModel = new SampleCollectionInformationModel();
	...
	createAll(size: number = 10): SampleCollectionInformationModel[] {
		let newModels: SampleCollectionInformationModel[] = [];
	...
	/**
	 * Given a blank model of type Sample Collection Information, populate the model with its properties.
	 ...
	 */
	populate(model: SampleCollectionInformationModel) {
		...
		model.patientInformed = faker.random.boolean();
		model.patientConsented = faker.random.boolean();
		model.patientFastedEnough = faker.random.boolean();
		model.medicationTakenIn24Hours = faker.random.boolean();
		model.medicationTakenIn72Hours = faker.random.boolean();
