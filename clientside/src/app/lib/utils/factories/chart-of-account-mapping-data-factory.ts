...
import {ChartOfAccountMappingModel} from '../../../models/chartOfAccountMapping/chart_of_account_mapping.model';
import * as faker from 'faker';

export class ChartOfAccountMappingDataFactory extends AbstractDataFactory<ChartOfAccountMappingModel> {
	...
	create(): ChartOfAccountMappingModel {
		let newModel = new ChartOfAccountMappingModel();
	...
	createAll(size: number = 10): ChartOfAccountMappingModel[] {
		let newModels: ChartOfAccountMappingModel[] = [];
	...
	/**
	 * Given a blank model of type Chart of Account Mapping, populate the model with its properties.
	 ...
	 */
	populate(model: ChartOfAccountMappingModel) {
		...
		model.accountTypeDescription = faker.random.words();
