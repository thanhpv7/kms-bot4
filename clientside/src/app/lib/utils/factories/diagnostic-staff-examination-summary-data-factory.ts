...
export class DiagnosticStaffExaminationSummaryDataFactory extends AbstractDataFactory<DiagnosticStaffExaminationSummaryModel> {
	...
	create(): DiagnosticStaffExaminationSummaryModel {
		let newModel = new DiagnosticStaffExaminationSummaryModel();
	...
	createAll(size: number = 10): DiagnosticStaffExaminationSummaryModel[] {
		let newModels: DiagnosticStaffExaminationSummaryModel[] = [];
	...
	/**
	 * Given a blank model of type Diagnostic Staff Examination Summary, populate the model with its properties.
	 ...
	 */
	populate(model: DiagnosticStaffExaminationSummaryModel) {
		...
		model.medicalTranscriber = faker.random.words();
		model.referringRoom = faker.random.words();
		model.referingUnit = faker.random.words();
		model.examinationLocation = faker.random.words();
		model.additionalNotes = faker.random.words();
		model.conclusion = faker.random.words();
