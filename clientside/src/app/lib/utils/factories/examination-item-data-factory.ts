...
export class ExaminationItemDataFactory extends AbstractDataFactory<ExaminationItemModel> {
	...
	create(): ExaminationItemModel {
		let newModel = new ExaminationItemModel();
	...
	createAll(size: number = 10): ExaminationItemModel[] {
		let newModels: ExaminationItemModel[] = [];
	...
	/**
	 * Given a blank model of type Examination Item, populate the model with its properties.
	 ...
	 */
	populate(model: ExaminationItemModel) {
		...
		model.itemCode = faker.random.words();
		model.description = faker.random.words();
		model.sample = faker.random.words();
		model.examinationLevel = faker.random.words();
		model.examinationType = faker.random.words();
