...
import {PatientEmergencyContactDetailModel} from '../../../models/patientEmergencyContactDetail/patient_emergency_contact_detail.model';
import * as faker from 'faker';

export class PatientEmergencyContactDetailDataFactory extends AbstractDataFactory<PatientEmergencyContactDetailModel> {
	...
	create(): PatientEmergencyContactDetailModel {
		let newModel = new PatientEmergencyContactDetailModel();
	...
	createAll(size: number = 10): PatientEmergencyContactDetailModel[] {
		let newModels: PatientEmergencyContactDetailModel[] = [];
	...
	/**
	 * Given a blank model of type Patient Emergency Contact Detail, populate the model with its properties.
	 ...
	 */
	populate(model: PatientEmergencyContactDetailModel) {
		...
		model.givenName = faker.random.words();
		...
		model.officePhoneNo = faker.random.words();
		model.extensionNo = faker.random.words();
		model.emailAddress = faker.random.words();
		model.idCard = faker.random.words();
