...
import {TariffSchemaModel} from '../../../models/tariffSchema/tariff_schema.model';
import * as faker from 'faker';

export class TariffSchemaDataFactory extends AbstractDataFactory<TariffSchemaModel> {
	...
	create(): TariffSchemaModel {
		let newModel = new TariffSchemaModel();
	...
	createAll(size: number = 10): TariffSchemaModel[] {
		let newModels: TariffSchemaModel[] = [];
	...
	/**
	 * Given a blank model of type Tariff Schema, populate the model with its properties.
	 ...
	 */
	populate(model: TariffSchemaModel) {
		...
		model.schemaCode = faker.random.words();
		model.schemeName = faker.random.words();
