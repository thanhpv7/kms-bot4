...
export class PrescriptionHeaderDataFactory extends AbstractDataFactory<PrescriptionHeaderModel> {
	...
	create(): PrescriptionHeaderModel {
		let newModel = new PrescriptionHeaderModel();
	...
	createAll(size: number = 10): PrescriptionHeaderModel[] {
		let newModels: PrescriptionHeaderModel[] = [];
	...
	/**
	 * Given a blank model of type Prescription Header, populate the model with its properties.
	 ...
	 */
	populate(model: PrescriptionHeaderModel) {
		...
		model.prescriptionNumber = faker.random.words();
		model.prescriptionType = faker.random.words();
		model.queueNo = faker.random.words();
		model.originalPrescriptionNumber = faker.random.words();
		model.prescriptionDate = new Date();
		model.isCompound = faker.random.boolean();
		model.isIteration = faker.random.boolean();
		model.status = faker.random.words();
		model.isUrgent = faker.random.boolean();
		model.restricted = faker.random.boolean();
		model.notes = faker.random.words();
		model.revisionNo = faker.random.number();
