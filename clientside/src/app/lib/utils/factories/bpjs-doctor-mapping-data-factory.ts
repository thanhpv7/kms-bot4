...
import {BpjsDoctorMappingModel} from '../../../models/bpjsDoctorMapping/bpjs_doctor_mapping.model';
import * as faker from 'faker';

export class BpjsDoctorMappingDataFactory extends AbstractDataFactory<BpjsDoctorMappingModel> {
	...
	create(): BpjsDoctorMappingModel {
		let newModel = new BpjsDoctorMappingModel();
	...
	createAll(size: number = 10): BpjsDoctorMappingModel[] {
		let newModels: BpjsDoctorMappingModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Doctor Mapping, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsDoctorMappingModel) {
		...
		model.bpjsMedicalStaff = faker.random.words();
		model.notes = faker.random.words();
		model.refCodeService = faker.random.words();
