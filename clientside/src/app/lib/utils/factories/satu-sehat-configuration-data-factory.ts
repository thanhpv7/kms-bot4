...
import {SatuSehatConfigurationModel} from '../../../models/satuSehatConfiguration/satu_sehat_configuration.model';
import * as faker from 'faker';

export class SatuSehatConfigurationDataFactory extends AbstractDataFactory<SatuSehatConfigurationModel> {
	...
	create(): SatuSehatConfigurationModel {
		let newModel = new SatuSehatConfigurationModel();
	...
	createAll(size: number = 10): SatuSehatConfigurationModel[] {
		let newModels: SatuSehatConfigurationModel[] = [];
	...
	/**
	 * Given a blank model of type Satu Sehat Configuration, populate the model with its properties.
	 ...
	 */
	populate(model: SatuSehatConfigurationModel) {
		...
		model.organizationID = faker.random.words();
		model.clientID = faker.random.words();
		model.secretKey = faker.random.words();
		model.tokenRequest = faker.random.words();
		model.baseURL = faker.random.words();
