...
export class LocationDataFactory extends AbstractDataFactory<LocationModel> {
	...
	create(): LocationModel {
		let newModel = new LocationModel();
	...
	createAll(size: number = 10): LocationModel[] {
		let newModels: LocationModel[] = [];
	...
	/**
	 * Given a blank model of type Location, populate the model with its properties.
	...
	populate(model: LocationModel) {
		...
		model.locationName = faker.random.words();
		model.aisle = faker.random.words();
		model.shelf = faker.random.words();
		model.bin = faker.random.words();
