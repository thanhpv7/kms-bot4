...
import {WarehouseInventoryModel} from '../../../models/warehouseInventory/warehouse_inventory.model';
import * as faker from 'faker';

export class WarehouseInventoryDataFactory extends AbstractDataFactory<WarehouseInventoryModel> {
	...
	create(): WarehouseInventoryModel {
		let newModel = new WarehouseInventoryModel();
	...
	createAll(size: number = 10): WarehouseInventoryModel[] {
		let newModels: WarehouseInventoryModel[] = [];
	...
	/**
	 * Given a blank model of type Warehouse Inventory, populate the model with its properties.
	 ...
	 */
	populate(model: WarehouseInventoryModel) {
		...
		model.reservedQuantity = faker.random.number();
