...
export class PcareDiagnosaDataFactory extends AbstractDataFactory<PcareDiagnosaModel> {
	...
	create(): PcareDiagnosaModel {
		let newModel = new PcareDiagnosaModel();
	...
	createAll(size: number = 10): PcareDiagnosaModel[] {
		let newModels: PcareDiagnosaModel[] = [];
	...
	/**
	 * Given a blank model of type PCare Diagnosa, populate the model with its properties.
	 ...
	 */
	populate(model: PcareDiagnosaModel) {
		...
		model.kodeDiagnosa = faker.random.words();
		model.namaDiagnosa = faker.random.words();
		model.nonSpesialis = faker.random.boolean();
