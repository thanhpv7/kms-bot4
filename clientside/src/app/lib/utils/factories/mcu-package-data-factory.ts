...
export class McuPackageDataFactory extends AbstractDataFactory<McuPackageModel> {
	...
	create(): McuPackageModel {
		let newModel = new McuPackageModel();
	...
	createAll(size: number = 10): McuPackageModel[] {
		let newModels: McuPackageModel[] = [];
	...
	/**
	 * Given a blank model of type MCU Package, populate the model with its properties.
	 ...
	 */
	populate(model: McuPackageModel) {
		...
		model.mcuPackageCode = faker.random.words();
		model.description = faker.random.words();
		model.packageType = faker.random.words();
