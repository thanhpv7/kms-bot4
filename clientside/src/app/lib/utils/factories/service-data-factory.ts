...
export class ServiceDataFactory extends AbstractDataFactory<ServiceModel> {
	...
	createAll(size: number = 10): ServiceModel[] {
		let newModels: ServiceModel[] = [];
	...
	/**
	 * Given a blank model of type Service, populate the model with its properties.
	...
	populate(model: ServiceModel) {
		...
		model.serviceNumber = faker.random.words();
		model.serviceName = faker.random.words();
		model.sectionCode = faker.random.words();
		model.byPassDoctorAssignment = faker.random.boolean();
		model.hasExaminationItem = faker.random.boolean();
		model.revenueCenter = faker.random.words();
		model.serviceCode = faker.random.words();
