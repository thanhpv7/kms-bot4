...
export class BpjsClaimSubmissionDataFactory extends AbstractDataFactory<BpjsClaimSubmissionModel> {
	...
	create(): BpjsClaimSubmissionModel {
		let newModel = new BpjsClaimSubmissionModel();
	...
	createAll(size: number = 10): BpjsClaimSubmissionModel[] {
		let newModels: BpjsClaimSubmissionModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Claim Submission, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsClaimSubmissionModel) {
		...
		model.insuranceProvider = faker.random.words();
		model.treatmentType = faker.random.words();
		model.followUpPlan = faker.random.words();
		model.controlDate = new Date();
		model.additionalDiagnosis = faker.random.words();
		model.additionalProcedure = faker.random.words();
		// % protected region % [Add Kelas Rawatcustom enum randomisation here] off begin
		// Always initialise to the first enum literal. Turn on the protected region and put in your own custom enum
		// initialisation logic.
		model.kelasRawat = KelasRawatEnum.TANPA_KELAS;
		// % protected region % [Add Kelas Rawatcustom enum randomisation here] end
