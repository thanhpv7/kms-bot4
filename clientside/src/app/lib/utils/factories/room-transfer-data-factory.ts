...
export class RoomTransferDataFactory extends AbstractDataFactory<RoomTransferModel> {
	...
	create(): RoomTransferModel {
		let newModel = new RoomTransferModel();
	...
	createAll(size: number = 10): RoomTransferModel[] {
		let newModels: RoomTransferModel[] = [];
	...
	/**
	 * Given a blank model of type Room Transfer, populate the model with its properties.
	 ...
	 */
	populate(model: RoomTransferModel) {
		...
		model.transferringStaffName = faker.random.words();
