...
export class InpatientMedicalExaminationRecordDataFactory extends AbstractDataFactory<InpatientMedicalExaminationRecordModel> {
	...
	create(): InpatientMedicalExaminationRecordModel {
		let newModel = new InpatientMedicalExaminationRecordModel();
	...
	createAll(size: number = 10): InpatientMedicalExaminationRecordModel[] {
		let newModels: InpatientMedicalExaminationRecordModel[] = [];
	...
	/**
	 * Given a blank model of type Inpatient Medical Examination Record, populate the model with its properties.
	 ...
	 */
	populate(model: InpatientMedicalExaminationRecordModel) {
		...
		model.inpatientCase = faker.random.words();
		model.serviceCase = faker.random.words();
		model.dependencyLevel = faker.random.words();
		model.currentDependency = faker.random.words();
		model.infantCondition = faker.random.words();
