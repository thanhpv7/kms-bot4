...
export class InventoryBatchCorrectionDataFactory extends AbstractDataFactory<InventoryBatchCorrectionModel> {
	...
	create(): InventoryBatchCorrectionModel {
		let newModel = new InventoryBatchCorrectionModel();
	...
	createAll(size: number = 10): InventoryBatchCorrectionModel[] {
		let newModels: InventoryBatchCorrectionModel[] = [];
	...
	/**
	 * Given a blank model of type Inventory Batch Correction, populate the model with its properties.
	 ...
	 */
	populate(model: InventoryBatchCorrectionModel) {
		...
		model.batchCorrectionNumber = faker.random.words();
		model.approvalStatus = faker.random.words();
		model.transactionDate = new Date();
		// % protected region % [Add Transaction Typecustom enum randomisation here] off begin
		// Always initialise to the first enum literal. Turn on the protected region and put in your own custom enum
		// initialisation logic.
		model.transactionType = BatchCorrectionTypeEnum.ADJUSTMENT_BATCH;
		// % protected region % [Add Transaction Typecustom enum randomisation here] end
