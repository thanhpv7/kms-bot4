...
import {BpjsPcareDPHOModel} from '../../../models/bpjsPcareDPHO/bpjs_pcare_dpho.model';
import * as faker from 'faker';

export class BpjsPcareDPHODataFactory extends AbstractDataFactory<BpjsPcareDPHOModel> {
	...
	create(): BpjsPcareDPHOModel {
		let newModel = new BpjsPcareDPHOModel();
	...
	createAll(size: number = 10): BpjsPcareDPHOModel[] {
		let newModels: BpjsPcareDPHOModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Pcare DPHO, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsPcareDPHOModel) {
		...
		model.kdobat = faker.random.words();
		model.nmobat = faker.random.words();
		model.sedia = faker.random.words();
