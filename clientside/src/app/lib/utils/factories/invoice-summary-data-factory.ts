...
export class InvoiceSummaryDataFactory extends AbstractDataFactory<InvoiceSummaryModel> {
	...
	create(): InvoiceSummaryModel {
		let newModel = new InvoiceSummaryModel();
	...
	createAll(size: number = 10): InvoiceSummaryModel[] {
		let newModels: InvoiceSummaryModel[] = [];
	...
	/**
	 * Given a blank model of type Invoice Summary, populate the model with its properties.
	 ...
	 */
	populate(model: InvoiceSummaryModel) {
		...
		model.taxOnOtherDiscount = faker.random.boolean();
		model.otherDiscount = faker.random.number();
		model.taxOnOtherFee = faker.random.boolean();
		model.otherFee = faker.random.number();
		model.tax = faker.random.number();
		model.totalInvoice = faker.random.number();
		model.availableDownPayment = faker.random.number();
		model.totalCoverage = faker.random.number();
		model.outstandingAmount = faker.random.number();
		model.bpjsCoverage = faker.random.number();
