...
export class InventoryAdjustmentDataFactory extends AbstractDataFactory<InventoryAdjustmentModel> {
	...
	create(): InventoryAdjustmentModel {
		let newModel = new InventoryAdjustmentModel();
	...
	createAll(size: number = 10): InventoryAdjustmentModel[] {
		let newModels: InventoryAdjustmentModel[] = [];
	...
	/**
	 * Given a blank model of type Inventory Adjustment, populate the model with its properties.
	 ...
	 */
	populate(model: InventoryAdjustmentModel) {
		...
		model.adjustmentNumber = faker.random.words();
		...
		model.adjustmentType = faker.random.words();
