...
export class InvoicePaymentInsuranceDataFactory extends AbstractDataFactory<InvoicePaymentInsuranceModel> {
	...
	create(): InvoicePaymentInsuranceModel {
		let newModel = new InvoicePaymentInsuranceModel();
	...
	createAll(size: number = 10): InvoicePaymentInsuranceModel[] {
		let newModels: InvoicePaymentInsuranceModel[] = [];
	...
	/**
	 * Given a blank model of type Invoice Payment Insurance, populate the model with its properties.
	 ...
	 */
	populate(model: InvoicePaymentInsuranceModel) {
		...
		model.mainInsurance = faker.random.boolean();
