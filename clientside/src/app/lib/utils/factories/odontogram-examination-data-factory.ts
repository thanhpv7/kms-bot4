...
export class OdontogramExaminationDataFactory extends AbstractDataFactory<OdontogramExaminationModel> {
	...
	create(): OdontogramExaminationModel {
		let newModel = new OdontogramExaminationModel();
	...
	createAll(size: number = 10): OdontogramExaminationModel[] {
		let newModels: OdontogramExaminationModel[] = [];
	...
	/**
	 * Given a blank model of type Odontogram Examination, populate the model with its properties.
	 ...
	 */
	populate(model: OdontogramExaminationModel) {
		...
		model.odontogramData = faker.random.words();
		model.isPermanent = faker.random.boolean();
