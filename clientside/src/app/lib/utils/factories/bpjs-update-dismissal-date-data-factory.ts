...
export class BpjsUpdateDismissalDateDataFactory extends AbstractDataFactory<BpjsUpdateDismissalDateModel> {
	...
	create(): BpjsUpdateDismissalDateModel {
		let newModel = new BpjsUpdateDismissalDateModel();
	...
	createAll(size: number = 10): BpjsUpdateDismissalDateModel[] {
		let newModels: BpjsUpdateDismissalDateModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Update Dismissal Date, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsUpdateDismissalDateModel) {
		...
		model.deathCertificateLetterNo = faker.random.words();
		model.timeOfDeath = new Date();
		model.dismissalDate = new Date();
		model.policeReportNo = faker.random.words();
