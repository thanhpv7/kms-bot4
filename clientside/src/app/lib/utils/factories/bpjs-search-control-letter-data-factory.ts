...
export class BpjsSearchControlLetterDataFactory extends AbstractDataFactory<BpjsSearchControlLetterModel> {
	...
	create(): BpjsSearchControlLetterModel {
		let newModel = new BpjsSearchControlLetterModel();
	...
	createAll(size: number = 10): BpjsSearchControlLetterModel[] {
		let newModels: BpjsSearchControlLetterModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Search Control Letter, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsSearchControlLetterModel) {
		...
		model.controlLetterNo = faker.random.words();
		model.controlPlanDate = faker.random.words();
		model.date = faker.random.words();
		model.controlType = faker.random.words();
		...
		model.polyclinicDestination = faker.random.words();
		model.doctorDestination = faker.random.words();
		model.polyclinicSource = faker.random.words();
		model.doctorSource = faker.random.words();
		...
		model.provider = faker.random.words();
		model.referralProvider = faker.random.words();
		model.referralSource = faker.random.words();
