...
export class TransferOrderStockDetailDataFactory extends AbstractDataFactory<TransferOrderStockDetailModel> {
	...
	create(): TransferOrderStockDetailModel {
		let newModel = new TransferOrderStockDetailModel();
	...
	createAll(size: number = 10): TransferOrderStockDetailModel[] {
		let newModels: TransferOrderStockDetailModel[] = [];
	...
	/**
	 * Given a blank model of type Transfer Order Stock Detail, populate the model with its properties.
	 ...
	 */
	populate(model: TransferOrderStockDetailModel) {
		...
		model.itemNumber = faker.random.number();
		model.sourceAvailableQuantity = faker.random.number();
		model.destinationSOH = faker.random.number();
		model.quantityOrdered = faker.random.number();
		model.uom = faker.random.words();
		model.transactionStatus = faker.random.words();
		model.quantityIssued = faker.random.number();
		model.quantityReceived = faker.random.number();
		model.sourceMinLevel = faker.random.number();
		model.destinationMaxLevel = faker.random.number();
		model.quantityTransferred = faker.random.number();
		model.quantityFulfilled = faker.random.number();
		model.orderDateTime = new Date();
