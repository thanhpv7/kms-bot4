...
export class InvoicePaymentSelfPayingDataFactory extends AbstractDataFactory<InvoicePaymentSelfPayingModel> {
	...
	create(): InvoicePaymentSelfPayingModel {
		let newModel = new InvoicePaymentSelfPayingModel();
	...
	createAll(size: number = 10): InvoicePaymentSelfPayingModel[] {
		let newModels: InvoicePaymentSelfPayingModel[] = [];
	...
	/**
	 * Given a blank model of type Invoice Payment Self Paying, populate the model with its properties.
	 ...
	 */
	populate(model: InvoicePaymentSelfPayingModel) {
