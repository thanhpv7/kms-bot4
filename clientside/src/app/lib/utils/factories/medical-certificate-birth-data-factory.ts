...
import {MedicalCertificateBirthModel} from '../../../models/medicalCertificateBirth/medical_certificate_birth.model';
import * as faker from 'faker';

export class MedicalCertificateBirthDataFactory extends AbstractDataFactory<MedicalCertificateBirthModel> {
	...
	create(): MedicalCertificateBirthModel {
		let newModel = new MedicalCertificateBirthModel();
	...
	createAll(size: number = 10): MedicalCertificateBirthModel[] {
		let newModels: MedicalCertificateBirthModel[] = [];
	...
	/**
	 * Given a blank model of type Medical Certificate Birth, populate the model with its properties.
	 ...
	 */
	populate(model: MedicalCertificateBirthModel) {
		...
		model.patientName = faker.random.words();
		model.spouseName = faker.random.words();
		model.numberOfChildren = faker.random.number();
		model.birthNotes = faker.random.words();
