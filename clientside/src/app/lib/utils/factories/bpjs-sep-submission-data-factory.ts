...
import {BpjsSEPSubmissionModel} from '../../../models/bpjsSEPSubmission/bpjs_sep_submission.model';
import * as faker from 'faker';

export class BpjsSEPSubmissionDataFactory extends AbstractDataFactory<BpjsSEPSubmissionModel> {
	...
	create(): BpjsSEPSubmissionModel {
		let newModel = new BpjsSEPSubmissionModel();
	...
	createAll(size: number = 10): BpjsSEPSubmissionModel[] {
		let newModels: BpjsSEPSubmissionModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS SEP Submission, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsSEPSubmissionModel) {
		...
		model.submissionType = faker.random.words();
