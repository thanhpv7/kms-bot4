...
export class TariffPackageItemDataFactory extends AbstractDataFactory<TariffPackageItemModel> {
	...
	create(): TariffPackageItemModel {
		let newModel = new TariffPackageItemModel();
	...
	createAll(size: number = 10): TariffPackageItemModel[] {
		let newModels: TariffPackageItemModel[] = [];
	...
	/**
	 * Given a blank model of type Tariff Package Item, populate the model with its properties.
	 ...
	 */
	populate(model: TariffPackageItemModel) {
		...
		model.tariffName = faker.random.words();
		model.quantity = faker.random.number();
		model.unit = faker.random.words();
		model.modifiable = faker.random.boolean();
