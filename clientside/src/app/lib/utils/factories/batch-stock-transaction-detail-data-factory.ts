...
export class BatchStockTransactionDetailDataFactory extends AbstractDataFactory<BatchStockTransactionDetailModel> {
	...
	create(): BatchStockTransactionDetailModel {
		let newModel = new BatchStockTransactionDetailModel();
	...
	createAll(size: number = 10): BatchStockTransactionDetailModel[] {
		let newModels: BatchStockTransactionDetailModel[] = [];
	...
	/**
	 * Given a blank model of type Batch Stock Transaction Detail, populate the model with its properties.
	 ...
	 */
	populate(model: BatchStockTransactionDetailModel) {
		...
		model.batchNo = faker.random.words();
		model.expirationDate = new Date();
		model.itemLineNumber = faker.random.number();
		model.transactionNumber = faker.random.words();
		model.transactionDate = new Date();
		model.batchTransactionType = faker.random.words();
		model.batchQtyUoi = faker.random.number();
		model.batchQtyUomTransaction = faker.random.number();
		model.uoi = faker.random.words();
		model.uomTransaction = faker.random.words();
		model.batchItemNotes = faker.random.words();
