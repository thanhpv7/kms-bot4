...
export class BpjsJasaRaharjaDataFactory extends AbstractDataFactory<BpjsJasaRaharjaModel> {
	...
	create(): BpjsJasaRaharjaModel {
		let newModel = new BpjsJasaRaharjaModel();
	...
	createAll(size: number = 10): BpjsJasaRaharjaModel[] {
		let newModels: BpjsJasaRaharjaModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Jasa Raharja, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsJasaRaharjaModel) {
		...
		model.initialSEPNo = faker.random.words();
		model.guaranteeLetterNo = faker.random.words();
		model.incidentDate = new Date();
		model.registrationNo = faker.random.words();
		model.noSEPSuplesi = faker.random.words();
		model.tanggalSEPSuplesi = new Date();
