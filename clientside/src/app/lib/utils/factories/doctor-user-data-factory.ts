...
export class DoctorUserDataFactory extends UserDataFactory<DoctorUserModel> {
	...
	create(): DoctorUserModel {
		let newModel = new DoctorUserModel();
	...
	createAll(size: number = 10): DoctorUserModel[] {
		let newModels: DoctorUserModel[] = [];
	...
	/**
	 * Given a blank model of type Doctor User, populate the model with its properties.
	 ...
	 */
	populate(model: DoctorUserModel) {
