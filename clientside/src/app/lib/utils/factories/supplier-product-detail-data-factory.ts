...
export class SupplierProductDetailDataFactory extends AbstractDataFactory<SupplierProductDetailModel> {
	...
	create(): SupplierProductDetailModel {
		let newModel = new SupplierProductDetailModel();
	...
	createAll(size: number = 10): SupplierProductDetailModel[] {
		let newModels: SupplierProductDetailModel[] = [];
	...
	/**
	 * Given a blank model of type Supplier Product Detail, populate the model with its properties.
	 ...
	 */
	populate(model: SupplierProductDetailModel) {
		...
		model.unitCode = faker.random.words();
		model.unitName = faker.random.words();
