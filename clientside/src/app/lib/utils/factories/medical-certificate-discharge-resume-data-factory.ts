...
export class MedicalCertificateDischargeResumeDataFactory extends AbstractDataFactory<MedicalCertificateDischargeResumeModel> {
	...
	create(): MedicalCertificateDischargeResumeModel {
		let newModel = new MedicalCertificateDischargeResumeModel();
	...
	createAll(size: number = 10): MedicalCertificateDischargeResumeModel[] {
		let newModels: MedicalCertificateDischargeResumeModel[] = [];
	...
	/**
	 * Given a blank model of type Medical Certificate  Discharge Resume, populate the model with its properties.
	 ...
	 */
	populate(model: MedicalCertificateDischargeResumeModel) {
		...
		model.admissionDateTime = new Date();
		model.dischargeDateTime = new Date();
		model.condition = faker.random.words();
		model.mainDiagnosis = faker.random.words();
		model.action = faker.random.words();
