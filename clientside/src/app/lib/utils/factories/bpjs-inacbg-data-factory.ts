...
import {BpjsINACBGModel} from '../../../models/bpjsINACBG/bpjs_inacbg.model';
import * as faker from 'faker';

export class BpjsINACBGDataFactory extends AbstractDataFactory<BpjsINACBGModel> {
	...
	create(): BpjsINACBGModel {
		let newModel = new BpjsINACBGModel();
	...
	createAll(size: number = 10): BpjsINACBGModel[] {
		let newModels: BpjsINACBGModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS INACBG, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsINACBGModel) {
		...
		model.stateProvince = faker.random.words();
		model.cityRegency = faker.random.words();
		model.regional = faker.random.words();
		model.hospitalClass = faker.random.words();
		model.inacbgTariff1 = faker.random.words();
		model.inacbgTariff2 = faker.random.words();
		model.additionalUpgradeVIP = faker.random.words();
		model.payplanID = faker.random.words();
		model.paymentPlanCode = faker.random.words();
		model.tariffCode = faker.random.words();
		model.url = faker.random.words();
		model.username = faker.random.words();
		model.password = faker.random.words();
		model.secretKey = faker.random.words();
		model.coderID = faker.random.words();
