...
export class BpjsPcareKesadaranDataFactory extends AbstractDataFactory<BpjsPcareKesadaranModel> {
	...
	create(): BpjsPcareKesadaranModel {
		let newModel = new BpjsPcareKesadaranModel();
	...
	createAll(size: number = 10): BpjsPcareKesadaranModel[] {
		let newModels: BpjsPcareKesadaranModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Pcare Kesadaran, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsPcareKesadaranModel) {
		...
		model.kdsadar = faker.random.words();
		model.nmsadar = faker.random.words();
		model.kesadaranInternal = faker.random.words();
