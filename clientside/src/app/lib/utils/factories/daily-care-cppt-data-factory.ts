...
export class DailyCareCPPTDataFactory extends AbstractDataFactory<DailyCareCPPTModel> {
	...
	create(): DailyCareCPPTModel {
		let newModel = new DailyCareCPPTModel();
	...
	createAll(size: number = 10): DailyCareCPPTModel[] {
		let newModels: DailyCareCPPTModel[] = [];
	...
	/**
	 * Given a blank model of type Daily Care CPPT, populate the model with its properties.
	 ...
	 */
	populate(model: DailyCareCPPTModel) {
		...
		model.verifiedDate = new Date();
		model.verifiedBy = faker.random.words();
		model.canceledDate = new Date();
		model.canceledBy = faker.random.words();
		model.status = faker.random.words();
		model.cpptDateTime = new Date();
