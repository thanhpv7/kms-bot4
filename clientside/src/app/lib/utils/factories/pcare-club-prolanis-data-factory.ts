...
export class PcareClubProlanisDataFactory extends AbstractDataFactory<PcareClubProlanisModel> {
	...
	create(): PcareClubProlanisModel {
		let newModel = new PcareClubProlanisModel();
	...
	createAll(size: number = 10): PcareClubProlanisModel[] {
		let newModels: PcareClubProlanisModel[] = [];
	...
	/**
	 * Given a blank model of type PCare Club Prolanis, populate the model with its properties.
	 ...
	 */
	populate(model: PcareClubProlanisModel) {
		...
		model.kodeProgram = faker.random.words();
		model.namaProgram = faker.random.words();
		model.tanggalMulai = faker.random.words();
		model.tanggalAkhir = faker.random.words();
		model.alamatClub = faker.random.words();
		model.namaClub = faker.random.words();
		model.noHPKetua = faker.random.words();
		model.namaKetua = faker.random.words();
