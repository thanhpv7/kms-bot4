...
export class BpjsListTaskDataFactory extends AbstractDataFactory<BpjsListTaskModel> {
	...
	create(): BpjsListTaskModel {
		let newModel = new BpjsListTaskModel();
	...
	createAll(size: number = 10): BpjsListTaskModel[] {
		let newModels: BpjsListTaskModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS List Task, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsListTaskModel) {
		...
		model.kodebooking = faker.random.words();
		model.taskname = faker.random.words();
		model.taskid = faker.random.number();
		model.waktu = faker.random.words();
		model.wakturs = faker.random.words();
