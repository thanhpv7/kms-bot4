...
export class PrivilegeDataFactory extends AbstractDataFactory<PrivilegeModel> {
	...
	create(): PrivilegeModel {
		let newModel = new PrivilegeModel();
	...
	createAll(size: number = 10): PrivilegeModel[] {
		let newModels: PrivilegeModel[] = [];
	...
	/**
	 * Given a blank model of type Privilege, populate the model with its properties.
	...
	populate(model: PrivilegeModel) {
		...
		model.targetEntity = faker.random.words();
		model.allowCreate = faker.random.boolean();
		model.allowRead = faker.random.boolean();
		model.allowUpdate = faker.random.boolean();
		model.allowDelete = faker.random.boolean();
