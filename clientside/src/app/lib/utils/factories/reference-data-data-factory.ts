...
export class ReferenceDataDataFactory extends AbstractDataFactory<ReferenceDataModel> {
	...
	create(): ReferenceDataModel {
		let newModel = new ReferenceDataModel();
	...
	createAll(size: number = 10): ReferenceDataModel[] {
		let newModels: ReferenceDataModel[] = [];
	...
	/**
	 * Given a blank model of type Reference Data, populate the model with its properties.
	 ...
	 */
	populate(model: ReferenceDataModel) {
		...
		model.referenceType = faker.random.words();
		model.code = faker.random.words();
		model.description = faker.random.words();
		model.associatedValues = faker.random.words();
		model.sequence = faker.random.number();
		model.filter = faker.random.words();
