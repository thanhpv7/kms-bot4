...
export class ProcessWorkflowDataFactory extends AbstractDataFactory<ProcessWorkflowModel> {
	...
	create(): ProcessWorkflowModel {
		let newModel = new ProcessWorkflowModel();
	...
	createAll(size: number = 10): ProcessWorkflowModel[] {
		let newModels: ProcessWorkflowModel[] = [];
	...
	/**
	 * Given a blank model of type Process Workflow, populate the model with its properties.
	 ...
	 */
	populate(model: ProcessWorkflowModel) {
		...
		model.stepNumber = faker.random.number();
		// % protected region % [Add Module Namecustom enum randomisation here] off begin
		// Always initialise to the first enum literal. Turn on the protected region and put in your own custom enum
		// initialisation logic.
		model.moduleName = ModuleNameEnum.REGISTRATION;
		// % protected region % [Add Module Namecustom enum randomisation here] end
		model.processName = faker.random.words();
		model.referenceID = faker.random.words();
		model.processDateTime = new Date();
