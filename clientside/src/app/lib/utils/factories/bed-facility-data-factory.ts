...
import {BedFacilityModel} from '../../../models/bedFacility/bed_facility.model';
import * as faker from 'faker';

export class BedFacilityDataFactory extends AbstractDataFactory<BedFacilityModel> {
	...
	create(): BedFacilityModel {
		let newModel = new BedFacilityModel();
	...
	createAll(size: number = 10): BedFacilityModel[] {
		let newModels: BedFacilityModel[] = [];
	...
	/**
	 * Given a blank model of type Bed Facility, populate the model with its properties.
	 ...
	 */
	populate(model: BedFacilityModel) {
		...
		model.bedNumber = faker.random.words();
		model.bedName = faker.random.words();
		model.treatmentClass = faker.random.words();
		model.availability = faker.random.words();
