...
export class PurchaseRequisitionDataFactory extends AbstractDataFactory<PurchaseRequisitionModel> {
	...
	create(): PurchaseRequisitionModel {
		let newModel = new PurchaseRequisitionModel();
	...
	createAll(size: number = 10): PurchaseRequisitionModel[] {
		let newModels: PurchaseRequisitionModel[] = [];
	...
	/**
	 * Given a blank model of type Purchase Requisition, populate the model with its properties.
	 ...
	 */
	populate(model: PurchaseRequisitionModel) {
		...
		model.purchaseRequisitionNumber = faker.random.words();
		...
		model.requiredDate = new Date();
		model.totalEstimationOrder = faker.random.number();
		model.redraftingDateTime = new Date();
		model.submissionDateTime = new Date();
		model.approvalDateTime = new Date();
		model.rejectionDateTime = new Date();
		model.voidingDateTime = new Date();
