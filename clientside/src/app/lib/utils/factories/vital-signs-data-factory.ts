...
export class VitalSignsDataFactory extends AbstractDataFactory<VitalSignsModel> {
	...
	create(): VitalSignsModel {
		let newModel = new VitalSignsModel();
	...
	createAll(size: number = 10): VitalSignsModel[] {
		let newModels: VitalSignsModel[] = [];
	...
	/**
	 * Given a blank model of type Vital Signs, populate the model with its properties.
	 ...
	 */
	populate(model: VitalSignsModel) {
		...
		model.time = faker.random.words();
		model.hr = faker.random.words();
		model.rr = faker.random.words();
		model.temprature = faker.random.words();
		model.o2Saturation = faker.random.words();
		model.remarks = faker.random.words();
