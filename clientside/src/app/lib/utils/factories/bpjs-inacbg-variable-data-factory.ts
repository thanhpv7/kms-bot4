...
export class BpjsINACBGVariableDataFactory extends AbstractDataFactory<BpjsINACBGVariableModel> {
	...
	create(): BpjsINACBGVariableModel {
		let newModel = new BpjsINACBGVariableModel();
	...
	createAll(size: number = 10): BpjsINACBGVariableModel[] {
		let newModels: BpjsINACBGVariableModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS INACBG Variable, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsINACBGVariableModel) {
		...
		model.variableCode = faker.random.words();
		model.english = faker.random.words();
		model.indonesian = faker.random.words();
