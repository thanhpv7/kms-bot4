...
import {BpjsAccidentMasterModel} from '../../../models/bpjsAccidentMaster/bpjs_accident_master.model';
import * as faker from 'faker';

export class BpjsAccidentMasterDataFactory extends AbstractDataFactory<BpjsAccidentMasterModel> {
	...
	create(): BpjsAccidentMasterModel {
		let newModel = new BpjsAccidentMasterModel();
	...
	createAll(size: number = 10): BpjsAccidentMasterModel[] {
		let newModels: BpjsAccidentMasterModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Accident Master, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsAccidentMasterModel) {
		...
		model.nosep = faker.random.words();
		model.tglkejadian = faker.random.words();
		model.ppkpelsep = faker.random.words();
		model.kdprop = faker.random.words();
		model.kdkab = faker.random.words();
		model.kdkec = faker.random.words();
		model.ketkejadian = faker.random.words();
		model.nosepsuplesi = faker.random.words();
