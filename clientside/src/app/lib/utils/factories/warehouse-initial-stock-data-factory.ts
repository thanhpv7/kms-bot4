...
import {WarehouseInitialStockModel} from '../../../models/warehouseInitialStock/warehouse_initial_stock.model';
import * as faker from 'faker';

export class WarehouseInitialStockDataFactory extends AbstractDataFactory<WarehouseInitialStockModel> {
	...
	create(): WarehouseInitialStockModel {
		let newModel = new WarehouseInitialStockModel();
	...
	createAll(size: number = 10): WarehouseInitialStockModel[] {
		let newModels: WarehouseInitialStockModel[] = [];
	...
	/**
	 * Given a blank model of type Warehouse Initial Stock , populate the model with its properties.
	 ...
	 */
	populate(model: WarehouseInitialStockModel) {
		...
		model.minimumLevel = faker.random.number();
		model.maximumLevel = faker.random.number();
