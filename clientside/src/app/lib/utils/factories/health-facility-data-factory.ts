...
export class HealthFacilityDataFactory extends AbstractDataFactory<HealthFacilityModel> {
	...
	create(): HealthFacilityModel {
		let newModel = new HealthFacilityModel();
	...
	createAll(size: number = 10): HealthFacilityModel[] {
		let newModels: HealthFacilityModel[] = [];
	...
	/**
	 * Given a blank model of type Health Facility, populate the model with its properties.
	 ...
	 */
	populate(model: HealthFacilityModel) {
		...
		model.healthFacilityID = faker.random.words();
		...
		model.province = faker.random.words();
		...
		model.salutation = faker.random.words();
		model.firstName = faker.random.words();
		model.lastName = faker.random.words();
		model.phoneNumberPOC = faker.random.words();
		model.mobilePhoneNumberPOC = faker.random.words();
		model.emailPOC = faker.random.words();
		model.healthFacilityType = faker.random.words();
