...
export class PcareNonCompoundDrugDataFactory extends AbstractDataFactory<PcareNonCompoundDrugModel> {
	...
	create(): PcareNonCompoundDrugModel {
		let newModel = new PcareNonCompoundDrugModel();
	...
	createAll(size: number = 10): PcareNonCompoundDrugModel[] {
		let newModels: PcareNonCompoundDrugModel[] = [];
	...
	/**
	 * Given a blank model of type PCare NonCompound Drug, populate the model with its properties.
	 ...
	 */
	populate(model: PcareNonCompoundDrugModel) {
		...
		model.drugCode = faker.random.words();
		model.drugName = faker.random.words();
		model.signaDays = faker.random.number();
		model.signaTimes = faker.random.number();
		model.drugQty = faker.random.number();
		model.day = faker.random.number();
		model.requestQty = faker.random.number();
		model.kdObatSK = faker.random.words();
		model.racikan = faker.random.boolean();
		model.kdRacikan = faker.random.words();
