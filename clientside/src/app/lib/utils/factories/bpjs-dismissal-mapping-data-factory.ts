...
export class BpjsDismissalMappingDataFactory extends AbstractDataFactory<BpjsDismissalMappingModel> {
	...
	create(): BpjsDismissalMappingModel {
		let newModel = new BpjsDismissalMappingModel();
	...
	createAll(size: number = 10): BpjsDismissalMappingModel[] {
		let newModels: BpjsDismissalMappingModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Dismissal Mapping, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsDismissalMappingModel) {
		...
		model.internalDismissal = faker.random.words();
