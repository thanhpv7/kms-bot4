...
export class BpjsDismissalConditionDataFactory extends AbstractDataFactory<BpjsDismissalConditionModel> {
	...
	create(): BpjsDismissalConditionModel {
		let newModel = new BpjsDismissalConditionModel();
	...
	createAll(size: number = 10): BpjsDismissalConditionModel[] {
		let newModels: BpjsDismissalConditionModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Dismissal Condition, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsDismissalConditionModel) {
		...
		model.conditionName = faker.random.words();
		model.conditionCode = faker.random.words();
