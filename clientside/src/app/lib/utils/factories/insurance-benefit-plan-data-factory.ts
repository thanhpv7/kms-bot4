...
export class InsuranceBenefitPlanDataFactory extends AbstractDataFactory<InsuranceBenefitPlanModel> {
	...
	create(): InsuranceBenefitPlanModel {
		let newModel = new InsuranceBenefitPlanModel();
	...
	createAll(size: number = 10): InsuranceBenefitPlanModel[] {
		let newModels: InsuranceBenefitPlanModel[] = [];
	...
	/**
	 * Given a blank model of type Insurance Benefit Plan, populate the model with its properties.
	 ...
	 */
	populate(model: InsuranceBenefitPlanModel) {
		...
		model.benefitPlanNo = faker.random.words();
		model.benefitPlanName = faker.random.words();
		model.maximumCoveragePerTreatment = faker.random.number();
		model.maximumCoveragePerYear = faker.random.number();
