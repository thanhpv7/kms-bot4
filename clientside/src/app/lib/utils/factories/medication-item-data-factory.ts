...
import {MedicationItemModel} from '../../../models/medicationItem/medication_item.model';
import * as faker from 'faker';

export class MedicationItemDataFactory extends AbstractDataFactory<MedicationItemModel> {
	...
	create(): MedicationItemModel {
		let newModel = new MedicationItemModel();
	...
	createAll(size: number = 10): MedicationItemModel[] {
		let newModels: MedicationItemModel[] = [];
	...
	/**
	 * Given a blank model of type Medication Item, populate the model with its properties.
	 ...
	 */
	populate(model: MedicationItemModel) {
		...
		model.requestedQty = faker.random.number();
		model.outstandingQty = faker.random.number();
		model.returnedQty = faker.random.number();
		model.issuedQty = faker.random.number();
		model.administeredQty = faker.random.number();
		model.takenHomeQty = faker.random.number();
		model.dosageFrequency = faker.random.number();
		model.dosageQty = faker.random.number();
		model.dose = faker.random.number();
		...
		model.totalIssuedQuantity = faker.random.number();
		model.totalAdministeredQuantity = faker.random.number();
		model.totalTakenHomeQuantity = faker.random.number();
		model.totalReturnedQuantity = faker.random.number();
		model.externalMedicationName = faker.random.words();
		model.approvalStatus = faker.random.words();
		model.inventoryRoundingValue = faker.random.number();
		model.dtdQuantity = faker.random.number();
