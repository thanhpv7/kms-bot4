...
import {InvoiceItemModel} from '../../../models/invoiceItem/invoice_item.model';
import {TariffTypeEnum, tariffTypeEnumArray} from '../../../enums/tariff-type.enum';
import * as faker from 'faker';

export class InvoiceItemDataFactory extends AbstractDataFactory<InvoiceItemModel> {
	...
	create(): InvoiceItemModel {
		let newModel = new InvoiceItemModel();
	...
	createAll(size: number = 10): InvoiceItemModel[] {
		let newModels: InvoiceItemModel[] = [];
	...
	/**
	 * Given a blank model of type Invoice Item, populate the model with its properties.
	 ...
	 */
	populate(model: InvoiceItemModel) {
		...
		model.totalOtherComponentPrice = faker.random.number();
		model.quantity = faker.random.number();
		model.unit = faker.random.words();
		model.discount = faker.random.number();
		model.tax = faker.random.boolean();
		model.itemTotal = faker.random.number();
