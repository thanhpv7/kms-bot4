...
export class PatientGeneralInfoDataFactory extends AbstractDataFactory<PatientGeneralInfoModel> {
	...
	create(): PatientGeneralInfoModel {
		let newModel = new PatientGeneralInfoModel();
	...
	createAll(size: number = 10): PatientGeneralInfoModel[] {
		let newModels: PatientGeneralInfoModel[] = [];
	...
	/**
	 * Given a blank model of type Patient General Info, populate the model with its properties.
	 ...
	 */
	populate(model: PatientGeneralInfoModel) {
		...
		model.importantNotes = faker.random.words();
		model.hasOutstandingPayment = faker.random.boolean();
