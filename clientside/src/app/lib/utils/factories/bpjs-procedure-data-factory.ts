...
export class BpjsProcedureDataFactory extends AbstractDataFactory<BpjsProcedureModel> {
	...
	create(): BpjsProcedureModel {
		let newModel = new BpjsProcedureModel();
	...
	createAll(size: number = 10): BpjsProcedureModel[] {
		let newModels: BpjsProcedureModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Procedure, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsProcedureModel) {
		...
		model.procedureCode = faker.random.words();
		model.procedureName = faker.random.words();
