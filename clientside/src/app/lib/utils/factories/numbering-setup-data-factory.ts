...
export class NumberingSetupDataFactory extends AbstractDataFactory<NumberingSetupModel> {
	...
	create(): NumberingSetupModel {
		let newModel = new NumberingSetupModel();
	...
	createAll(size: number = 10): NumberingSetupModel[] {
		let newModels: NumberingSetupModel[] = [];
	...
	/**
	 * Given a blank model of type Numbering Setup, populate the model with its properties.
	 ...
	 */
	populate(model: NumberingSetupModel) {
		...
		// % protected region % [Add Numbering Typecustom enum randomisation here] off begin
		// Always initialise to the first enum literal. Turn on the protected region and put in your own custom enum
		// initialisation logic.
		model.numberingType = NumberingTypeEnum.PRESCRIPTION_NON_COMPOUND;
		// % protected region % [Add Numbering Typecustom enum randomisation here] end
		model.prefix = faker.random.words();
		// % protected region % [Add Date Formatcustom enum randomisation here] off begin
		// Always initialise to the first enum literal. Turn on the protected region and put in your own custom enum
		// initialisation logic.
		model.dateFormat = DateFormatEnum.YYYYMMDD;
		// % protected region % [Add Date Formatcustom enum randomisation here] end
		model.sufix = faker.random.words();
		model.lastCounter = faker.random.number();
		model.editable = faker.random.boolean();
		model.resetPerDay = faker.random.boolean();
		model.counterLength = faker.random.number();
		model.lastUpdated = new Date();
		model.preview = faker.random.words();
