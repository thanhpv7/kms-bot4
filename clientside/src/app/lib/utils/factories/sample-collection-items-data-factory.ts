...
export class SampleCollectionItemsDataFactory extends AbstractDataFactory<SampleCollectionItemsModel> {
	...
	create(): SampleCollectionItemsModel {
		let newModel = new SampleCollectionItemsModel();
	...
	createAll(size: number = 10): SampleCollectionItemsModel[] {
		let newModels: SampleCollectionItemsModel[] = [];
	...
	/**
	 * Given a blank model of type Sample Collection Items, populate the model with its properties.
	 ...
	 */
	populate(model: SampleCollectionItemsModel) {
		...
		model.sampleNumber = faker.random.words();
		model.sampleTakingLocation = faker.random.words();
		model.volume = faker.random.words();
		model.container = faker.random.words();
		model.transport = faker.random.words();
		model.sampleCollectionDateTime = new Date();
		model.sampleProcessingDateTime = new Date();
		model.sampleDeliveryDateTime = new Date();
