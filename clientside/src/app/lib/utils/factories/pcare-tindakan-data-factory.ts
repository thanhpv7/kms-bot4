...
import {PcareTindakanModel} from '../../../models/pcareTindakan/pcare_tindakan.model';
import * as faker from 'faker';

export class PcareTindakanDataFactory extends AbstractDataFactory<PcareTindakanModel> {
	...
	create(): PcareTindakanModel {
		let newModel = new PcareTindakanModel();
	...
	createAll(size: number = 10): PcareTindakanModel[] {
		let newModels: PcareTindakanModel[] = [];
	...
	/**
	 * Given a blank model of type PCare Tindakan, populate the model with its properties.
	 ...
	 */
	populate(model: PcareTindakanModel) {
		...
		model.kodeTindakan = faker.random.words();
		model.namaTindakan = faker.random.words();
		model.maksimalTarif = faker.random.words();
		model.withvalue = faker.random.boolean();
		model.tindakanInternal = faker.random.words();
