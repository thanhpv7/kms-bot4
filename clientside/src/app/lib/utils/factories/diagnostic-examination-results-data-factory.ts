...
export class DiagnosticExaminationResultsDataFactory extends AbstractDataFactory<DiagnosticExaminationResultsModel> {
	...
	create(): DiagnosticExaminationResultsModel {
		let newModel = new DiagnosticExaminationResultsModel();
	...
	createAll(size: number = 10): DiagnosticExaminationResultsModel[] {
		let newModels: DiagnosticExaminationResultsModel[] = [];
	...
	/**
	 * Given a blank model of type Diagnostic Examination Results, populate the model with its properties.
	 ...
	 */
	populate(model: DiagnosticExaminationResultsModel) {
		...
		model.result = faker.random.words();
		model.outsideRange = faker.random.boolean();
		model.low = faker.random.number();
		model.high = faker.random.number();
		model.unit = faker.random.words();
		model.sampleNo = faker.random.words();
		model.sampleType = faker.random.words();
		model.repeated = faker.random.boolean();
