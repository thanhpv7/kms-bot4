...
export class TariffPackageDataFactory extends AbstractDataFactory<TariffPackageModel> {
	...
	create(): TariffPackageModel {
		let newModel = new TariffPackageModel();
	...
	createAll(size: number = 10): TariffPackageModel[] {
		let newModels: TariffPackageModel[] = [];
	...
	/**
	 * Given a blank model of type Tariff Package, populate the model with its properties.
	 ...
	 */
	populate(model: TariffPackageModel) {
		...
		model.tariffPackageCode = faker.random.words();
		model.tariffPackageName = faker.random.words();
