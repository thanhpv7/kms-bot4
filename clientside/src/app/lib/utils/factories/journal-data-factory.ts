...
export class JournalDataFactory extends AbstractDataFactory<JournalModel> {
	...
	createAll(size: number = 10): JournalModel[] {
		let newModels: JournalModel[] = [];
	...
	/**
	 * Given a blank model of type Journal, populate the model with its properties.
	...
	populate(model: JournalModel) {
		...
		model.journalNumber = faker.random.words();
		model.transactionDate = new Date();
		model.referenceNumber = faker.random.words();
		model.transactionType = faker.random.words();
		model.transactionDetailType = faker.random.words();
		model.accountNumber = faker.random.words();
		model.debit = faker.random.number();
		model.credit = faker.random.number();
		model.postingStatus = faker.random.words();
		model.postingDateTime = new Date();
		model.supplierNumber = faker.random.words();
		model.supplierName = faker.random.words();
		model.customerNumber = faker.random.words();
		model.customerName = faker.random.words();
