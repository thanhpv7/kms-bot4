...
export class SepDataFactory extends AbstractDataFactory<SepModel> {
	...
	createAll(size: number = 10): SepModel[] {
		let newModels: SepModel[] = [];
	...
	/**
	 * Given a blank model of type SEP, populate the model with its properties.
	...
	populate(model: SepModel) {
		...
		model.sepData = faker.random.words();
