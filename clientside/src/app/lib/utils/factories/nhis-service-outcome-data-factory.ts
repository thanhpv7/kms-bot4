...
import {NhisServiceOutcomeModel} from '../../../models/nhisServiceOutcome/nhis_service_outcome.model';
import * as faker from 'faker';

export class NhisServiceOutcomeDataFactory extends AbstractDataFactory<NhisServiceOutcomeModel> {
	...
	create(): NhisServiceOutcomeModel {
		let newModel = new NhisServiceOutcomeModel();
	...
	createAll(size: number = 10): NhisServiceOutcomeModel[] {
		let newModels: NhisServiceOutcomeModel[] = [];
	...
	/**
	 * Given a blank model of type NHIS Service Outcome, populate the model with its properties.
	 ...
	 */
	populate(model: NhisServiceOutcomeModel) {
		...
		model.dismissalTypes = faker.random.words();
