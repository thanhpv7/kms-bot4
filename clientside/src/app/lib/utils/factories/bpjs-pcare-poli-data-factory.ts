...
export class BpjsPcarePoliDataFactory extends AbstractDataFactory<BpjsPcarePoliModel> {
	...
	create(): BpjsPcarePoliModel {
		let newModel = new BpjsPcarePoliModel();
	...
	createAll(size: number = 10): BpjsPcarePoliModel[] {
		let newModels: BpjsPcarePoliModel[] = [];
	...
	/**
	 * Given a blank model of type BPJS Pcare Poli, populate the model with its properties.
	 ...
	 */
	populate(model: BpjsPcarePoliModel) {
		...
		model.polisakit = faker.random.boolean();
