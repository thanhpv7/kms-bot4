...
export class PatientDetailDataFactory extends AbstractDataFactory<PatientDetailModel> {
	...
	create(): PatientDetailModel {
		let newModel = new PatientDetailModel();
	...
	createAll(size: number = 10): PatientDetailModel[] {
		let newModels: PatientDetailModel[] = [];
	...
	/**
	 * Given a blank model of type Patient Detail, populate the model with its properties.
	 ...
	 */
	populate(model: PatientDetailModel) {
		...
		model.activePatientID = faker.random.words();
		model.patientClassification = faker.random.words();
		model.patientType = faker.random.words();
