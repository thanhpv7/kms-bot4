...
export class PatientVisitPaymentSelfPayingDataFactory extends AbstractDataFactory<PatientVisitPaymentSelfPayingModel> {
	...
	create(): PatientVisitPaymentSelfPayingModel {
		let newModel = new PatientVisitPaymentSelfPayingModel();
	...
	createAll(size: number = 10): PatientVisitPaymentSelfPayingModel[] {
		let newModels: PatientVisitPaymentSelfPayingModel[] = [];
	...
	/**
	 * Given a blank model of type Patient Visit Payment Self Paying, populate the model with its properties.
	 ...
	 */
	populate(model: PatientVisitPaymentSelfPayingModel) {
		...
		model.sponsorName = faker.random.words();
		model.sponsorAddress = faker.random.words();
		model.sponsorPhoneNo = faker.random.words();
		model.sponsorship = faker.random.words();
