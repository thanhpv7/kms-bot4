...
describe('Textfield component against tests', () => {
	...
	beforeEach(waitForAsync (() => {
		TestBed.configureTestingModule({
			...
			// placeholder id and label until they are updated in tests
			// Causes errors in multiple tests if id or label is null
			textFieldComponent.id = 'placeholder-id';
			textFieldComponent.label = 'placeholder';
		});
	}));
	...
	it('should not display an error message when there is no validation error', () => {
		fixture.detectChanges();
		const textFieldElement = fixture.debugElement.query(By.css('div.input-group__error-text'));

		expect(textFieldElement).toBeNull();
	});
	...
	it('should display an error message when there is a validation error', () => {
		...
		// Create a validation error and add it to the element.
		textFieldComponent.validationErrors = validationError;
		textFieldComponent.dirty = true;
		fixture.detectChanges();

		const textFieldElement = fixture.debugElement.query(By.css('div.input-group__error-text'));
		const errorMessage = textFieldElement.query(By.css('div.input-group__error-text > p')).nativeElement;
