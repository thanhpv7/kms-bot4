...
describe('Login Component', () => {
	...
	const clickLoginButton = (): void => {
		const loginDebugEl = loginComponentFixture.debugElement;
		const loginBtnDebugEl = loginDebugEl.queryAll(By.css('button'))
				.find((button) => button.nativeElement.textContent.trim() === 'Login');
	...
	it('should have a valid button group', () => {
		...
		// Don't check for registration button here.  Will check that it behaves as intended in another test
	...
	it ('should hide the registration button when no user types can be registered', () => {
		// % protected region % [Add any additional logic for "should hide the registration button when no user types can be registered" before the main body here] off begin
		// % protected region % [Add any additional logic for "should hide the registration button when no user types can be registered" before the main body here] end

		loginComponent.userTypes = [];
		loginComponentFixture.detectChanges();

		const registerBtn = loginComponentFixture.debugElement.query(By.css('a.btn.btn--md.btn--secondary'));
		expect(registerBtn).toBeNull();

		// % protected region % [Add any additional logic for "should hide the registration button when no user types can be registered" after the main body here] off begin
		// % protected region % [Add any additional logic for "should hide the registration button when no user types can be registered" after the main body here] end

	});

	it ('should skip register select page when only one user type can be registered', () => {
		// % protected region % [Add any additional logic for "should skip register select page when only one user type can be registered" before the main body here] off begin
		// % protected region % [Add any additional logic for "should skip register select page when only one user type can be registered" before the main body here] end
		...
		const registerBtn = loginEl.querySelector<HTMLButtonElement>('a.btn.btn--md.btn--secondary');
		expect(registerBtn.textContent).toEqual('Register', 'Register button not found');
		expect(registerBtn.getAttribute('href')).toEqual('/register/' + loginComponent.userTypes[0].value);

		// % protected region % [Add any additional logic for "should skip register select page when only one user type can be registered" after the main body here] off begin
		// % protected region % [Add any additional logic for "should skip register select page when only one user type can be registered" after the main body here] end

	});

	it ('should show the register select page when more than one user type can be registered', () => {
		// % protected region % [Add any additional logic for "should show the register select page when more than one user type can be registered" before the main body here] off begin
		// % protected region % [Add any additional logic for "should show the register select page when more than one user type can be registered" before the main body here] end
		...
		const registerBtn = loginEl.querySelector<HTMLButtonElement>('a.btn.btn--md.btn--secondary');
		expect(registerBtn.textContent).toEqual('Register', 'Register button not found');
		expect(registerBtn.getAttribute('href')).toEqual('/register');

		// % protected region % [Add any additional logic for "should show the register select page when more than one user type can be registered" after the main body here] off begin
		// % protected region % [Add any additional logic for "should show the register select page when more than one user type can be registered" after the main body here] end
	});
	...
	it('should have a valid forgot password link', () => {
		...
		const loginEl: HTMLElement = loginComponentFixture.nativeElement;
		const formEl: HTMLElement = loginEl.querySelector('form.login');
		const forgotPasswordEl = formEl.querySelector('a.login__forgot-password') as HTMLAnchorElement;
	...
	it('should validate the email correctly if wrong email', () => {
		...
		// % protected region % [Add any additional logic for "should validate the email correctly if wrong email" after the main body here] off begin
		// % protected region % [Add any additional logic for "should validate the email correctly if wrong email" after the main body here] end
	});
	...
	it('should validate the password correctly if empty', () => {
		...
		// % protected region % [Add any additional logic for "should validate the password correctly if empty" after the main body here] off begin
		// % protected region % [Add any additional logic for "should validate the password correctly if empty" after the main body here] end
	});
