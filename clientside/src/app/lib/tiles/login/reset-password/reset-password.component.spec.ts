...
describe('Reset Password Component', () => {
	...
	it('should have buttons with correct class', () => {
		...
		// Check buttons
		const buttonElements: DebugElement[] = resetPasswordComponentFixture.debugElement.queryAll(By.css('button'));

		// Need to check for the update password button, and for the buttons used to toggle showing the password
		let showPasswordButtonCount = 0;
		let updateButtonCount = 0;

		resetPasswordComponentFixture.detectChanges();

		expect(buttonElements.length).toBe(3, 'Should have 3 buttons in button group');

		const updateButton = buttonElements[0];

		const expectedUpdateButtonClass = ['btn', 'btn--md'];

		let actualUpdateButtonClass;
		buttonElements.forEach(button => {
			actualUpdateButtonClass = updateButton.nativeElement.className.split(' ');
			expect(expectedUpdateButtonClass.every(buttonClass => actualUpdateButtonClass.includes(buttonClass)))
					.toBeTruthy('Reset button classes do not match the class names in the standard');

			// We know we should have two buttons for toggling show password and one for updating password
			// We count all buttons here and then make assertions once the loop is complete, so that we are not assuming an order
			if (button.nativeElement.textContent === '') {
				showPasswordButtonCount++;
			} else if (button.nativeElement.textContent === 'Update Password') {
				updateButtonCount++;
			} else {
				// Jasmine doesn't seem to have a mechanism for purposely failing a test so this seemed like a decent alternative
				// As we know what values we should see for the text in our buttons, if the test gets here then there is an invalid value
				expect(false).toBeTruthy(button.nativeElement.textContent + 'is not a valid text value for buttons in this component');
			}

		});

		expect(showPasswordButtonCount).toBe(2, 'Should have 2 buttons for toggling showing the password');
		expect(updateButtonCount).toBe(1, 'Should have one update password button');
	...
	it('should call reset password function when click button with valid form', () => {
		...
		expect(resetPasswordButton.nativeElement.disabled).toBeFalsy();
	...
	it('should disable reset password button when form inputs are invalid', () => {
		// % protected region % [Add any additional before logic for 'should not call reset password function when click button with valid form' here] off begin
		// % protected region % [Add any additional before logic for 'should not call reset password function when click button with valid form' here] end
		...
		expect(updatePasswordButton.nativeElement.disabled).toBeTruthy();

		// % protected region % [Add any additional after logic for 'should not call reset password function when click button with valid form' here] off begin
		// % protected region % [Add any additional after logic for 'should not call reset password function when click button with valid form' here] end
	});
