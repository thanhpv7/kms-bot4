	...
	constructor(
		...
		let routerSubscription = this.routerState$.subscribe(routerState => {
			...
			if (!this.token || !this.username) {
				// % protected region % [Override the default post-password-reset redirect here] off begin
				// Unsubscribe to avoid infinite loop
				this.routerStore.dispatch(new NavigateRoutingAction(['/login']));
				routerSubscription.unsubscribe();
				// % protected region % [Override the default post-password-reset redirect here] end
			}
	...
	onResetPasswordClicked() {
		...
		this.authenticationService.resetPassword(this.username, this.token, password).subscribe(
			(response) => {
				...
				// % protected region % [Override toaster and redirect details here on password reset clicked] off begin
				...
				// % protected region % [Override toaster and redirect details here on password reset clicked] end
