...
// If using an instance of the group interface for grouping attributes within a single entity
// (i.e. not using multi entity crud mode) then only id and display name are required
export interface Group {
	...
	// % protected region % [Add any additional attributes to the group interface here] off begin
	// % protected region % [Add any additional attributes to the group interface here] end
}
...
export interface ModelRelation {
	// id of the relation, by default will usually be the name of the related entity or it's plural.  Used for graphql queries
	id: string;
	// Name of the Relation to bind with, used for associating the attribute in the entity with the attribute in the form group
	name: string;
	...
	displayName: string;
	// The name of the entity associated with this relation.  Used for fetching the data structure from fetchPrepareReferenceRequirements, which greatly simplifies the setup logic for crud edit relations
	entityName: string;
...
export abstract class AbstractModel {
	...
	static getProps(): ModelProperty[] {
		let idDisplayName = 'ID';
		let createdDisplayName = 'Created Date';
		let modifiedDisplayName = 'Modified Date';
		// % protected region % [Add any additional logic to getProps before returning] off begin
		// % protected region % [Add any additional logic to getProps before returning] end
		return [
			{
				name: 'id',
	...
	toJSON() {
		return {
			...
			// % protected region % [Add additional toJSON attributes here] off begin
