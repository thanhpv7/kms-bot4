...
describe('Authentication Interceptor', () => {
	...
	it('should insert XSRF token as header correctly', () => {
		...
		expect(httpRequest.request.headers.has('X-XSRF-TOKEN')).toBeTruthy('XSRF header not found');
		expect(httpRequest.request.headers.get('X-XSRF-TOKEN')).toEqual(XSRF_TOKEN, 'XSRF header mismatch');
