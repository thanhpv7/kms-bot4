		...
		return next.handle(req).pipe(
			catchError((error: HttpErrorResponse) => {
				...
				if (error.error instanceof ErrorEvent) {
					...
					// If on login page, pass along some connection errors as other errors are handled by the login page itself
					if (this.routerState.urls[0] === 'login' && error.status > 0) {
					...
					switch (error.status) {
						case 400:
							...
							if (error.url.includes('/import-csv')) {
								this.toastrService.error(error.error, 'An error occurred while importing the CSV File',
						...
						case 401:
							...
							let errorMessage: ErrorMessage = {
								...
								body: 'Unable to perform operation'
							};
							...
							if (this.routerState.urls && this.routerState.urls[0].toLowerCase() !== 'login') {
						...
						case 0:
							// Handle the scenario where the connection is cancelled, this can occur when the server is unreachable.
							// % protected region % [Add any additional error handling logic for 0 here] off begin
							this.toastrService.error('Connection failure!', 'Could not complete request.');
							break;
							// % protected region % [Add any additional error handling logic for 0 here] end
						default:
