...
import {initialAmendmentDetailCollectionState} from './amendment_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseAmendmentDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.AmendmentDetailModelActionTypes.CREATE_AMENDMENT_DETAIL_OK:
				const createdModel = (action as modelAction.AmendmentDetailActionOK ).stateConfig.targetModel;
				...
				AmendmentDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.CREATE_AMENDMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AmendmentDetailModelActionTypes.CREATE_ALL_AMENDMENT_DETAIL_OK:
				const createdModels = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					AmendmentDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.CREATE_ALL_AMENDMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AmendmentDetailModelActionTypes.DELETE_AMENDMENT_DETAIL_OK:
				const deletedModelId = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.DELETE_ALL_AMENDMENT_DETAIL_OK:
				const deletedModelIds = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.DELETE_AMENDMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AmendmentDetailModelActionTypes.UPDATE_AMENDMENT_DETAIL_OK:
				const updatedModel = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModel;
				...
				AmendmentDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.UPDATE_AMENDMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.AmendmentDetailModelActionTypes.UPDATE_ALL_AMENDMENT_DETAIL_OK:
				const updatedModels = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.UPDATE_ALL_AMENDMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AmendmentDetailModelActionTypes.FETCH_AMENDMENT_DETAIL_OK:
				let fetchedModel = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModel;
				...
				AmendmentDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.FETCH_ALL_AMENDMENT_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.AmendmentDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => AmendmentDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.FETCH_AMENDMENT_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.AmendmentDetailActionOK).audits;
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.FETCH_AMENDMENT_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.AmendmentDetailActionOK).audits;
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.FETCH_AMENDMENT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => AmendmentDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.FETCH_LAST_AMENDMENT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.AmendmentDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => AmendmentDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.COUNT_AMENDMENT_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.AmendmentDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.AmendmentDetailModelActionTypes.INITIALISE_AMENDMENT_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.AmendmentDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.AmendmentDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialAmendmentDetailCollectionState
					};
				}
