...
import {initialWarehouseInitialStockCollectionState} from './warehouse_initial_stock.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseWarehouseInitialStockAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.WarehouseInitialStockModelActionTypes.CREATE_WAREHOUSE_INITIAL_STOCK_OK:
				const createdModel = (action as modelAction.WarehouseInitialStockActionOK ).stateConfig.targetModel;
				...
				WarehouseInitialStockModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.CREATE_WAREHOUSE_INITIAL_STOCK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.CREATE_ALL_WAREHOUSE_INITIAL_STOCK_OK:
				const createdModels = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					WarehouseInitialStockModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.CREATE_ALL_WAREHOUSE_INITIAL_STOCK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.DELETE_WAREHOUSE_INITIAL_STOCK_OK:
				const deletedModelId = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.DELETE_ALL_WAREHOUSE_INITIAL_STOCK_OK:
				const deletedModelIds = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.DELETE_WAREHOUSE_INITIAL_STOCK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.UPDATE_WAREHOUSE_INITIAL_STOCK_OK:
				const updatedModel = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModel;
				...
				WarehouseInitialStockModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.UPDATE_WAREHOUSE_INITIAL_STOCK_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.WarehouseInitialStockModelActionTypes.UPDATE_ALL_WAREHOUSE_INITIAL_STOCK_OK:
				const updatedModels = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.UPDATE_ALL_WAREHOUSE_INITIAL_STOCK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.FETCH_WAREHOUSE_INITIAL_STOCK_OK:
				let fetchedModel = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModel;
				...
				WarehouseInitialStockModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.FETCH_ALL_WAREHOUSE_INITIAL_STOCK_OK:
				const totalCountAfterFetchAll = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => WarehouseInitialStockModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.FETCH_WAREHOUSE_INITIAL_STOCK_AUDIT_OK:
				const audits = (action as modelAction.WarehouseInitialStockActionOK).audits;
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.FETCH_WAREHOUSE_INITIAL_STOCK_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.WarehouseInitialStockActionOK).audits;
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.FETCH_WAREHOUSE_INITIAL_STOCK_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => WarehouseInitialStockModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.FETCH_LAST_WAREHOUSE_INITIAL_STOCK_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => WarehouseInitialStockModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.COUNT_WAREHOUSE_INITIAL_STOCKS_OK:
				const totalCountAfterCount = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.WarehouseInitialStockModelActionTypes.INITIALISE_WAREHOUSE_INITIAL_STOCK_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.WarehouseInitialStockActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.WarehouseInitialStockActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialWarehouseInitialStockCollectionState
					};
				}
