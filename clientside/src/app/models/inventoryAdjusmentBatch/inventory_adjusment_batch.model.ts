 ...
 */
export class InventoryAdjusmentBatchModel extends AbstractModel {
	/**
	...
	readonly className = 'InventoryAdjusmentBatchModel';
	...
	get valueToDisplay(): string {
		...
		return InventoryAdjusmentBatchModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	inventoryAdjustmentItem: InventoryAdjustmentItemModel;
	...
	modelPropGroups: { [s: string]: Group } = InventoryAdjusmentBatchModel.modelPropGroups;
	...
	static getRelations(): ModelRelation[] {
		return super.getRelations().concat([
			{
				id: 'inventoryAdjustmentItem',
				type: ModelRelationType.ONE,
				name: 'inventoryAdjustmentItemId',
				// % protected region % [Customise your label for Inventory Adjustment Item here] off begin
				label: 'Inventory Adjustment Item',
				// % protected region % [Customise your label for Inventory Adjustment Item here] end
				entityName: 'InventoryAdjustmentItemModel',
				// % protected region % [Customise your display name for Inventory Adjustment Item here] off begin
				// TODO change implementation to use OrderBy or create new metamodel property DisplayBy
				displayName: 'itemLineNumber',
				// % protected region % [Customise your display name for Inventory Adjustment Item here] end
				validators: [
					// % protected region % [Add other validators for Inventory Adjustment Item here] off begin
					// % protected region % [Add other validators for Inventory Adjustment Item here] end
				],
				// % protected region % [Add any additional field for relation Inventory Adjustment Item here] off begin
				// % protected region % [Add any additional field for relation Inventory Adjustment Item here] end
			},
			{
				id: 'stockCatalogue',
				type: ModelRelationType.ONE,
	 ...
	 */
	static deepParse(data: string | { [K in keyof InventoryAdjusmentBatchModel]?: InventoryAdjusmentBatchModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new InventoryAdjusmentBatchModel(data);
		}
		...
		if (json.inventoryAdjustmentItem) {
			currentModel.inventoryAdjustmentItemId = json.inventoryAdjustmentItem.id;
			returned = _.union(returned, InventoryAdjustmentItemModel.deepParse(json.inventoryAdjustmentItem));
		}
	 ...
	 * @example
	 *
	 * `let inventoryAdjusmentBatchModel = new InventoryAdjusmentBatchModel(data);`
	 *
	 * @param data The input data to be initialised as the InventoryAdjusmentBatchModel,
	 *    it is expected as a JSON string or as a nullable InventoryAdjusmentBatchModel.
	 */
	constructor(data?: string | Partial<InventoryAdjusmentBatchModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<InventoryAdjusmentBatchModel>
				: data;
			...
			this.inventoryAdjustmentItemId = json.inventoryAdjustmentItemId;
			this.inventoryAdjustmentItem = json.inventoryAdjustmentItem;
	...
	toJSON() {
		return {
			...
			inventoryAdjustmentItemId: this.inventoryAdjustmentItemId,
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		InventoryAdjusmentBatchModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): InventoryAdjusmentBatchModel {
		...
		if (updates.inventoryAdjustmentItemId) {
			newModelJson.inventoryAdjustmentItemId = updates.inventoryAdjustmentItemId;
		}
		...
		return new InventoryAdjusmentBatchModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof InventoryAdjusmentBatchModel)) {
			return {};
		}
	...
	syncRelations() {
		if (!this.inventoryAdjustmentItem) {
			this.inventoryAdjustmentItemId = null;
		} else {
			this.inventoryAdjustmentItemId = this.inventoryAdjustmentItem.id;
		}
