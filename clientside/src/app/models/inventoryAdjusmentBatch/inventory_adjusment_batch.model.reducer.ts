...
import {initialInventoryAdjusmentBatchCollectionState} from './inventory_adjusment_batch.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInventoryAdjusmentBatchAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InventoryAdjusmentBatchModelActionTypes.CREATE_INVENTORY_ADJUSMENT_BATCH_OK:
				const createdModel = (action as modelAction.InventoryAdjusmentBatchActionOK ).stateConfig.targetModel;
				...
				InventoryAdjusmentBatchModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.CREATE_INVENTORY_ADJUSMENT_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.CREATE_ALL_INVENTORY_ADJUSMENT_BATCH_OK:
				const createdModels = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InventoryAdjusmentBatchModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.CREATE_ALL_INVENTORY_ADJUSMENT_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.DELETE_INVENTORY_ADJUSMENT_BATCH_OK:
				const deletedModelId = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.DELETE_ALL_INVENTORY_ADJUSMENT_BATCH_OK:
				const deletedModelIds = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.DELETE_INVENTORY_ADJUSMENT_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.UPDATE_INVENTORY_ADJUSMENT_BATCH_OK:
				const updatedModel = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModel;
				...
				InventoryAdjusmentBatchModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.UPDATE_INVENTORY_ADJUSMENT_BATCH_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InventoryAdjusmentBatchModelActionTypes.UPDATE_ALL_INVENTORY_ADJUSMENT_BATCH_OK:
				const updatedModels = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.UPDATE_ALL_INVENTORY_ADJUSMENT_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_INVENTORY_ADJUSMENT_BATCH_OK:
				let fetchedModel = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModel;
				...
				InventoryAdjusmentBatchModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_ALL_INVENTORY_ADJUSMENT_BATCH_OK:
				const totalCountAfterFetchAll = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InventoryAdjusmentBatchModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_INVENTORY_ADJUSMENT_BATCH_AUDIT_OK:
				const audits = (action as modelAction.InventoryAdjusmentBatchActionOK).audits;
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_INVENTORY_ADJUSMENT_BATCH_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InventoryAdjusmentBatchActionOK).audits;
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_INVENTORY_ADJUSMENT_BATCH_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InventoryAdjusmentBatchModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.FETCH_LAST_INVENTORY_ADJUSMENT_BATCH_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InventoryAdjusmentBatchModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.COUNT_INVENTORY_ADJUSMENT_BATCHS_OK:
				const totalCountAfterCount = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InventoryAdjusmentBatchModelActionTypes.INITIALISE_INVENTORY_ADJUSMENT_BATCH_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InventoryAdjusmentBatchActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInventoryAdjusmentBatchCollectionState
					};
				}
