...
import {initialPrescriptionItemCollectionState} from './prescription_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePrescriptionItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PrescriptionItemModelActionTypes.CREATE_PRESCRIPTION_ITEM_OK:
				const createdModel = (action as modelAction.PrescriptionItemActionOK ).stateConfig.targetModel;
				...
				PrescriptionItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.CREATE_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionItemModelActionTypes.CREATE_ALL_PRESCRIPTION_ITEM_OK:
				const createdModels = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PrescriptionItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.CREATE_ALL_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionItemModelActionTypes.DELETE_PRESCRIPTION_ITEM_OK:
				const deletedModelId = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.DELETE_ALL_PRESCRIPTION_ITEM_OK:
				const deletedModelIds = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.DELETE_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionItemModelActionTypes.UPDATE_PRESCRIPTION_ITEM_OK:
				const updatedModel = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModel;
				...
				PrescriptionItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.UPDATE_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PrescriptionItemModelActionTypes.UPDATE_ALL_PRESCRIPTION_ITEM_OK:
				const updatedModels = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.UPDATE_ALL_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionItemModelActionTypes.FETCH_PRESCRIPTION_ITEM_OK:
				let fetchedModel = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModel;
				...
				PrescriptionItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.FETCH_ALL_PRESCRIPTION_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PrescriptionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PrescriptionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.FETCH_PRESCRIPTION_ITEM_AUDIT_OK:
				const audits = (action as modelAction.PrescriptionItemActionOK).audits;
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.FETCH_PRESCRIPTION_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PrescriptionItemActionOK).audits;
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.FETCH_PRESCRIPTION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PrescriptionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.FETCH_LAST_PRESCRIPTION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PrescriptionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PrescriptionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.COUNT_PRESCRIPTION_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.PrescriptionItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PrescriptionItemModelActionTypes.INITIALISE_PRESCRIPTION_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PrescriptionItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPrescriptionItemCollectionState
					};
				}
