...
import {initialSampleCollectionInformationCollectionState} from './sample_collection_information.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSampleCollectionInformationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SampleCollectionInformationModelActionTypes.CREATE_SAMPLE_COLLECTION_INFORMATION_OK:
				const createdModel = (action as modelAction.SampleCollectionInformationActionOK ).stateConfig.targetModel;
				...
				SampleCollectionInformationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.CREATE_SAMPLE_COLLECTION_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.CREATE_ALL_SAMPLE_COLLECTION_INFORMATION_OK:
				const createdModels = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SampleCollectionInformationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.CREATE_ALL_SAMPLE_COLLECTION_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.DELETE_SAMPLE_COLLECTION_INFORMATION_OK:
				const deletedModelId = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.DELETE_ALL_SAMPLE_COLLECTION_INFORMATION_OK:
				const deletedModelIds = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.DELETE_SAMPLE_COLLECTION_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.UPDATE_SAMPLE_COLLECTION_INFORMATION_OK:
				const updatedModel = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModel;
				...
				SampleCollectionInformationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.UPDATE_SAMPLE_COLLECTION_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SampleCollectionInformationModelActionTypes.UPDATE_ALL_SAMPLE_COLLECTION_INFORMATION_OK:
				const updatedModels = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.UPDATE_ALL_SAMPLE_COLLECTION_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.FETCH_SAMPLE_COLLECTION_INFORMATION_OK:
				let fetchedModel = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModel;
				...
				SampleCollectionInformationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.FETCH_ALL_SAMPLE_COLLECTION_INFORMATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SampleCollectionInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.FETCH_SAMPLE_COLLECTION_INFORMATION_AUDIT_OK:
				const audits = (action as modelAction.SampleCollectionInformationActionOK).audits;
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.FETCH_SAMPLE_COLLECTION_INFORMATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SampleCollectionInformationActionOK).audits;
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.FETCH_SAMPLE_COLLECTION_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SampleCollectionInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.FETCH_LAST_SAMPLE_COLLECTION_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SampleCollectionInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.COUNT_SAMPLE_COLLECTION_INFORMATIONS_OK:
				const totalCountAfterCount = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SampleCollectionInformationModelActionTypes.INITIALISE_SAMPLE_COLLECTION_INFORMATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SampleCollectionInformationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SampleCollectionInformationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialSampleCollectionInformationCollectionState
					};
				}
