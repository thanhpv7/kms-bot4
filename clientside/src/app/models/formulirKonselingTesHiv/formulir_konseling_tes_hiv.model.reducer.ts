...
import {initialFormulirKonselingTesHivCollectionState} from './formulir_konseling_tes_hiv.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseFormulirKonselingTesHivAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.FormulirKonselingTesHivModelActionTypes.CREATE_FORMULIR_KONSELING_TES_HIV_OK:
				const createdModel = (action as modelAction.FormulirKonselingTesHivActionOK ).stateConfig.targetModel;
				...
				FormulirKonselingTesHivModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.CREATE_FORMULIR_KONSELING_TES_HIV_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.CREATE_ALL_FORMULIR_KONSELING_TES_HIV_OK:
				const createdModels = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					FormulirKonselingTesHivModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.CREATE_ALL_FORMULIR_KONSELING_TES_HIV_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.DELETE_FORMULIR_KONSELING_TES_HIV_OK:
				const deletedModelId = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.DELETE_ALL_FORMULIR_KONSELING_TES_HIV_OK:
				const deletedModelIds = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.DELETE_FORMULIR_KONSELING_TES_HIV_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.UPDATE_FORMULIR_KONSELING_TES_HIV_OK:
				const updatedModel = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModel;
				...
				FormulirKonselingTesHivModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.UPDATE_FORMULIR_KONSELING_TES_HIV_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.FormulirKonselingTesHivModelActionTypes.UPDATE_ALL_FORMULIR_KONSELING_TES_HIV_OK:
				const updatedModels = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.UPDATE_ALL_FORMULIR_KONSELING_TES_HIV_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.FETCH_FORMULIR_KONSELING_TES_HIV_OK:
				let fetchedModel = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModel;
				...
				FormulirKonselingTesHivModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.FETCH_ALL_FORMULIR_KONSELING_TES_HIV_OK:
				const totalCountAfterFetchAll = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => FormulirKonselingTesHivModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.FETCH_FORMULIR_KONSELING_TES_HIV_AUDIT_OK:
				const audits = (action as modelAction.FormulirKonselingTesHivActionOK).audits;
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.FETCH_FORMULIR_KONSELING_TES_HIV_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.FormulirKonselingTesHivActionOK).audits;
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.FETCH_FORMULIR_KONSELING_TES_HIV_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => FormulirKonselingTesHivModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.FETCH_LAST_FORMULIR_KONSELING_TES_HIV_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => FormulirKonselingTesHivModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.COUNT_FORMULIR_KONSELING_TES_HIVS_OK:
				const totalCountAfterCount = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.FormulirKonselingTesHivModelActionTypes.INITIALISE_FORMULIR_KONSELING_TES_HIV_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.FormulirKonselingTesHivActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialFormulirKonselingTesHivCollectionState
					};
				}
