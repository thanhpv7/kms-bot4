...
import {initialIntraoperativeRecordsCollectionState} from './intraoperative_records.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseIntraoperativeRecordsAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.IntraoperativeRecordsModelActionTypes.CREATE_INTRAOPERATIVE_RECORDS_OK:
				const createdModel = (action as modelAction.IntraoperativeRecordsActionOK ).stateConfig.targetModel;
				...
				IntraoperativeRecordsModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.CREATE_INTRAOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.CREATE_ALL_INTRAOPERATIVE_RECORDS_OK:
				const createdModels = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					IntraoperativeRecordsModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.CREATE_ALL_INTRAOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.DELETE_INTRAOPERATIVE_RECORDS_OK:
				const deletedModelId = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.DELETE_ALL_INTRAOPERATIVE_RECORDS_OK:
				const deletedModelIds = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.DELETE_INTRAOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.UPDATE_INTRAOPERATIVE_RECORDS_OK:
				const updatedModel = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModel;
				...
				IntraoperativeRecordsModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.UPDATE_INTRAOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.IntraoperativeRecordsModelActionTypes.UPDATE_ALL_INTRAOPERATIVE_RECORDS_OK:
				const updatedModels = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.UPDATE_ALL_INTRAOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.FETCH_INTRAOPERATIVE_RECORDS_OK:
				let fetchedModel = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModel;
				...
				IntraoperativeRecordsModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.FETCH_ALL_INTRAOPERATIVE_RECORDS_OK:
				const totalCountAfterFetchAll = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => IntraoperativeRecordsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.FETCH_INTRAOPERATIVE_RECORDS_AUDIT_OK:
				const audits = (action as modelAction.IntraoperativeRecordsActionOK).audits;
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.FETCH_INTRAOPERATIVE_RECORDS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.IntraoperativeRecordsActionOK).audits;
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.FETCH_INTRAOPERATIVE_RECORDS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => IntraoperativeRecordsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.FETCH_LAST_INTRAOPERATIVE_RECORDS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => IntraoperativeRecordsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.COUNT_INTRAOPERATIVE_RECORDSS_OK:
				const totalCountAfterCount = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.IntraoperativeRecordsModelActionTypes.INITIALISE_INTRAOPERATIVE_RECORDS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.IntraoperativeRecordsActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialIntraoperativeRecordsCollectionState
					};
				}
