 ...
 */
export class MedicalRecordNextVerificationChecklistModel extends AbstractModel {
	/**
	...
	readonly className = 'MedicalRecordNextVerificationChecklistModel';
	...
	get valueToDisplay(): string {
		...
		return MedicalRecordNextVerificationChecklistModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = MedicalRecordNextVerificationChecklistModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof MedicalRecordNextVerificationChecklistModel]?: MedicalRecordNextVerificationChecklistModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new MedicalRecordNextVerificationChecklistModel(data);
		}
	 ...
	 * @example
	 *
	 * `let medicalRecordNextVerificationChecklistModel = new MedicalRecordNextVerificationChecklistModel(data);`
	 *
	 * @param data The input data to be initialised as the MedicalRecordNextVerificationChecklistModel,
	 *    it is expected as a JSON string or as a nullable MedicalRecordNextVerificationChecklistModel.
	 */
	constructor(data?: string | Partial<MedicalRecordNextVerificationChecklistModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<MedicalRecordNextVerificationChecklistModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		MedicalRecordNextVerificationChecklistModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): MedicalRecordNextVerificationChecklistModel {
		...
		return new MedicalRecordNextVerificationChecklistModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof MedicalRecordNextVerificationChecklistModel)) {
			return {};
		}
