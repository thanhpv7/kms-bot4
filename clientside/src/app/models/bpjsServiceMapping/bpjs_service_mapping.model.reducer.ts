...
import {initialBpjsServiceMappingCollectionState} from './bpjs_service_mapping.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsServiceMappingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsServiceMappingModelActionTypes.CREATE_BPJS_SERVICE_MAPPING_OK:
				const createdModel = (action as modelAction.BpjsServiceMappingActionOK ).stateConfig.targetModel;
				...
				BpjsServiceMappingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.CREATE_BPJS_SERVICE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.CREATE_ALL_BPJS_SERVICE_MAPPING_OK:
				const createdModels = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsServiceMappingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.CREATE_ALL_BPJS_SERVICE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.DELETE_BPJS_SERVICE_MAPPING_OK:
				const deletedModelId = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.DELETE_ALL_BPJS_SERVICE_MAPPING_OK:
				const deletedModelIds = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.DELETE_BPJS_SERVICE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.UPDATE_BPJS_SERVICE_MAPPING_OK:
				const updatedModel = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModel;
				...
				BpjsServiceMappingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.UPDATE_BPJS_SERVICE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsServiceMappingModelActionTypes.UPDATE_ALL_BPJS_SERVICE_MAPPING_OK:
				const updatedModels = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.UPDATE_ALL_BPJS_SERVICE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.FETCH_BPJS_SERVICE_MAPPING_OK:
				let fetchedModel = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModel;
				...
				BpjsServiceMappingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.FETCH_ALL_BPJS_SERVICE_MAPPING_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsServiceMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.FETCH_BPJS_SERVICE_MAPPING_AUDIT_OK:
				const audits = (action as modelAction.BpjsServiceMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.FETCH_BPJS_SERVICE_MAPPING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsServiceMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.FETCH_BPJS_SERVICE_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsServiceMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.FETCH_LAST_BPJS_SERVICE_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsServiceMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.COUNT_BPJS_SERVICE_MAPPINGS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsServiceMappingModelActionTypes.INITIALISE_BPJS_SERVICE_MAPPING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsServiceMappingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsServiceMappingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsServiceMappingCollectionState
					};
				}
