...
import {initialBpjsDoctorMappingCollectionState} from './bpjs_doctor_mapping.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsDoctorMappingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsDoctorMappingModelActionTypes.CREATE_BPJS_DOCTOR_MAPPING_OK:
				const createdModel = (action as modelAction.BpjsDoctorMappingActionOK ).stateConfig.targetModel;
				...
				BpjsDoctorMappingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.CREATE_BPJS_DOCTOR_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.CREATE_ALL_BPJS_DOCTOR_MAPPING_OK:
				const createdModels = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsDoctorMappingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.CREATE_ALL_BPJS_DOCTOR_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.DELETE_BPJS_DOCTOR_MAPPING_OK:
				const deletedModelId = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.DELETE_ALL_BPJS_DOCTOR_MAPPING_OK:
				const deletedModelIds = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.DELETE_BPJS_DOCTOR_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.UPDATE_BPJS_DOCTOR_MAPPING_OK:
				const updatedModel = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModel;
				...
				BpjsDoctorMappingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.UPDATE_BPJS_DOCTOR_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsDoctorMappingModelActionTypes.UPDATE_ALL_BPJS_DOCTOR_MAPPING_OK:
				const updatedModels = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.UPDATE_ALL_BPJS_DOCTOR_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.FETCH_BPJS_DOCTOR_MAPPING_OK:
				let fetchedModel = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModel;
				...
				BpjsDoctorMappingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.FETCH_ALL_BPJS_DOCTOR_MAPPING_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsDoctorMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.FETCH_BPJS_DOCTOR_MAPPING_AUDIT_OK:
				const audits = (action as modelAction.BpjsDoctorMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.FETCH_BPJS_DOCTOR_MAPPING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsDoctorMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.FETCH_BPJS_DOCTOR_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsDoctorMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.FETCH_LAST_BPJS_DOCTOR_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsDoctorMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.COUNT_BPJS_DOCTOR_MAPPINGS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsDoctorMappingModelActionTypes.INITIALISE_BPJS_DOCTOR_MAPPING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsDoctorMappingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsDoctorMappingCollectionState
					};
				}
