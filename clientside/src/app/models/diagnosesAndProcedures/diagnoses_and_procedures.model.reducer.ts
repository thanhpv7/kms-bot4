...
import {initialDiagnosesAndProceduresCollectionState} from './diagnoses_and_procedures.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDiagnosesAndProceduresAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DiagnosesAndProceduresModelActionTypes.CREATE_DIAGNOSES_AND_PROCEDURES_OK:
				const createdModel = (action as modelAction.DiagnosesAndProceduresActionOK ).stateConfig.targetModel;
				...
				DiagnosesAndProceduresModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.CREATE_DIAGNOSES_AND_PROCEDURES_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.CREATE_ALL_DIAGNOSES_AND_PROCEDURES_OK:
				const createdModels = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DiagnosesAndProceduresModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.CREATE_ALL_DIAGNOSES_AND_PROCEDURES_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.DELETE_DIAGNOSES_AND_PROCEDURES_OK:
				const deletedModelId = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.DELETE_ALL_DIAGNOSES_AND_PROCEDURES_OK:
				const deletedModelIds = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.DELETE_DIAGNOSES_AND_PROCEDURES_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.UPDATE_DIAGNOSES_AND_PROCEDURES_OK:
				const updatedModel = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModel;
				...
				DiagnosesAndProceduresModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.UPDATE_DIAGNOSES_AND_PROCEDURES_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DiagnosesAndProceduresModelActionTypes.UPDATE_ALL_DIAGNOSES_AND_PROCEDURES_OK:
				const updatedModels = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.UPDATE_ALL_DIAGNOSES_AND_PROCEDURES_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.FETCH_DIAGNOSES_AND_PROCEDURES_OK:
				let fetchedModel = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModel;
				...
				DiagnosesAndProceduresModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.FETCH_ALL_DIAGNOSES_AND_PROCEDURES_OK:
				const totalCountAfterFetchAll = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DiagnosesAndProceduresModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.FETCH_DIAGNOSES_AND_PROCEDURES_AUDIT_OK:
				const audits = (action as modelAction.DiagnosesAndProceduresActionOK).audits;
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.FETCH_DIAGNOSES_AND_PROCEDURES_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DiagnosesAndProceduresActionOK).audits;
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.FETCH_DIAGNOSES_AND_PROCEDURES_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DiagnosesAndProceduresModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.FETCH_LAST_DIAGNOSES_AND_PROCEDURES_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DiagnosesAndProceduresModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.COUNT_DIAGNOSES_AND_PROCEDURESS_OK:
				const totalCountAfterCount = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DiagnosesAndProceduresModelActionTypes.INITIALISE_DIAGNOSES_AND_PROCEDURES_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DiagnosesAndProceduresActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDiagnosesAndProceduresCollectionState
					};
				}
