...
import {initialPatientPaymentInsuranceCollectionState} from './patient_payment_insurance.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientPaymentInsuranceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientPaymentInsuranceModelActionTypes.CREATE_PATIENT_PAYMENT_INSURANCE_OK:
				const createdModel = (action as modelAction.PatientPaymentInsuranceActionOK ).stateConfig.targetModel;
				...
				PatientPaymentInsuranceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.CREATE_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_INSURANCE_OK:
				const createdModels = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientPaymentInsuranceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_PAYMENT_INSURANCE_OK:
				const deletedModelId = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_INSURANCE_OK:
				const deletedModelIds = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.UPDATE_PATIENT_PAYMENT_INSURANCE_OK:
				const updatedModel = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModel;
				...
				PatientPaymentInsuranceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.UPDATE_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientPaymentInsuranceModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_INSURANCE_OK:
				const updatedModels = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_PAYMENT_INSURANCE_OK:
				let fetchedModel = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModel;
				...
				PatientPaymentInsuranceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_ALL_PATIENT_PAYMENT_INSURANCE_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientPaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_PAYMENT_INSURANCE_AUDIT_OK:
				const audits = (action as modelAction.PatientPaymentInsuranceActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_PAYMENT_INSURANCE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientPaymentInsuranceActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_PAYMENT_INSURANCE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientPaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.FETCH_LAST_PATIENT_PAYMENT_INSURANCE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientPaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.COUNT_PATIENT_PAYMENT_INSURANCES_OK:
				const totalCountAfterCount = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientPaymentInsuranceModelActionTypes.INITIALISE_PATIENT_PAYMENT_INSURANCE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientPaymentInsuranceCollectionState
					};
				}
