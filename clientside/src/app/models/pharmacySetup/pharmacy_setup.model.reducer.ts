...
import {initialPharmacySetupCollectionState} from './pharmacy_setup.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePharmacySetupAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PharmacySetupModelActionTypes.CREATE_PHARMACY_SETUP_OK:
				const createdModel = (action as modelAction.PharmacySetupActionOK ).stateConfig.targetModel;
				...
				PharmacySetupModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.CREATE_PHARMACY_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacySetupModelActionTypes.CREATE_ALL_PHARMACY_SETUP_OK:
				const createdModels = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PharmacySetupModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.CREATE_ALL_PHARMACY_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacySetupModelActionTypes.DELETE_PHARMACY_SETUP_OK:
				const deletedModelId = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.DELETE_ALL_PHARMACY_SETUP_OK:
				const deletedModelIds = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.DELETE_PHARMACY_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacySetupModelActionTypes.UPDATE_PHARMACY_SETUP_OK:
				const updatedModel = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModel;
				...
				PharmacySetupModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.UPDATE_PHARMACY_SETUP_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PharmacySetupModelActionTypes.UPDATE_ALL_PHARMACY_SETUP_OK:
				const updatedModels = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.UPDATE_ALL_PHARMACY_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacySetupModelActionTypes.FETCH_PHARMACY_SETUP_OK:
				let fetchedModel = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModel;
				...
				PharmacySetupModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.FETCH_ALL_PHARMACY_SETUP_OK:
				const totalCountAfterFetchAll = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PharmacySetupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PharmacySetupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.FETCH_PHARMACY_SETUP_AUDIT_OK:
				const audits = (action as modelAction.PharmacySetupActionOK).audits;
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.FETCH_PHARMACY_SETUP_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PharmacySetupActionOK).audits;
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.FETCH_PHARMACY_SETUP_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PharmacySetupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.FETCH_LAST_PHARMACY_SETUP_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PharmacySetupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PharmacySetupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.COUNT_PHARMACY_SETUPS_OK:
				const totalCountAfterCount = (action as modelAction.PharmacySetupActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PharmacySetupModelActionTypes.INITIALISE_PHARMACY_SETUP_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PharmacySetupActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PharmacySetupActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPharmacySetupCollectionState
					};
				}
