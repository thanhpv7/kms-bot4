...
import {initialClaimUserCollectionState} from './claim_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseClaimUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ClaimUserModelActionTypes.CREATE_CLAIM_USER_OK:
				const createdModel = (action as modelAction.ClaimUserActionOK ).stateConfig.targetModel;
				...
				ClaimUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ClaimUserModelActionTypes.CREATE_CLAIM_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ClaimUserModelActionTypes.CREATE_ALL_CLAIM_USER_OK:
				const createdModels = (action as modelAction.ClaimUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ClaimUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ClaimUserModelActionTypes.CREATE_ALL_CLAIM_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ClaimUserModelActionTypes.DELETE_CLAIM_USER_OK:
				const deletedModelId = (action as modelAction.ClaimUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ClaimUserModelActionTypes.DELETE_ALL_CLAIM_USER_OK:
				const deletedModelIds = (action as modelAction.ClaimUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ClaimUserModelActionTypes.DELETE_CLAIM_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ClaimUserModelActionTypes.UPDATE_CLAIM_USER_OK:
				const updatedModel = (action as modelAction.ClaimUserActionOK).stateConfig.targetModel;
				...
				ClaimUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ClaimUserModelActionTypes.UPDATE_CLAIM_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ClaimUserModelActionTypes.UPDATE_ALL_CLAIM_USER_OK:
				const updatedModels = (action as modelAction.ClaimUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ClaimUserModelActionTypes.UPDATE_ALL_CLAIM_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ClaimUserModelActionTypes.FETCH_CLAIM_USER_OK:
				let fetchedModel = (action as modelAction.ClaimUserActionOK).stateConfig.targetModel;
				...
				ClaimUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ClaimUserModelActionTypes.FETCH_ALL_CLAIM_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.ClaimUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ClaimUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ClaimUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ClaimUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ClaimUserModelActionTypes.FETCH_CLAIM_USER_AUDIT_OK:
				const audits = (action as modelAction.ClaimUserActionOK).audits;
				...
				break;
			case modelAction.ClaimUserModelActionTypes.FETCH_CLAIM_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ClaimUserActionOK).audits;
				...
				break;
			case modelAction.ClaimUserModelActionTypes.FETCH_CLAIM_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ClaimUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ClaimUserModelActionTypes.FETCH_LAST_CLAIM_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ClaimUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ClaimUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ClaimUserModelActionTypes.COUNT_CLAIM_USERS_OK:
				const totalCountAfterCount = (action as modelAction.ClaimUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ClaimUserModelActionTypes.INITIALISE_CLAIM_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ClaimUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ClaimUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialClaimUserCollectionState
					};
				}
