...
import {initialBpjsPcareKegiatanKelompokCollectionState} from './bpjs_pcare_kegiatan_kelompok.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcareKegiatanKelompokAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.CREATE_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				const createdModel = (action as modelAction.BpjsPcareKegiatanKelompokActionOK ).stateConfig.targetModel;
				...
				BpjsPcareKegiatanKelompokModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.CREATE_BPJS_PCARE_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.CREATE_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				const createdModels = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcareKegiatanKelompokModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.CREATE_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				const deletedModelId = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.DELETE_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				const deletedModelIds = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.UPDATE_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				const updatedModel = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModel;
				...
				BpjsPcareKegiatanKelompokModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.UPDATE_BPJS_PCARE_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.UPDATE_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				const updatedModels = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.UPDATE_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				let fetchedModel = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModel;
				...
				BpjsPcareKegiatanKelompokModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_ALL_BPJS_PCARE_KEGIATAN_KELOMPOK_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcareKegiatanKelompokModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_KEGIATAN_KELOMPOK_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_KEGIATAN_KELOMPOK_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_KEGIATAN_KELOMPOK_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcareKegiatanKelompokModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.FETCH_LAST_BPJS_PCARE_KEGIATAN_KELOMPOK_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcareKegiatanKelompokModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.COUNT_BPJS_PCARE_KEGIATAN_KELOMPOKS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcareKegiatanKelompokModelActionTypes.INITIALISE_BPJS_PCARE_KEGIATAN_KELOMPOK_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcareKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcareKegiatanKelompokCollectionState
					};
				}
