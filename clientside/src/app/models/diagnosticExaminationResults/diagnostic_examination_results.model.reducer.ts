...
import {initialDiagnosticExaminationResultsCollectionState} from './diagnostic_examination_results.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDiagnosticExaminationResultsAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DiagnosticExaminationResultsModelActionTypes.CREATE_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				const createdModel = (action as modelAction.DiagnosticExaminationResultsActionOK ).stateConfig.targetModel;
				...
				DiagnosticExaminationResultsModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.CREATE_DIAGNOSTIC_EXAMINATION_RESULTS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.CREATE_ALL_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				const createdModels = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DiagnosticExaminationResultsModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.CREATE_ALL_DIAGNOSTIC_EXAMINATION_RESULTS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.DELETE_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				const deletedModelId = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.DELETE_ALL_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				const deletedModelIds = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.DELETE_DIAGNOSTIC_EXAMINATION_RESULTS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.UPDATE_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				const updatedModel = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModel;
				...
				DiagnosticExaminationResultsModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.UPDATE_DIAGNOSTIC_EXAMINATION_RESULTS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DiagnosticExaminationResultsModelActionTypes.UPDATE_ALL_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				const updatedModels = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.UPDATE_ALL_DIAGNOSTIC_EXAMINATION_RESULTS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				let fetchedModel = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModel;
				...
				DiagnosticExaminationResultsModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_ALL_DIAGNOSTIC_EXAMINATION_RESULTS_OK:
				const totalCountAfterFetchAll = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DiagnosticExaminationResultsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_DIAGNOSTIC_EXAMINATION_RESULTS_AUDIT_OK:
				const audits = (action as modelAction.DiagnosticExaminationResultsActionOK).audits;
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_DIAGNOSTIC_EXAMINATION_RESULTS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DiagnosticExaminationResultsActionOK).audits;
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_DIAGNOSTIC_EXAMINATION_RESULTS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DiagnosticExaminationResultsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.FETCH_LAST_DIAGNOSTIC_EXAMINATION_RESULTS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DiagnosticExaminationResultsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.COUNT_DIAGNOSTIC_EXAMINATION_RESULTSS_OK:
				const totalCountAfterCount = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DiagnosticExaminationResultsModelActionTypes.INITIALISE_DIAGNOSTIC_EXAMINATION_RESULTS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DiagnosticExaminationResultsActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDiagnosticExaminationResultsCollectionState
					};
				}
