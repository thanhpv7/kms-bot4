...
import {initialPcareMCUCollectionState} from './pcare_mcu.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareMCUAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareMCUModelActionTypes.CREATE_PCARE_MCU_OK:
				const createdModel = (action as modelAction.PcareMCUActionOK ).stateConfig.targetModel;
				...
				PcareMCUModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareMCUModelActionTypes.CREATE_PCARE_MCU_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareMCUModelActionTypes.CREATE_ALL_PCARE_MCU_OK:
				const createdModels = (action as modelAction.PcareMCUActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareMCUModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareMCUModelActionTypes.CREATE_ALL_PCARE_MCU_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareMCUModelActionTypes.DELETE_PCARE_MCU_OK:
				const deletedModelId = (action as modelAction.PcareMCUActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareMCUModelActionTypes.DELETE_ALL_PCARE_MCU_OK:
				const deletedModelIds = (action as modelAction.PcareMCUActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareMCUModelActionTypes.DELETE_PCARE_MCU_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareMCUModelActionTypes.UPDATE_PCARE_MCU_OK:
				const updatedModel = (action as modelAction.PcareMCUActionOK).stateConfig.targetModel;
				...
				PcareMCUModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareMCUModelActionTypes.UPDATE_PCARE_MCU_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareMCUModelActionTypes.UPDATE_ALL_PCARE_MCU_OK:
				const updatedModels = (action as modelAction.PcareMCUActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareMCUModelActionTypes.UPDATE_ALL_PCARE_MCU_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareMCUModelActionTypes.FETCH_PCARE_MCU_OK:
				let fetchedModel = (action as modelAction.PcareMCUActionOK).stateConfig.targetModel;
				...
				PcareMCUModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareMCUModelActionTypes.FETCH_ALL_PCARE_MCU_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareMCUActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareMCUActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareMCUActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareMCUModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareMCUModelActionTypes.FETCH_PCARE_MCU_AUDIT_OK:
				const audits = (action as modelAction.PcareMCUActionOK).audits;
				...
				break;
			case modelAction.PcareMCUModelActionTypes.FETCH_PCARE_MCU_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareMCUActionOK).audits;
				...
				break;
			case modelAction.PcareMCUModelActionTypes.FETCH_PCARE_MCU_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareMCUModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareMCUModelActionTypes.FETCH_LAST_PCARE_MCU_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareMCUActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareMCUModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareMCUModelActionTypes.COUNT_PCARE_MCUS_OK:
				const totalCountAfterCount = (action as modelAction.PcareMCUActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareMCUModelActionTypes.INITIALISE_PCARE_MCU_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareMCUActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareMCUActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareMCUCollectionState
					};
				}
