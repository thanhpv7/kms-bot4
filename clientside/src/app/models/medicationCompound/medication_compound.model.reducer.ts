...
import {initialMedicationCompoundCollectionState} from './medication_compound.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicationCompoundAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicationCompoundModelActionTypes.CREATE_MEDICATION_COMPOUND_OK:
				const createdModel = (action as modelAction.MedicationCompoundActionOK ).stateConfig.targetModel;
				...
				MedicationCompoundModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.CREATE_MEDICATION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationCompoundModelActionTypes.CREATE_ALL_MEDICATION_COMPOUND_OK:
				const createdModels = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicationCompoundModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.CREATE_ALL_MEDICATION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationCompoundModelActionTypes.DELETE_MEDICATION_COMPOUND_OK:
				const deletedModelId = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.DELETE_ALL_MEDICATION_COMPOUND_OK:
				const deletedModelIds = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.DELETE_MEDICATION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationCompoundModelActionTypes.UPDATE_MEDICATION_COMPOUND_OK:
				const updatedModel = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModel;
				...
				MedicationCompoundModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.UPDATE_MEDICATION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicationCompoundModelActionTypes.UPDATE_ALL_MEDICATION_COMPOUND_OK:
				const updatedModels = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.UPDATE_ALL_MEDICATION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationCompoundModelActionTypes.FETCH_MEDICATION_COMPOUND_OK:
				let fetchedModel = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModel;
				...
				MedicationCompoundModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.FETCH_ALL_MEDICATION_COMPOUND_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicationCompoundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicationCompoundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.FETCH_MEDICATION_COMPOUND_AUDIT_OK:
				const audits = (action as modelAction.MedicationCompoundActionOK).audits;
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.FETCH_MEDICATION_COMPOUND_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicationCompoundActionOK).audits;
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.FETCH_MEDICATION_COMPOUND_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicationCompoundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.FETCH_LAST_MEDICATION_COMPOUND_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicationCompoundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicationCompoundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.COUNT_MEDICATION_COMPOUNDS_OK:
				const totalCountAfterCount = (action as modelAction.MedicationCompoundActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicationCompoundModelActionTypes.INITIALISE_MEDICATION_COMPOUND_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicationCompoundActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicationCompoundActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicationCompoundCollectionState
					};
				}
