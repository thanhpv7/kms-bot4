...
import {initialWarehouseUserCollectionState} from './warehouse_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseWarehouseUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.WarehouseUserModelActionTypes.CREATE_WAREHOUSE_USER_OK:
				const createdModel = (action as modelAction.WarehouseUserActionOK ).stateConfig.targetModel;
				...
				WarehouseUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.CREATE_WAREHOUSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseUserModelActionTypes.CREATE_ALL_WAREHOUSE_USER_OK:
				const createdModels = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					WarehouseUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.CREATE_ALL_WAREHOUSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseUserModelActionTypes.DELETE_WAREHOUSE_USER_OK:
				const deletedModelId = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.DELETE_ALL_WAREHOUSE_USER_OK:
				const deletedModelIds = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.DELETE_WAREHOUSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseUserModelActionTypes.UPDATE_WAREHOUSE_USER_OK:
				const updatedModel = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModel;
				...
				WarehouseUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.UPDATE_WAREHOUSE_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.WarehouseUserModelActionTypes.UPDATE_ALL_WAREHOUSE_USER_OK:
				const updatedModels = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.UPDATE_ALL_WAREHOUSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseUserModelActionTypes.FETCH_WAREHOUSE_USER_OK:
				let fetchedModel = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModel;
				...
				WarehouseUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.FETCH_ALL_WAREHOUSE_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.WarehouseUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => WarehouseUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.FETCH_WAREHOUSE_USER_AUDIT_OK:
				const audits = (action as modelAction.WarehouseUserActionOK).audits;
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.FETCH_WAREHOUSE_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.WarehouseUserActionOK).audits;
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.FETCH_WAREHOUSE_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => WarehouseUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.FETCH_LAST_WAREHOUSE_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.WarehouseUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => WarehouseUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.COUNT_WAREHOUSE_USERS_OK:
				const totalCountAfterCount = (action as modelAction.WarehouseUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.WarehouseUserModelActionTypes.INITIALISE_WAREHOUSE_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.WarehouseUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.WarehouseUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialWarehouseUserCollectionState
					};
				}
