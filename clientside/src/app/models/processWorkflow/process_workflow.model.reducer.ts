...
import {initialProcessWorkflowCollectionState} from './process_workflow.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseProcessWorkflowAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ProcessWorkflowModelActionTypes.CREATE_PROCESS_WORKFLOW_OK:
				const createdModel = (action as modelAction.ProcessWorkflowActionOK ).stateConfig.targetModel;
				...
				ProcessWorkflowModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.CREATE_PROCESS_WORKFLOW_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ProcessWorkflowModelActionTypes.CREATE_ALL_PROCESS_WORKFLOW_OK:
				const createdModels = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ProcessWorkflowModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.CREATE_ALL_PROCESS_WORKFLOW_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ProcessWorkflowModelActionTypes.DELETE_PROCESS_WORKFLOW_OK:
				const deletedModelId = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.DELETE_ALL_PROCESS_WORKFLOW_OK:
				const deletedModelIds = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.DELETE_PROCESS_WORKFLOW_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ProcessWorkflowModelActionTypes.UPDATE_PROCESS_WORKFLOW_OK:
				const updatedModel = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModel;
				...
				ProcessWorkflowModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.UPDATE_PROCESS_WORKFLOW_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ProcessWorkflowModelActionTypes.UPDATE_ALL_PROCESS_WORKFLOW_OK:
				const updatedModels = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.UPDATE_ALL_PROCESS_WORKFLOW_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ProcessWorkflowModelActionTypes.FETCH_PROCESS_WORKFLOW_OK:
				let fetchedModel = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModel;
				...
				ProcessWorkflowModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.FETCH_ALL_PROCESS_WORKFLOW_OK:
				const totalCountAfterFetchAll = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ProcessWorkflowActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ProcessWorkflowModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.FETCH_PROCESS_WORKFLOW_AUDIT_OK:
				const audits = (action as modelAction.ProcessWorkflowActionOK).audits;
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.FETCH_PROCESS_WORKFLOW_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ProcessWorkflowActionOK).audits;
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.FETCH_PROCESS_WORKFLOW_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ProcessWorkflowModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.FETCH_LAST_PROCESS_WORKFLOW_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ProcessWorkflowActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ProcessWorkflowModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.COUNT_PROCESS_WORKFLOWS_OK:
				const totalCountAfterCount = (action as modelAction.ProcessWorkflowActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ProcessWorkflowModelActionTypes.INITIALISE_PROCESS_WORKFLOW_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ProcessWorkflowActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ProcessWorkflowActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialProcessWorkflowCollectionState
					};
				}
