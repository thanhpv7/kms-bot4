...
import {initialRetailPharmacyInvoiceCollectionState} from './retail_pharmacy_invoice.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRetailPharmacyInvoiceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RetailPharmacyInvoiceModelActionTypes.CREATE_RETAIL_PHARMACY_INVOICE_OK:
				const createdModel = (action as modelAction.RetailPharmacyInvoiceActionOK ).stateConfig.targetModel;
				...
				RetailPharmacyInvoiceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.CREATE_RETAIL_PHARMACY_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.CREATE_ALL_RETAIL_PHARMACY_INVOICE_OK:
				const createdModels = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RetailPharmacyInvoiceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.CREATE_ALL_RETAIL_PHARMACY_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.DELETE_RETAIL_PHARMACY_INVOICE_OK:
				const deletedModelId = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.DELETE_ALL_RETAIL_PHARMACY_INVOICE_OK:
				const deletedModelIds = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.DELETE_RETAIL_PHARMACY_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.UPDATE_RETAIL_PHARMACY_INVOICE_OK:
				const updatedModel = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModel;
				...
				RetailPharmacyInvoiceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.UPDATE_RETAIL_PHARMACY_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RetailPharmacyInvoiceModelActionTypes.UPDATE_ALL_RETAIL_PHARMACY_INVOICE_OK:
				const updatedModels = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.UPDATE_ALL_RETAIL_PHARMACY_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.FETCH_RETAIL_PHARMACY_INVOICE_OK:
				let fetchedModel = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModel;
				...
				RetailPharmacyInvoiceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.FETCH_ALL_RETAIL_PHARMACY_INVOICE_OK:
				const totalCountAfterFetchAll = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RetailPharmacyInvoiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.FETCH_RETAIL_PHARMACY_INVOICE_AUDIT_OK:
				const audits = (action as modelAction.RetailPharmacyInvoiceActionOK).audits;
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.FETCH_RETAIL_PHARMACY_INVOICE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RetailPharmacyInvoiceActionOK).audits;
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.FETCH_RETAIL_PHARMACY_INVOICE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RetailPharmacyInvoiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.FETCH_LAST_RETAIL_PHARMACY_INVOICE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RetailPharmacyInvoiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.COUNT_RETAIL_PHARMACY_INVOICES_OK:
				const totalCountAfterCount = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RetailPharmacyInvoiceModelActionTypes.INITIALISE_RETAIL_PHARMACY_INVOICE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RetailPharmacyInvoiceActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialRetailPharmacyInvoiceCollectionState
					};
				}
