...
import {initialVitalSignsCollectionState} from './vital_signs.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseVitalSignsAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.VitalSignsModelActionTypes.CREATE_VITAL_SIGNS_OK:
				const createdModel = (action as modelAction.VitalSignsActionOK ).stateConfig.targetModel;
				...
				VitalSignsModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.VitalSignsModelActionTypes.CREATE_VITAL_SIGNS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VitalSignsModelActionTypes.CREATE_ALL_VITAL_SIGNS_OK:
				const createdModels = (action as modelAction.VitalSignsActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					VitalSignsModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.VitalSignsModelActionTypes.CREATE_ALL_VITAL_SIGNS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VitalSignsModelActionTypes.DELETE_VITAL_SIGNS_OK:
				const deletedModelId = (action as modelAction.VitalSignsActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.VitalSignsModelActionTypes.DELETE_ALL_VITAL_SIGNS_OK:
				const deletedModelIds = (action as modelAction.VitalSignsActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.VitalSignsModelActionTypes.DELETE_VITAL_SIGNS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VitalSignsModelActionTypes.UPDATE_VITAL_SIGNS_OK:
				const updatedModel = (action as modelAction.VitalSignsActionOK).stateConfig.targetModel;
				...
				VitalSignsModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VitalSignsModelActionTypes.UPDATE_VITAL_SIGNS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.VitalSignsModelActionTypes.UPDATE_ALL_VITAL_SIGNS_OK:
				const updatedModels = (action as modelAction.VitalSignsActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.VitalSignsModelActionTypes.UPDATE_ALL_VITAL_SIGNS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VitalSignsModelActionTypes.FETCH_VITAL_SIGNS_OK:
				let fetchedModel = (action as modelAction.VitalSignsActionOK).stateConfig.targetModel;
				...
				VitalSignsModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VitalSignsModelActionTypes.FETCH_ALL_VITAL_SIGNS_OK:
				const totalCountAfterFetchAll = (action as modelAction.VitalSignsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.VitalSignsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.VitalSignsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => VitalSignsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VitalSignsModelActionTypes.FETCH_VITAL_SIGNS_AUDIT_OK:
				const audits = (action as modelAction.VitalSignsActionOK).audits;
				...
				break;
			case modelAction.VitalSignsModelActionTypes.FETCH_VITAL_SIGNS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.VitalSignsActionOK).audits;
				...
				break;
			case modelAction.VitalSignsModelActionTypes.FETCH_VITAL_SIGNS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => VitalSignsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VitalSignsModelActionTypes.FETCH_LAST_VITAL_SIGNS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.VitalSignsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => VitalSignsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VitalSignsModelActionTypes.COUNT_VITAL_SIGNSS_OK:
				const totalCountAfterCount = (action as modelAction.VitalSignsActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.VitalSignsModelActionTypes.INITIALISE_VITAL_SIGNS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.VitalSignsActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.VitalSignsActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialVitalSignsCollectionState
					};
				}
