...
import {initialBpjsPcareDPHOCollectionState} from './bpjs_pcare_dpho.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcareDPHOAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcareDPHOModelActionTypes.CREATE_BPJS_PCARE_DPHO_OK:
				const createdModel = (action as modelAction.BpjsPcareDPHOActionOK ).stateConfig.targetModel;
				...
				BpjsPcareDPHOModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.CREATE_BPJS_PCARE_DPHO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.CREATE_ALL_BPJS_PCARE_DPHO_OK:
				const createdModels = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcareDPHOModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.CREATE_ALL_BPJS_PCARE_DPHO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.DELETE_BPJS_PCARE_DPHO_OK:
				const deletedModelId = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.DELETE_ALL_BPJS_PCARE_DPHO_OK:
				const deletedModelIds = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.DELETE_BPJS_PCARE_DPHO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.UPDATE_BPJS_PCARE_DPHO_OK:
				const updatedModel = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModel;
				...
				BpjsPcareDPHOModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.UPDATE_BPJS_PCARE_DPHO_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcareDPHOModelActionTypes.UPDATE_ALL_BPJS_PCARE_DPHO_OK:
				const updatedModels = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.UPDATE_ALL_BPJS_PCARE_DPHO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.FETCH_BPJS_PCARE_DPHO_OK:
				let fetchedModel = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModel;
				...
				BpjsPcareDPHOModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.FETCH_ALL_BPJS_PCARE_DPHO_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcareDPHOModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.FETCH_BPJS_PCARE_DPHO_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcareDPHOActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.FETCH_BPJS_PCARE_DPHO_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcareDPHOActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.FETCH_BPJS_PCARE_DPHO_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcareDPHOModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.FETCH_LAST_BPJS_PCARE_DPHO_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcareDPHOModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.COUNT_BPJS_PCARE_DPHOS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcareDPHOModelActionTypes.INITIALISE_BPJS_PCARE_DPHO_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcareDPHOActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcareDPHOCollectionState
					};
				}
