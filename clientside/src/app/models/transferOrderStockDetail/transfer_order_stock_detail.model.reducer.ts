...
import {initialTransferOrderStockDetailCollectionState} from './transfer_order_stock_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTransferOrderStockDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TransferOrderStockDetailModelActionTypes.CREATE_TRANSFER_ORDER_STOCK_DETAIL_OK:
				const createdModel = (action as modelAction.TransferOrderStockDetailActionOK ).stateConfig.targetModel;
				...
				TransferOrderStockDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.CREATE_TRANSFER_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.CREATE_ALL_TRANSFER_ORDER_STOCK_DETAIL_OK:
				const createdModels = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TransferOrderStockDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.CREATE_ALL_TRANSFER_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.DELETE_TRANSFER_ORDER_STOCK_DETAIL_OK:
				const deletedModelId = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.DELETE_ALL_TRANSFER_ORDER_STOCK_DETAIL_OK:
				const deletedModelIds = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.DELETE_TRANSFER_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.UPDATE_TRANSFER_ORDER_STOCK_DETAIL_OK:
				const updatedModel = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModel;
				...
				TransferOrderStockDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.UPDATE_TRANSFER_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TransferOrderStockDetailModelActionTypes.UPDATE_ALL_TRANSFER_ORDER_STOCK_DETAIL_OK:
				const updatedModels = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.UPDATE_ALL_TRANSFER_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.FETCH_TRANSFER_ORDER_STOCK_DETAIL_OK:
				let fetchedModel = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModel;
				...
				TransferOrderStockDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.FETCH_ALL_TRANSFER_ORDER_STOCK_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TransferOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.FETCH_TRANSFER_ORDER_STOCK_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.TransferOrderStockDetailActionOK).audits;
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.FETCH_TRANSFER_ORDER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TransferOrderStockDetailActionOK).audits;
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.FETCH_TRANSFER_ORDER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TransferOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.FETCH_LAST_TRANSFER_ORDER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TransferOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.COUNT_TRANSFER_ORDER_STOCK_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TransferOrderStockDetailModelActionTypes.INITIALISE_TRANSFER_ORDER_STOCK_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TransferOrderStockDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTransferOrderStockDetailCollectionState
					};
				}
