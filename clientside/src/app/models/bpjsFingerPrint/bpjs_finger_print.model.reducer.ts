...
import {initialBpjsFingerPrintCollectionState} from './bpjs_finger_print.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsFingerPrintAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsFingerPrintModelActionTypes.CREATE_BPJS_FINGER_PRINT_OK:
				const createdModel = (action as modelAction.BpjsFingerPrintActionOK ).stateConfig.targetModel;
				...
				BpjsFingerPrintModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.CREATE_BPJS_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.CREATE_ALL_BPJS_FINGER_PRINT_OK:
				const createdModels = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsFingerPrintModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.CREATE_ALL_BPJS_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.DELETE_BPJS_FINGER_PRINT_OK:
				const deletedModelId = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.DELETE_ALL_BPJS_FINGER_PRINT_OK:
				const deletedModelIds = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.DELETE_BPJS_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.UPDATE_BPJS_FINGER_PRINT_OK:
				const updatedModel = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModel;
				...
				BpjsFingerPrintModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.UPDATE_BPJS_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsFingerPrintModelActionTypes.UPDATE_ALL_BPJS_FINGER_PRINT_OK:
				const updatedModels = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.UPDATE_ALL_BPJS_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.FETCH_BPJS_FINGER_PRINT_OK:
				let fetchedModel = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModel;
				...
				BpjsFingerPrintModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.FETCH_ALL_BPJS_FINGER_PRINT_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsFingerPrintModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.FETCH_BPJS_FINGER_PRINT_AUDIT_OK:
				const audits = (action as modelAction.BpjsFingerPrintActionOK).audits;
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.FETCH_BPJS_FINGER_PRINT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsFingerPrintActionOK).audits;
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.FETCH_BPJS_FINGER_PRINT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsFingerPrintModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.FETCH_LAST_BPJS_FINGER_PRINT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsFingerPrintModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.COUNT_BPJS_FINGER_PRINTS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsFingerPrintModelActionTypes.INITIALISE_BPJS_FINGER_PRINT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsFingerPrintActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsFingerPrintCollectionState
					};
				}
