 ...
 */
export class BpjsFingerPrintModel extends AbstractModel {
	/**
	...
	readonly className = 'BpjsFingerPrintModel';
	...
	get valueToDisplay(): string {
		...
		return BpjsFingerPrintModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BpjsFingerPrintModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof BpjsFingerPrintModel]?: BpjsFingerPrintModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BpjsFingerPrintModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bpjsFingerPrintModel = new BpjsFingerPrintModel(data);`
	 *
	 * @param data The input data to be initialised as the BpjsFingerPrintModel,
	 *    it is expected as a JSON string or as a nullable BpjsFingerPrintModel.
	 */
	constructor(data?: string | Partial<BpjsFingerPrintModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BpjsFingerPrintModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BpjsFingerPrintModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BpjsFingerPrintModel {
		...
		return new BpjsFingerPrintModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BpjsFingerPrintModel)) {
			return {};
		}
