...
import {initialBpjsProcedureCollectionState} from './bpjs_procedure.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsProcedureAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsProcedureModelActionTypes.CREATE_BPJS_PROCEDURE_OK:
				const createdModel = (action as modelAction.BpjsProcedureActionOK ).stateConfig.targetModel;
				...
				BpjsProcedureModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.CREATE_BPJS_PROCEDURE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsProcedureModelActionTypes.CREATE_ALL_BPJS_PROCEDURE_OK:
				const createdModels = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsProcedureModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.CREATE_ALL_BPJS_PROCEDURE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsProcedureModelActionTypes.DELETE_BPJS_PROCEDURE_OK:
				const deletedModelId = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.DELETE_ALL_BPJS_PROCEDURE_OK:
				const deletedModelIds = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.DELETE_BPJS_PROCEDURE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsProcedureModelActionTypes.UPDATE_BPJS_PROCEDURE_OK:
				const updatedModel = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModel;
				...
				BpjsProcedureModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.UPDATE_BPJS_PROCEDURE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsProcedureModelActionTypes.UPDATE_ALL_BPJS_PROCEDURE_OK:
				const updatedModels = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.UPDATE_ALL_BPJS_PROCEDURE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsProcedureModelActionTypes.FETCH_BPJS_PROCEDURE_OK:
				let fetchedModel = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModel;
				...
				BpjsProcedureModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.FETCH_ALL_BPJS_PROCEDURE_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsProcedureActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsProcedureModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.FETCH_BPJS_PROCEDURE_AUDIT_OK:
				const audits = (action as modelAction.BpjsProcedureActionOK).audits;
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.FETCH_BPJS_PROCEDURE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsProcedureActionOK).audits;
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.FETCH_BPJS_PROCEDURE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsProcedureModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.FETCH_LAST_BPJS_PROCEDURE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsProcedureActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsProcedureModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.COUNT_BPJS_PROCEDURES_OK:
				const totalCountAfterCount = (action as modelAction.BpjsProcedureActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsProcedureModelActionTypes.INITIALISE_BPJS_PROCEDURE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsProcedureActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsProcedureActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsProcedureCollectionState
					};
				}
