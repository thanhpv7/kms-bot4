...
import {initialPurchaseRequisitionCollectionState} from './purchase_requisition.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePurchaseRequisitionAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PurchaseRequisitionModelActionTypes.CREATE_PURCHASE_REQUISITION_OK:
				const createdModel = (action as modelAction.PurchaseRequisitionActionOK ).stateConfig.targetModel;
				...
				PurchaseRequisitionModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.CREATE_PURCHASE_REQUISITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.CREATE_ALL_PURCHASE_REQUISITION_OK:
				const createdModels = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PurchaseRequisitionModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.CREATE_ALL_PURCHASE_REQUISITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.DELETE_PURCHASE_REQUISITION_OK:
				const deletedModelId = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.DELETE_ALL_PURCHASE_REQUISITION_OK:
				const deletedModelIds = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.DELETE_PURCHASE_REQUISITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.UPDATE_PURCHASE_REQUISITION_OK:
				const updatedModel = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModel;
				...
				PurchaseRequisitionModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.UPDATE_PURCHASE_REQUISITION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PurchaseRequisitionModelActionTypes.UPDATE_ALL_PURCHASE_REQUISITION_OK:
				const updatedModels = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.UPDATE_ALL_PURCHASE_REQUISITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.FETCH_PURCHASE_REQUISITION_OK:
				let fetchedModel = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModel;
				...
				PurchaseRequisitionModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.FETCH_ALL_PURCHASE_REQUISITION_OK:
				const totalCountAfterFetchAll = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PurchaseRequisitionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.FETCH_PURCHASE_REQUISITION_AUDIT_OK:
				const audits = (action as modelAction.PurchaseRequisitionActionOK).audits;
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.FETCH_PURCHASE_REQUISITION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PurchaseRequisitionActionOK).audits;
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.FETCH_PURCHASE_REQUISITION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PurchaseRequisitionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.FETCH_LAST_PURCHASE_REQUISITION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PurchaseRequisitionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.COUNT_PURCHASE_REQUISITIONS_OK:
				const totalCountAfterCount = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PurchaseRequisitionModelActionTypes.INITIALISE_PURCHASE_REQUISITION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PurchaseRequisitionActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PurchaseRequisitionActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPurchaseRequisitionCollectionState
					};
				}
