...
import {initialCashReceiptCollectionState} from './cash_receipt.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCashReceiptAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CashReceiptModelActionTypes.CREATE_CASH_RECEIPT_OK:
				const createdModel = (action as modelAction.CashReceiptActionOK ).stateConfig.targetModel;
				...
				CashReceiptModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CashReceiptModelActionTypes.CREATE_CASH_RECEIPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashReceiptModelActionTypes.CREATE_ALL_CASH_RECEIPT_OK:
				const createdModels = (action as modelAction.CashReceiptActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CashReceiptModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CashReceiptModelActionTypes.CREATE_ALL_CASH_RECEIPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashReceiptModelActionTypes.DELETE_CASH_RECEIPT_OK:
				const deletedModelId = (action as modelAction.CashReceiptActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashReceiptModelActionTypes.DELETE_ALL_CASH_RECEIPT_OK:
				const deletedModelIds = (action as modelAction.CashReceiptActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashReceiptModelActionTypes.DELETE_CASH_RECEIPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashReceiptModelActionTypes.UPDATE_CASH_RECEIPT_OK:
				const updatedModel = (action as modelAction.CashReceiptActionOK).stateConfig.targetModel;
				...
				CashReceiptModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashReceiptModelActionTypes.UPDATE_CASH_RECEIPT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CashReceiptModelActionTypes.UPDATE_ALL_CASH_RECEIPT_OK:
				const updatedModels = (action as modelAction.CashReceiptActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashReceiptModelActionTypes.UPDATE_ALL_CASH_RECEIPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashReceiptModelActionTypes.FETCH_CASH_RECEIPT_OK:
				let fetchedModel = (action as modelAction.CashReceiptActionOK).stateConfig.targetModel;
				...
				CashReceiptModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashReceiptModelActionTypes.FETCH_ALL_CASH_RECEIPT_OK:
				const totalCountAfterFetchAll = (action as modelAction.CashReceiptActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CashReceiptActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CashReceiptActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CashReceiptModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashReceiptModelActionTypes.FETCH_CASH_RECEIPT_AUDIT_OK:
				const audits = (action as modelAction.CashReceiptActionOK).audits;
				...
				break;
			case modelAction.CashReceiptModelActionTypes.FETCH_CASH_RECEIPT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CashReceiptActionOK).audits;
				...
				break;
			case modelAction.CashReceiptModelActionTypes.FETCH_CASH_RECEIPT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CashReceiptModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashReceiptModelActionTypes.FETCH_LAST_CASH_RECEIPT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CashReceiptActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CashReceiptModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashReceiptModelActionTypes.COUNT_CASH_RECEIPTS_OK:
				const totalCountAfterCount = (action as modelAction.CashReceiptActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CashReceiptModelActionTypes.INITIALISE_CASH_RECEIPT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CashReceiptActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CashReceiptActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCashReceiptCollectionState
					};
				}
