...
import {initialBpjsTreatmentHistoryCollectionState} from './bpjs_treatment_history.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsTreatmentHistoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsTreatmentHistoryModelActionTypes.CREATE_BPJS_TREATMENT_HISTORY_OK:
				const createdModel = (action as modelAction.BpjsTreatmentHistoryActionOK ).stateConfig.targetModel;
				...
				BpjsTreatmentHistoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.CREATE_BPJS_TREATMENT_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.CREATE_ALL_BPJS_TREATMENT_HISTORY_OK:
				const createdModels = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsTreatmentHistoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.CREATE_ALL_BPJS_TREATMENT_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.DELETE_BPJS_TREATMENT_HISTORY_OK:
				const deletedModelId = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.DELETE_ALL_BPJS_TREATMENT_HISTORY_OK:
				const deletedModelIds = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.DELETE_BPJS_TREATMENT_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.UPDATE_BPJS_TREATMENT_HISTORY_OK:
				const updatedModel = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModel;
				...
				BpjsTreatmentHistoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.UPDATE_BPJS_TREATMENT_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsTreatmentHistoryModelActionTypes.UPDATE_ALL_BPJS_TREATMENT_HISTORY_OK:
				const updatedModels = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.UPDATE_ALL_BPJS_TREATMENT_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_BPJS_TREATMENT_HISTORY_OK:
				let fetchedModel = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModel;
				...
				BpjsTreatmentHistoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_ALL_BPJS_TREATMENT_HISTORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsTreatmentHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_BPJS_TREATMENT_HISTORY_AUDIT_OK:
				const audits = (action as modelAction.BpjsTreatmentHistoryActionOK).audits;
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_BPJS_TREATMENT_HISTORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsTreatmentHistoryActionOK).audits;
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_BPJS_TREATMENT_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsTreatmentHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.FETCH_LAST_BPJS_TREATMENT_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsTreatmentHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.COUNT_BPJS_TREATMENT_HISTORYS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsTreatmentHistoryModelActionTypes.INITIALISE_BPJS_TREATMENT_HISTORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsTreatmentHistoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsTreatmentHistoryCollectionState
					};
				}
