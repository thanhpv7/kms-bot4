...
import {initialPatientCaseHistoryCollectionState} from './patient_case_history.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientCaseHistoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientCaseHistoryModelActionTypes.CREATE_PATIENT_CASE_HISTORY_OK:
				const createdModel = (action as modelAction.PatientCaseHistoryActionOK ).stateConfig.targetModel;
				...
				PatientCaseHistoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.CREATE_PATIENT_CASE_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.CREATE_ALL_PATIENT_CASE_HISTORY_OK:
				const createdModels = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientCaseHistoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.CREATE_ALL_PATIENT_CASE_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.DELETE_PATIENT_CASE_HISTORY_OK:
				const deletedModelId = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.DELETE_ALL_PATIENT_CASE_HISTORY_OK:
				const deletedModelIds = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.DELETE_PATIENT_CASE_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.UPDATE_PATIENT_CASE_HISTORY_OK:
				const updatedModel = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModel;
				...
				PatientCaseHistoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.UPDATE_PATIENT_CASE_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientCaseHistoryModelActionTypes.UPDATE_ALL_PATIENT_CASE_HISTORY_OK:
				const updatedModels = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.UPDATE_ALL_PATIENT_CASE_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.FETCH_PATIENT_CASE_HISTORY_OK:
				let fetchedModel = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModel;
				...
				PatientCaseHistoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.FETCH_ALL_PATIENT_CASE_HISTORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientCaseHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.FETCH_PATIENT_CASE_HISTORY_AUDIT_OK:
				const audits = (action as modelAction.PatientCaseHistoryActionOK).audits;
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.FETCH_PATIENT_CASE_HISTORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientCaseHistoryActionOK).audits;
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.FETCH_PATIENT_CASE_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientCaseHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.FETCH_LAST_PATIENT_CASE_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientCaseHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.COUNT_PATIENT_CASE_HISTORYS_OK:
				const totalCountAfterCount = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientCaseHistoryModelActionTypes.INITIALISE_PATIENT_CASE_HISTORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientCaseHistoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientCaseHistoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientCaseHistoryCollectionState
					};
				}
