...
import {initialHemodialysisExaminationCollectionState} from './hemodialysis_examination.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseHemodialysisExaminationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.HemodialysisExaminationModelActionTypes.CREATE_HEMODIALYSIS_EXAMINATION_OK:
				const createdModel = (action as modelAction.HemodialysisExaminationActionOK ).stateConfig.targetModel;
				...
				HemodialysisExaminationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.CREATE_HEMODIALYSIS_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.CREATE_ALL_HEMODIALYSIS_EXAMINATION_OK:
				const createdModels = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					HemodialysisExaminationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.CREATE_ALL_HEMODIALYSIS_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.DELETE_HEMODIALYSIS_EXAMINATION_OK:
				const deletedModelId = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.DELETE_ALL_HEMODIALYSIS_EXAMINATION_OK:
				const deletedModelIds = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.DELETE_HEMODIALYSIS_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.UPDATE_HEMODIALYSIS_EXAMINATION_OK:
				const updatedModel = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModel;
				...
				HemodialysisExaminationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.UPDATE_HEMODIALYSIS_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.HemodialysisExaminationModelActionTypes.UPDATE_ALL_HEMODIALYSIS_EXAMINATION_OK:
				const updatedModels = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.UPDATE_ALL_HEMODIALYSIS_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.FETCH_HEMODIALYSIS_EXAMINATION_OK:
				let fetchedModel = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModel;
				...
				HemodialysisExaminationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.FETCH_ALL_HEMODIALYSIS_EXAMINATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => HemodialysisExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.FETCH_HEMODIALYSIS_EXAMINATION_AUDIT_OK:
				const audits = (action as modelAction.HemodialysisExaminationActionOK).audits;
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.FETCH_HEMODIALYSIS_EXAMINATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.HemodialysisExaminationActionOK).audits;
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.FETCH_HEMODIALYSIS_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => HemodialysisExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.FETCH_LAST_HEMODIALYSIS_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => HemodialysisExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.COUNT_HEMODIALYSIS_EXAMINATIONS_OK:
				const totalCountAfterCount = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.HemodialysisExaminationModelActionTypes.INITIALISE_HEMODIALYSIS_EXAMINATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.HemodialysisExaminationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.HemodialysisExaminationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialHemodialysisExaminationCollectionState
					};
				}
