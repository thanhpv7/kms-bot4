...
import {initialHospitalBranchInformationCollectionState} from './hospital_branch_information.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseHospitalBranchInformationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.HospitalBranchInformationModelActionTypes.CREATE_HOSPITAL_BRANCH_INFORMATION_OK:
				const createdModel = (action as modelAction.HospitalBranchInformationActionOK ).stateConfig.targetModel;
				...
				HospitalBranchInformationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.CREATE_HOSPITAL_BRANCH_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.CREATE_ALL_HOSPITAL_BRANCH_INFORMATION_OK:
				const createdModels = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					HospitalBranchInformationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.CREATE_ALL_HOSPITAL_BRANCH_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.DELETE_HOSPITAL_BRANCH_INFORMATION_OK:
				const deletedModelId = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.DELETE_ALL_HOSPITAL_BRANCH_INFORMATION_OK:
				const deletedModelIds = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.DELETE_HOSPITAL_BRANCH_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.UPDATE_HOSPITAL_BRANCH_INFORMATION_OK:
				const updatedModel = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModel;
				...
				HospitalBranchInformationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.UPDATE_HOSPITAL_BRANCH_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.HospitalBranchInformationModelActionTypes.UPDATE_ALL_HOSPITAL_BRANCH_INFORMATION_OK:
				const updatedModels = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.UPDATE_ALL_HOSPITAL_BRANCH_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.FETCH_HOSPITAL_BRANCH_INFORMATION_OK:
				let fetchedModel = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModel;
				...
				HospitalBranchInformationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.FETCH_ALL_HOSPITAL_BRANCH_INFORMATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => HospitalBranchInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.FETCH_HOSPITAL_BRANCH_INFORMATION_AUDIT_OK:
				const audits = (action as modelAction.HospitalBranchInformationActionOK).audits;
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.FETCH_HOSPITAL_BRANCH_INFORMATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.HospitalBranchInformationActionOK).audits;
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.FETCH_HOSPITAL_BRANCH_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => HospitalBranchInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.FETCH_LAST_HOSPITAL_BRANCH_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => HospitalBranchInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.COUNT_HOSPITAL_BRANCH_INFORMATIONS_OK:
				const totalCountAfterCount = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.HospitalBranchInformationModelActionTypes.INITIALISE_HOSPITAL_BRANCH_INFORMATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.HospitalBranchInformationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.HospitalBranchInformationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialHospitalBranchInformationCollectionState
					};
				}
