...
import {initialCashRefundCollectionState} from './cash_refund.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCashRefundAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CashRefundModelActionTypes.CREATE_CASH_REFUND_OK:
				const createdModel = (action as modelAction.CashRefundActionOK ).stateConfig.targetModel;
				...
				CashRefundModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CashRefundModelActionTypes.CREATE_CASH_REFUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashRefundModelActionTypes.CREATE_ALL_CASH_REFUND_OK:
				const createdModels = (action as modelAction.CashRefundActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CashRefundModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CashRefundModelActionTypes.CREATE_ALL_CASH_REFUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashRefundModelActionTypes.DELETE_CASH_REFUND_OK:
				const deletedModelId = (action as modelAction.CashRefundActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashRefundModelActionTypes.DELETE_ALL_CASH_REFUND_OK:
				const deletedModelIds = (action as modelAction.CashRefundActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashRefundModelActionTypes.DELETE_CASH_REFUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashRefundModelActionTypes.UPDATE_CASH_REFUND_OK:
				const updatedModel = (action as modelAction.CashRefundActionOK).stateConfig.targetModel;
				...
				CashRefundModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashRefundModelActionTypes.UPDATE_CASH_REFUND_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CashRefundModelActionTypes.UPDATE_ALL_CASH_REFUND_OK:
				const updatedModels = (action as modelAction.CashRefundActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashRefundModelActionTypes.UPDATE_ALL_CASH_REFUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashRefundModelActionTypes.FETCH_CASH_REFUND_OK:
				let fetchedModel = (action as modelAction.CashRefundActionOK).stateConfig.targetModel;
				...
				CashRefundModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashRefundModelActionTypes.FETCH_ALL_CASH_REFUND_OK:
				const totalCountAfterFetchAll = (action as modelAction.CashRefundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CashRefundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CashRefundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CashRefundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashRefundModelActionTypes.FETCH_CASH_REFUND_AUDIT_OK:
				const audits = (action as modelAction.CashRefundActionOK).audits;
				...
				break;
			case modelAction.CashRefundModelActionTypes.FETCH_CASH_REFUND_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CashRefundActionOK).audits;
				...
				break;
			case modelAction.CashRefundModelActionTypes.FETCH_CASH_REFUND_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CashRefundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashRefundModelActionTypes.FETCH_LAST_CASH_REFUND_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CashRefundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CashRefundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashRefundModelActionTypes.COUNT_CASH_REFUNDS_OK:
				const totalCountAfterCount = (action as modelAction.CashRefundActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CashRefundModelActionTypes.INITIALISE_CASH_REFUND_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CashRefundActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CashRefundActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCashRefundCollectionState
					};
				}
