...
import {initialBodyChartExaminationCollectionState} from './body_chart_examination.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBodyChartExaminationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BodyChartExaminationModelActionTypes.CREATE_BODY_CHART_EXAMINATION_OK:
				const createdModel = (action as modelAction.BodyChartExaminationActionOK ).stateConfig.targetModel;
				...
				BodyChartExaminationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.CREATE_BODY_CHART_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BodyChartExaminationModelActionTypes.CREATE_ALL_BODY_CHART_EXAMINATION_OK:
				const createdModels = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BodyChartExaminationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.CREATE_ALL_BODY_CHART_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BodyChartExaminationModelActionTypes.DELETE_BODY_CHART_EXAMINATION_OK:
				const deletedModelId = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.DELETE_ALL_BODY_CHART_EXAMINATION_OK:
				const deletedModelIds = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.DELETE_BODY_CHART_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BodyChartExaminationModelActionTypes.UPDATE_BODY_CHART_EXAMINATION_OK:
				const updatedModel = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModel;
				...
				BodyChartExaminationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.UPDATE_BODY_CHART_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BodyChartExaminationModelActionTypes.UPDATE_ALL_BODY_CHART_EXAMINATION_OK:
				const updatedModels = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.UPDATE_ALL_BODY_CHART_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BodyChartExaminationModelActionTypes.FETCH_BODY_CHART_EXAMINATION_OK:
				let fetchedModel = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModel;
				...
				BodyChartExaminationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.FETCH_ALL_BODY_CHART_EXAMINATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BodyChartExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BodyChartExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.FETCH_BODY_CHART_EXAMINATION_AUDIT_OK:
				const audits = (action as modelAction.BodyChartExaminationActionOK).audits;
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.FETCH_BODY_CHART_EXAMINATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BodyChartExaminationActionOK).audits;
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.FETCH_BODY_CHART_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BodyChartExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.FETCH_LAST_BODY_CHART_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BodyChartExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BodyChartExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.COUNT_BODY_CHART_EXAMINATIONS_OK:
				const totalCountAfterCount = (action as modelAction.BodyChartExaminationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BodyChartExaminationModelActionTypes.INITIALISE_BODY_CHART_EXAMINATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BodyChartExaminationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BodyChartExaminationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBodyChartExaminationCollectionState
					};
				}
