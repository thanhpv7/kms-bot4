...
import {initialRoleCollectionState} from './role.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRoleAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RoleModelActionTypes.CREATE_ROLE_OK:
				const createdModel = (action as modelAction.RoleActionOK ).stateConfig.targetModel;
				...
				RoleModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RoleModelActionTypes.CREATE_ROLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoleModelActionTypes.CREATE_ALL_ROLE_OK:
				const createdModels = (action as modelAction.RoleActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RoleActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RoleModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RoleModelActionTypes.CREATE_ALL_ROLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoleModelActionTypes.DELETE_ROLE_OK:
				const deletedModelId = (action as modelAction.RoleActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RoleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoleModelActionTypes.DELETE_ALL_ROLE_OK:
				const deletedModelIds = (action as modelAction.RoleActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RoleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoleModelActionTypes.DELETE_ROLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoleModelActionTypes.UPDATE_ROLE_OK:
				const updatedModel = (action as modelAction.RoleActionOK).stateConfig.targetModel;
				...
				RoleModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoleModelActionTypes.UPDATE_ROLE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RoleModelActionTypes.UPDATE_ALL_ROLE_OK:
				const updatedModels = (action as modelAction.RoleActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RoleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoleModelActionTypes.UPDATE_ALL_ROLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoleModelActionTypes.FETCH_ROLE_OK:
				let fetchedModel = (action as modelAction.RoleActionOK).stateConfig.targetModel;
				...
				RoleModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoleModelActionTypes.FETCH_ALL_ROLE_OK:
				const totalCountAfterFetchAll = (action as modelAction.RoleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RoleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RoleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RoleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RoleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoleModelActionTypes.FETCH_ROLE_AUDIT_OK:
				const audits = (action as modelAction.RoleActionOK).audits;
				...
				break;
			case modelAction.RoleModelActionTypes.FETCH_ROLE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RoleActionOK).audits;
				...
				break;
			case modelAction.RoleModelActionTypes.FETCH_ROLE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RoleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RoleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RoleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RoleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RoleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoleModelActionTypes.FETCH_LAST_ROLE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RoleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RoleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RoleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RoleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RoleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoleModelActionTypes.COUNT_ROLES_OK:
				const totalCountAfterCount = (action as modelAction.RoleActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RoleModelActionTypes.INITIALISE_ROLE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RoleActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RoleActionOK).stateConfig.queryParams;
