...
import {initialInventoryBatchCorrectionItemCollectionState} from './inventory_batch_correction_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInventoryBatchCorrectionItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.CREATE_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				const createdModel = (action as modelAction.InventoryBatchCorrectionItemActionOK ).stateConfig.targetModel;
				...
				InventoryBatchCorrectionItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.CREATE_INVENTORY_BATCH_CORRECTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.CREATE_ALL_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				const createdModels = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InventoryBatchCorrectionItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.CREATE_ALL_INVENTORY_BATCH_CORRECTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				const deletedModelId = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.DELETE_ALL_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				const deletedModelIds = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.UPDATE_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				const updatedModel = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModel;
				...
				InventoryBatchCorrectionItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.UPDATE_INVENTORY_BATCH_CORRECTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InventoryBatchCorrectionItemModelActionTypes.UPDATE_ALL_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				const updatedModels = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.UPDATE_ALL_INVENTORY_BATCH_CORRECTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				let fetchedModel = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModel;
				...
				InventoryBatchCorrectionItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_ALL_INVENTORY_BATCH_CORRECTION_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InventoryBatchCorrectionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_ITEM_AUDIT_OK:
				const audits = (action as modelAction.InventoryBatchCorrectionItemActionOK).audits;
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InventoryBatchCorrectionItemActionOK).audits;
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InventoryBatchCorrectionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.FETCH_LAST_INVENTORY_BATCH_CORRECTION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InventoryBatchCorrectionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.COUNT_INVENTORY_BATCH_CORRECTION_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InventoryBatchCorrectionItemModelActionTypes.INITIALISE_INVENTORY_BATCH_CORRECTION_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InventoryBatchCorrectionItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInventoryBatchCorrectionItemCollectionState
					};
				}
