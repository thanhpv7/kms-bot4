...
import {initialTariffPackageServiceAssignmentCollectionState} from './tariff_package_service_assignment.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTariffPackageServiceAssignmentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.CREATE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				const createdModel = (action as modelAction.TariffPackageServiceAssignmentActionOK ).stateConfig.targetModel;
				...
				TariffPackageServiceAssignmentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.CREATE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.CREATE_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				const createdModels = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TariffPackageServiceAssignmentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.CREATE_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.DELETE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				const deletedModelId = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.DELETE_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				const deletedModelIds = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.DELETE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.UPDATE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				const updatedModel = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModel;
				...
				TariffPackageServiceAssignmentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.UPDATE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TariffPackageServiceAssignmentModelActionTypes.UPDATE_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				const updatedModels = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.UPDATE_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				let fetchedModel = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModel;
				...
				TariffPackageServiceAssignmentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_ALL_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TariffPackageServiceAssignmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_AUDIT_OK:
				const audits = (action as modelAction.TariffPackageServiceAssignmentActionOK).audits;
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TariffPackageServiceAssignmentActionOK).audits;
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TariffPackageServiceAssignmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.FETCH_LAST_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TariffPackageServiceAssignmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.COUNT_TARIFF_PACKAGE_SERVICE_ASSIGNMENTS_OK:
				const totalCountAfterCount = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TariffPackageServiceAssignmentModelActionTypes.INITIALISE_TARIFF_PACKAGE_SERVICE_ASSIGNMENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TariffPackageServiceAssignmentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTariffPackageServiceAssignmentCollectionState
					};
				}
