 ...
 */
export class TariffPackageServiceAssignmentModel extends AbstractModel {
	/**
	...
	readonly className = 'TariffPackageServiceAssignmentModel';
	...
	get valueToDisplay(): string {
		...
		return TariffPackageServiceAssignmentModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = TariffPackageServiceAssignmentModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof TariffPackageServiceAssignmentModel]?: TariffPackageServiceAssignmentModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new TariffPackageServiceAssignmentModel(data);
		}
	 ...
	 * @example
	 *
	 * `let tariffPackageServiceAssignmentModel = new TariffPackageServiceAssignmentModel(data);`
	 *
	 * @param data The input data to be initialised as the TariffPackageServiceAssignmentModel,
	 *    it is expected as a JSON string or as a nullable TariffPackageServiceAssignmentModel.
	 */
	constructor(data?: string | Partial<TariffPackageServiceAssignmentModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<TariffPackageServiceAssignmentModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		TariffPackageServiceAssignmentModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): TariffPackageServiceAssignmentModel {
		...
		return new TariffPackageServiceAssignmentModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof TariffPackageServiceAssignmentModel)) {
			return {};
		}
