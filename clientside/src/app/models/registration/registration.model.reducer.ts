...
import {initialRegistrationCollectionState} from './registration.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRegistrationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RegistrationModelActionTypes.CREATE_REGISTRATION_OK:
				const createdModel = (action as modelAction.RegistrationActionOK ).stateConfig.targetModel;
				...
				RegistrationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RegistrationModelActionTypes.CREATE_REGISTRATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationModelActionTypes.CREATE_ALL_REGISTRATION_OK:
				const createdModels = (action as modelAction.RegistrationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RegistrationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RegistrationModelActionTypes.CREATE_ALL_REGISTRATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationModelActionTypes.DELETE_REGISTRATION_OK:
				const deletedModelId = (action as modelAction.RegistrationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RegistrationModelActionTypes.DELETE_ALL_REGISTRATION_OK:
				const deletedModelIds = (action as modelAction.RegistrationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RegistrationModelActionTypes.DELETE_REGISTRATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationModelActionTypes.UPDATE_REGISTRATION_OK:
				const updatedModel = (action as modelAction.RegistrationActionOK).stateConfig.targetModel;
				...
				RegistrationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationModelActionTypes.UPDATE_REGISTRATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RegistrationModelActionTypes.UPDATE_ALL_REGISTRATION_OK:
				const updatedModels = (action as modelAction.RegistrationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RegistrationModelActionTypes.UPDATE_ALL_REGISTRATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationModelActionTypes.FETCH_REGISTRATION_OK:
				let fetchedModel = (action as modelAction.RegistrationActionOK).stateConfig.targetModel;
				...
				RegistrationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationModelActionTypes.FETCH_ALL_REGISTRATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.RegistrationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RegistrationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RegistrationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RegistrationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationModelActionTypes.FETCH_REGISTRATION_AUDIT_OK:
				const audits = (action as modelAction.RegistrationActionOK).audits;
				...
				break;
			case modelAction.RegistrationModelActionTypes.FETCH_REGISTRATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RegistrationActionOK).audits;
				...
				break;
			case modelAction.RegistrationModelActionTypes.FETCH_REGISTRATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RegistrationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationModelActionTypes.FETCH_LAST_REGISTRATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RegistrationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RegistrationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationModelActionTypes.COUNT_REGISTRATIONS_OK:
				const totalCountAfterCount = (action as modelAction.RegistrationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RegistrationModelActionTypes.INITIALISE_REGISTRATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RegistrationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RegistrationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialRegistrationCollectionState
					};
				}
