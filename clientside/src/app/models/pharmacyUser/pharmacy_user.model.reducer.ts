...
import {initialPharmacyUserCollectionState} from './pharmacy_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePharmacyUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PharmacyUserModelActionTypes.CREATE_PHARMACY_USER_OK:
				const createdModel = (action as modelAction.PharmacyUserActionOK ).stateConfig.targetModel;
				...
				PharmacyUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.CREATE_PHARMACY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacyUserModelActionTypes.CREATE_ALL_PHARMACY_USER_OK:
				const createdModels = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PharmacyUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.CREATE_ALL_PHARMACY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacyUserModelActionTypes.DELETE_PHARMACY_USER_OK:
				const deletedModelId = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.DELETE_ALL_PHARMACY_USER_OK:
				const deletedModelIds = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.DELETE_PHARMACY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacyUserModelActionTypes.UPDATE_PHARMACY_USER_OK:
				const updatedModel = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModel;
				...
				PharmacyUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.UPDATE_PHARMACY_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PharmacyUserModelActionTypes.UPDATE_ALL_PHARMACY_USER_OK:
				const updatedModels = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.UPDATE_ALL_PHARMACY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PharmacyUserModelActionTypes.FETCH_PHARMACY_USER_OK:
				let fetchedModel = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModel;
				...
				PharmacyUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.FETCH_ALL_PHARMACY_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PharmacyUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PharmacyUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.FETCH_PHARMACY_USER_AUDIT_OK:
				const audits = (action as modelAction.PharmacyUserActionOK).audits;
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.FETCH_PHARMACY_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PharmacyUserActionOK).audits;
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.FETCH_PHARMACY_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PharmacyUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.FETCH_LAST_PHARMACY_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PharmacyUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PharmacyUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.COUNT_PHARMACY_USERS_OK:
				const totalCountAfterCount = (action as modelAction.PharmacyUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PharmacyUserModelActionTypes.INITIALISE_PHARMACY_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PharmacyUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PharmacyUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPharmacyUserCollectionState
					};
				}
