...
import {initialNewbornDetailCollectionState} from './newborn_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNewbornDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NewbornDetailModelActionTypes.CREATE_NEWBORN_DETAIL_OK:
				const createdModel = (action as modelAction.NewbornDetailActionOK ).stateConfig.targetModel;
				...
				NewbornDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.CREATE_NEWBORN_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NewbornDetailModelActionTypes.CREATE_ALL_NEWBORN_DETAIL_OK:
				const createdModels = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NewbornDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.CREATE_ALL_NEWBORN_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NewbornDetailModelActionTypes.DELETE_NEWBORN_DETAIL_OK:
				const deletedModelId = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.DELETE_ALL_NEWBORN_DETAIL_OK:
				const deletedModelIds = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.DELETE_NEWBORN_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NewbornDetailModelActionTypes.UPDATE_NEWBORN_DETAIL_OK:
				const updatedModel = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModel;
				...
				NewbornDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.UPDATE_NEWBORN_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NewbornDetailModelActionTypes.UPDATE_ALL_NEWBORN_DETAIL_OK:
				const updatedModels = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.UPDATE_ALL_NEWBORN_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NewbornDetailModelActionTypes.FETCH_NEWBORN_DETAIL_OK:
				let fetchedModel = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModel;
				...
				NewbornDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.FETCH_ALL_NEWBORN_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NewbornDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NewbornDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.FETCH_NEWBORN_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.NewbornDetailActionOK).audits;
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.FETCH_NEWBORN_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NewbornDetailActionOK).audits;
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.FETCH_NEWBORN_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NewbornDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.FETCH_LAST_NEWBORN_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NewbornDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NewbornDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.COUNT_NEWBORN_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.NewbornDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NewbornDetailModelActionTypes.INITIALISE_NEWBORN_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NewbornDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NewbornDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNewbornDetailCollectionState
					};
				}
