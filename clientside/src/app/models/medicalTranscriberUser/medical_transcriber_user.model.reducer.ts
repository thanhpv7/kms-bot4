...
import {initialMedicalTranscriberUserCollectionState} from './medical_transcriber_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicalTranscriberUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicalTranscriberUserModelActionTypes.CREATE_MEDICAL_TRANSCRIBER_USER_OK:
				const createdModel = (action as modelAction.MedicalTranscriberUserActionOK ).stateConfig.targetModel;
				...
				MedicalTranscriberUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.CREATE_MEDICAL_TRANSCRIBER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.CREATE_ALL_MEDICAL_TRANSCRIBER_USER_OK:
				const createdModels = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicalTranscriberUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.CREATE_ALL_MEDICAL_TRANSCRIBER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.DELETE_MEDICAL_TRANSCRIBER_USER_OK:
				const deletedModelId = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.DELETE_ALL_MEDICAL_TRANSCRIBER_USER_OK:
				const deletedModelIds = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.DELETE_MEDICAL_TRANSCRIBER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.UPDATE_MEDICAL_TRANSCRIBER_USER_OK:
				const updatedModel = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModel;
				...
				MedicalTranscriberUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.UPDATE_MEDICAL_TRANSCRIBER_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicalTranscriberUserModelActionTypes.UPDATE_ALL_MEDICAL_TRANSCRIBER_USER_OK:
				const updatedModels = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.UPDATE_ALL_MEDICAL_TRANSCRIBER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.FETCH_MEDICAL_TRANSCRIBER_USER_OK:
				let fetchedModel = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModel;
				...
				MedicalTranscriberUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.FETCH_ALL_MEDICAL_TRANSCRIBER_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicalTranscriberUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.FETCH_MEDICAL_TRANSCRIBER_USER_AUDIT_OK:
				const audits = (action as modelAction.MedicalTranscriberUserActionOK).audits;
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.FETCH_MEDICAL_TRANSCRIBER_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicalTranscriberUserActionOK).audits;
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.FETCH_MEDICAL_TRANSCRIBER_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicalTranscriberUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.FETCH_LAST_MEDICAL_TRANSCRIBER_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicalTranscriberUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.COUNT_MEDICAL_TRANSCRIBER_USERS_OK:
				const totalCountAfterCount = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicalTranscriberUserModelActionTypes.INITIALISE_MEDICAL_TRANSCRIBER_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicalTranscriberUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicalTranscriberUserCollectionState
					};
				}
