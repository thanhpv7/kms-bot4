...
import {initialTariffDefinitionCollectionState} from './tariff_definition.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTariffDefinitionAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TariffDefinitionModelActionTypes.CREATE_TARIFF_DEFINITION_OK:
				const createdModel = (action as modelAction.TariffDefinitionActionOK ).stateConfig.targetModel;
				...
				TariffDefinitionModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.CREATE_TARIFF_DEFINITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionModelActionTypes.CREATE_ALL_TARIFF_DEFINITION_OK:
				const createdModels = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TariffDefinitionModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.CREATE_ALL_TARIFF_DEFINITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionModelActionTypes.DELETE_TARIFF_DEFINITION_OK:
				const deletedModelId = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.DELETE_ALL_TARIFF_DEFINITION_OK:
				const deletedModelIds = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.DELETE_TARIFF_DEFINITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionModelActionTypes.UPDATE_TARIFF_DEFINITION_OK:
				const updatedModel = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModel;
				...
				TariffDefinitionModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.UPDATE_TARIFF_DEFINITION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TariffDefinitionModelActionTypes.UPDATE_ALL_TARIFF_DEFINITION_OK:
				const updatedModels = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.UPDATE_ALL_TARIFF_DEFINITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionModelActionTypes.FETCH_TARIFF_DEFINITION_OK:
				let fetchedModel = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModel;
				...
				TariffDefinitionModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.FETCH_ALL_TARIFF_DEFINITION_OK:
				const totalCountAfterFetchAll = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TariffDefinitionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TariffDefinitionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.FETCH_TARIFF_DEFINITION_AUDIT_OK:
				const audits = (action as modelAction.TariffDefinitionActionOK).audits;
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.FETCH_TARIFF_DEFINITION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TariffDefinitionActionOK).audits;
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.FETCH_TARIFF_DEFINITION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TariffDefinitionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.FETCH_LAST_TARIFF_DEFINITION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TariffDefinitionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.COUNT_TARIFF_DEFINITIONS_OK:
				const totalCountAfterCount = (action as modelAction.TariffDefinitionActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TariffDefinitionModelActionTypes.INITIALISE_TARIFF_DEFINITION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TariffDefinitionActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TariffDefinitionActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTariffDefinitionCollectionState
					};
				}
