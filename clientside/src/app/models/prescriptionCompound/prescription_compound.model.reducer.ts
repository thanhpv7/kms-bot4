...
import {initialPrescriptionCompoundCollectionState} from './prescription_compound.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePrescriptionCompoundAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PrescriptionCompoundModelActionTypes.CREATE_PRESCRIPTION_COMPOUND_OK:
				const createdModel = (action as modelAction.PrescriptionCompoundActionOK ).stateConfig.targetModel;
				...
				PrescriptionCompoundModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.CREATE_PRESCRIPTION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.CREATE_ALL_PRESCRIPTION_COMPOUND_OK:
				const createdModels = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PrescriptionCompoundModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.CREATE_ALL_PRESCRIPTION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.DELETE_PRESCRIPTION_COMPOUND_OK:
				const deletedModelId = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.DELETE_ALL_PRESCRIPTION_COMPOUND_OK:
				const deletedModelIds = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.DELETE_PRESCRIPTION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.UPDATE_PRESCRIPTION_COMPOUND_OK:
				const updatedModel = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModel;
				...
				PrescriptionCompoundModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.UPDATE_PRESCRIPTION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PrescriptionCompoundModelActionTypes.UPDATE_ALL_PRESCRIPTION_COMPOUND_OK:
				const updatedModels = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.UPDATE_ALL_PRESCRIPTION_COMPOUND_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.FETCH_PRESCRIPTION_COMPOUND_OK:
				let fetchedModel = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModel;
				...
				PrescriptionCompoundModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.FETCH_ALL_PRESCRIPTION_COMPOUND_OK:
				const totalCountAfterFetchAll = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PrescriptionCompoundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.FETCH_PRESCRIPTION_COMPOUND_AUDIT_OK:
				const audits = (action as modelAction.PrescriptionCompoundActionOK).audits;
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.FETCH_PRESCRIPTION_COMPOUND_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PrescriptionCompoundActionOK).audits;
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.FETCH_PRESCRIPTION_COMPOUND_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PrescriptionCompoundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.FETCH_LAST_PRESCRIPTION_COMPOUND_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PrescriptionCompoundModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.COUNT_PRESCRIPTION_COMPOUNDS_OK:
				const totalCountAfterCount = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PrescriptionCompoundModelActionTypes.INITIALISE_PRESCRIPTION_COMPOUND_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PrescriptionCompoundActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PrescriptionCompoundActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPrescriptionCompoundCollectionState
					};
				}
