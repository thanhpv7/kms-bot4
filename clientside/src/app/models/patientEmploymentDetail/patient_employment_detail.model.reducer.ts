...
import {initialPatientEmploymentDetailCollectionState} from './patient_employment_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientEmploymentDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientEmploymentDetailModelActionTypes.CREATE_PATIENT_EMPLOYMENT_DETAIL_OK:
				const createdModel = (action as modelAction.PatientEmploymentDetailActionOK ).stateConfig.targetModel;
				...
				PatientEmploymentDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.CREATE_PATIENT_EMPLOYMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.CREATE_ALL_PATIENT_EMPLOYMENT_DETAIL_OK:
				const createdModels = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientEmploymentDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.CREATE_ALL_PATIENT_EMPLOYMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.DELETE_PATIENT_EMPLOYMENT_DETAIL_OK:
				const deletedModelId = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.DELETE_ALL_PATIENT_EMPLOYMENT_DETAIL_OK:
				const deletedModelIds = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.DELETE_PATIENT_EMPLOYMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.UPDATE_PATIENT_EMPLOYMENT_DETAIL_OK:
				const updatedModel = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModel;
				...
				PatientEmploymentDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.UPDATE_PATIENT_EMPLOYMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientEmploymentDetailModelActionTypes.UPDATE_ALL_PATIENT_EMPLOYMENT_DETAIL_OK:
				const updatedModels = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.UPDATE_ALL_PATIENT_EMPLOYMENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.FETCH_PATIENT_EMPLOYMENT_DETAIL_OK:
				let fetchedModel = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModel;
				...
				PatientEmploymentDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.FETCH_ALL_PATIENT_EMPLOYMENT_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientEmploymentDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.FETCH_PATIENT_EMPLOYMENT_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.PatientEmploymentDetailActionOK).audits;
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.FETCH_PATIENT_EMPLOYMENT_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientEmploymentDetailActionOK).audits;
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.FETCH_PATIENT_EMPLOYMENT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientEmploymentDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.FETCH_LAST_PATIENT_EMPLOYMENT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientEmploymentDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.COUNT_PATIENT_EMPLOYMENT_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientEmploymentDetailModelActionTypes.INITIALISE_PATIENT_EMPLOYMENT_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientEmploymentDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientEmploymentDetailCollectionState
					};
				}
