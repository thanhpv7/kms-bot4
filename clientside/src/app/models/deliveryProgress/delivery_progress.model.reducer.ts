...
import {initialDeliveryProgressCollectionState} from './delivery_progress.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDeliveryProgressAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DeliveryProgressModelActionTypes.CREATE_DELIVERY_PROGRESS_OK:
				const createdModel = (action as modelAction.DeliveryProgressActionOK ).stateConfig.targetModel;
				...
				DeliveryProgressModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.CREATE_DELIVERY_PROGRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DeliveryProgressModelActionTypes.CREATE_ALL_DELIVERY_PROGRESS_OK:
				const createdModels = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DeliveryProgressModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.CREATE_ALL_DELIVERY_PROGRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DeliveryProgressModelActionTypes.DELETE_DELIVERY_PROGRESS_OK:
				const deletedModelId = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.DELETE_ALL_DELIVERY_PROGRESS_OK:
				const deletedModelIds = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.DELETE_DELIVERY_PROGRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DeliveryProgressModelActionTypes.UPDATE_DELIVERY_PROGRESS_OK:
				const updatedModel = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModel;
				...
				DeliveryProgressModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.UPDATE_DELIVERY_PROGRESS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DeliveryProgressModelActionTypes.UPDATE_ALL_DELIVERY_PROGRESS_OK:
				const updatedModels = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.UPDATE_ALL_DELIVERY_PROGRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DeliveryProgressModelActionTypes.FETCH_DELIVERY_PROGRESS_OK:
				let fetchedModel = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModel;
				...
				DeliveryProgressModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.FETCH_ALL_DELIVERY_PROGRESS_OK:
				const totalCountAfterFetchAll = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DeliveryProgressActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DeliveryProgressModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.FETCH_DELIVERY_PROGRESS_AUDIT_OK:
				const audits = (action as modelAction.DeliveryProgressActionOK).audits;
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.FETCH_DELIVERY_PROGRESS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DeliveryProgressActionOK).audits;
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.FETCH_DELIVERY_PROGRESS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DeliveryProgressModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.FETCH_LAST_DELIVERY_PROGRESS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DeliveryProgressActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DeliveryProgressModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.COUNT_DELIVERY_PROGRESSS_OK:
				const totalCountAfterCount = (action as modelAction.DeliveryProgressActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DeliveryProgressModelActionTypes.INITIALISE_DELIVERY_PROGRESS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DeliveryProgressActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DeliveryProgressActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDeliveryProgressCollectionState
					};
				}
