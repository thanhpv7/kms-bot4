...
import {initialBpjsPcareRegistrationsCollectionState} from './bpjs_pcare_registrations.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcareRegistrationsAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcareRegistrationsModelActionTypes.CREATE_BPJS_PCARE_REGISTRATIONS_OK:
				const createdModel = (action as modelAction.BpjsPcareRegistrationsActionOK ).stateConfig.targetModel;
				...
				BpjsPcareRegistrationsModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.CREATE_BPJS_PCARE_REGISTRATIONS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.CREATE_ALL_BPJS_PCARE_REGISTRATIONS_OK:
				const createdModels = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcareRegistrationsModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.CREATE_ALL_BPJS_PCARE_REGISTRATIONS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.DELETE_BPJS_PCARE_REGISTRATIONS_OK:
				const deletedModelId = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.DELETE_ALL_BPJS_PCARE_REGISTRATIONS_OK:
				const deletedModelIds = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.DELETE_BPJS_PCARE_REGISTRATIONS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.UPDATE_BPJS_PCARE_REGISTRATIONS_OK:
				const updatedModel = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModel;
				...
				BpjsPcareRegistrationsModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.UPDATE_BPJS_PCARE_REGISTRATIONS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcareRegistrationsModelActionTypes.UPDATE_ALL_BPJS_PCARE_REGISTRATIONS_OK:
				const updatedModels = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.UPDATE_ALL_BPJS_PCARE_REGISTRATIONS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_BPJS_PCARE_REGISTRATIONS_OK:
				let fetchedModel = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModel;
				...
				BpjsPcareRegistrationsModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_ALL_BPJS_PCARE_REGISTRATIONS_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcareRegistrationsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_BPJS_PCARE_REGISTRATIONS_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcareRegistrationsActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_BPJS_PCARE_REGISTRATIONS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcareRegistrationsActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_BPJS_PCARE_REGISTRATIONS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcareRegistrationsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.FETCH_LAST_BPJS_PCARE_REGISTRATIONS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcareRegistrationsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.COUNT_BPJS_PCARE_REGISTRATIONSS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcareRegistrationsModelActionTypes.INITIALISE_BPJS_PCARE_REGISTRATIONS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcareRegistrationsActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcareRegistrationsCollectionState
					};
				}
