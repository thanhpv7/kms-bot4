...
import {initialNhisClaimCollectionState} from './nhis_claim.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisClaimAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisClaimModelActionTypes.CREATE_NHIS_CLAIM_OK:
				const createdModel = (action as modelAction.NhisClaimActionOK ).stateConfig.targetModel;
				...
				NhisClaimModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisClaimModelActionTypes.CREATE_NHIS_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimModelActionTypes.CREATE_ALL_NHIS_CLAIM_OK:
				const createdModels = (action as modelAction.NhisClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisClaimModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisClaimModelActionTypes.CREATE_ALL_NHIS_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimModelActionTypes.DELETE_NHIS_CLAIM_OK:
				const deletedModelId = (action as modelAction.NhisClaimActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimModelActionTypes.DELETE_ALL_NHIS_CLAIM_OK:
				const deletedModelIds = (action as modelAction.NhisClaimActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimModelActionTypes.DELETE_NHIS_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimModelActionTypes.UPDATE_NHIS_CLAIM_OK:
				const updatedModel = (action as modelAction.NhisClaimActionOK).stateConfig.targetModel;
				...
				NhisClaimModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimModelActionTypes.UPDATE_NHIS_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisClaimModelActionTypes.UPDATE_ALL_NHIS_CLAIM_OK:
				const updatedModels = (action as modelAction.NhisClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimModelActionTypes.UPDATE_ALL_NHIS_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimModelActionTypes.FETCH_NHIS_CLAIM_OK:
				let fetchedModel = (action as modelAction.NhisClaimActionOK).stateConfig.targetModel;
				...
				NhisClaimModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimModelActionTypes.FETCH_ALL_NHIS_CLAIM_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisClaimActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisClaimActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisClaimModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimModelActionTypes.FETCH_NHIS_CLAIM_AUDIT_OK:
				const audits = (action as modelAction.NhisClaimActionOK).audits;
				...
				break;
			case modelAction.NhisClaimModelActionTypes.FETCH_NHIS_CLAIM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisClaimActionOK).audits;
				...
				break;
			case modelAction.NhisClaimModelActionTypes.FETCH_NHIS_CLAIM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisClaimModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimModelActionTypes.FETCH_LAST_NHIS_CLAIM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisClaimActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisClaimModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimModelActionTypes.COUNT_NHIS_CLAIMS_OK:
				const totalCountAfterCount = (action as modelAction.NhisClaimActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisClaimModelActionTypes.INITIALISE_NHIS_CLAIM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisClaimActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisClaimActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisClaimCollectionState
					};
				}
