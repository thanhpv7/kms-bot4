...
import {initialReferenceHeaderCollectionState} from './reference_header.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseReferenceHeaderAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ReferenceHeaderModelActionTypes.CREATE_REFERENCE_HEADER_OK:
				const createdModel = (action as modelAction.ReferenceHeaderActionOK ).stateConfig.targetModel;
				...
				ReferenceHeaderModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.CREATE_REFERENCE_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceHeaderModelActionTypes.CREATE_ALL_REFERENCE_HEADER_OK:
				const createdModels = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ReferenceHeaderModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.CREATE_ALL_REFERENCE_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceHeaderModelActionTypes.DELETE_REFERENCE_HEADER_OK:
				const deletedModelId = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.DELETE_ALL_REFERENCE_HEADER_OK:
				const deletedModelIds = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.DELETE_REFERENCE_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceHeaderModelActionTypes.UPDATE_REFERENCE_HEADER_OK:
				const updatedModel = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModel;
				...
				ReferenceHeaderModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.UPDATE_REFERENCE_HEADER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ReferenceHeaderModelActionTypes.UPDATE_ALL_REFERENCE_HEADER_OK:
				const updatedModels = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.UPDATE_ALL_REFERENCE_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceHeaderModelActionTypes.FETCH_REFERENCE_HEADER_OK:
				let fetchedModel = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModel;
				...
				ReferenceHeaderModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.FETCH_ALL_REFERENCE_HEADER_OK:
				const totalCountAfterFetchAll = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ReferenceHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ReferenceHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.FETCH_REFERENCE_HEADER_AUDIT_OK:
				const audits = (action as modelAction.ReferenceHeaderActionOK).audits;
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.FETCH_REFERENCE_HEADER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ReferenceHeaderActionOK).audits;
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.FETCH_REFERENCE_HEADER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ReferenceHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.FETCH_LAST_REFERENCE_HEADER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ReferenceHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ReferenceHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.COUNT_REFERENCE_HEADERS_OK:
				const totalCountAfterCount = (action as modelAction.ReferenceHeaderActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ReferenceHeaderModelActionTypes.INITIALISE_REFERENCE_HEADER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ReferenceHeaderActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ReferenceHeaderActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialReferenceHeaderCollectionState
					};
				}
