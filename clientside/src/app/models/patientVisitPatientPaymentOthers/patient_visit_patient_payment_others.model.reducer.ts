...
import {initialPatientVisitPatientPaymentOthersCollectionState} from './patient_visit_patient_payment_others.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientVisitPatientPaymentOthersAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.CREATE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				const createdModel = (action as modelAction.PatientVisitPatientPaymentOthersActionOK ).stateConfig.targetModel;
				...
				PatientVisitPatientPaymentOthersModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.CREATE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.CREATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				const createdModels = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientVisitPatientPaymentOthersModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.CREATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				const deletedModelId = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.DELETE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				const deletedModelIds = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.UPDATE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				const updatedModel = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModel;
				...
				PatientVisitPatientPaymentOthersModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.UPDATE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.UPDATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				const updatedModels = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.UPDATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				let fetchedModel = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModel;
				...
				PatientVisitPatientPaymentOthersModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_ALL_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientVisitPatientPaymentOthersModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_AUDIT_OK:
				const audits = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).audits;
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).audits;
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientVisitPatientPaymentOthersModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.FETCH_LAST_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientVisitPatientPaymentOthersModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.COUNT_PATIENT_VISIT_PATIENT_PAYMENT_OTHERSS_OK:
				const totalCountAfterCount = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientVisitPatientPaymentOthersModelActionTypes.INITIALISE_PATIENT_VISIT_PATIENT_PAYMENT_OTHERS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientVisitPatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientVisitPatientPaymentOthersCollectionState
					};
				}
