...
import {initialStockCatalogueCollectionState} from './stock_catalogue.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseStockCatalogueAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.StockCatalogueModelActionTypes.CREATE_STOCK_CATALOGUE_OK:
				const createdModel = (action as modelAction.StockCatalogueActionOK ).stateConfig.targetModel;
				...
				StockCatalogueModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.CREATE_STOCK_CATALOGUE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockCatalogueModelActionTypes.CREATE_ALL_STOCK_CATALOGUE_OK:
				const createdModels = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					StockCatalogueModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.CREATE_ALL_STOCK_CATALOGUE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockCatalogueModelActionTypes.DELETE_STOCK_CATALOGUE_OK:
				const deletedModelId = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.DELETE_ALL_STOCK_CATALOGUE_OK:
				const deletedModelIds = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.DELETE_STOCK_CATALOGUE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockCatalogueModelActionTypes.UPDATE_STOCK_CATALOGUE_OK:
				const updatedModel = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModel;
				...
				StockCatalogueModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.UPDATE_STOCK_CATALOGUE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.StockCatalogueModelActionTypes.UPDATE_ALL_STOCK_CATALOGUE_OK:
				const updatedModels = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.UPDATE_ALL_STOCK_CATALOGUE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockCatalogueModelActionTypes.FETCH_STOCK_CATALOGUE_OK:
				let fetchedModel = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModel;
				...
				StockCatalogueModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.FETCH_ALL_STOCK_CATALOGUE_OK:
				const totalCountAfterFetchAll = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.StockCatalogueActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => StockCatalogueModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.FETCH_STOCK_CATALOGUE_AUDIT_OK:
				const audits = (action as modelAction.StockCatalogueActionOK).audits;
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.FETCH_STOCK_CATALOGUE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.StockCatalogueActionOK).audits;
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.FETCH_STOCK_CATALOGUE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => StockCatalogueModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.FETCH_LAST_STOCK_CATALOGUE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.StockCatalogueActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => StockCatalogueModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.COUNT_STOCK_CATALOGUES_OK:
				const totalCountAfterCount = (action as modelAction.StockCatalogueActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.StockCatalogueModelActionTypes.INITIALISE_STOCK_CATALOGUE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.StockCatalogueActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.StockCatalogueActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialStockCatalogueCollectionState
					};
				}
