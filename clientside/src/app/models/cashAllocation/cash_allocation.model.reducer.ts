...
import {initialCashAllocationCollectionState} from './cash_allocation.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCashAllocationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CashAllocationModelActionTypes.CREATE_CASH_ALLOCATION_OK:
				const createdModel = (action as modelAction.CashAllocationActionOK ).stateConfig.targetModel;
				...
				CashAllocationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CashAllocationModelActionTypes.CREATE_CASH_ALLOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashAllocationModelActionTypes.CREATE_ALL_CASH_ALLOCATION_OK:
				const createdModels = (action as modelAction.CashAllocationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CashAllocationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CashAllocationModelActionTypes.CREATE_ALL_CASH_ALLOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashAllocationModelActionTypes.DELETE_CASH_ALLOCATION_OK:
				const deletedModelId = (action as modelAction.CashAllocationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashAllocationModelActionTypes.DELETE_ALL_CASH_ALLOCATION_OK:
				const deletedModelIds = (action as modelAction.CashAllocationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashAllocationModelActionTypes.DELETE_CASH_ALLOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashAllocationModelActionTypes.UPDATE_CASH_ALLOCATION_OK:
				const updatedModel = (action as modelAction.CashAllocationActionOK).stateConfig.targetModel;
				...
				CashAllocationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashAllocationModelActionTypes.UPDATE_CASH_ALLOCATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CashAllocationModelActionTypes.UPDATE_ALL_CASH_ALLOCATION_OK:
				const updatedModels = (action as modelAction.CashAllocationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashAllocationModelActionTypes.UPDATE_ALL_CASH_ALLOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashAllocationModelActionTypes.FETCH_CASH_ALLOCATION_OK:
				let fetchedModel = (action as modelAction.CashAllocationActionOK).stateConfig.targetModel;
				...
				CashAllocationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashAllocationModelActionTypes.FETCH_ALL_CASH_ALLOCATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.CashAllocationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CashAllocationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CashAllocationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CashAllocationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashAllocationModelActionTypes.FETCH_CASH_ALLOCATION_AUDIT_OK:
				const audits = (action as modelAction.CashAllocationActionOK).audits;
				...
				break;
			case modelAction.CashAllocationModelActionTypes.FETCH_CASH_ALLOCATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CashAllocationActionOK).audits;
				...
				break;
			case modelAction.CashAllocationModelActionTypes.FETCH_CASH_ALLOCATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CashAllocationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashAllocationModelActionTypes.FETCH_LAST_CASH_ALLOCATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CashAllocationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CashAllocationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashAllocationModelActionTypes.COUNT_CASH_ALLOCATIONS_OK:
				const totalCountAfterCount = (action as modelAction.CashAllocationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CashAllocationModelActionTypes.INITIALISE_CASH_ALLOCATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CashAllocationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CashAllocationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCashAllocationCollectionState
					};
				}
