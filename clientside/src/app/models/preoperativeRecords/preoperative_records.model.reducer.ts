...
import {initialPreoperativeRecordsCollectionState} from './preoperative_records.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePreoperativeRecordsAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PreoperativeRecordsModelActionTypes.CREATE_PREOPERATIVE_RECORDS_OK:
				const createdModel = (action as modelAction.PreoperativeRecordsActionOK ).stateConfig.targetModel;
				...
				PreoperativeRecordsModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.CREATE_PREOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.CREATE_ALL_PREOPERATIVE_RECORDS_OK:
				const createdModels = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PreoperativeRecordsModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.CREATE_ALL_PREOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.DELETE_PREOPERATIVE_RECORDS_OK:
				const deletedModelId = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.DELETE_ALL_PREOPERATIVE_RECORDS_OK:
				const deletedModelIds = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.DELETE_PREOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.UPDATE_PREOPERATIVE_RECORDS_OK:
				const updatedModel = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModel;
				...
				PreoperativeRecordsModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.UPDATE_PREOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PreoperativeRecordsModelActionTypes.UPDATE_ALL_PREOPERATIVE_RECORDS_OK:
				const updatedModels = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.UPDATE_ALL_PREOPERATIVE_RECORDS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.FETCH_PREOPERATIVE_RECORDS_OK:
				let fetchedModel = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModel;
				...
				PreoperativeRecordsModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.FETCH_ALL_PREOPERATIVE_RECORDS_OK:
				const totalCountAfterFetchAll = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PreoperativeRecordsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.FETCH_PREOPERATIVE_RECORDS_AUDIT_OK:
				const audits = (action as modelAction.PreoperativeRecordsActionOK).audits;
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.FETCH_PREOPERATIVE_RECORDS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PreoperativeRecordsActionOK).audits;
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.FETCH_PREOPERATIVE_RECORDS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PreoperativeRecordsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.FETCH_LAST_PREOPERATIVE_RECORDS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PreoperativeRecordsModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.COUNT_PREOPERATIVE_RECORDSS_OK:
				const totalCountAfterCount = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PreoperativeRecordsModelActionTypes.INITIALISE_PREOPERATIVE_RECORDS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PreoperativeRecordsActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PreoperativeRecordsActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPreoperativeRecordsCollectionState
					};
				}
