...
import {initialReturnSupplierCollectionState} from './return_supplier.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseReturnSupplierAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ReturnSupplierModelActionTypes.CREATE_RETURN_SUPPLIER_OK:
				const createdModel = (action as modelAction.ReturnSupplierActionOK ).stateConfig.targetModel;
				...
				ReturnSupplierModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.CREATE_RETURN_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierModelActionTypes.CREATE_ALL_RETURN_SUPPLIER_OK:
				const createdModels = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ReturnSupplierModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.CREATE_ALL_RETURN_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierModelActionTypes.DELETE_RETURN_SUPPLIER_OK:
				const deletedModelId = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.DELETE_ALL_RETURN_SUPPLIER_OK:
				const deletedModelIds = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.DELETE_RETURN_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierModelActionTypes.UPDATE_RETURN_SUPPLIER_OK:
				const updatedModel = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModel;
				...
				ReturnSupplierModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.UPDATE_RETURN_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ReturnSupplierModelActionTypes.UPDATE_ALL_RETURN_SUPPLIER_OK:
				const updatedModels = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.UPDATE_ALL_RETURN_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierModelActionTypes.FETCH_RETURN_SUPPLIER_OK:
				let fetchedModel = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModel;
				...
				ReturnSupplierModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.FETCH_ALL_RETURN_SUPPLIER_OK:
				const totalCountAfterFetchAll = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ReturnSupplierActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ReturnSupplierModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.FETCH_RETURN_SUPPLIER_AUDIT_OK:
				const audits = (action as modelAction.ReturnSupplierActionOK).audits;
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.FETCH_RETURN_SUPPLIER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ReturnSupplierActionOK).audits;
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.FETCH_RETURN_SUPPLIER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ReturnSupplierModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.FETCH_LAST_RETURN_SUPPLIER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ReturnSupplierModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.COUNT_RETURN_SUPPLIERS_OK:
				const totalCountAfterCount = (action as modelAction.ReturnSupplierActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ReturnSupplierModelActionTypes.INITIALISE_RETURN_SUPPLIER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ReturnSupplierActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ReturnSupplierActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialReturnSupplierCollectionState
					};
				}
