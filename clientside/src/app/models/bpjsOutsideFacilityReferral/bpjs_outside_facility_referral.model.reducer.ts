...
import {initialBpjsOutsideFacilityReferralCollectionState} from './bpjs_outside_facility_referral.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsOutsideFacilityReferralAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.CREATE_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				const createdModel = (action as modelAction.BpjsOutsideFacilityReferralActionOK ).stateConfig.targetModel;
				...
				BpjsOutsideFacilityReferralModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.CREATE_BPJS_OUTSIDE_FACILITY_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.CREATE_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				const createdModels = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsOutsideFacilityReferralModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.CREATE_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.DELETE_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				const deletedModelId = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.DELETE_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				const deletedModelIds = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.DELETE_BPJS_OUTSIDE_FACILITY_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.UPDATE_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				const updatedModel = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModel;
				...
				BpjsOutsideFacilityReferralModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.UPDATE_BPJS_OUTSIDE_FACILITY_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.UPDATE_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				const updatedModels = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.UPDATE_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				let fetchedModel = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModel;
				...
				BpjsOutsideFacilityReferralModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_ALL_BPJS_OUTSIDE_FACILITY_REFERRAL_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsOutsideFacilityReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_BPJS_OUTSIDE_FACILITY_REFERRAL_AUDIT_OK:
				const audits = (action as modelAction.BpjsOutsideFacilityReferralActionOK).audits;
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_BPJS_OUTSIDE_FACILITY_REFERRAL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsOutsideFacilityReferralActionOK).audits;
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_BPJS_OUTSIDE_FACILITY_REFERRAL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsOutsideFacilityReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.FETCH_LAST_BPJS_OUTSIDE_FACILITY_REFERRAL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsOutsideFacilityReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.COUNT_BPJS_OUTSIDE_FACILITY_REFERRALS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsOutsideFacilityReferralModelActionTypes.INITIALISE_BPJS_OUTSIDE_FACILITY_REFERRAL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsOutsideFacilityReferralActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsOutsideFacilityReferralCollectionState
					};
				}
