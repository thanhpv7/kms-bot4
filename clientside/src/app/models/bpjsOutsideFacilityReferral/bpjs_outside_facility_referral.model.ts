 ...
 */
export class BpjsOutsideFacilityReferralModel extends AbstractModel {
	/**
	...
	readonly className = 'BpjsOutsideFacilityReferralModel';
	...
	get valueToDisplay(): string {
		...
		return BpjsOutsideFacilityReferralModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BpjsOutsideFacilityReferralModel.modelPropGroups;
	...
	static getProps(): ModelProperty[] {
		return super.getProps().concat([
			...
			{
				name: 'referredPPKCode',
				// % protected region % [Set displayName for Referred PPK Code here] off begin
				displayName: 'Referred PPK Code',
				// % protected region % [Set displayName for Referred PPK Code here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Referred PPK Code here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Referred PPK Code here] end
				// % protected region % [Set isSensitive for Referred PPK Code here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Referred PPK Code here] end
				// % protected region % [Set readonly for Referred PPK Code here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Referred PPK Code here] end
				validators: [
					// % protected region % [Add other validators for Referred PPK Code here] off begin
					// % protected region % [Add other validators for Referred PPK Code here] end
				],
				// % protected region % [Add any additional model attribute properties for Referred PPK Code here] off begin
				// % protected region % [Add any additional model attribute properties for Referred PPK Code here] end
			},
			{
				name: 'referredPPKName',
				// % protected region % [Set displayName for Referred PPK Name here] off begin
				displayName: 'Referred PPK Name',
				// % protected region % [Set displayName for Referred PPK Name here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Referred PPK Name here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Referred PPK Name here] end
				// % protected region % [Set isSensitive for Referred PPK Name here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Referred PPK Name here] end
				// % protected region % [Set readonly for Referred PPK Name here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Referred PPK Name here] end
				validators: [
					// % protected region % [Add other validators for Referred PPK Name here] off begin
					// % protected region % [Add other validators for Referred PPK Name here] end
				],
				// % protected region % [Add any additional model attribute properties for Referred PPK Name here] off begin
				// % protected region % [Add any additional model attribute properties for Referred PPK Name here] end
			},
			{
				name: 'sepNo',
			...
			{
				name: 'visitPlanDate',
				// % protected region % [Set displayName for Visit Plan Date here] off begin
				displayName: 'Visit Plan Date',
				// % protected region % [Set displayName for Visit Plan Date here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Visit Plan Date here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Visit Plan Date here] end
				// % protected region % [Set isSensitive for Visit Plan Date here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Visit Plan Date here] end
				// % protected region % [Set readonly for Visit Plan Date here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Visit Plan Date here] end
				validators: [
					// % protected region % [Add other validators for Visit Plan Date here] off begin
					// % protected region % [Add other validators for Visit Plan Date here] end
				],
				// % protected region % [Add any additional model attribute properties for Visit Plan Date here] off begin
				// % protected region % [Add any additional model attribute properties for Visit Plan Date here] end
			},
			{
				name: 'diagnose',
			...
			{
				name: 'referralPolyclinic',
				// % protected region % [Set displayName for Referral Polyclinic here] off begin
				displayName: 'Referral Polyclinic',
				// % protected region % [Set displayName for Referral Polyclinic here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Referral Polyclinic here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Referral Polyclinic here] end
				// % protected region % [Set isSensitive for Referral Polyclinic here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Referral Polyclinic here] end
				// % protected region % [Set readonly for Referral Polyclinic here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Referral Polyclinic here] end
				validators: [
					// % protected region % [Add other validators for Referral Polyclinic here] off begin
					// % protected region % [Add other validators for Referral Polyclinic here] end
				],
				// % protected region % [Add any additional model attribute properties for Referral Polyclinic here] off begin
				// % protected region % [Add any additional model attribute properties for Referral Polyclinic here] end
			},
			{
				name: 'gender',
	 ...
	 */
	static deepParse(data: string | { [K in keyof BpjsOutsideFacilityReferralModel]?: BpjsOutsideFacilityReferralModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BpjsOutsideFacilityReferralModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bpjsOutsideFacilityReferralModel = new BpjsOutsideFacilityReferralModel(data);`
	 *
	 * @param data The input data to be initialised as the BpjsOutsideFacilityReferralModel,
	 *    it is expected as a JSON string or as a nullable BpjsOutsideFacilityReferralModel.
	 */
	constructor(data?: string | Partial<BpjsOutsideFacilityReferralModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BpjsOutsideFacilityReferralModel>
				: data;
			...
			this.name = json.name;
			this.referredPPKCode = json.referredPPKCode;
			this.referredPPKName = json.referredPPKName;
			this.sepNo = json.sepNo;
			this.treatmentClass = json.treatmentClass;
			this.sepDate = json.sepDate;
			this.visitPlanDate = json.visitPlanDate;
			this.diagnose = json.diagnose;
			this.referralType = json.referralType;
			this.referralPolyclinic = json.referralPolyclinic;
	...
	toJSON() {
		return {
			...
			name: this.name,
			referredPPKCode: this.referredPPKCode,
			referredPPKName: this.referredPPKName,
			sepNo: this.sepNo,
			treatmentClass: this.treatmentClass,
			sepDate: this.sepDate,
			visitPlanDate: this.visitPlanDate,
			diagnose: this.diagnose,
			referralType: this.referralType,
			referralPolyclinic: this.referralPolyclinic,
			gender: this.gender,
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BpjsOutsideFacilityReferralModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BpjsOutsideFacilityReferralModel {
		...
		if (updates.referredPPKCode) {
			newModelJson.referredPPKCode = updates.referredPPKCode;
		}

		if (updates.referredPPKName) {
			newModelJson.referredPPKName = updates.referredPPKName;
		}
		...
		if (updates.visitPlanDate) {
			newModelJson.visitPlanDate = updates.visitPlanDate;
		}
		...
		if (updates.referralPolyclinic) {
			newModelJson.referralPolyclinic = updates.referralPolyclinic;
		}
		...
		return new BpjsOutsideFacilityReferralModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BpjsOutsideFacilityReferralModel)) {
			return {};
		}
