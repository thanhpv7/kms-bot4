...
import {initialPatientGeneralInfoCollectionState} from './patient_general_info.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientGeneralInfoAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientGeneralInfoModelActionTypes.CREATE_PATIENT_GENERAL_INFO_OK:
				const createdModel = (action as modelAction.PatientGeneralInfoActionOK ).stateConfig.targetModel;
				...
				PatientGeneralInfoModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.CREATE_PATIENT_GENERAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.CREATE_ALL_PATIENT_GENERAL_INFO_OK:
				const createdModels = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientGeneralInfoModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.CREATE_ALL_PATIENT_GENERAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.DELETE_PATIENT_GENERAL_INFO_OK:
				const deletedModelId = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.DELETE_ALL_PATIENT_GENERAL_INFO_OK:
				const deletedModelIds = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.DELETE_PATIENT_GENERAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.UPDATE_PATIENT_GENERAL_INFO_OK:
				const updatedModel = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModel;
				...
				PatientGeneralInfoModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.UPDATE_PATIENT_GENERAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientGeneralInfoModelActionTypes.UPDATE_ALL_PATIENT_GENERAL_INFO_OK:
				const updatedModels = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.UPDATE_ALL_PATIENT_GENERAL_INFO_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.FETCH_PATIENT_GENERAL_INFO_OK:
				let fetchedModel = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModel;
				...
				PatientGeneralInfoModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.FETCH_ALL_PATIENT_GENERAL_INFO_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientGeneralInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.FETCH_PATIENT_GENERAL_INFO_AUDIT_OK:
				const audits = (action as modelAction.PatientGeneralInfoActionOK).audits;
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.FETCH_PATIENT_GENERAL_INFO_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientGeneralInfoActionOK).audits;
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.FETCH_PATIENT_GENERAL_INFO_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientGeneralInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.FETCH_LAST_PATIENT_GENERAL_INFO_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientGeneralInfoModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.COUNT_PATIENT_GENERAL_INFOS_OK:
				const totalCountAfterCount = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientGeneralInfoModelActionTypes.INITIALISE_PATIENT_GENERAL_INFO_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientGeneralInfoActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientGeneralInfoActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientGeneralInfoCollectionState
					};
				}
