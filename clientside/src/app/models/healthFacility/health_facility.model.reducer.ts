...
import {initialHealthFacilityCollectionState} from './health_facility.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseHealthFacilityAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.HealthFacilityModelActionTypes.CREATE_HEALTH_FACILITY_OK:
				const createdModel = (action as modelAction.HealthFacilityActionOK ).stateConfig.targetModel;
				...
				HealthFacilityModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.CREATE_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HealthFacilityModelActionTypes.CREATE_ALL_HEALTH_FACILITY_OK:
				const createdModels = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					HealthFacilityModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.CREATE_ALL_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HealthFacilityModelActionTypes.DELETE_HEALTH_FACILITY_OK:
				const deletedModelId = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.DELETE_ALL_HEALTH_FACILITY_OK:
				const deletedModelIds = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.DELETE_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HealthFacilityModelActionTypes.UPDATE_HEALTH_FACILITY_OK:
				const updatedModel = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModel;
				...
				HealthFacilityModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.UPDATE_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.HealthFacilityModelActionTypes.UPDATE_ALL_HEALTH_FACILITY_OK:
				const updatedModels = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.UPDATE_ALL_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HealthFacilityModelActionTypes.FETCH_HEALTH_FACILITY_OK:
				let fetchedModel = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModel;
				...
				HealthFacilityModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.FETCH_ALL_HEALTH_FACILITY_OK:
				const totalCountAfterFetchAll = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.HealthFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => HealthFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.FETCH_HEALTH_FACILITY_AUDIT_OK:
				const audits = (action as modelAction.HealthFacilityActionOK).audits;
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.FETCH_HEALTH_FACILITY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.HealthFacilityActionOK).audits;
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.FETCH_HEALTH_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => HealthFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.FETCH_LAST_HEALTH_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.HealthFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => HealthFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.COUNT_HEALTH_FACILITYS_OK:
				const totalCountAfterCount = (action as modelAction.HealthFacilityActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.HealthFacilityModelActionTypes.INITIALISE_HEALTH_FACILITY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.HealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.HealthFacilityActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialHealthFacilityCollectionState
					};
				}
