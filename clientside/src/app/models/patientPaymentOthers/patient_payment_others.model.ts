 ...
 */
export class PatientPaymentOthersModel extends AbstractModel {
	/**
	...
	readonly className = 'PatientPaymentOthersModel';
	...
	get valueToDisplay(): string {
		...
		return PatientPaymentOthersModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = PatientPaymentOthersModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof PatientPaymentOthersModel]?: PatientPaymentOthersModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new PatientPaymentOthersModel(data);
		}
	 ...
	 * @example
	 *
	 * `let patientPaymentOthersModel = new PatientPaymentOthersModel(data);`
	 *
	 * @param data The input data to be initialised as the PatientPaymentOthersModel,
	 *    it is expected as a JSON string or as a nullable PatientPaymentOthersModel.
	 */
	constructor(data?: string | Partial<PatientPaymentOthersModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<PatientPaymentOthersModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		PatientPaymentOthersModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): PatientPaymentOthersModel {
		...
		return new PatientPaymentOthersModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof PatientPaymentOthersModel)) {
			return {};
		}
