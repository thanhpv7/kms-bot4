...
import {initialPatientPaymentOthersCollectionState} from './patient_payment_others.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientPaymentOthersAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientPaymentOthersModelActionTypes.CREATE_PATIENT_PAYMENT_OTHERS_OK:
				const createdModel = (action as modelAction.PatientPaymentOthersActionOK ).stateConfig.targetModel;
				...
				PatientPaymentOthersModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.CREATE_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_OTHERS_OK:
				const createdModels = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientPaymentOthersModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.DELETE_PATIENT_PAYMENT_OTHERS_OK:
				const deletedModelId = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_OTHERS_OK:
				const deletedModelIds = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.DELETE_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.UPDATE_PATIENT_PAYMENT_OTHERS_OK:
				const updatedModel = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModel;
				...
				PatientPaymentOthersModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.UPDATE_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientPaymentOthersModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_OTHERS_OK:
				const updatedModels = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_OTHERS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.FETCH_PATIENT_PAYMENT_OTHERS_OK:
				let fetchedModel = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModel;
				...
				PatientPaymentOthersModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.FETCH_ALL_PATIENT_PAYMENT_OTHERS_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientPaymentOthersModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.FETCH_PATIENT_PAYMENT_OTHERS_AUDIT_OK:
				const audits = (action as modelAction.PatientPaymentOthersActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.FETCH_PATIENT_PAYMENT_OTHERS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientPaymentOthersActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.FETCH_PATIENT_PAYMENT_OTHERS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientPaymentOthersModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.FETCH_LAST_PATIENT_PAYMENT_OTHERS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientPaymentOthersModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.COUNT_PATIENT_PAYMENT_OTHERSS_OK:
				const totalCountAfterCount = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientPaymentOthersModelActionTypes.INITIALISE_PATIENT_PAYMENT_OTHERS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientPaymentOthersActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientPaymentOthersActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientPaymentOthersCollectionState
					};
				}
