 ...
 */
export class MedicalFeeItemComponentModel extends AbstractModel {
	/**
	...
	readonly className = 'MedicalFeeItemComponentModel';
	...
	get valueToDisplay(): string {
		...
		return MedicalFeeItemComponentModel.displayAttributes.map((attr) => this[attr]).join(' ');
		// % protected region % [Change displayName here if needed] end
	}

	/**
	 * Price of Component (2 digits decimal).
	 */
	price: number;
	...
	modelPropGroups: { [s: string]: Group } = MedicalFeeItemComponentModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof MedicalFeeItemComponentModel]?: MedicalFeeItemComponentModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new MedicalFeeItemComponentModel(data);
		}
	 ...
	 * @example
	 *
	 * `let medicalFeeItemComponentModel = new MedicalFeeItemComponentModel(data);`
	 *
	 * @param data The input data to be initialised as the MedicalFeeItemComponentModel,
	 *    it is expected as a JSON string or as a nullable MedicalFeeItemComponentModel.
	 */
	constructor(data?: string | Partial<MedicalFeeItemComponentModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<MedicalFeeItemComponentModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		MedicalFeeItemComponentModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): MedicalFeeItemComponentModel {
		...
		return new MedicalFeeItemComponentModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof MedicalFeeItemComponentModel)) {
			return {};
		}
