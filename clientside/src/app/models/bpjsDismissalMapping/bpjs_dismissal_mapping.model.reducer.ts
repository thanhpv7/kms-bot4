...
import {initialBpjsDismissalMappingCollectionState} from './bpjs_dismissal_mapping.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsDismissalMappingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsDismissalMappingModelActionTypes.CREATE_BPJS_DISMISSAL_MAPPING_OK:
				const createdModel = (action as modelAction.BpjsDismissalMappingActionOK ).stateConfig.targetModel;
				...
				BpjsDismissalMappingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.CREATE_BPJS_DISMISSAL_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.CREATE_ALL_BPJS_DISMISSAL_MAPPING_OK:
				const createdModels = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsDismissalMappingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.CREATE_ALL_BPJS_DISMISSAL_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.DELETE_BPJS_DISMISSAL_MAPPING_OK:
				const deletedModelId = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.DELETE_ALL_BPJS_DISMISSAL_MAPPING_OK:
				const deletedModelIds = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.DELETE_BPJS_DISMISSAL_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.UPDATE_BPJS_DISMISSAL_MAPPING_OK:
				const updatedModel = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModel;
				...
				BpjsDismissalMappingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.UPDATE_BPJS_DISMISSAL_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsDismissalMappingModelActionTypes.UPDATE_ALL_BPJS_DISMISSAL_MAPPING_OK:
				const updatedModels = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.UPDATE_ALL_BPJS_DISMISSAL_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.FETCH_BPJS_DISMISSAL_MAPPING_OK:
				let fetchedModel = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModel;
				...
				BpjsDismissalMappingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.FETCH_ALL_BPJS_DISMISSAL_MAPPING_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsDismissalMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.FETCH_BPJS_DISMISSAL_MAPPING_AUDIT_OK:
				const audits = (action as modelAction.BpjsDismissalMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.FETCH_BPJS_DISMISSAL_MAPPING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsDismissalMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.FETCH_BPJS_DISMISSAL_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsDismissalMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.FETCH_LAST_BPJS_DISMISSAL_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsDismissalMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.COUNT_BPJS_DISMISSAL_MAPPINGS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsDismissalMappingModelActionTypes.INITIALISE_BPJS_DISMISSAL_MAPPING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsDismissalMappingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsDismissalMappingCollectionState
					};
				}
