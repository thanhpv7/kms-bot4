...
import {initialBedFacilityCollectionState} from './bed_facility.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBedFacilityAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BedFacilityModelActionTypes.CREATE_BED_FACILITY_OK:
				const createdModel = (action as modelAction.BedFacilityActionOK ).stateConfig.targetModel;
				...
				BedFacilityModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BedFacilityModelActionTypes.CREATE_BED_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedFacilityModelActionTypes.CREATE_ALL_BED_FACILITY_OK:
				const createdModels = (action as modelAction.BedFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BedFacilityModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BedFacilityModelActionTypes.CREATE_ALL_BED_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedFacilityModelActionTypes.DELETE_BED_FACILITY_OK:
				const deletedModelId = (action as modelAction.BedFacilityActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BedFacilityModelActionTypes.DELETE_ALL_BED_FACILITY_OK:
				const deletedModelIds = (action as modelAction.BedFacilityActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BedFacilityModelActionTypes.DELETE_BED_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedFacilityModelActionTypes.UPDATE_BED_FACILITY_OK:
				const updatedModel = (action as modelAction.BedFacilityActionOK).stateConfig.targetModel;
				...
				BedFacilityModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedFacilityModelActionTypes.UPDATE_BED_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BedFacilityModelActionTypes.UPDATE_ALL_BED_FACILITY_OK:
				const updatedModels = (action as modelAction.BedFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BedFacilityModelActionTypes.UPDATE_ALL_BED_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedFacilityModelActionTypes.FETCH_BED_FACILITY_OK:
				let fetchedModel = (action as modelAction.BedFacilityActionOK).stateConfig.targetModel;
				...
				BedFacilityModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedFacilityModelActionTypes.FETCH_ALL_BED_FACILITY_OK:
				const totalCountAfterFetchAll = (action as modelAction.BedFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BedFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BedFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BedFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedFacilityModelActionTypes.FETCH_BED_FACILITY_AUDIT_OK:
				const audits = (action as modelAction.BedFacilityActionOK).audits;
				...
				break;
			case modelAction.BedFacilityModelActionTypes.FETCH_BED_FACILITY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BedFacilityActionOK).audits;
				...
				break;
			case modelAction.BedFacilityModelActionTypes.FETCH_BED_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BedFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedFacilityModelActionTypes.FETCH_LAST_BED_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BedFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BedFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedFacilityModelActionTypes.COUNT_BED_FACILITYS_OK:
				const totalCountAfterCount = (action as modelAction.BedFacilityActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BedFacilityModelActionTypes.INITIALISE_BED_FACILITY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BedFacilityActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BedFacilityActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBedFacilityCollectionState
					};
				}
