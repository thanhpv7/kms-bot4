...
import {initialMedicationItemCollectionState} from './medication_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicationItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicationItemModelActionTypes.CREATE_MEDICATION_ITEM_OK:
				const createdModel = (action as modelAction.MedicationItemActionOK ).stateConfig.targetModel;
				...
				MedicationItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicationItemModelActionTypes.CREATE_MEDICATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationItemModelActionTypes.CREATE_ALL_MEDICATION_ITEM_OK:
				const createdModels = (action as modelAction.MedicationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicationItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicationItemModelActionTypes.CREATE_ALL_MEDICATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationItemModelActionTypes.DELETE_MEDICATION_ITEM_OK:
				const deletedModelId = (action as modelAction.MedicationItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationItemModelActionTypes.DELETE_ALL_MEDICATION_ITEM_OK:
				const deletedModelIds = (action as modelAction.MedicationItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationItemModelActionTypes.DELETE_MEDICATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationItemModelActionTypes.UPDATE_MEDICATION_ITEM_OK:
				const updatedModel = (action as modelAction.MedicationItemActionOK).stateConfig.targetModel;
				...
				MedicationItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationItemModelActionTypes.UPDATE_MEDICATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicationItemModelActionTypes.UPDATE_ALL_MEDICATION_ITEM_OK:
				const updatedModels = (action as modelAction.MedicationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationItemModelActionTypes.UPDATE_ALL_MEDICATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationItemModelActionTypes.FETCH_MEDICATION_ITEM_OK:
				let fetchedModel = (action as modelAction.MedicationItemActionOK).stateConfig.targetModel;
				...
				MedicationItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationItemModelActionTypes.FETCH_ALL_MEDICATION_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicationItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicationItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicationItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationItemModelActionTypes.FETCH_MEDICATION_ITEM_AUDIT_OK:
				const audits = (action as modelAction.MedicationItemActionOK).audits;
				...
				break;
			case modelAction.MedicationItemModelActionTypes.FETCH_MEDICATION_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicationItemActionOK).audits;
				...
				break;
			case modelAction.MedicationItemModelActionTypes.FETCH_MEDICATION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicationItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationItemModelActionTypes.FETCH_LAST_MEDICATION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicationItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicationItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationItemModelActionTypes.COUNT_MEDICATION_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.MedicationItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicationItemModelActionTypes.INITIALISE_MEDICATION_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicationItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicationItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicationItemCollectionState
					};
				}
