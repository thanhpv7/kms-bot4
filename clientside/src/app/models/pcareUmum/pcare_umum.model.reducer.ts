...
import {initialPcareUmumCollectionState} from './pcare_umum.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareUmumAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareUmumModelActionTypes.CREATE_PCARE_UMUM_OK:
				const createdModel = (action as modelAction.PcareUmumActionOK ).stateConfig.targetModel;
				...
				PcareUmumModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareUmumModelActionTypes.CREATE_PCARE_UMUM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareUmumModelActionTypes.CREATE_ALL_PCARE_UMUM_OK:
				const createdModels = (action as modelAction.PcareUmumActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareUmumModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareUmumModelActionTypes.CREATE_ALL_PCARE_UMUM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareUmumModelActionTypes.DELETE_PCARE_UMUM_OK:
				const deletedModelId = (action as modelAction.PcareUmumActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareUmumModelActionTypes.DELETE_ALL_PCARE_UMUM_OK:
				const deletedModelIds = (action as modelAction.PcareUmumActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareUmumModelActionTypes.DELETE_PCARE_UMUM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareUmumModelActionTypes.UPDATE_PCARE_UMUM_OK:
				const updatedModel = (action as modelAction.PcareUmumActionOK).stateConfig.targetModel;
				...
				PcareUmumModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareUmumModelActionTypes.UPDATE_PCARE_UMUM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareUmumModelActionTypes.UPDATE_ALL_PCARE_UMUM_OK:
				const updatedModels = (action as modelAction.PcareUmumActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareUmumModelActionTypes.UPDATE_ALL_PCARE_UMUM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareUmumModelActionTypes.FETCH_PCARE_UMUM_OK:
				let fetchedModel = (action as modelAction.PcareUmumActionOK).stateConfig.targetModel;
				...
				PcareUmumModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareUmumModelActionTypes.FETCH_ALL_PCARE_UMUM_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareUmumActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareUmumActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareUmumActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareUmumModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareUmumModelActionTypes.FETCH_PCARE_UMUM_AUDIT_OK:
				const audits = (action as modelAction.PcareUmumActionOK).audits;
				...
				break;
			case modelAction.PcareUmumModelActionTypes.FETCH_PCARE_UMUM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareUmumActionOK).audits;
				...
				break;
			case modelAction.PcareUmumModelActionTypes.FETCH_PCARE_UMUM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareUmumModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareUmumModelActionTypes.FETCH_LAST_PCARE_UMUM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareUmumActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareUmumModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareUmumModelActionTypes.COUNT_PCARE_UMUMS_OK:
				const totalCountAfterCount = (action as modelAction.PcareUmumActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareUmumModelActionTypes.INITIALISE_PCARE_UMUM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareUmumActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareUmumActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareUmumCollectionState
					};
				}
