 ...
 */
export class PatientVisitPatientPaymentInsuranceModel extends AbstractModel {
	/**
	...
	readonly className = 'PatientVisitPatientPaymentInsuranceModel';
	...
	get valueToDisplay(): string {
		...
		return PatientVisitPatientPaymentInsuranceModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	/**
	 * Coverage Balance, with limitation 2 decimal digits.
	 */
	...
	modelPropGroups: { [s: string]: Group } = PatientVisitPatientPaymentInsuranceModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof PatientVisitPatientPaymentInsuranceModel]?: PatientVisitPatientPaymentInsuranceModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new PatientVisitPatientPaymentInsuranceModel(data);
		}
	 ...
	 * @example
	 *
	 * `let patientVisitPatientPaymentInsuranceModel = new PatientVisitPatientPaymentInsuranceModel(data);`
	 *
	 * @param data The input data to be initialised as the PatientVisitPatientPaymentInsuranceModel,
	 *    it is expected as a JSON string or as a nullable PatientVisitPatientPaymentInsuranceModel.
	 */
	constructor(data?: string | Partial<PatientVisitPatientPaymentInsuranceModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<PatientVisitPatientPaymentInsuranceModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		PatientVisitPatientPaymentInsuranceModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): PatientVisitPatientPaymentInsuranceModel {
		...
		return new PatientVisitPatientPaymentInsuranceModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof PatientVisitPatientPaymentInsuranceModel)) {
			return {};
		}
