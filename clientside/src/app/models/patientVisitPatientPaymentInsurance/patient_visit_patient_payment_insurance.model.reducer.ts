...
import {initialPatientVisitPatientPaymentInsuranceCollectionState} from './patient_visit_patient_payment_insurance.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientVisitPatientPaymentInsuranceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.CREATE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				const createdModel = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK ).stateConfig.targetModel;
				...
				PatientVisitPatientPaymentInsuranceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.CREATE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.CREATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				const createdModels = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientVisitPatientPaymentInsuranceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.CREATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				const deletedModelId = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.DELETE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				const deletedModelIds = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.DELETE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.UPDATE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				const updatedModel = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModel;
				...
				PatientVisitPatientPaymentInsuranceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.UPDATE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.UPDATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				const updatedModels = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.UPDATE_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				let fetchedModel = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModel;
				...
				PatientVisitPatientPaymentInsuranceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_ALL_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientVisitPatientPaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_AUDIT_OK:
				const audits = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).audits;
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).audits;
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientVisitPatientPaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.FETCH_LAST_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientVisitPatientPaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.COUNT_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCES_OK:
				const totalCountAfterCount = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientVisitPatientPaymentInsuranceModelActionTypes.INITIALISE_PATIENT_VISIT_PATIENT_PAYMENT_INSURANCE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientVisitPatientPaymentInsuranceActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientVisitPatientPaymentInsuranceCollectionState
					};
				}
