...
import * as modelAction from './journal.model.action';
import {ModelState} from '../model.state';
import {initialJournalCollectionState} from './journal.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseJournalAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.JournalModelActionTypes.CREATE_JOURNAL_OK:
				const createdModel = (action as modelAction.JournalActionOK ).stateConfig.targetModel;
				...
				JournalModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.JournalModelActionTypes.CREATE_JOURNAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.JournalModelActionTypes.CREATE_ALL_JOURNAL_OK:
				const createdModels = (action as modelAction.JournalActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.JournalActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					JournalModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.JournalModelActionTypes.CREATE_ALL_JOURNAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.JournalModelActionTypes.DELETE_JOURNAL_OK:
				const deletedModelId = (action as modelAction.JournalActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.JournalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.JournalModelActionTypes.DELETE_ALL_JOURNAL_OK:
				const deletedModelIds = (action as modelAction.JournalActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.JournalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.JournalModelActionTypes.DELETE_JOURNAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.JournalModelActionTypes.UPDATE_JOURNAL_OK:
				const updatedModel = (action as modelAction.JournalActionOK).stateConfig.targetModel;
				...
				JournalModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.JournalModelActionTypes.UPDATE_JOURNAL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.JournalModelActionTypes.UPDATE_ALL_JOURNAL_OK:
				const updatedModels = (action as modelAction.JournalActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.JournalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.JournalModelActionTypes.UPDATE_ALL_JOURNAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.JournalModelActionTypes.FETCH_JOURNAL_OK:
				let fetchedModel = (action as modelAction.JournalActionOK).stateConfig.targetModel;
				...
				JournalModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.JournalModelActionTypes.FETCH_ALL_JOURNAL_OK:
				const totalCountAfterFetchAll = (action as modelAction.JournalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.JournalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.JournalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.JournalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => JournalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.JournalModelActionTypes.FETCH_JOURNAL_AUDIT_OK:
				const audits = (action as modelAction.JournalActionOK).audits;
				...
				break;
			case modelAction.JournalModelActionTypes.FETCH_JOURNAL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.JournalActionOK).audits;
				...
				break;
			case modelAction.JournalModelActionTypes.FETCH_JOURNAL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.JournalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.JournalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.JournalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.JournalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => JournalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.JournalModelActionTypes.FETCH_LAST_JOURNAL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.JournalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.JournalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.JournalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.JournalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => JournalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.JournalModelActionTypes.COUNT_JOURNALS_OK:
				const totalCountAfterCount = (action as modelAction.JournalActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.JournalModelActionTypes.INITIALISE_JOURNAL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.JournalActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.JournalActionOK).stateConfig.queryParams;
