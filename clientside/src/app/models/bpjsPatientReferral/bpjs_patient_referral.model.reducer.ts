...
import {initialBpjsPatientReferralCollectionState} from './bpjs_patient_referral.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPatientReferralAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPatientReferralModelActionTypes.CREATE_BPJS_PATIENT_REFERRAL_OK:
				const createdModel = (action as modelAction.BpjsPatientReferralActionOK ).stateConfig.targetModel;
				...
				BpjsPatientReferralModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.CREATE_BPJS_PATIENT_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.CREATE_ALL_BPJS_PATIENT_REFERRAL_OK:
				const createdModels = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPatientReferralModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.CREATE_ALL_BPJS_PATIENT_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.DELETE_BPJS_PATIENT_REFERRAL_OK:
				const deletedModelId = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.DELETE_ALL_BPJS_PATIENT_REFERRAL_OK:
				const deletedModelIds = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.DELETE_BPJS_PATIENT_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.UPDATE_BPJS_PATIENT_REFERRAL_OK:
				const updatedModel = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModel;
				...
				BpjsPatientReferralModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.UPDATE_BPJS_PATIENT_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPatientReferralModelActionTypes.UPDATE_ALL_BPJS_PATIENT_REFERRAL_OK:
				const updatedModels = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.UPDATE_ALL_BPJS_PATIENT_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.FETCH_BPJS_PATIENT_REFERRAL_OK:
				let fetchedModel = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModel;
				...
				BpjsPatientReferralModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.FETCH_ALL_BPJS_PATIENT_REFERRAL_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPatientReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.FETCH_BPJS_PATIENT_REFERRAL_AUDIT_OK:
				const audits = (action as modelAction.BpjsPatientReferralActionOK).audits;
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.FETCH_BPJS_PATIENT_REFERRAL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPatientReferralActionOK).audits;
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.FETCH_BPJS_PATIENT_REFERRAL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPatientReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.FETCH_LAST_BPJS_PATIENT_REFERRAL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPatientReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.COUNT_BPJS_PATIENT_REFERRALS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPatientReferralModelActionTypes.INITIALISE_BPJS_PATIENT_REFERRAL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPatientReferralActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPatientReferralActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPatientReferralCollectionState
					};
				}
