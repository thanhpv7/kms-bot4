...
import {initialGoodsReceiveNoteCollectionState} from './goods_receive_note.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseGoodsReceiveNoteAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.GoodsReceiveNoteModelActionTypes.CREATE_GOODS_RECEIVE_NOTE_OK:
				const createdModel = (action as modelAction.GoodsReceiveNoteActionOK ).stateConfig.targetModel;
				...
				GoodsReceiveNoteModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.CREATE_GOODS_RECEIVE_NOTE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.CREATE_ALL_GOODS_RECEIVE_NOTE_OK:
				const createdModels = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					GoodsReceiveNoteModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.CREATE_ALL_GOODS_RECEIVE_NOTE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_OK:
				const deletedModelId = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.DELETE_ALL_GOODS_RECEIVE_NOTE_OK:
				const deletedModelIds = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE_OK:
				const updatedModel = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModel;
				...
				GoodsReceiveNoteModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.GoodsReceiveNoteModelActionTypes.UPDATE_ALL_GOODS_RECEIVE_NOTE_OK:
				const updatedModels = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.UPDATE_ALL_GOODS_RECEIVE_NOTE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_OK:
				let fetchedModel = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModel;
				...
				GoodsReceiveNoteModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.FETCH_ALL_GOODS_RECEIVE_NOTE_OK:
				const totalCountAfterFetchAll = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => GoodsReceiveNoteModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_AUDIT_OK:
				const audits = (action as modelAction.GoodsReceiveNoteActionOK).audits;
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.GoodsReceiveNoteActionOK).audits;
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => GoodsReceiveNoteModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.FETCH_LAST_GOODS_RECEIVE_NOTE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => GoodsReceiveNoteModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.COUNT_GOODS_RECEIVE_NOTES_OK:
				const totalCountAfterCount = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.GoodsReceiveNoteModelActionTypes.INITIALISE_GOODS_RECEIVE_NOTE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.GoodsReceiveNoteActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialGoodsReceiveNoteCollectionState
					};
				}
