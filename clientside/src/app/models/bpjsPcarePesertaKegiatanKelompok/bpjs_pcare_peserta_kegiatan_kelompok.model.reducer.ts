...
import {initialBpjsPcarePesertaKegiatanKelompokCollectionState} from './bpjs_pcare_peserta_kegiatan_kelompok.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcarePesertaKegiatanKelompokAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.CREATE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				const createdModel = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK ).stateConfig.targetModel;
				...
				BpjsPcarePesertaKegiatanKelompokModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.CREATE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.CREATE_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				const createdModels = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcarePesertaKegiatanKelompokModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.CREATE_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				const deletedModelId = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.DELETE_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				const deletedModelIds = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.DELETE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.UPDATE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				const updatedModel = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModel;
				...
				BpjsPcarePesertaKegiatanKelompokModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.UPDATE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.UPDATE_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				const updatedModels = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.UPDATE_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				let fetchedModel = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModel;
				...
				BpjsPcarePesertaKegiatanKelompokModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_ALL_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcarePesertaKegiatanKelompokModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).audits;
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).audits;
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcarePesertaKegiatanKelompokModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.FETCH_LAST_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcarePesertaKegiatanKelompokModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.COUNT_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOKS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcarePesertaKegiatanKelompokModelActionTypes.INITIALISE_BPJS_PCARE_PESERTA_KEGIATAN_KELOMPOK_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcarePesertaKegiatanKelompokActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcarePesertaKegiatanKelompokCollectionState
					};
				}
