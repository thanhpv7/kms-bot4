 ...
 */
export class MedicalCertificateSickLeaveModel extends AbstractModel {
	/**
	...
	readonly className = 'MedicalCertificateSickLeaveModel';
	...
	get valueToDisplay(): string {
		...
		return MedicalCertificateSickLeaveModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = MedicalCertificateSickLeaveModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof MedicalCertificateSickLeaveModel]?: MedicalCertificateSickLeaveModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new MedicalCertificateSickLeaveModel(data);
		}
	 ...
	 * @example
	 *
	 * `let medicalCertificateSickLeaveModel = new MedicalCertificateSickLeaveModel(data);`
	 *
	 * @param data The input data to be initialised as the MedicalCertificateSickLeaveModel,
	 *    it is expected as a JSON string or as a nullable MedicalCertificateSickLeaveModel.
	 */
	constructor(data?: string | Partial<MedicalCertificateSickLeaveModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<MedicalCertificateSickLeaveModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		MedicalCertificateSickLeaveModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): MedicalCertificateSickLeaveModel {
		...
		return new MedicalCertificateSickLeaveModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof MedicalCertificateSickLeaveModel)) {
			return {};
		}
