...
import {initialSupplierBankAccountCollectionState} from './supplier_bank_account.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSupplierBankAccountAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SupplierBankAccountModelActionTypes.CREATE_SUPPLIER_BANK_ACCOUNT_OK:
				const createdModel = (action as modelAction.SupplierBankAccountActionOK ).stateConfig.targetModel;
				...
				SupplierBankAccountModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.CREATE_SUPPLIER_BANK_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierBankAccountModelActionTypes.CREATE_ALL_SUPPLIER_BANK_ACCOUNT_OK:
				const createdModels = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SupplierBankAccountModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.CREATE_ALL_SUPPLIER_BANK_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierBankAccountModelActionTypes.DELETE_SUPPLIER_BANK_ACCOUNT_OK:
				const deletedModelId = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.DELETE_ALL_SUPPLIER_BANK_ACCOUNT_OK:
				const deletedModelIds = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.DELETE_SUPPLIER_BANK_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierBankAccountModelActionTypes.UPDATE_SUPPLIER_BANK_ACCOUNT_OK:
				const updatedModel = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModel;
				...
				SupplierBankAccountModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.UPDATE_SUPPLIER_BANK_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SupplierBankAccountModelActionTypes.UPDATE_ALL_SUPPLIER_BANK_ACCOUNT_OK:
				const updatedModels = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.UPDATE_ALL_SUPPLIER_BANK_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierBankAccountModelActionTypes.FETCH_SUPPLIER_BANK_ACCOUNT_OK:
				let fetchedModel = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModel;
				...
				SupplierBankAccountModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.FETCH_ALL_SUPPLIER_BANK_ACCOUNT_OK:
				const totalCountAfterFetchAll = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SupplierBankAccountActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SupplierBankAccountModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.FETCH_SUPPLIER_BANK_ACCOUNT_AUDIT_OK:
				const audits = (action as modelAction.SupplierBankAccountActionOK).audits;
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.FETCH_SUPPLIER_BANK_ACCOUNT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SupplierBankAccountActionOK).audits;
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.FETCH_SUPPLIER_BANK_ACCOUNT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SupplierBankAccountModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.FETCH_LAST_SUPPLIER_BANK_ACCOUNT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SupplierBankAccountActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SupplierBankAccountModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.COUNT_SUPPLIER_BANK_ACCOUNTS_OK:
				const totalCountAfterCount = (action as modelAction.SupplierBankAccountActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SupplierBankAccountModelActionTypes.INITIALISE_SUPPLIER_BANK_ACCOUNT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SupplierBankAccountActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SupplierBankAccountActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialSupplierBankAccountCollectionState
					};
				}
