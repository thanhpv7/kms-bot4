...
import {initialBpjsSearchControlLetterCollectionState} from './bpjs_search_control_letter.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsSearchControlLetterAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsSearchControlLetterModelActionTypes.CREATE_BPJS_SEARCH_CONTROL_LETTER_OK:
				const createdModel = (action as modelAction.BpjsSearchControlLetterActionOK ).stateConfig.targetModel;
				...
				BpjsSearchControlLetterModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.CREATE_BPJS_SEARCH_CONTROL_LETTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.CREATE_ALL_BPJS_SEARCH_CONTROL_LETTER_OK:
				const createdModels = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsSearchControlLetterModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.CREATE_ALL_BPJS_SEARCH_CONTROL_LETTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.DELETE_BPJS_SEARCH_CONTROL_LETTER_OK:
				const deletedModelId = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.DELETE_ALL_BPJS_SEARCH_CONTROL_LETTER_OK:
				const deletedModelIds = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.DELETE_BPJS_SEARCH_CONTROL_LETTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.UPDATE_BPJS_SEARCH_CONTROL_LETTER_OK:
				const updatedModel = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModel;
				...
				BpjsSearchControlLetterModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.UPDATE_BPJS_SEARCH_CONTROL_LETTER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsSearchControlLetterModelActionTypes.UPDATE_ALL_BPJS_SEARCH_CONTROL_LETTER_OK:
				const updatedModels = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.UPDATE_ALL_BPJS_SEARCH_CONTROL_LETTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_BPJS_SEARCH_CONTROL_LETTER_OK:
				let fetchedModel = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModel;
				...
				BpjsSearchControlLetterModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_ALL_BPJS_SEARCH_CONTROL_LETTER_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsSearchControlLetterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_BPJS_SEARCH_CONTROL_LETTER_AUDIT_OK:
				const audits = (action as modelAction.BpjsSearchControlLetterActionOK).audits;
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_BPJS_SEARCH_CONTROL_LETTER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsSearchControlLetterActionOK).audits;
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_BPJS_SEARCH_CONTROL_LETTER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsSearchControlLetterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.FETCH_LAST_BPJS_SEARCH_CONTROL_LETTER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsSearchControlLetterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.COUNT_BPJS_SEARCH_CONTROL_LETTERS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsSearchControlLetterModelActionTypes.INITIALISE_BPJS_SEARCH_CONTROL_LETTER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsSearchControlLetterActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsSearchControlLetterCollectionState
					};
				}
