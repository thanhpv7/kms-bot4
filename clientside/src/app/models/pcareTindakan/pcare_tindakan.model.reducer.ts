...
import {initialPcareTindakanCollectionState} from './pcare_tindakan.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareTindakanAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareTindakanModelActionTypes.CREATE_PCARE_TINDAKAN_OK:
				const createdModel = (action as modelAction.PcareTindakanActionOK ).stateConfig.targetModel;
				...
				PcareTindakanModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.CREATE_PCARE_TINDAKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareTindakanModelActionTypes.CREATE_ALL_PCARE_TINDAKAN_OK:
				const createdModels = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareTindakanModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.CREATE_ALL_PCARE_TINDAKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareTindakanModelActionTypes.DELETE_PCARE_TINDAKAN_OK:
				const deletedModelId = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.DELETE_ALL_PCARE_TINDAKAN_OK:
				const deletedModelIds = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.DELETE_PCARE_TINDAKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareTindakanModelActionTypes.UPDATE_PCARE_TINDAKAN_OK:
				const updatedModel = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModel;
				...
				PcareTindakanModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.UPDATE_PCARE_TINDAKAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareTindakanModelActionTypes.UPDATE_ALL_PCARE_TINDAKAN_OK:
				const updatedModels = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.UPDATE_ALL_PCARE_TINDAKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareTindakanModelActionTypes.FETCH_PCARE_TINDAKAN_OK:
				let fetchedModel = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModel;
				...
				PcareTindakanModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.FETCH_ALL_PCARE_TINDAKAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareTindakanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareTindakanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.FETCH_PCARE_TINDAKAN_AUDIT_OK:
				const audits = (action as modelAction.PcareTindakanActionOK).audits;
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.FETCH_PCARE_TINDAKAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareTindakanActionOK).audits;
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.FETCH_PCARE_TINDAKAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareTindakanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.FETCH_LAST_PCARE_TINDAKAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareTindakanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareTindakanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.COUNT_PCARE_TINDAKANS_OK:
				const totalCountAfterCount = (action as modelAction.PcareTindakanActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareTindakanModelActionTypes.INITIALISE_PCARE_TINDAKAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareTindakanActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareTindakanActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareTindakanCollectionState
					};
				}
