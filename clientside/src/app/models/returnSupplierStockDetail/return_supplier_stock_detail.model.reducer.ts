...
import {initialReturnSupplierStockDetailCollectionState} from './return_supplier_stock_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseReturnSupplierStockDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ReturnSupplierStockDetailModelActionTypes.CREATE_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				const createdModel = (action as modelAction.ReturnSupplierStockDetailActionOK ).stateConfig.targetModel;
				...
				ReturnSupplierStockDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.CREATE_RETURN_SUPPLIER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.CREATE_ALL_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				const createdModels = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ReturnSupplierStockDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.CREATE_ALL_RETURN_SUPPLIER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.DELETE_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				const deletedModelId = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.DELETE_ALL_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				const deletedModelIds = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.DELETE_RETURN_SUPPLIER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.UPDATE_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				const updatedModel = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModel;
				...
				ReturnSupplierStockDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.UPDATE_RETURN_SUPPLIER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ReturnSupplierStockDetailModelActionTypes.UPDATE_ALL_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				const updatedModels = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.UPDATE_ALL_RETURN_SUPPLIER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				let fetchedModel = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModel;
				...
				ReturnSupplierStockDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_ALL_RETURN_SUPPLIER_STOCK_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ReturnSupplierStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_RETURN_SUPPLIER_STOCK_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.ReturnSupplierStockDetailActionOK).audits;
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_RETURN_SUPPLIER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ReturnSupplierStockDetailActionOK).audits;
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_RETURN_SUPPLIER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ReturnSupplierStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.FETCH_LAST_RETURN_SUPPLIER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ReturnSupplierStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.COUNT_RETURN_SUPPLIER_STOCK_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ReturnSupplierStockDetailModelActionTypes.INITIALISE_RETURN_SUPPLIER_STOCK_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ReturnSupplierStockDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialReturnSupplierStockDetailCollectionState
					};
				}
