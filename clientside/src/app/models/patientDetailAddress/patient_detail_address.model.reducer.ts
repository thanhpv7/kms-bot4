...
import {initialPatientDetailAddressCollectionState} from './patient_detail_address.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientDetailAddressAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientDetailAddressModelActionTypes.CREATE_PATIENT_DETAIL_ADDRESS_OK:
				const createdModel = (action as modelAction.PatientDetailAddressActionOK ).stateConfig.targetModel;
				...
				PatientDetailAddressModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.CREATE_PATIENT_DETAIL_ADDRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailAddressModelActionTypes.CREATE_ALL_PATIENT_DETAIL_ADDRESS_OK:
				const createdModels = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientDetailAddressModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.CREATE_ALL_PATIENT_DETAIL_ADDRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailAddressModelActionTypes.DELETE_PATIENT_DETAIL_ADDRESS_OK:
				const deletedModelId = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.DELETE_ALL_PATIENT_DETAIL_ADDRESS_OK:
				const deletedModelIds = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.DELETE_PATIENT_DETAIL_ADDRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailAddressModelActionTypes.UPDATE_PATIENT_DETAIL_ADDRESS_OK:
				const updatedModel = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModel;
				...
				PatientDetailAddressModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.UPDATE_PATIENT_DETAIL_ADDRESS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientDetailAddressModelActionTypes.UPDATE_ALL_PATIENT_DETAIL_ADDRESS_OK:
				const updatedModels = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.UPDATE_ALL_PATIENT_DETAIL_ADDRESS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailAddressModelActionTypes.FETCH_PATIENT_DETAIL_ADDRESS_OK:
				let fetchedModel = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModel;
				...
				PatientDetailAddressModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.FETCH_ALL_PATIENT_DETAIL_ADDRESS_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientDetailAddressActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientDetailAddressModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.FETCH_PATIENT_DETAIL_ADDRESS_AUDIT_OK:
				const audits = (action as modelAction.PatientDetailAddressActionOK).audits;
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.FETCH_PATIENT_DETAIL_ADDRESS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientDetailAddressActionOK).audits;
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.FETCH_PATIENT_DETAIL_ADDRESS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientDetailAddressModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.FETCH_LAST_PATIENT_DETAIL_ADDRESS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientDetailAddressActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientDetailAddressModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.COUNT_PATIENT_DETAIL_ADDRESSS_OK:
				const totalCountAfterCount = (action as modelAction.PatientDetailAddressActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientDetailAddressModelActionTypes.INITIALISE_PATIENT_DETAIL_ADDRESS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientDetailAddressActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientDetailAddressActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientDetailAddressCollectionState
					};
				}
