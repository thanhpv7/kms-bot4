...
import {initialStockTransactionDetailCollectionState} from './stock_transaction_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseStockTransactionDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.StockTransactionDetailModelActionTypes.CREATE_STOCK_TRANSACTION_DETAIL_OK:
				const createdModel = (action as modelAction.StockTransactionDetailActionOK ).stateConfig.targetModel;
				...
				StockTransactionDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.CREATE_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionDetailModelActionTypes.CREATE_ALL_STOCK_TRANSACTION_DETAIL_OK:
				const createdModels = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					StockTransactionDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.CREATE_ALL_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionDetailModelActionTypes.DELETE_STOCK_TRANSACTION_DETAIL_OK:
				const deletedModelId = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.DELETE_ALL_STOCK_TRANSACTION_DETAIL_OK:
				const deletedModelIds = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.DELETE_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionDetailModelActionTypes.UPDATE_STOCK_TRANSACTION_DETAIL_OK:
				const updatedModel = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModel;
				...
				StockTransactionDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.UPDATE_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.StockTransactionDetailModelActionTypes.UPDATE_ALL_STOCK_TRANSACTION_DETAIL_OK:
				const updatedModels = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.UPDATE_ALL_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionDetailModelActionTypes.FETCH_STOCK_TRANSACTION_DETAIL_OK:
				let fetchedModel = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModel;
				...
				StockTransactionDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.FETCH_ALL_STOCK_TRANSACTION_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.StockTransactionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => StockTransactionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.FETCH_STOCK_TRANSACTION_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.StockTransactionDetailActionOK).audits;
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.FETCH_STOCK_TRANSACTION_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.StockTransactionDetailActionOK).audits;
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.FETCH_STOCK_TRANSACTION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => StockTransactionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.FETCH_LAST_STOCK_TRANSACTION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.StockTransactionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => StockTransactionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.COUNT_STOCK_TRANSACTION_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.StockTransactionDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.StockTransactionDetailModelActionTypes.INITIALISE_STOCK_TRANSACTION_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.StockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.StockTransactionDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialStockTransactionDetailCollectionState
					};
				}
