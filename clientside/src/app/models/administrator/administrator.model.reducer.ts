...
import {initialAdministratorCollectionState} from './administrator.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseAdministratorAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.AdministratorModelActionTypes.CREATE_ADMINISTRATOR_OK:
				const createdModel = (action as modelAction.AdministratorActionOK ).stateConfig.targetModel;
				...
				AdministratorModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.AdministratorModelActionTypes.CREATE_ADMINISTRATOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministratorModelActionTypes.CREATE_ALL_ADMINISTRATOR_OK:
				const createdModels = (action as modelAction.AdministratorActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					AdministratorModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.AdministratorModelActionTypes.CREATE_ALL_ADMINISTRATOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministratorModelActionTypes.DELETE_ADMINISTRATOR_OK:
				const deletedModelId = (action as modelAction.AdministratorActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdministratorModelActionTypes.DELETE_ALL_ADMINISTRATOR_OK:
				const deletedModelIds = (action as modelAction.AdministratorActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdministratorModelActionTypes.DELETE_ADMINISTRATOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministratorModelActionTypes.UPDATE_ADMINISTRATOR_OK:
				const updatedModel = (action as modelAction.AdministratorActionOK).stateConfig.targetModel;
				...
				AdministratorModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministratorModelActionTypes.UPDATE_ADMINISTRATOR_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.AdministratorModelActionTypes.UPDATE_ALL_ADMINISTRATOR_OK:
				const updatedModels = (action as modelAction.AdministratorActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdministratorModelActionTypes.UPDATE_ALL_ADMINISTRATOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministratorModelActionTypes.FETCH_ADMINISTRATOR_OK:
				let fetchedModel = (action as modelAction.AdministratorActionOK).stateConfig.targetModel;
				...
				AdministratorModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministratorModelActionTypes.FETCH_ALL_ADMINISTRATOR_OK:
				const totalCountAfterFetchAll = (action as modelAction.AdministratorActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.AdministratorActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.AdministratorActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => AdministratorModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministratorModelActionTypes.FETCH_ADMINISTRATOR_AUDIT_OK:
				const audits = (action as modelAction.AdministratorActionOK).audits;
				...
				break;
			case modelAction.AdministratorModelActionTypes.FETCH_ADMINISTRATOR_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.AdministratorActionOK).audits;
				...
				break;
			case modelAction.AdministratorModelActionTypes.FETCH_ADMINISTRATOR_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => AdministratorModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministratorModelActionTypes.FETCH_LAST_ADMINISTRATOR_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.AdministratorActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => AdministratorModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministratorModelActionTypes.COUNT_ADMINISTRATORS_OK:
				const totalCountAfterCount = (action as modelAction.AdministratorActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.AdministratorModelActionTypes.INITIALISE_ADMINISTRATOR_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.AdministratorActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.AdministratorActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialAdministratorCollectionState
					};
				}
