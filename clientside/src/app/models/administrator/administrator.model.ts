...
export class AdministratorModel extends UserModel {
	...
	readonly className = 'AdministratorModel';
	...
	get valueToDisplay(): string {
		...
		return AdministratorModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = AdministratorModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof AdministratorModel]?: AdministratorModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new AdministratorModel(data);
		}
	 ...
	 * @example
	 *
	 * `let administratorModel = new AdministratorModel(data);`
	 *
	 * @param data The input data to be initialised as the AdministratorModel,
	 *    it is expected as a JSON string or as a nullable AdministratorModel.
	 */
	constructor(data?: string | Partial<AdministratorModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<AdministratorModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		AdministratorModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): AdministratorModel {
		...
		return new AdministratorModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof AdministratorModel)) {
			return {};
		}
