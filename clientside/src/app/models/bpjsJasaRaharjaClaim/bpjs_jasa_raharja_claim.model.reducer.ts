...
import {initialBpjsJasaRaharjaClaimCollectionState} from './bpjs_jasa_raharja_claim.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsJasaRaharjaClaimAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.CREATE_BPJS_JASA_RAHARJA_CLAIM_OK:
				const createdModel = (action as modelAction.BpjsJasaRaharjaClaimActionOK ).stateConfig.targetModel;
				...
				BpjsJasaRaharjaClaimModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.CREATE_BPJS_JASA_RAHARJA_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.CREATE_ALL_BPJS_JASA_RAHARJA_CLAIM_OK:
				const createdModels = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsJasaRaharjaClaimModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.CREATE_ALL_BPJS_JASA_RAHARJA_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.DELETE_BPJS_JASA_RAHARJA_CLAIM_OK:
				const deletedModelId = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.DELETE_ALL_BPJS_JASA_RAHARJA_CLAIM_OK:
				const deletedModelIds = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.DELETE_BPJS_JASA_RAHARJA_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.UPDATE_BPJS_JASA_RAHARJA_CLAIM_OK:
				const updatedModel = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModel;
				...
				BpjsJasaRaharjaClaimModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.UPDATE_BPJS_JASA_RAHARJA_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.UPDATE_ALL_BPJS_JASA_RAHARJA_CLAIM_OK:
				const updatedModels = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.UPDATE_ALL_BPJS_JASA_RAHARJA_CLAIM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_BPJS_JASA_RAHARJA_CLAIM_OK:
				let fetchedModel = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModel;
				...
				BpjsJasaRaharjaClaimModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_ALL_BPJS_JASA_RAHARJA_CLAIM_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsJasaRaharjaClaimModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_BPJS_JASA_RAHARJA_CLAIM_AUDIT_OK:
				const audits = (action as modelAction.BpjsJasaRaharjaClaimActionOK).audits;
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_BPJS_JASA_RAHARJA_CLAIM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsJasaRaharjaClaimActionOK).audits;
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_BPJS_JASA_RAHARJA_CLAIM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsJasaRaharjaClaimModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.FETCH_LAST_BPJS_JASA_RAHARJA_CLAIM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsJasaRaharjaClaimModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.COUNT_BPJS_JASA_RAHARJA_CLAIMS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsJasaRaharjaClaimModelActionTypes.INITIALISE_BPJS_JASA_RAHARJA_CLAIM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsJasaRaharjaClaimActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsJasaRaharjaClaimCollectionState
					};
				}
