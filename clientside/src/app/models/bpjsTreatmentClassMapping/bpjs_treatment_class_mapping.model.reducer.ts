...
import {initialBpjsTreatmentClassMappingCollectionState} from './bpjs_treatment_class_mapping.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsTreatmentClassMappingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.CREATE_BPJS_TREATMENT_CLASS_MAPPING_OK:
				const createdModel = (action as modelAction.BpjsTreatmentClassMappingActionOK ).stateConfig.targetModel;
				...
				BpjsTreatmentClassMappingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.CREATE_BPJS_TREATMENT_CLASS_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.CREATE_ALL_BPJS_TREATMENT_CLASS_MAPPING_OK:
				const createdModels = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsTreatmentClassMappingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.CREATE_ALL_BPJS_TREATMENT_CLASS_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.DELETE_BPJS_TREATMENT_CLASS_MAPPING_OK:
				const deletedModelId = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.DELETE_ALL_BPJS_TREATMENT_CLASS_MAPPING_OK:
				const deletedModelIds = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.DELETE_BPJS_TREATMENT_CLASS_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.UPDATE_BPJS_TREATMENT_CLASS_MAPPING_OK:
				const updatedModel = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModel;
				...
				BpjsTreatmentClassMappingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.UPDATE_BPJS_TREATMENT_CLASS_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsTreatmentClassMappingModelActionTypes.UPDATE_ALL_BPJS_TREATMENT_CLASS_MAPPING_OK:
				const updatedModels = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.UPDATE_ALL_BPJS_TREATMENT_CLASS_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_BPJS_TREATMENT_CLASS_MAPPING_OK:
				let fetchedModel = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModel;
				...
				BpjsTreatmentClassMappingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_ALL_BPJS_TREATMENT_CLASS_MAPPING_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsTreatmentClassMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_BPJS_TREATMENT_CLASS_MAPPING_AUDIT_OK:
				const audits = (action as modelAction.BpjsTreatmentClassMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_BPJS_TREATMENT_CLASS_MAPPING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsTreatmentClassMappingActionOK).audits;
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_BPJS_TREATMENT_CLASS_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsTreatmentClassMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.FETCH_LAST_BPJS_TREATMENT_CLASS_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsTreatmentClassMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.COUNT_BPJS_TREATMENT_CLASS_MAPPINGS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsTreatmentClassMappingModelActionTypes.INITIALISE_BPJS_TREATMENT_CLASS_MAPPING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsTreatmentClassMappingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsTreatmentClassMappingCollectionState
					};
				}
