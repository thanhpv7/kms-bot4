...
import {initialBpjsDrugGenericPRBCollectionState} from './bpjs_drug_generic_prb.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsDrugGenericPRBAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsDrugGenericPRBModelActionTypes.CREATE_BPJS_DRUG_GENERIC_PRB_OK:
				const createdModel = (action as modelAction.BpjsDrugGenericPRBActionOK ).stateConfig.targetModel;
				...
				BpjsDrugGenericPRBModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.CREATE_BPJS_DRUG_GENERIC_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.CREATE_ALL_BPJS_DRUG_GENERIC_PRB_OK:
				const createdModels = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsDrugGenericPRBModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.CREATE_ALL_BPJS_DRUG_GENERIC_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.DELETE_BPJS_DRUG_GENERIC_PRB_OK:
				const deletedModelId = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.DELETE_ALL_BPJS_DRUG_GENERIC_PRB_OK:
				const deletedModelIds = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.DELETE_BPJS_DRUG_GENERIC_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.UPDATE_BPJS_DRUG_GENERIC_PRB_OK:
				const updatedModel = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModel;
				...
				BpjsDrugGenericPRBModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.UPDATE_BPJS_DRUG_GENERIC_PRB_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsDrugGenericPRBModelActionTypes.UPDATE_ALL_BPJS_DRUG_GENERIC_PRB_OK:
				const updatedModels = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.UPDATE_ALL_BPJS_DRUG_GENERIC_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_BPJS_DRUG_GENERIC_PRB_OK:
				let fetchedModel = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModel;
				...
				BpjsDrugGenericPRBModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_ALL_BPJS_DRUG_GENERIC_PRB_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsDrugGenericPRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_BPJS_DRUG_GENERIC_PRB_AUDIT_OK:
				const audits = (action as modelAction.BpjsDrugGenericPRBActionOK).audits;
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_BPJS_DRUG_GENERIC_PRB_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsDrugGenericPRBActionOK).audits;
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_BPJS_DRUG_GENERIC_PRB_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsDrugGenericPRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.FETCH_LAST_BPJS_DRUG_GENERIC_PRB_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsDrugGenericPRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.COUNT_BPJS_DRUG_GENERIC_PRBS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsDrugGenericPRBModelActionTypes.INITIALISE_BPJS_DRUG_GENERIC_PRB_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsDrugGenericPRBActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsDrugGenericPRBCollectionState
					};
				}
