...
import {initialExaminationGroupCollectionState} from './examination_group.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseExaminationGroupAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ExaminationGroupModelActionTypes.CREATE_EXAMINATION_GROUP_OK:
				const createdModel = (action as modelAction.ExaminationGroupActionOK ).stateConfig.targetModel;
				...
				ExaminationGroupModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.CREATE_EXAMINATION_GROUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationGroupModelActionTypes.CREATE_ALL_EXAMINATION_GROUP_OK:
				const createdModels = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ExaminationGroupModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.CREATE_ALL_EXAMINATION_GROUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationGroupModelActionTypes.DELETE_EXAMINATION_GROUP_OK:
				const deletedModelId = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.DELETE_ALL_EXAMINATION_GROUP_OK:
				const deletedModelIds = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.DELETE_EXAMINATION_GROUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationGroupModelActionTypes.UPDATE_EXAMINATION_GROUP_OK:
				const updatedModel = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModel;
				...
				ExaminationGroupModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.UPDATE_EXAMINATION_GROUP_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ExaminationGroupModelActionTypes.UPDATE_ALL_EXAMINATION_GROUP_OK:
				const updatedModels = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.UPDATE_ALL_EXAMINATION_GROUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationGroupModelActionTypes.FETCH_EXAMINATION_GROUP_OK:
				let fetchedModel = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModel;
				...
				ExaminationGroupModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.FETCH_ALL_EXAMINATION_GROUP_OK:
				const totalCountAfterFetchAll = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ExaminationGroupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ExaminationGroupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.FETCH_EXAMINATION_GROUP_AUDIT_OK:
				const audits = (action as modelAction.ExaminationGroupActionOK).audits;
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.FETCH_EXAMINATION_GROUP_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ExaminationGroupActionOK).audits;
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.FETCH_EXAMINATION_GROUP_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ExaminationGroupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.FETCH_LAST_EXAMINATION_GROUP_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ExaminationGroupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ExaminationGroupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.COUNT_EXAMINATION_GROUPS_OK:
				const totalCountAfterCount = (action as modelAction.ExaminationGroupActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ExaminationGroupModelActionTypes.INITIALISE_EXAMINATION_GROUP_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ExaminationGroupActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ExaminationGroupActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialExaminationGroupCollectionState
					};
				}
