...
import {initialFacilityHistoryCollectionState} from './facility_history.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseFacilityHistoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.FacilityHistoryModelActionTypes.CREATE_FACILITY_HISTORY_OK:
				const createdModel = (action as modelAction.FacilityHistoryActionOK ).stateConfig.targetModel;
				...
				FacilityHistoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.CREATE_FACILITY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityHistoryModelActionTypes.CREATE_ALL_FACILITY_HISTORY_OK:
				const createdModels = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					FacilityHistoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.CREATE_ALL_FACILITY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityHistoryModelActionTypes.DELETE_FACILITY_HISTORY_OK:
				const deletedModelId = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.DELETE_ALL_FACILITY_HISTORY_OK:
				const deletedModelIds = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.DELETE_FACILITY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityHistoryModelActionTypes.UPDATE_FACILITY_HISTORY_OK:
				const updatedModel = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModel;
				...
				FacilityHistoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.UPDATE_FACILITY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.FacilityHistoryModelActionTypes.UPDATE_ALL_FACILITY_HISTORY_OK:
				const updatedModels = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.UPDATE_ALL_FACILITY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityHistoryModelActionTypes.FETCH_FACILITY_HISTORY_OK:
				let fetchedModel = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModel;
				...
				FacilityHistoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.FETCH_ALL_FACILITY_HISTORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.FacilityHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => FacilityHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.FETCH_FACILITY_HISTORY_AUDIT_OK:
				const audits = (action as modelAction.FacilityHistoryActionOK).audits;
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.FETCH_FACILITY_HISTORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.FacilityHistoryActionOK).audits;
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.FETCH_FACILITY_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => FacilityHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.FETCH_LAST_FACILITY_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.FacilityHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => FacilityHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.COUNT_FACILITY_HISTORYS_OK:
				const totalCountAfterCount = (action as modelAction.FacilityHistoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.FacilityHistoryModelActionTypes.INITIALISE_FACILITY_HISTORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.FacilityHistoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.FacilityHistoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialFacilityHistoryCollectionState
					};
				}
