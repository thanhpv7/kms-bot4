...
import {initialHospitalGroupInformationCollectionState} from './hospital_group_information.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseHospitalGroupInformationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.HospitalGroupInformationModelActionTypes.CREATE_HOSPITAL_GROUP_INFORMATION_OK:
				const createdModel = (action as modelAction.HospitalGroupInformationActionOK ).stateConfig.targetModel;
				...
				HospitalGroupInformationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.CREATE_HOSPITAL_GROUP_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.CREATE_ALL_HOSPITAL_GROUP_INFORMATION_OK:
				const createdModels = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					HospitalGroupInformationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.CREATE_ALL_HOSPITAL_GROUP_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.DELETE_HOSPITAL_GROUP_INFORMATION_OK:
				const deletedModelId = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.DELETE_ALL_HOSPITAL_GROUP_INFORMATION_OK:
				const deletedModelIds = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.DELETE_HOSPITAL_GROUP_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.UPDATE_HOSPITAL_GROUP_INFORMATION_OK:
				const updatedModel = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModel;
				...
				HospitalGroupInformationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.UPDATE_HOSPITAL_GROUP_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.HospitalGroupInformationModelActionTypes.UPDATE_ALL_HOSPITAL_GROUP_INFORMATION_OK:
				const updatedModels = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.UPDATE_ALL_HOSPITAL_GROUP_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.FETCH_HOSPITAL_GROUP_INFORMATION_OK:
				let fetchedModel = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModel;
				...
				HospitalGroupInformationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.FETCH_ALL_HOSPITAL_GROUP_INFORMATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => HospitalGroupInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.FETCH_HOSPITAL_GROUP_INFORMATION_AUDIT_OK:
				const audits = (action as modelAction.HospitalGroupInformationActionOK).audits;
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.FETCH_HOSPITAL_GROUP_INFORMATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.HospitalGroupInformationActionOK).audits;
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.FETCH_HOSPITAL_GROUP_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => HospitalGroupInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.FETCH_LAST_HOSPITAL_GROUP_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => HospitalGroupInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.COUNT_HOSPITAL_GROUP_INFORMATIONS_OK:
				const totalCountAfterCount = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.HospitalGroupInformationModelActionTypes.INITIALISE_HOSPITAL_GROUP_INFORMATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.HospitalGroupInformationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.HospitalGroupInformationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialHospitalGroupInformationCollectionState
					};
				}
