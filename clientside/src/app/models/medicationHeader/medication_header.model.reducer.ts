...
import {initialMedicationHeaderCollectionState} from './medication_header.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicationHeaderAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicationHeaderModelActionTypes.CREATE_MEDICATION_HEADER_OK:
				const createdModel = (action as modelAction.MedicationHeaderActionOK ).stateConfig.targetModel;
				...
				MedicationHeaderModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.CREATE_MEDICATION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationHeaderModelActionTypes.CREATE_ALL_MEDICATION_HEADER_OK:
				const createdModels = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicationHeaderModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.CREATE_ALL_MEDICATION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationHeaderModelActionTypes.DELETE_MEDICATION_HEADER_OK:
				const deletedModelId = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.DELETE_ALL_MEDICATION_HEADER_OK:
				const deletedModelIds = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.DELETE_MEDICATION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationHeaderModelActionTypes.UPDATE_MEDICATION_HEADER_OK:
				const updatedModel = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModel;
				...
				MedicationHeaderModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.UPDATE_MEDICATION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicationHeaderModelActionTypes.UPDATE_ALL_MEDICATION_HEADER_OK:
				const updatedModels = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.UPDATE_ALL_MEDICATION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationHeaderModelActionTypes.FETCH_MEDICATION_HEADER_OK:
				let fetchedModel = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModel;
				...
				MedicationHeaderModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.FETCH_ALL_MEDICATION_HEADER_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicationHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicationHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.FETCH_MEDICATION_HEADER_AUDIT_OK:
				const audits = (action as modelAction.MedicationHeaderActionOK).audits;
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.FETCH_MEDICATION_HEADER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicationHeaderActionOK).audits;
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.FETCH_MEDICATION_HEADER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicationHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.FETCH_LAST_MEDICATION_HEADER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicationHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicationHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.COUNT_MEDICATION_HEADERS_OK:
				const totalCountAfterCount = (action as modelAction.MedicationHeaderActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicationHeaderModelActionTypes.INITIALISE_MEDICATION_HEADER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicationHeaderActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicationHeaderActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicationHeaderCollectionState
					};
				}
