...
import {initialBpjsBedAvailabilityCollectionState} from './bpjs_bed_availability.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsBedAvailabilityAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsBedAvailabilityModelActionTypes.CREATE_BPJS_BED_AVAILABILITY_OK:
				const createdModel = (action as modelAction.BpjsBedAvailabilityActionOK ).stateConfig.targetModel;
				...
				BpjsBedAvailabilityModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.CREATE_BPJS_BED_AVAILABILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.CREATE_ALL_BPJS_BED_AVAILABILITY_OK:
				const createdModels = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsBedAvailabilityModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.CREATE_ALL_BPJS_BED_AVAILABILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.DELETE_BPJS_BED_AVAILABILITY_OK:
				const deletedModelId = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.DELETE_ALL_BPJS_BED_AVAILABILITY_OK:
				const deletedModelIds = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.DELETE_BPJS_BED_AVAILABILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.UPDATE_BPJS_BED_AVAILABILITY_OK:
				const updatedModel = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModel;
				...
				BpjsBedAvailabilityModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.UPDATE_BPJS_BED_AVAILABILITY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsBedAvailabilityModelActionTypes.UPDATE_ALL_BPJS_BED_AVAILABILITY_OK:
				const updatedModels = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.UPDATE_ALL_BPJS_BED_AVAILABILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_BPJS_BED_AVAILABILITY_OK:
				let fetchedModel = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModel;
				...
				BpjsBedAvailabilityModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_ALL_BPJS_BED_AVAILABILITY_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsBedAvailabilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_BPJS_BED_AVAILABILITY_AUDIT_OK:
				const audits = (action as modelAction.BpjsBedAvailabilityActionOK).audits;
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_BPJS_BED_AVAILABILITY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsBedAvailabilityActionOK).audits;
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_BPJS_BED_AVAILABILITY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsBedAvailabilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.FETCH_LAST_BPJS_BED_AVAILABILITY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsBedAvailabilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.COUNT_BPJS_BED_AVAILABILITYS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsBedAvailabilityModelActionTypes.INITIALISE_BPJS_BED_AVAILABILITY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsBedAvailabilityActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsBedAvailabilityCollectionState
					};
				}
