...
import {initialBpjsDiagnoseCollectionState} from './bpjs_diagnose.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsDiagnoseAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsDiagnoseModelActionTypes.CREATE_BPJS_DIAGNOSE_OK:
				const createdModel = (action as modelAction.BpjsDiagnoseActionOK ).stateConfig.targetModel;
				...
				BpjsDiagnoseModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.CREATE_BPJS_DIAGNOSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.CREATE_ALL_BPJS_DIAGNOSE_OK:
				const createdModels = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsDiagnoseModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.CREATE_ALL_BPJS_DIAGNOSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.DELETE_BPJS_DIAGNOSE_OK:
				const deletedModelId = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.DELETE_ALL_BPJS_DIAGNOSE_OK:
				const deletedModelIds = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.DELETE_BPJS_DIAGNOSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.UPDATE_BPJS_DIAGNOSE_OK:
				const updatedModel = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModel;
				...
				BpjsDiagnoseModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.UPDATE_BPJS_DIAGNOSE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsDiagnoseModelActionTypes.UPDATE_ALL_BPJS_DIAGNOSE_OK:
				const updatedModels = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.UPDATE_ALL_BPJS_DIAGNOSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.FETCH_BPJS_DIAGNOSE_OK:
				let fetchedModel = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModel;
				...
				BpjsDiagnoseModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.FETCH_ALL_BPJS_DIAGNOSE_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsDiagnoseModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.FETCH_BPJS_DIAGNOSE_AUDIT_OK:
				const audits = (action as modelAction.BpjsDiagnoseActionOK).audits;
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.FETCH_BPJS_DIAGNOSE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsDiagnoseActionOK).audits;
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.FETCH_BPJS_DIAGNOSE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsDiagnoseModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.FETCH_LAST_BPJS_DIAGNOSE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsDiagnoseModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.COUNT_BPJS_DIAGNOSES_OK:
				const totalCountAfterCount = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsDiagnoseModelActionTypes.INITIALISE_BPJS_DIAGNOSE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsDiagnoseActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsDiagnoseActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsDiagnoseCollectionState
					};
				}
