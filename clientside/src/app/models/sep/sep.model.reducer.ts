...
import * as modelAction from './sep.model.action';
import {ModelState} from '../model.state';
import {initialSepCollectionState} from './sep.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSepAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SepModelActionTypes.CREATE_SEP_OK:
				const createdModel = (action as modelAction.SepActionOK ).stateConfig.targetModel;
				...
				SepModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SepModelActionTypes.CREATE_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SepModelActionTypes.CREATE_ALL_SEP_OK:
				const createdModels = (action as modelAction.SepActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SepActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SepModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SepModelActionTypes.CREATE_ALL_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SepModelActionTypes.DELETE_SEP_OK:
				const deletedModelId = (action as modelAction.SepActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SepActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SepModelActionTypes.DELETE_ALL_SEP_OK:
				const deletedModelIds = (action as modelAction.SepActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SepActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SepModelActionTypes.DELETE_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SepModelActionTypes.UPDATE_SEP_OK:
				const updatedModel = (action as modelAction.SepActionOK).stateConfig.targetModel;
				...
				SepModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SepModelActionTypes.UPDATE_SEP_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SepModelActionTypes.UPDATE_ALL_SEP_OK:
				const updatedModels = (action as modelAction.SepActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SepActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SepModelActionTypes.UPDATE_ALL_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SepModelActionTypes.FETCH_SEP_OK:
				let fetchedModel = (action as modelAction.SepActionOK).stateConfig.targetModel;
				...
				SepModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SepModelActionTypes.FETCH_ALL_SEP_OK:
				const totalCountAfterFetchAll = (action as modelAction.SepActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SepActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SepActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SepActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SepModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SepModelActionTypes.FETCH_SEP_AUDIT_OK:
				const audits = (action as modelAction.SepActionOK).audits;
				...
				break;
			case modelAction.SepModelActionTypes.FETCH_SEP_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SepActionOK).audits;
				...
				break;
			case modelAction.SepModelActionTypes.FETCH_SEP_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SepActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SepActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SepActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SepActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SepModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SepModelActionTypes.FETCH_LAST_SEP_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SepActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SepActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SepActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SepActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SepModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SepModelActionTypes.COUNT_SEPS_OK:
				const totalCountAfterCount = (action as modelAction.SepActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SepModelActionTypes.INITIALISE_SEP_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SepActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SepActionOK).stateConfig.queryParams;
