...
import {initialCompoundPrescriptionDetailCollectionState} from './compound_prescription_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCompoundPrescriptionDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CompoundPrescriptionDetailModelActionTypes.CREATE_COMPOUND_PRESCRIPTION_DETAIL_OK:
				const createdModel = (action as modelAction.CompoundPrescriptionDetailActionOK ).stateConfig.targetModel;
				...
				CompoundPrescriptionDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.CREATE_COMPOUND_PRESCRIPTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.CREATE_ALL_COMPOUND_PRESCRIPTION_DETAIL_OK:
				const createdModels = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CompoundPrescriptionDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.CREATE_ALL_COMPOUND_PRESCRIPTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_DETAIL_OK:
				const deletedModelId = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.DELETE_ALL_COMPOUND_PRESCRIPTION_DETAIL_OK:
				const deletedModelIds = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.UPDATE_COMPOUND_PRESCRIPTION_DETAIL_OK:
				const updatedModel = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModel;
				...
				CompoundPrescriptionDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.UPDATE_COMPOUND_PRESCRIPTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CompoundPrescriptionDetailModelActionTypes.UPDATE_ALL_COMPOUND_PRESCRIPTION_DETAIL_OK:
				const updatedModels = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.UPDATE_ALL_COMPOUND_PRESCRIPTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_DETAIL_OK:
				let fetchedModel = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModel;
				...
				CompoundPrescriptionDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_ALL_COMPOUND_PRESCRIPTION_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CompoundPrescriptionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.CompoundPrescriptionDetailActionOK).audits;
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CompoundPrescriptionDetailActionOK).audits;
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CompoundPrescriptionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.FETCH_LAST_COMPOUND_PRESCRIPTION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CompoundPrescriptionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.COUNT_COMPOUND_PRESCRIPTION_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CompoundPrescriptionDetailModelActionTypes.INITIALISE_COMPOUND_PRESCRIPTION_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CompoundPrescriptionDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCompoundPrescriptionDetailCollectionState
					};
				}
