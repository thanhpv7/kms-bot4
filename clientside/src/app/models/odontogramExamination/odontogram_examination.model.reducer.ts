...
import {initialOdontogramExaminationCollectionState} from './odontogram_examination.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseOdontogramExaminationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.OdontogramExaminationModelActionTypes.CREATE_ODONTOGRAM_EXAMINATION_OK:
				const createdModel = (action as modelAction.OdontogramExaminationActionOK ).stateConfig.targetModel;
				...
				OdontogramExaminationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.CREATE_ODONTOGRAM_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OdontogramExaminationModelActionTypes.CREATE_ALL_ODONTOGRAM_EXAMINATION_OK:
				const createdModels = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					OdontogramExaminationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.CREATE_ALL_ODONTOGRAM_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OdontogramExaminationModelActionTypes.DELETE_ODONTOGRAM_EXAMINATION_OK:
				const deletedModelId = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.DELETE_ALL_ODONTOGRAM_EXAMINATION_OK:
				const deletedModelIds = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.DELETE_ODONTOGRAM_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OdontogramExaminationModelActionTypes.UPDATE_ODONTOGRAM_EXAMINATION_OK:
				const updatedModel = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModel;
				...
				OdontogramExaminationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.UPDATE_ODONTOGRAM_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.OdontogramExaminationModelActionTypes.UPDATE_ALL_ODONTOGRAM_EXAMINATION_OK:
				const updatedModels = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.UPDATE_ALL_ODONTOGRAM_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OdontogramExaminationModelActionTypes.FETCH_ODONTOGRAM_EXAMINATION_OK:
				let fetchedModel = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModel;
				...
				OdontogramExaminationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.FETCH_ALL_ODONTOGRAM_EXAMINATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.OdontogramExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => OdontogramExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.FETCH_ODONTOGRAM_EXAMINATION_AUDIT_OK:
				const audits = (action as modelAction.OdontogramExaminationActionOK).audits;
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.FETCH_ODONTOGRAM_EXAMINATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.OdontogramExaminationActionOK).audits;
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.FETCH_ODONTOGRAM_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => OdontogramExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.FETCH_LAST_ODONTOGRAM_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.OdontogramExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => OdontogramExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.COUNT_ODONTOGRAM_EXAMINATIONS_OK:
				const totalCountAfterCount = (action as modelAction.OdontogramExaminationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.OdontogramExaminationModelActionTypes.INITIALISE_ODONTOGRAM_EXAMINATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.OdontogramExaminationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.OdontogramExaminationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialOdontogramExaminationCollectionState
					};
				}
