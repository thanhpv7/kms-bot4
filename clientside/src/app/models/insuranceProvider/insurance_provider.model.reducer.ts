...
import {initialInsuranceProviderCollectionState} from './insurance_provider.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInsuranceProviderAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InsuranceProviderModelActionTypes.CREATE_INSURANCE_PROVIDER_OK:
				const createdModel = (action as modelAction.InsuranceProviderActionOK ).stateConfig.targetModel;
				...
				InsuranceProviderModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.CREATE_INSURANCE_PROVIDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceProviderModelActionTypes.CREATE_ALL_INSURANCE_PROVIDER_OK:
				const createdModels = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InsuranceProviderModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.CREATE_ALL_INSURANCE_PROVIDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceProviderModelActionTypes.DELETE_INSURANCE_PROVIDER_OK:
				const deletedModelId = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.DELETE_ALL_INSURANCE_PROVIDER_OK:
				const deletedModelIds = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.DELETE_INSURANCE_PROVIDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceProviderModelActionTypes.UPDATE_INSURANCE_PROVIDER_OK:
				const updatedModel = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModel;
				...
				InsuranceProviderModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.UPDATE_INSURANCE_PROVIDER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InsuranceProviderModelActionTypes.UPDATE_ALL_INSURANCE_PROVIDER_OK:
				const updatedModels = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.UPDATE_ALL_INSURANCE_PROVIDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceProviderModelActionTypes.FETCH_INSURANCE_PROVIDER_OK:
				let fetchedModel = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModel;
				...
				InsuranceProviderModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.FETCH_ALL_INSURANCE_PROVIDER_OK:
				const totalCountAfterFetchAll = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InsuranceProviderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InsuranceProviderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.FETCH_INSURANCE_PROVIDER_AUDIT_OK:
				const audits = (action as modelAction.InsuranceProviderActionOK).audits;
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.FETCH_INSURANCE_PROVIDER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InsuranceProviderActionOK).audits;
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.FETCH_INSURANCE_PROVIDER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InsuranceProviderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.FETCH_LAST_INSURANCE_PROVIDER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InsuranceProviderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InsuranceProviderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.COUNT_INSURANCE_PROVIDERS_OK:
				const totalCountAfterCount = (action as modelAction.InsuranceProviderActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InsuranceProviderModelActionTypes.INITIALISE_INSURANCE_PROVIDER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InsuranceProviderActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InsuranceProviderActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInsuranceProviderCollectionState
					};
				}
