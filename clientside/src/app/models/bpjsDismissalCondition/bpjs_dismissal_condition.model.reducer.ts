...
import {initialBpjsDismissalConditionCollectionState} from './bpjs_dismissal_condition.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsDismissalConditionAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsDismissalConditionModelActionTypes.CREATE_BPJS_DISMISSAL_CONDITION_OK:
				const createdModel = (action as modelAction.BpjsDismissalConditionActionOK ).stateConfig.targetModel;
				...
				BpjsDismissalConditionModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.CREATE_BPJS_DISMISSAL_CONDITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.CREATE_ALL_BPJS_DISMISSAL_CONDITION_OK:
				const createdModels = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsDismissalConditionModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.CREATE_ALL_BPJS_DISMISSAL_CONDITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.DELETE_BPJS_DISMISSAL_CONDITION_OK:
				const deletedModelId = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.DELETE_ALL_BPJS_DISMISSAL_CONDITION_OK:
				const deletedModelIds = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.DELETE_BPJS_DISMISSAL_CONDITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.UPDATE_BPJS_DISMISSAL_CONDITION_OK:
				const updatedModel = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModel;
				...
				BpjsDismissalConditionModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.UPDATE_BPJS_DISMISSAL_CONDITION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsDismissalConditionModelActionTypes.UPDATE_ALL_BPJS_DISMISSAL_CONDITION_OK:
				const updatedModels = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.UPDATE_ALL_BPJS_DISMISSAL_CONDITION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.FETCH_BPJS_DISMISSAL_CONDITION_OK:
				let fetchedModel = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModel;
				...
				BpjsDismissalConditionModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.FETCH_ALL_BPJS_DISMISSAL_CONDITION_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsDismissalConditionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.FETCH_BPJS_DISMISSAL_CONDITION_AUDIT_OK:
				const audits = (action as modelAction.BpjsDismissalConditionActionOK).audits;
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.FETCH_BPJS_DISMISSAL_CONDITION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsDismissalConditionActionOK).audits;
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.FETCH_BPJS_DISMISSAL_CONDITION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsDismissalConditionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.FETCH_LAST_BPJS_DISMISSAL_CONDITION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsDismissalConditionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.COUNT_BPJS_DISMISSAL_CONDITIONS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsDismissalConditionModelActionTypes.INITIALISE_BPJS_DISMISSAL_CONDITION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsDismissalConditionActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsDismissalConditionCollectionState
					};
				}
