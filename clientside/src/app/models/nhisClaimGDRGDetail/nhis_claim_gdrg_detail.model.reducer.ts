...
import {initialNhisClaimGDRGDetailCollectionState} from './nhis_claim_gdrg_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisClaimGDRGDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisClaimGDRGDetailModelActionTypes.CREATE_NHIS_CLAIM_GDRG_DETAIL_OK:
				const createdModel = (action as modelAction.NhisClaimGDRGDetailActionOK ).stateConfig.targetModel;
				...
				NhisClaimGDRGDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.CREATE_NHIS_CLAIM_GDRG_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.CREATE_ALL_NHIS_CLAIM_GDRG_DETAIL_OK:
				const createdModels = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisClaimGDRGDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.CREATE_ALL_NHIS_CLAIM_GDRG_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.DELETE_NHIS_CLAIM_GDRG_DETAIL_OK:
				const deletedModelId = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.DELETE_ALL_NHIS_CLAIM_GDRG_DETAIL_OK:
				const deletedModelIds = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.DELETE_NHIS_CLAIM_GDRG_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.UPDATE_NHIS_CLAIM_GDRG_DETAIL_OK:
				const updatedModel = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModel;
				...
				NhisClaimGDRGDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.UPDATE_NHIS_CLAIM_GDRG_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisClaimGDRGDetailModelActionTypes.UPDATE_ALL_NHIS_CLAIM_GDRG_DETAIL_OK:
				const updatedModels = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.UPDATE_ALL_NHIS_CLAIM_GDRG_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_NHIS_CLAIM_GDRG_DETAIL_OK:
				let fetchedModel = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModel;
				...
				NhisClaimGDRGDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_ALL_NHIS_CLAIM_GDRG_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisClaimGDRGDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_NHIS_CLAIM_GDRG_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.NhisClaimGDRGDetailActionOK).audits;
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_NHIS_CLAIM_GDRG_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisClaimGDRGDetailActionOK).audits;
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_NHIS_CLAIM_GDRG_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisClaimGDRGDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.FETCH_LAST_NHIS_CLAIM_GDRG_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisClaimGDRGDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.COUNT_NHIS_CLAIM_GDRG_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisClaimGDRGDetailModelActionTypes.INITIALISE_NHIS_CLAIM_GDRG_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisClaimGDRGDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisClaimGDRGDetailCollectionState
					};
				}
