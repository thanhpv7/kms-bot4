...
import * as modelAction from './location.model.action';
import {ModelState} from '../model.state';
import {initialLocationCollectionState} from './location.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseLocationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.LocationModelActionTypes.CREATE_LOCATION_OK:
				const createdModel = (action as modelAction.LocationActionOK ).stateConfig.targetModel;
				...
				LocationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.LocationModelActionTypes.CREATE_LOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LocationModelActionTypes.CREATE_ALL_LOCATION_OK:
				const createdModels = (action as modelAction.LocationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.LocationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					LocationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.LocationModelActionTypes.CREATE_ALL_LOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LocationModelActionTypes.DELETE_LOCATION_OK:
				const deletedModelId = (action as modelAction.LocationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.LocationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.LocationModelActionTypes.DELETE_ALL_LOCATION_OK:
				const deletedModelIds = (action as modelAction.LocationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.LocationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.LocationModelActionTypes.DELETE_LOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LocationModelActionTypes.UPDATE_LOCATION_OK:
				const updatedModel = (action as modelAction.LocationActionOK).stateConfig.targetModel;
				...
				LocationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LocationModelActionTypes.UPDATE_LOCATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.LocationModelActionTypes.UPDATE_ALL_LOCATION_OK:
				const updatedModels = (action as modelAction.LocationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.LocationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.LocationModelActionTypes.UPDATE_ALL_LOCATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.LocationModelActionTypes.FETCH_LOCATION_OK:
				let fetchedModel = (action as modelAction.LocationActionOK).stateConfig.targetModel;
				...
				LocationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LocationModelActionTypes.FETCH_ALL_LOCATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.LocationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.LocationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.LocationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.LocationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => LocationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LocationModelActionTypes.FETCH_LOCATION_AUDIT_OK:
				const audits = (action as modelAction.LocationActionOK).audits;
				...
				break;
			case modelAction.LocationModelActionTypes.FETCH_LOCATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.LocationActionOK).audits;
				...
				break;
			case modelAction.LocationModelActionTypes.FETCH_LOCATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.LocationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.LocationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.LocationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.LocationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => LocationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LocationModelActionTypes.FETCH_LAST_LOCATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.LocationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.LocationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.LocationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.LocationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => LocationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.LocationModelActionTypes.COUNT_LOCATIONS_OK:
				const totalCountAfterCount = (action as modelAction.LocationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.LocationModelActionTypes.INITIALISE_LOCATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.LocationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.LocationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialLocationCollectionState
					};
				}
