...
import {initialPatientEmergencyContactDetailCollectionState} from './patient_emergency_contact_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientEmergencyContactDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientEmergencyContactDetailModelActionTypes.CREATE_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				const createdModel = (action as modelAction.PatientEmergencyContactDetailActionOK ).stateConfig.targetModel;
				...
				PatientEmergencyContactDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.CREATE_PATIENT_EMERGENCY_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.CREATE_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				const createdModels = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientEmergencyContactDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.CREATE_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.DELETE_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				const deletedModelId = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.DELETE_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				const deletedModelIds = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.DELETE_PATIENT_EMERGENCY_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.UPDATE_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				const updatedModel = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModel;
				...
				PatientEmergencyContactDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.UPDATE_PATIENT_EMERGENCY_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientEmergencyContactDetailModelActionTypes.UPDATE_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				const updatedModels = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.UPDATE_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				let fetchedModel = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModel;
				...
				PatientEmergencyContactDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_ALL_PATIENT_EMERGENCY_CONTACT_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientEmergencyContactDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_PATIENT_EMERGENCY_CONTACT_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.PatientEmergencyContactDetailActionOK).audits;
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_PATIENT_EMERGENCY_CONTACT_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientEmergencyContactDetailActionOK).audits;
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_PATIENT_EMERGENCY_CONTACT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientEmergencyContactDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.FETCH_LAST_PATIENT_EMERGENCY_CONTACT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientEmergencyContactDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.COUNT_PATIENT_EMERGENCY_CONTACT_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientEmergencyContactDetailModelActionTypes.INITIALISE_PATIENT_EMERGENCY_CONTACT_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientEmergencyContactDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientEmergencyContactDetailCollectionState
					};
				}
