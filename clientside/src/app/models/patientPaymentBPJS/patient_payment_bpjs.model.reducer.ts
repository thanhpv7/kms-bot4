...
import {initialPatientPaymentBPJSCollectionState} from './patient_payment_bpjs.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientPaymentBPJSAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientPaymentBPJSModelActionTypes.CREATE_PATIENT_PAYMENT_BPJS_OK:
				const createdModel = (action as modelAction.PatientPaymentBPJSActionOK ).stateConfig.targetModel;
				...
				PatientPaymentBPJSModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.CREATE_PATIENT_PAYMENT_BPJS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_BPJS_OK:
				const createdModels = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientPaymentBPJSModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_BPJS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.DELETE_PATIENT_PAYMENT_BPJS_OK:
				const deletedModelId = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_BPJS_OK:
				const deletedModelIds = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.DELETE_PATIENT_PAYMENT_BPJS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.UPDATE_PATIENT_PAYMENT_BPJS_OK:
				const updatedModel = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModel;
				...
				PatientPaymentBPJSModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.UPDATE_PATIENT_PAYMENT_BPJS_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientPaymentBPJSModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_BPJS_OK:
				const updatedModels = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_BPJS_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.FETCH_PATIENT_PAYMENT_BPJS_OK:
				let fetchedModel = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModel;
				...
				PatientPaymentBPJSModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.FETCH_ALL_PATIENT_PAYMENT_BPJS_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientPaymentBPJSModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.FETCH_PATIENT_PAYMENT_BPJS_AUDIT_OK:
				const audits = (action as modelAction.PatientPaymentBPJSActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.FETCH_PATIENT_PAYMENT_BPJS_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientPaymentBPJSActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.FETCH_PATIENT_PAYMENT_BPJS_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientPaymentBPJSModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.FETCH_LAST_PATIENT_PAYMENT_BPJS_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientPaymentBPJSModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.COUNT_PATIENT_PAYMENT_BPJSS_OK:
				const totalCountAfterCount = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientPaymentBPJSModelActionTypes.INITIALISE_PATIENT_PAYMENT_BPJS_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientPaymentBPJSActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientPaymentBPJSCollectionState
					};
				}
