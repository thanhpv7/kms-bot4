...
import {initialNhisMedicineCollectionState} from './nhis_medicine.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisMedicineAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisMedicineModelActionTypes.CREATE_NHIS_MEDICINE_OK:
				const createdModel = (action as modelAction.NhisMedicineActionOK ).stateConfig.targetModel;
				...
				NhisMedicineModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.CREATE_NHIS_MEDICINE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisMedicineModelActionTypes.CREATE_ALL_NHIS_MEDICINE_OK:
				const createdModels = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisMedicineModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.CREATE_ALL_NHIS_MEDICINE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisMedicineModelActionTypes.DELETE_NHIS_MEDICINE_OK:
				const deletedModelId = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.DELETE_ALL_NHIS_MEDICINE_OK:
				const deletedModelIds = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.DELETE_NHIS_MEDICINE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisMedicineModelActionTypes.UPDATE_NHIS_MEDICINE_OK:
				const updatedModel = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModel;
				...
				NhisMedicineModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.UPDATE_NHIS_MEDICINE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisMedicineModelActionTypes.UPDATE_ALL_NHIS_MEDICINE_OK:
				const updatedModels = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.UPDATE_ALL_NHIS_MEDICINE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisMedicineModelActionTypes.FETCH_NHIS_MEDICINE_OK:
				let fetchedModel = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModel;
				...
				NhisMedicineModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.FETCH_ALL_NHIS_MEDICINE_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisMedicineActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisMedicineModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.FETCH_NHIS_MEDICINE_AUDIT_OK:
				const audits = (action as modelAction.NhisMedicineActionOK).audits;
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.FETCH_NHIS_MEDICINE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisMedicineActionOK).audits;
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.FETCH_NHIS_MEDICINE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisMedicineModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.FETCH_LAST_NHIS_MEDICINE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisMedicineActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisMedicineModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.COUNT_NHIS_MEDICINES_OK:
				const totalCountAfterCount = (action as modelAction.NhisMedicineActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisMedicineModelActionTypes.INITIALISE_NHIS_MEDICINE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisMedicineActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisMedicineActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisMedicineCollectionState
					};
				}
