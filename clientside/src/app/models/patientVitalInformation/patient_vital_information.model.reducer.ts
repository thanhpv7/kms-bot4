...
import {initialPatientVitalInformationCollectionState} from './patient_vital_information.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientVitalInformationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientVitalInformationModelActionTypes.CREATE_PATIENT_VITAL_INFORMATION_OK:
				const createdModel = (action as modelAction.PatientVitalInformationActionOK ).stateConfig.targetModel;
				...
				PatientVitalInformationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.CREATE_PATIENT_VITAL_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVitalInformationModelActionTypes.CREATE_ALL_PATIENT_VITAL_INFORMATION_OK:
				const createdModels = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientVitalInformationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.CREATE_ALL_PATIENT_VITAL_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVitalInformationModelActionTypes.DELETE_PATIENT_VITAL_INFORMATION_OK:
				const deletedModelId = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.DELETE_ALL_PATIENT_VITAL_INFORMATION_OK:
				const deletedModelIds = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.DELETE_PATIENT_VITAL_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVitalInformationModelActionTypes.UPDATE_PATIENT_VITAL_INFORMATION_OK:
				const updatedModel = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModel;
				...
				PatientVitalInformationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.UPDATE_PATIENT_VITAL_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientVitalInformationModelActionTypes.UPDATE_ALL_PATIENT_VITAL_INFORMATION_OK:
				const updatedModels = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.UPDATE_ALL_PATIENT_VITAL_INFORMATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVitalInformationModelActionTypes.FETCH_PATIENT_VITAL_INFORMATION_OK:
				let fetchedModel = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModel;
				...
				PatientVitalInformationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.FETCH_ALL_PATIENT_VITAL_INFORMATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientVitalInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientVitalInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.FETCH_PATIENT_VITAL_INFORMATION_AUDIT_OK:
				const audits = (action as modelAction.PatientVitalInformationActionOK).audits;
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.FETCH_PATIENT_VITAL_INFORMATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientVitalInformationActionOK).audits;
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.FETCH_PATIENT_VITAL_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientVitalInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.FETCH_LAST_PATIENT_VITAL_INFORMATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientVitalInformationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientVitalInformationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.COUNT_PATIENT_VITAL_INFORMATIONS_OK:
				const totalCountAfterCount = (action as modelAction.PatientVitalInformationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientVitalInformationModelActionTypes.INITIALISE_PATIENT_VITAL_INFORMATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientVitalInformationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientVitalInformationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientVitalInformationCollectionState
					};
				}
