...
import {initialInternalOrderCollectionState} from './internal_order.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInternalOrderAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InternalOrderModelActionTypes.CREATE_INTERNAL_ORDER_OK:
				const createdModel = (action as modelAction.InternalOrderActionOK ).stateConfig.targetModel;
				...
				InternalOrderModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InternalOrderModelActionTypes.CREATE_INTERNAL_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderModelActionTypes.CREATE_ALL_INTERNAL_ORDER_OK:
				const createdModels = (action as modelAction.InternalOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InternalOrderModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InternalOrderModelActionTypes.CREATE_ALL_INTERNAL_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderModelActionTypes.DELETE_INTERNAL_ORDER_OK:
				const deletedModelId = (action as modelAction.InternalOrderActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InternalOrderModelActionTypes.DELETE_ALL_INTERNAL_ORDER_OK:
				const deletedModelIds = (action as modelAction.InternalOrderActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InternalOrderModelActionTypes.DELETE_INTERNAL_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderModelActionTypes.UPDATE_INTERNAL_ORDER_OK:
				const updatedModel = (action as modelAction.InternalOrderActionOK).stateConfig.targetModel;
				...
				InternalOrderModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderModelActionTypes.UPDATE_INTERNAL_ORDER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InternalOrderModelActionTypes.UPDATE_ALL_INTERNAL_ORDER_OK:
				const updatedModels = (action as modelAction.InternalOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InternalOrderModelActionTypes.UPDATE_ALL_INTERNAL_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderModelActionTypes.FETCH_INTERNAL_ORDER_OK:
				let fetchedModel = (action as modelAction.InternalOrderActionOK).stateConfig.targetModel;
				...
				InternalOrderModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderModelActionTypes.FETCH_ALL_INTERNAL_ORDER_OK:
				const totalCountAfterFetchAll = (action as modelAction.InternalOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InternalOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InternalOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InternalOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderModelActionTypes.FETCH_INTERNAL_ORDER_AUDIT_OK:
				const audits = (action as modelAction.InternalOrderActionOK).audits;
				...
				break;
			case modelAction.InternalOrderModelActionTypes.FETCH_INTERNAL_ORDER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InternalOrderActionOK).audits;
				...
				break;
			case modelAction.InternalOrderModelActionTypes.FETCH_INTERNAL_ORDER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InternalOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderModelActionTypes.FETCH_LAST_INTERNAL_ORDER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InternalOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InternalOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderModelActionTypes.COUNT_INTERNAL_ORDERS_OK:
				const totalCountAfterCount = (action as modelAction.InternalOrderActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InternalOrderModelActionTypes.INITIALISE_INTERNAL_ORDER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InternalOrderActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InternalOrderActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInternalOrderCollectionState
					};
				}
