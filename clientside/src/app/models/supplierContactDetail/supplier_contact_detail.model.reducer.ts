...
import {initialSupplierContactDetailCollectionState} from './supplier_contact_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSupplierContactDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SupplierContactDetailModelActionTypes.CREATE_SUPPLIER_CONTACT_DETAIL_OK:
				const createdModel = (action as modelAction.SupplierContactDetailActionOK ).stateConfig.targetModel;
				...
				SupplierContactDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.CREATE_SUPPLIER_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierContactDetailModelActionTypes.CREATE_ALL_SUPPLIER_CONTACT_DETAIL_OK:
				const createdModels = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SupplierContactDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.CREATE_ALL_SUPPLIER_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierContactDetailModelActionTypes.DELETE_SUPPLIER_CONTACT_DETAIL_OK:
				const deletedModelId = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.DELETE_ALL_SUPPLIER_CONTACT_DETAIL_OK:
				const deletedModelIds = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.DELETE_SUPPLIER_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierContactDetailModelActionTypes.UPDATE_SUPPLIER_CONTACT_DETAIL_OK:
				const updatedModel = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModel;
				...
				SupplierContactDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.UPDATE_SUPPLIER_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SupplierContactDetailModelActionTypes.UPDATE_ALL_SUPPLIER_CONTACT_DETAIL_OK:
				const updatedModels = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.UPDATE_ALL_SUPPLIER_CONTACT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierContactDetailModelActionTypes.FETCH_SUPPLIER_CONTACT_DETAIL_OK:
				let fetchedModel = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModel;
				...
				SupplierContactDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.FETCH_ALL_SUPPLIER_CONTACT_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SupplierContactDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SupplierContactDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.FETCH_SUPPLIER_CONTACT_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.SupplierContactDetailActionOK).audits;
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.FETCH_SUPPLIER_CONTACT_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SupplierContactDetailActionOK).audits;
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.FETCH_SUPPLIER_CONTACT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SupplierContactDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.FETCH_LAST_SUPPLIER_CONTACT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SupplierContactDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SupplierContactDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.COUNT_SUPPLIER_CONTACT_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.SupplierContactDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SupplierContactDetailModelActionTypes.INITIALISE_SUPPLIER_CONTACT_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SupplierContactDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SupplierContactDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialSupplierContactDetailCollectionState
					};
				}
