...
import {initialSatuSehatConfigurationCollectionState} from './satu_sehat_configuration.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSatuSehatConfigurationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SatuSehatConfigurationModelActionTypes.CREATE_SATU_SEHAT_CONFIGURATION_OK:
				const createdModel = (action as modelAction.SatuSehatConfigurationActionOK ).stateConfig.targetModel;
				...
				SatuSehatConfigurationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.CREATE_SATU_SEHAT_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.CREATE_ALL_SATU_SEHAT_CONFIGURATION_OK:
				const createdModels = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SatuSehatConfigurationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.CREATE_ALL_SATU_SEHAT_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.DELETE_SATU_SEHAT_CONFIGURATION_OK:
				const deletedModelId = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.DELETE_ALL_SATU_SEHAT_CONFIGURATION_OK:
				const deletedModelIds = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.DELETE_SATU_SEHAT_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.UPDATE_SATU_SEHAT_CONFIGURATION_OK:
				const updatedModel = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModel;
				...
				SatuSehatConfigurationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.UPDATE_SATU_SEHAT_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SatuSehatConfigurationModelActionTypes.UPDATE_ALL_SATU_SEHAT_CONFIGURATION_OK:
				const updatedModels = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.UPDATE_ALL_SATU_SEHAT_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.FETCH_SATU_SEHAT_CONFIGURATION_OK:
				let fetchedModel = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModel;
				...
				SatuSehatConfigurationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.FETCH_ALL_SATU_SEHAT_CONFIGURATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SatuSehatConfigurationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.FETCH_SATU_SEHAT_CONFIGURATION_AUDIT_OK:
				const audits = (action as modelAction.SatuSehatConfigurationActionOK).audits;
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.FETCH_SATU_SEHAT_CONFIGURATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SatuSehatConfigurationActionOK).audits;
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.FETCH_SATU_SEHAT_CONFIGURATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SatuSehatConfigurationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.FETCH_LAST_SATU_SEHAT_CONFIGURATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SatuSehatConfigurationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.COUNT_SATU_SEHAT_CONFIGURATIONS_OK:
				const totalCountAfterCount = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SatuSehatConfigurationModelActionTypes.INITIALISE_SATU_SEHAT_CONFIGURATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SatuSehatConfigurationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialSatuSehatConfigurationCollectionState
					};
				}
