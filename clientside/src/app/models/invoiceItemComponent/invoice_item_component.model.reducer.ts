...
import {initialInvoiceItemComponentCollectionState} from './invoice_item_component.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInvoiceItemComponentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InvoiceItemComponentModelActionTypes.CREATE_INVOICE_ITEM_COMPONENT_OK:
				const createdModel = (action as modelAction.InvoiceItemComponentActionOK ).stateConfig.targetModel;
				...
				InvoiceItemComponentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.CREATE_INVOICE_ITEM_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.CREATE_ALL_INVOICE_ITEM_COMPONENT_OK:
				const createdModels = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InvoiceItemComponentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.CREATE_ALL_INVOICE_ITEM_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.DELETE_INVOICE_ITEM_COMPONENT_OK:
				const deletedModelId = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.DELETE_ALL_INVOICE_ITEM_COMPONENT_OK:
				const deletedModelIds = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.DELETE_INVOICE_ITEM_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.UPDATE_INVOICE_ITEM_COMPONENT_OK:
				const updatedModel = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModel;
				...
				InvoiceItemComponentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.UPDATE_INVOICE_ITEM_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InvoiceItemComponentModelActionTypes.UPDATE_ALL_INVOICE_ITEM_COMPONENT_OK:
				const updatedModels = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.UPDATE_ALL_INVOICE_ITEM_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.FETCH_INVOICE_ITEM_COMPONENT_OK:
				let fetchedModel = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModel;
				...
				InvoiceItemComponentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.FETCH_ALL_INVOICE_ITEM_COMPONENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InvoiceItemComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.FETCH_INVOICE_ITEM_COMPONENT_AUDIT_OK:
				const audits = (action as modelAction.InvoiceItemComponentActionOK).audits;
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.FETCH_INVOICE_ITEM_COMPONENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InvoiceItemComponentActionOK).audits;
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.FETCH_INVOICE_ITEM_COMPONENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InvoiceItemComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.FETCH_LAST_INVOICE_ITEM_COMPONENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InvoiceItemComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.COUNT_INVOICE_ITEM_COMPONENTS_OK:
				const totalCountAfterCount = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InvoiceItemComponentModelActionTypes.INITIALISE_INVOICE_ITEM_COMPONENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InvoiceItemComponentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InvoiceItemComponentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInvoiceItemComponentCollectionState
					};
				}
