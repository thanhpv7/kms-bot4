...
import {initialMedicalExaminationRecordCollectionState} from './medical_examination_record.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicalExaminationRecordAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicalExaminationRecordModelActionTypes.CREATE_MEDICAL_EXAMINATION_RECORD_OK:
				const createdModel = (action as modelAction.MedicalExaminationRecordActionOK ).stateConfig.targetModel;
				...
				MedicalExaminationRecordModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.CREATE_MEDICAL_EXAMINATION_RECORD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.CREATE_ALL_MEDICAL_EXAMINATION_RECORD_OK:
				const createdModels = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicalExaminationRecordModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.CREATE_ALL_MEDICAL_EXAMINATION_RECORD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.DELETE_MEDICAL_EXAMINATION_RECORD_OK:
				const deletedModelId = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.DELETE_ALL_MEDICAL_EXAMINATION_RECORD_OK:
				const deletedModelIds = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.DELETE_MEDICAL_EXAMINATION_RECORD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.UPDATE_MEDICAL_EXAMINATION_RECORD_OK:
				const updatedModel = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModel;
				...
				MedicalExaminationRecordModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.UPDATE_MEDICAL_EXAMINATION_RECORD_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicalExaminationRecordModelActionTypes.UPDATE_ALL_MEDICAL_EXAMINATION_RECORD_OK:
				const updatedModels = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.UPDATE_ALL_MEDICAL_EXAMINATION_RECORD_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_OK:
				let fetchedModel = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModel;
				...
				MedicalExaminationRecordModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.FETCH_ALL_MEDICAL_EXAMINATION_RECORD_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicalExaminationRecordModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_AUDIT_OK:
				const audits = (action as modelAction.MedicalExaminationRecordActionOK).audits;
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicalExaminationRecordActionOK).audits;
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.FETCH_MEDICAL_EXAMINATION_RECORD_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicalExaminationRecordModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.FETCH_LAST_MEDICAL_EXAMINATION_RECORD_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicalExaminationRecordModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.COUNT_MEDICAL_EXAMINATION_RECORDS_OK:
				const totalCountAfterCount = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicalExaminationRecordModelActionTypes.INITIALISE_MEDICAL_EXAMINATION_RECORD_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicalExaminationRecordActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicalExaminationRecordCollectionState
					};
				}
