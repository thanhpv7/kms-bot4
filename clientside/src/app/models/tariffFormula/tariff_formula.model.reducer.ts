...
import {initialTariffFormulaCollectionState} from './tariff_formula.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTariffFormulaAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TariffFormulaModelActionTypes.CREATE_TARIFF_FORMULA_OK:
				const createdModel = (action as modelAction.TariffFormulaActionOK ).stateConfig.targetModel;
				...
				TariffFormulaModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.CREATE_TARIFF_FORMULA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffFormulaModelActionTypes.CREATE_ALL_TARIFF_FORMULA_OK:
				const createdModels = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TariffFormulaModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.CREATE_ALL_TARIFF_FORMULA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffFormulaModelActionTypes.DELETE_TARIFF_FORMULA_OK:
				const deletedModelId = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.DELETE_ALL_TARIFF_FORMULA_OK:
				const deletedModelIds = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.DELETE_TARIFF_FORMULA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffFormulaModelActionTypes.UPDATE_TARIFF_FORMULA_OK:
				const updatedModel = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModel;
				...
				TariffFormulaModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.UPDATE_TARIFF_FORMULA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TariffFormulaModelActionTypes.UPDATE_ALL_TARIFF_FORMULA_OK:
				const updatedModels = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.UPDATE_ALL_TARIFF_FORMULA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffFormulaModelActionTypes.FETCH_TARIFF_FORMULA_OK:
				let fetchedModel = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModel;
				...
				TariffFormulaModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.FETCH_ALL_TARIFF_FORMULA_OK:
				const totalCountAfterFetchAll = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TariffFormulaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TariffFormulaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.FETCH_TARIFF_FORMULA_AUDIT_OK:
				const audits = (action as modelAction.TariffFormulaActionOK).audits;
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.FETCH_TARIFF_FORMULA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TariffFormulaActionOK).audits;
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.FETCH_TARIFF_FORMULA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TariffFormulaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.FETCH_LAST_TARIFF_FORMULA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TariffFormulaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TariffFormulaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.COUNT_TARIFF_FORMULAS_OK:
				const totalCountAfterCount = (action as modelAction.TariffFormulaActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TariffFormulaModelActionTypes.INITIALISE_TARIFF_FORMULA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TariffFormulaActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TariffFormulaActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTariffFormulaCollectionState
					};
				}
