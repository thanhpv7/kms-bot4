...
import {initialPcareStatusPulangCollectionState} from './pcare_status_pulang.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareStatusPulangAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareStatusPulangModelActionTypes.CREATE_PCARE_STATUS_PULANG_OK:
				const createdModel = (action as modelAction.PcareStatusPulangActionOK ).stateConfig.targetModel;
				...
				PcareStatusPulangModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.CREATE_PCARE_STATUS_PULANG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareStatusPulangModelActionTypes.CREATE_ALL_PCARE_STATUS_PULANG_OK:
				const createdModels = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareStatusPulangModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.CREATE_ALL_PCARE_STATUS_PULANG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareStatusPulangModelActionTypes.DELETE_PCARE_STATUS_PULANG_OK:
				const deletedModelId = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.DELETE_ALL_PCARE_STATUS_PULANG_OK:
				const deletedModelIds = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.DELETE_PCARE_STATUS_PULANG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareStatusPulangModelActionTypes.UPDATE_PCARE_STATUS_PULANG_OK:
				const updatedModel = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModel;
				...
				PcareStatusPulangModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.UPDATE_PCARE_STATUS_PULANG_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareStatusPulangModelActionTypes.UPDATE_ALL_PCARE_STATUS_PULANG_OK:
				const updatedModels = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.UPDATE_ALL_PCARE_STATUS_PULANG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareStatusPulangModelActionTypes.FETCH_PCARE_STATUS_PULANG_OK:
				let fetchedModel = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModel;
				...
				PcareStatusPulangModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.FETCH_ALL_PCARE_STATUS_PULANG_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareStatusPulangActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareStatusPulangModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.FETCH_PCARE_STATUS_PULANG_AUDIT_OK:
				const audits = (action as modelAction.PcareStatusPulangActionOK).audits;
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.FETCH_PCARE_STATUS_PULANG_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareStatusPulangActionOK).audits;
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.FETCH_PCARE_STATUS_PULANG_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareStatusPulangModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.FETCH_LAST_PCARE_STATUS_PULANG_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareStatusPulangActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareStatusPulangModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.COUNT_PCARE_STATUS_PULANGS_OK:
				const totalCountAfterCount = (action as modelAction.PcareStatusPulangActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareStatusPulangModelActionTypes.INITIALISE_PCARE_STATUS_PULANG_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareStatusPulangActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareStatusPulangActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareStatusPulangCollectionState
					};
				}
