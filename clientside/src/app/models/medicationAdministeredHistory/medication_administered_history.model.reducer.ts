...
import {initialMedicationAdministeredHistoryCollectionState} from './medication_administered_history.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicationAdministeredHistoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicationAdministeredHistoryModelActionTypes.CREATE_MEDICATION_ADMINISTERED_HISTORY_OK:
				const createdModel = (action as modelAction.MedicationAdministeredHistoryActionOK ).stateConfig.targetModel;
				...
				MedicationAdministeredHistoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.CREATE_MEDICATION_ADMINISTERED_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.CREATE_ALL_MEDICATION_ADMINISTERED_HISTORY_OK:
				const createdModels = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicationAdministeredHistoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.CREATE_ALL_MEDICATION_ADMINISTERED_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.DELETE_MEDICATION_ADMINISTERED_HISTORY_OK:
				const deletedModelId = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.DELETE_ALL_MEDICATION_ADMINISTERED_HISTORY_OK:
				const deletedModelIds = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.DELETE_MEDICATION_ADMINISTERED_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.UPDATE_MEDICATION_ADMINISTERED_HISTORY_OK:
				const updatedModel = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModel;
				...
				MedicationAdministeredHistoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.UPDATE_MEDICATION_ADMINISTERED_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicationAdministeredHistoryModelActionTypes.UPDATE_ALL_MEDICATION_ADMINISTERED_HISTORY_OK:
				const updatedModels = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.UPDATE_ALL_MEDICATION_ADMINISTERED_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_MEDICATION_ADMINISTERED_HISTORY_OK:
				let fetchedModel = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModel;
				...
				MedicationAdministeredHistoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_ALL_MEDICATION_ADMINISTERED_HISTORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicationAdministeredHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_MEDICATION_ADMINISTERED_HISTORY_AUDIT_OK:
				const audits = (action as modelAction.MedicationAdministeredHistoryActionOK).audits;
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_MEDICATION_ADMINISTERED_HISTORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicationAdministeredHistoryActionOK).audits;
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_MEDICATION_ADMINISTERED_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicationAdministeredHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.FETCH_LAST_MEDICATION_ADMINISTERED_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicationAdministeredHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.COUNT_MEDICATION_ADMINISTERED_HISTORYS_OK:
				const totalCountAfterCount = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicationAdministeredHistoryModelActionTypes.INITIALISE_MEDICATION_ADMINISTERED_HISTORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicationAdministeredHistoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicationAdministeredHistoryCollectionState
					};
				}
