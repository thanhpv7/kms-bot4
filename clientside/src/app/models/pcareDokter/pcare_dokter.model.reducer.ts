...
import {initialPcareDokterCollectionState} from './pcare_dokter.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareDokterAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareDokterModelActionTypes.CREATE_PCARE_DOKTER_OK:
				const createdModel = (action as modelAction.PcareDokterActionOK ).stateConfig.targetModel;
				...
				PcareDokterModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareDokterModelActionTypes.CREATE_PCARE_DOKTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDokterModelActionTypes.CREATE_ALL_PCARE_DOKTER_OK:
				const createdModels = (action as modelAction.PcareDokterActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareDokterModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareDokterModelActionTypes.CREATE_ALL_PCARE_DOKTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDokterModelActionTypes.DELETE_PCARE_DOKTER_OK:
				const deletedModelId = (action as modelAction.PcareDokterActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareDokterModelActionTypes.DELETE_ALL_PCARE_DOKTER_OK:
				const deletedModelIds = (action as modelAction.PcareDokterActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareDokterModelActionTypes.DELETE_PCARE_DOKTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDokterModelActionTypes.UPDATE_PCARE_DOKTER_OK:
				const updatedModel = (action as modelAction.PcareDokterActionOK).stateConfig.targetModel;
				...
				PcareDokterModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDokterModelActionTypes.UPDATE_PCARE_DOKTER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareDokterModelActionTypes.UPDATE_ALL_PCARE_DOKTER_OK:
				const updatedModels = (action as modelAction.PcareDokterActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareDokterModelActionTypes.UPDATE_ALL_PCARE_DOKTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDokterModelActionTypes.FETCH_PCARE_DOKTER_OK:
				let fetchedModel = (action as modelAction.PcareDokterActionOK).stateConfig.targetModel;
				...
				PcareDokterModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDokterModelActionTypes.FETCH_ALL_PCARE_DOKTER_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareDokterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareDokterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareDokterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareDokterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDokterModelActionTypes.FETCH_PCARE_DOKTER_AUDIT_OK:
				const audits = (action as modelAction.PcareDokterActionOK).audits;
				...
				break;
			case modelAction.PcareDokterModelActionTypes.FETCH_PCARE_DOKTER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareDokterActionOK).audits;
				...
				break;
			case modelAction.PcareDokterModelActionTypes.FETCH_PCARE_DOKTER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareDokterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDokterModelActionTypes.FETCH_LAST_PCARE_DOKTER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareDokterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareDokterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDokterModelActionTypes.COUNT_PCARE_DOKTERS_OK:
				const totalCountAfterCount = (action as modelAction.PcareDokterActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareDokterModelActionTypes.INITIALISE_PCARE_DOKTER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareDokterActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareDokterActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareDokterCollectionState
					};
				}
