...
import * as modelAction from './supplier.model.action';
import {ModelState} from '../model.state';
import {initialSupplierCollectionState} from './supplier.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSupplierAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SupplierModelActionTypes.CREATE_SUPPLIER_OK:
				const createdModel = (action as modelAction.SupplierActionOK ).stateConfig.targetModel;
				...
				SupplierModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SupplierModelActionTypes.CREATE_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierModelActionTypes.CREATE_ALL_SUPPLIER_OK:
				const createdModels = (action as modelAction.SupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SupplierModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SupplierModelActionTypes.CREATE_ALL_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierModelActionTypes.DELETE_SUPPLIER_OK:
				const deletedModelId = (action as modelAction.SupplierActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierModelActionTypes.DELETE_ALL_SUPPLIER_OK:
				const deletedModelIds = (action as modelAction.SupplierActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierModelActionTypes.DELETE_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierModelActionTypes.UPDATE_SUPPLIER_OK:
				const updatedModel = (action as modelAction.SupplierActionOK).stateConfig.targetModel;
				...
				SupplierModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierModelActionTypes.UPDATE_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SupplierModelActionTypes.UPDATE_ALL_SUPPLIER_OK:
				const updatedModels = (action as modelAction.SupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierModelActionTypes.UPDATE_ALL_SUPPLIER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierModelActionTypes.FETCH_SUPPLIER_OK:
				let fetchedModel = (action as modelAction.SupplierActionOK).stateConfig.targetModel;
				...
				SupplierModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierModelActionTypes.FETCH_ALL_SUPPLIER_OK:
				const totalCountAfterFetchAll = (action as modelAction.SupplierActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SupplierActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SupplierModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierModelActionTypes.FETCH_SUPPLIER_AUDIT_OK:
				const audits = (action as modelAction.SupplierActionOK).audits;
				...
				break;
			case modelAction.SupplierModelActionTypes.FETCH_SUPPLIER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SupplierActionOK).audits;
				...
				break;
			case modelAction.SupplierModelActionTypes.FETCH_SUPPLIER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SupplierActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SupplierActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SupplierModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierModelActionTypes.FETCH_LAST_SUPPLIER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SupplierActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SupplierActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SupplierActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SupplierModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierModelActionTypes.COUNT_SUPPLIERS_OK:
				const totalCountAfterCount = (action as modelAction.SupplierActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SupplierModelActionTypes.INITIALISE_SUPPLIER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SupplierActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SupplierActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialSupplierCollectionState
					};
				}
