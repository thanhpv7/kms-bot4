...
import {initialPcareActionExaminationCollectionState} from './pcare_action_examination.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareActionExaminationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareActionExaminationModelActionTypes.CREATE_PCARE_ACTION_EXAMINATION_OK:
				const createdModel = (action as modelAction.PcareActionExaminationActionOK ).stateConfig.targetModel;
				...
				PcareActionExaminationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.CREATE_PCARE_ACTION_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareActionExaminationModelActionTypes.CREATE_ALL_PCARE_ACTION_EXAMINATION_OK:
				const createdModels = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareActionExaminationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.CREATE_ALL_PCARE_ACTION_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareActionExaminationModelActionTypes.DELETE_PCARE_ACTION_EXAMINATION_OK:
				const deletedModelId = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.DELETE_ALL_PCARE_ACTION_EXAMINATION_OK:
				const deletedModelIds = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.DELETE_PCARE_ACTION_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareActionExaminationModelActionTypes.UPDATE_PCARE_ACTION_EXAMINATION_OK:
				const updatedModel = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModel;
				...
				PcareActionExaminationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.UPDATE_PCARE_ACTION_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareActionExaminationModelActionTypes.UPDATE_ALL_PCARE_ACTION_EXAMINATION_OK:
				const updatedModels = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.UPDATE_ALL_PCARE_ACTION_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareActionExaminationModelActionTypes.FETCH_PCARE_ACTION_EXAMINATION_OK:
				let fetchedModel = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModel;
				...
				PcareActionExaminationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.FETCH_ALL_PCARE_ACTION_EXAMINATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareActionExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareActionExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.FETCH_PCARE_ACTION_EXAMINATION_AUDIT_OK:
				const audits = (action as modelAction.PcareActionExaminationActionOK).audits;
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.FETCH_PCARE_ACTION_EXAMINATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareActionExaminationActionOK).audits;
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.FETCH_PCARE_ACTION_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareActionExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.FETCH_LAST_PCARE_ACTION_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareActionExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareActionExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.COUNT_PCARE_ACTION_EXAMINATIONS_OK:
				const totalCountAfterCount = (action as modelAction.PcareActionExaminationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareActionExaminationModelActionTypes.INITIALISE_PCARE_ACTION_EXAMINATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareActionExaminationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareActionExaminationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareActionExaminationCollectionState
					};
				}
