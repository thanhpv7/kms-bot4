...
import {initialBpjsDiagnosePRBCollectionState} from './bpjs_diagnose_prb.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsDiagnosePRBAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsDiagnosePRBModelActionTypes.CREATE_BPJS_DIAGNOSE_PRB_OK:
				const createdModel = (action as modelAction.BpjsDiagnosePRBActionOK ).stateConfig.targetModel;
				...
				BpjsDiagnosePRBModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.CREATE_BPJS_DIAGNOSE_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.CREATE_ALL_BPJS_DIAGNOSE_PRB_OK:
				const createdModels = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsDiagnosePRBModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.CREATE_ALL_BPJS_DIAGNOSE_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.DELETE_BPJS_DIAGNOSE_PRB_OK:
				const deletedModelId = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.DELETE_ALL_BPJS_DIAGNOSE_PRB_OK:
				const deletedModelIds = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.DELETE_BPJS_DIAGNOSE_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.UPDATE_BPJS_DIAGNOSE_PRB_OK:
				const updatedModel = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModel;
				...
				BpjsDiagnosePRBModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.UPDATE_BPJS_DIAGNOSE_PRB_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsDiagnosePRBModelActionTypes.UPDATE_ALL_BPJS_DIAGNOSE_PRB_OK:
				const updatedModels = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.UPDATE_ALL_BPJS_DIAGNOSE_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_BPJS_DIAGNOSE_PRB_OK:
				let fetchedModel = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModel;
				...
				BpjsDiagnosePRBModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_ALL_BPJS_DIAGNOSE_PRB_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsDiagnosePRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_BPJS_DIAGNOSE_PRB_AUDIT_OK:
				const audits = (action as modelAction.BpjsDiagnosePRBActionOK).audits;
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_BPJS_DIAGNOSE_PRB_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsDiagnosePRBActionOK).audits;
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_BPJS_DIAGNOSE_PRB_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsDiagnosePRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.FETCH_LAST_BPJS_DIAGNOSE_PRB_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsDiagnosePRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.COUNT_BPJS_DIAGNOSE_PRBS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsDiagnosePRBModelActionTypes.INITIALISE_BPJS_DIAGNOSE_PRB_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsDiagnosePRBActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsDiagnosePRBCollectionState
					};
				}
