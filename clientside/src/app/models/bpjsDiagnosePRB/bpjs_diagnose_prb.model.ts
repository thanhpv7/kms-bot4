 ...
 */
export class BpjsDiagnosePRBModel extends AbstractModel {
	/**
	...
	readonly className = 'BpjsDiagnosePRBModel';
	...
	get valueToDisplay(): string {
		...
		return BpjsDiagnosePRBModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BpjsDiagnosePRBModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof BpjsDiagnosePRBModel]?: BpjsDiagnosePRBModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BpjsDiagnosePRBModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bpjsDiagnosePRBModel = new BpjsDiagnosePRBModel(data);`
	 *
	 * @param data The input data to be initialised as the BpjsDiagnosePRBModel,
	 *    it is expected as a JSON string or as a nullable BpjsDiagnosePRBModel.
	 */
	constructor(data?: string | Partial<BpjsDiagnosePRBModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BpjsDiagnosePRBModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BpjsDiagnosePRBModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BpjsDiagnosePRBModel {
		...
		return new BpjsDiagnosePRBModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BpjsDiagnosePRBModel)) {
			return {};
		}
