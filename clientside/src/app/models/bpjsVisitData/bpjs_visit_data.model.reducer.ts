...
import {initialBpjsVisitDataCollectionState} from './bpjs_visit_data.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsVisitDataAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsVisitDataModelActionTypes.CREATE_BPJS_VISIT_DATA_OK:
				const createdModel = (action as modelAction.BpjsVisitDataActionOK ).stateConfig.targetModel;
				...
				BpjsVisitDataModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.CREATE_BPJS_VISIT_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsVisitDataModelActionTypes.CREATE_ALL_BPJS_VISIT_DATA_OK:
				const createdModels = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsVisitDataModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.CREATE_ALL_BPJS_VISIT_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsVisitDataModelActionTypes.DELETE_BPJS_VISIT_DATA_OK:
				const deletedModelId = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.DELETE_ALL_BPJS_VISIT_DATA_OK:
				const deletedModelIds = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.DELETE_BPJS_VISIT_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsVisitDataModelActionTypes.UPDATE_BPJS_VISIT_DATA_OK:
				const updatedModel = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModel;
				...
				BpjsVisitDataModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.UPDATE_BPJS_VISIT_DATA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsVisitDataModelActionTypes.UPDATE_ALL_BPJS_VISIT_DATA_OK:
				const updatedModels = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.UPDATE_ALL_BPJS_VISIT_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsVisitDataModelActionTypes.FETCH_BPJS_VISIT_DATA_OK:
				let fetchedModel = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModel;
				...
				BpjsVisitDataModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.FETCH_ALL_BPJS_VISIT_DATA_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsVisitDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsVisitDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.FETCH_BPJS_VISIT_DATA_AUDIT_OK:
				const audits = (action as modelAction.BpjsVisitDataActionOK).audits;
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.FETCH_BPJS_VISIT_DATA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsVisitDataActionOK).audits;
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.FETCH_BPJS_VISIT_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsVisitDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.FETCH_LAST_BPJS_VISIT_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsVisitDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsVisitDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.COUNT_BPJS_VISIT_DATAS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsVisitDataActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsVisitDataModelActionTypes.INITIALISE_BPJS_VISIT_DATA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsVisitDataActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsVisitDataActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsVisitDataCollectionState
					};
				}
