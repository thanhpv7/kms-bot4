 ...
 */
export class MedicalCertificateMentalHealthModel extends AbstractModel {
	/**
	...
	readonly className = 'MedicalCertificateMentalHealthModel';
	...
	get valueToDisplay(): string {
		...
		return MedicalCertificateMentalHealthModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = MedicalCertificateMentalHealthModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof MedicalCertificateMentalHealthModel]?: MedicalCertificateMentalHealthModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new MedicalCertificateMentalHealthModel(data);
		}
	 ...
	 * @example
	 *
	 * `let medicalCertificateMentalHealthModel = new MedicalCertificateMentalHealthModel(data);`
	 *
	 * @param data The input data to be initialised as the MedicalCertificateMentalHealthModel,
	 *    it is expected as a JSON string or as a nullable MedicalCertificateMentalHealthModel.
	 */
	constructor(data?: string | Partial<MedicalCertificateMentalHealthModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<MedicalCertificateMentalHealthModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		MedicalCertificateMentalHealthModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): MedicalCertificateMentalHealthModel {
		...
		return new MedicalCertificateMentalHealthModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof MedicalCertificateMentalHealthModel)) {
			return {};
		}
