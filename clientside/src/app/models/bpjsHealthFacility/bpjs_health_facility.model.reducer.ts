...
import {initialBpjsHealthFacilityCollectionState} from './bpjs_health_facility.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsHealthFacilityAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsHealthFacilityModelActionTypes.CREATE_BPJS_HEALTH_FACILITY_OK:
				const createdModel = (action as modelAction.BpjsHealthFacilityActionOK ).stateConfig.targetModel;
				...
				BpjsHealthFacilityModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.CREATE_BPJS_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.CREATE_ALL_BPJS_HEALTH_FACILITY_OK:
				const createdModels = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsHealthFacilityModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.CREATE_ALL_BPJS_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.DELETE_BPJS_HEALTH_FACILITY_OK:
				const deletedModelId = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.DELETE_ALL_BPJS_HEALTH_FACILITY_OK:
				const deletedModelIds = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.DELETE_BPJS_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.UPDATE_BPJS_HEALTH_FACILITY_OK:
				const updatedModel = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModel;
				...
				BpjsHealthFacilityModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.UPDATE_BPJS_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsHealthFacilityModelActionTypes.UPDATE_ALL_BPJS_HEALTH_FACILITY_OK:
				const updatedModels = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.UPDATE_ALL_BPJS_HEALTH_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.FETCH_BPJS_HEALTH_FACILITY_OK:
				let fetchedModel = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModel;
				...
				BpjsHealthFacilityModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.FETCH_ALL_BPJS_HEALTH_FACILITY_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsHealthFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.FETCH_BPJS_HEALTH_FACILITY_AUDIT_OK:
				const audits = (action as modelAction.BpjsHealthFacilityActionOK).audits;
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.FETCH_BPJS_HEALTH_FACILITY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsHealthFacilityActionOK).audits;
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.FETCH_BPJS_HEALTH_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsHealthFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.FETCH_LAST_BPJS_HEALTH_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsHealthFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.COUNT_BPJS_HEALTH_FACILITYS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsHealthFacilityModelActionTypes.INITIALISE_BPJS_HEALTH_FACILITY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsHealthFacilityActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsHealthFacilityCollectionState
					};
				}
