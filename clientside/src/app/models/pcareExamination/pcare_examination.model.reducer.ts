...
import {initialPcareExaminationCollectionState} from './pcare_examination.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareExaminationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareExaminationModelActionTypes.CREATE_PCARE_EXAMINATION_OK:
				const createdModel = (action as modelAction.PcareExaminationActionOK ).stateConfig.targetModel;
				...
				PcareExaminationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.CREATE_PCARE_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareExaminationModelActionTypes.CREATE_ALL_PCARE_EXAMINATION_OK:
				const createdModels = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareExaminationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.CREATE_ALL_PCARE_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareExaminationModelActionTypes.DELETE_PCARE_EXAMINATION_OK:
				const deletedModelId = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.DELETE_ALL_PCARE_EXAMINATION_OK:
				const deletedModelIds = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.DELETE_PCARE_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareExaminationModelActionTypes.UPDATE_PCARE_EXAMINATION_OK:
				const updatedModel = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModel;
				...
				PcareExaminationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.UPDATE_PCARE_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareExaminationModelActionTypes.UPDATE_ALL_PCARE_EXAMINATION_OK:
				const updatedModels = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.UPDATE_ALL_PCARE_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareExaminationModelActionTypes.FETCH_PCARE_EXAMINATION_OK:
				let fetchedModel = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModel;
				...
				PcareExaminationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.FETCH_ALL_PCARE_EXAMINATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.FETCH_PCARE_EXAMINATION_AUDIT_OK:
				const audits = (action as modelAction.PcareExaminationActionOK).audits;
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.FETCH_PCARE_EXAMINATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareExaminationActionOK).audits;
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.FETCH_PCARE_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.FETCH_LAST_PCARE_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.COUNT_PCARE_EXAMINATIONS_OK:
				const totalCountAfterCount = (action as modelAction.PcareExaminationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareExaminationModelActionTypes.INITIALISE_PCARE_EXAMINATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareExaminationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareExaminationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareExaminationCollectionState
					};
				}
