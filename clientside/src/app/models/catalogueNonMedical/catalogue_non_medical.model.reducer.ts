...
import {initialCatalogueNonMedicalCollectionState} from './catalogue_non_medical.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCatalogueNonMedicalAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CatalogueNonMedicalModelActionTypes.CREATE_CATALOGUE_NON_MEDICAL_OK:
				const createdModel = (action as modelAction.CatalogueNonMedicalActionOK ).stateConfig.targetModel;
				...
				CatalogueNonMedicalModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.CREATE_CATALOGUE_NON_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.CREATE_ALL_CATALOGUE_NON_MEDICAL_OK:
				const createdModels = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CatalogueNonMedicalModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.CREATE_ALL_CATALOGUE_NON_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.DELETE_CATALOGUE_NON_MEDICAL_OK:
				const deletedModelId = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.DELETE_ALL_CATALOGUE_NON_MEDICAL_OK:
				const deletedModelIds = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.DELETE_CATALOGUE_NON_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.UPDATE_CATALOGUE_NON_MEDICAL_OK:
				const updatedModel = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModel;
				...
				CatalogueNonMedicalModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.UPDATE_CATALOGUE_NON_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CatalogueNonMedicalModelActionTypes.UPDATE_ALL_CATALOGUE_NON_MEDICAL_OK:
				const updatedModels = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.UPDATE_ALL_CATALOGUE_NON_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.FETCH_CATALOGUE_NON_MEDICAL_OK:
				let fetchedModel = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModel;
				...
				CatalogueNonMedicalModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.FETCH_ALL_CATALOGUE_NON_MEDICAL_OK:
				const totalCountAfterFetchAll = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CatalogueNonMedicalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.FETCH_CATALOGUE_NON_MEDICAL_AUDIT_OK:
				const audits = (action as modelAction.CatalogueNonMedicalActionOK).audits;
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.FETCH_CATALOGUE_NON_MEDICAL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CatalogueNonMedicalActionOK).audits;
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.FETCH_CATALOGUE_NON_MEDICAL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CatalogueNonMedicalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.FETCH_LAST_CATALOGUE_NON_MEDICAL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CatalogueNonMedicalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.COUNT_CATALOGUE_NON_MEDICALS_OK:
				const totalCountAfterCount = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CatalogueNonMedicalModelActionTypes.INITIALISE_CATALOGUE_NON_MEDICAL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CatalogueNonMedicalActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCatalogueNonMedicalCollectionState
					};
				}
