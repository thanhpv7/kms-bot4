...
import {initialPurchaseOrderCollectionState} from './purchase_order.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePurchaseOrderAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PurchaseOrderModelActionTypes.CREATE_PURCHASE_ORDER_OK:
				const createdModel = (action as modelAction.PurchaseOrderActionOK ).stateConfig.targetModel;
				...
				PurchaseOrderModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.CREATE_PURCHASE_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderModelActionTypes.CREATE_ALL_PURCHASE_ORDER_OK:
				const createdModels = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PurchaseOrderModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.CREATE_ALL_PURCHASE_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderModelActionTypes.DELETE_PURCHASE_ORDER_OK:
				const deletedModelId = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.DELETE_ALL_PURCHASE_ORDER_OK:
				const deletedModelIds = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.DELETE_PURCHASE_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderModelActionTypes.UPDATE_PURCHASE_ORDER_OK:
				const updatedModel = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModel;
				...
				PurchaseOrderModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.UPDATE_PURCHASE_ORDER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PurchaseOrderModelActionTypes.UPDATE_ALL_PURCHASE_ORDER_OK:
				const updatedModels = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.UPDATE_ALL_PURCHASE_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderModelActionTypes.FETCH_PURCHASE_ORDER_OK:
				let fetchedModel = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModel;
				...
				PurchaseOrderModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.FETCH_ALL_PURCHASE_ORDER_OK:
				const totalCountAfterFetchAll = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PurchaseOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PurchaseOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.FETCH_PURCHASE_ORDER_AUDIT_OK:
				const audits = (action as modelAction.PurchaseOrderActionOK).audits;
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.FETCH_PURCHASE_ORDER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PurchaseOrderActionOK).audits;
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.FETCH_PURCHASE_ORDER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PurchaseOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.FETCH_LAST_PURCHASE_ORDER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PurchaseOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.COUNT_PURCHASE_ORDERS_OK:
				const totalCountAfterCount = (action as modelAction.PurchaseOrderActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PurchaseOrderModelActionTypes.INITIALISE_PURCHASE_ORDER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PurchaseOrderActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PurchaseOrderActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPurchaseOrderCollectionState
					};
				}
