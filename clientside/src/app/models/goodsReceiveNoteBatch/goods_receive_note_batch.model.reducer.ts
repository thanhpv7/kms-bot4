...
import {initialGoodsReceiveNoteBatchCollectionState} from './goods_receive_note_batch.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseGoodsReceiveNoteBatchAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.CREATE_GOODS_RECEIVE_NOTE_BATCH_OK:
				const createdModel = (action as modelAction.GoodsReceiveNoteBatchActionOK ).stateConfig.targetModel;
				...
				GoodsReceiveNoteBatchModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.CREATE_GOODS_RECEIVE_NOTE_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.CREATE_ALL_GOODS_RECEIVE_NOTE_BATCH_OK:
				const createdModels = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					GoodsReceiveNoteBatchModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.CREATE_ALL_GOODS_RECEIVE_NOTE_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_BATCH_OK:
				const deletedModelId = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.DELETE_ALL_GOODS_RECEIVE_NOTE_BATCH_OK:
				const deletedModelIds = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.DELETE_GOODS_RECEIVE_NOTE_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE_BATCH_OK:
				const updatedModel = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModel;
				...
				GoodsReceiveNoteBatchModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.UPDATE_GOODS_RECEIVE_NOTE_BATCH_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.GoodsReceiveNoteBatchModelActionTypes.UPDATE_ALL_GOODS_RECEIVE_NOTE_BATCH_OK:
				const updatedModels = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.UPDATE_ALL_GOODS_RECEIVE_NOTE_BATCH_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_BATCH_OK:
				let fetchedModel = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModel;
				...
				GoodsReceiveNoteBatchModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_ALL_GOODS_RECEIVE_NOTE_BATCH_OK:
				const totalCountAfterFetchAll = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => GoodsReceiveNoteBatchModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_BATCH_AUDIT_OK:
				const audits = (action as modelAction.GoodsReceiveNoteBatchActionOK).audits;
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_BATCH_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.GoodsReceiveNoteBatchActionOK).audits;
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_GOODS_RECEIVE_NOTE_BATCH_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => GoodsReceiveNoteBatchModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.FETCH_LAST_GOODS_RECEIVE_NOTE_BATCH_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => GoodsReceiveNoteBatchModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.COUNT_GOODS_RECEIVE_NOTE_BATCHS_OK:
				const totalCountAfterCount = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.GoodsReceiveNoteBatchModelActionTypes.INITIALISE_GOODS_RECEIVE_NOTE_BATCH_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.GoodsReceiveNoteBatchActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialGoodsReceiveNoteBatchCollectionState
					};
				}
