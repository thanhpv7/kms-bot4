...
import {initialBatchStockBalanceCollectionState} from './batch_stock_balance.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBatchStockBalanceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BatchStockBalanceModelActionTypes.CREATE_BATCH_STOCK_BALANCE_OK:
				const createdModel = (action as modelAction.BatchStockBalanceActionOK ).stateConfig.targetModel;
				...
				BatchStockBalanceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.CREATE_BATCH_STOCK_BALANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalanceModelActionTypes.CREATE_ALL_BATCH_STOCK_BALANCE_OK:
				const createdModels = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BatchStockBalanceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.CREATE_ALL_BATCH_STOCK_BALANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalanceModelActionTypes.DELETE_BATCH_STOCK_BALANCE_OK:
				const deletedModelId = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.DELETE_ALL_BATCH_STOCK_BALANCE_OK:
				const deletedModelIds = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.DELETE_BATCH_STOCK_BALANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalanceModelActionTypes.UPDATE_BATCH_STOCK_BALANCE_OK:
				const updatedModel = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModel;
				...
				BatchStockBalanceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.UPDATE_BATCH_STOCK_BALANCE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BatchStockBalanceModelActionTypes.UPDATE_ALL_BATCH_STOCK_BALANCE_OK:
				const updatedModels = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.UPDATE_ALL_BATCH_STOCK_BALANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalanceModelActionTypes.FETCH_BATCH_STOCK_BALANCE_OK:
				let fetchedModel = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModel;
				...
				BatchStockBalanceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.FETCH_ALL_BATCH_STOCK_BALANCE_OK:
				const totalCountAfterFetchAll = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BatchStockBalanceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BatchStockBalanceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.FETCH_BATCH_STOCK_BALANCE_AUDIT_OK:
				const audits = (action as modelAction.BatchStockBalanceActionOK).audits;
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.FETCH_BATCH_STOCK_BALANCE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BatchStockBalanceActionOK).audits;
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.FETCH_BATCH_STOCK_BALANCE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BatchStockBalanceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.FETCH_LAST_BATCH_STOCK_BALANCE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BatchStockBalanceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BatchStockBalanceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.COUNT_BATCH_STOCK_BALANCES_OK:
				const totalCountAfterCount = (action as modelAction.BatchStockBalanceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BatchStockBalanceModelActionTypes.INITIALISE_BATCH_STOCK_BALANCE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BatchStockBalanceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BatchStockBalanceActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBatchStockBalanceCollectionState
					};
				}
