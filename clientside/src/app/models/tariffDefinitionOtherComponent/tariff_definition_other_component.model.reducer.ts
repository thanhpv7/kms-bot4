...
import {initialTariffDefinitionOtherComponentCollectionState} from './tariff_definition_other_component.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTariffDefinitionOtherComponentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.CREATE_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				const createdModel = (action as modelAction.TariffDefinitionOtherComponentActionOK ).stateConfig.targetModel;
				...
				TariffDefinitionOtherComponentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.CREATE_TARIFF_DEFINITION_OTHER_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.CREATE_ALL_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				const createdModels = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TariffDefinitionOtherComponentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.CREATE_ALL_TARIFF_DEFINITION_OTHER_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.DELETE_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				const deletedModelId = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.DELETE_ALL_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				const deletedModelIds = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.DELETE_TARIFF_DEFINITION_OTHER_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.UPDATE_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				const updatedModel = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModel;
				...
				TariffDefinitionOtherComponentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.UPDATE_TARIFF_DEFINITION_OTHER_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TariffDefinitionOtherComponentModelActionTypes.UPDATE_ALL_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				const updatedModels = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.UPDATE_ALL_TARIFF_DEFINITION_OTHER_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				let fetchedModel = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModel;
				...
				TariffDefinitionOtherComponentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_ALL_TARIFF_DEFINITION_OTHER_COMPONENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TariffDefinitionOtherComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_TARIFF_DEFINITION_OTHER_COMPONENT_AUDIT_OK:
				const audits = (action as modelAction.TariffDefinitionOtherComponentActionOK).audits;
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_TARIFF_DEFINITION_OTHER_COMPONENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TariffDefinitionOtherComponentActionOK).audits;
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_TARIFF_DEFINITION_OTHER_COMPONENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TariffDefinitionOtherComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.FETCH_LAST_TARIFF_DEFINITION_OTHER_COMPONENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TariffDefinitionOtherComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.COUNT_TARIFF_DEFINITION_OTHER_COMPONENTS_OK:
				const totalCountAfterCount = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TariffDefinitionOtherComponentModelActionTypes.INITIALISE_TARIFF_DEFINITION_OTHER_COMPONENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TariffDefinitionOtherComponentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTariffDefinitionOtherComponentCollectionState
					};
				}
