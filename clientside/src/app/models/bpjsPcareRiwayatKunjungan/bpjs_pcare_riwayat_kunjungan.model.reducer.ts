...
import {initialBpjsPcareRiwayatKunjunganCollectionState} from './bpjs_pcare_riwayat_kunjungan.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcareRiwayatKunjunganAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.CREATE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				const createdModel = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK ).stateConfig.targetModel;
				...
				BpjsPcareRiwayatKunjunganModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.CREATE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.CREATE_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				const createdModels = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcareRiwayatKunjunganModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.CREATE_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.DELETE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				const deletedModelId = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.DELETE_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				const deletedModelIds = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.DELETE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.UPDATE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				const updatedModel = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModel;
				...
				BpjsPcareRiwayatKunjunganModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.UPDATE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.UPDATE_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				const updatedModels = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.UPDATE_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				let fetchedModel = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModel;
				...
				BpjsPcareRiwayatKunjunganModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_ALL_BPJS_PCARE_RIWAYAT_KUNJUNGAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcareRiwayatKunjunganModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_BPJS_PCARE_RIWAYAT_KUNJUNGAN_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_BPJS_PCARE_RIWAYAT_KUNJUNGAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_BPJS_PCARE_RIWAYAT_KUNJUNGAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcareRiwayatKunjunganModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.FETCH_LAST_BPJS_PCARE_RIWAYAT_KUNJUNGAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcareRiwayatKunjunganModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.COUNT_BPJS_PCARE_RIWAYAT_KUNJUNGANS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcareRiwayatKunjunganModelActionTypes.INITIALISE_BPJS_PCARE_RIWAYAT_KUNJUNGAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcareRiwayatKunjunganActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcareRiwayatKunjunganCollectionState
					};
				}
