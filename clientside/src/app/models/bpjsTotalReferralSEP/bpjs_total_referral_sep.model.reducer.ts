...
import {initialBpjsTotalReferralSEPCollectionState} from './bpjs_total_referral_sep.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsTotalReferralSEPAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsTotalReferralSEPModelActionTypes.CREATE_BPJS_TOTAL_REFERRAL_SEP_OK:
				const createdModel = (action as modelAction.BpjsTotalReferralSEPActionOK ).stateConfig.targetModel;
				...
				BpjsTotalReferralSEPModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.CREATE_BPJS_TOTAL_REFERRAL_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.CREATE_ALL_BPJS_TOTAL_REFERRAL_SEP_OK:
				const createdModels = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsTotalReferralSEPModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.CREATE_ALL_BPJS_TOTAL_REFERRAL_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.DELETE_BPJS_TOTAL_REFERRAL_SEP_OK:
				const deletedModelId = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.DELETE_ALL_BPJS_TOTAL_REFERRAL_SEP_OK:
				const deletedModelIds = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.DELETE_BPJS_TOTAL_REFERRAL_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.UPDATE_BPJS_TOTAL_REFERRAL_SEP_OK:
				const updatedModel = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModel;
				...
				BpjsTotalReferralSEPModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.UPDATE_BPJS_TOTAL_REFERRAL_SEP_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsTotalReferralSEPModelActionTypes.UPDATE_ALL_BPJS_TOTAL_REFERRAL_SEP_OK:
				const updatedModels = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.UPDATE_ALL_BPJS_TOTAL_REFERRAL_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_BPJS_TOTAL_REFERRAL_SEP_OK:
				let fetchedModel = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModel;
				...
				BpjsTotalReferralSEPModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_ALL_BPJS_TOTAL_REFERRAL_SEP_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsTotalReferralSEPModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_BPJS_TOTAL_REFERRAL_SEP_AUDIT_OK:
				const audits = (action as modelAction.BpjsTotalReferralSEPActionOK).audits;
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_BPJS_TOTAL_REFERRAL_SEP_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsTotalReferralSEPActionOK).audits;
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_BPJS_TOTAL_REFERRAL_SEP_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsTotalReferralSEPModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.FETCH_LAST_BPJS_TOTAL_REFERRAL_SEP_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsTotalReferralSEPModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.COUNT_BPJS_TOTAL_REFERRAL_SEPS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsTotalReferralSEPModelActionTypes.INITIALISE_BPJS_TOTAL_REFERRAL_SEP_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsTotalReferralSEPActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsTotalReferralSEPCollectionState
					};
				}
