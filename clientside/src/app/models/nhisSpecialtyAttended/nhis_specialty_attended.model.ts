 ...
 */
export class NhisSpecialtyAttendedModel extends AbstractModel {
	/**
	...
	readonly className = 'NhisSpecialtyAttendedModel';
	...
	get valueToDisplay(): string {
		...
		return NhisSpecialtyAttendedModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = NhisSpecialtyAttendedModel.modelPropGroups;
	...
	static getProps(): ModelProperty[] {
		return super.getProps().concat([
			...
			{
				name: 'services',
				// % protected region % [Set displayName for Services here] off begin
				displayName: 'Services',
				// % protected region % [Set displayName for Services here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Services here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Services here] end
				// % protected region % [Set isSensitive for Services here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Services here] end
				// % protected region % [Set readonly for Services here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Services here] end
				validators: [
				...
				// % protected region % [Add any additional model attribute properties for Services here] off begin
				// % protected region % [Add any additional model attribute properties for Services here] end
			},
	 ...
	 */
	static deepParse(data: string | { [K in keyof NhisSpecialtyAttendedModel]?: NhisSpecialtyAttendedModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new NhisSpecialtyAttendedModel(data);
		}
	 ...
	 * @example
	 *
	 * `let nhisSpecialtyAttendedModel = new NhisSpecialtyAttendedModel(data);`
	 *
	 * @param data The input data to be initialised as the NhisSpecialtyAttendedModel,
	 *    it is expected as a JSON string or as a nullable NhisSpecialtyAttendedModel.
	 */
	constructor(data?: string | Partial<NhisSpecialtyAttendedModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<NhisSpecialtyAttendedModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		NhisSpecialtyAttendedModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): NhisSpecialtyAttendedModel {
		...
		if (updates.services) {
			newModelJson.services = updates.services;
		}

		return new NhisSpecialtyAttendedModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof NhisSpecialtyAttendedModel)) {
			return {};
		}
