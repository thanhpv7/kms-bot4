...
import {initialBpjsControlSpecialistCollectionState} from './bpjs_control_specialist.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsControlSpecialistAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsControlSpecialistModelActionTypes.CREATE_BPJS_CONTROL_SPECIALIST_OK:
				const createdModel = (action as modelAction.BpjsControlSpecialistActionOK ).stateConfig.targetModel;
				...
				BpjsControlSpecialistModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.CREATE_BPJS_CONTROL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.CREATE_ALL_BPJS_CONTROL_SPECIALIST_OK:
				const createdModels = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsControlSpecialistModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.CREATE_ALL_BPJS_CONTROL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.DELETE_BPJS_CONTROL_SPECIALIST_OK:
				const deletedModelId = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.DELETE_ALL_BPJS_CONTROL_SPECIALIST_OK:
				const deletedModelIds = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.DELETE_BPJS_CONTROL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.UPDATE_BPJS_CONTROL_SPECIALIST_OK:
				const updatedModel = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModel;
				...
				BpjsControlSpecialistModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.UPDATE_BPJS_CONTROL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsControlSpecialistModelActionTypes.UPDATE_ALL_BPJS_CONTROL_SPECIALIST_OK:
				const updatedModels = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.UPDATE_ALL_BPJS_CONTROL_SPECIALIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.FETCH_BPJS_CONTROL_SPECIALIST_OK:
				let fetchedModel = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModel;
				...
				BpjsControlSpecialistModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.FETCH_ALL_BPJS_CONTROL_SPECIALIST_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsControlSpecialistModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.FETCH_BPJS_CONTROL_SPECIALIST_AUDIT_OK:
				const audits = (action as modelAction.BpjsControlSpecialistActionOK).audits;
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.FETCH_BPJS_CONTROL_SPECIALIST_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsControlSpecialistActionOK).audits;
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.FETCH_BPJS_CONTROL_SPECIALIST_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsControlSpecialistModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.FETCH_LAST_BPJS_CONTROL_SPECIALIST_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsControlSpecialistModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.COUNT_BPJS_CONTROL_SPECIALISTS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsControlSpecialistModelActionTypes.INITIALISE_BPJS_CONTROL_SPECIALIST_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsControlSpecialistActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsControlSpecialistCollectionState
					};
				}
