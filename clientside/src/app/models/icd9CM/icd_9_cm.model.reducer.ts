...
import {initialIcd9CMCollectionState} from './icd_9_cm.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseIcd9CMAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.Icd9CMModelActionTypes.CREATE_ICD_9_CM_OK:
				const createdModel = (action as modelAction.Icd9CMActionOK ).stateConfig.targetModel;
				...
				Icd9CMModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.Icd9CMModelActionTypes.CREATE_ICD_9_CM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd9CMModelActionTypes.CREATE_ALL_ICD_9_CM_OK:
				const createdModels = (action as modelAction.Icd9CMActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					Icd9CMModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.Icd9CMModelActionTypes.CREATE_ALL_ICD_9_CM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd9CMModelActionTypes.DELETE_ICD_9_CM_OK:
				const deletedModelId = (action as modelAction.Icd9CMActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.Icd9CMModelActionTypes.DELETE_ALL_ICD_9_CM_OK:
				const deletedModelIds = (action as modelAction.Icd9CMActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.Icd9CMModelActionTypes.DELETE_ICD_9_CM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd9CMModelActionTypes.UPDATE_ICD_9_CM_OK:
				const updatedModel = (action as modelAction.Icd9CMActionOK).stateConfig.targetModel;
				...
				Icd9CMModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd9CMModelActionTypes.UPDATE_ICD_9_CM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.Icd9CMModelActionTypes.UPDATE_ALL_ICD_9_CM_OK:
				const updatedModels = (action as modelAction.Icd9CMActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.Icd9CMModelActionTypes.UPDATE_ALL_ICD_9_CM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd9CMModelActionTypes.FETCH_ICD_9_CM_OK:
				let fetchedModel = (action as modelAction.Icd9CMActionOK).stateConfig.targetModel;
				...
				Icd9CMModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd9CMModelActionTypes.FETCH_ALL_ICD_9_CM_OK:
				const totalCountAfterFetchAll = (action as modelAction.Icd9CMActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.Icd9CMActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.Icd9CMActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => Icd9CMModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd9CMModelActionTypes.FETCH_ICD_9_CM_AUDIT_OK:
				const audits = (action as modelAction.Icd9CMActionOK).audits;
				...
				break;
			case modelAction.Icd9CMModelActionTypes.FETCH_ICD_9_CM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.Icd9CMActionOK).audits;
				...
				break;
			case modelAction.Icd9CMModelActionTypes.FETCH_ICD_9_CM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => Icd9CMModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd9CMModelActionTypes.FETCH_LAST_ICD_9_CM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.Icd9CMActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => Icd9CMModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd9CMModelActionTypes.COUNT_ICD_9_CMS_OK:
				const totalCountAfterCount = (action as modelAction.Icd9CMActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.Icd9CMModelActionTypes.INITIALISE_ICD_9_CM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.Icd9CMActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.Icd9CMActionOK).stateConfig.queryParams;
