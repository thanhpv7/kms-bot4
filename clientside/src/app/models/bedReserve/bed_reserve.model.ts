 ...
 */
export class BedReserveModel extends AbstractModel {
	/**
	...
	readonly className = 'BedReserveModel';
	...
	get valueToDisplay(): string {
		...
		return BedReserveModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BedReserveModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof BedReserveModel]?: BedReserveModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BedReserveModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bedReserveModel = new BedReserveModel(data);`
	 *
	 * @param data The input data to be initialised as the BedReserveModel,
	 *    it is expected as a JSON string or as a nullable BedReserveModel.
	 */
	constructor(data?: string | Partial<BedReserveModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BedReserveModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BedReserveModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BedReserveModel {
		...
		return new BedReserveModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BedReserveModel)) {
			return {};
		}
