...
import {initialBedReserveCollectionState} from './bed_reserve.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBedReserveAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BedReserveModelActionTypes.CREATE_BED_RESERVE_OK:
				const createdModel = (action as modelAction.BedReserveActionOK ).stateConfig.targetModel;
				...
				BedReserveModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BedReserveModelActionTypes.CREATE_BED_RESERVE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedReserveModelActionTypes.CREATE_ALL_BED_RESERVE_OK:
				const createdModels = (action as modelAction.BedReserveActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BedReserveModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BedReserveModelActionTypes.CREATE_ALL_BED_RESERVE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedReserveModelActionTypes.DELETE_BED_RESERVE_OK:
				const deletedModelId = (action as modelAction.BedReserveActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BedReserveModelActionTypes.DELETE_ALL_BED_RESERVE_OK:
				const deletedModelIds = (action as modelAction.BedReserveActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BedReserveModelActionTypes.DELETE_BED_RESERVE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedReserveModelActionTypes.UPDATE_BED_RESERVE_OK:
				const updatedModel = (action as modelAction.BedReserveActionOK).stateConfig.targetModel;
				...
				BedReserveModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedReserveModelActionTypes.UPDATE_BED_RESERVE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BedReserveModelActionTypes.UPDATE_ALL_BED_RESERVE_OK:
				const updatedModels = (action as modelAction.BedReserveActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BedReserveModelActionTypes.UPDATE_ALL_BED_RESERVE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BedReserveModelActionTypes.FETCH_BED_RESERVE_OK:
				let fetchedModel = (action as modelAction.BedReserveActionOK).stateConfig.targetModel;
				...
				BedReserveModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedReserveModelActionTypes.FETCH_ALL_BED_RESERVE_OK:
				const totalCountAfterFetchAll = (action as modelAction.BedReserveActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BedReserveActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BedReserveActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BedReserveModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedReserveModelActionTypes.FETCH_BED_RESERVE_AUDIT_OK:
				const audits = (action as modelAction.BedReserveActionOK).audits;
				...
				break;
			case modelAction.BedReserveModelActionTypes.FETCH_BED_RESERVE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BedReserveActionOK).audits;
				...
				break;
			case modelAction.BedReserveModelActionTypes.FETCH_BED_RESERVE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BedReserveModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedReserveModelActionTypes.FETCH_LAST_BED_RESERVE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BedReserveActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BedReserveModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BedReserveModelActionTypes.COUNT_BED_RESERVES_OK:
				const totalCountAfterCount = (action as modelAction.BedReserveActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BedReserveModelActionTypes.INITIALISE_BED_RESERVE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BedReserveActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BedReserveActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBedReserveCollectionState
					};
				}
