...
import {initialBirthHistoryCollectionState} from './birth_history.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBirthHistoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BirthHistoryModelActionTypes.CREATE_BIRTH_HISTORY_OK:
				const createdModel = (action as modelAction.BirthHistoryActionOK ).stateConfig.targetModel;
				...
				BirthHistoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.CREATE_BIRTH_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BirthHistoryModelActionTypes.CREATE_ALL_BIRTH_HISTORY_OK:
				const createdModels = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BirthHistoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.CREATE_ALL_BIRTH_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BirthHistoryModelActionTypes.DELETE_BIRTH_HISTORY_OK:
				const deletedModelId = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.DELETE_ALL_BIRTH_HISTORY_OK:
				const deletedModelIds = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.DELETE_BIRTH_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BirthHistoryModelActionTypes.UPDATE_BIRTH_HISTORY_OK:
				const updatedModel = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModel;
				...
				BirthHistoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.UPDATE_BIRTH_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BirthHistoryModelActionTypes.UPDATE_ALL_BIRTH_HISTORY_OK:
				const updatedModels = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.UPDATE_ALL_BIRTH_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BirthHistoryModelActionTypes.FETCH_BIRTH_HISTORY_OK:
				let fetchedModel = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModel;
				...
				BirthHistoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.FETCH_ALL_BIRTH_HISTORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BirthHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BirthHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.FETCH_BIRTH_HISTORY_AUDIT_OK:
				const audits = (action as modelAction.BirthHistoryActionOK).audits;
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.FETCH_BIRTH_HISTORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BirthHistoryActionOK).audits;
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.FETCH_BIRTH_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BirthHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.FETCH_LAST_BIRTH_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BirthHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BirthHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.COUNT_BIRTH_HISTORYS_OK:
				const totalCountAfterCount = (action as modelAction.BirthHistoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BirthHistoryModelActionTypes.INITIALISE_BIRTH_HISTORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BirthHistoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BirthHistoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBirthHistoryCollectionState
					};
				}
