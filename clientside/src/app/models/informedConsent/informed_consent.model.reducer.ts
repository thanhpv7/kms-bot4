...
import {initialInformedConsentCollectionState} from './informed_consent.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInformedConsentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InformedConsentModelActionTypes.CREATE_INFORMED_CONSENT_OK:
				const createdModel = (action as modelAction.InformedConsentActionOK ).stateConfig.targetModel;
				...
				InformedConsentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InformedConsentModelActionTypes.CREATE_INFORMED_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InformedConsentModelActionTypes.CREATE_ALL_INFORMED_CONSENT_OK:
				const createdModels = (action as modelAction.InformedConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InformedConsentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InformedConsentModelActionTypes.CREATE_ALL_INFORMED_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InformedConsentModelActionTypes.DELETE_INFORMED_CONSENT_OK:
				const deletedModelId = (action as modelAction.InformedConsentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InformedConsentModelActionTypes.DELETE_ALL_INFORMED_CONSENT_OK:
				const deletedModelIds = (action as modelAction.InformedConsentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InformedConsentModelActionTypes.DELETE_INFORMED_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InformedConsentModelActionTypes.UPDATE_INFORMED_CONSENT_OK:
				const updatedModel = (action as modelAction.InformedConsentActionOK).stateConfig.targetModel;
				...
				InformedConsentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InformedConsentModelActionTypes.UPDATE_INFORMED_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InformedConsentModelActionTypes.UPDATE_ALL_INFORMED_CONSENT_OK:
				const updatedModels = (action as modelAction.InformedConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InformedConsentModelActionTypes.UPDATE_ALL_INFORMED_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InformedConsentModelActionTypes.FETCH_INFORMED_CONSENT_OK:
				let fetchedModel = (action as modelAction.InformedConsentActionOK).stateConfig.targetModel;
				...
				InformedConsentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InformedConsentModelActionTypes.FETCH_ALL_INFORMED_CONSENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.InformedConsentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InformedConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InformedConsentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InformedConsentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InformedConsentModelActionTypes.FETCH_INFORMED_CONSENT_AUDIT_OK:
				const audits = (action as modelAction.InformedConsentActionOK).audits;
				...
				break;
			case modelAction.InformedConsentModelActionTypes.FETCH_INFORMED_CONSENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InformedConsentActionOK).audits;
				...
				break;
			case modelAction.InformedConsentModelActionTypes.FETCH_INFORMED_CONSENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InformedConsentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InformedConsentModelActionTypes.FETCH_LAST_INFORMED_CONSENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InformedConsentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InformedConsentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InformedConsentModelActionTypes.COUNT_INFORMED_CONSENTS_OK:
				const totalCountAfterCount = (action as modelAction.InformedConsentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InformedConsentModelActionTypes.INITIALISE_INFORMED_CONSENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InformedConsentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InformedConsentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInformedConsentCollectionState
					};
				}
