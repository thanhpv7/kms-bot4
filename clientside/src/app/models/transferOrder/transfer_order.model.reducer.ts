...
import {initialTransferOrderCollectionState} from './transfer_order.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTransferOrderAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TransferOrderModelActionTypes.CREATE_TRANSFER_ORDER_OK:
				const createdModel = (action as modelAction.TransferOrderActionOK ).stateConfig.targetModel;
				...
				TransferOrderModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TransferOrderModelActionTypes.CREATE_TRANSFER_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderModelActionTypes.CREATE_ALL_TRANSFER_ORDER_OK:
				const createdModels = (action as modelAction.TransferOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TransferOrderModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TransferOrderModelActionTypes.CREATE_ALL_TRANSFER_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderModelActionTypes.DELETE_TRANSFER_ORDER_OK:
				const deletedModelId = (action as modelAction.TransferOrderActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransferOrderModelActionTypes.DELETE_ALL_TRANSFER_ORDER_OK:
				const deletedModelIds = (action as modelAction.TransferOrderActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransferOrderModelActionTypes.DELETE_TRANSFER_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderModelActionTypes.UPDATE_TRANSFER_ORDER_OK:
				const updatedModel = (action as modelAction.TransferOrderActionOK).stateConfig.targetModel;
				...
				TransferOrderModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderModelActionTypes.UPDATE_TRANSFER_ORDER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TransferOrderModelActionTypes.UPDATE_ALL_TRANSFER_ORDER_OK:
				const updatedModels = (action as modelAction.TransferOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransferOrderModelActionTypes.UPDATE_ALL_TRANSFER_ORDER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransferOrderModelActionTypes.FETCH_TRANSFER_ORDER_OK:
				let fetchedModel = (action as modelAction.TransferOrderActionOK).stateConfig.targetModel;
				...
				TransferOrderModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderModelActionTypes.FETCH_ALL_TRANSFER_ORDER_OK:
				const totalCountAfterFetchAll = (action as modelAction.TransferOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TransferOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TransferOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TransferOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderModelActionTypes.FETCH_TRANSFER_ORDER_AUDIT_OK:
				const audits = (action as modelAction.TransferOrderActionOK).audits;
				...
				break;
			case modelAction.TransferOrderModelActionTypes.FETCH_TRANSFER_ORDER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TransferOrderActionOK).audits;
				...
				break;
			case modelAction.TransferOrderModelActionTypes.FETCH_TRANSFER_ORDER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TransferOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderModelActionTypes.FETCH_LAST_TRANSFER_ORDER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TransferOrderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TransferOrderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransferOrderModelActionTypes.COUNT_TRANSFER_ORDERS_OK:
				const totalCountAfterCount = (action as modelAction.TransferOrderActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TransferOrderModelActionTypes.INITIALISE_TRANSFER_ORDER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TransferOrderActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TransferOrderActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTransferOrderCollectionState
					};
				}
