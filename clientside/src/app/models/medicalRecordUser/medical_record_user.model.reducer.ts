...
import {initialMedicalRecordUserCollectionState} from './medical_record_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicalRecordUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicalRecordUserModelActionTypes.CREATE_MEDICAL_RECORD_USER_OK:
				const createdModel = (action as modelAction.MedicalRecordUserActionOK ).stateConfig.targetModel;
				...
				MedicalRecordUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.CREATE_MEDICAL_RECORD_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalRecordUserModelActionTypes.CREATE_ALL_MEDICAL_RECORD_USER_OK:
				const createdModels = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicalRecordUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.CREATE_ALL_MEDICAL_RECORD_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalRecordUserModelActionTypes.DELETE_MEDICAL_RECORD_USER_OK:
				const deletedModelId = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.DELETE_ALL_MEDICAL_RECORD_USER_OK:
				const deletedModelIds = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.DELETE_MEDICAL_RECORD_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalRecordUserModelActionTypes.UPDATE_MEDICAL_RECORD_USER_OK:
				const updatedModel = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModel;
				...
				MedicalRecordUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.UPDATE_MEDICAL_RECORD_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicalRecordUserModelActionTypes.UPDATE_ALL_MEDICAL_RECORD_USER_OK:
				const updatedModels = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.UPDATE_ALL_MEDICAL_RECORD_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalRecordUserModelActionTypes.FETCH_MEDICAL_RECORD_USER_OK:
				let fetchedModel = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModel;
				...
				MedicalRecordUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.FETCH_ALL_MEDICAL_RECORD_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicalRecordUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicalRecordUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.FETCH_MEDICAL_RECORD_USER_AUDIT_OK:
				const audits = (action as modelAction.MedicalRecordUserActionOK).audits;
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.FETCH_MEDICAL_RECORD_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicalRecordUserActionOK).audits;
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.FETCH_MEDICAL_RECORD_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicalRecordUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.FETCH_LAST_MEDICAL_RECORD_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicalRecordUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicalRecordUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.COUNT_MEDICAL_RECORD_USERS_OK:
				const totalCountAfterCount = (action as modelAction.MedicalRecordUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicalRecordUserModelActionTypes.INITIALISE_MEDICAL_RECORD_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicalRecordUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicalRecordUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicalRecordUserCollectionState
					};
				}
