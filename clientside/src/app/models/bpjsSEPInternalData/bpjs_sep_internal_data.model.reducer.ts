...
import {initialBpjsSEPInternalDataCollectionState} from './bpjs_sep_internal_data.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsSEPInternalDataAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsSEPInternalDataModelActionTypes.CREATE_BPJS_SEP_INTERNAL_DATA_OK:
				const createdModel = (action as modelAction.BpjsSEPInternalDataActionOK ).stateConfig.targetModel;
				...
				BpjsSEPInternalDataModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.CREATE_BPJS_SEP_INTERNAL_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.CREATE_ALL_BPJS_SEP_INTERNAL_DATA_OK:
				const createdModels = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsSEPInternalDataModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.CREATE_ALL_BPJS_SEP_INTERNAL_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.DELETE_BPJS_SEP_INTERNAL_DATA_OK:
				const deletedModelId = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.DELETE_ALL_BPJS_SEP_INTERNAL_DATA_OK:
				const deletedModelIds = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.DELETE_BPJS_SEP_INTERNAL_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.UPDATE_BPJS_SEP_INTERNAL_DATA_OK:
				const updatedModel = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModel;
				...
				BpjsSEPInternalDataModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.UPDATE_BPJS_SEP_INTERNAL_DATA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsSEPInternalDataModelActionTypes.UPDATE_ALL_BPJS_SEP_INTERNAL_DATA_OK:
				const updatedModels = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.UPDATE_ALL_BPJS_SEP_INTERNAL_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_BPJS_SEP_INTERNAL_DATA_OK:
				let fetchedModel = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModel;
				...
				BpjsSEPInternalDataModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_ALL_BPJS_SEP_INTERNAL_DATA_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsSEPInternalDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_BPJS_SEP_INTERNAL_DATA_AUDIT_OK:
				const audits = (action as modelAction.BpjsSEPInternalDataActionOK).audits;
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_BPJS_SEP_INTERNAL_DATA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsSEPInternalDataActionOK).audits;
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_BPJS_SEP_INTERNAL_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsSEPInternalDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.FETCH_LAST_BPJS_SEP_INTERNAL_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsSEPInternalDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.COUNT_BPJS_SEP_INTERNAL_DATAS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsSEPInternalDataModelActionTypes.INITIALISE_BPJS_SEP_INTERNAL_DATA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsSEPInternalDataActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsSEPInternalDataCollectionState
					};
				}
