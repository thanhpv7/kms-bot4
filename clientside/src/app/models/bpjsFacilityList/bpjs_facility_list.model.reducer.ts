...
import {initialBpjsFacilityListCollectionState} from './bpjs_facility_list.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsFacilityListAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsFacilityListModelActionTypes.CREATE_BPJS_FACILITY_LIST_OK:
				const createdModel = (action as modelAction.BpjsFacilityListActionOK ).stateConfig.targetModel;
				...
				BpjsFacilityListModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.CREATE_BPJS_FACILITY_LIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFacilityListModelActionTypes.CREATE_ALL_BPJS_FACILITY_LIST_OK:
				const createdModels = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsFacilityListModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.CREATE_ALL_BPJS_FACILITY_LIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFacilityListModelActionTypes.DELETE_BPJS_FACILITY_LIST_OK:
				const deletedModelId = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.DELETE_ALL_BPJS_FACILITY_LIST_OK:
				const deletedModelIds = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.DELETE_BPJS_FACILITY_LIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFacilityListModelActionTypes.UPDATE_BPJS_FACILITY_LIST_OK:
				const updatedModel = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModel;
				...
				BpjsFacilityListModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.UPDATE_BPJS_FACILITY_LIST_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsFacilityListModelActionTypes.UPDATE_ALL_BPJS_FACILITY_LIST_OK:
				const updatedModels = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.UPDATE_ALL_BPJS_FACILITY_LIST_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsFacilityListModelActionTypes.FETCH_BPJS_FACILITY_LIST_OK:
				let fetchedModel = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModel;
				...
				BpjsFacilityListModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.FETCH_ALL_BPJS_FACILITY_LIST_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsFacilityListActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsFacilityListModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.FETCH_BPJS_FACILITY_LIST_AUDIT_OK:
				const audits = (action as modelAction.BpjsFacilityListActionOK).audits;
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.FETCH_BPJS_FACILITY_LIST_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsFacilityListActionOK).audits;
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.FETCH_BPJS_FACILITY_LIST_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsFacilityListModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.FETCH_LAST_BPJS_FACILITY_LIST_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsFacilityListActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsFacilityListModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.COUNT_BPJS_FACILITY_LISTS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsFacilityListActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsFacilityListModelActionTypes.INITIALISE_BPJS_FACILITY_LIST_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsFacilityListActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsFacilityListActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsFacilityListCollectionState
					};
				}
