...
import {initialBpjsHospitalizationPlanCollectionState} from './bpjs_hospitalization_plan.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsHospitalizationPlanAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsHospitalizationPlanModelActionTypes.CREATE_BPJS_HOSPITALIZATION_PLAN_OK:
				const createdModel = (action as modelAction.BpjsHospitalizationPlanActionOK ).stateConfig.targetModel;
				...
				BpjsHospitalizationPlanModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.CREATE_BPJS_HOSPITALIZATION_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.CREATE_ALL_BPJS_HOSPITALIZATION_PLAN_OK:
				const createdModels = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsHospitalizationPlanModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.CREATE_ALL_BPJS_HOSPITALIZATION_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.DELETE_BPJS_HOSPITALIZATION_PLAN_OK:
				const deletedModelId = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.DELETE_ALL_BPJS_HOSPITALIZATION_PLAN_OK:
				const deletedModelIds = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.DELETE_BPJS_HOSPITALIZATION_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.UPDATE_BPJS_HOSPITALIZATION_PLAN_OK:
				const updatedModel = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModel;
				...
				BpjsHospitalizationPlanModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.UPDATE_BPJS_HOSPITALIZATION_PLAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsHospitalizationPlanModelActionTypes.UPDATE_ALL_BPJS_HOSPITALIZATION_PLAN_OK:
				const updatedModels = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.UPDATE_ALL_BPJS_HOSPITALIZATION_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_BPJS_HOSPITALIZATION_PLAN_OK:
				let fetchedModel = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModel;
				...
				BpjsHospitalizationPlanModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_ALL_BPJS_HOSPITALIZATION_PLAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsHospitalizationPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_BPJS_HOSPITALIZATION_PLAN_AUDIT_OK:
				const audits = (action as modelAction.BpjsHospitalizationPlanActionOK).audits;
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_BPJS_HOSPITALIZATION_PLAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsHospitalizationPlanActionOK).audits;
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_BPJS_HOSPITALIZATION_PLAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsHospitalizationPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.FETCH_LAST_BPJS_HOSPITALIZATION_PLAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsHospitalizationPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.COUNT_BPJS_HOSPITALIZATION_PLANS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsHospitalizationPlanModelActionTypes.INITIALISE_BPJS_HOSPITALIZATION_PLAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsHospitalizationPlanActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsHospitalizationPlanCollectionState
					};
				}
