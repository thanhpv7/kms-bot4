...
import {initialReferenceDataCollectionState} from './reference_data.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseReferenceDataAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ReferenceDataModelActionTypes.CREATE_REFERENCE_DATA_OK:
				const createdModel = (action as modelAction.ReferenceDataActionOK ).stateConfig.targetModel;
				...
				ReferenceDataModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.CREATE_REFERENCE_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceDataModelActionTypes.CREATE_ALL_REFERENCE_DATA_OK:
				const createdModels = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ReferenceDataModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.CREATE_ALL_REFERENCE_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceDataModelActionTypes.DELETE_REFERENCE_DATA_OK:
				const deletedModelId = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.DELETE_ALL_REFERENCE_DATA_OK:
				const deletedModelIds = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.DELETE_REFERENCE_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceDataModelActionTypes.UPDATE_REFERENCE_DATA_OK:
				const updatedModel = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModel;
				...
				ReferenceDataModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.UPDATE_REFERENCE_DATA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ReferenceDataModelActionTypes.UPDATE_ALL_REFERENCE_DATA_OK:
				const updatedModels = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.UPDATE_ALL_REFERENCE_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ReferenceDataModelActionTypes.FETCH_REFERENCE_DATA_OK:
				let fetchedModel = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModel;
				...
				ReferenceDataModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.FETCH_ALL_REFERENCE_DATA_OK:
				const totalCountAfterFetchAll = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ReferenceDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ReferenceDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.FETCH_REFERENCE_DATA_AUDIT_OK:
				const audits = (action as modelAction.ReferenceDataActionOK).audits;
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.FETCH_REFERENCE_DATA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ReferenceDataActionOK).audits;
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.FETCH_REFERENCE_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ReferenceDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.FETCH_LAST_REFERENCE_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ReferenceDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ReferenceDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.COUNT_REFERENCE_DATAS_OK:
				const totalCountAfterCount = (action as modelAction.ReferenceDataActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ReferenceDataModelActionTypes.INITIALISE_REFERENCE_DATA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ReferenceDataActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ReferenceDataActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialReferenceDataCollectionState
					};
				}
