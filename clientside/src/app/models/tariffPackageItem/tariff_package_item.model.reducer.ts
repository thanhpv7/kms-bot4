...
import {initialTariffPackageItemCollectionState} from './tariff_package_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTariffPackageItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TariffPackageItemModelActionTypes.CREATE_TARIFF_PACKAGE_ITEM_OK:
				const createdModel = (action as modelAction.TariffPackageItemActionOK ).stateConfig.targetModel;
				...
				TariffPackageItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.CREATE_TARIFF_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageItemModelActionTypes.CREATE_ALL_TARIFF_PACKAGE_ITEM_OK:
				const createdModels = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TariffPackageItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.CREATE_ALL_TARIFF_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageItemModelActionTypes.DELETE_TARIFF_PACKAGE_ITEM_OK:
				const deletedModelId = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.DELETE_ALL_TARIFF_PACKAGE_ITEM_OK:
				const deletedModelIds = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.DELETE_TARIFF_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageItemModelActionTypes.UPDATE_TARIFF_PACKAGE_ITEM_OK:
				const updatedModel = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModel;
				...
				TariffPackageItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.UPDATE_TARIFF_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TariffPackageItemModelActionTypes.UPDATE_ALL_TARIFF_PACKAGE_ITEM_OK:
				const updatedModels = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.UPDATE_ALL_TARIFF_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageItemModelActionTypes.FETCH_TARIFF_PACKAGE_ITEM_OK:
				let fetchedModel = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModel;
				...
				TariffPackageItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.FETCH_ALL_TARIFF_PACKAGE_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TariffPackageItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TariffPackageItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.FETCH_TARIFF_PACKAGE_ITEM_AUDIT_OK:
				const audits = (action as modelAction.TariffPackageItemActionOK).audits;
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.FETCH_TARIFF_PACKAGE_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TariffPackageItemActionOK).audits;
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.FETCH_TARIFF_PACKAGE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TariffPackageItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.FETCH_LAST_TARIFF_PACKAGE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TariffPackageItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TariffPackageItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.COUNT_TARIFF_PACKAGE_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.TariffPackageItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TariffPackageItemModelActionTypes.INITIALISE_TARIFF_PACKAGE_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TariffPackageItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TariffPackageItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTariffPackageItemCollectionState
					};
				}
