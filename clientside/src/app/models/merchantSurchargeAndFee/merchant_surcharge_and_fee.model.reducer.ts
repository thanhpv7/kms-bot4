...
import {initialMerchantSurchargeAndFeeCollectionState} from './merchant_surcharge_and_fee.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMerchantSurchargeAndFeeAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.CREATE_MERCHANT_SURCHARGE_AND_FEE_OK:
				const createdModel = (action as modelAction.MerchantSurchargeAndFeeActionOK ).stateConfig.targetModel;
				...
				MerchantSurchargeAndFeeModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.CREATE_MERCHANT_SURCHARGE_AND_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.CREATE_ALL_MERCHANT_SURCHARGE_AND_FEE_OK:
				const createdModels = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MerchantSurchargeAndFeeModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.CREATE_ALL_MERCHANT_SURCHARGE_AND_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.DELETE_MERCHANT_SURCHARGE_AND_FEE_OK:
				const deletedModelId = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.DELETE_ALL_MERCHANT_SURCHARGE_AND_FEE_OK:
				const deletedModelIds = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.DELETE_MERCHANT_SURCHARGE_AND_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.UPDATE_MERCHANT_SURCHARGE_AND_FEE_OK:
				const updatedModel = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModel;
				...
				MerchantSurchargeAndFeeModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.UPDATE_MERCHANT_SURCHARGE_AND_FEE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MerchantSurchargeAndFeeModelActionTypes.UPDATE_ALL_MERCHANT_SURCHARGE_AND_FEE_OK:
				const updatedModels = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.UPDATE_ALL_MERCHANT_SURCHARGE_AND_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_MERCHANT_SURCHARGE_AND_FEE_OK:
				let fetchedModel = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModel;
				...
				MerchantSurchargeAndFeeModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_ALL_MERCHANT_SURCHARGE_AND_FEE_OK:
				const totalCountAfterFetchAll = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MerchantSurchargeAndFeeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_MERCHANT_SURCHARGE_AND_FEE_AUDIT_OK:
				const audits = (action as modelAction.MerchantSurchargeAndFeeActionOK).audits;
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_MERCHANT_SURCHARGE_AND_FEE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MerchantSurchargeAndFeeActionOK).audits;
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_MERCHANT_SURCHARGE_AND_FEE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MerchantSurchargeAndFeeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.FETCH_LAST_MERCHANT_SURCHARGE_AND_FEE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MerchantSurchargeAndFeeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.COUNT_MERCHANT_SURCHARGE_AND_FEES_OK:
				const totalCountAfterCount = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MerchantSurchargeAndFeeModelActionTypes.INITIALISE_MERCHANT_SURCHARGE_AND_FEE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MerchantSurchargeAndFeeActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMerchantSurchargeAndFeeCollectionState
					};
				}
