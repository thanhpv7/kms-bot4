...
import {initialNurseVitalSignMeasurementCollectionState} from './nurse_vital_sign_measurement.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNurseVitalSignMeasurementAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NurseVitalSignMeasurementModelActionTypes.CREATE_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				const createdModel = (action as modelAction.NurseVitalSignMeasurementActionOK ).stateConfig.targetModel;
				...
				NurseVitalSignMeasurementModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.CREATE_NURSE_VITAL_SIGN_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.CREATE_ALL_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				const createdModels = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NurseVitalSignMeasurementModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.CREATE_ALL_NURSE_VITAL_SIGN_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.DELETE_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				const deletedModelId = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.DELETE_ALL_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				const deletedModelIds = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.DELETE_NURSE_VITAL_SIGN_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.UPDATE_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				const updatedModel = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModel;
				...
				NurseVitalSignMeasurementModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.UPDATE_NURSE_VITAL_SIGN_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NurseVitalSignMeasurementModelActionTypes.UPDATE_ALL_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				const updatedModels = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.UPDATE_ALL_NURSE_VITAL_SIGN_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				let fetchedModel = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModel;
				...
				NurseVitalSignMeasurementModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_ALL_NURSE_VITAL_SIGN_MEASUREMENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NurseVitalSignMeasurementModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_NURSE_VITAL_SIGN_MEASUREMENT_AUDIT_OK:
				const audits = (action as modelAction.NurseVitalSignMeasurementActionOK).audits;
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_NURSE_VITAL_SIGN_MEASUREMENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NurseVitalSignMeasurementActionOK).audits;
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_NURSE_VITAL_SIGN_MEASUREMENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NurseVitalSignMeasurementModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.FETCH_LAST_NURSE_VITAL_SIGN_MEASUREMENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NurseVitalSignMeasurementModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.COUNT_NURSE_VITAL_SIGN_MEASUREMENTS_OK:
				const totalCountAfterCount = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NurseVitalSignMeasurementModelActionTypes.INITIALISE_NURSE_VITAL_SIGN_MEASUREMENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NurseVitalSignMeasurementActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNurseVitalSignMeasurementCollectionState
					};
				}
