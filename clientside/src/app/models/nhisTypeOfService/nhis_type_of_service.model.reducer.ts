...
import {initialNhisTypeOfServiceCollectionState} from './nhis_type_of_service.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisTypeOfServiceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisTypeOfServiceModelActionTypes.CREATE_NHIS_TYPE_OF_SERVICE_OK:
				const createdModel = (action as modelAction.NhisTypeOfServiceActionOK ).stateConfig.targetModel;
				...
				NhisTypeOfServiceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.CREATE_NHIS_TYPE_OF_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.CREATE_ALL_NHIS_TYPE_OF_SERVICE_OK:
				const createdModels = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisTypeOfServiceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.CREATE_ALL_NHIS_TYPE_OF_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.DELETE_NHIS_TYPE_OF_SERVICE_OK:
				const deletedModelId = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.DELETE_ALL_NHIS_TYPE_OF_SERVICE_OK:
				const deletedModelIds = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.DELETE_NHIS_TYPE_OF_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.UPDATE_NHIS_TYPE_OF_SERVICE_OK:
				const updatedModel = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModel;
				...
				NhisTypeOfServiceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.UPDATE_NHIS_TYPE_OF_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisTypeOfServiceModelActionTypes.UPDATE_ALL_NHIS_TYPE_OF_SERVICE_OK:
				const updatedModels = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.UPDATE_ALL_NHIS_TYPE_OF_SERVICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.FETCH_NHIS_TYPE_OF_SERVICE_OK:
				let fetchedModel = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModel;
				...
				NhisTypeOfServiceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.FETCH_ALL_NHIS_TYPE_OF_SERVICE_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisTypeOfServiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.FETCH_NHIS_TYPE_OF_SERVICE_AUDIT_OK:
				const audits = (action as modelAction.NhisTypeOfServiceActionOK).audits;
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.FETCH_NHIS_TYPE_OF_SERVICE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisTypeOfServiceActionOK).audits;
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.FETCH_NHIS_TYPE_OF_SERVICE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisTypeOfServiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.FETCH_LAST_NHIS_TYPE_OF_SERVICE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisTypeOfServiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.COUNT_NHIS_TYPE_OF_SERVICES_OK:
				const totalCountAfterCount = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisTypeOfServiceModelActionTypes.INITIALISE_NHIS_TYPE_OF_SERVICE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisTypeOfServiceActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisTypeOfServiceCollectionState
					};
				}
