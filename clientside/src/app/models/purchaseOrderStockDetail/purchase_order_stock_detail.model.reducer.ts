...
import {initialPurchaseOrderStockDetailCollectionState} from './purchase_order_stock_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePurchaseOrderStockDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PurchaseOrderStockDetailModelActionTypes.CREATE_PURCHASE_ORDER_STOCK_DETAIL_OK:
				const createdModel = (action as modelAction.PurchaseOrderStockDetailActionOK ).stateConfig.targetModel;
				...
				PurchaseOrderStockDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.CREATE_PURCHASE_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.CREATE_ALL_PURCHASE_ORDER_STOCK_DETAIL_OK:
				const createdModels = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PurchaseOrderStockDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.CREATE_ALL_PURCHASE_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.DELETE_PURCHASE_ORDER_STOCK_DETAIL_OK:
				const deletedModelId = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.DELETE_ALL_PURCHASE_ORDER_STOCK_DETAIL_OK:
				const deletedModelIds = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.DELETE_PURCHASE_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.UPDATE_PURCHASE_ORDER_STOCK_DETAIL_OK:
				const updatedModel = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModel;
				...
				PurchaseOrderStockDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.UPDATE_PURCHASE_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PurchaseOrderStockDetailModelActionTypes.UPDATE_ALL_PURCHASE_ORDER_STOCK_DETAIL_OK:
				const updatedModels = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.UPDATE_ALL_PURCHASE_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_PURCHASE_ORDER_STOCK_DETAIL_OK:
				let fetchedModel = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModel;
				...
				PurchaseOrderStockDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_ALL_PURCHASE_ORDER_STOCK_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PurchaseOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_PURCHASE_ORDER_STOCK_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.PurchaseOrderStockDetailActionOK).audits;
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_PURCHASE_ORDER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PurchaseOrderStockDetailActionOK).audits;
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_PURCHASE_ORDER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PurchaseOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.FETCH_LAST_PURCHASE_ORDER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PurchaseOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.COUNT_PURCHASE_ORDER_STOCK_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PurchaseOrderStockDetailModelActionTypes.INITIALISE_PURCHASE_ORDER_STOCK_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PurchaseOrderStockDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPurchaseOrderStockDetailCollectionState
					};
				}
