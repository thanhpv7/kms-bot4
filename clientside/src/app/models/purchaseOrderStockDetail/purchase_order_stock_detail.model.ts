 ...
 */
export class PurchaseOrderStockDetailModel extends AbstractModel {
	/**
	...
	readonly className = 'PurchaseOrderStockDetailModel';
	...
	get valueToDisplay(): string {
		...
		return PurchaseOrderStockDetailModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	purchaseRequisitionStockDetailId: string;

	purchaseRequisitionStockDetail: PurchaseRequisitionStockDetailModel;
	...
	modelPropGroups: { [s: string]: Group } = PurchaseOrderStockDetailModel.modelPropGroups;
	...
	static getRelations(): ModelRelation[] {
		return super.getRelations().concat([
			...
			{
				id: 'purchaseRequisitionStockDetail',
				type: ModelRelationType.ONE,
				name: 'purchaseRequisitionStockDetailId',
				// % protected region % [Customise your label for Purchase Requisition Stock Detail here] off begin
				label: 'Purchase Requisition Stock Detail',
				// % protected region % [Customise your label for Purchase Requisition Stock Detail here] end
				entityName: 'PurchaseRequisitionStockDetailModel',
				// % protected region % [Customise your display name for Purchase Requisition Stock Detail here] off begin
				// TODO change implementation to use OrderBy or create new metamodel property DisplayBy
				displayName: 'soh',
				// % protected region % [Customise your display name for Purchase Requisition Stock Detail here] end
				validators: [
					// % protected region % [Add other validators for Purchase Requisition Stock Detail here] off begin
					// % protected region % [Add other validators for Purchase Requisition Stock Detail here] end
				],
				// % protected region % [Add any additional field for relation Purchase Requisition Stock Detail here] off begin
				// % protected region % [Add any additional field for relation Purchase Requisition Stock Detail here] end
			},
			{
				id: 'stockCatalogue',
				type: ModelRelationType.ONE,
	 ...
	 */
	static deepParse(data: string | { [K in keyof PurchaseOrderStockDetailModel]?: PurchaseOrderStockDetailModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new PurchaseOrderStockDetailModel(data);
		}
		...
		// Incoming one to many
		if (json.purchaseRequisitionStockDetail) {
			currentModel.purchaseRequisitionStockDetailId = json.purchaseRequisitionStockDetail.id;
			returned = _.union(returned, PurchaseRequisitionStockDetailModel.deepParse(json.purchaseRequisitionStockDetail));
		}
	 ...
	 * @example
	 *
	 * `let purchaseOrderStockDetailModel = new PurchaseOrderStockDetailModel(data);`
	 *
	 * @param data The input data to be initialised as the PurchaseOrderStockDetailModel,
	 *    it is expected as a JSON string or as a nullable PurchaseOrderStockDetailModel.
	 */
	constructor(data?: string | Partial<PurchaseOrderStockDetailModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<PurchaseOrderStockDetailModel>
				: data;
			...
			this.purchaseRequisitionStockDetailId = json.purchaseRequisitionStockDetailId;
			this.purchaseRequisitionStockDetail = json.purchaseRequisitionStockDetail;
	...
	toJSON() {
		return {
			...
			purchaseRequisitionStockDetailId: this.purchaseRequisitionStockDetailId,
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		PurchaseOrderStockDetailModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): PurchaseOrderStockDetailModel {
		...
		if (updates.purchaseRequisitionStockDetailId) {
			newModelJson.purchaseRequisitionStockDetailId = updates.purchaseRequisitionStockDetailId;
		}
		...
		return new PurchaseOrderStockDetailModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof PurchaseOrderStockDetailModel)) {
			return {};
		}
		...
		return _.omit(diff, [
			...
			'purchaseOrderId',
			'purchaseOrder',
			'purchaseRequisitionStockDetailId',
	...
	syncRelations() {
		...
		if (!this.purchaseRequisitionStockDetail) {
			this.purchaseRequisitionStockDetailId = null;
		} else {
			this.purchaseRequisitionStockDetailId = this.purchaseRequisitionStockDetail.id;
		}
