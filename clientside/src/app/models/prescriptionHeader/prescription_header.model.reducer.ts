...
import {initialPrescriptionHeaderCollectionState} from './prescription_header.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePrescriptionHeaderAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PrescriptionHeaderModelActionTypes.CREATE_PRESCRIPTION_HEADER_OK:
				const createdModel = (action as modelAction.PrescriptionHeaderActionOK ).stateConfig.targetModel;
				...
				PrescriptionHeaderModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.CREATE_PRESCRIPTION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.CREATE_ALL_PRESCRIPTION_HEADER_OK:
				const createdModels = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PrescriptionHeaderModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.CREATE_ALL_PRESCRIPTION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.DELETE_PRESCRIPTION_HEADER_OK:
				const deletedModelId = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.DELETE_ALL_PRESCRIPTION_HEADER_OK:
				const deletedModelIds = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.DELETE_PRESCRIPTION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.UPDATE_PRESCRIPTION_HEADER_OK:
				const updatedModel = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModel;
				...
				PrescriptionHeaderModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.UPDATE_PRESCRIPTION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PrescriptionHeaderModelActionTypes.UPDATE_ALL_PRESCRIPTION_HEADER_OK:
				const updatedModels = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.UPDATE_ALL_PRESCRIPTION_HEADER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.FETCH_PRESCRIPTION_HEADER_OK:
				let fetchedModel = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModel;
				...
				PrescriptionHeaderModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.FETCH_ALL_PRESCRIPTION_HEADER_OK:
				const totalCountAfterFetchAll = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PrescriptionHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.FETCH_PRESCRIPTION_HEADER_AUDIT_OK:
				const audits = (action as modelAction.PrescriptionHeaderActionOK).audits;
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.FETCH_PRESCRIPTION_HEADER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PrescriptionHeaderActionOK).audits;
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.FETCH_PRESCRIPTION_HEADER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PrescriptionHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.FETCH_LAST_PRESCRIPTION_HEADER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PrescriptionHeaderModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.COUNT_PRESCRIPTION_HEADERS_OK:
				const totalCountAfterCount = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PrescriptionHeaderModelActionTypes.INITIALISE_PRESCRIPTION_HEADER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PrescriptionHeaderActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PrescriptionHeaderActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPrescriptionHeaderCollectionState
					};
				}
