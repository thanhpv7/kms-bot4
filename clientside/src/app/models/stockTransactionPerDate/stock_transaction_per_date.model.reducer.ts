...
import {initialStockTransactionPerDateCollectionState} from './stock_transaction_per_date.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseStockTransactionPerDateAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.StockTransactionPerDateModelActionTypes.CREATE_STOCK_TRANSACTION_PER_DATE_OK:
				const createdModel = (action as modelAction.StockTransactionPerDateActionOK ).stateConfig.targetModel;
				...
				StockTransactionPerDateModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.CREATE_STOCK_TRANSACTION_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.CREATE_ALL_STOCK_TRANSACTION_PER_DATE_OK:
				const createdModels = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					StockTransactionPerDateModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.CREATE_ALL_STOCK_TRANSACTION_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE_OK:
				const deletedModelId = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.DELETE_ALL_STOCK_TRANSACTION_PER_DATE_OK:
				const deletedModelIds = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.UPDATE_STOCK_TRANSACTION_PER_DATE_OK:
				const updatedModel = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModel;
				...
				StockTransactionPerDateModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.UPDATE_STOCK_TRANSACTION_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.StockTransactionPerDateModelActionTypes.UPDATE_ALL_STOCK_TRANSACTION_PER_DATE_OK:
				const updatedModels = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.UPDATE_ALL_STOCK_TRANSACTION_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_OK:
				let fetchedModel = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModel;
				...
				StockTransactionPerDateModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.FETCH_ALL_STOCK_TRANSACTION_PER_DATE_OK:
				const totalCountAfterFetchAll = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => StockTransactionPerDateModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_AUDIT_OK:
				const audits = (action as modelAction.StockTransactionPerDateActionOK).audits;
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.StockTransactionPerDateActionOK).audits;
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => StockTransactionPerDateModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.FETCH_LAST_STOCK_TRANSACTION_PER_DATE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => StockTransactionPerDateModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.COUNT_STOCK_TRANSACTION_PER_DATES_OK:
				const totalCountAfterCount = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.StockTransactionPerDateModelActionTypes.INITIALISE_STOCK_TRANSACTION_PER_DATE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.StockTransactionPerDateActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.StockTransactionPerDateActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialStockTransactionPerDateCollectionState
					};
				}
