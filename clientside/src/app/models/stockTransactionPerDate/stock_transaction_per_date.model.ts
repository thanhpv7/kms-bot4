 ...
 */
export class StockTransactionPerDateModel extends AbstractModel {
	/**
	...
	readonly className = 'StockTransactionPerDateModel';
	...
	get valueToDisplay(): string {
		...
		return StockTransactionPerDateModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = StockTransactionPerDateModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof StockTransactionPerDateModel]?: StockTransactionPerDateModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new StockTransactionPerDateModel(data);
		}
	 ...
	 * @example
	 *
	 * `let stockTransactionPerDateModel = new StockTransactionPerDateModel(data);`
	 *
	 * @param data The input data to be initialised as the StockTransactionPerDateModel,
	 *    it is expected as a JSON string or as a nullable StockTransactionPerDateModel.
	 */
	constructor(data?: string | Partial<StockTransactionPerDateModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<StockTransactionPerDateModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		StockTransactionPerDateModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): StockTransactionPerDateModel {
		...
		return new StockTransactionPerDateModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof StockTransactionPerDateModel)) {
			return {};
		}
