...
import {initialTransactionDetailTypeMappingCollectionState} from './transaction_detail_type_mapping.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTransactionDetailTypeMappingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TransactionDetailTypeMappingModelActionTypes.CREATE_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				const createdModel = (action as modelAction.TransactionDetailTypeMappingActionOK ).stateConfig.targetModel;
				...
				TransactionDetailTypeMappingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.CREATE_TRANSACTION_DETAIL_TYPE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.CREATE_ALL_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				const createdModels = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TransactionDetailTypeMappingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.CREATE_ALL_TRANSACTION_DETAIL_TYPE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				const deletedModelId = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.DELETE_ALL_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				const deletedModelIds = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.UPDATE_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				const updatedModel = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModel;
				...
				TransactionDetailTypeMappingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.UPDATE_TRANSACTION_DETAIL_TYPE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TransactionDetailTypeMappingModelActionTypes.UPDATE_ALL_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				const updatedModels = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.UPDATE_ALL_TRANSACTION_DETAIL_TYPE_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				let fetchedModel = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModel;
				...
				TransactionDetailTypeMappingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_ALL_TRANSACTION_DETAIL_TYPE_MAPPING_OK:
				const totalCountAfterFetchAll = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TransactionDetailTypeMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_MAPPING_AUDIT_OK:
				const audits = (action as modelAction.TransactionDetailTypeMappingActionOK).audits;
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_MAPPING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TransactionDetailTypeMappingActionOK).audits;
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TransactionDetailTypeMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.FETCH_LAST_TRANSACTION_DETAIL_TYPE_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TransactionDetailTypeMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.COUNT_TRANSACTION_DETAIL_TYPE_MAPPINGS_OK:
				const totalCountAfterCount = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TransactionDetailTypeMappingModelActionTypes.INITIALISE_TRANSACTION_DETAIL_TYPE_MAPPING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TransactionDetailTypeMappingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTransactionDetailTypeMappingCollectionState
					};
				}
