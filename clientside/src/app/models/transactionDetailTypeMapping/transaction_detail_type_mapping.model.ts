 ...
 */
export class TransactionDetailTypeMappingModel extends AbstractModel {
	/**
	...
	readonly className = 'TransactionDetailTypeMappingModel';
	...
	get valueToDisplay(): string {
		...
		return TransactionDetailTypeMappingModel.displayAttributes.map((attr) => this[attr]).join(' ');
		// % protected region % [Change displayName here if needed] end
	}

	/**
	 * OPB = Opening Balance, ADJ = Adjustment, GRN = Goods Received Note, SPR = Supplier Return, WTO = Warehouse Transfer Out, WTI = Warehouse Transfer IN.
	 */
	...
	modelPropGroups: { [s: string]: Group } = TransactionDetailTypeMappingModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof TransactionDetailTypeMappingModel]?: TransactionDetailTypeMappingModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new TransactionDetailTypeMappingModel(data);
		}
	 ...
	 * @example
	 *
	 * `let transactionDetailTypeMappingModel = new TransactionDetailTypeMappingModel(data);`
	 *
	 * @param data The input data to be initialised as the TransactionDetailTypeMappingModel,
	 *    it is expected as a JSON string or as a nullable TransactionDetailTypeMappingModel.
	 */
	constructor(data?: string | Partial<TransactionDetailTypeMappingModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<TransactionDetailTypeMappingModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		TransactionDetailTypeMappingModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): TransactionDetailTypeMappingModel {
		...
		return new TransactionDetailTypeMappingModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof TransactionDetailTypeMappingModel)) {
			return {};
		}
