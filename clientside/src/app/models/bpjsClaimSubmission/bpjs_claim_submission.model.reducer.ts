...
import {initialBpjsClaimSubmissionCollectionState} from './bpjs_claim_submission.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsClaimSubmissionAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsClaimSubmissionModelActionTypes.CREATE_BPJS_CLAIM_SUBMISSION_OK:
				const createdModel = (action as modelAction.BpjsClaimSubmissionActionOK ).stateConfig.targetModel;
				...
				BpjsClaimSubmissionModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.CREATE_BPJS_CLAIM_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.CREATE_ALL_BPJS_CLAIM_SUBMISSION_OK:
				const createdModels = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsClaimSubmissionModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.CREATE_ALL_BPJS_CLAIM_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.DELETE_BPJS_CLAIM_SUBMISSION_OK:
				const deletedModelId = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.DELETE_ALL_BPJS_CLAIM_SUBMISSION_OK:
				const deletedModelIds = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.DELETE_BPJS_CLAIM_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.UPDATE_BPJS_CLAIM_SUBMISSION_OK:
				const updatedModel = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModel;
				...
				BpjsClaimSubmissionModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.UPDATE_BPJS_CLAIM_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsClaimSubmissionModelActionTypes.UPDATE_ALL_BPJS_CLAIM_SUBMISSION_OK:
				const updatedModels = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.UPDATE_ALL_BPJS_CLAIM_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.FETCH_BPJS_CLAIM_SUBMISSION_OK:
				let fetchedModel = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModel;
				...
				BpjsClaimSubmissionModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.FETCH_ALL_BPJS_CLAIM_SUBMISSION_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsClaimSubmissionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.FETCH_BPJS_CLAIM_SUBMISSION_AUDIT_OK:
				const audits = (action as modelAction.BpjsClaimSubmissionActionOK).audits;
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.FETCH_BPJS_CLAIM_SUBMISSION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsClaimSubmissionActionOK).audits;
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.FETCH_BPJS_CLAIM_SUBMISSION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsClaimSubmissionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.FETCH_LAST_BPJS_CLAIM_SUBMISSION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsClaimSubmissionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.COUNT_BPJS_CLAIM_SUBMISSIONS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsClaimSubmissionModelActionTypes.INITIALISE_BPJS_CLAIM_SUBMISSION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsClaimSubmissionActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsClaimSubmissionCollectionState
					};
				}
