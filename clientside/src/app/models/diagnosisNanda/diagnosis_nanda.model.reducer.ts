...
import {initialDiagnosisNandaCollectionState} from './diagnosis_nanda.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDiagnosisNandaAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DiagnosisNandaModelActionTypes.CREATE_DIAGNOSIS_NANDA_OK:
				const createdModel = (action as modelAction.DiagnosisNandaActionOK ).stateConfig.targetModel;
				...
				DiagnosisNandaModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.CREATE_DIAGNOSIS_NANDA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosisNandaModelActionTypes.CREATE_ALL_DIAGNOSIS_NANDA_OK:
				const createdModels = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DiagnosisNandaModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.CREATE_ALL_DIAGNOSIS_NANDA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosisNandaModelActionTypes.DELETE_DIAGNOSIS_NANDA_OK:
				const deletedModelId = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.DELETE_ALL_DIAGNOSIS_NANDA_OK:
				const deletedModelIds = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.DELETE_DIAGNOSIS_NANDA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosisNandaModelActionTypes.UPDATE_DIAGNOSIS_NANDA_OK:
				const updatedModel = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModel;
				...
				DiagnosisNandaModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.UPDATE_DIAGNOSIS_NANDA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DiagnosisNandaModelActionTypes.UPDATE_ALL_DIAGNOSIS_NANDA_OK:
				const updatedModels = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.UPDATE_ALL_DIAGNOSIS_NANDA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosisNandaModelActionTypes.FETCH_DIAGNOSIS_NANDA_OK:
				let fetchedModel = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModel;
				...
				DiagnosisNandaModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.FETCH_ALL_DIAGNOSIS_NANDA_OK:
				const totalCountAfterFetchAll = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DiagnosisNandaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DiagnosisNandaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.FETCH_DIAGNOSIS_NANDA_AUDIT_OK:
				const audits = (action as modelAction.DiagnosisNandaActionOK).audits;
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.FETCH_DIAGNOSIS_NANDA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DiagnosisNandaActionOK).audits;
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.FETCH_DIAGNOSIS_NANDA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DiagnosisNandaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.FETCH_LAST_DIAGNOSIS_NANDA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DiagnosisNandaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DiagnosisNandaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.COUNT_DIAGNOSIS_NANDAS_OK:
				const totalCountAfterCount = (action as modelAction.DiagnosisNandaActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DiagnosisNandaModelActionTypes.INITIALISE_DIAGNOSIS_NANDA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DiagnosisNandaActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DiagnosisNandaActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDiagnosisNandaCollectionState
					};
				}
