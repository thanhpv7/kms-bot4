...
import {initialNurseUserCollectionState} from './nurse_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNurseUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NurseUserModelActionTypes.CREATE_NURSE_USER_OK:
				const createdModel = (action as modelAction.NurseUserActionOK ).stateConfig.targetModel;
				...
				NurseUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NurseUserModelActionTypes.CREATE_NURSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseUserModelActionTypes.CREATE_ALL_NURSE_USER_OK:
				const createdModels = (action as modelAction.NurseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NurseUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NurseUserModelActionTypes.CREATE_ALL_NURSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseUserModelActionTypes.DELETE_NURSE_USER_OK:
				const deletedModelId = (action as modelAction.NurseUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NurseUserModelActionTypes.DELETE_ALL_NURSE_USER_OK:
				const deletedModelIds = (action as modelAction.NurseUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NurseUserModelActionTypes.DELETE_NURSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseUserModelActionTypes.UPDATE_NURSE_USER_OK:
				const updatedModel = (action as modelAction.NurseUserActionOK).stateConfig.targetModel;
				...
				NurseUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseUserModelActionTypes.UPDATE_NURSE_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NurseUserModelActionTypes.UPDATE_ALL_NURSE_USER_OK:
				const updatedModels = (action as modelAction.NurseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NurseUserModelActionTypes.UPDATE_ALL_NURSE_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NurseUserModelActionTypes.FETCH_NURSE_USER_OK:
				let fetchedModel = (action as modelAction.NurseUserActionOK).stateConfig.targetModel;
				...
				NurseUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseUserModelActionTypes.FETCH_ALL_NURSE_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.NurseUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NurseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NurseUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NurseUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseUserModelActionTypes.FETCH_NURSE_USER_AUDIT_OK:
				const audits = (action as modelAction.NurseUserActionOK).audits;
				...
				break;
			case modelAction.NurseUserModelActionTypes.FETCH_NURSE_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NurseUserActionOK).audits;
				...
				break;
			case modelAction.NurseUserModelActionTypes.FETCH_NURSE_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NurseUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseUserModelActionTypes.FETCH_LAST_NURSE_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NurseUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NurseUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NurseUserModelActionTypes.COUNT_NURSE_USERS_OK:
				const totalCountAfterCount = (action as modelAction.NurseUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NurseUserModelActionTypes.INITIALISE_NURSE_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NurseUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NurseUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNurseUserCollectionState
					};
				}
