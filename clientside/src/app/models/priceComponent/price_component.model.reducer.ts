...
import {initialPriceComponentCollectionState} from './price_component.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePriceComponentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PriceComponentModelActionTypes.CREATE_PRICE_COMPONENT_OK:
				const createdModel = (action as modelAction.PriceComponentActionOK ).stateConfig.targetModel;
				...
				PriceComponentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PriceComponentModelActionTypes.CREATE_PRICE_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PriceComponentModelActionTypes.CREATE_ALL_PRICE_COMPONENT_OK:
				const createdModels = (action as modelAction.PriceComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PriceComponentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PriceComponentModelActionTypes.CREATE_ALL_PRICE_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PriceComponentModelActionTypes.DELETE_PRICE_COMPONENT_OK:
				const deletedModelId = (action as modelAction.PriceComponentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PriceComponentModelActionTypes.DELETE_ALL_PRICE_COMPONENT_OK:
				const deletedModelIds = (action as modelAction.PriceComponentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PriceComponentModelActionTypes.DELETE_PRICE_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PriceComponentModelActionTypes.UPDATE_PRICE_COMPONENT_OK:
				const updatedModel = (action as modelAction.PriceComponentActionOK).stateConfig.targetModel;
				...
				PriceComponentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PriceComponentModelActionTypes.UPDATE_PRICE_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PriceComponentModelActionTypes.UPDATE_ALL_PRICE_COMPONENT_OK:
				const updatedModels = (action as modelAction.PriceComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PriceComponentModelActionTypes.UPDATE_ALL_PRICE_COMPONENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PriceComponentModelActionTypes.FETCH_PRICE_COMPONENT_OK:
				let fetchedModel = (action as modelAction.PriceComponentActionOK).stateConfig.targetModel;
				...
				PriceComponentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PriceComponentModelActionTypes.FETCH_ALL_PRICE_COMPONENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.PriceComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PriceComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PriceComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PriceComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PriceComponentModelActionTypes.FETCH_PRICE_COMPONENT_AUDIT_OK:
				const audits = (action as modelAction.PriceComponentActionOK).audits;
				...
				break;
			case modelAction.PriceComponentModelActionTypes.FETCH_PRICE_COMPONENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PriceComponentActionOK).audits;
				...
				break;
			case modelAction.PriceComponentModelActionTypes.FETCH_PRICE_COMPONENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PriceComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PriceComponentModelActionTypes.FETCH_LAST_PRICE_COMPONENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PriceComponentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PriceComponentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PriceComponentModelActionTypes.COUNT_PRICE_COMPONENTS_OK:
				const totalCountAfterCount = (action as modelAction.PriceComponentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PriceComponentModelActionTypes.INITIALISE_PRICE_COMPONENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PriceComponentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PriceComponentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPriceComponentCollectionState
					};
				}
