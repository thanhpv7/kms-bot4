...
import {initialInsuranceBenefitPlanCollectionState} from './insurance_benefit_plan.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInsuranceBenefitPlanAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InsuranceBenefitPlanModelActionTypes.CREATE_INSURANCE_BENEFIT_PLAN_OK:
				const createdModel = (action as modelAction.InsuranceBenefitPlanActionOK ).stateConfig.targetModel;
				...
				InsuranceBenefitPlanModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.CREATE_INSURANCE_BENEFIT_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.CREATE_ALL_INSURANCE_BENEFIT_PLAN_OK:
				const createdModels = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InsuranceBenefitPlanModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.CREATE_ALL_INSURANCE_BENEFIT_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.DELETE_INSURANCE_BENEFIT_PLAN_OK:
				const deletedModelId = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.DELETE_ALL_INSURANCE_BENEFIT_PLAN_OK:
				const deletedModelIds = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.DELETE_INSURANCE_BENEFIT_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.UPDATE_INSURANCE_BENEFIT_PLAN_OK:
				const updatedModel = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModel;
				...
				InsuranceBenefitPlanModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.UPDATE_INSURANCE_BENEFIT_PLAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InsuranceBenefitPlanModelActionTypes.UPDATE_ALL_INSURANCE_BENEFIT_PLAN_OK:
				const updatedModels = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.UPDATE_ALL_INSURANCE_BENEFIT_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.FETCH_INSURANCE_BENEFIT_PLAN_OK:
				let fetchedModel = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModel;
				...
				InsuranceBenefitPlanModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.FETCH_ALL_INSURANCE_BENEFIT_PLAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InsuranceBenefitPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.FETCH_INSURANCE_BENEFIT_PLAN_AUDIT_OK:
				const audits = (action as modelAction.InsuranceBenefitPlanActionOK).audits;
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.FETCH_INSURANCE_BENEFIT_PLAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InsuranceBenefitPlanActionOK).audits;
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.FETCH_INSURANCE_BENEFIT_PLAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InsuranceBenefitPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.FETCH_LAST_INSURANCE_BENEFIT_PLAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InsuranceBenefitPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.COUNT_INSURANCE_BENEFIT_PLANS_OK:
				const totalCountAfterCount = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InsuranceBenefitPlanModelActionTypes.INITIALISE_INSURANCE_BENEFIT_PLAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InsuranceBenefitPlanActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInsuranceBenefitPlanCollectionState
					};
				}
