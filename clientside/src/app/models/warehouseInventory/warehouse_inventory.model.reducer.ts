...
import {initialWarehouseInventoryCollectionState} from './warehouse_inventory.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseWarehouseInventoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.WarehouseInventoryModelActionTypes.CREATE_WAREHOUSE_INVENTORY_OK:
				const createdModel = (action as modelAction.WarehouseInventoryActionOK ).stateConfig.targetModel;
				...
				WarehouseInventoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.CREATE_WAREHOUSE_INVENTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInventoryModelActionTypes.CREATE_ALL_WAREHOUSE_INVENTORY_OK:
				const createdModels = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					WarehouseInventoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.CREATE_ALL_WAREHOUSE_INVENTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInventoryModelActionTypes.DELETE_WAREHOUSE_INVENTORY_OK:
				const deletedModelId = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.DELETE_ALL_WAREHOUSE_INVENTORY_OK:
				const deletedModelIds = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.DELETE_WAREHOUSE_INVENTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInventoryModelActionTypes.UPDATE_WAREHOUSE_INVENTORY_OK:
				const updatedModel = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModel;
				...
				WarehouseInventoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.UPDATE_WAREHOUSE_INVENTORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.WarehouseInventoryModelActionTypes.UPDATE_ALL_WAREHOUSE_INVENTORY_OK:
				const updatedModels = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.UPDATE_ALL_WAREHOUSE_INVENTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseInventoryModelActionTypes.FETCH_WAREHOUSE_INVENTORY_OK:
				let fetchedModel = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModel;
				...
				WarehouseInventoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.FETCH_ALL_WAREHOUSE_INVENTORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.WarehouseInventoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => WarehouseInventoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.FETCH_WAREHOUSE_INVENTORY_AUDIT_OK:
				const audits = (action as modelAction.WarehouseInventoryActionOK).audits;
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.FETCH_WAREHOUSE_INVENTORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.WarehouseInventoryActionOK).audits;
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.FETCH_WAREHOUSE_INVENTORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => WarehouseInventoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.FETCH_LAST_WAREHOUSE_INVENTORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.WarehouseInventoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => WarehouseInventoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.COUNT_WAREHOUSE_INVENTORYS_OK:
				const totalCountAfterCount = (action as modelAction.WarehouseInventoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.WarehouseInventoryModelActionTypes.INITIALISE_WAREHOUSE_INVENTORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.WarehouseInventoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.WarehouseInventoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialWarehouseInventoryCollectionState
					};
				}
