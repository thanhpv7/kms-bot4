...
import {initialInvoicePaymentInsuranceCollectionState} from './invoice_payment_insurance.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInvoicePaymentInsuranceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InvoicePaymentInsuranceModelActionTypes.CREATE_INVOICE_PAYMENT_INSURANCE_OK:
				const createdModel = (action as modelAction.InvoicePaymentInsuranceActionOK ).stateConfig.targetModel;
				...
				InvoicePaymentInsuranceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.CREATE_INVOICE_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.CREATE_ALL_INVOICE_PAYMENT_INSURANCE_OK:
				const createdModels = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InvoicePaymentInsuranceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.CREATE_ALL_INVOICE_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.DELETE_INVOICE_PAYMENT_INSURANCE_OK:
				const deletedModelId = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.DELETE_ALL_INVOICE_PAYMENT_INSURANCE_OK:
				const deletedModelIds = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.DELETE_INVOICE_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.UPDATE_INVOICE_PAYMENT_INSURANCE_OK:
				const updatedModel = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModel;
				...
				InvoicePaymentInsuranceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.UPDATE_INVOICE_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InvoicePaymentInsuranceModelActionTypes.UPDATE_ALL_INVOICE_PAYMENT_INSURANCE_OK:
				const updatedModels = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.UPDATE_ALL_INVOICE_PAYMENT_INSURANCE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_INVOICE_PAYMENT_INSURANCE_OK:
				let fetchedModel = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModel;
				...
				InvoicePaymentInsuranceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_ALL_INVOICE_PAYMENT_INSURANCE_OK:
				const totalCountAfterFetchAll = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InvoicePaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_INVOICE_PAYMENT_INSURANCE_AUDIT_OK:
				const audits = (action as modelAction.InvoicePaymentInsuranceActionOK).audits;
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_INVOICE_PAYMENT_INSURANCE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InvoicePaymentInsuranceActionOK).audits;
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_INVOICE_PAYMENT_INSURANCE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InvoicePaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.FETCH_LAST_INVOICE_PAYMENT_INSURANCE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InvoicePaymentInsuranceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.COUNT_INVOICE_PAYMENT_INSURANCES_OK:
				const totalCountAfterCount = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InvoicePaymentInsuranceModelActionTypes.INITIALISE_INVOICE_PAYMENT_INSURANCE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InvoicePaymentInsuranceActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInvoicePaymentInsuranceCollectionState
					};
				}
