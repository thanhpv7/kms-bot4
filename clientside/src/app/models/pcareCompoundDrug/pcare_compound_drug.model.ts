...
import {PcareCompoundDrugItemModel} from '../pcareCompoundDrugItem/pcare_compound_drug_item.model';
 ...
 */
export class PcareCompoundDrugModel extends AbstractModel {
	/**
	...
	readonly className = 'PcareCompoundDrugModel';
	...
	get valueToDisplay(): string {
		...
		return PcareCompoundDrugModel.displayAttributes.map((attr) => this[attr]).join(' ');
		// % protected region % [Change displayName here if needed] end
	}

	/**
	 * Compound Name - Nama Racikan.
	 */
	compoundName: string;
	...
	pcareCompoundDrugItemsIds: string[] = [];

	pcareCompoundDrugItems: PcareCompoundDrugItemModel[];

	modelPropGroups: { [s: string]: Group } = PcareCompoundDrugModel.modelPropGroups;
	...
	static getProps(): ModelProperty[] {
		return super.getProps().concat([
			...
			{
				name: 'packageQty',
				// % protected region % [Set displayName for Package Qty here] off begin
				displayName: 'Package Qty',
				// % protected region % [Set displayName for Package Qty here] end
				type: ModelPropertyType.NUMBER,
				// % protected region % [Set display element type for Package Qty here] off begin
				elementType: ElementType.NUMBER,
				// % protected region % [Set display element type for Package Qty here] end
				// % protected region % [Set isSensitive for Package Qty here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Package Qty here] end
				// % protected region % [Set readonly for Package Qty here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Package Qty here] end
				validators: [
					// % protected region % [Add other validators for Package Qty here] off begin
					// % protected region % [Add other validators for Package Qty here] end
				],
				// % protected region % [Add any additional model attribute properties for Package Qty here] off begin
				// % protected region % [Add any additional model attribute properties for Package Qty here] end
			},
			{
				name: 'day',
			...
			{
				name: 'namaObatNonDPHO',
				// % protected region % [Set displayName for Nama Obat Non DPHO here] off begin
				displayName: 'Nama Obat Non DPHO',
				// % protected region % [Set displayName for Nama Obat Non DPHO here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Nama Obat Non DPHO here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Nama Obat Non DPHO here] end
				// % protected region % [Set isSensitive for Nama Obat Non DPHO here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Nama Obat Non DPHO here] end
				// % protected region % [Set readonly for Nama Obat Non DPHO here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Nama Obat Non DPHO here] end
				validators: [
					// % protected region % [Add other validators for Nama Obat Non DPHO here] off begin
					// % protected region % [Add other validators for Nama Obat Non DPHO here] end
				],
				// % protected region % [Add any additional model attribute properties for Nama Obat Non DPHO here] off begin
				// % protected region % [Add any additional model attribute properties for Nama Obat Non DPHO here] end
			},
			{
				name: 'drugQty',
	...
	static getRelations(): ModelRelation[] {
		return super.getRelations().concat([
			{
				id: 'pcareCompoundDrugItems',
				type: ModelRelationType.MANY,
				name: 'pcareCompoundDrugItemsIds',
				// % protected region % [Customise your 1-1 or 1-M label for PCare Compound Drug Items here] off begin
				label: 'PCare Compound Drug Items',
				// % protected region % [Customise your 1-1 or 1-M label for PCare Compound Drug Items here] end
				entityName: 'PcareCompoundDrugItemModel',
				// % protected region % [Customise your display name for PCare Compound Drug Items here] off begin
				displayName: 'drugCode',
				// % protected region % [Customise your display name for PCare Compound Drug Items here] end
				validators: [
					// % protected region % [Add other validators for PCare Compound Drug Items here] off begin
					// % protected region % [Add other validators for PCare Compound Drug Items here] end
				],
				// % protected region % [Add any additional field for relation PCare Compound Drug Items here] off begin
				// % protected region % [Add any additional field for relation PCare Compound Drug Items here] end
			},
			{
				id: 'pcareExamination',
	 ...
	 */
	static deepParse(data: string | { [K in keyof PcareCompoundDrugModel]?: PcareCompoundDrugModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new PcareCompoundDrugModel(data);
		}
		...
		if (json.pcareCompoundDrugItems) {
			currentModel.pcareCompoundDrugItemsIds = json.pcareCompoundDrugItems.map(model => model.id);
			returned = _.union(returned, _.flatten(json.pcareCompoundDrugItems.map(model => PcareCompoundDrugItemModel.deepParse(model))));
		}
	 ...
	 * @example
	 *
	 * `let pcareCompoundDrugModel = new PcareCompoundDrugModel(data);`
	 *
	 * @param data The input data to be initialised as the PcareCompoundDrugModel,
	 *    it is expected as a JSON string or as a nullable PcareCompoundDrugModel.
	 */
	constructor(data?: string | Partial<PcareCompoundDrugModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<PcareCompoundDrugModel>
				: data;

			this.compoundName = json.compoundName;
			this.signaDays = json.signaDays;
			this.signaTimes = json.signaTimes;
			this.packageQty = json.packageQty;
			this.day = json.day;
			this.requestQty = json.requestQty;
			this.kdObatSK = json.kdObatSK;
			this.racikan = json.racikan;
			this.kdRacikan = json.kdRacikan;
			this.namaObatNonDPHO = json.namaObatNonDPHO;
			this.drugQty = json.drugQty;
			this.pcareExaminationId = json.pcareExaminationId;
			this.pcareExamination = json.pcareExamination;
			this.pcareCompoundDrugItemsIds = json.pcareCompoundDrugItemsIds;
			this.pcareCompoundDrugItems = json.pcareCompoundDrugItems;
	...
	toJSON() {
		return {
			...
			namaObatNonDPHO: this.namaObatNonDPHO,
			drugQty: this.drugQty,
			pcareExaminationId: this.pcareExaminationId,
			pcareCompoundDrugItemsIds: this.pcareCompoundDrugItemsIds,
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		PcareCompoundDrugModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): PcareCompoundDrugModel {
		...
		if (updates.packageQty) {
			newModelJson.packageQty = updates.packageQty;
		}
		...
		if (updates.namaObatNonDPHO) {
			newModelJson.namaObatNonDPHO = updates.namaObatNonDPHO;
		}
		...
		if (updates.pcareCompoundDrugItemsIds) {
			newModelJson.pcareCompoundDrugItemsIds = updates.pcareCompoundDrugItemsIds;
		}

		return new PcareCompoundDrugModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof PcareCompoundDrugModel)) {
			return {};
		}
