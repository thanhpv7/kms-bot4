...
import {initialPcareCompoundDrugCollectionState} from './pcare_compound_drug.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareCompoundDrugAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareCompoundDrugModelActionTypes.CREATE_PCARE_COMPOUND_DRUG_OK:
				const createdModel = (action as modelAction.PcareCompoundDrugActionOK ).stateConfig.targetModel;
				...
				PcareCompoundDrugModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.CREATE_PCARE_COMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.CREATE_ALL_PCARE_COMPOUND_DRUG_OK:
				const createdModels = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareCompoundDrugModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.CREATE_ALL_PCARE_COMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.DELETE_PCARE_COMPOUND_DRUG_OK:
				const deletedModelId = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.DELETE_ALL_PCARE_COMPOUND_DRUG_OK:
				const deletedModelIds = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.DELETE_PCARE_COMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.UPDATE_PCARE_COMPOUND_DRUG_OK:
				const updatedModel = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModel;
				...
				PcareCompoundDrugModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.UPDATE_PCARE_COMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareCompoundDrugModelActionTypes.UPDATE_ALL_PCARE_COMPOUND_DRUG_OK:
				const updatedModels = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.UPDATE_ALL_PCARE_COMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_OK:
				let fetchedModel = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModel;
				...
				PcareCompoundDrugModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.FETCH_ALL_PCARE_COMPOUND_DRUG_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareCompoundDrugModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_AUDIT_OK:
				const audits = (action as modelAction.PcareCompoundDrugActionOK).audits;
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareCompoundDrugActionOK).audits;
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareCompoundDrugModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.FETCH_LAST_PCARE_COMPOUND_DRUG_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareCompoundDrugModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.COUNT_PCARE_COMPOUND_DRUGS_OK:
				const totalCountAfterCount = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareCompoundDrugModelActionTypes.INITIALISE_PCARE_COMPOUND_DRUG_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareCompoundDrugActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareCompoundDrugCollectionState
					};
				}
