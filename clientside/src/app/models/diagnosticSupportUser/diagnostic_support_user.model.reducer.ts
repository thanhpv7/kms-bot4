...
import {initialDiagnosticSupportUserCollectionState} from './diagnostic_support_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDiagnosticSupportUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DiagnosticSupportUserModelActionTypes.CREATE_DIAGNOSTIC_SUPPORT_USER_OK:
				const createdModel = (action as modelAction.DiagnosticSupportUserActionOK ).stateConfig.targetModel;
				...
				DiagnosticSupportUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.CREATE_DIAGNOSTIC_SUPPORT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.CREATE_ALL_DIAGNOSTIC_SUPPORT_USER_OK:
				const createdModels = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DiagnosticSupportUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.CREATE_ALL_DIAGNOSTIC_SUPPORT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_USER_OK:
				const deletedModelId = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.DELETE_ALL_DIAGNOSTIC_SUPPORT_USER_OK:
				const deletedModelIds = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.UPDATE_DIAGNOSTIC_SUPPORT_USER_OK:
				const updatedModel = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModel;
				...
				DiagnosticSupportUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.UPDATE_DIAGNOSTIC_SUPPORT_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DiagnosticSupportUserModelActionTypes.UPDATE_ALL_DIAGNOSTIC_SUPPORT_USER_OK:
				const updatedModels = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.UPDATE_ALL_DIAGNOSTIC_SUPPORT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_USER_OK:
				let fetchedModel = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModel;
				...
				DiagnosticSupportUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.FETCH_ALL_DIAGNOSTIC_SUPPORT_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DiagnosticSupportUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_USER_AUDIT_OK:
				const audits = (action as modelAction.DiagnosticSupportUserActionOK).audits;
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DiagnosticSupportUserActionOK).audits;
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DiagnosticSupportUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.FETCH_LAST_DIAGNOSTIC_SUPPORT_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DiagnosticSupportUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.COUNT_DIAGNOSTIC_SUPPORT_USERS_OK:
				const totalCountAfterCount = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DiagnosticSupportUserModelActionTypes.INITIALISE_DIAGNOSTIC_SUPPORT_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DiagnosticSupportUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDiagnosticSupportUserCollectionState
					};
				}
