 ...
 */
export class PatientPaymentSelfPayingModel extends AbstractModel {
	/**
	...
	readonly className = 'PatientPaymentSelfPayingModel';
	...
	get valueToDisplay(): string {
		...
		return PatientPaymentSelfPayingModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = PatientPaymentSelfPayingModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof PatientPaymentSelfPayingModel]?: PatientPaymentSelfPayingModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new PatientPaymentSelfPayingModel(data);
		}
	 ...
	 * @example
	 *
	 * `let patientPaymentSelfPayingModel = new PatientPaymentSelfPayingModel(data);`
	 *
	 * @param data The input data to be initialised as the PatientPaymentSelfPayingModel,
	 *    it is expected as a JSON string or as a nullable PatientPaymentSelfPayingModel.
	 */
	constructor(data?: string | Partial<PatientPaymentSelfPayingModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<PatientPaymentSelfPayingModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		PatientPaymentSelfPayingModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): PatientPaymentSelfPayingModel {
		...
		return new PatientPaymentSelfPayingModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof PatientPaymentSelfPayingModel)) {
			return {};
		}
