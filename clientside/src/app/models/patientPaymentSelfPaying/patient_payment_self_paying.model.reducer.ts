...
import {initialPatientPaymentSelfPayingCollectionState} from './patient_payment_self_paying.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientPaymentSelfPayingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientPaymentSelfPayingModelActionTypes.CREATE_PATIENT_PAYMENT_SELF_PAYING_OK:
				const createdModel = (action as modelAction.PatientPaymentSelfPayingActionOK ).stateConfig.targetModel;
				...
				PatientPaymentSelfPayingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.CREATE_PATIENT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_SELF_PAYING_OK:
				const createdModels = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientPaymentSelfPayingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.CREATE_ALL_PATIENT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.DELETE_PATIENT_PAYMENT_SELF_PAYING_OK:
				const deletedModelId = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.DELETE_ALL_PATIENT_PAYMENT_SELF_PAYING_OK:
				const deletedModelIds = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.DELETE_PATIENT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.UPDATE_PATIENT_PAYMENT_SELF_PAYING_OK:
				const updatedModel = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModel;
				...
				PatientPaymentSelfPayingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.UPDATE_PATIENT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientPaymentSelfPayingModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_SELF_PAYING_OK:
				const updatedModels = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.UPDATE_ALL_PATIENT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_PATIENT_PAYMENT_SELF_PAYING_OK:
				let fetchedModel = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModel;
				...
				PatientPaymentSelfPayingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_ALL_PATIENT_PAYMENT_SELF_PAYING_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientPaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_PATIENT_PAYMENT_SELF_PAYING_AUDIT_OK:
				const audits = (action as modelAction.PatientPaymentSelfPayingActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_PATIENT_PAYMENT_SELF_PAYING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientPaymentSelfPayingActionOK).audits;
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_PATIENT_PAYMENT_SELF_PAYING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientPaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.FETCH_LAST_PATIENT_PAYMENT_SELF_PAYING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientPaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.COUNT_PATIENT_PAYMENT_SELF_PAYINGS_OK:
				const totalCountAfterCount = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientPaymentSelfPayingModelActionTypes.INITIALISE_PATIENT_PAYMENT_SELF_PAYING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientPaymentSelfPayingCollectionState
					};
				}
