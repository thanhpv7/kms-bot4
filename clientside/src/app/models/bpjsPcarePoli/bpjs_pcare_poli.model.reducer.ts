...
import {initialBpjsPcarePoliCollectionState} from './bpjs_pcare_poli.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcarePoliAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcarePoliModelActionTypes.CREATE_BPJS_PCARE_POLI_OK:
				const createdModel = (action as modelAction.BpjsPcarePoliActionOK ).stateConfig.targetModel;
				...
				BpjsPcarePoliModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.CREATE_BPJS_PCARE_POLI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.CREATE_ALL_BPJS_PCARE_POLI_OK:
				const createdModels = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcarePoliModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.CREATE_ALL_BPJS_PCARE_POLI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.DELETE_BPJS_PCARE_POLI_OK:
				const deletedModelId = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.DELETE_ALL_BPJS_PCARE_POLI_OK:
				const deletedModelIds = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.DELETE_BPJS_PCARE_POLI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.UPDATE_BPJS_PCARE_POLI_OK:
				const updatedModel = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModel;
				...
				BpjsPcarePoliModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.UPDATE_BPJS_PCARE_POLI_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcarePoliModelActionTypes.UPDATE_ALL_BPJS_PCARE_POLI_OK:
				const updatedModels = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.UPDATE_ALL_BPJS_PCARE_POLI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.FETCH_BPJS_PCARE_POLI_OK:
				let fetchedModel = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModel;
				...
				BpjsPcarePoliModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.FETCH_ALL_BPJS_PCARE_POLI_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcarePoliModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.FETCH_BPJS_PCARE_POLI_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcarePoliActionOK).audits;
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.FETCH_BPJS_PCARE_POLI_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcarePoliActionOK).audits;
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.FETCH_BPJS_PCARE_POLI_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcarePoliModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.FETCH_LAST_BPJS_PCARE_POLI_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcarePoliModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.COUNT_BPJS_PCARE_POLIS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcarePoliModelActionTypes.INITIALISE_BPJS_PCARE_POLI_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcarePoliActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcarePoliActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcarePoliCollectionState
					};
				}
