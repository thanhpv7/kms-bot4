 ...
 */
export class BpjsPcarePoliModel extends AbstractModel {
	/**
	...
	readonly className = 'BpjsPcarePoliModel';
	...
	get valueToDisplay(): string {
		...
		return BpjsPcarePoliModel.displayAttributes.map((attr) => this[attr]).join(' ');
	 ...
	 * .
	 */
	polisakit: boolean = false;

	serviceId: string;

	service: ServiceModel;

	modelPropGroups: { [s: string]: Group } = BpjsPcarePoliModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof BpjsPcarePoliModel]?: BpjsPcarePoliModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BpjsPcarePoliModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bpjsPcarePoliModel = new BpjsPcarePoliModel(data);`
	 *
	 * @param data The input data to be initialised as the BpjsPcarePoliModel,
	 *    it is expected as a JSON string or as a nullable BpjsPcarePoliModel.
	 */
	constructor(data?: string | Partial<BpjsPcarePoliModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BpjsPcarePoliModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BpjsPcarePoliModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BpjsPcarePoliModel {
		...
		return new BpjsPcarePoliModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BpjsPcarePoliModel)) {
			return {};
		}
