...
import {initialPatientConsentCollectionState} from './patient_consent.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientConsentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientConsentModelActionTypes.CREATE_PATIENT_CONSENT_OK:
				const createdModel = (action as modelAction.PatientConsentActionOK ).stateConfig.targetModel;
				...
				PatientConsentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientConsentModelActionTypes.CREATE_PATIENT_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientConsentModelActionTypes.CREATE_ALL_PATIENT_CONSENT_OK:
				const createdModels = (action as modelAction.PatientConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientConsentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientConsentModelActionTypes.CREATE_ALL_PATIENT_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientConsentModelActionTypes.DELETE_PATIENT_CONSENT_OK:
				const deletedModelId = (action as modelAction.PatientConsentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientConsentModelActionTypes.DELETE_ALL_PATIENT_CONSENT_OK:
				const deletedModelIds = (action as modelAction.PatientConsentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientConsentModelActionTypes.DELETE_PATIENT_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientConsentModelActionTypes.UPDATE_PATIENT_CONSENT_OK:
				const updatedModel = (action as modelAction.PatientConsentActionOK).stateConfig.targetModel;
				...
				PatientConsentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientConsentModelActionTypes.UPDATE_PATIENT_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientConsentModelActionTypes.UPDATE_ALL_PATIENT_CONSENT_OK:
				const updatedModels = (action as modelAction.PatientConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientConsentModelActionTypes.UPDATE_ALL_PATIENT_CONSENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientConsentModelActionTypes.FETCH_PATIENT_CONSENT_OK:
				let fetchedModel = (action as modelAction.PatientConsentActionOK).stateConfig.targetModel;
				...
				PatientConsentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientConsentModelActionTypes.FETCH_ALL_PATIENT_CONSENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientConsentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientConsentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientConsentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientConsentModelActionTypes.FETCH_PATIENT_CONSENT_AUDIT_OK:
				const audits = (action as modelAction.PatientConsentActionOK).audits;
				...
				break;
			case modelAction.PatientConsentModelActionTypes.FETCH_PATIENT_CONSENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientConsentActionOK).audits;
				...
				break;
			case modelAction.PatientConsentModelActionTypes.FETCH_PATIENT_CONSENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientConsentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientConsentModelActionTypes.FETCH_LAST_PATIENT_CONSENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientConsentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientConsentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientConsentModelActionTypes.COUNT_PATIENT_CONSENTS_OK:
				const totalCountAfterCount = (action as modelAction.PatientConsentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientConsentModelActionTypes.INITIALISE_PATIENT_CONSENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientConsentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientConsentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientConsentCollectionState
					};
				}
