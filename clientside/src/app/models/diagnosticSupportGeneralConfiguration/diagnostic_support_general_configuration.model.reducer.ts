...
import {initialDiagnosticSupportGeneralConfigurationCollectionState} from './diagnostic_support_general_configuration.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDiagnosticSupportGeneralConfigurationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.CREATE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				const createdModel = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK ).stateConfig.targetModel;
				...
				DiagnosticSupportGeneralConfigurationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.CREATE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.CREATE_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				const createdModels = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DiagnosticSupportGeneralConfigurationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.CREATE_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				const deletedModelId = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.DELETE_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				const deletedModelIds = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.DELETE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.UPDATE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				const updatedModel = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModel;
				...
				DiagnosticSupportGeneralConfigurationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.UPDATE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.UPDATE_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				const updatedModels = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.UPDATE_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				let fetchedModel = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModel;
				...
				DiagnosticSupportGeneralConfigurationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_ALL_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DiagnosticSupportGeneralConfigurationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_AUDIT_OK:
				const audits = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).audits;
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).audits;
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DiagnosticSupportGeneralConfigurationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.FETCH_LAST_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DiagnosticSupportGeneralConfigurationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.COUNT_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATIONS_OK:
				const totalCountAfterCount = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DiagnosticSupportGeneralConfigurationModelActionTypes.INITIALISE_DIAGNOSTIC_SUPPORT_GENERAL_CONFIGURATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DiagnosticSupportGeneralConfigurationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDiagnosticSupportGeneralConfigurationCollectionState
					};
				}
