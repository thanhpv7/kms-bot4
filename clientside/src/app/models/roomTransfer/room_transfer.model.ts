 ...
 */
export class RoomTransferModel extends AbstractModel {
	/**
	...
	readonly className = 'RoomTransferModel';
	...
	get valueToDisplay(): string {
		...
		return RoomTransferModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	transferringStaffId: string;

	transferringStaff: StaffModel;

	modelPropGroups: { [s: string]: Group } = RoomTransferModel.modelPropGroups;
	...
	static getProps(): ModelProperty[] {
		return super.getProps().concat([
			...
			{
				name: 'transferringStaffName',
				// % protected region % [Set displayName for Transferring Staff Name here] off begin
				displayName: 'Transferring Staff Name',
				// % protected region % [Set displayName for Transferring Staff Name here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Transferring Staff Name here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Transferring Staff Name here] end
				// % protected region % [Set isSensitive for Transferring Staff Name here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Transferring Staff Name here] end
				// % protected region % [Set readonly for Transferring Staff Name here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Transferring Staff Name here] end
				validators: [
					// % protected region % [Add other validators for Transferring Staff Name here] off begin
					// % protected region % [Add other validators for Transferring Staff Name here] end
				],
				// % protected region % [Add any additional model attribute properties for Transferring Staff Name here] off begin
				// % protected region % [Add any additional model attribute properties for Transferring Staff Name here] end
			},
	...
	static getRelations(): ModelRelation[] {
		return super.getRelations().concat([
			...
			{
				id: 'transferringStaff',
				type: ModelRelationType.ONE,
				name: 'transferringStaffId',
				// % protected region % [Customise your label for Transferring Staff here] off begin
				label: 'Transferring Staff',
				// % protected region % [Customise your label for Transferring Staff here] end
				entityName: 'StaffModel',
				// % protected region % [Customise your display name for Transferring Staff here] off begin
				// TODO change implementation to use OrderBy or create new metamodel property DisplayBy
				displayName: 'staffNumber',
				// % protected region % [Customise your display name for Transferring Staff here] end
				validators: [
					// % protected region % [Add other validators for Transferring Staff here] off begin
					// % protected region % [Add other validators for Transferring Staff here] end
				],
				// % protected region % [Add any additional field for relation Transferring Staff here] off begin
				// % protected region % [Add any additional field for relation Transferring Staff here] end
			},
		]);
	}
	 ...
	 */
	static deepParse(data: string | { [K in keyof RoomTransferModel]?: RoomTransferModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new RoomTransferModel(data);
		}
		...
		// Incoming one to many
		if (json.transferringStaff) {
			currentModel.transferringStaffId = json.transferringStaff.id;
			returned = _.union(returned, StaffModel.deepParse(json.transferringStaff));
		}
	 ...
	 * @example
	 *
	 * `let roomTransferModel = new RoomTransferModel(data);`
	 *
	 * @param data The input data to be initialised as the RoomTransferModel,
	 *    it is expected as a JSON string or as a nullable RoomTransferModel.
	 */
	constructor(data?: string | Partial<RoomTransferModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<RoomTransferModel>
				: data;
			...
			this.transferringStaffName = json.transferringStaffName;
			...
			this.transferringStaffId = json.transferringStaffId;
			this.transferringStaff = json.transferringStaff;
	...
	toJSON() {
		return {
			...
			transferringStaffName: this.transferringStaffName,
			bedFacilityId: this.bedFacilityId,
			medicalExaminationRecordId: this.medicalExaminationRecordId,
			transferringStaffId: this.transferringStaffId,
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		RoomTransferModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): RoomTransferModel {
		...
		if (updates.transferringStaffName) {
			newModelJson.transferringStaffName = updates.transferringStaffName;
		}
		...
		if (updates.transferringStaffId) {
			newModelJson.transferringStaffId = updates.transferringStaffId;
		}

		return new RoomTransferModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof RoomTransferModel)) {
			return {};
		}
	...
	syncRelations() {
		...
		if (!this.transferringStaff) {
			this.transferringStaffId = null;
		} else {
			this.transferringStaffId = this.transferringStaff.id;
		}
