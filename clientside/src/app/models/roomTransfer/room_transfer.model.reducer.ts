...
import {initialRoomTransferCollectionState} from './room_transfer.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRoomTransferAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RoomTransferModelActionTypes.CREATE_ROOM_TRANSFER_OK:
				const createdModel = (action as modelAction.RoomTransferActionOK ).stateConfig.targetModel;
				...
				RoomTransferModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RoomTransferModelActionTypes.CREATE_ROOM_TRANSFER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomTransferModelActionTypes.CREATE_ALL_ROOM_TRANSFER_OK:
				const createdModels = (action as modelAction.RoomTransferActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RoomTransferModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RoomTransferModelActionTypes.CREATE_ALL_ROOM_TRANSFER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomTransferModelActionTypes.DELETE_ROOM_TRANSFER_OK:
				const deletedModelId = (action as modelAction.RoomTransferActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoomTransferModelActionTypes.DELETE_ALL_ROOM_TRANSFER_OK:
				const deletedModelIds = (action as modelAction.RoomTransferActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoomTransferModelActionTypes.DELETE_ROOM_TRANSFER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomTransferModelActionTypes.UPDATE_ROOM_TRANSFER_OK:
				const updatedModel = (action as modelAction.RoomTransferActionOK).stateConfig.targetModel;
				...
				RoomTransferModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomTransferModelActionTypes.UPDATE_ROOM_TRANSFER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RoomTransferModelActionTypes.UPDATE_ALL_ROOM_TRANSFER_OK:
				const updatedModels = (action as modelAction.RoomTransferActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoomTransferModelActionTypes.UPDATE_ALL_ROOM_TRANSFER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomTransferModelActionTypes.FETCH_ROOM_TRANSFER_OK:
				let fetchedModel = (action as modelAction.RoomTransferActionOK).stateConfig.targetModel;
				...
				RoomTransferModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomTransferModelActionTypes.FETCH_ALL_ROOM_TRANSFER_OK:
				const totalCountAfterFetchAll = (action as modelAction.RoomTransferActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RoomTransferActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RoomTransferActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RoomTransferModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomTransferModelActionTypes.FETCH_ROOM_TRANSFER_AUDIT_OK:
				const audits = (action as modelAction.RoomTransferActionOK).audits;
				...
				break;
			case modelAction.RoomTransferModelActionTypes.FETCH_ROOM_TRANSFER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RoomTransferActionOK).audits;
				...
				break;
			case modelAction.RoomTransferModelActionTypes.FETCH_ROOM_TRANSFER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RoomTransferModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomTransferModelActionTypes.FETCH_LAST_ROOM_TRANSFER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RoomTransferActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RoomTransferModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomTransferModelActionTypes.COUNT_ROOM_TRANSFERS_OK:
				const totalCountAfterCount = (action as modelAction.RoomTransferActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RoomTransferModelActionTypes.INITIALISE_ROOM_TRANSFER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RoomTransferActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RoomTransferActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialRoomTransferCollectionState
					};
				}
