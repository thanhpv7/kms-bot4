...
import {initialBatchStockBalancePerDateCollectionState} from './batch_stock_balance_per_date.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBatchStockBalancePerDateAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BatchStockBalancePerDateModelActionTypes.CREATE_BATCH_STOCK_BALANCE_PER_DATE_OK:
				const createdModel = (action as modelAction.BatchStockBalancePerDateActionOK ).stateConfig.targetModel;
				...
				BatchStockBalancePerDateModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.CREATE_BATCH_STOCK_BALANCE_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.CREATE_ALL_BATCH_STOCK_BALANCE_PER_DATE_OK:
				const createdModels = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BatchStockBalancePerDateModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.CREATE_ALL_BATCH_STOCK_BALANCE_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.DELETE_BATCH_STOCK_BALANCE_PER_DATE_OK:
				const deletedModelId = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.DELETE_ALL_BATCH_STOCK_BALANCE_PER_DATE_OK:
				const deletedModelIds = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.DELETE_BATCH_STOCK_BALANCE_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.UPDATE_BATCH_STOCK_BALANCE_PER_DATE_OK:
				const updatedModel = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModel;
				...
				BatchStockBalancePerDateModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.UPDATE_BATCH_STOCK_BALANCE_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BatchStockBalancePerDateModelActionTypes.UPDATE_ALL_BATCH_STOCK_BALANCE_PER_DATE_OK:
				const updatedModels = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.UPDATE_ALL_BATCH_STOCK_BALANCE_PER_DATE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_BATCH_STOCK_BALANCE_PER_DATE_OK:
				let fetchedModel = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModel;
				...
				BatchStockBalancePerDateModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_ALL_BATCH_STOCK_BALANCE_PER_DATE_OK:
				const totalCountAfterFetchAll = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BatchStockBalancePerDateModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_BATCH_STOCK_BALANCE_PER_DATE_AUDIT_OK:
				const audits = (action as modelAction.BatchStockBalancePerDateActionOK).audits;
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_BATCH_STOCK_BALANCE_PER_DATE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BatchStockBalancePerDateActionOK).audits;
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_BATCH_STOCK_BALANCE_PER_DATE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BatchStockBalancePerDateModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.FETCH_LAST_BATCH_STOCK_BALANCE_PER_DATE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BatchStockBalancePerDateModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.COUNT_BATCH_STOCK_BALANCE_PER_DATES_OK:
				const totalCountAfterCount = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BatchStockBalancePerDateModelActionTypes.INITIALISE_BATCH_STOCK_BALANCE_PER_DATE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BatchStockBalancePerDateActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBatchStockBalancePerDateCollectionState
					};
				}
