 ...
 */
export class BatchStockBalancePerDateModel extends AbstractModel {
	/**
	...
	readonly className = 'BatchStockBalancePerDateModel';
	...
	get valueToDisplay(): string {
		...
		return BatchStockBalancePerDateModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BatchStockBalancePerDateModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof BatchStockBalancePerDateModel]?: BatchStockBalancePerDateModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BatchStockBalancePerDateModel(data);
		}
	 ...
	 * @example
	 *
	 * `let batchStockBalancePerDateModel = new BatchStockBalancePerDateModel(data);`
	 *
	 * @param data The input data to be initialised as the BatchStockBalancePerDateModel,
	 *    it is expected as a JSON string or as a nullable BatchStockBalancePerDateModel.
	 */
	constructor(data?: string | Partial<BatchStockBalancePerDateModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BatchStockBalancePerDateModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BatchStockBalancePerDateModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BatchStockBalancePerDateModel {
		...
		return new BatchStockBalancePerDateModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BatchStockBalancePerDateModel)) {
			return {};
		}
