...
import {initialFacilityUserCollectionState} from './facility_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseFacilityUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.FacilityUserModelActionTypes.CREATE_FACILITY_USER_OK:
				const createdModel = (action as modelAction.FacilityUserActionOK ).stateConfig.targetModel;
				...
				FacilityUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.FacilityUserModelActionTypes.CREATE_FACILITY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityUserModelActionTypes.CREATE_ALL_FACILITY_USER_OK:
				const createdModels = (action as modelAction.FacilityUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					FacilityUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.FacilityUserModelActionTypes.CREATE_ALL_FACILITY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityUserModelActionTypes.DELETE_FACILITY_USER_OK:
				const deletedModelId = (action as modelAction.FacilityUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FacilityUserModelActionTypes.DELETE_ALL_FACILITY_USER_OK:
				const deletedModelIds = (action as modelAction.FacilityUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FacilityUserModelActionTypes.DELETE_FACILITY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityUserModelActionTypes.UPDATE_FACILITY_USER_OK:
				const updatedModel = (action as modelAction.FacilityUserActionOK).stateConfig.targetModel;
				...
				FacilityUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityUserModelActionTypes.UPDATE_FACILITY_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.FacilityUserModelActionTypes.UPDATE_ALL_FACILITY_USER_OK:
				const updatedModels = (action as modelAction.FacilityUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FacilityUserModelActionTypes.UPDATE_ALL_FACILITY_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FacilityUserModelActionTypes.FETCH_FACILITY_USER_OK:
				let fetchedModel = (action as modelAction.FacilityUserActionOK).stateConfig.targetModel;
				...
				FacilityUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityUserModelActionTypes.FETCH_ALL_FACILITY_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.FacilityUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.FacilityUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.FacilityUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => FacilityUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityUserModelActionTypes.FETCH_FACILITY_USER_AUDIT_OK:
				const audits = (action as modelAction.FacilityUserActionOK).audits;
				...
				break;
			case modelAction.FacilityUserModelActionTypes.FETCH_FACILITY_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.FacilityUserActionOK).audits;
				...
				break;
			case modelAction.FacilityUserModelActionTypes.FETCH_FACILITY_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => FacilityUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityUserModelActionTypes.FETCH_LAST_FACILITY_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.FacilityUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => FacilityUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FacilityUserModelActionTypes.COUNT_FACILITY_USERS_OK:
				const totalCountAfterCount = (action as modelAction.FacilityUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.FacilityUserModelActionTypes.INITIALISE_FACILITY_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.FacilityUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.FacilityUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialFacilityUserCollectionState
					};
				}
