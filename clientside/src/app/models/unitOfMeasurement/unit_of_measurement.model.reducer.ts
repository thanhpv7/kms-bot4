...
import {initialUnitOfMeasurementCollectionState} from './unit_of_measurement.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseUnitOfMeasurementAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.UnitOfMeasurementModelActionTypes.CREATE_UNIT_OF_MEASUREMENT_OK:
				const createdModel = (action as modelAction.UnitOfMeasurementActionOK ).stateConfig.targetModel;
				...
				UnitOfMeasurementModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.CREATE_UNIT_OF_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.CREATE_ALL_UNIT_OF_MEASUREMENT_OK:
				const createdModels = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					UnitOfMeasurementModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.CREATE_ALL_UNIT_OF_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.DELETE_UNIT_OF_MEASUREMENT_OK:
				const deletedModelId = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.DELETE_ALL_UNIT_OF_MEASUREMENT_OK:
				const deletedModelIds = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.DELETE_UNIT_OF_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.UPDATE_UNIT_OF_MEASUREMENT_OK:
				const updatedModel = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModel;
				...
				UnitOfMeasurementModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.UPDATE_UNIT_OF_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.UnitOfMeasurementModelActionTypes.UPDATE_ALL_UNIT_OF_MEASUREMENT_OK:
				const updatedModels = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.UPDATE_ALL_UNIT_OF_MEASUREMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.FETCH_UNIT_OF_MEASUREMENT_OK:
				let fetchedModel = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModel;
				...
				UnitOfMeasurementModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.FETCH_ALL_UNIT_OF_MEASUREMENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => UnitOfMeasurementModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.FETCH_UNIT_OF_MEASUREMENT_AUDIT_OK:
				const audits = (action as modelAction.UnitOfMeasurementActionOK).audits;
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.FETCH_UNIT_OF_MEASUREMENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.UnitOfMeasurementActionOK).audits;
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.FETCH_UNIT_OF_MEASUREMENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => UnitOfMeasurementModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.FETCH_LAST_UNIT_OF_MEASUREMENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => UnitOfMeasurementModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.COUNT_UNIT_OF_MEASUREMENTS_OK:
				const totalCountAfterCount = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.UnitOfMeasurementModelActionTypes.INITIALISE_UNIT_OF_MEASUREMENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.UnitOfMeasurementActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.UnitOfMeasurementActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialUnitOfMeasurementCollectionState
					};
				}
