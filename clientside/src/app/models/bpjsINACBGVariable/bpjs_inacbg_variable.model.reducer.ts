...
import {initialBpjsINACBGVariableCollectionState} from './bpjs_inacbg_variable.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsINACBGVariableAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsINACBGVariableModelActionTypes.CREATE_BPJS_INACBG_VARIABLE_OK:
				const createdModel = (action as modelAction.BpjsINACBGVariableActionOK ).stateConfig.targetModel;
				...
				BpjsINACBGVariableModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.CREATE_BPJS_INACBG_VARIABLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.CREATE_ALL_BPJS_INACBG_VARIABLE_OK:
				const createdModels = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsINACBGVariableModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.CREATE_ALL_BPJS_INACBG_VARIABLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.DELETE_BPJS_INACBG_VARIABLE_OK:
				const deletedModelId = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.DELETE_ALL_BPJS_INACBG_VARIABLE_OK:
				const deletedModelIds = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.DELETE_BPJS_INACBG_VARIABLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.UPDATE_BPJS_INACBG_VARIABLE_OK:
				const updatedModel = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModel;
				...
				BpjsINACBGVariableModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.UPDATE_BPJS_INACBG_VARIABLE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsINACBGVariableModelActionTypes.UPDATE_ALL_BPJS_INACBG_VARIABLE_OK:
				const updatedModels = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.UPDATE_ALL_BPJS_INACBG_VARIABLE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.FETCH_BPJS_INACBG_VARIABLE_OK:
				let fetchedModel = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModel;
				...
				BpjsINACBGVariableModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.FETCH_ALL_BPJS_INACBG_VARIABLE_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsINACBGVariableModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.FETCH_BPJS_INACBG_VARIABLE_AUDIT_OK:
				const audits = (action as modelAction.BpjsINACBGVariableActionOK).audits;
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.FETCH_BPJS_INACBG_VARIABLE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsINACBGVariableActionOK).audits;
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.FETCH_BPJS_INACBG_VARIABLE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsINACBGVariableModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.FETCH_LAST_BPJS_INACBG_VARIABLE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsINACBGVariableModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.COUNT_BPJS_INACBG_VARIABLES_OK:
				const totalCountAfterCount = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsINACBGVariableModelActionTypes.INITIALISE_BPJS_INACBG_VARIABLE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsINACBGVariableActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsINACBGVariableCollectionState
					};
				}
