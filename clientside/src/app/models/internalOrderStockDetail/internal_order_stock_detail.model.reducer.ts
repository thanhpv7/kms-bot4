...
import {initialInternalOrderStockDetailCollectionState} from './internal_order_stock_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInternalOrderStockDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InternalOrderStockDetailModelActionTypes.CREATE_INTERNAL_ORDER_STOCK_DETAIL_OK:
				const createdModel = (action as modelAction.InternalOrderStockDetailActionOK ).stateConfig.targetModel;
				...
				InternalOrderStockDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.CREATE_INTERNAL_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.CREATE_ALL_INTERNAL_ORDER_STOCK_DETAIL_OK:
				const createdModels = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InternalOrderStockDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.CREATE_ALL_INTERNAL_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.DELETE_INTERNAL_ORDER_STOCK_DETAIL_OK:
				const deletedModelId = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.DELETE_ALL_INTERNAL_ORDER_STOCK_DETAIL_OK:
				const deletedModelIds = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.DELETE_INTERNAL_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.UPDATE_INTERNAL_ORDER_STOCK_DETAIL_OK:
				const updatedModel = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModel;
				...
				InternalOrderStockDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.UPDATE_INTERNAL_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InternalOrderStockDetailModelActionTypes.UPDATE_ALL_INTERNAL_ORDER_STOCK_DETAIL_OK:
				const updatedModels = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.UPDATE_ALL_INTERNAL_ORDER_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.FETCH_INTERNAL_ORDER_STOCK_DETAIL_OK:
				let fetchedModel = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModel;
				...
				InternalOrderStockDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.FETCH_ALL_INTERNAL_ORDER_STOCK_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InternalOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.FETCH_INTERNAL_ORDER_STOCK_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.InternalOrderStockDetailActionOK).audits;
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.FETCH_INTERNAL_ORDER_STOCK_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InternalOrderStockDetailActionOK).audits;
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.FETCH_INTERNAL_ORDER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InternalOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.FETCH_LAST_INTERNAL_ORDER_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InternalOrderStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.COUNT_INTERNAL_ORDER_STOCK_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InternalOrderStockDetailModelActionTypes.INITIALISE_INTERNAL_ORDER_STOCK_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InternalOrderStockDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInternalOrderStockDetailCollectionState
					};
				}
