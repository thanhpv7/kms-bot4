...
import {initialDoctorScheduleCollectionState} from './doctor_schedule.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDoctorScheduleAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DoctorScheduleModelActionTypes.CREATE_DOCTOR_SCHEDULE_OK:
				const createdModel = (action as modelAction.DoctorScheduleActionOK ).stateConfig.targetModel;
				...
				DoctorScheduleModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.CREATE_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorScheduleModelActionTypes.CREATE_ALL_DOCTOR_SCHEDULE_OK:
				const createdModels = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DoctorScheduleModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.CREATE_ALL_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorScheduleModelActionTypes.DELETE_DOCTOR_SCHEDULE_OK:
				const deletedModelId = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.DELETE_ALL_DOCTOR_SCHEDULE_OK:
				const deletedModelIds = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.DELETE_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorScheduleModelActionTypes.UPDATE_DOCTOR_SCHEDULE_OK:
				const updatedModel = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModel;
				...
				DoctorScheduleModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.UPDATE_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DoctorScheduleModelActionTypes.UPDATE_ALL_DOCTOR_SCHEDULE_OK:
				const updatedModels = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.UPDATE_ALL_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorScheduleModelActionTypes.FETCH_DOCTOR_SCHEDULE_OK:
				let fetchedModel = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModel;
				...
				DoctorScheduleModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.FETCH_ALL_DOCTOR_SCHEDULE_OK:
				const totalCountAfterFetchAll = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DoctorScheduleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DoctorScheduleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.FETCH_DOCTOR_SCHEDULE_AUDIT_OK:
				const audits = (action as modelAction.DoctorScheduleActionOK).audits;
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.FETCH_DOCTOR_SCHEDULE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DoctorScheduleActionOK).audits;
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.FETCH_DOCTOR_SCHEDULE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DoctorScheduleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.FETCH_LAST_DOCTOR_SCHEDULE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DoctorScheduleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DoctorScheduleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.COUNT_DOCTOR_SCHEDULES_OK:
				const totalCountAfterCount = (action as modelAction.DoctorScheduleActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DoctorScheduleModelActionTypes.INITIALISE_DOCTOR_SCHEDULE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DoctorScheduleActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDoctorScheduleCollectionState
					};
				}
