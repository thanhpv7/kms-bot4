...
import {initialTariffPackageCollectionState} from './tariff_package.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTariffPackageAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TariffPackageModelActionTypes.CREATE_TARIFF_PACKAGE_OK:
				const createdModel = (action as modelAction.TariffPackageActionOK ).stateConfig.targetModel;
				...
				TariffPackageModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TariffPackageModelActionTypes.CREATE_TARIFF_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageModelActionTypes.CREATE_ALL_TARIFF_PACKAGE_OK:
				const createdModels = (action as modelAction.TariffPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TariffPackageModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TariffPackageModelActionTypes.CREATE_ALL_TARIFF_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageModelActionTypes.DELETE_TARIFF_PACKAGE_OK:
				const deletedModelId = (action as modelAction.TariffPackageActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageModelActionTypes.DELETE_ALL_TARIFF_PACKAGE_OK:
				const deletedModelIds = (action as modelAction.TariffPackageActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageModelActionTypes.DELETE_TARIFF_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageModelActionTypes.UPDATE_TARIFF_PACKAGE_OK:
				const updatedModel = (action as modelAction.TariffPackageActionOK).stateConfig.targetModel;
				...
				TariffPackageModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageModelActionTypes.UPDATE_TARIFF_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TariffPackageModelActionTypes.UPDATE_ALL_TARIFF_PACKAGE_OK:
				const updatedModels = (action as modelAction.TariffPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffPackageModelActionTypes.UPDATE_ALL_TARIFF_PACKAGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffPackageModelActionTypes.FETCH_TARIFF_PACKAGE_OK:
				let fetchedModel = (action as modelAction.TariffPackageActionOK).stateConfig.targetModel;
				...
				TariffPackageModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageModelActionTypes.FETCH_ALL_TARIFF_PACKAGE_OK:
				const totalCountAfterFetchAll = (action as modelAction.TariffPackageActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TariffPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TariffPackageActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TariffPackageModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageModelActionTypes.FETCH_TARIFF_PACKAGE_AUDIT_OK:
				const audits = (action as modelAction.TariffPackageActionOK).audits;
				...
				break;
			case modelAction.TariffPackageModelActionTypes.FETCH_TARIFF_PACKAGE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TariffPackageActionOK).audits;
				...
				break;
			case modelAction.TariffPackageModelActionTypes.FETCH_TARIFF_PACKAGE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TariffPackageModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageModelActionTypes.FETCH_LAST_TARIFF_PACKAGE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TariffPackageActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TariffPackageModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffPackageModelActionTypes.COUNT_TARIFF_PACKAGES_OK:
				const totalCountAfterCount = (action as modelAction.TariffPackageActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TariffPackageModelActionTypes.INITIALISE_TARIFF_PACKAGE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TariffPackageActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TariffPackageActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTariffPackageCollectionState
					};
				}
