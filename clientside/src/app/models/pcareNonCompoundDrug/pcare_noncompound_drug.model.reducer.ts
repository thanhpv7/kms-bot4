...
import {initialPcareNonCompoundDrugCollectionState} from './pcare_noncompound_drug.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareNonCompoundDrugAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareNonCompoundDrugModelActionTypes.CREATE_PCARE_NONCOMPOUND_DRUG_OK:
				const createdModel = (action as modelAction.PcareNonCompoundDrugActionOK ).stateConfig.targetModel;
				...
				PcareNonCompoundDrugModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.CREATE_PCARE_NONCOMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.CREATE_ALL_PCARE_NONCOMPOUND_DRUG_OK:
				const createdModels = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareNonCompoundDrugModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.CREATE_ALL_PCARE_NONCOMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.DELETE_PCARE_NONCOMPOUND_DRUG_OK:
				const deletedModelId = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.DELETE_ALL_PCARE_NONCOMPOUND_DRUG_OK:
				const deletedModelIds = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.DELETE_PCARE_NONCOMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.UPDATE_PCARE_NONCOMPOUND_DRUG_OK:
				const updatedModel = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModel;
				...
				PcareNonCompoundDrugModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.UPDATE_PCARE_NONCOMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareNonCompoundDrugModelActionTypes.UPDATE_ALL_PCARE_NONCOMPOUND_DRUG_OK:
				const updatedModels = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.UPDATE_ALL_PCARE_NONCOMPOUND_DRUG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_PCARE_NONCOMPOUND_DRUG_OK:
				let fetchedModel = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModel;
				...
				PcareNonCompoundDrugModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_ALL_PCARE_NONCOMPOUND_DRUG_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareNonCompoundDrugModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_PCARE_NONCOMPOUND_DRUG_AUDIT_OK:
				const audits = (action as modelAction.PcareNonCompoundDrugActionOK).audits;
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_PCARE_NONCOMPOUND_DRUG_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareNonCompoundDrugActionOK).audits;
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_PCARE_NONCOMPOUND_DRUG_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareNonCompoundDrugModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.FETCH_LAST_PCARE_NONCOMPOUND_DRUG_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareNonCompoundDrugModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.COUNT_PCARE_NONCOMPOUND_DRUGS_OK:
				const totalCountAfterCount = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareNonCompoundDrugModelActionTypes.INITIALISE_PCARE_NONCOMPOUND_DRUG_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareNonCompoundDrugActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareNonCompoundDrugCollectionState
					};
				}
