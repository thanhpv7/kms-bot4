...
import {initialRoomFacilityCollectionState} from './room_facility.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRoomFacilityAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RoomFacilityModelActionTypes.CREATE_ROOM_FACILITY_OK:
				const createdModel = (action as modelAction.RoomFacilityActionOK ).stateConfig.targetModel;
				...
				RoomFacilityModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.CREATE_ROOM_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomFacilityModelActionTypes.CREATE_ALL_ROOM_FACILITY_OK:
				const createdModels = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RoomFacilityModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.CREATE_ALL_ROOM_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomFacilityModelActionTypes.DELETE_ROOM_FACILITY_OK:
				const deletedModelId = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.DELETE_ALL_ROOM_FACILITY_OK:
				const deletedModelIds = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.DELETE_ROOM_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomFacilityModelActionTypes.UPDATE_ROOM_FACILITY_OK:
				const updatedModel = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModel;
				...
				RoomFacilityModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.UPDATE_ROOM_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RoomFacilityModelActionTypes.UPDATE_ALL_ROOM_FACILITY_OK:
				const updatedModels = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.UPDATE_ALL_ROOM_FACILITY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RoomFacilityModelActionTypes.FETCH_ROOM_FACILITY_OK:
				let fetchedModel = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModel;
				...
				RoomFacilityModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.FETCH_ALL_ROOM_FACILITY_OK:
				const totalCountAfterFetchAll = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RoomFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RoomFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.FETCH_ROOM_FACILITY_AUDIT_OK:
				const audits = (action as modelAction.RoomFacilityActionOK).audits;
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.FETCH_ROOM_FACILITY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RoomFacilityActionOK).audits;
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.FETCH_ROOM_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RoomFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.FETCH_LAST_ROOM_FACILITY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RoomFacilityActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RoomFacilityModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.COUNT_ROOM_FACILITYS_OK:
				const totalCountAfterCount = (action as modelAction.RoomFacilityActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RoomFacilityModelActionTypes.INITIALISE_ROOM_FACILITY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RoomFacilityActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RoomFacilityActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialRoomFacilityCollectionState
					};
				}
