...
import {initialCashierUserCollectionState} from './cashier_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCashierUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CashierUserModelActionTypes.CREATE_CASHIER_USER_OK:
				const createdModel = (action as modelAction.CashierUserActionOK ).stateConfig.targetModel;
				...
				CashierUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CashierUserModelActionTypes.CREATE_CASHIER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashierUserModelActionTypes.CREATE_ALL_CASHIER_USER_OK:
				const createdModels = (action as modelAction.CashierUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CashierUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CashierUserModelActionTypes.CREATE_ALL_CASHIER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashierUserModelActionTypes.DELETE_CASHIER_USER_OK:
				const deletedModelId = (action as modelAction.CashierUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashierUserModelActionTypes.DELETE_ALL_CASHIER_USER_OK:
				const deletedModelIds = (action as modelAction.CashierUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashierUserModelActionTypes.DELETE_CASHIER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashierUserModelActionTypes.UPDATE_CASHIER_USER_OK:
				const updatedModel = (action as modelAction.CashierUserActionOK).stateConfig.targetModel;
				...
				CashierUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashierUserModelActionTypes.UPDATE_CASHIER_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CashierUserModelActionTypes.UPDATE_ALL_CASHIER_USER_OK:
				const updatedModels = (action as modelAction.CashierUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CashierUserModelActionTypes.UPDATE_ALL_CASHIER_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CashierUserModelActionTypes.FETCH_CASHIER_USER_OK:
				let fetchedModel = (action as modelAction.CashierUserActionOK).stateConfig.targetModel;
				...
				CashierUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashierUserModelActionTypes.FETCH_ALL_CASHIER_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.CashierUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CashierUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CashierUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CashierUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashierUserModelActionTypes.FETCH_CASHIER_USER_AUDIT_OK:
				const audits = (action as modelAction.CashierUserActionOK).audits;
				...
				break;
			case modelAction.CashierUserModelActionTypes.FETCH_CASHIER_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CashierUserActionOK).audits;
				...
				break;
			case modelAction.CashierUserModelActionTypes.FETCH_CASHIER_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CashierUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashierUserModelActionTypes.FETCH_LAST_CASHIER_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CashierUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CashierUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CashierUserModelActionTypes.COUNT_CASHIER_USERS_OK:
				const totalCountAfterCount = (action as modelAction.CashierUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CashierUserModelActionTypes.INITIALISE_CASHIER_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CashierUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CashierUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCashierUserCollectionState
					};
				}
