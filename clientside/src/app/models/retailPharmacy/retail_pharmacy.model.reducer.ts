...
import {initialRetailPharmacyCollectionState} from './retail_pharmacy.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRetailPharmacyAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RetailPharmacyModelActionTypes.CREATE_RETAIL_PHARMACY_OK:
				const createdModel = (action as modelAction.RetailPharmacyActionOK ).stateConfig.targetModel;
				...
				RetailPharmacyModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.CREATE_RETAIL_PHARMACY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyModelActionTypes.CREATE_ALL_RETAIL_PHARMACY_OK:
				const createdModels = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RetailPharmacyModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.CREATE_ALL_RETAIL_PHARMACY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyModelActionTypes.DELETE_RETAIL_PHARMACY_OK:
				const deletedModelId = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.DELETE_ALL_RETAIL_PHARMACY_OK:
				const deletedModelIds = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.DELETE_RETAIL_PHARMACY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyModelActionTypes.UPDATE_RETAIL_PHARMACY_OK:
				const updatedModel = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModel;
				...
				RetailPharmacyModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.UPDATE_RETAIL_PHARMACY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RetailPharmacyModelActionTypes.UPDATE_ALL_RETAIL_PHARMACY_OK:
				const updatedModels = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.UPDATE_ALL_RETAIL_PHARMACY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyModelActionTypes.FETCH_RETAIL_PHARMACY_OK:
				let fetchedModel = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModel;
				...
				RetailPharmacyModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.FETCH_ALL_RETAIL_PHARMACY_OK:
				const totalCountAfterFetchAll = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RetailPharmacyActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RetailPharmacyModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.FETCH_RETAIL_PHARMACY_AUDIT_OK:
				const audits = (action as modelAction.RetailPharmacyActionOK).audits;
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.FETCH_RETAIL_PHARMACY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RetailPharmacyActionOK).audits;
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.FETCH_RETAIL_PHARMACY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RetailPharmacyModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.FETCH_LAST_RETAIL_PHARMACY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RetailPharmacyModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.COUNT_RETAIL_PHARMACYS_OK:
				const totalCountAfterCount = (action as modelAction.RetailPharmacyActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RetailPharmacyModelActionTypes.INITIALISE_RETAIL_PHARMACY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RetailPharmacyActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RetailPharmacyActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialRetailPharmacyCollectionState
					};
				}
