...
import {initialInventoryBatchCorrectionCollectionState} from './inventory_batch_correction.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInventoryBatchCorrectionAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InventoryBatchCorrectionModelActionTypes.CREATE_INVENTORY_BATCH_CORRECTION_OK:
				const createdModel = (action as modelAction.InventoryBatchCorrectionActionOK ).stateConfig.targetModel;
				...
				InventoryBatchCorrectionModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.CREATE_INVENTORY_BATCH_CORRECTION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.CREATE_ALL_INVENTORY_BATCH_CORRECTION_OK:
				const createdModels = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InventoryBatchCorrectionModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.CREATE_ALL_INVENTORY_BATCH_CORRECTION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION_OK:
				const deletedModelId = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.DELETE_ALL_INVENTORY_BATCH_CORRECTION_OK:
				const deletedModelIds = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.DELETE_INVENTORY_BATCH_CORRECTION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.UPDATE_INVENTORY_BATCH_CORRECTION_OK:
				const updatedModel = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModel;
				...
				InventoryBatchCorrectionModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.UPDATE_INVENTORY_BATCH_CORRECTION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InventoryBatchCorrectionModelActionTypes.UPDATE_ALL_INVENTORY_BATCH_CORRECTION_OK:
				const updatedModels = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.UPDATE_ALL_INVENTORY_BATCH_CORRECTION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_OK:
				let fetchedModel = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModel;
				...
				InventoryBatchCorrectionModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_ALL_INVENTORY_BATCH_CORRECTION_OK:
				const totalCountAfterFetchAll = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InventoryBatchCorrectionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_AUDIT_OK:
				const audits = (action as modelAction.InventoryBatchCorrectionActionOK).audits;
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InventoryBatchCorrectionActionOK).audits;
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_INVENTORY_BATCH_CORRECTION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InventoryBatchCorrectionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.FETCH_LAST_INVENTORY_BATCH_CORRECTION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InventoryBatchCorrectionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.COUNT_INVENTORY_BATCH_CORRECTIONS_OK:
				const totalCountAfterCount = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InventoryBatchCorrectionModelActionTypes.INITIALISE_INVENTORY_BATCH_CORRECTION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InventoryBatchCorrectionActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInventoryBatchCorrectionCollectionState
					};
				}
