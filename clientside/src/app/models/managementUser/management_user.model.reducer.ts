...
import {initialManagementUserCollectionState} from './management_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseManagementUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ManagementUserModelActionTypes.CREATE_MANAGEMENT_USER_OK:
				const createdModel = (action as modelAction.ManagementUserActionOK ).stateConfig.targetModel;
				...
				ManagementUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ManagementUserModelActionTypes.CREATE_MANAGEMENT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ManagementUserModelActionTypes.CREATE_ALL_MANAGEMENT_USER_OK:
				const createdModels = (action as modelAction.ManagementUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ManagementUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ManagementUserModelActionTypes.CREATE_ALL_MANAGEMENT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ManagementUserModelActionTypes.DELETE_MANAGEMENT_USER_OK:
				const deletedModelId = (action as modelAction.ManagementUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ManagementUserModelActionTypes.DELETE_ALL_MANAGEMENT_USER_OK:
				const deletedModelIds = (action as modelAction.ManagementUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ManagementUserModelActionTypes.DELETE_MANAGEMENT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ManagementUserModelActionTypes.UPDATE_MANAGEMENT_USER_OK:
				const updatedModel = (action as modelAction.ManagementUserActionOK).stateConfig.targetModel;
				...
				ManagementUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ManagementUserModelActionTypes.UPDATE_MANAGEMENT_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ManagementUserModelActionTypes.UPDATE_ALL_MANAGEMENT_USER_OK:
				const updatedModels = (action as modelAction.ManagementUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ManagementUserModelActionTypes.UPDATE_ALL_MANAGEMENT_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ManagementUserModelActionTypes.FETCH_MANAGEMENT_USER_OK:
				let fetchedModel = (action as modelAction.ManagementUserActionOK).stateConfig.targetModel;
				...
				ManagementUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ManagementUserModelActionTypes.FETCH_ALL_MANAGEMENT_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.ManagementUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ManagementUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ManagementUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ManagementUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ManagementUserModelActionTypes.FETCH_MANAGEMENT_USER_AUDIT_OK:
				const audits = (action as modelAction.ManagementUserActionOK).audits;
				...
				break;
			case modelAction.ManagementUserModelActionTypes.FETCH_MANAGEMENT_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ManagementUserActionOK).audits;
				...
				break;
			case modelAction.ManagementUserModelActionTypes.FETCH_MANAGEMENT_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ManagementUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ManagementUserModelActionTypes.FETCH_LAST_MANAGEMENT_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ManagementUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ManagementUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ManagementUserModelActionTypes.COUNT_MANAGEMENT_USERS_OK:
				const totalCountAfterCount = (action as modelAction.ManagementUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ManagementUserModelActionTypes.INITIALISE_MANAGEMENT_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ManagementUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ManagementUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialManagementUserCollectionState
					};
				}
