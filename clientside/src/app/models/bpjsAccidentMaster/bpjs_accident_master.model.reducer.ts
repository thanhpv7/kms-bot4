...
import {initialBpjsAccidentMasterCollectionState} from './bpjs_accident_master.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsAccidentMasterAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsAccidentMasterModelActionTypes.CREATE_BPJS_ACCIDENT_MASTER_OK:
				const createdModel = (action as modelAction.BpjsAccidentMasterActionOK ).stateConfig.targetModel;
				...
				BpjsAccidentMasterModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.CREATE_BPJS_ACCIDENT_MASTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.CREATE_ALL_BPJS_ACCIDENT_MASTER_OK:
				const createdModels = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsAccidentMasterModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.CREATE_ALL_BPJS_ACCIDENT_MASTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.DELETE_BPJS_ACCIDENT_MASTER_OK:
				const deletedModelId = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.DELETE_ALL_BPJS_ACCIDENT_MASTER_OK:
				const deletedModelIds = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.DELETE_BPJS_ACCIDENT_MASTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.UPDATE_BPJS_ACCIDENT_MASTER_OK:
				const updatedModel = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModel;
				...
				BpjsAccidentMasterModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.UPDATE_BPJS_ACCIDENT_MASTER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsAccidentMasterModelActionTypes.UPDATE_ALL_BPJS_ACCIDENT_MASTER_OK:
				const updatedModels = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.UPDATE_ALL_BPJS_ACCIDENT_MASTER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.FETCH_BPJS_ACCIDENT_MASTER_OK:
				let fetchedModel = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModel;
				...
				BpjsAccidentMasterModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.FETCH_ALL_BPJS_ACCIDENT_MASTER_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsAccidentMasterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.FETCH_BPJS_ACCIDENT_MASTER_AUDIT_OK:
				const audits = (action as modelAction.BpjsAccidentMasterActionOK).audits;
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.FETCH_BPJS_ACCIDENT_MASTER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsAccidentMasterActionOK).audits;
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.FETCH_BPJS_ACCIDENT_MASTER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsAccidentMasterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.FETCH_LAST_BPJS_ACCIDENT_MASTER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsAccidentMasterModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.COUNT_BPJS_ACCIDENT_MASTERS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsAccidentMasterModelActionTypes.INITIALISE_BPJS_ACCIDENT_MASTER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsAccidentMasterActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsAccidentMasterCollectionState
					};
				}
