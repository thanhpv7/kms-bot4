...
import {initialDailyCareCPPTCollectionState} from './daily_care_cppt.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDailyCareCPPTAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DailyCareCPPTModelActionTypes.CREATE_DAILY_CARE_CPPT_OK:
				const createdModel = (action as modelAction.DailyCareCPPTActionOK ).stateConfig.targetModel;
				...
				DailyCareCPPTModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.CREATE_DAILY_CARE_CPPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DailyCareCPPTModelActionTypes.CREATE_ALL_DAILY_CARE_CPPT_OK:
				const createdModels = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DailyCareCPPTModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.CREATE_ALL_DAILY_CARE_CPPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DailyCareCPPTModelActionTypes.DELETE_DAILY_CARE_CPPT_OK:
				const deletedModelId = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.DELETE_ALL_DAILY_CARE_CPPT_OK:
				const deletedModelIds = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.DELETE_DAILY_CARE_CPPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DailyCareCPPTModelActionTypes.UPDATE_DAILY_CARE_CPPT_OK:
				const updatedModel = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModel;
				...
				DailyCareCPPTModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.UPDATE_DAILY_CARE_CPPT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DailyCareCPPTModelActionTypes.UPDATE_ALL_DAILY_CARE_CPPT_OK:
				const updatedModels = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.UPDATE_ALL_DAILY_CARE_CPPT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DailyCareCPPTModelActionTypes.FETCH_DAILY_CARE_CPPT_OK:
				let fetchedModel = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModel;
				...
				DailyCareCPPTModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.FETCH_ALL_DAILY_CARE_CPPT_OK:
				const totalCountAfterFetchAll = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DailyCareCPPTActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DailyCareCPPTModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.FETCH_DAILY_CARE_CPPT_AUDIT_OK:
				const audits = (action as modelAction.DailyCareCPPTActionOK).audits;
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.FETCH_DAILY_CARE_CPPT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DailyCareCPPTActionOK).audits;
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.FETCH_DAILY_CARE_CPPT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DailyCareCPPTModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.FETCH_LAST_DAILY_CARE_CPPT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DailyCareCPPTActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DailyCareCPPTModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.COUNT_DAILY_CARE_CPPTS_OK:
				const totalCountAfterCount = (action as modelAction.DailyCareCPPTActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DailyCareCPPTModelActionTypes.INITIALISE_DAILY_CARE_CPPT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DailyCareCPPTActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DailyCareCPPTActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDailyCareCPPTCollectionState
					};
				}
