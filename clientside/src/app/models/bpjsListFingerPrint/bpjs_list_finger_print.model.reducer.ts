...
import {initialBpjsListFingerPrintCollectionState} from './bpjs_list_finger_print.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsListFingerPrintAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsListFingerPrintModelActionTypes.CREATE_BPJS_LIST_FINGER_PRINT_OK:
				const createdModel = (action as modelAction.BpjsListFingerPrintActionOK ).stateConfig.targetModel;
				...
				BpjsListFingerPrintModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.CREATE_BPJS_LIST_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.CREATE_ALL_BPJS_LIST_FINGER_PRINT_OK:
				const createdModels = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsListFingerPrintModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.CREATE_ALL_BPJS_LIST_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.DELETE_BPJS_LIST_FINGER_PRINT_OK:
				const deletedModelId = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.DELETE_ALL_BPJS_LIST_FINGER_PRINT_OK:
				const deletedModelIds = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.DELETE_BPJS_LIST_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.UPDATE_BPJS_LIST_FINGER_PRINT_OK:
				const updatedModel = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModel;
				...
				BpjsListFingerPrintModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.UPDATE_BPJS_LIST_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsListFingerPrintModelActionTypes.UPDATE_ALL_BPJS_LIST_FINGER_PRINT_OK:
				const updatedModels = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.UPDATE_ALL_BPJS_LIST_FINGER_PRINT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.FETCH_BPJS_LIST_FINGER_PRINT_OK:
				let fetchedModel = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModel;
				...
				BpjsListFingerPrintModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.FETCH_ALL_BPJS_LIST_FINGER_PRINT_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsListFingerPrintModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.FETCH_BPJS_LIST_FINGER_PRINT_AUDIT_OK:
				const audits = (action as modelAction.BpjsListFingerPrintActionOK).audits;
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.FETCH_BPJS_LIST_FINGER_PRINT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsListFingerPrintActionOK).audits;
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.FETCH_BPJS_LIST_FINGER_PRINT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsListFingerPrintModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.FETCH_LAST_BPJS_LIST_FINGER_PRINT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsListFingerPrintModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.COUNT_BPJS_LIST_FINGER_PRINTS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsListFingerPrintModelActionTypes.INITIALISE_BPJS_LIST_FINGER_PRINT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsListFingerPrintActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsListFingerPrintCollectionState
					};
				}
