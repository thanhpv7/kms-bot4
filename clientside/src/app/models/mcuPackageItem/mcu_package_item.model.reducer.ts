...
import {initialMcuPackageItemCollectionState} from './mcu_package_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMcuPackageItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.McuPackageItemModelActionTypes.CREATE_MCU_PACKAGE_ITEM_OK:
				const createdModel = (action as modelAction.McuPackageItemActionOK ).stateConfig.targetModel;
				...
				McuPackageItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.CREATE_MCU_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageItemModelActionTypes.CREATE_ALL_MCU_PACKAGE_ITEM_OK:
				const createdModels = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					McuPackageItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.CREATE_ALL_MCU_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageItemModelActionTypes.DELETE_MCU_PACKAGE_ITEM_OK:
				const deletedModelId = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.DELETE_ALL_MCU_PACKAGE_ITEM_OK:
				const deletedModelIds = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.DELETE_MCU_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageItemModelActionTypes.UPDATE_MCU_PACKAGE_ITEM_OK:
				const updatedModel = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModel;
				...
				McuPackageItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.UPDATE_MCU_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.McuPackageItemModelActionTypes.UPDATE_ALL_MCU_PACKAGE_ITEM_OK:
				const updatedModels = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.UPDATE_ALL_MCU_PACKAGE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.McuPackageItemModelActionTypes.FETCH_MCU_PACKAGE_ITEM_OK:
				let fetchedModel = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModel;
				...
				McuPackageItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.FETCH_ALL_MCU_PACKAGE_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.McuPackageItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => McuPackageItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.FETCH_MCU_PACKAGE_ITEM_AUDIT_OK:
				const audits = (action as modelAction.McuPackageItemActionOK).audits;
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.FETCH_MCU_PACKAGE_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.McuPackageItemActionOK).audits;
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.FETCH_MCU_PACKAGE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => McuPackageItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.FETCH_LAST_MCU_PACKAGE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.McuPackageItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => McuPackageItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.COUNT_MCU_PACKAGE_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.McuPackageItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.McuPackageItemModelActionTypes.INITIALISE_MCU_PACKAGE_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.McuPackageItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.McuPackageItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMcuPackageItemCollectionState
					};
				}
