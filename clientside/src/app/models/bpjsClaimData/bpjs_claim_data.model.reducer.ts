...
import {initialBpjsClaimDataCollectionState} from './bpjs_claim_data.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsClaimDataAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsClaimDataModelActionTypes.CREATE_BPJS_CLAIM_DATA_OK:
				const createdModel = (action as modelAction.BpjsClaimDataActionOK ).stateConfig.targetModel;
				...
				BpjsClaimDataModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.CREATE_BPJS_CLAIM_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimDataModelActionTypes.CREATE_ALL_BPJS_CLAIM_DATA_OK:
				const createdModels = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsClaimDataModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.CREATE_ALL_BPJS_CLAIM_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimDataModelActionTypes.DELETE_BPJS_CLAIM_DATA_OK:
				const deletedModelId = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.DELETE_ALL_BPJS_CLAIM_DATA_OK:
				const deletedModelIds = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.DELETE_BPJS_CLAIM_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimDataModelActionTypes.UPDATE_BPJS_CLAIM_DATA_OK:
				const updatedModel = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModel;
				...
				BpjsClaimDataModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.UPDATE_BPJS_CLAIM_DATA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsClaimDataModelActionTypes.UPDATE_ALL_BPJS_CLAIM_DATA_OK:
				const updatedModels = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.UPDATE_ALL_BPJS_CLAIM_DATA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsClaimDataModelActionTypes.FETCH_BPJS_CLAIM_DATA_OK:
				let fetchedModel = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModel;
				...
				BpjsClaimDataModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.FETCH_ALL_BPJS_CLAIM_DATA_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsClaimDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsClaimDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.FETCH_BPJS_CLAIM_DATA_AUDIT_OK:
				const audits = (action as modelAction.BpjsClaimDataActionOK).audits;
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.FETCH_BPJS_CLAIM_DATA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsClaimDataActionOK).audits;
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.FETCH_BPJS_CLAIM_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsClaimDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.FETCH_LAST_BPJS_CLAIM_DATA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsClaimDataActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsClaimDataModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.COUNT_BPJS_CLAIM_DATAS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsClaimDataActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsClaimDataModelActionTypes.INITIALISE_BPJS_CLAIM_DATA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsClaimDataActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsClaimDataActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsClaimDataCollectionState
					};
				}
