...
import {initialBpjsControlPlanCollectionState} from './bpjs_control_plan.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsControlPlanAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsControlPlanModelActionTypes.CREATE_BPJS_CONTROL_PLAN_OK:
				const createdModel = (action as modelAction.BpjsControlPlanActionOK ).stateConfig.targetModel;
				...
				BpjsControlPlanModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.CREATE_BPJS_CONTROL_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlPlanModelActionTypes.CREATE_ALL_BPJS_CONTROL_PLAN_OK:
				const createdModels = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsControlPlanModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.CREATE_ALL_BPJS_CONTROL_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlPlanModelActionTypes.DELETE_BPJS_CONTROL_PLAN_OK:
				const deletedModelId = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.DELETE_ALL_BPJS_CONTROL_PLAN_OK:
				const deletedModelIds = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.DELETE_BPJS_CONTROL_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlPlanModelActionTypes.UPDATE_BPJS_CONTROL_PLAN_OK:
				const updatedModel = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModel;
				...
				BpjsControlPlanModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.UPDATE_BPJS_CONTROL_PLAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsControlPlanModelActionTypes.UPDATE_ALL_BPJS_CONTROL_PLAN_OK:
				const updatedModels = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.UPDATE_ALL_BPJS_CONTROL_PLAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsControlPlanModelActionTypes.FETCH_BPJS_CONTROL_PLAN_OK:
				let fetchedModel = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModel;
				...
				BpjsControlPlanModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.FETCH_ALL_BPJS_CONTROL_PLAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsControlPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsControlPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.FETCH_BPJS_CONTROL_PLAN_AUDIT_OK:
				const audits = (action as modelAction.BpjsControlPlanActionOK).audits;
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.FETCH_BPJS_CONTROL_PLAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsControlPlanActionOK).audits;
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.FETCH_BPJS_CONTROL_PLAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsControlPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.FETCH_LAST_BPJS_CONTROL_PLAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsControlPlanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsControlPlanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.COUNT_BPJS_CONTROL_PLANS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsControlPlanActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsControlPlanModelActionTypes.INITIALISE_BPJS_CONTROL_PLAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsControlPlanActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsControlPlanActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsControlPlanCollectionState
					};
				}
