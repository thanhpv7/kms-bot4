...
import {initialInvoiceSummaryCollectionState} from './invoice_summary.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInvoiceSummaryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InvoiceSummaryModelActionTypes.CREATE_INVOICE_SUMMARY_OK:
				const createdModel = (action as modelAction.InvoiceSummaryActionOK ).stateConfig.targetModel;
				...
				InvoiceSummaryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.CREATE_INVOICE_SUMMARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceSummaryModelActionTypes.CREATE_ALL_INVOICE_SUMMARY_OK:
				const createdModels = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InvoiceSummaryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.CREATE_ALL_INVOICE_SUMMARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceSummaryModelActionTypes.DELETE_INVOICE_SUMMARY_OK:
				const deletedModelId = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.DELETE_ALL_INVOICE_SUMMARY_OK:
				const deletedModelIds = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.DELETE_INVOICE_SUMMARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceSummaryModelActionTypes.UPDATE_INVOICE_SUMMARY_OK:
				const updatedModel = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModel;
				...
				InvoiceSummaryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.UPDATE_INVOICE_SUMMARY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InvoiceSummaryModelActionTypes.UPDATE_ALL_INVOICE_SUMMARY_OK:
				const updatedModels = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.UPDATE_ALL_INVOICE_SUMMARY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceSummaryModelActionTypes.FETCH_INVOICE_SUMMARY_OK:
				let fetchedModel = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModel;
				...
				InvoiceSummaryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.FETCH_ALL_INVOICE_SUMMARY_OK:
				const totalCountAfterFetchAll = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InvoiceSummaryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InvoiceSummaryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.FETCH_INVOICE_SUMMARY_AUDIT_OK:
				const audits = (action as modelAction.InvoiceSummaryActionOK).audits;
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.FETCH_INVOICE_SUMMARY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InvoiceSummaryActionOK).audits;
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.FETCH_INVOICE_SUMMARY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InvoiceSummaryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.FETCH_LAST_INVOICE_SUMMARY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InvoiceSummaryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InvoiceSummaryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.COUNT_INVOICE_SUMMARYS_OK:
				const totalCountAfterCount = (action as modelAction.InvoiceSummaryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InvoiceSummaryModelActionTypes.INITIALISE_INVOICE_SUMMARY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InvoiceSummaryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InvoiceSummaryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInvoiceSummaryCollectionState
					};
				}
