...
import {initialNhisInvestigationCollectionState} from './nhis_investigation.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisInvestigationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisInvestigationModelActionTypes.CREATE_NHIS_INVESTIGATION_OK:
				const createdModel = (action as modelAction.NhisInvestigationActionOK ).stateConfig.targetModel;
				...
				NhisInvestigationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.CREATE_NHIS_INVESTIGATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisInvestigationModelActionTypes.CREATE_ALL_NHIS_INVESTIGATION_OK:
				const createdModels = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisInvestigationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.CREATE_ALL_NHIS_INVESTIGATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisInvestigationModelActionTypes.DELETE_NHIS_INVESTIGATION_OK:
				const deletedModelId = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.DELETE_ALL_NHIS_INVESTIGATION_OK:
				const deletedModelIds = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.DELETE_NHIS_INVESTIGATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisInvestigationModelActionTypes.UPDATE_NHIS_INVESTIGATION_OK:
				const updatedModel = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModel;
				...
				NhisInvestigationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.UPDATE_NHIS_INVESTIGATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisInvestigationModelActionTypes.UPDATE_ALL_NHIS_INVESTIGATION_OK:
				const updatedModels = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.UPDATE_ALL_NHIS_INVESTIGATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisInvestigationModelActionTypes.FETCH_NHIS_INVESTIGATION_OK:
				let fetchedModel = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModel;
				...
				NhisInvestigationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.FETCH_ALL_NHIS_INVESTIGATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisInvestigationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisInvestigationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.FETCH_NHIS_INVESTIGATION_AUDIT_OK:
				const audits = (action as modelAction.NhisInvestigationActionOK).audits;
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.FETCH_NHIS_INVESTIGATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisInvestigationActionOK).audits;
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.FETCH_NHIS_INVESTIGATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisInvestigationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.FETCH_LAST_NHIS_INVESTIGATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisInvestigationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisInvestigationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.COUNT_NHIS_INVESTIGATIONS_OK:
				const totalCountAfterCount = (action as modelAction.NhisInvestigationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisInvestigationModelActionTypes.INITIALISE_NHIS_INVESTIGATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisInvestigationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisInvestigationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisInvestigationCollectionState
					};
				}
