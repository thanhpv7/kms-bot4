...
import {initialRegistrationUserCollectionState} from './registration_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRegistrationUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RegistrationUserModelActionTypes.CREATE_REGISTRATION_USER_OK:
				const createdModel = (action as modelAction.RegistrationUserActionOK ).stateConfig.targetModel;
				...
				RegistrationUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.CREATE_REGISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationUserModelActionTypes.CREATE_ALL_REGISTRATION_USER_OK:
				const createdModels = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RegistrationUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.CREATE_ALL_REGISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationUserModelActionTypes.DELETE_REGISTRATION_USER_OK:
				const deletedModelId = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.DELETE_ALL_REGISTRATION_USER_OK:
				const deletedModelIds = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.DELETE_REGISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationUserModelActionTypes.UPDATE_REGISTRATION_USER_OK:
				const updatedModel = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModel;
				...
				RegistrationUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.UPDATE_REGISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RegistrationUserModelActionTypes.UPDATE_ALL_REGISTRATION_USER_OK:
				const updatedModels = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.UPDATE_ALL_REGISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RegistrationUserModelActionTypes.FETCH_REGISTRATION_USER_OK:
				let fetchedModel = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModel;
				...
				RegistrationUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.FETCH_ALL_REGISTRATION_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RegistrationUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RegistrationUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.FETCH_REGISTRATION_USER_AUDIT_OK:
				const audits = (action as modelAction.RegistrationUserActionOK).audits;
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.FETCH_REGISTRATION_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RegistrationUserActionOK).audits;
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.FETCH_REGISTRATION_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RegistrationUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.FETCH_LAST_REGISTRATION_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RegistrationUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RegistrationUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.COUNT_REGISTRATION_USERS_OK:
				const totalCountAfterCount = (action as modelAction.RegistrationUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RegistrationUserModelActionTypes.INITIALISE_REGISTRATION_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RegistrationUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RegistrationUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialRegistrationUserCollectionState
					};
				}
