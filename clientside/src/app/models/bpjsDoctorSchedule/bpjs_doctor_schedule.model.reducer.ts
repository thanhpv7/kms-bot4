...
import {initialBpjsDoctorScheduleCollectionState} from './bpjs_doctor_schedule.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsDoctorScheduleAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsDoctorScheduleModelActionTypes.CREATE_BPJS_DOCTOR_SCHEDULE_OK:
				const createdModel = (action as modelAction.BpjsDoctorScheduleActionOK ).stateConfig.targetModel;
				...
				BpjsDoctorScheduleModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.CREATE_BPJS_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.CREATE_ALL_BPJS_DOCTOR_SCHEDULE_OK:
				const createdModels = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsDoctorScheduleModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.CREATE_ALL_BPJS_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.DELETE_BPJS_DOCTOR_SCHEDULE_OK:
				const deletedModelId = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.DELETE_ALL_BPJS_DOCTOR_SCHEDULE_OK:
				const deletedModelIds = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.DELETE_BPJS_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.UPDATE_BPJS_DOCTOR_SCHEDULE_OK:
				const updatedModel = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModel;
				...
				BpjsDoctorScheduleModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.UPDATE_BPJS_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsDoctorScheduleModelActionTypes.UPDATE_ALL_BPJS_DOCTOR_SCHEDULE_OK:
				const updatedModels = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.UPDATE_ALL_BPJS_DOCTOR_SCHEDULE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_BPJS_DOCTOR_SCHEDULE_OK:
				let fetchedModel = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModel;
				...
				BpjsDoctorScheduleModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_ALL_BPJS_DOCTOR_SCHEDULE_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsDoctorScheduleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_BPJS_DOCTOR_SCHEDULE_AUDIT_OK:
				const audits = (action as modelAction.BpjsDoctorScheduleActionOK).audits;
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_BPJS_DOCTOR_SCHEDULE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsDoctorScheduleActionOK).audits;
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_BPJS_DOCTOR_SCHEDULE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsDoctorScheduleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.FETCH_LAST_BPJS_DOCTOR_SCHEDULE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsDoctorScheduleModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.COUNT_BPJS_DOCTOR_SCHEDULES_OK:
				const totalCountAfterCount = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsDoctorScheduleModelActionTypes.INITIALISE_BPJS_DOCTOR_SCHEDULE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsDoctorScheduleActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsDoctorScheduleCollectionState
					};
				}
