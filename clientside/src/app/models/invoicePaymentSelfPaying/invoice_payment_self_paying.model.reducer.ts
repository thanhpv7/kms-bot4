...
import {initialInvoicePaymentSelfPayingCollectionState} from './invoice_payment_self_paying.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInvoicePaymentSelfPayingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.CREATE_INVOICE_PAYMENT_SELF_PAYING_OK:
				const createdModel = (action as modelAction.InvoicePaymentSelfPayingActionOK ).stateConfig.targetModel;
				...
				InvoicePaymentSelfPayingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.CREATE_INVOICE_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.CREATE_ALL_INVOICE_PAYMENT_SELF_PAYING_OK:
				const createdModels = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InvoicePaymentSelfPayingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.CREATE_ALL_INVOICE_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.DELETE_INVOICE_PAYMENT_SELF_PAYING_OK:
				const deletedModelId = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.DELETE_ALL_INVOICE_PAYMENT_SELF_PAYING_OK:
				const deletedModelIds = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.DELETE_INVOICE_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.UPDATE_INVOICE_PAYMENT_SELF_PAYING_OK:
				const updatedModel = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModel;
				...
				InvoicePaymentSelfPayingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.UPDATE_INVOICE_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InvoicePaymentSelfPayingModelActionTypes.UPDATE_ALL_INVOICE_PAYMENT_SELF_PAYING_OK:
				const updatedModels = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.UPDATE_ALL_INVOICE_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_INVOICE_PAYMENT_SELF_PAYING_OK:
				let fetchedModel = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModel;
				...
				InvoicePaymentSelfPayingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_ALL_INVOICE_PAYMENT_SELF_PAYING_OK:
				const totalCountAfterFetchAll = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InvoicePaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_INVOICE_PAYMENT_SELF_PAYING_AUDIT_OK:
				const audits = (action as modelAction.InvoicePaymentSelfPayingActionOK).audits;
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_INVOICE_PAYMENT_SELF_PAYING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InvoicePaymentSelfPayingActionOK).audits;
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_INVOICE_PAYMENT_SELF_PAYING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InvoicePaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.FETCH_LAST_INVOICE_PAYMENT_SELF_PAYING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InvoicePaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.COUNT_INVOICE_PAYMENT_SELF_PAYINGS_OK:
				const totalCountAfterCount = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InvoicePaymentSelfPayingModelActionTypes.INITIALISE_INVOICE_PAYMENT_SELF_PAYING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InvoicePaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInvoicePaymentSelfPayingCollectionState
					};
				}
