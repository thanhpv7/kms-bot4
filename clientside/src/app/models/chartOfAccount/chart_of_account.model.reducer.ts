...
import {initialChartOfAccountCollectionState} from './chart_of_account.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseChartOfAccountAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ChartOfAccountModelActionTypes.CREATE_CHART_OF_ACCOUNT_OK:
				const createdModel = (action as modelAction.ChartOfAccountActionOK ).stateConfig.targetModel;
				...
				ChartOfAccountModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.CREATE_CHART_OF_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountModelActionTypes.CREATE_ALL_CHART_OF_ACCOUNT_OK:
				const createdModels = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ChartOfAccountModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.CREATE_ALL_CHART_OF_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountModelActionTypes.DELETE_CHART_OF_ACCOUNT_OK:
				const deletedModelId = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.DELETE_ALL_CHART_OF_ACCOUNT_OK:
				const deletedModelIds = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.DELETE_CHART_OF_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountModelActionTypes.UPDATE_CHART_OF_ACCOUNT_OK:
				const updatedModel = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModel;
				...
				ChartOfAccountModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.UPDATE_CHART_OF_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ChartOfAccountModelActionTypes.UPDATE_ALL_CHART_OF_ACCOUNT_OK:
				const updatedModels = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.UPDATE_ALL_CHART_OF_ACCOUNT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountModelActionTypes.FETCH_CHART_OF_ACCOUNT_OK:
				let fetchedModel = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModel;
				...
				ChartOfAccountModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.FETCH_ALL_CHART_OF_ACCOUNT_OK:
				const totalCountAfterFetchAll = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ChartOfAccountActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ChartOfAccountModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.FETCH_CHART_OF_ACCOUNT_AUDIT_OK:
				const audits = (action as modelAction.ChartOfAccountActionOK).audits;
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.FETCH_CHART_OF_ACCOUNT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ChartOfAccountActionOK).audits;
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.FETCH_CHART_OF_ACCOUNT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ChartOfAccountModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.FETCH_LAST_CHART_OF_ACCOUNT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ChartOfAccountModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.COUNT_CHART_OF_ACCOUNTS_OK:
				const totalCountAfterCount = (action as modelAction.ChartOfAccountActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ChartOfAccountModelActionTypes.INITIALISE_CHART_OF_ACCOUNT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ChartOfAccountActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ChartOfAccountActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialChartOfAccountCollectionState
					};
				}
