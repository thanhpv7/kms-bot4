...
import {initialChartOfAccountMappingCollectionState} from './chart_of_account_mapping.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseChartOfAccountMappingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ChartOfAccountMappingModelActionTypes.CREATE_CHART_OF_ACCOUNT_MAPPING_OK:
				const createdModel = (action as modelAction.ChartOfAccountMappingActionOK ).stateConfig.targetModel;
				...
				ChartOfAccountMappingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.CREATE_CHART_OF_ACCOUNT_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.CREATE_ALL_CHART_OF_ACCOUNT_MAPPING_OK:
				const createdModels = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ChartOfAccountMappingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.CREATE_ALL_CHART_OF_ACCOUNT_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.DELETE_CHART_OF_ACCOUNT_MAPPING_OK:
				const deletedModelId = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.DELETE_ALL_CHART_OF_ACCOUNT_MAPPING_OK:
				const deletedModelIds = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.DELETE_CHART_OF_ACCOUNT_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.UPDATE_CHART_OF_ACCOUNT_MAPPING_OK:
				const updatedModel = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModel;
				...
				ChartOfAccountMappingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.UPDATE_CHART_OF_ACCOUNT_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ChartOfAccountMappingModelActionTypes.UPDATE_ALL_CHART_OF_ACCOUNT_MAPPING_OK:
				const updatedModels = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.UPDATE_ALL_CHART_OF_ACCOUNT_MAPPING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.FETCH_CHART_OF_ACCOUNT_MAPPING_OK:
				let fetchedModel = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModel;
				...
				ChartOfAccountMappingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.FETCH_ALL_CHART_OF_ACCOUNT_MAPPING_OK:
				const totalCountAfterFetchAll = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ChartOfAccountMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.FETCH_CHART_OF_ACCOUNT_MAPPING_AUDIT_OK:
				const audits = (action as modelAction.ChartOfAccountMappingActionOK).audits;
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.FETCH_CHART_OF_ACCOUNT_MAPPING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ChartOfAccountMappingActionOK).audits;
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.FETCH_CHART_OF_ACCOUNT_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ChartOfAccountMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.FETCH_LAST_CHART_OF_ACCOUNT_MAPPING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ChartOfAccountMappingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.COUNT_CHART_OF_ACCOUNT_MAPPINGS_OK:
				const totalCountAfterCount = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ChartOfAccountMappingModelActionTypes.INITIALISE_CHART_OF_ACCOUNT_MAPPING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ChartOfAccountMappingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialChartOfAccountMappingCollectionState
					};
				}
