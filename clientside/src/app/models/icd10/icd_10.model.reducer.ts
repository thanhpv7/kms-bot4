...
import * as modelAction from './icd_10.model.action';
import {ModelState} from '../model.state';
import {initialIcd10CollectionState} from './icd_10.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseIcd10Action).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.Icd10ModelActionTypes.CREATE_ICD_10_OK:
				const createdModel = (action as modelAction.Icd10ActionOK ).stateConfig.targetModel;
				...
				Icd10Model.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.Icd10ModelActionTypes.CREATE_ICD_10_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd10ModelActionTypes.CREATE_ALL_ICD_10_OK:
				const createdModels = (action as modelAction.Icd10ActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					Icd10Model.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.Icd10ModelActionTypes.CREATE_ALL_ICD_10_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd10ModelActionTypes.DELETE_ICD_10_OK:
				const deletedModelId = (action as modelAction.Icd10ActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.Icd10ModelActionTypes.DELETE_ALL_ICD_10_OK:
				const deletedModelIds = (action as modelAction.Icd10ActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.Icd10ModelActionTypes.DELETE_ICD_10_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd10ModelActionTypes.UPDATE_ICD_10_OK:
				const updatedModel = (action as modelAction.Icd10ActionOK).stateConfig.targetModel;
				...
				Icd10Model.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd10ModelActionTypes.UPDATE_ICD_10_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.Icd10ModelActionTypes.UPDATE_ALL_ICD_10_OK:
				const updatedModels = (action as modelAction.Icd10ActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.Icd10ModelActionTypes.UPDATE_ALL_ICD_10_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.Icd10ModelActionTypes.FETCH_ICD_10_OK:
				let fetchedModel = (action as modelAction.Icd10ActionOK).stateConfig.targetModel;
				...
				Icd10Model.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd10ModelActionTypes.FETCH_ALL_ICD_10_OK:
				const totalCountAfterFetchAll = (action as modelAction.Icd10ActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.Icd10ActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.Icd10ActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => Icd10Model.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd10ModelActionTypes.FETCH_ICD_10_AUDIT_OK:
				const audits = (action as modelAction.Icd10ActionOK).audits;
				...
				break;
			case modelAction.Icd10ModelActionTypes.FETCH_ICD_10_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.Icd10ActionOK).audits;
				...
				break;
			case modelAction.Icd10ModelActionTypes.FETCH_ICD_10_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => Icd10Model.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd10ModelActionTypes.FETCH_LAST_ICD_10_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.Icd10ActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => Icd10Model.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.Icd10ModelActionTypes.COUNT_ICD_10S_OK:
				const totalCountAfterCount = (action as modelAction.Icd10ActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.Icd10ModelActionTypes.INITIALISE_ICD_10_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.Icd10ActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.Icd10ActionOK).stateConfig.queryParams;
