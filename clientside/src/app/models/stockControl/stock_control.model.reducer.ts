...
import {initialStockControlCollectionState} from './stock_control.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseStockControlAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.StockControlModelActionTypes.CREATE_STOCK_CONTROL_OK:
				const createdModel = (action as modelAction.StockControlActionOK ).stateConfig.targetModel;
				...
				StockControlModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.StockControlModelActionTypes.CREATE_STOCK_CONTROL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockControlModelActionTypes.CREATE_ALL_STOCK_CONTROL_OK:
				const createdModels = (action as modelAction.StockControlActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					StockControlModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.StockControlModelActionTypes.CREATE_ALL_STOCK_CONTROL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockControlModelActionTypes.DELETE_STOCK_CONTROL_OK:
				const deletedModelId = (action as modelAction.StockControlActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockControlModelActionTypes.DELETE_ALL_STOCK_CONTROL_OK:
				const deletedModelIds = (action as modelAction.StockControlActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockControlModelActionTypes.DELETE_STOCK_CONTROL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockControlModelActionTypes.UPDATE_STOCK_CONTROL_OK:
				const updatedModel = (action as modelAction.StockControlActionOK).stateConfig.targetModel;
				...
				StockControlModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockControlModelActionTypes.UPDATE_STOCK_CONTROL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.StockControlModelActionTypes.UPDATE_ALL_STOCK_CONTROL_OK:
				const updatedModels = (action as modelAction.StockControlActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockControlModelActionTypes.UPDATE_ALL_STOCK_CONTROL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockControlModelActionTypes.FETCH_STOCK_CONTROL_OK:
				let fetchedModel = (action as modelAction.StockControlActionOK).stateConfig.targetModel;
				...
				StockControlModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockControlModelActionTypes.FETCH_ALL_STOCK_CONTROL_OK:
				const totalCountAfterFetchAll = (action as modelAction.StockControlActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.StockControlActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.StockControlActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => StockControlModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockControlModelActionTypes.FETCH_STOCK_CONTROL_AUDIT_OK:
				const audits = (action as modelAction.StockControlActionOK).audits;
				...
				break;
			case modelAction.StockControlModelActionTypes.FETCH_STOCK_CONTROL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.StockControlActionOK).audits;
				...
				break;
			case modelAction.StockControlModelActionTypes.FETCH_STOCK_CONTROL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.StockControlActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.StockControlActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.StockControlActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => StockControlModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockControlModelActionTypes.FETCH_LAST_STOCK_CONTROL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.StockControlActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.StockControlActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.StockControlActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => StockControlModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockControlModelActionTypes.COUNT_STOCK_CONTROLS_OK:
				const totalCountAfterCount = (action as modelAction.StockControlActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.StockControlModelActionTypes.INITIALISE_STOCK_CONTROL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.StockControlActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.StockControlActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialStockControlCollectionState
					};
				}
