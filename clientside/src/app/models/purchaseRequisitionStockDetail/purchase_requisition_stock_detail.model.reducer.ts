...
import {initialPurchaseRequisitionStockDetailCollectionState} from './purchase_requisition_stock_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePurchaseRequisitionStockDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.CREATE_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				const createdModel = (action as modelAction.PurchaseRequisitionStockDetailActionOK ).stateConfig.targetModel;
				...
				PurchaseRequisitionStockDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.CREATE_PURCHASE_REQUISITION_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.CREATE_ALL_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				const createdModels = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PurchaseRequisitionStockDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.CREATE_ALL_PURCHASE_REQUISITION_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.DELETE_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				const deletedModelId = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.DELETE_ALL_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				const deletedModelIds = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.DELETE_PURCHASE_REQUISITION_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.UPDATE_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				const updatedModel = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModel;
				...
				PurchaseRequisitionStockDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.UPDATE_PURCHASE_REQUISITION_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.UPDATE_ALL_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				const updatedModels = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.UPDATE_ALL_PURCHASE_REQUISITION_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				let fetchedModel = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModel;
				...
				PurchaseRequisitionStockDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_ALL_PURCHASE_REQUISITION_STOCK_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PurchaseRequisitionStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_PURCHASE_REQUISITION_STOCK_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.PurchaseRequisitionStockDetailActionOK).audits;
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_PURCHASE_REQUISITION_STOCK_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PurchaseRequisitionStockDetailActionOK).audits;
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_PURCHASE_REQUISITION_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PurchaseRequisitionStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.FETCH_LAST_PURCHASE_REQUISITION_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PurchaseRequisitionStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.COUNT_PURCHASE_REQUISITION_STOCK_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PurchaseRequisitionStockDetailModelActionTypes.INITIALISE_PURCHASE_REQUISITION_STOCK_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PurchaseRequisitionStockDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPurchaseRequisitionStockDetailCollectionState
					};
				}
