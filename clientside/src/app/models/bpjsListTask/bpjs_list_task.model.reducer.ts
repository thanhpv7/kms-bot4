...
import {initialBpjsListTaskCollectionState} from './bpjs_list_task.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsListTaskAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsListTaskModelActionTypes.CREATE_BPJS_LIST_TASK_OK:
				const createdModel = (action as modelAction.BpjsListTaskActionOK ).stateConfig.targetModel;
				...
				BpjsListTaskModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.CREATE_BPJS_LIST_TASK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListTaskModelActionTypes.CREATE_ALL_BPJS_LIST_TASK_OK:
				const createdModels = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsListTaskModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.CREATE_ALL_BPJS_LIST_TASK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListTaskModelActionTypes.DELETE_BPJS_LIST_TASK_OK:
				const deletedModelId = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.DELETE_ALL_BPJS_LIST_TASK_OK:
				const deletedModelIds = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.DELETE_BPJS_LIST_TASK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListTaskModelActionTypes.UPDATE_BPJS_LIST_TASK_OK:
				const updatedModel = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModel;
				...
				BpjsListTaskModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.UPDATE_BPJS_LIST_TASK_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsListTaskModelActionTypes.UPDATE_ALL_BPJS_LIST_TASK_OK:
				const updatedModels = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.UPDATE_ALL_BPJS_LIST_TASK_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsListTaskModelActionTypes.FETCH_BPJS_LIST_TASK_OK:
				let fetchedModel = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModel;
				...
				BpjsListTaskModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.FETCH_ALL_BPJS_LIST_TASK_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsListTaskActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsListTaskModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.FETCH_BPJS_LIST_TASK_AUDIT_OK:
				const audits = (action as modelAction.BpjsListTaskActionOK).audits;
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.FETCH_BPJS_LIST_TASK_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsListTaskActionOK).audits;
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.FETCH_BPJS_LIST_TASK_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsListTaskModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.FETCH_LAST_BPJS_LIST_TASK_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsListTaskActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsListTaskModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.COUNT_BPJS_LIST_TASKS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsListTaskActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsListTaskModelActionTypes.INITIALISE_BPJS_LIST_TASK_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsListTaskActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsListTaskActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsListTaskCollectionState
					};
				}
