...
import {initialPcareCompoundDrugItemCollectionState} from './pcare_compound_drug_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareCompoundDrugItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareCompoundDrugItemModelActionTypes.CREATE_PCARE_COMPOUND_DRUG_ITEM_OK:
				const createdModel = (action as modelAction.PcareCompoundDrugItemActionOK ).stateConfig.targetModel;
				...
				PcareCompoundDrugItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.CREATE_PCARE_COMPOUND_DRUG_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.CREATE_ALL_PCARE_COMPOUND_DRUG_ITEM_OK:
				const createdModels = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareCompoundDrugItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.CREATE_ALL_PCARE_COMPOUND_DRUG_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.DELETE_PCARE_COMPOUND_DRUG_ITEM_OK:
				const deletedModelId = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.DELETE_ALL_PCARE_COMPOUND_DRUG_ITEM_OK:
				const deletedModelIds = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.DELETE_PCARE_COMPOUND_DRUG_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.UPDATE_PCARE_COMPOUND_DRUG_ITEM_OK:
				const updatedModel = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModel;
				...
				PcareCompoundDrugItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.UPDATE_PCARE_COMPOUND_DRUG_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareCompoundDrugItemModelActionTypes.UPDATE_ALL_PCARE_COMPOUND_DRUG_ITEM_OK:
				const updatedModels = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.UPDATE_ALL_PCARE_COMPOUND_DRUG_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_ITEM_OK:
				let fetchedModel = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModel;
				...
				PcareCompoundDrugItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_ALL_PCARE_COMPOUND_DRUG_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareCompoundDrugItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_ITEM_AUDIT_OK:
				const audits = (action as modelAction.PcareCompoundDrugItemActionOK).audits;
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareCompoundDrugItemActionOK).audits;
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_PCARE_COMPOUND_DRUG_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareCompoundDrugItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.FETCH_LAST_PCARE_COMPOUND_DRUG_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareCompoundDrugItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.COUNT_PCARE_COMPOUND_DRUG_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareCompoundDrugItemModelActionTypes.INITIALISE_PCARE_COMPOUND_DRUG_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareCompoundDrugItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareCompoundDrugItemCollectionState
					};
				}
