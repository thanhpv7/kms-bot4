 ...
 */
export class PatientVisitPaymentSelfPayingModel extends AbstractModel {
	/**
	...
	readonly className = 'PatientVisitPaymentSelfPayingModel';
	...
	get valueToDisplay(): string {
		...
		return PatientVisitPaymentSelfPayingModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = PatientVisitPaymentSelfPayingModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof PatientVisitPaymentSelfPayingModel]?: PatientVisitPaymentSelfPayingModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new PatientVisitPaymentSelfPayingModel(data);
		}
	 ...
	 * @example
	 *
	 * `let patientVisitPaymentSelfPayingModel = new PatientVisitPaymentSelfPayingModel(data);`
	 *
	 * @param data The input data to be initialised as the PatientVisitPaymentSelfPayingModel,
	 *    it is expected as a JSON string or as a nullable PatientVisitPaymentSelfPayingModel.
	 */
	constructor(data?: string | Partial<PatientVisitPaymentSelfPayingModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<PatientVisitPaymentSelfPayingModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		PatientVisitPaymentSelfPayingModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): PatientVisitPaymentSelfPayingModel {
		...
		return new PatientVisitPaymentSelfPayingModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof PatientVisitPaymentSelfPayingModel)) {
			return {};
		}
