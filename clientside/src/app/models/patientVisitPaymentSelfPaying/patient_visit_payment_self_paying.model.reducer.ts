...
import {initialPatientVisitPaymentSelfPayingCollectionState} from './patient_visit_payment_self_paying.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientVisitPaymentSelfPayingAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.CREATE_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				const createdModel = (action as modelAction.PatientVisitPaymentSelfPayingActionOK ).stateConfig.targetModel;
				...
				PatientVisitPaymentSelfPayingModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.CREATE_PATIENT_VISIT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.CREATE_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				const createdModels = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientVisitPaymentSelfPayingModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.CREATE_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.DELETE_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				const deletedModelId = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.DELETE_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				const deletedModelIds = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.DELETE_PATIENT_VISIT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.UPDATE_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				const updatedModel = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModel;
				...
				PatientVisitPaymentSelfPayingModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.UPDATE_PATIENT_VISIT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.UPDATE_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				const updatedModels = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.UPDATE_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				let fetchedModel = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModel;
				...
				PatientVisitPaymentSelfPayingModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_ALL_PATIENT_VISIT_PAYMENT_SELF_PAYING_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientVisitPaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_PATIENT_VISIT_PAYMENT_SELF_PAYING_AUDIT_OK:
				const audits = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).audits;
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_PATIENT_VISIT_PAYMENT_SELF_PAYING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).audits;
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_PATIENT_VISIT_PAYMENT_SELF_PAYING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientVisitPaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.FETCH_LAST_PATIENT_VISIT_PAYMENT_SELF_PAYING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientVisitPaymentSelfPayingModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.COUNT_PATIENT_VISIT_PAYMENT_SELF_PAYINGS_OK:
				const totalCountAfterCount = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientVisitPaymentSelfPayingModelActionTypes.INITIALISE_PATIENT_VISIT_PAYMENT_SELF_PAYING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientVisitPaymentSelfPayingActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientVisitPaymentSelfPayingCollectionState
					};
				}
