...
import {initialBpjsTreatmentRoomCollectionState} from './bpjs_treatment_room.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsTreatmentRoomAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsTreatmentRoomModelActionTypes.CREATE_BPJS_TREATMENT_ROOM_OK:
				const createdModel = (action as modelAction.BpjsTreatmentRoomActionOK ).stateConfig.targetModel;
				...
				BpjsTreatmentRoomModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.CREATE_BPJS_TREATMENT_ROOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.CREATE_ALL_BPJS_TREATMENT_ROOM_OK:
				const createdModels = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsTreatmentRoomModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.CREATE_ALL_BPJS_TREATMENT_ROOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.DELETE_BPJS_TREATMENT_ROOM_OK:
				const deletedModelId = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.DELETE_ALL_BPJS_TREATMENT_ROOM_OK:
				const deletedModelIds = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.DELETE_BPJS_TREATMENT_ROOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.UPDATE_BPJS_TREATMENT_ROOM_OK:
				const updatedModel = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModel;
				...
				BpjsTreatmentRoomModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.UPDATE_BPJS_TREATMENT_ROOM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsTreatmentRoomModelActionTypes.UPDATE_ALL_BPJS_TREATMENT_ROOM_OK:
				const updatedModels = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.UPDATE_ALL_BPJS_TREATMENT_ROOM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_BPJS_TREATMENT_ROOM_OK:
				let fetchedModel = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModel;
				...
				BpjsTreatmentRoomModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_ALL_BPJS_TREATMENT_ROOM_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsTreatmentRoomModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_BPJS_TREATMENT_ROOM_AUDIT_OK:
				const audits = (action as modelAction.BpjsTreatmentRoomActionOK).audits;
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_BPJS_TREATMENT_ROOM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsTreatmentRoomActionOK).audits;
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_BPJS_TREATMENT_ROOM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsTreatmentRoomModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.FETCH_LAST_BPJS_TREATMENT_ROOM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsTreatmentRoomModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.COUNT_BPJS_TREATMENT_ROOMS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsTreatmentRoomModelActionTypes.INITIALISE_BPJS_TREATMENT_ROOM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsTreatmentRoomActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsTreatmentRoomCollectionState
					};
				}
