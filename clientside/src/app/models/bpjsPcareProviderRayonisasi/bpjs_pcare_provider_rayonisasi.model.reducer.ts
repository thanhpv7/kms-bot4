...
import {initialBpjsPcareProviderRayonisasiCollectionState} from './bpjs_pcare_provider_rayonisasi.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcareProviderRayonisasiAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.CREATE_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				const createdModel = (action as modelAction.BpjsPcareProviderRayonisasiActionOK ).stateConfig.targetModel;
				...
				BpjsPcareProviderRayonisasiModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.CREATE_BPJS_PCARE_PROVIDER_RAYONISASI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.CREATE_ALL_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				const createdModels = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcareProviderRayonisasiModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.CREATE_ALL_BPJS_PCARE_PROVIDER_RAYONISASI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.DELETE_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				const deletedModelId = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.DELETE_ALL_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				const deletedModelIds = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.DELETE_BPJS_PCARE_PROVIDER_RAYONISASI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.UPDATE_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				const updatedModel = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModel;
				...
				BpjsPcareProviderRayonisasiModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.UPDATE_BPJS_PCARE_PROVIDER_RAYONISASI_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.UPDATE_ALL_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				const updatedModels = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.UPDATE_ALL_BPJS_PCARE_PROVIDER_RAYONISASI_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				let fetchedModel = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModel;
				...
				BpjsPcareProviderRayonisasiModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_ALL_BPJS_PCARE_PROVIDER_RAYONISASI_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcareProviderRayonisasiModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_BPJS_PCARE_PROVIDER_RAYONISASI_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_BPJS_PCARE_PROVIDER_RAYONISASI_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_BPJS_PCARE_PROVIDER_RAYONISASI_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcareProviderRayonisasiModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.FETCH_LAST_BPJS_PCARE_PROVIDER_RAYONISASI_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcareProviderRayonisasiModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.COUNT_BPJS_PCARE_PROVIDER_RAYONISASIS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcareProviderRayonisasiModelActionTypes.INITIALISE_BPJS_PCARE_PROVIDER_RAYONISASI_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcareProviderRayonisasiActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcareProviderRayonisasiCollectionState
					};
				}
