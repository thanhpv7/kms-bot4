 ...
 */
export class BpjsPcareProviderRayonisasiModel extends AbstractModel {
	/**
	...
	readonly className = 'BpjsPcareProviderRayonisasiModel';
	...
	get valueToDisplay(): string {
		...
		return BpjsPcareProviderRayonisasiModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BpjsPcareProviderRayonisasiModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof BpjsPcareProviderRayonisasiModel]?: BpjsPcareProviderRayonisasiModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BpjsPcareProviderRayonisasiModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bpjsPcareProviderRayonisasiModel = new BpjsPcareProviderRayonisasiModel(data);`
	 *
	 * @param data The input data to be initialised as the BpjsPcareProviderRayonisasiModel,
	 *    it is expected as a JSON string or as a nullable BpjsPcareProviderRayonisasiModel.
	 */
	constructor(data?: string | Partial<BpjsPcareProviderRayonisasiModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BpjsPcareProviderRayonisasiModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BpjsPcareProviderRayonisasiModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BpjsPcareProviderRayonisasiModel {
		...
		return new BpjsPcareProviderRayonisasiModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BpjsPcareProviderRayonisasiModel)) {
			return {};
		}
