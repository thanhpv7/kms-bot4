...
import {initialStockTransactionPerDateTypeCollectionState} from './stock_transaction_per_date_type.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseStockTransactionPerDateTypeAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.StockTransactionPerDateTypeModelActionTypes.CREATE_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				const createdModel = (action as modelAction.StockTransactionPerDateTypeActionOK ).stateConfig.targetModel;
				...
				StockTransactionPerDateTypeModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.CREATE_STOCK_TRANSACTION_PER_DATE_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.CREATE_ALL_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				const createdModels = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					StockTransactionPerDateTypeModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.CREATE_ALL_STOCK_TRANSACTION_PER_DATE_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				const deletedModelId = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.DELETE_ALL_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				const deletedModelIds = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.DELETE_STOCK_TRANSACTION_PER_DATE_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.UPDATE_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				const updatedModel = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModel;
				...
				StockTransactionPerDateTypeModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.UPDATE_STOCK_TRANSACTION_PER_DATE_TYPE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.StockTransactionPerDateTypeModelActionTypes.UPDATE_ALL_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				const updatedModels = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.UPDATE_ALL_STOCK_TRANSACTION_PER_DATE_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				let fetchedModel = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModel;
				...
				StockTransactionPerDateTypeModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_ALL_STOCK_TRANSACTION_PER_DATE_TYPE_OK:
				const totalCountAfterFetchAll = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => StockTransactionPerDateTypeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_TYPE_AUDIT_OK:
				const audits = (action as modelAction.StockTransactionPerDateTypeActionOK).audits;
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_TYPE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.StockTransactionPerDateTypeActionOK).audits;
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_STOCK_TRANSACTION_PER_DATE_TYPE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => StockTransactionPerDateTypeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.FETCH_LAST_STOCK_TRANSACTION_PER_DATE_TYPE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => StockTransactionPerDateTypeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.COUNT_STOCK_TRANSACTION_PER_DATE_TYPES_OK:
				const totalCountAfterCount = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.StockTransactionPerDateTypeModelActionTypes.INITIALISE_STOCK_TRANSACTION_PER_DATE_TYPE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.StockTransactionPerDateTypeActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialStockTransactionPerDateTypeCollectionState
					};
				}
