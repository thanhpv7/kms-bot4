...
import {initialCatalogueMedicalCollectionState} from './catalogue_medical.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCatalogueMedicalAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CatalogueMedicalModelActionTypes.CREATE_CATALOGUE_MEDICAL_OK:
				const createdModel = (action as modelAction.CatalogueMedicalActionOK ).stateConfig.targetModel;
				...
				CatalogueMedicalModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.CREATE_CATALOGUE_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueMedicalModelActionTypes.CREATE_ALL_CATALOGUE_MEDICAL_OK:
				const createdModels = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CatalogueMedicalModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.CREATE_ALL_CATALOGUE_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueMedicalModelActionTypes.DELETE_CATALOGUE_MEDICAL_OK:
				const deletedModelId = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.DELETE_ALL_CATALOGUE_MEDICAL_OK:
				const deletedModelIds = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.DELETE_CATALOGUE_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueMedicalModelActionTypes.UPDATE_CATALOGUE_MEDICAL_OK:
				const updatedModel = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModel;
				...
				CatalogueMedicalModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.UPDATE_CATALOGUE_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CatalogueMedicalModelActionTypes.UPDATE_ALL_CATALOGUE_MEDICAL_OK:
				const updatedModels = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.UPDATE_ALL_CATALOGUE_MEDICAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CatalogueMedicalModelActionTypes.FETCH_CATALOGUE_MEDICAL_OK:
				let fetchedModel = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModel;
				...
				CatalogueMedicalModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.FETCH_ALL_CATALOGUE_MEDICAL_OK:
				const totalCountAfterFetchAll = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CatalogueMedicalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CatalogueMedicalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.FETCH_CATALOGUE_MEDICAL_AUDIT_OK:
				const audits = (action as modelAction.CatalogueMedicalActionOK).audits;
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.FETCH_CATALOGUE_MEDICAL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CatalogueMedicalActionOK).audits;
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.FETCH_CATALOGUE_MEDICAL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CatalogueMedicalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.FETCH_LAST_CATALOGUE_MEDICAL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CatalogueMedicalActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CatalogueMedicalModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.COUNT_CATALOGUE_MEDICALS_OK:
				const totalCountAfterCount = (action as modelAction.CatalogueMedicalActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CatalogueMedicalModelActionTypes.INITIALISE_CATALOGUE_MEDICAL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CatalogueMedicalActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CatalogueMedicalActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCatalogueMedicalCollectionState
					};
				}
