...
import {initialRetailPharmacyStockDetailCollectionState} from './retail_pharmacy_stock_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseRetailPharmacyStockDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.RetailPharmacyStockDetailModelActionTypes.CREATE_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				const createdModel = (action as modelAction.RetailPharmacyStockDetailActionOK ).stateConfig.targetModel;
				...
				RetailPharmacyStockDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.CREATE_RETAIL_PHARMACY_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.CREATE_ALL_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				const createdModels = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					RetailPharmacyStockDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.CREATE_ALL_RETAIL_PHARMACY_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.DELETE_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				const deletedModelId = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.DELETE_ALL_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				const deletedModelIds = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.DELETE_RETAIL_PHARMACY_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.UPDATE_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				const updatedModel = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModel;
				...
				RetailPharmacyStockDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.UPDATE_RETAIL_PHARMACY_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.RetailPharmacyStockDetailModelActionTypes.UPDATE_ALL_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				const updatedModels = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.UPDATE_ALL_RETAIL_PHARMACY_STOCK_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				let fetchedModel = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModel;
				...
				RetailPharmacyStockDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_ALL_RETAIL_PHARMACY_STOCK_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => RetailPharmacyStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_RETAIL_PHARMACY_STOCK_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.RetailPharmacyStockDetailActionOK).audits;
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_RETAIL_PHARMACY_STOCK_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.RetailPharmacyStockDetailActionOK).audits;
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_RETAIL_PHARMACY_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => RetailPharmacyStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.FETCH_LAST_RETAIL_PHARMACY_STOCK_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => RetailPharmacyStockDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.COUNT_RETAIL_PHARMACY_STOCK_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.RetailPharmacyStockDetailModelActionTypes.INITIALISE_RETAIL_PHARMACY_STOCK_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.RetailPharmacyStockDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialRetailPharmacyStockDetailCollectionState
					};
				}
