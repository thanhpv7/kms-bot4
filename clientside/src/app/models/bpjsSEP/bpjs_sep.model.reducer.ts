...
import {initialBpjsSEPCollectionState} from './bpjs_sep.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsSEPAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsSEPModelActionTypes.CREATE_BPJS_SEP_OK:
				const createdModel = (action as modelAction.BpjsSEPActionOK ).stateConfig.targetModel;
				...
				BpjsSEPModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.CREATE_BPJS_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPModelActionTypes.CREATE_ALL_BPJS_SEP_OK:
				const createdModels = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsSEPModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.CREATE_ALL_BPJS_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPModelActionTypes.DELETE_BPJS_SEP_OK:
				const deletedModelId = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.DELETE_ALL_BPJS_SEP_OK:
				const deletedModelIds = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.DELETE_BPJS_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPModelActionTypes.UPDATE_BPJS_SEP_OK:
				const updatedModel = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModel;
				...
				BpjsSEPModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.UPDATE_BPJS_SEP_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsSEPModelActionTypes.UPDATE_ALL_BPJS_SEP_OK:
				const updatedModels = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.UPDATE_ALL_BPJS_SEP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPModelActionTypes.FETCH_BPJS_SEP_OK:
				let fetchedModel = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModel;
				...
				BpjsSEPModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.FETCH_ALL_BPJS_SEP_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsSEPActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsSEPModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.FETCH_BPJS_SEP_AUDIT_OK:
				const audits = (action as modelAction.BpjsSEPActionOK).audits;
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.FETCH_BPJS_SEP_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsSEPActionOK).audits;
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.FETCH_BPJS_SEP_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsSEPModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.FETCH_LAST_BPJS_SEP_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsSEPActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsSEPModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.COUNT_BPJS_SEPS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsSEPActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsSEPModelActionTypes.INITIALISE_BPJS_SEP_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsSEPActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsSEPActionOK).stateConfig.queryParams;
