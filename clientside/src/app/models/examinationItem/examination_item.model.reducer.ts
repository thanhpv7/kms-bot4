...
import {initialExaminationItemCollectionState} from './examination_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseExaminationItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ExaminationItemModelActionTypes.CREATE_EXAMINATION_ITEM_OK:
				const createdModel = (action as modelAction.ExaminationItemActionOK ).stateConfig.targetModel;
				...
				ExaminationItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.CREATE_EXAMINATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationItemModelActionTypes.CREATE_ALL_EXAMINATION_ITEM_OK:
				const createdModels = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ExaminationItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.CREATE_ALL_EXAMINATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationItemModelActionTypes.DELETE_EXAMINATION_ITEM_OK:
				const deletedModelId = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.DELETE_ALL_EXAMINATION_ITEM_OK:
				const deletedModelIds = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.DELETE_EXAMINATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationItemModelActionTypes.UPDATE_EXAMINATION_ITEM_OK:
				const updatedModel = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModel;
				...
				ExaminationItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.UPDATE_EXAMINATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ExaminationItemModelActionTypes.UPDATE_ALL_EXAMINATION_ITEM_OK:
				const updatedModels = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.UPDATE_ALL_EXAMINATION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ExaminationItemModelActionTypes.FETCH_EXAMINATION_ITEM_OK:
				let fetchedModel = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModel;
				...
				ExaminationItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.FETCH_ALL_EXAMINATION_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ExaminationItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ExaminationItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.FETCH_EXAMINATION_ITEM_AUDIT_OK:
				const audits = (action as modelAction.ExaminationItemActionOK).audits;
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.FETCH_EXAMINATION_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ExaminationItemActionOK).audits;
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.FETCH_EXAMINATION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ExaminationItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.FETCH_LAST_EXAMINATION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ExaminationItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ExaminationItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.COUNT_EXAMINATION_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.ExaminationItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ExaminationItemModelActionTypes.INITIALISE_EXAMINATION_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ExaminationItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ExaminationItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialExaminationItemCollectionState
					};
				}
