...
import {initialFluidBalanceDetailCollectionState} from './fluid_balance_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseFluidBalanceDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.FluidBalanceDetailModelActionTypes.CREATE_FLUID_BALANCE_DETAIL_OK:
				const createdModel = (action as modelAction.FluidBalanceDetailActionOK ).stateConfig.targetModel;
				...
				FluidBalanceDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.CREATE_FLUID_BALANCE_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.CREATE_ALL_FLUID_BALANCE_DETAIL_OK:
				const createdModels = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					FluidBalanceDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.CREATE_ALL_FLUID_BALANCE_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.DELETE_FLUID_BALANCE_DETAIL_OK:
				const deletedModelId = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.DELETE_ALL_FLUID_BALANCE_DETAIL_OK:
				const deletedModelIds = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.DELETE_FLUID_BALANCE_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.UPDATE_FLUID_BALANCE_DETAIL_OK:
				const updatedModel = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModel;
				...
				FluidBalanceDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.UPDATE_FLUID_BALANCE_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.FluidBalanceDetailModelActionTypes.UPDATE_ALL_FLUID_BALANCE_DETAIL_OK:
				const updatedModels = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.UPDATE_ALL_FLUID_BALANCE_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.FETCH_FLUID_BALANCE_DETAIL_OK:
				let fetchedModel = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModel;
				...
				FluidBalanceDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.FETCH_ALL_FLUID_BALANCE_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => FluidBalanceDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.FETCH_FLUID_BALANCE_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.FluidBalanceDetailActionOK).audits;
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.FETCH_FLUID_BALANCE_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.FluidBalanceDetailActionOK).audits;
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.FETCH_FLUID_BALANCE_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => FluidBalanceDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.FETCH_LAST_FLUID_BALANCE_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => FluidBalanceDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.COUNT_FLUID_BALANCE_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.FluidBalanceDetailModelActionTypes.INITIALISE_FLUID_BALANCE_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.FluidBalanceDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.FluidBalanceDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialFluidBalanceDetailCollectionState
					};
				}
