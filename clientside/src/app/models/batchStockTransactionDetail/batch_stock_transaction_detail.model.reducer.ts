...
import {initialBatchStockTransactionDetailCollectionState} from './batch_stock_transaction_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBatchStockTransactionDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BatchStockTransactionDetailModelActionTypes.CREATE_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				const createdModel = (action as modelAction.BatchStockTransactionDetailActionOK ).stateConfig.targetModel;
				...
				BatchStockTransactionDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.CREATE_BATCH_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.CREATE_ALL_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				const createdModels = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BatchStockTransactionDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.CREATE_ALL_BATCH_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.DELETE_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				const deletedModelId = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.DELETE_ALL_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				const deletedModelIds = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.DELETE_BATCH_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.UPDATE_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				const updatedModel = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModel;
				...
				BatchStockTransactionDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.UPDATE_BATCH_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BatchStockTransactionDetailModelActionTypes.UPDATE_ALL_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				const updatedModels = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.UPDATE_ALL_BATCH_STOCK_TRANSACTION_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				let fetchedModel = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModel;
				...
				BatchStockTransactionDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_ALL_BATCH_STOCK_TRANSACTION_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BatchStockTransactionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_BATCH_STOCK_TRANSACTION_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.BatchStockTransactionDetailActionOK).audits;
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_BATCH_STOCK_TRANSACTION_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BatchStockTransactionDetailActionOK).audits;
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_BATCH_STOCK_TRANSACTION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BatchStockTransactionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.FETCH_LAST_BATCH_STOCK_TRANSACTION_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BatchStockTransactionDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.COUNT_BATCH_STOCK_TRANSACTION_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BatchStockTransactionDetailModelActionTypes.INITIALISE_BATCH_STOCK_TRANSACTION_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BatchStockTransactionDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBatchStockTransactionDetailCollectionState
					};
				}
