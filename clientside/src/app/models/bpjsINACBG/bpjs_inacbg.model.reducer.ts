...
import {initialBpjsINACBGCollectionState} from './bpjs_inacbg.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsINACBGAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsINACBGModelActionTypes.CREATE_BPJS_INACBG_OK:
				const createdModel = (action as modelAction.BpjsINACBGActionOK ).stateConfig.targetModel;
				...
				BpjsINACBGModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.CREATE_BPJS_INACBG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGModelActionTypes.CREATE_ALL_BPJS_INACBG_OK:
				const createdModels = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsINACBGModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.CREATE_ALL_BPJS_INACBG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGModelActionTypes.DELETE_BPJS_INACBG_OK:
				const deletedModelId = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.DELETE_ALL_BPJS_INACBG_OK:
				const deletedModelIds = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.DELETE_BPJS_INACBG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGModelActionTypes.UPDATE_BPJS_INACBG_OK:
				const updatedModel = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModel;
				...
				BpjsINACBGModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.UPDATE_BPJS_INACBG_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsINACBGModelActionTypes.UPDATE_ALL_BPJS_INACBG_OK:
				const updatedModels = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.UPDATE_ALL_BPJS_INACBG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsINACBGModelActionTypes.FETCH_BPJS_INACBG_OK:
				let fetchedModel = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModel;
				...
				BpjsINACBGModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.FETCH_ALL_BPJS_INACBG_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsINACBGActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsINACBGModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.FETCH_BPJS_INACBG_AUDIT_OK:
				const audits = (action as modelAction.BpjsINACBGActionOK).audits;
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.FETCH_BPJS_INACBG_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsINACBGActionOK).audits;
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.FETCH_BPJS_INACBG_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsINACBGModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.FETCH_LAST_BPJS_INACBG_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsINACBGActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsINACBGModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.COUNT_BPJS_INACBGS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsINACBGActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsINACBGModelActionTypes.INITIALISE_BPJS_INACBG_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsINACBGActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsINACBGActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsINACBGCollectionState
					};
				}
