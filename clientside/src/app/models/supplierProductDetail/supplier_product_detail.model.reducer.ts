...
import {initialSupplierProductDetailCollectionState} from './supplier_product_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseSupplierProductDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.SupplierProductDetailModelActionTypes.CREATE_SUPPLIER_PRODUCT_DETAIL_OK:
				const createdModel = (action as modelAction.SupplierProductDetailActionOK ).stateConfig.targetModel;
				...
				SupplierProductDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.CREATE_SUPPLIER_PRODUCT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierProductDetailModelActionTypes.CREATE_ALL_SUPPLIER_PRODUCT_DETAIL_OK:
				const createdModels = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					SupplierProductDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.CREATE_ALL_SUPPLIER_PRODUCT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierProductDetailModelActionTypes.DELETE_SUPPLIER_PRODUCT_DETAIL_OK:
				const deletedModelId = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.DELETE_ALL_SUPPLIER_PRODUCT_DETAIL_OK:
				const deletedModelIds = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.DELETE_SUPPLIER_PRODUCT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierProductDetailModelActionTypes.UPDATE_SUPPLIER_PRODUCT_DETAIL_OK:
				const updatedModel = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModel;
				...
				SupplierProductDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.UPDATE_SUPPLIER_PRODUCT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.SupplierProductDetailModelActionTypes.UPDATE_ALL_SUPPLIER_PRODUCT_DETAIL_OK:
				const updatedModels = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.UPDATE_ALL_SUPPLIER_PRODUCT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.SupplierProductDetailModelActionTypes.FETCH_SUPPLIER_PRODUCT_DETAIL_OK:
				let fetchedModel = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModel;
				...
				SupplierProductDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.FETCH_ALL_SUPPLIER_PRODUCT_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.SupplierProductDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => SupplierProductDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.FETCH_SUPPLIER_PRODUCT_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.SupplierProductDetailActionOK).audits;
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.FETCH_SUPPLIER_PRODUCT_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.SupplierProductDetailActionOK).audits;
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.FETCH_SUPPLIER_PRODUCT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => SupplierProductDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.FETCH_LAST_SUPPLIER_PRODUCT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.SupplierProductDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => SupplierProductDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.COUNT_SUPPLIER_PRODUCT_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.SupplierProductDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.SupplierProductDetailModelActionTypes.INITIALISE_SUPPLIER_PRODUCT_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.SupplierProductDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.SupplierProductDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialSupplierProductDetailCollectionState
					};
				}
