...
import {initialCoTreatingDoctorCollectionState} from './co_treating_doctor.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCoTreatingDoctorAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CoTreatingDoctorModelActionTypes.CREATE_CO_TREATING_DOCTOR_OK:
				const createdModel = (action as modelAction.CoTreatingDoctorActionOK ).stateConfig.targetModel;
				...
				CoTreatingDoctorModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.CREATE_CO_TREATING_DOCTOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.CREATE_ALL_CO_TREATING_DOCTOR_OK:
				const createdModels = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CoTreatingDoctorModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.CREATE_ALL_CO_TREATING_DOCTOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.DELETE_CO_TREATING_DOCTOR_OK:
				const deletedModelId = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.DELETE_ALL_CO_TREATING_DOCTOR_OK:
				const deletedModelIds = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.DELETE_CO_TREATING_DOCTOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.UPDATE_CO_TREATING_DOCTOR_OK:
				const updatedModel = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModel;
				...
				CoTreatingDoctorModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.UPDATE_CO_TREATING_DOCTOR_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CoTreatingDoctorModelActionTypes.UPDATE_ALL_CO_TREATING_DOCTOR_OK:
				const updatedModels = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.UPDATE_ALL_CO_TREATING_DOCTOR_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.FETCH_CO_TREATING_DOCTOR_OK:
				let fetchedModel = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModel;
				...
				CoTreatingDoctorModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.FETCH_ALL_CO_TREATING_DOCTOR_OK:
				const totalCountAfterFetchAll = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CoTreatingDoctorModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.FETCH_CO_TREATING_DOCTOR_AUDIT_OK:
				const audits = (action as modelAction.CoTreatingDoctorActionOK).audits;
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.FETCH_CO_TREATING_DOCTOR_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CoTreatingDoctorActionOK).audits;
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.FETCH_CO_TREATING_DOCTOR_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CoTreatingDoctorModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.FETCH_LAST_CO_TREATING_DOCTOR_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CoTreatingDoctorModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.COUNT_CO_TREATING_DOCTORS_OK:
				const totalCountAfterCount = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CoTreatingDoctorModelActionTypes.INITIALISE_CO_TREATING_DOCTOR_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CoTreatingDoctorActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CoTreatingDoctorActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCoTreatingDoctorCollectionState
					};
				}
