...
import {initialBpjsPcareKunjunganRujukanCollectionState} from './bpjs_pcare_kunjungan_rujukan.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcareKunjunganRujukanAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.CREATE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				const createdModel = (action as modelAction.BpjsPcareKunjunganRujukanActionOK ).stateConfig.targetModel;
				...
				BpjsPcareKunjunganRujukanModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.CREATE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.CREATE_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				const createdModels = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcareKunjunganRujukanModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.CREATE_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.DELETE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				const deletedModelId = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.DELETE_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				const deletedModelIds = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.DELETE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.UPDATE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				const updatedModel = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModel;
				...
				BpjsPcareKunjunganRujukanModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.UPDATE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.UPDATE_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				const updatedModels = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.UPDATE_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				let fetchedModel = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModel;
				...
				BpjsPcareKunjunganRujukanModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_ALL_BPJS_PCARE_KUNJUNGAN_RUJUKAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcareKunjunganRujukanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_BPJS_PCARE_KUNJUNGAN_RUJUKAN_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_BPJS_PCARE_KUNJUNGAN_RUJUKAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_BPJS_PCARE_KUNJUNGAN_RUJUKAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcareKunjunganRujukanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.FETCH_LAST_BPJS_PCARE_KUNJUNGAN_RUJUKAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcareKunjunganRujukanModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.COUNT_BPJS_PCARE_KUNJUNGAN_RUJUKANS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcareKunjunganRujukanModelActionTypes.INITIALISE_BPJS_PCARE_KUNJUNGAN_RUJUKAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcareKunjunganRujukanActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcareKunjunganRujukanCollectionState
					};
				}
