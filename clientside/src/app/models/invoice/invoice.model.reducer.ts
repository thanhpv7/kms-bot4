...
import * as modelAction from './invoice.model.action';
import {ModelState} from '../model.state';
import {initialInvoiceCollectionState} from './invoice.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInvoiceAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InvoiceModelActionTypes.CREATE_INVOICE_OK:
				const createdModel = (action as modelAction.InvoiceActionOK ).stateConfig.targetModel;
				...
				InvoiceModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InvoiceModelActionTypes.CREATE_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceModelActionTypes.CREATE_ALL_INVOICE_OK:
				const createdModels = (action as modelAction.InvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InvoiceModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InvoiceModelActionTypes.CREATE_ALL_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceModelActionTypes.DELETE_INVOICE_OK:
				const deletedModelId = (action as modelAction.InvoiceActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceModelActionTypes.DELETE_ALL_INVOICE_OK:
				const deletedModelIds = (action as modelAction.InvoiceActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceModelActionTypes.DELETE_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceModelActionTypes.UPDATE_INVOICE_OK:
				const updatedModel = (action as modelAction.InvoiceActionOK).stateConfig.targetModel;
				...
				InvoiceModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceModelActionTypes.UPDATE_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InvoiceModelActionTypes.UPDATE_ALL_INVOICE_OK:
				const updatedModels = (action as modelAction.InvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceModelActionTypes.UPDATE_ALL_INVOICE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceModelActionTypes.FETCH_INVOICE_OK:
				let fetchedModel = (action as modelAction.InvoiceActionOK).stateConfig.targetModel;
				...
				InvoiceModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceModelActionTypes.FETCH_ALL_INVOICE_OK:
				const totalCountAfterFetchAll = (action as modelAction.InvoiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InvoiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InvoiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceModelActionTypes.FETCH_INVOICE_AUDIT_OK:
				const audits = (action as modelAction.InvoiceActionOK).audits;
				...
				break;
			case modelAction.InvoiceModelActionTypes.FETCH_INVOICE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InvoiceActionOK).audits;
				...
				break;
			case modelAction.InvoiceModelActionTypes.FETCH_INVOICE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InvoiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceModelActionTypes.FETCH_LAST_INVOICE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InvoiceActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InvoiceModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceModelActionTypes.COUNT_INVOICES_OK:
				const totalCountAfterCount = (action as modelAction.InvoiceActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InvoiceModelActionTypes.INITIALISE_INVOICE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InvoiceActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InvoiceActionOK).stateConfig.queryParams;
