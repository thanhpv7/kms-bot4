...
import {initialTransactionDetailTypeCollectionState} from './transaction_detail_type.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTransactionDetailTypeAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TransactionDetailTypeModelActionTypes.CREATE_TRANSACTION_DETAIL_TYPE_OK:
				const createdModel = (action as modelAction.TransactionDetailTypeActionOK ).stateConfig.targetModel;
				...
				TransactionDetailTypeModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.CREATE_TRANSACTION_DETAIL_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.CREATE_ALL_TRANSACTION_DETAIL_TYPE_OK:
				const createdModels = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TransactionDetailTypeModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.CREATE_ALL_TRANSACTION_DETAIL_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE_OK:
				const deletedModelId = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.DELETE_ALL_TRANSACTION_DETAIL_TYPE_OK:
				const deletedModelIds = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.DELETE_TRANSACTION_DETAIL_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.UPDATE_TRANSACTION_DETAIL_TYPE_OK:
				const updatedModel = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModel;
				...
				TransactionDetailTypeModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.UPDATE_TRANSACTION_DETAIL_TYPE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TransactionDetailTypeModelActionTypes.UPDATE_ALL_TRANSACTION_DETAIL_TYPE_OK:
				const updatedModels = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.UPDATE_ALL_TRANSACTION_DETAIL_TYPE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_OK:
				let fetchedModel = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModel;
				...
				TransactionDetailTypeModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.FETCH_ALL_TRANSACTION_DETAIL_TYPE_OK:
				const totalCountAfterFetchAll = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TransactionDetailTypeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_AUDIT_OK:
				const audits = (action as modelAction.TransactionDetailTypeActionOK).audits;
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TransactionDetailTypeActionOK).audits;
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.FETCH_TRANSACTION_DETAIL_TYPE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TransactionDetailTypeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.FETCH_LAST_TRANSACTION_DETAIL_TYPE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TransactionDetailTypeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.COUNT_TRANSACTION_DETAIL_TYPES_OK:
				const totalCountAfterCount = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TransactionDetailTypeModelActionTypes.INITIALISE_TRANSACTION_DETAIL_TYPE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TransactionDetailTypeActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TransactionDetailTypeActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTransactionDetailTypeCollectionState
					};
				}
