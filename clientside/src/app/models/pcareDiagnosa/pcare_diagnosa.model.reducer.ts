...
import {initialPcareDiagnosaCollectionState} from './pcare_diagnosa.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareDiagnosaAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareDiagnosaModelActionTypes.CREATE_PCARE_DIAGNOSA_OK:
				const createdModel = (action as modelAction.PcareDiagnosaActionOK ).stateConfig.targetModel;
				...
				PcareDiagnosaModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.CREATE_PCARE_DIAGNOSA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDiagnosaModelActionTypes.CREATE_ALL_PCARE_DIAGNOSA_OK:
				const createdModels = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareDiagnosaModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.CREATE_ALL_PCARE_DIAGNOSA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDiagnosaModelActionTypes.DELETE_PCARE_DIAGNOSA_OK:
				const deletedModelId = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.DELETE_ALL_PCARE_DIAGNOSA_OK:
				const deletedModelIds = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.DELETE_PCARE_DIAGNOSA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDiagnosaModelActionTypes.UPDATE_PCARE_DIAGNOSA_OK:
				const updatedModel = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModel;
				...
				PcareDiagnosaModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.UPDATE_PCARE_DIAGNOSA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareDiagnosaModelActionTypes.UPDATE_ALL_PCARE_DIAGNOSA_OK:
				const updatedModels = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.UPDATE_ALL_PCARE_DIAGNOSA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareDiagnosaModelActionTypes.FETCH_PCARE_DIAGNOSA_OK:
				let fetchedModel = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModel;
				...
				PcareDiagnosaModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.FETCH_ALL_PCARE_DIAGNOSA_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareDiagnosaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareDiagnosaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.FETCH_PCARE_DIAGNOSA_AUDIT_OK:
				const audits = (action as modelAction.PcareDiagnosaActionOK).audits;
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.FETCH_PCARE_DIAGNOSA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareDiagnosaActionOK).audits;
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.FETCH_PCARE_DIAGNOSA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareDiagnosaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.FETCH_LAST_PCARE_DIAGNOSA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareDiagnosaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareDiagnosaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.COUNT_PCARE_DIAGNOSAS_OK:
				const totalCountAfterCount = (action as modelAction.PcareDiagnosaActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareDiagnosaModelActionTypes.INITIALISE_PCARE_DIAGNOSA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareDiagnosaActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareDiagnosaActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareDiagnosaCollectionState
					};
				}
