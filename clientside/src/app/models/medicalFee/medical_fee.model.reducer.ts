...
import {initialMedicalFeeCollectionState} from './medical_fee.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMedicalFeeAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MedicalFeeModelActionTypes.CREATE_MEDICAL_FEE_OK:
				const createdModel = (action as modelAction.MedicalFeeActionOK ).stateConfig.targetModel;
				...
				MedicalFeeModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.CREATE_MEDICAL_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalFeeModelActionTypes.CREATE_ALL_MEDICAL_FEE_OK:
				const createdModels = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MedicalFeeModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.CREATE_ALL_MEDICAL_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalFeeModelActionTypes.DELETE_MEDICAL_FEE_OK:
				const deletedModelId = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.DELETE_ALL_MEDICAL_FEE_OK:
				const deletedModelIds = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.DELETE_MEDICAL_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalFeeModelActionTypes.UPDATE_MEDICAL_FEE_OK:
				const updatedModel = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModel;
				...
				MedicalFeeModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.UPDATE_MEDICAL_FEE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MedicalFeeModelActionTypes.UPDATE_ALL_MEDICAL_FEE_OK:
				const updatedModels = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.UPDATE_ALL_MEDICAL_FEE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MedicalFeeModelActionTypes.FETCH_MEDICAL_FEE_OK:
				let fetchedModel = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModel;
				...
				MedicalFeeModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.FETCH_ALL_MEDICAL_FEE_OK:
				const totalCountAfterFetchAll = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MedicalFeeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MedicalFeeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.FETCH_MEDICAL_FEE_AUDIT_OK:
				const audits = (action as modelAction.MedicalFeeActionOK).audits;
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.FETCH_MEDICAL_FEE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MedicalFeeActionOK).audits;
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.FETCH_MEDICAL_FEE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MedicalFeeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.FETCH_LAST_MEDICAL_FEE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MedicalFeeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MedicalFeeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.COUNT_MEDICAL_FEES_OK:
				const totalCountAfterCount = (action as modelAction.MedicalFeeActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MedicalFeeModelActionTypes.INITIALISE_MEDICAL_FEE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MedicalFeeActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MedicalFeeActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMedicalFeeCollectionState
					};
				}
