...
import {initialBpjsPRBCollectionState} from './bpjs_prb.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPRBAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPRBModelActionTypes.CREATE_BPJS_PRB_OK:
				const createdModel = (action as modelAction.BpjsPRBActionOK ).stateConfig.targetModel;
				...
				BpjsPRBModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.CREATE_BPJS_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBModelActionTypes.CREATE_ALL_BPJS_PRB_OK:
				const createdModels = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPRBModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.CREATE_ALL_BPJS_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBModelActionTypes.DELETE_BPJS_PRB_OK:
				const deletedModelId = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.DELETE_ALL_BPJS_PRB_OK:
				const deletedModelIds = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.DELETE_BPJS_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBModelActionTypes.UPDATE_BPJS_PRB_OK:
				const updatedModel = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModel;
				...
				BpjsPRBModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.UPDATE_BPJS_PRB_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPRBModelActionTypes.UPDATE_ALL_BPJS_PRB_OK:
				const updatedModels = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.UPDATE_ALL_BPJS_PRB_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBModelActionTypes.FETCH_BPJS_PRB_OK:
				let fetchedModel = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModel;
				...
				BpjsPRBModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.FETCH_ALL_BPJS_PRB_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.FETCH_BPJS_PRB_AUDIT_OK:
				const audits = (action as modelAction.BpjsPRBActionOK).audits;
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.FETCH_BPJS_PRB_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPRBActionOK).audits;
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.FETCH_BPJS_PRB_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.FETCH_LAST_BPJS_PRB_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPRBActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPRBModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.COUNT_BPJS_PRBS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPRBActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPRBModelActionTypes.INITIALISE_BPJS_PRB_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPRBActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPRBActionOK).stateConfig.queryParams;
