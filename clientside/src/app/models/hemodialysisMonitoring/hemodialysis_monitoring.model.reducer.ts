...
import {initialHemodialysisMonitoringCollectionState} from './hemodialysis_monitoring.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseHemodialysisMonitoringAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.HemodialysisMonitoringModelActionTypes.CREATE_HEMODIALYSIS_MONITORING_OK:
				const createdModel = (action as modelAction.HemodialysisMonitoringActionOK ).stateConfig.targetModel;
				...
				HemodialysisMonitoringModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.CREATE_HEMODIALYSIS_MONITORING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.CREATE_ALL_HEMODIALYSIS_MONITORING_OK:
				const createdModels = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					HemodialysisMonitoringModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.CREATE_ALL_HEMODIALYSIS_MONITORING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.DELETE_HEMODIALYSIS_MONITORING_OK:
				const deletedModelId = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.DELETE_ALL_HEMODIALYSIS_MONITORING_OK:
				const deletedModelIds = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.DELETE_HEMODIALYSIS_MONITORING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.UPDATE_HEMODIALYSIS_MONITORING_OK:
				const updatedModel = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModel;
				...
				HemodialysisMonitoringModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.UPDATE_HEMODIALYSIS_MONITORING_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.HemodialysisMonitoringModelActionTypes.UPDATE_ALL_HEMODIALYSIS_MONITORING_OK:
				const updatedModels = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.UPDATE_ALL_HEMODIALYSIS_MONITORING_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.FETCH_HEMODIALYSIS_MONITORING_OK:
				let fetchedModel = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModel;
				...
				HemodialysisMonitoringModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.FETCH_ALL_HEMODIALYSIS_MONITORING_OK:
				const totalCountAfterFetchAll = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => HemodialysisMonitoringModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.FETCH_HEMODIALYSIS_MONITORING_AUDIT_OK:
				const audits = (action as modelAction.HemodialysisMonitoringActionOK).audits;
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.FETCH_HEMODIALYSIS_MONITORING_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.HemodialysisMonitoringActionOK).audits;
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.FETCH_HEMODIALYSIS_MONITORING_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => HemodialysisMonitoringModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.FETCH_LAST_HEMODIALYSIS_MONITORING_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => HemodialysisMonitoringModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.COUNT_HEMODIALYSIS_MONITORINGS_OK:
				const totalCountAfterCount = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.HemodialysisMonitoringModelActionTypes.INITIALISE_HEMODIALYSIS_MONITORING_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.HemodialysisMonitoringActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialHemodialysisMonitoringCollectionState
					};
				}
