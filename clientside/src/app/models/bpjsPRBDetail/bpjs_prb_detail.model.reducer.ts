...
import {initialBpjsPRBDetailCollectionState} from './bpjs_prb_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPRBDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPRBDetailModelActionTypes.CREATE_BPJS_PRB_DETAIL_OK:
				const createdModel = (action as modelAction.BpjsPRBDetailActionOK ).stateConfig.targetModel;
				...
				BpjsPRBDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.CREATE_BPJS_PRB_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.CREATE_ALL_BPJS_PRB_DETAIL_OK:
				const createdModels = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPRBDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.CREATE_ALL_BPJS_PRB_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.DELETE_BPJS_PRB_DETAIL_OK:
				const deletedModelId = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.DELETE_ALL_BPJS_PRB_DETAIL_OK:
				const deletedModelIds = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.DELETE_BPJS_PRB_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.UPDATE_BPJS_PRB_DETAIL_OK:
				const updatedModel = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModel;
				...
				BpjsPRBDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.UPDATE_BPJS_PRB_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPRBDetailModelActionTypes.UPDATE_ALL_BPJS_PRB_DETAIL_OK:
				const updatedModels = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.UPDATE_ALL_BPJS_PRB_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.FETCH_BPJS_PRB_DETAIL_OK:
				let fetchedModel = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModel;
				...
				BpjsPRBDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.FETCH_ALL_BPJS_PRB_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPRBDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.FETCH_BPJS_PRB_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.BpjsPRBDetailActionOK).audits;
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.FETCH_BPJS_PRB_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPRBDetailActionOK).audits;
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.FETCH_BPJS_PRB_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPRBDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.FETCH_LAST_BPJS_PRB_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPRBDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.COUNT_BPJS_PRB_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPRBDetailModelActionTypes.INITIALISE_BPJS_PRB_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPRBDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPRBDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPRBDetailCollectionState
					};
				}
