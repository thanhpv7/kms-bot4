...
import {initialBpjsSEPSubmissionCollectionState} from './bpjs_sep_submission.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsSEPSubmissionAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsSEPSubmissionModelActionTypes.CREATE_BPJS_SEP_SUBMISSION_OK:
				const createdModel = (action as modelAction.BpjsSEPSubmissionActionOK ).stateConfig.targetModel;
				...
				BpjsSEPSubmissionModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.CREATE_BPJS_SEP_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.CREATE_ALL_BPJS_SEP_SUBMISSION_OK:
				const createdModels = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsSEPSubmissionModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.CREATE_ALL_BPJS_SEP_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.DELETE_BPJS_SEP_SUBMISSION_OK:
				const deletedModelId = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.DELETE_ALL_BPJS_SEP_SUBMISSION_OK:
				const deletedModelIds = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.DELETE_BPJS_SEP_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.UPDATE_BPJS_SEP_SUBMISSION_OK:
				const updatedModel = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModel;
				...
				BpjsSEPSubmissionModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.UPDATE_BPJS_SEP_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsSEPSubmissionModelActionTypes.UPDATE_ALL_BPJS_SEP_SUBMISSION_OK:
				const updatedModels = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.UPDATE_ALL_BPJS_SEP_SUBMISSION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_BPJS_SEP_SUBMISSION_OK:
				let fetchedModel = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModel;
				...
				BpjsSEPSubmissionModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_ALL_BPJS_SEP_SUBMISSION_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsSEPSubmissionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_BPJS_SEP_SUBMISSION_AUDIT_OK:
				const audits = (action as modelAction.BpjsSEPSubmissionActionOK).audits;
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_BPJS_SEP_SUBMISSION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsSEPSubmissionActionOK).audits;
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_BPJS_SEP_SUBMISSION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsSEPSubmissionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.FETCH_LAST_BPJS_SEP_SUBMISSION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsSEPSubmissionModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.COUNT_BPJS_SEP_SUBMISSIONS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsSEPSubmissionModelActionTypes.INITIALISE_BPJS_SEP_SUBMISSION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsSEPSubmissionActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsSEPSubmissionCollectionState
					};
				}
