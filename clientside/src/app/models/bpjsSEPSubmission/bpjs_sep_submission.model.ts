 ...
 */
export class BpjsSEPSubmissionModel extends AbstractModel {
	/**
	...
	readonly className = 'BpjsSEPSubmissionModel';
	...
	get valueToDisplay(): string {
		...
		return BpjsSEPSubmissionModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = BpjsSEPSubmissionModel.modelPropGroups;
	...
	static getProps(): ModelProperty[] {
		return super.getProps().concat([
			...
			{
				name: 'submissionType',
				// % protected region % [Set displayName for Submission Type here] off begin
				displayName: 'Submission Type',
				// % protected region % [Set displayName for Submission Type here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for Submission Type here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for Submission Type here] end
				// % protected region % [Set isSensitive for Submission Type here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for Submission Type here] end
				// % protected region % [Set readonly for Submission Type here] off begin
				readOnly: false,
				// % protected region % [Set readonly for Submission Type here] end
				validators: [
					// % protected region % [Add other validators for Submission Type here] off begin
					// % protected region % [Add other validators for Submission Type here] end
				],
				// % protected region % [Add any additional model attribute properties for Submission Type here] off begin
				// % protected region % [Add any additional model attribute properties for Submission Type here] end
			},
			{
				name: 'sepDate',
	 ...
	 */
	static deepParse(data: string | { [K in keyof BpjsSEPSubmissionModel]?: BpjsSEPSubmissionModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new BpjsSEPSubmissionModel(data);
		}
	 ...
	 * @example
	 *
	 * `let bpjsSEPSubmissionModel = new BpjsSEPSubmissionModel(data);`
	 *
	 * @param data The input data to be initialised as the BpjsSEPSubmissionModel,
	 *    it is expected as a JSON string or as a nullable BpjsSEPSubmissionModel.
	 */
	constructor(data?: string | Partial<BpjsSEPSubmissionModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<BpjsSEPSubmissionModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		BpjsSEPSubmissionModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): BpjsSEPSubmissionModel {
		...
		if (updates.submissionType) {
			newModelJson.submissionType = updates.submissionType;
		}
		...
		return new BpjsSEPSubmissionModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof BpjsSEPSubmissionModel)) {
			return {};
		}
