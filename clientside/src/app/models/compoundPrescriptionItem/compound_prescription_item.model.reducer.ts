...
import {initialCompoundPrescriptionItemCollectionState} from './compound_prescription_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseCompoundPrescriptionItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.CompoundPrescriptionItemModelActionTypes.CREATE_COMPOUND_PRESCRIPTION_ITEM_OK:
				const createdModel = (action as modelAction.CompoundPrescriptionItemActionOK ).stateConfig.targetModel;
				...
				CompoundPrescriptionItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.CREATE_COMPOUND_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.CREATE_ALL_COMPOUND_PRESCRIPTION_ITEM_OK:
				const createdModels = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					CompoundPrescriptionItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.CREATE_ALL_COMPOUND_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_ITEM_OK:
				const deletedModelId = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.DELETE_ALL_COMPOUND_PRESCRIPTION_ITEM_OK:
				const deletedModelIds = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.DELETE_COMPOUND_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.UPDATE_COMPOUND_PRESCRIPTION_ITEM_OK:
				const updatedModel = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModel;
				...
				CompoundPrescriptionItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.UPDATE_COMPOUND_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.CompoundPrescriptionItemModelActionTypes.UPDATE_ALL_COMPOUND_PRESCRIPTION_ITEM_OK:
				const updatedModels = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.UPDATE_ALL_COMPOUND_PRESCRIPTION_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_ITEM_OK:
				let fetchedModel = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModel;
				...
				CompoundPrescriptionItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_ALL_COMPOUND_PRESCRIPTION_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => CompoundPrescriptionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_ITEM_AUDIT_OK:
				const audits = (action as modelAction.CompoundPrescriptionItemActionOK).audits;
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.CompoundPrescriptionItemActionOK).audits;
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_COMPOUND_PRESCRIPTION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => CompoundPrescriptionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.FETCH_LAST_COMPOUND_PRESCRIPTION_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => CompoundPrescriptionItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.COUNT_COMPOUND_PRESCRIPTION_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.CompoundPrescriptionItemModelActionTypes.INITIALISE_COMPOUND_PRESCRIPTION_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.CompoundPrescriptionItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialCompoundPrescriptionItemCollectionState
					};
				}
