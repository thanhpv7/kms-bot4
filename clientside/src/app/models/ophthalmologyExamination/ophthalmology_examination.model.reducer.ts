...
import {initialOphthalmologyExaminationCollectionState} from './ophthalmology_examination.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseOphthalmologyExaminationAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.OphthalmologyExaminationModelActionTypes.CREATE_OPHTHALMOLOGY_EXAMINATION_OK:
				const createdModel = (action as modelAction.OphthalmologyExaminationActionOK ).stateConfig.targetModel;
				...
				OphthalmologyExaminationModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.CREATE_OPHTHALMOLOGY_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.CREATE_ALL_OPHTHALMOLOGY_EXAMINATION_OK:
				const createdModels = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					OphthalmologyExaminationModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.CREATE_ALL_OPHTHALMOLOGY_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.DELETE_OPHTHALMOLOGY_EXAMINATION_OK:
				const deletedModelId = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.DELETE_ALL_OPHTHALMOLOGY_EXAMINATION_OK:
				const deletedModelIds = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.DELETE_OPHTHALMOLOGY_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.UPDATE_OPHTHALMOLOGY_EXAMINATION_OK:
				const updatedModel = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModel;
				...
				OphthalmologyExaminationModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.UPDATE_OPHTHALMOLOGY_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.OphthalmologyExaminationModelActionTypes.UPDATE_ALL_OPHTHALMOLOGY_EXAMINATION_OK:
				const updatedModels = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.UPDATE_ALL_OPHTHALMOLOGY_EXAMINATION_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.FETCH_OPHTHALMOLOGY_EXAMINATION_OK:
				let fetchedModel = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModel;
				...
				OphthalmologyExaminationModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.FETCH_ALL_OPHTHALMOLOGY_EXAMINATION_OK:
				const totalCountAfterFetchAll = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => OphthalmologyExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.FETCH_OPHTHALMOLOGY_EXAMINATION_AUDIT_OK:
				const audits = (action as modelAction.OphthalmologyExaminationActionOK).audits;
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.FETCH_OPHTHALMOLOGY_EXAMINATION_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.OphthalmologyExaminationActionOK).audits;
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.FETCH_OPHTHALMOLOGY_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => OphthalmologyExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.FETCH_LAST_OPHTHALMOLOGY_EXAMINATION_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => OphthalmologyExaminationModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.COUNT_OPHTHALMOLOGY_EXAMINATIONS_OK:
				const totalCountAfterCount = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.OphthalmologyExaminationModelActionTypes.INITIALISE_OPHTHALMOLOGY_EXAMINATION_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.OphthalmologyExaminationActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialOphthalmologyExaminationCollectionState
					};
				}
