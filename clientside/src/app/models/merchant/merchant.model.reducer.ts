...
import * as modelAction from './merchant.model.action';
import {ModelState} from '../model.state';
import {initialMerchantCollectionState} from './merchant.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseMerchantAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.MerchantModelActionTypes.CREATE_MERCHANT_OK:
				const createdModel = (action as modelAction.MerchantActionOK ).stateConfig.targetModel;
				...
				MerchantModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.MerchantModelActionTypes.CREATE_MERCHANT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantModelActionTypes.CREATE_ALL_MERCHANT_OK:
				const createdModels = (action as modelAction.MerchantActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					MerchantModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.MerchantModelActionTypes.CREATE_ALL_MERCHANT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantModelActionTypes.DELETE_MERCHANT_OK:
				const deletedModelId = (action as modelAction.MerchantActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MerchantModelActionTypes.DELETE_ALL_MERCHANT_OK:
				const deletedModelIds = (action as modelAction.MerchantActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MerchantModelActionTypes.DELETE_MERCHANT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantModelActionTypes.UPDATE_MERCHANT_OK:
				const updatedModel = (action as modelAction.MerchantActionOK).stateConfig.targetModel;
				...
				MerchantModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantModelActionTypes.UPDATE_MERCHANT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.MerchantModelActionTypes.UPDATE_ALL_MERCHANT_OK:
				const updatedModels = (action as modelAction.MerchantActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.MerchantModelActionTypes.UPDATE_ALL_MERCHANT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.MerchantModelActionTypes.FETCH_MERCHANT_OK:
				let fetchedModel = (action as modelAction.MerchantActionOK).stateConfig.targetModel;
				...
				MerchantModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantModelActionTypes.FETCH_ALL_MERCHANT_OK:
				const totalCountAfterFetchAll = (action as modelAction.MerchantActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.MerchantActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.MerchantActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => MerchantModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantModelActionTypes.FETCH_MERCHANT_AUDIT_OK:
				const audits = (action as modelAction.MerchantActionOK).audits;
				...
				break;
			case modelAction.MerchantModelActionTypes.FETCH_MERCHANT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.MerchantActionOK).audits;
				...
				break;
			case modelAction.MerchantModelActionTypes.FETCH_MERCHANT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.MerchantActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.MerchantActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.MerchantActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => MerchantModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantModelActionTypes.FETCH_LAST_MERCHANT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.MerchantActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.MerchantActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.MerchantActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => MerchantModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.MerchantModelActionTypes.COUNT_MERCHANTS_OK:
				const totalCountAfterCount = (action as modelAction.MerchantActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.MerchantModelActionTypes.INITIALISE_MERCHANT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.MerchantActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.MerchantActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialMerchantCollectionState
					};
				}
