...
import {initialInvoiceItemCollectionState} from './invoice_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInvoiceItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InvoiceItemModelActionTypes.CREATE_INVOICE_ITEM_OK:
				const createdModel = (action as modelAction.InvoiceItemActionOK ).stateConfig.targetModel;
				...
				InvoiceItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.CREATE_INVOICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemModelActionTypes.CREATE_ALL_INVOICE_ITEM_OK:
				const createdModels = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InvoiceItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.CREATE_ALL_INVOICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemModelActionTypes.DELETE_INVOICE_ITEM_OK:
				const deletedModelId = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.DELETE_ALL_INVOICE_ITEM_OK:
				const deletedModelIds = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.DELETE_INVOICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemModelActionTypes.UPDATE_INVOICE_ITEM_OK:
				const updatedModel = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModel;
				...
				InvoiceItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.UPDATE_INVOICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InvoiceItemModelActionTypes.UPDATE_ALL_INVOICE_ITEM_OK:
				const updatedModels = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.UPDATE_ALL_INVOICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoiceItemModelActionTypes.FETCH_INVOICE_ITEM_OK:
				let fetchedModel = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModel;
				...
				InvoiceItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.FETCH_ALL_INVOICE_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InvoiceItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InvoiceItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.FETCH_INVOICE_ITEM_AUDIT_OK:
				const audits = (action as modelAction.InvoiceItemActionOK).audits;
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.FETCH_INVOICE_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InvoiceItemActionOK).audits;
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.FETCH_INVOICE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InvoiceItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.FETCH_LAST_INVOICE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InvoiceItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InvoiceItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.COUNT_INVOICE_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.InvoiceItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InvoiceItemModelActionTypes.INITIALISE_INVOICE_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InvoiceItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InvoiceItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInvoiceItemCollectionState
					};
				}
