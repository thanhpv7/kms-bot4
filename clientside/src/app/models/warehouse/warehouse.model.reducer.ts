...
import {initialWarehouseCollectionState} from './warehouse.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseWarehouseAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.WarehouseModelActionTypes.CREATE_WAREHOUSE_OK:
				const createdModel = (action as modelAction.WarehouseActionOK ).stateConfig.targetModel;
				...
				WarehouseModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.WarehouseModelActionTypes.CREATE_WAREHOUSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseModelActionTypes.CREATE_ALL_WAREHOUSE_OK:
				const createdModels = (action as modelAction.WarehouseActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					WarehouseModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.WarehouseModelActionTypes.CREATE_ALL_WAREHOUSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseModelActionTypes.DELETE_WAREHOUSE_OK:
				const deletedModelId = (action as modelAction.WarehouseActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseModelActionTypes.DELETE_ALL_WAREHOUSE_OK:
				const deletedModelIds = (action as modelAction.WarehouseActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseModelActionTypes.DELETE_WAREHOUSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseModelActionTypes.UPDATE_WAREHOUSE_OK:
				const updatedModel = (action as modelAction.WarehouseActionOK).stateConfig.targetModel;
				...
				WarehouseModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseModelActionTypes.UPDATE_WAREHOUSE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.WarehouseModelActionTypes.UPDATE_ALL_WAREHOUSE_OK:
				const updatedModels = (action as modelAction.WarehouseActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.WarehouseModelActionTypes.UPDATE_ALL_WAREHOUSE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.WarehouseModelActionTypes.FETCH_WAREHOUSE_OK:
				let fetchedModel = (action as modelAction.WarehouseActionOK).stateConfig.targetModel;
				...
				WarehouseModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseModelActionTypes.FETCH_ALL_WAREHOUSE_OK:
				const totalCountAfterFetchAll = (action as modelAction.WarehouseActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.WarehouseActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.WarehouseActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => WarehouseModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseModelActionTypes.FETCH_WAREHOUSE_AUDIT_OK:
				const audits = (action as modelAction.WarehouseActionOK).audits;
				...
				break;
			case modelAction.WarehouseModelActionTypes.FETCH_WAREHOUSE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.WarehouseActionOK).audits;
				...
				break;
			case modelAction.WarehouseModelActionTypes.FETCH_WAREHOUSE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => WarehouseModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseModelActionTypes.FETCH_LAST_WAREHOUSE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.WarehouseActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => WarehouseModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.WarehouseModelActionTypes.COUNT_WAREHOUSES_OK:
				const totalCountAfterCount = (action as modelAction.WarehouseActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.WarehouseModelActionTypes.INITIALISE_WAREHOUSE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.WarehouseActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.WarehouseActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialWarehouseCollectionState
					};
				}
