...
import {initialBpjsSpecialReferralCollectionState} from './bpjs_special_referral.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsSpecialReferralAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsSpecialReferralModelActionTypes.CREATE_BPJS_SPECIAL_REFERRAL_OK:
				const createdModel = (action as modelAction.BpjsSpecialReferralActionOK ).stateConfig.targetModel;
				...
				BpjsSpecialReferralModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.CREATE_BPJS_SPECIAL_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.CREATE_ALL_BPJS_SPECIAL_REFERRAL_OK:
				const createdModels = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsSpecialReferralModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.CREATE_ALL_BPJS_SPECIAL_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.DELETE_BPJS_SPECIAL_REFERRAL_OK:
				const deletedModelId = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.DELETE_ALL_BPJS_SPECIAL_REFERRAL_OK:
				const deletedModelIds = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.DELETE_BPJS_SPECIAL_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.UPDATE_BPJS_SPECIAL_REFERRAL_OK:
				const updatedModel = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModel;
				...
				BpjsSpecialReferralModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.UPDATE_BPJS_SPECIAL_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsSpecialReferralModelActionTypes.UPDATE_ALL_BPJS_SPECIAL_REFERRAL_OK:
				const updatedModels = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.UPDATE_ALL_BPJS_SPECIAL_REFERRAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.FETCH_BPJS_SPECIAL_REFERRAL_OK:
				let fetchedModel = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModel;
				...
				BpjsSpecialReferralModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.FETCH_ALL_BPJS_SPECIAL_REFERRAL_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsSpecialReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.FETCH_BPJS_SPECIAL_REFERRAL_AUDIT_OK:
				const audits = (action as modelAction.BpjsSpecialReferralActionOK).audits;
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.FETCH_BPJS_SPECIAL_REFERRAL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsSpecialReferralActionOK).audits;
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.FETCH_BPJS_SPECIAL_REFERRAL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsSpecialReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.FETCH_LAST_BPJS_SPECIAL_REFERRAL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsSpecialReferralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.COUNT_BPJS_SPECIAL_REFERRALS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsSpecialReferralModelActionTypes.INITIALISE_BPJS_SPECIAL_REFERRAL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsSpecialReferralActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsSpecialReferralCollectionState
					};
				}
