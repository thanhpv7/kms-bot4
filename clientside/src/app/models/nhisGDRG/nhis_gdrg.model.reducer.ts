...
import {initialNhisGDRGCollectionState} from './nhis_gdrg.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisGDRGAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisGDRGModelActionTypes.CREATE_NHIS_GDRG_OK:
				const createdModel = (action as modelAction.NhisGDRGActionOK ).stateConfig.targetModel;
				...
				NhisGDRGModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.CREATE_NHIS_GDRG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisGDRGModelActionTypes.CREATE_ALL_NHIS_GDRG_OK:
				const createdModels = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisGDRGModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.CREATE_ALL_NHIS_GDRG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisGDRGModelActionTypes.DELETE_NHIS_GDRG_OK:
				const deletedModelId = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.DELETE_ALL_NHIS_GDRG_OK:
				const deletedModelIds = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.DELETE_NHIS_GDRG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisGDRGModelActionTypes.UPDATE_NHIS_GDRG_OK:
				const updatedModel = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModel;
				...
				NhisGDRGModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.UPDATE_NHIS_GDRG_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisGDRGModelActionTypes.UPDATE_ALL_NHIS_GDRG_OK:
				const updatedModels = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.UPDATE_ALL_NHIS_GDRG_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisGDRGModelActionTypes.FETCH_NHIS_GDRG_OK:
				let fetchedModel = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModel;
				...
				NhisGDRGModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.FETCH_ALL_NHIS_GDRG_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisGDRGActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisGDRGModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.FETCH_NHIS_GDRG_AUDIT_OK:
				const audits = (action as modelAction.NhisGDRGActionOK).audits;
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.FETCH_NHIS_GDRG_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisGDRGActionOK).audits;
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.FETCH_NHIS_GDRG_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisGDRGModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.FETCH_LAST_NHIS_GDRG_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisGDRGActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisGDRGModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.COUNT_NHIS_GDRGS_OK:
				const totalCountAfterCount = (action as modelAction.NhisGDRGActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisGDRGModelActionTypes.INITIALISE_NHIS_GDRG_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisGDRGActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisGDRGActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisGDRGCollectionState
					};
				}
