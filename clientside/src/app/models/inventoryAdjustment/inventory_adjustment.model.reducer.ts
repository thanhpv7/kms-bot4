...
import {initialInventoryAdjustmentCollectionState} from './inventory_adjustment.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInventoryAdjustmentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InventoryAdjustmentModelActionTypes.CREATE_INVENTORY_ADJUSTMENT_OK:
				const createdModel = (action as modelAction.InventoryAdjustmentActionOK ).stateConfig.targetModel;
				...
				InventoryAdjustmentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.CREATE_INVENTORY_ADJUSTMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.CREATE_ALL_INVENTORY_ADJUSTMENT_OK:
				const createdModels = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InventoryAdjustmentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.CREATE_ALL_INVENTORY_ADJUSTMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.DELETE_INVENTORY_ADJUSTMENT_OK:
				const deletedModelId = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.DELETE_ALL_INVENTORY_ADJUSTMENT_OK:
				const deletedModelIds = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.DELETE_INVENTORY_ADJUSTMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.UPDATE_INVENTORY_ADJUSTMENT_OK:
				const updatedModel = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModel;
				...
				InventoryAdjustmentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.UPDATE_INVENTORY_ADJUSTMENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InventoryAdjustmentModelActionTypes.UPDATE_ALL_INVENTORY_ADJUSTMENT_OK:
				const updatedModels = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.UPDATE_ALL_INVENTORY_ADJUSTMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_OK:
				let fetchedModel = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModel;
				...
				InventoryAdjustmentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.FETCH_ALL_INVENTORY_ADJUSTMENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InventoryAdjustmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_AUDIT_OK:
				const audits = (action as modelAction.InventoryAdjustmentActionOK).audits;
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InventoryAdjustmentActionOK).audits;
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InventoryAdjustmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.FETCH_LAST_INVENTORY_ADJUSTMENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InventoryAdjustmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.COUNT_INVENTORY_ADJUSTMENTS_OK:
				const totalCountAfterCount = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InventoryAdjustmentModelActionTypes.INITIALISE_INVENTORY_ADJUSTMENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InventoryAdjustmentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InventoryAdjustmentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInventoryAdjustmentCollectionState
					};
				}
