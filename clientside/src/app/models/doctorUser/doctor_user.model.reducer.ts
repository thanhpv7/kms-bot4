...
import {initialDoctorUserCollectionState} from './doctor_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseDoctorUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.DoctorUserModelActionTypes.CREATE_DOCTOR_USER_OK:
				const createdModel = (action as modelAction.DoctorUserActionOK ).stateConfig.targetModel;
				...
				DoctorUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.DoctorUserModelActionTypes.CREATE_DOCTOR_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorUserModelActionTypes.CREATE_ALL_DOCTOR_USER_OK:
				const createdModels = (action as modelAction.DoctorUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					DoctorUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.DoctorUserModelActionTypes.CREATE_ALL_DOCTOR_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorUserModelActionTypes.DELETE_DOCTOR_USER_OK:
				const deletedModelId = (action as modelAction.DoctorUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DoctorUserModelActionTypes.DELETE_ALL_DOCTOR_USER_OK:
				const deletedModelIds = (action as modelAction.DoctorUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DoctorUserModelActionTypes.DELETE_DOCTOR_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorUserModelActionTypes.UPDATE_DOCTOR_USER_OK:
				const updatedModel = (action as modelAction.DoctorUserActionOK).stateConfig.targetModel;
				...
				DoctorUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorUserModelActionTypes.UPDATE_DOCTOR_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.DoctorUserModelActionTypes.UPDATE_ALL_DOCTOR_USER_OK:
				const updatedModels = (action as modelAction.DoctorUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.DoctorUserModelActionTypes.UPDATE_ALL_DOCTOR_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.DoctorUserModelActionTypes.FETCH_DOCTOR_USER_OK:
				let fetchedModel = (action as modelAction.DoctorUserActionOK).stateConfig.targetModel;
				...
				DoctorUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorUserModelActionTypes.FETCH_ALL_DOCTOR_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.DoctorUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.DoctorUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.DoctorUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => DoctorUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorUserModelActionTypes.FETCH_DOCTOR_USER_AUDIT_OK:
				const audits = (action as modelAction.DoctorUserActionOK).audits;
				...
				break;
			case modelAction.DoctorUserModelActionTypes.FETCH_DOCTOR_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.DoctorUserActionOK).audits;
				...
				break;
			case modelAction.DoctorUserModelActionTypes.FETCH_DOCTOR_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => DoctorUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorUserModelActionTypes.FETCH_LAST_DOCTOR_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.DoctorUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => DoctorUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.DoctorUserModelActionTypes.COUNT_DOCTOR_USERS_OK:
				const totalCountAfterCount = (action as modelAction.DoctorUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.DoctorUserModelActionTypes.INITIALISE_DOCTOR_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.DoctorUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.DoctorUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialDoctorUserCollectionState
					};
				}
