...
import {initialPrivilegeCollectionState} from './privilege.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePrivilegeAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PrivilegeModelActionTypes.CREATE_PRIVILEGE_OK:
				const createdModel = (action as modelAction.PrivilegeActionOK ).stateConfig.targetModel;
				...
				PrivilegeModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PrivilegeModelActionTypes.CREATE_PRIVILEGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrivilegeModelActionTypes.CREATE_ALL_PRIVILEGE_OK:
				const createdModels = (action as modelAction.PrivilegeActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PrivilegeModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PrivilegeModelActionTypes.CREATE_ALL_PRIVILEGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrivilegeModelActionTypes.DELETE_PRIVILEGE_OK:
				const deletedModelId = (action as modelAction.PrivilegeActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrivilegeModelActionTypes.DELETE_ALL_PRIVILEGE_OK:
				const deletedModelIds = (action as modelAction.PrivilegeActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrivilegeModelActionTypes.DELETE_PRIVILEGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrivilegeModelActionTypes.UPDATE_PRIVILEGE_OK:
				const updatedModel = (action as modelAction.PrivilegeActionOK).stateConfig.targetModel;
				...
				PrivilegeModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrivilegeModelActionTypes.UPDATE_PRIVILEGE_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PrivilegeModelActionTypes.UPDATE_ALL_PRIVILEGE_OK:
				const updatedModels = (action as modelAction.PrivilegeActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PrivilegeModelActionTypes.UPDATE_ALL_PRIVILEGE_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PrivilegeModelActionTypes.FETCH_PRIVILEGE_OK:
				let fetchedModel = (action as modelAction.PrivilegeActionOK).stateConfig.targetModel;
				...
				PrivilegeModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrivilegeModelActionTypes.FETCH_ALL_PRIVILEGE_OK:
				const totalCountAfterFetchAll = (action as modelAction.PrivilegeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PrivilegeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PrivilegeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PrivilegeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrivilegeModelActionTypes.FETCH_PRIVILEGE_AUDIT_OK:
				const audits = (action as modelAction.PrivilegeActionOK).audits;
				...
				break;
			case modelAction.PrivilegeModelActionTypes.FETCH_PRIVILEGE_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PrivilegeActionOK).audits;
				...
				break;
			case modelAction.PrivilegeModelActionTypes.FETCH_PRIVILEGE_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PrivilegeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrivilegeModelActionTypes.FETCH_LAST_PRIVILEGE_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PrivilegeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PrivilegeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PrivilegeModelActionTypes.COUNT_PRIVILEGES_OK:
				const totalCountAfterCount = (action as modelAction.PrivilegeActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PrivilegeModelActionTypes.INITIALISE_PRIVILEGE_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PrivilegeActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PrivilegeActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPrivilegeCollectionState
					};
				}
