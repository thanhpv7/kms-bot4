...
import {initialTariffSchemaCollectionState} from './tariff_schema.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseTariffSchemaAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.TariffSchemaModelActionTypes.CREATE_TARIFF_SCHEMA_OK:
				const createdModel = (action as modelAction.TariffSchemaActionOK ).stateConfig.targetModel;
				...
				TariffSchemaModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.CREATE_TARIFF_SCHEMA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffSchemaModelActionTypes.CREATE_ALL_TARIFF_SCHEMA_OK:
				const createdModels = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					TariffSchemaModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.CREATE_ALL_TARIFF_SCHEMA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffSchemaModelActionTypes.DELETE_TARIFF_SCHEMA_OK:
				const deletedModelId = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.DELETE_ALL_TARIFF_SCHEMA_OK:
				const deletedModelIds = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.DELETE_TARIFF_SCHEMA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffSchemaModelActionTypes.UPDATE_TARIFF_SCHEMA_OK:
				const updatedModel = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModel;
				...
				TariffSchemaModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.UPDATE_TARIFF_SCHEMA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.TariffSchemaModelActionTypes.UPDATE_ALL_TARIFF_SCHEMA_OK:
				const updatedModels = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.UPDATE_ALL_TARIFF_SCHEMA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.TariffSchemaModelActionTypes.FETCH_TARIFF_SCHEMA_OK:
				let fetchedModel = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModel;
				...
				TariffSchemaModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.FETCH_ALL_TARIFF_SCHEMA_OK:
				const totalCountAfterFetchAll = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.TariffSchemaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => TariffSchemaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.FETCH_TARIFF_SCHEMA_AUDIT_OK:
				const audits = (action as modelAction.TariffSchemaActionOK).audits;
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.FETCH_TARIFF_SCHEMA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.TariffSchemaActionOK).audits;
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.FETCH_TARIFF_SCHEMA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => TariffSchemaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.FETCH_LAST_TARIFF_SCHEMA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.TariffSchemaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => TariffSchemaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.COUNT_TARIFF_SCHEMAS_OK:
				const totalCountAfterCount = (action as modelAction.TariffSchemaActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.TariffSchemaModelActionTypes.INITIALISE_TARIFF_SCHEMA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.TariffSchemaActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.TariffSchemaActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialTariffSchemaCollectionState
					};
				}
