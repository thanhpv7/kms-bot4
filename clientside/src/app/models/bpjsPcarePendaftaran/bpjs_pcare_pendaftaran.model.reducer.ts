...
import {initialBpjsPcarePendaftaranCollectionState} from './bpjs_pcare_pendaftaran.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcarePendaftaranAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcarePendaftaranModelActionTypes.CREATE_BPJS_PCARE_PENDAFTARAN_OK:
				const createdModel = (action as modelAction.BpjsPcarePendaftaranActionOK ).stateConfig.targetModel;
				...
				BpjsPcarePendaftaranModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.CREATE_BPJS_PCARE_PENDAFTARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.CREATE_ALL_BPJS_PCARE_PENDAFTARAN_OK:
				const createdModels = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcarePendaftaranModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.CREATE_ALL_BPJS_PCARE_PENDAFTARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.DELETE_BPJS_PCARE_PENDAFTARAN_OK:
				const deletedModelId = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.DELETE_ALL_BPJS_PCARE_PENDAFTARAN_OK:
				const deletedModelIds = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.DELETE_BPJS_PCARE_PENDAFTARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.UPDATE_BPJS_PCARE_PENDAFTARAN_OK:
				const updatedModel = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModel;
				...
				BpjsPcarePendaftaranModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.UPDATE_BPJS_PCARE_PENDAFTARAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcarePendaftaranModelActionTypes.UPDATE_ALL_BPJS_PCARE_PENDAFTARAN_OK:
				const updatedModels = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.UPDATE_ALL_BPJS_PCARE_PENDAFTARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_BPJS_PCARE_PENDAFTARAN_OK:
				let fetchedModel = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModel;
				...
				BpjsPcarePendaftaranModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_ALL_BPJS_PCARE_PENDAFTARAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcarePendaftaranModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_BPJS_PCARE_PENDAFTARAN_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcarePendaftaranActionOK).audits;
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_BPJS_PCARE_PENDAFTARAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcarePendaftaranActionOK).audits;
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_BPJS_PCARE_PENDAFTARAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcarePendaftaranModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.FETCH_LAST_BPJS_PCARE_PENDAFTARAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcarePendaftaranModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.COUNT_BPJS_PCARE_PENDAFTARANS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcarePendaftaranModelActionTypes.INITIALISE_BPJS_PCARE_PENDAFTARAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcarePendaftaranActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcarePendaftaranCollectionState
					};
				}
