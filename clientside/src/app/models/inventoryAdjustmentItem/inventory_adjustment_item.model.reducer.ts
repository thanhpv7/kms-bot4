...
import {initialInventoryAdjustmentItemCollectionState} from './inventory_adjustment_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInventoryAdjustmentItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InventoryAdjustmentItemModelActionTypes.CREATE_INVENTORY_ADJUSTMENT_ITEM_OK:
				const createdModel = (action as modelAction.InventoryAdjustmentItemActionOK ).stateConfig.targetModel;
				...
				InventoryAdjustmentItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.CREATE_INVENTORY_ADJUSTMENT_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.CREATE_ALL_INVENTORY_ADJUSTMENT_ITEM_OK:
				const createdModels = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InventoryAdjustmentItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.CREATE_ALL_INVENTORY_ADJUSTMENT_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.DELETE_INVENTORY_ADJUSTMENT_ITEM_OK:
				const deletedModelId = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.DELETE_ALL_INVENTORY_ADJUSTMENT_ITEM_OK:
				const deletedModelIds = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.DELETE_INVENTORY_ADJUSTMENT_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.UPDATE_INVENTORY_ADJUSTMENT_ITEM_OK:
				const updatedModel = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModel;
				...
				InventoryAdjustmentItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.UPDATE_INVENTORY_ADJUSTMENT_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InventoryAdjustmentItemModelActionTypes.UPDATE_ALL_INVENTORY_ADJUSTMENT_ITEM_OK:
				const updatedModels = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.UPDATE_ALL_INVENTORY_ADJUSTMENT_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_ITEM_OK:
				let fetchedModel = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModel;
				...
				InventoryAdjustmentItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_ALL_INVENTORY_ADJUSTMENT_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InventoryAdjustmentItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_ITEM_AUDIT_OK:
				const audits = (action as modelAction.InventoryAdjustmentItemActionOK).audits;
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InventoryAdjustmentItemActionOK).audits;
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_INVENTORY_ADJUSTMENT_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InventoryAdjustmentItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.FETCH_LAST_INVENTORY_ADJUSTMENT_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InventoryAdjustmentItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.COUNT_INVENTORY_ADJUSTMENT_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InventoryAdjustmentItemModelActionTypes.INITIALISE_INVENTORY_ADJUSTMENT_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InventoryAdjustmentItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInventoryAdjustmentItemCollectionState
					};
				}
