...
import {initialServiceItemAssignmentCollectionState} from './service_item_assignment.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseServiceItemAssignmentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ServiceItemAssignmentModelActionTypes.CREATE_SERVICE_ITEM_ASSIGNMENT_OK:
				const createdModel = (action as modelAction.ServiceItemAssignmentActionOK ).stateConfig.targetModel;
				...
				ServiceItemAssignmentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.CREATE_SERVICE_ITEM_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.CREATE_ALL_SERVICE_ITEM_ASSIGNMENT_OK:
				const createdModels = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ServiceItemAssignmentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.CREATE_ALL_SERVICE_ITEM_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.DELETE_SERVICE_ITEM_ASSIGNMENT_OK:
				const deletedModelId = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.DELETE_ALL_SERVICE_ITEM_ASSIGNMENT_OK:
				const deletedModelIds = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.DELETE_SERVICE_ITEM_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.UPDATE_SERVICE_ITEM_ASSIGNMENT_OK:
				const updatedModel = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModel;
				...
				ServiceItemAssignmentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.UPDATE_SERVICE_ITEM_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ServiceItemAssignmentModelActionTypes.UPDATE_ALL_SERVICE_ITEM_ASSIGNMENT_OK:
				const updatedModels = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.UPDATE_ALL_SERVICE_ITEM_ASSIGNMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.FETCH_SERVICE_ITEM_ASSIGNMENT_OK:
				let fetchedModel = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModel;
				...
				ServiceItemAssignmentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.FETCH_ALL_SERVICE_ITEM_ASSIGNMENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ServiceItemAssignmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.FETCH_SERVICE_ITEM_ASSIGNMENT_AUDIT_OK:
				const audits = (action as modelAction.ServiceItemAssignmentActionOK).audits;
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.FETCH_SERVICE_ITEM_ASSIGNMENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ServiceItemAssignmentActionOK).audits;
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.FETCH_SERVICE_ITEM_ASSIGNMENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ServiceItemAssignmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.FETCH_LAST_SERVICE_ITEM_ASSIGNMENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ServiceItemAssignmentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.COUNT_SERVICE_ITEM_ASSIGNMENTS_OK:
				const totalCountAfterCount = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ServiceItemAssignmentModelActionTypes.INITIALISE_SERVICE_ITEM_ASSIGNMENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ServiceItemAssignmentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialServiceItemAssignmentCollectionState
					};
				}
