...
import {initialBpjsGeneralCollectionState} from './bpjs_general.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsGeneralAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsGeneralModelActionTypes.CREATE_BPJS_GENERAL_OK:
				const createdModel = (action as modelAction.BpjsGeneralActionOK ).stateConfig.targetModel;
				...
				BpjsGeneralModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.CREATE_BPJS_GENERAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsGeneralModelActionTypes.CREATE_ALL_BPJS_GENERAL_OK:
				const createdModels = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsGeneralModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.CREATE_ALL_BPJS_GENERAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsGeneralModelActionTypes.DELETE_BPJS_GENERAL_OK:
				const deletedModelId = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.DELETE_ALL_BPJS_GENERAL_OK:
				const deletedModelIds = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.DELETE_BPJS_GENERAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsGeneralModelActionTypes.UPDATE_BPJS_GENERAL_OK:
				const updatedModel = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModel;
				...
				BpjsGeneralModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.UPDATE_BPJS_GENERAL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsGeneralModelActionTypes.UPDATE_ALL_BPJS_GENERAL_OK:
				const updatedModels = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.UPDATE_ALL_BPJS_GENERAL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsGeneralModelActionTypes.FETCH_BPJS_GENERAL_OK:
				let fetchedModel = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModel;
				...
				BpjsGeneralModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.FETCH_ALL_BPJS_GENERAL_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsGeneralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsGeneralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.FETCH_BPJS_GENERAL_AUDIT_OK:
				const audits = (action as modelAction.BpjsGeneralActionOK).audits;
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.FETCH_BPJS_GENERAL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsGeneralActionOK).audits;
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.FETCH_BPJS_GENERAL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsGeneralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.FETCH_LAST_BPJS_GENERAL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsGeneralActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsGeneralModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.COUNT_BPJS_GENERALS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsGeneralActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsGeneralModelActionTypes.INITIALISE_BPJS_GENERAL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsGeneralActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsGeneralActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsGeneralCollectionState
					};
				}
