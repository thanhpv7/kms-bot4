...
import {initialServiceItemCollectionState} from './service_item.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseServiceItemAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ServiceItemModelActionTypes.CREATE_SERVICE_ITEM_OK:
				const createdModel = (action as modelAction.ServiceItemActionOK ).stateConfig.targetModel;
				...
				ServiceItemModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ServiceItemModelActionTypes.CREATE_SERVICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemModelActionTypes.CREATE_ALL_SERVICE_ITEM_OK:
				const createdModels = (action as modelAction.ServiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ServiceItemModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ServiceItemModelActionTypes.CREATE_ALL_SERVICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemModelActionTypes.DELETE_SERVICE_ITEM_OK:
				const deletedModelId = (action as modelAction.ServiceItemActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceItemModelActionTypes.DELETE_ALL_SERVICE_ITEM_OK:
				const deletedModelIds = (action as modelAction.ServiceItemActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceItemModelActionTypes.DELETE_SERVICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemModelActionTypes.UPDATE_SERVICE_ITEM_OK:
				const updatedModel = (action as modelAction.ServiceItemActionOK).stateConfig.targetModel;
				...
				ServiceItemModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemModelActionTypes.UPDATE_SERVICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ServiceItemModelActionTypes.UPDATE_ALL_SERVICE_ITEM_OK:
				const updatedModels = (action as modelAction.ServiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceItemModelActionTypes.UPDATE_ALL_SERVICE_ITEM_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceItemModelActionTypes.FETCH_SERVICE_ITEM_OK:
				let fetchedModel = (action as modelAction.ServiceItemActionOK).stateConfig.targetModel;
				...
				ServiceItemModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemModelActionTypes.FETCH_ALL_SERVICE_ITEM_OK:
				const totalCountAfterFetchAll = (action as modelAction.ServiceItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ServiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ServiceItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ServiceItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemModelActionTypes.FETCH_SERVICE_ITEM_AUDIT_OK:
				const audits = (action as modelAction.ServiceItemActionOK).audits;
				...
				break;
			case modelAction.ServiceItemModelActionTypes.FETCH_SERVICE_ITEM_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ServiceItemActionOK).audits;
				...
				break;
			case modelAction.ServiceItemModelActionTypes.FETCH_SERVICE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ServiceItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemModelActionTypes.FETCH_LAST_SERVICE_ITEM_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ServiceItemActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ServiceItemModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceItemModelActionTypes.COUNT_SERVICE_ITEMS_OK:
				const totalCountAfterCount = (action as modelAction.ServiceItemActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ServiceItemModelActionTypes.INITIALISE_SERVICE_ITEM_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ServiceItemActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ServiceItemActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialServiceItemCollectionState
					};
				}
