 ...
 */
export class PatientSupportingDocumentModel extends AbstractModel {
	/**
	...
	readonly className = 'PatientSupportingDocumentModel';
	...
	get valueToDisplay(): string {
		...
		return PatientSupportingDocumentModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = PatientSupportingDocumentModel.modelPropGroups;
	...
	static getProps(): ModelProperty[] {
		return super.getProps().concat([
			...
			{
				name: 'fileName',
				// % protected region % [Set displayName for File Name here] off begin
				displayName: 'File Name',
				// % protected region % [Set displayName for File Name here] end
				type: ModelPropertyType.STRING,
				// % protected region % [Set display element type for File Name here] off begin
				elementType: ElementType.INPUT,
				// % protected region % [Set display element type for File Name here] end
				// % protected region % [Set isSensitive for File Name here] off begin
				isSensitive: false,
				// % protected region % [Set isSensitive for File Name here] end
				// % protected region % [Set readonly for File Name here] off begin
				readOnly: false,
				// % protected region % [Set readonly for File Name here] end
				validators: [
					// % protected region % [Add other validators for File Name here] off begin
					// % protected region % [Add other validators for File Name here] end
				],
				// % protected region % [Add any additional model attribute properties for File Name here] off begin
				// % protected region % [Add any additional model attribute properties for File Name here] end
			},
			{
				name: 'description',
	 ...
	 */
	static deepParse(data: string | { [K in keyof PatientSupportingDocumentModel]?: PatientSupportingDocumentModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new PatientSupportingDocumentModel(data);
		}
	 ...
	 * @example
	 *
	 * `let patientSupportingDocumentModel = new PatientSupportingDocumentModel(data);`
	 *
	 * @param data The input data to be initialised as the PatientSupportingDocumentModel,
	 *    it is expected as a JSON string or as a nullable PatientSupportingDocumentModel.
	 */
	constructor(data?: string | Partial<PatientSupportingDocumentModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<PatientSupportingDocumentModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		PatientSupportingDocumentModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): PatientSupportingDocumentModel {
		...
		if (updates.fileName) {
			newModelJson.fileName = updates.fileName;
		}
		...
		return new PatientSupportingDocumentModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof PatientSupportingDocumentModel)) {
			return {};
		}
		...
		for (const key of _.keys(this)) {
			...
			} else if (['supportingDocument'].includes(key)) {;
