...
import {initialPatientSupportingDocumentCollectionState} from './patient_supporting_document.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientSupportingDocumentAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientSupportingDocumentModelActionTypes.CREATE_PATIENT_SUPPORTING_DOCUMENT_OK:
				const createdModel = (action as modelAction.PatientSupportingDocumentActionOK ).stateConfig.targetModel;
				...
				PatientSupportingDocumentModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.CREATE_PATIENT_SUPPORTING_DOCUMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.CREATE_ALL_PATIENT_SUPPORTING_DOCUMENT_OK:
				const createdModels = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientSupportingDocumentModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.CREATE_ALL_PATIENT_SUPPORTING_DOCUMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.DELETE_PATIENT_SUPPORTING_DOCUMENT_OK:
				const deletedModelId = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.DELETE_ALL_PATIENT_SUPPORTING_DOCUMENT_OK:
				const deletedModelIds = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.DELETE_PATIENT_SUPPORTING_DOCUMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.UPDATE_PATIENT_SUPPORTING_DOCUMENT_OK:
				const updatedModel = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModel;
				...
				PatientSupportingDocumentModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.UPDATE_PATIENT_SUPPORTING_DOCUMENT_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientSupportingDocumentModelActionTypes.UPDATE_ALL_PATIENT_SUPPORTING_DOCUMENT_OK:
				const updatedModels = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.UPDATE_ALL_PATIENT_SUPPORTING_DOCUMENT_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.FETCH_PATIENT_SUPPORTING_DOCUMENT_OK:
				let fetchedModel = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModel;
				...
				PatientSupportingDocumentModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.FETCH_ALL_PATIENT_SUPPORTING_DOCUMENT_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientSupportingDocumentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.FETCH_PATIENT_SUPPORTING_DOCUMENT_AUDIT_OK:
				const audits = (action as modelAction.PatientSupportingDocumentActionOK).audits;
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.FETCH_PATIENT_SUPPORTING_DOCUMENT_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientSupportingDocumentActionOK).audits;
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.FETCH_PATIENT_SUPPORTING_DOCUMENT_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientSupportingDocumentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.FETCH_LAST_PATIENT_SUPPORTING_DOCUMENT_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientSupportingDocumentModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.COUNT_PATIENT_SUPPORTING_DOCUMENTS_OK:
				const totalCountAfterCount = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientSupportingDocumentModelActionTypes.INITIALISE_PATIENT_SUPPORTING_DOCUMENT_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientSupportingDocumentActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientSupportingDocumentCollectionState
					};
				}
