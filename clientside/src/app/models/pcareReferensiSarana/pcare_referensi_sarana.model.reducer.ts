...
import {initialPcareReferensiSaranaCollectionState} from './pcare_referensi_sarana.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePcareReferensiSaranaAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PcareReferensiSaranaModelActionTypes.CREATE_PCARE_REFERENSI_SARANA_OK:
				const createdModel = (action as modelAction.PcareReferensiSaranaActionOK ).stateConfig.targetModel;
				...
				PcareReferensiSaranaModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.CREATE_PCARE_REFERENSI_SARANA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.CREATE_ALL_PCARE_REFERENSI_SARANA_OK:
				const createdModels = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PcareReferensiSaranaModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.CREATE_ALL_PCARE_REFERENSI_SARANA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.DELETE_PCARE_REFERENSI_SARANA_OK:
				const deletedModelId = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.DELETE_ALL_PCARE_REFERENSI_SARANA_OK:
				const deletedModelIds = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.DELETE_PCARE_REFERENSI_SARANA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.UPDATE_PCARE_REFERENSI_SARANA_OK:
				const updatedModel = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModel;
				...
				PcareReferensiSaranaModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.UPDATE_PCARE_REFERENSI_SARANA_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PcareReferensiSaranaModelActionTypes.UPDATE_ALL_PCARE_REFERENSI_SARANA_OK:
				const updatedModels = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.UPDATE_ALL_PCARE_REFERENSI_SARANA_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.FETCH_PCARE_REFERENSI_SARANA_OK:
				let fetchedModel = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModel;
				...
				PcareReferensiSaranaModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.FETCH_ALL_PCARE_REFERENSI_SARANA_OK:
				const totalCountAfterFetchAll = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PcareReferensiSaranaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.FETCH_PCARE_REFERENSI_SARANA_AUDIT_OK:
				const audits = (action as modelAction.PcareReferensiSaranaActionOK).audits;
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.FETCH_PCARE_REFERENSI_SARANA_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PcareReferensiSaranaActionOK).audits;
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.FETCH_PCARE_REFERENSI_SARANA_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PcareReferensiSaranaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.FETCH_LAST_PCARE_REFERENSI_SARANA_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PcareReferensiSaranaModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.COUNT_PCARE_REFERENSI_SARANAS_OK:
				const totalCountAfterCount = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PcareReferensiSaranaModelActionTypes.INITIALISE_PCARE_REFERENSI_SARANA_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PcareReferensiSaranaActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPcareReferensiSaranaCollectionState
					};
				}
