...
import {initialObstetricAndGynecologyHistoryCollectionState} from './obstetric_and_gynecology_history.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseObstetricAndGynecologyHistoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.CREATE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				const createdModel = (action as modelAction.ObstetricAndGynecologyHistoryActionOK ).stateConfig.targetModel;
				...
				ObstetricAndGynecologyHistoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.CREATE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.CREATE_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				const createdModels = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ObstetricAndGynecologyHistoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.CREATE_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.DELETE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				const deletedModelId = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.DELETE_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				const deletedModelIds = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.DELETE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.UPDATE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				const updatedModel = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModel;
				...
				ObstetricAndGynecologyHistoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.UPDATE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.UPDATE_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				const updatedModels = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.UPDATE_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				let fetchedModel = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModel;
				...
				ObstetricAndGynecologyHistoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_ALL_OBSTETRIC_AND_GYNECOLOGY_HISTORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ObstetricAndGynecologyHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_OBSTETRIC_AND_GYNECOLOGY_HISTORY_AUDIT_OK:
				const audits = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).audits;
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_OBSTETRIC_AND_GYNECOLOGY_HISTORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).audits;
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_OBSTETRIC_AND_GYNECOLOGY_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ObstetricAndGynecologyHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.FETCH_LAST_OBSTETRIC_AND_GYNECOLOGY_HISTORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ObstetricAndGynecologyHistoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.COUNT_OBSTETRIC_AND_GYNECOLOGY_HISTORYS_OK:
				const totalCountAfterCount = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ObstetricAndGynecologyHistoryModelActionTypes.INITIALISE_OBSTETRIC_AND_GYNECOLOGY_HISTORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ObstetricAndGynecologyHistoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialObstetricAndGynecologyHistoryCollectionState
					};
				}
