...
import {initialNhisServiceOutcomeCollectionState} from './nhis_service_outcome.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNhisServiceOutcomeAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NhisServiceOutcomeModelActionTypes.CREATE_NHIS_SERVICE_OUTCOME_OK:
				const createdModel = (action as modelAction.NhisServiceOutcomeActionOK ).stateConfig.targetModel;
				...
				NhisServiceOutcomeModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.CREATE_NHIS_SERVICE_OUTCOME_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.CREATE_ALL_NHIS_SERVICE_OUTCOME_OK:
				const createdModels = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NhisServiceOutcomeModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.CREATE_ALL_NHIS_SERVICE_OUTCOME_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.DELETE_NHIS_SERVICE_OUTCOME_OK:
				const deletedModelId = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.DELETE_ALL_NHIS_SERVICE_OUTCOME_OK:
				const deletedModelIds = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.DELETE_NHIS_SERVICE_OUTCOME_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.UPDATE_NHIS_SERVICE_OUTCOME_OK:
				const updatedModel = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModel;
				...
				NhisServiceOutcomeModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.UPDATE_NHIS_SERVICE_OUTCOME_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NhisServiceOutcomeModelActionTypes.UPDATE_ALL_NHIS_SERVICE_OUTCOME_OK:
				const updatedModels = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.UPDATE_ALL_NHIS_SERVICE_OUTCOME_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.FETCH_NHIS_SERVICE_OUTCOME_OK:
				let fetchedModel = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModel;
				...
				NhisServiceOutcomeModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.FETCH_ALL_NHIS_SERVICE_OUTCOME_OK:
				const totalCountAfterFetchAll = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NhisServiceOutcomeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.FETCH_NHIS_SERVICE_OUTCOME_AUDIT_OK:
				const audits = (action as modelAction.NhisServiceOutcomeActionOK).audits;
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.FETCH_NHIS_SERVICE_OUTCOME_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NhisServiceOutcomeActionOK).audits;
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.FETCH_NHIS_SERVICE_OUTCOME_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NhisServiceOutcomeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.FETCH_LAST_NHIS_SERVICE_OUTCOME_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NhisServiceOutcomeModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.COUNT_NHIS_SERVICE_OUTCOMES_OK:
				const totalCountAfterCount = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NhisServiceOutcomeModelActionTypes.INITIALISE_NHIS_SERVICE_OUTCOME_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NhisServiceOutcomeActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNhisServiceOutcomeCollectionState
					};
				}
