...
import {initialVaccinationOrderDetailCollectionState} from './vaccination_order_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseVaccinationOrderDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.VaccinationOrderDetailModelActionTypes.CREATE_VACCINATION_ORDER_DETAIL_OK:
				const createdModel = (action as modelAction.VaccinationOrderDetailActionOK ).stateConfig.targetModel;
				...
				VaccinationOrderDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.CREATE_VACCINATION_ORDER_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.CREATE_ALL_VACCINATION_ORDER_DETAIL_OK:
				const createdModels = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					VaccinationOrderDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.CREATE_ALL_VACCINATION_ORDER_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.DELETE_VACCINATION_ORDER_DETAIL_OK:
				const deletedModelId = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.DELETE_ALL_VACCINATION_ORDER_DETAIL_OK:
				const deletedModelIds = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.DELETE_VACCINATION_ORDER_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.UPDATE_VACCINATION_ORDER_DETAIL_OK:
				const updatedModel = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModel;
				...
				VaccinationOrderDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.UPDATE_VACCINATION_ORDER_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.VaccinationOrderDetailModelActionTypes.UPDATE_ALL_VACCINATION_ORDER_DETAIL_OK:
				const updatedModels = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.UPDATE_ALL_VACCINATION_ORDER_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.FETCH_VACCINATION_ORDER_DETAIL_OK:
				let fetchedModel = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModel;
				...
				VaccinationOrderDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.FETCH_ALL_VACCINATION_ORDER_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => VaccinationOrderDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.FETCH_VACCINATION_ORDER_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.VaccinationOrderDetailActionOK).audits;
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.FETCH_VACCINATION_ORDER_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.VaccinationOrderDetailActionOK).audits;
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.FETCH_VACCINATION_ORDER_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => VaccinationOrderDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.FETCH_LAST_VACCINATION_ORDER_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => VaccinationOrderDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.COUNT_VACCINATION_ORDER_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.VaccinationOrderDetailModelActionTypes.INITIALISE_VACCINATION_ORDER_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.VaccinationOrderDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialVaccinationOrderDetailCollectionState
					};
				}
