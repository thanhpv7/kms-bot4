...
import {initialNumberingSetupCollectionState} from './numbering_setup.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseNumberingSetupAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.NumberingSetupModelActionTypes.CREATE_NUMBERING_SETUP_OK:
				const createdModel = (action as modelAction.NumberingSetupActionOK ).stateConfig.targetModel;
				...
				NumberingSetupModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.CREATE_NUMBERING_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NumberingSetupModelActionTypes.CREATE_ALL_NUMBERING_SETUP_OK:
				const createdModels = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					NumberingSetupModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.CREATE_ALL_NUMBERING_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NumberingSetupModelActionTypes.DELETE_NUMBERING_SETUP_OK:
				const deletedModelId = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.DELETE_ALL_NUMBERING_SETUP_OK:
				const deletedModelIds = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.DELETE_NUMBERING_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NumberingSetupModelActionTypes.UPDATE_NUMBERING_SETUP_OK:
				const updatedModel = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModel;
				...
				NumberingSetupModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.UPDATE_NUMBERING_SETUP_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.NumberingSetupModelActionTypes.UPDATE_ALL_NUMBERING_SETUP_OK:
				const updatedModels = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.UPDATE_ALL_NUMBERING_SETUP_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.NumberingSetupModelActionTypes.FETCH_NUMBERING_SETUP_OK:
				let fetchedModel = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModel;
				...
				NumberingSetupModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.FETCH_ALL_NUMBERING_SETUP_OK:
				const totalCountAfterFetchAll = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.NumberingSetupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => NumberingSetupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.FETCH_NUMBERING_SETUP_AUDIT_OK:
				const audits = (action as modelAction.NumberingSetupActionOK).audits;
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.FETCH_NUMBERING_SETUP_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.NumberingSetupActionOK).audits;
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.FETCH_NUMBERING_SETUP_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => NumberingSetupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.FETCH_LAST_NUMBERING_SETUP_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.NumberingSetupActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => NumberingSetupModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.COUNT_NUMBERING_SETUPS_OK:
				const totalCountAfterCount = (action as modelAction.NumberingSetupActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.NumberingSetupModelActionTypes.INITIALISE_NUMBERING_SETUP_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.NumberingSetupActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.NumberingSetupActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialNumberingSetupCollectionState
					};
				}
