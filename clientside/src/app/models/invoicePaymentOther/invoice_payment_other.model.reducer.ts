...
import {initialInvoicePaymentOtherCollectionState} from './invoice_payment_other.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseInvoicePaymentOtherAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.InvoicePaymentOtherModelActionTypes.CREATE_INVOICE_PAYMENT_OTHER_OK:
				const createdModel = (action as modelAction.InvoicePaymentOtherActionOK ).stateConfig.targetModel;
				...
				InvoicePaymentOtherModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.CREATE_INVOICE_PAYMENT_OTHER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.CREATE_ALL_INVOICE_PAYMENT_OTHER_OK:
				const createdModels = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					InvoicePaymentOtherModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.CREATE_ALL_INVOICE_PAYMENT_OTHER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.DELETE_INVOICE_PAYMENT_OTHER_OK:
				const deletedModelId = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.DELETE_ALL_INVOICE_PAYMENT_OTHER_OK:
				const deletedModelIds = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.DELETE_INVOICE_PAYMENT_OTHER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.UPDATE_INVOICE_PAYMENT_OTHER_OK:
				const updatedModel = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModel;
				...
				InvoicePaymentOtherModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.UPDATE_INVOICE_PAYMENT_OTHER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.InvoicePaymentOtherModelActionTypes.UPDATE_ALL_INVOICE_PAYMENT_OTHER_OK:
				const updatedModels = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.UPDATE_ALL_INVOICE_PAYMENT_OTHER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.FETCH_INVOICE_PAYMENT_OTHER_OK:
				let fetchedModel = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModel;
				...
				InvoicePaymentOtherModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.FETCH_ALL_INVOICE_PAYMENT_OTHER_OK:
				const totalCountAfterFetchAll = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => InvoicePaymentOtherModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.FETCH_INVOICE_PAYMENT_OTHER_AUDIT_OK:
				const audits = (action as modelAction.InvoicePaymentOtherActionOK).audits;
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.FETCH_INVOICE_PAYMENT_OTHER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.InvoicePaymentOtherActionOK).audits;
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.FETCH_INVOICE_PAYMENT_OTHER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => InvoicePaymentOtherModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.FETCH_LAST_INVOICE_PAYMENT_OTHER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => InvoicePaymentOtherModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.COUNT_INVOICE_PAYMENT_OTHERS_OK:
				const totalCountAfterCount = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.InvoicePaymentOtherModelActionTypes.INITIALISE_INVOICE_PAYMENT_OTHER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.InvoicePaymentOtherActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialInvoicePaymentOtherCollectionState
					};
				}
