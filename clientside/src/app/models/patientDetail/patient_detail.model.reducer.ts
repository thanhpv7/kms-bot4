...
import {initialPatientDetailCollectionState} from './patient_detail.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BasePatientDetailAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.PatientDetailModelActionTypes.CREATE_PATIENT_DETAIL_OK:
				const createdModel = (action as modelAction.PatientDetailActionOK ).stateConfig.targetModel;
				...
				PatientDetailModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.PatientDetailModelActionTypes.CREATE_PATIENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailModelActionTypes.CREATE_ALL_PATIENT_DETAIL_OK:
				const createdModels = (action as modelAction.PatientDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					PatientDetailModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.PatientDetailModelActionTypes.CREATE_ALL_PATIENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailModelActionTypes.DELETE_PATIENT_DETAIL_OK:
				const deletedModelId = (action as modelAction.PatientDetailActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientDetailModelActionTypes.DELETE_ALL_PATIENT_DETAIL_OK:
				const deletedModelIds = (action as modelAction.PatientDetailActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientDetailModelActionTypes.DELETE_PATIENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailModelActionTypes.UPDATE_PATIENT_DETAIL_OK:
				const updatedModel = (action as modelAction.PatientDetailActionOK).stateConfig.targetModel;
				...
				PatientDetailModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailModelActionTypes.UPDATE_PATIENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.PatientDetailModelActionTypes.UPDATE_ALL_PATIENT_DETAIL_OK:
				const updatedModels = (action as modelAction.PatientDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.PatientDetailModelActionTypes.UPDATE_ALL_PATIENT_DETAIL_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.PatientDetailModelActionTypes.FETCH_PATIENT_DETAIL_OK:
				let fetchedModel = (action as modelAction.PatientDetailActionOK).stateConfig.targetModel;
				...
				PatientDetailModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailModelActionTypes.FETCH_ALL_PATIENT_DETAIL_OK:
				const totalCountAfterFetchAll = (action as modelAction.PatientDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.PatientDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.PatientDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => PatientDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailModelActionTypes.FETCH_PATIENT_DETAIL_AUDIT_OK:
				const audits = (action as modelAction.PatientDetailActionOK).audits;
				...
				break;
			case modelAction.PatientDetailModelActionTypes.FETCH_PATIENT_DETAIL_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.PatientDetailActionOK).audits;
				...
				break;
			case modelAction.PatientDetailModelActionTypes.FETCH_PATIENT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => PatientDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailModelActionTypes.FETCH_LAST_PATIENT_DETAIL_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.PatientDetailActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => PatientDetailModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.PatientDetailModelActionTypes.COUNT_PATIENT_DETAILS_OK:
				const totalCountAfterCount = (action as modelAction.PatientDetailActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.PatientDetailModelActionTypes.INITIALISE_PATIENT_DETAIL_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.PatientDetailActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.PatientDetailActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialPatientDetailCollectionState
					};
				}
