...
import {initialBpjsPcareKesadaranCollectionState} from './bpjs_pcare_kesadaran.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseBpjsPcareKesadaranAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.BpjsPcareKesadaranModelActionTypes.CREATE_BPJS_PCARE_KESADARAN_OK:
				const createdModel = (action as modelAction.BpjsPcareKesadaranActionOK ).stateConfig.targetModel;
				...
				BpjsPcareKesadaranModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.CREATE_BPJS_PCARE_KESADARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.CREATE_ALL_BPJS_PCARE_KESADARAN_OK:
				const createdModels = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					BpjsPcareKesadaranModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.CREATE_ALL_BPJS_PCARE_KESADARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.DELETE_BPJS_PCARE_KESADARAN_OK:
				const deletedModelId = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.DELETE_ALL_BPJS_PCARE_KESADARAN_OK:
				const deletedModelIds = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.DELETE_BPJS_PCARE_KESADARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.UPDATE_BPJS_PCARE_KESADARAN_OK:
				const updatedModel = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModel;
				...
				BpjsPcareKesadaranModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.UPDATE_BPJS_PCARE_KESADARAN_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.BpjsPcareKesadaranModelActionTypes.UPDATE_ALL_BPJS_PCARE_KESADARAN_OK:
				const updatedModels = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.UPDATE_ALL_BPJS_PCARE_KESADARAN_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_BPJS_PCARE_KESADARAN_OK:
				let fetchedModel = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModel;
				...
				BpjsPcareKesadaranModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_ALL_BPJS_PCARE_KESADARAN_OK:
				const totalCountAfterFetchAll = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => BpjsPcareKesadaranModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_BPJS_PCARE_KESADARAN_AUDIT_OK:
				const audits = (action as modelAction.BpjsPcareKesadaranActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_BPJS_PCARE_KESADARAN_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.BpjsPcareKesadaranActionOK).audits;
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_BPJS_PCARE_KESADARAN_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => BpjsPcareKesadaranModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.FETCH_LAST_BPJS_PCARE_KESADARAN_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => BpjsPcareKesadaranModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.COUNT_BPJS_PCARE_KESADARANS_OK:
				const totalCountAfterCount = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.BpjsPcareKesadaranModelActionTypes.INITIALISE_BPJS_PCARE_KESADARAN_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.BpjsPcareKesadaranActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialBpjsPcareKesadaranCollectionState
					};
				}
