...
import {initialServiceFacilityCategoryCollectionState} from './service_facility_category.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseServiceFacilityCategoryAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.ServiceFacilityCategoryModelActionTypes.CREATE_SERVICE_FACILITY_CATEGORY_OK:
				const createdModel = (action as modelAction.ServiceFacilityCategoryActionOK ).stateConfig.targetModel;
				...
				ServiceFacilityCategoryModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.CREATE_SERVICE_FACILITY_CATEGORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.CREATE_ALL_SERVICE_FACILITY_CATEGORY_OK:
				const createdModels = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					ServiceFacilityCategoryModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.CREATE_ALL_SERVICE_FACILITY_CATEGORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.DELETE_SERVICE_FACILITY_CATEGORY_OK:
				const deletedModelId = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.DELETE_ALL_SERVICE_FACILITY_CATEGORY_OK:
				const deletedModelIds = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.DELETE_SERVICE_FACILITY_CATEGORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.UPDATE_SERVICE_FACILITY_CATEGORY_OK:
				const updatedModel = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModel;
				...
				ServiceFacilityCategoryModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.UPDATE_SERVICE_FACILITY_CATEGORY_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.ServiceFacilityCategoryModelActionTypes.UPDATE_ALL_SERVICE_FACILITY_CATEGORY_OK:
				const updatedModels = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.UPDATE_ALL_SERVICE_FACILITY_CATEGORY_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_SERVICE_FACILITY_CATEGORY_OK:
				let fetchedModel = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModel;
				...
				ServiceFacilityCategoryModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_ALL_SERVICE_FACILITY_CATEGORY_OK:
				const totalCountAfterFetchAll = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => ServiceFacilityCategoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_SERVICE_FACILITY_CATEGORY_AUDIT_OK:
				const audits = (action as modelAction.ServiceFacilityCategoryActionOK).audits;
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_SERVICE_FACILITY_CATEGORY_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.ServiceFacilityCategoryActionOK).audits;
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_SERVICE_FACILITY_CATEGORY_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => ServiceFacilityCategoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.FETCH_LAST_SERVICE_FACILITY_CATEGORY_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => ServiceFacilityCategoryModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.COUNT_SERVICE_FACILITY_CATEGORYS_OK:
				const totalCountAfterCount = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.ServiceFacilityCategoryModelActionTypes.INITIALISE_SERVICE_FACILITY_CATEGORY_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.ServiceFacilityCategoryActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialServiceFacilityCategoryCollectionState
					};
				}
