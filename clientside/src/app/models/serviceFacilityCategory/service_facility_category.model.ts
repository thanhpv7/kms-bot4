 ...
 */
export class ServiceFacilityCategoryModel extends AbstractModel {
	/**
	...
	readonly className = 'ServiceFacilityCategoryModel';
	...
	get valueToDisplay(): string {
		...
		return ServiceFacilityCategoryModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = ServiceFacilityCategoryModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof ServiceFacilityCategoryModel]?: ServiceFacilityCategoryModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new ServiceFacilityCategoryModel(data);
		}
	 ...
	 * @example
	 *
	 * `let serviceFacilityCategoryModel = new ServiceFacilityCategoryModel(data);`
	 *
	 * @param data The input data to be initialised as the ServiceFacilityCategoryModel,
	 *    it is expected as a JSON string or as a nullable ServiceFacilityCategoryModel.
	 */
	constructor(data?: string | Partial<ServiceFacilityCategoryModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<ServiceFacilityCategoryModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		ServiceFacilityCategoryModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): ServiceFacilityCategoryModel {
		...
		return new ServiceFacilityCategoryModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof ServiceFacilityCategoryModel)) {
			return {};
		}
