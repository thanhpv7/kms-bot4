...
import {initialAdministrationUserCollectionState} from './administration_user.model.state';
...
export function modelReducer(currentState: ModelState, action: Action): ModelState {
	...
	const className = (action as modelAction.BaseAdministrationUserAction).className;
	...
	return produce(currentState, draft => {
		...
		switch (action.type) {
			case modelAction.AdministrationUserModelActionTypes.CREATE_ADMINISTRATION_USER_OK:
				const createdModel = (action as modelAction.AdministrationUserActionOK ).stateConfig.targetModel;
				...
				AdministrationUserModel.deepParse(createdModel).forEach(model => {
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.CREATE_ADMINISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministrationUserModelActionTypes.CREATE_ALL_ADMINISTRATION_USER_OK:
				const createdModels = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterCreateAll = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				...
				createdModels.forEach((m) => {
					AdministrationUserModel.deepParse(m).forEach(model => {
						draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
						(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
					});
				});
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.CREATE_ALL_ADMINISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministrationUserModelActionTypes.DELETE_ADMINISTRATION_USER_OK:
				const deletedModelId = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModelId;
				const collectionIdAfterDelete = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.DELETE_ALL_ADMINISTRATION_USER_OK:
				const deletedModelIds = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModelIds;
				const collectionIdAfterDeleteAll = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.DELETE_ADMINISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministrationUserModelActionTypes.UPDATE_ADMINISTRATION_USER_OK:
				const updatedModel = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModel;
				...
				AdministrationUserModel.deepParse(updatedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.UPDATE_ADMINISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;

			case modelAction.AdministrationUserModelActionTypes.UPDATE_ALL_ADMINISTRATION_USER_OK:
				const updatedModels = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterUpdateAll = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.UPDATE_ALL_ADMINISTRATION_USER_FAIL:
				// TODO: Add handling of failure
				break;
			case modelAction.AdministrationUserModelActionTypes.FETCH_ADMINISTRATION_USER_OK:
				let fetchedModel = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModel;
				...
				AdministrationUserModel.deepParse(fetchedModel).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.FETCH_ALL_ADMINISTRATION_USER_OK:
				const totalCountAfterFetchAll = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchAll = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchAll = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchAll = (action as modelAction.AdministrationUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchAll.map(model => AdministrationUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.FETCH_ADMINISTRATION_USER_AUDIT_OK:
				const audits = (action as modelAction.AdministrationUserActionOK).audits;
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.FETCH_ADMINISTRATION_USER_AUDITS_BY_ENTITY_ID_OK:
				const auditsByEntityId = (action as modelAction.AdministrationUserActionOK).audits;
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.FETCH_ADMINISTRATION_USER_WITH_QUERY_OK:
				const totalCountAfterFetchWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchWithQuery.map(model => AdministrationUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.FETCH_LAST_ADMINISTRATION_USER_WITH_QUERY_OK:
				const totalCountAfterFetchLastWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionCount;
				const fetchedModelsAfterFetchLastWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.targetModels;
				const collectionIdAfterFetchLastWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				const queryParamsAfterFetchLastWithQuery = (action as modelAction.AdministrationUserActionOK).stateConfig.queryParams;
				...
				_.flatten(fetchedModelsAfterFetchLastWithQuery.map(model => AdministrationUserModel.deepParse(model))).forEach(model => {
					draft[model.className].data[model.id] = _.assignWith(draft[model.className].data[model.id], model,
					(objValue, srcValue) => _.isUndefined(srcValue) ? objValue : srcValue);
				});
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.COUNT_ADMINISTRATION_USERS_OK:
				const totalCountAfterCount = (action as modelAction.AdministrationUserActionOK).stateConfig.totalCount;
				...
				break;
			case modelAction.AdministrationUserModelActionTypes.INITIALISE_ADMINISTRATION_USER_COLLECTION_STATE:
				const collectionIdToInitialise =  (action as modelAction.AdministrationUserActionOK).stateConfig.collectionId;
				const queryParamsToInitialise = (action as modelAction.AdministrationUserActionOK).stateConfig.queryParams;
				...
				if (!draft[className].collectionState[collectionIdToInitialise]) {
					draft[className].collectionState[collectionIdToInitialise] = {
						...initialAdministrationUserCollectionState
					};
				}
