 ...
 */
export class NhisClaimInvestigationDetailModel extends AbstractModel {
	/**
	...
	readonly className = 'NhisClaimInvestigationDetailModel';
	...
	get valueToDisplay(): string {
		...
		return NhisClaimInvestigationDetailModel.displayAttributes.map((attr) => this[attr]).join(' ');
	...
	modelPropGroups: { [s: string]: Group } = NhisClaimInvestigationDetailModel.modelPropGroups;
	 ...
	 */
	static deepParse(data: string | { [K in keyof NhisClaimInvestigationDetailModel]?: NhisClaimInvestigationDetailModel[K] }, currentModel?): AbstractModel[] {
		if (currentModel == null) {
			currentModel = new NhisClaimInvestigationDetailModel(data);
		}
	 ...
	 * @example
	 *
	 * `let nhisClaimInvestigationDetailModel = new NhisClaimInvestigationDetailModel(data);`
	 *
	 * @param data The input data to be initialised as the NhisClaimInvestigationDetailModel,
	 *    it is expected as a JSON string or as a nullable NhisClaimInvestigationDetailModel.
	 */
	constructor(data?: string | Partial<NhisClaimInvestigationDetailModel>) {
		super(data);

		if (data) {
			const json = typeof data === 'string'
				? JSON.parse(data) as Partial<NhisClaimInvestigationDetailModel>
				: data;
	...
	getPropDisplayNames(): { [s: string]: ModelProperty } {
		const returned = {};
		NhisClaimInvestigationDetailModel.getProps().map(prop => returned[prop.name] = prop);
		return returned;
	}

	applyUpdates(updates: any): NhisClaimInvestigationDetailModel {
		...
		return new NhisClaimInvestigationDetailModel(newModelJson);
	}
	...
	difference(other: AbstractModel): any {
		if (!(other instanceof NhisClaimInvestigationDetailModel)) {
			return {};
		}
